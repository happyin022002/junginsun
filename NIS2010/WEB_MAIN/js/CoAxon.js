/**
===============================================================================
프로그램  명  : 자바스크립트 파일 동적 로드 처리 관련 공통 함수 정의 Script
프로그램개요  : js파일동적로드,이벤트동적로드, 디버깅함수 등을 정의한다.
작   성   자  : 류현수,이경희
작   성   일  : 2009-11-08
===============================================================================
수정자/수정일 : 
...
*/
    ...
    /**
     * @fileoverview 업무에서 공통으로 사용하는 자바스크립트파일로 자바스크립트 파일 동적 로드 처리 관련 함수가 정의되어 있다.
     * @author 한진해운
     */

    /**
     * @class CoAxon : 자바스크립트 파일 동적 로드 처리 관련 함수가 정의되어 있다.
     */
    function CoAxon() {
        this.ComAssembler  		= ComAssembler;
        this.ComLog        		= ComLog;
        this.ComController 		= ComController;
        this.ComEvent      		= ComEvent;
        this.ComDebug      		= ComDebug;
        this.ComDebugFunction 	= ComDebugFunction;
        this.ComWebKey			= ComWebKey;
    }
    ...
    */
    ...
    function ComAssembler(id)
    {
	    ...
	    //<script language="javascript" src="js/CoAxon.js" path="apps/nis2010/prototype/proposal/jsp/ui_pri_0003.js">
  ...
  ///////////////////////// ComController AREA ///////////////////////////////
        ...
        try
        {
            if (BrowserDetect.browser == "Explorer") {
                for (var i=2; i<arguments.length; i++) {
                    ...
                    //document.getElementById(arguments[i]).attachEvent("on" + eventName, eval(functionName));
                }
            }else {
                for (var i=2; i<arguments.length; i++) {
     ...
     */
     ComEvent.prototype.removeListener = function( id, eventName, functionName )
     {
        ...
        {
            if (BrowserDetect.browser == "Explorer") {
                document.getElementById(id).detachEvent('on' + eventName, eval(functionName));
            }else {
                document.getElementById(id).removeEventListener(eventName, eval(functionName), false);
            }
        }catch(err){
   ...
   ///////////////////////// DEBUG AREA ///////////////////////////////
     var ConSole = {
        ...
        loadDiv : function()
        {
            ...
            var _logger = _FrameDoc.createElement('<div id="AXON_Logger" ALIGN=CENTER onkeypress="if(event.keyCode==27) parent.ComLog.close();" style="border:2px solid #000; padding:2px; overflow:hidden; textOverflow:hidden; background-color:#e4e4e4; width:100%; height:100%"/>');
 ...
 ///////////////////////// DEBUG AREA ///////////////////////////////
     ...
     var BrowserDetect = {
        init: function () {
            ...
            ComLog('You\'re using ' + BrowserDetect.browser + ' ' + BrowserDetect.version + ' on ' + BrowserDetect.OS + '!');
        },
        ...
        try {
        	...
        	if (!oLogWin || oLogWin.closed) {        	    
                var CreateLogWindow = function()
                {
                	...
                	if (oLogWin==null) return ComShowMessage("[ComDebug] window.open()으로 팝업여는데 실패했습니다. Internet Explorer 옵션 설정을 변경하세요!!");
                    ...
                    if(bFirstOpen) {
                    	...
                    	var sScript = "<body onkeypress='if(event.keyCode==27) self.close();' style='margin:0,0,0,0' >"
            ...
            ComShowMessage("ComDebug 창 오류입니다. 디버깅창을 닫고 다시 실행(Ctrl+Shift+X)하세요!!");
    ...
    function ComDebugEvent(objid, fname, args){
        try {    
            ...
            //ComFuncErrMsg(err.message); 
            ComShowMessage("ComDebug 창 오류입니다. 디버깅창을 닫고 다시 실행(Ctrl+Shift+D)하세요!!");
        }
    }
    ...
    if (ComWebKey.JS_PATH != ""){
	    ...
	    for(var i=0; i< s.length-2; i++) {
    		...
    		//<script language="javascript" src="js/CoAxon.js" path="apps/enis/esm/agt/agtaudit/agtaudit/jsp/ESM_AGT_010.js">
    		//시스템공통js파일     : apps/enis/esm/CoEsm.js
    		//서스시스템공통js파일 : apps/enis/esm/agt/CoAgt.js
	        if (i>=2 && i<=3) {
	        	//파일명 = "Co" + 시스템명(첫글자대문자) + ".js"
			    axon_assembler.include(js_path + "Co" + s[i].charAt(0).toUpperCase() + s[i].substr(1));	        	
	        }
	    }
	    js_path += "script/" + s[s.length-1].split(".")[0];
	    
	    //업무자바스크립트 파일을 include 한다.
	    axon_assembler.include(js_path);
    }

...
*/
	...
	*/ 
	function ComIsBtnEnable(name){ 
		var tds = document.getElementsByTagName("td"); 
		for ( var int = 0; int < document.images.length; int++) {
			if(document.images[int].name == name){
				if(document.images[name].isDisabled){
					return false;
				}else{
					break;
				}
		...
		for ( var i = 0; i < tds.length; i++) {
			...
			if (td.className.length > 0 && td.name == name) { 
				if (td.className.indexOf('_1') > 0) 
					return false; 
			}else if(td.className.length > 0 && td.name == "no_"+name){
				return false; 
			}
		}
		return true; 
	...
	*/ 
	function ComColFontName(sheetObj,cols,fontName) {
		...
		for(var i = 0 ; i < colNames.length  ; i++){
			sheetObj.CellFont("FontName", sheetObj.HeaderRows, colNames[i], sheetObj.LastRow, colNames[i]) = fontName;
		}
	}
	...
	*/ 
    function ComSheetRowSwitch(sheetObj, chkCol){
		...
		if(chkrow.length == 2){
			...
			for(var i = 1; i > -1 ; i--){
				sheetObj.CheckAll2(chkCol) = 0;
				sheetObj.CellValue(chkrow[i],chkCol)=1;
				sXml[i] = ComMakeSearchXml(sheetObj, false, chkCol, Cols, true);
			}
			
			sheetObj.LoadSearchXml(sXml[1], true, chkrow[0]);
			sheetObj.LoadSearchXml(sXml[0], true, chkrow[1]);
		}

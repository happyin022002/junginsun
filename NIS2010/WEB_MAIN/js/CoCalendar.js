/**
===============================================================================
프로그램  명  : 달력 관련 공통 함수 정의 Script
프로그램개요  : 달력 팝업 관련 공통 함수를 정의한다.
작   성   자  : 장회수
작   성   일  : 
===============================================================================
수정자/수정일 : 이경희/2008.11.31
수정사유/내역 : DIV형달력처리 및 jsdoc을 위한 주석 처리
===============================================================================
*/
    ...
    /**
     * @class CoCalendar : 달력 관련 각종 함수들을 정의 한다.
     */
    function CoCalendar() {
        this.ComCalendar         = ComCalendar;
        this.ComCalendarGrid     = ComCalendarGrid ;
        this.ComCalendarFromTo   = ComCalendarFromTo;
    }

    /*------------------여기서 부터 공통자바스크립트 함수를 정의한다.     ------------------*/

    //2008-12-11 : 달력은 무조건 DIV로 하기로 했기 때문에 Div의 이름을 확정한다.
    //cal = new ComCalendar(); 이렇게 하면 window.open으로 열수 있고,
    //cal = new ComCalendar("myCal"); 이렇게 하면 div로 열수 있었음.
    var CAL_DIV_NAME = "AlpsCalendar";
    var viframe = null;
    /**
    ...
    function ComCalendar_tmpReturnFunction(y,m,d) {
        if (window.ComCalendar_targetInput!=null) {
            ...
            if (window.ComCalendar_targetInput.tagName=="OBJECT") {
                //Row인자가 -1이면 IBSheet 행 전체의 값을 변경하고, Row값이 특정행의 값이면 해당 행에만 값을 변경한다.
                if (gRow==-1) {
                    // update entire rows
                    window.ComCalendar_targetInput.Redraw = false;
                    for ( var k = window.ComCalendar_targetInput.HeaderRows ; k <= window.ComCalendar_targetInput.LastRow ; k++ ) {
                        ...
                        if ( !window.ComCalendar_targetInput.RowEditable(k)) continue;
                        var oldDate = window.ComCalendar_targetInput.CellValue(k, gCol);
                        if( oldDate.length > 10 )
                        	window.ComCalendar_targetInput.CellValue2(k, gCol) = sVal + oldDate.substring(9,oldDate.length);
                        else
                        	window.ComCalendar_targetInput.CellValue2(k, gCol) = sVal;
                        		
                    }
                    window.ComCalendar_targetInput.Redraw = true;
                } else {
                    // update one row
                    var oldDate = window.ComCalendar_targetInput.CellValue(gRow, gCol);
                    if( oldDate.length > 10 )
                    	window.ComCalendar_targetInput.CellValue2(gRow, gCol) = sVal + oldDate.substring(9,oldDate.length);
                    else
                    	window.ComCalendar_targetInput.CellValue2(gRow, gCol) = sVal;
                }
            } else {
    ...
    **/
    ...
    function ComCalendar_setDisplayType(type) {
        ...
        if (this.displayType!=type && this.calendar_type!="fromto") {
        	var iHeight = 0;
	        switch (type) {
	        	case "year":	iHeight = 118; break;
	        	case "month":	iHeight = 141; break;
	        	case "quarter":	iHeight = 100; break;
	        	default: 		iHeight = 233; break;
	        }
	        this.setSize(this.width, iHeight);
	    }
    ...
    function ComCalendarGrid_select(inputobj, row, col, format) {
        ...
        window.ComCalendar_targetInput = inputobj;
        if (inputobj.CellValue(gRow,gCol)!="") {
            var time = getDateFromFormat(inputobj.CellValue(gRow,gCol),format);
            if (time==0) { this.currentDate=null; }
            else { this.currentDate=new Date(time); }
            }
        else { this.currentDate=null; }
    ...
    function ComCalendarGrid_select2(input_obj, ibsheet_obj, row, col, format)
    {
        ...
        else if (ibsheet_obj.CellValue(row,col)!="") {
            var time = getDateFromFormat(ibsheet_obj.CellValue(row,col),format);
            if (time==0) { this.currentDate=null; }
            else { this.currentDate=new Date(time); }
            }
        else { this.currentDate=null; }
    ...
    function ComCalendar_getStyles() {
        return "<link href=\"/hanjin/css/alps_contents.css\" rel=\"stylesheet\" type=\"text/css\">\n";
    }
    ...
    function ComCalendar_getCalendar() {
        ...
        if (this.type == "WINDOW" || this.use_gebi) {
            result += "<html>\n"
            result += "<head>\n"
            result += "<title>ALPS Calendar!!</title>\n"
            result += "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\">\n"
            result += "<link href=\"/hanjin/css/alps_contents.css\" rel=\"stylesheet\" type=\"text/css\">\n"
            
            if (this.calendar_type=="fromto") {
                result += "<script language=\"JavaScript\" src=\"/hanjin/js/CoCommon.js\"></script>\n";
                result += "<script language=\"JavaScript\" src=\"/hanjin/js/CoMessage.js\"></script>\n";
                result += "<script language=\"javascript\">                     \n";
                result += "  var focused='from';                                \n";
                result += "  function whichField(obj){                          \n";
                result += "    focused = obj;                                   \n";
                result += "  }                                                  \n";
                ...
                result += "      	"+windowref+"document.all.f_cmd.value  = '1';                                   \n";
                result += "    		document.all.from.focus();                   \n";
                result += "     }                                                  \n";
                result += "  }                                                  \n";
                result += "                                                     \n";
                result += "  function returnFromTo(){                                \n";
                result += "    var tmpInterval = ComGetDaysBetween(from,to);                   \n";
                result += "    if(!ComIsDate(from)||!ComIsDate(to)){                              \n";
                ...
                result += "      }                                                                \n";
                result += "    }                                                                  \n";
                result += "    "+windowref+"ComCalendar_tmpReturnFunctionFromTo(from.value, to.value);     \n";
                ...
                result += "</script>                                            \n";
                result += "</head>\n";
                result += "<body  style='overflow:hidden;' onkeypress='if (event.keyCode==27) btn_close.fireEvent(\"onclick\")'> \n";
            ...
            result += "<table width=\"200\"  cellpadding=\"0\" cellspacing=\"0\" style=\"background color:#F4FCFE;border: #8F6BE0 1px solid;\"><tr><td>\n"
            result += "<table width=\"100%\" class=\"popup\" cellpadding=\"4\" border=\"0\"> \n"
            result += "<tr><td style=\"height:5px; background-color: #7474C9;\"></td></tr>\n"
        ...
        if (this.displayType=="date" || this.displayType=="week-end") {
            ...
            var sFromToParam = (this.calendar_type=="fromto")?",from.value,to.value":"";

            result += "		<!-- : ( Search Options ) (S) -->\n"
            result += "		<table class=\"search\" width=\"100%\" style=\"align=\"center\"> \n"
            ...
            result += "							<td width=\"18%\"><img src=\"/hanjin/img/btn_calendar_next.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\""+sRefresh+"("+this.index+","+month+","+(year+1)+sFromToParam+");\"><br>\n"
            result += "							                 <img src=\"/hanjin/img/btn_calendar_back.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\""+sRefresh+"("+this.index+","+month+","+(year-1)+sFromToParam+");\"></td>\n"
            result += "							<td width=\"20%\" style=\"font-size:14px\">"+this.monthAbbreviations[month-1]+"</td>\n"
            result += "							<td width=\"8%\"><img src=\"/hanjin/img/btn_calendar_next.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\""+sRefresh+"("+this.index+","+next_month+","+next_month_year+sFromToParam+");\"><br>\n"
            result += "							               <img src=\"/hanjin/img/btn_calendar_back.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\""+sRefresh+"("+this.index+","+last_month+","+last_month_year+sFromToParam+");\"></td>\n"
            result += "							<td width=\"17%\"></td>\n"
            result += "						</tr>\n"
            result += "					</table>\n"
            result += "					<table class=\"height_2\"><tr><td></td></tr></table>\n"
            result += "					<!-- : ( Calendar ) (E) -->\n"
            result += "					<table width=\"210\" border=\"0\" align=\"center\" class=\"grid2\"> \n"
            result += "						<tr class=\"tr2_head2\" style=\"text-align:center\">\n"
            result += "							<td width=\"30\">Sun</td>\n"
            result += "							<td width=\"30\">Mon</td>\n"
            result += "							<td width=\"30\">Tue</td>\n"
            result += "							<td width=\"30\">Wed</td>\n"
            result += "							<td width=\"30\">Thu</td>\n"
            result += "							<td width=\"30\">Fri</td>\n"
            result += "							<td width=\"30\">Sat</td>\n"
            result += "						</tr>\n"
            for (var row=1; row<=6; row++) {
                ...
                for (var col=1; col<=7; col++) {
                    ...
                    var sDateStyle = "";
                    if(selected_month != month) sDateStyle = "style=\"color:#dfdfdf;\"";
                    ...
                    result += "						<td"+dayClass+">";
                    if (this.calendar_type=="fromto") {
                      result += "<a href=\"javascript:setFromTo('"+selected_year+"','"+selected_month_temp+"','"+selected_date_temp+"');\" " + sDateStyle + ">"+display_date_temp+"</a></td>\n";
                    }else{
                      //if ((display_month == this.currentDate.getMonth()+1) && (display_date==this.currentDate.getDate()) && (display_year==this.currentDate.getFullYear())) {
                      result += "<a href='javascript:"+windowref+this.returnFunction+"("+selected_year+","+selected_month+","+selected_date+");"+windowref+this.endFunction+"();"+windowref+"ComCalendar_hideCalendar(\""+this.index+"\");' " + sDateStyle + ">"+display_date_temp+"</a></td>\n";
                    }
            ...
            result += "					<table class=\"height_2\"><tr><td></td></tr></table>\n"
            if (this.calendar_type=="fromto") {
                result += "					<table border=\"0\" align=\"center\" class=\"grid2\" style=\"width:210; background-color:white;\">\n"
                result += "						<tr >\n"
                result += "						<td width=\"39\" class=\"tr2_head2\">From</td>\n"
                result += "						<td width=\"68\"><input name=\"from\" type=\"text\" class=\"noinput\" style=\"width:67;\" value=\""+from+"\" onFocus=\"this.select(); whichField(this.name)\" readonly></td>\n"
                result += "						<td width=\"25\" class=\"tr2_head\">To</td>\n"
                result += "						<td width=\"68\"><input name=\"to\" type=\"text\" class=\"noinput\" style=\"width:67;\" value=\""+to+"\" onFocus=\"this.select(); whichField(this.name)\" readonly></td>\n"
                result += "						</tr>\n"
                result += "					</table>\n"
            }
            result += "			</td></tr>\n"
        ...
        if (this.displayType=="year") {
			...
			result += '						<tr class=\"h23\">\n';
			result += '							<td width=\"17%\"> </td>\n';
			result += '							<td style=\"font-size:14px\" align=\"center\">\n';
			result += '								<img src=\"/hanjin/img/bu_prev02.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\"'+sRefresh+'('+this.index+',1,'+(year-10)+');\">\n';
			result += '								&nbsp; '+ year +' &nbsp;\n';
			result += '								<img src=\"/hanjin/img/bu_next02.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\"'+sRefresh+'('+this.index+',1,'+(year+10)+');\">\n';
			result += '							</td>\n';
            ...
            for (var row=0; row<2; row++) {
                ...
                for (var col=0; col<5; col++) {
					result += '							<td><a href=\"javascript:'+windowref+this.returnFunction+'('+fromYear+',1,1);'+windowref+this.endFunction+'();'+windowref+'ComCalendar_hideCalendar('+this.index+');\" >'+fromYear+'</A></td>\n';
                    fromYear++;
                }
				result += '						</tr>\n';
            }
        ...
        if (this.displayType=="month") {
			...
			result += '							<td width=\"17%\"> </td>\n';
			result += '							<td style=\"font-size:14px\" align=\"center\">\n';
			result += '								<img src=\"/hanjin/img/bu_prev02.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\"'+sRefresh+'('+this.index+',1,'+(year-1)+');\">\n';
			result += '								&nbsp; '+ year +' &nbsp;\n';
			result += '								<img src=\"/hanjin/img/bu_next02.gif\" align=\"absmiddle\" style=\"cursor:hand\" onclick=\"'+sRefresh+'('+this.index+',1,'+(year+1)+');\">\n';
			result += '							</td>\n';
			result += '							<td width=\"18%\"> </td>\n';
			result += '						</tr>\n';
			result += '					</table>\n';
            ...
            for (var row=0; row<3; row++) {
                ...
                for (var col=0; col<4; col++) {
					result += '							<td><a href=\"javascript:'+windowref+this.returnFunction+'('+year+','+month+',1);'+windowref+this.endFunction+'();'+windowref+'ComCalendar_hideCalendar('+this.index+');\" >'+month.toString().lpad(2, '0')+'</A></td>\n';
                    month++;
                }
				result += '						</tr>\n';
            }
        ...
        // Common
        if (this.type == "WINDOW" || this.use_gebi) {
            result += "<table cellpadding=\"0\" cellspacing=\"0\" style=\"background color:#EFEFF6;\">\n"
            result += "<tr><td height=\"30\" align=\"center\" width=\"258\">\n"
            result += "		<table width=\"100%\" class=\"button\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\"> \n"
            result += "       	<tr><td class=\"btn3_bg\">\n"
            result += "		    <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n"
            result += "		    <tr>\n"
            
            if (this.calendar_type=="fromto" && this.validateForm != "novalidaterform") {
                result += "				<td width=\"72\"><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"button\">\n"
                result += "					<tr id=\"btn_close\" style=\"cursor:hand\" onclick=\"returnFromTo();\">\n"
                result += "                 <td width=\"14\"><img src=\"/hanjin/img/btn_1_left.gif\"></td>\n"
                result += "					<td class=\"btn1\">Apply</td>\n"
                result += "					<td width=\"10\"><img src=\"/hanjin/img/btn_1_right.gif\"></td></tr></table>\n"
                result += "             </td>\n"
            }
//            else{
//            	  result += "			<td width=\"72\"><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"button\">\n"
//                result += "					<tr id=\"btn_close\" style=\"cursor:hand\" onclick=\"returnNoFromTo();\">\n"
//                result += "                 <td width=\"14\"><img src=\"/hanjin/img/btn_1_left.gif\"></td>\n"
//                result += "					<td class=\"btn1\">Apply</td>\n"
//                result += "					<td width=\"10\"><img src=\"/hanjin/img/btn_1_right.gif\"></td></tr></table>\n"
//                result += "             </td>\n"
//            }
            ...
            result += "				<td width=\"72\"><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"button\">\n"
            result += "					<tr id=\"btn_close\" style=\"cursor:hand\" onclick=\"" + windowref + "ComCalendar_hideCalendar("+this.index+");\">\n"
            result += "                 <td width=\"14\"><img src=\"/hanjin/img/btn_1_left.gif\"></td>\n"
            result += "					<td class=\"btn1\">Close</td>\n"
            result += "					<td width=\"10\"><img src=\"/hanjin/img/btn_1_right.gif\"></td></tr></table>\n"
            result += "             </td>\n"
            result += "		    </tr></table>\n"
            result += "		    </td></tr>\n"
    ...
    function ComCalendarWindow_getXYPosition(anchorname) {
        ...
        if (window.ComCalendar_targetInput.tagName=="OBJECT") {
        	...
        	var CellY = 0;
        	var CellWidth = window.ComCalendar_targetInput.ColWidth(gCol);
            if (gRow < window.ComCalendar_targetInput.HeaderRows) {
              CellY = coordinates.y +window.ComCalendar_targetInput.RowTop(window.ComCalendar_targetInput.TopRow);
            } else {
            ...
            if (window.ComCalendar_targetInput.CountPosition!= 0)  this.y += 16;
        }else{
    ...
    function ComCalendarWindow() {
        ...
        if (this.divName != null) {
            if (this.use_gebi) {
                // Iframe은 컬랜더 호출시 한번만 생성한다.2009-08-14
                var fName = this.divName + "IFrame";
                document.getElementById(this.divName).innerHTML="<IFRAME id='"+ fName +"' frameborder=0 style='position:absolute; width:100%; height:100%' /> " ;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class EES_DMT_4107 : EES_DMT_4107 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_DMT_4107() {
             ...
             case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
            		...
            		InitDataProperty(0,		cnt++,	dtData,			150,		daLeft,		false,		"payr_cntc_pnt_fax_no",	false,	"",		dfNone,				0,			false,		false);
            		InitDataProperty(0,		cnt++,	dtHidden,		150,		daLeft,		false,		"payr_cntc_pnt_eml",	false,	"",		dfNone,				0,			false,		false);
        ...
        switch(sAction) {
        	...
        	case IBFAXSEND:
        		...
        		for(var i = 1; i < total_row+1 ; i++) 
        		{
					...
					{
						send_cnt++;
						if(sheetObj.CellValue(i, "payr_cntc_pnt_fax_no") == "")	continue;
						...
						for(var j = i+1 ;  j < total_row+1 ; j++) 
						{
							...
							{
								if(sheetObj.CellValue(j, "payr_cntc_pnt_fax_no") == "")	continue;
								
								if(sheetObj.CellValue(i, "payr_cntc_pnt_fax_no") == sheetObj.CellValue(j, "payr_cntc_pnt_fax_no")){
									dup_check++;
								}
							}
						}
						...
						if(dup_check == 0) 
						{
							payr_faxnos += sheetObj.CellValue(i, "payr_cntc_pnt_fax_no") +";";
						}
					}
				}
...
//        		if (!ComShowCodeConfirm("DMT01092", msg1))	return;
        		...
        		if(ComGetObjValue(formObj.jspno) == "4011" && ComGetObjValue(formObj.jspno) == "4012") {//OTS화면에서 첨부파일 체크 일 경우 팩스전송을 한번 더 호출한다.
        			if(ComGetObjValue(formObj.s_cust_seq) == "Y"){
	        			...
	        			sXml = sheetObj.GetSaveXml("DMTCommonFaxEmailGS.do", FormQueryString(formObj));
        			}
        		}

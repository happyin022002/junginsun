...
/*------------------ 다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ui_dmt_4011 : ui_dmt_4011 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ui_dmt_4011() {
        ...
        try {
            ...
            switch(srcName) {
                ...
                case "btn1_preview": // 4012 OPEN
                    ...
                    var vtInvCnt = isAllCheckVtInv(sheetObjects[0]);
                    // 가Invoice 만 선택했다면 Preview 를 실행하지 않는다.
                    if (vtInvCnt == 1) {
                    	ComShowCodeMessage("DMT01174");
                    	return;
                    } else if (vtInvCnt == 2) {
                    ...
                    for ( var sh3 = 1 ; sh3 < sheetObjects[0].RowCount+3 ; sh3++ ) {
                ...
                case "btn2_sheetset": // 4101 OPEN
                    ComOpenPopupWithTarget('/hanjin/EES_DMT_4101.do?issoff='+formObject.h_usr_off.value+'&tftp2='+sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"tarftp")+'&sheetp=O&jspno=EES_DMT_4011', 725, 770, "", "0,1,1,1,1,1,1", true,true);
                break;
                
                case "btn2_sheetoption": // 4103 OPEN
                    ComOpenPopupWithTarget('/hanjin/EES_DMT_4103.do?issoff='+formObject.h_usr_off.value+'&jspno=EES_DMT_4011', 625, 650, "", "0,1,1,1,1,1,1", true);
                break;
                ...
                case "btn1_detail": // INVOICE NO 로 분기 4004 OR 4002
                	openInvoiceCreationIssueBooking(sheetObjects[0]);
                break;
...
//                    formObject.rd_fxeml_fax_no         .value = ComGetObjValue(formObject.payr_faxnos);
                ...
                case "btn1_down_excel":
                    ...
                    sheetObjects[1].SpeedDown2Excel(-1, false, false, '', '', false, false, '', false,'CheckBox','',false,'',true);
                    //sheetObjects[1].SpeedDown2Excel(-1,false,false,"","/apps/alps/ees/dmt/dmtinvoicemgt/invoiceissuecollectionmgt/script/EES_DMT_4011_XSL.xml");                    
                break;
...
//                        alert("Data2SearchXml 함수의 sheet_obj 인자는 IBSheet가 아닙니다.");
                    ...
                    var rowcount = rdObj[i].RowCount + rdObj[i].headerRows - 3;
...
//                document.form.invno.value = tInvNo;
    ...
    function loadPage() {
    	 ...
    	 var opnSheetObj = opener.document.form.tab_order.value == 0 ? opener.document.form.sheet1 : opener.document.form.sheet2;
        ...
        switch(sheetID) {
            ...
            case "sheet1":      // sheet1 init 인보이스
                with (sheetObj) {
                    ...
                    var HeadTitle    = "||Seq.|INV No.|Virtual Y/N|VVD CD|BKG No.|B/L No.|POR|POL|POD|DEL|Cur.|Billing AMT|TAX AMT|Payable AMT|Collected Status in ERP|Collected Status in ERP|Collected Status in ERP|Uncollected AMT|Type|Issue DT|INV Over Days|Local S/Rep.|SC|RFA|TAA|SHPR Code|SHPR NM|CNEE Code|CNEE NM|INCURRED|CMDT|EXCEPT|AFTER DC|INV RMK|DMDT_INV_TP_CD";
                    var HeadTitle1  = "||Seq.|INV No.|Virtual Y/N|VVD CD|BKG No.|B/L No.|POR|POL|POD|DEL|Cur.|Billing AMT|TAX AMT|Payable AMT|Collected Amount|Collected Amount|Collected Date|Uncollected AMT|Type|Issue DT|INV Over Days|Local S/Rep.|SC|RFA|TAA|SHPR Code|SHPR NM|CNEE Code|CNEE NM|INCURRED|CMDT|EXCEPT|AFTER DC|INV RMK|DMDT_INV_TP_CD";
                    var HeadTitle2  = "||Seq.|INV No.|Virtual Y/N|VVD CD|BKG No.|B/L No.|POR|POL|POD|DEL|Cur.|Billing AMT|TAX AMT|Payable AMT|Charge|Tax|Collected Date|Uncollected AMT|Type|Issue DT|INV Over Days|Local S/Rep.|SC|RFA|TAA|SHPR Code|SHPR NM|CNEE Code|CNEE NM|INCURRED|CMDT|EXCEPT|AFTER DC|INV RMK|DMDT_INV_TP_CD";
                    
                    var headCount = ComCountHeadTitle(HeadTitle2) + 6;
                    ...
                    InitDataProperty( 0   , cnt++ , dtData         , 90    , daCenter  , true     , "invnoo"   			, false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 70    , daCenter  , true     , "dmdt_vt_inv_yn"	, false    , ""         , dfNone        );
                    ...
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "col_charge"   			, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 80   , daRight    , true     , "col_tax"   			, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daCenter  , true     , "col_date"   			, false    , ""         , dfNone );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "uncol_amt"   			, false    , ""         , dfFloat   , 2 );
                    ...
                    InitDataProperty( 0   , cnt++ , dtData         , 90    , daRight   , true     , "invovd"   			, false    , ""         , dfNullInteger );
                    ...
                    InitDataProperty( 0   , cnt++ , dtData         , 120   , daLeft	   , true    , "sh_cust_nm"		, false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 80    , daCenter  , true     , "cn_cust_cd"		, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 120   , daLeft	   , true    , "cn_cust_nm"		, false    , ""         , dfNone 		);
                    ...
                    InitDataProperty( 0   , cnt++ , dtData         , 120   , daLeft	   , true    , "inv_rmk"			, false    , ""         , dfNone 		);
                    ...
                    InitDataProperty( 0   , cnt++ , dtHidden       , 80    , daCenter  , true     , "dmdt_vt_inv_no"	, false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtHidden       , 80    , daCenter  , true     , "dmdt_vt_inv_sts_cd", false    , ""         , dfNone        );
...
//                    CountPosition = 0;
            ...
            case "sheet2":      // sheet1 init 엑셀
                with (sheetObj) {
                    ...
                    var HeadTitle    = "SEQ|Invoice No|Charge|VVD|S/C No.|Rfa No.|Taa No.|B/L No|SHPR|SHPR|CNEE|CNEE|LOC|POL|POD|DEL|CNTR|TS|From DT|To DT|F/T CMNC|F/T End|F/D|Over|CUR|INCURRED|CMDT|EXCEPT|AFTER DC|Net Amount|Tax Amount|Total Amount|Collected Status in ERP|Collected Status in ERP|Collected Status in ERP|Collected Status in ERP|Issue Date|INV Over Day|INV RMK";
                    var HeadTitle1  = "SEQ|Invoice No|Charge|VVD|S/C No.|Rfa No.|Taa No.|B/L No|SHPR|SHPR|CNEE|CNEE|LOC|POL|POD|DEL|CNTR|TS|From DT|To DT|F/T CMNC|F/T End|F/D|Over|CUR|INCURRED|CMDT|EXCEPT|AFTER DC|Net Amount|Tax Amount|Total Amount|Collected Amount|Collected Amount\nTax|Collected Date|Uncollected AMT|Issue Date|INV Over Day|INV RMK";
                    var HeadTitle2  = "SEQ|Invoice No|Charge|VVD|S/C No.|Rfa No.|Taa No.|B/L No|SHPR|SHPR|CNEE|CNEE|LOC|POL|POD|DEL|CNTR|TS|From DT|To DT|F/T CMNC|F/T End|F/D|Over|CUR|INCURRED|CMDT|EXCEPT|AFTER DC|Net Amount|Tax Amount|Total Amount|Collected Amount|Collected Amount\nTax|Collected Date|Uncollected AMT|Issue Date|INV Over Day|INV RMK";
                    ...
                    // 데이터 속성  [ ROW , COL   , DATATYPE       , WIDTH , DATAALIGN , COLMERGE , SAVENAME   , KEYFIELD , CALCULOGIC , DATAFORMAT , POINTCOUNT , UPDATEEDIT , INSERTEDIT , EDITLEN , FULLINPUT , SORTENABLE , TOOLTIP , ALLCHECK , SAVESTATUS , FORMATFIX ]
                    InitDataProperty( 0   , cnt++ , dtSeq          , 30    , daCenter  , true     , "SEQ"                                              );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "invnoo"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "charge"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "vvdcdd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "scnooo"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100    , daCenter  , true     , "rfa_no"		, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 100    , daCenter  , true     , "taa_no"		, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "blnooo"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 80    , daCenter  , true     , "sh_cust_cd"	, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 120   , daLeft	   , true     , "sh_cust_nm"	, false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 80    , daCenter  , true     , "cn_cust_cd"	, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 120   , daLeft	   , true     , "cn_cust_nm"	, false    , ""         , dfNone 		);
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "loccdd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "pol_cd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "pod_cd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "del_cd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "cntrno"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "tyszcd"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "stpdfr"   , false    , ""         , dfNone     );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "stpdto"   , false    , ""         , dfNone     );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "ftcmnc"   , false    , ""         , dfNone     );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "ftcmpl"   , false    , ""         , dfNone     );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daRight   , true     , "freedy"   , false    , ""         , dfNullInteger );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daRight   , true     , "overdy"   , false    , ""         , dfNullInteger );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "currcy"   , false    , ""         , dfNone        );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "org_chg_amt"	, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "cmdt_expt_amt"		, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "sc_rfa_expt_amt"	, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "aft_expt_dc_amt"	, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "netamt"   , false    , ""         , dfNullFloat , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "taxamt"   , false    , ""         , dfNullFloat , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "totamt"   , false    , ""         , dfNullFloat  , 2 );
                    
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "col_charge"   			, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtHidden         , 80   , daRight   , true     , "col_tax"   			, false    , ""         , dfFloat   , 2 );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daCenter   , true     , "col_date"   			, false    , ""         , dfNone );
                    InitDataProperty( 0   , cnt++ , dtData         , 110   , daRight   , true     , "uncol_amt"   			, false    , ""         , dfFloat   , 2 );
                    
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daCenter  , true     , "issudt"   , false    , ""         , dfNone     );
                    InitDataProperty( 0   , cnt++ , dtData         , 100   , daRight   , true     , "oveday"   , false    , ""         , dfNullInteger );
                    InitDataProperty( 0   , cnt++ , dtData         , 200   , daLeft	   , true     , "inv_rmk"		, false    , ""         , dfNone 		);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            ...
            case IBSEARCH:      //조회
                ...
                sheetObj.DoSearch4Post( "EES_DMT_4011GS.do" , FormQueryString(formObj) );
...
// ComOpenWait(false);
            ...
            case IBSEARCH_ASYNC01:      //Remark 조회
                ...
                var sXml = sheetObj.GetSearchXml("EES_DMT_4011GS.do",FormQueryString(formObj));
...
// ComOpenWait(false);
                ...
                var rtnRemark = ComGetEtcData(sXml, "rtnRemark");
                    ...
                    document.form.inter_rmk.value= ComGetEtcData(sXml, "INTER_RMK");
...
// ComOpenWait(false);
            ...
            case IBSEARCH_ASYNC03:      //조회
                ...
                for ( var z01 = 1 ; z01 < sheetObjects[0].RowCount+3 ; z01++  ) {
                    if ( sheetObjects[0].CellValue(z01,1) == 1 ) {
                    	var invNoColNm = sheetObjects[0].CellValue(z01, "dmdt_vt_inv_yn") == "Y" ? "dmdt_vt_inv_no" : "invnoo";

                    	tInvNo = sheetObjects[0].CellValue(z01, invNoColNm) + "," + tInvNo;
                        tCreOf = sheetObjects[0].CellValue(z01,"isseof")    + "," + tCreOf;
                    }
                }
                document.form.invno.value = tInvNo;
...
// ComOpenWait(false);
            ...
            case IBSAVE:        //저장
                ...
                var sXml = sheetObj.GetSaveXml("EES_DMT_4011GS.do", FormQueryString(formObj));
...
//        axon_event.addListener      ( 'mouseout'  , 'obj_mouseout'  , 'td_ch', 'ch', 'rlse_dt');
    ...
    function sheet1_OnSearchEnd( sheetObj , ErrMsg ) {
    	...
    	with (sheetObj) {
            CheckAll(1) = 1;
            //Selected Total 계산될 금액필드를 초기화시켜준다.
            clearSelectedTotalAmt();
            for ( var x = 3 ; x < LastRow + 1 ; x++ ) {
                sheet1_OnClickXTC( sheetObj , x , 0 , 0 );
                
                // Virtual Invoice 일 경우, 폰트색상을 빨간색으로 설정해준다.
                if (CellValue(x, "dmdt_vt_inv_yn") == "Y") {
                	CellFont("FontColor", x, 2, x, 31) = RgbColor(255, 0, 0);
                	CellFont("FontBold" , x, 2, x, 31) = true;
                }
            }

            document.form.vinvqty.value = RowCount;
            document.form.tftp2.value  = sheetObjects[0].CellValue(3, "tarftp");
            document.form.invno.value  = sheetObjects[0].CellValue(3, "invnoo");
            document.form.sheetp.value = sheetObjects[0].CellValue(3, "taa_no");
        }
    	//collected total 
        var t_col_charge = ComReplaceStr(document.form.t_col_charge.value,",");
        var t_col_tax = ComReplaceStr(document.form.t_col_tax.value,",");
        document.form.t_col_total.value = DmtAddComma( ComRound ( eval(t_col_charge) + eval(t_col_tax) ) + "" , "#,###.00");
        //전체 수금 완료여부 
        if(eval(ComReplaceStr(document.form.t_uncol_amt.value,",")) == 0 && eval(ComReplaceStr(document.form.t_col_total.value,"," )) > 0){
        	document.form.t_col_status.value = 'Y';	
        }
    }
    ...
    function sheet1_OnClickXTC( sheetObj , Row , Col , Value ) {
        with (sheetObj) {
            ...
            var charge= ComReplaceStr(document.form.t_col_charge.value,",");
            var tax= ComReplaceStr(document.form.t_col_tax.value,",");
            var uncolAmt= ComReplaceStr(document.form.t_uncol_amt.value,",");
            ...
            if( ComIsNull(charge) ) { charge = 0; }
            if( ComIsNull(tax) ) { tax = 0; }
            if( ComIsNull(uncolAmt) ) { uncolAmt = 0; }
            ...
            if ( vChk == 1 ) { // 선택해제
                document.form.vbilamt.value = DmtAddComma( ComRound ( eval(val01) - eval(CellValue(Row, "bilamt")) ) + "" , "#,###.00");
                document.form.vtaxamt.value = DmtAddComma( ComRound ( eval(val02) - eval(CellValue(Row, "taxamt")) ) + "" , "#,###.00");
                document.form.vpayamt.value = DmtAddComma( ComRound ( eval(val03) - eval(CellValue(Row, "invamt")) ) + "" , "#,###.00");
                ...
                document.form.t_col_charge.value = DmtAddComma( ComRound ( eval(charge) - eval(sheetObjects[0].CellValue(Row, "col_charge")) ) + "" , "#,###.00");
                document.form.t_col_tax.value = DmtAddComma( ComRound ( eval(tax) - eval(sheetObjects[0].CellValue(Row, "col_tax")) ) + "" , "#,###.00");
                
                t_col_charge = ComReplaceStr(document.form.t_col_charge.value,",");
                t_col_tax = ComReplaceStr(document.form.t_col_tax.value,",");
                document.form.t_col_total.value = DmtAddComma( ComRound ( eval(t_col_charge) + eval(t_col_tax) ) + "" , "#,###.00");
//                document.form.t_col_total.value = DmtAddComma( ComRound ( eval(document.form.t_col_charge.value) + eval(document.form.t_col_tax.value) ) + "" , "#,###.00");
                
                document.form.t_uncol_amt.value = DmtAddComma( ComRound ( eval(uncolAmt) - eval(sheetObjects[0].CellValue(Row, "uncol_amt")) ) + "" , "#,###.00");

                CellValue(Row, 1) = 0;
                sheetObjects[2].CellValue(Row, 1) = 0;
            } 
            else { // 선택해제
                document.form.vbilamt.value = DmtAddComma( ComRound ( eval(CellValue(Row, "bilamt")) + eval(val01) ) + "" , "#,###.00");
                document.form.vtaxamt.value = DmtAddComma( ComRound ( eval(CellValue(Row, "taxamt")) + eval(val02) ) + "" , "#,###.00");
                document.form.vpayamt.value = DmtAddComma( ComRound ( eval(CellValue(Row, "invamt")) + eval(val03) ) + "" , "#,###.00");
                ...
                document.form.t_col_charge.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "col_charge")) + eval(charge) ) + "" , "#,###.00");
                document.form.t_col_tax.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "col_tax")) + eval(tax) ) + "" , "#,###.00");
                ...
                document.form.t_uncol_amt.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "uncol_amt")) + eval(uncolAmt) ) + "" , "#,###.00");
        ...
        //날짜 넣기
        var col_date = sheetObjects[0].CellValue(Row, "col_date");        
        if( col_date != ''){
        	if(document.form.t_col_date.value == ''){
        		document.form.t_col_date.value = col_date;
        	}else if(col_date > document.form.t_col_date.value ){
        		document.form.t_col_date.value = col_date;
        	}
        }
    }     
    ...
    function sheet1_OnMouseDown( sheetObj , Button , Shift , X , Y ) {
        with (sheetObj) {
            ...
            if ( MouseRow == 1 && MouseCol == 1 && Button == 1 ) {
                if ( CellText( 0 , 1 ) == 0 ) {
                    ...
                    document.form.t_col_charge.value = "0.00";
                    document.form.t_col_tax.value = "0.00";
                    document.form.t_col_total.value = "0.00";
                    document.form.t_uncol_amt.value = "0.00";
    ...
    function sheet1_OnClick( sheetObj , Row , Col , Value ) {
        with (sheetObj) {
            if ( Row > 0 && Col == 1 ) {
                ...
                if( ComIsNull(vdcamt)    ) { vdcamt = 0;    }
                if ( vChk == 1 ) { // 선택해제
                    document.form.vbilamt.value = DmtAddComma( ComRound ( eval(val01) - eval(sheetObjects[0].CellValue(Row, "bilamt")) ) + "" , "#,###.00");
                    document.form.vtaxamt.value = DmtAddComma( ComRound ( eval(val02) - eval(sheetObjects[0].CellValue(Row, "taxamt")) ) + "" , "#,###.00");
                    document.form.vpayamt.value = DmtAddComma( ComRound ( eval(val03) - eval(sheetObjects[0].CellValue(Row, "invamt")) ) + "" , "#,###.00");
...
//                    document.form.t_col_total.value = DmtAddComma( ComRound ( eval(document.form.t_col_charge.value) + eval(document.form.t_col_tax.value) ) + "" , "#,###.00");
                    ...
                    document.form.vbilamt.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "bilamt")) + eval(val01) ) + "" , "#,###.00");
                    document.form.vtaxamt.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "taxamt")) + eval(val02) ) + "" , "#,###.00"); 
                    document.form.vpayamt.value = DmtAddComma( ComRound ( eval(sheetObjects[0].CellValue(Row, "invamt"))  + eval(val03) ) + "" , "#,###.00");
...
//                    document.form.t_col_total.value = DmtAddComma( ComRound ( eval(document.form.t_col_charge.value) + eval(document.form.t_col_tax.value) ) + "" , "#,###.00");
            ...
            document.form.tftp2.value  = sheetObjects[0].CellValue(Row, "tarftp");
            document.form.invno.value  = sheetObjects[0].CellValue(Row, "invnoo");
            document.form.sheetp.value = sheetObjects[0].CellValue(Row, "taa_no");
    ...
    function sheet1_OnClickXTC2( sheetObj , Row , Col , Value ) {
        with (sheetObj) {
            ...
            if ( vChk == 1 ) { // 선택해제
                document.form.vbilamt.value = DmtAddComma( ( eval(val01) - eval(sheetObjects[0].CellValue(Row, "bilamt")) )+"" , "#,###.00");
                document.form.vtaxamt.value = DmtAddComma( ( eval(val02) - eval(sheetObjects[0].CellValue(Row, "taxamt")) )+"" , "#,###.00");
                document.form.vpayamt.value = DmtAddComma( ( eval(val03) - eval(sheetObjects[0].CellValue(Row, "invamt")) )+"" , "#,###.00");
            } 
            else if ( vChk == 0 ) { // 선택해제
                document.form.vbilamt.value = DmtAddComma( ( eval(sheetObjects[0].CellValue(Row, "bilamt")) + eval(val01) )+"" , "#,###.00");
                document.form.vtaxamt.value = DmtAddComma( ( eval(sheetObjects[0].CellValue(Row, "taxamt")) + eval(val02) )+"" , "#,###.00"); 
                document.form.vpayamt.value = DmtAddComma( ( eval(sheetObjects[0].CellValue(Row, "invamt")) + eval(val03) )+"" , "#,###.00");
            } 
        }
    }
    ...
    function sheet1_OnDblClick(sheetObj, Row, Col, CellX, CellY, CellW, CellH) {
    	...
    	openInvoiceCreationIssueBooking(sheetObj);
    }    
...
//        ComSetObjValue ( formObj.payr_emailnos         , comboObj.GetText( searchIndex , 3 ) );
    ...
    /**
     * 조회결과를 더블클릭하거나, Detail 버튼 클릭시 Invoice Creation&Issue - Booking 화면을 팝업으로 띄워준다.
     */  
    function openInvoiceCreationIssueBooking(sheetObj) {
    	
        var formObj = document.form;
    	
    	var dmdtVtInvYn = sheetObj.CellValue(sheetObj.SelectRow, "dmdt_vt_inv_yn");
    	var dmdtVtInvNo = sheetObj.CellValue(sheetObj.SelectRow, "dmdt_vt_inv_no");
        var tempInvNo   = sheetObj.CellValue(sheetObj.SelectRow, "invnoo");
        var invTpCd     = sheetObj.CellValue(sheetObj.SelectRow, "dmdt_inv_tp_cd");

        if (dmdtVtInvYn == "Y") {
        	tempInvNo = dmdtVtInvNo;
        }
        ...
        if (invTpCd == "M") {
            var url = "EES_DMT_4004P.do"
                ...
                +"&caller=4011"
                +"&ctrt_flg="+ ComGetObjValue(formObj.ctrt_flg) 
                ;
        	...
        	var ofcCd     		= sheetObj.CellValue(sheetObj.SelectRow, "isseof");
        	var bkgNo     		= sheetObj.CellValue(sheetObj.SelectRow, "bkgnoo");
        	var dmdtTrfCd 		= sheetObj.CellValue(sheetObj.SelectRow, "tarftp");
        	var dmdtVtInvStsCd 	= sheetObj.CellValue(sheetObj.SelectRow, "dmdt_vt_inv_sts_cd");
        	...
        	doActionIBSheet(sheetObj, 		 document.form, IBSEARCH);						// 조회
            doActionIBSheet(sheetObj,        document.form, IBSEARCH_ASYNC01);				// Remark 조회
            doActionIBCombo(sheetObjects[3], document.form, COMMAND02, COMMAND02, "", "");	// Payer별 Email, FAX 번호 조회
        }    	
    }
    
    /*
     * 팝업 메뉴 종료시 자동조회를 수행하면서 금액을 재계산하는데, 
     * 이때 중복금액 계산을 막기 위해서 계산전에 금액 필드를 공백으로 초기화 시켜준다.
     */
    function clearSelectedTotalAmt() {
    	var formObj = document.form;
    	
    	ComSetObjValue(formObj.vbilamt,   "");
    	ComSetObjValue(formObj.vtaxamt,   "");
    	ComSetObjValue(formObj.vpayamt,   "");
    	ComSetObjValue(formObj.vincamt,   "");
    	ComSetObjValue(formObj.vcmdtamt,  "");
    	ComSetObjValue(formObj.vexcepamt, "");
    	ComSetObjValue(formObj.vdcamt,    "");
    }  
    
    /*
     * 선택한 항목이 모두 Virtual Invoice 인지 여부를 알려주는 함수
     */    
    function isAllCheckVtInv(sheetObj) {
    	
    	var strCheckedRow = sheetObj.FindCheckedRow("CheckBox");

    	var index = strCheckedRow.lastIndexOf("|");
    	if (index > 0) {
    		strCheckedRow = strCheckedRow.substr(0, index);
    	}

    	var arrChkRows = strCheckedRow.split("|");
    	
    	var nVtInvCnt = 0;
 		for(var i=0; i < arrChkRows.length; i++) {
 			var dmdtVtInvYn = sheetObj.CellValue(arrChkRows[i], "dmdt_vt_inv_yn");
 			if (dmdtVtInvYn == 'Y') {
 				nVtInvCnt++;
 			}
 		}
 		...
 		if ( nVtInvCnt == 0 ){
 			return 0;
 		} else if ( arrChkRows.length == nVtInvCnt ){
 			return 1;
 		} else {
 			return 2;
 		}

// 		return arrChkRows.length == nVtInvCnt;
    }
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class EES_DMT_7019 : EES_DMT_7019 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_DMT_7019() {
 ...
 // 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
	...
	function loadPage() {
		...
		for (var i = 0; i < uploadObjects.length; i++) {
			//1. 기본 환경 설정
			ComConfigUpload(uploadObjects[i], "/hanjin/EES_DMT_7019GS.do");
		}
	...
	function initSheet(sheetObj, sheetNo) {
		...
		with (sheetObj) {
			...
			var HeadTitle = "|Sel.|File Name|File Size|Upload date";
     ...
     function doActionIBSheet(sheetObj, formObj, sAction) {
         ...
         switch(sAction) {
	         ...
	         case IBSEARCH:
	 			...
	 			// 2.조회조건으로 조회실행
	 			var sXml = sheetObj.GetSearchXml("EES_DMT_7019GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(PREFIX));

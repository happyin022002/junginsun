 ...
 * 2012.06.19 신자영 [CHM-201218336-01] INVENTORY / M/B 기능 수정 요청
	...
	try {
		...
		case "btn_loc_cd": //Location 조회 팝업
			...
			if (formObject.locationBy.value != 'AR' && formObject.locationBy.value != 'AC') {
				...
				} else if (formObject.locationBy.value == 'YY') { //Yard
					var param = "?cnt_cd=" + cnt_cd + "&loc_cd=" + loc_cd;
		...
		case "btn_downexcel":
			if (tabIndex == 0) {
				...
				t2sheet1.Down2Excel(-1, false, false, true, "",
						"/apps/alps/ees/cim/cntroperationperformancemgt/eqmatchbacknloadfactormgt/script/EES_CIM_1025_XLS.xml");
			} else if (tabIndex == 2) {
				//t3sheet1.Down2Excel(-1, false, false, true);
				t3sheet1.Down2Excel(-1, false, false, true, "",
				        "/apps/alps/ees/cim/cntroperationperformancemgt/eqmatchbacknloadfactormgt/script/EES_CIM_1025_XLS2.xml");
			}
			break;
		...
		case "btn_Print":
			...
			ComOpenPopupWithTarget('/hanjin/EES_CIM_1918.do', 775, 770, "", "0,1,1,1,1,1,1", true);
			break;
...
function initSheet(sheetObj, sheetNo, HeadTitle) {
	...
	case "t2sheet1": //t2sheet1 init
		with (sheetObj) {
			...
			//                ComDebug("HeadTitle [" + HeadTitle + "]");
			var headCnt = HeadTitle.split("|").length;
			//                ComDebug("headCnt [" + headCnt + "]");
			SheetWidth = (headCnt - 4) * 60 + 20;
			if (SheetWidth > 975) {
				SheetWidth = 975;
			}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sXml = sheetObj.GetSearchXml("EES_CIM_1018GS.do", FormQueryString(formObj));
...
function doActionIBSheet2(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			ComOpenWait(true);
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("EES_CIM_10182GS.do", FormQueryString(formObj));
...
function doActionIBSheet3(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			ComOpenWait(true);
			formObj.f_cmd.value = SEARCH02;
			var sXml = sheetObj.GetSearchXml("EES_CIM_10183GS.do", FormQueryString(formObj));
		...
		if (sAction != IBSEARCH_ASYNC01) {
			...
			if (location.value == "" && sAction == IBSEARCH) {
				if (locationBy.value == 'RL' || locationBy.value == 'LE' || locationBy.value == 'LS' || locationBy.value == 'ES' || locationBy.value == 'SS'
						|| locationBy.value == "CC" || locationBy.value == "RE" || locationBy.value == "YY") {
...
*/
		...
		//document.form.rpt_tpsz      .value = document.form.tpsz      .value;
		...
		document.form.rpt_tnroute.value = document.getElementById('tnRoute').options[document.getElementById('tnRoute').options.selectedIndex].text;
		document.form.rpt_soc.value = document.getElementById('soc').options[document.getElementById('soc').options.selectedIndex].text;
		document.form.rpt_company.value = document.form.company.value;

...
/**
 * @extends 
 * @class ees_cim_1023 : ees_cim_1023 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ees_cim_1023() {
	...
	try {
		...
		case "btn_loc_cd": //Location 조회 팝업
			...
			if (formObject.inquiryLevel.value != 'A') {
				if (formObject.inquiryLevel.value == 'Y') { //Country
					...
					ComOpenPopup("/hanjin/COM_ENS_061.do", 755, 630, "popupFinish", "1,0,1,1,1,1,1,1", true);
				} else {
		...
		case "btn_DownExcel":
			if (tabIndex == 0) {
				...
				doActionIBSheet3(t3sheet1, formObject, IBDOWNEXCEL);
			}
			break;
		...
		case "btn_Print":
			...
			ComOpenPopupWithTarget('/hanjin/EES_CIM_1923.do', 775, 750, "", "0,1,1,1,1,1,1", true);
			break;
...
function initSheet(sheetObj, sheetNo, HeadTitle) {
	...
	case "t1sheet1": //t1sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 60, daCenterTop, true, "yard_cd", false, "", dfNone);
	...
	case "t2sheet1": //t2sheet1 init
		with (sheetObj) {
			...
			for ( var RowCnt = 0; RowCnt < 4; RowCnt++) {
				...
				//데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
				if (RowCnt == 3) {
					InitDataProperty(RowCnt, cnt++, dtData, 60, daCenterTop, true, "loc_cd", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtData, 70, daCenterTop, true, "yard_cd", false, "", dfNone);
					...
					InitDataProperty(RowCnt, cnt++, dtData, 70, daCenterTop, true, "yard_cd", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtData, 60, daCenter, true, "division", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtAutoSum, 60, daRight, false, "total", false, "", dfInteger, 1);
	...
	case "t3sheet1": //t3sheet1 init
		with (sheetObj) {
			...
			//행정보설정[필수][HEADROWS,DATAROWS,VIEWROWS,ONEPAGEROWS=100]
			//DATAROWS에 따라 초기 SUM row가 변하니 유의할 것
			...
			if (HeadTitle == null || HeadTitle == "") {
				HeadTitle = "PORT|YARD|TP/SZ|Division|Total";
				InitColumnInfo(5, 0, 0, true);
			}
			var headCount = HeadTitle.split("|").length;
			SheetWidth = (headCount - 5) * 70 + 340;
			if (SheetWidth > 975) {
				SheetWidth = 975;
			}
...
/*
			...
			for ( var RowCnt = 0; RowCnt < 4; RowCnt++) {
				...
				if (RowCnt == 3) {
					InitDataProperty(RowCnt, cnt++, dtData, 60, daCenterTop, true, "loc_cd", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtData, 70, daCenterTop, true, "yard_cd", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtData, 60, daCenter, true, "vvd", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtData, 60, daCenter, true, "division", false, "", dfNone);
					InitDataProperty(RowCnt, cnt++, dtAutoSum, 60, daRight, false, "total", false, "", dfNone, 1);

					for ( var i = 0; i < headCount - 5; i++) {
						arEtcData = "_" + i;
						InitDataProperty(RowCnt, cnt++, dtAutoSum, 60, daRight, false, "qty" + arEtcData, false, "", dfNone, 1);
					}
				} else {
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		...
		var sXml = sheetObj.GetSearchXml("EES_CIM_1023GS.do", FormQueryString(formObj));
...
function doActionIBSheet2(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		...
		var sXml = sheetObj.GetSearchXml("EES_CIM_10232GS.do", FormQueryString(formObj));
		sheetObj.LoadSearchXml(sXml);
		sheetObj.WaitImageVisible = true;
		ComOpenWait(false);
		break;
...
function doActionIBSheet3(sheetObj, formObj, sAction) {
	...
	if (validateForm(sheetObj, formObj, sAction)) {
		...
		var sXml = sheetObj.GetSearchXml("EES_CIM_10233GS.do", FormQueryString(formObj));
		sPeriod = ComGetEtcData(sXml, "head");


		var HeadTitle = "PORT|YARD|TP/SZ|Division|Total|" + sPeriod;
		sheetObj.Reset();
...
function t1sheet1_OnChangeSum(sheetObj) {
	...
	sheetObj.ColHidden("division") = true;
...
function t3sheet1_OnChangeSum(sheetObj, Row) {
	with (sheetObj) {
		...
		SumText(0, "yard_cd") = "G.Total";
		SumText(1, "yard_cd") = "G.Total";
		SumText(2, "yard_cd") = "G.Total";
		SumText(3, "yard_cd") = "G.Total";
...
function t3sheet1_OnSearchEnd(sheetObj, ErrMsg) {
	with (sheetObj) {
		...
		CellAlign(LastRow - 3, "yard_cd") = daCenterTop;
		...
		//G.Total Title SET		
		SetMergeCell((sheetObj.LastRow - 3), 0, 4, 3);
...
*/
		...
		document.form.rpt_company.value = "SML";
		...
		CellAlign(LastRow, "loc_cd") = daCenterTop;
		CellAlign(LastRow - 3, "yard_cd") = daCenterTop;
		CellAlign(LastRow - 2, "yard_cd") = daCenterTop;
		CellAlign(LastRow - 1, "yard_cd") = daCenterTop;
		CellAlign(LastRow, "yard_cd") = daCenterTop;
		CellAlign(LastRow - 3, "division") = daCenter;
		CellAlign(LastRow - 2, "division") = daCenter;
		CellAlign(LastRow - 1, "division") = daCenter;
		CellAlign(LastRow, "division") = daCenter;

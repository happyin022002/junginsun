...
/**
 * @extends 
 * @class ees_cim_1020 : ees_cim_1020 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ees_cim_1020() {
...
function loadPage() {
	...
	axon_event.addListener('focus', 'yardnextfocus', 'yardnext');
...
function sheet1_OnLoadFinish(sheetObj) {
	...
	comboObjects[2].Enable = false;
	comboObjects[3].DisabledBackColor = "#FFFFFF";
	comboObjects[3].Enable = false;
	document.getElementById("froms").focus();
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //t1sheet1 init
		with (sheetObj) {
			...
			InitColumnInfo(arrCntrTypeSize.length + 4, 0, 0, true);
			...
			InitDataProperty(RowCnt, cnt++, dtData, 60, daCenterTop, true, "lane_cd", false, "", dfNone);
			InitDataProperty(RowCnt, cnt++, dtData, 80, daCenterTop, true, "vvd", false, "", dfNone);
			InitDataProperty(RowCnt, cnt++, dtData, 60, daCenter, false, "division", false, "", dfNone);
			InitDataProperty(RowCnt, cnt++, dtAutoSum, 60, daRight, false, "total", false, "", dfNone);

			for ( var i = 0; i < arrCntrTypeSize.length; i++) {
				arEtcData = "_" + i;
				InitDataProperty(RowCnt, cnt++, dtAutoSum, 60, daRight, false, "qty" + arEtcData, false, "", dfNone);
			}
//			InitUserFormat(RowCnt, "total", "#,###", ",") ;
...
//			InitUserFormat(RowCnt, "total", "#,###",",") ;
			...
			InitDataProperty(RowCnt, cnt++, dtData, 80, daCenterTop, true, "vvd", false, "", dfNone);
			InitDataProperty(RowCnt, cnt++, dtData, 60, daCenter, false, "division", false, "", dfNone);
			InitDataProperty(RowCnt, cnt++, dtData, 60, daRight, false, "total", false, "", dfNone);

			for ( var i = 0; i < arrCntrTypeSize.length; i++) {
				arEtcData = "_" + i;
				InitDataProperty(RowCnt, cnt++, dtData, 60, daRight, false, "qty" + arEtcData, false, "", dfNone);
			}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		...
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sXml = sheetObj.GetSearchXml("EES_CIM_1020GS.do", FormQueryString(formObj));
	...
	case IBSEARCH_ASYNC04: //Yard COMBO 조회
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sYard = ComGetEtcData(sXml, "sLane");
			if (sYard != undefined) {
				var arrYard = sYard.split(",");
				MakeComboObject(formObj.yard, arrYard);
...
/*
		SumText(1, "lane_cd") = "G.Total";

		SumText(2, "lane_cd") = "G.Total";
		SumText(3, "lane_cd") = "G.Total";
...
*/		SumText(0, "division") = "I/B";
		...
		CellAlign(LastRow - 3, "lane_cd") = daCenter;
		CellAlign(LastRow - 2, "lane_cd") = daCenterTop;
		CellAlign(LastRow - 1, "lane_cd") = daCenterTop;
		CellAlign(LastRow, "lane_cd") = daCenterTop;
		CellAlign(LastRow - 3, "vvd") = daCenter;
 ...
 * @return
 */
function yard_OnBlur(KeyCode, Shift) {
	flag = false;
...
function yard_OnFocus(comboObj) {

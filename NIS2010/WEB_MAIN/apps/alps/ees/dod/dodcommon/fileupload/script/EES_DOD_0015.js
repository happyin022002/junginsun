...
/**
 * [EES_DOD_0015] File Upload
 * @extends
 * @class File Upload 생성을 위한 화면에서 사용하는 업무 스크립트를
 *        정의한다.
 */
function EES_DOD_0015() {
...
function loadPage() {
    ...
    for(var i=0; i < uploadObjects.length ; i++){
	    //1. 기본 환경 설정
	    ComConfigUpload(uploadObjects[i], "/hanjin/EES_DOD_0015GS.do");	
	}    
...
function initSheet(sheetObj, sheetNo) {
	...
	with (sheetObj) {
		...
		case "sheet1": 
			...
			var HeadTitle1 = "|Sel.|Seq.|File Name|ID|Date|Download|atch_file_lnk_id|atch_file_lnk_seq|file_path_url|file_sav_id";
...
function processButtonClick() {
	...
	var openSheetObj;
	var prefix = document.form.tab_gubun.value;
	if(prefix == "sheet1_"){
		openSheetObj = opener.document.t1sheet1;
	}else if(prefix == "sheet2_"){
		openSheetObj = opener.document.t2sheet1;
	}else{
		openSheetObj = opener.document.sheet1;
	}
	...
	switch (srcName) {	
        ...
        case "btn2_Save":   
    		...
    		openSheetObj.CellValue2(document.form.row.value, "atch_file_lnk_cnt") = sheetObjects[0].RowCount("R") ; 
    		openSheetObj.CellValue2(document.form.row.value, "atch_file_lnk_id") = sheetObjects[0].CellValue(1, "atch_file_lnk_id") ;
    		
            break;            
		case "btn1_Close":
    		openSheetObj.CellValue2(document.form.row.value, prefix + "atch_file_lnk_cnt") = sheetObjects[0].RowCount("R") ;			
			self.close();
            break; 
	}
}
...
/*	var fileName = sheet.OpenFileDialog(ComGetMsg("DOD00026","file."));	
...
function doActionIBSheet(sheetObj, formObj, sAction) {	
	 ...
	 switch (sAction) {
	 	...
	 	case IBSEARCH: //조회
			...
			var sXml = sheetObj.GetSearchXml("EES_DOD_0015GS.do", FormQueryString(frm));
	 	...
	 	case IBSAVE: 
	 		...
	 		// atch_file_lnk_id 증가 확인
			if(sheetObj.RowCount > 1 && sheetObj.RowCount("R") > 0) {
				// 최초 첨부하는 것이 아니면 기존 atch_file_lnk를 유지하기 위해 이전 첨부된 atch_file_lnk_id를 재사용하도록
				var iRow = sheetObj.FindStatusRow("I");		
		 		var iRows = iRow.split(";");
			
		 		for(var i = 0 ; i < iRows.length - 1 ; i++) {	
		 			sheetObj.CellValue2(iRows[i], "atch_file_lnk_id") = sheetObj.CellValue(1 , "atch_file_lnk_id");
		 		}
			}
	 		...
	 		if (!ComIsNull(upload1.LocalFiles)) {			
	 			...
	 			var atchFileLnkId = ComGetEtcData( sXml, "ATCH_FILE_LNK_ID");
	 			if (errorYn == "N") {
	 				document.form.atch_file_lnk_id.value = atchFileLnkId;
	 				doActionIBSheet(sheetObjects[0], document.form, IBSEARCH);
	 			}			
	 		// 일반
	 		} else {			
	 			param = FormQueryString(frm) + "&" + sheetObj.GetSaveString();			
	 			var sXml = sheetObj.GetSaveXml("EES_DOD_0015GS.do",  param);			
	 			sheetObj.LoadSaveXml(sXml);			
	 			var errorYn = ComGetEtcData( sXml, "ERROR_YN");
	 			if (errorYn == "N") {
	 				doActionIBSheet(sheetObjects[0], document.form, IBSEARCH);

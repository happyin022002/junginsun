	...
	*/
	function loadPage() {
		...
		for(var k=0;k<comboObjects.length;k++){
 		...
 		var cust_flg = window.dialogArguments['cust_flg'];
 		var cust_type = window.dialogArguments['cust_type'];
 		...
 		formObj.cust_flg.value = cust_flg;
 		formObj.cust_type.value = cust_type;
...
//		axon_event.addListenerForm  ( 'change' , 'obj_change' , document.form );
			...
			case "sheet1":	  //IBSheet1 init
				with (sheetObj) {
				   ...
				   //전체Edit 허용 여부 [선택, Default false]
					...
					var HeadTitle = "|Seq.|Office|Customer Code|Customer Name|Contract Office|S/C No.|RFA No.|CNTR No.|TP/SZ|TRO RTN Yard|Actual RTN Yard|RTN Date|TRO Date|BKG No.|POR|POL|POD|DEL|Invoice Cust|Invoice Cust|Cur.|General Tariff|Special Tariff|Adjust AMT|Invoice AMT|Invoice AMT(USD)|Pending AMT|Exemption|INV Office|I/F Date|I/F User";
					...
					InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"cust_cd",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		200,	daLeft,		false,	"cust_nm",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"contract_ofc",		false,          "",       dfNone,    0,    false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60,		daCenter,	false,	"tpsz_cd",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"rtn_yd",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"a_rtn_yd",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		70,		daCenter,	false,	"rtn_dt",			false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		70,		daCenter,	false,	"tro_dt",			false,          "",       dfNone,    0,    false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		80,	    daCenter,	false,	"inv_cust_cd",		false,          "",       dfNone,    0,    false,       true);
					InitDataProperty(0, cnt++, dtData,		200,	daLeft,	    false,	"inv_cust_nm",		false,          "",       dfNone,    0,    false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		100,	daRight,	false,	"adj_amt",			false,          "",       dfFloat,	 2,    false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daRight,	false,	"inv_amt",			false,          "",       dfFloat,   2,    false,       true);
					InitDataProperty(0, cnt++, dtData,		120,	daRight,	false,	"inv_usd_amt",		false,          "",       dfFloat,   2,    false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daRight,	false,	"pen_amt",			false,          "",       dfFloat,   2,    false,       true);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
		   case IBSEARCH:		//조회
				...
				//sheetObj.DoSearch4Post("EES_DOD_0017GS.do", FormQueryString(formObj));
	    		sheetObj.WaitImageVisible=false;
    			ComOpenWait(true);
				formObj.f_cmd.value = SEARCH;
				
    			var sXml = sheetObj.GetSearchXml("EES_DOD_0017GS.do",FormQueryString(formObj));
	...
	function getBackEndJobStatus() {
		...
		var sXml = sheetNum.GetSearchXml("EES_DOD_0017GS.do", FormQueryString(formObj));
	...
	function getBackEndJobLoadFile() {
		...
		ComOpenWait(false);
		var sXml = sheetNum.GetSearchXml("EES_DOD_0017GS.do", FormQueryString(form));

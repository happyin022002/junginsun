...
/**
 * @extends DOD
 * @class EES_DOD_MULTI : 예)Booking 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_DOD_MULTI() {
...
function comPopupOK(sheetObj,formObject) {
	...
	if(errcnt > 0){
		...
		opener.getDOD_Return(rArray,return_val);  //호출하는 부모js에 getDOD_Return펑션을 붙여넣으면됩니다.
		window.close();
	}	
}

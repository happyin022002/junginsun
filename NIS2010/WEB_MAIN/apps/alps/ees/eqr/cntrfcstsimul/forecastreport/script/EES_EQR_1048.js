...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    /**
    * @extends 
    * @class EES_EQR_1048 : EES_EQR_1048 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
    */
    function EES_EQR_1048() {
        ...
        try {
            ...
            switch(srcName) {
            	case "btn_RowAdd":
            		...
            		sheetObjects[1].InitCellProperty(1, "to_yd_cd", dtCombo); // CELL TYPE을 DTDATA으로 변경
            		sheetObjects[1].InitCellProperty(1, "to_etb_dt", dtCombo); // CELL TYPE을 DTDATA으로 변경
            		sheetObjects[1].CellComboItem(1, "to_yd_cd", " |"+to_yd_cd, " |"+to_yd_cd);  // " |" 은 첫째줄을 비우기 위한 조치임
            		sheetObjects[1].CellComboItem(1, "to_etb_dt", " |"+to_etb_dt, " |"+to_etb_dt);  
...
//	                InitDataCombo(0, "to_etb_dt", " |"+to_etb_dt, " |"+to_etb_dt, " ","");	            	
            		...
            		for ( var j=6; j<headCount2-1; j++ ) {
            			if(sheetObjects[1].ColSaveName(j).substring(3) == "fcast_qty"){
            				...
            				sheetObjects[1].CellValue2(sheetObjects[1].SelectRow, j) = 0;
            			}
            		}            		            
            		
            		sheetObjects[1].CellValue2(sheetObjects[1].SelectRow, "cre_seq") = 1;
...
//        	
//        	sheetObjects[1].Editable = false; // 2번째 쉬트 잠금
//        }else if (level_cd == "2") {
//        	ComBtnDisable("btn_save");    // SAVE 버튼 잠금
//        	
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            ...
            case IBSEARCH:      //조회
                ...
                var sXml = sheetObj.GetSearchXml("EES_EQR_1048GS.do",FormQueryString(formObj));
    		...
    		case IBSEARCH02:      //공통조회
    			...
    			var sXml = sheetObj.GetSearchXml("EES_EQR_1048GS.do" , FormQueryString(form));
            ...
            case IBDOWNEXCEL:      // 입력
            	if(formObj.curr_flag.value == "T"){	
            		sheetObjects[0].Down2Excel(0,false,true,true,'','',false,false,'',false,'lvl');
            		sheetObj.Down2Excel(0,true,true,true,'','',false,false,'',false,'div|lvl|cre_seq|rpt_seq|to_etb_dt_org|ibflag');
            	}else{
    		 ...
    		 switch (ColSaveName(Col)) {
    		 	 ...
    		 	 case "vvd":    		 	
    		 		if(Value.length != 9){
		                ...
		                var sXml = GetSearchXml("EES_EQR_1048GS.do",FormQueryString(formObj)+"&vvd="+Value);
    	...
    	var formObj = document.form;
  		...
  		if ( ErrMsg == "" ) {
  			if ( save_flag ) {   				
  				...
  				for ( var j=6; j<=23; j++ ) { // A5		
  				    if ( sheetObjects[1].rowCount > 0 ) { 		    			
  				    	opener_obj.setRepoInValue(week_seq, sheet_row, sheetObjects[1].ColSaveName(j).substr(0,2), sheetObjects[1].CellValue(sheetObjects[1].LastRow, sheetObjects[1].ColSaveName(j)));
  				    } else { //sheet_num
			...
			case IBSAVE:
				for(var i = 1; i <= LastRow; i++){
					...
					if(RowStatus(i) == "I" || RowStatus(i) == "U"  ) {
						...
						for ( var j=6; j<=23; j++ ) { // A5								
							qty_vol += sheetObjects[1].CellValue(i, sheetObjects[1].ColSaveName(j));
						}
						if(qty_vol==0) {

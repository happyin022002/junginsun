...
msgs["CTM10047"] = "Do you want to delete [value] ?";
...
msgs["CTM10055"] = "Unmatched yard code between VVD Terminal and Origin Yard for container VL/VD";
...
msgs["CTM20078"] = "This VVD Code has same POL / POD LCC.";

//msgs["CTM20080"] = "Container can't be updated. Please refer to the reason in remark.";
msgs["CTM20080"] = "Container can't be updated. Please refer to the reason in ERR Message.";
...
msgs["CTM20120"] = "Please check period. The maximum period is 3 months";
msgs["CTM20121"] = "Please check period. The maximum period is 1 months";
msgs["CTM20122"] = "Deleting ID/XX is allowed in its own, not with other movement together";
...
msgs["CTM30013"] = "{?msg1} has no EDI connection!!";    // [EES_CTM_0426] Release/Redelivery Order

msgs["CTM30014"] = "Do you want to print content?";    // [EES_CTM_0451] Release/Redelivery Order RD

msgs["CTM30015"] = "Inquiry data exceeds maximum 300 containers.";   // [EES_CTM_0464] Multi Cntr No input

msgs["CTM30016"] = "Movement Creation Error [Auto Creation][{?msg1}]"; // [EES_CTM_0406] Domestic Container 처리
msgs["CTM30017"] = "Retry movement update after retrieving the latest data"
...
function parseCTNRNo(CTNRNO, formObj) {
    ...
    if (CtnrVal.length >= 3) {
        ...
        if (formObj.cntr_rsk_flg) {
        	formObj.cntr_rsk_flg.value = CtnrVal[7];
        	//TYPE SIZE가 R로 시작하면 cntr_rsk_flg 값이 'Y' 면 'Y', 'N' 또는 null 이면 'N'으로 표시
        	//그외 사항은 null로 표시
        	if (formObj.ctnr_tpsz_cd.value.substring(0,1) == "R"){
        		if (formObj.cntr_rsk_flg.value == ""){
        			formObj.cntr_rsk_flg.value = "N";
        		} else {
        			formObj.cntr_rsk_flg.value = CtnrVal[7];
        		}
        	} else {
        		formObj.cntr_rsk_flg.value = "";
        	}
        }
    } else {
        if (formObj.check_digit) {
...
function body_onkeydown(keyCode) {
    ...
    if (obj==null) {
        ...
        var td=tds[i];
        if (td.name == "btn_retrieve" || td.name == "btn_Retrieve") {
            td.fireEvent('onclick');
            break;
        }
        }
    } else {
...
function parseYardMultiCombo(YardCode, comboObj) {
    ...
    comboObj.ValidChar(2, 1);    // 영대문자 + 숫자만 입력
    ...
    try {
        if (BrowserDetect.browser == "Explorer") {
            ...
            for (var i=2; i<arguments.length; i++) {
                //document.getElementById(arguments[i]).addEventListener(eventName, eval(functionName), false);
            }
        }
    } catch(err) {
    ...
    try {
        ...
        if (rtn) {
            for (i = 0; i < eventSaction.length; i++) {
                ...
                if (srcName == event.srcElement.name) {
                    if (srcFunc.length > 0)
                        eval(srcFunc + "()");
                }
            }
...
function yard_onkeypress() {
    ...
    if (p_yard.length == 5) {
        ...
        parseYardMultiCombo(rtnValue, document.getElementById("p_yard2"));
...
function cntr_onkeypress() {
    ...
    var sheetObj = sheetObjects[0];
    xml = sheetObj.GetSearchXml ("CTMCommonGS.do", "f_cmd=" + SEARCH10 + "&p_cntrno=" + cntrno);
    rtnValue = ComGetEtcData(xml, "rtnValue");
...
function setEventProcess(eventObj) {
    ...
    for (i = 0 ; i <= len ; i++) {
        ...
        if(elementName == "sheet") {
            formElement.attachEvent("onblur", eval("sheet_blur"));
        } else if(elementName == "sheet1") {
            formElement.attachEvent("onblur", eval("sheet1_blur"));
        } else if(elementName == "sheet2") {
            formElement.attachEvent("onblur", eval("sheet2_blur"));
            ...
            formElement.attachEvent("onkeypress", eval("obj_keypress"));
            formElement.attachEvent("onkeyup", eval("obj_keyup"));
            formElement.attachEvent("onkeydown", eval("obj_keydown"));
            formElement.attachEvent("onfocus", eval("date_focus"));
            formElement.attachEvent("onblur", eval("obj_blur"));
        } else {
            /** 이벤트를 설정하기 위하여 Element List를 생성한다. */
            /** 이벤트는 KeyPress 와 KeyUp만 사용하도록 지정한다. */
            formElement.attachEvent("onkeypress", eval("obj_keypress"));
     ...
     if (event.keyCode == "9") {
        ...
        try {
            ...
            if (elementName == "p_cntrno") {
                if (cntr_search()) {
//                	alert("keydown 0430	-  true");
                    return;
                } else {
                    tmp = setFocusIndex (formElement, 0);
                    formElement.select();
                    formElement.focus();
//                    alert("keydown 0430	-  false");
                    return false;
                }
...
//                	alert("55555555555555	p_cntrno");
            ...
            if (flgNext) {
                ...
                if (curKeyCode == "9") {
                    ...
                    curKeyCode = "";
                    try {
                        //obj = getElementById(curEventElement);
                        //document.form.p_vvdcd.value = document.form.p_vvdcd.value + ":" + eventElement.name;
                        eventElement.select();
                        eventElement.focus();
                    } catch (e) {}
                } else {
            ...
            switch(elementName) {
                case "p_cntrno" :
//                	alert("4444444444444	p_cntrno");
                    if (curValue.length < 10) {
                        if (document.form.check_digit == "[object]") document.form.check_digit.value = "";
                        if (document.form.ctnr_tpsz_cd == "[object]") document.form.ctnr_tpsz_cd.value = "";
                        if (document.form.cntr_rsk_flg == "[object]") document.form.cntr_rsk_flg.value = "";
                        if (curKeyCode != "9") return;
                    }
                    break;
                case "p_yard1":
                    if (curValue.length < 5) {
                        if (document.getElementById("p_yard2") == "[object]") document.getElementById("p_yard2").RemoveAll();
                    }
                    break;
                default :
                    flgNext = true;
        ...
        alert ("오류가 발생 했음 :" + e + ":" + eventElement.name);
    }
}
 ...
 */
 function cntr_search() {
	 ...
	 if (onShowErrMsg) {
	    ...
	    if (cntrno == "") return;
	    if (cntrno.length < 10) {
	        if (frmObj.check_digit == "[object]") frmObj.check_digit.value = "";
	        if (frmObj.ctnr_tpsz_cd == "[object]") frmObj.ctnr_tpsz_cd.value = "";
	        if (frmObj.cntr_rsk_flg == "[object]") frmObj.cntr_rsk_flg.value = "";
	    }
...
//	            document.form.p_vvdcd.value = document.form.p_vvdcd.value + ":" + event.type;
	        ...
	        obj = document.getElementById("p_yard2");
...
function getFocus() {
    ...
    try {
        if (event.srcElement.type == 'text') {
            srcValue = event.srcElement.value.split("-").join("");
            event.srcElement.value = srcValue;
        }
    } catch (e) {
        srcValue = '';
    }
...
function obj_enterProcess (eventName) {
    ...
    if (obj==null) {
        ...
        for (var i = 0; i < tds.length; i++) {
            var td=tds[i];
            if (td.name == "btn_retrieve" || td.name == "btn_Retrieve") {
            	/* Date 입력 필드에서 Retrieve 버튼 object로 focus를 이동시켜 onblur Event를 발생 시킴 */
                td.focus();
...
//alert (Row + "::::" + sheetObj.RowStatus(Row))
    ...
    nowStatus = sheetObj.RowStatus(Row);
    if (Sts != 'C' && Sts != '' && Sts != 'M' && Sts != 'B') {	// 2011.08.08 Sts != 'B' 추가
        sheetObj.CellValue(Row, 5) = 'N';
    }
    if (nowStatus == 'I' || nowStatus == 'D') {
        return;
    } else {
...
function code_search(Value, Column, Table) {
    if (!Value || !Column || !Table) return false;
    var sheetObj = sheetObjects[0];
    var coCtmXml = sheetObj.GetSearchXml ("CTMCommonGS.do", "f_cmd=" + SEARCH19 + "&code_value=" + Value + "&column_nm=" + Column + "&table_nm=" + Table);
    var rtnValue = ComGetEtcData(coCtmXml, "rtnValue");
    if (rtnValue) {
        if (rtnValue == "1") {
            return true;
 ...
 */
 function DomSetFormObjDisable(form, flag) {
     ...
     for ( var i = 0; i < elems.length; i++) {
         if (elems[i].type.length > 0 && elems[i].type != "hidden" && elems[i].disabletype != "no") {
     ...
     for ( var i = 0; i < obj.length; i++) {
         if (obj[i].getAttribute("name").substring(0, 4) == "btn_" && obj[i].disabletype != "no") {
             obj[i].style.cursor = styleCursor;
             obj[i].disabled = flag;
 ...
 **********************************************************************/
 function sheet_blur() {
     var sheet = event.srcElement;
     var row = sheet.SelectRow;
     var col = sheet.SelectCol;
     if (row == "") row = 0;
     if (col == "") col = 0;
     try {
...
function multiComboOnCheckClick(comboObj, index, htmlObj) {
    ...
    if (comboObj.Text == null || comboObj.Text == "") {
        ...
        if (htmlObj == "[object]") htmlObj.value = "";
        ...
        // 다른Index가 선택된 경우는 Index 0을 해제
            ...
            // Submit할 내용 Define
            if (htmlObj == "[object]") htmlObj.value = "'" + ComReplaceStr(comboObj.Code, ",", "', '") + "'";
        }
    }
}

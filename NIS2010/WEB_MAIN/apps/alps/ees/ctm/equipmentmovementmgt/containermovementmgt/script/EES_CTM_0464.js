		...
		switch(sheetNo) {
			case 1:      //sheet1 init
				with (sheetObj) {
					...
					var HeadTitle = "Seq.|Chk|CNTR NO|Digit|cntr";
					...
					InitDataProperty(0, cnt++ , dtData,     200,   daCenter,  false,    "multiplekey",  false,		"",       dfNone,        0,       true,       true,	  11,	 true,	 true,	   "",	  false);
...
*/
				...
				if (cntr_no.length > 10) {
			     	sheetObj.CellValue2(row, "digit") = cntr_no.substring(10,11);
	...
	function comPopupOK(sheetObj,formObject) {
		...
		if(errcnt > 0){       
			...
			if(opener != null) {
				// 일반 팝업인 경우
				opener.getCntr_Multi(rArray,return_val);  //호출하는 부모js에 getMnr_Multi 붙여넣으면됩니다.
			} else {
				// Modal 팝업인 경우
				window.dialogArguments.getCntr_Multi(rArray,return_val);
			}

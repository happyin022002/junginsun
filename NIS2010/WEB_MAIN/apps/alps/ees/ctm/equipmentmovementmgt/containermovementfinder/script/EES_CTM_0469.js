 ...
 * @FileName : EES_CTM_0469.js
 * @FileTitle : Multi BKG No input
 * Open Issues :
 * Change history :
 * @LastModifyDate :
 * @LastModifier :
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends CTM   
     * @class ees_ctm_0464 : ees_ctm_0464 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ees_ctm_0469() {    
		...
		for(i=0;i<sheetObjects.length;i++) { 
		   ...
		   if (arr.length > 0 &&  document.form.orgval.value !="" ){
			   ...
			   for(i=0; i < arr.length; i++) {
					...
					sheetObjects[0].CellValue2(insrow, 'multiplekey') = arr[i];
					sheetObjects[0].CellValue2(insrow, 'bkg_no') =  arr[i];
	...
	function initSheet(sheetObj,sheetNo) {
		...
		case 1 :
			with (sheetObj) {
				...
				var HeadTitle = "Seq.|Chk|BKG NO|bkg";
				...
				InitDataProperty(0, cnt++ , dtData,     200,   daCenter,  false,    "multiplekey",  false,		"",       dfNone,        0,       true,       true,	  13,	 true,	 true,	   "",	  false);
				InitDataProperty(0, cnt++ , dtHidden,    0,   daCenter,   false,    "bkg_no",      false,		"",       dfNone,        0,       false,      false,  13,	 true,	 true,	   "",	  false);
	...
	function sheet1_OnChange(sheetObj, row, col, value) {
		...
		case "multiplekey" :
			...
			sheetObj.CellValue(row, "bkg_no") = value;
			sheetObj.CellValue(row, "check") = "1"; 
			break;
		case "check" :
			if (value == "1") {
				var chkval = trim(sheetObj.CellValue(row, "bkg_no"));
				if (chkval == "") { 
	...
	function comPopupOK(sheetObj,formObject) {
			...
			for(var i = 0; i < rows; i++) {
				if(sheetObj.CellValue(i, "check") == 1) {
					...
					for(var j=0; j<cArray.length; j++) {
						chkval=sheetObj.CellValue(i, 'bkg_no');
		...
		if(errcnt > 0){       
			...
			if(opener != null) {
				// 일반 팝업인 경우
				opener.getBKG_Multi(rArray,return_val);  //호출하는 부모js에 getMnr_Multi 붙여넣으면됩니다.
			} else {
				// Modal 팝업인 경우
				window.dialogArguments.getBKG_Multi(rArray,return_val);
			}

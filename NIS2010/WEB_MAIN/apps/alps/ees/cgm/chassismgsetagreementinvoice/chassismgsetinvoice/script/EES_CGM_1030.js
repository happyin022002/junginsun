...
/**
 * @extends
 * @class ees_cgm_1030 : ees_cgm_1030 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ees_cgm_1030() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: // sheet1 init
		with (sheetObj) {
			...
			var HeadTitle = "|Seq.|Lessor Chassis No.|SML Chassis No.|Actual Chassis No.|Invoice Reference No.|Invoice No.|Charge Type|On-Hire Loc.|On-Hire Date|Billing Start Date|Off-Hire Loc.|Off-Hire Date|Billing End Date|Used Days|Rate|Total|Verify Status|||||"
					+ "|||||||||||||||||||||";
			...
			InitDataProperty(0, cnt++, dtData, 80, daCenter, false, "inv_chg_tp_nm", false, "", dfNone, 0, false, false);
			...
			InitDataProperty(0, cnt++, dtData, 120, daCenter, false, "inv_bil_st_dt", false, "", dfDateYmd, 0, false, false);
			...
			InitDataProperty(0, cnt++, dtData, 120, daCenter, false, "inv_bil_end_dt", false, "", dfDateYmd, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 80, daCenter, false,	"inv_lse_use_dys", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 50, daRight, false,"inv_lse_rt_amt", false, "", dfFloat, 2, false, false, 10);
			InitDataProperty(0, cnt++, dtData, 80, daRight, false, "inv_lse_chg_amt", false, "", dfFloat, 2, false, false, 10);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case MULTI03: // Verify Insert => Verify Search
		...
		if (insResult == "SUCCESS") {
			...
			var sXml = sheetObj.GetSearchXml("EES_CGM_1030GS.do", sParam);
			sheetObj.LoadSearchXml(sXml);
			// verify 성공여부를 hidden parametre에 저장 (성공 : Y, 실패 : N)
			// Service에서의 체크로직이 성공인지를 확인한다.
			if (sheetObj.FindText("vrfy_scs_flg", 'N') == -1) {

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class ees_cgm_2098 : ees_cgm_2098 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
		...
		switch(sAction) {
	    	case IBSEARCH:      //조회
				...
				ComOpenWait(true);
	     		...
	     		var sXml = sheetObj.GetSearchXml("EES_CGM_2098GS.do" , FormQueryString(formObj), '', true);
	    	...
	    	case IBSAVE:        //저장
				...
				*/
				...
				var sXml = sheetObj.GetSaveXml("EES_CGM_2098GS.do", sParam);
	    	...
	    	case IBCREATE:
				...
				ComOpenWait(true);
	     		...
	     		var sXml = sheetObjects[0].GetSaveXml("EES_CGM_2098GS.do", sParam);
	    	...
	    	case IBDELETE:
				...
				ComOpenWait(true);	
	     		...
	     		var sXml = sheetObjects[0].GetSaveXml("EES_CGM_2098GS.do", FormQueryString(formObj));
	        		...
	        		for(var i=0; i<arrStatusRows.length-1; i++){
	        			...
	        			if(sheetObj.cellValue(arrStatusRows[i],"chg_cd") == 'PDM'){
	        				if(ComTrim(sheetObj.cellValue(arrStatusRows[i],"eq_no"))==""){
	        					ComShowCodeMessage('CGM10004','SML M.G.Set No.');

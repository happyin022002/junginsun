...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class ees_cgm_1123 : ees_cgm_1123 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ees_cgm_1123() {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {	
			case IBSEARCH:	  //조회
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("EES_CGM_1123GS.do" , FormQueryString(formObj), '', true); 
...
//				document.form.inv_dt.value = ComCgmNullToBlank(sheetObj.EtcData("inv_dt"));
			...
			case IBSAVE:		//저장 , Confirm
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("EES_CGM_1123GS.do", sParam);
			...
			case IBDELETE:	  // 삭제
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("EES_CGM_1123GS.do", FormQueryString(formObj));
				sheetObj.LoadSaveXml(sXml);
				ComOpenWait(false);
				break;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class EES_CGM_2036 : EES_CGM_2036 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			var HeadTitle1 = "|Seq.|S.Provider|Invoice No.|CSR No.|Status|Cost Month|Rev Month|Net Amount|Tax DIV|Tax|Total Amount|Issue DT|Receive DT|Confirm DT|CRE User"
								+ "|pay_inv_seq";
	...
	case "sheet2":
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 120, daRight, true, "pay_tax_amt", 	false,	"", dfFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 120, daRight, true, "pay_cr_amt", 	false,	"", dfFloat, 2, false, false);
			...
			if (sheetObj.id == "sheet1")
			{
		    	...
		    	formObj.f_cmd.value = SEARCH;
		 		...
		 		if(validateForm(sheetObj,formObj,sAction))
		 		{
			 		sheetObj.WaitImageVisible=false;
			 		ComOpenWait(true);
		 			var sXml = sheetObj.GetSearchXml("EES_CGM_2036GS.do" , FormQueryString(formObj));

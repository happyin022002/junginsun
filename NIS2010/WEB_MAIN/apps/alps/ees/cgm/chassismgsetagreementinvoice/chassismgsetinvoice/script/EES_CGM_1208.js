...
/**
 * @extends
 * @class EES_CGM_1208 : EES_CGM_1208 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_CGM_1208() {
...
//	axon_event.addListener('change', 'obj_change', 'cost_yrmon');  // 불필요한 이벤트 호출로 주석처리함, 신용찬, 2014-03-26
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			var HeadTitle1 = "|Seq.|Pool Name|Invoice No.|CSR No.|Status|Cost Month|Rev Month|Net Amount|Tax DIV|Tax|Total Amount|Issue DT|Receive DT|Confirm DT|CRE User|pay_inv_seq";
			...
			InitDataProperty(0, cnt++, dtData, 80, daCenter, true, "chss_pool_cd", 			false,	"", dfNone, 0, false, false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
			...
			if (sheetObj.id == "sheet1")
			{
		    	...
		    	formObj.f_cmd.value = SEARCH;
		 		...
		 		if(validateForm(sheetObj,formObj,sAction))
		 		{
			 		sheetObj.WaitImageVisible=false;
			 		ComOpenWait(true);		 			
		 			var sXml = sheetObj.GetSearchXml("EES_CGM_1208GS.do" , FormQueryString(formObj));
	...
	case IBSEARCH_ASYNC04:
 	...
 	case IBRESET: //CHSS_POOL_CD
 		...
 		// combo control, 결과 문자열, Text Index, Code Index
	  	MakeComboObject3(formObj.chss_pool_cd, poolArr, 0, 0);     
 		idx++;

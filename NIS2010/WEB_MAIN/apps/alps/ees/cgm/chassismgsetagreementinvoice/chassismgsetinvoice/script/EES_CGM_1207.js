...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class ees_cgm_1207 : ees_cgm_1207 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
	function ees_cgm_1207() {
	...
	function processButtonClick(){
	    ...
	    try {
	        ...
	        switch(srcName) {
				...
				case "btn_Cancel":
					if(validateForm(sheetObject1,formObject,IBRESET) != false) {	
				...
				case "btns_vndr":	// Lessor Code 가져오기 팝업
					ComOpenPopupWithTarget('/hanjin/COM_ENS_0C1.do', 700, 455, "vndr_seq:p_vndr_seq|vndr_lgl_eng_nm:vndr_lgl_eng_nm", "1,0,1,1,1,1", true);
					break;
...
//		alert("firstDayInPreviousMonth");
	...
	function initControl(){
		...
		var m = "";
		// 2015-05-13 Chang-Young Kim Cost Month를 현재월로 Setting => 현재월 X, 전달 O
    	...
    	switch(sheetID) {
	    	...
	    	case "sheet1":
	    		with (sheetObj) {
	                ...
	                var HeadTitle1 = "|Seq.|CHK|Cost Month|Invoice No.|Pool Name|Net Amount|Tax Div.|Tax|Rate Sum|Tax Sum|Credit Sum|CSR No.|Total Amount|Status||||||||||";
					...
					InitDataProperty(0, cnt++ , dtData,			90,	daCenter,true,	"chss_pool_cd",			false,	"",	dfNone,			0,	true, true);
					...
					InitDataProperty(0, cnt++ , dtData,			70,	daRight, true,	"lse_chg_smry_amt",		false,	"",	dfNullFloat,	2,	false, false);
					InitDataProperty(0, cnt++ , dtData,			70,	daRight, true,	"inv_tax_smry_amt",		false,	"",	dfNullFloat,	2,	false, false);
					InitDataProperty(0, cnt++ , dtData,			70,	daRight, true,	"inv_cr_smry_amt",		false,	"",	dfNullFloat,	2,	false, false);
					...
					InitDataProperty(0, cnt++ , dtData,			90,	daLeft,	 true,	"status",			false,	"",	dfNone,			0,  false,false);
	        ...
	        case "sheet2":
	            with (sheetObj) {
	                ...
	                var HeadTitle1 = "|Agreement No.|CNTR NO.|CHSS No.|CHSS TPSZ|Charge Type|Cost Code|Account|Currency|Tax|Credit Amount|Amount";
					...
					InitDataProperty(0, cnt++ , dtData,			120,daCenter,true,"inv_cust_eq_no",		 false,	"",	dfNone,		0,	true, false);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
	    ...
	    switch(sAction) {
			...
			case IBSEARCH:      //조회
				...
				formObj.vndr_seq.value = formObj.p_vndr_seq.value;
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);	
				
				var sXml = sheetObj.GetSearchXml("EES_CGM_1207GS.do" , FormQueryString(formObj), '', true); 
			...
			case IBSAVE:        // Invoice Confirm
	    		...
	    		if(iCheckRow > 0){
					...
					formObj.f_cmd.value = MULTI01;
		     		...
		     		var sXml = sheetObj.GetSaveXml("EES_CGM_1207GS.do", sParam);
...
//				formObj.vndr_seq.RemoveAll(); -- script Error
...
//    			} 
    		...
    		case IBSAVE:
    			for(var i = 1; i <= sheetObj.RowCount; i++){
    				if(sheetObj.CellValue(i, "del_chk") == 1){
    					if(sheetObj.CellValue(i, "inv_tax_rt") != 0.00 && sheetObj.CellValue(i, "inv_tax_rt") != "" && sheetObj.CellValue(i, "inv_tax_rt") != null){
    						if(sheetObj.CellValue(i, "inv_tax_clt_tp_cd") == ""){
    							ComShowCodeMessage('CGM10004','Tax Div. ');
    		    				sheetObj.SelectCell(i, "inv_tax_clt_tp_cd");
    		    				return false;
    						}
    					}
    					
    					if(sheetObj.CellValue(i, "chss_mgst_inv_sts_cd") != "S"){
    						ComShowCodeMessage('CGM20052');
    						sheetObj.SelectCell(i, "status");
    						return false;
    					}
    		...
    		case IBRESET:
    			for(var i = 1; i <= sheetObj.RowCount; i++){
    				if(sheetObj.CellValue(i, "del_chk") == 1){
	    				switch(sheetObj.CellValue(i, "status_cd")){
		    	    		case "C":
		    	    		case "R":
		    	    		case "J":
		    	    		case "E":
		    	    		...
		    	    		default:
		    	    			ComShowCodeMessage('CGM20053', sheetObj.CellValue(i, "status"));
		    	    			sheetObj.SelectCell(i, "status");
								return false;
	    				}
    				}
	...
	function sheet1_OnClick(sheetObj, Row, Col, Value, CellX, CellY, CellW, CellH){
		...
		if(sheetObj.ColSaveName(Col) == 'del_chk'){
			...
			if(sheetObj.cellValue(Row,"chss_mgst_inv_knd_cd") == 'LS'){
...
//			doActionBtnEnable(invStatus, statusCd);
    ...
    function sheet1_OnDblClick(sheetObj, Row, Col, CellX, CellY, CellW, CellH){
    	...
    	if(sheetObj.ColSaveName(Col) != 'del_chk'){
	    	...
	    	document.form.vndr_seq.value = sheetObj.CellValue(Row, "vndr_seq");
	...
	/**
	 * DIV MultiCombo 의  OnChange 이벤트처리 <br>
	 * DIV 종류에 따라 Cost Office Code 를 조회한다.
	 ...
	 * @version 2009.09.28
	 */  
    function chss_mgst_inv_knd_cd_OnChange(comboObj, Index_Code, Text){
    	// Cost Office Code 조회 (최초 LSE 이기 때문에 DB 에서 검색)
    	doActionIBSheet(sheetObjects[2], document.form, IBSEARCH_ASYNC01);
    	// S.Provider 조회
    	doActionIBSheet(sheetObjects[2], document.form, IBSEARCH_ASYNC02);

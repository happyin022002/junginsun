...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class ees_cgm_1028 : ees_cgm_1028 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
	function ees_cgm_1028() {
	    	...
	    	case 1: // sheet1 init
	    		with (sheetObj) {
	                ...
	                InitDataProperty(0, cnt++ , dtCheckBox,		30,		daCenter,	true,	"del_chk",			false,	"", dfNone,	0, true,  true);
	 				InitDataProperty(0, cnt++ , dtData,			130,	daCenter,	false,	"inv_ref_no",		false,	"",	dfNone,	0, false,  true, 25);
	 				InitDataProperty(0, cnt++ , dtPopupEdit,	150,	daCenter,		false,	"agmt_no", 			false,	"",	dfNone,	0, true,  true, 9);
	 				InitDataProperty(0, cnt++ , dtData,			130,	daCenter,	false,	"vndr_seq",			false,	"",	dfNone,	0, false, false);
	 			...
	 			if (actionFlag){	
		    		...
		    		// 체크표시된 Row 를 구함.
	 				...
	 				if(arrChkRows.length == 1 && arrDelRows.length == 1){
			 			...
			 			var sXml = sheetObj.GetSaveXml("EES_CGM_1028GS.do", sParam);
	 	...
	 	if (Col == agmtNoCol && Row !=0) {
	 		...
	 		if (cellValue != ''){
	 			...
	 			if(cellValue.length != 9 || !ComIsNumber(cellValue.substring(3))){
	 				...
	 				formObj.eq_knd_cd.value = EQ_KND_CD_CHASSIS;
			        var sXml = sheetObj.GetSearchXml("EES_CGM_1028GS.do", FormQueryString(formObj), '', true);

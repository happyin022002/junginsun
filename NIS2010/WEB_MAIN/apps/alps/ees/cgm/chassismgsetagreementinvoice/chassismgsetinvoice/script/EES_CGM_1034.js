...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class ees_cgm_1034 : ees_cgm_1034 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
	function ees_cgm_1034() {
	...
	function loadPage() {
		...
		comboObjects[0].Text2 = "LSE";
		comboObjects[0].Code2 = "LS";
    	...
    	switch(sheetID) {
	    	...
	    	case "sheet1":
	    		with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtData,		   130,	daCenter,true,	"inv_no",			false,	"",	dfNone,			0,	false, false);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
	    ...
	    switch(sAction) {
			...
			case IBSEARCH:      //조회
				...
				var sXml = sheetObj.GetSearchXml("EES_CGM_1034GS.do" , FormQueryString(formObj), '', true); 
			...
			case IBSAVE:        // Invoice Confirm
	    		...
	    		if(iCheckRow > 0){
					...
					formObj.f_cmd.value = MULTI01;
		     		...
		     		var sXml = sheetObj.GetSaveXml("EES_CGM_1034GS.do", sParam);
	...
	function validateForm(sheetObj,formObj,sAction){
    	...
    	switch(sAction) {
    		case IBSEARCH:
    			if(document.chss_mgst_inv_knd_cd.Text == ''){

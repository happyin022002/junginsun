...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class ees_cgm_2032 : ees_cgm_2032 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
				...
				case "btn_InvoiceImportAudit":
					...
					if(iCheckRow > 0){
						...
						if(sChgSeq != ''){
							...
							ComOpenWindowCenter('/hanjin/EES_CGM_2085.do' + param, sWinName, 980, 550, true);
				...
				case "btn_AuditResultCreation":
					...
					if(iCheckRow > 0){
						...
						if(sChgSeq != ''){
							...
							ComOpenWindowCenter('/hanjin/EES_CGM_2098.do' + param, sWinName, 980, 600, true);
	    ...
	    switch(sheetNo) {
	        case 1:      // sheet1 init
	            with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtData,			280,daLeft,	 false,	"vndr_lgl_eng_nm",	false, "", dfNone,	  0, false, true);
					InitDataProperty(0, cnt++ , dtData,			120,daLeft,	 false,	"lse_chg_sts_desc",	false, "", dfNone,	  0, false, true);
			...
			case 2:      // sheet2 init
	        	with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtData,	100, daRight,  false, "inv_smry_amt",	  false, "", dfFloat,   2, false, true);
					InitDataProperty(0, cnt++ , dtData,	100, daRight,  true,  "pay_lse_chg_amt",  false, "", dfFloat,   2, false, true);
					InitDataProperty(0, cnt++ , dtData,	100, daRight,  true,  "debit_amt",	      false, "", dfFloat,   2, false, true);
					InitDataProperty(0, cnt++ , dtData,	100, daRight,  true,  "cr_smry_amt",	  false, "", dfFloat,   2, false, true);
					InitDataProperty(0, cnt++ , dtData,	100, daRight,  true,  "tax_smry_amt",	  false, "", dfFloat,   2, false, true);
					InitDataProperty(0, cnt++ , dtData,	90,	 daCenter, true,  "cre_ofc_cd",	 	  false, "", dfNone,	0, false, true);
					InitDataProperty(0, cnt++ , dtData,	90,	 daCenter, true,  "cre_usr_id",		  false, "", dfNone,	0, false, true);
					InitDataProperty(0, cnt++ , dtData,	95, daCenter, true,  "cre_dt", 		  false, "", dfDateYmd, 0, false, true);
	    ...
	    switch(sAction) {
	    	...
	    	case IBSEARCH:      //조회 (Retrieve 버튼을 클릭했을 때)
		    	formObj.f_cmd.value = SEARCH;
		 		...
		 		ComOpenWait(true);	 		     		
	     		var sXml = sheetObj.GetSearchXml("EES_CGM_2032GS.do" , FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1"), '', true);
	    	...
	    	case IBSAVE:        // 저장 (Charge Creation)
	    		...
	    		if(iCheckRow > 0){
	    			...
	    			for(var i=0; i<arrCheckRows.length-1; i++){
		         	...
		         	var sXml = sheetObj.GetSaveXml("EES_CGM_2032GS.do", sParam + "&" + ComGetPrefixParam(prefix));
		    	...
		    	if(iCheckRow > 0){
		    		if(chgCreSeq != ''){
			    		...
			    		formObj.eq_knd_cd.value = EQ_KND_CD_MGSET;
			         	...
			         	sParam = sParam + "&";
			         	sParam = sParam + FormQueryString(formObj);
			         	var sXml = sheetObjects[1].GetSaveXml("EES_CGM_2032GS.do", sParam + "&" + ComGetPrefixParam("sheet1"));
			     		sheetObjects[1].RemoveAll();
	...
	function sheet1_OnClick(sheetObj, Row, Col, Value, CellX, CellY, CellW, CellH){
		...
		// Charge Creation 완료	
			...
			formObj.f_cmd.value = SEARCH01;
     		...
     		var sXml = sheetObj.GetSearchXml("EES_CGM_2032GS.do" , FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet2"), '', true );     	

...
*@FileName : ees_cgm_multy.js
*@FileTitle : Multi data select common
*Open Issues : 이화면은 서버단을 타지 않는다 CgmComEvent 은 덤프 이벤트이다 
*Change history :  
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends Cgm  
     * @class ees_cgm_multy : ees_cgm_multy 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ees_cgm_multy() {    
		...
		switch(colName){
			case 'multiplekey':
				for (var i = 0; i < inputStr.length; i++) {
					...
					if (oneChar != "") {
						if ( hanCheck(oneChar) ) { 
							ComShowCodeMessage("CGM20080", "Multiple Inquiry data!");
	...
	function comPopupOK(sheetObj,formObject) {
			...
			for(var i = 0; i < rows; i++) {
				if(sheetObj.CellValue(i, "check") == 1) {
					...
					for(var j=0; j<cArray.length; j++) {
						...
						if(chkval=="") {  
							ComShowCodeMessage("CGM20082", "Multiple Inquiry data!"); 
		...
		if(errcnt > 0){       
			ComShowCodeMessage("CGM20083", "Multiple Inquiry data!"); 
			//sheetObj.CellValue(xxx,'multiplekey')=""; 
			sheetObj.CellEditable(dupcnt,'multiplekey') = true;
			sheetObj.SelectCell(dupcnt, 'multiplekey');  
		}else{     
			opener.getCgm_Multi(rArray,return_val);  //호출하는 부모js에 getCgm_Multi 붙여넣으면됩니다.
			window.close();                  
		}	
	}   

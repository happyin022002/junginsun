...
/**
 * @extends
 * @class EES_CGM_1153 : EES_CGM_1153 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_CGM_1153() {
		...
		try {
			...
			case "btn_detail":
	            ...
	            if (ComIsBtnEnable(srcName)) {
	                ComOpenWindowCenter("/hanjin/EES_CGM_1154.do?" +
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		case IBSEARCH: //조회
			if (validateForm(sheetObj, formObj, sAction)){
				...
				sheetObj.DoSearch("EES_CGM_1153GS.do", FormQueryString(formObj));
				ComOpenWait(false);			
			}
			break;

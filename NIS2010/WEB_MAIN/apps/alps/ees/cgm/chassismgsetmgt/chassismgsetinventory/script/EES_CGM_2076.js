...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class EES_CGM_2076 : EES_CGM_2076 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_CGM_2076() {
 	...
 	try {
         ...
         switch(srcName) {
            ...
            case "btn_print":
				...
				ComOpenPopupWithTarget('/hanjin/EES_CGM_2205.do', 775, 800, "", "0,1,1,1,1,1,1", true);
                break;
...
function sheet1_OnLoadFinish(sheetObj) {
    ...
    axon_event.addListenerForm('keyup', 'obj_keyup', form);
 	...
 	comboObjects[comboCnt++] = document.disp_cd;
 	...
 	comboObjects[comboCnt++] = document.dmg_snd;
  	...
  	// Disposal MultiCombo 값 초기화
  	var arrDisp = new Array();
  	arrDisp[0] = "I|Including";
  	arrDisp[1] = "E|Excluding";
  	arrDisp[2] = "O|Only";
  	makeComboObject(document.form.disp_cd, arrDisp, 1, 0, 0);
         ...
         case "sheet1":
             with (sheetObj) {
                ...
                //전체Edit 허용 여부 [선택, Default false]
                 ...
                 var HeadTitle = "Scc/Type|Lease Term|TOTAL|UMG|CLG";
                 ...
                 InitDataProperty(0, cnt++ , dtData,      220,    daCenter,  true,     "group1",    		false,	"",      dfNone, 0, false, false);
                 InitDataProperty(0, cnt++ , dtData,   	  220,    daCenter,  false,     "agmt_lstm_cd",  	false,	"",      dfNone, 0, false, false);
                 InitDataProperty(0, cnt++ , dtAutoSum,  180,    daRight,  false,     "eq_tpsz_cd_total",  			false,	"",      dfNumber, 0, false, false);
                 InitDataProperty(0, cnt++ , dtAutoSum,  180,    daRight,   false,     "eq_tpsz_cd_umg",	false,	"",      dfNumber, 0, false, false);
                 InitDataProperty(0, cnt++ , dtAutoSum,   180,    daRight,   false,     "eq_tpsz_cd_clg",	false,	"",      dfNumber, 0, false, false);
     	...
     	case IBSEARCH:      //조회
	    	...
	    	*/
	 		...
	 		if(validateForm(sheetObj,formObj,sAction))
	 		{
		 		sheetObj.WaitImageVisible=false;
		 		ComOpenWait(true);	 				 			
	 			var sXml = sheetObj.GetSearchXml("EES_CGM_2076GS.do" , FormQueryString(formObj));
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg)
{
	with(sheetObj)
	{
        with(sheetObj)
        {
            ShowSubSum(0, "2|3|4", -1, false, false,	-1,"group1=Sub Total");
        }
 ...
 */ 
 function sheet1_OnMouseDown (Button, Shift, X, Y){
  ...
  function sheet1_OnDblClick(sheetObj, Row, Col, CellX, CellY, CellW, CellH){
  	...
  	if(document.group1.Index == 4 && groupValue1 != "Sub Total") // Lessor 처리를 해주어야 한다. 
  	{
  		var dispText = groupValue1;
  		if(dispText.length >=8)
  		{
  			//기존 groupValue1 = dispText.substring(0,6);
  	...
  	if(document.group1.Index == 5 && groupValue1 != "Sub Total") // Lessor 처리를 해주어야 한다. 
  	{
  		var dispText = groupValue1;
  	...
  	if(groupValue1.substring(0,9) == "Sub Total")
  	{
		...
		s2_group1 = "SubSum";
  		if(document.group1.Index == 4) // Lessor 처리를 해주어야 한다. 
  		{
  			var dispText = sheetObj.CellValue(Row-1, "group1");
  			if(dispText.length >= 8)
  			{
  	...
  	var agmtLstmCd = sheetObj.cellValue(Row, "agmt_lstm_cd");
  	var s2EqTpszCd = "";
  	//if(comboObjects[2].Code == "") 선택이 안되었을때는 Code == ""
  	
  	if(parseInt(Col) == 2)s2EqTpszCd="TOT";
  	else if(parseInt(Col) == 3)s2EqTpszCd="UMG";
  	else if(parseInt(Col) == 4)s2EqTpszCd="CLG";
	...
	*/
	...
	var disp_cd = comboObjects[2].Code;
	/*

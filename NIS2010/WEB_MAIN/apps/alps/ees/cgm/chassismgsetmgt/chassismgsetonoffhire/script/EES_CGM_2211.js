...
/**
 * @extends
 * @class EES_CGM_2211 : EES_CGM_2211 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_CGM_2211() {
...
function processButtonClick(){
	...
	try {
		...
		case "btn_New":
			...
			formObject.dio[0].selected = true;
	...
	case "sheet1":
		with (sheetObj) {
			...
			for(var i = 0; i < 2; i++) {
				...
				InitDataProperty(i, cnt++ , dtData,             100,    daCenter, true,  "ym",         false,  "",     dfNone);
				for(var j=0; j<arrTpSz2.length; j++){
					InitDataProperty(i, cnt++ , dtData,         100,    daRight,  true,  arrTpSz2[j] + "",          false,  "",     dfNone,   0,  false,    true);
				}
				InitDataProperty(i, cnt++ , dtData,             100,    daRight,  true,  "div_total",   false,  "",     dfNone,   0,  false,    true);
				InitDataProperty(i, cnt++ , dtData,             100,    daRight,  true,  "ratio",       false,  "",     dfNone,   0,  false,    true);
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	if(validateForm(sheetObj,formObj,sAction))
		if (sheetObj.id == "sheet1"){
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("EES_CGM_2211GS.do", FormQueryString(formObj));

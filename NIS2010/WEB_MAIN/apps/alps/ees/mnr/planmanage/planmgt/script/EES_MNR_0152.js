...
/**
 * @extends
 * @class ees_mnr_0152 : ees_mnr_0152 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ees_mnr_0152() {
...
function initSheet(sheetObj, eqKndCd) {
	...
	case "sheet1": // sheet2 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtSeq, 29, daCenter, true, "Seq");
			InitDataProperty(0, cnt++, dtData, 93, daCenter, true,"ctrl_ofc_cd", true, "", dfEngUpKey, 0, false, false);
			// TP/SZ 컬럼 생성 시작-------------------------------------------------//
			var calCulSum = "";
			for ( var i = 0; i < disPlayTpSz.length; i++) {
...
function sheet1_OnSaveEnd(sheetObj, ErrMsg) {
	if (ErrMsg == "") {
		...
		doActionIBSheet(sheetObjects[0], document.form, IBSEARCH); //재조회
	...
	with (formObj) {
		...
		// 저장시 체크
			...
			if(strMnrOfficeLevel == "L2") {
				// 조회후 저장 메세지
				if (retrieveClick == 0) {
					var saveTypeName = "save";
					if (sActionIdx == 1) {
						saveTypeName = "save";
					} else if (sActionIdx == 2) {
						saveTypeName = "confirm";
					} else if (sActionIdx == 3) {
						saveTypeName = "delete";
					} else {
						saveTypeName = "save";
					}
					ComShowCodeMessage("MNR00199", "Disposal Planning " + saveTypeName );
					return false;
				}
				// 조회값이 존재 하지 않으면

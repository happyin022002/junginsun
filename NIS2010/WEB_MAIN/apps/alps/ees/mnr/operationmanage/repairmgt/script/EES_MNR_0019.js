...
* 2011.06.08 김종옥 [CHM-201111303-01] ALPS MNR-> Repair-Estimate Creation 시, Verify 후, Verify Result결과에 대하여 Point 주는 기능 요청
* 2011.12.27 신혜정 [CHM-201115280] Reefer Uint Maker 필드 추가 
* 2012.11.09 조경완 [ICM-201200829] 환경변수 세팅
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends  
     * @class EES_MNR_0019 : EES_MNR_0019 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_MNR_0019() {
	    ...
	    switch(comboNo) {        
			   case 1:  
		           with (comboObj) {       
					   ...
					   SetColWidth("200|60|90|55|70|155|140|150");		 	 	   	                                                   
					   DropHeight = 160;                   
			       }  	 
	           break;   
        ...
        switch(sheetObj.id) { 
            ...
            case "sheet2":      // sheet2 init
            	 with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtHidden,   50,daCenter,  				false,  "file_seq",        	false,           "",      dfNone,    0,     	  false,       false);
...
//						SheetWidth = 380;
						...
						InitDataProperty(0, cnt++ , dtHidden,     	50,    daCenter,  false,   prefix + "file_seq",   		false,          "",      dfNone,      0,     true,      true);										
                 ...
                 if(validateForm(sheetObj,formObj,sAction)){    
				 	if(sheetObj.id =="sheet2"){	    
						...
						f_query += 'req_end_dt=' +  formObj.req_end_dt.value;
         				...
         				sheetObj.DoSearch4Post("EES_MNR_0019GS.do",f_query); 
					}	 			   	   		   
				  }         
                break; 	
...
//					    			sheetObjects[1].CellValue(i, "rpr_sts_cd") = "HS";
//					    		}
//					    	}
//					    }else{
...
//					    }
			...
			case IBCREATE:        //Request     
	              if(validateForm(sheetObj,formObj,sAction)){
						...
						sParam += 'req_st_dt=' +  formObj.req_st_dt.value  + '&';
						sParam += 'req_end_dt=' +  formObj.req_end_dt.value + '&';	
				...
				if(formObj.sp_name.GetCount() == 0){ 
					...
					if(arrResult != null){ 	
						for(var i = 0; i < arrResult.length;i ++){
							...
							formObj.sp_name.InsertItem(i, tempComboText ,String(i));    
						}       	   				 				 						
					} else {        		 			
		...
		if(sheetObj.ColSaveName(Col) == "del_chk"){
			if(sheetObj.CellValue(Row, "sold_eq") == "SOLD EQ"){
				if(sheetObj.CellValue(Row, "del_chk") == 0){
					ComShowCodeMessage("MNR00416");//이율규
				}
			}	
		}
	}
		...
		if(fileName.indexOf("\\") == -1) {  
	 	...
	 	if(!badFile) {  
			...
			var sXml = upObj.DoUpload(true);
	 		...
	 		sheetObj.InitCellProperty(Row, prefix+ "file_dw", dtImage);
...
//					if(obj.value == ""){
//						alert(1);
//						MnrNullToBlank(ComGetObjValue(obj));
////						formObj.obj.name.value = "";
//					} 
//				   	break;
			}  		
		} 	

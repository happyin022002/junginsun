...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /** 	
     * @extends   
     * @class EES_MNR_0023 : EES_MNR_0023 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_MNR_0023() {
...
// 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
	...
	*/
    ...
    function loadPage() {
		...
		//2012.11.23 조경완 [CHM-201220893-01]  Estimate Auditing 화면으로 넘어온 parameter 가 있다면 자동 조회 
		if(paramRqstEqNo != "" && popFlag == "Y"){
			doActionIBSheet(sheetObjects[1],document.form,IBSEARCH);
			sheetObjects[1].SelectCell(1,1); 
			sheet2_OnDblClick(sheetObjects[1],1,1);	
		}
    }
        ...
        switch(sheetObj.id) { 
            ...
            case "sheet2":      // sheet2 init
            	 with (sheetObj) {
					...
					InitColumnInfo(headCount + 25, 3, 0, true);  	
					...
					InitDataProperty(0, cnt++ , dtHidden, 			  0,	daCenter,  	false,  "vrfy_rslt_rmk",    false,  "",     dfNone,    	0,  true,   true);
					...
					InitDataProperty(0, cnt++ , dtHidden, 			  0,	daCenter,  	false,  "rct_rpr_flg",      false,  "",     dfNone,    	0,  true,   true);
			...
			case "t1sheet1":      // sheet1 init
                with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtAutoSum,  	50,		daRight,		true,	"rpr_lbr_hrs",				false,	"",	dfFloat, 	2,	true,	true,	6,	false); 
					InitDataProperty(0,	cnt++,	dtData,			50,		daRight,		true,	"rpr_lbr_rt",				false,	"",	dfFloat,	2,	true,	true, 	7,	false);
					InitDataProperty(0,	cnt++,	dtAutoSum,		50,		daRight,		true,	"lbr_cost_amt",				false,	"Round(|rpr_lbr_hrs|*|rpr_lbr_rt|,2)",	dfFloat,		2,	true,	true, 	13,	false);
					InitDataProperty(0,	cnt++,	dtAutoSum,		70,		daRight,		true,	"mtrl_cost_amt",			false,	"",	dfFloat,		2,	true,	true, 	13,	false);
					InitDataProperty(0,	cnt++,	dtAutoSum,		60,		daRight,		true,	"mnr_wrk_amt",				false,	"Round(|lbr_cost_amt|+|mtrl_cost_amt|,2)",	dfFloat,		2,	true,	true, 	13,	false);
					InitDataProperty(0,	cnt++,	dtAutoSum,		90,		daRight,		true,	"mnr_wrk_usd_amt",			false,	"",	dfFloat,		2,	false,	false, 	13,	false);
					InitDataProperty(0,	cnt++,	dtCombo,		140,	daLeft,			true,	"mnr_vrfy_tp_cd",			false,	"",	dfNone,			0,	false,	false); 																
																				 
					//basic 데이타 저장용	  
					//타리프 hour basic
					InitDataProperty(0,	cnt++,	dtAutoSum,		50,		daRight,		true,	"rpr_lbr_bzc_hrs",			false,	"",	dfFloat,		2,	false,	false);
					//어그리먼트 rate basic
					InitDataProperty(0,	cnt++,	dtData,			50,		daRight,		true,	"rpr_lbr_bzc_rt",			false,	"",	dfFloat,		2,	false,	false);
					//둘이 곱한거 basic
					InitDataProperty(0,	cnt++,	dtAutoSum,  	50,		daRight,		true,	"mnr_lbr_bzc_amt",			false,	"Round(|rpr_lbr_bzc_hrs|*|rpr_lbr_bzc_rt|,2)",				dfFloat,		2,	false,	false, 	13,	false);
					//타리프 material basic
					InitDataProperty(0,	cnt++,	dtAutoSum,		70,		daRight,		true,	"lbr_mtrl_bzc_amt",			false,	"",	dfFloat,		2,	false,	false);
					//전체 더한거 basic				
					InitDataProperty(0,	cnt++,	dtAutoSum,		60,		daRight,		true,	"mnr_agmt_amt",				false,	"Round(|mnr_lbr_bzc_amt|+|lbr_mtrl_bzc_amt|,2)",			dfFloat,		2,	true,	true, 	13,	false);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {  
			 case IBSEARCH:      //목록조회   rqst_type  
				...
				if(paramRqstEqNo != ""){
					formObj.rqst_eq_no.value = paramRqstEqNo;
					formObj.rqst_ref_no.value = paramRqstRefNo;
				}else{
                 ...
                 if(validateForm(sheetObj,formObj,sAction)){    
				 	if(sheetObj.id =="sheet2"){	 
				 		//2012.11.23 조경완 [CHM-201220893-01]  Estimate Auditing 화면으로 넘어온 parameter 를 이용하여 조회 
						var f_query = '';         
						...
						f_query += 'rqst_eq_no=' + formObj.rqst_eq_no.value + '&';
						f_query += 'rqst_ref_no=' + formObj.rqst_ref_no.value + '&';
						f_query += 'pop_flag=' + popFlag;
									
         				sheetObj.DoSearch4Post("EES_MNR_0023GS.do",f_query); 
					}	 			   	   		   
				  }  
                break; 	
...
//						btnAproByVrfyRslt();
					    ...
					    sParam = "f_cmd=" + formObj.f_cmd.value +      
								 "&thumbnailFileSeq=" + uploadFileSeq; 	

					    var sXml = sheetObj.GetSaveXml("EES_MNR_0023GS.do", sParam);
						
						var div = document.getElementById('thumbnail_sheet'); //썸네일 HTML 코드 불러오기
						div.innerHTML = "";
//						alert(ComGetEtcData(sXml, "htmlCodeForThumbnail"));
						div.innerHTML = ComGetEtcData(sXml, "htmlCodeForThumbnail");
						//썸네일 세팅 종료					    
			...
			case IBCREATE:        //Reject
	              if(validateForm(sheetObj,formObj,sAction)){
						...
						if((paramRqstEqNo == "" ||paramRqstEqNo.length == 0) && popFlag != "Y"){				 	  	
							sheetObjects[1].LoadSaveXml(sXml);  
							...
							//2012.11.23 조경완 [CHM-201220893-01] Pop-up 일 경우 시트를 초기화하지 않는다 
			...
			case IBSAVE:        //Approval	    
	             if(validateForm(sheetObj,formObj,sAction)){
					    ...
					    if((paramRqstEqNo == "" ||paramRqstEqNo.length == 0) && popFlag != "Y"){
						    ...
						    //2012.11.23 조경완 [CHM-201220893-01] Pop-up 일 경우 시트를 초기화하지 않는다 ㅖ
							if(ComGetEtcData(sXml, "TRANS_RESULT_KEY") == "S"){
								ComShowCodeMessage("MNR00023");
								ComBtnDisable("btn_Verify");
								ComBtnDisable("btn_Reject");
              ...
              if(validateForm(sheetObj,formObj,sAction)){
					...
					if(MnrComGetErrMsg(sXml) == null){
						...
						for(var j = 2; j <= sheetObj.LastRow - 1; j++) {
							...
							//[CHM-201111515-01] Verify 후, Verify Result결과에 대하여 RgbColor 추가
							if(mnrVrfyTpCd == "UH"){
			 ...
			 case IBCLEAR:      //초기화  
				MnrWaitControl(true);
				//MnrFormSetReadOnly(formObj,true,"rpr_yd_cd"); 
	...
	/**
	 * 썸네일 이미지 클릭시 이벤트 관련
	 * 선택한 이미지의 원본 이미지가 팝업으로 호출된다.
	 */ 
	function OpenThumbnailWindow(bigurl) 
	{
		var subBigUrl = bigurl.replace("key=tb_", "key=")
		
	    var newWindow = window.open("", "pictureViewer", 
	        "location=no, menubar=no, scrollbars=no, titlebar=no, toolbar=no");
	    newWindow.document.writeln("<html>");
	    newWindow.document.writeln("<head>");
	    newWindow.document.writeln("<SCRIPT LANGUAGE='JavaScript'>");
	    newWindow.document.writeln("function fnResize(){");
	    newWindow.document.writeln("self.resizeTo(img.width+30, img.height+60);}");
	    newWindow.document.writeln("</SCRIPT>");
	    newWindow.document.writeln("</head>");
	    newWindow.document.writeln("<body onload='fnResize();' >");
	    newWindow.document.writeln("<a href='javascript:window.close();'>");
	    newWindow.document.writeln("<img id='img' src=" + subBigUrl + " alt='Click to close'/>");
	...
	function t2_sheet8_OnPopupClick(sheetObj,Row,Col){
		...
		if(fileName.indexOf("\\") == -1) {  
			...
			if(fileType != "GIF" && fileType != "BMP" && fileType != "TIFF" && fileType != "TIF" && fileType != "JPG" && fileType != "PNG" && fileType != "ZIP"){
				badFile = true;
	 	...
	 	if(!badFile) {  
			...
			if(sXml.indexOf("<SHEET>") != 1){
				if(sXml.indexOf("<ERROR>") == 1){
					if(sXml.match(msgs['MNR00407']) || sXml.match(msgs['MNR00408'])){
						ComShowCodeMessage("MNR00405");	
					} else if(sXml.match(msgs['MNR00409'])){
...
//	/** 
//	 * Description Detail의 Verify Result 항목이 Success가 아니면서 Remark 항목이 비어 있으면 Approval 버튼 비활성화 처리
//	 * SheetObjects[1] Double Click 또는 Remark 변경 시
//	 */
//	function btnAproByVrfyRslt(){
//		var formObj = document.form;
//		var errCnt = 0;
//		for( var idx = 2; idx < sheetObjects[2].LastRow; idx++ ){
//			if( formObj.mnr_rpr_rmk.value == "" ){
//				if( sheetObjects[2].CellValue(idx,"mnr_vrfy_tp_cd") != "SS" ){
//					errCnt = errCnt + 1;
//				}
//			}
//		}
...
//	}
	...
	function oneClickDownload() { // 이율규
		var downloadFiles = "";
		var downloadFilesName = "";
		
		var url = location.href;
		
		url = url.split("//");
		url = url[1].substr(0, url[1].indexOf('/'));
		url = "http://" + url + "/hanjin/FileDownload?key=";
		
		for(var i = 2; i < sheetObjects[10].Rows; i++){
//			if(sheetObjects[10].CellValue(i,"del_chk") == 1){
				if(downloadFiles != ""){
					downloadFiles += "|";
				}
				...
				downloadFiles += url+sheetObjects[10].CellText(i, "file_path_nm");
				downloadFilesName += document.form.rqst_eq_no.value + "\\" + sheetObjects[10].CellText(i, "org_file_nm");
//			}
		}
		//rqst_eq_no rqst_ref_no
		
		
		uploadObjects[0].Download(downloadFiles, '', downloadFilesName);
	}
	
	
/*	function t2_sheet8_OnMouseDown(Row, Col){
		alert(1);
		alert(Row);
		alert(Col);
		alert(sheetObjects[10].CellValue(Row, Col));
		if(CellValue(Row, Col) == "del_chk"){
			alert(1)
		}
	}*/

...
/**
 * @extends 
 * @class ees_mnr_0032 : ees_mnr_0032 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
...
function initSheet(sheetObj,sheetNo) {
	...
	case 1:      //t1sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++ , dtData,    	200,    daLeft,  true,    prefix +"pig_eng_nm",     false,          "",      dfNone, 0,  false , false      );
			...
			InitDataProperty(0, cnt++ , dtData,      80,    daCenter,  false,     prefix +"mnr_ord_seq",     false,          "",      dfNone, 0,  false , false      );
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	switch(sAction) {
		case IBCLEAR:  //NEW
			...
			//Combo 세팅
			formObj.eq_knd_cd.RemoveAll();
			formObj.rpr_rslt_sts.RemoveAll();
			formObj.eq_knd_cd.Code2 = "-1";
			formObj.rpr_rslt_sts.Code2 = "-1"; 
			...
			{	
				...
				{
                    if(i==1){
            			formObj.rpr_rslt_sts.InsertItem(0, "All" ,"");
					}
					for(var j = 0; j < comboList[i].length;j++)
					{   
						var xmlVal = comboList[i][j].split("|");  
						if(i==0){
							formObj.eq_knd_cd.InsertItem(j, xmlVal[1] ,xmlVal[0]);
						}else if(i==1){
							formObj.rpr_rslt_sts.InsertItem(j + 1, xmlVal[1] ,xmlVal[0]);
						}
					}	
				}
				else
		...
		case IBSEARCH:      //Retrieve
			...
			//alert("IBSEARCH : " + "\n"+ sParam);
			var sXml   = sheetObj.GetSearchXml("EES_MNR_0032GS.do", sParam);
		...
		case IBSAVE:        //저장
			...
			//ComDebug(sParam);
			var sXml = sheetObj.GetSaveXml("EES_MNR_0032GS.do", sParam);

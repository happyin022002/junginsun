...
* 2009.12.03 김완규     
* 1.0 Creation  			
* 2011.03.03 김영오 Closing Time그리드 컬럼 문구 변경
* 2011.03.04 김영오 두번째 입찰은 + 25 단위로 가능하도록 하는 기능 삭제처리(정필성차장 요청)
* 2011-03-15 김영오 2011-03-04에 형상관리 한 내용이 몇몇PC에서 정상적으로 반영되지않아 다시 운영에 반영
* 2011-04-11 김영오
...
document.onclick = processButtonClick;
		...
		try {
			...
			switch(srcName) {
				...
				case "btn_DownExcelDetail": 
					sheetObjects[2].Down2Excel(-1, false, false, true, "", "", false, false, "Unit Sale");
					break;
            ...
            case "sheet2":
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false] 
					...
					InitDataProperty(0, cnt++ , dtData,			120, 	daCenter,	true,	"disp_bultn_dt",	false,	"",		dfNone,			0,	false,	false);
					...
					InitDataProperty(0, cnt++ , dtData,   		85,		daRight,  	false,  "part_ut_amt",     	false,  "",     dfNullInteger,  0,  false,  false);
					InitDataProperty(0, cnt++ , dtData,   		75,		daRight,  	false,  "disp_cfm_qty",    	false,  "",     dfNullInteger,  0,  false,  false);
					//hidden     
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daRight,  	false,  "mnr_disp_rmk");
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daRight,  	false,  "file_seq");
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daRight,  	false,  "disp_local_dt");
			...
			case "t1_sheet1": 
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++ , dtData,			120,	daCenter,	false,	"disp_rlse_no",			false,	"",	dfNone,		0,	false,	false);			//Release No
					InitDataProperty(0, cnt++ , dtData,			100,	daLeft,		false,	"inv_sts_nm",			false,	"",	dfNone,		0,	false,	false);			//Invoice Status
					InitDataProperty(0, cnt++ , dtData,			120,	daLeft,		false,	"mnr_disp_dtl_rmk",		false,	"",	dfNone,		0,	true,	true,	4000);	//Remark(s)
					//Hidden  	   
			...
			case "t2_sheet1":  
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++ , dtData,			120,	daLeft,		false,	"mnr_disp_dtl_rmk",		false,	"",	dfNone,		0,	true,	true,	4000);	//Remark(s)
					//Hidden
					InitDataProperty(0, cnt++ , dtHidden,		0, 		daCenter,	false,	"eq_tpsz_cd",			false,	"",	dfNone,		0,	false,	false);		//TS
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daLeft,  	false,  "disp_yd_nm");	
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daLeft,  	false, 	"file_seq");	 	
					InitDataProperty(0, cnt++ , dtHidden,   	0,		daCenter,  	false,  "disp_no");	 	
					InitDataProperty(0, cnt++ , dtHidden,		0,		daCenter,	false,	"disp_dtl_seq");
    ...
    function doActionIBSheet(sheetObj,formObj,sAction,sActionIdx) {
        ...
        switch(sAction) {
        	...
        	case IBCLEAR:      
				...
				var sXml = sheetObj.GetSaveXml("EES_MNR_S304GS.do", sParam);
			...
			case IBSAVE:
              if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml = sheetObj.GetSearchXml("EES_MNR_S304GS.do", sParam);
					...
					if(vBiddingStatus == "O"){
						...
						var sParam = sParam1 + "&" + sParam2 + "&" + FormQueryString(formObj); 
	            	  	var sXml = sheetObjects[0].GetSaveXml("EES_MNR_S304GS.do", sParam); 
        	...
        	case "PRINT":
        		if(validateForm(sheetObj,formObj,sAction)) {
        			...
        			formObj.com_mrdPath.value = 'apps/alps/ees/mnr/operationmanage/repairmgt/report/EES_MNR_0184.mrd';
	...
	function sheet1_OnSearchEnd(sheetObj,ErrMsg) {
		...
		formObj.disp_local_dt.value = sheetObj.EtcData("disp_local_dt"); 
	}
	/**
	 * 헤더 리스트 더블클릭시 상세조회 
	 * @param sheetObj
	...
	function t2_sheet1_OnChange(sheetObj, Row, Col,Value) {
		with(sheetObj) {					
			...
			//3.입찰은 3번만 가능
				...
				if(sheetObj.CellValue(Row, "vndr_bid_knt") != 0){
					var bidDifAmt = parseFloat(parseFloat(sheetObj.CellValue(Row, "part_ut_amt")) - parseFloat(sheetObj.CellValue(Row, "tmp_part_ut_amt"))).toFixed(0);	 
					if(bidDifAmt != 25){	
...
/* ********* User Functions ************* */
    ...
    function setBtnEdit(biddingStatus){
		...
		for(i=2; i<=3; i++) {
    		for(var j=sheetObjects[i].HeaderRows; j<=sheetObjects[i].LastRow; j++) {
    			var dispRlseNo = sheetObjects[i].CellValue(j, "disp_rlse_no");	//Release No
    			var mnrPrnrSeq = sheetObjects[i].CellValue(j, "mnr_prnr_seq");	//mnr_prnr_seq
	...
	function getMnrPrnrCntCd() {
		...
		for(i=2; i<=3; i++) {
    		for(var j=sheetObjects[i].HeaderRows; j<=sheetObjects[i].LastRow; j++) {
    			mnrPrnrCntCd = sheetObjects[i].CellValue(j, "mnr_prnr_cnt_cd");	//mnr_prnr_cnt_cd
    			mnrPrnrSeq   = sheetObjects[i].CellValue(j, "mnr_prnr_seq");	//mnr_prnr_seq
	 ...
	 * @return
	 */
	function modifyCellValue(sheetObj, sheetIdx) {
		for(var i=sheetObj.HeaderRows; i<=sheetObj.LastRow; i++) {
			var mnrPrnrSeq = sheetObj.CellValue(i, "mnr_prnr_seq");
			if(mnrPrnrSeq=="" || mnrPrnrSeq==null) {
				sheetObj.CellValue(i, "disp_rlse_no") = ""; //Release No
				if(sheetIdx==2) {
					sheetObj.CellValue(i, "eq_no") = "";  //EQ No
				}
			}
		}
	}

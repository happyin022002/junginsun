...
* 2011.09.01 신혜정 [CHM-201113261-01] ALPS MNR Disposal Request 화면에서 Exchange Rate
* 2011.11.22 김상수 [CSR 선처리] Bidding 시, Starting Date와 Closing Date 수정 건
                                 - 미주지역 global buyer들은 만2일밖에 되지 않아 만3일로 정정
...
* 2013.02.04 조경완 [CHM-201322692-01] 중복 RQST되는 Disposal No 삭제 및 Logic 검토/보완  요청
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class EES_MNR_0156 : EES_MNR_0156 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_MNR_0156() {
...
//에러 로우 배경색
var errRowColor = 12510975;  //RgbColor(255, 230, 190) 값
    	...
    	try {
    		var srcName = window.event.srcElement.getAttribute("name");
				switch(srcName) {
					...
					case "btn_Save":
						if(formObject.self_ofc_cd.value == "MEXBA"){
		            ...
		            case "btns_DownFile":
		            	// Contract or Bidding에 따라 다른 엑셀 파일로 다운로드
		            	if(tabObjects[0].SelectedIndex == 0){
			            	if(formObject.disp_tp_cd.Code == "C"){
				            	// 시트 설정시 빈줄을 넣었으나, 모든 시트를 초기화하는 기존 로직들이 있는 관계로, 여기에서 다시 추가 처리
				            	sheetObjects[6].RemoveAll();
				            	sheetObjects[6].DataInsert(-1);
				            	sheetObjects[6].Down2Excel();
			            	} else if (formObject.disp_tp_cd.Code == "B"){
				            	// 시트 설정시 빈줄을 넣었으나, 모든 시트를 초기화하는 기존 로직들이 있는 관계로, 여기에서 다시 추가 처리
				            	sheetObjects[7].RemoveAll();
				            	sheetObjects[7].DataInsert(-1);
				            	sheetObjects[7].Down2Excel();
			            	}
		            	}
		                break;
					...
					case "btn_Inquiry":
						if(tabObjects[0].SelectedIndex == 0){
							ComOpenPopup('/hanjin/EES_MNR_0158.do?mnr_disp_trf_tp_cd=' + formObject.disp_tp_cd.Code + '&cost_ofc_cd=' + formObject.self_ofc_cd.value, 900, 440, 'setPopData_DSPC', '0,1,1,1,1,1,1,1', false, false, 1, 1, 3);
						}
					break;
...
//					InitDataProperty(0, cnt++ , dtHidden,   	0,		daCenter,  	false,   "apro_ofc_cd",     false,      "",     dfNone,    		0,     true,       true);
			...
			case "t1_sheet1":
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++ , dtCheckBox,		40,		daCenter,	true,		"del_chk");
                    InitDataProperty(0, cnt++ , dtSeq,       	30,    	daCenter,  	false,  	"Seq",     			false,      "",    		dfNone     );
					InitDataProperty(0, cnt++ , dtData,			100, 	daLeft,		false,		"eq_no",			true,		"",			dfNone,			0,		true,		true,	14);
			...
			case "sheet5":   // sheet5 init - (hidden) 엑셀 양식 다운로드용 - Sale Type : Contract
                with (sheetObj) {
	                ...
	                var HeadTitle = "     EQ No.     |     U.Price     |     Remark(s)     ";
	                ...
	                // 데이터속성 [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
                    InitDataProperty(0, cnt++, dtData,    200,    daLeft,    false,    "eq_no");
                    InitDataProperty(0, cnt++, dtData,    200,    daRight,   false,    "disp_ut_prc");
                    InitDataProperty(0, cnt++, dtData,    300,    daLeft,    false,    "mnr_disp_dtl_rmk");
			...
			case "sheet6":   // sheet6 init - (hidden) 엑셀 양식 다운로드용 - Sale Type : Bidding
	            with (sheetObj) {
        ...
        switch(sAction) {
			case IBSEARCH:      //조회
				if(validateForm(sheetObj,formObj,sAction)){
				    ...
				    var sXml = sheetObj.GetSaveXml("EES_MNR_0156GS.do", sParam);
			...
			case IBINSERT:      // 행입력
				...
				if(tabObjects[0].SelectedIndex == 0){
					...
					// Bidding건인 경우에는 U.Price를 수정할 수 없다.
					if(formObj.disp_tp_cd.Code == "B"){
						sheetObj.CellEditable(Row, "disp_ut_prc") = false;
					} else {
						sheetObj.CellEditable(Row, "disp_ut_prc") = true;
					}
			...
			case IBSAVE:        //저장
			  // Total Amt 소숫점 2자리 반올림
			  var disp_st_prc_temp  = ComRound(ComGetUnMaskedValue(formObj.disp_st_prc.value, 'float'), 2);
			  formObj.disp_st_prc.value = ComAddComma2(disp_st_prc_temp, "#,###");
              ...
              if(validateForm(sheetObj,formObj,sAction)){
				    ...
				    var sXml = sheetObjects[0].GetSaveXml("EES_MNR_0156GS.do", sParam);
			...
			case IBBATCH:        //request
              if(validateForm(sheetObj,formObj,sAction)){
					...
					formObj.apro_ofc_cd.Code = appOfcdefCode;
			...
			case IBROWSEARCH:      //건당 상세 조회
                 if(validateForm(sheetObj,formObj,sAction)){
						...
						for(var x = 1 ; x <= sheetObjects[3].RowCount;x++){
							//1. Repair Cost를 'Sale Category'옆에 추가해주세요. 2. Manual 입력이며 Damage 인 경우에 입력가능합니다
...
//					    sheetObjects[2].LoadSearchXml(sXml);
						...
						//공통콤보 정보를 가져온다.
						//in_eq_knd_cd,eq_knd_cd 세팅  EQ_TYPE
						formObj.eq_knd_cd.Code = eqKnddefCode;
						formObj.disp_tp_cd.Code = dispTpdefCode;
						formObj.curr_cd.Code = "USD";
						formObj.disp_sts_cd.Code = "HS";
				...
				for(var i = 5; i < comboList.length;i++){
					if(comboList[i] != null){
						...
						if(comboSaveNames[i - 5] == "disp_tp_cd"){
							...
							sheetObjects[3].InitDataCombo (0, comboSaveNames[i - 5], sheetComboText, sheetComboCode ,sheetComboDefault[i - 5]);
						} else if(comboSaveNames[i - 5] == "disp_vrfy_tp_cd"){
							sheetObjects[3].InitDataCombo (0, comboSaveNames[i - 5], sheetComboText, sheetComboCode ,sheetComboDefault[i - 5]);
							sheetObjects[4].InitDataCombo (0, comboSaveNames[i - 5], sheetComboText, sheetComboCode ,sheetComboDefault[i - 5]);
						}
					}
				}
...
//			    sheetObjects[2].LoadSearchXml(sXml);
            ...
            case "dupChk":      //조회
            	...
            	for (var i=sheetObj.HeaderRows; i<=sheetObj.LastRow; i++) {
            		if(i == 1){
            			...
            			eqNoList = eqNoList + "," + sheetObj.CellValue(i, "eq_no");
            		}
            	}
            	formObj.eq_no_list.value = eqNoList;
				sParam = FormQueryString(formObj);
				var sXml = sheetObj.GetSaveXml("EES_MNR_0156GS.do", sParam);
				dubDispNo = ComGetEtcData(sXml,"dub_disp_no");
		...
		with(formObj){
	    	switch(sAction) {
				...
				case IBSAVE:
					...
					//TP/SZ가 있어야 한다.
					for(var x = 1 ; x <= sheetObjects[3].RowCount;x++){
						if(sheetObjects[3].CellValue(x,"eq_tpsz_cd") == ""){
							ComShowCodeMessage("MNR00016");
							sheetObjects[3].SelectCell(x,"eq_no");
							return false;
						}
					}
					...
					if(formObj.disp_no.value == "NEW"){
						doActionIBSheet(sheetObjects[3],formObj,"dupChk");
						
						if(dubDispNo != ""){
							ComShowCodeMessage("MNR00388", dubDispNo);
							return false;
						}
					}
					break;
				...
				case IBBATCH:
					...
					//[CHM-201322692-01] 중복 RQST되는 Disposal No 삭제 및 Logic 검토/보완  요청
					for(var x = 1 ; x <= sheetObjects[3].RowCount;x++){
						if(sheetObjects[3].CellValue(x,"eq_tpsz_cd") == ""){
							ComShowCodeMessage("MNR00016");
							sheetObjects[3].SelectCell(x,"eq_no");
	...
	function sheet2_OnDblClick(sheetObj,Row,Col)
    {
		...
		with(sheetObj){
			...
			formObj.disp_tp_cd.Code = CellValue(Row,"re_disp_tp_cd");  // sale type
...
//			formObj.apro_ofc_cd.Code = CellValue(Row,"apro_ofc_cd");
//			formObj.curr_cd.Code = CellValue(Row,"curr_cd");			
			formObj.apro_ofc_cd.Code = CellValue(Row,"re_apro_ofc_cd");
	...
	function t1_sheet1_OnChange(sheetObj,Row, Col, Value)	{
		...
		if (sheetObj.ColSaveName(Col) == "eq_no"){
			...
			// 엑셀 업로드 오류건의 경우, 배경색 복원
			if (sheetObj.RowBackColor(Row) == errRowColor) {
				sheetObj.Redraw = false;
				restoreRowSetColor(sheetObj, Row);
				sheetObj.Redraw = true;
			}
		...
		} else if((sheetObj.ColSaveName(Col) == "rpr_cost_amt")&& (formObj.disp_tp_cd.Code == "B") ){

			var rpr_cost_amt    = Number(sheetObj.CellValue(Row,"rpr_cost_amt"));
			var disp_ut_prc     = Number(sheetObj.CellValue(Row,"disp_ut_prc"));  
			var disp_trf_ut_prc = Number(sheetObj.CellValue(Row,"disp_trf_ut_prc"));
			if((sheetObj.CellValue(Row,"eq_tpsz_cd") == "D2"||sheetObj.CellValue(Row,"eq_tpsz_cd") == "D4"|| 
			    sheetObj.CellValue(Row,"eq_tpsz_cd") == "D5"||sheetObj.CellValue(Row,"eq_tpsz_cd") == "D7" )) {
			//	if (disp_trf_ut_prc > rpr_cost_amt ) {
					var newprice = 0;
					if (rpr_cost_amt < 51) {
						newprice = disp_trf_ut_prc;
					} else {
						newprice = disp_trf_ut_prc - (rpr_cost_amt -50); 	
					}
					 
					var sixprice = disp_trf_ut_prc * 0.6;
					if (newprice <= sixprice) {
						newprice = sixprice;
					}
					if (newprice != disp_trf_ut_prc) {
						sheetObj.CellValue2(Row,"disp_ut_prc") = (Math.round(newprice/10)) * 10 ;
					} else {
						sheetObj.CellValue2(Row,"disp_ut_prc") = newprice;
					}
					
				
			//	} else {
			//		sheetObj.CellValue2(Row,"rpr_cost_amt") = 0;
			//		sheetObj.CellValue2(Row,"disp_ut_prc")  = sheetObj.CellValue(Row,"disp_trf_ut_prc"); 
			//		sheetObj.SelectCell(Row,Col);
			//	}
			}
	      // By Hwang   		
	...
	function disp_tp_cd_OnChange(comboObj,Index_Code, Text){
		...
		var formObj  = document.form;
		var toDayTime = ComGetNowInfo("ymd")+" "+ComGetNowInfo("hms");

		//bidding
		if(comboObj.Code == "B"){
			MnrFormSetReadOnly(formObj,false,"disp_st_dt|disp_end_dt|disp_bultn_dt|disp_pkup_st_dt|disp_pkup_end_dt");
			ComSetObjValue(formObj.disp_bultn_dt,toDay); // Posting DT
			ComSetObjValue(formObj.disp_st_dt,toDayTime); // Start/Close Time
			ComSetObjValue(formObj.disp_end_dt,ComGetDateAdd(toDay, "D", 4) + " 23:59:59");
			ComSetObjValue(formObj.disp_pkup_st_dt,ComGetDateAdd(toDay, "D", 5)); // Pick Up Period
			ComSetObjValue(formObj.disp_pkup_end_dt,ComGetDateAdd(toDay, "D", 12));
		} else {
			// Contract
			MnrFormSetReadOnly(formObj,true,"disp_st_dt|disp_end_dt|disp_bultn_dt");
			ComSetObjValue(formObj.disp_bultn_dt,""); // Posting DT
			ComSetObjValue(formObj.disp_st_dt,""); // Start/Close Time
	...
	*/
	...
	function post_excel_upload(sheetObj,Row)	{
		var formObj = document.form;

		var checkEqn = sheetObj.CellValue(Row,"eq_no");
		var checkEqType = formObj.eq_knd_cd.Code;
		var dispNo = formObj.disp_no.value;
		var retArray = MnrGeneralCodeCheck(sheetObjects[0],"DSPEQN",checkEqn + "," + checkEqType + ","+ dispNo);
		
		// insert 초기화 처리 : Add Row 이벤트와 동일
		sheetObj.CellValue2(Row,"disp_ut_tp_cd") = "E";
		sheetObj.CellValue2(Row,"eq_tpsz_cd") = "";
		sheetObj.CellValue2(Row,"disp_qty") = "1";
		sheetObj.CellValue2(Row,"mkr_nm") = "";
		sheetObj.CellValue2(Row,"disp_vrfy_tp_cd") = "";
		...
		// 직접 Add Row시에는 이와 같이 설정되므로
		sheetObj.CellValue2(Row,"disp_rsn_cd") = "C";
		
		if(retArray == null || retArray[0] == null){
			errRowSetColor(sheetObj, Row);  // 색깔로 라인 구분 표시
			ComShowCodeMessage("MNR00165",checkEqn,"EQ No.");	
		} else {	
			var tempText = retArray[0].split("|");
			if(tempText[1] != 'OK'){
				...
				sheetObj.CellEditable(Row, "disp_yd_cd") = true;
				post_excel_upload_set(sheetObj,formObj.eq_knd_cd.Code,sheetObj.CellValue(Row,"eq_no"),ComGetNowInfo("ymd"),Row);
			}
		}
		// Bidding건인 경우에는 U.Price를 수정할 수 없다.
		if(formObj.disp_tp_cd.Code == "B"){
	...
	function post_excel_upload_set(sheetObj,sEqType,sEqNo,sTotalLossDate,Row){
		var formObj = document.form;
		var sCostType = "";
		if(sEqType == "U"){
			sCostType = "MRDRRC";
    ...
    function errRowSetColor(sheetObj, row) {
        with(sheetObj) {
            RowBackColor(row) = errRowColor;
        }
    }

    // 색처리 복원
    function restoreRowSetColor(sheetObj, row) {
        with(sheetObj) {
            RowBackColor(row) = 0;  // 매뉴얼의 -1이 적용되지 않음
        }
    }

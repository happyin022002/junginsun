...
/**
 * @extends
 * @class EES_MNR_0120 : EES_MNR_0120 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_MNR_0120() { 
    ...
    switch(sheetID) {    	
        case "sheet1":
            with (sheetObj) {
	            ...
	            InitDataProperty(0, cnt++ , dtCombo,		70, daLeft,		false,	"eq_type",		false,	"",      dfNone     );	                  
	            InitDataProperty(0, cnt++ , dtData,			70,	daCenter,	false,	"tpsz",			false,	"",      dfNone	    );
	            ...
	            InitDataProperty(0, cnt++ , dtData,			70, daCenter,	false,	"yard_cd",		false,	"",      dfNone     );                  
	            InitDataProperty(0, cnt++ , dtData,			80, daLeft,		false,	"vndr_seq",		false,	"",      dfNone     );
	            InitDataProperty(0, cnt++ , dtData,			100,daLeft,		false,	"sp_nm",		false,	"",      dfNone     );
	            InitDataProperty(0, cnt++ , dtData,			70, daCenter,	false,	"fqa_qty",		false,	"",      dfNone     );
	            InitDataProperty(0, cnt++ , dtData,			70,	daCenter,	false,	"fqa_dt",		false,	"",      dfNone   	);
	            InitDataProperty(0, cnt++ , dtData,			70,	daCenter,	false,	"curr",		    false,	"",      dfNone   	);
	            InitDataProperty(0, cnt++ , dtAutoSum,		70,	daRight,	false,	"unit",			false,	"",      dfInteger	);	                  
	            InitDataProperty(0, cnt++ , dtAutoSum,		100,daRight,	false,	"amt",			false,	"",      dfNullFloat,	2);
				InitDataProperty(0, cnt++ , dtAutoSum,		100,daRight,	false,	"avg_amt",		false,	"",      dfNullFloat,	2);
	            InitDataProperty(0, cnt++ , dtData,			100,daRight,	false,	"avg_days",		false,	"",      dfInteger     );      
...
//Sheet관련 프로세스 처리
 function doActionIBSheet(sheetObj, formObj, sAction) {
     ...
     switch(sAction) {
		...
		case IBSEARCH_ASYNC02:	//조회(RHQ 콤보 변경시)	
			sheetObj.WaitImageVisible = false;
			if ( validateForm(sheetObj, formObj, sAction) ) { 
...
function eq_type_OnChange(comboObj,Index_Code, Text){ 
	...
	if(selTpSz.length == 0){ 
 		...
 		formObj.tp_sz_cd.Enable = true;		
		formObj.tp_sz_cd.InsertItem(0,"ALL","ALL");   		
...
function sheet1_OnSearchEnd(sheetObj,ErrMsg){
	...
	var bFloat = parseFloat(sheetObj.SumValue(0,"unit") + "");  
	var avgFloat = 0;	    
	if(bFloat != 0){ 			 

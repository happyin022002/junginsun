...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends
     * @class EES_MNR_0245 : EES_MNR_0245 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
			...
			case "sheet1":
				with(sheetObj) {
					...
					for(var i = 0; i < 4; i++) {
						...
						InitDataProperty(i, cnt++ , dtData,			100,	daCenter,	true,	"apro_mon",			false,	"",	dfDateYm);
			...
			case IBSEARCH:			//조회
				if(validateForm(sheetObj, formObj, sAction)) {
					if(sheetObj.id == "sheet1") {
						...
						ComOpenWait(true);
						var sXml = sheetObj.GetSearchXml("EES_MNR_0245GS.do", FormQueryString(formObj));
			...
			if(LastRow > 3) {
				...
				for(var i = LastRow -4; i > LastRow -8; i--) {
					for(var j = 0; j < LastCol; j++) {
						...
						if(ColSaveName(j) == "apro_mon") {
							CellAlign(i +4, "apro_mon") = daCenter;
						}
					}
				}
				...
				sheetObj.CellValue2(LastRow -3, "apro_mon") = "G.TTL";
				sheetObj.CellValue2(LastRow -2, "apro_mon") = "G.AMT";
				sheetObj.CellValue2(LastRow -1, "apro_mon") = "G.AVG";
				sheetObj.CellValue2(LastRow, "apro_mon") = "Ratio";
			}
    	}
    }

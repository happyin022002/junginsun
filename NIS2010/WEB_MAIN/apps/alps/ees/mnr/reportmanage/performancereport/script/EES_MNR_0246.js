...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends
     * @class EES_MNR_0246 : EES_MNR_0246 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_MNR_0246() {
			...
			case "sheet1":
				with(sheetObj) {
					...
					for(var i = 0; i < 3; i++) {
						...
						InitDataProperty(i, cnt++ , dtData,			60,		daCenter,	true,	"cust_cd",			false,	"",	dfNone);
						InitDataProperty(i, cnt++ , dtData,			150,	daLeft,		true,	"cust_nm",			false,	"",	dfNone);
						InitDataProperty(i, cnt++ , dtData,			60,		daCenter,	true,	"curr_cd",			false,	"",	dfNone);
						InitDataProperty(i, cnt++ , dtHidden,		50,		daCenter,	true,	"curr_cnt",			false,	"",	dfNone);
						...
						InitDataProperty(i, cnt++ , dtData,			60,  	daRight,	false,	"rato_tpsz_dp00",	false,	"",	dfNone);
			...
			case IBSEARCH:			//조회
				if(validateForm(sheetObj, formObj, sAction)) {
					if(sheetObj.id == "sheet1") {
						...
						ComOpenWait(true);
						var sXml = sheetObj.GetSearchXml("EES_MNR_0246GS.do", FormQueryString(formObj));
	...
	function sheet1_OnDblClick(sheetObj, Row, Col, CellX, CellY, CellW, CellH) {
		...
		var vVndrNm = sheetObj.CellValue(Row,"cust_nm");
		...
		switch (sName) {
			...
			case "pfmc_tpsz_dp28": case "pfmc_tpsz_dp29": case "pfmc_tpsz_dp30":
				...
				params += "&h_cust_cd="+ sheetObj.CellValue(Row,"cust_cd");

				if(vVndrNm == "S.TTL") {
					params += "&h_vndr_nm="+ sheetObj.CellValue(Row -3,"cust_nm");
					params += "&h_curr_cd=";
				} else {
					params += "&h_vndr_nm="+ sheetObj.CellValue(Row,"cust_nm");
    	...
    	with(sheetObj) {
			...
			if(LastRow > 2) {
				for(var i = 0; i < LastRow -3; i++) {
					if(CellValue(i, "cust_nm") == "S.TTL" ) {
						RowBackColor(i) = MNR_TOTCOL_BACK_COLOR;
						CellAlign(i, "cust_nm") = daCenter;
					}
				}
				...
				if(480 + (viewCnt * 80) > 984) {
					SheetWidth = 984;
				} else {
					SheetWidth = 490 + (viewCnt * 80);
				}
				...
				sheetObj.CellValue2(LastRow -2, "cust_cd") = "G.TTL";
				sheetObj.CellValue2(LastRow -1, "cust_cd") = "G.TTL";
				sheetObj.CellValue2(LastRow, "cust_cd") = "G.TTL";

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class EES_LSE_0025 : EES_LSE_0025 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function EES_LSE_0025() {
    	...
    	try {
            ...
            switch(srcName) {
				...
				case "btn_loadexcel":
					...
					ComEnableObject(formObj.btns_search2, false);
					ComSetFocus(formObj.agmt_seq);
					sheetObject.LoadExcel(0, 1, "", -1, -1, "", false,false,"1=>4");
					formObj.cntr_no_list.value = "";
					for (var i = 2; i <= sheetObject.RowCount+1; i++){
						if (formObj.cntr_no_list.value == ""){
							formObj.cntr_no_list.value = sheetObject.CellValue(i,4);
						} else {
							formObj.cntr_no_list.value = formObj.cntr_no_list.value + "," + sheetObject.CellValue(i,4);
						}
					}
					if (sheetObject.RowCount > 0){
					   doActionIBSheet(sheetObject,formObj,IBSEARCH_ASYNC05);
					} else {
	            		ComShowCodeMessage("LSE00001","Row");
	            		return;							
					}						
        ...
        switch(sheetNo) {
            case 1:      //t1sheet1 init
                with (sheetObj) {
					...
					var HeadTitle = "|Seq.|Imme Exit|AGMT No.|CNTR No.|TP/SZ|On-hire|On-hire|Current Yard|MVMT|ACT Date|F/M||";
                    var HeadTitle1 = "|Seq.|Imme Exit|AGMT No.|CNTR No.|TP/SZ|Date|Yard|Current Yard|MVMT|ACT Date|F/M||";
                    ...
                    InitDataProperty(0, cnt++ , dtData,     	80,   	daCenter,  	true,   "agmt_seq",     	false,  "",	dfNone,     0,		false,		false);
                    InitDataProperty(0, cnt++ , dtData,     	100,   	daCenter,  	true,   "cntr_no",     	false,  "",	dfNone,     0,		false,		false, 11);
                    ...
                    InitDataProperty(0, cnt++ , dtData,      	80,   	daCenter,  	true,   "onh_yd_cd",  	false,  "",	dfNone,     0,		false,		false);
                    InitDataProperty(0, cnt++ , dtData,      	80,   	daCenter,  	true,   "crnt_yd_cd",   false,  "",	dfNone,     0,		false,		false);
                    InitDataProperty(0, cnt++ , dtData,      	50,   	daCenter,  	true,   "cnmv_sts_cd",  false,  "",	dfNone,     0,		false,		false);
                    InitDataProperty(0, cnt++ , dtData,      	80,   	daCenter,  	true,   "cnmv_dt",  	false,  "",	dfDateYmd,  0,		false,		false);
                    InitDataProperty(0, cnt++ , dtData,      	40,    	daCenter,  	true,   "full_flg",     false,  "",	dfNone,     0,		false,		false);
			...
			case IBSEARCH:			//조회
				if(validateForm(sheetObj, formObj, sAction)) {
					if(sheetObj.id == "sheet1") {
						...
						//------------------------------------ dtCheckBox에서의 CheckAllImageVisible 버그발생(2010.01.21)
						//sheetObj.SpeedOption = "NOFIT,NOSUM,NOCALC,NOROWHEIGHT, NOMERGEROW, NOTRIM, NOTDTAG, NOCOMBO";
          				//sheetObj.DoSearch("EES_LSE_0025GS.do",FormQueryString(formObj));
						formObj.cntr_no_list.value = '';
					}
				}
				break;
			...
			case IBSAVE:			//저장
				if(validateForm(sheetObj, formObj, sAction)) {
					if(sheetObj.id == "sheet1") {
						...
						sheetObj.DoSave("EES_LSE_0025GS.do", FormQueryString(formObj));
					}
				}
				break;

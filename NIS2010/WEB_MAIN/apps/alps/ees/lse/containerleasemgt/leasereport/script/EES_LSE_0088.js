...
/**
 * @extends
 * @class EES_LSE_0088 : EES_LSE_0088 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
	...
	case "sheet1":
		with (sheetObj) {
			...
			for(var i = 0; i < 2; i++) {
				...
				InitDataProperty(i, cnt++ , dtData,             55,    daRight,  true,  "ratio",       false,  "",     dfNone,   0,  false,    true);
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	if(validateForm(sheetObj,formObj,sAction))
		if (sheetObj.id == "sheet1"){
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("EES_LSE_0088GS.do", FormQueryString(formObj));

...
/**
 * @extends
 * @class EES_LSE_0039 : EES_LSE_0039 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_LSE_0039() {
...
function doActionIBSheet(sheetObj,formObj,sAction, CondParam, PageNo) {
	...
	if(validateForm(sheetObj,formObj,sAction))
		if (sheetObj.id == "sheet1"){
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("EES_LSE_0039GS.do", FormQueryString(formObj));
	...
	if(validateForm(sheetObj,formObj,sAction)) {
		if(sheetObj.id == "sheet2") {
			...
			sheetObj.WaitImageVisible = false;
			ComOpenWait(true);
			sheetObj.DoSearch("EES_LSE_0039GS2.do",FormQueryString(formObj));
			ComOpenWait(false);
		}
	}
	break;
	...
	case IBSEARCHAPPEND:	//조회(페이징처리)
		if(sheetObj.id == "sheet2") {
			...
			sheetObj.WaitImageVisible = false;
			ComOpenWait(true);
			sheetObj.DoSearch4Post("EES_LSE_0039GS2.do", CondParam, "iPage="+ PageNo, true);
			ComOpenWait(false);
		}
		break;

...
/**
 * @extends
 * @class EES_LSE_0090 : EES_LSE_0090 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function EES_LSE_0090() {
	...
	case "sheet1":
		with (sheetObj) {
			...
			for(var i = 0; i < 2; i++) {
				...
				InitDataProperty(i, cnt++ , dtData,             80,    daCenter, true,  "rcc",         false,  "",     dfNone);
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	if(validateForm(sheetObj,formObj,sAction))
		if (sheetObj.id == "sheet1"){
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("EES_LSE_0090GS.do", FormQueryString(formObj));

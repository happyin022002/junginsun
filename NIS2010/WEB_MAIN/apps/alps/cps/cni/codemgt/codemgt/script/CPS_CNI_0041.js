 ...
 * 2009.04.17 진윤오
 * 1.0 Creation
 * 2010.06.18 윤진영 SRM-201006196 delf_flg 체크 인/아웃시 retrieve 실행  
 * 2010.06.21 윤진영 SRM-201006196 조회조건에 아무것도 없을때 uncheck out 시 retrieve 미실행으로 수정
 * 2010.07.01 윤진영 CHM-201004242 Grid Title 변경 Vender -> Refund / Vndr Code 
=========================================================*/
...
function initSheet(sheetObj, sheetNo) {
	...
	with (sheetObj) {
		...
		case "sheet1": 
			...
			InitDataProperty(0, cnt++ , dtData,			130,    daCenter,	true,		"vndr_seq",			false,		"",				dfNone,		0,			true,		true);			
...
function processButtonClick() {
	...
	switch (srcName) {	
	    ...
	    case "btn1_Select":
	    	...
	    	var partyVo = {
	    			clm_pty_no:sheet1.CellValue(row , "clm_pty_no"),
	    			clm_pty_abbr_nm:sheet1.CellValue(row , "clm_pty_abbr_nm"),
	    			pty_nm:sheet1.CellValue(row , "pty_nm"),
	    			intl_phn_no:sheet1.CellValue(row , "intl_phn_no"),
	    			phn_no:sheet1.CellValue(row , "phn_no"),			
	    			loc_cd:sheet1.CellValue(row , "loc_cd"),
	    			pty_eml:sheet1.CellValue(row , "pty_eml")
	    	};
...
function doActionIBSheet(sAction) {	
	if (sAction == SEARCHLIST01) {
		...
		var sXml = sheet1.GetSearchXml("CPS_CNI_0041GS.do", FormQueryString(frm));

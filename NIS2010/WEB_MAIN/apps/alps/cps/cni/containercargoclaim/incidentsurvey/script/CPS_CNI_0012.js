 ...
 */
 function setCurrency(rowArray) { 
    frm.svyr_locl_curr_cd.value = rowArray[0][3];
	if(rowArray[0][3] == "USD"){
...
function doActionIBSheet(sAction) {
	...
	if (sAction == SEARCHLIST01) {
		...
		var sXml = sheet1.GetSearchXml("CPS_CNI_0012GS.do", FormQueryString(frm),"",true);		
		...
		if (arrXml.length > 0) {			
			...
			if (list.length > 0) {
				...
				frm.n3rd_labl_pty_cd.value = surveyVO["n3rd_labl_pty_cd"];
				...
				ComSetObjValue(frm.svyr_tp_cd, surveyVO["svyr_tp_cd"]);
		...
		param += "&" + saveString;	
		var sXml = sheet1.GetSaveXml("CPS_CNI_0012GS.do", param);		

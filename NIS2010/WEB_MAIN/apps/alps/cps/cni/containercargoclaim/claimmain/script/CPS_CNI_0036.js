...
/**
 * [CPS_CNI_0036] Incident-Claim Inquiry
 * @extends
 * @class Find 생성을 위한 화면에서 사용하는 업무 스크립트를
 *        정의한다.
 */
function CPS_CNI_0036() {
	...
	switch(sheetObj.id) {
		case "sheet1":      //sheet1 init
			with (sheetObj) {
				...
				InitDataProperty(0, cnt++ , dtHidden,	80,	daCenter,	true,		"trns_area_cd",		false,      "",				dfNone,		0,			false,		false);
				...
				InitDataProperty(0, cnt++ , dtHidden,	100,daCenter,	true,		"trns_seq",		    false,	    "",				dfNone,		0,			false,		false);
	...
	if(sheet.RowCount <= 0 )  {
		...
		for(var row = 1; row <= sheet1.RowCount+1 ; row++){
			if(sheet1.CellValue(row,"div").toUpperCase()  == "FROM"){
				...
				sheet1.CellFontColor(row, "cgo_clm_no") = sheet1.RgbColor(255, 0, 0);
				sheet1.CellFontUnderline(row, "cgo_clm_no") = true;
			...
			if(sheet1.CellValue(row,"div").toUpperCase()  == "TO"){
				...
				if(equalsRole("CNI03") || equalsRole("CNI93")){
					...
					if(usr_area == sheet1.CellValue(row,"trns_area_cd")){
						sheet1.CellEditable(row, "clm_trns_auth_cd") = true;
						sheet1.CellEditable(row, "trns_to_ofc_cd") = true;
						sheet1.CellEditable(row, "trns_to_usr_id") = true;
						sheet1.CellEditable(row, "trns_to_dt") = true;
...
function sheet1_OnClick(sheetObj, row, col, value) {
	...
	if (sheetObj.ColSaveName(col) == "trns_rmk") {
		...
		if(equalsRole("CNI03")|| equalsRole("CNI93")){
			...
			if(frm.usr_area.value.trim() == sheet1.CellValue(row,"clm_area_cd").trim()){
				ComShowMemoPad(sheetObj, null, null, null, null, null, 1300);
			}
		}else if(equalsRole("CNI01")){
...
function sheet1_OnDblClick(sheetObj, row, col) {
	if (col <= 8) {
		var cgoClmNo = sheetObj.CellValue(row , "cgo_clm_no");
...
function doActionIBSheet(sAction, Row) {
	...
	if (sAction == SEARCHLIST01) {
		...
		var sXml = sheet1.GetSearchXml("CPS_CNI_0036GS.do", FormQueryString(frm));		
		...
		var sXml = sheet1.GetSearchXml("CPS_CNI_0036GS.do" , "f_cmd="+SEARCHLIST02+"&trns_to_ofc_cd="+ofcCd);
		...
		var sXml = sheet1.GetSearchXml("CPS_CNI_0036GS.do" , "f_cmd="+SEARCHLIST03+"&trns_to_ofc_cd="+ofcCd+"&trns_to_usr_id="+usrId);
		...
		frm.com_mrdPath.value = "apps/alps/cps/cni/containercargoclaim/claimmain/report/CPS_CNI_0089.mrd";
		popupRd(1000,600);
	} 
}

...
function processButtonClick() {
	...
	try {
		...
		case "btn2_RowAdd2":
			...
			var blNo = sheet1.CellValue(sheet1.SelectRow, "cgo_clm_ref_bl_no");
			if(blNo!='') sheet2.CellValue2(row , "cgo_clm_ref_bl_no") = blNo;	
			break;
		...
		case "btn_Movement":
			...
			if (cntrNo != "" && cntrTpSzCd != "") {
				...
				ComOpenPopup('EES_CTM_0408.do' + paramVal, 1036, 680, "getInqMVMT", "1,0,1,1,1,1,1", false);
			}
			break;
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 0, daCenter, true,
					"cgo_clm_ref_bl_no", true, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++ , dtHidden, 0, daCenter, true, "cgo_clm_no");
			InitDataProperty(0, cnt++ , dtHidden, 0, daCenter, true, "mn_bl_flg");
			
			InitDataValid(0,    "cgo_clm_ref_bl_no",    vtEngUpOther, "1234567890");
	...
	case "sheet2": //sheet2 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtDataSeq, 0, daCenter, true, "seq",
					false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtData, 0, daCenter, true,
					"cgo_clm_ref_cntr_no", true, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtData, 0, daCenter, true, "bl_tp_cd", false,
					"", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 0, daCenter, true,
					"cgo_clm_ref_bl_no", true, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++ , dtHidden, 0, daCenter, true, "cgo_clm_no");
			InitDataProperty(0, cnt++ , dtHidden, 0, daCenter, true, "mn_cntr_flg");
						
			InitDataValid(0,    "cgo_clm_ref_cntr_no",    vtEngUpOther, "1234567890");
			InitDataValid(0,    "cgo_clm_ref_bl_no",    vtEngUpOther, "1234567890");
...
function doActionIBSheet(sAction) {
	...
	if (sAction == SEARCHLIST01) {
		...
		var sXml = sheet3.GetSearchXml("CPS_CNI_0010GS.do",
		...
		var sXml = sheet3.GetSaveXml("CPS_CNI_0010GS.do", param);
...
function sheet1_OnChange(sheetObj, row, col, value) {
	...
	if (sheetObj.ColSaveName(col) == "cgo_clm_ref_bl_no") {
		...
		for( var i=0 ; i <= lastRow ; i++ ) {			
			if (sheet1.CellValue(i, "cgo_clm_ref_bl_no") == value) {
				ComShowCodeMessage("CNI00002" , "B/L No.( "+ value + " )");
				sheet1.CellValue(row, "cgo_clm_ref_bl_no") = "";
			}
		}
	}
}
...
function setSheetCgoClmNo() {
	...
	for(var row=0; row<=sheet1.LastRow; row++) {			
		if (sheet1.RowStatus(row) == "I")  {			
			sheet1.CellValue2(row , "cgo_clm_no") = ccno;				
		}
	}
	
	for(var row=0; row<=sheet2.LastRow; row++) {			
		if (sheet2.RowStatus(row) == "I")  {			
			sheet2.CellValue2(row , "cgo_clm_no") = ccno;				
		}
	}
}
...
function chkInsertSheet1(row) {
	...
	if(cnt>1){
		sheet1.CellValue2(row,"mn_bl_flg") = "N";

	}else if(cnt==1){
		
		sheet1.CellValue2(row,"mn_bl_flg") = "Y";
	}
...
function chkCntrflg() {
	...
	for(var row=0; row<=sheet2.LastRow; row++) {
		flg = sheet2.CellValue(row , "mn_cntr_flg");
...
function setCntrflg() {
	...
	if(!chkCntrflg()){		
		for(var row=0; row<=sheet2.LastRow; row++) {			
			if (sheet2.RowStatus(row) == "I")  {			
				sheet2.CellValue2(row,"mn_cntr_flg") = 'Y';
				return;
			}
		}
	}
...
function setRefBlNo(){
	var flg = '';
	for(var row=0; row<=sheet1.LastRow; row++) {
		flg = sheet1.CellValue(row , "mn_bl_flg");
			
		if(flg=='Y'){
			frm.cgo_clm_ref_bl_no.value = sheet1.CellValue(row, "cgo_clm_ref_bl_no");
			return;
		}
	}
}
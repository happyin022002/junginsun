...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class Vessel VVD & Name Inquiry : Vessel VVD & Name Inquiry 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function CPS_CNI_0306() {
    ...
    function initControl() {
        ...
        axon_event.addListener ('keyup', 'obj_keyup' , "vvd_cd");							//- VVD가 4자리  입력됐을때 keyup 이벤트 발생시
    ...
    /**
     * HTML Control의 Vessel Code 4자리 입력시 Vessel Name을 조회한다. <br>
     **/
    function obj_keyup(){
    	switch(event.srcElement.name){
	        case "vvd_cd":
	        	...
	        	//대문자인 경우만 실행
	        	if ((keyValue >= 65 && keyValue <= 90) || (keyValue >= 96 && keyValue <= 105)) {
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++ , dtData,		20,		daCenter,	true,		"skd_dir_cd",			false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		150,	daLeft,		true,		"vsl_eng_nm",			false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		45,		daCenter,	true,		"tov",					false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		45,		daCenter,	true,		"vsl_own_ind_cd",		false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		45,		daCenter,	true,		"slan_cd",				false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		75,		daCenter,	true,		"lnch_dt",				false,  "",	dfDateYmd,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		150,	daLeft,		true,		"cnt_nm",				false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		25,		daCenter,	true,		"vsl_rgst_cnt_cd",		false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		45,		daCenter,	true,		"blk_vsl_clss_cd",		false,  "",	dfNone,		0,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		70,		daRight,	true,		"grs_rgst_tong_wgt",	false,  "",	dfFloat,	3,	false,		false);
					InitDataProperty(0, cnt++ , dtData,		70,		daRight,	true,		"smr_dwt_wgt",			false,  "",	dfFloat,	3,	false,		false);
               }
                break;
        }
    }
				...
				if (Col == "vvd_cd") {//VVD 조회
					...
					var obj = eval("document."+formObj.name+"."+Col);
		   			...
		   			if(typeof result == "undefined" || result == "" ) {
						...
						ComAlertFocus(formObj.vvd_cd, ComGetEtcData(sXml, "errMsg"));
						return;
					} else {
	...
	*/
	function sheet1_OnDblClick(sheetObj, row, col) {	
		...
		var vvdVo = {
			vvd_cd:sheetObj.CellValue(row , "vsl_cd")+sheetObj.CellValue(row , "skd_voy_no")+sheetObj.CellValue(row , "skd_dir_cd"),
			vsl_eng_nm:sheetObj.CellValue(row , "vsl_eng_nm")
		};

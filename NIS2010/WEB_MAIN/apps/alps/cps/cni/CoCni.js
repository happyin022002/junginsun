...
msgs["CNI00037"] = "Impossible to Draft. Not available CODL [UI] Code!";
msgs["CNI00038"] = "Can't be Created New Claim No!";
msgs["CNI00039"] = "Impossible to Case Closed. Not available CODL [UI] Code!";
msgs["CNI00110"] = "Approval Status is pending or complete !";
msgs["CNI00111"] = "Please map S/Provider with MDM Code on Main Code Creation !";
...
msgs["CNI09068"] = "E-mail Address {?msg1} is invalid."
msgs["CNI09069"] = "There is no changed data";
...
function popupLocation() {
	...
	ComOpenPopup("COM_ENS_051.do", 770, 450, "setLocation", display);
...
function popupClientDefaultSetup() {	
	var url = "CPS_CNI_0001.do?popupYn=Y";
...
function popupVessel() {	
	...
	//http://localhost:7001/hanjin/COM_ENS_0A1.do?vsl_cd=&vsl_eng_nm=&car_cd=&classId=COM_ENS_0A1&pop_mode=1&display=1,0,1,1,1,1,1&func=getCOM_ENS_0A1_1
}
...
function popupVVD() {	
	...
	//http://localhost:7001/hanjin/COM_ENS_0B2.do?etdeta=A&sdate=&edate=&vvd_cd=&loc_cd=&lane_cd=&oper=&classId=COM_ENS_0B2&pop_mode=1&display=1,0,1,1,1,1,1,1&func=getCOM_ENS_0B2_1
}
...
function popupClaimMainView(cgoClmNo) {	
	if (cgoClmNo == null || cgoClmNo == undefined) {
 	...
 	var reqWin = openWinCenter(url,winName,1024,580);	
 	reqWin.focus();
 	return reqWin;
}
...
function CoCniGetCombo(formObj, sheetObj, cdFlag, cdId, comboCode, comboText, nullYn) {
	...
	var sXml = sheetObj.GetSearchXml("CPS_CNI_0301GS.do" , FormQueryString(formObj)+param);
...
function CoCniSetMakeMultiCombo(formObj, comboText, comboCode, Col, msgCd) {
	if(typeof comboCode != "undefined" && comboCode != "") {
 		...
 		var objCombo = eval("document.all.combo_"+Col);
            ...
            if (ComTrim(sVal)=="") {
                ...
                if (nTabPos != null) { 
                    if (ComParseInt(nTabPos) < 3){
                	   tabObjects[0].SelectedIndex = ComParseInt(nTabPos);
                    } else {
                       tabObjects[1].SelectedIndex = (ComParseInt(nTabPos) - 3) 
                    }
                }   

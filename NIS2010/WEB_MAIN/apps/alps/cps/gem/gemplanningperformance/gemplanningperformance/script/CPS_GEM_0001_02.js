 ...
 * 보완: Committee 의 경우, 모든 Office, Expense에 대하여 Transfer 할 수 있도록 보완 
        Form Office, Expense 선택시 To office, Expense 제약 조건 제거
=========================================================*/
...
function checkExpnInfo(div) {
	...
	if (div == "FM") {
		...
		// [CHM-201432499] 2014.10.23 JBLEE
		// [Start] - Commmit 일 경우, 모든 Office, 비용에 대하여 이관 가능
		var authFlg          = frm.auth_flg.value;	    
		var fmGenExpnGroupCd = frm.fm_gen_expn_group_cd.value;
		var toGenExpnCd      = frm.to_gen_expn_cd.value;
		...
		// [End] - Commmit 일 경우, 모든 Office, 비용에 대하여 이관 가능
		
		param += "&gen_expn_group_cd=" + fmGenExpnGroupCd;
		param += "&gen_expn_cd=" + toGenExpnCd ;
...
function processButtonClick() {
	...
	case "t2btns_Authorized2":
		...
		// [CHM-201432499] 2014.10.23 JBLEE
		// Commmit 일 경우, 모든 Office, 비용에 대하여 이관 가능 - Start
		var authFlg          = frm.auth_flg.value;	    
		var fmGenExpnGroupCd = frm.fm_gen_expn_group_cd.value;
		var fmTicCd          = frm.fm_tic_cd.value;
		...
		// Commmit 일 경우, 모든 Office, 비용에 대하여 이관 가능 - End		
		
		var param = "lang_div=" + ComGetObjValue(frm.lang_div);
		param += "&ofc_cd=" + frm.to_ofc_lvl3.value;
		param += "&usr_tic_cd=" + fmTicCd;
		param += "&gen_expn_group_cd=" + fmGenExpnGroupCd;
		param += "&sel_div=S";
		var url = "CPS_GEM_0101.do?"+param;
		var winName = "CPS_GEM_0101";
	...
	case "btn_Print":
		if (!ComIsNull(frm.gen_expn_rqst_no.value)) {
		    ...
		    frm3.mrd.value = "apps/alps/cps/gem/gemplanningperformance/gemplanningperformance/report/CPS_GEM_0024.mrd";
...
function onChangeToOfcLvl1() {
	...
	// Commmit 일 경우, 모든 Office, 비용에 대하여 이관 가능 - Start
	var authFlg   = frm.auth_flg.value;
	var fmOfcLvl3 = frm.fm_ofc_lvl3.value	

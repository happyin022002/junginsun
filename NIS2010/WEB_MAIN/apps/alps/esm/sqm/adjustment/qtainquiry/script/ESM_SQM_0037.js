...
/**
 * @extends
 * @class ESM_SQM_0037 : ESM_SQM_0037 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0037() {
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0037GS.do", FormQueryString(formObj));
			...
			if (arrXml.length > 5)
				ComXml2ComboItem(arrXml[5], formObj.f_ofc_vw_cd, "code", "name");
			if (arrXml.length > 6)
...
//			if (!validateForm(sheetObj, formObj, sAction)) return; // 3Q에 Week를 40주로 검색하거나 하는 경우에 validation이 걸려, 해제함(실제로 데이터는 저런 경우가 있으므로)

			ComOpenWait(true);

			formObj.f_cmd.value = SEARCH;

			sheetObj.DoSearch("ESM_SQM_0037GS.do", FormQueryString(formObj));
...
//			ComOpenWait(false);
		...
		case "RawDataDownExcel":		// Raw Data 엑셀 다운로드
			...
			document.location.href = "ESM_SQM_0037DL.do?" + param;
...
function rlane_change() {
	...
	if ( bse_yr != "" && ofc_vw_cd != "" && trd_cd != "" && trd_cd != "All" ) {
		param =       bse_tp_cd
              + "|" + bse_yr
              + "|" + bse_qtr_cd
              + "|" + trd_cd;
...
function validateForm(sheetObj, formObj, sAction){
	switch(sAction) {
		case IBSEARCH:  // 화면 조회시
			var bse_qtr_cd = ComGetObjValue(formObj.f_bse_qtr_cd); //Quarter
			var period_fm  = "";
			var period_to  = "";
			...
			if ( formObj.chk_week.checked ) {
				...
				if ( period_fm != "" || period_to != "" ) {
					if (   period_fm == ""
						...
						ComShowCodeMessage('SQM00025', bse_qtr_cd);
						return false;
					}
				}
			} else {

...
/**
 * @extends
 * @class ESM_SQM_0225 : ESM_SQM_0225 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0225() {
		...
		switch(srcName) {
			case "btn_Retrieve":
				if(formObj.rdoOp_cd[0].checked){
					if(ComGetObjValue(formObj.f_year_tp_cd) == "I"){
						doActionIBSheet(sheetObj, formObj, IBSEARCH); // Initially fixed
					}else if(ComGetObjValue(formObj.f_year_tp_cd) == "C"){
						doActionIBSheet(sheetObj, formObj, SEARCH01); // Currently Updated
					}
			...
			case "btn_RawDataDownExcel":				
				if(formObj.rdoOp_cd[0].checked){
					if(ComGetObjValue(formObj.f_year_tp_cd) == "I"){
						doActionIBSheet(sheetObj, formObj, "RawDataDownExcel"); // Initially fixed
					}else if(ComGetObjValue(formObj.f_year_tp_cd) == "C"){
						doActionIBSheet(sheetObj, formObj, "RawDataDownExcelCurrentlyUpdated"); // Currently Updated
					}
				}else if(formObj.rdoOp_cd[1].checked){
					if(ComGetObjValue(formObj.f_year_tp_cd) == "I"){
						doActionIBSheet(sheetObj, formObj, "RawDataDownExcelPreviousVersion"); // Initially fixed
					}else if(ComGetObjValue(formObj.f_year_tp_cd) == "C"){
						doActionIBSheet(sheetObj, formObj, "RawDataDownExcelCurrentlyUpdatedPreviousVersion"); // Currently Updated
					}
				}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0225GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시 (Initially Fixed)
			if (!validateForm(sheetObj, formObj, sAction)) return;
			ComOpenWait(true);
			formObj.f_cmd.value = SEARCH;
			searchParams = FormQueryString(formObj);
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0225GS.do", searchParams);
			sheetObj.LoadSearchXml(rtnXml);
			ComOpenWait(false);
			break;
			
		case SEARCH01:          // 화면 조회 시 (Currently Updated)
			if (!validateForm(sheetObj, formObj, sAction)) return;
			ComOpenWait(true);
...
//			ComOpenWait(false);
		...
		case "RawDataDownExcel":		// Raw Data 엑셀 다운로드
			...
			document.location.href = "ESM_SQM_0225DL.do?" + param;

			ComOpenWait(false);
			break;
			
		case "RawDataDownExcelCurrentlyUpdated":		// Raw Data 엑셀 다운로드
			ComOpenWait(true);
			//Year, Quarter, Office view user 선택. office level은 항상 office, week, vvd, pol-pod pair check
			...
			document.location.href = "ESM_SQM_0225DL2.do?" + param;
		...
		case "RawDataDownExcelCurrentlyUpdatedPreviousVersion":		// Raw Data 엑셀 다운로드
			ComOpenWait(true);
			//Year, Quarter, Office view user 선택. office level은 항상 office, week, vvd, pol-pod pair check
			var param = "f_cmd="         + COMMAND04
			          + "&f_bse_tp_cd="  + ComGetObjValue(formObj.f_bse_tp_cd)
			          + "&f_bse_yr="     + ComGetObjValue(formObj.f_bse_yr)
			          + "&f_ofc_vw_cd="  + ComGetObjValue(formObj.f_ofc_vw_cd)
			          + "&f_ofc_lvl=03"

...
/**
 * @extends 
 * @class ESM_SQM_0032 : ESM_SQM_0032 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0032() {
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0032GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시
			...
			max = 0;
			ComOpenWait(true);
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0032GS.do",searchParams);	
		...
		case IBSAVE:          // 화면 저장시
			...
			sheetObj.DoSave("ESM_SQM_0032GS.do", searchParams, -1, false);
			//ComOpenWait(false);
		...
		case MULTI01:		// Creation시에
			...
			var sXml = sheetObj.GetSaveXml("ESM_SQM_0032GS.do", searchParams);
			var State = ComGetEtcData(sXml, ComWebKey.Trans_Result_Key);
			ComOpenWait(false);
		...
		case "FigureInquiry":  // Figure Inquiry 클릭시
			...
			window.showModalDialog("ESM_SQM_0043.do?" + searchParams + "&div_period=" + document.getElementById("div_period_based_on_rev_month").innerHTML, window, "dialogHeight:510px;dialogWidth:900px;center:yes;resizable:no;scroll:no;status:no;unadorned:yes;");
			break;
		...
		case "RhqRowAdd":  // Rhq Row Add 클릭시
			...
			sheetObj.InitCellProperty(row , "rhq_cd", dtCombo);
			...
			var param        =   "&f_bse_yr="      +f_bse_yr  
					            ...
					            +"&f_dir_cd="     +f_dir_cd
					            ;
			var sXml = sheetObj.GetSearchXml("CommonGS.do", "f_cmd="+ SEARCH02 + "&code_name=rhqByPortion&code_param=" + param + "&all_flag=");
			ComSqmSetIBCombo(sheetObj, sXml, "rhq_cd", true);
	...
	switch(sName){
		...
		case "to_vvd_cd":
			if(value.length == 9){
				...
				var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0032GS.do","f_cmd="+SEARCH01+"&vvd="+value+"&"+params);	
				ComOpenWait(false);
 		...
 		case IBSAVE:  // 화면 저장시에
 			...
 			for(var i=sheetObj.HeaderRows;i<sheetObj.Rows-1;i++){
 				if(sheetObj.CellValue(i,"rhq_cd") == ""){
 					ComShowCodeMessage('SQM00024','RHQ');//Please Input RHQ.
 					return false;
 				}

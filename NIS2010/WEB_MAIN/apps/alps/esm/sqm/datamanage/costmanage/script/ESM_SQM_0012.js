...
/**
 * @extends 
 * @class ESM_SQM_0012 : ESM_SQM_0012 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0012() {
	...
	switch(sheetNo) {
		case 1:		// sheet1 init
			with (sheetObj) {
				...
				var HeadTitle =  "STS|SEL|Trade|Lane Bound|New Lane";
				...
				InitDataProperty(0,	cnt++,	dtCheckBox,		50,	daCenter,	true,	"add_flg",	false,	"",	dfNone,	0,	true,	true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBSEARCH:          // 화면 조회
			...
			sheetObj.DoSearch4Post("ESM_SQM_0012GS.do", searchParams);
			
			var checkRow = sheetObj.FindCheckedRow("add_flg");
			var arrRow = checkRow.split("|");
			
			for (i=0; i<arrRow.length-1; i++) {
				sheetObj.CellEditable(arrRow[i], "add_flg") = false;
			}
			break;
		...
		case "Apply":          // Apply 버튼 클릭
			...
			var sXml = sheetObj.GetSaveXml("ESM_SQM_0012GS.do", searchParams + "&" + saveStr);

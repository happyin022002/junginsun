...
/**
 * @extends 
 * @class ESM_SQM_0205 : ESM_SQM_0205 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0205() {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			with (sheetObj) {
				...
				var HeadTitle1 =  "STS|Trade|Sub Trade|IAS Region|R.Lane|Lane Bound|P/F SKD Group|P/F SKD Ver.|Route|Select";
				...
				InitDataProperty(0,	cnt++,	dtData,			95,   daCenter,	 true,	"pf_grp_cd",	false,	"",	dfNone,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtData,			85,   daCenter,	 false,	"pf_svc_tp_cd",	false,	"",	dfNone,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtData,			250,  daLeft,	 false,	"pf_rout_desc",	false,	"",	dfNone,	0,	false,	false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0205GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시
			...
			ComOpenWait(true);
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0205GS.do",searchParams);
		...
		case MULTI01:		// Creation시에
			...
			sheetObj.DoSave("ESM_SQM_0205GS.do", searchParams, -1, false);
			ComOpenWait(false);
	...
	switch(sAction) {
		case MULTI01:  // Add-Creation
			...
			if(formObj.act_cnt.value == "0"){ //해당 lane에 active된 pol-pod pair 있는지 체크
				ComShowCodeMessage("SQM00047", formObj.f_rlane_cd.Code);
				return false;
			}

...
/**
 * @extends
 * @class ESM_SQM_0218 : ESM_SQM_0218 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0218() {
...
function initSheet(sheetObj,sheetNo) {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			...
			var HeadTitle1 =  "SEQ|Year|Quarter|Office View|Trade|Sub Trade|IAS Region|R.Lane|Lane Bound|Trade Direction|Month|Supply|VVD Count|RHQ|Office|POL|POD|Main/Sector|Load|G.RPB|G.REV|CM Cost(PA)|CM Cost(RA)|CMCB(PA)|CMCB(RA)|CM(PA)|CM(RA)|CMPB(PA)|CMPB(RA)";
			...
			RangeBackColor(0, 18,0, 20) = RgbColor(203,210,248);
			RangeBackColor(0, 25,0, 28) = RgbColor(203,210,248);
			}
			break;
	}
}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0218GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          //화면 조회시
			...
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0218GS.do", searchParams);

...
/**
 * @extends 
 * @class ESM_SQM_0029 : ESM_SQM_0029 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0029() {
...
function initSheet(sheetObj,sheetNo) {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			...
			InitDataProperty(0,	cnt++,	dtAutoSum,	65,		daRight,	true,	"fnl_bsa_capa",	false,	"",	dfInteger,	0,	false,	false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0029GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          //화면 조회시
			...
			sheetObj.DoSearch("ESM_SQM_0029GS.do", FormQueryString(formObj));
  ...
  function validateForm(sheetObj, formObj, sAction){
  	...
  	switch(sAction) {
  		...
  		case IBSEARCH:  // 화면 조회시에
  			var yr = ComGetObjValue(formObj.f_bse_yr); //Quarter
  			if(formObj.f_fm_mon.value!=""&&formObj.f_to_mon.value!=""){
  				if(formObj.f_fm_mon.value<"00"
  				        ||formObj.f_to_mon.value>"53"){
  						ComShowCodeMessage('SQM00025',yr); 
...
//   		  else
//   			msg = level + " / " + year + "." + qtaMonArr["1Q"][0] + " ~ " + year + "." + qtaMonArr["4Q"][1];
//   	  }else{
//   		  if(ComGetObjValue(formObj.f_fm_mon) != "" && ComGetObjValue(formObj.f_to_mon) != "")
//   			  msg = level + " / Office View : " + view + " / " + year + "." + ComGetObjValue(formObj.f_fm_mon) + " ~ " + year + "." + ComGetObjValue(formObj.f_to_mon);  
//   		  else
//   			 msg = level + " / Office View : " + view + " / " + year + "." + qtaMonArr["1Q"][0] + " ~ " + year + "." + qtaMonArr["4Q"][1];  
//   	  }
//   	  

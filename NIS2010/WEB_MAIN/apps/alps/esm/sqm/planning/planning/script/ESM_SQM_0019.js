...
/**
 * @extends 
 * @class ESM_SQM_0019 : ESM_SQM_0019 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0019() {
	...
	try {
		...
		switch(srcName) {
			...
			case "btn_Pop0020":
	        	...
	        	ComOpenPopup("/hanjin/ESM_SQM_0020.do?"+searchParams+ "&div_period=" + document.getElementById("div_period").innerHTML, 910, 530, "", "0,0", true);
				break;
			case "btn_Pop0021":
				if (sheetObject.isDataModified == true) {
	        	...
	        	ComOpenPopup("/hanjin/ESM_SQM_0021.do?"+searchParams+ "&div_period=" + document.getElementById("div_period").innerHTML, 910, 530, "", "0,0", true);
				break;
		}
	} catch(e) {
		if( e == "[object Error]") {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			with (sheetObj) {
				...
				InitDataProperty(0,	cnt++,	dtData,		60,	  daCenter,	 true,	"ofc_vw_cd",		false,	"",	dfNone,		0,	false,	false);
				...
				InitDataProperty(0,	cnt++,	dtAutoSum,	85,   daRight,	 true,	"gid_lod_potn_rto",	false,	"",	dfFloat,	2,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,  85,   daRight,	 true,	"lod_potn_rto",		false,	"",	dfFloat,	2,	true,	true,	5);
				InitDataProperty(0,	cnt++,	dtAutoSum,	85,   daRight,   true,	"gid_rev_potn_rto",	false,	"",	dfFloat,	2,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	85,   daRight,	 true,	"rev_potn_rto",		false,	"",	dfFloat,	2,	true,	true,	5);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0019GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시
			...
			ComOpenWait(true);
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0019GS.do",searchParams);	
			ComOpenWait(false);
		...
		case IBSAVE:          // 화면 저장시
			...
			sheetObj.DoSave("ESM_SQM_0019GS.do", searchParams, -1, false);
		...
		case MULTI01:          // 화면 confirm시
			...
			sheetObj.DoSearch("ESM_SQM_0019GS.do", searchParams);
  ...
  function toggleButtons(step) {
  	switch (step) {
  		case "INIT":
  			...
  			ComBtnDisable("btn_Pop0020");
  			ComBtnDisable("btn_Pop0021");
  			break;
  		...
  		case "SEARCH":
  			...
  			ComBtnEnable("btn_Pop0020");
  			ComBtnEnable("btn_Pop0021");
  			break;

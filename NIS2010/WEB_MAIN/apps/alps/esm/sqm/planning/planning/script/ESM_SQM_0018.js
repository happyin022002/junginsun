...
/**
 * @extends 
 * @class ESM_SQM_0018 : ESM_SQM_0018 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0018() {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			with (sheetObj) {
				...
				var HeadTitle1 = "SEQ|Year|Quarter|N.OB/OB|Office View|Trade|Trade\nBound|Trade\nDirection|RHQ|RHQ Load QTA Summary|RHQ Load QTA Summary|RHQ Load QTA Summary|RHQ G.REV QTA Summary|RHQ G.REV QTA Summary|RHQ G.REV QTA Summary|RHQ Past PFMC Per Week|RHQ Past PFMC Per Week|RHQ Past PFMC Per Week|RHQ Past PFMC Per Week|RHQ Simulation vs RHQ Past PFMC|RHQ Simulation vs RHQ Past PFMC|RHQ Simulation vs RHQ Past PFMC|RHQ Simulation vs RHQ Past PFMC";
				var HeadTitle2 = "SEQ|Year|Quarter|N.OB/OB|Office View|Trade|Trade\nBound|Trade\nDirection|RHQ|TTL Load|Weekly Load|RHQ %|TTL G.REV|Weekly G.REV|RHQ %|Load|Load %|G.REV|G.REV %|Load|Load %|G.REV|G.REV %";
				
				//컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
				InitColumnInfo(ComCountHeadTitle(HeadTitle1) + 1, 9, 0, true);
				...
				InitDataProperty(0,	cnt++,	dtAutoSum,	80,	daRight,	true,	"rhq_wk_qty",	false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"rhq_qty_rto",	false,	"",	dfNone,		0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"rhq_rev",		false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	90,	daRight,	true,	"rhq_wk_rev",	false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"rhq_rev_rto",	false,	"",	dfNone,		0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"pst_wk_qty",	false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"pst_qty_rto",	false,	"",	dfNone,		0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"pst_wk_rev",	false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"pst_rev_rto",	false,	"",	dfNone,		0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"diff_qty",		false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"diff_qty_rto",	false,	"",	dfNone,		0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"diff_rev",		false,	"",	dfInteger,	0,	false,	false);
				InitDataProperty(0,	cnt++,	dtAutoSum,	70,	daRight,	true,	"diff_rev_rto",	false,	"",	dfNone,		0,	false,	false);
			}
			break;
	}
}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBSEARCH:          // 화면 조회 시
			...
			var rtnXml = sheetObj.GetSearchXml("ESM_SQM_0018GS.do", searchParams);

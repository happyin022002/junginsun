...
/**
 * @extends
 * @class ESM_SQM_0503 : ESM_SQM_0503 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_SQM_0503() {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
			with (sheetObj) {
				...
				var HeadTitle1 =  "STS|SEQ|Trade|New Lane|Copy Source Lane";
				...
				InitDataProperty(0,	cnt++,	dtCombo,		110,	daCenter,	true,	"trd_cd",			true,	"",	dfNone,		0,	false,	true);
				InitDataProperty(0,	cnt++,	dtCombo,		160,	daCenter,	true,	"rlane_cd",		true,	"",	dfNone,		0,	false,	true);
				InitDataProperty(0,	cnt++,	dtCombo,		160,	daCenter,	true,	"src_rlane_cd",			true,	"",	dfNone,		0,	false,	true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
 	...
 	switch(sAction) {
	 	case IBCLEAR:          // 화면 접속 시
			...
			var sXml   = sheetObj.GetSearchXml("ESM_SQM_0503GS.do", FormQueryString(formObj));
		...
		case SEARCH01:		//Trade에 맞는 Lane 조회
			...
			sheetObj.CellComboItem(sheetObj.SelectRow, "src_rlane_cd",  " |"+rlane[0],  " |"+rlane[1]);
			ComOpenWait(false);
			break;
		...
		case IBCREATE:          // creation
			...
			if (sheetObj.CellValue(sheetObj.SelectRow, "rlane_cd") == sheetObj.CellValue(sheetObj.SelectRow, "src_rlane_cd")) {
			...
			formObj.f_src_rlane_cd.value = sheetObj.CellValue(sheetObj.SelectRow, "src_rlane_cd");
			formObj.f_rlane_cd.value =  sheetObj.CellValue(sheetObj.SelectRow, "rlane_cd");
			formObj.f_cmd.value = MULTI;
 			var sXml = sheetObj.GetSaveXml("ESM_SQM_0503GS.do", FormQueryString(formObj));
			ComOpenWait(false);

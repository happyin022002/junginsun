...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
        	 ...
        	 switch(srcName) {
                ...
                case "btn_add":
                	if(validateForm(sheetObject,formObject)) {
						...
						sheetObject.CellText(row, "mkt_rt_yrmon") = form.mktRtYrmon.value.replace("-","");
						inputReadOnly("1");
					}
					break;
                ...
                case "mkt_rt_yrmon_cal":
					var cal = new ComCalendar();
					cal.setDisplayType('month');
					cal.select(form.mktRtYrmon, 'yyyy-MM');
                    break;
    ...
    function inputReadOnly(flag) {
    	if(flag == "1") {
    		form.mktRtYrmon.readOnly = true;
    		form.mkt_rt_yrmon_cal.style.cursor = "default";
    		document.images["mkt_rt_yrmon_cal"].name = "no_mkt_rt_yrmon_cal";
    	} else {
    		form.mktRtYrmon.readOnly = false;
    		form.mkt_rt_yrmon_cal.style.cursor = "hand";
    		document.images["mkt_rt_yrmon_cal"].name = "mkt_rt_yrmon_cal";
    	}
    }
    	 	...
    	 	case 1:      //sheet1 init
    	 		with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle1 = "|Sel|Seq|mkt_rt_yrmon|mkt_rt_seq|Market Rate Coverage|Market Rate Coverage|Built Type|Current Market Rate|Last Updated\nPeriod";
                    var HeadTitle2 = "|Sel|Seq|mkt_rt_yrmon|mkt_rt_seq|From Size|To Size|Built Type|Current Market Rate|Last Updated\nPeriod";
					...
					InitDataProperty(0, cnt++ , dtHidden,		30,    	daRight,   	false,  "mkt_rt_yrmon",		false,	"",	dfNone,			0,	true,	true);
					InitDataProperty(0, cnt++ , dtHidden,		30,    	daRight,   	false,  "mkt_rt_seq",		false,	"",	dfInteger,		0,	true,	true);
                    ...
                    InitDataProperty(0, cnt++ , dtData,   	 	150,    daLeft,  	true,   "bld_tp_nm",     	false,  "", dfNone,     	0,  true,   true, 20);
                    InitDataProperty(0, cnt++ , dtData,   	 	150,    daRight,   	true,   "mkt_rt_amt", 		true,  	"", dfNullFloat, 	2,  true,   true, 12);
                    InitDataProperty(0, cnt++ , dtData,			100,    daRight,   	true,   "prev_mkt_rt_ytmon",false,	"",	dfDateYm,			0,	false,	false);
    	...
    	switch(sAction) {
	    	case IBSEARCH:      // 조회
	    		if(validateForm(sheetObj,formObj,sAction)) {
					...
					sheetObj.DoSearch("ESM_FMS_0065GS.do", FormQueryString(formObj));
					inputReadOnly("1");
				}
				break;
			...
			case IBSAVE:        // 저장
		  	  	if(validateForm(sheetObj,formObj,sAction)) {
		  	  		...
		  	  		sheetObj.DoSave("ESM_FMS_0065GS.do", FormQueryString(formObj)); 
		  	  	}
		  	  	break;
    ...
    /**
     * HTML Control의 onblur이벤트에서 Period의 Validation을 체크한다.<br>
     **/
    ...
    function validateForm(sheetObj,formObj,sAction){
    	...
    	with(formObj){
    		this.mktRtYrmon = mktRtYrmon.value.replace("/", "");
	...
	/**
     * 조회한 데이터가 과거 데이터인 경우 ibflag상태를 Insert상태로 바꾼다.<br>
     */
    function sheet1_OnSearchEnd(sheetObj, errMsg) {
    	for(var row=2; row<=sheetObj.LastRow; row++) {
    		if(sheetObj.CellValue(row,"mkt_rt_yrmon") != sheetObj.CellValue(row,"prev_mkt_rt_ytmon")) {

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_FMS_0041 : ESM_FMS_0041 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_FMS_0041() {
            ...
            switch(srcName) {
					...
					if(sheetObject.RowCount > 0){
						...
						var vsl_eng_nm = formObject.vsl_eng_name.value;
				 		...
				 		if (   csr_no.substring(2,3) == 'P'  
				 			...
				 			ComOpenPopup("ESM_FMS_0075.do?csr_no="+ csr_no+"&pgm_id=esm_fms_0041&vsl_eng_nm="+vsl_eng_nm+"", 500, 153, "sendMail", "1,0,1,1,1,1", false, false, 0, 0, 0, "ESM_FMS_0075");
				 		} else {
				...
				case "btn_slip":
					...
					if(sheetObject.RowCount > 0){
						...
						var vsl_eng_nm = formObject.vsl_eng_name.value;
						var csr_curr_cd = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "csr_curr_cd");
						
						if(formObject.popup.value == 'yes') {
							ComOpenPopup("ESM_FMS_0042.do?csr_no="+ csr_no+"&vsl_cd="+vsl_cd+"&vsl_eng_nm="+vsl_eng_nm+"&csr_curr_cd="+csr_curr_cd, 1024, 477, "setSlipInquiry", "1,0,1,1,1,1", false, false, 0, 0, 0, "ESM_FMS_0042");
			 			} else {
			 				ComOpenPopup("ESM_FMS_0042.do?csr_no="+ csr_no+"&vsl_cd="+vsl_cd+"&vsl_eng_nm="+vsl_eng_nm+"&csr_curr_cd="+csr_curr_cd, 1024, 477, "setSlipInquiry", "1,0,1,1,1,1", false, false, 0, 0, 0, "ESM_FMS_0042");
			 			}
					} else {
				...
				case "btn_vslpop":
					ComOpenPopup("ESM_FMS_0022.do", 520, 440,"setVslCd", "1,0,1,1,1", false);
					break;
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle1 = "|Seq|CSR Status|CSR No.|G/L Date|Produced By|Currency|CSR Amount|CSR Description|Paid Date|Request Team|Rqst Date|Ownr Code|Ownr Name|Evid Type|Deduction|Rqst Amt|Diff Desc|Cxl Flg|Cxl Desc|Vsl Code|Vsl Eng Name|Slip Type|Flet Ctrt Tp Cd";
					...
					InitDataProperty(0, cnt++ , dtData,   		100,   daRight,   false,    "rqst_amt",   	false,     "", 		dfNullFloat,	2, 		false,		false);
					InitDataProperty(0, cnt++ , dtData,   		290,   daLeft,    false,    "csr_desc",   	false,     "",    	dfNone, 		0,     	false,		false);
					...
					InitDataProperty(0, cnt++ , dtHidden,   	100,   daRight,   false,    "csr_amt",  	false,     "", 		dfNullFloat, 	2,     	false,		false);
        ...
        switch(sAction) {
           ...
           case IBSEARCH:      //조회
			 ...
			 case IBSAVE:        //저장
				 	...
				 	if(submitFlg == "N") return;
				 	if(!saveConfirm()) return;
		 			
				 	formObj.f_cmd.value = MULTI;
	
			    	var sParam = FormQueryString(formObj);
			    	var sXml = sheetObj.GetSaveXml("ESM_FMS_0041GS.do", sParam);
	...
	function sheet1_OnClick(sheetObj, row, col, cellX, cellY, cellW, cellH) {
		 ...
		 form.save_csr_no.value = sheetObj.CellValue(row, "csr_no");
 		 form.flet_ctrt_tp_cd.value = sheetObj.CellValue(row, "flet_ctrt_tp_cd");
 		 form.slip_type.value = sheetObj.CellValue(row, "slip_type");
 		 
 		 if(sheetObj.CellValue(row, "apro_flg") == "Submitted") submitFlg = "Y";
 		 else submitFlg = "N";
	}
    ...
    function sheet1_OnSearchEnd(sheetObj, errMsg) {
    	if(sheetObj.RowCount > 0){
    		...
    		form.save_csr_no.value = sheetObj.CellValue(1, "csr_no");
    		form.flet_ctrt_tp_cd.value = sheetObj.CellValue(1, "flet_ctrt_tp_cd");
    		form.slip_type.value = sheetObj.CellValue(1, "slip_type");
    		
    		if(sheetObj.CellValue(1, "apro_flg") == "Submitted") submitFlg = "Y";
    		 else submitFlg = "N";
    	} else {
    ...
    function saveConfirm() {
 		...
 		var okYn = ComShowConfirm(ComGetMsg("FMS00002"));

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_FMS_0025 : ESM_FMS_0025 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_FMS_0025() {
				...
				switch(srcName) {
			        ...
			        case "btn_confirm":
	    	        	...
	    	        	for(var i=0; i<sheetObject.LastRow ;i++) {
	    	        		...
	    	        		charterExpData.acct_nm	= sheetObject.CellValue(row,"acct_nm");
	    	        		charterExpData.acct_cd	= sheetObject.CellValue(row,"acct_cd");
	    	        		charterExpData.vvd_bunker	= sheetObject.CellValue(row,"vvd_bunker");
	    	        		charterExpData.curr_cd	= sheetObject.CellValue(row,"curr_cd");
	    	        		charterExpData.inv_amt = sheetObject.CellValue(row,"inv_amt");
	    	        		charterExpData.inv_desc	= sheetObject.CellValue(row,"inv_desc");
	    	        		charterExpData.ctr_cd	= sheetObject.CellValue(row,"ctr_cd");
	    	        		charterExpData.slp_loc_cd = sheetObject.CellValue(row,"slp_loc_cd");
	    	        		charterExpData.flet_iss_tp_cd = sheetObject.CellValue(row,"flet_iss_tp_cd");
	    	        		charterExpData.inv_seq	= sheetObject.CellValue(row,"inv_seq");
	    	        		charterExpData.inv_dtl_seq = sheetObject.CellValue(row,"inv_dtl_seq");
	    	        		charterExpData.chtr_pay_rcv_cd = sheetObject.CellValue(row,"chtr_pay_rcv_cd");
	    	        		//charterExpData.loc_cd	= sheetObject.CellValue(row,"loc_cd");
	    	        		charterExpData.flet_src_tp_cd = sheetObject.CellValue(row,"flet_src_tp_cd");
	    	        		charterExpData.flet_ctrt_no = sheetObject.CellValue(row,"flet_ctrt_no");
	    	        		
	    	        		aryData[idx++] = charterExpData;
	    	        	}
	    				opener.setCharterersExp(aryData);
	    				self.close();
            ...
            case 1:
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++ , dtData,      	105,	daCenter,	false,		"chtr_pay_rcv_cd",	false,	"",		dfNone,				0,	false,	false);
					...
					InitDataProperty(0, cnt++ , dtData,      	227,	daLeft,		false,		"inv_desc",			false,	"",		dfNone,				0,	false,	false);

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    function loadPage() {
		...
		//////////////--csy start
		//vessel명 조회
		doActionIBSheet(sheetObjects[0], form,IBROWSEARCH,'Vessel'); 
	
		//Duration 날짜, 시간 포맷
		form.eff_dt.value = form.eff_dt.value.substring(0,4)+"-"+form.eff_dt.value.substring(4,6)+"-"+form.eff_dt.value.substring(6,8);
		form.from_time.value = form.from_time.value.substring(0,2)+":"+form.from_time.value.substring(2,4);
		form.exp_dt.value = form.exp_dt.value.substring(0,4)+"-"+form.exp_dt.value.substring(4,6)+"-"+form.exp_dt.value.substring(6,8);
		form.to_time.value = form.to_time.value.substring(0,2)+":"+form.to_time.value.substring(2,4);
		
		//Days 계산
		getInvUsdDys();		
		
		if (document.form.cr_chk_flg.value == "Y") {
			form.inv_usd_dys.value = "-" + form.inv_usd_dys.value;
		}

		//Contract No. 콤보 설정
		var sXml=sheetObjects[0].GetSearchXml("ESM_FMS_0091GS.do?f_cmd="+SEARCH03+"&typeFlag=TI&vsl_cd="+form.vsl_cd.value+"&ctrtFlag=");  
		var list = ComFmsSheetXml2ListMap(sXml);
		for(var i = 0 ; i < list.length; i++) {
			ComAddComboItem(form.flet_ctrt_no ,list[i]["flet_ctrt_no"] , list[i]["flet_ctrt_no"]);
		}	
		
		//Processing & Completed 탭에서 Ind 컬럼이 Saved 상태일때 Contract No 콤보의 첫번째 데이터로 하단 그리드 자동조회
		if(opener.document.tab1.SelectedIndex == 2) {
			var openSheetObj = opener.document.t3sheet1;
			var checkedRows = openSheetObj.FindCheckedRow("sheet3_chk") ;
			var arrRow = checkedRows.split("|");			
			var element = form.flet_ctrt_no;	
						
			if(element.length > 1 && openSheetObj.CellValue(arrRow[0],"sheet3_offh_ind")=="Saved") { //Contract No가 존재하고 Ind 컬럼이 Saved 때 첫번째 값으로 자동조회(change 이벤트를 태움)
		...
		if (form.vnor_offh_ind.value == "Approval" || form.vnor_offh_ind.value == "Non Approval") {
			ComBtnDisable("btn_save");
		}
		
    	if (opener.document.tab1.SelectedIndex == 0) {
    		var openSheetObj = opener.document.t1sheet1;
    		for(var i=openSheetObj.HeaderRows; i<openSheetObj.Rows; i++) {
    			if(openSheetObj.CellValue(i, "sheet1_chk")==1 && openSheetObj.CellValue(i, "sheet1_vnor_itm_cd")=="OH") {
    				searchFlg = "Y";
    			}
    		}
    	} else {
		...
		if (form.sheet_no.value == "sheet3") {
			form.flet_ctrt_no.value = form.opener_flet_ctrt_no.value;
			form.inv_seq.value = form.opener_inv_seq.value;
			form.condition.value = "I";
    ...
    function sheet2_OnLoadFinish(sheetObj) {   
		...
		CoFmsGetCombo('FORM', document.form, sheetObj, 'CD03388', 'flet_offh_rsn_cd', 'flet_offh_rsn_nm');
		
		var length = form.flet_offh_rsn_cd.length;
		
		if(length > 0) {
			for(var i=0; i<length; i++) {
				if(form.flet_offh_rsn_cd.options[i].value == form.vnor_offh_knd_cd.value) {
            ...
            case "sheet2":      //t1sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle = "|Sel|Seq|Item Name|Account Code|UOM|Value|Price|Cur 1|Amount|Cur 2|Amount|Approval|Description|FletCtrtNo|FletIssTpCd|InvSeq|InvDtlSeq|AcctItmSeq|Ori Amount|Ori Amount2|From Date|To Date|Inv Usd Dys|Sort Key|Fir Amount|Fir Amount2|vnor_seq|vnor_itm_seq|vnor_itm_val2";
					...
					InitDataProperty(0, cnt++ , dtCombo,   	70,    daCenter,  false,     prefix + "vnor_itm_ut_cd", false,          "",      dfNone,      0,     true,       true);
					InitDataProperty(0, cnt++ , dtData,      	70,    daRight,  	false,     prefix + "vnor_itm_val",		false,          "",      dfNullFloat,     2,     true,       true);
					InitDataProperty(0, cnt++ , dtData,      	70,    daRight,  	false,     prefix + "vnor_itm_prc",		false,          "",      dfNullFloat,     2,     true,       true);
					
					InitDataProperty(0, cnt++ , dtData,      	50,    daCenter,  false,     prefix + "curr_cd",			true,          "",      dfEngUpKey,  0,     true,       true, 3);
					InitDataProperty(0, cnt++ , dtData,       99,    daRight,   	false,     prefix + "inv_amt", 			true,          "",      dfNullFloat, 2,     true,        true,  15);
					InitDataProperty(0, cnt++ , dtData,      	45,    daCenter,  false,     prefix + "curr_cd2",			false,          "",      dfNone,      0,     false,       false, 3);
					InitDataProperty(0, cnt++ , dtData,       99,    daRight,   	false,     prefix + "inv_amt2", 			false,          "",      dfNullFloat, 2,     false,        false,  15);
					InitDataProperty(0, cnt++ , dtData,      	60,    daCenter,  false,     prefix + "slp_tp_cd",     		false,          "",      dfNone,      0,     false,       false);
					InitDataProperty(0, cnt++ , dtData,      	247,   daLeft,    	false,     prefix + "inv_desc",     	    true,          "",      dfNone,      0,     true,       true, 50);
...
//----------------------------------					
					...
					InitDataProperty(0, cnt++ , dtHidden,      	99,    daRight,    false,     prefix + "vnor_seq", 		false,          "",      dfNone, 	0,     true,        true);
					InitDataProperty(0, cnt++ , dtHidden,      	99,    daRight,    false,     prefix + "vnor_itm_seq",	false,          "",      dfNone, 	0,     true,        true);
					InitDataProperty(0, cnt++ , dtHidden,      	70,    daRight,  	false,     prefix + "vnor_itm_val2",	false,      "",      dfNullFloat,2,     false,       false);
        ...
        switch(sAction) {
        	...
        	case IBSEARCH:      //조회
        		...
        		if(!validateForm(sheetObj,formObj,sAction))  return true;
        	   	...
        	   	var sXml = sheetObj.GetSearchXml("ESM_FMS_0091GS.do" , FormQueryString(formObj)+"&" + ComGetPrefixParam(aryPrefix));
    			...
    			if (form.cr_chk_flg.value == "Y") {
    				form.usd_inv_amt.value = "-" + form.usd_inv_amt.value;
    			}  			
			...
			case IBSAVE:        //저장
				...
				var sXml = sheetObj.GetSaveXml("ESM_FMS_0091GS.do", sParam);
				var arrXml = sXml.split("|$$|");
				...
				if(!formObj.condition[1].checked) {
					...
					formObj.flet_offh_rsn_cd.disabled = true;
				}				
				...
				if (form.sheet_no.value == "sheet1") {
					opener.afterDetailSave();
				}
			 ...
			 case IBROWSEARCH:      //조회
				 	...
				 	if(kind == "Vessel") {
				    	...
				    	formObj.f_cmd.value = SEARCH10;
			   			...
			   			var sXml = sheetObj.GetSearchXml("ESM_FMS_0091GS.do" , FormQueryString(formObj));
		        		...
		        		var sXml = sheetObj.GetSearchXml("ESM_FMS_0076GS.do" , FormQueryString(formObj));
			   			...
			   			if(typeof currCd == "undefined" || currCd == "") {
		   					...
		   					ComFmsSetInitCellProperty(sheetObj, row, 9, "inv_amt", currCd, prefix, 2);
    ...
    function initControl() {
        ...
        axon_event.addListener  ('change', 'contract_no_change', 'flet_ctrt_no');		//Contract No 변경시 발생 이벤트
        axon_event.addListener  ('change', 'vvd_accident_change', 'bunker_vvd');			//VVD CD 변경시 발생 이벤트
    ...
    function contract_no_change() {
    	...
    	if (searchFlg == "Y" || document.form.condition[1].checked) {
	    	//VVD CD 변경시 하단 그리드 조회
	    	vvd_accident_change();  
    	} else {
    		document.form.search_yn.value = "Y";
    	}
/*    	
    	<Detail 버튼 시 연결되는 FMS_ACCT_ITM 테이블  ACCT_CD / ACCT_ITM_SEQ>						
    	HF : HFO (M/T)  : 511191  14						
    	MD : MDO(M/T)   : 511191  15						
    	LF : LSFO(M/T)  : 511191  44						
    	LD : LSDO(M/T)  : 511191  56						
    	PC : Port Charge     : 510993  74						
    	TC : Terminal Charge : 510993  75						
    	OT : Others          : 510993  59
*/    	
    	var itm = "";
    	var itmNm = "";
    	...
    	//Confirm 탭에서 체크된 행 하단 그리드에 추가    	
    	if(opener.document.tab1.SelectedIndex == 0 && document.form.condition[0].checked) {
    		var prefix = "inv_";
    		var openSheetObj = opener.document.t1sheet1;
    		
    		for(var i=openSheetObj.HeaderRows; i<openSheetObj.Rows; i++) {
    			if(openSheetObj.CellValue(i, "sheet1_chk")==1 && openSheetObj.CellValue(i, "sheet1_vnor_itm_cd")!="OH") { //체크박스에 체크되고, item 컬럼에 Off-Hire Time 이 아닌 데이터만 등록 
    				var newRow = sheetObjects[1].DataInsert(-1);
    				
    				itm = openSheetObj.CellValue(i, "sheet1_vnor_itm_cd");
    				
    		    	if(itm == "HF"){
    		    		itmNm = "F.O. Settlement Cost for Irregular Accident";
    		    		accountCode = "511191";
    		    		acctItmSeq = "14";
    		    	}else if(itm == "MD"){
    		    		itmNm = "D.O. Settlement Cost for Irregular Accident";
    		    		accountCode = "511191";
    		    		acctItmSeq = "15";    		
    		    	}else if(itm == "LF"){
    		    		itmNm = "L.S.F.O. Settlement Cost for Irregular Accident";
    		    		accountCode = "511191";
    		    		acctItmSeq = "44";
    		    	}else if(itm == "LD"){
    		    		itmNm = "L.S.D.O. Settlement Cost for Irregular Accident";
    		    		accountCode = "511191";
    		    		acctItmSeq = "56";
    		    	}else if(itm == "PC"){
    		    		itmNm = "Port Charge";
    				...
    				sheetObjects[1].CellValue2(newRow, prefix+"acct_itm_nm") = itmNm;
    				sheetObjects[1].CellValue2(newRow, prefix+"acct_cd") = accountCode;
    				
    				sheetObjects[1].CellValue2(newRow, prefix+"vnor_itm_ut_cd") = openSheetObj.CellValue(i, "sheet1_vnor_itm_ut_cd");
    				sheetObjects[1].CellValue2(newRow, prefix+"vnor_itm_val") = openSheetObj.CellValue(i, "sheet1_vnor_itm_val");
    				sheetObjects[1].CellValue2(newRow, prefix+"vnor_itm_val2") = openSheetObj.CellValue(i, "sheet1_vnor_itm_val");    				
    				sheetObjects[1].CellValue2(newRow, prefix+"acct_itm_seq") = acctItmSeq;
    				sheetObjects[1].CellValue2(newRow, prefix+"vnor_seq") = openSheetObj.CellValue(i, "sheet1_vnor_seq");
    				sheetObjects[1].CellValue2(newRow, prefix+"vnor_itm_seq") = openSheetObj.CellValue(i, "sheet1_vnor_itm_seq");    				
    				/*    				
    				if(newRow > 1){
    					sheetObjects[1].CellValue2(newRow, prefix+"flet_ctrt_no") = sheetObjects[1].CellValue(newRow-1, prefix+"flet_ctrt_no");
    					sheetObjects[1].CellValue2(newRow, prefix+"inv_dtl_seq") = Number(sheetObjects[1].CellValue(newRow-1, prefix+"inv_dtl_seq"))+1;
    				}
    				*/
    				sheetObjects[1].CellValue2(newRow, prefix+"flet_ctrt_no") = document.form.flet_ctrt_no.value;
    				if (newRow == 1) {
    					sheetObjects[1].CellValue2(newRow, prefix+"inv_dtl_seq") = newRow;
    				} else {
    					sheetObjects[1].CellValue2(newRow, prefix+"inv_dtl_seq") = Number(sheetObjects[1].CellValue(newRow-1, prefix+"inv_dtl_seq"))+1;
    				}
    				
    				sheetObjects[1].CellEditable(newRow, prefix+"vnor_itm_ut_cd") = true;
    				sheetObjects[1].CellEditable(newRow, prefix+"vnor_itm_val") = true;			
    				sheetObjects[1].CellEditable(newRow, prefix+"vnor_itm_prc") = true;
    				sheetObjects[1].CellEditable(newRow, prefix+"curr_cd") = true;
    				sheetObjects[1].CellEditable(newRow, prefix+"inv_amt") = true;    				
    				sheetObjects[1].CellEditable(newRow, prefix+"inv_desc") = true;
    			}
    		}    		
    	}
    }
    
    function vvd_accident_change() {
    	//하단 그리드 조회
    ...
    function sheet2_OnSearchEnd(sheetObj, ErrMsg) {
    	...
    	for (var ir=1; ir<=sheetObj.LastRow; ir++){
    		...
    		if(sheetObj.CellValue(ir, prefix+"slp_tp_cd") == "Y") {
    			...
    			sheetObj.CellEditable(ir, prefix+"vnor_itm_prc") = false;
    		...
    		if (document.form.cr_chk_flg.value == "Y" && searchFlg == "Y") {
    			if (sheetObj.CellValue(ir, prefix+"inv_amt") > 0) {
     			   sheetObj.CellValue2(ir, prefix+"inv_amt") = "-" + sheetObj.CellValue(ir, prefix+"inv_amt");
     			} 
    		}
    	}
		...
		for(var i=openSheetObj.HeaderRows; i<openSheetObj.Rows; i++) {
			if(openSheetObj.CellValue(i, "sheet1_chk")==1 && openSheetObj.CellValue(i, "sheet1_vnor_itm_cd")=="OH") {
	    		sheetObj.CellValue2(1, prefix+"vnor_seq") = openSheetObj.CellValue(i, "sheet1_vnor_seq");
	    		sheetObj.CellValue2(1, prefix+"vnor_itm_seq") = openSheetObj.CellValue(i, "sheet1_vnor_itm_seq");
			}
		}
	...
	function setDuration(aryPopupData){
		...
		//--csy
		//Contract No 를 초기화해서 사용자가 다시 재조회할 수 있도록 함
	    ...
	    if(val == "I") {
	    	...
	    	if(val == "C") {
	    		...
	    		if(val == "V" || form.search_yn.value == "") {
		    		...
		    		ComOpenPopup("ESM_FMS_0073.do?flet_ctrt_no=" + flet_ctrt_no + "&vsl_cd=" + vsl_cd + "&vsl_eng_nm=" + vsl_eng_nm, 920, 480, "setVmsDuration", "1,0,1,1,1,1", true, false, 0, 0, 0, "ESM_FMS_0073");
	    		}
	    	}
	    }
	}
	...
	function sheet2_OnChange(sheetObj,row,col,value) {
		...
		// vnor_itm_val가 Server로 못 넘어가서
		// vnor_itm_va2로 세팅 후 Server로 넘김.
		var colName = sheetObj.ColSaveName(col);
		if(colName == "inv_"+"vnor_itm_val"){
			sheetObj.CellValue2(row, "inv_"+"vnor_itm_val2") = value;
		}		
   
		if (colName == "inv_"+"vnor_itm_val" || colName == "inv_"+"vnor_itm_prc") {
			var floatVal = parseFloat(sheetObj.CellValue(row, "inv_"+"vnor_itm_val"));
			var floatPrc = parseFloat(sheetObj.CellValue(row, "inv_"+"vnor_itm_prc"));
	
		   sheetObj.CellValue(row, "inv_"+"inv_amt") = floatVal * floatPrc;
		}	
	}
	...
	function invAmtOnChange(sheetObj,row,col,value) {
		...
		if (colAlias==(prefix + "inv_amt")) {
			var row_inv_amt = 0;
			for(var ir=1; ir<=sheetObj.LastRow; ir++) {
				row_inv_amt = row_inv_amt + parseFloat(sheetObj.CellText(ir,col).trimAll(","));
			}
			
			var currCdCol = sheetObj.SaveNameCol(prefix + "curr_cd");
			var currCdValue = sheetObj.CellText(1,currCdCol).toLowerCase();
			
			if (document.all.totalAmount.style.display != "none") {
				eval("form."+currCdValue+"_inv_amt").value = ComAddComma(ComTrunc(row_inv_amt));
			}
    ...
    function setBunkerVvd(flag) {
    	...
    	CoFmsGetBizCombo('FORM', document.form, sheetObjects[0], 'bunker_vvd', 'bunker_vvd_text', '2', 'ESM_FMS_0091GS.do', 'FMS01232');
	...
	function sheet2_OnPopupClick(sheetObj,Row,Col){
		if(sheetObj.RowStatus(Row) == "I") {
			ComOpenPopup("ESM_FMS_0076.do?flet_acct_cate_cd=OF", 550, 455, "setProgramNo", "1,0,1,1,1,1", true, false, Row, Col, 0);
		} else {
			return;
		}
	}

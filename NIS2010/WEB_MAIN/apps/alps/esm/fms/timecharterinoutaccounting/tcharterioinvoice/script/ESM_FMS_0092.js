...
function loadPage() {
    ...
    for(var i=0; i < uploadObjects.length ; i++){
	    //1. 기본 환경 설정
	    ComConfigUpload(uploadObjects[i], "/hanjin/ESM_FMS_0092GS.do");	
	}    
...
function initSheet(sheetObj, sheetNo) {
	...
	with (sheetObj) {
		...
		case "sheet1": 
			...
			var HeadTitle1 = "|Sel.|Seq.|File Name|ID|Date|Download|atch_file_lnk_id|atch_file_lnk_seq|file_path_url|file_sav_id|atch_file_oa_lnk_id|atch_file_oa_lnk_seq";
			...
			InitDataProperty(0, cnt++ , dtPopup,      		280,  daLeft,      false,   "file_upld_nm",   	true,       "",    dfNone,       0,          false,		true,	50);						
            ...
            InitDataProperty(0, cnt++ , dtHidden,	    	80,		daCenter,	true,		"atch_file_lnk_id");
            InitDataProperty(0, cnt++ , dtHidden,	    	30,		daCenter,	true,		"atch_file_lnk_seq");
            InitDataProperty(0, cnt++ , dtHidden,	    	30,		daCenter,	true,		"file_path_url");
            InitDataProperty(0, cnt++ , dtHidden,	    	30,		daCenter,	true,	    "file_sav_id");
            InitDataProperty(0, cnt++ , dtHidden,	    	80,		daCenter,	true,		"atch_file_oa_lnk_id");
            InitDataProperty(0, cnt++ , dtHidden,	    	30,		daCenter,	true,		"atch_file_oa_lnk_seq");
...
function processButtonClick() {
	...
	var openSheetObj = opener.document.t1sheet1;       // New) Off-Hire Expenses
	var openSheetObj2 = opener.document.sheet1;        // Bunker Data Management
	var openSheetObj3 = opener.document.sheetOACncl;   // Multi-Cancellation for O/A
	var oaSheetObj   = opener.document.sheet1;         // Owner's Account Entry
	var openSheetObj4 = opener.document.sheet;         // New) Owner's Account
	
		
/*	if(form.tab_gubun.value == "sheet_") { 
		var oaSheetObj = opener.document.sheet;    // Owner's Account 
	} else {
		var oaSheetObj = opener.document.sheet1;   // New) Owner's Account
	}*/
	
	var tabGubun = form.tab_gubun.value;
	var chkFlg = form.chk_flg.value;
	...
	switch (srcName) {	
        ...
        case "btn2_Save":
        	...
        	if(tabGubun == "slp_") { // Owner's Account Entry
        		oaSheetObj.CellValue(form.row.value, tabGubun + "atch_file_oa_lnk_cnt") = sheetObjects[0].RowCount("R") ;
        		
        	} else if(tabGubun == "sheet_") {  //New) Owner's Account, Multi-Cancellation for O/A
        		
        		if(chkFlg == "Y") { //New) Owner's Account
        			openSheetObj4.CellValue(form.row.value, tabGubun + "attach") = sheetObjects[0].RowCount("R") ;
        		}
        		else {
        			openSheetObj3.CellValue(form.row.value, tabGubun + "atch_file_oa_lnk_cnt") = sheetObjects[0].RowCount("R") ;
        		}
        		
        	} else if(tabGubun == "sheet1") { // Bunker Data Management
        		openSheetObj2.CellValue(form.row.value, "atch_file_lnk_cnt") = sheetObjects[0].RowCount("R") ;
             			
        	} else { // New) Off-Hire Expenses
        		openSheetObj.CellValue(form.row.value, tabGubun + "atch_file_flet_knt") = sheetObjects[0].RowCount("R") ; 
        	}
            break;
		case "btn1_Close":
    		if(tabGubun == "slp_") {  // Owner's Account Entry
    			oaSheetObj.CellValue(form.row.value, tabGubun + "atch_file_oa_lnk_cnt") = sheetObjects[0].RowCount("R") ;    			
    			if(oaSheetObj.CellValue(form.row.value, tabGubun + "ibflag") != "I"){
        			oaSheetObj.CellValue(form.row.value, tabGubun + "ibflag") = "R";    				
    			}
...
function sheet1_OnPopupClick(sheet,row,col){
	...
	if(fileSize >= 5485760){		
		sheet.RowDelete(sheet.row, false);
		alert("Each attached file size will not exceed 5MB ["+fileSize+"]");
		return false;
	}
	
	if(fileSize == 0) {
		alert("Please close file");
		return false;
	}
	
		
	if(fileName.indexOf("\\") !=-1) {
		sheet.CellValue2(row, "file_path_url")= fileName;
...
function doActionIBSheet(sheetObj, formObj, sAction) {	
	 ...
	 switch (sAction) {
	 	...
	 	case IBSEARCH: //조회
			...
			var sXml = sheetObj.GetSearchXml("ESM_FMS_0092GS.do", FormQueryString(frm));
		...
		// 일반
	 			...
	 			var sXml = sheetObj.GetSaveXml("ESM_FMS_0092GS.do",  param);			
	 			sheetObj.LoadSaveXml(sXml);			
	 			var errorYn = ComGetEtcData( sXml, "ERROR_YN");
	 			if (errorYn == "N") {
	 				doActionIBSheet(sheetObjects[0], document.form, IBSEARCH);

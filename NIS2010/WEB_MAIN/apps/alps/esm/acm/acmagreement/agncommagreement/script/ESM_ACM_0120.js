...
/**
 * @extends
 * @class esm_acm_0120 : esm_acm_0120 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_acm_0120() {
...
var sheetCnt = 0;
var URL_ESM_ACM = 'ESM_ACM_0120GS.do';
var prefix = "sheet1_";
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtDummyCheck, 30, daCenter, false, "radio", false, "", dfNone, 0, true, true);
	...
	try {
		...
		case "btn_Select":
			...
			for(var ir=1; ir<=sheetObject.LastRow; ir++) {
				if(sheetObject.CellValue(ir, "radio") == 1){
					chgList = chgList + sheetObject.CellValue(ir, "chg_cd") + ",";
				}
			}
			
			opener.sheetObjects[8].CellValue(1, "comm_chg_cd") = chgList.substring(0, chgList.lastIndexOf(","));
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg) {
	...
	var chgList = document.form.chg_list.value.split(",");

	for(var ir=1; ir<=sheetObj.LastRow; ir++) {
		for(var k=0; k<chgList.length; k++){
			if(sheetObj.CellValue(ir, "chg_cd") == chgList[k]){
				sheetObj.CellValue(ir, "radio") = 1;
			}
		}
	}
...
//		}
		...
		// 2.조회조건으로 조회실행
		sheetObj.DoSearch4Post(URL_ESM_ACM, '', FormQueryString(formObj), false);
		// 3.조회후 결과처리
	...
	case IBSEARCHAPPEND:
		...
		// 2.조회조건으로 조회실행
		sheetObj.DoSearch4Post(URL_ESM_ACM, '', FormQueryString(formObj), true);
		break;
	}
}

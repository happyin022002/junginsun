...
//        }
    ...
    function loadPage() {
        ...
        if (document.form.brkg_flg.value == 1){
        	generalBrkg = "Brokerage";
        	document.form.non_ddc_rev.value = document.form.brkg_crnt_rev_amt.value;	
        	document.form.ttl_chr_ddc.value = document.form.brkg_ddct_chg_amt.value;	
        	document.form.ttl_trs_ddc.value = document.form.brkg_ddct_trsp_amt.value;	
        	document.form.net_rev.value = document.form.brkg_post_rev_amt.value;	
        	
        } else if(document.form.brkg_flg.value == 2){
        	generalBrkg = "Cross";
        	document.form.non_ddc_rev.value = document.form.crs_crnt_rev_amt.value;	
        	document.form.ttl_chr_ddc.value = document.form.crs_ddct_chg_amt.value;	
        	document.form.ttl_trs_ddc.value = document.form.crs_ddct_trsp_amt.value;	
        	document.form.net_rev.value = document.form.crs_post_rev_amt.value;	
        } else {
        	generalBrkg = "General Commission";
        	document.form.non_ddc_rev.value = document.form.crnt_rev_amt.value;	
        	document.form.ttl_chr_ddc.value = document.form.ddct_chg_amt.value;	
        	document.form.ttl_trs_ddc.value = document.form.ddct_trsp_amt.value;	
        	document.form.net_rev.value = document.form.post_rev_amt.value;	
        }
...
//                    SheetWidth = mainTableB2S1.clientWidth;    // 200px
                ...
                case "box3sheet4":    // 2017.08.25 Charge Commission 추가
                    var cnt = 0;
                    // 높이 설정
                    style.height = GetSheetHeight(5);

                    // 전체 너비 설정
                    SheetWidth = mainTableB3S4.clientWidth;    // 829px

                    // 행정보설정[필수][HEADROWS, DATAROWS, VIEWROWS, ONEPAGEROWS=500]
                    InitRowInfo(2, 1, 13, 500);

                    // 컬럼 헤더타이틀
                    var HeadTitle0  = "STS|Charge|Bound|Rate|Curr.|Per|BKG AMT|Commission|Commission|Commission|Commission";
                    var HeadTitle1  = "STS|Charge|Bound|Rate|Curr.|Per|BKG AMT|Comm. Rate|Comm. Rate|LCL AMT|USD AMT";
                    ...
                    InitDataProperty(0, cnt++, dtData,         100,    daCenter,    true,    "chg_cd");
                    InitDataProperty(0, cnt++, dtData,         60,     daCenter,    true,    "io_bnd_cd");
                    InitDataProperty(0, cnt++, dtData,         100,    daRight,     true,    "chg_ut_amt",    		false,    "",    dfFloat,    2);
                    InitDataProperty(0, cnt++, dtData,         60,     daCenter,    true,    "curr_cd");
                    InitDataProperty(0, cnt++, dtData,         50,     daCenter,    true,    "rat_ut_cd");
                    InitDataProperty(0, cnt++, dtData,         100,    daRight,     true,    "chg_amt",    			false,    "",    dfFloat,    2);
                    InitDataProperty(0, cnt++, dtData,         80,     daRight,     true,    "chg_comm_rt",        	false,    "",    dfFloat,    2);
                    InitDataProperty(0, cnt++, dtData,         40,     daCenter,    true,    "chg_comm_curr_cd");
                    InitDataProperty(0, cnt++, dtData,         120,    daRight,     true,    "pay_comm_amt",        false,    "",    dfFloat,    2);
                    InitDataProperty(0, cnt++, dtData,         100,    daRight,     true,    "comm_amt",        	false,    "",    dfFloat,    2);

                    ColIndent("chg_ut_amt") = 2;
                    ColIndent("chg_amt") = 2;
                    ColIndent("chg_comm_rt") = 2;
                    ColIndent("pay_comm_amt") = 2;
                    ColIndent("comm_amt") = 2;
    ...
    function doActionIBSheet(shtObj, frmObj, sAction, CondParam, PageNo) {
        switch (sAction) {
            ...
            case IBSEARCH:       // 조회
                ...
                var xmlStr = shtObj.GetSearchXml("ESM_ACM_0105GS.do", FormQueryString(frmObj)).split("|$$|");
                for (var i=0; i<xmlStr.length-1; i++) {
                	
                    sheetObjects[i].LoadSearchXml(xmlStr[i]);
                }
                sheetObjects[4].LoadSearchXml(xmlStr[9], true, -1); 
                if( frmObj.brkg_flg.value == 1 ) {
                	...
                	} else if(frmObj.brkg_flg.value == 2){
                		frmObj.non_ddc_rev.value = ComAddComma2(frmObj.non_ddc_rev.value==""?"0":frmObj.non_ddc_rev.value, "#,###.00"); //Non Deducted Rev.
                        frmObj.ttl_chr_ddc.value = ComAddComma2(frmObj.ttl_chr_ddc.value==""?"0":frmObj.ttl_chr_ddc.value, "#,###.00"); //Total Charge Deduction
                        frmObj.ttl_trs_ddc.value = ComAddComma2(frmObj.ttl_trs_ddc.value==""?"0":frmObj.ttl_trs_ddc.value, "#,###.00"); //Total Transportation Deduction
                        frmObj.net_rev.value = ComAddComma2(frmObj.net_rev.value==""?"0":frmObj.net_rev.value, "#,###.00"); //Net Revenue

			...
			case 1:      //sheet1 init
				with (sheetObj) {
				   ...
				   //전체Edit 허용 여부 [선택, Default false]
					...
					InitColumnInfo(33, 4 , 0, true);

					// 해더에서 처리할 수 있는 각종 기능을 설정한다
					InitHeadMode(true, true, false, true, false,false) ;

					var HeadTitle = "Del.|STS|SEQ|Office|AGMT Customer|Name|Shipper|Name|POR TP|POR|POL TP|POL|POD TP|POD|EFF DT|EXP DT|SC No|RFA No|Commodity TP|Commodity|Commodity|Type|CHG Type|Rate|Box AMT|TEU AMT|FEU AMT|REU AMT|RTEU AMT|RFEU AMT|Specific CHG|Kind";
					...
					InitDataProperty(0, cnt++ , dtData,       70,    daCenter,   false,    "ar_ofc_cd",           false,     "",         dfNone,          0,     false,      true,       5);
					InitDataProperty(0, cnt++ , dtPopupEdit,  120,    daCenter,   false,    "brog_cnt_cust_seq",   true,     "",         dfNone,          0,     false,      true,       8);
					...
					InitDataProperty(0, cnt++ , dtData,       70,    daCenter,   false,    "to_eff_dt",           false,    "",         dfDateYmd,       0,     true,       true);
      				InitDataProperty(0, cnt++ , dtPopupEdit,  70,    daCenter,   false,    "sc_no",               false,    "",         dfNone,          0,     true,       true,       9);
					InitDataProperty(0, cnt++ , dtPopupEdit,  70,    daCenter,   false,    "rfa_no",              false,    "",         dfNone,          0,     true,       true,       11);
					...
					InitDataProperty(0, cnt++ , dtCombo,      120,    daLeft,   false,    "brog_div_cd",         false,    "",         dfNone,          0,     true,       true);
					...
					InitDataProperty(0, cnt++ , dtHidden,       70,    daRight,    false,    "brog_rf_rt",          false,    "",         dfFloat,         3,     true,       true,       15);
					InitDataProperty(0, cnt++ , dtData,      70,    daRight,     false,    "brog_rteu_rt",       false,    "",         dfFloat,          3,     true,       true,       15);
					InitDataProperty(0, cnt++ , dtData,      70,    daRight,     false,    "brog_rfeu_rt",       false,    "",         dfFloat,          3,     true,       true,       15);
					InitDataProperty(0, cnt++ , dtPopupEdit,      130,    daLeft,     false,    "brog_chg_ctnt",       false,    "",         dfNone,          0,     true,       true,       50);
					InitDataProperty(0, cnt++ , dtCombo,      80,    daLeft,   false,    "brog_knd_cd",         false,    "",         dfNone,          0,     true,       true);
					InitDataProperty(0, cnt++ , dtHidden,      50,    daCenter,   false,   "brog_rt_seq",         false,    "",         dfNone,          0,     false,      false);
					...
					InitDataCombo (0,"brog_div_cd",brogDivText,brogDivCode);
					InitDataCombo (0,"brog_tp_cd",brogTpText,brogTpCode);
					InitDataCombo (0,"brog_knd_cd",brogKndText,brogKndCode);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_AGT_0057GS.do", agtQryStr(formObj));
				break;

			case IBSAVE:        //저장
				formObj.f_cmd.value = MULTI;
				sheetObj.DoSave("ESM_AGT_0057GS.do", agtQryStr(formObj));
				break;
			...
			case IBINSERT:      // 입력
				...
				sheetObj.CellValue(newRow, "ar_ofc_cd") = formObj.ofc_cd.value;
...
//2011.07.13 화면에서 POL/POD type을 지정하는 경우 기 입력되어 있던 POL/POD 정보가 삭제되지 않도록 program 변경 
/*            if( saveNm=="por_grp_tp_cd" || saveNm=="pol_grp_tp_cd" || saveNm=="pod_grp_tp_cd" ) {
               ...
               if( Value.trim() != "" ) {
                   ...
                   if( !/[A-Z]{2}[0-9]{6}/.test(Value) && "000000" != Value && "*000000" != Value ) {
                       ...
                       if( seq == 999999 ) {
                    	   ...
                    	   sRow = Row;
                    	   form1.param1.value = Value.trim();
                    	   form1.target = "frmHidden";
                    	   form1.action = "ESM_AGT_0057FR.do";
                    	   form1.submit();
                    	   /*
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if ( saveNm == "brog_cnt_cust_seq" || saveNm == "shpr_cnt_seq" ) {
			...
			ComOpenPopup(url, width, height, func, display, true, false, Row, Col);
		}else if( saveNm == "sc_no" || saveNm == "rfa_no" || saveNm == "brog_chg_ctnt") {
			...
			var cCustCntCd = "";
			var cCustSeq = "";
			formObj.rowNum.value = Row;
			formObj.colNum.value = Col;
			...
			if(saveNm == "rfa_no"){
				width = 860;
				height = 475;
				url = "ESM_BKG_0654.do?pgmNo=ESM_BKG_0654&bkg_no="+bkgNo+"&bkg_vvd="+bkgVvd+"&por_cd="+porCd+"&del_cd="+delCd+"&s_cust_cnt_cd="+sCustCntCd+"&s_cust_seq="+sCustSeq+"&c_cust_cnt_cd="+cCustCntCd+"&c_cust_seq="+cCustSeq+"&func="+func;
			}else if(saveNm == "brog_chg_ctnt"){

			   width = 760;
			   height = 438;
			   func = "sheet1_setSheetData5";
			   
	    	   var classId = "ESM_AGT_0005";
	    	   var sheet = "4";

	    	   url = 'ESM_AGT_0005.do?pgmNo=ESM_AGT_0005&classId='+classId+'&sheet='+sheet;
	 ...
	 */
	function sheet1_setSheetData5(rowArray, row, col) {
		var formObj = document.form;
		var sheetObj = sheetObjects[0];
		var colArray = rowArray[0];
		row = formObj.rowNum.value;
		...
		sheetObj.CellValue2(row, col) = colArray;
	}
	/**
	...
	function setBrogDivCd( sheetObj, Row, Col ) {
		...
		with(sheetObj) {
			...
			if ( value == "BA" || value == "BF" ) {
				...
				CellValue2(Row, "brog_rteu_rt") = 0;
				CellValue2(Row, "brog_rfeu_rt") = 0;
				...
				CellEditable(Row, "brog_rteu_rt") = false;
				CellEditable(Row, "brog_rfeu_rt") = false;
				...
				CellEditable(Row, "brog_rteu_rt") = true;
				CellEditable(Row, "brog_rfeu_rt") = true;
				CellEditable(Row, "brog_chg_ctnt") = false;
	...
	function checkBrogDivCd( sheetObj, Row, Col ) {
		...
		with(sheetObj) {
			...
			if ( value == "BA" || value == "BF" ) { // 반드시 입력
                ...
                var brog_rteu_rt = CellValue(Row, "brog_rteu_rt").trim();
                var brog_rfeu_rt = CellValue(Row, "brog_rfeu_rt").trim();

                if(isNaN(brog_teu_rt)) brog_teu_rt = "0.000";
                if(isNaN(brog_feu_rt)) brog_feu_rt = "0.000";
                if(isNaN(brog_rteu_rt)) brog_rteu_rt = "0.000";
                if(isNaN(brog_rfeu_rt)) brog_rfeu_rt = "0.000";

                if( Math.floor(brog_teu_rt) > 0 || Math.floor(brog_feu_rt) > 0 || Math.floor(brog_rteu_rt) > 0 || Math.floor(brog_rfeu_rt) > 0) {
                    var flg = true;
                    ...
                    if( Math.floor(brog_rteu_rt) < 0 ) {
                        flg = false;
                    }
                    if( Math.floor(brog_rfeu_rt) < 0 ) {
                        flg = false;
                    }
    ...
    function setCustomerName(){
    	...
    	sheetObj.CellValue2(sRow, "brog_cnt_cust_nm") = formObj.custNm.value;
    }

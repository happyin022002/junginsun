...
/*
 * Approval하기 위해 선택한 행의 정보를 Array에 담는다.
 */
...
/*
 * 그리드에서 Amount가 변하면 USD_AMT, LCL_AMT 재계산
 */
			...
			case IBSEARCH_ASYNC03:	//Audit
				...
				for(var i=0; i<rows; i++) {
				    if(sheetObj.CellText(i+2,"check") == 1){
				    	...
				    	var bkg_no = sheetObj.CellValue(i+2,"bkg_no");
				    	var ac_seq = sheetObj.CellValue(i+2,"ac_seq");
						...
						if(bkg_sts_cd == "X" && ac_seq ==1 ){
							ComShowCodeMessage("AGT10092", bkg_no, "", "");
							sheetObj.CellValue(i+2,"check") = 0;//
							return false;
						}
		     		}

	...
	switch(sheetNo) {
		case 1:      //sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "Del.|STS|SEQ|Customer|Vendor|Name|*|*|*|*";
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	switch(sAction) {
	    case IBSEARCH:		//조회
			...
			sheetObj.DoSearch4Post("ESM_AGT_0058GS.do", agtQryStr(formObj));
			break;

		case IBSAVE:		//저장
			if(!validateForm(sheetObj,formObj,sAction))	return false;
			formObj.f_cmd.value = MULTI;
			sheetObj.DoSave("ESM_AGT_0058GS.do", agtQryStr(formObj));
			break;
...
//        if(cust_cd == "") cust_cd = formObj.cust_cnt_cd.value;
        ...
        if(chkStr == "1,0") {
        	//Radio PopUp
        	ComOpenPopup('/hanjin/COM_ENS_041.do' + param, 770, 470, 'getESM_AGT_058_1', dispaly, true, false, row, col, 0);
        }
        /*
        else if(chkStr == "0,1") {
...
//        alert(sheetObj.CellValue(row,"vndr_cnt_cd"));
        ...
        if(chkStr == "1,0") {
        	//Radio PopUp
        	ComOpenPopup('/hanjin/COM_ENS_0C1.do' + param, 770, 470, 'getESM_AGT_058_4', dispaly, true, false, row, col, 0);
        }
        /*
        else if(chkStr == "0,1") {
            //CheckBox PopUp (멀티 데이터 선택용) => Sheet를 대상으로 하는 경우는 의미가 없음
 ...
 */
function getESM_AGT_058_1(rowArray, row, col) {
 ...
 */
function getESM_AGT_058_4(rowArray, row, col) {
    var sheetObj = sheetObjects[0];

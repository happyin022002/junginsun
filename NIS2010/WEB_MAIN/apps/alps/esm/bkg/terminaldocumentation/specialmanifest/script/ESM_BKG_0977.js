...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_0977 : ESM_BKG_0977 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0977() {
             ...
             case "sheet1":
                 with (sheetObj) {
                    ...
                    InitDataProperty(0, cnt++ , dtCheckBox,		30, 	daCenter,	true, 		prefix+"del_chk",			false,		"",  	dfNone,			0,		true,		true);
 					InitDataProperty(0, cnt++ , dtData,			120,	daCenter,	true,		prefix+"imdg_un_no",		false,		"",		dfNone,			0,		true,		true,	4);
 					InitDataProperty(0, cnt++ , dtCombo,		0,		daLeft,		true,		prefix+"spcl_id_desc",		false,		"",		dfNone,			0,		true,		true);

 					InitDataProperty(0, cnt++ , dtHidden,		120,	daCenter,	true,		prefix+"old_imdg_un_no",	false,		"",		dfNone,			0,		true,		true);
 					InitDataProperty(0, cnt++ , dtHidden,		120,	daCenter,	true,		prefix+"anr_spcl_tp_id",	false,		"",		dfNone,			0,		true,		true);

 					//InitDataCombo(0, prefix+"spcl_id_desc", "|AMN: Ammonium Nitrate|SPR : Explosives|ZTG : Very Toxic Gases", "|AMN|SPR|ZTG",	"", "", 0);
 					//InitDataCombo(0, prefix+"spcl_id_desc", " |AMN: Ammonium Nitrate|SPR : Explosives|ZTG : Very Toxic Gases", " |AMN: Ammonium Nitrate|SPR : Explosives|ZTG : Very Toxic Gases",	" ");
 					InitDataCombo(0, prefix+"spcl_id_desc", " |Ammonium Nitrate|Explosives|Very Toxic Gases", " |AMN|SPR|ZTG",	" ");

 					CountPosition = 0;
 					
					InitDataValid(0, prefix+"imdg_un_no",  vtNumericOnly);
   ...
   // Sheet관련 프로세스 처리
     ...
     function sheet1_OnDblClick(sheetObj,Row,Col) {
    	...
    	var pageType = document.form.pageType.value;
     	...
     	if(pageType != "MAIN") {
			var obj = new Object(); 
			obj.cd = sheetObj.CellValue(Row, "sheet1_imdg_un_no");
    	 ...
    	 switch(sAction) {
			...
			case IBSAVE: {
				...
				for(var i=1; i <= sheet1RowCnt; i++) {
					...
					if(sheetObj.CellValue(i,"sheet1_imdg_un_no") == "") {
			...
			case COMMAND01 :
				...
				if(sheet1RowCnt == 0 || (sheet1RowCnt == 1 && (sheetObj.CellValue(sheetObj.SelectRow,"sheet1_imdg_un_no") == "" || sheetObj.CellValue(sheetObj.SelectRow,"spcl_id_desc") == ""))) {
					ComShowCodeMessage('BKG00095');
					return false;
				}
     ...
     function sheet1_OnSaveEnd(sheetObj, ErrMsg) {
 		...
 		if (ErrMsg == "") {
 	    	...
 	    	for(i=1; i <= maxSheetLen; i++) {
 	    		sheetObj.CellValue(i, 'sheet1_old_imdg_un_no') = sheetObj.CellValue(i, 'sheet1_imdg_un_no');
 	    	}

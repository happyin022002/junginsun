...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
             ...
             case "sheet1":
                 with (sheetObj) {
                    ...
                    InitDataProperty(0, cnt++ , dtCheckBox,		40, 	daCenter,	true, 		prefix + "del_chk",			false,"",  dfNone,0,true,true);
 					InitDataProperty(0, cnt++ , dtData,			150,	daCenter,	true,		prefix + "anr_fwrd_id",		false,		"",		dfEngUpKey,		0,		true,		true, 6);
 					InitDataProperty(0, cnt++ , dtData,			0,		daLeft,		true,		prefix + "fwrd_nm",			false,		"",		dfNone,			0,		true,		true);

 					InitDataProperty(0, cnt++ , dtHidden,		150,	daCenter,	true,		prefix + "old_anr_fwrd_id",	false,		"",		dfNone,			0,		true,		true);
   ...
   // Sheet관련 프로세스 처리
    	...
    	var sheet1RowCnt = sheetObj.RowCount;
  	    ...
  	    switch(sAction) {
			case IBSEARCH: { // 조회
    			...
    			/*
    			var anrFwrdId = formObj.anr_fwrd_id.value;
    			var fwrdNm = formObj.fwrd_nm.value;
    			
    			if(anrFwrdId == "" && fwrdNm == "") {
					//ComShowMessage("Forwarder Code또는Forwarder Name 중 반드시 1개 이상 입력해야합니다.");
			...
			case IBSAVE: {
				...
				for(var i=1; i <= sheet1RowCnt; i++) {
					...
					if(sheetObj.CellValue(i,"sheet1_anr_fwrd_id") == "") {
						
						//ComShowMessage("저장대상중 Forwarder Code가 없는 Row가 있습니다.");
			...
			case COMMAND01 :
				...
				if(sheet1RowCnt == 0 || (sheet1RowCnt == 1 && (sheetObj.CellValue(sheetObj.SelectRow,"sheet1_anr_fwrd_id") == "" || sheetObj.CellValue(sheetObj.SelectRow,"sheet1_anr_fwrd_nm") == ""))) {
					ComShowCodeMessage('BKG00095');
					return false;
				}
     ...
     function sheet1_OnDblClick(sheetObj,Row,Col) {
     	...
     	//alert("Forwarder Name이 Dangerous Cargo Inquiry 화면에 해당 칼럼에 자동 기입되고 화면은 닫힘.");
    	var pageType = document.form.pageType.value;
    	
    	if(pageType != "MAIN") {
			var obj = new Object(); 
			   
			obj.cd = sheetObj.CellValue(Row, "sheet1_anr_fwrd_id");
			obj.nm = sheetObj.CellValue(Row, "sheet1_fwrd_nm");
    ...
    function sheet1_OnSaveEnd(sheetObj, ErrMsg) {
		...
		if (ErrMsg == "") {
	    	...
	    	for(i=1; i <= maxSheetLen; i++) {
	    		sheetObj.CellValue(i, 'sheet1_old_anr_fwrd_id') = sheetObj.CellValue(i, 'sheet1_anr_fwrd_id');
	    	}

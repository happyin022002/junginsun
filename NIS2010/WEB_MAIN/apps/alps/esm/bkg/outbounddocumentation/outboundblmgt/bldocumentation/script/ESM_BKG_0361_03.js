...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0361_03 : esm_bkg_0361_03 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0361_03() {
 		...
 		switch(sheetNo) {
 			case 1:      //sheet1 init
 				with (sheetObj) {
 					...
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"shpr_tax_no",		false,	"",			dfNone,		0,			false,		false,14);
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"cnee_tax_no",		false,	"",			dfNone,		0,			false,		false,14);
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"ntfy_tax_no",		false,	"",			dfNone,		0,			false,		false,14);
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"brz_decl_no",		false,	"",			dfNone,		0,			false,		false,11);
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					formObj.caFlg.value = (sheetObj.EtcData("caFlg"));
 					formObj.bdrFlg.value = (sheetObj.EtcData("bdrFlg"));
// 					BDR이고 C/A일 경우 Import 화면 버튼(Delete,Save) 및 일부 항목 비활성화
 					if(formObj.caFlg.value =='N' && formObj.bdrFlg.value == 'Y' && document.form3.get_io_bnd_cd.value=="I"){
 						disableImport(sheetObj);
 					}
 				}
 			break;
 			...
 			case IBSAVE:        //저장
 				...
 				if(validateForm(sheetObj,formObj,sAction)) {
 					...
 					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="BR";
 					}else if(formObj.name=="form2"){
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "imp_"); 
 						sheetObj.CellValue(1,"cnt_cd")="BR";
 					}
 					
 					if(sheetObj.CellValue(1,0)=='I'&&sheetObj.CellValue(1,"shpr_tax_no")==''&&sheetObj.CellValue(1,"cnee_tax_no")==''&&sheetObj.CellValue(1,"ntfy_tax_no")==''&&sheetObj.CellValue(1,"brz_decl_no")==''){
 						sheetObj.CellValue(1,0)='D';
 					}
 					
 					// Copy To Description: Save 시점에 Copy -> Check시 Copy 로 변경
 					/*
 					if(sheetObj.isDataModified&&sheetObj.CellValue(1,0)!='D'){
 						if(formObj.name=="form"){
		 					if(formObj.exp_shpr_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Shipper CPF/CPNJ "+formObj.exp_shpr_tax_no.value;	
		 					}
		 					if(formObj.exp_cnee_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Consignee CPF/CPNJ "+formObj.exp_cnee_tax_no.value;	
		 					}
		 					if(formObj.exp_ntfy_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Notify CPF/CPNJ "+formObj.exp_ntfy_tax_no.value;	
		 					}
		 					if(formObj.exp_brz_decl_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Notify DDE/SD "+formObj.exp_brz_decl_no.value;	
		 					}
	 					}else{
	 						if(formObj.imp_shpr_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Shipper CPF/CPNJ "+formObj.imp_shpr_tax_no.value;	
		 					}
		 					if(formObj.imp_cnee_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Consignee CPF/CPNJ "+formObj.imp_cnee_tax_no.value;	
		 					}
		 					if(formObj.imp_ntfy_tax_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Notify CPF/CPNJ "+formObj.imp_ntfy_tax_no.value;	
		 					}
		 					if(formObj.imp_brz_decl_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n Notify DDE/SD "+formObj.imp_brz_decl_no.value;	
		 					}
	 					}
 					}
 					*/
 					...
 					if(document.form4.tabclosechk.value=="Y"){
 						if(sheetObj.isDataModified){							
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_03GS.do", FormQueryString(formObj) ,"");
 						}
 					}else{
 						saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_03GS.do", FormQueryString(formObj) ,"");
 					}
...
/*
		if(sAction=='2'&&sheetObj.CellValue(1,"ibflag")!='D'){	
	 		if(formObj.name=="form"){
	 			objs = document.all.item("tabLayer");
	 			if(formObj.exp_shpr_tax_no.value!=''&&formObj.exp_shpr_tax_no.value.length<14){
		 			setTab1();
	             	ComAlertFocus(formObj.exp_shpr_tax_no, ComGetMsg("COM12174","14"));
	             	
	             	return false;
		 		}
		 		if(formObj.exp_cnee_tax_no.value!=''&&formObj.exp_cnee_tax_no.value.length<14){
		 			setTab1();
	             	ComAlertFocus(formObj.exp_cnee_tax_no, ComGetMsg("COM12174","14"));
	             	return false;
		 		}	
		 		if(formObj.exp_ntfy_tax_no.value!=''&&formObj.exp_ntfy_tax_no.value.length<14){
		 			setTab1();
	             	ComAlertFocus(formObj.exp_ntfy_tax_no, ComGetMsg("COM12174","14"));
	        		return false;
		 		}	
		 		if(formObj.exp_brz_decl_no.value!=''&&formObj.exp_brz_decl_no.value.length<11){
		 			setTab1();
	             	ComAlertFocus(formObj.exp_brz_decl_no, ComGetMsg("COM12174","11"));
	        		return false;
		 		}	
	 		}else if(formObj.name=="form2"){
	 			objs = document.all.item("tabLayer");
	 			if(formObj.imp_shpr_tax_no.value!=''&&formObj.imp_shpr_tax_no.value.length<14){
	 				setTab2();
	             	ComAlertFocus(formObj.imp_shpr_tax_no, ComGetMsg("COM12174","14"));
	             	return false;
		 		}
		 		if(formObj.imp_cnee_tax_no.value!=''&&formObj.imp_cnee_tax_no.value.length<14){
		 			setTab2();
	             	ComAlertFocus(formObj.imp_cnee_tax_no, ComGetMsg("COM12174","14"));
	        		return false;
		 		}	
		 		if(formObj.imp_ntfy_tax_no.value!=''&&formObj.imp_ntfy_tax_no.value.length<14){
		 			setTab2();
	             	ComAlertFocus(formObj.imp_ntfy_tax_no, ComGetMsg("COM12174","14"));
	        		return false;
		 		}	
		 		if(formObj.imp_brz_decl_no.value!=''&&formObj.imp_brz_decl_no.value.length<11){
		 			setTab2();
	             	ComAlertFocus(formObj.imp_brz_decl_no, ComGetMsg("COM12114","11"));
	        		return false;
		 		}
	 		}	 		
		} 
*/	
 ...
 // 시트 데이터 초기화
    ...
    function disableImport(sheetObj) {
    	formObj.imp_shpr_tax_no.readOnly = true;
		formObj.imp_cnee_tax_no.readOnly = true;
		formObj.imp_ntfy_tax_no.readOnly = true;
		formObj.imp_brz_decl_no.readOnly = true;
		formObj.imp_shpr_tax_no.className = "input2";
		formObj.imp_cnee_tax_no.className = "input2";
		formObj.imp_ntfy_tax_no.className = "input2";
		formObj.imp_brz_decl_no.className = "input2";
		ComBtnDisable("btn_delete2");
		ComBtnDisable("btn_save2");
    }

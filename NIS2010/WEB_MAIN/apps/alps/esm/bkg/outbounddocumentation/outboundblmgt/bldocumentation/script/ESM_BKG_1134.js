...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class ESM_BKG_1054 : ESM_BKG_1051 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_1134() {
    	...
    	try {
            ...
            switch(srcName) {
				...
				case "btn_select":
					...
					var tcnt = sheetObjects[0].LastRow;
					for ( var ix = 1; ix <= tcnt; ix++) {
						if(sheetObjects[0].CellValue(ix,"Chk")==1){
						var obj = new Object();
						 obj.cntr_no= sheetObjects[0].CellValue(ix, "cntr_no");
						 obj.cntr_tpsz_cd= sheetObjects[0].CellValue(ix, 'cntr_tpsz_cd');
						 obj.cntr_seal_no= sheetObjects[0].CellValue(ix, 'cntr_seal_no');
						 obj.cntr_vol_qty= sheetObjects[0].CellValue(ix, 'cntr_vol_qty');
						 obj.pck_qty= sheetObjects[0].CellValue(ix, 'pck_qty');
						 obj.pck_tp_cd= sheetObjects[0].CellValue(ix, 'pck_tp_cd');
						 obj.cntr_wgt= sheetObjects[0].CellValue(ix, 'cntr_wgt');
						 obj.meas_qty= sheetObjects[0].CellValue(ix, 'meas_qty');
						 obj.wgt_ut_cd='KGS';
						 obj.bkg_no = formObject.bkg_no.value;
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle1 = "Sel.|Container No.|TS|Seal No.|Vol.|Package|Package|Weight|Measure";
                  ...
                  //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                   ...
                   // InitDataProperty(0, cnt++, dtRadioCheck,   40, 	  daCenter, false,    "radio", 		 false, 	"", 		dfNone, 	0, 		true, true);
					...
					InitDataProperty(0, cnt++, dtData,         100,    daCenter,  true,    "cntr_no",       false,    "",         dfNone,    2,          false,       false);      
					InitDataProperty(0, cnt++, dtData,         40,    daCenter,  true,    "cntr_tpsz_cd",  false,    "",         dfNone,    2,          false,       false);      
					InitDataProperty(0, cnt++, dtData,         100,    daCenter,  true,    "cntr_seal_no",   false,    "",         dfNone,    2,          false,       false);      
					InitDataProperty(0, cnt++, dtData,         50,    daRight,  false,    "cntr_vol_qty",    false,    "",         dfNone,    2,          false,       false);
					InitDataProperty(0, cnt++, dtData,         60,    daRight,  true,    "pck_qty",     false,    "",         dfNone,    2,          false,       false);
					InitDataProperty(0, cnt++, dtData,         50,    daCenter,  true,    "pck_tp_cd",     false,    "",         dfNone,    2,          false,       false);
					
					InitDataProperty(0, cnt++, dtData,         80,    daRight,  true,    "cntr_wgt",      false,    "",         dfNone,    2,          false,       false);      
					InitDataProperty(0, cnt++, dtData,         60,    daRight,  true,    "meas_qty",      false,    "",         dfNone,    2,          false,       false);
        ...
        switch(sAction) {
			...
			case IBSEARCH:      //조회
				if(validateForm(sheetObj,formObj,sAction)) {
                    ...
                    var rXml = sheetObj.GetSearchXml("ESM_BKG_1134GS.do", FormQueryString(formObj));
					//alert(rXml);

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_1118 : ESM_BKG_1118 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_1118() {
...
// 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
    ...
    function loadPage() {
		...
		if(document.form.sr_sts_cd.value =='I'){
        ...
        switch(sAction) {
          case IBSEARCH:      //조회
				formObject.f_cmd.value = SEARCH;
          		ComOpenWait(true);
          		var sXml = sheetObj.GetSearchXml("ESM_BKG_1118GS.do", FormQueryString(formObject));
				ComOpenWait(false);
          		ComOpenWait(true);
          ...
          case IBSAVE:        //저장
		  		if(validateForm(formObject)) {
		  			...
		  			var sXml = sheetObj.GetSaveXml("ESM_BKG_1118GS.do", FormQueryString(formObject));

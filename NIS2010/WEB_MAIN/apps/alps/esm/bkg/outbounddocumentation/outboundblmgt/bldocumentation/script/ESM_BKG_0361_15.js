...
*@FileName : ESM_BKG_0361_15.js
*@FileTitle : Export / Import Information (Togo)
*Open Issues :
*Change history :
 		...
 		switch(sheetNo) {
 			case 1:      //sheet1 init
 				with (sheetObj) {
 					...
 					var HeadTitle = "|Seq|bkg_no|io_bnd_cd|xpt_imp_seq|cnt_cd|tg_ectn_no";
 					...
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"tg_ectn_no",		false,	"",			dfNone,		0,			false,		false);
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					sheetObj.DoSearch("ESM_BKG_0361_15GS.do", FormQueryString(formObj));
...
// 					BDR이고 C/A일 경우 Import 화면 버튼(Delete,Save) 및 일부 항목 비활성화
 			...
 			case IBSAVE:        //저장
 				...
 				if(validateForm(sheetObj,formObj,sAction)) {
 					...
 					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="TG";
 					}else if(formObj.name=="form2"){
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "imp_"); 
 						sheetObj.CellValue(1,"cnt_cd")="TG";
 					}
 					if(sheetObj.CellValue(1,0)=='I'&&sheetObj.CellValue(1,"tg_ectn_no")==''){
 					...
 					if(document.form4.tabclosechk.value=="Y"){
 						if(sheetObj.IsDataModified){
							...
							if(chkIssuedFlg()){
								ComShowCodeMessage("BKG95103","ECTN","TOGO");
								break;
							}
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_15GS.do", FormQueryString(formObj) ,"");
 							if(saveMsgFlg){
 								copyToDesc();
 							}
 						}
 			...
 			case IBDELETE:      // 입력
	 			if(formObj.name=="form"){
	 				formObj.exp_tg_ectn_no.value='';
	 			}else{	
	 				formObj.imp_tg_ectn_no.value='';
	 			}
				sheetObj.CellValue(1,0)='D';	
 			break;	
...
//            }
 ...
 // 시트 데이터 초기화
    ...
    /**
     * 토고의 경우 무조건 ECTN이 B/L Body에 찍혀야 함.
     */
    ...
    function copyToDesc(){
    	...
    	var ectn = "";
    	if('O'==ioFlg){
    		ectn = document.form.exp_tg_ectn_no.value;
    		if(ectn!=""&&ectn.length>0){
    	...
    	if('I'==ioFlg){
    		ectn = document.form2.imp_tg_ectn_no.value;
    		if(ectn!=""&&ectn.length>0){
        		opener.document.form.dg_cmdt_desc.value = opener.document.form.dg_cmdt_desc.value +
    			  "\n ECTN(I) : "+ectn;
        		opener.document.form.dirty_flag.value = 'Y'
...
//    	}
    ...
    function disableExportImport(sheetObj) {
    	document.form.exp_tg_ectn_no.readOnly = true;
    	document.form2.imp_tg_ectn_no.readOnly = true;
		document.form.exp_tg_ectn_no.className = "input2";
		document.form2.imp_tg_ectn_no.className = "input2";
		ComBtnDisable("btn_delete");
		ComBtnDisable("btn_delete2");
		ComBtnDisable("btn_save");
		ComBtnDisable("btn_save2");

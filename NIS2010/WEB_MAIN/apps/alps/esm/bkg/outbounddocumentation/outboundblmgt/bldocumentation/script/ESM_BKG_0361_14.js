...
*@FileName : ESM_BKG_0361_14.js
*@FileTitle : Export / Import Information (Nigeria)
*Open Issues :
*Change history :
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0361_14 : esm_bkg_0361_14 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0361_14() {
 		...
 		switch(sheetNo) {
 			case 1:      //sheet1 init
 				with (sheetObj) {
 					...
 					var HeadTitle = "|Seq|bkg_no|io_bnd_cd|xpt_imp_seq|cnt_cd|ng_exs_ens_no";
 					...
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"ng_exs_ens_no",		false,	"",			dfNone,		0,			false,		false,14);
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					sheetObj.DoSearch("ESM_BKG_0361_14GS.do", FormQueryString(formObj));
 					formObj.caFlg.value = (sheetObj.EtcData("caFlg"));
 					formObj.bdrFlg.value = (sheetObj.EtcData("bdrFlg"));
 					formObj.issFlg.value = (sheetObj.EtcData("issFlg"));
// 					BDR이고 C/A일 경우 Import 화면 버튼(Delete,Save) 및 일부 항목 비활성화
 					if(formObj.caFlg.value =='N' && formObj.bdrFlg.value == 'Y'){
 						disableExportImport(sheetObj);
 					}
 				}
 			break;
 			...
 			case IBSAVE:        //저장
 				...
 				if(validateForm(sheetObj,formObj,sAction)) {
 					...
 					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="NG";
 					}else if(formObj.name=="form2"){
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "imp_"); 
 						sheetObj.CellValue(1,"cnt_cd")="NG";
 					}
 					if(sheetObj.CellValue(1,0)=='I'&&sheetObj.CellValue(1,"ng_exs_ens_no")==''){
 						sheetObj.CellValue(1,0)='D';
 					}
 					formObj.f_cmd.value = MULTI;
 					
 					
 					//탭을 close할 때 데이터 바뀐 것이 있고, confirm 메시지가 떴을 때 확인 버튼을 누르면 copy
 					if(document.form4.tabclosechk.value=="Y"){
 						if(sheetObj.IsDataModified){
 							 // B/L issue 탭에서 issued flg가 Y이면 데이터 수정 불가
							if(chkIssuedFlg()){
								ComShowCodeMessage("BKG95103");
								break;
							}
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_14GS.do", FormQueryString(formObj) ,"");
 							if(saveMsgFlg)
 								copyToDesc();
 						}
 					}else{ //탭 close랑 상관 없이 데이터 바뀐 것이 있고, confirm 메시지가 떴을 때 확인버튼 누르면 copy
 						if(sheetObj.IsDataModified){
 							 // B/L issue 탭에서 issued flg가 Y이면 데이터 수정 불가
							if(chkIssuedFlg()){
								ComShowCodeMessage("BKG95103");
 			...
 			case IBDELETE:      // 입력
	 			if(formObj.name=="form"){
	 				formObj.exp_ng_exs_ens_no.value='';
	 			}else{	
	 				formObj.imp_ng_exs_ens_no.value='';
	 			}
				sheetObj.CellValue(1,0)='D';	
 			break;	
...
//            }
 ...
 // 시트 데이터 초기화
    ...
    /**
     * 나이지리아의 경우 무조건 EXS or ENS 정보가 B/L Body에 찍혀야 함.
     */
    
    function copyToDesc(){
    	var ioFlg = document.form3.get_io_bnd_cd.value;
    	var exsNo = "";
    	var ensNo = "";
    	if('O'==ioFlg){
    		exsNo = document.form.exp_ng_exs_ens_no.value;
    		if(exsNo!=""&&exsNo.length>0){
    	...
    	if('I'==ioFlg){
    		ensNo = document.form2.imp_ng_exs_ens_no.value;
    		if(ensNo!=""&&ensNo.length>0){
        		opener.document.form.dg_cmdt_desc.value = opener.document.form.dg_cmdt_desc.value +
    			  "\n ENS : "+ensNo;
        		opener.document.form.dirty_flag.value = 'Y'
        	}
...
//    	if(chkObj.name="exp_ng_exs_ens_no_cpy_desc_flg"&&chkObj.checked==true){
//    		opener.document.form.dg_cmdt_desc.value = opener.document.form.dg_cmdt_desc.value +
//			  "\n EXS NO. "+document.form.exp_ng_exs_ens_no.value;
//    	}
//    	if(chkObj.name="imp_ng_exs_ens_no_cpy_desc_flg"&&chkObj.checked==true)){
//    		opener.document.form.dg_cmdt_desc.value = opener.document.form.dg_cmdt_desc.value +
//			  "\n ENS NO. "+document.form.imp_ng_exs_ens_no.value;
//    	}
    }
    ...
    function disableExportImport(sheetObj) {
    	document.form.exp_ng_exs_ens_no.readOnly = true;
    	document.form2.imp_ng_exs_ens_no.readOnly = true;
		document.form.exp_ng_exs_ens_no.className = "input2";
		document.form2.imp_ng_exs_ens_no.className = "input2";
		ComBtnDisable("btn_delete");
		ComBtnDisable("btn_delete2");
		ComBtnDisable("btn_save");
		ComBtnDisable("btn_save2");
    ...
    function chkIssuedFlg(){
    	var exp_issFlg = document.form.issFlg.value;
		var imp_issFlg = document.form2.issFlg.value;
		var flg = false;
		if("Y"==exp_issFlg || "Y"==imp_issFlg){
			flg = true;
		}
		return flg;
    }

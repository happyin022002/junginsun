...
/**
 * @extends
 * @class esm_bkg_9463 : esm_bkg_9463 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_bkg_9463() {
...
var uploadCnt = 0;
var URL_ESM_BKG = 'ESM_BKG_9463GS.do';
var prefix = "sheet1_";
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "pol_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "n3pty_ofc_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "payr_cust_cnt_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "payr_cust_seq");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "obl_iss_ofc_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "n3pty_bl_chg_ttl_amt");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "frt_term_cd");
			...
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "file_seq");
...
function sheet1_OnSaveEnd(sheetObj, ErrMsg) {
	...
	if(sheetObj.RowCount>0){
		opener.sheetObjects[0].CellValue(opener.sheetObjects[0].SelectRow ,"bl_atch") = "Y";
	}else{
		opener.sheetObjects[0].CellValue(opener.sheetObjects[0].SelectRow ,"bl_atch") = "N";
	}
...
function selectFile(sheetObj, Row, Col, _insert) {
	...
	if (Col == '' || Col == '2') {
		...
		if (typeof file_nm == "undefined" || file_nm == "File Name" || (file_nm != "" && _insert)) {
			...
			sheetObj.CellValue2(Row, prefix + "bkg_no") = formObj.bkg_no.value;
			sheetObj.CellValue2(Row, prefix + "pol_cd") = formObj.pol_cd.value;
			sheetObj.CellValue2(Row, prefix + "n3pty_ofc_cd") = formObj.n3pty_ofc_cd.value;
			sheetObj.CellValue2(Row, prefix + "payr_cust_cnt_cd") = formObj.payr_cust_cnt_cd.value;
			sheetObj.CellValue2(Row, prefix + "payr_cust_seq") = formObj.payr_cust_seq.value;
			sheetObj.CellValue2(Row, prefix + "obl_iss_ofc_cd") = formObj.obl_iss_ofc_cd.value;
			sheetObj.CellValue2(Row, prefix + "n3pty_bl_chg_ttl_amt") = formObj.n3pty_bl_chg_ttl_amt.value;
			sheetObj.CellValue2(Row, prefix + "frt_term_cd") = formObj.frt_term_cd.value;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0361_10 : esm_bkg_0361_10 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0361_10() {
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					sheetObj.DoSearch("ESM_BKG_0361_10GS.do", FormQueryString(formObj));
 				}
 			break;
 			...
 			case IBSAVE:        
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="PE";
 					...
 					if(document.form4.tabclosechk.value=="Y"){					
 						if(sheetObj.isDataModified){
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_10GS.do", FormQueryString(formObj) ,"");	
 						}
 					}else{
 						saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_10GS.do", FormQueryString(formObj) ,"");
 					}

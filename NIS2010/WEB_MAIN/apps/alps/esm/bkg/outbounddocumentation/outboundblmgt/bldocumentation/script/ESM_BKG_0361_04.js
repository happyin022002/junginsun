...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0361_04 : esm_bkg_0361_04 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0361_04() {
...
//						rowDelete();
					...
					case "pop_date":
						...
						cal.select(formObject.exp_ida_shp_bil_iss_dt, 'yyyy-MM-dd');
						break;
            } // end switch
    	}catch(e) {
    		if( e == "[object Error]") {
 		...
 		switch(sheetNo) {
 			case 1:      //sheet1 init
 				with (sheetObj) {
 					...
 					var HeadTitle = "|Sel.|Seq|bkg_no|io_bnd_cd|xpt_imp_seq|cnt_cd|ida_iec_no|Cntr No.|S/Bill No.|S/Bill Date";
 					...
 					InitDataProperty(0,	cnt++,	dtHidden,	100,	daCenter,	false,	"cnt_cd",			 false,	"",			dfNone,		0,			false,		false);
 					InitDataProperty(0,	cnt++,	dtHidden,	80,		daCenter,	false,	"ida_iec_no",		 false,	"",			dfNone,		0,			false,		false,11);
 					InitDataProperty(0,	cnt++,	dtCombo,	140,	daCenter,	false,	"cntr_no",	         false,	"",			dfNone,		0,			true,		true);
 					InitDataProperty(0,	cnt++,	dtData,		150,	daCenter,	false,	"ida_shp_bil_no",	 false,	"",			dfNone,		0,			true,		true);
// 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"ida_shp_bil_iss_dt",false,	"",			dfNone,		0,			true,		true);
 					InitDataProperty(0, cnt++, dtPopupEditFormat, 140, daCenter, false, "ida_shp_bil_iss_dt", false, "", dfDateYmd, 0, true,  true, 8);
 			...
 			case 2:      //sheet1 init
 				with (sheetObj) {
 				   ...
 				   //전체Edit 허용 여부 [선택, Default false]
 					...
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"ida_iec_no",		false,	"",			dfNone,		0,			false,		false,11);
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					var sXml = sheetObj.GetSearchXml("ESM_BKG_0361_04GS.do", FormQueryString(formObj));
...
// 					sheetObj.DoSearch("ESM_BKG_0361_04GS.do", FormQueryString(formObj));
 			...
 			case IBINSERT:      // 입력				
				...
				sheetObj.CellValue(newRow, "xpt_imp_seq") = formObj.exp_xpt_imp_seq.value;
				sheetObj.CellValue(newRow, "cnt_cd") = "IN";
			break;
 			...
 			case IBSAVE:        //저장
 				...
 				if(validateForm(sheetObj,formObj,sAction)) {
 					...
 					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="IN";
 					}else if(formObj.name=="form2"){
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "imp_"); 
 						sheetObj.CellValue(1,"cnt_cd")="IN";
 					}
 					if(sheetObj.CellValue(1,0)=='I'&&(sheetObj.CellValue(1,"ida_iec_no")==''&&sheetObj.CellValue(1,"ida_shp_bil_no")=='')){
 					...
 					if(sheetObj.isDataModified&&sheetObj.CellValue(1,0)!='D'){
 						if(formObj.name=="form"){
		 					if(formObj.exp_ida_iec_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n IEC "+formObj.exp_ida_iec_no.value;	
		 					}
	 					}else{
	 						if(formObj.imp_ida_iec_cpy_desc_flg.checked==true){
		 						window.opener.document.form.dg_cmdt_desc.value=window.opener.document.form.dg_cmdt_desc.value +
		 																"\n IEC "+formObj.imp_ida_iec_no.value;	
		 					}
	 					}
 					}
 					*/
 					...
 					if(document.form4.tabclosechk.value=="Y"){
 						if(sheetObj.isDataModified){							
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_04GS.do", FormQueryString(formObj) ,"");
 						}
 					}else{
 						saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_04GS.do", FormQueryString(formObj) ,"");
 					}
...
//            }
		...
		if(sAction=='2'&&sheetObj.CellValue(1,"ibflag")!='D'){	
	 		if(formObj.name=="form"){
	 			objs = document.all.item("tabLayer");
//		 		if(formObj.exp_ida_iec_no.value!=''&&formObj.exp_ida_iec_no.value.length<11){
//		 			if(formObj.exp_ida_shp_bil_no.value==''){
//			 			setTab1();
//			 			ComAlertFocus(formObj.exp_ida_iec_no, ComGetMsg("COM12174","11"));	  
//			 			return false;
//		 			}
//		 		}
	 			
	 			for(var i = sheetObj.HeaderRows; i < sheetObj.Rows; i++){
	 				if(sheetObj.CellValue(i,'ida_shp_bil_no')!='' && sheetObj.CellValue(i,'ida_shp_bil_iss_dt')==''){
	 					ComAlertFocus(sheetObj.CellValue(i,'ida_shp_bil_iss_dt'), ComGetMsg("BKG00445","S/Bill Date"));	  
			 			return false;
	 				} 
	 				if(sheetObj.CellValue(i,'ida_shp_bil_no')=='' && sheetObj.CellValue(i,'ida_shp_bil_iss_dt')!=''){
	 					ComAlertFocus(sheetObj.CellValue(i,'ida_shp_bil_no'), ComGetMsg("BKG00445","S/Bill No."));	  
			 			return false;
	 				} 
	 			}
	 			
//		 		if(formObj.exp_ida_shp_bil_no.value!=''&& formObj.exp_ida_shp_bil_iss_dt.value==''){
//		 			ComAlertFocus(formObj.exp_ida_shp_bil_iss_dt, ComGetMsg("BKG00445","S/Bill Date"));	  
//		 			return false;
//		 		}
//		 		if(formObj.exp_ida_shp_bil_no.value==''&& formObj.exp_ida_shp_bil_iss_dt.value!=''){
//		 			ComAlertFocus(formObj.exp_ida_shp_bil_no, ComGetMsg("BKG00445","S/Bill No."));	  
//		 			return false;
//		 		}
	 		}else if(formObj.name=="form2"){
	 			objs = document.all.item("tabLayer");
	 			if(formObj.imp_ida_iec_no.value!=''&&formObj.imp_ida_iec_no.value.length<11){
	 				setTab2();
	             	ComAlertFocus(formObj.imp_ida_iec_no, ComGetMsg("COM12174","11"));
	             	return false;
		 		}
	 		}	 		
		} 
 ...
 // 시트 데이터 초기화
    ...
    function initControl() {    	
    	...
    	axon_event.addListenerFormat("blur" , "form_onBlur" , document.form);
...
//		}
    ...
    function fnCntrComboItem(sheetObj, row, col, val) {
    	...
    	for ( var j = 1; j <= sheetObjects[2].RowCount; j++) {
    		if (sheetObjects[2].CellValue(j, "DelChk") == "0") {
    			cntr_name += sheetObjects[2].CellValue(j, "val") + "|";
    			cntr_val += sheetObjects[2].CellValue(j, "val") + "|";
    		}
    	}
    	...
    	var checkCntr = sheetObjects[2].FindText("DelChk", "0", 0, 2);
    	var i = row;
    	if (checkCntr > 0) {
			sheetObjects[0].CellComboItem(i, "cntr_no", " |" + cntr_name, " |" + cntr_val);
    	} else {
    		if (val != "") {
    			sheetObjects[0].CellComboItem(i, "cntr_no", " |" + sheetObjects[0].CellValue(i, "cntr_no"), " |" + sheetObjects[0].CellValue(i, "cntr_no"));
//    			sheetObjects[1].CellEditable(i, "cntr_tpsz_cd") = false;
//    			sheetObjects[1].CellEditable(i, "cntr_vol_qty") = false;
    		} else {
    			sheetObjects[0].CellComboItem(i, "cntr_no", " ", " ");
...
//    			sheetObjects[1].CellEditable(i, "cntr_vol_qty") = true;
    ...
    function sheet1_OnPopupClick(sheetObj, Row, Col) {
    	if ("ida_shp_bil_iss_dt"==sheetObj.ColSaveName(Col)) {
    		new ComCalendarGrid().select(sheetObj, Row, Col, 'yyyy-MM-dd');
    	}
    }
    
    function rowDelete() {
    	var drow = ComRowHideDelete(sheetObjects[0], "sel");
    	var rowCnt = 0;
    	var rCnt = 0;
    	for ( var i = 2; i <= sheetObjects[0].RowCount + 1; i++) {
    		if (sheetObjects[0].CellValue(i, "ibflag") != "D") {
    			rowCnt++;
    			rCnt++;
    			sheetObjects[0].CellValue2(i, "Seq") = rowCnt;
    		}
    	}
    	if (rCnt < 1) {

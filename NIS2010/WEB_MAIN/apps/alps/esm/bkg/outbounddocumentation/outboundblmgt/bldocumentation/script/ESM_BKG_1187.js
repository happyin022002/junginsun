...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_1187 : ESM_BKG_1187 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
function ESM_BKG_1187() {
...
// 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
    ...
    function loadPage() {
 	...
 	function initControl() {
 		...
 		axon_event.addListenerForm  ('beforedeactivate', 'bkg1187_obj_deactivate', formObject); //- 포커스 나갈때
 		axon_event.addListenerFormat('beforeactivate', 'bkg1187_activate', formObject); //- 포커스 들어갈때
            ...
            case 'sheet1':      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle1 = "|CHK|Seq.|Booking No.|ST|F/M|First VVD|ETB|Lane|POL|POL|BKG Date|BKG Staff|EDI Send Date|EDI Status|EDI Send ID|";
					...
					InitDataProperty(0, cnt++ , dtData,			30,		daCenter,	true,		"pol_yd_cd",		false,			"",		dfNone,			0,		false,		true);
					...
					InitDataProperty(0, cnt++ , dtData,			70,		daCenter,	true,		"result",			false,			"",		dfNone,			0,		false,		true);
  ...
  // Sheet관련 프로세스 처리
    function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		case IBSEARCH:      //조회
			...
			ComOpenWait(true);       			
        	var sXml = sheetObj.GetSearchXml("ESM_BKG_1187GS.do", FormQueryString(formObj));
        	ComOpenWait(false);       			
		...
		case "btn2_EDITransmit":
			...
			for (var idx = 0; idx < arrBkgNos.length; idx++) {
				sXml = sheetObj.GetSaveXml("ESM_BKG_1187GS.do", params + "&sheet1_ibflag=U&sheet1_bkg_no="+ arrBkgNos[idx]);

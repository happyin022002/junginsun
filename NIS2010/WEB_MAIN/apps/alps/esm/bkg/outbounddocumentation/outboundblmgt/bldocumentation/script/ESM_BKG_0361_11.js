...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0361_11 : esm_bkg_0361_11 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0361_11() {
 		...
 		switch(sheetNo) {
 			case 1:      //sheet1 init
 				with (sheetObj) {
 					...
 					var HeadTitle = "|Seq|bkg_no|io_bnd_cd|xpt_imp_seq|cnt_cd|tr_shpr_tax_id|tr_cnee_tax_id|tr_ntfy_tax_id";
 					...
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"tr_shpr_tax_id",	false,	"",			dfNone,		0,			false,		false);
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"tr_cnee_tax_id",	false,	"",			dfNone,		0,			false,		false);
 					InitDataProperty(0,	cnt++,	dtData,		80,		daCenter,	false,	"tr_ntfy_tax_id",	false,	"",			dfNone,		0,			false,		false); 	
 				}
 			break;
 			case 2:      //sheet1 init
 				with (sheetObj) {
 		...
 		switch(sAction) {
 			...
 			case IBSEARCH:      //조회
 				if(validateForm(sheetObj, formObj, sAction)) {
 					...
 					sheetObj.DoSearch("ESM_BKG_0361_11GS.do", FormQueryString(formObj));
 				}
 			break;
 			...
 			case IBSAVE:        
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					if(formObj.name=="form"){
 						...
 						sheetObj.CellValue(1,"cnt_cd")="TR";
 					...
 					if(sheetObj.CellValue(1,0)=='I'&&sheetObj.CellValue(1,"tr_shpr_tax_id")==''
 						&&sheetObj.CellValue(1,"tr_cnee_tax_id")==''&&sheetObj.CellValue(1,"tr_ntfy_tax_id")==''){
 						sheetObj.CellValue(1,0)='D';
 					}
 					
 					if(sheetObj.CellValue(1,0)=='U'&&sheetObj.CellValue(1,"tr_shpr_tax_id")==''
 						&&sheetObj.CellValue(1,"tr_cnee_tax_id")==''&&sheetObj.CellValue(1,"tr_ntfy_tax_id")==''){
 						sheetObj.CellValue(1,0)='D';
 					}
 					if(formObj.name=="form2"){
	 					if(formObj.imp_tr_cnee_tax_id.value==""&&formObj.imp_tr_ntfy_tax_id.value==""){
	 						ComShowCodeMessage("BKG95116");
	 						break;
	 					}
 					}
 					...
 					if(document.form4.tabclosechk.value=="Y"){					
 						if(sheetObj.isDataModified){
 							saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_11GS.do", FormQueryString(formObj) ,"");	
 						}
 					}else{
 						saveMsgFlg = sheetObj.DoSave("ESM_BKG_0361_11GS.do", FormQueryString(formObj) ,"");
 					}

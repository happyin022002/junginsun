	...
	switch(sheetID) {
     	case "sheet1":      //sheet1 init
	        with (sheetObj) {
	            ...
	            var HeadTitle1 = "|Sel.|Seq.|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|SHPR|SHPR|SHPR|SHPR|SHPR|SHPR|SHPR|SHPR|CNEE|CNEE|CNEE|CNEE|CNEE|CNEE|CNEE|CNEE|NTFY|NTFY|NTFY|NTFY|NTFY|NTFY|NTFY|NTFY|TP|TR|DG Cargo|DG Cargo|DG Cargo|DG Cargo|RF|MEA|FRT|RCV|DE|CNTR|||Transmission Status|| ";
	            var HeadTitle2 = "|Sel.|Seq.|B/L No.|BKG No.|POL|POD|DEL|Package|Package|Weight|Weight|Seal|Seal|Seal|Nm|Ad|Cnt|St/P|EC|Fax|Eml|Phn|Nm|Ad|Cnt|St/P|UO|Fax|Eml|Phn|Nm|Ad|Cnt|St/P|UO|Fax|Eml|Phn|TP|TR|DG|NM|TEL|UN|RF|MEA|FRT|RCV|DE|CNTR|||Transmission Status|| ";
	            ...
	            InitDataProperty(0, cnt++ , dtData, 		25,		daCenter,	true,     "cntc_pson_nm",  		false,    "",  dfNone, 0,   true,	true);
	            InitDataProperty(0, cnt++ , dtData, 		25,		daCenter,	true,     "cntc_pson_telcm_no",  		false,    "",  dfNone, 0,   true,	true);
	            InitDataProperty(0, cnt++ , dtData, 		25,		daCenter,	true,     "imdg_un_no",  		false,    "",  dfNone, 0,   true,	true);

	            InitDataProperty(0, cnt++ , dtData, 		25,		daCenter,	true,     "rc_flg",  		false,    "",  dfNone, 0,   true,	true);
	            InitDataProperty(0, cnt++ , dtData, 		30,		daCenter,	true,     "mea_qty",  		false,    "",  dfNone, 0,   true,	true);
	            InitDataProperty(0, cnt++ , dtData, 		30,		daCenter,	true,     "frt_term_cd",  		false,    "",  dfNone, 0,   true,	true);
	            ...
	            InitDataProperty(0, cnt++ , dtData, 		110,	daLeft,		true,     "mf_snd_dt",  	false,    "",  dfUserFormat2, 0,   true,	true,	-1,	false,	false);

	            InitDataProperty(0, cnt++ , dtHidden,		100,	daLeft,		false,	  "down_csv",		false,	  "",  dfNone,	0,		false,		false);
				InitUserFormat2(0, "mf_snd_dt", "####-##-## ##:##:##", "-|:" );	
				
				CountPosition = 0;
				if(document.form.str_pgm_no.value != "ESM_BKG_1046-1") {
            ...
            sheetObj.SetMergeCell(0, 53, 2, 2); 
	        break;
...
//					formObj.submit();
					...
					ComOpenWait(false);
					sheetObj.Down2Text("", "", "40", sFileName, "", "", false, false, true);
			   	}
	        }
	        else{
                sheetObj.Redraw = false;           
	    	...
	    	for( var i = 2; i <= sheetObject1.RowCount+1; i++ ) {
	    		...
	    		if ( sheetObject1.CellValue(i, "Chk") == "1" &&  sheetObject1.CellValue(i, "bkg_cgo_tp_cd") == "F"  ) // when the row checked
				{
					...
					if(document.form.str_pgm_no.value != "ESM_BKG_1046-1") {
						...
						//Package ~ SHPR.Ad 중 'N'인 항목 있으면 에러 메세지 
						for( var j=8; j<= 16; j++){
						...
						//CNEE.Nm ~ CNEE.Ad 중 'N'인 항목 있으면 에러 메세지
	 					for( var j=23; j<= 24; j++){
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						//NTFY.Nm ~ NTFY.Ad 중 'N'인 항목 있으면 에러 메세지
	 					for( var j=31; j<= 32; j++){
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						//DG Cargo.NM ~ DG Cargo.UN 중 'N'인 항목 있으면 에러 메세지
						for( var j=42; j<= 44; j++){	
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						// when the column value is N
						//Package ~ SHPR.EC 중 'N'인 항목 있으면 에러 메세지 
						for( var j=8; j<= 19; j++){
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						//CNEE.Nm ~ CNEE.UO 중 'N'인 항목 있으면 에러 메세지
	 					for( var j=23; j<= 27; j++){
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						//NTFY.Nm ~ NTFY.UOB 중 'N'인 항목 있으면 에러 메세지
	 					for( var j=31; j<= 35; j++){
							if(sheetObject1.CellValue(i,j)== "N") {
								ComShowCodeMessage("BKG06155", sheetObject1.CellValue(i, "bl_no"));
								return false;
						...
						// Shipper 의 Fax, Eml, Phn 중 하나라도 입력이 안되면 에러 메세지
						if(sheetObject1.CellValue(i,20)== "N" && sheetObject1.CellValue(i,21)== "N" && sheetObject1.CellValue(i,22)== "N") {
							ComShowCodeMessage("BKG06155",   sheetObject1.CellValue(i, "bl_no"));
							return false;
						}
						
						// Consignee 의 Fax, Eml, Phn 중 하나라도 입력이 안되면 에러 메세지
						if(sheetObject1.CellValue(i,28)== "N" && sheetObject1.CellValue(i,29)== "N" && sheetObject1.CellValue(i,30)== "N") {
							ComShowCodeMessage("BKG06155",   sheetObject1.CellValue(i, "bl_no"));
							return false;
						}
						
						// Notify 의 Fax, Eml, Phn 중 하나라도 입력이 안되면 에러 메세지
						if(sheetObject1.CellValue(i,36)== "N" && sheetObject1.CellValue(i,37)== "N" && sheetObject1.CellValue(i,38)== "N") {
							ComShowCodeMessage("BKG06155",   sheetObject1.CellValue(i, "bl_no"));
							return false;
							}
						}// end if

 ...
 *@FileName : ESM_BKG_N002.js
 *@FileTitle : Canada Export: Manifest Transmit
 *Open Issues :
 *Change history :
 *@LastModifyDate : 
 *@LastModifier : 
	...
	try {
		...
		case "btn_addbl":
			if (validateForm(sheetObjects[1], formObject, COMMAND01)) {
				...
				ComOpenWindowCenter("/hanjin/ESM_BKG_0029.do?" + pgmNo + "&type=add&vvd=" + vvd + "&pod=" + pod + "&eta=" + eta, "0029", 1020, 660);
			}
			break;
		case "btn_editbl":
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "Seq.|VVD|POL|POD|ETD|FROB|Customs|SENT TIME|A6A|CNTR COUNT|B/L COUNT|etl_dt|bdr_flg|Terminal";
			...
			InitDataProperty(0, cnt++, dtData,   150, daCenter, false, "etd_dt", false, "", dfNone, 0, false, true);
			InitDataProperty(0, cnt++, dtHidden, 60,  daCenter, false, "frob_flg", false, "", dfNone, 0, false, true);
	...
	case "sheet2": //sheet2 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtHidden,40, daCenterTop, true, "cstms_file_tp_cd", false, "", dfNone, 0, true, true);
			...
			InitDataProperty(0, cnt++, dtHidden, 120, daCenterTop, true, "rail_crr_ref_no", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtHidden, 100, daCenterTop, true, "usa_ib_trsp_no", false, "", dfNone, 0, true, true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case SEARCH:
		...
		sheetObj.DoSearch("ESM_BKG_N002GS.do", FormQueryString(formObj));
	...
	case MULTI: //Transmit
		...
		var sXml = sheetObj.GetSaveXml("ESM_BKG_N002GS.do", sParam);
 ...
 function doActionValidationResult(sheetObj, sKey) {
 	var sXml = sheetObj.GetSearchXml("ESM_BKG_N002GS.do?f_cmd=" + SEARCH03 + "&key=" + sKey);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case SEARCH:
		...
		if (ComIsEmpty(formObj.pol_cd)) {
			ComShowMessage("POL" + Msg_Required);
...
function sheet1_OnDblClick(sheetObj, row, col) {
	...
	sheetObjects[1].DoSearch("ESM_BKG_N002GS.do?pol_cd=" + pol_cd + "&pod_cd=" + pod_cd + "&frob_flg=" + frob_flg + "&cstms_port_cd="
	...
	if (sheetObj.CellValue(row, "bdr_flg") == "Y") {

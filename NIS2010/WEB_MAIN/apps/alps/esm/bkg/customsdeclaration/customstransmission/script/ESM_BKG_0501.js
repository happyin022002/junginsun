...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 500, daLeft, false, "edi_snd_log_ctnt", false, "", dfNone, 0, true, true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBDOWNEXCEL:	// EXCEL DOWNLOAD
		if (sheetObj.RowCount == 0) {
			...
			ComOpenWait(true);
			var xmlUrl = "http://"+location.hostname +":"+ location.port + "/hanjin/apps/alps/esm/bkg/customsdeclaration/customsreport/script/ESM_BKG_0501.xml";
...
function rdLoad() {
	...
	sXml += "<ETC>";
	sXml += "<MSG_TYPE>" + opener_sheet.CellValue(row, "trsm_msg_tp_id") + "</MSG_TYPE>"
	...
	sXml += "<OFFICE>" + opener_sheet.CellValue(row, "snd_usr_ofc_cd") + "</OFFICE>"
	sXml += "<USER>" + opener_sheet.CellValue(row, "snd_usr_id") + "</USER>"
	sXml += "<SEND_DATE>" + opener_sheet.CellValue(row, "snd_dt") + "</SEND_DATE>"
	sXml += "<SEND_TIME>" + opener_sheet.CellValue(row, "snd_hm") + "</SEND_TIME>"
	sXml += "</ETC>";
	sXml += "</SHEET>";
	...
	rdObjects[0].FileOpen(RD_path + 'apps/alps/esm/bkg/customsdeclaration/customsreport/report/ESM_BKG_0880.mrd', RDServer);

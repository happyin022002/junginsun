...
function processButtonClick() {
	...
	try {
        ...
        case "ai_gubun":
        	...
        	if ( formObject.ai_gubun[0].checked ) {
        		formObject.ai_type.disabled = false;
        		formObject.status_gubun.disabled = true;
        	...
        	} else if ( formObject.ai_gubun[1].checked ) {
        		formObject.ai_type.disabled = true;
        		formObject.status_gubun.disabled = false;
	    ...
	    switch(event.srcElement.name) {
			case "ai_gubun":
				if ( formObject.ai_gubun[0].checked ) {
					...
					document.getElementById('status_gubun').style.background="#CCCCCC";					
				} else {
					document.getElementById('ai_type').style.background="#CCCCCC";					
					document.getElementById('status_gubun').style.background="#FFFFFF";					
				} 
			break;				
	    }
 }
...
function obj_change() {
	var formObject = document.form;
	    switch(event.srcElement.name) {
			case "status_gubun":
				if (formObject.status_gubun.value == "A" ) { 
					ComBtnEnable("btn_DeleteAI");
				} else { // D
					ComBtnDisable("btn_DeleteAI");
				}
...
function loadPage() {
	...
	axon_event.addListener("change","obj_change", "status_gubun");
	...
	if ( formObject.pgmNo.value == "ESM_BKG_0028_1" || formObject.pgmNo.value == "ESM_BKG_0028_3") {
		formObject.ai_gubun[1].disabled = true;
	}
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = " ||Seq.|AI Type|Manifest File No.|M/H|B.Filer|Master B/L No.|ERR|B.STS|T.VVD|T.BDR|C/A No.|B.POL|B.POD|VSL EDI|B/L EDI|B/L EDI|B/L EDI|C.Filer|C.STS|Action Code||bkg_no|full_mty_cd|cstms_trsm_sts_cd";
			var HeadTitle2 = " ||Seq.|AI Type|Manifest File No.|M/H|B.Filer|Master B/L No.|ERR|B.STS|T.VVD|T.BDR|C/A No.|B.POL|B.POD|VSL EDI|Sent|VVD|Sent Time|C.Filer|C.STS|Action Code|";
...
function doActionIBSheet(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	case SEARCH: //Retrieve
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			sheetObj.DoSearch("ESM_BKG_0028GS.do", FormQueryString(formObj));
			end_no = 0;
			ComOpenWait(false);
		}
		break;
	...
	case MULTI: //Save
		if (validateForm(sheetObj, formObj, sAction)) {
			ComOpenWait(true);
			var sParam = sheetObj.GetSaveString() + "&f_cmd=" + MULTI + "&cnt_cd=" + formObj.cnt_cd.value;
			var sXml = sheetObjects[1].GetSaveXml("ESM_BKG_0028GS.do", sParam);
			// 에러가 없으면 다음 Row AI
			if (ComBkgErrMessage(sheetObjects[1], sXml)) {
				sheetObjects[1].LoadSaveXml(sXml);
				if (sheetObj.CheckedRows("chk") > 0 && sheetObjects[1].RowCount > 0) {
					for ( var i = 2; i < sheetObj.RowCount + 2; i++) {
						//AI 성공한 BL은 체크박스를 푼다.
						if (sheetObj.CellValue(i, "bl_no") == sheetObjects[1].CellValue(1, "bl_no")) {
	...
	case IBSEARCHAPPEND: // 페이징 조회
		formObj.f_cmd.value = SEARCH;
		sheetObj.DoSearch("ESM_BKG_0028GS.do", CondParam, "page_no=" + PageNo + "&end_no=" + end_no, true);
		break;
			...
			if (sheetObj.CheckedRows("chk") > 0 ) {
				...
				for ( var i = 2; i < sheetObj.RowCount + 2; i++) {
					...
					if ( sheetObj.CellValue(i, "chk") == "1" ) {
						var sParam = sheetObj.GetSaveString() + "&" + FormQueryString(formObj) + "&bl_no=" + sheetObj.CellValue(i, "bl_no");
						var sXml = sheetObj.GetSaveXml("ESM_BKG_0028GS.do", sParam);
...
function sheet1_OnDblClick(sheetObj, row, col) {
	...
	if (document.form.cnt_cd.value == "US") {
		...
		ComOpenWindowCenter("/hanjin/ESM_BKG_0034.do" + params, "ESM_BKG_0034", 1020, 660);
	} else {

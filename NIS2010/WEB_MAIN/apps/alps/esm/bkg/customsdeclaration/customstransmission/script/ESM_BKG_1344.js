...
function checkBoundCd(formObj) {
	if (formObj.io_bnd_cd[0].checked) {
		...
		formObj.tml_cd.disabled= false;
		formObj.tml_cd.className="input";
		...
		if(comboObjects[0].GetCount() == 3){
			comboObjects[0].DeleteItem("E");
			comboObjects[0].DeleteItem(" ");
		...
		formObj.tml_cd.disabled= true;
		formObj.tml_cd.className="input2";
    ...
    switch(sAction)
    {
		case IBSEARCH:      //조회
			...
			if(validateForm(sheetObj, formObj, sAction))
			{
				...
				sheetObj.DoSearch("ESM_BKG_1344GS.do", FormQueryString(formObj));
				ComOpenWait(false);
		...
		case MODIFY:
			if(ComShowCodeConfirm('BKG95003', 'save')) {
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("ESM_BKG_1344GS.do", FormQueryString(formObj));
...
*/
		...
		case MULTI05:
			...
			if (doProcess) {
/*
				// Inbound 의 경우 Discharge 전송 (Trans 가 A 이거나 D 인 경우만) - 2013-10-28 : 세관24는 미전송
				if (formObj.io_bnd_cd[0].checked &&
						(formObj.trans_target.value=="A" || formObj.trans_target.value=="D") ) {
					formObj.f_cmd.value = MULTI04;
					ComOpenWait(true);
					sXml = sheetObj.GetSaveXml("ESM_BKG_1344GS.do", FormQueryString(formObj));
...
function doActionValidationResult(sheetObj, sKey) {
 	//ComShowMessage("1");
 	var sXml = sheetObj.GetSearchXml("ESM_BKG_1344GS.do?f_cmd=" + COMMAND01 + "&key=" + sKey);

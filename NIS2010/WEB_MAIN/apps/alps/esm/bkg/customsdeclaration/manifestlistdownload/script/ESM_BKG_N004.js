 ...
 *@FileName : ESM_BKG_N004.js
 *@FileTitle : Canada Export: B/L Inquiry
 *Open Issues :
 *Change history :
 *@LastModifyDate : 
 *@LastModifier : 
...
function processButtonClick() {
	...
	try {
		...
		case "btn_Container":
			if (validateForm(sheetObject, formObject, COMMAND01)) {
				ComOpenWindowCenter("/hanjin/ESM_BKG_N007.do?pgmNo=ESM_BKG_N007&bl_no=" + bl_no + "&cnt_cd=CA", "0037", 620, 400);
			}
			break;
		case "btn_CM":
			if (validateForm(sheetObject, formObject, COMMAND01)) {
				ComOpenWindowCenter("/hanjin/ESM_BKG_N008.do?pgmNo=ESM_BKG_N008&bl_no=" + bl_no + "&cnt_cd=CA", "0036", 850, 510);
			}
			break;
		case "btn_ViewMsg":
...
function doActionIBSheet(sheetObj, formObj, sAction, tabno) {
	...
	case INIT:
		...
		sXml = sheetObj.GetSearchXml("ESM_BKG_N004GS.do", FormQueryString(formObj));
	...
	case MODIFY: //Save
		...
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			sXml = sheetObjects[0].GetSaveXml("ESM_BKG_N004GS.do", sParam);
			// 저장성공메시지
	...
	case REMOVE: //Delete, Reactivate
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			sXml = sheetObj.GetSaveXml("ESM_BKG_N004GS.do", sParam);
			sheetObj.loadSaveXml(sXml);
...
function obj_KeyUp() {
	...
	if (srcName == "frm_del_cd" && ComChkLen(formObj.frm_del_cd, 5) == 2) {
		...
		var sXml = sheetObjects[0].GetSearchXml("ESM_BKG_N004GS.do", FormQueryString(formObj));

...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
			...
			var HeadTitle = "|VVD|POL|POD|ETA|FROB|Customs|SENT TIME|XI|CNTR COUNT|B/L COUNT|||||";
			...
			InitDataProperty(0, cnt++, dtHidden, 30, daCenter, false, prefix + "ibflag");
			...
			InitDataProperty(0, cnt++, dtData, 80, daCenter, false, prefix + "xi", false, "", dfNone, 0, true, true);
	...
	case 2: //sheet2 init
		with (sheetObj) {
			...
			// 데이터속성 [ROW, COL, DATATYPE, WIDTH, DATAALIGN, COLMERGE, SAVENAME, KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT,
			// UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
			InitDataProperty(0, cnt++, dtHidden, 30, daCenter, true, prefix + "ibflag");
...
function doActionIBSheet(sheetObj, formObj, sAction, row) {
	...
	case INIT:
		...
		if ("sheet1" == sheetObj.id) {
			...
			var sXml = sheetObj.GetSearchXml("ESM_BKG_0615GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
	...
	case SEARCH01: //조회
		...
		if ("sheet1" == sheetObj.id) {
			sheetObj.DoSearch("ESM_BKG_0615GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
...
//		} else {
			if (miVal == "Y") {
				...
				if (pgmNo == "ESM_BKG_0615" && eval(bdrVal) > 0) {
					if (host.indexOf("localhost") > -1 || host.indexOf("127.0.0.1") > -1) {
						ComBtnEnable("btn_Transmit");				// 개발
						ComBtnEnable("btn_Transmit_e");				// 개발
					}else{
...
//		}
		...
		if ("sheet1" == sheetObj.id)
			sheetObject2.DoSearch("ESM_BKG_0615GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet2_"));
	...
	case COMMAND01:
		...
		ComOpenWait(true);
		var sXml = sheetObj.GetSaveXml("ESM_BKG_0615GS.do", sParam);
...
function doActionValidationResult(sheetObj, sKey) {
	...
	var sXml = sheetObj.GetSearchXml("ESM_BKG_0615GS.do?f_cmd=" + SEARCH03 + "&key=" + sKey);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case SEARCH01:
		...
		if (formObj.pageNo.value == "ESM_BKG_0615") {

...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "|vsl_cd|skd_voy_no|skd_dir_cd|VPS_PORT_CD|Carrier Code|Arrival Date|CRN|Captain Name|Total WGT|TEU Full|FEU Full|OTH Full|TEU Empty|FEU Empty|OTH Empty|crr_cd_val|cnd_vsl_cd|del_flag|CREW|PASSENGER|ACTUAL ARRIVAL DATE";
			...
			InitDataProperty(0, cnt++, dtData, 0, daCenter, false, "act_arr_dt", false, "", dfNone, 0, false, false);
		}
		break;
	}
}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case MULTI: //Save
		...
		sheetObj.DoSave("ESM_BKG_0015GS.do", sParam, -1, false);
		ComOpenWait(false);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case MULTI: //수정
		...
		if( formObj.frm_act_arr_da.value == "" ) {
			formObj.frm_act_arr_dt.value = "";
		}
		break;
	...
	case REMOVE:
		...
		if(sAction == ADD) { //Transmit A6
			if( formObj.frm_crw_knt.value == "" ) {
				ComShowCodeMessage('BKG00104', "CREW");
				return false;
			}
			if( formObj.frm_pasg_cnt.value == "" ) {
				ComShowCodeMessage('BKG00104', "Passenger");
				return false;
			}
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg) {
    with (sheetObj) {
    	var formObject = document.form;
    	var actdt = sheetObj.CellValue(1, "act_arr_dt");
    	formObject.frm_act_arr_da.value = actdt.substr(0,10); 
    	formObject.frm_act_arr_tm.value = actdt.substr(11,5);
    }
}
...
function processButtonClick() {
	...
	try {
		...
		case "btn_calendar":
			...
			cal.select(formObject.s_vps_etd_dt, formObject.e_vps_etd_dt, 'yyyy-MM-dd');
			break;
			
		case "btn_delete":
			var sUrl = "ESM_BKG_N011.do?";
			var sParam = "pgmNo=ESM_BKG_N011";
			if(sheetObject.SelectRow > 0){
				sParam = sParam +"&cvy_ref_no="+sheetObject.CellValue(sheetObject.SelectRow,"cvy_ref_no");
			}
			ComOpenWindowCenter(sUrl+sParam, "ESM_BKG_N011", 900, 500, false);		
			break;	
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "|Sel.|Lane|VVD|Operator|POL|ETD|CRN|vsl_cd|skd_voy_no|skd_dir_cd|is_crn_no|check_flag|upd_crn|pre_crr|max_cvy_ref_no";
			...
			InitDataProperty(0, cnt++, dtData, 200, daCenter, false, "vps_etd_dt", false, "", dfUserFormat2, 0, false, false);
			...
			InitDataProperty(0, cnt++, dtHidden, 100, daCenter, true, "max_cvy_ref_no", false, "", dfNone, 0, true, true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case INIT: //init
		formObj.s_vps_etd_dt.value = ComGetNowInfo('ymd', '-');
		formObj.e_vps_etd_dt.value = ComGetDateAdd(null, 'd', 14, '-');
	...
	case SEARCH: //Retrieve
		if (validateForm(sheetObj, formObj, sAction)) {
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("ESM_BKG_N010GS.do", FormQueryString(formObj));
	...
	case MULTI: //Save
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			if (ComShowCodeConfirm("BKG00350")) {
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("ESM_BKG_N010GS.do", sParam);
				if (ComBkgErrMessage(sheetObj, sXml)) {
					formObj.f_cmd.value = SEARCH;
					sheetObj.DoSearch("ESM_BKG_N010GS.do", FormQueryString(formObj));
				} else {
		...
		if (ComShowCodeConfirm("BKG00390")) {
			...
			for ( var i = 1; i <= sheetObj.RowCount; i++) {
				if (sheetObj.CellValue(i, "check") == 1) {
					if(preVvd != sheetObj.CellValue(i, "vvd_cd")){
						vIndex++;
						preVvd = sheetObj.CellValue(i, "vvd_cd");
					}
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case SEARCH:
		...
		//널체크 및 포멧 길이체크
		if ((ComIsNull(formObj.vps_port_cd) || ComIsNull(formObj.s_vps_etd_dt) || ComIsNull(formObj.e_vps_etd_dt))
				&& (ComIsNull(formObj.vvd_cd)) && (ComIsNull(formObj.cvy_ref_no))) {
			ComShowCodeMessage('BKG00626', '(POL and ETD) or (VVD) or (CRN)');

...
/**
 * @extends 
 * @class B/L Inquiry: C/M Information 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_BKG_N008() {
 ...
 function doActionIBSheet(sheetObj,formObj,sAction) {
     ...
     switch(sAction) {
		case IBSEARCH:      //조회
	        ...
	        var sXml = sheetObj.GetSearchXml("ESM_BKG_N008GS.do", FormQueryString(formObj));
		...
		case IBSAVE:        //저장
			...
			for(var i = sheetObj.HeaderRows; i < sheetObj.Rows; i++){
		 	...
		 	if(ComShowCodeConfirm('BKG95003', 'save')){   // Do you want to ...?
				...
				sheetObj.DoSave("ESM_BKG_N008GS.do", FormQueryString(formObj), -1, false);
	    		ComOpenWait(false);
		 	}
			break;

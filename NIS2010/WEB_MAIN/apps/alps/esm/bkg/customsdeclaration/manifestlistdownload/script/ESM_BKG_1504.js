...
//		try {
			...
			switch (srcName) {
				...
				case "btn_bl_inquiry":
					...
					var blNo = shtObj.CellValue(chkRowArr[0], "bl_no");
					for (var i=1; i<chkRowArr.length-1; i++) {
						if (blNo != shtObj.CellValue(chkRowArr[i], "bl_no")) {
							ComShowCodeMessage("BKG01134");
							return;
						}
					}
					ComOpenPopup("ESM_BKG_1502.do?pop_mode=Y&bl_no=" + blNo, 1002, 585, "", "1,0", true);
					break;
			} // end switch
		...
		with (shtObj) {
			switch (shtObj.id) {
				...
				case "sheet1":
					...
					InitDataProperty(0, cnt++, dtData,         80,    daCenter,    true,    "jp_snd_log_id", false,    "",      dfNone,    0,     false,    false);
					InitDataProperty(0, cnt++, dtData,         80,    daCenter,    true,    "ack_sts",       false,    "",      dfNone,    0,     false,    false);

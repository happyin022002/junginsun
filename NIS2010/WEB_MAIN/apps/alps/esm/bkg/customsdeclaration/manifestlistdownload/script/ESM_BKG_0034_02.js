 ...
 ***************************************************************************************/
	...
	/**
	 * @extends
	 * @class ESM_BKG_0034_02 : ESM_BKG_0034_02 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	...
	/**
	 * Sheet 기본 설정 및 초기화 body 태그의 onLoad 이벤트핸들러 구현 화면을 브라우저에서 로딩한 후에 
	 * 선처리해야 하는 기능을 추가한다
	 */
	function loadPage() {
	...
	/**
	 * t2sheet1 시트 데이터 조회 후 처리
	 */
	function t2sheet1_OnSearchEnd(sheetObj, ErrMsg)  {
		if (sheetObj.TotalRows == 0) {
			sheetObj.RemoveAll();
			var row = sheetObj.DataInsert();
			sheetObj.CellValue(row,"bkg_cust_tp_cd") = "A";
			row = sheetObj.DataInsert();
			sheetObj.CellValue(row,"bkg_cust_tp_cd") = "F";
		}
	}
	...
	function t2sheet2_OnSearchEnd(sheetObj2, ErrMsg)  {
		...
		var custTp = formObj.cust_tp.value;	
		 if (ErrMsg == "") {
			if(custTp == "A"){
				formObj.antf_cust_nm.value = sheetObj2.EtcData("cust_nm");
				formObj.antf_cust_addr.value = sheetObj2.EtcData("cust_addr");
			}
			else if(custTp == "F"){
				formObj.fwdr_cust_nm.value = sheetObj2.EtcData("cust_nm");
				formObj.fwdr_cust_addr.value = sheetObj2.EtcData("cust_addr");
			}
		 }
		 else{
			if(custTp == "A"){
			...
			case IBSEARCH: // 조회
				if (validateForm(sheetObj,formObj,sAction)) {
					...
					if( sheetObj.RowCount == 2 ) {
						IBS_CopyRowToForm(sheetObj, formObj, 1, "antf_");
						IBS_CopyRowToForm(sheetObj, formObj, 2, "fwdr_");
					}else{
						formObj.reset();
					}
					//ComOpenWait(false);

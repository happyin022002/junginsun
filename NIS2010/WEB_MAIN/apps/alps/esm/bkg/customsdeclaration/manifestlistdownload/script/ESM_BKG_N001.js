...
function doActionIBSheet(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	case IBSEARCH: //조회
		...
		var sXml = sheetObj.GetSearchXml("ESM_BKG_N001GS.do", FormQueryString(formObj));
	...
	case IBSEARCHAPPEND: // 하단 상세 그리드 조회
		...
		var sXml = sheetObjects[1].GetSearchXml("ESM_BKG_N001GS.do?pol_cd=" + vPol + "&pod_cd=" + vPod + "&clpt_seq="+ vClptSeq, FormQueryString(formObj));
	...
	case IBSAVE: // Data Download
		...
		var sXml = sheetObj.GetSaveXml("ESM_BKG_N001GS.do", sParam);
	...
	case SEARCH03:
		...
		ComOpenWait(true, true);
		if(formObj.chkAllPol.checked){
			sheetObj.DoSearch("ESM_BKG_N001GS.do?clpt_seq="+ vClptSeq, FormQueryString(formObj));
			// 모든 pol에 해당하는 전체  Container 체크
			sheetObj.CheckAll2("del_chk") = 1;
		}
		else{
			sheetObj.DoSearch("ESM_BKG_N001GS.do?pol_cd=" + vPol + "&pod_cd=" + vPod + "&clpt_seq="+ vClptSeq, FormQueryString(formObj));
		}
		ComOpenWait(false);
	}
}
...
function doActionValidationResult(sheetObj, sKey) {
	var sXml = sheetObjects[2].GetSearchXml("ESM_BKG_N001GS.do?f_cmd=" + SEARCH03 + "&key=" + sKey);

...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			var HeadTitle1 = "| |bl_no|cstms_port_cd|type_cd|msg_func_id|vsl_cd|skd_voy_no|skd_dir_cd|snd_dt|mvmt_ref_no|rcv_mvmt_ref_no|vvd|vsl_eng_nm|lloyd_no|por_cd|bkg_pol_cd|bkg_pod_cd|del_cd|rcv_term_cd|de_term_cd|pck_qty|pck_tp_cd|act_wgt|wgt_ut_cd|meas_qty|meas_ut_cd|cstms_desc|pol_cd|cstms_yd_cd";
			...
			InitDataProperty(0, cnt++, dtData, 50, daLeft, true, "mvmt_ref_no",
					false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 50, daLeft, true, "rcv_mvmt_ref_no",
					false, "", dfNone, 0, false, false);
...
var arn_yn ="";
var eu_1st_port_clpt_seq =""; // Add. 2015.11
...
function doActionIBSheet(sheetObj, formObj, sAction, Row, Col) {
	...
	case IBSEARCH: // Retrieve
		...
		var sXml = sheetObj.GetSearchXml("ESM_BKG_1107GS.do",
		...
		trsm_val = ComGetEtcData(arrXml[0],"trsm_val");
		eu_1st_port_clpt_seq = ComGetEtcData(arrXml[0],"eu_1st_port_clpt_seq"); // Add. 2015.11
		formObj.eu_1st_port_clpt_seq.value = eu_1st_port_clpt_seq;
		...
		if (State == "S") {
			...
			if( form.cstms_yd_cd.value.substring(0,2) == "GB" && formObj.mvmt_ref_no.value != "" && form.kts_send_dt.value < "2011012123") 				
	...
	case SEARCH01: // 조회
		...
		var sXml = sheetObj.GetSearchXml("ESM_BKG_1107GS.do", params);
	...
	case IBRESET: // New
		...
		b_MrnReactivate.style.display='none';
	...
	case IBSAVE: // Save
		...
		// "t3_"));
		var sXml = sheetObj.GetSaveXml("ESM_BKG_1107GS.do", params);
		var State = ComGetEtcData(sXml, ComWebKey.Trans_Result_Key);
		if (State == "S") {
			sheetObj.Redraw = false;
	...
	case MULTI01: // Transmit Manifest
		...
		if (trsm_val == "Y") {
			if(form.prtPgmNo.value != 'ESM_BKG_1108_2'){
    			ComShowCodeMessage('BKG06151');
    			return false;    
			}
		}
		
		if (form.ata_chk_flg.value != "N" && location.href.indexOf("alpsdev") < 0 && ata_yn == "Y") {
			ComShowCodeMessage('BKG01140');
			return false;    
		}
		
		if (dr_yn != "N") {
			ComShowCodeMessage("BKG01138", formObj.vvd.value, "");
			return;
		}
		if (arn_yn == "Y") {
			ComShowCodeMessage('BKG01145',formObj.vvd.value);
			return;    
		}
		
		// POFE 가 NL 이고 전송은 했으나 아직 ACK가 안 온 상태에서 전송 불가하게 수정
		// POFE 가 NL 이고 전송은 했으나 SYSTEM ACK 만 수신된 상태에서 전송 불가
	    var pofeCntCd = ComGetObjValue(formObj.cstms_port_cd).substring(0,2);
	    var msgFuncId = formObj.msg_func_id.value;
	    if ( msgFuncId == "Not Received" || msgFuncId.substring(msgFuncId.length, msgFuncId.length-3) == "(S)"){
			if(pofeCntCd == "NL") {
				ComShowMessage(ComGetMsg("BKG06144",formObj.bl_no.value)); 
				return false;
			} else {
		    	if(!ComShowConfirm(ComGetMsg("BKG01141",formObj.bl_no.value)) ) { 
		    		return false;
		    	}
			}
		}
...
function SetButtonStatus() {
	if (form.bl_no.value == "" || form.cstms_yd_cd.value == "" || form.cstms_yd_cd.value == 'undefined') {
		...
		if (ens_edi_svc_flg == "Y" && form.trsm_blck_flg.value == 'N')
	...
	if(form.strPgmNo.value == 'ESM_BKG_1107_P' && form.prtPgmNo.value == 'ESM_BKG_1108_2'){
		if(form.mvmt_ref_no.value != ""){
			b_MrnDelete.style.display='';
			b_MrnReactivate.style.display='none';
		}else if (form.mvmt_ref_no.value == ""){
			if(form.rcv_mvmt_ref_no.value != ""){
				b_MrnDelete.style.display='none';
				b_MrnReactivate.style.display='';
			}else{
				b_MrnDelete.style.display='none';
				b_MrnReactivate.style.display='none';
			}

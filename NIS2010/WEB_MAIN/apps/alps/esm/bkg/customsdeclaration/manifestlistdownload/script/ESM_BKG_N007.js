...
function doActionIBSheet(sheetObj,formObj,sAction,Row,Col) {
	...
	switch(sAction) {
		case IBSEARCH:      //조회
			...
			sheetObj.DoSearch("ESM_BKG_N007GS.do", FormQueryString(formObj));
    		ComOpenWait(false);
			break;
			...
			if(!validateForm(sheetObj,formObj,sAction)) return false;
		 	if(ComShowCodeConfirm('BKG95003', 'save')){   // Do you want to ...?
		        ...
		        if(sheetObj.GetSaveString() != ""){
			        ...
			        sheet1Xml = sheetObj.GetSaveXml("ESM_BKG_N007GS.do", params);
			        ...
			        if (state == "S") {
				        ...
				        if(sheetObj2.GetSaveString() != ""){
				        	...
				        	sheetObj2.DoSave("ESM_BKG_N007GS.do", FormQueryString(formObj), -1, false);
				        }
		...
		case SEARCH01: // 그리드에서 Container No. 입력시 조회
			...
			var params = FormQueryString(formObj)+"&"+sheetObj.GetSaveString();
			var sXml = sheetObj.GetSearchXml("ESM_BKG_N007GS.do", params);

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_1115 : ESM_BKG_1115 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
...
/*********************** KEY EVENT START ********************/ 	 
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
			...
			case IBSEARCH : // 조회
				...
				var sXml =  sheetObj.GetSearchXml("ESM_BKG_1115GS.do", FormQueryString(formObj));
    ...
    function sheet1_OnSearchEnd(sheetObj, ErrMsg) {
        with (sheetObj) {
        	...
        	if(FindText('error_code',form.p_error_code.value) < 0){
        		form.err_desc.value = "";
        	} else {
        		var sParam = "";
        		for(var i=1; i<= RowCount; i++) {
        			var temp = CellValue(i, "error_code");
        			if(temp == form.err_code.value){
        				if( sParam != "" ){
        					sParam = sParam + "\n" + (CellValue(i, "remark") != "" ? CellValue(i, "remark") : CellValue(i, "error_description"));
        				} else{
        					sParam = (CellValue(i, "remark") != "" ? CellValue(i, "remark") : CellValue(i, "error_description"));
        				}
        			}
        		}
        		form.err_desc.value = sParam;
        	}
        }
    }	
		...
		switch(sheetID) {
			...
			case "sheet1":
					with (sheetObj) {
						...
						InitDataProperty(0, cnt++ , dtHidden,   60,     daCenter,  true,  "cnt_cd",            false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,     60,     daCenter,  true,  "error_code",        false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,     300,    daLeft,    true,  "error_description", false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,     400,    daLeft,    true,  "remark",            false,  "",  dfNone,  0,  false,  false);
						CountPosition = 0;
						ShowButtonImage = 0;
						WordWrap = true;						

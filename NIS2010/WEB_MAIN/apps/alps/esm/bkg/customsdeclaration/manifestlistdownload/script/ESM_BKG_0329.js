	...
	try 
	{
		...
		switch(srcName) 
		{
			...
			case "btn_Transmission":
				...
				f.name = "transmitForm";
				f.method = "POST";
				f.action = "/hanjin/ESM_BKG_0344.do?pgmNo=ESM_BKG_0344";
...
//					if(formObj.sel_type.value=="A"){
//						addHiddenField(f, "in_type", 	" ");
//					}else{
...
//					}
				...
				addHiddenField(f, "in_tml2", 	formObj.in_hn2.value);
			...
			case "btn_EmailS":
				var SelIndex = sheetObject.SaveNameCol("sheet1_sel");
				...
				{					
					...
					{
						sheetObjects[0].CellValue(i, SelIndex) = 0;
						
					}
				}
				
	            if (sheetObject.CheckedRows("sheet1_sel") > 300) {
...
//			if (!crossChk) {
				var HeadTitle1 = " |||||Sel.|Seq.|B/L No.|BKG No.|D/L|R/O|OVVD|MSN|Correction|TP|FE|Booking|Booking|Booking|Booking|Booking|Booking|Booking|Booking|SHPR|SHPR|CNEE|CNEE|NTFY|NTFY|CNTR|BAC|W/H|DESC|T/R|CM|BZ|S/R|E/L NO.|E/L NO.|S/C|Cust TP|Customer Name||I/B Manifest(T/S Unmatch CHK)|I/B Manifest(T/S Unmatch CHK)|I/B Manifest(T/S Unmatch CHK)|I/B Manifest(T/S Unmatch CHK)|I/B Manifest(T/S Unmatch CHK)|I/B Manifest(T/S Unmatch CHK)|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume|T/S Empty Volume||||||||";
				var HeadTitle2 = " |||||Sel.|Seq.|B/L No.|BKG No.|D/L|R/O|OVVD|MSN|Correction|TP|FE|POL|POD|Package|Package|Weight|Weight|Measure|Measure|N|A|N|A|N|A|CNTR|BAC|W/H|DESC|T/R|CM|BZ|S/R|E/L NO.|E/L NO.|S/C|Cust TP|BL_TP|Customer Name|Package|Package|Weight|Weight|Match|Pre VVD|||||||VVD|Bond Area|Disch. CY|ETA|Dwell||||||||";
//			} else {
...
//			}
			...
			InitDataProperty(0, cnt++ , dtHidden, 		30, daCenter,   true,   prefix + "err_chk");
...
//			}
			...
			InitDataProperty(0, cnt++ , dtData,			30,	daCenter,	true,	prefix + "si_flg",		false,	"",  dfNone,		0,	false,true);
			...
			InitDataProperty(0, cnt++ , dtData,			80,	daCenter,	true,	prefix + "cstms_crr_in_loc_wh_cd",		false,	"",  dfNone,		0,	false,true);
			...
			CountPosition = 0; 
			SetmergeCell(0, 38, 2, 2);  // EL N/O
			SetmergeCell(0, 42, 2, 2);  // Customer Name
	...
	with(sheetObj) 
	{
		...
		//dtHiddenStatus
			...
			InitDataProperty(0, cnt++ , dtHiddenStatus,	30,	daCenter,	true,	prefix + "ibflag");
			InitDataProperty(0, cnt++ , dtData, 		30, daCenter,   true,   prefix + "err_chk");
			InitDataProperty(0, cnt++ , dtHidden, 		30, daCenter,   true,   prefix + "hidden3");
			InitDataProperty(0, cnt++ , dtHidden, 		30, daCenter,   true,   prefix + "cntr_cnt");
			InitDataProperty(0, cnt++ , dtHidden,		100,daCenter,	true,	prefix + "a_bkg_no",	false,	"",  dfNone,		0,	true,true);
			InitDataProperty(0, cnt++ , dtCheckBox,		40,	daCenter,	true,	prefix + "sel",			false,	"",  dfNone,		0,	true,true,  0, false, false);
...
function doActionIBSheet(sheetObj,formObj,sAction) 
{
	...
	switch(sAction) 
	{
		case INIT:      //Default
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("ESM_BKG_0329GS.do", FormQueryString(formObj));
		...
		if (!crossChk) {
			...
			if(funcSelectValidate(formObj) == true)
			{
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("ESM_BKG_0329GS.do", FormQueryString(formObj) + "&" + ret);
				var arrXml = sXml.split("|$$|");
...
//				initSheet(sheetObj, 0); 
			...
			if(funcSelectValidate(formObj) == true)
			{
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("ESM_BKG_0329GS.do", FormQueryString(formObj) + "&" + ret1);
				var arrXml = sXml.split("|$$|");
...
//				formObj.cntr_total.value = ComGetEtcData(sXml, "cntr_total");
		...
		case IBINSERT:      // B/L List DownLoad
			if(funcSelectValidate(formObj) == true)
			{
			    ...
			    var sXml = sheetObjects[0].GetSaveXml("ESM_BKG_0329GS.do", sParam);
		...
		case IBDELETE:
			if (ComShowCodeConfirm('BKG00535')) {
				...
				sheetObj.WaitImageVisible=false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("ESM_BKG_0329GS.do", sParam + "&"+FormQueryString(formObj) + "&" + ret);
...
//		 	    }
		...
		break;
		case COMMAND03: // ELNO email 전송
						
			if(funcSelectValidate(formObj) == true)
			{

	            //처리 단위 300
	            if (sheetObjects[0].CheckedRows("sheet1_sel") > 300) {
	            	ComShowCodeMessage("BKG08325","300");
	           	 	return false;
	            } 
				...
				var sXml = sheetObj.GetSaveXml("ESM_BKG_0329GS.do", sParam + "&"+FormQueryString(formObj) + "&" + ret);
				//sheetObj.LoadSaveXml(sXml);
				var sendCount = ComGetEtcData(sXml, "sendcount");		
				
			    ComShowMessage(ComResultMessage(sXml) + " Count=" + sendCount);
								
				ComOpenWait(false);
				
			}
			break;
		case COMMAND04: // Console email 전송
...
function doActionValidationResult(sheetObj, sKey) {
	var formObj = document.form;
	var sXml = sheetObj.GetSearchXml("ESM_BKG_0329GS.do?f_cmd=" + SEARCH03
			+ "&key=" + sKey);
...
function funcOnFocus(objName)
{
	...
	if(objName == 'pol')
	{
		...
		form.in_hn2.disabled=false;
		form.in_hn2.className="input";
		...
		document.all.si_flg1.style.display="inline";
		document.all.si_flg2.style.display="inline";
	...
	else if(objName == 'pod')
	{
		...
		form.in_hn2.disabled=true;
		form.in_hn2.className="input2";
		document.all.el_no1.style.display="none";
		document.all.el_no2.style.display="none";
		document.all.si_flg1.style.display="none";
		document.all.si_flg2.style.display="none";
...
function funcSetBoundOption(formObj, op)
{
	...
	else if(op == 'in')
	{
		...
		var optionList = [" ","A : Local","R : T/S", "M : eMpty Local","T : eMpty T/S"];
		var valueList  = [" ","A","R","M","T"];
		for(var i=0; i<optionList.length; i++){
			in_select.options[i] = new Option(optionList[i], valueList[i]);
			if(i==0){
...
function funcSelectValidate(formObj)
{
	...
	else if(formObj.in_bound.value == 'O')
	{
		...
		var in_hn2 = formObj.in_hn2.value;
		...
		formObj.in_pod_tmnl.value = in_pol + in_hn2;
...
function funcShowValueRows()
{
	...
	if ( formObj.rad_crscheck.checked ){
		...
		var ColEr = sheetObj.SaveNameCol("sheet1_err_chk");
		var ColBl = sheetObj.SaveNameCol("sheet1_sc");
		var ColEl = sheetObj.SaveNameCol("sheet1_elno_a");
		var ColSi = sheetObj.SaveNameCol("sheet1_si_flg");
		...
		if(ViewOptionErrCheck == '' && ViewOptionBlType == '' && ViewOptionElNo == '' && ViewOptionSiFlg == '' && (ViewOptionCorrection == '' || ViewOptionCorrection == '01') && ViewOptionCargoType == '' )
		{
			for(var i=2;i < sheetObj.RowCount + 2; i++)
			{
				sheetObj.RowHidden(i) = false;
		...
		else
		{
			for(var i=2;i <= sheetObj.RowCount + 2; i++)
			{
				...
				if(ViewOptionSiFlg != '')
				{
					if(ViewOptionSiFlg != sheetObj.CellValue(i, ColSi))
					{
						sheetObj.RowHidden(i) = true;
						continue;
					}
...
{		
	...
	for(var i=2;i < sheetObj.RowCount + 2;i++)
	{
		if(sheetObj.CellValue(i, prefix+"err_chk") == "E") sheetObj.CellFontColor(i, prefix + "bl_no" ) = sheetObj.RgbColor(255, 0, 0); 
 ...
 * @return
 */
function funcSiFlgOnChange(obj)
{
	ViewOptionSiFlg = obj.value;
	funcShowValueRows();
}

 ...
 * 1.3 2011.07.20 민정호 [CHM-201112024] Split 01-China 24hr Manifest 관련 화면보완 및 추가
 * 1.4 2011.08.05 민정호 [CHM-201111822] Split 05-R4J Rule Upgrade 관련 소스품질 향상을 위한 조치 건
=========================================================*/
/****************************************************************************************
...
var sheetCnt = 0;
var dataDownloadSucces = "";	//  Unable to move to Manifest Transmit Please complete the manifest download first
...
function processButtonClick(){
	...
	try {
		...
		switch(srcName) {
			...
			case "btn_manifest_transmit":
				if(dataDownloadSucces == "SUCCESS"){	
					var trans_type = '';
					for(i=0; i<3; i++){
						if(document.all.trans_type[i].checked  == true){
							trans_type = document.all.trans_type[i].value;
					    }
					}
				
					var sParam = "&vvd="+formObject.vvd.value+
								 "&pol_cd="+formObject.pol_cd.value+
								 "&pod_cd="+formObject.pod_cd.value+								 
								 "&bkg_cgo_tp_cd="+formObject.bkg_cgo_tp_cd.value+
					 			 "&trans_type="+trans_type;					
//					ComOpenWindowCenter("ESM_BKG_1046-1.do?MENU=Y&pgmNo=ESM_BKG_1046-1&callGubun=ESM_BKG_0216"+sParam, "ESM_BKG_1046", 1024, 660);
					location.href = "ESM_BKG_1046-1.do?MENU=Y&pgmNo=ESM_BKG_1046-1&callGubun=ESM_BKG_0216"+sParam;					
				}else{
					ComShowCodeMessage('BKG08193'); 
					return;
				}	
		break;			
		} // end switch
...
//					formObj.submit();
					...
					ComOpenWait(false);
					sheetObj.Down2Text("", "", "30", sFileName, "", "", false, false, true);		
			   	}
	        }
	        else{
                sheetObj.Redraw = false;           

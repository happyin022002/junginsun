...
/**
 * @extends
 * @class ESM_BKG-0478 : ESM_BKG-0478 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_bkg_0478() {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtHiddenStatus, 40, daCenter, true,
					"ibflag");
			InitDataProperty(0, cnt++, dtData, 80, daCenter, true, "sub_seq");
			InitDataProperty(0, cnt++, dtData, 60, daLeft, true, "edi_snd_msg",
...
function doActionIBSheet(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	case IBSEARCHAPPEND: // 페이징 조회
		...
		ComOpenWait(true);
		if (PageNo >= 1) {
			sheetObj.DoSearch("ESM_BKG_0478GS.do", CondParam,
					"iPage=" + PageNo, true);
		} else {
			sheetObj.DoSearch("ESM_BKG_0478GS.do", FormQueryString(formObj),
					"iPage=1", false);

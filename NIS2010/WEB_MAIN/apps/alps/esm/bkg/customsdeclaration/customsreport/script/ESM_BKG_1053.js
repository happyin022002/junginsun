...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_1053 : ESM_BKG_1053 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_1053() {
     ...
     function sheet1_OnSearchEnd(sheetObj, ErrMsg) {
         with (sheetObj) {
             ...
             for(var i= HeaderRows; i<= LastRow; i++) {
                ...
                tot_bl = (tot_bl*1) + (CellValue(i, "bl_cnt")*1);
                tot_acpt = (tot_acpt*1) + (CellValue(i, "acpt_cnt")*1); 
                tot_rjct = (tot_rjct*1) + (CellValue(i, "rjct_cnt")*1);
                tot_n_rcv = (tot_n_rcv*1) + (CellValue(i, "n_rcv_cnt")*1); 
                tot_wait = (tot_wait*1) + (CellValue(i, "wait_cnt")*1);
                
             }
             ColumnSort("1|2|3|4|5|6");
             
             formObj.div_total_bl_cnt.value = tot_bl;
             formObj.div_acc_bl_cnt.value = tot_acpt;
             formObj.div_rej_bl_cnt.value = tot_rjct;
             formObj.div_nrcv_bl_cnt.value = tot_n_rcv;
             formObj.div_donld_bl_cnt.value = tot_wait;
             formObj.div_unsent_bl_cnt.value = tot_bl - (tot_acpt+tot_rjct+tot_n_rcv+tot_wait);
         }//end width
     }	     
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
			...
			case IBSEARCH : // 조회
				...
				var sXml =  sheetObj.GetSearchXml("ESM_BKG_1053GS.do", FormQueryString(formObj));
		...
		switch(sheetID) {
			...
			case "sheet1":
					with (sheetObj) {
						...
						if(document.form.p_rhq_gb[0].checked){
							HeadTitle1 = "|VVD|LANE|RHQ|POL OFC|POL|POL ETB|B/L Count|Send Date|GAP|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|";
							HeadTitle2 = "|VVD|LANE|RHQ|POL OFC|POL|POL ETB|B/L Count|Send Date|GAP|Sent|Sent|Sent|Sent|Amend|";
							HeadTitle3 = "|VVD|LANE|RHQ|POL OFC|POL|POL ETB|B/L Count|Send Date|GAP|Count|Acpt|Rjct|N.rcvd|Amend|";
						} else if (document.form.p_rhq_gb[1].checked){
							HeadTitle1 = "|VVD|LANE|RHQ|BKG OFC|POL|POL ETB|B/L Count|Send Date|GAP|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|CCAM (B/L)|";
							HeadTitle2 = "|VVD|LANE|RHQ|BKG OFC|POL|POL ETB|B/L Count|Send Date|GAP|Sent|Sent|Sent|Sent|Amend|";
							HeadTitle3 = "|VVD|LANE|RHQ|BKG OFC|POL|POL ETB|B/L Count|Send Date|GAP|Count|Acpt|Rjct|N.rcvd|Amend|";
						}
						...
						InitDataProperty(0, cnt++ , dtData,  65,  daCenter,  true,  "bkg_ofc_cd",	false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "pol_cd",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  115, daCenter,  true,  "vps_etb_dt",	false,  "",  dfNone,  0,  false,  false);
						
						InitDataProperty(0, cnt++ , dtData,  70,  daCenter,  true,  "bl_cnt",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  115, daCenter,  true,  "snd_dt",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  50,  daRight,   true,  "gap",			false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "snd_cnt",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "acpt_cnt",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "rjct_cnt",		false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "n_rcv_cnt",	false,  "",  dfNone,  0,  false,  false);
						...
						InitDataProperty(0, cnt++ , dtData,  60,  daCenter,  true,  "amend_cnt",	false,  "",  dfNone,  0,  false,  false);
						InitDataProperty(0, cnt++ , dtHidden,  60,  daCenter,  true,  "wait_cnt",	false,  "",  dfNone,  0,  false,  false);

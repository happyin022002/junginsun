...
function processButtonClick(){
	...
	try {
		...
		case "btn_listprint":			
			...
			if (chkCnt > 0) {				
				var xmlUrl = "http://"+location.hostname +":"+ location.port + "/hanjin/apps/alps/esm/bkg/customsdeclaration/customsreport/script/ESM_BKG_0340.xml";
		...
		case "btn_formprint":
			...
			for(var i=1; i <= sheetObject1.RowCount; i++) {
				if (sheetObject1.CellValue(i, "Sel")==1) { 
					params = params + "&ibflag=R"+ 
					         ...
					         "&mrn_no="+formObject.mrn_no.value+
			...
			if (chkCnt > 0) {	
				...
				ComOpenWait(true);
				// Sheet2에 조회 XML 생성
				var sXml = sheetObject2.GetSearchXml("ESM_BKG_0340GS.do", params, "", true);
				sheetObject2.LoadSearchXml(sXml, false, -1, false);
				ComOpenWait(false);
				ComOpenWindowCenter("/hanjin/ESM_BKG_0828.do", "0828", 1024, 720, false);
			}else {
				ComShowCodeMessage('BKG00394');
				ComOpenWait(false);
			}
...
function initSheet(sheetObj,sheetNo) {
	...
	with (sheetObj) {
		...
		InitDataProperty(0, cnt++ , dtData,      	110,    daCenter,    false,     "cstms_dchg_loc_wh_cd", false,    "",      dfNone, 		0,     false,		false);
	...
	with (sheetObj) {
		...
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "mf_seq_no");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "bl_no");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "pck_qty");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "pck_tp_cd");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "act_wgt");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "cntr_no");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "cstms_dchg_loc_wh_cd");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "bonded_wh");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "bonded_tp");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "cust_nm");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "vsl_eng_nm");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "co_nm");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "mrn_no");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "mrn_chk_no");
		InitDataProperty(0, cnt++ , dtData,			45,     daCenter,    false,     "vps_etb_dt");
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	case IBSEARCH:      //조회
		...
		if(validateForm(sheetObj,formObj,sAction)) {		
				...
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("ESM_BKG_0340GS.do", FormQueryString(formObj));

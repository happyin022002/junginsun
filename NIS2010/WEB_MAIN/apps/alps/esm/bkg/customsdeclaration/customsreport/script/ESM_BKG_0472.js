...
/**
 * @extends
 * @class ESM_BKG-0472 : ESM_BKG-0472 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_bkg_0472() {
...
//	if(document.form.jp_msg_tp_cd.value != "JP24"){		
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 			80, daCenter, 	true,	"rcv_dtl_seq");
			InitDataProperty(0, cnt++, dtData, 			60, daLeft, 	true, 	"edi_rcv_msg",		false, "", dfNone, 0, false, false);
...
function doActionIBSheet(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	case IBSEARCHAPPEND: // 페이징 조회
		...
		ComOpenWait(true);
		if (PageNo >= 1) {
			sheetObj.DoSearch("ESM_BKG_0472GS.do", CondParam,
					"iPage=" + PageNo, true);
		} else {
			sheetObj.DoSearch("ESM_BKG_0472GS.do", FormQueryString(formObj),
					"iPage=1", false);

...
var URL_ESM_BKG = 'ESM_BKG_0207GS.do';
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 220, daLeft, false, prefix + "file_nm", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 60, daCenter, false, prefix + "file_size", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtData, 150, daCenter, false, prefix + "cargo_contain", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtImage, 5, daCenter, true, prefix + "multiPopup", false, "", dfNone, 0, true, true);
			...
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "cargo_cnt");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "dcgo_seq");
...
/*
		if (document.form.bkg_no.value == '') {
			...
			document.form.ridr_tp_cd.value = 'D';
		}
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg) {
	with (sheetObj) {
		...
		DataLinkMouse(prefix + "multiPopup") = true;
	}
...
function setMultiSelectCheck() {
	...
	try {
		...
		sheetObjects[0].CellValue2(current_Row, prefix + "dcgo_seq") = r_value;
		sheetObjects[0].CellValue2(current_Row, prefix + "cargo_contain") = r_text;
	...
	case IBSEARCH: //조회
		...
		if (sheetObj.RowCount > 0) {
			...
			for ( var row = 1; row <= sheetObj.LastRow; row++) {
				if ('B' != _val) {
					// 이미지 변경하기
					if (sheetObj.CellValue(row, prefix + "cargo_cnt") > 1) {
						sheetObj.CellImage(row, prefix + "multiPopup") = 0;
					} else {
						sheetObj.CellImage(row, prefix + "multiPopup") = 1;
					}
				}
				//size 변경하기 
	...
	with (formObj) {
		...
		if (sAction == IBSAVE) {
			...
			for ( var row = 1; row <= c_row; row++) {
				...
				/**  by 전성진 2009.11.11 삭제요망 
					if ('B' != _val) {
						var v_dcgo_seq = sheetObj.CellValue(row, prefix + "dcgo_seq");
						if (v_dcgo_seq == '') {
							ComShowMessage(row + "행의   [Container No./CGO Seq]은 필수값입니다. ");
							sheetObj.SelectCell(row, prefix + "multiPopup");
							return false;
						}
					}
				 */
...
function selectFile(sheetObj, Row, Col, _insert) {
	...
	if (Col == '' || Col == '2') {
		//파일명이 없거나, 신규생성Row이거나, 파일을 새로 선택했을 경우 파일선택창을 띄운다.
		var file_nm = sheetObj.CellText(Row, prefix + "file_nm");
		...
		if (fileName.indexOf("\\") != -1) {
			...
			sheetObj.CellValue2(Row, prefix + "ridr_tp_cd") = ComGetObjValue(formObj.ridr_tp_cd);
			if ('B' != ComGetObjValue(formObj.ridr_tp_cd)) {
				sheetObj.CellImage(Row, prefix + "multiPopup") = 1;
			}
		}

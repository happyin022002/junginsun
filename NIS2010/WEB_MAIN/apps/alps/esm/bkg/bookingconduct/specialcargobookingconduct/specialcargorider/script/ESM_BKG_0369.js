...
/**
 * @extends
 * @class esm_bkg_0369 : esm_bkg_0369 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_bkg_0369() {
...
var uploadCnt = 0;
var URL_ESM_BKG = 'ESM_BKG_0369GS.do';
var prefix = "sheet1_";
...
/*		if (document.form.bkg_no.value == '') {
		...
		if (opener &&
			...
			opener.document.form.isInquiry &&
			opener.document.form.isInquiry.value== "Y" &&
            opener.document.URL.indexOf("ESM_BKG_0079_06_Q") > 0) {
 ...
 * 임시
 */
function downloadLOIFormat(){
	hiddenFrame.location.href = "/hanjin/FileDownload?key=" + "myctcp_20160419170207415.docx"; 
}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: //조회
		...
		if (sheetObj.RowCount > 0) {
			...
			if (fn_filteredData(sheetObj, prefix + "ridr_tp_cd", check_value)) {
				//size 변경하기 

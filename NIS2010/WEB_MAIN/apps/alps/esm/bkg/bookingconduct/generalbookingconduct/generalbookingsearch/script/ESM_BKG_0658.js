...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   ...
   /**
     * @extends 
     * @class ui_bkg_0658 : ui_bkg_0658 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ui_bkg_0658() {
 ...
 // 공통전역변수
    ...
    function doActionIBSheet(sheetObj,formObj) {
		...
		var sXml = sheetObj.getSearchXml("ESM_BKG_0658GS.do" , FormQueryString(formObj));
		...
		if(ComGetEtcData(sXml,"TRANS_RESULT_KEY") == "S"){
			...
			if(calllFunc != ''){			
				 if(!opener){
					 eval('window.dialogArguments.'+calllFunc)(new Array(stopOffLocCd, stopOffCntcPhnNo, stopOffCntcPsonNm, stopOffDiffRmk));
				 }else{	 
					 try {
						 eval('opener.'+calllFunc)(new Array(stopOffLocCd, stopOffCntcPhnNo, stopOffCntcPsonNm, stopOffDiffRmk));
					 }catch(e) {
					 }	   
				 }     				
    ...
    function  obj_keyup(){
    	...
    	switch (event.srcElement.name) {
 			case "stop_off_diff_rmk":			
 				var frm = document.form.stop_off_diff_rmk.value.length;
 				if(frm > 1000){
 					ComShowCodeMessage("BKG01137", "1000");
 					document.form.stop_off_diff_rmk.value = document.form.stop_off_diff_rmk.value.substring(0, 1000);    		
 				}
 				break;
    	}	
    }
...
//			if(calllFunc != ''){			
//				eval('window.dialogArguments.'+calllFunc)(null);
//			}         			
			window.close();      		
    	}else{
...
//        	}else if(formObject.stop_off_diff_rmk.value.length > 45){
//        		ComShowCodeMessage("COM12173","Remark","45");
        	}else{

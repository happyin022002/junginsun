...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class esm_bkg_0701 : esm_bkg_0701 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esm_bkg_0701() {
...
//						break;
					...
					case "btn_Filtered_Bl":
			        	...
			        	try {
							...
							var sXml = sheetObjects[0].GetSearchXml("ESM_BKG_0701GS.do", FormQueryString(formObject));
        ...
        /**
         ...
         function getBackEndJobStatus() {
	   		try {
	         	...
	         	var sXml = sheetObjects[0].GetSearchXml("ESM_BKG_0701GS.do", FormQueryString(form));
        ...
        function loadPage() {
			...
			for(i=0;i<sheetObjects.length;i++){
     	    ...
     	    axon_event.addListenerForm('beforeactivate', 'obj_activate', document.form);
   	        ...
   	        var dt_type  = ComGetObjValue(form.dt_type[0]);
        ...
        function rct_rhq_cd_OnChange(comboObj, code, text) {
        	...
        	if(comboObj.Index == "0") {
     		...
     		if(comboObj.GetCount () > 0 && comboObj.Index != "-1") {
 				...
 				formObj.rct_ofc_cd.Code = formObj.strOfc_cd.value;
...
function initIBComboItem() {
	...
	getComboObject(comboObjects, 'bdr_status_cd').Code = 'N'; 
}
	   		...
	   		case "sheet1":      //sheet1 init:      //sheet1 init
	               with (sheetObj) {
	                   ...
	                   //데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
						...
						InitDataProperty(0, cnt++ , dtData,		    75, daCenter,	true,	"rdn_staus",		    false,	"",  dfNone,	0,	false    ,false ); //5. 추가
						...
						InitDataProperty(0, cnt++ , dtData,			100,daCenter,	true,	"rev_aud_stl_knd_nm",	false,	"",  dfNone,	0,	false    ,false );
 						...
 						InitDataProperty(0, cnt++ , dtCheckBox,		40, daCenter,	true,	"mnl_stl_rqst_flg",		false,	"",  dfNone,	0,	true    ,true,   -1, 	false,	true, "",	false );
						...
						InitDataProperty(0, cnt++ , dtData,			200,daLeft,		true,	"rgn_grp_avc_rmk",		false,	"",  dfNone,	0,	false    ,false );
						...
						InitDataProperty(0, cnt++ , dtHidden, 		90, daLeft,     false, 	"bkg_no",     			false,   "", dfNone,    0,  false    ,false );
		       			InitDataProperty(0, cnt++ , dtHidden, 		90, daLeft, 	false,  "ctrt_tp_cd", 			false,   "", dfNone, 	0,  false    ,false );
		       			InitDataProperty(0, cnt++ , dtHidden,		90, daLeft,		false,	"bkg_contract",			false,   "", dfNone, 	0,  false    ,false );
 						InitDataProperty(0, cnt++ , dtHidden,		50,	daLeft,	    false,	"rev_aud_sts_cd",		false,	 "", dfNone,	0,	false    ,false );
 						InitDataProperty(0, cnt++ , dtHidden,		200,daLeft,		true,	"umch_rsn_rmk_bk",		false,	"",  dfNone,	0,	false    ,false );
            ...
            switch(sAction) {
            	...
            	case IBSEARCH:      //조회
            		...
            		if (validateForm(sheetObj,formObj,sAction)) {
            			...
            			var sXml = sheet1.GetSearchXml("ESM_BKG_0701GS.do" , FormQueryString(formObj));
            			sheet1.LoadSearchXml(sXml);    // Grid1.
    			...
    			case IBSAVE:        //저장
	    				...
	    				if(sheet1.CellValue(i, "mnl_stl_rqst_flg") == "1"){		
	    					if(sheet1.CellValue(i, "mnl_stl_tp") == "" 
	    						|| sheet1.CellValue(i, "umch_rsn_rmk") == ""){
	    						ComShowCodeMessage("BKG95107");  //Please select Manual Settle Kind and update Remark.
	    						return false;
	    					}
	    				}
	   	 			}
					...
					var sXml = sheet1.GetSaveXml("ESM_BKG_0701GS.do", sParam);
    			...
    			for(var i=2; i<= sheet1.RowCount+2; i++){
    				...
    				if(sheet1.CellValue(i, "chk") == "1"){		
    					...
    					if(sheet1.CellValue(i, "bdr_flg") == "Yes"){
    			...
    			for(var i=2; i<= sheet1.RowCount+2; i++){
    				...
    				if(sheet1.CellValue(i, "chk") == "1"){		
    					if(sheet1.CellValue(i, "umch_rsn_rmk") == "" || sheet1.CellValue(i,"umch_rsn_rmk_bk") == ""){
    			...
    			case IBSEARCH_ASYNC02: // Re Audit
					...
					if (ComShowCodeConfirm("BKG95029")) { // "Do you want to execute Re-Audit?"
						...
						var sXml = sheet1.GetSaveXml("ESM_BKG_0701GS.do", params);
...
//        	setFontColor("white");
			...
			for(var i = startRow1; i < endRow1; i++) {
 	 			...
 	 			if(!ComIsEmpty(sheetObj.CellValue(i,"umch_al"))) unmatch_al++;
 	 			if(!ComIsEmpty(sheetObj.CellValue(i,"umch_all"))) unmatch_all++;
 	 			...
 	 			if(sheetObj.CellValue(i, "rev_aud_sts_cd") != "U") {
 	 				...
 	 				sheetObj.CellEditable(i, "mnl_stl_rqst_flg") = false;
 	 				sheetObj.CellEditable(i, "mnl_stl_tp") = false;
 	 				sheetObj.CellBackColor(i, "mnl_stl_rqst_flg") = sheetObj.RgbColor(239,240,243);
 	 			}
			...
			unmatched_case_count = unmatch_al + unmatch_all + unmatch_b + unmatch_c + unmatch_d + unmatch_e + unmatch_f + unmatch_g;
			...
			/* umch_rsn_rmk_bk 에 Apply 전 값을 위해 셋팅 */
 	 		for(var i=2; i<= sheetObj.RowCount+2; i++){
 	 			sheetObj.CellValue2(i,"umch_rsn_rmk_bk") = sheetObj.CellValue(i,"umch_rsn_rmk");
...
//					break;
    	...
    	function sheet1_OnSearchEnd(sheetObj, ErrMsg)
    	{
    		with(sheetObj)
    		{
    			var blank1 = "              ";
    			var blank2 = "  ";
    			CellComboItem(2, "bkg_contract", "VN001700" + blank1 + "VN001701|OFT:D4:USD1000" + blank2 + "OFT:D4:USD1200", "1");
    			CellComboItem(4, "bkg_contract", "DTH:D4:AUD20" + blank2 + "DTH:D4:AUD120", "1");
    	...
    	*/
 	 	...
 	 	/** 
         * settle 시 체크된 status를 U로 세팅한다. <br>
         * <br><b>Example :</b>
         * <pre>
 ...
 function sheet1_OnMouseDown(sheetObj, Button, Shift, X, Y) {
	 ...
	 */
  	 	function removeAll(formObj) {
  	 		...
  	 		comboObjects[0].Code = gRctRhqCd; // 선택변경 없을시 이벤트 발생위해 comboObjects[1] 생성위해 한번더...    	 		
  	 		/***************************************/

...
* --------------------------------------------------------
* history
* 2012.02.16 김보배 [CHM-201215888] CA Summary 화면에 Contract Sales Rep. 추가
* 2012.03.26 변종건 [CHM-201216730] Split 07-RD 리포트 성능을 위한 개선 요청(PreparedStatement)
* 2013.02.13 류대영 [CHM-201323012] C/A Summary 화면 RHQ level 칼럼 추가 및 exemption 요청
=========================================================*/
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_0174 : ESM_BKG_0174 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0174() {
   ...
   */
	...
	/**
 	...
 	function initComboEditable(combo, comboId){
 		with (combo) {
 			if(comboId == "dlv_ctnt_cd" || comboId == "rhq_cd"){
 				//alert(comboId);
 				MultiSelect = false;
    ...
    */
    	...
    	switch(sheetID) {
    		...
    		case "sheet2":    	            	
                with (sheetObj) {
					...
					var HeadTitle1 = "|RHQ of C/A OFC|BKG OFC|C/A OFC|Contract OFC|C.Sales Rep.|Route|Route|Route|Route|B/L Q'ty|Total Volume\n of Created B/L|C/A Q'ty|" + 
					                 "C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|Exemption Case|Class|Class|Class|" + 
					                 "Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind";
					var HeadTitle2 = "|RHQ of C/A OFC|BKG OFC|C/A OFC|Contract OFC|C.Sales Rep.|POR|POL|POD|DEL|B/L Q'ty|Total Volume\n of Created B/L|C/A Q'ty|" + 
					                  	"M|D|C|G|A|O|C.Haul|M.Haul|Exempt|R|N|E|A|B|C|D|E|F|G|H|I|J|K";
					...
					InitDataProperty(0, cnt++ , dtData,80,	daCenter,true,	"ctrt_srep_cd"		,false,	"",	dfNone,	0,	false,	true);
					...
					InitDataProperty(0, cnt++ , dtData,52,	daCenter,true,	"tot_cnt_bl_ttl"	,false,	"",	dfNone,	0,	false,	true);
					...
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"cnt_rsn_d"			,false,	"",	dfNone,	0,	false,	true);
					...
					//InitDataProperty(0, cnt++ , dtHiddenStatus,	daCenter,true,	"exemption"		,false,	"",	dfNone,	0,	false,	true);
    		...
    		case "sheet3":      //sheet3 init
    			with (sheetObj) {
					...
					var HeadTitle1 = "|B/L No|Bkg No|Revenue|Non Revenue|Expense|VVD|B/L OBRD Date|BKG OFC|RHQ of C/A OFC|Contract OFC|C.Sales Rep.|POR CD|POL CD|POD CD|DEL CD|C/A No|C/A Date|" + 
					                 "C/A OFC|C/A Staff|C/A Reason|Exemption|Exempt Case|Kind_A|Kind_B|Kind_C|Kind_D|Kind_E|Kind_F|Kind_G|Kind_H|Kind_I|Kind_J|Kind_K|" + 
					...
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"rev"		,false,	"",	dfNone,	0,	false,	true);
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"nonrev"		,false,	"",	dfNone,	0,	false,	true);
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"exp"		,false,	"",	dfNone,	0,	false,	true);						
					...
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"exemption"		,false,	"",	dfNone,	0,	false,	true);
					InitDataProperty(0, cnt++ , dtData,30,	daCenter,true,	"cnt_exempt"		,false,	"",	dfNone,	0,	false,	true);
    ...
    function setDisplayOP(){        	 
    	...
    	with (sheetObjects[1]) {	        	 
			...
			if ( formObj.ca_reason_g.checked == true && (formObj.off_dis_op_4.checked == true ||
				 formObj.off_dis_op_3.checked == true && formObj.off_dis_op_2.checked == true || 
				 !ComIsNull(formObj.contract_off) || !ComIsNull(formObj.ca_issue_off) || !ComIsNull(formObj.bkg_off) )
				)
				{
				
					HeadTitle1 = "|RHQ of C/A OFC|BKG OFC|C/A OFC|Contract OFC|C.Sales Rep.|B/L Q'ty|Total Volume\nof Contracted B/L|C/A Q'ty|" + 
					//  "C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|Exempt|Class|Class|Class|" + 
					"C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|C/A Reason|Exempt|Class|Class|" + 
					"Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind|Kind";
					HeadTitle2 = "|RHQ of C/A OFC|BKG OFC|C/A OFC|Contract OFC|C.Sales Rep.|B/L Q'ty|Total Volume\nof Contracted B/L|C/A Q'ty|" + 
					"M|D|C|G|A|R|C.Haul|M.Haul|Exempt|R|N|A|B|C|D|E|F|G|H|I|J|K";
				} else {
			...
			if (formObj.off_dis_op_2.checked == true || formObj.bkg_off.value != ''){					 				
				InitDataProperty(0, cnt++ , dtData,60,	daCenter,true,	"bkg_ofc_cd"		,false,	"",	dfNone,	0,	false,	true);
			}else{
...
//			if (formObj.off_dis_op_4.checked == true){
			if ( formObj.ca_reason_g.checked == true && (formObj.off_dis_op_4.checked == true ||
				...
				InitDataProperty(0, cnt++ , dtData,100,	daCenter,true,	"ctrt_srep_cd"		,false,	"",	dfNone,	0,	false,	true);
			}else{
				InitDataProperty(0, cnt++ , dtHidden,100,	daCenter,true,	"ctrt_ofc_cd"		,false,	"",	dfNone,	0,	false,	true);				
				InitDataProperty(0, cnt++ , dtHidden,100,	daCenter,true,	"ctrt_srep_cd"		,false,	"",	dfNone,	0,	false,	true);
			}
...
//			if ((formObj.off_dis_op_2.checked == true || formObj.off_dis_op_3.checked == true || (formObj.ca_reason_g.value == "G" && formObj.off_dis_op_4.checked == true)) ||
			if ((formObj.off_dis_op_2.checked == true || formObj.off_dis_op_3.checked == true ||  formObj.off_dis_op_4.checked == true)  ||
				(formObj.bkg_off.value != '' || formObj.ca_issue_off.value != '' || formObj.contract_off.value != ''))
			{
				InitDataProperty(0, cnt++ , dtAutoSum,120,	daCenter,true,	"tot_cnt_bl_ttl"		,false,	"",	dfInteger,	0,	false,	true);
			}else{
				InitDataProperty(0, cnt++ , dtHidden,120,	daCenter,true,	"tot_cnt_bl_ttl"		,false,	"",	dfInteger,	0,	false,	true);
			...
			InitDataProperty(0, cnt++ , dtAutoSum,40,	daCenter,true,	"cnt_rsn_d"			,false,	"",	dfInteger,	0,	false,	true);
    	...
    	try {
            ...
            switch(srcName) {                   
				...
				case "btn_rd":
					...
					doActionIBSheet(sheetObjects[2],document.form,COMMAND01);
					doPrint(sheetObjects[2],document.form);
     		    break;     		    
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
    	...
    	switch(sAction) {
    		case INIT:      //INIT
    			...
    			sXml = sheetObj.GetSearchXml("ESM_BKG_0174GS.do" , FormQueryString(formObj));
    ...
    */
    ...
    function validateForm(sheetObject1,formObject,sAction){    	
    	...
    	*/
    	...
    	if (!ComIsNull(formObject.corr_from_dt) 
	  			&& !ComIsNull(formObject.corr_to_dt) 
	  			&& ComGetDaysBetween(formObject.corr_from_dt.value, formObject.corr_to_dt.value) >= 31){
 			ComShowCodeMessage("BKG50469");//Can't Input Date Over 31 days!
 			formObject.corr_from_dt.focus();
 			return false;
 		}
    	
    	if (!ComIsNull(formObject.cre_from_dt) 
	  			&& !ComIsNull(formObject.cre_to_dt) 
	  			&& ComGetDaysBetween(formObject.cre_from_dt.value, formObject.cre_to_dt.value) >= 31){
 			ComShowCodeMessage("BKG50469");//Can't Input Date Over 31 days!
 			formObject.cre_from_dt.focus();
			return false;
 		}
    ...
    */    
    function addTemplate(){	   
    	var param = "";
    	var pWin = ComOpenWindow("/hanjin/ESM_BKG_0767.do" + param,"open0767", "statebar=no,width=700,height=600,left=200,top=0");
	    //ComOpenPopup("/hanjin/ESM_BKG_0767.do" + param, 700, 590, 'reSearch', "1,0,1,1,1,1,1");
    }
    ...
    */
	...
	function setSqlCondition(){		  
		...
		if (tempStr != ""){		  
			if (formObj.ca_reason_d.checked == true){			  
				tempStr += " OR COR.CA_RSN_CD = 'D' ";
				tempVal += ",D";
			}
		}else{		  
			if (formObj.ca_reason_d.checked == true){			  
				tempStr = " COR.CA_RSN_CD = 'D' ";
				tempVal = "D";
			}
		}
		...
		if (tempStr != ""){		  
			if (formObj.off_dis_op_4.checked == true){			      		  
				tempStr += ",CTRT_OFC_CD ,CTRT_SREP_CD";
				tempCnt += ",CNT.CTRT_OFC_CD ,CNT.CTRT_SREP_CD ";
				tempVal += ",Contract";
				tempSql += " || 'Contract OFC:' || CTRT_OFC_CD || 'C.Sales Rep.:' || CTRT_SREP_CD ";
			}
		}else{		  
			if (formObj.off_dis_op_4.checked == true){	  		  
				tempStr += ",CTRT_OFC_CD ,CTRT_SREP_CD ";
   ...
   * */
	function doPrint(sheetObject1, formObject){
	   	with (sheetObject1) {
	   		ComOpenWait(true);				
...
//		rdParam    = "/rp ['"+p_bkg_no+"'] ['"+p_ca_no+"'] /riprnmargin /rwait";
    	rdParam    = "/rv bkg_no['"+p_bkg_no+"'] ca_no['"+p_ca_no+"'] /riprnmargin /rwait";

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_0620 : ESM_BKG_0620 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_0620() {
                ...
                switch(srcName) {
		                ...
		                case "btn_Retrieve":
		                	if ( formObject.io_bnd_cd[0].checked ) {
		                		sheetObjects[0].InitHeadRow(0, "| |Booking No.|Seq.|Booking Q'ty|Booking Q'ty|TRO|Q'ty|P/Up Date|P/Up CY|R/Term|Door|Supplier Name|POL|Remark(s)|", true);
		                	} else {
		                		sheetObjects[0].InitHeadRow(0, "| |Booking No.|Seq.|Booking Q'ty|Booking Q'ty|TRO|Q'ty|P/Up Date|P/Up CY|D/Term|Door|Supplier Name|POL|Remark(s)|", true);
		                	} 
            ...
            switch(sheetNo) {
                case 1:      //sheet1 init
                    with (sheetObj) {
    					...
    					var HeadTitle1 = "| |Booking No.|Seq.|Booking Q'ty|Booking Q'ty|TRO|Q'ty|P/Up Date|P/Up CY|R/Term|Door|Supplier Name|POL|Remark(s)|";                                                                                                                                                                                                 
                        var HeadTitle2 = "| |Booking No.|Seq.|TEU|FEU|TRO|Q'ty|Request|Return CY|S/O|EQ Office|Contact Point|POD|Remark(s)|";                                                                                                                                                                                                                 
						...
						InitDataProperty(rowIdx, cnt++ , dtData,					90,		daCenter,	true,			"dor_arr_dt",			false,			"",      dfNone,			0,		true,		true);                               
						InitDataProperty(rowIdx, cnt++ , dtData,					90,		daCenter,	true,			"pkup_loc_cd",			false,			"",      dfNone,			0,		true,		true);                               
						                                                                                                                                                                                                                                                                                                             
						InitDataProperty(rowIdx, cnt++ , dtData,					60,		daCenter,	true,			"rcv_term_cd",			false,			"",      dfNone,			0,		true,		true);                               
						InitDataProperty(rowIdx, cnt++ , dtData,					70,		daCenter,	true,			"dor_loc_cd",			false,			"",      dfNone,			0,		true,		true);                               
						...
						InitDataProperty(rowIdx, cnt++ , dtData,					60,		daCenter,	true,			"so_flg",				false,			"",      dfNone,			0,		true,		true);                       
        ...
        function doActionIBSheet(sheetObj,formObj,sAction) {
            ...
            switch(sAction) {
             ...
             case IBCLEAR: // 화면 로딩시 코드 조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_BKG_0620GS.do", FormQueryString(formObj));
				
				var arrXml = sXml.split("|$$|");
				
				if (arrXml.length > 1) 
					ComXml2ComboItem(arrXml[1], formObj.zone_code, "val", "name");
				if (arrXml.length > 0) 
					ComXml2ComboItem(arrXml[0], formObj.bkg_sts_cd, "val", "val|name");
				
				formObj.bkg_sts_cd.InsertItem(0, '', '');
				formObj.zone_code.InsertItem(0, '', '');
              ...
              case IBSEARCH:      //조회
    	          if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml = sheetObj.GetSearchXml("ESM_BKG_0620_1GS.do", FormQueryString(formObj));
					var arrXml = sXml.split("|$$|");

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
     ...
     * @author 한진해운
     * @extends 
     * @class esm_bkg_0437  생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esm_bkg_0437() {
...
//            	formObject.elements["region"].Index = 0;  //ALL
            	formObject.elements["sts"   ].Index = 2;  //All
            	formObject.elements["bkg_ctrt_tp_cd"].Index = 0;
    ...
    function doActionIBSheet(sheetObj,formObj,sAction,Row,Col,PageNo) {
        ...
        switch (sAction) {
            case IBSEARCH:      //조회
                ...
                var sXml = sheetObj.GetSearchXml("ESM_BKG_0437GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
            ...
            case IBSEARCHAPPEND:  // 페이징 조회
                ...
                sheetObj.DoSearch4Post("ESM_BKG_0437GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix), "iPage=" + PageNo, true);
            break;
            case SEARCH01:      //조회
            	 ...
            	 */     
            	sheetObj.Redraw = false;
                var sXml = sheetObj.GetSearchXml("ESM_BKG_0437GS.do", FormQueryString(formObj));
...
//                formObj.elements["region"].Index = 0;  //ALL
                ...
                if (arrXml.length > 8) 
                	ComXml2ComboItem(arrXml[8], formObj.elements["bkg_ctrt_tp_cd"], "val", "name");
                formObj.elements["bkg_ctrt_tp_cd"].Index =2 ;
                
                if(arrXml.length > 9){
                	arrXml[9] = ComReplaceStr(arrXml[9], "val|multidesc|ibflag|desc|name|", "cd|multidesc|ibflag|desc|nm|");
                	ComSetIBCombo(sheetObj, arrXml[9], prefix+"inp_pic_usr_id", "", 1, "", "", true);
                	ComSetIBCombo(sheetObjects[1], arrXml[9], prefix+"inp_pic_usr_id", "", 1, "", "", true);
                }
                if(arrXml.length > 10){
                	arrXml[10] = ComReplaceStr(arrXml[10], "val|multidesc|ibflag|desc|name|", "cd|multidesc|ibflag|desc|nm|");
                	ComSetIBCombo(sheetObj, arrXml[10], prefix+"rt_pic_usr_id", "", 1, "", "", true);
                	ComSetIBCombo(sheetObjects[1], arrXml[10], prefix+"rt_pic_usr_id", "", 1, "", "", true);
                }
            ...
            case MODIFY01:        //저장
	            ...
	            WaitImageVisible = false;
	            ComOpenWait(true);  //대기이미지 표시
				var sXml = sheetObj.GetSaveXml("ESM_BKG_0437GS.do?f_cmd="+MODIFY01, SaveStr);
            ...
            case MODIFY02:
            	...
            	var saveStr = sheetObj.GetSaveString(false, true, "sheet1_ibflag"); 
            	if(saveStr != ""){
	            	var sXml = sheetObj.GetSaveXml("ESM_BKG_0437GS.do", FormQueryString(formObj)+ "&" +saveStr); 
            ...
            case IBDELETE: // 삭제
	            ...
	            WaitImageVisible = false;
	            ComOpenWait(true);  //대기이미지 표시
				var sXml = sheetObj.GetSaveXml("ESM_BKG_0437GS.do?f_cmd="+REMOVELIST, SaveStr);
    ...
    function showQueueDetail(sheetObj,rowIdx) {
        ...
        if (isBkgLink(rowIdx)) {
	        var param = "?bkg_no="+sheetObj.CellValue(rowIdx, prefix+"bkg_no")
	        	...
	        	+ "&row_idx="+rowIdx;
	        popWinObj = ComOpenWindowCenter2("/hanjin/ESM_BKG_0438.do"+param, "Queue Detail", 1020,630,false,"scrollbars=no,resizable=yes");
     	} else {
        ...
        switch (sheetObj.id) {
            case "sheet1":
                with (sheetObj) {
                    ...
                    var HeadTitle = "|Sel|Seq.|Read|Ugt|SI Kind|Ver|Return|Freq.|R/F|To|SRC|SRC|Booking No.|BL No.\n(from e-SI)|CNTR Qty|Split|DEL\nConti|Lane|VVD|I|R|Q|F|FO|STS|ESI|MF|POL|ETD Date|SI Transfer\nDate|DOC.Cut off|BKG OFC|S.OFC|S.Rep|Shipper|Consignee|Contract|Contract|POD|DEL|PCT Date|Est. BDR\nDate|Return Mail|PIC From|PIC|Input PIC|Rate PIC|Last Updated|Message|Email Subject"
                                  + "|message_yn|sr_crnt_info_cd|sr_crnt_sts_cd|max_sr_no|crnt_usr_id|img_file_ip|img_file_path_ctnt|img_file_nm|img_file_real_path|bkg_sts_cd|pic_cng_id|rtn_fm_usr_id|rtn_to_usr_id|rtn_to_rtn_usr_id|sr_sts_cd|rtn_to_rtn_sts_cd|pnd_flg|sr_kind_cd|sr_knd_cd|src_cd|return_src|sr_wrk_sts_usr_id|sr_no|fax_log_ref_no|rtn_to_usr_remark|sr_wrk_sts_usr_nm|sr_amd_tp_cd|xter_sndr_id|xter_rqst_no|xter_rqst_seq|doc_tp_cd|rgn_ofc_cd|act_sr_amd_seq";//hidden
                    ...
                    InitDataProperty(0, cnt++, dtHidden,   30,  daCenter, false, prefix+"urgency",				false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtHidden,   30,  daCenter, false, prefix+"rc_inp_flg",			false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtData,     90,  daLeft,   false, prefix+"bl_no_ctnt",			false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     90,  daLeft,   false, prefix+"bkg_cz_desc",			false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     30,  daCenter, false, prefix+"split_sts_cd",			false, "", dfEngUpKey,        0, true, true ,1); 
                    InitDataProperty(0, cnt++, dtData,     40,  daCenter, false, prefix+"del_conti_cd",			false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtData,     180, daLeft,   false, prefix+"consignee",			false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     30,  daLeft,   false, prefix+"bkg_ctrt_tp_cd",		false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     80 , daLeft,   false, prefix+"contract_no",			false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     45,  daCenter, false, prefix+"pod_cd",				false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     45,  daCenter, false, prefix+"del_cd",				false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtHidden,   100, daLeft,   false, prefix+"rtn_fm_usr_id",		false, "", dfNone,        0, false, false);  //PIC From
                    InitDataProperty(0, cnt++, dtHidden,   100, daLeft,   false, prefix+"pic_id",				false, "", dfNone,        0, false, false);  //PIC
                    InitDataProperty(0, cnt++, dtComboEdit ,    100, daLeft,   false, prefix+"inp_pic_usr_id",		false, "", dfNone,        0, true,  false);
                    InitDataProperty(0, cnt++, dtComboEdit ,    100, daLeft,   false, prefix+"rt_pic_usr_id",		false, "", dfNone,        0, true,  false);
                    ...
                    InitDataProperty(0, cnt++, dtHidden,   0,   daCenter, false, prefix+"act_sr_amd_seq"   );
            ...
            case "sheet2":
                with (sheetObj) {
                    ...
                    //행정보설정[필수][HEADROWS,DATAROWS,VIEWROWS,ONEPAGEROWS=100]
                    InitRowInfo(1, 1, 3, rowsPerPage);
                    var HeadTitle = "|Sel|Seq.|Read|Ugt|SI Kind|Ver|Return|Freq.|R/F|To|SRC|SRC|Booking No.|BL No.|CNTR Qty|Split|DEL\nConti|Lane|VVD|I|R|Q|F|FO|STS|ESI|MF|POL|ETD Date|SI Transfer\nDate|DOC.Cut off|BKG OFC|S.OFC|S.Rep|Shipper|Consignee|Contract|Contract|POD|DEL|PCT Date|Est. BDR\nDate|Return Mail|PIC From|PIC|Input PIC|Rate PIC|Last Updated|Message|Email Subject"
                    var headCount = ComCountHeadTitle(HeadTitle);
                    //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                    InitColumnInfo(headCount, 11, 0, true);
                    // 해더에서 처리할 수 있는 각종 기능을 설정한다
                    ...
                    InitDataProperty(0, cnt++, dtCheckBox, 25,  daCenter, true,  prefix+"sel",				   false, "", dfNone,        0, true,  true );
                    InitDataProperty(0, cnt++, dtDataSeq,  30,  daCenter, true,  prefix+"seq");
                    InitDataProperty(0, cnt++, dtHidden,   35,  daCenter, false, prefix+"read",                false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtHidden,   30,  daCenter, false, prefix+"urgency",             false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     115, daCenter, false, prefix+"sr_kind",             false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     30,  daCenter, false, prefix+"sr_amd_seq",          false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtData,     90,  daLeft,   false, prefix+"bkg_cz_desc",         false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     30,  daCenter, false, prefix+"split_sts_cd",        false, "", dfEngUpKey,    0, true, true ,1); 
                    InitDataProperty(0, cnt++, dtData,     40,  daCenter, false, prefix+"del_conti_cd",        false, "", dfEngUpKey,    0, true, true ,1); 
                    InitDataProperty(0, cnt++, dtData,     40,  daCenter, false, prefix+"slan_cd",			   false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     70,  daCenter, false, prefix+"vvd_cd",              false, "", dfNone,        0, false, false);
                    ...
                    InitDataProperty(0, cnt++, dtData,     180, daLeft,   false, prefix+"shipper",             false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     180, daLeft,   false, prefix+"consignee",           false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     180, daLeft,   false, prefix+"bkg_ctrt_tp_cd",      false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     180, daLeft,   false, prefix+"contract_no",         false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     45,  daCenter, false, prefix+"pod_cd",              false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     45,  daCenter, false, prefix+"del_cd",              false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtData,     100, daCenter, false, prefix+"pct_date",            false, "", dfUserFormat2, 0, false, false);
                    InitDataProperty(0, cnt++, dtData,     100, daCenter, false, prefix+"bdr_date",            false, "", dfUserFormat2, 0, false, false);
                    InitDataProperty(0, cnt++, dtData,     150, daLeft,   false, prefix+"rtn_to_usr_eml",      false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtHidden,   100, daLeft,   false, prefix+"rtn_fm_usr_id",       false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtHidden,   100, daLeft,   false, prefix+"pic_id",              false, "", dfNone,        0, false, false);
                    InitDataProperty(0, cnt++, dtCombo,     100, daLeft,   false, prefix+"inp_pic_usr_id",      false, "", dfNone,        0, true, false);
                    InitDataProperty(0, cnt++, dtCombo,     100, daLeft,   false, prefix+"rt_pic_usr_id",       false, "", dfNone,        0, true, false);
                    InitDataProperty(0, cnt++, dtData,     100, daCenter, false, prefix+"last_date",           false, "", dfUserFormat2, 0, false, false);
    ...
    function callSplitPop(sheetObj,rowIdx) {
        ...
        var param   = new Array("xter_sndr_id","xter_rqst_no","xter_rqst_seq","sr_knd_cd","sr_no");
 		...
 		sParam +="&openerPgmNo=ESM_BKG_0437"

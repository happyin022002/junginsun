...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
     ...
     * @author 한진해운
     * @extends 
     * @class esm_bkg_0071  생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
	function esm_bkg_0071() {
	...
	function doActionIBSheet(sheetObj, formObj, sAction, Row, Col) {
		...
		switch (sAction) {
			case IBSEARCH: // 조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_BKG_0071GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
	...
	function sheet1_OnDblClick(sheetObj, rowIdx, colIdx) {
		var vvd_cd = sheetObj.CellValue(rowIdx, prefix + "vvd_cd");
		var pol_cd = sheetObj.CellValue(rowIdx, prefix + "pol_cd");
		var pod_cd = sheetObj.CellValue(rowIdx, prefix + "pod_cd");
		...
		switch (sheetObj.id) {
			case "sheet1":
				with (sheetObj) {
					...
					var HeadTitle2 = "|Lane|Dir|V.V.D|POL|POL/ETD|POD|Day|BDR Time\n(BKG POL Local Time)|BDR|Day|BDR Time\n(BKG POL Local Time)|BDR";
					...
					InitDataProperty(0, cnt++, dtData, 35, daCenter, true, prefix + "skd_dir_cd", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 110, daCenter, true, prefix + "vvd_cd", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 80, daCenter, true, prefix + "pol_cd", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 90, daCenter, true, prefix + "vps_etd_dt", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 80, daCenter, true, prefix + "pod_cd", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 60, daCenter, true, prefix + "trnk_bdr_dys", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 160, daCenter, true, prefix + "trnk_bdr_dt", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 30, daCenter, true, prefix + "trnk_bdr_flg", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 50, daCenter, true, prefix + "fdr_bdr_dys", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 160, daCenter, true, prefix + "fdr_bdr_dt", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 30, daCenter, true, prefix + "fdr_bdr_flg", false, "", dfNone, 0, false, false);
					/*
					 * 

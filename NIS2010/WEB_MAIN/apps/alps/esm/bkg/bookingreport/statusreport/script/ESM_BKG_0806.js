...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    function loadPage() {
		...
		rdOpen(rdObject, formObject,param );
	}
	...
	*/
	function RdParam(formObject,sheetObject,prefix) { 
		...
		strResult ="bkg_no["+ key1+")] " + "cntr_no["+ key2+")] " + "bkg_cust_tp_cd["+ key3+")] " + "language['"+ key4+"'] "+ "mphn_no['"+ key5+"'] "+ "fax_no['"+ key6+"'] "+ "snd_dt['"+ key7+"'] strUsr_id[''] strOffice_cd[''] cnmv_sts_cd[''] vvd_cd['']";
	...
	*/
	function rdOpen(rdObject, formObject, param){
		...
		//alert(RD_FormQueryString(formObject , 1 ));
		// /rp [" + param + "] /riprnmargin /rprncenteropt [1] /rwait
		//var rdParam = "/rp [" + param + "] /riprnmargin /rwait";
		var rdParam = "/rv " + param + " /riprnmargin /rwait";

		// 열고자 하는 RD 파일을 지정한다.
		var strPath = RD_path+"apps/alps/esm/bkg/bookingreport/statusreport/report/ESM_BKG_0806.mrd";
		
		Rdviewer.SetMsgTitle("Loading Confirmation by Shipper Preview & Print");
		
		Rdviewer.FileOpen(strPath, RDServer + rdParam); 
		//Rdviewer.EnableToolbar (15); //인쇄 툴바 버튼 Enable

...
//			sheetObjects[0].LoadSearchXml(bkgXML);
		  ...
		  load_flg = "Y";
		  /* UI ESM_BKG_0726 에서는 SeaWayBill for A/R, Sea Waybill 만 출력 하며 최초 Open 시   SeaWayBill for A/R로 셋업 */
		  if("ESM_BKG_0726" == form.param_ui_id.value){
			  comboObjects[0].Code = '6';
		  }
     	...
     	for (var i = HeaderRows; i < HeaderRows + RowCount ; i++) {
				if(	formObject.form_type.Code == "4"){//################# Original B/L
					if(CellValue(i, "bl_tp_cd") == "W"){
						...
						error_cnt++;
						continue; 
					}else if(CellValue(i,"date_flg") == 'Y'){
						setError(i, msgs['BKG08318']);
						error_cnt++;
						continue; 
...
/**
	...
	function form_type_OnChange(comboObj) {
		...
		/* UI ESM_BKG_0726 에서는 SeaWayBill 만 출력 가능 */
		if("ESM_BKG_0726" == form.param_ui_id.value && !('5' == form.form_type.Code || '6' == form.form_type.Code)){
			if("Y" == load_flg){
				form.form_type.Code = '6';
				ComShowCodeMessage('BKG08089');
			}
		}
		
		/* UI ESM_BKG_1164 에서는 Non-Negotiable Feeder B/L 만 출력 가능 */
		if("ESM_BKG_1164" == form.param_ui_id.value && !('7' == form.form_type.Code)){
			if("Y" == load_flg){
				form.form_type.Code = '7';
              ...
              with (sheetObj) {
                ...
                //전체Edit 허용 여부 [선택, Default false]
                 Editable = false;
								...
								InitRowInfo(1, 1, 16, 50);

		            var HeadTitle1 = " |BKG No.|B/L No.|Result|Remarks|B/L Type|Special CGO|Issued|OB/L Print|Released|No. of OB/L|Rider YN|ORG_PPD_RCV_CD | ORG_N3PTY_PPD_CD|obl_block_flg|hbl_tp";
               ...
               //데이터속성    [ROW, COL,  DATATYPE,		WIDTH, DATAALIGN,	COLMERGE,	SAVENAME,		KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
							...
							InitDataProperty(0, cnt++ , dtHidden,			 	50, daCenter,	true,		"date_flg" ); 
							InitDataProperty(0, cnt++ , dtHidden,			 	50, daCenter,	true,		"hbl_tp" ); 

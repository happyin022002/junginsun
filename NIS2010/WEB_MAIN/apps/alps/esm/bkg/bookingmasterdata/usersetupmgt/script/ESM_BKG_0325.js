...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {
		...
		case IBINSERT: // 입력
			...
			if("BLA"==blckTpCd){
				sheetObj.CellValue(sheetObj.SelectRow,4) = "BLA";
			}else if("YEL"==blckTpCd){
				sheetObj.CellValue(sheetObj.SelectRow,4) = "YEL";

				//blck_kw_ctnt에 YELLOW라고 넣어줌
				//기존 하드코드 테이블에 keyword 컬럼의 
				//다음 컬럼 값에 YELLOW가 다 찍혀져 있고 이걸 가지고 YELLOW인지 판단해 
				//bkg main과 e-bkg쪽에서 비교 연산하기 때문에 넣어줌. 
				//해당 로직을 빼고 싶다면, 1. 관련 쿼리수정, 2. bkg, ebkg의 관련 화면 js에 인자값 설정해주는 곳 제거 해야함.
				//관련 화면 ESM_BKG_0079_01, 0079_05, 0229_01, 0229_02 
				sheetObj.CellValue(sheetObj.SelectRow,6) = "YELLOW";  
			}else if("CAL"==blckTpCd){
				sheetObj.CellValue(sheetObj.SelectRow,4) = "CAL";
			}
			break;

...
* 2015.05.20 조창우
* 1.0 Creation
* 참조 : ESM_BKG_0949 Documentation Cut-Off Time Creation
=========================================================*/
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_BKG_1185 : ESM_BKG_1185 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_BKG_1185() {
                ...
                case "sheet1":
                    with (sheetObj) {
                       ...
                       //전체Edit 허용 여부 [선택, Default false]
    					...
    					var HeadTitle1 = "| |  |VVD|POL|Terminal|Destination Country|Lane|Conti|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|VGM Cut-off Time|User ID|Office|Update Date";
	   					var HeadTitle2 = "| |  |VVD|POL|Terminal|Destination Country|Lane|Conti|ETA|ETB|ETD|DAY|Time|Time|DAY|DAY|Excluding Holiday|User ID|Office|Update Date";
    					...
    					InitDataProperty(0, cnt++ , dtDelCheck,			25,		daCenter,	true,		prefix + "del");
    					InitDataProperty(0, cnt++ , dtSeq,				25,		daCenter,	true,		prefix + "Seq");
    					InitDataProperty(0, cnt++ , dtData,				100,	daCenter,	true,		prefix + "vvd_cd",			false,		"",		dfNone,			0,		false,		true,	9);
    					InitDataProperty(0, cnt++ , dtData,				100,	daCenter,	true,		prefix + "loc_cd",			false,		"",		dfEngUpKey,			0,		false,		true,	5);
    					InitDataProperty(0, cnt++ , dtData,				80,		daCenter,	true,		prefix + "yd_cd",			false,		"",		dfNone,		0,		false,		true,	7);
    					InitDataProperty(0, cnt++ , dtCombo,			140,	daCenter,	true,		prefix + "dest_cnt_cd",		false,		"",		dfNone,			0,		false,		true);
    					...
    					InitDataProperty(0, cnt++ , dtData,				50,		daCenter,	true,		prefix + "vgm_itval_hrs",		false,		"",		dfNone,			0,		true,		true);
    					InitDataProperty(0, cnt++ , dtData,				80,		daCenter,	true,		prefix + "hour",			false,		"",		dfNone,			0,		false,		true);
    		            InitDataProperty(0, cnt++,  dtCombo,     		110,   	daLeft, 	true, 		prefix + "vgm_clz_dy_cd",   false, 		"", 	dfNone,         0,  	true,  		true);
    		            InitDataProperty(0, cnt++, 	dtData,      		80,   	daLeft, 	true, 		prefix + "vgm_clz_dy_hrs",  false, 		"", 	dfTimeHm,       0,  	true, 		false);
    					...
    					InitDataProperty(0, cnt++ , dtHidden,  			0,     	daCenter,   true,   	prefix + "vgm_clz_tp_cd"); 
    		            ...
    		            InitViewFormat(0, prefix + "vgm_clz_dy_hrs", "hh:mm");
    		            InitDataValid(0, prefix + "loc_cd", vtEngUpOther);
    		            InitDataValid(0, prefix + "vvd_cd", vtEngUpOther, "1234567890");
    		            InitDataValid(0, prefix + "yd_cd", vtEngUpOther, "1234567890");
    					//InitDataCombo(0, "dest_cnt_cd", "All|Korea", "All|Korea",	"", "", 0);
    					//InitDataCombo(0, "vsl_slan_cd", "All|CAX", "All|CAX",	"", "", 0);
    					//CountPosition = 0;
    		            
    		            // 칼럼 Lane 에서 고정틀 설정
    		            sheetObj.FrozenCols = 9;
    			}
    			break;
            }
        }
         		...
         		var srcName = window.event.srcElement.getAttribute("name");
     				...
     				switch(srcName) {
     					case "btn2_RowAdd":
     						...
     						sheetObject.CellValue2(sheetObject.SelectRow, prefix + "vgm_itval_hrs") = " ";
     						sheetObject.CellValue2(sheetObject.SelectRow, prefix + "vgm_clz_dy_cd") = " ";
      ...
      // Sheet관련 프로세스 처리
        function doActionIBSheet(sheetObj,formObj,sAction) {
            ...
            switch(sAction) {
              	...
              	case IBSEARCH:      //조회
		            ...
		            sheetObj.DoSearch("ESM_BKG_1185GS.do",FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
		 		...
		 		case IBSAVE:        //저장
	                ...
	                sheetObj.DoSave("ESM_BKG_1185GS.do", sParam);
				...
				case SEARCH01:      //조회(service lane code)
					...
					var searchXml = sheetObj.GetSearchXml("ESM_BKG_1185GS.do" ,FormQueryString(formObj));
	            ...
	            case IBSEARCH_ASYNC02:
	            	...
	            	var sXml = sheetObj.GetSearchXml("ESM_BKG_1185GS.do", FormQueryString(formObj));
	            	
	                var arrData = ComXml2ComboString(sXml, "val", "name");
	                if (arrData != null && arrData.length == 2) {
					    sheetObj.InitDataCombo(0, prefix + "vgm_clz_dy_cd", arrData[1], arrData[0]);
	                }
        		...
        		case IBSAVE:        //저장
        			...
        			for(var i = 2 ; i < sheetObj.RowCount+2  ; i++){
        				...
        				if (sheetObj.RowStatus(i) == 'I' || sheetObj.RowStatus(i) == 'U'){
        					...
        					if (sheetObj.CellValue(i, prefix + "loc_cd") == ''){
        					...
        					if (sheetObj.CellValue(i, prefix + "eta") == '1'){
        						...
        						sheetObj.CellValue2(i, prefix + "vgm_clz_tp_cd") = 'A';
        					}else if (sheetObj.CellValue(i, prefix + "etb") == '1'){
        						
        						sheetObj.CellValue2(i, prefix + "vgm_clz_tp_cd") = 'B';
        					}else if (sheetObj.CellValue(i, prefix + "etd") == '1'){
        						
        						sheetObj.CellValue2(i, prefix + "vgm_clz_tp_cd") = 'D';
        					}else if (sheetObj.CellValue(i, prefix + "day") == '1'){
        						
        						sheetObj.CellValue2(i, prefix + "vgm_clz_tp_cd") = 'Y';
        					}
        ...
        function sheet1_OnClick(sheetObj,Row, Col, Value){
    		...
    		if (sheetObj.ColSaveName(Col) == ( prefix + "eta")){
    			...
    			sheetObj.CellValue2(Row, prefix + "vgm_clz_dy_cd") = "";
    			sheetObj.CellValue2(Row, prefix + "vgm_clz_dy_hrs") = "";
    			...
    			sheetObj.CellValue2(Row, prefix + "vgm_itval_hrs") = "";
    			sheetObj.CellValue2(Row, prefix + "vgm_clz_dy_hrs") = "17:00";
    		}
           ...
           * 국가코드가 VN일 경우 'E''W''S''N' DIRECTION CODE로 VGM 계산 가능하도록 처리함.
           */
         function sheet1_OnAfterEdit(sheetObj,Row, Col, Value){

        	 var formObj = document.form;
     		if (Col =='4'|| Col =='3'){
     			if(sheetObj.CellValue(Row,4).substr(0,2) == "VN"){
         			if(sheetObj.CellValue(Row,3).length != 9 && (sheetObj.CellValue(Row,3)!="E" &&  sheetObj.CellValue(Row,3)!="W" &&  sheetObj.CellValue(Row,3)!="S" &&  sheetObj.CellValue(Row,3)!="N")){
             				sheetObj.CellValue2(Row,3) = "ALL";
             				}
     			}else{ 
     				if(sheetObj.CellValue(Row,3).length != 	9){
     				sheetObj.CellValue2(Row,3) = "ALL";
     				}
     			}
     		}
...
//        	var formObj = document.form;
//    		if (Col =='4'|| Col =='3'){
//    			if(sheetObj.CellValue(Row,Col).substr(0,2) == "VN" && ){
//    				if(Col =='3'){
//        			    if (sheetObj.CellValue(Row,Col).length != 9 && (sheetObj.CellValue(Row,Col)!="E" &&  sheetObj.CellValue(Row,Col)!="W" &&  sheetObj.CellValue(Row,Col)!="S" &&  sheetObj.CellValue(Row,Col)!="N") ){
   	...
   	function keyFieldEnable (sheetObj,Row, Col, Value) {
		...
		if (sheetObj.ColSaveName(Col) == ( prefix + "eta") || sheetObj.ColSaveName(Col) == ( prefix + "etb") || sheetObj.ColSaveName(Col) == ( prefix + "etd")){
			...
			if (sheetObj.CellValue(Row, (prefix + "eta")) == 0 || sheetObj.CellValue(Row, (prefix + "etb")) == 0 || sheetObj.CellValue(Row, (prefix + "etd")) == 0){
				sheetObj.CellEditable(Row, prefix + "vgm_clz_dy_cd") = false;
				sheetObj.CellEditable(Row, prefix + "vgm_clz_dy_hrs") = false;
        	    sheetObj.CellEditable(Row, prefix + "vgm_itval_hrs") = true;
			...
			if (sheetObj.CellValue(Row, (prefix + "day")) == 0 ){
        	    sheetObj.CellEditable(Row, prefix + "vgm_itval_hrs") = false;
        	    sheetObj.CellEditable(Row, prefix + "hour") = false;
        	     
				sheetObj.CellEditable(Row, prefix + "vgm_clz_dy_cd") = true;
				sheetObj.CellEditable(Row, prefix + "vgm_clz_dy_hrs") = true;
			}

...
/**
 * @extends 
 * @class esm_bkg_1131 : esm_bkg_1131 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function esm_bkg_1131() {
...
var URL_ESM_BKG = 'ESM_BKG_1131GS.do';
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 		 220, daCenter, false, prefix + "file_nm", false, "", dfNone, 0, false, false);
			...
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "file_desc");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "rgn_ofc_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "loc_cd");
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, true, prefix + "dp_seq");
...
function divLayer_Config() {
	...
	var iframeHTML = 'apps/alps/esm/bkg/bookingmasterdata/bookingmastermgt/jsp/ESM_BKG_1131.HTML';
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSAVE: //저장
		...
		if(upObj.GetFileSize(filePath) > 5*1024*1024){
			ComShowCodeMessage("BKG00370");//"File size can't not exceeds 5MB.";
			return;
		}
	...
	case IBDELETE: // 삭제
		if(sheetObj.CellValue(sheetObj.SelectRow, prefix + "file_nm") != ""){
	...
	with (formObj) {
		...
		if (sAction == IBSAVE) {
			...
			if (c_row == 0) {
				...
				ComRowHideDelete(sheetObj, prefix + "file_nm");
				return;
			}
			var _val = ComGetObjValue(formObj.ridr_tp_cd);
...
function selectFile(sheetObj, Row, Col, _insert) {
	...
	if (Col == '' || Col == '2') {
		...
		if (fileName.indexOf("\\") != -1) {
			...
			if(fileName.substr(0,fileName.indexOf(".")).length >= 20){

 ...
 */
	...
	function loadPage() {
    	...
    	axon_event.addListenerForm('change', 'getVvdPortList', document.form); //- 포커스 나갈때
...
//					}
					...
					if (bdrAuth == "0,0") { // 권한 없음
						...
						if(ComGetObjValue(formObj.bdr_flg) == "N"){ // 조회조건 BDR = "NO"
							...
							if (sheetObj.EtcData("total_booking_cnt") != sheetObj.EtcData("btr_booking_cnt")) {
...
//							if (sheetObj.EtcData("total_booking_cnt") == sheetObj.EtcData("btr_booking_cnt")) {
			...
			case IBCREATE: //VVD_BDR
				...
				if (validateForm(sheetObj, formObj, sAction)) {
					if (ComShowCodeConfirm("BKG80842")) {
						...
						var SaveXml = sheetObj.GetSaveXml("ESM_BKG_0596GS.do", FormQueryString(formObj));
			...
			case REMOVELIST: //VVD_BDR CANCEL
				...
				if (validateForm(sheetObj, formObj, sAction)) {
					if (ComShowCodeConfirm("BKG80844")) {
	...
	/**
	 * 화면에서 입력한 VVD에 해당하는 POL정보를 가져와 콤보박스에 셋팅.
	 */
    function getVvdPortList(){
      	...
      	switch( event.srcElement.name ){
      		case "vvd_cd":
	 	 		...
	 	 		var sXml = sheetObjects[0].GetSearchXml("ESM_BKG_0596GS.do", FormQueryString(formObj));

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class esm_bkg_0133 : esm_bkg_0964 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esm_bkg_0133() {
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
			case IBSEARCH:      //조회
				...
				var rvwFlg = document.form.frm_edo_rvw_flg.value;
                //다중조회
                var aryPrefix = new Array("sheet1_", "sheet2_", "sheet3_"); //prefix 문자열 배열
                var sXml = sheetObjects[1].GetSearchXml("ESM_BKG_0133GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
        ...
        if (ErrMsg == "") {
            if(sheetObj.RowCount > 0){
	            ...
	            for(var i=1; i<=sheetObj.RowCount; i++) {
	                ...
	                }else if (sheetObj.CellValue(i,prefix + "edo_pty_cd") == "PR") {
	    	           	formObj.pr_pty_nm.value = sheetObj.CellValue(i, prefix + "pty_nm1")+sheetObj.CellValue(i, prefix + "pty_nm2");
	    	           	formObj.pr_pty_eml.value = sheetObj.CellValue(i, prefix + "pty_eml");
	    	           	if (sheetObj.CellValue(i,prefix + "pty_rgst_no").length == 10) {
	    	           		formObj.pr_pty_rgst_no.value = sSaupFormat(sheetObj.CellValue(i,prefix + "pty_rgst_no"));
	    	           	} else {
	    	           		formObj.pr_pty_rgst_no.value = sheetObj.CellValue(i,prefix + "pty_rgst_no");
	    	           	}
	                }
	            }	                
            }

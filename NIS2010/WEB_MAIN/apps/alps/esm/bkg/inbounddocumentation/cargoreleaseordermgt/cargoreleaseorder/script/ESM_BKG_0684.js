...
//                        strArg += " /rfonttype40";//2010-03-29 by sungho Arial Unicode Font 사용 시 글자의 윗부분이 잘리는 현상을 해결
                        ...
                        formObject.com_mrdTitle.value = "Bangladesh Cargo Release Order";
                        formObject.com_mrdDisableToolbar.value = "";
                        formObject.com_mrdBodyTitle.value = "Bangladesh Cargo Release Order";
                        ComOpenRDPopup();
...
//                        ComOpenWindow('/hanjin/ESM_BKG_1089.do' + paramVal, 'win4', 'width=600,height=270');
                    ...
                    case "img_refInfo_attr_ctnt3":
                        var cal = new ComCalendar();
                        cal.select(formObject.refInfo_attr_ctnt3, 'yyyy-MM-dd');
                    break;
   ...
   ************************************************************************************/
    ...
    function obj_keypress(){
        ...
        }else if(srcName == 'refInfo_attr_ctnt3'){
            ComKeyOnlyNumber(event.srcElement,"-");
        }
    }
...
//       ComBtnDisable("btn_f_cntr_rls");
   ...
   function buttonEnabledAll(){
    ...
    function inputParamReset(){
        ...
        //라디오버튼 초기화
        document.form.split_flg[0].checked = false;
        document.form.split_flg[1].checked = false;
             ...
             *****************************************************************/
             ...
             with (sheetObj) {
                 ...
                 var HeadTitle = " |BKG_NO|INTER_RMK|DO_HLD_FLG|CSTMS_REF_NM|CSTMS_REF_CTNT|CSTMS_ASGN_NM|CSTMS_ASGN_CTNT|CY_OP_CD|INFO_CGO_FLG|SPLIT_FLG"+
                 				 " |ATTR_CTNT1|ATTR_CTNT2|ATTR_CTNT3|ATTR_CTNT4|ATTR_CTNT5|ATTR_CTNT6";
                 ...
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt1"         ,   false,     "",        dfNone,     0,          false,       true);
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt2"         ,   false,     "",        dfNone,     0,          false,       true);
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt3"         ,   false,     "",        dfNone,     0,          false,       true);
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt4"         ,   false,     "",        dfNone,     0,          false,       true);
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt5"         ,   false,     "",        dfNone,     0,          false,       true);
                 InitDataProperty(0,   cnt++ ,     dtData,      80,    daCenter,  false,   prefix +"attr_ctnt6"         ,   false,     "",        dfNone,     0,          false,       true);
             ...
             ***********************************************************************/
             ...
             with (sheetObj) {
                 ...
                 var HeadTitle = " |BL회수여부|BL발행통수|O/BL ISSUE|OFFICE|DATE|O/BL RECEIVED|OFFICE|DATE|OTHER DOC RECEIVE|OFFICE|DATE|OTR DOC CGOR FLG| | | | |" +
                 				 " |IBD_DOC_RCV_FLG|IBD_DOC_RCV_OFC_CD|IBD_DOC_RCV_USR_ID|IBD_DOC_RCV_DT";
...
//                    var sXml = sheetObjects["blInfo"].GetSearchXml("ESM_BKG_0684GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
                    var sXml = sheetObjects["blInfo"].GetSearchXml("ESM_BKG_0684GS.do", param + "&" + ComGetPrefixParam(aryPrefix));
...
//                    var sXml = sheetObjects["blInfo"].GetSearchXml("ESM_BKG_0684GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
            ...
            case IBSAVE:    //저장
                ...
                if(validateForm(sheetObj, formObj, sAction)){
                    ...
                    var sXml = sheetObj.GetSaveXml("ESM_BKG_0684GS.do", sparam);
...
//                    sheetObjects["euDoRlseStsBl"].CellValue2(1, "euDoRlseStsBl_ibflag") = 'U';
                	sheetObjects["euDoRlseStsBl"].RowStatus(1) = "U";
                    sheetObj.DoSave("ESM_BKG_0684GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("euDoRlseStsBl_"),'', false);
                } else {
                    //변경이 없어도 필요한 인자값 추출을 위해 상태를 강제로 변경시킨다.
                    //sheetObjects["euDoRlseStsCntr"].CellValue2(1, "euDoRlseStsCntr_ibflag") = 'U';
                    //sheetObj.DoSave("ESM_BKG_0684GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("euDoRlseStsCntr_"),'', false);
...
//              document.getElementById("otsRcvInfo_tot_ots_sts_cd").value = "Y";
    ...
    /**
     * Bangladesh D/O Release 기본 정보 조회 IBSheet를 조회하고 나서 처리할 사항
     */
...
//                ComBtnEnable("btn_f_cntr_rls");
      ...
      /**
       * Bangladesh D/O Release Reference 정보 조회 IBSheet를 조회하고 나서 처리할 사항
       */
...
//          ComBtnEnable("btn_obl_cancel");
    ...
    /**
     * Hidden IBSheet를 조회하고 나서 처리할 사항
     * Bangladesh세관 신고를 위한 B/L INFO 추출
     */

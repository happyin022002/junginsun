...
* 2012.01.10 서미진 [CHM-201215426] Special Note , Rate 의 Commodity 별, Route 별 Note conversion 항목에 By Charge 컬럼 추가
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0033 : ESM_PRI_0033 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
  ...
  /**
         	 ...
         	 case "sheet1":
                 with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
	                 ...
	                 InitDataProperty(0, cnt++ , dtHidden,    	40, 	daCenter,  	false,  "rout_seq");
	                 InitDataProperty(0, cnt++ , dtHidden,    	40, 	daCenter,  	false,  "rout_note_seq");
	                 ...
	                 InitDataProperty(0, cnt++ , dtHidden,    	40, 	daCenter,  	false,  "prc_prog_sts_cd");
	 				 InitDataProperty(0, cnt++ , dtHidden, 		90, 	daLeft, 	false, 	"n1st_cmnc_amdt_seq");
         	...
         	case "sheet2":
         		with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     InitDataProperty(0, cnt++ , dtData,       100,  daCenter,  true,  "bkg_no",               false,  "", dfNone,             0,     true,       true,    13);
                     ...
                     InitDataProperty(0, cnt++ , dtHidden,			80,   daCenter,  true,	"gen_spcl_rt_tp_cd",		false,	"",	dfNone, 			0,     true,       true);
   ...
   /**
    * Route Note 의 DETAIL SHEET를 클릭할때 호출되는 function <br>
	...
	function doRowChange(sheetM, sheetD, OldRow, NewRow, OldCol, NewCol, appendRow) {
		...
		if (!isFiredNested && (!openSheet2YN || OldRow != NewRow)) {    
			...
			if(sheetM.CellValue(NewRow, "note_clss_cd") =="D" || sheetM.CellValue(NewRow, "note_clss_cd") =="C") {
				...
				formObj.rout_seq.value = sheetM.CellValue(NewRow, "rout_seq");
				formObj.rout_note_seq.value = sheetM.CellValue(NewRow, "rout_note_seq");
    ...
    */ 
	function sheet1_OnSelectCell(sheetObj, OldRow, OldCol, NewRow, NewCol)  {
		...
		if(openSheet1YN) {				
			runSheet1YN = true;
			formObj.note_ctnt.value = sheetObj.CellValue(sheetObj.SelectRow, "note_ctnt");
				...
				case IBSEARCH: // 조회
	 			...
	 			case IBSEARCHAPPEND: // 조회
					...
					var sNm = sheetObj.GetComboInfo(0,"chg_rule_def_cd","Text");
		 			...
		 			formObj.f_cmd.value = SEARCH01;
	  				var sXml = sheetObj.GetSearchXml("ESM_PRI_0033GS.do", FormQueryString(formObj));
	 			...
	 			case IBSAVE: // 저장  	
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified) {	
				  			...
				  			formObj.note_conv_mapg_id.value = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"note_conv_mapg_id");			  			
							...
							var sParam = FormQueryString(formObj);
			  				...
			  				var sXml = sheetObj.GetSaveXml("ESM_PRI_0033GS.do", sParam);
				  			...
				  			var sXml = sheetObj.GetSaveXml("ESM_PRI_0033GS.do", FormQueryString(formObj));
				  			sheetObj.LoadSaveXml(sXml);
		  				}
						...
						if(sheetObj.RowCount > 0){
							obj.note_conv_flg = "1";
							obj.master_seq = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "rout_seq");
							obj.detail_seq = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "rout_note_seq");
							obj.amdt_seq = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "amdt_seq");
	 			...
	 			case IBINSERT: // Row Add
					...
					var idx = sheetObj.DataInsert();
	 				...
	 				sheetObj.CellValue2(idx, "exp_dt") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"exp_dt");					
	 				sheetObj.CellValue2(idx, "eff_dt") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"eff_dt");
					...
					sheetObj.CellValue2(idx, "note_conv_tp_cd") = "R"; //Proposal Rate Route Note 
					...
					if(sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"note_clss_cd") == "G") {
						...
						sheetObj.CellValue2(idx, "chg_rule_tp_cd") = "C";//CHARGE
						sheetObj.CellValue2(idx, "chg_rule_def_cd") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "chg_cd");
			 			sheetObj.CellValue2(idx, "note_conv_chg_cd") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "chg_cd");
					}
				      		...
				      		if(arrRow != null && arrRow.length > 0) {  
				      			...
				      			for(var i=0; i<arrRow.length; i++) {
									...
									sheetObj.CellValue2(arrRow[i], "amdt_seq") 			= formObj.amdt_seq.value;
					      			...
					      			sheetObj.CellValue2(arrRow[i], "note_conv_tp_cd") 	= "R";  //Proposal Rate Route Note 
				      			}
				      		}
				      	} else {
     ...
     */  
 	function sheet2_OnChange(sheetObj, Row, Col, Value) {
 		...
 		switch(colName)
     	{
 			...
 			case "eff_dt":	
 				var effDt = ComGetDateAdd(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "eff_dt"), "D", 0, "");
		 		var expDt = ComGetDateAdd(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "exp_dt"), "D", 0, "");
 			...
 			case "rt_appl_tp_cd":	
 				...
 				if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
 					...
 					&& chgRuleDefCd != "RAC" && chgRuleDefCd != "FAR" && chgRuleDefCd != "FAD"
 					&& chgRuleDefCd != "SPS" ) {//[CHM-201325096]FAR, FAD 추가, SM Prime Service 추가 - 송민석
                ...
                } else if((chgRuleDefCd == "SPS")){//SM Prime Service // 송민석
                    if (Value != "S" ){ //Subject만 선택 가능함
                        ComShowCodeMessage("PRI00333", sheetObj.CellText(Row, Col));
     ...
     function checkDuration(sheetObj) {
 		...
 		if(rowCount > 0){
 			for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
	 			...
	 			//날짜가 역전되도 AUTORATING시 APP데이터가 필요하기 때문에 VALIDATION제외 - RATE(COMMODITY, ROUTE)
 				/*
 				[CHM-201537221] S/C Note Conversion 입력시 APP Rule code에 날짜 validation 추가 요청 으로 조건에서 제외함.
     ...
     */
 	...
 	function validateDupCheck(sheetObj) {
 		...
 		var rowM = sheetObj.ColValueDupRows("chg_rule_def_cd|chg_rule_tp_cd|rule_appl_chg_tp_cd|rule_appl_chg_cd|bkg_rat_ut_cd|bkg_prc_cgo_tp_cd" +
 		 		...
 		 		"|bkg_slan_cd|bkg_vvd_cd|bkg_no|bkg_soc_flg|bkg_act_cust_def_cd|bkg_dir_call_flg|bkg_mst_hbl_tp_cd|bkg_ts_port_def_cd" +
 		...
 		if (rowM != "") {
            ...
            var hrows = sheetObj.HeaderRows;
 			...
 			for(var i=0; i<rowArr.length; i++) {
                ...
                dupValue += "\t" + sheetObj.CellValue(rowArr[i], "bkg_no");
 				...
 				for(var j=0; j<rowObj.length; j++) {
 	                ...
 	                temValue += "\t" + sheetObj.CellValue(rowObj[j], "bkg_no");
...
//	 			 				ComShowCodeMessage("PRI00303", "Sheet", rowObj[j]);
	...
	function sheet1_OnSearchEnd(sheetObj, errMsg){
 	...
 	function sheet2_OnSearchEnd(sheetObj, errMsg){ 		
 		if(errMsg == "") {
	 		...
	 		var currAmdtSeq = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "amdt_seq");
	 		var currSrcInfoCd = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "src_info_cd");
	 		
	 		// C/TYPE SET
			comboObjects[0].Code2 = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"note_chg_tp_cd");
     ...
     * @param {int} Row 필수 IBSheet Object 의 Row Index
      ...
      */ 		
      function sheet2_OnSaveEnd(sheetObj, ErrMsg)  {    	 
		  ...
		  sheetObjects[0].CellValue2(sheetObjects[0].SelectRow, "note_chg_tp_cd") = comboObjects[0].Code;
      }
        ...
        sheetObj.CellEditable(Row, "bkg_no")               = false;
 		...
 		switch(Value)
     	{
     		...
     		case "SPS":
                ...
                sheetObj.CellEditable(Row, "bkg_no")             = bEdit;
     ...
     */ 	
  	function checkMandatoryValidation(sheetObj, Row) {
 		...
 		if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
 			...
 			&& chgRuleDefCd != "ARB" && chgRuleDefCd != "ADD" && chgRuleDefCd != "NOT"
 			&& chgRuleDefCd != "RAC" && chgRuleDefCd != "FAR" && chgRuleDefCd != "FAD"
 			&& chgRuleDefCd != "SPS" ) {//[CHM-201325096]FAR, FAD 추가 , SM Prime Service 송민석
 		...
 		} else if (chgRuleDefCd == "APP" || chgRuleDefCd == "FAR" || chgRuleDefCd == "FAD"
 		            ||  chgRuleDefCd == "SPS" ) { // [CHM-201325096] FAR, FAD는 APP와 동일하게 처리 해달라고 함, SM Prime Serive - 송민석
 			if(sheetObj.CellValue(Row, "eff_dt") == "") {
 				ComShowCodeMessage("PRI00316","Effective Date");
 				sheetObj.SelectCell(Row, "eff_dt");
 				return false;
...
// 			} 
   ...
   */ 	
     ...
     */	
      ...
      * RULE코드를 선택하면 CHG_RULE_TP_CD:R 이고 NOTE_CONV_RULE_CD에 코드값등록  <br>
      * CHARGE코드를 선택하면 CHG_RULE_TP_CD:C 이고 NOTE_CONV_CHG_CD에 코드값등록  <br>
      * <br><b>Example :</b>
      * <pre>

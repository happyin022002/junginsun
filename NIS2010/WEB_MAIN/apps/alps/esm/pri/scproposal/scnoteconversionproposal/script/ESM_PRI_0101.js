...
* 2015.07.23 최성환 [CHM-201537221] S/C Note Conversion 입력시 APP Rule code에 날짜 validation 추가 요청  - checkDuration
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0101 : ESM_PRI_0101 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0101() {
                 ...
                 with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     InitDataProperty(0, cnt++ , dtSeq,  		60,   	daCenter, 	false,  "seq");
 					 InitDataProperty(0, cnt++ , dtCombo, 		200,	daCenter,	false,	"note_clss_cd",  	false,	"",      dfNone, 		0,     false,       false);
	 			...
	 			case IBSEARCHAPPEND: // 조회
					...
					var sNm = sheetObj.GetComboInfo(0,"chg_rule_def_cd","Text");
		 			...
		 			formObj.f_cmd.value = SEARCH01;
	  				var sXml = sheetObj.GetSearchXml("ESM_PRI_0101GS.do", FormQueryString(formObj));
					...
					//선택한 SEQ의 DURATION EXP_DT를 가져온다.
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified) {
							...
							var sParam = FormQueryString(formObj);
			  				...
			  				var sXml = sheetObj.GetSaveXml("ESM_PRI_0101GS.do", sParam); 
				  			...
				  			var sXml = sheetObj.GetSaveXml("ESM_PRI_0101GS.do", FormQueryString(formObj));
				  			sheetObj.LoadSaveXml(sXml);
		  				}
     ...
     */ 	
  	function checkMandatoryValidation(sheetObj, Row) {
 		...
 		if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
 			...
 			&& chgRuleDefCd != "ARB" && chgRuleDefCd != "ADD" && chgRuleDefCd != "NOT"
 			&& chgRuleDefCd != "RAC" && chgRuleDefCd != "FAR" && chgRuleDefCd != "FAD"
 			&& chgRuleDefCd != "SPS") {//[CHM-201325096]FAR, FAD 추가, SM Prime Serivce 추가 -송민석
 			...
 			if(sheetObj.CellValue(Row, "eff_dt") == "") {
 		       ...
 		       ||  chgRuleDefCd == "SPS") {//[CHM-201325096]FAR, FAD 추가, SM Prime Serive 추가 -송민석
 			if(sheetObj.CellValue(Row, "eff_dt") == "") {
 				ComShowCodeMessage("PRI00316","Effective Date");
 				sheetObj.SelectCell(Row, "eff_dt");
 				return false;
     ...
     */	
 	...
 	function insertChargeRuleType(sheetObj, Row) {
 		...
 		if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
 			...
 			&& chgRuleDefCd != "ARB" && chgRuleDefCd != "ADD" && chgRuleDefCd != "NOT"
 			&& chgRuleDefCd != "RAC" && chgRuleDefCd != "FAR" && chgRuleDefCd != "FAD"
 			&& chgRuleDefCd != "SPS") {//[CHM-201325096]FAR, FAD 추가

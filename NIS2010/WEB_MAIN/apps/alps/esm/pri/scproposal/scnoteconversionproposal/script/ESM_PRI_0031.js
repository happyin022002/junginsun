...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0031 : ESM_PRI_0031 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0031() {
         	 ...
         	 case "sheet1":
                 with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
	                 ...
	                 InitDataProperty(0, cnt++ , dtHidden, 		90, 	daLeft, 	false, 	"nd_cnt");
	                 InitDataProperty(0, cnt++ , dtHidden, 		90, 	daLeft, 	false, 	"na_cnt");
         	...
         	case "sheet3":
         		with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     InitDataProperty(0, cnt++ , dtCombo,   		79,   daCenter,  true,	"conv_prc_rcv_term_cd",		false,	"",	dfNone, 			0,     true,       true);
                     InitDataProperty(0, cnt++ , dtCombo,			79,   daCenter,  true,	"conv_prc_de_term_cd",		false,	"",	dfNone, 			0,     true,       true);
	 		...
	 		switch (sAction) {
				...
				case IBSEARCHAPPEND: // 조회
	 			...
	 			case IBSEARCH_ASYNC01: // 조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_PRI_0031GS.do", FormQueryString(formObj));
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified) {
							...
							var sParam = FormQueryString(formObj);
			  				...
			  				var sXml = sheetObj.GetSaveXml("ESM_PRI_0031GS.do", sParam); 
			  				...
			  				ComOpenWait(true);
				  			var sXml = sheetObj.GetSaveXml("ESM_PRI_0031GS.do", FormQueryString(formObj));
				  			sheetObj.LoadSaveXml(sXml);
		  				}
						...
						if(sheetObj.RowCount > 0){
							obj.note_conv_flg = "1";
							obj.master_seq = sheetObjects[1].CellValue(sheetObjects[1].SelectRow, "cmdt_hdr_seq");
							obj.detail_seq = sheetObjects[1].CellValue(sheetObjects[1].SelectRow, "cmdt_note_seq");
				...
				case IBINSERT: // Row Add
					...
					sheetObj.CellValue2(idx, "note_conv_tp_cd") = "C"; //Proposal Rate Bullet Note
					sheetObj.CellValue2(idx, "note_conv_seq") = parseInt(ComPriGetMax(sheetObj, "note_conv_seq")) + 1;
					
					if(sheetObjects[1].CellValue(sheetObjects[1].SelectRow,"note_clss_cd") == "G") {					
					...
					} else if(sheetObjects[1].CellValue(sheetObjects[1].SelectRow,"note_clss_cd") == "S") { 
						sheetObj.CellValue2(idx, "chg_rule_tp_cd") = "C";//CHARGE
				      		...
				      		if(arrRow != null && arrRow.length > 0) {  
				      			...
				      			for(var i=0; i<arrRow.length; i++) {
									...
									sheetObj.CellValue2(arrRow[i], "amdt_seq") 			= formObj.amdt_seq.value;
					      			...
					      			sheetObj.CellValue2(arrRow[i], "note_conv_tp_cd") 	= "C"; //Proposal Rate Bullet Note
				      			}
				      		}
				      	} else {
				...
				case "chg_rule_def_cd":		
					...
					if (Value != null && Value != "" && Value.length == 3) {
						...
						if (sCode.indexOf(Value) < 0) {
							...
							sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + "&etc1=" + Value);
				...
				case "rt_appl_tp_cd":	
					...
					if(Value == "A" || Value == "F") {
	 				...
	 				if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
	 					...
	 					&& chgRuleDefCd != "ADD" && chgRuleDefCd != "FAR" && chgRuleDefCd != "FAD"
...
//				} 
	...
	*/ 	
		...
		function insertChargeRuleType(sheetObj, Row) {
			...
			if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
				...
				&& chgRuleDefCd != "ARB" && chgRuleDefCd != "ADD" && chgRuleDefCd != "NOT"
	 			...
	 			&& chgRuleDefCd != "SPS" ) {//[CHM-201325096]FAR, FAD 추가, 20171101 SPS(SM Prime Service) 추가-송민석
	  ...
	  /**
	    ...
	    */
	    function restylingPagePostTr(sheetObj) {
	    	...
	    	var cmdtCurRow = sheetObj.SelectRow;
	    		    	
       		for(var i=sheetObj.HeaderRows; sheetObj.RowCount > 0 && i <= sheetObj.LastRow; i++) {
       			var ndCnt = sheetObj.CellValue(i, "nd_cnt");
       			var naCnt = sheetObj.CellValue(i, "na_cnt");
       			cmdtCurRow = sheetObj.CellValue(i, "cmdt_hdr_seq");
       			if (cmdtCurRow < 0) {
    				continue;
    			}
       			       			
       			if (ndCnt == 0) {
       				sheetObj.CellFont("FontStrikethru", cmdtCurRow, 1, cmdtCurRow, sheetObj.LastCol) = true;
    			} else {
    				sheetObj.CellFont("FontStrikethru", cmdtCurRow, 1, cmdtCurRow, sheetObj.LastCol) = false;
    			}
    			
    			if (naCnt == 0) {
    				sheetObj.RowFontColor(cmdtCurRow) = sheetObj.RgbColor(0,0,0);
    			} else {
    				if(sLgcyIfFlg != "Y") {
    					sheetObj.RowFontColor(cmdtCurRow) = sheetObj.RgbColor(255,0,0);
    				}
    			}
       		}
     ...
     * Commodity Note 에 데이터가 없을 경우 버튼을 제어한다. <br>
     * <br><b>Example :</b>
     * <pre>
     * buttonControl()
     * </pre>
  	...
  	function buttonControl(){
        ...
        var reqUsrFlg = formObj.req_usr_flg.value;
 		...
 		try{		
			 ...
			 * 3. 로그인 유저의 오피스 코드가 'PKGSA,SELCMQ 경우

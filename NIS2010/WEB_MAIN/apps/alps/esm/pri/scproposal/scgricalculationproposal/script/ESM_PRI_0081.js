   ...
   /**
	...
	function loadPage() {
        ...
        if (document.form.gen_spcl_rt_tp_cd.value == "G") {
        	document.form.cmdtActcustTp[1].disabled = true;
        }
        ...
        if (document.form.gri_cal_readonly.value == "Y") {
        	btnReadOnly();
        } 
	}
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case 1: // sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|gen_spcl_rt_tp_cd|GRI Group Seq.|cmdt_seq|CMDT Type|Commodity Code|Description|";
				...
				InitDataProperty(0, cnt++, dtDataSeq, 70, daCenter, false, "Seq");
                ...
                InitDataProperty(0, cnt++, dtPopupEdit, 120, daCenter, false, "prc_cmdt_def_cd", true, "", dfNone, 0, true, true, 6);
                InitDataProperty(0, cnt++, dtData, 220, daLeft, false, "prc_cmdt_def_nm", true, "", dfNone, 0, false, false);
		...
		case 2: // sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|gen_spcl_rt_tp_cd|GRI Group Seq.|act_cust_seq|Customer Code|Customer Code|Actual Customer Name";
				var headCount = ComCountHeadTitle(HeadTitle);
	...
	function sheet2_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colName == "cust_cnt_cd" || colName == "cust_seq") {
			...
			var param =  "&cust_cnt_cd=" + sheetObj.CellValue(Row, "cust_cnt_cd") + "&cust_seq=" + sheetObj.CellValue(Row, "cust_seq") ;
			//alert(FormQueryString(formObj) )
			var sXml = sheetObj.GetSearchXml("ESM_PRI_0081GS.do", FormQueryString(formObj) + param);
			var arrData = ComPriXml2ComboString(sXml, "customer_cd", "cust_lgl_eng_nm");
			if (arrData != null && arrData.length > 0 && arrData[1] != "") {
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "prc_cmdt_def_cd") {
			if (sheetObj.CellEditable(Row, "prc_cmdt_def_cd")) {
				...
				sUrl += "&grp_cd=" + PRI_SP_SCP + "&commodity_cmd=CG&prc_cmdt_tp_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "prc_cmdt_tp_cd") + "&multi_yn=Y";
				...
				if (rtnVal != null && rtnVal.length > 0) {
					for (var i = 0; i < rtnVal.length; i++) {
						if (i == 0) {
							sheetObj.CellValue2(Row, "prc_cmdt_def_cd") = rtnVal[i].cd;
							sheetObj.CellValue2(Row, "prc_cmdt_def_nm") = rtnVal[i].nm;
							...
							sheetObj.CellValue2(idx, "prc_cmdt_def_cd") = rtnVal[i].cd;
							sheetObj.CellValue2(idx, "prc_cmdt_def_nm") = rtnVal[i].nm;
							
							if (rtnVal[i].cd.length == 5) {
								sheetObj.CellValue2(idx, "prc_cmdt_tp_cd") = "G";
							} else if (rtnVal[i].cd.length == 6) {
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "cust_seq") {
			if (sheetObj.CellEditable(Row, "cust_seq")) {
	            var sUrl = "/hanjin/ESM_PRI_0082.do?" + FormQueryString(document.form);
	            sUrl += "&is_popup=true&cust_cnt_cd="+sheetObj.CellValue(Row, "cust_cnt_cd")+"&cust_seq="+sheetObj.CellValue(Row, "cust_seq");
				 
	            var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0082", 640, 460, true);
				if (rtnVal != null){
		...
		try {
	        ...
	        case IBSAVE: // OK
	            ...
	            for (var i = sheetObjects[0].HeaderRows; sheetObjects[0].RowCount > 0 && i <= sheetObjects[0].LastRow; i++) {
	        		var n1stOrdRef;
	        		if (sheetObjects[0].CellValue(i, "prc_cmdt_tp_cd") == "G") {
	        			n1stOrdRef = 1;
	        		} else if (sheetObjects[0].CellValue(i, "prc_cmdt_tp_cd") == "R") {
	        			n1stOrdRef = 2;
	        		} else if (sheetObjects[0].CellValue(i, "prc_cmdt_tp_cd") == "C") {
	        			n1stOrdRef = 3;
	        		} else {
	        			n1stOrdRef = 99;
	        		}
	        		var prevStatus = sheetObjects[0].RowStatus(i);
	        		sheetObjects[0].CellValue2(i, "n1st_ord_ref") =  n1stOrdRef;
	        		sheetObjects[0].RowStatus(i) = prevStatus;
	            }
	            
	        	sheetObjects[0].ColumnSort("n1st_ord_ref|prc_cmdt_def_cd", "ASC", "DESC|ASC", true);
			...
			case IBSEARCH: // 조회
				...
				sXml = dialogArguments.getSheetXml(3);
	...
	/**
	 * CLOSE 버튼을 제외한 모든 버튼들의 작동을 막는다.<br>
	 * @param 
	 * @return 없음
	 * @author 서미진
	 * @version 2012.03.27
    ...
    function btnReadOnly() {	
    	...
    	document.form.cmdtActcustTp[0].disabled = true;
    	document.form.cmdtActcustTp[1].disabled = true;

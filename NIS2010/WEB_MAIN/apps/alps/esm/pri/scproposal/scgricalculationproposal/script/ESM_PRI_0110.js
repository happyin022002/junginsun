...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "rout_pnt_loc_def_cd") {
			if (sheetObj.CellEditable(Row, "rout_pnt_loc_def_cd")) {
				...
				sUrl += "&group_cmd=" + PRI_SP_SCP + "&location_cmd=LGC&loc_tp_cd="
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "rout_via_port_def_cd") {
			if (sheetObj.CellEditable(Row, "rout_via_port_def_cd")) {
	            var sUrl = "/hanjin/ESM_PRI_4026.do?" + FormQueryString(document.form);
	            sUrl += "&group_cmd=" + PRI_SP_SCP + "&location_cmd=LGC&loc_tp_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "rout_via_port_tp_cd")+"&multi_yn=Y";
					...
					for ( var i = 0; i < rtnVal.length; i++) {
						if (i == 0) {
							...
							if (rtnVal[i].cd.length == 2) {
								...
								sheetObj.CellValue2(idx, "rout_via_port_tp_cd") = "G";
							} else if (rtnVal[i].cd.length == 5) {
    ...
    function btnReadOnly() {	
    	...
    	document.form.rt_pnt[0].disabled = true;
    	document.form.rt_pnt[1].disabled = true;
    	document.form.rt_pnt[2].disabled = true;
    	document.form.rt_pnt[3].disabled = true;

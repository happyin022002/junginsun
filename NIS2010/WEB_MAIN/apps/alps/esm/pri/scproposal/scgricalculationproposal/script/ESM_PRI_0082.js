...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends
	 * @class ESM_PRI_0082 : ESM_PRI_0082 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESM_PRI_0082() {
		...
		switch (sheetID) {
		 	case "sheet1":
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtData, 		100, 	daCenter, 	true, 	"customer_cd", 				false, "", dfNone, 0, false, false);
		...
		switch (sAction) {
			case IBSEARCH: // 조회
				...
				formObj.f_cmd.value = SEARCH01;
 				sheetObj.DoSearch("ESM_PRI_0082GS.do", FormQueryString(formObj));
...
//            }
	...
	function buttonOkClick(sheetObj) {
		...
		rtnObject.custCd 	= sheetObj.CellValue(Row, "customer_cd");				//Customer Number
		...
		var rtnArray = new Array();
	  	...
	  	if(radioType == "C") { //Customer Code
			...
			for(var i=0; i<chkArr.length; i++) {
				var rtnObject = new Object();
				rtnObject.cust_cd = sheetObjects[0].CellValue(chkArr[i], "customer_cd");

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0057_07 : ESM_PRI_0057_07 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    		...
    		switch(srcName) {
            	case "btn_gricalc":
	            	if (formObject.prop_no.value != "" && formObject.amdt_seq.value != "" && formObject.svc_scp_cd.value != "") {
	            		ComPriOpenWindowCenter("/hanjin/ESM_PRI_0113.do?" + FormQueryString(formObject), "ESM_PRI_0113", 605, 455, true);
	                }
                break;
    	...
    	switch (sheetID) {
    		...
    		case "sheet1":
    			with (sheetObj) {
    				...
    				var HeadTitle = "Seq.|propNo|amdtSeq|Point|Description|Trans Mode|Term|Base Port|VIA|D/Call|Per|Cargo Type|Commodity|Currency|Proposal|C.Offer|Final|EFF Date|EXP Date|Source|Status|GRI|GRI|seq|Note|Accept Staff/Team|Accept Date||||||||||";
    				...
    				InitDataProperty(0, cnt++, dtHidden, 		60,   daCenter,  false,	"prop_no");
    				...
    				InitDataProperty(0, cnt++, dtData,    		70,   daLeft,	 false,	"rout_pnt_loc_def_nm");
    				...
    				InitDataProperty(0, cnt++, dtCheckBox,   	60,   daCenter,  false,	"dir_call_flg");
    				InitDataProperty(0, cnt++, dtCombo,  		40,   daCenter,  false,	"rat_ut_cd");
    				InitDataProperty(0, cnt++, dtCombo,     	90,   daCenter,  false,	"prc_cgo_tp_cd");
    				InitDataProperty(0, cnt++, dtData,   		80,   daCenter,  false,	"prc_cmdt_def_cd");
    				InitDataProperty(0, cnt++, dtCombo,      	70,   daCenter,  false,	"curr_cd");
    				InitDataProperty(0, cnt++, dtData,      	70,   daRight,   false,	"prop_frt_rt_amt",  true,  	"", dfNullFloat,	2);
    				InitDataProperty(0, cnt++, dtData,      	70,   daRight,   false,	"coffr_frt_rt_amt", true,  	"", dfNullFloat,	2);
    				InitDataProperty(0, cnt++, dtData,      	70,   daRight,   false,	"fnl_frt_rt_amt",   true,  	"", dfNullFloat,	2);
    				InitDataProperty(0, cnt++, dtData,      	80,   daCenter,  false,	"eff_dt",			false,	"",	dfDateYmd);
    				InitDataProperty(0, cnt++, dtData,      	80,   daCenter,  false,	"exp_dt",			false,	"",	dfDateYmd);
    				InitDataProperty(0, cnt++, dtCombo,     	90,   daCenter,  false,	"src_info_cd");
    				InitDataProperty(0, cnt++, dtCombo,  		80,   daCenter,  false,	"prc_prog_sts_cd");
    				InitDataProperty(0, cnt++, dtData,      	20,   daCenter,  false,	"gri_appl_tp_cd");
    				InitDataProperty(0, cnt++, dtData,      	70,   daRight,   false,	"gri_appl_amt");
    				InitDataProperty(0, cnt++, dtData,      	30,   daRight,   false,	"note_dp_seq");
    				InitDataProperty(0, cnt++, dtData,      	90,   daLeft,    false,	"add_chg_note_ctnt");
    				InitDataProperty(0, cnt++, dtData,      	120,  daLeft,    false,	"acpt_usr_nm");
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
	     	...
	     	case IBSEARCH:	//조회
	     		...
	     		var sXml = sheetObj.getSearchXml("ESM_PRI_0057_07GS.do", FormQueryString(formObj));
			...
			case IBSEARCH_ASYNC04: // radio button 초기화

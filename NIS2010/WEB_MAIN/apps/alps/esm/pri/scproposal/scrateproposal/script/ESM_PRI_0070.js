   ...
   /**
	...
	function sheet1_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colName == "cust_cnt_cd" || colName == "cust_seq") {
			...
			if ( checkValidAfil ( sheetObj, sheetObj.CellValue(Row, "cust_cnt_cd"), sheetObj.CellValue(Row, "cust_seq") ) == false ) {
				arrData = null;
			}
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "cust_seq") {
			if (sheetObj.CellEditable(Row, "cust_seq")) {
				...
				if ( checkValidAfil ( sheetObj, rtnVal.custCntCd, rtnVal.custSeq ) == false ){
					rtnVal = null;
				}
				if (rtnVal != null){
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		try {
	        ...
	        case IBSEARCH_ASYNC13: // Accept
	            ...
	            var sXml = sheetObj.GetSaveXml("ESM_PRI_0070GS.do", sParam);
    ...
    function checkValidAfil ( sheetObj, cust_cnt_cd, cust_seq ) {
    	var rtn = true;
    	var formObj = document.form;
    	
		if ( formObj.cust_type.value == "A" ) {
			formObj.f_cmd.value = SEARCH31;
			var param = "&etc1=" + cust_cnt_cd + "&etc2=" + cust_seq;
			var aXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
			var aCnt = ComGetEtcData(aXml, "cnt");
			if( parseInt ( aCnt ) <= 0 ){
				ComShowMessage ( "Customer Code is not Valid Affiliate");
				rtn = false;
			} else if ( parseInt ( aCnt ) > 0 ) {
				rtn = true;
			}
		} 
		return rtn;

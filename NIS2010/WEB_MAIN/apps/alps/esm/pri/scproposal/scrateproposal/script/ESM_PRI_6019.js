...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_6019 : ESM_PRI_6019 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_6019() {
 ...
 document.onclick = processButtonClick;
              ...
              case "sheet1":      //sheet1 init
                 with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     var HeadTitle1 = "|Sel.|Seq.|Commodity|Origin|Origin|O.Via|D.Via|Dest.|Dest.|Per|CGO\nType|Surcharge|Surcharge|Surcharge|Surcharge|prop_no|amdt_seq|svc_scp_cd|gen_spcl_rt_tp_cd|scg_adj_seq|prc_cmdt_tp_cd|org_loc_tp_cd|org_via_loc_tp_cd|dest_via_loc_tp_cd|dest_loc_tp_cd";
 					var HeadTitle2 = "|Sel.|Seq.|Commodity|Code|Term|O.Via|D.Via|Code|Term|Per|CGO\nType|Code|Cur.|Amount|(USD)|prop_no|amdt_seq|svc_scp_cd|gen_spcl_rt_tp_cd|scg_adj_seq|prc_cmdt_tp_cd|org_loc_tp_cd|org_via_loc_tp_cd|dest_via_loc_tp_cd|dest_loc_tp_cd";
              ...
              with (sheetObj) {
                 ...
                 //전체Edit 허용 여부 [선택, Default false]
                  ...
                  //데이터속성    [ROW, COL,  DATATYPE,			WIDTH,	DATAALIGN,	COLMERGE,	SAVENAME,		KEYFIELD,	CALCULOGIC, DATAFORMAT,		POINTCOUNT, UPDATEEDIT,		INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
					...
					InitDataProperty(0, cnt++ , dtData,				120,   	daCenter,  	true,		"prop_no",   			false,          "",      dfNone,      		0,			false,      false); 					
					InitDataProperty(0, cnt++ , dtData,				120,   	daCenter,  	true,		"amdt_seq",   			false,          "",      dfNone,      		0,			false,      false);
					InitDataProperty(0, cnt++ , dtData,				120,   	daCenter,  	true,		"svc_scp_cd",   			false,          "",      dfNone,      		0,			false,      false);
					InitDataProperty(0, cnt++ , dtData,				120,   	daCenter,  	true,		"gen_spcl_rt_tp_cd",   			false,          "",      dfNone,      		0,			false,      false);
         ...
         switch(sAction) {
 			case IBSEARCH:      //조회
	            ...
	            formObj.f_cmd.value = SEARCH;
 				...
 				var sXml =  sheetObj.GetSearchXml("ESM_PRI_6019GS.do", FormQueryString(formObj) );
				var sCode = sheetObj.GetComboInfo(0, "chg_cd", "Code");
				var sText = sheetObj.GetComboInfo(0, "chg_cd", "Text");
 				var arrData = ComPriXml2Array(sXml, "chg_cd|chg_nm");
 			...
 			case IBSAVE:        //저장
				...
				//sheetObj.DoSave("ESM_PRI_6019GS.do", FormQueryString(formObj),-1,false );
				
				var sXml = sheetObj.GetSaveXml("ESM_PRI_6019GS.do", sheetObj.GetSaveString() + "&" + FormQueryString(formObj)  );
     ...
     */           
     function initCombo(formObj){  
		...
		//	처리 중 대기 이미지를 표시하지 않도록 설정한다. 
 		...
 		//공통  CD02070 SC RECEIVING TERM CODE     		
		var sXml = sheetObj.GetSearchXml("PRICommonGS.do" , FormQueryString(formObj)+"&cd=CD02070");
		var arrXml = sXml.split("|$$|");
		if ( arrXml[0] != null)	setIBCombo(sheetObj, arrXml[0], "prc_rcv_term_cd", true, 0);						
 		formObj.f_cmd.value = SEARCH20;
 		//공통  CD02071 SC RECEIVING TERM CODE
		var sXml = sheetObj.GetSearchXml("PRICommonGS.do" , FormQueryString(formObj)+"&cd=CD02071");
		var arrXml = sXml.split("|$$|");
	...
	function getXChgRateFromYrMon(){
    	var tmpXchgYrMon = "";
       	var formObj = document.form
 		var formParam = "f_cmd="+COMMAND25+"&etc1="+formObj.prop_no.value;
 		formParam = formParam + "&etc2="+formObj.amdt_seq.value;
 		formParam = formParam + "&etc3="+formObj.svc_scp_cd.value;
 		...
 		var loc_type="";
       	...
       	switch(colName)
       	{
   	    	case "prc_cmdt_def_cd":		
 	  	  		var sUrl = "ESM_PRI_6050.do";
 	  	  		var params = new Array();  	  		
 	  	  		params["prop_no"] = formObj.prop_no.value;
 	  	  		params["amdt_seq"] = formObj.amdt_seq.value;
 	  	  		params["svc_scp_cd"] = formObj.svc_scp_cd.value;
 	  	  		params["cmdt_def_cd"] = sheetObj.CellValue(row,"prc_cmdt_def_cd");
 	  	  		window.Params = params;
 	  	  		//FormQueryString(formObj);
 	  	  		
 	  			var rtnVal = ComOpenWindowCenter(sUrl , "ESM_PRI_6050", 900, 365, true);
 	  			if (rtnVal != null){
   	    	...
   	    	case "dest_loc_def_cd":
			  	...
			  	params["dest_loc_tp_cd"] = sheetObj.CellValue(row,"dest_loc_tp_cd");
 	  	 		...
 	  	 		params["loc_type"] = loc_type;
 	  	  		window.Params = params; 	  	  		
 	  			var rtnVal = ComOpenWindowCenter(sUrl, "ESM_PRI_6051", 900, 400, true);
 	  			if (rtnVal != null){

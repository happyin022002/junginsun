   ...
   /**
	...
	function sheet1_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colName == "note_clss_cd") {
			...
			} else if (Value == "C") {		//route note 에서는 Chassis Exception 입력 불가		
				ComShowCodeMessage("PRI00333", "Chassis");				
				sheetObj.CellValue2(Row, "note_clss_cd") = "";
			} else {
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "note_ctnt_pop") {
			...
			if (sheetObj.CellEditable(Row, "note_clss_cd")) {
	            ...
	            var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0089", 600, 377, true);
...
//			}
			...
			if(!ComIsNull(sheetObj.CellValue(Row, "note_conv_mapg_id")))	{
				if(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_clss_cd") == "D") {
		            ...
		            ComOpenPopup('EES_DMT_2001.do?prop_no=' + formObj.prop_no.value + '&caller=0097', 1024, 700, 'findCustomer', '1,0,1,1,1,1,1', true);
					...
					var sUrl = "/hanjin/ESM_PRI_0033.do?"+sParam;			
					var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0033", 1000, 625, true);
					...
					if(rtnVal != null && rtnVal.length > 0) {
						...
						for(var i=0; i < rtnVal.length; i++) {
							...
							for(var j = sheetObj.HeaderRows; j <= sheetObj.LastRow; j++) {
								if(sheetObj.CellValue(j, "rout_seq") == rtnVal[i].master_seq 
										&& sheetObj.CellValue(j, "rout_note_seq") == rtnVal[i].detail_seq
						...
						dialogArguments.updateRoutNoteConvMapgChk(rtnVal);
					}
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
	    try {
	        ...
	        case IBSEARCH_ASYNC13: // Accept
	            ...
	            var sXml = sheetObj.GetSaveXml("ESM_PRI_0097GS.do", sParam);
    ...
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case IBSAVE: // 저장
            ...
            for (var i = sheetObj.HeaderRows; sheetObj.RowCount > 0 && i <= sheetObj.LastRow; i++) {
            	...
            	if (sheetObj.CellValue(i, "note_clss_cd") == "C") {
            		checkCHSS = true;
            	}
            }

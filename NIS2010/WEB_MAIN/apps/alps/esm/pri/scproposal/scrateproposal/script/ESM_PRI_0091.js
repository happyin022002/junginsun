...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    var amendSaveFlag = false; //AMEND 하고 CONVERSION 팝업클릭시 호출불가.
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case 1: // sheet1 init
			with (sheetObj) {
                ...
                InitDataProperty(0, cnt++, dtCombo, 70, daCenter, false, "note_clss_cd", true, "", dfNone, 0, true, true);
                InitDataProperty(0, cnt++, dtComboEdit, 65, daCenter, false, "chg_cd", false, "", dfNone, 0, true, true);
                InitDataProperty(0, cnt++, dtData, 255, daLeft, false, "note_ctnt", true, "", dfNone, 0, false, false);
				...
				InitDataProperty(0, cnt++, dtHidden, 100, daCenter, false, "note_chg_tp_cd", true, "", dfNone, 0, true, true);
	...
	function sheet1_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colName == "note_clss_cd") {
			if (Value == "S") {
				sheetObj.CellEditable(Row, "chg_cd") = true;
			} else {
				sheetObj.CellEditable(Row, "chg_cd") = false;
	...
	function sheet1_OnClick(sheetObj, Row, Col, Value) {
		...
		if (colName == "note_ctnt") {
			if (sheetObj.CellEditable(Row, "note_clss_cd")) {
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "note_ctnt_pop") {
			if (sheetObj.CellValue(Row, "note_clss_cd") == "") {
				
				return false;
			}
			if (sheetObj.CellEditable(Row, "note_clss_cd")) {
	            var sUrl = "/hanjin/ESM_PRI_0089.do?" + FormQueryString(document.form);
	            sUrl += "&note_clss_cd=" + sheetObj.CellValue(Row, "note_clss_cd");
	            sUrl += "&chg_cd=" + sheetObj.CellValue(Row, "chg_cd");
	            
	            var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0089", 600, 382, true);
...
//			}
			...
			//수정사항이 존재할경우 CONVERSION POPUP OPEN 불가
			...
			if(!ComIsNull(sheetObj.CellValue(Row, "note_conv_mapg_id")))	{
				if(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_clss_cd") == "D") {
		            ...
		            //sUrl += "&note_clss_cd=" + sheetObj.CellValue(Row, "note_clss_cd");
		            //var rtnVal = ComOpenWindowCenter(sUrl, "ESM_PRI_2001", 1010, 705, true);
		            
		            ComOpenPopup('EES_DMT_2001.do?prop_no=' + formObj.prop_no.value + '&caller=0091', 1024, 700, 'findCustomer', '1,0,1,1,1,1,1', true);
					...
					sParam += "&cmdt_hdr_seq=" + sheetObj.CellValue( Row, "cmdt_hdr_seq");
					sParam += "&cmdt_note_seq=" + sheetObj.CellValue( Row, "cmdt_note_seq");
					sParam += "&gen_spcl_rt_tp_cd=" + sheetObj.CellValue( Row, "gen_spcl_rt_tp_cd");
					sParam += "&eff_dt=" + sheetObj.CellValue( Row, "eff_dt");
					sParam += "&exp_dt=" + sheetObj.CellValue( Row, "exp_dt");
					sParam += "&master_seq=" + dialogArguments.sheetObjects[0].SelectRow;
					sParam += "&detail_seq=" + sheetObjects[0].SelectRow;
					sParam += "&prop_sts_cd=" +  formObj.prc_prop_sts_cd.value;
					
					var sUrl = "/hanjin/ESM_PRI_0031.do?"+sParam;			
					var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0031", 1000, 705, true);
					/*
					if (rtnVal != null) {
						//CONVERSION 화면에서 선택ROW가 변동가능하기 때문에 FOR문절 사용
						for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
							if(sheetObj.CellValue(i, "cmdt_hdr_seq") == rtnVal.master_seq 
									&& sheetObj.CellValue(i, "cmdt_note_seq") == rtnVal.detail_seq
  									&& sheetObj.CellValue(i, "amdt_seq") == rtnVal.amdt_seq ) {
								sheetObj.CellValue2(i, "note_conv_mapg_id_chk") = rtnVal.note_conv_flg;
								sheetObj.CellValue2(i, "note_chg_tp_cd") = rtnVal.note_chg_tp_cd;
							}
						}
					}
					*/
					...
					if (rtnVal != null && rtnVal.length > 0) {
						...
						for(var i=0; i < rtnVal.length; i++) {
							...
							for(var j = sheetObj.HeaderRows; j <= sheetObj.LastRow; j++) {
								if(sheetObj.CellValue(j, "cmdt_hdr_seq") == rtnVal[i].master_seq 
										&& sheetObj.CellValue(j, "cmdt_note_seq") == rtnVal[i].detail_seq
	  									&& sheetObj.CellValue(j, "amdt_seq") == rtnVal[i].amdt_seq ) {
									var prevStatus = sheetObj.RowStatus(j);
									sheetObj.CellValue2(j, "note_conv_mapg_id_chk") = rtnVal[i].note_conv_flg;
									sheetObj.CellValue2(j, "note_chg_tp_cd") = rtnVal[i].note_chg_tp_cd;
									sheetObj.RowStatus(j) = prevStatus;
								}
							}
						}
						dialogArguments.updateCmdtNoteConvMapgChk(rtnVal);
					}
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
        try {
	        ...
	        case IBSEARCH_ASYNC13: // Accept
	            ...
	            var sXml = sheetObj.GetSaveXml("ESM_PRI_0091GS.do", sParam);
	        ...
	        /*
	            for (var i = sheetObj.HeaderRows; sheetObj.RowCount > 0 && i <= sheetObj.LastRow; i++) {
	            	if (sheetObj.CellValue(i, "n1st_ord_ref") == "") {
	            		var n1stOrdRef;
	            		if (sheetObj.CellValue(i, "note_clss_cd") == "G") {
	            			n1stOrdRef = 1;
	            		} else if (sheetObj.CellValue(i, "note_clss_cd") == "S") {
	            			n1stOrdRef = 2;
	            		} else if (sheetObj.CellValue(i, "note_clss_cd") == "D") {
	            			n1stOrdRef = 3;
	            		} else if (sheetObj.CellValue(i, "note_clss_cd") == "O") {
	            			n1stOrdRef = 4;
	            		} else {
	            			n1stOrdRef = 99;
	            		}
			...
			case IBSEARCH: // parent sheet에서 조회
	            ...
	            var arrSurcharge = dialogArguments.getSurchargeList(5);
				var sCd = " |";
				var sNm = " |";
				...
				//sheetObj.ColumnSort("n1st_ord_ref|cmdt_note_seq|amdt_seq", "ASC", "ASC|ASC|ASC", true);
				sheetObj.ColumnSort("cmdt_note_seq|amdt_seq", "ASC", "ASC|ASC", true);
    ...
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case IBSAVE: // 저장
            ...
            for (var i = sheetObj.HeaderRows; sheetObj.RowCount > 0 && i <= sheetObj.LastRow; i++) {
            	if (sheetObj.RowStatus(i) == "D" || sheetObj.CellValue(i, "src_info_cd") == "AD"
            		|| sheetObj.CellValue(i, "amdt_seq") != formObj.amdt_seq.value) {
            		continue;
            	}
            	
            	if (sheetObj.CellValue(i, "note_clss_cd") == "S" && sheetObj.CellValue(i, "chg_cd") == "") {
    				ComShowCodeMessage("PRI00316", "Surcharge");
    				sheetObj.SelectCell(i, "chg_cd");
    				return false;
            	}
            	
            	if (sheetObj.CellValue(i, "note_clss_cd") == "D") {
            		checkDMT = true;
            	}
            	
            	if (sheetObj.CellValue(i, "note_clss_cd") == "C") {  //[CHM-201327107] CHSS 노트 항목 추가
            		checkCHSS = true;
            	}
            }
    ...
    function setLineEnable(sheetObj, idx) {
    	...
    	if (sheetObj.CellValue(idx, "n1st_cmnc_amdt_seq") == document.form.amdt_seq.value
    		...
    		&& sheetObj.CellValue(idx, "src_info_cd") != "AD") {
	        	sheetObj.CellEditable(idx, "note_clss_cd") = true;
	        	if (sheetObj.CellValue(idx, "note_clss_cd") == "S") {
	        		sheetObj.CellEditable(idx, "chg_cd") = true;
	        	} else {
	        		sheetObj.CellEditable(idx, "chg_cd") = false;
	        	}
	        	sheetObj.CellEditable(idx, "note_conv_mapg_id") = true;
	        	//sheetObj.CellEditable(idx, "note_conv_mapg_id_pop") = true;
	        	sheetObj.CellEditable(idx, "note_chg_tp_cd") = true;
	        	
	        	sheetObj.ColBackColor("note_ctnt") = sheetObj.EditableColor;
		} else {
        	sheetObj.CellEditable(idx, "note_clss_cd") = false;
        	sheetObj.CellEditable(idx, "chg_cd") = false;
        	sheetObj.CellEditable(idx, "note_conv_mapg_id") = false;
        	//sheetObj.CellEditable(idx, "note_conv_mapg_id_pop") = false;
        	sheetObj.CellEditable(idx, "note_chg_tp_cd") = false;
        	
        	sheetObj.ColBackColor("note_ctnt") = sheetObj.UnEditableColor;
		}
...
//    	}
    ...
    function getSheetXml(sheetNo) {
    	...
    	sArr[0] =  dialogArguments.getSheetXml(0);

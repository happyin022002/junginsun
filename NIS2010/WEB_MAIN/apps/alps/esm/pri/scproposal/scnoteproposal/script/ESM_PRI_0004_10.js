...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0004_10 : ESM_PRI_0004_10 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0004_10() {
  			...
  			if(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_clss_cd") == "D") {
				...
				ComOpenPopup('EES_CGM_1209.do?prop_no=' + formObj.prop_no.value, 1024, 580, '', '1,0,1,1,1,1,1', true);		
  				...
  				if(!ComIsNull(sheetObj.CellValue(Row, "note_conv_mapg_id"))) {
  					...
  					var sUrl = "/hanjin/ESM_PRI_0055.do?"+sParam;			
  					var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_0055", 1020, 605, true);
  		      
  					if (rtnVal != null) {
  						//CONVERSION 화면에서 선택ROW가 변동가능하기 때문에 FOR문절 사용
  						for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
  							if(sheetObj.CellValue(i, "note_seq") == rtnVal.master_seq 
  									&& sheetObj.CellValue(i, "note_ctnt_seq") == rtnVal.detail_seq
  									&& sheetObj.CellValue(i, "amdt_seq") == rtnVal.amdt_seq ) {
  								sheetObj.CellValue2(i, "note_conv_flg") = rtnVal.note_conv_flg;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_0057_13 : ESM_PRI_0057_13 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
...
//			  			  			  SORTENABLE, TOOLTIP, 	  ALLCHECK,   SAVESTATUS, FORMATFIX] 
					 ...
					 InitDataProperty(0, cnt++, dtData,   430, daLeft,	 false,	"blpl_tit_nm",	    false,  "", dfNone,	  0, false, false);
                     ...
                     InitDataProperty(0, cnt++, dtHidden, 80,  daCenter, false,	"src_info_cd",	    false, "", dfNone,	  0, false, false);
                     InitDataProperty(0, cnt++, dtData,   90,  daCenter, false, "src_info_dtl",     false, "", dfNone, 	  0, false, false);
                     InitDataProperty(0, cnt++, dtHidden, 80,  daCenter, false,	"prc_prog_sts_cd",  false, "", dfNone,	  0, false, false);
                     InitDataProperty(0, cnt++, dtData,   80,  daCenter, false, "prc_prog_sts_dtl", false, "", dfNone, 	  0, false, false);
                     InitDataProperty(0, cnt++ , dtData,  135, daLeft, false,	"acpt_usr_nm",	    false,	"",dfNone, 	  0, false, false);
...
//			  			  			  SORTENABLE, TOOLTIP, 	  ALLCHECK,   SAVESTATUS, FORMATFIX] 
					 ...
					 InitDataProperty(0, cnt++, dtData,   430, daLeftTop,false,	"blpl_ctnt",	    false,  "", dfNone,	  0, false, false);
                     InitDataProperty(0, cnt++, dtData,   70, daCenter, true,	"eff_dt",	    false, "", dfDateYmd, 0, false, false);
 					 InitDataProperty(0, cnt++, dtData,   70, daCenter, true,	"exp_dt",		    false, "", dfDateYmd, 0, false, false);
                     InitDataProperty(0, cnt++, dtHidden,  80,  daCenter, false,	"src_info_cd",	    false, "", dfNone,	  0, false, false);
                     InitDataProperty(0, cnt++, dtData,    90,  daCenter, false, "src_info_dtl",     false, "", dfNone, 	  0, false, false);
     ...
     */
  	function sheet1_OnSelectCell(sheetObj, OldRow, OldCol, NewRow, NewCol)  {
         if (OldRow != NewRow) {
             // Amend Row의 Highlight 색상을 다르게 표시
             changeSelectBackColor(sheetObj, sheetObj.CellValue(NewRow, "n1st_cmnc_amdt_seq"), document.form.amdt_seq.value);
         }
    ...
    /**
     * Master Sheet Row가 변경될 경우 Detail을 조회하는 function <br>
     * <br><b>Example :</b>
     * <pre>
     *		doRowChange(sheetM, sheetD, OldRow, NewRow, OldCol, NewCol);
     * </pre>
     * @param {ibsheet} sheetM 필수 Master IBSheet Object
     * @param {ibsheet} sheetD 필수 Detail IBSheet Object
     * @param {int} OldRow 필수 이전에 선택된 셀의 Row Index
     * @param {int} OldCol 필수 이전에 선택된 셀의 Column Index
     * @param {int} NewRow 필수 현재 선택된 셀의 Row Index
     * @param {int} NewCol 필수 현재 선택된 셀의 Column Index     * 
	...
	function doRowChange(sheetM, sheetD, OldRow, NewRow, OldCol, NewCol) {
		...
		if (OldRow != NewRow) {			
			formObj.blpl_seq.value = sheetM.CellValue(NewRow, "blpl_seq");
         ...
         try{
             switch(sAction) {			         
	 			case IBSEARCH: // 조회
	 				...
	 				var sXml = sheetObj.GetSearchXml("ESM_PRI_0057_13GS.do" , FormQueryString(formObj));

...
/**
 * @extends 
 * @class ESM_PRI_0130_01 : ESM_PRI_0130_01 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_0130_01() {
...
function processButtonClick(){
	...
	try {
        ...
        switch(srcName) {
    	    ...
    	    case "btn_BLinquiry":
    	    	if(sheetObject.RowCount > 0){
    	    		...
    	    		if(sheetObject.CellValue(selcRow, "bkg_count")*1 <= 10000){
						var rep_chg_cd = sheetObject.CellValue(selcRow, "rep_chg_cd");
						var mdtr_cd = sheetObject.CellValue(selcRow, "mdtr_cd");
						var chg_cd = sheetObject.CellValue(selcRow, "chg_cd");
						var cust_clss = sheetObject.CellValue(selcRow, "cust_clss");
						var rvis_cntr_cust_tp_cd = sheetObject.CellValue(selcRow, "rvis_cntr_cust_tp_cd");
						var ctrt_cust_cnt_cd = sheetObject.CellValue(selcRow, "ctrt_cust_cnt_cd");
						var ctrt_cust_seq = sheetObject.CellValue(selcRow, "ctrt_cust_seq");
						var cust_grp_id = sheetObject.CellValue(selcRow, "cust_grp_id");
						var svc_scp_cd = sheetObject.CellValue(selcRow, "svc_scp_cd");
						var rhq_cd = sheetObject.CellValue(selcRow, "rhq_cd");
						var ctrt_ofc_cd = sheetObject.CellValue(selcRow, "ctrt_ofc_cd");
	        	    	...
	        	    	var sParam = "rep_chg_cd="+rep_chg_cd+"&mdtr_cd="+mdtr_cd+"&chg_cd="+chg_cd+"&cust_clss="+cust_clss+"&rvis_cntr_cust_tp_cd="+rvis_cntr_cust_tp_cd
	        	    				+"&ctrt_cust_cnt_cd="+ctrt_cust_cnt_cd+"&ctrt_cust_seq="+ctrt_cust_seq+"&cust_grp_id="+cust_grp_id+"&svc_scp_cd="+svc_scp_cd+"&rhq_cd="+rhq_cd
	        	    				+"&ctrt_ofc_cd="+ctrt_ofc_cd+"&per_cd="+per_cd+"&f_year="+f_year+"&f_fm_wk="+f_fm_wk+"&f_to_wk="+f_to_wk
	        	    				+"&f_sls_mon="+f_sls_mon+"&f_fm_mon="+f_fm_mon+"&f_to_mon="+f_to_mon+"&start_dt="+start_dt+"&end_dt="+end_dt+"&ui_id=ESM_PRI_0130_01";
    	    ...
    	    case "btn_cust_grp_id":
    	    	var sUrl = "/hanjin/ESM_PRI_4014.do?is_popup=true&ui_id=ESM_PRI_0130_01&cust_tp=G";
				var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_4014", 640, 500, true);
				if (rtnVal != null && rtnVal.length > 0) {
					if(rtnVal.length == "1"){
						if( formObject.cust_grp_id_multi.value != ""){
							formObject.cust_grp_id_multi.value = formObject.cust_grp_id_multi.value + ", " + rtnVal[0].cust_grp_id;
						}else{
							formObject.cust_grp_id_multi.value = rtnVal[0].cust_grp_id;
						}
					}else{
						for (var i = 0; i < rtnVal.length; i++) {
							if( formObject.cust_grp_id_multi.value != ""){
								formObject.cust_grp_id_multi.value = formObject.cust_grp_id_multi.value + ", " + rtnVal[i].cust_grp_id;
							}else{
								formObject.cust_grp_id_multi.value = rtnVal[i].cust_grp_id;
							}
						}							
					}
				}
    	    ...
    	    case "btn_cust_cd":
    	    	var sUrl = "/hanjin/ESM_PRI_4014.do?is_popup=true&ui_id=ESM_PRI_0130_01&cust_tp=C";
				var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_4014", 640, 500, true);
				if (rtnVal != null && rtnVal.length > 0) {
					if(rtnVal.length == "1"){
						if( formObject.cust_cd_multi.value != ""){
							formObject.cust_cd_multi.value = formObject.cust_cd_multi.value + ", " + rtnVal[0].cust_cd;
						}else{
							formObject.cust_cd_multi.value = rtnVal[0].cust_cd;
						}
					}else{
						for (var i = 0; i < rtnVal.length; i++) {
							if( formObject.cust_cd_multi.value != ""){
								formObject.cust_cd_multi.value = formObject.cust_cd_multi.value + ", " + rtnVal[i].cust_cd;
							}else{
								formObject.cust_cd_multi.value = rtnVal[i].cust_cd;
							}
						}							
					}
				}
    	    ...
    	    case "btn_del_cust_grp_id":
    	    	formObject.cust_grp_id_multi.value = "";
    	    	formObject.cust_grp_id.value = "";
				break; 
				
    	    case "btn_del_cust_cd":
    	    	formObject.cust_cd_multi.value = "";
    ...
    switch(sheetNo) {
        case 1:      //sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "FLAG|Rep.\nCharge|Mandatory\nRating|Charge\nCode|Customer\nClassification|Customer\nType|rvis_cntr_cust_tp_cd|Customer\nGroup Code|Customer\nCode|ctrt_cust_cnt_cd|ctrt_cust_seq|Scope|RHQ|C.OFC|Per|Tariff Total|Rating Total|Collection Ratio(%)|BKG Count";
				...
				InitDataProperty(0, cnt++ , dtData,			70,		daCenter,	true,	"rep_chg_cd",               false,  "",   dfNone,   0   );
				InitDataProperty(0, cnt++ , dtData,			80,		daCenter,	true,	"mdtr_cd",                  false,  "",   dfNone,   0  );
				InitDataProperty(0, cnt++ , dtData,			70,	 	daCenter,	true,	"chg_cd",                   false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtData,			90,		daCenter,	true,	"cust_clss",                false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtData,			80,		daCenter,	true,	"cust_tp_cd",               false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		90,		daCenter,	true,	"rvis_cntr_cust_tp_cd",     false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		90,		daCenter,	true,	"cust_grp_id",              false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		80,		daCenter,	true,	"cust_cd",                  false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		80,		daCenter,	true,	"ctrt_cust_cnt_cd",         false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		80,		daCenter,	true,	"ctrt_cust_seq",            false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		70,		daCenter,	true,	"svc_scp_cd",               false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		80,		daCenter,	true,	"rhq_cd",                   false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		80,		daCenter,	true,	"ctrt_ofc_cd",              false,  "",   dfNone,   0);
				InitDataProperty(0, cnt++ , dtHidden,		60,		daCenter,	true,	"per_cd",                   false,  "",   dfNone,   0);
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg){
    ...
    sheetObj.ColHidden("cust_grp_id") = true;
    sheetObj.ColHidden("cust_cd") = true;
    ...
    for(var i=headCnt; i<=sheetObj.LastRow; i++){
		if(sheetObj.CellValue(i, "cust_grp_id") != "" ){
			sheetObj.ColHidden("cust_grp_id") = false;
		}
		if(sheetObj.CellValue(i, "cust_cd") != "" ){
			sheetObj.ColHidden("cust_cd") = false;
		}
		if(sheetObj.CellValue(i, "svc_scp_cd") != "" ){
...
function rep_chg_cd_OnChange() {
	var formObj = document.form;
	if(formObj.rep_chg_cd.text == ""){
		formObj.chg_cd_multi.RemoveAll();
	}else{	
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	switch(sAction) {
		...
		case IBCLEAR: 
			sheetObj.RemoveAll();
			formObj.reset();
			formObj.rep_chg_cd.text = "";
			formObj.chg_cd_multi.RemoveAll();
			formObj.cust_tp_cd.text = "";
			formObj.rhq_cd.text = "";
        ...
        case IBCREATE:      // File DownLoad 상태조회
        	...
        	try {
		        ...
		        var rep_chg_cd = sheetObj.CellValue(selcRow, "rep_chg_cd");
				var mdtr_cd = sheetObj.CellValue(selcRow, "mdtr_cd");
				var chg_cd = sheetObj.CellValue(selcRow, "chg_cd");
				var cust_clss = sheetObj.CellValue(selcRow, "cust_clss");
				var rvis_cntr_cust_tp_cd = sheetObj.CellValue(selcRow, "rvis_cntr_cust_tp_cd");
				var ctrt_cust_cnt_cd = sheetObj.CellValue(selcRow, "ctrt_cust_cnt_cd");
				var ctrt_cust_seq = sheetObj.CellValue(selcRow, "ctrt_cust_seq");
				var cust_grp_id = sheetObj.CellValue(selcRow, "cust_grp_id");
				var svc_scp_cd = sheetObj.CellValue(selcRow, "svc_scp_cd");
				var rhq_cd = sheetObj.CellValue(selcRow, "rhq_cd");
				var ctrt_ofc_cd = sheetObj.CellValue(selcRow, "ctrt_ofc_cd");
		    	...
		    	var sParam = "f_cmd="+f_cmd+"&jb_id="+jb_id+"&f_excel="+f_excel+"&rep_chg_cd="+rep_chg_cd+"&mdtr_cd="+mdtr_cd+"&chg_cd="+chg_cd+"&cust_clss="+cust_clss+"&rvis_cntr_cust_tp_cd="+rvis_cntr_cust_tp_cd
		    				+"&ctrt_cust_cnt_cd="+ctrt_cust_cnt_cd+"&ctrt_cust_seq="+ctrt_cust_seq+"&cust_grp_id="+cust_grp_id+"&svc_scp_cd="+svc_scp_cd+"&rhq_cd="+rhq_cd
		    				+"&ctrt_ofc_cd="+ctrt_ofc_cd+"&per_cd="+per_cd+"&f_year="+f_year+"&f_fm_wk="+f_fm_wk+"&f_to_wk="+f_to_wk
		    				+"&f_sls_mon="+f_sls_mon+"&f_fm_mon="+f_fm_mon+"&f_to_mon="+f_to_mon+"&start_dt="+start_dt+"&end_dt="+end_dt+"&ui_id=ESM_PRI_0130_01";
		...
		case SEARCH02:      //입력한 C.OFC 유효성 검사
...
function validateForm(sheetObj,formObj,sAction){
	 with(formObj){
		...
		if(rep_chg_cd.text == "" ){
		    ComShowMessage(ComGetMsg("PRI00316", "Rep. Charge"));
		    formObj.rep_chg_cd.focus();
		    return false;
		}
...
function initIBComboItem() {
    ...
    ComPriTextCode2ComboItem(custTpCdComboValue,   custTpCdComboText,   getComboObject(comboObjects, 'cust_tp_cd'), "|", "\t" );
    document.form.cust_tp_cd.text = "ALL";
}
...
function getOfc(rowArray) {
	...
	if( formObj.ctrt_ofc_cd_multi.value != ""){
		formObj.ctrt_ofc_cd_multi.value = formObj.ctrt_ofc_cd_multi.value + "," + colArray[3];
	} else{
		formObj.ctrt_ofc_cd_multi.value = colArray[3];
	}
	formObj.ctrt_ofc_cd_multi.focus();
}
    
function chkOfcCdValid(){
	var formObj = document.form;
	if(formObj.ctrt_ofc_cd_multi.value != ""){
		formObj.ofc_cd.value = formObj.ctrt_ofc_cd_multi.value;
...
function setParam(formObj){
	...
	if(formObj.cust_grp_id_multi.value != ""){
		var cust_grp_id_arr = formObj.cust_grp_id_multi.value.split(",");
		formObj.cust_grp_id.value = "'"+(ComReplaceStr(cust_grp_id_arr.toString(), ",", "', '"))+"'";
	}
	if(formObj.cust_cd_multi.value != ""){
		var cust_cd_arr = formObj.cust_cd_multi.value.split(",");
		formObj.cust_cd.value = "'"+(ComReplaceStr(cust_cd_arr.toString(), ",", "', '"))+"'";
	}
	...
	if(formObj.ctrt_ofc_cd_multi.value != ""){
		var ctrt_ofc_cd_arr = formObj.ctrt_ofc_cd_multi.value.split(",");
		formObj.ctrt_ofc_cd.value = "'"+(ComReplaceStr(ctrt_ofc_cd_arr.toString(), ",", "', '"))+"'";
	}
}
...
function getBackEndJobStatus() {
    ...
    try {
    	if (formObj.f_excel.value == "Y") {
    		...
    		formObj.f_cmd.value = SEARCH;
            var sXml = sheetObj.GetSearchXml("ESM_PRI_0130_01GS.do", FormQueryString(formObj));
            var jobState = ComGetEtcData(sXml, "jb_sts_flg");
            if (jobState == "3") {

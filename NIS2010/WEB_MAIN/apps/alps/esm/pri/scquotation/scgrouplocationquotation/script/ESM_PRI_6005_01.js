...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_6005_01 : ESM_PRI_6005_01 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_6005_01() {
		...
		switch (sheetID) {
			case "sheet1":
				with (sheetObj) {
					...
					var HeadTitle = "|Sel.|Seq.|Service Scope|Gline Seq.|Group Location Seq.|Group Code|Description|src_info_cd|Group Code Backup";
...
//					MessageText("UserMsg17") = " " + ComGetMsg("PRI00317");
			...
			case "sheet2":
				with (sheetObj) {
					...
					var HeadTitle = "|Sel.|Seq.|Service Scope|Gline Seq.|Group Location Seq.|Group Location Detail Seq.|Code|Description|src_info_cd|Source";
					...
					InitDataValid(0, "loc_nm", vtEngUpOther, "0123456789");        // 영문대문자,숫자만 입력 
	...
	function sheet1_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colname == "prc_grp_loc_cd") {
			if (Value.length != 4) {
				...
				var sXml = sheetObjects[1].GetSearchXml("ESM_PRI_6005_01GS.do", sParam);
    ...
    */  	
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "loc_cd") {
			var sUrl = "/hanjin/ESM_PRI_4026.do?group_cmd=" + PRI_SG + "&location_cmd=L&svc_scp_cd=" + formObj.svc_scp_cd.value;
		...
		switch (sAction) {
			case IBSEARCH: // 조회
				if (validateForm(sheetObj,document.form,sAction)) {
					...
					try {
						...
						sheetObj.DoSearch("ESM_PRI_6005_01GS.do" , FormQueryString(formObj));
						ComOpenWait(false);
			...
			case IBSAVE: // 저장
				...
				try {
					...
					var sXml = sheetObj.GetSaveXml("ESM_PRI_6005_01GS.do", sParam);
...
//					sheetObj.CellValue(idx, "grp_loc_seq") = "";
...
//					sheetObj.CellValue(idx, "grp_loc_dtl_seq") = "";

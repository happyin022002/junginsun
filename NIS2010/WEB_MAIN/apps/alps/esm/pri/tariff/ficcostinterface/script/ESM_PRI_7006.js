...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			var prefix = "sheet3_";
			var HeadTitle1 = "||Seq.|Scope|Country|Country|Guideline Tariff|Guideline Tariff|Guideline Tariff|Guideline Tariff|Guideline Tariff|Cost Tariff|Cost Tariff|Cost Tariff";
			var HeadTitle2 = "||Seq.|Scope|Code|Description|No.|AMD\nNo.|Creation\nDate|Status|Interfaced\nCost Tariff No|No.|Confirmed\nDate|Count";
			...
			InitDataProperty(0, cnt++, dtDataSeq, 		30, 	daCenter, 	true, prefix + "chk");
			InitDataProperty(0, cnt++, dtData, 			40, 	daCenter, 	true, prefix + "svc_scp_cd", 		false, "", dfNone, 0, false, false, 3);
			InitDataProperty(0, cnt++, dtData, 			40, 	daCenter, 	true, prefix + "cnt_cd", 			false, "", dfNone, 0, false, false, 50);
			InitDataProperty(0, cnt++, dtData, 			0, 	daLeft, 	true, prefix + "cnt_nm", 			false, "", dfNone, 0, false, false, 50);
			InitDataProperty(0, cnt++, dtData, 			100,	daCenter, 	true, prefix + "ihc_trf_no", 		false, "", dfUserFormat, 0, false, false, 9);
			InitDataProperty(0, cnt++, dtData, 			40, 	daRight, 	true, prefix + "amdt_seq", 			false, "", dfNone, 0, false, false, 3);
			...
			InitDataProperty(0, cnt++, dtData, 			85, 	daCenter, 	true, prefix + "ihc_cost_trf_no",	false, "", dfNone, 0, false, false, 1);
			InitDataProperty(0, cnt++, dtHidden, 		80,	daCenter, 	true, prefix + "cost_trf_no", 		false, "", dfNone, 0, false, false, 10);
			InitDataProperty(0, cnt++, dtData, 			0, 	daCenter, 	true, prefix + "eff_fm_dt", 		false, "", dfUserFormat, 0, false, false, 10);
			InitDataProperty(0, cnt++, dtData, 			40, 	daRight, 	true, prefix + "row_no", 			false, "", dfInteger, 0, false, false, 3);
			
			sheetObj.InitUserFormat(0, prefix +"eff_fm_dt", "####-##-##", "-");
			sheetObj.InitUserFormat(0, prefix +'upd_dt', "####-##-##", "-");
	...
	case "sheet3":    // validation 용 sheet
		with (sheetObj) {
			...
			var HeadTitle1 = "ibflag|svc_scp_cd|cost_cnt_cd|eff_dt|rhq_cd|org_dest_tp_cd";
			...
			InitDataProperty(0, cnt++, dtData,	 		200, 		daCenter, 	false, "cost_cnt_cd", 	false, "", dfNone, 0, true, true);
	...
	case "sheet4":    // save 용 sheet
		with (sheetObj) {
			...
			var HeadTitle1 = "ibflag|svc_scp_cd|cost_cnt_cd|rhq_cd|eff_dt|org_dest_tp_cd|ori_svc_scp_cd|ori_ihc_trf_no|ori_amdt_seq";
			...
			InitDataProperty(0, cnt++, dtStatus, 		30, 	daCenter,	false, "ibflag");
			InitDataProperty(0, cnt++, dtData, 			50, 		daCenter, 	false, "svc_scp_cd", 		false, "", dfNone, 0, 		false, true, 3);
			InitDataProperty(0, cnt++, dtData,	 		50, 		daCenter, 	false, "cost_cnt_cd", 	false, "", dfNone, 0, false, true);
			...
			InitDataProperty(0, cnt++, dtData, 			150, 		daCenter, 	false, "ori_ihc_trf_no", 		false, "", dfNone, 0, 		false, true);
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		try {
				...
				case IBSAVE:
					...
					var sXml = sheetObjects[3].GetSaveXml("ESM_PRI_7006GS.do", sParam);
...
function validateForm(sheetObj, formObj, sAction) {
    ...
    case IBSAVE:       
        ...
        // validation 1 - CNT_CD가 COPY 하려는 SCOPE에 있는가?
        formObj.f_cmd.value = SEARCH02;
        for(var i=1; i <= sheetObjects[1].RowCount("I"); i++){
            ...
            sheetObjects[2].CellValue(newRow, "cost_cnt_cd") = formObj.cntCd.value;	
        ...
        var sXml = sheetObjects[2].GetSearchXml("ESM_PRI_7006GS.do", sParam);    
        sheetObjects[2].LoadSearchXml(sXml);	
      
 		// validation 2 - check exist IHC tariff's status
        formObj.f_cmd.value = SEARCH03;
        for(var i = sheetObjects[2].HeaderRows; i <= sheetObjects[2].LastRow ; i++){
        	if(sheetObjects[2].CellValue(i, "cost_cnt_cd") !=""){
        		sheetObjects[2].CellValue(i, "org_dest_tp_cd") = formObj.org_dest_tp_cd.value;
        	}
        }
        var sParam = FormQueryString(formObj) + "&" + sheetObjects[2].GetSaveString(false);      
        var sXml = sheetObj.GetSearchXml("ESM_PRI_7006GS.do", sParam);    
        var arrData  = ComPriXml2Array(sXml, "cnt_nm|cost_cnt_cd|ihc_trf_no");   
        if(arrData != undefined){
        	var copycheck = "";
            for(var i = 0; i < arrData.length; i++){
            	copycheck += parseInt(i)+1 +") " + arrData[i][0] + ", " + arrData[i][1] + ", G/L tariff no : " + arrData[i][2] + "\n" ;
            }
            ComShowCodeMessage('PRI07027',copycheck); 
            return false;
        }
   
 		// validation 3 - check exist IHC tariff's effective date
        ...
        if(arrData != undefined){
            ...
            ComShowCodeMessage('PRI07028',copycheck); 
            return false;
        } 
        ...
        if(sheetObjects[2].RowCount != 0){
	        for(var i = sheetObjects[2].HeaderRows; i <= sheetObjects[2].LastRow ; i++){
	        	if(sheetObjects[2].CellValue(i, "cost_cnt_cd") !=""){
		        	var arrRow = sheetObjects[2].CellValue(i, "cost_cnt_cd").split(",");
		        	if(arrRow.length > 0){
		        		for(var j=0 ; j < arrRow.length ; j++){
		        			...
		        			sheetObjects[3].CellValue(newRow, "cost_cnt_cd") = arrRow[j];		        			
		                	...
		                	for(var k= sheetObjects[0].HeaderRows; k <= sheetObjects[0].LastRow ; k++){
		                		if(sheetObjects[0].CellValue(k, prefix+"cnt_cd") == sheetObjects[3].CellValue(newRow, "cost_cnt_cd")){
		                			sheetObjects[3].CellValue(newRow, "ori_svc_scp_cd") = sheetObjects[0].CellValue(k, prefix+"svc_scp_cd");
		                			sheetObjects[3].CellValue(newRow, "ori_ihc_trf_no") = sheetObjects[0].CellValue(k, prefix+"ihc_trf_no");

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_3510 : ESM_PRI_3510 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_3510() {
    ...
    */
     ...
     */
   		...
   		switch (sAction) {
	   		...
	   		case IBSAVE: // 저장
			 	...
			 	var befPubDt = sheetObj.CellValue(sRow, "bef_pub_dt");			 	
	   			var ruleTpCd = sheetObj.CellValue(sRow, "trf_rule_amdt_tp_cd");
	   			var isSuperUser = document.form.isSuperUser.value;
	   			...
	   			if(pubDt < curDate && isSuperUser != "true") {
	   			...
	   			if(ruleTpCd == "A" || ruleTpCd == "I") {
	   				if(ret > -30 && isSuperUser != "true") {
	   					ComShowCodeMessage("PRI06006");
	   					return false;   					
	   				}
	   			}
	   			//[New Publication Date] must be earlier than Expiration date. 
	   			if(expDt != "" && pubDt > expDt && isSuperUser == "true"){
 	...
 	function sheet1_OnSearchEnd(sheetObj, errMsg){
 		...
 		formObj.rule_no.value 		= sheetObj.CellValue(sheetObj.SelectRow, "trf_rule_no");

		sheetObj.Cellvalue2(sheetObj.SelectRow, "trf_rule_sts_cd") = "F"; //Publish

...
function processButtonClick() {
	...
	try {
        ...
        case "btn_ihc_ovr_cgo": 			
        	...
        	document.getElementById("t" + (tabIndex + 1) + "frame").contentWindow.open_ihc_ovr_cgo();
            break;  
		}
	} catch (e) {
...
function selectCountry(){
	...
	ComOpenPopup("/hanjin/ESM_PRI_7027.do?" + parameter,565, 480, 'getCountry', "1,0,1,1,1", true);
}
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			var HeadTitle1 = "ibflag|ORG_DEST_TP_CD|SVC_SCP_CD|ACC_DT|PNT_LOC_CD|BSE_PORT_LOC_CD|CNT_CD|ZIP_CD";
			...
			InitDataProperty(0, cnt++, dtData, 			80, daCenter, 	true, "cnt_cd",	 				false, "", dfNone, 0, false, false);		
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	try {
		...
		case IBSEARCH:
            ...
            sheetObjects[0].CellValue(iRow, 'cnt_cd') = formObj.cnt_cd.value ;
            sheetObjects[0].CellValue(iRow, 'zip_cd') = formObj.zip_cd.value ;
	        clearAllTabPages();         
...
function loadTabPage(tabIndex) {
	...
	if (objTabWindow.location.href == "" || objTabWindow.location.href == "about:blank") {
		...
		switch (tabIndex) {
		case 0:
			sUrl = "ESM_PRI_7004_01.do";
			break;
		case 1:
			sUrl = "ESM_PRI_7004_02.do";
			break;
		}
	...
	if(sheetObjects[0].RowCount > 0){
	    ...
	    var sCntCd = sheetObjects[0].CellValue(iRow, 'cnt_cd'); 
	    var sZipCd = sheetObjects[0].CellValue(iRow, 'zip_cd'); 
	}

    if (sSvcScpCd != null && sSvcScpCd != "" && sOrgDestTpCd != null && sOrgDestTpCd != "" && sAccDt != null && sAccDt != "") {	
    		document.getElementById("t" + (tabIndex + 1) + "frame").contentWindow.tabLoadSheet(sSvcScpCd, sOrgDestTpCd, sAccDt, sPntLocCd, sBsePortLocCd, sCntCd, sZipCd );
    	}       
}

/**
 * Tab에 조회된 결과 중에 선택한 row의 SPCL CGO Tariff의 over weight service code에 따라
 *  OverWeightCgo 버튼의 색을 파란색으로 변경한다. <br>
 * <br><b>Example :</b>
 * @param  sOvrWgtCgoSvcFlg
 * @return 없음
 * @author 전윤주
 * @version 2013.07.30
 */ 

function changeBtnOverWeightCgoColor(sOvrWgtCgoSvcFlg) {	
	if(sOvrWgtCgoSvcFlg == "Y" ){
		document.getElementById("btn_ihc_ovr_cgo").style.color = "red";
	} else {

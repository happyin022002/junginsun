    ...
    */
    function loadPage() {
        ...
        if(formObj.in_svc_scp_cd.value != ""){
        	...
        	comboObjects[1].Code = formObj.in_cnt_cd.value ;  	
        	comboObjects[2].Code = formObj.in_svc_scp_cd.value ;
        	doActionIBSheet(sheetObjects[0],document.form,IBSEARCH);
        }else{
...
//       					document.form.cost_cnt_cd.value = document.form.cost_cnt_cd.oldvalue;
//    					doActionIBSheet(sheetObjects[0],document.form,MULTI08);
//    				}
//       				
//       				if (rslt) {
//       					document.form.cost_cnt_cd.value = srcValue;
//       				}
//    			}
//
        ...
        try{
            ...
            case IBSEARCH: 		//retrieve form   	               
                if (!validateForm(sheetObjects[0],document.form,sAction)) {
	 			...
	 			var sXml = sheetObj.GetSearchXml("ESM_PRI_7031GS.do", FormQueryString(formObj));	 	
	 			...
	 			// checking tariff no.						
 		 		if ( sheetObjects[0].CellValue( sheetObjects[0].SelectRow , "ihc_trf_no")  != ""){		
 		 			...
 		 			comboObjects[3].Code2 = sheetObjects[0].CellValue( sheetObjects[0].SelectRow,"ihc_trf_amdt_tp_cd");
 		 			...
 		 			formObj.cfm_ofc_cd.value = sheetObjects[0].CellValue( sheetObjects[0].SelectRow,"cfm_ofc_cd");
 		 			formObj.cre_usr_id.value = sheetObjects[0].CellValue( sheetObjects[0].SelectRow,"cre_usr_id");
 		 			...
 		 			//RHQ_CD가 없는 interface data를 위해서 rhq_cd 넣어주기 
 		 			if(sheetObjects[0].CellValue( sheetObjects[0].SelectRow , "rhq_cd") == "" ){
 		 				sheetObjects[0].CellValue2( sheetObjects[0].SelectRow , "rhq_cd") = formObj.cd.value;
 		 				sheetObjects[0].CellValue2( sheetObjects[0].SelectRow , "ibflag") = "R";
 		 			}
 	                ...
 	                clearAllTabPages();
 		 			...
 		 			document.form.svc_scp_cd.focus();	 			
 		 		}
                break;
                
            case SEARCH01:  // Country combo    
        		var formObj = document.form;
            ...
            case IBSAVE: // Save                  
                ...
                sParam = sParam + "&" + sParamSheet;
	 			...
	 			var sXml = sheetObjects[0].DoSave("ESM_PRI_7031GS.do", sParam, -1, false );  		 		
        		ComOpenWait(false); //->waiting->End
		     	if(sXml){
		     		ComPriSaveCompleted();
		     	}   
            ...
            case MULTI01: // Amend
                ...
                sParam = sParam + "&" + sParamSheet;
                var sXml = sheetObj.GetSearchXml("ESM_PRI_7031GS.do", sParam);    
            ...
            case MULTI02: // Cancel
                ...
                var sParam = "&f_cmd=" + formObj.f_cmd.value; 
                sParam = sParam + "&" + sParamSheet;
                var sXml = sheetObj.GetSaveXml("ESM_PRI_7031GS.do", sParam);                
            ...
            case MULTI03: // Confirm
                ...
                var effDt = formObj.eff_dt.value;
			 	var pubDt = ComGetNowInfo();	
	   			var amdTpCd = comboObjects[3].Code;
                ...
                //TAW, TAE, ASW, ASE Scope일 경우에는 버튼이 Publish 로 변경되므로 확인 메시지를 변경해준다.
	   			...
	   			var SvcScpCd = comboObjects[2].Code;
                if (SvcScpCd == "TAW" || SvcScpCd == "TAE" || SvcScpCd == "ASW" || SvcScpCd == "ASE"){
                	if(!ComShowCodeConfirm("PRI06005", "Inland Add-on tariff")) {
	 			 		return false;
	 			 	}
                } else {
        ...
        case "sheet1":
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
				...
				var HeadTitle1 = "ibflag|Seq|SVC_SCP_CD|IHC_TRF_NO|AMDT_SEQ|IHC_TRF_AMDT_TP_CD|EFF_DT|CRE_DT|CFM_DT|EXP_DT|COST_TRF_NO|SPCL_COUNT|ORG_DEST_TP_CD|CFM_OFC_CD|" +
									 "CRE_USR_ID|FIC_PROP_STS_CD|FIC_PROP_STS_NM|LOCL_CURR_CD|COST_CNT_CD|RHQ_CD";
                ...
                InitDataProperty(0, cnt++, 	dtData, 				70, 		daCenter, 	true, 	"cfm_ofc_cd", false, "", dfNone, 0, false, false);       
    ...
    */
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case MULTI03: // Confirm
            ...
            // validation check 1 - total value is 0
            var nowMode = "";
 			var re = document.getElementById("t" + (0 + 1) + "frame").contentWindow.checkNowMode();
            nowMode = re ;
            ...
            sParam = sParam + "&" + sParamSheet;
 			var sXml = sheetObj.GetSearchXml("ESM_PRI_7031GS.do", sParam);  	
 	        var arrData  = ComPriXml2Array(sXml, "usa_cost_trf_svc_mod_nm|total_count");   
        	var check = "";
            if(arrData != undefined){
	            for(var i = 0; i < arrData.length; i++){
	            	if(arrData[i][1] != 0){
	            		if(arrData[i][0] == nowMode ){
	                        for (var i = 0; i <= tabObjects.length; i++) {
            ...
            if(check != ""){	
            	...
            	ComShowCodeMessage('PRI07042',noticemsg);    
            	return false;	
            }
       
            // validation check 2 - rate tariff tuning or not
            ...
            if(check != ""){	
            	ComShowCodeMessage('PRI07015',check); 
            	return false;	
            }
            break;
        }
     ...
     */ 
  	...
  	function setConfirmText(){
  		var formObj = document.form;
  		var confirmTxt = comboObjects[2].Code;
   ...
   function getCountry(aryPopupData, Row, Col, SheetIdx) {
	...
	function loadTabPage(tabIndex) {
		...
		if (objTabWindow.location.href == "" || objTabWindow.location.href == "about:blank") {
			...
			switch (tabIndex) {
			case 0:
				sUrl = "ESM_PRI_7031_01.do";
				break;
			case 1:
				sUrl = "ESM_PRI_7031_02.do";
				break;
			}
        ...
        var sCostCntCd = sheetObj1.CellValue(sRow, "cost_cnt_cd");    
        var sFicPropStsCd = sheetObj1.CellValue(sRow, "fic_prop_sts_cd");    
        ...
        if (sRow != -1 && sSvcScpCd != null && sSvcScpCd != "" && sAmdtSeq != null && sAmdtSeq != "" && sSvcScpCd != null && sSvcScpCd != "" 
        	...
        	&& sOrgDestTpCd != null && sOrgDestTpCd != "" && sLoclCurrCd != null && sLoclCurrCd != "") {	
        		document.getElementById("t" + (tabIndex + 1) + "frame").contentWindow.tabLoadSheet(sIhcTrfNo, sAmdtSeq, sSvcScpCd, sFicPropStsCd, sEffDt, sCostCntCd, sOrgDestTpCd, sLoclCurrCd, sFicPropStsCd);
        	}       
	}
	...
	/**
   	...
   	function ihc_trf_amdt_tp_cd_OnChange(comboObj, code, text) {	   		
   		if (text != null) { 
   			sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "ihc_trf_amdt_tp_cd") = comboObjects[3].Code;
   		}
   	}
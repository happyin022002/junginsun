...
*@FileName : ESM_PRI_7001_01.js
*@FileTitle : IHC Tariff Creation & Amendment - Dry tab
*Open Issues :
*Change history :
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/ 
   /**
	...
	var apply_20_col = "gline_20ft_frt_rt_amt";
	var apply_40_col = "gline_40ft_frt_rt_amt";
	var apply_45_col = "gline_45ft_frt_rt_amt";		// 45' Cost 추가
        ...
        try {
            ...
            case "btn_load_excel": 
				var sParam = "svc_scp_cd=" + formObj.svc_scp_cd.value 
								...
								+ "&locl_curr_cd=" +	formObj.locl_curr_cd.value 
								+ "&rhq_cd=" + formObj.rhq_cd.value ;
				
            	var rtnVal = ComOpenPopup("/hanjin/ESM_PRI_7014.do?"+ sParam,	950, 600, '', "1,0,1,1,1,1,1", true);
	            if (rtnVal) { 
					doActionIBSheet(sheetObjects[0],document.form,SEARCH01);
	            }
                break;  
    ...
    */
    function loadPage() {
		...
		sheetObjects[0].ColHidden("gline_locl_curr_20ft_amt") = true;
		sheetObjects[0].ColHidden("gline_locl_curr_40ft_amt") = true;
		sheetObjects[0].ColHidden("gline_locl_curr_45ft_amt") = true;	// 45' Cost 추가
		sheetObjects[0].ColHidden("diff_locl_20ft") = true;
		sheetObjects[0].ColHidden("diff_locl_40ft") = true;
		sheetObjects[0].ColHidden("diff_locl_45ft") = true;		// 45' Cost 추가
    	parent.loadTabPage(); 
    }
...
//        axon_event.addListenerFormat('mousemove', 'obj_onmousemove', document.form);
	...
	function obj_click() {                                                                                                                                                                                                                                                                                                                                                                                                                                          
		...
		// Application Type 선택                                                                                                                                                                                                                                                                                                                                                                                                                               				
		if (event.srcElement.name == "node_code_flg") {                    
			if(event.srcElement.checked == true){
				sheetObjects[0].ColHidden("pnt_loc_cd") = true;
				sheetObjects[0].ColHidden("pnt_nod_cd") = false;
				
				sheetObjects[0].ColHidden("hub_loc_cd") = true;
				sheetObjects[0].ColHidden("hub_nod_cd") = false;
				
				sheetObjects[0].ColHidden("bse_port_loc_cd") = true;
				sheetObjects[0].ColHidden("bse_port_nod_cd") = false;
			}else{
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        try{
            ...
            case IBSEARCH: 		//retrieve form
				...
				sheetObjects[0].ColHidden("gline_20ft_frt_rt_amt") = false;
				sheetObjects[0].ColHidden("gline_40ft_frt_rt_amt") = false;
				sheetObjects[0].ColHidden("gline_45ft_frt_rt_amt") = false;	// 45' Cost 추가
				sheetObjects[0].ColHidden("gline_locl_curr_20ft_amt") = true;
				sheetObjects[0].ColHidden("gline_locl_curr_40ft_amt") = true;
				sheetObjects[0].ColHidden("gline_locl_curr_45ft_amt") = true;	// 45' Cost 추가
				sheetObjects[0].ColHidden("cost_20ft_frt_rt_amt") = false;
				sheetObjects[0].ColHidden("cost_40ft_frt_rt_amt") = false;
				sheetObjects[0].ColHidden("cost_45ft_frt_rt_amt") = false;	// 45' Cost 추가
				sheetObjects[0].ColHidden("diff_20ft") = false;
				sheetObjects[0].ColHidden("diff_40ft") = false;
				sheetObjects[0].ColHidden("diff_45ft") = false;	// 45' Cost 추가
				sheetObjects[0].ColHidden("diff_locl_20ft") = true;
				sheetObjects[0].ColHidden("diff_locl_40ft") = true;
				sheetObjects[0].ColHidden("diff_locl_45ft") = true;		// 45' Cost 추가
	 			...
	 			if(formObj.rhq_cd.value == "HAMRU"){
	 				...
	 				|| formObj.svc_scp_cd.value == "EAN" || formObj.svc_scp_cd.value == "EAS"){
	 	                sheetObjects[0].CellValue2( 0 , 'gline_locl_curr_20ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 0 , 'gline_locl_curr_40ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 0 , 'gline_locl_curr_45ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";		// 45' Cost 추가
	 	                sheetObjects[0].CellValue2( 1 , 'gline_locl_curr_20ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 1 , 'gline_locl_curr_40ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 1 , 'gline_locl_curr_45ft_amt') = "Tariff ("+ formObj.locl_curr_cd.value+")";		// 45' Cost 추가
	 	                sheetObjects[0].CellValue2( 0 , 'diff_locl_20ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 0 , 'diff_locl_40ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 0 , 'diff_locl_45ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";		// 45' Cost 추가
	 	                sheetObjects[0].CellValue2( 1 , 'diff_locl_20ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 1 , 'diff_locl_40ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";
	 	                sheetObjects[0].CellValue2( 1 , 'diff_locl_45ft') = "Diff. ("+ formObj.locl_curr_cd.value+")";		// 45' Cost 추가
	 					sheetObjects[0].ColHidden("gline_20ft_frt_rt_amt") = true;
	 					sheetObjects[0].ColHidden("gline_40ft_frt_rt_amt") = true;
	 					sheetObjects[0].ColHidden("gline_45ft_frt_rt_amt") = true;	// 45' Cost 추가
	 					sheetObjects[0].ColHidden("gline_locl_curr_20ft_amt") = false;
	 					sheetObjects[0].ColHidden("gline_locl_curr_40ft_amt") = false;
	 					sheetObjects[0].ColHidden("gline_locl_curr_45ft_amt") = false;		// 45' Cost 추가
	 					sheetObjects[0].ColHidden("cost_20ft_frt_rt_amt") = true;
	 					sheetObjects[0].ColHidden("cost_40ft_frt_rt_amt") = true;
	 					sheetObjects[0].ColHidden("cost_45ft_frt_rt_amt") = true;		// 45' Cost 추가
	 					sheetObjects[0].ColHidden("diff_20ft") = true;
	 					sheetObjects[0].ColHidden("diff_40ft") = true;
	 					sheetObjects[0].ColHidden("diff_45ft") = true;	// 45' Cost 추가
	 					sheetObjects[0].ColHidden("diff_locl_20ft") = false;
	 					sheetObjects[0].ColHidden("diff_locl_40ft") = false;
	 					sheetObjects[0].ColHidden("diff_locl_45ft") = false;	// 45' Cost 추가
	 					apply_20_col = "gline_locl_curr_20ft_amt";
	 					apply_40_col = "gline_locl_curr_40ft_amt";
	 					apply_45_col = "gline_locl_curr_45ft_amt";	// 45' Cost 추가
	 				}
	 			}
                break;
            ...
            case SEARCH01:   // retrieve sheet 
            	...
            	clearAllgridForms();
	 			...
	 			var sXml = sheetObj.GetSearchXml("ESM_PRI_7001_01GS.do", FormQueryString(formObj));	 	 	 
	 	        ...
	 	        comboObjects[2].Index = -1;	// 45' Cost 추가
	 			...
	 			if(formObj.amdt_seq.value == "0" && formObj.fic_prop_sts_cd.value == "I" ){		
	 		    	...
	 		    	for(var i=topRow; i<=lastRow; i++) {       
						sheetObj.CellEditable(i, apply_20_col ) = true;        
						sheetObj.CellEditable(i, apply_40_col ) = true;   
		 				// EUR일 경우만 45' Cost 수정 가능
						if(formObj.rhq_cd.value == "HAMRU"){
							sheetObj.CellEditable(i, apply_45_col ) = true;
						}
	 		    	}
	 		    	buttonControl("INIT");	
	 			}else{
            ...
            case IBSAVE: // Save                  
                ...
                var sXml = sheetObjects[0].DoSave("ESM_PRI_7001_01GS.do", sParam, -1, false );  
        		ComOpenWait(false); //->waiting->End
		     	if(sXml){
		     		ComPriSaveCompleted();
		     	}   
			...
			case MULTI09: // Add
                ...
                sheetObjects[0].CellEditable(iRow, apply_20_col ) = true;
                sheetObjects[0].CellEditable(iRow, apply_40_col ) = true;
 				// EUR일 경우만 45' Cost 수정 가능
				if(formObj.rhq_cd.value == "HAMRU") {
					sheetObjects[0].CellEditable(iRow, apply_45_col ) = true;
				}
            ...
            case MULTI07: // Apply          
        		...
        		var fixPercent45 = comboObjects[2].Code;	//	45' Cost 추가
        		...
        		var flatPercent45App = formObj.flat_percent_45_app.value == '' ? 0 : parseFloat(formObj.flat_percent_45_app.value);	//	45' Cost 추가
            	...
            	for (var i = star_idex ; i < all_row ; i++ ){
            		if(sheetObjects[0].CellValue( i, "amdt_seq") != sheetObjects[0].CellValue( i, "n1st_cmnc_amdt_seq") ){
            			...
            			if(!sheetObjects[0].CellFont("FontStrikethru", i, 1, i, sheetObjects[0].LastCol)){
	        		        ...
	        		        // new row
	        		        sheetObj.CellEditable(idx, apply_20_col) = true;
	        		        sheetObj.CellEditable(idx, apply_40_col) = true ;   
			 				// EUR일 경우만 45' Cost 수정 가능
							if(formObj.rhq_cd.value == "HAMRU"){
								sheetObj.CellEditable(idx, apply_45_col) = true;	// 45' Cost 추가
							}
	        		        ...
	        		        if(flatPercent20App != 0 && fixPercent20 != ""){
		        				if(fixPercent20 == "F"){
		        					sheetObj.CellValue( idx , apply_20_col ) = parseFloat(sheetObj.CellValue( idx , apply_20_col )) + flatPercent20App;
		            			}
			            		
			            		if(fixPercent20 == "P"){
			            			sheetObjects[0].CellValue( idx , apply_20_col ) = parseFloat(sheetObjects[0].CellValue( idx , apply_20_col )) * (1  + (flatPercent20App)/100);
			            		}
	        		        }
		            		
		            		if(flatPercent40App != 0 && fixPercent40 != ""){
			            		if(fixPercent40 == "F"){
			            			sheetObjects[0].CellValue( idx , apply_40_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_40_col )) + flatPercent40App;
			            		}
			            		
			            		if(fixPercent40 == "P"){
			            			sheetObjects[0].CellValue( idx , apply_40_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_40_col )) * (1  + (flatPercent40App)/100);
			            		}
		            		}		            		
		            		
		            		// 45' Cost 추가
		            		if(flatPercent45App != 0 && fixPercent45 != ""){
			            		if(fixPercent45 == "F"){
			            			sheetObjects[0].CellValue( idx , apply_45_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_45_col )) + flatPercent45App;
			            		}
			            		
			            		if(fixPercent45 == "P"){
			            			sheetObjects[0].CellValue( idx , apply_45_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_45_col )) * (1  + (flatPercent45App)/100);
			            		}
		            		}
		            		i++;
		            		all_row++;	  
                		...
                		if(sheetObj.CellEditable(i, apply_20_col)){
	        				if(fixPercent20 == "F"){
		            			sheetObjects[0].CellValue( i, apply_20_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_20_col )) + flatPercent20App;
	            			}
		            		
		            		if(fixPercent20 == "P"){
		            			sheetObjects[0].CellValue( i, apply_20_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_20_col )) * (1  + (flatPercent20App)/100);
		            		}

		            		if(fixPercent40 == "F"){
		            			sheetObjects[0].CellValue( i, apply_40_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_40_col )) + flatPercent40App;
		            		}
		            		
		            		if(fixPercent40 == "P"){
		            			sheetObjects[0].CellValue( i, apply_40_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_40_col )) * (1  + (flatPercent40App)/100);
		            		}
		            		
		            		// 45' Cost 추가
	            			if(fixPercent45 == "F"){
	            				sheetObjects[0].CellValue( i, apply_45_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_45_col )) + flatPercent45App;
	            			}
	            			
	            			if(fixPercent45 == "P"){
	            				sheetObjects[0].CellValue( i, apply_45_col ) = parseFloat(sheetObjects[0].CellValue( i, apply_45_col )) * (1  + (flatPercent45App)/100);
	            			}
                		}
            		}
            	}
            ...
            case MODIFY01: // g/l tuning
            	...
            	for (var i = sheetObjects[0].HeaderRows; i <= last_row_count; i++) {
            		if(sheetObj.CellEditable(i, apply_20_col )){
            			var gl20 = sheetObjects[0].CellValue( i, apply_20_col );
                		var gl40 = sheetObjects[0].CellValue( i, apply_40_col );               			
                		var gl45 = sheetObjects[0].CellValue( i, apply_45_col );	// 45' Cost 추가
            			var gl20int =  ""+gl20 * 100 ;
            			var gl40int =  ""+gl40 * 100 ;
            			var gl45int =  ""+gl45 * 100 ;	// 45' Cost 추가
            			
            			var gl20trunc = ""+Math.floor(gl20);
            			var gl40trunc = ""+Math.floor(gl40);
            			var gl45trunc = ""+Math.floor(gl45);	// 45' Cost 추가
        				...
        				if(gl20int.substring(gl20int.length, gl20int.length-3) != "000" || gl20int.substring(gl20int.length, gl20int.length-3) != "500" ){
        					if(gl20trunc.charAt(gl20trunc.length-1) == "0" || gl20trunc.charAt(gl20trunc.length-1) == "5" ){
        						sheetObjects[0].CellValue( i, apply_20_col ) = parseInt(gl20);
        					}else if(gl20trunc.charAt(gl20trunc.length-1) < 5){						
        						sheetObjects[0].CellValue( i, apply_20_col ) = gl20trunc.substr(0, gl20trunc.length -1) + "5" ; 
        					}else if(gl20trunc.charAt(gl20trunc.length-1) > 5){				
        						sheetObjects[0].CellValue( i, apply_20_col ) = Math.ceil(gl20 * 0.1) * 10 ;     
        					}        						
            			}
        				...
        				if(gl40int.substring(gl40int.length, gl40int.length-3) != "000" || gl40int.substring(gl40int.length, gl40int.length-3) != "500" ){
        					if(gl40trunc.charAt(gl40trunc.length-1) == "0" || gl40trunc.charAt(gl40trunc.length-1) == "5" ){
        						sheetObjects[0].CellValue( i, apply_40_col ) = parseInt(gl40);
        					}else if(gl40trunc.charAt(gl40trunc.length-1) < 5){
        						sheetObjects[0].CellValue( i, apply_40_col ) = gl40trunc.substr(0, gl40trunc.length -1) + "5" ;       					
        					}else if(gl40trunc.charAt(gl40trunc.length-1) > 5){		
        						sheetObjects[0].CellValue( i, apply_40_col ) = Math.ceil(gl40 * 0.1) * 10 ;     		
        					}
	            		}
    					...
    					if(gl45int.substring(gl45int.length, gl45int.length-3) != "000" || gl45int.substring(gl45int.length, gl45int.length-3) != "500" ){
    						if(gl45trunc.charAt(gl45trunc.length-1) == "0" || gl45trunc.charAt(gl45trunc.length-1) == "5" ){
    							sheetObjects[0].CellValue( i, apply_45_col ) = parseInt(gl45);
    						}else if(gl45trunc.charAt(gl45trunc.length-1) < 5){
    							sheetObjects[0].CellValue( i, apply_45_col ) = gl45trunc.substr(0, gl45trunc.length -1) + "5" ;       					
    						}else if(gl45trunc.charAt(gl45trunc.length-1) > 5){		
    							sheetObjects[0].CellValue( i, apply_45_col ) = Math.ceil(gl45 * 0.1) * 10 ;     		
    						}
    					}
        ...
        case "sheet1":
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
				...
				var HeadTitle1 = "ibflag|Sel.|Seq|RTSeq|Point|Node Point|Description|Hub|Node Hub|Base\nPort|Node Base\nPort|Contract\nTerm|Trans\nMode|Tariff (USD)|Tariff (USD)|Tariff (USD)|Tariff (LOCAL)|Tariff (LOCAL)|Tariff (LOCAL)|" +
									 "Diff. (USD)|Diff. (USD)|Diff. (USD)|Diff. (LOCAL)|Diff. (LOCAL)|Diff. (LOCAL)|Total Cost|Total Cost|Total Cost|Total Cost|Total Cost|Total Cost|Total Cost|LOC\nGroup|Full trans cost (USD)|Full trans cost (USD)|Full trans cost (USD)|Max cargo weight\nw/o surcharge (Ton)|Max cargo weight\nw/o surcharge (Ton)|Max cargo weight\nw/o surcharge (Ton)|" +
									 "MTY trans cost (USD)|MTY trans cost (USD)|MTY trans cost (USD)|TMNL cost (USD)|TMNL cost (USD)|TMNL cost (USD)|M/B Ratio\n(%, in SCC)|M/B Ratio\n(%, in SCC)|M/B Ratio (%, in SCC)|EFF Date|EXP Date|Source|" +
									 "IHC_TRF_NO|AMDT_SEQ|N1ST_CMNC_AMDT_SEQ|SVC_SCP_CD|PRC_TRF_CRE_TP_CD|ORG_DEST_TP_CD|IHC_CGO_TP_CD|RHQ_CD";  
				var HeadTitle2 = "ibflag|Sel.|Seq|RTSeq|Point|Node Point|Description|Hub|Node Hub|Base\nPort|Node Base\nPort|Contract\nTerm|Trans\nMode|Tariff (USD)|Tariff (USD)|Tariff (USD)|Tariff (LOCAL)|Tariff (LOCAL)|Tariff (LOCAL)|" +
									 "Diff. (USD)|Diff. (USD)|Diff. (USD)|Diff. (LOCAL)|Diff. (LOCAL)|Diff. (LOCAL)|USD|USD|USD|Local Currency|Local Currency|Local Currency|Local Currency|LOC\nGroup|Full trans cost (USD)|Full trans cost (USD)|Full trans cost (USD)|Max cargo weight\nw/o surcharge (Ton)|Max cargo weight\nw/o surcharge (Ton)|Max cargo weight\nw/o surcharge (Ton)|" +
									 "MTY trans cost (USD)|MTY trans cost (USD)|MTY trans cost (USD)|TMNL cost (USD)|TMNL cost (USD)|TMNL cost (USD)|M/B Ratio\n(%, in SCC)|M/B Ratio\n(%, in SCC)|M/B Ratio (%, in SCC)|EFF Date|EXP Date|Source|" +
									 "IHC_TRF_NO|AMDT_SEQ|N1ST_CMNC_AMDT_SEQ|SVC_SCP_CD|PRC_TRF_CRE_TP_CD|ORG_DEST_TP_CD|IHC_CGO_TP_CD|RHQ_CD";  		
				var HeadTitle3 = "ibflag|Sel.|Seq|RTSeq|Point|Node Point|Description|Hub|Node Hub|Base\nPort|Node Base\nPort|Contract\nTerm|Trans\nMode|20'|40'|45'|20'|40'|45'|" +
									 "20'|40'|45'|20'|40'|45'|20'|40'|45'|Cur.|20'|40'|45'|LOC\nGroup|20'|40'|45'|20'|40'|45'|20'|40'|45'|20'|40'|45'|20'|40'|45'|EFF Date|EXP Date|Source|" +
									 "IHC_TRF_NO|AMDT_SEQ|N1ST_CMNC_AMDT_SEQ|SVC_SCP_CD|PRC_TRF_CRE_TP_CD|ORG_DEST_TP_CD|IHC_CGO_TP_CD|RHQ_CD";   
                ...
                InitDataProperty(0, cnt++, 	dtData,			80, 		daRight,		true, 	"gline_locl_curr_20ft_amt", false, "", dfFloat, 2, false, false, 9);
                InitDataProperty(0, cnt++, 	dtData,			80, 		daRight,		true, 	"gline_locl_curr_40ft_amt", false, "", dfFloat, 2, false, false, 9);                                  
                InitDataProperty(0, cnt++, 	dtData,			80, 		daRight,		true, 	"gline_locl_curr_45ft_amt", false, "", dfFloat, 2, false, false, 9);		// 45' Cost 추가
                ...
                InitDataProperty(0, cnt++, 	dtData, 		80, 			daRight, 	true, 	"diff_45ft",	         false,	"", dfNone, 0, false, false);		// 45' Cost 추가
                InitDataProperty(0, cnt++, 	dtData, 		80, 			daRight,	true, 	"diff_locl_20ft",	         false,	"", dfNone,  0, false, false);                                
                InitDataProperty(0, cnt++, 	dtData, 		80, 			daRight, 	true, 	"diff_locl_40ft",	         false,	"", dfNone, 0, false, false);   
                InitDataProperty(0, cnt++, 	dtData, 		80, 			daRight, 	true, 	"diff_locl_45ft",	         false,	"", dfNone, 0, false, false);		// 45' Cost 추가
                ...
                InitDataProperty(0, cnt++, 	dtData, 	   100, 		daCenter, 	true, 	"ihc_cost_loc_grp_no",   false, "", dfNone, 0, false, false);                                
    			...
    			InitDataProperty(0, cnt++,  dtData, 		80,			daRight, 	true, 	"trsp_20ft_agmt_wgt", 	 false, "", dfNullFloat, 2, false, false, 9);
     			InitDataProperty(0, cnt++,  dtData, 		80, 		daRight, 	true, 	"trsp_40ft_agmt_wgt", 	 false, "", dfNullFloat, 2, false, false, 9);
                InitDataProperty(0, cnt++,	dtData, 		80, 		daRight, 	true, 	"trsp_45ft_agmt_wgt", 	 false, "", dfNullFloat, 2, false, false, 9);		// 45' Cost 추가
                ...
                sheetObj.SetMergeCell (0, 13, 2, 3);
                sheetObj.SetMergeCell (0, 16, 2, 3);
                sheetObj.SetMergeCell (0, 19, 2, 3);
                sheetObj.SetMergeCell (0, 22, 2, 3);
                sheetObj.SetMergeCell (0, 33, 2, 3);
                sheetObj.SetMergeCell (0, 36, 2, 3);
                sheetObj.SetMergeCell (0, 39, 2, 3);
                sheetObj.SetMergeCell (0, 42, 2, 3);
                sheetObj.SetMergeCell (0, 45, 2, 3);
    ...
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case MULTI03: // Confirm
            ...
            for (var i = sheetObjects[0].HeaderRows; i <= sheetObjects[0].LastRow; i++) {
            	...
            	if(sheetObj.CellEditable(i, apply_20_col )){
	            	if(sheetObjects[0].CellValue( i, apply_20_col ) =="0" || sheetObjects[0].CellValue( i, apply_20_col ) ==""){
	            		ComShowCodeMessage('PRI07007', sheetObjects[0].CellValue( i, "pnt_loc_cd" ), "Dry" ); 
	            		sheetObjects[0].SelectCell( i, apply_20_col );
	            		 return false;
	            	}
	            	
	            	if(sheetObjects[0].CellValue( i, apply_40_col ) =="0" || sheetObjects[0].CellValue( i, apply_40_col ) ==""){
	            		ComShowCodeMessage('PRI07007', sheetObjects[0].CellValue( i, "pnt_loc_cd"), "Dry" ); 
	            		sheetObjects[0].SelectCell( i, apply_40_col );
	            		 return false;
	            	}
	            	...
	            	// 확인 후 적용
//	            	if(formObj.rhq_cd.value == "HAMRU") {
//	            		if(sheetObjects[0].CellValue( i, apply_45_col ) =="0" || sheetObjects[0].CellValue( i, apply_45_col ) ==""){
//	            			ComShowCodeMessage('PRI07007', sheetObjects[0].CellValue( i, "pnt_loc_cd"), "Dry" ); 
//	            			sheetObjects[0].SelectCell( i, apply_45_col );
//	            			return false;
//	            		}
//	            	}
	    			...
	    			if(gl20int.substring(gl20int.length, gl20int.length-3) != "000" && gl20int.substring(gl20int.length, gl20int.length-3) != "500" ){
	    				ComShowCodeMessage('PRI07015','Dry'); 
	           		 	return false;				
	    			}
...
//	    			}
        ...
        case MULTI07: // Apply
           if (comboObjects[0].Code != "" && formObj.flat_percent_20_app.value == "") {
            ...
            // 45' Cost 추가
            if (formObj.rhq_cd.value == "HAMRU") {
            	if (comboObjects[2].Code != "" && formObj.flat_percent_45_app.value == "") {
            ...
            // 45' Cost 추가
            if(formObj.rhq_cd.value == "HAMRU") {
            	if (comboObjects[0].Code =="" && formObj.flat_percent_20_app.value == "" && comboObjects[1].Code =="" & formObj.flat_percent_40_app.value == "" && comboObjects[2].Code =="" && formObj.flat_percent_45_app.value == "") {
            		ComShowCodeMessage('PRI00351'); 
                    return false;
            	}
            } else if (comboObjects[0].Code =="" && formObj.flat_percent_20_app.value == "" && comboObjects[1].Code =="" & formObj.flat_percent_40_app.value == "" && comboObjects[2].Code =="") {
              	ComShowCodeMessage('PRI00351'); 
               return false;
           }
    ...
    */      
    function clearAllForms(){   
        ...
        formObj.node_code_flg.checked=false; 
    ...
    */ 
    ...
    function sheet1_OnChange(sheetObj, Row, Col, Value) {
    	...
    	if (colName == "pnt_loc_cd") {
    		...
    		if(sheetObj.CellValue(Row, 'prc_trf_cre_tp_cd') == 'G') {
    			...
    			// EUR이 아닐 경우 40'와 45'를 동일하게 보여준다.
    			if(document.form.rhq_cd.value != "HAMRU") {
    				sheetObj.CellValue2(Row, 'diff_45ft') = 'N/A';
    			}
    		} else {
    			...
    			// EUR이 아닐 경우 40'와 45'를 동일하게 보여준다.
    			if(document.form.rhq_cd.value != "HAMRU") {
    				sheetObj.CellValue2(Row, 'diff_45ft') = ComGetMaskedValue(v_result, "float");
    				sheetObj.CellValue2(Row, 'gline_45ft_frt_rt_amt') = sheetObj.CellValue(Row, 'gline_40ft_frt_rt_amt');
    			}
    		}
    	// 45' Cost 추가
    	} else if (colName == "gline_45ft_frt_rt_amt" ) {
    		if(sheetObj.CellValue(Row, 'prc_trf_cre_tp_cd') == 'G') {
    			sheetObj.CellValue2(Row, 'diff_45ft') = 'N/A';
    		} else {
    			var v_gline_45ft_frt_rt_amt = parseFloat(sheetObj.CellValue(Row, "gline_45ft_frt_rt_amt" ));
    			var v_cost_45ft_frt_rt_amt =  parseFloat(sheetObj.CellValue(Row, 'cost_45ft_frt_rt_amt'));
    			// null 처리
    			if(isNaN(v_cost_45ft_frt_rt_amt)) {
    				v_cost_45ft_frt_rt_amt = 0;
    			}
    			v_cost_45ft_frt_rt_amt = (v_cost_45ft_frt_rt_amt == null)?0:v_cost_45ft_frt_rt_amt;
    			var v_result =  parseFloat(Math.round((v_gline_45ft_frt_rt_amt - v_cost_45ft_frt_rt_amt)*100.0)/100.0);
    			if( v_result == Math.floor(v_result) ){
    				v_result = v_result+".00";
    			}
    			sheetObj.CellValue2(Row, 'diff_45ft') = ComGetMaskedValue(v_result, "float");
    		}
    	} else if (colName == "gline_locl_curr_20ft_amt" ) {
    		if(sheetObj.CellValue(Row, 'prc_trf_cre_tp_cd') == 'G') {
    			sheetObj.CellValue2(Row, 'diff_locl_20ft') = 'N/A';
    		} else {
    			var v_gline_20ft_frt_rt_amt = parseFloat(sheetObj.CellValue(Row, "gline_locl_curr_20ft_amt" ));
    			var v_cost_20ft_frt_rt_amt =  parseFloat(sheetObj.CellValue(Row, 'locl_curr_cost_20ft_frt_rt_amt'));
    			var v_result =  parseFloat(Math.round((v_gline_20ft_frt_rt_amt - v_cost_20ft_frt_rt_amt)*100.0)/100.0);
    			if( v_result == Math.floor(v_result) ){
    				v_result = v_result+".00";
    			}    			
    			sheetObj.CellValue2(Row, 'diff_locl_20ft') = ComGetMaskedValue(v_result, "float");
    		}
    	} else if (colName == "gline_locl_curr_40ft_amt" ) {
    		if(sheetObj.CellValue(Row, 'prc_trf_cre_tp_cd') == 'G') {
    			sheetObj.CellValue2(Row, 'diff_locl_40ft') = 'N/A';
    		} else {
    			var v_gline_40ft_frt_rt_amt = parseFloat(sheetObj.CellValue(Row, "gline_locl_curr_40ft_amt" ));
    			var v_cost_40ft_frt_rt_amt =  parseFloat(sheetObj.CellValue(Row, 'locl_curr_cost_40ft_frt_rt_amt'));
    			var v_result =  parseFloat(Math.round((v_gline_40ft_frt_rt_amt - v_cost_40ft_frt_rt_amt)*100.0)/100.0);
    			if( v_result == Math.floor(v_result) ){
    				v_result = v_result+".00";
    			}
    			sheetObj.CellValue2(Row, 'diff_locl_40ft') = ComGetMaskedValue(v_result, "float");
    		}
   		// 45' Cos 추가
    	} else if (colName == "gline_locl_curr_45ft_amt" ) {
    		if(sheetObj.CellValue(Row, 'prc_trf_cre_tp_cd') == 'G') {
    			sheetObj.CellValue2(Row, 'diff_locl_45ft') = 'N/A';
    		} else {
    			var v_gline_45ft_frt_rt_amt = parseFloat(sheetObj.CellValue(Row, "gline_locl_curr_45ft_amt" ));
    			var v_cost_45ft_frt_rt_amt =  parseFloat(sheetObj.CellValue(Row, 'locl_curr_cost_45ft_frt_rt_amt'));
    			// null 처리
    			if(isNaN(v_cost_45ft_frt_rt_amt)) {
    				v_cost_45ft_frt_rt_amt = 0;
    			}
    			...
    			sheetObj.CellValue2(Row, 'diff_locl_45ft') = ComGetMaskedValue(v_result, "float");
    		}
    	}
    }
    ...
    function setLineEnable(sheetObj, idx) {
    	...
    	if (sheetObj.CellValue(idx, "n1st_cmnc_amdt_seq") == document.form.amdt_seq.value
        	...
        	sheetObj.CellEditable(idx, apply_20_col ) = false;
        	sheetObj.CellEditable(idx, apply_40_col ) = false;
        	sheetObj.CellEditable(idx, apply_45_col ) = false;	// 45' Cost 추가
		}
     ...
     * @version 2009.05.21
     */ 
	function tabLoadSheet(sIhcTrfNo, sAmdtSeq, sSvcScpCd, sFicPropStsCd, sEffDt, sCostCntCd, sOrgDestTpCd, sLoclCurrCd, sRhqCd ) {
		...
		if (formObject.ihc_trf_no.value != sIhcTrfNo || formObject.amdt_seq.value != sAmdtSeq || formObject.svc_scp_cd.value != sSvcScpCd 
		  ...
		  || formObject.org_dest_tp_cd.value != sOrgDestTpCd || formObject.rhq_cd.value != sRhqCd) {
			...
			formObject.rhq_cd.value = sRhqCd;
			
			// HAMRU 경우만 45' Cost 수정 가능
			if(sRhqCd == "HAMRU") {
				formObject.fix_percent_45.enable = true;
				formObject.flat_percent_45_app.readOnly = false;
				formObject.flat_percent_45_app.setAttribute("className","input");
			} else {
				formObject.fix_percent_45.enable = false;
				formObject.flat_percent_45_app.readOnly = true;
				formObject.flat_percent_45_app.setAttribute("className","input2");
			}
			buttonControl("LOAD");
        ...
        for (i = sheetObj.HeaderRows ; i < n; i++) {
            for (j = 0 ; j <= m ; j++) {
                if (sheetObj.ToolTipText(i, j) != "") {
                	...
                	if(!sheetObjects[0].CellEditable(i, apply_20_col )){ 
                    ...
                    sheetObj.CellFontColor(i, j) = sheetObj.RgbColor(sheetObj.CellFontColor(i, apply_20_col ),0,0);

...
*@FileName : ESM_PRI_9051_02.js
*@FileTitle : S/C Auth Hardcoding Management - Filed Cancel Activate
*Open Issues :
*Change history :
...
/**
 * @extends 
 * @class ESM_PRI_9051_02 : ESM_PRI_9051_02 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_9051_02() {
   ...
   */ 
			...
			case "sheet1":      // sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle1  = "|Seq.|Office|GID|Name|Y/N";
					...
					InitDataProperty(0, cnt++ , dtSeq,		    80, daCenter,  	false,		"seq",   				false,   	"",      	dfNone,     0,			false,       	false);
					InitDataProperty(0, cnt++ , dtData,    		150,daCenter,  	false,		"auth_apro_usr_ofc_cd", false,    	"",      	dfEngUpKey, 0,			false,       	true,	6);
					InitDataProperty(0, cnt++ , dtPopupEdit,    200,daLeft,  	false,		"auth_apro_usr_id",   	true,     	"",      	dfNone,     0,			false,       	true,	20);
					InitDataProperty(0, cnt++ , dtData,    		400,daLeft,  	false,		"auth_apro_usr_nm",   	false,     	"",      	dfNone,     0,			false,       	false);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        case IBSEARCH:      //조회
        	...
        	ComOpenWait(true);   
        	sheetObj.RemoveAll();
        	var sXml = sheetObj.GetSearchXml("ESM_PRI_9051_02GS.do", FormQueryString(formObj) );
		 ...
		 case IBSAVE:        //저장
			//저장시 Row Add 대상으로 기존 것과 중복이 있는지 확인.
		 	...
		 	if (sheetObj.IsDataModified) {
		 	   sheetObj.DoSave("ESM_PRI_9051_02GS.do",FormQueryString(formObj) );			 	   
		 	} else {
    ...
    */      
    function sheet1_OnPopupClick(sheetObj, Row, Col){
        ...
        switch(colName)
        {
            case "auth_apro_usr_id": 
		     	 var ofc_cd = sheetObj.CellValue(Row,"auth_apro_usr_ofc_cd");
		         ComOpenPopup('/hanjin/COM_ENS_091.do?ofc_cd='+ofc_cd, 780, 535, 'getCOM_ENS_091_1', '1,0,1,1,1,1,1,1',true, true);
                break;    	                    	         
        }
    }     
    ...
    function getCOM_ENS_091_1(rowArray) {
    	...
    	//셋팅하지 않고 팝업!
      	sheetObject1.CellValue(sheetObject1.SelectRow,"auth_apro_usr_id") 		= colArray[4]	;
      	sheetObject1.CellValue(sheetObject1.SelectRow,"auth_apro_usr_nm") 		= colArray[5]	;
      	sheetObject1.CellValue(sheetObject1.SelectRow,"auth_apro_usr_ofc_cd")  = colArray[3]	;
    	...
    	case IBSAVE: 
    		...
    		if (sheetObj.IsDataModified ) {
    			...
    			clearTooltip();
				 ...
				 var rowM = sheetObj.ColValueDup("auth_apro_usr_id",true);
				 if (rowM >= 0) {
					 ...
					 for (var i = sheetObj.LastRow; i >= 0; i--) {
			             ...
			             if(sheetObj.CellValue(i,"auth_apro_usr_id") == sheetObj.CellValue(rowM,"auth_apro_usr_id")) {
			             	add2Tooltip(i, "auth_apro_usr_id", msg);
			             }
			         }
    ...
    function sheet1_OnDblClick(sheetObj, Row, Col) {
        ...
        var prc_usr_auth_tp_cd = formObj.prc_usr_auth_tp_cd.value;
        var auth_apro_usr_id = sheetObj.CellValue(Row, "auth_apro_usr_id");
        var popParams = "title=Filed Cancel Activity (History)&prc_ctrt_tp_cd="+prc_ctrt_tp_cd+"&prc_usr_auth_tp_cd="+prc_usr_auth_tp_cd+"&auth_apro_usr_id="+auth_apro_usr_id;
        ComPriOpenWindowCenter("/hanjin/ESM_PRI_9452.do?"+popParams, "ESM_PRI_9452",900, 320, true);
    }
     ...
     */  	
   	function sheet1_OnChange(sheetObj, Row, Col) {
	      ...
	      var formObj = document.form;
    		...
    		switch(colName){
    		case "auth_apro_usr_ofc_cd":
    			  var ofc_cd = sheetObj.CellValue(Row,"auth_apro_usr_ofc_cd");
    			  ofc_cd = ofc_cd.toUpperCase();
    			  sheetObj.CellValue2(Row,"auth_apro_usr_ofc_cd") = ofc_cd;
    			  sheetObj.CellValue2(Row,"auth_apro_usr_id") = "";
	              sheetObj.CellValue2(Row,"auth_apro_usr_nm") = "";
    			break;
    		case "auth_apro_usr_id":
    			var user_cd = sheetObj.CellValue(Row,Col);
    			if(user_cd !="") {
    	            var sParam = "user_cd="+user_cd+"&f_cmd="+SEARCH;
    	            var sXml = sheetObj.GetSearchXml("COM_ENS_091GS.do", sParam);
    	            var arrText = ComPriXml2Array(sXml, "usr_id|usr_nm|usr_eml|ofc_cd");
    	            if(arrText==undefined){
        	        	sheetObj.CellValue2(Row,"auth_apro_usr_ofc_cd") = "";
    	            	sheetObj.CellValue2(Row,"auth_apro_usr_id") = "";
    	            	sheetObj.CellValue2(Row,"auth_apro_usr_nm") = "";
    	            }else{    
     	            		sheetObj.CellValue2(Row,"auth_apro_usr_ofc_cd") = arrText[0][3];
     	            		sheetObj.CellValue2(Row,"auth_apro_usr_id") = arrText[0][0]; 	   
     	            		sheetObj.CellValue2(Row,"auth_apro_usr_nm") = arrText[0][1]; 	   
     	            }
    	        }else{
    	        	sheetObj.CellValue2(Row,"auth_apro_usr_ofc_cd") = "";
    	        	sheetObj.CellValue2(Row,"auth_apro_usr_id") = "";

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends Pri
     * @class ESM_PRI_0009 : ESM_PRI_0009 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0009 () {
        ...
        try {
            ...
            if (srcName == "ofc_cd") {
                ...
                if (formObj.ofc_cd.value == "") {
                    // Office Code 가 '' 이면 전체 조직도 조회
                    doActionIBSheet(sheetObject1,formObj,IBSEARCH);
                } else {
                    var idx = sheetObject1.FindText("ofc_cd", formObj.ofc_cd.value);
                    if (idx != -1) {    // 입력한 Office Code 가 조직도에 있을 경우
                        // 조직도에 해당 Office 가 있는 Node 를 펼침
                        sheetObject1.ShowTreeLevel(0,1);
                        // 조직도에서 선택
                    ...
                    } else {    // 입력한 Office Code 가 조직도에 없을 경우
    ...
    function obj_keyup() {
        try {
            ...
            if (event.keyCode == 13) {
                if (srcName == "ofc_cd") {
                    // Office Code 에서 Enter 입력시 focus 를 강제로 이동시켜서 타 이벤트를 발생시킴
                    document.body.focus();
                } else {
                    // Office Code 이외 일 경우 조회함수 호출
                    doActionIBSheet(sheetObject2, formObj, IBSEARCH);
                }
            }
        } catch(e) {
        ...
        switch (comboObj.id) {
            ...
            case "usr_id":
                with (comboObj) {
                    ...
                    ValidChar(1, 3);    // 영문,숫자,특수문자만 입력가능
                }
                break;
        }
    }
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            case IBSEARCH_ASYNC01: // office code 입력 시, 사원콤보리스트 조회
                ...
                var sXml = sheetObj.GetSearchXml("ESM_PRI_4100GS.do", FormQueryString(formObj));
            ...
            case IBSEARCH:      //조회
                ...
                if (validateForm(sheetObj,formObj,sAction)) {
                    if (sheetObj.id == "sheet1") {
                        ...
                        sheetObj.DoSearch("ESM_PRI_4100GS.do", FormQueryString(formObj));
            ...
            case IBSAVE:        //저장
                ...
                if (validateForm(sheetObj,formObj,sAction)) {
                    ...
                    sheetObj.DoSave("ESM_PRI_4100GS.do", FormQueryString(formObj), -1, false);
                }
                ComOpenWait(false);
                break;
    ...
    /**
     * OnChange 이벤트 발생시 호출되는 function <br>
     * Authority 를 'YES'로 선택하면 Effective Date 항목을 enable시키고 그 외는 disable 시킨다. <br>
     * <br><b>Example :</b>
     * <pre>
     *
     * </pre>
     ...
     * 조직도에서 조직명을 클릭하면 해당 조직내의 사원의 권한정보를 조회 <br>
     * <br><b>Example :</b>
     * <pre>
     *
     * </pre>

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends Pri
     * @class ESM_PRI_0014 : S/C Prefix Creation 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0014() {
    	...
    	switch(sheetNo) {
        	case 1:      // sheet1 init
            	with (sheetObj) {
            		...
            		InitDataProperty(0, cnt++ , dtData,		  		65,	daCenter,	true,	"sc_pfx_cd",	true,	"",		dfNone,		0,			false,		true,	3,	true);
            		InitDataProperty(0, cnt++ , dtData,				95,	daLeft,		true,	"sc_pfx_desc",  true,	"",		dfNone,		0,			true,		true,   100);
        
            		InitDataValid(0,  "sc_pfx_cd",	vtEngUpOnly);		// 영문대문자만 입력
            		InitDataValid(0,  "sc_pfx_desc", vtEngOther, PRI_VALID_CHAR);  // 한글제외
            		MessageText("UserMsg17") = "3 alphabets only allow for input";
    ...
    function validateForm(sheetObj,formObj,sAction){
    	...
    	with(sheetObj){
    	    ...
    	    if (sParam != "" && IsDataModified) {
                ...
                var sXml = GetSearchXml("ESM_PRI_0014GS.do", sParam);

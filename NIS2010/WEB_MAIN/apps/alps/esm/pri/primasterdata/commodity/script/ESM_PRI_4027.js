...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends
	 * @class ESM_PRI_4027 : ESM_PRI_4027 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESM_PRI_4027() {
	...
	function grp_cmdt_seq_OnChange(comboObj, code, text) {
		...
		sheetObjects[1].DoSearch("ESM_PRI_4027GS.do", FormQueryString(formObj));
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch (sAction) {
			...
			case IBSEARCH_ASYNC10: // 조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_PRI_4027GS.do", FormQueryString(formObj));

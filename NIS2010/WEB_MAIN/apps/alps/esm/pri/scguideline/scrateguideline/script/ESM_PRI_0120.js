...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
        ...
        switch (sheetNo) {
	        case 1: // sheet1 init
	            with (sheetObj) {
		            ...
		            InitDataProperty(0, cnt++, dtData, 50, daCenter, false, "mqc_rng_fm_qty", false, "", dfNullInteger, 0, true, true, 6);
		            InitDataProperty(0, cnt++, dtData, 50, daCenter, false, "mqc_rng_to_qty", false, "", dfNullInteger, 0, true, true, 6);
	                ...
	                InitDataProperty(0, cnt++, dtData, 100, daRight, false, "frt_rt_amt", false, "", dfNullFloat, 2, true, true, 9);
	                ...
	                InitDataCombo(0, "rat_ut_cd", perComboText, perComboValue);
	                InitDataCombo(0, "prc_cgo_tp_cd", cargoTpComboText, cargoTpComboValue);
    ...
    function sheet1_OnChange(sheetObj, Row, Col, Value) {
        ...
        if (colName == "prc_cgo_tp_cd") {
            ...
            if (Value.length == 6) {
                ...
                var param = "&cd=" + Value + "&nm=sg" + "&etc1=" + formObj.svc_scp_cd.value + "&etc2=" + formObj.gline_seq.value;
                var sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
                var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
                if (arrData != null && arrData.length > 0 && arrData[1] != "") {
                    sheetObj.CellValue2(Row, "prc_cmdt_def_nm") = arrData[1];
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        case IBSAVE: // Save
            ...
            var sXml = sheetObj.GetSaveXml("ESM_PRI_0120GS.do", sParam);
...
//                        }
                    ...
                    mqcRngFmQty = sheetObj.CellValue(i, "mqc_rng_fm_qty");
                    mqcRngToQty = sheetObj.CellValue(i, "mqc_rng_to_qty");
                    perTypeCode = sheetObj.CellValue(i, "rat_ut_cd");
                    cargoTypeCode = sheetObj.CellValue(i, "prc_cgo_tp_cd");
                    rateAmt = sheetObj.CellValue(i, "frt_rt_amt");
                    if (mqcRngFmQty != "" || mqcRngToQty != "" || perTypeCode != "" || cargoTypeCode != "" || rateAmt != "") {
                        ...
                        if (rateAmt == "") {
                            ...
                            isClear = false;
                        } else if (rateAmt != "" && parseFloat(rateAmt) <= 0.00) {
                            add2Tooltip(i, "frt_rt_amt", ComGetMsg("PRI00321", "Rate", "0"));
                            isClear = false;
                ...
                var sParam = FormQueryString(formObj) + "&" + sheetObj.GetSaveString();
                var sXml = sheetObject2.GetSearchXml("ESM_PRI_0120GS.do", sParam);
     ...
     * @param {int} col 필수 해당 셀의 Column Index
     * @param {string} msg 필수 툴팁에 들어갈 메세지
     * @return 없음
     * @author 박성수
     * @version 2010.03.11
     */

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class Origin/Destination Arbitrary Charge Guideline Creation : Origin/Destination Arbitrary Charge Guideline Creation 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_0001_04() {
     		...
     		switch(srcName) {
				...
				case "btn_LoadExcel":
					if (validateForm(sheetObject1,formObject,IBLOADEXCEL)) {					
						...
						var str = ComOpenWindowCenter("/hanjin/ESM_PRI_0067.do?"+sParam, "", getWidth(900), getHeight(425), true);
						fontChange();
						parent.setTabStyle();
					}else{
             ...
             case "sheet1":
                 with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     var HeadTitle = "|Sel.|Seq.|ARBSEQ|pnttpcd|Point|Description|Trans. Mode|Term|bptpcd|Base Port|vptpcd|VIA|Direct Call|Per|Cargo Type|Cur|Rate|svcscpcd|glineseq|orgdesttpcd";
...
//				  	  				  SORTENABLE, TOOLTIP, 	  ALLCHECK,   SAVESTATUS, FORMATFIX]  
                     ...
                     InitDataProperty(0, cnt++ , dtData,      	 120,daLeft,	false, "loc_des",             false,  "", dfNone,	  0,  false, false);
                     InitDataProperty(0, cnt++ , dtCombo,      	 90, daCenter,  false, "prc_trsp_mod_cd",     false, "", dfNone, 	  0,  true,  true); 
                     InitDataProperty(0, cnt++ , dtCombo,      	 70, daCenter,  false, "rcv_de_term_cd",      true,  "", dfNone, 	  0,  true,  true);					 
 					 InitDataProperty(0, cnt++ , dtHidden,    	 40, daCenter,  true,  "bse_port_tp_cd");
                     InitDataProperty(0, cnt++ , dtPopupEdit,    85, daCenter,  false, "bse_port_def_cd",     true,  "", dfNone, 	  0,  true,  true, 5);
                     InitDataProperty(0, cnt++ , dtHidden,    	 40, daCenter,  true,  "via_port_tp_cd");
 					 InitDataProperty(0, cnt++ , dtPopupEdit,    85, daCenter,  false, "via_port_def_cd",     false, "", dfNone, 	  0,  true,  true, 5);
                     InitDataProperty(0, cnt++ , dtCheckBox,   	 75, daCenter,  false, "dir_call_flg",        false, "", dfNone, 	  0,  true,  true,-1,false,true,"",false);
                     InitDataProperty(0, cnt++ , dtCombo,  		 50, daCenter,  false, "rat_ut_cd",      	  true,  "", dfNone, 	  0,  true,  true);
 					 InitDataProperty(0, cnt++ , dtCombo,     	 85, daCenter,  false, "prc_cgo_tp_cd",       false, "", dfNone, 	  0,  true,  true);
                     InitDataProperty(0, cnt++ , dtCombo,      	 60, daCenter,  false, "curr_cd",      	      true,  "", dfNone, 	  0,  true,  true);
                     InitDataProperty(0, cnt++ , dtData,      	 75, daRight,   false, "frt_rt_amt",          true,  "", dfNullFloat, 2,  true,  true,  9);
         ...
         try{
             switch(sAction) {
	 	 		case IBSEARCH_ASYNC01: // 화면조회시 Customer Type 재조회
	 				...
	 				var sXml = sheetObj.GetSearchXml("ESM_PRI_0001_04GS.do" , FormQueryString(formObj));
	  			...
	  			case IBCLEAR: // 화면 로딩시 
	 				...
	 				//공통 cargo type
	 				sheetObj.InitDataCombo(0,"prc_cgo_tp_cd", cargoCdComboText, cargoCdComboValue);			
	 				break;
	 			case IBSEARCH:      //조회	
	 			...
	 			case IBSAVE:        //저장
	 				...
	 				if (sParamSheet == ""){
	  				...
	  				var sXml = sheetObj.GetSaveXml("ESM_PRI_0001_04GS.do", sParam+"&"+sParamSheet);
	 			...
	 			case IBINSERT:      // 입력
	 				...
	 				sheetObj.CellValue(Row, "org_dest_tp_cd") = formObj.org_dest_tp_cd.value;	
	 				sheetObj.CellValue(Row,"arb_seq") = parseInt(ComPriGetMax(sheetObj, "arb_seq"))+ 1;				
    	  	...
    	  	case IBSAVE: // 저장
		       if (parent.document.form.cfm_flg.value == "Yes"){
				...
				if (sheetObjects[0].IsDataModified ) {					
					var rowM = sheetObjects[0].ColValueDup("rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|bse_port_def_cd|via_port_def_cd|dir_call_flg|rat_ut_cd|prc_cgo_tp_cd");
					 if (rowM >= 0) {
...
//						var perCd = sheetObj.CellValue(i,"rat_ut_cd").substr(0,1)
//						if (perCd != "A" && perCd != "F" && perCd != "O" && perCd != "Q" && perCd != "S" && perCd != "P" ){
//			    			if (sheetObj.CellValue(i, "prc_cgo_tp_cd") == "AK"){
//				    			ComShowCodeMessage('PRI08003');
//				    			sheetObj.CellValue(i, "prc_cgo_tp_cd") = "";
//				    			sheetObj.SelectCell(i,"prc_cgo_tp_cd");
//				    			return false;
//			    			}
//			    		}						
//					}					 
     ...
     * Origin,Destination을 클릭함에 따라 Type에 해당하는 Term을 재 조회 한다.
     * <br><b>Example :</b>
     * <pre>
     *     
     * </pre>
     ...
     * </pre>
     * @param 없음
     * @return {int} 현재 선택된 Radio 버튼의 index값
     * @author 공백진
     * @version 2009.04.17
     */ 	 	
    function getTpCd(){
    ...
    */  	
    	...
    	switch(colname)
    	{
	    	...
	    	case "via_port_def_cd":
	    		...
	    		if (Value.length == 4){
	 				...
	 				if (arrData != null && arrData.length > 0) {
	 					if (arrData[1]!=""){
	 						sheetObj.CellValue2(Row,"via_port_tp_cd")="G";	 						
	 					}else{	
	 						ComShowCodeMessage("PRI00315");
	 						cellClear(sheetObj,Row, colname);
	 					}
	    	...
	    	case "dir_call_flg":
	    		...
	    		if (Value == "1"){
    				...
    				sheetObj.PopupButtonImage(Row, "via_port_def_cd") = 0;
	    		}else{
	    			sheetObj.CellEditable(Row,"via_port_def_cd") = true;
	    			sheetObj.PopupButtonImage(Row, "via_port_def_cd") = 1;
	    		}
	    	...
	    	case "rat_ut_cd":
	    		if (sheetObj.CellValue(Row, "prc_cgo_tp_cd") == "AK"){
		    		...
		    		var preCd = sheetObj.CellValue(Row, "rat_ut_cd").charAt(0);
    	...
    	function sheet1_OnSearchEnd(sheetObj, errMsg){
    		for(i = 1 ; i < sheetObj.Rows; i++){
    			if (sheetObj.CellValue(i, "dir_call_flg") =="1" ){
    				...
    				sheetObj.PopupButtonImage(i, "via_port_def_cd") = 0;
    			}else{
    				sheetObj.CellEditable(i,"via_port_def_cd") = true;
    				sheetObj.PopupButtonImage(i, "via_port_def_cd") = 1;
    			}
    		}
    ...
    /**
    * Point와 B.Port에 동일 Location 등록 check function <br>
    * <br><b>Example :</b>
    * <pre>
    * 		locationDupChk(sheetObj,1,1)
    * </pre>
    * @param {ibsheet} sheetObj 필수 IBSheet Object
    * @param {int} Row 필수 check할 해당 셀의 Row Index  
    * @param {int} Col 필수 check할 해당 셀의 Col Index 
    * @return 없음
    * @author 공백진
    * @version 2009.04.17
    */  
    ...
    * @param {int} Col 필수 값을 초기화할 해당 셀의 Col Index     
    * @param {str} msg PRI공통 메세지    
    * @return 없음
    * @author 공백진
    * @version 2009.04.17
    */  	    
    ...
    */  	      	 
	function sheet1_OnPopupClick(sheetObj, Row, Col)
	{
		...
		if (colName == "rout_pnt_loc_def_cd"){
			...
			var sUrl = "/hanjin/ESM_PRI_4026.do?group_cmd=" + PRI_SG + "&location_cmd=LG&svc_scp_cd=" + formObj.svc_scp_cd.value + "&gline_seq=" + formObj.gline_seq.value;
			var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_4026", 700, 325, true);
			var tpCd = "G";
			if (rtnVal != null){
				sheetObj.CellValue2(Row, Col) = rtnVal.cd;	

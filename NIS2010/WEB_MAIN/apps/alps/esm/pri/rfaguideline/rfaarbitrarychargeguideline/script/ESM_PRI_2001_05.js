...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_2001_05 : ESM_PRI_2001_05 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_2001_05() {
...
//      var prefix = sheetID+"_";
        ...
        switch(sheetID) {
            case "sheet1":
                with (sheetObj) {
                    ...
                    InitDataProperty(0, cnt++, dtCombo,         90,     daCenter,   false,  "prc_trsp_mod_cd",      false,  "", dfEngUpKey,     0,  true,   true);
                    ...
                    InitDataProperty(0, cnt++, dtPopupEdit,     80,     daCenter,   false,  "bse_port_def_cd",      true,   "", dfNone,         0,  true,   true,   5);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
        	...
        	case IBSEARCH:      //조회
				...
				sheetObj.DoSearch("ESM_PRI_2001_05GS.do", FormQueryString(formObj));
        	...
        	case IBSEARCH_ASYNC01: // radio button select
				...
				var sXml = sheetObj.GetSearchXml("ESM_PRI_2001_05GS.do" , FormQueryString(formObj));
        	...
        	case IBSAVE:
				...
				var sParam = FormQueryString(formObj);
 				...
 				var sXml = sheetObj.GetSaveXml("ESM_PRI_2001_05GS.do", sParam+"&"+sParamSheet);
        	...
        	case MODIFY01:
				...
				var sXml = sheetObj.GetSaveXml("ESM_PRI_2001_05GS.do", sParamSheet);
				sheetObj.LoadSaveXml(sXml);
				ComOpenWait(false);
	            break;
            ...
            case IBSEARCH_ASYNC06: //Search FIC Route
                ...
                var sXml = sheetObjects[2].getSearchXml("ESM_PRI_2001_05GS.do", sParam);
            ...
            case IBSEARCH_ASYNC08: //Search FIC Guide Line
                ...
                if (arrDesc != null && arrDesc.length > 0) {
                    ...
                    if( sheetObj.CellValue(Row, "rat_ut_cd") == ""){
                        sheetObj.CellValue2(Row, "rat_ut_cd") = "D2";
                    }
		...
		if (event.srcElement.name == "org_dest_tp_cd") {
			...
			if(getActSheet().IsDataModified) {
				if(ComShowCodeConfirm('PRI00006')) {
					if(!doActionIBSheet(getActSheet(), formObj, MODIFY01)) {
						returnRadioButton();
						return;
					}
				} else {
			...
			showSheetObj(formObj.svc_scp_cd.value);
	...
	function tabLoadSheet(sSvcScpCd, sGlineSeq, isAproUsr,effDt) {
		...
		if(formObject.svc_scp_cd.value != sSvcScpCd || formObject.gline_seq.value != sGlineSeq) {
			...
			doActionIBSheet(sheetObjects[2], document.form, IBSEARCH_ASYNC03);
	         showSheetObj(formObject.svc_scp_cd.value);
		  	...
		  	case IBSAVE: // 저장
	            if(!ComPriConfirmSave()) {
			  	...
			  	for(var i=1; i<=rCnt; i++) {
			  		if(sheetObj.CellValue(i, "frt_rt_amt") == 0) {
				...
				if(sheetObj.IsDataModified ) {
					var rowM = sheetObj.ColValueDup("rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|rat_ut_cd|prc_cgo_tp_cd|curr_cd");
					if (rowM >= 0) {
    ...
    /**
      ...
      */         
       ...
       * @version 2009.07.30
       */
      function showSheetObj(svcScpCd){

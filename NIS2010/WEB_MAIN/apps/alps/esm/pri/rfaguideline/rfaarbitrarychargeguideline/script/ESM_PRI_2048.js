...
/**
 * @extends
 * @class ESM_PRI_2048 : ESM_PRI_2048 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //sheet1 init
		with (sheetObj) {
            ...
            InitDataProperty(0, cnt++ , dtData,    		60,    	daCenter,  	true, 	"rout_pnt_loc_def_cd",		false,	"",	dfNone, 		0,	true,	true,	5);
            ...
            InitDataProperty(0, cnt++ , dtCombo,      	60,    	daCenter,  	true,  	"rcv_de_term_cd",    		false, 	"",	dfNone, 		0,	true, 	true);
            InitDataProperty(0, cnt++ , dtData,   		60,    	daRight,  	true,  	"min_cgo_wgt",    			false,	"",	dfNullFloat,	2,	true,	true,	4);
            InitDataProperty(0, cnt++ , dtData,  		50,    	daRight,  	true,  	"max_cgo_wgt",      		false,	"", dfNullFloat, 	2,	true, 	true,	4);
            InitDataProperty(0, cnt++ , dtData,      	80,    	daCenter,  	true,  	"bse_port_def_cd",     		false,	"", dfNone, 		0,	true, 	true,	5);
            InitDataProperty(0, cnt++ , dtCombo,      	50,    	daCenter,  	true,  	"rat_ut_cd",     			false,	"", dfNone, 		0, 	true, 	true);
            ...
            InitDataProperty(0, cnt++ , dtData,   		50,    	daRight,  	true,  	"frt_rt_amt",         		false, 	"",	dfNullFloat, 	2, 	true,  	true,	9);
	...
	case 2: //컬럼 VALIDATION - 디비조회시 사용
		with (sheetObj) {
            ...
            InitDataProperty(0, cnt++ , dtHidden,      	60,    	daCenter,  	true,  	"bse_port_def_cd");
            InitDataProperty(0, cnt++ , dtHidden,      	60,    	daCenter,  	true,  	"rat_ut_cd");

            InitDataProperty(0, cnt++ , dtHidden,  		50,    	daCenter,  	true,  	"prc_cgo_tp_cd");
            InitDataProperty(0, cnt++ , dtHidden,    	40,    	daCenter,  	true, 	"curr_cd");
            InitDataProperty(0, cnt++ , dtHidden,   	50,    	daRight,  	true,  	"frt_rt_amt");
	...
	try {
		...
		switch (sAction) {
			...
			case IBSEARCH: //조회			
				...
				var sXml = sheetObj.GetSearchXml("ESM_PRI_2048GS.do", sParam);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case IBSAVE: // 저장
		...
		if (sheetObjects[0].IsDataModified) {
			...
			sXml = sheetObj.GetSearchXml("ESM_PRI_2001_03GS.do", FormQueryString(formObj));
...
function validateSheetData(sheetObj, color) {
	...
	for ( var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
		...
		if (sheetObj.CellValue(i, "frt_rt_amt") == ""
				|| sheetObj.CellValue(i, "frt_rt_amt").length > 10) {
...
function checkDBCodeExist(sheetObj, color) {
	...
	for ( var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
		...
		//CHECK버튼클릭시에는 '0','1'로구분되고, SAVE버튼클릭시에는 '0'일경우가 없기때문에 PASS

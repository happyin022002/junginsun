 ...
 ***************************************************************************************/
		...
		try {
			...
			case "btn_copy":
				if (validateForm(sheetObjects[0],document.form,IBCOPYROW)) {
					var sUrl = "/hanjin/ESM_PRI_2002.do?svc_scp_cd=" + getComboObject(comboObjects, "svc_scp_cd").Code + "&gline_seq=" + getComboObject(comboObjects, "gline_seq").Code;
					ComPriOpenWindowCenter(sUrl, "ESM_PRI_2002", 698, 275, true);
				}
	...
	function obj_deactivate() {
	    ...
	    if (srcName == "exp_dt" && formObject.exp_dt.value != "") {
	    	var effDt = comboObjects[1].Text;
	    	var expDt = formObject.exp_dt.value;
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
        try {
			...
			case IBSEARCH_ASYNC20: // 화면 로딩시 Tab Count 조회
				...
				subDataCnt = 0;
				var sXml = sheetObj.GetSearchXml("ESM_PRI_2001GS.do", FormQueryString(formObj));
			...
			case IBSAVE: // Save
				...
				var sXml = sheetObj.GetSaveXml("ESM_PRI_2001GS.do", FormQueryString(formObj));
				...
				formObj.f_cmd.value = SEARCH02;
				sXml = sheetObj.GetSearchXml("ESM_PRI_2001GS.do", FormQueryString(formObj));
	...
	function initTab(tabObj, tabNo) {
		...
		case 1:
			with (tabObj) {
				...
				var cnt = 0;
				InsertTab(-1, "Location Group ", 0);
				InsertTab(-1, "Commodity Group ", 1);
				InsertTab(-1, "Arbitrary", 2);
				InsertTab(-1, "Rate", 3);
	...
	function loadTabPage(tabIndex) {
		...
		if (sSvcScpCd != null && sSvcScpCd != "" && sGlineSeq != null && sGlineSeq != "" && parseInt(sGlineSeq) > 0) {
			...
			if (objTabWindow.tabLoadSheet) {
				...
				objTabWindow.tabLoadSheet( sSvcScpCd, sGlineSeq, isAproUsr && document.form.cfm_flg.value.toUpperCase() != "YES",formObj.eff_dt.value.replace(/[^0-9]/gi, ""));
			}
		}
    ...
    function getFrameTabId(tabIndex) {
        ...
        case 2:
            ...
            if (index == 0) {
                ...
                sUrl = "ESM_PRI_2001_05.do";
                frameId = "t4frame";
                divIndex = "tabLayer4";
            } else if (index == 2) {
                sUrl = "ESM_PRI_2001_12.do";
                frameId = "t5frame";
                divIndex = "tabLayer5";
            }               
            break;
    ...
    function checkArbiRate() {
        var rsltIndex = 0;
        var svcScpCd = getComboObject(comboObjects, "svc_scp_cd").Code;
          
        if(ComCheckGuideExcepSvcScpCd(svcScpCd)) {
            rsltIndex = 0;
        } else {
            //if ("AEW" == svcScpCd || "AEE" == svcScpCd) {
            //    rsltIndex = 1;
            //} else {
                rsltIndex = 2;
            //}

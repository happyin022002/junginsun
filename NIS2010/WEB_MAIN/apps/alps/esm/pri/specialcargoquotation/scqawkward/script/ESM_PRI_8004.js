...
* 2013.09.30 송호진 [CHM-201326912] CHM-201326410 변경에서 발생한 긴급반영에 대한 후속 조치 
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_8004 : ESM_PRI_8004 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_8004() {
...
var tmpRqstNo = "";
var simFlg = false; // 사용자가 내용을 수정해서 시뮬레이션 해봤는지 여부, true 면 pending/approval/reject 할 수 없음
...
document.onclick = processButtonClick;
	...
	function check_temporary_data_exists() {
		var formObj = document.form;
		if ( formObj.scq_ver_no_tmp.value != null && ComTrim(formObj.scq_ver_no_tmp.value) != '' ){
			doActionIBSheet ( sheetObjects[2], document.form, MULTI12 ); //삭제
        ...
        try {
            ...
            switch(srcName) {
                ...
                case "btn_pending":
                	...
                	sheetObjects[2].CellValue(1,"prog_sts_cd") = "P";
                	sheetObjects[2].CellValue(1,"pq_prog_sts_cd") = "P";
				...
				case "btn_approval":
					...
					preStsCd = sheetObjects[2].CellValue(1,"prog_sts_cd");
					sheetObjects[2].CellValue(1,"pre_prog_sts_cd") = sheetObjects[2].CellValue(1,"prog_sts_cd");
                	sheetObjects[2].CellValue(1,"prog_sts_cd") = "A";
                	sheetObjects[2].CellValue(1,"pq_prog_sts_cd") = "A";
                	doActionIBSheet(sheetObjects[0],document.form,MULTI10);
                	ComOpenWait(false);
                	toggleButtons(sheetObjects[2].CellValue(1,"prog_sts_cd"));
					toggleForms(sheetObjects[2].CellValue(1,"prog_sts_cd"));
				...
				case "btn_reject":
					...
					preStsCd = sheetObjects[2].CellValue(1,"prog_sts_cd");
					sheetObjects[2].CellValue(1,"pre_prog_sts_cd") = sheetObjects[2].CellValue(1,"prog_sts_cd");
                	sheetObjects[2].CellValue(1,"prog_sts_cd") = "R";
                	sheetObjects[2].CellValue(1,"pq_prog_sts_cd") = "R";
                	doActionIBSheet(sheetObjects[0],document.form,MULTI10);
                	ComOpenWait(false);
                	toggleButtons(sheetObjects[2].CellValue(1,"prog_sts_cd"));
					toggleForms(sheetObjects[2].CellValue(1,"prog_sts_cd"));
...
//    					formObj.pod_yd_cd.value = rtnVal[rtnVal.length-1].ib_yd_cd.substring(5,7);
                ...
                case "btn_customer2":
	                var rtnVal = ComPriOpenWindowCenter("/hanjin/ESM_PRI_4014.do?is_popup=true&nmd_cust_flg=N&cust_cnt_cd="+formObj.act_cust_cnt_cd.value+"&cust_seq="+formObj.act_cust_seq.value, "", 640, 465, true);
	            	//var rSC_no = formObj.sc_no.value;
	            	if (rtnVal != null){
				...
				case "btn_attachfile":
					var urlParam = "ESM_PRI_8101.do?p_scq_rqst_no="+formObj.scq_rqst_no.value
									+"&p_spcl_cgo_tp_cd=AK";
        			if(sheetObjects[2].CellValue(1,"auth") != "1"){
    ...
    */      
    ...
    function loadPage() {
		...
		if (document.form.p_rqstno.value){
			...
			ComOpenWait(true);
        	doActionIBSheet(sheetObjects[1], document.form, SEARCH04); // ver_no 조회
        	doActionIBSheet(sheetObjects[2], document.form, SEARCH01); // 히든 헤더 sheet 조회
			doActionIBSheet(sheetObjects[2], document.form, IBSEARCH); // Cargo sheet 조회
			doActionIBSheet(sheetObjects[1], document.form, SEARCH02); // Extar Handling Cost by Route 조회
			doActionIBSheet(sheetObjects[1], document.form, SEARCH07); // svc_scp 조회
			updateForm();
			ComOpenWait(false);
			toggleButtons(sheetObjects[2].CellValue(1,"prog_sts_cd"));
			toggleForms(sheetObjects[2].CellValue(1,"prog_sts_cd"));
    ...
    function initSheet(sheetObj,sheetNo) {
        ...
        if ( sheetObjects.length > 0 && sheetObjects[2].RowCount > 0 ) {
        	meas_sys_cd = sheetObjects[2].CellValue ( 1, "meas_sys_cd_vw");
        }
        ...
        switch(sheetNo) {
             case 1:      //sheet1 init
                with (sheetObj) {
                    ...
                    HeadTitle1 = "||TP\nSZ|Q'ty|cmdt_cd|Commodity|Overall("+len_unit+")|Overall("+len_unit+")|Overall("+len_unit+")|Over Dimension("+len_unit+")|Over Dimension("+len_unit+")|Over Dimension("+len_unit+")|Over Dimension("+len_unit+")|Over Dimension("+len_unit+")|"
										+"Gross\nWeight\n("+wgt_unit+")|ttl_dim_len|ttl_dim_wdt|ttl_dim_hgt|ovr_fwrd_len|ovr_bkwd_len|ovr_lf_len|ovr_rt_len|ovr_hgt|grs_wgt|Void\n(Q'ty)\n(Q2/Q4)|Proposal Rate\n(USD)|Proposal Rate\n(USD)|Approval Rate\n(USD)|Approval Rate\n(USD)|Total Approval Rate\n(USD)\n(Including Void)|scq_rqst_no|scq_ver_no|cgo_seq|scq_ver_no_tmp";
                    ...
                    InitDataProperty(0, cnt++,    dtData,    40,     daCenter,   true,     "cntr_qty",          false,     "",    dfNone,    0,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,  65,     daCenter,   true,     "cmdt_cd",        	false,     "",    dfNone,    0,    false,    false);
                    InitDataProperty(0, cnt++,    dtPopup,  105,     daLeft,   true,     "cmdt_nm",        	false,     "",    dfNone,    0,    false,    false);
                    
                    if ( meas_sys_cd == "M" || meas_sys_cd == "" ) {
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_len_vw",       false,     "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_wdt_vw",       false,     "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_hgt_vw",       false,     "",    dfNone,    0,    false,    false);
	                                        
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_fwrd_len_vw",      false,    "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_bkwd_len_vw",      false,    "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_lf_len_vw",        false,    "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_rt_len_vw",        false,    "",    dfNone,    0,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_hgt_vw",           false,    "",    dfNone,    0,    false,    false);
	                    
	                    InitDataProperty(0, cnt++,    dtData,    70,     daRight,    true,     "grs_wgt_vw",           false,     "",    dfNone,    0,    false,    false);
                    } else if ( meas_sys_cd == "I" ) {
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_len_vw",       false,     "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_wdt_vw",       false,     "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    50,     daRight,    false,    "ttl_dim_hgt_vw",       false,     "",    dfNullFloat,    2,    false,    false);
	                                        
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_fwrd_len_vw",      false,    "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_bkwd_len_vw",      false,    "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_lf_len_vw",        false,    "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_rt_len_vw",        false,    "",    dfNullFloat,    2,    false,    false);
	                    InitDataProperty(0, cnt++,    dtData,    40,     daRight,    false,    "ovr_hgt_vw",           false,    "",    dfNullFloat,    2,    false,    false);
	                    
	                    InitDataProperty(0, cnt++,    dtData,    70,     daRight,    true,     "grs_wgt_vw",           false,     "",    dfNullFloat,    2,    false,    false);                    	
                    }
                    InitDataProperty(0, cnt++,    dtHidden,    50,     daRight,    false,    "ttl_dim_len",       false,     "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    50,     daRight,    false,    "ttl_dim_wdt",       false,     "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    50,     daRight,    false,    "ttl_dim_hgt",       false,     "",    dfNullFloat,    2,    false,    false);
                                        
                    InitDataProperty(0, cnt++,    dtHidden,    40,     daRight,    false,    "ovr_fwrd_len",      false,    "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    40,     daRight,    false,    "ovr_bkwd_len",      false,    "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    40,     daRight,    false,    "ovr_lf_len",        false,    "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    40,     daRight,    false,    "ovr_rt_len",        false,    "",    dfNullFloat,    2,    false,    false);
                    InitDataProperty(0, cnt++,    dtHidden,    40,     daRight,    false,    "ovr_hgt",           false,    "",    dfNullFloat,    2,    false,    false);
                    
                    InitDataProperty(0, cnt++,    dtHidden,    70,     daRight,    true,     "grs_wgt",           false,     "",    dfNullFloat,    2,    false,    false);                    	
                    InitDataProperty(0, cnt++,    dtData,    60,     daCenter,   true,     "ovr_void_slt_qty", 	false,    "",    dfNone,    0,    false,    false);
                    
                    InitDataProperty(0, cnt++,    dtData,	 70,     daRight,    false,    "prop_bsrt_amt",     false,    "",    dfNone,    0,    false,    false);
                    InitDataProperty(0, cnt++,    dtData, 	 70,     daRight,    false,    "prop_void_rt_amt",  false,    "",    dfNone,    0,    false,    false);
                    InitDataProperty(0, cnt++,    dtData, 	 70,     daRight,    false,    "apro_bsrt_amt",     false,    "",    dfNone,    0,    true,   true);
                    InitDataProperty(0, cnt++,    dtData, 	 50,     daRight,    false,    "apro_void_rt_amt",  false,    "",    dfNone,    0,    true,   true);
					...
					if ( meas_sys_cd != "I" ) {
						...
						InitDataValid(0, "ovr_fwrd_len_vw", vtNumericOnly);
						InitDataValid(0, "ovr_bkwd_len_vw", vtNumericOnly);
						InitDataValid(0, "ovr_lf_len_vw", vtNumericOnly);
						InitDataValid(0, "ovr_rt_len_vw", vtNumericOnly);
						InitDataValid(0, "ovr_hgt_vw", vtNumericOnly);
             ...
             case 2:      //sheet3 init
                with (sheetObj) {
                    ...
                    var HeadTitle1 = "|Route|rout_tp_seq|Lane|Route|Route|Terminal Handling Cost|Terminal Handling Cost|Terminal Handling Cost|Terminal Handling Cost|Terminal Handling Cost|Terminal Handling Cost|T/S|T/S|Shuttle|Shuttle|Add-On|Add-On|scq_rqst_no|scq_ver_no|rout_seq|scq_ver_no_tmp|n_cost_zero_exist|w_cost_zero_exist|g_cost_zero_exist|t_cost_zero_exist|s_cost_zero_exist|a_cost_zero_exist";
                    var HeadTitle2 = "|Route|rout_tp_seq|Lane|In|Out|Normal|Normal|Wire|Wire|S.Gear|S.Gear|T/S|T/S|Shuttle|Shuttle|Add-On|Add-On||||";
					var headCount = ComCountHeadTitle(HeadTitle1);
             ...
             case 3:      //sheet3 init
                with (sheetObj) {
                    ...
                    "pg_prog_sts_cd|prog_ofc_cd|prog_usr_id|prog_usr_nm|prog_dt|prog_rmk|cust_lgl_eng_nm|prog_sts_nm|auth|"+
                    "pre_prog_sts_cd|rslt_flg|last_flg|act_cust_cnt_cd|act_cust_seq|act_cust_nm|scq_bid_flg|meas_sys_cd|meas_sys_cd_vw";
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        switch(sAction) {
        	...
        	case IBCREATE: // 신규생성 (New 버튼)
                ...
                formObj.prop_srep_cd_combo.RemoveAll();
                ComPriXml2ComboItem(sXml, formObj.prop_srep_cd_combo, "cd", "cd|nm");
                formObj.prop_srep_cd_combo.InsertItem(0, "", "");
                formObj.prop_srep_nm.value = "";
        	break;
        	
        	case MULTI10: // 헤더 시트, cargo 시트 ///////////////////////////////////////////////////
...
//	        		sheetObjects[1].CellValue(i,"scq_rqst_no") = formObj.scq_rqst_no.value;
//	        		sheetObjects[1].CellValue(i,"scq_ver_no") = formObj.scq_ver_no.value;
//	        		sheetObjects[1].CellValue(i,"rout_seq") = i-1;
//    			}
    			// sParam 셋팅
...
//				}
        	...
        	case SEARCH01:	// 헤더 정보 조회
				...
				// Header 의 MEAS _SYS_CD 와 관련하여 sheet 재 정림
				var sXml = "";
...
//    }   
	...
	function toggleButtons(mode) {
		...
		if (sheetObjects[2].CellValue(1,"auth") != "1"
			||sheetObjects[2].CellValue(1,"apro_ofc_cd") != document.form.in_usr_ofc_cd.value) { // 승인 권한 없으면
			*/
		if (!checkAuth()) { // 승인 권한 없으면
		...
		}else if ((liveVerNo != document.form.scq_ver_no.value) 
		     && (mode=="Q"||mode=="O")) { //liveVer 아니거나
			ComBtnEnable("btn_retrieve");
			ComBtnEnable("btn_new");
			ComBtnEnable("btn_sendmail");
			ComBtnEnable("btn_attachfile");
	...
	function toggleForms(mode) {
		...
		if(sheetObjects[2].CellValue(1,"delt_flg")=="Y"){
			mode = "Deleted";
		}
		/*
		if(sheetObjects[2].CellValue(1,"auth") == "1"
			&& sheetObjects[2].CellValue(1,"apro_ofc_cd") == document.form.in_usr_ofc_cd.value){//승인 권한 있으면
			*/
		if(checkAuth()){//승인 권한 있으면
			switch (mode) {
				case "Q":
				case "O":
				case "P":
					...
					toggleFormsClass(formObj.rmk_appr, 0);
	...
	function sendMail() {
		...
		var meas_sys_cd = "";
		if ( sheetObjects.length >= 3 && sheetObjects[2].RowCount > 0 ) {
			meas_sys_cd = sheetObjects[2].CellValue(1,"meas_sys_cd"); // [MEAS_CMB_ID].Code;
		}
		var len_unit = "";
		var wgt_unit = "";
		if ( meas_sys_cd == "M" || meas_sys_cd == "" ) {
		...
		content += "<br><br> --- ALPS link -------------------------------- "
				+ "<br>"
				+ "<a href='" + server_url + "hanjin/ESM_PRI_8004.do?p_rqstno="+formObj.scq_rqst_no.value+"'>"
				+ "Click Here</a><br>";
		*/
		//ts port 찾기
		var tmp_tsport = "";
		...
		content += "<br><br> --- Basic Information of Request ---------------------- "
				...
				+ "<br> 2. Approval Office             : "+formObj.apro_ofc_cd.value
		...
		//메일 제목
		var mailtitle = "Awkward Cargo Quotation for "+formObj.scq_rqst_no.value+" / POL : "+formObj.pol_cd.value+" POD : "+formObj.pod_cd.value + " / " + formObj.ctrt_pty_nm.value.substring ( 0, 15 );
    ...
    */  	    
     function sheet1_OnChange(sheetObj, Row, Col, Value)
     {
     	...
     	{
     		case "apro_bsrt_amt":
     		case "apro_void_rt_amt":
     			var total_basic = 0;
     			var total_void  = 0;
     			
	     		for(var i=2;i<=sheetObj.LastRow;i++){//mds
     				//20ft (void에 2를 더 곱함) - Box 단위로 바꾸면서 곱하기 2 제외시킴 - 로직 구분 필요 없음
	     			total_basic = total_basic + (sheetObj.CellValue(i,"cntr_qty")*sheetObj.CellValue(i,"apro_bsrt_amt"));
	     			total_void  = total_void  + ((sheetObj.CellValue(i,"cntr_qty")*sheetObj.CellValue(i,"apro_void_rt_amt"))*(sheetObj.CellValue(i,"ovr_void_slt_qty")*1));
	     		}
	     		ttlAmtCalculate(sheetObj, Row);
//	     		for(var i=2;i<=sheetObj.LastRow;i++){	
...
//     			}	
     			...
     			formObj.apro_basic_rate.value = total_basic;		
     			formObj.apro_void_rate.value = total_void;
     			break;	
...
function sheet2_OnPopupClick(sheetObj, Row, Col)
    ...
    function updateHdrSheet(){
		...
		//PRI_SCQ_PROG 를 위한 값		
		sheetObj.CellValue(1,"prog_rmk")       = document.form.rmk_appr.value;
		//sheetObj.CellValue(1,"prog_ofc_cd")    = document.form.in_usr_ofc_cd.value;
		//sheetObj.CellValue(1,"spcl_cgo_tp_cd") = "AK";
		
		sheetObj.CellValue(1,"rqst_srep_cd") = formObj.prop_srep_cd.value;
		sheetObj.CellValue(1,"svc_scp_cd")  = formObj.svc_scp_cd.value;
    ...
    function updateForm(){
		...
		formObj.prop_srep_cd.value = sheetObj.CellValue(1,"rqst_srep_cd");
		...
		comboObjects[MEAS_CMB_ID].Code2          = sheetObj.CellValue(1,"meas_sys_cd_vw"); // meas_sys_cd 의 OnChange Event 가 발생하지 않도록 한다. Event Handling Script 거 현재는 없지만 혹시모를 나중을 대비함.
     ...
     * <br><b>Example :</b>
     * <pre>
     *    prop_srep_cd_combo_OnChange(comboObj, code, text);
     * </pre>
     * @param   {IBMultiCombo} comboObj 필수 IBMultiCombo Object
     ...
     * @version 2009.04.17
     */     
    function prop_srep_cd_combo_OnChange(comboObj, code, text) {	
        var formObj = document.form;
        var sheetObj = sheetObjects[2];
...
//    		toggleForms("CREATE");  
	...
	function overDimensionSettingLength() {
		...
		var SumLength = sheetObjects[0].CellValue(row, "ttl_dim_len_vw");
		...
		if (front > 0) {
			...
			sheetObjects[0].CellValue(row, "ovr_fwrd_len_vw") = front;
//			document.getElementById("ovr_fwrd_len").value = sheetObjects[0].CellValue(row, "ovr_fwrd_len");
			lengthCnt++
		} else {
			sheetObjects[0].CellValue(row, "ovr_fwrd_len_vw") = "0";
//			document.getElementById("ovr_fwrd_len").value = "0";
		...
		if (rear > 0) {
			...
			sheetObjects[0].CellValue(row, "ovr_bkwd_len_vw") = rear;
//			document.getElementById("ovr_bkwd_len").value = sheetObjects[1].CellValue(row, "ovr_bkwd_len");
			lengthCnt++
		} else {
			sheetObjects[0].CellValue(row, "ovr_bkwd_len_vw") = "0";
//			document.getElementById("ovr_bkwd_len").value = "0";
	...
	function overDimensionSettingWidth() {
		...
		var row = sheetObjects[0].SelectRow;
		var SumWidth = sheetObjects[0].CellValue(row, "ttl_dim_wdt_vw");
		var cntr_tpsz_cd = sheetObjects[0].CellText(row, "cntr_tpsz_cd");
		var meas_sys_cd = comboObjects[MEAS_CMB_ID].Code;
		var O2_WDT = 0;
		var A2_WDT = 0;
		...
		if (left > 0) {
			...
			sheetObjects[0].CellValue(row, "ovr_lf_len_vw") = left;
//			document.getElementById("ovr_lf_len").value = sheetObjects[1].CellValue(row, "ovr_lf_len");
		} else {
			sheetObjects[0].CellValue(row, "ovr_lf_len_vw") = "0";
//			document.getElementById("ovr_lf_len").value = "0";
		...
		if (right > 0) {
			...
			sheetObjects[0].CellValue(row, "ovr_rt_len_vw") = right;
//			document.getElementById("ovr_rt_len").value = sheetObjects[1].CellValue(row, "ovr_rt_len");
		} else {
			sheetObjects[0].CellValue(row, "ovr_rt_len_vw") = "0";
//			document.getElementById("ovr_rt_len").value = "0";
	...
	function overDimensionSettingHeight() {
		...
		var row = sheetObjects[0].SelectRow;
		var SumHeight = sheetObjects[0].CellValue(row, "ttl_dim_hgt_vw");
		var cntr_tpsz_cd = sheetObjects[0].CellText(row, "cntr_tpsz_cd");
		var meas_sys_cd = comboObjects[MEAS_CMB_ID].Code;
		var F4_HGT = 0;
		var A2_HGT = 0;
		...
		if (height > 0) {
			...
			sheetObjects[0].CellValue(row, "ovr_hgt_vw") = height;
//			document.getElementById("ovr_hgt").value = sheetObjects[1].CellValue(row, "ovr_hgt");
		} else {
			sheetObjects[0].CellValue(row, "ovr_hgt_vw") = "0";
...
//		cornerPostStatus();
	...
	function voidSpaceValue() {
		...
		var row = sheetObjects[0].SelectRow;
		var width1 = sheetObjects[0].CellValue(row, "ovr_lf_len_vw");
		var width2 = sheetObjects[0].CellValue(row, "ovr_rt_len_vw");
		var height = sheetObjects[0].CellValue(row, "ovr_hgt_vw");
 ...
 */
	...
	function validateForm(sheetObj,formObj,sAction){
   			switch (sAction) {
	  			...
	  			case MULTI10: // 
	  				...
	  				if(sheetObjects[2].CellValue(1,"prog_sts_cd") != "P" 
	  					&& sheetObjects[2].CellValue(1,"prog_sts_cd") != "R"
	  					&& sheetObjects[2].CellValue(1,"prog_sts_cd") != "D"
	  					&& sheetObjects[2].CellValue(1,"prog_sts_cd") != "C" ){
		  				
		  				if (sheetObjects[2].CellValue(1,"apro_eff_dt") == "" 
		  					||sheetObjects[2].CellValue(1,"apro_exp_dt") == "") {
		  					ComShowCodeMessage("PRI09007"); //Mandatory field is missing.
		  					return false;
		  				}
		  				if(sheetObjects[2].CellValue(1,"apro_eff_dt") > sheetObjects[2].CellValue(1,"apro_exp_dt")){
		  					ComShowCodeMessage("PRI09009"); //Invalid duration
		  					return false;
		  				}
		  				if( sheetObjects[2].CellValue(1,"prop_eff_dt") != sheetObjects[2].CellValue(1,"apro_eff_dt")
		  					||sheetObjects[2].CellValue(1,"prop_exp_dt") != sheetObjects[2].CellValue(1,"apro_exp_dt") ){
						...
						if(sheetObjects[0].RowCount != 0){
				  			for(var i=2; i<=sheetObjects[0].LastRow; i++){	
				  				if ((sheetObjects[0].CellValue(i,"ibflag")!="D")
				  					&&(sheetObjects[0].CellValue(i,"apro_bsrt_amt") == ""
				  						||(sheetObjects[0].CellValue(i,"ovr_void_slt_qty") > "0" && sheetObjects[0].CellValue(i,"apro_void_rt_amt") == ""))){
			  				...
			  				for(var i=2; i<=sheetObjects[0].LastRow; i++){	
				  				if((sheetObjects[0].CellValue(i,"prop_bsrt_amt") != "" && sheetObjects[0].CellValue(i,"prop_bsrt_amt") != "0")
				  						&&(sheetObjects[0].CellValue(i,"prop_bsrt_amt") != sheetObjects[0].CellValue(i,"apro_bsrt_amt")) ){
				  					ComShowCodeMessage("PRI09022","Basic Rate"); //'{?msg1} not same. So Request can not Approved.'
				  					return false;
				  				}
				  				if(((sheetObjects[0].CellValue(i,"prop_void_rt_amt") != "" && sheetObjects[0].CellValue(i,"prop_void_rt_amt") != "0" )
				  						&& sheetObjects[0].CellValue(i,"ovr_void_slt_qty") > 0)
				  						&&(sheetObjects[0].CellValue(i,"prop_void_rt_amt") != sheetObjects[0].CellValue(i,"apro_void_rt_amt")) ){
				  					ComShowCodeMessage("PRI09022","Void Rate"); //'{?msg1} not same. So Request can not Approved.'
				  					return false;
				  				}
			  				}
						}
	  				...
	  				if(sheetObjects[2].CellValue(1,"prog_sts_cd") == "R" && sheetObjects[2].CellValue(1,"prog_rmk") == ""){
	...
	function setSheetsEditable(editable){
		if(editable && sheetObjects[2].CellValue(1,"delt_flg")!="Y"){
			for(var i=2; i<=sheetObjects[0].LastRow; i++){
				...
				sheetObjects[0].CellEditable(i,"apro_bsrt_amt") = true;   
				sheetObjects[0].CellEditable(i,"apro_void_rt_amt") = true;
				sheetObjects[0].CellBackColor(i,"apro_bsrt_amt") = sheetObj.RgbColor(205, 255, 255);
				if(sheetObjects[0].CellValue(i,"ovr_void_slt_qty")!=""
					&& sheetObjects[0].CellValue(i,"ovr_void_slt_qty")!="0"){
					sheetObjects[0].CellBackColor(i,"apro_void_rt_amt") = sheetObj.RgbColor(205, 255, 255);
				}
			}
			for(var i=2; i<=sheetObjects[1].LastRow; i++){
...
//				sheetObjects[0].CellEditable(i,"del_chk") = false;         
				...
				sheetObjects[0].CellEditable(i,"grs_wgt") = false;         
				sheetObjects[0].CellEditable(i,"ovr_void_slt_qty") = false;
				sheetObjects[0].CellEditable(i,"apro_bsrt_amt") = false;   
				sheetObjects[0].CellEditable(i,"apro_void_rt_amt") = false;
			}
			for(var i=2; i<=sheetObjects[1].LastRow; i++){
...
//		}
	...
	function cargoDetailDisplay() {
		...
		/*
		var n_cost = new Array();
		var w_cost = new Array();
		var g_cost = new Array();
		var t_cost = new Array();
		var s_cost = new Array();
		var a_cost = new Array();
		n_cost.length = fobjRout.TotalRows;
		w_cost.length = fobjRout.TotalRows;
		g_cost.length = fobjRout.TotalRows;
		t_cost.length = fobjRout.TotalRows;
		s_cost.length = fobjRout.TotalRows;
		a_cost.length = fobjRout.TotalRows;
		*/
    ...
    function meas_sys_cd_vw_OnChange(comboObj, code, text) {
    	...
    	sheetObjects[2].CellValue2( 1, "meas_sys_cd_vw") = code;
    ...
    function convertMeasSysCd ( sheetObj, code ){
    	...
    	for(var i=sheetObj.HeaderRows; i<=sheetObj.LastRow; i++){
    		...
    		sheetObj.CellValue2 ( i, "grs_wgt_vw"     ) = grs_wgt;

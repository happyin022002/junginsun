...
/**
 * @extends
 * @class ESM_PRI_2051 : ESM_PRI_2051 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_2051() {
	...
	try {
		switch (sAction) {
			case IBSEARCH: //조회	
				...
				var sXml = sheetObj.GetSearchXml("ESM_PRI_2051GS.do", sParam);
			...
			case IBSAVE: //저장
				...
				if((ComShowCodeConfirm("PRI00001")) ) {	
					...
					sheetObj.DoSave("ESM_PRI_2051GS.do", FormQueryString(formObj), -1, false);
				}
				break;
...
function validateSheetData(sheetObj, color) {
	...
	for ( var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
		...
		if (sheetObj.CellValue(i, "cust_def_cd") == "" && ComTrim(sheetObj.CellText(i, "cust_def_cd")) != "") {
		...
		if (sheetObj.CellValue(i, "prc_cgo_tp_cd") == "" && ComTrim(sheetObj.CellText(i, "prc_cgo_tp_cd")) != "") {
		...
		if (sheetObj.CellValue(i, "prop_frt_rt_amt") == "" || sheetObj.CellValue(i, "prop_frt_rt_amt") == 0 || sheetObj.CellValue(i, "prop_frt_rt_amt").length > 10) {
...
function funcGuidelineAmt(sheetObj, Row) {
	...
	var sXml = sheetObjects[1].getSearchXml("ESM_PRI_2003_05GS.do", sParam);

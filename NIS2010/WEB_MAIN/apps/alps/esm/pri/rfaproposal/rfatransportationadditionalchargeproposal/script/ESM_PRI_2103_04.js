...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_2103_04 : ESM_PRI_2103_04 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_2103_04() {
    	...
    	switch (sheetID) {
    		...
    		case "sheet1":
    			with (sheetObj) {
    				...
    				var HeadTitle = "state|Sel.|Seq.|Point|Description|Trans Mode|Term|Weight\n(Ton<=)|Weight\n(<Ton)|Base\nPort|Actual\nCustomer|Customer Name|Per|CGO\nType|Cur.|Proposal|C.Offer|Final|EFF Date|EXP Date|Source|Status|||||||||";
		...
		if (event.srcElement.name == "org_dest_tp_cd") {
			if(sheetObjects[0].IsDataModified) {
				if(ComShowCodeConfirm('PRI00006')) {
					doActionIBSheet(sheetObjects[0], formObj, MODIFY05);
				} else {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
					...
					if(chkArr.length > 0) {
						...
						for(j=0;j < chkArr.length;j++) {
							if(sheetObj.CellValue(chkArr[j]+sRow,"n1st_cmnc_amdt_seq")!=amdt_seq){
								amendRow(sheetObj,formObj,chkArr[j]+sRow,"D","rout_pnt_loc_def_cd|rout_pnt_loc_def_nm|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|" +
						...
						if(sheetObj.CellValue(sheetObj.SelectRow,"n1st_cmnc_amdt_seq") == amdt_seq) {
							...
							amendRow(sheetObj,formObj,sheetObj.SelectRow,"D","rout_pnt_loc_def_cd|rout_pnt_loc_def_nm|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|" +
							"cust_cnt_cd|rat_ut_cd|prc_cgo_tp_cd|curr_cd|prop_frt_rt_amt|coffr_frt_rt_amt|fnl_frt_rt_amt");
						}
					}					
				}
			...
			case MODIFY01:	//Accept
				...
				acceptRows(sheetObjects[0], document.form, "ESM_PRI_2003_04GS.do");
				ComOpenWait(false);
				break;
			...
			case MODIFY02:	//Accept Cancel
				...
				acceptCancelRows(sheetObjects[0], document.form, "ESM_PRI_2003_04GS.do");
				ComOpenWait(false);
				break;
			...
			case MODIFY03:	//Accept All
				...
				acceptAllRows(sheetObjects[0], document.form, "ESM_PRI_2003_04GS.do");
				ComOpenWait(false);
				break;
			...
			case MODIFY04:	//Cancel All
				...
				acceptCancelAllRows(sheetObjects[0], document.form, "ESM_PRI_2003_04GS.do");
				ComOpenWait(false);
				break;
			...
			case COMMAND01:	//Amend
				...
				if(chkArr.length > 0){
					if(chkArr.length > 1){					
						...
						amendRow(sheetObjects[0], document.form, sheetObjects[0].SelectRow, "M", "rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|" +
			...
			case COMMAND02: // Amend Cancel
				...
				if(chkArr.length > 0){
					if(chkArr.length > 1){					
						...
						amendCancelRow(sheetObjects[0], document.form, sheetObjects[0].SelectRow, "M", "rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|" +
							"min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|cust_cnt_cd|rat_ut_cd|prc_cgo_tp_cd|curr_cd|prop_frt_rt_amt");
					}
				}else{ 
					amendCancelRow(sheetObjects[0], document.form, sheetObjects[0].SelectRow, "M", "rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|" +
			...
			case IBLOADEXCEL:      //upload excel
				...
				var sUrl = "/hanjin/ESM_PRI_2050.do?";
				sUrl += "prop_no="+propNO;
	...
	function amendRow(sheetObj, formObj, sRow, sAction, sCols) {
		...
		var sColNames	 = "rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd" +
				"|cust_cnt_cd|rat_ut_cd|prc_cgo_tp_cd|curr_cd|prop_frt_rt_amt";
...
//			sheetObj.CellValue2(preRow, "ibflag") = "U";
		...
		switch (sAction) {
			case IBSEARCH: // 조회			
  			...
  			case IBSAVE: // 저장
				...
				if(amdtSeq == 0) {
					var rowM = sheetObjects[0].ColValueDup("amdt_seq|rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|cust_cnt_cd|rat_ut_cd|prc_cgo_tp_cd|cust_cnt_cd|curr_cd", false);
					if (rowM >= 0) {
						ComShowCodeMessage("PRI00303", "Sheet", rowM);
						return false;
					}
				} else {
					var dupRow = ComPriAmendDupCheck(sheetObjects[0], "amdt_seq|rout_pnt_loc_def_cd|prc_trsp_mod_cd|rcv_de_term_cd|min_cgo_wgt|max_cgo_wgt|bse_port_def_cd|cust_cnt_cd|rat_ut_cd|prc_cgo_tp_cd|cust_cnt_cd|curr_cd", amdtSeq);
					if(dupRow >= 0) {

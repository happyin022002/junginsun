...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_2041_04 : ESM_PRI_2041_04 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_2041_04() {
		...
		switch (sheetID) {
			case "sheet1":
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtHidden, 		40, daLeft, true, "svc_scp_cd", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtHidden, 		40, daLeft, true, "grp_loc_seq", false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtCombo, 		70, daCenter, 	true, "org_dest_tp_cd",	false, "", dfNone, 		0, false, 	false);
					InitDataProperty(0, cnt++, dtData, 			90, daCenter, true, "prc_grp_loc_cd", false, "", dfEngUpKey, 0, false, false, 4, true);
					InitDataProperty(0, cnt++, dtData, 			120, daLeft, true, "prc_grp_loc_desc", false, "", dfEngKey, 0, false, false, 100);
					InitDataProperty(0, cnt++, dtHidden, 		20, daLeft, true, "prc_prog_sts_cd", false, "", dfEngKey, 0, false, false, 100);
					InitDataProperty(0, cnt++, dtHidden, 		20, daLeft, true, "src_info_cd", false, "", dfEngKey, 0, false, false, 100);
			...
			case "sheet2":
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtHidden, 		90, daLeft, 	true, "amdt_seq", 		false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtHidden, 		90, daLeft, 	true, "svc_scp_cd", 	false, "", dfNone, 0, false, false);					
					InitDataProperty(0, cnt++, dtHidden, 		90, daLeft, 	true, "grp_loc_seq", 	false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtHidden, 		90, daLeft, 	true, "grp_loc_dtl_seq",false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtData, 			100,daCenter, 	true, "loc_cd", 		false, "", dfEngUpKey, 0, false, false, 5, true);
					InitDataProperty(0, cnt++, dtData, 			170,daLeft, 	true, "loc_nm", 		false, "", dfEngKey, 0, false, false);
					...
					InitDataProperty(0, cnt++, dtCombo, 		60, daCenter, 	true, "src_info_cd", 	false, "", dfNone, 0, false, false);
					InitDataProperty(0, cnt++, dtCombo, 		60, daCenter, 	true, "prc_prog_sts_cd",false, "", dfNone, 0, false, false);
		...
		try {
			...
			switch (sAction) {
				case IBSEARCH: // 조회		
					...
					sheetObj.DoSearch("ESM_PRI_2041_04GS.do" , FormQueryString(formObj));
					break;
...
//			formObject.dur_dup_flg.value = sDurDupFlg ;
	...
	function sheet2_OnSearchEnd(sheetObj, errMsg){
		...
		if(amdt_seq!=0){
			for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
				...
				else if(sheetObj.CellValue(i,"amdt_seq") == sheetObj.CellValue(i,"n1st_cmnc_amdt_seq")){
	...
	*/		
	function applyMasterStyle(sheetObj){
		...
		var newCnt = ComPriSheetFilterRows(sheetObj, "src_info_cd|eff_dt", "NW|"+eff_dt);
		var ndlCnt = ComPriSheetFilterRows(sheetObj, "src_info_cd|ibflag", "NW|D");
		var amdCnt = ComPriSheetFilterRows(sheetObj, "amdt_seq|eff_dt", amdt_seq+"|"+eff_dt);
		var delCnt = ComPriSheetFilterRows(sheetObj, "src_info_cd|amdt_seq", "AD|"+amdt_seq);
		var actCnt = ComPriSheetFilterRows(sheetObj, "eff_dt|prc_prog_sts_cd", eff_dt+"|A");

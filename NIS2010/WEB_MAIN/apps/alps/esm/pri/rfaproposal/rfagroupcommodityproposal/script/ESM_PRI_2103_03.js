...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_2103_03 : ESM_PRI_2103_03 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_2103_03() {
			...
			case "sheet1":
				with (sheetObj) {
					...
					// SAVESTATUS, FORMATFIX]
                     ...
                     InitDataProperty(0, cnt++ , dtData,    		90,   	daCenter,  	false,	"prc_grp_cmdt_cd",			true,           "",      dfNone, 			0,     false,     false, 5);
 					 InitDataProperty(0, cnt++ , dtData,     		150,   	daLeft, 	false,	"prc_grp_cmdt_desc",		true,           "",      dfNone, 			0,     false,     false);
	                 ...
	                 InitDataProperty(0, cnt++ , dtHidden,    	 	20, 	daCenter,  	false,  "prc_prog_sts_cd");
 					 ...
 					 InitDataValid(0, "prc_grp_cmdt_cd", vtEngUpOther,"0123456789");
 					 InitDataValid(0, "prc_grp_cmdt_desc", vtExceptChar,"/");
 					 ShowButtonImage = 2;
				...
				with (sheetObj) {
					...
					// SAVESTATUS, FORMATFIX]
	                 InitDataProperty(0, cnt++ , dtHiddenStatus,	30,    	daCenter,	false,	"ibflag");
 					 ...
 					 InitDataProperty(0, cnt++ , dtCombo,    	 	85, 	daCenter,  	false,  "prc_cmdt_tp_cd",	true,          	"",      dfNone, 			0,     false, 	false);
					 InitDataProperty(0, cnt++ , dtPopupEdit, 		85,    	daCenter,  	false,	"prc_cmdt_def_cd",	true,          	"",      dfNone, 			0,     false,	false, 6);
 					 InitDataProperty(0, cnt++ , dtData,     		250,   	daLeft,  	false,	"prc_cmdt_def_nm",	false,          "",      dfNone, 			0,     false, 	false);
		...
		try {
			...
			switch (sAction) {
				case IBSEARCH: // 조회
					...
					sheetObj.DoSearch("ESM_PRI_2003_03GS.do" , FormQueryString(formObj));
					buttonControl();
					break;
				...
				case IBSEARCH_ASYNC02: // 조회 - Group 삭제시 rate 사용여부 확인
					if (validateForm(sheetObj,document.form,sAction)) {
						...
						var sXml = sheetObj.GetSearchXml("ESM_PRI_2003_03GS.do", sParam);
				...
				case IBSAVE: // 저장
					...
					if (!supressConfirm && !ComPriConfirmSave()) {
	  				...
	  				ComOpenWait(true);
					var sXml = sheetObj.GetSaveXml("ESM_PRI_2003_03GS.do", sParam);
					...
					if(amdt_seq=="0"){
						...
						if (sheetObj.id == "sheet1") {
							...
							for(j=0;j < chkArr.length;j++){
								...
								if(sheetObj.CellValue(chkArr[j],"src_info_cd")=="NW"||sheetObj.RowStatus(chkArr[j])=="I"){
									sheetObj.CellValue2(chkArr[j],"chk")= "1";
								} else {
							...
							comChangeValue(sheetObj, "ibflag", "D", "chk", "1");
						}else{
				...
				case IBCOPYROW: // Guideline Copy
					if (!supressConfirm && !ComShowCodeConfirm("PRI03006")) {
						...
						var sParam = FormQueryString(formObj);
						var sXml = sheetObj.GetSaveXml("ESM_PRI_2003_03GS.do", sParam+"&prc_cust_tp_cd="+parent.comboObjects[2].Code);
				...
				case COMMAND01: // Amend
					...
					if(chkArr.length > 0){
						...
						comSheetAmendRow(sheetObj,formObj,sheetObj.SelectRow,"M","prc_cmdt_tp_cd|prc_cmdt_def_cd");					
					}
					sheetObj.SelectCell(sheetObj.SelectRow, "prc_cmdt_def_cd");
				...
				case COMMAND02: // Amend Cancel
					...
					if(chkArr.length > 0){
						...
						comSheetAmendCancelRow(sheetObj,formObj,sheetObj.SelectRow,"M", "prc_cmdt_tp_cd|prc_cmdt_def_cd");					
					}
		...
		var prcCmdtTpCd = sheetObj.CellValue(Row,"prc_cmdt_tp_cd");
       	...
       	switch(colName)
       	{
   	    	case "prc_cmdt_def_cd":
   	    		...
   	    		var sts = sheetObj.CellValue(Row, "src_info_cd");
   	    		var sUrl = "/hanjin/ESM_PRI_4027.do?prc_cmdt_tp_cd="+ prcCmdtTpCd + "&commodity_cmd=CR&grp_cd="+PRI_RP_SCP;
...
//			formObject.dur_dup_flg.value = sDurDupFlg ;
    ...
    */	
	...
	function sheet2_OnSearchEnd(sheetObj, errMsg){
		...
		if(sts=="I"){
			if(amdt_seq==0){
				 sheetObj.InitDataProperty(0, 6, dtCombo,    85, daCenter, false, "prc_cmdt_tp_cd",  true, "", dfNone, 0, true, true);
				 sheetObj.InitDataProperty(0, 7, dtPopupEdit, 85, daCenter, false, "prc_cmdt_def_cd", true, "", dfNone,	0, true, true, 6);				
			}else{
				 sheetObj.InitDataProperty(0, 6, dtCombo,    85, daCenter, false, "prc_cmdt_tp_cd",  true, "", dfNone, 0, false, true);
				 sheetObj.InitDataProperty(0, 7, dtPopupEdit, 85, daCenter, false, "prc_cmdt_def_cd", true, "", dfNone,	0, false, true, 6);
			}
		}else{
			 sheetObj.InitDataProperty(0, 6, dtCombo,    85, daCenter, false, "prc_cmdt_tp_cd",  true, "", dfNone, 0, false, false);
			 sheetObj.InitDataProperty(0, 7, dtPopupEdit, 85, daCenter, false, "prc_cmdt_def_cd", true, "", dfNone,	0, false, false, 6);
		}
		...
		if(amdt_seq!=0){
			for(i=1 ; i < sheetObj.Rows; i++){
				if(sheetObj.CellValue(i,"amdt_seq") != amdt_seq){ 
					...
					if(sts == "I"){
						sheetObj.CellEditable(i,"prc_cmdt_tp_cd") = true;		
    ...
    */	
	function buttonControl(mode){
		...
		try{
			...
			sheetObjects[0].InitDataProperty(0, 8, dtData, 150, daLeft, false, "prc_grp_cmdt_desc", true, "", dfEngKey, 0, false, false, 100);
			sheetObjects[1].InitDataProperty(0, 6, dtCombo,    85, daCenter, false, "prc_cmdt_tp_cd",  true, "", dfNone, 0, false, false);
			sheetObjects[1].InitDataProperty(0, 7, dtPopupEdit, 85, daCenter, false,"prc_cmdt_def_cd", 	true, "", dfNone, 	0, false, false, 6);

...
* 2014.12.01 최성환 [CHM-201432700] Retroactive RFA Minimization관련 시스템 개발요청
* 2015.04.22 전지예 [CHM-201535165] RFA match back 팝업화면 추가
* 2015.05.06 전지예 [CHM-201535708] Duration 화면의 문구 변경
* 2015.06.02 전지예 [CHM-201536176] [RFA] Retroactive RFA일 경우 팝업메세지 삽입
...
* 2016.05.13 [CHM-201641607] 구주 지역 RFA Creation 화면의 Rate 화면 로딩 관련
* 2016.07.20 [CHM-201642287] Master RFA Cancel시 Basic RFA 점검 로직 개발 요청
* 2016.07.28 [CHM-201642541] Contract RFA 상에서 Request Office 변경 정정기능
=========================================================*/
/**************************************************************************************** 
    ...
    function processButtonClick() {
        ...
        try {
            ...
            case "btn_save":    
                ...
                //현재 RfaCtrtTpCd = B.   
                if(sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") != "" && sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") == "B" 
            		&&  sheetObjects[0].CellValue(1,"prop_sts_cd") == "I") {
                	
	            	//시작일자 : 오늘부터 시작~
	            	//종료일자 : Master Exp를 넘어서는 안됨.
	            	formObj.f_cmd.value = SEARCH25;
	            	//뒤자리 달력에서 입력한 값이 더 작으면 에러
	            	if (ComGetDaysBetween(  ComGetNowInfo('ymd', '')  , sheetObjects[0].CellValue(1, "ctrt_eff_dt")  ) < 0) {
	            		ComShowCodeMessage('PRI01160'); //msgs['PRI01160'] = 'Retroactive Filing is not allowed.';
	            		formObj.ctrt_eff_dt.focus();
	            		return; 
	            	} 
	            	//Master Duration Exp 보다 이전의 값을 선택해야한다. Exp_dt 보다 초과해서는 안된다.
	            	...
	            	if (ComGetDaysBetween(  sheetObjects[0].CellValue(1, "ctrt_exp_dt")  , check_org_exp_dt ) < 0) {
            ...
            case "btn_amend":   
                ...
                var rtnVal = ComPriOpenWindowCenter("/hanjin/ESM_PRI_2040.do?"+sParam, "ESM_PRI_2040", 700, 225, true);   

                if (rtnVal != null && rtnVal =="Y"){
                	// Amend 시 Arbitrary G/L Amt.를 최신으로 갱신
...
//                    doActionIBSheet(sheetObjects[0],document.form,IBSEARCH);
            ...
            // Basic RFA용 Amend (CHM-201642287)
            case "btn_auto_amend":
            	...
            	var sMst_Rfa_No = sheetObj.CellValue(1,"mst_rfa_no");
            	var sEffDt = ComGetMaskedValue(ComGetDateAdd(sheetObj.CellValue(1,"eff_dt"), "D", 1),"ymd");
            	var sExpDt = ComGetMaskedValue(sheetObj.CellValue(1, "exp_dt"),"ymd");
            	
            	var sParam = FormQueryString(formObj) + "&mst_rfa_no=" + sMst_Rfa_No + "&eff_dt=" + sEffDt + "&exp_dt=" + sExpDt;
                var rtnVal = ComPriOpenWindowCenter("/hanjin/ESM_PRI_2017.do?"+sParam, "ESM_PRI_2017",1100, 450, true);
              ...
              //RFA 효율화를 위한 요청 (1차) (CHM-201640671)
            case "btn_request":
            	//rfa_ctrt_tp_cd = 'B' (Basic) && mst_rfa_no is not null 조건 충족할 경우에만 바로 Approve
            	if(sheetObjects[0].CellValue(1,"mst_rfa_no") != "" && sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") == "B" 
            		&&  sheetObjects[0].CellValue(1,"prop_sts_cd") == "I") {
            		doActionIBSheet(sheetObjects[0],document.form,COMMAND09); //기존거를 하나로 통합해서 사용 one transaction use 
...
//                    ComOpenWait(false); //->waiting->End
            ...
            case "btn_dem_pop":            
                ...
                var sUrl = "/hanjin/EES_DMT_2003.do?" + "prop_no="+formObj.prop_no.value 
                		 + "&amdt_seq=" + formObj.amdt_seq.value + "&caller=2003";
                var rtnVal = ComPriOpenWindowCenter(sUrl, "EES_DMT_2003", 1010, 700, true);
            ...
            case "btn_afil_pop":
                ...
                var sParam = getParameters(srcName, "");                
                var rtnVal = ComPriOpenWindowCenter("/hanjin/ESM_PRI_2003_06.do?"+sParam, "ESM_PRI_2003_06", 1020, 315, true);
    ...
    */
    function loadPage() {
        ...
        //=======================================================================================================================
        //RFA 효율화를 위한 요청 (1차) (CHM-201640671)
        //master에서 온것과 조회시 타입 B 인것 두가지로 구분해서 기존가 분리해서 사용. 
        //else if(form.mst_prop_no.value != "") {
        else if("null" != form.mst_prop_no.value && null != form.mst_prop_no.value && "" != form.mst_prop_no.value) {
        	//Master -> Basic 으로 copy onLoad 시 호출
        	// 전달화면에서 mst_rfa_no 를 파람으로 전달 해야함.
        	form.prop_no.value = form.mst_prop_no.value;
        	doActionIBSheet(sheetObjects[0],form,IBSEARCH); //조회 후 SearchEnd Type = B 일경우
    ...
    /**
     * [Basic]화면의 모든 버튼들의 Enable/Disable을 처리하는 함수. <br> 기존 : buttonControl()
     */
    function basicInitControl() {
    	var formObj = document.form;
    	// Rate tab focus   	
    	tabObjects[0].SelectedIndex = 3;  //Rate Tab 활성화 선택 NYC160015
...
//    	tabObjects[0].TabEnable(3) = true;
        ...
        showButton("btn_auto_amend");
        
        if(sheetObjects[0].CellValue(1,"mst_rfa_no") != "" && sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") == "B" & sheetObjects[0].CellValue(1,"prop_sts_cd") == "I"){
        	//활성화
        ...
        } else if(sheetObjects[0].CellValue(1,"mst_rfa_no") != "" && sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") == "B" & sheetObjects[0].CellValue(1,"prop_sts_cd") == "A"){
        	//비활성화
        ...
        //BASIC도 DEM/DET기능 추가 되었음. 송민석 20170531
        //ComBtnDisable("btn_dem_pop"); 
        //document.getElementById("btn_dem").style.color = "";
        ComBtnDisable("btn_free_pop"); 
        document.getElementById("btn_free").style.color ="";
        ComBtnDisable("btn_afil_pop"); 
        document.getElementById("btn_afil").style.color = "";
                
        //document.getElementById("btn_dur").style.color ="";
		...
		*/
		...
		if(sheetObjects[0].CellValue(1,"mst_rfa_no") != "" && sheetObjects[0].CellValue(1,"rfa_ctrt_tp_cd") == "B" & sheetObjects[0].CellValue(1,"prop_sts_cd") == "I"){
			 //변경가능
			formObj.ctrt_eff_dt.readOnly = false;	//변경가능
	        formObj.ctrt_exp_dt.readOnly = false; //변경가능   
	        formObj.prop_srep_cd.enable = true; //변경가능
	        // Master RFA Cancel시 Basic RFA 점검 로직 개발 요청 (CHM-201642287) 
	        // 0회차에서만 Basic으로 변경 가능
	        if(sheetObjects[0].CellValue(1,"amdt_seq") == "0") {
	        	//formObj.rfa_ctrt_tp_cd.enable = true;
	            //20170626 0일때도 수정 못하도록 변경함.
		...
		//rate tab 자동 호출
    	var frameId = "t9frame";
    	var sUrl = "ESM_PRI_2003_13.do";
    	var divIndex = "tabLayer9";
    ...
    function obj_deactivate() {
        ...
        switch(eleName){
            ...
            case "prop_mvc":
                ComChkObjValid(event.srcElement);   
                break;                     
            case "prop_ofc_cd":	// [CHM-201642541] Request Office 변경 시 Sales Rep.콤보 변경
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        try{
            ...
            case IBSEARCH: // 조회    
                ...
                //RFA 효율화를 위한 요청 (1차) (CHM-201640671)
                // Basic + Initial
                if(sheetObj.CellValue(1,"mst_rfa_no") != "" &&  sheetObj.CellValue(1,"rfa_ctrt_tp_cd") == "B"){
                	basicInitControl();
                	
                	// Basic RFA용 Amend (CHM-201642287)
                	if(sheetObj.CellValue(1,"prop_sts_cd") == "A") {
        		    	formObj.f_cmd.value = SEARCH30;
        		        var sXml = sheetObjects[0].GetSearchXml("ESM_PRI_2003GS.do", FormQueryString(formObj));
        		        var amendableYN = ComGetEtcData(sXml, "amendable_YN");
                		
                		// Basic일 경우 버튼 활성화 & Master RFA가 변경되었는지 여부 체크
                		// btn_auto_amend 이 Master가 변경된게 있으면 Red없으면 Black
        		        if(amendableYN == "Y") {
        		        	ComBtnEnable("btn_auto_amend");
        		        } else {
        		        	ComBtnDisable("btn_auto_amend");
        		        }
                ...
                //M- > B/A/C 로 카피해온 Amd#0의 Initial 상태에서는 Copy 버튼 비활성화 되어야 함.
                if(sheetObj.CellValue(1,"mst_rfa_no") != "" &&  sheetObj.CellValue(1,"amdt_seq") == "0" && sheetObj.CellValue(1,"prop_sts_cd") == "I"){     
                    ComBtnDisable("btn_copy");
                }
                //M- > B/A/C 로 카피해온 Amd#0의 Initial 상태에서는 Type B 아니면 변경 타입 변경 불가 - 비활성화.
                if(sheetObj.CellValue(1,"mst_rfa_no") != "" && sheetObj.CellValue(1,"rfa_ctrt_tp_cd") != "B" &&  sheetObj.CellValue(1,"amdt_seq") == "0" && sheetObj.CellValue(1,"prop_sts_cd") == "I"){     
                    formObj.rfa_ctrt_tp_cd.enable = false;
                }
            ...
            case IBCREATE: // New
                ...
                sheetObj.CellValue2(1, "rfa_ctrt_tp_cd") = "S";
...
//                }  
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////
            //RFA 효율화를 위한 요청 (1차) (CHM-201640671)                
            case COMMAND09: //Request + Approve
            	ComOpenWait(true); //->waiting->start
            	if (!validateForm(sheetObj,document.form,sAction)) {  
    	        	return false;
    	        }
    ...
    function initSheet(sheetObj, sheetNo) {
        ...
        case "sheet1":
            with (sheetObj) {
                ...
                HeadTitle += "|dmdt_ft_tp_cd|so_kup|dmdt_ft_tp_cd_ori|apro_usr_flg_ori|act_cm|est_cm|cre_tp|copy_auth|upd_dt|rfa_ctrt_tp_cd|mst_rfa_no|mst_amdt_seq"
                ...
                //add Master
                InitDataProperty(0, cnt++, dtData, 90, daLeft, false, "mst_rfa_no", false, "", dfNone, 0, false, false);
                InitDataProperty(0, cnt++, dtData, 90, daLeft, false, "mst_amdt_seq", false, "", dfNone, 0, false, false);
        ...
        case "sheet2":
            with (sheetObj) {
                ...
                var HeadTitle =  "|Sel.|Seq.|Prop No.|Amendment Seq|SVC Scope|Duration|Duration|Duration|Target MQC|Unit";
                    ...
                    HeadTitle += "|Status|req_usr_flg|apro_usr_flg|prc_prog_sts_cd|src_info_cd|submqcori|submqcutori|DUR_DUP_FLG|pre_ext_scp";
                var headCount = ComCountHeadTitle(HeadTitle);
                    // 컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                    InitColumnInfo(headCount, 0, 0, true);
                // 해더행정보[필수][ROW,HEADTEXT,ROWMERGE=false, HIDDEN=false]
                ...
                InitDataProperty(0, cnt++, dtData, 			110, daCenter, false, "tgt_mvc_qty", 	  false, "", dfInteger, 0, false, false);
                InitDataProperty(0, cnt++, dtCombo,  		60,  daCenter, false, "cntr_lod_ut_cd",   false, "", dfNone,    0, false, false);
                ...
                InitDataProperty(0, cnt++, dtData,   		110, daCenter, false, "prop_scp_srep_cd", true,  "", dfNone, 	0, false, false);
                ...
                InitDataProperty(0, cnt++, dtHidden, 		100, daCenter, false, "cntr_lod_ut_cd_ori",false, "", dfNone, 0, false, false);//2016-07-18 TEU/FEU Unit Conversion으로 인한 추가
    ...
    */
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case COMMAND02://REQUEST 
	        ...
	        // 영업사원이 소급인 RFA를 Request 할 때
	        if(soKup == 'Y' && (req_usr_flg=="Y" || apro_usr_flg=="Y")) {
                if (!ComShowCodeConfirm("PRI07056")) {
                    return false;
                }
	        }
        ...
        case COMMAND09: //Request + Approve	
	        ...
	        if (!ctrtDateMatch){
	           ...
	           //DEM/DET CHECK BASIC DEM/DET 추가
            if ( checkDmdtData() !="Y"){
                return false;
            }    
            //MASTER RFA의 DEM/DET 새 버전이 있는지 확인
            if ( checkDemDetExceptionNewVer() =="Y"){
                ComShowCodeMessage("PRI07067");
                return false;
            }    
                        
            
	        
	        // Auto Amend한 당일이 아닐 경우 Request + Approve 진행할 수 없다. (CHM-201642287)
	        if(formObj.amdt_seq.value != "0") {
	        	if(ComGetDaysBetween( ComGetNowInfo('ymd', ''),  ComGetUnMaskedValue(formObj.cre_dt.value, "ymd") ) != 0) {
	        		ComShowCodeMessage('PRI07066'); // msgs['PRI07066'] = 'Please cancel Basic RFA initial status and go to Basic RFA Auto Amend.';
	        		return false;
	        	}
	        }
        ...
        case COMMAND04: // Approve        
	        ...
	        var checkRetroYN = checkRetroactiveExist(); 
	        //Retroactive 대상 RFA : HKG14A1672,  Retroactive 비대상 : BNE14A0019
	        //Y : Retroactive RFA 대상이여서 Retroactive 사유를 입력할 수 있는 자동 팝업 오픈 
	        if(checkRetroYN == 'Y') {
	        	...
	        	var sAmdtSeq = sheetObj.CellValue(1, "amdt_seq");
	        	var sParam = "prop_no="+sPropNo+"&amdt_seq="+sAmdtSeq;
	        	var rtnVal = ComPriOpenWindowCenter("/hanjin/ESM_PRI_2045.do?"+sParam, "ESM_PRI_2045", 650, 500, true);   
	        	//팝업 창에서 정상적으로 저장이 되었다면 Y 로 리턴 받고 다음 단계로 진행함.
	        	if (rtnVal == undefined || rtnVal != "Y"){
	        		//정상적으로 저장하지 않은 상태로 팝업에서 종료 했을 경우는 승인단계를 정지함.  
		        	return false;
	        	}
	        } //End of the checkRetroactiveExist() clause.
			...
			// 하위 탭 승인되지 않았을 경우 Summary 팝업 
			if (rValue != "Y" || rValue == "SCP") {
				var sheetObj1 = sheetObjects[0];
				...
				var sParam = FormQueryString(formObj) + "&pre_amdt_seq=" + sPreAmdtSeq + "&prc_prop_sts_cd=" + sPropStsCd + "&eff_dt=" + sEffDt + "&exp_dt=" + sExpDt;
				sParam+= "&pre_exp_dt=" + sPreExpDt+ "&is_req_usr=" + sIsReqUsr + "&is_apro_usr=" + sIsAproUsr + "&dur_dup_flg=" + sDurDupFlg;
				var iWidth = 1024;
				var iHeight = 700;
				...
				var sFeatures = "scroll:auto;status:no;help:no;dialogWidth:" + iWidth + "px;dialogHeight:" + iHeight + "px;dialogLeft:" + leftpos + ";dialogTop:" + toppos;
				var rtnVal = ComOpenWindow("/hanjin/ESM_PRI_2036.do?" + sParam, "ESM_PRI_2036", sFeatures, true);
				
				// 정상적으로 저장하지 않은 상태로 팝업에서 종료 했을 경우는 승인단계를 정지함.
				if (typeof rtnVal == "undefined" || rtnVal != "Y"){
					return false;
				}
			}
        ...
        case IBSAVE: // Save
            ...
            //RFA 효율화를 위한 요청 (1차) (CHM-201640671) 
            //S/C 만 사용 (Basic 이 아닐 경우만 확인)
	        if( getRfaCtrtTpCd() != "B" ) {
	                
	        		/**2016.07.18 C.RFA Logic 변경 요청 by. 이종욱
	        		 * Contract Type amend 1회차 이상인 경우에도 Target MVC 값이 모든 Scope MQC들의 합보다 크거나 같아야 함  
	        		 * Unit이 FEU일 경우 TEU로 환산하여 계산하는 Logic 추가
	        		 * */	  
	        	   //Main MQC
	        	   var mainMqc = 0;
	        	   // Unit Conversion
	        	   if(sheetObjects[0].CellValue(1, "cntr_lod_ut_cd") == "F"){ 	//FEU
	        		   mainMqc = sheetObjects[0].CellValue(1, "tgt_mvc_qty")*2;
	        	   }else{					//TEU
	        		   mainMqc = sheetObjects[0].CellValue(1, "tgt_mvc_qty");	        		   
	        	   }
	        	   //Sub MQC
	               var subMqc = 0;
	               var sheetObj = sheetObjects[1];
	               for (var i = 1; i <= sheetObj.RowCount; i++){
	            	   var tgtMqcOri = 0;
	            	   // Unit Conversion
	            	   if(sheetObj.CellValue(i, "cntr_lod_ut_cd_ori") == "F"){	//FEU
	            		   	tgtMqcOri = ComParseInt(sheetObj.CellValue(i, "tgt_mvc_qty_ori"))*2;
	            	   }else{					//TEU
	            		   	tgtMqcOri = ComParseInt(sheetObj.CellValue(i, "tgt_mvc_qty_ori"));
	            	   }
	            	   		subMqc += tgtMqcOri;
	            	   		
	            	   if ( (sheetObj.CellValue(i,"tgt_mvc_qty") != sheetObj.CellValue(i,"tgt_mvc_qty_ori")) || (sheetObj.CellValue(i,"cntr_lod_ut_cd") != sheetObj.CellValue(i,"cntr_lod_ut_cd_ori")) ){
	            		   var tgtMqc = 0;
	            		   // Unit Conversion
	            		   if(sheetObj.CellValue(i, "cntr_lod_ut_cd") == "F"){	//FEU
	            			   tgtMqc =  ComParseInt(sheetObj.CellValue(i, "tgt_mvc_qty"))*2;
	            		   }else{ 				//TEU
	            			   tgtMqc =  ComParseInt(sheetObj.CellValue(i, "tgt_mvc_qty"));
	            		   }
	            		   
	            		   subMqc += tgtMqc - tgtMqcOri;
	            	   }           	            		   
	               }
	               /** End */
		           	...
		           	if (sheetObjects[0].CellValue(1,"amdt_seq") == "0"){
		                	if(sheetObjects[0].CellValue(1,"ctrt_eff_dt") != "" && sheetObjects[0].CellValue(1,"ctrt_exp_dt") != "" && parseInt(sheetObjects[0].CellValue(1, "ctrt_exp_dt")) - parseInt(sheetObjects[0].CellValue(1, "ctrt_eff_dt")) >= 30){
		                		...
		                		}else if(sheetObjects[0].CellValue(1, "rfa_ctrt_tp_cd") == "S"){
...
//		               }
    ...
    function getFrameTabId(tabIndex) {
    	...
    	case 0: {
    		frameId = "t1frame";
    		sUrl = "ESM_PRI_2003_02.do";
    		divIndex = "tabLayer1";
    		break;
    	}
    	case 1: {
    		frameId = "t2frame";
    		sUrl = "ESM_PRI_2003_03.do";
    		divIndex = "tabLayer2";
    		break;
    	}
    	case 2: {
    		...
    		if (index == 0) {
    			frameId = "t3frame";
    			sUrl = "ESM_PRI_2003_04.do";
    			divIndex = "tabLayer3";
    		} else if (index == 1) {
    			frameId = "t4frame";
    			sUrl = "ESM_PRI_2003_05.do";
    			divIndex = "tabLayer4";
    		} else if (index == 2) {
    			frameId = "t8frame";
    			sUrl = "ESM_PRI_2003_12.do";
    			divIndex = "tabLayer8";
    		}
    		break;
    	}
    	case 3: {
    		...
    		if (index == 0) {
    			frameId = "t5frame";
    			sUrl = "ESM_PRI_2003_07.do";
    			divIndex = "tabLayer5";
    		} else if (index == 1) {
    			frameId = "t6frame";
    			sUrl = "ESM_PRI_2003_08.do";
    			divIndex = "tabLayer6";
    		} else if (index == 2) {
    			frameId = "t9frame";
    			sUrl = "ESM_PRI_2003_13.do";
    			divIndex = "tabLayer9";
    		}
    		break;
    	}
    ...
    function loadTabPage(tabIndex, selRow) {
    	...
    	var sheetObj2 = sheetObjects[1];
    	var sRow = sheetObj2.SelectRow;
    	if (selRow != null && typeof selRow != "undefined") {
    		sRow = selRow;
    	}
    ...
    */ 
    function buttonControl(){     
        ...
        try{
            ...
            // [CHM-201642287] Basic Auto Amend
            hiddenButton("btn_auto_amend");
            ...
            switch(sts) {
                ...
                case 'Q':   // Requested               
                    ...
                    if(apro_usr_flg=="Y"){                    
                        ...
                        // Main 정보에 미승인이 있는 경우 approve 비활성화
                        }else{
                            ComBtnDisable("btn_approve");
                            ComBtnEnable("btn_coffer"); 
                        }                                            
    	...
    	switch (sts){
	    	case "I":
	    		if (amdtSeq =="0"){
                	 ...
                	 * Target MVC 수정 활성화 
                	 * */
	                    formObj.tgt_mvc_qty.readOnly = false; //true->false
	                    formObj.tgt_mvc_qty.setAttribute("className","input"); //input2 -> input1
	                    formObj.cntr_lod_ut_cd.enable = true;
	                 /** End */
                    }else{
        	            formObj.tgt_mvc_qty.readOnly = false;
...
//        var preIbflag = sheetObj.CellValue(1,"ibflag");
    ...
    function sheet2_OnSearchEnd(sheetObj, errMsg){
        ...
        for ( var i = 1; i <= sheetObj.RowCount; i++ ){
            ...
            sheetObj.CellValue2(i,"cntr_lod_ut_cd_ori") = sheetObj.CellValue(i,"cntr_lod_ut_cd");//2016-07-18 TEU/FEU Unit Conversion으로 인한 추가
    ...
    */  
    function scopeStatusChange(scpCd){
        ...
        if (arrData !=null && arrData.length > 0){  
            ...
            // [CHM-201538236] Scope에 대한 갱신 여부는 Approve 버튼에 는 영향이 없고 C/offer에만 관련있도록 수정 2015.10.28 Request by  SELCMU/김현경
            if (arrData[0] == "A"){       
                // 모두 accept가 되었는지 확인
                if (checkAccept() == "Y"){
                    if(apro_usr_flg == "Y"){            
        ...
        if (arrData != null && arrData.length > 0) {
            for (var i = 0; i < arrData.length; i++){
                ...
                if (arrData[i][0] == "CALC_CHK"){// RATE CALCULATE
                	/* 2017.02.23 Match Back, Calculate Button Click을 필수에서 Option으로 변경함 (송민석)
                	chkType = "1";
                	if (arrData[i][1] != ""){
                		rValue = "N";
                		errMsg = arrData[i][1];
...
// ///////////////////////////////////////////////////////////////////////
 ...
 /**
    * 화면에서 Approve 이전에 Retroactive 대상여부를 확인 한후  <BR>
    * 대상이면 다음 절차인 소급 사유내용 저장을 유도한다..<BR>
    * <br><b>Example :</b>
    * <pre>
    *     checkRetroactiveExist();
    * </pre>
    *  
    * @return string , Y : Retroactive 대상, N or null: Retroactive 대상 아님.
    * @author 최성환
    * @version 2014.11.17
    */
   function checkRetroactiveExist(){
    	
       var formObj = document.form;
       var sheetObj = sheetObjects[0]
       var rValue = 'N';	//소급대상아님.                       
       formObj.f_cmd.value = SEARCH23;	//SEARCH23
       var sParam = "prop_no="+sheetObj.CellValue(1, "prop_no")+ "&" + FormQueryString(formObj) ; 
       var sXml = sheetObj.GetSearchXml("ESM_PRI_2003GS.do" , sParam);    
       //결과값이 'Y' 일경우만 소급 대상 RFA 그외는 정상적인 RFA
       rValue = ComGetEtcData(sXml, "RSLT");
       
       if(rValue == undefined || rValue == '' || rValue != 'Y') {
    	   rValue = 'N';
       }  
       return rValue;
    }   
...
/**
 * Amend를 하였을 경우 Arbitrary의 Prop_rate_amt 값을 Guide 금액으로 변경.
 */
function updateArbitraryGuideValue(sheetObj) {
    var sParam = "f_cmd=" + MULTI07;
    	sParam += "&prop_no="+ sheetObj.CellValue(1, 'prop_no');
    	sParam += "&amdt_seq=" + (parseInt(sheetObj.CellValue(1, 'amdt_seq')) +1);
		...
		if (sXml.indexOf("ERROR") >= 0) {
			...
			//2초마다 getBackEndJobStatus를 호출해 backend job이 끝났는지 확인한다.
//			timerID = setInterval(getBackEndJobStatus, 2000);
 ...
 * <br><b>Example :</b>
 * <pre>
 *      Amend 후 Arbitrary G/L Amt  update가 완료되면 RFA를 재조회
 * </pre>
 * 
 * @return 없음
 */
function getBackEndJobStatus() {
	...
	if (jobStatus == "3") {
//		clearInterval(timerID); // [CHM-201641607]구주 지역 RFA Creation 화면의 Rate 화면 로딩 관련
		doActionIBSheet(sheetObjects[0],document.form,IBSEARCH);
 ...
 */     
 function rfa_ctrt_tp_cd_OnChange(comboObj, code, text) {
     ...
     //RFA 효율화를 위한 요청 (1차) (CHM-201640671) 
     //현재 RfaCtrtTpCd = B 이면서 Spot / Contract 로 타입 변경시 다른 전체 TAB 활성화.     
     if(code != "B" && getRfaCtrtTpCd() == "B"){
     	tabObjects[0].SelectedIndex = 1;  //Rate Tab 활성화 선택 NYC160015
    	tabObjects[0].TabEnable(0) = true;
    	tabObjects[0].TabEnable(1) = true;
    	tabObjects[0].TabEnable(2) = true;
    	tabObjects[0].TabEnable(4) = true;
//    	tabObjects[0].TabEnable(3) = true; //이미 선택되어져서 활성화 불필요
     }
     
     if(code == "B"){	// type B 로 선택시 'Spot' 으로 강제 치환 함. 현재 화면에서 Basic 등록 못함.
    	 ComShowCodeMessage('PRI07061');	 //msgs['PRI07061'] = 'Please select the Spot or CONTRACT Type.';
    	 comboObj.Code2 = "S";
    	 com_change_sheet( sheetObj, "rfa_ctrt_tp_cd" ); //add 
    	 return;
     } else if(code == "C"){
     	ComShowCodeMessage('PRI07051');	//msgs['PRI07051'] = '> Contract Customer : whose contract has\n   1) fixed rate\n   2) MQC/Award during the contract period. \n\n> When you select the "CONTRACT"\n   A. Creation wise\n       1. "Target MVC" & "Target MQC" are mandatory items. \n       2. Able to put the same Actual Customer in several rows at\n          commodity group for inputting various commodity.\n           - However, various actual customers cannot put in at\n             commodity group. \n\n   B. Amendment wise\n       3. Cannot make extention on EXP. Date.\n           - After completion of the contract period, we must re-create\n             the RFA.\n       4. Cannot make amendment on RFA type, Target MVC, once\n          confirmed.';
     	//RfaCtrtTpCd = B 조회 후 다른 TYPE인 C/S로 변경시에만 buttonControl() 적용함.
     	//1.먼저 Button 상태를 정리한 후 
     	if( getRfaCtrtTpCd() == "B" ){
     		buttonControl(); //add
     	}
     	//2. 1번 이후 rfa_ctrt_tp_cd 의 값을 변경한다.
     	com_change_sheet( sheetObj, "rfa_ctrt_tp_cd" );
    	return;
     }else{
     	ComShowCodeMessage('PRI07052');	//msgs['PRI07052'] = '> Spot customer : R/R(or PSC) target customer whose rate\n                         is moving in line with the market.\n\n> When you select the "Spot" : Same as current RFA logic.';
     	//RfaCtrtTpCd = B 조회 후 다른 TYPE인 C/S로 변경시에만 buttonControl() 적용함.
     	//1.먼저 Button 상태를 정리한 후 
     	if( getRfaCtrtTpCd() == "B" ){
     		buttonControl(); //add
 ...
 /**
  * Basic RFA Request  시 DEM/DET Exception을 Update해야 하는 상황인지 체크 한다. <br>
  * Update해야 하는 상황이면 메시지 처리 후 진행하지 않는다. <br>
  * 
  * <br><b>Example :</b>
  * <pre>
  *      checkDuration();
  ...
  * @param  없음
  * @return {string} <br>
  *          Y : Master의 dem/det exception이 변경됨<br>
  *          N : Master의 dem/det exception이 변경 안되었음<br>
  * @author 송민석
  * @version 2017.06.15
  */ 
 function checkDemDetExceptionNewVer(){
     ...
     var rValue = ComGetEtcData(sXml, "MST_RFA_VER_UPPR_YN");

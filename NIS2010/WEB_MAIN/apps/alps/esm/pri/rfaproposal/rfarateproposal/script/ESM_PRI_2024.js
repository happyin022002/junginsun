...
* 2011.05.25 이행지 [CHM-201111048-01] ALPS Error 처리- Route Note null 입력금지, 4000Byte 이상 입력금지
...
//			switch (srcName) {
//			    case "ta_note_ctnt":
//			        sheetObjects[0].CellValue(sheetObjects[0].SelectRow , "note_ctnt") = document.form.ta_note_ctnt.value;
//			        sheetObjects[0].CellValue(sheetObjects[0].SelectRow , "note_ctnt_tooltip") = document.form.ta_note_ctnt.value;
//			        break;
//			}
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case 1: // sheet1 init
			with (sheetObj) {
                ...
                InitDataProperty(0, cnt++, dtHidden, 100, daCenter, false, "rout_note_seq", true, "", dfNone, 0, false, false);                
			...
			if ((bIsReqUsr || bIsAproUsr) && document.form.prc_prop_sts_cd.value == "I"
				...
				for (var i = sheetObjects[1].HeaderRows; i <= sheetObjects[1].LastRow; i++) {	
					if(sheetObj.CellValue(sheetObj.SelectRow, "src_info_cd") == "AM") {
		 				sheetObjects[1].CellEditable(i, "chg_rule_def_cd") = false;
		 			} else {
		 				sheetObjects[1].CellEditable(i, "chg_rule_def_cd") = true;
		 			}
		 			disableColumnValidation(sheetObjects[1], i, 2, sheetObjects[1].CellValue(i,"chg_rule_def_cd"));
        ...
        try {
	        ...
	        case IBSEARCH_ASYNC11: // Amend
	        	...
	        	for (var i = sheetObjects[1].LastRow; sheetObjects[1].RowCount > 0 && i >= sheetObjects[1].HeaderRows; i--) {
	        		...
	        		if (sheetObjects[1].CellValue(i, "note_conv_rule_cd") == "APP") {
	        			continue;
	        		}
	        		
	        		if (sheetObjects[1].CellValue(i, "note_conv_mapg_id") == sheetObj.CellValue(idx, "note_conv_mapg_id")) {
	        			if (sheetObjects[1].CellValue(i, "exp_dt") < formObj.eff_dt.value) {
	        				sheetObjects[1].RowHidden(i) = true;
	        				sheetObjects[1].RowStatus(i) = "D";
	        			}
	
	        			if (sheetObjects[1].CellValue(i, "eff_dt") > formObj.exp_dt.value) {
	        				sheetObjects[1].RowHidden(i) = true;
	        				sheetObjects[1].RowStatus(i) = "D";
	        			}
	        		}
	        	...
	        	for (var i = sheetObjects[1].LastRow; sheetObjects[1].RowCount > 0 && i >= sheetObjects[1].HeaderRows; i--) {
	        		...
	        		if (sheetObjects[1].CellValue(i, "note_conv_mapg_id") == sheetObj.CellValue(idx, "note_conv_mapg_id")) {
	        			if (sheetObjects[1].CellValue(i, "eff_dt") < formObj.eff_dt.value) {
	        				sheetObjects[1].CellValue2(i, "eff_dt") = formObj.eff_dt.value;
	        			}
	
	        			if (sheetObjects[1].CellValue(i, "exp_dt") > formObj.exp_dt.value) {
	        				sheetObjects[1].CellValue2(i, "exp_dt") = formObj.exp_dt.value;
	        			}
	        		}
	        	}
	        	...
	        	sheet1_OnSelectCell(sheetObj, idx - 1, sheetObj.SelectCol, idx, sheetObj.SelectCol);
	        ...
	        case IBSEARCH_ASYNC12: // Amend Cancel
	        	...
	        	for (var i = sheetObjects[1].LastRow; sheetObjects[1].RowCount > 0 && i >= sheetObjects[1].HeaderRows; i--) {
	        		...
	        		if (sheetObjects[1].CellValue(i, "note_conv_mapg_id") == newNoteConvMapgId) {
	    				sheetObjects[1].RowHidden(i) = true;
	    				sheetObjects[1].RowStatus(i) = "D";
	        		}
	        	}
	        	for (var i = sheetObjects[1].LastRow; sheetObjects[1].RowCount > 0 && i >= sheetObjects[1].HeaderRows; i--) {
	        		...
	        		if (sheetObjects[1].CellValue(i, "note_conv_mapg_id") == prevNoteConvMapgId) {
	        			sheetObjects[1].SelectCell(i, 0);
	        			var copiedIdx = sheetObjects[1].DataCopy();
	        			
	        			sheetObjects[1].CellValue2(copiedIdx, "note_conv_mapg_id") = newNoteConvMapgId;
	        			sheetObjects[1].CellValue2(copiedIdx, "amdt_seq") = formObj.amdt_seq.value;
	        			
	        			/*
	        			// 2009-12-02
	        			if (sheetObjects[1].CellValue(copiedIdx, "exp_dt") == formObj.pre_exp_dt.value) {
	        				sheetObjects[1].CellValue2(copiedIdx, "exp_dt") = formObj.exp_dt.value;
	        			}
	        			*/
	        			
	        			// 2010-08-04
	        			//NOTE에 이전SEQ의 MAIN-DURATION 정보를 가지고 있어야 한다.
	        			if (sheetObjects[1].CellValue(copiedIdx, "exp_dt") == formObj.before_exp_dt.value) {
	        				sheetObjects[1].CellValue2(copiedIdx, "exp_dt") = formObj.exp_dt.value;
	        			}
	        		}
	        	}
	        ...
	        case IBSEARCH_ASYNC13: // Accept
	            ...
	            var sXml = sheetObj.GetSaveXml("ESM_PRI_2024GS.do", sParam);
	        ...
	        case IBSAVE: // OK
				...
				dialogArguments.setSheetXml(sXml, 11);
				
				sXml = ComPriSheet2Xml(sheetObjects[1]);
				dialogArguments.setSheetXml(sXml, 15);
			...
			case IBSEARCH: // parent sheet에서 조회
				...
				sXml = dialogArguments.getSheetXml(15);
				...
				if (arrData != null && arrData.length > 0) {
					...
					sheetObjects[1].InitDataCombo(0,2, sNm, sCd,"", "", 0, "", "", sChgCdVisiable);
				}			
				...
				for (var i = sheetObjects[1].HeaderRows; sheetObjects[1].RowCount > 0 && i <= sheetObjects[1].LastRow; i++) {
		     	...
		     	if(document.form.svc_scp_cd.value == 'TPW' || document.form.svc_scp_cd.value == 'ACW'){
		     		sheetObjects[1].ColHidden("bkg_esvc_tp_cd") = false;
		     	}else{
		     		sheetObjects[1].ColHidden("bkg_esvc_tp_cd") = true;
		     	}
		     	
				sXml = dialogArguments.getSheetXml(11);
				...
				sheetObjects[0].SelectRow = sheetObjects[0].LastRow;
			...
			case IBINSERT: // 입력
	            ...
	            sheetObj.CellValue(idx, "rout_note_seq") = parseInt(ComPriGetMax(sheetObj, "rout_note_seq"))+ 1;
			...
			case IBDELETE: // 삭제
	        	...
	        	for (var i = arrCheckedRows.length - 1; i >= 0; i--) {
	            	for (var j = sheetObjects[1].LastRow; sheetObjects[1].RowCount > 0 && j >= sheetObjects[1].HeaderRows; j--) {
	            		if (sheetObjects[1].RowStatus(j) == "D") {
	            			continue;
	            		}
	            		if (sheetObjects[1].CellValue(j, "note_conv_mapg_id") == sheetObj.CellValue(arrCheckedRows[i], "note_conv_mapg_id")) {
	        				sheetObjects[1].RowHidden(j) = true;
	        				sheetObjects[1].RowStatus(j) = "D";
	            		}
	            	}
	            	...
	            	if (sheetObj.CellValue(arrCheckedRows[i], "n1st_cmnc_amdt_seq") != formObj.amdt_seq.value) {
	            		sheetObj.SelectRow = arrCheckedRows[i];
	               		...
	               		idx = dialogArguments.doRowAmend(sheetObj, "AD");
	        	...
	        	sheet1_OnSelectCell(sheetObj, -1, sheetObj.SelectCol, idx, sheetObj.SelectCol);
			...
			case COMMAND10: //insert
				...
				sheetObj.CellValue2(idx, "note_conv_mapg_id") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"note_conv_mapg_id");
				sheetObj.CellValue2(idx, "note_conv_tp_cd") = "R";
			...
			case COMMAND12: //PASTE			
				...
				if((ComShowCodeConfirm("PRI00016")) ) {
					...
					var sXml = sheetObj.GetSearchXml("ESM_PRI_2024GS.do", FormQueryString(formObj));				
					var arrData = ComPriXml2Array(sXml, "chg_rule_def_cd"); 
						...
						//////////////////////////////////////						
			      		...
			      		if(arrRow != null && arrRow.length > 0) {  
			      			...
			      			for(var i=0; i<arrRow.length; i++) {
				      			...
				      			sheetObj.CellValue2(arrRow[i], "note_conv_mapg_id") = sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"note_conv_mapg_id");
								...
								sheetObj.CellValue2(arrRow[i], "amdt_seq") 			= formObj.amdt_seq.value;
				      			...
				      			sheetObj.CellValue2(arrRow[i], "note_conv_tp_cd") 	= "R";
    ...
    function validateForm(sheetObj, formObj, sAction) {
        ...
        case IBSAVE: // 저장
			...
			if(sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "amdt_seq") == amdtSeq
					&& sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "n1st_cmnc_amdt_seq") == amdtSeq) {
			...
			if (sheetObjects[1].IsDataModified ) {
				...
				for(var i = sheetObjects[1].HeaderRows; i <= sheetObjects[1].LastRow; i++) {	 			
		 			if(sheetObjects[1].CellValue(i, "bkg_vvd_cd") != ""  && sheetObjects[1].CellValue(i, "bkg_vvd_cd").length != 9 && sheetObjects[1].RowStatus(i) != "D") {
    ...
    function updateNoteConvChecked() {
    	var noteConvMapgId = sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_conv_mapg_id");
    	var prevRowStatus = sheetObjects[0].RowStatus(sheetObjects[0].SelectRow);
    	
    	for (var i = sheetObjects[1].HeaderRows; sheetObjects[1].RowCount > 0 && i <= sheetObjects[1].LastRow; i++) {
    		if (sheetObjects[1].CellValue(i, "note_conv_mapg_id") == noteConvMapgId && sheetObjects[1].RowStatus(i) != "D") {
    			sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_conv_mapg_id_chk") = "1";
    			sheetObjects[0].RowStatus(sheetObjects[0].SelectRow) = prevRowStatus;
    			return;
    		}
    	}
    	
		sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "note_conv_mapg_id_chk") = "0";
		sheetObjects[0].RowStatus(sheetObjects[0].SelectRow) = prevRowStatus;
		return;
    }
   ...
   */
   function sheet2_OnPopupClick(sheetObj, Row, Col, Value) {
	...
	function callBackTerminalCode(rowArray){
	     ...
	     if(rowArray != null) {
	    	 sheetObjects[1].CellValue(sheetObjects[1].SelectRow, "bkg_yd_cd") = colArray[3];
	     } else {
	    	 sheetObjects[1].CellValue(sheetObjects[1].SelectRow, "bkg_yd_cd") = "";
	     }
	}
	 ...
	 */ 
 	function checkDuration(sheetObj) {
		...
		for (var a = sheetObjects[0].HeaderRows; sheetObjects[0].RowCount > 0 && a <= sheetObjects[0].LastRow; a++) {
			if (sheetObjects[0].RowHidden(a) == true) {
				continue;
			}
			if (sheetObjects[0].CellValue(a, "amdt_seq") != formObj.amdt_seq.value) {
				continue;
			}
			if (sheetObjects[0].CellValue(a, "n1st_cmnc_amdt_seq") != formObj.amdt_seq.value) {
				continue;
			}
			
			var noteConvMapgId = sheetObjects[0].CellValue(a, "note_conv_mapg_id");
			...
			switch(sts) {
				case 'I':   // Initial
					if(apro_usr_flg == "true" || req_usr_flg == "true" ){
						...
						if( sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "amdt_seq") != amdtSeq 
  								|| sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "src_info_cd") == "AD"
  	  							|| sheetObjects[0].RowHidden(sheetObjects[0].SelectRow) == true
								|| (sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "amdt_seq")  == amdtSeq
										&& sheetObjects[0].CellValue(sheetObjects[0].SelectRow, "eff_dt")!= effDt)) {
								disableButton("btn_copy");
								disableButton("btn_paste");
								disableButton("btn_rowadd3");
								disableButton("btn_rowcopy");
    ...
    /**
  	...
  	function ta_note_ctnt_OnChange(){
  		 ...
  		 if(strNote.trim() == ""){
  			 ...
  			 ComShowCodeMessage("PRI00316","Route Note");
  		 } else {

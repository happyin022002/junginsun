...
*@LastVersion : 1.0
* 1.0 Creation
* 2013.04.29 전윤주 [CHM-201324515] 예외 location의 경우 trans mode가 feeder 이면 Door 운임으로 간주하지 않아 DIH, OIH를 입력하지 않도록 변경
=========================================================*/
/****************************************************************************************
	...
	function CYDoorCrossCheck() {
		...
		if(funcChkTypeStatus() == 'A') {
			...
			for ( var i = sheetObj5.HeaderRows; i < sheetObj5.HeaderRows + sheetObj5.RowCount; i++) {
				if (sheetObj5.RowStatus(i) == "D" || sheetObj5.CellValue(i, "amdt_seq") != amdtSeq || sheetObj5.CellValue(i, "src_info_cd") == "AD") {
					continue;
				}
			...
			for ( var i = sheetObj6.HeaderRows; i < sheetObj6.HeaderRows + sheetObj6.RowCount; i++) {
				if (sheetObj6.RowStatus(i) == "D" || sheetObj6.CellValue(i, "amdt_seq") != amdtSeq || sheetObj6.CellValue(i, "src_info_cd") == "AD") {
					continue;
				}
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case 5: // sheet5 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|cmdt_hdr_seq|rout_seq|org_dest_tp_cd|rout_pnt_seq|Location Type|Location Code|Description|Loc Group|Term|Origin Via|Trans Mode|EFF Date|EXP Date|Source Code|Source|Status Code|Status|n1st_cmnc_amdt_seq|Accept User|1st ord|2nd ord|org_cy_dor_rt_tp_cd|bse_port_loc_cd||||||||||||||||||";
				var headCount = ComCountHeadTitle(HeadTitle);
				...
				InitDataProperty(0, cnt++, dtCombo, 100, daCenter, false, "rout_pnt_loc_tp_cd", true, "", dfNone, 0, true, true);
				InitDataProperty(0, cnt++, dtPopupEdit, 100, daCenter, false, "rout_pnt_loc_def_cd", true, "", dfNone, 0, true, true, 5);
				InitDataProperty(0, cnt++, dtData, 130, daLeft, false, "rout_pnt_loc_def_nm", true, "", dfNone, 0, false, false);
				InitDataProperty(0, cnt++, dtData, 90, daCenter, false, "group_no", false, "", dfNone, 0, false, false);				
				InitDataProperty(0, cnt++, dtCombo, 60, daCenter, false, "rcv_de_term_cd", true, "", dfNone, 0, true, true);
				InitDataProperty(0, cnt++, dtComboEdit, 100, daCenter,  	false,  "bse_port_def_cd",     	false,  	"", dfEngUpKey, 	0,  true,   true,	5);
				InitDataProperty(0, cnt++, dtCombo, 85, daCenter, false, "prc_trsp_mod_cd", false, "", dfNone, 0, true, true);
				InitDataProperty(0, cnt++, dtData, 65, daCenter, false, "eff_dt", false, "", dfDateYmd, 0, false, false);
				InitDataProperty(0, cnt++, dtData, 65, daCenter, false, "exp_dt", false, "", dfDateYmd, 0, false, false);
				InitDataProperty(0, cnt++, dtHidden, 100, daCenter, false, "src_info_cd", false, "", dfNone, 0, false, false);
				InitDataProperty(0, cnt++, dtData, 80, daCenter, false, "src_info_nm", false, "", dfNone, 0, false, false);
                ...
                InitDataProperty(0, cnt++, dtHidden, 100, daCenter, false, "bse_port_loc_cd", true, "", dfNone, 0, false, false);
		...
		case 6: // sheet6 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|cmdt_hdr_seq|rout_seq|org_dest_tp_cd|rout_pnt_seq|Location Type|Location Code|Description|Loc Group|Term|Destination Via|Trans Mode|EFF Date|EXP Date|Source Code|Source|Status Code|Status|n1st_cmnc_amdt_seq|Accept User|1st ord|2nd ord|dest_cy_dor_rt_tp_cd|bse_port_loc_cd||||||||||||||||||";
				var headCount = ComCountHeadTitle(HeadTitle);
        ...
        try {
			...
			switch (sAction) {
		        ...
		        case IBSEARCH_ASYNC13: // Accept
		            ...
		            var sXml = sender.GetSaveXml("ESM_PRI_2030GS.do", sParam);
		        ...
		        case IBSAVE: // OK
		            ...
		            for (var a = 0; a <= 3; a++) {
			            ...
			            for (var i = sheetObjects[idx].HeaderRows; sheetObjects[idx].RowCount > 0 && i <= sheetObjects[idx].LastRow; i++) {
			            	...
			            	if(idx == 4 || idx == 5) {
			            		var preStatus = sheetObjects[idx].RowStatus(i);
			            		sheetObjects[idx].CellValue2(i, "bse_port_loc_cd") = sheetObjects[idx].CellValue(i, "bse_port_def_cd");
			            		sheetObjects[idx].RowStatus(i) = preStatus;
			            		if(formObj.prc_prop_sts_cd.value=="I" && sheetObjects[idx].IsDataModified) {
			            			if(idx == 4) {
		            ...
		            window.close();
		             if (exTransaction) {
		            	dialogArguments.routeAcceptReload(true);
						dialogArguments.restylingPagePostTr(true);
					}
				...
				case IBINSERT: // 입력
		            ...
		            if (curPntViaType == "P") {
				        ...
				        var checkStr = funcChkTypeStatus() ;
			            if( checkStr == "A" && sheetObj.id == "sheet5"  ){
			            	sheetObj.CellValue2(idx, "rcv_de_term_cd") = "D";
			            } else if( checkStr == "A" && sheetObj.id == "sheet6"  ){
				            sheetObj.CellValue2(idx, "rcv_de_term_cd") = "D";
			            } else{
				...
				case IBSEARCH_ASYNC06:
					...
					if(cyDorRtTpCdVal == 'D') {
						...
						if (sender.EtcData(ComWebKey.Trans_Result_Key) == "F") { 
						...
						if(arr != null && arr.length > 0){
							...
							var bsePortDefCdCombo = arr[1].split("|");
							if(arr[1] != undefined && arr[1] != null && bsePortDefCdCombo.length == 1) {
								sheetObj.CellValue(Row, "bse_port_def_cd") = bsePortDefCdCombo[0];
							}
						}
					}
					break;
...
// 		cy_dor_rt_tp_cd가 NULL인 경우는 데이터적으로 오류다. 'C' 처리 하지 않는다.
//		if(prc_io_bnd_cd == v_origin) {
...
//		}
		...
		if((prc_io_bnd_cd == v_origin && sheetObj.CellValue(Row, "org_cy_dor_rt_tp_cd") == 'D') || (prc_io_bnd_cd == v_destination && sheetObj.CellValue(Row, "dest_cy_dor_rt_tp_cd") == 'D')) {
			var sParam = FormQueryString(formObj);
			sParam += "&svc_scp_cd=" + sheetObj.CellValue(Row, "svc_scp_cd");
			sParam += "&rout_pnt_loc_def_cd=" + sheetObj.CellValue(Row, "rout_pnt_loc_def_cd");
			sParam += "&bse_port_def_cd=" + sheetObj.CellValue(Row, "bse_port_def_cd");
        ...
        case IBSEARCH_ASYNC15: // 라디오버튼 이동시 Validation
        	...
        	if (sheetObj.id == "sheet1" || sheetObj.id == "sheet4" ) {
        		...
        		for(var idx= sheetObj.HeaderRows; idx < sheetObj.Rows; idx++) {
        			var preStatus = sheetObj.RowStatus(idx);
        			sheetObj.CellValue2(idx, "bse_port_loc_cd") = sheetObj.CellValue(idx, "bse_port_def_cd");
        			sheetObj.RowStatus(idx) = preStatus;
        		}
            	dupRow = ComPriAmendDupCheck(sheetObj, "rout_pnt_loc_tp_cd|rout_pnt_loc_def_cd|rcv_de_term_cd|bse_port_loc_cd|prc_trsp_mod_cd", formObj.amdt_seq.value);
//            	dupRow = ComPriAmendDupCheck(sheetObj, "rout_pnt_loc_tp_cd|rout_pnt_loc_def_cd|rcv_de_term_cd|bse_port_def_cd|prc_trsp_mod_cd", formObj.amdt_seq.value);
	...
	function checkBasePort(sheetObj, Row, Value) {
		Value = sheetObj.CellValue(Row,"bse_port_def_cd");
 		if (ComTrim(Value) != "" ) {
 			var formObj = document.form;
			formObj.f_cmd.value = SEARCH02;
			var sender = sheetObjects[6];
			var sParam = FormQueryString(formObj);	
			sParam += "&loc_type_cd=L&loc_cd=" + ComTrim(Value);
			var sXml = sender.getSearchXml("ESM_PRI_2030GS.do", sParam);
			var arr = ComPriXml2ComboString(sXml, "base_port_list", "base_port_list");
	...
	/**
	 * Group No 체크
	 *  - CY일경우는 그룹을 체크하지 않음.<br>
	 *  - Door일 경우는 그룹이 다르면 입력 불가. N/A는 하나만 입력 가능. 
     * @return boolean
     * @author 이은섭
     * @version 2009.05.20
     */
	function checkGroupNoCyDoor(inOrgDestTpcd) {
		...
		if (sheetObj.IsDataModified) {
			...
			var amdtSeq = document.form.amdt_seq.value;
			for ( var i = sRow; i < eRow; i++) {
				if (sheetObj.RowStatus(i) == "D" || sheetObj.CellValue(i, "amdt_seq") != amdtSeq || sheetObj.CellValue(i, "src_info_cd") == "AD") {
					continue;
				}
	...
	function searchCYDoorLocationCheck( sheetObj, row, value , locType, sheetID){
		...
		if(sheetObj.CellValue(row, "rcv_de_term_cd") != 'D') {
			...
			var trns_mod = sheetObj.CellValue(row, "prc_trsp_mod_cd"); 
			if(sheetID == 'sheet5') {
				if(orgExptLoc != null && orgExptLoc !== undefined) {
					for(var i =0; i < orgExptLoc.length; i++) {
						if(loc_cd == orgExptLoc[i]&& trns_mod !='F') { //예외 처리 된 location 중에서 trans mode가 feeder 인 경우는 OIH 넣을 필요 없음
							cy_dor_rt_tp_cd = 'D';
							break;
						}
					}
				...
				if(destExptLoc != null && destExptLoc !== undefined) {
					for(var i =0; i < destExptLoc.length; i++) {
						if(loc_cd == destExptLoc[i]&& trns_mod !='F') {//예외 처리 된 location 중에서 trans mode가 feeder 인 경우는 DIH 넣을 필요 없음
							cy_dor_rt_tp_cd = 'D';
							break;
						}
					}

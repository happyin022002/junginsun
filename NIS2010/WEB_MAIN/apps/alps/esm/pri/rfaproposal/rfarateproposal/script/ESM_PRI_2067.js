 ...
 *@FileName : ESM_PRI_2067.js
 *@FileTitle : RFA Proposal Creation - Rate [Load Excel-For AEE/AEW] (V Type)
 *Open Issues :
 *Change history :
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: // sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "||cmdt_rout|CMDT\nSeq.|Commodity Group|Commodity Group|Actual Customer|Actual Customer|Route\nSeq.|Origin|Origin|Origin|Origin|Origin|O.Via|D.Via|Destination|Destination|Destination|Destination|Destination|Rate|Rate|Rate|Rate|Rate|Rate||||||||||||||";
			var HeadTitle2 = "||cmdt_rout|CMDT\nSeq.|Code|Description|Code|Description|Route\nSeq.|Code|Description|Loc Group|Term|Transmode|Code|Code|Code|Description|Loc Group|Term|Transmode|PER|Cargo Type|Cur|Rate|BOF|IHC||||||||||||||";
			...
			InitDataProperty(0, cnt++, dtData, 70, daRight, false, "fic_prop_rt_amt", false, "", dfNullFloat, 2, true, true);

			InitDataProperty(0, cnt++, dtHidden, 80, daCenter, false, "fic_gline_rt_amt", false, "", dfNone, 0, true, true);
...
function sheet1_OnChange(sheetObj, Row, Col, Value) {
	...
	if (colName == 'cmdt_dp_seq' || colName == 'rout_dp_seq') {
		...
		if (Value == "AK") {
			...
			funcGuideLineAmount(sheetObj, Row);
		}
	...
	} else if (colName == "prop_frt_rt_amt" || colName == 'fic_prop_rt_amt') {
		sheetObj.CellValue(Row, "prop_bof_amt") = sheetObj.CellValue(Row, "prop_frt_rt_amt") - sheetObj.CellValue(Row, "fic_prop_rt_amt");
	...
	try {
		...
		case IBSAVE: // Save
			...
			if (bBackEndJob) {
				...
				var sXml = sheetObj.GetSaveXml("ESM_PRI_2067GS.do", sParam);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case IBSEARCH_ASYNC01: // Check
		...
		for ( var i = sheetObj.HeaderRows, n = sheetObj.HeaderRows + sheetObj.RowCount; i < n; i++) {
			...
			if (sheetObj.CellValue(i, "rout_dp_seq") != "") {
				if (!chkMdtryRate) {
					...
					add2Tooltip(prevRouteRowRate, "fic_prop_rt_amt", ComGetMsg("PRI00316", "Rate"));
					isClear = false;
					prevRouteRowRate = i;
				} else {
					chkMdtryRate = false;
			...
			ficPropRtAmt = sheetObj.CellValue(i, "fic_prop_rt_amt");
			if (perTypeCode != "" || cargoTypeCode != "" || propRate != "") {
				...
				if (ficPropRtAmt == "") {
					add2Tooltip(i, "fic_prop_rt_amt", ComGetMsg("PRI00335", "Rate"));
					isClear = false;
				} else if (ficPropRtAmt != "" && parseFloat(ficPropRtAmt) <= 0.00) {
					add2Tooltip(i, "fic_prop_rt_amt", ComGetMsg("PRI00321", "Rate", "0"));
					isClear = false;
				} else if (ficPropRtAmt != "" && parseFloat(ficPropRtAmt) >= 9999999.99) {
					add2Tooltip(i, "fic_prop_rt_amt", ComGetMsg("PRI00336", "Rate", "9999999.99"));
					isClear = false;
				} else {
				}
			...
			if (sheetObj.CellValue(i, 'rat_ut_cd') != '' && sheetObj.CellValue(i, 'prc_cgo_tp_cd') != '') {
		...
		var sParam = FormQueryString(formObj) + "&" + sheetObj.GetSaveString();
		var sXml = sheetObject2.GetSearchXml("ESM_PRI_2067GS.do", sParam);
...
function funcGuideLineAmount(targetSheet, Row) {
	...
	if (ficGlineInfo.length > 0) {
		if (ratUtCd != '' && prcCgoTpCd != '') {
			targetSheet.CellValue2(Row, 'fic_gline_rt_amt') = ficGlineInfo[0];
			if (ficGlineInfo[0] == 'N/A') {
				targetSheet.CellValue(Row, 'fic_prop_rt_amt') = 0.00;
			} else {
				if(targetSheet.CellValue(Row, 'curr_cd') == 'USD') {
					targetSheet.CellValue(Row, 'fic_prop_rt_amt') = ficGlineInfo[0];
				} else {
					if(targetSheet.CellValue(Row, 'curr_cd') == locCurrCd) {
						targetSheet.CellValue(Row, "fic_prop_rt_amt") = ficGlineInfo[3];
					} else {
						targetSheet.CellValue(Row, 'fic_prop_rt_amt') = changeCurrency(ficGlineInfo[0], targetSheet.CellValue(Row, 'curr_cd'));
					}
				}
			}
			targetSheet.CellValue(Row, 'fic_rt_use_sts_cd') = ficGlineInfo[1];
			targetSheet.CellValue(Row, 'optm_trsp_mod_flg') = ficGlineInfo[2];
		} else {
			targetSheet.CellValue2(Row, 'prop_frt_rt_amt') = '';
			targetSheet.CellValue2(Row, 'prop_bof_amt') = '';
			targetSheet.CellValue2(Row, 'fic_prop_rt_amt') = '';
			targetSheet.CellValue2(Row, 'fic_gline_rt_amt') = '';
			targetSheet.CellValue2(Row, 'fic_rt_use_sts_cd') = '';
			targetSheet.CellValue2(Row, 'optm_trsp_mod_flg') = '';
...
function changeCurrency(inFicGlineRtAmt, inCurrCd) {
	...
	if(!ComIsNull(inCurrCd)) {
		...
		var sXml = sheetObjects[1].getSaveXml("ESM_PRI_2003_08GS.do", params);

   ...
   /**
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case 1: // sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|General Special Rate Type Code|GRI Group Seq.|cmdt_seq|CMDT Type|Commodity Code|Description|";
		...
		case 2: // sheet1 init
			with (sheetObj) {
				...
				var HeadTitle = "|Sel.|Seq.|prop_no|amdt_seq|svc_scp_cd|General Special Rate Type Code|GRI Group Seq.|act_cust_seq|Customer Code|Customer Code|Actual Customer Name";
				var headCount = ComCountHeadTitle(HeadTitle);
	...
	function sheet2_OnChange(sheetObj, Row, Col, Value) {
		...
		if (colName == "cust_cnt_cd" || colName == "cust_seq") {
			...
			var param = "&etc1=" + sheetObj.CellValue(Row, "cust_cnt_cd") + "&etc2=" + sheetObj.CellValue(Row, "cust_seq")+"&etc=N";
			var sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
			var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
			if (arrData != null && arrData.length > 0 && arrData[1] != "") {
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "prc_cmdt_def_cd") {
			if (sheetObj.CellEditable(Row, "prc_cmdt_def_cd")) {
				...
				sUrl += "&grp_cd=" + PRI_RP_SCP + "&commodity_cmd=CRG&prc_cmdt_tp_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "prc_cmdt_tp_cd") + "&multi_yn=Y";
					...
					for (var i = 0; i < rtnVal.length; i++) {
						if (i == 0) {
							...
							if (rtnVal[i].cd.length == 5) {
								...
								sheetObj.CellValue2(idx, "prc_cmdt_tp_cd") = "R";
							} else if (rtnVal[i].cd.length == 6) {

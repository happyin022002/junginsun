	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "rout_pnt_loc_def_cd") {
			if (sheetObj.CellEditable(Row, "rout_pnt_loc_def_cd")) {
				...
				//RFA 효율화를 위한 요청 (1차) (CHM-201640671)
				// 팝업 호출 시 location_cmd=LGC&loc_tp_cd="  //LGC - L:Location , G:Location Group, C:Country 로 구분해서 활성화 시킴.
				//Master Location Inquiry 
				var sUrl = "/hanjin/ESM_PRI_4226.do?" + FormQueryString(document.form);
				//var sUrl = "/hanjin/ESM_PRI_4026.do?" + FormQueryString(document.form);
				sUrl += "&group_cmd=" + PRI_RP_SCP + "&location_cmd=LC&loc_tp_cd="
				//sUrl += "&group_cmd=" + PRI_RP_SCP + "&location_cmd=LGC&loc_tp_cd="  //LGC - L:Location , G:Location Group, C:Country
						+ sheetObj.CellValue(sheetObj.SelectRow, "rout_pnt_loc_tp_cd") + "&org_dest_cd=" + formObj.org_dest_tp_cd.value + "&multi_yn=Y";
				var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_4226", 700, 325, true);
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "rout_via_port_def_cd") {
			if (sheetObj.CellEditable(Row, "rout_via_port_def_cd")) {
	            ...
	            var sUrl = "/hanjin/ESM_PRI_4226.do?" + FormQueryString(document.form);
	            //var sUrl = "/hanjin/ESM_PRI_4026.do?" + FormQueryString(document.form);
	            sUrl += "&group_cmd=" + PRI_RP_SCP + "&location_cmd=LC&loc_tp_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "rout_via_port_tp_cd")+"&multi_yn=Y";
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
        try {
			...
			case IBCLEAR: // 로딩시 코드조회
				...
				//공통 - type
				//CoPri.js 참조 Location Type - LOCATION_TYPE5 코드 하드 코딩 
				sheetObjects[0].InitDataCombo(0,"rout_pnt_loc_tp_cd",	 LOCATION_TYPE5[1], LOCATION_TYPE5[0], " ", " ", 0);		
				sheetObjects[1].InitDataCombo(0,"rout_via_port_tp_cd", LOCATION_TYPE5[1], LOCATION_TYPE5[0], " ", " ", 0);
				sheetObjects[2].InitDataCombo(0,"rout_via_port_tp_cd", LOCATION_TYPE5[1], LOCATION_TYPE5[0], " ", " ", 0);
				sheetObjects[3].InitDataCombo(0,"rout_pnt_loc_tp_cd", 	 LOCATION_TYPE5[1], LOCATION_TYPE5[0], " ", " ", 0);
			
				//공통 term
				//RFA 효율화를 위한 요청 (1차) (CHM-201640671)
				formObj.f_cmd.value = SEARCH19;
				sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + "&cd=CD03501"); //Free In <- CD02138
				setIBCombo(sheetObjects[0], sXml, "rcv_de_term_cd", true, 0);
				sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + "&cd=CD03500"); //Free Out <- CD02139

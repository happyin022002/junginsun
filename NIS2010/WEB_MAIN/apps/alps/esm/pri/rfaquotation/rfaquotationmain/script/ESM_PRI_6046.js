...
/**
 * @extends Pri
 * @class ESM_PRI_6046 : ESM_PRI_6046 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_6046() {
	...
	try {
		...
		case "frm_rate_g_cnt":
			if (formObject.frm_rate_g_cnt.checked) {
				if (formObject.frm_grp_loc_cnt.disabled != true) {
					formObject.frm_grp_loc_cnt.checked = true;
				}
				if (formObject.frm_grp_cmdt_cnt.disabled != true) {
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBCLEAR:
		...
		formObj.cd.value = "CD01704";
		sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj));
		ComPriXml2ComboItem(sXml, formObj.dmdt_ft_tp_cd, "cd", "nm");
	...
	case IBSAVE: // 저장
		...
		try {
			...
			var sXml = sheetObj.GetSaveXml("ESM_PRI_6046GS.do", FormQueryString(formObj));
...
function validateForm(sheetObj, formObj, sAction) {
	with (sheetObj) {
		...
		case IBSAVE:
			...
			if (ComIsEmpty(getDmdtFtTpCd())) {
				ComPriInputValueFailed("select", "Free Time", comboObjects[0]);
				return false;
			}
...
/**
 * Input[:checkbox] Used Flag
 */
function rateCopyCheck() {
	var formObj = document.form;
	if(!formObj.frm_rate_g_cnt.disabled) {
		...
		if(!ComCheckGuideExcepSvcScpCd(svcScpCd) && addOnEndExpDt > ctrtEffDt) {

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_6014_02 : ESM_PRI_6014_02 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_6014_02() {
...
//					MessageText("UserMsg17") = " " + ComGetMsg("PRI00317");
			...
			case "sheet2":
				with (sheetObj) {
					...
					var HeadTitle = "|Sel.|Seq.|qttn_no|qttn_ver_no|Group Commodity Seq.|Group Commodity Detail Seq.|CMDT Type|CMDT Code|Description|src_info_cd|Source";
					...
					InitDataProperty(0, cnt++, dtCombo, 90, daCenter, false, "prc_cmdt_tp_cd", true, "", dfEngUpKey, 0, true, true);
					InitDataProperty(0, cnt++, dtPopupEdit, 90, daCenter, false, "prc_cmdt_def_cd", true, "", dfEngKey, 0, true, true, 6, false);
					InitDataProperty(0, cnt++, dtData,		150,	daLeft,	  true,	  "prc_cmdt_def_nm",	false,	"",	dfNone,	0, false,  false);	
...
//					MessageText("UserMsg17") = " " + ComGetMsg("PRI00318");
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col)
	{
		...
		if (colName == "prc_cmdt_def_cd"){
			var sUrl = "/hanjin/ESM_PRI_4027.do?grp_cd="+ PRI_RG+"&commodity_cmd=CR";
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch (sAction) {
			case IBCLEAR: // 화면 로딩시 
				...
				//공통 cmdt type
				sheetObj.InitDataCombo(0,"prc_cmdt_tp_cd", COMODITY_TYPE2[1], COMODITY_TYPE2[0], "", "R", 0); 			
	...
	//			setIBCombo(sheetObj,sXml,"prc_cmdt_tp_cd",false,0);
			...
			case IBSEARCH: // 조회
				if (validateForm(sheetObj,document.form,sAction)) {
					try {
						...
						sheetObj.DoSearch("ESM_PRI_6014_02GS.do" , FormQueryString(formObj));
						ComOpenWait(false);
			...
			case IBSAVE: // 저장
				...
				try {
					...
					var sXml = sheetObj.GetSaveXml("ESM_PRI_6014_02GS.do", sParam);
...
//				if (validateForm(sheetObj,document.form,sAction)) {
	        	...
	        	if (sheetObj.id == "sheet1") {
					...
					var sParam = "f_cmd="+SEARCH03+"&"+sheetObj.GetSaveString(false, false, "chk");
		             ...
		             var sXml = sheetObjects[1].GetSearchXml("ESM_PRI_6014_02GS.do", sParam);
...
//				sheetObj1.CellEditable(i, "prc_grp_cmdt_cd") = flag;
			...
			for (var i = 1; i <= sheetObj2.RowCount;i++) {
				...
				sheetObj2.CellEditable(i, "prc_cmdt_tp_cd") = flag;
    ...
    */
     ...
     */
 	function sheetColResize() {
 		...
 		sheetObj2.FitColWidth("0|8|7|0|0|0|0|20|17|32|0|16");

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class RFA Quotation Rate - Origin & Destination : RFA Quotation Rate - Origin & Destination 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_6059() {
...
//	                InitDataValid(0,  "rout_pnt_loc_def_cd",	vtEngUpOnly);		// 영문대문자만 입력
      ...
      function doActionIBSheet(sheetObj,formObj,sAction) {
          ...
          switch(sAction) {
 			case IBCLEAR: // 화면 로딩시 
 				...
 				sheetObjects[1].InitDataCombo(0,"rout_pnt_loc_tp_cd",  LOCATION_TYPE2[1], LOCATION_TYPE2[0], " ", " ", 0);
 				sheetObjects[2].InitDataCombo(0,"rout_via_port_tp_cd", LOCATION_TYPE2[1], LOCATION_TYPE2[0], " ", " ", 0);
 				sheetObjects[3].InitDataCombo(0,"rout_via_port_tp_cd", LOCATION_TYPE2[1], LOCATION_TYPE2[0], " ", " ", 0);
 				sheetObjects[4].InitDataCombo(0,"rout_pnt_loc_tp_cd",  LOCATION_TYPE2[1], LOCATION_TYPE2[0], " ", " ", 0);
 				
// 				//Term combo
//         		formObj.f_cmd.value = SEARCH20;
//         		//공통 term, trans mode
//         		formObj.cd.value = "CD02070";//Origin
//				if(radioObj[3].checked == true){
//					formObj.cd.value = "CD02071";  //Destination
//				}
//				
...
//				setIBCombo(sheetObjects[4], sXml, "rcv_de_term_cd", true, 0);
...
// 				
// 				
// 				if ( arrXml[0] != null)	setIBCombo(sheetObjects[1],arrXml[0],"rcv_de_term_cd",true,0);
// 				if ( arrXml[1] != null)	setIBCombo(sheetObjects[1],arrXml[1],"prc_trsp_mod_cd",true,0);
// 				
// 				
// 				if ( arrXml[0] != null)	setIBCombo(sheetObjects[4],arrXml[0],"rcv_de_term_cd",true,0);
// 				if ( arrXml[1] != null)	setIBCombo(sheetObjects[4],arrXml[1],"prc_trsp_mod_cd",true,0);
				...
				param += ":CD01720:N"
 				...
 				sXml = sheetObj.GetSearchXml("PRICommonGS.do" , FormQueryString(formObj));
 				var arrXml = sXml.split("|$$|");
 				if ( arrXml[0] != null)	setIBCombo(sheetObj,arrXml[0],"rcv_de_term_cd",true,0,'CY');
 				if ( arrXml[1] != null)	setIBCombo(sheetObj,arrXml[1],"prc_trsp_mod_cd",true,0);
// 				setIBCombo(sheetObj, sXml, "rat_ut_cd", false, 0, "D2");
 				
 				document.form.f_cmd.value = SEARCH19;
 				document.form.cd.value="CD02070";
 				var sXml2 = sheetObjects[0].GetSearchXml("PRICommonGS.do", FormQueryString(document.form));
 				
 				if ( sXml2 != null)	setIBCombo(sheetObjects[1],sXml2,"rcv_de_term_cd",true,0);
 				if ( arrXml[1] != null)	setIBCombo(sheetObjects[1],arrXml[1],"prc_trsp_mod_cd",true,0);
 				
 				document.form.cd.value="CD02071";
 				sXml2 = sheetObjects[0].GetSearchXml("PRICommonGS.do", FormQueryString(document.form));
 					
 				if ( sXml2 != null)	setIBCombo(sheetObjects[4],sXml2,"rcv_de_term_cd",true,0);
 				if ( arrXml[1] != null)	setIBCombo(sheetObjects[4],arrXml[1],"prc_trsp_mod_cd",true,0);
			...
			case IBSEARCH:      //조회
				...
				for (i = 5; i < 9; i++){
					...
					sheetObjects[i-4].LoadSearchXml(sXml);
				}
 			...
 			case IBINSERT:      // 입력
				...
				//term defoult setting CY
				if(currentSheet == 1 ||currentSheet == 4) {
					sheetObj.CellValue(idx, "rcv_de_term_cd") = "CY";
				}
...
//					setRcvDeTermNm(sheetObj);
					if(sheetObjects[i].RowCount > 0) {
						...
						dialogArguments.setSheetXml(sXml, i+4);
					} else {
						dialogArguments.removeSheetXml(sXml, i+4);
					}	
				}
	            break; 				
          }
      ...
      /**
       * 저장시 rcv_de_term_nm에 콤보 Text를 세팅한다. <br>
       * <br><b>Example :</b>
       * <pre>
       *     setRcvDeTermNm(sheetObj)
       * </pre>
...
//     			}
    ...
    */ 	
    ...
    function changeCombo(){
      ...
      */ 	   	 
   	function setColHidden()
   	{
  		...
  		if (idx == 2 || idx == 3){
  			hiddenVal = true;
  			sheetObjects[0].InitDataProperty(0, 14, dtCombo, 90, daCenter, false, "rcv_de_term_cd",  false, "",	dfNone,	0,	true,	true);
  		} else {
  			hiddenVal = false;
  			sheetObjects[0].InitDataProperty(0, 14, dtCombo, 90, daCenter, false, "rcv_de_term_cd",  true, "",	dfNone,	0,	true,	true);
  		}
     ...
     * @version 2009.05.07
     */ 	 
   	function getOriDestTpCd(formObj) {
   		var radioObj = formObj.rt_pnt;
   		var oriDestTpCd = "O";
     	...
     	switch(colname)
     	{
 	    	case "rout_pnt_loc_def_cd":
 	    		...
 	    		if (Value.length==5){
 	    			...
 	    			var param = "&nm=rqscp" + "&etc3=" + formObj.svc_scp_cd.value + "&etc4=" + tpCd;
 	    			...
 	    			var currentSheet = 0;
 	    	  		...
 	    	  		if( currentSheet == 0 || currentSheet == 1){
 	    	  			targetCode = "O"
 	    	  		}else{
 	    	  			targetCode = "D"
 	    	  		}
 	    			...
 	    			var param = "&etc1=" + formObj.qttn_no.value + "&etc2=" + formObj.qttn_ver_no.value	+"&etc3="+targetCode+"&etc5=RQ";	
 	  				var sXml = sheetObj.GetSearchXml("PRICommonGS.do", FormQueryString(formObj)+param); 	  				

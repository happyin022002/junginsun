	...
	if (ficGlineInfo.length > 0) {
		if (ratUtCd != '' && prcCgoTpCd != '') {
			...
			if (ficGlineInfo[0] == 'N/A') {
				targetSheet.CellValue(Row, 'fic_qttn_rt_amt') = 0.00;
			} else {
				targetSheet.CellValue(Row, 'fic_qttn_rt_amt') = ficGlineInfo[0];
			}
			targetSheet.CellValue(Row, 'fic_rt_use_sts_cd') = ficGlineInfo[1];
			targetSheet.CellValue(Row, 'optm_trsp_mod_flg') = ficGlineInfo[2];
		} else {
			targetSheet.CellValue2(Row, 'qttn_rt_amt') = '';
			targetSheet.CellValue2(Row, 'qttn_bof_amt') = '';
			targetSheet.CellValue2(Row, 'fic_qttn_rt_amt') = '';
...
function sheet1_OnChange(sheetObj, Row, Col, Value) {
	...
	} else if (colName == "qttn_rt_amt" || colName == 'fic_qttn_rt_amt') {
		sheetObj.CellValue(Row, "qttn_bof_amt") = sheetObj.CellValue(Row, "qttn_rt_amt") - sheetObj.CellValue(Row, "fic_qttn_rt_amt");
	}
}
	...
	try {
		...
		case IBSAVE: // Save
			...
			try {
				...
				var sXml = sheetObj.GetSaveXml("ESM_PRI_6095GS.do", sParam);
	...
	case IBSEARCH_ASYNC01: // Check
		...
		try {
			...
			for ( var i = sheetObj.HeaderRows, n = sheetObj.HeaderRows + sheetObj.RowCount; i < n; i++) {
				...
				var ficQttnRtAmt = sheetObj.CellValue(i, "fic_qttn_rt_amt");

				if (perTypeCode != "" || cargoTypeCode != "" || qttnRate != "" || ficQttnRtAmt != '') {
					chkMdtryRate = true;
					if (perTypeCode == "") {
					...
					if (ficQttnRtAmt == "") {
						add2Tooltip(i, "fic_qttn_rt_amt", ComGetMsg("PRI00335", "Rate"));
						isClear = false;
					} else if (ficQttnRtAmt != "" && parseFloat(ficQttnRtAmt) <= 0.00) {
						add2Tooltip(i, "fic_qttn_rt_amt", ComGetMsg("PRI00321", "Rate", "0"));
						isClear = false;
					} else if (ficQttnRtAmt != "" && parseFloat(ficQttnRtAmt) >= 9999999.99) {
						add2Tooltip(i, "fic_qttn_rt_amt", ComGetMsg("PRI00336", "Rate", "9999999.99"));
						isClear = false;
					} else {
					}
			...
			var sParam = FormQueryString(formObj) + "&" + sheetObj.GetSaveString();
			var sXml = sheetObjects[1].GetSearchXml("ESM_PRI_6095GS.do", sParam);
 ...
 * 
 */
function funcCellEnabledControl(sheetObj, Row) {
	var formObj = document.form;
	if (formObj.svc_scp_cd.value == 'AEE') {
		if (sheetObj.CellValue(Row, 'org_rout_pnt_loc_def_cd') == '') {
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1": // sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "|||CMDT\nSeq.|Commodity Group|Commodity Group|Route\nSeq.|Origin|Origin|Origin|Origin|Origin|O.Via|D.Via|Destination|Destination|Destination|Destination|Destination|PER|Cargo Type|Rate(USD)|Rate(USD)|Rate(USD)|Guide Amt|Rt Use Status Cd||||||";
			var HeadTitle2 = "|||CMDT\nSeq.|Code|Description|Route\nSeq.|Code|Description|Loc Group|Term|Transmode|Code|Code|Code|Description|Loc Group|Term|Transmode|PER|Cargo Type|Rate|BOF|IHC|Guide Amt|Rt Use Status Cd||||||";
			...
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "fic_qttn_rt_amt", false, "", dfNullFloat, 2, true, true, 9);
			InitDataProperty(0, cnt++, dtHidden, 0, daRight, false, "fic_gline_rt_amt", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, false, "fic_rt_use_sts_cd", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtHidden, 0, daCenter, false, "optm_trsp_mod_flg", false, "", dfNone, 0, false, false);
			...
			if (document.form.svc_scp_cd.value == 'AEE') {

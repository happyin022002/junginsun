...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	function initSheet(sheetObj, sheetNo) {
	    ...
	    case "sheet1": // sheet1 init
			with (sheetObj) {
                ...
                var HeadTitle2 = "|CMDT\nSeq.|Code|Description|Route\nSeq.|Code|Description|Term|Transmode|Code|Code|Code|Description|Term|Transmode|Dry 20'|Dry 40'|Dry 40'HC|Dry 45'|RF 40'HC|RD 40'HC";
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        case IBSAVE: // Save
            ...
            try {
	            ...
	            var sXml = sheetObj.GetSaveXml("ESM_PRI_6056GS.do", sParam);
	...
	//                            formObj.f_cmd.value = SEARCH10;
	//                            var param = "&cd=" + cmdtCode + "&nm=rq" + "&etc1=" + formObj.qttn_no.value + "&etc2=" + formObj.qttn_ver_no.value;
	//                            var sXml = sheetObjects[1].GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
	//                            var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
	//                            if (arrData == null || arrData.length <= 0) {
	...
	//                            formObj.f_cmd.value = SEARCH11;
	//                            var param = "&cd=" + orgPntCode + "&nm=rq" + "&etc1=" + formObj.qttn_no.value + "&etc2=" + formObj.qttn_ver_no.value;
	//                            var sXml = sheetObjects[1].GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
	//                            var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
	//                            if (arrData == null || arrData.length <= 0) {
	//                                add2Tooltip(i, "org_rout_pnt_loc_def_cd", ComGetMsg("PRI00315"));
	...
	//                        formObj.f_cmd.value = SEARCH11;
	//                        var param = "&cd=" + orgViaCode + "&nm=rq" + "&etc1=" + formObj.qttn_no.value + "&etc2=" + formObj.qttn_ver_no.value;
	//                        var sXml = sheetObjects[1].GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
	//                        var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
	//                        if (arrData == null || arrData.length <= 0) {
	//                            add2Tooltip(i, "org_rout_via_port_def_cd", ComGetMsg("PRI00315"));
	...
	//                        formObj.f_cmd.value = SEARCH11;
	//                        var param = "&cd=" + destViaCode + "&nm=rq" + "&etc1=" + formObj.qttn_no.value + "&etc2=" + formObj.qttn_ver_no.value;
	//                        var sXml = sheetObjects[1].GetSearchXml("PRICommonGS.do", FormQueryString(formObj) + param);
	//                        var arrData = ComPriXml2ComboString(sXml, "cd", "nm");
	//
	//                        if (arrData == null || arrData.length <= 0) {

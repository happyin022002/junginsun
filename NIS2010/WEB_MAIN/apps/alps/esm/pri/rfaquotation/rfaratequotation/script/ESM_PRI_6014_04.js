...
/**
 * @extends
 * @class ESM_PRI_6014_04 : ESM_PRI_6014_03 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_6014_04() {
...
function processButtonClick() {
	...
	try {
		...
		case "btn_Load":
			...
			if (fic_rt_tp_cd == 'A') {
				if (rtn == 6) {
					ComPriOpenWindowCenter('/hanjin/ESM_PRI_6095.do?' + sUrl, "ESM_PRI_6095", 955, 556, true, "no");
				} else if (rtn == 7) {
					ComPriOpenWindowCenter('/hanjin/ESM_PRI_6096.do?' + sUrl, "ESM_PRI_6096", 955, 556, true, "no");
				}
			} else {
				if (rtn == 6) {
...
function ficRtTpCd_OnChange() {
	...
	controlSheetDisplay();
	doActionIBSheet(getRateSheet(), document.form, IBCLEAR);
...
function controlSheetDisplay() {
	...
	if (formObj.fic_rt_tp_cd[1].checked) {
		...
		if (isOViaMandatory) {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("org_rout_via_port_def_cd"), dtData, 100, daCenter, false, "org_rout_via_port_def_cd", true, "", dfNone, 0, false, false);
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("dest_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "dest_rout_via_port_def_cd", false, "", dfNone, 0, true, true);
			sheetObjects[5].InitDataProperty(0, sheetObjects[5].SaveNameCol("bse_port_def_cd"), dtData, 100, daCenter, false, "bse_port_def_cd", true, "", dfNone, 0, false, false);

		} else if (isDViaMandatory) {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("org_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "org_rout_via_port_def_cd", false, "", dfNone, 0, true, true);
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("dest_rout_via_port_def_cd"), dtData, 100, daCenter, false, "dest_rout_via_port_def_cd", true, "", dfNone, 0, false, false);
			sheetObjects[8].InitDataProperty(0, sheetObjects[8].SaveNameCol("bse_port_def_cd"), dtData, 100, daCenter, false, "bse_port_def_cd", true, "", dfNone, 0, false, false);
		}
	} else {
		...
		if (isOViaMandatory) {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("org_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "org_rout_via_port_def_cd", true, "", dfNone, 0, true, true);
		} else {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("org_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "org_rout_via_port_def_cd", false, "", dfNone, 0, true, true);
		}
		// D.Via 필수여부
		if (isDViaMandatory) {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("dest_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "dest_rout_via_port_def_cd", true, "", dfNone, 0, true, true);
		} else {
			sheetObjects[1].InitDataProperty(0, sheetObjects[1].SaveNameCol("dest_rout_via_port_def_cd"), dtPopup, 100, daCenter, false, "dest_rout_via_port_def_cd", false, "", dfNone, 0, true, true);
		}
	}
		...
		case IBSEARCH: // 조회
			...
			try {
				...
				if (sheetObj.id == "sheet1") {
					...
					sheetObj.DoSearch("ESM_PRI_6014_04GS.do", FormQueryString(formObj));
					...
					if (arrXml.length > 2) {
						sheetObjects[5].LoadSearchXml(arrXml[2]); // Hidden. Grid2의 Origin Point.
						if (formObj.svc_scp_cd.value == 'AEE') {
							funcRouteAppendGuideInfo(sheetObjects[5]);
						}
					}
					if (arrXml.length > 3)
						sheetObjects[6].LoadSearchXml(arrXml[3]); // Hidden. Grid2의 Origin Via.
					...
					if (arrXml.length > 5) {
						...
						if (formObj.svc_scp_cd.value == 'AEW') {
							funcRouteAppendGuideInfo(sheetObjects[8]);
						}
					}
				}
			} catch (e) {
		...
		case IBSAVE: // 저장
			...
			if (sheetObj.id == "sheet1") {
				try {
					...
					var sXml = sheetObj.GetSaveXml("ESM_PRI_6014_04GS.do", sParam);
...
function funcRouteAppendGuideInfo(sheetObj) {
	for ( var i = sheetObj.HeaderRows; i < sheetObj.Rows; i++) {
		...
		funcFicRoute(sheetObj, i);
		sheetObj.RowStatus(i) = status;
	}
	updateRouteMinValue(sheetObjects[1], sheetObjects[1].SelectRow);
}
...
function validateForm(sheetObj, formObj, sAction) {
	...
	case IBSAVE: // 저장
		if (sheetObj.id == "sheet1") {
			...
			if (getValidRowCount(RATE_SHEET) <= 0) {
			...
			if (RATE_SHEET.id == 'sheet12') {
				var zeroQttnRtAmtRow = RATE_SHEET.FindText('qttn_rt_amt', '0.00');
				if (zeroQttnRtAmtRow > -1) {
					ComShowCodeMessage("PRI00321", "Quotation", "0.00");
					return false;
				}

				var zeroFicQttnRtAmtRow = RATE_SHEET.FindText('fic_qttn_rt_amt', '0.00');
				if (zeroFicQttnRtAmtRow > -1) {
					ComShowCodeMessage("PRI00321", "IHC", "0.00");
					return false;
				}
			}
		}
	...
	if (OldRow != NewRow) {
		...
		if (sheetObjects[1].IsDataModified || RATE_SHEET.IsDataModified || sheetObjects[5].IsDataModified || sheetObjects[6].IsDataModified || sheetObjects[7].IsDataModified
			...
			if (ComShowCodeConfirm("PRI00006")) {
				supressConfirm = true;
				var rslt = doActionIBSheet(RATE_SHEET, document.form, IBSAVE);
			...
			if (sAction == IBINSERT) {
				...
				var sXml = sheetObjects[0].GetSearchXml("ESM_PRI_6014_03GS.do", FormQueryString(formObj));
	...
	if (OldRow != NewRow) {
		if (sheetObjects[0].IsDataModified || sheetObjects[5].IsDataModified || sheetObjects[6].IsDataModified || sheetObjects[7].IsDataModified || sheetObjects[8].IsDataModified) {
			...
			if (ComShowCodeConfirm("PRI00006")) {
				...
				var rslt = doActionIBSheet(saveSheet, document.form, IBSAVE);
				supressConfirm = false;
			}
...
function sheet1_OnPopupClick(sheetObj, Row, Col) {
	...
	if (colName == "prc_cmdt_def_cd") {
		var sUrl = "/hanjin/ESM_PRI_6060.do?qttn_no=" + formObj.qttn_no.value + "&qttn_ver_no=" + formObj.qttn_ver_no.value + "&cmdt_hdr_seq=" + formObj.cmdt_hdr_seq.value + "&fic_rt_tp_cd="
				+ funcCheckFicRtTpCdVal();
...
function sheet2_OnPopupClick(sheetObj, Row, Col) {
	...
	var sUrl = "/hanjin/ESM_PRI_6094.do?";
	...
	if (colName == "org_rout_pnt_loc_def_cd") {
		...
		var rtnVal = ComPriOpenWindowCenter(sUrl, "ESM_PRI_6059", 980, 324, true);
...
function sheet12_OnChange(sheetObj, Row, Col, Value) {
	...
	if (colName == 'fic_gline_rt_amt') {
		...
		if (ficGlineRtAmt == "N/A") {
			...
			sheetObj.CellValue2(Row, "fic_qttn_rt_amt") = 0;
			sheetObj.CellValue2(Row, "diff_qttn_rt_amt") = "N/A";
		} else {
			sheetObj.CellValue2(Row, "qttn_rt_amt") = ficGlineRtAmt;
			sheetObj.CellValue2(Row, "qttn_bof_amt") = 0.00;
			sheetObj.CellValue2(Row, "fic_qttn_rt_amt") = ficGlineRtAmt;
			sheetObj.CellValue2(Row, "diff_qttn_rt_amt") = 0.00;
		}
	}
	...
	if (colName == "qttn_rt_amt") {
		...
		if (Value != null && Value != "") {
			...
			var sFicPropRtAmt = sheetObj.CellValue(Row, "fic_qttn_rt_amt"); // Proposal IHC 금액
			...
			sheetObj.CellValue2(Row, "qttn_bof_amt") = ComGetMaskedValue((nPropFrtRtAmt - nFicPropRtAmt) / 100, "float");
		}
		if (sheetObj.RowStatus(Row) == "U") {
			sheetObj.CellValue2(Row, "qttn_rt_adj_tp_cd") = "M";
	...
	if (colName == "fic_qttn_rt_amt") {
		var sFicPropRtAmt = sheetObj.CellValue(Row, "fic_qttn_rt_amt");
		if (!sFicPropRtAmt) {
			return false;
		}
		var nFicPropRtAmt = Number(sFicPropRtAmt);
		...
		if (sFicGlineRtAmt != "N/A") {
			...
			sheetObj.CellValue2(Row, "diff_qttn_rt_amt") = ComAddComma2(((nFicPropRtAmt - nFicGlineRtAmt) / 100).toString(), "#,###.00");
		}
 ...
 * @param Row
 */
function funcFicRoute(sheetObj, Row) {
	var formObj = document.form;
	var sender = sheetObjects[10];
	formObj.f_cmd.value = SEARCH01;
	var sParam = FormQueryString(formObj);
	sParam += "&rout_pnt_loc_def_cd=" + sheetObj.CellValue(Row, "rout_pnt_loc_def_cd") + "&bse_port_def_cd=" + sheetObj.CellValue(Row, "bse_port_def_cd") + "&rcv_de_term_cd="
			+ sheetObj.CellValue(Row, "rcv_de_term_cd") + "&prc_trsp_mod_cd=" + sheetObj.CellValue(Row, "prc_trsp_mod_cd");
...
function updateRateIhcValue(routeSheet, Row) {
	...
	for ( var i = sheet12.HeaderRows; i < sheet12.Rows; i++) {
		...
		if (ficGlineInfo[0] == '' || ficGlineInfo[0] == 'N/A') {
			sheet12.CellValue2(i, 'fic_gline_rt_amt') = 'N/A';
			sheet12.CellValue(i, 'fic_qttn_rt_amt') = 0;
			sheet12.CellValue2(i, 'diff_qttn_rt_amt') = 'N/A';
		} else {
			sheet12.CellValue2(i, 'fic_gline_rt_amt') = ficGlineInfo[0];
			sheet12.CellValue2(i, 'fic_qttn_rt_amt') = 0;
			sheet12.CellValue(i, 'fic_qttn_rt_amt') = ficGlineInfo[0];
		}
		sheet12.CellValue2(i, 'fic_rt_use_sts_cd') = ficGlineInfo[1];
		sheet12.CellValue2(i, 'optm_trsp_mod_flg') = ficGlineInfo[2];
	}
	sheet12.Redraw = true;
}
...
function calcRate2(sheetObj, Row) {
	...
	if (ficGlineInfo[0] == '' || ficGlineInfo[0] == 'N/A') {
		sheetObj.CellValue(Row, "fic_qttn_rt_amt") = 0.00;
	} else {
		sheetObj.CellValue(Row, "fic_qttn_rt_amt") = ficGlineInfo[0];
	}

	var nFicPropRtAmt = Number(sheetObj.CellValue(Row, "fic_qttn_rt_amt"));
	nFicPropRtAmt = nFicPropRtAmt * 100;
	nFicPropRtAmt = nFicPropRtAmt.toFixed();
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet2": // Grid 2
		with (sheetObj) {
			...
			var HeadTitle = "|Sel.|dataSeq|Seq.|qttn_no|qttn_ver_no|gen_spcl_rt_tp_cd|cmdt_hdr_seq|rout_seq|Origin|O.Via|D.Via|Dest."
					+ "|fic_rt_use_sts_cd|optm_trsp_mod_flg|dr_20ft_amt|rf_20ft_amt|dg_20ft_amt|dr_40ft_amt|rf_40ft_amt|dg_40ft_amt"
	...
	case "sheet12": // Grid 12 Rate
		with (sheetObj) {
			...
			var HeadTitle1 = "|Sel.|Seq.|Per|Cargo Type|Cur.|Quotation|Quotation|Quotation|Quotation|Surcharge|Cost|Cost|CMPB|CMPB\nGuideline|OPB|Diff.|Source"
					+ "|Rate Adjust|Rate Adjust|qttn_no|qttn_ver_no|cmdt_hdr_seq|rout_seq|rt_seq|src_info_cd" + "|qttn_init_rt_amt|prs_pfit_cm_uc_amt|prs_pfit_cmpb_amt"
					+ "|fic_gline_rt_amt|fic_rt_use_sts_cd|optm_trsp_mod_flg";
			var HeadTitle2 = "|Sel.|Seq.|Per|Cargo Type|Cur.|Total|BOF|IHC|Diff|Surcharge|Cost|Cost|CMPB|CMPB\nGuideline|OPB|Diff.|Source"
					+ "|Rate Adjust|Rate Adjust|qttn_no|qttn_ver_no|cmdt_hdr_seq|rout_seq|rt_seq|src_info_cd" + "|qttn_init_rt_amt|prs_pfit_cm_uc_amt|prs_pfit_cmpb_amt"
					+ "|fic_gline_rt_amt|fic_rt_use_sts_cd|optm_trsp_mod_flg";
			...
			InitDataProperty(0, cnt++, dtData, 85, daRight, true, "fic_qttn_rt_amt", true, "", dfNullFloat, 2, true, true, 9);
			InitDataProperty(0, cnt++, dtData, 70, daRight, true, "diff_qttn_rt_amt", false, "", dfNone, 2, false, false, 9);
	...
	case "sheet6": // point origin
		with (sheetObj) {
			...
			var HeadTitle = "|Sel.|Seq.|1|2|3|4|5|6|Location Type|Location Code|Description|src_info_cd|display_seq|Term|rcv_de_term_nm|Trans. Mode|O.Via"
	...
	case "sheet9": // point dest
		with (sheetObj) {
			...
			var HeadTitle = "|Sel.|Seq.|1|2|3|4|5|6|Location Type|Location Code|Description|src_info_cd|display_seq|Term|rcv_de_term_nm|Trans. Mode|D.Via"
					+ "|bse_port_loc_cd|base_port_list|optm_trsp_mod_flg|fic_rout_cmb_tp_cd|fic_rt_use_sts_cd|fic_gline_upd_dt|group_no"
					+ "|dr_20ft_amt|rf_20ft_amt|dg_20ft_amt|dr_40ft_amt|rf_40ft_amt|dg_40ft_amt"
					+ "|locl_curr_cd|dr_locl_20ft_amt|rf_locl_20ft_amt|dg_locl_20ft_amt|dr_locl_40ft_amt|rf_locl_40ft_amt|dg_locl_40ft_amt";
	...
	case "sheet13": // Grid 13 Rate
		with (sheetObj) {
			...
			var HeadTitle1 = "CMDT\nSeq.|Commodity Group|Commodity Group|Route\nSeq.|Origin|Origin|Origin|Origin|O.Via|D.Via|Destination|Destination|Destination|Destination|PER|Cargo Type|Rate(USD)|Rate(USD)|Rate(USD)";
			var HeadTitle2 = "CMDT\nSeq.|Code|Description|Route\nSeq.|Code|Description|Term|Transmode|Code|Code|Code|Description|Term|Transmode|PER|Cargo Type|Rate|BOF|IHC";
			var headCount = ComCountHeadTitle(HeadTitle2);
			...
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "fic_qttn_rt_amt", true, "", dfNullFloat, 2, false, false);
			WaitImageVisible = false;
		}
		break;
	...
	case "sheet14": // Excel Download용 Sheet(Horizontal(AEE/AEW))
		with (sheetObj) {
			...
			var HeadTitle1 = "CMDT\nSeq.|Commodity Group|Commodity Group|Route\nSeq.|Origin|Origin|Origin|Origin|O.Via|D.Via|Destination|Destination|Destination|Destination|D.Call|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)|Rate(USD)";
			var HeadTitle2 = "CMDT\nSeq.|Code|Description|Route\nSeq.|Code|Description|Term|Transmode|Code|Code|Code|Description|Term|Transmode|Y/N|Dry 20'|Dry 20'|Dry 20'|Dry 40'|Dry 40'|Dry 40'|Dry 40'HC|Dry 40'HC|Dry 40'HC|Dry 45'|Dry 45'|Dry 45'|RF 40'HC|RF 40'HC|RF 40'HC|RD 40'HC|RD 40'HC|RD 40'HC";
			var HeadTitle3 = "CMDT\nSeq.|Code|Description|Route\nSeq.|Code|Description|Term|Transmode|Code|Code|Code|Description|Term|Transmode|Y/N|Quotation|BOF|IHC|Quotation|BOF|IHC|Quotation|BOF|IHC|Quotation|BOF|IHC|Quotation|BOF|IHC|Quotation|BOF|IHC";
			...
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_dry20", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_dry40", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_bof_dry40", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_dry40", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_dry40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_bof_dry40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_dry40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_dry45", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_bof_dry45", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_dry45", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_rf40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_bof_rf40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_rf40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_rd40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_bof_rd40hc", true, "", dfNullFloat, 2, false, false);
			InitDataProperty(0, cnt++, dtData, 100, daRight, false, "rate_fic_rd40hc", true, "", dfNullFloat, 2, false, false);
			...
			sheetObj.SetMergeCell(1, 15, 1, 3);
			sheetObj.SetMergeCell(1, 18, 1, 3);

...
/**
 * @extends
 * @class ESM_PRI_6015_05 : ESM_PRI_6015_05 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_PRI_6015_05() {
 ...
 * @version 2009.04.17
 */
function sheet12_OnLoadFinish(sheetObj) {
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet2": // Grid 2
		with (sheetObj) {
			...
			var HeadTitle = "|Seq.|qttn_no|qttn_ver_no|cmdt_hdr_seq|rout_seq|Origin|O.Via|D.Via|Dest.|org_cy_dor_rt_tp_cd|dest_cy_dor_rt_tp_cd";
			...
			InitDataProperty(0, cnt++, dtHidden, 110, daCenter, false, "org_cy_dor_rt_tp_cd", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtHidden, 110, daCenter, false, "dest_cy_dor_rt_tp_cd", false, "", dfNone, 0, true, true);
	...
	case "sheet4": // Grid 4 Rate
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtCombo, 60, daCenter, true, "rat_ut_cd", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtCombo, 90, daCenter, true, "prc_cgo_tp_cd", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtCombo, 50, daCenter, true, "curr_cd", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtData, 85, daRight, true, "qttn_rt_amt", false, "", dfNullFloat, 2, true, true, 9);
	...
	case "sheet12": // Grid 4 Rate
		with (sheetObj) {
			...
			var HeadTitle1 = "|Seq.|Per|Cargo Type|Cur.|Quotation|Quotation|Quotation|Quotation|Quotation|Quotation|Eff.Date|Exp.Date|Source|Rate Adjust|fic_org_gline_rt_amt|fic_org_rt_use_sts_cd|org_optm_trsp_mod_flg|fic_dest_gline_rt_amt|fic_dest_rt_use_sts_cd|dest_optm_trsp_mod_flg";
			var HeadTitle2 = "|Seq.|Per|Cargo Type|Cur.|Total|BOF|OIH|DIH|Diff|Diff|Eff.Date|Exp.Date|Source|Rate Adjust|fic_org_gline_rt_amt|fic_org_rt_use_sts_cd|org_optm_trsp_mod_flg|fic_dest_gline_rt_amt|fic_dest_rt_use_sts_cd|dest_optm_trsp_mod_flg";
			var HeadTitle3 = "|Seq.|Per|Cargo Type|Cur.|Total|BOF|OIH|DIH|OIH|DIH|Eff.Date|Exp.Date|Source|Rate Adjust|fic_org_gline_rt_amt|fic_org_rt_use_sts_cd|org_optm_trsp_mod_flg|fic_dest_gline_rt_amt|fic_dest_rt_use_sts_cd|dest_optm_trsp_mod_flg";
			...
			InitDataProperty(0, cnt++, dtData, 100, daRight, true, "fic_org_qttn_rt_amt", false, "", dfFloat, 2, true, true);
			InitDataProperty(0, cnt++, dtData, 100, daRight, true, "fic_dest_qttn_rt_amt", false, "", dfFloat, 2, true, true);
			InitDataProperty(0, cnt++, dtData, 100, daRight, true, "org_diff_qttn_rt_amt", false, "", dfNone, 2, true, true);
			InitDataProperty(0, cnt++, dtData, 100, daRight, true, "dest_diff_qttn_rt_amt", false, "", dfNone, 2, true, true);
			...
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "fic_org_gline_rt_amt", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "fic_org_rt_use_sts_cd", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "org_optm_trsp_mod_flg", false, "", dfNone, 0, false, false);
			
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "fic_dest_gline_rt_amt", false, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "dest_org_rt_use_sts_cd", false, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtHidden, 50, daCenter, false, "dest_optm_trsp_mod_flg", false, "", dfNone, 0, false, false);
			CountPosition = 0;
			WaitImageVisible = false;
		}
		break;
...
function sheet1_OnSelectCell(sheetObj, OldRow, OldCol, NewRow, NewCol) {
	var formObj = document.form;
	formObj.cmdt_hdr_seq.value = sheetObj.CellValue(NewRow, "cmdt_hdr_seq");
	formObj.src_info_cd.value = sheetObj.CellValue(NewRow, "seq");
	...
	case IBSEARCH: // 조회
		...
		try {
			...
			if (sheetObj.id == "sheet1") {
				...
				sheetObj.DoSearch("ESM_PRI_6015_05GS.do", FormQueryString(formObj));
				...
				formObj.f_cmd.value = SEARCH03;
				var sXml = sheetObj.GetSearchXml("ESM_PRI_6015_05GS.do", FormQueryString(formObj));
				var arrXml = sXml.split("|$$|");

				if (arrXml.length > 0)
					sheetObjects[4].LoadSearchXml(arrXml[0]); // commodity.
				if (arrXml.length > 1)
					sheetObj.LoadSearchXml(arrXml[1]); // Grid3.
				if (arrXml.length > 2)

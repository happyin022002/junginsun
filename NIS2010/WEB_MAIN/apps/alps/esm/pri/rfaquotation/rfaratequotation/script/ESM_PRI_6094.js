...
function initSheet(sheetObj, sheetNo) {
	...
	case 5: // sheet5 init
		with (sheetObj) {
			...
			var HeadTitle = "|Sel.|Seq.|1|2|3|4|5|6|Location Type|Location Code|Description|Loc Group|src_info_cd|display_seq|Term|Base Port|bse_port_loc_cd|Trans. Mode"
			...
			InitDataProperty(0, cnt++, dtComboEdit, 90, daCenter, false, "bse_port_def_cd", true, "", dfNone, 0, true, true);
			InitDataProperty(0, cnt++, dtHidden, 30, daLeft, false, "bse_port_loc_cd", true, "", dfNone, 0, false, false);
			InitDataProperty(0, cnt++, dtCombo, 90, daCenter, false, "prc_trsp_mod_cd", true, "", dfNone, 0, true, true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	try {
			...
			if (checkStr != "X") {
				...
				if( pntSheet.IsDataModified ){
					...
					if (strBasePortCd != strViaPortCd) {
						deleteAndInsertVia(viaSheet, basePortLocCdList, 'V', 'O');
					}
				}
				...
				if( pntSheet.IsDataModified ){
					...
					var strViaPortCd = viaPortLocCdList.join("|");
					if (strBasePortCd != strViaPortCd) {
						deleteAndInsertVia(viaSheet, basePortLocCdList, 'V', 'D');
					}	
				}			
			}
			// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		case IBSEARCH_ASYNC06: // Search FIC Route
			...
			sParam += "&svc_scp_cd=" + sheetObj.CellValue(Row, "svc_scp_cd") + "&rout_pnt_loc_def_cd=" + sheetObj.CellValue(Row, "rout_pnt_loc_def_cd") + "&bse_port_def_cd="
					+ sheetObj.CellValue(Row, "bse_port_def_cd") + "&rcv_de_term_cd=" + sheetObj.CellValue(Row, "rcv_de_term_cd") + "&prc_trsp_mod_cd=" + sheetObj.CellValue(Row, "prc_trsp_mod_cd")
		...
		case IBSEARCH_ASYNC07: // Search FIC Route
			...
			var sParam = FormQueryString(formObj);
			sParam += "&svc_scp_cd=" + sheetObj.CellValue(Row, "svc_scp_cd") + "&rout_pnt_loc_def_cd=" + sheetObj.CellValue(Row, "rout_pnt_loc_def_cd") + "&bse_port_def_cd="
					+ sheetObj.CellValue(Row, "bse_port_def_cd") + "&rcv_de_term_cd=" + sheetObj.CellValue(Row, "rcv_de_term_cd") + "&eff_dt=" + sheetObj.CellValue(Row, "eff_dt");
			var sXml = sender.getSearchXml("ESM_PRI_6094GS.do", sParam);
...
function deleteAndInsertVia(viaSheet, viaPortLocCdList, inCurPntViaType, inCurOrgDestType) {
	var formObj = document.form;
	if (formObj.qttn_no.value == "" || formObj.qttn_ver_no.value == "" || formObj.svc_scp_cd.value == "") {
		return false;
	}

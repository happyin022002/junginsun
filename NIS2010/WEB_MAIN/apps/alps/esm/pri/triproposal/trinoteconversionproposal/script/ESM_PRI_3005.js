...
=========================================================
    ...
    * 2015.04.02송호진  [CHM-201535140] S/C Note conversion에 중복 체크 로직 수정요청 - validateDupCheck 상의 로직 오류 수정 
=========================================================*/
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_PRI_3005 : ESM_PRI_3005 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_PRI_3005() {
         	...
         	case "sheet1":
         		with (sheetObj) {
                    ...
                    //전체Edit 허용 여부 [선택, Default false]
                     ...
                     InitDataProperty(0, cnt++ , dtPopupEditFormat,	100,  daCenter,  true,	"exp_dt",     				false,	"",	dfDateYmd, 		 	0,     true,       true);
                     ...
                     InitDataProperty(0, cnt++ , dtCombo,        	75,   daCenter,  true,	"rule_appl_chg_tp_cd", 		false,	"",	dfNone, 			0,     true,       true);                     
                     InitDataProperty(0, cnt++ , dtCombo,   	 	60,   daCenter,  true,	"conv_rat_ut_cd",   		false,	"",	dfNone, 			0,     true,       true);
                     InitDataProperty(0, cnt++ , dtCombo,  			70,   daCenter,  true,	"conv_prc_cgo_tp_cd",		false,	"",	dfNone, 			0,     true,       true);
 		...
 		try {
	 		...
	 		switch (sAction) {
				case IBSEARCH_ASYNC01: // Service Scope 선택시, Duration조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_PRI_3005GS.do", FormQueryString(formObj));
				...
				case IBSEARCH_ASYNC02: // Duration선택후 INFO 정보 세팅
					...
					var sXml = sheetObj.GetSearchXml("ESM_PRI_3005GS.do", sParam);
	 			...
	 			case IBSAVE: // Save
		 			...
		 			if(isCopy) {
			 			if (!ComShowCodeConfirm('PRI00012')) {
		  				...
		  				ComOpenWait(true);			
		 				//생성
		 				if(comboObjects[1].Code == "" || comboObjects[1].Code == "X"){
		 					formObj.note_conv_mapg_id.value = getSYSGUID();
		 				}
		 				
	 					for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) { 						
	 						sheetObj.CellValue2(i, "note_conv_mapg_id") = formObj.note_conv_mapg_id.value;
	 						sheetObj.CellValue2(i, "trf_pfx_cd") = formObj.trf_pfx_cd.value;
	 						sheetObj.CellValue2(i, "trf_no") = formObj.trf_no.value;				
		 				...
		 				var sParam = FormQueryString(formObj);
						sParam += "&note_seq=" + comboObjects[1].Code;
		 				var sParamSheet = sheetObj.GetSaveString();
		  				if (sParamSheet != "") {
		  					sParam += "&" + ComPriSetPrifix(sParamSheet, "sheet1_");
		  				}
	 				...
	 				var sXml = sheetObj.GetSaveXml("ESM_PRI_3005GS.do", sParam);
	 				...
	 				comboObjects[1].Code = ""; //duration list 를 가져오기 위해서 공백처리
	 				formObj.f_cmd.value = SEARCH02;
	 				sXml = sheetObj.GetSearchXml("ESM_PRI_3005GS.do", FormQueryString(formObj));
	 				ComPriXml2ComboItem(sXml, formObj.note_seq, "note_seq", "eff_dt|exp_dt|eff_dt");
	 				comboObjects[1].InsertItem(0, "||", "X");
	 				...
	 				if (code == null || code == "" || code == "X") {
	 					...
	 					comboObjects[1].Code = code; //onChange 이벤트 발생
	 				}
	 				break;
 	...
 	function sheet1_OnChange(sheetObj, Row, Col, Value) {
 		...
 		switch(colName)
     	{
 			...
 			case "exp_dt":	
 				...
 				if(sheetObj.CellValue(Row, "exp_dt") > expDt && sheetObj.CellValue(Row, "exp_dt") != "" ) {
 					ComShowCodeMessage("PRI08016");
 		...
 		if(rowCount > 0){
 			for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
				if (sheetObj.RowStatus(i) == "D") {
 				...
 				if((expDt != "")  &&  ((sheetObj.CellValue(i, "exp_dt") > expDt) || (sheetObj.CellValue(i, "exp_dt") == ""))) {
 					ComShowCodeMessage("PRI08016");
 					sheetObj.CellValue2(i, "exp_dt") = expDt;
		...
		case IBCREATE: // New
   		...
   		case IBCOPYROW:
   			...
   			*/
   			...
   			if (comboObjects[0].Code == "" || comboObjects[1].Text == "") {
 				return false;
 			}
   ...
   */
   	function initComboChargeRuleCode(sheetObj, formObj) {   		
   		...
   		var tXml = sheetObj.GetSearchXml("ESM_PRI_3005GS.do", FormQueryString(formObj));
   ...
   /*	
 	...
 	function note_seq_OnChange(comboObj, code, text) {
 		...
 		formObj.exp_dt_hidden.value = expText; //CONVERSION EXP_DT변경시 이전날짜와 비교하기위해 사용
 		
 		if (code == null || code == "" || code == "X") {
 			if(sheetObjects[0].RowCount > 0 && !isCopy){
 	...
 	/**
	 * 키를 개체 위에서 눌럿다 놓았을 때 발생하는 이벤트이다.<br>
	 * <br><b>Example :</b>
	 * <pre>
	 *   
	 * </pre>
  ...
  /**
	* 메인 Duration에서 입력한 EFF_DT 값을 SHEET의 EFF_DT에 자동 변경한다.<br>
	* <br><b>Example :</b>
	* <pre>
	* modifyConversionEffDt (effDt);
	* </pre>
	* @param {string} effDt 필수 form object
	* @return 없음
	* @author 최성민
	* @version 2009.08.13
	*/       
  	function modifyConversionEffDt (effDt) {
  		...
  		if (cfmFlg != "Y" && effDt.length == 8) {
 	 		for(var i = sheetObjects[0].HeaderRows; i <= sheetObjects[0].LastRow; i++) {
  				if(!isCopy){
 	 				if(bEffDt == sheetObjects[0].CellValue(i, "eff_dt")) {
 	 					sheetObjects[0].CellValue2(i, "eff_dt") =effDt;
 	 				} else if(effDt > sheetObjects[0].CellValue(i, "eff_dt")) {
 	 					sheetObjects[0].CellValue2(i, "eff_dt") =effDt;
 	 				}
  				} else if(isCopy){
 	 				sheetObjects[0].CellValue2(i, "eff_dt") =effDt;
   ...
   /**
	* 메인 Duration에서 입력한 EXP_DT 값을 SHEET의 EXP_DT에 자동 변경한다.<br>
	* <br><b>Example :</b>
	* <pre>
	* modifyConversionExpDt (expDt);
	* </pre>
	* @param {string} expDt 필수 form object
	* @return 없음
	* @author 최성민
	* @version 2009.08.13
	*/    
  	function modifyConversionExpDt (expDt) {
 		...
 		var bExpDt = formObj.exp_dt_hidden.value; 
  		...
  		if (cfmFlg != "Y" && expDt.length == 8) { 
 	 		...
 	 		for(var i = sheetObjects[0].HeaderRows; i <= sheetObjects[0].LastRow; i++) {
 	 			if(!isCopy){
 	 				if(bExpDt == sheetObjects[0].CellValue(i, "exp_dt")) {
 	 					sheetObjects[0].CellValue2(i, "exp_dt") =expDt;
 	 				} else if(expDt < sheetObjects[0].CellValue(i, "exp_dt")) {
 	 					sheetObjects[0].CellValue2(i, "exp_dt") =expDt;
 	 				}
 		 		} else if(isCopy){
 		 			sheetObjects[0].CellValue2(i, "exp_dt") =expDt;
 	 		...
 	 		//현재 EXP_DT를 exp_dt_hidden 에 넣는다.

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
 /**
     ...
     * @author 한진해운
     * @extends 
     * @class esm_bis_0743  생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esm_bis_0743_01() {
...
//      ComOpenWait(false);
    ...
    function rdOpen(formObject, RiderYn, HBYn,p_bkg_no,bl_cpy_knt,obl_rider_flg){
        ...
        var formHisCntr = formObject.bl_ca_yn.checked == true ? "BIS_CNTR_HIS" : "BIS_CONTAINER"; // form_his_cntr
        var formHisBkg = formObject.bl_ca_yn.checked == true ? "BIS_BKG_HIS" : "BIS_BOOKING"; // form_his_bkg
        var formHisMkd = formObject.bl_ca_yn.checked == true ? "BIS_BL_MK_DESC_HIS" : "BIS_BL_MK_DESC"; // form_his_mkd
        var formHisXpt = formObject.bl_ca_yn.checked == true ? "BIS_XPT_IMP_LIC_HIS" : "BIS_XPT_IMP_LIC"; // form_his_xpt
        var formHisBl = formObject.bl_ca_yn.checked == true ? "BIS_BL_DOC_HIS" : "BIS_BL_DOC"; // form_his_bl
        ...
        if(getRadioValue2(form.paper_type) == '1'){
            strFacePath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_OBL_A4.mrd";
        }else if(getRadioValue2(form.paper_type) == '4'){
            strFacePath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_OBL_LETTER.mrd";
        }else if(getRadioValue2(form.paper_type) == '10'){
            strFacePath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_OBL_DOT.mrd";
        }
        /*
        /rv form_bkgNo[(  'ATLZ4030031', 'TPEZ1295047', 'MUCZ3270004', 'HAMY5080012', 'SZPZ1145010', 'BKK6C135013' )]
             ...
             form_CorrNo[] ---> Default
             form_his_cntr[BIS_CONTAINER] ---> Default
             form_his_bkg[BIS_BOOKING] ---> Default
             form_his_mkd[BIS_BL_MK_DESC] ---> Default
             form_his_xpt[BIS_XPT_IMP_LIC] ---> Default
             form_his_bl[BIS_BL_DOC] ---> Default
        /rp [] ---> Default
        /riprnmargin
        */
        ...
        //alert("rdParam_Face : [" + rdParam_Face + "]");
        ...
        if ( obl_rider_flg =="Y") {
            ...
            RiderPrintYn = true;
            if(getRadioValue2(form.paper_type) == '1'){
                strRiderPath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_A4_Rider.mrd";
            }else if(getRadioValue2(form.paper_type) == '4'){
                strRiderPath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_LETTER_Rider.mrd";
            }else if(getRadioValue2(form.paper_type) == '10'){
                strRiderPath = RD_path+"apps/alps/esm/bis/blinformationmgt/blinformationmgt/report/ESM_BIS_0109_DOT_Rider.mrd";
            }
            /*
            /rv form_bkgNo[(  'ATLZ4030031', 'TPEZ1295047', 'MUCZ3270004', 'HAMY5080012', 'SZPZ1145010', 'BKK6C135013' )]
                 form_hiddeData[N] ---> Default
...
//      Rdviewer_Face.FileOpen(strFacePath, RDServer + rdParam_Face);
        /* 2010.04.01 BIS RD 는 BAT 에 붙어서 출력하므로 RDServerBAT 으로 수정 */
        Rdviewer_Face.FileOpen(strFacePath, RDServerBAT + rdParam_Face);

        if (RiderPrintYn) {
            Rdviewer_Rider.SetMessageboxShow(1); //에러메시지만 출력함
//          Rdviewer_Rider.FileOpen(strRiderPath, RDServer + rdParam_Rider);
            /* 2010.04.01 BIS RD 는 BAT 에 붙어서 출력하므로 RDServerBAT 으로 수정 */
            Rdviewer_Rider.FileOpen(strRiderPath, RDServerBAT + rdParam_Rider);
        }
        ...
        if (RiderPrintYn == true && Rider_PrnCnt > 0) {
            ...
            // 인쇄시 뜨는 대화상자 모두 숨김 : 4
            // Rdviewer_main.SetPrintInfo (프린터 드라이버 이름, 인쇄매수, 1-한부씩 인쇄, 2-낱장인쇄, 1-DB접속다이얼로그만 숨김, 2-상태다이얼로그까지 숨김, 3-프린트 다이얼로그까지 숨김, 4-프린터 취소 다이얼로그까지 숨김)
            Rdviewer_Rider.SetPrintInfo (Rider_PrnDrv, Rider_PrnCnt, 1, 4);
...
/*********************** KEY EVENT END ********************/
   ...
   function setPrintcnt(){
    if(form.form_type.Code == "0"){
        ...
        form.face_print_cnt.options[3].selected = true; // 프린트시 저장된 것이 없으면 기본으로 3장 출력
        form.rider_print_cnt.options[3].selected = true;
  ...
  function doPrint(sheetObject1, formObject,releaseYn){
    ...
    success_cnt = 0;
        ...
        for (var i = HeaderRows; i < HeaderRows + RowCount ; i++) {
                ...
                ComOpenWait(true);
                rdStart(CellValue(i,"bkg_no"),CellValue(i,"bl_cpy_knt"),CellValue(i,"obl_rider_flg"));
                ComOpenWait(false);
     ...
     function doActionIBSheet(sheetObj,formObj,sAction,Row,Col,PageNo) {
         ...
         switch(sAction) {
                        ...
                        case SEARCH02:      //조회
                            ...
                            formObj.f_cmd.value = SEARCH02;
//                          formObj.bkg_no.value = "'"+ComReplaceStr(formObj.p_bkg_no.value,",","','")+"'";
                            formObj.module.value = "BIS";
              ...
              with (sheetObj) {
                ...
                //전체Edit 허용 여부 [선택, Default false]
                 Editable = false;
                    ...
                    var HeadTitle1 = " |BKG No.|B/L No.|Result|Remarks|B/L Type|Special CGO|Issued|OB/L Print|Released|No. of OB/L|Rider YN";

...
* 2017.05.15 김동호 ERP MAS Shutdown으로 인한 기능 추가 : RowDel, Month Copy
=========================================================*/ 
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0152 : ESM_MAS_0152 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0152() {
  	...
  	*/
  		...
  		switch(sheetNo) {
  			case 1:		//sheet2 init
  				with (sheetObj) {
  					...
  					InitColumnInfo(11, 5, 0, true);//컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
  	...
  	function sheet1_OnChange(sheetObj, Row, Col, value){
  	    ...
  	    if(sheetObj.ColSaveName(Col) == "trd_cd"){
			...
			var sXml = sheetObj.GetSearchXml("ESM_MAS_0152GS.do", param);
  		...
  		switch(sAction) {
  			...
  			case IBSEARCH://조회
  				if(validateForm(sheetObj,formObj,sAction)){
      	    		...
      	    		sheetObj.DoSearch4Post("ESM_MAS_0152GS.do", masFormQueryString(formObj));
  			...
  			case IBSAVE:	//저장
  				if(validateForm(sheetObj,formObj,sAction)){
                    ...
                    var rowM = sheetObj.ColValueDup("rev_yrmon|trd_cd|rlane_cd|dir_cd", false);
                    if (rowM >= 0) {
                   ...
                   for (idx=0; idx<arrRow.length-1; idx++){
                       ...
                       if(sheetObj.CellValue(arrRow[idx],"rlane_cd") == ""){
                           ComShowMessage(ComGetMsg("MAS00013","Lane"));
                           return false;
                       }   
                       if(sheetObj.CellValue(arrRow[idx],"dir_cd") == ""){
                           ComShowMessage(ComGetMsg("MAS00013","Bound"));

                           return false;
                       }
                       if(sheetObj.CellValue(arrRow[idx],"trd_ttl_amt") == "" || sheetObj.CellValue(arrRow[idx],"trd_ttl_amt") == "0"){
                           ComShowMessage(ComGetMsg("MAS00013","AMT"));
                           return false;
                       }  
                       if(sheetObj.CellValue(arrRow[idx],"trd_ttl_qty") == "" || sheetObj.CellValue(arrRow[idx],"trd_ttl_qty") == "0"){
                           ComShowMessage(ComGetMsg("MAS00013","VOL"));
  					...
  					sheetObj.DoSave("ESM_MAS_0152GS.do", masFormQueryString(formObj));
  					ComOpenWait(false);
  				}
  				break;
...
//				sheetObj.Down2Excel(-1, false, false, true);
  			...
  			case IBSEARCH_ASYNC01: // Month Copy
                var display = "0,1";
                ComOpenPopup("ESM_MAS_0173.do?classId=ESM_MAS_0152&check_cnt=Y", 250, 200, "MRI Inquiry (PA/RA)", display, true, false);
	...
	function f_trd_cd_OnChange(obj) {
		...
		if (formObj.istrade[1].checked){
			if (obj.Text != "") {
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0152GS.do", FormQueryString(formObj));

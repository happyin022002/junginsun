...
function tab1_OnChange(tabObj , nItem){
	...
	beforetab = nItem;
	//ESM_MAS_0316
	var arr_f_cobcost = ["54350000","54350009","54250009","54150009","54200009","53101000","53102000","53200000","65000000","54400000"];
...
function doActionIBSheet(sheetObj,formObj,sAction) {
    ...
    switch(sAction) {
        ...
        case IBSEARCH:      //결과조회
            ...
            if("65000000" == ComGetObjValue(formObj.f_cobcost) ){
            	ComSetObjValue(formObj.f_avg_grp_cd, "XX");
                formObj.f_cmd.value = SEARCH;
                sheetObjects[9].DoSearch("ESM_MAS_0315GS.do", masFormQueryString(formObj));
                ComSetObjValue(formObj.f_avg_grp_cd, "");
            }
        ...
        case IBSAVE:	//저장
			...
			if(validateForm(sheetObj,formObj,sAction)){
				...
				if(sheetObj.id == "t9sheet1"){
					if(sheetObjects[8].RowCount("U") == 0 && sheetObjects[9].RowCount("U") == 0)
						sheetObj.DoSave("ESM_MAS_0315GS.do", masFormQueryString(formObj));
					if(sheetObjects[8].RowCount("U") > 0)
						sheetObjects[8].DoSave("ESM_MAS_0315GS.do", masFormQueryString(formObj));
					if(sheetObjects[9].RowCount("U") > 0)
						sheetObjects[9].DoSave("ESM_MAS_0315GS.do", masFormQueryString(formObj));
		...
		case IBDOWNEXCEL:	//엑셀 다운로드
			if("65000000" == ComGetObjValue(formObj.f_cobcost) ){
				...
				sheetObjects[8].SpeedDown2Excel(-1, true, false);
			}else{
				sheetObj.SpeedDown2Excel(-1, false, false);
			}		
			break;
...
function t10sheet1_OnSaveEnd(ErrMsg){
	var formObject = document.form;
	doActionIBSheet(sheetObjects[10],formObject,IBSEARCH);
}
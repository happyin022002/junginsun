...
/**
 * @extends 
 * @class ESM_MAS_0002 : ESM_MAS_0002 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0002() {
...
document.onclick = processButtonClick;
	...
	function goURL( kind ){
		if ( kind == "1" )	{
			document.location.href = "/hanjin/ESM_MAS_0002.do";
		} else if ( kind == "2") {
			document.location.href = "/hanjin/ESM_MAS_0003.do?pgmNo=ESM_MAS_0003";
		}
	}
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0002_1GS.do", masFormQueryString(formObj));
				var arrXml = sXml.split("|$$|");
				if (arrXml.length > 0)
					ComMasSetIBCombo(sheetObj, arrXml[0], "stnd_cost_tp_cd", false, 0);
				if (arrXml.length > 1)
					ComMasSetIBCombo(sheetObj, arrXml[1], "mgrp_cost_nm", false, 0);
				if (arrXml.length > 2)
					ComMasSetIBCombo(sheetObj, arrXml[2], "sgrp_cost_nm", false, 0);
				if (arrXml.length > 3)
					ComMasSetIBCombo(sheetObj, arrXml[3], "ra_mn_cost_nm", false, 0);
				if (arrXml.length > 4)
					ComMasSetIBCombo(sheetObj, arrXml[4], "ra_sgrp_cost_nm", false, 0);
			...
			case IBSEARCH:	//조회
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSearch4Post("ESM_MAS_0002GS.do", masFormQueryString(formObj, 'cond1|code'));
					ComOpenWait(false);
				}
				break;
			...
			case IBSAVE:		//저장
				...
				sheetObj.DoSave("ESM_MAS_0002GS.do", masFormQueryString(formObj, 'cond1|code'));
				ComOpenWait(false);
				}
				break;
			...
			case IBROWSEARCH: //행내용 업데이트
				...
				sheetObj.DoRowSearch("ESM_MAS_0002GS2.do", masFormQueryString(formObj2));
				ComOpenWait(false);
				break;
	...
	function doActionIBSheet2(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBROWSEARCH: //행내용 업데이트
				...
				var formObj2 = document.hiddenF;
				formObj2.f_cmd.value = MULTI02;
				sheetObj.DoRowSearch("ESM_MAS_0002GS3.do", masFormQueryString(formObj2));
				ComOpenWait(false);
				break;
		}
	}

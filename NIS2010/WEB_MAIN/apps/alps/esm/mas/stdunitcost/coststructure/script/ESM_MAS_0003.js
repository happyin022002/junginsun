...
/**
 * @extends 
 * @class ESM_MAS_0003 : ESM_MAS_0003 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0003() {
...
document.onclick = processButtonClick;
	...
	function initSheet(sheetObj,sheetNo, hdCode, hdText) {
		...
		switch(sheetNo) {
			case 1:
				...
				InitDataProperty(0,cnt++,	dtData,		45,	daCenter,	true,	"mas_cost_src_prt_cd",	false,	"",	dfNone,		0,	true,	true,	2,	false,	false);
				...
				InitDataProperty(0,cnt++,	dtData,		60,	daCenter,	true,	"mas_cost_src_cd",		true,	"",	dfNone,		0,	true,	true,	8);
				InitDataProperty(0,cnt++,	dtData,		150,daLeft,		true,	"mas_cost_src_cd_nm",	false,	"",	dfNone,		0,	true,	true);
				...
				InitDataProperty(0,cnt++,	dtCheckBox,	80,	daCenter,	true,	"bkg_rev_mcgo_flg"     ,	false,	"",	dfNone,	0,	true,	true,	-1,	false,	false);
				...
				InitDataValid(0, "mas_cost_src_prt_cd", vtEngUpOnly);
	...
	*/
	...
	function goURL( kind ){
		if ( kind == "1" )	{
			document.location.href	= "/hanjin/ESM_MAS_0002.do?pgmNo=ESM_MAS_0002";
		} else if ( kind == "2") {
			document.location.href = "/hanjin/ESM_MAS_0003.do";
		}
	}
    ...
    function f_sgrp_cost_cd_OnChange(obj, code){
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0003_1GS.do", FormQueryString(formObj));
	...
	function openHideCheckBox(){
		ComOpenWindow('ESM_MAS_0124.do?headerCode='+document.form.f_header_code.value, 'Hide Pop', 'width=300,height=430,menubar=0,status=0,scrollbars=0,resizable=0');
		//noRtnPopup('ESM_MAS_0124.do?headerCode='+document.form.header_code.value, 'width=300,height=430,menubar=0,status=0,scrollbars=0,resizable=0')
	}
	...
	function sheet1_OnChange(sheetObj, Row, Col, value){
		...
		if(sheetObj.ColSaveName(Col) == "sgrp_cost_cd_desc"){
			...
			var sXml = sheetObj.GetSearchXml("ESM_MAS_0003_1GS.do", param);
			var arrXml = sXml.split("|$$|");
			if (arrXml.length > 0)
				ComMasSetIBCombo(sheetObj, arrXml[0], "stnd_cost_nm", true,0,Row);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회				
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0003_1GS.do", masFormQueryString(formObj));
				...
				*/
				...
				if (arrXml.length > 0)
					ComMasSetIBCombo(sheetObj, arrXml[0], "sgrp_cost_cd_desc", false, 0);
				
				if (arrXml.length > 1)
					ComMasSetIBCombo(sheetObj, arrXml[1], "stnd_cost_nm", false, 0);			
			...
			case IBSEARCH:	//조회
				...
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSearch4Post("ESM_MAS_0003GS.do", masFormQueryString(formObj, 'f_header_code|f_header_text|code'));
					ComOpenWait(false);
				}				
			...
			case IBSAVE:		//저장
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSave("ESM_MAS_0003GS.do", masFormQueryString(formObj, 'code'));
					//sheetObj.DoAllSave("ESM_MAS_0003GS.do", FormQueryString(formObj));
					ComOpenWait(false);
				}
				break;

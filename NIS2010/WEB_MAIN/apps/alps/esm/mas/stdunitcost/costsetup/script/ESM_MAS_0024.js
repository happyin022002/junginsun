		...
		try {
			...
			switch (srcName) {					
				...
				case "btn_weekcopy":		//팝업창(Week Copy)		
			  	     var display = "0,1";
			  	       ComOpenPopup("ESM_MAS_9001.do?classId=ESM_MAS_0024&rlane_cd="+document.form.rlane_cd.value+"&prev_week="+prevWeek, 250, 200, "AverageUcCopy", display, true, false);
			  	    break;
		...
		switch (sAction) {
	    	...
	    	case IBCLEAR:          //조회
	    		...
	    		if (frmObj.cost_yrmon.value == "") {
					...
					var sXml = shtObj.GetSearchXml("ESM_MAS_0021GS.do", masFormQueryString(frmObj));
			...
			case IBSEARCH:    // 조회
				...
				shtObj.DoSearch("ESM_MAS_0024GS.do", FormQueryString(frmObj));
				shtObj.Redraw = true;
				ComOpenWait(false);
				break;
			...
			case IBSAVE:    // 저장     
					...
					for (i=0; i<shtObj.LastRow-14; i++) {
						if (shtObj.CellValue(i, "stnd_cost_nm") == "Vessel Charter Revenue") {
							...
							if (shtObj.CellValue(i, "vsl_cd").length != 4 || shtObj.CellValue(i, "vsl_cd") == " ") {
								ComShowCodeMessage("MAS10068", "Vessel");		// {?msg1} must be inputted
				...
				if (!ComShowCodeConfirm("MAS00006")) {		// Do you want to save?
					...
					shtObj.DoAllSave("ESM_MAS_0024GS.do", FormQueryString(frmObj));
					doActionIBSheet(shtObj, frmObj, IBSEARCH); 
					ComOpenWait(false);
     		...
     		case "vsl_cd":
     			var celltxt  = sheetObj.EditValue;
     			 ...
     			 if (colName == "vsl_cd" && celltxt.length == 4){
	     					...
	     					if (sheetObj.CellValue(i, "stnd_cost_cd") == "43101011") {		//vessel charter revenue만 체크함
	     						...
	     						for (j=stNo-1; j>0; j--) {												//다음줄부터 체크
	     							if (sheetObj.CellValue(j, "stnd_cost_cd") == "43101011") {						//그다음에 나오는 vessel charter revenue 값 찾음
	     								...
	     								if (curVsl == cmpVsl) {
	     		    						 ComShowCodeMessage("MAS10015", "Vessel");
	     		    						 for (k=j; k<j+14; k++) {
	 				...
	 				var sXml = sheetObj.GetSearchXml("ESM_MAS_0024GS.do", param);
	 				
	 				if (ComGetEtcData(sXml, "vsl_cnt") == 0) {
	 					ComShowCodeMessage("MAS10071", sheetObj.CellValue(Row, Col));
	 					if (formObj.rlane_cd.value == "CDMCO") {	//Vessel Charter화면
		 					for (i=0; i<14; i++) {

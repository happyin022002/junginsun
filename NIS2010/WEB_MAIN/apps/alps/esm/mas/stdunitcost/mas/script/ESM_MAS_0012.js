...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0012 : ESM_MAS_0012 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0012() {
        	...
        	try {
                ...
                switch(srcName) {
        			...
        			case "btn_Month_Copy":		//팝업창(Month Copy)
        	     	       var display = "0,1";
        	     	       ComOpenPopup("ESM_MAS_0173.do?classId=ESM_MAS_0012", 250, 200, "AverageUcCopy", display, true, false);
        	     	break;
                } // end switch
        	}catch(e) {
        		if( e == "[object Error]") {
        ...
        function doActionIBSheet(sheetObj,formObj,sAction) {        	
            ...
            switch(sAction) {
            	case IBCLEAR:          //조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_MAS_0012GS.do", FormQueryString(formObj));
               ...
               case IBSEARCH:      //조회
                    if(validateForm(sheetObj,formObj,sAction)) {                	
                        ...
                        sheetObj.DoSearch4Post("ESM_MAS_0012GS.do", masFormQueryString(formObj));
                        ComOpenWait(false);
                    }
                    break;
                case IBSAVE:       //저장
                  if(validateForm(sheetObj,formObj,sAction)) {
                      ...
                      sheetObj.DoAllSave("ESM_MAS_0012GS.do", masFormQueryString(formObj, 'f_cost_yrmon'));
                      ComOpenWait(false);
                  }
                  break;

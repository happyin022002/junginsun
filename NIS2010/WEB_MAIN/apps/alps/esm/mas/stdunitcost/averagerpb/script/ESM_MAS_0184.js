...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0184 : ESM_MAS_0184 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0184() {
    ...
    /* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
    	...
    	function doActionIBSheet(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
    			...
    			case IBSAVE:        //저장
    		        ...
    		        sheetObj.DoSearch4Post("ESM_MAS_0184GS.do", masFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
    	...
    	function doActionIBSheet2(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
            	case IBSAVE: // 저장
    				...
    				sheetObj.DoSearch4Post("ESM_MAS_0184GS.do", FormQueryString(formObj));
    				ComOpenWait(false);
    				break;
    		}
    	}
       ...
       /**
        	...
        	switch (sAction) {
            	case IBSAVE:
            		if(formObj.f_target_yrmon.value==""){
            			ComShowCodeMessage('MAS10009','Taget Month','YYYY-MM');
            			return false;
            		}
            		if(formObj.f_fm_yrwk.value=="" || formObj.f_to_yrwk.value==""){
            			ComShowCodeMessage('MAS10009','Period','YYYY-WK');
            			return false;
            		}
            		...
            		if(duration>5){
            			ComShowCodeMessage('MAS10007', '5')
            			return false;
            		}
            		formObj.f_cmd.value = SEARCH01;
            		sheetObj.DoSearch4Post("ESM_MAS_0184GS.do", masFormQueryString(formObj));
            		var TagetYrMonStatus = sheetObj.EtcData("TagetYrMonStatus");
            		if(TagetYrMonStatus=="Y"){
            			if(!ComShowConfirm('('+formObj.f_target_yrmon.value+')  '+ComGetMsg('MAS10069'))){
            				return false;
            			}
            		}else{
            			if(!ComShowConfirm(ComGetMsg('MAS10020'))){
            				return false;
            			}
            		}
            }
        ...
        function validateForm2(sheetObj,formObj,sAction){
        	switch (sAction) {
            	case IBSAVE:
            		...
            		var duration = yrdif+wkdif;
            		if(duration>30){
            			ComShowCodeMessage('MAS10007', '30')
            			return false;
            		}
            		break;
            }
        ...
        function monitoringBatchJob(){
        	...
        	var sXml = sheetObj.GetSearchXml("ESM_MAS_0184GS.do", FormQueryString(formObj));

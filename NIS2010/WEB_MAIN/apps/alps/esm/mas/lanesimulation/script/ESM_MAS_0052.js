...
* 2010.10.26 윤진영 [CHM-200901026] IMU노선으로 Simulation 진행시, Step 2 - TMNL Transit Time 메뉴에서 Creation시
*            위와같은 메시지 뜨고 진행이 안됨 처리
* 2010.02.23 윤진영 조회조건의 frequency 필수 항목으로 변경, duration1 editable false처리 (신준모씨 req)
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
        	case IBCLEAR:          //조회
				...
				if (arrXml.length > 6)
					ComMasSetIBCombo(sheetObj, arrXml[6], "skd_dir_cd", false, 0);
            ...
            case IBSEARCH:      //조회
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoSearch4Post("ESM_MAS_0052GS.do", masFormQueryString(formObj));
            ...
            case IBROWSEARCH:  //Yard code조회
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0052GS5.do",masFormQueryString(formObj)+"&row_no="+row_no+"&port_cd="+sheetObj.CellValue(row_no,"port_cd")+"&skd_dir_cd="+sheetObj.CellValue(row_no,"skd_dir_cd"),true);

				var arrXml = sXml.split("|$$|");
				if (arrXml.length > 0)
					ComMasSetIBCombo(sheetObj, arrXml[0], "port_yd", "" ,0, row_no);
            ...
            case IBCREATE:        // 인터페이스  : Port day를 받아 온다
               if(sheetObj.CellValue(sheetObj.SelectRow, "vol_cnt") > 0 ){
            	   ComShowMessage(ComGetMsg("MAS10045"));
                   return;
                }
            ...
            case IBSAVE:        //저장
                ...
                if(formObj.f_brth_itval_dys.value == '' || formObj.f_brth_itval_dys.value == null) {
              	  ComShowMessage(ComGetMsg("MAS10048"));
               	  //frequency is empty..
                  return;
                }
...
//                }
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoAllSave("ESM_MAS_0052GS.do", masFormQueryString(formObj));
                ComOpenWait(false);
                if(formObj.f_save_flg.value=="false") {
                  ComShowMessage(ComGetMsg("MAS10049"));
                } else {
                  ComShowMessage(ComGetMsg("MAS10050"));
                }
    ...
    function doActionIBSheet2(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            case IBSEARCH:      //조회
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoSearch4Post("ESM_MAS_0052GS2.do", masFormQueryString(formObj));
                ComOpenWait(false);
                break;
            ...
            case IBSAVE:        //저장
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoSave("ESM_MAS_0052GS2.do", masFormQueryString(formObj));
                ComOpenWait(false);
                break;
...
//        sect_no = sheetObj2.CellValue(sheetObj2.SelectRow, "sect_no");
        ...
        switch(sAction) {
            case IBSEARCH:      //조회
                ...
                var sXml = sheetObj.GetSearchXml("ESM_MAS_0052GS3.do", masFormQueryString(formObj)+"&sect_no="+p_sect_no);
...
//                loadCnt = sheetObj2.CellValue(sheetObj2.SelectRow, "lod_ttl_qty");
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoAllSave("ESM_MAS_0052GS3.do?totSum="+totSum+"&lod_ttl_qty="+v_lod_ttl_qty+"&sect_no="+p_sect_no, masFormQueryString(formObj));
                ComOpenWait(false);
...
//                }
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoRowSearch("ESM_MAS_0052GS3.do", masFormQueryString(formObj));
                ComOpenWait(false);
            ...
            case IBCOPYROW:        // 인터페이스  : Port정보를 넘겨준다.
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoRowSearch("ESM_MAS_0052GS.do", masFormQueryString(formObj));
                ComOpenWait(false);
                var Result = sheetObj.EtcData("result");
                sheetObj.RemoveEtcData();
                if (Result=="OK"){
...
//        sect_no = sheetObj2.CellValue(sheetObj2.SelectRow, "sect_no");
        ...
        switch(sAction) {
            case IBSEARCH:          //조회
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                var sXml = sheetObj.GetSearchXml("ESM_MAS_0052GS4.do", masFormQueryString(formObj)+"&sect_no="+p_sect_no);
...
//                    ComEtcDataToForm(formObj, sheetObj);             // ETC데이터를 form변수에 세팅한다.
            ...
            case IBSAVE:            //저장
				...
				sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
                sheetObj.DoSave("ESM_MAS_0052GS4.do", masFormQueryString(formObj)+"&sect_no="+p_sect_no);
                ComOpenWait(false);
                var Result = sheetObj.EtcData("result");
                sheetObj.RemoveEtcData();
                if (Result=="OK"){
    ...
    function doActionIBSheet5(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            case IBSEARCH:          //조회
                ...
                sheetObj.WaitImageVisible = false;
                ComOpenWait(true);
                sheetObj.DoSearch4Post("ESM_MAS_0052GS5.do", masFormQueryString(formObj));
                ComOpenWait(false);
                break;
            ...
            case IBSAVE:            //저장
                ...
                sheetObj.WaitImageVisible = false;
                ComOpenWait(true);
                sheetObj.DoSave("ESM_MAS_0052GS5.do", masFormQueryString(formObj));
                ComOpenWait(false);
                break;
    	 ...
    	 var formObj = document.form;
      	...
      	if (ComGetObjValue(obj) != "") {
         	var simNo = ComGetObjValue(formObj.f_sim);
 	 		...
 	 		var sXml = sheetObj.GetSearchXml("ESM_MAS_0052GS5.do", masFormQueryString(formObj));
 	 		var arrXml = sXml.split("|$$|");
    ...
    function chkMendatory(){
        ...
        for(i=4; i<sheetObj2.LastRow; i++){
             ...
             if(sheetObj2.CellValue(i,"skd_dir_cd")== 0 ) {
                ComShowMessage(ComGetMsg("MAS10002",register +" Skd Dir Cd"));
    		        return false;
             }
             if(sheetObj2.CellValue(i,"port_cd")== 0 ) {
                ComShowMessage(ComGetMsg("MAS10002",register +" Port Cd"));
    		        return false;
             }
             if(sheetObj2.CellValue(i, "port_yd")== 0 ) {
                ComShowMessage(ComGetMsg("MAS10002",register +" Port Yard"));
    		        return false;
             }
             if(sheetObj2.CellValue(i, "turn_port_flg")== 0 ) {
                ComShowMessage(ComGetMsg("MAS10002",register +" Turn Port Flag"));
    		        return false;
             }
        }
        return true;

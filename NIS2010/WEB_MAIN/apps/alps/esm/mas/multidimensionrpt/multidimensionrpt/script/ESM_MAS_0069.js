...
/**
 * @extends 
 * @class ESM_MAS_0069 : ESM_MAS_0069 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0069() {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:		//조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0069GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
				break;
		...
		switch(sAction) {
			case IBSEARCH:		//조회
				...
				if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
					...
					sheetObj.DoSearch4Post("ESM_MAS_0069GS2.do", masFormQueryString(formObj));
					ComOpenWait(false);
				} else {

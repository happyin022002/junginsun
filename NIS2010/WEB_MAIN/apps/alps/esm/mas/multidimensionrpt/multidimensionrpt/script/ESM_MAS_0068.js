...
/**
 * @extends 
 * @class ESM_MAS_0068 : ESM_MAS_0068 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0068() {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:		//조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0068GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	function doActionIBSheet2(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:		//조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0066GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	*/
	function validateForm(sheetObj,formObj,sAction){
		...
		if(ComIsNull(formObj.f_bkg_no)) {
			ComShowMessage(ComGetMsg('MAS10002', 'Booking Number'));

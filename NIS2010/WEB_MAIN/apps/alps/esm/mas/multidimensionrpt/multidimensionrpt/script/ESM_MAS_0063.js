...
/**
 * @extends 
 * @class ESM_MAS_0063 : ESM_MAS_0063 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0063() {
	...
	function openBKGDetail(){
		...
		if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
			if(sheetObjects[1].RowCount>0){//sheet2에 데이터가 있을때
				...
				ComOpenWindow('ESM_MAS_0065.do?' + masFormQueryString(document.form) + cond, '_BKG'
						, 'width=750,height=400,menubar=0,status=0,scrollbars=0,resizable=0');
				
				//noRtnPopup('ESM_MAS_0065.do?' + masFormQueryString(document.form) + cond ,
				//'width=750,height=400,menubar=0,status=0,scrollbars=0,resizable=0');
			} else {
	...
	* */
	...
	function chgOffice(obj){
         ...
         if(obj.Text != ""){
 			...
 			var sXml = sheetObj.GetSearchXml("ESM_MAS_0063GS.do", masFormQueryString(formObj));
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:	//조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0063GS.do", masFormQueryString(formObj)+"&f_sch_mode=1");
				ComOpenWait(false);
				break;
		...
		switch(sAction) {
			case IBSEARCH:	//조회
				...
				if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
					...
					sheetObj.DoSearch4Post("ESM_MAS_0063GS2.do", masFormQueryString(formObj) + "&f_sch_mode=2");
					ComOpenWait(false);
				} else {
					ComShowMessage(ComGetMsg('MAS10005', 'Sheet1'));
				}

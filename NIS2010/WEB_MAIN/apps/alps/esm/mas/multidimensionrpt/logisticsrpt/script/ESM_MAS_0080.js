...
* 2014.09.15 박은주 [CHM-201431910] [MAS] Logistics Exp by Lane 화면에 Trade Direction 칼럼추가 요청
* 
...
/**
* @extends 
* @class ESM_MAS_0080 : ESM_MAS_0080 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
*/
function ESM_MAS_0080() {
...
function f_trd_cd_OnChange(obj) {
	...
	if (obj.Text != "") {
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0080GS.do", masFormQueryString(formObj));
	...
	case 1:	 //sheet1 init
		with (sheetObj) {
			 ...
			 if (location.hostname != "") InitHostInfo(location.hostname, location.port, page_path);//Host정보 설정[필수][HostIp, Port, PagePath]
				...
				var HeadTitle  = "H|Subsum|S.Month|R.Month|Week|Trade|Lane|B/D|Load\n(TEU)|Void Vol\n(TEU)|Cost|Cost|Cost|Unit Cost (Cost per Load)|Unit Cost (Cost per Load)|Unit Cost (Cost per Load)";
	  ...
	  case 2:	 //sheet2 init
		 with (sheetObj) {
			 ...
			 var HeadTitle = "H|Trade|Lane|B/D|Load (TEU)|Cost Group|In/Out|Trade Dir.|KPI|Volume (BOX)|Cost|Unit Cost";
	...
	case IBSEARCH:	 //조회			
		if(validateForm(sheetObj,formObj,sAction)) {
			if ( sheetObj.id == "sheet1" ) {
					...
					sheetObj.DoSearch4Post("ESM_MAS_0080GS.do", masFormQueryString(formObj));
					ComOpenWait(false);
			   }
...
//					 }
					 ...
					 //20100415 이중환, FormQueryString -> masFormQueryString 변경
					 sheetObj.DoSearch4Post("ESM_MAS_0080GS2.do", masFormQueryString(formObj));
					} else {
						ComShowMessage(ComGetMsg('MAS10005'));
					}
				}
			}

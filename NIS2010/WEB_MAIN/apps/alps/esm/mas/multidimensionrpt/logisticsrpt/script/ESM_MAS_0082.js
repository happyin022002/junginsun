...
*Change history :
	...
	* 2017.05.31 김동호 [CSR #1049] 조회 시 MANDATORY 조건 완화(2017년 이후 자료는 RHQ All, 기간 무제한) 
=========================================================*/
/**
 * @fileoverview 
 * @author 한진해운
...
* @class ESM_MAS_0082 : ESM_MAS_0082 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
*/
function ESM_MAS_0082() {
	...
	*/
    function f_trd_cd_OnChange(obj) {
		...
		if (obj.Text != "") {
			...
			var sXml = sheetObj.GetSearchXml("ESM_MAS_0082GS.do", masFormQueryString(formObj));
			...
			case IBSEARCH:	//조회			    
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					if ( sheetObj.id == "sheet1" ) {
						...
						sheetObj.DoSearch4Post("ESM_MAS_0082GS.do", masFormQueryString(formObj));
						ComOpenWait(false);
						...
						//20100415 이중환, FormQueryString -> masFormQueryString 변경
						sheetObj.DoSearch4Post("ESM_MAS_0082GS2.do", masFormQueryString(formObj));
						ComOpenWait(false);
					}
				}
				break;
	...
	function chkValidSearch(){
		...
		with(formObj){
			...
			if(parseInt(f_year.value) < 2017) {
			    ...
			    ComShowMessage(ComGetMsg("MAS10038","2 Months. (Before 2017 year)"));
			    f_fm_mon.focus();
				return false;
    			}
    			if(vChkPrd == "W" && (ComParseInt(f_to_wk.value) - ComParseInt(f_fm_wk.value)) > 1){
    			    ComShowMessage(ComGetMsg("MAS10038","2 Weeks. (Before 2017 year)"));
    			    f_fm_wk.focus();
    				return false;
    			}

    			if(f_rhq_cd.Code == "All") {
    			    ComShowMessage("RHQ is mandatory condition. (Before 2017 year).");
    			    f_rhq_cd.focus();
    			    return false;
    			}
			}

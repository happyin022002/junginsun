...
* 2015.04.28 손진환 [CHM-201535625] Inquiry by Lane - 검색 조건 유효성 검사 추가 
*=========================================================*/
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class ESM_MAS_0062 : ESM_MAS_0062 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0062() {
                ...
                case 1:      //sheet1 init
                    with (sheetObj) {
                        ...
                        if(formObj.f_view_tpsz[0].checked) {
                            HeadTitle  = HeadTitle + "TP/SZ|LOAD|REV|CM COST|CM|CM2 COST|CM2|RPB(BOX)||CM CPB(BOX)|CMB(BOX)|OP COST|OP||OP CPB(BOX)|OPB(BOX)" ;
                        }else{
                            HeadTitle  = HeadTitle + "TP/SZ|LOAD|REV|CM COST|CM|CM2 COST|CM2|RPB(TEU)||CM CPB(TEU)|CMB(TEU)|OP COST|OP||OP CPB(TEU)|OPB(TEU)" ;
                        }
    	...
    	*/
	    function openRouteDetail() {
			...
			if (sheetObjects[0].RowCount > 0) {//sheet1에 데이터가 있을때
				...
				ComOpenWindow('ESM_MAS_0147.do?' + masFormQueryString(document.form) + cond, 'AA', 'width=1010,height=450,menubar=0,status=1,scrollbars=0,resizable=1');
			} else {
                    ...
                    if( f_bkg_disp.checked == true ){
                    	if( f_chkprd[0].checked == true ) {
                    		...
                    		if(ComParseInt(ComGetObjValue(f_to_wk))-ComParseInt(ComGetObjValue(f_fm_wk)) >= 4){
                                // [MAS10003] : Week 는(은) 4wk 만 처리할수 있습니다.
                                ComShowMessage(ComGetMsg("MAS10003", "Week", "4wk"));
                                ComSetFocus(f_to_wk);
                                return false;
                            }
                    	...
                    	if( f_chkprd[1].checked == true ) {
                    		//ComShowMessage( f_fm_mon.value + " / " + f_to_mon.value + " / " + ComGetObjValue(formObj.f_trd_cd) );
                     		if(ComParseInt(ComGetObjValue(f_to_mon))-ComParseInt(ComGetObjValue(f_fm_mon)) >= 1){
                                // [MAS10003] : Month 는(은) 1Month 만 처리할수 있습니다.
                                ComShowMessage(ComGetMsg("MAS10003", "Month", "1Month"));
        ...
        function changeType() {
	     ...
	     function f_key_acct_group_cd_OnChange(obj, code){
	         ...
	         if(obj.Text != ""){
	 			...
	 			var sXml = sheetObj.GetSearchXml("ESM_MAS_0062GS.do", masFormQueryString(formObj));
...
//                sheetObj.SumValue(0, "opb")    = eval("(" + sheetObj.SumValue(0, "cm")+ "-" + sheetObj.SumValue(0, "op_cost")+ ")/" + sheetObj.SumValue(0, "load")).toFixed(2);
        ...
        function sheet1_OnDblClick(sheetObj, row, col, value){
            ...
            if(formObj.f_bkg_disp.checked){
                ...
                ComOpenWindow('ESM_MAS_0061POP.do'+param, 'Inquiry By BKG', "width=1050,height=850,menubar=0,status=1,scrollbars=1,resizable=1");
            }
        }
        ...
        function doActionIBSheet(sheetObj, formObj, sAction, a, PageNo) {
            ...
            switch(sAction) {
                ...
                case IBSEARCH:      //조회
                    ...
                    //20100414 이중환, FormQueryString -> masFormQueryString 변경
                    sheetObj.DoSearch4Post("ESM_MAS_0062GS.do", masFormQueryString(formObj));
                    ComOpenWait(false);
                    break;
        ...
        function validateForm(sheetObj,formObj,sAction){
            ...
            with(formObj){
                if (f_year.value == "") {
                    ComAlertFocus(f_year, ComGetMsg("MAS10002", "Year", ""));
                    return false;
                }

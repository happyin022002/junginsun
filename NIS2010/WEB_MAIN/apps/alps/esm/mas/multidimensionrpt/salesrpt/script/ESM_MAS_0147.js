...
* =========================================================
* History
* 2008.05.30 PEJ N200805260011 MAS Report REV 산출 수정 요청
* 2009.05.08 박상희 N200904170011 Report Item 추가(2) : CM COST, OP COST 추가 
* 2009.05.11 박상희 N200904170011 - OPB total 관련 searchend 수정 
* 2009.05.14 박상희 N200904170011 - Header명 변경  2009.05.14 
*                                - bound disp. & wk disp. 체크시 BKG Detail 팝업에도 반영 
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0147 : ESM_MAS_0147 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0147() {
    		...
    		switch(sheetNo) {
    		    case 1:	//sheet1 init
    				with (sheetObj) {
    					...
    					InitColumnInfo(22, 2, 0, true); //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
    					InitHeadMode(true, false, false, true, false,false);// 해더에서 처리할 수 있는 각종 기능을 설정한다

    					var HeadTitle = "ORG|DEST|POR|REV POL|REV POD|DEL|TP/SZ|Load|REV|CM COST|CM|CM2 COST|CM2" ;

    					if(formObj.f_view_tpsz.value == "box") {
                            HeadTitle  = HeadTitle + "|RPB(BOX)||CM CPB(BOX)|CMB(BOX)|OP COST|OP||OP CPB(BOX)|OPB(BOX)" ;
                        }else{
                            HeadTitle  = HeadTitle + "|RPB(TEU)||CM CPB(TEU)|CMB(TEU)|OP COST|OP||OP CPB(TEU)|OPB(TEU)" ;
                        }					
    					...
    					InitDataProperty(0, cnt++ , dtData,     40,         daCenter, true, "org",         false, "", dfNone,      0, true, true);
                        InitDataProperty(0, cnt++ , dtData,     40,         daCenter, true, "dest",        false, "", dfNone,      0, true, true);
                       
                        InitDataProperty(0, cnt++ , dtData,     60,         daCenter, true, "bkg_por_cd",         false, "", dfNone,      0, true, true);
                        InitDataProperty(0, cnt++ , dtData,     60,         daCenter, true, "rev_pol_cd",         false, "", dfNone,      0, true, true);
                        InitDataProperty(0, cnt++ , dtData,     60,         daCenter, true, "rev_pod_cd",         false, "", dfNone,      0, true, true);
                        InitDataProperty(0, cnt++ , dtData,     60,         daCenter, true, "bkg_del_cd",         false, "", dfNone,      0, true, true);
                        ...
                        InitDataProperty(0, cnt++ , dtAutoSum,  70,         daRight,  true,  "rev",        false, "", dfFloatOrg,  2, false,  false);
...
//    					RangeBackColor(0, 13, 0, 16) =  RgbColor(170,210,130)  // ENIS
//    					RangeBackColor(1, 13, 1, 20) = RgbColor(222, 251, 248);  // ENIS
          ...
          if(!validateForm(sheetObjects[0],document.form,IBSEARCH)) return false;
              if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때	
                //    formObj.f_cmd.value = SEARCHLIST01;       
                 ...
                 var formObj = document.form;	
                 formObj.f_cmd.value = "";
                 cond = cond + "&rev_pol_cd=" + obj1.CellValue(row, "rev_pol_cd")
    					+ "&rev_pod_cd=" + obj1.CellValue(row, "rev_pod_cd")
    					+ "&bkg_por_cd=" + obj1.CellValue(row, "bkg_por_cd")
    					+ "&bkg_del_cd=" + obj1.CellValue(row, "bkg_del_cd")					
    					+ "&s_wk_cd=" + formObj.f_wk_cd.value					
    					+ "&s_wk_sts=" + formObj.f_wk_sts.value	
    					+ "&f_bkg_sts=" + formObj.f_bkg_sts.value
    					+ "&s_trd_cd=" + formObj.f_trd_cd.value    					
			            + "&s_rlane_cd=" + formObj.f_rlane_cd.value 
			            + "&s_dir_cd=" + formObj.f_dir_cd.value 			        
  //  			 openWindow('ESM_MAS_0148.do?'+ FormQueryString(document.form)+cond, 
  //  			    '_BKG', 'width=950,height=450,menubar=0,status=0,scrollbars=0,resizable=0')	
    		     //20100414 이중환, FormQueryString -> masFormQueryString 변경    					
    			 ComOpenWindow2('ESM_MAS_0148.do?'+ masFormQueryString(document.form)+cond, 
     			    '_BKG', 'width=1010,height=450,menubar=0,status=1,scrollbars=0,resizable=1')	
        ...
        function changeViewColumn(){               
            var sheetObj = sheetObjects[0];              
                if (document.form.f_pro_lvl.value == "O" && document.form.f_pro_vw.value == "P"){
...
//                sheetObj.SumValue(0, "opb")    = eval("(" + sheetObj.SumValue(0, "cm")+ "-" + sheetObj.SumValue(0, "op_cost")+ ")/" + sheetObj.SumValue(0, "load")).toFixed(2);
    	...
    	*/
    	function sheet1_OnDblClick(sheetObj , row, col){
    		if(validateForm(sheetObj,document.form,IBSEARCH)) openBKGDetail();
    	}
    	...
    	function doActionIBSheet(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
    			case IBSEARCH:		//조회
    				...
    				formObj.f_cmd.value = SEARCHLIST01;
    	            //20100414 이중환, FormQueryString -> masFormQueryString 변경
    				sheetObj.DoSearch4Post("ESM_MAS_0147GS.do", masFormQueryString(formObj));
    				ComOpenWait(false);
    				break;

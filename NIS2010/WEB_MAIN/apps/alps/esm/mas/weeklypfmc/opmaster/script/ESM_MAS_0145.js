...
/**
 * @extends 
 * @class ESM_MAS_0145 : ESM_MAS_0145 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0145() {
   ...
   /**
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                with (sheetObj) {
					...
					var HeadTitle  = "Del.|STS|SEQ|Trade|Sub Trade|S.Lane|R.Lane|Bound|IOC|Lane T/P|Lane T/P2|Step Up/Down|VVD|From|To|Update User ID|Update Time";
					...
					InitDataProperty(0, cnt++ , dtCombo,	100,	daCenter,	true,		"op_lane_tp_cd",	false,		"",		dfNone,		0,		true,		true);
    ...
    function getEdtDate(result) {
    	...
    	if(result == null || result == "" || result == "null"){
    		ComShowMessage(ComGetMsg('MAS10027',selValue));
    ...
    function sheet1_OnChange(sheetObj,Row,Col,Value) {
    	...
    	with(sheetObj) {
    		...
    		if (ColSaveName(Col) == "vvd_cd") {
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0145GS2.do", masFormQueryString(formObj));
...
//				if (0 < arrXml.length) {
				if (arrXml.length > 0) {
					ComMasSetIBCombo(sheetObj, arrXml[0], "vsl_lane_tp_cd", true, 0);
				}
				if (arrXml.length > 1) {
					// 2011.09.15 Lane T/P2 콤보코드 변경
					var arrData = ComXml2ComboString(arrXml[1], "code", "name");
			...
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0145GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
			...
			case IBSAVE:        //저장
				...
				sheetObj.DoSave("ESM_MAS_0145GS.do", masFormQueryString(formObj,'f_cmd',true), -1, false);
				ComOpenWait(false);
				doActionIBSheet(sheetObjects[0],document.form,IBSEARCH);
				break;
...
//				sheetObj.DataCopy(-1); // 마지막행에 행삽입
   ...
   /**
    ...
    function checkDate(sheetObj){
    	for(var k=1; k<=sheetObj.LastRow-1; k++){			
					var from_value = sheetObj.CellValue(k,'lane_aply_fm_dt');				
					...
					var to_value = sheetObj.CellValue(k,'lane_aply_to_dt');				
					...
					var from_value_of_the_next_seq = sheetObj.CellValue(k+1,'lane_aply_fm_dt');
					...
					var to_value_of_the_next_seq = sheetObj.CellValue(k+1,'lane_aply_to_dt');
					...
					if( year_of_from_value > year_of_to_value ){
						sheetObj.CellBackColor(k,'lane_aply_to_dt') = sheetObj.RgbColor(192, 0, 0);
						sheetObj.CellBackColor(k,'lane_aply_fm_dt') = sheetObj.RgbColor(192, 0, 0);
					...
					if(year_of_to_value > year_of_from_value_of_the_next_seq){
						sheetObj.CellBackColor(k,'lane_aply_to_dt') = sheetObj.RgbColor(192, 0, 0);
						sheetObj.CellBackColor(k+1,'lane_aply_fm_dt') = sheetObj.RgbColor(192, 0, 0);
					...
					if(year_of_to_value > year_of_to_value_of_the_next_seq){
						sheetObj.CellBackColor(k,'lane_aply_to_dt') = sheetObj.RgbColor(192, 0, 0);
						sheetObj.CellBackColor(k+1,'lane_aply_to_dt') = sheetObj.RgbColor(192, 0, 0);

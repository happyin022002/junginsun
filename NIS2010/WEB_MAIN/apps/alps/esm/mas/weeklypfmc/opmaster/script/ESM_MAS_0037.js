...
/**
 * @extends
 * @class ESM_MAS_0037 : ESM_MAS_0037 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0037() {
        ...
        try {
            switch(srcName) {
                case "btn_Retrieve":
                    ...
                    sheetObject.ColHidden("seq") = true;
                    break;
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                with (sheetObj) {
                    ...
                    InitColumnInfo(26 + aryCD.length, 4, 0, true);              //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                    ...
                    InitDataProperty(0, cnt++, dtData,       60, daCenter, true, "bsa_vsl_flg",     false, "", dfEngUpKey,  0, false, false, 1);
                    ...
                    InitDataProperty(0,	cnt++,	dtSeq,		20,	    daCenter,   true,	    "seq",			    false,	    "",		dfNone,		0,		false,		false);
                    InitDataProperty(0,	cnt++,	dtHidden,	30,	    daCenter,   true,	    "auto_flg",			false,	    "",		dfNone,		0,		false,		false);
...
//			var sXml = sheetObj.GetSearchXml("ESM_MAS_0037GS2.do", masFormQueryString(formObj));
    ...
    function sheet1_OnChange(sheetObj, Row, Col, Value) {
        ...
        if(sheetObj.ColSaveName(Col) == "vop_cd"){
        	if(Value == "") {
        		...
        		var sXml = sheetObj.GetSearchXml("ESM_MAS_0037GS2.do", param);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            ...
            case IBSEARCH:      //조회
                ...
                var xml = sheetObj.GetSearchXml("ESM_MAS_0037GS.do", masFormQueryString(formObj));
            ...
            case IBSAVE:        //저장
				...
				var splitSaveString = saveString.split("ibflag");
				var rowNumberForRollbackAutoFlag = new Array();

				for(i=1; i<splitSaveString.length; i++){ //splitSaveString[0]은 버림(null이 들어가 있음)
					
					if((splitSaveString[i].indexOf('=U&')) != -1){
						
						var extractedRawData = splitSaveString[i].split("&seq="); // sub_trade별 데이터와 rownum을 가져옴
						var stringOfExtractedSequence = extractedRawData[1].replace(/\D/g,''); // seq뒤의 &문자 제거
						var extractedSequence = Number(stringOfExtractedSequence) + 1;//헤더가 2줄이므로, 첫번째 데이터의 rownum은 2부터 시작함!!!!!!!!!!
						
						var extractedData1 = extractedRawData[0];
						var extractedData2 = extractedData1.split("&crr_cd=");
						var extractedData3 = extractedData2[1].split("&vsl_prc=");
						var extractedData4 = extractedData3[0].split("&"); // 0번배열은 버리고, 1번 배열부터 subtrade=capa 이런식으로 데이터가 들어가 있음
						var capacityForComparison = "";
						var originalCapacity = "";
						var subTradeCode = new Array();
							
						for(i=1; i<extractedData4.length; i++){ // extractedData4[0]은 버려야 함
							capacityForComparison = capacityForComparison + extractedData4[i].replace(/\D/g,'');
							subTradeCode[i-1] = extractedData4[i].replace("=", "").replace(".", "").replace(/0/gi, "").replace(/1/gi, "").replace(/2/gi, "").replace(/3/gi, "").replace(/4/gi, "").replace(/5/gi, "").replace(/6/gi, "").replace(/7/gi, "").replace(/8/gi, "").replace(/9/gi, ""); // =, ., 숫자를 제거하여 sub_trade 이름만 빼냄
							var stringOfOriginalCapacity = sheetObjects[0].CellSearchValue(extractedSequence, subTradeCode[i-1]).toString();
							originalCapacity = originalCapacity + stringOfOriginalCapacity.replace(".", "");
						}
						
						var originalValueOfOperator = sheetObjects[0].CellSearchValue(extractedSequence, "vop_cd");
						var originalValueOfOwner = sheetObjects[0].CellSearchValue(extractedSequence, "vsl_oshp_cd");
						var originalValueOfVesselClass = sheetObjects[0].CellSearchValue(extractedSequence, "vsl_clss_capa");
						var originalValueOfPortClass = sheetObjects[0].CellSearchValue(extractedSequence, "port_clss_capa");
						var originalValueOfDesignedCapacity = sheetObjects[0].CellSearchValue(extractedSequence, "vsl_dznd_capa");
						var originalValueOfStandardLoadableCapacity = sheetObjects[0].CellSearchValue(extractedSequence, "stnd_ldb_capa");
						
						var vesselCode = sheetObjects[0].CellSearchValue(extractedSequence, "vsl_cd");
						
						if(originalValueOfOperator != sheetObjects[0].CellValue(extractedSequence, "vop_cd") 
								|| originalValueOfOwner != sheetObjects[0].CellValue(extractedSequence, "vsl_oshp_cd")
								|| originalValueOfVesselClass != sheetObjects[0].CellValue(extractedSequence, "vsl_clss_capa")
								|| originalValueOfPortClass != sheetObjects[0].CellValue(extractedSequence, "port_clss_capa")
								|| originalValueOfDesignedCapacity != sheetObjects[0].CellValue(extractedSequence, "vsl_dznd_capa")
								|| originalValueOfStandardLoadableCapacity != sheetObjects[0].CellValue(extractedSequence, "stnd_ldb_capa")
								|| capacityForComparison != originalCapacity
								){							
							sheetObjects[0].CellValue(extractedSequence, "auto_flg") = "Y";
//							sheetObjects[0].CellValue(extractedSequence, "ibflag") = "I";
							
							rowNumberForRollbackAutoFlag[i-1] = extractedSequence;
						}
					}
				}
				///////////////////////////////////////////////////////////////////////////////
                ...
                sheetObj.DoSave("ESM_MAS_0037GS.do", masFormQueryString(formObj), -1, false);
                
                checkFromToDate(vesselCode);
                
//                for(i=0; i<rowNumberForRollbackAutoFlag.length; i++){
//					sheetObjects[0].CellValue(rowNumberForRollbackAutoFlag[i], "auto_flg") = "";
//				}
   ...
   /**
    ...
    function checkFromToDate(vesselCode){
		var formObj = document.form;
		
		formObj.f_cmd.value = SEARCHLIST04;
		
		var searchParams = "f_cmd=" + SEARCHLIST04 
							+ "&date_check_flag=Y"
							+ "&f_vsl_cd=" + vesselCode
							;
		
		var sXml = sheetObjects[0].GetSearchXml("ESM_MAS_0037GS.do", searchParams);
		
//		if(sXml.indexOf("TOTAL='0'") == -1)
		if(sXml.replace(/\D/g,'') != '0'){
			ComShowMessage(ComGetMsg("MAS10076"));
			var param = "?";
	         param = param + "f_vsl_cd=" + vesselCode;
	         ComOpenWindow2('ESM_MAS_0146.do'+param,'', 'width=1100,height=550,menubar=no,status=no,scrollbars=no,resizable=yes');
		}
	}
...
* 2010.05.17 아키위배사항 formcommand에서 command 01~40 사용금지 적용
* 2010.06.17 Lane History PopUP창 가로 사이즈 조정 (700->900)
* 2010.09.01 MAS Architecture 위배사항 수정
* 2011.09.15 AES Trade VSL Pool노선의 OP1 및 OP4 산출 로직 변경
* 2012.06.08 삭제된 데이터를 원복할 수 있도록 기능 변경
* 2012.09.17 실시간 영업현황 관련 UI- Create Lane Table 기능 추가
* 2013.10.17 [MAS] Trade Dir. 당팀 직접 수정 가능하게 변경
* 2015.08.31 Split15-[그룹사 표준 코드 시행] SML 프로그램 대응 및 데이타 마이그레이션 작업 요청
* 2016.03.03 Create Lane Table, Create Vessel Table history 자동 관리
*/
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
/**
 * @extends 
 * @class ESM_MAS_0036 : ESM_MAS_0036 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0036() {
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                with (sheetObj) {
					...
					InitColumnInfo(28, 3, 0, true);															//컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
					...
					if( document.form.f_usr_ofc_cd.value=="SELCSG" || document.form.f_usr_ofc_cd.value=="SELAPM"){
						AuthControl = true;
					}
					...
					InitDataProperty(0,	cnt++,	dtSeq,		1,	    daCenter,   true,	    "seq",			    false,	    "",		dfNone,		0,		false,		false);
...
//        }
    ...
    function sheet1_OnChange(sheetObj, row, col, value){
     ...
     function sheet1_OnDblClick(sheetObj, row, col, value){
         ...
         ComOpenWindow2('ESM_MAS_0145.do'+param,'', 'width=900,height=400,menubar=0,status=0,scrollbars=0,resizable=0');
    }
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0036GS3.do", masFormQueryString(formObj));
				...
				if (arrXml.length > 2)
					ComMasSetIBCombo(sheetObj, arrXml[2], "trd_cd", true, 0);
				if (arrXml.length > 3)
					ComMasSetIBCombo(sheetObj, arrXml[3], "sub_trd_cd", true, 0);
				if (arrXml.length > 4)
					ComMasSetIBCombo(sheetObj, arrXml[4], "vsl_lane_tp_cd", true, 0);
				if (arrXml.length > 5)
					ComMasSetIBCombo(sheetObj, arrXml[5], "dir_cd", true, 0);
				if (arrXml.length > 6)
					ComMasSetIBCombo(sheetObj, arrXml[6], "ioc_cd", true, 0);
				if (arrXml.length > 7) {
				...
				if (arrXml.length > 8)
					ComMasSetIBCombo(sheetObj, arrXml[8], "hul_bnd_cd", true, 0);
				if (arrXml.length > 9)
					ComMasSetIBCombo(sheetObj, arrXml[9], "ias_rgn_cd", true, 0);
			...
			case IBSEARCH:      //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0036GS.do", masFormQueryString(formObj));
				sheetObj.LoadSearchXml(sXml);
				
				//sheetObj.DoSearch4Post("ESM_MAS_0036GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
				break;
			...
			case IBSAVE:        //저장
				...
				if(arRow.length > 1){
    				for(j=0; j<arRow.length ; j++){
    				    if(arRow[j] != ""){
            				...
            				if(cnt>1){
            				    // [MAS10026] : T/P, Eur., T/A, Intra Asia, Trunk IPC 중 하나만 입력하세요.
            				    ComShowMessage(ComGetMsg("MAS10026", "T/P, Eur., T/A, Intra Asia, Trunk IPC "));
            				    sheetObj.SelectCell(arRow[j], "trns_pcf_flg");
            				    return false;
            				}
            				cnt = 0;
	            			if(sheetObj.CellValue(arRow[j], "trd_cd") == "IAS" && sheetObj.CellValue(arRow[j], "ias_rgn_cd") == ""){
	            				ComShowMessage(ComGetMsg("MAS10068", "IAS Region"));
	            				return false;
	            			}
    				    }
    				}
				...
				for(i=1; i<splitSaveString.length; i++){ //splitSaveString[0]은 버림(null이 들어가 있음)
					if((splitSaveString[i].indexOf('=U&')) != -1){
						var extractedRawSequence = splitSaveString[i].split("seq="); // rownum을 가져옴
						var stringOfExtractedSequence = extractedRawSequence[1].replace(/\D/g,''); // seq뒤의 &문자 제거
						var extractedSequence = Number(stringOfExtractedSequence) //for문 세팅에서 i를 1부터 시작하도록 했으므로, 여기서는 i에서 1을 제거하여 0번째 배열부터 값이 들어가게 함
						
						var originalValueOfLaneType1 = sheetObjects[0].CellSearchValue(extractedSequence, "vsl_lane_tp_cd");
						var originalValueOfLaneType2 = sheetObjects[0].CellSearchValue(extractedSequence, "op_lane_tp_cd");
						var originalValueOfPortStepUpDown = sheetObjects[0].CellSearchValue(extractedSequence, "stup_flg");
						
						var tradeCode = sheetObjects[0].CellSearchValue(extractedSequence, "trd_cd");
						var rlaneCode = sheetObjects[0].CellSearchValue(extractedSequence, "rlane_cd");
						var dirCode = sheetObjects[0].CellSearchValue(extractedSequence, "dir_cd");
						var iocCode = sheetObjects[0].CellSearchValue(extractedSequence, "ioc_cd");
						
						if(originalValueOfLaneType1 != sheetObjects[0].CellValue(extractedSequence, "vsl_lane_tp_cd")
								|| originalValueOfLaneType2 != sheetObjects[0].CellValue(extractedSequence, "op_lane_tp_cd")
								|| originalValueOfPortStepUpDown != sheetObjects[0].CellValue(extractedSequence, "stup_flg")){							
				...
				sheetObj.DoSave("ESM_MAS_0036GS.do", masFormQueryString(formObj,'f_cmd',true), -1, false);
		
				checkFromToDate(tradeCode, rlaneCode, dirCode, iocCode);
...
//				}
			...
			case IBROWSEARCH:
			    ...
			    sheetObj.DoRowSearch("ESM_MAS_0036GS2.do?sRow=" +sheetObj.SelectRow+ "&trade=" +sheetObj.CellValue(sheetObj.SelectRow, "trd_cd") , masFormQueryString(formObj,'f_cmd',true));
			    break;
    ...
    /**
     * LANE HISTORY[ESM_MAS_145] 팝업 화면이 닫히면서 콜하는 메소드
     * 
     */
    function retrieve(){
	...
	*/
	...
	function checkFromToDate(tradeCode, rlaneCode, dirCode, iocCode){
		var formObj = document.form;
		
		formObj.f_cmd.value = SEARCHLIST02;
		
		var searchParams = "f_cmd=" + SEARCHLIST02 
							+ "&date_check_flag=Y"
							+ "&f_trd_cd=" + tradeCode
							+ "&f_rlane_cd=" + rlaneCode
							+ "&f_dir_cd=" + dirCode
							+ "&f_ioc_cd=" + iocCode
							;
		
		var sXml = sheetObjects[0].GetSearchXml("ESM_MAS_0036GS.do", searchParams);
...
//		if(sXml.indexOf("TOTAL='0'") == -1)
		if(sXml.replace(/\D/g,'') != '0'){
			...
			var param = "?";
	         param = param + "f_trd_cd=" + tradeCode;
	         param = param + "&f_rlane_cd=" + rlaneCode;
	         param = param + "&f_dir_cd=" + dirCode;
	         param = param + "&f_ioc_cd=" + iocCode;
	         ComOpenWindow2('ESM_MAS_0145.do'+param,'', 'width=900,height=400,menubar=0,status=0,scrollbars=0,resizable=0');
		}
	}

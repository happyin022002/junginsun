...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0179 : ESM_MAS_0179 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0179() {
...
*/
 ...
 function doActionIBSheet(sheetObj,formObj,sAction) {
     ...
     case IBCLEAR:          //조회
 		...
 		var sXml = sheetObj.GetSearchXml("ESM_MAS_0179GS.do", masFormQueryString(formObj));
 		var arrXml = sXml.split("|$$|");
 		var prevWeek = "";
 		if (0 < arrXml.length) {
 	...
 	case IBSEARCH:      //조회
 		...
 		if(formObj.f_to_wk.value != "" && formObj.f_to_wk.value.length != 2) formObj.f_to_wk.value = fillZero(formObj.f_to_wk.value, 2, '0','left');
 		  ...
 		  sheetObj.DoSearch4Post("ESM_MAS_0179GS2.do", masFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
 		  ComOpenWait(false);
 	      break;
 	...
 	case IBCREATE:      //생성
 	    ...
 	    if (ComShowConfirm(ComGetMsg('MAS10020')) == true) {
 	       ...
 	       sheetObj.DoSearch4Post("ESM_MAS_0179GS.do", masFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
 	       ComOpenWait(false);

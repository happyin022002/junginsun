...
/**
 * @extends 
 * @class ESM_MAS_0108 : ESM_MAS_0108 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0108() {
...
function initSheet(sheetObj,sheetNo) {
  switch(sheetNo) {
      case 1:      //sheet1 init
        var com_head1 = "YYYY-MM|Week|Trade|Sub\nTrade|Lane|IOC|VVD|Trade Dir.|Charter-out\n(a)";
        var com_head2 = "|Total Network Cost\n(a+b)";
        var head1 = "|Cost allocation process for SML Sales"
                  + "|Cost allocation process for SML Sales"
                  + "|Cost allocation process for SML Sales"
                  + "|Cost allocation process for SML Sales"
                  + "|Cost allocation process for SML Sales";
        var head2 = "|SML Sales(Initial)|1st TS Alloc.|2nd TS Alloc.|Internal Pricing|SML Sales final(b)";
...
function f_seltrade_OnChange(obj,value,text) {
 	...
 	if ("All"!=value) {
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0108GS.do", masFormQueryString(formObj));
...
function setSpcChtrRevMssCnt() {
    ...
    var sXml = sheetObj.GetSearchXml("ESM_MAS_0154GS.do", masFormQueryString(formObj));
    var arrXml = sXml.split("|$$|");
	if (0 < arrXml.length) {
		if (0 < Number(ComGetEtcData(arrXml[0],"rowCount"))) {
	        ...
	        ComOpenWindow2("ESM_MAS_0154.do" + strUrl,'', "width=800,height=300,menubar=0,status=0,scrollbars=0,resizable=1");
		} else {
	    	if (ComShowConfirm(ComGetMsg("MAS10033"))) {
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	...
	case IBCLEAR:          //조회
  	...
  	case IBSEARCH:      //조회
          ...
          sheetObj.DoSearch4Post("ESM_MAS_0108GS.do", masFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
          ComOpenWait(false);
          break;
//
//      case IBCREATE:      //Apply to P/L
//          if(!validateForm(sheetObj,formObj,sAction)) return false;
//          //if (ComShowConfirm(ComGetMsg('MAS10033')) == true) {
//          // 업무처리중 버튼사용 금지 처리
//          sheetObj.WaitImageVisible = false;
//          ComOpenWait(true);
...
//
//          if (err_cd == "00000") {
//              ComShowMessage(ComGetMsg('MAS10018','Apply to P/L Chart'));
//          }
//          ComOpenWait(false);
//           //}
//          break;
//          
      ...
      case IBCREATE:      //Apply to P/L
          ...
          ComOpenWait(true);
          formObj.f_cmd.value = MULTI01;
          var sXml = sheetObj.GetSearchXml("ESM_MAS_0108GS.do", masFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
...
function getBackEndJobSearch() {
	...
	if (err_cd == "00000") {
		ComShowCodeMessage('MAS10018','Apply to P/L Chart');
	} else{

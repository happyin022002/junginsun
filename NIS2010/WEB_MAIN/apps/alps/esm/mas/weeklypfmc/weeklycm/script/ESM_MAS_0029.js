...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	/**
	 * @extends
	 * @class ESM_MAS_0029 : ESM_MAS_0029 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESM_MAS_0029() {
	...
	*/
	...
	function setIBMultiCombo(sheetObj, sXml, objName){
		...
		var arrData = ComMasXml2SheetMultiComboString(sXml, "code", "code");
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				*/
				...
				if (arrXml.length > 5)
					ComMasSetIBCombo(sheetObj, arrXml[5], "trd_cd", true, 0);
				if (arrXml.length > 6)
					ComMasSetIBCombo(sheetObj, arrXml[6], "sub_trd_cd", true, 0);
				if (arrXml.length > 7)
					ComMasSetIBCombo(sheetObj, arrXml[7], "slan_cd", true, 0);
				if (arrXml.length > 8)
					ComMasSetIBCombo(sheetObj, arrXml[8], "rlane_cd", true, 0);
				if (arrXml.length > 9)
					ComMasSetIBCombo(sheetObj, arrXml[9], "vsl_lane_tp_cd", true, 0);
				if (arrXml.length > 10)
					ComMasSetIBCombo(sheetObj, arrXml[10], "dir_cd", true, 0);
				if (arrXml.length > 11)
					ComMasSetIBCombo(sheetObj, arrXml[11], "ioc_cd", true, 0);
				if (arrXml.length > 12)
			...
			case IBSEARCH:      //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0029GS.do", masFormQueryString(formObj));

				var arrXml = sXml.split("|$$|");
				if (arrXml.length > 0)
					ComMasSetIBCombo(sheetObj, arrXml[0], "sub_trd_cd", true, 0);
				if (arrXml.length > 1)
					ComMasSetIBCombo(sheetObj, arrXml[1], "slan_cd", true, 0);
				if (arrXml.length > 2)
					ComMasSetIBCombo(sheetObj, arrXml[2], "rlane_cd", true, 0);
				/*--------------------------------------------*/

				formObj.f_cmd.value = SEARCHLIST01;
				var sParam = masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date');
				sheetObj.DoSearch4Post("ESM_MAS_0029GS.do", sParam);
				/*--------------------------------------------*/
				ComOpenWait(false);
				break;
			...
			case IBSAVE:        //저장
				...
				sheetObj.DoSave("ESM_MAS_0029GS.do", masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date'), -1, false);				
				...
				if (sParamSheet != "") {
 				...
 				var sXml = sheetObj.GetSaveXml("ESM_MAS_0029GS.do", sParam);
				//////////////////////////////			
				ComOpenWait(false);
			...
			case IBBATCH:      //수동 배치
				...
				if (!ComShowCodeConfirm("MAS10025")) {
					return false;
				}
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0029GS.do", masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date'));
...
//				
//				popup = "";
//				popup = window.showModalDialog("ESM_MAS_0113.do", window, "dialogWidth:300px;dialogHeight:140px;scroll:no");
//				formObj.f_type_cd.value = popup;
//				/////////////////////////////////////////////
//				
//				formObj.f_cmd.value = MULTI02;
//				// [MAS10020] : 정보를 생성하시겠습니까?
//				if (ComShowConfirm(ComGetMsg("MAS10020"))==false) return false;
//				ComOpenWait(true);
//				formObj.button_key.value = sAction;
//				var sXml = sheetObj.GetSearchXml("ESM_MAS_0029GS.do", masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date'));
//				var backendJobKey = ComGetEtcData(sXml, "BackEndJobKey");
...
//				break;
			...
			case IBCREATE:
				...
				if(formObj.f_type_cd.value != "MAS"){
				...
				switch(BatchStatus){
					case "4": // 작업 submit	
						...
						break;
					case "P"://해당 작업이 진행 중 
						ComShowCodeMessage("MAS00003", "Target VVD Creation");
						ComOpenWait(false);  
						break;
					default: break;							
				}   
			...
			case IBSEARCH_ASYNC01:          // BSA Flag 가 선택 된 항차에 대해서 BSA 0 으로 만들어 준다.
				...
				// [MAS10032] : 선택된 항차의 BSA 값을 0으로 만드시겠습니까?
				if (ComShowConfirm(ComGetMsg("MAS10036"))==false) return false;
				ComOpenWait(true);
				formObj.f_cmd.value = MULTI05;
				sheetObj.DoSearch4Post("ESM_MAS_0029GS.do", masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date'));
				ComOpenWait(false);
				...
				if (err_cd == "00000") {
					ComShowMessage(ComGetMsg('MAS10018', 'Apply BSA 0'));
				}
			...
			case IBSEARCH_ASYNC02:
				...
				ComOpenPopup("ESM_MAS_0112.do?"+getMthdArg, 800, 500, "Month VVD I/F", display, false, false);
				break;
				 */
		}
	}
	...
	function sheet1_OnChange(sheetObj, Row, Col, value){
		...
		if(sheetObj.ColSaveName(Col) == "trd_cd"){
			...
			var sXml = sheetObj.GetSearchXml("ESM_MAS_0029GS.do", param);

			var arrXml = sXml.split("|$$|");
			if (arrXml.length > 0)
				ComMasSetIBCombo(sheetObj, arrXml[0], "sub_trd_cd", true, 0, Row);
			if (arrXml.length > 1)
				ComMasSetIBCombo(sheetObj, arrXml[1], "slan_cd", true, 0, Row);
			if (arrXml.length > 2)
				ComMasSetIBCombo(sheetObj, arrXml[2], "rlane_cd", true, 0, Row);
	...
	function popVvdCheck(){
		var formObj = document.form;
		var param = "?" + masFormQueryString(formObj, 'param1|param2|param3|param4|param5|param6|param7|param8|f_chkprevcre|fm_date|to_date');
		ComOpenWindow2('ESM_MAS_0142.do'+param, '', 'width=900, height=500, menubar=0, status=1, scrollbars=0, resizable=0');
	}
		...
		switch (sAction) {
	 		...
	 		case IBBATCH: // batch
				...
				if((formObj.f_to_wk.value - formObj.f_fm_wk.value) > 15){
					// [MAS10003] : BSA 는(은) 16주 만 처리할수 있습니다.
					ComShowCodeMessage("MAS10003", "BSA & VVD Creation", "15 Weeks");
	 		...
	 		case IBCREATE: // create
				...
				if((formObj.f_to_wk.value - formObj.f_fm_wk.value) > 53){
					// [MAS10003] : BSA 는(은) 16주 만 처리할수 있습니다.
					ComShowCodeMessage("MAS10003", "Target VVD Creation", "53 Weeks");
					formObj.f_to_wk.focus();
					return false;
				}
	 		...
	 		case IBRESET: // Create T/S Q`ty
	 			...
	 			if(!chkValidSearch()) {
					...
					if((formObj.f_to_wk.value - formObj.f_fm_wk.value) > 16){
						// [MAS10003] : BSA 는(은) 16주 만 처리할수 있습니다.
						ComShowCodeMessage("MAS10003", "Create T/S Qty", "16 Weeks");
						formObj.f_to_wk.focus();
						return false;
					}
	  		 break;
	 		...
	 		case IBSEARCH_ASYNC01: // Apply BSA 0
	 			...
	 			if(!chkValidSearch()) {
					...
					if((formObj.f_to_wk.value - formObj.f_fm_wk.value) > 16){
						// [MAS10003] : BSA 는(은) 16주 만 처리할수 있습니다.
						ComShowCodeMessage("MAS10003", "Apply BSA 0", "16 Weeks");
						formObj.f_to_wk.focus();
						return false;
					}
	  		 break;
		...
		if (button_id == "CREATE"){
			...
			if(err_cd == "00000"){
				if(rtn_vsl_cd == "" || rtn_vsl_cd == null || rtn_vsl_cd =="null" || rtn_vsl_cd == undefined || rtn_vsl_cd == "undefined"){
					...
					// [MAS10022] : 저장된 Vessel정보에 [vessel 정보] 데이터가 있습니다. 확인하여 주시기 바랍니다.
					ComShowMessage(ComGetMsg("MAS10022", rtn_vsl_cd));
				}
			...
			if (err_cd == "00000") {
				ComShowMessage(ComGetMsg('MAS10018', button_id));
			} else if (err_cd == "00028") {
				ComShowMessage("ERROR(MAS00028): " + err_msg);
			} else{
				ComShowMessage("ERROR(MAS00028): " + err_msg);
			}	
		}
    ...
    function monitoringBatchJob(){
    	...
    	if( BatchStatus == "P" ){
    		...
    		ComShowCodeMessage('MAS10018',"Target VVD Creation"); 
    		ComOpenWait(false);
    		doActionIBSheet(sheetObj,formObj,IBSEARCH);
    	}
    }

...
/**
 * @extends 
 * @class ESM_MAS_0251 : ESM_MAS_0251 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0251() {
...
document.onclick = processButtonClick;
    ...
    function loadPage() {
		...
		sheetObjects[0].InitDataCombo(0, "usa_io_bnd_cd", " |I|O", " |I|O");
		sheetObjects[0].InitDataCombo(0, "bkg_rcv_de_term_cd", " |D|Y", " |D|Y");
		
		
        loadingMode = false;
        ComSetFocus(document.form.f_fm_mon);
    }
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                with (sheetObj) {
                    ...
                    var HeadTitle0  = "STS|DEL.|YYYY-MM|Contract No\n(S/C, RFA, TAA)|SCC|Direction\n(US based)|REC/DEL Term|Unit Cost|CTRT SEQ";                    
                    ...
                    InitDataProperty(0,     cnt++,  dtData,         100,    daCenter,   true,       "ctrt_no",                  false,   "",   dfNone,   0,  true,  true);
                    InitDataProperty(0,     cnt++,  dtData,         50,     daCenter,   true,       "scc_cd",                   false,   "",   dfNone,   0,  true,  true);                    
                    InitDataProperty(0,     cnt++,  dtComboEdit,    90,     daCenter,   true,       "usa_io_bnd_cd",            false,   "",   dfNone,   0,  true,  true, 1);
                    InitDataProperty(0,     cnt++,  dtComboEdit,    100,    daCenter,   true,       "bkg_rcv_de_term_cd",       false,   "",   dfNone,   0,  true,  true, 1);
                    InitDataProperty(0,     cnt++,  dtData,         70,     daCenter,   true,       "stnd_uc_amt",              false,   "",   dfFloat,  1,  true,  true);
                    InitDataProperty(0,     cnt++,  dtHidden,       60,     daCenter,   true,       "ctrt_seq",                 false,   "",   dfNone,   0,  true,  true);

                    CountPosition  = 0 ;
                    
                    InitDataValid(0, "cost_yrmon", vtNumericOnly);
                    InitDataValid(0, "ctrt_no", vtEngUpOther, "0123456789");
    ...
    function sheet1_OnChange(sheetObj, row, col, value){
        ...
        if(sheetObj.ColSavename(col) == "slan_cd"){
            if(sheetObj.CellValue(row,"slan_cd") != ""){
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0251GS.do", param);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {        
			...
			case IBSEARCH:              //조회
                ...
                sheetObj.DoSearch4Post("ESM_MAS_0251GS.do", masFormQueryString(formObj));
            ...
            case IBSAVE:                //저장
                ...
                sheetObj.DoSave("ESM_MAS_0251GS.do", masFormQueryString(formObj,'f_cmd',true), -1, true);
...
//	        	}
            ...
            case IBCREATE:
                ...
                sheetObj.DoSearch("ESM_MAS_0251GS.do", FormQueryString(formObj));          
   ...
   /**
 	...
 	*/
 	function checkValidationAllData(sheetObj) {
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0251GS.do", sParam);

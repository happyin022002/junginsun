...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0119 : ESM_MAS_0119 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0119() {
  ...
  /* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
  	...
  	*/
  		...
  		switch(sheetNo) {
  			case 1:		//sheet2 init
  				with (sheetObj) {
  					...
  					InitDataProperty(0, cnt++, dtCombo,	75,		daCenter,	true,	"mas_cost_src_cd",		true,	"",	dfNone,		0,	false,	true, 8);
  					InitDataProperty(0, cnt++, dtData,	150,	daLeft,		true,	"mas_cost_src_cd_nm",	false,	"",	dfNone,		0,	false,	false);
  	...
  	/**
  	* 쉬트 값이 변경 되었을 경우 mas_cost_src_cd_nm 값 셋팅
  	* tml_ut_cd 값 유효성 체크
  	*/
  	function sheet1_OnChange(sheetObj, row, col, value){
  		var tmpSN = sheetObj.ColSaveName(col);
  		if(tmpSN == "mas_cost_src_cd")	{
  			//sheetObj.CellValue2(row,"mas_cost_src_cd_nm") = srcTxtArr[sheetObj.GetComboInfo(row,col, "SelectedIndex")];
  			sheetObj.CellValue2(row,"mas_cost_src_cd_nm") = ComGetComboText(sheetObj, row, col,1);
  			//mas_cost_src_cd_nm을 setting 
  		} else 	if(tmpSN == "tml_ut_cd"){
  			if(value.substring(0, 7) != sheetObj.CellValue(row, "tml_cd"))	{
  				ComShowCodeMessage('MAS10035');
  	...
  	function doActionIBSheet(sheetObj,formObj,sAction) {
  		...
  		switch(sAction) {
	    	case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0119GS.do", masFormQueryString(formObj));
				var arrXml = sXml.split("|$$|");

				if (arrXml.length > 0)
					ComMasSetIBCombo(sheetObj, arrXml[0], "mas_cost_src_cd", true, 0, 0, "","", true);
				
				/*if (arrXml.length > 1)
					ComMasSetIBCombo(sheetObj, arrXml[1], "tml_ut_cd", false,0);*/
  			...
  			case IBSEARCH:	//조회
  				if(validateForm(sheetObj,formObj,sAction)){
					...
					ComOpenWait(true);
  					...
  					sheetObj.DoSearch4Post("ESM_MAS_0119GS.do", masFormQueryString(formObj));
  					ComOpenWait(false);
  				}
  				break;
  			...
  			case IBSAVE:		//저장
  				if(validateForm(sheetObj,formObj,sAction)){
					...
					ComOpenWait(true);
  					...
  					sheetObj.DoSave("ESM_MAS_0119GS.do", masFormQueryString(formObj));
  					ComOpenWait(false);
  				}
  				break;

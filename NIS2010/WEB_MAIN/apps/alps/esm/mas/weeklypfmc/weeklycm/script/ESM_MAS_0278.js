...
/**
 * @extends Bkg
 * @class ESM_MAS_0278 : 예)Booking 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0278() {
...
function loadPage() {
	...
	// 부모화면에서 exp_cust_cd 컬럼에 입력한 값을 팝업 시트에 셋팅해 준다.
	var Row = 1;
	var f_exp_cust_cd = formObject.f_exp_cust_cd.value;
	var fExpCustCd = f_exp_cust_cd.split(",");	
	for (var i = 0; i < fExpCustCd.length; i++) {		
		if(i >= 0 && i <= 5) {
			sheetObj.CellValue((Row + i), "multiplekey") = fExpCustCd[i];
		} else {
...
function comPopupOK(sheetObj,formObject) {
	...
	if(errcnt > 0){
		...
		opener.getMAS_ENS_278(rArray,return_val);  //호출하는 부모js에 getTRS_ENS_906펑션을 붙여넣으면됩니다.
		window.close();
	}	
}
 ...
 */
function getMAS_ENS_278(rowArray,return_val) {
	var formObject = document.form;

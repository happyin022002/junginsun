...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0117 : ESM_MAS_0117 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0117() {
		...
		try {
			...
			switch(srcName) {
    			...
    			case "btn_Month_Copy":		//팝업창(Month Copy)
	  	            var display = "0,1";
	  	            ComOpenPopup("ESM_MAS_0173.do?classId=ESM_MAS_0117", 250, 200, "SMU Cost (RA)", display, true, false);
	  	            break;	
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {				
		...
		switch(sAction) {
			...
			case IBSEARCH:			//조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0117GS.do", masFormQueryString(formObj,'param1|param2|param3|param4'));
				ComOpenWait(false);
				break;
			...
			case IBSAVE:	//저장
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSave("ESM_MAS_0117GS.do", masFormQueryString(formObj,'f_cmd',true));
					ComOpenWait(false);
				}
				break;
            ...
            case IBCREATE:        //SAQ로부터 SMU 단가 I/F
               if(validateForm(sheetObj,formObj,sAction)){
               	if (ComShowConfirm(ComGetMsg('MAS10020')) == true) {
                       ...
                       var sXml = sheetObj.GetSearchXml("ESM_MAS_0117GS.do",masFormQueryString(formObj));

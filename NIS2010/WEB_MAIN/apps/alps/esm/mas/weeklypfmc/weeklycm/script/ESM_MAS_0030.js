...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0030 : ESM_MAS_0030 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0030() {
	...
	*/ 
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_MAS_0030GS.do", masFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	function f_trd_cd_OnChange(obj,value,text) {
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0030GS.do", masFormQueryString(formObj));
		var arrXml = sXml.split("|$$|");
		if (arrXml.length > 0)

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class ESM_MAS_0177 : ESM_MAS_0177 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0177() {
		...
		try {
			...
			switch(srcName) {
	            ...
	            case "btn_load_excel":
	  			...
	  			case "btn_month_copy":
	  		        var display = "0,1";
        	        ComOpenPopup("ESM_MAS_0173.do?classId=ESM_MAS_0177", 250, 200, "LaneTableCopy", display, true, false);
	  				break;      	
    ...
    function doActionIBSheet(sheetObj,formObj,sAction, tAction) {
        ...
        switch(sAction) {
			...
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_MAS_0177GS.do", FormQueryString(formObj));
				...
				if (arrXml.length > 2)
					ComMasSetIBCombo(sheetObj, arrXml[2], "sub_trd_cd", true, 0);	
				ComOpenWait(false);
				break;
            ...
            case IBSEARCH:      //조회			
                ...
                sheetObj.DoSearch("ESM_MAS_0177GS.do", FormQueryString(formObj));
                ComOpenWait(false);
                break;
        	...
        	case IBSAVE: // 저장
				...
				//sheetObj.DoSave("ESM_MAS_0177GS.do", FormQueryString(formObj), -1, false);
				sheetObj.DoAllSave("ESM_MAS_0177GS.do", masFormQueryString(formObj));
			...
			case IBSEARCH: // 조회
	  		...
	  		case IBSAVE:
				if(formObj.f_cost_yrmon.value == "") {
					...
					formObj.f_cost_yrmon.focus();
					return false;
				}else if (sheetObj.IsDataModified && sheetObj.GetSaveString() == "") {
					return false;
				}
 	...
 	function sheet1_OnChange(sheetObj, Row, Col, Value) {
     	...
     	{ 			
     		...
     		case "trd_cd":
 				...
 				var sXml = sheetObj.GetSearchXml("ESM_MAS_0177GS.do", param);
 				...
 				if (arrXml.length > 1) {
 					ComMasSetIBCombo(sheetObj, arrXml[1], "rlane_cd", true, 0, Row);
 				}
 				break; 	

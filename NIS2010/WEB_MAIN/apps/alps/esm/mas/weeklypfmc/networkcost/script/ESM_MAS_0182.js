...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_MAS_0182 : ESM_MAS_0182 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_MAS_0182() {
  ...
  /* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
  		...
  		try {
  			...
  			switch(srcName) {
  				...
  				case "btn_Month_Copy":		//팝업창(Month Copy)
  	        	       var display = "0,1";
  	        	       ComOpenPopup("ESM_MAS_0173.do?classId=ESM_MAS_0182", 250, 200, "AverageUcCopy", display, true, false);
  	        	       break;	
  				case "btn_Rowadd":
    ...
    /**
   	...
   	function f_trd_cd_OnChange(obj,value,text) {
  		...
  		var sXml = sheetObj.GetSearchXml("ESM_MAS_0182GS.do", masFormQueryString(formObj));
  	...
  	function doActionIBSheet(sheetObj,formObj,sAction) {
  		...
  		switch(sAction) {
  			case IBCLEAR:          //조회
  				...
  				var arrXml = sXml.split("|$$|");
  				if (arrXml.length > 0)
  					ComMasSetIBCombo(sheetObj, arrXml[0], "rlane_cd", true, 0);
  			...
  			case IBSEARCH:		//조회			
  				if(validateForm(sheetObj,formObj,sAction)){	
  					...
  					sheetObj.DoSearch4Post("ESM_MAS_0182GS.do", masFormQueryString(formObj));
  					ComOpenWait(false);
  				}
  				break;
  			...
  			case IBSAVE:        // 저장
  				if(!validateForm(sheetObj,formObj,sAction)) return false;
  					...
  					sheetObj.DoSave("ESM_MAS_0182GS.do", FormQueryString(formObj), -1, false);
  					ComOpenWait(false);
  				//}
  				break;
  			...
  			case IBCREATE:		//CREATE
  				...
  				if(ComReplaceStr(formObj.f_cost_yrmon.value,'-','') < nowYrMon){
  					...
  					sheetObj.DoSearch4Post("ESM_MAS_0182GS.do", FormQueryString(formObj));
  					ComOpenWait(false);
  				}	
  				break;				
     ...
     function openLaneDetail(){
  	    ...
  	    //ComOpenWindow('ESM_MAS_0177.do?cost_use_tp_cd=C','Lane_Detail', 'width=330, height=350,menubar=0,status=0,scrollbars=0,resizable=1');
  	    ComOpenWindow('ESM_MAS_0177.do?cost_use_tp_cd=C','Lane_Detail', 'width=400, height=350,menubar=0,status=0,scrollbars=0,resizable=1');

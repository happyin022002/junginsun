    			...
    			case "btn_save":
    				...
    				if(validateForm(formObject)){
    					...
    					if (back_end_job_sts == 2){
    						ComShowMessage(ComGetMsg("MAS00003","Back-end Job"));
    						return false;
    					}
    					if(formObject.f_src_mon.value >= formObject.f_tar_mon.value){
		    			 	ComShowCodeMessage('MAS10051');
		    			 	formObject.f_src_mon.focus();
		    			 	return false;
	    				}
    					
    					if(ComShowConfirm(ComGetMsg('MAS10052'))){    						
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        switch(sAction) {
            case IBSAVE:        //저장
                ...
                if(formObj.check_cnt.value=="Y"){//이미 데이터가 존재 하는지 확인한다.
                    if(formObj.f_class_id.value == "ESM_MAS_0015") { //  DEM/DET 3RD조회수정
                        formObj.f_cmd.value = SEARCH17;
                    }else if(formObj.f_class_id.value == "ESM_MAS_0152") { //  MRI Inquiry (PA/RA)
                        formObj.f_cmd.value = SEARCH18;
                    }else if(form.f_class_id.value == "ESM_MAS_0157"){//Agent Other Commission Inquiry (PA/RA)
                        formObj.f_cmd.value = SEARCH19;                        
                    }
                    var sXml = sheetObj.GetSaveXml("ESM_MAS_0173GS.do", masFormQueryString(formObj));
                    
                    var dataCnt = ComGetEtcData(sXml, "DATA_COUNT");
                     
                    if( dataCnt > 0){
                        if(!confirm("Target Month에 데이터가 "+dataCnt+"건 존재 합니다. 삭제 하고 다시 copy 하시겠습니까?")){//
                            return;
                        }
                    }
                ...
                if(formObj.f_class_id.value == "ESM_MAS_0125") {
                	formObj.f_cmd.value = MULTI02;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0182") {
			 		formObj.f_cmd.value = MULTI01;
			 		formObj.f_cost_use_tp_cd.value = "C";
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0013") {
			 		formObj.f_cmd.value = MULTI03;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0012") { // ABC/STP Copy
			 		formObj.f_cmd.value = MULTI04;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0009") { // EQ Repo Copy
			 		formObj.f_cmd.value = MULTI05;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0115") { // EMU Cost
			 		formObj.f_cmd.value = MULTI06;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0021") { // Manual Cost Set up
			 		formObj.f_cmd.value = MULTI07;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0042") { // Dailyhire by Cht-VSL (PA)
			 		formObj.f_cmd.value = MULTI08;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0043") { // AVG-hire by Own-VSL (PA)
			 		formObj.f_cmd.value = MULTI09;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0117") { // SMU Cost (RA)
			 		formObj.f_cmd.value = MULTI10;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0014") { // Domestic Saving Credit
			 		formObj.f_cmd.value = MULTI11;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0177") { // Lane Table (1Cycle)
			 		formObj.f_cmd.value = MULTI12;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0318") { // Set Target L/F
			 		formObj.f_cmd.value = MULTI13;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0330") { // Average U/C_Vessel Pooling1 (OP Fixed cost)
			 		formObj.f_cmd.value = MULTI14;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0331") { // Vessel register
			 		formObj.f_cmd.value = MULTI15;
			 	} else if(formObj.f_class_id.value == "ESM_MAS_0333") { // Average U/C_Vessel Pooling2
			 		formObj.f_cmd.value = MULTI16;
	            } else if(formObj.f_class_id.value == "ESM_MAS_0015") { //  DEM/DET 3RD조회수정
	                formObj.f_cmd.value = MULTI17;
                } else if(formObj.f_class_id.value == "ESM_MAS_0152") { //  MRI Inquiry (PA/RA)
                    formObj.f_cmd.value = MULTI18;	    
                } else if(formObj.f_class_id.value == "ESM_MAS_0157") { //  Agent Other Commission Inquiry (PA/RA)
                    formObj.f_cmd.value = MULTI19;    
                } else if(formObj.f_class_id.value == "ESM_MAS_0338") { // Agreed Network Cost Ratio Creation
                ...
                if (formObj.f_class_id.value == "ESM_MAS_0009"){ // EQ Repo는 back-end로 처리 한다.
                	ComOpenWait(true);
                	 var sXml = sheetObj.GetSaveXml("ESM_MAS_0173GS.do", masFormQueryString(formObj));
...
//                	sheetObj.DoAllSave("ESM_MAS_0173GS.do", masFormQueryString(formObj,'f_cmd',true), -1, false);
                	var sXml = sheetObj.GetSaveXml("ESM_MAS_0173GS.do", masFormQueryString(formObj));
                	var save_result = ComGetEtcData(sXml, "TRANS_RESULT_KEY")
                	
                	if (save_result == 'S'){
                		ComShowMessage(ComGetMsg("MAS00004","Data Copy"));
                		window.close();
                	} else {
                		ComShowMessage(ComGetMsg("MAS00005","Data Copy","Data Copy"));
                	}
                }
//                ComOpenWait(false);
                break;
	...
	*/
	...
	function getBackEndJobStatus() {
		...
		var sXml = sheetObj.GetSearchXml("ESM_MAS_0173GS.do", FormQueryString(formObj));

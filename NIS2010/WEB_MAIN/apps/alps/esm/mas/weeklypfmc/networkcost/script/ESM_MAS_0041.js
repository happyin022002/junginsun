...
/**
 * @extends 
 * @class ESM_MAS_0041 : ESM_MAS_0041 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_MAS_0041() {
    ...
    switch(sheetNo) {
        case 1:      //sheet1 init
            ...
            //전체 너비 설정
                ...
                var HeadTitle0  = "STS|YYYY-MM|Week|Trade|Sub-Trade|S. Lane|R. Lane|Bound|Trade Dir.|Vessel code|Class|Cons/Day|Average Cons|Unit Cost/Ton";
                ...
                InitDataProperty(0,     cnt++,  dtData,         54,     daCenter,   true,       "trd_cd",           false,     "",         dfNone,       0,  false,  true, 3, true);
                InitDataProperty(0,     cnt++,  dtData,         78,     daCenter,   true,       "sub_trd_cd",       false,     "",         dfNone,       0,  false,  true, 2, true);
                ...
                InitDataProperty(0,     cnt++,  dtCombo,        54,     daCenter,   true,       "slan_dir_cd",      true,      "",         dfNone,       0,  false,  true);
                ...
                InitDataProperty(0,     cnt++,  dtAutoSum,      90,     daRight,    true,       "foil_avg_csm",     false,     "",         dfFloatOrg,   2,  false,  false);
...
function sheet1_OnChange(sheetObj, row, col, value){
    ...
    if(sheetObj.ColSavename(col) == "slan_cd"){
        if(sheetObj.CellValue(row,"slan_cd") != ""){
			...
			var sXml = sheetObj.GetSearchXml("ESM_MAS_0041GS.do", param);
...
//				if (arrXml.length > 3)
		...
		case IBSEARCH:              //조회
            ...
            sheetObj.DoSearch4Post("ESM_MAS_0041GS.do", masFormQueryString(formObj));
		...
		case IBSAVE:                //저장
            ...
            sheetObj.DoSave("ESM_MAS_0041GS.do", masFormQueryString(formObj,'f_cmd',true), -1, false);
        ...
        case IBCREATE:
            ...
            sheetObj.DoSearch("ESM_MAS_0041GS.do", FormQueryString(formObj));          
...
function checkValidationAllData(sheetObj) {
	...
	var sXml = sheetObj.GetSearchXml("ESM_MAS_0041GS.do", sParam);
...
function setInit_combo(comboObj, codeItem, codeId, codeInit){
	...
	var sXml = sheetObjects[0].GetSearchXml("ESM_MAS_0041GS2.do", vParam);
	ComXml2ComboItem(sXml, comboObj, "code", "name");
	comboObj.Code = codeInit;
}    

function setCostModi(){
	var formObj = document.form;
	var sUrl = "";
	var sParam = "?f_yrtype="+ComGetObjValue(formObj.f_yrtype)
+"&f_yearweek="+ComGetObjValue(formObj.f_yearweek)
...
+"&f_hul_bnd_cd="+ComGetObjValue(formObj.f_hul_bnd_cd);
	sUrl = "/hanjin/ESM_MAS_0321.do"+sParam;
	ComOpenPopup(sUrl, 950, 600, "", "0,0", true, false);
	//ComOpenPopup(sUrl, 950, 600, "", "0,0", false, false);
}
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0015 : ESM_COA_0015 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0015() {
		...
		switch(sheetNo) {
			case 1:	//sheet2 init
				with (sheetObj) {
					...
					InitColumnInfo(12, 4, 0, true);//컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
					InitHeadMode(false, false, false, true, false,false)//([SortEnable], [ColumnMove], [AllCheckEnable],[UserResize], [RowMove],[Head3D])

					var HeadTitle = "STS|No.|TP/SZ|H_C/A Code|C/A Code|H_SO Code|SO Code|Unit Cost|AMT|Vol.|H_CTRT/AVG|CTRT/AVG" ;
					...
					InitDataProperty(0, cnt++ , dtHidden,	80,		daLeft,		false,	"coa_cost_src_cd",		false,	"",	dfNone,		0,	false,	false);
					InitDataProperty(0, cnt++ , dtData,		220,	daLeft,		false,	"coa_cost_src_cd_nm",	false,	"",	dfNone,		0,	false,	false);
		...
		switch(sAction) {
			...
			case IBSEARCH:	//조회
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSearch4Post("ESM_COA_0015GS.do", coaFormQueryString(formObj));
					ComOpenWait(false);
				}
				break;
			...
			case IBSAVE:	//저장
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSave("ESM_COA_0015GS.do", coaFormQueryString(formObj));
					ComOpenWait(false);
				}
				break;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0009 : ESM_COA_0009 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0009() {
		...
		try {
			...
			switch(srcName) {
				...
				case "btn_Month_Copy":		//팝업창(Month Copy)
		     	       var display = "0,1";
		     	       ComOpenPopup("ESM_COA_0173.do?classId=ESM_COA_0009", 250, 200, "AverageUcCopy", display, true, false);
		     	    break;						
...
//}	
    ...
    ////////full 화면 더블 클릭 //////////////////////////////////
  	...
  	function doActionIBSheet(sheetObj,formObj,sAction) {
  		...
  		switch(sAction) {
  			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0009GS.do", FormQueryString(formObj));
  			...
  			case IBSEARCH:	//조회
  				...
  				if(sheetObj.id == "sheet1"){
  					...
  					sheetObj.DoSearch4Post("ESM_COA_0009GS.do", coaFormQueryString(formObj));
     ...
     /**
  	...
  	function validateForm(sheetObj,formObj,sAction){
  		...
  		if (formObj.f_cost_loc_grp_cd.selectedIndex == 1 && formObj.f_lcc_cd.Text == ""){
  			ComShowCodeMessage('COA10002', 'LCC Code');
  		...
  		if (formObj.f_cost_loc_grp_cd.selectedIndex == 2 && formObj.f_rcc_cd.Text == ""){
  			ComShowCodeMessage('COA10002', 'RCC Code');
 	...
 	function setPeriod(obj){
		...
		var sXml = sheetObj.GetSearchXml("ESM_COA_0009GS.do", coaFormQueryString(formObj));
		var arrXml = sXml.split("|$$|");

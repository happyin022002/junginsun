...
* 2011.06.24 이행지 [CHM-201111807-01] COA NODE 단가 화면 저장시 발생될 수 있는 문제 해결-컬럼명 변경 (USD AMT-> Curr. AMT), Link Cost 변경시 alert창 문제 해결
* 2011.07.04 이행지 [CHM-201111961-01] [COA] Node/Link U/C Adjustment 화면 칼럼명 변경 (Link Cost 컬럼명 변경:USD AMT-> Curr. AMT)
...
* 2014.08.27 박은주 [CHM-201431751] [COA] Link U/C Adjustment 조회/입력조건의 Vendor 추가요청(S/P NO)
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0137 : ESM_COA_0137 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0137() {
    	...
    	function initSheet(sheetObj,sheetNo) {
    		...
    		case 2:		//sheet2 init
    			with (sheetObj) {
                    ...
                    sheetObj.RangeFontColor(sheetObj.lastRow, 4, sheetObj.lastRow, 4) = RgbColor(255, 0  , 0  );     						   				
       				sheetObj.DataInsert(-1);
       				sheetObj.CellValue(sheetObj.lastRow, "kind") = "COA_COST_SRC_CD";
       				sheetObj.CellValue(sheetObj.lastRow, "desciption") = "비용계정";
       				sheetObj.CellValue(sheetObj.lastRow, "colName") = "COA_COST_SRC_CD";
       				sheetObj.CellValue(sheetObj.lastRow, "inEquality") = "=";   				
    		...
    		case 3:		//sheet3 init
    			with (sheetObj) {
    			    ...
    			    if(f_table_name=="COA_NOD_AVG_STND_COST"){
                        ...
                        InitDataProperty(0  , cnt++, dtCombo   , 70   , daCenter , false   , "coa_cost_src_cd_v"  , false   , ""        ,	dfNone      ,	0        , false     ,	true   );
                        InitDataProperty(0  , cnt++, dtData    , 100  , daCenter , false   , "coa_cost_src_cd"    , true    , ""        ,	dfEngUpKey  ,	0        , false     ,	true , 6    );
...
//                        InitDataProperty(0  , cnt++, dtData     , 50   , daCenter , false   , "co_cd"              , true    , ""        ,   dfNone       ,  0        , false     ,  true , 1  );
      ...
      function sheet3_OnChange(sheetObj, row, col, value){
          ...
          if(sheetObj.ColSaveName(col) == "coa_cost_src_cd"){
              if(srcStndCd.indexOf(sheetObj.CellValue(row, "coa_cost_src_cd"))>0) {
                  sheetObj.CellValue(row,"coa_cost_src_cd_v") = sheetObj.CellValue(row, "coa_cost_src_cd");       
                  sheetObj.CellValue(row,"stnd_cost_cd") = sheetObj.CellText(row, "coa_cost_src_cd_v");   
              }
              else {
                  sheetObj.CellValue(row,"coa_cost_src_cd_v") = "";       
    	...
    	function doActionIBSheet(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
    			case IBCLEAR:          //조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_COA_0137GS.do", coaFormQueryString(formObj));
					
					var arrXml = sXml.split("|$$|");
					if (arrXml.length > 0){
						srcStndCd = arrXml[0];
						ComCoaSetIBCombo(sheetObjects[2], arrXml[0], "coa_cost_src_cd_v", true, 0);
					}
					if (arrXml.length > 1){
						var f_table_name = document.form.f_table_name.value;
						if(f_table_name != "COA_LNK_AVG_STND_COST"){
							srcStndCd = arrXml[1];
							ComCoaSetIBCombo(sheetObjects[2], arrXml[1], "slan_cd", true, 0);
    			...
    			case IBSEARCH:		           //조회
    			    if(sheetObj==sheetObjects[0]){              // 조건창 
        				...
        				sheetObj.DoSearch4Post("ESM_COA_0137GS.do", coaFormQueryString(formObj));  				                   
    			    }
                        ...
                        sheetObj.DoSearch4Post("ESM_COA_0137GS.do", params+"&"+coaFormQueryString(formObj));
                        ComOpenWait(false);
    			    }
    				break;	
    			case IBSAVE:                  // 저장
        			...
        			sheetObj.DoSave("ESM_COA_0137GS.do", coaFormQueryString(formObj), -1, true);			
        			ComOpenWait(false);
    				break;					

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0160 : ESM_COA_0160 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0160() {
    		...
    		switch(sAction) {
    			...
    			case IBSEARCH:	//조회
    				if(validateForm(sheetObj,formObj,sAction)){
    					...
    					sheetObj.DoSearch4Post("ESM_COA_0160GS.do", coaFormQueryString(formObj));
    					ComOpenWait(false);
    				}
    				break;
    			...
    			case IBSAVE:		//저장
    				if(validateForm(sheetObj,formObj,sAction)){
    					...
    					sheetObj.DoSave("ESM_COA_0160GS.do", coaFormQueryString(formObj));
    					//sheetObj.DoAllSave("ESM_COA_160GS.do", FormQueryString(formObj));
    					ComOpenWait(false);
    				}
    				break;

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0004 : ESM_COA_0004 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
      function ESM_COA_0004() {
    	...
    	function doActionIBSheet(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
    			case IBCLEAR:          //조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_COA_0004GS.do", FormQueryString(formObj));
    			...
    			case IBSEARCH:		//조회
    				if(validateForm(sheetObj,formObj,sAction)){
    					...
    					sheetObj.DoSearch4Post("ESM_COA_0004GS.do", coaFormQueryString(formObj));
    				...
    				if(validateForm(sheetObj,formObj,sAction)){
    					if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
    						if(sheetObjects[0].CheckedRows("sel_check")>0){//체크된행이 있는경우
    							...
    							//선택된 행들을 쿼리로
    			    		    //20100415 이중환, FormQueryString -> coaFormQueryString 변경
    							sheetObj.DoSearch4Post("ESM_COA_0004GS2.do", coaFormQueryString(formObj) +"&" +sheetObjects[0].GetSaveString(false, true, "sel_check"));
    							ComOpenWait(false);
    						} else {

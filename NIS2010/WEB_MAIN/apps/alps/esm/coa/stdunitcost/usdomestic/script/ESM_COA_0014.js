...
* 2012.11.20 송호진 [CHM-201221090-01] [COA] US DOMESTIC 반영 - CNTR_SZ_CD -> CNTR_TPSZ_CD 컬럼명 변경에 따른 Sheet 상의 컬럼명 Text 변경 ( Size -> Type Size )
* 2012.11.21 송호진 [CHM-201221090-01] [COA] US DOMESTIC 반영 - TRP, EQ off-hire Tab 특정 컬럼의 Sum 값 로직 변경, LoadSearchXml Wait Image 표시 관련 수정 
...
* 2014.04.16 박다은 [CHM-201429852] [COA] SELCOT, SELCON 조직 변경(추가) 관련 COA 관련 작업 요청
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class ESM_COA_0014 : ESM_COA_0014 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESM_COA_0014() {
...
/*					
				case "btn_month_copy":		//팝업창(Month Copy)
		     	       var display = "0,1";
		     	       ComOpenPopup("ESM_COA_0173.do?classId=ESM_COA_0014", 250, 160, "AverageUcCopy", display, true, false);
		     	    break;	
...
*/				
    ...
    function loadPage() {
    	...
    	// [CHM-201429852] [COA] SELCOT, SELCON 조직 변경(추가) 관련 COA 관련 작업 요청
		if(formObj.v_ofc_cd.value != "SELCSG" && formObj.v_ofc_cd.value != "SELCTY" && formObj.v_ofc_cd.value != "SELOPA"&& formObj.v_ofc_cd.value != "SELOPB"){
			/*
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
        	...
        	case IBSEARCH:	// ECC, LCC, RCC 콤보박스 조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0014GS.do", coaFormQueryString(formObj));
...
/*				
			case MULTI01: // 수정한 cost 저장
				...
				if ( sheetObj.DoSave("ESM_COA_0014GS.do", coaFormQueryString(formObj), -1, false) ) {
			...
			case COMMAND01 : // 데이터 create
				...
				if( formObj.coa_ut_cost_cre_sts_cnt.value == 0 && !ComShowConfirm(ComGetMsg("COA10020"))) {
	            	return false;
	            }

				var sParam = "";  
            	formObj.f_cmd.value = COMMAND01;
				sParam += "&" + coaFormQueryString(formObj);
				//sheetObj.WaitImageVisible = false;
				ComOpenWait(true);
 				var sXml = sheetObj.GetSaveXml("ESM_COA_0014GS.do", sParam)
...
*/            
			...
			case SEARCH02 : // CREATE 하기 전 COA_MTY_ECC_UT_COST & COA_UT_COST_CRE_STS 테이블 데이터 유무 확인
				
				if(!validateForm(sheetObj,formObj,sAction))return;
				
				formObj.f_cmd.value = SEARCH02;
				var sXml = sheetObj.GetSearchXml("ESM_COA_0014GS.do", coaFormQueryString(formObj));
	    		var mtyEccUtCostCnt = ComGetEtcData(sXml, "coa_mty_ecc_ut_cost_cnt");
	    		var utCostCreStsCnt = ComGetEtcData(sXml, "coa_ut_cost_cre_sts_cnt");

				formObj.coa_mty_ecc_ut_cost_cnt.value = mtyEccUtCostCnt;
				formObj.coa_ut_cost_cre_sts_cnt.value = utCostCreStsCnt;
				break;
        }
    ...
    function doActionValidationResult(sheetObj, sKey) {
    	var sXml = sheetObj.GetSearchXml("ESM_COA_0014GS.do?f_cmd=" + COMMAND02 + "&key=" + sKey);
    	...
    	if (sJbStsFlg == "SUCCESS") {
    		...
    		//sheetObj.WaitImageVisible = true;
    		// 에러메시지 보여주고
    		ComShowMessage(ComGetMsg("COA00001"));
    		clearInterval(intervalId);
    	}
    }
	    ...
	    switch(sAction) {
	    	...
	    	case COMMAND01: {
	    		...
	    		if(formObj.f_cre_start_dt.value == "" || formObj.f_cre_end_dt.value == "") {
		  			ComShowCodeMessage('COA10015', 'Period');
		  			ComSetFocus(formObj.f_cre_start_dt);
		  			return false;	
		  		}
...
/*
	    		if(ComParseInt(formObj.f_cre_end_dt.value.replace(eval("/-/gi"), ""))
                    ...
                    ComShowMessage(ComGetMsg("COA10003", "Creation month", "6 months"));
                    ComSetFocus(formObj.f_cre_start_dt);
                    return false;
	    		}
*/	    		
	    		...
	    		if(formObj.coa_mty_ecc_ut_cost_cnt.value == 0) {
		  			ComShowCodeMessage('COA10015', 'MT repo simulated cost');
		  			return false;	
		  		}
	    		
	    		if(formObj.coa_ut_cost_cre_sts_cnt.value > 0) {

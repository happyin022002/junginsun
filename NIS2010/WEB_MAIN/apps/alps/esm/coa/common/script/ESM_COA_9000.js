...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
	 * @extends
	 * @class ESM_COA_9000 : ESM_COA_9000 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESM_COA_9000() {
        ...
        switch(sheetNo) {
			case 1:
				...
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtData,      250,    daCenter,  true,    "coa_bat_rmk",    true,           "",       dfNone,       	0,     false,      true);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_9000GS.do", coaFormQueryString(formObj));
			...
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_9000GS.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
			...
			switch(sAction) {
				case IBSAVE:
					...
					for (idx=1; idx<lastRow+1; idx++){
						if(sheetObj.CellValue(idx,"bkg_no")==""||sheetObj.CellValue(idx,"coa_bat_rmk")==""){

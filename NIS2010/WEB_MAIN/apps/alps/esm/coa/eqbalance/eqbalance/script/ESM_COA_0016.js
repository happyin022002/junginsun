...
* 2011.01.11 이행지 [CHM-201108260-01]  [COA]Rule-Set 화면 보완사항 처리
   ...
   /**
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0016GS2.do", coaFormQueryString(formObj));
				...
				ComCoaSetIBCombo(sheetObjects[1], arrXml[0], "cntr_tpsz_cd", false, 0);
				//ComCoaSetIBCombo(sheetObjects[1], arrXml[3], "cntr_org_dest_cd", false, 0); -- OP|DEL 로 세팅
				ComOpenWait(false);
				break;
			...
			case IBSAVE:        //저장
				...
				sheetObj.DoAllSave("ESM_COA_0016GS.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	function doActionIBSheet2(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0016GS2.do", coaFormQueryString(formObj));
				excel_load_yn = "N";
				ComOpenWait(false);
				break;
		}
	...
	function doActionIBSheet3(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0016GS3.do", coaFormQueryString(formObj)+"&f_ori_dest="+cntr_org_dest_cd);
				excel_load_yn = "N";
				ComOpenWait(false);
				break;
	...
	function doSave(){
		...
		if(ComShowConfirm(ComGetMsg('COM130101'))){
			...
			sheetObj.DoSave("ESM_COA_0016GS3.do", coaFormQueryString(formObj)+"&excel_load_yn="+excel_load_yn,-1,false);
			excel_load_yn = "N";
...
//    	if(sheetObj.id == "sheet2") return true;
		...
		with(formObj){
			...
			if(formObj.f_cntr_tpsz_cd.Code == "" || formObj.f_cntr_tpsz_cd.Code == null ){
				...
				ComShowCodeMessage('COA10002','Type/Size');
    ...
    function sheet2_OnChange (sheetObj, row , col, value ){
    	...
    	if (sheetObj.ColSaveName(col) == "ecc_cd" && sheetObj.CellValue(row, "ecc_cd") !='' ) {
    		...
    		var sXml = sheetObj.GetSearchXml("ESM_COA_0016GS2.do", coaFormQueryString(formObj)+"&eccCd="+ecc_cd);

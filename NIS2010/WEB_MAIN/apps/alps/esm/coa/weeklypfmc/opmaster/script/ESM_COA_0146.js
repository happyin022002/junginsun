...
* 2011.01.14 김기종 Ticket ID:CHM-201108359-01 Vessel History 화면 기능보완 - 저장후 sheet 콤보 초기화 순서변경
...
/**
 * @extends 
 * @class ESM_COA_0146 : ESM_COA_0146 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
	function ESM_COA_0146() {
	...
	function getEdtDate(selRow,result) {
		...
		if(result == null || result == "" || result == "null"){
			ComShowMessage(ComGetMsg('COA10027',sheetObject1.CellValue(selRow,"vvd_cd")));
	...
	function sheet1_OnChange(sheetObj,Row,Col,Value) {
		...
		with(sheetObj) {
			...
			if (ColSaveName(Col) == "vvd_cd") {
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0146GS.do", param);
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0146GS.do", coaFormQueryString(formObj))
				var arrXml = sXml.split("|$$|");
			...
			case IBSEARCH:      //조회
				...
				var xml = sheetObj.GetSearchXml("ESM_COA_0146GS.do", coaFormQueryString(formObj))

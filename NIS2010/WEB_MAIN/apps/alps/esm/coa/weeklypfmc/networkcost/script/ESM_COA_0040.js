...
*@FileName : ESM_COA_0040.js
*@FileTitle : COA_Port Tariff 관련 스크립트
*Open Issues :
*Change history :
...
* 2011.06.16 최성민 [CHM-201111497-01] COA 운항일수 산정 로직 변경 - 스크립트 오류수정
...
/**
 * @extends 
 * @class ESM_COA_0040 : ESM_COA_0040 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0040() {
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                with (sheetObj) {
                    ...
                    var HeadTitle  = "STS|SEL||Status|YYYY-MM|Week|S.Lane|Vessel|Voyage|Dir.|Trade Dir.|TTL Exp.|TTL Exp(PSO)";                   
...
//  mySheet.ColHidden("cost_wk") = false
     ...
     */
     function sheet1_OnDblClick(sheetObj , row, col){
    	 ...
    	 pop_rtn_value = ComOpenPopup("/hanjin/ESM_COA_0181.do?"+param, 800, 500, "actionAfter0181", "0,0", true);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
			...
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0040GS2.do", coaFormQueryString(formObj));
...
//                sheetObj.DoSearch4Post("ESM_COA_0040GS.do", coaFormQueryString(formObj));
                
				var sXml = sheetObj.GetSearchXml("ESM_COA_0040GS.do", sParam);
				sheetObjects[0].LoadSearchXml(sXml);
				ComOpenWait(false);
				var statusCode = ComGetEtcData(sXml, "BatchStatus" );
				switch(statusCode){
					case "4"://작업실행완료						
						ComShowMessage(ComGetMsg("COA10066", "Port Expense Calculation", "10")); 
						break;
					case "5":// Error 발생
...
//	    					chk_msg += "====================\n";    
    ...
    /**
     * port, cy의 유효성 결과값 반환 (ESM_COA_5127에서 호출함.)
     */
...
////              ComShowMessage(ComGetMsg('COA10002', 'Local Curr.'));
////          }
//        }
//        return rtn;
//    }

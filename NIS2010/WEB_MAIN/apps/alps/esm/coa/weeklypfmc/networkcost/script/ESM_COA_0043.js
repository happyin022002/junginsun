...
* 2009.12.18 최인경 ALPS내 ENIS기능점검 수정
...
/**
 * @extends 
 * @class ESM_COA_0043 : ESM_COA_0043 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0043() {
        ...
        try {
            ...
            switch(srcName) {
    			...
    			case "btn_Month_Copy":		//팝업창(Month Copy)
	  	            var display = "0,1";
	  	            ComOpenPopup("ESM_COA_0173.do?classId=ESM_COA_0043", 250, 200, "AVG hire by Own VSL", display, true, false);
	  	            break;	
        ...
        switch(sheetNo) {
            case 1:      //sheet1 init
                ...
                with (sheetObj) {
                    ...
                    InitColumnInfo(colNo, 5, 0, true);                                       //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                    ...
                    InitDataProperty(0, cnt++,  dtData,     50,     daCenter,       false,      "cost_wk",      false,      "",         dfNone,         0,      false,      false, 2, true);
                    InitDataProperty(0, cnt++,  dtData,     70,     daCenter,       false,      "vsl_cd",       false,      "",         dfEngUpKey,     0,      false,      false);
                    InitDataProperty(0, cnt++,  dtData,     70,     daRight,        false,      "vsl_clss_capa",false,      "",         dfNullInteger,  0,      false,      false);
                    for(j=0; j<aryNo; j++) {
            ...
            case 2:      //sheet2 init
                ...
                with (sheetObj) {
                    ...
                    InitRowInfo(1 , 1, 9, 100);                                         //행정보설정[필수][HEADROWS,DATAROWS,VIEWROWS,ONEPAGEROWS=100]
                    InitColumnInfo(colNo, 3, 0, true);                                      //컬럼정보설정[필수][COLS,FROZENCOL,LEFTHEADCOLS=0,FROZENMOVE=false]
                    InitHeadMode(true, false, false, true, false,false)                 // 해더에서 처리할 수 있는 각종 기능을 설정한다
                    var HeadTitle  = "YYYY-MM|Week|VSL Class|"+headerNM+"|Total|Remark|STS";
                    InitHeadRow(0, HeadTitle, true);                                    //해더행정보[필수][ROW,HEADTEXT,ROWMERGE=false, HIDDEN=false]
                    var tot = "";
                    for(j=0; j<aryNo; j++){
                         tot += "|t"+aryCD[j]+"|";
                    ...
                    //데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
                    InitDataProperty(0, cnt++,  dtData,     60,     daCenter,   false,  "",     false,  "",     dfDateYm,       0,  true,   true);
                    InitDataProperty(0, cnt++,  dtData,     50,     daCenter,   false,  "week",     false,  "",     dfNone,         0,  true,   true);
                    InitDataProperty(0, cnt++,  dtData,     70,     daRight,    false,  "",     false,  "",     dfNullInteger,  0,  true,   true);
                    for(j=0; j<aryNo; j++) InitDataProperty(0,cnt++,dtAutoAvg,130,daRight,false,"t"+aryCD[j], false,    "",     dfNullInteger,  0,  true,   true);

                    InitDataProperty(0, cnt++,  dtAutoAvg,  60,     daRight,    false,  "",     false,  tot,    dfNullInteger,  0,  true,   true);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
	        case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0043GS2.do", coaFormQueryString(formObj));
            ...
            case IBSEARCH:      //조회
                ...
                if(validateForm(sheetObj,formObj,sAction)){
                    ...
                    var xmlEtc = sheetObj.GetSearchXml("ESM_COA_0043GS.do", coaFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
            ...
            case IBSAVE:        //저장
                ...
                if(validateForm(sheetObj,formObj,sAction)){
                    ...
                    sheetObj.DoSave("ESM_COA_0043GS.do", coaFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'), -1, false);

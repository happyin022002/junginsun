...
/**
 * @extends 
 * @class ESM_COA_0174 : ESM_COA_0174 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0174() {
...
/* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
		...
		try {
			...
			switch(srcName) {
				...
				case "btn_Month_Copy":		//팝업창(Month Copy)
	        	       var display = "0,1";
	        	       ComOpenPopup("ESM_COA_0173.do", 250, 200, "AverageUcCopy", display, true, false);
	        	       break;	
				case "btn_rowadd":
	...
	*/
    ...
    function sheet1_OnChange(sheetObj, row, col, value){
        ...
        if(sheetObj.ColSaveName(col) == "trd_cd"){
    		...
    		var sXml = sheetObj.GetSearchXml("ESM_COA_0174GS.do", param);
 	...
 	function f_trd_cd_OnChange(obj,value,text) {
 		...
 		var sheetObj = sheetObjects[0];
		formObj.f_cmd.value = SEARCHLIST10;
		var sXml = sheetObj.GetSearchXml("ESM_COA_0174GS.do", coaFormQueryString(formObj));
		var arrXml = sXml.split("|$$|");
		if (arrXml.length > 0)
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:		//조회			
				...
				//sheetObj.DoSearch4Post("ESM_COA_0174GS.do", coaFormQueryString(formObj));
			...
			case IBSAVE:        // 저장
				...
				sheetObj.DoSave("ESM_COA_0174GS.do", coaFormQueryString(formObj), -1, false);
				ComOpenWait(false);
				break;
				
			case IBCREATE:		//CREATE
				var strUrl = "ESM_COA_0184.do" + "?f_rpb_yrmon="+formObj.f_cost_yrmon.value+"&classId=ESM_COA_0174";
   ...
   function openLaneDetail(){
	    ...
	    //ComOpenWindow('ESM_COA_0177.do?cost_yrmon='+formObj.f_cost_yrmon.value,'Lane_Detail', 'width=550, height=650,menubar=0,status=0,scrollbars=0,resizable=1');
	    ComOpenPopup('ESM_COA_0177.do?cost_yrmon='+formObj.f_cost_yrmon.value, 550, 650, 'setPeriod', '1,0,1,1,1,1,1,1');
	  //  ComOpenWindow('ESM_COA_0177.do?cost_use_tp_cd=A','Lane_Detail', 'width=330, height=350,menubar=0,status=0,scrollbars=0,resizable=1');
   }

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0118 : ESM_COA_0118 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0118() {
    		...
    		switch(sheetNo) {
    			case 1:			//sheet1 init
    				with (sheetObj) {
    					...
    					InitDataProperty(0, cnt++, dtCombo,	      	65,	daCenter,	true,	"coa_cost_src_cd",		true,	"",	dfNone,			0,	false,	true);
    					InitDataProperty(0, cnt++, dtData,	      	200,daLeft,		true,	"coa_cost_src_cd_nm",	false,	"",	dfNone,			0,	false,	false);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
    	...
    	switch(sAction) {
	    	case IBCLEAR:    	
				...
				if (arrXml.length > 0) {
					ComCoaSetIBCombo(sheetObj, arrXml[0], "tml_cd", true, 0);
				}
				if (arrXml.length > 1) {
					ComCoaSetIBCombo(sheetObj, arrXml[1], "cntr_tpsz_cd", true, 0);
				}
				if (arrXml.length > 2) {
					ComCoaSetIBCombo(sheetObj, arrXml[2], "coa_cost_src_cd", true, 0, 0,"","", true);
				}
				if (arrXml.length > 3) {
					ComCoaSetIBCombo(sheetObj, arrXml[3], "locl_curr_cd", true, 0);
				}
				if (arrXml.length > 4) {
					ComCoaSetIBCombo(sheetObj, arrXml[4], "tml_trf_itm_cd", true, 0, 0,"","", true);
				}
				if (arrXml.length > 5) {
					ComCoaSetIBCombo(sheetObj, arrXml[5], "tml_trf_dtl_cd", true, 0, 0,"","", true);
				}
				if (arrXml.length > 6) {
					ComCoaSetIBCombo(sheetObj, arrXml[6], "uc_slan_cd", true, 0);
				}
				formObj.sXml.value="";
				ComOpenWait(false);
				break;
	    	...
	    	case IBSEARCH:			//조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0118GS.do", coaFormQueryString(formObj));				
    			break;
    		...
    		case IBSAVE:			//저장				
				...
				for(var i = sheetObj.HeaderRows; i <= sheetObj.LastRow; i++) {
					if(sheetObj.RowStatus(i) == "I") {    						
						tmlUtCd =   sheetObj.CellValue(i, "tml_cd") + 
									sheetObj.CellValue(i, "coa_cost_src_cd") + 
				...
				sheetObj.DoSave("ESM_COA_0118GS.do", coaFormQueryString(formObj));		
    			break;
 		...
 		switch(colName)
     	{
 			case "coa_cost_src_cd":
 	    		if (Value != ""){
 	    			...
 	    			sheetObj.CellValue2(Row, "coa_cost_src_cd_nm") = sDesc[1];
 	    			 	
 	    			//Tariff Item 세팅
 	    			var sParam = "f_cmd=" + SEARCHLIST10;
 	    			sParam = sParam + "&tml_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "tml_cd");
 	    			sParam = sParam + "&coa_cost_src_cd=" + sheetObj.CellValue(sheetObj.SelectRow, "coa_cost_src_cd");
 	    			var sXml = sheetObj.GetSearchXml("ESM_COA_0118GS.do", sParam);
 	    			var arrXml = sXml.split("|$$|");
 	    			if (ComGetTotalRows(arrXml[0]) > 0) {
 	    				ComCoaSetIBCombo(sheetObj, arrXml[0], "tml_trf_itm_cd", true, 0, Row,"","", true);
 	    			} else {
 	    				sheetObj.CellComboItem(Row,"tml_trf_itm_cd", " ", " ");
 	    			} 	    			
 	    		} else {
 	    			sheetObj.CellValue2(Row, "coa_cost_src_cd_nm") = "";
 	    		}
    		...
    		case "tml_trf_itm_cd":
 	    		if (Value != ""){
 	    			...
 	    			if (ComGetTotalRows(arrXml[0]) > 0) {
 	    				ComCoaSetIBCombo(sheetObj, arrXml[0], "tml_trf_dtl_cd", true, 0, Row,"","", true);
 	    			} else {
    		...
    		case "tml_cd":
 	    		if (Value != "" && sheetObj.CellValue(Row, "coa_cost_src_cd") != ""){

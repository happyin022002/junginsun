...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class ESM_COA_0121 : ESM_COA_0121 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0121() {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0121GS2.do", coaFormQueryString(formObj));
				var arrXml = sXml.split("|$$|");
				if (arrXml.length > 0){			ComCoaSetIBCombo(sheetObj, arrXml[0], "trd_cd", true, 0); 	}
				if (arrXml.length > 1){			ComCoaSetIBCombo(sheetObj, arrXml[1], "sub_trd_cd", true, 0);	}
				if (arrXml.length > 2){			ComCoaSetIBCombo(sheetObj, arrXml[2], "rlane_cd", true, 0);	}
				if (arrXml.length > 3){			
					ComCoaSetIBCombo(sheetObj, arrXml[3], "dir_cd", true, 0);	
					ComCoaSetIBCombo(sheetObj, arrXml[3], "conv_dir_cd", true, 0);	
				}
				ComOpenWait(false);
				break;
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0121GS.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
			...
			case IBSAVE:      //저장
				...
				sheetObj.DoSave("ESM_COA_0121GS.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	function validateForm(sheetObj, formObj, sAction){
		...
		for(a=1;a<(sheetObj.RowCount+1);a++){
			if( (sheetObj.CellValue(a, "dir_cd")== null) || (sheetObj.CellValue(a, "dir_cd")== '') ){
				ComShowMessage(ComGetMsg("COA10026","Bound-Original"));// 
				return;
			}
			if( (sheetObj.CellValue(a, "conv_dir_cd")== null) || (sheetObj.CellValue(a, "conv_dir_cd")== '') ){
				ComShowMessage(ComGetMsg("COA10026","Bound-Adjusted"));
				return;
			}					
		}
...
//					); /* 지우지말것. */
					if( (aDirCd == bDirCd) && (aTrdCd == bTrdCd) ){
							ComShowMessage(ComGetMsg("COM131301","uniquely")); //COA10026 - 'Please select {?msg1}.' ,COM131301 - uniquerly Duplication occurred.
							return false;
					}
				}
			}
	...
	*/
	function sheet1_OnChange(sheetObj, row, col, value){
        ...
        if(sheetObj.ColSaveName(col) == "trd_cd"){
    		...
    		var sXml = sheetObj.GetSearchXml("ESM_COA_0121GS2.do", param);

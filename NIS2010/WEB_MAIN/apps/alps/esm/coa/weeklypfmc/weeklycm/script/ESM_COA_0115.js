...
*@FileName : ESM_COA_0115.js
*@FileTitle :EMU Cost (RA)
*Open Issues :
*Change history : CSR NO. R200802265271 COA 조회 기간 관련 수정 요청  
*               : CSR No. N200802290005 COA_EMU, 샤시 비용 관련 수정
...
*                  - POD 조회 조건 추가 포함 
* 2009.01.23 전윤주 CSR No. N200901230005 COA_EMU Credit 내 EQ Cost 제외 
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0115 : ESM_COA_0115 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0115() {
		...
		try {
			...
			switch(srcName) {
				...
				case "btn_Month_Copy":		//팝업창(Month Copy)
		     	       var display = "0,1";
		     	       ComOpenPopup("ESM_COA_0173.do?classId=ESM_COA_0115", 250, 200, "AverageUcCopy", display, true, false);
		     	    break;	
			} // end switch
		} catch(e) {
			if( e == "[object Error]") {
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0115GS.do", FormQueryString(formObj));
			...
			case IBSEARCH:			//조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0115GS.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	*/
	...
	function validateCond(formObj) {
		...
		if ((formObj.f_from_ecc_cd.Text == "")|| (formObj.f_from_ecc_cd.Text == " ")){
			ComShowCodeMessage('COA10002', 'fromECC');
			ComSetFocus(formObj.f_cost_yrmon);			
...
//			ComShowCodeMessage('COA10002', 'POD');
//			ComSetFocus(formObj.f_cost_yrmon);			

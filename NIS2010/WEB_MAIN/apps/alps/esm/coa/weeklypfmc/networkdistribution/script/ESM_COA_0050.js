...
* 2013.01.15 서미진 [CHM-201322375] Period 조회시, 주차만 셋팅하여 default 값에 년도와 주차 pair 가 맞지 않는 부분 수정 
...
* 2014.01.02 최성민 [CHM-201327860]  약정율 및 OP5, OP6 배부로직 변경
* 2014.03.11 최성민 [CHM-201429166]  [COA] TS Allocation2 (OP5 및 OP6) 배부 로직 변경
=========================================================
*/
...
/**
 * @extends 
 * @class ESM_COA_0050 : ESM_COA_0050 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0050() {
...
function initSheet(sheetObj,sheetNo) {
  switch(sheetNo) {
      case 1:      //sheet1 init
        var com_head = "R.Month|Week|Trade|Lane|IOC|Trade\nDir.|VVD|HJS BSA|Unit Cost\nPer Slot|HJS Sales AMT";
        var head1 = "|Network Cost Assignment"
                  ...
                  + "|Network Cost Assignment"
                  + "|Network Cost Assignment"
                  + "|Actual Operating Profit (OP1)"
                  + "|Actual Operating Profit (OP1)"
                  + "|Actual Operating Profit (OP1)"
                  + "|Operating Profit(Commitment base)"
                  + "|Operating Profit(Commitment base)"
                  + "|Operating Profit(Commitment base)"
                  + "|Operating Profit(Commitment base)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  ...
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)"
                  + "|Adjusted Operating Profit(Commitment Base) (OP5)";
        var head2 = "|R.Month|Week|Trade|Lane|IOC|Trade\nDir.|VVD|Status"
                  + "|Vol(TEU)|Ratio(%)|Assigned AMT"
                  + "|Trade|약정율(%)|Initial AMT|배부율(%)"
	              + "|Before\nAdjustment"
	              + "|Before\nAdjustment"
	              + "|Before\nAdjustment"
                  + "|After Adjustment"
                  ...
                  + "|After Adjustment"
                  + "|Ref";
        var head3 = "|R.Month|Week|Trade|Lane|IOC|Trade\nDir.|VVD|Status"
	              + "|Vol(TEU)|Ratio(%)|Assigned AMT"
	              + "|Trade|약정율(%)|Initial AMT|배부율(%)"
	              + "|Before\nAdjustment"
	              + "|Before\nAdjustment"
	              ...
	              + "|TTL"
	              + "|Ref";
        var head4 = "|R.Month|Week|Trade|Lane|IOC|Trade\nDir.|VVD|Status"
	              + "|Vol(TEU)|Ratio(%)|Assigned AMT"
	              + "|Trade|약정율(%)|Initial AMT|배부율(%)" 
	              + "|To Lane|To Trade Oth|TTL"
	              + "|To Lane|To Trade Oth|Add Cost|TTL"
	              + "|Ref";
        

       //alert(ComCountHeadTitle(head1)+"|"+ComCountHeadTitle(head2)+"|"+ComCountHeadTitle(head3)+"|"+ComCountHeadTitle(head4));
        ...
        with (sheetObj) {
          ...
          InitDataProperty(0, cnt++, dtData,    40, daCenter, true, "m_hul_bnd_cd",         	false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    90, daCenter, true, "m_vvd_cd",                false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    90, daCenter, true, "fnl_hjs_bsa_capa",      	false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    70, daRight,  true, "m_ts_uc_amt",             false, "", dfFloatOrg,   1, false, false);
          ...
          InitDataProperty(0, cnt++, dtData,    50, daCenter, true, "d_trd_cd",               false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    55, daCenter, true, "d_rlane_cd",             false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    40, daCenter, true, "d_ioc_cd",               false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    40, daCenter, true, "d_hul_bnd_cd",        		false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    90, daCenter, true, "d_vvd_cd",               false, "", dfNone,    0, false, false);
          InitDataProperty(0, cnt++, dtData,    45, daCenter, true, "d_locl_ts_sts_cd",       false, "", dfNone,    0, false, false);

          InitDataProperty(0, cnt++, dtAutoSum, 70, daRight,  true, "d_ts_qty",               false, "", dfFloatOrg, 1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 70, daRight,  true, "d_ts_rto",               false, "", dfFloatOrg, 4, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 90, daRight,  true, "d_fx_cost_dtrb_amt",     false, "", dfFloatOrg, 1, false, false);

          InitDataProperty(0, cnt++, dtData,    50, daCenter, true, "dd_trd_cd",              	false, "", dfNone,     0, false, false);
          InitDataProperty(0, cnt++, dtData, 	70, daRight,  true, "d_bsa_cmmt_rto",    	  	false, "", dfFloatOrg, 4, false, false);
          //InitDataProperty(0, cnt++, dtAutoSum, 75, daRight,  true, "d_cmmt_qty",             	false, "", dfFloatOrg, 1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 80, daRight,  true, "d_cmmt_bse_cost_amt",    	false, "", dfFloatOrg, 1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 70, daRight,  true, "d_cmmt_bse_cost_rto",    	false, "", dfFloatOrg, 4, false, false);
          //InitDataProperty(0, cnt++, dtHidden, 	90, daRight,  true, "d_cmmt_fnl_cost_amt",    	false, "", dfFloatOrg, 1, false, false);
          

          InitDataProperty(0, cnt++, dtAutoSum, 80, daRight, 	true, "to_lane_init_amt", 		false, "", dfFloatOrg,	1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 80, daRight,  	true, "to_trd_otr_init_amt",	false, "", dfFloatOrg, 	1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 80, daRight,  	true, "to_init_ttl_amt",    	false, "", dfFloatOrg, 	1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 90, daRight,  	true, "adj_lane_fnl_amt",    	false, "", dfFloatOrg, 	1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 90, daRight,  	true, "adj_trd_otr_fnl_amt", 	false, "", dfFloatOrg, 	1, false, false);
          //InitDataProperty(0, cnt++, dtAutoSum, 80, daRight,  	true, "adj_lane_ttl_amt",    	false, "", dfFloatOrg, 1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 80, daRight,  	true, "d_cmmt_add_cost_amt",    false, "", dfFloatOrg, 1, false, false);
          InitDataProperty(0, cnt++, dtAutoSum, 90, daRight,  	true, "adj_fnl_amt",    		false, "", dfFloatOrg, 	1, false, false);
          InitDataProperty(0, cnt++, dtData,    90, daCenter,  	true, "adj_remark",    			false, "", dfNone, 		0, false, false);
...
function f_seltrade_OnChange(obj,value,text) {
 	...
 	if ("All"!=value) {
		...
		var sXml = sheetObj.GetSearchXml("ESM_COA_0050GS.do", coaFormQueryString(formObj));
...
//		  ComOpenWait(true);
		  ...
		  sheetObj.DoSearch4Post("ESM_COA_0050GS.do", coaFormQueryString(formObj,'param1|param2|param3|param4|param5|param6|param7|param8'));
//		  ComOpenWait(false);
		  sheetObj.WaitImageVisible = false;
		  var BatchStatus = sheetObj.EtcData("BatchStatus");	       
	      switch(BatchStatus){
				case "P"://해당 작업이 진행 중 
					ComShowCodeMessage("COA00003", "TS Allocation Creation");
					break;					
	      }  
	      break;
...
function monitoringBatchJob(){
	...
	var sXml = sheetObj.GetSearchXml("ESM_COA_0050GS.do", FormQueryString(formObj));

...
/**
 * @extends 
 * @class ESM_COA_0159 : ESM_COA_0159 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0159() {
...
function setSheetObject(sheet_obj) {
 ...
 function f_cobtrade_OnChange(obj,value,text) {
 	...
 	if ("All"!=value) {
 		...
 		var sXml = sheetObj.GetSearchXml("ESM_COA_0159GS2.do", coaFormQueryString(formObj));
    ...
    sheetObj.ShowDebugMsg = false;
        ...
        case IBSEARCH:      //조회
            if (validateCond(formObj,sAction)) {
                ...
                sheetObj.DoSearch4Post("ESM_COA_0159GS.do", coaFormQueryString(formObj,'param1|param2|param3|param4'));
                ComOpenWait(false);
            }
            break;
        ...
        case IBSAVE:        // 저장
            if (validateSheet(sheetObj)) {
                if (sheetObj.RowCount > 0) {
                    ...
                    sheetObj.DoAllSave("ESM_COA_0159GS.do", coaFormQueryString(formObj,'f_cmd',true));
                    ComOpenWait(false);
                }
            }
            break;

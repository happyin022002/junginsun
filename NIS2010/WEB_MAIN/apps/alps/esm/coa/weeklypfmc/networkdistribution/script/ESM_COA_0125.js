...
* 2011.05.26 최성민 [CHM-201006017-01] COA 약정율 로직 추가 - 신규 생성 테이블로 변경 Month Copy 기능 추가
...
/**
 * @extends 
 * @class ESM_COA_0125 : ESM_COA_0125 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0125() {
    ...
    try {
        ...
        switch(srcName) {
			...
			case "btn_Month_Copy":		//팝업창(Month Copy)
        	       var display = "0,1";
        	       ComOpenPopup("ESM_COA_0173.do?classId=ESM_COA_0125", 250, 160, "", display, true, false);
        	       break;	
...
function f_cobtrade_OnChange(obj,value,text) {
	...
	if ("All"!=value) {
		...
		var sXml = sheetObj.GetSearchXml("ESM_COA_0125GS.do", coaFormQueryString(formObj));
	...
	with(sheetObj){
		...
		if (colName == "delflag") {
			...
			for(var i=0; i< 14; i++) {
 			...
 			var sXml = sheetObj.GetSearchXml("ESM_COA_0125GS.do", sParam);
 			
 			var arrXml = sXml.split("|$$|");
 			if (arrXml.length > 0) {
 				ComCoaSetIBCombo(sheetObj, arrXml[0], "fm_rlane_cd", true, 0);
 			}
...
function doActionIBSheet(sheetObj,formObj,sAction) {
  ...
  switch(sAction) {
		...
		case IBCLEAR:          //조회
			...
			if (4 < arrXml.length) {
				ComCoaSetIBCombo(sheetObj, arrXml[4], "fm_trd_cd", true, 0);
			}
			if (5 < arrXml.length) {
				ComCoaSetIBCombo(sheetObj, arrXml[5], "fm_rlane_cd", true, 0);
			}
			if (6 < arrXml.length) {
				ComCoaSetIBCombo(sheetObj, arrXml[6], "fm_ioc_cd", false, 0);
			}
			if (7 < arrXml.length) {
				ComCoaSetIBCombo(sheetObj, arrXml[7], "fm_hul_bnd_cd", true, 0);
				ComCoaSetIBCombo(sheetObj, arrXml[7], "to_hul_bnd_cd", true, 0);
			}
      ...
      case IBSEARCH:      //조회
          if (validateCond(formObj,sAction)) {
              ...
              sheetObj.DoSearch4Post("ESM_COA_0125GS.do", coaFormQueryString(formObj));
              ComOpenWait(false);
          }
          break;
      ...
      case IBSAVE:        // 저장
          if (validateSheet(sheetObj)) {
              if (sheetObj.RowCount > 0) {
                  ...
                  //sheetObj.DoSave("ESM_COA_125GS.do", coaFormQueryString(formObj), -1, false);
                  sheetObj.DoSave("ESM_COA_0125GS.do", coaFormQueryString(formObj,'f_cmd',true),"");
                  ComOpenWait(false);
              }
          } 
          break;
...
function getFromTradeDir(sheetObj, Row, Col, Value) {
	...
	with(sheetObj){
		...
		*/		
		...
		var sXml = GetSearchXml("ESM_COA_0125GS.do", sParam); 		

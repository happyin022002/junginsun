...
/**
 * @extends 
 * @class ESM_COA_0067 : ESM_COA_0067 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0067() {
	...
	function openCostDetail(){
		if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
			if(sheetObjects[1].RowCount>0){//sheet2에 데이터가 있을때
				...
				//noRtnPopup('ESM_COA_0066.do?'+ coaFormQueryString(document.form)+cond, 'width=750,height=400,menubar=0,status=0,scrollbars=0,resizable=0');
			} else {
	...
	*/
	...
	function chgOffice(obj){
         ...
         if(obj.Text != ""){
 			...
 			var sXml = sheetObj.GetSearchXml("ESM_COA_0067GS.do", coaFormQueryString(formObj));
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:	//조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0067GS.do", coaFormQueryString(formObj)+"&f_sch_mode=4");
				ComOpenWait(false);
				break;
		...
		switch(sAction) {
			case IBSEARCH:	//조회
				...
				if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
					...
					sheetObj.DoSearch4Post("ESM_COA_0067GS2.do", coaFormQueryString(formObj) + "&f_sch_mode=3");
					ComOpenWait(false);
				} else {
					ComShowMessage(ComGetMsg('COA10005', 'Sheet1'));
				}

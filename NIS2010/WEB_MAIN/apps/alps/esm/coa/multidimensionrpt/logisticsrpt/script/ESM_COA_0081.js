...
/**
* @extends 
* @class ESM_COA_0081 : ESM_COA_0081 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
*/
function ESM_COA_0081() {
		...
		switch(sheetNo) {
			case 1:	//sheet1 init
				with (sheetObj) {
					...
					var HeadTitle = "H|subsum|S.Month|R.Month|Week|RHQ|Control Office|H|Cost Group|H|KPI|Volume (BOX)|Expense|Unit Cost|kpiOrder";
		...
		case IBSEARCH:	//조회
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					if ( sheetObj.id == "sheet1" ) {
						...
						sheetObj.DoSearch4Post("ESM_COA_0081GS.do", coaFormQueryString(formObj));
						//ComOpenWait(false);
					}
						...
						if(sheetObjects[0].RowCount>0){//sheet1에 데이터가 있을때
							...
							//20100415 이중환, FormQueryString -> coaFormQueryString 변경
							sheetObj.DoSearch4Post("ESM_COA_0081GS2.do", coaFormQueryString(formObj));
						} else {

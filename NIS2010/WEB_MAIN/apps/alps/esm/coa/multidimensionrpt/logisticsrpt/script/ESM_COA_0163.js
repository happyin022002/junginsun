...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0163 : ESM_COA_0163 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0163() {
    ...
    document.onclick = processButtonClick;
    	...
    	*/
    	function sheet1_OnSearchEnd(sheetObj, errMessge) {  
    		if(sheetObj.RowCount < 1) {
    			ComShowMessage(ComGetMsg("COA10062"));
    		}
    		...
    		if(sheetObj.FindText("r_rmk","Null") != -1) {
    			...
    			ComShowMessage(ComGetMsg("COA10059"));
    		}
            ...
            switch(sAction) {
                case IBSEARCH:	//조회
                	if(validateForm(sheetObj,formObj,sAction)) {
                		...
                		sheetObj.DoSearch4Post("ESM_COA_0163GS.do", coaFormQueryString(formObj));
                		ComOpenWait(false);
                	}
                	break;
    	...
    	*/
    	function validateForm(sheetObj,formObj,sAction){
    		with(formObj){
    			if(f_from.value =="") {
    			   ComShowMessage(ComGetMsg('COA10002', 'From'));
    			    ComSetFocus(formObj.f_from);
    			    return false;
    			}
    			if(f_to.value =="") {
    			    ComShowMessage(ComGetMsg('COA10002', 'To'));

 ...
 *  2008.09.18 김태윤 N200808278919 Spilt 01-COA_Report내 수정사항 (searchBKG061List, searchBkgAbcstp156List 쿼리수정)
...
/**
 * @extends 
 * @class ESM_COA_0156 : ESM_COA_0156 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0156() {
		...
		switch(sheetNo) {
			...
			case 3:		//sheet3 init
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtData,		150,	daLeft,	    true,	"coa_cost_src_cd_nm");
...
//					RangeBackColor(1, 6, 1, 7) = RgbColor(222, 251, 248);
	...
	*/
	function popupAbcStpUnitCost(){
		var param = '?pgmNo=ESM_COA_0012&f_bkg_no='+document.form.f_bkg_no.value+'&pro_vw='+document.form.f_pro_vw.value;
		//ComOpenWindow('ESM_COA_0012.do?'+param, 'ABC/STP Unit Cost', 'width=900,height=540,menubar=0,status=1,scrollbars=1,resizable=1');
		ComOpenWindow('ESM_COA_0012.do' + param, 'Shipper Pop', 'width=1020,height=480,menubar=0,status=1,scrollbars=1,resizable=1');
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0156GS2.do", coaFormQueryString(formObj));
...
//					sheetObj.DoSearch4Post("ESM_COA_156GS.do", coaFormQueryString(formObj));
					var xml = sheetObjects[0].GetSearchXml("ESM_COA_0156GS.do", coaFormQueryString(formObj));

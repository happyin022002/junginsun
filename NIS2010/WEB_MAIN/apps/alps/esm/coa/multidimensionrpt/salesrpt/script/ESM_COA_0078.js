...
* 2008.06.03 PEJ N200805300001 COA_화면 개발 및 수정
...
* 2009.02.04 김태윤 N200901190016 - COA_조직개편 관련 기능 수정
...
* 2011.06.23 최성민 [CHM-201111639-01] [COA]CNEE 조회 기능 추가_Office Report by Daily BKG Creation
...
/**
 * @extends
 * @class ESM_COA_0078 : ESM_COA_0078 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0078() {
...
//                    ColHidden("opcost") = true;
//                    ColHidden("cmcost") = true;
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            ...
            case IBSEARCH:          // 첫번째 Sheet 조회
                ...
                sheetObj.DoSearch4Post("ESM_COA_0078GS.do", coaFormQueryString(formObj));
                ComOpenWait(false);
                break;
            ...
            case IBSEARCHAPPEND:  // 두번째 Sheet조회
                ...
                if(ComGetObjValue(formObj.f_pro_vw) == "R") sheetObj2.DoSearch4Post("ESM_COA_0078GS2.do", coaFormQueryString(formObj));
                ComOpenWait(false);
                break;
    ...
    function sheet1_OnDblClick(sheetObj, row, col, value){
        ...
        if(formObj.f_isbkg.checked){
            param =  "?f_pro_vw="+getComboObjValue(formObj.f_pro_vw)
                    ...
                    +"&pgmNo=ESM_COA_0061";
            ComOpenWindow('ESM_COA_0061POP.do' + param, 'Inquiry by BKG'
...
//        ComShowMessage(rowArray.length+ " : " + colArray[0]+":"+colArray[1]+":"+colArray[2]+":"+colArray[3]+":"+colArray[4]+":"+colArray[5]+":"+colArray[6]+":"+colArray[7]+":"+colArray[8]);
    ...
    function validateForm(sheetObj,formObj,sAction){
        with(formObj){
			if(formObj.f_fm_date.value == "" && formObj.f_to_date.value == ""){
			    // [COA10002] : Period을(를) 입력하세요..
			    ComAlertFocus(formObj.f_fm_date, ComGetMsg("COA10002", "Period"));
			    return false;
			}
			if(formObj.f_vsl_cd.value != "" && formObj.f_skd_voy_no.value != "" && formObj.f_dir_cd.value !="" ) {
			    if(ComGetDaysBetween(formObj.f_fm_date.value,formObj.f_to_date.value) > 31){
			        ...
			        // ['COA10008'] : 1 Month 을(를)  초과할수 없습니다.
			        ComAlertFocus(f_to_date, ComGetMsg("COA10008", "1 Month"));
			        return false;
			    }
			} else {
    			if( parseInt(getComboObjValue(formObj.f_rhq_cd)) < 3){
    			    if(ComGetDaysBetween(formObj.f_fm_date.value,formObj.f_to_date.value) > 0){
    			        // ['COA10008'] : 1 Days 을(를)  초과할수 없습니다.
    			        ComAlertFocus(f_to_date, ComGetMsg("COA10008", "1 Days"));
    			        return false;
    			    }
    			} else {
    			    if(ComGetDaysBetween(formObj.f_fm_date.value,formObj.f_to_date.value) > 6){
    			        // ['COA10008'] : 7 Days 을(를)  초과할수 없습니다.
    			        ComAlertFocus(f_to_date, ComGetMsg("COA10008", "7 Days"));
    			        return false;
    			    }
    			}
     ...
     */
     function f_trd_cd_OnChange(obj) {
     	...
     	if (obj.Text != "") {
     		...
     		var sXml = sheetObj.GetSearchXml("ESM_COA_0078GS.do", coaFormQueryString(formObj));
     		var arrXml = sXml.split("|$$|");
     		if (arrXml.length > 0)
    ...
    function chgViewColumn(){
        ...
        if(getComboObjValue(formObj.f_pro_lvl) == "C" || getComboObjValue(formObj.f_pro_lvl) == "M"){
            ...
            if(getComboObjValue(formObj.f_pro_vw) == "P"){
                ComShowMessage(ComGetMsg("COA10056"));

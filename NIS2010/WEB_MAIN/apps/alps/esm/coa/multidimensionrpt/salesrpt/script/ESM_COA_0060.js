...
* 2008.06.30 박은주 N200806127354 COA_조회권한 관련 요청
...
* 2009.04.03 김태윤 N200903170123 - COA_Multi-dimension report 조회권한 변경, 디자인 수정
...
* 2010.09.01 김기종 CSR No. CHM-201004982-01 COA Architecture 위배사항 수정
...
* 2012.11.13  원종규 [CHM-201221335]	[COA] Inquiry by customized condition 기능 관련: SELCDA일 경우 BKG_No 헤더 선택 시 제한 조건을 적용하지 않도록 변경
* 2013.01.11 서미진 [CHM-201322375] Period 조회시, 주차만 셋팅하여 default 값에 년도와 주차 pair가 맞지 않는 부분 수정
* 2013.01.21 성미영 [CHM-201322531] [COA] Inquiry by Customized Condition 버튼 수정: GA, RA check Box 추가 
* 2013.05.29 김수정[CHM-201324876]   [COA] COA Report내 "IAS Region " / "Bound2" 추가
* 2013.07.01 최성민[CHM-201325476] [COA] Inquiry by Customized Condition 검색 기간 변경
...
/**
 * @extends
 * @class ESM_COA_0060 : ESM_COA_0060 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESM_COA_0060() {
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                    ...
                    if(formObj.f_include_ts.checked) {
                        colTotNum = colCnt + 21 + varCnt ;                  
                    } else {
                        colTotNum = colCnt + 20 + varCnt ;                    	
                    }
                    ...
                    if(formObj.f_view_tpsz[0].checked) {
                        HeadTitle = HeadTitle + "BOUND|TP/SZ|LOAD|REV|CM COST|CM|CM2 COST|CM2|OP COST|BKG OP|RPB(BOX)|CM_COST|CM CPB(BOX)|CMB(BOX)|OP_COST|OP CPB(BOX)|OPB(BOX)" ;
                    } else {
                        HeadTitle = HeadTitle + "BOUND|TP/SZ|LOAD|REV|CM COST|CM|CM2 COST|CM2|OP COST|BKG OP|RPB(TEU)|CM_COST|CM CPB(TEU)|CMB(TEU)|OP_COST|OP CPB(TEU)|OPB(TEU)" ;
                    }
    ...
    function sheet1_OnDblClick(sheetObj, row, col, value){
        if(sheetObj.SaveNameCol("BKG_NO")>=0 ){
            ...
            var param = "?f_pro_vw="  +getIbComboObjValue(formObj.f_pro_vw)
                       ...
                       +"&f_s_bkg_no="+bkg_no
                       +"&pgmNo=ESM_COA_0061";
			ComOpenWindow2('ESM_COA_0061POP.do'+param,'Inquiry by BKG', 'width=850,height=760,menubar=0,status=1,scrollbars=1,resizable=1');
        }
    }
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
        	case IBCLEAR:          //조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_COA_0060GS.do", FormQueryString(formObj));
				*/
...
//				if(formObj.f_excel.value == "N"){
	                ...
	                var xml = sheetObj.GetSearchXml("ESM_COA_0060GS.do", sParam);
...
//	                formObj.f_shipper.value = formObj.txtShipper.value;
//	                var sXml = sheetObj.GetSaveXml("ESM_COA_0060GS.do", coaFormQueryString(formObj));
...
//				}
            ...
            case IBRESET:          //Header 정보를 조회한다.
                ...
                //sheetObj.DoSearch4Post("ESM_COA_0060GS.do", FormQueryString(formObj));
                //20100414 이중환, FormQueryString -> coaFormQueryString 변경
                sheetObj.DoSearch4Post("ESM_COA_0060GS3.do", coaFormQueryString(formObj));
                //ComOpenWait(false);
    ...
    function validateForm(sheetObj,formObj,sAction){
        with(formObj){
            ...
            if(f_chkprd[1].checked){
                ...
                if ( f_to_mon.value - f_fm_mon.value > 1 ) {
                    // [COA10038] = Duration 은 2 개월 미만이어야 합니다.
            ...
            if(f_chkprd[0].checked ){
                ...
                if ( f_to_wk.value - f_fm_wk.value > 9 ) {
                    // [COA10038] = Duration 은 10 주 미만이어야 합니다.
                    ComShowMessage(ComGetMsg("COA10038","10 Weeks"));
                    f_to_wk.focus();
                    return false;
                }
            }
            ...
            if(getIbComboObjValue(ofc_lvl) == '1' && usr_ofc_cd != "SELCSG"){
                if(parseInt(getIbComboObjValue(f_rhq_cd)) == 1){ //Head Office 일때 Trade 선택하도록
                      if(sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0){
                    	   /* [CHM-201325476] BKG_NO, BL_NO 조회할 경우 MON:2개월 미만, WK: 10미만
                    	   if(f_chkprd[1].checked){
                               if ( f_to_mon.value - f_fm_mon.value > 1 ) {
                                   // [COA10038] = Duration 은 2 개월 미만이어야 합니다.
                                   ComShowMessage(ComGetMsg("COA10038","2 Months"));
...
//                         if(getIbComboObjValue(f_rlane_cd) == ""){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
...
//                         }
                ...
                if(parseInt(getIbComboObjValue(f_rhq_cd)) == 2  ){ //Regional Group 일때, OFC 선택하도록
                      if(sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0 ){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
                          if(getIbComboObjValue(f_sls_ofc_cd) == "" && getIbComboObjValue(f_trd_cd) == ""){
                             ComShowMessage(ComGetMsg('COM12114','Trade'));
                             ComSetFocus(f_trd_cd);
                             return false;
                          }
                          if(getIbComboObjValue(f_sls_ofc_cd) == "" && getIbComboObjValue(f_rlane_cd) == ""){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
                              ComShowMessage(ComGetMsg('COM12114','Lane'));
                              ComSetFocus(f_rlane_cd);
                              return false;
                          }
                      }else{
                    	  if(getIbComboObjValue(f_sls_ofc_cd) == "" && getIbComboObjValue(f_rlane_cd) == ""){
                    		  // Excel Down
                    		  formObj.f_excel.value = "Y";
                    	  }
                      }
                ...
                if(parseInt(getIbComboObjValue(f_rhq_cd)) == 3  ){ //Sub Regional Group 일때, Trade 선택하도록
                      if(sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0 ){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
                ...
                if(parseInt(getIbComboObjValue(f_rhq_cd)) == 4  ){ //Area 일때, Trade 선택하도록
                      if(sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
                ...
                if(parseInt(getIbComboObjValue(f_rhq_cd)) == 5  ){ //Sales Office 일때, Trade 선택하도록
                      if(sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0){    //헤더에 BL_NO포함시 Lane 필수 선택하도록
            ...
            }else if(usr_ofc_cd == "SELCSG"){
            	if(getIbComboObjValue(f_trd_cd) == "" && (sheetObj.SaveNameCol("BKG_NO")>=0 || sheetObj.SaveNameCol("BL_NO")>=0)){
             	   /* [CHM-201325476] BKG_NO, BL_NO 조회할 경우 MON:1개월 미만, WK: 1주미만 */
             	   if(f_chkprd[1].checked){
//                        if ( f_to_mon.value - f_fm_mon.value > 0 ) {
//                            // [COA10038] = Duration 은 1 개월 미만이어야 합니다.
//                            ComShowMessage(ComGetMsg("COA10038","1 Month"));
//                            formObj.f_to_mon.focus();
//                            return false;
//                        }
             		  ComShowMessage(ComGetMsg("COA10038","1 Week"));
             		  f_chkprd[1].focus();
             		  return false;
                    } else if(f_chkprd[0].checked ){
                        if ( f_to_wk.value - f_fm_wk.value > 0 ) {
                            // [COA10038] = Duration 은 1 주 미만이어야 합니다.
                            ComShowMessage(ComGetMsg("COA10038","1 Week"));
                            f_to_wk.focus();
                            return false;
                        }
...
//            if(!ComChkObjValid(f_to_wk, null, null, "yw")) return false;
    ...
    function chgGroup(param){
		...
		var sXml = sheetObj.GetSearchXml("ESM_COA_0060GS.do", coaFormQueryString(formObj));
		var arrXml = sXml.split("|$$|");
		if (arrXml.length > 0)
     ...
     function f_selgroup_OnChange(obj, code){
    	...
    	// if(code != "" && document.form.f_include_ts.checked) {
    	//	 document.form.f_include_ts.checked = false;
    	// }
...
//	    				formObj.f_mt_pu_yd_cd.Index = 0;
    ...
    function openRPTFormPopUp(){
    	...
    	ComOpenWindow2('ESM_COA_0059.do'+param, '', 'width=600, height=440, menubar=0, status=1, scrollbars=0, resizable=0');
    ...
    function getBackEndJobStatus() {
        ...
        if (jobState == "3") {
            if (formObj.f_excel.value == "Y") {    // File Download
                ...
                document.location.href = "ESM_COA_0060DL.do?" + coaFormQueryString(formObj);

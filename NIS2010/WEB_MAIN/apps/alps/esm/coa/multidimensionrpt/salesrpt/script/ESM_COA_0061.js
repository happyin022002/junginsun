...
* 1.0 Creation
* History :
* 2008.03.26 PEJ   N200803190021 COA inquiry by BKG 화면 및 excel down 변경 요청  
...
* 2009.12.31 윤진영 CHM-200901869 COA_BKG_COST_ACT_GRP_SMRY SRC Detail과 ABC/STP 테이블 6개월 유지를 위한 INDEX 추가
*            COA_BKG_COST_SRC_DTL,COA_BKG_SVC_TRNS_PRC 해당 Summary TABLE Summary로직을 BATCH PROCESS에 추가
...
* 2013.11.11 김수정 [CHM-201327408] [COA] Inquiry By Booking MT OP/RTN 정보 추가
=========================================================*/
/****************************************************************************************
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESM_COA_0061 : ESM_COA_0061 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESM_COA_0061() {
    ...
    /* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
    		...
    		switch(sheetNo) {
    			...
    			case 3:		//sheet3 init
    				with (sheetObj) {
    					...
    					var HeadTitle  = "Node Link|Node Link|Activity Group|Activity Group|Cost Group/Cost Element|Cost Group/Cost Element|Feeder Term|Feeder Term|Amount|avg_lvl_chk" ;
    					var HeadTitle1 = "Node Link|Node Link|Activity Group|Activity Group|Cost Group/Cost Element|Cost Group/Cost Element|Rev_Term|Del_Term|Amount|avg_lvl_chk" ;
    					InitHeadRow(0, HeadTitle, true);
    					InitHeadRow(1, HeadTitle1, false);
    	...
    	*/
    	function chgOption() {
    		...
    		sheetObject3.DoSearch4Post("ESM_COA_0061GS2.do", coaFormQueryString(formObj));
    	}
    	/**
    	* hidden값 변경<br>
    	*
...
//                ComSetObjValue(formObj.f_pro_lvl,"C");             
    	...
    	*/
    	function btng_remarks_OnClick(sheetObj){
        	...
        	+ coaFormQueryString(document.form), '_RMK', 'width=780,height=680,menubar=0,status=0,scrollbars=1,resizable=1');
    	}
    	...
    	*/
    	function sheet1_OnDblClick(sheetObj , row, col , value) {
            ...
            //20100414 이중환, FormQueryString -> coaFormQueryString 변경
    		var sXml = sheetObject2.GetSearchXml("ESM_COA_0061GS2.do", coaFormQueryString(formObj));
    	...
    	function doActionIBSheet(sheetObj,formObj,sAction) {
    		...
    		switch(sAction) {
    			...
    			case IBCLEAR: //조회
					...
					var sXml = sheetObj.GetSearchXml("ESM_COA_0061GS.do", coaFormQueryString(formObj));
    			...
    			case IBSEARCH:		//조회
    				if(validateForm(sheetObj,formObj,sAction))
    				{
    					if ( sheetObj.id == "sheet1" ) {
    						...
    						formObj.f_cmd.value = SEARCHLIST01;
    		                //20100414 이중환, FormQueryString -> coaFormQueryString 변경
    						sheetObj.DoSearch4Post("ESM_COA_0061GS.do", coaFormQueryString(formObj));
    					    // Form Data setting
    						//------------------------------------
    		                ...
    		                //20100414 이중환, FormQueryString -> coaFormQueryString 변경
    						var sXml = sheetObj.GetSearchXml("ESM_COA_0061GS2.do", coaFormQueryString(formObj));
    						...
    						formObj.f_cmd.value = SEARCHLIST03;
    		                //20100414 이중환, FormQueryString -> coaFormQueryString 변경
    						sheetObj.DoSearch4Post("ESM_COA_0061GS2.do", coaFormQueryString(formObj));
    						ComOpenWait(false);
    					}
    				}

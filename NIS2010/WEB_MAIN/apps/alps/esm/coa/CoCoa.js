 ...
 * 2011.03.02 김상수 [CHM-201109144-01] js상의 validation함수 정리 및  coCoa.js로 소스이동
 * 2011.03.18 김상수 [CHM-201109282-01] Split 04-ALPS의 Location 조회불가건 수정 보완 요청
 *                                      - 공통메소드중 ESM_COA_0029에서 호출되지 않아야 할 기능 수정
 * 2011.05.26 최성민 [CHM-201006017-01] COA 약정율 로직 추가 - coaPeriod() 함수 수정
 * 2011.06.02 최성민 [CHM-201111115-01] Split 02-Split 03-ALPS Error 처리 요청  -  메세지 추가(COA00001~)
 * 2011.06.22 김민아 [CHM-201111640-01] chkWM()에서 ESM_COA_0060 UI인 경우 해당 js에서 처리하는 로직 추가
 * 2011.07.01 이석준 [CHM-201111498-01] 메세지 추가,COA10060,COA00003
 * 2011.10.07 최성민 [CHM-201113373-01] 메세지 추가,COA10061
 * 2011.11.01 최성민 [CHM-201114173-01] 메세지 추가,COA10062
 * 2012.01.02 최성민 [CHM-201114896-01] ComTextCode2ComboItem() 추가 * 
 * 2012.01.31 김종준 [CHM-201215754-01] COA10063,COA10064 메세지 추가 
 ...
 * 2012.01.02 최윤성 [CHM-201222075-01] 메세지 추가,COA10067
...
*=========================================================*/
    ...
    msgs['COA10060'] = 'Zero Amount can not be saved!!.';
    msgs['COA10061'] = 'Creation is not allowed before WK 40, 2011 due to system logic change.';
    msgs['COA10062'] = 'If a route cost is not displayed(found), please check the Pre CM/OP Simulation menu.\nIf the problem continues, please contact NOA for further assistance.';
    msgs['COA10063'] = 'Target VVD Sent to FCM - Completed';
    msgs['COA10064'] = 'Do you want to VVD Send?';
    msgs['COA10065'] = 'Please check one. RF only or Special only';
    msgs['COA10066'] = '{?msg1} Request is submitted.\nPlease wait for {?msg2} minutes to create data.';
    msgs['COA10067'] = 'Please check one.\nCore Customer or Regional Customer or Local Customer.';
    msgs['COA10068'] = '{?msg1} must be inputted';
    msgs['COA10069'] = 'Average RPB was created. \n\n Do you want to create data?';
    msgs['COA10070'] = 'Data will be changed, will you continue ?';
//    msgs['COA10071'] = '{?msg1} is invalid Vessel.';
//    msgs['COA10072'] = 'Source week can\'t be preceded by target week \nor shouldn\'t be the same as target week.';
    msgs['COA10073'] = 'Your booking is under Network Constraint in PRD.\nLocation (USIWS : DEL) - ITEM(Others) - Remark((COE\'s Request)\nThis location is not allowed for import due to reduction of M/B cost.)\n- SVC(N)';
   ...
   */
    ...
    function selectDownExcelMethod(sheetObj){
        ...
        var rtn = window.showModalDialog("ESM_COA_3002.do", "", sFeature);
        return rtn;
    }
...
function chkWM(param1, param2) {
    if (param1 == 'W') {
        //ESM_COA_0029 UI가 아니고 sls_yrmon컬럼과 cost_yrmon컬럼이 둘 다 존재할때
        if (location.pathname.toUpperCase().indexOf("ESM_COA_0029") == -1 && sheetObjects[0] == "[object]" && sheetObjects[0].SaveNameCol("sls_yrmon") != -1 && sheetObjects[0].SaveNameCol("cost_yrmon") != -1) {
            //ESM_COA_0060 UI인 경우 해당 js에서 처리
        	if(location.pathname.toUpperCase().indexOf("ESM_COA_0060") != -1){
            	viewWeek();
            }else{
        ...
        if(f_chkprd[0].checked) {
            if (f_fm_wk.value == ""  || f_to_wk.value == "") {
                ...
                if (!ComIsWeek(f_fm_wk)){
                    // [COA10009] = 'Please enter Week correctly.\n\n Format : WW
            ...
            if (f_fm_mon.value == "" || f_to_mon.value == "") {
                ...
                if(!ComIsMonth(f_fm_mon)){
                    // [COA10009] = 'Please enter Month correctly.\n\n Format : MM
 ...
 * initPeriod_ofc->coaPeriod1_ofc
 */
function coaPeriod1_ofc(){
    document.write("\n <!--  -------------------------------------------------------------------------------------------- --> ");
    document.write("\n <table border='0'> ");
    document.write("\n     <tr class='h23'> ");
    document.write("\n         <td width='50'>&nbsp;&nbsp;W/M</td> ");
 ...
 * initPeriod2_ofc ->coaPeriod2_ofc
 */
 function coaPeriod2_ofc(){
    document.write("\n <!--  -------------------------------------------------------------------------------------------- --> ");
    document.write("\n <table border='0'> ");
    document.write("\n     <tr class='h23'> ");
    document.write("\n         <td width='50'>&nbsp;&nbsp;W/M</td> ");
     ...
     * initPeriod3_ofc ->coaPeriod3_ofc
     */
function coaPeriod3_ofc(){
    document.write("\n <!--  -------------------------------------------------------------------------------------------- --> ");
    document.write("\n <table border='0'> ");
    document.write("\n     <tr class='h23'> ");
    document.write("\n         <td width='50'>&nbsp;&nbsp;W/M</td> ");
...
//          alert(plain_text2);
     ...
     * @version 2010.08.01
     */
    function ComCoaSetIBCombo(sheetObj, sXml, title, iBlank, sCol,iRow, dCode, dText, bFlag){
        var showCol = 0;
    ...
    function ComCoaSetPeriod4(obj){
      ...
      */
     function ComCoaSetPeriod5(obj,divName){
         var formObj = document.form;
         var sheetObj = sheetObjects[0];
         if (null==obj) return;
     ...
     /**
       * Location을 Check한다.
       */
      function ComCoaCheckLocCd(f_type_cd, obj, f_d_term, f_tml_cd, f_pol_pod_cd, spcl_cgo_cd){
           var formObj = document.form;
           var sheetObj = sheetObjects[0];
      ...
      function chkFormat(obj){
          ...
          if(formObj.txtShipper.value.length > 2 && !ComIsNumber((formObj.txtShipper.value).substring(2))){
        	  ComShowMessage(ComGetMsg('COA10009', 'Shipper', 'AA123456'));

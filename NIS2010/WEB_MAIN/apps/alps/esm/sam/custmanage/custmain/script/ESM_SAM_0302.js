 ...
 ***************************************************************************************/
	...
	function processButtonClick() {
		...
		try {
			...
			/*case "btn_CheckDup":
				...
				var rtn=ComOpenPopup('/hanjin/ESM_SAM_0304.do?' + param, 780, 470, '', '0,0', true); 
				break;*/
			...
			case "btn_com_ens_0M1": // country pop-up
             ...
             case "btn_com_ens_n13": // currency pop-up
   				...
   				break;
 			case "btn_esm_sam_0301": //group customer pop-up
				var param="";	
	    		param=param + "cust_grp_id=" + form.cust_grp_id.value;	    		
	    		ComOpenPopup('/hanjin/ESM_SAM_0301.do?' + param, 780, 430, 'setCustGrpId', '1,0,1,1,1,1,1,1', true);				
				break;
	...
	function loadPage() {
	   ...
	   formObj.indiv_corp_div_cd.Enable=false;
	   ...
	   formObj.indus_desc.disabled = true;
	   formObj.cust_locl_lang_nm.disabled = true;
	   formObj.cust_abbr_nm.disabled = true;
    ...
    function initControl() {
	        ...
	        case "sheet1":   //sheet1 init
	            with (sheetObj) {
	              ...
	              //전체Merge 종류 [선택, Default msNone]
					...
					InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "id_npwp_rgst_no");
	 ...
	 /**
	 * ESM_SAM_0302 콤보 데이타를 가져온다.
	 **/
	 function initComboSetVal(sheetObj,formObj){
	 	formObj.f_cmd.value = SEARCH01;
		var sXml = sheetObj.GetSearchXml("ESM_SAM_0302GS.do", FormQueryString(formObj));
		     ...
		     switch(sAction) {
				...
				case MULTI:        //저장
					...
					if( formObj.creflag.value == "Y" && formObj.cust_cd.value == "" ) {
						...
						ComOpenPopup('/hanjin/ESM_SAM_0304.do?' + param, 780, 470, "getESM_SAM_0302_saveflag", '0,0', true);
					} else {
				...
				case MULTI01:        //저장
					...
					var SaveXml = sheetObj.GetSaveXml("ESM_SAM_0302GS.do", sParam);
		...
		//			formObj.cust_cnt_cd.readOnly = false;
		    	...
		    	case SEARCH13:      //User MDM Auth check
					if(validateForm(sheetObj,formObj,sAction)){
						...
						var sParam=FormQueryString(formObj);
						var sXml=sheetObj.GetSearchXml("ESM_SAM_0309GS.do", sParam);
				        var result=ComGetEtcData(sXml, "result");
				        if(result=="N"){
				...
				/*case SEARCH05:      //Vendor Code check
					if(validateForm(sheetObj,formObj,sAction)){
						...
						var sParam=FormQueryString(formObj);
						var sXml=sheetObj.GetSearchData("ESM_SAM_0302GS.do", sParam);
				        var result=ComGetEtcData(sXml, "result");
				        if(result==""){
...
/*		function t2bsheet1_adress_OnValidation(sheetObj, rowIdx, colIdx, value) {
       ...
       */
	     ...
	     * Call Back Function  
	     */
	   function getESM_SAM_0302_saveflag(saveFlag) {
			var formObj   = document.form;
 		    var confMsg;
			var saveMsg;
...
//	      		event.srcElement.focus();
		...
		function t2bsheet1_keyman_OnPopupClick(sheetObj, row, col){
			...
			switch(col_name) {
				...
				case "KEYMANPop":
					var goUrl = "";
					goUrl = "/hanjin/ESM_SAM_0308.do?";
					if (sheetObj.CellValue(row,"ibflag")!="I"){
						ComOpenWindowCenter(goUrl+"&keyman_seq="+sheetObj.CellValue(row,"keyman_seq"),"ESM_SAM_0308",800,380,true);
					}
...
/*	    function t2bsheet1_keyman_OnDblClick(sheetObj, row, col) {
	    	var formObj=document.form;
				...
				if ( keyman_seq != "" ) {	
		    		...
		    		ComOpenWindowCenter("/hanjin/ESM_SAM_0308.do?"+param, "ESM_SAM_0308", 800, 380, true, "yes");
				}
	    }*/
		...
		function searchValidationData(sheetObj, val_type, val_value, row) {
	    	...
	    	if (val_type == "Sales Rep Code") {
		    	...
		    	sXml=sheetObj.GetSearchXml("ESM_SAM_0302GS.do", sParam);
...
//	            				document.form.mlt_trd_acct_flg.focus();
	    ...
	    function formSettingVal(sheetObj,formObj,sXml){
			...
			formObj.id_npwp_rgst_no.value			= sheetObj.CellValue(1, "id_npwp_rgst_no");

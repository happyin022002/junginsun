...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
     ...
     * @author 
     * @extends 
     * @class esm_sam_0307  생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
	function esm_sam_0307() {
	...
	function processButtonClick() {
		...
		try {
			...
			switch (srcName) {
				...
				case "btn_Create":
					form.ibflag.value="I";
					ComOpenPopup('/hanjin/ESM_SAM_0308.do?' + param, 800, 380, '', '1,0,1,1,1,1,1,1', true);
					break;
				case "btn_BKGList":
	...
	function doActionIBSheet(sheetObj, formObj, sAction, Row, Col) {
		...
		switch (sAction) {
			case IBSEARCH: // 조회
				...
				var sXml = sheetObj.GetSearchXml("ESM_SAM_0307GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
	...
	**/
	...
	function fncT2SetDataClick(){
	    ...
	    form.ibflag.value="U";
	    ComOpenWindowCenter(goUrl+encodeURI(clickParam),"ESM_SAM_0308",800,380,true);

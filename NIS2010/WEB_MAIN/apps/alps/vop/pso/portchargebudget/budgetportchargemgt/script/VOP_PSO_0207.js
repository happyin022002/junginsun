...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class vop_pso_0207 : vop_pso_0207 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function vop_pso_0207() {
          ...
          switch(sheetid) {
    				...
    				case "sheet1":
    					with (sheetObj) {
    							...
    							var HeadTitle1 = "|Actual Month|Revenue Month|Account Code|Revenue lane|Port|Revenue VVD|Estimate Cost|Actual Cost|Accrual Cost|SYS_SRC_ID|ESTM_SEQ_NO";
				...
				case IBSEARCH: //조회
					if (validateForm(sheetObj, formObj, sAction)){
						if (sheetObj.id == "sheet1") {
							...
							sheetObj.DoSearch("VOP_PSO_0207GS.do", FormQueryString(formObj)
				...
				case IBSAVE: //저장
					if (validateForm(sheetObj, formObj, sAction)){
						if (sheetObj.id == "sheet1") {
							...
							sheetObj.DoSave("VOP_PSO_0207GS.do", FormQueryString(formObj)
									+ "&" + ComGetPrefixParam("sheet1_"));
							ComOpenWait(false);
						}
					}

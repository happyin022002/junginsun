...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtCombo, 		100, daLeft,   false,	"stv_dmg_cate_cd",	true,	"",	dfNone, 0, true, true);
...
function validateForm(sheetObj, formObj, sAction) {
	...
	if (sAction == IBSAVE) {
		...
		for ( var ir = 1; ir <= sheetObj.LastRow; ir++) {
			var bfCode = sheetObj.CellValue(ir, "stv_dmg_cate_cd") + ":"
					+ sheetObj.CellValue(ir, "stv_dmg_cd");
			if (bfCode != "") {
				for ( var jr = 1; jr <= sheetObj.LastRow; jr++) {
					var afCode = sheetObj.CellValue(jr, "stv_dmg_cate_cd")
							+ ":" + sheetObj.CellValue(jr, "stv_dmg_cd");

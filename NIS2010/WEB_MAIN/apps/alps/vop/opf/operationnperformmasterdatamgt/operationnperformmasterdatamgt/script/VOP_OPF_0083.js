...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class vop_opf_0083 : vop_opf_0083 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
                ...
                case "sheet2":
                    with (sheetObj) {
                       ...
                       //전체Edit 허용 여부 [선택, Default false]
						...
						InitDataProperty(0, cnt++ , dtData,	50,	daCenter,	true,	prefix+"rstwg_rsn_cd",			false,	"",  dfEngUpKey,	0,	false,	true, 2);
                ...
                case "sheet3":
                    with (sheetObj) {
                       ...
                       //전체Edit 허용 여부 [선택, Default false]
						...
						InitDataProperty(0, cnt++ , dtData,	50,	daCenter,	true,	prefix+"rstwg_cd_tp_cd",	false,	"",  dfNone, 0,	true,	true);
						InitDataProperty(0, cnt++ , dtData,	170,daLeft,		true,	prefix+"shifting_method",	false,	"",  dfNone, 0,	true,	true);
						InitDataProperty(0, cnt++ , dtData,	50,	daCenter,	true,	prefix+"rstwg_rsn_cd",			false,	"",  dfEngUpKey,	0,	true,	true, 2);
						InitDataProperty(0, cnt++ , dtData,	170,daLeft,		true,	prefix+"rstwg_rsn_cd_full_desc",false,	"",  dfEngDnKey,	0,	true,	true, 500);
        	...
        	if(sheetObj.RowCount>0){
        		...
        		for(i=1; i< (sheetObj.RowCount+1); i++){
        			for(j=i+1; j< (sheetObj.RowCount+1); j++){
            			...
            			if(sheetObj.CellValue(i,"sheet2_rstwg_rsn_cd") != null 
		...
		function downExcel(sheetObj1, sheetObj2, sheetObj3){
			...
			for(var idxRow = sheetObj2.HeaderRows; idxRow <= sheetObj2.LastRow; idxRow++){
				...
				if(idxRow <= sheetObj1.LastRow){
					sheetObj3.CellValue(insRow, prefix3 +"rstwg_cd_tp_cd") = sheetObj1.CellValue(idxRow, prefix1 +"rstwg_cd_tp_cd");
					sheetObj3.CellValue(insRow, prefix3 +"shifting_method") = sheetObj1.CellValue(idxRow, prefix1 +"shifting_method");
				}

				sheetObj3.CellValue(insRow, prefix3 +"rstwg_rsn_cd") = sheetObj2.CellValue(idxRow, prefix2 +"rstwg_rsn_cd");
				sheetObj3.CellValue(insRow, prefix3 +"rstwg_rsn_cd_full_desc") = sheetObj2.CellValue(idxRow, prefix2 +"rstwg_rsn_cd_full_desc");
			}

			sheetObj3.Down2Excel(1, false, false, true, "", "", false, false, "",
									 false, prefix3 + "ibflag", 
									"", false, false, "");
		}

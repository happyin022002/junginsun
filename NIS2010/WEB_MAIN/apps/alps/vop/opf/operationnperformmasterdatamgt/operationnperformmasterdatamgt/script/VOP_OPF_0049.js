...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
			...
			InitDataProperty(0, cnt++, dtData, 			250, daLeft,	true,	"stv_dmg_cd_desc", false, "", dfNone, 0, true, true, 500);
			InitDataProperty(0, cnt++, dtCombo, 		100, daLeft, 	true,	"stv_dmg_cate_cd", true, "", dfNone, 0, true, true);
	...
	case IBSEARCH: //조회
		if (validateForm(sheetObj, formObj, sAction)) {
			if (sheetObj.id == "sheet1") {
				...
				sheetObj
						.DoSearch("VOP_OPF_0049GS.do", FormQueryString(formObj));
			}
		}
		break;
...
function validateForm(sheetObj, formObj, sAction) {
	if (sAction == IBSAVE) {
		...
		for ( var ir = 1; ir <= sheetObj.LastRow; ir++) {
			var bfCode = sheetObj.CellValue(ir, "stv_dmg_cate_cd") + ":" + sheetObj.CellValue(ir, "stv_dmg_cd");
			if (bfCode != "") {
				for ( var jr = 1; jr <= sheetObj.LastRow; jr++) {
					var afCode = sheetObj.CellValue(jr, "stv_dmg_cate_cd") + ":" + sheetObj.CellValue(jr, "stv_dmg_cd");
					if (ir != jr && afCode == bfCode) {
						ComShowCodeMessage("OPF50005", 'Data');	//'{?msg1} is duplicated.'
						if (sheetObj.RowStatus(jr) == "I") {

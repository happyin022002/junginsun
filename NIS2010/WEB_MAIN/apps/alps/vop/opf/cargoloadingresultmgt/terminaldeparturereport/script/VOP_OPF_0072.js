...
/**
 * @extends 
 * @class VOP_OPF_0072 : VOP_OPF_0072 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_OPF_0072() {
...
function processButtonClick() {
	...
	switch (srcName) {
	    case "btn_Row_Add":
		   ...
		   sheetObject.CellValue2(row,	"atch_file_lnk_id")           = form.atch_file_lnk_id.value;
		   sheetObject.CellValue2(row,	"atch_file_lnk_seq")    = parseInt ( ComGetMax ( sheetObject, "atch_file_lnk_seq" )) + 1;
 ...
 function loadPage() {
	 ...
	 for(var i=0; i < uploadObjects.length ; i++){
	    //1. 기본 환경 설정
		 ComConfigUpload(uploadObjects[i], "/hanjin/VOP_OPF_0072GS.do");	
	 }
	 
	 // Upload 버튼 비활성화 여부
	 if (frm.is_upload_possible.value == "N") {
 ...
 function initSheet(sheetObj, sheetNo) {
 	...
 	with (sheetObj) {
 		...
 		case "sheet1": 
			...
			InitDataProperty(0, cnt++ , dtPopup,      		360,    daLeft,	false,      "file_upld_nm",     	 true,       "",      dfNone,       0,          false,		true,	50);						
			...
			InitDataProperty(0, cnt++ , dtImage,				40,		daCenter,	true,		"file_download", false,      "",	  dfNone,		0,			false,		false);
			 InitDataProperty(0, cnt++ , dtHidden,	    	0,		daCenter,	true,		"atch_file_lnk_id");
		    InitDataProperty(0, cnt++ , dtHidden,	    	0,		daCenter,	true,		"atch_file_lnk_seq");
		    InitDataProperty(0, cnt++ , dtHidden,	    	0,		daCenter,	true,		"file_path_url");
 ...
 function sheet1_OnClick(sheet,row,col,value){
 	if (sheet.ColSaveName(col)!= "file_download" || sheet.RowStatus(row)=="I") {
 		return;
	}
 ...
 function sheet1_OnDblClick(sheet, row, col) {
  	...
  	if (sheet.ColSaveName(col)!= "file_upld_nm" || sheet.RowStatus(row)=="I") {
  		return;
	}
...
function doActionIBSheet(sheetObj, formObj, sAction) {	
 	 switch (sAction) {
 	 	case IBSEARCH: //조회
			...
			var sXml = sheetObj.GetSearchXml("VOP_OPF_0072GS.do", FormQueryString(frm));
 	 	...
 	 	case IBSAVE:
			...
			// 업로드할 파일이 있을때
	 		if (!ComIsNull(upload1.LocalFiles)) {
	 			...
	 			pause(3700); // 중요!!! <-- ALPSA DB의 COM_UPLD_FILE 테이블에 Insert 후 재조회 하는 시간차 동안 Sleep 한다.
	 			ComOpenWait(false);
	 		} else {
	 			param = FormQueryString(frm) + "&" + sheetObj.GetSaveString();
	 			var sXml = sheetObj.GetSaveXml("VOP_OPF_0072GS.do",  param);
	 			sheetObj.LoadSaveXml(sXml);
	 		}

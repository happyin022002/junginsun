...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class vop_scg_0042 : vop_scg_0042 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
		...
		if(addRowFlag) {
			...
			if(filePath.indexOf("\\") !=-1) {
				with(sheetObj) {
					...
					SelectCell(SelectRow,prefix+"imdg_pck_instr_cd");				//포커스 이동
				}
			}
		}
	}
    ...
    function loadPage() {
        ...
        for(var i=0;i<uploadObjects.length;i++){
		    //1. 기본 환경 설정
		    ComConfigUpload(uploadObjects[i], "/hanjin/VOP_SCG_0042GS.do");
		}
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle = "|Sel.|No.|FileSetYn|Seq|File Sav Id||Packing Instructions\nProvisions|Sub|File Name|Desc.|ID|Date";
                    ...
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"imdg_pck_instr_seq", 		true,     "",      dfNone,      0,     false,   true);
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"file_sav_id", 		        true,     "",      dfNone,      0,     true,    true);
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"old_imdg_pck_instr_cd", 	true,     "",      dfNone,      0,     true,    true);
                    
                    InitDataProperty(0, cnt++ , dtData,   		   140,    daLeft,	   true,    prefix+"imdg_pck_instr_cd", 		true,     "",      dfNone,      0,     true,    true,  10);
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daCenter,   true,    prefix+"imdg_pck_instr_sub_seq", 	false,    "",      dfNone,      0,     true,    true,  2);
                    InitDataProperty(0, cnt++ , dtData,			   120,    daLeft,	   true,    prefix+"file_nm", 	    		    true,     "",      dfNone,      0,     false,   false, 200);
                    InitDataProperty(0, cnt++ , dtData,			   500,    daLeft,	   true,    prefix+"pck_desc", 	    		    false,    "",      dfNone,      0,     true,    true,  4000);
                    ...
                    InitDataValid(0, prefix+"imdg_pck_instr_cd", vtEngUpOther, "0123456789");
               }
               break;
        }
    }
        ...
        switch(sAction) {
			...
			case IBSAVE:        //저장
				...
				if (upObj.LocalFiles == "") {
					...
					var sXml = sheetObj.GetSaveXml("VOP_SCG_0042GS.do", sParam);
	...
	function sheet1_OnDblClick(sheetObj, Row, Col, Value){		
		...
		parent.location.href = "/hanjin/FileDownload?key="+sheetObj.CellText(Row, prefix+"file_sav_id");
		return;
	}

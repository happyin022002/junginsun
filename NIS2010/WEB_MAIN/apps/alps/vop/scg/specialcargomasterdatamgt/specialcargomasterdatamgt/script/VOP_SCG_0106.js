...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class VOP_SCG_0106 : VOP_SCG_0106 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function VOP_SCG_0106() {
		...
		switch(sheetNo) {
			case 1:      // sheet1 init
				with (sheetObj) {
					...
					var HeadTitle = "|||Del|Seq|UN No.|DP\n No.|Organic Peroxide|Type of\n IBC|Maximum\n Quantity(litres)|Control\n Temperature|Emergency\n Temperature";
					...
					InitDataProperty(0, cnt++ , dtDataSeq,	    40,		daCenter,	false,	"sub_seq",		     true,	"",      	dfNone,			0,			true,		true);
					InitDataProperty(0, cnt++ , dtData,			50,		daCenter,	false,	"imdg_un_no",	 false,	"",			dfNone,			0,			true,		true, 4);
					InitDataProperty(0, cnt++ , dtData,			40,		daCenter,	false,	"regu_dp_no",		 true,	"",			dfNone,			0,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			300,	daLeft,		false,	"org_prx_desc",		false,	"",			dfNone,			0,			true,		true);
					InitDataProperty(0, cnt++ , dtData,			70,		daCenter,	false,	"imdg_pck_cd",		false,	"",			dfNone,			0,			true,		true);
					InitDataProperty(0, cnt++ , dtData,			100,	daRight,	false,	"max_capa_qty",		false,	"",			dfNone,			0,			true,		true);
					InitDataProperty(0, cnt++ , dtData,			90,		daRight,	false,	"imdg_ctrl_temp",		false,	"",			dfNone,			0,			true,		true);
					InitDataProperty(0, cnt++ , dtData,			70,		daRight,	false,	"imdg_emer_temp",		false,	"",			dfNone,			0,			true,		true);
					InitDataValid(0, "imdg_pck_cd", vtEngUpOther, "0123456789"); // 영대문자,숫자만 입력
					InitDataValid(0, "max_capa_qty", vtNumericOther, "."); // 숫자만 입력
					InitDataValid(0, "imdg_ctrl_temp", vtNumericOther, "."); // 숫자만 입력
					InitDataValid(0, "imdg_emer_temp", vtNumericOther, "."); // 숫자만 입력
		...
		switch(sAction) {
        	case IBSEARCH:      //조회
        		if(validateForm(sheetObj,formObj,sAction)){
        	 		formObj.f_cmd.value = SEARCH;
	        	   	sheetObj.DoSearch("VOP_SCG_0106GS.do", FormQueryString(formObj));
				}
        		break;
        	case IBSAVE:      //Country Code조회
        		if(validateForm(sheetObj,formObj,sAction)){
    				...
    				sheetObj.DoAllSave("VOP_SCG_0106GS.do", FormQueryString(formObj));

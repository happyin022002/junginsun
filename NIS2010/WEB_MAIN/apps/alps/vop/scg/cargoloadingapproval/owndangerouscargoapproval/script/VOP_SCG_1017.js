...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class vop_scg_1017 : vop_scg_1017 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function vop_scg_1017() {
	 		...
	 		case "sheet1":      //sheet1 init
	     		with (sheetObj) {
					...
					style.height = 260;
         			...
         			InitDataProperty(0, cnt++ , dtData,		40,	daCenter,	true,	"bb_cgo_seq",			false,	"",	dfNone);
         			...
         			InitDataProperty(0, cnt++ , dtData,     70, daRight,  	true,   "grs_wgt",				false,  "", dfFloat,	2,	false	);
        ...
        switch(sAction) {
        	...
        	case IBSEARCH:      //조회
        		if(validateForm(sheetObj,formObj,sAction))
        			...
        			sheetObj.DoSearch("VOP_SCG_1017GS.do", FormQueryString(formObj));
...
//			}
     ...
     function sheet1_OnSearchEnd(sheetObj, ErrMsg)
     {
    	 with(sheetObj)
    	 {
    		 ...
    		 var nextSeq = findSeq;
    		 var rowNumIdx = 15; // Row Number 를 저장해 두는 Array 상의 Index. 비교 대상 컬럼 갯수와 같은값. 2011.03.02
...
//    		 for (var i = 2; i <= LastRow; i ++)
    		 {
    			 ...
    			 if (headerRows != nextSeq && CellValue(i, "spcl_cgo_rqst_seq") == maxSeq) {
    				 ...
    				 arrAftData[k][0] = CellValue(i, "cmdt_nm");
    				 arrAftData[k][1] = CellValue(i, "dim_len");
    				 arrAftData[k][2] = CellValue(i, "dim_wdt");
    				 arrAftData[k][3] = CellValue(i, "dim_hgt");
    				 arrAftData[k][4] = CellValue(i, "grs_wgt");
    				 arrAftData[k][5] = CellValue(i, "net_wgt");
    				 arrAftData[k][6] = CellValue(i, "bb_cgo_seq");
    				 // Lane, VVD, POL, POD 추가 . 2011.03.07    				 
    				 arrAftData[k][7] = CellValue(i, "slan_cd");
    				 arrAftData[k][8] = CellValue(i, "vsl_cd");
    				 arrAftData[k][9] = CellValue(i, "skd_voy_no");
    				 arrAftData[k][10] = CellValue(i, "skd_dir_cd");
    				 arrAftData[k][11] = CellValue(i, "pol_cd");
    				 arrAftData[k][12] = CellValue(i, "pod_cd");
    				 // 비교 대상 컬럼 ( VSL_PRE_PST_CD, VSL_SEQ ) 추가.2011.03.08
    				 arrAftData[k][13] = CellValue(i, "vsl_pre_pst_cd");
    				 arrAftData[k][14] = CellValue(i, "vsl_seq");
    				 
    				 arrAftData[k][rowNumIdx] = i;
    				 k++;
    			 }
    			 if (CellValue(findSeq-1, "bb_cgo_seq") == "") {
    				 findSeq--;
    			 }
    			 if (headerRows != nextSeq && CellValue(findSeq-1, "spcl_cgo_rqst_seq") == CellValue(i, "spcl_cgo_rqst_seq")) {
    				 arrBefData[j] = new Array();
    				 arrBefData[j][0] = CellValue(i, "cmdt_nm");
    				 arrBefData[j][1] = CellValue(i, "dim_len");
    				 arrBefData[j][2] = CellValue(i, "dim_wdt");
    				 arrBefData[j][3] = CellValue(i, "dim_hgt");
    				 arrBefData[j][4] = CellValue(i, "grs_wgt");
    				 arrBefData[j][5] = CellValue(i, "net_wgt");
    				 arrBefData[j][6] = CellValue(i, "bb_cgo_seq");
    				 arrBefData[j][7] = CellValue(i, "slan_cd");
    				 arrBefData[j][8] = CellValue(i, "vsl_cd");
    				 arrBefData[j][9] = CellValue(i, "skd_voy_no");
    				 arrBefData[j][10] = CellValue(i, "skd_dir_cd");
    				 arrBefData[j][11] = CellValue(i, "pol_cd");
    				 arrBefData[j][12] = CellValue(i, "pod_cd");
    				 arrBefData[j][13] = CellValue(i, "vsl_pre_pst_cd");
    				 arrBefData[j][14] = CellValue(i, "vsl_seq");
    		 ...
    		 for (var x = 0; x <= j-1; x++)
    		 {
        		 for (var z = 0; z <= k-1; z++)
        		 {
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][0] != arrAftData[z][0]) CellFontColor(arrAftData[z][rowNumIdx], "cmdt_nm") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][1] != arrAftData[z][1]) CellFontColor(arrAftData[z][rowNumIdx], "dim_len") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][2] != arrAftData[z][2]) CellFontColor(arrAftData[z][rowNumIdx], "dim_wdt") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][3] != arrAftData[z][3]) CellFontColor(arrAftData[z][rowNumIdx], "dim_hgt") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][4] != arrAftData[z][4]) CellFontColor(arrAftData[z][rowNumIdx], "grs_wgt") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][5] != arrAftData[z][5]) CellFontColor(arrAftData[z][rowNumIdx], "net_wgt") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][7] != arrAftData[z][7]) CellFontColor(arrAftData[z][rowNumIdx], "slan_cd") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][8] != arrAftData[z][8]) CellFontColor(arrAftData[z][rowNumIdx], "vsl_cd") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][9] != arrAftData[z][9]) CellFontColor(arrAftData[z][rowNumIdx], "skd_voy_no") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][10] != arrAftData[z][10]) CellFontColor(arrAftData[z][rowNumIdx], "skd_dir_cd") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][11] != arrAftData[z][11]) CellFontColor(arrAftData[z][rowNumIdx], "pol_cd") = RgbColor(255, 0, 0);
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][13] == arrAftData[z][13] && arrBefData[x][14] == arrAftData[z][14] && arrBefData[x][12] != arrAftData[z][12]) CellFontColor(arrAftData[z][rowNumIdx], "pod_cd") = RgbColor(255, 0, 0);

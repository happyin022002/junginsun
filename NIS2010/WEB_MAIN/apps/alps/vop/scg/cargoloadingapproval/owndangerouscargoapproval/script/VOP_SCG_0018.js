...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class VOP_SCG_0018 : VOP_SCG_0018 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function VOP_SCG_0018() {
    	...
    	switch(shtID) {
    		case "sheet1":     
    			with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtData,		100,		daCenter,	false,	"cntr_tpsz_cd",		false,	"",	dfNone,		0,		false,		true);
					InitDataProperty(0, cnt++ , dtData,		100,		daRight,	true,	"op_cntr_qty",		false,	"", dfFloat,	2,		false,		true);
					InitDataProperty(0, cnt++ , dtData,		100,		daRight,	false,	"rf_cgo_qty",		false,	"", dfFloat,	2,		false,		true);
    				CountPosition = 0;
    			}
    			break;
    		...
    		case "sheet2":      //t2sheet1 init
    			with (sheetObj) {
                    ...
                    var HeadTitle = "|ibflag|Seq.|Container No.|Container No.|Vol.|Genset|Volt|Appr.|bkg_no|rc_seq|cmdt_cd|cmdt_nm|cdo_temp|" +
                    				...
                    				"rf_dcgo_seq|ctrl_atms_flg|modi_atms_flg|humid_ctrl_flg|cntr_drn_cd|diff_rmk|modifyaproflg";
					...
					InitDataProperty(0, cnt++ , dtData,			82,	daCenter,		true,		"cntr_no",			false,			"",      dfNone,			0,		true,		true);
					InitDataProperty(0, cnt++ , dtData,			23,	daCenter,		false,		"cntr_tpsz_cd",		false,			"",      dfNone,			0,		true,		true);
					InitDataProperty(0, cnt++ , dtData,			30,	daCenter,		false,		"cntr_vol_qty",		false,			"",      dfFloat,			1,		true,		true);
					InitDataProperty(0, cnt++ , dtCheckBox,		50,	daCenter,		false,		"pwr_spl_cbl_flg",	false,			"",      dfNone,			0,		true,		true);
					InitDataProperty(0, cnt++ , dtData,			35,	daCenter,		false,		"vltg_no",			false,			"",      dfNone,			0,		true,		true);
					InitDataProperty(0, cnt++ , dtData,			20,	daCenter,		false,		"spcl_cgo_apro_cd",	false,			"",      dfNone,			0,		true);
					...
					InitDataProperty(0, cnt++ , dtHidden,		20,	daCenter,		false,		"cdo_temp",			false,			"",      dfNone,			0,		true);						
					
					InitDataProperty(0, cnt++ , dtHidden,		20,	daCenter,		false,		"fdo_temp",			false,			"",      dfNone,			0,		true);
            ...
            case "sheet3":      //sheet3 init
            	with (sheetObj) {
    				...
    				InitDataProperty(0, cnt++ , dtData,			40,		daRight,		false,		"pck_qty",			false,		"",      dfNullInteger,			2,		true,		true);
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
    	...
    	switch(sAction) {
	     	...
	     	case IBSEARCH_ASYNC01: //RJT CD 조회
	     		...
	     		var sXml = sheetObj.GetSearchXml("VOP_SCG_0031GS.do", formParams+"&spcl_cgo_cate_cd=RF");
    		...
    		case IBSEARCH:      //조회
     			if (document.getElementById("scg_flg").value == "RF") {
    				oSheetObj = openerObj.sheetObjects[6];
    				
    				//자사 Reefer일 경우 메일 송신시 구분을 위해 오리지널 상태값 유지
    				formObj.org_spcl_cgo_auth_cd.value = oSheetObj.CellValue(oSheetObj.SelectRow, "org_spcl_cgo_auth_cd");
    			}else{
...
//    	 }
     ...
     function sheet2_OnSearchEnd(sheetObj, Row, Col, Value) {
    	 ...
    	 for (var i=1;i<=sheetObjects[1].RowCount;i++){
    		 if (sheetObjects[1].CellText(i, "Seq") == document.getElementById("rc_seq").value) {
     ...
     function onPopupClick(srcName){
    	 ...
    	 if (srcName == "btns_Commodity") {
    		 ...
    		 ComOpenPopup("ESM_BKG_0696POP.do?searcheKeyOpener="+formObj.pck_tp_cd.value, 400, 470, "","0,0,1,1,1", true);
    		 ...
    		 /*VVD별 Aproval History를 보고 싶은겨우 아래 링크를 활성화한다.
     		  ...
     		  * ComOpenPopup("VOP_SCG_1018.do?scg_flg="+formObj.scg_flg.value+"&bkg_no="+formObj.bkg_no.value+"&vsl_cd="+formObj.vvd_cd.value.substring(0,4)+"&skd_voy_no="+formObj.vvd_cd.value.substring(4,8)+"&skd_dir_cd="+formObj.vvd_cd.value.substring(8,9), 1000, 450, "", '0,0', true);
    		 */
    		 ...
    		 if (row == 2) {
    			 ...
    			 formObj.rc_seq.value = oSheetObj.CellValue( parseInt(row)-1 , "rc_seq");
		 ...
		 if (formObj.spcl_cgo_auth_cd.value == "A") {
			 ...
			 */
			 ...
			 if (formObj.mailYn.value == "Y") {
	    		 for (var i = 2; i <= oSheetObj.LastRow; i ++)
	    		 {
	    			 if (befVVD == oSheetObj.CellText(i, "vsl_cd")+oSheetObj.CellText(i, "skd_voy_no")+oSheetObj.CellText(i, "skd_dir_cd")) {
     ...
     function sendReqMail(sheetObj, formObj) {
    	 ...
    	 //상태가 'P'인 경우와 Approved 화면이 아닌 경우에는 메일 수신 대상을 VVD+'R'상태 인 해당 BKG과 같은 BKG을 대상으로 한다.

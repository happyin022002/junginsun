...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends 
     * @class vop_scg_0079 : vop_scg_0079 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function vop_scg_0079() {
	...
	function selectFile(sheetObj, addRowFlag){
			...
			if(filePath.indexOf("\\") !=-1) {
				with(sheetObj) {
					...
					CellValue2(SelectRow, prefix+"vsl_opr_tp_cd")=  document.form.vsl_opr_tp_cd.value;			    //파일명 설정
    ...
    function loadPage() {
        ...
        for(var i=0;i<uploadObjects.length;i++){
		    //1. 기본 환경 설정
		    ComConfigUpload(uploadObjects[i], "/hanjin/VOP_SCG_0079GS.do");
		}
            ...
            case 1:      //sheet1 init
                with (sheetObj) {
                   ...
                   //전체Edit 허용 여부 [선택, Default false]
                    ...
                    var HeadTitle = "|Sel.|No.|FileSetYn|Seq|File Sav Id||Vessel Operator|File Name|ID|Date";
                    ...
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"imdg_crr_rstr_file_seq", 		true,     "",      dfNone,      0,     true,    true);
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"file_sav_id", 		        true,     "",      dfNone,      0,     true,    true);
                    InitDataProperty(0, cnt++ , dtHidden,   	   0,      daLeft,	   true,    prefix+"", 	true,     "",      dfNone,      0,     true,    true);
                    
                    InitDataProperty(0, cnt++ , dtData,   		   100,    daLeft,	   true,    prefix+"vsl_opr_tp_cd", 		false,     "",      dfNone,      0,     false,    false,  10);
                    InitDataProperty(0, cnt++ , dtData,			   150,    daLeft,	   true,    prefix+"file_nm", 	    		    false,     "",      dfNone,      0,     false,   false, 200);
                    InitDataProperty(0, cnt++ , dtData,			   100,    daCenter,   true,    prefix+"cre_usr_id", 	 		 	false,    "",      dfNone,      0,     false,    false);

                    InitDataProperty(0, cnt++ , dtData,			   100,    daCenter,   true,    prefix+"cre_dt", 	  				false,    "",      dfDateYmd,   0,     false,    false);
                    
                    InitDataValid(0, prefix+"vsl_opr_tp_cd", vtEngUpOther, "0123456789");
               }
               break;
        }
    }
    ...
    function doActionIBSheet(sheetObj, formObj, sAction) {
        ...
        switch(sAction) {
			...
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch("VOP_SCG_0079GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));	
			...
			//	if(!validateForm(sheetObj,formObj,sAction)) return true;
				...
				if (upObj.LocalFiles == "") {
					...
					var sXml = sheetObj.GetSaveXml("VOP_SCG_0079GS.do", sParam);

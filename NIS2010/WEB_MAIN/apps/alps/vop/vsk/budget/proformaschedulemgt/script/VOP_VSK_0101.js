...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class VOP_VSK_0101 : VOP_VSK_0101 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function VOP_VSK_0101() {
        ...
        case "sheet2":      //sheet1 init
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
				...
				InitDataProperty(0, cnt++ , dtData,			35,		daRight,	true,	prefix+"act_wrk_hrs",		false,	"",			dfFloat,		1,      true,		true,		3,			false,		false);
...
//					var sXml = sheetObj.GetSearchXml("VOP_VSK_0101GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
			        var sXml = sheetObj.GetSearchXml("VOP_VSK_0101GS.do", parm + "&" + ComGetPrefixParam(aryPrefix));
			...
			case SEARCH02: //Port Code 변경 시.
				...
				var sXml = sheetObj.GetSearchXml("VOP_VSK_0101GS.do?loc_cd="+loc_cd, sParam);
				ComOpenWait(false);
			...
			case SEARCH04: //Yard Code 변경시...
				...
				var sXml = sheetObj.GetSearchXml("VOP_VSK_0101GS.do", sParam);
			...
			case IBSAVE:        //저장
				...
				if(validateForm(sheetObj,formObj,sAction)){										
					...
					formObj.f_cmd.value = MULTI;
		     	   	...
		     	   	var sXml = sheetObj.GetSaveXml("VOP_VSK_0101GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
		     	   	ComOpenWait(false);

	...
	try {
		...
		case "btn_DownExcel":
			...
			if(sheetObjects[1].SearchRows>0){
				ComOpenWait(true);
				sheetObjects[1].Down2Excel(-1, true, true, true, "", "", false, false, "Vessel Code", true, "", "", false, false, "");
				ComOpenWait(false);
			}
			
			if(sheetObjects[2].SearchRows>0){
				ComOpenWait(true);
				sheetObjects[2].Down2Excel(-1, true, true, true, "", "", false, false, "Port Code", true, "", "", false, false, "");
				ComOpenWait(false);
			}
...
function initSheet(sheetObj) {
	...
	case "sheet1": // Remark Grid
		with(sheetObj) {
			...
			InitDataProperty(0, cnt++ , dtData,     	40,	daCenter,	true,			"",					false,	"",		dfNone,				0,			false, 	false);
	...
	case "sheet3": // Port Names
		with(sheetObj) {
            ...
            //데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
			InitDataProperty(0, cnt++ , dtData,     	40,	daCenter,	true,			"port_cd",	false,	"",		dfNone,				0,			false, 	false);
			InitDataProperty(0, cnt++ , dtData,     	80,	daCenter,	true,			"port_nm",	false,	"",		dfNone,				0,			false, 	false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: // 조회
		...
		ComOpenWait(true);
		var sXml = sheetObj.GetSearchXml("VOP_VSK_0104GS.do" , sParam);
		ComOpenWait(false);
...
function ComSheetObject4VSK(objectid, sheetid, idx){
    try {
    	...
    	var ifr = document.createElement('<iframe name="ifr_' + sheetid + '" id="ifr_' + sheetid + '" frameborder="no" scrolling="no" width="100%" height="0" src="VOP_VSK_0104_01.do" onload=postLoad("ifr_' + sheetid + '",' + idx + ')></iframe>');
...
function postProcess(idx){
	...
	if(flag){
		...
		for(var i=0; i<vslEngNms.length; i++){
			...
			sheetObjects[1].CellValue2(Row, 0) = vslEngNm[0];
			sheetObjects[1].CellValue2(Row, 1) = vslEngNm[1];
		}
		
		for(var i=0; i<portNms.length; i++){
			Row = sheetObjects[2].DataInsert();
			portNm = portNms[i].split("|");
			sheetObjects[2].CellValue2(Row, 0) = portNm[0];
			sheetObjects[2].CellValue2(Row, 1) = portNm[1];
		}

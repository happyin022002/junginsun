				...
				if(ComIsBtnEnable('btn_Simulation')){
					...
					if(check){
						...
						for(var i=0; i<vessels.length; i++){
							...
							var sUrl = "/hanjin/VOP_VSK_0112.do";
...
//                InitHeadRow(5, HeadTitle6, true);
                ...
                //데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
					...
					InitDataProperty(i, cnt++ , dtData,		40,	daCenter,	true,			"VVD2",					false,	"",		dfNone,				0,			false, 	false);
    ...
    switch(sAction) {
       ...
       case IBSEARCH:      //조회
			...
			if ( sheetObj.id == "sheet1"){
				...
				//sheetObj.DoSearch("VOP_VSK_0103GS_1.do" , sParam);
				ComOpenWait(true);
				var sXml = sheetObj.GetSearchXml("VOP_VSK_0103GS.do" , sParam);
				ComOpenWait(false);
		...
		case IBSAVE:        //저장
			...
			if (validateForm(sheetObj, formObj, sAction)){
				...
				ComOpenWait(true);
				var sXml = sheetObj.GetSaveXml("VOP_VSK_0103GS.do" , sParam + param);
				ComOpenWait(false);
		...
		case SEARCH02: // simulation
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0103_1GS.do" , sParam);
			ComOpenWait(false);
			HeadCol1 = ComGetEtcData(sXml, "HeadTitle1");          
            HeadCol2 = ComGetEtcData(sXml, "HeadTitle2");          
            HeadCol3 = ComGetEtcData(sXml, "HeadTitle3");          
	...
	with(sheetObj){
    	...
    	if(!simplePIN && (PortPosition(col)==-1 || "skd_rmk"==sheetObj.ColSaveName(col))){
    		...
    		if(rVal){
    			...
    			if(!simplePIN){
    				// phase in 시작한 포트 표시
	            	CellValue2(row+sheet2StatusRowPos+dataSetCnt, col) = "I:" + rVal.rsn_cd; 
...
//                	CellValue2(dataStartRow+sheet2StatusRowPos, dataStartCol+1) = "I:" + rVal.rsn_cd;
    				CellValue2(dataStartRow+sheet2StatusRowPos, col) = "I:" + rVal.rsn_cd; 
...
function openPfTypePopup(sheetObj, Row){
	var formObj = document.form;
	var sUrl = "/hanjin/VOP_VSK_0111.do?vsl_slan_cd=" + formObj.vsl_slan_cd.value;

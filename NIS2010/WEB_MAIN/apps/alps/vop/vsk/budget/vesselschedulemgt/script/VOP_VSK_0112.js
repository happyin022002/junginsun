...
/**
 * @extends
 * @classVOP_VSK_0112 : VOP_VSK_0112 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_VSK_0112() {
	...
	try {
		...
		case "btn_simulation":
			...
			for ( var i = 1; i <= sheetObj.RowCount; i++) {
				...
				if (sheetObj.CellValue(i, "bkg_status") == "Booking" &&	(sheetObj.CellValue(i, "vir_bkg_status") == "BKG" || sheetObj.CellValue(i, "vir_bkg_status") == "BKGNOD")) {
					bkgVirVVDs.push(vvd);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: // 조회
		...
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0112GS.do", sParam);
...
function sheet1_OnSearchEnd(sheetObj) {
	...
	for ( var i = sheetObj.HeaderRows; i < dataRows + sheetObj.HeaderRows; i++) {
		...
		//::2014-06-20::--if (sheetObj.CellValue(i, "update_flag") == "NO") {
		if (sheetObj.CellValue(i, "update_flag") == "NO") {
			sheetObj.RowBackColor(i) 	= sheetObj.RgbColor(255, 168, 80); // 지우지 못하는 Booking VVD + LIVE에 존재하는 VVD
			enableDelete 				= false;
		} else if (sheetObj.CellValue(i, "bkg_status") == "Booking") {

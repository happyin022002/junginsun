...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class vop_vsk_0113 : vop_vsk_0113 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function vop_vsk_0113() {
		...
		switch(sAction) {
		   ...
		   case IBSEARCH:      //조회
				if(validateForm(sheetObj,formObj,sAction)){
					...
					ComOpenWait(true);
					var sXml = sheetObj.GetSearchXml("VOP_VSK_0113GS.do", sParam);
					ComOpenWait(false);
			...
			case COMMAND08:      // Date
				...
				var sParam = FormQueryString(formObj);
				sXml = sheetObj.GetSearchXml("VOP_VSK_0113GS.do", sParam);
    ...
    function isCheckCarrierCd(sheetObj, formObj){
    	...
    	if(ComChkLen(formObj.carrier_cd, 3) == 2){
    		...
    		if(crrCd == null || crrCd == undefined || crrCd == ""){
    			ComShowCodeMessage('VSK00113', formObj.carrier_cd.value);
    			formObj.carrier_cd.value = "";
    			return false;
    		}else{
    			return true;

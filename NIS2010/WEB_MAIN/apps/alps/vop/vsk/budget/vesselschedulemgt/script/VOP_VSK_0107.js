...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class VOP_VSK_0107 : VOP_VSK_0107 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function VOP_VSK_0107() {
...
//					
//					sUrl = "/hanjin/VOP_VSK_0107.do?vsl_cd="+vsl_cd+"&skd_voy_no="+skd_voy_no+"&skd_dir_cd="+skd_dir_cd;
		...
		switch(sAction) {
			...
			case SEARCH01:		//VSL_CD Check
				if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml = sheetObj.GetSearchXml("VOP_VSK_0107GS.do", sParam);
...
//		sheetObj.CellFontColor(Row, Col) = sheetObj.CellBackColor(Row, Col);
	...
	function callRDOpen(sheetObj, formObj){
		...
		if(VskIsNull(rdParam)){
			...
			formObj.com_mrdPath.value      = "apps/alps/vop/vsk/scheduleplanningoperation/vesselschedulemgt/report/VOP_VSK_0107.mrd";

...
/**
 * @extends
 * @class VOP_VSK_0106 : VOP_VSK_0106 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_VSK_0106() {
...
//			tabIndex = -1;
			...
			var HeadTitle1 = "|Sel.|VVD|VVD|VVD|VSL Name|Lane|P/F SKD\nType|Status|VirStatus|Created\nDate|Start\nDate|Carrier|Carrier|turn_skd_voy_no|turn_skd_dir_cd|skd_sts_mnl_flg";
			var HeadTitle2 = "||VSL|VOY NO.|DIR|VSL Name|Lane|P/F SKD\nType|Status|VirStatus|Created\nDate|Start\nDate|CURT|ACT||turn_skd_voy_no|turn_skd_dir_cd|skd_sts_mnl_flg";
...
//			InitDataProperty(0, cnt++, dtPopupEdit, 70, daCenter, true, prefix + "act_crr_cd", false, "",  dfEngUpKey, 0, true, true, 3);
			InitDataProperty(0, cnt++, dtHidden, 70, daCenter, true, prefix + "act_crr_cd", false, "",  dfEngUpKey, 0, true, true, 3);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: // 조회
		...
		if (validateForm(sheetObj, formObj, sAction)){
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0106GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
			ComOpenWait(false);
	...
	case IBDELETE: // 삭제
		...
		var sXml = sheetObj.GetSaveXml("VOP_VSK_0106GS.do", FormQueryString(formObj) + "&" + SaveStr);
		ComOpenWait(false);
...
//		var sXml = sheetObj.GetSaveXml("VOP_VSK_0106GS.do", sParam + "&" + ComGetSaveString(sheetObj));
//		sheetObj.LoadSearchXml(sXml);
//		ComOpenWait(false);
//		
//		if(VskGetErrorXml(sXml)){
...
//		break;
	...
	case SEARCH01: // Lane Code 조회
		...
		if ( sheetObj.id == "sheet1"){
			ComOpenWait(true);
			var sParam = FormQueryString(formObj);
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0106GS.do" , sParam);
			ComOpenWait(false);
...
//		var sXml = sheetObj.GetSaveXml("VOP_VSK_0106GS.do", sParam);
//		ComOpenWait(false);
//		sheetObj.LoadSearchXml(sXml);
//		
//		if(VskGetErrorXml(sXml)){

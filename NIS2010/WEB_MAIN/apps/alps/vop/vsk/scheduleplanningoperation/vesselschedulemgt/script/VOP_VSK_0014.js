...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class vop_vsk_0014 : vop_vsk_0014 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function vop_vsk_0014() {
...
//					InitDataProperty(0, cnt++ , dtCombo,		80,		daCenter,	false,		prefix+"turn_clpt_ind_seq",			false,		"",			dfNone,				0,			false,		false,		1,			false,		false);
					...
					InitDataProperty(0, cnt++ , dtHidden,		0,		daCenter,	true,		prefix+"delt_flg",					false,		"",			dfNone,				0,			false,		false);
		...
		switch(sAction) {
			...
			case IBSAVE:        //Save
				if(validateForm(sheetObj,formObj,sAction)){
					...
					ComOpenWait(true);
					var sXml = sheetObj.GetSaveXml("VOP_VSK_0014GS.do", sParam);
					ComOpenWait(false);
			...
			case COMMAND01:		// P/F SKED Use
				if(validateForm(sheetObj, formObj, COMMAND01)){
					...
					var sUrl = "/hanjin/VOP_VSK_0220.do?" + urlTail;
    	...
    	if(rtnObjs){
			...
			if(rtnDatas){
				if(rtnDatas.length > 0){
					...
					//PF의 yard 중 삭제된 경우 사용하지 못하도록
					var cnt = 0;
					var deltYd = "";
					for(var i=sheetObj.HeaderRows; i<sheetObj.HeaderRows+sheetObj.RowCount; i++){
						if(sheetObj.CellValue(i, "sheet1_delt_flg")=="Y"){
							if(cnt==0){
								deltYd = sheetObj.CellValue(i, "sheet1_vps_port_cd")+sheetObj.CellValue(i, "sheet1_tml_cd");
							}else{
								deltYd = deltYd + ", " + sheetObj.CellValue(i, "sheet1_vps_port_cd")+sheetObj.CellValue(i, "sheet1_tml_cd");
							}
							sheetObj.CellValue2(i, "sheet1_tml_cd") = "";
							cnt++;
						}
					}
					if(cnt>0){
						ComShowMessage("Deleted Yard ["+ deltYd +"] Exist. Please Select Terminal Code.");
					}
				}
			}
    	}

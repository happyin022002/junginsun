...
function initSheet(sheetObj, HeadCol1, HeadCol2, HeadCol3, HeadCol4, HeadCol5, HeadCol6, HeadCol7) {
	...
	case "sheet1": // sheet1 init
		with (sheetObj) {
			...
			var HeadTitle2 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol2+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
			var HeadTitle3 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol3+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
			var HeadTitle4 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol4+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
			var HeadTitle5 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol5+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
			var HeadTitle6 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol6+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
			var HeadTitle7 = "|| |VSL\nNM|VSL\nCD|VOY.\nNO.|DIR|OPR"+HeadCol7+"|RMK|cre_dt|cre_usr_id|upd_dt|upd_usr_id";
            ...
            for(var i = 0 ; i < dataSetCnt ; i++){
    			...
    			InitDataProperty(i, cnt++ , dtData,		1,	daLeft,		true,			"vsl_eng_nm");
    			...
    			InitDataProperty(i, cnt++ , dtData,		44,	daCenter,	true,			"VVD4",					false,	"",		dfNone,				0,			false, 	false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case SEARCH04: // Yard Full Name 조회
...
function sheet1_OnSearchEnd(sheetObj, ErrMsg){
	...
	for(var i=0; i<sheetObj.LastRow; i++){
		sheetObj.CellFontColor(i, 3)  =  sheetObj.CellBackColor(i, 3);
		if(i>6){
			sheetObj.CellFontColor(i, 3) = sheetObj.RgbColor(255, 255, 255);
		}
	}
	...
	if(portNum <= 11){ 	// 포트의 수가 11개 이하이면 그리드의 좌우 크기를 딱 맞게 조절한다.
		width = 182 + sheetObj.ColWidth("VVD2") + ( portNum * 68 ) + 2; // 138 = [vsl_cd(44) + skd_voy_no(44) + RMK(50)] // 2 : 그리드 경계값  :: 138->182 ("OPR" 컬럼추가에따른 44width Plus)::
		if(totRows > 14){
...
function Down2Excel(Mode, UseOpenFile){
	...
	for(var i=0; i<sheetObjects[0].LastRow; i++){
		sheetObjects[0].CellFontColor(i, 3)  =  sheetObjects[0].CellFontColor(i, 4);
		if(i>6){
			sheetObjects[0].CellFontColor(i, 3) = sheetObjects[0].RgbColor(0, 0, 0);
		}
	}
	sheetObjects[0].ColWidth(3) = 150;
	
	sheetObjects[0].Down2Excel(Mode, UseOpenFile, false, true, "", "apps/alps/vop/vsk/scheduleplanningoperation/vesselschedulemgt/script/VOP_VSK_0012_01.xml", false, false, "Long Range SKD", true, "", "", false, false, "");

	for(var i=0; i<sheetObjects[0].LastRow; i++){
		sheetObjects[0].CellFontColor(i, 3)  =  sheetObjects[0].CellBackColor(i, 3);
		if(i>6){
			sheetObjects[0].CellFontColor(i, 3) = sheetObjects[0].RgbColor(255, 255, 255);
		}
	}
	sheetObjects[0].ColWidth(3) = 1;
...
function sheet1_OnSelectCell(sheetObj, OldRow, OldCol, NewRow, NewCol){
	...
	if(OldRow >= sheetObj.HeaderRows){
		...
		sheetObj.CellBackColor(OldRow, "VVD4") = sheetObj.RgbColor(0, 0, 0);
		...
		sheetObj.CellBackColor(NewRow, "VVD4") = sheetObj.RgbColor(128, 128, 255);
	}
	...
	if(OldRow >= sheetObj.HeaderRows){
		...
		sheetObj.CellValue2(row_skd_dir_cd, "VVD4") = pfTypes;
	}
}
...
function popupVVDInquiry(vslCd, skdVoyNo, skdDirCd){
	...
	var skdDirCd1="";
	var skdDirCd2="";
	var sUrl = "/hanjin/VOP_VSK_0019.do?pop_mode=Y&vsl_cd=" + vslCd;
...
function sheet1_OnMouseMove(sheetObj, Button, Shift, X, Y){
    ...
    if(sheetObj.RowCount > 0){
        ...
        if(Row > 6 && Col < 6){
	        	var sText = sheetObj.CellValue(Row,"vsl_eng_nm");
	        	if(sText != ""){
        	...
        	var yardCode = sheetObj.CellValue(Row, Col);
        	var yd_nm = formObj.yd_nm.value;
        	
    		if (yardCode.length == 7 && yardCode != formObj.yd_cd.value) {
    			formObj.yd_cd.value = yardCode;
    			yd_nm = doActionIBSheet(sheetObj, formObj, SEARCH04);
    			formObj.yd_nm.value = yd_nm;
    		}
    		
    		if (yd_nm != "") {
        		sheetObj.MouseToolTipText = yd_nm;
        		sheetObj.MousePointer = "Hand";
    		} else {
        		sheetObj.MouseToolTipText = "";
        		sheetObj.MousePointer = "Default";

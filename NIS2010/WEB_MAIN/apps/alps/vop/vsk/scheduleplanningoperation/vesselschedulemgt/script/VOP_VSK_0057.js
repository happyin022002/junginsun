...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class vop_vsk_0057 : vop_vsk_0057 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function vop_vsk_0057() {
		...
		switch(sAction) {
			...
			case SEARCH01:		//Terminal(Yard) List
				if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml = sheetObj.GetSearchXml("VOP_VSK_0057GS.do", sParam);
			...
			case IBSAVE:        //Save
				if(validateForm(sheetObj,formObj,sAction)){
					...
					ComOpenWait(true);
					var sXml = sheetObj.GetSaveXml("VOP_VSK_0057GS.do", sParam);
					ComOpenWait(false);

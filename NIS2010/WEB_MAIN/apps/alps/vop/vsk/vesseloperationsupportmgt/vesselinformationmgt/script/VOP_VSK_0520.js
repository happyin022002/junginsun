 ...
 *@FileTitle : View Option (Vessel Summary popup) 
 *Open Issues :
 *Change history :
 ...
 ***************************************************************************************/
/**
 * @fileoverview Vessel Summary 화면의 Popup화면
 * @author CLT
 */

/**
 * @extends
 * @class VOP_VSK_0520 : VOP_VSK_0520 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_VSK_0520() {
...
function processButtonClick() {
	...
	try {
		...
		//var id = window.event.srcElement.getAttribute("value"); 
	
		switch (srcName) {
			case "btn_ok":
				var getChk = document.getElementsByName("chk[]");
				var rtnStr1 = "";
				var rtnStr2 = "";
				var rtn = new Array();
				for(var i=0; i<getChk.length; i++){
					if( !getChk[i].checked ){
						rtnStr1 += getChk[i].value;
						rtnStr1 += "|";
					}else if(getChk[i].checked){
						rtnStr2 += getChk[i].value;
						rtnStr2 += "|";
					}
				}
				rtn[0] = rtnStr1;

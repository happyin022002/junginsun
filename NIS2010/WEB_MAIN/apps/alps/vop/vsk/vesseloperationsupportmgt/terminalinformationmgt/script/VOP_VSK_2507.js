...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class VOP_VSK_2507 : VOP_VSK_2507 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function VOP_VSK_2507() {
...
//     sheetCnt = sheetObjects.length ;   
     ...
     for(var i=0; i < uploadObjects.length ; i++){
 	    //1. 기본 환경 설정
 	    ComConfigUpload(uploadObjects[i], "/hanjin/VOP_VSK_2507GS.do");	
 	}    
 ...
 function initSheet(sheetObj, sheetNo) {
 	...
 	with (sheetObj) {
 		...
 		case "sheet1": 
 			...
 			var HeadTitle1 = "|Sel.|Seq.|File Name|ID|Date|Download|port_cd|atch_file_seq|file-knt|file_path|file_sav_id";
            ...
            InitDataProperty(0, cnt++ , dtHidden,	    0,		daCenter,	true,		"atch_file_knt");
 ...
 function processButtonClick() {
 	...
 	switch (srcName) {	
         case "btn2_Row_Add":
            ...
            sheetObject.CellValue2(row,	"port_cd")          = form.port_cd.value;
 		...
 		case "btn1_Close":
 			//alert(sheetObjects[0].CellValue(1, "atch_file_knt"));
 			var editable	= window.form.editable.value;
 			
 			if(editable == "Y"){
 				var opener	= window.dialogArguments;
 				opener.setAtchFileKnt(sheetObjects[0].CellValue(1, "atch_file_knt"));
 				self.close();
 			}else{
 				self.close();
 			}
 ...
 function sheet1_OnPopupClick(sheet,row,col){
 	var fileName = sheet.OpenFileDialog(ComGetMsg("VSK57016","file."));		//Please select  {?msg1}.//
 	 ...
 	 switch (sAction) {
 	 	...
 	 	case IBSEARCH: //조회
 			////frm.f_cmd.value = SEARCH;	
 	 		...
 	 		//alert(formObj.port_cd.value);
 	 	
 			var sXml 	= sheetObj.GetSearchXml("VOP_VSK_2507GS.do", FormQueryString(formObj));
 	 	...
 	 	case IBSAVE: 
 	 		...
 	 		if (ComIsNull(saveString))  {			
 	 			ComShowCodeMessage("VSK57017");	//There is no data to save.//
 	 			return;
 	 		}
 	 		...
 	 		// Upload
	 	 		...
	 	 		// 일반
	 	 			...
	 	 			var sXml 	= sheetObj.GetSaveXml("VOP_VSK_2507GS.do",  param);			

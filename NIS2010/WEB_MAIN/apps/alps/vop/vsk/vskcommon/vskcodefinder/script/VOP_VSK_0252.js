...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class VOP_VSK_0252 : VOP_VSK_0252 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function VOP_VSK_0252() {
...
function loadPage() {
 ...
 function obj_keyup(){
 	...
 	try {
 		...
 		if( e == "[object Error]") {
 			ComShowCodeMessage('VSK00011');	//처리오류가 발생하였습니다.
 		} else {
 			ComShowMessage(e);
 		}
 	}
        ...
        case 1:      // sheet1 init
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
    			...
    			//InitDataProperty(0, cnt++ , dtRadioCheck,	0,    	daCenter,  	false,    	"radio"		);
    			//InitDataProperty(0, cnt++ , dtCheckBox,		0,		daCenter,	false,	  	"checkbox"	);
				InitDataProperty(0, cnt++ , dtData,			100,	daCenter,	false,		prefix+"code",						false,	"",		dfNone,			0,			true,		true);
	 			InitDataProperty(0, cnt++ , dtData,			200,	daLeft,		false,		prefix+"name",						false,	"",		dfNone,			0,			true,		true);
...
function doActionIBSheet(sheetObj,formObj,sAction) {
    ...
    switch(sAction) {
		...
		if(validateForm(sheetObj, formObj, sAction)){
			...
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0252GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
...
function sheet1_OnClick(sheetObj, Row, Col){
	window.returnValue = sheetObj.CellValue(sheetObj.SelectRow, 'sheet1_code');
}

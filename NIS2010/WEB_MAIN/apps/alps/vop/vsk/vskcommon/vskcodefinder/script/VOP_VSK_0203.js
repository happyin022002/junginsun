...
/**
 * @extends 
 * @class vop_vsk_0203 : vop_vsk_0203 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function vop_vsk_0203() {
...
function setComboObject(combo_obj) {  
 ...
 function loadPage() {
	  ...
	  //alert('\n\nloadPage  >> initControl = ['+initControl+']');
...
function initControl() {
	...
	axon_event.addListener			('click'	, 'chk_use_change'	, 'chk_use'	);
	axon_event.addListenerForm		('activate'	, 'obj_activate'	, form);
	axon_event.addListenerForm		('blur'		, 'obj_blur'		, form		);	
	axon_event.addListenerFormat  	('change'	, 'obj_change_dt' 	, form		);
...
function chk_use_change(){
	
	var is_chk_use	= document.form.chk_use.checked;
	
	if(is_chk_use){
		sheetObjects[0].ColHidden("sheet1_pf_svc_tp_cd")	= false;
		document.form.fm_dt.disabled						= false;
		document.form.to_dt.disabled						= false;
		document.getElementById("period").style.display		= "inline";
	}else{
		sheetObjects[0].ColHidden("sheet1_pf_svc_tp_cd")	= true;
		document.form.fm_dt.disabled						= true;
		document.form.to_dt.disabled						= true;
...
function obj_activate() {
	...
	switch(srcName){
		case "fm_dt":
		case "to_dt":
			ComClearSeparator(event.srcElement,"ymd","-"); // 입력시 마스크 안보이기
			event.srcElement.select();
			break;
	}
}
    ...
    switch(sAction) {
       ...
       case IBSEARCH:      // 조회
			...
			if (sheetObj.id == "sheet1") {				
				...
				//var sXml = sheetObj.GetSearchXml("VOP_VSK_0203GS.do?op_=0203", FormQueryString(formObj));
         ...
         case 1:      // sheet1 init
             with (sheetObj) {
					...
					//전체Edit 허용 여부 [선택, Default false]
					 ...
					 var HeadTitle = "|Lane|Lane Name|Service\nType|PFS Type";
					...
					InitDataProperty(0, cnt++ , dtHiddenStatus,	0,		daCenter,	false,	prefix+"ibflag",		false,	"",	dfNone,	0,	true,	true);
					InitDataProperty(0, cnt++ , dtData,			50,		daCenter,	false,	prefix+"vsl_slan_cd",	false,	"",	dfNone,	0,	true,	true);
					InitDataProperty(0, cnt++ , dtData,			200,	daLeft	,	false,	prefix+"vsl_slan_nm",	false,	"",	dfNone,	0,	true,	true);
					InitDataProperty(0, cnt++ , dtData,			50,		daCenter,	false,	prefix+"vsl_svc_tp_cd",	false,	"",	dfNone,	0,	true,	true);
					InitDataProperty(0, cnt++ , dtData,			50,		daCenter,	false,	prefix+"pf_svc_tp_cd",	false,	"",	dfNone,	0,	true,	true);
     ...
     switch(sAction) {
        case IBSEARCH:      //조회
        	  if(flag == "grid"){
	              ...
	              //sheetObj.DoSearch("VOP_VSK_0203GS.do?op_=0203",FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
	        	  sheetObj.DoSearch("VOP_VSK_0203GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet1_"));
        		  ...
        		  //var sXml = sheetObj.GetSearchXml("VOP_VSK_0203GS.do?op_=0203" , FormQueryString(formObj));
        		  var sXml = sheetObj.GetSearchXml("VOP_VSK_0203GS.do" , FormQueryString(formObj));
...
function sheet1_OnClick(sheetObj, Row, Col){
 ...
 function chk_use_OnClick(){
	 
	 alert('chk_use_OnClick');

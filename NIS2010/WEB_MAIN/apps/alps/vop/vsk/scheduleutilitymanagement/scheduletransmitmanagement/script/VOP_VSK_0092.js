...
/**
 * @extends
 * @class VOP_VSK_0092 : VOP_VSK_0092 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_VSK_0092() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: // sheet1 init
		with (sheetObj) {
			...
			var HeadTitle1 = "|Port|yd_cd|Terminal\nName|Lane|VVD|Vessel Name|OPR|Coastal SKD|Coastal SKD|Coastal SKD|ETA notice via Auto TLX|ETA notice via Auto TLX|ETA notice via Auto TLX|Sending date|Sent status|Auto TLX|Result|Result|Result|clpt_ind_seq|trsm_his_seq|tmp1|tmp2|tmp3|tmp4|tmp5|vsl_cd|skd_voy_no|skd_dir_cd|vsl_fax_no|vsl_tlx_no|vsl_eml|dflt_fax_imst_cd|dflt_tlx_imst_cd|trsm_ownr_cd|lane_pic_eml|trsm_purp_cd";
			var HeadTitle2 = "|Port|yd_cd|Terminal\nName|Lane|VVD|Vessel Name|OPR|ETA|ETB|ETD|ETA|ETB|ETD|Sending date|Sent status|Auto TLX|Time of adj. RPM|Old RPM|New RPM|clpt_ind_seq|trsm_his_seq|tmp1|tmp2|tmp3|tmp4|tmp5|vsl_cd|skd_voy_no|skd_dir_cd|vsl_fax_no|vsl_tlx_no|vsl_eml|dflt_fax_imst_cd|dflt_tlx_imst_cd|trsm_ownr_cd|lane_pic_eml|trsm_purp_cd";
			...
			InitDataProperty(0, cnt++ , dtHidden,		95,		daCenter,	true,	prefix+"ntc_eta_dt",			false,	"",			dfUserFormat2,	0,			true,		true);
			InitDataProperty(0, cnt++ , dtHidden,		95,		daCenter,	true,	prefix+"ntc_etb_dt",			false,	"",			dfUserFormat2,	0,			true,		true);
			InitDataProperty(0, cnt++ , dtHidden,		95,		daCenter,	true,	prefix+"ntc_etd_dt",			false,	"",			dfUserFormat2,	0,			true,		true);
			...
			InitDataProperty(0, cnt++ , dtHidden,		100,	daCenter,	true,	prefix+"rpm_adj_dt",			false,	"",			dfUserFormat2,	0,			true,		true);
			InitDataProperty(0, cnt++ , dtHidden,		70,		daRight,	true,	prefix+"org_rpm_pwr",			false,	"",			dfNone,			0,			true,		true);
			InitDataProperty(0, cnt++ , dtHidden,		70,		daRight,	true,	prefix+"crnt_rpm_pwr",			false,	"",			dfNone,			0,			true,		true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH: // 조회
		if(validateForm(sheetObj,formObj,sAction)){
			...
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0092GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
	...
	case SEARCH01: //Port Code Check
		if(validateForm(sheetObj,formObj,sAction)){
			...
			var sXml = sheetObj.GetSearchXml("VOP_VSK_0092GS.do", FormQueryString(formObj));
			return sXml;
		}
		break;
	...
	case IBSAVE:
		if(validateForm(sheetObj,formObj,sAction)){
			...
			var sXml = sheetObj.GetSaveXml("VOP_VSK_0092GS.do", SaveStr + "&"+FormQueryString(formObj)+"&" + ComGetPrefixParam(aryPrefix));
			ComOpenWait(false);
...
function validateForm(sheetObj,formObj,sAction){
	switch(sAction) {
		...
		case IBSEARCH:
			...
			if(!VskCheckSpecificMonthPeriod(formObj.fm_eta_dt, formObj.to_eta_dt, 1)){
...
function openAutoFaxPop(sheetObj, Row){
	var url = "/hanjin/VOP_VSK_0292.do?vslCd="
	...
	ComOpenPopup(url, 600, 325, '', '0,0', true, true);

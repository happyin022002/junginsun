...
/**
 * @extends 
 * @class VOP_VSK_0292 : VOP_VSK_0292 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function VOP_VSK_0292() {
     ...
     switch(sAction) {
         case IBSAVE:         //저장
    		...
    		if(validateForm(sheetObj,formObj,sAction)){
				...
				var rXml = sheetObj.GetSaveXml("VOP_VSK_0292GS.do", FormQueryString(formObj));
				if (ComGetEtcData(rXml,"TRANS_RESULT_KEY") == "S"){
					ComShowCodeMessage('VSK02003');		//"Sked change notice has been sent successfully."
					...
					ComShowCodeMessage('VSK02004');		//"Sked change notice has not been sent, Please Try Again !"
				}     			
	    		ComOpenWait(false);
    		}
	    	break;

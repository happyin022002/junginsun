...
msgs["VSK00018"] = "In case of EGSUZ port, period should be set within one month.";
msgs["VSK00019"] = "Only PAPAC is available"; 
...
msgs['VSK00032'] = 'Port ({?msg1}) - Please input the exact ETA, ETB, ETD.';
...
msgs['VSK00044'] = 'Please input the remark for specific Delay Reason.';
...
msgs['VSK00055'] = "This Port ATA({?msg1}) is faster than \nETD({?msg2}) of previous port [{?msg3}].";
msgs['VSK00056'] = "ATB ({?msg1}) is faster than ATA ({?msg2}).";
msgs['VSK00057'] = "ATD ({?msg1}) is faster than ATB ({?msg2}).";
msgs['VSK00058'] = "This Port ATD({?msg1}) is faster than \nETA({?msg2}) of next Port [{?msg3}].";
...
msgs["VSK00098"] = '({?msg1}) row(s), VVD({?msg2}) - Please input the exact ETA, ETB, ETD.';
...
msgs["VSK00107"] = "It must be less than or equal to Proforma Distance({?msg1}).";
msgs["VSK00108"] = "Please check the updated schedule.{?msg1}.";
...
msgs['VSK01021'] = 'Schedule is reversed. Please, check the below information. \n\n{?msg1}\n{?msg2}';
msgs['VSK01022'] = 'Schedule is reversed with Next Port SKD. \n({?msg1})'
msgs['VSK01023'] = 'Schedule is reversed with Previous and Next Port SKD. \n({?msg1})\n({?msg2})'

msgs['VSK02001'] = 'Sked change notice has been sent successfully.';
msgs['VSK02002'] = 'Sked change notice has not been sent, Please Try Again !';
msgs['VSK02003'] = 'Pre Stowage Plan notice has been sent successfully.';
msgs['VSK02004'] = 'Pre Stowage Plan notice has not been sent, Please Try Again !';
msgs['VSK02005'] = 'Pls input hours as negative number for advance';
msgs['VSK02006'] = 'Pls input hours as positive number for delay';
msgs['VSK02007'] = 'From port and To port may not be the same. ';
msgs['VSK02008'] = 'There is no changed data';
msgs['VSK02009'] = 'Data is Changed successfully';

msgs['VSK02010'] = 'Min Range can not be greater than Max Range.';
msgs['VSK02011'] = 'From and To port yard group may not be the same.';
msgs['VSK02012'] = 'If from and to port are equal, yard group cannot be the All status.';


// VOSI이전관련 OPF UI메시지 변환

msgs['VSK50003'] = '{?msg1} is not available.';
...
msgs['VSK57016'] = "Please select  {?msg1}.";
msgs['VSK57017'] = 'There is no data to save.';
msgs['VSK57018'] = '({?msg2}) must be bigger than ({?msg1}) for {?msg3}.';
msgs['VSK57019'] = 'There is no data to send to EDI';

msgs['VSK57020'] = 'EDI transmitted successfully';
msgs['VSK57021'] = 'VVD ({?msg1}) will be transmitted through EDI.\n\n Do you want to proceed?';
msgs['VSK57022'] = 'This function is under development.';
msgs['VSK57023'] = "Can't use function of Yard Change due to Doulbe calling same port\nand please use function of 'Reverse Call'.";
msgs['VSK57024'] = "Can't use function of Yard Change due to Doulbe calling same port.";
msgs['VSK57025'] = "Please check the entered schedules are correct.";
msgs['VSK57026'] = "Vessel will be phased out from this lane within 1 month.\nPls use \"Phase out\" Button and input phase out reason.";
msgs['VSK57027'] = 'Please input the Skip Reason Code of ({?msg1}).';
...
*/
/*
function VskXml2ComboString(xmlStr, codeCol, textCol) {
...
* 	new Array("MNR_GEN_CD","MNR_CD_ID","MNR_CD_DP_DESC","PRNT_CD_ID","CD00001","MNR_CD_DP_SEQ"),    
* 	new Array("MNR_GEN_CD","MNR_CD_ID","MNR_CD_DP_DESC","PRNT_CD_ID","CD00001","MNR_CD_DP_SEQ")
* )  
* MNR_GEN_CD    ==> 서버에서 조회할 테이블의 테이블명 
* MNR_CD_ID     ==> 서버에서 조회할 테이블의 코드컬럼명
* PRNT_CD_ID    ==> 서버에서 조회할 테이블의 키값이 되는 컬럼명 where PRNT_CD_ID = 'CD00001' 로 들어감 
* CD00001       ==> 서버에서 조회할 테이블의 키값의 값 위에 'CD00001' 에 매칭되는값 
* MNR_CD_DP_SEQ ==> 서버에서 조회할 테이블의 SORT할 컬럼명 
* 
* RETURN VALUE  
...
*/ 
/*
function VskComSearchCombo(sheetObj,sCondition){
	var f_query = '';
	//쿼리 스트링 조합시작    
	...
	for (var i = 0;i < sCondition.length; i++){
		f_query += 'ibflag=X&';
		f_query += 'del_chk=0&';
		f_query += 'table_nm' + '=' + sCondition[i][0] + '&';	
		f_query += 'cd_col_nm' + '=' + sCondition[i][1] + '&';
		f_query += 'desc_col_nm' + '=' + sCondition[i][2] + '&';
		f_query += 'prnt_col_nm' + '=' + sCondition[i][3] + '&';
		f_query += 'prnt_col_value' + '=' + sCondition[i][4] + '&';  
		f_query += 'order_by_col_nm' + '=' + sCondition[i][5] + '&';
	}	
	//마지막에 &를 없애기 위함    
	if (f_query != "")
      f_query = f_query.substr(0, f_query.length -1);	
	
	var sXml = sheetObj.GetSearchXml("VSK_COMGS.do","" ,f_query,true);
	var arrXml = sXml.split("|$$|");
	
	var retValue = new Array();
	for(var i = 0; i < arrXml.length; i++){   
		retValue[i] =  VskXml2ComboString(arrXml[i], "cd_id", "cd_desc");
	}   
...
*/
/*
function VskMakeComboObject(comboObj, arrStrNm, arrStrCd) {
...
* @return
*/
function VskCheckSpecificMonthPeriod(fmDtObj, toDtObj, monthCount){
	var fmDt 	= ComReplaceStr(fmDtObj.value, "-", "");
	var toDt 	= ComReplaceStr(toDtObj.value, "-", "");

...
/*****Case more than two additional sheets tab sheet is used to specify a variable *****/
        ...
        try {
        	...
        	switch(srcName) {
        	case "btn_History":
	        	var tblNo = 'MDM_CNTR_SZ';
	        	var cntrSzCd = formObj.cntr_sz_cd.value;
	        	var mstKey = nullToBlank(cntrSzCd);
	        	if (mstKey == "") {
					ComShowCodeMessage("CCD00038", "Container Size");
					return false;
				}
...
//				formObj.srep_cd.focus();
    		...
    		case "input_seach_btn":
    			ComOpenPopup('/hanjin/COM_COM_0003.do?mdm_yn='+formObj.mdm_yn.value, 600, 430, 'getMdmCntrSz', "1,0,1", true);
    			break;				
            }
        }catch(e) {
            if( e == "[object Error]") {
		        ...
		        case "sheet1":   //sheet1 init
		            with (sheetObj) {
		              ...
		              //전체Merge 종류 [선택, Default msNone]
						...
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_sz_desc");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_sz_teu_capa");
	    ...
	    function doActionIBSheet(sheetObj,formObj,sAction) {
	    	...
	    	case SEARCH:				//Retrieve
	    		...
	    		if (validateForm(sheetObj, formObj, sAction)) {
	    		    ...
	    		    var sXml = sheetObj.GetSearchXml("BCM_CCD_0006GS.do", FormQueryString(formObj));				
	    			...
	    			formObj.cntr_sz_cd.value=sheetObj.CellValue(1, "cntr_sz_cd");
	    			if (formObj.cntr_sz_cd.value.length != 0){
	    				formObj.cntr_sz_desc.value=sheetObj.CellValue(1, "cntr_sz_desc");
	    				formObj.cntr_sz_teu_capa.value=sheetObj.CellValue(1, "cntr_sz_teu_capa");
			    		...
			    		formObj.cntr_sz_cd.readOnly=true;
			    		formObj.cntr_sz_cd.style.backgroundColor="#bebebe";
			    		ComBtnEnable("btn_Save");
	    			} else {
	    		...
	    		if (validateForm(sheetObj, formObj, sAction)) {
	    			formObj.f_cmd.value=MULTI;
	          	    ...
	          	    if(ComShowCodeConfirm("COM130101", "Data")){
		    			...
		    			var SaveXml = sheetObj.GetSaveXml("BCM_CCD_0006GS.do", sParam);
	    	...
	    	case IBCLEAR:
	    		formObj.reset();
	    		formObj.cntr_sz_cd.readOnly=false;		
	    		formObj.ibflag.value = "I";
	    		//formObj.creflag.value = "Y";

	    		formObj.cntr_sz_cd.className= "input1";
	    		formObj.cntr_sz_cd.style.backgroundColor="#d4f6ff";
	    	...
	    	case SEARCH02:      //Container Type Code check
				if(validateForm(sheetObj,formObj,sAction)){
					...
					var sParam=FormQueryString(formObj);
					var sXml=sheetObj.GetSearchXml("BCM_CCD_0006GS.do", sParam);
			        var result=ComGetEtcData(sXml, "result");
			        if(result != ""){
			        	ComShowCodeMessage("CCD00004", "Container Size Code");
			        	formObj.cntr_sz_cd.value="";
			        }
					ComOpenWait(false);
				}
				break;
	...
	var srcObj;
  	...
  	switch(srcName) {
	  	case "delt_flg":
			...
			break;
	  	case "cntr_sz_cd":
	  		if(formObj.cntr_sz_cd.value.length>0 && formObj.creflag.value == 'Y'){
       			doActionIBSheet(sheetObjects[0], formObj, SEARCH02);
    			if(formObj.cntr_sz_cd.value.length==0){
					document.form.cntr_sz_cd.focus();
    			}else{
    				document.form.cntr_sz_teu_capa.focus();
    			}
       		} else if (formObj.cntr_sz_cd.value.length>0) {
...
function getMdmCntrSz(rowArray) {
	...
	formObj.cntr_sz_cd.value=colArray[3];		

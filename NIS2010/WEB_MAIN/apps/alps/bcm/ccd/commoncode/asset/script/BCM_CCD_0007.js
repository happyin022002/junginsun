 ...
 ***************************************************************************************/
    /**
     * @extends 
     * @class BCM_CCD_0007 : BCM_CCD_0007 on the screen for creating the script defines the task using.
     */
...
/*****Case more than two additional sheets tab sheet is used to specify a variable *****/
        ...
        try {
        	...
        	switch(srcName) {
        	case "btn_History":
	        	var tblNo = 'MDM_CNTR_TP_SZ';
	        	var cntrTpszCd = formObj.cntr_tpsz_cd.value;
	        	var mstKey = nullToBlank(cntrTpszCd);
	        	if (mstKey == "") {
					ComShowCodeMessage("CCD00038", "Container Type Size");
					return false;
				}
...
//				formObj.srep_cd.focus();
            ...
            case "btn_Create":
 				doActionIBSheet(sheetObject,formObj,IBCLEAR);
 				formObj.cntr_tpsz_cd.readOnly=true;
	    		formObj.cntr_tpsz_cd.style.backgroundColor="#bebebe";
 				formObj.creflag.value="Y";
 				formObj.cntr_tpsz_desc.focus();
 				ComBtnEnable("btn_Save");
 				break;
    		case "input_seach_btn":
    			ComOpenPopup('/hanjin/COM_ENS_0G1.do?mdm_yn='+formObj.mdm_yn.value, 600, 430, 'getCntr_tpsz_cd', "1,0,1", true);
    			break;				
            }
        }catch(e) {
            if( e == "[object Error]") {
	 ...
	 **/
	 function initComboSetVal(sheetObj,formObj){
	 	...
	 	var sXml = sheetObj.GetSearchXml("BCM_CCD_0007GS.do", FormQueryString(formObj));
		
		var arrXml = sXml.split("|$$|");
		if (arrXml.length > 0) 
			ComXml2ComboItem(arrXml[0], formObj.cntr_tp_cd, "cd", "cd|cd_desc");
		if (arrXml.length > 1) 
			ComXml2ComboItem(arrXml[1], formObj.cntr_sz_cd, "cd", "cd|cd_desc");
		if (arrXml.length > 2) 
			ComXml2ComboItem(arrXml[2], formObj.cntr_tpsz_iso_cd, "cd", "cd|cd_desc");
		if (arrXml.length > 3) 
			ComXml2ComboItem(arrXml[3], formObj.cntr_tpsz_psa_cd, "cd", "cd|cd_desc");
		if (arrXml.length > 4) 
			ComXml2ComboItem(arrXml[4], formObj.cntr_tpsz_grp_cd, "cd", "cd|cd_desc");
	 }
      ...
      */
      function initCombo(comboObj, comboId) {
    	  if (comboId == "cntr_tp_cd"){
        	  comboObj.BackColor = "#CCFFFD";
    	  }
    	  if (comboId == "cntr_sz_cd"){
        	  comboObj.BackColor = "#CCFFFD";
    	  }
		        ...
		        case "sheet1":   //sheet1 init
		            with (sheetObj) {
		              ...
		              //전체Merge 종류 [선택, Default msNone]
						...
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_desc");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_rmk");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tp_cd");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_sz_cd");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_lodg_wgt");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_lodg_capa");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_tare_wgt");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_psa_cd");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_iso_cd");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "cntr_tpsz_grp_cd");
						InitDataProperty(0, cnt++ , dtData,         40,    daCenter,  false,   "aciac_div_cd");
	    ...
	    function doActionIBSheet(sheetObj,formObj,sAction) {
	    	...
	    	case SEARCH:				//Retrieve
	    		...
	    		if (validateForm(sheetObj, formObj, sAction)) {
	    			...
	    			formObj.cntr_tpsz_cd.value=sheetObj.CellValue(1, "cntr_tpsz_cd");
	    			if (formObj.cntr_tpsz_cd.value.length != 0){
	    				formObj.cntr_tpsz_desc.value=sheetObj.CellValue(1, "cntr_tpsz_desc");
	    				formObj.cntr_tpsz_rmk.value=sheetObj.CellValue(1, "cntr_tpsz_rmk");
	    				formObj.cntr_tpsz_lodg_wgt.value=sheetObj.CellValue(1, "cntr_tpsz_lodg_wgt");
	    				formObj.cntr_tpsz_lodg_capa.value=sheetObj.CellValue(1, "cntr_tpsz_lodg_capa");
	    				formObj.cntr_tpsz_tare_wgt.value=sheetObj.CellValue(1, "cntr_tpsz_tare_wgt");

	    				ComSetObjValue(formObj.cntr_tp_cd,sheetObj.CellValue(1, "cntr_tp_cd"));
	    				ComSetObjValue(formObj.cntr_sz_cd,sheetObj.CellValue(1, "cntr_sz_cd"));
	    				ComSetObjValue(formObj.cntr_tpsz_psa_cd,sheetObj.CellValue(1, "cntr_tpsz_psa_cd"));
	    				ComSetObjValue(formObj.cntr_tpsz_iso_cd,sheetObj.CellValue(1, "cntr_tpsz_iso_cd"));
	    				ComSetObjValue(formObj.cntr_tpsz_grp_cd,sheetObj.CellValue(1, "cntr_tpsz_grp_cd"));
	    		...
	    		if (validateForm(sheetObj, formObj, sAction)) {
	    			formObj.f_cmd.value=MULTI;
	          	    ...
	          	    if(ComShowCodeConfirm("COM130101", "Data")){
		    			...
		    			var SaveXml = sheetObj.GetSaveXml("BCM_CCD_0007GS.do", sParam);
	    	...
	    	case IBCLEAR:
	    		formObj.reset();
	    		formObj.cntr_tpsz_cd.readOnly=false;		
	    		formObj.ibflag.value = "I";
	    		ComSetObjValue(formObj.cntr_tp_cd,"");
				ComSetObjValue(formObj.cntr_sz_cd,"");
				ComSetObjValue(formObj.cntr_tpsz_psa_cd,"");
				ComSetObjValue(formObj.cntr_tpsz_iso_cd,"");
				ComSetObjValue(formObj.cntr_tpsz_grp_cd,"");
	    		//formObj.creflag.value = "Y";

	    		formObj.cntr_tpsz_cd.className= "input1";
	    		formObj.cntr_tpsz_cd.style.backgroundColor="#d4f6ff";
	    	...
	    	case SEARCH02:      //Container Type Code check
				if(validateForm(sheetObj,formObj,sAction)){
					...
					formObj.cntr_tpsz_cd.value=ComGetObjValue(formObj.cntr_tp_cd)+ComGetObjValue(formObj.cntr_sz_cd);
					alert(formObj.cntr_tpsz_cd.value);
					var sParam=FormQueryString(formObj);
					var sXml=sheetObj.GetSearchXml("BCM_CCD_0007GS.do", sParam);
			        var result=ComGetEtcData(sXml, "result");
			        if(result != ""){
			        	ComShowCodeMessage("CCD00004", "Container Type Size Code");
			        	formObj.cntr_tpsz_cd.value="";
			        } else {
			        	formObj.cntr_tpsz_cd.value= result;
			        }
					ComOpenWait(false);
				}
				break;
...
function validateForm(sheetObj, formObj, sAction) {
     with(formObj){
         switch ( sAction ) {
             case SEARCH:
                 if (formObj.cntr_tpsz_cd.value.length == 0){
                	 ComShowCodeMessage("CCD00001", "Container Type Size Code");
                     formObj.cntr_tpsz_cd.focus();
                     return false;
                 }
                 break;
             case MULTI:  
            	 if (ComGetObjValue(formObj.cntr_sz_cd) == "" || ComGetObjValue(formObj.cntr_sz_cd) == null){
            	 	 ComShowCodeMessage("CCD00001", "Container Size Code");
                     formObj.cntr_sz_cd.focus();
                     return false;
                 } else if (formObj.cntr_tpsz_desc.value.length == 0){
                	 ComShowCodeMessage("CCD00001", "Container Type Size Name");
                	 formObj.cntr_tpsz_desc.focus();
                	 alert(3);
                	 return false;
                	 ...
                	 return false;
         		 } else if (ComGetObjValue(formObj.cntr_tp_cd) == "" || ComGetObjValue(formObj.cntr_tp_cd) == null){
            	 ...
            	 if(formObj.creflag.value == 'Y'){
            		/*doActionIBSheet(sheetObjects[0], formObj, SEARCH02);
         			if(formObj.cntr_tpsz_cd.value.length==0){
         				...
         				document.form.cntr_tpsz_desc.focus();
         			}*/
            		ComOpenWait(true);
 					formObj.f_cmd.value=SEARCH02;
 					formObj.cntr_tpsz_cd.value=ComGetObjValue(formObj.cntr_tp_cd)+ComGetObjValue(formObj.cntr_sz_cd);
	...
	var srcObj;
  	...
  	switch(srcName) {
	  	case "delt_flg":
			...
			break;
	  	case "cntr_tpsz_cd":
	  		if (formObj.cntr_tpsz_cd.value.length>0 && formObj.creflag.value != 'Y') {

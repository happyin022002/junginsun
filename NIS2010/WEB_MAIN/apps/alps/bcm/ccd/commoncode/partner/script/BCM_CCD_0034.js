 ...
 ***************************************************************************************/
	...
	function processButtonClick() {
		...
		try {
			...
			switch (srcName) {
			case "btn_History":
				var tblNo = 'MDM_CARRIER';
	        	var crrCd = formObject.crr_cd.value;
	        	var mstKey = nullToBlank(crrCd);
				if (mstKey == "") {
					ComShowCodeMessage("CCD00038", "Carrier Code");
					return false;
				}
			...
			case "btn_search":
	    		...
	    		ComOpenPopup('/hanjin/COM_ENS_0N1.do?' + param, 620, 500, 'setCallBack0B2', '1,0,1,1,1,1,1,1', true);
				break;
			} 
		} catch (e) {
			if (e == "[object Error]") {
...
//	 } 
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case "sheet1":
		    with(sheetObj){
				...
				//2018.04.27 Min 주석처리 및 초기화
				  /*
			      if (location.hostname != "")
			      var HeadTitle1="|ibflag|deltflg|crrCd|crrNm|bkgEdiCd";
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		case IBSEARCH: //Retrieve
			...
			var sXml=sheetObj.GetSearchXml("BCM_CCD_0034GS.do", sParam);
	        ...
	        if(crr_nm == ""){
    	    	...
    	    	*/
	        	...
	        	//formObj.crr_cd.style.backgroundColor="#E8E7EC";
		...
		case IBSAVE:        //Save
			...
			if(formObj.creflag.value != "N" && formObj.rqst_no.value == ''){
 			...
 			if(ComShowConfirm(ComGetMsg(tmpMsg,"Data"))){
     			ComOpenWait(true);
     			var sXml=sheetObj.GetSaveXml("BCM_CCD_0034GS.do", sParam + "&" + ComGetPrefixParam("sheet1_"));
     			...
     			if(formObj.creflag.value != "N" && formObj.rqst_no.value == ''){
					ComShowCodeMessage("CCD00060", "generated");
				} else {
    ...
    function validateForm(sheetObj, formObj, sAction) {
      switch (sAction) {
		case IBSEARCH:
			...
			//Min 
			//alert("formObj.rqst_no.value : " + formObj.rqst_no.value);
			...
			if(formObj.rqst_no.value == ""){
	    		...
	    		if(formObj.crr_cd.value != "" && formObj.crr_cd.value.length != 3){
	    			ComShowCodeMessage("CCD00001", "Carrier Code");	//문구 변경가능하면 교체하는 방향으로....
	    			return false;
	    		}
			}
    		break;

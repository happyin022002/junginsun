 ...
 ***************************************************************************************/
    ...
    function processButtonClick(){
        ...
        try {
            ...
            switch(srcName) {
            case "btn_History":
            	var tblNo = 'MDM_SUB_TRD';
	        	var subTrdCd = formObj.sub_trd_cd.value;
	        	var mstKey = nullToBlank(subTrdCd);
	        	
				if (mstKey == "") {
					ComShowCodeMessage("CCD00038", "Sub Trade Code");
					return false;
				}
            ...
            case "btn_Save":
            	...
            	break;
			case "btns_subtrade":
 		        //var sUrl="/hanjin/COM_COM_0013.do?code=" + formObj.sub_trd_cd.value + "&main_page=" + "false"+ "&mdm_yn=" + "Y";
				var sUrl="/hanjin/COM_ENS_081.do?sub_trade_cd=" + formObj.sub_trd_cd.value +"&main_page=false&mode=rev";
 	...
 	function doActionIBCombo(sheetObj,formObj,sAction,sComboObj,sComboAction,sComboKey){
 		switch (sAction) {
 			case SEARCH01: // load page 시
 				...
 				var sXml=sheetObj.GetSearchXml("BCM_CCD_0031GS.do", "f_cmd=" + SEARCH01);
	 			...
	 			for(var i=0; i<rtnValue.length; i++){
	 				...
	 				if(i==0){
	 					var tempCdName = "";
		 				for(k=0; k<cdName.length; k++){
		 					
	 						if(cdName[k].trim() != "")
	 							tempCdName += cdValue[k] + " - " + cdName[k];
	 						else 
	 							tempCdName += "";
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
    	...
    	case SEARCH02:				//query
    		if (validateForm(sheetObj, formObj, sAction)) {
	    		...
	    		var sXml=sheetObj.GetSearchXml("BCM_CCD_0031GS.do", sParam);
	    		...
	    		if(ComXmlString(sXml, "delt_flg") == null || ComXmlString(sXml, "delt_flg") == ""){
	    			...
	    			//Min 2018.05.16 코드 생성권한은 누구한테 줘야 하나?...
...
//       		    			formObj.sub_trd_nm.focus();
	    			...
	    			//ComShowConfirm(ComGetMsg("CCD00034", "Revenue Lane Code"))
	    			//if (ComShowCodeConfirm("COM130407", "Sub-Trade Code")) {
	    			if(ComShowConfirm(ComGetMsg("CCD00034", "Sub-Trade Code"))){
	    		...
	    		formObj.trd_cd.Code = getValueForCombo(ComXmlString(sXml, "trd_cd"));
    	...
    	case MULTI01:				//save
    		if (validateForm(sheetObj, formObj, sAction)) {
	    		...
	    		if(ComShowConfirm(ComGetMsg("COM130101", "Data"))){
 	    		...
 	    		var sParam=FormQueryString(formObj);
 	    		var sXml=sheetObj.GetSaveXml("BCM_CCD_0031GS.do", sParam);
 	    		var sav=ComGetEtcData(sXml, "TRANS_RESULT_KEY");
   ...
   function get_subTrdCd(rowArray) {
	   ...
	   var sheetObj=sheetObjects[0];
	    var formObj=document.form;
	   	...
	   	formObj.sub_trd_cd.value=colArray[4];

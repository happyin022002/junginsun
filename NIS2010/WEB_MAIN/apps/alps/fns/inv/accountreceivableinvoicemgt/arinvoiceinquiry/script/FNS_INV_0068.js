...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class FNS_INV_0068 : FNS_INV_0068 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function FNS_INV_0068() {
		...
		switch(sheetNo) {
			case 1:      //t1sheet1 init
				with (sheetObj) {
					...
					var HeadTitle = "|RHQ|Office|CUST Code|Cust Name|B/L No.|BKG No.|S/C No.|VVD|Good Date|S/A Date|IF Date|IF No.|POL|POD|SRC CUR|SRC AMT|USD AMT";
					...
					InitDataProperty(rowCnt, cnt++ , dtData,			60,		daCenter,	true,		"sc_no",			false,          "",      dfNone);
  	...
  	function initCombo(comboObj, comboNo) {
		switch (comboObj.id) {
			...
			case "chg_cd":
				with (comboObj) {
					InsertItem(0, "All",    "A");
		            InsertItem(1, "GCF",    "GCF");
		            InsertItem(2, "SAF",    "SAF");
		            InsertItem(3, "SAC",    "SAC");
			...
			case IBSEARCH:      //조회
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					var arrStr2 = formObj.ar_hd_qtr_ofc_cd.Code;
	     			...
	     			sheetObj.DoSearch("FNS_INV_0068GS.do", FormQueryString(formObj));
				}
			break;
			
			case IBSEARCH_ASYNC10: // 화면 로딩시 AR Office 조회
				formObj.f_cmd.value = SEARCH01;
 				var sXml = sheetObj.GetSearchXml("FNS_INV_0068GS.do", FormQueryString(formObj));

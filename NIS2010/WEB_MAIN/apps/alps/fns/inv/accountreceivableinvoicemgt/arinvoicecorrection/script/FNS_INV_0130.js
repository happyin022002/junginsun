...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class FNS_INV_0130 : FNS_INV_0130 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function FNS_INV_0130() {
	...
	function processButtonClick(){
		...
		try {
			...
			case "btn_retrive":
				...
				formObject.ar_ofc_cd.enable = false;
				
				if (formObject.ofc.value == 'SAOSC') {
					formObject.due_dt.disabled = false;
					document.getElementById('dueDt').style.display = 'Inline';
					document.getElementById('bankAccTable1').style.display = 'Inline';
					document.getElementById('bankAccTable2').style.display = 'Inline';
					document.getElementById('bankAccTable3').style.display = 'Inline';
					document.getElementById('bankAccTable4').style.display = 'Inline';
					document.getElementById('bankAccTable5').style.display = 'Inline';

					
				} else {
					formObject.due_dt.disabled = true;
					document.getElementById('dueDt').style.display = 'none';
					document.getElementById('bankAccTable1').style.display = 'none';
					document.getElementById('bankAccTable2').style.display = 'none';
					document.getElementById('bankAccTable3').style.display = 'none';
					document.getElementById('bankAccTable4').style.display = 'none';
					document.getElementById('bankAccTable5').style.display = 'none';
			...
			case "btn_new":
				...
				formObject.frm_bl_src_no.readOnly=false;
				formObject.frm_bkg_no.readOnly=false;
				formObject.split_cnt.readOnly=false;					
				//formObject.if_no.className = "input1";
				formObject.frm_bl_src_no.className = "input1";
				formObject.frm_bkg_no.className = "input1";
				formObject.split_cnt.className = "input1";
				//formObject.if_no.focus();
				formObject.frm_bl_src_no.focus();
				formObject.split_cnt.value = "2"
			
			    // 탭 오브젝트를 삭제한다.
				for (var i =0; i < formObject.split_cnt.value; i++) {
					//formObject.tabLayer[i].style.display = "none";
					var objs = document.all.item("tabLayer");
					objs[i].style.display = "none";
				}
				formObject.ar_ofc_cd.enable = true;
				break;
			...
			case "btns_calendar": //달력버튼
				...
				cal.select(formObject.due_dt, 'yyyy-MM-dd');
				break;
			...
			case "btn_rfano":
				ComOpenPopup('/hanjin/FNS_INV_0090.do?pgmNo=FNS_INV_0090&rfa_no='+formObject.frm_rfa_no.value, 600, 366, '', '0,0');  
				break; 
			case "btn_scno":
				ComOpenPopup('/hanjin/FNS_INV_0091.do?pgmNo=FNS_INV_0091&sc_no='+formObject.frm_sc_no.value, 600, 366, '', '0,0');  
				break; 
	...
	function loadPage() {
		...
		//form.if_no.focus();
		form.frm_bl_src_no.focus();
		
		form.split_cnt.value = "2"
		

		// SAOSC 에만 보여줄 테이블을 숨긴다.	
		if (form.ofc.value == 'SAOSC') {
			form.due_dt.disabled = false;
			document.getElementById('dueDt').style.display = 'Inline';
			document.getElementById('bankAccTable1').style.display = 'Inline';
			document.getElementById('bankAccTable2').style.display = 'Inline';
			document.getElementById('bankAccTable3').style.display = 'Inline';
	...
	function initSheet(sheetObj,sheetNo) {
		...
		with (sheetObj) {
			...
			var HeadTitle = "|ar_if_no|act_cust_cnt_cd|act_cust_seq|bkg_teu_qty|bkg_feu_qty|inv_ref_no|hjs_stf_ctnt|inv_rmk|io_bnd_cd|sail_dt|cust_cd|bank_acct_prn_opt_cd|due_dt";
			...
			InitDataProperty(0, cnt++ , dtData,  90,    daCenter,  true,    "bank_acct_prn_opt_cd");
			InitDataProperty(0, cnt++ , dtData,  90,    daCenter,  true,    "due_dt");
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		if(validateForm(sheetObj,formObj,sAction)){
			...
			for(var i=4;i<sheetObjects.length;i++){
				ComConfigSheet (sheetObjects[i] );	
				initSheet(sheetObjects[i],i+1);
				ComEndConfigSheet(sheetObjects[i]);	
			}
			...
			var sXml = sheetObj.GetSearchXml("FNS_INV_0130GS.do" , FormQueryString(formObj));					
	
			var arrXml = sXml.split("|$$|");
	
			var chg_size = Number(ComGetEtcData(arrXml[0],"chg_size"));
			if(chg_size<document.form.split_cnt.value) {
				ComShowCodeMessage("INV00160");
				ComBtnDisable("btn_save");
				ComOpenWait(false);  
				return;
			}
			...
			formObj.due_dt.value = sheetObjects[0].CellText(1, "due_dt");
			...
			document.form.ar_if_no.value = sheetObjects[0].CellValue(1,"ar_if_no");	
				...
				for(cnt = 0;cnt < split_cnt; cnt++){
					for(var i=0;i<sheetObjects.length;i++){
						if(sheetObjects[i].id=="t"+cnt+"sheet1"){
							...
							for(var k=1;k<=sheetObjects[i].RowCount;k++){
					...
					document.form.inv_rmk[cnt].value = sheetObjects[0].CellValue(1,"inv_rmk");	

					document.form.hjs_stf_ctnt[cnt].value = document.form.user_nm.value;	
					for(var x=1;x<sheetObjects[2].RowCount+1;x++){
					...
					for(var i=0;i<sheetObjects.length;i++){
						if(sheetObjects[i].id=="t"+cnt+"sheet1"){
							if (cnt > 0) {
								for(var k=1;k<=chg_size;k++){
									// 탭의 chg를 0으로 초기화 한다.
									sheetObjects[i].CellValue2(k,"trf_rt_amt") = 0;
									sheetObjects[i].CellValue2(k,"chg_amt") = 0;
								}	
								reSum(sheetObjects[i]);
							}
						}
					}
				}
			...
			// OFC_CD가 SAOSC일 경우에 Bank Account을 Disable시킨다.
			var ofcCd = formObj.ofc.value;
			var split_cnt = Number(document.form.split_cnt.value);
			if (ofcCd == "SAOSC") {		
				for (var cnt = 1; cnt < split_cnt+1; cnt++) {
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[0].disabled = false;
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[1].disabled = false;
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[2].disabled = false;
				}			
			} else {			
				for (var cnt = 1; cnt < split_cnt+1; cnt++) {
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[0].disabled = true;
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[1].disabled = true;
					document.getElementsByName('bank_acct_prn_opt_cd'+cnt)[2].disabled = true;
				}
			}
			
			
			// 체크가 정상이면 커스터머 쪽을 활성화 한다.
			for (var i =0; i < formObj.split_cnt.value; i++) {
				formObj.act_cust_cnt_cd[i].readOnly=false;
				formObj.act_cust_seq[i].readOnly=false;
				
				formObj.act_cust_cnt_cd[i].className="input1";
				formObj.act_cust_seq[i].className="input1";					
		...
		if(formObj.split_cnt.value != ""){
			for(cnt = 0;cnt < formObj.split_cnt.value; cnt++){
				...
				for (var rCnt = 0; rCnt <3; rCnt++) {
					if (document.getElementsByName('bank_acct_prn_opt_cd'+(cnt+1))[rCnt].checked) {
						sheetObjects[9].CellValue(cnt+1,"bank_acct_prn_opt_cd") = document.getElementsByName('bank_acct_prn_opt_cd'+(cnt+1))[rCnt].value;
					}					
				}
					...
					if(sheetObjects[i].id=="t"+cnt+"sheet1"){
						...
						for(c = 1; c<=sheetObjects[i].RowCount;c++){
							if(sheetObjects[i].CellValue(c,"chg_amt")!="0"&&sheetObjects[i].CellValue(c,"Seq")!=""){										
								...
								if(prev_ser_no!=sheetObjects[i].CellValue(c,"ar_if_ser_no")){
									serNo = serNo+1;
									arIfChgSeq = 1;	
								...
								prev_ser_no = sheetObjects[i].CellValue(c,"ar_if_ser_no");
		...
		if(validateForm(sheetObj,formObj,sAction)){
			...
			// 체크가 정상이면 커스터머 쪽을 비활성화 한다.
			for (var i =0; i < formObj.split_cnt.value; i++) {
				formObj.act_cust_cnt_cd[i].readOnly=true;
				formObj.act_cust_seq[i].readOnly=true;
				
				formObj.act_cust_cnt_cd[i].className="input2";
				formObj.act_cust_seq[i].className="input2";					
			}
			...
			var sXml = sheetObj.GetSearchXml("FNS_INV_0130GS.do", FormQueryString(formObj)+ "&" + ComSetPrifix(sheetObjects[9].GetSaveString(true),"s0sheet1_")+ "&" + ComSetPrifix(sheetObjects[10].GetSaveString(true),"s0sheet2_")+ "&" + ComSetPrifix(sheetObjects[11].GetSaveString(true),"s0sheet3_")+ "&" + ComSetPrifix(sheetObjects[12].GetSaveString(true),"s0sheet4_") ,"");
			...
			// invNo를 취득해서 화면에 저장해두는걸로 변환해야 한다.
			invNosXml = sXml;

			var sStr = ComGetEtcData(sXml,"inv_nos");	
			formObj.f_inv.value = sStr; 
			...
			formObj.frm_bl_src_no.readOnly=true;
			formObj.frm_bkg_no.readOnly=true;
			formObj.split_cnt.readOnly=true;					
			//formObj.if_no.className = "input2";
			formObj.frm_bl_src_no.className = "input2";
			formObj.frm_bkg_no.className = "input2";
...
//		}
	...
	function validateForm(sheetObj,formObj,sAction){
		with(formObj){
			...
			if( formObj.frm_bl_src_no.value.trim() == ""  && formObj.frm_bkg_no.value.trim() == "") {
				ComShowCodeMessage("INV00004");
				
				if( formObj.frm_bl_src_no.value.trim() == ""){
					formObj.frm_bl_src_no.focus();
				} else 	if( formObj.frm_bkg_no.value.trim() == ""){
					formObj.frm_bkg_no.focus();
				}
			...
			if(formObj.split_cnt.value != ""){
				...
				// ofc_cd가 SAOSC인 경우 DUE_DT를 체크한다.
				if (formObj.ofc.value == 'SAOSC') {
					if (formObj.due_dt.value == "") {
						ComShowCodeMessage("INV00004");
						formObj.due_dt.focus();
						return;
					}
				}
				
				for(var cnt = 0;cnt < formObj.split_cnt.value; cnt++){						
				...
				for (var i=0; i<document.form.split_curr_cd.length; i++) {						
				...
				for(var cnt = 0;cnt < document.form.split_cnt.value; cnt++){
				...
				for(var a = 1; a<=sheetObjects[1].RowCount;a++){
					var aChgAmt = 0;						
					...
					if(aChgCd!=""){
						...
						var cChgAmt = 0;							
						for (var c = 1; c<=sheetObjects[10].RowCount;c++){
	...
	function openEmail() {
		...
		if (ar_ofc_cd == "SZPSC" || ar_ofc_cd == "XMNSC" || ar_ofc_cd == "CANSO") {
			//       		ComOpenPopup('/hanjin/FNS_INV_0037.do?issueGubn=I', 1010, 660, '', '0,0'); 
			ComOpenWindowCenter("FNS_INV_0037.do?issueGubn=I", "pop", 1010, 700);
		} else {
			//       		ComOpenPopup('/hanjin/FNS_INV_0034_02.do?issueGubn=I', 1010, 660, '', '0,0');  
			ComOpenWindowCenter("FNS_INV_0034_02.do?issueGubn=S", "pop", 1010, 700);
		}
	}
	...
	function rdOpen(Rdviewer, inv_no, line_num, user_nm, ofc_cd, logo, vvd, port_cd, attach, paperYn){
		...
		var rdFile = "";		
		var if_no =  document.form.ar_if_no.value;
	
		if (ofc_cd == "BOMSC") {
			//rdFile = "FNS_INV_0543.mrd";    
			rdFile = "FNS_INV_0550.mrd";		//2018.05.16 인도지역 Split Invoice Issue 기능 보완 
		} else if (ofc_cd == "SAOSC") {
			// 0542   == 
			rdFile = "FNS_INV_0544.mrd";
		} else {
			// 현재는 BOMSC, SAOSC만 이외는  alert
			ComShowCodeMessage("INV00161");
			ComOpenWait(false);  
			return;
		}
...
//	
	...
	/**
	 *  Paper/Go to send버튼을 눌렀을때 Save이후의 로직<br>
	 *  
	 */
	function afterSave(sXml) {
		...
		if(arrXml.length > 0) {
			...
			if (arrStr.length > 1) {
				...
				form.f_inv.value = sStr;
				//alert(form.f_inv.value);
			...
			if (state == "S" && r_invs != "") {	 						
				...
				// 여기서 'INVNo1','INVNO2' 형태를 파싱해야함!!
				var arrStr3 = sStr2[0].replace(/'/g,"").split(",");
				var v_copy_cnt = form.copy_cnt.value;  
				
				if (form.email_flag.value == "N") {
					rdObjects[0].SetAppendReport(1);
					for(var i=0; i<arrStr3.length; i++){
						//alert(arrStr3[i]);
						//alert("rd Open Call !! arrStr3.length ="+arrStr3.length);
	...
	/**
	 *  Split된 CHG SHEET의 자동계산로직<br>
	 *  
	 */
	function calc_tabsheet_chg(sheetObj, Row, Col,Value, n){
		
	
		// sheetObjects[1] = Retrieve 버튼을 눌러서 조회된 CHG
		// sheetObjects[4] = tab1 Sheet에 설정된 CHG
		// sheetObjects[5] = tab2 Sheet에 설정된 CHG
		// sheetObjects[6] = tab3 Sheet에 설정된 CHG
		// sheetObjects[7] = tab4 Sheet에 설정된 CHG
		// sheetObjects[8] = tab5 Sheet에 설정된 CHG
		...
		if(sheetObj.ColSaveName(Col)=="rat_as_cntr_qty"||sheetObj.ColSaveName(Col)=="trf_rt_amt"){
			...
			// 1. Value는 0과 Sheet의 Value만 가능하게함. 
			var currRate  = sheetObjects[1].CellValue(Row,"trf_rt_amt");
			//alert(currRate);
			var changeValue = sheetObj.CellValue(Row,Col);
			changeValue = Number(changeValue);
			
			var splitCnt = Number(document.form.split_cnt.value);
			
			//alert(changeValue);
			if ((Value != currRate) && (Value !=0)) {
				// 조회된 CHG 또는 0이 아니면 에러로 하고 초기값을 넣는다..
				sheetObjects[n+3].CellValue(Row,"trf_rt_amt") = currRate;
				ComShowCodeMessage("INV00162");
			}
			
			
			// 2. 값이 0으로 되었다면 다음탭의 CHG를 CurrCHG로 바꾼다 
			// 현재의 탭이 마지막 탭이 아니라면 다음 탭의 시트의 값에 Sheet의 Value를 넣는다.
			if (Value==0) {
				
				if (n != splitCnt) {
					sheetObjects[n+4].CellValue(Row,"trf_rt_amt") = currRate;
				} else {
					// 마지막탭이라면 어떻게 해야할까?
					// 값이 설정 안되게 한다. // 즉 그대로...
					sheetObjects[n+3].CellValue(Row,"trf_rt_amt") = currRate;
	...
	function t0sheet1_OnChange(sheetObj, Row, Col,Value){
		 calc_tabsheet_chg(sheetObj, Row, Col,Value,1);
	}
	/**
	...
	function t1sheet1_OnChange(sheetObj, Row, Col,Value){
		 calc_tabsheet_chg(sheetObj, Row, Col,Value,2);
	}
	...
	function t2sheet1_OnChange(sheetObj, Row, Col,Value){
		calc_tabsheet_chg(sheetObj, Row, Col,Value,3);
	}
	/**
	...
	function t3sheet1_OnChange(sheetObj, Row, Col,Value){
		calc_tabsheet_chg(sheetObj, Row, Col,Value,4);
	}
	...
	function t4sheet1_OnChange(sheetObj, Row, Col,Value){
		calc_tabsheet_chg(sheetObj, Row, Col,Value,5);
	}
	...
	function obj_keypress(){
	 ...
	 function getFnsInv0110(){
		...
		function fn_ComGetMaskedValue(value) {
			...
			var value = document.form.due_dt.value;
		
			value = ComReplaceStr(value,"-","");
			if(value!=''){
				if (value.length < 8) {
					ComShowCodeMessage("INV00024");
					document.form.due_dt.value = "";
					return;
				} 
				...
				document.form.due_dt.value = ret;
			}
		}

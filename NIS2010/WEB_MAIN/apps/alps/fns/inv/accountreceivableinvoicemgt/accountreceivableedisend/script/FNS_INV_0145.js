 ...
 * @FileName : FNS_INV_0145.js
 * @FileTitle : EDI Submission (ADIDAS)
 * Open Issues :
 * Change history :
...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1" :
		with (sheetObj) {
			...
			InitDataProperty(rowCnt,	cnt++,	dtHidden,		0,	    daCenter,		true,		 "io_bnd_cd",		false,		"",			dfNone,    	0,			false,		false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH : // Retrieve.
		if (validateForm(sheetObj,formObj,sAction)) {
			...
			var sXml = sheetObj.GetSearchXml("FNS_INV_0145GS.do", FormQueryString(formObj));
	...
	case IBINSERT : // Save.
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sXml = sheetObj.GetSaveXml("FNS_INV_0145GS.do", FormQueryString(formObj) + "&" + ComSetPrifix(sheetObj.GetSaveString(false, true, "DelChk"), "sheet1_"));

			if (sXml.indexOf("ERROR") < 1) {
				ComShowCodeMessage("INV00051");

				formObj.f_cmd.value = SEARCHLIST;
				sXml = sheetObj.GetSearchXml("FNS_INV_0145GS.do", FormQueryString(formObj));
				var arrXml = sXml.split("|$$|");
				if (arrXml[0] != null) {
					sheetObj.LoadSearchXml(arrXml[0]);
					if (sheetObjects[0].RowCount == 0) {

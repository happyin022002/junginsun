...
function initSheet(sheetObj, sheetNo) {
	...
	case "sheet1" :
		with (sheetObj) {
			...
			var HeadTitle = "||Sel.|B/L No.|EDI Date|EDI Status|FRT_USD|CHG_OTH|TTL_USD|VVD|S/A Date|Order NO|POR|POL|POD|DEL";
...
function initCombo(comboObj, comboNo) {
	...
	case "cust_cd":
		with (comboObj) {
			...
			InsertItem(0, "US 029316",		"US029316");
			InsertItem(1, "US 035766",	    "US035766");
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH : // Retrieve.
		if (validateForm(sheetObj,formObj,sAction)) {
			...
			var sXml = sheetObj.GetSearchXml("FNS_INV_0125GS.do", FormQueryString(formObj));
	...
	case IBINSERT : // Save.
		...
		if (validateForm(sheetObj, formObj, sAction)) {
			...
			var sXml = sheetObj.GetSaveXml("FNS_INV_0125GS.do", FormQueryString(formObj) + "&" + ComSetPrifix(sheetObj.GetSaveString(false, true, "sel_chk"), "sheet1_"));

			if (sXml.indexOf("ERROR") < 1) {
				ComShowCodeMessage("INV00051");

				formObj.f_cmd.value = SEARCHLIST;
				sXml = sheetObj.GetSearchXml("FNS_INV_0125GS.do", FormQueryString(formObj));
				var arrXml = sXml.split("|$$|");
				if (arrXml[0] != null) {
					sheetObj.LoadSearchXml(arrXml[0]);
					if (sheetObjects[0].RowCount == 0) {
...
function cust_cd_OnChange(comboObj, indexCode, text) {
	var formObj = document.form;
	fn_cust_nm();
	return false;
...
function fn_cust_nm() {
	...
	if (form.cust_cd.CODE.trim()!="") {
		form.cust_cnt_cd.value = form.cust_cd.CODE.substring(0,2);
		form.cust_seq.value = ComLpad(form.cust_cd.CODE.substring(2,8).trim(), 6, "0");	

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	/**
	 * @extends 
	 * @class FNS_INV_0144 : FNS_INV_0144 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function FNS_INV_0144() {
		...
		switch(sheetID) {
			...
			case "sheet1":
				with (sheetObj) {
					...
					var HeadTitle = "||Sel.|Invoice Date|CNTR No.|B/L No.|Invoice No.|Invoice Seq.|Invoice Cur|Invoice AMT|Charge Code|Charge AMT|Sent Status||";
					...
					InitDataProperty(rowCnt,	cnt++,	dtData,			90,		daCenter,		true,		 "iss_dt",			false,			"",		dfDateYmd,      	0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			100,	daCenter,		true,		 "cntr_no",			false,			"",		dfNone,      	0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			100,	daCenter,		true,		 "bl_src_no",				false,			"",		dfNone,	0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			90,		daCenter,		true,		 "inv_no",				false,			"",		dfNone,    0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			90,		daCenter,		true,		 "inv_seq",				false,			"",		dfInteger,	0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			90,		daCenter,		true,		 "curr_cd",				false,			"",		dfNone,		0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			100,		daRight,	true,		 "ttl_trf_rt_amt",				false,			"",		dfNone,     	0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			90,		daCenter,		false,		 "chg_tp_cd",				false,			"",		dfNone,		0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			100,		daRight,	false,		 "trf_rt_amt",				false,			"",		dfNone,		0,		false,		false);
					InitDataProperty(rowCnt,	cnt++,	dtData,			80,		daCenter,		false,		 "edi_snd_flg",				false,			"",		dfNone,			0,		false,		false);
//					InitDataProperty(rowCnt,	cnt++,	dtHidden,		0,		daCenter,		false,		 "ack_rslt_cd",				false,			"",		dfNone,    		0,		false,		false);
		...
		switch(sAction) {
			...
			case IBSEARCH:      //조회
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					var sXml = sheetObj.GetSearchXml("FNS_INV_0144GS.do", FormQueryString(formObj));
			...
			case IBINSERT:        //저장
				if(validateForm(sheetObj,formObj,sAction)) {
					...
					var sXml = sheetObj.GetSaveXml("FNS_INV_0144GS.do",FormQueryString(formObj) + "&" + ComSetPrifix(sheetObj.GetSaveString(false,true,"DelChk"),"sheet1_") );
						...
						for (var i = 1 ; i < idx+1 ; i++){
							for (var j = 1 ; j < sheetObj.RowCount+1 ; j++){
								if (sheetObj.CellValue(j,"DelChk") == '1'){
									if(sheetObj.CellValue(j,"chg_tp_cd") =='OFT' || sheetObj.CellValue(j,"chg_tp_cd") =='GOH' || sheetObj.CellValue(j,"chg_tp_cd") =='DIS'||sheetObj.CellValue(j,"chg_tp_cd") =='DIH'){
//										||sheetObj.CellValue(j,"chg_tp_cd") =='WSC'
									} else {

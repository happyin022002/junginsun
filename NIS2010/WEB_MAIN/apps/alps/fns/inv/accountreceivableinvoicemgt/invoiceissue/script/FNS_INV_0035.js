	...
	/**
	 * fns_inv_0035 : fns_inv_0035 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다. <br>
	 * <br><b>Example :</b>
	 * <pre>
	 *     fns_inv_0035()
	 * </pre>
    ...
    function getPreviewIssue(){   
		...
		if (formObject.sel_option[0].checked || formObject.sel_option[2].checked){                         		
	...
	function processButtonClick(){
		...
		try {
				...
				if (formObject.sel_option[0].checked || formObject.sel_option[2].checked){                         		
					...
					if (formObject.sel_option[0].checked) {
						...
						for (var i = 0; i < sheetObject.RowCount; i++) {
							for (var j = 0; j < 5; j++) {
								if(sheetObject.CellValue(i+1, j) != "") {
									rinvs = rinvs + sheetObject.CellValue(i+1, j) + "|";		
								}
							}
						}
						formObject.r_invs.value = rinvs;
					}
					...
					if (formObject.r_invs.value != "") {
						...
						if (formObject.iss_ofc_cd.value == "SGNSC") {
							...
							for(var i=0; i<arrStr.length-1; i++){
								...
								rdParam = rdParam + "/rv frm1_inv_no["+arrStr[i]+"] frm1_ofc_cd ["+formObject.iss_ofc_cd.value+"] frm1_ar_ofc_cd["+formObject.iss_ofc_cd.value+"] frm1_inv_gb ["+arrStr[i].substr(0,1)+"]" + "|";
																				
							}
							
							formObject.com_mrdPath.value = rdFiles ;
			...
			case "btn_new":
				...
				formObject.ind_type[0].checked = true;
				clickOption();
	...
	//	axon_event.addListenerFormat('beforeactivate', 'objActivate', formObject);      
		...
		if (formObject.sel_option[2].checked) {
          	ComSetDisplay("sel_option_all", false); 
		} else {
			ComSetDisplay("sel_if_no", false); 
		}
...
//			}
	...
	function clickOption() {
		...
		if (formObj.sel_option[0].checked) {
			...
			ComSetDisplay("sel_option_all", true); 
			ComSetDisplay("sel_if_no", false); 
		} else if (formObj.sel_option[2].checked) {  //AR IF NO
			formObj.f_inv.value = "";
			formObj.t_inv.value = "";
			formObj.f_inv.readOnly = true;
			formObj.t_inv.readOnly = true;
	...
	function openEmail() {
		...
		if (issOfcCd == "SZPSC" || issOfcCd == "XMNSC" || issOfcCd == "CANSO") {
			ComOpenWindowCenter("FNS_INV_0037.do?issueGubn=R&issOfcCd="+issOfcCd, "pop", 1010, 700);
		} else {
			ComOpenWindowCenter("FNS_INV_0034_02.do?issueGubn=R&issOfcCd="+issOfcCd, "pop", 1010, 700);
		}
	}
	...
	function openFnsInv0106() {
		...
		} else if (formObject.sel_option[2].checked) {
		...
		ComOpenPopup('/hanjin/FNS_INV_0106.do?sel_option='+selOption+'&iss_ofc_cd='+issOfcCd, 800, 475, 'getFnsInv0106', '1,0,1,1,1', false, false);
	}    
	...
	/**
	 * Invoice Search (FNS_INV_0109) 팝업에서 호출하는 함수 <br>
	 * <br><b>Example :</b>
	 * <pre>
	 *     openFnsInv0109()
	 * </pre>
	 ...
	 * @version 2009.10.20
	 */ 
	function rdOpen(Rdviewer, inv_no, line_num, user_nm, ofc_cd, logo, vvd, port_cd, attach, paperYn, cmbFlg){
		var rdFile = "";
		var loginOfcCd = "";
		var rdParam = "";
		var issue_type = "";
		...
		if (ofc_cd == "HAMSC" || ofc_cd == "HAMRU") {
			...
			if(document.form.sel_option[2].checked){
				rdFile = "FNS_INV_0546.mrd";
			} else {
				//2017.07.31 인도 GST 세법 변경 관련 보완
				if(document.form.ind_type[0].checked){
			...
			if(document.form.exrate_type[0].checked){

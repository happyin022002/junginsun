...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	function initSheet(sheetObj,sheetNo) {
		...
		with (sheetObj) {
			...
			var HeadTitle       = "|Office|Actual Payer Code|Actual Payer Name|Customer Code on B/L|Customer Name on B/L|Update Date|Updated by";
			...
			InitDataProperty(rowCnt, cnt++ , dtData,     120,    	daCenter,  	false,    "act_cust",     		false,          "",      dfNone );
			InitDataProperty(rowCnt, cnt++ , dtData,     300, 		daLeft,  		false,    "act_cust_nm",    	false,          "",      dfNone );
			InitDataProperty(rowCnt, cnt++ , dtData,     150,    	daCenter,  	false,    "cust",     			false,          "",      dfNone );
			InitDataProperty(rowCnt, cnt++ , dtData,     300,    	daLeft,  		false,    "cust_nm",	     	false,          "",      dfNone );
			InitDataProperty(rowCnt, cnt++ , dtData,     150,    	daCenter,  	false,    "upd_dt",     		false,          "",      dfNone );
			InitDataProperty(rowCnt, cnt++ , dtData,     100, 		daCenter,  	false,    "upd_usr_id",     	false,          "",      dfNone );                                                                                                     
		...
		switch(sAction) {	
			...
			case IBSEARCH:      //조회
				...
				if(validateForm(sheetObj,formObj,sAction)) { 
					...
					ComOpenWait(true);
					sheetObj.DoSearch("FNS_INV_0142GS.do",FormQueryString(formObj));
					ComOpenWait(false);
				} else {
					break;
				}
			...
			case IBSEARCH_ASYNC10: // 화면 로딩시 AR Office 조회
				...
				var arrStrTmp = sStr.split("|");
				var arrStr2Tmp = arrStrTmp[1].split("^");
				var head_ofc_cd = arrStr2Tmp[0];
				
				if(head_ofc_cd =="NYCRA"){
				//2015.09.08 백승일(AR_OFC_CD가 아닌 OFFICE도 포함되도록 수정('BOSSO', 'CLESO', 'CLTSO', 'MSPSO', 'SAVSO')
					sStr = sStr+'|NYCRA^BOSSO^B^NYCRA^USD^N^USATL^USA^N^2^US^66356^A^BL^N^N^^0^0^A^N|NYCRA^CLESO^B^NYCRA^USD^N^USATL^USA^N^2^US^66356^A^BL^N^N^^0^0^A^N|NYCRA^CLTSO^B^NYCRA^USD^N^USATL^USA^N^2^US^66356^A^BL^N^N^^0^0^A^N|NYCRA^MSPSO^B^NYCRA^USD^N^USATL^USA^N^2^US^66356^A^BL^N^N^^0^0^A^N|NYCRA^SAVSO^B^NYCRA^USD^N^USATL^USA^N^2^US^66356^A^BL^N^N^^0^0^A^N'
				}

...
=========================================================*/
	...
	/**
	 * fns_inv_0114 : fns_inv_0114 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다. <br>
	 * <br><b>Example :</b>
	 * <pre>
	 *     fns_inv_0114()
	 * </pre>
	...
	var CHARGE_CODE = 'CD00001';
	var UNIT_CODE   = 'CD00002'
	...
	function processButtonClick(){
		...
		try {
			...
			case "btn_add":			        	
				...
				if(formObject.gubun[0].checked) sheetObject.CellValue2(newRow,'intg_cd_id') = CHARGE_CODE;
				if(formObject.gubun[1].checked) sheetObject.CellValue2(newRow,'intg_cd_id') = UNIT_CODE;
	...
	function initSheet(sheetObj,sheetNo) {
		...
		with (sheetObj) {
			...
			var HeadTitle = "|SEQ|SEL||SML Code|SML Code Name|GLOVIS Code|GLOVIS Code Name";
			...
			InitDataProperty(0, cnt++ , dtData,     	  100,   daCenter,  true,    "hjs_cd_val_ctnt",    	true,    "",      dfNone,		0,	false,		true,	100);
			InitDataProperty(0, cnt++ , dtData,     	  300,   daLeft,  true,    "hjs_cd_val_desc",     true,    "",      dfNone,		0,	false,		false,	200);
			InitDataProperty(0, cnt++ , dtData,     	  100,   daCenter,  true,    "cust_cd_val_ctnt",     	true,    "",      dfNone,		0,	true,		true,	100);
			...
			InitDataValid(0,    "cust_cd_val_ctnt",   vtEngUpOther,"1,2,3,4,5,6,7,8,9,0");
		}
		break;
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		if(validateForm(sheetObj,formObj,sAction)) {
			if(sheetObj.id == "sheet1") {
				...
				if(formObj.gubun[0].checked) formObj.intg_cd_id.value = CHARGE_CODE;
				if(formObj.gubun[1].checked) formObj.intg_cd_id.value = UNIT_CODE;
				
				sheetObj.DoSearch("FNS_INV_0114GS.do", FormQueryString(formObj));
			}
		}			
		...
		if(validateForm(sheetObj,formObj,sAction)){
			...
			sheetObj.DoSave("FNS_INV_0114GS.do", FormQueryString(formObj)); 
	...
	function obj_click() {
        ...
        switch (event.srcElement.name) {					
			case "gubun":
			if (formObj.gubun[0].checked && !chg_ind) {
				sheetObject.removeAll();
			} else if(formObj.gubun[1].checked && !code_ind){
				sheetObject.removeAll();
			}
            chg_ind = formObj.gubun[0].checked;
			code_ind = formObj.gubun[1].checked;

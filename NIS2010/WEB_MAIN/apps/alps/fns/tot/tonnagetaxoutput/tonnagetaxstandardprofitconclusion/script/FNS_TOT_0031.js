...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    */    
    ...
    function calculate_data(sheetObj, row, col, val){
		 ...
		 var etd_dt          =             sheetObj.CellValue(row,prefix+"etd_dt");
		 var voy_dys         = ComParseInt(sheetObj.CellValue(row,prefix+"voy_dys"));
		 var to_vvd_stl_dt   =             sheetObj.CellText (row,prefix+"to_vvd_stl_dt");
...
//2010.04.16 Check Logic 뺄 것 By 박효숙 차장        	
        	// 마지막 출항일자로 부터 일수가 해당월의 마지막날을 초과하지 않도록 체크
//        	cal_last_day = ComGetDateAdd(etd_dt, "D", voy_dys);
//        	mon_last_day = etd_dt.substring(0,4)+ "-"+etd_dt.substring(4,6)+"-"+ComGetLastDay(etd_dt.substring(0,4), etd_dt.substring(4,6)) ;
//
//        	if(ComGetDaysBetween(cal_last_day, mon_last_day)<0){
//        		
//        		ComShowCodeMessage('TOT00053');
//        		sheetObj.CellValue2(row,col) = old_voy_dys;  
//        		
//        		return;
//        	}
        
        	//if(formObj.stl_yrmon.value)
       	 sheetObj.CellValue2(row,23) = "Y";        	 
        }		
        
        //alert("old_voy_dys : "+old_voy_dys+" ldb_capa_qty : "+ldb_capa_qty+" vsl_dznd_capa : "+vsl_dznd_capa+" tong_flet_tp_cd : "+tong_flet_tp_cd+"  per_ton_rev : "+per_ton_rev);
        //alert("bsa_capa : "+bsa_capa+" chtr_bsa_capa : "+chtr_bsa_capa+" act_bsa_capa : "+act_bsa_capa+" tong_flet_tp_cd : "+tong_flet_tp_cd+"  val : "+val);
		//return;
        // Capa, Capa Diff 구하기
        
         if(vsl_dznd_capa>ldb_capa_qty){
        	 capa = vsl_dznd_capa;
        	 capa_diff = vsl_dznd_capa - ldb_capa_qty;   //  vsl_dznd_capa는  ldb_capa_qty로 대신하므로 capa_diff는 항상 0이다
         }else{
        	 capa = ldb_capa_qty;
         }
        
         // 선박 TYPE에 따라 사용율 구하기
         if(tong_flet_tp_cd == "J"){
        	
        	 if(bsa_capa>act_bsa_capa){
        		
        		 usage = ComRound((bsa_capa+chtr_bsa_capa+capa_diff)/capa*100,3);
        	 }else{
        		
        		 usage = ComRound((act_bsa_capa+chtr_bsa_capa+capa_diff)/capa*100,3);
        	 }
         }else{
         ...
         if (trdCd == "YYY" || trdCd == "ZZZ"){
        	 taxable_amt = ComParseInt(per_ton_rev * ComParseInt(val), 0) ;
         }else{
        	 taxable_amt = ComParseInt(per_ton_rev * ComParseInt(val)* (usage/100),0) ;
         }
      ...
      //alert("t_taxable_amt : "+t_taxable_amt);
       ...
       //alert("dys_diff : "+dys_diff+" t_taxable_amt : "+t_taxable_amt+" t_day : "+t_day+" t_old_day : "+t_old_day+"  dys_diff : "+dys_diff);
		 ...
		 var sXml = sheetObj.GetSaveXml("FNS_TOT_0031GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(aryPrefix));
				...
				case IBSEARCH:      //조회
	             if (validateForm(sheetObj, formObj, sAction)){
	                 ...
	                 if ( sheetObj.id == "sheet1"){
		    			...
		    			var sXml = sheetObj.GetSearchXml("FNS_TOT_0031GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));

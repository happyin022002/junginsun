 ...
 *@FileName : FNS_JOO_0082.js
 *@FileTitle : [FNS_JOO_0082] File Upload
 *Open Issues :
 *Change history :
...
//    sheetCnt = sheetObjects.length ;   
    ...
    for(var i=0; i < uploadObjects.length ; i++){
	    //1. 기본 환경 설정
	    ComConfigUpload(uploadObjects[i], "/hanjin/FNS_JOO_0082GS.do");	
	}    
...
function doActionIBSheet(sheetObj, formObj, sAction) {	
	 ...
	 switch (sAction) {
	 	...
	 	case IBSEARCH: //조회
			...
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0082GS.do", FormQueryString(frm));
		...
		// 일반
	 			...
	 			var sXml = sheetObj.GetSaveXml("FNS_JOO_0082GS.do",  param);			
	 			sheetObj.LoadSaveXml(sXml);			
	 			var errorYn = ComGetEtcData( sXml, "ERROR_YN");
	 			if (errorYn == "N") {
	 				doActionIBSheet(sheetObjects[0], document.form, IBSEARCH);

...
function loadPage() {
    ...
    for(var i=0; i < uploadObjects.length ; i++){
	    //1. 기본 환경 설정
	    ComConfigUpload(uploadObjects[i], "/hanjin/FNS_JOO_0098GS.do");	
	}    
...
function initSheet(sheetObj, sheetNo) {
	...
	with (sheetObj) {
		...
		case "sheet1": 
			...
			var HeadTitle1 = "|Sel.|Seq.|File Name|Doc Kind|ID|Date|Download|jo_crr_cd|crr_cntc_seq|file_path|file_sav_id";
			...
			InitDataProperty(0, cnt++ , dtPopup,      		280,  daLeft,      false,   "file_nm",     	 	true,       "",    dfNone,       0,          false,		true,	50);
			InitDataProperty(0, cnt++,  dtCombo,           80,    daCenter,  true,    "jo_agmt_file_tp_cd", true, "",    dfNone,       0,         false ,     true );						
            ...
            InitDataProperty(0, cnt++ , dtHidden,	    	0,		daCenter,	true,		"csr_no");
			...
			InitDataCombo(0, "jo_agmt_file_tp_cd", " |Contract|General"," |C|G|");            
...
function doActionIBSheet(sheetObj, formObj, sAction) {	
	 ...
	 switch (sAction) {
	 	...
	 	case IBSEARCH: //조회
			...
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0098GS.do", FormQueryString(frm));
		...
		// 일반
	 			...
	 			var sXml = sheetObj.GetSaveXml("FNS_JOO_0098GS.do",  param);			
	 			sheetObj.LoadSaveXml(sXml);			
	 			var errorYn = ComGetEtcData( sXml, "ERROR_YN");
	 			if (errorYn == "N") {
	 				doActionIBSheet(sheetObjects[0], document.form, IBSEARCH);

 ...
 */
	...
	/**
	 * @extends 
	 * @class FNS_JOO_0088 : FNS_JOO_0088 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function FNS_JOO_0088() {
...
/* 개발자 작업	*/
	...
	function loadPage() {
		...
		sheetObjects[0].ColHidden(prefix+"estm1_bsa_qty") = false;
		sheetObjects[0].ColHidden(prefix+"estm1_bsa_slt_prc") = false;
		sheetObjects[0].ColHidden(prefix+"estm1_amt") = false;
		sheetObjects[0].ColHidden(prefix+"estm2_bsa_qty") = false;
		sheetObjects[0].ColHidden(prefix+"estm2_bsa_slt_prc") = false;
		sheetObjects[0].ColHidden(prefix+"estm2_amt") = false;
		sheetObjects[0].ColHidden(prefix+"cal_amt") = false;
		sheetObjects[0].ColHidden(prefix+"change_item") = false;

		sheetObjects[0].ColHidden(prefix+"adj_estm1_bsa_qty") = true;
		sheetObjects[0].ColHidden(prefix+"adj_estm1_bsa_slt_prc") = true;
		sheetObjects[0].ColHidden(prefix+"adj_estm1_amt") = true;
		sheetObjects[0].ColHidden(prefix+"adj_estm2_bsa_qty") = true;
		sheetObjects[0].ColHidden(prefix+"adj_estm2_bsa_slt_prc") = true;
		sheetObjects[0].ColHidden(prefix+"adj_estm2_amt") = true;
		sheetObjects[0].ColHidden(prefix+"adj_cal_amt") = true;
		sheetObjects[0].ColHidden(prefix+"adj_change_item") = true;
	...
	function initControl() {
		...
		axon_event.addListener('click', 'fnDocClick', "btn_estm_to_back");
		axon_event.addListener('click', 'fnDocClick', "btn_estm_to_next");
		...
		axon_event.addListener('click', 'estm_option_click', 'estm_option');
	...
	function estm_option_click(){
		var formObject = document.form;
		if(formObject.estm_option[0].checked == true){
			sheetObjects[0].ColHidden(prefix+"estm1_bsa_qty") = false;
			sheetObjects[0].ColHidden(prefix+"estm1_bsa_slt_prc") = false;
			sheetObjects[0].ColHidden(prefix+"estm1_amt") = false;
			sheetObjects[0].ColHidden(prefix+"estm2_bsa_qty") = false;
			...
			sheetObjects[0].ColHidden(prefix+"adj_cal_amt") = true;
			sheetObjects[0].ColHidden(prefix+"adj_change_item") = true;
		}else if(formObject.estm_option[1].checked == true){
			sheetObjects[0].ColHidden(prefix+"estm1_bsa_qty") = true;
			sheetObjects[0].ColHidden(prefix+"estm1_bsa_slt_prc") = true;
			sheetObjects[0].ColHidden(prefix+"estm1_amt") = true;
			sheetObjects[0].ColHidden(prefix+"estm2_bsa_qty") = true;
			sheetObjects[0].ColHidden(prefix+"estm2_bsa_slt_prc") = true;
			sheetObjects[0].ColHidden(prefix+"estm2_amt") = true;
			sheetObjects[0].ColHidden(prefix+"cal_amt") = true;
			sheetObjects[0].ColHidden(prefix+"change_item") = true;

			sheetObjects[0].ColHidden(prefix+"adj_estm1_bsa_qty") = false;
			sheetObjects[0].ColHidden(prefix+"adj_estm1_bsa_slt_prc") = false;
			sheetObjects[0].ColHidden(prefix+"adj_estm1_amt") = false;
			sheetObjects[0].ColHidden(prefix+"adj_estm2_bsa_qty") = false;
			sheetObjects[0].ColHidden(prefix+"adj_estm2_bsa_slt_prc") = false;
			sheetObjects[0].ColHidden(prefix+"adj_estm2_amt") = false;
			sheetObjects[0].ColHidden(prefix+"adj_cal_amt") = false;
			sheetObjects[0].ColHidden(prefix+"adj_change_item") = false;
		}
		UF_setCond("1");
		...
		switch (sheetNo) {
			case 1: //t1sheet1 init
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtData,					80,		daRight,		true,		prefix+"adj_estm1_bsa_qty",			false, 	"", 		dfNone,			0, 	false, 	false);		// Estimate Month 1 / BSA
					InitDataProperty(0, cnt++, dtData,					90,		daRight,		true,		prefix+"adj_estm1_bsa_slt_prc",	false, 	"", 		dfNone,			2, 	false, 	false);		// Estimate Month 1 / Slot\nPrice
					InitDataProperty(0, cnt++, dtAutoSum,			120,		daRight,		true,		prefix+"adj_estm1_amt",				false, 	"", 		dfFloat,				2, 	false, 	false);		// Estimate Month 1 / Estimate\nCost
					
					InitDataProperty(0, cnt++, dtData,					80,		daRight,		true,		prefix+"adj_estm2_bsa_qty",			false, 	"", 		dfNone,			0, 	false, 	false);		// Estimate Month 2 / BSA
					InitDataProperty(0, cnt++, dtData,					90,		daRight,		true,		prefix+"adj_estm2_bsa_slt_prc",	false, 	"", 		dfNone,			2, 	false, 	false);		// Estimate Month 2 / Slot\nPrice
					InitDataProperty(0, cnt++, dtAutoSum,			120,		daRight,		true,		prefix+"adj_estm2_amt",				false, 	"", 		dfFloat,				2, 	false, 	false);		// Estimate Month 2 / Estimate\nCost
					
					InitDataProperty(0, cnt++, dtAutoSum,			120,		daRight,		true,		prefix+"adj_cal_amt",						false, 	"", 		dfFloat,				2, 	false, 	false);		// INC\nDEC
					InitDataProperty(0, cnt++, dtData,					80,		daCenter,		true,		prefix+"adj_change_item",				false, 	"", 		dfNone,			0, 	false, 	false );	// Changed\nItem
					InitDataProperty(0, cnt++, dtData,					100,		daLeft,			true,		prefix+"adj_rmk",							false, 	"", 		dfNone,			0, 	true, 	true );		// Remarks

					InitDataProperty(0, cnt++, dtHidden,				0,			daCenter,		false,		prefix+"exe_yrmon",						false, 	"", 		dfNone,			0, 	false, 	false );	// exe_yrmon
					InitDataProperty(0, cnt++, dtHidden,				0,			daCenter,		false,		prefix+"estm_option",					false, 	"", 		dfNone,			0, 	false, 	false );	// estm_option
					InitDataProperty(0, cnt++, dtHidden,				0,			daCenter,		false,		prefix+"jb_exe_yrmon",					false, 	"", 		dfNone,			0, 	false, 	false );	// jb_exe_yrmon
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch (sAction) {
			...
			case IBSEARCH: //조회
				...
				sheetObj.DoSearch("FNS_JOO_0088GS.do", param);
				break;
				
			case IBROWSEARCH:
				formObj.f_cmd.value = SEARCHLIST01;
				formObj.cre_flg.value = "N";
				var sXml = sheetObj.GetSearchXml("FNS_JOO_0088GS.do", FormQueryString(formObj));
			...
			case IBSAVE: //SAVE 버튼
				...
				try{
					var sXml = sheetObj.GetSaveXml("FNS_JOO_0088GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
				}finally{
					ComOpenWait(false);
				}
				...
				//저장 후 재조회
				formObj.f_cmd.value = SEARCH;
				var sXml2 = sheetObj.GetSearchXml("FNS_JOO_0088GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
 	   	  		sheetObjects[0].LoadSearchXml(sXml2);
				
				for(i=sheetObj.LastRow; i > 0 ; i--){
					if (sheetObj.CellValue(i, prefix+"rev_yrmon") == "TOTAL") {
							sheetObj.CellValue(i+1, prefix+"rev_yrmon") = "";
					}
				}
		...
		with (formObj) {
			switch (sAction) {
				case IBSEARCH:
					...
					if (formObj.rev_yrmon.value.length == 0){
						ComShowCodeMessage('JOO00089');
	...
	function UF_reset() {
		...
		formObj.rev_yrmon.value   = gYyyyMM;
		...
		UF_addMonth(formObj.exe_yrmon, -2);
		UF_addMonth(formObj.rev_yrmon, -1);

		formObj.re_divr_cd[0].checked = true;
		formObj.estm_option[0].checked = true;
		formObj.vvd.value = "";
		formObj.changed_option.checked = true;
	...
	function fnDocClick() {
		...
		switch (obj.name) {
			...
			case "btn_estm_to_back":
				UF_addMonth(formObj.rev_yrmon, -1);
				UF_setCond("1");
				break;
	
			case "btn_estm_to_next":
				UF_addMonth(formObj.rev_yrmon, +1);
				UF_setCond("1");
				break;

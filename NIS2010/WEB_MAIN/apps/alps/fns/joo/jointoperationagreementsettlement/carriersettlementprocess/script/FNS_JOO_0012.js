...
/**
 * @extends 
 * @class fns_joo_0012 : fns_joo_0012 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function fns_joo_0012() {
...
//			InitDataProperty(0, cnt++, dtHidden      ,   0, daLeft  , false, prefix+"pre_stl_seq"             , false, "", dfNone, 0, false, false);
			...
			InitDataProperty(0, cnt++, dtData        ,  30, daCenter, false, prefix+"cxl_vvd_flg"      , false, "", dfNone, 0, false, false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {
		...
		case IBSAVE: //중복체크하는 저장
			...
			var sXml = sheetObj.GetSaveXml("FNS_JOO_0012GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
			ComOpenWait(false);
		...
		case IBCREATE:
			/**
			if (document.form.jo_stl_itm_cd.Code.length != 0){
				ComShowCodeMessage("JOO00060");
				document.form.jo_stl_itm_cd.Code = "";
				return;
			}
			*/
			
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0012GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));

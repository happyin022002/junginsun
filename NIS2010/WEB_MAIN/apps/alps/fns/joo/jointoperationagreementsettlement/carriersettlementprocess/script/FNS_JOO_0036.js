 ...
 * 2011.03.28 신자영 [CHM-201109619-01] ALPS / JOO / Summary of Monthly Clearance Status by Carrier - Revenue Month 반영
=========================================================*/
/****************************************************************************************
...
/**
 * @extends 
 * @class FNS_JOO_0036 : FNS_JOO_0036 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function FNS_JOO_0036() {
...
function processButtonClick() {
	...
	try {
		...
		case "btn_new":
			...
			formObject.re_divr_cd.index = 0;
			formObject.jo_stl_itm_cd.index = 0;
 ...
 */
 function loadPage(crrCombo,abbrSheet,dirSheet,staSheet) {
  ...
  //Axon 이벤트 처리1. 이벤트catch
    ...
    axon_event.addListener('click', 'click_view_flag', 'view_flag');
         ...
         case 1:      // sheet1 init
             with (sheetObj) {
                ...
                //전체Edit 허용 여부 [선택, Default false]
				 ...
				 var HeadTitle1 = "|Month|Revenue\nMonth|Carrier|Trade|Lane|Combined\nNo.|Revenue\nVVD|Cur.|Revenue|Expense|Balance|Beneficiary\nLine|Due Date|Remark";
				 //var HeadTitle2 = "|Month|Carrier|Trade|Lane|No.||Cur.|Revenue|Expense|Balance||Due Date|Remark";
                 ...
                 //데이터속성    [ROW, COL,  DATATYPE,		WIDTH, DATAALIGN,		COLMERGE,	SAVENAME,		KEYFIELD, CALCULOGIC, DATAFORMAT,	POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
					...
					InitDataProperty(0, cnt++ , dtData,			80,		daCenter,		true,		prefix+"acct_yrmon",	false,		"",			dfDateYm,		0,			false,		false);
                    InitDataProperty(0, cnt++ , dtData,			80,		daCenter,		false,		prefix+"rev_yrmon",		false,		"",			dfDateYm,		0,			false,		false);
					InitDataProperty(0, cnt++ , dtData,         80,     daCenter,       true,       prefix+"jo_crr_cd",     false,      "",         dfNone,         0,          false,      false);
					InitDataProperty(0, cnt++ , dtData,         80,     daCenter,       true,       prefix+"trd_cd",        false,      "",         dfNone,         0,          false,      false);
                    InitDataProperty(0, cnt++ , dtData,         80,     daCenter,       true,       prefix+"rlane_cd",      false,      "",         dfNone,         0,          false,      false);
					InitDataProperty(0, cnt++ , dtData,			130,	daCenter,		true,		prefix+"stl_cmb_seq",	false,		"",			dfNone,			0,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			100,	daCenter,		true,		prefix+"vvd",			false,		"",			dfNone,			0,			false,		false);
					
					InitDataProperty(0, cnt++ , dtData,			100,	daCenter,		true,		prefix+"locl_curr_cd",	false,		"",			dfNone,			0,			false,		false);
										
					InitDataProperty(0, cnt++ , dtData,			90,		daRight,		true,		prefix+"jo_rev",		false,		"",			dfFloat,		2,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			90,		daRight,		true,		prefix+"jo_exp",		false,		"",			dfFloat,		2,			false,		false);
					InitDataProperty(0, cnt++ , dtData,	    	90,		daRight,		true,		prefix+"jo_balance",	false,		"",			dfFloat,		2,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			90,		daCenter,		true,		prefix+"benefit_line",	false,		"",			dfNone,			0,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			90,		daCenter,		true,		prefix+"eff_dt",		false,		"",			dfNone,			0,			false,		false);
					InitDataProperty(0, cnt++ , dtData,			90,		daCenter,		true,		prefix+"jo_smry_rmk",	false,		"",			dfNone,			0,			true,		true);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {
		case IBSEARCH: //조회
				...
				var sXml = sheetObj.GetSearchXml("FNS_JOO_0036GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
        		...
        		if (validateForm(sheetObj, formObj, sAction)) {
					...
					for(i=sheetObj.LastRow; i > 0 ; i--){
						if (sheetObj.CellValue(i, prefix+"acct_yrmon") == "TOTAL") {
								...
								sheetObj.RowDelete(i, false);	
						} else if (sheetObj.CellValue(i, prefix+"jo_crr_cd") == "") {
								sheetObj.RowDelete(i, false);	
						}
					}

					var SaveStr = ComGetSaveString(sheetObj); // 배열입니다.
					var sXml = sheetObj.GetSaveXml("FNS_JOO_0036GS.do", SaveStr + "&" + FormQueryString(formObj)+ "&" + ComGetPrefixParam("sheet1_"));
								
					//저장 후 재조회
					formObj.f_cmd.value = SEARCH;
					var sXml2 = sheetObj.GetSearchXml("FNS_JOO_0036GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
    	...
    	case "jo_stl_itm_cd": 
            with (comboObj) { 
 			...
 			//	SetColWidth("0|30");
 				...
 				ValidChar(2,1);//영문대문자+숫자만 입력가능
 				InsertItem(0, "", "");
 				InsertItem(1, "BSA", "S/H");
 				InsertItem(2, "OUS", "OUS");
 				InsertItem(3, "R/F", "R/F");
 				InsertItem(4, "Other", "OTH");
 				Index2 = 0;
  		    }
  			break;
    	case "re_divr_cd": 
	...
	with(sheetObj)
	{
   		for(var i=1; i<=sheetObj.LastRow; i++) {
    		if (sheetObj.CellValue(i, prefix+"acct_yrmon") == "TOTAL") {
				...
				sheetObj.CellValue(i, prefix+"jo_smry_rmk") = '';
				...
				sheetObj.CellValue(i, prefix+"vvd") = '';
				sheetObj.CellValue(i, prefix+"jo_smry_rmk") = '';
			}
			if (sheetObj.CellValue(i, prefix+"acct_yrmon") == "TOTAL") {				
				sheetObj.CellValue(i, prefix+"stl_cmb_seq") = '';
			...
			if (document.form.usdamount_chk2.checked){
				sheetObj.CellValue(i, prefix+"locl_curr_cd") = "USD";
			}
...
function checkCol(sheetObj, formObj){
	if(formObj.vvd_chk.checked == true || formObj.combined_chk.checked == true){
		sheetObj.ColHidden(prefix + "eff_dt") = true;
		sheetObj.ColHidden(prefix + "jo_smry_rmk") = true;
	}else if(formObj.vvd_chk.checked == false || formObj.combined_chk.checked == false){

		sheetObj.ColHidden(prefix + "eff_dt") = false;
		sheetObj.ColHidden(prefix + "jo_smry_rmk") = false;
	}
	if(formObj.vvd_chk.checked == true){
...
function click_combined_chk(){
	...
	if (document.form.combined_chk.checked || document.form.vvd_chk.checked || document.form.view_flag[2].checked){
...
function click_view_flag(){
	if (document.form.view_flag[2].checked || document.form.vvd_chk.checked || document.form.combined_chk.checked){
		ComBtnDisable("btn_save");
	}else{
		ComBtnEnable("btn_save");
	}
...
function click_vvd_chk(){
	...
	if(document.form.vvd_chk.checked == true){
		//sheetObj.ColHidden(prefix + "rev_yrmon") = false;
	}else{
		//sheetObj.ColHidden(prefix + "rev_yrmon") = true;
	}

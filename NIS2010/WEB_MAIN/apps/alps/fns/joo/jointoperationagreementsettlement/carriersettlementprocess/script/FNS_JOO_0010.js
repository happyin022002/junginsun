...
/**
 * @extends 
 * @class fns_joo_0010 : fns_joo_0010 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function fns_joo_0010() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //t1sheet1 init
		with (sheetObj) {
			...
			var HeadTitle  = "STS||VSL|VOY|Dir|Fin.\nDir|Dep. Port|Dep. Port|1st BSA + Swap|1st BSA + Swap|Used Slot|Used Slot";
			...
			InitDataProperty(0, cnt++, dtData        ,  50, daCenter, true, prefix+"fm_port_cd",     false, "", dfNone       ,0,false,false);//9
			InitDataProperty(0, cnt++, dtData        ,  50, daCenter, true, prefix+"to_port_cd",     false, "", dfNone       ,0,false,false);//10
			InitDataProperty(0, cnt++, dtData        ,  50, daRight , true, prefix+"fnl_hjs_bsa_qty",false, "", dfInteger    ,0,false,false);//11
			InitDataProperty(0, cnt++, dtData        ,  70, daRight , true, prefix+"fnl_bsa_wgt"    ,false, "", dfFloat      ,2,false,false);//12
			InitDataProperty(0, cnt++, dtData        ,  50, daRight , true, prefix+"usd_slt_bsa_qty",false, "", dfInteger    ,0, true, true);//13
			InitDataProperty(0, cnt++, dtData        ,  70, daRight , true, prefix+"usd_slt_wgt"    ,false, "", dfFloat      ,2, true, true);//14
			InitDataProperty(0, cnt++, dtData        ,  40, daRight , true, prefix+"over_usd_qty"   ,false, "|sheet1_usd_slt_bsa_qty|-|sheet1_fnl_hjs_bsa_qty|", dfInteger,0,false,false);//15
			InitDataProperty(0, cnt++, dtData        ,  80, daRight , true, prefix+"over_usd_wgt"   ,false, "|sheet1_usd_slt_wgt|-|sheet1_fnl_bsa_wgt|", dfFloat,2,false,false);//16
			//2010.03.25 BSA Per Weight I/F후에도 수정가능하게 해달라 (박효숙 차장)
			//InitDataProperty(0, cnt++, dtData        ,  60, daRight , true, prefix+"bsa_per_wgt"    ,false, "", dfFloat,2,false,false);//17
			InitDataProperty(0, cnt++, dtData        ,  60, daRight , true, prefix+"bsa_per_wgt"    ,false, "", dfFloat,2,false, true);//17
			InitDataProperty(0, cnt++, dtData        ,  60, daRight , true, prefix+"bsa_qty"        ,false, "IF (IF(|sheet1_over_usd_qty| >= Round(|sheet1_over_usd_wgt|/|sheet1_bsa_per_wgt|,0), |sheet1_over_usd_qty|, Round(|sheet1_over_usd_wgt|/|sheet1_bsa_per_wgt|,0)) > 0 , IF(|sheet1_over_usd_qty| >= Round(IF(|sheet1_bsa_per_wgt|=0,0,|sheet1_over_usd_wgt|/|sheet1_bsa_per_wgt|),0), |sheet1_over_usd_qty|, Round(IF(|sheet1_bsa_per_wgt|=0,0,|sheet1_over_usd_wgt|/|sheet1_bsa_per_wgt|),0)),0)", dfInteger,0,false,false);//18
			InitDataProperty(0, cnt++, dtData        ,  70, daRight , true, prefix+"bsa_slt_prc"    ,false, "", dfFloat,2,false,false);//19
			InitDataProperty(0, cnt++, dtData        ,  80, daCenter, true, prefix+"locl_curr_cd"   ,false, "", dfNone,0,false,false);//20
			InitDataProperty(0, cnt++, dtAutoSum     ,  80, daRight , true, prefix+"stl_locl_amt"   ,false, "Round(|sheet1_bsa_qty|*|sheet1_bsa_slt_prc|,2)", dfFloat,2,false,false);
			InitDataProperty(0, cnt++, dtData        ,  80, daCenter, true, prefix+"stl_cmb_seq"    ,false, "", dfNone,0,false,false);
			InitDataProperty(0, cnt++, dtData        ,  40, daCenter, true, prefix+"rvs_cmb_flg"    ,false, "", dfNone,0,false,false);
			...
			InitDataProperty(0, cnt++, dtHidden      ,   0, daCenter, false, prefix+"uc_bss_port_etd_dt", false, "", dfNone       ,0,false,false);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {
		...
		case IBCREATE:
			...
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0010GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
		...
		case IBSAVE: //저장
			...
			ComOpenWait(true);
			var sXml = sheetObj.GetSaveXml("FNS_JOO_0010GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
			ComOpenWait(false);
...
//			break;
		...
		case IBSEARCH_ASYNC01:
			...
			if (CHECKVVD=="N"){
				...
				sheetObj.CellValue2(gCurRow, prefix+"locl_curr_cd"      ) = ComGetEtcData(sXml,"locl_curr_cd");
...
//				sheetObj.CellValue2(gCurRow, prefix+"fnl_bsa_wgt"       ) = ComGetEtcData(sXml,"fnl_bsa_wgt");
				...
				sheetObj.CellValue2(gCurRow, prefix+"locl_curr_cd"   ) = "";
...
//				sheetObj.CellValue2(gCurRow, prefix+"fns_bsa_wgt"    ) = "";
	        ...
	        sheetObj.CellEditable(gCurRow, prefix+"to_port_cd") = true;

	        sheetObj.InitCellProperty(gCurRow, 6, dtCombo, daCenter, prefix+"fm_port_cd" , "", dfNone);
	        sheetObj.InitCellProperty(gCurRow, 7, dtCombo, daCenter, prefix+"to_port_cd" , "", dfNone);

	        //Change event를 타지 못하게 하기 위함...
	        gChange = false;
	        sheetObj.CellComboItem(gCurRow, prefix+"fm_port_cd" , portList, portList, 0);
	        sheetObj.CellComboItem(gCurRow, prefix+"to_port_cd" , portList, portList, 0);
	        gChange = true;
	        
	        sheetObj.CellValue2(gCurRow, prefix+"fm_port_cd") = "";
	        sheetObj.CellValue2(gCurRow, prefix+"to_port_cd") = "";
		...
		case IBSEARCH_ASYNC02: 
			...
			var CHECKVVD = ComGetEtcData(sXml,"CHECKVVD");
			if (CHECKVVD=="N"){
				sheetObj.CellValue2(gCurRow, prefix+"usd_slt_bsa_qty") = ComGetEtcData(sXml,"usd_slt_bsa_qty");
				sheetObj.CellValue2(gCurRow, prefix+"usd_slt_wgt"    ) = ComGetEtcData(sXml,"usd_slt_wgt");
		...
		case IBSEARCH_ASYNC03: 
			...
			var CHECKVVD = ComGetEtcData(sXml,"CHECKVVD");
			if (CHECKVVD=="N"){
				sheetObj.CellValue2(gCurRow, prefix+"bsa_slt_prc"    ) = ComGetEtcData(sXml,"bsa_slt_prc");
		        sheetObj.SelectCell(gCurRow, prefix+"over_usd_qty",true);
			}else{
				if (CHECKVVD=="E"){
					ComShowCodeMessage("JOO00143");
...
function sheet1_OnChange(sheetObj, Row, Col, Value) {
	...
	if (sName == prefix+"fm_port_cd" && gChange){
		...
		var fmPortCd = sheetObj.CellValue(Row, prefix+"fm_port_cd");
	...
	if (sName == prefix+"to_port_cd" && gChange){
		var fmPortCd = sheetObj.CellValue(Row, prefix+"fm_port_cd");
		var toPortCd = sheetObj.CellValue(Row, prefix+"to_port_cd");
		...
		//if (ucBssPortEtdDt.length < 8){
		//	ComShowCodeMessage("JOO00154");
		//	sheetObj.CellValue2(Row, prefix+"uc_bss_port_etd_dt") = "";
		//	return;
		//}
...
function existsSamePair(sheetObj, Row, vsl, voy, dir, fin, fmPort, toPort, usdQty, usdWgt, amt){
	...
	for(var inx=sheetObj.HeaderRows; inx<=sheetObj.LastRow; inx++){
		...
		lvsl    = sheetObj.CellValue(inx, prefix+"vsl_cd");
		lvoy    = sheetObj.CellValue(inx, prefix+"skd_voy_no");
		ldir    = sheetObj.CellValue(inx, prefix+"skd_dir_cd");
		lfin    = sheetObj.CellValue(inx, prefix+"rev_dir_cd");
		lfmPort = sheetObj.CellValue(inx, prefix+"fm_port_cd");
		ltoPort = sheetObj.CellValue(inx, prefix+"to_port_cd");
		lusdQty = sheetObj.CellValue(inx, prefix+"usd_slt_bsa_qty");
		lusdWgt = sheetObj.CellValue(inx, prefix+"usd_slt_wgt");
		lamt    = sheetObj.CellValue(inx, prefix+"stl_locl_amt");

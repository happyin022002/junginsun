...
/**
 * @extends 
 * @class fns_joo_0013 : fns_joo_0013 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function fns_joo_0013() {
...
function initControl() {
    ...
    document.form.acct_yrmon.focus();
}
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1 :
		with (sheetObj) {
			...
			var HeadTitle = "STS||Vessel|Voyage|Dir.|Fin.Dir|CUR.|Amount|Remarks|Combined\nNo|RVS|ACCR_YRMON|STL_VVD_SEQ|STL_SEQ|TRD_CD|JO_CRR_CD|RLANE_CD|RE_DIVR_CD|JO_STL_ITM_CD|JO_MNU_NM|STL_BZC_PORT_CD|ETA_DT|SLIP_NO|STL_ADJ_FLG|STL_LST_FLG|UC_BSS_PORT_CD|UC_BSS_PORT_ETD_DT|Dup.";
			...
			InitDataProperty(0, cnt++, dtData        , 360, daLeft  , true , prefix+"stl_rmk"     , false, "", dfNone, 0, true , true , 50);
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {
		case IBSEARCH: //조회
			...
			sheetObj.DoSearch("FNS_JOO_0013GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
			formObj.acct_yrmon.focus();
			break;
		...
		case IBSAVE: //저장
			...
			var sXml = sheetObj.GetSaveXml("FNS_JOO_0013GS.do", SaveStr + "&" + FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
			ComOpenWait(false);
		...
		//VVD 10자리 입력시  stl_vvd_seq, stl_bzc_port_cd, eta_dt, uc_bss_port_cd, uc_bss_port_etd_dt
		...
		case IBCREATE:
			...
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0013GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(prefix));
		...
		case IBSEARCH_ASYNC10: //마감여부 조회
			...
			var sXml = sheetObj.GetSearchXml("FNS_JOO_0013GS.do", FormQueryString(formObj));

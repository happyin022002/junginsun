...
%>	
	...
	function callParentFunc() {		
		...
		if("<%=popKind%>" == "radio" || "<%=popKind%>" == "check") {
			if("<%=sheet%>" == 6){				//ESM_AGT_037.jsp 용 추가
				rArray = getESM_AGT_037("info_cd");
			}else if("<%=sheet%>" == 5){				//ESM_AGT_006.jsp 용 추가
				rArray = getESM_AGT_006();
			}else if("<%=sheet%>" == 4){		//ESM_AGT_005.jsp 용 추가
				rArray = getESM_AGT_005();
			}else if("<%=sheet%>" == 35){		//ESM_AGT_035.jsp 용 추가
	...
	function getESM_AGT_005(colName) {
		...
		checkRows1 = sheetObj1.CheckedRows("check");
		checkRows2 = sheetObj2.CheckedRows("check");
		...
		if(checkRows == 0) {  			
	  		...
	  		if(checkRows1 > 0){
	  			for(var i = 0; i < rows1; i++) {
					if(sheetObj1.CellValue(i, "check") == 1) {					
		  	...
		  	if(checkRows2 > 0){
	  			for(var j = 0; j < rows2; j++) {
					if(sheetObj2.CellValue(j, "check") == 1) {					
	  		...
	  		if(checkRows > 0){
	  			for(var i = 0; i < rows; i++) {
					if(sheetObj.CellValue(i, "check") == 1) {
			  			shtObjValue = shtObjValue + setInt + "-" +sheetObj.CellValue(i, "code") + "," ;
			  		}
			  	}
		  	}
		  	check++	  	

 ...
 *@FileName : ESD_SCE_0144.js
 *@FileTitle : 24 hrs Vessel Delay
 *Open Issues :
 *Change history :
...
/**
 * @extends
 * @class ESD_SCE_0144 : ESD_SCE_0144 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */

function ESD_SCE_0144() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: 
		with (sheetObj) {
			...
			var HeadTitle = "CNTR NO|Type & Size|BKG No|BL No|Lane|VVD|POL|POD|DEL|"
							+"Delivery Term|Planned ETA|Estimated ETA|Delayed Hours|"
							...
							//+"Consignee|E-mailing list|Sent status|Exception Status|Notify|E-mailing list|Sent status|Exception Status|Batch Date|Dwell Type";
...
//			InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"so_mode");
			...
			InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"pol_cd");
			InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"pod_cd");
			InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"del_cd");
			InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"de_term_cd");
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"vps_pln_eta_dt");
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"vps_eta_dt");
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"dlay_call_hrs");
...
//	sheetObj.ShowDebugMsg = 1;
	...
	case IBSEARCH:
		...
		if( parent.document.form.t4_load_flg.value == 'Y' ) return;
//		ComOpenWait(true);
		formObj.f_cmd.value = SEARCH ;
		sheetObj.DoSearch4Post("ESD_SCE_0144GS.do", FormQueryString(formObj));
		//메인페이지에 해당 tab의 데이터 load 여부를 'Y'로 입력
		parent.document.form.t4_load_flg.value = 'Y';
		//메인tab의 total count값을 set해줌.
		parent.setTabTotal(3, sheetObj.TotalRows);
		break;
...
function tabDownText() {
	...
	sheetObjects[0].Down2Text("", "\t", "", "24hrsVesselDwell.txt", "", "");

}
function tabDownPrint() {
	sheetObjects[0].Down2Print(true, poLandscape , "24 hrs Vessel Delay", 1.5, 1.5);
}
...
function sheet1_OnSearchEnd(sheetObj)
{
	...
	var idx1 = coSceSaveNameCol(sheetObj, 'cnee_eml_cnt');
	var idx2 = coSceSaveNameCol(sheetObj, 'ntfy_eml_cnt');

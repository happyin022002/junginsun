...
/**
 * @extends
 * @class ESD_SCE_0160 : ESD_SCE_0160 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */


function ESD_SCE_0160() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: 
		with (sheetObj) {
			...
			var HeadTitle = "tab1Total|tab2Total";
...
//		alert(formObj.pod_cd.value);
//		alert(SceFrmQryString(formObj));
		sheetObj.DoSearch4Post("ESD_SCE_0160GS.do", SceFrmQryString(formObj));
...
//		ComOpenWait(false);
	...
	case IBSAVE:
		...
		sheetObj.DoSave("ESD_SCE_0160GS.do", SceFrmQryString(formObj));
		break;
	}
}
...
function getScNoList() {
	...
	sheetObj.DoSearch4Post("ESD_SCE_0160GS.do", query);
...
function loadTabPage(tabIndex) {
	...
	if (objTabWindow.location.href == "" || objTabWindow.location.href == "about:blank") {
		...
		switch (tabIndex) {
		case 0:
			sUrl = "ESD_SCE_0161.do"; 
			break;
		case 1:
			sUrl = "ESD_SCE_0162.do"; 
			break;
		}
...
function openEmailCopy(){
	...
	var tabIndex = tabObjects[0].SelectedIndex;
	var objTabWindow = document.getElementById("t" + (tabIndex + 1) + "frame").contentWindow;
	var sheetObj = objTabWindow.sheetObjects[0];
	var iCheckRow = sheetObj.FindCheckedRow("chk");
	var selectRow = sheetObj.SelectRow;
    var selectCntrNo =sheetObj.CellValue(selectRow,'cntr_no');
	var arrRow = iCheckRow.split("|");
	if (arrRow.length-1 < 2){
		//ComShowMessage(ComGetMsg('SCE90018'));
		ComShowMessage("1개이상  check 가 되어 있어야 합니다.");
  		return ;
	}
	var count = "0";
	var multi_value ="";
	  for (idx=0; idx<arrRow.length-1; idx++){
			if(idx == 0){
					multi_value = sheetObj.CellValue(arrRow[idx], "cntr_no") ;
			}else{
					multi_value = multi_value + ',' + sheetObj.CellValue(arrRow[idx], "cntr_no");
			}  
			count++;
	  }
      formObj.cntr_no_list.value = multi_value;
       window.open ("ESD_SCE_0164.do?cntr_no="+selectCntrNo+"&search_dt="+formObj.search_dt.value+"&cntr_count="+count+"&cntr_list="+multi_value, "list", "scroll=no, status=YES, help=no ,width=500,height=200");
 }

 function saveDwllRsnType() {
	 var formObj = document.form ;
	 var tabIndex = tabObjects[0].SelectedIndex;
	 var objTabWindow = document.getElementById("t" + (tabIndex + 1) + "frame").contentWindow;
	 var sheetObj = objTabWindow.sheetObjects[0];

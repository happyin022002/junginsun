 ...
 *@FileName : ESD_SCE_0162.js
 *@FileTitle :SCEM - Candidate Inquiry by Dwell Type - 72 hrs Destination Dwell (Rail Cargo Only)
 *Open Issues :
 *Change history :
...
/**
 * @extends
 * @class ESD_SCE_0162 : ESD_SCE_0162 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */

function ESD_SCE_0162() {
...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: 
		with (sheetObj) {
			...
			var HeadTitle = "||SEQ|CNTR NO|Type & Size|BKG No|BL No|Lane|VVD|POD|"
							+"Rail Origin Yard|Rail Dest Yard|Delivery Term|Movement/Yard|Movement date|Current Location|"
							+"Arrival Date&Time|Last Free Day|Dwell Hours|Total Dwell Reasons|Dwell Reasons Type|Pickup Notice|"
			...
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"rail_org_yd_cd", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"rail_dest_yd_cd", false,"", dfNone,     0, false , false);
			...
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"crnt_loc_nm", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"pkup_aval_dt", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"lst_free_dt", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtData,		100,	daCenter,	false,	"dwll_hrs", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtData,      150,    daLeft,     false,  "dwll_rsn", false,"", dfNone,     0, false , false);
			InitDataProperty(0, cnt++, dtCombo,      180,    daLeft,     false,  "dwll_rsn_tp_cd", false,"", dfNone,     0, true , true);
			InitDataProperty(0, cnt++, dtData,		120,	daCenter,	false,	"pkup_ntc_evnt_dt", false,"", dfNone,     0, false , false);
...
//		ComOpenWait(true);
		...
		sheetObj.DoSearch4Post("ESD_SCE_0162GS.do", FormQueryString(formObj));
...
function sheet1_OnSearchEnd(sheetObj)
{
	...
	for(var i=0 ; i < rowcount ; i++){
		if(sheetObj.cellValue(i , "shpr_excp_sts") == 'Y'){
...
function sheet1_OnChange(sheetObj,Row, Col, Value) {
	if(sheetObj.ColSaveName(Col) == "dwll_rsn_tp_cd"){
		if (sheetObj.CellValue(Row,"dwll_rsn_tp_cd") != sheetObj.CellValue(Row,"dwll_rsn_tp_cd1")){
			var f_cmd = SEARCH04;
			sheetObj.DoRowSearch("ESD_SCE_0162GS1.do" ,"row=" + Row +"&eml_snd_dt=" + sheetObj.CellValue(Row,'eml_snd_dt')+"&dwll_tm_tp_cd=" + sheetObj.CellValue(Row,'dwll_tm_tp_cd')+"&cntr_no=" + sheetObj.CellValue(Row,'cntr_no')+"&bkg_no=" + sheetObj.CellValue(Row,'bkg_no')+"&dwll_rsn_tp_cd=" + sheetObj.CellValue(Row,'dwll_rsn_tp_cd')+"&f_cmd=" + f_cmd ,false);

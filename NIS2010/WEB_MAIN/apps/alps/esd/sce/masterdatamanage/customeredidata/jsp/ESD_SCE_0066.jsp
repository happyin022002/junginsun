...
<%
	...
	String edi_sts1 = JSPUtil.getNull(request.getParameter("edi_sts1"));
	//String call_sceern = JSPUtil.getNull(request.getParameter("call_screen"));
	StringTokenizer tokenTpszcd = new StringTokenizer(edi_sts1, ",");
	String edi_cust_idx = JSPUtil.getNull(request.getParameter("edi_cust_idx"));
	...
	//Group by 추가
		if (edi_sts1 !=""){
			edi_sts_str = " and cgo.edi_stnd_sts_cd in (" + codeUtil.convertStr(edi_sts1)+ ")";
		}else {
			edi_sts_str = " ";
		}
	String queryOption = "cgo.edi_grp_cd = '" + cs_grp_id + "' and cgo.edi_stnd_sts_cd = sts.edi_stnd_sts_cd and sts.co_div_cd = cgo.co_div_cd "+edi_sts_str+" GROUP BY cgo.edi_stnd_sts_cd, sts.edi_sts_desc order by edi_sts_seq ";
	
	//EDI STS콤보에서 선택한 값이 있는 경우
	String edi_idx = JSPUtil.getNull(request.getParameter("edi_selected_idx"));
	if (strSize == 1) {
		//EDI STS콤보에서 선택한 값이 없는 경우
		if (edi_idx.equals("")) edi_idx = "1";
	}else {
		//EDI STS콤보에서 선택한 값이 없는 경우
	...
	if (request.getParameter("cust_opt") != null && !request.getParameter("cust_opt").equals("")){
		...
		if (strSize == 1) {
			custQryOPT = " edi_grp_cd = '"+cs_grp_id+"' AND EDI_STND_STS_CD = '"+edi_sts1+"' " ;
		}else {
	...
	if (strSize == 1) {
		combo2 = codeUtil.searchCodeCombo("cust_st"," EDI_GRP_CGO where  "+ custQryOPT
		        ...
		        ,"CUST_EDI_STS_CD || ' - ' || "+custQryCOL+ " " ,""," onChange=javascript:custcomboChange('cust_sts',this.value); ","0:: ");
	...
	String[] result = combo2.split("</option>");
    int resultlng = result.length;
...
<input type="hidden" name="edi_cust_idx" value ='<%=edi_cust_idx %>'>
<input type="hidden" name="edi_sts1" value = '<%= edi_sts1 %>'>
<input type="hidden" name="edi_cust_count" value = '<%= resultlng %>'>
<!-- OUTER - POPUP (S)tart -->
...
<tr><td valign="top">
		...
		<tr><td height="79" class="title"><img src="/hanjin/img/icon_title_dot.gif" align="absmiddle">&nbsp;Cargo Tracking EDI Send - Group</td></tr>
		</table>
					...
					<table class="search" border="0" style="width:500;">
						<tr class="h23">
						    ...
						    <td>&nbsp;<%=codeUtil.searchCodeCombo("edi_sts"," edi_grp_cgo cgo , edi_cgo_stnd_sts sts where " + queryOption
							        ...
							        ,"cgo.edi_stnd_sts_cd || ' - ' || sts.edi_sts_desc cs_desc , MIN(sts.edi_sts_seq) edi_sts_seq    " ,""," onChange=javascript:comboChange('edi_sts'); ","0:: ")%>
							</td>
<!-- [ComboUSAGE] getCodeCombo(String selectName, String table, String valueField, String textField, String orderByField, String options, String addOption)	-->
						...
						<tr class="h23">
							...
							<td>&nbsp;<input name="act_dt" onKeyUp='enterCheck();' type="text" class="input" style="width:130; text-transform:uppercase;ime-mode:disabled" dataformat="ymd" value="" maxlength="16" onfocusout="javascript:onObjectFocusout()">
							    &nbsp;(YYYY-MM-DD HH:MM)
							</td>
						</tr>
						<tr class="h23">
							<td width="21%">&nbsp;&nbsp; Location </td>
							<td>&nbsp;<input name="nod" type="text" class="input" style="width:130; text-transform:uppercase;"  value="" maxlength="5">
							</td>
						</tr>

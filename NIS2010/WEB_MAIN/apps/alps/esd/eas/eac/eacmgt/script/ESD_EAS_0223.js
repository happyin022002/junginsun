...
function initSheet(sheetObj, sheetNo) {
  ...
  switch(sheetNo) {
	  ...
	  with (sheetObj) {
		  ...
		  var HeadTitle1 = "|||Completion|File Name|File Size|Date(Local)|eac_no|eac_file_seq|file_sav_id|file_path_desc" ;
		  ...
		  InitDataProperty(0, cnt++ , dtCombo,     70,  daLeft, true, "eac_cmpl_cd",      false,    "",  dfNone,     0,     false,  false,    9,   false,   true,     "",    false);
		  ...
		  InitDataProperty(0, cnt++ , dtHidden,     80,  daLeft, true, "eac_no", false,    "",  dfNone,     0,     false,  false,    6,   false,   true,     "",    false);
		  InitDataProperty(0, cnt++ , dtHidden,     0,  daLeft, true, "eac_file_seq", false,    "",  dfNone,     0,     false,  false,    6,   false,   true,     "",    false);
		  ...
		  InitDataCombo(0, 'eac_cmpl_cd', eac_cmpl_cdText, eac_cmpl_cdCode);
	  }
  break;
  }
}
...
function loadPage() {
	...
	for ( var i = 0; i < uploadObjects.length; i++) {
		//1. 기본 환경 설정
		ComConfigUpload(uploadObjects[i], "/hanjin/" + "ESD_EAS_0223GS.do");
	}
...
function doActionIBSheet(sheetObj,formObj,sAction) {
    ...
    switch(sAction) {
       case IBSEARCH:
	   	    ...
	   	    sheetObj.DoSearch4Post("ESD_EAS_0223GS.do", FormQueryString(formObj));
			break;
       case IBSAVE:      // 저장
           ...
           if (upObj.LocalFiles == "") {
               ...
               var sXml = sheetObj.GetSaveXml("ESD_EAS_0223GS.do", sParam);
           } else {
...
function fnRtnFileY() {
	...
	if (sheetObj.RowCount > 0) {	
		ComSetObjValue(parentObj.atch_file_id,"Y");
	}else{
		ComSetObjValue(parentObj.atch_file_id,"");
	}
}

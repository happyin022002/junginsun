...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_EAS_0217 : ESD_EAS_0217 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_EAS_0217() {
		...
		try {
			...
			switch(srcName) {
				...
				case "btns_calendar_s":
					...
					cal.select(frm.s_eac_yrmon_fm, 'yyyy-MM');
				break;
	...
	function loadPage() {
		...
		frm.s_eac_yrmon_fm.value = ComGetNowInfo("ym");	    // Entered Date 현재일자 셋팅
		frm.s_eac_yrmon_to.value = ComGetNowInfo("ym");
//		getMonthBetween(frm.s_eac_yrmon_fm);
		initControl();
...
//		    			alert(GetSheetHeight(20));
						...
						InitDataProperty(0, cnt++ , dtData,         110,  	daLeft,     true,   "eac_no",          false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
						...
						InitDataProperty(0, cnt++ , dtData,         90,  	daCenter,   true,   "bkg_no",          false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
						...
						InitDataProperty(0, cnt++ , dtData,         150,  	daLeft,     true,   "eac_inter_rmk",   false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
						...
						InitDataProperty(0, cnt++ , dtData,         75,  	daCenter,   true,   "locl_cre_dt",     false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
...
//						InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,     true,   "rjct_usr_nm",     false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
						...
						InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,     true,   "n3pty_sts_nm",    false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
						InitDataProperty(0, cnt++ , dtData,         100,  	daRight,    true,   "n3pty_amt",       false,    "",      dfNullFloat,     2,          true,        true,   0,  false, true,  "", false);
			...
			switch (sAction) {
				...
				case IBSEARCH:
					if(!validateForm(sheetObj,frm,sAction)) return false;
						...
						var sXml = sheetObjects[1].GetSearchXml("ESD_EAS_0217GS.do", sParam);
...
//		    	frm.s_ofc_cd.Index2=0;
			...
			break;  
			case "s_eac_bil_tp_cd":    
				var eacCode = frm.s_eac_tp_cd.Code;
		...
		*/
		function validateForm(sheetObj,frm,sAction){
			switch(sAction) {
			case IBSEARCH :
				if(frm.s_eac_yrmon_fm.value == "" || frm.s_eac_yrmon_to.value == ""){
					ComShowCodeMessage('COM130201', 'Audit Month'); // Booking No 값을 입력하셔야 합니다;
					return false;
				}

				var ls_fm_dt = removeBar(frm.s_eac_yrmon_fm.value) + '01';
				var ls_to_dt = removeBar(frm.s_eac_yrmon_to.value) + '01';
...
//	  		var pgmNo = 'ESD_EAS_0201';
//	  		var pgmUrl = '/hanjin/ESD_EAS_0201.do';
//	    	var src = "&pgmUrl="+ComReplaceStr(pgmUrl,"/","^")+"&EAC_NO="+eacNo+"&READ_ONLY_FLG = 'Y'"   
//	    	ComOpenPopup("alpsMain.screen?"+src, 1024, 700, "", "1,0", true, false);

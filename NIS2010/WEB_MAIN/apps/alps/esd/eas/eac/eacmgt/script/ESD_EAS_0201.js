...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/ 
	...
	/**
	* @extends 
	* @class ESD_EAS_0201 : ESD_EAS_0201 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	*/

   function ESD_EAS_0201() {
	...
	//Inv.Amount,Should be Amount, Audit Amount 값이 변경이 안되면 개산안한다.
	var amtDataSave = 0.00;
	...
	*/
	...
	// File Attach Read Only Flg
	var attachFileReadOnly = "";
		...
		try {
			...
			switch(srcName) {
				...
				case "btng_tpb_if":
					...
					break;
				case "btn_delete":
					doActionIBSheet(sheetObjects[2],frm,"btn_delete");
					break;
				case "btn_issue":
				...
				case "btng_preview":
					...
					break;
				case "btng_history":
					doActionIBSheet(sheetObjects[1],frm,"btng_history");
					break;
				case "btn_attach":
					openEacFileAttach(frm.eac_no.value, frm.eac_cmpl_cd.Code);
					break;
				case "btn_print":		//Print 화면
					
	  				var eacNo = frm.eac_no.value;
	  				var pgmUrl = 'ESD_EAS_0215.do?';
	  				var param = "&EAC_NO=" + eacNo;
	  				var src = "&pgmUrl=" + pgmUrl + param;
	  				
	  				if (eacNo == "" || eacNo == null) {
	  					ComShowCodeMessage('EAS90045','EAC No');
	  					break;
	  				}
	...
	*/
	function loadPage() {
		ComBtnDisable("btn_issue"); // issue 무조건 비활성화하고 저장 후 권한에 따라 활성화 한다. 
		frm = document.form;
		// 텝설정
...
//		}
		...
		// 심사팀의 경우 Entered Date, Audit Month를 변경 할 수 있다.
		// 2015-04-23 심사팀의 경우 KPI Office 변경 가능.  
		if(rwAuthCd == 'W' && ofcLevel == 'H'){
			frm.eac_inp_dt.className = "input1";      // Entered Date
			frm.eac_inp_dt.readOnly = false;      // Entered Date

			frm.eac_yrmon.className = "input1";    // Audit Month
			frm.eac_yrmon.readOnly = false;      // Audit Month
			frm.btn_audit_month.style.display = "Inline"; // Audit Month 버튼 활성화
			
			frm.kpi_ofc_cd.className = "input1"; 
			frm.kpi_ofc_cd.readOnly = false;
		}else{
			frm.btn_audit_month.style.display = "none"; // Audit Month 비활성화 (심사팀만 권한로직에서 활성화 시킨다.)
		}
		
		if(frm.eac_no.value != ""){
//			ComBtnEnable("btng_tpb_if")   // TPB I/F 저장된 데이터라 활성화
	...
	function comboSetting(){
		...
		frm.n3pty_expn_tp_cd.DeleteItem("EXT"); 
		//frm.n3pty_expn_tp_cd
		frm.curr_cd.Index=0;
		...
		// Status
		ComXml2ComboItem(arrXml[8], frm.dis_eac_sts_cd, "apro_usr_nm", "dis_eac_sts_nm");	// Status
		frm.dis_eac_sts_cd.InsertItem(0, "", "");
		var arrStsCd = ComXml2ComboString(arrXml[8],  "apro_usr_nm", "dis_eac_sts_nm");
		if (arrStsCd != undefined ) {
			var arrStsCdLength = arrStsCd[0].split("|").length;
			frm.dis_eac_sts_cd.Index=arrStsCdLength;
		}
...
//					if (lgin_ofc_cd == "NYCNAG" || lgin_ofc_cd == "NYCNOG"  || lgin_ofc_cd == "HAMUAG"  || lgin_ofc_cd == "HAMUOG"  || lgin_ofc_cd == "SHAAAG"  || lgin_ofc_cd == "SHAAOG"  || lgin_ofc_cd == "SINWAG"  || lgin_ofc_cd == "SINWOG") {
//						comboObj.InsertItem(0, "1st Audit", "1");
//						comboObj.InsertItem(1, "2nd Audit", "2");
//						comboObj.Index=0;
//					}else{
	...
	function obj_blur(){
		...
		switch(obj.name) {
			...
			case "respb_ofc_cd":
				...
				check_available_office("RES");
			break;
			case "kpi_ofc_cd":
				check_available_office("KPI");
			break;
			case "yd_cd":
				yd_cd_change();
			break;
			case "inv_amt":
				if(amtDataSave != obj.value){
					foramtComma(obj)
					inv_Change();
				}
			break;
...
//				style.height = GetSheetHeight(8);
				...
				InitDataProperty(0, cnt++ , dtData,        100,  	daCenter,   true,   "eac_rjct_desc",              false,    "",      dfNone,          0,          true,        true,   0,  false, true,  "", false);
	...
	*/ 
	...
	function doActionIBSheet(sheetObj, frm, sAction) {
		...
		switch (sAction) {
			...
			break;
			case "IBSEARCH02":
//				if(frm.vndr_cntc_pnt_nm.GetCount()<1){
//					return;
//				}
				sheetObj.RemoveAll();
...
//					frm.vndr_cntc_pnt_nm.Code = frm.vndr_cntc_pnt_seq.value;
...
//  							s_tpb_seq(sheetObj, i+1);
						...
						if(frm.eac_no.value.value != "") sheetObj.CellValue2(intV,"eac_no") = frm.eac_no.value
  			...
  			case "btng_tpb_if":
		        ...
		        if(EasXmlString(sXml,"n3pty_no")!=""){
		        	ComShowCodeMessage('EAS90060'); // This Expense Audit case has been interfaced already to TPB case.(이미 I/F 하였습니다.)
		        	return;
		        }
...
//				doActionIBSheet(sheetObj,frm,'btn_retrieve');
  				...
  				break;			
  			case "btn_delete":	  //삭제
  				if(!ComShowCodeConfirm("EAS90061")){ // Do you want to delete? 삭제 하시겠습니까?
					return false;
				}
				
  				if(!validateForm(sheetObj,frm,sAction)) return false;
  				frm.f_cmd.value = MODIFY01;
//  				frm.eac_sts_cd.value = "SC";
  				sheetObjects[2].CellValue2(1,"eac_sts_cd") = "SC";
  				
  				var params = sheetObjects[2].GetSaveString(false, true)  + "&" + FormQueryString(frm);
  				var sXml = sheetObjects[2].GetSaveXml("ESD_EAS_0201GS.do", params);
  				

				var State = ComGetEtcData(sXml, ComWebKey.Trans_Result_Key);
				if (State != "S") {
					ComShowCodeMessage('EAS90062'); // Deleted Fail
					ComOpenWait(false);
					return false;
				} else if (State == "S") {
					ComShowCodeMessage('EAS90063'); // EAC has been deleted
					authorityUiControlInit();
					
  					//Issue 결재 정보는 퍼포먼스를 약간이라도 향상 시키기 위하여 재조회를 하지 않고 화면에서 처리
  					var sUsrNm = frm.session_usr_nm.value;
  					var rownum = frm.dis_eac_sts_cd.InsertItem(-1, "Deleted", sUsrNm);
  					frm.dis_eac_sts_cd.Index=rownum;
				}
  			...
  			case "btn_issue":	  //issue
  				...
  				if(!ComShowCodeConfirm("EAS90064")){ // Issued 하시겠습니까?
					return false;
				}
...
//				if(eacAtpCdVal == 1){
				if(eacAtpCdVal == 1||eacAtpCdVal == 2){	
...
//  					sheetObjects[2].CellValue2(1,"eac_sts_cd") = "RC";
				...
				var sParam = sheetObjects[2].GetSaveString(false, true) + "&" + FormQueryString(frm);
  				...
  				if (State != "S") {
  					...
  					var rownum = 0;
  					if(eacAtpCdVal == 1||eacAtpCdVal == 2){
  						rownum = frm.dis_eac_sts_cd.InsertItem(-1, "Auditor confirm", sUsrNm);
//  	  				}else if(eacAtpCdVal == 2){
//  	  					rownum = frm.dis_eac_sts_cd.InsertItem(-1, "RHQ Confirm", sUsrNm);
  	  				}else if(eacAtpCdVal == 3){
  	  					rownum = frm.dis_eac_sts_cd.InsertItem(-1, "HQ Confirm", sUsrNm);
  	  				}
  					frm.dis_eac_sts_cd.Index=rownum;
  				}
  				break;			
  			case "btng_preview":	  //preview
  				...
  				// Personnel Config, Office Config 에 등록되어 있는지 확인한다.
				frm.f_cmd.value = SEARCH16;
            	var params = FormQueryString(frm);
	         	var sXml = sheetObj3.GetSaveXml("ESD_EAS_0201GS.do", params);
	         	
	         	var usrExistFlag = EasXmlString(sXml,"usr_exist_flag");
	         	var usrEml = EasXmlString(sXml,"usr_eml");
	         	var ntcCcRcvEml = EasXmlString(sXml,"ntc_cc_rcv_eml");
	         	var ofcExistFlag = EasXmlString(sXml,"ofc_exist_flag");
	         	
	         	if(usrExistFlag == "N"){
	         		ComShowCodeMessage('EAS90085'); // Personnel Config 에 등록된 유저 정보가 없습니다.
	         		return;
	         	}
	         	
	         	if(ofcExistFlag == "N"){
	         		ComShowCodeMessage('EAS90086'); // Office Config 에 등록된 오피스 정보가 없습니다
	         		return;
	         	}
...
//  				+"&EAC_EML_USE_FLG="+sheetObj.CellValue(1,"eac_eml_use_flg")+"&EAC_FAX_USE_FLG="+sheetObj.CellValue(1,"eac_fax_use_flg")+
//  						    "&NTC_CC_RCV_EML="+ntcCcRcvEml+"&VNDR_EML="+frm.vndr_eml.value+"&FAX_NO="+frm.fax_no.value;	
  				var src = "&pgmUrl=" + pgmUrl + param;
  				
  				ComOpenPopup(pgmUrl + src, 1024, 700, "", "1,0,1,1,1", true);  
//  				ComPostOpenWindow(pgmUrl + src,  "", "ESD_EAS_0211","width=1024,height=780");  				
  				break;			
  			case "btng_history":	  //history
  				
  				var eacNo = frm.eac_no.value;
  				var pgmUrl = 'ESD_EAS_0222.do?';
  				var param = "&EAC_NO=" + eacNo;
//  				+"&EAC_EML_USE_FLG="+sheetObj.CellValue(1,"eac_eml_use_flg")+"&EAC_FAX_USE_FLG="+sheetObj.CellValue(1,"eac_fax_use_flg")+
//  				"&NTC_CC_RCV_EML="+sheetObj.CellValue(1,"ntc_cc_rcv_eml");	
  				var src = "&pgmUrl=" + pgmUrl + param;
  				
  				ComOpenPopup(pgmUrl + src, 1024, 670, "", "1,0,1,1,1", true);  
//  				ComPostOpenWindow(pgmUrl + src,  "", "ESD_EAS_0211","width=1024,height=780");  				
  				break;			
		}
	}
	...
	*/
	function validateForm(sheetObj,frm,sAction){
		 switch(sAction) {
			 ...
			 case "btng_tpb_if" : 
	  				...
	  				var n3ptyExpnTpCd = frm.n3pty_expn_tp_cd.Code;
	  				var trdPartyVal = frm.trd_party_val.value;
	  				
	  				if( n3ptyExpnTpCd == "EXT"){
  						return true;
	  				}
	  				...
	  				if( n3ptyExpnTpCd == ""){
	  					ComShowCodeMessage('COM130201', 'TPB Exp.Type'); // S/P Code 값을 입력하셔야 합니다.
  						return false;
	  				}
	  				
	  				if( n3ptyExpnTpCd == "PSO"){
	  				...
	  				if( n3ptyExpnTpCd == "TES" || n3ptyExpnTpCd == "TRS" ){
	  					if (bkgNo == "") {
			 ...
			 case IBSAVE : 				 
					...
					if(!savechkValidate())return false;
	  				...
	  				if(rwAuthCd != 'R' ){
	  					...
	  					}else if (!ComShowCodeConfirm("COM130101")){// Do you want to save {?msg1}?
	  						return false; 
	  					}
	  				}else{
	  					if(!ComShowCodeConfirm("COM130101")){// Do you want to save {?msg1}?
	  						return false; 
	  					}
	  				}
	  				...
	  				break;
			 case "btng_preview" :
				 if(frm.vndr_cntc_pnt_nm.Code==""){
					 ComShowCodeMessage('COM130201', 'Contact Point'); // Contact Point에 값을 입력하셔야 합니다;
					 return false;
				 }
	...
	function savechkValidate(){
		...
		var eacTpCd      = frm.eac_tp_cd.Code;
			...
			if (frm.eac_expn_tp_cd.Code == "") {
				ComShowCodeMessage('COM130201', 'Expense Type'); // Expense Type 값을 입력하셔야 합니다
				frm.eac_expn_tp_cd.focus();	
				return false;
			}
			...
			// Agent Commission / Others 일때는	S/P Code / Invoice No  필수 제외
			if(frm.eac_expn_tp_cd.Code !="ACM" && frm.eac_expn_tp_cd.Code !="EXT"){
				if (vndrSeq == "") {
	...
	function eac_expn_tp_cd_OnChange(comboObj,Index_Code, Text){   
		...
		// S/P Code /  Invoice No  필수 제어
		if(frm.eac_expn_tp_cd.Code=="ACM" || frm.eac_expn_tp_cd.Code=="EXT"){
			frm.n3pty_src_no.className = "input"
			frm.vndr_seq.className = "input"
			frm.n3pty_expn_tp_cd.Code = "";
			frm.n3pty_expn_tp_cd.Enable = false;
		}else{
			frm.n3pty_src_no.className = "input1"
			frm.vndr_seq.className = "input1"
			frm.n3pty_expn_tp_cd.Code = frm.eac_expn_tp_cd.Code;
			frm.n3pty_expn_tp_cd.Enable = false;
		}
		
		
//		frm.n3pty_expn_tp_cd.Index=frm.eac_expn_tp_cd.Index;
		doActionIBCombo(frm.n3pty_bil_tp_cd); // TPB Code
	...
	// tpb 버튼을 컨트롤한다.
	function tpbButtonCntr(){
		if(frm.eac_no.value !=""){
			if(frm.eac_expn_tp_cd.Code=="EXT" || frm.n3pty_expn_tp_cd.Code ==""){
				ComBtnDisable("btng_tpb_if"); // TPB I/F 비활성화
				ComBtnDisable("btng_RowAdd"); // TPB Grid Add button 비활성화
				ComBtnDisable("btng_RowDel"); // TPB Grid Del button 비활성화
				frm.n3pty_bil_tp_cd.BackColor = "#FFFFFF";
				frm.vndr_cust_div_cd.BackColor = "#FFFFFF";
				frm.bkg_no.className = "input1";
			}else{
				ComBtnEnable("btng_tpb_if"); // TPB I/F 활성화
				ComBtnEnable("btng_RowAdd"); // TPB Grid Add button 활성화
				ComBtnEnable("btng_RowDel"); // TPB Grid Del button 활성화
				frm.n3pty_bil_tp_cd.BackColor = "#CCFFFD";
				frm.vndr_cust_div_cd.BackColor = "#CCFFFD";
				frm.bkg_no.className = "input1";
			}			
		}else{
			if(frm.eac_expn_tp_cd.Code=="EXT" || frm.n3pty_expn_tp_cd.Code ==""){
				...
				ComBtnDisable("btng_tpb_if"); // TPB I/F 비활성화
				ComBtnEnable("btng_RowAdd"); // TPB Grid Add button 비활성화
				ComBtnEnable("btng_RowDel"); // TPB Grid Del button 비활성화
			}
...
//			doActionIBCombo(frm.vndr_cntc_pnt_nm); // Contact Point
	...
	*/
	function  responsible_office_change(){
	   ...
	   var sXml=sheetObjects[0].GetSearchXml("ESD_EAS_0201GS.do", FormQueryString(frm));
	   var isflag = EasXmlString(sXml,"iResflag");
	   if(isflag==0){
	 ...
	 */
	function  check_available_office(sMode){
		var OfcCd = ""
			
		if (sMode == "KPI") {
			OfcCd =  frm.kpi_ofc_cd.value;
		}else if (sMode == "RES") {
			OfcCd =  frm.respb_ofc_cd.value;
		}
		...
		var sParam = "f_cmd=" + SEARCH07 + "&respb_ofc_cd=" + OfcCd;
		var sXml=sheetObjects[0].GetSearchXml("ESD_EAS_0201GS.do", sParam);
		var isflag = EasXmlString(sXml,"isflag");

	    if(isflag==0){
			if (sMode == "KPI") {
			    ComShowCodeMessage('COM132202', 'KPI. Office'); // 사용 할 수 없는 KPI. Office 
			    frm.kpi_ofc_cd.value="";
			}else if (sMode == "RES") {
			    ComShowCodeMessage('COM132202', 'Resp. Office'); // 사용 할 수 없는 Resp. Office 
			    frm.respb_ofc_cd.value="";
			}
	    }
	}	  
...
//		invAudAmt = parseFloat(ComReplaceStr(invAmtVal,",","")) - parseFloat(ComReplaceStr(invCngAmtVal,",",""));
		// CHM-201536006, Amount 오류 관련 수정
		invAudAmt = (ComReplaceStr(invAmtVal,",","") - ComReplaceStr(invCngAmtVal,",","")).toFixed(2);
...
//		}
	...
	*/
	function open3rdPtyNo(val) {
		ComOpenPopup("/hanjin/ESD_TPB_0115.do?pgmNo=ESD_TPB_0115&s_direct_tpb_no="+val, 1024, 600, 'callBack3rdPtyNo', '1,0,1,1,1',true);
...
//		frm.eac_reg_usr_nm.value    = sheetObj.CellValue(1,"audr_usr_id"      );
		...
		frm.curr_cd.Code            = sheetObj.CellValue(1,"curr_cd"          );
		...
		frm.eac_rjct_desc.value     = sheetObj.CellValue(1,"eac_rjct_desc"    );
...
//		frm.eml_snd_dt.value        = sheetObj.CellValue(1,"eml_snd_dt"       );
		frm.eac_sys_if_cd.value     = sheetObj.CellValue(1,"eac_sys_if_cd"    );
...
//			inputArr[i].disabled = "disabled";
		...
		for(i=0;i<imgArr.length ; i++){
			...
			attachFileReadOnly = "Y";
		}
		frm.btn_n3pty_no.style.display = "";
		frm.btn_attach.style.display = "Inline";
		frm.btn_party_val.style.display = "";
		...
		if(stsVal != "SC"){
			...
			frm.expn_evid_desc.className = "input";    // Relevant Evidence. No.
			frm.eac_inter_rmk.className = "textarea"; // Internal note
			frm.bkg_no.className = "input";      // Booking No.
			frm.bl_no.className = "input";      // B/L No.
			frm.trd_party_val.className = "input";      // 3	RD Party
			...
			frm.expn_evid_desc.readOnly = false;  // Relevant Evidence. No.	
			frm.eac_inter_rmk.readOnly = false;   // Internal note
			
			frm.bkg_no.readOnly = false;   // Booking No.
			frm.bl_no.readOnly = false;   // B/L No.
			frm.trd_party_val.readOnly = false;   // 3RD Party
		}
		
		
		// 삭제버튼은 SAVE 상태의 자료만 삭제 가능하다. (오피스, RHQ, HQ 공통)
		if(stsVal != "IS"){
			ComBtnDisable("btn_delete"); // Delete
		}

		//  Print 버튼은 Save이후엔 Delete를 제외하곤 항상 활성화 필요
		if(stsVal = "" || stsVal == "SC"){
			ComBtnDisable("btn_print"); // print; 
		}else{
			ComBtnEnable("btn_print"); // print; 
		}
		
		frm.vndr_cntc_pnt_nm.Enable  = false; // Contact Point
		frm.n3pty_expn_tp_cd.Enable  = false; // TPB Exp.Type
		frm.n3pty_bil_tp_cd.Enable   = true; // TPB Code
		frm.vndr_cust_div_cd.Enable  = true; // 3RD Party

		if(frm.eac_sts_cd.value == "SC" || frm.eac_sts_cd.value == "RR" || frm.eac_sts_cd.value == "HR"){
			ComBtnDisable("btng_tpb_if"); // TPB I/F 삭제 ,RHQ Reject, HQ Reject 일때만 비활성화
		}else{
			ComBtnEnable("btng_tpb_if");  // TPB I/F 활성화
		}
...
//		ComBtnDisable("btn_delete"); // Delete
	...
	//권한에 따른 화면 제어(초기화)
	function authorityUiControlInit(){
		ComResetAll();
...
//			inputArr[i].disabled = "disabled";
		...
		frm.eac_inp_dt.className = "input2";      // Entered Date
		frm.vndr_seq.className = "input1";        // S/P Code
		frm.n3pty_src_no.className = "input1";    // Invoice No/Date
		
		frm.eac_yrmon.className = "input2";    // Audit Month

		frm.bkg_no.className = "input1";          // Booking No.
		frm.inv_aud_usd_amt.className = "input1"; // Audit Amount
		frm.inv_aud_amt.className = "input1";     // USD Amount
		frm.respb_ofc_cd.className = "input1";     // USD Amount
		
		frm.audr_ofc_cd.className = "input2";     // Auditor
		frm.eac_reg_usr_nm.className = "input2";  // Auditor Nm
		
		frm.eac_no.className = "input2";          // EAC No
		frm.vndr_nm.className = "input2";         // S/P Nm
		frm.eac_sts_nm.className = "input2";      // Status
		frm.eac_rjct_ofc_cd.className = "input2"; // Rejected by
		frm.eac_rjct_usr_nm.className = "input2"; // Rejected by Nm
		
		frm.ntc_his_seq.className = "input2";     // No. of notice
		frm.vndr_lgl_eng_nm.className = "input2"; // S/P Name
		frm.eng_addr.className = "input2";        // Address
		frm.zip_cd.className = "input2";          // ZIP Code
		frm.vndr_eml.className = "input2";        // E-mail
		frm.phn_no.className = "input2";          // Phone
		frm.fax_no.className = "input2";          // FAX  
		//frm.trd_party_val.className = "input2";   // 3RD Party
		
		frm.trd_party_nm.className = "input2";    // 3RD Party Nm
		frm.n3pty_no.className = "input2";        // TPB No
		frm.tpb_ofc_cd.className = "input2";      // TPB Office
		frm.ots_sts_nm.className = "input2";      // TPB Status
		frm.tpb_inv_amt.className = "input2";     // Recovery(US$)
		
		frm.atch_file_id.className = "input2";     // Attach
		frm.kpi_ofc_cd.className = "input2";          // KPI Office
		frm.apro_usr_nm.className = "input2";     // Status
		
		frm.audr_ofc_cd.readOnly = true;
		frm.eac_reg_usr_nm.readOnly = true;
		frm.vndr_nm.readOnly = true;
		
		frm.eac_inp_dt.readOnly = true;      // Entered Date
		frm.eac_yrmon.readOnly = true;      // Audit Month
		
		frm.ntc_his_seq.readOnly = true;      // No. of notice
		frm.vndr_lgl_eng_nm.readOnly = true;  // S/P Name
		frm.eng_addr.readOnly = true;         // Address
		frm.zip_cd.readOnly = true;           // ZIP Code
		frm.vndr_eml.readOnly = true;         // E-mail
		frm.phn_no.readOnly = true;           // Phone
		frm.fax_no.readOnly = true;           // FAX   
		//frm.trd_party_val.readOnly = true;    // 3RD Party
		frm.trd_party_nm.readOnly = true;     // 3RD Party Nm
		frm.n3pty_no.readOnly = true;         // TPB No
		frm.tpb_ofc_cd.readOnly = true;       // TPB Office
		frm.ots_sts_nm.readOnly = true;       // TPB Status
		frm.tpb_inv_amt.readOnly = true;      // Recovery(US$)
		
		frm.eac_rjct_ofc_cd.readOnly = true;
		frm.eac_rjct_usr_nm.readOnly = true;
		frm.kpi_ofc_cd.readOnly = true;
		frm.apro_usr_nm.readOnly = true;
		
		for(i=0;i<textareaArr.length ; i++){
				textareaArr[i].readOnly = false;
				textareaArr[i].className = "textarea";
		}
		
		frm.eac_desc.className = "textarea1";    // Audit Detail (Reason)
		frm.eac_rsn_desc.className = "textarea1";    // Action taken
		
		frm.eac_rjct_desc.className = "textarea";     // Reason of rejection
		frm.eac_rjct_rsn.className = "textarea2";      // Reason of Unapproval

		frm.eac_rjct_rsn.readOnly = true;
		frm.eac_rjct_desc.readOnly = false;
		...
		//콤보박스 비활성화
		frm.eac_apro_tp_cd.Enable    = true;  // Type
		frm.eac_expn_tp_cd.Enable    = true;  // Expense Type
		frm.eac_tp_cd.Enable         = true;  // EAC Type
		frm.eac_bil_tp_cd.Enable     = true;   // EAC Type Detail
		frm.curr_cd.Enable           = true;   // Currency
		frm.eac_rsn_cd.Enable        = true;  // Action Type
		frm.eac_cmpl_cd.Enable       = true; // Completion
		frm.vndr_cntc_pnt_nm.Enable  = true; // Contact Point
		frm.n3pty_expn_tp_cd.Enable  = true; // TPB Exp.Type
		frm.n3pty_bil_tp_cd.Enable   = true; // TPB Code
		
		// 버튼 활성화
		ComBtnEnable("btn_save"); // Save
		ComBtnEnable("btn_delete"); // Delete
		ComBtnEnable("btn_issue"); // Issue
		ComBtnEnable("btn_print"); // print
		//ComBtnEnable("btng_RowAdd"); // Row Add
		//ComBtnEnable("btng_RowDel"); // Row Delete
		//ComBtnEnable("btng_tpb_if"); // TPB I/F
...
//		ComBtnEnable("btn_save");// 저장
	...
	*/
	function openEacFileAttach(eacNo, eacCmplCd) {
		if (frm.eac_no.value == "") {
			ComShowCodeMessage('EAS90057');
			return;
		}
//ex) attachFileReadOnly : YN ( Upload 버튼 비활성화, Delete 버튼 활성화)
//		attachFileReadOnly = getAttachFileReadOnly();
		var url = "/hanjin/ESD_EAS_0223.do?eac_no="+eacNo+"&eac_cmpl_cd="+eacCmplCd+"&attach_file_readonly="+attachFileReadOnly;
	...
	* @param : status code
	*/
	function dis_eac_sts_cd_OnChange(comboObj,Index_Code, Text){
		frm.apro_usr_nm.value = Index_Code;
	}
	
	

//	function getAttachFileReadOnly(){	
//		var sEacSttsCd = sheetObjects[2].CellValue(1,"eac_sts_cd");
//		var sBtnUpRdOnly = "N";		// Upload Button - Y : Disable , N: enable
//		var sBtnDtRdOnly = "N";		// Delete Button - Y : Disable , N: enable
//
//		// Read Only Option 이면 1순위로 File Attach 버튼 비활성화
//		if (frm.read_only_flg.value == "Y") {
//			sBtnUpRdOnly = "Y";
//			sBtnDtRdOnly = "Y"
//			return sBtnUpRdOnly + sBtnDtRdOnly;
//		} 
//
//		if (sEacSttsCd == "SC"||sEacSttsCd == "RR"||sEacSttsCd == "HR" ){
//			// 삭제, reject 된 경우- 모든 ofclevel 에서 file uplaod, delete 불가 
//			sBtnUpRdOnly = "Y";
//			sBtnDtRdOnly = "Y";
//		} else if (sEacSttsCd == "IS"||sEacSttsCd == "AC" ){
//			// RHQ Confirm 전 - 모든 ofclevel 에서 file uplaod, delete 가능 
//			sBtnUpRdOnly = "N";
//			sBtnDtRdOnly = "N";
//		} else if (sEacSttsCd == "RC" ){
//			// RHQ Confirm 이후 - Office 에서는 Delete가 불가, upload만 가능, RHQ, Head Office 모두 가능
//			sBtnUpRdOnly = "N";
//			sBtnDtRdOnly = "N";
//			if (ofcLevel == "O") {
//				sBtnDtRdOnly = "Y";
//			}
//		} else if (sEacSttsCd == "HC" ){
//			// HC Confirm 이후  - Office,RHQ 에서는 Delete가 불가, upload만 가능, Head Office 모두 가능
//			sBtnUpRdOnly = "N";
//			sBtnDtRdOnly = "N";

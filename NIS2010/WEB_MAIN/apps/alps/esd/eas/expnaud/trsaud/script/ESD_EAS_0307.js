...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_EAS_0307 : ESD_EAS_0307 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_EAS_0307() {
...
//					break;
	...
	function loadPage() {
		...
		getEasIbComboList(frm.s_cst_cd , s_cst_cdCode , s_cst_cdText , 'ALL'); 	// Cost Code Setting
		getEasIbComboList(frm.s_rcv_trm , s_rcv_trmCode , s_rcv_trmText , 'ALL'); 	// Receive Term Setting
		getEasIbComboList(frm.s_dlvry_trm , s_dlvry_trmCode , s_dlvry_trmText , 'ALL'); 	// Delivery Term Setting
...
//		getMonthBetween(frm.s_snd_fm_dt);
		...
		frm.s_rcv_trm.enable = false;
		frm.s_dlvry_trm.enable = false;
	...
	function initCombo(comboObj) {
		switch(comboObj.id) {
			case "s_wo_sts_cd":
				...
				comboObj.InsertItem(1, "W/O Issued", "WI");
				comboObj.InsertItem(2, "Invoice Saved", "SV");
				comboObj.InsertItem(3, "Invoice Confirmed", "CF");
				comboObj.InsertItem(4, "CSR Creation", "SV");
				comboObj.InsertItem(5, "Invoice Saved", "RA");
				comboObj.InsertItem(6, "AP Intefaced", "IF");
				comboObj.InsertItem(7, "Paid", "PD");
				comboObj.Index2=0;
				break;
			...
			case "s_cst_cd":
				...
				for ( var i = 0, j = comboObj.GetCount() ; i < j; i++ ){
					comboObj.index2 = i;
					sCode = comboObj.Code;
					if (sCode!= 'CY' && sCode != 'DR' && sCode != 'LS' && sCode != 'TS'){
						comboObj.DeleteItem(sCode);
				...
				comboObj.InsertItem(0, "", "");	// All Option 추가		 
				comboObj.index2 = 0;
				break;
				
			case "s_dlvry_trm":
				comboObj.DeleteItem("H");		// C'Haul on CY 는 제거
				comboObj.Index2=0;
				break;  		
		}
...
//		combosXml = sheetObj.GetSearchXml("ESD_EAS_0307GS.do", FormQueryString(frm));
	 	...
	 	switch(sheetNo) {
	    	case 1:      //sheet1 init
		    	with (sheetObj) {
					...
					var HeadTitle1 = "||Seq.|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|BKG Data|S/O Data|S/O Data|S/O Data|S/O Data";
					HeadTitle1 += 	 "|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|S/O Data|EAC I/F";
	
					var HeadTitle2 = "||Seq.|Lane|VVD|BKG No.|CNTR No.|TP/SZ|Bound|Term|TRO|POR|POL|POD|DEL|CUR|Surcharge|Surcharge\n(USD)|From|Via|To|Door"
					HeadTitle2 += 	 "|Trans\nmode|Cost\nmode|S/O No.|W/O No.|W/O S/P|Invoice No.|W/O CUR|W/O Amount|Inv CUR|Inv Amount|Inv Amount\n(USD)|S/O User|Inv User|Reason|Internal Remark|Reason of\nNode Change|Sepecial Instruction|EAC I/F";
...
//					HeadRowHeight = 12;
					...
					InitDataProperty(0, cnt++ , dtData,         50,  	daCenter,   true,   "slan_cd",         		false,    "",      dfDateYm,        0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daCenter,   true,   "vvd",      	   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         95,  	daCenter,   true,   "bkg_no", 		   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					...
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "trsp_bnd_cd",          false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "bkg_rcvde_term_cd",    false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "tro_flg",  	  		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         65,  	daCenter,   true,   "por_nod_cd",         	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         65,  	daCenter,   true,   "pol_nod_cd",        	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         65,  	daCenter,   true,   "pod_nod_cd",         	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         65,  	daCenter,   true,   "del_nod_cd",     		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         40,  	daCenter,   true,   "bkg_curr_cd", 	   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daRight,   	true,   "bkg_scg_amt",     		false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daRight,   	true,   "bkg_scg_usd_amt", 		false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					...
					InitDataProperty(0, cnt++ , dtData,         40,  	daCenter,   true,   "trsp_cost_dtl_mod_cd", false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					...
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "wo_vndr_seq",   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         100,  	daCenter,   true,   "inv_no",       		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         60,  	daCenter,   true,   "curr_cd",     			false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daRight,    true,   "wo_amt",  	   			false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         55,  	daCenter,   true,   "inv_curr_cd",  	   	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         85,  	daRight,   	true,   "inv_amt",       		false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);					
					InitDataProperty(0, cnt++ , dtData,         85,  	daRight,    true,   "inv_usd_amt",       	false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,   	true,   "so_usr_nm",   			false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,   	true,   "inv_usr_nm",     		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         150,  	daLeft,   	true,   "trsp_purp_rsn",     	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         150,  	daLeft,     true,   "inter_rmk",      		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         150,  	daLeft,     true,   "cng_rsn_desc",      	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
...
//						alert(sParam);
						var sXml = sheetObj.GetSearchXml("ESD_EAS_0307GS.do", sParam);
...
//			var pgmNo = 'ESD_EAS_0307';
//			var pgmUrl = 'ESD_EAS_0307.do?';
...
//				var sXml = sheetObj.GetSearchXml("ESD_EAS_0307GS.do", FormQueryString(frm));
...
//				frm.s_eac_reg_usr_id.RemoveAll();
		...
		function s_bnd_cd_OnChange(comboObj,Index_Code, Text){   
			var obj = comboObj;
			switch(obj.name) {
				case "s_bnd_cd":
					if (obj.Code == "I") {
						document.getElementById("td_rcv_trm").style.display = "block";
						document.getElementById("td_dlvry_trm").style.display = "none";
						frm.s_rcv_trm.enable = true;
						frm.s_dlvry_trm.enable = true;
						frm.s_rcv_trm.index2 = 0;
						frm.s_dlvry_trm.index2 = 0;
					} else if (obj.Code == "O") {
						document.getElementById("td_rcv_trm").style.display = "none";
						document.getElementById("td_dlvry_trm").style.display = "block";
						frm.s_rcv_trm.enable = true;
						frm.s_dlvry_trm.enable = true;
						frm.s_rcv_trm.index2 = 0;
						frm.s_dlvry_trm.index2 = 0;
...
//			var sXml=sheetObjects[1].GetSearchXml("ESD_EAS_0307GS.do", FormQueryString(frm));
...
//		}	  		
		...
		function validateForm(sheetObj,formObj,sAction){
			...
			case IBSEARCH :
				...
				if(formObj.s_fm_dt.value == "" || formObj.s_to_dt.value == "" ){
					ComShowCodeMessage("COM130403", "Date"); // S/O creation period;
					return false;
				}
		...
		function checkPeriodValidation(objFm,objTo){
			...
			if ( sTo > sMax){
				ComShowCodeMessage("COM132001", "S/O creation period","1 Month"); // msgs['COM132001'] = '{?msg1} exceeds maximum duration {?msg2}.';
				return false;
			}
		...
		function eac_transfer(sheetObj,Row,Col){
			...
			var eqNoList = "";
			var soNoList = "";
			var costCdList = "";	// Cost Code doesn't exist in 0307. this value need for eac i/f. ( default : XXXXXX )
			var temp = "";	
			var ofc_cd = "";
			var vndr_seq = "";
			var cur_cd = "";
			...
			for (var idx=0; idx < arrRow.length - 1 ; idx++){	// 마지막 불필요한 값을 실행 못하게 하기 위해 arrRow.length 에 - 1 처리.
  					...
  					// 다건 선택시  선택된 Row 들은 Bkg no,  BKG NO, W/O Currency, W/O S/P Code 값이 동일 애야  EAC I/F 할 수 있다. 
  					if (idx == 0 ) {
  						orgKey = ComTrim(sheetObj.CellValue(arrRow[idx],"bkg_no")) + ComTrim(sheetObj.CellValue(arrRow[idx],"curr_cd")) + ComTrim(sheetObj.CellValue(arrRow[idx],"wo_vndr_seq")); 
  					} else {
  						tempKey = ComTrim(sheetObj.CellValue(arrRow[idx],"bkg_no")) + ComTrim(sheetObj.CellValue(arrRow[idx],"curr_cd")) + ComTrim(sheetObj.CellValue(arrRow[idx],"wo_vndr_seq"));
  					}
  					
  					if ( orgKey != tempKey && tempKey != "") {
  						ComShowMessage(ComGetMsg("EAS90101"));
  						return;
  					}
					...
					if (eac_if_flg == "Y") {
  					...
  					costCdList = costCdList + 'XXXXXX' + ",";		// Un-match Route 의 경우 Cost Code 가 없기 때문에 강제로 'XXXXXX' 값을 넘긴다.
		    ...
		    // for Data Setting
		   	var theURL = "ESD_EAS_0224.do?p_sys_div_cd="	+ "TRS"													//  System 구분 : Transportation 
										+"&p_sys_if_cd="	+ "TR3"													// 	System 별 interface 구분 : TR3 (Un-match Route)								
										+"&p_eq_no="		+ eqNoList												
										+"&p_tp_sz="		+ tpSzList												

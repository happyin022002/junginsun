...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_EAS_0308 : ESD_EAS_0308 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_EAS_0308() {
...
//                	var targetObjList = "loc_dpth_cd:s_loc|loc_cd:s_ecc";		// 일단 Code만 받도록 수정
                	var targetObjList = "loc_cd:s_ecc";
				...
				case "btn_cust":
					ComOpenPopup('/hanjin/COM_ENS_041.do', 770, 430, 'getCustomerCode', '1,0,1,1,1,1,1,1');
					break;            		
...
//		getMonthBetween(frm.s_snd_fm_dt);
		...
		getEasIbComboList(frm.s_de_term_cd , s_de_term_cdCode , s_de_term_cdText , 'ALL'); 					// Delivery Term
	...
	*/ 
	function initCombo(comboObj) {
		switch(comboObj.id) {
			case "s_cust_tp_cd":
				...
				comboObj.InsertItem(1, "Shpper", "S");
				comboObj.InsertItem(2, "Consignee ", "C");
				comboObj.Index2=0;
				break;  
			case "s_eac_if":
				comboObj.RemoveAll();
	...
	function comboLoding(){
		...
		ComXml2ComboItem(combosXml, frm.s_cntr_tpsz,   "code_cd", "code_nm");			// Container Type Size ( Code, Name 같음)
		frm.s_cntr_tpsz.InsertItem(0, "", "");	// All Option 추가 
		
		frm.s_cntr_tpsz.MultiSelect = true;
		frm.s_cntr_tpsz.index = 0;						
	}
	 	...
	 	switch(sheetNo) {
	    	case 1:      //sheet1 init
		    	with (sheetObj) {
					...
					var HeadTitle1 = "||Seq.|ECC|BKG\nStatus|CNTR No.|CNTR\nTP/SZ|BKG No.|POR|POL|POD|DEL|DEL\nTerm|Shipper Code|Cnee Code|Delivery\nYard|Actual\nReturn Yard|S/C|RFA";
					HeadTitle1 += 	 "|Charge\nCode|CUR|Rate As|Rate|PPD/CCT|TPB No.|TPB Amount|EAC I/F";
...
//					HeadRowHeight = 12;
					...
					InitDataProperty(0, cnt++ , dtData,         50,  	daCenter,   true,   "ecc_cd",          		false,    "",      dfDateYm,        0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "bkg_sts_cd",      	   	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         95,  	daCenter,   true,   "cntr_no", 		   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "cntr_tpsz_cd",   	   	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					...
					InitDataProperty(0, cnt++ , dtData,         45,  	daCenter,   true,   "de_term_cd",         	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daCenter,   true,   "shp_cd",         		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daCenter,   true,   "cne_cd",     			false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         75,  	daCenter,   true,   "del_yd_cd", 	   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         75,  	daCenter,   true,   "act_rtn_yd_cd",     	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "sc_no", 				false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daCenter,   true,   "rfa_no", 				false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);					
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "bkg_scg_cd",       	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "bkg_scg_cur_cd",      	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         70,  	daRight,   	true,   "bkg_scg_rat_ut_cd",   	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         70,  	daRight,   	true,   "bkg_scg_amt",      	false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daCenter,   true,   "bkg_scg_term_nm",     	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daLeft,     true,   "tpb_no",    	   		false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         80,  	daRight,    true,   "tpb_amt",   			false,    "",      dfNullFloat,     2,          false,        false,   0,  false, true,  "", false);
					InitDataProperty(0, cnt++ , dtData,         40,  	daCenter,   true,   "eac_if_flg",       	false,    "",      dfNone,          0,          false,        false,   0,  false, true,  "", false);
				}
				break;
//	    	case 2:      //sheet1 init
//	    		with (sheetObj) {
			...
			switch (sAction) {
				...
				case IBSEARCH:
					if(validateForm(sheetObj,frm,sAction)){
						...
						var sXml = sheetObj.GetSearchXml("ESD_EAS_0308GS.do", sParam);
...
//	        		}
		...
		*/
		...
		function openAuditdetail(){
			...
			var pgmNo = 'ESD_EAS_0308';
			var pgmUrl = 'ESD_EAS_0308.do?';
...
//				sheetObj.WaitImageVisible = false;
				var sXml = sheetObj.GetSearchXml("ESD_EAS_0308GS.do", FormQueryString(frm));
//				sheetObj.WaitImageVisible = true;
		...
		*/
		function customer_change(){
			if(frm.s_cust_cd.value =="" ){
				frm.s_cust_cd.value="";
				frm.s_cust_nm.value="";
				return;
			}
			frm.f_cmd.value = SEARCH03;
			var sXml=sheetObjects[0].GetSearchXml("ESD_EAS_0308GS.do", FormQueryString(frm));
			var custCd = EasXmlString(sXml,"code_nm");

			if(custCd == 0 ||custCd == null||custCd == undefined){
				ComShowCodeMessage('COM132202', 'Customer Code'); //사용할수 없는 Customer Code 
				frm.s_cust_cd.value="";
				frm.s_cust_nm.value="";
				return;
			}
			frm.s_cust_nm.value = EasXmlString(sXml,"code_nm");
		...
		*/
		function getLocation(rowArray) {
			for(var i=0; i<rowArray.length; i++){
				var colArray = rowArray[0];
				frm.s_loc.value =colArray[3];
			}
		}	  
		
		function s_cntr_tpsz_OnCheckClick(comboObj, index, code) {
			if(index==0) {
		 ...
		 * customer Popup
		 */
		function getCustomerCode(rowArray){
			var formObj = document.form;
			var colArray = '';
		...
		function validateForm(sheetObj,formObj,sAction){
			...
			case IBSEARCH :
				...
				if(formObj.s_ecc.value == "" ){
					ComShowCodeMessage("COM130403", "ECC"); //ECC 값을 입력하셔야 합니다;
					return false;
				}
				
				if( formObj.s_fm_dt.value == "" || formObj.s_to_dt.value == "" ){
					ComShowCodeMessage("COM130403", "MT Return Period"); 	// MT Return Period 값을 입력하셔야 합니다;
					return false;
				}
		...
		function checkPeriodValidation(objFm,objTo){
			...
			if ( sTo > sMax){
				ComShowCodeMessage("COM132001", "MT Return Period","2 Months"); // msgs['COM132001'] = '{?msg1} exceeds maximum duration {?msg2}.';
				return false;
			}
		...
		/**
		 * EAC transfer
		 * Drop Off Charge는 EAC I/F 를 한 건씩 처리 한다.
		 */		
		function eac_transfer(sheetObj,Row,Col){
			...
			for (var idx=0; idx < arrRow.length - 1 ; idx++){	// 마지막 불필요한 값을 실행 못하게 하기 위해 arrRow.length 에 - 1 처리.
				...
				mty_rtn_yd = ComTrim(sheetObj.CellValue(arrRow[idx],"act_rtn_yd_cd"));
			...
			// for Data Setting 
		   	var theURL = "ESD_EAS_0224.do?p_sys_div_cd="		+ "TRS"													// System 구분 : Transportation 
										+"&p_sys_if_cd="		+ "TR4"													// System 별 interface 구분 : TR4 (Drop Off Charge)
										...
										+"&p_if_mty_rtn_yd="	+ mty_rtn_yd;											// Empty Return Yard							

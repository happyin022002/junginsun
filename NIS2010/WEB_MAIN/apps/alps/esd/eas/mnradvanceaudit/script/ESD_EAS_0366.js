 ...
 ***************************************************************************************/
	/**
	 * @class ESD_EAS_0366 : 예)Multiple Repair CNTR by Area 조회 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESD_EAS_0366() {
...
//		doActionIBSheet(sheetObject,formObject,IBSEARCH);
	...
	function initCombo(comboObj, no){
		...
		switch(comboId){
			...
			case "s_cost_dtl_cd":
				...
				var sXml = sheetObjects[2].GetSearchXml("MNR_COMGS.do","" ,f_query,true);
			...
			case "sheet1":	  //IBSheet1 init
				with (sheetObj) {
				   ...
				   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++, dtData,		80,		daRight,	true,	"dv_value",			false,          "",       dfFloat,	2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daCenter,	true,	"ttl_no_of_cases",	false,          "",       dfNone,	0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		100,	daLeft,		false,	"vndr_nm",			false,          "",       dfNone,	0,     false,       true);
			...
			case "sheet2":	  //IBSheet1 init
				with (sheetObj) {
				   ...
				   //전체Edit 허용 여부 [선택, Default false]
					...
					var HeadTitle = "SEQ|RHQ|Office|CNTR No.|Type/Size|W/O CUR|W/O AMT|W/O AMT\n(USD)|W/O Issue\nDate|W/O No|W/O User|S/P Code|S/P Name|Cleaning\nType|Repair Date|Work Yard|W/O Verify\nResult|vndr_cnt";
					var headCount = ComCountHeadTitle(HeadTitle);
					...
					//데이터속성    [ROW, COL,  DATATYPE,   WIDTH, DATAALIGN, COLMERGE, SAVENAME,  KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
					InitDataProperty(0, cnt++, dtSeq,		70,		daCenter,	false,	"seq",				false,          "",       dfNone,	0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		80,		daRight,	false,	"wo_amt",			false,          "",       dfNumber,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,		daRight,	false,	"chg_cost_amt",		false,          "",       dfFloat,	2,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		90,		daCenter,	false,	"cl_type",			false,          "",       dfNone,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"rpr_rslt_dt",		false,          "",       dfNone,	0,     false,       true);
		...
		switch(sAction) {
		   case IBSEARCH:		//조회
			    if(validateForm(sheetObj, formObj, sAction)){
	    			...
	    			var sXml = sheetObj.GetSearchXml("ESD_EAS_0366GS.do",EasFrmQryString(formObj));
			...
			case IBROWSEARCH:	// confirm
				...
				sheetObj.DoSearch("ESD_EAS_0366GS.do", EasFrmQryString(formObj));
				break;
	...
	*/
	...
	function getBackEndJobStatus() {
		...
		var sXml = sheetNum.GetSearchXml("ESD_EAS_0366GS.do", FormQueryString(formObj));
	...
	function getBackEndJobLoadFile() {
		...
		ComOpenWait(false);
		var sXml = sheetNum.GetSearchXml("ESD_EAS_0366GS.do", FormQueryString(form));

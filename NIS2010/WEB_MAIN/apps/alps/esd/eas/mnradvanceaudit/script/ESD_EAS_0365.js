 ...
 ***************************************************************************************/
	/**
	 * @class ESD_EAS_0365 : 예)Multiple Repair CNTR by Period 조회 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESD_EAS_0365() {
...
//		doActionIBSheet(sheetObject,formObject,IBSEARCH);
	...
	function initForm(){
		...
		formObj.s_eq_tpsz_cd.Code2 = "";
//		formObj.s_ownership.Code2 = "";
		formObj.s_rcc_cd.Code2 = "";
		
		formObj.s_start_dt.value = ComGetDateAdd(null, "d", -90, "-");
		formObj.s_end_dt.value = ComGetDateAdd(null, "d", 0, "-");
		formObj.s_rpr_cnt.value = "3";
	}
	...
	function initCombo(comboObj, no){
		...
		switch(comboId){
			...
			break;
//			case "s_ownership":
//				getEasIbComboList(formObj.s_ownership , s_ownershipCode , s_ownershipText , 'ALL');
//			break;
			case "s_rcc_cd":
				getEasIbComboList(formObj.s_rcc_cd , s_rcc_cdCode , s_rcc_cdText , 'ALL');
			break;
		}
	}
			...
			case "sheet1":	  //IBSheet1 init
				with (sheetObj) {
				   ...
				   //전체Edit 허용 여부 [선택, Default false]
					...
					InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"lstm_cd",			false,          "",       dfNone,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		70,		daCenter,	false,	"ownr_co_cd",		false,          "",       dfNone,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"entry_day",		false,          "",       dfNone,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,		daRight,	false,	"using_days",		false,          "",       dfNumber,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		90,		daRight,	false,	"dv_value",			false,          "",       dfFloat,	2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,		daCenter,	false,	"no_of_case",		false,          "",       dfNone,	0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		100,	daRight,	false,	"total_amt",		false,          "",       dfFloat,	2,     false,       true);
		...
		switch(sAction) {
		   case IBSEARCH:		//조회
			    if(validateForm(sheetObj, formObj, sAction)){
	    			...
	    			var sXml = sheetObj.GetSearchXml("ESD_EAS_0365GS.do",EasFrmQryString(formObj));
			...
			case IBROWSEARCH:	// confirm
				...
				sheetObj.DoSearch("ESD_EAS_0365GS.do", EasFrmQryString(formObj));
				break;
		...
		switch(sAction) {
			case IBSEARCH:
				...
				if(ComGetDaysBetween(formObj.s_start_dt, formObj.s_end_dt) < 0){
	                ...
	                ComSetFocus(formObj.s_start_dt);
	                return false;
				} else if(!ComIsNull(formObj.s_prefix_eq_no)) {			// 장비번호 앞자리 4자리만 허용
					if(formObj.s_prefix_eq_no.value.length != 4){
						ComShowMessage(ComGetMsg("COM12114", "Serial Range. Only four characters can be entered"));
						formObj.s_prefix_eq_no.focus();
						return false;
					} else if(formObj.s_from_eq_no.value.length != 7) {	// 장비번호 뒷자리 7자리만 허용
						ComShowMessage(ComGetMsg("COM12114", "Serial Range. Only seven characters can be entered"));
						formObj.s_from_eq_no.focus();
						return false;
					} else if(formObj.s_from_eq_no.value.length != 7) {	// 장비번호 뒷자리 7자리만 허용
						ComShowMessage(ComGetMsg("COM12114", "Serial Range. Only seven characters can be entered"));
						formObj.s_to_eq_no.focus();
						return false;
					}
				}
	...
	function getBackEndJobStatus() {
		...
		var sXml = sheetNum.GetSearchXml("ESD_EAS_0365GS.do", FormQueryString(formObj));
	...
	function getBackEndJobLoadFile() {
		...
		ComOpenWait(false);
		var sXml = sheetNum.GetSearchXml("ESD_EAS_0365GS.do", FormQueryString(form));

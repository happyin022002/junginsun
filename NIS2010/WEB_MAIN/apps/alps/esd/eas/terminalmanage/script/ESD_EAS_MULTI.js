...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
	...
	function comPopupOK(sheetObj,formObject) {
			...
			for(var i = 0; i < rows; i++) {
				if(sheetObj.CellValue(i, "check") == 1) {
					...
					for(var j=0; j<cArray.length; j++) {
						...
						if(chkval=="") {  
							ComShowCodeMessage("EAS05014", "data"); 
		...
		if(errcnt > 0){       
			ComShowCodeMessage("EAS00144", "Input data"); 
			sheetObj.CellEditable(dupcnt,'multiplekey') = true;
			sheetObj.SelectCell(dupcnt, 'multiplekey');  
		}else{
			if(opener != null) {
				// 일반 팝업인 경우
				opener.getEas_Multi(rArray,return_val);  //호출하는 부모js에 getMnr_Multi 붙여넣으면됩니다.
			} else {
				// Modal 팝업인 경우
				window.dialogArguments.getEas_Multi(rArray,return_val);
			}

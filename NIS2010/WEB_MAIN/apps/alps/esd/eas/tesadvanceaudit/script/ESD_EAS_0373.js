...
*@FileName : ESD_EAS_0373.js
*@FileTitle : TES Auto Audit - Off-Dock CY Audit Detail
*Open Issues :
*Change history :
 ...
 ***************************************************************************************/
	/**
	 * @class ESD_EAS_0373 : 예)M&R Invoice Charge 조회 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESD_EAS_0373() {
		...
		try {
            ...
            case "btn_agreement" :
            	if( objs[0].style.display == "inline" ){
            		...
            		check_openAgreement(sheetObject3);
				}
       			break;

            case "btn_eac_transfer" :
            	if( objs[0].style.display == "inline" ){
            		openEacTransfer(sheetObject, 'TM');
				}else if( objs[1].style.display == "inline" ){
					openEacTransfer(sheetObject2, 'SD');
				}else if( objs[2].style.display == "inline" ){
					openEacTransfer(sheetObject3, 'SP');
				}
       			break;
	...
	*/
		...
		switch(sheetNo) {
			case "sheet1":	  //IBSheet1 init
				with (sheetObj) {
					...
					var HeadTitle  = HeadTitleCM + "|Estimation|Estimation|Average(3M)|Average(3M)|(Avg)\nRate" + HeadTitleHD;
					var HeadTitle2 = HeadTitleCM + "|Volume|Amount|Volume|Amount|(Avg)\nRate" + HeadTitleHD;
					...
					InitDataProperty(0, cnt++, dtData,		60,	daCenter,	true,	"rc_flg",			false,          "",       dfNone,    0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtAutoSum,	90,	daRight,	true,	"inv_amt",			false,          "",       dfFloatOrg,    2,     false,       true);
			...
			case "sheet2":	  //IBSheet1 init
				with (sheetObj) {
					...
					//행정보설정[필수][HEADROWS,DATAROWS,VIEWROWS,ONEPAGEROWS=100]
					InitRowInfo(2, 1, 10, 100);
					var HeadTitleCM= "|Sel.|Seq.|Cal.\nType|Cost\nCode|Type/\nSize|DG|Cntr\nVol|Year/\nMonth|Stay\nDays|F/Days|Paid\nDays|Exclude\nDays|Over\nDays|UOM|Rate|Agmt\nCurr.|Exch.\nRate|Inv.\nAmount|Remark|Verify\nResult";
					var HeadTitleHD= "|EAC\nI/F|exceed_avg_flg|tml_inv_tp_cd|yd_cd|vndr_seq|vndr_nm|inv_no|inv_cfm_dt|inv_ofc_cd|sto_cntr_sz_nm|expn_aud_seq|fm_prd_dt|to_prd_dt|tml_calc_ind_cd|sto_dys_ind_cd|agmt_no|agmt_ver_no|lgs_cost_full_nm";
					var HeadTitle  = HeadTitleCM + "|Average(3M)|Average(3M)|Average(3M)|(Avg)\nRate|Estimation|Estimation|Estimation" + HeadTitleHD;
					var HeadTitle2 = HeadTitleCM + "|Volume|Over Days|Amount|(Avg)\nRate|Volume|Over Days|Amount" + HeadTitleHD;
					...
					InitDataProperty(0, cnt++, dtData,		50,	daCenter,	true,	"lgs_cost_cd",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		40,	daCenter,	true,	"cntr_tpsz_cd",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daCenter,	true,	"dcgo_ind_flg",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	60,	daRight,	true,	"cntr_vol",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daCenter,	true,	"rev_yrmon",		false,          "",       dfNone,    0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60,	daCenter,	true,	"curr_cd",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daRight,	true,	"inv_xch_rt",		false,          "",       dfFloatOrg,    2,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	100,daRight,	true,	"inv_amt",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daLeft,		true,	"calc_rmk",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60, daCenter,	true,	"dscr_ctnt",		false,          "",       dfNone,    0,     false,       true);
			...
			case "sheet3":	  //IBSheet1 init
				with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtData,		70,	daCenter,	true,	"wrk_dt",			false,          "",       dfDateYm,  0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60,	daLeft,		true,	"calc_rmk",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	80,	daRight,	true,	"avg_vol",			false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	100,daRight,	true,	"avg_amt",			false,          "",       dfFloatOrg,    2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,	daRight,	true,	"avg_rt",			false,          "",       dfFloatOrg,    2,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	80,	daRight,	true,	"estm_vol",			false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	100,daRight,	true,	"estm_amt",			false,          "",       dfFloatOrg,    2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		30,	daCenter,	true,	"eac_flg",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"tml_inv_tp_cd");
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
		   case IBSEARCH:		//조회
				...
				var sXml = sheetObj.GetSearchXml("ESD_EAS_0373GS2.do", FormQueryString(formObj));
	...
	function sheet3_OnSearchEnd(sheetObj, errMsg){
		var formObj = document.form;
		ComSetObjValue(formObj.inv_ttl_amt3, sheetObj.SumText(0, "inv_amt"));
		ComSetObjValue(formObj.estm_ttl_amt3, sheetObj.SumText(0, "estm_amt"));
		ComSetObjValue(formObj.diff_amt3, ComAddComma2(ComAddComma(sheetObj.SumValue(0, "inv_amt")-sheetObj.SumValue(0, "estm_amt")), "#,###.00"));
 	}
	...
	function openEacTransfer(sheetObj, type){
		...
		if (arrRow.length == 1) {
			...
			var sStoCntrSzNm	= "";
			if ( type == "TM" ) {
				sRcFlg	= sheetObj.CellValue(sheetObj.SelectRow, "rc_flg")
			}
			if ( type == 'SP' ) {
				sFpCalcPrdCd = sheetObj.CellValue(sheetObj.SelectRow, "fp_calc_prd_cd")
			}
			if ( type != 'SP' ) {
				sStoCntrSzNm = sheetObj.CellValue(sheetObj.SelectRow, "sto_cntr_sz_nm")
			}
			...
			var sParam = "?"
				...
				+ "&p_if_cntr_tpsz_cd="
				+ "&p_if_sto_cntr_sz_nm="+sStoCntrSzNm
				+ "&p_if_io_bnd_cd="
				+ "&p_if_dcgo_flg="
				+ "&p_if_rc_flg="+sRcFlg
				+ "&p_if_tml_wrk_dy_cd="
	 ...
	 * @param y_pos
	 */	
	function sheet2_OnMouseMove(sheetObj, buttonValue, shiftValue, x_pos, y_pos)
	{
		sheetToolTip(sheetObj, "lgs_cost_cd", "lgs_cost_full_nm");
	}

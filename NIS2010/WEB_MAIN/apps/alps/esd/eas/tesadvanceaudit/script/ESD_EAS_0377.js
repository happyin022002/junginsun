 ...
 ***************************************************************************************/
	/**
	 * @class ESD_EAS_0377 : 예)M&R Invoice Charge 조회 화면에서 사용하는 업무 스크립트를 정의한다.
	 */
	function ESD_EAS_0377() {
	...
	function processButtonClick(){
		...
		try {
            ...
            case "btn_agreement" :
       			if(sheetObject.SelectRow >= sheetObject.HeaderRows){
       				var vtmlAgmtSeq = sheetObject.CellValue(sheetObject.SelectRow, "agmt_no");
	...
	*/
		...
		switch(sheetNo) {
			case "sheet1":	  //IBSheet1 init
				with (sheetObj) {
					...
					var HeadTitleCM= "|Sel.|Seq.|Cal.\nType|Cost\nCode|Type/\nSize|I/O|DG|R/F|Applied\nDate|IPC|Mode|Lane|Vol.\nTier|Cal.\nVol|Rvis.\nVol|UOM|Rate|Agmt\nCurr.|Exch.\nRate|Inv. Amount|Remark|Verify\nResult|Vol Diff";
					var HeadTitleHD= "|EAC\nI/F|tml_inv_tp_cd|yd_cd|vndr_seq|inv_no|inv_ofc_cd|fm_prd_dt|to_prd_dt|tml_calc_ind_cd|sto_dys_ind_cd|agmt_no|agmt_ver_no|lgs_cost_full_nm";
					var HeadTitle  = HeadTitleCM + "|Estimation|Estimation|Estimation|Diff\nRatio(%)|3 Months(of 3 Months ago)|3 Months(of 3 Months ago)|3 Months(of 3 Months ago)|3 Months(of 3 Months ago)|Agmt./Avg.\nRate|Agmt./Avg.\nRate|No of\nVoy" + HeadTitleHD;
					var HeadTitle2 = HeadTitleCM + "|Mvmt Vol.|Est Vol.|Est Amt|Diff\nRatio(%)|Inv Vol.|Mvmt Vol.|Frequency(%)|Amount|Agreement|Average|No of\nVoy" + HeadTitleHD;
					...
					InitDataProperty(0, cnt++, dtData,		30,	daCenter,	true,	"rc_flg",			false,          "",       dfNone,    0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60,	daRight,	true,	"tier_vol",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	60,	daRight,	true,	"calc_vol_qty",		false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	60,	daRight,	true,	"rvis_vol_qty",		false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		50,	daCenter,	true,	"vol_tr_ut_cd",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,	daRight,	true,	"ctrt_rt",			false,          "",       dfFloatOrg,    	2,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60, daCenter,	true,	"dscr_ctnt",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60, daCenter,	true,	"vol_diff_flg",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	60,	daRight,	true,	"vvd_vol",			false,          "",       dfNullInteger,    0,     false,       true);
					...
					InitDataProperty(0, cnt++, dtData,		60,	daRight,	true,	"n3mon_tot_inv_vol",	false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daRight,	true,	"n3mon_tot_vol",	false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		90,	daRight,	true,	"freq"				,	false,          "",       dfFloat,   1,     false,       true);
					InitDataProperty(0, cnt++, dtAutoSum,	100,daRight,	true,	"n3mon_tot_amt",	false,          "",       dfFloatOrg,    	2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,	daRight,	true,	"agmt_rt"			,	false,          "",       dfFloatOrg,    	2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		80,	daRight,	true,	"unit_avg_cost"	,	false,          "",       dfFloatOrg,		2,     false,       true);
					InitDataProperty(0, cnt++, dtData,		60,	daRight,	true,	"vvd_cnt",			false,          "",       dfNullInteger,    0,     false,       true);
					InitDataProperty(0, cnt++, dtData,		30,	daCenter,	true,	"eac_flg",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"tml_inv_tp_cd",	false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"yd_cd",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"vndr_seq",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"inv_no",			false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"inv_ofc_cd",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"fm_prd_dt",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"to_prd_dt",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"tml_calc_ind_cd",	false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"sto_dys_ind_cd",	false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"agmt_no",		false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"agmt_ver_no",	false,          "",       dfNone,    0,     false,       true);
					InitDataProperty(0, cnt++, dtHidden,	60,	daCenter,	true,	"lgs_cost_full_nm",	false,          "",       dfNone,    0,     false,       true);
					CountPosition = 0 ;
					HeadRowHeight = 20;
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
		   case IBSEARCH:		//조회
				...
				var sXml = sheetObj.GetSearchXml("ESD_EAS_0377GS2.do", FormQueryString(formObj));
	...
	function sheet1_OnSearchEnd(sheetObj, errMsg){
		...
		ComSetObjValue(formObj.estm_ttl_amt, sheetObj.SumTExt(0, "estm_amt"));

		var vInv_amt = ComReplaceStr(sheetObj.SumText(0, "inv_amt"), ",","");
		var vEstm_amt = ComReplaceStr(sheetObj.SumText(0, "estm_amt"), ",","");
		ComSetObjValue(formObj.diff_amt, ComAddComma2(ComAddComma(ComTrunc(vInv_amt-vEstm_amt, 2)), "#,###.00"));
 	}
    ...
    function sheet1_OnClick(sheetObj, Row, Col){
    	var formObj = document.form;
		ComSetObjValue(formObj.s_lgs_cost_cd, sheetObj.CellValue(sheetObj.SelectRow, "lgs_cost_cd"));
    }
    ...
    /**
     * Container Volume 조회
     * Booking/Documentation > Operation > Loading/Discharging List > Container Discharging List (CDL) POPUP OPEN
     */
	function openCntrVol() {
		var formObj = document.form;
		var sUrl = "";
		sUrl = "/hanjin/ESM_BKG_0159-2.do";
		var sParam = "?MENU=Y&pgmNo=ESM_BKG_0159-2"
		sUrl += sParam;
	...
	/**
	 * Marine Terminal Invoice History Retrive
	 */
	function openHistory(){
		var formObj = document.form;
		var sUrl = "";
		sUrl = "/hanjin/ESD_EAS_0378.do";
		var sParam = "?"
		...
		+"&s_lgs_cost_cd="	+ ComGetObjValue(formObj.s_lgs_cost_cd)
		;	
		sUrl += sParam;
	...
	function openEacTransfer(sheetObj){
		...
		if (arrRow.length == 1) {
			...
			var sParam = "?"
				...
				+ "&p_if_dcgo_flg="
				+ "&p_if_rc_flg="+sheetObj.CellValue(sheetObj.SelectRow, "rc_flg")

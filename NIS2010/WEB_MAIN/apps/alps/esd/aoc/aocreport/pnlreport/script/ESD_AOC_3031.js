 ...
 *@FileName : ESD_AOC_3031.js
 *@FileTitle : EUR Profit & Loss report for Inland BIZ
 *Open Issues :
 *Change history :
...
/**
 * @extends 
 * @class ESD_AOC_3031 : ESD_AOC_3031 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_AOC_3031() {
 	...
 	switch(elementObj.name){
 		case "s_cust_cd":
 			if(!isNull(elementObj.value)){
				...
				var sXml = sheetObjects[1].GetSearchXml("ESD_AOC_3031GS.do", FormQueryString(formObj));
	...
	switch(sheetNo) {
		case 1:      //sheet1 init
			with (sheetObj) {
				...
				var HeadTitle1 = 				"Sts|ALL|SEQ|div|grp_no|Contract\nOffice|Service\nScope|Bound|Bound|W/O\nOffice|Customer\nType|Customer\nType|Revenue\nType|Revenue\nType|Parent S/P|Parent S/P|From|Via|To|Door|Distance|Distance";
					HeadTitle1 = HeadTitle1 + 	"|TTL No\nof BKG|TTL Cntr\nin BKG|TTL Box\nin INV|TTL TEU\nin INV|Total\nRev(USD)";
					HeadTitle1 = HeadTitle1 + 	"|Total Cost(USD) by Cost Type|Total Cost(USD) by Cost Type|Total Cost(USD) by Cost Type|Total Cost(USD) by Cost Type|Total Cost(USD) by Cost Type|Total Cost(USD) by Cost Type";
					HeadTitle1 = HeadTitle1 + 	"|P&L(USD) by Cost Type|P&L(USD) by Cost Type|P&L(USD) by Cost Type|P&L(USD) by Cost Type";
				
				var HeadTitle2 = 				"Sts|ALL|SEQ|div|grp_no|Contract\nOffice|Service\nScope|Bound|Bound|W/O\nOffice|Customer\nType|Customer\nType|Revenue\nType|Revenue\nType|Code|Name|From|Via|To|Door|Km|R.Link";
		...
		case 4:      // Excel grid (hidden) - Sales View original DB data
			with (sheetObj) {
				//높이 설정
				style.height = GetSheetHeight(12);
				//전체 너비 설정
		...
		case 5:      // Excel grid (hidden) - Operation View original DB data
			with (sheetObj) {
				//높이 설정
				style.height = GetSheetHeight(12);
				//전체 너비 설정
	...
	switch(sAction) {
		case IBSEARCH:		//Retrieve
			if(!validateForm(sheetObj,formObj,sAction)) return;
	   		...
	   		formObj.s_search.value = AocFrmQryString(formObj); //s_search는 호출 된 팝업(ESD_AOC_3032)에서 사용할 조회 조건
	   		var sXml = sheetObj.GetSearchXml("ESD_AOC_3031GS.do", AocFrmQryString(formObj));
	...
	switch(sAction){
		case IBSEARCH:
			if( formObj.r_prd_bkg_div[0].checked == true ){
				...
				if( diffDate > 91 ){
					ComShowCodeMessage("AOC90043","Three Months");
					return false;
				}
			} else{
				//BKG No 또는 RFA No 입력 여부 체크
				if( formObj.s_bkg_no.value == "" && formObj.s_ctrt_no.value == "" ){
...
function initForm(){
	...
	comboObjects[3].Text = "All"; //Profit & Loss -> All로 변경 (2016-06-27 이민경 CHM-201642128 : [AOC] Profit & Loss Report for inland BIZ 변경)
...
function sheet1_OnSearchEnd(sheetObj,errMsg) {
	for( var idx = parseInt(sheetObj.HeaderRows); idx <= sheetObj.LastRow; idx++ ){
		if( sheetObj.CellValue(idx,"div") == "2" ){
			sheetObj.CellFont("FontBold", idx, "ibflag", idx, "pl_inv_usd_amt") = true;
...
function sheet2_OnSearchEnd(sheetObj,errMsg) {
	...
	if(formObj.r_view[0].checked == true){
		formObj.s_wo_ofc_cd2.value = "";
		formObj.c_wo_sub_ofc2.checked = false;
		formObj.s_io_bnd_cd.value = "";
		formObj.s_ctrt_ofc_cd2.value = "";
		formObj.c_ctrt_sub_ofc2.checked = false;
	} else if (formObj.r_view[1].checked == true){
		formObj.s_ctrt_ofc_cd1.value = "";
		formObj.c_ctrt_sub_ofc1.checked = false;
		comboObjects[0].Text = "All";
		formObj.s_wo_ofc_cd1.value = "";
		formObj.c_wo_sub_ofc1.checked = false;
	}	
...
function combo_pnl_div_OnChange(comboObj, Index_Code, Text) {
	if (Index_Code == 'A') {
		if(comboObjects[3].GetCount() == 4) return;
		comboObjects[3].DeleteItem('P');
		comboObjects[3].DeleteItem('L');
...
* 콤보가 다수일 경우 콤보 수만큼 case를 추가하여 시트 초기화모듈을 구성한다 
*/ 
function insertCombo_pnl_div() {
	if(comboObjects[3].GetCount() == 4) return;
	var formObj = document.form;	
	formObj.f_cmd.value = SEARCH01;
	var sXml = sheetObjects[2].GetSearchXml("ESD_AOC_3031GS.do", AocFrmQryString(formObj));
	var arrXml = sXml.split("|$$|");
  	ComXml2ComboItem(arrXml[3], formObj.combo_pnl_tp, "val", "name");
  	comboObjects[3].Index = 0;
}
...
function popUpCall(p_url, p_div, p_row){
	...
	var vEnd = "')";
	var vSeparator = "','";
	var sUrl = "/hanjin/ESD_AOC_3032.do";
	...
	//s_param 은 팝업 창에서 조회 조건으로 사용(ESD_AOC_3032)
	...
	window.open(sUrl + param, "", myOption);
...
function chkOptTp(param){
	if( param == "P" ){
		...
		document.form.s_xcld_svc_scp.disabled = false;
		document.form.c_xcld_ofc.disabled = false;
		...
		document.form.c_wo_sub_ofc2.disabled = false;
		document.form.s_wo_ofc_cd2.className = "input1";
		document.form.c_ctrt_sub_ofc2.disabled = false;
		...
		document.form.s_ctrt_no.className = "input2";
		...
		document.form.s_ctrt_no.readOnly = true;
	} else{
		for( var idx = 0; idx < document.form.f_chkprd.length; idx++ ){
		...
		document.form.s_xcld_svc_scp.disabled = true;
		document.form.c_xcld_ofc.disabled = true;
		...
		document.form.c_wo_sub_ofc2.disabled = true;
		document.form.s_wo_ofc_cd2.className = "input2";
		document.form.c_ctrt_sub_ofc2.disabled = true;
	    ...
	    document.form.s_ctrt_no.className = "input";
		...
		document.form.s_ctrt_no.readOnly = false;
	}
...
function chkView(param){
	if( param == "S" ){
//		document.all.div_rhq_cd.style.display = "inline";
...
//		document.all.div_rhq_cd.style.display = "none";
...
function setPeriod(obj){
	...
	if( chkValidPeriod() ){
		...
		document.getElementById("div_period").innerHTML = ".....";
		formObj.f_cmd.value = SEARCH01;
		sheetObj.DoSearch4Post("ESD_AOC_0999GS.do", AocFrmQryString(formObj));
...
function chkValidPeriod(){
	...
	with(formObj){
		...
		if( f_chkprd[1].checked && i_fm_wm.value > i_to_wm.value ){
			ComShowMessage(ComGetMsg("AOC90044","Month of From should be equal to or less than To."));
			i_to_wm.focus();
			return false;
		}	
		if( f_chkprd[0].checked && i_fm_wm.value > i_to_wm.value ){
			ComShowMessage(ComGetMsg("AOC90044","Week of From should be equal to or less than To."));
			i_to_wm.focus();
			return false;
		}
...
function chkSubOfc(obj) {
	...
	if( obj.checked == true ){
		...
		var sParam = FormQueryString(document.form) + "&ctrl_so_office="+ofcCd;;
		var sXml = sheetObjects[2].GetSearchXml("ESD_AOC_0999GS.do", sParam);

		if( obj.name == "c_ctrt_sub_ofc1" ){
			document.form.s_ctrt_ofc_cd1.value = ComGetEtcData(sXml, "subOfcCd").substr(1,ComGetEtcData(sXml, "subOfcCd").length);
		} else if( obj.name == "c_wo_sub_ofc1" ){
			document.form.s_wo_ofc_cd1.value = ComGetEtcData(sXml, "subOfcCd").substr(1,ComGetEtcData(sXml, "subOfcCd").length);
		} else if( obj.name == "c_wo_sub_ofc2" ){
			document.form.s_wo_ofc_cd2.value = ComGetEtcData(sXml, "subOfcCd").substr(1,ComGetEtcData(sXml, "subOfcCd").length);
		} else if( obj.name == "c_ctrt_sub_ofc2" ){
			document.form.s_ctrt_ofc_cd2.value = ComGetEtcData(sXml, "subOfcCd").substr(1,ComGetEtcData(sXml, "subOfcCd").length);
		}
	} else {
		if( obj.name == "c_ctrt_sub_ofc1" ){

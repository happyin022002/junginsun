...
/**
 * @extends 
 * @class ESD_AOC_3111 : ESD_AOC_3111 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_AOC_3111() {
 	...
 	switch(elementObj.name){ 	    	
 		case "cnt_cd":
 			if(!isNull(elementObj.value)){
				...
				var sXml = sheetObjects[1].GetSearchXml("ESD_AOC_3111GS.do", FormQueryString(formObj), true);
	...
	switch(sAction) {
		case IBSEARCH:		//Retrieve
			if(!validateForm(sheetObj,formObj,sAction)) return;
	   		...
	   		sheetObj.DoSearch4Post("ESD_AOC_3111GS.do", AocFrmQryString(formObj));
	   		break;
	   		
		case SEARCH01:		//RHQ Office Code
			if(!validateForm(sheetObj,formObj,sAction)) return;
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("ESD_AOC_3111GS.do", AocFrmQryString(formObj));
...
function goCostTrfMgmt(){
	...
	if( sheetObjects[0].Cellvalue(sheetObjects[0].SelectRow,"trf_tp") == "Inland" ){
		var sUrl = "";
		var winNm = "";
		if(document.form.rhq_cd.value == "SHARC"){
		...
		var sUrl = "/hanjin/ESD_AOC_3122.do";
		var winNm = "ESD_AOC_3122";
		var param = '?trf_no='+sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"cost_trf_no");
		var myOption = "width=1024,height=640,menubar=0,status=0,scrollbars=0,resizable=1";
	}

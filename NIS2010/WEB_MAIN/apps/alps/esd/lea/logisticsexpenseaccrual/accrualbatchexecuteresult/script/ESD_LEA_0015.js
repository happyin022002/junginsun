...
* 1.0 Creation
* @History
* 2009-03-27  Volume Incentive 및 Other Carrier 조회 옵션 조건 추가
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_LEA_0015 : ESD_LEA_0015 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    		...
    		try {
    	       ...
    	       switch(srcName) {
                ...
                case "btns_calendar1":
       	         	...
       	         	cal.select(formObject.frm_exe_yrmon_from, 'yyyy-MM');
       	         	break;

                case "btns_calendar2":
       	         	var cal = new ComCalendar();
       	         	cal.setDisplayType('month');
       	         	cal.select(formObject.frm_exe_yrmon_to, 'yyyy-MM');
       	         	break;
                    ...
                    with (sheetObj) {
                     ...
                     // 높이 설정
                       ...
                       // 해더에서 처리할 수 있는 각종 기능을 설정한다
                        ...
                        var HeadTitle  = "G/L\n Month|Rev.\n Month|RHQ|Parent\nOffice|Control\nOffice|S/P Seq|S/P Name|Cost Type Ⅰ|Cost Type Ⅱ|Cost\n Code|Acct\n Code|1st\n Node|2nd\n Node|3rd\n Node|4th\n Node|Currency|Local\n Amount|USD\n Amount" ;
    					...
    					InitDataProperty(0, cnt++ , dtData ,	300,    daLeft,		true ,  "vndr_lgl_eng_nm"	,	false,          "",       dfNone ,   		0,     false,	false); 	
    					...
    					InitDataProperty(0, cnt++ , dtData ,	90,    	daRight ,  	true ,	"locl_cost_amt"		,	false,          "",       dfFloat ,			2,     false,	false);  
    					InitDataProperty(0, cnt++ , dtAutoSum ,	90,    	daRight ,  	true ,	"usd_cost_amt"		,	false,          "",       dfFloat ,			2,     false,	false); 
...
//    	}
        ...
        function validateForm(sheetObj,formObj,sAction){
        	var rtn = true;
            with(formObj){
            	if((12*(formObj.frm_exe_yrmon_to.value.substr(0,4) - formObj.frm_exe_yrmon_from.value.substr(0,4))
                 		+ (formObj.frm_exe_yrmon_to.value.substr(5,2) - formObj.frm_exe_yrmon_from.value.substr(5,2))+1)>3)	
            	{
            		ComShowMessage(ComGetMsg("LEA90028","3"));
            		
            		rtn = false;
            	}else if( formObj.f_cost_type_f.checked == false && formObj.f_cost_type_m.checked == false && formObj.f_cost_type_c.checked == false &&
        				formObj.f_cost_type_fv.checked == false && formObj.f_cost_type_ev.checked == false && formObj.f_other_carrier_expense.checked == false) {
        			ComShowMessage(ComGetMsg("COM12113", "Check Box"));
       			
        			rtn = false;
        		
        		}
        		else if ( formObj.frm_exe_yrmon_from.value =='' || formObj.frm_exe_yrmon_to.value == '') {
        			ComShowMessage(ComGetMsg("LEA90001"));
        			ComSetFocus(formObj.frm_exe_yrmon_from);
        			rtn = false;		
        		}
        		else {
        			rtn = true;
    	...
    	*/
    	function changeReport(tp){
    	    ...
    	    if(tp == '1') {// //Office 비활성화, RHQ만 선택 가능
            	...
            	formObj.f_ctrl_ofc_cd.display = "All";
...
//    		setCookie("form_lea_cost_type_f",(document.form.f_cost_type_f.checked == true ?"1":"0"));
    		...
    		document.form.f_other_carrier_expense.value = (document.form.f_other_carrier_expense.checked == true ?"1":"0");
    	}
...
//    	}
         ...
         */
        function dateSetup() {
            if(document.form.frm_exe_yrmon_to.value.substr(5,2)== '01'){
           	 document.form.frm_exe_yrmon_from.value = ((document.form.frm_exe_yrmon_to.value.substr(0,4)-1) + "-11");
            }else if(document.form.frm_exe_yrmon_to.value.substr(5,2)== '02'){
           	 document.form.frm_exe_yrmon_from.value = ((document.form.frm_exe_yrmon_to.value.substr(0,4)-1) + "-12");
            }else if(document.form.frm_exe_yrmon_to.value.substr(5,2)== '12'){
           	 document.form.frm_exe_yrmon_from.value = ((document.form.frm_exe_yrmon_to.value.substr(0,4)) + "-10");
            }else
            {

           	 document.form.frm_exe_yrmon_from.value = ((document.form.frm_exe_yrmon_to.value.substr(0,4)) + "-0" + (document.form.frm_exe_yrmon_to.value.substr(5,2)-2));
            }

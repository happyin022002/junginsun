/**
 * @fileoverview Marine Terminal Storage Invoice Creation & Correction -- 3rd Party Billing Input 화면에서 사용하는 업무 스크립트를 정의한다.
 * @author SM LINE
 */

/**
 * @extends Tes
 * @class ESD_TES_9234 : Marine Terminal Storage Invoice Creation & Correction -- 3rd Party Billing Input 화면에서 사용하는 업무 스크립트를 정의한다.
 */
		 ...
		 case 1:      //sheet1 init
			with (sheetObj) {
			   ...
			   //전체Edit 허용 여부 [선택, Default false]
				...
				InitDataProperty(0, cnt++, dtCombo,     100,    daLeft,  false,    "n3pty_bil_tp_cd",       true,           "",       dfNone,    0,     true,       true);
				// 2008-07-02 3rd party interface 로직변경요청 CSR start
				...
				InitDataProperty(0, cnt++, dtHidden,       65,  daRight,   true,    "tmp_dtl_seq",        false,          "",    dfNone);
				InitDataProperty(0, cnt++, dtHidden,       65,  daRight,   true,    "tmp_tpb_seq",        false,          "",    dfNone);
...
//			    }
				...
				sheetObj.DoSearch4Post("ESD_TES_9234Popup.do", tesFrmQryStr(formObj));
			    break;
            ...
            case IBSAVE: 
				...
				if (sheetObj.RowCount > 0 && sheetObj.IsDataModified){
				    ...
				    for (var i=sheetObj.HeaderRows; i<(sheetObj.HeaderRows + sheetObj.RowCount); i++){
// 여기서 체크박스 체크해야 할듯... 체크박스가 체크된게 한건이라도 없으면 alert후 리턴.
                    ...
                    // 체크박스 체크된 값이 한건이라도 없으면 Alert Return... 해야할 것									
					...
					var savexml = sheetObj.GetSaveXml("ESD_TES_9234Popup.do", param+'&'+tesFrmQryStr(formObj));
...
//			if (document.form.calc_tp_cd.value=='A'){
				for (var i=1; i<=sheetObj.RowCount; i++){
					...
					if ((sheetObj.CellValue(i,'tml_if_ofc_cd')==null || sheetObj.CellValue(i,'tml_if_ofc_cd')=='') || 
						(sheetObj.CellValue(i,'tml_if_seq')==null || sheetObj.CellValue(i,'tml_if_seq')=='')) {
...
//			}
	...
	function sheet1_OnSaveEnd(sheet, Row, Col, Value) {
	    // Opener Sheet 해당 Row에 3rd Party Cell에 'Y'표시

...
/**
 * @fileoverview Marine Terminal Storage Invoice Creation & Correction -- FileUpload 화면에서 사용하는 업무 스크립트를 정의한다.
 * @author SM LINE
 */

/**
 * @extends Tes
 * @class ESD_TES_9142 : Marine Terminal Storage Invoice Creation & Correction -- FileUpload 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_TES_9142() {
    ...
    /**
     * 지정한 row, columns editable 속성을 true 로 설정
	 * @param {ibsheet}	sheetObj		IBSheet Object
	 * @param {int}		rownum			특정 셀의 Row Index
	 * @param {int}		colnms			특정 셀의 Column Index 
     * @return
     */
	...
	*/
    ...
    function loadPage() {
	 ...
	 /**
	  * Coincidence, Discrepancy, CalcByDay sheet의 data 삭제
	  * @return
	  */
	function chkOprShtData(){
		if (hasOprShtData()){
			...
			if (!confirm(ComGetMsg('TES24067'))){
				...
				window.dialogArguments.removeStorageInvoice01();
				window.focus();
			}
			return false;
		}
	...
	/**
	 * Coincidence, Discrepancy, CalcByDay sheet의 data가 존재하는지 확인 
	 * @return
	 */
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
		   case IBSEARCH:      //조회
				...
				var sXml = sheetObj.GetSearchXml("ESD_TES_9142Popup.do", param+'&'+tesFrmQryStr(formObj),"",true);
           ...
           case IBSEARCH_ASYNC01:
                ...
                var param = sheetObj.GetSaveString(true,false);
                var searchXml = sheetObj.GetSearchXml("ESD_TES_9142Popup.do", param+'&'+tesFrmQryStr(formObj),"",true);
                sheetObj.RemoveAll();
           ...
           case IBSEARCH_ASYNC02:
                ...
                var searchXml = sheetObj.GetSearchXml("ESD_TES_9142Popup.do", tesFrmQryStr(formObj),"",true);
                sheetObj.RemoveAll();
				if (searchXml!=null && searchXml!='') sheetObj.LoadSearchXml(searchXml);
                break;
		   // eBilling - E
	...
	/**
	 * 입력받은 Gate In Date 와 Gate Out Date 의 유효성 검사
	 * @param {ibsheet}	sheet		IBSheet Object
	 * @return
	 */
	...
	function val_chk(sheet) {
		if (sheet.RowCount > 0) {
			...
			for (var i=1; i<=sheet.RowCount; i++){
				...
				if (!chkGtOutDt() && (sheet.CellValue(i,'inv_gate_out_dt')==null || sheet.CellValue(i,'inv_gate_out_dt')=='')) {
				...
				*/
				...
				if (sheet.CellValue(i,'cntr_sty_cd')==null || sheet.CellValue(i,'cntr_sty_cd')==''){
					isValid = false;
				}
				if (!isValid){
					sheet.RowFontColor(i) = sheet.RgbColor(255, 0, 0);

...
* 2010.10.27 최 선     1.3 [CHM-201006612] Expense Summary 조회 항목 추가 개발 요청
* 2010.12.17 손은주 1.4 [CHM-201007628] Report 화면에 Negotiated 값 반영 칼럼 추가 요청
* 2011.01.04 최 선     1.5 [CHM-201108174] Report 화면 중 S/P code 선택 시 오류팝업 수정요청
* 2011.02.08 최 선     1.6 [CHM-201108757] W/O Amount 20' Sum, 40' Sum 컬럼 추가
* 2011.07.20 민정호 1.7 [CHM-201112196] Expense Summary Report에 Invoice 상태코드를 조회조건에 추가 
* 2012.02.23 금병주 1.6 [CHM-201216258] [TRS] Expense Summary Excel Down 버튼 추가
...
/**
 * @extends Bkg
 * @class ESD_TRS_0107 : Booking ??? ?? ???? ???? ?? ????? ????.
 */
function ESD_TRS_0107() {
...
function initSheet(sheetObj,sheetNo) {
    ...
    if( sheetNo == 1 || sheetNo == 2 ) {
          with (sheetObj) {
              // 높이 설정
//                style.height = 240;
              if (sheetNo == 1) style.height = 340; else style.height = 0;
              //전체 너비 설정
             ...
             //전체Edit 허용 여부 [선택, Default false]
              ...
              var HeadTitle1 =  "Check\nBox|Seq|Office|S/P Code|S/P Name|S/P Code|SML Office|Month|Cost\nMode|Trans\nMode|BND|S/O Type|From|Via|To|Door|ETS|Km|R.Link|INV Curr|" +
              					...
              					"20’|40’|Teu|Box|" +
              					"D2|D4|D5|D7|R2|R4|R5|R7|R8|F2|F4|F5|A2|A4|A5|P2|P4|O2|O4|O5|O7|S2|S4|T2|T4|DX|DW" ;
              ...
              InitDataProperty(0, cnt++ , dtRadioCheck, 50,    daCenter, true,   "chk"                 , false,          "",       dfNone,       0,     false,       false,     false,       false,     false);
              ...
              InitDataProperty(0, cnt++ , dtData,       80,    daCenter, true,   "via_nod_cd"          , false,          "",       dfNone,       0,     false,       false);
              InitDataProperty(0, cnt++ , dtData,       80,    daCenter, true,   "to_nod_cd"           , false,          "",       dfNone,       0,     false,       false);
              InitDataProperty(0, cnt++ , dtData,       80,    daCenter, true,   "dor_nod_cd"          , false,          "",       dfNone,       0,     false,       false);
           ...
           // invoice surcharge d2 2012.02.23 kbj
              ...
              // invoice surcharge d4 2012.02.23 kbj
           ...
           // invoice surcharge 2012.02.23 kbj
           ...
           // 데이터 추가 W/O D2 Additional 2012.02.23 kbj
           ...
           // 데이터 추가 W/O D4 Additional 2012.02.23 kbj
           ...
           // 데이터 추가 W/O Additional 2012.02.23 kbj
           ...
           // 데이터 추가 W/O D Additional USD 2012.02.23 kbj
           ...
           // 데이터 추가 W/O D4 Additional USD 2012.02.23 kbj
           ...
           // 데이터 추가 W/O Additional USD 2012.02.23 kbj
           ...
           // 2012.05.29 [CHM-201217633] [TRS] 구주 Hinterland T/F 및 시스템 개발 Project  SHIN DONG IL
              ...
              RangeBackColor(1, 13, 1, 49) = RgbColor(222, 251, 248);  // ENIS   
              HeadRowHeight= 10;                                                
         }
        ...
        */
        switch(srcName) {
          ...
          case "btng_downexcel":
            ...
            { 
                //if (sheetObject1.CellValue(i,'div') != "00000000000") {
          ...
          case "btng_detailinquiry":
            ...
            { 
            	...
            	ComPostOpenWindow('ESD_TRS_0211.do', "noRtnPopup", "width=800,height=700,menubar=0,status=0,scrollbars=0,resizable=0");
...
function doActionIBSheet(sheetObj,formObj,sAction) {
  ...
  switch(sAction) {
	 case COMMAND01:
		 	...
		 	var sXml = sheetObj.GetSearchXml("ESD_TRS_0107GS.do", FormQueryString(formObj));
     ...
     case IBSEARCH:   //조회
      ...
      formObj.hid_to_date.value = removeBar(formObj.to_date.value);
			...
			if(formObj.hid_grid_flg.value == 'Y'){
				sheetObj.DoSearch4Post("ESD_TRS_0107GS.do", TrsFrmQryString(formObj));
			}else if(formObj.hid_grid_flg.value == 'N'){
			  	ComOpenWait(true);
			    formObj.f_cmd.value = SEARCH01;
			    formObj.target = "_blank"
				formObj.action = "ESD_TRS_0247.do?"+ TrsFrmQryString(formObj);
			    formObj.submit();
			    ComOpenWait(false);
			}
...
//조회 결과가 2000건이 넘을 경우 위 down xls 버튼을 클릭하라는 메시지 추가 2012.02.23 kbj
...
/**조회 조건 By Yard일 경우에만 Distance관련 컬럼을 보여준다.
...
function getCount(sheetObj, formObj){
	...
	var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_0107GS.do", TrsFrmQryString(formObj));

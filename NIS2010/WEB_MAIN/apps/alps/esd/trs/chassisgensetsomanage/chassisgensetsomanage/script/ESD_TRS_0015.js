...
/**
 * @extends Bkg
 * @class ESD_TRS_0015 : 예)Booking 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_TRS_0015() {
...
/* 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
            ...
            switch(srcName) {
        	    ...
        	    case "btn_apply":
					...
					opener.setPopupValue(formObject.pop_fm_loc.value, 
									document.pop_fm_yard.Code, 
									formObject.pop_to_loc.value, 
									document.pop_to_yard.Code,
									formObject.pop_transMode.value,
									formObject.pop_remark.value,
									self.window
									);
        	        break;
	...
	function getComboList(obj)
	{
		...
		var comboObj = eval('document.'+obj+'_yard');
	...
	*/
	function openHireYardPopup( btn_obj )
	{
		...
		//ComOpenPopup('/hanjin/COM_ENS_061.do', 770, 470, 'getCOM_ENS_061_2', "1,0,1,1,1,1,1,1,1,1");
	...
	*/
	function setFmNode(rowArray) {
		...
		document.form.pop_fm_loc.value = loc;
		getComboList('pop_fm');
		document.pop_fm_yard.CODE = yard;
	}
	...
	*/
	function setToNode(rowArray) {
		...
		document.form.pop_to_loc.value = loc;
		getComboList('pop_to');
		document.pop_to_yard.CODE = yard;
	}

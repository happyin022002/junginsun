...
/**
 * @extends 
 * @class ESD_TRS_3015 : ESD_TRS_3015 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_TRS_3015() {
 	...
 	switch(elementObj.name){ 	    	
 		case "cnt_cd":
 			if(!isNull(elementObj.value)){
				...
				var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_3015GS.do", FormQueryString(formObj), true);
	...
	switch(sAction) {
		case IBSEARCH:		//Retrieve
			if(!validateForm(sheetObj,formObj,sAction)) return;
	   		...
	   		sheetObj.DoSearch4Post("ESD_TRS_3015GS.do", TrsFrmQryString(formObj));
	   		break;
	   		
		case SEARCH01:		//RHQ Office Code
			if(!validateForm(sheetObj,formObj,sAction)) return;
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("ESD_TRS_3015GS.do", TrsFrmQryString(formObj));
		...
		case MULTI:		//Batch Creation
			...
			sheetObj.DoSave("ESD_TRS_3015GS.do", TrsFrmQryString(formObj), "chk");
			break;
...
function sheet1_OnClick(sheetObj,Row,Col,Value){
	...
	if( colNm == "sts_nm" && sheetObj.CellValue(Row,"sts_cd") == "E" ){
		...
		var myOption = "width=780,height=640,menubar=0,status=0,scrollbars=0,resizable=1";

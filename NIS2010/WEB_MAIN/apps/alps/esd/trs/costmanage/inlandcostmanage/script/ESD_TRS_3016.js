...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_TRS_3016 : esd_trs_3016 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esd_trs_3016() {
...
// 버튼 네임으로 구분하여 프로세스를 분기처리하는 이벤트핸들러 */
    ...
    function setRHQMultiCombo() {
		...
		var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_3016GS.do", sParam);
        ...
        switch(sheetID) {
            case "sheet1":
                with (sheetObj) {
					...
					InitDataProperty(0, cnt++, dtCombo, 75,		daCenter,	true,	prefix+"rhq_cd", 		true, "", dfNone, 0, true, true);
	...
	function sheet1_OnChange(sheetObj, Row, Col, Value){
		...
		if (sName == "sheet1_cnt_cd" && sheetObj.CellValue(Row , "sheet1_cnt_cd") != "") {
			...
			if(ComGetEtcData(sXml, "cnt_nm") == "" || ComGetEtcData(sXml, "cnt_nm") == "null"){
				...
				sheetObj.SelectCell(Row, "sheet1_cnt_cd", true);
			}else{
        ...
        switch(sAction) {
			case IBSEARCH: //Retrieve
				if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml = sheetObj.GetSearchXml("ESD_TRS_3016GS.do", sParam);
					if(sXml.length>0){
			...
			case MULTI01: //Save
				if(validateForm(sheetObj,formObj,sAction)){
					...
					sheetObj.DoSave("ESD_TRS_3016GS.do", TrsFrmQryString(formObj), -1,false);
//					var sParam =  ComGetSaveString(sheetObj, true, false);
//					if( sParam == ""){ return;}
//					var sXml = sheetObj.GetSaveXml("ESD_TRS_3016GS.do", FormQueryString(formObj) + "&" + sParam, true);	        			
//					sheetObj.LoadSaveXml(sXml);
			...
			case MULTI02: //Delete
				...
				sheetObj.DoSave("ESD_TRS_3016GS.do", TrsFrmQryString(formObj), "sheet1_hid_del_chk",true);
				break;
        }
    }
	 	...
	 	switch(elementObj.name){ 	    	
	 		...
	 		case "curr_cd":
	 			if(!isNull(elementObj.value)){
					...
					var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_3016GS.do", FormQueryString(formObj), true);
					var err_flg = ComGetEtcData(sXml, "err_flg");

...
*@FileName : ESD_TRS_3007.js
*@FileTitle : Inland Cost Inquiry
*Open Issues :
*Change history :
*@LastModifyDate : 
*@LastModifier : 
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_TRS_3007 : esd_trs_3007 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esd_trs_3007() {
        ...
        switch(sheetID) {
            ...
            case "sheet2":
                with (sheetObj) {
					...
					var HeadTitle1 = "Flag|Country|BND|Tariff No|Status|Effective\nFrom|Trans\nMode|EQ\nSize|Reefer|Reefer|Reefer|DG|DG|DG|Overweight (Ton)|Overweight (Ton)|Overweight (Ton)|Overweight (Ton)|Overweight (Ton)|Creation|Creation|Creation|Update|Update|Update";
					var HeadTitle2 = "Flag|Country|BND|Tariff No|Status|Effective\nFrom|Trans\nMode|EQ\nSize|SVC|Fixed|%|SVC|Fixed|%|SVC|From|To|Fixed|%|Time|User|Office|Time|User|Office";
        	...
        	case "sheet3":	//Cost Tariff No.의 Validation Check를 위한 임시 Sheet
        		with (sheetObj) {
					// 높이 설정
                    style.height = 320;
                    //전체 너비 설정
        ...
        switch(sAction) {
			case IBSEARCH: //Retrieve
				if(validateForm(sheetObj,formObj,sAction)){
					...
					formObj.excel_io_bnd_cd.value 		= comboObjects[2].Code;
					...
					formObj.excel_chk_incl.value 		= formObj.chk_incl.value;
					...
					var sXml = sheetObj.GetSearchXml("ESD_TRS_3007GS.do", sParam);
			...
			case IBDOWNEXCEL:
				...
				formObj.action = "ESD_TRS_3007EXCEL.do?"+ TrsFrmQryString(formObj);
			    formObj.submit();
			    ComOpenWait(false);
		        break;
    ...
    function validateForm(sheetObj,formObj,sAction){
		switch (sAction) {
			case IBSEARCH:
				if(comboObjects[0].Code == "" && ComIsEmpty(formObj.eff_to_dt.value) && ComIsEmpty(formObj.loc_nod_cd.value) && ComIsEmpty(formObj.hub_nod_cd.value) && ComIsEmpty(formObj.port_nod_cd.value) && ComIsEmpty(formObj.cost_trf_no.value)){
    	...
    	switch(obj.name) {
    		...
    		case "cost_trf_no":
    			if( formObj.cost_trf_no.value != "" ){
    				doActionIBSheet(sheetObjects[2], formObj, SEARCH02, 0);
    			}
    			break;
    	}
    }
	...
	function fun_incl() {
		var vchk_incl = document.form.chk_incl;
	
		if(vchk_incl.checked == true){
			document.form.cost_sel_rout_flg.value="Y";
		}else{
			document.form.cost_sel_rout_flg.value="N";
		}
	}

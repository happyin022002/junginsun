...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_TRS_3002 : esd_trs_3002 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function esd_trs_3002() {
	...
	function loadPage() {
		...
		var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_3001GS.do", sParam);
         ...
         switch(sheetID) {
        	case "sheet1":
        		with (sheetObj) {
					...
					HeadTitle1 += "|cost_trf_no|cost_trf_rout_seq|cost_rout_grp_no|cost_rout_grp_no_rnk|rcv_de_term_cd|Flag";

					var HeadTitle2 = "Port-LOC|R/D\nTerm||Sel.|Del.|Port|Hub|LOC|Empty\nPU/RTN|Trans\nMode|20’|40’|SCC|20’|40’";
					HeadTitle2    += "|System\nSource|System\nAmount|MTY YD\n(AGMT)|MTY\nDiff|Weight\n(AGMT)|Rate\nType";
					HeadTitle2    += "|System\nSource|System\nAmount|MTY YD\n(AGMT)|MTY\nDiff|Weight\n(AGMT)|Rate\nType";
					HeadTitle2    += "|System\nAmount|System\nAmount|System\nSource|System\nAmount|System\nSource|System\nAmount|S/P\nCode|S/P\nName|Com-\nbined|S/P\nCode|S/P\nName|OLD\nAGMT";
					HeadTitle2    += "|cost_trf_no|cost_trf_rout_seq|cost_rout_grp_no|cost_rout_grp_no_rnk|rcv_de_term_cd|Flag";
         ...
         switch(sAction) {
			case IBSEARCH:      //Sort
				if (sheetObj.id == "sheet1"){
		        	...
		        	var sParam = FormQueryString(formObj);
					var sXml = sheetObj.GetSearchXml("ESD_TRS_3002GS.do", sParam);
					if(sXml.length>0){
			...
			case MULTI01: //Apply
				if(validateForm(sheetObj,formObj,sAction)){
					...
					if( sParam == ""){ return;}
					var sXml = sheetObj.GetSaveXml("ESD_TRS_3002GS.do", FormQueryString(formObj) + "&" + sParam, true);	        			
			...
			case MULTI02: //Delete
				if(validateForm(sheetObj,formObj,sAction)){
		        	...
		        	var sXml = sheetObj.GetSaveXml("ESD_TRS_3002GS.do", sParam);
					sheetObj.LoadSaveXml(sXml);
				}
				break;
         }

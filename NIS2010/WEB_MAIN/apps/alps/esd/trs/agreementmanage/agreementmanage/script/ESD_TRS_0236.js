...
//	 var trsp_agmt_rt_tp_cd = formObj.fm_trsp_agmt_rt_tp_cd.value;
	 ...
	 if(resultcheck == 1){
		 ...
		 var param = "?gubun2=0231&route_gubun=route&gubun=save&"+TrsFrmQryString(formObj); 
 ...
 function openRateHistory(sheetObject, row)
 {
	...
	var myOption = "width=980,height=530,menubar=0,status=0,scrollbars=0,resizable=0";
	var param = "?route_gubun=route&gubun=save&"+TrsFrmQryString(formObj);
...
function sheet_main_popup(sheetObj, row, col) {
  	...
  	formObject.fm_agmt_trsp_tp_cd.value = sheetObj.CellValue(row, "agmt_trsp_tp_cd");

...
function loadPage() {
	...
	for (var i = 0; i < uploadObjects.length; i++) {
		//1. 기본 환경 설정
		ComConfigUpload(uploadObjects[i], "/hanjin/" + "ESD_TRS_0977GS.do");
	}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	case IBSEARCH:
		...
		sheetObj.DoSearch4Post("ESD_TRS_0977GS.do", FormQueryString(formObj));
		break;
	case IBSAVE: // 저장
		...
		if (upObj.LocalFiles == "") {
			...
			var sXml = sheetObj.GetSaveXml("ESD_TRS_0977GS.do", sParam);
		} else {
...
function selectFile(sheetObj, Row, Col, _insert) {
	...
	if (Col == '' || sheetObj.ColSaveName(Col) == 'file_nm') {
		...
		if (typeof file_nm == "undefined" || file_nm == "File Name"
				|| (file_nm != "" && _insert)) {
...
function fnRtnFileY() {
	...
	if (sheetObj.RowCount > 0) {
		...
		parentObj.sheet1.CellValue(formObj.attchRow.value, 'expn_aud_rslt_rmk') = atch_file_lnk_id;
		parentObj.sheet1.CellValue(formObj.attchRow.value, 'attach_flag') = "Y";
	} else {
		ComSetObjValue(parentObj.atch_file_lnk_flg, "");;
		parentObj.sheet1.CellValue(formObj.attchRow.value, 'expn_aud_rslt_rmk') = "";
		parentObj.sheet1.CellValue(formObj.attchRow.value, 'attach_flag') = "N";
	}
}

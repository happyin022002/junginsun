...
<%@ page import="com.hanjin.syscommon.common.table.TrsTrspSvcOrdVO"%>
<%
try{
...
<%
		...
		for(int i=0;popTrsTrspSvcOrdVOs!=null && i<popTrsTrspSvcOrdVOs.length; i++ ) {
			...
			org_tro_seq = org_TrsTrspSvcOrdVO.getTroSeq();
			duplicateFlag = false;
			for(int k=0; mainTrsTrspSvcOrdVOs != null && k<mainTrsTrspSvcOrdVOs.length; k++){
				...
				if( (org_bkg_no.equals(tgt_TrsTrspSvcOrdVO.getBkgNo()) ||
					 ...
					 org_tro_seq.equals(tgt_TrsTrspSvcOrdVO.getTroSeq()) ){
					duplicateFlag = true;
			...
			if(duplicateFlag) org_TrsTrspSvcOrdVO.setSpclInstrRmk(tgt_so_seq);

			popTrsTrspSvcOrdVOs[i] = org_TrsTrspSvcOrdVO;
		}

		if (popTrsTrspSvcOrdVOs != null ) totalCnt = popTrsTrspSvcOrdVOs.length;
%>
<SHEET>
	<DATA>
<%

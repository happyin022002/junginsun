...
===============================================================================
 History
 2012.05.03 변종건 [CHM-201217633] Inland Cost Batch Creation 생성
=========================================================*/
/****************************************************************************************
...
/**
 * @extends 
 * @class ESD_TRS_3004 : ESD_TRS_3004 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_TRS_3004() {
 	...
 	switch(elementObj.name){ 	    	
 		case "cnt_cd":
 			if(!isNull(elementObj.value)){
				...
				var sXml = sheetObjects[1].GetSearchXml("ESD_TRS_3004GS.do", FormQueryString(formObj), true);
	...
	switch(sAction) {
		case IBSEARCH:		//Retrieve
			if(!validateForm(sheetObj,formObj,sAction)) return;
	   		...
	   		sheetObj.DoSearch4Post("ESD_TRS_3004GS.do", TrsFrmQryString(formObj));
	   		break;
	   		
		case SEARCH01:		//RHQ Office Code
			if(!validateForm(sheetObj,formObj,sAction)) return;
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("ESD_TRS_3004GS.do", TrsFrmQryString(formObj));
		...
		case MULTI:		//Batch Creation
			...
			sheetObj.DoSave("ESD_TRS_3004GS.do", TrsFrmQryString(formObj), "chk");
			break;
...
function validateForm(sheetObj,formObj,sAction){
	switch(sAction){
		...
		case MULTI:
        	...
        	for( var idx = 0+parseInt(sheetObj.HeaderRows); idx <= sheetObj.LastRow; idx++ ){
        		if( sheetObj.CellValue(idx,"chk") == "1" && (sheetObj.CellValue(idx,"curr_cd") == "" || sheetObj.CellValue(idx,"xch_rt") == "") ){
        			ComShowCodeMessage("TRS90427");
        			return false;
        		}
        	}

        	//Check된 항목 중 진행 상태가  Waiting 또는 Progressing 경우 진행 불가, 진행 상태가 Created 또는 Updated 경우 확인 메시지창 띄움
        	var msgFlg = "N";
        	for( var idx = 0+parseInt(sheetObj.HeaderRows); idx <= sheetObj.LastRow; idx++ ){
				if( sheetObj.CellValue(idx,"chk") == "1" && (sheetObj.CellValue(idx,"sts_cd") == "W" || sheetObj.CellValue(idx,"sts_cd") == "P") ){
		...
		case MULTI01:
			...
			if( !ComShowCodeConfirm("TRS90430") ){
    			return false;
    		}
		...
		case MULTI02:
        	...
        	if( !ComShowCodeConfirm("TRS90441") ){
    			return false;
    		}
		...
		case MULTI03:
        	...
        	if( !ComShowCodeConfirm("TRS90440") ){
    			return false;
    		}
...
function sheet1_OnClick(sheetObj,Row,Col,Value){
	...
	if( colNm == "sts_nm" && sheetObj.CellValue(Row,"sts_cd") == "E" ){
		var sUrl = "/hanjin/ESD_TRS_3020.do";
...
function goCostTrfMgmt() {
	...
	var sUrl = "/hanjin/ESD_TRS_3001.do";
	...
	window.open(sUrl + param, "ESD_TRS_3001", myOption);
}

...
/**
 * @extends 
 * @class ESD_TRS_3011 : ESD_TRS_3011 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
function ESD_TRS_3011() {
	...
	switch(sheetNo) {
		case 1:      //sheet1 init
			with (sheetObj) {
				...
				var HeadTitle =  "Sts|RHQ|Tariff No|Status|Status|CURR|Progress|Creation\nTime|Creation\nUser|Update\nTime|Update\nUser";
				...
				InitDataProperty(0, cnt++, 	dtHiddenStatus,		40,	daCenter,	true,		"ibflag",   	false, 		"", 		dfNone, 	0, 			false, 		false,  		 0, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   			70,	daCenter, 	true,    	"rhq_cd",  		false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   100,	daCenter, 	true,    	"cost_trf_no",  false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtHidden,  		    60,	daCenter, 	true,    	"sts_cd",       false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   100,	daCenter, 	true,    	"sts_nm",       false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtHidden,   		60,	daCenter, 	true,    	"curr_cd",      false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   			80,	daCenter, 	true,    	"prog_ratio",   false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   150,	daCenter, 	true,    	"cre_dt",       false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   150,	daLeft, 	true,    	"cre_usr_id",   false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   150,	daCenter, 	true,    	"upd_dt",       false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
				InitDataProperty(0, cnt++, 	dtData,   		   150,	daLeft, 	true,    	"upd_usr_id",   false, 		"", 		dfNone, 	0, 			false, 		false, 			30, 	false,	 	true, 		"", 		false);
	...
	switch(sAction) {
		case IBSEARCH:		//Retrieve
			if(!validateForm(sheetObj,formObj,sAction)) return;
	   		...
	   		sheetObj.DoSearch4Post("ESD_TRS_3011GS.do", TrsFrmQryString(formObj));
	   		break;
			
		case SEARCH01:		//RHQ Office Code
			if(!validateForm(sheetObj,formObj,sAction)) return;
			formObj.f_cmd.value = SEARCH01;
			var sXml = sheetObj.GetSearchXml("ESD_TRS_3011GS.do", TrsFrmQryString(formObj));
		...
		case MULTI:		//Batch Creation
			...
			sheetObj.DoSave("ESD_TRS_3011GS.do", FormQueryString(formObj), -1, false);
			break;
	}
}
...
function validateForm(sheetObj,formObj,sAction){
	switch(sAction){
		case MULTI:
			// Waiting 또는 Progressing 경우 진행 불가, 진행 상태가 Created 또는 Updated 경우 확인 메시지창 띄움
			var msgFlg = "N";
        	if( sheetObj.RowCount > 0){
				if( sheetObj.CellValue(sheetObj.SelectRow,"sts_cd") == "W" || sheetObj.CellValue(sheetObj.SelectRow,"sts_cd") == "P" ){
					ComShowCodeMessage("TRS90416");
					return false;
				}
				if( sheetObj.CellValue(sheetObj.SelectRow,"sts_cd") == "B" || sheetObj.CellValue(sheetObj.SelectRow,"sts_cd") == "U" || sheetObj.CellValue(sheetObj.SelectRow,"sts_cd") == "E" ){
					msgFlg = "Y";
				}
			}
			if( msgFlg == "Y" ){
				if( !ComShowCodeConfirm("TRS90417") ){
	    			return false;
	    		}
			}
			break;
...
function sheet1_OnClick(sheetObj,Row,Col,Value){
	...
	if( colNm == "sts_nm" && sheetObj.CellValue(Row,"sts_cd") == "E" ){
		var sUrl = "/hanjin/ESD_TRS_3021.do";
		...
		window.open(sUrl + param, "ESD_TRS_3020", myOption);
	}
}
...
function goCostTrfMgmt(){
	...
	if( sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"cost_trf_no") == "" ){
		ComShowCodeMessage("TRS90385","Tariff No.");
		return;
	}
	...
	&& sheetObjects[0].CellValue(sheetObjects[0].SelectRow,"sts_cd") != "C" ){
		ComShowCodeMessage("TRS90423");
		return;
	}
	
	var sUrl = "/hanjin/ESD_TRS_3009.do";
	...
	window.open(sUrl + param, "ESD_TRS_3009", myOption);
}

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_SPE_1010 : ESD_SPE_1010 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_SPE_1010() {
	 	...
	 	switch(sheetNo) {
	    	case 1:      //sheet1 init
		    	with (sheetObj) {
					...
					var HeadTitle2 = "ibflag||Year|EG ID|EG Name|KPI|Performance(%)|Target(%)|Weight(%)|Score|sp_kpi_tp_cd|Type Code|Target(%)|Weight(%)|USEFLAG";
...
//					alert("Wgt 합은 100이여야 합니다.");
	...
	function setCellProperty(sheetObj){
		...
		for( var idx = parseInt(sheetObj.HeaderRows); idx <= sheetObj.LastRow-1; idx++ ){			
			...
			var useflag = sheetObj.CellValue(idx,"useflag");

...
*@FileName : ESD_SPE_1009.js
*@FileTitle : Evaluation Group Mapping & Basic Evaluation Inquiry
*Open Issues :
*Change history :
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_SPE_1009 : ESD_SPE_1009 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_SPE_1009() {
	 	...
	 	switch(sheetNo) {
	    	case 1:      //sheet1 init
		    	with (sheetObj) {
					...
					var HeadTitle1 = "ibflag|Chk|EG ID|EG Name|S/P ID|S/P Name|Year|Basic Evaluation|Basic Evaluator|Evaluated Date";
					...
					InitDataProperty(0, cnt++ , dtPopupEdit,    100,  	daLeft,     true,   "sp_seq",           false,    "",      dfNone,          0,          true,         true,   10);
					InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,     true,   "sp_name",         	false,    "",      dfNone,          0,          false,        false,  10);
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "ev_yr",            false,    "",      dfNone,          0,          true,         true,   4);
					InitDataProperty(0, cnt++ , dtComboEdit,    170,  	daLeft,     true,   "bzc_ev_grd_cd",    false,    "",      dfNone,          0,          true,         true,   10);
					InitDataProperty(0, cnt++ , dtData,         100,  	daCenter,   true,   "evr_usr_id",       false,    "",      dfNone,          0,          false,        false,  10);
					InitDataProperty(0, cnt++ , dtData,         100,  	daCenter,   true,   "ev_dt",            false,    "",      dfDateYmd,       0,          false,        false,  10);
					InitDataProperty(0, cnt++ , dtHidden,       100,  	daCenter,   true,   "sp_bzc_eg_seq",    false,    "",      dfNone,          0,          false,        false,  10);
...
//					ComOpenWait(true);
					var sXml = sheetObj.GetSearchXml("ESD_SPE_1008GS.do", sParam);
					
					if(ComGetTotalRows(sXml)==1 && SpeXmlString(sXml,"sp_seq")==""){
...
//					ComOpenWait(false);
			...
			case IBSAVE:
				...
				var sParam = sheetObj.GetSaveString(false, true) + "&" + FormQueryString(frm);
			        ...
			        var sXml = sheetObjects[0].GetSaveXml("ESD_SPE_1008GS.do", sParam);
...
//				sheetObj.SpeedDown2Excel(1);
			...
			case IBCOPYROW:	// Copy Row
				...
				sheetObj.CellValue2(Row, 'ev_yr')     = frm.s_ev_yr.value ;
				sheetObj.CellValue2(Row, 'bzc_ev_grd_cd')   = "" ;
//				sheetObj.CellValue2(Row, 'evr_usr_id')  = "" ;
				sheetObj.CellValue2(Row, 'ev_date')     = "" ;
				break;					
		}
	}
...
//	    	frm.s_eg_ofc_cd.Index=0;
     ...
     */ 	  
	  function sheet1_OnLoadFinishLoad(sheetObj){
			...
			// 공통 테이블에서 조회할 키
//			sheetObj.WaitImageVisible = false;
			var sXml = sheetObj.GetSearchXml("ESD_SPE_1008GS.do", FormQueryString(frm));
			
		    var rhqOfcCd = ComXml2ComboString(sXml,  "bzc_ev_grd_cd", "bzc_ev_grd_nm");
		    sheetObj.InitDataCombo(0, "bzc_ev_grd_cd", " |" + rhqOfcCd[1], " |" + rhqOfcCd[0]);    // IBSheet내 Combo 초기화
	 		...
	 		with(sheetObj) {
				...
				switch(sName) {
					case "sp_seq":		//Yard Code No Pop-up
						...
						saveColValueDup(sheetObj, row, col ,value);
						break;
					case "ev_yr":		//Yard Code No Pop-up
						saveColValueDup(sheetObj, row, col ,value);
						break;
				}
	 		}
	  ...
	  // 입력받은 값이 저장 가능한값인지 확인한다.
	  function saveColValueDup(sheetObj, row, col ,value){
	    	if(sheetObj.CellValue(row, "eg_id") !="" && sheetObj.CellValue(row, "sp_seq") !="" && sheetObj.CellValue(row, "ev_yr") !=""  ){
	    		
	    		
	    		for(var i=0;i<sheetObj.RowCount;i++){
	    			if(i != row && (sheetObj.CellValue(row, "eg_id") == sheetObj.CellValue(i, "eg_id") && 
	    					        sheetObj.CellValue(row, "sp_seq") == sheetObj.CellValue(i, "sp_seq") &&
	    					        sheetObj.CellValue(row, "ev_yr") == sheetObj.CellValue(i, "ev_yr")    )){
	    				ComShowCodeMessage('COM12115','Basic Evaluation');
						if(col == sheetObj.SaveNameCol("sp_seq")){
						   sheetObj.CellValue2(row,"sp_seq") = "";
						   sheetObj.CellValue2(row,"sp_name") = "";
						}else if(col == sheetObj.SaveNameCol("ev_yr")){
						   sheetObj.CellValue2(row,"ev_yr") = "";
						}
						    return;
	    			}
	    		}
	    		frm.f_cmd.value = COMMAND02;
	    		frm.g_eg_id.value  = sheetObj.CellValue(row, "eg_id"  );
	    		frm.g_sp_seq.value = sheetObj.CellValue(row, "sp_seq" );
	    		frm.g_ev_yr.value  = sheetObj.CellValue(row, "ev_yr"  );
	    		var sParam = FormQueryString(frm);
	    		var sXml = sheetObj.GetSearchXml("ESD_SPE_1008GS.do", sParam);
	 ...
	 */
	 function sheet1_OnClick(sheetObj, row, col, value) {
			...
			switch(sName) {
				case "bzc_ev_grd_cd":		//Yard Code No Pop-up
					if(frm.log_usr_id.value!=sheetObj.CellValue(row,"evr_usr_id")){
						
						ComShowCodeMessage('COM131604', 'Basic Evaluation'); //Basic Evaluation 를 update 할 권한이 없습니다.
						sheetObj.CellEditable(row,"bzc_ev_grd_cd")=false;
					}
					break;
			}
  ...
  function rep_OnPopupClick() {
		...
		var param ="?EV_SVC_CATE_CD="+evSvcCateCd+"&EV_SVC_CATE_NM="+evSvcCateNm;
		ComOpenPopup('/hanjin/ESD_SPE_1022.do' + param, 699, 422, 'getESD_POPUP1022', '1,0,1,1,1',false);
   }	    
  
	/**
	* S/P Code 팝업호출 : 팝업에서 단일 선택을 한경우..
	*/
	function getESD_POPUP1022(rowArray) {
		var sheetObj = sheetObjects[0];
		...
		var dupVal = sheetObj.ColValueDup("eg_id|sp_seq", false)
		if(dupVal> 0){
			if(sheetObj.CellValue(dupVal,"sp_seq")!=""){
				ComShowCodeMessage('COM12115','S/P ID');
				sheetObj.CellValue2(row,"sp_seq")="";
				sheetObj.CellValue2(row,"sp_name")="";
//				return;
			}
	...
	/**
	* 등록된 S/P 정보를 조회 하여 코드와 명칭을 입력한다.
	*/
...
//			frm.ev_svc_cate_cd.value = frm.s_ev_svc_cate_cd.Code;
			var sXml=sheetObjects[0].GetSearchXml("ESD_SPE_1022GS.do", FormQueryString(frm));
...
//	  			ComShowCodeMessage('SPE10002', 'EG ID'); //EG ID 가 존재하지 않습니다.
			 ...
			 if(sheetObj.CellValue(sheetObj.RowCount, 'ev_yr')==""){
				sheetObj.CellValue2(sheetObj.RowCount, 'ev_yr')     = frm.s_ev_yr.value ;
			 }
			 sheetObj.SelectCell(sheetObj.RowCount, 0);
				
	  		 for(var i=1; i<= Rowcount; i++){
	   			if(cellValue(i,'sp_seq')==""){
	   				// bzc_evr_usr_id null 이면 저장하지 않은 데이터여서 상태값을 입력으로 변경한다.

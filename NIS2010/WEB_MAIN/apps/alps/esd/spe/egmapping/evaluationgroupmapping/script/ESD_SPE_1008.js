...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_SPE_1008 : ESD_SPE_1008 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_SPE_1008() {
	 	...
	 	switch(sheetNo) {
	    	case 1:      //sheet1 init
		    	with (sheetObj) {
					...
					InitDataProperty(0, cnt++ , dtPopupEdit,    100,  	daLeft,     true,   "sp_seq",           true,     "",      dfNone,          0,          true,         true,   10);
					InitDataProperty(0, cnt++ , dtData,         100,  	daLeft,     true,   "sp_name",         	false,    "",      dfNone,          0,          false,        false,  10);
					InitDataProperty(0, cnt++ , dtData,         70,  	daCenter,   true,   "ev_yr",            true,     "",      dfNone,          0,          true,         true,   4);
					InitDataProperty(0, cnt++ , dtComboEdit,    120,  	daLeft,     true,   "bzc_ev_grd_cd",    false,    "",      dfNone,          0,          true,         true,   10);
...
//				sheetObj.CellValue2(Row, 'evr_usr_id')  = "" ;
				...
				sheetObj.CellEditable(Row, 'ev_yr') = true;
				break;					
		}
	}
	  ...
	  function sheet1_OnSearchEnd(sheetObj, ErrMsg){
	  	  with(sheetObj){
	  		 ...
	  		 for(var i=1; i<= Rowcount; i++){
	   			if(cellValue(i,'sp_seq')=="" || cellValue(i,'ev_yr')==""){
	   				// bzc_evr_usr_id null 이면 저장하지 않은 데이터여서 상태값을 입력으로 변경한다.
	   				RowStatus(i)="I";
	   			}else{
	   				CellEditable(i, "ev_yr") = false; 
	   			}
	  		 }
	   	  }
	  } 		

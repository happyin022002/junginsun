   ...
   /**
	...
	function processButtonClick(){
		...
		if(window.event.srcElement.getAttribute("name") == "c_date_kind_flag"){
			if(window.event.srcElement.getAttribute("value") == "R" || window.event.srcElement.getAttribute("value") == "H"){
...
//					InitDataProperty(  0 ,cnt++ ,dtHidden    ,50    ,daCenter    ,true    ,""                      );
					...
					InitDataProperty(  0 ,cnt++ ,dtCombo      ,150    ,daCenter    ,true    ,"wrtf_rsn_cd"         ,false   ,""        ,dfNone      ,0         ,true     ,true     );
	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			...
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch("ESD_TPB_0138GS.do", final_retrieve_querystrings );
				break;
			
			case REMOVELIST:
				formObj.f_cmd.value = REMOVE;
				sheetObj.DoSave("ESD_TPB_0138GS.do", tpbFrmQryStr(formObj));
				formObj.f_cmd.value = SEARCHLIST;
				final_retrieve_querystrings = tpbFrmQryStr(formObj); 
				sheetObj.DoSearch("ESD_TPB_0138GS.do", final_retrieve_querystrings );
				break;
...
//		document.form.stl_rqst_rmk.value = '';
		...
		if(errMsg==null || errMsg == ''){
			...
			sheetObj.DoSearch("ESD_TPB_0138GS.do", final_retrieve_querystrings ); /// sheet data reload
		}
	}

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_TPB_0805 : ESD_TPB_0805 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_TPB_0805() {
    ...
    var str_placeholder = 'Please explain in details (with file attachment if necessary).';
    var is_placeholder;
    ...
    /* 업무별전역변수는 아래 부분에 추가 선언하여 사용한다. */
  	...
  	function loadPage() {
  		...
  		var n3pty_stl_tp_cd = opener.document.form.n3pty_stl_tp_cd.value;
...
//          }
  		...
  		if (form.s_ra_rmk1.value == "") {
  			form.s_ra_rmk1.value = str_placeholder;
			is_placeholder = true;
		} else {
  		...
  		try {
  			...
  			switch(srcName) {
  				...
  				case "btn_save":
  				    ...
  				    rtnVal[3] = document.form.s_wrtf_rsn_cd.value;
...
//  		ifr.document.location.href = "TPBFileDownload.do?fileNo="+document.form.s_file_no.value+"&f_cmd="+SEARCH+"&downloadLink=Y&col=1";
	...
	function f_set_placeholder(e) {
		if(is_placeholder) {
			is_placeholder = false;
			form.s_ra_rmk1.value='';
		}
    	
	}
	
	function f_chk_length(reason) {   
		if(form.s_ra_rmk1.value.length < 1) {
    		form.s_ra_rmk1.value = str_placeholder;
    		is_placeholder = true;
    	}
	}
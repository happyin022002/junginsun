...
*@FileName : ESD_TPB_0128.js
*@FileTitle : JO TPB Inquiry
*Open Issues :
*Change history :
...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends 
     * @class ESD_TPB_0128 : ESD_TPB_0128 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    function ESD_TPB_0128() {
  		...
  		switch(sheetNo) {
  			case 1:	  //IBSheet1 init
  				with (sheetObj) {
                    ...
                    var HeadTitle2 = " |TPB No.|Invoice No.|TPB Seq No.|Seq.|Main|Sub-Cd|Sub|EQ Kind|EQ No.|BKG No.|B/L No.|VVD|S/P Inv. No.|Cur.|Amount|USD|Cur.|Amount|Cur.|Amount|Cur.|Amount|Office|User|Date|Office|User|Date|Overdue days|Remark";
  					...
  					InitDataProperty(0, cnt++, dtHidden,    	100,    daCenter,  	 true,    "n3pty_no",         false,          "",       dfNone,    0,     false,       false);
  					...
  					InitDataProperty(0, cnt++, dtData,      	110,     daRight,  	 true,    "if_amt",           false,          "",      dfFloat,    2,     false,       false);
  					InitDataProperty(0, cnt++, dtData,      	110,     daRight,  	 true,    "if_amt_usd",       false,          "",       dfNone,    2,     false,       false);
  					InitDataProperty(0, cnt++, dtData,      	 50,    daCenter, 	 true,    "cfm_curr_cd",      false,          "",       dfNone,    0,     false,       false);
  					InitDataProperty(0, cnt++, dtData,      	110,     daRight,  	 true,    "cfm_amt",          false,          "",      dfFloat,    2,     false,       false);
  					...
  					InitDataProperty(0, cnt++, dtData,      	 80,    daCenter, 	 true,    "overdue_days",     false,          "",       dfNone,    0,     false,       false);

  					InitDataProperty(0, cnt++, dtData,      	130,      daLeft,	 true,    "cfm_rmk",          false,          "",       dfNone,    0,     false,       false);
  			...
  			case 2: //sheet2 init
  				with (sheetObj) {
  					var cnt = 0;
  					// 높이 설정
  					style.height = 0; //getSheetHeight(1);
...
//  						ComShowCodeMessage('TPB90010','Cancel Close ','','');
  				...
  				case "bttn_print":
  					...
  					break;
  				case "btn_retrieve":
  					doActionIBSheet(document.form.sheet2,formObject,SEARCH);
  	...
  	function doActionIBSheet(sheetObj,formObj,sAction) {
  		...
  		switch(sAction) {
  			case SEARCH:	  //조회
  				...
  				sheetObj.DoSearch4Post("ESD_TPB_0128GS.do", tpbFrmQryStr(formObj));

  				formObj.s_n3pty_no.value 			= sheetObj.CellValue(1,"n3pty_no");
                formObj.s_n3pty_inv_no.value 		= sheetObj.CellValue(1,"n3pty_inv_no");
                formObj.s_ofc_cd.value 				= sheetObj.CellValue(1,"ofc_cd");
                formObj.s_ots_sts_nm.value 			= sheetObj.CellValue(1,"ots_sts_nm");
                formObj.s_overdue.value 			= sheetObj.CellValue(1,"overdue");
                formObj.s_vndr_cust_div_cd.value 	= sheetObj.CellValue(1,"vndr_cust_div_cd");
                formObj.s_trd_party_val.value 		= sheetObj.CellValue(1,"trd_party_val");
                formObj.s_trd_party_nm.value	 	= sheetObj.CellValue(1,"trd_party_nm");
                formObj.s_csr_no.value 				= sheetObj.CellValue(1,"csr_no");
                formObj.s_roc_in.value 				= sheetObj.CellValue(1,"roc_in");
                formObj.s_roc_out.value 			= sheetObj.CellValue(1,"roc_out");
  			...
  			case SEARCH01:	  //조회
                ...
                if ( formObj.s_n3pty_no.value.length==14 || formObj.s_n3pty_inv_no.value.length == 11){
  			...
  			case IBSAVE:		//저장
  				...
  				sheetObj.DoSave("ESD_TPB_0128GS.do", tpbFrmQryStr(formObj));
  				sheetObj.RemoveAll();
  				break;

  			case COMMAND01:		// Cancel By Super User
  				// ComShowMessage("Cancel By Super User / "+formObj.s_detail_n3pty_no.value); 
  			...
  			case COMMAND02:		// Close Process By General User
  				// ComShowMessage("Close Process / "+formObj.s_detail_n3pty_no.value); 
  				...
  				//===== Email 수신자 정보 가져오기 // Removed By Kim Jin-seung In 2007-09-13
  				//if(formObj.toEmail.value == ""){
  				//	ComShowCodeMessage("COM12113","Staff","","");
  				//	tpb_getEmailInfo();
  				//	return;
  				//}
                  //===== show messge according to the Source (TES/TRS/MNR) // Added By Kim Jin-seung In 2007-09-13
...
//                  //===== open Recovery Activity Popup // Added By Kim Jin-seung In 2007-09-13
//  				var read = formObj.s_readonly.value; //yoon 메뉴에 따라서...
//  				openCollectionActPopup(formObj.s_detail_n3pty_no.value, '', '',read);
  //
                  //===== process close SAVE =====
...
//  				ComShowCodeMessage('TPB90012');
//  				document.form.action = "ESD_TPB_116.do"; 
...
//  		}
  	...
  	function tpb_getEmailInfo(){
  		//comPopupWith2ndSheet('/hanjin/COM_ENS_091.do', 770, 580, 'getStaff_formail', '1,0,1,1,1,1,1', false);
  		var classId = "COM_ENS_092";
  		var param = '?classId='+classId;
  		comPopup('/hanjin/COM_ENS_092.do' + param, 850, 550, 'getStaff_formail', 'none', true);
  	}
      ...
      function openProcessClosePopup(formObject, processCloseCase){
      	//alert("ESD_TPB_0128.js - 621 수정필요");
      	return;
      	var theURL = "ESD_TPB_917.do?processCloseCase="+processCloseCase;

...
*@FileName : CoTpb.js
*@FileTitle : TPB 업무공통 관련 스크립트 및 TPB 공통 메시지
*Open Issues :
*Change history :
*@LastModifyDate : 2009-08-11
*@LastModifier : O Wan-Ki
*@LastVersion : 1.0
* 2009-08-11 O Wan-Ki 1.0 최초 생성
...
* 2012.05.11 조원주 [CHM-201217307-01] [TPB_Process Management] W/O Request 사유 세분화
========================================================================================--*/
...
==============================================================================--*/
	...
	msgs['TPB90030'] = '(Null)' ; // fileupload에서 사용 
	...
	msgs['TPB90079'] = 'Invalid Group Selection (max:300)'; // TPB Candidate Confirmation. At ESD_TPB_0105
	...
	msgs['TPB90104'] = 'Please Note on Non-TPB ';
	msgs['TPB90105'] = 'Write-off Reason is not inputted';
	msgs['TPB90106'] = 'Preventive measure is not inputted';
	msgs['TPB90107'] = 'ROCed TPB can\'t be ROCed back.';
    msgs['TPB90108'] = "The period of Date can't be over {?msg1}.";
    msgs['TPB90109'] = "{?msg1} must be earlier than {?msg2}.";
    msgs['TPB90110'] = "VVD does not match with BKG Route Information. Do you want to proceed?";
    msgs['TPB90111'] = "Invalid VVD !  Can not find \"Finance Direction Code\"\nPlease input another VVD or location.";		
    msgs['TPB90112'] = "Duplicated container number. please check !\n[EQ No : {?msg1}]";
    msgs['TPB90113'] = "{?msg1} must be earlier than or equal to the {?msg2}.";
    msgs['TPB90114'] = 'Please try to Register again with TPB office !' ; // TPB Manual Registration. At ESD_TPB_0103
    msgs['TPB90115'] = "You can\'t send EDI.(Load ID is missing.) Do you want to proceed?"
    msgs['TPB90116'] = 'Please try again after update.({?msg1})';
    
	msgs['TPB00079'] = "EDI was transmitted successfully.";
	msgs['TPB00080'] = "Transmit Failed.";
	 ...
	 */
	function getTPBGenCombo(id,method,mode,obj,all,paramName, otherObjs, callback) {
		...
		eval("document."+ifr).location.href = "TPBCommonCode.do?id="+id+"&method="+method+"&mode="+mode+"&obj="+obj+"&all="+all+"&"+param+"&otherObjs="+otherObjs+"&callback="+callback;
	}
...
//		}
	...
	function getVendor(rArray) {
		...
		//alert("abc:"+document.all.s_trd_party_val.value);
		if(document.all.s_trd_party_nm != null) {
			document.all.s_trd_party_nm.value = cArray[4];
		}
       ...
       // if (str1 != "ESD_TPB_0139" && str1 != "ESD_TPB_0140"){
        if (str1 != "ESD_TPB_0140"){	
        	s_trd_party_val_OnBlur('V', true);
        } else {
        	document.all.s_trd_party_val.value = cArray[2];
        	document.getElementById("s_trd_party_val").focus();
        	document.getElementById("s_trd_party_val").blur();
        }
	}
	...
	function getVendor_SP(rArray) {
		...
		if (document.all.s_src_vndr_no != undefined) document.all.s_src_vndr_no.value = cArray[2];
		if (document.all.s_src_vndr_cnt_cd != undefined) document.all.s_src_vndr_cnt_cd.value = cArray[7];
		
	}

	function getVendor_SP2(rArray) {
		var cArray = rArray[0];

		if (document.all.s_src_vndr_no2 != undefined) document.all.s_src_vndr_no2.value = cArray[8];		// cntr + seq
		if (document.all.s_src_vndr_nm != undefined) document.all.s_src_vndr_nm.value = cArray[4];			// description
...
//		document.all.s_trd_party_val.value = cArray[7]+cArray[2];
	 ...
	 * Sheet에서 popup 호출하였을 경우
	 */
	function getVendor_SP2_sheet(rArray, row, col) {
		
		var sheetObj = sheetObjects[0];		
		var cArray = rArray[0];
//alert("multi:"+cArray[0] +" :"+cArray[1]+" :"+cArray[2]+" :"+cArray[3]);

//		sheetObj.CellValue(row, col) =  cArray[8];				// cntr + seq , CellValue 가 Change 이벤트 발생함. 3rdParyVal의 접합성 유무는 Sheet 의 Change 이벤트에서 Check.
		sheetObj.CellValue(row, col) =  cArray[2];
	}
...
//		}
	...
	function get3rdPartyTarget_sheet(val, Row, Col, sheetObj, id1, id2) {
		...
		} else if(strTrd_party=='SP2') {
			ComOpenPopup('/hanjin/COM_ENS_0C1.do', 700, 450, 'getVendor_SP2_sheet', '1,0,1,1,1,1,1,1', true, false, Row, Col);
		} else {
			ComShowCodeMessage('TPB90015'); 
		    try { sheetObj.SelectCell(Row, "vndr_cust_div_cd"); } catch(e) { ComShowMessage(e); } /// set focus		
		}
...
//			obj.value = "0.00"; 
	...
	function getThirdPartyBillingCaseHorizontally(hdiv,ord,method,obj,func){
	 ...
	 function tpb_chkLenByByte(obj, len, name){
		...
		var G_LIST_TYPE = 1;
		function func_rhq_ctrl_ofc_list(rhqType, reqType){
			var rType = 2;
			if(rhqType != undefined && rhqType != null){
				rType = rhqType;				
			}
			...
			if(document.form.s_office_level.value == "H"){
				getTPBGenCombo('s_if_rhq_cd','searchHandleRHQList','F','',rType, new Array("s_ofc_cd_for_rhq","s_office_level"));
			} else if(document.form.s_office_level.value == "R"){
				ComClearCombo(document.form.s_if_rhq_cd);
				getTPBGenCombo('s_if_rhq_cd','searchHandleRHQList','F','',rType, new Array("s_ofc_cd_for_rhq","s_office_level"), '', "if_rhq_cd_OnChange");
				//ComAddComboItem(document.form.s_if_rhq_cd, document.form.s_rhq_cd_for_rhq.value, document.form.s_rhq_cd_for_rhq.value, "if_rhq_cd_OnChange");
//				setTimeout("if_rhq_cd_OnChange();",300);
				...
				getTPBGenCombo('s_if_ctrl_cd','searchCtrlOffice','F','','1', new Array("s_ofc_cd_for_rhq","s_office_level"));
				if_rhq_cd_OnChange(); //  Control Office ComboBox, TPB Office ComboBox 에 값 할당.
//				if_ctrl_cd_OnChange(); // TPB Office ComboBox 에 값 할당.
			}
		...
		/**
	  	...
	  	function if_rhq_cd_OnChange(){
			...
			if(f.s_office_level.value == "H"){ // HO
				...
				getTPBGenCombo('s_if_ctrl_cd','searchControlOfficeList','F','',G_LIST_TYPE, new Array("s_if_rhq_cd","s_office_level"), '', "if_ctrl_cd_OnChange");
		...
		function if_ctrl_cd_OnChange(){
			...
			if(f.s_office_level.value == "H"){ // HO
				ComClearCombo(f.s_if_ofc_cd);
				getTPBGenCombo('s_if_ofc_cd','searchTPBOfficeList','F','',G_LIST_TYPE, new Array("s_if_ctrl_cd","s_office_level"));
			} else if(f.s_office_level.value == "R"){ //RHQ
				ComClearCombo(f.s_if_ofc_cd);
				getTPBGenCombo('s_if_ofc_cd','searchTPBOfficeList','F','',G_LIST_TYPE, new Array("s_if_ctrl_cd","s_office_level"));
		...
		**/
	  	    	  ...
	  	    	  //IB에서 제공하는 컨트롤의 값을 조합한다.
	  	    		  ...
	  	    		  switch(form.elements[i].classid){
	  	    		  	...
	  	    		  	case CLSID_IBMCOMBO: // IBMultiCombo 경우
	  	    		  	     name[j] = form.elements[i].name==""?form.elements[i].id:form.elements[i].name;
	  	    		  	     if(form.elements[i].UseCode) value[j] = form.elements[i].Code;
	  	    		  	     else value[j] = form.elements[i].Text;
	  	    		  	     j++;
	  	    		  	     break;
	  	    		  }
	  	    	  }
	    ...
	    function amtReCalculate()
	    {
	  		...
	  		{
	  			invAmt = parseFloat(invAmt) + parseFloat(sheetObjects[idx].ComputeSum("|inv_dtl_amt|"));
	  		}
	  		
	    	var vatAmt = 0;
	  		for(var idx=0;idx<sheetObjects.length-1;idx++)
	  		{
	  			vatAmt = parseFloat(vatAmt) + parseFloat(sheetObjects[idx].ComputeSum("|vat_dtl_amt|"));
	  		}
	    ...
	    //Invoice Detail Amount 적용 계산
	    function invDtlAddamtReCalculate()
	    {
	  		var addAmt = parseFloat(ComGetUnMaskedValue(document.form.s_add_amt.value,"float")); 
	  		if( isNaN(addAmt) || addAmt == undefined || addAmt == null || addAmt < 0) {
	  		    addAmt = 0.00;
	    	...
	    	var prcsCnt = document.all.prcs_cnt.value;
	  		...
	  		// Only VGM Cost Code start  !
	  		var invAddAmt = 0;
	  		for(var idx=0;idx<sheetObjects.length-1;idx++)
	  		{
	  			invAddAmt = parseFloat(invAddAmt) + parseFloat(sheetObjects[idx].ComputeSum("|inv_dtl_add_amt|"));
	  		}
	  		document.form.s_inv_dtl_add_amt.value = invAddAmt;
	  		if (invAddAmt >= 0) {
	  			addAmt = invAddAmt ;
	  		} 
	  		// Only VGM Cost Code end  !

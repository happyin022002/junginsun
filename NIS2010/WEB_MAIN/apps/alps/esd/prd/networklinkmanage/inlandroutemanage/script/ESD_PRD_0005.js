...
var comData2 ="";
var min_state ='MIN';
var schContiCd = ''; // search한 Continent Code가 Europe일 경우 E 저장
         ...
         try {
            ...
            *****************************/
            ...
            switch(srcName) {
        	    ...
        	    case "btn_retrieve":
//                    if(!checkMandatory() ) return; // 여기서 실제 서버로 보낼 HIDDEN의 i_ord_cd,i_dest_cd 를 셋팅한다.         	    
//    	            if( chkAmericanContinent(formObject) == true){
//                        ComShowMessage(ComGetMsg('PRD90074'));
//       	                break;  
//    	            }
//    	            if (chkAmericanContinent(formObject) == false){
//        	                 
//        	            doActionIBSheet(sheetObject,formObject,IBSEARCH);
//    	                //미주가 아니면 면 bottom form disable
...
//    	            }
    	            ...
    	            // checkMandatory()에서 하던 세팅을 여기로 가져온다.
...
//		            param = '?i_org_cd='+formObject.i_org_cd.value +'&i_dest_cd='+formObject.i_dest_cd.value+'&r_inbound='+formObject.rBtnIrgCd.value+'&nod_tp1='+formObject.rBtnNodTyCd1.value+'&nod_tp2='+formObject.rBtnNodTyCd1.value;
//
//    	            myWin = ComOpenWindowCenter('ESD_PRD_0006.do'+param , 'compop', 800, 520, false);
//    	            myWin.focus();
//        	        break;
        	    ...
        	    case "btn_save":
        	     ...
        	     case "btng_listlink":
    	            ...
    	            //param = '?i_org_cd='+ oriLoc +'&i_desRt_cd='+destLoc;
    	            param = '?i_org_cd='+ oriLoc +'&i_dest_cd='+destLoc+"&row=" + sheetObject1.SelectRow + "&col=" + sheetObject1.SelectCol;
        	      
        	        myWin = ComOpenWindowCenter('ESD_PRD_0007.do'+param, 'compop', 700, 530, false);
		            myWin.focus();
        	        break;

          	    case "btng_save":
          	        /*
          	         * 이 화면에선 미주는 사용 못함 (057 사용)
          	         */
        	   ...
        	   /*
        	    * SAVEAS는 
        	    * 1.FROM - TO 의 조회 조건과 다른 IRG를 만들수 있도록 한다.
        	    * 2.새로운 IRG를 만들때 비슷한 마스터를 더블클릭하여 디테일정보를 보인후 수정해서 새로운 IRG를 쉽게 만들수 있다.
        	    * 
        	    */     
               case "btng_saveas" :
...
//		    ComEnableObject(document.getElementById("btng_saveas"),false);
		}
		
		// 초기 WRS(M) disable
		ComEnableObject(document.form.i_wrs_mt_cd,false);
		...
		//2012.06.07 변종건 [CHM-201217633] TRS의 Cost Batch Error Inquiry 화면에서 PRD 화면 호출 기능 추가
		if( document.form.i_org_cd_ib.value != "" && document.form.i_dest_cd_ib.value != "" ){
			if(!checkInput()) return false;
			document.form.i_org_cd.value = document.form.i_org_cd_ib.value;
			document.form.i_dest_cd.value = document.form.i_dest_cd_ib.value;	
            
            doActionIBSheet(sheetObjects[0],document.form,IBSEARCH);
            //미주가 아니면 면 bottom form disable
            bottomFrmDisable(true);
            bottomFrmClear(document.form);
        ...
        switch(sheetNo) {
            case 1:      //IBSheet1 init
                with (sheetObj) {
                    ...
                    var HeadTitle = "Del.|Optimum|Alt.Opt.|BKG|CHK|STS|Priority|ORG Node|DEST Node|Route|Tmp Flg|FREE SHTL|Total T/T|C.user|C.OFC|C.Date|U.user|U.Date|D.user|D.Date|Remarks||||" ;

                    //해더행정보[필수][ROW,HEADTEXT,ROWMERGE=false, HIDDEN=false]
                    InitHeadRow(0, HeadTitle, true);

                    //데이터속성    [ROW, COL,  DATATYPE,        WIDTH, DATAALIGN, COLMERGE, SAVENAME,                KEYFIELD, CALCULOGIC, DATAFORMAT,      POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
                    InitDataProperty(0, cnt++ , dtDelCheck,   50,    daCenter,  false,    "delChk",                  false,          "",       dfNone,         0,     true,       true, -1, false, true, "", true);
                    InitDataProperty(0, cnt++ , dtCheckBox,   60,    daCenter,  false,    "inlnd_rout_optm_flg",     false,          "",       dfNone,         0,     true,       true, -1, false, true, "", false);
                    InitDataProperty(0, cnt++ , dtCheckBox,   60,    daCenter,  false,    "altn_optm_flg",            false,          "",       dfNone,         0,     true,       true, -1, false, true, "", false);
                    InitDataProperty(0, cnt++ , dtCheckBox,   30,    daCenter,  false,    "bkg_chk",                 false,          "",       dfNone,         0,     true,       true, -1, false, true, "", false);
                    ...
                    InitDataProperty(0, cnt++ , dtPopupEditFormat,70,daCenter,  false,    "prio_seq",                true,           "",       dfInteger,      0,     true,       false, 2);
                    ...
                    InitDataProperty(0, cnt++ , dtData,       70,    daCenter,  false,    "cre_ofc",                 false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,       70,    daCenter,  false,    "cre_date",                false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,       70,    daCenter,  false,    "upd_id",                  false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,       70,    daCenter,  false,    "upd_date",                false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtHidden,     70,    daCenter,  false,    "del_id",                  false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtHidden,     70,    daCenter,  false,    "del_date",                false,          "",       dfNone,         0,     false,       true);
                    ...
                    InitDataProperty(0, cnt++ , dtHidden,     80,    daCenter,  false,    "ori_loc",                 false,          "",       dfNone,         0,     false,       true);
                    InitDataProperty(0, cnt++ , dtHidden,     30,    daCenter,  false,    "ori_loc_type",            false,          "",       dfNone,         0,     false,       true);
...
//                    InitDataCombo (0, "prio_seq", " |", " |");
            ...
            case 2:      //IBSheet2 init
                with (sheetObj) {
                    ...
                    var HeadTitle = "Del.|STS|SEQ|Clear|From Node|From Node|To Node|To Node|Mode|S/P Name|S/P Name|T/T(DD.HH)|AGMT No|Reference No|CombinedMode|C/TOFC|Junction|a|b|c|d|e|f|g" ;
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
           ...
           case IBSEARCH:      //조회
               ...
               sheetObj.DoSearch4Post("ESD_PRD_0005GS.do", PrdFQString(formObj));
               priority_seq = sheetObjects[0].EtcData("prio_seq_combo");
               maxPrioSeq = sheetObjects[0].EtcData("maxPrioSeq");
               schContiCd = sheetObjects[0].EtcData("sch_conti_cd");
            ...
            case IBSAVE:        //마스트 저장
                 ...
                 var rst = sheetObj.DoSave("ESD_PRD_0005GS.do", PrdFQString(formObj));
                 if (!rst) return;
           ...
           case SEARCH03:
              formObj.f_cmd.value = SEARCH03;
 	          ...
 	          var sXml = sheetObj.GetSearchXml("ESD_PRD_0005GS.do", sParam);
// 	          formObj.cnt_cd.value = sXml;
...
/**
 * 시트에서 PRIORITY 팝업 호출
 * - pop_center_open() 호출 : row, col 정보를 Parameter로 전달  
 */
    function sheet1_OnPopupClick(sheetObj, row, col){   
        var formObj = document.form;
        if ( sheetObj.ColSaveName(col) == "prio_seq" || (sheetObj.RowStatus(row) =="U"  && sheetObj.ColSaveName(col)=="wrs_chk" && sheetObj.CellValue(row,"wrs_chk") =="1" ) )
        {
            ...
            irgCd   = formObj.rBtnIrgCd.value;
            typeCd  = formObj.rBtnNodTyCd1.value;
            var param = '?f_cmd=' + SEARCH12 +'&i_org_cd='+ oriLoc +'&i_dest_cd='+destLoc+"&row=" + sheetObj.SelectRow + "&col=" + sheetObj.SelectCol+"&r_inbound="+irgCd+"&r_btn_irg_cd="+irgCd+"&r_btn_nod_ty_cd="+typeCd+"&rout_seq="+rout_seq;
...
//            ComEnableObject(document.getElementById("btng_save"), true);
    ...
    {
        for(var i = sheetObj.HeaderRows ; i < sheetObj.Rows ; i++){
            var lnk_org  = sheetObj.CellValue(i,"lnk_org_loc")+sheetObj.CellValue(i,"lnk_org_type");
            var lnk_dest = sheetObj.CellValue(i,"lnk_dest_loc")+sheetObj.CellValue(i,"lnk_dest_type");
            /*
             * 미주는 이메뉴에서 사용 못함.(057사용)
             */
    ...
    function chkMandatoryForAmerican(formObj, sheetObj) {
            ...
            var wrsFlIdx = formObj.i_wrs_fl_cd.selectedIndex;
            var wrsMtIdx = formObj.i_wrs_mt_cd.selectedIndex;
            var findRd = false;
    ...
    function chkCombindMode(sheetObj){
            ...
            if(inv.substring(0,1) == 'C' || inv.substring(0,1) == 'S') {
            ...
            } else { // inv billing,pattern 이 'C','S'가 아닐때 combind mode 가 셋팅 되있으면 sp가 같은지 확인 , 모드는 다른지 체크 
                var sp = '';
                ...
                for(var j = sheetObj.HeaderRows ; j < sheetObj.Rows ; j++) {
                    if( sheetObj.CellValue(j,"combined_md")==1 && firstCombindMode == false && sheetObj.RowCount > 1){ //첫번쨰 combind면 
                        ...
                        first_trsp_mod_cd= sheetObj.CellValue(j, "trsp_mod_cd");
                        ...
                        if( first_trsp_mod_cd == sheetObj.CellValue(j, "trsp_mod_cd") ) {
                ...
                if( firstCombindMode == true && firstCombindCnt <2 ){ // 체크된게 1건일때 
                    ComShowMessage(ComGetMsg('PRD90024'));
                    combindChk = false;
                    return combindChk;  
                }
        ...
        switch(sAction) {
           ...
           case IBSEARCH:      //Sheet1에서 클릭시 상세 조회 ->Sheet2에 표시 
            ...
            case IBSAVE:        //detail 저장( combind 체크는 하고 온 상태)
                // c/TOFC는 detail 조회시 미주,rail이면 enable 시킴, (현재 입력 필수는 아님)
              // if(!comCheckRequiredField(formObj)) return;
                if(validateForm(sheetObj,formObj,sAction))  
                
                //20071205  이주석은 제거 -- 미주 레일이던 아니던 상태가 전부 'I'일때  new route 
                //20071205 ROW ADD 가 있는것-> 신규라는 것 (SAVE)
                var isNew = false;
                for (var i = sheetObj.HeaderRows ; i < sheetObj.Rows ; i++){
                ...
                sXml = sheetObj.GetSaveXml("ESD_PRD_0005MST_ROWSEARCH.do",SaveStr+"&"+PrdFQString(formObj)+"&isNew="+isNew);  
    ...
    function checkWRS(sheetObj,irgGb) {
        ...
        for (var i = sheetObj.HeaderRows ; i < sheetObj.Rows ; i++){
            ...
            if(inv =='S2R' ) {
                if(irgGb !='M' && formObj.i_wrs_fl_cd.selectedIndex > 0){
                  formObj.i_inv.focus();
                  return false;
                } else if(irgGb =='M' && formObj.i_wrs_mt_cd.selectedIndex > 0){
                  formObj.i_inv.focus();
                  return false;
                }
            }
...
//    }
    ...
    function checkRoute2(sheetObj,formObj) {
        ...
        routDestNod[k] = "";
         ...
         if(arIRow.length >= 2 && arURow.length==1  ) {
             ...
             //<!-- sheetObj2 라우트의 처음과  끝 을 히든으로 갖는다  --> 
             // 마스터 그리드에서 더블클릭할때도 셋팅해둔다. formObj.i_rout_org_nod_cd,formObj.i_rout_dest_nod_cd
            ...
            // 조회 조건 을 가져온다     
             ...
             var hRoutOrgNodCd = formObj.i_rout_org_nod_cd.value;     //마스터 그리드에서 더블클릭으로 셋팅된 값 ,수정될 라우터의 마스터 org
    ...
    function checkRoute3(sheetObj,formObj) {
        ...
        routDestNod[k] = "";
         ...
         // 각링크를 수정하거나 신규로 안만들더라도 마스터 정보만 수정할수도 있다 (Y: 신규, N:link 수정, M:마스터 수정) 
         if(formObj.i_new_route_cd.value != "Y" && formObj.i_new_route_cd.value != "N")  
         {
             formObj.i_new_route_cd.value = "M";   
         }
...
//    	        ComShowMessage("조회조건에 맞지않는 Route가 있습니다.");
    ...
    function sheet1_OnChange(sheetObj,Row,Col,Value) {
    	...
    	switch (evntCol) {
    	case "delChk":
//    		optm_flag 가 있는 route를 삭제할때 OPTM_DEL_ABLE_FLG 가 N 이면 삭제 못함.
//    		OPTM_DEL_ABLE_FLG 는 fm- to 가 같고 seq 가 다른 irg 가 있으면 'N'
    	...
    	case "bkg_chk":
    		if (evntVal == 0 && sheetObj.CellValue(Row, "inlnd_rout_optm_flg") == 1) {
    			...
    			sheetObj.CellValue2(Row, "bkg_chk") = 1;
    			return false;
    		}
    		break;
...
//        	}else 
        	if (evntVal == 1) {
        		...
        		if (sheetObj.CellValue(Row, "lnk_knt").parseInt() > 2) {
                ...
                // sheetObj.FindText 해서 가져온 값으로 같은 Group Optm 제거 E----------------------------------------
    	...
    	case "altn_optm_flg":
         	if (evntVal == 1) {
        		...
        		//alt optm flag 처리 ---------------------------------------
        		if(sheetObj.CellValue(Row, "inlnd_rout_optm_flg") == 1 ) {
        			ComShowCodeMessage("PRD90144");
        			sheetObj.CellValue2(Row, "altn_optm_flg") = 0;
        			return;
        		}
                ...
                //체크된 행번호를 "|"로 연결하여 반환한다
                var org_idx = sheetObj.FindCheckedRow ("altn_optm_flg" );
                //가져온 행을 배열로 반든다.
                var arrRow = org_idx.split("|");
                for (idx=0; idx<arrRow.length-1; idx++){
                	// 현재 체크된 row 또는 현재 체크된 org_node 와 체크된 배열의 org_node 가 다를때 또는 
                	if(Row == arrRow[idx] || org_nod_cd != sheetObj.CellValue(arrRow[idx],"rout_org_nod_cd") ||  dest_nod_cd != sheetObj.CellValue(arrRow[idx],"rout_dest_nod_cd")  ){
                    	 ...
                    	 sheetObj.CellValue2(arrRow[idx],"altn_optm_flg")="0";
                     }
                }
                // sheetObj.FindText 해서 가져온 값으로 같은 Group Optm 제거 E----------------------------------------
...
//    	if(((evntCol == "bkg_chk" && evntVal == 0) || (evntCol == "inlnd_rout_tmp_flg" && evntVal == 1)) && sheetObj.CellValue(Row, "inlnd_rout_optm_flg") == 1) {
//    		ComShowCodeMessage("PRD90137");
////    		alert("Under Optimum flag Check,\nBKG Flag uncheck and Temp Flag ceck is impossible.\nPlz recheck status of Optimum Flag.");
//    		sheetObj.CellValue2(Row, "bkg_chk") = 1;
//    		sheetObj.CellValue2(Row, "inlnd_rout_tmp_flg") = 0;
//    		return false;
//    	}
        ...
        //  bkg_flg cheak => prio_seq is not 0.
     	if(sheetObj.ColSaveName(Col)=="bkg_chk" ) {
     	    if(sheetObj.CellValue(Row,"prio_seq" )=='0' ) {
     	        sheetObj.CellValue2(Row,"bkg_chk" )='N';
...
//        // Optimum Flag 처리
//        if(evntCol == "inlnd_rout_optm_flg") {
//        	if (evntVal == 0) {
//        		// 자가 uncheck 안됨
//        		sheetObj.CellValue(Row, "inlnd_rout_optm_flg") = 1;
//        	}else if (evntVal == 1) {
             ...
             sheetObj.ColSaveName(Col)=="lnk_org_type" || sheetObj.ColSaveName(Col)=="lnk_dest_type" ) ) 
        {
               ...
               sheetObj.CellValue(Row,"lnk_org_type")!="" && sheetObj.CellValue(Row,"lnk_dest_type")!="" ) 
            {
                ...
                if(sheetObj.EtcData("rowCount")==0) {
                    ...
                    sheetObj.CellValue2(Row,"lnk_dest_type") ="";
 ...
 */
    function sheet2_OnPopupClick(sheetObj, row, col){   
        ...
        if ( sheetObj.ColSaveName(col) == "vndr_seq"  )
        {
            ...
            // Radio PopUp  
            if(chkStr == "1,0") {
            	ComOpenPopup('/hanjin/COM_ENS_0C1.do' + param , 700, 450, 'getVendor', dispaly, true, false, row, col, 1);
 ...
 */
   ...
   /**
    ...
    function bottomFrmClear(formObj) {
        ...
        ComClearObject(formObj.i_wrs_fl_cd);
        ComClearObject(formObj.i_wrs_mt_cd);
        ComClearObject(formObj.i_route_rmk);
        ComClearObject(formObj.i_bkg_flg);
        ComClearObject(formObj.i_optm_flg);
        ComClearObject(formObj.i_altn_optm_flg);
...
//        	ComEnableObject(formObj.i_optm_flg,false);
...
//        	ComEnableObject(formObj.i_optm_flg,true);
     ...
     */
     function checkInput() {
         ...
         if(i_org_cd_ib.length < 2 && i_dest_cd_ib.length <2 && schOptmFlg){
         ...
         if(i_org_cd_ib.length < 2 && !schOptmFlg){
			 ComShowMessage(ComGetMsg('PRD90120','2'));
			 formObject.i_org_cd_ib.select();
			 formObject.i_org_cd_ib.focus();  
			 return false;
...
//		 if(org == "US" || org == "CA" || org == "MX") {
		 ...
		 // Check Input Field To
   		//if(i_org_cd_ib.length == 0 || !(i_dest_cd_ib.length ==5 || i_dest_cd_ib.length ==7)){
   			//ComShowMessage(ComGetMsg('PRD90120','To','5 or 7'));
   			//메세지 변경. 2자리 이상입력으로 수정함.  2012.01.10 Kim Hyun Hwa
		 if(i_dest_cd_ib.length <2 && !schOptmFlg){
   			ComShowMessage(ComGetMsg('PRD90120','2'));
...
//     }
     ...
     function checkOptimum(sheetObj) {
    	 ...
    	 if(formObj.i_optm_flg.checked == true && sheetObj.RowCount > 2) {

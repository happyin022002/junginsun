...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
   /**
    ...
    /**
     * @extends
     * @class COM_CSR_0004 : COM_CSR_0004 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
    			...
    			switch(srcName) {
    				...
    				case "btng_ok_k":
    					...
    					if(!isValidSaupja(saupjano)){
    							showErrMessage("올바른 사업자 등록번호가 아닙니다. 다시 확인하세요.");
    							return false;
    					}
    					*/
        ...
        function doActionIBSheet(sheetObj,formObj,sAction) {
            ...
            switch(sAction) {
               ...
               case IBSEARCH:      //조회
                    ...
                    sheetObj.DoSearch4Post("COM_CSR_0004.do", FormQueryString(formObj));
                    break;
      ...
      // Sheet관련 프로세스 처리
        function doActionIBSheet1(sheetObj,formObj,sAction) {
            ...
            switch(sAction) {
               ...
               case IBSEARCH:      //조회
               	sheetObj.DoSearch4Post("COM_CSR_0004GS.do", csrFrmQryStr(formObj));
               break; 
            }
        }
...
//                }
    				...
    				if(opener_obj.document.form.eviInputFlg.value=="Y"){
    						...
    						for(var i=0;i<4;i++){
    								if(eval("opener_obj.document.form.evi_ctnt"+(k+1)).value != "" || eval("opener_obj.document.form.evi_ctnt"+(k+2)).value != "" || eval("opener_obj.document.form.evi_ctnt"+(k+3)).value != ""){
    										...
    										total_amt = parseFloat(sheetObjects[0].CellValue(i+1,6)*100)+parseFloat(csr_deleteComma(total_amt));
    								}
    						}
    						document.form.total_amt.value = csr_addComma(total_amt/100);
    				}
        ...
        */
        ...
        function sheet2_OnSearchEnd(sheetObj,errMsg){
            ...
            var gsFlg= sheetObj.EtcData("gs_flg");
            if(gsFlg=="SEARCH02"){

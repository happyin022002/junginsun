...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
	...
	*/
	...
	/**
	* @extends
	* @class ADM_SYS_0018 : ADM_SYS_0018 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
	*/
	function ADM_SYS_0018() {
			...
			switch (srcName) {
				...
				case "btn_detail":
					...
					if (sRow < 1) {
						...
						if (shtObj.CellValue(sRow, "apsts_cd") == "P") {  // Request일때만 Approval 팝업 호출
							// ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll)
							ComOpenPopup("ADM_SYS_0019.do", 700, 445, "", "0,0", true, false);    // param은 0019팝업내의 sheet1_OnLoadFinish에서 구성
						} else {
							// ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll)
		...
		with (shtObj) {
			switch (shtObj.id) {
				...
				case "sheet1":
					...
					// 컬럼 헤더타이틀
					var HeadTitle  = "STS|Sel|User ID|User Name|Office|Office Type|Requested Date|STS|Comment|apro_rqst_no|apsts_cd|apro_usr_nm|apro_rmk";
					...
					InitDataProperty(0, cnt++, dtData,         80,      daCenter,    false,    "rqst_usr_id",     false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         120,     daCenter,    false,    "rqst_usr_nm",     false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         70,      daCenter,    false,    "rqst_ofc_cd",     false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         80,      daCenter,    false,    "ofc_tp_cd",       false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         110,     daCenter,    false,    "rqst_st_dt",      false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         80,      daCenter,    false,    "apsts_nm",        false,     "",    dfNone,    0,    false,     false);
					InitDataProperty(0, cnt++, dtData,         300,     daLeft,      false,    "rqst_rmk",        false,     "",    dfNone,    0,    false,     false);

					InitDataProperty(0, cnt++, dtHidden,       90,      daCenter,    false,    "apro_rqst_no");
					InitDataProperty(0, cnt++, dtHidden,       80,      daCenter,    false,    "apsts_cd");
					InitDataProperty(0, cnt++, dtHidden,       120,     daLeft,      false,    "apro_usr_nm");
					InitDataProperty(0, cnt++, dtHidden,       300,     daLeft,      false,    "apro_rmk");

					ColIndent("rqst_rmk") = 2;
					AutoRowHeight = false;
	...
	function doActionIBSheet(shtObj, frmObj, sAction, CondParam, PageNo) {
		switch (sAction) {
			...
			case IBSEARCH:    // 조회
				...
				shtObj.DoSearch("ADM_SYS_0018GS.do", FormQueryString(frmObj));
				ComOpenWait(false);
				break;
		}
	}

...
*@FileName : PrdCreateManageDBDAOSearchMixedRtermRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class PrdCreateManageDBDAOSearchMixedRtermRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public PrdCreateManageDBDAOSearchMixedRtermRSQL(){
		setQuery();
		...
		query.append("FileName : PrdCreateManageDBDAOSearchMixedRtermRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	NVL((SELECT rTerm" ).append("\n"); 
		query.append("           FROM(" ).append("\n"); 
		query.append("                 SELECT DISTINCT RCV_TERM_CD rTerm, DECODE(RCV_TERM_CD, 'D',1,'S',2,'Y',3,'T',4,5) ORD " ).append("\n"); 
		...
		query.append("         )," ).append("\n"); 
		query.append("         (" ).append("\n"); 
		query.append("           SELECT rTerm" ).append("\n"); 
		query.append("           FROM(" ).append("\n"); 
		query.append("                 SELECT DISTINCT RCV_TERM_CD rTerm, DECODE(RCV_TERM_CD, 'D',1,'S',2,'Y',3,'T',4,5) ORD " ).append("\n"); 
		...
		query.append("    (SELECT (CASE WHEN P.CNT > 0 THEN 'D' ELSE 'Y' END) rTerm" ).append("\n"); 
		query.append("    FROM " ).append("\n"); 
		query.append("    (" ).append("\n"); 
		query.append("        SELECT COUNT(*) CNT " ).append("\n"); 
		query.append("        FROM PRD_NODE " ).append("\n"); 
		query.append("        WHERE LOC_CD = @[por]" ).append("\n"); 
		query.append("        AND NOD_TP_CD = 'Z' " ).append("\n"); 
		query.append("        AND NVL(DELT_FLG, 'X') <> 'Y' " ).append("\n"); 
		query.append("    ) P)" ).append("\n"); 
		query.append("END rTerm" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 

...
*@FileName : TesAdvanceAuditDBDAOSeachOffDockTmnlAudRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TesAdvanceAuditDBDAOSeachOffDockTmnlAudRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Off-Dock Invoice의 Terminal을 심사한다.
	  * </pre>
	  */
	public TesAdvanceAuditDBDAOSeachOffDockTmnlAudRSQL(){
		setQuery();
		...
		query.append("FileName : TesAdvanceAuditDBDAOSeachOffDockTmnlAudRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("				, CASE WHEN VRFY_RSLT_CD_CTNT IS NOT NULL THEN 'C' ELSE 'S'" ).append("\n"); 
		query.append("				  END AS VRFY_RSLT_AUD_CD" ).append("\n"); 
		query.append("				, CASE WHEN NVL(VOL_AUD_TGT_QTY, 0) > 0 THEN 'F' ELSE 'S'" ).append("\n"); 
		...
		query.append("								, CASE WHEN NVL(CALC_VOL_QTY,0) <> NVL(RVIS_VOL_QTY,0) THEN 1 ELSE 0 END VOL_AUD_TGT_QTY  -- Cal Vol과 Rvse Vol이 다르면 심사대상" ).append("\n"); 
		...
		query.append("									AND		CASE WHEN AA.TML_CALC_IND_CD = 'TP' THEN 'O'" ).append("\n"); 
		query.append("												WHEN AA.TML_CALC_IND_CD = 'SP' THEN 'A'" ).append("\n"); 
		query.append("											END =" ).append("\n"); 
		query.append("											CASE WHEN AA.TML_CALC_IND_CD = 'TP' THEN X.STO_AUD_QTY_CLSS_CD" ).append("\n"); 
		query.append("												WHEN AA.TML_CALC_IND_CD = 'SP' AND X.STO_AUD_QTY_CLSS_CD IN ('I', 'O') THEN 'A'" ).append("\n"); 
		query.append("											END" ).append("\n"); 
		query.append("									AND		NVL(X.RC_FLG, 'N') = NVL(AA.RC_FLG, 'N')" ).append("\n"); 
		...
		query.append("											END =" ).append("\n"); 
		query.append("											CASE WHEN AA.RC_FLG = 'Y' THEN 'R' ELSE 'Y' END" ).append("\n"); 
		...
		query.append("										, H.FM_PRD_DT AS FM_PRD_DT" ).append("\n"); 
		query.append("										, H.TO_PRD_DT AS TO_PRD_DT" ).append("\n"); 

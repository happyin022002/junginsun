...
public class AgreementCorrectionDBDAOSearchCorrRateAgmtRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#if (${fm_eq_knd_cd} == 'U')  " ).append("\n"); 
		query.append("    ,'AGMT Approval Date'" ).append("\n"); 
		query.append("    ,'Update Date'" ).append("\n"); 
		query.append("    ,'Date Gap'" ).append("\n"); 
		...
		query.append("    ,(SELECT LOC_NM FROM MDM_LOCATION X WHERE X.LOC_CD = FM_NOD_CD) FM_NOD_CD_NM" ).append("\n"); 
		query.append("    ,(SELECT LOC_NM FROM MDM_LOCATION X WHERE X.LOC_CD = VIA_NOD_CD) VIA_NOD_CD_NM" ).append("\n"); 
		query.append("    ,(SELECT LOC_NM FROM MDM_LOCATION X WHERE X.LOC_CD = DOR_NOD_CD) DOR_NOD_CD_NM" ).append("\n"); 
		query.append("    ,(SELECT LOC_NM FROM MDM_LOCATION X WHERE X.LOC_CD = TO_NOD_CD) TO_NOD_CD_NM" ).append("\n"); 
		query.append("    ,NVL((SELECT YD_NM FROM MDM_YARD X WHERE X.YD_CD = FM_NOD_CD||FM_NOD_YD), (SELECT LSE_CO_YD_NM FROM MDM_LSE_CO_YD X WHERE X.LSE_CO_YD_CD = FM_NOD_CD||FM_NOD_YD)) FM_NOD_YD_NM" ).append("\n"); 
		query.append("    ,NVL((SELECT YD_NM FROM MDM_YARD X WHERE X.YD_CD = VIA_NOD_CD||VIA_NOD_YD), (SELECT LSE_CO_YD_NM FROM MDM_LSE_CO_YD X WHERE X.LSE_CO_YD_CD = VIA_NOD_CD||VIA_NOD_YD)) VIA_NOD_YD_NM" ).append("\n"); 
		query.append("    ,(SELECT ZN_NM FROM MDM_ZONE X WHERE X.ZN_CD = DOR_NOD_CD||DOR_NOD_YD) DOR_NOD_YD_NM" ).append("\n"); 
		query.append("    ,NVL((SELECT YD_NM FROM MDM_YARD X WHERE X.YD_CD = TO_NOD_CD||TO_NOD_YD), (SELECT LSE_CO_YD_NM FROM MDM_LSE_CO_YD X WHERE X.LSE_CO_YD_CD = TO_NOD_CD||TO_NOD_YD)) TO_NOD_YD_NM" ).append("\n"); 
		query.append("#end " ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		...
		query.append("               ,MAX(DECODE(E.WTR_RCV_TERM_CD, '0', NULL, E.WTR_RCV_TERM_CD)) WTR_RCV_TERM_CD" ).append("\n"); 
		query.append("               ,MAX(DECODE(E.WTR_DE_TERM_CD, '0', NULL, E.WTR_DE_TERM_CD)) WTR_DE_TERM_CD" ).append("\n"); 
		...
		query.append("               ,TO_CHAR(E.AGMT_APRO_DT,'YYYYMMDD')  AS AGMT_APRO_DT" ).append("\n"); 
		query.append("               ,TO_CHAR(E.UPD_DT,'YYYYMMDD')  AS UPD_DT" ).append("\n"); 
		query.append("               ,TO_CHAR(TO_DATE(TO_CHAR(E.AGMT_APRO_DT,'YYYYMMDD'),'YYYYMMDD') - TO_DATE(TO_CHAR(E.UPD_DT,'YYYYMMDD'),'YYYYMMDD')) AS DT_GAP " ).append("\n"); 
		...
		query.append("                ,TO_CHAR(E.AGMT_APRO_DT,'YYYYMMDD')" ).append("\n"); 
		query.append("                ,TO_CHAR(E.UPD_DT,'YYYYMMDD')" ).append("\n"); 
		query.append("                ,CASE WHEN SUBSTR(E.TRSP_AGMT_EQ_TP_SZ_CD,1,2) = 'AL' THEN 1" ).append("\n"); 
		query.append("                      WHEN SUBSTR(E.TRSP_AGMT_EQ_TP_SZ_CD,2,2) = 'AL' THEN 1" ).append("\n"); 
		query.append("                      WHEN SUBSTR(E.TRSP_AGMT_EQ_TP_SZ_CD,3,2) = 'AL' THEN 1" ).append("\n"); 
		query.append("                      ELSE 0" ).append("\n"); 

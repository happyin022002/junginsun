...
*@FileName : TESInvoiceCommonDBDAOValidateEDIInvoice01RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TESInvoiceCommonDBDAOValidateEDIInvoice01RSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ValidateEDIInvoice01
	  * </pre>
	  */
	public TESInvoiceCommonDBDAOValidateEDIInvoice01RSQL(){
		setQuery();
		...
		query.append("FileName : TESInvoiceCommonDBDAOValidateEDIInvoice01RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("			, CASE WHEN H.VNDR_SEQ IN ('158002','114776') AND D.LGS_COST_CD IS NOT NULL AND D.LGS_COST_CD IN ('SVSCRF','TMRFGO','TMRFMO','TMRFPL','TMRFPT','TMRFXX','TMRFGR') --TPSZ Validation에서 제외 (2013.8.20 양양선 부장 요청)" ).append("\n"); 
		...
		query.append("				   -- CHM-201432352 [TESLEA] 철도운송 RF 화물 Genset 관련 Cost Code 생성 (TMRFGR) 추가 - 4347-10-13" ).append("\n"); 
		query.append("				   WHEN H.VNDR_SEQ IN ('180020','186666','176307') AND D.LGS_COST_CD IS NOT NULL AND D.LGS_COST_CD IN ('SVSCRF','TMRFGO','TMRFMO','TMRFPL','TMRFPT','TMRFXX','TMRFGR') " ).append("\n"); 
		query.append("				   AND (TES_GET_EDI_CNTR_TPSZ_CD_FNC(H.VNDR_SEQ,D.CNTR_TPSZ_CD) IS NULL OR SUBSTR(TES_GET_EDI_CNTR_TPSZ_CD_FNC(H.VNDR_SEQ,D.CNTR_TPSZ_CD),1,1) != 'R')" ).append("\n"); 

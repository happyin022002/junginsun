 ...
 *@FileName : ESD_PRD_0070HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESD_PRD_0070HTMLAction extends HTMLActionSupport{

	/**
	 * ESD_PRD_0070HTMLAction 객체를 생성
	 */
	public ESD_PRD_0070HTMLAction(){
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0070Event event = new EsdPrd0070Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		FormCommand command = FormCommand.fromRequest(request);

		if(command.isCommand(FormCommand.SEARCH)){
			event.setPickupReturnCYVO((PickupReturnCYVO) getVO(request, PickupReturnCYVO.class));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setPickupReturnCYVOs((PickupReturnCYVO[]) getVOs(request, PickupReturnCYVO.class));
		}

...
*@FileName : ESD_SPE_1012HTMLAction.java
...
public class ESD_SPE_1012HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_SPE_1012HTMLAction 객체를 생성
	 */
	public ESD_SPE_1012HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdSpe1012Event event = new EsdSpe1012Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setKpiPerformanceVO((KpiPerformanceVO)getVO(request, KpiPerformanceVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setKpiPerformanceVO((KpiPerformanceVO)getVO(request, KpiPerformanceVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setKpiPerformanceVO((KpiPerformanceVO)getVO(request, KpiPerformanceVO .class));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setKpiPerformanceVOs((KpiPerformanceVO[])getVOs(request, KpiPerformanceVO .class, ""));
		}

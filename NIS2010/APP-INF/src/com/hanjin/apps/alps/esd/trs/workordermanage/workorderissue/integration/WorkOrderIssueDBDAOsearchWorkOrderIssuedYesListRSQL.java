...
public class WorkOrderIssueDBDAOsearchWorkOrderIssuedYesListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("ROUND(TRS_AGMT_RATE_CC_PKG.GET_CAL_DIST_BTWN_NOD_FNC( 'ML' , 'D' , A.TRSP_COST_DTL_MOD_CD , A.TRSP_BND_CD , A.FM_NOD_CD , A.VIA_NOD_CD , A.DOR_NOD_CD , A.TO_NOD_CD ),2) AS DISTANCE_UOM ," ).append("\n"); 
		...
		query.append("CASE WHEN A.CUST_NOMI_TRKR_FLG = 'Y' AND A.CUST_NOMI_TRKR_IND_CD IS NULL THEN 'CNT'" ).append("\n"); 
		query.append("     WHEN A.CUST_NOMI_TRKR_FLG = 'N' THEN 'HJS'" ).append("\n"); 
		query.append("     ELSE A.CUST_NOMI_TRKR_IND_CD" ).append("\n"); 
		query.append(" END AS po_cust_nomi_trkr_ind_cd," ).append("\n"); 
		...
		query.append("                                                            NVL(A.BZC_AMT,0)+NVL(A.FUEL_SCG_AMT,0)+NVL(A.SCG_VAT_AMT,0)+NVL(A.nego_amt,0)+NVL(A.etc_add_amt,0)+NVL(A.toll_fee_amt,0)     po_local_curr_tot_amt," ).append("\n"); 
		query.append("round(trs_common_pkg.GET_CONVERSION_USD_AMT_FNC( A.CURR_CD, NVL(A.BZC_AMT,0)+NVL(A.FUEL_SCG_AMT,0)+NVL(A.SCG_VAT_AMT,0)+NVL(A.nego_amt,0)+NVL(A.etc_add_amt,0)+NVL(A.toll_fee_amt,0), to_char(WRK.cre_dt, 'YYYYMM' ) ), 2)  po_usd_curr_tot_amt,  --- 변환" ).append("\n"); 
		...
		query.append("      ,(SELECT CASE WHEN COUNT(X.PRC_CTRT_NO) > 0 THEN 'Y'" ).append("\n"); 
		...
		query.append("                              END " ).append("\n"); 
		query.append("           AND X.DISP_STS_CD = '03'                     " ).append("\n"); 
		query.append("        ) AS CTRT_TP_FLG" ).append("\n"); 
		query.append("      ,NVL((SELECT X.CUST_NOMI_TRKR_IND_CD" ).append("\n"); 
		query.append("              FROM TRS_CUST_NOMI_TRKR X" ).append("\n"); 
		query.append("                  ,BKG_BOOKING Y" ).append("\n"); 
		query.append("             WHERE A.BKG_NO = Y.BKG_NO" ).append("\n"); 
		query.append("               AND X.PRC_CTRT_TP_CD = DECODE(Y.SC_NO,'','R','S')" ).append("\n"); 
		...
		query.append("           ),'HJS') AS CTRT_TP_CD" ).append("\n"); 
		...
		query.append("     AND IN_SVC.BKG_NO IN (SELECT X.BKG_NO" ).append("\n"); 
		...
		query.append("-----------------------------------------------------" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("AND A.HJL_NO IS" ).append("\n"); 
		query.append(" NULL" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${vgm_flg} == 'Y')" ).append("\n"); 
		query.append("AND A.VGM_FLG = 'Y'" ).append("\n"); 
		query.append("#elseif(${vgm_flg} == 'N')" ).append("\n"); 
		query.append("AND (A.VGM_FLG IS NULL OR A.VGM_FLG = 'N')" ).append("\n"); 
		query.append("#end" ).append("\n"); 

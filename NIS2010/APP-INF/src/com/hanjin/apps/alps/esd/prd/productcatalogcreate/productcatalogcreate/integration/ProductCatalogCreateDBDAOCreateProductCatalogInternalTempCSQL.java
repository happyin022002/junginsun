...
public class ProductCatalogCreateDBDAOCreateProductCatalogInternalTempCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    DG_SPCL_FLG,RF_SPCL_FLG,SPCL_AWK_CGO_FLG,BB_SPCL_FLG,SOC_FLG" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		...
		query.append("           AND 'Y' = DECODE(NVL(M2.INLND_ROUT_BKG_FLG, 'N'), 'N', 'N', DECODE(NVL(M2.INLND_ROUT_TMP_FLG, 'N'), 'N', 'Y', 'N'))" ).append("\n"); 
		...
		query.append("                          WHEN @[rTerm] ='D' THEN (SELECT Z.REP_YD_CD FROM MDM_ZONE Z WHERE Z.ZN_CD = OB_ORG AND NVL(Z.DELT_FLG,'N') = 'N')" ).append("\n"); 
		...
		query.append("                          WHEN @[dTerm] ='D' THEN (SELECT Z.REP_YD_CD FROM MDM_ZONE Z WHERE Z.ZN_CD = IB_DEST AND NVL(Z.DELT_FLG,'N') = 'N')" ).append("\n"); 
		...
		query.append("                        RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,OB_ORG,OB_DEST ORDER BY FULL_RTN_PRIO_SEQ,OB_MODE_PRIO_SEQ,OB_OPTM_FLG,OB_BKG_FLG,OB_MODE_PRIO_SEQ,OB_TMP_FLG,OB_PRIO_SEQ) OB_RK," ).append("\n"); 
		...
		query.append("                        CHECK_ROUT," ).append("\n"); 
		query.append("                        IB_ORG,IB_DEST,IB_PRIO_SEQ,IB_SEQ," ).append("\n"); 
		query.append("                        RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,IB_ORG,IB_DEST ORDER BY IB_MODE_PRIO_SEQ,IB_OPTM_FLG,IB_BKG_FLG,IB_TMP_FLG,IB_PRIO_SEQ ) IB_RK," ).append("\n"); 
		...
		query.append("                                                                                   DECODE(@[rTerm],'D',(SELECT REP_ZN_CD FROM MDM_LOCATION L WHERE LOC_CD = @[por] AND NVL(L.DELT_FLG,'N')='N'),@[por]||'%')" ).append("\n"); 
		...
		query.append("                                                                                   DECODE(@[dTerm],'D',(SELECT REP_ZN_CD FROM MDM_LOCATION L WHERE LOC_CD = @[del] AND NVL(L.DELT_FLG,'N')='N'),@[del]||'%')" ).append("\n"); 
		...
		query.append("                                     WHERE YD_CD=TRIM(DECODE(INSTR(SKD_STR,'POL1'),0,NULL,SUBSTR(SKD_STR, 82+length('POL1'),7))) ) POL1_S," ).append("\n"); 
		...
		query.append("                                     WHERE YD_CD=TRIM(DECODE(LNK_KNT,4,TRIM(DECODE(INSTR(SKD_STR,'POD4'),0,NULL,SUBSTR(SKD_STR,491+length('POD4'),7))) ," ).append("\n"); 
		query.append("                                                   3,TRIM(DECODE(INSTR(SKD_STR,'POD3'),0,NULL,SUBSTR(SKD_STR,368+length('POD3'),7)))," ).append("\n"); 
		query.append("                                                   2,TRIM(DECODE(INSTR(SKD_STR,'POD2'),0,NULL,SUBSTR(SKD_STR,245+length('POD2'),7)))," ).append("\n"); 
		query.append("                                                   TRIM(DECODE(INSTR(SKD_STR,'POD1'),0,NULL,SUBSTR(SKD_STR,122+length('POD1'),7)))" ).append("\n"); 
		...
		query.append("                )" ).append("\n"); 
		query.append("                WHERE 1 = CASE WHEN @[rTerm] IN ('D','Y') THEN DECODE(MTPU_CY, NULL, 0, 1)" ).append("\n"); 
		query.append("                               ELSE 1 END" ).append("\n"); 
		query.append("                  AND 1 = CASE WHEN @[dTerm] IN ('D','Y') THEN DECODE(MTRTN_CY, NULL, 0, 1)" ).append("\n"); 
		query.append("                               ELSE 1 END" ).append("\n"); 
		query.append("            )" ).append("\n"); 

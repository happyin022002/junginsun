...
*@FileName : AgreementImportDBDAOSearchHjlHndlFeeHisRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AgreementImportDBDAOSearchHjlHndlFeeHisRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 2012.05.23 SHIN DONG IL [] HJL HANDLING FEE SEARCH INCLUED HISTORY
	  * </pre>
	  */
	public AgreementImportDBDAOSearchHjlHndlFeeHisRSQL(){
		setQuery();
		...
		query.append("FileName : AgreementImportDBDAOSearchHjlHndlFeeHisRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(",A.HNDL_FEE_HIS_SEQ" ).append("\n"); 
		...
		query.append(",REPLACE(A.EFF_FM_DT,'-','') ORG_EFF_FM_DT" ).append("\n"); 
		...
		query.append("FROM (SELECT B.HJL_OFC_CD" ).append("\n"); 
		query.append(",9999 HNDL_FEE_HIS_SEQ" ).append("\n"); 
		query.append(",B.VNDR_SEQ" ).append("\n"); 
		query.append(",B.CURR_CD" ).append("\n"); 
		query.append(",B.COST_RCVR_AMT" ).append("\n"); 
		query.append(",B.COMM_AMT" ).append("\n"); 
		query.append(",B.TTL_AMT" ).append("\n"); 
		query.append(",TO_CHAR (TO_DATE (B.EFF_FM_DT), 'YYYY-MM-DD') EFF_FM_DT" ).append("\n"); 
		...
		query.append("FROM TRS_HJL_HNDL_FEE B" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		query.append("#if ($ofc_cd.size() > 0)" ).append("\n"); 
		query.append("AND B.HJL_OFC_CD in (" ).append("\n"); 
		...
		query.append("AND TO_DATE(@[fm_dt],'YYYYMMDD') BETWEEN  B.EFF_FM_DT AND TO_DATE('29991231','YYYYMMDD')" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("UNION ALL" ).append("\n"); 
		query.append("SELECT C.HJL_OFC_CD" ).append("\n"); 
		query.append(",C.HNDL_FEE_HIS_SEQ" ).append("\n"); 
		...
		query.append(",TO_CHAR (TO_DATE (C.EFF_FM_DT), 'YYYY-MM-DD') EFF_FM_DT" ).append("\n"); 
		query.append(",TO_CHAR (TO_DATE (C.EFF_TO_DT), 'YYYY-MM-DD') EFF_TO_DT" ).append("\n"); 
		...
		query.append("FROM TRS_HJL_HNDL_FEE_HIS C" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		query.append("#if ($ofc_cd.size() > 0)" ).append("\n"); 
		query.append("AND C.HJL_OFC_CD in (" ).append("\n"); 
		query.append("#foreach( ${key} in ${ofc_cd})" ).append("\n"); 
		query.append("#if($velocityCount < $ofc_cd.size())" ).append("\n"); 
		query.append("'$key'," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${fm_dt} != '')" ).append("\n"); 
		query.append("AND TO_DATE (@[fm_dt], 'YYYYMMDD') BETWEEN C.EFF_FM_DT AND C.EFF_TO_DT" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(") A" ).append("\n"); 
		query.append("ORDER BY A.HJL_OFC_CD,A.HNDL_FEE_HIS_SEQ DESC" ).append("\n"); 

...
*@FileName : SpotBidPerformanceReportDBDAO.java
...
* 1.0 최초 생성
=========================================================*/
package com.hanjin.apps.alps.esd.trs.biddingmanage.spotbidperformancereport.integration;
 ...
 *
 * @author 
 * @see SpotBidPerformanceReportBCImpl 참조
 * @since J2EE 1.4
 */
public class SpotBidPerformanceReportDBDAO extends DBDAOSupport {


	/**
	 * SpotBidPerformanceReport 의 데이타 모델에 해당되는 값을 불러온다.<br>
	 * @param event
	 * @param soffice_cd
	 * @return
	 * @throws DAOException
	 */
	public DBRowSet srchSpotBidPerfRpt(EsdTrs0092Event event, String soffice_cd) throws DAOException {
...
//		List<String> contractNo = new ArrayList();
		List<String> spotBidNo = new ArrayList();
...
//		String r_cnt_flg        = JSPUtil.getNull(event.getCntFlg());				//cnt_flg 
		...
		String r_spotbidno		= JSPUtil.getNull(event.getSpotBidNo());			// Spot Bid No
		String r_radio_profit 	= JSPUtil.getNull(event.getRadioProfit());			// Radio Profit
		String r_sel_profit 	= JSPUtil.getNull(event.getSelProfit());     		// Select Profit
		String r_sel_spotbidsts 	= JSPUtil.getNull(event.getSelSpotBidSts());     	// Select Spot Bid Status
		...
		if(!r_spotbidno.equals("")){
			String arrSpotBidNo[] = event.getSpotBidNo().split(",");
			for(int i=0;i<arrSpotBidNo.length;i++){   
				spotBidNo.add(arrSpotBidNo[i]);   
			} 
		}
		param.put("r_spotbidno", spotBidNo);		// Spot Bid No Array
		
		param.put("r_radio_profit", r_radio_profit);		// Radio Profit
		param.put("r_sel_profit", r_sel_profit);			// Radio Profit
		
		param.put("r_sel_spotbidsts", r_sel_spotbidsts);	// Select Spot Bid Status

		try {
			dRs = new SQLExecuter("").executeQuery(new SpotBidPerformanceReportDBDAOSrchSpotBidPerfRptRSQL(), param,param);

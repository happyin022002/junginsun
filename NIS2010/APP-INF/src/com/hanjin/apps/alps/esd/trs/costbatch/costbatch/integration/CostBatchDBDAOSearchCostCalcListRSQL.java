...
public class CostBatchDBDAOSearchCostCalcListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", (SELECT INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL WHERE INTG_CD_ID = 'CD03047' AND INTG_CD_VAL_CTNT = NVL(Y.STS_CD,Z.BAT_PROG_STS_CD) AND ROWNUM <= 1) STS_NM" ).append("\n"); 
		...
		query.append(", Y.ELAPSE_TIME" ).append("\n"); 
		...
		query.append(", TRS_INLND_COST_TRF_HDR B" ).append("\n"); 
		...
		query.append(")" ).append("\n"); 
		query.append("AND     (B.COST_TRF_NO) IN ( SELECT MAX(COST_TRF_NO) FROM TRS_INLND_COST_TRF_HDR GROUP BY CNT_CD, IO_BND_CD )" ).append("\n"); 

...
public class CARIssueTransferSlipManageDBDAOGetAutoRevVVDListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		  SELECT DISTINCT                                                  -- 수정(20070419)" ).append("\n"); 
		query.append("			   NVL(DECODE(NVL(D.TML_CRR_CD,'SML'),'SML'," ).append("\n"); 
		query.append("				 SUBSTR(TES_BKG_REV_VVD_FNC(L.BKG_NO),1,4),'CNTC'),'CNTC') FINC_VVD," ).append("\n"); 
		query.append("			   NVL(DECODE(NVL(D.TML_CRR_CD,'SML'),'SML'," ).append("\n"); 
		query.append("				 DECODE(SUBSTR(TES_BKG_REV_VVD_FNC(L.BKG_NO),1,4),'CFDR'," ).append("\n"); 
		query.append("/* 2008-04-18: 이경한 대리 요청 - 'CFDR'일 경우 COA_BKG_INFO의 VSL_CD와 SKD_VOY_NO를 기준으로 년월을 딴다. CFDR일 경우 VSL_CD는 'FD'+YY 형식이라고 함..." ).append("\n"); 
		query.append("   	           COA_BKG_INFO ==> COA_RGST_BKG 변경 2010-02-22 " ).append("\n"); 
		query.append("               [CHM-201536336]Split29-주간 MAS Open에 따른 타모듈 프로그램 적용 요청" ).append("\n"); 
		query.append("               COA_RGST_BKG ==> MAS_RGST_BKG 변경  2015-06-22  */" ).append("\n"); 
		query.append("				 (SELECT SUBSTR(B.VSL_CD,3,2)||SUBSTR(B.SKD_VOY_NO,1,2) FROM MAS_RGST_BKG B WHERE B.BKG_NO=L.BKG_NO)," ).append("\n"); 
		query.append("				 SUBSTR(TES_BKG_REV_VVD_FNC(L.BKG_NO),5,4)),TO_CHAR(H.ISS_DT,'YYMM'))," ).append("\n"); 
		query.append("				 TO_CHAR(H.ISS_DT,'YYMM')) FINC_SKD_VOY_NO," ).append("\n"); 
		query.append("			   NVL(DECODE(NVL(D.TML_CRR_CD,'SML'),'SML'," ).append("\n"); 
		query.append("				 SUBSTR(TES_BKG_REV_VVD_FNC(L.BKG_NO),9,2),'MM'),'MM') FINC_SKD_DIR_CD," ).append("\n"); 
		query.append("			   L.CNTR_NO," ).append("\n"); 
		query.append("			   L.TML_SO_OFC_CTY_CD," ).append("\n"); 
		query.append("			   L.TML_SO_SEQ," ).append("\n"); 
		query.append("			   L.TML_SO_CNTR_LIST_SEQ" ).append("\n"); 
		...
		query.append("		GROUP BY NVL(D.TML_CRR_CD,'SML'), H.ISS_DT, L.CNTR_NO, L.TML_SO_OFC_CTY_CD, L.TML_SO_SEQ," ).append("\n"); 
		query.append("				 L.TML_SO_CNTR_LIST_SEQ, D.TML_CRR_CD, L.BKG_NO" ).append("\n"); 
		query.append("		  UNION ALL" ).append("\n"); 

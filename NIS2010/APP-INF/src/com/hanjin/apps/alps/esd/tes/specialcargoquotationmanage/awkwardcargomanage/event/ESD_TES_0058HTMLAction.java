...
*@FileName : ESD_TES_0058HTMLAction.java
...
public class ESD_TES_0058HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_TES_0058HTMLAction 객체를 생성
	 */
	public ESD_TES_0058HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdTes0058Event event = new EsdTes0058Event();
		String bkgNo      = JSPUtil.getParameter(request, "bkg_no");
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgNo(bkgNo);
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setTesBbCgoCostVO ((TesBbCgoCostVO) getVO(request, TesBbCgoCostVO .class  ));
			event.setTesBbCgoCostVOs ((TesBbCgoCostVO[]) getVOs(request, TesBbCgoCostVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setTesBbCgoCostVO ((TesBbCgoCostVO) getVO(request, TesBbCgoCostVO .class  ));
			event.setTesBbCgoCostVOs ((TesBbCgoCostVO[]) getVOs(request, TesBbCgoCostVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setTesBbCgoCostVOs ((TesBbCgoCostVO[]) getVOs(request, TesBbCgoCostVO .class, "sheet5_"));
			event.setTesBbCgoDtlVOs ((TesBbCgoDtlVO[]) getVOs(request, TesBbCgoDtlVO .class, "sheet6_"));
		}
		return  event;
	}

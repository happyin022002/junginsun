...
*@FileName : ESD_AOC_3034HTMLAction.java
 ...
 * @since J2EE 1.6
 */
public class ESD_AOC_3034HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_AOC_3034HTMLAction 객체를 생성
	 */
	public ESD_AOC_3034HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsdAoc3034Event event = new EsdAoc3034Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setCntCd(JSPUtil.getParameter(request, "sel_cnt_cd", "") );
		}else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setCurrCd(JSPUtil.getParameter(request, "sel_curr_cd", "") );
		}else if(command.isCommand(FormCommand.SEARCH04)) {
			event.setCurrCd(JSPUtil.getParameter(request, "curr_cd", "") );
		}else if(command.isCommand(FormCommand.SEARCH05)) {
			event.setCntCd(JSPUtil.getParameter(request, "sel_cnt_cd", "") );
			event.setRhqCd(JSPUtil.getParameter(request, "sel_rhq_cd", "") );
		}else if(command.isCommand(FormCommand.MULTI01)) { 
			event.setCntDefaultCurrVOS((CntDefaultCurrVO[])getVOs(request, CntDefaultCurrVO .class, "sheet1_"));

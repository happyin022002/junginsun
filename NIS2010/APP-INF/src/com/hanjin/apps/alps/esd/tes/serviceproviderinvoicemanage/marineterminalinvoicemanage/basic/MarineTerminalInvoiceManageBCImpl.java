...
* 2015-03-24 : [CHM-201534788]GW Agmt Link 기준 변경 (SAve->Confirm) 
* 2015-03-30 : [CHM-201534573]TES: MR Invoice의 CNTR Verify시 Bkg 찾기 logic 추가 (공통항차 입력)
=========================================================*/
...
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes1004Event;
...
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9072Event;
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9190Event;
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9191Event;
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9220Event;
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9232Event;
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.event.EsdTes9252Event;
...
import com.hanjin.apps.alps.esd.tes.serviceproviderinvoicemanage.marineterminalinvoicemanage.integration.MarineTerminalInvoiceManageDBDAO;
 ...
 *
 * @author parkyeonjin
 * @see MarineTerminalInvoiceManageBC 각 DAO 클래스 참조
 * @since J2EE 1.4 
 */
public class MarineTerminalInvoiceManageBCImpl   extends BasicCommandSupport implements MarineTerminalInvoiceManageBC {
	...
	/**
	 * MarineTerminalInvoiceManageBCImpl 객체 생성<br>
	 * MarineTerminalInvoiceManageDBDAO를 생성한다.<br>
	 */
...
//		if(log.isDebugEnabled())log.debug("\n==========MarineTerminalInvoiceManageBCImpl    checkVvdCode() ============event.getParams => ");
//		DBRowSet rowSet=null;
//		try {
//			rowSet = dbDao.checkVVDType(event.getEventParams());
//			return new ESD_TES_901EventResponse(rowSet,"SUCCESS");
...
//	}
	...
	/**
	 * 조회 이벤트 처리<br>
	 * searchTerminalInvoiceBasicInfo 대한 조회 이벤트 처리<br>
	 *
	...
	/**
	 * 조회 이벤트 처리<br>
	 * searchMarineTerminalCNTRList 대한 조회 이벤트 처리<br>
	 *
	 * @param e EsdTes0001Event
...
//			rowSetArr[1] = dbDao.searchTerminalInvoiceContainerList(event.getParams(),"DC");
	...
	/**
	 * 조회 이벤트 처리<br>
	 * searchMarineTerminalInvoice 대한 리스트 조회 이벤트 처리<br>
	 *
	 * @param e Event
	 * @return response EventResponse
	 * @exception EventException
	...
	/**
	 * 조회 이벤트 처리<br>
	 * searchMarineTerminalInvoiceCosts  비용에 대한 리스트 조회<br>
	 *
	 * @param e Event
	 * @return response EventResponse
	 * @exception EventException
	...
	/**
	 * 조회 이벤트 처리<br>
	 * searchMarineTerminalInvoiceCNTRList 컨테이너 정보에 대한 리스트 조회 이벤트 처리<br>
	 *
	 * @param e Event
	 * @return response EventResponse
	 * @exception EventException
	...
	/**
	 * 추가 이벤트 처리<br>
	 * createTerminalInvoiceBasicInfo 에 대한 추가 이벤트 처리<br>
	 *
	 * @param e EsdTes0001Event
...
//				throw new DAOException("Check status of the invoice("+event.getTesTmlSoHdrVO().getInvNo()+"). \nPlease retrieve invoice again.");
	...
	public EventResponse removeTerminalEDIInvoice(Event e) throws EventException {
		EsdTes0013Event event=(EsdTes0013Event)e;
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		if(log.isDebugEnabled())log.debug("\n==========MarineTerminalInvoiceManageBCImpl    removeTerminalEDIInvoice() ============");

		try {
			//dbDao.removeTerminalEDIInvoice(event.getEventParams(), event.getSignOnUserAccount().getUsr_id(), event.getSignOnUserAccount().getOfc_cd());
			dbDao.removeTerminalEDIInvoice(event.getTesEdiSoHdrVO(), event.getSignOnUserAccount().getUsr_id(), event.getSignOnUserAccount().getOfc_cd());
			return eventResponse;
		} catch (DAOException de) {
			log.error("err "+de.toString(),de);
			throw new EventException(de.getMessage());
	...
	public EventResponse getVrfyTmls(Event e) throws EventException {
		log.debug("\n==========MarineTerminalInvoiceManageBCImpl.getVrfyTmls() ============");

		EsdTes9010Event event = (EsdTes9010Event)e;
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		DBRowSet rowSet = null;

		String CURR_CLPT_IND_SEQ = null;
		String CLPT_YD_KNT = null;
		String VRFY_TERMINALS = null;

		try {
			if (event!=null && event.getTesCommonVO()!=null){
				if(!event.getTesCommonVO().getVvd().substring(0,4).equalsIgnoreCase("CNTC")){
					rowSet = dbDao.getVrfyTmls(event);	
				}				
			}
			if (rowSet!=null && rowSet.next()){
				CURR_CLPT_IND_SEQ 	= JSPUtil.getNull(rowSet.getString("CURR_CLPT_IND_SEQ"));
				CLPT_YD_KNT 		= JSPUtil.getNull(rowSet.getString("CLPT_YD_KNT"));
				VRFY_TERMINALS		= JSPUtil.getNull(rowSet.getString("VRFY_TERMINALS"));
			}
//			etcData.put("successFlag", "SUCCESS");
			eventResponse.setETCData("CURR_CLPT_IND_SEQ", JSPUtil.getNull(CURR_CLPT_IND_SEQ));
			eventResponse.setETCData("CLPT_YD_KNT", JSPUtil.getNull(CLPT_YD_KNT));
			eventResponse.setETCData("VRFY_TERMINALS", JSPUtil.getNull(VRFY_TERMINALS));
		} catch (SQLException se) {
			log.error(se.getMessage(), se);
			throw new EventException(se.getMessage());
		} catch (DAOException de) {
	...
	/**
	 * 추가 이벤트 처리
	 * Invoice Header부분 저장 시  ACCM Vol. Data 적용여부 확인 후
	 * User가 입력한 ACCM Vol을 저장한다.
	 * 
	 * @param e EsdTes0001Event
	 * @exception EventException
	 */
...
//					mgSetCnt = 
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESD_TES_0001 화면에 대한 multiTerminalInvoiceDetail 멀티 이벤트 처리<br>
	 *
	 * @param e EsdTes0001Event
	 * @return EventResponse GeneralEventResponse
	 * @exception EventException
	...
	public EventResponse multiTerminalInvoiceDetail(Event e) throws EventException{
		...
		try {
			...
			// Confirm 상태일때. formCommand (MODIFY01 = 141)로 변경.(2010-04-27)
			if( Integer.parseInt(event.getTesCommonVo().getFCmd()) == 141 ){
				// DTL에 있는데 TPB에 없는건 조회.
				tpbCnt = dbDao.multiTerminalInvoiceDetailSearchCount(tesTmlSoDtlVO);
			
				// DTL에 SVXXJO건이 있고 TPB에 등록된건이 없으면.
				if ( tpbCnt > 0 ) {
			...
			for( int i=0; tesTmlSoDtlVOs!=null && i<tesTmlSoDtlVOs.length; i++ ){
				...
				if (tesTmlSoDtlVOs[i].getIbflag().equals("I")) {
					...
					if(JSPUtil.getNull(tesTmlSoDtlVOs[i].getToTrVolVal()).equals("")){
						tesTmlSoDtlVOs[i].setToTrVolVal("99999");
					}
			...
			etcMap.put( "atb_dt", event.getTesCommonVo().getAtbDt() );
			
			//dbDao.multiTerminalInvoiceDetailInsert(insVoList);
			dbDao.multiTerminalInvoiceDetailInsert(insVoList, etcMap);
			dbDao.multiTerminalInvoiceDetailUpdate(updVoList);
			if (delVoList!=null && delVoList.size()>0) {
				dbDao.removeTerminalInvoiceBreakBulkCost(delVoList);
			}
	...
	public EventResponse updateContainerListRvisFlg(Event e) throws EventException {
		...
		try {
			...
			//Auto인 경우 동일한 CNTR_NO에 맵핑 되어 있고, 동일한 그룹의 COST_CODE가 존재하면 모두 업데이트 처리한다.
			dbDao.updateMarineTerminalRvisVolAll(event.getTesTmlSoDtlVO());
	...
	*/
					tesTmlSoRvisListVO.setPlugIn(marineTerminalInvoiceCommonVOs[i].getRvisPlugIn());
					tesTmlSoRvisListVO.setPlugOut(marineTerminalInvoiceCommonVOs[i].getRvisPlugOut());
					tesTmlSoRvisListVO.setPlugTerm(marineTerminalInvoiceCommonVOs[i].getRvisPlugTerm());
...
log.debug("tesTmlSoCntrListVOs===>"+tesTmlSoCntrListVOs);
	...
	/**
	 * 추가 이벤트 처리<br>
	 * removeTerminalInvoiceContainerList 컨테이너 삭제<br>
	 *
	 * @param e EsdTes0001Event
	 * @return EventResponse 
	 * @exception EventException
	...
	public EventResponse removeTerminalInvoiceContainerList(Event e) throws EventException {
		...
		try {
			...
			dbDao.removeTerminalInvoiceBreakBulkCost(event);
...
log.debug("event.getMarineTerminalInvoiceCommonVO().getDeleteMode()===>"+event.getMarineTerminalInvoiceCommonVO().getDeleteMode());			
			if(event.getMarineTerminalInvoiceCommonVO().getDeleteMode().equals("ROW")){
				...
				dbDao.removeTerminalInvoiceBreakBulkCost(voList);
...
//	}
	...
	public EventResponse calculateTerminalInvoiceCost(Event e) throws EventException {
		...
		try {
			rowSetArr[0] = dbDao.calculateTerminalInvoiceCost(event);
			rowSetArr[1] = dbDao.searchTerminalInvoiceTotalAmount(event.getTesTmlSoHdrVO());
			rowSetArr[2] = dbDao.searchTerminalInvoiceTotalAmountForPopup(event.getTesTmlSoDtlVO());
...
//	/**
//	 * Account Code 조회 이벤트 처리<br>
//	 * MarineTerminalInvoiceManage화면에 대한 Account Code 조회 이벤트 처리<br>
//	 *
//	 * @param e EsdTes0001Event
		...
		try {
			...
			log.debug("event.getTesTmlSoHdrVO().getTmlSoOfcCtyCd===>"+event.getTesTmlSoHdrVO().getTmlSoOfcCtyCd());		    	
				...
				while (dbRowSet.next()) {
					...
					tesTmlSoCntrListVO.setLoclCreDt(event.getSignOnUserAccount().getOfc_cd());
					tesTmlSoCntrListVO.setLoclUpdDt(event.getSignOnUserAccount().getOfc_cd());
					tesTmlSoCntrListVO.setClptIndSeq(dbRowSet.getString("clpt_ind_seq"));
					tesTmlSoCntrListVO.setCallYdIndSeq(dbRowSet.getString("call_yd_ind_seq"));
					
//					//[CHM-201534573]TES: MR Invoice의 CNTR Verify시 Bkg 찾기 logic 추가 (공통항차 입력)
//					if(dbRowSet.getString("cntr_sty_cd").equals("F") && dbRowSet.getString("dscr_ind_cd").equals("NH")){
//						dbRowSet2 = dbDao.getCntcBkgNoInfo(tesTmlSoCntrListVO, dbRowSet.getString("yd_cd"));
//						
//						while(dbRowSet2.next()){
//							tesTmlSoCntrListVO.setBkgNo(dbRowSet2.getString("bkg_no"));	
//							tesTmlSoCntrListVO.setVrfyRsltIndCd("CO");
//							tesTmlSoCntrListVO.setDscrIndCd("");
//							tesTmlSoCntrListVO.setIocCd(dbRowSet2.getString("ioc_cd"));			
//							tesTmlSoCntrListVO.setLaneCd(dbRowSet2.getString("lane_cd"));			
//							tesTmlSoCntrListVO.setLoclTsIndCd(dbRowSet2.getString("locl_ts_ind_cd"));
//							tesTmlSoCntrListVO.setRcvdeTermIndCd(dbRowSet2.getString("rcvde_term_ind_cd"));
//							tesTmlSoCntrListVO.setBlNo(dbRowSet2.getString("bl_no"));	
//							tesTmlSoCntrListVO.setTmlTrnsModCd(dbRowSet2.getString("tml_trns_mod_cd"));
//						}					
//					}
	...
	/**
	 * ESD 업무 시나리오 마감작업<br>
	 * MarineTerminalInvoiceManage업무 시나리오 종료시 관련 내부객체 해제<br>
	 */
	public void doEnd() {
		dbDao = null;
	}
...
log.debug("tesFileImpTmpVOs[i].getCntrNo()===>"+tesFileImpTmpVOs[i].getCntrNo());
	...
	/**
	 * CNTC 항차이고 NH인 경우 BKG No. 조회 UPDATE
	 * Create에서 로직 분리
	 *
	 * @param cntrListVO TesTmlSoCntrListVO
	 * @return List<TesTmlSoCntrListVO>
	 * @exception EventException
	 */
	public List<TesTmlSoCntrListVO> searchCntcBkgNoContainerList(TesTmlSoCntrListVO cntrListVO) throws EventException {
		TesTmlSoCntrListVO tesTmlSoCntrListVO = null;
		DBRowSet dbRowSet2	= null;
		...
		try {
			...
			if ( cntrListVO != null ) {
				List<SearchCntcBkgNoCntrListVO> rsList	= dbDao.searchCntcBkgNoCntrList(cntrListVO);
				
				if ( rsList != null && rsList.size() > 0 ) {
					for ( SearchCntcBkgNoCntrListVO listVo : rsList) {
						tesTmlSoCntrListVO = new TesTmlSoCntrListVO();
						
						if ( StringUtil.isEmpty( listVo.getBkgNo() ) ) {
							// [CHM-201534573]TES: MR Invoice의 CNTR Verify시 Bkg 찾기 logic 추가 (공통항차 입력)
							if (	"CNTC".equals( listVo.getVslCd() ) && "F".equals( listVo.getCntrStyCd() ) && "NH".equals( listVo.getDscrIndCd() ) ) {
								dbRowSet2 = dbDao.getCntcBkgNoInfo(listVo);
								if ( dbRowSet2 != null ) {
									while ( dbRowSet2.next() ) {
...
//	/**
//	 * 저장 이벤트 처리<br>
//	 * Verify한 내역을 TES_TML_SO_CNTR_LIST에 저장하기 바로 직전에 우선 지운다.<br>
//	 *
//	 * @param e EsdTes0001Event
//	 * @return EventResponse ESD_TES_0001EventResponse
//	 * @exception EventException
...
//	}
	...
	public EventResponse searchHJSListOnlyList(Event e) throws EventException {
		...
		if(log.isDebugEnabled())log.debug("\n==========MarineTerminalInvoiceManageBCImpl    searchHJSListOnlyList() ============");
		//데이터 전송을 위해 DB ResultSet을 구현한 객체
		DBRowSet rowSet=null;
		try {
			...
			if(event.getMarineTerminalInvoiceCommonVO().getVvdType().equals("B")){
				rowSet = dbDao.searchHJSListOnlyListCNTC(event);
			}else{
				rowSet = dbDao.searchHJSListOnlyList(event);	
			}
	...
	public EventResponse searchEDIInvoiceContainerList(Event e) throws EventException {
		if(log.isDebugEnabled())log.debug("\n==========MarineTerminalInvoiceManageBCImpl    searchEDIInvoiceContainerList() ============");
		...
		DBRowSet rowSet = null;
		try {
			rowSet = dbDao.searchEDIInvoiceContainerList(event);
			eventResponse.setRs(rowSet);
...
//			String	delCntrSeq	= JSPUtil.getNull(event.getMarineTerminalInvoiceCommonVO().getDelCntrSeq());// mr inv 사용하지않음
...
log.debug("event.getTesTmlSoDtlVO().getLgsCostCd()==================>"+event.getTesTmlSoDtlVO().getLgsCostCd());
	...
	/**
	 * MarineTerminal Contaner List 조회화면 시작
	 * @author kimjinjoo
	 */
	...
	/**
	 * MarineTerminal Container List 조회
	 * Accumulate_Hidden 값에 대한 이벤트 처리
	 *
	 * @param e EsdTes0017Event
	 * @return EventResponse EventResponse
	 * @exception EventException
...
//	}
	...
	/**
	 * Terminal Invoice 조회화면 - Summary //시작
	 * @author kimjinjoo
	 */
	...
	public EventResponse searchTerminalExpenseSummary(Event e) throws EventException {
		...
		try {
			...
			rowSet=dbDao.searchTerminalExpenseSummary(event, iPage);
...
//			rowSet=dbDao.searchTerminalExpenseVolumeSummary(event.getParam_map());
			...
			int	iPage	= event.getiPage();
			rowSet=dbDao.searchTerminalExpenseVolumeSummary(event, iPage);
	...
	/**
	 * 수정 이벤트 처리<br>
	 * marineterminalinvoicemanage화면에 대한 수정 이벤트 처리<br>
	 *
	...
	public EventResponse modifyTerminalInvoiceConfirm(Event e) throws EventException {
		...
		try {
			...
			/*//Marine Terminal Invoice이고, ACCM대상인지 확인해서 두 가지 조건을 만족시킬 경우, ACCM Data도 Update 해준다.
	...
	public EventResponse searchEDIInvoiceRHManualContainerList(Event e) throws EventException {
		...
		log.debug("\n==========MarineTerminalInvoiceManageBCImpl    searchEDIInvoiceRHManualContainerList() ============");

		try {
			rowSet = dbDao.searchEDIInvoiceRHManualContainerList(event);
			eventResponse.setRs(rowSet);
			etcData.put("successFlag", "SUCCESS");
			eventResponse.setETCData(etcData);
	...
	/**
	 * 2009-03-12 [R200903110001] : EDI전송 manual cntr목록 조회
	 *
	 * @param e ESD_TES_919Event
	 * @return response ESD_TES_919EventResponse
	 * @exception EventException
	...
	public EventResponse searchEDIInvoiceManualContainerList(Event e) throws EventException {
		...
		log.debug("\n==========MarineTerminalInvoiceManageBCImpl    searchEDIInvoiceManualContainerList() ============");

		try {
			rowSet = dbDao.searchEDIInvoiceManualContainerList(event);
	...
	/**
	 * Terminal Invoice 조회화면 - Summary //End
	 * @author kimjinjoo
	 */
...
//				throw new DAOException("\n [checkDualVVD] Dup Check error exists on VVD. \nClick Retrieve Button to process Invoice. VVD : ");
	...
	/**
	 * MR Invoice 입력시 FIO 조건 CNTR verification 결과 보완  by J PARK  2011-07-25
	 * @param e EsdTes9400Event
	 * @return response GeneralEventResponse
	 * @exception EventException
	 */
	public EventResponse modifyContainerFromCoToDC(Event e) throws EventException{
		EsdTes0001Event event=(EsdTes0001Event)e;
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		if(log.isDebugEnabled())log.debug("\n==========MarineTerminalInvoiceManageBCImpl    modifyContainerFromCoToDC() ============");
	 
	 	try {
			dbDao.modifyContainerFromCoToDC(event);
			eventResponse.setFlowFlag("Success");
			return eventResponse;
		} catch (DAOException de) {
			log.error("err "+de.toString(),de);
			throw new EventException(de.getMessage());
	 ...
	 * @throws EventException
	 */	
	public  List<SearchCostCodeDetailListVO> searchCostCodeDetailList(SearchCostCodeDetailListVO paramVO) throws EventException{
		try {
			return dbDao.searchCostCodeDetailList(paramVO);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("COM12240",new String[]{}).getMessage(),ex);
		} catch (Exception ex) {
	...
	public EventResponse searchPortSkdDetailList(Event e) throws EventException{
		...
		try {
			...
			eventResponse.setRs(dbDao.searchPortSkdDetailList(event.getInfoVO(), event.getCmnVO()));
			eventResponse.setETCData("successFlag", "SUCCESS");
	...
	public EventResponse searchTerminalGSTInvoiceSummary(Event e) throws EventException {
		...
		try {
			...
			eventResponse.setRs(dbDao.searchTerminalGSTInvoiceSummary(event));
			etcData.put("successFlag", "SUCCESS" );
			eventResponse.setETCData(etcData);

...
public class COPSearchDBDAOSearchTargetCOPInfoList2RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT /*+ ORDERED USE_NL(MAIN MST DTL) */ main.pctl_no , REPLACE(main.org_nod_cd_val,'->','/') org_nod_cd_val 																				" ).append("\n"); 
		...
		query.append(" FROM (  				  																																		" ).append("\n"); 
		query.append("       SELECT  pctl_no , SUBSTR(MAX(SYS_CONNECT_BY_PATH(ORG_NOD_CD_VAL,'/')),2) org_nod_cd_val  																" ).append("\n"); 
		query.append("         FROM (  		  																																		" ).append("\n"); 
		query.append("		        SELECT pctl_no " ).append("\n"); 
		query.append("		             , org_nod_cd || DECODE(nod_lnk_div_cd, 'L','->' || intg_cd_val_dp_desc) org_nod_cd_val                                        " ).append("\n"); 

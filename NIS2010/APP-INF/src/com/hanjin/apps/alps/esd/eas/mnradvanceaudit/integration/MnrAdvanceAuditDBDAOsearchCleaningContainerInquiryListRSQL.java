...
*@FileName : MnrAdvanceAuditDBDAOsearchCleaningContainerInquiryListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MnrAdvanceAuditDBDAOsearchCleaningContainerInquiryListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 특정기간 동안 Cleaning한 Container 조회 목록
	  * </pre>
	  */
	public MnrAdvanceAuditDBDAOsearchCleaningContainerInquiryListRSQL(){
		setQuery();
		...
		query.append("FileName : MnrAdvanceAuditDBDAOsearchCleaningContainerInquiryListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("               AND      MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(A.MNR_INP_DT, 'YYYYMM'), A.CURR_CD, 'USD', B.COST_AMT) > TO_NUMBER(@[s_rpr_amt])" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("             )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("SELECT   DENSE_RANK() OVER(ORDER BY EQ_NO) AS SEQ" ).append("\n"); 
		...
		query.append("       , LOC_CNT AS TTL_NO_OF_CASES" ).append("\n"); 
		query.append("       , LOC_CD" ).append("\n"); 
		query.append("       , SUM(CHG_COST_AMT) OVER(PARTITION BY EQ_NO) AS TTL_COST_AMT" ).append("\n"); 
		query.append("       , YD_CD" ).append("\n"); 
		query.append("       , VNDR_SEQ" ).append("\n"); 
		query.append("       , VNDR_NM" ).append("\n"); 
		query.append("       , VNDR_CNT AS NO_OF_CASES" ).append("\n"); 
		query.append("       , CHG_COST_AMT AS COST_AMT" ).append("\n"); 
		query.append("FROM     ROW_DATA" ).append("\n"); 
		query.append("WHERE    1 = 1" ).append("\n"); 
		query.append("AND      LOC_CNT > NVL(TO_NUMBER(@[s_rpr_cnt]), 2)" ).append("\n"); 

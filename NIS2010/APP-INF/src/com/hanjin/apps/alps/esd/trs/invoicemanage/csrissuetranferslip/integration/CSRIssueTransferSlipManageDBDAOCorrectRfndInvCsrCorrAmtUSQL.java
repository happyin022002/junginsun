...
public class CSRIssueTransferSlipManageDBDAOCorrectRfndInvCsrCorrAmtUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", CASE WHEN W.INV_CURR_CD IN ('KRW','JPY','TWD') THEN ROUND(TRSP_RFND_INV_AMT, 0)" ).append("\n"); 
		query.append("ELSE                                     ROUND(TRSP_RFND_INV_AMT, 2)" ).append("\n"); 
		query.append("END" ).append("\n"); 
		query.append("-" ).append("\n"); 
		query.append("CASE WHEN W.INV_CURR_CD IN ('KRW','JPY','TWD') THEN ROUND((RF.TRSP_RFND_INV_AMT/RFND_MON_KNT), 0)" ).append("\n"); 
		...
		query.append(",  TO_NUMBER	(	MONTHS_BETWEEN	(   	TO_DATE(TO_CHAR(HNDL_PRD_TO_DT,'YYYYMM'), 'YYYYMM')" ).append("\n"); 

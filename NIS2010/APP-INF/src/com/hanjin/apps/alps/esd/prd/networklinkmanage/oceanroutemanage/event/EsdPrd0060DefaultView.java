	...
	public String makeXML(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if(serverException != null){
				strErrMsg = new ErrorHandler(serverException).loadPopupMessage();//메세지 코드 존재하지 않음
				//strErrMsg = "=========================================================\n"
				...
				if(e.getFormCommand().isCommand(FormCommand.SEARCH) || e.getFormCommand().isCommand(FormCommand.MULTI)){
					...
					if(rsVoList != null){
						for(int i = 0; i < rsVoList.size(); i++){
							...
							//xmlString.append("<TD EDIT= 'FALSE'><![CDATA[" + JSPUtil.getNull(colValues.get("rmk")) + "]]></TD>");
					...
					xmlString.append(eventResponse.getUserMessage());

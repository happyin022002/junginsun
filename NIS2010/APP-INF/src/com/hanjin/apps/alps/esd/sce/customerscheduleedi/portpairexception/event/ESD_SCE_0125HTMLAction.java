 ...
 *@FileName : ESD_SCE_0120HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.esd.sce.exceptionmanage 화면을 통해 서버로 전송되는 HTML DOM 객체의
 ...
 * @see ExceptionManageEvent 참조
 * @since J2EE 1.6
 */

public class ESD_SCE_0125HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESD_SCE_011HTMLAction 객체를 생성
	 */
	public ESD_SCE_0125HTMLAction() {
		
	}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ExceptionManageEvent로 파싱하여 request에 셋팅<br>
	 * 
	 * @param request
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdSce0125Event event = new EsdSce0125Event();
		if (command.isCommand(FormCommand.SEARCH)) {
			event.setExceptionalRouteVO((ExceptionalRouteVO) getVO(request, ExceptionalRouteVO.class));
		}else if (command.isCommand(FormCommand.SEARCH01)) {
			event.setPartnerId(JSPUtil.getParameter(request, "partner_id", ""));
		}else if (command.isCommand(FormCommand.MULTI)) {
			event.setExceptionalRouteVOs((ExceptionalRouteVO[]) getVOs(request, ExceptionalRouteVO.class,"sheet1_"));

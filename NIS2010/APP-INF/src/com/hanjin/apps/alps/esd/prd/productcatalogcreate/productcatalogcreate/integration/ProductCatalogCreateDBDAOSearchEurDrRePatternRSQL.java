...
public class ProductCatalogCreateDBDAOSearchEurDrRePatternRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("DECODE(M3.POR_NOD_CD,H.POR_NOD_CD,'N','Y') POR_TML_DIFF_FLG," ).append("\n"); 
		...
		query.append("DECODE(M3.DEL_NOD_CD,H.DEL_NOD_CD,'N','Y') DEL_TML_DIFF_FLG," ).append("\n"); 
		...
		query.append("FROM PRD_BKG_COP_MAP M, BKG_BOOKING B, SCE_COP_HDR H, PRD_PROD_CTL_MST M3, SCE_TRO_MAPG TM, BKG_EUR_TRO TRO, BKG_EUR_TRO_DTL TDTL, PRD_PROD_CTL_MST M2 ," ).append("\n"); 
		query.append("     (" ).append("\n"); 
		...
		query.append("AND TDTL.BKG_NO = TRO.BKG_NO" ).append("\n"); 
		query.append("AND TDTL.TRO_SEQ = TRO.TRO_SEQ" ).append("\n"); 
		query.append("AND TDTL.TRO_SUB_SEQ = CASE WHEN TRO.IO_BND_CD = 'O' " ).append("\n"); 
		query.append("                                 THEN (SELECT MIN(TRO_SUB_SEQ) FROM BKG_EUR_TRO_DTL IDTL" ).append("\n"); 
		query.append("                                       WHERE IDTL.BKG_NO = TRO.BKG_NO" ).append("\n"); 
		query.append("                                         AND TDTL.TRO_SEQ = TRO.TRO_SEQ" ).append("\n"); 
		query.append("                                         AND TDTL.DOR_ADDR_TP_CD = 'D')" ).append("\n"); 
		query.append("                            ELSE (SELECT MAX(TRO_SUB_SEQ) FROM BKG_EUR_TRO_DTL IDTL" ).append("\n"); 
		query.append("                                       WHERE IDTL.BKG_NO = TRO.BKG_NO" ).append("\n"); 
		query.append("                                         AND TDTL.TRO_SEQ = TRO.TRO_SEQ" ).append("\n"); 
		query.append("                                         AND TDTL.DOR_ADDR_TP_CD = 'D')" ).append("\n"); 

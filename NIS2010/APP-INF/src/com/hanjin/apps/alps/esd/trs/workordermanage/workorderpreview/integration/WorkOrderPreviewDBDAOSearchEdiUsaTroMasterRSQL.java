...
public class WorkOrderPreviewDBDAOSearchEdiUsaTroMasterRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT  NVL(SO.TRSP_WO_OFC_CTY_CD||SO.TRSP_WO_SEQ,@[trsp_wo_ofc_cty_cd] || @[trsp_wo_seq]) AS wo_no " ).append("\n"); 
		...
		query.append(" 		,LTRIM(TO_CHAR((SELECT SUM(NVL(x.bzc_amt,0)+NVL(x.nego_amt,0)+NVL(x.etc_add_amt,0)+NVL(x.fuel_scg_amt,0)+NVL(x.scg_vat_amt,0)+NVL(x.toll_fee_amt,0))" ).append("\n"); 
		query.append(" 						  FROM trs_trsp_svc_ord x" ).append("\n"); 
		query.append("				   		 WHERE x.trsp_wo_ofc_cty_cd= so.trsp_wo_ofc_cty_cd" ).append("\n"); 
		...
		query.append("	 AND phn_vndr_cntc.PRMRY_CHK_FLG = 'Y'" ).append("\n"); 

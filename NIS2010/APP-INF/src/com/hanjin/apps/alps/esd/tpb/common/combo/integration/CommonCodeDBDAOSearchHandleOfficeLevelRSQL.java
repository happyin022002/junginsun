...
public class CommonCodeDBDAOSearchHandleOfficeLevelRSQL implements ISQLTemplate{
	...
	public CommonCodeDBDAOSearchHandleOfficeLevelRSQL(){
		...
		params.put("is_include_control_office",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void SetQuery(){
		...
		query.append("       DECODE( office_level, 'C', (SELECT DISTINCT A.n3pty_ofc_tp_cd FROM TPB_HNDL_OFC A WHERE A.ofc_cd = ofc_cd AND n3pty_ofc_tp_cd = 'T') ) AS priv_cd" ).append("\n");
		...
		query.append("          AND delt_flg = 'N'" ).append("\n");
		query.append("		  AND ofc_cd = @[s_ofc_cd]" ).append("\n");
		query.append("#if (${is_include_control_office} == true)" ).append("\n");
		query.append("       -----" ).append("\n");
		query.append("       UNION ALL" ).append("\n");
		query.append("       ----- CONTROL -----" ).append("\n");
		query.append("       SELECT n3pty_ofc_tp_cd AS office_level," ).append("\n"); 
		query.append("              ofc_cd," ).append("\n");
		query.append("              (SELECT rhq_cd FROM TPB_HNDL_OFC WHERE rhq_cd IS NOT NULL AND ofc_cd = HN.ofc_cd ) AS rhq_cd," ).append("\n");

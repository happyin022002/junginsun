...
public class InlandRouteManageDBDAOUpdatePsUSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * UpdatePs
	  * 유럽 조건 제외
	  *              WHEN (SELECT CONTI_CD FROM MDM_SUBCONTINENT WHERE SCONTI_CD = (SELECT SCONTI_CD FROM MDM_COUNTRY WHERE CNT_CD = SUBSTR(@[i_rout_org_nod_cd], 1,2)))  <> 'E' THEN 'N'
	  *              WHEN (SELECT CONTI_CD FROM MDM_SUBCONTINENT WHERE SCONTI_CD = (SELECT SCONTI_CD FROM MDM_COUNTRY WHERE CNT_CD = SUBSTR(@[i_rout_dest_nod_cd], 1,2))) <> 'E' THEN 'N'
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("       inlnd_rout_bkg_flg = DECODE(@[i_bkg_flg],'1','Y','Y','Y','N') ,  " ).append("\n"); 
		query.append("       inlnd_rout_optm_flg = " ).append("\n"); 
		query.append("        CASE WHEN PCTL_IO_BND_CD = 'B' THEN 'N'" ).append("\n"); 
		query.append("             WHEN @[i_optm_flg] IN ('1', 'Y') THEN 'Y'" ).append("\n"); 

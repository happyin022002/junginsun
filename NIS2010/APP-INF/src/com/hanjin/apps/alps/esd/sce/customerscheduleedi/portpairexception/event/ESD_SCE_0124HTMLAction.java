...
*@FileName : ESD_SCE_0124HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.esm.saq.modelconstraintmanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_SCE_0124HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_SCE_0124HTMLAction 객체를 생성
	 */
	public ESD_SCE_0124HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdSce0124Event event = new EsdSce0124Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setBlockLaneVOs((BlockLaneVO[])getVOs(request, BlockLaneVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setBlockLaneVO((BlockLaneVO)getVO(request,BlockLaneVO.class));
		}

...
import com.hanjin.apps.alps.esd.sce.copdetailreceive.integration.CopDetailReceiveDBDAO;
...
//			}			
	...
	public boolean addSceActRcvIf(SceActRcvIfVO actVo) throws EventException{
		...
		try {
			...
			//롱비치 터미널 USLGBM1에서 ACT_STS_MAPG_CD = A로 322 EDI 수신시 UR 상태도 같이 수신한 것으로 처리 (2018.09.29 BY 연규석 차장) CSR #3583
			//INBOUND는 터미널에서 A상태를 보내주지 않으므로 BOUND 체크는 하지 않음 (결국 FORRUD,FIRRUD 중 FORRUD 하고만 Actual Mapping이 된다.)
			if(actVo.getActRcvTpCd().equals("3") && actVo.getActStsMapgCd().equals("A") && actVo.getNodCd().equals("USLGBM1") && actVo.getActCd().equals("SSOFA")){
				actVo.setActStsMapgCd("UR");
				actVo.setCreUsrId(actVo.getActCd()+"(A=>UR)");
				dbDao.addSceActRcvIf(actVo);
			}
	 ...
	 * @exception EventException
	 */
	public boolean addSceCSMTgtEurVO(SceActRcvIfVO actVo) throws EventException{
		boolean resultFlag=false;
...
//			log.info("\n1.GAP구하기:["+skdEditVo.getRcvEvntGapDesc()+"]");
			if(skdEditVo.getRcvEvntGapDesc() != null && !("0").equals(skdEditVo.getRcvEvntGapDesc())){
				...
				//2.Bound Schedule Duplication Check OE:TRO Estimate/IE:TRI Estimate
...
//								log.info("\n railDestN1stEtaDt:"+railDestN1stEtaDt+"  actHisVO.getCntrNo():"+actHisVO.getCntrNo()+"  actHisVO.getBkgNo():"+actHisVO.getBkgNo()+"  copNoEta.substring(17, 21):"+copNoEta.substring(17, 21)+"  Integer.toString(copItchgChk):"+Integer.toString(copItchgChk)+"  copNoEta.substring(0, 3):"+copNoEta.substring(0, 3));
								...
								// 2014.08.29 [CHM-201431416] 신규 EDI 315 추가 - EARN (Rail ETA at DEST)
								if(!"XXXXXXX".equals(actHisVO.getNodCd()) && "EARN".equals(ediStsCd)){
									...
									edi315VO.setEdiStatus(ediStsCd);
									edi315VO.setBlNo(actHisVO.getBkgNo());
									edi315VO.setBkgNo(actHisVO.getBkgNo());
									edi315VO.setEventDt(actHisVO.getRailDestN1stEtaDt());
									edi315VO.setEventYard(actHisVO.getNodCd());
									edi315VO.setCreId(actHisVO.getCreUsrId());
									edi315VO.setUpdId(actHisVO.getUpdUsrId());
									edi315VO.setCopNo(dtlMapgVO.getCopNo());
									edi315VO.setCopDtlSeq(dtlMapgVO.getCopDtlSeq());
									edi315VO.setActRcvIfKeyDt(actHisVO.getActRcvDt());
									edi315VO.setActRcvIfKeyNo(actHisVO.getActRcvNo());
									edi315VO.setMvmtSts(actHisVO.getActStsMapgCd());
...
// 변경 로직 Point						
// (POL) ETD-ETA-ETB (POD) 순서로 EVENT가 발송하는데 FEEDER의 경우 이전에는 ETB가 존재하지 않아 마지막 Estimate값이 ETA였음
// 현재는 FEEDER, MOTHER 모두 ETB가 존재하므로 ETB인경우에 대해서 다음 VVD가 실제 Schedule을 가진 VVD인지 여부를 확인하여
// Schedule을 가진 VVD라면 자기 자신만을 그렇지 않다면 Schedule을 가진 VVD 이전까지 estimate 를 변경하도록 해야 함
// Schedule이 존재하는 VVD가 없다면 이후 Schedule을 모두 Update 하면 됨
						
						//4.Estimate GAP이 발생한  ETB
...
//				log.info("\n NO_DATA_FOUND :"+vpsVO.getVpsRcvDt()+", "+vpsVO.getVpsRcvNo());
        ...
        if(vpsVO.getVpsEvntTpCd().equals("ETA")||vpsVO.getVpsEvntTpCd().equals("ETD")
        		||vpsVO.getVpsEvntTpCd().equals("ETB")){  // 2015.03.03 김민정 [CHM-201534483] Anchored Vessel 관련 ETB, ATB 수신 화주 추가
//        	String checkLastVvdDt = "";
        	DBRowSet dbRowset = null;	        
//	        log.info("\n  ");      
	        try {	            	
	            	//입력받은 vvd의 기받은 날짜를 비교하기 위해서 
	        		// 2013.11.15 CHM-201327471 315 EDI 로직 수정 (ETA, ETD 시 최근발송된 날째 체크 삭제처리.
//	            	checkLastVvdDt = searchLastVvdDtUnit(vpsVO);
	            	
//	            	log.info("\n checkLastVvdDt ["+checkLastVvdDt+"]");
 
//	            	if ((checkLastVvdDt == null)
//		                || (checkLastVvdDt.compareTo(vpsVO.getVpsEvntDt().substring(0, 8))!= 0)) {
...
//							log.info("\n VSL SKD:: edi315VO.getCopDtlSeq()"+edi315VO.getCopDtlSeq()+"------------------------------");
							...
							log.error("\n VSL SKD:: dbRowset.getString(edi_sts)"+dbRowset.getString("edi_sts")+"------------------------------");
							log.error("\n VSL SKD:: vpsVO.getClptIndSeq()"+vpsVO.getClptIndSeq()+"------------------------------");
							log.error("\n VSL SKD:: vpsVO.getVpsRcvDt()"+vpsVO.getVpsRcvDt()+"------------------------------");
							log.error("\n VSL SKD:: vpsVO.getVpsRcvDt()"+vpsVO.getVpsRcvDt()+"------------------------------");
							edi315VO.setClptIndSeq(vpsVO.getClptIndSeq());
							edi315VO.setActRcvIfKeyDt(vpsVO.getVpsRcvDt());
							edi315VO.setActRcvIfKeyNo(vpsVO.getVpsRcvNo());							
...
//						log.info("\n CNTR 관련 COP 없음===================== ");
					...
					// <______________________________________________________________ CHM-201324593 : Added 
						...
						if(actHisVO.getActStsMapgCd().equals("OP")||((actHisVO.getActStsMapgCd().equals("MT")||actHisVO.getActStsMapgCd().equals("XX"))&& !actVo.getBkgNo().equals(""))){
							// CHM-201326862 [SCEM] COP-COA 연결 로직 변경
							
							//CHM-201326862 : manual replan 일때는 COA 호출을 막기 위해 remark ("MnlRpln") 를 넘김
							
							String copUpdRmk = actVo.getCopUpdRmk();
							
							
							if ( modifyCOPNodeByMVMT(actHisVO)) {
								
								if(!"MnlRpln".equals(copUpdRmk) && !"MnlRplnByUI".equals(copUpdRmk)){
									// Call COA IF method
									...
									masBkgComIfVo.setBkgNo(actVo.getBkgNo());
...
//								}								
							...
							// CHM-201326664 : Added ______________________________________________________________> 
							else if	( 
										...
										&&
										(
											( dtlMapgVO.getActCd().endsWith("FTWMDO") && dtlMapgVO.getActStsMapgCd().equals("TN") )
											||
											( dtlMapgVO.getActCd().endsWith("FTWMAD") && dtlMapgVO.getActStsMapgCd().equals("TS") )
										)
										&&
										( !checkShuttleBarge(dtlMapgVO) )
									)
							{
								/*  T/S Shuttle Barge mapping 이 아닌 Case 는 무시한다 */
									actVo.setActUmchTpCd("63");
									errCd = "02";
									actVo.setErrMsg("Not T/S Shuttle Barge mapping");
									modifySceActRcvIfByError(actVo);
									actHisVO.setActUmchTpCd(actVo.getActUmchTpCd());
									actHisVO.setErrMsg(actVo.getErrMsg());					
							}
...
//							    log.info("\n 10-1.Actual Mapping 정상처리 stnd_edi_sts_cd====");
							    //10-1.Actual Mapping 정상처리 stnd_edi_sts_cd
							    //Node 불일치로 EDI 전송이 안됐던 건이 Replan에 의해 일치된경우 315전송을 시도한다
							    if( (actVo.getActStsMapgCd().equals("OC") && actVo.getBfActUmchTpCd() != null && actVo.getBfActUmchTpCd().equals("30") && actVo.getActUmchTpCd().equals("99") && pro_flg.equals("REPLAN")) ||((actVo.getCreTpCd()==null || !actVo.getCreTpCd().equals("C")) && errCd.equals("00") && !dtlMapgVO.getStndEdiStsCd().equals("") && (pro_flg.equals("BATCH") || (dtlMapgVO.getRplnBatSndFlg().equals("Y") && pro_flg.equals("REPLAN"))))){
...
//									}
								  ...
								  //10-1.Actual Mapping 정상처리 stnd_edi_sts_cd
								  //Node 불일치로 EDI 전송이 안됐던 건이 Replan에 의해 일치된경우 315전송을 시도한다
								    if( (actVo.getActStsMapgCd().equals("OC") && actVo.getBfActUmchTpCd() != null && actVo.getBfActUmchTpCd().equals("30") && actVo.getActUmchTpCd().equals("99") && pro_flg.equals("REPLAN")) ||((actVo.getCreTpCd()==null || !actVo.getCreTpCd().equals("C")) && actVo.getActUmchTpCd().equals("99") && errCd.equals("00") && !dtlMapgVO.getStndEdiStsCd().equals("") && (pro_flg.equals("BATCH") || (dtlMapgVO.getRplnBatSndFlg().equals("Y") && pro_flg.equals("REPLAN"))))){
...
//				SceActRcvHisVO actHistVO = new SceActRcvHisVO();
		...
		//		------            end if;  					
								...
								// 2013.12.09 [CHM-201327733] Double calling 시 315 EDI 전송 로직 보완 요청
								log.error("\n VSL ACT:: edi315CopVO.getActStndEdiStsCd()"+edi315CopVO.getActStndEdiStsCd()+"------------------");
								log.error("\n VSL ACT:: actVo.getClptIndSeq()"+actVo.getClptIndSeq()+"------------------");
								log.error("\n VSL ACT:: actVo.getActRcvDt()"+actVo.getActRcvDt()+"------------------");
								log.error("\n VSL ACT:: actVo.getActRcvNo()"+actVo.getActRcvNo()+"------------------");
								edi315VO.setClptIndSeq(actVo.getClptIndSeq());								
								edi315VO.setActRcvIfKeyDt(actVo.getActRcvDt());
								edi315VO.setActRcvIfKeyNo(actVo.getActRcvNo());
...
//		copMapgVO.setActDt(act_dt);  /* 2015-06-07 [poong] 현재 사용되지 값으로 convertToSkdVo()에서 잘못 사용될수 있어 삭제  */
...
//			if(!pro_flg.equals("REPLAN") && sendingEdiCount>0) searchSendEDIList(tempVVD+"", "VVD", outCopNo);
	...
	private SceCopSkdHisVO convertToSkdVo(SceActRcvHisVO actHisVO, CopDtlActMapgVO dtlMapgVO) throws EventException{
		...
		skdVO.setBfrActDt(dtlMapgVO.getActDt()); //2015-06-07 [poong] new Actual date 이전 db에 저장되있던 old Actual date; 
	...
	/**
	 * TS 지역 Barge Case인지 체크한다 <br>
	 * 
	 * @param CopDtlActMapgVO dtlMapgVO
	 * @return boolean
	 * @exception EventException
	 */
	public boolean  checkShuttleBarge(CopDtlActMapgVO dtlMapgVO) throws EventException{
		boolean resultFlag=false;
		
		try {

			resultFlag=dbDao.checkShuttleBarge(dtlMapgVO)>0?true:false;  

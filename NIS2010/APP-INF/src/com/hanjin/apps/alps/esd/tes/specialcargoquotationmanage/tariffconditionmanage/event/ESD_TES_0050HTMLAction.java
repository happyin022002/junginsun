...
*@FileName : ESD_TES_0050HTMLAction.java
 ...
 * - Parsing 한 정보를 Event로 변환, request에 담아 GuaranteeManageSC로 실행요청<br>
...
public class ESD_TES_0050HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_TES_0050HTMLAction 객체를 생성
	 */
	public ESD_TES_0050HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdTes0050Event event = new EsdTes0050Event();

		if (command.isCommand(FormCommand.SEARCH)) {
			event.setTesTrfCondVO((TesTrfCondVO)getVO(request,TesTrfCondVO.class));
		} else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setTesTrfCondVO((TesTrfCondVO)getVO(request,TesTrfCondVO.class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setTesTrfCondVO((TesTrfCondVO)getVO(request,TesTrfCondVO.class,"sheet3_"));
			event.setComTesCondDtlVOs((ComTesCondDtlVO[])getVOs(request,ComTesCondDtlVO.class,"sheet4_"));

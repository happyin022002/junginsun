...
public class OceanLinkBackEndDBDAOOceanLinkMergeUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      USING (select @[polprot]  ORG_LOC_CD,@[podprot]  DEST_LOC_CD, @[lanecd]  VSL_SLAN_CD " ).append("\n"); 
		query.append("             from DUAL )  B " ).append("\n"); 
		query.append("      ON  (A.ORG_LOC_CD=B.ORG_LOC_CD AND A.DEST_LOC_CD = B.DEST_LOC_CD AND A.TS_IND_CD='D' AND A.N1ST_LANE_CD = B.VSL_SLAN_CD AND A.N1ST_LANE_FDR_FLG = 'N' ) " ).append("\n"); 
		query.append("WHEN MATCHED THEN  " ).append("\n"); 
		query.append("     UPDATE SET UPD_IND_CD ='S' ,N1ST_POL_CD=@[polprot]  ,N1ST_POD_CD=@[podprot]   ,N1ST_SKD_DIR_CD=@[dircd]  , " ).append("\n"); 

CREATE OR REPLACE PROCEDURE SCE_ACT_RCV_HIS_DEL_PRC(STARTDATE IN  CHAR, ENDDATE IN CHAR)
AUTHID CURRENT_USER
IS
...
BEGIN
    ...
    V_SYS_NM        := 'SCE';
    V_PROG_NM       := 'SCE_ACT_RCV_HIS';
    V_OWN_NM        := 'NISADM';
    V_ENT_NM        := 'SCE_ACT_RCV_HIS';
    V_TAB_NM        := 'SCE_ACT_RCV_HIS';
    V_INS_ROW_CNT   := 0;
    V_LOG_MSG       := '';
    V_LOG_MSG_MAIN  := '';
    V_ERRNO         := '';
...
LOOP
    ...
    DELETE  FROM SCE_ACT_RCV_HIS WHERE  ACT_RCV_DT >= STARTDATE
                                   AND  ACT_RCV_DT < ENDDATE    --AND CRE_DT < TO_DATE ('20080101', 'YYYYMMDD')
                                                    AND ROWNUM <= 10000 ;

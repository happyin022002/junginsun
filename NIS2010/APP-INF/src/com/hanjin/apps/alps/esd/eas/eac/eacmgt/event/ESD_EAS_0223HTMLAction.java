...
*@FileName : ESD_EAS_0223HTMLAction.java
...
public class ESD_EAS_0223HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_EAS_0223HTMLAction 객체를 생성
	 */
	public ESD_EAS_0223HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		EsdEas0223Event event = new EsdEas0223Event();
		...
		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setEacFileInVO((EacFileInVO)getVO(request, EacFileInVO.class));
		} else if(command.isCommand(FormCommand.MULTI)) {
				EacFileInVO eacFileInVO 	= (EacFileInVO)getVO(request, EacFileInVO.class);
				EacFileOutVO[] eacFileOutVOs 	= (EacFileOutVO[])getVOs(request, EacFileOutVO.class,"");
				if (keys != null ) {
					eacFileInVO.setKeys(keys);
				}
				eacFileInVO.setEacFileOutVOs(eacFileOutVOs);
				event.setEacFileInVO(eacFileInVO);
		}

...
public class InvoiceManageDBDAOSearchInvoiceRevisionDetailListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT   NVL(T.IDA_SAC_CD, G.IDA_SAC_CD) AS IDA_SAC_CD" ).append("\n"); 
		query.append("       , (" ).append("\n"); 
		query.append("           SELECT   S.IDA_SAC_NM" ).append("\n"); 
		query.append("           FROM     BKG_IDA_SAC_MST S" ).append("\n"); 
		query.append("           WHERE    1 = 1" ).append("\n"); 
		query.append("           AND      S.IDA_SAC_CD = NVL(T.IDA_SAC_CD, G.IDA_SAC_CD)" ).append("\n"); 
		...
		query.append("       , (SELECT TP.N3PTY_IF_TP_CD FROM TPB_N3RD_PTY_BIL_TP TP WHERE TP.N3PTY_BIL_TP_CD = A.N3PTY_BIL_TP_CD) N3PTY_TP_CD" ).append("\n"); 
		...
		query.append("                                      ), ( SELECT OFC_CD FROM TPB_OTS_GRP WHERE N3PTY_NO = A.N3PTY_NO )" ).append("\n"); 
		...
		query.append("       , TPB_GET_INV_CURR_CHG_FNC(@[s_from_curr_cd],@[s_curr_cd],B.VAT_AMT, TPB_GET_LCL_DATE_FNC(SYSDATE,@[user_ofc_cd]) ) S_VAT_AMT /* TPB LOCAL DATE */" ).append("\n"); 
		query.append("       , TPB_GET_INV_CURR_CHG_FNC(@[s_from_curr_cd],@[s_curr_cd],A.ADD_AMT, TPB_GET_LCL_DATE_FNC(SYSDATE,@[user_ofc_cd]) ) INV_DTL_ADD_AMT /* TPB LOCAL DATE */" ).append("\n"); 
		...
		query.append("       , B.VAT_AMT AS S_VAT_AMT" ).append("\n"); 
		query.append("       , NVL(A.ADD_AMT,0) INV_DTL_ADD_AMT " ).append("\n"); 
		...
		query.append("       , A.ACCT_CD ACCT_CD_ORG" ).append("\n"); 
		query.append("       , A.OTS_DTL_SEQ" ).append("\n"); 
		query.append("       , CASE WHEN NVL(A.VAT_DTL_AMT,0) <> 0 THEN 1 ELSE 0 END VAT_DTL_CHK" ).append("\n"); 
		query.append("       , C.OFC_CD AS INV_ISS_OFC_CD" ).append("\n"); 
		query.append("       , ( SELECT NVL(A.LOD_ID, MAX(R.CUST_REF_NO_CTNT)) FROM BKG_REFERENCE R WHERE R.BKG_NO(+) = A.BKG_NO AND R.CNTR_NO(+) = A.EQ_NO AND R.BKG_REF_TP_CD ='MSLD' ) LOD_ID  " ).append("\n"); 
		query.append("FROM     TPB_INV_RVIS_DTL A" ).append("\n"); 
		query.append("       , TPB_INV_RVIS B" ).append("\n"); 
		query.append("       , TPB_INVOICE C" ).append("\n"); 
		query.append("       , TPB_N3RD_PTY_BIL_TP T" ).append("\n"); 
		query.append("       , MDM_CHARGE G" ).append("\n"); 
		query.append("WHERE    1 = 1" ).append("\n"); 
		...
		query.append("AND      A.N3PTY_BIL_TP_CD = T.N3PTY_BIL_TP_CD" ).append("\n"); 
		query.append("AND      T.CHG_CD = G.CHG_CD(+)" ).append("\n"); 
		query.append("AND      A.N3PTY_INV_NO = @[s_n3pty_inv_no]" ).append("\n"); 
		query.append("-- SQL Log Error related logic" ).append("\n"); 
		query.append("#if (${s_n3pty_inv_his_seq} == 'Invoice No.')" ).append("\n"); 
		query.append("AND      A.N3PTY_INV_RVIS_SEQ = ''" ).append("\n"); 
		query.append("#else" ).append("\n"); 

...
public class InlandRouteManageDBDAOInlandRouteSaveAsUsInsert1PsCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("             )                                                           " ).append("\n"); 
		query.append("      VALUES (@[i_rout_org_nod_cd], @[i_rout_dest_nod_cd], TO_NUMBER(@[i_rout_seq])," ).append("\n"); 
		query.append("             NVL(TO_NUMBER(@[next_prio_seq]),0),                                                " ).append("\n"); 
		query.append("              @[i_inv], @[i_rout_pln_cd],  " ).append("\n"); 
		query.append("              @[i_mcntr_rout_flg], @[i_route_rmk], @[cre_ofc_cd], @[cre_usr_id] ,@[upd_usr_id] ," ).append("\n"); 
		query.append("               sysdate, @[i_wrs_fl_cd], @[i_wrs_mt_cd],  " ).append("\n"); 
		query.append("              @[i_bkg_flg] ,@[r_inbound]," ).append("\n"); 

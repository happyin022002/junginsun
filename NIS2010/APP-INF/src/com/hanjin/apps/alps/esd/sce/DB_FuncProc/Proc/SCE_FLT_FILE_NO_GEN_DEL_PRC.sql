CREATE OR REPLACE PROCEDURE SCE_FLT_FILE_NO_GEN_DEL_PRC(NAME IN  CHAR )
AUTHID CURRENT_USER
IS
...
BEGIN
    ...
    V_SYS_NM        := 'SCE';
    V_PROG_NM       := 'SCE_FLT_FILE_NO_GEN';
    V_OWN_NM        := 'NISADM';
    V_ENT_NM        := 'SCE_FLT_FILE_NO_GEN';
    V_TAB_NM        := 'SCE_FLT_FILE_NO_GEN';
    V_INS_ROW_CNT   := 0;
    V_LOG_MSG       := '';
    V_LOG_MSG_MAIN  := '';
    V_ERRNO         := '';
...
LOOP
    ...
    DELETE  FROM SCE_FLT_FILE_NO_GEN WHERE EDI_SND_YRMONDY <= '071231'  AND CRE_DT < TO_DATE ('20080101', 'YYYYMMDD') 
                                            AND ROWNUM <= 10000 ;

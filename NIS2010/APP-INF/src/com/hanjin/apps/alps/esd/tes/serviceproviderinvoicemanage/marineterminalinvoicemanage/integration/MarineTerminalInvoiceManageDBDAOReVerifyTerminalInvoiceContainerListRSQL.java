...
public class MarineTerminalInvoiceManageDBDAOReVerifyTerminalInvoiceContainerListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		, NVL(@[clpt_ind_seq], '1') AS CLPT_IND_SEQ" ).append("\n"); 
		query.append("		, NVL(@[call_yd_ind_seq], '1') AS CALL_YD_IND_SEQ" ).append("\n"); 
		...
		query.append("			   (SELECT B.CNTR_NO, 'Double billing Inv : '||MAX(A.INV_NO) CNTR_RMK" ).append("\n"); 
		...
		query.append("											'O',DECODE(VF.POL_CD,B.POL_CD,DECODE(VF.POL_CD,B.POD_CD,'L','T'),'T'))" ).append("\n"); 
		...
		query.append("																	  DECODE(VF.POL_CD,VF.FDR_POD_CD,'B','F')))" ).append("\n"); 
		...
		query.append("			MDM_LOCATION BR, MDM_LOCATION BD" ).append("\n"); 
		...
		query.append("				VF.SLAN_CD, B.BL_NO, B.RCV_TERM_CD||'/'||B.DE_TERM_CD, M.BKG_NO ,--M.BKG_NO_SPLIT," ).append("\n"); 

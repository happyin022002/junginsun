...
public class CustomerEdiDBDAOSearchEDIPerformanceReportRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("IE AS ( /* EDI_GRP_CUST를 WITH절로 변경, 3회 조회됨 */" ).append("\n"); 
		...
		query.append("                   #set( $sVelocityCount = $sVelocityCount + 1 )   " ).append("\n"); 
		query.append("    	           #end " ).append("\n"); 
		query.append("           THEN TO_CHAR(ACT_DT, 'YYYY/MM/DD HH24:MI:SS')" ).append("\n"); 
		query.append("           END)   $ele1$uline$tmps" ).append("\n"); 
		query.append("  #set($fVelocityCount = $fVelocityCount+1)" ).append("\n"); 
		query.append("  #set($ediStsCount = $ediStsCount + 1)" ).append("\n"); 
		query.append("  #end  /*foreach*/" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("    , TO_CHAR(ROUND(COUNT(S.EDI_STS_CD)/COUNT(S.BKG_NO)*100,1),'990.9')||'%' PFMC" ).append("\n"); 
		query.append("   FROM (" ).append("\n"); 
		query.append("        SELECT /*+ USE_NL(M R) */" ).append("\n"); 
		...
		query.append("    , 'PFMC' RFA_NO" ).append("\n"); 
		query.append("#if(${edi_sts} !='' && ${cust_cd} !='')" ).append("\n"); 
		query.append("  #set($uline = '_V_')" ).append("\n"); 
		query.append("  #set($tmps = '')" ).append("\n"); 
		query.append("  #set($fVelocityCount = 1)" ).append("\n"); 
		...
		query.append("                   #set( $sVelocityCount = $sVelocityCount + 1 )   " ).append("\n"); 
		query.append("    	           #end " ).append("\n"); 
		query.append("           THEN C_PER" ).append("\n"); 
		query.append("           END)   $ele1$uline$tmps" ).append("\n"); 
		query.append("  #set($fVelocityCount = $fVelocityCount+1)" ).append("\n"); 
		query.append("  #set($ediStsCount = $ediStsCount + 1)" ).append("\n"); 
		query.append("  #end  /*foreach*/" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("    , TO_CHAR(ROUND((AVG(B_CNT)/AVG(N_CNT)*100),1),'990.9')||'%' PFMC" ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		query.append("        SELECT /*+ USE_NL(M R) */" ).append("\n"); 
		query.append("          COUNT(R.BKG_NO) B_CNT" ).append("\n"); 
		query.append("        , COUNT(M.CNTR_NO) N_CNT  " ).append("\n"); 
		query.append("        , TO_CHAR(ROUND((COUNT(R.BKG_NO)/COUNT(M.CNTR_NO)*100),1),'990.9')||'%' C_PER" ).append("\n"); 

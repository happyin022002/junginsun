...
*@FileName : ESD_SCE_0080HTMLAction.java
...
public class ESD_SCE_0080HTMLAction  extends HTMLActionSupport{
	
	/** Constructor
     * 
     */
	public ESD_SCE_0080HTMLAction(){
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdSce0080Event event = new EsdSce0080Event();   
        HashMap map = null;
        Collection models;
        FormCommand f_cmd = FormCommand.fromRequest(request);
        
        if (f_cmd.isCommand(FormCommand.SEARCH01)){
        	event.setSearchEstimationListVO((SearchEstimationListVO)getVO(request, SearchEstimationListVO .class));
        } else if(f_cmd.isCommand(FormCommand.SEARCH02)){
        	map = getParameterArray(request); 
        	event = new EsdSce0080Event();
        }
        
        if(f_cmd.isCommand(FormCommand.COMMAND01)){
        	map = getParameterArray(request); 
            models = getParametersArray(request);
            event = new EsdSce0080Event();
//        } else if(f_cmd.isCommand(FormCommand.COMMAND02)){
//        	map = getParameterArray(request); 
//        	event = new EsdSce0080Event();
//        } else if(f_cmd.isCommand(FormCommand.COMMAND03)){
//        	map = getParameterArray(request); 
//        	event = new EsdSce0080Event();
        }

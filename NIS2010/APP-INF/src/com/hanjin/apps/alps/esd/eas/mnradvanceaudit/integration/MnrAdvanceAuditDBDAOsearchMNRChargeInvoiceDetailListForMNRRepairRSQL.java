...
*@FileName : MnrAdvanceAuditDBDAOsearchMNRChargeInvoiceDetailListForMNRRepairRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MnrAdvanceAuditDBDAOsearchMNRChargeInvoiceDetailListForMNRRepairRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * MNR REPAIR에서 상세 항목을 요청하는 경우 아직 I/F가 안된 항목이 있으므로 별도의 SQL을 사용한다.
	  * </pre>
	  */
	public MnrAdvanceAuditDBDAOsearchMNRChargeInvoiceDetailListForMNRRepairRSQL(){
		setQuery();
		...
		query.append("FileName : MnrAdvanceAuditDBDAOsearchMNRChargeInvoiceDetailListForMNRRepairRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("(" ).append("\n"); 
		query.append("    SELECT MNR_COMMON_PKG.MNR_GET_RHQ_OFC_FNC(C.ISS_OFC_CD) RHQ_INV_OFC_CD" ).append("\n"); 
		query.append("         , A.ORD_ISS_OFC_CD WO_OFC_CD" ).append("\n"); 
		...
		query.append("         , C.MNR_PRNR_SEQ VNDR_SEQ" ).append("\n"); 
		...
		query.append("         , MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MAX(C.ISS_DT), 'YYYYMM'), MAX(C.CURR_CD), MAX(A.CURR_CD), SUM(B.INV_AMT)) - SUM(B.COST_AMT) CHG_WO_AMT" ).append("\n"); 
		query.append("         , MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MAX(C.ISS_DT), 'YYYYMM'), MAX(C.CURR_CD), 'USD', SUM(B.INV_AMT)) INV_USD_AMT" ).append("\n"); 
		query.append("         , (" ).append("\n"); 
		...
		query.append("               AND WRK.MNR_PRNR_SEQ = C.MNR_PRNR_SEQ" ).append("\n"); 
		query.append("               AND WRK.INV_NO = C.INV_NO" ).append("\n"); 
		...
		query.append("       AND C.INV_NO = @[s_inv_no]" ).append("\n"); 
		query.append("       AND C.MNR_PRNR_SEQ = @[s_vndr_seq]" ).append("\n"); 

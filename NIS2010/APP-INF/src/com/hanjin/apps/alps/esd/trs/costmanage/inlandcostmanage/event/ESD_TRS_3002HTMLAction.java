 ...
 * @since J2EE 1.6
 */
public class ESD_TRS_3002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_TRS_3002HTMLAction 객체를 생성
	 */
	public ESD_TRS_3002HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsdTrs3002Event event = new EsdTrs3002Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setInlandCostConditionVO((InlandCostConditionVO)getVO(request, InlandCostConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setInlandCostConditionVO((InlandCostConditionVO)getVO(request, InlandCostConditionVO .class));
			event.setInlandCostDetailVOS((InlandCostDetailVO[])getVOs(request, InlandCostDetailVO .class,""));

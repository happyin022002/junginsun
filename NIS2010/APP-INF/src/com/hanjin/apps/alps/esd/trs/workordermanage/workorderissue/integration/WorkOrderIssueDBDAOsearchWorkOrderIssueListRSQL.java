...
public class WorkOrderIssueDBDAOsearchWorkOrderIssueListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	NVL(DECODE(A.CUST_CNT_CD||A.CUST_SEQ, 'XX0', '', A.CUST_CNT_CD||A.CUST_SEQ),TRS_GET_CUST_INFO_FNC(A.BKG_NO)) CUST_CNT_CD_SEQ ," ).append("\n"); 
		...
		query.append("	CASE WHEN NVL(MCNTR_BDL_SEQ,'0') != 0 THEN TO_CHAR(NVL(MTY_BDL_CNTR_QTY,'0')) ELSE '1' END BUNDLING_NO," ).append("\n"); 
		...
		query.append("	DECODE(A.CUST_NOMI_TRKR_FLG, 'Y', 'CNT', 'HJS')      po_sp_type," ).append("\n"); 
		query.append("	A.CUST_NOMI_TRKR_FLG      po_cust_nomi_trkr_flg," ).append("\n"); 
		query.append("	A.CUST_CNT_CD             po_cust_cnt_cd," ).append("\n"); 
		query.append("	A.CUST_SEQ                po_cust_seq," ).append("\n"); 
		query.append("	A.CUST_CNT_CD||CUST_SEQ   po_cust_cnt_cd_seq," ).append("\n"); 
		...
		query.append("	A.SCG_VAT_AMT             po_vat_scg_rt," ).append("\n"); 
		query.append("	''                        po_over_wgt_scg_rt," ).append("\n"); 
		query.append("	A.BZC_AMT+A.FUEL_SCG_AMT+A.nego_amt+A.etc_add_amt     po_local_curr_tot_amt," ).append("\n"); 
		query.append("	round(trs_common_pkg.GET_CONVERSION_USD_AMT_FNC( A.CURR_CD, A.BZC_AMT+A.FUEL_SCG_AMT+A.nego_amt+A.etc_add_amt, to_char(WRK.cre_dt, 'YYYYMM' ) ), 2)  po_usd_curr_tot_amt,  --- 변환" ).append("\n"); 
		query.append("	''                      po_rtn_cd," ).append("\n"); 
		query.append("	''                      po_rtn_msg" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("	, NVL(( SELECT 'Y' " ).append("\n"); 
		query.append("              FROM TRS_TRSP_SVC_ORD BUNDL" ).append("\n"); 
		...
		query.append("		       AND BUNDL.TRSP_SO_SEQ = A.TRSP_SO_SEQ ),'N') bundling_flg" ).append("\n"); 
		...
		query.append("     ,(SELECT BKG.BLCK_STWG_CD FROM BKG_BOOKING BKG WHERE BKG.BKG_NO = NVL(A.BKG_NO,A.BL_NO)) AS BLCK_STWG" ).append("\n"); 
		...
		query.append("	AND A.TRSP_WO_OFC_CTY_CD IS NOT NULL" ).append("\n"); 
		query.append("	AND A.INV_NO IS NULL											" ).append("\n"); 
		query.append("	AND	WRK.TRSP_WO_OFC_CTY_CD	= SUBSTR(@[form_usr_ofc_cd],1,3)" ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("	AND A.TRSP_SO_STS_CD IN ('R','C')" ).append("\n"); 
		query.append("	AND A.TRSP_WO_OFC_CTY_CD IS NULL" ).append("\n"); 
		query.append("	AND A.INV_NO IS NULL		" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${fmdate} != '' && ${todate} != '' && ${dt_radio} != '')  " ).append("\n"); 
		...
		query.append("		AND SUBSTR(A.FM_NOD_CD,0,5) = @[fm_nod]" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		...
		query.append("		AND SUBSTR(A.TO_NOD_CD,0,5) = @[to_nod]" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		...
		query.append("		AND SUBSTR(A.VIA_NOD_CD,0,5) = @[via_nod]" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		...
		query.append("		AND SUBSTR(A.DOR_NOD_CD,0,5) = @[dor_nod]" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		...
		query.append("#if($gp_id.size() > 0 ) " ).append("\n"); 
		query.append("AND A.MCNTR_BDL_GRP_SEQ IN (" ).append("\n"); 
		query.append("	#foreach($code IN ${gp_id})  " ).append("\n"); 
		query.append("		#if($velocityCount == 1)  " ).append("\n"); 
		query.append("			'$code'" ).append("\n"); 
		query.append("		#else  " ).append("\n"); 
		query.append("			 ,'$code'" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${wo_radio} == 'N') " ).append("\n"); 
		query.append("	#if(${cnt_flg} != '')" ).append("\n"); 
		query.append("		#if (${cnt_flg} == 'N') " ).append("\n"); 
		query.append("         AND 0 = (SELECT COUNT(X.PRC_CTRT_NO)" ).append("\n"); 
		query.append("		#elseif (${cnt_flg} == 'Y') " ).append("\n"); 
		query.append("	     AND 0 <> (SELECT COUNT(X.PRC_CTRT_NO)" ).append("\n"); 
		query.append("		#else " ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("        		     FROM TRS_CUST_NOMI_TRKR X" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if($arr_sc_rfa_cd.size() > 0) " ).append("\n"); 
		query.append("   #if (${radio_contract} == 'S' )" ).append("\n"); 
		query.append("     AND A.BKG_NO IN (SELECT X.BKG_NO" ).append("\n"); 
		query.append("                        FROM BKG_BOOKING X" ).append("\n"); 
		query.append("                       WHERE 1=1" ).append("\n"); 
		query.append("                         AND X.SC_NO IN (" ).append("\n"); 
		query.append("                                         #foreach( ${key} in ${arr_sc_rfa_cd}) " ).append("\n"); 
		query.append("                                           #if($velocityCount < $arr_sc_rfa_cd.size()) " ).append("\n"); 
		...
		query.append("                        FROM BKG_BOOKING X" ).append("\n"); 
		query.append("                       WHERE 1=1" ).append("\n"); 
		query.append("                         AND X.RFA_NO IN (" ).append("\n"); 
		query.append("                                         #foreach( ${key} in ${arr_sc_rfa_cd}) " ).append("\n"); 
		query.append("                                           #if($velocityCount < $arr_sc_rfa_cd.size()) " ).append("\n"); 
		query.append("                                             UPPER('${key}'), " ).append("\n"); 
		query.append("                                           #else " ).append("\n"); 

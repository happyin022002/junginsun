CREATE OR REPLACE PROCEDURE SCE_EXCEPTION_RAIL_ETA_PRC
(
       IN_COP_NO              IN  VARCHAR2
      --,IN_COP_GRP_SEQ         IN  VARCHAR2
      ,IN_COP_DTL_SEQ         IN  VARCHAR2
      ,IN_RAIL_ETA_DT         IN  VARCHAR2 
      ,in_prc_flg             IN  VARCHAR2 
)
authid current_user 
IS
/****************************************************************************************************************************
   Name     :   SCE_EXCEPTION_RAIL_ETA_PRC 
   Purpose  :   Rail ETA Delay Exception 판별
   Ver      :   1.0
   Author   :   JSAN
   Date     :   2008.07.21
   Related  :   SCE_ACT_RCV_MVMT_PRC
   -----------------------------------------------------------------------------------------------------------------------
   1. 내용  : Rail Road로부터 수신하는 Rail ETA EDI 정보에 의해, 새롭게 수신한 Rail ETA가 이전 ETA와 비교하여, 
              Tolerance 이상 Delay 될 경우, Exception으로 관리
   2. 판별 Logic : COP에 Rail ETA가 EDI를 통해 Update 될 시, 이전의 Rail ETA와 비교하여 Set-up 정의된 Tolerance를 
              초과할 경우 Exception 판별
   -----------------------------------------------------------------------------------------------------------------------
   Update   :   2008.08.21  JSAN. Container Attach된 COP만 Exception 대상으로 판별하도록 수정
...
CURSOR SEARCH_RAIL_ETA_EXPT_INFO(P_COP_NO      VARCHAR2
                           --,P_COP_GRP_SEQ      VARCHAR2
                           ,P_COP_DTL_SEQ      VARCHAR2
                           ,P_RAIL_ETA_DT      VARCHAR2) IS
SELECT   --Activity
...
FROM   ( SELECT       --Activity
                     ...
                     --EXPT
                     ,A.OCCR_DT
                     ,NUMTODSINTERVAL(TO_DATE(P_RAIL_ETA_DT,'YYYY/MM/DD HH24:MI:SS')-A.FM_ESTM_DT ,'DAY') GAP
...
BEGIN
       ...
       END LOOP;
        ...
        IF(v_chk_expt = 'R1' OR v_chk_expt = 'R2') THEN 
            ...
            SET COP_EXPT_STS_CD = 'R'
               ...
               ,EXPT_RSOLV_DT   = SYSDATE
               ,TO_ACT_CD       = FM_ACT_CD
               ,TO_ESTM_DT      = TO_DATE(IN_RAIL_ETA_DT,'YYYY/MM/DD HH24:MI:SS')
               ,TO_UPD_DT       = SYSDATE
               ,upd_usr_id      = 'EXPT_RLETA'
               ,upd_dt          = SYSDATE
            WHERE  COP_NO      = in_cop_no
       ...
       /* Rail ETA Delay Exception 등록 대상 */
       --FOR CUR_EXPT IN  SEARCH_RAIL_ETA_EXPT_INFO(IN_COP_NO,IN_COP_GRP_SEQ,IN_COP_DTL_SEQ,IN_RAIL_ETA_DT)
       FOR CUR_EXPT IN  SEARCH_RAIL_ETA_EXPT_INFO(IN_COP_NO,IN_COP_DTL_SEQ,IN_RAIL_ETA_DT)
            LOOP
                       ...
                       VALUES (V_COP_EXPT_NO
                             ...
                             ,CUR_EXPT.FM_UPD_DT
                             ,CUR_EXPT.FM_ACT_CD
                             ,TO_DATE(IN_RAIL_ETA_DT,'YYYY/MM/DD HH24:MI:SS')
                             ,''
                             ,SYSDATE
                             ,CUR_EXPT.BL_MST_NO
                             ,CUR_EXPT.SC_NO
...
END SCE_EXCEPTION_RAIL_ETA_PRC;
/

...
//		DBRowSet rs = null;		
		...
		faxEmailIndicator	= event.getFaxEmailIndicator() == null || "".equals(event.getFaxEmailIndicator()) ? null : event.getFaxEmailIndicator();
		selectedVndrSeq		= event.getSelectedVndrSeq() == null || "".equals(event.getSelectedVndrSeq()) ? null : event.getSelectedVndrSeq();
		selectedCtrlOfcCd	= event.getSelectedCtrlOfcCd() == null || "".equals(event.getSelectedCtrlOfcCd()) ? null : event.getSelectedCtrlOfcCd();
		selectedLocCd		= event.getSelectedLocCd() == null || "".equals(event.getSelectedLocCd()) ? null : event.getSelectedLocCd();
		loginUserOfcCd		= event.getLoginUserOfcCd() == null || "".equals(event.getLoginUserOfcCd())	? null : event.getLoginUserOfcCd();
		loginUserId			= event.getLoginUserId() == null || "".equals(event.getLoginUserId()) ? null : event.getLoginUserId();
			...
			for(i=0; i<models.length; i++){		
				...
				if(model.getIbflag().length()>0){
					...
					if(i==0){
						...
						delCnt = new SQLExecuter("DEFAULT").executeUpdate(new WorkOrderCCManageDBDAODeleteFaxDSQL(), param, param);
			...
			for(i=0; i<models.length; i++){	
				...
				if(model.getIbflag().length()>0){
					...
					if(i==0){
						...
						delCnt = new SQLExecuter("DEFAULT").executeUpdate(new WorkOrderCCManageDBDAODeleteEmlDSQL(), param, param);
						if(delCnt == Statement.EXECUTE_FAILED){
							throw new DAOException("Fail to delete SQL");
						}
					}

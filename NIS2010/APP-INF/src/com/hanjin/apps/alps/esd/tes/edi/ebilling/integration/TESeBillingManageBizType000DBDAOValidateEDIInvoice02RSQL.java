...
*@FileName : TESeBillingManageBizType000DBDAOValidateEDIInvoice02RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TESeBillingManageBizType000DBDAOValidateEDIInvoice02RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public TESeBillingManageBizType000DBDAOValidateEDIInvoice02RSQL(){
		setQuery();
		...
		query.append("FileName : TESeBillingManageBizType000DBDAOValidateEDIInvoice02RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        WHEN X.TML_INV_TP_CD IN ('TM') AND X.VVD_CD IS NOT NULL" ).append("\n"); 
		query.append("		THEN " ).append("\n"); 
		query.append("            CASE " ).append("\n"); 
		query.append("            WHEN (  SELECT" ).append("\n"); 
		query.append("					   (SELECT DECODE(L.VSL_SVC_TP_CD,'O',DECODE(NVL(M.CRR_CD,''),'SML','H','C'),'H')" ).append("\n"); 
		query.append("						FROM   VSK_VSL_SKD S, MDM_VSL_SVC_LANE L, MDM_VSL_CNTR M" ).append("\n"); 
		...
		query.append("						AND    S.VSL_CD      = M.VSL_CD(+) )||'|'||TO_CHAR(V.VPS_ETB_DT,'YYYY-MM-DD')||'|'||A.REV_YRMON ATB_DT" ).append("\n"); 

...
*@FileName : BkgCopManageDBDAOCopySceActRcvIfBySplitCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BkgCopManageDBDAOCopySceActRcvIfBySplitCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Split 이전 org bkg + Split 이전 org bkg의 partial bkg list
	  * minus
	  * Split된 new bkg + Split된 new bkg의 partial bkg list
	  * 
	  * 의 Actual data를 new bkg 의 대표 bkg no로 copy한다.
	  * </pre>
	  */
	public BkgCopManageDBDAOCopySceActRcvIfBySplitCSQL(){
		setQuery();
		...
		query.append("FileName : BkgCopManageDBDAOCopySceActRcvIfBySplitCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("DECODE(ACT_RCV_TP_CD, '2', '', @[new_bkg_no]) ," ).append("\n"); 
		query.append("DECODE(ACT_RCV_TP_CD, '2', '', @[cntr_no]) ," ).append("\n"); 
		query.append("ACT_DT ," ).append("\n"); 
		query.append("ACT_STS_MAPG_CD ," ).append("\n"); 
		query.append("NOD_CD ," ).append("\n"); 
		query.append("ACT_RCV_TP_CD ," ).append("\n"); 
		...
		query.append("ACT_GDT ," ).append("\n"); 
		query.append("ACT_DAT_RCV_DT ," ).append("\n"); 
		query.append("DECODE(ACT_RCV_TP_CD, '2', '', @[new_cop_no]) ," ).append("\n"); 
		query.append("FAX_SND_RSLT_FLG ," ).append("\n"); 
		query.append("EML_SND_RSLT_FLG ," ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("        MINUS  /* OLD BKG + OLD PARTIAL BKG에서 발생된 ACTUAL 정보중 NEW BKG +  NEW PARTIAL BKG에 존재하는 ACTUAL 정보를 제외한다. */" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        SELECT A.CNTR_NO, A.ACT_DT, A.ACT_RCV_TP_CD, A.ACT_STS_MAPG_CD, A.NOD_CD FROM SCE_ACT_RCV_IF A," ).append("\n"); 
		query.append("        (SELECT H2.COP_NO, H2.BKG_NO, H2.CNTR_NO FROM SCE_COP_HDR H1, SCE_COP_HDR H2" ).append("\n"); 
		query.append("          WHERE 1=1" ).append("\n"); 
		query.append("            AND H1.BKG_NO = @[new_bkg_no]" ).append("\n"); 
		query.append("            AND H1.CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("            AND H1.TRNK_VSL_CD = H2.TRNK_VSL_CD" ).append("\n"); 
		query.append("            AND H1.TRNK_SKD_VOY_NO = H2.TRNK_SKD_VOY_NO" ).append("\n"); 
		query.append("            AND H1.TRNK_SKD_DIR_CD = H2.TRNK_SKD_DIR_CD" ).append("\n"); 
		query.append("            AND H1.CNTR_NO = H2.CNTR_NO" ).append("\n"); 
		query.append("            AND H2.COP_STS_CD IN ('C', 'T')" ).append("\n"); 
		...
		query.append("     FROM" ).append("\n"); 
		query.append("    SCE_ACT_RCV_IF I," ).append("\n"); 
		query.append("    (   /* OLD BKG +  OLD PARTIAL BKG?? ??? ACTUAL ??? ????. */" ).append("\n"); 
		...
		query.append("        SELECT A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.CLPT_IND_SEQ,  A.ACT_DT, A.ACT_RCV_TP_CD, A.ACT_STS_MAPG_CD, A.NOD_CD FROM SCE_ACT_RCV_IF A, SCE_COP_DTL B," ).append("\n"); 
		query.append("        (SELECT H2.COP_NO, H2.BKG_NO FROM SCE_COP_HDR H1, SCE_COP_HDR H2" ).append("\n"); 
		query.append("          WHERE 1=1" ).append("\n"); 
		query.append("            AND H1.BKG_NO = @[new_bkg_no]" ).append("\n"); 
		query.append("            AND H1.CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("            AND H1.TRNK_VSL_CD = H2.TRNK_VSL_CD" ).append("\n"); 
		...
		query.append("        AND B.COP_DTL_SEQ BETWEEN 4000 AND 6000" ).append("\n"); 
		...
		query.append("        AND A.ACT_RCV_TP_CD = '2'" ).append("\n"); 
		query.append("        AND SUBSTR(A.ACT_STS_MAPG_CD, 3, 1) = SUBSTR(B.ACT_CD, 5, 1)" ).append("\n"); 
		query.append("        AND A.CRE_USR_ID NOT IN ('CrtBy1TimeSplt', 'CrtBy1TimeCmbn')            " ).append("\n"); 
		query.append("    ) K" ).append("\n"); 
		query.append("    WHERE 1=1" ).append("\n"); 

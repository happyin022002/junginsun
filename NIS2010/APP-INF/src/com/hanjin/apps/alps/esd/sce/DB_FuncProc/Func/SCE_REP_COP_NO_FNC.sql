CREATE OR REPLACE FUNCTION SCE_REP_COP_NO_FNC ( v_bkg_no VARCHAR2, v_cntr_tpsz_cd VARCHAR2 )

/*******************************************************************************
   1. Object Name      : SCE_REP_COP_NO_FNC
   2. Version          : 1.0
   ...
   7. Revision History : 2009.09.15 김인수 최초 생성
                         ...
                         2010.04.02 COA에서 cntr_tpsz_cd 에 bkg_qty_dtl 에 지정된 값만 넣으므로 가장 마지막에
                                    COP_NO 추출시 cntr_tpsz_cd 조건 재 추가
...
--1) s/o 존재 여부
OPEN FIND_SO_STS;
FETCH FIND_SO_STS INTO v_tmp_cop_no, v_so_cnt;

IF FIND_SO_STS%FOUND THEN    
    rep_cop_no := v_tmp_cop_no;

    dbms_output.put_line('so found / v_tmp_cop_no = ' || v_tmp_cop_no);
...
-- 2) tro 존재 여부
    ...
    FOR V_SEARCH_TRO IN (
       select cop_no, NVL(ob_tro_flg, 'N') as ob_tro_flg, NVL(ib_tro_flg, 'N') as ib_tro_flg
        from sce_cop_hdr
        ...
        from (
            ...
            from (
                select cop_no
                      ,count(*) as cnt 
                from  sce_cop_hdr
                where bkg_no       = v_bkg_no      --v_bkg_no AARY1140001

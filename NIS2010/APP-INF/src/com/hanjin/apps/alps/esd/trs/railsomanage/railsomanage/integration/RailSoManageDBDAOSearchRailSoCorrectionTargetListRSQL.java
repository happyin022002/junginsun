...
public class RailSoManageDBDAOSearchRailSoCorrectionTargetListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT /*+ ORDERED USE_HASH(TMP AGMT) USE_NL(D MVMT) */" ).append("\n"); 
		...
		query.append("    MAX(SUBSTR(MVMT.ORG_YD_CD, 1, 5)) MVMT_YARD," ).append("\n"); 
		query.append("	MAX(MVMT.MVMT_STS_CD) MVMT_STATUS,  " ).append("\n"); 
		...
		query.append("    MAX(TMP.CXL_RQST_RJCT_RSN) CXL_RQST_RJCT_RSN," ).append("\n"); 
		...
		query.append("	MAX(TMP.CUST_NM) SHIP_NM," ).append("\n"); 
		query.append("    MAX(CASE WHEN TMP.CXL_RQST_FLG = 'N' AND NVL(TMP.CXL_RQST_RJCT_FLG, 'N') = 'Y' AND TMP.DELT_FLG = 'N' THEN 'Cancellation Requested Rejected'" ).append("\n"); 
		query.append("             WHEN TMP.CXL_RQST_FLG = 'Y' AND NVL(TMP.CXL_RQST_RJCT_FLG, 'N') = 'N' AND TMP.DELT_FLG = 'N' THEN 'Cancellation Requested'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'I' AND TMP.DELT_FLG = 'N' AND O.BIL_EDI_RCV_RSLT_CD = 'E' THEN 'EDI Error'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'V' AND TMP.DELT_FLG = 'N' AND O.BIL_EDI_RCV_RSLT_CD = 'A' THEN 'Acknowledged'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'I' AND TMP.DELT_FLG = 'N' AND O.BIL_EDI_RCV_RSLT_CD = 'A' THEN 'Acknowledged'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'I' AND TMP.SPND_FLG = 'N' AND TMP.DELT_FLG = 'N' AND O.BIL_ISS_STS_CD = 'X' THEN 'Rail Billing Cancelled'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'I' AND TMP.SPND_FLG = 'N' AND TMP.DELT_FLG = 'N' THEN 'Rail Billed'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'C' AND TMP.DELT_FLG = 'N' THEN 'Requested'" ).append("\n"); 
		query.append("             WHEN TMP.TRSP_SO_STS_CD = 'R' AND TMP.SPND_FLG = 'N' AND TMP.DELT_FLG = 'N' THEN 'Requested'" ).append("\n"); 
		...
		query.append("		,(" ).append("\n"); 
		query.append("			SELECT" ).append("\n"); 
		query.append("    			MAX(TO_CHAR(SKD.EVNT_DT, 'YYYYMMDD')) || '$' ||" ).append("\n"); 
		query.append("    			MAX(TO_CHAR(SKD.EVNT_DT, 'HH24:MI:SS'))" ).append("\n"); 
		query.append("			FROM" ).append("\n"); 
		query.append("				BKG_DOC_PROC_SKD SKD" ).append("\n"); 
		query.append("			WHERE X.BKG_NO = SKD.BKG_NO" ).append("\n"); 
		query.append("		) BKG_UPD" ).append("\n"); 
		query.append("/*" ).append("\n"); 
		query.append("		,(" ).append("\n"); 
		query.append("			SELECT MAX(C.CUST_NM)" ).append("\n"); 
		query.append("			  FROM BKG_CUSTOMER C" ).append("\n"); 
		query.append("			 WHERE BKG_CUST_TP_CD = 'S'" ).append("\n"); 
		query.append("			   AND X.BKG_NO = C.BKG_NO" ).append("\n"); 
		query.append("		) SHIPPER_NAME" ).append("\n"); 
		query.append("*/" ).append("\n"); 
		query.append("    FROM" ).append("\n"); 
		query.append("        (" ).append("\n"); 
		query.append("            SELECT /*+ ORDERED USE_NL(A A1 B) */" ).append("\n"); 
		...
		query.append("                A.CXL_RQST_RJCT_RSN AS CXL_RQST_RJCT_RSN," ).append("\n"); 
		...
		query.append("                REPLACE(TRANSLATE(NVL(C.CUST_NM, ''), CHR(13)||CHR(10)||CHR(38), ' '), '''', ' ') AS CUST_NM," ).append("\n"); 
		...
		query.append("TRS_TRSP_EDI_RAIL_ORD O" ).append("\n"); 
		...
		query.append("AND   TMP.BKG_NO = D.BKG_NO(+)" ).append("\n"); 
		query.append("AND   TMP.EQ_NO = D.CNTR_NO(+)" ).append("\n"); 
		...
		query.append("AND   TMP.TRSP_SO_OFC_CTY_CD = O.TRSP_SO_OFC_CTY_CD(+)" ).append("\n"); 
		query.append("AND   TMP.TRSP_SO_SEQ = O.TRSP_SO_SEQ(+)" ).append("\n"); 
		query.append("" ).append("\n"); 

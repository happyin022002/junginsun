...
*@FileName : ProductCatalogCreateDBDAOSearchSoSubRoutRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ProductCatalogCreateDBDAOSearchSoSubRoutRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ProductCatalogCreateDBDAOSearchSoSubRoutRSQL(){
		setQuery();
		...
		query.append("FileName : ProductCatalogCreateDBDAOSearchSoSubRoutRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("( " ).append("\n"); 
		query.append("    SELECT RK,RK2,FM_NOD_CD,TO_NOD_CD,MODE_CD,VNDR_SEQ,COMB_MOD,COST_ACT_SEQ, " ).append("\n"); 
		...
		query.append("           END) LAG_SEQ,COP_NO " ).append("\n"); 
		query.append("                        " ).append("\n"); 
		query.append("    FROM (        " ).append("\n"); 
		query.append("        SELECT  " ).append("\n"); 
		query.append("               COP_NO, " ).append("\n"); 
		query.append("               DENSE_RANK() OVER (PARTITION BY COP_NO ORDER BY COST_ACT_GRP_SEQ,SEQ) RK, " ).append("\n"); 
		query.append("               DENSE_RANK() OVER (PARTITION BY COP_NO,COST_ACT_GRP_SEQ ORDER BY SEQ) RK2,                " ).append("\n"); 
		query.append("               FM_NOD_CD,TO_NOD_CD,MODE_CD,VNDR_SEQ,COMB_MOD,COST_ACT_GRP_SEQ COST_ACT_SEQ, " ).append("\n"); 
		query.append("               LAG(TO_NOD_CD,1) OVER(PARTITION BY COP_NO ORDER BY COST_ACT_GRP_SEQ, SEQ) LAG_NOD, " ).append("\n"); 
		query.append("               LAG(COST_ACT_GRP_SEQ,1) OVER (PARTITION BY COP_NO ORDER BY COST_ACT_GRP_SEQ,SEQ) LAG_COST_ACT_GRP_SEQ " ).append("\n"); 
		query.append("        FROM ( " ).append("\n"); 
		query.append("            SELECT COP_NO,SEQ,FM_NOD_CD,TO_NOD_CD,MODE_CD,VNDR_SEQ,COMB_MOD,COST_ACT_GRP_SEQ " ).append("\n"); 
		query.append("            FROM ( " ).append("\n"); 
		...
		query.append("                (                                                                                                 " ).append("\n"); 
		query.append("                CASE F_N0                                                                                         " ).append("\n"); 
		query.append("                    WHEN 0 THEN TO_NUMBER(SUBSTR(ROUT,10,6))    WHEN 1 THEN TO_NUMBER(SUBSTR(ROUT,25,6))   " ).append("\n"); 
		...
		query.append("            SELECT /*+ INDEX(XPKTRS_TRSP_RAIL_BIL_VNDR_SET B) */ COP_NO, " ).append("\n"); 
		...
		query.append("                (CASE WHEN COUNT(B.SUB_RAIL_SEQ) OVER (PARTITION BY B.TRSP_SO_OFC_CTY_CD ,B.TRSP_SO_SEQ) > 1  " ).append("\n"); 
		...
		query.append("              AND A.TRSP_FRST_FLG  <> 'Y'   " ).append("\n"); 
		...
		query.append("               LPAD(ROUT.VNDR_SEQ,6,0)||'@*'||ROUT.COMB_MOD||DECODE(TO_NOD_CD,NULL,NULL,'@*')||   " ).append("\n"); 
		query.append("               ROUT.TO_NOD_CD   " ).append("\n"); 
		query.append("               , '-')   " ).append("\n"); 
		query.append("               ),'@*','-')||'%'   " ).append("\n"); 
		query.append("        ,'-%','%') SUB_ROUT   " ).append("\n"); 

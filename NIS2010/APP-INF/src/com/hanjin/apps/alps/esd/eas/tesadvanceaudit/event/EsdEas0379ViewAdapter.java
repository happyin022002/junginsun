...
*@FileName : EsdEas0379ViewAdapter.java
...
public class EsdEas0379ViewAdapter extends EasDefaultViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix, Event event)
	{
        ...
        if( formcommand.isCommand(FormCommand.SEARCHLIST01) ){
            ...
            for(int i = 0; i < realCnt; i++)
            {
                ...
                String sDscrCtnt = getNull(colValues.get("dscr_ctnt")).toString();
                String sCalcTpCdAud = getNull(colValues.get("calc_tp_cd_aud")).toString();
                ...
                for(int j = 0; j < colCnt; j++) {
                	...
                	if (colName.equals("calc_vol_qty")) {
                		...
                		sbufXML.append((new StringBuilder())
                				...
                				.append(JSPUtil.getNull(getNull(colValues.get(realColNms[j])).toString()))
                				.append("</TD>").toString());
                	}else if (colName.equals("calc_tp_cd")) {
                		sbufXML.append((new StringBuilder())
                				.append("<TD")
                				.append(sCalcTpCdAud.equals("Y")?" COLOR='255, 0, 0'":"")
                				.append(">")
                				.append(JSPUtil.getNull(getNull(colValues.get(realColNms[j])).toString()))
                				.append("</TD>").toString());
                	}else if (colName.equals("dscr_ctnt")) {
                		sbufXML.append((new StringBuilder())
                				.append("<TD")
                				.append(!sDscrCtnt.equals("")?" COLOR='255, 0, 0'":"")
                				.append(">")
                				.append(JSPUtil.getNull(getNull(colValues.get(realColNms[j])).toString()))
                				.append("</TD>").toString());
                	}else{

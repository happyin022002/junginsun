...
public class OndockRailChargeInvoiceManageDBDAOSearchOndockRailChargeCostCalculationListNewRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		 SUM(DECODE(SIGN(TO_TR_VOL_VAL-CAL_VOL),-1,CAL_VOL-TO_TR_VOL_VAL,CAL_VOL)) *" ).append("\n"); 
		query.append("		   DECODE(D.TML_AGMT_VOL_UT_CD,'C',C.AGMT_RT,D.AGMT_UT_RT)            INV_AMT," ).append("\n"); 
		query.append("		 SUM(DECODE(SIGN(TO_TR_VOL_VAL-CAL_VOL),-1,CAL_VOL-TO_TR_VOL_VAL,CAL_VOL)) *" ).append("\n"); 
		query.append("		   DECODE(D.TML_AGMT_VOL_UT_CD,'C',C.AGMT_RT,D.AGMT_UT_RT)            CALC_AMT," ).append("\n"); 
		...
		query.append("  FROM ( SELECT DECODE(L.CNTR_STY_CD,'F',DECODE(SUBSTR(C.COST_CODE,6,1),'F',C.COST_CODE)," ).append("\n"); 
		...
		query.append("		 AND    DECODE(L.CNTR_STY_CD,'F','F','M') = SUBSTR(C.COST_CODE,6,1)" ).append("\n"); 
		...
		query.append("			= DECODE(D.TML_AGMT_VOL_UT_CD,'C',DECODE(I.RC_FLG,'Y',DECODE(SUBSTR(I.TPSZ,1,1),'R','D'||SUBSTR(I.TPSZ,2,1),I.TPSZ),I.TPSZ),'N')" ).append("\n"); 
		...
		query.append("  GROUP BY I.COST_CODE, I.TPSZ, I.IO, I.TML_WRK_DY_CD, I.FM, I.DCGO_CLSS_CD, D.FM_TR_VOL_VAL, D.TO_TR_VOL_VAL," ).append("\n"); 

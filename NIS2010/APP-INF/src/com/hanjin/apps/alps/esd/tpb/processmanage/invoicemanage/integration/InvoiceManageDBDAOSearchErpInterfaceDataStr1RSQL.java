...
public class InvoiceManageDBDAOSearchErpInterfaceDataStr1RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,DECODE(NVL(OTL.N3PTY_TP_CD,'X'), 'R', DTL.VSL_CD, DECODE(OTS.N3PTY_EXPN_TP_CD,'MNR','CNTC',                   DECODE((SELECT COUNT(1) FROM AR_MST_REV_VVD WHERE VSL_CD = DTL.VSL_CD AND SKD_VOY_NO = DTL.SKD_VOY_NO AND SKD_DIR_CD = SUBSTR(DTL.FINC_DIR_CD,1,1) AND DELT_FLG = 'N'),0,'CNTC',DTL.VSL_CD))) AS VSL_CD" ).append("\n"); 
		query.append("      ,DECODE(NVL(OTL.N3PTY_TP_CD,'X'), 'R', DTL.SKD_VOY_NO, DECODE(OTS.N3PTY_EXPN_TP_CD,'MNR',TO_CHAR(SYSDATE,'YYMM'),  DECODE((SELECT COUNT(1) FROM AR_MST_REV_VVD WHERE VSL_CD = DTL.VSL_CD AND SKD_VOY_NO = DTL.SKD_VOY_NO AND SKD_DIR_CD = SUBSTR(DTL.FINC_DIR_CD,1,1) AND DELT_FLG = 'N'),0,TO_CHAR(SYSDATE,'YYMM'),DTL.SKD_VOY_NO))) AS SKD_VOY_NO" ).append("\n"); 
		query.append("      ,DECODE(NVL(OTL.N3PTY_TP_CD,'X'), 'R', DTL.FINC_DIR_CD, DECODE(OTS.N3PTY_EXPN_TP_CD,'MNR','MM',                     DECODE((SELECT COUNT(1) FROM AR_MST_REV_VVD WHERE VSL_CD = DTL.VSL_CD AND SKD_VOY_NO = DTL.SKD_VOY_NO AND SKD_DIR_CD = SUBSTR(DTL.FINC_DIR_CD,1,1) AND DELT_FLG = 'N'),0,'MM',DTL.FINC_DIR_CD))) AS FINC_DIR_CD" ).append("\n"); 
		...
		query.append("                                                                                DECODE( MAX(BT.N3PTY_IF_TP_CD), 'R',MAX(Z.GL_DT), NULL)) GL_DT" ).append("\n"); 
		query.append("                      FROM TPB_OTS_GRP A, TPB_INVOICE X, TPB_INV_RVIS Y, TPB_INV_RVIS_DTL Z, TPB_N3RD_PTY_BIL_TP BT" ).append("\n"); 
		...
		query.append("                       AND Z.N3PTY_BIL_TP_CD = BT.N3PTY_BIL_TP_CD" ).append("\n"); 
		...
		query.append("  FROM TPB_OTS_GRP OTS, TPB_INVOICE INV, TPB_INV_RVIS HDR, TPB_INV_RVIS_DTL DTL, TPB_OTS_DTL OTL" ).append("\n"); 

...
public class OffdockCYInvoiceManageDBDAOVerifyOffdockCYInvoiceVolumeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("			Z.TML_TRNS_MOD_CD" ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		...
		query.append(" 											   P.INV_GATE_IN_DT," ).append("\n"); 
		query.append("                                               P.INV_GATE_OUT_DT" ).append("\n"); 
		...
		query.append("/** 2014-06-17 양양선 부장 요청으로 Off dock invoice에서 Double Billing 구분위해서 TML_COST_GRP_CD 조건에 추가하여 TMNL/Storage별로 D/Billing check **/" ).append("\n"); 
		query.append("#if     (${tml_cost_grp_cd} != '' and ${tml_cost_grp_cd}=='TD') " ).append("\n"); 
		query.append("							      AND    (A.TML_COST_GRP_CD = 'TD' OR A.TML_COST_GRP_CD = 'TP' OR A.TML_COST_GRP_CD = 'SD')" ).append("\n"); 
		query.append("#elseif (${tml_cost_grp_cd} != '' and ${tml_cost_grp_cd}=='TP')" ).append("\n"); 
		query.append("								  AND    (A.TML_COST_GRP_CD = 'TP' OR A.TML_COST_GRP_CD = 'TD')" ).append("\n"); 
		query.append("#elseif (${tml_cost_grp_cd} != '' and ${tml_cost_grp_cd}=='SD')" ).append("\n"); 
		query.append("								  AND    (A.TML_COST_GRP_CD = 'SD' OR A.TML_COST_GRP_CD = 'TD')" ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("								  AND    A.TML_COST_GRP_CD = 'SP'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("						   M3.TML_TRNS_MOD_CD" ).append("\n"); 
		query.append("                    FROM (" ).append("\n"); 
		...
		query.append("								   M2.TML_TRNS_MOD_CD" ).append("\n"); 
		...
		query.append("											MIO.TML_TRNS_MOD_CD" ).append("\n"); 
		query.append("                                  FROM ( -- 업로드된 데이타의 Seq 구분하는 로직 추가 2015.09.17(조아영D 요청)" ).append("\n"); 
		query.append("                    					SELECT  /*+ ORDERED */LAG(M.MVMT_STS_CD)  OVER (PARTITION BY M.CNTR_NO, P.TML_SO_TMP_SEQ ORDER BY M.CNTR_NO, M.CNMV_EVNT_DT, M.CNMV_SEQ) PREV_MVMT_STS," ).append("\n"); 
		query.append("                      							M.MVMT_STS_CD," ).append("\n"); 
		query.append("												-- GATE OUT이 없고 GATE IN만 있는 경우 조건 추가 - 4347-10-22" ).append("\n"); 
		...
		query.append("												M.MVMT_TRSP_MOD_CD TML_TRNS_MOD_CD" ).append("\n"); 
		query.append("                       	                FROM TES_FILE_IMP_TMP P, CTM_MOVEMENT M, BKG_BOOKING B" ).append("\n"); 
		query.append("                       	                WHERE P.TML_SO_OFC_CTY_CD = @[tml_so_ofc_cty_cd]" ).append("\n"); 
		query.append("                         	              AND P.TML_SO_SEQ = @[tml_so_seq]" ).append("\n"); 
		query.append("                      	 	              AND M.CNMV_EVNT_DT BETWEEN (TO_DATE(@[fm_prd_dt], 'YYYY-MM-DD')-730 ) AND (TO_DATE(@[to_prd_dt], 'YYYY-MM-DD'))+ 0.99999421" ).append("\n"); 
		...
		query.append("                                       M2.MI_STS," ).append("\n"); 
		query.append("                                       M2.MO_STS," ).append("\n"); 
		query.append("									   M2.TML_TRNS_MOD_CD) M3) CALC,              " ).append("\n"); 

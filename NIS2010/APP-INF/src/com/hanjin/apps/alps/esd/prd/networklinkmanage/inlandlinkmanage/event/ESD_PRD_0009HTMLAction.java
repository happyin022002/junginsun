 ...
 *@FileName : ESD_PRD_0009HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESD_PRD_0009HTMLAction extends HTMLActionSupport{

	/**
	 * ESD_PRD_0009HTMLAction 객체를 생성
	 */
	public ESD_PRD_0009HTMLAction(){
	}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ESD_PRD_009Event로 파싱하여 request에 셋팅<br>
	 * 
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0009Event event = new EsdPrd0009Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		event.setPrdInlndEachLnkVO((PrdInlndEachLnkVO) getVO(request, PrdInlndEachLnkVO.class));
//		if(command.isCommand(FormCommand.SEARCHLIST)){
//		}else 
		if(command.isCommand(FormCommand.MULTI)){ //검색
			event.setSearchInlandLinkManageListVOs((SearchInlandLinkManageListVO[]) getVOs(request, SearchInlandLinkManageListVO.class, ""));
		}else if(command.isCommand(FormCommand.MODIFY01)){//수정
			event.setPrdInlndEachLnkVOs((PrdInlndEachLnkVO[]) getVOs(request, PrdInlndEachLnkVO.class, ""));
		}

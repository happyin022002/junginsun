...
public class ESD_TRS_0977HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdTrs0977Event event = new EsdTrs0977Event(); // 수정해야함. 꼭 묵
		...
		if (request.getContentType() != null
			...
			try {
				FileUpload fileUpload = new FileUpload(request,
						SubSystemConfigFactory.get("TRS.MODULE.ID"),
						SubSystemConfigFactory.get("COM.ENCODING.EUCKR"), event);
				request = fileUpload.getRequest();
				keyList = (List<String>) request.getAttribute(SiteConfigFactory
						.get("COM.FILE.UPLOAD.KEYS"));
		...
		if (command.isCommand(FormCommand.MULTI)) {
			TrsFileVO[] autoAuditFileVOs = (TrsFileVO[]) getVOs(request,
					TrsFileVO.class);
			...
			if (keyList != null && autoAuditFileVOs != null) {
				...
				for (int i = 0; i < autoAuditFileVOs.length; i++) {
					TrsFileVO vo = autoAuditFileVOs[i];
			...
			event.setTrsFileVOs(autoAuditFileVOs);
		} else {
			event.setTrsFileVO((TrsFileVO) getVO(request, TrsFileVO.class));
		}

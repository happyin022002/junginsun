...
import com.hanjin.apps.alps.esd.trs.workordermanage.workorderissue.integration.WorkOrderIssueDBDAO;
 ...
 * @author poong_yeon
 * @param <WoIssueListVO>
 * @see ESD_TRS_023EventResponse,WorkOrderIssueBC 각 DAO 클래스 참조
 * @since J2EE 1.4
 */
public class WorkOrderIssueBCImpl extends BasicCommandSupport implements WorkOrderIssueBC {
	...
	/**
	 * WorkOrderIssueBCImpl 객체 생성<br>
	 * WorkOrderIssueDBDAO를 생성한다.<br>
	 */
...
//				bil_cs_nm = bil_cs_nm + "|"+rowSet.getString("N3PTY_BIL_TP_NM");
	...
	public EventResponse modifySave(Event e) throws EventException {
		EsdTrs0023Event event=(EsdTrs0023Event)e;
		try {
			ArrayList returnList = dbDao.modifySave(event);
			GeneralEventResponse eventResponse = new GeneralEventResponse();			
			eventResponse.setRsVo(returnList);
			return eventResponse;
		} catch (DAOException de) {

...
public class InvoiceManageDBDAOSearchTPBHandlingRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT   /*+ INDEX(B XAK1TPB_OTS_GRP) */" ).append("\n"); 
		...
		query.append("       , NVL(A.N3PTY_TP_CD,'G') AS  N3PTY_IF_TP_CD" ).append("\n"); 
		...
		query.append("       , CASE WHEN ( SELECT COUNT(1) FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO ) = 0 THEN A.CFM_USR_ID" ).append("\n"); 
		query.append("    	      ELSE ( SELECT CRE_USR_ID FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO AND OTS_STS_SEQ = ( SELECT MAX(OTS_STS_SEQ) FROM TPB_OTS_GRP_STS WHERE N3PTY_NO = B.N3PTY_NO AND OTS_STS_CD = 'T' ) )" ).append("\n"); 
		query.append("       	 END AS CFM_USR_ID" ).append("\n"); 
		query.append("       , CASE WHEN ( SELECT COUNT(1) FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO) = 0 THEN ( SELECT USR_NM FROM COM_USER U WHERE U.USR_ID = A.CFM_USR_ID )" ).append("\n"); 
		query.append("    	      ELSE ( SELECT USR_NM FROM COM_USER U WHERE U.USR_ID = (SELECT CRE_USR_ID FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO AND OTS_STS_SEQ = (SELECT MAX(OTS_STS_SEQ) FROM TPB_OTS_GRP_STS WHERE N3PTY_NO = B.N3PTY_NO AND OTS_STS_CD = 'T' ) ) )" ).append("\n"); 
		query.append("       	 END AS CFM_USR_NM" ).append("\n"); 
		query.append("       , CASE WHEN ( SELECT COUNT(1) FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO ) = 0 THEN TO_CHAR(TPB_GET_LCL_DATE_FNC(B.CFM_DT,@[s_user_ofc_cd]),'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("    	      ELSE TO_CHAR(TPB_GET_LCL_DATE_FNC(( SELECT MAX(CRE_DT) FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO ),@[s_user_ofc_cd]),'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("    	 END AS CFM_DT" ).append("\n"); 
		query.append("       , CASE WHEN ( SELECT COUNT(1) FROM TPB_OTS_GRP_STS WHERE OTS_STS_CD = 'T' AND N3PTY_NO = B.N3PTY_NO ) = 0 THEN TRUNC(SYSDATE - B.CFM_DT)" ).append("\n"); 
		...
		query.append("       , CASE WHEN A.VNDR_CUST_DIV_CD = 'V' THEN 'Y'" ).append("\n"); 
		query.append("              WHEN A.VNDR_CUST_DIV_CD = 'C' THEN 'Y'" ).append("\n"); 
		query.append("              ELSE 'N'" ).append("\n"); 
		query.append("         END AS INVOICE_ABLE" ).append("\n"); 
		...
		query.append("              WHEN B.VNDR_CUST_DIV_CD = 'C' THEN B.CUST_CNT_CD||TO_CHAR(B.CUST_SEQ)" ).append("\n"); 
		query.append("              WHEN B.VNDR_CUST_DIV_CD = 'S' THEN B.N3PTY_OFC_CD" ).append("\n"); 
		query.append("              ELSE ''" ).append("\n"); 
		query.append("         END AS N3PTY_CD_O" ).append("\n"); 
		query.append("       , CASE WHEN ( SELECT COUNT(*) FROM TPB_ADJ_STS WHERE N3PTY_NO = B.N3PTY_NO AND STL_STS_LST_FLG = 'Y' AND N3PTY_STL_TP_CD = 'O' AND STL_APRO_OFC_CD = STL_TO_CLT_CNG_OFC_CD AND B.N3PTY_INV_NO IS NULL ) = 0 THEN 'N' ELSE 'Y' END AS ROCROLLBACK_ABLE" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("#if (${n3pty_if_tp_cd} != '')" ).append("\n"); 
		query.append("AND      A.N3PTY_BIL_TP_CD IN ( SELECT N3PTY_BIL_TP_CD FROM TPB_N3RD_PTY_BIL_TP WHERE N3PTY_IF_TP_CD = @[n3pty_if_tp_cd] )" ).append("\n"); 
		...
		query.append("       , NVL2(R.CLT_DT, ( SELECT NVL(MAX(USR_NM), R.UPD_USR_ID) FROM COM_USER U WHERE U.USR_ID(+) = R.UPD_USR_ID ), '') AS ERPIF_USR_NM" ).append("\n"); 
		...
		query.append("       , 'Y' AS REVISE_ABLE" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${if_flag} == 'N')" ).append("\n"); 
		query.append("AND      R.CLT_DT IS NULL" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${s_n3pty_no} == '' && ${s_n3pty_inv_no_search} == '' && ${s_bkg_no_all} == '' && ${s_bl_no_all} == '')" ).append("\n"); 
		query.append("	#if (${s_status} == 'O')" ).append("\n"); 

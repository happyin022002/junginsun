CREATE OR REPLACE PROCEDURE SCE_CLM_IF_DEL_PRC(NAME IN  CHAR)
AUTHID CURRENT_USER
IS
...
BEGIN
    ...
    MIG_UTL_PURGE.PRC_MIG_PGM_LOG (
        ...
        I_ENT_NM => V_ENT_NM,       I_TAB_NM => 'SCE_CLM_IF',   I_ACT_TP   => 'D',          I_INS_ROW_CNT => 0,         I_LOG_MSG => V_LOG_MSG_MAIN,
...
--        
    ...
    LOOP
        ...
        FORALL I IN VA_CNTR_NO.FIRST..VA_CNTR_NO.LAST
             DELETE SCE_CLM_IF    WHERE CNTR_NO = VA_CNTR_NO(I) AND CNMV_YR = VA_CNMV_YR(I) AND CNMV_ID_NO = VA_CNMV_ID_NO(I) AND CLM_SEQ = VA_CLM_SEQ(I); -- 20
             DBMS_OUTPUT.PUT_LINE('[' ||  SQL%ROWCOUNT  || '] 2 ');
             V_SCE_CLM_IF_CNT := SQL%ROWCOUNT;
            V_INS_SCE_CLM_IF_CNT := V_INS_SCE_CLM_IF_CNT + V_SCE_CLM_IF_CNT;
        COMMIT;        
             -- SUB STEP LOG FINISH
            MIG_UTL_PURGE.PRC_MIG_PGM_LOG_SUB_STEP (
               I_LOG_MODE => 'LOG_FINISH',     I_LOG_SEQ => V_LOG_SEQ2,      I_LOG_SUB_SEQ => V_LOG_SUB_SEQ,  I_SUB_STEP_DESC => V_SUB_STEP_DESC,
               I_PRC_ROW_CNT => V_SCE_CLM_IF_CNT, I_LOG_MSG => V_LOG_MSG,    
               O_LOG_SUB_SEQ => V_LOG_SUB_SEQ, O_ERRNO => V_ERRNO_SUB_STEP, O_ERRMSG => V_ERRMSG  );
...
--               
    ...
    MIG_UTL_PURGE.PRC_MIG_PGM_LOG (
        ...
        I_ENT_NM => V_ENT_NM,       I_TAB_NM =>  'SCE_CLM_IF',   I_INS_ROW_CNT => V_INS_SCE_CLM_IF_CNT, I_LOG_MSG => V_LOG_MSG_MAIN,     I_AUTHOR_NM => V_AUTHOR_NM,

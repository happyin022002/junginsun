CREATE OR REPLACE PROCEDURE SCE_EDI_IF_DTL_DEL_PRC(STARTDATE IN  CHAR, ENDDATE IN CHAR)
IS
...
BEGIN
    ...
    V_SYS_NM        := 'SCE';
    V_PROG_NM       := 'SCE_EDI_IF_DTL';
    V_OWN_NM        := 'NISADM';
    V_ENT_NM        := 'SCE_EDI_IF_DTL';
    V_TAB_NM        := 'SCE_EDI_IF_DTL';
    V_INS_ROW_CNT   := 0;
    V_LOG_MSG       := '';
    V_LOG_MSG_MAIN  := '';
    V_ERRNO         := '';
...
LOOP
    ...
    DELETE  FROM SCE_EDI_IF_DTL WHERE  CRE_DT BETWEEN TO_DATE(STARTDATE, 'YYYYMMDD') AND TO_DATE(ENDDATE,'YYYYMMDD') AND ROWNUM <= 10000;

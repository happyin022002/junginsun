...
public class ESD_TES_0001HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		codeLength5 = request.getParameterValues("vvd_ibflag")!=null?request.getParameterValues("vvd_ibflag").length:0;
		
		if(log.isDebugEnabled())log.debug("\n\n ESD_TES_0001HTMLAction -> codeLength = "+codeLength+"\n\n");
		if(log.isDebugEnabled())log.debug("\n\n ESD_TES_0001HTMLAction -> codeLength2 = "+codeLength2+"\n\n");
		if(log.isDebugEnabled())log.debug("\n\n ESD_TES_0001HTMLAction -> codeLength3 = "+codeLength3+"\n\n");
		if(log.isDebugEnabled())log.debug("\n\n ESD_TES_0001HTMLAction -> codeLength4 = "+codeLength4+"\n\n");
		if(log.isDebugEnabled())log.debug("\n\n ESD_TES_0001HTMLAction -> codeLength5 = "+codeLength5+"\n\n");
...
//			log.debug("[param_name:" + param_name + "] - value:" + request.getParameter(param_name));
		...
		TES_TML_SO_ACCM      	tes_tml_so_accm       		 = TES_TML_SO_ACCM     		 .fromRequest(request);
    	...
    	// 테이블 미사용 제거 주석처리 ( 2009-03-12 )
//        TES_TML_SO_PAY_DYS   	tes_tml_so_pay_dys    		 = TES_TML_SO_PAY_DYS  		 .fromRequest(request);
        TerminalInvoiceVVDList	terminalInvoiceVVDList		 = TerminalInvoiceVVDList	 .fromRequest(request);
		Collection 			 	tes_tml_so_cntr_lists 		 = TES_TML_SO_CNTR_LIST	  	 .fromRequest(request, codeLength2);
		Collection 			 	tes_tml_so_dtls      		 = TES_TML_SO_DTL			 .fromRequest(request, codeLength2);
...
//		Collection 			 	terminal_invoice_n3pty_lists = TerminalInvoiceN3rdPartyIF.fromRequest(request, codeLength4);
		...
		Collection 			 	temp_vvd_lists 			 	 = TerminalInvoiceVVDList.fromRequest(request, codeLength5);

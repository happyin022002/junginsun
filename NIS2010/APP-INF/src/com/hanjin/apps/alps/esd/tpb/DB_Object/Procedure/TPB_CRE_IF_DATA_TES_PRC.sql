CREATE OR REPLACE PROCEDURE NISADM."TPB_CRE_IF_DATA_TES_PRC"
 ...
 /*******************************************************************************
   ...
   7. Revision History : 2008.09.04 Kim Jin-seung   1.0  Created
                         ...
                         2011.05.02 변종건 [CHM-201110061-01] [TPB] TES I/F 게런티 건에 대해서 ofc_cd 누락현상 수정
						             2013.05.15 신자영 [CHM-201324216] [TPB] SINWA TPB OFC 등록
...
BEGIN
    ...
    SELECT v_ots_dtl_seq, TML_IF_SEQ, N3PTY_BIL_TP_CD,
           -- SINWA setting 관련 변경 
           --DECODE(TML_IF_OFC_CD, 'SINWA', 'SINWA', TPB_GET_HNDL_OFC_FNC('R',TPB_GET_N3PTY_OFC_CD_FNC(TML_IF_OFC_CD))),
           DECODE(TML_IF_OFC_CD, 'SINRS', 'SINRS', TPB_GET_HNDL_OFC_FNC('R',TPB_GET_N3PTY_OFC_CD_FNC(TML_IF_OFC_CD))),
           TML_IF_OFC_CD,
           --TPB_GET_N3PTY_OFC_CD_FNC(TML_IF_OFC_CD),
			CASE 
                WHEN TML_IF_OFC_CD ='SINRS' -- 'SINWA'
                THEN 'SINRS' --'SINWA'
                WHEN GNTE_DIV_FLG IS NULL
                THEN
                ...
                ELSE 
                     (SELECT H.OFC_CD 
                        FROM TES_GNTE_HDR H, MDM_ORGANIZATION O 
                       WHERE H.OFC_CD = O.OFC_CD(+) 
                         AND O.REP_CUST_CNT_CD(+) = 'US' 
                         AND O.OFC_KND_CD(+) = '2' 
                         AND H.GNTE_NO = I.INV_NO
                         AND ROWNUM = 1)
			END AS OFC_CD,
           (

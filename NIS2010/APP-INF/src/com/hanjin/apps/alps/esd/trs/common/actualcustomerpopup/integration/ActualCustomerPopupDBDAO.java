...
public class ActualCustomerPopupDBDAO extends DBDAOSupport {
	...
	/**
	 * ActualCustomer의 이름을 가져온다.<br>
	 *
	 * @param event EsdTrs0914Event
	 * @return DBRowSet DB 처리 결과
	 * @throws DAOException
	 */
	public DBRowSet searchActualCustomerName(EsdTrs0914Event event) throws DAOException {
		DBRowSet dRs = null;
...
//			String sContiCd= event.getConti_cd(); //fm_loc_conti_cd -Main화면에서 넘겨 받음.
//			String sCustInfoIndicator	= "A".equals(sContiCd) || "M".equals(sContiCd) ? "XEUR" : "EUR";
			String sCustInfoIndicator			= "XEUR";
			...
			SQLExecuter sqlExe = new SQLExecuter("DEFAULT");	        
			ActualCustomerPopupDBDAOSearchActualCustomerNameRSQL template = new ActualCustomerPopupDBDAOSearchActualCustomerNameRSQL();	        
	        dRs = sqlExe.executeQuery(template, param, param);	

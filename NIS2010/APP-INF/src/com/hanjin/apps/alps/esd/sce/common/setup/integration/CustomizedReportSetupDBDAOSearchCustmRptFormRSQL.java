...
public class CustomizedReportSetupDBDAOSearchCustmRptFormRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT A.RPT_COL_NM COLDESC2, B.RPT_COL_NM COLDESC3-- REPORT SETUP INFO" ).append("\n"); 
		query.append("FROM SCE_USA_INLND_OP_RPT_FOM A, SCE_USA_INLND_OP_RPT_COL B" ).append("\n"); 
		query.append("WHERE A.RPT_USR_ID = @[user_id]" ).append("\n"); 
		query.append("AND A.RPT_COL_NM = B.RPT_COL_DESC" ).append("\n"); 
		query.append("AND A.MOFC_ID = @[user_ofc_cd]" ).append("\n"); 

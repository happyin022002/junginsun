CREATE OR REPLACE PROCEDURE LEA_ESTM_COST_IF_DEL_PRC(NAME IN  CHAR )
AUTHID CURRENT_USER
IS
...
BEGIN
    ...
    V_SYS_NM        := 'LEA';
    V_PROG_NM       := 'LEA_ESTM_COST_IF';
    V_OWN_NM        := 'NISADM';
    V_ENT_NM        := 'LEA_ESTM_COST_IF';
    V_TAB_NM        := 'LEA_ESTM_COST_IF';
    V_INS_ROW_CNT   := 0;
    V_PRC_ROW_CNT  := 0;
    V_LOG_MSG       := '';
    V_LOG_MSG_MAIN  := '';
...
LOOP
    ...
    DELETE  FROM LEA_ESTM_COST_IF WHERE UPD_DT BETWEEN TO_DATE('20090901', 'YYYYMMDD') AND TO_DATE('20091031', 'YYYYMMDD') 
                                        AND ROWNUM <= 10000 ;

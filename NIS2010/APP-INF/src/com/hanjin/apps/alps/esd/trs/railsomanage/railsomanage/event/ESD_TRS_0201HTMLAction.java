...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.esd.trs.railsomanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_TRS_0201HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		sctrlOfcCd				 = "NYCRAO".equals(sctrlOfcCd) ? "PHXSA" : sctrlOfcCd;
		...
		String bkgNoVerify     	 = JSPUtil.getNull(request.getParameter("bkg_no_verify"));
...
//		String eqTpszVerify      = JSPUtil.getNull(request.getParameter("eq_tpsz_verify"));
		...
		if(command.isCommand(FormCommand.SEARCH04)){
			...
			event.setComboSvcProvider(comboSvcProvider);
				...
				|| command.isCommand(FormCommand.SEARCH11)
				|| command.isCommand(FormCommand.MULTI02)
				|| command.isCommand(FormCommand.SEARCH09) ){
			event.setTrsTrspRailBilOrdVos((TrsTrspRailBilOrdVO[])getVOs(request, TrsTrspRailBilOrdVO.class, ""));
			event.setSplanFromDate(splanFromDate);
			event.setSplanToDate(splanToDate);
			event.setSfromLocationCd(sfromLocationCd);
			...
			event.setEqNoVerify(eqNoVerify);
			event.setToNodVerify(toNodVerify);
			event.setBkgNoVerify(bkgNoVerify);
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setTrsTrspRailBilOrdVos((TrsTrspRailBilOrdVO[])getVOs(request, TrsTrspRailBilOrdVO.class, ""));
			event.setSctrlOfcCd(sctrlOfcCd);
		}else if(command.isCommand(FormCommand.MULTI03)||command.isCommand(FormCommand.REMOVE01)) {
			SearchRailSoManageHdrVO hdrVo = new SearchRailSoManageHdrVO();
			SearchRailSoManageHdrVO[] searchRailSoManageHdrVos = null;
			searchRailSoManageHdrVos = hdrVo.fromRequestGrid(request);
			event.setSearchRailSoManageHdrVos(searchRailSoManageHdrVos);
			...
			//S/O no delete 체크 여부를 위한 parameter event setting
			String seq = request.getParameter("fTrspSoSeq");
			String ofc = request.getParameter("fTrspSoOfcCd");
			event.setFTrspSoSeq(seq);
			event.setFTrspSoOfcCd(ofc);
		
		}else {
			// OTHER
		}
		
		// EsdTrs0201Event PARAMETERS Return To JSP--------------------------

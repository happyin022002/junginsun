...
*@FileName : CSRIssueTransferSlipManageEAIDAO.java
 ...
 * @since J2EE 1.4
 */
public class CSRIssueTransferSlipManageEAIDAO extends EAIDAOSupport {
	 ...
	 */
	public FNS0080003Document[] transferAtOnceTRS024ToEAIByWS(DBRowSet rowSet, String csr_no, ComAproRqstRoutVO rout) throws DAOException, EventException {
		...
		try {
			...
			log.debug("\n\n TRS CSR Approval request [USING CSR_NO :"+csr_no+"]\n\n");
            ...
            log.error("\n TRS FNS0080003Document[] : CSR_NO:" + JSPUtil.getNull(csr_no) + " - docReq.length:"+docReq.length+" (row_cnt:"+rsCnt+")   <<<<\n");
            /*** 
			...
			if(rowSet != null) {
				...
				log.debug("\n ===========> REQUEST START ================>\n");
				String agmtCfmCd = JSPUtil.getNull(new ApprovalUtil().searchAgmtCfmCd(csr_no));
				...
				for( int i=0; i<docReq.length; i++){
					...
					headerType.setInstanceId("FNS008-"+ (new SimpleDateFormat("yyMMddHHmm")).format(new Date())+"TRS"+JSPUtil.getNull(csr_no)+"("+JSPUtil.getLPAD(String.valueOf(i+1),2,"0")+"/"+JSPUtil.getLPAD(String.valueOf(docReq.length),2,"0")+")");
					...
					while (cnt <= 100 && rowSet.next()) {
			            ...
			            setInvoiceDatas(rowSet, invoiceReq, rout, agmtCfmCd);
			            cnt++;
					}
					
					log.error("\n ### CSR_NO:"+csr_no+"  /  docReq["+i+"].instanceId:"+headerType.getInstanceId()+
	 ...
	 * @throws Exception
	 */
	private void setInvoiceDatas(DBRowSet rowSet, APInvoiceRequest invoiceReq, ComAproRqstRoutVO rout, String agmtCfmCd) throws Exception {
	      /**
		...
		try {
			if (rowSet != null) {
				...
				log.error(">>>> CSRIssueTransferSlipManageEAIDAO setInvoiceDatas HDR_CSR_NO : " +(String)rowSet.getString("HDR_CSR_NO")+"<<<<<<<<<");
...
//				invoiceReq.setHTERMSDATE				((String)rowSet.getString("HDR_INV_TERM_DT"						));
				invoiceReq.setHTERMSDATE				((String)rowSet.getString("HDR_INV_DT"							));
				...
				log.error(">>>> CSRIssueTransferSlipManageEAIDAO setInvoiceDatas CSR_NO : " +(String)rowSet.getString("CSR_NO")+"<<<<<<<<<");
				log.error(">>>> CSRIssueTransferSlipManageEAIDAO setInvoiceDatas ATTRIBUTE1 : " +(rout != null ? (JSPUtil.getNull(rout.getAproUsrJbTitNm())+"/"+JSPUtil.getNull(rout.getAproUsrNm())) : "")+"<<<<<<<<<");
				invoiceReq.setHATTRIBUTE1				(rout != null ? (JSPUtil.getNull(rout.getAproUsrJbTitNm())+"/"+JSPUtil.getNull(rout.getAproUsrNm())) : "");
				...
				log.error(">>>>>>> CSRIssueTransferSlipManageEAIDAO setInvoiceDatas HINTERFACEDATE : "+ rowSet.getString("HDR_IF_DT")+ "<<<<<<<");
				...
				//Agreement 존재여부 Flag 추가.
				invoiceReq.setHREFERENCEKEY1(JSPUtil.getNull(agmtCfmCd));
				log.error(">>>> CSRIssueTransferSlipManageEAIDAO setInvoiceDatas HREFERENCEKEY1 : " +JSPUtil.getNull(agmtCfmCd)+"<<<<<<<<<");
			}
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw new EventException(e.getMessage());

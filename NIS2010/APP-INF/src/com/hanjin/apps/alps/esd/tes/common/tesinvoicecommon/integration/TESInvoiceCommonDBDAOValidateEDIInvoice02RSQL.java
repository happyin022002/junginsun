...
*@FileName : TESInvoiceCommonDBDAOValidateEDIInvoice02RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TESInvoiceCommonDBDAOValidateEDIInvoice02RSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ValidateEDIInvoice02
	  * </pre>
	  */
	public TESInvoiceCommonDBDAOValidateEDIInvoice02RSQL(){
		setQuery();
		...
		query.append("FileName : TESInvoiceCommonDBDAOValidateEDIInvoice02RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("		CASE WHEN ((X.VNDR_SEQ IN ('158002','114776') AND SUBSTR(X.TML_INV_TP_CD,1,1) = 'M' AND X.IO_IND_CD IS NOT NULL AND X.IO_IND_CD IN('I','O','B'))" ).append("\n"); 
		query.append("				OR (X.VNDR_SEQ IN ('180020','186666','176307') AND X.TML_INV_TP_CD IN ('TM')))" ).append("\n"); 
		...
		query.append("    -- B - 2009-03-16: CNTC항차 VALIDATION 기준 변경 - REV_VVD MASTER에만 있으면 일단 통과" ).append("\n"); 
		...
		query.append("    -- E - 2009-03-16: CNTC항차 VALIDATION 기준 변경 - REV_VVD MASTER에만 있으면 일단 통과" ).append("\n"); 
		...
		query.append("        UNION" ).append("\n"); 
		query.append("        SELECT DISTINCT H.TML_INV_TP_CD, H.VNDR_SEQ, H.YD_CD, H.IO_IND_CD, D.VSL_CD||D.SKD_VOY_NO||D.SKD_DIR_CD VVD_CD" ).append("\n"); 
		query.append("        FROM TES_EDI_SO_HDR H, TES_EDI_SO_CNTR_LIST D" ).append("\n"); 
		query.append("        WHERE 1 = 1" ).append("\n"); 
		query.append("          AND H.TML_EDI_SO_OFC_CTY_CD = @[tml_edi_so_ofc_cty_cd]" ).append("\n"); 
		query.append("          AND H.TML_EDI_SO_SEQ = @[tml_edi_so_seq]" ).append("\n"); 
		...
		query.append("          AND H.TML_EDI_SO_SEQ = D.TML_EDI_SO_SEQ" ).append("\n"); 
		query.append("          AND H.TML_INV_TP_CD IS NOT NULL" ).append("\n"); 
		query.append("          AND H.VNDR_SEQ IN ('180020','186666','176307') AND H.TML_INV_TP_CD IN ('TM') --// 2011-06-02: 일반적인 eBilling일 경우 처리 - 현재는 HJNC/HJGT만 대상" ).append("\n"); 

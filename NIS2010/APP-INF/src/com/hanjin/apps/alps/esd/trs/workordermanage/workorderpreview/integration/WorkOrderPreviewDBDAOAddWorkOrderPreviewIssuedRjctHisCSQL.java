...
public class WorkOrderPreviewDBDAOAddWorkOrderPreviewIssuedRjctHisCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    , GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC(@[cre_ofc_cd])" ).append("\n"); 
		query.append("    , COMMCODE_PKG.GET_COMDTL_NAME_FNC('CD00957',TMP.TRSP_RJCT_RSN_CD)" ).append("\n"); 
		query.append("--'Power Shortage by S/P'" ).append("\n"); 
		...
		query.append("    AND TMP.TRSP_RJCT_RSN_CD IN ('R', 'I')" ).append("\n"); 
		query.append("    AND 0 = (SELECT COUNT(1) FROM TRS_TRSP_WRK_ORD_RJCT_HIS WHERE 1=1 AND TRSP_SO_OFC_CTY_CD = TMP.TRSP_SO_OFC_CTY_CD AND TRSP_SO_SEQ = TMP.TRSP_SO_SEQ AND TRSP_WO_OFC_CTY_CD = NVL(A.TRSP_WO_OFC_CTY_CD, TMP.TRSP_WO_OFC_CTY_CD) AND TRSP_WO_SEQ = NVL(A.TRSP_WO_SEQ, TMP.TRSP_WO_SEQ))" ).append("\n"); 

...
*@FileName : USATruckEdiWoAckManageDBDAOsearchWorkOrderDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class USATruckEdiWoAckManageDBDAOsearchWorkOrderDetailRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public USATruckEdiWoAckManageDBDAOsearchWorkOrderDetailRSQL(){
		setQuery();
		...
		query.append("FileName : USATruckEdiWoAckManageDBDAOsearchWorkOrderDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       to_char(so.edi_snd_dt,'YYYY-MM-DD') as edi_snd_dt ," ).append("\n"); 
		query.append("       wo.wo_vndr_seq ," ).append("\n"); 
		query.append("       mv.vndr_lgl_eng_nm as vndr_nm ," ).append("\n"); 
		query.append("       mv.phn_no ," ).append("\n"); 
		query.append("       mv.vndr_eml ," ).append("\n"); 
		query.append("      (select usr_eml" ).append("\n"); 
		query.append("         from com_user" ).append("\n"); 
		query.append("        where usr_id = (select cre_usr_id from trs_trsp_wrk_ord_his where (trsp_wo_ofc_cty_cd, trsp_wo_seq) in ((wo.trsp_wo_ofc_cty_cd, wo.trsp_wo_seq)) and rownum =1)) as usr_eml" ).append("\n"); 
		...
		query.append("          s.edi_snd_dt" ).append("\n"); 
		...
		query.append("      (" ).append("\n"); 
		query.append("       SELECT V1.VNDR_SEQ" ).append("\n"); 
		query.append("            , V1.VNDR_LGL_ENG_NM" ).append("\n"); 
		query.append("            , (SELECT V2.INTL_PHN_NO || PHN_NO" ).append("\n"); 
		query.append("                 FROM MDM_VNDR_CNTC_PNT V2" ).append("\n"); 
		query.append("                WHERE V1.VNDR_SEQ = V2.VNDR_SEQ " ).append("\n"); 
		query.append("                  AND V2.PRMRY_CHK_FLG = 'Y'  " ).append("\n"); 
		query.append("                  AND V2.CNTC_DIV_CD = 'PHN') AS PHN_NO " ).append("\n"); 
		query.append("            , (SELECT V2.VNDR_EML" ).append("\n"); 
		query.append("                 FROM MDM_VNDR_CNTC_PNT V2" ).append("\n"); 
		query.append("                WHERE V1.VNDR_SEQ = V2.VNDR_SEQ " ).append("\n"); 
		query.append("                  AND V2.PRMRY_CHK_FLG = 'Y'  " ).append("\n"); 
		query.append("                  AND V2.CNTC_DIV_CD = 'EMAIL') AS VNDR_EML" ).append("\n"); 
		query.append("         FROM MDM_VENDOR V1" ).append("\n"); 
		query.append("        WHERE V1.DELT_FLG = 'N' " ).append("\n"); 

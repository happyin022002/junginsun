...
public class OffdockCYInvoiceManageDBDAOCalOffdockCYInvoiceCostByDayRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("								   FF.TML_SO_CNTR_LIST_SEQ" ).append("\n"); 
		query.append("							FROM (" ).append("\n"); 
		...
		query.append("										F.TML_SO_CNTR_LIST_SEQ" ).append("\n"); 
		query.append("								FROM (" ).append("\n"); 
		query.append("									SELECT /*+ ORDERED */I.COST_CODE COST_CODE," ).append("\n"); 
		...
		query.append("--										DECODE([sto_dys_ind_cd], 'IO', TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'), DECODE(I.GO, NULL, TO_DATE([to_prd_dt], 'YYYYMMDD'), TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'))) - TO_DATE(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDD'), 1, 8), 'YYYYMMDD') + DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI,'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1)) STAY_DAY," ).append("\n"); 
		query.append("									   DECODE(@[sto_dys_ind_cd], 'IO', TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'), DECODE(I.GO, NULL, TO_DATE(@[to_prd_dt], 'YYYYMMDD'), DECODE(SIGN(TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD') - I.GO), 1, TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'), TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD')))) " ).append("\n"); 
		query.append("									   - TO_DATE(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDD'), 1, 8), 'YYYYMMDD') " ).append("\n"); 
		query.append("									   + DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1)) AS STAY_DAY, -- SP 기준일경우 TO PRD DATE 기준으로 적용 변경 (2015-03-10)" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("--										DECODE([sto_dys_ind_cd], 'IO', I.GO, DECODE(I.GO, NULL, TO_DATE(replace([to_prd_dt], '-'), 'YYYYMMDD'), I.GO)) GIO_O," ).append("\n"); 
		query.append("										DECODE(@[sto_dys_ind_cd], 'IO', I.GO, DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), DECODE(SIGN(TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD') - I.GO), 1, I.GO,TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD')))) GIO_O, -- SP 기준일경우 TO PRD DATE 기준으로 적용 변경 (2015-03-10)" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                                        DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), DECODE(SIGN(TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD') - I.GO), 1, I.GO,TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'))) INV_DT_O, -- SP 기준일경우 TO PRD DATE 기준으로 적용 변경 (2015-03-10)												   " ).append("\n"); 
		...
		query.append("--										DECODE(I.GO, NULL, TO_DATE([to_prd_dt], 'YYYYMMDD'), TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD')) - TO_DATE(SUBSTR(TO_CHAR(TO_DATE([fm_prd_dt], 'YYYYMMDD'), 'YYYYMMDD'), 1, 8), 'YYYYMMDD') + DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI,'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1)) STAY_DAY2," ).append("\n"); 
		query.append("										DECODE(I.GO, NULL, TO_DATE(@[to_prd_dt], 'YYYYMMDD'), DECODE(SIGN(TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD') - I.GO), 1, TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'), TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'))) " ).append("\n"); 
		query.append("										- TO_DATE(SUBSTR(TO_CHAR(TO_DATE(@[fm_prd_dt], 'YYYYMMDD'), 'YYYYMMDD'), 1, 8), 'YYYYMMDD') " ).append("\n"); 
		query.append("										+ DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1))  AS STAY_DAY2, -- SP 기준일경우 TO PRD DATE 기준으로 적용 변경 (2015-03-10)" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("										I.TML_SO_CNTR_LIST_SEQ," ).append("\n"); 
		query.append("										D.CURR_CD CURR," ).append("\n"); 
		query.append("										I.RC_FLG" ).append("\n"); 
		query.append("									FROM (" ).append("\n"); 
		query.append("										SELECT DECODE(CNTR_STY_CD, 'F', DECODE(SUBSTR(C.COST_CODE, 5, 2), 'FL', C.COST_CODE), DECODE(SUBSTR(C.COST_CODE, 5, 2), 'MT', C.COST_CODE)) COST_CODE," ).append("\n"); 
		...
		query.append("									  AND DECODE(RD.DCGO_APLY_TP_CD, 'S', DECODE(I.DG, '9', R.DCGO_N9TH_CLSS_FLG, 'N'), 'N') = DECODE(RD.DCGO_APLY_TP_CD, 'S', DECODE(I.DG, '9', 'Y', 'N'), 'N') ) F )FF ) ) " ).append("\n"); 

...
* 2011.09.22 김영철 [] Customer Code가 'SAMSUNG_ELEC'인 경우에는 Lane 별, 나머지는 Lane, Port별로 구분되도록 수정요청
=========================================================*/
...
import com.hanjin.apps.alps.esd.sce.emailjobmanage.vskemailsend.integration.VskEmailSendDBDAO;
...
public class VskEmailSendBCImpl extends BasicCommandSupport implements VskEmailSendBC {
	 ...
	 * @see com.hanjin.apps.alps.esd.sce.emailjobmanage.vskemailsend.basic.VskEmailSendBC#sendVslSkdEmail(java.lang.String)
	 */
	public void sendVslSkdEmail(String eml_jb_id) throws EventException {
		...
		try {
			...
			for(int i = 0; i < vslSkdEmlVo.size(); i++){	
				// Customer Code가 'SAMSUNG_ELEC'가 아닌경우에는 Port별로 표를 만들기 위함.
				if( i != 0 && !"SAMSUNG_ELEC".equals(vslSkdEmlVo.get(i).getEmlGrpId()) &&
					...
					vslSkdEmlVo.get(i-1).getToPortCd().equals(vslSkdEmlVo.get(i).getToPortCd())){
					...
					// Customer Code가 'SAMSUNG_ELEC'인 경우에는 Lane 별로 만들기 위함.
				}else if( i != 0 && "SAMSUNG_ELEC".equals(vslSkdEmlVo.get(i).getEmlGrpId()) &&
						vslSkdEmlVo.get(i-1).getEmlGrpId().equals(vslSkdEmlVo.get(i).getEmlGrpId()) &&
						vslSkdEmlVo.get(i-1).getSubscEml().equals(vslSkdEmlVo.get(i).getSubscEml()) &&
						vslSkdEmlVo.get(i-1).getVslSlanCd().equals(vslSkdEmlVo.get(i).getVslSlanCd())){

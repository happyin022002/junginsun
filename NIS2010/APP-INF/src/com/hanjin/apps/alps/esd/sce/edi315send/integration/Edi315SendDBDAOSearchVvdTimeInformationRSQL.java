...
public class Edi315SendDBDAOSearchVvdTimeInformationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      NVL(decode(D.CALL_SGN_NO, 'T.B.N.', 'TBN', 'T.B.N', 'TBN', D.CALL_SGN_NO), ' ') VSL_CALLSIGN1," ).append("\n"); 
		query.append("      NVL(decode(D.LLOYD_NO, 'T.B.N.', 'TBN', 'T.B.N', 'TBN', D.LLOYD_NO), ' ') VSL_LLOYDCODE1," ).append("\n"); 
		...
		query.append("	  AND b.CLPT_IND_SEQ(+) = POL_CLPT_IND_SEQ -- [CHM-201535084] IKEA (TP ID: IKEA.EBCCNS1) ETD 로직 보완 요청" ).append("\n"); 
		query.append("      AND a.vsl_cd = c.vsl_cd(+)" ).append("\n"); 
		query.append("      AND a.skd_voy_no = c.skd_voy_no(+)" ).append("\n"); 
		query.append("      AND a.skd_dir_cd = c.skd_dir_cd(+)" ).append("\n"); 
		query.append("      AND a.pod_cd = c.vps_port_cd(+)" ).append("\n"); 
		query.append("--      AND c.CLPT_IND_SEQ(+) = '1'" ).append("\n"); 
		query.append("      AND c.CLPT_IND_SEQ(+) = POD_CLPT_IND_SEQ -- [CHM-201535084] IKEA (TP ID: IKEA.EBCCNS1) ETD 로직 보완 요청" ).append("\n"); 

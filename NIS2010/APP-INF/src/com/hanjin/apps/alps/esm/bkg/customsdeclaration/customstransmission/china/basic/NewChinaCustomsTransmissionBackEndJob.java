 ...
 * @since J2EE 1.4
 */
public class NewChinaCustomsTransmissionBackEndJob extends BackEndCommandSupport {
	...
	public String doStart() throws Exception {
		/***********************************************************
		 * New China FlatFile 생성
		 ***********************************************************/
		log.debug("New China FlatFile 생성======>");
	...
	public String transmitManifest() throws EventException {
		...
		ChinaCustmsAgnVO chinaCustmsAgnVO = null;
		...
		try {
			...
			List<ChinaCustmsAgnVO> chinaCustmsAgnVOs= dbDao.searchCustmsChnAgn(transKeyVOs);
						
			if(chinaCustmsAgnVOs.size() < 1){
				log.debug("chinaCustmsAgnVOs.size()===>"+chinaCustmsAgnVOs.size());
				throw new EventException(new ErrorHandler("BKG40110", new String[] {"Please Chinese Agent Set-up(China 24hr Manifest)!"}).getMessage());
			}
			log.debug("chinaCustmsAgnVOs===>"+chinaCustmsAgnVOs.toString());
			if(chinaCustmsAgnVOs.size() > 0){
				for(int i=0 ; i < chinaCustmsAgnVOs.size() ; i++){
					chinaCustmsAgnVO = chinaCustmsAgnVOs.get(i);
					//if(polList != null){
					...
					List<ChinaCncusBlListVO> chinaCncusBlListVOs = dbDao.searchCncusBlAgn ( transKeyVOs , chinaCustmsAgnVO.getPodCd() );
					strFlatFile = new String[chinaCncusBlListVOs.size()];
					for ( int j=0; j < chinaCncusBlListVOs.size(); j++ ) {
						log.debug("\n Cycle_Num=====>"+ i + j);
						log.debug("\n Cycle_Num=====>"+ i + j);
						//strFlatFile[j] = makeChinaManifestTransmitFlatFile(chinaCustmsAgnVO, transKeyVOs,chinaCncusBlListVOs.get(j) ,pol);
						strFlatFile[j] = makeChinaManifestTransmitFlatFile(chinaCustmsAgnVO, transKeyVOs,chinaCncusBlListVOs.get(j) ,polList.get(0));
						flatFile.append(strFlatFile[j]);

						/*==============================================================*/
						/* Queue 전송													*/
						/*==============================================================*/
						SendFlatFileVO sendFlatFileVO = new SendFlatFileVO();
						sendFlatFileVO.setFlatFile(strFlatFile[j]);
						...
						{		
							...
							sendFlatFileVO1.setFlatFile(strFlatFile[j].replaceFirst("CHNCUS", "SHACUS"));
						...
						{		
							SendFlatFileVO sendFlatFileVO1 = new SendFlatFileVO();
							sendFlatFileVO1.setFlatFile(strFlatFile[j].replaceFirst("CHNCUS", "NBOCUS"));
							sendFlatFileVO1.setQueueNm(SubSystemConfigFactory.get("BKG.ALPSBKG_UBIZHJS_CNCUS.IBMMQ.QUEUE"));
							flatFileAckVO1 = new BookingUtil().sendFlatFile(sendFlatFileVO1);
						}
	 ...
	 * 
	 * @param ChinaCustmsAgnVO chinaCustmsAgnVO
	 * @param List<TransKeyVO> transKeyVOs 
	 * @param ChinaCncusBlListVO chinaCncusBlListVO
	 * @param String pol 
	 * @return String
	 * @exception EventException
	 */
	private String makeChinaManifestTransmitFlatFile( ChinaCustmsAgnVO chinaCustmsAgnVO, List<TransKeyVO> transKeyVOs,ChinaCncusBlListVO chinaCncusBlListVO, String pol ) throws EventException {
		...
		try {
			...
			String header = command.searchCstmsEdiHeader("SML", chinaCustmsAgnVO.getEdiRcvId().toUpperCase(), "IFCSUM");
...
//			transKeyVO.setSendDate(sendDate);
			...
			blNo = chinaCncusBlListVO.getBlNo();
			blData = chinaCncusBlListVO.getBlData().split("\t");
			podRout = blData[32].split(";");
			
			String markStr = (blData[34] + blData[35] + blData[36] + blData[37] + blData[38]).trim();

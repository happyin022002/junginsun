 ...
 *@FileName : ESM_BKG_N001HTMLAction.java
 *@FileTitle : ESM_BKG_N001HTMLAction
 *Open Issues :
 *Change history :
 *@LastModifyDate :
 *@LastModifier :
 ...
 *
 * @author
 * @see EsmBkg1528Event 참조
 * @since J2EE 1.6
 */
public class ESM_BKG_N001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_N001HTMLAction 객체를 생성
	 */
	public ESM_BKG_N001HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkgN001Event event = new EsmBkgN001Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			ManifestListCondVO condVO = (ManifestListCondVO)getVO(request, CndCstmsMfListCondVO.class);
			condVO.setPgmNo("ESM_BKG_N001");
			event.setManifestListCondVO(condVO);

		} else if(command.isCommand(FormCommand.ADD)||command.isCommand(FormCommand.MULTI05)) {
			CndManifestModificationVO[] detailVOs = (CndManifestModificationVO[])getVOs(request, CndManifestModificationVO.class);
			detailVOs[0].setPgmNo("ESM_BKG_N001");
			event.setManifestListDetailVOs(detailVOs);

		} else if(command.isCommand(FormCommand.MULTI)) {
			UsaManifestListDetailVO[] detailVOs = new UsaManifestListDetailVO[1];
			detailVOs[0].setUsaDownloadSummaryVOs((UsaDownloadSummaryVO[])getVOs(request, UsaDownloadSummaryVO.class));
			detailVOs[0].setPgmNo("ESM_BKG_N001");
			event.setManifestListDetailVOs(detailVOs);

...
* 2010.09.01 김기종 CSR No. CHM-201004982-01 COA Architecture 위배사항 수정 (CommonSC)                                                                        ->SQLException 삭제하고 Exception 추가
...
* 2011.01.26 최윤성 [CHM-201108537-01] ESM_COA_0037 화면에서 사용하는 VslOwner Code 추가
* 2011.06.22 김민아 [CHM-201111640-01] ESM_COA_0060 화면에서 사용하는 RF Core Acct Code 추가
 ...
 * 
 * @author eunju park 
 * @see ESM_COA_051EventResponse,LaneSimulationBC 각 DAO 클래스 참조
 * @since J2EE 1.4
 */
public class CommonBCImpl extends BasicCommandSupport implements CommonBC {
	...
	public List<GetCodeSelectVO> getCodeSelectList(String codeItem, String code) throws EventException {
    	...
    	DBRowSet dRs = null;
	        try {	        	
        	    ...
        	    }else if(codeItem.equalsIgnoreCase("totCoaCode")){
        	    	dRs = dbDao.searchTotCoaCodeList();

       	    	 //2009.12.24 추가( ESM_COA_174.JSP화면에서 사용)
        	    }else if(codeItem.equalsIgnoreCase("avgSubGrp")){		  	   // 70. Average SUB GROUP코드,이름을 가져온다.
	        	   dRs = dbDao.searchAvgSubGrpList();
        	    }else if(codeItem.equalsIgnoreCase("avgCoaCode")){		  	   // 69. Average COA코드,이름을 가져온다.
        	    	dRs = dbDao.searchAvgCoaCodeList(code);
            	//2009.12.18[CHM-200901902] ESM_COA_5113.JSP화면에서 사용[ESM_COA_0078]
        	    ...
        	    // 2011.01.26 최윤성 [CHM-201108537-01] ESM_COA_0037 화면에서 사용  
        	    ...
        	    }else if(codeItem.equalsIgnoreCase("costSrcAcctGrpCd")){		//COA_COST_SRC_ACCT 의 그룹별 코드 리스트
   ...
   /**
	...
	public String getUserLevel(String ofc_cd) throws EventException {
		...
		try {		
			/**
			 * 서울의 관리조직에 COA의 REPORT 조회 권한을 주기 위해 예외처리함
			 * SELS,SELC,SELP로 시작하는 조직 
			 * 2008.02.21 변경 : SELB로 시작하는 조직과 SELSC를 제외한 SEL로 시작하는 조직에에 COA REPORT에 대한 조회 권한을 줌
			 ...
			 */
			if(
				((!ofc_cd.substring(0,4).equals("SELB") && !ofc_cd.equals("SELSA")) && ofc_cd.substring(0,3).equals("SEL")) ||
				(!ofc_cd.substring(0,4).equals("PUSM") && ofc_cd.substring(0,3).equals("PUS"))||
				(ofc_cd.equals("HJSM")) 
			   ){
				rtn_ofc_lvl = "1";
			} else if(ofc_cd.equals("PHXSA")){
				rtn_ofc_lvl = searchOFCLevel("LGBSC");
			} else if(ofc_cd.equals("SLCSC")){
				rtn_ofc_lvl = searchOFCLevel("SEASC");
			} else if(ofc_cd.equals("SELSA")){
				rtn_ofc_lvl = searchOFCLevel("SELSC");
			} else {
	...
	public void createVslLayupWeekCopy(WeekCopyVO weekCopyVO, SignOnUserAccount account) throws EventException {
		...
		try {
          ...
          //1. COA_MNL_DTL_COST테이블에서  TARGET 해당주를 삭제한다.
            dbDao.removeVslLayupWeekCopy(weekCopyVO);
            
          //2. COA_MNL_DTL_COST테이블에  SOURCE 해당주를 복사해서  TARGET 데이타를 생성한다. 

 ...
 * @since J2EE 1.5
 */
public class EsmBkg0510ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		for(int i=0;i<realCnt;i++){
			...
			for (int j = 0 ; j < colCnt ; j++) {
				...
				if (colName.equals("fax_no")) 
				{
					// 전송 성공/실패 여부 색상 표시 : 성공(S)-blue, 실패(F)-red,진행중(P)-pink
					strTmp = getNull(colValues.get("hld_fax_snd_rslt"));					
					...
					tdAttr = tdAttr + " TOOL-TIP=\"" + getNull(colValues.get("hld_fax_snd_rslt_nm")) + "\"";
				}
				else if (colName.equals("ntc_eml")) 
				{
					//  전송 성공/실패 여부 색상 표시 : 성공(S)-blue, 실패(F)-red,진행중(P)-pink
					strTmp = getNull(colValues.get("hld_eml_snd_rslt"));

					if ("S".equals(strTmp)) tdAttr = tdAttr + " COLOR=\"0,0,255\""; // blue
					else if ("F".equals(strTmp)) tdAttr = tdAttr + " COLOR=\"255,0,0\""; // red
					else if ("P".equals(strTmp)) tdAttr = tdAttr + " COLOR=\"255,0,192\""; // Pink
					
					tdAttr = tdAttr + " TOOL-TIP=\"" + getNull(colValues.get("hld_eml_snd_rslt_nm")) + "\"";
				}
				else if (colName.equals("hld_diff_rmk")) {
					tdAttr = tdAttr + " BGCOLOR=\"255,255,255\"";
				}

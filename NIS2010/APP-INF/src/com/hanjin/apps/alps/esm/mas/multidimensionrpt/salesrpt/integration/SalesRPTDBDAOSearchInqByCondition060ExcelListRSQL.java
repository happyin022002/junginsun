...
*@FileName : SalesRPTDBDAOSearchInqByCondition060ExcelListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SalesRPTDBDAOSearchInqByCondition060ExcelListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 0060화면에서 엑셀로 다운로드 받기위해서 사용함.화면출력안됨
	  * </pre>
	  */
	public SalesRPTDBDAOSearchInqByCondition060ExcelListRSQL(){
		setQuery();
		params = new HashMap<String,String[]>();
		query.append("/*").append("\n"); 
		query.append("Path : com.hanjin.apps.alps.esm.mas.multidimensionrpt.salesrpt.integration").append("\n"); 
		query.append("FileName : SalesRPTDBDAOSearchInqByCondition060ExcelListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	''" ).append("\n"); 
		query.append("#if(${wk_sts} == 'Y')" ).append("\n"); 
		query.append("	  , COST_YRMON" ).append("\n"); 
		query.append("	  , SLS_YRMON" ).append("\n"); 
		query.append("	  , COST_WK" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#foreach(${keys} IN ${keyList7})" ).append("\n"); 
		query.append("    #if (${keys} == 'rt_dt')" ).append("\n"); 
		...
		query.append("    	,NVL(VGM,0) AS VGM" ).append("\n"); 
		query.append("	 #else" ).append("\n"); 
		query.append("      ,$keys AS $keys" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${dir_sts} == 'Y')" ).append("\n"); 
		...
		query.append("		)RE_DT" ).append("\n"); 
		query.append("	 #elseif(${keys} == 'vgm_wgt')" ).append("\n"); 
		query.append("    	,(SELECT SUM(VGM_WGT) FROM BKG_CONTAINER WHERE BKG_NO = chg_bkg_no) AS VGM" ).append("\n"); 
		query.append("	 #else" ).append("\n"); 
		query.append("      ,$keys AS $keys" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("      ,ROUND(SUM(LOAD),3)                                                   AS LOAD" ).append("\n"); 
		query.append("#if (${mdm_charge_cd} != '')" ).append("\n"); 
		query.append("      ,ROUND(SUM(CHARGE_REV),3)                                   AS REV" ).append("\n"); 
		...
		query.append("#else" ).append("\n"); 
		query.append("      ,ROUND(SUM(BKG_REV),3)              AS REV " ).append("\n"); 
		query.append("--      ,SUM(MISC_REV)             AS MISC_REV  --20150706 추가" ).append("\n"); 
		query.append("--      ,SUM(DEM_DET)              AS DEMDET  --20150706 추가" ).append("\n"); 
		query.append("      ,ROUND(NVL(SUM(CM_COST),0),3)                        AS CMC" ).append("\n"); 
		query.append("      ,ROUND(SUM(BKG_REV+MISC_REV+DEM_DET)- NVL(SUM(CM_COST),0),3) AS CM	--20150706 " ).append("\n"); 
		query.append("#if(${pro_lvl} =='M') -- 2012.01.02 SHKIM CM2 ADD" ).append("\n"); 
		query.append("	  ,ROUND(SUM(OWN_FDR_AMT),3) AS CM2C" ).append("\n"); 
		query.append("	  ,ROUND(SUM(BKG_REV+MISC_REV+DEM_DET) - NVL(SUM(CM_COST,0)) - SUM(OWN_FDR_AMT),3) AS CM2	  " ).append("\n"); 
		query.append("#else	  " ).append("\n"); 
		query.append("	  ,0 AS CM2C" ).append("\n"); 
		query.append("	  ,0 AS CM2" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("      ,ROUND(SUM(BKG_REV)/SUM(LOAD),3)                                      AS G_RPB" ).append("\n"); 
		query.append("      ,ROUND(NVL(SUM(CM_COST),0),3)                                         AS CM_COST" ).append("\n"); 
		query.append("      ,ROUND(NVL(SUM(CM_COST),0) / SUM(LOAD) ,3)                            AS CMCOST" ).append("\n"); 
		query.append("      ,ROUND((SUM(BKG_REV+MISC_REV+DEM_DET) - NVL(SUM(CM_COST),0) ) / SUM(LOAD),3)  AS CMB" ).append("\n"); 
		query.append("      ,ROUND(SUM(OP_COST),3)                                                AS OPC" ).append("\n"); 
		query.append("      ,ROUND(SUM(BKG_REV+MISC_REV+DEM_DET) - NVL(SUM(CM_COST),0) - SUM(OP_COST),3)  AS OP" ).append("\n"); 
		query.append("      ,ROUND(SUM(OP_COST),3)                                                AS OP_COST" ).append("\n"); 
		query.append("      ,ROUND(SUM(OP_COST)/SUM(LOAD),3)                                      AS OPCOST" ).append("\n"); 
		query.append("      ,ROUND((SUM(BKG_REV+MISC_REV+DEM_DET) - NVL(SUM(CM_COST),0) - SUM(OP_COST))/SUM(LOAD),3) AS OPB" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("  " ).append("\n"); 
		...
		query.append("	     , MAX((SELECT MAS_GET_CD_NM_FNC('CD03264', RFA_MN.$keys)" ).append("\n"); 

...
public class BLDocumentationCMDBDAOmodifyBkgCntrPkcMeasWgtUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	, CNTR_VOL_QTY = CASE WHEN (SELECT SUM(CNTR_VOL_QTY) " ).append("\n"); 
		query.append("								  FROM BKG_CONTAINER " ).append("\n"); 
		query.append("								 WHERE (BKG_NO = @[mst_bkg_no] OR BKG_NO = @[bkg_no]) " ).append("\n"); 
		query.append("								   AND CNTR_NO = @[cntr_no]) > 1 THEN 1" ).append("\n"); 
		query.append("						  ELSE (SELECT SUM(CNTR_VOL_QTY) " ).append("\n"); 
		query.append("								  FROM BKG_CONTAINER " ).append("\n"); 
		query.append("								 WHERE (BKG_NO = @[mst_bkg_no] OR BKG_NO = @[bkg_no]) " ).append("\n"); 
		query.append("								   AND CNTR_NO = @[cntr_no]) END" ).append("\n"); 

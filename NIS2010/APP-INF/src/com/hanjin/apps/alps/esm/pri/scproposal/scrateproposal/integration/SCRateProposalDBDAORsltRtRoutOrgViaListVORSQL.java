...
*@FileName : SCRateProposalDBDAORsltRtRoutOrgViaListVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCRateProposalDBDAORsltRtRoutOrgViaListVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCRateProposalDBDAORsltRtRoutOrgViaListVORSQL(){
		setQuery();
		...
		query.append("FileName : SCRateProposalDBDAORsltRtRoutOrgViaListVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      ,FIRST_VALUE(DECODE(ROUT_VIA_PORT_TP_CD, 'G', 1, 'L', 2, 99)) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, ORG_DEST_TP_CD, ROUT_VIA_SEQ ORDER BY AMDT_SEQ) AS N1ST_ORD_REF" ).append("\n"); 
		query.append("      ,FIRST_VALUE(ROUT_VIA_PORT_DEF_CD) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, ORG_DEST_TP_CD, ROUT_VIA_SEQ ORDER BY AMDT_SEQ) AS N2ND_ORD_REF" ).append("\n"); 
		...
		query.append("              ,SRC_INFO_CD" ).append("\n"); 
		query.append("              ,N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,LEAD(N1ST_CMNC_AMDT_SEQ) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, ORG_DEST_TP_CD, ROUT_VIA_SEQ ORDER BY AMDT_SEQ) AS NEXT_N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,ACPT_USR_ID" ).append("\n"); 
		query.append("              ,ACPT_OFC_CD" ).append("\n"); 
		...
		query.append(" ORDER BY N1ST_ORD_REF, N2ND_ORD_REF, ROUT_VIA_SEQ, AMDT_SEQ" ).append("\n"); 

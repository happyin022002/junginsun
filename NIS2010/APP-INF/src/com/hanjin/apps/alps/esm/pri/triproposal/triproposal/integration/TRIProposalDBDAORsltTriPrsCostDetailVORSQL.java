...
*@FileName : TRIProposalDBDAORsltTriPrsCostDetailVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TRIProposalDBDAORsltTriPrsCostDetailVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  *    Cost Detail 화면(  PRI_6084) 에서 Cost Detail List를 추출하는 쿼리
	  * 
	  * * History
	  * 2015.06.26 CHM-201536492 Split05-주간 MAS Open에 따른 타모듈 프로그램 적용 요청
	  * </pre>
	  */
	public TRIProposalDBDAORsltTriPrsCostDetailVORSQL(){
		setQuery();
		...
		query.append("FileName : TRIProposalDBDAORsltTriPrsCostDetailVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("/* tri_prop_no,amdt_seq, rout_cs_no,rout_cs_src_dt,cost_tp  파라메터 추가*/" ).append("\n"); 
		...
		query.append("         , NVL(PROP_FRT_RT_AMT,0) + NVL(PRS_SCG_AMT,0) AMT   /* PRI_SP_SCP_RT : RATE + SURCHARGE */" ).append("\n"); 
		...
		query.append("                            WHERE REV_YRMON = '200903' )  /* MAS_BZC_COST_YRMON_FNC('', '')) ???????????????????????*/" ).append("\n"); 
		...
		query.append("            AND A3.REV_YRMON(+) = '200903' -- MAS_BZC_COST_YRMON_FNC('', '')" ).append("\n"); 
		...
		query.append("                   ,A2.COST_ACT_GRP_SEQ" ).append("\n"); 
		query.append("                   ,MAS_GET_COM_NM_FNC('COST_ACT_GRP_CD', A2.ACT_GRP_CD) GRP" ).append("\n"); 

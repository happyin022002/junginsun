...
* 2009.09.24 김기종 
* 1.0 Creation
* 1.1 2014.08.05 이한나[CHM-201431231] O/B CNTR MOVEMENT STATUS REPORT 보완
=========================================================*/
...
public class OutBdMovementStsListInVO extends AbstractValueObject {
	...
	/* Column Info */
	private String rdCntrQty = null;
	/* Column Info */
	private String trunkPodCd = null;
	/* Column Info */
	private String periodFlg = null;	
	...
	public HashMap<String, String> getColumnValues(){
		...
		this.hashColumns.put("rd_cntr_qty", getRdCntrQty());
		this.hashColumns.put("trunk_pod_cd", getTrunkPodCd());
		this.hashColumns.put("period_flg", getPeriodFlg());		
	...
	public HashMap<String, String> getFieldNames(){
		...
		this.hashFields.put("rd_cntr_qty", "rdCntrQty");
		this.hashFields.put("trunk_pod_cd", "trunkPodCd");
		this.hashFields.put("period_flg", "periodFlg");
	...
	public String getRdCntrQty() {
		return rdCntrQty;
	}

	public void setRdCntrQty(String rdCntrQty) {
		this.rdCntrQty = rdCntrQty;
	}
	...
	public void fromRequest(HttpServletRequest request) {
		...
		setRdCntrQty(JSPUtil.getParameter(request, "rd_cntr_qty", ""));
		setTrunkPodCd(JSPUtil.getParameter(request, "trunk_pod_cd", ""));
		setPeriodFlg(JSPUtil.getParameter(request, "period_flg", ""));		
	...
	public OutBdMovementStsListInVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		...
		try {
			...
			String[] rdCntrQty = (JSPUtil.getParameter(request, prefix	+ "rd_cntr_qty", length));
			String[] trunkPodCd = (JSPUtil.getParameter(request, prefix	+ "trunk_pod_cd", length));
			String[] periodFlg = (JSPUtil.getParameter(request, prefix	+ "period_flg", length));
			...
			for (int i = 0; i < length; i++) {
				...
				if (rdCntrQty[i] != null)
					model.setRdCntrQty(rdCntrQty[i]);
				if (trunkPodCd[i] != null)
					model.setTrunkPodCd(trunkPodCd[i]);
				if (periodFlg[i] != null)
					model.setTrunkPodCd(periodFlg[i]);
	...
	*/
	public void unDataFormat(){
		...
		this.rdCntrQty = this.rdCntrQty .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.trunkPodCd = this.trunkPodCd .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.periodFlg = this.periodFlg.replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");		

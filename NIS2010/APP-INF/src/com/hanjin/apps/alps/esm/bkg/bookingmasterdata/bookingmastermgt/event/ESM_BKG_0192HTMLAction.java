...
public class ESM_BKG_0192HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0926HTMLAction 객체를 생성
     */
    public ESM_BKG_0192HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0192Event event = new EsmBkg0192Event();

		if (command.isCommand(FormCommand.SEARCH))
		{
			// Auth Setup 정보 조회
			event.setInfoVO((SearchInBoundCustListVO) getVO(request, SearchInBoundCustListVO.class));
		}
		else if (command.isCommand(FormCommand.SEARCH01))
		{
			// Auth Setup 정보 수정
			event.setInfoVO2((BkgCustTmpltVO) getVO(request, BkgCustTmpltVO.class));
			
		}
		else if (command.isCommand(FormCommand.MULTI))
		{
			// Auth Setup 정보 수정
			event.setInfoVO2s((BkgCustTmpltVO[]) getVOs(request, BkgCustTmpltVO.class, "sheet2_"));

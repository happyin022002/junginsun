...
=========================================================*/
package com.hanjin.apps.alps.esm.bkg.customsdeclaration.customsreport.korea.basic;

import java.util.ArrayList;
import java.util.List;

import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customsreport.basic.CustomsReportBCImpl;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customsreport.korea.integration.KoreaCustomsReportDBDAO;
 ...
 * - NIS2010-CustomsReport 대한 비지니스 로직을 처리한다.<br>
 *
 * @author charves
 * @see UI_BKG-0215EventResponse,CustomsReportBC 각 DAO 클래스 참조
 * @since J2EE 1.6
 */
public class KorCustomsReportBCImpl extends CustomsReportBCImpl {
	...
	public ReportHistDetailVO searchTransmitHist(ReportHistCondVO reportHistContainer) throws EventException
	{
		...
		try 
		{
			...
			if (list!=null && ("5CG".equals(fromClient.getInMsgType()) || "5LI".equals(fromClient.getInMsgType()) ||
								"6SK".equals(fromClient.getInMsgType()) || "6SJ".equals(fromClient.getInMsgType())) ) {
	 ...
	 * @exception EventException
	 */
	public WareHouseVO[] searchWareHouseInfo(WareHouseCondVO warehouseCondVO)
	...
	/**
	 * Discharging CY Code List 조회
	 * @param DischCoCondVO dischCoCondVO
	 * @return DischCoVO[]
	 * @exception EventException
	 */
	public DischCoVO[] searchDischCoList(DischCoCondVO dischCoCondVO) throws EventException {
		DischCoVO[] dischCoVOs = null;
		// 파라메터 객체 변환
		KorDischCoCondVO korDischCoCondVO = (KorDischCoCondVO)dischCoCondVO;
		
		try {
			// 조회 처리 
			dischCoVOs = dbDao.searchDischCoList(korDischCoCondVO);
		} catch (DAOException de) {
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), de);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), ex);
	...
	public void manageDischCoList(DischCoCondVO[] dischCoCondVOs) throws EventException {
		// 파라메터 객체 변환
		KorDischCoCondVO[] korDischCoCondVOs = (KorDischCoCondVO[])dischCoCondVOs;
		List<KorDischCoCondVO> removeVOs 	= new ArrayList<KorDischCoCondVO>();
		List<KorDischCoCondVO> addVOs 		= new ArrayList<KorDischCoCondVO>();
		List<KorDischCoCondVO> modifyVOs 	= new ArrayList<KorDischCoCondVO>();
		try {
			// 작업 구분 분할
			for(int i=0; i < korDischCoCondVOs.length; i++) {
				
				if (korDischCoCondVOs[i].getIbflag().equals("D")) {
					removeVOs.add(korDischCoCondVOs[i]);
				}else if (korDischCoCondVOs[i].getIbflag().equals("I")) {
					addVOs.add(korDischCoCondVOs[i]);
				}else if (korDischCoCondVOs[i].getIbflag().equals("U")) {
					modifyVOs.add(korDischCoCondVOs[i]);
				}
			}
			
			// REMOVE
			dbDao.removeDischCoList(removeVOs.toArray(new KorDischCoCondVO[0]));
			// INSERT
			dbDao.addDischCoList(addVOs.toArray(new KorDischCoCondVO[0]));
			// MODIFY
			dbDao.modifyDischCoList(modifyVOs.toArray(new KorDischCoCondVO[0]));
	...
	/**
	 * Korea에서 입/출항 선박 신고 적하목록 ACK 문서의 상세내역 조회
	 * @param TransmitHistDtlCondVO transmitHistDtlCondVO
	 * @return TransmitHistDtlVO[] 
	 * @exception EventException
	 */
	public List<ReceiveLogDetailVO> searchReceiveLog(ReceiveLogCondVO ReceiveLogCondVO) throws EventException {
		// 최종 조회 결과 배열
		KorRcvHistDtlCondVO rcvHistDtlCondVO = (KorRcvHistDtlCondVO)ReceiveLogCondVO;
		List<KorRcvHistDtlVO> rcvHistDtlVOs = null;
		List<ReceiveLogDetailVO> receiveLogDetailVOs = null;
		
		try {
			rcvHistDtlVOs = dbDao.searchRcvHistDtl(rcvHistDtlCondVO);
			
			receiveLogDetailVOs = (List<ReceiveLogDetailVO>) (Object) rcvHistDtlVOs;

...
public class KorManifestListDBDAOaddBlInfoKorCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       BL_MEAS_UT_CD, BD_AREA_CD, CSTMS_CRR_IN_LOC_WH_CD, IMDG_CLSS_CD, KR_CSTMS_WH_TP_CD, KR_WH_CD, " ).append("\n"); 
		...
		query.append("       IB_DMST_PORT_CD, IB_VSL_CD, IB_SKD_VOY_NO, IB_SKD_DIR_CD, IB_ETA_DT, BB_CGO_WGT, BB_CGO_MEAS_QTY, " ).append("\n"); 
		...
		query.append("    , DECODE(@[bkg_cgo_tp], 'P', 'E', 'R', 'E', @[msn_bltp])" ).append("\n"); 
		...
		query.append("     #if (((${expt_kcd_tp}=='R' || ${kcd_tp}=='E')) && ${bkg_cgo_tp} =='P')" ).append("\n"); 
		query.append("      , '03077006'" ).append("\n"); 
		query.append("      #else " ).append("\n"); 
		query.append("      , CASE WHEN @[kt_port] = 'KRPUS' AND @[bkg_cgo_tp] = 'P' THEN 'ST' ELSE @[bond_area_code] END" ).append("\n"); 
		...
		query.append("    , DECODE(@[bkg_cgo_tp], 'P', 'EMPTY CONTAINERS', nvl(BKG_SPCLCHAR_CONV_FNC(@[desc1], 'Y'), ' '))" ).append("\n"); 
		...
		query.append("    , DECODE(@[bkg_cgo_tp],'P','M',@[us_bound]) " ).append("\n"); 

...
*@FileName : SCProposalMainDBDAOsearchEmailTargetUserListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCProposalMainDBDAOsearchEmailTargetUserListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCProposalMainDBDAOsearchEmailTargetUserListRSQL(){
		setQuery();
		...
		query.append("FileName : SCProposalMainDBDAOsearchEmailTargetUserListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        ,PRI_SP_HDR A3" ).append("\n"); 
		query.append("        ,PRI_SP_SCP_MN A4" ).append("\n"); 
		query.append("        ,BKG_HIS_DTL A5" ).append("\n"); 
		query.append("    WHERE A1.BKG_STS_CD <> 'X'" ).append("\n"); 
		query.append("      AND A1.BKG_NO = A2.BKG_NO" ).append("\n"); 
		query.append("      AND A1.SC_NO = A3.SC_NO		-- file 대상 S/C" ).append("\n"); 
		query.append("      AND A3.PROP_NO = A4.PROP_NO" ).append("\n"); 
		query.append("	  AND A4.PROP_NO =  @[prop_no]  -- file 대상 S/C, " ).append("\n"); 
		query.append("      AND A4.AMDT_SEQ = @[amdt_seq] " ).append("\n"); 
		query.append("      AND A4.SVC_SCP_CD = A1.SVC_SCP_CD" ).append("\n"); 
		query.append("      AND (A2.RT_APLY_DT BETWEEN A4.EFF_DT AND A4.EXP_DT --file 한 S/C scope의 effective date 이후에 application date가 존재하는 경우" ).append("\n"); 
		query.append("          OR A4.EXP_DT <A2.RT_APLY_DT) -- file 한 S/C의 exp date 이후에 rate 날짜가 있는 경우 -- 적용 S/C가 없어지면서 다시 rating 해야 함" ).append("\n"); 

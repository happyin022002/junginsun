 ...
 * 2008.03.19 전윤주 : CSR No. N200802290005 COA_EMU, 샤시 비용 관련 수정[115]               
 * 2008.03.21 박은주 N200802280015 COA_Report 관련 수정 요청_3번항목  
 ...
 * 2008.04.22 박칠서 N200803130011 Monthly VVD 확정 시 COA only를 이유로 delete 된 항차의 경우, 
                                   SKD 배치가 돌때 항차가 다시 살아나지 않도록 수정
 * 2008.04.29 임옥영 N200804280007 Source Data Download 수정
 * 2008.06.24 박은주 N200806120005 COA_Report 조회 오류[035] 
 ...
 * 2009.01.23 전윤주 CSR No. N200901230005 COA_EMU Credit 내 EQ Cost 제외 [115]
...
import com.hanjin.apps.alps.esm.coa.weeklypfmc.weeklycm.vo.SearchSeasonalSMUCostPopListVO;	// shkim0531
...
import com.hanjin.apps.alps.esm.coa.weeklypfmc.weeklycm.vo.SearchVslRgstCountVO;
...
public class WeeklyCMDBDAO extends DBDAOSupport {
	 ...
	 * ESM_COA_0117 저장
	 * @param List<CoaSltMgmtUtVO> coaSltMgmtUtVO
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifySMUPfmc(List<CoaSltMgmtUtVO> coaSltMgmtUtVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaSltMgmtUtVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new WeeklyCMDBDAOCoaSltMgmtUtVOUSQL(), coaSltMgmtUtVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	 ...
	 *
	 * @param List<CoaLaneDirConvVO> insertVoList
	 * @return int[]
	 * @exception DAOException
	 * @exception Exception
	 */
	public int[] addLaneBoundSwitch(List<CoaLaneDirConvVO> insertVoList) throws DAOException,Exception {
		int insCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
	 ...
	 *
	 * @param List<CoaLaneDirConvVO> removeVoList
	 * @return int[]
	 * @exception DAOException
	 * @exception Exception
	 */
	public int[] removeLaneBoundSwitch(List<CoaLaneDirConvVO> removeVoList) throws DAOException,Exception {
		int delCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
	 ...
	 * ESM_COA_0118 저장
	 * @param List<CoaInterOwnTmlCostVO> coaInterOwnTmlCostVO
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifyOwnTMLPfmc(List<CoaInterOwnTmlCostVO> coaInterOwnTmlCostVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaInterOwnTmlCostVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new WeeklyCMDBDAOCoaInterOwnTmlCostVOUSQL(), coaInterOwnTmlCostVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	 ...
	 * 
	 * @param List<CoaInterOwnTmlCostVO> delModels
	 * @exception DAOException
	 */
	public void removeOwnTMLPfmc(List<CoaInterOwnTmlCostVO> delModels) throws DAOException,Exception {
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			int delCnt[] = null;
			if(delModels.size() > 0){				
				HashMap<String, Object> velParam = new HashMap<String, Object>();					
				delCnt = sqlExe.executeBatch((ISQLTemplate)new WeeklyCMDBDAOCoaInterOwnTmlCostVODSQL(), delModels, velParam);
				for(int i = 0; i < delCnt.length; i++){
					if(delCnt[i]== Statement.EXECUTE_FAILED)
	 ...
	 * ESM_COA_0119 화면 추가
	 * @param List<CoaTmlTrfGrpVO> coaTmlTrfGrpVO
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] addUOM0119(List<CoaTmlTrfGrpVO> coaTmlTrfGrpVO) throws DAOException,Exception {
		int insCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaTmlTrfGrpVO .size() > 0){
				insCnt = sqlExe.executeBatch((ISQLTemplate)new WeeklyCMDBDAOCoaTmlTrfGrpVOCSQL(), coaTmlTrfGrpVO,null);
				for(int i = 0; i < insCnt.length; i++){
					if(insCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	 ...
	 * @throws Exception
	 */
	public int[] modifyUOM0119(List<CoaTmlTrfGrpVO> coaTmlTrfGrpVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaTmlTrfGrpVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new WeeklyCMDBDAOCoaTmlTrfGrpVOUSQL(), coaTmlTrfGrpVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	...
	public List<CoaEMUCreditListVO> searchEmuCreditTableList(SearchConditionVO searchConditionVO) throws DAOException {
		DBRowSet dbRowset = null;
		List<CoaEMUCreditListVO> list = null;
		//query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		//velocity parameter
		Map<String, Object> velParam = new HashMap<String, Object>();
		...
		try{
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new WeeklyCMDBDAOCoaEMUCreditListVORSQL(), param, velParam);
			list = (List)RowSetUtil.rowSetToVOs(dbRowset, CoaEMUCreditListVO .class);
		}catch(SQLException se){
			log.error(se.getMessage(),se);
			throw new DAOException(new ErrorHandler(se).getMessage(), se);
		}catch(Exception ex){
	...
	public List<SearchOPCreditRtPortPairVO> searchCreditRtPortPairList(SearchOPCreditRtPortPairVO searchOPCreditRtPortPairVO, String queryType) throws DAOException {
		...
		try{
			...
			if(queryType.equals("OP")){
				...
				dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new WeeklyCMDBDAOSearchOPCreditRtPortPairRSQL2(), param, velParam);
			}

...
*@FileName : ESM_MAS_0253HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0251Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0253HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0253HTMLAction 객체를 생성
	 */
	public ESM_MAS_0253HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0253Event event = new EsmMas0253Event();  		
    	    	
    	
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setSearchChassisCostVO((SearchChassisCostVO)getVO(request, SearchChassisCostVO.class));
		} else if(command.isCommand(FormCommand.SEARCHLIST02)) {
			event.setSearchChassisCostVO((SearchChassisCostVO)getVO(request, SearchChassisCostVO.class));		
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchChassisCostVOs((SearchChassisCostVO[])getVOs(request, SearchChassisCostVO.class, ""));
		} else if(command.isCommand(FormCommand.COMMAND01)) {			
			event.setSearchChassisCostVO((SearchChassisCostVO)getVO(request, SearchChassisCostVO.class));	
		} else if(command.isCommand(FormCommand.COMMAND09)) {            
            event.setSearchChassisCostVO((SearchChassisCostVO)getVO(request, SearchChassisCostVO.class));
            event.setKey((String)request.getParameter("key")); 
		} else{

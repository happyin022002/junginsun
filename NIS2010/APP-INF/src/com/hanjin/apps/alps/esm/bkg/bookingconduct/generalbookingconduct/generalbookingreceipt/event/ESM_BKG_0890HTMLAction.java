...
*@FileName : ESM_BKG_0890HTMLAction.java
...
public class ESM_BKG_0890HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0890HTMLAction 객체를 생성
	 */
	public ESM_BKG_0890HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0890Event event = new EsmBkg0890Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO .class));
			event.setCallTp(request.getParameter("callTp"));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO .class));
			event.setBkgQtyDtlVOs((BkgQtyDtlVO[])getVOs(request, BkgQtyDtlVO.class,"sheet1_"));
		}

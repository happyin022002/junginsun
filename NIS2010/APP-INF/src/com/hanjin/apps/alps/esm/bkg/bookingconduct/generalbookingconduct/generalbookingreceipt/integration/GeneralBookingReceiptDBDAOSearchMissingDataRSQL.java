...
public class GeneralBookingReceiptDBDAOSearchMissingDataRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 2011.04.25 이일민 [CHM-201110328] BKG Qty (TRO개수) 비교 로직 보완 -Partial 케이스 고려
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("     WHERE upper(@[ob_srep_cd]) NOT IN (SELECT UPPER(SREP_CD) FROM MDM_SLS_REP WHERE SREP_STS_CD = 'N')" ).append("\n"); 
		query.append("#if (${qty_modify_flag} == 'Y' || ${route_modify_flag} == 'Y')" ).append("\n"); 
		query.append("    UNION" ).append("\n"); 
		query.append("    SELECT 'TRO volume'" ).append("\n"); 
		query.append("     FROM (SELECT QTY.BKG_NO" ).append("\n"); 
		query.append("                 ,QTY.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("          ,CEIL(QTY.OP_CNTR_QTY) AS OP_CNTR_QTY" ).append("\n"); 
		query.append("          ,NVL(CNTR.CNTR_PRT_FLG, 'N') AS CNTR_PRT_FLG" ).append("\n"); 
		query.append("          ,NVL(CNTR.CNTR_CNT, 0) AS CNTR_CNT" ).append("\n"); 
		query.append("      FROM BKG_QUANTITY QTY" ).append("\n"); 
		query.append("          ,(SELECT CNTR.BKG_NO" ).append("\n"); 
		query.append("                  ,CNTR.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                  ,MAX(CNTR_PRT_FLG) AS CNTR_PRT_FLG" ).append("\n"); 
		query.append("              ,COUNT(CNTR.CNTR_NO) AS CNTR_CNT" ).append("\n"); 
		query.append("          FROM BKG_CONTAINER CNTR" ).append("\n"); 
		query.append("         WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("         GROUP BY CNTR.BKG_NO" ).append("\n"); 
		query.append("                  ,CNTR.CNTR_TPSZ_CD) CNTR" ).append("\n"); 
		query.append("     WHERE QTY.BKG_NO = CNTR.BKG_NO (+)" ).append("\n"); 
		query.append("       AND QTY.CNTR_TPSZ_CD = CNTR.CNTR_TPSZ_CD (+)" ).append("\n"); 
		query.append("       AND QTY.BKG_NO = @[bkg_no]) QTY," ).append("\n"); 
		...
		query.append("      AND CASE WHEN QTY.CNTR_PRT_FLG = 'Y' THEN GREATEST(QTY.CNTR_CNT, QTY.OP_CNTR_QTY)" ).append("\n"); 
		query.append("               ELSE CEIL(QTY.OP_CNTR_QTY)" ).append("\n"); 
		query.append("          END  < NVL(TRO_QTY, 0)" ).append("\n"); 
		...
		query.append("	UNION ALL" ).append("\n"); 
		query.append("	SELECT CNTR_TPSZ_CD || ' Qty is different form booking. Please check '|| CNTR_TPSZ_CD || ' Qty again.' MESSAGE" ).append("\n"); 
		query.append("#if (${ca_flg} == 'Y')" ).append("\n"); 
		...
		query.append("     WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("	   AND DRY_CGO_FLG = 'N' AND SOC_FLG = 'N'" ).append("\n"); 
		query.append("	   AND AWK_CGO_FLG = 'N' AND DCGO_FLG = 'N' AND RC_FLG = 'N' AND BB_CGO_FLG = 'N'" ).append("\n"); 
		query.append("	   AND MER_HNGR_FLG = 'N' AND CRR_HNGR_FLG = 'N'" ).append("\n"); 
		query.append("	   AND CRR_HNGR_SGL_BAR_USE_FLG = 'N' AND CRR_HNGR_DBL_BAR_USE_FLG = 'N' AND CRR_HNGR_TPL_BAR_USE_FLG = 'N'" ).append("\n"); 
		query.append("	   AND EQ_SUBST_CNTR_TPSZ_CD IS NULL" ).append("\n"); 

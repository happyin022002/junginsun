...
*@FileName : ESM_PRI_0009HTMLAction.java
...
public class ESM_PRI_4100HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_0009HTMLAction 객체를 생성
	 */
	public ESM_PRI_4100HTMLAction() {}
	...
	public Event perform (HttpServletRequest request) throws HTMLActionException {
        ...
        EsmPri4100Event event = new EsmPri4100Event();
        log.debug("aaaaaad : " + request.getParameter("trd_cd"));
        if (command.isCommand(FormCommand.MULTI)) {
            RsltAuthorizationVO[] vos = (RsltAuthorizationVO[]) getVOs(request, RsltAuthorizationVO.class, "");

            if (vos != null) {
                for (int i = 0, n = vos.length; i < n; i++) {
                    vos[i].setTrdCd(request.getParameter("trd_cd"));
                }
            }
            event.setRsltAuthorizationVOS(vos);
        } else if (command.isCommand(FormCommand.SEARCH01)) {
            event.setOrganizationVO((OrganizationVO) getVO(request, OrganizationVO.class));
        } else if (command.isCommand(FormCommand.SEARCH02)) {
            event.setRsltAuthorizationVO((RsltAuthorizationVO) getVO(request, RsltAuthorizationVO.class));

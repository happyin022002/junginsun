...
public class CostStructureDBDAOSearchCostStructure0137List2RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 2014.10.06 박은주 [CHM-201432147] [MAS] Node/Link U/C Adjustment 화면 : COST_ACT_GRP_CD 추가 , CO_CD 제거 요청
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("       ,A.MAS_COST_SRC_CD AS MAS_COST_SRC_CD_V" ).append("\n"); 
		...
		query.append("      ,(    SELECT DECODE(SUM(DECODE(SUBSTR(MAS_COST_SRC_CD, 1, 1), 'S', 0, LNK_TTL_QTY))" ).append("\n"); 
		query.append("                         , 0, null" ).append("\n"); 
		query.append("                         ,ROUND(SUM(LNK_TTL_AMT)/SUM(DECODE(SUBSTR(MAS_COST_SRC_CD, 1, 1), 'S', 0, LNK_TTL_QTY)), 3))" ).append("\n"); 
		...
		query.append("FROM MAS_LNK_AVG_STND_COST A, MAS_COST_SRC_ACCT B" ).append("\n"); 
		query.append("WHERE A.MAS_COST_SRC_CD = B.MAS_COST_SRC_CD" ).append("\n"); 
		...
		query.append("COUNT(*) ROWCNT" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("FROM MAS_NOD_AVG_STND_COST A, MAS_COST_SRC_ACCT B" ).append("\n"); 
		query.append("WHERE A.MAS_COST_SRC_CD = B.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("    #foreach( ${condition} in ${conditionArr})" ).append("\n"); 
		query.append("    AND A.${condition.colName} ${condition.inEquality} ${condition.value}" ).append("\n"); 
		query.append("    #end" ).append("\n"); 

...
public class StatusReportDBDAOSearchBDRBookingStatusList1RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            C.VSL_CD||C.SKD_VOY_NO||C.SKD_DIR_CD VVD_CD, /*9자리*/" ).append("\n"); 
		...
		query.append("          " ).append("\n"); 
		query.append("  CASE WHEN C.TRNK_BDR_FLG='Y' or C.TRNK_AUTO_BDR_FLG='Y' THEN C.TRNK_AUTO_BDR_DT" ).append("\n"); 
		query.append("                WHEN C.TRNK_MNL_BDR_FLG='Y'  THEN C.TRNK_MNL_BDR_DT" ).append("\n"); 
		query.append("                ELSE  NVL(C.TRNK_ESTM_BDR_DT, A.VPS_ETD_DT + NVL(B.TRNK_BDR_DYS, 0))" ).append("\n"); 
		query.append("            END AS TRNK_BDR_DT,/*trnk_bdr_time*/  " ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("            CASE WHEN C.TRNK_BDR_FLG='Y' OR C.TRNK_AUTO_BDR_FLG='Y' OR C.TRNK_MNL_BDR_FLG='Y'  THEN 'Yes'" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("            CASE WHEN C.FDR_BDR_FLG='Y' or C.FDR_AUTO_BDR_FLG='Y' THEN C.FDR_AUTO_BDR_DT" ).append("\n"); 
		query.append("                WHEN C.FDR_MNL_BDR_FLG='Y'  THEN C.FDR_MNL_BDR_DT" ).append("\n"); 
		query.append("                ELSE  NVL(C.FDR_ESTM_BDR_DT, A.VPS_ETD_DT + NVL(B.TRNK_BDR_DYS, 0))" ).append("\n"); 
		query.append("            END AS FDR_BDR_DT,  /*feeder_bdr_time*/" ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("            CASE WHEN C.FDR_BDR_FLG='Y' OR C.FDR_AUTO_BDR_FLG='Y' OR C.FDR_MNL_BDR_FLG='Y'  THEN 'Yes'" ).append("\n"); 
		query.append("                 ELSE  'No'" ).append("\n"); 
		query.append("            END AS FDR_BDR_FLG, /*feeder_bdr*/" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("        AND ( CASE WHEN C.TRNK_BDR_FLG='Y' or C.TRNK_AUTO_BDR_FLG='Y' THEN C.TRNK_AUTO_BDR_DT" ).append("\n"); 
		query.append("                           WHEN C.TRNK_MNL_BDR_FLG='Y'  THEN C.TRNK_MNL_BDR_DT" ).append("\n"); 
		query.append("                           ELSE  NVL(C.TRNK_ESTM_BDR_DT, A.VPS_ETD_DT + NVL(B.TRNK_BDR_DYS, 0))" ).append("\n"); 
		query.append("                           END) >= TO_DATE(@[bdr_dt] , 'YYYY-MM-DD')" ).append("\n"); 
		query.append("                                             " ).append("\n"); 
		query.append("        AND ( CASE WHEN C.TRNK_BDR_FLG='Y' or C.TRNK_AUTO_BDR_FLG='Y' THEN C.TRNK_AUTO_BDR_DT" ).append("\n"); 
		query.append("                           WHEN C.TRNK_MNL_BDR_FLG='Y'  THEN C.TRNK_MNL_BDR_DT" ).append("\n"); 
		query.append("                           ELSE  NVL(C.TRNK_ESTM_BDR_DT, A.VPS_ETD_DT + NVL(B.TRNK_BDR_DYS, 0))" ).append("\n"); 
		query.append("                           END) < TO_DATE(@[bdr_dt], 'YYYY-MM-DD') + 1                                    " ).append("\n"); 
		query.append("        #end   " ).append("\n"); 

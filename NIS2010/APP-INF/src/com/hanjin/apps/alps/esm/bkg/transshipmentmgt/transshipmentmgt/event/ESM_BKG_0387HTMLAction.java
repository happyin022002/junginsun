...
public class ESM_BKG_0387HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			String[] oldNextVVD = JSPUtil.getParameter(request,"sheet3_old_next_vvd", length);
			String[] relayTmnl = JSPUtil.getParameter(request,"sheet3_relay_tmnl", length);
			String[] relaySeq = JSPUtil.getParameter(request,"sheet3_relay_seq", length);
			...
			for(int i=0;i<length;i++){
				if (sChk[i].equals("1")){
					...
					event.setOldNextVvd(oldNextVVD[i]);
					event.setRelayTmnl(relayTmnl[i]);
					event.setRelaySeq(relaySeq[i]);
					break;
				}
			}

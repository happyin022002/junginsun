...
public class CndCustomsTransmissionDBDAOsearchCndCstmsSndHisListRSQL implements ISQLTemplate{
	...
	public CndCustomsTransmissionDBDAOsearchCndCstmsSndHisListRSQL(){
		...
		params.put("io_bond_cd",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("   AND  A.IO_BND_CD = NVL(@[io_bond_cd],'I')" ).append("\n"); 
		query.append("   AND  C1.HRD_CDG_ID(+) = 'CND_ACK_CODE'" ).append("\n"); 
		query.append("--   AND  A.TRSM_MSG_TP_ID NOT IN ( 'EXA','EXS','EXE' ) --IO_BND_CD로 구분 " ).append("\n"); 
		...
		query.append("  #end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${trsm_msg_tp_id} == '' && ${bl_no} != '')" ).append("\n"); 
		query.append(" /* 해당 VVD만 조회 */" ).append("\n"); 
		query.append("     AND  ( L.VSL_CD, L.SKD_VOY_NO, L.SKD_DIR_CD) IN ( SELECT ABL.VSL_CD, ABL.SKD_VOY_NO, ABL.SKD_DIR_CD " ).append("\n"); 

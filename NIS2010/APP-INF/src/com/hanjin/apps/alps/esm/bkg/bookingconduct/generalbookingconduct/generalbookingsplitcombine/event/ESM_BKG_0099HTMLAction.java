...
public class ESM_BKG_0099HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.DEFAULT) ||command.isCommand(FormCommand.SEARCH)) {
			...
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
		}else if(command.isCommand(FormCommand.MULTI) || command.isCommand(FormCommand.COMMAND04)
				||command.isCommand(FormCommand.COMMAND05)){ //split
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
			...
			for (int idx=0;idx<arrSplitNo.length;idx++){
				...
				//EQ subst tp,qty 처리
				splitQtyVO.setEqSubstCntrTpszCd(arrData[3]);
				splitQtyVO.setEqSubstCgoQty(arrData[4]);
				splitQtyVO.setRdCgoFlag(arrData[5]);

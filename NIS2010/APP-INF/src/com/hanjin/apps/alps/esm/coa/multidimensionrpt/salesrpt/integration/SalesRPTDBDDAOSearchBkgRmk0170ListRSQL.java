...
public class SalesRPTDBDDAOSearchBkgRmk0170ListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                WHERE REV_YRMON = COA_BZC_COST_YRMON_FNC(@[f_bkg_no])" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		...
		query.append("          AND A3.REV_YRMON(+)    = COA_BZC_COST_YRMON_FNC(@[f_bkg_no])" ).append("\n"); 
		...
		query.append("        ,A1.COA_COST_SRC_CD                                        -- 2011.01.11 이윤정 [CHM-201108216-01] TMFDFL 계정에 대해서 TMFDMT으로 변환하여 보여주는 부분을 제거." ).append("\n"); 
		...
		query.append("        AND COA_COST_SRC_PRT_CD IN(DECODE(@[f_pro_lvl], 'C', 'CO', 'CO'), DECODE(@[f_pro_vw], 'P', 'PA', 'RA'))" ).append("\n"); 
		query.append("		AND	STND_COST_TP_CD IN('C', DECODE(@[f_pro_lvl], 'C', 'C', 'M', 'C','O')) " ).append("\n"); 
		...
		query.append("        AND A1.COA_COST_SRC_CD  NOT IN ('92404011', '65000000', '65901011', '65901021')" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		...
		query.append("         ) STND_COST_NM" ).append("\n"); 
		query.append("        ,B1.RA_ACCT_CD COA_COST_SRC_CD" ).append("\n"); 
		query.append("        ,B2.SLS_ACT_DESC COA_COST_SRC_NM" ).append("\n"); 
		...
		query.append("    FROM COA_BKG_SVC_TRNS_PRC B1" ).append("\n"); 
		query.append("        ,COA_OFC_ROUT_MAPG B2" ).append("\n"); 
		query.append("    WHERE B1.SLS_ACT_CD       = B2.SLS_ACT_CD" ).append("\n"); 
		query.append("        AND B1.OFC_CLSS_CD      = B2.OFC_CLSS_CD" ).append("\n"); 
		query.append("        AND B2.COST_YRMON       = COA_BZC_COST_YRMON_FNC(B1.BKG_NO)" ).append("\n"); 
		...
		query.append("        AND B1.COA_COST_SRC_CD  NOT IN ('91401011')" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("        AND B1.COA_COST_SRC_CD  NOT IN ('92404011', '65000000', '65901011', '65901021', '91401011')" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("    )" ).append("\n"); 

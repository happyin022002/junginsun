...
*@FileName : ESM_MAS_0159HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Kim Ki Dae
 * @see EsmMas0159Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0159HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0159HTMLAction 객체를 생성
	 */
	public ESM_MAS_0159HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0159Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0159Event event = new EsmMas0159Event();  		
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkDistributionCommonVO((NetworkDistributionCommonVO)getVO(request, NetworkDistributionCommonVO.class));			
			event.setMasLaneTsUtCostVOs((MasLaneTsUtCostVO[])getVOs(request, MasLaneTsUtCostVO.class));			
		}								
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkDistributionCommonVO((NetworkDistributionCommonVO)getVO(request, NetworkDistributionCommonVO.class));

 ...
 *
 * @author Jun Yong Jin
 * @see com.hanjin.apps.alps.esm.bkg.bookingconduct.ebookingconduct.ebookingreceipt.event.ESM_BKG_0229_05HTMLAction
 * @since J2EE 1.6
 */
public class EsmBkg022905ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		String[] realColNms = getColHeader(vos.get(0));
		String[] changedColNms = getChangedColNms(realColNms, prefix);
		
		if(vo.getMaxRows()>0){
			totCnt = vo.getMaxRows();
		}
		if(Arrays.asList(changedColNms).contains("multidesc")){
		...
		for(int i=0;i<realCnt;i++){
			...
			if ( oldCntrNo != null && !oldCntrNo.equals(JSPUtil.getNull(colValues.get("cntr_no"))) ) cntrSeq++;
			
			if ( !"".equals(JSPUtil.getNull(colValues.get("val"))) ) {
				sbufXML.append("	<TR><![CDATA[");
				sbufXML.append(JSPUtil.getNull(colValues.get("val")) + DELIMITER);
				sbufXML.append(JSPUtil.getNull(colValues.get("multidesc")) + DELIMITER);
				sbufXML.append(JSPUtil.getNull(colValues.get("ibflag")) + DELIMITER);
				sbufXML.append(JSPUtil.getNull(colValues.get("desc")) + DELIMITER);
				sbufXML.append("]]></TR>\n");				
			} else {
				if ( !"".equals(JSPUtil.getNull(colValues.get("diff_rmk"))) ) {
					...
					sbufXML.append("		<TD><![CDATA[" + JSPUtil.getNull(colValues.get("si_no")) + "]]></TD>\n");
					sbufXML.append("		<TD><![CDATA[" + JSPUtil.getNull(colValues.get("si_no")) + "]]></TD>\n");
					sbufXML.append("	</TR>\n");
					...
					sbufXML.append("		<TD><![CDATA[" + colValues.get("ncm_no") + "]]></TD>\n");
					sbufXML.append("		<TD><![CDATA[" + JSPUtil.getNull(colValues.get("xter_rqst_seq")) + "]]></TD>\n");
					sbufXML.append("		<TD><![CDATA[" + colValues.get("ncm_multi_no") + "]]></TD>\n");
					sbufXML.append("	</TR>\n");
		...
		//			}
					...
					sbufXML.append("		<TD><![CDATA[" + colValues.get("wpm_trt_cd") + "]]></TD>\n");				
					
					sbufXML.append("		<TD><![CDATA[" + JSPUtil.getNull(colValues.get("cntr_tpsz_cd")) + "]]></TD>\n");
					sbufXML.append("		<TD><![CDATA[" + JSPUtil.getNull(colValues.get("cntr_tpsz_cd")) + "]]></TD>\n");
					sbufXML.append("	</TR>\n");

...
*@FileName : KrWharfageDecMgtDBDAOsearchKrWhfChgListGenSumRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class KrWharfageDecMgtDBDAOsearchKrWhfChgListGenSumRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public KrWharfageDecMgtDBDAOsearchKrWhfChgListGenSumRSQL(){
		setQuery();
		...
		query.append("FileName : KrWharfageDecMgtDBDAOsearchKrWhfChgListGenSumRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT B.RT_TEU, B.RT_FEU, B.RT_HCB," ).append("\n"); 
		query.append("E.BKG_TEU, E.BKG_FEU, E.BKG_HCB," ).append("\n"); 
		query.append("B.TTL_AMT, B.BL_CNT" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(SELECT SUM(DECODE(A.CNTR_SZ_CD, '2', A.RAT_AS_QTY_SUM, 0)) AS RT_TEU," ).append("\n"); 
		query.append("SUM(DECODE(A.CNTR_SZ_CD, '4', A.RAT_AS_QTY_SUM, 0)) AS RT_FEU," ).append("\n"); 
		query.append("SUM(DECODE(A.CNTR_SZ_CD, '7', A.RAT_AS_QTY_SUM, 0)) AS RT_HCB," ).append("\n"); 
		query.append("SUM(A.AMOUNT) AS TTL_AMT, COUNT(DISTINCT A.BL_NO) AS BL_CNT" ).append("\n"); 
		query.append("FROM ( SELECT A.BKG_NO," ).append("\n"); 
		query.append("(A.BL_NO || A.BL_TP_CD) AS BL_NO, A.CUST_NM, A.XPT_REF_NO," ).append("\n"); 
		...
		query.append("AND B.BKG_STS_CD NOT IN ('A', 'X')" ).append("\n"); 
		...
		query.append("#end ) A) B," ).append("\n"); 
		query.append("(SELECT SUM(C.TEU_QTY) AS BKG_TEU, SUM(C.FEU_QTY) AS BKG_FEU, SUM(C.HCB_QTY) AS BKG_HCB" ).append("\n"); 
		query.append("FROM ( SELECT A.BKG_NO," ).append("\n"); 
		query.append("NVL(SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) =  '2' THEN OP_CNTR_QTY ELSE 0 END), 0) AS TEU_QTY," ).append("\n"); 
		query.append("NVL(SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) IN ('3', '4', '5') THEN OP_CNTR_QTY ELSE 0 END), 0) AS FEU_QTY," ).append("\n"); 
		query.append("NVL(SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) IN ('2', '3', '4', '5') THEN 0 ELSE OP_CNTR_QTY END), 0) AS HCB_QTY" ).append("\n"); 
		...
		query.append("WHERE B.BKG_NO = A.BKG_NO" ).append("\n"); 
		query.append("AND B.CNTR_TPSZ_CD NOT LIKE 'Q%'" ).append("\n"); 
		query.append("GROUP BY A.BKG_NO ) C" ).append("\n"); 
		query.append("WHERE C.BKG_NO IN (SELECT DISTINCT D.BKG_NO" ).append("\n"); 
		query.append("FROM ( SELECT A.BKG_NO," ).append("\n"); 
		query.append("(A.BL_NO || A.BL_TP_CD) AS BL_NO, A.CUST_NM, A.XPT_REF_NO," ).append("\n"); 
		query.append("B.RAT_UT_CD," ).append("\n"); 
		query.append("B.RAT_AS_QTY_SUM," ).append("\n"); 

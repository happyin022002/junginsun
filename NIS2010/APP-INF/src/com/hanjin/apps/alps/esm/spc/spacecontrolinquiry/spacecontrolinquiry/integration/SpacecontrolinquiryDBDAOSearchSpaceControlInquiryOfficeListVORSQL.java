...
public class SpacecontrolinquiryDBDAOSearchSpaceControlInquiryOfficeListVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("           M.SKD_VOY_NO," ).append("\n"); 
		query.append("           M.SKD_DIR_CD," ).append("\n"); 
		query.append("           SUM(DECODE(D.TS_FLG, 'N', D.BKG_AVAL_TTL_QTY)) AS VOL," ).append("\n"); 
		query.append("           SUM(DECODE(D.TS_FLG, 'N', D.BKG_AVAL_TTL_WGT)) AS WGT," ).append("\n"); 
		query.append("           SUM(DECODE(D.TS_FLG, 'Y', D.BKG_AVAL_TTL_QTY)) AS ALC_TS_VOL," ).append("\n"); 
		query.append("           SUM(DECODE(D.TS_FLG, 'Y', D.BKG_AVAL_TTL_WGT)) AS ALC_TS_WGT " ).append("\n"); 
		query.append("      FROM VVDS             M," ).append("\n"); 
		...
		query.append("       AND D.POD_YD_CD IN (SELECT V.YD_CD" ).append("\n"); 
		query.append("                             FROM VSK_VSL_PORT_SKD V" ).append("\n"); 
		query.append("                            WHERE V.SLAN_CD = SUBSTR(M.RLANE_CD, 1, 3)" ).append("\n"); 
		...
		query.append("       -- POD와 동일하게 POL도 YARD 존재 여부 CHECK" ).append("\n"); 
		query.append("       AND D.POL_YD_CD IN (SELECT V.YD_CD" ).append("\n"); 
		query.append("                             FROM VSK_VSL_PORT_SKD V" ).append("\n"); 
		query.append("                            WHERE V.SLAN_CD = SUBSTR(M.RLANE_CD, 1, 3)" ).append("\n"); 
		query.append("                              AND V.VSL_CD = M.VSL_CD" ).append("\n"); 
		query.append("                              AND V.SKD_VOY_NO = M.SKD_VOY_NO" ).append("\n"); 
		query.append("                              AND V.SKD_DIR_CD = M.SKD_DIR_CD)" ).append("\n"); 
		query.append("       --AND SUBSTR(M.RLANE_CD, 1, 3) = V.SLAN_CD" ).append("\n"); 
		...
		query.append("       --AND SUBSTR(D.POD_YD_CD, 1, LENGTH(NVL(V.YD_CD, V.VPS_PORT_CD))) = NVL(V.YD_CD, V.VPS_PORT_CD)" ).append("\n"); 
		query.append("  GROUP BY M.TRD_CD    ," ).append("\n"); 
		query.append("           M.SUB_TRD_CD," ).append("\n"); 
		query.append("           M.RLANE_CD  ," ).append("\n"); 
		query.append("           M.DIR_CD    ," ).append("\n"); 
		...
		query.append("           NVL(SUM(BKG_VOL_VGM),0) AS BKG_VOL_VGM,NVL(SUM(BKG_WGT_VGM),0) AS BKG_WGT_VGM," ).append("\n"); 
		query.append("           SUM(BKG_TS_VOL) AS BKG_TS_VOL," ).append("\n"); 
		query.append("           SUM(BKG_TS_WGT) AS BKG_TS_WGT," ).append("\n"); 
		query.append("           NVL(SUM(BKG_TS_VOL_VGM),0) AS BKG_TS_VOL_VGM,NVL(SUM(BKG_TS_WGT_VGM),0) AS BKG_TS_WGT_VGM" ).append("\n"); 
		...
		query.append("                   DECODE(TS_FLG, 'N', BKG_VOL_VGM) AS BKG_VOL_VGM," ).append("\n"); 
		query.append("				   DECODE(TS_FLG, 'N', BKG_WGT_VGM) AS BKG_WGT_VGM," ).append("\n"); 
		...
		query.append("                   DECODE(TS_FLG, 'Y', BKG_VOL_VGM) AS BKG_TS_VOL_VGM," ).append("\n"); 
		query.append("				   DECODE(TS_FLG, 'Y', BKG_WGT_VGM) AS BKG_TS_WGT_VGM" ).append("\n"); 
		query.append("              FROM (" ).append("\n"); 
		query.append("                   SELECT M.TRD_CD    ," ).append("\n"); 
		query.append("                       M.SUB_TRD_CD," ).append("\n"); 
		query.append("                       M.RLANE_CD  ," ).append("\n"); 
		...
		query.append("       AND D.POL_YD_CD <> '0000000' " ).append("\n"); 
		query.append("  GROUP BY M.TRD_CD    ," ).append("\n"); 
		query.append("           M.SUB_TRD_CD," ).append("\n"); 
		query.append("           M.RLANE_CD  ," ).append("\n"); 
		query.append("           M.DIR_CD    ," ).append("\n"); 
		...
		query.append("         SUM(BKG_WGT) AS BKG_WGT, NVL(SUM(BKG_VOL_VGM),0) AS BKG_VOL_VGM, NVL(SUM(BKG_WGT_VGM),0) AS BKG_WGT_VGM, " ).append("\n"); 
		query.append("         SUM(BKG_TS_VOL)          AS BKG_TS_VOL," ).append("\n"); 
		query.append("         SUM(BKG_TS_WGT)          AS BKG_TS_WGT,NVL(SUM(BKG_TS_VOL_VGM),0) AS BKG_TS_VOL_VGM,NVL(SUM(BKG_TS_WGT_VGM),0) AS BKG_TS_WGT_VGM," ).append("\n"); 
		query.append("         " ).append("\n"); 
		...
		query.append("                   NULL      ," ).append("\n"); 
		query.append("                   NULL      ," ).append("\n"); 
		query.append("                   NULL     ,NULL     ,NULL     ," ).append("\n"); 
		query.append("                   NULL AS BKG_TS_VOL," ).append("\n"); 
		query.append("                   NULL AS BKG_TS_WGT ,NULL AS BKG_TS_VOL_VGM  ,NULL AS BKG_TS_WGT_VGM                            " ).append("\n"); 
		...
		query.append("                   WGT,BKG_VOL_VGM,BKG_WGT_VGM," ).append("\n"); 
		query.append("                   BKG_TS_VOL AS BKG_TS_VOL," ).append("\n"); 
		query.append("                   BKG_TS_WGT AS BKG_TS_WGT," ).append("\n"); 
		query.append("				   BKG_TS_VOL_VGM AS BKG_TS_VOL_VGM,BKG_TS_WGT_VGM AS BKG_TS_WGT_VGM" ).append("\n"); 

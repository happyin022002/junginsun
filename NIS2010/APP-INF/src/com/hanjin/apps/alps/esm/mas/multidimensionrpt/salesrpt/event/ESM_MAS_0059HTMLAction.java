...
*@FileName : ESM_MAS_0059HTMLAction.java
 ...
 * @author Kim Ki Dae
 * @see EsmMas0059Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0059HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0059HTMLAction 객체를 생성
	 */
	public ESM_MAS_0059HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0060Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0059Event event = new EsmMas0059Event();  		
		...
		if(command.isCommand(FormCommand.SEARCHLIST)) {
			...
			event.setMasRptItmInfoMstVOs((MasRptItmInfoMstVO[])getVOs(request, MasRptItmInfoMstVO.class));
			event.setMasRptItmInfoDtlVOs((MasRptItmInfoDtlVO[])getVOs(request, MasRptItmInfoDtlVO.class));
		}		
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setSalesRPTCommonVO((SalesRPTCommonVO)getVO(request, SalesRPTCommonVO.class));

...
public class KorCustomsTransmissionDBDAOmakeDiscBodyFlatFileRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            KT.CSTMS_CRR_IN_LOC_WH_CD || '~' || /* CARRY-IN CY CODE*/" ).append("\n"); 
		query.append("            (SELECT SUBSTR(LOC_NM, 1, 50)" ).append("\n"); 
		query.append("             FROM BKG_DCHG_LOC" ).append("\n"); 
		query.append("             WHERE OTR_DCHG_CD = KT.CSTMS_CRR_IN_LOC_WH_CD" ).append("\n"); 
		query.append("             AND (PORT_CD = '*' OR PORT_CD = @[port_cd])" ).append("\n"); 
		query.append("             AND (SKD_DIR_CD = '*' OR SKD_DIR_CD = SUBSTR(@[vvd], 9, 1))" ).append("\n"); 
		query.append("             AND (SLAN_CD = '*' OR SLAN_CD = (SELECT VSL_SLAN_CD" ).append("\n"); 
		...
		query.append("             AND ROWNUM = 1" ).append("\n"); 
		query.append("             )|| '~' || /* CARRY-IN CY NAME */" ).append("\n"); 

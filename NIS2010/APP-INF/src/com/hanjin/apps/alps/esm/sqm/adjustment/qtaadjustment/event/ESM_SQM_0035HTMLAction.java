...
*@FileName       : ESM_SQM_0035HTMLAction.java
*@FileTitle      : QTA Edit
*Open Issues     :
*Change history  :
 ...
 * @author JEONGMIN CHO
 * @see EsmSqm0035Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0035HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SQM_0035HTMLAction 객체를 생성
	 */
	public ESM_SQM_0035HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0035Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmSqm0035Event event = new EsmSqm0035Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI) || command.isCommand(FormCommand.MULTI01) || command.isCommand(FormCommand.MULTI02) || command.isCommand(FormCommand.MULTI03)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setSqmAlocQtaVOS((SqmAlocQtaVO[])getVOs(request, SqmAlocQtaVO .class,""));
		}

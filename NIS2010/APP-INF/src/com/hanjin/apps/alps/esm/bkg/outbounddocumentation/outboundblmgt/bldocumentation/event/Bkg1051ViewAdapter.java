 ...
 * @since J2EE 1.5
 */
public class Bkg1051ViewAdapter extends ViewAdapter {
    ...
    protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
        ...
        for(int i = 0; i < realCnt; i++) {
            ...
            for(int j = 0; j < colCnt; j++) {
                sbufXML.append(("0".equals(colValues.get(realColNms[j]))) ? "" : colValues.get(realColNms[j]));
                if((j + 1) != colCnt) sbufXML.append(DELIMITER);
            }
            sbufXML.append("]]></TR>\n");

            sbufXML.append("    <TR><![CDATA[");
            for(int j = 0; j < colCnt; j++) {
                if("cntr_tpsz_cd".equals(realColNms[j])) sbufXML.append(this.nvl(colValues.get(realColNms[j])));
                else if("category".equals(realColNms[j])) sbufXML.append("CNTR");
                else sbufXML.append("");
                if((j + 1) != colCnt) sbufXML.append(DELIMITER);
            }
            sbufXML.append("]]></TR>\n");
            ...
            for(int j = 0; j < colCnt; j++) {
                if("cntr_tpsz_cd".equals(realColNms[j])) sbufXML.append(this.nvl(colValues.get(realColNms[j])));
                else if("category".equals(realColNms[j])) sbufXML.append("Difference");
                else sbufXML.append("");
                if((j + 1) != colCnt) sbufXML.append(DELIMITER);
            }
            sbufXML.append("]]></TR>\n");
        ...
        try {
            ...
            while(rs.next()) {
                ...
                for(int j = 1; j < colCount; j++) {
                    sb.append((("0".equals(rs.getObject(j))) ? "" : rs.getObject(j)) + DELIMITER);
                }

...
public class ESM_COA_0136HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0136HTMLAction 객체를 생성
	 */
	public ESM_COA_0136HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmCoa0136Event event = new EsmCoa0136Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCoaCntrRepoRoutEccVOS((CoaCntrRepoRoutEccVO[])getVOs(request, CoaCntrRepoRoutEccVO .class,""));
			SearchConditionVO svo = (SearchConditionVO)getVO(request, SearchConditionVO .class);
			svo.unDataFormat();
			event.setSearchConditionVO(svo);
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setSearchEQBalance0136ListVO((SearchEQBalance0136ListVO)getVO(request, SearchEQBalance0136ListVO .class));
			SearchConditionVO svo = (SearchConditionVO)getVO(request, SearchConditionVO .class);
			svo.unDataFormat();
			event.setSearchConditionVO(svo);
		}

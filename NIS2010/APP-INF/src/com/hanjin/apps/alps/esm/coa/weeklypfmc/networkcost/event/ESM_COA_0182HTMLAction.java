...
*@FileName : ESM_COA_0182HTMLAction.java
*@FileTitle : Average 
*Open Issues :
*Change history :
...
public class ESM_COA_0182HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0182HTMLAction 객체를 생성
	 */
	public ESM_COA_0182HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmCoa0182Event event = new EsmCoa0182Event();
		
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setCoaOpAvgUtCostVOS((CoaOpAvgUtCostVO[])getVOs(request, CoaOpAvgUtCostVO.class));
		}else if(command.isCommand(FormCommand.MULTI02)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setCoaOpAvgUtCostVO((CoaOpAvgUtCostVO)getVOs(request, CoaOpAvgUtCostVO.class));
		}else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}

...
*@FileName : ESM_PRI_0040HTMLAction.java
...
public class ESM_PRI_0040HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_0040HTMLAction 객체를 생성
	 */
	public ESM_PRI_0040HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri0040Event event = new EsmPri0040Event();		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setRsltRealCustInquiryVO((RsltRealCustInquiryVO)getVO(request, RsltRealCustInquiryVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setSchCustVO((SchCustVO)getVO(request, SchCustVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setPriSpRealCustVO((PriSpRealCustVO)getVO(request, PriSpRealCustVO .class));	
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setRsltRealCustInquiryVO((RsltRealCustInquiryVO)getVO(request, RsltRealCustInquiryVO .class));
			PriSpRealCustVO[] vo = (PriSpRealCustVO[])getVOs(request, PriSpRealCustVO.class,"");
			if( vo != null ){
				for(int i = 0 ; i < vo.length ; i++){
					if("1".equals(vo[i].getRepCustFlg())){
						vo[i].setRepCustFlg("Y");
					}else{
						vo[i].setRepCustFlg("N");
					}
				}
			}
			event.setPriSpRealCustVOs(vo);

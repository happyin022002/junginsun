...
*@FileName : ESM_MAS_0036HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Kim Ki Dae
 * @see EsmMas0036Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0036HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0036HTMLAction 객체를 생성
	 */
	public ESM_MAS_0036HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0036Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0036Event event = new EsmMas0036Event();  		
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setMasLaneRgstVOs((MasLaneRgstVO[])getVOs(request, MasLaneRgstVO.class));
		}	
		else if(command.isCommand(FormCommand.MULTI02)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setComboVO((EsmMas0036ComboVO)getVO(request, EsmMas0036ComboVO.class));
		}
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}

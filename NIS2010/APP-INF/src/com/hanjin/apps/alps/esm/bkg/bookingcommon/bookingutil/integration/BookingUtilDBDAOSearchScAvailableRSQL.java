...
*@FileName : BookingUtilDBDAOSearchScAvailableRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BookingUtilDBDAOSearchScAvailableRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  *    - BKG의 application date가 아닌 S/C Expiration date를 가지고 SC NO 확인 로직 실행
	  * </pre>
	  */
	public BookingUtilDBDAOSearchScAvailableRSQL(){
		setQuery();
		...
		query.append("FileName : BookingUtilDBDAOSearchScAvailableRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                  end appl_dt" ).append("\n"); 
		query.append("           from" ).append("\n"); 
		query.append("            (select appl_dt,(select max(d.ctrt_exp_dt)" ).append("\n"); 
		query.append("                             from pri_sp_hdr h, pri_sp_mn m, pri_sp_dur d" ).append("\n"); 
		...
		query.append("				SELECT 0.5 RANK, LAST_DAY(ADD_MONTHS(TO_DATE(BHCC.ATTR_CTNT9,'YYYYMMDD'),-1)) appl_dt " ).append("\n"); 
		query.append("				FROM BKG_HRD_CDG_CTNT BHCC" ).append("\n"); 
		query.append("				WHERE BHCC.HRD_CDG_ID = 'NO_RATE_PROC_CTR'" ).append("\n"); 
		query.append("				AND   BHCC.ATTR_CTNT8 LIKE '%'||NVL(@[sc_no],'X123456789')||'%'" ).append("\n"); 
		query.append("				AND  SYSDATE <= TO_DATE(BHCC.ATTR_CTNT9,'YYYYMMDD') + 0.99999" ).append("\n"); 

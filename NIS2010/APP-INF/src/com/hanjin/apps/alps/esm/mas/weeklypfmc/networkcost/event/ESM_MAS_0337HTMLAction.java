...
**@FileName : ESM_MAS_0337HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0337Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0337HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0337HTMLAction 객체를 생성
	 */
	public ESM_MAS_0337HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0337Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0337Event event = new EsmMas0337Event();  	
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)) { // retrieve		
 		...
 		} else if(command.isCommand(FormCommand.COMMAND01)) { // creation
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));			
		} else if(command.isCommand(FormCommand.MULTI01)) { // save
            event.setMasGenExpnAsgnVOs((MasGenExpnAsgnVO[])getVOs(request, MasGenExpnAsgnVO.class));         
        }

...
public class PerformanceReportDBDAOSaelsPerformanceReportByDownLoadRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("         ,SUBSTR(REPLACE(TRANSLATE(NVL(CUSTOMER.CUST_LGL_ENG_NM, ' '), CHR(13)||CHR(10), ' '), CHR(34), ' '), 1, 30) CUST_NM" ).append("\n"); 
		...
		query.append("         ,SUBSTR(REPLACE(TRANSLATE(NVL(DECODE(BKG.CUST_TO_ORD_FLG, 'Y', N1.CUST_NM, C1.CUST_NM), ' '), CHR(13)||CHR(10), ' '), CHR(34), ' '), 1, 30) CNNF_NM" ).append("\n"); 
		query.append("         ,F1.CUST_CNT_CD||TO_CHAR(F1.CUST_SEQ) FORWARDER_CD" ).append("\n"); 
		query.append("         ,SUBSTR(REPLACE(TRANSLATE(NVL(F1.CUST_NM,' '), CHR(13)||CHR(10), ' '), CHR(34), ' '), 1, 30) FORWARDER_NM" ).append("\n"); 
		...
		query.append("         ,BKG_CUSTOMER F1         " ).append("\n"); 
		...
		query.append("         ,MAS_RGST_BKG REV" ).append("\n"); 
		...
		query.append("   AND    BKG.BKG_NO = F1.BKG_NO(+)" ).append("\n"); 
		query.append("   AND    F1.BKG_CUST_TP_CD(+) = 'F'" ).append("\n"); 
		...
		query.append("   AND REV.IOC_CD ='O'" ).append("\n"); 
		query.append("#elseif (${ioc_cd} != '' && ${ioc_cd} == 'I')" ).append("\n"); 
		query.append("   --AND    SUBSTR(BKG.ORG_SCONTI_CD,0,1) = SUBSTR(BKG.DEST_SCONTI_CD,0,1)" ).append("\n"); 
		query.append("   AND REV.IOC_CD ='I'" ).append("\n"); 
		query.append("#end   " ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("         ,A.FORWARDER_CD" ).append("\n"); 
		query.append("         ,A.FORWARDER_NM" ).append("\n"); 
		...
		query.append("                    NVL(SUM(CASE WHEN CNTR_TPSZ_CD IN ('D2','D3','C2')                                              THEN CNTR_VOL_QTY END),0)  D2," ).append("\n"); 
		query.append("                    NVL(SUM(CASE WHEN CNTR_TPSZ_CD IN ('D4','D6','D8','D9','DX','DY','DW','DZ','C4')     THEN CNTR_VOL_QTY END),0)  D4," ).append("\n"); 
		...
		query.append("                    ----------------------------------------------" ).append("\n"); 
		query.append("                    NVL(SUM(CASE WHEN CNTR_TPSZ_CD IN ('D2','D3','C2')                                          THEN CNTR_VOL_QTY END),0)  +" ).append("\n"); 
		query.append("                    NVL(SUM(CASE WHEN CNTR_TPSZ_CD IN ('D4','D5','D6','D7','D8','D9','DX','DY','DW','DZ','C4')  THEN CNTR_VOL_QTY END),0) * 2 +" ).append("\n"); 
		...
		query.append("                    M.BKG_NO," ).append("\n"); 
		query.append("                    NVL(SUM(CASE WHEN C.RAT_UT_CD IN ('D2','D3','C2')                                             THEN C.CHG_AMT/NVL(A3.USD_LOCL_XCH_RT,1) END),0)  REV_D2," ).append("\n"); 
		query.append("                    NVL(SUM(CASE WHEN C.RAT_UT_CD IN ('D4','D6','D8','D9','DX','DY','DW','DZ','C4')    THEN C.CHG_AMT/NVL(A3.USD_LOCL_XCH_RT,1) END),0)  REV_D4," ).append("\n"); 
		...
		query.append("             AND    CH.HJS_CHG_ACCT_CD IN ('411111','411914','411915')          --> 조건 확인할 것" ).append("\n"); 

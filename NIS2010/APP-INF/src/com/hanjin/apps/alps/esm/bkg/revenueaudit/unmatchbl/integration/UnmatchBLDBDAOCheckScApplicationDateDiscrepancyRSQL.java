...
*@FileName : UnmatchBLDBDAOCheckScApplicationDateDiscrepancyRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class UnmatchBLDBDAOCheckScApplicationDateDiscrepancyRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ApplicationDateDiscrepancy
	  * 
	  * [20170417]  김동호 Application Date가 맞지 않아도
	  *                             BKG Rate 기준 계약의 Amend No에 해당하는
	  *                             유효기간 내에 있으면 A2에러 무시
	  *                             (김경남 부장님 요청)
	  * [20170511]  김동호 04/17 요청 사항 철회 (김경남 부장님 요청)
	  * </pre>
	  */
	public UnmatchBLDBDAOCheckScApplicationDateDiscrepancyRSQL(){
		setQuery();
		...
		query.append("FileName : UnmatchBLDBDAOCheckScApplicationDateDiscrepancyRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                      BK.POL_ETD_DT BETWEEN SD.CTRT_EFF_DT AND SD.CTRT_EXP_DT" ).append("\n"); 
		query.append("                  OR  BK.POL_ETB_DT BETWEEN SD.CTRT_EFF_DT AND SD.CTRT_EXP_DT" ).append("\n"); 
		query.append("                  )" ).append("\n"); 
		query.append("          THEN  'Y'" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("SELECT  CASE" ).append("\n"); 
		query.append("        WHEN  BK.SC_NO IN ( SELECT ATTR_CTNT2 FROM BKG_HRD_CDG_CTNT" ).append("\n"); 
		query.append("                             WHERE HRD_CDG_ID ='AUD_EXPT_LIST'" ).append("\n"); 
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		query.append("                                  ) AND BK.REV_APLY_TP_CD ='C' " ).append("\n"); 
		query.append("          THEN  '[POL ETD][' || TO_CHAR(BK.POL_ETD_DT, 'YYYY-MM-DD') || ']'" ).append("\n"); 
		query.append("        WHEN  BK.SC_NO NOT IN (  SELECT ATTR_CTNT2 FROM BKG_HRD_CDG_CTNT" ).append("\n"); 
		query.append("                             WHERE HRD_CDG_ID ='AUD_EXPT_LIST'" ).append("\n"); 
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		query.append("                                  ) AND BK.REV_APLY_TP_CD = 'C'" ).append("\n"); 
		query.append("          THEN  '[CRD][' || TO_CHAR(BK.CGO_RCV_DT, 'YYYY-MM-DD') || ']'" ).append("\n"); 
		...
		query.append("        AND     SD.SVC_SCP_CD   IN (BK.SVC_SCP_CD,DECODE(BK.SVC_SCP_CD,'ACE','TPE','MXE','TPE'))" ).append("\n"); 
		query.append("        ) SD" ).append("\n"); 
		...
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		query.append("                                  ) AND BK.REV_APLY_TP_CD ='C' " ).append("\n"); 
		query.append("          THEN  '[POL ETD][' || TO_CHAR(BK.POL_ETD_DT, 'YYYY-MM-DD') || ']' || CHR(10) || '[Application Date][' || TO_CHAR(BK.RT_APLY_DT, 'YYYY-MM-DD') || ']'" ).append("\n"); 
		query.append("        WHEN  BK.SC_NO NOT IN (  SELECT ATTR_CTNT2 FROM BKG_HRD_CDG_CTNT" ).append("\n"); 
		query.append("                             WHERE HRD_CDG_ID ='AUD_EXPT_LIST'" ).append("\n"); 
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		query.append("                                  ) AND BK.REV_APLY_TP_CD = 'C'" ).append("\n"); 
		...
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		query.append("                              ) AND BK.REV_APLY_TP_CD ='C' " ).append("\n"); 
		query.append("			  AND NVL(BK.POL_ETD_DT, BK.RT_APLY_DT) = BK.RT_APLY_DT" ).append("\n"); 
		query.append("          THEN  'Y'" ).append("\n"); 
		query.append("        WHEN   BK.SC_NO NOT IN (  SELECT ATTR_CTNT2 FROM BKG_HRD_CDG_CTNT" ).append("\n"); 
		query.append("                             WHERE HRD_CDG_ID ='AUD_EXPT_LIST'" ).append("\n"); 
		query.append("                               AND ATTR_CTNT1 ='S' -- Hard Coding 제거 " ).append("\n"); 
		...
		query.append("          THEN  'Y'" ).append("\n"); 
		query.append("        WHEN      BK.REV_APLY_TP_CD <> 'C'" ).append("\n"); 
		query.append("              AND BK.RT_APLY_DT BETWEEN BK.POL_ETB_DT AND BK.POL_ETD_DT" ).append("\n"); 
		query.append("          THEN  'Y'" ).append("\n"); 
		query.append("        ELSE  'N'" ).append("\n"); 
		query.append("        END" ).append("\n"); 
		query.append("        = 'N'" ).append("\n"); 

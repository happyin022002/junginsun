...
public class SCReportDBDAORsltSearchSCPerformanceDetailListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 
	  * * History--------------------------------------------------
	  * * 2011.02.17 이행지 [CHM-201109050-01] S/C Performance Summary 화면의 Display 체크관련 수정
	...
	public void setQuery(){
		...
		query.append("                WHEN  L3.CNT_CD IN ( 'US', 'CA' ) THEN NVL(( SELECT SUBSTR(SVC_MOD_CD, 2, 2) FROM MAS_USA_SVC_MOD A WHERE A.ORG_RGN_CD = L3.RGN_CD AND A.DEST_RGN_CD = L4.RGN_CD ), 'OT' )" ).append("\n"); 
		query.append("                WHEN  L2.CNT_CD IN ( 'US', 'CA' ) AND BK.POL_CD = BK.POR_CD AND BK.RCV_TERM_CD NOT IN ( 'D', 'H' ) THEN 'PO'" ).append("\n"); 
		query.append("                WHEN  L2.CNT_CD IN ( 'US', 'CA' ) THEN NVL(( SELECT SUBSTR(SVC_MOD_CD, 2, 2) FROM MAS_USA_SVC_MOD A WHERE A.ORG_RGN_CD = L2.RGN_CD AND A.DEST_RGN_CD = L1.RGN_CD ), 'OT' )" ).append("\n"); 
		...
		query.append("	#if (${chk_vvd} != '' && ${chk_slan_cd} != '' )" ).append("\n"); 
		query.append("	COA_MON.COST_YRMON	," ).append("\n"); 
		query.append("	COA_MON.COST_WK		," ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("	RSLT.TRD_CD            ," ).append("\n"); 
		query.append("	RSLT.SKD_DIR_CD        ," ).append("\n"); 
		query.append("	RSLT.SUB_TRD_CD        ," ).append("\n"); 
		query.append("	RSLT.ORG_CNT," ).append("\n"); 
		query.append("	#if (${chk_slan_cd} != '')" ).append("\n"); 
		query.append("	RSLT.SLAN_CD           ," ).append("\n"); 
		query.append("	#end	" ).append("\n"); 
		query.append("	#if (${chk_vvd} != '')" ).append("\n"); 
		query.append("	RSLT.VVD               ," ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if (${chk_gen_spcl_rt_tp_cd} != '')" ).append("\n"); 
		query.append("	RSLT.GEN_SPCL_RT_TP_CD ," ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if (${chk_cmdt_nm} != '')" ).append("\n"); 
		query.append("	RSLT.CMDT_NM           ," ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if (${chk_act_cust_nm} != '')" ).append("\n"); 
		query.append("	RSLT.ACT_CUST_NM       ," ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if (${chk_usa_svc_mod_cd} != '')" ).append("\n"); 
		query.append("	RSLT.USA_SVC_MOD_CD    ," ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		...
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	RSLT.SVC_SCP_CD        ," ).append("\n"); 
		...
		query.append("			CMDT_CD           ," ).append("\n"); 
		query.append("			USA_SVC_MOD_CD    ," ).append("\n"); 
		query.append("            SUBSTR(POR_CD,1,2) AS ORG_CNT," ).append("\n"); 
		query.append("			POR_CD            ," ).append("\n"); 
		query.append("			POL_CD            ," ).append("\n"); 
		query.append("			POD_CD            ," ).append("\n"); 
		query.append("			DEL_CD            ," ).append("\n"); 
		...
		query.append("#if (${chk_slan_cd} != '' && ${chk_vvd} != '')" ).append("\n"); 
		query.append(", MAS_MON_VVD COA_MON" ).append("\n"); 
		query.append("WHERE COA_MON.VSL_CD =  SUBSTR(RSLT.VVD,1,4) " ).append("\n"); 
		query.append("	AND COA_MON.SKD_VOY_NO =  SUBSTR(RSLT.VVD,5,4)" ).append("\n"); 
		query.append("	AND COA_MON.DIR_CD =  SUBSTR(RSLT.VVD,-1)" ).append("\n"); 
		query.append("	AND COA_MON.TRD_CD = RSLT.TRD_CD" ).append("\n"); 
		query.append("	AND COA_MON.SLAN_CD = RSLT.SLAN_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("ORDER BY" ).append("\n"); 
		query.append("#if (${chk_slan_cd} != '' && ${chk_vvd} != '')" ).append("\n"); 
		query.append("	COA_MON.COST_YRMON	," ).append("\n"); 

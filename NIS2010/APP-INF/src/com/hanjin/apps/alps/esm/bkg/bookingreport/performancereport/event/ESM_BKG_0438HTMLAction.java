...
*@FileName : ESM_BKG_0438HTMLAction.java
 ...
 * @see EsmBkg0438Event 참조
 * @since J2EE 1.5
 */

public class ESM_BKG_0438HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0421HTMLAction 객체를 생성
     */
    public ESM_BKG_0438HTMLAction() {}

    /**
     * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
     * HttpRequst의 정보를 EsmBkg0438Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0438Event event = new EsmBkg0438Event();
		...
		//}else if (command.isCommand(FormCommand.SEARCH01)){//User Group Id Search
			...
			event.setInfoVO((DocQueueDetailListVO) getVO(request, DocQueueDetailListVO.class));
		}else if (command.isCommand(FormCommand.MODIFY03)){//User Group Id Search
			event.setInfoVO((DocQueueDetailListVO) getVO(request, DocQueueDetailListVO.class));	
		}else if (command.isCommand(FormCommand.MODIFY04)){//User Group Id Search
			event.setInfoVO((DocQueueDetailListVO) getVO(request, DocQueueDetailListVO.class));
		...
		event.setCallPgmType(JSPUtil.getParameter(request, "call_pgm_type"));

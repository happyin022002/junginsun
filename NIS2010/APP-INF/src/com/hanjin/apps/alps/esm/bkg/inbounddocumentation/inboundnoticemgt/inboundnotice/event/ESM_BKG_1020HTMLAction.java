...
*@FileName : ESM_BKG_1020HTMLAction.java
...
public class ESM_BKG_1020HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1020HTMLAction 객체를 생성
	 */
	public ESM_BKG_1020HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1020Event event = new EsmBkg1020Event();
		
		// I/B 저장
		if(command.isCommand(FormCommand.REMOVE)) {
			
			event.setArrNtcWdVOs((BkgArrNtcWdVO[])getVOs(request, BkgArrNtcWdVO.class,"sheet1_"));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setArrNtcWdVOs((BkgArrNtcWdVO[])getVOs(request, BkgArrNtcWdVO.class,"sheet1_"));
			
		}else if(command.isCommand(FormCommand.SEARCH)) {
			event.setArrNtcWdVO((BkgArrNtcWdVO) getVO(request, BkgArrNtcWdVO.class));

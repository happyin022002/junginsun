...
public class CostStructureDBDAOMultiCostStructure0137CSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * History--------------------------------
	  * 2011.06.21 이행지 [CHM-201111781-01] [MAS] 평균단가 입력화면 R/Lane 조건추가
	  * 2011.06.34 이행지 [CHM-201111807-01] MAS NODE 단가 화면 저장시 발생될 수 있는 문제 해결- R.Lane컬럼 NVL처리
	  * 2014.08.27 박은주 [CHM-201431751] [MAS] Link U/C Adjustment 조회/입력조건의 Vendor 추가요청
	  * 2014.10.06 박은주 [CHM-201432147] [MAS] Node/Link U/C Adjustment 화면 : COST_ACT_GRP_CD 추가요청
	  * </pre>
	  */
	...
	public void setQuery(){
		query.append("#if (${f_table_name} != 'MAS_NOD_AVG_STND_COST') " ).append("\n"); 
		query.append("MERGE INTO MAS_LNK_AVG_STND_COST B1	" ).append("\n"); 
		...
		query.append("              ,DECODE(@[cost_fx_flg], '1', 'Y', 'N')  COST_FX_FLG" ).append("\n"); 
		...
		query.append("  AND  B1.VNDR_SEQ            = B2.VNDR_SEQ" ).append("\n"); 
		...
		query.append("    (COST_YRMON    , LNK_FM_NOD_CD, LNK_TO_NOD_CD    , CO_CD            , CNTR_TPSZ_CD, FULL_MTY_CD , MAS_COST_SRC_CD,	" ).append("\n"); 
		query.append("    COST_LOC_GRP_CD, VNDR_SEQ	  , COST_ACT_GRP_CD  , LOCL_CURR_CD     , STND_COST_USD_AMT, LNK_TTL_QTY , LNK_TTL_AMT , COST_VOL_CD,						" ).append("\n"); 
		query.append("    CRE_USR_ID     , CRE_DT       , UPD_USR_ID       , UPD_DT	        , COST_FX_FLG , B1.COST_CALC_RMK)" ).append("\n"); 
		query.append("    VALUES																											" ).append("\n"); 
		query.append("    (B2.COST_YRMON    , B2.LNK_FM_NOD_CD, B2.LNK_TO_NOD_CD    , B2.CO_CD            , B2.CNTR_TPSZ_CD, B2.FULL_MTY_CD , B2.MAS_COST_SRC_CD,	" ).append("\n"); 
		query.append("    B2.COST_LOC_GRP_CD, B2.VNDR_SEQ     , B2.COST_ACT_GRP_CD  , B2.LOCL_CURR_CD     , B2.STND_COST_USD_AMT, B2.LNK_TTL_QTY , B2.LNK_TTL_AMT , B2.COST_VOL_CD,						" ).append("\n"); 
		...
		query.append("MERGE INTO MAS_NOD_AVG_STND_COST B1							" ).append("\n"); 
		query.append("USING ( SELECT @[cost_yrmon]  COST_YRMON" ).append("\n"); 
		query.append("              ,@[full_mty_cd]  FULL_MTY_CD" ).append("\n"); 
		query.append("              ,@[cntr_tpsz_cd]  CNTR_TPSZ_CD" ).append("\n"); 
		query.append("              ,@[cost_loc_grp_cd]  COST_LOC_GRP_CD" ).append("\n"); 
		query.append("              ,NVL(@[slan_cd],'NNN') SLAN_CD" ).append("\n"); 
		...
		query.append("WHEN NOT MATCHED THEN" ).append("\n"); 
		query.append("    INSERT" ).append("\n"); 
		query.append("    (B1.COST_YRMON       , B1.FULL_MTY_CD, B1.CNTR_TPSZ_CD, B1.COST_LOC_GRP_CD, B1.SLAN_CD, B1.NOD_CD    , B1.MAS_COST_SRC_CD, B1.LOCL_CURR_CD," ).append("\n"); 
		...
		query.append("    VALUES" ).append("\n"); 
		query.append("    (B2.COST_YRMON       , B2.FULL_MTY_CD, B2.CNTR_TPSZ_CD, B2.COST_LOC_GRP_CD, NVL(B2.SLAN_CD,'NNN'), B2.NOD_CD    , B2.MAS_COST_SRC_CD, B2.LOCL_CURR_CD," ).append("\n"); 

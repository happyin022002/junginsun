...
*@FileName : IHCGuideLineDBDAOModifyIHCGuidelineDetailLocalConfirmUSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class IHCGuideLineDBDAOModifyIHCGuidelineDetailLocalConfirmUSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public IHCGuideLineDBDAOModifyIHCGuidelineDetailLocalConfirmUSQL(){
		setQuery();
		...
		query.append("FileName : IHCGuideLineDBDAOModifyIHCGuidelineDetailLocalConfirmUSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("   SET ( GLINE_LOCL_CURR_DG_20FT_AMT" ).append("\n"); 
		...
		query.append("             , DECODE(DCGO_SVC_FLG_45, 'Y' , SUB.GLINE_DG_45FT ,  null ) -- 45' Cost 추가" ).append("\n"); 
		...
		query.append("                SELECT DECODE(CGO1.DG_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_20FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO1.DG_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_20FT_AMT + (RT.GLINE_LOCL_CURR_20FT_AMT * CGO1.DG_RT_RTO / 100)) GLINE_DG_20FT" ).append("\n"); 
		query.append("                     , DECODE(CGO2.DG_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_40FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO2.DG_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_40FT_AMT + (RT.GLINE_LOCL_CURR_40FT_AMT * CGO2.DG_RT_RTO / 100)) GLINE_DG_40FT" ).append("\n"); 
		query.append("                     , DECODE(CGO3.DG_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_45FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO3.DG_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_45FT_AMT + (RT.GLINE_LOCL_CURR_45FT_AMT * CGO3.DG_RT_RTO / 100)) GLINE_DG_45FT -- 45' Cost 추가" ).append("\n"); 
		query.append("                     , DECODE(CGO1.OVR_WGT_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_20FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO1.OVR_WGT_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_20FT_AMT + (RT.GLINE_LOCL_CURR_20FT_AMT * CGO1.OVR_WGT_RT_RTO / 100)) GLINE_OV_20FT" ).append("\n"); 
		query.append("                     , DECODE(CGO2.OVR_WGT_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_40FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO2.OVR_WGT_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_40FT_AMT + (RT.GLINE_LOCL_CURR_40FT_AMT * CGO2.OVR_WGT_RT_RTO / 100)) GLINE_OV_40FT" ).append("\n"); 
		query.append("                     , DECODE(CGO3.OVR_WGT_FLT_PCT_TP_CD, 'F', RT.GLINE_LOCL_CURR_45FT_AMT + ROUND(TPB_GET_LOCL_AMT_FNC(CGO3.OVR_WGT_RT_AMT, RT.LOCL_CURR_CD," ).append("\n"); 
		query.append("                       (SELECT TO_DATE(MAX(R.ACCT_XCH_RT_YRMON), 'YYYYMM') ACCT_XCH_RT_YRMON FROM GL_MON_XCH_RT R WHERE R.ACCT_XCH_RT_LVL = '1' AND DELT_FLG = 'N' AND R.CURR_CD = RT.LOCL_CURR_CD )), 0)," ).append("\n"); 
		query.append("                              'P', RT.GLINE_LOCL_CURR_45FT_AMT + (RT.GLINE_LOCL_CURR_45FT_AMT * CGO3.OVR_WGT_RT_RTO / 100)) GLINE_OV_45FT -- 45' Cost 추가" ).append("\n"); 
		...
		query.append("                   AND CGO3.PRC_INLND_TRF_CNTR_TPSZ_CD(+) = '45'" ).append("\n"); 
		query.append("                   " ).append("\n"); 

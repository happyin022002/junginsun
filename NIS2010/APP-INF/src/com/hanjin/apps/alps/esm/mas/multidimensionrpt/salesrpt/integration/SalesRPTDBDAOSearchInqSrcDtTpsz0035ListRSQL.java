...
public class SalesRPTDBDAOSearchInqSrcDtTpsz0035ListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 2015.05.14 [CHM-201535771] 손진환 [MAS] CGO RCV DT를 부킹 모듈 상에서 OC Status 발생 date를 바로 바라볼 수 있도록 로직 수정
	  * 2015.05.14 [CHM-201535424] 손진환 [MAS] COA 상 Fixed Rate 반영
	  * 2015.05.27 [CHM-201536029] 손진환 [MAS] COA 상 Fixed Rate 반영 요청 CSR
	  * 2015.07.06 [] 김시몬 CM Cost Total에서 DEMDET, MIS REV 제외에 따른 보완
	...
	public void setQuery(){
		...
		query.append("      , 'Source' MST_RFA_ROUT_ID" ).append("\n"); 
		...
		query.append("      , 'G/Customer Type(C.Customer)' AGMT_CUST_TP_CD" ).append("\n"); 
		...
		query.append("    --  ,'CM(BKG) Total' CM_BKG_TOTAL" ).append("\n"); 
		query.append("			" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("      #foreach($key in ${allcols}) " ).append("\n"); 
		query.append("      ,'OP_$key' OP_$key" ).append("\n"); 
		query.append("      #end	" ).append("\n"); 
		query.append("      ,'OP Total'	 OP_TOTAL" ).append("\n"); 
		query.append("	#end " ).append("\n"); 
		query.append("	  		" ).append("\n"); 
		...
		query.append("    --,CM_BKG_TOTAL" ).append("\n"); 
		query.append("	 " ).append("\n"); 
		query.append("  #if(${f_pro_lvl} =='O')" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("   	#foreach($key in ${allcols}) " ).append("\n"); 
		...
		query.append("    " ).append("\n"); 
		query.append("  #foreach($key in ${allcols}) 	 " ).append("\n"); 
		query.append("              ,TO_CHAR(SUM(DECODE(A2.SPCL_CNTR_TPSZ_CD, '$key',  A2.BKG_REV+A2.BKG_OFT_REV+A2.BKG_MISC_REV+A2.SCR_CHG_REV+A2.DMDT_COM_AMT - DECODE(@[f_pro_vw],'P',A2.PA_CM_COST_TTL_AMT,'R',A2.RA_CM_COST_TTL_AMT)))) AS CM_$key" ).append("\n"); 
		query.append("  #end		        " ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("	          ,TO_CHAR(SUM(A2.BKG_REV+A2.BKG_OFT_REV+A2.BKG_MISC_REV+A2.SCR_CHG_REV+A2.DMDT_COM_AMT - DECODE(@[f_pro_vw],'P',A2.PA_CM_COST_TTL_AMT,'R',A2.RA_CM_COST_TTL_AMT)) )  AS CM_TOTAL 	" ).append("\n"); 
		query.append("              ,TO_CHAR(SUM(A2.BKG_REV+A2.BKG_OFT_REV+A2.BKG_MISC_REV+A2.SCR_CHG_REV - DECODE(@[f_pro_vw],'P',A2.PA_CM_COST_TTL_AMT,'R',A2.RA_CM_COST_TTL_AMT)" ).append("\n"); 
		query.append("                       + NVL(A2.CNTR_STERM_RNTL_LAND_AMT, 0) + NVL(A2.CNTR_LTERM_RNTL_LAND_AMT, 0) + NVL(A2.CNTR_MNR_LAND_AMT, 0) " ).append("\n"); 
		...
		query.append("								+ NVL(A2.CNTR_INSUR_NORM_AMT, 0)" ).append("\n"); 
		query.append("					" ).append("\n"); 
		query.append("								))  AS CM_BKG_TOTAL -- CM(BKG)는 CM 계산에서 수입 DEM/DET, 비용 장비비 제외" ).append("\n"); 
		query.append("	" ).append("\n"); 
		query.append("  #if(${f_pro_lvl} =='O')" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		#foreach($key in ${allcols}) " ).append("\n"); 
		query.append("              ,TO_CHAR(SUM(DECODE(A2.SPCL_CNTR_TPSZ_CD, '$key',  DECODE(@[f_pro_vw],'P',A6.PA_OP_COST_TTL_AMT,'R',0)))) AS OP_COST_$key" ).append("\n"); 
		query.append("        #end" ).append("\n"); 
		query.append("	  	      ,TO_CHAR(SUM(DECODE(@[f_pro_vw],'P',A6.PA_OP_COST_TTL_AMT,'R',0)))                              AS OP_COST_TOTAL	" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		#foreach($key in ${allcols}) " ).append("\n"); 
		query.append("              ,TO_CHAR(SUM(DECODE(A2.SPCL_CNTR_TPSZ_CD, '$key',  DECODE(@[f_pro_vw],'P',(A2.BKG_REV+A2.BKG_OFT_REV+A2.BKG_MISC_REV+A2.SCR_CHG_REV+A2.DMDT_COM_AMT) - A2.PA_CM_COST_TTL_AMT - A6.PA_OP_COST_TTL_AMT,'R',0)))) AS OP_$key" ).append("\n"); 
		query.append("        #end	" ).append("\n"); 
		query.append("		      ,TO_CHAR(SUM(A2.BKG_REV+A2.BKG_OFT_REV+A2.BKG_MISC_REV+A2.SCR_CHG_REV+A2.DMDT_COM_AMT) - NVL(SUM(A2.PA_CM_COST_TTL_AMT),0) - NVL(SUM(A6.PA_OP_COST_TTL_AMT),0))                              AS OP_TOTAL " ).append("\n"); 
		query.append("  #end" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("  #if(${f_include_ts} =='Y' && ${f_rlane_cd} != 'RBCCO')" ).append("\n"); 

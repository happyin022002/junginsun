...
public class ESM_BKG_0920HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0920HTMLAction 객체를 생성
	 */
	public ESM_BKG_0920HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0920Event event = new EsmBkg0920Event();
		...
		String troSeq  = JSPUtil.getNullNoTrim(JSPUtil.getParameter(request, "tro_seq".trim(),   ""));
		String uiId    = JSPUtil.getNullNoTrim(JSPUtil.getParameter(request, "ui_id".trim(),     ""));
		...
		if(command.isCommand(FormCommand.MULTI)) {
			boundCd = JSPUtil.getNullNoTrim(JSPUtil.getParameter(request, "io_bnd_cd".trim(), ""));
			troSeq  = JSPUtil.getNullNoTrim(JSPUtil.getParameter(request, "tro_seq".trim(),   ""));
			uiId    = JSPUtil.getNullNoTrim(JSPUtil.getParameter(request, "ui_id".trim(),     ""));
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
			event.setBoundCd(boundCd);
			event.setTroSeq (troSeq);
			event.setUiId   (uiId);
			
			event.setArrBkgBlNoVO((BkgBlNoVO[])getVOs(request, BkgBlNoVO.class, "sheet1_")); 
		}

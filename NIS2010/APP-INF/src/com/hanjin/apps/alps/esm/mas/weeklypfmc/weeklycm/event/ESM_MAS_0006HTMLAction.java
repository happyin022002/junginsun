...
**@FileName : ESM_MAS_0006HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0006Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0006HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0006HTMLAction 객체를 생성
	 */
	public ESM_MAS_0006HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0006Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0006Event event = new EsmMas0006Event();  	
    			
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setOfcRoleSetupVO((OfcRoleSetupVO)getVO(request, OfcRoleSetupVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {			
			event.setOfcRoleSetupVOs((OfcRoleSetupVO[])getVOs(request, OfcRoleSetupVO.class, ""));			
		}
		else {
			event.setOfcRoleSetupVO((OfcRoleSetupVO)getVO(request, OfcRoleSetupVO.class));

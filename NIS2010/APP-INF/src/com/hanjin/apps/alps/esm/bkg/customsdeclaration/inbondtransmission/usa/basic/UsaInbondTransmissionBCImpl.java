...
=========================================================*/
package com.hanjin.apps.alps.esm.bkg.customsdeclaration.inbondtransmission.usa.basic;
...
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.inbondtransmission.basic.InbondTransmissionBCImpl;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.inbondtransmission.usa.integration.UsaInbondTransmissionDBDAO;
...
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.inbondtransmission.usa.integration.UsaACEInbondTransmissionDBDAO;	// 추가ACE-Inbond
...
public class UsaInbondTransmissionBCImpl extends InbondTransmissionBCImpl {
	...
	public List<InbondMibListDetailVO> searchInbondManifestList(InbondListCondVO inbondListCondVO ) throws EventException{
		...
		List<UsaInbondManifestDetailListVO> localVos = null;
		...
		try {
			...
			// 0408 화면에서 호출.( Paperless MIB Generate 화면 최상위 시트 조회)
				...
				if(vo == null){
					...
					if(usaInbondManifestDetailVOs != null && usaInbondManifestDetailVOs.size() > 0){
					...
					// getTrspAuthDt 에 "to query bl list for AI transmit" 값이 들어 있는 경우는 transmit 할 때,
					...
					if( ! "to query bl list for AI transmit".equals(vo.getTrspAuthDt())){
						...
						localVos = dbDao.searchInbondManifestDeatilList(vo);
	...
	public String transmitMIB(MibTransmitVO[] mibTransmitVO) throws EventException{
		try {
			...
			// crr_bat_no를 구한다.
            String crrBatNo = "SMLM" + vvd + podLoc + dbDao.searchCarrierBatchNo();
			...
			for(int i = 0; i < len ; i++){
				...
				if(i == 0){
					...
					blParams = blParams + "||to_clob('";
				}
				
				blParams = blParams + vo.getBlNo();
				...
				// 533.Cntr Arrival/Export전송(Flag:'C')
					...
					tmpstr7 = dbAceInbondDao.searchH01ForCntr(blNo, arrOrExpDate, pLocCd, cntrNo, flag2, arrOrExpTime);						
				}
				// ESM_BKG_0034 DVI버튼 클릭시
				if("DIV".equals(divInd)) {
					tmpstr7 = dbDao.searchH01ForDiversion(blNo, IRS_NO);
				}
				...
				mfDownloadBC.updateCntrArrExpByBlCntr(blNo, cntrNo, transGubun);
			}
			...
			/*
             * 20100402 경종윤 
			 * "ZCRSMLM" 뒤 라인갯수 5자리 제거
			tmpBuf = new StringBuffer(Integer.toString(lineCnt + 3));
			for(idx = tmpBuf.length(); idx < 5; idx++){
				tmpBuf.insert(0, "0");													
			}
            String tmpstr8 = new StringBuffer("ZCRSMLM").append(tmpBuf.toString()).toString();
            */
            String tmpstr8 = String.format("%-80s\n%-80s", "ZCRSMLM", "/*");
            ...
            */
            ...
            StringBuffer hiFile02 = new StringBuffer(tmpstr6)
		    		.append(hiFile01.toString())
		    		.append(tmpstr8);
            /*********************************************
			// Log Add Start.

...
public class FFCmpnApprovalDBDAOModifyFFCmpnCSRRevVVDUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                       AND A.IF_DT IS NULL" ).append("\n"); 
		query.append("                       AND A.UPD_DT BETWEEN TO_DATE(REPLACE(@[date_fm], '-', ''), 'YYYYMMDD') AND TO_DATE(REPLACE(@[date_to], '-', ''), 'YYYYMMDD') + 0.99999" ).append("\n"); 
		query.append("#elseif (${date_div} == 'P')" ).append("\n"); 
		query.append("                       AND A.APRO_DT BETWEEN TO_DATE(REPLACE(@[date_fm], '-', ''), 'YYYYMMDD') AND TO_DATE(REPLACE(@[date_to], '-', ''), 'YYYYMMDD') + 0.99999" ).append("\n"); 

...
/**
 * CndExpManifestListDownloadBackEndJob Business Logic Command implementation<br>
 * CndExpManifestListDownloadBackEndJob handling business logic<br>
 *
 * @author
 * @see Related DAO class
 * @since J2EE 1.6
 */
public class CndExpManifestListDownloadBackEndJob extends BackEndCommandSupport {
	private static final long serialVersionUID = -3034414164961318353L;
	private ManifestListDetailVO[] manifestListDetailVOs;
	private SignOnUserAccount account;
	private CndExpManifestListDownloadDBDAO dbDao;
...
//				if (paramVO.getBkgPodCd().startsWith("CA") && paramVO.getBkgDelCd().startsWith("US")) iUsCnt++;
			}
...
*/
			...
			for (int i = 0; i < manifestListDetailVOs.length; i++) {
				...
				if ("M".equals(paramVO.getBlType())) {
					 ...
					 * House B/L 조회
					 *****************************************/
					bkgCstmsAdvBlVO = dbDao.searchBkgBookingHBl(paramVO.getBkgNo(), paramVO.getBlNos(), sVslCd, sSkdVoyNo, sSkdDirCd);
				}
				...
				log.debug("getPgmNo >>> " + manifestListDetailVOs[0].getPgmNo());
				log.debug("paramVO.getHubLocCd() >>> " + paramVO.getHubLocCd());
				// AI Transmit 화면에서 AI Type 이 ADD 인 경우에는 setup 된 HUB 값을 가져옴
				if("ESM_BKG_N003".equals(manifestListDetailVOs[0].getPgmNo()) && !"".equals(paramVO.getHubLocCd()) ) {
					bkgCstmsAdvBlVO.setHubLocCd(paramVO.getHubLocCd());
					bkgCstmsAdvBlVO.setIbdLocGdsDesc(paramVO.getIbdLocGdsDesc());
				}
	 ...
	 * @param CndExpManifestListDownloadDBDAO dbDao
	 */
	public void setManifestListDetailVO(ManifestListDetailVO[] manifestListDetailVOs, SignOnUserAccount account, CndExpManifestListDownloadDBDAO dbDao) {
		if (manifestListDetailVOs != null) this.manifestListDetailVOs = Arrays.copyOf(manifestListDetailVOs, manifestListDetailVOs.length);

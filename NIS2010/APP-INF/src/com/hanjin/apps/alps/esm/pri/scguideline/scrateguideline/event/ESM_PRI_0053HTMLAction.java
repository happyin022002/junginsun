...
public class ESM_PRI_0053HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri0053Event event = new EsmPri0053Event();

		if (command.isCommand(FormCommand.MULTI)) {
			event.setPriSgRtMqcRngVOS((PriSgRtMqcRngVO[]) getVOs(request, PriSgRtMqcRngVO.class, ""));
		} else if (command.isCommand(FormCommand.SEARCH)) {
			event.setPriSgRtMqcRngVO((PriSgRtMqcRngVO) getVO(request, PriSgRtMqcRngVO.class));
		} else {
			event.setPriSgRtMqcRngVO((PriSgRtMqcRngVO) getVO(request, PriSgRtMqcRngVO.class));
		}

...
*@FileName : LaneSimulationDBDAOSearchSimPort1RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class LaneSimulationDBDAOSearchSimPort1RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public LaneSimulationDBDAOSearchSimPort1RSQL(){
		setQuery();
		...
		query.append("FileName : LaneSimulationDBDAOSearchSimPort1RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("FROM MAS_SIM_TML_INFO A1" ).append("\n"); 
		query.append(",(" ).append("\n"); 
		query.append("SELECT SECT_NO" ).append("\n"); 
		query.append(",NVL(LEAD(SECT_NO) OVER(ORDER BY SECT_NO)" ).append("\n"); 
		query.append(",LAG(SECT_NO,1,0) OVER(ORDER BY SECT_NO)" ).append("\n"); 
		query.append(") OTR_SECT" ).append("\n"); 
		query.append(",SKD_DIR_CD" ).append("\n"); 
		...
		query.append(",COUNT(DISTINCT B1.CONTI_CD) OVER() PNDLM_CNT" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT DISTINCT /*+ LEADING(A1 A2 A4 A3) USE_NL(A1 A2) USE_HASH(A2) */" ).append("\n"); 
		query.append("DECODE(A3.NUM, 1, SUBSTR(A1.TML_CD,1,5)" ).append("\n"); 
		query.append(", 2, SUBSTR(A1.TML_CD,1,5)" ).append("\n"); 
		query.append(") TML_CD" ).append("\n"); 
		query.append(",DECODE(A3.NUM, 1, DECODE(A1.TURN_PORT_IND_CD, 'Y', DECODE(A1.SKD_DIR_CD,'E','W','E') , A1.SKD_DIR_CD)" ).append("\n"); 
		...
		query.append(") A1" ).append("\n"); 
		query.append(",MAS_SIM_SVC_LANE A2" ).append("\n"); 
		query.append("WHERE 1=1" ).append("\n"); 
		...
		query.append("AND B2.SECT_NO  = NVL('001',B2.SECT_NO)" ).append("\n"); 

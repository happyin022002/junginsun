...
*@FileName : ESM_BKG_0320HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.nis2010.esm.bkg.customsdeclaration.manifestlistdownload 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESM_BKG_0320HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0926HTMLAction 객체를 생성
     */
    public ESM_BKG_0320HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0320Event event = new EsmBkg0320Event();

		if (command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01))
		{
			// Auth Setup 정보 조회
			event.setInfoVO((BkgInternetAuthorityVO) getVO(request, BkgInternetAuthorityVO.class));
			if (command.isCommand(FormCommand.SEARCH01)){
				event.getInfoVO().setUsrId(request.getParameter("ch_usr_id"));
			}
		}
		else if (command.isCommand(FormCommand.MULTI))
		{
			// Auth Setup 정보 수정
			event.setInfoVOs((BkgInternetAuthorityVO[]) getVOs(request, BkgInternetAuthorityVO.class, "sheet1_"));

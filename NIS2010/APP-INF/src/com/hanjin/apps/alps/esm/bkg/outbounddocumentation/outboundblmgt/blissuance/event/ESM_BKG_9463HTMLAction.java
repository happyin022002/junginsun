...
*@FileName : ESM_BKG_9463HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ESM_BKG_9463HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_9463HTMLAction 객체를 생성
	 */
	public ESM_BKG_9463HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		log.debug("[START::  ESM_BKG_9463HTMLAction  ]==========");

		EsmBkg9463Event event = new EsmBkg9463Event();
		String[] keys =null;
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			BlAtchVO blAtchVO = (BlAtchVO)getVO(request, BlAtchVO.class);
			event.setBlAtchVO(blAtchVO);

		}
		else if(command.isCommand(FormCommand.MULTI)) {

//			BlAtchVO blAtchVO 	= (BlAtchVO)getVO(request, BlAtchVO.class);
			BlAtchVO[] blAtchVOs 	= (BlAtchVO[])getVOs(request, BlAtchVO .class,"sheet1_");
			if(keys!=null){
				int keyleng = 0;
				for(int i=0;i<blAtchVOs.length;i++){
					if(!"D".equals(blAtchVOs[i].getIbflag()))
						blAtchVOs[i].setKeys(keys[keyleng++]);
				}
			}

			event.setBlAtchVOs(blAtchVOs);

		}

		request.setAttribute("Event", event);
		log.debug("[END:: perform ESM_BKG_9463HTMLAction  ]==========");
		return  event;
	}

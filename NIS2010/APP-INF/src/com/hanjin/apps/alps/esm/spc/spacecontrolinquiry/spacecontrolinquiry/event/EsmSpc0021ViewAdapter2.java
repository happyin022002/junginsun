...
*@FileTitle : Daily Forecast Status-Allocation Status(HO)
*Open Issues :
*Change history :
...
* 2010.06.16 CHOI.Y.S - Ticket ID : CHM-201004165 - BSA 값을 int로 받던 부분을 Double 수정
...
public class EsmSpc0021ViewAdapter2 extends ViewAdapter{
	...
	{
		log.debug("EsmSpc0021ViewAdapter2");
...
//				}
				...
				if( !pre_gid.equals(gid) ){
					if(!((aq_cd.equals("-") && dir_cd.equals("TOTAL")) || (rlane_cd.equals("TOTAL") && aq_cd.equals("-")))){
							...
							for(int m = 1 ; m <= 5 ; m++){
								...
								switch (w) {
									...
									case 6:
										...
										tempw2 = viewList.get(i).getBkgw6();
										break;
								}
						...
						//hg(guide), hc1(USMode), hc2(Account), hc3(Dest)
						sbufXML.append("<TD INDENT=\"1\"></TD>\n");
						sbufXML.append("<TD INDENT=\"1\"></TD>\n");
						sbufXML.append("<TD INDENT=\"1\"></TD>\n");
						sbufXML.append("<TD INDENT=\"1\"></TD>\n");					
...
//					} else if(!"+".equals(usa_bkg_mod_cd) && !"+".equals(acct_cd) && !"+".equals(dest_loc_cd)){
				...
				sbufXML.append("<TR LEVEL=\"" + ("Guide".equals(gid_aq)?level2:level)+ "\" BGCOLOR=\"").append(aq_cd.equals("-")?(dir_cd.equals("TOTAL")?"233,241,254":"247,231,236"):(!acct_lvl.isEmpty()?(ofc_cd.equals("+")?"203,230,254":"253,251,151"):"")).append("\">\n");
				sbufXML.append("<TD><![CDATA[" + dest + "]]></TD>\n");
				String tmpArea = gid_aq.equals("Guide") && "Z".equals(acct_lvl) ? aq_cd:gid_aq;
				String tmpOfc = level==0?"":(aq_cd.equals("-")?"TTL":(gid_aq.equals("Guide") && "Z".equals(acct_lvl) ?aq_cd:ofc_cd));
				String tmpSubOfc = level==0?"":(aq_cd.equals("-")?"TTL":(gid_aq.equals("Guide") && "Z".equals(acct_lvl) ?aq_cd:sub_ofc_cd));
				...
				sbufXML.append("<TD " + (acct_lvl.isEmpty()?"":(ofc_cd.equals("+")?"INDENT=\"1\"":"INDENT=\"2\"")) + ">").append(tmpArea).append("</TD>\n");
				sbufXML.append("<TD>").append(tmpOfc).append("</TD>\n");
				sbufXML.append("<TD>").append((tmpOfc.equals("+")?"":tmpSubOfc)).append("</TD>\n");

				sbufXML.append("<TD>" + (level==3&&acct_lvl.isEmpty()?"+":("Z".equals(acct_lvl)?"*":acct_lvl)) + "</TD>\n");//Z: Lane의 USAMODE/ACCOUT/DEST SUBSUM표시
				...
				if("Guide".equals(gid_aq)){ //6, 5, 4
					...
					sbufXML.append("<TD>" +  (level2<=3?"":usa_bkg_mod_cd)  + "</TD>\n");//add office가 (+) 이고 cust control cd가 null이 아니면 ""로 표시함
					if("+".equals(acct_cd)) {
						if("Z".equals(acct_lvl)) acct_cd = "";
						sbufXML.append("<TD DATA-ALIGN=\"daCenter\">" + (level2==0||level2<=4?"":acct_cd) + "</TD>\n");//add
					} else {
						sbufXML.append("<TD DATA-ALIGN=\"daLeft\"><![CDATA["+ (level2==0?"":acct_cd)  + "]]></TD>\n");//add
					}
					if("+".equals(dest_loc_cd) && "Z".equals(acct_lvl)) dest_loc_cd = "";
					sbufXML.append("<TD>" + (level2<=5?"":dest_loc_cd)  + "</TD>\n");//add
				} else {
					sbufXML.append("<TD>" +  (level<=3?"":usa_bkg_mod_cd)  + "</TD>\n");//add office가 (+) 이고 cust control cd가 null이 아니면 ""로 표시함
					if("+".equals(acct_cd)) {
						sbufXML.append("<TD DATA-ALIGN=\"daCenter\">" + (level==0||level<=4?"":acct_cd) + "</TD>\n");//add
					} else {
						sbufXML.append("<TD DATA-ALIGN=\"daLeft\"><![CDATA["+ (level==0?"":acct_cd)  + "]]></TD>\n");//add
					}
					sbufXML.append("<TD>" + (level<=5?"":dest_loc_cd)  + "</TD>\n");//add
				}

				sbufXML.append("<TD>" +  ("Guide".equals(gid_aq)?level2:level) + "</TD>\n");
				sbufXML.append("<TD>" + (level==3?"OFFICE":(level<4&&!acct_lvl.isEmpty()?"TOTAL":(row_num++))) + "</TD>\n");
				
				//hg(Guide), hc1(USMode), hc2(Account), hc3(Dest)
				sbufXML.append("<TD>" + (level<4&&!acct_lvl.isEmpty()?"1":"0") + "</TD>\n");
				sbufXML.append("<TD>" + ("Z".equals(acct_lvl)&&!usa_bkg_mod_cd.isEmpty()?"1":"0") + "</TD>\n");
				sbufXML.append("<TD>" + ("Z".equals(acct_lvl)&&!acct_cd.isEmpty()?"1":"0") + "</TD>\n");
				sbufXML.append("<TD>" + ("Z".equals(acct_lvl)&&!dest_loc_cd.isEmpty()?"1":"0") + "</TD>\n");
...
//					sbufXML.append("<TD>" + JSPUtil.getNull(rowSets[1].getString(typ[t]+w+m)) + "</TD>\n");
				...
				if( !next_gid.equals(gid) && (!next_dest.equals(dest) || !rlane_cd.equals("TOTAL") || !next_sub_trd_cd.equals(sub_trd_cd)) ){
					...
					double[] tbw   = new double[duration+1];
					double[][] tfw = new double[duration+1][6];
					tbw = sub_trd_cd.equals("TOTAL")?tbsaw:(dest.equals(" ")&&sub_trd_cd.equals("IA")?ibsaw:(rlane_cd.equals("TOTAL")?sbsaw:bsaw));
					tfw = sub_trd_cd.equals("TOTAL")?tbkgWgt:(dest.equals(" ")&&sub_trd_cd.equals("IA")?ibkgWgt:(rlane_cd.equals("TOTAL")?sbkgWgt:bkgWgt));
					...
					for(int w = 1 ; w <= duration ; w++){
						...
						for(int m = 1 ; m <= 1 ; m++){
							...
							sbufXML.append("<TD DATA-ALIGN=\"daRight\">" + (lbsaw==0?0:Math.round(lfctw*100.0/lbsaw)) + "%</TD>\n");
						}
					}

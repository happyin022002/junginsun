...
*@FileName : RFARateProposalDBDAOPriRpScpRtGlineMappingCostDetailVOUSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateProposalDBDAOPriRpScpRtGlineMappingCostDetailVOUSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public RFARateProposalDBDAOPriRpScpRtGlineMappingCostDetailVOUSQL(){
		setQuery();
		...
		query.append("FileName : RFARateProposalDBDAOPriRpScpRtGlineMappingCostDetailVOUSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("" ).append("\n"); 
		query.append("MERGE INTO PRI_RP_SCP_RT A " ).append("\n"); 
		query.append("USING (" ).append("\n"); 
		query.append("        SELECT  PROP_NO, AMDT_SEQ, SVC_SCP_CD" ).append("\n"); 
		query.append("               ,CMDT_HDR_SEQ, ROUT_SEQ, RT_SEQ, CMPB_AMT       " ).append("\n"); 
		query.append("          FROM  (" ).append("\n"); 
		query.append("                SELECT  SCP_MN.PROP_NO, SCP_MN.AMDT_SEQ, SCP_MN.SVC_SCP_CD" ).append("\n"); 
		query.append("                       ,RATE.CMDT_HDR_SEQ, RATE.ROUT_SEQ, RATE.RT_SEQ, MAPG.MAPG_SCRE, MAPG.CURR_CD, MAPG.CMPB_AMT" ).append("\n"); 
		query.append("                       ,ROW_NUMBER() OVER (PARTITION BY SCP_MN.PROP_NO, SCP_MN.AMDT_SEQ, SCP_MN.SVC_SCP_CD" ).append("\n"); 
		query.append("                       						,RATE.CMDT_HDR_SEQ, RATE.ROUT_SEQ, RATE.RT_SEQ " ).append("\n"); 
		...
		query.append("                       ,PRI_RP_SCP_RT        RATE" ).append("\n"); 
		query.append("                       ,PRI_RP_SCP_RT_USD_ROUT_CS  ROUT" ).append("\n"); 
		query.append("                       ,PRI_PRS_USD_ROUT_CS_INFO   ROUT_INFO " ).append("\n"); 
		query.append("                       ,PRI_CMPB_GLINE_MAPG  MAPG" ).append("\n"); 
		query.append("                       ,(SELECT DISTINCT A.PROP_NO, A.AMDT_SEQ, A.SVC_SCP_CD, A.CMDT_HDR_SEQ, A.CMDT_SEQ, " ).append("\n"); 
		...
		query.append("                		        LEFT OUTER JOIN  PRI_RP_SCP_GRP_CMDT B" ).append("\n"); 
		...
		query.append("                        		       )" ).append("\n"); 
		query.append("                		        LEFT OUTER JOIN PRI_RP_SCP_GRP_CMDT_DTL C" ).append("\n"); 
		...
		query.append("                     " ).append("\n"); 
		query.append("                   AND  MN.PRC_CTRT_CUST_TP_CD	= DECODE(MAPG.PRS_CUST_TP_CD, 'M', MN.PRC_CTRT_CUST_TP_CD, MAPG.PRS_CUST_TP_CD)" ).append("\n"); 
		query.append("                   AND  MN.CTRT_CUST_CNT_CD = DECODE(MAPG.CUST_CNT_CD, NULL, MN.CTRT_CUST_CNT_CD, MAPG.CUST_CNT_CD)" ).append("\n"); 
		query.append("                   AND  MN.CTRT_CUST_SEQ       = DECODE(MAPG.CUST_SEQ, NULL, MN.CTRT_CUST_SEQ, MAPG.CUST_SEQ)" ).append("\n"); 
		...
		query.append("                   AND  MAPG.MQC_RNG_FM_QTY <= DECODE(SCP_MN.TGT_MVC_QTY, NULL, DECODE(MN.TGT_MVC_QTY, NULL, 0, MN.TGT_MVC_QTY), SCP_MN.TGT_MVC_QTY)" ).append("\n"); 
		query.append("                   AND  MAPG.MQC_RNG_TO_QTY >= DECODE(SCP_MN.TGT_MVC_QTY, NULL, DECODE(MN.TGT_MVC_QTY, NULL, 0, MN.TGT_MVC_QTY), SCP_MN.TGT_MVC_QTY)" ).append("\n"); 

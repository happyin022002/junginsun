...
public class SpaceControlInquiryDBDAOSearchSpaceControlRDRSummaryListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                   SUM(B.BSA) AS BSA  ,SUM(B.BSA_WGT) AS BSA_WGT  ," ).append("\n"); 
		...
		query.append("                             NVL(B.BSA_SLOT, 0) AS BSA,NVL(B.BSA_WGT, 0) AS BSA_WGT," ).append("\n"); 
		...
		query.append("                             A.OPR_CD," ).append("\n"); 
		query.append("                             A.REGION," ).append("\n"); 
		query.append("                             NVL(B.BSA_SLOT, 0),NVL(B.BSA_WGT, 0)" ).append("\n"); 
		...
		query.append("           SUM(DECODE(Z1.NUM, ${key}, NVL(Z1.BSA_WGT  , 0), 0)) AS BSA_WGT$key    ," ).append("\n"); 
		...
		query.append("    	 ROUND(DECODE(Z3.BSA_WGT$key, 0, 0, DECODE(Z3.TTL_WGT$key, 0, 0, Z3.TTL_WGT$key * 100 / Z3.BSA_WGT$key)), 1) || '%' AS TTL_WGT_LF$key ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("				   SUM(Z2.BSA_WGT$key)   AS BSA_WGT$key  ," ).append("\n"); 
		...
		query.append("         DECODE(OPR_CD, 'VVD', '00000', 'ITEM', '00001', 'SML', '00002', 'COS', '00003', 'KKL', '00004', 'YML', '00005', 'UAC', '00006', 'TOTAL', 'ZZZZZ', OPR_CD)" ).append("\n"); 

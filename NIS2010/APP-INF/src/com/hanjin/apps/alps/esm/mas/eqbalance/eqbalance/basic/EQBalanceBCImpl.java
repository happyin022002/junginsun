...
*  2012.03.22 김종준 [CHM-201217091-01] EMU 관련 로직 보완/변경 검토 요청의 건 -POR 별 Credit Ratio를 setup 할수 있는 화면추가 (Rule-set)  
 =========================================================*/
package com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.basic;
...
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.integration.EQBalanceDBDAO;
...
/**
 * ALPS-MAS Business Logic Basic Command implementation<br> - ALPS-MAS에 대한 비지니스 로직을 처리한다.<br>
 * 
 * @author Sangwook_nam
...
public class EQBalanceBCImpl extends BasicCommandSupport implements EQBalanceBC {
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESM_MAS_0016 화면에 대한 멀티 이벤트 처리<br>
	 * @param MasCntrRepoShtgInfoVO[] masCntrRepoShtgInfoVO
	 * @param SearchConditionVO searchConditionVO
	...
	public void multiEQBalance00163(MasCntrRepoShtgInfoVO[] masCntrRepoShtgInfoVO, SearchConditionVO searchConditionVO, SignOnUserAccount account, String excel_load_yn) throws EventException{
		try {
			List<MasCntrRepoShtgInfoVO> updateVoList = new ArrayList<MasCntrRepoShtgInfoVO>();
			List<MasCntrRepoShtgInfoVO> insertVoList = new ArrayList<MasCntrRepoShtgInfoVO>();
			List<MasCntrRepoShtgInfoVO> deleteVoList = new ArrayList<MasCntrRepoShtgInfoVO>();
			if ( excel_load_yn.equals("Y")) {	//엑셀 업로드일 경우 년원에 해당 데이타 삭제후
				...
				dbDao.removeCostYrmonMasCntrRepoShtgInfo(searchConditionVO);	
			}
			
			for ( int i=0; i<masCntrRepoShtgInfoVO .length; i++ ) {
				  if (masCntrRepoShtgInfoVO[i].getIbflag().equals("U")){
					masCntrRepoShtgInfoVO[i].setUpdUsrId(account.getUsr_id());	
					masCntrRepoShtgInfoVO[i].setCostYrmon(searchConditionVO.getFCostYrmon().replaceAll("-", ""));
					masCntrRepoShtgInfoVO[i].setCntrTpszCd(searchConditionVO.getFCntrTpszCd());					
					updateVoList.add(masCntrRepoShtgInfoVO[i]);					
					
				} else if (masCntrRepoShtgInfoVO[i].getIbflag().equals("I")){
					masCntrRepoShtgInfoVO[i].setUpdUsrId(account.getUsr_id());	
					masCntrRepoShtgInfoVO[i].setCostYrmon(searchConditionVO.getFCostYrmon().replaceAll("-", ""));
					masCntrRepoShtgInfoVO[i].setCreUsrId(account.getUsr_id());
					insertVoList.add(masCntrRepoShtgInfoVO[i]);					
				} else if (masCntrRepoShtgInfoVO[i].getIbflag().equals("D")){
					masCntrRepoShtgInfoVO[i].setCostYrmon(searchConditionVO.getFCostYrmon().replaceAll("-", ""));
					masCntrRepoShtgInfoVO[i].setCntrTpszCd(searchConditionVO.getFCntrTpszCd());
					  
					deleteVoList.add(masCntrRepoShtgInfoVO[i]);					
				}
			}
	 ...
	 * ESM_MAS_0018 수정
	 * @param MasCntrRepoCrVO[] masCntrRepoCrVO
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
	public void multiEQBalance0018(MasCntrRepoCrVO[] masCntrRepoCrVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasCntrRepoCrVO> updateVoList = new ArrayList<MasCntrRepoCrVO>();
			for ( int i=0; i<masCntrRepoCrVO .length; i++ ) {
				if ( masCntrRepoCrVO[i].getIbflag().equals("U")){
					masCntrRepoCrVO[i].setUpdUsrId(account.getUsr_id());
					updateVoList.add(masCntrRepoCrVO[i]);
				}
			}
	 ...
	 * ESM_MAS_0136 화면 수정<br>
	 * @param MasCntrRepoRoutEccVO[] masCntrRepoRoutEccVO
	 * @param SearchConditionVO searchConditionVO
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
	public void multiEQBalance0136(MasCntrRepoRoutEccVO[] masCntrRepoRoutEccVO, SearchConditionVO searchConditionVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasCntrRepoRoutEccVO> updateVoList = new ArrayList<MasCntrRepoRoutEccVO>();
			for ( int i=0; i<masCntrRepoRoutEccVO .length; i++ ) {
				 if ( masCntrRepoRoutEccVO[i].getIbflag().equals("U")){
					masCntrRepoRoutEccVO[i].setCreUsrId(account.getUsr_id());
					masCntrRepoRoutEccVO[i].setUpdUsrId(account.getUsr_id());
					masCntrRepoRoutEccVO[i].setCntrTpszCd(searchConditionVO.getFCntrTpszCd());
					updateVoList.add(masCntrRepoRoutEccVO[i]);
				} 
			}
	 ...
	 * ESM_MAS_0161 화면 수정<br>
	 * @param MasCntrRepoRoutEccVO[] masCntrRepoRoutEccVO
	 * @param SearchConditionVO searchConditionVO
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
	public void multiEQBalance0161(MasCntrRepoRoutEccVO[] masCntrRepoRoutEccVO, SearchConditionVO searchConditionVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasCntrRepoRoutEccVO> updateVoList = new ArrayList<MasCntrRepoRoutEccVO>();
			for ( int i=0; i<masCntrRepoRoutEccVO .length; i++ ) {
				 if ( masCntrRepoRoutEccVO[i].getIbflag().equals("U")){

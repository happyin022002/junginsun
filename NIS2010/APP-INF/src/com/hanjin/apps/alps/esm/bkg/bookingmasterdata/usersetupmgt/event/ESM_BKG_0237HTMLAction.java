...
*@FileName : ESM_BKG_0237HTMLAction.java
...
public class ESM_BKG_0237HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0237HTMLAction 객체를 생성
	 */
	public ESM_BKG_0237HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0237Event event = new EsmBkg0237Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setBkgCustChkPntVOs((BkgCustChkPntVO[])getVOs(request, BkgCustChkPntVO .class,"sheet_"));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgCustChkPntVO((BkgCustChkPntVO)getVO(request, BkgCustChkPntVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setBkgChkPntItemVO((BkgChkPntItemVO)getVO(request, BkgChkPntItemVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setChkPntTpCd(JSPUtil.getParameter(request,"chk_pnt_tp_cd"));
			event.setChkPntItmSeq(JSPUtil.getParameter(request,"chk_pnt_itm_seq"));;
		}

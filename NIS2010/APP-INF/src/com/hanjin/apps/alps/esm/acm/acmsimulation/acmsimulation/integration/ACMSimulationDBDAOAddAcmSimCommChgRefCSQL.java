...
*@FileName : ACMSimulationDBDAOAddAcmSimCommChgRefCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ACMSimulationDBDAOAddAcmSimCommChgRefCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  *  - PRI 테이블에 승인 flag 추가 됨.(SCG_RQST_PROC_CD = 'A')
	  * </pre>
	  */
	public ACMSimulationDBDAOAddAcmSimCommChgRefCSQL(){
		setQuery();
		...
		query.append("FileName : ACMSimulationDBDAOAddAcmSimCommChgRefCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("INSERT INTO ACM_SIM_COMM_CHG_REF" ).append("\n"); 
		...
		query.append("                        FROM MAS_USA_SVC_MOD A " ).append("\n"); 
		...
		query.append("      AND SRT.SCG_RQST_PROC_CD     = 'A'" ).append("\n"); 
		...
		query.append("      AND 'Y' = CASE WHEN NVL(SRT.POR_TP_CD,'x') <> 'G' AND NVL(DECODE (PRF.POR_USE_FLG, 'Y', SRT.POR_DEF_CD, '*'), '*') IN (POR.LOC_CD, POR.RGN_CD, POR.CNT_CD, '*' ) THEN 'Y'" ).append("\n"); 
		query.append("                     WHEN NVL(SRT.POR_TP_CD,'x') =  'G' AND SRT.POR_DEF_CD IN (" ).append("\n"); 
		query.append("                                                                            SELECT DISTINCT SCG_GRP_LOC_CD " ).append("\n"); 
		query.append("                                                                            FROM PRI_SCG_GRP_LOC M, PRI_SCG_GRP_LOC_DTL D " ).append("\n"); 
		query.append("                                                                            WHERE 1=1" ).append("\n"); 
		query.append("                                                                            AND M.SVC_SCP_CD = ARR.SVC_SCP_CD " ).append("\n"); 
		query.append("                                                                            AND M.CHG_CD = SRT.CHG_CD" ).append("\n"); 
		query.append("                                                                            AND D.DTL_LOC_DEF_CD IN (POR.LOC_CD, POR.CNT_CD)" ).append("\n"); 
		query.append("                                                                            AND M.SVC_SCP_CD = D.SVC_SCP_CD" ).append("\n"); 
		query.append("                                                                            AND M.CHG_CD = D.CHG_CD" ).append("\n"); 
		query.append("                                                                            AND M.GRP_LOC_SEQ = D.GRP_LOC_SEQ ) THEN 'Y' " ).append("\n"); 
		query.append("                     ELSE 'N' " ).append("\n"); 
		query.append("                END" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("      AND 'Y' = CASE WHEN NVL(SRT.POL_TP_CD,'x') <> 'G' AND NVL(DECODE (PRF.POL_USE_FLG, 'Y', SRT.POL_DEF_CD, '*'), '*') IN (POL.LOC_CD, POL.RGN_CD, POL.CNT_CD, '*' ) THEN 'Y' " ).append("\n"); 
		query.append("                     WHEN NVL(SRT.POL_TP_CD,'x') =  'G' AND SRT.POL_DEF_CD IN (" ).append("\n"); 
		query.append("                                                                                SELECT DISTINCT SCG_GRP_LOC_CD " ).append("\n"); 
		query.append("                                                                                FROM PRI_SCG_GRP_LOC M, PRI_SCG_GRP_LOC_DTL D " ).append("\n"); 
		query.append("                                                                                WHERE 1=1" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = ARR.SVC_SCP_CD " ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = SRT.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND D.DTL_LOC_DEF_CD IN (POL.LOC_CD, POL.CNT_CD)" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = D.SVC_SCP_CD" ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = D.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND M.GRP_LOC_SEQ = D.GRP_LOC_SEQ ) THEN 'Y'" ).append("\n"); 
		query.append("                " ).append("\n"); 
		...
		query.append("                END" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("      AND 'Y' = CASE WHEN NVL(SRT.POD_TP_CD,'x') <> 'G' AND NVL(DECODE (PRF.POD_USE_FLG, 'Y', SRT.POD_DEF_CD, '*'), '*') IN (POD.LOC_CD, POD.RGN_CD, POD.CNT_CD, '*' ) THEN 'Y' " ).append("\n"); 
		query.append("                     WHEN NVL(SRT.POD_TP_CD,'x') =  'G' AND SRT.POD_DEF_CD IN (" ).append("\n"); 
		query.append("                                                                                SELECT DISTINCT SCG_GRP_LOC_CD " ).append("\n"); 
		query.append("                                                                                FROM PRI_SCG_GRP_LOC M, PRI_SCG_GRP_LOC_DTL D " ).append("\n"); 
		query.append("                                                                                WHERE 1=1" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = ARR.SVC_SCP_CD " ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = SRT.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND D.DTL_LOC_DEF_CD IN (POD.LOC_CD, POD.CNT_CD)" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = D.SVC_SCP_CD" ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = D.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND M.GRP_LOC_SEQ = D.GRP_LOC_SEQ ) THEN 'Y'" ).append("\n"); 
		query.append("                " ).append("\n"); 
		...
		query.append("                END" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("      AND 'Y' = CASE WHEN NVL(SRT.DEL_TP_CD,'x') <> 'G' AND NVL(DECODE (PRF.DEL_USE_FLG, 'Y', SRT.DEL_DEF_CD, '*'), '*') IN (DEL.LOC_CD, DEL.RGN_CD, DEL.CNT_CD, '*' ) THEN 'Y' " ).append("\n"); 
		query.append("                     WHEN NVL(SRT.DEL_TP_CD,'x') =  'G' AND SRT.DEL_DEF_CD IN (" ).append("\n"); 
		query.append("                                                                                SELECT DISTINCT SCG_GRP_LOC_CD " ).append("\n"); 
		query.append("                                                                                FROM PRI_SCG_GRP_LOC M, PRI_SCG_GRP_LOC_DTL D " ).append("\n"); 
		query.append("                                                                                WHERE 1=1" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = ARR.SVC_SCP_CD " ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = SRT.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND D.DTL_LOC_DEF_CD IN (DEL.LOC_CD, DEL.CNT_CD)" ).append("\n"); 
		query.append("                                                                                AND M.SVC_SCP_CD = D.SVC_SCP_CD" ).append("\n"); 
		query.append("                                                                                AND M.CHG_CD = D.CHG_CD" ).append("\n"); 
		query.append("                                                                                AND M.GRP_LOC_SEQ = D.GRP_LOC_SEQ ) THEN 'Y'" ).append("\n"); 
		query.append("                    ELSE 'N' " ).append("\n"); 
		...
		query.append("    --;)----------------------------------------------------------------------------------------------- " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'CFR' AND ARR.RCV_TERM_CD = 'S' THEN 'CFR' " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'CFD' AND ARR.DE_TERM_CD  = 'S' THEN 'CFD' " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'CFR' THEN '*' " ).append("\n"); 
		...
		query.append("             /*   - BKG R term에 I 혹은 T가 있는 경우 - OTH 불공제                */ " ).append("\n"); 
		query.append("             /*   - BKG D term에 O 혹은 T가 있는 경우 - DTH 불공제                */ " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'THC'  AND(ARR.RCV_TERM_CD IN ('T','I','O') or ARR.DE_TERM_CD IN ('T','I','O')) THEN '*' " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'OTH'  AND ARR.RCV_TERM_CD IN ('T','I') THEN '*' " ).append("\n"); 
		query.append("             WHEN PRF.CHG_CD      = 'DTH'  AND ARR.DE_TERM_CD  IN ('T','O') THEN '*' " ).append("\n"); 
		query.append("             /* R/D Term 에 따라 THC 공제 여부 체크 로직 추가 - 임종한 과장 요청[CHM-201111599]*/ " ).append("\n"); 
		...
		query.append("                 AND S3.VPS_PORT_CD   = DECODE (SRT.CNL_TZ_CD, 'P', 'PAPAC', 'S', 'EGSUC') " ).append("\n"); 

...
public class SpecialCargoReceiptDBDAOAwkBkgInfoVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",   (SELECT LOC_NM FROM MDM_LOCATION WHERE LOC_CD = A.POL_CD) POL_NM  " ).append("\n"); 
		query.append(",   (SELECT LOC_NM FROM MDM_LOCATION WHERE LOC_CD = A.POD_CD) POD_NM" ).append("\n"); 
		query.append(",   (SELECT CD.INTG_CD_VAL_DP_DESC " ).append("\n"); 
		query.append("	   FROM COM_INTG_CD_DTL CD " ).append("\n"); 
		query.append("	  WHERE CD.INTG_CD_ID = 'CD02146'" ).append("\n"); 
		query.append("	    AND CD.INTG_CD_VAL_CTNT = A.STWG_CD) STWG_CD" ).append("\n"); 
		query.append(",	a.XTER_SI_REF_NO" ).append("\n"); 

...
public class TransshipmentMgtDBDAOsearchBkgListForPortAssignRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                , (select /*+ index(ts1 xpkbkg_vvd) */" ).append("\n"); 
		query.append("						  max(decode(rownum,1,ts1.pod_cd))" ).append("\n"); 
		query.append("        			from bkg_vvd ts1" ).append("\n"); 
		query.append("        			where bk.bkg_no = ts1.bkg_no) ts1" ).append("\n"); 
		query.append("                , (select /*+ index(ts2 xpkbkg_vvd) */" ).append("\n"); 
		query.append("						  max(decode(rownum,2,ts2.pod_cd))" ).append("\n"); 
		query.append("        			from bkg_vvd ts2" ).append("\n"); 
		query.append("        			where bk.bkg_no = ts2.bkg_no) ts2" ).append("\n"); 
		query.append("                , (select /*+ index(ts3 xpkbkg_vvd) */" ).append("\n"); 
		query.append("						  max(decode(rownum,3,ts3.pod_cd))" ).append("\n"); 
		...
		query.append("           and in_vvd.pod_yd_cd     = @[port]||@[yard]" ).append("\n"); 
		query.append("           #end" ).append("\n"); 

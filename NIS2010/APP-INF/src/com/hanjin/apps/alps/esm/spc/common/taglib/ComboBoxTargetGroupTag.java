 ...
 * @since J2EE 1.4
 */
public class ComboBoxTargetGroupTag extends ComboBoxTag {
	private static transient org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ComboBoxTargetGroupTag.class);
	String ofc = "";
	...
	protected String[][] getOptionList() throws EventException {
		...
		String[][] datas = null;
		try {
			response = (ComboxEventResponse) sc.searchTargetGroupComboList(ofc, getDel());
			DBRowSet rs = response.getRs();
			datas = new String[rs.getRowCount()][2];
			int idx = 0;
			while (rs.next()) {
				datas[idx][0] = rs.getString("grp_cd");
				datas[idx][1] = rs.getString("grp_cd") + "|" + rs.getString("grp_desc");
				idx++;
			}
		} catch (Exception e) {

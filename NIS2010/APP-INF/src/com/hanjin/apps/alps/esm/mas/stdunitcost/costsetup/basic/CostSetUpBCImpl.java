...
import com.hanjin.apps.alps.esm.mas.stdunitcost.costsetup.integration.CostSetUpDBDAO;
 ...
 *
 * @author Jeon Yun Joo
 * @see ESM_MAS_0012EventResponse,MASBC 각 DAO 클래스 참조
 * @since J2EE 1.6
 */
public class CostSetUpBCImpl extends BasicCommandSupport implements CostSetUpBC {
	...
	public void multiOtherCostSetup(SearchCostSetUpListVO[] searchCostSetUpListVOs, SearchConditionVO conditionVO, SignOnUserAccount account) throws EventException{
		try {			
			List<MasMnlCostStupVO> multiVoList = new ArrayList<MasMnlCostStupVO>();
			String[] arrSearch = conditionVO.getFYearweek().split("[-]");
			String cost_yrwk = arrSearch[0]+arrSearch[1];

			MasMnlCostStupVO masMnlCostStupVO;
			...
			for ( int i=0; i<searchCostSetUpListVOs .length; i++ ) {
				...
				if (!searchCostSetUpListVOs[i].getTpsAmt().equals(searchCostSetUpListVOs[i].getTpsOldAmt())){
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("TPS");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getTpsAmt());
					multiVoList.add(masMnlCostStupVO);
		  	     }
				if (!searchCostSetUpListVOs[i].getAesAmt().equals(searchCostSetUpListVOs[i].getAesOldAmt())){	
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("AES");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getAesAmt());
					multiVoList.add(masMnlCostStupVO);
				}
				...
				if (!searchCostSetUpListVOs[i].getTasAmt().equals(searchCostSetUpListVOs[i].getTasOldAmt())){
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("TAS");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getTasAmt());
					multiVoList.add(masMnlCostStupVO);
				}
				if (!searchCostSetUpListVOs[i].getIasAmt().equals(searchCostSetUpListVOs[i].getIasOldAmt())){
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("IAS");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getIasAmt());
					multiVoList.add(masMnlCostStupVO);
				}
				if (!searchCostSetUpListVOs[i].getEmsAmt().equals(searchCostSetUpListVOs[i].getEmsOldAmt())){
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("EMS");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getEmsAmt());						
					multiVoList.add(masMnlCostStupVO);
				}
				if (!searchCostSetUpListVOs[i].getComAmt().equals(searchCostSetUpListVOs[i].getComOldAmt())){
					masMnlCostStupVO = new MasMnlCostStupVO();
					masMnlCostStupVO.setRlaneCd(searchCostSetUpListVOs[i].getItmCd());
					masMnlCostStupVO.setTrdCd("COM");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getComAmt());
					multiVoList.add(masMnlCostStupVO);
				}
			}
			}
	...
	public void multiMTAbcCostSetup(SearchCostSetUpListVO[] searchCostSetUpListVOs, SearchConditionVO conditionVO, SignOnUserAccount account) throws EventException{
		try {			
			List<MasMnlCostStupVO> updateVoList = new ArrayList<MasMnlCostStupVO>();
			String[] arrSearch = conditionVO.getFYearweek().split("[-]");
			String cost_yrmon = arrSearch[0]+arrSearch[1];

			MasMnlCostStupVO masMnlCostStupVO = new MasMnlCostStupVO();
			//General Expense 에 일괄 업데이트
			MasMnlCostStupVO genExpVO = new MasMnlCostStupVO();
			...
			for ( int i=0; i<searchCostSetUpListVOs .length; i++ ) {	
				if ( searchCostSetUpListVOs[i].getIbflag().equals("U")){
					...
					masMnlCostStupVO.setTrdCd("COM");
					masMnlCostStupVO.setOtrExpnAmt(searchCostSetUpListVOs[i].getComAmt());
					updateVoList.add(masMnlCostStupVO);
	...
	public EventResponse createManualCostStupCopy(SearchConditionVO searchConditionVO, SignOnUserAccount account) throws EventException {
		...
		try {
          ...
          //1. MAS_MNL_COST_STUP테이블에서  TARGET 해당월을 삭제한다.
            dbDao.removeManualCostStupCopy(param);
          //2. MAS_MNL_COST_STUP테이블에  SOURCE 해당월을 복사해서  TARGET 데이타를 생성한다. 
	...
	public List<MtyRepoTESTRSCostVO> searchInterfaceData(SearchConditionVO searchConditionVO) throws EventException {
		try {
			return dbDao.searchInterfaceData(searchConditionVO);
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
	...
	public List<GeneralExpenseVO> searchGeneralExpenseAsList(GeneralExpenseVO generalExpenseVO) throws EventException {
		try { 
			generalExpenseVO.setCostYrmon(generalExpenseVO.getCostYrmon().replaceAll("-", ""));
			return dbDao.searchGeneralExpenseAsList(generalExpenseVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12203",new String[]{}).getMessage(), ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("COM12203",new String[]{}).getMessage(), ex);

...
*@FileName : BookingUtilDBDAOSearchCanadaFrobRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BookingUtilDBDAOSearchCanadaFrobRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 해당 vvd 안에 케나다 port를 거치는지 조회한다.
	  * </pre>
	  */
	public BookingUtilDBDAOSearchCanadaFrobRSQL(){
		setQuery();
		...
		query.append("FileName : BookingUtilDBDAOSearchCanadaFrobRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("AND	nvl(skd_cng_sts_cd, ' ') <> 'S') ) )" ).append("\n"); 
		query.append("AND SUBSTR(VVD.POL_CD, 1, 2) <> 'US'--POL이 미국일 때는 제외" ).append("\n"); 
		query.append("AND SUBSTR(VVD.POL_CD, 1, 2) <> 'CA'--POL이 CANADA일 때는 제외" ).append("\n"); 
		query.append("AND SUBSTR(VVD.POD_CD, 1, 2) <> 'CA'--POD가 CANADA일 때는 제외" ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("AND	nvl(skd_cng_sts_cd, ' ') <> 'S') ) )" ).append("\n"); 
		query.append("AND SUBSTR(@[pol_cd], 1, 2) <> 'US'--POL이 미국일 때는 제외" ).append("\n"); 
		query.append("AND SUBSTR(@[pol_cd], 1, 2) <> 'CA'--POL이 CANADA일 때는 제외" ).append("\n"); 
		query.append("AND SUBSTR(@[pod_cd], 1, 2) <> 'CA'--POD가 CANADA일 때는 제외" ).append("\n"); 

...
* 2012.05.16 박성진
* 1.0 Creation
* 2012.12.05 이윤정 CHM-201221834 : ACM Audit 기능에서 제계산때 마이너스 금액으로 떠서 앞으로 지불나갈 같은 벤더코드에서 자동 빠져서 계산 될수 있도록 시스템 교정 요청
* 2013.03.07 이윤정 CHM-201323197 : Canada Brokerage 시스템 로직 변경 요청 & 데이터 요청
* 2013.04.04 이윤정 CHM-201323818 : Special compensation audit상 Remarks 로직 조정 요청 
* 2013.04.30 이윤정 CHM-201324213 : 부킹캔슬 되거나 Split->Combine된 건은, 기지급된 금액이 회수될 수 있는 로직 추가 요청
* 2013.06.20 이윤정 CHM-201325292 : 잘못나간 Compensation 회수로직 추가 요청(zero sum)
* 2013.06.20 이윤정 CHM-201325297 : Compensation 지급 시, f/fwdr 코드 입력 구분로직 변경 요청
=========================================================*/
package com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.basic;

import java.util.List;

import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.integration.FFCommCalculationDBDAO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchACMFFContractInfoVO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchFFAGMTRateInfoVO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchFFBKGQTYInfoVO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchFFBookingInfoVO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchFFRevLanebndInfoVO;
import com.hanjin.apps.alps.esm.acm.acmcalculation.ffcommcalculation.vo.SearchFFSADateVO;
...
//				createZeroSumComm( con, bkgMap );
			...
			//CHM-201323197 : Canada Brokerage 시스템 로직 변경 요청 & 데이터 요청 시작
			// Canada Brokerage로 지급불가되어야 할 case 체크 한다.
			String canadaFFCmpn = dbDao.searchCanadaFFCmpn(bkgNo);
			
			if("1".equals(canadaFFCmpn) ){
				// 안지워진 것 대비... ACM_FF_CMPN_REV,	ACM_FF_CMPN_DTL, ACM_FF_CMPN
				this.removeFFCommNotIF(BkgInfolist);
				
				// 데이타가 존재하지 않을 경우 Error를 ACM_AGN_BKG_INFO에 저장한다.
				BkgInfolist.setCommProcRsltRsn(new ErrorHandler("ACM00046").getUserMessage());
				dbDao.modifyFFMasterErrorMSG( bkgNo,BkgInfolist.getCommProcRsltRsn());
			...
			//CHM-201323197 : Canada Brokerage 시스템 로직 변경 요청 & 데이터 요청 끝
...
//			
			...
			if("X".equals(BkgInfolist.getBkgStsCd())) {	// Interface 했는데 status가 cancel인 경우 Cancel amt를 구해온다.
				...
				if("1".equals(BkgInfolist.getFfCheck())) {
					...
					searchFFBKGCancelInfo(BkgInfolist); // CHM-201325292 : 잘못나간 Compensation 회수로직 추가 요청(zero sum)
					return;
				}
				...
				if("1".equals(BkgInfolist.getShFfCheckFlag())){
					...
					if("0".equals(canadaChk)) {
						// [CHM-201323818] Special compensation audit상 Remarks 로직 조정 요청 
						// 데이타가 존재하지 않을 경우 Error를 ACM_AGN_BKG_INFO에 저장한다.
						BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00047", new String[]{ BkgInfolist.getShprCntCd() + BkgInfolist.getShprCustSeq(),BkgInfolist.getBkgFfCntCd() + BkgInfolist.getBkgFfSeq()}).getUserMessage());
				...
				if(ffCustShprInfo != null && ffCustShprInfo.length() > 0){ 
					...
					BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00035", new String[]{ BkgInfolist.getFfCntCd() + BkgInfolist.getFfCustSeq(), BkgInfolist.getShprCntCd() + BkgInfolist.getShprCustSeq()}).getUserMessage());// Freight Forwarder Customer has interests with Shipper. Freight Forwarder[$s] Shipper[$s] 
					modifyFFErrorMSG(BkgInfolist); 
					
					
//					CHM-201221834
//					ACM Audit 기능에서 제계산때 마이너스 금액으로 떠서 앞으로 지불나갈 같은 벤더코드에서 자동 빠져서 계산 될수 있도록 시스템 교정 요청
					String old_if_cnt = "0";
					old_if_cnt = dbDao.searchOldIfCnt(bkgNo);
					if(!old_if_cnt.equalsIgnoreCase("0")){
						BkgInfolist.setNewFfCmpnSeq(dbDao.searchNewFFSeq(bkgNo)); 
						dbDao.addFFCommZeroSumExclu(BkgInfolist);						
					}
				...
				if ("US".equals(BkgInfolist.getFfCntCd()) && ("*".equals(BkgInfolist.getFmcNo()) || "".equals(BkgInfolist.getFmcNo())))
				{
					...
					BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00043", new String[]{ BkgInfolist.getFfCntCd()+BkgInfolist.getFfCustSeq() }).getUserMessage()); // Freight Forwarder does not have FMC No! Freight Forwarder[$s]
					modifyFFErrorMSG(BkgInfolist); 
					return;
				}
				// Error Message를 체크하여 Error Message가 존재할 경우 return 한다.
				...
				if( "999999".equals(BkgInfolist.getFfCustSeqTmp())) {
					...
					if( ffAGMTRateInfoList.size() > 1 ) {
						...
						if(!(totalAmt != 0)) { // 부동소수점의 == 사용은 지양한다.
							ffAGMTRateInfo = null; // Brokerage Rate List를 clear 한다.
						}
					}
				}
				...
				if( errorFlag ) {
					if(ffAGMTRateInfoList.size() > 1) {
						BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00026", new String[]{ "["+BkgInfolist.getBkgOfcCd()+"] ["  + BkgInfolist.getFfCntCd()+ "] [" + BkgInfolist.getFfCustSeq()+ "]"}).getUserMessage()); // Agreement Rate Info has multi-row selected!, Please check up the Agreement Rate Info! [$s]
						modifyFFErrorMSG(BkgInfolist); 
					} else {
						BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00036", new String[]{ BkgInfolist.getBkgOfcCd(), BkgInfolist.getFfCntCd() + BkgInfolist.getFfCustSeq()}).getUserMessage()); // Brokerage Agreement Rate Info does not exist! Office[$s] Freight Forwarder[$s] 
					}
					return;
				}
				...
				if(ffAGMTRateInfo.getFfDivCd().endsWith("BA")){
					...
					// ffCalcAmt '0'일 경우 Error Message 처리한다.
					if(!( Double.parseDouble(ffCalcAmt) != 0)) { // 부동소수점의 == 사용은 지양한다.
						BkgInfolist.setFfCmpnRmk(new ErrorHandler("ACM00029").getUserMessage()); // BF' BL Menifested Rate Amount in Brokerage/FF is '0' error!
						return;
					}
					actCommAmt = Double.parseDouble(BkgInfoTmp.getActCommAmt());
					BkgInfolist.setFfChgAmt(ffCalcAmt);
...
//				}
	...
	public void searchFFBKGCancelInfo(SearchFFBookingInfoVO BkgInfolist) throws EventException{
		...
		log.debug("\n\n\n yjyjyj BkgInfolist.toString() : "+BkgInfolist.toString());
...
//				BKG Cancel 상계 로직 추가  CHM-201324213 : 부킹캔슬 되거나 Split->Combine된 건은, 기지급된 금액이 회수될 수 있는 로직 추가 요청
//				return;
			}
			ff = dbDao.searchFFCancelDataCheck(bkgNo, ffCmpnCancelSeq); 
			log.debug("\n\n\n yjyj Integer.parseInt(ff) : "+ff);

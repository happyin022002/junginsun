 ...
 * @LastVersion : 1.0
 * 2007-01-05 Junghyung_kim 1.0 최초 생성
 * 2014.06.10 이윤정 [CHM-201430635] [ACM] R4J 패치 사전 작업 진행 (미사용 함수 주석처리로 작업 진행)
 * 2015.01.22 Sang-Hyun Kim 10만불 관련 EAI 추가 field 처리
 */
...
/**
 * eNIS-agt에 대한 EAI 처리를 담당
 * - eNIS-agt Business Logic을 처리하기 위한 EAI 작업수행.
 * 
 * @author Junghyung_kim
 * @see BRKGAuditBCImpl 참조
 * @since J2EE 1.4
 */
public class AGNCommApprovalEAIDAO extends EAIDAOSupport {
	 ...
	 * @throws DAOException
	 */	
	public FNS0080003Document[] transferAtOnceACMToEAIByWS(String csrNo, DBRowSet rowSet, ComAproRqstRoutVO rout) throws DAOException {
		...
		try {
			...
			log.info("\n ACM START CSRNO : "+csrNo+", FNS0080003Document[] - arr_leng:"+docReq.length+" <<<<\n");
			...
			if(rowSet != null) { 
				for (int i=0; i<docReq.length; i++){
		            ...
		            log.info("\n transferAtOnceACMToEAIByWS == "+params);
		            log.info("\n transferAtOnceACMToEAIByWS == "+param);
		            invoiceCollReq = fNS0080003Req.addNewDataArea().addNewAPInvoiceCollection();
		            log.info("\n invoiceCollReq="+invoiceCollReq);
		            cnt = 1; //reset cnt

					// Agreement 존재여부 Flag 조회.
					String agmtEvidCfmFnlCd = JSPUtil.getNull(new ApprovalUtil().searchAgmtCfmCd(csrNo));
					while (cnt<=100 && rowSet.next()) {
						invoiceReq = invoiceCollReq.addNewAPInvoiceRequest();
						log.info("\n invoiceReq====="+invoiceReq);
			            setInvoiceDatas(rowSet, invoiceReq, timestamp, rout, agmtEvidCfmFnlCd);
			            cnt++;
					}
	 ...
	 * @throws Exception
	 */
	private void setInvoiceDatas(DBRowSet rowSet, APInvoiceRequest invoiceReq, String timestamp, ComAproRqstRoutVO rout, String agmtEvidCfmFnlCd) throws Exception {
...
//				invoiceReq.setHATTRIBUTE1((String)rowSet.getString("HDR_ATTR_CTNT1"));
				...
				log.error(">>>>>AGNCommApprovalEAIDAO.setInvoiceDatas : " + rowSet.getString("HDR_IF_DT"));
				...
				invoiceReq.setHREFERENCEKEY1(agmtEvidCfmFnlCd);
			}
		} catch (Exception e) {
			log.error(e.getMessage(),e);
			throw new DAOException(e.getMessage());
...
//	/**
//	 * JMS 연동 / 비동기<br>
//	 * 범한EDI 전송용 (ESM_ACM_010,  ESM_ACM_036) Agent Commission 내역을 Request or Approval(Audit)시 NIS로 인터페이스한다.<br>
//	 * 모든 데이타를 한번에 연동 처리한다.<br>
//	 * 2014.06.10 이윤정 [CHM-201430635] [ACM] R4J 패치 사전 작업 진행 (미사용 함수 주석처리로 작업 진행)
//	 * @param DBRowSet rowSet
//	 * @throws DAOException
...
//			log.error("\n ACM START ESM0620001Document[] - arr_leng:"+docReq.length+" <<<<\n");
//			
//            String instanceId = "";
//            String timestamp  = "";
//            
...
////					}
//		            timestamp = (new SimpleDateFormat("yyMMddHHmm")).format(new Date());
////		            instanceId = "ESM062-" + timestamp + "ACM" +"("+startNo+"/"+endNo+")"; //2007.05.31 modify
//		            instanceId = "ESM062-" + timestamp + "ACM";

...
public class MultiDimensionRPTDBDAOSearchPfitLssRptItmListRSQL implements ISQLTemplate{
	...
	public MultiDimensionRPTDBDAOSearchPfitLssRptItmListRSQL(){
		...
		params.put("f_op_view",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("	, CASE  WHEN @[f_op_view] ='OP1' AND RPT_ITM_DESC = 'OPCOST01' THEN 'OP Cost Total(OP5)'" ).append("\n"); 
		query.append("			WHEN @[f_op_view] ='OP1' AND RPT_ITM_DESC = 'OPCOST02' THEN 'OP-OP5'" ).append("\n"); 
		query.append("			WHEN @[f_op_view] ='OP4' AND RPT_ITM_DESC = 'OPCOST01' THEN 'OP Cost Total(OP6)'" ).append("\n"); 
		query.append("			WHEN @[f_op_view] ='OP4' AND RPT_ITM_DESC = 'OPCOST02' THEN 'OP4-OP6'" ).append("\n"); 
		query.append("			WHEN @[f_op_view] ='OP5' AND RPT_ITM_DESC = 'OPCOST01' THEN 'OP Cost Total(OP5)'" ).append("\n"); 
		query.append("			WHEN @[f_op_view] ='OP6' AND RPT_ITM_DESC = 'OPCOST01' THEN 'OP Cost Total(OP6)'" ).append("\n"); 
		query.append("			ELSE  RPT_ITM_DESC END AS RPT_ITM_DESC" ).append("\n"); 
		query.append("FROM MAS_PFIT_LSS_RPT_ITM " ).append("\n"); 
		query.append("WHERE RPT_VW_CD = @[f_pro_vw] " ).append("\n"); 
		query.append("  AND STND_COST_CD NOT IN ( '52101013', '52201013', '52101012', '52201012'" ).append("\n"); 
		...
		query.append("			AND (STND_COST_TP_CD IN ('S','C',@[str_display],DECODE(@[f_tab],'Y','U')) OR STND_COST_CD IN ('OPCOST00','OPCTOTAL','OPB00000')) " ).append("\n"); 
		query.append("		#elseif (${str_display} =='O')" ).append("\n"); 
		query.append("			AND STND_COST_TP_CD IN ('S','C',@[str_display],DECODE(@[f_tab],'Y','U','G','G')) " ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		#if(${f_op_view} =='OP5' || ${f_op_view} =='OP6')" ).append("\n"); 
		query.append("			AND STND_COST_CD NOT IN ('OPCOST01','OPCOST02')" ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("        " ).append("\n"); 
		query.append("        #if(${f_ias_flg} == 'Y')" ).append("\n"); 
		query.append("			AND STND_COST_CD NOT IN ('OPCOST01','OPCOST02', 'NTCOINIT', 'NTMASLOC')" ).append("\n"); 
		query.append("        #end" ).append("\n"); 
		...
		query.append("			AND STND_COST_TP_CD IN ('S','C','O',DECODE(@[f_tab],'G','G')) " ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("	#end		" ).append("\n"); 

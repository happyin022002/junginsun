...
public class FullCostDBDAOSearchLinkCostListVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * MAS_LNK_AVG_STND_COST, MAS_COST_SRC_ACCT, MAS_STND_ACCT 테이블의 데이터 조회(Empty)   
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("FROM MAS_COST_SRC_ACCT A1, MAS_STND_ACCT A2" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		query.append("AND A1.COST_SRC_SYS_CD = 'TRS'" ).append("\n"); 
		query.append("AND A1.STND_COST_CD = A2.STND_COST_CD" ).append("\n"); 
		query.append("GROUP BY A1.STND_COST_CD, A1.MAS_COST_SRC_CD) B2" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		query.append("AND B1.MAS_COST_SRC_CD = B2.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("GROUP BY B1.LNK_FM_NOD_CD, B1.LNK_TO_NOD_CD, B2.STND_COST_CD, CUBE(B2.MAS_COST_SRC_CD)" ).append("\n"); 

 ...
 *@FileName : ESM_PRI_7006HTMLAction
 ...
 * @see EsmPri7006Event 참조
 * @since J2EE 1.6
 */

public class ESM_PRI_7006HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 7116963942112395646L;

	/**
	 * ESM_PRI_7006HTMLAction 객체를 생성
	 */
	public ESM_PRI_7006HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri7006Event event = new EsmPri7006Event();
		if (command.isCommand(FormCommand.MULTI)) {
			event.setCopyTariffIhcVOs((CopyTariffIhcVO[]) getVOs(request, CopyTariffIhcVO.class, ""));
			event.setiHCGuidelineMainVOs((IHCGuidelineMainVO[]) getVOs(request, IHCGuidelineMainVO.class, ""));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.setCheckCopyServiceScopeVOs((CheckCopyServiceScopeVO[]) getVOs(request, CheckCopyServiceScopeVO.class, ""));

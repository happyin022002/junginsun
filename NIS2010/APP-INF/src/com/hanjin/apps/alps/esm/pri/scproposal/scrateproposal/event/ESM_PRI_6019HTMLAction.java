...
*@FileName : ESM_PRI_6019HTMLAction.java
...
public class ESM_PRI_6019HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6019HTMLAction 객체를 생성
	 */
	public ESM_PRI_6019HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri6019Event event = new EsmPri6019Event();

		if(command.isCommand(FormCommand.MULTI)) {
			event.setPriSpScpScgAdjVOS((PriSpScpScgAdjVO[])getVOs(request, PriSpScpScgAdjVO .class,""));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setPriSpScpScgAdjVOS((PriSpScpScgAdjVO[])getVOs(request, PriSpScpScgAdjVO .class,""));	
 
		}else if(command.isCommand(FormCommand.SEARCH)) {
			event.setRsltPriSurchargeAdjustListVO((RsltPriSurchargeAdjustListVO)getVO(request, RsltPriSurchargeAdjustListVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setRsltPriCheckSurchargeNoteListVO((RsltPriCheckSurchargeNoteListVO)getVO(request, RsltPriCheckSurchargeNoteListVO .class));
		}

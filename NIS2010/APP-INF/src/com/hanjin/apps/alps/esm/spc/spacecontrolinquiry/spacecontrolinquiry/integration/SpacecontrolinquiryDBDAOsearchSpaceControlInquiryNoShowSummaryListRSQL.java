...
public class SpacecontrolinquiryDBDAOsearchSpaceControlInquiryNoShowSummaryListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${mon_flg} == 'Y')" ).append("\n"); 
		query.append("       AND P.R_MONTH IS NOT NULL" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		...
		query.append("                   || MAX(DECODE(ROWNUM, 5, '|'||TRD_CD, ''))" ).append("\n"); 
		query.append("                   || MAX(DECODE(ROWNUM, 6, '|'||TRD_CD, ''))" ).append("\n"); 
		query.append("                   || MAX(DECODE(ROWNUM, 7, '|'||TRD_CD, '')) AS TRD_CD," ).append("\n"); 

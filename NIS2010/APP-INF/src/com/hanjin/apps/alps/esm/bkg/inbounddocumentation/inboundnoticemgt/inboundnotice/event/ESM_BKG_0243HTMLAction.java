 ...
 * @see EsmBkg0243Event 참조
 * @since J2EE 1.4
 */

public class ESM_BKG_0243HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0243HTMLAction 객체를 생성
	 */
	public ESM_BKG_0243HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0243Event event = new EsmBkg0243Event();
		
		// 저장
		if(command.isCommand(FormCommand.MULTI01)) {
			BkgArrNtcVO[] bkgArrNtcVOS = (BkgArrNtcVO[]) getVOs(request, BkgArrNtcVO.class,"sheet2_");
			
			event.setBkgArrNtcVOS(bkgArrNtcVOS);
		...
		// 조회
			...
			event.setAnFomCd(request.getParameter("an_fom_cd"));
			event.setAgent(request.getParameter("chn_agn_cd"));
			
			//event.setMrnRtnYdVO((ArrNtcWdDtlVO) getVO(request, ArrNtcWdDtlVO.class));

...
*@FileName : ACMSimulationDBDAOManageAGNCommMassSimListCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ACMSimulationDBDAOManageAGNCommMassSimListCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ACMSimulationDBDAOManageAGNCommMassSimListCSQL(){
		setQuery();
		...
		query.append("FileName : ACMSimulationDBDAOManageAGNCommMassSimListCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("INSERT INTO ACM_CALC_BAT(BAT_RCV_DT,BAT_RCV_SEQ,BKG_NO,COMM_TP_CD,BAT_FLG,BAT_ITM_NM,BAT_END_DT,SLAN_CD,RLANE_CD,VSL_CD,SKD_VOY_NO,SKD_DIR_CD,REV_DIR_CD,ORG_LOC_CD,DEST_LOC_CD,BDR_FLG,CALL_FM_SRC_ID,BAT_DESC,CRE_USR_ID,CRE_DT,UPD_USR_ID,UPD_DT,CALC_FM_DT,CALC_TO_DT,AGN_AGMT_NO)" ).append("\n"); 
		...
		query.append(" (SELECT TO_CHAR('S' || TO_CHAR(SYSDATE, 'YYMMDD') || TO_CHAR( NVL( MAX(TO_NUMBER(SUBSTR(BAT_ITM_NM, 8,4))) + 1, 1) , 'FM0000')) FROM ACM_CALC_BAT WHERE BAT_RCV_DT = TO_CHAR(SYSDATE, 'YYYYMMDD') AND COMM_TP_CD = 'S' AND SUBSTR(BAT_ITM_NM, 2,6) = TO_CHAR(SYSDATE, 'YYMMDD')) AS BAT_ITM_NM," ).append("\n"); 
		query.append(" NULL BAT_END_DT," ).append("\n"); 
		...
		query.append("			MAX(R.SLAN_CD) SLAN_CD," ).append("\n"); 
		query.append("			MAX(R.RLANE_CD) RLANE_CD," ).append("\n"); 
		query.append("			SUBSTR(R.VSL_CD||R.SKD_VOY_NO||R.DIR_CD, 0, 4) VSL_CD," ).append("\n"); 
		query.append("			SUBSTR(R.VSL_CD||R.SKD_VOY_NO||R.DIR_CD, 5, 4) SKD_VOY_NO," ).append("\n"); 
		query.append("			SUBSTR(R.VSL_CD||R.SKD_VOY_NO||R.DIR_CD, 9) DIR_CD," ).append("\n"); 
		query.append("			MAX(R.REV_DIR_CD) REV_DIR_CD" ).append("\n"); 
		query.append("      FROM ACM_AGN_COMM A," ).append("\n"); 
		query.append("           MAS_RGST_BKG R," ).append("\n"); 
		...
		query.append("	   (SELECT COLUMN_VALUE FROM TABLE(BKG_SPLIT_CLOB_FNC(${bkg_no}" ).append("\n"); 
		query.append("	    )))" ).append("\n"); 
		...
		query.append("       AND R.VSL_CD||R.SKD_VOY_NO||R.DIR_CD IN (${vvd_cd})    --## ${}로 받음" ).append("\n"); 
		query.append("       #end" ).append("\n"); 
		query.append("    #end" ).append("\n"); 

...
**@FileName : ESM_MAS_0223HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0223Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0223HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0223HTMLAction 객체를 생성
	 */
	public ESM_MAS_0223HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0223Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0223Event event = new EsmMas0223Event();  	
    			
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setOnewayCntrUploadVO((OnewayCntrUploadVO)getVO(request, OnewayCntrUploadVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {			
			event.setOnewayCntrUploadVOs((OnewayCntrUploadVO[])getVOs(request, OnewayCntrUploadVO.class, ""));			
		}
		else {
			event.setOnewayCntrUploadVO((OnewayCntrUploadVO)getVO(request, OnewayCntrUploadVO.class));

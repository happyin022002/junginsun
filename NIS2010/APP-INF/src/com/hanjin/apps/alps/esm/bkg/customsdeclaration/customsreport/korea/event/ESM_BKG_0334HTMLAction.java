...
*@FileName : ESM_BKG_0334HTMLAction.java
*@FileTitle : ESM_BKG_0334HTMLAction
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0334HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 2792293631096541043L;

	public ESM_BKG_0334HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		// EVENT 생성
		EsmBkg0334Event event = new EsmBkg0334Event();
		
		// COMMAND 구분에 따른 처리
		if(command.isCommand(FormCommand.SEARCH)) 
    	{
			// 조회의 경우 처리
			KorDischLocCondVO korDischLocCondVO = (KorDischLocCondVO)getVO(request, KorDischLocCondVO.class);
			event.setKorDischLocCondVO(korDischLocCondVO);
    	}else if( command.isCommand(FormCommand.MULTI))	{
    		// 입력/수정/삭제의 경우 처리
    		event.setKorDischLocCondVOs((KorDischLocCondVO[])getVOs(request, KorDischLocCondVO.class));    		
    	}

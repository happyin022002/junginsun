 ...
 * 1.0 Creation
=========================================================*/
package com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.basic;

import java.util.List;

import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.basic.CustomsTransmissionBCImpl;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.integration.DubaiCustomsTransmissionDBDAO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.vo.DubaiBlInfoVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.vo.DubaiCntrInfoVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.vo.DubaiCntrMfInfoVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.vo.DubaiManifestTransmitVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.dubai.vo.DubaiVvdInfoVO;
 ...
 * @see ESM_BKG_1085EventResponse,BrcsCustomsTransmissionBC 각 DAO 클래스 참조
 * @since J2EE 1.6
 */
public class DubaiCustomsTransmissionBCImpl extends CustomsTransmissionBCImpl {
	...
	public String transmitManifest(ManifestTransmitVO[] manifestTransmitVOs, SignOnUserAccount account)
		...
		try
		{
			...
			{
				// (1) VESSEL 정보
				if("AEAUH".equals(dubaiManifestTransmitVOs[i].getPodCd())){
					if(i == 0){
						dubaiVvdInfoVO = dbDao.searchFlatFileVvdInfo(dubaiManifestTransmitVOs[i]);
						// 1. AGENT CODE("A214")
						flatFile.append(setFlatFile("A214", false));
						// 2. IDENTIFIER("VES" - Vessel Record)
						flatFile.append(setFlatFile("VES", false));
						// 3. SER NO(Line Serial Number)
						flatFile.append(setFlatFile(Integer.toString(++seqCnt), false));
						// 4. VESSEL NAME
						flatFile.append(setFlatFile(dubaiVvdInfoVO.getVslNm(), false));
						// 5. AGENT'S VOYAGE NUMBER
						flatFile.append(setFlatFile(dubaiVvdInfoVO.getSkdVoyNo() + dubaiVvdInfoVO.getSkdDirCd(), false));
						// 6. EXPECTED TO ARRIVE DATE
						flatFile.append(setFlatFile(dubaiVvdInfoVO.getEtaDt(), true));
					}
					// (2) BL 정보
					...
					if (dubaiBlInfoVO != null)
					{
						// 1. AGENT CODE("A214")
						flatFile.append(setFlatFile("A214", false));
						// 2. IDENTIFIER("MCM" - Port of Loading)
						flatFile.append(setFlatFile("MCM", false));
						// 3. SER NO(Line Serial Number)
						flatFile.append(setFlatFile(Integer.toString(++seqCnt), false));
						// 4. BILL OF LADING NO(MASTER BL)
						flatFile.append(setFlatFile(dubaiBlInfoVO.getBlNo(), false));
						// 5. PORT CODE OF LOADING
						flatFile.append(setFlatFile(dubaiBlInfoVO.getPolCd(), true));
						
						// 1. AGENT CODE("A214")
						flatFile.append(setFlatFile("A214", false));
						// 2. IDENTIFIER("DCM" - Hosue BL)
						flatFile.append(setFlatFile("DCM", false));
						// 3. SER NO(Line Serial Number)
						flatFile.append(setFlatFile(Integer.toString(++seqCnt), false));		
						// 4. BILL OF LADING NO(House BL)
						flatFile.append(setFlatFile(dubaiBlInfoVO.getBlNo(), false));
						// 5. PACKAGE TYPE CODE
						...
						// 12. GROSS WEIGHT IN KG
						flatFile.append(setFlatFile(dubaiBlInfoVO.getGrsWgt(), false));
						// 13. CARGO VOLUME IN CUBICMETRE
						flatFile.append(setFlatFile(dubaiBlInfoVO.getMeasQty(), false));
						// 14. CARGO CODE
						flatFile.append(setFlatFile(dubaiBlInfoVO.getDuCgoCd(), true));

//						// 4. ORGINAL BOL NUMBER
...
//						blCnt++;
					...
					for (int cntrIdx = 0; cntrIdx < dubaiCntrInfoVOList.size(); cntrIdx++)
					{
						...
						dubaiCntrMfInfoVO = dubaiCntrMfInfoVOList.get(0);
						// 1. AGENT CODE("A214")
						flatFile.append(setFlatFile("A214", false));
						// 2. IDENTIFIER("CTR" - Containers Record)
						flatFile.append(setFlatFile("CTR", false));
						// 3. SER NO(Line Serial Number)
						flatFile.append(setFlatFile(Integer.toString(++seqCnt), false));
						// 4. CONTAINER NUMBER
						flatFile.append(setFlatFile(dubaiCntrInfoVO.getCntrNo()+ dubaiCntrInfoVO.getCheckDigit(), false));
						// 5. SEAL NO
						flatFile.append(setFlatFile(dubaiCntrInfoVO.getCntrSealNo(), false));
						// 6. SIZE
						flatFile.append(setFlatFile(dubaiCntrInfoVO.getSz(), false));
						// 7. SERVICE
						flatFile.append(setFlatFile("", false));
						// 8. CONSIGNMENT WEIGHT IN KG
						flatFile.append(setFlatFile(dubaiCntrMfInfoVO.getCntrMfWgt(), false));
						// 9. CONSIGNMENT VOLUME IN CUBIC METRES
						flatFile.append(setFlatFile(dubaiCntrMfInfoVO.getMeasQty(), false));
						// 10. REMARKS
						flatFile.append(setFlatFile("", true));
					}

				}  	// POD가 "AEAUH"(Abu Dhabi)인 경우 FLAT 생성(2013.10.14)
				else{
					if (i == 0)
					{
						// (1) 배정보
	...
	//					flatFile.append(setFlatFile(dubaiVvdInfoVO.getDuInstlNo(), false));
					...
					// AESHJ, AEKLF PORT 인 경우에는 BOL, CTR, CON, CTR, CON 이런 구조로 변경
					// [CHM-201322611] Manifest/U.A.E Manifest 요청
					if("AESHJ".equals(dubaiVvdInfoVO.getPodCd())||"AEKLF".equals(dubaiVvdInfoVO.getPodCd()))
					{
						dubaiCntrInfoVOList = dbDao.searchFlatFileCntrList(dubaiManifestTransmitVOs[i]);
						dubaiCntrMfInfoVOList = dbDao.searchFlatFileCntrMfList(dubaiManifestTransmitVOs[i]);
						for (int cntrIdx = 0; cntrIdx < dubaiCntrInfoVOList.size(); cntrIdx++)
						{
							...
							flatFile.append(setFlatFile(dubaiCntrInfoVO.getCntrSealNo(), false));
							// 5. CONTAINER TYPE
							flatFile.append(setFlatFile(dubaiCntrInfoVO.getCntrTpszIsoCd(), true));
							cntrCnt++;
							
							String cntrNo = "";						
							cntrNo = dubaiCntrInfoVO.getCntrNo()+dubaiCntrInfoVO.getCheckDigit();
							
							for (int cntrMfIdx = 0; cntrMfIdx < dubaiCntrMfInfoVOList.size(); cntrMfIdx++)
							{
								dubaiCntrMfInfoVO = dubaiCntrMfInfoVOList.get(cntrMfIdx);
								if( cntrNo.equals(dubaiCntrMfInfoVO.getCntrNo()))
								{
				...
				} // POD가 "AEAUH"(Abu Dhabi)가 아닌 경우 FLAT 생성(2013.10.14)
			} // end for (BL)
			// (5) CNTR COUNT / BL COUNT 정보
			if (blCnt > 0 && !"AEAUH".equals(dubaiManifestTransmitVOs[0].getPodCd()))
			{

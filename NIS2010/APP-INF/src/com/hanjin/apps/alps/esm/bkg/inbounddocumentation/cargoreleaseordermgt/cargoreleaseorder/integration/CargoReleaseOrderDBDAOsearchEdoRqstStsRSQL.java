...
public class CargoReleaseOrderDBDAOsearchEdoRqstStsRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", " ).append("\n"); 
		query.append("CASE ( SELECT  COUNT(1)" ).append("\n"); 
		query.append("			  FROM  BKG_EDO_CNTR_PTY_TRSP EC" ).append("\n"); 
		query.append("			 WHERE EC.EDO_RQST_NO = X.EDO_RQST_NO" ).append("\n"); 
		query.append("			 AND ROWNUM =1" ).append("\n"); 
		query.append("      )" ).append("\n"); 
		query.append("WHEN 0 THEN 'N'" ).append("\n"); 
		query.append("ELSE 'Y'" ).append("\n"); 
		query.append("END AS CNTR_SELF_TRSP_FLAG" ).append("\n"); 
		query.append(" " ).append("\n"); 
		...
		query.append("     , MAX(RPTY.PTY_EML)          AS PR_PTY_EML            -- 세금계산서 공급 받는자 email " ).append("\n"); 

...
=========================================================*/
package com.hanjin.apps.alps.esm.bkg.customsdeclaration.manifestlistdownload.vietnam.event;
...
public class ESM_BKG_1149HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if( command.isCommand(FormCommand.SEARCH) ){
			...
			VietnamManifestListCondVO vietnamManifestListCondVO = (VietnamManifestListCondVO) getVO(request,VietnamManifestListCondVO.class);
			VietnamManifestListBlInfoVO[] vietnamManifestListBlInfoVOs = (VietnamManifestListBlInfoVO[]) getVOs(request,VietnamManifestListBlInfoVO.class, "sheet1_");
			
			
			List<VietnamManifestListBlInfoVO> VietnamBlList = new ArrayList<VietnamManifestListBlInfoVO>();
			
			
			if( vietnamManifestListBlInfoVOs != null )
			{
				for( int i = 0; i < vietnamManifestListBlInfoVOs.length; i++ )
				{
					VietnamBlList.add(vietnamManifestListBlInfoVOs[i]);
				}
			}

			VietnamManifestTransmitVO vietnamManifestTransmitVOs[] = new VietnamManifestTransmitVO[1];
			vietnamManifestTransmitVOs[0] = new VietnamManifestTransmitVO();
			
			vietnamManifestTransmitVOs[0].setVietnamManifestListCondVO(vietnamManifestListCondVO);
			vietnamManifestTransmitVOs[0].setVietnamManifestListBlInfoVOs(VietnamBlList);
			event.setManifestTransmitVOs(vietnamManifestTransmitVOs);

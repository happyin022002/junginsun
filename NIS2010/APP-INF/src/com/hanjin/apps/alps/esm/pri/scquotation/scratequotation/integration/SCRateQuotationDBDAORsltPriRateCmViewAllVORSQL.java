...
*@FileName : SCRateQuotationDBDAORsltPriRateCmViewAllVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCRateQuotationDBDAORsltPriRateCmViewAllVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCRateQuotationDBDAORsltPriRateCmViewAllVORSQL(){
		setQuery();
		...
		query.append("FileName : SCRateQuotationDBDAORsltPriRateCmViewAllVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	SELECT QTTN_NO ,QTTN_VER_NO ,GEN_SPCL_RT_TP_CD ,CMDT_HDR_SEQ ,ROUT_SEQ" ).append("\n"); 
		...
		query.append("		FROM 	VW_PRI_SQ_RT_ROUT_PNT_ORI" ).append("\n"); 
		...
		query.append("		FROM 	VW_PRI_SQ_RT_ROUT_PNT_DEST" ).append("\n"); 
		query.append("		UNION ALL" ).append("\n"); 
		query.append("		SELECT " ).append("\n"); 
		query.append("			QTTN_NO" ).append("\n"); 
		query.append("			,QTTN_VER_NO" ).append("\n"); 
		...
		query.append("		FROM 	VW_PRI_SQ_RT_ROUT_VIA_ORI" ).append("\n"); 
		query.append("		UNION ALL" ).append("\n"); 
		query.append("		SELECT " ).append("\n"); 
		query.append("			QTTN_NO" ).append("\n"); 
		query.append("			,QTTN_VER_NO" ).append("\n"); 
		...
		query.append("		FROM 	VW_PRI_SQ_RT_ROUT_VIA_DEST" ).append("\n"); 
		query.append("	)" ).append("\n"); 
		query.append("	GROUP BY QTTN_NO ,QTTN_VER_NO ,GEN_SPCL_RT_TP_CD ,CMDT_HDR_SEQ ,ROUT_SEQ" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("SELECT QTTN_NO,QTTN_VER_NO,GEN_SPCL_RT_TP_CD,CMDT_HDR_SEQ,ROUT_SEQ" ).append("\n"); 
		...
		query.append("	,DECODE(TOT_PRE_TOT_QTY,0,0,TO_CHAR(ROUND(NVL(PRS_PRE_LOD_QTY,0) / TOT_PRE_TOT_QTY * 100,2),'990.99')) || '%'  AS PRE_SHARE  --Previous - Share(%) - 해당 Route Performance의 전체 Performance대비 Share" ).append("\n"); 
		...
		query.append("	,DECODE(TOT_ESTM_TOT_QTY,0,0,TO_CHAR(ROUND(NVL(PRS_ESTM_LOD_QTY,0) / TOT_ESTM_TOT_QTY * 100 ,2),'990.99')) || '%' AS ESTM_SHARE -- Estimate - Share(%)  -- 해당 Route Performance의 전체 Performance대비 Share" ).append("\n"); 
		...
		query.append("FROM (" ).append("\n"); 
		query.append("	SELECT CR.QTTN_NO,CR.QTTN_VER_NO,CR.GEN_SPCL_RT_TP_CD,CR.CMDT_HDR_SEQ,CR.ROUT_SEQ" ).append("\n"); 
		...
		query.append("	FROM PRI_SQ_RT_CMDT_HDR H" ).append("\n"); 
		query.append("		, VW_PRI_SQ_RT_CMDT C" ).append("\n"); 
		query.append("		, PRI_SQ_RT_CMDT_ROUT CR" ).append("\n"); 

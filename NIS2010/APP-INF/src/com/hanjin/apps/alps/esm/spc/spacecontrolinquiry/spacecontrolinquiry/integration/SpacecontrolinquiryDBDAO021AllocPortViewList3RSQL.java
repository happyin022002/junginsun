...
*@FileName : SpacecontrolinquiryDBDAO021AllocPortViewList3RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SpacecontrolinquiryDBDAO021AllocPortViewList3RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SpacecontrolinquiryDBDAO021AllocPortViewList3RSQL(){
		setQuery();
		...
		query.append("FileName : SpacecontrolinquiryDBDAO021AllocPortViewList3RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       AND A2.PROP_STS_CD       = 'F' " ).append("\n"); 
		...
		query.append("#if (${check_sector5} == 'Y')" ).append("\n"); 
		query.append("                     SQM_SCTR_CFM_QTA   SQ, " ).append("\n"); 
		query.append("                     SQM_SCTR_PAIR_MGMT SQP, " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                 AND A.COST_YR || A.COST_WK BETWEEN O.OFC_APLY_FM_YRWK AND O.OFC_APLY_TO_YRWK" ).append("\n"); 
		query.append("                 AND B.SLS_RGN_OFC_CD = O.OFC_CD" ).append("\n"); 
		query.append("#if (${rhq2} != '')" ).append("\n"); 
		...
		query.append("           DECODE(Z1.TRD_CD||Z1.LOD_SPL_CNG_FLG,'IASY',SUM(DECODE(Z1.NUM, ${key}, Z1.BKG_WGT, 0)),SUM(DECODE(Z1.NUM, ${key}, NVL(Z1.BSA_WGT, 0), 0))) AS BSA${key}1_WGT, " ).append("\n"); 
		query.append("#if(${checkVvd2} == 'Y')" ).append("\n"); 
		...
		query.append("         DECODE(Z4.T2, 'Y', 'Z', Z4.CUST_CTRL_CD) CUST_CTRL_CD, " ).append("\n"); 
		query.append("         DECODE(Z4.USA_BKG_MOD_CD, 'OTH', 'OTHERS', Z4.USA_BKG_MOD_CD) USA_BKG_MOD_CD," ).append("\n"); 
		query.append("         NVL(Z4.ACCT_CD, '+') AS ACCT_CD1, " ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("                                                Z4.TRD_CD, Z4.SUB_TRD_CD, Z4.RLANE_CD, Z4.DIR_CD, Z4.RHQ_CD, Z4.AQ_CD, Z4.VVD1, Z4.OFC_CD, DECODE(Z4.T2, 'Y', 'Z', NVL(Z4.CUST_CTRL_CD,0)) " ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("         DECODE(Z4.CUST_CTRL_CD, NULL, DECODE(T2, 'Y', 'GUIDE', ''), 'GUIDE') AS T" ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		...
		query.append("                   MAX(Z3.QTA${key}1) OVER(PARTITION BY Z3.DEST, Z3.TRD_CD, Z3.SUB_TRD_CD, Z3.RLANE_CD, Z3.DIR_CD, Z3.RHQ_CD, Z3.AQ_CD, Z3.OFC_CD, Z3.CUST_CTRL_CD, Z3.GID, Z3.VVD${key}) AS QTA${key}1," ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                   MAX(Z3.MDL${key}1) OVER (PARTITION BY Z3.DEST, Z3.TRD_CD, Z3.SUB_TRD_CD, Z3.RLANE_CD, Z3.DIR_CD, Z3.RHQ_CD, Z3.AQ_CD, Z3.OFC_CD) AS MDL_TTL${key}1," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                   ROUND(DECODE(MAX(Z3.QTA${key}1) OVER (PARTITION BY Z3.DEST, Z3.TRD_CD, Z3.SUB_TRD_CD, Z3.RLANE_CD, Z3.DIR_CD, Z3.RHQ_CD, Z3.AQ_CD, Z3.OFC_CD, Z3.CUST_CTRL_CD, Z3.GID, Z3.VVD${key}), 0," ).append("\n"); 
		query.append("                                0," ).append("\n"); 
		query.append("                                Z3.FCT${key}1 * 100 / MAX(Z3.QTA${key}1) OVER (PARTITION BY Z3.DEST, Z3.TRD_CD, Z3.SUB_TRD_CD, Z3.RLANE_CD, Z3.DIR_CD, Z3.RHQ_CD, Z3.AQ_CD, Z3.OFC_CD, Z3.CUST_CTRL_CD, Z3.GID, Z3.VVD${key})" ).append("\n"); 
		...
		query.append("                                         (RHQ_CD IS NULL AND AQ_CD IS NULL AND OFC_CD IS NULL AND CUST_CTRL_CD IS NULL AND ( (USA_BKG_MOD_CD IS NOT NULL AND ACCT_CD IS NULL AND DEST_LOC_CD IS NULL) OR (ACCT_CD IS NOT NULL AND USA_BKG_MOD_CD IS NULL AND DEST_LOC_CD IS NULL) OR (DEST_LOC_CD IS NOT NULL AND ACCT_CD IS NULL AND USA_BKG_MOD_CD IS NULL)))" ).append("\n"); 
		query.append("                                         OR (RHQ_CD IS NOT NULL AND AQ_CD IS NOT NULL AND OFC_CD IS NULL AND CUST_CTRL_CD IS NULL AND ( (USA_BKG_MOD_CD IS NOT NULL AND ACCT_CD IS NULL AND DEST_LOC_CD IS NULL) OR (ACCT_CD IS NOT NULL AND USA_BKG_MOD_CD IS NULL AND DEST_LOC_CD IS NULL) OR (DEST_LOC_CD IS NOT NULL AND ACCT_CD IS NULL AND USA_BKG_MOD_CD IS NULL)))" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                                        ) THEN 'Y'" ).append("\n"); 
		...
		query.append("                              CUBE(Z2.SUB_TRD_CD, Z2.DEST, Z2.RLANE_CD, Z2.DIR_CD, Z2.RHQ_CD, Z2.AQ_CD, Z2.OFC_CD, Z2.CUST_CTRL_CD, USA_BKG_MOD_CD, ACCT_CD, DEST_LOC_CD" ).append("\n"); 
		query.append("    #else" ).append("\n"); 
		query.append("                              Z2.TRD_CD    ," ).append("\n"); 
		query.append("                              Z2.SUB_TRD_CD," ).append("\n"); 
		query.append("                              CUBE(Z2.RLANE_CD, Z2.DEST, Z2.DIR_CD, Z2.RHQ_CD, Z2.AQ_CD, Z2.OFC_CD, Z2.CUST_CTRL_CD, USA_BKG_MOD_CD, ACCT_CD, DEST_LOC_CD" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("#if(${checkVvd2} == 'Y')" ).append("\n"); 
		query.append("                                  ,Z2.NUM_1" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("#if (${subtrade1} == '')" ).append("\n"); 
		query.append("                              Z2.TRD_CD  ," ).append("\n"); 
		query.append("                              CUBE(Z2.SUB_TRD_CD, Z2.RLANE_CD, Z2.DIR_CD, Z2.RHQ_CD, Z2.AQ_CD, Z2.OFC_CD, Z2.CUST_CTRL_CD, USA_BKG_MOD_CD, ACCT_CD, DEST_LOC_CD" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                              Z2.TRD_CD    ," ).append("\n"); 
		query.append("                              Z2.SUB_TRD_CD," ).append("\n"); 
		query.append("                              CUBE(Z2.RLANE_CD, Z2.DIR_CD, Z2.RHQ_CD, Z2.AQ_CD, Z2.OFC_CD, Z2.CUST_CTRL_CD , USA_BKG_MOD_CD, ACCT_CD, DEST_LOC_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if(${checkVvd2} == 'Y')" ).append("\n"); 
		query.append("                                  ,Z2.NUM_1" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("   AND NOT ( Z4.RHQ_CD <> '+' AND Z4.AQ_CD <> '+' AND Z4.OFC_CD <> '+' AND Z4.USA_BKG_MOD_CD <> '+' AND (Z4.ACCT_CD = '+' OR Z4.DEST_LOC_CD = '+'))" ).append("\n"); 
		...
		query.append("/*" ).append("\n"); 
		query.append("기존 Allocation Status의 기능을 지역본부에서 요청한 Format으로 구분" ).append("\n"); 
		query.append("2013.02.20 최윤성 [CHM-201322502-01] SPC 프로젝트 - 성수기 선복운영 개선을 위한 T/F추진" ).append("\n"); 
		query.append("2013.02.22 김시몬[CHM-201323235-01] WAX 노선은 Dest. 를 AU 로 집계되도록 보완, WAXIA 의 경우 Direction 변경(E=> W, W=> E)" ).append("\n"); 
		query.append("2013.02.28 김시몬[CHM-201322502-01] BKG실적 가져올때 MDM_DTL_REV_LANE 테이블과 연계시 TRD_CD를 체크하도록 수정" ).append("\n"); 
		...
		query.append("2014.02.26 신자영 [CHM-201428699] Daily FCST status report _ TTL 값 산출 logic변경" ).append("\n"); 
		query.append("2014.03.14 김시몬 [선처리] BKG RFA NULL 관련 보완" ).append("\n"); 
		query.append("2014.03.17 [CHM-201429601] SMP/Allocation control보완 요청 - SPC_GET_SMP_AMEND_FNC 적용" ).append("\n"); 
		...
		query.append("2014.11.04 박은주 [CHM-201432710]  Daily FCST -Dest/Local-IPI 조회기능 보완요청 QTA에 대해서 Yield Group 기준으로 중첩된 정보에 대해서는 0으로 나오도록 처리함(추가요청사항)" ).append("\n"); 

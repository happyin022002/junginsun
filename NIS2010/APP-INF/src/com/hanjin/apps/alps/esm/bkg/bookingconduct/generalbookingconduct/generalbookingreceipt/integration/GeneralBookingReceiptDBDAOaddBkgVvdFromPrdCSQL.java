...
public class GeneralBookingReceiptDBDAOaddBkgVvdFromPrdCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   AND SUBSTR(ORG_NOD_CD, 1, 5) <> CASE WHEN 0 < (SELECT COUNT(1) " ).append("\n"); 
		query.append("                                                    FROM BKG_HRD_CDG_CTNT" ).append("\n"); 
		query.append("                                                   WHERE 1=1 " ).append("\n"); 
		query.append("                                                     AND HRD_CDG_ID  = 'EQUAL_POL_POD' -- POL POD 같아도 ROUTE 저장 되는 경우" ).append("\n"); 
		query.append("                                                     AND ATTR_CTNT10 = 'Y'  " ).append("\n"); 
		query.append("                                                     AND ATTR_CTNT1  = SUBSTR(DEST_NOD_CD, 1, 5))" ).append("\n"); 
		query.append("                                   THEN 'X' -- 의미 없음   " ).append("\n"); 
		query.append("                                   ELSE SUBSTR(DEST_NOD_CD, 1, 5) END" ).append("\n"); 

...
*@FileName : SCRateProposalDBDAORsltRtCmdtDtlListVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCRateProposalDBDAORsltRtCmdtDtlListVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCRateProposalDBDAORsltRtCmdtDtlListVORSQL(){
		setQuery();
		...
		query.append("FileName : SCRateProposalDBDAORsltRtCmdtDtlListVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      ,FIRST_VALUE(DECODE(PRC_CMDT_TP_CD, 'G', 1, 'C', 2, 99)) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, CMDT_SEQ ORDER BY AMDT_SEQ) AS N1ST_ORD_REF" ).append("\n"); 
		query.append("      ,FIRST_VALUE(PRC_CMDT_DEF_CD) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, CMDT_SEQ ORDER BY AMDT_SEQ) AS N2ND_ORD_REF" ).append("\n"); 
		...
		query.append("              ,SRC_INFO_CD" ).append("\n"); 
		query.append("              ,N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,LEAD(N1ST_CMNC_AMDT_SEQ) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, CMDT_SEQ ORDER BY AMDT_SEQ) AS NEXT_N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,ACPT_USR_ID" ).append("\n"); 
		query.append("              ,ACPT_OFC_CD" ).append("\n"); 
		...
		query.append("           AND GEN_SPCL_RT_TP_CD = @[gen_spcl_rt_tp_cd]) A" ).append("\n"); 

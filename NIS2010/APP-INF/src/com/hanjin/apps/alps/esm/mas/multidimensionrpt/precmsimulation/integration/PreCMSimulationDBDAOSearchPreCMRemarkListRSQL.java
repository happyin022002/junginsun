...
public class PreCMSimulationDBDAOSearchPreCMRemarkListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 2014.12.30 김시몬 op 추가, ORDER BY 순서 변경
	  * 2015.01.08 김시몬 OP(Lane) 추가
	  * 2015.01.20 김시몬 OP(분기로 조회하도록 변경)
	...
	public void setQuery(){
		...
		query.append("				,'' VNDR" ).append("\n"); 
		query.append("                ,'Y' AS OP_CHK" ).append("\n"); 
		...
		query.append("				AND		a2.MAS_COST_SRC_CD = '43201011'   --DEM/DET계정과 동일한 설정으로" ).append("\n"); 
		...
		query.append("				,DECODE(A1.MAS_COST_SRC_CD" ).append("\n"); 
		...
		query.append("						,A1.MAS_COST_SRC_CD ) MAS_COST_SRC_CD -- 2011.01.11 이윤정 [CHM-201108216-01] TMFDFL 계정에 대해서 TMFDMT으로 변환하여 보여주는 부분을 제거." ).append("\n"); 
		query.append("				,DECODE(A1.MAS_COST_SRC_CD" ).append("\n"); 
		...
		query.append("						,MAS_GET_COM_NM_FNC('mas_cost_src_cd', A1.MAS_COST_SRC_CD) ) MAS_COST_SRC_NM -- 2011.01.11 이윤정 [CHM-201108216-01] TMFDFL 계정에 대해서 TMFDMT으로 변환하여 보여주는 부분을 제거." ).append("\n"); 
		query.append("				,DECODE(A1.MAS_COST_SRC_CD" ).append("\n"); 
		...
		query.append("				, 'Y' AS OP_CHK" ).append("\n"); 
		query.append("		FROM	MAS_COM_COST_PARA A1" ).append("\n"); 
		...
		query.append("								,B1.RA_ACCT_CD ) MAS_COST_SRC_CD   --STP Income을 STP Cost로" ).append("\n"); 
		...
		query.append("				FROM	MAS_COM_SVC_TRNS_PRC_PARA B1, MAS_OFC_ROUT_MAPG B2" ).append("\n"); 
		query.append("				WHERE	B1.SLS_ACT_CD = B2.SLS_ACT_CD" ).append("\n"); 
		query.append("				AND		B1.OFC_CLSS_CD = B2.OFC_CLSS_CD" ).append("\n"); 
		query.append("				AND		B2.COST_YRMON = MAS_BZC_COST_YRMON_FNC('')" ).append("\n"); 
		...
		query.append("		AND		MAS_COST_SRC_PRT_CD IN(DECODE(@[f_cob_profit_lv], 'C', 'CO', 'CO'), DECODE(@[f_cob_profit_vw], 'P', 'PA', 'RA'))   --MAS_COST_SRC_PRT_CD:R,P" ).append("\n"); 
		...
		query.append("        " ).append("\n"); 
		query.append("        SELECT NOD_CD" ).append("\n"); 
		query.append("               , NOD_CD||COST_ACT_GRP_SEQ AS COST_ACT_GRP_SEQ" ).append("\n"); 
		query.append("               , GRP" ).append("\n"); 
		query.append("               , STND_COST_CD" ).append("\n"); 
		query.append("               , NVL(STND_COST_NM,SGRP_COST_CD_DESC) AS STND_COST_NM" ).append("\n"); 
		query.append("               , ' '           AS SGRP_COST_CD" ).append("\n"); 
		query.append("               , STND_COST_NM AS SGRP_COST_CD_DESC               " ).append("\n"); 
		query.append("               , AMT" ).append("\n"); 
		query.append("               , DECODE(NVL(STND_COST_CD,'X'),'X',' ','Average') AS CTRT_RTN_FLG" ).append("\n"); 
		query.append("               , DECODE(NVL(STND_COST_CD,'X'),'54400000','Slot Price','53101000','Port Tariff/TEU','53102000','Canal Tariff/TEU','X',' ','TTL Cost/TEU')  AS COST_CALC_RMK" ).append("\n"); 
		query.append("               , STND_COST_CD  AS ACCT_DP_SEQ" ).append("\n"); 
		...
		query.append("                       , 'Common'              AS GRP" ).append("\n"); 
		query.append("                       , STND_COST_CD" ).append("\n"); 
		query.append("                       , STND_COST_NM" ).append("\n"); 
		query.append("                       , SGRP_COST_CD_DESC                       " ).append("\n"); 
		query.append("                       , SUM(TEU_UC_AMT * (CNTR_TEU + @[f_void_qty]))  AS AMT" ).append("\n"); 
		query.append("                  FROM (     " ).append("\n"); 
		query.append("                        SELECT   D1.TRD_CD||' / '||D1.RLANE_CD as NOD_CD" ).append("\n"); 
		query.append("                               , D2.ACCT_DP_SEQ        AS COST_ACT_GRP_SEQ" ).append("\n"); 
		query.append("                               , D1.STND_COST_CD       AS STND_COST_CD" ).append("\n"); 
		query.append("                               , D2.STND_COST_NM       AS STND_COST_NM" ).append("\n"); 
		query.append("                               , D2.MGRP_COST_CD_DESC  AS SGRP_COST_CD_DESC  " ).append("\n"); 
		query.append("                               , D1.TEU_UC_AMT " ).append("\n"); 
		query.append("                               , D3.CNTR_TEU" ).append("\n"); 
		...
		query.append("                                       DECODE(A.MGRP_COST_CD,'OV',1001,'VF',1004,'OF',1007,'GE',1009) AS ACCT_DP_SEQ" ).append("\n"); 
		...
		query.append("                       )" ).append("\n"); 
		query.append("                 GROUP BY NOD_CD, COST_ACT_GRP_SEQ, STND_COST_CD, STND_COST_NM, SGRP_COST_CD_DESC" ).append("\n"); 
		query.append("               )		" ).append("\n"); 
		query.append("   	   )" ).append("\n"); 
		query.append(" WHERE OP_CHK = 'Y'		" ).append("\n"); 

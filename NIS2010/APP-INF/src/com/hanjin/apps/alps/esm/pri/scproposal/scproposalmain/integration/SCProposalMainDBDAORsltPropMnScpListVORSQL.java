...
public class SCProposalMainDBDAORsltPropMnScpListVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 2015.03.30 송호진[CHM-201534006] Scope 영역 (Amd 색상 처리 표시 규칙) 관련 쿼리 수정
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("      ,MN.CHSS_EXPT_FLG" ).append("\n"); 
		query.append("      ,MN.GRI_APPL_FLG" ).append("\n"); 
		query.append("      ,MN.NEW_SCG_FLG" ).append("\n"); 
		query.append("      ,SIGN (" ).append("\n"); 
		query.append("            ( SELECT COUNT(1)" ).append("\n"); 
		query.append("              FROM   PRI_SP_SCP_MN X" ).append("\n"); 
		query.append("              WHERE  X.PROP_NO = MN.PROP_NO" ).append("\n"); 
		query.append("              AND    X.SVC_SCP_CD = MN.SVC_SCP_CD" ).append("\n"); 
		query.append("              AND    X.AMDT_SEQ < MN.AMDT_SEQ ) ) AS NEW_INS_FLG " ).append("\n"); 
		query.append("	  , ( SELECT SUM ( DECODE ( S.AMDT_FLG, 'Y', 1, 0 ) ) " ).append("\n"); 
		query.append("		  FROM 	 PRI_SP_SCP_AMDT_SMRY S" ).append("\n"); 
		query.append("	      WHERE  S.PROP_NO = MN.PROP_NO" ).append("\n"); 
		query.append("          AND    S.AMDT_SEQ = MN.AMDT_SEQ" ).append("\n"); 
		query.append("          AND    S.SVC_SCP_CD = MN.SVC_SCP_CD ) AS AMDT_CNT" ).append("\n"); 
		query.append("	  , ( SELECT SUM ( DECODE ( S.ACPT_FLG, 'Y', 1, 0 ) ) " ).append("\n"); 
		query.append("		  FROM 	 PRI_SP_SCP_AMDT_SMRY S" ).append("\n"); 
		query.append("	      WHERE  S.PROP_NO = MN.PROP_NO" ).append("\n"); 
		query.append("          AND    S.AMDT_SEQ = MN.AMDT_SEQ" ).append("\n"); 
		query.append("          AND    S.SVC_SCP_CD = MN.SVC_SCP_CD ) AS ACPT_CNT" ).append("\n"); 

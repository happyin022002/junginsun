...
public class DubaiCustomsTransmissionDBDAOsearchFlatFileBlInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	  ,(SELECT NVL(UN_LOC_CD, BL.ORG_PORT_CD) FROM MDM_LOCATION WHERE LOC_CD = BL.ORG_PORT_CD) ORG_PORT_CD" ).append("\n"); 
		query.append("	  ,(SELECT NVL(UN_LOC_CD, BL.POR_CD) FROM MDM_LOCATION WHERE LOC_CD = BL.POR_CD) POR_CD" ).append("\n"); 
		query.append("	  ,(SELECT NVL(UN_LOC_CD, BL.POL_CD) FROM MDM_LOCATION WHERE LOC_CD = BL.POL_CD) POL_CD " ).append("\n"); 
		query.append("	  ,(SELECT NVL(UN_LOC_CD, BL.POD_CD) FROM MDM_LOCATION WHERE LOC_CD = BL.POD_CD) POD_CD  " ).append("\n"); 
		query.append("	  ,(SELECT NVL(UN_LOC_CD, BL.DEL_CD) FROM MDM_LOCATION WHERE LOC_CD = BL.DEL_CD) DEL_CD  " ).append("\n"); 
		...
		query.append("      ,DECODE(SUBSTR(BL.DU_CNTR_SVC_TP_CD, 2,1), 'L', NVL(SUBSTR(REPLACE(BL.MK_NO_CTNT, CHR(13)||CHR(10), ''),1,200), 'NIL'), 'NIL')" ).append("\n"); 
		query.append("       AS MK_NO_CTNT" ).append("\n"); 
		query.append("      ,BL.DU_CMDT_CD" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(BL.CMDT_DESC, CHR(13)||CHR(10), ''),1,100) AS CMDT_DESC" ).append("\n"); 
		...
		query.append("      ,TRIM(TO_CHAR(NVL(BL.CGO_WGT,0), '0000000.0')) AS CGO_WGT" ).append("\n"); 
		query.append("      ,TRIM(TO_CHAR(NVL(BL.GRS_WGT,0), '0000000.0')) AS GRS_WGT" ).append("\n"); 
		query.append("      ,TRIM(TO_CHAR(NVL(BL.MEAS_QTY,0), '0000000.0')) AS MEAS_QTY" ).append("\n"); 
		...
		query.append("      ,SUBSTR(REPLACE(REPLACE(CS.CUST_NM , CHR(13)||CHR(10), ''), CHR(13), ''),1,30)         AS S_CUST_NM" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CS.CUST_ADDR , CHR(13)||CHR(10), ''), CHR(13), ''),1,240)      AS S_CUST_ADDR" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CS.ORG_CUST_NM , CHR(13)||CHR(10), ''), CHR(13), ''),1,30)     AS S_ORG_CUST_NM" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CS.ORG_CUST_ADDR , CHR(13)||CHR(10), ''), CHR(13), ''),1,240)  AS S_ORG_CUST_ADDR" ).append("\n"); 
		query.append("      ,CC.CUST_CNT_CD     AS C_CUST_CNT_CD" ).append("\n"); 
		query.append("      ,CC.DU_CUST_ID      AS C_DU_CUST_ID" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CC.CUST_NM , CHR(13)||CHR(10), ''), CHR(13), ''),1,48)         AS C_CUST_NM" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CC.CUST_ADDR , CHR(13)||CHR(10), ''), CHR(13), ''),1,240)      AS C_CUST_ADDR" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CC.ORG_CUST_NM , CHR(13)||CHR(10), ''), CHR(13), ''),1,30)     AS C_ORG_CUST_NM" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CC.ORG_CUST_ADDR , CHR(13)||CHR(10), ''), CHR(13), ''),1,240)  AS C_ORG_CUST_ADDR" ).append("\n"); 
		query.append("      ,CN.CUST_CNT_CD     AS N_CUST_CNT_CD" ).append("\n"); 
		query.append("      ,CN.DU_CUST_ID      AS N_DU_CUST_ID" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CN.CUST_NM , CHR(13)||CHR(10), ''), CHR(13), ''),1,48)         AS N_CUST_NM" ).append("\n"); 
		query.append("      ,SUBSTR(REPLACE(REPLACE(CN.CUST_ADDR , CHR(13)||CHR(10), ''), CHR(13), ''),1,240)      AS N_CUST_ADDR" ).append("\n"); 

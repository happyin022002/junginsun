...
public class ChinaManifestListDownloadDBDAOsearchBlCustListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        DECODE(LENGTH(TRIM(BC.CSTMS_DECL_CNT_CD)),    0, NVL(BC.CUST_CNT_CD, ' ')" ).append("\n"); 
		query.append("										   		 , NULL, NVL(BC.CUST_CNT_CD, ' ')" ).append("\n"); 
		query.append("												 , BC.CSTMS_DECL_CNT_CD" ).append("\n"); 
		query.append("			  ) AS CNT_CD," ).append("\n"); 
		query.append("        NVL(BC.CUST_SEQ, 0)			AS CUST_SEQ," ).append("\n"); 
		query.append("        NVL(REPLACE(REPLACE(REPLACE(BC.CUST_NM ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' '), REPLACE(REPLACE(REPLACE(CNEE.CUST_NM ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' ')) AS CUST_NM," ).append("\n"); 
		query.append("		DECODE(BC.BKG_CUST_TP_CD, " ).append("\n"); 
		query.append("				'S', NVL(REPLACE(REPLACE(REPLACE(BC.CUST_ADDR ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' '), ' ')" ).append("\n"); 
		query.append("              , 'N', DECODE(TRIM(BC.CUST_ADDR), NULL, REPLACE(REPLACE(REPLACE(CNEE.CUST_ADDR ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' '), NVL(REPLACE(REPLACE(REPLACE(BC.CUST_ADDR ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' '), ' '))" ).append("\n"); 
		query.append("              , 'C', DECODE(BB.CUST_TO_ORD_FLG, 'Y', 'TO ORDER', NVL(REPLACE(REPLACE(REPLACE(BC.CUST_ADDR ,CHR(34)),CHR(9),' '), CHR(13)||CHR(10),' '), ' '))" ).append("\n"); 
		query.append("              , NULL" ).append("\n"); 
		query.append("      	) CUST_ADDR," ).append("\n"); 
		query.append("		BC.EORI_NO AS RGST_NO," ).append("\n"); 
		query.append("		BC.CO_CHN_TP_CD," ).append("\n"); 
		query.append("		BC.CUST_PHN_NO," ).append("\n"); 
		query.append("		BC.CUST_FAX_NO," ).append("\n"); 
		query.append("		BC.CUST_EML," ).append("\n"); 
		query.append("		BC.EUR_CSTMS_ST_NM AS CHN_CSTMS_ST_NM," ).append("\n"); 

...
*@FileName : ESM_FMS_0054_2HTMLAction.java
...
public class ESM_FMS_0054_2HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0054_2HTMLAction 객체를 생성
	 */
	public ESM_FMS_0054_2HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms00542Event event = new EsmFms00542Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setVslCd(request.getParameter("vsl_cd"));
			event.setDckSelCd(request.getParameter("dck_sel_cd"));
			event.setCustomDckSkdVOS((CustomDckSkdVO[])getVOs(request, CustomDckSkdVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setVslCd(request.getParameter("vsl_cd"));
			event.setDckSelCd(request.getParameter("dck_sel_cd"));
			event.setCustomDckSkdVO((CustomDckSkdVO)getVO(request, CustomDckSkdVO .class));
		}

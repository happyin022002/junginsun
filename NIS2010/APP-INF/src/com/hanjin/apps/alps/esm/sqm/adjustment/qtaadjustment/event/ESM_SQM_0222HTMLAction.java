...
*@FileName       : ESM_SQM_0222HTMLAction.java
 ...
 * @author SQM USER
 * @see EsmSqm0222Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0222HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SQM_0222HTMLAction 객체를 생성
	 */
	public ESM_SQM_0222HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0222Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSqm0222Event event = new EsmSqm0222Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setManageQtaEditIasSectorVOS((ManageQtaEditIasSectorVO[])getVOs(request, ManageQtaEditIasSectorVO .class,""));
		} else if (command.isCommand(FormCommand.SEARCH01)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		} else if (command.isCommand(FormCommand.MULTI01)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setSqmSctrLodRevVOs((SqmSctrLodRevVO[])getVOs(request, SqmSctrLodRevVO .class,""));
		}

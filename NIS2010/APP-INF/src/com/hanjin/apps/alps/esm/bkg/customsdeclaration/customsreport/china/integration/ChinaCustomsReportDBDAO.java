...
public class ChinaCustomsReportDBDAO extends DBDAOSupport {
	...
	public List<DelModeListVO> searchDelMode(BkgCstmsChnDeModCondVO bkgCstmsChnDeModCondVO) throws DAOException {
		...
		try
		{
			...
			DBRowSet dbRowset = new SQLExecuter("").executeQuery(
					(ISQLTemplate) new ChinaCustomsReportDBDAOsearchDelModeRSQL(), param, velParam);
	...
	public List<ChinaManifestListTransmitHistDetailVO> searchChinaManifestSendList(ChinaTransmitHistCondVO chinaTransmitHistCondVO) throws DAOException {
		...
		try {
			...
			list = (List)RowSetUtil.rowSetToVOs(dbRowset,
					ChinaManifestListTransmitHistDetailVO.class);
		} catch (SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage());
		} catch (Exception ex) {
	...
	public List<ChinaManifestSendDetailListVO> searchChinaSendDetailList(String ediRefId, String podCd) throws DAOException {
		...
		try
		{
			...
			DBRowSet dbRowset = new SQLExecuter("DEFAULT").executeQuery(
					(ISQLTemplate) new ChinaCustomsReportDBDAOsearchChinaSendDetailListRSQL(), param, null);
	 ...
	 * @throws EventException
	 */
	public List<BkgCstmsCCAMListVO> searchBkgCCAMList(BkgCstmsCCAMCondVO bkgCstmsCCAMCondVO) throws DAOException {

		List<BkgCstmsCCAMListVO> list = null;
		Map<String, Object> param = new HashMap<String, Object>();
		// velocity parameter
		Map<String, Object> velParam = new HashMap<String, Object>();
		...
		try
		{
			...
			Map<String, String> mapVO = bkgCstmsCCAMCondVO.getColumnValues();
			param.putAll(mapVO);
			velParam.putAll(mapVO);
			
			DBRowSet dbRowset = new SQLExecuter("DEFAULT").executeQuery(
					(ISQLTemplate) new ChinaCustomsReportDBDAOsearchBkgCCAMRSQL(), param, velParam);
			list = (List) RowSetUtil.rowSetToVOs(dbRowset, BkgCstmsCCAMListVO.class);
		}
		catch (SQLException se)
		{
			throw new DAOException(new ErrorHandler(se).getMessage(), se);

...
*@FileName : SurchargeAutoRatingDBDAOSearchTPFSurchargeRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SurchargeAutoRatingDBDAOSearchTPFSurchargeRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * TPF Surcharge 정보 조회
	  * 
	  * 1. BKG의 POR/POL이 CN location이고, (단 CNHKG 제외) Prepaid OFFICE 또는 PPD 3rd Party Office에 CN Location 이외의 Office가 들어가고 금액이 존재할 경우, Charge화면의 SAVE 버튼을 누를때, TPF를 자동 Interface 한다. (HKGBB는 부과 대상임).
	  * 2. BKG의 DEL/POD가 CN location이고, (단 CNHKG 제외) collect OFFICE 또는 CCT 3rd Party Office에 CN Location 이외의 Office가 들어가고 금액이 존재할 경우, Charge화면의 SAVE 버튼을 누를때, TPF를 자동 Interface 한다. (HKGBB는 부과 대상임) 
	  *    ☞ TPF 등록된 Surcharge table을 참고 금액 USD50과 Payment term은 계약에 따라 표기 한다. (서차지 등록조건)
	  *    ☞ 3rd party office가 있는 경우 Collect 옆에 3rd party office를 입력해 준다.
	  *       단, 3rd party office가 복수일 경우는 Office를 지정하지 않는다.
	  * </pre>
	  */
	public SurchargeAutoRatingDBDAOSearchTPFSurchargeRSQL(){
		setQuery();
		...
		query.append("FileName : SurchargeAutoRatingDBDAOSearchTPFSurchargeRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}

...
**@FileName : ESM_MAS_0276HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0276Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0276HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0276HTMLAction 객체를 생성
	 */
	public ESM_MAS_0276HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0276Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0276Event event = new EsmMas0276Event();  	
    			
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setDemDetCostReportbyBKGVO((DemDetCostReportbyBKGVO)getVO(request, DemDetCostReportbyBKGVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {			
			event.setDemDetCostReportbyBKGVOs((DemDetCostReportbyBKGVO[])getVOs(request, DemDetCostReportbyBKGVO.class, ""));			
		}
		else {
			event.setDemDetCostReportbyBKGVO((DemDetCostReportbyBKGVO)getVO(request, DemDetCostReportbyBKGVO.class));

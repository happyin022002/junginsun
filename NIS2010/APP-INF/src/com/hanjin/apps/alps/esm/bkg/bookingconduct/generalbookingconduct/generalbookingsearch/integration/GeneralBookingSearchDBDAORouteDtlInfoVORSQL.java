...
public class GeneralBookingSearchDBDAORouteDtlInfoVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("			 , MAX(ESTM_DT) DEL_ETA" ).append("\n"); 
		...
		query.append("		   AND D.NOD_CD	= (CASE WHEN BK.POD_CD = 'USLAX' AND BK.DEL_CD = 'USLGB' AND BK.DE_TERM_CD = 'Y' " ).append("\n"); 
		query.append("                                    THEN H.POD_NOD_CD" ).append("\n"); 
		query.append("						        WHEN BK.POD_CD = 'USLGB' AND BK.DEL_CD = 'USLAX' AND BK.DE_TERM_CD = 'Y' " ).append("\n"); 
		query.append("		    		       			THEN H.POD_NOD_CD" ).append("\n"); 
		query.append("							    ELSE H.DEL_NOD_CD" ).append("\n"); 
		query.append("						   END)" ).append("\n"); 
		query.append("		   AND SUBSTR(D.ACT_CD, 5, 1) =" ).append("\n"); 
		query.append("	               		  (CASE WHEN BK.POD_CD = 'USLAX' AND BK.DEL_CD = 'USLGB' AND BK.DE_TERM_CD = 'Y' " ).append("\n"); 

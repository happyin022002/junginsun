...
*@FileName : UnmatchBLDBDAOCreateTaaSurchargeInputCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class UnmatchBLDBDAOCreateTaaSurchargeInputCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * createTaaSurchargeInput
	  * </pre>
	  */
	public UnmatchBLDBDAOCreateTaaSurchargeInputCSQL(){
		setQuery();
		...
		query.append("FileName : UnmatchBLDBDAOCreateTaaSurchargeInputCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    ,PRC_GEN_SPCL_RT_TP_CD ,PRC_CMDT_HDR_SEQ  ,PRC_ROUT_SEQ ,PRC_RT_SEQ   ,OP_CNTR_QTY    ,TRI_PROP_NO, TRF_ITM_NO  " ).append("\n"); 
		query.append("    ,CHG_CD                ,CURR_CD           " ).append("\n"); 
		query.append("    ,CHG_UT_AMT   " ).append("\n"); 
		query.append("    ,RAT_AS_QTY            ,CHG_AMT      " ).append("\n"); 
		query.append("    ,RAT_UT_CD             ,CGO_CATE_CD       ,FRT_INCL_XCLD_DIV_CD" ).append("\n"); 
		query.append("    ,FRT_TERM_CD           ,PROP_NO           ,AMDT_SEQ     ,SVC_SCP_CD" ).append("\n"); 
		...
		query.append("    ,@[ctrt_cntr_tpsz_cd]   ,@[rcv_term_cd]      ,@[de_term_cd]   ,@[dry_cgo_flg] ,@[awk_cgo_flg]" ).append("\n"); 
		query.append("    ,@[dcgo_flg]            ,@[rc_flg]           ,@[bb_cgo_flg]   ,@[soc_flg]     ,@[imdg_clss_cd] " ).append("\n"); 
		query.append("    ,'S'                    ,0                   ,0               ,0              ,@[op_cntr_qty] ,@[tri_prop_no],@[tri_no]" ).append("\n"); 
		query.append("    ,@[chg_cd]              ,@[curr_cd] " ).append("\n"); 
		query.append("    ,@[chg_ut_amt]" ).append("\n"); 
		query.append("    ,@[rat_as_qty]          ,NVL(TO_NUMBER(@[chg_ut_amt]),0) * NVL(TO_NUMBER(@[rat_as_qty]),0)" ).append("\n"); 
		query.append("    ,@[rat_ut_cd]           ,@[cgo_cate_cd]      ,@[incl_oft_flg]" ).append("\n"); 
		query.append("    ,@[frt_term_cd]          ,@[prop_no]          ,@[amdt_seq]      ,@[svc_scp_cd]" ).append("\n"); 

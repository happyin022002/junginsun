...
*@FileName : ESM_BSA_0041HTMLAction.java
*@FileTitle : ESM_BSA_0041HTMLAction
*Open Issues :
*Change history :
...
public class ESM_BSA_0041HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BSA_0041HTMLAction 객체를 생성
	 */
	public ESM_BSA_0041HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBsa0041Event event = new EsmBsa0041Event();
		BsaBudSltPrcCrrVO bsaBudSltPrcCrrVO = new BsaBudSltPrcCrrVO();
    	
		if(command.isCommand(FormCommand.SEARCHLIST) ) {
			event.setBsaYearlyPlanConditionVO((BsaYearlyPlanConditionVO)getVO(request, BsaYearlyPlanConditionVO .class));			
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setBsaYearlyPlanConditionVO((BsaYearlyPlanConditionVO)getVO(request, BsaYearlyPlanConditionVO .class));
			event.setBsaBudSltPrcVOs((BsaBudSltPrcVO[])getVOs(request,BsaBudSltPrcVO.class,""));
			
			String[] arrTemp = new String[2];
			arrTemp[0] = event.getBsaYearlyPlanConditionVO().getCrrcd().substring(1).replace("|", ",");
			arrTemp[1] = event.getBsaYearlyPlanConditionVO().getBsaopjbcd().substring(1).replace("|", ",");
			
			event.setBsaBudSltPrcCrrVOs(bsaBudSltPrcCrrVO.fromRequestGridConv(request, arrTemp));
		}
		return  event;
	}

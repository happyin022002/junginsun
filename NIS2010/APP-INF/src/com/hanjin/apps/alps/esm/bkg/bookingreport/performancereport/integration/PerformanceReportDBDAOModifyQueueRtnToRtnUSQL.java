...
public class PerformanceReportDBDAOModifyQueueRtnToRtnUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            RTN_TO_RTN_DT = GLOBALDATE_PKG.TIME_LOCAL_FNC('MYPKG'), " ).append("\n"); 
		query.append("			RTN_TO_RTN_END_DT = GLOBALDATE_PKG.TIME_LOCAL_FNC('MYPKG')," ).append("\n"); 
		query.append("			DPCS_DOC_FM_DT =  GLOBALDATE_PKG.TIME_LOCAL_FNC('MYPKG')," ).append("\n"); 
		query.append("			RTN_TO_RTN_HRS =  (SELECT SUM(SR_PROC_HRS) FROM BKG_SR_HIS H WHERE H.SR_KND_CD=X.SR_KND_CD AND H.SR_NO = X.SR_NO AND H.BKG_NO = X.BKG_NO AND H.SR_STS_CD = 'RT')" ).append("\n"); 
		query.append("           " ).append("\n"); 

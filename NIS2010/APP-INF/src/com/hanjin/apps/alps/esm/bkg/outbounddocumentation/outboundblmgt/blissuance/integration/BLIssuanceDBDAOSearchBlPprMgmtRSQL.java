...
*@FileName : BLIssuanceDBDAOSearchBlPprMgmtRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BLIssuanceDBDAOSearchBlPprMgmtRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * BLIssuanceDBDAOSearchBlPprMgmtRSQL
	  * </pre>
	  */
	public BLIssuanceDBDAOSearchBlPprMgmtRSQL(){
		setQuery();
		...
		query.append("FileName : BLIssuanceDBDAOSearchBlPprMgmtRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      ,PPR.CUST_DTRB_KNT" ).append("\n"); 
		...
		query.append("              AND AUTH.PRN_CUST_CNT_CD = PPR.CUST_CNT_CD" ).append("\n"); 
		query.append("              AND AUTH.PRN_CUST_SEQ = PPR.CUST_SEQ" ).append("\n"); 
		query.append("#if (${cre_from_dt} != '' && ${cre_from_dt} != '') " ).append("\n"); 
		query.append("              AND AUTH.N1ST_PRN_DT >= TO_DATE(@[cre_from_dt], 'YYYY-MM-DD')" ).append("\n"); 
		query.append("              AND AUTH.N1ST_PRN_DT < TO_DATE(@[cre_to_dt], 'YYYY-MM-DD') + 1" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("              AND AUTH.N1ST_PRN_DT >= DECODE(@[dtrb_yr],'2015',TO_DATE('20150401','YYYYMMDD'),TO_DATE(@[dtrb_yr]||'0101','YYYYMMDD'))" ).append("\n"); 
		query.append("              AND AUTH.N1ST_PRN_DT < DECODE(@[dtrb_yr],'2015',TO_DATE('20160101','YYYYMMDD'),SYSDATE) " ).append("\n"); 
		...
		query.append("              AND AUTH.N1ST_PRN_DT >= DECODE(@[dtrb_yr],'2015',TO_DATE('20150401','YYYYMMDD'),TO_DATE(@[dtrb_yr]||'0101','YYYYMMDD'))" ).append("\n"); 
		query.append("              AND AUTH.N1ST_PRN_DT < DECODE(@[dtrb_yr],'2015',TO_DATE('20160101','YYYYMMDD'),SYSDATE) " ).append("\n"); 
		query.append("        ),0) AS CUST_ACCM_WDR_KNT" ).append("\n"); 
		query.append("      ,TO_CHAR(PPR.UPD_DT,'YYYY.MM.DD') AS UPD_DT" ).append("\n"); 
		query.append("  FROM BKG_CUST_BL_PPR_MGMT PPR" ).append("\n"); 
		query.append("      ,MDM_CUSTOMER MDM" ).append("\n"); 
		query.append(" WHERE PPR.DTRB_YR = @[dtrb_yr]" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("ORDER BY PPR.DTRB_YR" ).append("\n"); 
		query.append("        ,PPR.RHQ_CD" ).append("\n"); 
		query.append("        ,PPR.OFC_CD" ).append("\n"); 
		query.append("        ,PPR.CUST_CNT_CD" ).append("\n"); 
		query.append("        ,PPR.CUST_SEQ" ).append("\n"); 

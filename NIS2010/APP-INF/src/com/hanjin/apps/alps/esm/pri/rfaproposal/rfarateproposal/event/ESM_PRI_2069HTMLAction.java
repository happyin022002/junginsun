 ...
 *@FileName : ESM_PRI_2069HTMLAction.java
...
public class ESM_PRI_2069HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_PRI_2069HTMLAction 객체를 생성
	 */
	public ESM_PRI_2069HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri2069Event event = new EsmPri2069Event();
		event.setFicRtTpCd(JSPUtil.getParameter(request, "fic_rt_tp_cd", ""));
		event.setInOrgDestTpCd(JSPUtil.getParameter(request, "in_org_dest_tp_cd", ""));
		if (command.isCommand(FormCommand.SEARCH01)) {
			event.setPriRpScpRtCmdtHdrVO((PriRpScpRtCmdtHdrVO) getVO(request, PriRpScpRtCmdtHdrVO.class));
			event.setRsltRtListVerticalExcelForAddOnTariffVOs((RsltRtListVerticalExcelForAddOnTariffVO[]) getVOs(request, RsltRtListVerticalExcelForAddOnTariffVO.class, ""));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.setFicRateLoadExcelGuidelineCheckVO((FicRateLoadExcelGuidelineCheckVO) getVO(request, FicRateLoadExcelGuidelineCheckVO.class));
			event.setFicRateLoadExcelGuidelineCheckVOs((FicRateLoadExcelGuidelineCheckVO[]) getVOs(request, FicRateLoadExcelGuidelineCheckVO.class, ""));

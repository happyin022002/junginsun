...
*@FileName : ESM_SPC_0098HTMLAction.java
...
public class ESM_SPC_0098HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SPC_0030HTMLAction 객체를 생성
	 */
	public ESM_SPC_0098HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSpc0098Event event = new EsmSpc0098Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setSpcHdHulVOs((SpcHdHulVO[])getVOs(request, SpcHdHulVO .class));
		}

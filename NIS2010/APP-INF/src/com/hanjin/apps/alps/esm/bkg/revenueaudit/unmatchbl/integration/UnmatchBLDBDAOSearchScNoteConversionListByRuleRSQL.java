...
public class UnmatchBLDBDAOSearchScNoteConversionListByRuleRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 심사를 위해 특정 Rule Type (Conversion Table상 Code)을 포함하는 SC Note를 조회
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("    AND     H.SC_NO = NVL(@[ctrt_no], H.SC_NO)" ).append("\n"); 
		...
		query.append("    ,  GEN_SPCL_RT_TP_CD AS RT_TP" ).append("\n"); 
		...
		query.append("            ,   M.SC_NO AS CTRT_NO" ).append("\n"); 
		query.append("            ,   'S' AS BKG_CTRT_TP_CD" ).append("\n"); 
		query.append("            ,   M.PROP_NO" ).append("\n"); 
		query.append("            ,   M.AMDT_SEQ" ).append("\n"); 
		...
		query.append("        AND     NC.NOTE_CONV_TP_CD = NVL(@[note_conv_tp_cd], NC.NOTE_CONV_TP_CD)" ).append("\n"); 
		query.append("     )" ).append("\n"); 
		query.append("ORDER BY CTRT_NO, PROP_NO, AMDT_SEQ, CTRT_PTY_NM, SVC_SCP_CD, GEN_SPCL_RT_TP_CD, NOTE_CONV_TP_CD, DP_SEQ" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("  SELECT  SC_NO AS CTRT_NO" ).append("\n"); 
		query.append("        , 'S' AS BKG_CTRT_TP_CD" ).append("\n"); 
		query.append("        , AMDT_SEQ" ).append("\n"); 
		query.append("        , CTRT_PTY_NM" ).append("\n"); 
		query.append("        , SVC_SCP_CD" ).append("\n"); 
		...
		query.append("            WHERE USR_ID = SC.UPD_USR_ID " ).append("\n"); 
		query.append("               AND USE_FLG = 'Y' ) AS USR_NM" ).append("\n"); 
		query.append("        ,  COUNT(DISTINCT SC_NO) OVER (PARTITION BY 1) CTRT_CNT" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("        SELECT  H.SC_NO" ).append("\n"); 
		query.append("              , M.AMDT_SEQ" ).append("\n"); 
		query.append("              , P.CTRT_PTY_NM" ).append("\n"); 
		...
		query.append("              , ( SELECT C.INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL C WHERE C.INTG_CD_ID = 'CD01711' AND C.INTG_CD_VAL_CTNT = NT.NOTE_CLSS_CD ) NOTE_CLSS_CD" ).append("\n"); 
		query.append("              , CT.NOTE_CTNT" ).append("\n"); 
		query.append("              , CT.CHG_CD" ).append("\n"); 
		...
		query.append("            ,   PRI_SP_SCP_NOTE_CTNT CT" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_NOTE NT     " ).append("\n"); 
		query.append("            ,   PRI_SP_CTRT_PTY P" ).append("\n"); 
		query.append("        WHERE   M.PROP_NO = H.PROP_NO" ).append("\n"); 
		query.append("        AND     M.AMDT_SEQ = (  SELECT  MAX ( X.AMDT_SEQ ) " ).append("\n"); 
		query.append("                                  FROM  PRI_SP_MN X " ).append("\n"); 
		...
		query.append("        AND     H.SC_NO = @[ctrt_no]" ).append("\n"); 
		...
		query.append("              , ( SELECT C.INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL C WHERE C.INTG_CD_ID = 'CD01705' AND C.INTG_CD_VAL_CTNT = CT.GEN_SPCL_RT_TP_CD ) AS RT_TP" ).append("\n"); 
		query.append("              , 'Commodity' AS NOTE_CONV_TP_CD" ).append("\n"); 
		query.append("              , ( SELECT C.INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL C WHERE C.INTG_CD_ID = 'CD01711' AND C.INTG_CD_VAL_CTNT = CT.NOTE_CLSS_CD ) NOTE_CLSS_CD" ).append("\n"); 
		query.append("              , CT.NOTE_CTNT" ).append("\n"); 
		query.append("              , CT.CHG_CD" ).append("\n"); 
		query.append("              ,   (SELECT EFF_DT" ).append("\n"); 
		query.append("                     FROM PRI_SP_SCP_MN " ).append("\n"); 
		...
		query.append("            ,   PRI_SP_MN M" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_MN S" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_RT_CNOTE CT" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_RT_CMDT_HDR NT" ).append("\n"); 
		query.append("            ,   PRI_SP_CTRT_PTY P" ).append("\n"); 
		query.append("        WHERE   M.PROP_NO = H.PROP_NO" ).append("\n"); 
		query.append("        AND     M.AMDT_SEQ = (  SELECT  MAX ( X.AMDT_SEQ ) " ).append("\n"); 
		query.append("                                  FROM  PRI_SP_MN X " ).append("\n"); 
		...
		query.append("            ,   PRI_SP_MN M" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_MN S" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_RT_CMDT_RNOTE CT" ).append("\n"); 
		query.append("            ,   PRI_SP_SCP_RT_CMDT_HDR NT" ).append("\n"); 
		query.append("            ,   PRI_SP_CTRT_PTY P" ).append("\n"); 
		query.append("        WHERE   M.PROP_NO = H.PROP_NO" ).append("\n"); 
		query.append("        AND     M.AMDT_SEQ = (  SELECT  MAX ( X.AMDT_SEQ ) " ).append("\n"); 
		...
		query.append("ORDER BY SC_NO, AMDT_SEQ, CTRT_PTY_NM, SVC_SCP_CD, RT_TP, NOTE_CONV_TP_CD, DP_SEQ" ).append("\n"); 
		query.append("#end" ).append("\n"); 

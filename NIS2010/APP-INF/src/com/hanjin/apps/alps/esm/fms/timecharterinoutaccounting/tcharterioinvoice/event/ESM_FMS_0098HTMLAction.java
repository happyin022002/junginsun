...
*@FileName : ESM_FMS_0098HTMLAction.java
*@FileTitle : FRGN Exchange Transaction(O/A)
*Open Issues :
*Change history :
...
public class ESM_FMS_0098HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0014HTMLAction 객체를 생성
	 */
	public ESM_FMS_0098HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmFms0098Event event = new EsmFms0098Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			
			event.setFrgnExchangeTransactionVO((FrgnExchangeTransactionVO)getVO(request, FrgnExchangeTransactionVO.class));
			
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setFrgnExchangeTransactionVO((FrgnExchangeTransactionVO)getVO(request, FrgnExchangeTransactionVO.class));
			event.setFrgnExchangeVOS((FrgnExchangeVO[]) getVOs(request, FrgnExchangeVO .class,""));
		}

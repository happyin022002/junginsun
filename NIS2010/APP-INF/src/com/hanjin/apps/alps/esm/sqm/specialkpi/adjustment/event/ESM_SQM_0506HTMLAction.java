...
*@FileName       : ESM_SQM_0506HTMLAction.java
 ...
 * @author 이혜민
 * @see EsmSqm0506Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0506HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SQM_0506HTMLAction 객체를 생성
	 */
	public ESM_SQM_0506HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0506Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmSqm0506Event event = new EsmSqm0506Event();
    	if(command.isCommand(FormCommand.SEARCH)) {
    		event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setSqmSpclNewOfcVOS((SqmSpclNewOfcVO[])getVOs(request, SqmSpclNewOfcVO .class,""));

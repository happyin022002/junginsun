...
*@FileName : EsmBkg0924ViewAdapter.java
 ...
 * @since J2EE 1.6
 */
public class EsmBkg0924ViewAdapter extends ViewAdapter{
	/**
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		for(int i=0;i<realCnt;i++){
			...
			if (bflag){ 
				sTmp0=colValues.get("ft40");
				colValues = vos.get(i+1).getColumnValues();
				sTmp1=colValues.get("ft40");
				colValues = vos.get(i+2).getColumnValues();
				sTmp2=colValues.get("ft40");
				colValues = vos.get(i+3).getColumnValues();
				sTmp3=colValues.get("ft40"); 
			}
			if (sTmp0.length()==0 && sTmp1.length()==0 && sTmp2.length()==0 && sTmp3.length()==0){ 
				i=i+3;
				bflag=true;
			}else{

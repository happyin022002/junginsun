 ...
 *@FileName : ESM_PRI_2082HTMLAction.java
...
public class ESM_PRI_2082HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * UI_PRI_0070HTMLAction 객체를 생성
	 */
	public ESM_PRI_2082HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri2082Event event = new EsmPri2082Event();
		event.setFicRtTpCd(JSPUtil.getParameter(request, "fic_rt_tp_cd", ""));
		if (command.isCommand(FormCommand.MODIFY01)) {
			event.setPriRpScpRtActCustVOS((PriRpScpRtActCustVO[]) getVOs(request, PriRpScpRtActCustVO.class, ""));
		} else if (command.isCommand(FormCommand.MODIFY02)) {
			event.setPriRpScpRtActCustVOS((PriRpScpRtActCustVO[]) getVOs(request, PriRpScpRtActCustVO.class, ""));
		} else if (command.isCommand(FormCommand.SEARCH01)) {
			event.setPriRpScpRtActCustVO((PriRpScpRtActCustVO) getVO(request, PriRpScpRtActCustVO.class));
		}

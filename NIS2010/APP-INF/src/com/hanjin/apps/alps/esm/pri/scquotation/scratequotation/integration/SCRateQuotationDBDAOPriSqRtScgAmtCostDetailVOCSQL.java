...
*@FileName : SCRateQuotationDBDAOPriSqRtScgAmtCostDetailVOCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCRateQuotationDBDAOPriSqRtScgAmtCostDetailVOCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCRateQuotationDBDAOPriSqRtScgAmtCostDetailVOCSQL(){
		setQuery();
		...
		query.append("FileName : SCRateQuotationDBDAOPriSqRtScgAmtCostDetailVOCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("" ).append("\n"); 
		query.append("MERGE INTO PRI_SQ_RT_SCG A" ).append("\n"); 
		query.append("USING ( " ).append("\n"); 
		query.append("        SELECT QTTN_NO, QTTN_VER_NO, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, RT_SEQ, " ).append("\n"); 
		query.append("               BKG_RAT_UT_CD, PRC_CGO_TP_CD, CHG_CD, CURR_CD, " ).append("\n"); 
		query.append("               ADJ_SCG_AMT, ADJ_SCG_USD_AMT" ).append("\n"); 
		query.append("          FROM (SELECT MN.QTTN_NO, MN.QTTN_VER_NO, MN.GEN_SPCL_RT_TP_CD, MN.CMDT_HDR_SEQ, MN.ROUT_SEQ, RT.RT_SEQ, " ).append("\n"); 
		...
		query.append("                  FROM PRI_SQ_RT_CMDT_ROUT MN," ).append("\n"); 
		query.append("                       PRI_SQ_RT_ROUT_PNT ORG," ).append("\n"); 
		query.append("                       PRI_SQ_RT_ROUT_PNT DST," ).append("\n"); 
		query.append("                       (SELECT QTTN_NO, QTTN_VER_NO, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, ROUT_VIA_PORT_DEF_CD " ).append("\n"); 
		query.append("                          FROM PRI_SQ_RT_ROUT_VIA " ).append("\n"); 
		query.append("                          WHERE QTTN_NO = @[qttn_no]  AND QTTN_VER_NO =  @[qttn_ver_no]  AND GEN_SPCL_RT_TP_CD =@[gen_spcl_rt_tp_cd] AND ORG_DEST_TP_CD = 'O' ) OVIA," ).append("\n"); 
		query.append("                       (SELECT QTTN_NO, QTTN_VER_NO, GEN_SPCL_RT_TP_CD, CMDT_HDR_SEQ, ROUT_SEQ, ROUT_VIA_PORT_DEF_CD " ).append("\n"); 
		query.append("                          FROM PRI_SQ_RT_ROUT_VIA " ).append("\n"); 
		query.append("                          WHERE QTTN_NO = @[qttn_no]  AND QTTN_VER_NO =  @[qttn_ver_no]  AND GEN_SPCL_RT_TP_CD =@[gen_spcl_rt_tp_cd] AND ORG_DEST_TP_CD = 'D' ) DVIA," ).append("\n"); 
		query.append("                       PRI_SQ_RT_CMDT CMDT," ).append("\n"); 
		query.append("                       PRI_SQ_RT RT," ).append("\n"); 
		query.append("                       PRI_SQ_SCG_ADJ SCG_ADJ," ).append("\n"); 
		...
		query.append("-- 모든 ROUTE를 갱신" ).append("\n"); 
		...
		query.append("		   AND EXISTS ( SELECT 'O' FROM PRI_SQ_RT_SCG SCG" ).append("\n"); 
		...
		query.append("INSERT ( A.QTTN_NO, A.QTTN_VER_NO, A.GEN_SPCL_RT_TP_CD, A.CMDT_HDR_SEQ, A.ROUT_SEQ, A.RT_SEQ, " ).append("\n"); 
		...
		query.append("VALUES ( B.QTTN_NO, B.QTTN_VER_NO, B.GEN_SPCL_RT_TP_CD, B.CMDT_HDR_SEQ, B.ROUT_SEQ, B.RT_SEQ,     	" ).append("\n"); 

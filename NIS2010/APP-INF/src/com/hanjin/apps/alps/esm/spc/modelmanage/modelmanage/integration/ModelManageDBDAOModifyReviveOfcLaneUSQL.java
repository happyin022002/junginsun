...
*@FileName : ModelManageDBDAOModifyReviveOfcLaneUSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ModelManageDBDAOModifyReviveOfcLaneUSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 삭제 처리된 office 또는 lane을 재사용하기 위해 살립니다.
	  * [CHM-201322502-01] SPC 프로젝트 - 성수기 선복운영 개선을 위한 T/F추진
	  * 2014.02.04 [CHM-201428383-01] RFA 로직 추가
	  * 2014.05.16 [CHM-201430353] SMP / AES 보완요청 - SC 입력 기능 추가
	  * </pre>
	  */
	public ModelManageDBDAOModifyReviveOfcLaneUSQL(){
		setQuery();
		...
		query.append("FileName : ModelManageDBDAOModifyReviveOfcLaneUSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       SPC_CTRL_MDL_MNL_CD  = @[spc_ctrl_mdl_mnl_cd]," ).append("\n"); 
		query.append("       SPC_CTRL_MDL_MNL_RMK = DECODE(@[spc_ctrl_mdl_mnl_cd], 'O', SPC_CTRL_MDL_MNL_RMK||'['||TO_CHAR(SYSDATE,'YYYYMMDD')||'|'||@[usr_id]||'|OFC REUSE]'" ).append("\n"); 
		query.append("                                                           , 'L', SPC_CTRL_MDL_MNL_RMK||'['||TO_CHAR(SYSDATE,'YYYYMMDD')||'|'||@[usr_id]||'|LANE REUSE]')," ).append("\n"); 
		query.append("       RHQ_UPD_USR_ID = 'INIT'," ).append("\n"); 
		query.append("       RHQ_UPD_DT = SYSDATE," ).append("\n"); 
		query.append("       OFC_UPD_USR_ID = 'INIT'," ).append("\n"); 
		query.append("       OFC_UPD_DT = SYSDATE," ).append("\n"); 
		query.append("       LANE_UPD_USR_ID = 'INIT'," ).append("\n"); 
		query.append("       LANE_UPD_DT = SYSDATE       " ).append("\n"); 

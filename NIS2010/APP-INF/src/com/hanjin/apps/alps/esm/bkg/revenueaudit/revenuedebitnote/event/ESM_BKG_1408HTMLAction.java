...
*@FileName : ESM_BKG_1408HTMLAction.java
*@FileTitle : Evidence
*Open Issues :
*Change history :
...
public class ESM_BKG_1408HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1408HTMLAction 객체를 생성
	 */
	public ESM_BKG_1408HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		log.debug("[START::  ESM_BKG_1408HTMLAction  ]==========");

		EsmBkg1408Event event = new EsmBkg1408Event();
		String[] keys =null;
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			AttachmentVO attachmentVO = (AttachmentVO)getVO(request, AttachmentVO.class);
			event.setAttachmentVO(attachmentVO);

		}
		else if(command.isCommand(FormCommand.MULTI)) {

			AttachmentVO[] attachmentVOs 	= (AttachmentVO[])getVOs(request, AttachmentVO .class,"sheet1_");
			event.setAttachmentVOS(attachmentVOs);
			event.setKeys(keys);

		}

		request.setAttribute("Event", event);
		log.debug("[END:: perform ESM_BKG_1408HTMLAction  ]==========");
		return  event;
	}

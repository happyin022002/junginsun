...
public class NetworkDistributionDBDAOApplyToPLOP62CSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("MERGE INTO MAS_PFIT_LSS_SMRY A USING" ).append("\n"); 
		...
		query.append("                      , CASE WHEN A.TRD_CD IN ('IAS','IES','IMS') THEN NVL(B.N6TH_ASGN_AMT, 0) + NVL(B.IPT_ASGN_AMT, 0)+NVL(B.N4TH_CO_AMT, 0)" ).append("\n"); 
		...
		query.append("                      , CASE WHEN A.TRD_CD IN ('IAS','IES','IMS') THEN NVL(B.N6TH_ASGN_AMT, 0) + NVL(B.IPT_ASGN_AMT, 0)" ).append("\n"); 
		...
		query.append("                      , CASE WHEN A.TRD_CD IN ('IAS','IES','IMS') THEN NVL(B.N6TH_ASGN_AMT, 0) - NVL(B.N4TH_HJS_SLS_AMT, 0) " ).append("\n"); 

...
public class ChinaCustomsReportDBDAOsearchChinaSendListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("FROM (  SELECT S1.*, ROWNUM RN, COUNT(*) OVER() AS TOTAL," ).append("\n"); 
		...
		query.append("		SELECT 	DISTINCT DECODE(BKG_CSTMS_CHN_SND_LOG.CHN_MF_SND_IND_CD,'O','O1','D','O2','P','P1') SEND_INDICATOR, " ).append("\n"); 
		query.append("				DECODE(BKG_CSTMS_CHN_SND_LOG.TRSM_MSG_TP_ID,'9','Original(POL)','0','Other Original(POD)','3','Delete','5','Change') SEND_INDICATOR_NM, " ).append("\n"); 
		...
		query.append("                						ROW_NUMBER() OVER(PARTITION BY B.EDI_REF_ID ORDER BY B.BKG_POD_CD ) RNUM, " ).append("\n"); 
		...
		query.append("								AND 	A.MF_SND_DT BETWEEN TO_DATE(@[start_snd_dt],'YYYY-MM-DD HH24:MI') " ).append("\n"); 
		...
		query.append("            					GROUP BY B.EDI_REF_ID,B.BKG_POD_CD	)" ).append("\n"); 
		query.append(" 					WHERE LEVEL = CNT" ).append("\n"); 
		query.append("					CONNECT BY RNUM = PRIOR RNUM + 1" ).append("\n"); 
		...
		query.append("#if (${date_check} != '')" ).append("\n"); 
		query.append("AND MF_SND_DT BETWEEN TO_DATE(@[start_snd_dt],'YYYY-MM-DD HH24:MI') " ).append("\n"); 
		query.append("AND TO_DATE(@[end_snd_dt],'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("AND INTG_CD_ID(+) = 'CD02300'" ).append("\n"); 

...
*@FileName : UnmatchBLDBDAORsltSearchAuditByCommodityAndRouteListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class UnmatchBLDBDAORsltSearchAuditByCommodityAndRouteListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * searchAuditByCommodityAndRouteListVO
	  * </pre>
	  */
	public UnmatchBLDBDAORsltSearchAuditByCommodityAndRouteListRSQL(){
		setQuery();
		...
		query.append("FileName : UnmatchBLDBDAORsltSearchAuditByCommodityAndRouteListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT  DENSE_RANK() OVER ( ORDER BY BKG_RHQ_CD, BKG_OFC_CD, RT_APLY_DT, BKG_NO ) BKG_SEQ ," ).append("\n"); 
		...
		query.append("        SUBSTR(REPLACE(S_CUST_NM,CHR(13)||CHR(10),' '), 0, 40) AS S_CUST_NM             ," ).append("\n"); 
		query.append("        SUBSTR(REPLACE(C_CUST_NM,CHR(13)||CHR(10),' '), 0, 40) AS C_CUST_NM             ," ).append("\n"); 
		query.append("        SUBSTR(REPLACE(N_CUST_NM,CHR(13)||CHR(10),' '), 0, 40) AS N_CUST_NM             ," ).append("\n"); 
		...
		query.append("				#if (${search_date} == 'ETD' || ${search_date} == 'APPL')" ).append("\n"); 
		query.append("					#if (${from_dt} != '')" ).append("\n"); 
		query.append("					" ).append("\n"); 
		query.append("					#end" ).append("\n"); 
		query.append("				#else			" ).append("\n"); 
		query.append("					/*+ LEADING( BK ) */" ).append("\n"); 
		query.append("				#end" ).append("\n"); 
		...
		query.append("                WHERE   A.INTG_CD_ID        = 'CD00698'" ).append("\n"); 
		query.append("                AND     A.INTG_CD_VAL_CTNT  = DECODE(BR.BKG_CTRT_TP_CD, 'S', SC.CTRT_CUST_VAL_SGM_CD, 'R', RF.CTRT_CUST_VAL_SGM_CD, 'T', TA.CTRT_CUST_VAL_SGM_CD)" ).append("\n"); 
		query.append("                ) CTRT_CUST_VAL_SGM_NM  ," ).append("\n"); 
		...
		query.append("                        RM.CTRT_CUST_VAL_SGM_CD" ).append("\n"); 
		...
		query.append("        RT_APLY_DT  ," ).append("\n"); 
		query.append("        BKG_NO      ," ).append("\n"); 
		query.append("        DECODE(CHG_CD, 'OFT', 1, 'O/F', 2, 3)  ," ).append("\n"); 
		query.append("        CHG_CD" ).append("\n"); 

...
public class GeneralBookingSearchDBDAOSearchTmnl301RcvIdForManualSendRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("SELECT NTC_KND_CD AS KIND, EDI_RCV_ID, 'R' BRAC_CD--CANCEL로 전송" ).append("\n"); 
		...
		query.append("FROM SENT_ID)" ).append("\n"); 
		query.append("UNION ALL" ).append("\n"); 
		query.append("SELECT DISTINCT RCV_ID.NTC_KND_CD AS KIND, RCV_ID.EDI_RCV_ID, DECODE(SENT_ID.TML_NTC_SND_STS_CD, 'R', 'N', 'M', 'U', @[brac]) BRAC_CD --마지막 전송이 R일 경우 'N', 아니면 U나 B로 전송" ).append("\n"); 
		...
		query.append("SELECT NTC_KND_CD AS KIND_CD, EDI_RCV_ID, @[brac] BRAC_CD" ).append("\n"); 
		query.append("FROM RCV_ID" ).append("\n"); 

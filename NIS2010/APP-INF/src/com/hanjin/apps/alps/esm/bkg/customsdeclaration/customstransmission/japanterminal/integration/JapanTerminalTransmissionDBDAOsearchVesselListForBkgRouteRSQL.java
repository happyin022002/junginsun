...
public class JapanTerminalTransmissionDBDAOsearchVesselListForBkgRouteRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * BKG Route로 잡혀 있는 Vessel List를 조회해 온다.
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("V.VSL_CD||V.SKD_VOY_NO||V.SKD_DIR_CD VVD_CD," ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("FROM BKG_VVD V, BKG_BOOKING B, VSK_VSL_PORT_SKD S, MDM_VSL_CNTR M" ).append("\n"); 
		query.append("WHERE 1=1" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("   				 AND (B.POR_NOD_CD = S.POR_YD_CD" ).append("\n"); 
		query.append("   				 OR B.POR_NOD_CD IS NULL)" ).append("\n"); 
		query.append("                " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append(") -- check aleady exist in SKD" ).append("\n"); 
		query.append("GROUP BY V.VSL_CD,V.SKD_VOY_NO,V.SKD_DIR_CD, B.POL_CD, B.POL_NOD_CD, B.POR_CD, B.POR_NOD_CD, S.VPS_ETA_DT, M.CALL_SGN_NO" ).append("\n"); 

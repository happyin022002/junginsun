 ...
 * 
 * @author OKYOUNG IM
 * @see ESM_MAS_002EventResponse,CostStructureBC 각 DAO 클래스 참조
 * @since J2EE 1.4
 */
public class CostStructureBCImpl extends BasicCommandSupport implements CostStructureBC {
	...
	/**
	 * 조회 이벤트 처리<br>
	 * ESM_MAS_0001(Set List Box)화면에 대한 조회 이벤트 처리<br>
	 * 
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESM_MAS_0001(Set List Box)화면에 대한 멀티 이벤트 처리<br>
	 * 
	...
	public void multiSpclRepoCntr(MasSpclRepoCntrRgstVO[] masSpclRepoCntrRgstVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasSpclRepoCntrRgstVO> insertVoList = new ArrayList<MasSpclRepoCntrRgstVO>();
			List<MasSpclRepoCntrRgstVO> updateVoList = new ArrayList<MasSpclRepoCntrRgstVO>();
			List<MasSpclRepoCntrRgstVO> deleteVoList = new ArrayList<MasSpclRepoCntrRgstVO>();
			
			for ( int i=0; i<masSpclRepoCntrRgstVO.length; i++ ) {
				if ( masSpclRepoCntrRgstVO[i].getIbflag().equals("I")){
					masSpclRepoCntrRgstVO[i].setRepoFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getRepoFlg()));
					masSpclRepoCntrRgstVO[i].setSpclCgoFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getSpclCgoFlg()));
					masSpclRepoCntrRgstVO[i].setDeltFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getDeltFlg()));
					masSpclRepoCntrRgstVO[i].setCreUsrId(account.getUsr_id());
					masSpclRepoCntrRgstVO[i].setUpdUsrId(account.getUsr_id());
					insertVoList.add(masSpclRepoCntrRgstVO[i]);
				} else if ( masSpclRepoCntrRgstVO[i].getIbflag().equals("U")){
					masSpclRepoCntrRgstVO[i].setRepoFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getRepoFlg()));
					masSpclRepoCntrRgstVO[i].setSpclCgoFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getSpclCgoFlg()));
					masSpclRepoCntrRgstVO[i].setDeltFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getDeltFlg()));
					masSpclRepoCntrRgstVO[i].setUpdUsrId(account.getUsr_id());
					updateVoList.add(masSpclRepoCntrRgstVO[i]);
				} else if ( masSpclRepoCntrRgstVO[i].getIbflag().equals("D")){
					masSpclRepoCntrRgstVO[i].setDeltFlg(Utils.change10ToYN(masSpclRepoCntrRgstVO[i].getDeltFlg()));
					deleteVoList.add(masSpclRepoCntrRgstVO[i]);
				}
			}
	...
	/**
	 * 조회 이벤트 처리<br>
	 * ESM_MAS_0002(Cost Element)화면에 대한 조회 이벤트 처리<br>
	 * 
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESM_MAS_0002(Cost Element)화면에 대한 멀티 이벤트 처리<br>
	 * 
	 * @param MasStndAcctVO[] vos
	 * @param String userId
	 * @return EventResponse
	 * @exception EventException
	 */
	public EventResponse multiCostCode(MasStndAcctVO[] vos, String userId) throws EventException{
	...
	/**
	 * 헤더가져오기<br>
	 * ESM_MAS_003(So Cost Code)화면에 대한 조회 이벤트 처리<br>
	 * 
...
//                    	log.debug("saveList===" + saveList.toString());
                    	...
                    	for(int k = 0 ; k < hdCdArr.length ; k++){
                    		...
                    		param2.put("mas_cost_src_cd", vos[i].getMasCostSrcCd());
...
//	/**
//	 * 멀티 이벤트 처리<br>
//	 * ESM_MAS_014 화면에 대한 멀티 이벤트 처리<br>
//	 * 
//	 * @param e ESM_MAS_016Event
//	 * @return EventResponse ESM_MAS_016EventResponse
//	 * @exception EventException
//	 */
//	/*
...
//		log.debug("\n ### createUnitCost");
//		// PDTO(Data Transfer Object including Parameters)
//		ESM_MAS_108Event event = (ESM_MAS_108Event) e;
//
//		try {
//			// 비동기 생성 실행
//			// -----------------------------------------------
...
//			// 해당년도 상태 만들기
//			// -----------------------------------------------
//			dbDao.addMAS_UT_COST_CRE_STS(event);
//			// -----------------------------------------------
//
...
//	 * @param e
//	 *            ESM_MAS_108Event
//	 * @return EventResponse ESM_MAS_0139EventResponse
//	 * @exception EventException
//	 */
//	/*
...
//			return new ESM_MAS_108EventResponse(rowSet,"SUCCESS");
//		} catch (DAOException de) {
//			log.error("err "+de.toString(),de);
//			throw new EventException(de.getMessage());
//		}
...
//			}
//			dbDao.modifyMAS_UT_COST_CRE_STS(msg);
//		} catch (DAOException de) {
//			log.error("err " + de.toString(), de);
//			try {
//				msg = msg + "|cost_cre_sts_cd:E"; // 에러발생 
...
//		log.debug("### modifyMAS_UT_COST_CRE_STS :" + msg);
//
//		try {
...
//	 * 
//	 * @param e ESM_MAS_138Event
//	 * @return EventResponse ESM_MAS_138EventResponse
//	 * @exception EventException
//	 */
...
//		   
//	try {
//		if(e.getEventName().equals("ESM_MAS_138Event")){
//			// PDTO(Data Transfer Object including Parameters)
//			ESM_MAS_138Event event = (ESM_MAS_138Event) e;	
//			DBRowSet rowSet=dbDao.searchList(event);
//			response = new ESM_MAS_138EventResponse(rowSet,"SUCCESS");
//			}
//			return response;
//			} catch (DAOException de) {
//				log.error("err "+de.toString(),de);
...
//	
	/**
	 * 조회 이벤트 처리<br>
	 * ESM_MAS_0138 화면에 대한 조회 이벤트 처리<br>
	 * 
	...
	public void multiCostStructure0139(MasTrnsFdrTermVO[] masTrnsFdrTermVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasTrnsFdrTermVO> insertupdateVoList = new ArrayList<MasTrnsFdrTermVO>();			
			List<MasTrnsFdrTermVO> deleteVoList = new ArrayList<MasTrnsFdrTermVO>();
			
			for ( int i=0; i<masTrnsFdrTermVO .length; i++ ) {
				if ( masTrnsFdrTermVO[i].getIbflag().equals("I")){
					 masTrnsFdrTermVO[i].setCreUsrId(account.getUsr_id());
					 masTrnsFdrTermVO[i].setUpdUsrId(account.getUsr_id());
					 insertupdateVoList.add(masTrnsFdrTermVO[i]);
				} else if ( masTrnsFdrTermVO[i].getIbflag().equals("U")){
					masTrnsFdrTermVO[i].setUpdUsrId(account.getUsr_id());									
					insertupdateVoList.add(masTrnsFdrTermVO[i]);
				} else if ( masTrnsFdrTermVO[i].getIbflag().equals("D")){
					deleteVoList.add(masTrnsFdrTermVO[i]);
				}
			}
	...
	public void multiCostStructure0140(MasTrnsTermCalcVO[] masTrnsTermCalcVO, SignOnUserAccount account) throws EventException{
		try {
			List<MasTrnsTermCalcVO> insertupdateVoList = new ArrayList<MasTrnsTermCalcVO>();			
			List<MasTrnsTermCalcVO> deleteVoList = new ArrayList<MasTrnsTermCalcVO>();
			for ( int i=0; i<masTrnsTermCalcVO .length; i++ ) {
				if ( masTrnsTermCalcVO[i].getIbflag().equals("I")||masTrnsTermCalcVO[i].getIbflag().equals("U")){
					masTrnsTermCalcVO[i].setCreUsrId(account.getUsr_id());
					masTrnsTermCalcVO[i].setUpdUsrId(account.getUsr_id());
					insertupdateVoList.add(masTrnsTermCalcVO[i]);
				} else if ( masTrnsTermCalcVO[i].getIbflag().equals("D")){
					deleteVoList.add(masTrnsTermCalcVO[i]);
				}
			}
	...
	public HashMap searchSoCode0160List() throws EventException {
		try {
        	...
        	if(rowSet != null){
				...
				while(rowSet.next()){
					sbCode.append(rowSet.getString("code"));
					sbText.append(rowSet.getString("name"));
				...
				returnVal.put("headerCode", sbCode.toString());
				returnVal.put("headerText", sbText.toString());
				return returnVal;
        	} else { 
			  }  
			return returnVal;
	...
	public EventResponse multiSoCode0160(GetVariableheader2VO vo, MasAgmtRstrMgmtConditionVO[] vos, SearchConditionVO condVo, SignOnUserAccount account) throws EventException{
		try{			
			...
			String mas_cost_src_cd = ""; //현재 mas_cost_src_cd
            
            if(vos.length > 0){
                for(int i = 0 ; i < vos.length ; i++){
                    for (int k = 0; k < headerArr.length; k++) {
                    	
						mas_cost_src_cd = headerArr[k];
						checkArr = vo.getHashAttribute(mas_cost_src_cd);
						...
						param.put("mas_cost_src_cd" , headerArr[k]);
	...
	public List<RevExpChargeVO> searchRevExpChargeList(RevExpChargeVO revExpChargeVO) throws EventException {
		try {			
			return dbDao.searchRevExpChargeList(revExpChargeVO);
		} catch (DAOException ex) {
			throw new EventException(ex.getMessage(),ex);
		} catch (Exception ex) {
			throw new EventException(ex.getMessage(),ex);
	...
	public void multiRevExpCharge(RevExpChargeVO[] revExpChargeVOs, SignOnUserAccount account) throws EventException {
		try {
			List<RevExpChargeVO> updateVoList = new ArrayList<RevExpChargeVO>();
			List<RevExpChargeVO> deleteVoList = new ArrayList<RevExpChargeVO>();
			for ( int i=0; i<revExpChargeVOs.length; i++ ) {
				if ( revExpChargeVOs[i].getIbflag().equals("U")){
					revExpChargeVOs[i].setUpdUsrId(account.getUsr_id());										
					updateVoList.add(revExpChargeVOs[i]);
				}else if ( revExpChargeVOs[i].getIbflag().equals("D")){
					deleteVoList.add(revExpChargeVOs[i]);
				}			
			}
			
			if ( updateVoList.size() > 0 ) {
				dbDao.modifyRevExpCharge(updateVoList);
			}
			if ( deleteVoList.size() > 0 ) {
				dbDao.removeRevExpCharge(deleteVoList);
			}
	...
	public void deleteRevExpCharge(RevExpChargeVO[] revExpChargeVOs, SignOnUserAccount account) throws EventException {
		try {			
			List<RevExpChargeVO> deleteVoList = new ArrayList<RevExpChargeVO>();
			for ( int i=0; i<revExpChargeVOs.length; i++ ) {
				if ( revExpChargeVOs[i].getIbflag().equals("D")){
					deleteVoList.add(revExpChargeVOs[i]);
				}			
			}

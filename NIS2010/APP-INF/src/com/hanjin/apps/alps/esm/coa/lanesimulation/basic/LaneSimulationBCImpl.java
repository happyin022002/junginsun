...
=========================================================*/
package com.hanjin.apps.alps.esm.coa.lanesimulation.basic;
...
import com.hanjin.apps.alps.esm.coa.lanesimulation.integration.LaneSimulationDBDAO;
...
import com.hanjin.apps.alps.esm.coa.lanesimulation.vo.SearchFileMgmtListVO;
import com.hanjin.apps.alps.esm.coa.lanesimulation.vo.SearchLaneInfoListVO;
...
import com.hanjin.apps.alps.esm.coa.lanesimulation.vo.SearchSimReportMasterListVO;
import com.hanjin.apps.alps.esm.coa.lanesimulation.vo.SearchSimRtnRowSetVO;
...
import com.hanjin.syscommon.common.table.CoaBnkCsmVO;
...
import com.hanjin.syscommon.common.table.CoaSimInfoVO;
...
public class LaneSimulationBCImpl extends BasicCommandSupport implements LaneSimulationBC {
    ...
    /**
     * 1. 기능 : sheet1조회 이벤트 처리(ESM_COA_0051)<p>
     * 2. 처리개요 : <p>
    ...
    /**
     * 1. 기능 : master sheet조회 이벤트 처리(ESM_COA_0051)<p>
     * 2. 처리개요 : <p>
     ...
     * 2. 처리개요 :  <p>
     *    - ESM_COA_0051 화면에 대한 멀티 이벤트 처리<br>
     * 3. 주의사항 : <p>
     * ===================================<br>
     * 4. 작성자/작성일 : parkeunju/2006.08.28<br>
     * ===================================<br>
     * 5. 수정사항<p> EsmCoa0051EventResponse("","SUCCESS");
     * 5.1 요구사항 ID :<p>
        	...
        	for ( int i=0; i<searchSimLaneListVO .length; i++ ) {
				if ( searchSimLaneListVO[i].getIbflag().equals("I")){
		        	...
		        	if(rowSet.getRowCount()==0 && i==0){ //master table 입력은 한번만 진행 
		        		...
		        		dbDao.addSimLane(searchSimLaneListVO[i]); //COA_SIM_INFO 머지
		        	}
		        	dbDao.addmultiSimLane(searchSimLaneListVO[i]); // detail table 다수 입력가능 COA_SIM_SVC_LANE INSERT
    ...
    /**
     * 1. 기능 : detail sheet 조회 이벤트 처리(ESM_COA_0051)<p>
     * 
    ...
    public void multiSimVesselInfo(SearchSimVesselListVO[] searchSimVesselListVO, SignOnUserAccount account) throws EventException{
    	...
    	try {
    		//COA_SIM_VSL_INFO의 여러 데이타 모델을 지정된 ibflag 값에 따라 DB에 반영한다.(추가, 수정)<br>
    		...
    		for(int i=0;i<searchSimVesselListVO.length-1;i++) {
    			...
    			if (vsl_flg.equals("Y") && !vsl_cd.equals("XXXX")) {
    				...
    				dbDao.addSimVslInfo(searchSimVesselListVO[i]);  //COA_SIM_VSL_RGST INSERT
    			} else if(ibflag.equals("U")) {
    				retVo.setUserId(account.getUsr_id());
    				dbDao.modifySimVslInfo(searchSimVesselListVO[i]); //COA_SIM_VSL_RGST UPDATE
    			}
    		}
    ...
    public void multiTsVolume(SearchConditionVO searchConditionVO,SearchTsVolumeVO[] searchTsVolumeVO,SignOnUserAccount account) throws EventException {
		try {
			List<CoaSimIntrTrnsVolVO> insertVoList = new ArrayList<CoaSimIntrTrnsVolVO>();
			List<CoaSimIntrTrnsVolVO> updateVoList = new ArrayList<CoaSimIntrTrnsVolVO>();
			List<CoaSimIntrTrnsVolVO> deleteVoList = new ArrayList<CoaSimIntrTrnsVolVO>();
			for ( int i=0; i<searchTsVolumeVO.length; i++ ) {
				if ( searchTsVolumeVO[i].getIbflag().equals("I")){
					CoaSimIntrTrnsVolVO vo = new CoaSimIntrTrnsVolVO();
					...
					CoaSimIntrTrnsVolVO vo2 = new CoaSimIntrTrnsVolVO();
	 ...
	 * @exception EventException
	 */
    public List<CoaSimNonOpExpnVO> searchNonOpExpnList(SearchConditionVO searchConditionVO) throws EventException {
		try {
	 ...
	 * 
	 * @param CoaSimNonOpExpnVO[] coaSimNonOpExpnVO
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
    public void multiNonOpExpn(CoaSimNonOpExpnVO[] coaSimNonOpExpnVO,SignOnUserAccount account) throws EventException {
		try {
			List<CoaSimNonOpExpnVO> insertVoList = new ArrayList<CoaSimNonOpExpnVO>();
			List<CoaSimNonOpExpnVO> updateVoList = new ArrayList<CoaSimNonOpExpnVO>();
			List<CoaSimNonOpExpnVO> deleteVoList = new ArrayList<CoaSimNonOpExpnVO>();
			for ( int i=0; i<coaSimNonOpExpnVO.length; i++ ) {
				if ( coaSimNonOpExpnVO[i].getIbflag().equals("I")){
					CoaSimNonOpExpnVO vo = new CoaSimNonOpExpnVO();
					vo.setCostYrmon(coaSimNonOpExpnVO[0].getCostYrmon());
					vo.setVslCd(coaSimNonOpExpnVO[i].getVslCd());
					vo.setUcAmt(coaSimNonOpExpnVO[i].getUcAmt());
					vo.setCreUsrId(account.getUsr_id());
					vo.setUpdUsrId(account.getUsr_id());
					insertVoList.add(vo);
				} else if ( coaSimNonOpExpnVO[i].getIbflag().equals("U")){
					CoaSimNonOpExpnVO vo = new CoaSimNonOpExpnVO();
					vo.setCostYrmon(coaSimNonOpExpnVO[0].getCostYrmon());
					vo.setVslCd(coaSimNonOpExpnVO[i].getVslCd());
					vo.setUcAmt(coaSimNonOpExpnVO[i].getUcAmt());
					...
					vo.setUpdUsrId(account.getUsr_id());
					updateVoList.add(vo);
				} else if ( coaSimNonOpExpnVO[i].getIbflag().equals("D")){
					CoaSimNonOpExpnVO vo = new CoaSimNonOpExpnVO();					
					vo.setCostYrmon(coaSimNonOpExpnVO[0].getCostYrmon());
					vo.setVslCd(coaSimNonOpExpnVO[i].getVslCd());
					deleteVoList.add(vo);
     ...
     * @return EventResponse ESM_COA_052EventResponse
     * @throws EventException
     */
    public SearchSimPortListVO searchSimPortList(SearchSimLaneListConditionVO vo) throws EventException {
    	try {
			...
			if(rSet != null) {
				while(rSet.next()) {
					vslCdClass = vslCdClass + rSet.getString("vsl_clss_capa")+"|";
					vslCdCnt   = vslCdCnt + rSet.getString("vsl_cnt")+"|";
				}
		...
		try {
    		if(vos != null) {
				...
				if(insertVoList.size() >  0) {
					...
					//COA_SIM_INFO 테이블에 FREQUENCY,DURATION 정보를 넣는다.
...
//    		//CoaSimTmlInfoVO masterVO = vo.getCoaSimTmlInfoVO();
...
//			String minMaxSpd = "";//coaSimTmlInfoVO.getMinMaxSpd();
...
//
//			//returnVO.setCoaSimTmlInfoVO(masterVO);
   ...
   /**
    ...
    public void createSimVolProj(MergyVolProjConditionVO vo
        ...
        try {
        	...
        	// 2.COA_SIM_VOL_PRJ 데이터 삭제
        	dbDao.deleteSimVolProj2(vo);
        	// 3.COA_SIM_CTRB_MGN_COST 데이터 삭제
        	dbDao.deleteSimVolProj(vo);
        	// 4.COA_SIM_VOL_PRJ 데이터 생성
        	dbDao.createSimVolProj1(vo, rtnVO, account);
        	// 5.COA_SIM_CTRB_MGN_COST 데이터 생성
        	dbDao.createSimVolProj2(vo,account);
        	// 6.COA_SIM_SVC_LANE 정보변경
...
//        	returnVo.setHeader(header.toString());
    ...
    public void modifyTMLOPDys(SearchTMLOPDysListVO vo,SearchTMLOPDysListVO[] vos,SignOnUserAccount account) throws EventException{
    	try {
			...
			//return new ESM_COA_052EventResponse("SUCCESS");
		} catch (DAOException ex) {
			log.error("err "+ex.toString(),ex);
			throw new EventException(ex.getMessage(),ex);
		} catch (Exception ex) {
    ...
    public void createTMLOPDys(SearchTMLOPDysListVO vo,SignOnUserAccount account) throws EventException{
		try {
			...
			if(turnPortIndFlg) {
				...
				throw new EventException(new ErrorHandler("COA00033").getMessage());
			}
		} catch (DAOException ex) {
			log.error("err "+ex.toString(),ex);
			throw new EventException(ex.getMessage(),ex);
     ...
     * @exception EventException
     */
    public void modifySimCMCost(CoaSimCtrbMgnCostVO[] vo, SearchSimConditionVO searchVO,SignOnUserAccount account) throws EventException {
		List<CoaSimCtrbMgnCostVO> insertVoList = new ArrayList<CoaSimCtrbMgnCostVO>();
        try {
        	if(vo != null){
     ...
     * 
     * @param CoaSimPortChgVO[] vo
     * @param SearchSimConditionVO searchVO
     * @param SignOnUserAccount account
     * @exception EventException
     */
    public void multiSimPortCharge(CoaSimPortChgVO[] vo, SearchSimConditionVO searchVO, SignOnUserAccount account) throws EventException {
    	List<CoaSimPortChgVO> insertVoList = new ArrayList<CoaSimPortChgVO>();
    	List<CoaSimPortChgVO> updateVoList = new ArrayList<CoaSimPortChgVO>();
        try {
        	if(vo != null){
        		for(int i=0; i< vo.length; i++){
     ...
     * 
     * @param CoaSimBnkCostVO[] vo
     * @param SearchSimConditionVO searchVO
     * @param SignOnUserAccount account
     * @exception EventException
     */
    public void createSimBunkerList(CoaSimBnkCostVO[] vo,SearchSimConditionVO searchVO, SignOnUserAccount account) throws EventException {
    	List<CoaSimBnkCostVO> updateVoList = new ArrayList<CoaSimBnkCostVO>();
        try {
        	if(vo != null){
        		for(int i=0; i< vo.length; i++){
    				vo[i].setSimDt(searchVO.getFSimDt());
     ...
     * @exception EventException
     */
    public void multiSimBunkerCost(CoaSimBnkCostVO[] vo, SearchSimConditionVO searchVO, SignOnUserAccount account) throws EventException {
    	List<CoaSimBnkCostVO> insertVoList = new ArrayList<CoaSimBnkCostVO>();
    	List<CoaSimBnkCostVO> deleteVoList = new ArrayList<CoaSimBnkCostVO>();
        try {
        	if(vo != null){
        		for(int i=0; i< vo.length; i++){
    				vo[i].setSimDt(searchVO.getFSimDt());
     ...
     * @exception EventException
     */
    public SearchSimDailyHireInfoVO searchSimDailyHireList(CoaSimCtrbMgnCostVO vo) throws EventException {
        try {
     ...
     *
     * @param CoaSimDlyHirVO[] vos
     * @param CreateSimDailyHireVO vo
     * @param SignOnUserAccount account
     * @exception EventException
     */
    public void multiSimDailyHire(CoaSimDlyHirVO[] vos,CreateSimDailyHireVO vo, SignOnUserAccount account) throws EventException {
		List<CoaSimDlyHirVO> insertVoList = new ArrayList<CoaSimDlyHirVO>();
        try {
        	if(vo != null){
        		for(int i=0; i< vos.length; i++){
     ...
     * @exception EventException
     */
    public int createSimNWCost(CoaSimCtrbMgnCostVO vo,SignOnUserAccount account) throws EventException{
        try {
     ...
     * @exception EventException
     */
    public SearchSimDailyHireInfoVO searchSimNWCostList(CoaSimCtrbMgnCostVO vo) throws EventException {
        try {
    ...
    public SearchSimDailyHireInfoVO searchSimAfterOcenaTSList(CoaSimCtrbMgnCostVO vo) throws EventException {
        try {
     ...
     * 
     * @param CoaTmChtrOutHirVO[] vos
     * @param SearchTocHireListVO[] vo
     * @param SignOnUserAccount account
     * @exception EventException
     */
    public void multiTOCHire(CoaTmChtrOutHirVO[] vos,SearchTocHireListVO[] vo,SignOnUserAccount account) throws EventException{
        try {
        	List<CoaTmChtrOutHirVO> insertList = new ArrayList<CoaTmChtrOutHirVO>();
        	List<CoaTmChtrOutHirVO> updateList = new ArrayList<CoaTmChtrOutHirVO>();
        	List<CoaTmChtrOutHirVO> deleteList = new ArrayList<CoaTmChtrOutHirVO>();
     ...
     * 
     * @param CoaBnkCsmVO coaBnkCsmVO
     * @return List<CoaBnkCsmVO>
     * @exception EventException
     */
    public List<CoaBnkCsmVO> searchSimConsumList(CoaBnkCsmVO coaBnkCsmVO) throws EventException {
    	try {
    		return dbDao.searchSimConsumList(coaBnkCsmVO);
    	} catch (DAOException de) {
    		log.error("err "+de.toString(),de);
    		throw new EventException(de.getMessage());
    	}
     ...
     * @exception EventException
     */
    public void multiSimConsum(CoaBnkCsmVO[] vo,SignOnUserAccount account) throws EventException{
		List<CoaBnkCsmVO> insertVoList = new ArrayList<CoaBnkCsmVO>();
		List<CoaBnkCsmVO> updateVoList = new ArrayList<CoaBnkCsmVO>();
		List<CoaBnkCsmVO> deleteVoList = new ArrayList<CoaBnkCsmVO>();
    	try {
        	if(vo != null){
        		for(int i=0; i< vo.length; i++){    		
        			if(vo[i].getIbflag().equals("I")) {
    ...
    /*================================================================================================================*/
    /**
     * Header 정보를 조회 이벤트 처리(ESM_COA_054)<p>
	 *
     * @param SearchSimLaneListConditionVO vo
     * @return SearchSimLaneListConditionVO
     * @exception EventException
...
/*    public SearchSimLaneListConditionVO searchSimReportNoList(SearchSimLaneListConditionVO vo) throws EventException {
        ...
        try {
			...
			rowSet = rtnVo.getDbRowSet();
   			while(rowSet.next()) {
    					...
    					+ " (SELECT sim_rmk FROM coa_sim_rpt_mst a1 WHERE a1.sim_dt = b1.sim_dt AND a1.sim_no = b1.sim_no AND a1.sim_rpt_no = b1.sim_rpt_no AND a1.sect_no = '" 
    ...
    */
    ...
    public SearchReportConditionVO searchSimSummaryReportList(SearchReportConditionVO vo) throws EventException {
        ...
        try {
        	...
        	if(rowSet2 != null){
				...
				while(rowSet2.next()){
					header = header + rowSet2.getString("code");
					headerNM = headerNM + rowSet2.getString("name");
					if(cnt != i){
						header = header + "|";
						headerNM = headerNM + "|";
					}
     ...
     * @throws EventException
     */
    public List<CoaSimInfoVO> createCoaSimRqst(PfSkdGRPVO pfSkdGRPVO, String ind, SignOnUserAccount account) throws EventException{
    	SearchSimLaneListConditionVO searchVo = new SearchSimLaneListConditionVO();
    	Map<String, Object> param = new HashMap<String, Object>();
    	List<CoaSimTmlInfoVO> list = new ArrayList<CoaSimTmlInfoVO>();
    	CommonBC command = new CommonBCImpl();
    	CoaSimTmlInfoVO tmlVo = null;
    	DBRowSet rowSet = null;
    	String v_sim_no = "";
    	String v_sim_dt = "";
    	int cnt = 0;
    	...
    	try {
    		CoaSimTmlInfoVO[] vo = new CoaSimTmlInfoVO[vskPfSkdDtlVOs.size()];
    		int len = vskPfSkdDtlVOs.size();

    		for(int i=0; i<len;i++){
    			if(i == len-1){
    				vo[i] = new CoaSimTmlInfoVO();
...
//	    				vo[i].setSimNo("001");
    		...
    		if (rowSet != null){
    			return (List)RowSetUtil.rowSetToVOs(rowSet, CoaSimInfoVO .class);
    		}else {
    			return null;
    		}

...
public class ESM_BKG_0325HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0077HTMLAction 객체를 생성
	 */
	public ESM_BKG_0325HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0325Event event = new EsmBkg0325Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			String blckKwTpCd = JSPUtil.getParameter(request, "blck_kw_tp_cd");
			event.setBlckKwTpCd(blckKwTpCd);
		}else if(command.isCommand(FormCommand.COMMAND01)){
			event.setBlckKwListVOs((BlckKwListVO[])getVOs(request, BlckKwListVO .class,""));
		}

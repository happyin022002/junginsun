...
*@FileName : ESM_MAS_0024HTMLAction.java
 ...
 * @since J2EE 1.6
 */
public class ESM_MAS_0024HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * UserHTMLAction 객체를 생성
	 */
	public ESM_MAS_0024HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        */
    	...
    	EsmMas0024Event event = new EsmMas0024Event();
		if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01)) {
			event.setVesselLayupVO((VesselLayupVO)getVO(request, VesselLayupVO.class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setVesselLayupVO((VesselLayupVO)getVO(request, VesselLayupVO.class));
			event.setVesselLayupVOs((VesselLayupVO[])getVOs(request, VesselLayupVO.class, ""));
		}

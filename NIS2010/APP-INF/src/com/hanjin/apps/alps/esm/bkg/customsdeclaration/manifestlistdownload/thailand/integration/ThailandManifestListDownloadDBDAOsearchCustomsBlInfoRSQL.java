...
public class ThailandManifestListDownloadDBDAOsearchCustomsBlInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    NVL( (SELECT REPLACE(S.CUST_NM,CHR(13)||CHR(10),'  ') AS CUST_NM     FROM BKG_CUSTOMER S WHERE S.BKG_NO=B.BKG_NO AND S.BKG_CUST_TP_CD='S' AND ROWNUM=1 ), ' ' ) AS SH_CUST_NM," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(S.CUST_ADDR,CHR(13)||CHR(10),'  ') AS CUST_ADDR FROM BKG_CUSTOMER S WHERE S.BKG_NO=B.BKG_NO AND S.BKG_CUST_TP_CD='S' AND ROWNUM=1 ), ' ' ) AS SH_CUST_ADDR," ).append("\n"); 
		query.append("	NVL( (SELECT REPLACE(F.CUST_NM,CHR(13)||CHR(10),'  ') AS CUST_NM     FROM BKG_CUSTOMER F WHERE F.BKG_NO=B.BKG_NO AND F.BKG_CUST_TP_CD='F' AND ROWNUM=1 ), ' ' ) AS FW_CUST_NM," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(C.CUST_NM,CHR(13)||CHR(10),'  ') AS CUST_NM     FROM BKG_CUSTOMER C WHERE C.BKG_NO=B.BKG_NO AND C.BKG_CUST_TP_CD='C' AND ROWNUM=1 ), ' ' ) AS CN_CUST_NM," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(S.CUST_ADDR,CHR(13)||CHR(10),'  ') AS CUST_ADDR FROM BKG_CUSTOMER S WHERE S.BKG_NO=B.BKG_NO AND S.BKG_CUST_TP_CD='C' AND ROWNUM=1 ), ' ' ) AS CN_CUST_ADDR," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(N.CUST_NM,CHR(13)||CHR(10),'  ') AS CUST_NM     FROM BKG_CUSTOMER N WHERE N.BKG_NO=B.BKG_NO AND N.BKG_CUST_TP_CD='N' AND ROWNUM=1 ), ' ' ) AS NO_CUST_NM," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(S.CUST_ADDR,CHR(13)||CHR(10),'  ') AS CUST_ADDR FROM BKG_CUSTOMER S WHERE S.BKG_NO=B.BKG_NO AND S.BKG_CUST_TP_CD='N' AND ROWNUM=1 ), ' ' ) AS NO_CUST_ADDR," ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(TO_CHAR(U.MK_DESC),CHR(13)||CHR(10),'  ') AS MK_DESC     FROM BKG_BL_MK_DESC U WHERE U.BKG_NO=B.BKG_NO AND ROWNUM=1 ), ' ' ) AS MK_DESC, " ).append("\n"); 
		...
		query.append("    NVL( (SELECT REPLACE(TO_CHAR(U.CMDT_DESC),CHR(13)||CHR(10),'  ') AS CMDT_DESC     FROM BKG_BL_MK_DESC U WHERE U.BKG_NO=B.BKG_NO AND ROWNUM=1 ), ' ' ) AS CMDT_DESC, " ).append("\n"); 

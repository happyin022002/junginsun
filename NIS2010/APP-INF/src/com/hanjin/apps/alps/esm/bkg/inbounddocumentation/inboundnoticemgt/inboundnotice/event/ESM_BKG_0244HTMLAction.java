...
*@FileName : ESM_BKG_0244HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0244HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
 
	/**
	 * ESM_BKG_0244HTMLAction 객체를 생성 
	 */
	public ESM_BKG_0244HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBkg0244Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0244Event event = new EsmBkg0244Event();
		FormCommand command = FormCommand.fromRequest(request);

		if(command.isCommand(FormCommand.SEARCH)) {

			BkgArrNtcAntfyVO vo = (BkgArrNtcAntfyVO)getVO(request, BkgArrNtcAntfyVO.class);
			event.setBkgArrNtcAntfyVO(vo);

		}

		if(command.isCommand(FormCommand.MULTI)) {
			event.setBkgArrNtcAntfyVOs((BkgArrNtcAntfyVO[])getVOs(request, BkgArrNtcAntfyVO.class, "sheet1_"));
		}

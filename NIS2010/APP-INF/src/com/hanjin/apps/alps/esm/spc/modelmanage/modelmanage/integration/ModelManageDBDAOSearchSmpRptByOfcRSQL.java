...
*@FileName : ModelManageDBDAOSearchSmpRptByOfcRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ModelManageDBDAOSearchSmpRptByOfcRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 조건에 맞는 SMP Loading 정보 Office 관점에서 조회합니다.
	  ...
	  * </pre>
	  */
	public ModelManageDBDAOSearchSmpRptByOfcRSQL(){
		setQuery();
		...
		query.append("FileName : ModelManageDBDAOSearchSmpRptByOfcRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("               WHERE --SUBSTR(M.SLS_YRMON, 1, 4) = SUBSTR(TO_CHAR(SYSDATE,'YYYYMM'),1,4)" ).append("\n"); 
		...
		query.append("                   ROUND(SUM(LOAD)) AS LOAD_QTY" ).append("\n"); 
		...
		query.append("                          RLANE_CD,MAX_CNT," ).append("\n"); 
		...
		query.append("                                  COUNT(1) OVER(PARTITION BY C.FNL_BSA_CAPA,C.DIR_CD) AS MAX_CNT" ).append("\n"); 
		...
		query.append("                       DTL.SUB_TRD_ADJ_QTY            " ).append("\n"); 
		query.append("                  FROM (" ).append("\n"); 
		...
		query.append("                               SUM(MCP.RLANE_ADJ_QTY) AS SUB_TRD_ADJ_QTY" ).append("\n"); 
		query.append("                          FROM SPC_MDL_CUST_REV_LANE   MCP," ).append("\n"); 
		...
		query.append("                         GROUP BY MCP.TRD_CD," ).append("\n"); 
		query.append("                               MCP.SUB_TRD_CD," ).append("\n"); 
		...
		query.append("                    THEN SUB_TRD_ADJ_QTY" ).append("\n"); 
		query.append("                    ELSE (LOAD_QTA - SUM(SUB_TRD_ADJ_QTY) OVER(PARTITION BY TRD_CD,SUB_TRD_CD,OFC_CD))" ).append("\n"); 
		query.append("               END AS C_LOAD_QTA," ).append("\n"); 
		query.append("               --DECODE(CUST_CTRL_CD,'A',SUB_TRD_ADJ_QTY,'B',SUB_TRD_ADJ_QTY,'C',(LOAD_QTA - SUM(SUB_TRD_ADJ_QTY) OVER(PARTITION BY TRD_CD,SUB_TRD_CD,OFC_CD))) C_LOAD_QTA," ).append("\n"); 
		query.append("               LOAD_QTA," ).append("\n"); 
		query.append("               RANK() OVER(PARTITION BY TRD_CD,SUB_TRD_CD,OFC_CD,CUST_CTRL_CD ORDER BY TRD_CD,SUB_TRD_CD,OFC_CD,CUST_LGL_ENG_NM, CUST_CTRL_CD ) AS CNT" ).append("\n"); 
		query.append("          FROM (  " ).append("\n"); 
		query.append("                SELECT TRD_CD," ).append("\n"); 
		query.append("                       SUB_TRD_CD," ).append("\n"); 
		query.append("                       OFC_CD," ).append("\n"); 
		...
		query.append("                       CUST_LGL_ENG_NM," ).append("\n"); 
		query.append("                       CUST_CTRL_CD," ).append("\n"); 
		query.append("                       MAX(NUM) AS NUM," ).append("\n"); 
		query.append("                       SUM(SUB_TRD_ADJ_QTY) AS SUB_TRD_ADJ_QTY," ).append("\n"); 
		query.append("                       SUM(LOAD_QTY) AS LOAD_QTA                       " ).append("\n"); 
		query.append("                  FROM (" ).append("\n"); 
		...
		query.append("                               DENSE_RANK() OVER(PARTITION BY B.TRD_CD ORDER BY B.TRD_CD, B.SUB_TRD_CD) AS NUM," ).append("\n"); 
		query.append("                               NVL(B.SUB_TRD_ADJ_QTY,0) AS SUB_TRD_ADJ_QTY," ).append("\n"); 
		query.append("                               NVL(A.LOAD_QTY,0) AS LOAD_QTY" ).append("\n"); 
		query.append("                          FROM (" ).append("\n"); 
		query.append("                                SELECT TRD_CD," ).append("\n"); 
		query.append("                                       SUB_TRD_CD," ).append("\n"); 
		...
		query.append("                                               0 AS SUB_TRD_ADJ_QTY  " ).append("\n"); 
		...
		query.append("       ,SUB_TRD_CD_${key}" ).append("\n"); 
		query.append("       ,SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("       ,DECODE(SUB_TRD_ADJ_RATIO_${key},0,SUB_TRD_ADJ_RATIO_${key},DECODE(SUM_SUB_TRD_ADJ_RATIO_${key},100,SUB_TRD_ADJ_RATIO_${key},DECODE(SUB_TRD_ADJ_RATIO_NUM${key},1,SUB_TRD_ADJ_RATIO_${key} + (100 - SUM_SUB_TRD_ADJ_RATIO_${key}),SUB_TRD_ADJ_RATIO_${key}))) AS SUB_TRD_ADJ_RATIO_${key}" ).append("\n"); 
		query.append("       ,C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("       ,DECODE(C_LOAD_QTA_RATIO_${key},0,C_LOAD_QTA_RATIO_${key},DECODE(SUM_C_LOAD_QTA_RATIO_${key},100,C_LOAD_QTA_RATIO_${key},DECODE(C_LOAD_QTA_RATIO_NUM${key},1,C_LOAD_QTA_RATIO_${key} + (100 - SUM_C_LOAD_QTA_RATIO_${key}),C_LOAD_QTA_RATIO_${key}))) AS C_LOAD_QTA_RATIO_${key}" ).append("\n"); 
		...
		query.append("               ,SUB_TRD_CD_${key}" ).append("\n"); 
		query.append("               ,SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("               ,SUB_TRD_ADJ_RATIO_${key}" ).append("\n"); 
		query.append("               ,C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("               ,C_LOAD_QTA_RATIO_${key}" ).append("\n"); 
		query.append("               -----       " ).append("\n"); 
		query.append("               ,SUM(SUB_TRD_ADJ_RATIO_${key}) OVER(PARTITION BY TRD_CD,SLS_RHQ_CD,OFC_CD,LVL,SUB_TRD_CD_${key}) AS SUM_SUB_TRD_ADJ_RATIO_${key}" ).append("\n"); 
		query.append("               ,SUM(C_LOAD_QTA_RATIO_${key}) OVER(PARTITION BY TRD_CD,SLS_RHQ_CD,OFC_CD,LVL,SUB_TRD_CD_${key})  AS SUM_C_LOAD_QTA_RATIO_${key}" ).append("\n"); 
		query.append("               ----" ).append("\n"); 
		query.append("               ,ROW_NUMBER() OVER(PARTITION BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL ORDER BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL,SUB_TRD_ADJ_RATIO_${key} DESC) AS SUB_TRD_ADJ_RATIO_NUM${key}" ).append("\n"); 
		query.append("               ,ROW_NUMBER() OVER(PARTITION BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL ORDER BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL,C_LOAD_QTA_RATIO_${key} DESC)  AS C_LOAD_QTA_RATIO_NUM${key} " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("          FROM (" ).append("\n"); 
		query.append("        " ).append("\n"); 
		query.append("                SELECT TRD_CD," ).append("\n"); 
		...
		query.append("                       LVL" ).append("\n"); 
		query.append("#foreach( $key in ${dur} )" ).append("\n"); 
		query.append("                       ,MAX(SUB_TRD_CD_${key})        AS SUB_TRD_CD_${key}" ).append("\n"); 
		query.append("                       ,NVL(MAX(SUB_TRD_ADJ_QTY_${key}),0)   AS SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("                       ,NVL(ROUND(SUM(DECODE(LVL,7,0,8,0,9,0,SUB_TRD_ADJ_RATIO_${key}))),0) AS SUB_TRD_ADJ_RATIO_${key}" ).append("\n"); 
		query.append("                       ,NVL(MAX(C_LOAD_QTA_${key}),0)        AS C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("                       ,NVL(ROUND(SUM(DECODE(LVL,7,0,8,0,9,0,C_LOAD_QTA_RATIO_${key}))),0)  AS C_LOAD_QTA_RATIO_${key}" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                  FROM (   " ).append("\n"); 
		query.append("                        SELECT TRD_CD," ).append("\n"); 
		query.append("                               SLS_RHQ_CD," ).append("\n"); 
		...
		query.append("                               ,SUB_TRD_CD_${key}" ).append("\n"); 
		query.append("                               ,SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("                               ,RATIO_TO_REPORT(SUB_TRD_ADJ_QTY_${key}) OVER(PARTITION BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL) * 100  AS SUB_TRD_ADJ_RATIO_${key}" ).append("\n"); 
		query.append("                               ,C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("                               ,RATIO_TO_REPORT(C_LOAD_QTA_${key}) OVER(PARTITION BY TRD_CD,SUB_TRD_CD_${key},SLS_RHQ_CD,OFC_CD,LVL) * 100 AS C_LOAD_QTA_RATIO_${key}" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                          FROM (" ).append("\n"); 
		query.append("                                  SELECT DISTINCT" ).append("\n"); 
		query.append("                                         Z3.TRD_CD," ).append("\n"); 
		query.append("                                         DECODE(Z3.SLS_RHQ_CD,'',DECODE(Z3.OFC_CD,'',DECODE(Z3.CUST_CTRL_CD,'','00000','00001'),'00002'),Z3.SLS_RHQ_CD) AS SLS_RHQ_CD," ).append("\n"); 
		...
		query.append("                                         Z3.CTRT_OFC_CD            AS CTRT_OFC_CD," ).append("\n"); 
		query.append("                                         Z3.SC_NO                  AS SC_NO," ).append("\n"); 
		query.append("                                         Z3.RFA_NO                 AS RFA_NO," ).append("\n"); 
		query.append("                                         " ).append("\n"); 
		...
		query.append("                                              WHEN Z3.SLS_RHQ_CD IS NOT NULL AND Z3.OFC_CD IS NOT NULL AND Z3.CUST_CTRL_CD IS NOT NULL AND ACCT_CLSS IS NULL AND CUST_LGL_ENG_NM IS NULL THEN" ).append("\n"); 
		query.append("                                                     6" ).append("\n"); 
		query.append("                                              WHEN Z3.SLS_RHQ_CD IS NOT NULL AND Z3.OFC_CD IS NOT NULL AND Z3.CUST_CTRL_CD IS NOT NULL AND ACCT_CLSS IS NOT NULL AND CUST_LGL_ENG_NM IS NULL THEN" ).append("\n"); 
		...
		query.append("#foreach( $key in ${dur} )" ).append("\n"); 
		query.append("                                         ,Z3.SUB_TRD_CD_${key}" ).append("\n"); 
		query.append("                                         ,Z3.SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("                                         ,Z3.C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                                    FROM (" ).append("\n"); 
		query.append("                                            SELECT " ).append("\n"); 
		query.append("                                                   Z2.TRD_CD," ).append("\n"); 
		query.append("                                                   O.N2ND_PRNT_OFC_CD      AS SLS_RHQ_CD," ).append("\n"); 
		query.append("                                                   NVL(Z2.OFC_CD,'TOTAL')  AS OFC_CD," ).append("\n"); 
		...
		query.append("#foreach( $key in ${dur} )" ).append("\n"); 
		query.append("                                                   ,MAX(DECODE(Z2.NUM,${key},Z2.SUB_TRD_CD))      AS SUB_TRD_CD_${key}                                   " ).append("\n"); 
		query.append("                                                   ,SUM(DECODE(Z2.NUM,${key},Z2.SUB_TRD_ADJ_QTY)) AS SUB_TRD_ADJ_QTY_${key}" ).append("\n"); 
		query.append("                                                   ,SUM(DECODE(Z2.NUM,${key},Z2.C_LOAD_QTA))      AS C_LOAD_QTA_${key}" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                                                                    (Z2.TRD_CD)," ).append("\n"); 
		query.append("                                                                    --(Z2.TRD_CD, Z2.CUST_CTRL_CD)," ).append("\n"); 
		query.append("                                                                    (Z2.TRD_CD, O.N2ND_PRNT_OFC_CD)," ).append("\n"); 
		query.append("                                                                    --(Z2.TRD_CD, O.N2ND_PRNT_OFC_CD, NVL(Z2.OFC_CD,'TOTAL'))," ).append("\n"); 
		query.append("                                                                    (Z2.TRD_CD, O.N2ND_PRNT_OFC_CD, Z2.CUST_CTRL_CD)," ).append("\n"); 
		query.append("                                                                    (Z2.TRD_CD, O.N2ND_PRNT_OFC_CD, NVL(Z2.OFC_CD,'TOTAL'), Z2.CUST_CTRL_CD)," ).append("\n"); 
		query.append("                                                                    (Z2.TRD_CD, O.N2ND_PRNT_OFC_CD, NVL(Z2.OFC_CD,'TOTAL'), Z2.CUST_CTRL_CD, Z2.ACCT_CLSS)," ).append("\n"); 
		query.append("                                                                    (Z2.TRD_CD, O.N2ND_PRNT_OFC_CD, NVL(Z2.OFC_CD,'TOTAL'), Z2.CUST_CTRL_CD, Z2.ACCT_CLSS, Z2.CUST_LGL_ENG_NM, Z2.CTRT_OFC_CD, Z2.RFA_NO, Z2.SC_NO)" ).append("\n"); 
		query.append("                                                                   )" ).append("\n"); 
		query.append("											UNION ALL" ).append("\n"); 
		query.append("                                            SELECT " ).append("\n"); 
		query.append("                                                   Z2.TRD_CD," ).append("\n"); 

...
*@FileName : ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpTSCostCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpTSCostCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Yard Cost 중 T/S ( T/S 에 적용 ) 금액 계산
	  * </pre>
	  */
	public ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpTSCostCSQL(){
		setQuery();
		...
		query.append("FileName : ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpTSCostCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("             ,  CASE WHEN IB_YD_CD = OB_YD_CD THEN 'S' ELSE 'D' END AS TML_AWK_TS_CD" ).append("\n"); 
		...
		query.append("             ,  CASE WHEN IB_YD_CD = OB_YD_CD THEN 'S' ELSE 'D' END" ).append("\n"); 
		query.append("        FROM    PRI_SCQ_AWK_ROUT_TMP A" ).append("\n"); 
		query.append("        WHERE   A.SCQ_RQST_NO = @[scq_rqst_no] " ).append("\n"); 
		query.append("        AND     A.SCQ_VER_NO = @[scq_ver_no_tmp]  " ).append("\n"); 
		query.append("        AND     A.ROUT_TP_CD = 'N' " ).append("\n"); 
		...
		query.append("    SELECT  B.ROUT_SEQ, B.ROUT_TP_CD, B.IO_BND_CD AS ROUT_IO_BND_CD, B.TML_AWK_TS_CD AS ROUT_TML_AWK_TS_CD, B.CGO_IO_GA_CD" ).append("\n"); 
		query.append("        ,   'T' AS COST_TP_CD " ).append("\n"); 
		query.append("        ,   MAX ( A.TML_AWK_TRF_VER_NO ) AS MAX_VER_NO" ).append("\n"); 
		query.append("        ,   A.IO_BND_CD, A.TML_AWK_TS_CD, A.IO_GA_CD, A.YD_CD, A.COND_NO" ).append("\n"); 
		query.append("        ,   CASE WHEN A.IO_BND_CD = 'A' AND A.TML_AWK_TS_CD = 'A' AND A.IO_GA_CD = 'A' THEN 1 ELSE 2 END AS PRIORITY" ).append("\n"); 
		query.append("        ,   MAX  ( CASE WHEN A.IO_BND_CD = 'A' AND A.TML_AWK_TS_CD = 'A' AND A.IO_GA_CD = 'A' THEN 1 ELSE 2 END ) " ).append("\n"); 
		query.append("            OVER ( PARTITION BY B.ROUT_SEQ, B.ROUT_TP_CD, B.IO_BND_CD, B.TML_AWK_TS_CD, B.CGO_IO_GA_CD, A.YD_CD ) AS MAX_PRIORITY" ).append("\n"); 
		query.append("    FROM    TES_AWK_CGO_TRF_DTL A, ROUT_CS B" ).append("\n"); 
		query.append("    WHERE   A.YD_CD = B.YD_CD" ).append("\n"); 
		query.append("    AND     A.TML_AWK_CGO_TRF_TP_CD = 'T'" ).append("\n"); 
		query.append("    AND     A.COND_NO NOT IN ( 9, 10 )" ).append("\n"); 
		query.append("    AND  (  " ).append("\n"); 
		query.append("               (  A.IO_BND_CD = B.IO_BND_CD AND A.TML_AWK_TS_CD = B.TML_AWK_TS_CD AND A.IO_GA_CD = B.CGO_IO_GA_CD )" ).append("\n"); 
		query.append("            OR (  A.IO_BND_CD = 'A'         AND A.TML_AWK_TS_CD = 'A'             AND A.IO_GA_CD = 'A'            ) " ).append("\n"); 
		query.append("         )" ).append("\n"); 
		query.append("    GROUP   BY  " ).append("\n"); 
		query.append("            B.ROUT_SEQ, B.ROUT_TP_CD, B.IO_BND_CD, B.TML_AWK_TS_CD, B.CGO_IO_GA_CD, A.IO_BND_CD, A.TML_AWK_TS_CD, A.IO_GA_CD, A.YD_CD, A.COND_NO" ).append("\n"); 
		query.append("           " ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("    WHERE   B.CGO_IO_GA_CD = A.CGO_IO_GA_CD" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", MAX_AMT_LIST AS (" ).append("\n"); 
		query.append("    SELECT" ).append("\n"); 
		query.append("            A.ROUT_SEQ, A.ROUT_IO_BND_CD, A.YD_CD" ).append("\n"); 
		query.append("        ,   A.COST_TP_CD" ).append("\n"); 
		query.append("        ,   B.CGO_SEQ, B.CNTR_QTY, C.USD_AMT" ).append("\n"); 
		query.append("        ,   C.SPCL_CGO_REF_SEQ" ).append("\n"); 
		query.append("        ,   MAX ( C.USD_AMT ) OVER ( PARTITION BY A.ROUT_SEQ, A.ROUT_IO_BND_CD, A.YD_CD, A.COST_TP_CD, B.CGO_SEQ ) AS MAX_USD_AMT" ).append("\n"); 
		...
		query.append("    AND     C.TML_AWK_CGO_TRF_TP_CD = 'T'" ).append("\n"); 
		...
		query.append("    ,   PRI_SCQ_AWK_COST_SEQ_FNC ( A.ROUT_TP_CD, A.COST_TP_CD, A.ROUT_IO_BND_CD ) AS YD_COST_SEQ" ).append("\n"); 
		query.append("    ,   ROW_NUMBER ( ) OVER ( PARTITION BY A.ROUT_SEQ, A.ROUT_IO_BND_CD, A.YD_CD, A.COST_TP_CD  ORDER BY A.CGO_SEQ ) AS YD_COST_DTL_SEQ" ).append("\n"); 
		query.append("    ,   A.ROUT_IO_BND_CD, A.YD_CD, A.COST_TP_CD, A.CGO_SEQ, A.CNTR_QTY, A.USD_AMT" ).append("\n"); 
		...
		query.append("GROUP   BY A.ROUT_SEQ, A.ROUT_TP_CD, A.ROUT_IO_BND_CD, A.YD_CD, A.COST_TP_CD, A.CGO_SEQ, A.CNTR_QTY, A.USD_AMT" ).append("\n"); 
		query.append("ORDER   BY A.ROUT_SEQ, A.ROUT_TP_CD, A.ROUT_IO_BND_CD, A.YD_CD, A.COST_TP_CD, A.CGO_SEQ" ).append("\n"); 

...
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.integration.PickUpNoticeDBDAO;
...
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.PkupNoRptEmlCtntVO;
...
public class PickUpNoticeBCImpl extends BasicCommandSupport implements PickUpNoticeBC {
	...
	private transient ArrivalNoticeDBDAO anDbDao = null;
	...
	public PickUpNoticeBCImpl() {
		...
		anDbDao = new ArrivalNoticeDBDAO();
	}
	...
	public void setupPkupNtcListByManual(PkupNtcManualListVO[] pkupNtcManualLists, SignOnUserAccount account) throws EventException{
		...
		try {
			...
			for (int i=0; i<pkupNtcManualLists.length; i++) {
				...
				//noVo.setRailOrgLocCd("");
				//noVo.setRailDestLocCd("");
				...
				//noVo.setUniPcfIndFlg("");
				//noVo.setCanPcfIndFlg("");
				//noVo.setPkupNtcIndCd("");
...
//					bkgPkupNtc.setPkupNo(vo.getPkupNo());
					//bkgPkupNtc.setRailRmpFreeDys(""); // (free dt - avl dt)
//					bkgPkupNtc.setLstFreeDt(vo.getLstFreeDt());
					bkgPkupNtc.setEdi322MvmtCd(vo.getEdi322MvmtCd());
//					bkgPkupNtc.setPkupYdCd(vo.getPkupYdCd());
//					bkgPkupNtc.setRtnYdCd(vo.getRtnYdCd());
...
//					bkgPkupNtc.setPkupAvalDt(vo.getPkupAvalDt());
					bkgPkupNtc.setBkgCustTpCd(vo.getBkgCustTpCd());
					//bkgPkupNtc.setAutoSndStopFlg("");
					//bkgPkupNtc.setAutoSndStopDt("");
					//bkgPkupNtc.setAutoSndStopUsrId("");
					//bkgPkupNtc.setAutoSndResmFlg("");
					//bkgPkupNtc.setAutoSndResmDt("");
					//bkgPkupNtc.setAutoSndResmUsrId("");					
					...
					// BL별 한번만 조회
					if (ss == 0) {
						//P/N Form Title Revise Check 
						reviseYN = anDbDao.searchReviseYN(vo.getBkgNo(),"PN");
					}
					...
					for (int k=0; k<bkgPkupNtcDtls.size(); k++) {
						...
						if (ss == 0) {
							...
							faxInfo.setTitle("PickUp Notice");
							faxInfo.setTmplParam("/rv bkg_no['" + bkgPkupNtcDtl.getBkgNo() + "'] ntc_seq['" + bkgPkupNtcDtl.getNtcSeq() + "'] usr_id['" + 
									             account.getUsr_id() + "'] form_revise_YN['"+reviseYN+"'] ofc_cd['" + account.getOfc_cd() + "'] p_pkup_ntc_fom_cd[''] p_pkup_yd_cd[''] p_rtn_yd_cd[''] p_rmk['']");
							...
							if ("Y".equals(vo.getEclzOblCpyFlg())) {
								...
								faxInfo.setSysCd("BKG");
								faxInfo.setBatchFlg("N");
								faxInfo.setTitle("PickUp Notice OBL");
	...
	public List<BkgNtcHisVO> sendPkupNtcByEmail(PkupNtcSendListVO[] pkupNtcSendLists, SignOnUserAccount account) throws EventException {
		...
		Date now = new Date();
		SimpleDateFormat sndFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String titNm = "[SM Line Corporation America] PickUp Notice";
		String reviseYN ="";
		try {
			 ...
			 * 
			 * * * * BL별 첫번째 데이타만 전송 * * *
			 *  >> BL별로 Pick-up Notice가 전송 되기 때문에 BL-Customer Type 별 데이터를 중복해서 전송하지 않게 하기 위한 처리.
			 *     BL-Customer Type 별 첫번째 컨테이너에서 메일 전송을 위해 대상 List에 Add 한다. 
			 *  
			 * BL1 - Con1 - C - C1 >> 전송 대상 mailInfos.add  details add >> group Detail List.add(details)
			 * BL1 - Con2 - C - C1 >>                         details add >> group Detail List.add(details)
			 * BL1 - Con3 - C - C1 >>                         details add >> group Detail List.add(details)
			 *  ---------------------------------- 전체 detailList.add(group Detail List)
			 *  
			 * BL2 - Con4 - C - B1,B2 >> 전송 대상 mailInfos.add  details add >> group Detail List.add(details)
			 * BL2 - Con5 - C - B1,B2 >>                         details add >> group Detail List.add(details)
			 * BL2 - Con6 - C - B1,B2 >>                         details add >> group Detail List.add(details)
			 *  ---------------------------------- 전체 detailList.add(group Detail List)
			 * 
			 * 전송 후 sendIds 내용
			 *  BL1 C1의 전송 ID SendID1
			 *  BL2 B1의 전송 ID SendID2
			 *  BL2 B2의 전송 ID SendID3
			 * 
			 * 
			 *  BL1 group Detail List 반복 
			 *    1.첫번째 BL1 Con1 details 반복시 맵에 C1 SendID1 매핑
			 *    2.맵에서 C1에 대한 Send ID를 조회하여 세팅한다.
			 *      결국 Con1,2,3 모두 같은 값이 세팅된다.
			 *       
			 *  BL2 group Detail List 반복 
			 *    1.첫번째 BL2 Con4 details 반복시 맵에 B1,B2 SendID2,3 매핑
			 *    2.맵에서 B1,B2에 대한 Send ID를 조회하여 세팅한다.
			 *      결국 Con4,5,6 모두 같은 값이 세팅된다. 
			 * 
			 * 
			 */
			for (int i=0; i<sortLists.size(); i++) {
					...
					for (int k=0; k<5; k++) {
						...
						if (bb == 0) {
							...
							rdVO.setRdParaCtnt("/rv bkg_no['"+bkgPkupNtcDtl.getBkgNo()+"'] ntc_seq['"+bkgPkupNtcDtl.getNtcSeq()+
									"'] form_revise_YN['"+reviseYN+"'] usr_id['" + account.getUsr_id() + "'] ofc_cd['" + account.getOfc_cd() + "']  p_pkup_ntc_fom_cd[''] p_pkup_yd_cd[''] p_rtn_yd_cd[''] p_rmk['']");
				...
				if ("Y".equals(comp)) {
					...
					if (strLen > 0 && "@".equals(oldStr) == false) {
						...
						if ( "@".equals(oldStr) && tmpStr.trim().length() > 0) {
							...
							tmpStr = tmpStr.replace("SM Line Corporation LIMITED"," ");
							tmpStr = tmpStr.replace(" ","@");

...
*@FileName : GeneralBookingSearchDBDAOsearchCustAdvisoryNoticeCntrCntRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class GeneralBookingSearchDBDAOsearchCustAdvisoryNoticeCntrCntRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * VVD에 대한 총 Container Count를 조회한다.
	  * 2017.09.28 iylee 조회조건 추가(Bkg No, Container No)
	  * </pre>
	  */
	public GeneralBookingSearchDBDAOsearchCustAdvisoryNoticeCntrCntRSQL(){
		setQuery();
		...
		query.append("FileName : GeneralBookingSearchDBDAOsearchCustAdvisoryNoticeCntrCntRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT SUM(EXCEL_CNT) + SUM(BST_CNT) || '@'|| SUM(EXCEL_CNT) || '@' || SUM(BST_CNT)  AS CNTR_CNT        " ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		query.append("            SELECT " ).append("\n"); 
		query.append("                    COUNT(DISTINCT DECODE(INQR2.SRC_DAT_TP_CD, 'E', C.CNTR_NO )) EXCEL_CNT" ).append("\n"); 
		query.append("                   ,COUNT(DISTINCT DECODE(INQR2.SRC_DAT_TP_CD, 'B', C.CNTR_NO )) BST_CNT  " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("            FROM   BKG_CUST_AVC_NTC_CNTR C" ).append("\n"); 
		query.append("                 ,( " ).append("\n"); 
		...
		query.append("     , BKG_COM_INTG_CD_NM_FNC('CD00960', INQR.NF_FAX_NTC_SND_RSLT_CD ) AS NF_EML_NTC_SND_RSLT_NM  -- E-MAIL??? ????" ).append("\n"); 
		...
		query.append("    , CASE WHEN (SELECT NEW_KEY_ACCT_FLG FROM MDM_CUSTOMER  WHERE CUST_CNT_CD = SUBSTR(INQR.CTRT_CD,1,2) AND CUST_SEQ = SUBSTR(INQR.CTRT_CD,3,8)) ='Y' THEN 'KC'" ).append("\n"); 
		query.append("           --WHEN (SELECT GLO_ACCT_FLG FROM MDM_CUSTOMER  WHERE CUST_CNT_CD = SUBSTR(INQR.CTRT_CD,1,2) AND CUST_SEQ = SUBSTR(INQR.CTRT_CD,3,8))='Y' THEN 'GA'" ).append("\n"); 
		query.append("           WHEN (SELECT RGN_ACCT_FLG FROM MDM_CUSTOMER  WHERE CUST_CNT_CD = SUBSTR(INQR.CTRT_CD,1,2) AND CUST_SEQ = SUBSTR(INQR.CTRT_CD,3,8)) ='Y' THEN 'RC'" ).append("\n"); 
		query.append("           ELSE 'LC'" ).append("\n"); 
		...
		query.append("          , MAX(DECODE(BD.BKG_CUST_TP_CD, 'V', BD.NTC_EML     , NULL))     AS NF_EML" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("WHERE INQR2.SRC_DAT_TP_CD = C.SRC_DAT_TP_CD" ).append("\n"); 
		query.append("    AND INQR2.BL_NO = C.BL_NO" ).append("\n"); 
		query.append("#if (${key_acct_flg} != '' || ${rgn_acct_flg} != ''  || ${lcl_acct_flg} != '')" ).append("\n"); 
		query.append("            AND  INQR2.ACCT_CLSS_FAX  IN (${search_clss_type})" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(")" ).append("\n"); 

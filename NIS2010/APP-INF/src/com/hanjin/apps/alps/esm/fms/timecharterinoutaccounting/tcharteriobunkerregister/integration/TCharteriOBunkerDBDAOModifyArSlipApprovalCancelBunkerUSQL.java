...
public class TCharteriOBunkerDBDAOModifyArSlipApprovalCancelBunkerUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("WHERE AR_SLP_TP_CD 	 = SUBSTR(@[csr_no],1,2)" ).append("\n"); 
		query.append("AND AR_SLP_FUNC_CD = SUBSTR(@[csr_no],3,1)" ).append("\n"); 
		query.append("AND AR_SLP_OFC_CD  = SUBSTR(@[csr_no],4,6)" ).append("\n"); 
		query.append("AND AR_SLP_ISS_DT  = SUBSTR(@[csr_no],10,6)" ).append("\n"); 
		query.append("AND AR_SLP_SER_NO  = SUBSTR(@[csr_no],16,5)" ).append("\n"); 

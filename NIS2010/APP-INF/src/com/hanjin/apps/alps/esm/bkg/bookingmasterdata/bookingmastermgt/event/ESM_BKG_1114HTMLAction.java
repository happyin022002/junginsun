...
*@FileName   : ESM_BKG_1114HTMLAction.java
...
public class ESM_BKG_1114HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1114HTMLAction 객체를 생성
	 */
	public ESM_BKG_1114HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1114Event event = new EsmBkg1114Event();
		
		if(command.isCommand(FormCommand.MULTI)) { 
			event.setZipCdListVOS((ZipCdListVO[])getVOs(request, ZipCdListVO .class,"sheet1_"));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			int iPage = JSPUtil.getParameterAsInt(request, "iPage", 1);
			event.setZipCdSchVO((ZipCdSchVO)getVO(request, ZipCdSchVO .class));

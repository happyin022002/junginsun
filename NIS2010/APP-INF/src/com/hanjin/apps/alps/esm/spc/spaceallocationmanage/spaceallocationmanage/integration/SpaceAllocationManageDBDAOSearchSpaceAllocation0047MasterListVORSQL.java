...
public class SpaceAllocationManageDBDAOSearchSpaceAllocation0047MasterListVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("               AND SUBSTR(V.SLS_YRMON, 1, 4)||V.COST_WK IN (SELECT /*+ INDEX(D XPKMAS_WK_PRD) */ " ).append("\n"); 
		...
		query.append("               AND V.DIR_CD     = SUBSTR(@[vvd], 9) " ).append("\n"); 
		...
		query.append(" " ).append("\n"); 
		query.append("                 AND (VPP.POL_CONTI    = VPP.OFC_CONTI OR (VPP.RLANE_CD = 'WAFIE' AND O.N2ND_PRNT_OFC_CD = @[office])) " ).append("\n"); 
		...
		query.append("         -- 2014.08.04 control 컬럼 추가 " ).append("\n"); 
		...
		query.append("         NVL(C.CTRL_FX_RT_FLG , 'N')          AS CTRL_FX_RT_FLG  , --2015.07.24 " ).append("\n"); 
		...
		query.append("                   SKD_DIR_CD    , " ).append("\n"); 
		query.append("                   ( " ).append("\n"); 
		query.append("                     SELECT    TO_CHAR(NVL(MAX(FD.FCNTR_LDB_SPC_CAPA), SPC_GET_WK_VVD_BSA_FNC('VOL', V.REP_TRD_CD, V.RLANE_CD, V.SKD_DIR_CD, V.COST_YR||V.COST_WK, V.VSL_CD||V.SKD_VOY_NO||V.SKD_DIR_CD)), 'FM00000000.000') " ).append("\n"); 
		query.append("                            || TO_CHAR(NVL(MAX(FD.FCNTR_LDB_WGT)     , SPC_GET_WK_VVD_BSA_FNC('WGT', V.REP_TRD_CD, V.RLANE_CD, V.SKD_DIR_CD, V.COST_YR||V.COST_WK, V.VSL_CD||V.SKD_VOY_NO||V.SKD_DIR_CD)), 'FM00000000.000') " ).append("\n"); 
		query.append("                     FROM SPC_ENG_FULL_DWN_LDB_SPC FD " ).append("\n"); 
		query.append("                     WHERE FD.TRD_CD    (+) = V.REP_TRD_CD " ).append("\n"); 
		query.append("                        AND FD.RLANE_CD  (+) = V.RLANE_CD " ).append("\n"); 
		query.append("                        AND FD.VSL_CD    (+) = V.VSL_CD " ).append("\n"); 
		query.append("                        AND FD.SKD_VOY_NO(+) = V.SKD_VOY_NO " ).append("\n"); 
		query.append("                        AND FD.SKD_DIR_CD(+) = V.SKD_DIR_CD " ).append("\n"); 
		query.append("                        AND FD.CLPT_CD   (+) = V.LST_LODG_PORT_CD " ).append("\n"); 
		...
		query.append("                   SUM(BKG_OCN_WGT) AS BKG_OCN_WGT, SUM(BKG_OCN_VOL_VGM) AS BKG_OCN_VOL_VGM , SUM(BKG_OCN_WGT_VGM) AS BKG_OCN_WGT_VGM, " ).append("\n"); 
		query.append("                   SUM(BKG_IPC_VOL) AS BKG_IPC_VOL, " ).append("\n"); 
		query.append("                   SUM(BKG_IPC_WGT) AS BKG_IPC_WGT, SUM(BKG_IPC_VOL_VGM) AS BKG_IPC_VOL_VGM, SUM(BKG_IPC_WGT_VGM) AS BKG_IPC_WGT_VGM," ).append("\n"); 
		...
		query.append("                         AND (VPP.POL_CONTI = VPP.OFC_CONTI OR (VPP.RLANE_CD = 'WAFIE' AND F.SLS_RHQ_CD = VPP.OFC_CD)) " ).append("\n"); 
		...
		query.append("                         AND (VPP.POL_CONTI = VPP.OFC_CONTI OR (VPP.RLANE_CD = 'WAFIE' AND A.SLS_RHQ_CD = VPP.OFC_CD)) " ).append("\n"); 
		query.append("                      UNION ALL " ).append("\n"); 
		...
		query.append("							 DECODE(IOC_CD, 'O', BKG_VOL_VGM) AS BKG_OCN_VOL_VGM, " ).append("\n"); 
		query.append("							 DECODE(IOC_CD, 'O', BKG_WGT_VGM) AS BKG_OCN_WGT_VGM, " ).append("\n"); 
		...
		query.append("                             DECODE(IOC_CD, 'I', BKG_VOL_VGM) AS BKG_IPC_VOL_VGM, " ).append("\n"); 
		query.append("							 DECODE(IOC_CD, 'I', BKG_WGT_VGM) AS BKG_IPC_WGT_VGM, " ).append("\n"); 
		query.append("                             DECODE(IOC_CD, 'T', BKG_TTL_QTY) AS BKG_TS_VOL, " ).append("\n"); 
		query.append("                             DECODE(IOC_CD, 'T', BKG_TTL_WGT) AS BKG_TS_WGT, " ).append("\n"); 
		query.append("                             DECODE(IOC_CD, 'T', BKG_VOL_VGM) AS BKG_TS_VOL_VGM, " ).append("\n"); 
		query.append("							 DECODE(IOC_CD, 'T', BKG_WGT_VGM) AS BKG_TS_WGT_VGM, " ).append("\n"); 
		query.append("                             CM_OP AS CM_OP	,    " ).append("\n"); 
		query.append("                             CM_OC AS CM_OC   ,    " ).append("\n"); 
		query.append("                             CM_VL AS CM_VL   ,  " ).append("\n"); 
		query.append("                             LST_LODG_PORT_ETD_DT, " ).append("\n"); 
		query.append("                             LST_LODG_PORT_CD " ).append("\n"); 
		query.append("                             ,DECODE(NVL(ALOC_STS_CD, 'X'), 'S', BKG_TTL_QTY, 'A' , BKG_TTL_QTY) AS BKG_BS_VOL " ).append("\n"); 
		query.append("                             ,DECODE(NVL(ALOC_STS_CD, 'X'), 'S', BKG_TTL_WGT, 'A' , BKG_TTL_WGT) AS BKG_BS_WGT " ).append("\n"); 
		query.append("                        FROM BKG_DATA " ).append("\n"); 
		query.append("                       WHERE  " ).append("\n"); 
		query.append("--                         TS_FLG = 'N' " ).append("\n"); 
		query.append("                          BKG_TTL_QTY + BKG_TTL_WGT > 0 " ).append("\n"); 
		query.append("                   ) V " ).append("\n"); 
		...
		query.append("/*" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("2010.10.06 최윤성 [CHM-201006339-01] Control by RHQ 더블콜 로직 보완" ).append("\n"); 

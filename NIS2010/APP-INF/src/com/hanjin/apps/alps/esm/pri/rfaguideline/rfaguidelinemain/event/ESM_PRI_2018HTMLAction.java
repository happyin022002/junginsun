 ...
 *@FileName : ESM_PRI_2018HTMLAction.java
...
public class ESM_PRI_2018HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri2018Event event = new EsmPri2018Event();

		if (command.isCommand(FormCommand.SEARCH01)) {
			event.setPriRgMnVO((PriRgMnVO) getVO(request, PriRgMnVO.class));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			PriRgMnVO vo = (PriRgMnVO) getVO(request, PriRgMnVO.class);
			vo.setEffDt(request.getParameter("scope_year"));
			event.setPriRgMnVO(vo);
		} else if (command.isCommand(FormCommand.SEARCH03)) {
			event.setRsltGlineScpEffDtListVO((RsltGlineScpEffDtListVO) getVO(request, RsltGlineScpEffDtListVO.class));

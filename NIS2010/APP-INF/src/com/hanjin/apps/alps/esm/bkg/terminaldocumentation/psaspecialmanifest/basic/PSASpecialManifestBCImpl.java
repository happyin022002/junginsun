...
* -------------------------------------------------------
* History
* 2012.03.13 김보배 [CHM-201216245] [BKG] PSA DG EDI 추가 요건
=========================================================*/
...
import com.hanjin.apps.alps.esm.bkg.terminaldocumentation.psaspecialmanifest.integration.PSASpecialManifestDBDAO;
...
public class PSASpecialManifestBCImpl extends BasicCommandSupport implements PSASpecialManifestBC {
	...
	public List<PSADgListDetailVO> searchPsaDgManifestList(PSADgListCondVO psaDgListCondVO) throws EventException {
		...
		List<PSADgListDetailVO> vslInfoList 	= null; 
		
		PSASpecialContainerVO psaSpecialContainerVO= null;
		...
		try {
			 ...
			 */
			vslInfoList = dbDao.serachPsaVslAtCommon(psaDgListCondVO);
			
			// booking쪽 위험물 데이타를 조회
			psaDgListCondVO.setAppendFlag("N");
			list = dbDao.searchPsaDgInfoAtBkgDg(psaDgListCondVO);

			// ContainerVO에 setting
			psaSpecialContainerVO = new PSASpecialContainerVO();

			psaSpecialContainerVO.setPsaDetailList(list);
    ...
    public List<PSASendHistoryDetailVO> searchPsaSendHistory(PSASendHistoryCondVO psaSendHistoryCondVO) throws EventException { 
		try {
			log.debug("\n psaSendHistoryCondVO==>"+psaSendHistoryCondVO.toString());
	...
	/**
	 * PSA위험물 세관신고 위해 FlatFile을 생성 및 전송
	 * 
	...
	public List<String> sendPsaDgManifestList(PSADgEdiVO[] psaDgEdiVOs, SignOnUserAccount account) throws EventException {
		...
		try {
			...
			// 리턴 FF에 구분선 넣어주기
			updateFlatFile = "";
			StringBuffer updateFlatFileBu = new StringBuffer();
			for(int i =0; i < updateFlatFileList.size(); i++) {
				updateFlatFileBu.append("[" + (i+1) + "]===============================\n");
				updateFlatFileBu.append(updateFlatFileList.get(i));
			}
			updateFlatFile = updateFlatFileBu.toString();
...
//		boolean inFlag2 = true;
		...
		List<PSACntrCgoVO> cntrCgoVOs = null;
		...
		List<PSADgSendDtlHistoryVO> dgSendDtlHistoryVOs = new ArrayList<PSADgSendDtlHistoryVO>(); // send detail list
		PSADgSendHistoryVO dgSendFlatFileHistoryVO = null; // FlatFile Log
		List<PSADgSendHistoryVO> dgSendFlatFileHistoryVOs = new ArrayList<PSADgSendHistoryVO>(); // FlatFile Log list
		/* log 저장 vo 선언  끝*/
		...
		try {
				...
				for(int idx = 0; idx < cntrBaseInfoVOsMaxSize; idx++) {
					...
					flatFile.append("{CONSIGNMENTS").append("\n");
						...
						if(!consignmentEmptyFlag) {
							...
							cntrCgoVOs = dbDao.searchPsaCntrCgoByCntrBase(cntrBaseInfoVO);
							
							for(int k = 0; k <cntrCgoVOs.size(); k++){
								cntrCgoVOs.get(k).setUsrId(usrId);
							}
							if(cntrCgoVOs != null) {
			...
			if(dgSendFlatFileHistoryVOs != null) {
				dbDao.addPsaSendFlatFileLog(dgSendFlatFileHistoryVOs);
			}
			...
			if(dgSendDtlHistoryVOs != null && dgSendDtlHistoryVOs.size() > 0) {
				dbDao.addPsaSendDtlLog(dgSendDtlHistoryVOs);
			}
			...
			sendFlatFileVO.setQueueNm(SubSystemConfigFactory.get("BKG.ALPSBKG_UBIZHJS_PSA_DG.IBMMQ.QUEUE"));
			//로컬 테스트시에 QUEUE Name
	...
	public void loadCstmsRcvMsg(String rcvMsg,String rcvGubun) throws EventException {
		...
		if(rcvGubun.equalsIgnoreCase("AlpsbkgTPsacusAckEven")) { // PSA위험물 신고 응답메시지 수신
	...
	private void loadAperakRcvMsg(String rcvMsg) throws EventException {
		...
		String msgTpId = "IFD";
		int RcvLogErrSeg = 1;
		PsaDGRcvMsgVO psaDGRcvMsgVO = null;
		//List<PsaDGRcvMsgVO> psaDGRcvMsgVOs = new ArrayList();

...
*@FileName : Kor24ManifestListDBDAOsearchMrnVslSysDtEtaEtdMaxDtRSQL.java
*@FileTitle :
*Open Issues :
*Change history :
...
public class Kor24ManifestListDBDAOsearchMrnVslSysDtEtaEtdMaxDtRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * searcMrnNoSysDtEtaEtdDt에서 조회값이 없을 경우 Vessel Port Skd상의 Max calling Indicator를 이용하여 다시 구한다.
	  * </pre>
	  */
	public Kor24ManifestListDBDAOsearchMrnVslSysDtEtaEtdMaxDtRSQL(){
		setQuery();
		...
		query.append("FileName : Kor24ManifestListDBDAOsearchMrnVslSysDtEtaEtdMaxDtRSQL").append("\n");
		query.append("*/").append("\n");
	}
	...
	public void setQuery(){
		...
		query.append("AND VPS.CLPT_IND_SEQ = (SELECT MAX(VPS2.CLPT_IND_SEQ)" ).append("\n");
		query.append("FROM VSK_VSL_PORT_SKD VPS2" ).append("\n");
		query.append("WHERE VPS2.VSL_CD = SUBSTR(@[in_vvd],1,4)" ).append("\n");
		query.append("AND VPS2.SKD_VOY_NO = SUBSTR(@[in_vvd],5,4)" ).append("\n");
		query.append("AND VPS2.SKD_DIR_CD = SUBSTR(@[in_vvd],9,1)" ).append("\n");
		query.append("AND VPS2.VPS_PORT_CD = DECODE(@[in_bound],'O', @[in_pol], @[in_pod])" ).append("\n");

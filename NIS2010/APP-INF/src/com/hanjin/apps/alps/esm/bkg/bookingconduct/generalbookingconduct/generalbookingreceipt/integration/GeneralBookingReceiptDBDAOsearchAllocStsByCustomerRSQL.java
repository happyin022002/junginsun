...
*@FileName : GeneralBookingReceiptDBDAOsearchAllocStsByCustomerRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class GeneralBookingReceiptDBDAOsearchAllocStsByCustomerRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Customer Allocation Status를 조회한다.
	  * </pre>
	  */
	public GeneralBookingReceiptDBDAOsearchAllocStsByCustomerRSQL(){
		setQuery();
		...
		query.append("FileName : GeneralBookingReceiptDBDAOsearchAllocStsByCustomerRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("              AND PSC.CMDT_CD = BK.CMDT_CD" ).append("\n"); 
		query.append("              AND ROWNUM = 1),9999) GRP_CMDT_CD" ).append("\n"); 
		query.append("      FROM BKG_BOOKING BK, BKG_VVD VVD, BKG_QUANTITY QTY, BKG_CUSTOMER SHPR" ).append("\n"); 
		...
		query.append("         , ALOC.SC_NO" ).append("\n"); 
		...
		query.append("     WHERE ALOC.BKG_ALOC_TP_CD = 'C'" ).append("\n"); 
		query.append("       AND ALOC.ALOC_USE_FLG   = 'Y'" ).append("\n"); 
		query.append("       AND NVL(BKG.SC_NO,'A')         = NVL(ALOC.SC_NO,             NVL(BKG.SC_NO,'A'))" ).append("\n"); 
		query.append("       AND NVL(BKG.RFA_NO,'A')        = NVL(ALOC.RFA_NO,            NVL(BKG.RFA_NO,'A'))" ).append("\n"); 
		...
		query.append("       " ).append("\n"); 
		query.append("       AND BKG.CMDT_CD          = NVL(ALOC.CMDT_CD,         BKG.CMDT_CD) " ).append("\n"); 
		query.append("       AND BKG.GRP_CMDT_CD      = NVL(ALOC.SCG_GRP_CMDT_SEQ,BKG.GRP_CMDT_CD)" ).append("\n"); 
		query.append("       AND BKG.CNTR_TPSZ_CD     = NVL(ALOC.CNTR_TPSZ_CD,    BKG.CNTR_TPSZ_CD) " ).append("\n"); 
		query.append("       AND BKG.CTRT_CUST_CNT_CD = NVL(ALOC.CTRT_CUST_CNT_CD,BKG.CTRT_CUST_CNT_CD) " ).append("\n"); 
		query.append("       AND BKG.CTRT_CUST_SEQ    = NVL(ALOC.CTRT_CUST_SEQ,   BKG.CTRT_CUST_SEQ) " ).append("\n"); 
		query.append("       AND BKG.CUST_CNT_CD      = NVL(ALOC.CUST_CNT_CD,     BKG.CUST_CNT_CD) " ).append("\n"); 
		query.append("       AND BKG.CUST_SEQ         = NVL(ALOC.CUST_SEQ,        BKG.CUST_SEQ) " ).append("\n"); 
		query.append("" ).append("\n"); 

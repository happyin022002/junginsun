...
*@FileName : EsmMas0153ViewAdapter.java
...
* 2009.09.15 송호진
* 1.0 Creation
* 2011.12.21 최윤성 [CHM-201115260-01] [MAS] Pre CM/OP Simulation화면 U.I 변경요청 - avgLvlChk 필드 추가
=========================================================*/
...
/**
 * ESM_MAS_0153 에 대한 ViewAdapter<br>
 ...
 * @since J2EE 1.6
 */
public class EsmMas0153ViewAdapter extends DefaultViewAdapter {

    public EsmMas0153ViewAdapter(){
    	super();
    }
		    	...
		    	try{
		    		if(totCnt > 0) {
			    		while(rowSet.next()){
			            	...
			            	strBuilder.append("<TD><![CDATA[" + JSPUtil.getNull(rowSet.getString("dys_dmt_expt_sea")) + "]]> </TD>" );
			            	strBuilder.append("<TD><![CDATA[" + JSPUtil.getNull(rowSet.getString("dys_dmt")) + "]]> </TD>" );	
                            strBuilder.append("<TD><![CDATA[" + JSPUtil.getNull(rowSet.getString("ttl_dys")) + "]]> </TD>" );   
		    	...
		    	try{
			    	...
			    	if (rowSet != null) {
			    		while (rowSet.next()) {
							...
							op_chk = rowSet.getString("op_chk");
							
							/*
							strBuilder.append("<TR level='1' >" );
							*/
							if( !("N".equals(rowSet.getString("op_chk")) && cost_act_grp_seq > 1000) ) {
			    	...
			    	//[2015-02-25] op_chk 체크 확인으로 주석처리 됨.
...
//            strBuilder.append("<ETC KEY='bkg_op_visible'>" + Utils.iif ( f_cob_profit_lv.equals("O"),"Y","N") +"</ETC>" );
            strBuilder.append("<ETC KEY='bkg_op_visible'>"+op_chk+"</ETC>" );
            strBuilder.append("</ETC-DATA>" );
	    }

...
*@FileName : ESM_BKG_0442HTMLAction.java
...
public class ESM_BKG_0442HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0440HTMLAction 객체를 생성 
	 */
	public ESM_BKG_0442HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0442Event event = new EsmBkg0442Event();
		ManifestListCondVO manifestListCondVO = new RocsManifestListCondVO(); 
		 
		if(command.isCommand(FormCommand.SEARCH)) {
			manifestListCondVO = (RocsManifestListCondVO)getVO(request, RocsManifestListCondVO.class);
			event.setRocsManifestListCondVO((RocsManifestListCondVO)manifestListCondVO);	
		}  else if(command.isCommand(FormCommand.MULTI)) { 
           RocsContainerSaveVO rocsContainerSaveVO = new RocsContainerSaveVO();
		   RocsBlModificationVO	rocsBlModificationVO = new RocsBlModificationVO();
		   
		   rocsBlModificationVO = (RocsBlModificationVO)getVO(request, RocsBlModificationVO.class);
		   
		   rocsContainerSaveVO.setRocsBlModificationVO(rocsBlModificationVO);
           rocsContainerSaveVO.setRocsSearchCntrListVOList((RocsSearchCntrListVO[]) getVOs(request, RocsSearchCntrListVO.class, "sheet1_"));
           rocsContainerSaveVO.setRocsSearchCargoDetailVOList((RocsSearchCargoDetailVO[]) getVOs(request, RocsSearchCargoDetailVO.class, "sheet2_"));
           			
			ManifestModificationVO[] vo = new ManifestModificationVO[1];
			vo[0] = rocsContainerSaveVO;

...
import com.hanjin.apps.alps.esm.mas.stdunitcost.mas.integration.MASDBDAO;
...
public class MASBCImpl extends BasicCommandSupport implements MASBC {
	...
	public void multiMAS0012(MasSvcTrnsPrcVO[] masSvcTrnsPrcVO, SignOnUserAccount account) throws EventException{
		try {			
			List<MasSvcTrnsPrcVO> updateVoList = new ArrayList<MasSvcTrnsPrcVO>();			
			for ( int i=0; i<masSvcTrnsPrcVO.length; i++ ) {				
				 if ( masSvcTrnsPrcVO[i].getIbflag().equals("U")){					
					masSvcTrnsPrcVO[i].setUpdUsrId(account.getUsr_id());					
					updateVoList.add(masSvcTrnsPrcVO[i]);					
				}
			}			

...
*@FileName : ESM_PRI_6026HTMLAction.java
...
public class ESM_PRI_6026HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6026HTMLAction 객체를 생성
	 */
	public ESM_PRI_6026HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri6026Event event = new EsmPri6026Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			InPrsAmendmentCmSummaryVO vo = (InPrsAmendmentCmSummaryVO)getVO(request, InPrsAmendmentCmSummaryVO .class);
			//입력하지 않았다면 전체에서 조회한다.
			if( vo.getFrmCtrtExpYr() == null || vo.getFrmCtrtExpYr().length() == 0){
				vo.setFrmCtrtEffYr("9999");
				vo.setFrmCtrtExpWk("01");
			}
			event.setInPrsAmendmentCmSummaryVO(vo);
		}else if(command.isCommand(FormCommand.SEARCH01) ) {
			event.setInPrsAmendmentCmSummaryVO((InPrsAmendmentCmSummaryVO)getVO(request, InPrsAmendmentCmSummaryVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02) ) {//EXCEL
			event.setInPrsAmendmentCmSummaryDownExcelVO((InPrsAmendmentCmSummaryDownExcelVO)getVO(request, InPrsAmendmentCmSummaryDownExcelVO .class));			
		}

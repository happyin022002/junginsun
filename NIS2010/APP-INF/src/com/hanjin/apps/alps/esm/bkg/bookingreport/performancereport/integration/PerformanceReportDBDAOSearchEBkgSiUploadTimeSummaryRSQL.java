...
*@FileName : PerformanceReportDBDAOSearchEBkgSiUploadTimeSummaryRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class PerformanceReportDBDAOSearchEBkgSiUploadTimeSummaryRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * eBkg&SI Upload Summary 조회
	  * * e-BKG은 BKG Office 기준, e-SI는 Doc. Perf Office GSO 기준에 의해 실적 귀속
	  * </pre>
	  */
	public PerformanceReportDBDAOSearchEBkgSiUploadTimeSummaryRSQL(){
		setQuery();
		...
		query.append("FileName : PerformanceReportDBDAOSearchEBkgSiUploadTimeSummaryRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT RHQ_CD, GSO_CD, BKG_OFC_CD, BKG_CNT, SI_CNT, USR_CNT, UPLD_TM, AVG_UPLD_TM, ROUND(UPLD_HH,2) UPLD_HH, ROUND(AVG_UPLD_HH,2) AVG_UPLD_HH" ).append("\n"); 
		...
		query.append("           COUNT(DISTINCT GSO_CD||BKG_OFC_CD||UPLD_USR_ID) USR_CNT, " ).append("\n"); 
		query.append("           BKG_GET_CONV_INTVAL_TIME_FNC(SUM(UPLD_TIME),'TM') UPLD_TM, " ).append("\n"); 
		query.append("           BKG_GET_CONV_INTVAL_TIME_FNC(SUM(UPLD_TIME)/COUNT(XTER_RQST_NO),'TM') AVG_UPLD_TM," ).append("\n"); 
		query.append("           (SUM(UPLD_TIME))  UPLD_HH," ).append("\n"); 
		query.append("           (SUM(UPLD_TIME)/COUNT(XTER_RQST_NO)) AVG_UPLD_HH" ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		query.append("            SELECT DISTINCT BKG_NO,XTER_RQST_NO,XTER_RQST_SEQ, U.BKG_OFC_CD," ).append("\n"); 
		...
		query.append("                   , BKG_E_BKG_TURNTIME_FNC('D',substr(U.POR_CD,1,2),RQST_DT,UPLD_DT) UPLD_TIME" ).append("\n"); 
		query.append("                   , RQST_DT, UPLD_DT" ).append("\n"); 
		query.append("            FROM (" ).append("\n"); 
		query.append("                   SELECT T.BKG_NO," ).append("\n"); 
		query.append("                          T.POR_CD," ).append("\n"); 
		query.append("                          T.BKG_OFC_CD," ).append("\n"); 
		query.append("                          T.UPLD_USR_ID," ).append("\n"); 
		...
		query.append("                                  U.UPLD_USR_ID," ).append("\n"); 
		...
		query.append("            WHERE U.UPLD_RANK = 1" ).append("\n"); 
		query.append("              AND U.RQST_RANK = 1" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("            )" ).append("\n"); 
		query.append("    WHERE UPLD_TIME > 0 " ).append("\n"); 
		query.append("    GROUP BY ROLLUP(RHQ_CD, BKG_OFC_CD, GSO_CD)" ).append("\n"); 
		query.append("    )" ).append("\n"); 

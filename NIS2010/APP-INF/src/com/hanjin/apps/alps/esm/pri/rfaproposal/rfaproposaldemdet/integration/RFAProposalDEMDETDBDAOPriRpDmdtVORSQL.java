...
*@FileName : RFAProposalDEMDETDBDAOPriRpDmdtVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFAProposalDEMDETDBDAOPriRpDmdtVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public RFAProposalDEMDETDBDAOPriRpDmdtVORSQL(){
		setQuery();
		...
		query.append("FileName : RFAProposalDEMDETDBDAOPriRpDmdtVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("--	 , (SELECT TO_CHAR(CTRT_EFF_DT, 'YYYYMMDD') FROM PRI_RP_DUR WHERE PROP_NO = A.PROP_NO" ).append("\n"); 
		query.append("--             AND AMDT_SEQ = A.N1ST_CMNC_AMDT_SEQ) EFF_DT" ).append("\n"); 
		query.append("--	 , CASE WHEN A.AMDT_SEQ = @[amdt_seq] THEN TO_CHAR(B.CTRT_EXP_DT,'YYYYMMDD')" ).append("\n"); 
		query.append("--     		ELSE (SELECT CASE WHEN A.AMDT_SEQ <= N.AMDT_SEQ THEN TO_CHAR(B.CTRT_EFF_DT - 1,'YYYYMMDD')" ).append("\n"); 
		query.append("--                              ELSE TO_CHAR(N.CTRT_EXP_DT,'YYYYMMDD')" ).append("\n"); 
		query.append("--                         END AS EXP_DT" ).append("\n"); 
		query.append("--                    FROM PRI_RP_DUR N" ).append("\n"); 
		query.append("--                   WHERE PROP_NO = B.PROP_NO AND AMDT_SEQ = B.AMDT_SEQ-1)" ).append("\n"); 

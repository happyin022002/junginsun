 ...
 * 2008.10.22 전윤주 CSR No.N200810210022 COA_RD2, RD5 Rule-Set 화면 보완 [016, 018, 136, 161]
 * 2009.04.20 전윤주 CSR No.N200904070092 COA_CM 계산 수식 변경 [019]   
...
public class EQBalanceDBDAO extends DBDAOSupport {
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESM_COA_0016 화면에 대한 멀티 이벤트 처리(UPDATE)<br>
	 * 
	 * @param List<CoaCntrRepoShtgInfoVO> updModels
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifyEQBalance00163(List<CoaCntrRepoShtgInfoVO> updModels) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(updModels.size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoShtgInfoVOUSQL(), updModels,null);
	...
	/**
	 * 멀티 이벤트 처리<br>
	 * ESM_COA_0016 화면에 대한 멀티 이벤트 처리(Insert)<br>
	 * 
	 * @param List<CoaCntrRepoShtgInfoVO> insModels
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] addEQBalance00163(List<CoaCntrRepoShtgInfoVO> insModels) throws DAOException,Exception {
		int insCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(insModels.size() > 0){
				insCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoShtgInfoVOCSQL(), insModels,null);
    ...
    /**
	 * 멀티 이벤트 처리<br>
	 * ESM_COA_0016 화면에 대한 멀티 이벤트 처리(DELETE)<br>
	 * 
	 * @param List<CoaCntrRepoShtgInfoVO> delModels
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	 public int[] removeEQBalance00163(List<CoaCntrRepoShtgInfoVO> delModels) throws DAOException, Exception {
		int insCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(delModels.size() > 0){	
					insCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoShtgInfoVODSQL(), delModels,null);
	...
	/**
	 * ESM_COA_0016 화면에 대한 이벤트 처리(DELETE)<br>
	 * 
	 * @param SearchConditionVO searchConditionVO
	 * @throws DAOException
	 * @throws Exception
	 */
	public void removeCostYrmonCoaCntrRepoShtgInfo(SearchConditionVO searchConditionVO) throws DAOException,Exception {
		//query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		//velocity parameter
		Map<String, Object> velParam = new HashMap<String, Object>();
		try {
			...
			SQLExecuter sqlExe = new SQLExecuter("");
			int result = sqlExe.executeUpdate((ISQLTemplate)new EQBalanceDBDAODeleteCostYrmonCoaCntrRepoShtgInfoVODSQL(), param, velParam);
   ...
   /**
	 ...
	 * Set Credit Ratio popup
	 * ESM_COA_0018 수정
	 * @param List<CoaCntrRepoCrVO> coaCntrRepoCrVO
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifyEQBalance0018(List<CoaCntrRepoCrVO> coaCntrRepoCrVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaCntrRepoCrVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoCrVOUSQL(), coaCntrRepoCrVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	 ...
	 * ESM_COA_0136 수정
	 * 
	 * @param List<CoaCntrRepoRoutEccVO> coaCntrRepoRoutEccVO
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifyEQBalance0136(List<CoaCntrRepoRoutEccVO> coaCntrRepoRoutEccVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaCntrRepoRoutEccVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoRoutEccVOPORUSQL(), coaCntrRepoRoutEccVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	 ...
	 * @throws Exception
	 */
	public int[] modifyEQBalance0161(List<CoaCntrRepoRoutEccVO> coaCntrRepoRoutEccVO) throws DAOException,Exception {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if(coaCntrRepoRoutEccVO .size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new EQBalanceDBDAOCoaCntrRepoRoutEccVODELUSQL(), coaCntrRepoRoutEccVO,null);
				for(int i = 0; i < updCnt.length; i++){
					if(updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
	...
	/**
	 * COA_CNTR_REPO_SHTG_INFO 테이블의 키중복 check  합니다. <br> dupCheck
	 * 
	 * @param costYrmon
	 * @param cntrOrgDestCd
	 * @param cntrTpszCd

...
*@FileName : ESM_FMS_0094HTMLAction.java
...
public class ESM_FMS_0094HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0094HTMLAction 객체를 생성
	 */
	public ESM_FMS_0094HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0094Event event = new EsmFms0094Event();
		
		if (command.isCommand(FormCommand.SEARCH)) {
			event.setCondSearchPrepaymentSettleListVO((CondSearchPrepaymentSettleListVO)getVO(request, CondSearchPrepaymentSettleListVO .class));
		}else if (command.isCommand(FormCommand.SEARCH01)) {
			event.setSlpIssDt(request.getParameter("slp_iss_dt"));
			event.setAproStep(request.getParameter("apro_step"));
			event.setEffDt(request.getParameter("eff_dt"));
			event.setCondSearchPrepaymentSettleVvdVOS((CondSearchPrepaymentSettleVvdVO[])getVOs(request, CondSearchPrepaymentSettleVvdVO .class));

...
public class AGNCommRequestDBDAOGetAgmtInfoListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		   CTRT_OFC_CD,CTRT_AR," ).append("\n"); 
		...
		query.append("			-- 2017.08.22 Charge Commission 추가" ).append("\n"); 
		query.append("        	,(SELECT DECODE(COUNT(1),0,0,1) " ).append("\n"); 
		query.append("              FROM  ACM_AGN_AGMT_CHG_COMM  H " ).append("\n"); 
		query.append("              WHERE H.AGN_CD       = D.AGN_CD " ).append("\n"); 
		query.append("                AND H.AGN_AGMT_NO  = D.AGN_AGMT_NO " ).append("\n"); 
		query.append("                AND H.IO_BND_CD    = D.IO_BND_CD " ).append("\n"); 
		query.append("                AND H.AC_TP_CD     = D.AC_TP_CD " ).append("\n"); 
		query.append("                AND H.AGN_AGMT_SEQ = D.AGN_AGMT_SEQ " ).append("\n"); 
		query.append("                AND ((D.CHG_COMM_DIV_CD = 'M' AND H.CHG_CD IN (SELECT CHG_CD FROM BKG_CHG_RT WHERE BKG_NO = @[bkg_no] AND FRT_INCL_XCLD_DIV_CD = 'N' AND FRT_TERM_CD = DECODE(D.CHG_COMM_PAY_TERM_CD, 'T', FRT_TERM_CD, '', FRT_TERM_CD, D.CHG_COMM_PAY_TERM_CD)))" ).append("\n"); 
		query.append("			 	  OR (D.CHG_COMM_DIV_CD = 'T' AND H.CHG_CD IN (SELECT CHG_CD FROM BKG_TRF_SCG_RT WHERE BKG_NO = @[bkg_no] AND FRT_INCL_XCLD_DIV_CD = 'N' AND FRT_TERM_CD = DECODE(D.CHG_COMM_PAY_TERM_CD, 'T', FRT_TERM_CD, '', FRT_TERM_CD, D.CHG_COMM_PAY_TERM_CD)))" ).append("\n"); 
		query.append("                  OR (D.CHG_COMM_DIV_CD = 'R' AND H.CHG_CD IN (SELECT CHG_CD FROM INV_AR_MN P, INV_AR_CHG Q WHERE P.AR_IF_NO = Q.AR_IF_NO AND P.BKG_NO = @[bkg_no] AND P.REV_TP_CD = 'M' AND P.IO_BND_CD = DECODE(D.CHG_COMM_PAY_TERM_CD, 'T', P.IO_BND_CD, '', P.IO_BND_CD, DECODE(D.CHG_COMM_PAY_TERM_CD, 'P', 'O', 'I')))))" ).append("\n"); 
		query.append("              )                                         AS MAT_CNT_COMMCHG " ).append("\n"); 
		...
		query.append("	-- 2017.08.22 Charge Commission 추가" ).append("\n"); 
		query.append("	AND ((AC_TP_CD = 'P' AND MAT_CNT_COMMCHG = 1) OR AC_TP_CD <> 'P')" ).append("\n"); 

...
*@FileName : ESM_BKG_0079HTMLAction.java
...
public class ESM_BKG_0079HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0079HTMLAction 객체를 생성
	 */
	public ESM_BKG_0079HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0079Event event = new EsmBkg0079Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {        //SearchBkgBlNoVO 
		...
		else if(command.isCommand(FormCommand.MULTI)) {    //completeCA
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
			event.setBccExistFlg(JSPUtil.getParameter(request, "bcc_exist_flg"));
		}
		else if(command.isCommand(FormCommand.MULTI01)) {  //cancelCA
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
		}
		else if(command.isCommand(FormCommand.COMMAND02)) {  //cancelCA
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
		}
		else if(command.isCommand(FormCommand.SEARCHLIST01)) {  //BCC check
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
		}

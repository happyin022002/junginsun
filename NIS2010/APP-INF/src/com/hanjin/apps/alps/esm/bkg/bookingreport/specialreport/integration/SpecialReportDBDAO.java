...
public class SpecialReportDBDAO extends DBDAOSupport {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<SpecialCargoManifestInfoVO> checkSpecialCargoManifestRd(SpecialCargoManifestInfoVO vo) throws DAOException {
		DBRowSet dbRowset = null;
		List<SpecialCargoManifestInfoVO> list = null;
		// query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		...
		try {
			...
			param.putAll(mapVO);
			velParam.putAll(mapVO);
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate) new SpecialReportDBDAOCheckSpecialCargoManifestRdRSQL(), param, velParam);
			list = (List) RowSetUtil.rowSetToVOs(dbRowset, SpecialCargoManifestInfoVO.class);
		} catch (SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage(), se);

...
public class TCharterIOInvoiceDBDAOSlipApprovalCancelInvoiceUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("AND		SLP_OFC_CD 	= DECODE(LENGTH(@[csr_no]), 19, SUBSTR(@[csr_no], 4, 5), SUBSTR(@[csr_no], 4, 6))" ).append("\n"); 
		query.append("AND		SLP_ISS_DT 	= DECODE(LENGTH(@[csr_no]), 19, SUBSTR(@[csr_no], 9, 6), SUBSTR(@[csr_no], 10, 6))" ).append("\n"); 
		query.append("AND		SLP_SER_NO 	= DECODE(LENGTH(@[csr_no]), 19, SUBSTR(@[csr_no], 15, 5), SUBSTR(@[csr_no], 16, 5))" ).append("\n"); 

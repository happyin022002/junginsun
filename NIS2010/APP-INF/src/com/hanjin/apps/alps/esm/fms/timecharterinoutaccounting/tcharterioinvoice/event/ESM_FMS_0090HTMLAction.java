...
*@FileName : ESM_FMS_0090HTMLAction.java
 ...
 * @author JUNGYOONTAE
 * @see EsmFms0090Event 참조
 * @since J2EE 1.5
 */

public class ESM_FMS_0090HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0090HTMLAction 객체를 생성
	 */
	public ESM_FMS_0090HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0090Event event = new EsmFms0090Event();
				
		FormCommand command = FormCommand.fromRequest(request);
		if(command.isCommand(FormCommand.SEARCH)) {						
			event.setSearchOffhireExpensesListVO((SearchOffhireExpensesListVO)getVO(request, SearchOffhireExpensesListVO.class));				
		}else if(command.isCommand(FormCommand.SEARCHLIST)) {
			event.setSearchOffhireExpensesListVO((SearchOffhireExpensesListVO)getVO(request, SearchOffhireExpensesListVO.class));			
		}else if(command.isCommand(FormCommand.MULTI)) {			
			event.setSearchOffhireExpensesListVOs((SearchOffhireExpensesListVO[])getVOs(request, SearchOffhireExpensesListVO.class,"sheet1_"));
		}else if(command.isCommand(FormCommand.MULTI01)) {			
			event.setSearchOffhireExpensesListVOs((SearchOffhireExpensesListVO[])getVOs(request, SearchOffhireExpensesListVO.class,"sheet1_"));
		}else if(command.isCommand(FormCommand.MULTI02)) {			
			event.setSearchOffhireExpensesListVOs((SearchOffhireExpensesListVO[])getVOs(request, SearchOffhireExpensesListVO.class,"sheet2_"));
		}else if(command.isCommand(FormCommand.MULTI03)) {			
			event.setSearchOffhireExpensesListVOs((SearchOffhireExpensesListVO[])getVOs(request, SearchOffhireExpensesListVO.class,"sheet2_"));
		}else if(command.isCommand(FormCommand.MULTI04)) {			
			event.setSearchOffhireExpensesListVOs((SearchOffhireExpensesListVO[])getVOs(request, SearchOffhireExpensesListVO.class,"sheet3_"));

...
*@FileName : ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * PRI_SCQ_AWK_YD_DTL_TMP 의 Yard - Cargo 별 Cost 계산 및 결과 생성
	  * </pre>
	  */
	public ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpCSQL(){
		setQuery();
		...
		query.append("FileName : ScqAwkwardDBDAOcalcPriScqAwkYdDtlTmpCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        ,   CASE WHEN  B.ROUT_TP_CD IN ( 'L', 'P' ) AND A.COND_NO NOT IN ( 3, 4 ) THEN 'N'" ).append("\n"); 
		query.append("             WHEN  ( B.ROUT_TP_CD IN ( 'L', 'P' ) AND A.COND_NO = 3 ) OR ( B.ROUT_TP_CD = 'N' AND A.COND_NO = 3 ) THEN 'G' " ).append("\n"); 
		query.append("             WHEN  ( B.ROUT_TP_CD IN ( 'L', 'P' ) AND A.COND_NO = 4 ) OR ( B.ROUT_TP_CD = 'N' AND A.COND_NO = 4 ) THEN 'W'" ).append("\n"); 
		query.append("             WHEN  B.ROUT_TP_CD = 'N' AND A.COND_NO NOT IN ( 3, 4 ) THEN 'T' END AS COST_TP_CD " ).append("\n"); 

...
/**
 * ENIS-Agent Commission에 대한 공통업무 DB 처리를 담당<br>
 * - ENIS-Agent Commission에 대한 공콩업무를 처리하기 위한 JDBC 작업수행.<br>
 * 
...
public class CommonDBDAO extends DBDAOSupport {
    ...
    /**
	 * 남미 계약 입력 화면용 Office 목록을 조회한다..<br>
	 * COM_INTG_CD_DTL
	 * 
	 * @param  code
	 * @return DBRowSet DB 처리 결과
	 * @throws DAOException
	 */
	public DBRowSet searchArOfcListCmpn(String code) throws DAOException {
		log.debug("\n AGT Common searchArOfcListCmpn \n");
		...
		try {
			...
			param.put("code", code );
			velParam.put("code", code );
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new AGTCommonDBDAOSearchAROfficeListCmpnRSQL(), param, velParam);
	 ...
	 * @throws DAOException
	 */
	public DBRowSet searchArOfcAgmtCmpn(String code) throws DAOException {
		log.debug("\n AGT Common searchArOfcAgmtCmpn \n");
		...
		try {
			...
			param.put("code", code );
			velParam.put("code", code );
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new AGTCommonDBDAOSearchAROfficeAgmtCmpnRSQL(), param, velParam);
    ...
    public HashMap<String, String> searchCustomerName(String cust_cd ) throws DAOException {
        ...
        try {
			...
			velParam.put("iCust_seq", iCust_seq );

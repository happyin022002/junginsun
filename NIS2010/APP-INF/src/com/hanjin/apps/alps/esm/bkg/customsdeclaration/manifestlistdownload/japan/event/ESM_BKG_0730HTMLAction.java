...
*Open Issues :
*Change history : 
*	2017.09.07 하대성 2017 Renewal Version Trasmit 반영
...
public class ESM_BKG_0730HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		ManifestListCondVO manifestListCondVO = new JapanManifestListCondVO();
		ManifestModificationVO[] manifestModificationVOs = null;
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			manifestModificationVOs = (JapanManifestModificationVO[])getVOs(request, JapanManifestModificationVO.class, "");
			event.setManifestModificationVOS((JapanManifestModificationVO[])manifestModificationVOs);
		} else if(command.isCommand(FormCommand.MULTI01)) { 
			japanManifestTransmitVOs = (JapanManifestTransmitVO[])getVOs(request, JapanManifestTransmitVO.class);
			log.info("##############");
			log.info("length:"+japanManifestTransmitVOs.length);
			log.info("##############");

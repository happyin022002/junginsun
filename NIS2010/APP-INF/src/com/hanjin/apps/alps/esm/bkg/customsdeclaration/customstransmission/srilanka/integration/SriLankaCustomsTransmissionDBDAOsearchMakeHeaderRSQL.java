...
public class SriLankaCustomsTransmissionDBDAOsearchMakeHeaderRSQL implements ISQLTemplate{
	...
	public SriLankaCustomsTransmissionDBDAOsearchMakeHeaderRSQL(){
		...
		params.put("edi_mt_removal",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("       RPAD(NVL(DECODE(@[edi_mt_removal], 'M', 'SLKCUS_E', 'SRILANKACUSTOMS'), ' '), 20, ' ')||" ).append("\n"); 
		...
		query.append("       RPAD(NVL(DECODE(@[edi_mt_removal], 'M', 'SLKCUS_E', 'CTS_SK'), ' '), 20, ' ')||" ).append("\n"); 
		query.append("       RPAD(NVL(TRIM('CUSCAR'), ' '), 10, ' ')||" ).append("\n"); 
		query.append("       'BKC'||" ).append("\n"); 
		query.append("       TO_CHAR(SYSDATE, 'RRMMDD')||" ).append("\n"); 
		query.append("       LTRIM(TO_CHAR(BKG_CSTM_EDI_SEQ.NEXTVAL, '000009'), ' ') MSG_HEADER," ).append("\n"); 
		query.append("	   TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC(@[ofc_cd], GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC(@[ofc_cd]),'GMT') - 4/24 ,'YYYYMMDDHH24MISS')||'EDT' DOC_DT" ).append("\n"); 
		query.append("FROM DUAL" ).append("\n"); 
		query.append("" ).append("\n"); 

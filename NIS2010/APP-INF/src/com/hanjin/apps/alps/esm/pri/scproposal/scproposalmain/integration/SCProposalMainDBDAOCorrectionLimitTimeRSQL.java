...
*@FileName : SCProposalMainDBDAOCorrectionLimitTimeRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCProposalMainDBDAOCorrectionLimitTimeRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * * 2017.04.11 송민석 처음 생성
	  * * 2017.11.28 송민석 [CSR-2620] 체크기간을 48시간에서 10일로 변경함
	  * </pre>
	  */
	public SCProposalMainDBDAOCorrectionLimitTimeRSQL(){
		setQuery();
		...
		query.append("FileName : SCProposalMainDBDAOCorrectionLimitTimeRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("SELECT trunc(((86400*(DATE_AFTER_10DAY-US_EST_SYS_DT))/60)/60)  etc1" ).append("\n"); 
		query.append("    ,trunc((86400*(DATE_AFTER_10DAY-US_EST_SYS_DT))/60)-(trunc(((86400*(DATE_AFTER_10DAY-US_EST_SYS_DT))/60)/60)*60) etc2" ).append("\n"); 
		...
		query.append("SELECT trunc(((86400*(DATE_AFTER_48-US_EST_SYS_DT))/60)/60)  etc1" ).append("\n"); 
		query.append("    ,trunc((86400*(DATE_AFTER_48-US_EST_SYS_DT))/60)-(trunc(((86400*(DATE_AFTER_48-US_EST_SYS_DT))/60)/60)*60) etc2" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("        SELECT GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC('NYCSC') US_EST_SYS_DT --현재 미 동부 시간 filing 기준  " ).append("\n"); 

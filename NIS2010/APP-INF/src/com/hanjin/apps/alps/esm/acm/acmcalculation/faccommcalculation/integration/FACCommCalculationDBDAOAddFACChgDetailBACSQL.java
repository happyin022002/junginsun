...
*@FileName : FACCommCalculationDBDAOAddFACChgDetailBACSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class FACCommCalculationDBDAOAddFACChgDetailBACSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * - FACCommCalculationDBDAOAddFACChgDetailBACSQL
	  * - 2015.08.05 Sang-Hyun Kim [CHM-201537067] Split01-[그룹사 표준 코드 시행] HJS 프로그램 대응 및 데이타 마이그레이션 작업 요청
	  * </pre>
	  */
	public FACCommCalculationDBDAOAddFACChgDetailBACSQL(){
		setQuery();
		...
		query.append("FileName : FACCommCalculationDBDAOAddFACChgDetailBACSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("#if (${grs_net_div_cd} == 'Y') " ).append("\n"); 
		query.append("  AND CHG_CD = 'OFT'" ).append("\n"); 
		query.append("#end		" ).append("\n"); 
		query.append("  AND A.CURR_CD = B.CURR_CD" ).append("\n"); 
		query.append("  AND B.ACCT_XCH_RT_YRMON = (CASE WHEN @[vps_etd_dt] > TO_CHAR(SYSDATE, 'YYYYMM') THEN TO_CHAR(ADD_MONTHS(SYSDATE, -1), 'YYYYMM')" ).append("\n"); 

...
public class TransferOrderIssueDBDAOSearchBkgForTroRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            THEN DECODE(@[io_bnd_cd], 'O', BK.FULL_RTN_YD_CD, BK.MTY_RTN_YD_CD)" ).append("\n"); 
		query.append("           ELSE  DECODE(@[io_bnd_cd], 'O', NVL(COP_DTL_OBD.PICK_UP_CY, BK.FULL_RTN_YD_CD), NVL(COP_DTL_IBD.MTY_YD,     BK.MTY_RTN_YD_CD)) " ).append("\n"); 
		query.append("      END RETURN_CY" ).append("\n"); 
		query.append("    , CASE WHEN (SELECT COUNT(*) FROM BKG_VVD WHERE BKG_NO = @[bkg_no]) > 4 " ).append("\n"); 
		query.append("            THEN DECODE(@[io_bnd_cd], 'O', BK.MTY_PKUP_YD_CD, BK.FULL_PKUP_YD_CD)" ).append("\n"); 
		query.append("           ELSE  DECODE(@[io_bnd_cd], 'O', NVL(COP_DTL_OBD.MTY_YD,     BK.MTY_PKUP_YD_CD), NVL(COP_DTL_IBD.PICK_UP_CY, BK.FULL_PKUP_YD_CD))" ).append("\n"); 
		query.append("      END PICKUP_CY" ).append("\n"); 
		query.append("    --------------------------------------------------------" ).append("\n"); 
		...
		query.append("    , NVL(BK.ALOC_STS_CD,'F') ALOC_STS_CD" ).append("\n"); 

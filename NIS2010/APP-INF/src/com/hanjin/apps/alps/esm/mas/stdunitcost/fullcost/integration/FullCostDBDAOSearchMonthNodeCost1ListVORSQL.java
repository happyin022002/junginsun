...
public class FullCostDBDAOSearchMonthNodeCost1ListVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * MAS_NOD_AVG_STND_COST 테이블의 데이터 조회 (Full)
	  * History-----------------------------
	  * 2011.06.34 이행지 [CHM-201111807-01] MAS NODE 단가 화면 저장시 발생될 수 있는 문제 해결- R.lane이 'NNN'인것만 Sum될 수 있도록 조건추가
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("       ,K.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("       ,MAS_GET_COM_NM_FNC('COST_ACT_GRP_CD', M.COST_ACT_GRP_CD) GRP" ).append("\n"); 
		query.append("       ,MAS_GET_COM_NM_FNC('STND_COST_CD', M.STND_COST_CD) STND_COST_NM" ).append("\n"); 
		query.append("       ,MAS_GET_COM_NM_FNC('MAS_COST_SRC_CD', K.MAS_COST_SRC_CD) MAS_COST_SRC_CD_NM" ).append("\n"); 
		...
		query.append("       ,DECODE(GROUPING(K.MAS_COST_SRC_CD), 1, 1, 2) LVL" ).append("\n"); 
		query.append("       ,'AVERAGE' COST_ASS_BSE_CD" ).append("\n"); 
		query.append("       ,DECODE(@[f_cost_loc_grp_cd], 'S', 'SCC', 'E', 'ECC', 'R', 'RCC', 'NODE') LOC_TYPE" ).append("\n"); 
		query.append("	     ,DECODE(GROUPING(K.MAS_COST_SRC_CD), 1,  MAS_GET_COM_NM_FNC('STND_COST_CD', M.STND_COST_CD), MAS_GET_COM_NM_FNC('MAS_COST_SRC_CD', K.MAS_COST_SRC_CD)) COST_NM" ).append("\n"); 
		...
		query.append("          FROM MAS_COST_SRC_ACCT B, MAS_STND_ACCT C, MAS_ACT_GRP_COST_MAPG D" ).append("\n"); 
		...
		query.append("           AND B.MAS_COST_SRC_CD = D.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("      " ).append("\n"); 
		...
		query.append("       ,(SELECT   D.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("                 ,D.LOCL_CURR_CD" ).append("\n"); 
		query.append("                 ,SUM(D.STND_COST_USD_AMT) STND_COST_USD_AMT" ).append("\n"); 
		query.append("            FROM MAS_NOD_AVG_STND_COST D" ).append("\n"); 
		query.append("           WHERE 1 = 1" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("             AND D.NOD_CD IN (" ).append("\n"); 
		query.append("	               NVL(MAS_LOC_FNC(@[nod_cd], DECODE(@[f_cost_loc_grp_cd], 'S', 'SCC','E' ,'ECC', 'R', 'RCC', 'NODE')), @[nod_cd])" ).append("\n"); 
		query.append("                 )" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("             AND D.CNTR_TPSZ_CD = MAS_UT_TPSZ_FNC((SELECT COST_UT_AMT_CD FROM MAS_COST_SRC_ACCT WHERE MAS_COST_SRC_CD = D.MAS_COST_SRC_CD), REPLACE(@[f_cntr_tpsz_cd], 'RD', 'R'))" ).append("\n"); 
		query.append("            #end" ).append("\n"); 
		query.append("             					" ).append("\n"); 
		...
		query.append("        GROUP BY MAS_COST_SRC_CD, LOCL_CURR_CD) K" ).append("\n"); 
		query.append("       " ).append("\n"); 
		query.append("  WHERE 1=1" ).append("\n"); 
		...
		query.append("  #end" ).append("\n"); 
		query.append("   AND M.MAS_COST_SRC_CD = K.MAS_COST_SRC_CD" ).append("\n"); 
		query.append("GROUP BY M.COST_ACT_GRP_CD, M.STND_COST_CD, CUBE(K.MAS_COST_SRC_CD), K.LOCL_CURR_CD" ).append("\n"); 

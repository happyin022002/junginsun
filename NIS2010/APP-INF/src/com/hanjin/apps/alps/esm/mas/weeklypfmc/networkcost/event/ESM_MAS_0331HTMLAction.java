...
*@FileName : ESM_MAS_0331HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Jong-Ock KIM
 * @see EsmMas0331Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0331HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0331HTMLAction 객체를 생성
	 */
	public ESM_MAS_0331HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0331Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0331Event event = new EsmMas0331Event();  		
    	
    	if(command.isCommand(FormCommand.SEARCH)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
    	} else if(command.isCommand(FormCommand.MULTI)) {
    		event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setAverageUCVesselRegisterVOs((AverageUCVesselRegisterVO[])getVOs(request, AverageUCVesselRegisterVO.class));
		}

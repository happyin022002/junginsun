...
public class CsScreenDBDAOsearchBlInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       ,TO_CHAR(SKD.VPS_ETA_DT,'YYYY/MM/DD HH24:MI')                              AS VPS_ETB_DT    -- POD ATA" ).append("\n"); 
		...
		query.append("       ,PRD.NOD_NM                                                                AS DEL_NM" ).append("\n"); 
		...
		query.append("			 WHEN CGO.OBL_RDEM_FLG ='Y' THEN 'Y'" ).append("\n"); 
		query.append("             ELSE 'N' END                                                         AS OBL_RDEM_FLG             " ).append("\n"); 
		query.append("       ,NVL(CGO.OBL_RDEM_FLG,'N')                                                 AS US_OBL_RDEM_FLG      " ).append("\n"); 
		query.append("       ,TO_CHAR( DECODE( BKG.BL_TP_CD,'W',ISS.OBL_ISS_DT" ).append("\n"); 
		query.append("                                         ,ISS.OBL_RDEM_DT ),'YYYY-MM-DD HH24:MI') AS OBL_RDEM_DT      " ).append("\n"); 
		query.append("       ,DECODE(BKG.BL_TP_CD,'W',ISS.OBL_ISS_OFC_CD,ISS.OBL_RDEM_OFC_CD )          AS OBL_RDEM_OFC_CD     " ).append("\n"); 
		query.append("       ,SHP.CUST_CNT_CD || DECODE(SHP.CUST_SEQ,'0','',LPAD(SHP.CUST_SEQ,6,'0'))   AS SHP_CUST_CD" ).append("\n"); 
		query.append("       ,SHP.CUST_NM                                                               AS SHP_CUST_NM" ).append("\n"); 
		query.append("       ,CSG.CUST_CNT_CD || DECODE(CSG.CUST_SEQ,'0','',LPAD(CSG.CUST_SEQ,6,'0'))   AS CSG_CUST_CD" ).append("\n"); 
		query.append("       ,CSG.CUST_NM                                                               AS CSG_CUST_NM" ).append("\n"); 
		query.append("       ,NOY.CUST_CNT_CD || DECODE(NOY.CUST_SEQ,'0','',LPAD(NOY.CUST_SEQ,6,'0'))   AS NOY_CUST_CD" ).append("\n"); 
		query.append("       ,NOY.CUST_NM                                                               AS NOY_CUST_NM" ).append("\n"); 
		query.append("       ,AOY.CUST_CNT_CD || DECODE(AOY.CUST_SEQ,'0','',LPAD(AOY.CUST_SEQ,6,'0'))   AS AOY_CUST_CD" ).append("\n"); 
		...
		query.append("         WHERE CLT_BL_NO = @[bkg_no]" ).append("\n"); 
		query.append("               AND OTS_BND_TP_CD = 'I'" ).append("\n"); 
		query.append("               AND  OTS_TJ_SEQ = (" ).append("\n"); 
		query.append("                         SELECT MAX(OTS_TJ_SEQ) " ).append("\n"); 
		query.append("                         FROM BKG_OUTSTANDING" ).append("\n"); 
		query.append("                         WHERE CLT_BL_NO = @[bkg_no]" ).append("\n"); 
		query.append("                           AND OTS_BND_TP_CD = 'I'" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("         AND ROWNUM = 1),'') AS COLL_OFC_CD -- 최종 I/F 된 ofc " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("       LEFT OUTER JOIN PRD_NODE PRD" ).append("\n"); 
		query.append("       ON ( PRD.NOD_CD   = BKG.DEL_NOD_CD" ).append("\n"); 
		query.append("            AND PRD.DELT_FLG = 'N') " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("       LEFT OUTER JOIN BKG_CSTMS_ADV_IBD IBD ON ( CNT_CD = 'US' AND IBD.BL_NO = BKG.BL_NO )" ).append("\n"); 

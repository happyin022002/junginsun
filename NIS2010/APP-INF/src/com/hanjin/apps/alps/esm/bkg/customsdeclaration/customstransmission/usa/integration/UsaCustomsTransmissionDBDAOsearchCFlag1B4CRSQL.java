...
public class UsaCustomsTransmissionDBDAOsearchCFlag1B4CRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       )" ).append("\n"); 
		query.append("      ,(" ).append("\n"); 
		query.append("        SELECT SUM(DECODE(DSPO_CD, '4A', CNTR_QTY, 0)) AS DSPO_CD_4A_QTY" ).append("\n"); 
		query.append("              ,SUM(DECODE(DSPO_CD, '1B', CNTR_QTY, DECODE(DSPO_CD, '4C', CNTR_QTY, 0))) AS DSPO_CD_1B4C_QTY" ).append("\n"); 
		query.append("          FROM BKG_CSTMS_ADV_RSLT" ).append("\n"); 
		query.append("         WHERE CNT_CD = 'US'" ).append("\n"); 
		query.append("           AND BL_NO  = @[bl_no]" ).append("\n"); 
		query.append("           AND NVL(RSND_FLG,'N') <> 'Y'" ).append("\n"); 

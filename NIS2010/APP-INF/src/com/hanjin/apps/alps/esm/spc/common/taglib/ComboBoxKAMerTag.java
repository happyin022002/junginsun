 ...
 * @since J2EE 1.4
 */
public class ComboBoxKAMerTag extends ComboBoxTag {
	private static transient org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ComboBoxKAMerTag.class);
	String showCol = "";
	...
	protected String[][] getOptionList() throws EventException {
		...
		String[][] datas = null;
		try {
			response = (ComboxEventResponse) sc.searchKAMerComboList(getDel());
			DBRowSet rs = response.getRs();
			datas = new String[rs.getRowCount()][2];
			int idx = 0;
			while (rs.next()) {
				datas[idx][0] = rs.getString("cd");
				datas[idx][1] = rs.getString("cd") + "|" + rs.getString("nm");
				idx++;
			}
		} catch (Exception e) {

...
*@FileName : RFAProposalMainDBDAOSearchProposalApprovalListVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFAProposalMainDBDAOSearchProposalApprovalListVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * RFAProposalMainDBDAO + SearchProposalApprovalListVORSQL
	  * </pre>
	  */
	public RFAProposalMainDBDAOSearchProposalApprovalListVORSQL(){
		setQuery();
		...
		query.append("FileName : RFAProposalMainDBDAOSearchProposalApprovalListVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	  , MN.PROP_PROG_SEQ																	 AS PROP_PROG_SEQ" ).append("\n"); 
		query.append("     -----------------------------------------------------------------------------------------------------------------------" ).append("\n"); 
		...
		query.append("         WHERE EMPE_CD = (SELECT PROG_USR_ID" ).append("\n"); 
		...
		query.append("                   AND PROP_PROG_SEQ = MN.PROP_PROG_SEQ - 1 " ).append("\n"); 
		...
		query.append("     ----------------------------------------------------------------------------------------------------------------------- " ).append("\n"); 
		query.append("     , MN.PROG_USR_ID                                                                        AS PROP_APRO_STAFF_CD" ).append("\n"); 
		query.append("     , (SELECT C.USR_NM" ).append("\n"); 
		query.append("          FROM COM_USER C" ).append("\n"); 
		query.append("         WHERE C.USR_ID = MN.PROG_USR_ID)                                                    AS PROP_APRO_STAFF_NM" ).append("\n"); 
		query.append("     , MN.PROG_OFC_CD                                                                        AS PROP_APRO_OFC_CD" ).append("\n"); 
		query.append("     , TO_CHAR (MN.PROG_DT, 'YYYYMMDD')                                                      AS PROP_APRO_DT       " ).append("\n"); 
		query.append("     -----------------------------------------------------------------------------------------------------------------------" ).append("\n"); 
		...
		query.append("      , (" ).append("\n"); 
		query.append("            SELECT MN.*" ).append("\n"); 
		query.append("              FROM " ).append("\n"); 
		query.append("                  (" ).append("\n"); 
		query.append("                    SELECT MN.*" ).append("\n"); 
		query.append("                          , PROG.PROP_PROG_SEQ" ).append("\n"); 
		query.append("                          , PROG.PROG_USR_ID" ).append("\n"); 
		query.append("                          , PROG.PROG_OFC_CD" ).append("\n"); 
		query.append("                          , PROG.PROG_DT" ).append("\n"); 
		query.append("                          , ROW_NUMBER () OVER(PARTITION BY PROG.PROP_NO, PROG.AMDT_SEQ ORDER BY PROG.PROP_NO, PROG.AMDT_SEQ , PROG.PROP_PROG_SEQ DESC) AS RNUM" ).append("\n"); 
		query.append("                      FROM (" ).append("\n"); 
		query.append("                            SELECT MN.*" ).append("\n"); 
		...
		query.append("                               AND MN.PROP_APRO_DT <= TO_DATE(@[exp_dt], 'YYYY-MM-DD') + 0.99999" ).append("\n"); 
		query.append("                               AND MN.PROP_APRO_DT >= TO_DATE(@[eff_dt], 'YYYY-MM-DD') " ).append("\n"); 
		query.append("							#end" ).append("\n"); 
		query.append("-------------------------------------------------------------------------------------------------------  기간 조회 종료" ).append("\n"); 
		query.append("                               AND MN.PROP_STS_CD IN ('A','R')             -- 현재 계약중 승인 건" ).append("\n"); 
		...
		query.append("                         , PRI_RP_PROG PROG" ).append("\n"); 
		query.append("                     WHERE MN.PROP_NO = PROG.PROP_NO" ).append("\n"); 
		query.append("                       AND MN.AMDT_SEQ = PROG.AMDT_SEQ" ).append("\n"); 
		query.append("                       AND PROG.PROP_STS_CD IN ('A','R')           -- 다건의 승인 건" ).append("\n"); 
		query.append("                   ) MN" ).append("\n"); 
		query.append("               WHERE RNUM < 2      -- 현재 계약의 승인 건 중 다건 중 최근 승인 건" ).append("\n"); 
		query.append("         ) MN" ).append("\n"); 
		...
		query.append("          WHERE PROP_NO = MN.PROP_NO" ).append("\n"); 
		query.append("            AND AMDT_SEQ = MN.AMDT_SEQ" ).append("\n"); 
		query.append("            AND PROP_PROG_SEQ = MN.PROP_PROG_SEQ - 1 ) = @[prop_ofc_cd]		-- Request Office" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("	AND MN.PROG_USR_ID IN (SELECT USR_ID FROM COM_USER WHERE UPPER(USR_NM) LIKE UPPER('%'||@[prop_apro_staff_cd]||'%'))		-- PROP_APRO_STAFF_CD " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${prop_apro_ofc_cd} != '')" ).append("\n"); 
		query.append("	AND MN.PROG_OFC_CD = @[prop_apro_ofc_cd]			-- Approval Office" ).append("\n"); 
		query.append("#end" ).append("\n"); 

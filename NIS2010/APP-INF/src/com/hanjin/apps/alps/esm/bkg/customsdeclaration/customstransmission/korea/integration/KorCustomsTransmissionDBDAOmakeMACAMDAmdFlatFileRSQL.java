...
public class KorCustomsTransmissionDBDAOmakeMACAMDAmdFlatFileRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#if (${kr_cstms_corr_id} == '03' && (${cstms_decl_tp_cd} == 'R' || ${cstms_decl_tp_cd} == 'T'))       " ).append("\n"); 
		...
		query.append("        DECODE(@[kr_cstms_corr_id],'01','A','02','B','03','D','04','F',@[kr_cstms_corr_id]) ||'~'||     /*  Correction Code     */" ).append("\n"); 
		...
		query.append("        DECODE(@[kr_meas_ut_cd],'M','6Q8','B','BLL',@[kr_meas_ut_cd])    ||'~'||     /*  용적톤 단위         */" ).append("\n"); 
		...
		query.append("		BKG_SPCLCHAR_CONV_FNC(SUBSTR(@[n_cust_nm],36,35),'Y')      ||'~'||     /*  NTFY Name 1         */" ).append("\n"); 
		query.append("		BKG_SPCLCHAR_CONV_FNC(SUBSTR(@[n_cust_nm],71,35),'Y')      ||'~'||     /*  NTFY Name 1         */" ).append("\n"); 
		...
		query.append("        ''                          ||'~'||	/*	NTFY Address4*/" ).append("\n"); 
		query.append("		''							||'~'||	/*	NTFY Address5 */" ).append("\n"); 
		query.append("        (SELECT NVL(L.UN_LOC_CD, L.LOC_CD)" ).append("\n"); 
		query.append("         FROM MDM_CUSTOMER C, MDM_LOCATION L" ).append("\n"); 

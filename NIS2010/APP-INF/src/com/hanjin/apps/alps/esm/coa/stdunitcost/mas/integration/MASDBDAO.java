...
public class MASDBDAO extends DBDAOSupport {
	 ...
	 * [처리대상] 정보를 [행위] 합니다.<br>
	 * 
	 * @param List<CoaSvcTrnsPrcVO> updModels
	 * @return int[]
	 * @throws DAOException
	 * @throws Exception
	 */
	public int[] modifyMAS0012(List<CoaSvcTrnsPrcVO> updModels) throws DAOException,Exception {
		int updCnt[] = null;
		try {			
			SQLExecuter sqlExe = new SQLExecuter("");
			if(updModels.size() > 0){
				updCnt = sqlExe.executeBatch((ISQLTemplate)new MASDBDAOCoaSvcTrnsPrcVOUSQL(), updModels,null);
	...
	public void removeAbcStpCost(HashMap<String, String> map) throws DAOException {
        ...
        try{
        	velParam.put("table_name", "COA_SVC_TRNS_PRC");
        	...
        	velParam.put("table_name", "COA_SVC_TRNS_OFC_MAPG");
        	saveCnt = new SQLExecuter("").executeUpdate((ISQLTemplate)new MASDBDAORemoveAbcStpCostDSQL(), map, velParam);
            if(saveCnt== Statement.EXECUTE_FAILED)
                throw new DAOException("Fail to modify SQL");  
            
        	velParam.put("table_name", "COA_OFC_ROUT_MAPG");
        	saveCnt = new SQLExecuter("").executeUpdate((ISQLTemplate)new MASDBDAORemoveAbcStpCostDSQL(), map, velParam);
            if(saveCnt== Statement.EXECUTE_FAILED)
                throw new DAOException("Fail to modify SQL");
            
        	velParam.put("table_name", "COA_SVC_TRNS_PRC_MGN");
        	saveCnt = new SQLExecuter("").executeUpdate((ISQLTemplate)new MASDBDAORemoveAbcStpCostDSQL(), map, velParam);
            if(saveCnt== Statement.EXECUTE_FAILED)
                throw new DAOException("Fail to modify SQL");            

...
public class AGNCommRequestDBDAOAddAcmSpclCmpnCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    (SELECT AR_OFC_CD FROM MDM_ORGANIZATION WHERE OFC_CD = SPCL_OFC_CD) AS AR_OFC_CD," ).append("\n"); 
		query.append("    (SELECT AP_OFC_CD FROM MDM_ORGANIZATION WHERE OFC_CD = SPCL_OFC_CD) AS AP_OFC_CD,    " ).append("\n"); 
		query.append("    (SELECT AP_CTR_CD FROM MDM_ORGANIZATION WHERE OFC_CD = SPCL_OFC_CD) AS AP_CTR_CD," ).append("\n"); 
		...
		query.append("            AND FRT_INCL_XCLD_DIV_CD = 'N' " ).append("\n"); 
		query.append("            AND CURR_CD              = 'USD' " ).append("\n"); 
		query.append("            AND BRT.CHG_CD IN ( 'OFT', 'GRI' ) " ).append("\n"); 
		query.append("            ) " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        ROW_NUMBER() OVER (PARTITION BY INFO.BKG_NO, SPCL_OFC_CD, NVL(INFO.SHPR,'*'), NVL(POR_GRP_TP_CD,'*'), NVL(POL_GRP_TP_CD,'*'), NVL(POD_GRP_TP_CD,'*'), NVL(DEL_GRP_TP_CD,'*') " ).append("\n"); 
		query.append("                     ORDER BY DECODE(NVL(CMDT_TP_CD,'*'),'*','0',CMDT_TP_CD) DESC ) CMDT_DUP_CHK," ).append("\n"); 
		query.append("        ROW_NUMBER() OVER (PARTITION BY INFO.BKG_NO, SPCL_OFC_CD ,NVL(CMDT_TP_CD,'*'),NVL(POR_GRP_TP_CD,'*'), NVL(POL_GRP_TP_CD,'*'), NVL(POD_GRP_TP_CD,'*'), NVL(DEL_GRP_TP_CD,'*')" ).append("\n"); 
		query.append("                     ORDER BY DECODE(NVL(INFO.SHPR,'*'),'*','0',INFO.SHPR) DESC ) SHPR_DUP_CHK," ).append("\n"); 
		query.append("        ROW_NUMBER() OVER (PARTITION BY INFO.BKG_NO, SPCL_OFC_CD ,NVL(CMDT_TP_CD,'*'),NVL(INFO.SHPR,'*'), NVL(POL_GRP_TP_CD,'*'), NVL(POD_GRP_TP_CD,'*'), NVL(DEL_GRP_TP_CD,'*')" ).append("\n"); 
		query.append("                     ORDER BY DECODE(NVL(POR_GRP_TP_CD,'*'),'*','0',POR_GRP_TP_CD) DESC ) POR_DUP_CHK," ).append("\n"); 
		query.append("        ROW_NUMBER() OVER (PARTITION BY INFO.BKG_NO, SPCL_OFC_CD ,NVL(CMDT_TP_CD,'*'),NVL(INFO.SHPR,'*'), NVL(POR_GRP_TP_CD,'*'), NVL(POD_GRP_TP_CD,'*'), NVL(DEL_GRP_TP_CD,'*')" ).append("\n"); 
		query.append("                     ORDER BY DECODE(NVL(POL_GRP_TP_CD,'*'),'*','0',POL_GRP_TP_CD) DESC ) POL_DUP_CHK," ).append("\n"); 
		query.append("        ROW_NUMBER() OVER (PARTITION BY INFO.BKG_NO, SPCL_OFC_CD ,NVL(CMDT_TP_CD,'*'),NVL(INFO.SHPR,'*'), NVL(POR_GRP_TP_CD,'*'), NVL(POL_GRP_TP_CD,'*'), NVL(DEL_GRP_TP_CD,'*')" ).append("\n"); 
		...
		query.append("                     ORDER BY DECODE(NVL(DEL_GRP_TP_CD,'*'),'*','0',DEL_GRP_TP_CD) DESC ) DEL_DUP_CHK" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("    FROM" ).append("\n"); 
		...
		query.append("        MAS_RGST_BKG            RGST," ).append("\n"); 
		query.append("        (" ).append("\n"); 
		query.append("            SELECT" ).append("\n"); 
		...
		query.append("    AND NVL(AGMT.DELT_FLG ,'N') = 'N'" ).append("\n"); 
		...
		query.append("    END " ).append("\n"); 
		query.append("/* DEL Location 해당 ROUTE별로 존재하는지 */ " ).append("\n"); 
		query.append("    AND  NVL (AGMT.DEL_ROUT_CD  , '*') = " ).append("\n"); 
		query.append("    CASE NVL (AGMT.DEL_GRP_TP_CD, '*') " ).append("\n"); 
		query.append("         WHEN '*' THEN '*' " ).append("\n"); 
		query.append("         WHEN '5' THEN INFO.DEL_CD " ).append("\n"); 
		query.append("         WHEN '4' THEN INFO.DEL_RGN_CD " ).append("\n"); 
		query.append("         WHEN '3' THEN INFO.DEL_CNT_CD " ).append("\n"); 
		query.append("         WHEN '2' THEN INFO.DEL_SCONTI_CD " ).append("\n"); 
		query.append("         WHEN '1' THEN INFO.DEL_CONTI_CD " ).append("\n"); 
		query.append("    END " ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("WHERE 1=1" ).append("\n"); 
		...
		query.append("AND DEL_DUP_CHK  = 1" ).append("\n"); 

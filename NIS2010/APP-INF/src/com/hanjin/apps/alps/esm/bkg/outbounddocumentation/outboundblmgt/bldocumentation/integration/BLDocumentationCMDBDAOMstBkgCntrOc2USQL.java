...
*@FileName : BLDocumentationCMDBDAOMstBkgCntrOc2USQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BLDocumentationCMDBDAOMstBkgCntrOc2USQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public BLDocumentationCMDBDAOMstBkgCntrOc2USQL(){
		setQuery();
		...
		query.append("FileName : BLDocumentationCMDBDAOMstBkgCntrOc2USQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       CTM.CNMV_CYC_NO, CTM.MVMT_STS_CD, DECODE(CTM.BKG_RCV_TERM_CD,'M', NULL, CTM.BKG_RCV_TERM_CD) BKG_RCV_TERM_CD, CTM.ORG_YD_CD," ).append("\n"); 
		...
		query.append("       CTM.SUBST_RULE_CD, DECODE(BKG.DE_TERM_CD,'M', NULL, BKG.DE_TERM_CD) DE_TERM_CD, CTM.CRE_USR_ID, CTM.UPD_USR_ID," ).append("\n"); 
		query.append("       BKG.POR_CD, CTM.VGM_WGT_QTY, CTM.VGM_WGT_UT_CD, CTM.VGM_SIG_CTNT, CTM.VGM_MZD_TP_CD" ).append("\n"); 
		...
		query.append("      ,A.DIFF_RMK     = B.CNMV_RMK" ).append("\n"); 
		...
		query.append("--      ,A.CGO_RCV_DT = TO_DATE(DECODE(A.CGO_RCV_DT, NULL, TO_CHAR(B.CNMV_EVNT_DT, 'YYYYMMDD HH24:MI:SS')" ).append("\n"); 
		query.append("--						        , TO_CHAR(A.CGO_RCV_DT, 'YYYYMMDD HH24:MI:SS')),'YYYYMMDD HH24:MI:SS')" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("#if(${vgm_upd_flg} == 'Y')" ).append("\n"); 
		query.append("	  ,A.VGM_WGT       =  B.VGM_WGT_QTY" ).append("\n"); 
		query.append("      ,A.VGM_WGT_UT_CD = DECODE(B.VGM_WGT_UT_CD,'KGM','KGS','LBR','LBS',B.VGM_WGT_UT_CD)" ).append("\n"); 
		query.append("      ,A.VGM_MZD_TP_CD = B.VGM_MZD_TP_CD" ).append("\n"); 
		query.append("      ,A.VGM_VRFY_SIG_CTNT = B.VGM_SIG_CTNT" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("#if (${mvmt_sts_cd} == 'OC' && ${vgm_ins_flg} == 'Y')" ).append("\n"); 
		query.append("								 VGM_WGT, VGM_WGT_UT_CD, VGM_VRFY_SIG_CTNT, VGM_MZD_TP_CD,  " ).append("\n"); 
		query.append("#end     " ).append("\n"); 
		...
		query.append("                           DECODE(CTM.BKG_RCV_TERM_CD,'M', NULL, CTM.BKG_RCV_TERM_CD) BKG_RCV_TERM_CD, CTM.ORG_YD_CD, CTM.DEST_YD_CD, 'Y', CTM.CNMV_RMK, " ).append("\n"); 
		query.append("                           DECODE(CTM.SUBST_RULE_CD, NULL, 'N', 'Y') RL, CTM.CNMV_EVNT_DT, CTM.SUBST_RULE_CD,  DECODE(BKG.DE_TERM_CD,'M', NULL, BKG.DE_TERM_CD) DE_TERM_CD, " ).append("\n"); 
		query.append("                           CTM.CRE_DT, CTM.UPD_DT, CTM.CRE_USR_ID, CTM.UPD_USR_ID," ).append("\n"); 
		query.append("#if (${mvmt_sts_cd} == 'OC' && ${vgm_ins_flg} == 'Y')" ).append("\n"); 
		query.append("                           CTM.VGM_WGT_QTY, DECODE(CTM.VGM_WGT_UT_CD,'KGM','KGS','LBR','LBS',CTM.VGM_WGT_UT_CD), " ).append("\n"); 
		query.append("                           CTM.VGM_SIG_CTNT,CTM.VGM_MZD_TP_CD," ).append("\n"); 
		query.append("#end " ).append("\n"); 
		query.append("#if (${mvmt_sts_cd} == 'OC')  " ).append("\n"); 
		...
		query.append("#if (${mvmt_sts_cd} == 'ID')" ).append("\n"); 
		query.append("   AND BKG.BKG_NO = (CASE WHEN NVL(BKG.CNTR_PRT_FLG,'N') = 'N' THEN @[bkg_no] ELSE BKG.BKG_NO END)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                    ) B" ).append("\n"); 
		query.append("ON (A.BKG_NO = B.BKG_NO AND A.CNTR_NO = B.CNTR_NO)" ).append("\n"); 
		query.append("WHEN MATCHED THEN " ).append("\n"); 

...
*@FileName : ESM_BKG_0705HTMLAction.java
...
public class ESM_BKG_0705HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0705HTMLAction 객체를 생성
	 */
	public ESM_BKG_0705HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0705Event event = new EsmBkg0705Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setCODBookingListInVO((CODBookingListInVO)getVO(request, CODBookingListInVO.class));
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setDiversionCAVO((DiversionCAVO)getVO(request, DiversionCAVO.class));
		}
        else if(command.isCommand(FormCommand.MULTI01)) {
			event.setCODBookingListOutVOs((CODBookingListOutVO[])getVOs(request, CODBookingListOutVO.class, ""));
		}

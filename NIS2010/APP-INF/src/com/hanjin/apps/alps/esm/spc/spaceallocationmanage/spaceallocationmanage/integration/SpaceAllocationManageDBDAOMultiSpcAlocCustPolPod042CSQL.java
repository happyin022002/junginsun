...
*@FileName : SpaceAllocationManageDBDAOMultiSpcAlocCustPolPod042CSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SpaceAllocationManageDBDAOMultiSpcAlocCustPolPod042CSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SPC_ALOC_CUST_POL_POD의 지정된 ibflag 값에 따라 DB에 반영한다.(입력/수정)
	  * [CHM-201322502-01] SPC 프로젝트 - 성수기 선복운영 개선을 위한 T/F추진
	  * 2013.08.13 진마리아 [trouble shooting] 성수기 HO 저장시 하위점소가 없는 경우 Sub Alloc 자동 저장 되도록 수정(update시 빠진 것 보완-nvl처리)
	  * </pre>
	  */
	public SpaceAllocationManageDBDAOMultiSpcAlocCustPolPod042CSQL(){
		setQuery();
		...
		query.append("FileName : SpaceAllocationManageDBDAOMultiSpcAlocCustPolPod042CSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                  Z.SPC_CTRL_ALOC_RMK    ," ).append("\n"); 
		query.append("                  Z.SPC_CTRL_ALOC_POL_RMK," ).append("\n"); 
		query.append("                  Z.SPC_CTRL_ALOC_POD_RMK," ).append("\n"); 
		...
		query.append("                            P.SPC_CTRL_ALOC_RMK    ," ).append("\n"); 
		query.append("                            P.SPC_CTRL_ALOC_POL_RMK," ).append("\n"); 
		query.append("                            P.SPC_CTRL_ALOC_POD_RMK," ).append("\n"); 
		...
		query.append("           T.BKG_AVAL_TTL_QTY      = NVL(C.BKG_AVAL_TTL_QTY ,    T.BKG_AVAL_TTL_QTY    )," ).append("\n"); 
		query.append("           T.BKG_AVAL_20FT_QTY     = NVL(C.BKG_AVAL_20FT_QTY,    T.BKG_AVAL_20FT_QTY   )," ).append("\n"); 
		query.append("           T.BKG_AVAL_40FT_QTY     = NVL(C.BKG_AVAL_40FT_QTY,    T.BKG_AVAL_40FT_QTY   )," ).append("\n"); 
		query.append("           T.BKG_AVAL_40FT_HC_QTY  = NVL(C.BKG_AVAL_40FT_HC_QTY, T.BKG_AVAL_40FT_HC_QTY)," ).append("\n"); 
		query.append("           T.BKG_AVAL_45FT_HC_QTY  = NVL(C.BKG_AVAL_45FT_HC_QTY, T.BKG_AVAL_45FT_HC_QTY)," ).append("\n"); 
		query.append("           T.BKG_AVAL_53FT_QTY     = NVL(C.BKG_AVAL_53FT_QTY,    T.BKG_AVAL_53FT_QTY   )," ).append("\n"); 
		query.append("           T.BKG_AVAL_RF_QTY       = NVL(C.BKG_AVAL_RF_QTY  ,    T.BKG_AVAL_RF_QTY     )," ).append("\n"); 
		query.append("           T.BKG_AVAL_TTL_WGT      = NVL(C.BKG_AVAL_TTL_WGT ,    T.BKG_AVAL_TTL_WGT    )," ).append("\n"); 
		...
		query.append("           --T.CUST_CNT_CD           = C.CUST_CNT_CD          , " ).append("\n"); 
		query.append("           --T.CUST_SEQ              = C.CUST_SEQ             , " ).append("\n"); 
		...
		query.append("           T.BKG_AVAL_20FT_DRY_QTY = NVL(C.BKG_AVAL_20FT_DRY_QTY , T.BKG_AVAL_20FT_DRY_QTY ),  " ).append("\n"); 
		query.append("	   T.BKG_AVAL_40FT_DRY_QTY = NVL(C.BKG_AVAL_40FT_DRY_QTY, T.BKG_AVAL_40FT_DRY_QTY),  " ).append("\n"); 
		query.append("	   T.BKG_AVAL_RD_QTY       = NVL(C.BKG_AVAL_RD_QTY,    T.BKG_AVAL_RD_QTY)          " ).append("\n"); 

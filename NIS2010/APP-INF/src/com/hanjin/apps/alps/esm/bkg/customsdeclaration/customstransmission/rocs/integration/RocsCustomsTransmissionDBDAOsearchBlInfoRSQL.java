...
public class RocsCustomsTransmissionDBDAOsearchBlInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      SELECT A.VSL_CD||A.SKD_VOY_NO||A.SKD_DIR_CD||'-'||B.BL_NO HEAD_CD, " ).append("\n"); 
		...
		query.append("               TO_CHAR(B.ETA_DT,'yyyymmdd') POD_ETA," ).append("\n"); 
		query.append("			   X.LOC_CD POL," ).append("\n"); 
		query.append("--               DECODE(B.PRE_RLY_PORT_CD,null,BKG_RTM_BL_POL_LOC.UN_LOC_CD,' ',BKG_RTM_BL_POL_LOC.UN_LOC_CD,BKG_RTM_BL_PRE_RLY.UN_LOC_CD) POL," ).append("\n"); 
		...
		query.append("               'SM LINE CORPORATION' SENDER_CD," ).append("\n"); 
		query.append("               'NL24182360' CARRIER_ID," ).append("\n"); 
		query.append("               'SM LINE CORPORATION' CARRIER," ).append("\n"); 
		query.append("				B.SHPR_EORI_NO SHPR_EORI," ).append("\n"); 
		query.append("                REPLACE(SUBSTR(B.SHPR_ADDR,1,INSTR(B.SHPR_ADDR,@[dif_char])-1),CHR(13)||CHR(10),' ') SHPR_NAME," ).append("\n"); 
		...
		query.append("                REPLACE(SUBSTR(B.SHPR_ADDR,INSTR(B.SHPR_ADDR,@[dif_char])+2,LENGTH(B.SHPR_ADDR)-instr(B.SHPR_ADDR,@[dif_char])+1),CHR(13)||CHR(10),' ') SHPR_ADDR," ).append("\n"); 
		query.append("				B.CNEE_EORI_NO CNEE_EORI," ).append("\n"); 
		query.append("                REPLACE(SUBSTR(B.CNEE_ADDR,1,INSTR(B.CNEE_ADDR,@[dif_char])-1),CHR(13)||CHR(10),' ') CNEE_NAME," ).append("\n"); 
		...
		query.append("                REPLACE(SUBSTR(B.CNEE_ADDR,INSTR(B.CNEE_ADDR,@[dif_char])+2,LENGTH(B.CNEE_ADDR)-instr(B.CNEE_ADDR,@[dif_char])+1),CHR(13)||CHR(10),' ') CNEE_ADDR," ).append("\n"); 
		query.append("				B.NTFY_EORI_NO NTFY_EORI," ).append("\n"); 
		query.append("                REPLACE(SUBSTR(B.NTFY_ADDR,1,INSTR(B.NTFY_ADDR,@[dif_char])-1),CHR(13)||CHR(10),' ') NTFY_NAME," ).append("\n"); 
		...
		query.append("                REPLACE(SUBSTR(B.NTFY_ADDR,INSTR(B.NTFY_ADDR,@[dif_char])+2,LENGTH(B.NTFY_ADDR)-instr(B.NTFY_ADDR,@[dif_char])+1),CHR(13)||CHR(10),' ') NTFY_ADDR," ).append("\n"); 
		query.append("               B.VSL_CALL_REF_NO," ).append("\n"); 
		query.append("               B.BKG_NO, " ).append("\n"); 
		query.append("               A.VSL_CD,A.SKD_VOY_NO,A.SKD_DIR_CD" ).append("\n"); 
		query.append("          FROM BKG_CSTMS_RTM_VSL A,BKG_CSTMS_RTM_BL B,MDM_VSL_CNTR C, MDM_LOCATION LR, MDM_LOCATION LD, " ).append("\n"); 
		...
		query.append("                  WHERE POD_CD 		= 'NLRTM') X" ).append("\n"); 
		...
		query.append("	   AND B.BL_NO = SUBSTR(@[bl_no],0,12)" ).append("\n"); 
		...
		query.append("    (SELECT VSL_CD,SKD_VOY_NO,SKD_DIR_CD, BL_NO, POD_CD, MVMT_REF_NO  FROM BKG_CSTMS_EUR_BL WHERE POD_CD = 'NLRTM' )E" ).append("\n"); 

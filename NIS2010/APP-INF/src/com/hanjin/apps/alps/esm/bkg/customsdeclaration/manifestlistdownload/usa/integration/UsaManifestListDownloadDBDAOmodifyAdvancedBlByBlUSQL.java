...
public class UsaManifestListDownloadDBDAOmodifyAdvancedBlByBlUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		-- B.POD,B.DEL 같으면 B.POD([TO-BE] 2009.10.23 로직 추가), 아니면 HUB(2010.01.15 AS-IS 로직 반영)                                                                                                                                         " ).append("\n"); 
		query.append("		,CSTMS_LOC_CD    = DECODE(@[cstms_loc_cd], NULL, @[hub_loc_cd], @[cstms_loc_cd])" ).append("\n"); 
		query.append("		-- [TO-BE] 2009.09.11 FROB_FLG 요건 추가" ).append("\n"); 
		query.append("		,FROB_FLG        = DECODE(SUBSTR(@[cstms_pod_cd],0,2), 'CA', 'Y', 'N')                                                               " ).append("\n"); 
		...
		query.append("		,CA_FLG          = NVL(@[ca_flg],'N')                                                                                                                                             " ).append("\n"); 
		...
		query.append("		,IF_DT           = GLOBALDATE_PKG.TIME_CONV_FNC('KRACY', SYSDATE, 'USNYC')                                                                                              " ).append("\n"); 
		...
		query.append("    	,ACT_FILE_SKD_DIR_CD = CASE WHEN SUBSTR(@[cstms_pol_cd],0,2) = 'CA' AND @[act_file_skd_dir_cd] IS NOT NULL THEN @[act_file_skd_dir_cd]" ).append("\n"); 

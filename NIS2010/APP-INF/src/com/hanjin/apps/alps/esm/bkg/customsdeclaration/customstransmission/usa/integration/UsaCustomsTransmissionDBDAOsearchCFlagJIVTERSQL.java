...
public class UsaCustomsTransmissionDBDAOsearchCFlagJIVTERSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                 = SUM(DECODE(TB.ENTR_NO, 'SML', TB.J_QTY, 0)) THEN 'J'" ).append("\n"); 
		...
		query.append("                 AND DECODE(@[icr_code], '54', DECODE(BL.MF_STS_CD, 'D', 0, BL.PCK_QTY), BL.PCK_QTY)" ).append("\n"); 
		query.append("                     = SUM(DECODE(TB.ENTR_NO, 'SML', DECODE(TB.ENTR_TP_NO, '62', TB.J_QTY, 0), 0)) THEN 'T'" ).append("\n"); 
		query.append("            WHEN BL.POD_CD = @[cus_loc]" ).append("\n"); 
		query.append("                 AND DECODE(@[icr_code], '54', DECODE(BL.MF_STS_CD, 'D', 0, BL.PCK_QTY), BL.PCK_QTY)" ).append("\n"); 
		query.append("                     = SUM(DECODE(TB.ENTR_NO, 'SML', DECODE(TB.ENTR_TP_NO, '63', TB.J_QTY, 0), 0)) THEN 'E'" ).append("\n"); 
		...
		query.append("              ,DECODE(SUBSTR(RS.ENTR_NO, 1, 3), 'V5N', 'SML', 'ETC') AS ENTR_NO" ).append("\n"); 
		...
		query.append("      GROUP BY DECODE(SUBSTR(RS.ENTR_NO, 1, 3), 'V5N', 'SML', 'ETC'), ENTR_TP_NO" ).append("\n"); 
		...
		query.append("GROUP BY BL.BL_NO, BL.PCK_QTY, BL.USA_LST_LOC_CD, BL.POD_CD, BL.MF_STS_CD" ).append("\n"); 

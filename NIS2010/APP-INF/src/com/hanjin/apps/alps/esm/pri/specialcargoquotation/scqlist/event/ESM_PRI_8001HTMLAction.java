...
*@FileName : ESM_PRI_8001HTMLAction.java
...
public class ESM_PRI_8001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_8001HTMLAction 객체를 생성
	 */
	public ESM_PRI_8001HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri8001Event event = new EsmPri8001Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setPriScqAwkBbVOS((PriScqAwkBbVO[])getVOs(request, PriScqAwkBbVO .class,""));
		} else if(command.isCommand(FormCommand.SEARCH01) || command.isCommand(FormCommand.SEARCH02)) {
			event.setPriScqAwkBbVO((PriScqAwkBbVO)getVO(request, PriScqAwkBbVO .class));
		}

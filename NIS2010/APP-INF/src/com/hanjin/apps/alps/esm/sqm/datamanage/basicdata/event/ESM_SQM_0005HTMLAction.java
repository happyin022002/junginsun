...
*@FileName       : ESM_SQM_0005HTMLAction.java
 ...
 * @author JEONGMIN CHO
 * @see EsmSqm0005Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0005HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SQM_0005HTMLAction 객체를 생성
	 */
	public ESM_SQM_0005HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0005Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmSqm0005Event event = new EsmSqm0005Event();
		...
		if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH02)) {
			...
			event.setSqmQtaTgtVvdVOS((SqmQtaTgtVvdVO[])getVOs(request, SqmQtaTgtVvdVO .class,""));
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		} else if(command.isCommand(FormCommand.MULTI01) || command.isCommand(FormCommand.SEARCH01)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setVbpIfFlg(JSPUtil.getParameter(request,"vbp_if_flg"));	
		}

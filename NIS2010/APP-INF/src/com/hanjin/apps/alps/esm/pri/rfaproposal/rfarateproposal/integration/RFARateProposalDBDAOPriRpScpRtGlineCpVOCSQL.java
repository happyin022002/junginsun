...
*@FileName : RFARateProposalDBDAOPriRpScpRtGlineCpVOCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateProposalDBDAOPriRpScpRtGlineCpVOCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public RFARateProposalDBDAOPriRpScpRtGlineCpVOCSQL(){
		setQuery();
		...
		query.append("FileName : RFARateProposalDBDAOPriRpScpRtGlineCpVOCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      ,@[amdt_seq]" ).append("\n"); 
		query.append("      ,@[svc_scp_cd]" ).append("\n"); 
		query.append("      ,DENSE_RANK() OVER(PARTITION BY D.SVC_SCP_CD, D.GLINE_SEQ ORDER BY D.CMDT_HDR_SEQ) AS NEW_CMDT_HDR_SEQ" ).append("\n"); 
		query.append("      ,DENSE_RANK() OVER(PARTITION BY D.SVC_SCP_CD, D.GLINE_SEQ, D.CMDT_HDR_SEQ ORDER BY D.ROUT_SEQ) AS NEW_ROUT_SEQ" ).append("\n"); 
		query.append("      ,DENSE_RANK() OVER(PARTITION BY D.SVC_SCP_CD, D.GLINE_SEQ, D.CMDT_HDR_SEQ, D.ROUT_SEQ ORDER BY D.RT_SEQ) AS NEW_RT_SEQ" ).append("\n"); 
		...
		query.append("          FROM PRI_RG_RT) D" ).append("\n"); 

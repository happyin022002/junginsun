 ...
 *@FileName : ESM_BKG_0751HTMLAction.java
 ...
 * @see EsmBkg0751Event 참조
 * @since J2EE 1.4
 */
public class ESM_BKG_0751HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = 1L;

	/**
	 * ESM_BKG_0751HTMLAction 객체를 생성
	 */
	public ESM_BKG_0751HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0751Event event = new EsmBkg0751Event();
		if (command.isCommand(FormCommand.SEARCH))
		{
			event.setUsWhfEmlListCondVO((UsWhfEmlListCondVO) getVO(request, UsWhfEmlListCondVO.class));
		}
		else if (command.isCommand(FormCommand.MULTI))
		{
			UsWhfEmlVO[] usWhfEmlVOs = (UsWhfEmlVO[]) getVOs(request, UsWhfEmlVO.class);
			for (int i = 0; i < usWhfEmlVOs.length; i++)
			{
				usWhfEmlVOs[i].setPortCd(request.getParameter("port_cd"));
			}
			event.setUsWhfEmlVOs(usWhfEmlVOs);
		}

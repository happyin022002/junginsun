...
import com.hanjin.apps.alps.esm.bsa.common.integration.CommonDBDAO;
...
public class CommonBCImpl extends BasicCommandSupport implements CommonBC {
    ...
    /**
     * 1. 기능 : Vsl Cd 목록을 조회한다.<p>
     * 2. 처리개요 : <p>
     * 3. 주의사항 : <p>
     * ===================================<br>
     * 4. 작성자/작성일 : sung-hun KIM/2012.03.05<br>
     * ===================================<br>
     * 5. 수정사항<p>
     * 5.1 요구사항 ID :<p>
     * - 수정자/수정일 :<p>
    ...
    public String searchVslCd(String vslCd) throws EventException {
    	try {
    		return dbDao.searchVslCd(vslCd);
    	} catch (DAOException de) {
    		log.error("err "+de.toString(),de);
    		throw new EventException(de.getMessage());
    	}
	...
	public List<GetCodeSelectVO> getCodeSelectList(String codeItem, String code) throws EventException {
    	...
    	DBRowSet dRs = null;
	        try {	        	
	        	...
	        	}else if(codeItem.equalsIgnoreCase("sLane")){			// Service Lane List	
	        		if(!code.equals("")) {
	        			dRs = dbDao.searchSLaneList(code);
	        		}	        		
	        	}else if(codeItem.equalsIgnoreCase("currency")){			// Currency	
	        			dRs = dbDao.searchCurrencyList(code);
	        	}
	        	else{
        	    //공통코드	

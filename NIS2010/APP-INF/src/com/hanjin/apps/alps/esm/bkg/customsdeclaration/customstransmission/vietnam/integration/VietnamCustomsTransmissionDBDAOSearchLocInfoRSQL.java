...
public class VietnamCustomsTransmissionDBDAOSearchLocInfoRSQL implements ISQLTemplate{
	...
	public VietnamCustomsTransmissionDBDAOSearchLocInfoRSQL(){
		...
		params.put("b_por",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("             SELECT   DECODE(P.VPS_PORT_CD, B.POL_CD, 'POL', B.POD_CD, 'POD', B.DEL_CD, 'DEL', 'TS') LOC_TYPE" ).append("\n"); 
		...
		query.append("					 ,(  SELECT LOC_NM FROM MDM_LOCATION WHERE LOC_CD = P.VPS_PORT_CD ) PORT_NM" ).append("\n"); 
		...
		query.append("             AND      P.VPS_PORT_CD IN ( B.POL_CD, B.POD_CD, B.DEL_CD )" ).append("\n"); 
		...
		query.append("					 ,MAX((SELECT LOC_NM FROM MDM_LOCATION WHERE LOC_CD = B.DEL_CD )) PORT_NM" ).append("\n"); 
		...
		query.append("					 ,MAX((SELECT LOC_NM FROM MDM_LOCATION WHERE LOC_CD = B.POR_CD)) PORT_NM" ).append("\n"); 
		query.append("                     ,MIN(D.ESTM_DT) LOC_ETA" ).append("\n"); 
		query.append("                     ,MAX(D.ESTM_DT) LOC_ETD" ).append("\n"); 
		query.append("             FROM     BKG_BOOKING B" ).append("\n"); 
		query.append("                     ,SCE_COP_HDR H" ).append("\n"); 
		...
		query.append("         ,CASE  WHEN @[pol_gubun] = '1' AND SUBSTR(@[b_por],1,2) = 'VN' AND SUBSTR(@[v_pol],1,2) = 'VN'" ).append("\n"); 
		query.append("                THEN NVL(MAX(DECODE(LOC_TYPE,'DEL',PORT_NM,NULL)),MAX(DECODE(LOC_TYPE,'POD',PORT_NM,NULL))) " ).append("\n"); 
		query.append("                ELSE NVL(MAX(DECODE(LOC_TYPE,'DEL',LOC_NM,NULL)),MAX(DECODE(LOC_TYPE,'POD',LOC_NM,NULL))) END LOC_NM" ).append("\n"); 

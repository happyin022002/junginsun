 ...
 *@FileName : ESM_PRI_7031_01HTMLAction.java
...
public class ESM_PRI_7031_01HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_PRI_7031_01HTMLAction 객체를 생성
	 */
	public ESM_PRI_7031_01HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri703101Event event = new EsmPri703101Event();

		if (command.isCommand(FormCommand.SEARCH01)) {
			event.setiHCGuidelineDetailVO((IHCGuidelineDetailVO) getVO(request, IHCGuidelineDetailVO.class));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.setSearchUsInlandServiceModeTotalVO((SearchUsInlandServiceModeTotalVO) getVO(request, SearchUsInlandServiceModeTotalVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setiHCGuidelineDetailVOs((IHCGuidelineDetailVO[]) getVOs(request, IHCGuidelineDetailVO.class, "sheet1_"));
		}
		return event;
	}

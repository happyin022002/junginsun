...
*@FileName : ESM_PRI_6014_02HTMLAction.java
...
public class ESM_PRI_6014_02HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6014_02HTMLAction 객체를 생성
	 */
	public ESM_PRI_6014_02HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri601402Event event = new EsmPri601402Event();
		...
		if (command.isCommand(FormCommand.SEARCH01)) {
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtVO((PriRqGrpCmdtVO) getVO(request, PriRqGrpCmdtVO.class));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtVO((PriRqGrpCmdtVO) getVO(request, PriRqGrpCmdtVO.class));
		} else if (command.isCommand(FormCommand.MULTI01)) {
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtVO((PriRqGrpCmdtVO) getVO(request, PriRqGrpCmdtVO.class));
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtVOs((PriRqGrpCmdtVO[]) getVOs(request, PriRqGrpCmdtVO.class, "sheet1_"));
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtDtlVOs((PriRqGrpCmdtDtlVO[]) getVOs(request, PriRqGrpCmdtDtlVO.class, "sheet2_"));
		} else if (command.isCommand(FormCommand.MULTI02)) {
			event.getGroupCommodityQuotationVO().setRsltPriRqMnVO((RsltPriRqMnVO) getVO(request, RsltPriRqMnVO.class));
		} else if (command.isCommand(FormCommand.SEARCH03)) {
			event.getGroupCommodityQuotationVO().setPriRqGrpCmdtVOs((PriRqGrpCmdtVO[]) getVOs(request, PriRqGrpCmdtVO.class));
		} 

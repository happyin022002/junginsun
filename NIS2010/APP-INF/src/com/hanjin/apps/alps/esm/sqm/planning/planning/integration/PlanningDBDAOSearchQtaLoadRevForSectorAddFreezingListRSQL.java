...
*@FileName : PlanningDBDAOSearchQtaLoadRevForSectorAddFreezingListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class PlanningDBDAOSearchQtaLoadRevForSectorAddFreezingListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * [Add Freezing List] 를 [조회]한다.
	  * 
	  * * History
	  * * 2014.06.20 [CHM-201430324] 이혜민 IAs Sector -프로젝트 안정화 및 3분기 판매목표 수립 지원
	  * 전체 PF GROUP 바뀌어서 들어왔을 경우 기존 PF GROUP VVD가 삭제되고 새로운 GROUP VVD 로 대체되지만 목록에는 기존 PF GROUP이 나오므로 조건 삭제
	  * 추후 삭제된 기존 PF GROUP 으로 다시 VVD가 들어오면 데이터 누락 가능성이 있음. VVD ADJUST 해준후 매뉴얼로 PAIR를 붙여줘야함. 
	  * * 2014.07.28 이혜민  QTA Set up by HO for IAS Sector_Add Freezing 시 Bound 삽입 DIR_CD 항목 추가
	  * * 2015.06.15 김용습 [CHM-201536164] 주간 MAS Open에 따른 타모듈 프로그램 적용 요청
	  * </pre>
	  */
	public PlanningDBDAOSearchQtaLoadRevForSectorAddFreezingListRSQL(){
		setQuery();
		...
		query.append("FileName : PlanningDBDAOSearchQtaLoadRevForSectorAddFreezingListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}

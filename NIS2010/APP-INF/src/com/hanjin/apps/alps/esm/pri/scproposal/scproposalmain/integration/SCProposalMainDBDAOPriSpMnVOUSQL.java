...
public class SCProposalMainDBDAOPriSpMnVOUSQL implements ISQLTemplate{
	...
	public SCProposalMainDBDAOPriSpMnVOUSQL(){
		...
		params.put("real_cust_val_sgm_cd",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("    ,REAL_CUST_VAL_SGM_CD = DECODE(@[real_cust_cnt_cd], 'XX','',NVL(@[real_cust_val_sgm_cd],REAL_CUST_VAL_SGM_CD) )" ).append("\n"); 
		...
		query.append("    ,SLS_LD_NO            = DECODE(@[sls_ld_no], 'XX','', NVL(@[sls_ld_no],SLS_LD_NO))                    " ).append("\n"); 

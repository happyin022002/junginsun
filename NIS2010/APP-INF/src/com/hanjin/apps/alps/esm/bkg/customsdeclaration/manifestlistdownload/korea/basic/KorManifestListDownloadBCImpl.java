...
* -----------------------------------------------------------
* History 
* 2010.10.22 경종윤 [CHM-201006653] Manifest Korea & WHF 오류 사항 보완 요청
* 				1. Manifest Print 문제 (Outbound)
*					- T/S, LOCAL 판단 로직 수정
...
* 2015.06.08 이한나[CHM-201536139] [긴급] Manifest generate 화면 Download 시 국내 개항간 화물의 Type 오류 로직 수정요청
=========================================================*/
...
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.korea.vo.DateVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.korea.vo.HjtRcvMsgVO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.customstransmission.korea.vo.KorBkgCstmsVvdSmryVO;
...
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.manifestlistdownload.korea.integration.KorManifestListDownloadDBDAO;
import com.hanjin.apps.alps.esm.bkg.customsdeclaration.manifestlistdownload.korea.vo.*;
 ...
 * @since J2EE 1.4
 */
public class KorManifestListDownloadBCImpl extends ManifestListDownloadBCImpl
{

	private static final long serialVersionUID = 7468543338172852812L;
	private transient KorManifestListDownloadDBDAO dbDao = null;
	//private transient Jp24ManifestListDownloadDBDAO jp24dbDao = null;
	private transient Jp24ManifestListDownloadEAIDAO jp24eaiDao = null;
    private transient GeneralBookingReceiptDBDAO gbrDbDao = null;
	/**
	 * KorManifestListDownloadBCImpl 객체 생성<br>
	 * KorManifestListDownloadDBDAO 생성한다.<br>
	 */
	public KorManifestListDownloadBCImpl()
	{
		dbDao = new KorManifestListDownloadDBDAO();
		//jp24dbDao = new Jp24ManifestListDownloadDBDAO(); // CHM-201641466
		jp24eaiDao = new Jp24ManifestListDownloadEAIDAO();
		gbrDbDao = new GeneralBookingReceiptDBDAO();
	}
	

	/**
	 * [0329] ELNO 누락건의 E-Mail 전송 CHM-201641466
	 * @param ManifestListCondVO manifestListCondVO
	 * @param SignOnUserAccount account
	 * @return List<BkgNtcHisVO> 
	 * @exception EventException 
	 */
	public List<BkgNtcHisVO> sendSimpleElNoEmail(ManifestListCondVO manifestListCondVO //arrNtcSendListVos						
		...
		KorManifestInfoVO[] blNtcSendListVos = condition.getKorManifestInfoVOs();
		...
		//개별적으로 보낼 제목과 본문 내용
		String shipperSubject  = "";
		String shipperContents = "";
		String shipperName = "";
		...
		String emlSndNo = "";
		String sUsrSubject = "수출면장 정보 누락 요청";
		
		StringBuffer sUsrContents = null;
		...
		try {
			...
			// 보내는사람: 시스템이 아닌 전송자를 명시.
			//String fromEmail = "noreply@smlines.com";
			String fromEmail = sUsrEml;
			
			sendMailVO.setFromEmail(fromEmail);
			sendMailVO.setFromName("SM Line");
			
			sUsrContents = new StringBuffer();
			//sUsrContents.append("BKG NO").append("\r\n");
								
			
			if ( blNtcSendListVos.length > 0) {
				sUsrSubject = sUsrSubject + " (출항모선 : " + condData.getInVvd() + ")";
			
				shipperContents = "\r\n" +
					 "\r\n안녕하세요." +
					 "\r\n현재 고객님의 수출면장 정보가 누락되어 있으므로, " +
					 "\r\nSM 상선 웹사이트에서 수출면장 정보를 입력해 주시기 바랍니다." +
					 "\r\n";
			}
			
			for (int i = 0; i < blNtcSendListVos.length; i++) {
				KorManifestInfoVO listVO = blNtcSendListVos[i];
								
				//emlSndRsltFlg = "N";
				if (i==0 || !blNtcSendListVos[i-1].getBkgNo().equals(blNtcSendListVos[i].getBkgNo()) ) {
					//log.debug("--------------------------- listVO "+listVO);
					//CHK 가 선택되어있을경우, ELNO Check값이 'N'(누락) 인 경우에만 실행
					if(listVO.getSel().equals("1") && listVO.getElnoA().equals("N")){
														
						shipperSubject  = "BKG " + listVO.getBkgNo() + " // Simple(Direct) 수출면장 보완요청";
						emlAddrString = "";
						telString = "";
						
						sendMailVO.setSubject(shipperSubject);
						sendMailVO.setTextContent(shipperContents);
					
						//emlAddrString = (dbDao.getCntcPsonEml(listVO.getBkgNo()) + "").replaceAll(" ", "").trim();
						
						// Booking No에 해당하는 email address, tel no를 구함.
						BkgCntcPsonVO = dbDao.searchCntcPsonEml(listVO.getBkgNo());
						
						if (BkgCntcPsonVO!=null) {
							emlAddrString = BkgCntcPsonVO.getCntcPsonEml();
							telString = BkgCntcPsonVO.getCntcPsonPhnNo();
						}
									
						shipperName = dbDao.searchBkgCustName(listVO.getBkgNo(), "S").trim();
						
						
						// email 주소가 존재한다면 mail을 전송한다.
						if (!"".equals(emlAddrString)) {
							...
							for (String tempEmlAddr : tempEmlAddrArray) {
								...
								if (emlSndNo != null && !"".equals(emlSndNo)) {
									...
									hisVOS.add(bkgNtcHisVO);	
									
									//sUsrContents = sUsrContents + "\r\n" + listVO.getBkgNo() + "\r" + tempEmlAddr;
							...
							emlAddrString = "NOT FOUND";
						}
						
						sUsrContents.append("<tr align=\'left\'><td>").append(listVO.getBkgNo()).append("</td>");
						sUsrContents.append("<td>").append(shipperName).append("</td>");
						sUsrContents.append("<td>").append(listVO.getPod()).append("</td>");
						sUsrContents.append("<td>").append(emlAddrString).append("</td>");				
						sUsrContents.append("<td>").append(telString).append("</td></tr>");				
			...
			if ( hisVOS.size() > 0 ) {
				// 사용자에게 전송 Booking no를 이메일로 전송.
				//sendMailVO.setSubject(sUsrSubject);
				//sendMailVO.setTextContent(sUsrContents.toString());
				
				//sendMailVO.setRecipient(sUsrEml);
				//emlSndNo = jp24eaiDao.sendMail(sendMailVO);
				mail.setBatFlg("N");
				mail.setHtmlTemplate("ESM_BKG_0329_01T.html");
				mail.setFrom(fromEmail);				//보내는 사람 메일주소			
				mail.setRecipient(fromEmail);		//받는 사람 메일주소
				
				
				mail.setSubject(sUsrSubject);			//메일 제목				
				
				//mail.setHtmlContent(emailSendInfoVO.getTextContent());		//메일 본문내용
												
				mail.setArg("detail_list", sUsrContents.toString());
					
				//mail.setGroupwareMail();   
				
				//mail.send();
				
				String sndId = mail.send();
				log.debug("@@@@@@ sendSimpleElNoEmail : sndId = "+sndId);
				log.debug("@@@@@@ argument : "+sUsrContents.toString());
	 ...
	 * @exception EventException 
	 */
	public List<BkgNtcHisVO> sendConsoleEmail(ManifestListCondVO manifestListCondVO //arrNtcSendListVos						
						, SignOnUserAccount account) throws EventException{
		...
		String emlSndNo = "";
		String sUsrSubject = "House B/L(Console) EDI 요청";
		...
		try {
			...
			if ( blNtcSendListVos.length > 0) {
				sUsrSubject = sUsrSubject + " (모선 : " + condData.getInVvd() + ")";
						
				shipperContents = "\r\n" +
					 "\r\nSM상선을 이용해 주셔서 감사합니다." +
					 "\r\n" +
					 "\r\n본 메일은 해당 모선에 하우스가 있는 화주에게 일괄 전송되는 메일로" +
					 "\r\n하우스 전송이 누락되거나(하우스 미취합) 오류가 없는지 해당 모선에" +
					 "\r\n운항 선사에 따라 유로지스( https://www.ulogishub.com ) 또는" +
					 "\r\nPLISM 3.0( https://www.plism.com )에서 확인 하시기 바랍니다." +
					 "\r\n";
			}
			...
			for (int i = 0; i < blNtcSendListVos.length; i++) {
				...
				if (i==0 || !blNtcSendListVos[i-1].getBkgNo().equals(blNtcSendListVos[i].getBkgNo()) ) {
					//log.debug("--------------------------- listVO "+listVO);
					//CHK 가 선택되어있을경우, ELNO Check값이 'N'(누락) 인 경우에만 실행
					if(listVO.getSel().equals("1") && listVO.getSc().equals("C")){
														
						shipperSubject  = "BKG " + listVO.getBkgNo() + " // House EDI(한국세관) 전송 및 오류 체크 요청";
						emlAddrString = "";
						telString = "";
			...
			if ( hisVOS.size() > 0 ) {
				...
				String sndId = mail.send();
				log.debug("@@@@@@ sendConsoleEmailSend : sndId = "+sndId);
				log.debug("@@@@@@ argument : "+sUsrContents.toString());
			} 
	...
	{
		KorMrnNoVO manifestInfoVO = (KorMrnNoVO)paramVO;
		...
		KorContainerVO container = new KorContainerVO();
		...
		try 
		{
			...
			if("bl".equals(bldl))
			{
				...
				container.setKorMrnNoVO(fromClient);
				list = dbDao.searchManifestInfoList( manifestInfoVO);
				container.setKorManifestInfoVOs(list);
...
//						// Inbound이고 A:Local Type인 경우 T/S가 'I'인 데이터만 검색되게 설정한다.
					...
					container.addKorBkgCntrQtyInfoVO(bkgCntrQtyInfo);
					//컨테이너 카운트
...
//			else if("dl".equals(bldl))
			else
			{
				...
				KorMrnVO korMrnVO = dbDao.searchMrnNoKor(manifestInfoVO);
				if(korMrnVO == null) return null;							
				
				manifestInfoVO.setMrnNo(korMrnVO.getMrnNo());
				manifestInfoVO.setMrnChkNo(korMrnVO.getMrnChkNo());
				manifestInfoVO.setEtaEtd(korMrnVO.getEtaEtd());
				manifestInfoVO.setEtdDt(korMrnVO.getEtdDt());				
				manifestInfoVO.setEtbDt(etbDt);				
				
				container.setKorMrnNoVO(manifestInfoVO);												
			...
			if("mc".equals(bldl)){
				List<KorManifestInfoVO> diffList = container.getKorManifestInfoVOs();
				...
				if(diffList != null){
					...
					container.setKorManifestInfoVOs(diffNewList);	
				}
			}
...
//							createType = "N";
						...
						if("O".equals(condData.getInBound()) && "P".equals(down.getFe()) && "KRPUS".equals(down.getPol())){
							korBkgCntrVOs = dbDao.searchBKGCNTRList(down.getABkgNo(), down.getABkgNoSplit(), down.getCBlNo(), condData.getInBound(), down.getFe());
						}else{
							korBkgCntrVOs = dbDao.searchBKGCNTRList(down.getABkgNo(), down.getABkgNoSplit(), "", condData.getInBkgNo(), down.getFe());
						}
						...
						korIbMtInfoVO = null;
//						if (down.getCntrNo()!=null && down.getCntrNo().length() > 1 && (kcd_tp.equals("E") || kcd_tp.equals("R")) )
//						{
						
						// export down 시에 과거 t/s import 신고 내역을 찾는다. down 시 bkg no와 cblno가 다를 경우 과거 내역이 있는 bl임
						if(!down.getABkgNo().equals(down.getCBlNo()) && (kcd_tp.equals("E") || kcd_tp.equals("R"))){
							
						
							//korIbMtInfoVO = dbDao.searchIbTsMtInfo(down.getCntrNo());
							korIbMtInfoVO = dbDao.searchIbTsMtInfo(down.getCBlNo());
							...
							if (korIbMtInfoVO!=null && korIbMtInfoVO.getInCgoTp().equals("P")) {
								...
								//korIbMtInfoVO.getPorCd();
//								down.setPol(korIbMtInfoVO.getPolCd());
//								down.setPod(korIbMtInfoVO.getPodCd());
								//korIbMtInfoVO.getDelCd();
								
								down.setBkgPor(korIbMtInfoVO.getPorCd());
								down.setBkgPol(korIbMtInfoVO.getPolCd());
								down.setBkgPod(korIbMtInfoVO.getPodCd());
								down.setBkgDel(korIbMtInfoVO.getDelCd());
...
//						{	
						...
						// export down 시에 과거 t/s import 신고 내역을 찾는다. down 시 bkg no와 cblno가 다를 경우 과거 내역이 있는 bl임 -2014 07 27
						if(!down.getABkgNo().equals(down.getCBlNo()) && (kcd_tp.equals("E") || kcd_tp.equals("R"))){
							// 조회 결과 세팅
						...
						if(down.getBkgPol()==null || down.getBkgPol().length() < 1){
							down.setBkgPol(node.getPol());
						}
						if(down.getBkgPod()==null || down.getBkgPod().length() < 1){
...
//									param1.setKcdTp		(cntr_kcd_tp				);
							...
							if(!"MAIL BOX".equals(cargoParam.getDescCode1()))
							{
								if("O".equals(condData.getInBound()))
								{
									if("R".equals(kcd_tp) || !down.getABkgNo().equals(down.getCBlNo()))
									{
										...
										//t/s MT EXPORT 신고 시 과거 BOOKING NO로 조회 -2014 07 28
										if(down.getIbMtyBkgNo() != null){
											elNoVO.setBkgNo(down.getIbMtyBkgNo());
										}
										...
										if(elNo !=null)
										{
											...
											korMailBoxVO.setPrtLodgFlg(down.getPrtLodgFlg());
						...
						if(exportList != null)
						{
							...
							for(int j=0;j<exportList.size(); j++)
							{
								...
								curElNo = exportNode.getEBmeElno();								
								
								// ELNO 가 없으면 SKIP 처리
								if (exportNode.getEBmeElno()==null || exportNode.getEBmeElno().trim().length() < 1) continue;
								// 동일한 ELNO가 2개 이상 입력된 경우 첫 번째 ELNO만 Download
								if (curElNo.equals(prevElNo) ) continue;
							...
							for(int j=0;j<custInfoList.size();j++)
							{
								...
								if (cust_kcd_tp!=null) {
									...
									for(int k=0; k < cust_kcd_tp.length; k++) {
										...
										//Shipper이면서 수입수출 환적이면서 address에 KOREA가 있으면 korea를 빈값으로 수정한다.
										if ("S".equals(custNode.getCBcsTp()) && (cust_kcd_tp[k].equals("T") || cust_kcd_tp[k].equals("R")) )
										{
											custNode.setCCustAddr(custNode.getCCustAddr().replaceAll("KOREA", ""));							
										}
						...
						paramVO.setVvdPodTmnlCd(condData.getInPodTmnl());
						...
						///////////////합계 type inbound:N, outbound:D 를 만들어준다
						if(korBkgCstmsVvdSmryVO.getBound().equals("I")){ 
							korBkgCstmsVvdSmryVO.setCreatedtype("N");
						}else{
							korBkgCstmsVvdSmryVO.setCreatedtype("D");
						}	
						max_seq2 = dbDao.searchMaxSeqVVDKor(korBkgCstmsVvdSmryVO);
						if(max_seq2 == null || max_seq2.equals("")) max_seq2 = "0";
						int j_max_seq = Integer.parseInt(max_seq2);
						///////////////합계 type inbound:N, outbound:D 를 만들어준다 end
						...
						else if(i_max_seq > 0)
						{
							...
							dbDao.modifyVVDInfoTemp(paramVO);
						}
						...
						if(j_max_seq == 0)
						{
							...
							if(paramVO.getBound().equals("I")){
								korBkgCstmsVvdSmryVO.setCreatedtype("N");
								paramVO.setCreatedtype("N");
							}else{
								korBkgCstmsVvdSmryVO.setCreatedtype("D");
								paramVO.setCreatedtype("D");
							}														
							j_max_seq ++;
							paramVO.setKvSeq(Integer.toString(j_max_seq));	
							dbDao.addVVDInfo(paramVO);
						}
						else if(j_max_seq > 0)
						{
							...
							String k_chk = dbDao.searchMaxSeqVVDYard(paramVO);
							if("N".equals(k_chk))
							{												
								j_max_seq ++;
								paramVO.setKvSeq(Integer.toString(j_max_seq));	
							...
							{
								String mf_snd_dt2 = dbDao.searchMaxSendCheckKor(paramVO);
								if("Y".equals(mf_snd_dt2))
								{
									j_max_seq ++;
									paramVO.setKvSeq(Integer.toString(j_max_seq));	
									dbDao.addVVDInfo(paramVO);
...
//					reDownList.get(0).getWhouse().length();
					...
					KorBkgCstmsVvdSmryVO oldMrnKor = null;
					...
					for(int idxDn =0; idxDn < reDownList.size(); idxDn++) 
					{
						...
						else 
						{
							...
							else
							{
								//kcd_tp = "T";
								//reDownNode.setTp("T");
								// 2015.06.08 [CHM-201536139] 
								if("KRPUS".equals(down_vvd_pod) || "KRKAN".equals(down_vvd_pod))
                                {
                                     if(reDownNode.getPod().startsWith("KR"))  {
                                         kcd_tp = "I";
                                         reDownNode.setTp("I");
                                    }
                                    else {
...
//						if("O".equals(condData.getInBound()) && "P".equals(down.getFe())){
//							korBkgCntrVOs = dbDao.searchBKGCNTRList(down.getABkgNo(), down.getABkgNoSplit(), down.getCBlNo(), condData.getInBound(), down.getFe());
//						}else{
//							korBkgCntrVOs = dbDao.searchBKGCNTRList(down.getABkgNo(), down.getABkgNoSplit(), "", condData.getInBkgNo(), down.getFe());
//						}
						
						if("O".equals(condData.getInBound()) && "P".equals(reDownNode.getFe()) && "KRPUS".equals(reDownNode.getPol())){
							korBkgCntrVOs = dbDao.searchBKGCNTRList(reDownNode.getABkgNo(), reDownNode.getABkgNoSplit(), reDownNode.getCBlNo(),condData.getInBound(),reDownNode.getFe());
						}else{
							korBkgCntrVOs = dbDao.searchBKGCNTRList(reDownNode.getABkgNo(), reDownNode.getABkgNoSplit(), "",condData.getInBound(),reDownNode.getFe());
						}
...
//						{
						// export down 시에 과거 t/s import 신고 내역을 찾는다. down 시 bkg no와 cblno가 다를 경우 과거 내역이 있는 bl임
						if(!reDownNode.getABkgNo().equals(reDownNode.getCBlNo()) && (reDownNode.getKcdTp().equals("E") || reDownNode.getKcdTp().endsWith("R")))
						{
							korIbMtInfoVO = dbDao.searchIbTsMtInfo(reDownNode.getCBlNo());
						}
...
//								if(searchTransMit != null)searchTransMit.setBKtSeq(Integer.toString(notTransCount));
						...
						if(before_tr_count != 0 || !"X".equals(transParam.getKtSts()))
						{
							...
							String max_seq2 = dbDao.searchMaxSeqVVDKor(paramVO);
							if(max_seq2 == null || max_seq2.equals("")) max_seq2 = "0";
							int seq_val2 = Integer.parseInt(max_seq2);
							///////////////합계 type inbound:N, outbound:D 를 만들어준다 end
							...
							if(seq_val2 == 0)
							{
								...
								paramVO.setKvSeq(Integer.toString(seq_val2 + 1));
								dbDao.addVVDInfo(paramVO);
							}
							else if(seq_val2 > 0)
							{
							...
							// 추가된 컨테이너 번호가 존재하고 KCD_TP 가 E이거나 R이면
//							if (reDownNode.getCntrNo()!=null && reDownNode.getCntrNo().length() > 1 && (reDownNode.getKcdTp().equals("E") || reDownNode.getKcdTp().endsWith("R")) )
//							{
							...
							if(!reDownNode.getABkgNo().equals(reDownNode.getCBlNo()) && (reDownNode.getKcdTp().equals("E") || reDownNode.getKcdTp().endsWith("R"))){
								...
								if (korIbMtInfoVO!=null && korIbMtInfoVO.getInCgoTp().equals("P")) {
									...
									reDownNode.setBkgPor(korIbMtInfoVO.getPorCd());
									reDownNode.setPol(korIbMtInfoVO.getPolCd());
									reDownNode.setPod(korIbMtInfoVO.getPodCd());
									reDownNode.setBkgDel(korIbMtInfoVO.getDelCd());
							...
							korManifestDNVO.setCstmsCrrInLocWhCd(reDownNode.getCstmsCrrInLocWhCd());
							...
							korManifestDNVO.setSiFlg(reDownNode.getSiFlg());	
...
//											param1.setKcdTp(cntr_kcd_tp);
							...
							if(!"MAIL BOX".equals(cargoParam.getDescCode1())) {
								...
								if("R".equals(reDownNode.getKcdTp())|| !reDownNode.getABkgNo().equals(reDownNode.getCBlNo())) 
								{
									...
									//t/s MT EXPORT 신고 시 과거 BOOKING NO로 조회 -2014 07 28
									if(reDownNode.getIbMtyBkgNo() != null ){
										elNoMakeVO.setBkgNo(reDownNode.getIbMtyBkgNo());
									}
									...
									if(elno != null)
									{
										...
										exportNoParam.setKtPort		(kt_port					);
										exportNoParam.setUsername	(account.getUsr_id()		);
										exportNoParam.setPrtLodgFlg (reDownNode.getPrtLodgFlg()	);
							...
							if(exportList != null)
							{
								...
								for(int i=0;i<exportList.size();i++)
								{
									...
									curElNo = korExportInfoDNVO.getEBmeElno();					
	...
	public List<ManifestListDetailVO> searchManifestList(ManifestListCondVO manifestListCondVO) throws EventException {
		...
		// IBBkgInfo 조회 결과를 담는 객체 
		KorBkgInfoVO outKorBkgInfoVO = null;
		// IBBkgInfo 조회 결과 리스트
		...
		try 
		{
			...
			if (outKorMrnInfoVO != null) {
				...
				if (inKorMrnInfoVO.getInMrnMode().equals("I")) {
					...
					// IBBkgInfo 리스트 조회
					korBkgInfoVOs = dbDao.searchIBBkgInfoList(inKorBkgInfoVO);
					if (korBkgInfoVOs!=null) {
						for(int i=0; i < korBkgInfoVOs.size(); i++) {
							// 결과 1건 꺼내옴.
							outKorBkgInfoVO = korBkgInfoVOs.get(i);
							
							// MODE 구분  ( BKG_POD 와 VVD_POD 가 동일하면 LOCAL, 그렇지 않으면 TS로 설정 )
							if (outKorBkgInfoVO.getBkgPodCd().equals(outKorBkgInfoVO.getVvdPodCd())) 
								outKorBkgInfoVO.setLocalTs("I");
							else
								outKorBkgInfoVO.setLocalTs("T");
							
							// CUST TP COUNT 추출 
							inKorCntCustVO.setBkgNo(outKorBkgInfoVO.getBkgBkgNo());
							
							// BL TYPE 조회
							blTp = dbDao.searchBlTp(outKorBkgInfoVO.getBkgBkgNo());
							// 조회결과 반영 
							outKorBkgInfoVO.setBlTp(blTp);
							
							// CUSTOMER TYPE 기본값 설정 (ST)
							outKorBkgInfoVO.setCstmTpCd("ST");
																
							// Discharging Location 조회
							korDiscLocVO.setPortCd  (outKorBkgInfoVO.getVvdPodCd()   );
							korDiscLocVO.setSlanCd  (outKorBkgInfoVO.getVvdSlanCd()  );
							korDiscLocVO.setSkdDirCd(inKorBkgInfoVO.getVvdSkdDirCd() );					
							discCd = dbDao.searchDiscLoc(korDiscLocVO);
							
							// MSN INFO 조회
							inKorMsnInfoVO.setBkgNo	 (outKorBkgInfoVO.getBkgBkgNo()	);
							...
							if (outKorMsnInfoVO==null) {
								if (!outKorBkgInfoVO.getBkgStsCd().equals("X") && 
									!outKorBkgInfoVO.getBkgStsCd().equals("S")) {
									// MSN 추가용 파라메터 설정
									inKorMsnInfoVO.setBkgNo			(outKorBkgInfoVO.getBkgBkgNo()			);
									inKorMsnInfoVO.setMrnNbr		(outKorMrnInfoVO.getMrnNo()				);
									inKorMsnInfoVO.setMrnChk		(outKorMrnInfoVO.getMrnChkNo()			);
									inKorMsnInfoVO.setLocalTs		(outKorBkgInfoVO.getLocalTs()			);
									inKorMsnInfoVO.setBlTp			(outKorBkgInfoVO.getBlTp()				);
									inKorMsnInfoVO.setIcdCstmTp		(outKorBkgInfoVO.getCstmTpCd()			);
									inKorMsnInfoVO.setDiscCd		(discCd									);
									inKorMsnInfoVO.setCstmsCrrInLocWhCd(discCd								);
								...
								outKorMsnInfoVO.setLocalTs	(outKorBkgInfoVO.getLocalTs()	);
								
								if (outKorBkgInfoVO.getBkgStsCd().equals("X") || 
									outKorBkgInfoVO.getBkgStsCd().equals("S")) {
									// MSN INFO 삭제 처리
									dbDao.removeMSNInfo(outKorMsnInfoVO);
								}else if (!outKorBkgInfoVO.getBkgStsCd().equals("X") && 
										  !outKorBkgInfoVO.getBkgStsCd().equals("S")) {
									// EDI 전송 체크 조회
				...
				if (outKorMrnNoVO==null) {
					...
					outKorMrnNoVO.setMrnNo	(outKorMrnInfoVO.getMrnNo()		);
					outKorMrnNoVO.setInBound(outKorMrnInfoVO.getIoBndCd()	);
				}
				...
				if (outKorMrnNoVO.getInBound().equals("I")) {
					...
					// 최종 MSN INFO 리스트 조회
					korMsnInfoVOs = dbDao.searchIBMSNInfoList(finalKorMsnInfoVO);
					
					// List 순회하며 LOCAL / TS 구분에 따라 담기
					for(int i=0; i < korMsnInfoVOs.size(); i++) {
						if (korMsnInfoVOs.get(i).getLocalTs().equals("I")) {
							localMsnInfoVOs.add(korMsnInfoVOs.get(i));
						}else {
							tsMsnInfoVOs.add(korMsnInfoVOs.get(i));
						}
					}
	...
	public DiscCYBondInfoVO searchDiscCYBondInfo(DiscCYBondInfoVO discCYBondInfoVO) throws EventException
	{
		...
		try {
			...
			bdrFlg = outKorDiscCYBondInfoVO.getBdrFlg(); // Add. 2015.06.30
			...
			outKorDiscCYBondInfoVO.setBdrFlg		(bdrFlg						);  // Add. 2015.06.30
			...
			outKorDiscCYBondInfoVO.setUpdUsrId		(korGrpMsnVO.getUpdUsrId()	);
			
			//Total Vol 조회 추가 Start(2018.03.19 - 하대성)
            ...
            if(bkgQuantity.size() != 0) {
            	totalVol = bkgQuantity.get(0).getCntrTpszCd() + "X" + bkgQuantity.get(0).getOpCntrQty();
            	if(bkgQuantity.size() > 1){
                	for (int i=1; i<bkgQuantity.size(); i++) {
                		totalVol = totalVol + "," + bkgQuantity.get(i).getCntrTpszCd() + "X" + bkgQuantity.get(i).getOpCntrQty();
                	}
                }
            	outKorDiscCYBondInfoVO.setTotalVol(totalVol);
            } 
			//Total Vol 조회 추가 End(2018.03.19 - 하대성)
	...
	public DiscCYBondInfoVO manageDiscCYBondInfo(DiscCYBondInfoVO discCYBondInfoVO) throws EventException 
	{
		...
		KorBkgHistVO outKorBkgHistVO = null; 
		int bkgSeq = 0;
		KorBkgHistVO outKorBkgHistDtlVO = null;
		...
		try {
			...
			// OLD 와 NEW 값이 다를 경우
			if (korOldDiscLocVO==null || !korOldDiscLocVO.getOldMsnDiscloc().equals(newMsnDiscloc)) {
				// BKG_NO 셋팅
	...
	public ManifestEditListVO searchManifestEditList(ManifestEditListCondVO manifestEditListCondVO) throws EventException {
		...
		// Customer Info 용 객체
		KorCustInqInfoVO 	custInqInfoVO 	= null;
		...
		try {
			...
			if (blInqInfoVO != null) {korManifestEditListVO.setKorBlInqInfoVO(blInqInfoVO);
		
			
			if (blInqInfoVO==null || blInqInfoVO.getCgoTrspCd()==null || blInqInfoVO.getCgoTrspCd().trim().length() < 1) {
				// Cargo Spec 정보 조회
				if(blInqInfoVO != null){
				...
				if (cgoSpec!=null && cgoSpec.compareTo("0") > 0 ) {
					...
					if (condVO.getCstmsDeclTpCd().equals("I")) {
						...
						if (blInqInfoVO != null && blInqInfoVO.getDelCd()!=null && blInqInfoVO.getDelCd().startsWith("KR")) {
							cgoSpec = "1";
						}else {
							cgoSpec = "2";
						}
		...
		try {
			...
			if (mode.equals("ADD")) {
				...
				if (transTp.equals("I") || transTp.equals("T")) {
					if(transTp.equals("T") && cargoTp.equals("P")){
						usaBndFlg = "T";
					}else if(transTp.equals("T") && !cargoTp.equals("P")){
						usaBndFlg = "R";
					}else if(transTp.equals("I") && !cargoTp.equals("P")){
						usaBndFlg = "A";
					}else if(transTp.equals("I") && cargoTp.equals("P")){
						usaBndFlg = "M";
					}else{
				...
				dbDao.addEmptyVslInfo(korBlInqInfoVO);
				// Max Sequence 추출
				BkgCstmsKrBlVO bkgCstmsKrBlVO = new BkgCstmsKrBlVO();
	...
	public BkgCstmsKrMfSeqNoVO[] modifyMsnBondedInfo(MsnBondInfoVO msnBondInfoVO) throws EventException {
		...
		try {
			...
			for(int i=0; i < bondInfoVOs.length; i++) {
				...
				bkgCstmsKrMfSeqNoVO.setCstmsCrrInLocWhCd(bondInfoVOs[i].getCstmsCrrInLocWhCd()	);
	...
	public BlInfoVO searchBlInfo(BlInfoCondVO blInfoCondVO) throws EventException {
		...
		try {
			...
			korAmendInfoVO.setIoBndCd		(condVO.getIoBndCd()		);
			...
			korCorrVO.setIoBndCd		(condVO.getIoBndCd()		);
		...
		try {
			...
			if (mode.equals("ADD")) {
				...
				if (korBlAmdVO.getIoBndCd().equals("I")) {
					if(korBlAmdVO.getCstmsDeclTpCd().equals("T") && korBlAmdVO.getBkgCgoTpCd().equals("P")){
						inType = "T";
					}else if(korBlAmdVO.getCstmsDeclTpCd().equals("T") && !korBlAmdVO.getBkgCgoTpCd().equals("P")){
						inType = "R";
					}else if(korBlAmdVO.getCstmsDeclTpCd().equals("I") && !korBlAmdVO.getBkgCgoTpCd().equals("P")){
						inType = "A";
					}else if(korBlAmdVO.getCstmsDeclTpCd().equals("I") && korBlAmdVO.getBkgCgoTpCd().equals("P")){
						inType = "M";
					}else{
...
//					throw new EventException(new ErrorHandler("BKG00889").getMessage());
				...
				korBkgDgVvdVO.setDgcoSeq    (vvdTokens[32]);
				// 처리 종료
				vvdTokens = null;
...
//			}
	...
	public void modifyManifestSummaryInfo(ManifestSmryCondVO manifestSmryCondVO) throws EventException {
		// 파라메터 변환
		KorManifestSmryCondVO condVO = (KorManifestSmryCondVO)manifestSmryCondVO;
		// POD TML 생성
		String podTml = "";
		if (condVO.getTmlCd()!=null && condVO.getTmlCd().length() > 0 && condVO.getPodCd() != null && condVO.getPodCd().length() > 0) podTml = condVO.getPodCd() + condVO.getTmlCd();
		if (condVO.getTmlCd()!=null && condVO.getTmlCd().length() > 0 && condVO.getPolCd() != null && condVO.getPolCd().length() > 0) podTml = condVO.getPolCd() + condVO.getTmlCd();
		// IN TYPE 처리 
		String inType = condVO.getInType();
...
//		if ("|A|B|C|D".indexOf(inType) > 0) portCd = condVO.getPolCd();
		...
		try {
			...
			if (inType.equals("A")) {
				// A인 경우
				// 파라메터 셋팅
				korBondCdVO.setVvd		(condVO.getVvd()		);
				korBondCdVO.setBdAreaCd	(condVO.getBdAreaCd()	);
				korBondCdVO.setUserId	(condVO.getUserId()		);
				korBondCdVO.setPortCd	(portCd					);
				korBondCdVO.setInType	(inType					);
				korBondCdVO.setIoBndCd	(condVO.getIoBndCd()	);
				korBondCdVO.setPodCd	(condVO.getPodCd()		);
				korBondCdVO.setPolCd	(condVO.getPolCd()		);
	...
	public void manageManifestSummaryInfo(ManiSumCondVO maniSumCondVO) throws EventException  {
		...
		try {
			// 파라메터 객체 변환
			KorManiSumCondVO condVO = (KorManiSumCondVO)maniSumCondVO;
			// POD TML 생성
			String podTml = "";
			if (condVO.getTmlCd()!=null && condVO.getTmlCd().length() > 0 && condVO.getPodCd() != null && condVO.getPodCd().length() >0) podTml = condVO.getPodCd() + condVO.getTmlCd();
			if (condVO.getTmlCd()!=null && condVO.getTmlCd().length() > 0 && condVO.getPolCd() != null && condVO.getPolCd().length() >0) podTml = condVO.getPolCd() + condVO.getTmlCd();
...
//			if ("|A|B|C|D".indexOf(condVO.getInType()) > 0) portCd = condVO.getPolCd();
			...
			// Container DATA 삭제
			korCntrDataVO.setVvd	(condVO.getVvd()	);
			korCntrDataVO.setPortCd	(portCd				);
			korCntrDataVO.setInType	(inType				);
			korCntrDataVO.setIoBndCd(condVO.getIoBndCd());
			korCntrDataVO.setPodCd	(condVO.getPodCd()	);
			korCntrDataVO.setPolCd	(condVO.getPolCd()	);
			korCntrDataVO.setPodTml	(podTml				);
			// 삭제
			dbDao.removeCNTRDataKor(korCntrDataVO);
			
			// CUSTOMER 정보 삭제
			korBlCustInfoVO.setVvd		(condVO.getVvd()	);
			korBlCustInfoVO.setPortCd	(portCd				);
			korBlCustInfoVO.setInType	(inType				);
			korBlCustInfoVO.setIoBndCd	(condVO.getIoBndCd());
			korBlCustInfoVO.setPodCd	(condVO.getPodCd()	);
			korBlCustInfoVO.setPolCd	(condVO.getPolCd()	);
			korBlCustInfoVO.setPodTml	(podTml				);
			// 삭제
			dbDao.removeBlCustInfoKor(korBlCustInfoVO);

			// Export DATA 삭제
			korExpDataVO.setVvd		(condVO.getVvd()	);
			korExpDataVO.setPortCd	(portCd				);
			korExpDataVO.setInType	(inType				);
			korExpDataVO.setIoBndCd	(condVO.getIoBndCd());
			korExpDataVO.setPodCd	(condVO.getPodCd()	);
			korExpDataVO.setPolCd	(condVO.getPolCd()	);
			...
			korDnHistVO.setMrnNo(condVO.getMrnNo()	);
			korDnHistVO.setVvd	(condVO.getVvd()	);
			// SEQ 조회
			...
			korDnHistVO.setUserId	(condVO.getUserId()		);
			korDnHistVO.setOfcCd	(condVO.getOfcCd()		);
			korDnHistVO.setPodCd	(condVO.getPodCd()		);
			korDnHistVO.setPolCd	(condVO.getPolCd()		);
...
//			if (condVO.getPodTml()!=null && condVO.getPodTml().length() > 0 && condVO.getPodCd() != null && condVO.getPodCd().length() >0) podTml = condVO.getPodCd() + condVO.getTmlCd();
//			if (condVO.getPodTml()!=null && condVO.getPodTml().length() > 0 && condVO.getPolCd() != null && condVO.getPolCd().length() >0) podTml = condVO.getPolCd() + condVO.getTmlCd();
...
//			if ("|A|B|C|D".indexOf(inType) > 0) portCd = condVO.getPolCd();
			...
			korBondCdVO.setUserId	(condVO.getUserId()	);
			korBondCdVO.setBdAreaCd (condVO.getBdAreaCd());
			korBondCdVO.setMfSndRcvrId (condVO.getInReceiver());
	...
	public void manageMrnCreateInfo(MrnCreateCondVO mrnCreateCondVO) throws EventException {
		...
		// 파라메터 변환
		KorMrnCreateCondVO condVO = (KorMrnCreateCondVO)mrnCreateCondVO;
		// MRN CHECK DIGIT : 국종망 4세대 반영으로 I(수입) or E(수출)로 변경
		String mrnChkNo  = "I".equals(condVO.getIoBndCd())?"I":"E";
		// 기존 3세대 
		//String mrnChkNo = makeMrnChkDigitNo(condVO.getMrnNo());
...
//			}
	...
	private String makeMrnChkDigitNo(String mrnNo) {
		...
		if(mrnLen > 0){ // 소스보안 품질로 수정 . 2015.10.16
	...
	public void removeMrnMsnCreateInfo(MrnCreateInfoVO[] mrnCreateInfoVOs) throws EventException {
		...
		// 파라메터 변환
		KorMrnCreateInfoVO[] korMrnCreateInfoVOs = (KorMrnCreateInfoVO[])mrnCreateInfoVOs;
		
		try {
			// MSN 정보 삭제
			dbDao.removeMsnCreateInfo(korMrnCreateInfoVOs);
			// MRN 정보 삭제
			dbDao.removeMrnCreateInfo(korMrnCreateInfoVOs);
	...
	public VslInfoNManifestVO manageVslInfoNManifestList(VslInfoNManifestCondVO vslInfoNManifestCondVO) throws EventException {
		...
		// 파라메터 변환
		KorVslInfoNManifestCondVO condVO = (KorVslInfoNManifestCondVO)vslInfoNManifestCondVO;
...
//		if ("|A|B|C|D".indexOf(inType) > 0) portCd = condVO.getPolCd();
		...
		try {
			...
			// Type E 이외의 경우
				...
				// MRN INFO 조회
				korMrnInfoKorVO.setPolCd	(condVO.getPolCd()	);
				korMrnInfoKorVO.setPodCd	(condVO.getPodCd()	);
				korMrnInfoKorVO.setVvd		(condVO.getVvd()	);
				korMrnInfoKorVO.setPodTml	(podTml				);
				korMrnInfoKorVO.setInType	(inType				);
				korMrnInfoKorVO = dbDao.searchMrnInfoKor(korMrnInfoKorVO);
				
				// 전송할 데이터 유무 체크
				korExistCntVO.setVvd	(condVO.getVvd()	);
				korExistCntVO.setPortCd	(portCd				);
				korExistCntVO.setInType	(inType				);
				korExistCntVO.setIoBndCd(condVO.getIoBndCd());
				korExistCntVO.setPodCd	(condVO.getPodCd()	);
				korExistCntVO.setPolCd	(condVO.getPolCd()	);
				...
				// Summary Info 조회
				korSumVO.setVvd		(condVO.getVvd()	);
				korSumVO.setInType	(inType				);
				korSumVO.setPortCd	(portCd				);
				korSumVO.setIoBndCd	(condVO.getIoBndCd());
				korSumVO.setPodCd	(condVO.getPodCd()	);
				korSumVO.setPolCd	(condVO.getPolCd()	);
...
//				bkgCstmsKrVvdSmryVO.setBdAreaChk		(blSummaryVO.getBdAreaChk()		);		
				// Summary Info 값 매핑
				...
				outVO.setBdAreaChk(blSummaryVO.getBdAreaChk());	
				outVO.setCrrInCyChk(blSummaryVO.getCrrInCyChk());
			}
		...
		if (condVOs!=null && condVOs.length > 0) {
			...
			try {
				...
				for(int i=0; i < condVOs.length; i++) {
					...
					if (korMsnNoCondVO.getFe().equals("P") && ( korMsnNoCondVO.getTp().equals("I") || korMsnNoCondVO.getTp().equals("T"))) {
	 ...
	 */
	 public List<KorDiscCYBondInfoVO> searchWhfRateList(DiscCYBondInfoVO discCYBondInfoVO) throws EventException{
		KorDiscCYBondInfoVO korDiscCYBondInfoVO  = (KorDiscCYBondInfoVO)discCYBondInfoVO;
		
		List<KorDiscCYBondInfoVO> list = null;
		
		try{
			// Rate테이블에 존재하는 WHF 정보 가져오기
			list = dbDao.searchWhfRateList(korDiscCYBondInfoVO);
	...
	public List<KorDiscCYBondInfoVO> searchWhfBbCgoWgtInfo(KorDiscCYBondInfoVO korDiscCYBondInfoVO) throws EventException{
		...
		try{
			// Rate테이블에 존재하는 WHF 정보 가져오기
			return dbDao.searchWhfBbCgoWgtInfo(korDiscCYBondInfoVO);
	...
	public List<KorDiscCYBondInfoVO> searchWhfPortRt(KorDiscCYBondInfoVO korDiscCYBondInfoVO) throws EventException{
		...
		try{
			return dbDao.searchWhfPortRt(korDiscCYBondInfoVO);
	...
	public List<KorRateHeadVO> searchWhfWaveInfo(KorDiscCYBondInfoVO korDiscCYBondInfoVO) throws EventException{
		...
		try{
			return dbDao.searchWhfWaveInfo(korDiscCYBondInfoVO);
	...
	public void modifyMrnCehckDigit(MrnCreateCondVO mrnCreateCondVO) throws EventException {
		...
		try {
			// 화면에 MRN Check Digit 을  입력하지 않은 경우 3세대 Check Digit 값으로 Assign
			if("".equals(condVO.getMrnChkNo())){
				condVO.setMrnChkNo(mrnChkNo);
			}
			
			dbDao.modifyMrnCehckDigit(condVO);
			
			// 기존에 생성된 MSN이 있다면 Check Digit값을 업데이트 해준다.
			// MRN INFO 조회용 파라메터 값 매핑
			inKorMrnInfoVO.setInMrnMode 	  ("I");
			inKorMrnInfoVO.setInMrnNo   	  (condVO.getMrnNo());
			inKorMrnInfoVO.setMrnChkNo   	  (condVO.getMrnChkNo());
			inKorMrnInfoVO.setInPort    	  (condVO.getPortCd());
			inKorMrnInfoVO.setInSkdDirCd	  (condVO.getVvd().substring(0, 4));
			inKorMrnInfoVO.setInSkdVoyageNo   (condVO.getVvd().substring(4, 8));
			inKorMrnInfoVO.setInVslCd		  (condVO.getVvd().substring(8, 9));

...
*@FileName : ESM_BKG_9466HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ESM_BKG_9466HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_9466HTMLAction 객체를 생성
	 */
	public ESM_BKG_9466HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		log.debug("[START::  ESM_BKG_9466HTMLAction  ]==========");

		EsmBkg9466Event event = new EsmBkg9466Event();
		String[] keys =null;
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			BlRiderInVO blRiderInVO 	= (BlRiderInVO)getVO(request, BlRiderInVO.class);
			BkgImgStoVO[] bkgImgStoVOs 	= (BkgImgStoVO[])getVOs(request, BkgImgStoVO .class,"sheet2_");
			blRiderInVO.setKeys(keys);
			blRiderInVO.setBkgImgStoVOs(bkgImgStoVOs);
			event.setBlRiderInVO(blRiderInVO);

		} else if(command.isCommand(FormCommand.COMMAND01)) {  
			event.setBlCertiRqstVOs((BlCertiRqstVO[])getVOs(request, BlCertiRqstVO .class, ""));
		}

		request.setAttribute("Event", event);
		log.debug("[END:: perform ESM_BKG_9466HTMLAction  ]==========");
		return  event;
	}

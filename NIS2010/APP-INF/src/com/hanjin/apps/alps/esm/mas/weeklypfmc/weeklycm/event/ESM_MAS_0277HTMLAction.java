...
**@FileName : ESM_MAS_0277HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0277Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0277HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0277HTMLAction 객체를 생성
	 */
	public ESM_MAS_0277HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0277Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0277Event event = new EsmMas0277Event();  	
    			
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setDemDetCostReportbyCustomerVO((DemDetCostReportbyCustomerVO)getVO(request, DemDetCostReportbyCustomerVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {			
			event.setDemDetCostReportbyCustomerVOs((DemDetCostReportbyCustomerVO[])getVOs(request, DemDetCostReportbyCustomerVO.class, ""));			
		}
		else {
			event.setDemDetCostReportbyCustomerVO((DemDetCostReportbyCustomerVO)getVO(request, DemDetCostReportbyCustomerVO.class));

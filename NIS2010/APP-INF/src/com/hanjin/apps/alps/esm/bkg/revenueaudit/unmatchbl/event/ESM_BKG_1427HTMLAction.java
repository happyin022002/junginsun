...
*@FileName : ESM_BKG_1427HTMLAction.java
...
public class ESM_BKG_1427HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1427HTMLAction 객체를 생성
	 */
	public ESM_BKG_1427HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1427Event event = new EsmBkg1427Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMultiRateBkgList1VO((MultiRateBkgList1VO)getVO(request, MultiRateBkgList1VO .class));
		}
		else if (command.isCommand(FormCommand.MULTI)) {	// Save
			event.setMultiRateBkgList1VOs((MultiRateBkgList1VO[])getVOs(request, MultiRateBkgList1VO.class));
		}
		else if (command.isCommand(FormCommand.MULTI01)) {	// Confirm
			event.setMultiRateBkgList1VOs((MultiRateBkgList1VO[])getVOs(request, MultiRateBkgList1VO.class));
		}
		else if (command.isCommand(FormCommand.MULTI02)) {	// Release
			event.setMultiRateBkgList1VOs((MultiRateBkgList1VO[])getVOs(request, MultiRateBkgList1VO.class));
		}

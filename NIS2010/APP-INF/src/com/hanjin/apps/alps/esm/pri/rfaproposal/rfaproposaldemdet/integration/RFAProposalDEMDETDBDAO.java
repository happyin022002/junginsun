...
/**
 * ALPS RfaProposalDEMDETDBDAO <br>
 * - ALPS-RFAProposal system Business Logic을 처리하기 위한 JDBC 작업수행.<br>
 * 
 * @author JaeYeon Kim
...
public class RFAProposalDEMDETDBDAO extends DBDAOSupport {
     ...
     * @exception DAOException
     */
    public void addProposalDmdtAmendBasic(List<RsltRoutHdrSmryListVO> insModels) throws DAOException,Exception {
        try {
            SQLExecuter sqlExe = new SQLExecuter("");
            int insCnt[] = null;
            if(insModels.size() > 0){
                insCnt = sqlExe.executeBatch((ISQLTemplate)new RFAProposalDEMDETDBDAOPriRpDmdtAmendBasicVOCSQL(), insModels,null);
                for(int i = 0; i < insCnt.length; i++){
                    if(insCnt[i]== Statement.EXECUTE_FAILED)
                        throw new DAOException("Fail to insert No"+ i + " SQL");
                }
     ...
     * @exception DAOException
     */
    public int addCopyRfaQuotationPriRqCpPriRpDmdt(RsltCopyToProposalVO vo) throws DAOException {
        // query parameter
        Map<String, Object> param = new HashMap<String, Object>();
        // velocity parameter
        Map<String, Object> velParam = new HashMap<String, Object>();
        try {
            ...
            SQLExecuter sqlExe = new SQLExecuter("");
            int result = sqlExe
                    .executeUpdate((ISQLTemplate) new RFAProposalDEMDETDBDAORqCpPriRpDmdtCSQL(),
                            param, velParam);
            if (result == Statement.EXECUTE_FAILED) {
                throw new DAOException("Fail to insert SQL");
            }

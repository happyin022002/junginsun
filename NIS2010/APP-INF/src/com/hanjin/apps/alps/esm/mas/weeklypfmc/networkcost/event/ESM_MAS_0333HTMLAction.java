...
*@FileName : ESM_MAS_0333HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Jong-Ock KIM
 * @see EsmMas0333Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0333HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0333HTMLAction 객체를 생성
	 */
	public ESM_MAS_0333HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0333Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0333Event event = new EsmMas0333Event();  		
    	...
    	if(command.isCommand(FormCommand.INIT)) {
			...
			event.setAverageUCVesselUtVOs((AverageUCVesselUtVO[])getVOs(request, AverageUCVesselUtVO.class));
    	} else if(command.isCommand(FormCommand.COMMAND01)) {
				event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		} else if(command.isCommand(FormCommand.SEARCH10)) {
			event.setAverageUCVesselUtVO((AverageUCVesselUtVO)getVO(request, AverageUCVesselUtVO.class));
		}

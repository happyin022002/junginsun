...
*@FileName : AGNCommRequestDBDAOAddAcmAgnCommFxCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AGNCommRequestDBDAOAddAcmAgnCommFxCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public AGNCommRequestDBDAOAddAcmAgnCommFxCSQL(){
		setQuery();
		...
		params.put("usd_fx_comm",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("pay_fx_comm",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : AGNCommRequestDBDAOAddAcmAgnCommFxCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(", (CASE WHEN ROUND( @[usd_fx_comm],2) - @[ppd_crnt_amt] < 0 AND @[max_ac_seq] < 2 THEN 'CN'  " ).append("\n"); 
		query.append("        WHEN ROUND( @[usd_fx_comm],2) - @[ppd_crnt_amt] = 0 THEN 'CZ'" ).append("\n"); 
		query.append("        ELSE 'CS'" ).append("\n"); 
		query.append("   END) AS AC_STS_CD" ).append("\n"); 
		query.append(", @[ppd_crnt_amt]                                 AS PPD_AMT" ).append("\n"); 
		query.append(", ROUND( @[usd_fx_comm],2)                       AS CRNT_AMT" ).append("\n"); 
		query.append(", ROUND( @[usd_fx_comm],2) - @[ppd_crnt_amt]      AS IF_AMT" ).append("\n"); 
		...
		query.append("       THEN ROUND(@[pay_xch_rt] * @[ppd_crnt_amt],2)" ).append("\n"); 
		query.append("       ELSE TO_NUMBER(@[ppd_pay_crnt_amt]) " ).append("\n"); 
		query.append("  END                                                     AS PAY_PPD_AMT" ).append("\n"); 
		query.append(", CASE WHEN (SELECT COUNT(DISTINCT CURR_CD) FROM ACM_AGN_COMM WHERE BKG_NO = @[bkg_no] AND AGN_CD = @[agn_cd] AND IO_BND_CD = @[io_bnd_cd] AND AC_TP_CD = @[ac_tp_cd] AND CURR_CD <> @[ofc_curr_cd]) > 0  " ).append("\n"); 
		query.append("       THEN ROUND( @[pay_xch_rt] * ROUND( @[usd_fx_comm],2),2)" ).append("\n"); 
		query.append("       ELSE ROUND( @[pay_fx_comm],2)" ).append("\n"); 
		query.append("  END                                                     AS PAY_CRNT_AMT" ).append("\n"); 
		query.append(", CASE WHEN (SELECT COUNT(DISTINCT CURR_CD) FROM ACM_AGN_COMM WHERE BKG_NO = @[bkg_no] AND AGN_CD = @[agn_cd] AND IO_BND_CD = @[io_bnd_cd] AND AC_TP_CD = @[ac_tp_cd] AND CURR_CD <> @[ofc_curr_cd]) > 0  " ).append("\n"); 
		query.append("       THEN ROUND( @[pay_xch_rt] * (ROUND( @[usd_fx_comm],2) - @[ppd_crnt_amt]),2)" ).append("\n"); 
		query.append("       ELSE ROUND( @[pay_fx_comm],2) - @[ppd_pay_crnt_amt]" ).append("\n"); 
		...
		query.append(", (CASE WHEN @[usd_fx_comm] > 0 THEN 'Calculation-Success' " ).append("\n"); 
		query.append("        WHEN @[usd_fx_comm] = 0 THEN 'Calculation-ZeroAMT'  " ).append("\n"); 
		...
		query.append(", @[usr_id]" ).append("\n"); 
		query.append(", SYSDATE" ).append("\n"); 
		query.append(", NULL AS WHLD_TAX_RT" ).append("\n"); 

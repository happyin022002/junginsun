...
public class StatusReportDBDAOBkgCMCroChkListByHBLVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("              ,CASE   WHEN HBL.PCK_QTY = SUM(VVD.CM_PCK) OVER( PARTITION  BY HBL.HBL_NO)  THEN  'Y'" ).append("\n"); 
		query.append("                      WHEN (BKG.USA_CSTMS_FILE_CD = '1' OR BKG.CND_CSTMS_FILE_CD = '1') AND " ).append("\n"); 
		query.append("                           (HBL.PCK_QTY IS NULL OR HBL.PCK_QTY != SUM(VVD.CM_PCK) OVER( PARTITION  BY HBL.HBL_NO) )THEN 'E'" ).append("\n"); 
		...
		query.append("   " ).append("\n"); 
		query.append("              ,CASE WHEN HBL.HBL_WGT  = SUM(VVD.CM_WGT) OVER( PARTITION  BY HBL.HBL_NO)  THEN  'Y'" ).append("\n"); 
		query.append("                      WHEN (BKG.USA_CSTMS_FILE_CD = '1' OR BKG.CND_CSTMS_FILE_CD = '1') AND " ).append("\n"); 
		query.append("                           (HBL.HBL_WGT IS NULL OR HBL.HBL_WGT != SUM(VVD.CM_WGT) OVER( PARTITION  BY HBL.HBL_NO) )THEN 'E'              " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("              ,CASE WHEN HBL.CMDT_MEAS_QTY = SUM(VVD.CM_MEAS) OVER( PARTITION  BY HBL.HBL_NO) THEN  'Y'" ).append("\n"); 

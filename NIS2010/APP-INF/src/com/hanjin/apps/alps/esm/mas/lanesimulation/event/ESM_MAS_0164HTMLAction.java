...
*@FileName       : ESM_MAS_169HTMLAction.java
*@FileTitle      : 
*Open Issues     :
*Change history  :
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.esm.mas.lanesimulation 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @since J2EE 1.6
 */
public class ESM_MAS_0164HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0169HTMLAction 객체를 생성
	 */
	public ESM_MAS_0164HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmMas0164Event event = new EsmMas0164Event();
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setMasSimNonOpExpnVOS((MasSimNonOpExpnVO[])getVOs(request, MasSimNonOpExpnVO.class));
		}
		return  event;
	}
	/**

...
*@FileName : ESM_BKG_0412HTMLAction.java
 ...
 * @see EsmBkg0412Event 참조
 * @since J2EE 1.5
 */

public class ESM_BKG_0412HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0421HTMLAction 객체를 생성
     */
    public ESM_BKG_0412HTMLAction() {}

    /**
     * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
     * HttpRequst의 정보를 EsmBkg0412Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0412Event event = new EsmBkg0412Event();
		
		if (command.isCommand(FormCommand.SEARCH)){ // Queue History 조회
			event.setBkgDpcsDocWeightVO ((BkgDpcsDocWeightVO) getVO(request, BkgDpcsDocWeightVO.class));
		//}else if (command.isCommand(FormCommand.SEARCH01)){//User Group Id Search
		} else if(command.isCommand(FormCommand.MULTI)) {
			log.debug(request.getParameter("ibflag"));
			event.setBkgDpcsDocWeightVOs((BkgDpcsDocWeightVO[])getVOs(request, BkgDpcsDocWeightVO.class,"sheet1_"));
		}

...
*@FileName : esm_bkg_0999HTMLAction.java
...
public class ESM_BKG_0999HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * esm_bkg_0999HTMLAction 객체를 생성
     */
    public ESM_BKG_0999HTMLAction() {}
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EsmBkg0999Event event = new EsmBkg0999Event();

        if(command.isCommand(FormCommand.MULTI)) {
            event.setBkgKorDoAttorneyDtlVOs((KorDoAttorneyDtlVO[])getVOs(request, KorDoAttorneyDtlVO .class,"sheet1_"));
        }
        else if(command.isCommand(FormCommand.SEARCH)) {
            event.setBkgKorDoAttorneyDtlVO((KorDoAttorneyDtlVO)getVO(request, KorDoAttorneyDtlVO .class));
        }
        //else if(command.isCommand(FormCommand.SEARCH01)) {
            //2009-06-09 서버단에서 중복 체크 확인
            //event.setBkgKorDoAttorneyDtlVO((BkgKorDoAttorneyDtlVO)getVO(request, BkgKorDoAttorneyDtlVO .class));
        //}

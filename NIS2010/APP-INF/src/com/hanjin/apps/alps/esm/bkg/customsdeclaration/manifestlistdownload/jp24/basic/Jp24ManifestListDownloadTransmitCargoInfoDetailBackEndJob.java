 ...
 *@FileName : Jp24ManifestListDownloadTransmitCargoInfoDetailBackEndJob
 *@FileTitle : Jp24ManifestListDownloadTransmitCargoInfoDetailBackEndJob
 *Open Issues :
 *Change history : 
 ...
 * @since J2EE 1.6
 */
public class Jp24ManifestListDownloadTransmitCargoInfoDetailBackEndJob extends BackEndCommandSupport {
	...
	public String doStart() throws Exception {
		...
		try {
			...
			for (int i=0; i<detailVOs.length; i++) {
				...
				if ("Y".equals(headerVO.getDelTrasmitFlag())) {
				...
				ediAdvJpCommonVOList = dbDao.searchEdiAdvJpBl(vvd, blNo, headerVO.getPolSplitNo(), headerVO.getPodSplitNo());
				...
				String pol4Naccs = ediAdvJpCommonVOList.get(0).getData06();
				String pod4Naccs = ediAdvJpCommonVOList.get(0).getData18();
				String etdFullInfo = DateTime.getFormatDate(ediAdvJpCommonVOList.get(0).getData10() + ediAdvJpCommonVOList.get(0).getData11(), "yyyyMMddHHmm", "yyyy-MM-dd HH:mm");
				...
				japanManifestListSendHeaderInfoVOList =  japanCustomsTransmissionDBDAO.searchSendHeader(vvd, polCd, account.getOfc_cd(), account.getUsr_id().replace("_", ""), detailVOs[i].getTSType(), sendDate, flatFileBodyLength, "");

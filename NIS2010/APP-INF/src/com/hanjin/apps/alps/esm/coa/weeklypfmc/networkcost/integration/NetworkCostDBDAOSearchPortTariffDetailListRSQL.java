...
public class NetworkCostDBDAOSearchPortTariffDetailListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  *                  터미널별 PSO 및 COA Data 수정가능토록 조회 쿼리
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("              , SUM((CASE WHEN SUBSTR(NVL(A4.YD_CD, A5.TML_CD), 1, 5) IN ('EGSUZ', 'PAPAC') THEN 0 ELSE A6.INV_USD_AMT END) ) PSO_PORT_AMT" ).append("\n"); 
		query.append("              , SUM(A5.PORT_USD_AMT) PORT_USD_AMT" ).append("\n"); 
		query.append("              , SUM((CASE WHEN SUBSTR(NVL(A4.YD_CD, A5.TML_CD), 1, 5) IN ('EGSUZ', 'PAPAC') THEN A6.INV_USD_AMT ELSE 0 END) ) PSO_CNL_AMT" ).append("\n"); 
		...
		query.append("                BETWEEN NVL(TO_CHAR(A2.VSL_APLY_FM_DT, 'YYYYMMDD'), '19000101') AND NVL(TO_CHAR(A2.VSL_APLY_TO_DT, 'YYYYMMDD'), '99991231')" ).append("\n"); 
		...
		query.append("            AND A1.SLS_YRMON LIKE SUBSTR(@[f_yearweek],1,4) || '%' AND A1.COST_WK = SUBSTR(@[f_yearweek], 6,2)" ).append("\n"); 

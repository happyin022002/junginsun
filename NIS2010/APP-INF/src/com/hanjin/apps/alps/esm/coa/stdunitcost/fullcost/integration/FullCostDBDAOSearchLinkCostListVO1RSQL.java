...
public class FullCostDBDAOSearchLinkCostListVO1RSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * COA_LNK_AVG_STND_COST, COA_COST_SRC_ACCT, COA_STND_ACCT 테이블의 데이터 조회(Full)
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append(",COA_GET_COM_NM_FNC('COST_ACT_GRP_CD', B2.COST_ACT_GRP_CD) GRP" ).append("\n"); 
		...
		query.append("AND LNK_FM_NOD_CD = NVL(COA_LOC_FNC(@[f_from], DECODE(@[f_cost_loc_grp_cd], 'S', 'SCC','E' ,'ECC', 'R', 'RCC', 'NODE')), @[f_from])" ).append("\n"); 
		query.append("AND LNK_TO_NOD_CD = NVL(COA_LOC_FNC(@[f_to], DECODE(@[f_cost_loc_grp_cd], 'S', 'SCC','E' ,'ECC', 'R'	, 'RCC', 'NODE')), @[f_to])" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("GROUP BY LNK_FM_NOD_CD, LNK_TO_NOD_CD, COA_COST_SRC_CD) B1" ).append("\n"); 
		...
		query.append("FROM COA_COST_SRC_ACCT A1, COA_STND_ACCT A2, COA_ACT_GRP_COST_MAPG A3" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		...
		query.append("GROUP BY A3.COST_ACT_GRP_CD, A1.STND_COST_CD, A1.COA_COST_SRC_CD) B2" ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		...
		query.append("GROUP BY B1.LNK_FM_NOD_CD, B1.LNK_TO_NOD_CD, B2.COST_ACT_GRP_CD, B2.STND_COST_CD, CUBE(B2.COA_COST_SRC_CD)" ).append("\n"); 

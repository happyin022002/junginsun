 ...
 *@FileName : ESM_PRI_7023HTMLAction.java
 ...
 * 
 * @author LEE EUN SUP
 * @see EsmPri7023Event 참조
 * @since J2EE 1.6
 */
public class ESM_PRI_7023HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 7116963942112395646L;

	/**
	 * ESM_PRI_7023HTMLAction 객체를 생성
	 */
	public ESM_PRI_7023HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri7023Event event = new EsmPri7023Event();
		if (command.isCommand(FormCommand.SEARCHLIST)) {
			event.setSvcScpCd(JSPUtil.getParameter(request, "svcScpCd"));
			event.setFdrTrfNo(JSPUtil.getParameter(request, "fdrTrfNo"));
			event.setVerMapgSeq(JSPUtil.getParameter(request, "verMapgSeq"));
		}
		return event;
	}

...
*@FileName : ESM_PRI_6084HTMLAction.java
...
public class ESM_PRI_6084HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6084HTMLAction 객체를 생성
	 */
	public ESM_PRI_6084HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri6084Event event = new EsmPri6084Event();
 
		
		if(command.isCommand(FormCommand.MULTI)) {
			RsltTriPrsCostListVO[] vos = (RsltTriPrsCostListVO[])getVOs(request, RsltTriPrsCostListVO .class);
			event.setRsltTriPrsCostListVOS(vos);			
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setRsltTriPrsCostListVO((RsltTriPrsCostListVO)getVO(request, RsltTriPrsCostListVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setRsltTriPrsCostDetailVO((RsltTriPrsCostDetailVO)getVO(request, RsltTriPrsCostDetailVO .class));

		}else if(command.isCommand(FormCommand.MULTI01)) {
			RsltTriPrsCostListVO[] vos = (RsltTriPrsCostListVO[])getVOs(request, RsltTriPrsCostListVO .class);
			event.setRsltTriPrsCostListVOS(vos);
			//PRI_TRI_RT_USD_ROUT_CS
			 
			PriTriRtUsdRoutCsVO routCsVO = (PriTriRtUsdRoutCsVO)getVO(request, PriTriRtUsdRoutCsVO .class);
			event.setPriTriRtUsdRoutCsVO(routCsVO); 

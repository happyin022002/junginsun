...
*@FileName : ESM_BKG_0594HTMLAction.java
...
public class ESM_BKG_0594HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0594HTMLAction 객체를 생성
	 */
	public ESM_BKG_0594HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0594Event event = new EsmBkg0594Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			log.debug(request.getParameter("ibflag"));
			event.setCgoClzTmStupVOs((CgoClzTmStupVO[])getVOs(request, CgoClzTmStupVO .class,""));
		} else if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01)) {
			event.setCgoClzTmStupVO((CgoClzTmStupVO)getVO(request, CgoClzTmStupVO .class));
		}

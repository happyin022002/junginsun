...
package com.hanjin.apps.alps.esm.bkg.customsdeclaration.manifestlistdownload.eur24.basic;
 ...
 * @since J2EE 1.4
 */
public class Eur24ManifestDownloadBCImpl extends ManifestListDownloadBCImpl {
	...
	public List<ManifestListDetailVO> searchManifestList(ManifestListCondVO manifestListCondVO) throws EventException {
		try{
			...
			*/
			return dbDao.searchEur24BlManifestList(manifestListCondVO);
	 ...
	 * @throws EventException
	 */
	public Eur24BlInfoVO searchBlInfo(Eur24BlInfoCondVO eur24BlInfoCondVO) throws EventException {
		try{
			// 파라메터 변환
			...
			Eur24BlGeneralInfoVO eur24BlGeneralInfoVO = dbDao.searchBlGeneral(blNo, vvd, cstmsPortCd,cstmsYdCd);
			if(eur24BlGeneralInfoVO != null){
				//cstmsPortCd = eur24BlGeneralInfoVO.getCstmsPortCd();
				if(vvd == null || vvd.equals("")) vvd = eur24BlGeneralInfoVO.getVvd();
				if(cstmsPortCd == null || cstmsPortCd.equals("")) vvd = eur24BlGeneralInfoVO.getCstmsPortCd();
	...
	public List<Eu24ExsListOBVO> searchEXSReportOB(Eu24ExsListOBVO eu24ExsListOBVO) throws EventException  {
		try{
			List<Eu24ExsListOBVO>  eu24ExsListOBVOs ;
			Eu24ExsListOBVO listVO = new Eu24ExsListOBVO();
			EU24EDIHistoryOBVO eU24EDIHistoryOBVO = new EU24EDIHistoryOBVO();
			
			eu24ExsListOBVOs = dbDao.searchEXSReportOB(eu24ExsListOBVO);
			
		    for (int i = 0; i <eu24ExsListOBVOs.size(); i++){
		    	listVO =  eu24ExsListOBVOs.get(i);
		    	
		    	System.out.println("*******1*********");
		    	if((listVO.getResult().equals("") || listVO.getResult() == null )&&listVO.getAck().equals("N/A")){
		    		System.out.println(listVO.getPBlNo() + "----"+ listVO.getBlNo());	
		    		eU24EDIHistoryOBVO = dbDao.searchEU24EDIHoldHistoryOB(listVO.getBlNo());
		    		System.out.println("*******3*********");
		    		if (eU24EDIHistoryOBVO != null) {
		    			eu24ExsListOBVOs.get(i).setResult(eU24EDIHistoryOBVO.getResult());
		    		}
		    		System.out.println("*******4*********");
		    	}
		    	
		    }
		   return eu24ExsListOBVOs;
		}catch (DAOException ex){
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), ex);
		}catch (Exception ex){
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), ex);

...
public class TransshipmentMgtDBDAOsearchRlyVslGrpAssignRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        , case when bk.dcgo_flg     = 'Y' then 'DG'" ).append("\n"); 
		query.append("               when bk.rc_flg       = 'Y' then 'RF'" ).append("\n"); 
		query.append("               when bk.awk_cgo_flg  = 'Y' then 'AK'" ).append("\n"); 
		query.append("               when bk.bb_cgo_flg   = 'Y' then 'BB' end spcl" ).append("\n"); 
		...
		query.append("   #end" ).append("\n"); 
		query.append("   #if (${blank_former_vvd}=='Y')" ).append("\n"); 
		query.append("    and (former_vvd.vsl_cd is null or former_vvd.vsl_cd in ('SMXX','SMYY','SMZZ'))" ).append("\n"); 
		query.append("    and bk.bkg_cre_dt > sysdate - 180" ).append("\n"); 
		query.append("   #end" ).append("\n"); 
		query.append("   #if (${blank_post_vvd}=='Y')" ).append("\n"); 

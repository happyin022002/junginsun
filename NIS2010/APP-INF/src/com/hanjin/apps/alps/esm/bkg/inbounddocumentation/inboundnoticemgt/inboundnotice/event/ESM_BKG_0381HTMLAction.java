...
*@FileName : ESM_BKG_0381HTMLAction.java
 ...
 * @see EsmBkg0381Event 참조
 * @since J2EE 1.4
 */

public class ESM_BKG_0381HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0381HTMLAction 객체를 생성
	 */
	public ESM_BKG_0381HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0381Event event = new EsmBkg0381Event();
		
		
		//FAX 전송
		if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchVO((ArrivalNoticeSearchVO) getVO(request, ArrivalNoticeSearchVO.class));
			
			ArrNtcSendListVO[] listVOS = (ArrNtcSendListVO[]) getVOs(request, ArrNtcSendListVO.class,"t1sheet1_");
		...
		//E-Mail 전송
			...
			if(listVOS != null && listVOS.length > 0){
				listVOS[0].setEdtSubject(request.getParameter("edt_subject"));
				listVOS[0].setEdtContents(request.getParameter("edt_contents"));
			}
			event.setListVOS(listVOS);
		...
		//RD id 조회
			...
			arrNtcMrdSearch.setBkgNo(bkgNo);
			
			event.setArrNtcMrdSearch(arrNtcMrdSearch);
			//event.setMdmCustomerVO((MdmCustomerVO) getVO(request, MdmCustomerVO.class));
		}

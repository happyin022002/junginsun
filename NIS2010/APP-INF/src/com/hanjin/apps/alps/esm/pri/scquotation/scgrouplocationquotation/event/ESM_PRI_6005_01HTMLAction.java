...
*@FileName : ESM_PRI_6005_1HTMLAction.java
...
public class ESM_PRI_6005_01HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6005_1HTMLAction 객체를 생성
	 */
	public ESM_PRI_6005_01HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri600501Event event = new EsmPri600501Event();
		...
		if (command.isCommand(FormCommand.SEARCH01)) {
			event.getGroupLocationQuotationVO().setPriSqGrpLocVO((PriSqGrpLocVO) getVO(request, PriSqGrpLocVO.class));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.getGroupLocationQuotationVO().setPriSqGrpLocVO((PriSqGrpLocVO) getVO(request, PriSqGrpLocVO.class));
		} else if (command.isCommand(FormCommand.MULTI01)) {
			event.getGroupLocationQuotationVO().setPriSqGrpLocVO((PriSqGrpLocVO) getVO(request, PriSqGrpLocVO.class));
			event.getGroupLocationQuotationVO().setPriSqGrpLocVOs((PriSqGrpLocVO[]) getVOs(request, PriSqGrpLocVO.class, "sheet1_"));
			event.getGroupLocationQuotationVO().setPriSqGrpLocDtlVOs((PriSqGrpLocDtlVO[]) getVOs(request, PriSqGrpLocDtlVO.class, "sheet2_"));
		} else if (command.isCommand(FormCommand.MULTI02)) {
			event.getGroupLocationQuotationVO().setRsltPriSqMnVO((RsltPriSqMnVO) getVO(request, RsltPriSqMnVO.class));
		} else if (command.isCommand(FormCommand.SEARCH03)) {
			event.getGroupLocationQuotationVO().setPriSqGrpLocVOs((PriSqGrpLocVO[]) getVOs(request, PriSqGrpLocVO.class));
			event.getGroupLocationQuotationVO().setPriSqGrpLocVO((PriSqGrpLocVO) getVO(request, PriSqGrpLocVO.class));
		} 

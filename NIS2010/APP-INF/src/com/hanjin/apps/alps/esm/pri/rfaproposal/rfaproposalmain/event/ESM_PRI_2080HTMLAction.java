...
*@FileName : ESM_PRI_2080HTMLAction.java
...
public class ESM_PRI_2080HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_2080HTMLAction 객체를 생성
	 */
	public ESM_PRI_2080HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri2080Event event = new EsmPri2080Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
            event.setRpScpGlineCopyVO((RpScpGlineCopyVO)getVO(request, RpScpGlineCopyVO.class));
        } else if(command.isCommand(FormCommand.MULTI)) {
			event.setRpScpGlineCopyVOS((RpScpGlineCopyVO[])getVOs(request, RpScpGlineCopyVO.class,""));
		} else {
		    event.setRpScpGlineCopyVO((RpScpGlineCopyVO)getVO(request, RpScpGlineCopyVO.class));
		}

...
*@FileName : RFAProposalMainDBDAOsearchEmailTargetUserListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFAProposalMainDBDAOsearchEmailTargetUserListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public RFAProposalMainDBDAOsearchEmailTargetUserListRSQL(){
		setQuery();
		...
		query.append("FileName : RFAProposalMainDBDAOsearchEmailTargetUserListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        ,PRI_RP_HDR A3" ).append("\n"); 
		query.append("        ,PRI_RP_SCP_MN A4" ).append("\n"); 
		query.append("        ,BKG_HIS_DTL A5" ).append("\n"); 
		query.append("    WHERE A1.BKG_STS_CD <> 'X'" ).append("\n"); 
		query.append("      AND A1.BKG_NO = A2.BKG_NO" ).append("\n"); 
		query.append("      AND A1.RFA_NO = A3.RFA_NO" ).append("\n"); 
		query.append("      AND A3.PROP_NO = A4.PROP_NO" ).append("\n"); 
		query.append("      AND A4.PROP_NO =  @[prop_no]  -- approve 대상 RFA PROP NO" ).append("\n"); 
		query.append("      AND A4.AMDT_SEQ = @[amdt_seq] -- approve 대상 RFA Amend No." ).append("\n"); 
		query.append("      AND A4.SVC_SCP_CD = A1.SVC_SCP_CD" ).append("\n"); 
		query.append("      AND (A2.RT_APLY_DT BETWEEN A4.EFF_DT AND A4.EXP_DT --approve 한 RFA scope의 effective date 이후에 application date가 존재하는 경우" ).append("\n"); 
		query.append("          OR A4.EXP_DT <A2.RT_APLY_DT) -- approve 한 RFA scope의 exp date 이후에 rate 날짜가 있는 경우 -- 적용 RFA가 없어지면서 다시 rating 해야 함" ).append("\n"); 

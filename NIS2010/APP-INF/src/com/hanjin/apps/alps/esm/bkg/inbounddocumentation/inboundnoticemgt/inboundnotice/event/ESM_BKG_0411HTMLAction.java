...
*@FileName : ESM_BKG_0411HTMLAction.java
...
public class ESM_BKG_0411HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0411HTMLAction 객체를 생성
	 */
	public ESM_BKG_0411HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0411Event event = new EsmBkg0411Event();
	
		if(command.isCommand(FormCommand.SEARCH)) {
			
			event.setNtcSndTpCd(JSPUtil.getParameter(request, "ntc_snd_tp_cd"));
			...
			BkgPkupNtcStupVO[] bkgPkupNtcStupVOs = (BkgPkupNtcStupVO[])getVOs(request, BkgPkupNtcStupVO.class,"sheet1_");
			BkgPkupNtcStupVO bkgPkupNtcStupVO = new BkgPkupNtcStupVO();
			if (bkgPkupNtcStupVOs!= null && bkgPkupNtcStupVOs.length > 0) bkgPkupNtcStupVO = bkgPkupNtcStupVOs[0];
			                 
			event.setBkgPkupNtcStupVO(bkgPkupNtcStupVO);
			event.setBkgPkupWdVOs((PkupWdVO[])getVOs(request, PkupWdVO.class, "sheet2_"));
			event.setBkgPkupNtcHrVOs((PkupNtcHrVO[])getVOs(request, PkupNtcHrVO.class, "sheet3_"));
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setPkupNtcSeq(JSPUtil.getParameter(request, "pkup_ntc_seq"));
		}

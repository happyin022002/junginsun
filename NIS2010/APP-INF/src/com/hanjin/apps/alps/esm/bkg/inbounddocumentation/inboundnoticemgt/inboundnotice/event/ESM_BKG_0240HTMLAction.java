...
*@FileName : ESM_BKG_0240HTMLAction.java
...
public class ESM_BKG_0240HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0240HTMLAction 객체를 생성
	 */
	public ESM_BKG_0240HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0240Event event = new EsmBkg0240Event();
		...
		if(command.isCommand(FormCommand.MULTI01)) {
			...
			//저장실행
			event.setBkgIbCustCntcVOs((BkgIbCustCntcVO[])getVOs(request, BkgIbCustCntcVO.class,"t1sheet_"));
		}		
		// MDM Master 조회
		else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setIntgCustSearchVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			log.debug("--------------------- "+event.getIntgCustSearchVO().getColumnValues());
			log.debug("--------------------- "+request.getParameter("cust_cnt_cd_ext"));
			//event.setMdmCustomerVO((MdmCustomerVO) getVO(request, MdmCustomerVO.class));
		}		
		// I/B Detail 조회
		else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setIntgCustSearchVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			//event.setBkgIbCustCntcVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			log.debug("---------------- event.setIntgCustSearchVO() [" + event.getIntgCustSearchVO().getColumnValues() + "]");
		}
		// O/B Detail 조회
		...
		// Invoice Detail 조회
			...
			//event.setIntgCustSearchVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			//event.setBkgCustTmpltVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			event.setMdmCrCustVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
		}
		// TRO Detail 조회
		else if(command.isCommand(FormCommand.SEARCH05)) {
			//event.setIntgCustSearchVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
			event.setBkgTroActCustVO((IntgCustSearchVO)getVO(request, IntgCustSearchVO.class));
		}

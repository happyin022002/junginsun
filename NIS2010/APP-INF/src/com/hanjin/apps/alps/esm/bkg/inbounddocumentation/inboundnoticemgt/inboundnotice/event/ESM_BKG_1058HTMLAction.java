...
*@FileName : ESM_BKG_1058HTMLAction.java
...
public class ESM_BKG_1058HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1058HTMLAction 객체를 생성
	 */
	public ESM_BKG_1058HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1058Event event = new EsmBkg1058Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setBkgPkupCntrRtnYdVOS((BkgPkupCntrRtnYdVO[])getVOs(request, BkgPkupCntrRtnYdVO .class, "sheet1_"));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {	
			event.setPkupCntrRtnYdVO((PkupCntrRtnYdVO)getVO(request, PkupCntrRtnYdVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {	
			event.setChkTp(JSPUtil.getParameter(request, "chk_tp"));

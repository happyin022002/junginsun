...
public class TCharterIOConsultationEAIDAO extends EAIDAOSupport {
	 ...
	 * @throws DAOException
	 */
	public void sendSlipApprovalToAP(String csrNo, List<SearchApSlipInterfaceListVO> searchApSlipInterfaceListVO, String agmtCntYn) throws DAOException {
	        ...
	        for (int i=0; i<arrLeng; i++) {
	        	...
	        	for (int j=voRowCnt; j<listSize; j++) {
	    			...
	    			setInvoiceDatas(searchApSlipInterfaceListVO.get(j), 
	    					invoiceReq, 
	    					timeStamp, listSize, currRow, agmtCntYn);
	...
	private void setInvoiceDatas(SearchApSlipInterfaceListVO searchApSlipInterfaceListVO, 
			...
			String timeStamp, int totCnt, int currRow, String agmtCntYn) throws Exception {
		...
		try {
			...
			if (!agmtCntYn.equals("")) { // Prepayments Settlement G/W 결재 안함
				invoiceReq.setHREFERENCEKEY1(agmtCntYn);
			} else {
			   invoiceReq.setHREFERENCEKEY1(JSPUtil.getNull(new ApprovalUtil().searchAgmtCfmCd(searchApSlipInterfaceListVO.getHdrCsrNo())));
			}

...
public class ESM_PRI_6005_02HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_6005_02HTMLAction 객체를 생성
	 */
	public ESM_PRI_6005_02HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri600502Event event = new EsmPri600502Event();
		...
		if (command.isCommand(FormCommand.SEARCH01)) {
			...
			event.getGroupCommodityQuotationVO().setPriSqGrpCmdtVO((PriSqGrpCmdtVO) getVO(request, PriSqGrpCmdtVO.class));
			event.getGroupCommodityQuotationVO().setPriSqGrpCmdtVOs((PriSqGrpCmdtVO[]) getVOs(request, PriSqGrpCmdtVO.class, "sheet1_"));
			event.getGroupCommodityQuotationVO().setPriSqGrpCmdtDtlVOs((PriSqGrpCmdtDtlVO[]) getVOs(request, PriSqGrpCmdtDtlVO.class, "sheet2_"));
		} else if (command.isCommand(FormCommand.MULTI02)) {
			event.getGroupCommodityQuotationVO().setRsltPriSqMnVO((RsltPriSqMnVO) getVO(request, RsltPriSqMnVO.class));
		} else if (command.isCommand(FormCommand.SEARCH03)) {
			event.getGroupCommodityQuotationVO().setPriSqGrpCmdtVOs((PriSqGrpCmdtVO[]) getVOs(request, PriSqGrpCmdtVO.class));
//			event.getGroupCommodityQuotationVO().setPriSqGrpCmdtVO((PriSqGrpCmdtVO) getVO(request, PriSqGrpCmdtVO.class));
		}

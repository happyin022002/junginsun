...
public class ESM_BKG_066801HTMLAction extends HTMLActionSupport {
     ...
     */
    public ESM_BKG_066801HTMLAction() {}
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EsmBkg066801Event event = new EsmBkg066801Event();
...
//        String poNo     = "";
        ...
        log.debug("===================================");
        log.debug("    ESM_BKG_0668HTMLAction Start   ");
        log.debug("===================================");
        ...
        if(command.isCommand(FormCommand.SEARCH)) {
        	...
        	log.debug("hbl_no  ===> " + request.getParameter("hbl_no"));
        	
            hblNo  = request.getParameter("hbl_no");

            event.setHblNo(hblNo);

            log.debug("===================================");
            log.debug("    SEARCH03 EVENT                 ");
            log.debug("    hblNo   : "+ hblNo                );
            log.debug("===================================");
            
        }else if(command.isCommand(FormCommand.SEARCH09)) {		// searchTopInfo
            if (blNo.isEmpty() == false) {
    	        strTemp = blNo.substring(blNo.length()-1);
            ...
            log.debug("=======================================");
            log.debug("    SEARCH09 EVENT                     ");
            log.debug("    bkgNo  : "+ bkgNo                   );
            log.debug("    blNo   : "+ blNo                    );
            log.debug("=======================================");
        }else if(command.isCommand(FormCommand.SEARCH10)) {		// Cargo Release BlInfo
            if (blNo.isEmpty() == false) {
    	        strTemp = blNo.substring(blNo.length()-1);

...
public class SpacecontrolinquiryDBDAOsearchSpaceControlInquiryNoShowTradeListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * Ticket No :  CHM-201113515-01 김종준 [SPC] Tag Library 제거 작업 - AND N.AQ_CD(+)    = O.AQ_CD 조건추가
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("           TO_DATE(P_YEAR||R_MONTH||'01', 'YYYYMMDD') AS DT," ).append("\n"); 
		...
		query.append("       AND R_MONTH IS NOT NULL" ).append("\n"); 
		...
		query.append("       V.SLS_YRMON = W.COST_YR||W.COST_MON" ).append("\n"); 
		query.append("       and substr(V.SLS_YRMON,1,4)||V.COST_WK = W.COST_YR||W.COST_WK" ).append("\n"); 
		query.append("--       AND V.IOC_CD    = 'O'" ).append("\n"); 
		...
		query.append("#if (${bound} != '')" ).append("\n"); 
		query.append("	   AND V.DIR_CD = W.DIR_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("			   AND N.AQ_CD	 (+) = O.AQ_CD" ).append("\n"); 

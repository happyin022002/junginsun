...
public class ESM_BKG_1129HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1079HTMLAction 객체를 생성
	 */
	public ESM_BKG_1129HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1129Event event = new EsmBkg1129Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setEqSubErrSchVO((EqSubErrSchVO)getVO(request, EqSubErrSchVO.class));
		}
		else if(command.isCommand(FormCommand.MULTI01)) {
			
			event.setEqSubErrSchVOs((EqSubErrSchVO[])getVOs(request, EqSubErrSchVO.class));

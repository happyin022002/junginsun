...
*@FileName : AGNCommRequestDBDAOAddAcmAgnCHGCommCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AGNCommRequestDBDAOAddAcmAgnCHGCommCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Add Acm Agn CHG Comm
	  * </pre>
	  */
	public AGNCommRequestDBDAOAddAcmAgnCHGCommCSQL(){
		setQuery();
		...
		params.put("pay_chg_comm",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("usd_chg_comm",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : AGNCommRequestDBDAOAddAcmAgnCHGCommCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(", (CASE WHEN  @[pay_xch_rt] = 0 THEN 'CS'" ).append("\n"); 
		query.append("        WHEN ROUND( @[usd_chg_comm],2) - @[ppd_crnt_amt] < 0 AND @[max_ac_seq] = 1 THEN 'CN'  " ).append("\n"); 
		query.append("        WHEN ROUND( @[usd_chg_comm],2) - @[ppd_crnt_amt] = 0 THEN 'CZ'" ).append("\n"); 
		query.append("        WHEN ROUND( @[usd_chg_comm],2) <= 0 AND @[ppd_crnt_amt] = 0 AND @[max_ac_seq] > 2  THEN 'CZ' -- SEQ 2 로 이미 상계 처리 된 경우 최종 커미션 CRNT 금액이 0 이하이고, IF 금액이 0 일 때 CZ 로 만들어 주기 위한 로직 " ).append("\n"); 
		...
		query.append("       WHEN @[max_ac_seq] > 1  AND ROUND( @[usd_chg_comm],2) < 0 THEN 0" ).append("\n"); 
		query.append("       ELSE ROUND( @[usd_chg_comm],2) " ).append("\n"); 
		query.append("  END                                        AS CRNT_AMT" ).append("\n"); 
		query.append(", CASE WHEN  @[pay_xch_rt] = 0 THEN 0" ).append("\n"); 
		query.append("       WHEN @[max_ac_seq] > 1  AND ROUND( @[usd_chg_comm],2) < 0 THEN 0 - @[ppd_crnt_amt]" ).append("\n"); 
		query.append("       ELSE ROUND( @[usd_chg_comm],2) - @[ppd_crnt_amt]    " ).append("\n"); 
		...
		query.append(", 0 AS DDCT_VIP_AMT" ).append("\n"); 
		query.append(", 0 AS COMM_FX_AMT" ).append("\n"); 
		...
		query.append("       THEN  ROUND(@[pay_chg_comm], 2)" ).append("\n"); 
		query.append("       WHEN  @[max_ac_seq] > 1  AND ROUND(@[pay_chg_comm], 2) < 0 THEN 0" ).append("\n"); 
		query.append("       ELSE  ROUND(@[pay_chg_comm], 2)" ).append("\n"); 
		query.append("  END                                                                            AS PAY_CRNT_AMT                   " ).append("\n"); 
		query.append(", CASE WHEN @[pay_xch_rt] = 0 THEN 0" ).append("\n"); 
		query.append("       WHEN (SELECT COUNT(DISTINCT CURR_CD) FROM ACM_AGN_COMM WHERE BKG_NO = @[bkg_no] AND AGN_CD = @[agn_cd] AND IO_BND_CD = @[io_bnd_cd] AND AC_TP_CD = @[ac_tp_cd] AND CURR_CD <> @[ofc_curr_cd]) > 0  " ).append("\n"); 
		query.append("       THEN  ROUND(@[pay_chg_comm], 2) - ROUND(@[pay_xch_rt] * @[ppd_crnt_amt] ,2)" ).append("\n"); 
		query.append("       WHEN @[max_ac_seq] > 1  AND ROUND(@[pay_chg_comm], 2) < 0 THEN 0 - @[ppd_pay_crnt_amt]" ).append("\n"); 
		query.append("       ELSE  ROUND(@[pay_chg_comm], 2) - @[ppd_pay_crnt_amt]" ).append("\n"); 
		...
		query.append(", (CASE WHEN @[pay_xch_rt] = 0 THEN 'Calculation-Success'  " ).append("\n"); 
		query.append("        WHEN @[usd_chg_comm] > 0 THEN 'Calculation-Success'  " ).append("\n"); 
		query.append("        WHEN @[usd_chg_comm] = 0 THEN 'Calculation-ZeroAMT'  " ).append("\n"); 

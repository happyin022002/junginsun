...
public class AusCustomsTransmissionDBDAOsearchBlGeneralForPRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(" " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[port_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SM LINE CORPORATION (AUSTRALIA) PTY LTD',DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_NM,1),NULL," ).append("\n"); 
		...
		query.append(" " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[port_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SUITE 23 - 02, 1 MARKET STREET',DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_NM,2), NULL, " ).append("\n"); 
		query.append("           DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,1), NULL,   " ).append("\n"); 
		query.append("                DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2), NULL, " ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2))," ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,1)),SCE_TOKEN_NL_FNC(BCS.CUST_NM,2))) shpr2,  " ).append("\n"); 
		query.append(" " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[port_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SYDNEY NSW 2000',DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,1), NULL, " ).append("\n"); 
		query.append("           DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2), NULL,  " ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2))," ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,1))) shpr3," ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[port_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','',DECODE(SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2), NULL, " ).append("\n"); 
		query.append("           SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,2))) shpr4,        " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[port_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','',SCE_TOKEN_NL_FNC(BCS.CUST_ADDR,3)) shpr5,         " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[pol_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SM LINE CORPORATION (AUSTRALIA) PTY LTD',DECODE(B.CUST_TO_ORD_FLG,'Y',SCE_TOKEN_NL_FNC(BCN.CUST_NM,1)," ).append("\n"); 
		...
		query.append("           DECODE(SUBSTR(@[pol_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SUITE 23 - 02, 1 MARKET STREET',DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_NM,2), NULL, " ).append("\n"); 
		query.append("           DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,1), NULL,   " ).append("\n"); 
		query.append("                DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2), NULL, " ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2))," ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,1)),SCE_TOKEN_NL_FNC(BCC.CUST_NM,2))) cnee2,       " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[pol_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','SYDNEY NSW 2000',DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,1), NULL, " ).append("\n"); 
		query.append("           DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2), NULL,  " ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2))," ).append("\n"); 
		query.append("                SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,1))) cnee3,         " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[pol_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','',DECODE(SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2), NULL, " ).append("\n"); 
		query.append("           SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,3),SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,2))) cnee4,     " ).append("\n"); 
		query.append("           DECODE(SUBSTR(@[pol_cd],1,2)||NVL(B.BKG_CGO_TP_CD,''),'AUP','',SCE_TOKEN_NL_FNC(BCC.CUST_ADDR,3)) cnee5,       " ).append("\n"); 
		...
		query.append("        NVL(B.BKG_CGO_TP_CD,'') cargotype," ).append("\n"); 
		query.append("        NVL(CMD.CMDT_NM,'') commodity," ).append("\n"); 
		query.append("        TRANSLATE(NVL(B.XTER_RMK,' '),CHR(13)||CHR(10),' ') xter_rmk," ).append("\n"); 
		query.append("        '' aus_quar," ).append("\n"); 
		query.append("        NVL(B.BKG_NO,'') bkg_no," ).append("\n"); 
		query.append("		DECODE(NVL(B.BKG_CGO_TP_CD,''),'R','P',NVL(B.BKG_CGO_TP_CD,'')) bkg_cgo_tp_cd," ).append("\n"); 
		...
		query.append("        TRANSLATE(NVL(COM.REP_CMDT_NM,' '),CHR(13)||CHR(10),' ')  cmdt_desc," ).append("\n"); 
		query.append("#if (${pol_cd}!= '')" ).append("\n"); 
		query.append("        DECODE(@[pol_cd], B.POL_CD, 'E', 'T') ei_ind" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("        DECODE(@[pod_cd], B.POD_CD, 'I', 'T') ei_ind" ).append("\n"); 

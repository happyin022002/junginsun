...
public class ESM_BKG_0079_08HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			String ctrtOfcCd = JSPUtil.getParameter(request, "ctrt_ofc_cd");
			String ctrtSrepCd = JSPUtil.getParameter(request, "ctrt_srep_cd");
			String bkgPolCd = JSPUtil.getParameter(request, "frm_t10sheet1_pol_cd");
			String bkgCtrtTpCd = JSPUtil.getParameter(request, "frm_t10sheet1_bkg_ctrt_tp_cd");
			...
			event.setCtrtSrepCd(ctrtSrepCd);
			
            event.setBkgPolCd(bkgPolCd);
            event.setBkgCtrtTpCd(bkgCtrtTpCd);
			
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			String ctrtType = JSPUtil.getParameter(request, "ctrtType");
			String ctrtNo = JSPUtil.getParameter(request, "ctrtNo");
			String application_date = JSPUtil.getParameter(request, "applicationDate");
			event.setCtrtType(ctrtType);
			event.setCtrtNo(ctrtNo);
			event.setApplication_date(application_date);
		
		}else if(command.isCommand(FormCommand.SEARCH03)){
			String bkg_no = JSPUtil.getParameter(request, "bkg_no");
			String caflag = JSPUtil.getParameter(request, "ca_flag");
			event.setBkg_no(bkg_no);
			event.setCaflag(caflag);
		}

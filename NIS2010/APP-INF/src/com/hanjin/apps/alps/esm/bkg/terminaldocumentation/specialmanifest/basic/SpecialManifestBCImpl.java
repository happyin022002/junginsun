...
* 2013.02.07 김보배 [CHM-201322793] [BKG] [FRFOS, Customs EDI] Receiver ID mapping 요청
* 2014.06.03 김보배 [CHM-201430491] (ANRBS)DG Declare EDI Transmit TPID 변경 요청(Version up issue)
* 2015.04.29 이한나 : 긴급반영 - 김현경 대리님 요청 , DEWVN port - Receiver ID 추가
...
import com.hanjin.apps.alps.esm.bkg.terminaldocumentation.specialmanifest.integration.SpecialManifestDBDAO;
...
import com.hanjin.apps.alps.esm.bkg.terminaldocumentation.specialmanifest.vo.SpecialContainerSaveVO;
...
/**
 * NIS2010-TerminalDocumentation Business Logic Basic Command implementation<br>
 * - NIS2010-TerminalDocumentation에 대한 비지니스 로직을 처리한다.<br>
 *
...
public class SpecialManifestBCImpl extends BasicCommandSupport implements SpecialManifestBC {
	...
	public List<DgListDetailVO> searchDgManifestList(DgListCondVO dgListCondVO) throws EventException {
		...
		List<DgListDetailVO> vslInfoList 	= null; 
		List<DgListDetailVO> autoSentList 	= null; 
		List<DgListDetailVO> ediSentStatusList = null; 
		List<DgListDetailVO> ssrNoList 		= null; 
		
		SpecialContainerVO specialContainerVO= null;
		List<DgListDetailVO> retVOList = new ArrayList<DgListDetailVO>();
		
		String vslInfoLocalYn   = "";
		String dgListLocalYn   = "";
		...
		try {
			// 0. Local vessel 정보 조회
			vslInfoList = dbDao.searchVslInfo(dgListCondVO);
			
			
			if(vslInfoList != null) {
				if(vslInfoList.size() > 0) {
					 ...
					 */
					vslInfoList = dbDao.serachVslAtCommon(dgListCondVO);
					...
					// 기타 지역에서는 UVI를 조회함
					ssrNoList = dbDao.searchSSRNo(dgListCondVO);

				}
				
				
				// 1. 위험물 데이타 조회
				list = dbDao.searchDgInfoAtLocal(dgListCondVO);
				
				if(list != null) {
					
					if(list.size() > 0) {													// 세관쪽 데이타를 조회한다.
						
						dgListLocalYn = "Y";
						
					} else {																// booking쪽 데이타를 조회 한다.
						// 2. booking쪽 데이타를 조회
				...
				autoSentList = dbDao.searchAutoSentInfo(dgListCondVO);
				
				// EDI전송 결과를 조회 해 온다.
				ediSentStatusList = dbDao.searchEdiSentStatus(dgListCondVO);
	
				// ContainerVO에 setting
				specialContainerVO = new SpecialContainerVO();
				...
				if(vslInfoList != null && vslInfoList.size() > 0) {
					...
					vslInfoList.get(0).setLocalDbYn(vslInfoLocalYn);
					
					if(vslInfoLocalYn.equals("N")) {
						
						if(ssrNoList != null && ssrNoList.size() > 0) {
							vslInfoList.get(0).setSvcRqstNo(ssrNoList.get(0).getSvcRqstNo());
						}
						
					}
					
					specialContainerVO.setVslInfo(vslInfoList.get(0));
				}

				if(autoSentList != null && autoSentList.size() > 0) {
					specialContainerVO.setAutoSentFlag(autoSentList.get(0).getAutoSndTpCd());
				} else {
					specialContainerVO.setAutoSentFlag("No");
				}
				
				if(ediSentStatusList != null && ediSentStatusList.size() > 0) {
					specialContainerVO.setEdiSentStatus(ediSentStatusList.get(0).getAckRcvStsCd());
				}
				
				if(copyFlag) {  
					dgListLocalYn = "N";
				}
				specialContainerVO.setDgListLocalYn(dgListLocalYn);
    ...
    public void manageDgManifestList(DgListModiVO[] dgListModiVOs, SignOnUserAccount account) throws EventException {
		try {
			DgListModiVO insertVslInfo = null;
			DgListModiVO updateVslInfo = null;

			List<DgListModiVO> insertDgList = new ArrayList<DgListModiVO>();
			List<DgListModiVO> updateDgList = new ArrayList<DgListModiVO>();
			List<DgListModiVO> deleteDgList = new ArrayList<DgListModiVO>();
			DgListModiVO	   dgListModiVO = null;
			
			DgListModiVO[] 	vslInfoList = null;
			DgListModiVO	vslInfo 	= null;
			DgListModiVO[] 	dgModiList 	= null;
			
			SpecialContainerSaveVO containerSaveVO = null;
			
			String localDbYn = "";
			String dgListLocalYn = "";

			if (dgListModiVOs != null && dgListModiVOs.length > 0) {
				
				containerSaveVO = (SpecialContainerSaveVO) dgListModiVOs[0];
				
				vslInfoList 	= containerSaveVO.getVslInfoList();		// 상단 Master 정보
				dgModiList 		= containerSaveVO.getDgMOdiList();		// 그리드 정보
				
				
				// 상단 Master 정보 저장 VO 구성
				if (vslInfoList != null && vslInfoList.length > 0) {
					vslInfo = vslInfoList[0];
					
					localDbYn = vslInfo.getLocalDbYn();
					
					vslInfo.setDType(containerSaveVO.getDType());
					vslInfo.setVvdCd(containerSaveVO.getVvdCd());
					vslInfo.setPortCd(containerSaveVO.getPortCd());

					vslInfo.setCreUsrId(account.getUsr_id());
					vslInfo.setUpdUsrId(account.getUsr_id());
					
					if("Y".equals(localDbYn)) { // update
						updateVslInfo = vslInfo;
					} else {	// insert
						insertVslInfo = vslInfo;
					}
					
				}
				
				// Dg List 정보 저장 VO 구성
				if(dgModiList != null && dgModiList.length > 0) {
					dgListLocalYn = containerSaveVO.getDgListLocalYn();
					
					
					for (int i = 0; i < dgModiList.length; i++) {
						
						dgListModiVO = dgModiList[i];
						dgListModiVO.setDType(containerSaveVO.getDType());
						dgListModiVO.setVvdCd(containerSaveVO.getVvdCd());
						dgListModiVO.setPortCd(containerSaveVO.getPortCd());
						dgListModiVO.setCreUsrId(account.getUsr_id());
						dgListModiVO.setUpdUsrId(account.getUsr_id());
						
						if("Y".equals(dgListLocalYn)) { // Local data 이면 update
						
							if (dgListModiVO.getIbflag().equals("I")){
								insertDgList.add(dgListModiVO);
							} else if (dgListModiVO.getIbflag().equals("U")){
								updateDgList.add(dgListModiVO);
							} else if ( dgListModiVO.getIbflag().equals("D")){
								deleteDgList.add(dgListModiVO);
							}
							
						} else {	// Booking Data이면
							if( dgListModiVO.getIbflag().equals("D") ) continue;
							insertDgList.add(dgListModiVO);
						}
			...
			if(insertVslInfo != null) {
				dbDao.addVslInfo(insertVslInfo);
			}
			if(updateVslInfo != null) {
				dbDao.modifyVslInfo(updateVslInfo);
			}
	...
	public List<String> sendDgManifestList(DgEdiVO[] dgEdiVOs, SignOnUserAccount account) throws EventException {
...
//			updateFlatFile = "";
	...
	private String sendFlatFile(List<DgEdiVO> dgEdiVOs, SignOnUserAccount account) throws EventException {
		...
		// Add. 2015.05.07 
		String usrEml = "" ; // user email
		String crrCd = "" ; // Carrier Code 
		...
		try {
			...
			usrEml = account.getUsr_eml(); // add. 2015.05.07 
				...
				if(inFlag) {
					...
					if(uiType.equalsIgnoreCase("ESM_BKG_0965")) { // ANR, ZEE
						...
						if(comPortCd.equals("BRV")) {
							...
							senderId = "SML";
							receiverId = "DAKOSY";
						}  else if(comPortCd.equals("WVN")) { // Add. 2015.04.29 [CHM-201535729]
							senderId = "SML";
							receiverId = "DBH";
					...
					header = command.searchCstmsEdiHeader(senderId, receiverId, headerType);
					...
					if(uiType.equalsIgnoreCase("ESM_BKG_0965") 
							&& (transType.equals("U") || transType.equals("C") || transType.equals("AC")) ) { // ANR + (UPDATE OR CANCEL OR AC)
						...
						if(mainPartiesVOs != null) {
							...
							for(int idx =0; idx < mainPartiesVOsMaxSize; idx++) {
								...
								if(mainPartiesVO != null) {
									...
									// 2015.05.07 [CHM-201535282]
									//if(comPortCd.equals("HAM") || comPortCd.equals("BRV")){
									if(usrEml.equals("") || usrEml== null){
										flatFile.append("EMAIL:")						.append("").append("\n");
									}else{
										flatFile.append("EMAIL:")						.append(usrEml).append("\n");
									}
//									}
									// end
						...
						// Add. 2015.04.22  [CHM-201535282]
						crrCd = dbDao.searchMdmVslCntrInfo(dgEdiVO.getVvdCd().substring(0,4));
						...
						if(mainMeansVO != null) {
							flatFile.append("{MAIN_MEANS").append("\n");
								...
								// Add. 2015.04.22  [CHM-201535282]
								flatFile.append("MAIN_CARRIER:")				.append(crrCd).append("\n");
				...
				if(!currBlNo.equals("")) {
					...
					// 컨테이터 기본정보를 조회한다.
					if(!"AC".equals(dgEdiVO.getSendType())) {
...
//												flatFile.append("NW_EXP_UNIT:")			.append("").append("\n");
			...
			sendFlatFileVO.setQueueNm(SubSystemConfigFactory.get("BKG.ALPSBKG_UBIZHJS_EURDG.IBMMQ.QUEUE")); 
	...
	public void loadCstmsRcvMsg(String rcvMsg,String rcvGubun) throws EventException {
		...
		if(rcvGubun.equalsIgnoreCase("AlpsbkgTEurbaplieEvent")) { // BAPLIE 수신 
			this.loadBaplieRcvMsg(rcvMsg);
		} else if(rcvGubun.equalsIgnoreCase("AlpsbkgTEurcusAckEven")) { // 구주위험물 신고 응답메시지 수신

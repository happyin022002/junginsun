...
*@FileName       : ESM_SQM_0207HTMLAction.java
 ...
 * @author SQM USER
 * @see EsmSqm0207Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0207HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	
	/**
	 * ESM_SQM_0207HTMLAction 객체를 생성
	 */
	public ESM_SQM_0207HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0207Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSqm0207Event event = new EsmSqm0207Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setSqmSctrPerfIfVOs((SqmSctrPerfIfVO[])getVOs(request, SqmSctrPerfIfVO .class));

...
=========================================================*/
package com.hanjin.apps.alps.esm.acm.acmsimulation;

import java.util.List;

import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.basic.ACMSimulationBC;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.basic.ACMSimulationBCImpl;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0010Event;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0011Event;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0033Event;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0106Event;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0110Event;
import com.hanjin.apps.alps.esm.acm.acmsimulation.acmsimulation.event.EsmAcm0115Event;
...
/**
 * ALPS-ACMSimulation Business Logic ServiceCommand<br>
 * - ALPS-ACMSimulation에 대한 비지니스 트랜잭션을 처리한다.
 *
 * @author KIM, Sang-Soo
...
public class ACMSimulationSC extends ServiceCommandSupport {
	 ...
	 * ALPS-ACMSimulation system 업무에서 발생하는 모든 이벤트의 분기처리<br>
	 *
	 * @param e Event
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if (e.getEventName().equalsIgnoreCase("EsmAcm0106Event")) {
			...
			if (e.getFormCommand().isCommand(FormCommand.SEARCH)) {
				...
				eventResponse = getNewSimulationNo(e);
			...
			} else if (e.getFormCommand().isCommand(FormCommand.MULTI01)) { //Add Batch
				eventResponse = manageAGNCommAddBatchList(e);
			} else if (e.getFormCommand().isCommand(FormCommand.MULTI02)) { //Add Batch
				eventResponse = manageSimCommAddBatchList(e);
			}
		}
		return eventResponse;
	}
...
//	}
	 ...
	 * @exception EventException
	 */
	private EventResponse getNewSimulationNo(Event e) throws EventException {
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		ACMSimulationBC command = new ACMSimulationBCImpl();

		try{
			String simNo = command.getNewSimulationNo();
			eventResponse.setETCData("sim_no", simNo);
		} catch(EventException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
		} catch(Exception ex) {
	 ...
	 * @exception EventException
	 */
	private EventResponse manageAGNCommAddBatchList(Event e) throws EventException {
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmAcm0033Event event = (EsmAcm0033Event)e;
		ACMSimulationBC command = new ACMSimulationBCImpl();
		...
		try{
			begin();
			command.manageAGNCommAddBatchList(event.getAGNCommMassSimVO(), account);
			eventResponse.setUserMessage(new ErrorHandler("XXXXXXXXX").getUserMessage());
			commit();
		} catch(EventException ex) {
			rollback();
	 ...
	 * @exception EventException
	 */
	private EventResponse manageSimCommAddBatchList(Event e) throws EventException {
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmAcm0033Event event = (EsmAcm0033Event)e;
		ACMSimulationBC command = new ACMSimulationBCImpl();
		...
		try{
			begin();
			command.manageSimCommAddBatchList(event.getAGNCommMassSimVO(), account);
			eventResponse.setUserMessage(new ErrorHandler("XXXXXXXXX").getUserMessage());
			commit();
		} catch(EventException ex) {
			rollback();

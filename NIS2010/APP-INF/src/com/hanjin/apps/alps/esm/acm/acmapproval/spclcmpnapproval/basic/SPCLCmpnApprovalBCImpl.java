...
import com.hanjin.apps.alps.esm.acm.acmapproval.spclcmpnapproval.integration.SPCLCmpnApprovalDBDAO;
import com.hanjin.apps.alps.esm.acm.acmapproval.spclcmpnapproval.integration.SPCLCmpnApprovalEAIDAO;
...
public class SPCLCmpnApprovalBCImpl extends BasicCommandSupport implements SPCLCmpnApprovalBC {
	...
	public void approvalSPCLCmpnApproval(SPCLCmpnApprovalMasterVO spCLCmpnApprovalMasterVO, SignOnUserAccount account) throws EventException{
		try {
			...
			//3.SELECT GL_DT
			String glDtStr = dbDao.searchSPCLCmpnCSRGLDT(spCLCmpnCSRHeaderVO);
			if("".equals(glDtStr) || glDtStr == null){
				glDtStr = spCLCmpnCSRHeaderVO.getEaiEvntDt(); //glDt가 null 일경우 공통csr 결재라인정할수 없음
				
			}
			spCLCmpnCSRHeaderVO.setGlDt(glDtStr);
//			spCLCmpnCSRHeaderVO.setGlDt(spCLCmpnCSRHeaderVO.getGlDt());
			...
			for(int i = 0 ; i < detailVoSize ; i++) {
				...
				if(!"V.A.T".equals(spCLCmpnCSRDetailVOs.get(i).getInvDesc()) && !"".equals(spCLCmpnCSRDetailVOs.get(i).getInvDesc())){
...
//			aproUtil.updateAproGwFlg(spCLCmpnCSRHeaderVO.getCsrNo(), spCLCmpnCSRHeaderVO.getApOfcCd()); 
			//===========================================================================
			...
			if(isEPRequest) {
				...
				log.debug("spCLCmpnApprovalMasterVO.getAproStep()==>"+spCLCmpnApprovalMasterVO.getAproStep());
				// COM_APRO_RQST_ROUT apro_step
				ComAproRqstRoutVO[] routeInfos = approval.convertApprovalRoute(spCLCmpnApprovalMasterVO.getAproStep());

...
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.integration.HoldNoticeDBDAO;
...
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.CustInfoVO;
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.FaxSendVO;
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.HldNtcBkgStaffSetupInfoVO;
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.HldNtcObStaffSetupInfoVO;
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.HldNtcSearchVO;
import com.hanjin.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt.inboundnotice.vo.HldNtcStaffInfoVO;
...
import com.hanjin.syscommon.common.table.BkgCstmsAdvDspoVO;
import com.hanjin.syscommon.common.table.BkgHldN3rdPtyBilCntrVO;
import com.hanjin.syscommon.common.table.BkgHldNtcDtlVO;
import com.hanjin.syscommon.common.table.BkgHldNtcUsrVO;
import com.hanjin.syscommon.common.table.BkgHldNtcVO;
...
/**
 * ALPS-InboundBLMgt Business Logic Basic Command implementation<br>
 * - ALPS-InboundBLMgt에 대한 비지니스 로직을 처리한다.<br>
 *
 * @author Park Mi Ok
 * @see ESM_BKG-0948EventResponse,HoldNoticeBC 각 DAO 클래스 참조
 * @since J2EE 1.4
 */
public class HoldNoticeBCImpl extends BasicCommandSupport implements HoldNoticeBC {
	...
	public List<BkgComboVO> searchHldNtcCode(String cntCd) throws EventException {
		try {
			// 검색조건(국가 Code +Hold Code)을 가지고 기 Setup된 정보를 조회한다.
	...
	/**
	 * Hold Notice User 정보 유효성을 검증한다.
	 * 1. POD 유효성을 검증한다.
	 * 2. 중복 데이터 존재 여부를 체크한다.<br>
	 * 
	...
	/**
	 * Hold Notice ( Pre-Hold )의 Hold Code별 Form정보를 저장 or수정한다.<br>
	 * 
	...
	public List<BkgNtcHisVO> sendPreHldNtcByFax (PreHldNtcSendListVO[] preHldNtcSendLists, SignOnUserAccount account) throws EventException {
		...
		try {			
			...
			for (int i=0; i<preHldNtcSendLists.length; i++) {
				...
				//hldNtcDtl.setHldFaxSndDt(vo.getHldFaxSndDt());
				//hldNtcDtl.setHldFaxSndGdt(vo.getHldFaxSndGdt());
				hldNtcDtl.setHldFaxSndUsrId(account.getUsr_id());
				//hldNtcDtl.setHldFaxSndId(vo.getHldFaxSndId());
				//hldNtcDtl.setHldFaxSndRsltCd(vo.getHldFaxSndRsltCd());
				...
				if ("Y".equals(vo.getHldEclzOblFlg())) {
					...
					faxInfo.setSysCd("BKG");
					faxInfo.setBatchFlg("N");
					faxInfo.setTitle("Hold Notice OBL");
	...
	public List<BkgNtcHisVO> sendPreHldNtcByEmail (PreHldNtcSendListVO[] preHldNtcSendLists, SignOnUserAccount account) throws EventException {
		...
		String titNm = "[SM Line America] U.S. Government Hold Notice";
		InboundNoticeEAIDAO eaiDao = new InboundNoticeEAIDAO();
		...
		try {			
			...
			for (int i=0; i<preHldNtcSendLists.length; i++) {
				...
				//hldNtcDtl.setHldEmlSndDt(vo.getHldEmlSndDt());
				//hldNtcDtl.setHldEmlSndGdt(vo.getHldEmlSndGdt());
				hldNtcDtl.setHldEmlSndUsrId(account.getUsr_id());
				//hldNtcDtl.setHldEmlSndId(vo.getHldEmlSndId());
				//hldNtcDtl.setHldEmlSndRsltCd(vo.getHldEmlSndRsltCd());
				hldNtcDtl.setCreUsrId(account.getUsr_id());
				hldNtcDtl.setCreDt("");
				hldNtcDtl.setUpdUsrId(account.getUsr_id());
				hldNtcDtl.setUpdDt("");
	...
	public List<BkgNtcHisVO> sendConfirmHldNtcByFax (ConfirmHldNtcSendListVO[] hldNtcSendLists, SignOnUserAccount account) throws EventException {
		...
		try {			
			...
			for (int i=0; i<hldNtcSendLists.length; i++) {
				...
				faxInfo.setSysCd("BKG");
				faxInfo.setBatchFlg("N");
				faxInfo.setTitle("Removal Notice");
				faxInfo.setTmplParam("/rv bkg_no['" + vo.getBkgNo() + "'] ntc_seq['" + vo.getNtcSeq() + "'] " + 
				...
				if ("Y".equals(vo.getHldEclzOblFlg())) {
					...
					faxInfo.setSysCd("BKG");
					faxInfo.setBatchFlg("N");
					faxInfo.setTitle("Removal Notice OBL");
					faxInfo.setTmplParam("/rv form_bkgNo[('" + vo.getBkgNo() + "')] " +
										    " form_type[2] " +
											" form_dataOnly[N] " +
											" form_manifest[N] " +
	...
	public List<BkgNtcHisVO> sendConfirmHldNtcByEmail (ConfirmHldNtcSendListVO[] hldNtcSendLists, SignOnUserAccount account) throws EventException {
		...
		String titNm = "[SM Line America] U.S. Government Hold Removal Notice";
		InboundNoticeEAIDAO eaiDao = new InboundNoticeEAIDAO();
		RDMailSendVO mailInfo = null;	
		List<RDMailSendVO> mailInfos = null;
		ComRptDsgnXptInfoVO rdVO = null;
...
//		List<BkgHldN3rdPtyBilCntrVO> cntrLists = null;
//		String cntrList = "";
...
//				cntrList = "";
//				cntrLists = dbDao.searchHldN3rdPtyBilCntr(vo.getBkgNo(), vo.getNtcSeq());
//				for (int c=0; c<cntrLists.size(); c++) {
//					cntrList = cntrList + cntrLists.get(c).getCntrNo() + "<br>"; 
//				}		
				...
				arguments = new HashMap<String, String>();
				//arguments.put("rcvrNm", hldNtc.getCustNm());
//				arguments.put("cntrList", cntrList);						
				mailInfo.setArguments(arguments);
	...
	private BkgHldNtcVO createHldNtc(String bkgNo, String ntcSeq, CstmsHldVO cstmsHld, CustInfoVO custInfo, String eclzOblFlg) throws EventException 
	{
		...
		hldNtc.setUpdDt("");		
		hldNtc.setBkgCustTpCd(custInfo.getBkgCustTpCd()); // 2010.04.07 추가
...
//					 "==========================================================================\n");
			...
			if ((bkgUsr.getUsrEml().trim()).length() > 0) 
			{
				...
				if ("R".equals(hldType)) {
					tmpMailInfo.setTemplate("ESM_BKG_0510_GW_REJ.html");
					obEmlTitNm = "Warning Notification [" + vvd + "] [" + blNo + "] " + "[R:Rejection] " + hldDt + " (" + cntCd + " LOCAL TIME)";
				} else { 
	...
	public List<BkgNtcHisVO> createCstmsHld (CstmsHldVO cstmsHld) {
		...
		String titNm = "[SM Line America] U.S. Government Hold Notice"; // 화주 메일 Title
		String sndrNm = "SM Line America"; // 메일 발신자명
		String sndrEml = "noreply@smlines.com"; // 메일 발신자 이메일
					...
					if ("Y".equals(hldNtcFlg) && "Y".equals(hldWd.getAutoNtcFlg())) {
						...
						for (int k=0; k<hldNtcDtls.size(); k++) {
							...
							if ("A".equals(hldNtcDtl.getFaxTpCd()) && (hldNtcDtl.getFaxNo().trim()).length() > 0) {
								...
								faxInfo.setTitle("Auto Hold Notice");
								...
								if ("Y".equals(hldNtc.getHldEclzOblFlg())) {
									...
									faxInfo.setSysCd("BKG");
									faxInfo.setBatchFlg("Y");
									faxInfo.setTitle("Auto Hold Notice OBL");
			...
			for (int m=0; m<ntcUsrs.size(); m++) {
				...
				if ("M".equals(ntcUsr.getNtcMzdCd()) || "E".equals(ntcUsr.getNtcMzdCd())) 
				{
					if ((ntcUsr.getNtcEml().trim()).length() > 0) 
					{
						...
						tmpMailInfo.setSndrEml("noreply@smlines.com");

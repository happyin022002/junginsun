...
public class AGNCommAuditDBDAOSearchAGNCommAuditListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       MAX(B.REV_VVD_CD) AS REV_VVD_CD," ).append("\n"); 
		...
		query.append("	   SUM(DECODE(A.AC_TP_CD, 'P', A.IF_AMT, 0)) AS CHG_COMM_AMT," ).append("\n"); 
		...
		query.append("       SUM(DECODE(SIGN(INSTR('GKHSCP', A.AC_TP_CD)), 1, A.IF_AMT, 0)) AS DFF_AMT," ).append("\n"); 
		...
		query.append("       SUM(decode(A.AC_TP_CD , 'G', A.CRNT_REV_AMT, 0)) AS CRNT_REV_AMT," ).append("\n"); 
		query.append("       SUM(decode(A.AC_TP_CD , 'G', A.DDCT_CHG_AMT, 0)) AS DDCT_CHG_AMT," ).append("\n"); 
		query.append("       SUM(decode(A.AC_TP_CD , 'G', A.DDCT_TRSP_AMT,0)) AS DDCT_TRSP_AMT," ).append("\n"); 
		query.append("       SUM(decode(A.AC_TP_CD , 'G', A.CRNT_REV_AMT,0) - decode(A.AC_TP_CD , 'G', A.DDCT_CHG_AMT,0) - decode(A.AC_TP_CD , 'G', A.DDCT_TRSP_AMT,0) - decode(A.AC_TP_CD , 'G', A.DDCT_SPCL_CMPN_AMT,0)) AS POST_REV_AMT," ).append("\n"); 
		...
		query.append("/* Commisstion Type */" ).append("\n"); 

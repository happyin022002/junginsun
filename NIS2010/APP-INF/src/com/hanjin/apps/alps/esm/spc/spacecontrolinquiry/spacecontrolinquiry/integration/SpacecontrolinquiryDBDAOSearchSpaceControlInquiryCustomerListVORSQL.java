...
public class SpacecontrolinquiryDBDAOSearchSpaceControlInquiryCustomerListVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                           (D.CUST_CNT_CD, D.CUST_SEQ, DECODE(M.OFC_DIV, 'RHQ', D.SLS_RHQ_CD, D.SLS_RGN_OFC_CD), D.POL_YD_CD, M.OFC_CD)," ).append("\n"); 
		query.append("                           (D.CUST_CNT_CD, D.CUST_SEQ, DECODE(M.OFC_DIV, 'RHQ', D.SLS_RHQ_CD, D.SLS_RGN_OFC_CD), D.POD_YD_CD, M.OFC_CD)" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("          FROM (" ).append("\n"); 
		query.append("		   SELECT SUBSTR(SHPR_CD, 1, 2) AS CUST_CNT_CD" ).append("\n"); 
		query.append("                , TO_NUMBER(NVL(SUBSTR(SHPR_CD, 3), 0))     AS CUST_SEQ" ).append("\n"); 
		query.append("                " ).append("\n"); 
		...
		query.append("                        , (SELECT NVL(C.CUST_CNT_CD, '  ')||C.CUST_SEQ" ).append("\n"); 
		...
		query.append("           Z.BKG_WGT,Z.BKG_VOL_VGM,Z.BKG_WGT_VGM" ).append("\n"); 
		...
		query.append("         Z.PORT_CD," ).append("\n"); 
		query.append("         Z.OFC_CD ," ).append("\n"); 
		query.append("         NVL(FCT_TEU, 0) + NVL(FCT_HC, 0) * 2 + NVL(FCT_45, 0) * 2 + NVL(FCT_53, 0) * 2 AS FC_TTL_TEU," ).append("\n"); 
		query.append("         Z.FCT_TEU," ).append("\n"); 
		query.append("         Z.FCT_HC ," ).append("\n"); 
		query.append("         Z.FCT_45 ," ).append("\n"); 
		query.append("         Z.FCT_53 ," ).append("\n"); 

 ...
 * @since J2EE 1.4
 */
public class ChinaManifestListDownloadBCImpl  extends ManifestListDownloadBCImpl {
	...
	public List<ManifestListDetailVO> searchManifestList(ManifestListCondVO manifestListCondVO) throws EventException {
		try {
			...
			ChinaManifestListDetailVO detailVO = new ChinaManifestListDetailVO();
			List<ManifestListDetailVO> list = null;
			
			if("csv".equals(condVO.getGubun())){
				// CSV 다운로드 시 데이터 총 개수 조회
				list = new ArrayList<ManifestListDetailVO>();
				detailVO.setTotal(dbDao.searchManifestDetailListCount(condVO));
			}else{
				// Vessel Info 조회
				BkgCstmsChnVvdVO vvdVO = dbDao.searchVvdInfo((ChinaManifestListCondVO)manifestListCondVO);
				// B/L list 조회
				list = dbDao.searchManifestList(condVO);
				if(vvdVO != null){
					detailVO.setVvd(vvdVO.getVslCd()+vvdVO.getSkdVoyNo()+vvdVO.getSkdDirCd());
					detailVO.setCallSgnNo(vvdVO.getVslCallSgnPortLocCd());
					detailVO.setPrePort(vvdVO.getPreClptCd());
					detailVO.setNxtPort(vvdVO.getNxtClptCd());
					detailVO.setVpsEtaDt(vvdVO.getEtaDt());
					detailVO.setVpsEtdDt(vvdVO.getEtdDt());
					detailVO.setVpsEtbDt(vvdVO.getEtbDt());
					detailVO.setVslEngNm(vvdVO.getVslNm());
				}
			}
...
//		return fileName ;
	...
	public void manageBlInfo(ChinaBlInfoVO chinaBlInfoVO, SignOnUserAccount account) throws EventException {
		try {
			...
			if(dgCntrVOs != null){
				List<BkgCstmsChnDgCgoVO> bkgCstmsChnDgCgoVOs = new ArrayList<BkgCstmsChnDgCgoVO>();
				...
				for(int i=0; i<dgCntrVOs.length; i++){
					...
					// Delete한 Row는 제외
					if("D".equals(dgCntrVO.getIbflag())) continue;
					...
					// TableVO List에 등록
					bkgCstmsChnDgCgoVOs.add(bkgCstmsChnDgCgoVO);
				}

				for(int i=0; i<bkgCstmsChnDgCgoVOs.size(); i++){
					dbDao.addBlDangerCntr(bkgCstmsChnDgCgoVOs.get(i));
				}
			}

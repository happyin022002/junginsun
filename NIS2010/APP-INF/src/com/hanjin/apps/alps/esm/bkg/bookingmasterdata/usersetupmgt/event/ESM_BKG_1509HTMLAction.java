...
*@FileName : ESM_BKG_1509HTMLAction.java
...
public class ESM_BKG_1509HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1509HTMLAction 객체를 생성
	 */
	public ESM_BKG_1509HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1509Event event = new EsmBkg1509Event();		
		
		if(command.isCommand(FormCommand.SEARCH)) {           
			event.setDraftBlImageVO((DraftBlImageVO)getVO(request, DraftBlImageVO.class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setDraftBlImageVOs((DraftBlImageVO[])getVOs(request, DraftBlImageVO.class));

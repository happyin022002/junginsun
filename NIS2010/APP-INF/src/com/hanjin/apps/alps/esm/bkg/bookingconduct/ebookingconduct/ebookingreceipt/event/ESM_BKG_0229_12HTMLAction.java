...
*@FileName : ESM_BKG_0229_12HTMLAction.java
...
public class ESM_BKG_0229_12HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0229_12HTMLAction 객체를 생성
	 */
	public ESM_BKG_0229_12HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg022912Event event = new EsmBkg022912Event();
		...
		{
			// BB
			AlpsBbVO[] alpsBbVO = (AlpsBbVO[]) getVOs(request, AlpsBbVO.class, "t12sheet1_");
			int iMaxBbSeq = 0;
			int iDataCnt = 0;
			if (!"".equals(alpsBbVO[0].getMaxBbCgoSeq()))
			{
				iMaxBbSeq = Integer.parseInt(alpsBbVO[0].getMaxBbCgoSeq());
			}
			for (int i = 0; i < alpsBbVO.length; i++)
			{
				if ("I".equals(alpsBbVO[i].getIbflag()))
				{
					alpsBbVO[i].setBbCgoSeq((iMaxBbSeq + 1) + "");
					iMaxBbSeq++;
					iDataCnt++;
				}
				else if ("U".equals(alpsBbVO[i].getIbflag()) || "D".equals(alpsBbVO[i].getIbflag()))
				{
					iDataCnt++;
				}
			}
			BkgBbCgoVO[] bkgBbCgoVO = new BkgBbCgoVO[iDataCnt];
			for (int i = 0, j = 0; i < alpsBbVO.length; i++)
			{
				if (!"R".equals(alpsBbVO[i].getIbflag()))
				{
					bkgBbCgoVO[j] = new BkgBbCgoVO();
					ObjectCloner.build(alpsBbVO[i], bkgBbCgoVO[j]);
					bkgBbCgoVO[j].setBbDcgoSeq("");
					j++;
				}
			}
			event.setBkgBbCgoVOs(bkgBbCgoVO);
		}

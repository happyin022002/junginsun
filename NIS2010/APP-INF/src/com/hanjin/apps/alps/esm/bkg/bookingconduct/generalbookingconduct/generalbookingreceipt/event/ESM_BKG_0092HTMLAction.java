...
*@FileName : ESM_BKG_0092HTMLAction.java
...
public class ESM_BKG_0092HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0092HTMLAction 객체를 생성
	 */
	public ESM_BKG_0092HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0092Event event = new EsmBkg0092Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setPolPodVvdVOs((PolPodVvdVO[])getVOs(request, PolPodVvdVO.class, ""));
			event.setDataYn(request.getParameter("dataYn"));
			event.setBkgNo(request.getParameter("bkg_no"));
		}else if(command.isCommand(FormCommand.SEARCH01)){
			event.setPolPodVvdVOs((PolPodVvdVO[])getVOs(request, PolPodVvdVO.class, ""));
			event.setEditRow(request.getParameter("edit_row"));

...
*@FileName : ESM_BKG_0300HTMLAction.java
...
public class ESM_BKG_0300HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0296HTMLAction 객체를 생성
	 */
	public ESM_BKG_0300HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmBkg0300Event event = new EsmBkg0300Event();
		
		if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01)) {
			event.setInPrintFormCondVO((InPrintFormCondVO) getVO(request,InPrintFormCondVO.class ));
			
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setInPrintFormModiVOs((InPrintFormModiVO[]) getVOs(request, InPrintFormModiVO.class));
		}

...
*@FileName : ESM_MAS_0182HTMLAction.java
*@FileTitle : Average 
*Open Issues :
*Change history :
...
public class ESM_MAS_0182HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0182HTMLAction 객체를 생성
	 */
	public ESM_MAS_0182HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmMas0182Event event = new EsmMas0182Event();
		
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setMasOpAvgUtCostVOS((MasOpAvgUtCostVO[])getVOs(request, MasOpAvgUtCostVO.class));
		}else if(command.isCommand(FormCommand.MULTI02)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setMasOpAvgUtCostVO((MasOpAvgUtCostVO)getVOs(request, MasOpAvgUtCostVO.class));
		}else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}

...
public class UsaManifestListDownloadDBDAOsearchManifestSummaryRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       CASE WHEN MAX(P.VPS_ETA_DT) IS NULL THEN ' ' ELSE TO_CHAR(MAX(P.VPS_ETA_DT), 'YYYYMMDD HH24') || 'h' END ETA,  " ).append("\n"); 
		...
		query.append("		CASE WHEN MAX(P2.VPS_ETD_DT) IS NULL THEN ' ' ELSE TO_CHAR(MAX(P2.VPS_ETD_DT), 'YYYYMMDD HH24') || 'h' END ETD," ).append("\n"); 
		query.append("		(" ).append("\n"); 
		...
		query.append("		, CASE WHEN MAX(P2.VPS_ETD_DT) IS NULL THEN ' ' ELSE TO_CHAR(MAX(P2.VPS_ETD_DT), 'YYYYMMDD HH24') || 'h' END " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${pol} != '') " ).append("\n"); 
		query.append("		, CASE WHEN MAX(P.VPS_ETA_DT) IS NULL THEN ' ' ELSE TO_CHAR(MAX(P.VPS_ETA_DT), 'YYYYMMDD HH24') || 'h' END " ).append("\n"); 
		query.append("#end" ).append("\n"); 

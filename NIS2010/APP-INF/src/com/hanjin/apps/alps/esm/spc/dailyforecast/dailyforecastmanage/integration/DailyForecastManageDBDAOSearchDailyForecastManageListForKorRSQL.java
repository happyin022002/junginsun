/*=========================================================
*Copyright(c) 2018 CyberLogitec
*@FileName : DailyForecastManageDBDAOSearchDailyForecastManageListForKorRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
*@LastModifyDate : 2018.03.28
*@LastModifier : 
*@LastVersion : 1.0
* 2018.03.28 
* 1.0 Creation
=========================================================*/
package com.hanjin.apps.alps.esm.spc.dailyforecast.dailyforecastmanage.integration;

import java.util.HashMap;
import org.apache.log4j.Logger;
import com.hanjin.framework.support.db.ISQLTemplate;

/**
 *
 * @author 
 * @see DAO 참조
 * @since J2EE 1.6
 */

public class DailyForecastManageDBDAOSearchDailyForecastManageListForKorRSQL implements ISQLTemplate{

	private StringBuffer query = new StringBuffer();
	
	Logger log =Logger.getLogger(this.getClass());
	
	/** Parameters definition in params/param elements */
	private HashMap<String,String[]> params = null;
	
	/**
	  * <pre>
	  * 2015.03.03 소스공백등 정리(쿼리가 너무 길어 콘솔로그에서 hint등이 빠져서)
	  * 2015.03.03 CHM-201534435 SQM QTA주가 변경 관련 적용 요청
	  * 2015.03.09 [CHM-201534504]SMP IAS 보완에 따른 FCST 추가 개발 요청
	  * 2015.03.10 [CHM-201534747]Forecast Input(Loading) 화면 정보 추가-Weight, US Control아닐경우 SUM제외
	  * 2015.03.13 [CHM-201534835]Forecast Input POL별 Summary 데이터 추가 요청
	  * 2015.08.12 이혜민 [CHM-201537257] BKG_ORG CUST_CTRL_CD 기준 변경. 
	  * 						(1. RFA_NO IS NULL, SC IS NULL 조건 삭제(DUMMY RFA뿐만 아니라 모든 RFA 비교) 
	  * 						2. SPC_MDL_CUST_RFA에 CMDT_CD 조인 조건 추가.)
	  * 2015.10.21 이혜민 [CHM-201538339] 사직 S.REP의 FCST Input의 데이터 삭제관련 보완 요청
	  * </pre>
	  */
	public DailyForecastManageDBDAOSearchDailyForecastManageListForKorRSQL(){
		setQuery();
		params = new HashMap<String,String[]>();
		String tmp = null;
		String[] arrTmp = null;
		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("vvd",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("duration",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("subtrade",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("suboffice",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("trade",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("ioc",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("year",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("salesoffice",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("salesrep",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("bound",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("week",new String[]{arrTmp[0],arrTmp[1]});

		query.append("/*").append("\n"); 
		query.append("Path : com.hanjin.apps.alps.esm.spc.dailyforecast.dailyforecastmanage.integration").append("\n"); 
		query.append("FileName : DailyForecastManageDBDAOSearchDailyForecastManageListForKorRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	
	public String getSQL(){
		return query.toString();
	}
	
	public HashMap<String,String[]> getParams() {
		return params;
	}

	/**
	 * Query 생성
	 */
	public void setQuery(){
		query.append("WITH PARAMS_ORG AS (" ).append("\n"); 
		query.append("    SELECT @[year]        AS COST_YR     ," ).append("\n"); 
		query.append("           @[week]        AS COST_WK     ," ).append("\n"); 
		query.append("           @[duration]    AS DUR         ," ).append("\n"); 
		query.append("           @[trade]       AS TRD_CD      ," ).append("\n"); 
		query.append("           @[subtrade]    AS SUB_TRD_CD  ," ).append("\n"); 
		query.append("           @[bound]       AS DIR_CD      ," ).append("\n"); 
		query.append("           @[ioc]         AS IOC_TS_CD   ," ).append("\n"); 
		query.append("           @[salesoffice] AS RGN_OFC_CD  ," ).append("\n"); 
		query.append("           @[suboffice]   AS SUB_OFC_CD  ," ).append("\n"); 
		query.append("           @[salesrep]    AS SALES_REP_CD," ).append("\n"); 
		query.append("           @[vvd]         AS VVD" ).append("\n"); 
		query.append("      FROM DUAL" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", PARAMS AS (" ).append("\n"); 
		query.append("    SELECT COST_YR       ," ).append("\n"); 
		query.append("           COST_WK       ," ).append("\n"); 
		query.append("           DUR           ," ).append("\n"); 
		query.append("           TRD_CD        ," ).append("\n"); 
		query.append("           SUB_TRD_CD    ," ).append("\n"); 
		query.append("           RLANE_CD      ," ).append("\n"); 
		query.append("           DIR_CD        ," ).append("\n"); 
		query.append("           IOC_TS_CD     ," ).append("\n"); 
		query.append("           RGN_OFC_CD    ," ).append("\n"); 
		query.append("           SUB_OFC_CD    ," ).append("\n"); 
		query.append("           SALES_REP_CD  ," ).append("\n"); 
		query.append("           VVD           ," ).append("\n"); 
		query.append("           REP_TRD_CD    ," ).append("\n"); 
		query.append("           REP_SUB_TRD_CD," ).append("\n"); 
		query.append("           VSL_SLAN_CD" ).append("\n"); 
		query.append("      FROM (  SELECT PO.COST_YR     ," ).append("\n"); 
		query.append("                     PO.COST_WK     ," ).append("\n"); 
		query.append("                     PO.DUR         ," ).append("\n"); 
		query.append("                     PO.TRD_CD      ," ).append("\n"); 
		query.append("                     PO.SUB_TRD_CD  ," ).append("\n"); 
		query.append("                     RL.RLANE_CD    ," ).append("\n"); 
		query.append("                     PO.DIR_CD      ," ).append("\n"); 
		query.append("                     PO.IOC_TS_CD   ," ).append("\n"); 
		query.append("                     PO.RGN_OFC_CD  ," ).append("\n"); 
		query.append("                     PO.SUB_OFC_CD  ," ).append("\n"); 
		query.append("                     PO.SALES_REP_CD," ).append("\n"); 
		query.append("                     PO.VVD         ," ).append("\n"); 
		query.append("                     RL.REP_TRD_CD  ," ).append("\n"); 
		query.append("                     SPC_GET_REP_SUB_TRD_FNC(RL.RLANE_CD) AS REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                     RL.VSL_SLAN_CD" ).append("\n"); 
		query.append("                FROM MDM_REV_LANE RL," ).append("\n"); 
		query.append("                     PARAMS_ORG   PO" ).append("\n"); 
		query.append("               WHERE PO.VVD IS NULL" ).append("\n"); 
		query.append("				 AND RL.DELT_FLG = 'N'" ).append("\n"); 
		query.append("#if (${lane} != '')" ).append("\n"); 
		query.append("                 AND RL.RLANE_CD IN ( ${lane} )" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("              UNION ALL" ).append("\n"); 
		query.append("              SELECT DISTINCT" ).append("\n"); 
		query.append("                     ''   AS COST_YR     ," ).append("\n"); 
		query.append("                     ''   AS COST_WK     ," ).append("\n"); 
		query.append("                     NULL AS DUR         ," ).append("\n"); 
		query.append("                     ''   AS TRD_CD      ," ).append("\n"); 
		query.append("                     ''   AS SUB_TRD_CD  ," ).append("\n"); 
		query.append("                     ''   AS RLANE_CD    ," ).append("\n"); 
		query.append("                     SUBSTR(PO.VVD, 9, 1) AS DIR_CD," ).append("\n"); 
		query.append("                     PO.IOC_TS_CD        ," ).append("\n"); 
		query.append("                     PO.RGN_OFC_CD       ," ).append("\n"); 
		query.append("                     PO.SUB_OFC_CD       ," ).append("\n"); 
		query.append("                     PO.SALES_REP_CD     ," ).append("\n"); 
		query.append("                     PO.VVD              ," ).append("\n"); 
		query.append("                     '' AS REP_TRD_CD    ," ).append("\n"); 
		query.append("                     '' AS REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                     RL.VSL_SLAN_CD" ).append("\n"); 
		query.append("                FROM VSK_VSL_SKD  VS," ).append("\n"); 
		query.append("                     MDM_REV_LANE RL," ).append("\n"); 
		query.append("                     PARAMS_ORG   PO" ).append("\n"); 
		query.append("               WHERE VS.VSL_CD      = SUBSTR(PO.VVD, 1, 4)" ).append("\n"); 
		query.append("                 AND VS.SKD_VOY_NO  = SUBSTR(PO.VVD, 5, 4)" ).append("\n"); 
		query.append("                 AND VS.SKD_DIR_CD  = SUBSTR(PO.VVD, 9, 1)" ).append("\n"); 
		query.append("                 AND VS.VSL_SLAN_CD = RL.VSL_SLAN_CD" ).append("\n"); 
		query.append("                 AND PO.VVD IS NOT NULL" ).append("\n"); 
		query.append("           )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVDLS AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           SEASON        ," ).append("\n"); 
		query.append("           SLS_YRMON     ," ).append("\n"); 
		query.append("           COST_YRMON    ," ).append("\n"); 
		query.append("           COST_WK       ," ).append("\n"); 
		query.append("           REP_TRD_CD    ," ).append("\n"); 
		query.append("           REP_SUB_TRD_CD," ).append("\n"); 
		query.append("           TRD_CD        ," ).append("\n"); 
		query.append("           SUB_TRD_CD    ," ).append("\n"); 
		query.append("           RLANE_CD      ," ).append("\n"); 
		query.append("           DIR_CD        ," ).append("\n"); 
		query.append("           RGN_OFC_CD    ," ).append("\n"); 
		query.append("           VSL_CD        ," ).append("\n"); 
		query.append("           SKD_VOY_NO    ," ).append("\n"); 
		query.append("           SKD_DIR_CD    ," ).append("\n"); 
		query.append("           IOC_CD        ," ).append("\n"); 
		query.append("           ETD_DT        ," ).append("\n"); 
		query.append("           CTRL_LVL      ," ).append("\n"); 
		query.append("           CTRL_HC       ," ).append("\n"); 
		query.append("           CTRL_45       ," ).append("\n"); 
		query.append("           CTRL_53       ," ).append("\n"); 
		query.append("           CTRL_RF       ," ).append("\n"); 
		query.append("           CTRL_WT       ," ).append("\n"); 
		query.append("           DECODE(NVL(( SELECT COUNT(*)" ).append("\n"); 
		query.append("                          FROM SPC_MDL_CUST_CTRL" ).append("\n"); 
		query.append("                         WHERE TRD_CD    = A.TRD_CD" ).append("\n"); 
		query.append("                           AND COST_YRWK = SUBSTR(A.SEASON, 1, 6)" ).append("\n"); 
		query.append("                           AND VER_SEQ   = SUBSTR(A.SEASON, 8)" ).append("\n"); 
		query.append("                           AND DELT_FLG  = 'N'), 0), 0, 'N', 'Y') AS CTRL_ACCT," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , BSE_QTR_CD" ).append("\n"); 
		query.append("      FROM (SELECT DISTINCT" ).append("\n"); 
		query.append("                   (SELECT /*+ INDEX_DESC (M XPKSPC_MDL_VER_MST) */ DECODE(HH_FLG, 'Y', COST_YRWK||'-'||VER_SEQ, '200001-1')" ).append("\n"); 
		query.append("                      FROM SPC_MDL_VER_MST M" ).append("\n"); 
		query.append("                     WHERE SUBSTR(COST_YRMON, 1, 4)||COST_WK BETWEEN M.VER_ST_YRWK AND M.VER_END_YRWK" ).append("\n"); 
		query.append("                       AND M.TRD_CD  = A.TRD_CD" ).append("\n"); 
		query.append("                       AND M.CFM_FLG = 'Y'" ).append("\n"); 
		query.append("                       AND ROWNUM = 1) AS SEASON," ).append("\n"); 
		query.append("                   SLS_YRMON     ," ).append("\n"); 
		query.append("                   COST_YRMON    ," ).append("\n"); 
		query.append("                   COST_WK       ," ).append("\n"); 
		query.append("                   REP_TRD_CD    ," ).append("\n"); 
		query.append("                   REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                   TRD_CD        ," ).append("\n"); 
		query.append("                   SUB_TRD_CD    ," ).append("\n"); 
		query.append("                   RLANE_CD      ," ).append("\n"); 
		query.append("                   DIR_CD        ," ).append("\n"); 
		query.append("                   RGN_OFC_CD    ," ).append("\n"); 
		query.append("                   VSL_CD        ," ).append("\n"); 
		query.append("                   SKD_VOY_NO    ," ).append("\n"); 
		query.append("                   SKD_DIR_CD    ," ).append("\n"); 
		query.append("                   IOC_CD        ," ).append("\n"); 
		query.append("                   ETD_DT        ," ).append("\n"); 
		query.append("                   CTRL_LVL      ," ).append("\n"); 
		query.append("                   CTRL_HC       ," ).append("\n"); 
		query.append("                   CTRL_45       ," ).append("\n"); 
		query.append("                   CTRL_53       ," ).append("\n"); 
		query.append("                   CTRL_RF       ," ).append("\n"); 
		query.append("                   CTRL_WT       ," ).append("\n"); 
		query.append("                   CTRL_ACCT     ," ).append("\n"); 
		query.append("                   HH_FLG" ).append("\n"); 
		query.append("                   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                   , BSE_QTR_CD" ).append("\n"); 
		query.append("              FROM (SELECT MV.SLS_YRMON    ," ).append("\n"); 
		query.append("                             MV.COST_YRMON   ," ).append("\n"); 
		query.append("                             MV.COST_WK      ," ).append("\n"); 
		query.append("                             P.REP_TRD_CD    ," ).append("\n"); 
		query.append("                             P.REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                             MV.TRD_CD       ," ).append("\n"); 
		query.append("                             MV.SUB_TRD_CD   ," ).append("\n"); 
		query.append("                             MV.RLANE_CD     ," ).append("\n"); 
		query.append("                             MV.DIR_CD       ," ).append("\n"); 
		query.append("                             P.RGN_OFC_CD    ," ).append("\n"); 
		query.append("                             MV.VSL_CD       ," ).append("\n"); 
		query.append("                             MV.SKD_VOY_NO   ," ).append("\n"); 
		query.append("                             MV.DIR_CD     AS SKD_DIR_CD," ).append("\n"); 
		query.append("                             DECODE(Q.DIR_CD, NULL, 'N', 'Y') AS HH_FLG," ).append("\n"); 
		query.append("                             MV.IOC_CD       ," ).append("\n"); 
		query.append("                             MV.LST_LODG_PORT_ETD_DT       AS ETD_DT  ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_LVL_CD     , 'L') AS CTRL_LVL," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_40FT_HC_FLG, 'N') AS CTRL_HC ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_45FT_HC_FLG, 'N') AS CTRL_45 ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_53FT_FLG   , 'N') AS CTRL_53 ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_RF_FLG     , 'N') AS CTRL_RF ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_WGT_FLG    , 'N') AS CTRL_WT ," ).append("\n"); 
		query.append("                             NVL(CO.ACCT_GRP_CTRL_FLG, 'N') AS CTRL_ACCT" ).append("\n"); 
		query.append("                             , NVL(CTRL_D2_FLG, 'N') CTRL_D2_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_D4_FLG, 'N') CTRL_D4_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_RD_FLG, 'N') CTRL_RD_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_ECC_FLG, 'N') CTRL_ECC_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_LOC_FLG , 'N') CTRL_LOC_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_USA_SVC_MOD_FLG , 'N') CTRL_USA_SVC_MOD_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_ACCT_FLG, 'N') CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , CASE WHEN MV.COST_YRMON >= '201501' " ).append("\n"); 
		query.append("                                  THEN CEIL(TO_NUMBER(SUBSTR(MV.COST_YRMON, -2))/3)||'Q' " ).append("\n"); 
		query.append("                                  ELSE CEIL(TO_NUMBER(DECODE(MV.COST_WK,'00','01','53','52',MV.COST_WK))/13)||'Q'" ).append("\n"); 
		query.append("                               END BSE_QTR_CD " ).append("\n"); 
		query.append("                        FROM SPC_ALOC_CTRL_OPT CO," ).append("\n"); 
		query.append("                             MAS_MON_VVD       MV," ).append("\n"); 
		query.append("                             PARAMS            P ," ).append("\n"); 
		query.append("                             SPC_HD_HUL_MST    Q" ).append("\n"); 
		query.append("                       WHERE SUBSTR(MV.SLS_YRMON, 1, 4)||MV.COST_WK IN ( SELECT /*+ INDEX(P XPKMAS_WK_PRD)*/ PRD.COST_YR||PRD.COST_WK COST_YRWK" ).append("\n"); 
		query.append("                                                                           FROM MAS_WK_PRD PRD" ).append("\n"); 
		query.append("                                                                          WHERE PRD.COST_YR||PRD.COST_WK >= P.COST_YR||P.COST_WK" ).append("\n"); 
		query.append("                                                                            AND ROWNUM <= P.DUR )" ).append("\n"); 
		query.append("                         AND MV.TRD_CD        = P.TRD_CD" ).append("\n"); 
		query.append("                         AND MV.SUB_TRD_CD LIKE P.SUB_TRD_CD||'%'" ).append("\n"); 
		query.append("                         AND MV.RLANE_CD   LIKE P.RLANE_CD||'%'" ).append("\n"); 
		query.append("                         AND MV.DIR_CD     LIKE P.DIR_CD||'%'" ).append("\n"); 
		query.append("                         AND (MV.DELT_FLG IS NULL OR MV.DELT_FLG = 'N')" ).append("\n"); 
		query.append("                         AND MV.RLANE_CD     <> 'RBCCO'" ).append("\n"); 
		query.append("                         AND CO.RLANE_CD  (+) = MV.RLANE_CD" ).append("\n"); 
		query.append("                         AND CO.DIR_CD    (+) = MV.DIR_CD" ).append("\n"); 
		query.append("                         AND CO.VSL_CD    (+) = MV.VSL_CD" ).append("\n"); 
		query.append("                         AND CO.SKD_VOY_NO(+) = MV.SKD_VOY_NO" ).append("\n"); 
		query.append("                         AND CO.SKD_DIR_CD(+) = MV.DIR_CD" ).append("\n"); 
		query.append("                         AND P.VVD IS NULL" ).append("\n"); 
		query.append("                         AND Q.TRD_CD     (+) = MV.TRD_CD" ).append("\n"); 
		query.append("                         AND Q.RLANE_CD   (+) = MV.RLANE_CD" ).append("\n"); 
		query.append("                         AND Q.DIR_CD     (+) = MV.DIR_CD" ).append("\n"); 
		query.append("                      UNION ALL" ).append("\n"); 
		query.append("                      SELECT MV.SLS_YRMON," ).append("\n"); 
		query.append("                             MV.COST_YRMON ," ).append("\n"); 
		query.append("                             MV.COST_WK  ," ).append("\n"); 
		query.append("                             SPC_GET_REP_TRD_FNC(MV.RLANE_CD)     AS REP_TRD_CD    ," ).append("\n"); 
		query.append("                             SPC_GET_REP_SUB_TRD_FNC(MV.RLANE_CD) AS REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                             MV.TRD_CD                  ," ).append("\n"); 
		query.append("                             MV.SUB_TRD_CD              ," ).append("\n"); 
		query.append("                             MV.RLANE_CD                ," ).append("\n"); 
		query.append("                             MV.DIR_CD                  ," ).append("\n"); 
		query.append("                             P.RGN_OFC_CD               ," ).append("\n"); 
		query.append("                             MV.VSL_CD                  ," ).append("\n"); 
		query.append("                             MV.SKD_VOY_NO              ," ).append("\n"); 
		query.append("                             MV.DIR_CD     AS SKD_DIR_CD," ).append("\n"); 
		query.append("                             DECODE(Q.DIR_CD, NULL, 'N', 'Y') AS HH_FLG," ).append("\n"); 
		query.append("                             MV.IOC_CD                  ," ).append("\n"); 
		query.append("                             MV.LST_LODG_PORT_ETD_DT       AS ETD_DT  ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_LVL_CD     , 'L') AS CTRL_LVL," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_40FT_HC_FLG, 'N') AS CTRL_HC ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_45FT_HC_FLG, 'N') AS CTRL_45 ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_53FT_FLG   , 'N') AS CTRL_53 ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_RF_FLG     , 'N') AS CTRL_RF ," ).append("\n"); 
		query.append("                             NVL(CO.CTRL_WGT_FLG    , 'N') AS CTRL_WT ," ).append("\n"); 
		query.append("                             NVL(CO.ACCT_GRP_CTRL_FLG, 'N') AS CTRL_ACCT" ).append("\n"); 
		query.append("                             , NVL(CTRL_D2_FLG, 'N') CTRL_D2_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_D4_FLG, 'N') CTRL_D4_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_RD_FLG, 'N') CTRL_RD_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_ECC_FLG, 'N') CTRL_ECC_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_LOC_FLG , 'N') CTRL_LOC_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_USA_SVC_MOD_FLG , 'N') CTRL_USA_SVC_MOD_FLG" ).append("\n"); 
		query.append("                             , NVL(CTRL_ACCT_FLG, 'N') CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , CASE WHEN MV.COST_YRMON >= '201501' " ).append("\n"); 
		query.append("                                  THEN CEIL(TO_NUMBER(SUBSTR(MV.COST_YRMON, -2))/3)||'Q' " ).append("\n"); 
		query.append("                                  ELSE CEIL(TO_NUMBER(DECODE(MV.COST_WK,'00','01','53','52',MV.COST_WK))/13)||'Q'" ).append("\n"); 
		query.append("                               END BSE_QTR_CD " ).append("\n"); 
		query.append("                        FROM SPC_ALOC_CTRL_OPT CO," ).append("\n"); 
		query.append("                             MAS_MON_VVD       MV," ).append("\n"); 
		query.append("                             PARAMS            P ," ).append("\n"); 
		query.append("                             SPC_HD_HUL_MST    Q" ).append("\n"); 
		query.append("                       WHERE MV.VSL_CD     = SUBSTR(P.VVD, 1, 4)" ).append("\n"); 
		query.append("                         AND MV.SKD_VOY_NO = SUBSTR(P.VVD, 5, 4)" ).append("\n"); 
		query.append("                         AND MV.DIR_CD     = SUBSTR(P.VVD, 9, 1)" ).append("\n"); 
		query.append("                         AND MV.IOC_CD     = DECODE(P.IOC_TS_CD, 'O', 'O', 'I')" ).append("\n"); 
		query.append("                         AND (MV.DELT_FLG IS NULL OR MV.DELT_FLG = 'N')" ).append("\n"); 
		query.append("                         AND CO.RLANE_CD  (+) = MV.RLANE_CD" ).append("\n"); 
		query.append("                         AND CO.DIR_CD    (+) = MV.DIR_CD" ).append("\n"); 
		query.append("                         AND CO.VSL_CD    (+) = MV.VSL_CD" ).append("\n"); 
		query.append("                         AND CO.SKD_VOY_NO(+) = MV.SKD_VOY_NO" ).append("\n"); 
		query.append("                         AND CO.SKD_DIR_CD(+) = MV.DIR_CD" ).append("\n"); 
		query.append("                         AND P.VVD IS NOT NULL" ).append("\n"); 
		query.append("                         AND Q.TRD_CD     (+) = MV.TRD_CD" ).append("\n"); 
		query.append("                         AND Q.RLANE_CD   (+) = MV.RLANE_CD" ).append("\n"); 
		query.append("                         AND Q.DIR_CD     (+) = MV.DIR_CD" ).append("\n"); 
		query.append("                   ) A" ).append("\n"); 
		query.append("        ) A" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_POL AS (" ).append("\n"); 
		query.append("    SELECT SEASON        ," ).append("\n"); 
		query.append("           SLS_YRMON     ," ).append("\n"); 
		query.append("           COST_YRMON    ," ).append("\n"); 
		query.append("           COST_WK       ," ).append("\n"); 
		query.append("           REP_TRD_CD    ," ).append("\n"); 
		query.append("           REP_SUB_TRD_CD," ).append("\n"); 
		query.append("           TRD_CD        ," ).append("\n"); 
		query.append("           SUB_TRD_CD    ," ).append("\n"); 
		query.append("           RLANE_CD      ," ).append("\n"); 
		query.append("           M.VSL_CD      ," ).append("\n"); 
		query.append("           M.SKD_VOY_NO  ," ).append("\n"); 
		query.append("           M.SKD_DIR_CD  ," ).append("\n"); 
		query.append("           NVL(V.YD_CD, M.POL_CD) AS POL_CD," ).append("\n"); 
		query.append("           ( SELECT DECODE(MAX(S.SKD_CNG_STS_CD), 'S', 0, NVL(DECODE(IOC_TS_CD, 'O', MAX(S.CLPT_SEQ), MIN(S.CLPT_SEQ)), -1))" ).append("\n"); 
		query.append("                   FROM VSK_VSL_PORT_SKD S" ).append("\n"); 
		query.append("                  WHERE S.VSL_CD      = M.VSL_CD" ).append("\n"); 
		query.append("                    AND S.SKD_VOY_NO  = M.SKD_VOY_NO" ).append("\n"); 
		query.append("                    AND S.SKD_DIR_CD  = M.SKD_DIR_CD" ).append("\n"); 
		query.append("                    AND S.VPS_PORT_CD = M.POL_CD" ).append("\n"); 
		query.append("                    AND (S.SKD_CNG_STS_CD IS NULL OR S.SKD_CNG_STS_CD <> 'S')" ).append("\n"); 
		query.append("           ) AS POL_SEQ," ).append("\n"); 
		query.append("           ( SELECT SPC_CONTI_CONV_FNC(MLOC.CONTI_CD, M.RLANE_CD,M.SKD_DIR_CD)" ).append("\n"); 
		query.append("                   FROM MDM_LOCATION MLOC" ).append("\n"); 
		query.append("                  WHERE M.POL_CD = MLOC.LOC_CD" ).append("\n"); 
		query.append("           ) AS POL_CONTI," ).append("\n"); 
		query.append("           ETD_DT    ," ).append("\n"); 
		query.append("           MIN(CD_DP_SEQ) OVER (PARTITION BY SLS_YRMON, COST_WK, REP_TRD_CD, REP_SUB_TRD_CD, RLANE_CD, M.VSL_CD, M.SKD_VOY_NO, M.SKD_DIR_CD, NVL(V.YD_CD, M.POL_CD), ETD_DT, SUB_OFC_CD, RGN_OFC_CD, IOC_TS_CD) AS CD_DP_SEQ," ).append("\n"); 
		query.append("           SUB_OFC_CD," ).append("\n"); 
		query.append("           RGN_OFC_CD," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           CTRL_LVL  ," ).append("\n"); 
		query.append("           CTRL_HC   ," ).append("\n"); 
		query.append("           CTRL_45   ," ).append("\n"); 
		query.append("           CTRL_53   ," ).append("\n"); 
		query.append("           CTRL_RF   ," ).append("\n"); 
		query.append("           CTRL_WT   ," ).append("\n"); 
		query.append("           CTRL_ACCT ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , BSE_QTR_CD" ).append("\n"); 
		query.append("      FROM (  SELECT 1 AS FLG        ," ).append("\n"); 
		query.append("                     V.SEASON        ," ).append("\n"); 
		query.append("                     V.SLS_YRMON     ," ).append("\n"); 
		query.append("                     V.COST_YRMON    ," ).append("\n"); 
		query.append("                     V.COST_WK       ," ).append("\n"); 
		query.append("                     V.REP_TRD_CD    ," ).append("\n"); 
		query.append("                     V.REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                     V.TRD_CD        ," ).append("\n"); 
		query.append("                     V.SUB_TRD_CD    ," ).append("\n"); 
		query.append("                     V.RLANE_CD      ," ).append("\n"); 
		query.append("                     V.VSL_CD        ," ).append("\n"); 
		query.append("                     V.SKD_VOY_NO    ," ).append("\n"); 
		query.append("                     V.SKD_DIR_CD    ," ).append("\n"); 
		query.append("                     M.POL_CD        ," ).append("\n"); 
		query.append("                     V.ETD_DT        ," ).append("\n"); 
		query.append("                     M.CD_DP_SEQ     ," ).append("\n"); 
		query.append("                     M.SLS_OFC_CD     AS SUB_OFC_CD," ).append("\n"); 
		query.append("                     M.SLS_RGN_OFC_CD AS RGN_OFC_CD," ).append("\n"); 
		query.append("                     M.IOC_TS_CD     ," ).append("\n"); 
		query.append("                     V.CTRL_LVL      ," ).append("\n"); 
		query.append("                     V.CTRL_HC       ," ).append("\n"); 
		query.append("                     V.CTRL_45       ," ).append("\n"); 
		query.append("                     V.CTRL_53       ," ).append("\n"); 
		query.append("                     V.CTRL_RF       ," ).append("\n"); 
		query.append("                     V.CTRL_WT       ," ).append("\n"); 
		query.append("                     V.CTRL_ACCT     ," ).append("\n"); 
		query.append("                     V.HH_FLG" ).append("\n"); 
		query.append("                     , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                     , V.BSE_QTR_CD" ).append("\n"); 
		query.append("                FROM VVDLS                  V," ).append("\n"); 
		query.append("                     SPC_FCAST_OFC_POL_MAPG M," ).append("\n"); 
		query.append("                     PARAMS                 P" ).append("\n"); 
		query.append("               WHERE M.REP_TRD_CD     = V.REP_TRD_CD" ).append("\n"); 
		query.append("                 AND M.REP_SUB_TRD_CD = V.REP_SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND M.TRD_CD         = V.TRD_CD" ).append("\n"); 
		query.append("                 AND M.SUB_TRD_CD     = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND M.RLANE_CD       = V.RLANE_CD" ).append("\n"); 
		query.append("                 AND M.DIR_CD         = V.DIR_CD" ).append("\n"); 
		query.append("                 AND M.IOC_TS_CD      = P.IOC_TS_CD" ).append("\n"); 
		query.append("                 AND M.SLS_OFC_CD     = P.RGN_OFC_CD" ).append("\n"); 
		query.append("                 AND M.BSE_YRWK       = ( SELECT /*+ INDEX_DESC(MI XPKSPC_FCAST_OFC_POL_MAPG) */MI.BSE_YRWK" ).append("\n"); 
		query.append("                                            FROM SPC_FCAST_OFC_POL_MAPG MI" ).append("\n"); 
		query.append("                                           WHERE MI.REP_TRD_CD     = M.REP_TRD_CD" ).append("\n"); 
		query.append("                                             AND MI.REP_SUB_TRD_CD = M.REP_SUB_TRD_CD" ).append("\n"); 
		query.append("                                             AND MI.RLANE_CD       = M.RLANE_CD" ).append("\n"); 
		query.append("                                             AND MI.DIR_CD         = M.DIR_CD" ).append("\n"); 
		query.append("                                             AND MI.IOC_TS_CD      = M.IOC_TS_CD" ).append("\n"); 
		query.append("                                             AND MI.SLS_OFC_CD     = M.SLS_OFC_CD" ).append("\n"); 
		query.append("                                             AND MI.BSE_YRWK      <= SUBSTR(V.SLS_YRMON, 1, 4)||V.COST_WK" ).append("\n"); 
		query.append("                                             AND ROWNUM = 1 )" ).append("\n"); 
		query.append("              UNION ALL" ).append("\n"); 
		query.append("              SELECT 2 AS FLG        ," ).append("\n"); 
		query.append("                     V.SEASON        ," ).append("\n"); 
		query.append("                     V.SLS_YRMON     ," ).append("\n"); 
		query.append("                     V.COST_YRMON    ," ).append("\n"); 
		query.append("                     V.COST_WK       ," ).append("\n"); 
		query.append("                     V.REP_TRD_CD    ," ).append("\n"); 
		query.append("                     V.REP_SUB_TRD_CD," ).append("\n"); 
		query.append("                     V.TRD_CD        ," ).append("\n"); 
		query.append("                     V.SUB_TRD_CD    ," ).append("\n"); 
		query.append("                     V.RLANE_CD      ," ).append("\n"); 
		query.append("                     V.VSL_CD        ," ).append("\n"); 
		query.append("                     V.SKD_VOY_NO    ," ).append("\n"); 
		query.append("                     V.SKD_DIR_CD    ," ).append("\n"); 
		query.append("                     M.POL_CD        ," ).append("\n"); 
		query.append("                     V.ETD_DT        ," ).append("\n"); 
		query.append("                     M.CD_DP_SEQ + 100 AS CD_DP_SEQ ," ).append("\n"); 
		query.append("                     M.SLS_OFC_CD      AS SUB_OFC_CD," ).append("\n"); 
		query.append("                     M.SLS_RGN_OFC_CD  AS RGN_OFC_CD," ).append("\n"); 
		query.append("                     M.IOC_TS_CD     ," ).append("\n"); 
		query.append("                     V.CTRL_LVL      ," ).append("\n"); 
		query.append("                     V.CTRL_HC       ," ).append("\n"); 
		query.append("                     V.CTRL_45       ," ).append("\n"); 
		query.append("                     V.CTRL_53       ," ).append("\n"); 
		query.append("                     V.CTRL_RF       ," ).append("\n"); 
		query.append("                     V.CTRL_WT       ," ).append("\n"); 
		query.append("                     V.CTRL_ACCT     ," ).append("\n"); 
		query.append("                     V.HH_FLG" ).append("\n"); 
		query.append("                     , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                     , V.BSE_QTR_CD" ).append("\n"); 
		query.append("                FROM VVDLS                      V," ).append("\n"); 
		query.append("                     SPC_IRR_FCAST_OFC_POL_MAPG M," ).append("\n"); 
		query.append("                     PARAMS                     P" ).append("\n"); 
		query.append("               WHERE M.REP_TRD_CD     = V.REP_TRD_CD" ).append("\n"); 
		query.append("                 AND M.REP_SUB_TRD_CD = V.REP_SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND M.TRD_CD         = V.TRD_CD" ).append("\n"); 
		query.append("                 AND M.SUB_TRD_CD     = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND M.RLANE_CD       = V.RLANE_CD" ).append("\n"); 
		query.append("                 AND M.DIR_CD         = V.DIR_CD" ).append("\n"); 
		query.append("                 AND M.IOC_TS_CD      = P.IOC_TS_CD" ).append("\n"); 
		query.append("                 AND M.VSL_CD         = V.VSL_CD" ).append("\n"); 
		query.append("                 AND M.SKD_VOY_NO     = V.SKD_VOY_NO" ).append("\n"); 
		query.append("                 AND M.SKD_DIR_CD     = V.SKD_DIR_CD" ).append("\n"); 
		query.append("                 AND M.SLS_OFC_CD     = P.RGN_OFC_CD" ).append("\n"); 
		query.append("           ) M," ).append("\n"); 
		query.append("           VSK_VSL_PORT_SKD V" ).append("\n"); 
		query.append("     WHERE M.VSL_CD     = V.VSL_CD" ).append("\n"); 
		query.append("       AND M.SKD_VOY_NO = V.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND M.SKD_DIR_CD = V.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND M.POL_CD     = V.VPS_PORT_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_POL_POD AS (" ).append("\n"); 
		query.append("    SELECT SEASON    ," ).append("\n"); 
		query.append("           SLS_YRMON ," ).append("\n"); 
		query.append("           COST_WK   ," ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           VSL_CD    ," ).append("\n"); 
		query.append("           SKD_VOY_NO," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           POL_CD    ," ).append("\n"); 
		query.append("           POL_SEQ   ," ).append("\n"); 
		query.append("           CD_DP_SEQ ," ).append("\n"); 
		query.append("           POL_CONTI ," ).append("\n"); 
		query.append("           ETD_DT    ," ).append("\n"); 
		query.append("           POD_CD    ," ).append("\n"); 
		query.append("           MIN(POD_SEQ) AS POD_SEQ," ).append("\n"); 
		query.append("           RGN_OFC_CD," ).append("\n"); 
		query.append("           SUB_OFC_CD," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POD_CONTI ," ).append("\n"); 
		query.append("           POL_SKIP  ," ).append("\n"); 
		query.append("           CTRL_LVL  ," ).append("\n"); 
		query.append("           CTRL_HC   ," ).append("\n"); 
		query.append("           CTRL_45   ," ).append("\n"); 
		query.append("           CTRL_53   ," ).append("\n"); 
		query.append("           CTRL_RF   ," ).append("\n"); 
		query.append("           CTRL_WT   ," ).append("\n"); 
		query.append("           CTRL_ACCT ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM (  SELECT SL.SEASON    ," ).append("\n"); 
		query.append("                     SL.SLS_YRMON ," ).append("\n"); 
		query.append("                     SL.COST_WK   ," ).append("\n"); 
		query.append("                     RL.TRD_CD    ," ).append("\n"); 
		query.append("                     RL.SUB_TRD_CD," ).append("\n"); 
		query.append("                     RL.RLANE_CD  ," ).append("\n"); 
		query.append("                     SL.VSL_CD    ," ).append("\n"); 
		query.append("                     SL.SKD_VOY_NO," ).append("\n"); 
		query.append("                     SL.SKD_DIR_CD," ).append("\n"); 
		query.append("                     SL.POL_CD    ," ).append("\n"); 
		query.append("                     SL.POL_SEQ   ," ).append("\n"); 
		query.append("                     SL.CD_DP_SEQ ," ).append("\n"); 
		query.append("                     SL.POL_CONTI ," ).append("\n"); 
		query.append("                     SL.ETD_DT    ," ).append("\n"); 
		query.append("                     SD.YD_CD AS POD_CD," ).append("\n"); 
		query.append("                     TO_NUMBER(DECODE(SD.SKD_CNG_STS_CD, 'S', NULL, SD.CLPT_SEQ)) AS POD_SEQ," ).append("\n"); 
		query.append("                     SL.RGN_OFC_CD," ).append("\n"); 
		query.append("                     SL.SUB_OFC_CD," ).append("\n"); 
		query.append("                     SL.IOC_TS_CD ," ).append("\n"); 
		query.append("                     SPC_CONTI_CONV_FNC(MLOC.CONTI_CD,SL.RLANE_CD,SL.SKD_DIR_CD) AS POD_CONTI," ).append("\n"); 
		query.append("                     DECODE(SL.POL_SEQ, NULL, 'Y', 'N')            AS POL_SKIP ," ).append("\n"); 
		query.append("                     SL.CTRL_LVL  ," ).append("\n"); 
		query.append("                     SL.CTRL_HC   ," ).append("\n"); 
		query.append("                     SL.CTRL_45   ," ).append("\n"); 
		query.append("                     SL.CTRL_53   ," ).append("\n"); 
		query.append("                     SL.CTRL_RF   ," ).append("\n"); 
		query.append("                     SL.CTRL_WT   ," ).append("\n"); 
		query.append("                     SL.CTRL_ACCT ," ).append("\n"); 
		query.append("                     RL.IOC_CD    ," ).append("\n"); 
		query.append("                     SL.HH_FLG" ).append("\n"); 
		query.append("                     , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                FROM VSK_VSL_PORT_SKD SD  ," ).append("\n"); 
		query.append("                     VVD_POL          SL  ," ).append("\n"); 
		query.append("                     MDM_DTL_REV_LANE RL  ," ).append("\n"); 
		query.append("                     MDM_LOCATION     MLOC," ).append("\n"); 
		query.append("                     PARAMS           P" ).append("\n"); 
		query.append("               WHERE SUBSTR(sl.pol_cd, 1, 5) NOT IN ('EGSUZ', 'PAPAC')" ).append("\n"); 
		query.append("                 AND SD.VPS_PORT_CD NOT IN ('EGSUZ', 'PAPAC')" ).append("\n"); 
		query.append("                 AND SL.POL_SEQ        >= 0" ).append("\n"); 
		query.append("                 AND SD.VSL_CD          = SL.VSL_CD" ).append("\n"); 
		query.append("                 AND SD.SKD_VOY_NO      = SL.SKD_VOY_NO" ).append("\n"); 
		query.append("                 AND SD.SKD_DIR_CD      = SL.SKD_DIR_CD" ).append("\n"); 
		query.append("                 AND (SD.SKD_CNG_STS_CD IS NULL OR SD.SKD_CNG_STS_CD <> 'S')" ).append("\n"); 
		query.append("                 AND SD.CLPT_SEQ        > SL.POL_SEQ" ).append("\n"); 
		query.append("                 AND SD.SLAN_CD         = P.VSL_SLAN_CD" ).append("\n"); 
		query.append("                 AND RL.RLANE_CD        = SL.RLANE_CD" ).append("\n"); 
		query.append("                 AND RL.VSL_SLAN_DIR_CD = SL.SKD_DIR_CD" ).append("\n"); 
		query.append("                 AND RL.FM_CONTI_CD     = SL.POL_CONTI" ).append("\n"); 
		query.append("                 AND SD.VPS_PORT_CD     = MLOC.LOC_CD" ).append("\n"); 
		query.append("                 AND RL.TO_CONTI_CD     = SPC_CONTI_CONV_FNC(MLOC.CONTI_CD, SL.RLANE_CD,SL.SKD_DIR_CD)" ).append("\n"); 
		query.append("                 AND RL.DELT_FLG        = 'N'" ).append("\n"); 
		query.append("                 AND DECODE(RL.FM_CONTI_CD,RL.TO_CONTI_CD, 'I', 'O') = DECODE(P.IOC_TS_CD, 'O', 'O', 'I')" ).append("\n"); 
		query.append("                 AND SL.POL_CD <> SD.YD_CD" ).append("\n"); 
		query.append("           )" ).append("\n"); 
		query.append("  GROUP BY SEASON    ," ).append("\n"); 
		query.append("           SLS_YRMON ," ).append("\n"); 
		query.append("           COST_WK   ," ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           VSL_CD    ," ).append("\n"); 
		query.append("           SKD_VOY_NO," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           POL_CD    ," ).append("\n"); 
		query.append("           POL_SEQ   ," ).append("\n"); 
		query.append("           CD_DP_SEQ ," ).append("\n"); 
		query.append("           POL_CONTI ," ).append("\n"); 
		query.append("           ETD_DT    ," ).append("\n"); 
		query.append("           POD_CD    ," ).append("\n"); 
		query.append("           RGN_OFC_CD," ).append("\n"); 
		query.append("           SUB_OFC_CD," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POD_CONTI ," ).append("\n"); 
		query.append("           POL_SKIP  ," ).append("\n"); 
		query.append("           CTRL_LVL  ," ).append("\n"); 
		query.append("           CTRL_HC   ," ).append("\n"); 
		query.append("           CTRL_45   ," ).append("\n"); 
		query.append("           CTRL_53   ," ).append("\n"); 
		query.append("           CTRL_RF   ," ).append("\n"); 
		query.append("           CTRL_WT   ," ).append("\n"); 
		query.append("           CTRL_ACCT ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", PARAM_OFC AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT RGN_OFC_CD, SUB_OFC_CD, SALES_REP_CD FROM PARAMS" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", OFFICES AS (" ).append("\n"); 
		query.append("#if (${salesoffice} != 'SELSC')" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           1 AS FLG    ," ).append("\n"); 
		query.append("           O.OFC_CD    ," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           SUBSTR(SYS_CONNECT_BY_PATH(OFC_CD, '/'), DECODE(LEVEL, 1, 2, 8), 5) AS SUB_OFC_CD," ).append("\n"); 
		query.append("           P.SALES_REP_CD" ).append("\n"); 
		query.append("      FROM SPC_OFC_LVL O," ).append("\n"); 
		query.append("           PARAM_OFC           P" ).append("\n"); 
		query.append("CONNECT BY PRIOR O.OFC_CD      = O.PRNT_OFC_CD" ).append("\n"); 
		query.append("             AND O.DELT_FLG    = 'N'" ).append("\n"); 
		query.append("             AND O.OFC_TP_CD  IN ('BB', 'BA', 'BS')" ).append("\n"); 
		query.append("             AND TO_CHAR(SYSDATE,'YYYYWW') BETWEEN O.OFC_APLY_FM_YRWK AND O.OFC_APLY_TO_YRWK" ).append("\n"); 
		query.append("      START WITH O.OFC_CD      = P.RGN_OFC_CD" ).append("\n"); 
		query.append("             AND P.SUB_OFC_CD IS NULL" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           2 AS FLG    ," ).append("\n"); 
		query.append("           O.OFC_CD    ," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           SUBSTR(SYS_CONNECT_BY_PATH(OFC_CD, '/'), 2, 5) AS SUB_OFC_CD," ).append("\n"); 
		query.append("           P.SALES_REP_CD" ).append("\n"); 
		query.append("      FROM SPC_OFC_LVL O," ).append("\n"); 
		query.append("           PARAM_OFC           P" ).append("\n"); 
		query.append("CONNECT BY PRIOR O.OFC_CD       = O.PRNT_OFC_CD" ).append("\n"); 
		query.append("             AND O.DELT_FLG     = 'N'" ).append("\n"); 
		query.append("             AND O.OFC_TP_CD   IN ('BB', 'BA', 'BS')" ).append("\n"); 
		query.append("             AND TO_CHAR(SYSDATE,'YYYYWW') BETWEEN O.OFC_APLY_FM_YRWK AND O.OFC_APLY_TO_YRWK" ).append("\n"); 
		query.append("             AND (P.RGN_OFC_CD <> P.SUB_OFC_CD OR LEVEL = 1)" ).append("\n"); 
		query.append("      START WITH O.OFC_CD       = P.SUB_OFC_CD" ).append("\n"); 
		query.append("             AND P.SUB_OFC_CD  IS NOT NULL" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           1 AS FLG," ).append("\n"); 
		query.append("           O.SLS_OFC_CD AS OFC_CD," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           DECODE(P.RGN_OFC_CD, 'SELSC', DECODE(O.SLS_REP_OFC_TEAM_CD, NULL, O.SLS_OFC_CD, O.SLS_REP_OFC_TEAM_CD), O.SLS_REP_OFC_TEAM_CD) AS SUB_OFC_CD," ).append("\n"); 
		query.append("           O.SREP_USR_ID AS SALES_REP_CD" ).append("\n"); 
		query.append("      FROM SPC_SLS_REP_TEAM_IF O," ).append("\n"); 
		query.append("           PARAM_OFC              P" ).append("\n"); 
		query.append("     WHERE O.SLS_RGN_OFC_CD = P.RGN_OFC_CD" ).append("\n"); 
		query.append("       AND O.SREP_USR_ID LIKE P.SALES_REP_CD||'%'" ).append("\n"); 
		query.append("       AND P.SUB_OFC_CD IS NULL" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           2 AS FLG," ).append("\n"); 
		query.append("           O.SLS_OFC_CD AS OFC_CD," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           DECODE(P.RGN_OFC_CD, 'SELSC', DECODE(O.SLS_REP_OFC_TEAM_CD, NULL, O.SLS_OFC_CD, O.SLS_REP_OFC_TEAM_CD), O.SLS_REP_OFC_TEAM_CD) AS SUB_OFC_CD," ).append("\n"); 
		query.append("           O.SREP_USR_ID AS SALES_REP_CD" ).append("\n"); 
		query.append("      FROM SPC_SLS_REP_TEAM_IF O," ).append("\n"); 
		query.append("           PARAM_OFC              P" ).append("\n"); 
		query.append("     WHERE DECODE(O.SLS_REP_OFC_TEAM_CD, NULL, O.SLS_OFC_CD, O.SLS_REP_OFC_TEAM_CD) LIKE P.SUB_OFC_CD||'%'" ).append("\n"); 
		query.append("       AND O.SREP_USR_ID LIKE P.SALES_REP_CD||'%'" ).append("\n"); 
		query.append("       AND P.SUB_OFC_CD IS NOT NULL" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", SREPS AS (" ).append("\n"); 
		query.append("    SELECT /*+ USE_NL(O SR) */SR.SREP_CD  ," ).append("\n"); 
		query.append("           SR.SREP_NM  ," ).append("\n"); 
		query.append("           O.RGN_OFC_CD," ).append("\n"); 
		query.append("           O.SUB_OFC_CD," ).append("\n"); 
		query.append("           O.OFC_CD" ).append("\n"); 
		query.append("      FROM MDM_SLS_REP SR," ).append("\n"); 
		query.append("           OFFICES O" ).append("\n"); 
		query.append("     WHERE 1=1 " ).append("\n"); 
		query.append("       AND SPC_SCR_OFC_CONV_FNC(SR.OFC_CD) = O.OFC_CD" ).append("\n"); 
		query.append("       AND O.RGN_OFC_CD   = O.RGN_OFC_CD" ).append("\n"); 
		query.append("       AND SR.SREP_CD LIKE O.SALES_REP_CD||'%'" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_PORT_SREP AS (" ).append("\n"); 
		query.append("    SELECT SUBSTR(V.SEASON,1,6) AS SEASON ," ).append("\n"); 
		query.append("           SUBSTR(V.SEASON,8)   AS VER_SEQ," ).append("\n"); 
		query.append("           V.SLS_YRMON           ," ).append("\n"); 
		query.append("           V.COST_WK             ," ).append("\n"); 
		query.append("           V.TRD_CD              ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD          ," ).append("\n"); 
		query.append("           V.RLANE_CD            ," ).append("\n"); 
		query.append("           V.SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("           V.IOC_TS_CD           ," ).append("\n"); 
		query.append("           SR.RGN_OFC_CD         ," ).append("\n"); 
		query.append("           SR.SUB_OFC_CD         ," ).append("\n"); 
		query.append("           SR.OFC_CD             ," ).append("\n"); 
		query.append("           SR.SREP_CD            ," ).append("\n"); 
		query.append("           SR.SREP_NM            ," ).append("\n"); 
		query.append("           V.POL_CD              ," ).append("\n"); 
		query.append("           V.POD_CD              ," ).append("\n"); 
		query.append("           V.POL_SKIP            ," ).append("\n"); 
		query.append("           V.ETD_DT              ," ).append("\n"); 
		query.append("           V.VSL_CD              ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO          ," ).append("\n"); 
		query.append("           V.SKD_DIR_CD          ," ).append("\n"); 
		query.append("           V.CTRL_LVL            ," ).append("\n"); 
		query.append("           V.CTRL_HC             ," ).append("\n"); 
		query.append("           V.CTRL_45             ," ).append("\n"); 
		query.append("           V.CTRL_53             ," ).append("\n"); 
		query.append("           V.CTRL_RF             ," ).append("\n"); 
		query.append("           V.CTRL_WT             ," ).append("\n"); 
		query.append("           V.CTRL_ACCT           ," ).append("\n"); 
		query.append("           V.HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM SREPS       SR," ).append("\n"); 
		query.append("           VVD_POL_POD V" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_PORT_SREP_CUST AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           VPS.COST_YR   ," ).append("\n"); 
		query.append("           VPS.COST_WK   ," ).append("\n"); 
		query.append("           VPS.TRD_CD    ," ).append("\n"); 
		query.append("           VPS.SUB_TRD_CD," ).append("\n"); 
		query.append("           VPS.RLANE_CD  ," ).append("\n"); 
		query.append("           VPS.DIR_CD    ," ).append("\n"); 
		query.append("           VPS.IOC_TS_CD ," ).append("\n"); 
		query.append("           VPS.RGN_OFC_CD," ).append("\n"); 
		query.append("           VPS.SUB_OFC_CD," ).append("\n"); 
		query.append("           VPS.OFC_CD    ," ).append("\n"); 
		query.append("           VPS.SREP_CD   ," ).append("\n"); 
		query.append("           VPS.SREP_NM   ," ).append("\n"); 
		query.append("           VPS.CUST_TP_CD," ).append("\n"); 
		query.append("           NVL(C.CUST_CTRL_CD, 'C') AS CUST_CTRL_CD," ).append("\n"); 
		query.append("           NVL(C.DTL_SEQ, 1)        AS DTL_SEQ     ," ).append("\n"); 
		query.append("           DECODE(VPS.HH_FLG, 'Y', C.SC_NO)  AS SC_NO," ).append("\n"); 
		query.append("           DECODE(VPS.HH_FLG, 'Y', C.RFA_NO) AS RFA_NO," ).append("\n"); 
		query.append("           NVL(( SELECT SUM(M.RLANE_ADJ_QTY)" ).append("\n"); 
		query.append("                       FROM SPC_MDL_CUST_REV_LANE M" ).append("\n"); 
		query.append("                      WHERE M.COST_YRWK      = VPS.SEASON" ).append("\n"); 
		query.append("                        AND M.VER_SEQ        = VPS.VER_SEQ" ).append("\n"); 
		query.append("                        AND M.TRD_CD         = VPS.TRD_CD" ).append("\n"); 
		query.append("                        AND M.SUB_TRD_CD     = VPS.SUB_TRD_CD" ).append("\n"); 
		query.append("                        AND M.RLANE_CD       = VPS.RLANE_CD" ).append("\n"); 
		query.append("                        AND M.SLS_RGN_OFC_CD = VPS.RGN_OFC_CD" ).append("\n"); 
		query.append("                        AND M.CUST_CNT_CD    = VPS.CUST_CNT_CD" ).append("\n"); 
		query.append("                        AND M.CUST_SEQ       = VPS.CUST_SEQ" ).append("\n"); 
		query.append("                        AND NVL(M.SC_NO,NVL(M.RFA_NO,'X')) = NVL(C.SC_NO,NVL(C.RFA_NO,'X'))" ).append("\n"); 
		query.append("                        AND M.DELT_FLG       = 'N'" ).append("\n"); 
		query.append("                   ),0) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("           VPS.CUST_CNT_CD," ).append("\n"); 
		query.append("           VPS.CUST_SEQ  ," ).append("\n"); 
		query.append("           VPS.POL_CD    ," ).append("\n"); 
		query.append("           VPS.POD_CD    ," ).append("\n"); 
		query.append("           VPS.VSL_CD    ," ).append("\n"); 
		query.append("           VPS.SKD_VOY_NO," ).append("\n"); 
		query.append("           VPS.SKD_DIR_CD," ).append("\n"); 
		query.append("           VPS.CTRL_LVL  ," ).append("\n"); 
		query.append("           VPS.CTRL_HC   ," ).append("\n"); 
		query.append("           VPS.CTRL_45   ," ).append("\n"); 
		query.append("           VPS.CTRL_53   ," ).append("\n"); 
		query.append("           VPS.CTRL_RF   ," ).append("\n"); 
		query.append("           VPS.CTRL_WT   ," ).append("\n"); 
		query.append("           VPS.CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("            SELECT SUBSTR(VPS.SLS_YRMON, 1, 4) AS COST_YR," ).append("\n"); 
		query.append("                   VPS.COST_WK   ," ).append("\n"); 
		query.append("                   VPS.SEASON    ," ).append("\n"); 
		query.append("                   VPS.VER_SEQ   ," ).append("\n"); 
		query.append("                   VPS.TRD_CD    ," ).append("\n"); 
		query.append("                   VPS.SUB_TRD_CD," ).append("\n"); 
		query.append("                   VPS.RLANE_CD  ," ).append("\n"); 
		query.append("                   VPS.SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("                   VPS.IOC_TS_CD ," ).append("\n"); 
		query.append("                   VPS.RGN_OFC_CD," ).append("\n"); 
		query.append("                   VPS.SUB_OFC_CD," ).append("\n"); 
		query.append("                   VPS.OFC_CD    ," ).append("\n"); 
		query.append("                   VPS.SREP_CD   ," ).append("\n"); 
		query.append("                   VPS.SREP_NM   ," ).append("\n"); 
		query.append("                   CM.RVIS_CNTR_CUST_TP_CD AS CUST_TP_CD," ).append("\n"); 
		query.append("                   VPS.HH_FLG," ).append("\n"); 
		query.append("                   CM.CUST_CNT_CD," ).append("\n"); 
		query.append("                   CM.CUST_SEQ   ," ).append("\n"); 
		query.append("                   VPS.POL_CD    ," ).append("\n"); 
		query.append("                   VPS.POD_CD    ," ).append("\n"); 
		query.append("                   VPS.VSL_CD    ," ).append("\n"); 
		query.append("                   VPS.SKD_VOY_NO," ).append("\n"); 
		query.append("                   VPS.SKD_DIR_CD," ).append("\n"); 
		query.append("                   VPS.CTRL_LVL  ," ).append("\n"); 
		query.append("                   VPS.CTRL_HC   ," ).append("\n"); 
		query.append("                   VPS.CTRL_45   ," ).append("\n"); 
		query.append("                   VPS.CTRL_53   ," ).append("\n"); 
		query.append("                   VPS.CTRL_RF   ," ).append("\n"); 
		query.append("                   VPS.CTRL_WT   ," ).append("\n"); 
		query.append("                   VPS.CTRL_ACCT" ).append("\n"); 
		query.append("                   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("              FROM SPC_SLS_REP_CUST CM ," ).append("\n"); 
		query.append("                   VVD_PORT_SREP    VPS" ).append("\n"); 
		query.append("             WHERE CM.SREP_CD   (+) = VPS.SREP_CD" ).append("\n"); 
		query.append("               AND NVL(CM.DELT_FLG, 'N') = 'N'" ).append("\n"); 
		query.append("               AND NVL(CM.INDIV_CUST_USE_FLG, 'Y') = 'Y'" ).append("\n"); 
		query.append("               AND CM.TRD_CD     (+) = VPS.TRD_CD" ).append("\n"); 
		query.append("               AND CM.SUB_TRD_CD (+) = VPS.SUB_TRD_CD" ).append("\n"); 
		query.append("            ) VPS," ).append("\n"); 
		query.append("            SPC_MDL_CUST_CTRL C" ).append("\n"); 
		query.append("      WHERE VPS.SEASON      = C.COST_YRWK(+)" ).append("\n"); 
		query.append("        AND VPS.VER_SEQ     = C.VER_SEQ(+)" ).append("\n"); 
		query.append("        AND VPS.CUST_CNT_CD = C.CUST_CNT_CD(+)" ).append("\n"); 
		query.append("        AND VPS.CUST_SEQ    = C.CUST_SEQ(+)" ).append("\n"); 
		query.append("        AND VPS.TRD_CD      = C.TRD_CD(+)" ).append("\n"); 
		query.append("        AND NVL(C.DELT_FLG(+),'N') = 'N'" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", USMODE AS (" ).append("\n"); 
		query.append("SELECT INTG_CD_VAL_CTNT USA_BKG_MOD_CD FROM COM_INTG_CD_DTL WHERE INTG_CD_ID = 'CD00777' AND INTG_CD_VAL_CTNT NOT IN('MLB', 'PORT')" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", MASTER_DATA AS (" ).append("\n"); 
		query.append("    SELECT 2 AS LVL1            ," ).append("\n"); 
		query.append("           3 - (GROUPING_ID(USA_BKG_MOD_CD) + GROUPING_ID(T.POL_CD) + GROUPING_ID(T.POD_CD)) AS LVL2," ).append("\n"); 
		query.append("           T.TRD_CD             ," ).append("\n"); 
		query.append("           T.SUB_TRD_CD         ," ).append("\n"); 
		query.append("           T.RLANE_CD           ," ).append("\n"); 
		query.append("           T.DIR_CD             ," ).append("\n"); 
		query.append("           T.IOC_TS_CD          ," ).append("\n"); 
		query.append("           T.RGN_OFC_CD         ," ).append("\n"); 
		query.append("           NULL AS OFC_CD       ," ).append("\n"); 
		query.append("           NULL AS SUB_OFC_CD   ," ).append("\n"); 
		query.append("           NULL AS SREP_CD      ," ).append("\n"); 
		query.append("           NULL AS SREP_NM      ," ).append("\n"); 
		query.append("           NULL AS CUST_TP_CD   ," ).append("\n"); 
		query.append("           NULL AS CUST_CTRL_CD ," ).append("\n"); 
		query.append("           NULL AS DTL_SEQ      ," ).append("\n"); 
		query.append("           NULL AS SC_NO        ," ).append("\n"); 
		query.append("           NULL AS RFA_NO       ," ).append("\n"); 
		query.append("           NULL AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("           NULL AS CUST_CNT_CD  ," ).append("\n"); 
		query.append("           NULL AS CUST_SEQ     ," ).append("\n"); 
		query.append("           T.COST_YR            ," ).append("\n"); 
		query.append("           T.COST_WK            ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD       ," ).append("\n"); 
		query.append("           T.POL_CD             ," ).append("\n"); 
		query.append("           T.POD_CD             ," ).append("\n"); 
		query.append("           T.VSL_CD             ," ).append("\n"); 
		query.append("           T.SKD_VOY_NO         ," ).append("\n"); 
		query.append("           T.SKD_DIR_CD         ," ).append("\n"); 
		query.append("           0 AS CHL             ," ).append("\n"); 
		query.append("           T.CTRL_LVL           ," ).append("\n"); 
		query.append("           T.CTRL_HC            ," ).append("\n"); 
		query.append("           T.CTRL_45            ," ).append("\n"); 
		query.append("           T.CTRL_53            ," ).append("\n"); 
		query.append("           T.CTRL_RF            ," ).append("\n"); 
		query.append("           T.CTRL_WT            ," ).append("\n"); 
		query.append("           T.CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM VVD_PORT_SREP_CUST T" ).append("\n"); 
		query.append("     , USMODE US" ).append("\n"); 
		query.append("     WHERE US.USA_BKG_MOD_CD = CASE WHEN NVL(CTRL_USA_SVC_MOD_FLG, 'N') = 'N' THEN 'OTH' ELSE US.USA_BKG_MOD_CD END" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( T.TRD_CD    , T.SUB_TRD_CD, T.RLANE_CD  , T.DIR_CD    , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.POL_CD    , T.POD_CD    , T.VSL_CD    , T.SKD_VOY_NO," ).append("\n"); 
		query.append("                             T.SKD_DIR_CD, T.COST_YR   , T.COST_WK   , T.CTRL_LVL  , T.CTRL_HC   ," ).append("\n"); 
		query.append("                             T.CTRL_45   , T.CTRL_53   , T.CTRL_RF   , T.CTRL_WT   , T.CTRL_ACCT  " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD), " ).append("\n"); 
		query.append("                             --Office, USMode, POL, POD" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD, T.RLANE_CD  , T.DIR_CD    , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.POL_CD    , T.VSL_CD    , T.SKD_VOY_NO, T.SKD_DIR_CD," ).append("\n"); 
		query.append("                             T.COST_YR   , T.COST_WK   , T.CTRL_LVL  , T.CTRL_HC   , T.CTRL_45   ," ).append("\n"); 
		query.append("                             T.CTRL_53   , T.CTRL_RF   , T.CTRL_WT , T.CTRL_ACCT " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD), " ).append("\n"); 
		query.append("                             --Office, USMode, POL" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD, T.RLANE_CD  , T.DIR_CD    , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.POL_CD    , T.VSL_CD    , T.SKD_VOY_NO, T.SKD_DIR_CD," ).append("\n"); 
		query.append("                             T.COST_YR   , T.COST_WK   , T.CTRL_LVL  , T.CTRL_HC   , T.CTRL_45   ," ).append("\n"); 
		query.append("                             T.CTRL_53   , T.CTRL_RF   , T.CTRL_WT , T.CTRL_ACCT " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)," ).append("\n"); 
		query.append("                             --Office, POL" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD, T.RLANE_CD  , T.DIR_CD    , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.VSL_CD    , T.SKD_VOY_NO, T.SKD_DIR_CD, T.COST_YR   ," ).append("\n"); 
		query.append("                             T.COST_WK   , T.CTRL_LVL  , T.CTRL_HC   , T.CTRL_45   , T.CTRL_53   ," ).append("\n"); 
		query.append("                             T.CTRL_RF   , T.CTRL_WT , T.CTRL_ACCT      " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD), " ).append("\n"); 
		query.append("                             --Office, USMode" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD, T.RLANE_CD  , T.DIR_CD    , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.VSL_CD    , T.SKD_VOY_NO, T.SKD_DIR_CD, T.COST_YR   ," ).append("\n"); 
		query.append("                             T.COST_WK   , T.CTRL_LVL  , T.CTRL_HC   , T.CTRL_45   , T.CTRL_53   ," ).append("\n"); 
		query.append("                             T.CTRL_RF   , T.CTRL_WT , T.CTRL_ACCT      " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG) " ).append("\n"); 
		query.append("                             --Office" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT 4 AS LVL1      ," ).append("\n"); 
		query.append("           4-(GROUPING_ID(USA_BKG_MOD_CD) +  GROUPING_ID(T.CUST_CNT_CD) + GROUPING_ID(T.POL_CD) + GROUPING_ID(T.POD_CD)) AS LVL2," ).append("\n"); 
		query.append("           T.TRD_CD       ," ).append("\n"); 
		query.append("           T.SUB_TRD_CD   ," ).append("\n"); 
		query.append("           T.RLANE_CD     ," ).append("\n"); 
		query.append("           T.DIR_CD       ," ).append("\n"); 
		query.append("           T.IOC_TS_CD    ," ).append("\n"); 
		query.append("           T.RGN_OFC_CD   ," ).append("\n"); 
		query.append("           T.OFC_CD       ," ).append("\n"); 
		query.append("           T.SUB_OFC_CD   ," ).append("\n"); 
		query.append("           T.SREP_CD      ," ).append("\n"); 
		query.append("           T.SREP_NM      ," ).append("\n"); 
		query.append("           T.CUST_TP_CD   ," ).append("\n"); 
		query.append("           T.CUST_CTRL_CD ," ).append("\n"); 
		query.append("           T.DTL_SEQ      ," ).append("\n"); 
		query.append("           T.SC_NO        ," ).append("\n"); 
		query.append("           T.RFA_NO       ," ).append("\n"); 
		query.append("           T.RLANE_ADJ_QTY," ).append("\n"); 
		query.append("           T.CUST_CNT_CD  ," ).append("\n"); 
		query.append("           T.CUST_SEQ     ," ).append("\n"); 
		query.append("           T.COST_YR      ," ).append("\n"); 
		query.append("           T.COST_WK      ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD ," ).append("\n"); 
		query.append("           T.POL_CD       ," ).append("\n"); 
		query.append("           T.POD_CD       ," ).append("\n"); 
		query.append("           T.VSL_CD       ," ).append("\n"); 
		query.append("           T.SKD_VOY_NO   ," ).append("\n"); 
		query.append("           T.SKD_DIR_CD   ," ).append("\n"); 
		query.append("           COUNT(DISTINCT T.CUST_CNT_CD) AS CHL," ).append("\n"); 
		query.append("           T.CTRL_LVL     ," ).append("\n"); 
		query.append("           T.CTRL_HC      ," ).append("\n"); 
		query.append("           T.CTRL_45      ," ).append("\n"); 
		query.append("           T.CTRL_53      ," ).append("\n"); 
		query.append("           T.CTRL_RF      ," ).append("\n"); 
		query.append("           T.CTRL_WT      ," ).append("\n"); 
		query.append("           T.CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM VVD_PORT_SREP_CUST T" ).append("\n"); 
		query.append("     , USMODE US" ).append("\n"); 
		query.append("      WHERE US.USA_BKG_MOD_CD = CASE WHEN NVL(CTRL_USA_SVC_MOD_FLG, 'N') = 'N' THEN 'OTH' ELSE US.USA_BKG_MOD_CD END" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( T.TRD_CD    , T.SUB_TRD_CD  , T.RLANE_CD     , T.DIR_CD     , T.IOC_TS_CD," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.OFC_CD      , T.SUB_OFC_CD   , T.SREP_CD    , T.SREP_NM  ," ).append("\n"); 
		query.append("                             T.CUST_TP_CD, T.CUST_CTRL_CD, T.RLANE_ADJ_QTY, T.CUST_CNT_CD, T.CUST_SEQ ," ).append("\n"); 
		query.append("                             T.DTL_SEQ   , T.SC_NO       , T.RFA_NO       , T.POL_CD     , T.POD_CD   ," ).append("\n"); 
		query.append("                             T.VSL_CD    , T.SKD_VOY_NO  , T.SKD_DIR_CD   , T.COST_YR    , T.COST_WK  ," ).append("\n"); 
		query.append("                             T.CTRL_LVL  , T.CTRL_HC     , T.CTRL_45      , T.CTRL_53    , T.CTRL_RF  ," ).append("\n"); 
		query.append("                             T.CTRL_45   , T.CTRL_53   , T.CTRL_RF   , T.CTRL_WT   , T.CTRL_ACCT " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , US.USA_BKG_MOD_CD), --Office, SalesRep, USmode, Customer, POL, POD" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD  , T.RLANE_CD     , T.DIR_CD     , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.OFC_CD      , T.SUB_OFC_CD   , T.SREP_CD    , T.SREP_NM   ," ).append("\n"); 
		query.append("                             T.CUST_TP_CD, T.CUST_CTRL_CD, T.RLANE_ADJ_QTY, T.CUST_CNT_CD, T.CUST_SEQ  ," ).append("\n"); 
		query.append("                             T.DTL_SEQ   , T.SC_NO       , T.RFA_NO       , T.POL_CD     , T.VSL_CD    ," ).append("\n"); 
		query.append("                             T.SKD_VOY_NO, T.SKD_DIR_CD  , T.COST_YR      , T.COST_WK    , T.CTRL_LVL  ," ).append("\n"); 
		query.append("                             T.CTRL_HC   , T.CTRL_45     , T.CTRL_53      , T.CTRL_RF    , T.CTRL_WT   ," ).append("\n"); 
		query.append("                             T.CTRL_ACCT" ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , US.USA_BKG_MOD_CD), --Office, SalesRep, USmode, Customer, POL" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD  , T.RLANE_CD     , T.DIR_CD     , T.IOC_TS_CD ," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.OFC_CD      , T.SUB_OFC_CD   , T.SREP_CD    , T.SREP_NM   ," ).append("\n"); 
		query.append("                             T.CUST_TP_CD, T.CUST_CTRL_CD, T.RLANE_ADJ_QTY, T.CUST_CNT_CD, T.CUST_SEQ  ," ).append("\n"); 
		query.append("                             T.DTL_SEQ   , T.SC_NO       , T.RFA_NO       , T.VSL_CD     , T.SKD_VOY_NO," ).append("\n"); 
		query.append("                             T.SKD_DIR_CD, T.COST_YR     , T.COST_WK      , T.CTRL_LVL   , T.CTRL_HC   ," ).append("\n"); 
		query.append("                             T.CTRL_45   , T.CTRL_53     , T.CTRL_RF      , T.CTRL_WT    , T.CTRL_ACCT  " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , US.USA_BKG_MOD_CD), --Office, SalesRep, USmode, Customer" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD  , T.RLANE_CD     , T.DIR_CD     , T.IOC_TS_CD," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.OFC_CD      , T.SUB_OFC_CD   , T.SREP_CD    , T.SREP_NM  ," ).append("\n"); 
		query.append("                             T.VSL_CD    , T.SKD_VOY_NO  , T.SKD_DIR_CD   , T.COST_YR    , T.COST_WK  ," ).append("\n"); 
		query.append("                             T.CTRL_LVL  , T.CTRL_HC     , T.CTRL_45      , T.CTRL_53    , T.CTRL_RF  ," ).append("\n"); 
		query.append("                             T.CTRL_WT , T.CTRL_ACCT          " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                             , US.USA_BKG_MOD_CD), --Office, SalesRep, USmode" ).append("\n"); 
		query.append("                           ( T.TRD_CD    , T.SUB_TRD_CD  , T.RLANE_CD     , T.DIR_CD     , T.IOC_TS_CD," ).append("\n"); 
		query.append("                             T.RGN_OFC_CD, T.OFC_CD      , T.SUB_OFC_CD   , T.SREP_CD    , T.SREP_NM  ," ).append("\n"); 
		query.append("                             T.VSL_CD    , T.SKD_VOY_NO  , T.SKD_DIR_CD   , T.COST_YR    , T.COST_WK  ," ).append("\n"); 
		query.append("                             T.CTRL_LVL  , T.CTRL_HC     , T.CTRL_45      , T.CTRL_53    , T.CTRL_RF  ," ).append("\n"); 
		query.append("                             T.CTRL_WT , T.CTRL_ACCT      " ).append("\n"); 
		query.append("                             , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)--Office, SalesRep" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", CTRT_FCAST AS (" ).append("\n"); 
		query.append("    SELECT LVL1           ," ).append("\n"); 
		query.append("           LVL2           ," ).append("\n"); 
		query.append("           TRD_CD         ," ).append("\n"); 
		query.append("           SUB_TRD_CD     ," ).append("\n"); 
		query.append("           RLANE_CD       ," ).append("\n"); 
		query.append("           IOC_TS_CD      ," ).append("\n"); 
		query.append("           RGN_OFC_CD     ," ).append("\n"); 
		query.append("           OFC_CD         ," ).append("\n"); 
		query.append("           SUB_OFC_CD     ," ).append("\n"); 
		query.append("           SREP_CD        ," ).append("\n"); 
		query.append("           CUST_TP_CD     ," ).append("\n"); 
		query.append("           CUST_CTRL_CD   ," ).append("\n"); 
		query.append("           DTL_SEQ        ," ).append("\n"); 
		query.append("           SC_NO          ," ).append("\n"); 
		query.append("           RFA_NO         ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY  ," ).append("\n"); 
		query.append("           CUST_CNT_CD    ," ).append("\n"); 
		query.append("           CUST_SEQ       ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           VSL_CD         ," ).append("\n"); 
		query.append("           SKD_VOY_NO     ," ).append("\n"); 
		query.append("           SKD_DIR_CD     ," ).append("\n"); 
		query.append("           1 CHL          ," ).append("\n"); 
		query.append("           CTRL_LVL       ," ).append("\n"); 
		query.append("           CTRL_HC        ," ).append("\n"); 
		query.append("           CTRL_45        ," ).append("\n"); 
		query.append("           CTRL_53        ," ).append("\n"); 
		query.append("           CTRL_RF        ," ).append("\n"); 
		query.append("           CTRL_WT        ," ).append("\n"); 
		query.append("           CTRL_ACCT      ," ).append("\n"); 
		query.append("           COST_YR        ," ).append("\n"); 
		query.append("           COST_WK        ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_QTY)     AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("           SUM(FCAST_20FT_QTY)    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_QTY)    AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_HC_QTY) AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_45FT_HC_QTY) AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_53FT_QTY)    AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_RF_QTY)      AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_WGT)     AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("           DECODE(MAX(FCAST_RMK), NULL, '', SUBSTR(MAX(SYS_CONNECT_BY_PATH('['||CSREP_NM||']'||FCAST_RMK, '|')), 2)) AS FCAST_RMK" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("              SELECT DISTINCT" ).append("\n"); 
		query.append("                     4 AS LVL1                     ," ).append("\n"); 
		query.append("                     1 AS LVL2                     ," ).append("\n"); 
		query.append("                     F.TRD_CD                      ," ).append("\n"); 
		query.append("                     F.SUB_TRD_CD                  ," ).append("\n"); 
		query.append("                     F.RLANE_CD                    ," ).append("\n"); 
		query.append("                     F.IOC_TS_CD                   ," ).append("\n"); 
		query.append("                     F.SLS_RGN_OFC_CD AS RGN_OFC_CD," ).append("\n"); 
		query.append("                     SR.OFC_CD        AS OFC_CD    ," ).append("\n"); 
		query.append("                     SR.SUB_OFC_CD                 ," ).append("\n"); 
		query.append("                     R.SREP_CD                     ," ).append("\n"); 
		query.append("                     NVL(SC.CUST_TP_CD, 'N')   AS CUST_TP_CD   ," ).append("\n"); 
		query.append("                     NVL(SC.CUST_CTRL_CD, 'C') AS CUST_CTRL_CD ," ).append("\n"); 
		query.append("                     NVL(SC.DTL_SEQ, 1) AS DTL_SEQ ," ).append("\n"); 
		query.append("                     SC.SC_NO                      ," ).append("\n"); 
		query.append("                     SC.RFA_NO                     ," ).append("\n"); 
		query.append("                     NVL(SC.RLANE_ADJ_QTY, 0)  AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("                     NVL(SC.CUST_CNT_CD, NVL((SELECT S.CUST_CNT_CD" ).append("\n"); 
		query.append("                                                FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                               WHERE S.TRD_CD      = F.TRD_CD" ).append("\n"); 
		query.append("                                                 AND S.SUB_TRD_CD  = F.SUB_TRD_CD" ).append("\n"); 
		query.append("                                                 AND S.SREP_CD     = R.SREP_CD" ).append("\n"); 
		query.append("                                                 AND S.CUST_CNT_CD = F.CUST_CNT_CD" ).append("\n"); 
		query.append("                                                 AND S.CUST_SEQ    = F.CUST_SEQ" ).append("\n"); 
		query.append("                                                 AND S.DELT_FLG    ='N'" ).append("\n"); 
		query.append("                                                 AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y'), 'XX')) AS CUST_CNT_CD," ).append("\n"); 
		query.append("                     NVL(SC.CUST_SEQ, NVL((SELECT S.CUST_SEQ" ).append("\n"); 
		query.append("                                             FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                            WHERE S.TRD_CD      = F.TRD_CD" ).append("\n"); 
		query.append("                                              AND S.SUB_TRD_CD  = F.SUB_TRD_CD" ).append("\n"); 
		query.append("                                              AND S.SREP_CD     = R.SREP_CD" ).append("\n"); 
		query.append("                                              AND S.CUST_CNT_CD = F.CUST_CNT_CD" ).append("\n"); 
		query.append("                                              AND S.CUST_SEQ    = F.CUST_SEQ" ).append("\n"); 
		query.append("                                              AND S.DELT_FLG    ='N'" ).append("\n"); 
		query.append("                                              AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y' ), 999999)) AS CUST_SEQ," ).append("\n"); 
		query.append("                     F.CUST_CNT_CD AS BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("                     F.CUST_SEQ    AS BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("                     F.VSL_CD       ," ).append("\n"); 
		query.append("                     F.SKD_VOY_NO   ," ).append("\n"); 
		query.append("                     F.SKD_DIR_CD   ," ).append("\n"); 
		query.append("                     1 AS CHL       ," ).append("\n"); 
		query.append("                     V.CTRL_LVL     ," ).append("\n"); 
		query.append("                     V.CTRL_HC      ," ).append("\n"); 
		query.append("                     V.CTRL_45      ," ).append("\n"); 
		query.append("                     V.CTRL_53      ," ).append("\n"); 
		query.append("                     V.CTRL_RF      ," ).append("\n"); 
		query.append("                     V.CTRL_WT      ," ).append("\n"); 
		query.append("                     V.CTRL_ACCT    ," ).append("\n"); 
		query.append("                     SUBSTR(V.SLS_YRMON, 1, 4) AS COST_YR ," ).append("\n"); 
		query.append("                     V.COST_WK      ," ).append("\n"); 
		query.append("                     F.FCAST_TTL_QTY," ).append("\n"); 
		query.append("                     NVL(F.FCAST_20FT_QTY, 0) AS FCAST_20FT_QTY," ).append("\n"); 
		query.append("                     NVL(F.FCAST_40FT_QTY ,0) AS FCAST_40FT_QTY," ).append("\n"); 
		query.append("                     F.FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("                     F.FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("                     F.FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("                     F.FCAST_RF_QTY     ," ).append("\n"); 
		query.append("                     F.FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("                     F.FCAST_RMK        ," ).append("\n"); 
		query.append("                     (SELECT SREP_NM" ).append("\n"); 
		query.append("                        FROM MDM_SLS_REP" ).append("\n"); 
		query.append("                       WHERE SREP_CD = F.SREP_USR_ID) AS CSREP_NM," ).append("\n"); 
		query.append("                     DENSE_RANK() OVER (PARTITION BY F.TRD_CD, F.SUB_TRD_CD, F.RLANE_CD, F.IOC_TS_CD, F.VSL_CD, F.SKD_VOY_NO, F.SKD_DIR_CD, F.CUST_CNT_CD, F.CUST_SEQ, F.FCAST_SEQ, F.SLS_RGN_OFC_CD ORDER BY F.SREP_USR_ID) AS RNUM" ).append("\n"); 
		query.append("                     , V.CTRL_D2_FLG, V.CTRL_D4_FLG, V.CTRL_RD_FLG, V.CTRL_ECC_FLG, V.CTRL_LOC_FLG , V.CTRL_USA_SVC_MOD_FLG , V.CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                FROM SPC_CTRT_FCAST_CUST F ," ).append("\n"); 
		query.append("                     VVD_PORT_SREP_CUST  SC," ).append("\n"); 
		query.append("                     SPC_SLS_REP_CUST    R ," ).append("\n"); 
		query.append("                     VVDLS               V ," ).append("\n"); 
		query.append("                     PARAMS              P ," ).append("\n"); 
		query.append("                     SREPS               SR" ).append("\n"); 
		query.append("               WHERE F.TRD_CD         = V.TRD_CD" ).append("\n"); 
		query.append("                 AND F.SUB_TRD_CD     = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND F.RLANE_CD       = V.RLANE_CD" ).append("\n"); 
		query.append("                 AND F.VSL_CD         = V.VSL_CD" ).append("\n"); 
		query.append("                 AND F.SKD_VOY_NO     = V.SKD_VOY_NO" ).append("\n"); 
		query.append("                 AND F.SKD_DIR_CD     = V.SKD_DIR_CD" ).append("\n"); 
		query.append("                 AND F.IOC_TS_CD      = P.IOC_TS_CD" ).append("\n"); 
		query.append("                 AND F.SLS_RGN_OFC_CD = P.RGN_OFC_CD" ).append("\n"); 
		query.append("                 AND F.RLANE_CD       = DECODE(P.VVD, NULL, P.RLANE_CD, F.RLANE_CD)" ).append("\n"); 
		query.append("                 AND F.CUST_CNT_CD    = R.CUST_CNT_CD" ).append("\n"); 
		query.append("                 AND F.CUST_SEQ       = R.CUST_SEQ" ).append("\n"); 
		query.append("                 AND F.TRD_CD         = R.TRD_CD" ).append("\n"); 
		query.append("                 AND F.SUB_TRD_CD     = R.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND R.SREP_CD        = NVL(SC.SREP_CD, R.SREP_CD)" ).append("\n"); 
		query.append("                 AND F.TRD_CD         = SC.TRD_CD      (+)" ).append("\n"); 
		query.append("                 AND F.CUST_CNT_CD    = SC.CUST_CNT_CD (+)" ).append("\n"); 
		query.append("                 AND F.CUST_SEQ       = SC.CUST_SEQ    (+)" ).append("\n"); 
		query.append("                 AND F.VSL_CD         = SC.VSL_CD      (+)" ).append("\n"); 
		query.append("                 AND F.SKD_VOY_NO     = SC.SKD_VOY_NO  (+)" ).append("\n"); 
		query.append("                 AND F.SKD_DIR_CD     = SC.SKD_DIR_CD  (+)" ).append("\n"); 
		query.append("                 AND NVL(F.SC_NO,NVL(F.RFA_NO, 'X')) = NVL(SC.SC_NO(+),NVL(SC.RFA_NO(+), 'X'))" ).append("\n"); 
		query.append("                 AND R.DELT_FLG       = 'N'" ).append("\n"); 
		query.append("                 AND R.SREP_CD        = SR.SREP_CD" ).append("\n"); 
		query.append("                 AND R.SREP_CD        LIKE DECODE(P.RGN_OFC_CD, 'SELSC', DECODE(P.SALES_REP_CD, NULL, SC.SREP_CD, P.SALES_REP_CD), P.SALES_REP_CD)||'%'" ).append("\n"); 
		query.append("           )" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS ((LVL1      , LVL2        , TRD_CD       , SUB_TRD_CD , RLANE_CD, IOC_TS_CD, RGN_OFC_CD, OFC_CD , SUB_OFC_CD, SREP_CD   ," ).append("\n"); 
		query.append("                           CUST_TP_CD, CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ, DTL_SEQ  , SC_NO     , RFA_NO , VSL_CD    , SKD_VOY_NO," ).append("\n"); 
		query.append("                           SKD_DIR_CD, COST_YR     , COST_WK      , CTRL_LVL   , CTRL_HC , CTRL_45  , CTRL_53   , CTRL_RF, CTRL_WT   , CTRL_ACCT               " ).append("\n"); 
		query.append("						   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)," ).append("\n"); 
		query.append("                          (LVL1      , LVL2        , TRD_CD       , SUB_TRD_CD , RLANE_CD, IOC_TS_CD, RGN_OFC_CD, OFC_CD , SUB_OFC_CD     , SREP_CD     ," ).append("\n"); 
		query.append("                           CUST_TP_CD, CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ, DTL_SEQ  , SC_NO     , RFA_NO , BKG_CUST_CNT_CD, BKG_CUST_SEQ," ).append("\n"); 
		query.append("                           VSL_CD    , SKD_VOY_NO  , SKD_DIR_CD   , COST_YR    , COST_WK , CTRL_LVL , CTRL_HC   , CTRL_45, CTRL_53        , CTRL_RF     ," ).append("\n"); 
		query.append("                           CTRL_WT   , CTRL_ACCT    " ).append("\n"); 
		query.append("                           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append("  START WITH RNUM = 1" ).append("\n"); 
		query.append("          CONNECT BY PRIOR TRD_CD      = TRD_CD" ).append("\n"); 
		query.append("                 AND PRIOR SUB_TRD_CD  = SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND PRIOR RLANE_CD    = RLANE_CD" ).append("\n"); 
		query.append("                 AND PRIOR IOC_TS_CD   = IOC_TS_CD" ).append("\n"); 
		query.append("                 AND PRIOR RGN_OFC_CD  = RGN_OFC_cD" ).append("\n"); 
		query.append("                 AND PRIOR OFC_cD      = OFC_CD" ).append("\n"); 
		query.append("                 AND PRIOR SUB_OFC_CD  = SUB_OFC_CD" ).append("\n"); 
		query.append("                 AND PRIOR SREP_CD     = SREP_CD" ).append("\n"); 
		query.append("                 AND PRIOR CUST_CNT_CD = CUST_CNT_CD" ).append("\n"); 
		query.append("                 AND PRIOR CUST_SEQ    = CUST_SEQ" ).append("\n"); 
		query.append("                 AND PRIOR DTL_SEQ     = DTL_SEQ" ).append("\n"); 
		query.append("                 AND PRIOR RNUM        = RNUM - 1" ).append("\n"); 
		query.append("    HAVING NOT (     BKG_CUST_CNT_CD IS NOT NULL" ).append("\n"); 
		query.append("                 AND CUST_CNT_CD = NVL(BKG_CUST_CNT_CD, 'X')" ).append("\n"); 
		query.append("                 AND CUST_SEQ    = NVL(BKG_CUST_SEQ, 1))" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVDS AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           SUBSTR(SEASON, 1, 6) AS SEASON," ).append("\n"); 
		query.append("           SUBSTR(SEASON, 8)    AS VER_SEQ," ).append("\n"); 
		query.append("           SLS_YRMON           ," ).append("\n"); 
		query.append("           COST_YRMON          ," ).append("\n"); 
		query.append("           COST_WK             ," ).append("\n"); 
		query.append("           REP_TRD_CD          ," ).append("\n"); 
		query.append("           REP_SUB_TRD_CD      ," ).append("\n"); 
		query.append("           TRD_CD              ," ).append("\n"); 
		query.append("           SUB_TRD_CD          ," ).append("\n"); 
		query.append("           RLANE_CD            ," ).append("\n"); 
		query.append("           VSL_CD              ," ).append("\n"); 
		query.append("           SKD_VOY_NO          ," ).append("\n"); 
		query.append("           SKD_DIR_CD          ," ).append("\n"); 
		query.append("           SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("           SUB_OFC_CD          ," ).append("\n"); 
		query.append("           RGN_OFC_CD          ," ).append("\n"); 
		query.append("           IOC_TS_CD           ," ).append("\n"); 
		query.append("           CTRL_LVL            ," ).append("\n"); 
		query.append("           CTRL_HC             ," ).append("\n"); 
		query.append("           CTRL_45             ," ).append("\n"); 
		query.append("           CTRL_53             ," ).append("\n"); 
		query.append("           CTRL_RF             ," ).append("\n"); 
		query.append("           CTRL_WT             ," ).append("\n"); 
		query.append("           CTRL_ACCT           ," ).append("\n"); 
		query.append("           ETD_DT              ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , BSE_QTR_CD" ).append("\n"); 
		query.append("      FROM VVD_POL" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", FCAST_DATA AS (" ).append("\n"); 
		query.append("    SELECT 4 - GROUPING_ID(SREP_CD) * 2 AS LVL1," ).append("\n"); 
		query.append("           4 - (GROUPING_ID(USA_BKG_MOD_CD) + GROUPING_ID(CUST_CNT_CD) + GROUPING_ID(POL_CD) + GROUPING_ID(POD_CD)) AS LVL2," ).append("\n"); 
		query.append("           TRD_CD          ," ).append("\n"); 
		query.append("           SUB_TRD_CD      ," ).append("\n"); 
		query.append("           RLANE_CD        ," ).append("\n"); 
		query.append("           DIR_CD          ," ).append("\n"); 
		query.append("           IOC_TS_CD       ," ).append("\n"); 
		query.append("           RGN_OFC_CD      ," ).append("\n"); 
		query.append("           OFC_CD          ," ).append("\n"); 
		query.append("           SUB_OFC_CD      ," ).append("\n"); 
		query.append("           SREP_CD         ," ).append("\n"); 
		query.append("           NULL AS SREP_NM ," ).append("\n"); 
		query.append("           CUST_TP_CD      ," ).append("\n"); 
		query.append("           CUST_CTRL_CD    ," ).append("\n"); 
		query.append("           DTL_SEQ         ," ).append("\n"); 
		query.append("           SC_NO           ," ).append("\n"); 
		query.append("           RFA_NO          ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY   ," ).append("\n"); 
		query.append("           CUST_CNT_CD     ," ).append("\n"); 
		query.append("           CUST_SEQ        ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD ," ).append("\n"); 
		query.append("           BKG_CUST_SEQ    ," ).append("\n"); 
		query.append("           POL_CD          ," ).append("\n"); 
		query.append("           POD_CD          ," ).append("\n"); 
		query.append("           VSL_CD          ," ).append("\n"); 
		query.append("           SKD_VOY_NO      ," ).append("\n"); 
		query.append("           SKD_DIR_CD      ," ).append("\n"); 
		query.append("           1 CHL           ," ).append("\n"); 
		query.append("           CTRL_LVL        ," ).append("\n"); 
		query.append("           CTRL_HC         ," ).append("\n"); 
		query.append("           CTRL_45         ," ).append("\n"); 
		query.append("           CTRL_53         ," ).append("\n"); 
		query.append("           CTRL_RF         ," ).append("\n"); 
		query.append("           CTRL_WT         ," ).append("\n"); 
		query.append("           CTRL_ACCT       ," ).append("\n"); 
		query.append("           MAX(COST_YR) AS COST_YR ," ).append("\n"); 
		query.append("           MAX(COST_WK) AS COST_WK ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_QTY)     AS FCAST_TTL_QTY ," ).append("\n"); 
		query.append("           SUM(FCAST_20FT_QTY)    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_QTY)    AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_HC_QTY) AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_45FT_HC_QTY) AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_53FT_QTY)    AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_RF_QTY)      AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_WGT)     AS FCAST_TTL_WGT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("           , SUM(FCAST_20FT_DRY_QTY)     AS FCAST_20FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(FCAST_40FT_DRY_QTY)     AS FCAST_40FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(FCAST_RD_QTY)           AS FCAST_RD_QTY" ).append("\n"); 
		query.append("           , SUM(CFM_20FT_DRY_QTY)       AS CFM_20FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(CFM_40FT_DRY_QTY)       AS CFM_40FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(CFM_RD_QTY)             AS CFM_RD_QTY" ).append("\n"); 
		query.append("           , SUM(USD_BKG_20FT_DRY_QTY)   AS USD_BKG_20FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(USD_BKG_40FT_DRY_QTY)   AS USD_BKG_40FT_DRY_QTY" ).append("\n"); 
		query.append("           , SUM(USD_BKG_RD_QTY)         AS USD_BKG_RD_QTY     " ).append("\n"); 
		query.append("      FROM (    SELECT F.TRD_CD    ," ).append("\n"); 
		query.append("                     F.SUB_TRD_CD," ).append("\n"); 
		query.append("                     F.RLANE_CD  ," ).append("\n"); 
		query.append("                     F.DIR_CD    ," ).append("\n"); 
		query.append("                     F.IOC_TS_CD ," ).append("\n"); 
		query.append("                     F.SLS_RGN_OFC_CD AS RGN_OFC_CD," ).append("\n"); 
		query.append("                     F.FCAST_OFC_CD AS OFC_CD ," ).append("\n"); 
		query.append("                     NVL(SC.SUB_OFC_CD, (SELECT SUB_OFC_CD" ).append("\n"); 
		query.append("                                           FROM SREPS" ).append("\n"); 
		query.append("                                          WHERE SREP_CD = F.SREP_USR_ID))  AS SUB_OFC_CD," ).append("\n"); 
		query.append("                     P.SUB_OFC_CD AS P_SUB_OFC_CD," ).append("\n"); 
		query.append("                     F.SREP_USR_ID  AS SREP_CD ," ).append("\n"); 
		query.append("                     NVL(SC.CUST_TP_CD, 'N') AS CUST_TP_CD," ).append("\n"); 
		query.append("                     NVL(SC.CUST_CTRL_CD, 'C') AS CUST_CTRL_CD," ).append("\n"); 
		query.append("                     NVL(SC.DTL_SEQ, 1) AS DTL_SEQ," ).append("\n"); 
		query.append("                     SC.SC_NO ," ).append("\n"); 
		query.append("                     SC.RFA_NO," ).append("\n"); 
		query.append("                     NVL(SC.RLANE_ADJ_QTY, 0) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("                     NVL(SC.CUST_CNT_CD, NVL((SELECT S.CUST_CNT_CD" ).append("\n"); 
		query.append("                                                FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                               WHERE S.TRD_CD      = F.TRD_CD" ).append("\n"); 
		query.append("                                                 AND S.SUB_TRD_CD  = F.SUB_TRD_CD" ).append("\n"); 
		query.append("                                                 AND S.SREP_CD     = F.SREP_USR_ID" ).append("\n"); 
		query.append("                                                 AND S.CUST_CNT_CD = F.CUST_CNT_CD" ).append("\n"); 
		query.append("                                                 AND S.CUST_SEQ    = F.CUST_SEQ" ).append("\n"); 
		query.append("                                                 AND S.DELT_FLG    ='N'" ).append("\n"); 
		query.append("                                                 AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y'), 'XX')) AS CUST_CNT_CD," ).append("\n"); 
		query.append("                     NVL(SC.CUST_SEQ, NVL((SELECT S.CUST_SEQ" ).append("\n"); 
		query.append("                                             FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                            WHERE S.TRD_CD      = F.TRD_CD" ).append("\n"); 
		query.append("                                              AND S.SUB_TRD_CD  = F.SUB_TRD_CD" ).append("\n"); 
		query.append("                                              AND S.SREP_CD     = F.SREP_USR_ID" ).append("\n"); 
		query.append("                                              AND S.CUST_CNT_CD = F.CUST_CNT_CD" ).append("\n"); 
		query.append("                                              AND S.CUST_SEQ    = F.CUST_SEQ" ).append("\n"); 
		query.append("                                              AND S.DELT_FLG    ='N'" ).append("\n"); 
		query.append("                                              AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y' ), 999999)) AS CUST_SEQ," ).append("\n"); 
		query.append("                     F.CUST_CNT_CD AS BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("                     F.CUST_SEQ    AS BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("                     F.POL_YD_CD   AS POL_CD         ," ).append("\n"); 
		query.append("                     F.POD_YD_CD   AS POD_CD         ," ).append("\n"); 
		query.append("                     F.VSL_CD    ," ).append("\n"); 
		query.append("                     F.SKD_VOY_NO," ).append("\n"); 
		query.append("                     F.SKD_DIR_CD," ).append("\n"); 
		query.append("                     1 AS CHL    ," ).append("\n"); 
		query.append("                     V.CTRL_LVL  ," ).append("\n"); 
		query.append("                     V.CTRL_HC   ," ).append("\n"); 
		query.append("                     V.CTRL_45   ," ).append("\n"); 
		query.append("                     V.CTRL_53   ," ).append("\n"); 
		query.append("                     V.CTRL_RF   ," ).append("\n"); 
		query.append("                     V.CTRL_WT   ," ).append("\n"); 
		query.append("                     V.CTRL_ACCT ," ).append("\n"); 
		query.append("                     SUBSTR(V.SLS_YRMON, 1, 4) AS COST_YR ," ).append("\n"); 
		query.append("                     V.COST_WK      ," ).append("\n"); 
		query.append("                     F.FCAST_TTL_QTY," ).append("\n"); 
		query.append("                     CASE WHEN F.FCAST_20FT_QTY IS NOT NULL AND F.FCAST_40FT_QTY IS NOT NULL THEN F.FCAST_20FT_QTY" ).append("\n"); 
		query.append("                                                                                             ELSE F.FCAST_TTL_QTY" ).append("\n"); 
		query.append("                      END AS FCAST_20FT_QTY," ).append("\n"); 
		query.append("                     NVL(F.FCAST_40FT_QTY, 0) AS FCAST_40FT_QTY ," ).append("\n"); 
		query.append("                     F.FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("                     F.FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("                     F.FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("                     F.FCAST_RF_QTY     ," ).append("\n"); 
		query.append("                     F.FCAST_TTL_WGT" ).append("\n"); 
		query.append("                     , V.CTRL_D2_FLG, V.CTRL_D4_FLG, V.CTRL_RD_FLG, V.CTRL_ECC_FLG, V.CTRL_LOC_FLG , V.CTRL_USA_SVC_MOD_FLG , V.CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                     , NVL(F.FCAST_20FT_DRY_QTY, 0) FCAST_20FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.FCAST_40FT_DRY_QTY, 0) FCAST_40FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.FCAST_RD_QTY, 0) FCAST_RD_QTY" ).append("\n"); 
		query.append("                     , NVL(F.CFM_20FT_DRY_QTY, 0) CFM_20FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.CFM_40FT_DRY_QTY, 0) CFM_40FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.CFM_RD_QTY, 0) CFM_RD_QTY" ).append("\n"); 
		query.append("                     , NVL(F.USD_BKG_20FT_DRY_QTY, 0) USD_BKG_20FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.USD_BKG_40FT_DRY_QTY, 0) USD_BKG_40FT_DRY_QTY" ).append("\n"); 
		query.append("                     , NVL(F.USD_BKG_RD_QTY, 0) USD_BKG_RD_QTY" ).append("\n"); 
		query.append("                       , F.USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                FROM VVD_PORT_SREP_CUST SC," ).append("\n"); 
		query.append("                     SPC_DLY_FCAST_CUST F ," ).append("\n"); 
		query.append("                     VVDS               V ," ).append("\n"); 
		query.append("                     PARAMS             P ," ).append("\n"); 
		query.append("                     MDM_CUSTOMER       M" ).append("\n"); 
		query.append("                WHERE SC.TRD_CD     (+) = F.TRD_CD" ).append("\n"); 
		query.append("                  AND SC.SUB_TRD_CD (+) = F.SUB_TRD_CD" ).append("\n"); 
		query.append("                  AND SC.RLANE_CD   (+) = F.RLANE_CD" ).append("\n"); 
		query.append("                  AND SC.DIR_CD     (+) = F.DIR_CD" ).append("\n"); 
		query.append("                  AND SC.IOC_TS_CD  (+) = F.IOC_TS_CD" ).append("\n"); 
		query.append("                  AND SC.VSL_CD     (+) = F.VSL_CD" ).append("\n"); 
		query.append("                  AND SC.SKD_VOY_NO (+) = F.SKD_VOY_NO" ).append("\n"); 
		query.append("                  AND SC.SKD_DIR_CD (+) = F.SKD_DIR_CD" ).append("\n"); 
		query.append("                  AND SC.POL_CD     (+) = F.POL_YD_CD" ).append("\n"); 
		query.append("                  AND SC.POD_CD     (+) = F.POD_YD_CD" ).append("\n"); 
		query.append("                  AND SC.SREP_CD    (+) = F.SREP_USR_ID" ).append("\n"); 
		query.append("                  AND SC.CUST_CNT_CD(+) = F.CUST_CNT_CD" ).append("\n"); 
		query.append("                  AND SC.CUST_SEQ   (+) = F.CUST_SEQ" ).append("\n"); 
		query.append("                  AND NVL(SC.SC_NO(+),NVL(SC.RFA_NO(+), 'XX')) = NVL(F.SC_NO, NVL(F.RFA_NO, 'XX'))" ).append("\n"); 
		query.append("#if (${salesoffice} == 'SELSC' && ${suboffice_size} > 0)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                  AND (    SC.TRD_CD IS NOT NULL" ).append("\n"); 
		query.append("                        OR F.FCAST_TTL_QTY     > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_20FT_QTY    > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_40FT_QTY    > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_40FT_HC_QTY > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_45FT_HC_QTY > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_53FT_QTY    > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_RF_QTY      > 0" ).append("\n"); 
		query.append("                        OR F.FCAST_TTL_WGT     > 0" ).append("\n"); 
		query.append("	                    OR F.FCAST_TTL_WGT     > 0" ).append("\n"); 
		query.append("                        OR NVL(F.FCAST_20FT_DRY_QTY, 0)> 0" ).append("\n"); 
		query.append("                        OR NVL(F.FCAST_40FT_DRY_QTY, 0)> 0" ).append("\n"); 
		query.append("                        OR NVL(F.FCAST_RD_QTY, 0)      > 0)" ).append("\n"); 
		query.append("                  AND F.TRD_CD         = V.TRD_CD" ).append("\n"); 
		query.append("                  AND F.SUB_TRD_CD     = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                  AND F.RLANE_CD       = V.RLANE_CD" ).append("\n"); 
		query.append("                  AND F.DIR_CD         = V.SKD_DIR_CD" ).append("\n"); 
		query.append("                  AND F.VSL_CD         = V.VSL_CD" ).append("\n"); 
		query.append("                  AND F.SKD_VOY_NO     = V.SKD_VOY_NO" ).append("\n"); 
		query.append("                  AND F.SKD_DIR_CD     = V.SKD_DIR_CD" ).append("\n"); 
		query.append("                  AND F.IOC_TS_CD      = P.IOC_TS_CD" ).append("\n"); 
		query.append("                  AND F.SLS_RGN_OFC_CD = P.RGN_OFC_CD" ).append("\n"); 
		query.append("                  AND F.RLANE_CD       = DECODE(P.VVD, NULL, P.RLANE_CD, F.RLANE_CD)" ).append("\n"); 
		query.append("                  AND F.CUST_CNT_CD    = M.CUST_CNT_CD" ).append("\n"); 
		query.append("                  AND F.CUST_SEQ       = M.CUST_SEQ" ).append("\n"); 
		query.append("#if (${salesoffice} == 'SELSC' && ${suboffice_size} > 0)" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                  AND F.SLS_OFC_CD  LIKE P.SUB_OFC_CD||'%'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                  AND F.SREP_USR_ID    LIKE DECODE(P.RGN_OFC_CD, 'SELSC', DECODE(P.SALES_REP_CD, NULL, SC.SREP_CD, P.SALES_REP_CD), P.SALES_REP_CD)||'%'" ).append("\n"); 
		query.append("           )" ).append("\n"); 
		query.append("  WHERE SUB_OFC_CD LIKE P_SUB_OFC_CD||'%'" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( TRD_CD , SUB_TRD_CD, RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD  , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, CUST_TP_CD, CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ , SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO , POL_CD    , POD_CD      , VSL_CD       , SKD_VOY_NO , SKD_DIR_CD, CTRL_LVL, CTRL_HC   ," ).append("\n"); 
		query.append("                              CTRL_45, CTRL_53   , CTRL_RF     , CTRL_WT      , CTRL_ACCT  " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD)," ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD     , RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD    , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, CUST_TP_CD     , CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ   , SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO , BKG_CUST_CNT_CD, BKG_CUST_SEQ, POL_CD       , VSL_CD     , SKD_VOY_NO, SKD_DIR_CD, CTRL_LVL  ," ).append("\n"); 
		query.append("                              CTRL_HC, CTRL_45        , CTRL_53     , CTRL_RF      , CTRL_WT    , CTRL_ACCT   " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD) ,  " ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD     , RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD  , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, CUST_TP_CD     , CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ , SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO , BKG_CUST_CNT_CD, BKG_CUST_SEQ, VSL_CD       , SKD_VOY_NO , SKD_DIR_CD, CTRL_LVL, CTRL_HC   ," ).append("\n"); 
		query.append("                              CTRL_45, CTRL_53        , CTRL_RF     , CTRL_WT      , CTRL_ACCT   " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD) ,                            " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                            ( TRD_CD  , SUB_TRD_CD     , RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD    , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD , CUST_TP_CD     , CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ   , SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO  , BKG_CUST_CNT_CD, BKG_CUST_SEQ, POL_CD       , POD_CD     , VSL_CD    , SKD_VOY_NO, SKD_DIR_CD," ).append("\n"); 
		query.append("                              CTRL_LVL, CTRL_HC        , CTRL_45     , CTRL_53      , CTRL_RF    , CTRL_WT   , CTRL_ACCT  " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD)," ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD, RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, CUST_TP_CD, CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ, SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO , POL_CD    , VSL_CD      , SKD_VOY_NO   , SKD_DIR_CD , CTRL_LVL  , CTRL_HC, CTRL_45   ," ).append("\n"); 
		query.append("                              CTRL_53, CTRL_RF   , CTRL_WT     , CTRL_ACCT" ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD)," ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD, RLANE_CD    , DIR_CD       , IOC_TS_CD  , RGN_OFC_CD, OFC_CD , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, CUST_TP_CD, CUST_CTRL_CD, RLANE_ADJ_QTY, CUST_CNT_CD, CUST_SEQ  , DTL_SEQ, SC_NO     ," ).append("\n"); 
		query.append("                              RFA_NO , VSL_CD    , SKD_VOY_NO  , SKD_DIR_CD   , CTRL_LVL   , CTRL_HC   , CTRL_45, CTRL_53   ," ).append("\n"); 
		query.append("                              CTRL_RF, CTRL_WT   , CTRL_ACCT" ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD)," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD, RLANE_CD  , DIR_CD    , IOC_TS_CD, RGN_OFC_CD, OFC_CD , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, VSL_CD    , SKD_VOY_NO, SKD_DIR_CD, CTRL_LVL , CTRL_HC   , CTRL_45, CTRL_53   ," ).append("\n"); 
		query.append("                              CTRL_RF, CTRL_WT   , CTRL_ACCT" ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD), " ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD, RLANE_CD  , DIR_CD    , IOC_TS_CD, RGN_OFC_CD, OFC_CD , SUB_OFC_CD," ).append("\n"); 
		query.append("                              SREP_CD, VSL_CD    , SKD_VOY_NO, SKD_DIR_CD, CTRL_LVL , CTRL_HC   , CTRL_45, CTRL_53   ," ).append("\n"); 
		query.append("                              CTRL_RF, CTRL_WT   , CTRL_ACCT" ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG), " ).append("\n"); 
		query.append("                            " ).append("\n"); 
		query.append("                            ( TRD_CD , SUB_TRD_CD, RLANE_CD  , DIR_CD  , IOC_TS_CD, RGN_OFC_CD, POL_CD , POD_CD ," ).append("\n"); 
		query.append("                              VSL_CD , SKD_VOY_NO, SKD_DIR_CD, CTRL_LVL, CTRL_HC  , CTRL_45   , CTRL_53, CTRL_RF," ).append("\n"); 
		query.append("                              CTRL_WT , CTRL_ACCT " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG),  " ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            ( TRD_CD    , SUB_TRD_CD, RLANE_CD, DIR_CD , IOC_TS_CD, RGN_OFC_CD, VSL_CD , SKD_VOY_NO," ).append("\n"); 
		query.append("                              SKD_DIR_CD, CTRL_LVL  , CTRL_HC , CTRL_45, CTRL_53  , CTRL_RF   , CTRL_WT , CTRL_ACCT " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD), " ).append("\n"); 
		query.append("                            --us                              " ).append("\n"); 
		query.append("                            ( TRD_CD    , SUB_TRD_CD, RLANE_CD, DIR_CD , IOC_TS_CD, RGN_OFC_CD, VSL_CD , SKD_VOY_NO," ).append("\n"); 
		query.append("                              SKD_DIR_CD, POL_CD    , CTRL_LVL  , CTRL_HC , CTRL_45, CTRL_53  , CTRL_RF   , CTRL_WT , CTRL_ACCT " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)," ).append("\n"); 
		query.append("                              " ).append("\n"); 
		query.append("                            --POL 추가" ).append("\n"); 
		query.append("                            ( TRD_CD    , SUB_TRD_CD, RLANE_CD, DIR_CD , IOC_TS_CD, RGN_OFC_CD, VSL_CD , SKD_VOY_NO," ).append("\n"); 
		query.append("                              SKD_DIR_CD, CTRL_LVL  , CTRL_HC , CTRL_45, CTRL_53  , CTRL_RF   , CTRL_WT , CTRL_ACCT " ).append("\n"); 
		query.append("                              , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", BKG_ORG AS (" ).append("\n"); 
		query.append("    SELECT TRD_CD              ," ).append("\n"); 
		query.append("           SUB_TRD_CD          ," ).append("\n"); 
		query.append("           RLANE_CD            ," ).append("\n"); 
		query.append("           DIR_CD               ," ).append("\n"); 
		query.append("           IOC_TS_CD            ," ).append("\n"); 
		query.append("           RGN_OFC_CD           ," ).append("\n"); 
		query.append("           OFC_CD," ).append("\n"); 
		query.append("           SUB_OFC_CD ," ).append("\n"); 
		query.append("           SREP_CD    ," ).append("\n"); 
		query.append("           NULL                   AS SREP_NM    ," ).append("\n"); 
		query.append("           CUST_TP_CD ,          " ).append("\n"); 
		query.append("			--SPC_MDL_CUST_CTRL에 CUST_CTRL_CD가 존재하면 먼저 적용. 존재하지 않으면서 AES_RFA_CUST_FLG != 'X'일때 SPC_MDL_CUST_RFA 테이블에 있는  CUST_CTRL_CD적용." ).append("\n"); 
		query.append("           NVL(" ).append("\n"); 
		query.append("               (SELECT DISTINCT CUST_CTRL_CD                    " ).append("\n"); 
		query.append("                  FROM SPC_MDL_CUST_CTRL" ).append("\n"); 
		query.append("                 WHERE COST_YRWK       = B.SEASON" ).append("\n"); 
		query.append("                   AND VER_SEQ         = B.VER_SEQ" ).append("\n"); 
		query.append("                   AND TRD_CD          = B.TRD_CD" ).append("\n"); 
		query.append("                   AND CUST_CNT_CD     = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                   AND CUST_SEQ        = B.CUST_SEQ" ).append("\n"); 
		query.append("                   AND NVL(SC_NO, NVL(RFA_NO, 'X')) = NVL(B.SC_NO,NVL(B.RFA_NO,'X'))" ).append("\n"); 
		query.append("                   AND DELT_FLG        = 'N')" ).append("\n"); 
		query.append("               , CASE WHEN AES_RFA_CUST_FLG != 'X' " ).append("\n"); 
		query.append("                      THEN AES_RFA_CUST_FLG           " ).append("\n"); 
		query.append("                      ELSE 'C'" ).append("\n"); 
		query.append("						END)" ).append("\n"); 
		query.append("                  AS CUST_CTRL_CD," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("           NVL((SELECT DISTINCT DTL_SEQ                    " ).append("\n"); 
		query.append("                  FROM SPC_MDL_CUST_CTRL" ).append("\n"); 
		query.append("                 WHERE COST_YRWK       = B.SEASON" ).append("\n"); 
		query.append("                   AND VER_SEQ         = B.VER_SEQ" ).append("\n"); 
		query.append("                   AND TRD_CD          = B.TRD_CD" ).append("\n"); 
		query.append("                   AND CUST_CNT_CD     = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                   AND CUST_SEQ        = B.CUST_SEQ" ).append("\n"); 
		query.append("                   AND NVL(SC_NO, NVL(RFA_NO, 'X')) = NVL(B.SC_NO, NVL(B.RFA_NO,'X'))" ).append("\n"); 
		query.append("                   AND DELT_FLG        = 'N'), 1) AS DTL_SEQ," ).append("\n"); 
		query.append("                   " ).append("\n"); 
		query.append("           NVL((SELECT SUM(RLANE_ADJ_QTY)" ).append("\n"); 
		query.append("                  FROM SPC_MDL_CUST_REV_LANE" ).append("\n"); 
		query.append("                 WHERE COST_YRWK       = B.SEASON" ).append("\n"); 
		query.append("                   AND VER_SEQ         = B.VER_SEQ" ).append("\n"); 
		query.append("                   AND TRD_CD          = B.TRD_CD" ).append("\n"); 
		query.append("                   AND SUB_TRD_CD      = B.SUB_TRD_CD" ).append("\n"); 
		query.append("                   AND RLANE_CD        = B.RLANE_CD" ).append("\n"); 
		query.append("                   AND SLS_RGN_OFC_CD  = B.RGN_OFC_CD" ).append("\n"); 
		query.append("                   AND CUST_CNT_CD     = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                   AND CUST_SEQ        = B.CUST_SEQ" ).append("\n"); 
		query.append("                   AND NVL(SC_NO, NVL(RFA_NO, 'X')) = NVL(B.SC_NO, NVL(B.RFA_NO,'X'))" ).append("\n"); 
		query.append("                   AND DELT_FLG        = 'N'), 0) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("           CUST_CNT_CD," ).append("\n"); 
		query.append("           CUST_SEQ   ," ).append("\n"); 
		query.append("           SC_NO," ).append("\n"); 
		query.append("           RFA_NO," ).append("\n"); 
		query.append("           POL_CD," ).append("\n"); 
		query.append("           POD_CD," ).append("\n"); 
		query.append("           VSL_CD    ," ).append("\n"); 
		query.append("           SKD_VOY_NO," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           COST_YR," ).append("\n"); 
		query.append("           COST_WK   ," ).append("\n"); 
		query.append("           VAL," ).append("\n"); 
		query.append("           CTRL_LVL," ).append("\n"); 
		query.append("           CTRL_HC ," ).append("\n"); 
		query.append("           CTRL_45 ," ).append("\n"); 
		query.append("           CTRL_53 ," ).append("\n"); 
		query.append("           CTRL_RF ," ).append("\n"); 
		query.append("           CTRL_WT ," ).append("\n"); 
		query.append("           CTRL_ACCT," ).append("\n"); 
		query.append("           CIF," ).append("\n"); 
		query.append("           FOB" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("            SELECT /*+ USE_NL(BV B H C) */VP.TRD_CD," ).append("\n"); 
		query.append("                   VP.SUB_TRD_CD          ," ).append("\n"); 
		query.append("                   VP.RLANE_CD            ," ).append("\n"); 
		query.append("                   VP.SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("                   P.IOC_TS_CD            ," ).append("\n"); 
		query.append("                   P.RGN_OFC_CD           ," ).append("\n"); 
		query.append("                   SPC_SCR_OFC_CONV_FNC(B.OB_SLS_OFC_CD) AS OFC_CD," ).append("\n"); 
		query.append("                   O.SUB_OFC_CD           AS SUB_OFC_CD ," ).append("\n"); 
		query.append("                   B.OB_SREP_CD           AS SREP_CD    ," ).append("\n"); 
		query.append("                   M.RVIS_CNTR_CUST_TP_CD AS CUST_TP_CD ," ).append("\n"); 
		query.append("                   VP.SEASON," ).append("\n"); 
		query.append("                   VP.VER_SEQ," ).append("\n"); 
		query.append("                   B.CTRT_CUST_CNT_CD AS CUST_CNT_CD," ).append("\n"); 
		query.append("                   B.CTRT_CUST_SEQ    AS CUST_SEQ   ," ).append("\n"); 
		query.append("                   SPC_GET_SMP_AMEND_FNC(VP.TRD_CD, VP.SEASON, VP.VER_SEQ, B.SC_NO) AS SC_NO," ).append("\n"); 
		query.append("                   CASE WHEN (VP.TRD_CD IN ('AES', 'IAS')) " ).append("\n"); 
		query.append("                               AND BC.CUST_CNT_CD = 'CN' " ).append("\n"); 
		query.append("                        THEN" ).append("\n"); 
		query.append("                            RFA.RFA_NO" ).append("\n"); 
		query.append("                        ELSE   " ).append("\n"); 
		query.append("                            SPC_GET_SMP_AMEND_FNC(VP.TRD_CD, VP.SEASON, VP.VER_SEQ, DECODE(SUBSTR(B.RFA_NO, 1, 3), 'DUM', '', B.RFA_NO))" ).append("\n"); 
		query.append("                   END AS RFA_NO," ).append("\n"); 
		query.append("                   " ).append("\n"); 
		query.append("                   CASE WHEN (VP.TRD_CD IN ('AES', 'IAS')) " ).append("\n"); 
		query.append("                              AND BC.CUST_CNT_CD = 'CN' " ).append("\n"); 
		query.append("                        THEN " ).append("\n"); 
		query.append("                            NVL(RFA.CUST_CTRL_CD,'X')" ).append("\n"); 
		query.append("                        ELSE 'X' " ).append("\n"); 
		query.append("                   END AS AES_RFA_CUST_FLG, " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                   NVL(BV.POL_YD_CD, BV.POL_CD) AS POL_CD," ).append("\n"); 
		query.append("                   NVL(BV.POD_YD_CD, BV.POD_CD) AS POD_CD," ).append("\n"); 
		query.append("                   BV.VSL_CD    ," ).append("\n"); 
		query.append("                   BV.SKD_VOY_NO," ).append("\n"); 
		query.append("                   BV.SKD_DIR_CD," ).append("\n"); 
		query.append("                   SUBSTR(VP.SLS_YRMON, 1, 4) AS COST_YR," ).append("\n"); 
		query.append("                   VP.COST_WK   ," ).append("\n"); 
		query.append("                   (" ).append("\n"); 
		query.append("                      SELECT    TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), '2', 1, 2) * Q.OP_CNTR_QTY), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), '2', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), '4', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), '5', Q.OP_CNTR_QTY, '9', Q.OP_CNTR_QTY, '8', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')                         || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), '7', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), 'W', Q.OP_CNTR_QTY, 0) + DECODE(SPC_GET_CNTR_SZ_FNC(Q.CNTR_TPSZ_CD), 'X', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(SPC_GET_CNTR_TP_FNC(Q.CNTR_TPSZ_CD), 'R', Q.OP_CNTR_QTY - Q.EQ_SUBST_CGO_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR((BL.ACT_WGT * DECODE(BL.WGT_UT_CD, 'LBS', 0.00045, 0.001)) + SUM(Q.OP_CNTR_QTY * ( SELECT TS.CNTR_TPSZ_TARE_WGT" ).append("\n"); 
		query.append("                                                                                                                             FROM MDM_CNTR_TP_SZ TS" ).append("\n"); 
		query.append("                                                                                                                            WHERE TS.CNTR_TPSZ_CD = Q.CNTR_TPSZ_CD)) * 0.001, 'FM0000000000.000')" ).append("\n"); 
		query.append("                             " ).append("\n"); 
		query.append("							 || TO_CHAR(SUM(DECODE(Q.CNTR_TPSZ_CD, 'D2', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(DECODE(Q.CNTR_TPSZ_CD, 'D4', Q.OP_CNTR_QTY, 0)), 'FM0000000000.000')" ).append("\n"); 
		query.append("                             || TO_CHAR(SUM(CASE WHEN Q.CNTR_TPSZ_CD LIKE 'R%' AND Q.EQ_SUBST_CNTR_TPSZ_CD LIKE 'D%' THEN Q.EQ_SUBST_CGO_QTY ELSE 0 END), 'FM0000000000.000')                         " ).append("\n"); 
		query.append("                        FROM BKG_QUANTITY Q" ).append("\n"); 
		query.append("                       WHERE B.BKG_NO      = Q.BKG_NO" ).append("\n"); 
		query.append("                         AND Q.OP_CNTR_QTY > 0" ).append("\n"); 
		query.append("                   ) AS VAL," ).append("\n"); 
		query.append("                   VP.CTRL_LVL," ).append("\n"); 
		query.append("                   VP.CTRL_HC ," ).append("\n"); 
		query.append("                   VP.CTRL_45 ," ).append("\n"); 
		query.append("                   VP.CTRL_53 ," ).append("\n"); 
		query.append("                   VP.CTRL_RF ," ).append("\n"); 
		query.append("                   VP.CTRL_WT ," ).append("\n"); 
		query.append("                   VP.CTRL_ACCT," ).append("\n"); 
		query.append("                   0 AS CIF," ).append("\n"); 
		query.append("                   0 AS FOB" ).append("\n"); 
		query.append("                   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                   , CASE " ).append("\n"); 
		query.append("                        WHEN VP.CTRL_USA_SVC_MOD_FLG = 'Y' AND (SUBSTR(B.POR_CD, 1, 2) IN ('CA', 'US') OR SUBSTR(B.DEL_CD, 1, 2) IN ('CA', 'US'))" ).append("\n"); 
		query.append("                        THEN SPC_USA_MODE_FNC(B.RCV_TERM_CD, B.DE_TERM_CD, B.POR_CD, B.POL_CD, B.POD_CD,B.DEL_CD)" ).append("\n"); 
		query.append("                     ELSE 'OTH' END USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                    , B.CMDT_CD" ).append("\n"); 
		query.append("                  FROM BKG_BOOKING      B ," ).append("\n"); 
		query.append("                       BKG_VVD          BV," ).append("\n"); 
		query.append("                       VVDS             VP," ).append("\n"); 
		query.append("                       OFFICES          O ," ).append("\n"); 
		query.append("                       PARAMS           P ," ).append("\n"); 
		query.append("                       MDM_DTL_REV_LANE RB," ).append("\n"); 
		query.append("                       BKG_BL_DOC       BL," ).append("\n"); 
		query.append("                       MDM_CUSTOMER     M," ).append("\n"); 
		query.append("                       BKG_CUSTOMER     BC,  " ).append("\n"); 
		query.append("					   SPC_MDL_CUST_RFA RFA " ).append("\n"); 
		query.append("                 WHERE 1=1" ).append("\n"); 
		query.append("                   AND B.BKG_STS_CD       IN ('W', 'F')" ).append("\n"); 
		query.append("                   AND B.BKG_CGO_TP_CD    IN ('F', 'B', 'R')" ).append("\n"); 
		query.append("                   AND B.BKG_NO            = BV.BKG_NO" ).append("\n"); 
		query.append("                   AND B.BKG_NO            = BL.BKG_NO" ).append("\n"); 
		query.append("                   AND P.VSL_SLAN_CD       = BV.SLAN_CD" ).append("\n"); 
		query.append("                   AND RB.RLANE_CD         = VP.RLANE_CD" ).append("\n"); 
		query.append("                   AND RB.VSL_SLAN_DIR_CD  = VP.SKD_DIR_CD" ).append("\n"); 
		query.append("                   AND RB.IOC_CD           = DECODE(P.IOC_TS_CD, 'T', 'I',P.IOC_TS_CD)" ).append("\n"); 
		query.append("                   AND RB.FM_CONTI_CD      = ( SELECT SPC_CONTI_CONV_FNC(MLOC.CONTI_CD, RB.RLANE_CD,RB.VSL_SLAN_DIR_CD)" ).append("\n"); 
		query.append("                                                 FROM MDM_LOCATION MLOC" ).append("\n"); 
		query.append("                                                WHERE MLOC.LOC_CD = BV.POL_CD )" ).append("\n"); 
		query.append("                   AND RB.TO_CONTI_CD      = ( SELECT SPC_CONTI_CONV_FNC(MLOC.CONTI_CD, RB.RLANE_CD,RB.VSL_SLAN_DIR_CD)" ).append("\n"); 
		query.append("                                                 FROM MDM_LOCATION MLOC" ).append("\n"); 
		query.append("                                                WHERE MLOC.LOC_CD = BV.POD_CD )" ).append("\n"); 
		query.append("                   AND RB.TRD_CD           = VP.TRD_CD" ).append("\n"); 
		query.append("                   AND RB.SUB_TRD_CD       = VP.SUB_TRD_CD" ).append("\n"); 
		query.append("                   AND RB.DELT_FLG         = 'N'" ).append("\n"); 
		query.append("                   AND BV.VSL_CD           = VP.VSL_CD" ).append("\n"); 
		query.append("                   AND BV.SKD_VOY_NO       = VP.SKD_VOY_NO" ).append("\n"); 
		query.append("                   AND BV.SKD_DIR_CD       = VP.SKD_DIR_CD" ).append("\n"); 
		query.append("                   AND BV.VSL_PRE_PST_CD  IN ( DECODE(P.IOC_TS_CD, 'T', NULL, 'T')," ).append("\n"); 
		query.append("                                               DECODE(P.IOC_TS_CD, 'T', 'S')," ).append("\n"); 
		query.append("                                               DECODE(P.IOC_TS_CD, 'T', 'U')       )" ).append("\n"); 
		query.append("                   AND SPC_SCR_OFC_CONV_FNC(B.OB_SLS_OFC_CD) = O.OFC_CD" ).append("\n"); 
		query.append("                   AND B.OB_SREP_CD LIKE O.SALES_REP_CD||'%'" ).append("\n"); 
		query.append("                   AND B.CTRT_CUST_CNT_CD  = M.CUST_CNT_CD" ).append("\n"); 
		query.append("                   AND B.CTRT_CUST_SEQ     = M.CUST_SEQ" ).append("\n"); 
		query.append("                   AND B.BKG_NO            = BC.BKG_NO  " ).append("\n"); 
		query.append("                   AND BC.BKG_CUST_TP_CD   = 'S'       " ).append("\n"); 
		query.append("                   AND RFA.CUST_CNT_CD(+)  = B.CTRT_CUST_CNT_CD " ).append("\n"); 
		query.append("                   AND RFA.CUST_SEQ(+)     = B.CTRT_CUST_SEQ " ).append("\n"); 
		query.append("                   AND RFA.POL_CD(+)       = B.POL_CD " ).append("\n"); 
		query.append("                   AND RFA.POD_CD(+)       = B.POD_CD " ).append("\n"); 
		query.append("                   AND RFA.CMDT_CD(+)      = B.CMDT_CD" ).append("\n"); 
		query.append("             ) B" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", BKG_DIV AS (" ).append("\n"); 
		query.append("    SELECT B.TRD_CD    ," ).append("\n"); 
		query.append("           B.SUB_TRD_CD," ).append("\n"); 
		query.append("           B.RLANE_CD  ," ).append("\n"); 
		query.append("           B.DIR_CD    ," ).append("\n"); 
		query.append("           B.IOC_TS_CD ," ).append("\n"); 
		query.append("           B.RGN_OFC_CD," ).append("\n"); 
		query.append("           B.OFC_CD    ," ).append("\n"); 
		query.append("           B.SUB_OFC_CD," ).append("\n"); 
		query.append("           B.SREP_CD   ," ).append("\n"); 
		query.append("           B.SREP_NM   ," ).append("\n"); 
		query.append("           NVL(SC.CUST_TP_CD, B.CUST_TP_CD) AS CUST_TP_CD  ," ).append("\n"); 
		query.append("--BKG을 가지고 있는 S.Rep이 FCST하지 않는 경우, 해당 S.Rep에는 A의 others로 따로 나와야 하므로 BKG 데이터를 가져감" ).append("\n"); 
		query.append("           NVL(SC.CUST_CTRL_CD, B.CUST_CTRL_CD) AS CUST_CTRL_CD," ).append("\n"); 
		query.append("           DECODE(NVL(SC.CUST_CTRL_CD, B.CUST_CTRL_CD), 'C', NVL(SC.DTL_SEQ, 1), NVL(SC.DTL_SEQ, B.DTL_SEQ)) AS DTL_SEQ," ).append("\n"); 
		query.append("           DECODE(NVL(SC.CUST_CTRL_CD, B.CUST_CTRL_CD), 'C', SC.SC_NO, NVL(SC.SC_NO, B.SC_NO))               AS SC_NO  ," ).append("\n"); 
		query.append("           DECODE(NVL(SC.CUST_CTRL_CD, B.CUST_CTRL_CD), 'C', SC.RFA_NO, NVL(SC.RFA_NO, B.RFA_NO))            AS RFA_NO ," ).append("\n"); 
		query.append("           DECODE(NVL(SC.CUST_CTRL_CD, B.CUST_CTRL_CD), 'C', NVL(SC.RLANE_ADJ_QTY, 0), NVL(SC.RLANE_ADJ_QTY, B.RLANE_ADJ_QTY)) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("           NVL(SC.CUST_CNT_CD, NVL((SELECT S.CUST_CNT_CD" ).append("\n"); 
		query.append("                                      FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                     WHERE S.TRD_CD      = B.TRD_CD" ).append("\n"); 
		query.append("                                       AND S.SUB_TRD_CD  = B.SUB_TRD_CD" ).append("\n"); 
		query.append("                                       AND S.SREP_CD     = B.SREP_CD" ).append("\n"); 
		query.append("                                       AND S.CUST_CNT_CD = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                                       AND S.CUST_SEQ    = B.CUST_SEQ" ).append("\n"); 
		query.append("                                       AND S.DELT_FLG    = 'N'" ).append("\n"); 
		query.append("                                       AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y' ), 'XX')) AS CUST_CNT_CD," ).append("\n"); 
		query.append("           NVL(SC.CUST_SEQ, NVL((SELECT S.CUST_SEQ" ).append("\n"); 
		query.append("                                   FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                  WHERE S.TRD_CD      = B.TRD_CD" ).append("\n"); 
		query.append("                                    AND S.SUB_TRD_CD  = B.SUB_TRD_CD" ).append("\n"); 
		query.append("                                    AND S.SREP_CD     = B.SREP_CD" ).append("\n"); 
		query.append("                                    AND S.CUST_CNT_CD = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                                    AND S.CUST_SEQ    = B.CUST_SEQ" ).append("\n"); 
		query.append("                                    AND S.DELT_FLG    = 'N'" ).append("\n"); 
		query.append("                                    AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y' ), 999999)) AS CUST_SEQ," ).append("\n"); 
		query.append("           B.CUST_CNT_CD AS BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           B.CUST_SEQ    AS BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           B.POL_CD    ," ).append("\n"); 
		query.append("           B.POD_CD    ," ).append("\n"); 
		query.append("           B.VSL_CD    ," ).append("\n"); 
		query.append("           B.SKD_VOY_NO," ).append("\n"); 
		query.append("           B.SKD_DIR_CD," ).append("\n"); 
		query.append("           B.COST_YR   ," ).append("\n"); 
		query.append("           B.COST_WK   ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL,  1, 14), 0))) AS BKG_TTL_QTY    ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 15, 14), 0))) AS BKG_20FT_QTY   ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 29, 14), 0))) AS BKG_40FT_QTY   ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 43, 14), 0))) AS BKG_40FT_HC_QTY," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 57, 14), 0))) AS BKG_45FT_HC_QTY," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 71, 14), 0))) AS BKG_53FT_QTY   ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 85, 14), 0))) AS BKG_RF_QTY     ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 99, 14), 0))) AS BKG_TTL_WGT    ," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 113,14), 0))) AS BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 127,14), 0))) AS BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("           (TO_NUMBER(NVL(SUBSTR(B.VAL, 141,14), 0))) AS BKG_RD_QTY     ," ).append("\n"); 
		query.append("           B.CTRL_LVL," ).append("\n"); 
		query.append("           B.CTRL_HC ," ).append("\n"); 
		query.append("           B.CTRL_45 ," ).append("\n"); 
		query.append("           B.CTRL_53 ," ).append("\n"); 
		query.append("           B.CTRL_RF ," ).append("\n"); 
		query.append("           B.CTRL_WT ," ).append("\n"); 
		query.append("           B.CTRL_ACCT," ).append("\n"); 
		query.append("           B.CIF," ).append("\n"); 
		query.append("           B.FOB," ).append("\n"); 
		query.append("           0 AS OTH" ).append("\n"); 
		query.append("           , B.CTRL_D2_FLG, B.CTRL_D4_FLG, B.CTRL_RD_FLG, B.CTRL_ECC_FLG, B.CTRL_LOC_FLG , B.CTRL_USA_SVC_MOD_FLG , B.CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("      FROM BKG_ORG            B ," ).append("\n"); 
		query.append("           VVD_PORT_SREP_CUST SC" ).append("\n"); 
		query.append("     WHERE SC.TRD_CD     (+) = B.TRD_CD" ).append("\n"); 
		query.append("       AND SC.SUB_TRD_CD (+) = B.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND SC.RLANE_CD   (+) = B.RLANE_CD" ).append("\n"); 
		query.append("       AND SC.DIR_CD     (+) = B.DIR_CD" ).append("\n"); 
		query.append("       AND SC.IOC_TS_CD  (+) = B.IOC_TS_CD" ).append("\n"); 
		query.append("       AND SC.VSL_CD     (+) = B.VSL_CD" ).append("\n"); 
		query.append("       AND SC.SKD_VOY_NO (+) = B.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND SC.SKD_DIR_CD (+) = B.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND SC.POL_CD     (+) = B.POL_CD" ).append("\n"); 
		query.append("       AND SC.POD_CD     (+) = B.POD_CD" ).append("\n"); 
		query.append("       AND SC.SREP_CD    (+) = B.SREP_CD" ).append("\n"); 
		query.append("       AND SC.CUST_TP_CD (+) = B.CUST_TP_CD" ).append("\n"); 
		query.append("       AND SC.CUST_CNT_CD(+) = B.CUST_CNT_CD" ).append("\n"); 
		query.append("       AND SC.CUST_SEQ   (+) = B.CUST_SEQ" ).append("\n"); 
		query.append("       AND NVL(SC.SC_NO (+), NVL(SC.RFA_NO (+), 'X')) = NVL(B.SC_NO, NVL(B.RFA_NO, 'X'))" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", BKG_DATA AS (" ).append("\n"); 
		query.append("    SELECT LVL1           ," ).append("\n"); 
		query.append("           LVL2           ," ).append("\n"); 
		query.append("           TRD_CD         ," ).append("\n"); 
		query.append("           SUB_TRD_CD     ," ).append("\n"); 
		query.append("           RLANE_CD       ," ).append("\n"); 
		query.append("           DIR_CD         ," ).append("\n"); 
		query.append("           IOC_TS_CD      ," ).append("\n"); 
		query.append("           RGN_OFC_CD     ," ).append("\n"); 
		query.append("           OFC_CD         ," ).append("\n"); 
		query.append("           SUB_OFC_CD     ," ).append("\n"); 
		query.append("           SREP_CD        ," ).append("\n"); 
		query.append("           SREP_NM        ," ).append("\n"); 
		query.append("           CUST_TP_CD     ," ).append("\n"); 
		query.append("           CUST_CTRL_CD   ," ).append("\n"); 
		query.append("           DTL_SEQ        ," ).append("\n"); 
		query.append("           SC_NO          ," ).append("\n"); 
		query.append("           RFA_NO         ," ).append("\n"); 
		query.append("           SUM(RLANE_ADJ_QTY) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("           CUST_CNT_CD    ," ).append("\n"); 
		query.append("           CUST_SEQ       ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           POL_CD         ," ).append("\n"); 
		query.append("           POD_CD         ," ).append("\n"); 
		query.append("           VSL_CD         ," ).append("\n"); 
		query.append("           SKD_VOY_NO     ," ).append("\n"); 
		query.append("           SKD_DIR_CD     ," ).append("\n"); 
		query.append("           CHL            ," ).append("\n"); 
		query.append("           COST_YR        ," ).append("\n"); 
		query.append("           COST_WK        ," ).append("\n"); 
		query.append("           CTRL_LVL       ," ).append("\n"); 
		query.append("           CTRL_HC        ," ).append("\n"); 
		query.append("           CTRL_45        ," ).append("\n"); 
		query.append("           CTRL_53        ," ).append("\n"); 
		query.append("           CTRL_RF        ," ).append("\n"); 
		query.append("           CTRL_WT        ," ).append("\n"); 
		query.append("           CTRL_ACCT      ," ).append("\n"); 
		query.append("           SUM(BKG_TTL_QTY)     AS BKG_TTL_QTY    ," ).append("\n"); 
		query.append("           SUM(BKG_20FT_QTY)    AS BKG_20FT_QTY   ," ).append("\n"); 
		query.append("           SUM(BKG_40FT_QTY)    AS BKG_40FT_QTY   ," ).append("\n"); 
		query.append("           SUM(BKG_40FT_HC_QTY) AS BKG_40FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(BKG_45FT_HC_QTY) AS BKG_45FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(BKG_53FT_QTY)    AS BKG_53FT_QTY   ," ).append("\n"); 
		query.append("           SUM(BKG_RF_QTY)      AS BKG_RF_QTY     ," ).append("\n"); 
		query.append("           SUM(BKG_TTL_WGT)     AS BKG_TTL_WGT    ," ).append("\n"); 
		query.append("           SUM(BKG_20FT_DRY_QTY)AS BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("           SUM(BKG_40FT_DRY_QTY)AS BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("           SUM(BKG_RD_QTY)      AS BKG_RD_QTY     , " ).append("\n"); 
		query.append("           SUM(CIF) AS CIF," ).append("\n"); 
		query.append("           SUM(FOB) AS FOB," ).append("\n"); 
		query.append("           SUM(OTH) AS OTH" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("           SELECT 4 - GROUPING_ID(F.SREP_CD) * 2 AS LVL1," ).append("\n"); 
		query.append("                   4 - (GROUPING_ID(USA_BKG_MOD_CD) + GROUPING_ID(CUST_CNT_CD) + GROUPING_ID(POL_CD) + GROUPING_ID(POD_CD)) AS LVL2," ).append("\n"); 
		query.append("                   F.TRD_CD         ," ).append("\n"); 
		query.append("                   F.SUB_TRD_CD     ," ).append("\n"); 
		query.append("                   F.RLANE_CD       ," ).append("\n"); 
		query.append("                   F.DIR_CD         ," ).append("\n"); 
		query.append("                   F.IOC_TS_CD      ," ).append("\n"); 
		query.append("                   F.RGN_OFC_CD     ," ).append("\n"); 
		query.append("                   F.OFC_CD         ," ).append("\n"); 
		query.append("                   F.SUB_OFC_CD     ," ).append("\n"); 
		query.append("                   F.SREP_CD        ," ).append("\n"); 
		query.append("                   NULL AS SREP_NM  ," ).append("\n"); 
		query.append("                   F.CUST_TP_CD     ," ).append("\n"); 
		query.append("                   F.CUST_CTRL_CD   ," ).append("\n"); 
		query.append("                   CASE WHEN F.CUST_CNT_CD = 'XX' AND F.BKG_CUST_CNT_CD IS NULL THEN 1 ELSE F.DTL_SEQ END AS DTL_SEQ," ).append("\n"); 
		query.append("                   CASE WHEN F.CUST_CNT_CD = 'XX' AND F.BKG_CUST_CNT_CD IS NULL THEN '' ELSE F.SC_NO  END AS SC_NO," ).append("\n"); 
		query.append("                   CASE WHEN F.CUST_CNT_CD = 'XX' AND F.BKG_CUST_CNT_CD IS NULL THEN '' ELSE F.RFA_NO END AS RFA_NO," ).append("\n"); 
		query.append("                   CASE WHEN F.CUST_CNT_CD = 'XX' AND F.BKG_CUST_CNT_CD IS NULL THEN 0  ELSE F.RLANE_ADJ_QTY END AS RLANE_ADJ_QTY  ," ).append("\n"); 
		query.append("                   F.CUST_CNT_CD    ," ).append("\n"); 
		query.append("                   F.CUST_SEQ       ," ).append("\n"); 
		query.append("                   F.BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("                   F.BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("                   F.POL_CD         ," ).append("\n"); 
		query.append("                   F.POD_CD         ," ).append("\n"); 
		query.append("                   F.VSL_CD         ," ).append("\n"); 
		query.append("                   F.SKD_VOY_NO     ," ).append("\n"); 
		query.append("                   F.SKD_DIR_CD     ," ).append("\n"); 
		query.append("                   0 AS CHL         ," ).append("\n"); 
		query.append("                   MAX(F.COST_YR) AS COST_YR," ).append("\n"); 
		query.append("                   MAX(F.COST_WK) AS COST_WK," ).append("\n"); 
		query.append("                   F.CTRL_LVL," ).append("\n"); 
		query.append("                   F.CTRL_HC ," ).append("\n"); 
		query.append("                   F.CTRL_45 ," ).append("\n"); 
		query.append("                   F.CTRL_53 ," ).append("\n"); 
		query.append("                   F.CTRL_RF ," ).append("\n"); 
		query.append("                   F.CTRL_WT ," ).append("\n"); 
		query.append("                   F.CTRL_ACCT ," ).append("\n"); 
		query.append("                   SUM(F.BKG_TTL_QTY)     AS BKG_TTL_QTY    ," ).append("\n"); 
		query.append("                   SUM(F.BKG_20FT_QTY)    AS BKG_20FT_QTY   ," ).append("\n"); 
		query.append("                   SUM(F.BKG_40FT_QTY)    AS BKG_40FT_QTY   ," ).append("\n"); 
		query.append("                   SUM(F.BKG_40FT_HC_QTY) AS BKG_40FT_HC_QTY," ).append("\n"); 
		query.append("                   SUM(F.BKG_45FT_HC_QTY) AS BKG_45FT_HC_QTY," ).append("\n"); 
		query.append("                   SUM(F.BKG_53FT_QTY)    AS BKG_53FT_QTY   ," ).append("\n"); 
		query.append("                   SUM(F.BKG_RF_QTY)      AS BKG_RF_QTY     ," ).append("\n"); 
		query.append("                   SUM(F.BKG_TTL_WGT)     AS BKG_TTL_WGT    ," ).append("\n"); 
		query.append("                   SUM(F.BKG_20FT_DRY_QTY)AS BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   SUM(F.BKG_40FT_DRY_QTY)AS BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   SUM(F.BKG_RD_QTY)      AS BKG_RD_QTY     ," ).append("\n"); 
		query.append("                   SUM(CIF) AS CIF," ).append("\n"); 
		query.append("                   SUM(FOB) AS FOB," ).append("\n"); 
		query.append("                   SUM(OTH) AS OTH" ).append("\n"); 
		query.append("                   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                   , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("              FROM BKG_DIV F" ).append("\n"); 
		query.append("          GROUP BY GROUPING SETS (( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD       , F.DIR_CD      , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD     , F.SREP_CD     , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD    , F.CUST_SEQ    , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.BKG_CUST_CNT_CD, F.BKG_CUST_SEQ, F.POL_CD    ," ).append("\n"); 
		query.append("                                      F.POD_CD      , F.VSL_CD         , F.SKD_VOY_NO     , F.SKD_DIR_CD  , F.CTRL_LVL  ," ).append("\n"); 
		query.append("                                      F.CTRL_HC     , F.CTRL_45        , F.CTRL_53        , F.CTRL_RF     , F.CTRL_WT   ," ).append("\n"); 
		query.append("                                      F.CTRL_ACCT        " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("									  )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD       , F.DIR_CD      , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD     , F.SREP_CD     , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD    , F.CUST_SEQ    , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.BKG_CUST_CNT_CD, F.BKG_CUST_SEQ, F.POL_CD    ," ).append("\n"); 
		query.append("                                      F.VSL_CD      , F.SKD_VOY_NO     , F.SKD_DIR_CD     , F.CTRL_LVL    , F.CTRL_HC   ," ).append("\n"); 
		query.append("                                      F.CTRL_45     , F.CTRL_53        , F.CTRL_RF        , F.CTRL_WT     , F.CTRL_ACCT             " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("									  ),                                     " ).append("\n"); 
		query.append("                                     ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD       , F.DIR_CD      , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD     , F.SREP_CD     , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD    , F.CUST_SEQ    , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.BKG_CUST_CNT_CD, F.BKG_CUST_SEQ, F.VSL_CD    ," ).append("\n"); 
		query.append("                                      F.SKD_VOY_NO  , F.SKD_DIR_CD     , F.CTRL_LVL       , F.CTRL_HC     , F.CTRL_45   ," ).append("\n"); 
		query.append("                                      F.CTRL_53     , F.CTRL_RF        , F.CTRL_WT        , F.CTRL_ACCT                              " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                      )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD  , F.SREP_CD   , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD , F.CUST_SEQ  , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.POL_CD      , F.POD_CD    , F.VSL_CD    ," ).append("\n"); 
		query.append("                                      F.SKD_VOY_NO  , F.SKD_DIR_CD     , F.CTRL_LVL    , F.CTRL_HC   , F.CTRL_45   ," ).append("\n"); 
		query.append("                                      F.CTRL_53     , F.CTRL_RF        , F.CTRL_WT     , F.CTRL_ACCT                                " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                      )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD  , F.SREP_CD   , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD , F.CUST_SEQ  , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.POL_CD      , F.VSL_CD    , F.SKD_VOY_NO," ).append("\n"); 
		query.append("                                      F.SKD_DIR_CD  , F.CTRL_LVL       , F.CTRL_HC     , F.CTRL_45   , F.CTRL_53   ," ).append("\n"); 
		query.append("                                      F.CTRL_RF     , F.CTRL_WT        , F.CTRL_ACCT                                               " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                      )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD  , F.SREP_CD   , F.CUST_TP_CD," ).append("\n"); 
		query.append("                                      F.CUST_CTRL_CD, F.RLANE_ADJ_QTY  , F.CUST_CNT_CD , F.CUST_SEQ  , F.DTL_SEQ   ," ).append("\n"); 
		query.append("                                      F.SC_NO       , F.RFA_NO         , F.VSL_CD      , F.SKD_VOY_NO, F.SKD_DIR_CD," ).append("\n"); 
		query.append("                                      F.CTRL_LVL    , F.CTRL_HC        , F.CTRL_45     , F.CTRL_53   , F.CTRL_RF   ," ).append("\n"); 
		query.append("                                      F.CTRL_WT     , F.CTRL_ACCT                                                                   " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                      )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD  , F.SREP_CD   , F.VSL_CD    ," ).append("\n"); 
		query.append("                                      F.SKD_VOY_NO  , F.SKD_DIR_CD     , F.CTRL_LVL    , F.CTRL_HC   , F.CTRL_45   ," ).append("\n"); 
		query.append("                                      F.CTRL_53     , F.CTRL_RF        , F.CTRL_WT     , F.CTRL_ACCT    " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                      ),                                 " ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.OFC_CD         , F.SUB_OFC_CD  , F.SREP_CD   , F.VSL_CD    ," ).append("\n"); 
		query.append("                                      F.SKD_VOY_NO  , F.SKD_DIR_CD     , F.CTRL_LVL    , F.CTRL_HC   , F.CTRL_45   ," ).append("\n"); 
		query.append("                                      F.CTRL_53     , F.CTRL_RF        , F.CTRL_WT     , F.CTRL_ACCT " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("                                      )," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.POL_CD         , F.POD_CD      , F.VSL_CD    , F.SKD_VOY_NO," ).append("\n"); 
		query.append("                                      F.SKD_DIR_CD  , F.CTRL_LVL       , F.CTRL_HC     , F.CTRL_45   , F.CTRL_53   ," ).append("\n"); 
		query.append("                                      F.CTRL_RF     , F.CTRL_WT        , F.CTRL_ACCT                                                " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.POL_CD         , F.VSL_CD      , F.SKD_VOY_NO, F.SKD_DIR_CD," ).append("\n"); 
		query.append("                                      F.CTRL_LVL    , F.CTRL_HC        , F.CTRL_45     , F.CTRL_53   , F.CTRL_RF   ," ).append("\n"); 
		query.append("                                      F.CTRL_WT     , F.CTRL_ACCT                                                                   " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)," ).append("\n"); 
		query.append("                                    ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.VSL_CD         , F.SKD_VOY_NO  , F.SKD_DIR_CD, F.CTRL_LVL  ," ).append("\n"); 
		query.append("                                      F.CTRL_HC     , F.CTRL_45        , F.CTRL_53     , F.CTRL_RF   , F.CTRL_WT   ," ).append("\n"); 
		query.append("                                      F.CTRL_ACCT  " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG, USA_BKG_MOD_CD)," ).append("\n"); 
		query.append("                                   ( F.TRD_CD      , F.SUB_TRD_CD     , F.RLANE_CD    , F.DIR_CD    , F.IOC_TS_CD ," ).append("\n"); 
		query.append("                                      F.RGN_OFC_CD  , F.VSL_CD         , F.SKD_VOY_NO  , F.SKD_DIR_CD, F.CTRL_LVL  ," ).append("\n"); 
		query.append("                                      F.CTRL_HC     , F.CTRL_45        , F.CTRL_53     , F.CTRL_RF   , F.CTRL_WT   ," ).append("\n"); 
		query.append("                                      F.CTRL_ACCT                                                                                   " ).append("\n"); 
		query.append("                                      , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG)" ).append("\n"); 
		query.append("                                 )" ).append("\n"); 
		query.append("            HAVING    SREP_CD     IS NULL" ).append("\n"); 
		query.append("                   OR POL_CD      IS NULL" ).append("\n"); 
		query.append("                   OR CUST_CNT_CD IS NOT NULL " ).append("\n"); 
		query.append("          )" ).append("\n"); 
		query.append("  GROUP BY LVL1           ," ).append("\n"); 
		query.append("           LVL2           ," ).append("\n"); 
		query.append("           TRD_CD         ," ).append("\n"); 
		query.append("           SUB_TRD_CD     ," ).append("\n"); 
		query.append("           RLANE_CD       ," ).append("\n"); 
		query.append("           DIR_CD         ," ).append("\n"); 
		query.append("           IOC_TS_CD      ," ).append("\n"); 
		query.append("           RGN_OFC_CD     ," ).append("\n"); 
		query.append("           OFC_CD         ," ).append("\n"); 
		query.append("           SUB_OFC_CD     ," ).append("\n"); 
		query.append("           SREP_CD        ," ).append("\n"); 
		query.append("           SREP_NM        ," ).append("\n"); 
		query.append("           CUST_TP_CD     ," ).append("\n"); 
		query.append("           CUST_CTRL_CD   ," ).append("\n"); 
		query.append("           DTL_SEQ        ," ).append("\n"); 
		query.append("           SC_NO          ," ).append("\n"); 
		query.append("           RFA_NO         ," ).append("\n"); 
		query.append("           CUST_CNT_CD    ," ).append("\n"); 
		query.append("           CUST_SEQ       ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           POL_CD         ," ).append("\n"); 
		query.append("           POD_CD         ," ).append("\n"); 
		query.append("           VSL_CD         ," ).append("\n"); 
		query.append("           SKD_VOY_NO     ," ).append("\n"); 
		query.append("           SKD_DIR_CD     ," ).append("\n"); 
		query.append("           CHL            ," ).append("\n"); 
		query.append("           COST_YR        ," ).append("\n"); 
		query.append("           COST_WK        ," ).append("\n"); 
		query.append("           CTRL_LVL       ," ).append("\n"); 
		query.append("           CTRL_HC        ," ).append("\n"); 
		query.append("           CTRL_45        ," ).append("\n"); 
		query.append("           CTRL_53        ," ).append("\n"); 
		query.append("           CTRL_RF        ," ).append("\n"); 
		query.append("           CTRL_WT        ," ).append("\n"); 
		query.append("           CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("           , USA_BKG_MOD_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", ALL_DATA AS (" ).append("\n"); 
		query.append("    SELECT 'M' AS FLG             ," ).append("\n"); 
		query.append("           LVL1                   ," ).append("\n"); 
		query.append("           DECODE(LVL1, 2, DECODE(SIGN(LVL2), 0, LVL2, LVL2+1), LVL2) AS LVL2," ).append("\n"); 
		query.append("           TRD_CD                 ," ).append("\n"); 
		query.append("           SUB_TRD_CD             ," ).append("\n"); 
		query.append("           RLANE_CD               ," ).append("\n"); 
		query.append("           DIR_CD                 ," ).append("\n"); 
		query.append("           IOC_TS_CD              ," ).append("\n"); 
		query.append("           RGN_OFC_CD             ," ).append("\n"); 
		query.append("           OFC_CD                 ," ).append("\n"); 
		query.append("           SUB_OFC_CD             ," ).append("\n"); 
		query.append("           SREP_CD                ," ).append("\n"); 
		query.append("           SREP_NM                ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD         ," ).append("\n"); 
		query.append("           CUST_TP_CD             ," ).append("\n"); 
		query.append("           CUST_CTRL_CD           ," ).append("\n"); 
		query.append("           DTL_SEQ                ," ).append("\n"); 
		query.append("           SC_NO                  ," ).append("\n"); 
		query.append("           RFA_NO                 ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY          ," ).append("\n"); 
		query.append("           CUST_CNT_CD            ," ).append("\n"); 
		query.append("           CUST_SEQ               ," ).append("\n"); 
		query.append("           NULL AS BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           NULL AS BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           POL_CD                 ," ).append("\n"); 
		query.append("           POD_CD                 ," ).append("\n"); 
		query.append("           VSL_CD                 ," ).append("\n"); 
		query.append("           SKD_VOY_NO             ," ).append("\n"); 
		query.append("           SKD_DIR_CD             ," ).append("\n"); 
		query.append("           COST_YR                ," ).append("\n"); 
		query.append("           COST_WK                ," ).append("\n"); 
		query.append("           CHL                    ," ).append("\n"); 
		query.append("           CTRL_LVL               ," ).append("\n"); 
		query.append("           CTRL_HC                ," ).append("\n"); 
		query.append("           CTRL_45                ," ).append("\n"); 
		query.append("           CTRL_53                ," ).append("\n"); 
		query.append("           CTRL_RF                ," ).append("\n"); 
		query.append("           CTRL_WT                ," ).append("\n"); 
		query.append("           CTRL_ACCT              ," ).append("\n"); 
		query.append("           0 AS FCAST_TTL_QTY     ," ).append("\n"); 
		query.append("           0 AS FCAST_20FT_QTY    ," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_QTY    ," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_HC_QTY ," ).append("\n"); 
		query.append("           0 AS FCAST_45FT_HC_QTY ," ).append("\n"); 
		query.append("           0 AS FCAST_53FT_QTY    ," ).append("\n"); 
		query.append("           0 AS FCAST_RF_QTY      ," ).append("\n"); 
		query.append("           0 AS FCAST_TTL_WGT     ," ).append("\n"); 
		query.append("           0 AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_RD_QTY      , " ).append("\n"); 
		query.append("           0 AS BKG_TTL_QTY       ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_QTY      ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_QTY      ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_HC_QTY   ," ).append("\n"); 
		query.append("           0 AS BKG_45FT_HC_QTY   ," ).append("\n"); 
		query.append("           0 AS BKG_53FT_QTY      ," ).append("\n"); 
		query.append("           0 AS BKG_RF_QTY        ," ).append("\n"); 
		query.append("           0 AS BKG_TTL_WGT       ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_RD_QTY        ,  " ).append("\n"); 
		query.append("           0 AS CIF               ," ).append("\n"); 
		query.append("           0 AS FOB               ," ).append("\n"); 
		query.append("           0 AS OTH               ," ).append("\n"); 
		query.append("           NULL AS FCAST_RMK" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM MASTER_DATA" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT 'F' AS FLG          ," ).append("\n"); 
		query.append("           LVL1                ," ).append("\n"); 
		query.append("           DECODE(LVL1, 2, DECODE(SIGN(LVL2), 0, LVL2, LVL2+1), LVL2) AS LVL2," ).append("\n"); 
		query.append("           TRD_CD              ," ).append("\n"); 
		query.append("           SUB_TRD_CD          ," ).append("\n"); 
		query.append("           RLANE_CD            ," ).append("\n"); 
		query.append("           DIR_CD              ," ).append("\n"); 
		query.append("           IOC_TS_CD           ," ).append("\n"); 
		query.append("           RGN_OFC_CD          ," ).append("\n"); 
		query.append("           OFC_CD              ," ).append("\n"); 
		query.append("           SUB_OFC_CD          ," ).append("\n"); 
		query.append("           SREP_CD             ," ).append("\n"); 
		query.append("           SREP_NM             ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD      ," ).append("\n"); 
		query.append("           CUST_TP_CD          ," ).append("\n"); 
		query.append("           CUST_CTRL_CD        ," ).append("\n"); 
		query.append("           DTL_SEQ             ," ).append("\n"); 
		query.append("           SC_NO               ," ).append("\n"); 
		query.append("           RFA_NO              ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY       ," ).append("\n"); 
		query.append("           CUST_CNT_CD         ," ).append("\n"); 
		query.append("           CUST_SEQ            ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD     ," ).append("\n"); 
		query.append("           BKG_CUST_SEQ        ," ).append("\n"); 
		query.append("           POL_CD              ," ).append("\n"); 
		query.append("           POD_CD              ," ).append("\n"); 
		query.append("           VSL_CD              ," ).append("\n"); 
		query.append("           SKD_VOY_NO          ," ).append("\n"); 
		query.append("           SKD_DIR_CD          ," ).append("\n"); 
		query.append("           COST_YR             ," ).append("\n"); 
		query.append("           COST_WK             ," ).append("\n"); 
		query.append("           CHL                 ," ).append("\n"); 
		query.append("           CTRL_LVL            ," ).append("\n"); 
		query.append("           CTRL_HC             ," ).append("\n"); 
		query.append("           CTRL_45             ," ).append("\n"); 
		query.append("           CTRL_53             ," ).append("\n"); 
		query.append("           CTRL_RF             ," ).append("\n"); 
		query.append("           CTRL_WT             ," ).append("\n"); 
		query.append("           CTRL_ACCT           ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_TTL_QTY)     AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_20FT_QTY)    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_40FT_QTY)    AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_40FT_HC_QTY) AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_45FT_HC_QTY) AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_53FT_QTY)    AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_RF_QTY)      AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_TTL_WGT)     AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_20FT_DRY_QTY)AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_40FT_DRY_QTY)AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FCAST_RD_QTY)      AS FCAST_RD_QTY      ," ).append("\n"); 
		query.append("           0 AS BKG_TTL_QTY    ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_QTY   ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_QTY   ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_HC_QTY," ).append("\n"); 
		query.append("           0 AS BKG_45FT_HC_QTY," ).append("\n"); 
		query.append("           0 AS BKG_53FT_QTY   ," ).append("\n"); 
		query.append("           0 AS BKG_RF_QTY     ," ).append("\n"); 
		query.append("           0 AS BKG_TTL_WGT    ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_RD_QTY        , " ).append("\n"); 
		query.append("           0 AS CIF            ," ).append("\n"); 
		query.append("           0 AS FOB            ," ).append("\n"); 
		query.append("           0 AS OTH            ," ).append("\n"); 
		query.append("           NULL AS FCAST_RMK" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM FCAST_DATA" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT 'B' AS FLG            ," ).append("\n"); 
		query.append("           LVL1                  ," ).append("\n"); 
		query.append("           DECODE(LVL1, 2, DECODE(SIGN(LVL2), 0, LVL2, LVL2+1), LVL2) AS LVL2," ).append("\n"); 
		query.append("           TRD_CD                ," ).append("\n"); 
		query.append("           SUB_TRD_CD            ," ).append("\n"); 
		query.append("           RLANE_CD              ," ).append("\n"); 
		query.append("           DIR_CD                ," ).append("\n"); 
		query.append("           IOC_TS_CD             ," ).append("\n"); 
		query.append("           RGN_OFC_CD            ," ).append("\n"); 
		query.append("           OFC_CD                ," ).append("\n"); 
		query.append("           SUB_OFC_CD            ," ).append("\n"); 
		query.append("           SREP_CD               ," ).append("\n"); 
		query.append("           SREP_NM               ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD        ," ).append("\n"); 
		query.append("           CUST_TP_CD            ," ).append("\n"); 
		query.append("           CUST_CTRL_CD          ," ).append("\n"); 
		query.append("           DTL_SEQ               ," ).append("\n"); 
		query.append("           SC_NO                 ," ).append("\n"); 
		query.append("           RFA_NO                ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY         ," ).append("\n"); 
		query.append("           CUST_CNT_CD           ," ).append("\n"); 
		query.append("           CUST_SEQ              ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD       ," ).append("\n"); 
		query.append("           BKG_CUST_SEQ          ," ).append("\n"); 
		query.append("           POL_CD                ," ).append("\n"); 
		query.append("           POD_CD                ," ).append("\n"); 
		query.append("           VSL_CD                ," ).append("\n"); 
		query.append("           SKD_VOY_NO            ," ).append("\n"); 
		query.append("           SKD_DIR_CD            ," ).append("\n"); 
		query.append("           COST_YR               ," ).append("\n"); 
		query.append("           COST_WK               ," ).append("\n"); 
		query.append("           CHL                   ," ).append("\n"); 
		query.append("           CTRL_LVL              ," ).append("\n"); 
		query.append("           CTRL_HC               ," ).append("\n"); 
		query.append("           CTRL_45               ," ).append("\n"); 
		query.append("           CTRL_53               ," ).append("\n"); 
		query.append("           CTRL_RF               ," ).append("\n"); 
		query.append("           CTRL_WT               ," ).append("\n"); 
		query.append("           CTRL_ACCT             ," ).append("\n"); 
		query.append("           0 AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("           0 AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           0 AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           0 AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("           0 AS FCAST_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS FCAST_RD_QTY        , " ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_TTL_QTY)     AS BKG_TTL_QTY    ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_20FT_QTY)    AS BKG_20FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_40FT_QTY)    AS BKG_40FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_40FT_HC_QTY) AS BKG_40FT_HC_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_45FT_HC_QTY) AS BKG_45FT_HC_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_53FT_QTY)    AS BKG_53FT_QTY   ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_RF_QTY)      AS BKG_RF_QTY     ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_TTL_WGT)     AS BKG_TTL_WGT    ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_20FT_DRY_QTY)AS BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_40FT_DRY_QTY)AS BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, BKG_RD_QTY)      AS BKG_RD_QTY      ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, CIF)             AS CIF            ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, FOB)             AS FOB            ," ).append("\n"); 
		query.append("           DECODE(LVL1||LVL2, '41', 0, OTH)             AS OTH            ," ).append("\n"); 
		query.append("           NULL AS FCAST_RMK" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM BKG_DATA" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT 'C' AS FLG            ," ).append("\n"); 
		query.append("           LVL1                  ," ).append("\n"); 
		query.append("           LVL2                  ," ).append("\n"); 
		query.append("           TRD_CD                ," ).append("\n"); 
		query.append("           SUB_TRD_CD            ," ).append("\n"); 
		query.append("           RLANE_CD              ," ).append("\n"); 
		query.append("           SKD_DIR_CD AS DIR_CD  ," ).append("\n"); 
		query.append("           IOC_TS_CD             ," ).append("\n"); 
		query.append("           RGN_OFC_CD            ," ).append("\n"); 
		query.append("           OFC_CD                ," ).append("\n"); 
		query.append("           SUB_OFC_CD            ," ).append("\n"); 
		query.append("           SREP_CD               ," ).append("\n"); 
		query.append("           NULL SREP_NM          ," ).append("\n"); 
		query.append("           NULL USA_BKG_MOD_CD   ," ).append("\n"); 
		query.append("           CUST_TP_CD            ," ).append("\n"); 
		query.append("           CUST_CTRL_CD          ," ).append("\n"); 
		query.append("           DTL_SEQ               ," ).append("\n"); 
		query.append("           SC_NO                 ," ).append("\n"); 
		query.append("           RFA_NO                ," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY         ," ).append("\n"); 
		query.append("           CUST_CNT_CD           ," ).append("\n"); 
		query.append("           CUST_SEQ              ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD       ," ).append("\n"); 
		query.append("           BKG_CUST_SEQ          ," ).append("\n"); 
		query.append("           NULL POL_CD           ," ).append("\n"); 
		query.append("           NULL POD_CD           ," ).append("\n"); 
		query.append("           VSL_CD                ," ).append("\n"); 
		query.append("           SKD_VOY_NO            ," ).append("\n"); 
		query.append("           SKD_DIR_CD            ," ).append("\n"); 
		query.append("           COST_YR               ," ).append("\n"); 
		query.append("           COST_WK               ," ).append("\n"); 
		query.append("           CHL                   ," ).append("\n"); 
		query.append("           CTRL_LVL              ," ).append("\n"); 
		query.append("           CTRL_HC               ," ).append("\n"); 
		query.append("           CTRL_45               ," ).append("\n"); 
		query.append("           CTRL_53               ," ).append("\n"); 
		query.append("           CTRL_RF               ," ).append("\n"); 
		query.append("           CTRL_WT               ," ).append("\n"); 
		query.append("           CTRL_ACCT             ," ).append("\n"); 
		query.append("           FCAST_TTL_QTY         ," ).append("\n"); 
		query.append("           FCAST_20FT_QTY        ," ).append("\n"); 
		query.append("           FCAST_40FT_QTY        ," ).append("\n"); 
		query.append("           FCAST_40FT_HC_QTY     ," ).append("\n"); 
		query.append("           FCAST_45FT_HC_QTY     ," ).append("\n"); 
		query.append("           FCAST_53FT_QTY        ," ).append("\n"); 
		query.append("           FCAST_RF_QTY          ," ).append("\n"); 
		query.append("           FCAST_TTL_WGT         ," ).append("\n"); 
		query.append("           0 AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("           0 AS FCAST_RD_QTY      , " ).append("\n"); 
		query.append("           0 AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("           0 AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("           0 AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("           0 AS BKG_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("           0 AS BKG_RD_QTY        , " ).append("\n"); 
		query.append("           0 AS CIF              ," ).append("\n"); 
		query.append("           0 AS FOB              ," ).append("\n"); 
		query.append("           0 AS OTH              ," ).append("\n"); 
		query.append("           FCAST_RMK" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM CTRT_FCAST" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", GROUP_TOT AS (" ).append("\n"); 
		query.append("    SELECT 'GT' AS FLG," ).append("\n"); 
		query.append("           3 LVL1," ).append("\n"); 
		query.append("           3 - (GROUPING_ID(POL_CD) + GROUPING_ID(POD_CD)) AS LVL2," ).append("\n"); 
		query.append("           TRD_CD      ," ).append("\n"); 
		query.append("           SUB_TRD_CD  ," ).append("\n"); 
		query.append("           RLANE_CD    ," ).append("\n"); 
		query.append("           DIR_CD      ," ).append("\n"); 
		query.append("           IOC_TS_CD   ," ).append("\n"); 
		query.append("           RGN_OFC_CD  ," ).append("\n"); 
		query.append("           CUST_CTRL_CD," ).append("\n"); 
		query.append("           POL_CD      ," ).append("\n"); 
		query.append("           POD_CD      ," ).append("\n"); 
		query.append("           NVL(VSL_CD    , 'TTL')  AS VSL_CD           ," ).append("\n"); 
		query.append("           NVL(SKD_VOY_NO, ' '  )  AS SKD_VOY_NO       ," ).append("\n"); 
		query.append("           NVL(SKD_DIR_CD, ' '  )  AS SKD_DIR_CD       ," ).append("\n"); 
		query.append("           NVL(COST_YR   , 'TTL')  AS COST_YR          ," ).append("\n"); 
		query.append("           NVL(COST_WK   , ' '  )  AS COST_WK          ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_QTY)      AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("           SUM(FCAST_20FT_QTY)     AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_QTY)     AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_HC_QTY)  AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_45FT_HC_QTY)  AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_53FT_QTY)     AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           SUM(FCAST_RF_QTY)       AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           SUM(FCAST_TTL_WGT)      AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("           SUM(FCAST_20FT_DRY_QTY) AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_40FT_DRY_QTY) AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("           SUM(FCAST_RD_QTY)       AS FCAST_RD_QTY    ," ).append("\n"); 
		query.append("           SUM(BKG_TTL_QTY)        AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("           SUM(BKG_20FT_QTY)       AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("           SUM(BKG_40FT_QTY)       AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("           SUM(BKG_40FT_HC_QTY)    AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("           SUM(BKG_45FT_HC_QTY)    AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("           SUM(BKG_53FT_QTY)       AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("           SUM(BKG_RF_QTY)         AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("          SUM(BKG_TTL_WGT)        AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("           SUM(BKG_20FT_DRY_QTY)   AS BKG_20FT_DRY_QTY ," ).append("\n"); 
		query.append("           SUM(BKG_40FT_DRY_QTY)   AS BKG_40FT_DRY_QTY ," ).append("\n"); 
		query.append("           SUM(BKG_RD_QTY)         AS BKG_RD_QTY" ).append("\n"); 
		query.append("      FROM ALL_DATA" ).append("\n"); 
		query.append("     WHERE LVL1 = 4" ).append("\n"); 
		query.append("       AND LVL2 = 4 " ).append("\n"); 
		query.append("       AND CTRL_ACCT = 'Y'" ).append("\n"); 
		query.append("       AND BKG_CUST_CNT_CD IS NULL" ).append("\n"); 
		query.append("       AND BKG_CUST_SEQ    IS NULL" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD)," ).append("\n"); 
		query.append("                           ( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD, POL_CD)," ).append("\n"); 
		query.append("                           ( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD, POL_CD, POD_CD)," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                           ( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, COST_YR, COST_WK )," ).append("\n"); 
		query.append("                           ( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD, POL_CD, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, COST_YR, COST_WK )," ).append("\n"); 
		query.append("                           ( TRD_CD, SUB_TRD_CD, RLANE_CD, DIR_CD, IOC_TS_CD, RGN_OFC_CD, CUST_CTRL_CD, POL_CD, POD_CD, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, COST_YR, COST_WK )" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append("  HAVING POL_CD IS NULL AND POD_CD IS NULL " ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", ALL_DATA_VVD AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           VSL_CD    ," ).append("\n"); 
		query.append("           SKD_VOY_NO," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           COST_YR   ," ).append("\n"); 
		query.append("           COST_WK" ).append("\n"); 
		query.append("      FROM ALL_DATA A" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", LANE_VVDS AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           V.TRD_CD    ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD," ).append("\n"); 
		query.append("           V.RLANE_CD  ," ).append("\n"); 
		query.append("           V.SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("           D.IOC_TS_CD ," ).append("\n"); 
		query.append("           V.VSL_CD    ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO," ).append("\n"); 
		query.append("           V.SKD_DIR_CD," ).append("\n"); 
		query.append("           SUBSTR(V.SLS_YRMON, 1, 4) AS COST_YR," ).append("\n"); 
		query.append("           V.COST_WK   ," ).append("\n"); 
		query.append("           V.ETD_DT    ," ).append("\n"); 
		query.append("           V.HH_FLG" ).append("\n"); 
		query.append("      FROM ALL_DATA_VVD D," ).append("\n"); 
		query.append("           VVDS         V" ).append("\n"); 
		query.append("     WHERE D.TRD_CD     = V.TRD_CD" ).append("\n"); 
		query.append("       AND D.SUB_TRD_CD = V.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND D.RLANE_CD   = V.RLANE_CD" ).append("\n"); 
		query.append("       AND D.DIR_CD     = V.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND D.VSL_CD     = V.VSL_CD" ).append("\n"); 
		query.append("       AND D.SKD_VOY_NO = V.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND D.SKD_DIR_CD = V.SKD_DIR_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", LANE_WK AS (" ).append("\n"); 
		query.append("    SELECT TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           COST_YR   ," ).append("\n"); 
		query.append("           COST_WK   ," ).append("\n"); 
		query.append("           COUNT(1) AS VVD_CNT" ).append("\n"); 
		query.append("      FROM LANE_VVDS" ).append("\n"); 
		query.append("  GROUP BY TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           COST_YR   ," ).append("\n"); 
		query.append("           COST_WK" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", LANES AS (" ).append("\n"); 
		query.append("    SELECT TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD" ).append("\n"); 
		query.append("      FROM LANE_WK" ).append("\n"); 
		query.append("  GROUP BY TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", WEEKS AS (" ).append("\n"); 
		query.append("    SELECT COST_YR," ).append("\n"); 
		query.append("           COST_WK," ).append("\n"); 
		query.append("           LVL IDX," ).append("\n"); 
		query.append("           VVD_CNT," ).append("\n"); 
		query.append("           ROWNUM AS RNUM" ).append("\n"); 
		query.append("      FROM (SELECT COST_YR," ).append("\n"); 
		query.append("                     COST_WK," ).append("\n"); 
		query.append("                     MAX(VVD_CNT) AS VVD_CNT" ).append("\n"); 
		query.append("                FROM LANE_WK" ).append("\n"); 
		query.append("            GROUP BY COST_YR," ).append("\n"); 
		query.append("                     COST_WK" ).append("\n"); 
		query.append("            ORDER BY COST_YR," ).append("\n"); 
		query.append("                     COST_WK)," ).append("\n"); 
		query.append("           (SELECT LEVEL LVL FROM DUAL CONNECT BY LEVEL <= 10 ) L" ).append("\n"); 
		query.append("     WHERE VVD_CNT >= LVL" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", LANE_WEEK AS (" ).append("\n"); 
		query.append("    SELECT L.TRD_CD," ).append("\n"); 
		query.append("           L.SUB_TRD_CD," ).append("\n"); 
		query.append("           L.RLANE_CD  ," ).append("\n"); 
		query.append("           L.DIR_CD    ," ).append("\n"); 
		query.append("           L.IOC_TS_CD ," ).append("\n"); 
		query.append("           W.COST_YR   ," ).append("\n"); 
		query.append("           W.COST_WK   ," ).append("\n"); 
		query.append("           W.IDX       ," ).append("\n"); 
		query.append("           W.RNUM" ).append("\n"); 
		query.append("      FROM WEEKS W," ).append("\n"); 
		query.append("           LANES L" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_RANKS AS (" ).append("\n"); 
		query.append("    SELECT V.TRD_CD    ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD," ).append("\n"); 
		query.append("           V.RLANE_CD  ," ).append("\n"); 
		query.append("           V.SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("           V.IOC_TS_CD ," ).append("\n"); 
		query.append("           V.VSL_CD    ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO," ).append("\n"); 
		query.append("           V.SKD_DIR_CD," ).append("\n"); 
		query.append("           V.COST_YR   ," ).append("\n"); 
		query.append("           V.COST_WK   ," ).append("\n"); 
		query.append("           V.HH_FLG    ," ).append("\n"); 
		query.append("           ROW_NUMBER() OVER (PARTITION BY V.TRD_CD, V.SUB_TRD_CD, V.RLANE_CD, V.SKD_DIR_CD, V.COST_YR, V.COST_WK" ).append("\n"); 
		query.append("                                  ORDER BY V.ETD_DT) AS RNK" ).append("\n"); 
		query.append("      FROM ALL_DATA_VVD D," ).append("\n"); 
		query.append("           LANE_VVDS    V" ).append("\n"); 
		query.append("     WHERE D.TRD_CD     = V.TRD_CD" ).append("\n"); 
		query.append("       AND D.SUB_TRD_CD = V.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND D.RLANE_CD   = V.RLANE_CD" ).append("\n"); 
		query.append("       AND D.DIR_CD     = V.DIR_CD" ).append("\n"); 
		query.append("       AND D.IOC_TS_CD  = V.IOC_TS_CD" ).append("\n"); 
		query.append("       AND D.VSL_CD     = V.VSL_CD" ).append("\n"); 
		query.append("       AND D.SKD_VOY_NO = V.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND D.SKD_DIR_CD = V.SKD_DIR_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_SEQ AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           VR.TRD_CD    ," ).append("\n"); 
		query.append("           VR.SUB_TRD_CD," ).append("\n"); 
		query.append("           VR.RLANE_CD  ," ).append("\n"); 
		query.append("           VR.DIR_CD    ," ).append("\n"); 
		query.append("           VR.IOC_TS_CD ," ).append("\n"); 
		query.append("           VR.VSL_CD    ," ).append("\n"); 
		query.append("           VR.SKD_VOY_NO," ).append("\n"); 
		query.append("           VR.SKD_DIR_CD," ).append("\n"); 
		query.append("           VR.COST_YR   ," ).append("\n"); 
		query.append("           VR.COST_WK   ," ).append("\n"); 
		query.append("           LW.RNUM      ," ).append("\n"); 
		query.append("           VR.HH_FLG" ).append("\n"); 
		query.append("      FROM LANE_WEEK LW," ).append("\n"); 
		query.append("           VVD_RANKS VR" ).append("\n"); 
		query.append("     WHERE VR.TRD_CD     = LW.TRD_CD" ).append("\n"); 
		query.append("       AND VR.SUB_TRD_CD = LW.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND VR.RLANE_CD   = LW.RLANE_CD" ).append("\n"); 
		query.append("       AND VR.DIR_CD     = LW.DIR_CD" ).append("\n"); 
		query.append("       AND VR.IOC_TS_CD  = LW.IOC_TS_CD" ).append("\n"); 
		query.append("       AND VR.COST_YR    = LW.COST_YR" ).append("\n"); 
		query.append("       AND VR.COST_WK    = LW.COST_WK" ).append("\n"); 
		query.append("       AND VR.RNK        = LW.IDX" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", VVD_POS AS (" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           VSL_CD    ," ).append("\n"); 
		query.append("           SKD_VOY_NO," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           COST_YR   ," ).append("\n"); 
		query.append("           COST_WK   ," ).append("\n"); 
		query.append("           RNUM      ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("      FROM VVD_SEQ" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT DISTINCT" ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           'TTL' AS VSL_CD    ," ).append("\n"); 
		query.append("           ' '   AS SKD_VOY_NO," ).append("\n"); 
		query.append("           ' '   AS SKD_DIR_CD," ).append("\n"); 
		query.append("           'TTL' AS COST_YR   ," ).append("\n"); 
		query.append("           NULL  AS COST_WK   ," ).append("\n"); 
		query.append("           (SELECT MAX(RNUM) + 1 FROM VVD_SEQ) AS RNUM  ," ).append("\n"); 
		query.append("           HH_FLG" ).append("\n"); 
		query.append("      FROM VVD_SEQ" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", POL_SEQ AS (" ).append("\n"); 
		query.append("    SELECT TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           SKD_DIR_CD     AS DIR_CD ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POL_CD    ," ).append("\n"); 
		query.append("           MIN(CD_DP_SEQ) AS POL_SEQ" ).append("\n"); 
		query.append("      FROM VVD_POL" ).append("\n"); 
		query.append("  GROUP BY TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POL_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", POL_POD_SEQ AS (" ).append("\n"); 
		query.append("    SELECT TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           SKD_DIR_CD     AS DIR_CD ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POL_CD    ," ).append("\n"); 
		query.append("           MIN(CD_DP_SEQ) AS POL_SEQ," ).append("\n"); 
		query.append("           POD_CD                   ," ).append("\n"); 
		query.append("           MIN(POD_SEQ)   AS POD_SEQ" ).append("\n"); 
		query.append("      FROM VVD_POL_POD" ).append("\n"); 
		query.append("  GROUP BY TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           SKD_DIR_CD," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           POL_CD    ," ).append("\n"); 
		query.append("           POD_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", QTA_DATA AS (" ).append("\n"); 
		query.append("    SELECT V.TRD_CD    ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD," ).append("\n"); 
		query.append("           V.RLANE_CD  ," ).append("\n"); 
		query.append("           V.DIR_CD    ," ).append("\n"); 
		query.append("           P.IOC_TS_CD ," ).append("\n"); 
		query.append("           MQ.RGN_OFC_CD        AS RGN_OFC_CD," ).append("\n"); 
		query.append("           NVL(V.VSL_CD, 'TTL') AS VSL_CD    ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO," ).append("\n"); 
		query.append("           V.SKD_DIR_CD," ).append("\n"); 
		query.append("           NVL(SUBSTR(V.SLS_YRMON, 1, 4), 'TTL') AS COST_YR," ).append("\n"); 
		query.append("           V.COST_WK ," ).append("\n"); 
		query.append("           V.CTRL_LVL," ).append("\n"); 
		query.append("           V.CTRL_HC ," ).append("\n"); 
		query.append("           V.CTRL_45 ," ).append("\n"); 
		query.append("           V.CTRL_53 ," ).append("\n"); 
		query.append("           V.CTRL_RF ," ).append("\n"); 
		query.append("           V.CTRL_WT ," ).append("\n"); 
		query.append("           V.CTRL_ACCT," ).append("\n"); 
		query.append("           SUM(MQ.LOD_QTY) AS LOD_QTY" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM SQM_QTA_RLSE_VER MQR," ).append("\n"); 
		query.append("           SQM_CFM_QTA      MQ ," ).append("\n"); 
		query.append("           VVDS             V  ," ).append("\n"); 
		query.append("           PARAMS           P" ).append("\n"); 
		query.append("     WHERE 1=1 " ).append("\n"); 
		query.append("       AND MQR.BSE_YR          = SUBSTR(V.COST_YRMON, 1, 4)" ).append("\n"); 
		query.append("       AND MQR.BSE_QTR_CD      = V.BSE_QTR_CD" ).append("\n"); 
		query.append("       AND MQR.SQM_VER_STS_CD  = 'R'" ).append("\n"); 
		query.append("       AND MQR.BSE_TP_CD       = 'Q' " ).append("\n"); 
		query.append("       AND MQ.QTA_RLSE_VER_NO  = MQR.QTA_RLSE_VER_NO" ).append("\n"); 
		query.append("       AND MQ.BSE_TP_CD        = MQR.BSE_TP_CD " ).append("\n"); 
		query.append("       AND MQ.BSE_YR           = MQR.BSE_YR" ).append("\n"); 
		query.append("       AND MQ.BSE_QTR_CD       = MQR.BSE_QTR_CD" ).append("\n"); 
		query.append("       AND MQ.QTA_TGT_CD       = 'D'  " ).append("\n"); 
		query.append("       AND MQ.OFC_VW_CD        = 'L'  " ).append("\n"); 
		query.append("       AND MQ.TRD_CD           = V.TRD_CD" ).append("\n"); 
		query.append("       AND MQ.RLANE_CD         = V.RLANE_CD" ).append("\n"); 
		query.append("       AND MQ.DIR_CD           = V.DIR_CD" ).append("\n"); 
		query.append("       AND MQ.VSL_CD           = V.VSL_CD" ).append("\n"); 
		query.append("       AND MQ.SKD_VOY_NO       = V.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND MQ.SKD_DIR_CD       = V.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND SPC_SCR_OFC_CONV_FNC(MQ.RGN_OFC_CD,'EsmSpc0102') = P.RGN_OFC_CD" ).append("\n"); 
		query.append("       AND MQ.RLANE_CD         = NVL(P.RLANE_CD, MQ.RLANE_CD)" ).append("\n"); 
		query.append("  GROUP BY V.TRD_CD     ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD ," ).append("\n"); 
		query.append("           V.RLANE_CD   ," ).append("\n"); 
		query.append("           V.DIR_CD     ," ).append("\n"); 
		query.append("           P.IOC_TS_CD  ," ).append("\n"); 
		query.append("           MQ.RGN_OFC_CD," ).append("\n"); 
		query.append("           SUBSTR(V.SLS_YRMON, 1, 4)," ).append("\n"); 
		query.append("           V.COST_WK    ," ).append("\n"); 
		query.append("           V.VSL_CD     ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO ," ).append("\n"); 
		query.append("           V.SKD_DIR_CD ," ).append("\n"); 
		query.append("           V.CTRL_LVL   ," ).append("\n"); 
		query.append("           V.CTRL_HC    ," ).append("\n"); 
		query.append("           V.CTRL_45    ," ).append("\n"); 
		query.append("           V.CTRL_53    ," ).append("\n"); 
		query.append("           V.CTRL_RF    ," ).append("\n"); 
		query.append("           V.CTRL_WT    ," ).append("\n"); 
		query.append("           V.CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", ALOC_DATA AS (" ).append("\n"); 
		query.append("    SELECT V.TRD_CD    ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD," ).append("\n"); 
		query.append("           V.RLANE_CD  ," ).append("\n"); 
		query.append("           A.DIR_CD    ," ).append("\n"); 
		query.append("           P.IOC_TS_CD ," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           NVL(V.VSL_CD, 'TTL') AS VSL_CD," ).append("\n"); 
		query.append("           V.SKD_VOY_NO," ).append("\n"); 
		query.append("           V.SKD_DIR_CD," ).append("\n"); 
		query.append("           NVL(SUBSTR(V.SLS_YRMON, 1, 4), 'TTL') AS COST_YR," ).append("\n"); 
		query.append("           V.COST_WK ," ).append("\n"); 
		query.append("           V.CTRL_LVL," ).append("\n"); 
		query.append("           V.CTRL_HC ," ).append("\n"); 
		query.append("           V.CTRL_45 ," ).append("\n"); 
		query.append("           V.CTRL_53 ," ).append("\n"); 
		query.append("           V.CTRL_RF ," ).append("\n"); 
		query.append("           V.CTRL_WT ," ).append("\n"); 
		query.append("           V.CTRL_ACCT," ).append("\n"); 
		query.append("#if(${suboffice} !='')" ).append("\n"); 
		query.append("           DECODE(SUM(A.BKG_AVAL_TTL_QTY), 0, SUM(A.ASGN_TTL_QTY), SUM(A.BKG_AVAL_TTL_QTY)) AS LOD_QTY" ).append("\n"); 
		query.append("           ,DECODE(SUM(A.BKG_AVAL_TTL_WGT), 0, SUM(A.ASGN_TTL_WGT), SUM(A.BKG_AVAL_TTL_WGT)) AS LOD_WT " ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("           SUM(A.ASGN_TTL_QTY) AS LOD_QTY" ).append("\n"); 
		query.append("           , SUM(A.ASGN_TTL_WGT) AS LOD_WT " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM SPC_ALOC_POL_POD A," ).append("\n"); 
		query.append("           VVD_POL_POD             V," ).append("\n"); 
		query.append("           PARAMS           P" ).append("\n"); 
		query.append("     WHERE 1=1 " ).append("\n"); 
		query.append("       AND A.RLANE_CD        = V.RLANE_CD" ).append("\n"); 
		query.append("       AND A.DIR_CD          = V.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND A.VSL_CD          = V.VSL_CD" ).append("\n"); 
		query.append("       AND A.SKD_VOY_NO      = V.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND A.SKD_DIR_CD      = V.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND A.SLS_RGN_OFC_CD  = P.RGN_OFC_CD" ).append("\n"); 
		query.append("       AND A.TS_FLG          = DECODE(P.IOC_TS_CD, 'T', 'Y', 'N')" ).append("\n"); 
		query.append("       AND A.IOC_CD          = DECODE(P.IOC_TS_CD, 'O', 'O', 'I')" ).append("\n"); 
		query.append("       AND A.RLANE_CD        = NVL(P.RLANE_CD, A.RLANE_CD)" ).append("\n"); 
		query.append("       -- MSSONG 추가 Ve" ).append("\n"); 
		query.append("	 AND A.POL_YD_CD      = V.POL_CD" ).append("\n"); 
		query.append("	 AND A.POD_YD_CD      = V.POD_CD" ).append("\n"); 
		query.append("#if(${suboffice} !='')" ).append("\n"); 
		query.append("       AND A.SLS_OFC_CD      = @[suboffice]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("  GROUP BY V.TRD_CD    ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD," ).append("\n"); 
		query.append("           V.RLANE_CD  ," ).append("\n"); 
		query.append("           A.DIR_CD    ," ).append("\n"); 
		query.append("           P.IOC_TS_CD ," ).append("\n"); 
		query.append("           P.RGN_OFC_CD," ).append("\n"); 
		query.append("           V.VSL_CD    ," ).append("\n"); 
		query.append("           V.SKD_VOY_NO," ).append("\n"); 
		query.append("           V.SKD_DIR_CD," ).append("\n"); 
		query.append("           SUBSTR(V.SLS_YRMON, 1, 4)," ).append("\n"); 
		query.append("           V.COST_WK   ," ).append("\n"); 
		query.append("           V.CTRL_LVL  ," ).append("\n"); 
		query.append("           V.CTRL_HC   ," ).append("\n"); 
		query.append("           V.CTRL_45   ," ).append("\n"); 
		query.append("           V.CTRL_53   ," ).append("\n"); 
		query.append("           V.CTRL_RF   ," ).append("\n"); 
		query.append("           V.CTRL_WT   ," ).append("\n"); 
		query.append("           V.CTRL_ACCT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", QTA_ALOC_DATA AS (" ).append("\n"); 
		query.append("    SELECT 1 AS LVL1 ," ).append("\n"); 
		query.append("           0 AS LVL2 ," ).append("\n"); 
		query.append("           TRD_CD    ," ).append("\n"); 
		query.append("           SUB_TRD_CD," ).append("\n"); 
		query.append("           RLANE_CD  ," ).append("\n"); 
		query.append("           DIR_CD    ," ).append("\n"); 
		query.append("           IOC_TS_CD ," ).append("\n"); 
		query.append("           RGN_OFC_CD," ).append("\n"); 
		query.append("           NULL AS CUST_CTRL_CD," ).append("\n"); 
		query.append("           NVL(VSL_CD    , 'TTL') AS VSL_CD    ," ).append("\n"); 
		query.append("           NVL(SKD_VOY_NO, ' ')   AS SKD_VOY_NO," ).append("\n"); 
		query.append("           NVL(SKD_DIR_CD, ' ')   AS SKD_DIR_CD," ).append("\n"); 
		query.append("           NVL(COST_YR   , 'TTL') AS COST_YR   ," ).append("\n"); 
		query.append("           COST_WK," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("           ROUND(SUM(QTA_LOD_QTY) / 2, 1) || ' / ' || ROUND(SUM(ALOC_LOD_QTA / 2), 1) || ' / ' || ROUND(SUM(ALOC_LOD_WT)) AS LOD_QTY" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("           ROUND(SUM(QTA_LOD_QTY)) || ' / ' || SUM(ALOC_LOD_QTA)|| ' / ' || ROUND(SUM(ALOC_LOD_WT)) AS LOD_QTY" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("              SELECT 1 AS FLG    ," ).append("\n"); 
		query.append("                     V.TRD_CD    ," ).append("\n"); 
		query.append("                     V.SUB_TRD_CD," ).append("\n"); 
		query.append("                     V.RLANE_CD  ," ).append("\n"); 
		query.append("                     V.DIR_CD    ," ).append("\n"); 
		query.append("                     V.IOC_TS_CD ," ).append("\n"); 
		query.append("                     P.RGN_OFC_CD," ).append("\n"); 
		query.append("                     V.VSL_CD    ," ).append("\n"); 
		query.append("                     V.SKD_VOY_NO," ).append("\n"); 
		query.append("                     V.SKD_DIR_CD," ).append("\n"); 
		query.append("                     V.COST_YR   ," ).append("\n"); 
		query.append("                     V.COST_WK   ," ).append("\n"); 
		query.append("                     A.LOD_QTY AS QTA_LOD_QTY ," ).append("\n"); 
		query.append("                     0         AS ALOC_LOD_QTA , 0 AS ALOC_LOD_WT" ).append("\n"); 
		query.append("                FROM QTA_DATA  A," ).append("\n"); 
		query.append("                     VVD_RANKS V," ).append("\n"); 
		query.append("                     PARAMS    P" ).append("\n"); 
		query.append("               WHERE 1=1 " ).append("\n"); 
		query.append("                 AND A.TRD_CD    (+) = V.TRD_CD" ).append("\n"); 
		query.append("                 AND A.SUB_TRD_CD(+) = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND A.RLANE_CD  (+) = V.RLANE_CD" ).append("\n"); 
		query.append("                 AND A.DIR_CD    (+) = V.DIR_CD" ).append("\n"); 
		query.append("                 AND A.VSL_CD    (+) = V.VSL_CD" ).append("\n"); 
		query.append("                 AND NVL(A.SKD_VOY_NO(+), ' ') = NVL(V.SKD_VOY_NO, ' ')" ).append("\n"); 
		query.append("                 AND NVL(A.SKD_DIR_CD(+), ' ') = NVL(V.SKD_DIR_CD, ' ')" ).append("\n"); 
		query.append("                 AND V.RLANE_CD  = NVL(P.RLANE_CD, V.RLANE_CD)" ).append("\n"); 
		query.append("              UNION ALL" ).append("\n"); 
		query.append("              SELECT 2 AS FLG    ," ).append("\n"); 
		query.append("                     V.TRD_CD    ," ).append("\n"); 
		query.append("                     V.SUB_TRD_CD," ).append("\n"); 
		query.append("                     V.RLANE_CD  ," ).append("\n"); 
		query.append("                     V.DIR_CD    ," ).append("\n"); 
		query.append("                     V.IOC_TS_CD ," ).append("\n"); 
		query.append("                     P.RGN_OFC_CD," ).append("\n"); 
		query.append("                     V.VSL_CD    ," ).append("\n"); 
		query.append("                     V.SKD_VOY_NO," ).append("\n"); 
		query.append("                     V.SKD_DIR_CD," ).append("\n"); 
		query.append("                     V.COST_YR   ," ).append("\n"); 
		query.append("                     V.COST_WK   ," ).append("\n"); 
		query.append("                     0         AS QTA_LOD_QTY ," ).append("\n"); 
		query.append("                     A.LOD_QTY AS ALOC_LOD_QTA, A.LOD_WT AS ALOC_LOD_WT" ).append("\n"); 
		query.append("                FROM ALOC_DATA A," ).append("\n"); 
		query.append("                     VVD_RANKS V," ).append("\n"); 
		query.append("                     PARAMS    P" ).append("\n"); 
		query.append("               WHERE 1=1 " ).append("\n"); 
		query.append("                 AND A.TRD_CD    (+) = V.TRD_CD" ).append("\n"); 
		query.append("                 AND A.SUB_TRD_CD(+) = V.SUB_TRD_CD" ).append("\n"); 
		query.append("                 AND A.RLANE_CD  (+) = V.RLANE_CD" ).append("\n"); 
		query.append("                 AND A.DIR_CD    (+) = V.DIR_CD" ).append("\n"); 
		query.append("                 AND A.VSL_CD    (+) = V.VSL_CD" ).append("\n"); 
		query.append("                 AND NVL(A.SKD_VOY_NO(+), ' ') = NVL(V.SKD_VOY_NO, ' ')" ).append("\n"); 
		query.append("                 AND NVL(A.SKD_DIR_CD(+), ' ') = NVL(V.SKD_DIR_CD, ' ')" ).append("\n"); 
		query.append("                 AND V.RLANE_CD  = NVL(P.RLANE_CD, V.RLANE_CD)" ).append("\n"); 
		query.append("           )" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( TRD_CD    , SUB_TRD_CD, RLANE_CD, DIR_CD    ," ).append("\n"); 
		query.append("                             IOC_TS_CD , RGN_OFC_CD, VSL_CD  , SKD_VOY_NO," ).append("\n"); 
		query.append("                             SKD_DIR_CD, COST_YR   , COST_WK               )," ).append("\n"); 
		query.append("                           ( TRD_CD    , SUB_TRD_CD, RLANE_CD, DIR_CD    ," ).append("\n"); 
		query.append("                             IOC_TS_CD , RGN_OFC_CD            ))" ).append("\n"); 
		query.append("    UNION ALL" ).append("\n"); 
		query.append("    SELECT 3 AS LVL1     ," ).append("\n"); 
		query.append("           0 AS LVL2     ," ).append("\n"); 
		query.append("           V.TRD_CD      ," ).append("\n"); 
		query.append("           V.SUB_TRD_CD  ," ).append("\n"); 
		query.append("           V.RLANE_CD    ," ).append("\n"); 
		query.append("           V.DIR_CD      ," ).append("\n"); 
		query.append("           V.IOC_TS_CD   ," ).append("\n"); 
		query.append("           P.RGN_OFC_CD  ," ).append("\n"); 
		query.append("           NVL(V.CUST_CTRL_CD, 'C') AS CUST_CTRL_CD," ).append("\n"); 
		query.append("           NVL(V.VSL_CD    , 'TTL') AS VSL_CD      ," ).append("\n"); 
		query.append("           NVL(V.SKD_VOY_NO, ' ')   AS SKD_VOY_NO  ," ).append("\n"); 
		query.append("           NVL(V.SKD_DIR_CD, ' ')   AS SKD_DIR_CD  ," ).append("\n"); 
		query.append("           NVL(V.COST_YR   , 'TTL') AS COST_YR     ," ).append("\n"); 
		query.append("           V.COST_WK     ," ).append("\n"); 
		query.append("#if(${suboffice} !='')" ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("           TO_CHAR(ROUND(SUM(A.BKG_AVAL_TTL_QTY) / 2, 1)) AS LOD_QTY" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("           TO_CHAR(SUM(A.BKG_AVAL_TTL_QTY)) AS LOD_QTY" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("           TO_CHAR(ROUND(SUM(A.ASGN_TTL_QTY) / 2, 1)) AS LOD_QTY" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("           TO_CHAR(SUM(A.ASGN_TTL_QTY)) AS LOD_QTY" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("      FROM SPC_ALOC_CUST_POL_POD A," ).append("\n"); 
		query.append("           (SELECT TRD_CD    ," ).append("\n"); 
		query.append("                     SUB_TRD_CD," ).append("\n"); 
		query.append("                     RLANE_CD  ," ).append("\n"); 
		query.append("                     SKD_DIR_CD AS DIR_CD," ).append("\n"); 
		query.append("                     IOC_TS_CD ," ).append("\n"); 
		query.append("                     VSL_CD    ," ).append("\n"); 
		query.append("                     SKD_VOY_NO," ).append("\n"); 
		query.append("                     SKD_DIR_CD," ).append("\n"); 
		query.append("                     COST_YR   ," ).append("\n"); 
		query.append("                     COST_WK   ," ).append("\n"); 
		query.append("                     RNK       ," ).append("\n"); 
		query.append("                     P.CUST_CTRL_CD," ).append("\n"); 
		query.append("                     P.RGN_OFC_CD" ).append("\n"); 
		query.append("                FROM VVD_RANKS  R," ).append("\n"); 
		query.append("                     (SELECT DISTINCT CUST_CTRL_CD, RGN_OFC_CD FROM GROUP_TOT ) P" ).append("\n"); 
		query.append("           )     V," ).append("\n"); 
		query.append("           VVDLS P" ).append("\n"); 
		query.append("     WHERE A.TRD_CD    (+) = V.TRD_CD" ).append("\n"); 
		query.append("       AND A.SUB_TRD_CD(+) = V.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND A.RLANE_CD  (+) = V.RLANE_CD" ).append("\n"); 
		query.append("       AND A.DIR_CD    (+) = V.DIR_CD" ).append("\n"); 
		query.append("       AND A.VSL_CD    (+) = V.VSL_CD" ).append("\n"); 
		query.append("       AND NVL(A.SKD_VOY_NO(+), ' ') = NVL(V.SKD_VOY_NO, ' ')" ).append("\n"); 
		query.append("       AND NVL(A.SKD_DIR_CD(+), ' ') = NVL(V.SKD_DIR_CD, ' ')" ).append("\n"); 
		query.append("       AND A.SLS_RGN_OFC_CD(+)       = V.RGN_OFC_CD" ).append("\n"); 
		query.append("       AND A.CUST_CTRL_CD  (+)       = V.CUST_CTRL_CD" ).append("\n"); 
		query.append("       AND V.TRD_CD     = P.TRD_CD" ).append("\n"); 
		query.append("       AND V.SUB_TRD_CD = P.SUB_TRD_CD" ).append("\n"); 
		query.append("       AND V.RLANE_CD   = P.RLANE_CD" ).append("\n"); 
		query.append("       AND V.DIR_CD     = P.DIR_CD" ).append("\n"); 
		query.append("       AND DECODE(V.IOC_TS_CD, 'T', 'I', V.IOC_TS_CD)  = P.IOC_CD" ).append("\n"); 
		query.append("       AND V.VSL_CD     = P.VSL_CD" ).append("\n"); 
		query.append("       AND V.SKD_VOY_NO = P.SKD_VOY_NO" ).append("\n"); 
		query.append("       AND V.SKD_DIR_CD = P.SKD_DIR_CD" ).append("\n"); 
		query.append("       AND P.CTRL_ACCT  = 'Y'" ).append("\n"); 
		query.append("#if(${suboffice} !='')" ).append("\n"); 
		query.append("       AND A.SLS_OFC_CD(+) = @[suboffice]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("  GROUP BY GROUPING SETS (( V.TRD_CD, V.SUB_TRD_CD, V.RLANE_CD, V.DIR_CD, V.IOC_TS_CD, P.RGN_OFC_CD, V.CUST_CTRL_CD )," ).append("\n"); 
		query.append("                           ( V.TRD_CD, V.SUB_TRD_CD, V.RLANE_CD, V.IOC_TS_CD, P.RGN_OFC_CD, V.CUST_CTRL_CD, V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, V.COST_YR, V.COST_WK ))" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append(", ALL_SUM_DATA AS (" ).append("\n"); 
		query.append("    SELECT LVL1           ," ).append("\n"); 
		query.append("           LVL2           ," ).append("\n"); 
		query.append("           MST_CNT        ," ).append("\n"); 
		query.append("           FCAST_CNT      ," ).append("\n"); 
		query.append("           BKG_CNT        ," ).append("\n"); 
		query.append("           TRD_CD         ," ).append("\n"); 
		query.append("           SUB_TRD_CD     ," ).append("\n"); 
		query.append("           RLANE_CD       ," ).append("\n"); 
		query.append("           DIR_CD         ," ).append("\n"); 
		query.append("           IOC_TS_CD      ," ).append("\n"); 
		query.append("           RGN_OFC_CD     ," ).append("\n"); 
		query.append("           SUB_OFC_CD     ," ).append("\n"); 
		query.append("           OFC_CD         ," ).append("\n"); 
		query.append("           SREP_CD        ," ).append("\n"); 
		query.append("           USA_BKG_MOD_CD ," ).append("\n"); 
		query.append("           CUST_CNT       ," ).append("\n"); 
		query.append("           CUST_TP_CD     ," ).append("\n"); 
		query.append("           CUST_CTRL_CD   ," ).append("\n"); 
		query.append("           DTL_SEQ        ," ).append("\n"); 
		query.append("           DECODE(SUBSTR(SC_RFA_NO,1,1),'1',SUBSTR(SC_RFA_NO,2),'') AS SC_NO," ).append("\n"); 
		query.append("           DECODE(TRD_CD,'AES',SUBSTR(SC_RFA_NO,2),'IAS',SUBSTR(SC_RFA_NO,2),'') AS RFA_NO," ).append("\n"); 
		query.append("           DECODE(TRD_CD,'AES',DECODE(SUBSTR(SC_RFA_NO,1,1),'1','S','R'),'IAS',DECODE(SUBSTR(SC_RFA_NO,1,1),'1','S','R'),'S') AS SC_FLG," ).append("\n"); 
		query.append("           RLANE_ADJ_QTY  ," ).append("\n"); 
		query.append("           CUST_CNT_CD    ," ).append("\n"); 
		query.append("           CUST_SEQ       ," ).append("\n"); 
		query.append("           BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("           BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("           POL_CD         ," ).append("\n"); 
		query.append("           POD_CD         ," ).append("\n"); 
		query.append("           VSL_CD         ," ).append("\n"); 
		query.append("           SKD_VOY_NO     ," ).append("\n"); 
		query.append("           SKD_DIR_CD     ," ).append("\n"); 
		query.append("           SUBSTR(COST_YRWK, 1, 4) AS COST_YR," ).append("\n"); 
		query.append("           SUBSTR(COST_YRWK, 5, 2) AS COST_WK," ).append("\n"); 
		query.append("           CHL            ," ).append("\n"); 
		query.append("           CTRL_LVL       ," ).append("\n"); 
		query.append("           CTRL_HC        ," ).append("\n"); 
		query.append("           CTRL_45        ," ).append("\n"); 
		query.append("           CTRL_53        ," ).append("\n"); 
		query.append("           CTRL_RF        ," ).append("\n"); 
		query.append("           CTRL_WT        ," ).append("\n"); 
		query.append("           CTRL_ACCT      ," ).append("\n"); 
		query.append("           FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("           FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("           FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("           FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("           FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("           FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("           FCAST_RF_QTY     ," ).append("\n"); 
		query.append("           FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("           FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("           FCAST_RD_QTY     ," ).append("\n"); 
		query.append("           FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("           BKG_TTL_QTY      ," ).append("\n"); 
		query.append("           BKG_20FT_QTY     ," ).append("\n"); 
		query.append("           BKG_40FT_QTY     ," ).append("\n"); 
		query.append("           BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("           BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("           BKG_53FT_QTY     ," ).append("\n"); 
		query.append("           BKG_RF_QTY       ," ).append("\n"); 
		query.append("           BKG_TTL_WGT      ," ).append("\n"); 
		query.append("           BKG_20FT_DRY_QTY ," ).append("\n"); 
		query.append("           BKG_40FT_DRY_QTY ," ).append("\n"); 
		query.append("           BKG_RD_QTY       ," ).append("\n"); 
		query.append("           CIF              ," ).append("\n"); 
		query.append("           FOB              ," ).append("\n"); 
		query.append("           OTH              ," ).append("\n"); 
		query.append("           TOT_CNT          ," ).append("\n"); 
		query.append("           FCAST_RMK        ," ).append("\n"); 
		query.append("           SUM(TTL_SUM) OVER (PARTITION BY A.TRD_CD, A.SUB_TRD_CD, A.RLANE_CD, A.DIR_CD, A.IOC_TS_CD, A.RGN_OFC_CD, A.SREP_CD, A.CUST_CNT_CD, A.CUST_SEQ, A.SC_RFA_NO) AS ZERO_CUST," ).append("\n"); 
		query.append("           CUST_TTL_CNT" ).append("\n"); 
		query.append("           , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("            SELECT NVL(A.LVL1, 0) AS LVL1," ).append("\n"); 
		query.append("                   NVL(A.LVL2, 0) AS LVL2," ).append("\n"); 
		query.append("                   SUM(DECODE(A.FLG, 'M', 1, 0)) AS MST_CNT  ," ).append("\n"); 
		query.append("                   SUM(DECODE(A.FLG, 'F', 1, 0)) AS FCAST_CNT," ).append("\n"); 
		query.append("                   SUM(DECODE(A.FLG, 'B', 1, 0)) AS BKG_CNT  ," ).append("\n"); 
		query.append("                   A.TRD_CD         ," ).append("\n"); 
		query.append("                   A.SUB_TRD_CD     ," ).append("\n"); 
		query.append("                   A.RLANE_CD       ," ).append("\n"); 
		query.append("                   A.DIR_CD         ," ).append("\n"); 
		query.append("                   A.IOC_TS_CD      ," ).append("\n"); 
		query.append("                   A.RGN_OFC_CD     ," ).append("\n"); 
		query.append("                   NVL(O.SUB_OFC_CD, A.OFC_CD) AS SUB_OFC_CD," ).append("\n"); 
		query.append("                   A.OFC_CD         ," ).append("\n"); 
		query.append("                   A.SREP_CD        ," ).append("\n"); 
		query.append("                   COUNT(A.CUST_TP_CD) AS CUST_CNT," ).append("\n"); 
		query.append("                   A.CUST_TP_CD     ," ).append("\n"); 
		query.append("                   A.CUST_CTRL_CD   ," ).append("\n"); 
		query.append("                   USA_BKG_MOD_CD, " ).append("\n"); 
		query.append("                   A.DTL_SEQ        ," ).append("\n"); 
		query.append("                   DECODE(A.DTL_SEQ, NULL, NULL, DECODE(NVL(MAX(A.SC_NO),'1'),'1','2'|| MAX(A.RFA_NO), '1'|| MAX(A.SC_NO))) AS SC_RFA_NO," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("                   ROUND(A.RLANE_ADJ_QTY / 2, 1) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                   A.RLANE_ADJ_QTY  ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                   A.CUST_CNT_CD    ," ).append("\n"); 
		query.append("                   A.CUST_SEQ       ," ).append("\n"); 
		query.append("                   A.BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("                   A.BKG_CUST_SEQ   ," ).append("\n"); 
		query.append("                   A.POL_CD         ," ).append("\n"); 
		query.append("                   A.POD_CD         ," ).append("\n"); 
		query.append("                   NVL(A.VSL_CD    , 'TTL') AS VSL_CD    ," ).append("\n"); 
		query.append("                   NVL(A.SKD_VOY_NO, ' ')   AS SKD_VOY_NO," ).append("\n"); 
		query.append("                   NVL(A.SKD_DIR_CD, ' ')   AS SKD_DIR_CD," ).append("\n"); 
		query.append("                   MAX(A.COST_YR) AS COST_YR," ).append("\n"); 
		query.append("                   MAX(A.COST_WK) AS COST_WK," ).append("\n"); 
		query.append("                   MAX(A.COST_YR||A.COST_WK) AS COST_YRWK," ).append("\n"); 
		query.append("                   SUM(A.CHL)     AS CHL    ," ).append("\n"); 
		query.append("                   A.CTRL_LVL," ).append("\n"); 
		query.append("                   A.CTRL_HC ," ).append("\n"); 
		query.append("                   A.CTRL_45 ," ).append("\n"); 
		query.append("                   A.CTRL_53 ," ).append("\n"); 
		query.append("                   A.CTRL_RF ," ).append("\n"); 
		query.append("                   A.CTRL_WT ," ).append("\n"); 
		query.append("                   A.CTRL_ACCT," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.FCAST_TTL_QTY) / 2, 1))     AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.FCAST_20FT_QTY) / 2, 1))    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_QTY))                  AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_HC_QTY))               AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_45FT_HC_QTY))               AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_53FT_QTY))                  AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_RF_QTY))                    AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_TTL_WGT))                   AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_20FT_DRY_QTY))AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_DRY_QTY))AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_RD_QTY))      AS FCAST_RD_QTY      ," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.BKG_TTL_QTY) / 2, 1))       AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.BKG_20FT_QTY) / 2, 1))      AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_QTY))                    AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_HC_QTY))                 AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_45FT_HC_QTY))                 AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_53FT_QTY))                    AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_RF_QTY))                      AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_TTL_WGT))                     AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.CIF) / 2, 2), '999,990.99') AS CIF              ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_20FT_DRY_QTY))  AS BKG_20FT_DRY_QTY ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_DRY_QTY))  AS BKG_40FT_DRY_QTY ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_RD_QTY))        AS BKG_RD_QTY       , " ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.FOB) / 2, 2), '999,990.99') AS FOB              ," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(SUM(A.OTH) / 2, 2), '999,990.99') AS OTH              ," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_TTL_QTY))     AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_20FT_QTY))    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_QTY))    AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_HC_QTY)) AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_45FT_HC_QTY)) AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_53FT_QTY))    AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_RF_QTY))      AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_TTL_WGT))     AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_20FT_DRY_QTY))AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_40FT_DRY_QTY))AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FCAST_RD_QTY))      AS FCAST_RD_QTY      ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_TTL_QTY))       AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_20FT_QTY))      AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_QTY))      AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_HC_QTY))   AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_45FT_HC_QTY))   AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_53FT_QTY))      AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_RF_QTY))        AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_TTL_WGT))       AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_20FT_DRY_QTY))  AS BKG_20FT_DRY_QTY ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_40FT_DRY_QTY))  AS BKG_40FT_DRY_QTY ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.BKG_RD_QTY))        AS BKG_RD_QTY       ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.CIF), '999,990.99') AS CIF              ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.FOB), '999,990.99') AS FOB              ," ).append("\n"); 
		query.append("                   TO_CHAR(SUM(A.OTH), '999,990.99') AS OTH              ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                   (SUM(A.RLANE_ADJ_QTY) + SUM(A.FCAST_TTL_QTY) + SUM(A.FCAST_40FT_HC_QTY) + SUM(A.FCAST_45FT_HC_QTY) + SUM(A.FCAST_53FT_QTY) + SUM(A.FCAST_RF_QTY) + SUM(A.FCAST_TTL_WGT) + SUM(A.BKG_TTL_QTY) + SUM(A.BKG_TTL_WGT) ) AS TTL_SUM," ).append("\n"); 
		query.append("                   NVL2(A.VSL_CD, 0, 1)              AS TOT_CNT          ," ).append("\n"); 
		query.append("                   MAX(FCAST_RMK)                    AS FCAST_RMK        ," ).append("\n"); 
		query.append("                   COUNT(CUST_CNT_CD) OVER (PARTITION BY A.TRD_CD, A.SUB_TRD_CD, A.RLANE_CD, A.DIR_CD, A.IOC_TS_CD, A.RGN_OFC_CD, A.SREP_CD) AS CUST_TTL_CNT" ).append("\n"); 
		query.append("                   , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("              FROM ALL_DATA A," ).append("\n"); 
		query.append("                   OFFICES  O" ).append("\n"); 
		query.append("             WHERE A.OFC_CD     = O.OFC_CD(+)" ).append("\n"); 
		query.append("#if (${salesoffice} == 'SELSC')" ).append("\n"); 
		query.append("               AND A.SUB_OFC_CD = O.SUB_OFC_CD  (+)" ).append("\n"); 
		query.append("               AND A.RGN_OFC_CD = O.RGN_OFC_CD  (+)" ).append("\n"); 
		query.append("               AND A.SREP_CD    = O.SALES_REP_CD(+)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("               AND NOT (A.LVL1 = 4 AND A.LVL2 = 1 AND A.FCAST_TTL_QTY = 0)" ).append("\n"); 
		query.append("          GROUP BY GROUPING SETS ( ( A.LVL1       , A.LVL2           , A.TRD_CD         , A.SUB_TRD_CD     , A.RLANE_CD     ," ).append("\n"); 
		query.append("                                     A.DIR_CD     , A.IOC_TS_CD      , A.CTRL_LVL       , A.CTRL_HC        , A.CTRL_45      ," ).append("\n"); 
		query.append("                                     A.CTRL_53    , A.CTRL_RF        , A.CTRL_WT        , A.CTRL_ACCT      , A.RGN_OFC_CD   , NVL(O.SUB_OFC_CD, A.OFC_CD)," ).append("\n"); 
		query.append("                                     A.OFC_CD     , A.SREP_CD        , A.CUST_TP_CD     , A.CUST_CTRL_CD   , A.RLANE_ADJ_QTY," ).append("\n"); 
		query.append("                                     A.CUST_CNT_CD, A.CUST_SEQ       , A.DTL_SEQ        , A.BKG_CUST_CNT_CD, A.BKG_CUST_SEQ ," ).append("\n"); 
		query.append("                                     A.POL_CD     , A.POD_CD         , A.VSL_CD         , A.SKD_VOY_NO     , A.SKD_DIR_CD      " ).append("\n"); 
		query.append("                                     , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG " ).append("\n"); 
		query.append("                                     , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                     ),                                     " ).append("\n"); 
		query.append("                                   ( A.LVL1       , A.LVL2           , A.TRD_CD         , A.SUB_TRD_CD     , A.RLANE_CD     ," ).append("\n"); 
		query.append("                                     A.DIR_CD     , A.IOC_TS_CD      , A.RGN_OFC_CD     , NVL(O.SUB_OFC_CD , A.OFC_CD)      , A.OFC_CD," ).append("\n"); 
		query.append("                                     A.SREP_CD    , A.CUST_TP_CD     , A.CUST_CTRL_CD   , A.RLANE_ADJ_QTY  , A.CUST_CNT_CD  ," ).append("\n"); 
		query.append("                                     A.CUST_SEQ   , A.DTL_SEQ        , A.BKG_CUST_CNT_CD, A.BKG_CUST_SEQ   , A.POL_CD       ," ).append("\n"); 
		query.append("                                     A.POD_CD                                                                                  " ).append("\n"); 
		query.append("                                     , USA_BKG_MOD_CD" ).append("\n"); 
		query.append("                                     ),                                     " ).append("\n"); 
		query.append("                                   ( A.TRD_CD     , A.SUB_TRD_CD     , A.RLANE_CD       , A.DIR_CD         , A.IOC_TS_CD    ," ).append("\n"); 
		query.append("                                     A.CTRL_LVL   , A.CTRL_HC        , A.CTRL_45        , A.CTRL_53        , A.CTRL_RF      ," ).append("\n"); 
		query.append("                                     A.CTRL_WT    , A.CTRL_ACCT      , A.RGN_OFC_CD     , A.VSL_CD         , A.SKD_VOY_NO   ," ).append("\n"); 
		query.append("                                     A.SKD_DIR_CD                                                                              " ).append("\n"); 
		query.append("                                     , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG " ).append("\n"); 
		query.append("                                     ),                                     " ).append("\n"); 
		query.append("                                   ( A.TRD_CD     , A.SUB_TRD_CD     , A.RLANE_CD       , A.DIR_CD         , A.IOC_TS_CD    ," ).append("\n"); 
		query.append("                                     A.RGN_OFC_CD  " ).append("\n"); 
		query.append("                                     )" ).append("\n"); 
		query.append("                                 )" ).append("\n"); 
		query.append("            HAVING NOT (     BKG_CUST_CNT_CD IS NOT NULL" ).append("\n"); 
		query.append("                         AND CUST_CNT_CD = NVL(BKG_CUST_CNT_CD, 'X')" ).append("\n"); 
		query.append("                         AND CUST_SEQ    = NVL(BKG_CUST_SEQ, 1))" ).append("\n"); 
		query.append("            UNION ALL" ).append("\n"); 
		query.append("            SELECT LVL1                        ," ).append("\n"); 
		query.append("                   LVL2                        ," ).append("\n"); 
		query.append("                   0 AS MST_CNT                ," ).append("\n"); 
		query.append("                   0 AS FCAST_CNT              ," ).append("\n"); 
		query.append("                   0 AS BKG_CNT                ," ).append("\n"); 
		query.append("                   TRD_CD                      ," ).append("\n"); 
		query.append("                   SUB_TRD_CD                  ," ).append("\n"); 
		query.append("                   RLANE_CD                    ," ).append("\n"); 
		query.append("                   DIR_CD                      ," ).append("\n"); 
		query.append("                   IOC_TS_CD                   ," ).append("\n"); 
		query.append("                   RGN_OFC_CD                  ," ).append("\n"); 
		query.append("                   NULL AS SUB_OFC_CD          ," ).append("\n"); 
		query.append("                   NULL AS OFC_CD              ," ).append("\n"); 
		query.append("                   NULL AS SREP_CD             ," ).append("\n"); 
		query.append("                   0    AS CUST_CNT            ," ).append("\n"); 
		query.append("                   NULL AS CUST_TP_CD          ," ).append("\n"); 
		query.append("                   CUST_CTRL_CD                ," ).append("\n"); 
		query.append("                   NULL USA_BKG_MOD_CD, " ).append("\n"); 
		query.append("                   NULL AS DTL_SEQ             ," ).append("\n"); 
		query.append("                   NULL AS SC_RFA_NO           ," ).append("\n"); 
		query.append("                   NULL AS RLANE_ADJ_QTY       ," ).append("\n"); 
		query.append("                   NULL AS CUST_CNT_CD         ," ).append("\n"); 
		query.append("                   NULL AS CUST_SEQ            ," ).append("\n"); 
		query.append("                   NULL AS BKG_CUST_CNT_CD     ," ).append("\n"); 
		query.append("                   NULL AS BKG_CUST_SEQ        ," ).append("\n"); 
		query.append("                   NULL AS POL_CD              ," ).append("\n"); 
		query.append("                   NULL AS POD_CD              ," ).append("\n"); 
		query.append("                   VSL_CD                      ," ).append("\n"); 
		query.append("                   SKD_VOY_NO                  ," ).append("\n"); 
		query.append("                   DECODE(VSL_CD, 'TTL', ' ', SKD_DIR_CD) AS SKD_DIR_CD," ).append("\n"); 
		query.append("                   COST_YR                     ," ).append("\n"); 
		query.append("                   COST_WK                     ," ).append("\n"); 
		query.append("                   COST_YR||COST_WK AS COST_YRWK," ).append("\n"); 
		query.append("                   0       AS CHL              ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_LVL         ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_HC          ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_45          ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_53          ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_RF          ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_WT          ," ).append("\n"); 
		query.append("                   NULL    AS CTRL_ACCT        ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   LOD_QTY AS FCAST_RD_QTY     ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_20FT_DRY_QTY ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_40FT_DRY_QTY ," ).append("\n"); 
		query.append("                   LOD_QTY AS BKG_RD_QTY       ," ).append("\n"); 
		query.append("                   NULL    AS CIF              ," ).append("\n"); 
		query.append("                   NULL    AS FOB              ," ).append("\n"); 
		query.append("                   NULL    AS OTH              ," ).append("\n"); 
		query.append("                   0       AS TTL_SUM          ," ).append("\n"); 
		query.append("                   0       AS TOT_CNT          ," ).append("\n"); 
		query.append("                   NULL    AS FCAST_RMK        ," ).append("\n"); 
		query.append("                   1       AS CUST_TTL_CNT" ).append("\n"); 
		query.append("                   , NULL AS CTRL_D2_FLG, NULL AS CTRL_D4_FLG, NULL AS CTRL_RD_FLG, NULL AS CTRL_ECC_FLG, NULL AS CTRL_LOC_FLG , NULL AS CTRL_USA_SVC_MOD_FLG , NULL AS CTRL_ACCT_FLG" ).append("\n"); 
		query.append("              FROM QTA_ALOC_DATA" ).append("\n"); 
		query.append("            UNION ALL" ).append("\n"); 
		query.append("            SELECT LVL1                      ," ).append("\n"); 
		query.append("                   LVL2                      ," ).append("\n"); 
		query.append("                   0 AS MST_CNT              ," ).append("\n"); 
		query.append("                   0 AS FCAST_CNT            ," ).append("\n"); 
		query.append("                   0 AS BKG_CNT              ," ).append("\n"); 
		query.append("                   TRD_CD                    ," ).append("\n"); 
		query.append("                   SUB_TRD_CD                ," ).append("\n"); 
		query.append("                   RLANE_CD                  ," ).append("\n"); 
		query.append("                   DIR_CD                    ," ).append("\n"); 
		query.append("                   IOC_TS_CD                 ," ).append("\n"); 
		query.append("                   RGN_OFC_CD                ," ).append("\n"); 
		query.append("                   NULL AS SUB_OFC_CD        ," ).append("\n"); 
		query.append("                   NULL AS OFC_CD            ," ).append("\n"); 
		query.append("                   NULL AS SREP_CD           ," ).append("\n"); 
		query.append("                   0    AS CUST_CNT          ," ).append("\n"); 
		query.append("                   NULL AS CUST_TP_CD        ," ).append("\n"); 
		query.append("                   CUST_CTRL_CD              ," ).append("\n"); 
		query.append("                   NULL USA_BKG_MOD_CD, " ).append("\n"); 
		query.append("                   NULL AS DTL_SEQ           ," ).append("\n"); 
		query.append("                   NULL AS SC_RFA_NO         ," ).append("\n"); 
		query.append("                   (" ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("                     SELECT ROUND(SUM(RLANE_ADJ_QTY) / 2, 1)" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                     SELECT SUM(RLANE_ADJ_QTY)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                       FROM (  SELECT DISTINCT" ).append("\n"); 
		query.append("                                      TRD_CD       ," ).append("\n"); 
		query.append("                                      SUB_TRD_CD   ," ).append("\n"); 
		query.append("                                      RLANE_CD     ," ).append("\n"); 
		query.append("                                      DIR_CD       ," ).append("\n"); 
		query.append("                                      IOC_TS_CD    ," ).append("\n"); 
		query.append("                                      RGN_OFC_CD   ," ).append("\n"); 
		query.append("                                      CUST_CTRL_CD ," ).append("\n"); 
		query.append("                                      RLANE_ADJ_QTY," ).append("\n"); 
		query.append("                                      NVL(SC_NO,RFA_NO)" ).append("\n"); 
		query.append("                                 FROM ALL_DATA" ).append("\n"); 
		query.append("                                WHERE LVL1 = 4" ).append("\n"); 
		query.append("                                  AND LVL2 = 3" ).append("\n"); 
		query.append("                                  AND RLANE_ADJ_QTY > 0) A" ).append("\n"); 
		query.append("                      WHERE A.TRD_CD       = T.TRD_CD" ).append("\n"); 
		query.append("                        AND A.SUB_TRD_CD   = T.SUB_TRD_CD" ).append("\n"); 
		query.append("                        AND A.RLANE_CD     = T.RLANE_CD" ).append("\n"); 
		query.append("                        AND A.CUST_CTRL_CD = T.CUST_CTRL_CD" ).append("\n"); 
		query.append("                   ) AS RLANE_ADJ_QTY," ).append("\n"); 
		query.append("                   NULL AS CUST_CNT_CD       ," ).append("\n"); 
		query.append("                   NULL AS CUST_SEQ          ," ).append("\n"); 
		query.append("                   NULL AS BKG_CUST_CNT_CD   ," ).append("\n"); 
		query.append("                   NULL AS BKG_CUST_SEQ      ," ).append("\n"); 
		query.append("                   POL_CD                    ," ).append("\n"); 
		query.append("                   POD_CD                    ," ).append("\n"); 
		query.append("                   VSL_CD                    ," ).append("\n"); 
		query.append("                   SKD_VOY_NO                ," ).append("\n"); 
		query.append("                   SKD_DIR_CD                ," ).append("\n"); 
		query.append("                   COST_YR                   ," ).append("\n"); 
		query.append("                   COST_WK                   ," ).append("\n"); 
		query.append("                   COST_YR||COST_WK AS COST_YRWK," ).append("\n"); 
		query.append("                   0    AS CHL               ," ).append("\n"); 
		query.append("                   NULL AS CTRL_LVL          ," ).append("\n"); 
		query.append("                   NULL AS CTRL_HC           ," ).append("\n"); 
		query.append("                   NULL AS CTRL_45           ," ).append("\n"); 
		query.append("                   NULL AS CTRL_53           ," ).append("\n"); 
		query.append("                   NULL AS CTRL_RF           ," ).append("\n"); 
		query.append("                   NULL AS CTRL_WT           ," ).append("\n"); 
		query.append("                   NULL AS CTRL_ACCT         ," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(FCAST_TTL_QTY     / 2, 1))," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(FCAST_20FT_QTY    / 2, 1))," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_QTY)   ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_HC_QTY)," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_45FT_HC_QTY)," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_53FT_QTY)   ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_RF_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_TTL_WGT)    ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_20FT_DRY_QTY)FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_DRY_QTY)FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_RD_QTY)     FCAST_RD_QTY,	" ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(BKG_TTL_QTY       / 2, 1))," ).append("\n"); 
		query.append("                   TO_CHAR(ROUND(BKG_20FT_QTY      / 2, 1))," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_HC_QTY)  ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_45FT_HC_QTY)  ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_53FT_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_RF_QTY)       ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_TTL_WGT)      ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_20FT_DRY_QTY) BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_DRY_QTY) BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_RD_QTY)       BKG_RD_QTY," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_TTL_QTY)    ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_20FT_QTY)   ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_QTY)   ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_HC_QTY)," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_45FT_HC_QTY)," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_53FT_QTY)   ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_RF_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_TTL_WGT)    ," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_20FT_DRY_QTY)FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_40FT_DRY_QTY)FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(FCAST_RD_QTY)     FCAST_RD_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_TTL_QTY )     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_20FT_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_HC_QTY)  ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_45FT_HC_QTY)  ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_53FT_QTY)     ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_RF_QTY)       ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_TTL_WGT)      ," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_20FT_DRY_QTY) BKG_20FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_40FT_DRY_QTY) BKG_40FT_DRY_QTY," ).append("\n"); 
		query.append("                   TO_CHAR(BKG_RD_QTY)       BKG_RD_QTY," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                   NULL AS CIF               ," ).append("\n"); 
		query.append("                   NULL AS FOB               ," ).append("\n"); 
		query.append("                   NULL AS OTH               ," ).append("\n"); 
		query.append("                   0    AS TTL_SUM          ," ).append("\n"); 
		query.append("                   DECODE(VSL_CD, 'TTL', 1, 0) AS TOT_CNT," ).append("\n"); 
		query.append("                   NULL AS FCAST_RMK         ," ).append("\n"); 
		query.append("                   0    AS CUST_TTL_CNT" ).append("\n"); 
		query.append("                   , NULL AS CTRL_D2_FLG, NULL AS CTRL_D4_FLG, NULL AS CTRL_RD_FLG, NULL AS CTRL_ECC_FLG, NULL AS CTRL_LOC_FLG , NULL AS CTRL_USA_SVC_MOD_FLG , NULL AS CTRL_ACCT_FLG" ).append("\n"); 
		query.append("              FROM GROUP_TOT T" ).append("\n"); 
		query.append("        ) A" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("  SELECT TRD_CD           ," ).append("\n"); 
		query.append("         SUB_TRD_CD       ," ).append("\n"); 
		query.append("         RLANE_CD         ," ).append("\n"); 
		query.append("         DIR_CD           ," ).append("\n"); 
		query.append("         IOC_TS_CD        ," ).append("\n"); 
		query.append("         RGN_OFC_CD       ," ).append("\n"); 
		query.append("#if (${salesoffice} == 'SELSC')" ).append("\n"); 
		query.append("         SUB_OFC_CD       ," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("         SUB_OFC_CD       ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("         OFC_CD           ," ).append("\n"); 
		query.append("         SREP_CD          ," ).append("\n"); 
		query.append("         SREP_NM          ," ).append("\n"); 
		query.append("         CUST_TP_CD       ," ).append("\n"); 
		query.append("         ACCT_LVL         ," ).append("\n"); 
		query.append("         FCAST_SEQ        ," ).append("\n"); 
		query.append("         SC_NO            ," ).append("\n"); 
		query.append("         RFA_NO           ," ).append("\n"); 
		query.append("         SC_FLG           ," ).append("\n"); 
		query.append("         GUIDE            ," ).append("\n"); 
		query.append("         DECODE(USA_BKG_MOD_CD2, 'OTH', 'OTHERS', USA_BKG_MOD_CD2) USA_BKG_MOD_CD," ).append("\n"); 
		query.append("         CUST_CNT_CD2 CUST_CNT_CD      ," ).append("\n"); 
		query.append("         CUST_SEQ2 CUST_SEQ         ," ).append("\n"); 
		query.append("         BKG_CUST_CNT_CD  ," ).append("\n"); 
		query.append("         BKG_CUST_SEQ     ," ).append("\n"); 
		query.append("         BKG_CUST_NM      ," ).append("\n"); 
		query.append("         CUST_NM          ," ).append("\n"); 
		query.append("         POL_CD           ," ).append("\n"); 
		query.append("         POD_CD           ," ).append("\n"); 
		query.append("         VSL_CD           ," ).append("\n"); 
		query.append("         SKD_VOY_NO       ," ).append("\n"); 
		query.append("         SKD_DIR_CD       ," ).append("\n"); 
		query.append("         COST_YR          ," ).append("\n"); 
		query.append("         COST_WK          ," ).append("\n"); 
		query.append("         LVL              ," ).append("\n"); 
		query.append("         LVL1             ," ).append("\n"); 
		query.append("         LVL2             ," ).append("\n"); 
		query.append("         ISEDITABLE       ," ).append("\n"); 
		query.append("         COLOR            ," ).append("\n"); 
		query.append("         RNUM             ," ).append("\n"); 
		query.append("         CHL              ," ).append("\n"); 
		query.append("         CTRL_LVL         ," ).append("\n"); 
		query.append("         CTRL_HC          ," ).append("\n"); 
		query.append("         CTRL_45          ," ).append("\n"); 
		query.append("         CTRL_53          ," ).append("\n"); 
		query.append("         CTRL_RF          ," ).append("\n"); 
		query.append("         CTRL_WT          ," ).append("\n"); 
		query.append("         POL_SEQ          ," ).append("\n"); 
		query.append("         POD_SEQ          ," ).append("\n"); 
		query.append("         MST_CNT          ," ).append("\n"); 
		query.append("         FCAST_CNT        ," ).append("\n"); 
		query.append("         BKG_CNT          ," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("         FCAST_TTL_TEU_QTY," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("         FCAST_TTL_TEU_QTY," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("         FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("         FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("         FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#elseif(${view_type} == 'TEU')" ).append("\n"); 
		query.append("         FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("         FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("         FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("         FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("         FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("         FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("         FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("         FCAST_RF_QTY     ," ).append("\n"); 
		query.append("         FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("         FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("         FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("         FCAST_RD_QTY     ," ).append("\n"); 
		query.append("         BKG_TTL_QTY      ," ).append("\n"); 
		query.append("         BKG_20FT_QTY     ," ).append("\n"); 
		query.append("         BKG_40FT_QTY     ," ).append("\n"); 
		query.append("         BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("         BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("         BKG_53FT_QTY     ," ).append("\n"); 
		query.append("         BKG_RF_QTY       ," ).append("\n"); 
		query.append("         BKG_TTL_WGT      ," ).append("\n"); 
		query.append("         BKG_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("         BKG_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("         BKG_RD_QTY       ,	" ).append("\n"); 
		query.append("         CIF              ," ).append("\n"); 
		query.append("         FOB              ," ).append("\n"); 
		query.append("         OTH              ," ).append("\n"); 
		query.append("         EXIST_FLG        ," ).append("\n"); 
		query.append("         NEW_FLG          ," ).append("\n"); 
		query.append("         TOT_CNT          ," ).append("\n"); 
		query.append("         FCAST_RMK        ," ).append("\n"); 
		query.append("         CTRL_ACCT        ," ).append("\n"); 
		query.append("         HH_FLG           ," ).append("\n"); 
		query.append("         SLS_RHQ_CD       ," ).append("\n"); 
		query.append("         ZERO_CUST" ).append("\n"); 
		query.append("         , CTRL_D2_FLG, CTRL_D4_FLG, CTRL_RD_FLG, CTRL_ECC_FLG, CTRL_LOC_FLG , CTRL_USA_SVC_MOD_FLG , CTRL_ACCT_FLG" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("      SELECT A.TRD_CD    ," ).append("\n"); 
		query.append("             A.SUB_TRD_CD," ).append("\n"); 
		query.append("             A.RLANE_CD  ," ).append("\n"); 
		query.append("             A.DIR_CD    ," ).append("\n"); 
		query.append("             A.IOC_TS_CD ," ).append("\n"); 
		query.append("             A.RGN_OFC_CD," ).append("\n"); 
		query.append("#if (${salesoffice} == 'SELSC')" ).append("\n"); 
		query.append("             NVL(A.SUB_OFC_CD,A.OFC_CD) AS SUB_OFC_CD," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("             A.SUB_OFC_CD," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("             A.OFC_CD     ," ).append("\n"); 
		query.append("             A.SREP_CD    ," ).append("\n"); 
		query.append("             SR.SREP_NM   ," ).append("\n"); 
		query.append("             A.CUST_TP_CD ," ).append("\n"); 
		query.append("             A.CUST_CTRL_CD  AS ACCT_LVL ," ).append("\n"); 
		query.append("             A.DTL_SEQ       AS FCAST_SEQ," ).append("\n"); 
		query.append("             A.SC_NO      ," ).append("\n"); 
		query.append("             A.RFA_NO     ," ).append("\n"); 
		query.append("             A.SC_FLG     ," ).append("\n"); 
		query.append("             A.RLANE_ADJ_QTY AS GUIDE    ," ).append("\n"); 
		query.append("             A.USA_BKG_MOD_CD USA_BKG_MOD_CD2," ).append("\n"); 
		query.append("             A.CUST_CNT_CD CUST_CNT_CD2," ).append("\n"); 
		query.append("             TO_CHAR(A.CUST_SEQ, 'FM000000') AS CUST_SEQ2," ).append("\n"); 
		query.append("             A.BKG_CUST_CNT_CD," ).append("\n"); 
		query.append("             TO_CHAR(A.BKG_CUST_SEQ, 'FM000000') AS BKG_CUST_SEQ," ).append("\n"); 
		query.append("             SC.CUST_LGL_ENG_NM AS BKG_CUST_NM," ).append("\n"); 
		query.append("             MC.CUST_LGL_ENG_NM AS CUST_NM    ," ).append("\n"); 
		query.append("             A.POL_CD     ," ).append("\n"); 
		query.append("             A.POD_CD     ," ).append("\n"); 
		query.append("             A.VSL_CD     ," ).append("\n"); 
		query.append("             A.SKD_VOY_NO ," ).append("\n"); 
		query.append("             A.SKD_DIR_CD ," ).append("\n"); 
		query.append("             DECODE(A.VSL_CD, 'TTL', 'TTL', A.COST_YR) AS COST_YR," ).append("\n"); 
		query.append("             DECODE(A.VSL_CD, 'TTL', ''   , A.COST_WK) AS COST_WK," ).append("\n"); 
		query.append("             A.LVL1 * 10 + A.LVL2 AS LVL," ).append("\n"); 
		query.append("             A.LVL1," ).append("\n"); 
		query.append("             A.LVL2," ).append("\n"); 
		query.append("             CASE" ).append("\n"); 
		query.append("                  WHEN     A.LVL1 = 4" ).append("\n"); 
		query.append("                       AND A.LVL2 > 1" ).append("\n"); 
		query.append("                       AND A.VSL_CD <> 'TTL'" ).append("\n"); 
		query.append("                       AND (A.MST_CNT <> 0 OR A.FCAST_CNT <> 0)" ).append("\n"); 
		query.append("                       AND (    (A.LVL2 = 4 AND (A.CTRL_LVL IN ('T', 'D') OR A.IOC_TS_CD <> 'O') )" ).append("\n"); 
		query.append("                             OR (A.IOC_TS_CD = 'O' AND A.CTRL_LVL NOT IN ('D', 'T') AND A.LVL2 = 3) ) THEN 'TRUE'" ).append("\n"); 
		query.append("                                                                                                      ELSE 'FALSE'                                                                    " ).append("\n"); 
		query.append("              END ISEDITABLE," ).append("\n"); 
		query.append("             CASE" ).append("\n"); 
		query.append("                  WHEN A.LVL1 = 0                THEN 0" ).append("\n"); 
		query.append("                  WHEN A.LVL1 = 1                THEN 1" ).append("\n"); 
		query.append("                  WHEN A.LVL1 = 2 AND A.LVL2 = 0 THEN 3" ).append("\n"); 
		query.append("                  WHEN A.LVL1 = 3 AND A.LVL2 = 0 THEN 1" ).append("\n"); 
		query.append("                  WHEN A.LVL2 = 0                THEN 2" ).append("\n"); 
		query.append("                  WHEN A.LVL1 = 4 AND A.LVL2 = 1 THEN 6" ).append("\n"); 
		query.append("                                                 ELSE 4" ).append("\n"); 
		query.append("              END COLOR," ).append("\n"); 
		query.append("             NVL(R.RNUM, 0)  AS RNUM," ).append("\n"); 
		query.append("             LEAST(A.CHL, 1) AS CHL ," ).append("\n"); 
		query.append("             A.CTRL_LVL ," ).append("\n"); 
		query.append("             NVL(A.CTRL_HC, MAX(A.CTRL_HC) OVER (PARTITION BY A.TRD_CD)) CTRL_HC," ).append("\n"); 
		query.append("             NVL(A.CTRL_45, MAX(A.CTRL_45) OVER (PARTITION BY A.TRD_CD)) CTRL_45," ).append("\n"); 
		query.append("             NVL(A.CTRL_53, MAX(A.CTRL_53) OVER (PARTITION BY A.TRD_CD)) CTRL_53," ).append("\n"); 
		query.append("             NVL(A.CTRL_RF, MAX(A.CTRL_RF) OVER (PARTITION BY A.TRD_CD)) CTRL_RF," ).append("\n"); 
		query.append("             NVL(A.CTRL_WT, MAX(A.CTRL_WT) OVER (PARTITION BY A.TRD_CD)) CTRL_WT," ).append("\n"); 
		query.append("             PLS.POL_SEQ," ).append("\n"); 
		query.append("             PDS.POD_SEQ," ).append("\n"); 
		query.append("             (A.MST_CNT)   AS MST_CNT  ," ).append("\n"); 
		query.append("             (A.FCAST_CNT) AS FCAST_CNT," ).append("\n"); 
		query.append("             (A.BKG_CNT)   AS BKG_CNT  ," ).append("\n"); 
		query.append("             --FCAST_TTL_TEU_QTY 추가(TEU + HC * 2 + 45FT * 2 + 53FT * 2), LVL1이 1이 아닌경우에만 적용(1인경우는 TGT/ALOC이므로 다른컬럼과 동일한 값으로 넣어줌)" ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("             DECODE(A.LVL1, 1, FCAST_TTL_QTY, ROUND(NVL(FCAST_TTL_QTY,0) / 2, 1) + NVL(FCAST_40FT_HC_QTY, 0) + NVL(FCAST_45FT_HC_QTY, 0) + NVL(FCAST_53FT_QTY, 0)) AS FCAST_TTL_TEU_QTY," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("             DECODE(A.LVL1, 1, FCAST_TTL_QTY, NVL(FCAST_TTL_QTY,0) + NVL(FCAST_40FT_HC_QTY, 0) * 2 + NVL(FCAST_45FT_HC_QTY, 0) * 2 + NVL(FCAST_53FT_QTY, 0) * 2)   AS FCAST_TTL_TEU_QTY," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("             (FCAST_TTL_QTY)     AS FCAST_TTL_QTY    ," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${view_type} == 'FEU')" ).append("\n"); 
		query.append("             0                   AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("             (FCAST_TTL_QTY)     AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#elseif(${view_type} == 'TEU')" ).append("\n"); 
		query.append("             (FCAST_TTL_QTY)     AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("             0                   AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("             (FCAST_20FT_QTY)    AS FCAST_20FT_QTY   ," ).append("\n"); 
		query.append("             (FCAST_40FT_QTY)    AS FCAST_40FT_QTY   ," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("             (FCAST_40FT_HC_QTY) AS FCAST_40FT_HC_QTY," ).append("\n"); 
		query.append("             (FCAST_45FT_HC_QTY) AS FCAST_45FT_HC_QTY," ).append("\n"); 
		query.append("             (FCAST_53FT_QTY)    AS FCAST_53FT_QTY   ," ).append("\n"); 
		query.append("             (FCAST_RF_QTY)      AS FCAST_RF_QTY     ," ).append("\n"); 
		query.append("             (FCAST_TTL_WGT)     AS FCAST_TTL_WGT    ," ).append("\n"); 
		query.append("             (FCAST_20FT_DRY_QTY)AS FCAST_20FT_DRY_QTY," ).append("\n"); 
		query.append("             (FCAST_40FT_DRY_QTY)AS FCAST_40FT_DRY_QTY," ).append("\n"); 
		query.append("             (FCAST_RD_QTY)      AS FCAST_RD_QTY     ," ).append("\n"); 
		query.append("             (BKG_TTL_QTY)       AS BKG_TTL_QTY      ," ).append("\n"); 
		query.append("             (BKG_20FT_QTY)      AS BKG_20FT_QTY     ," ).append("\n"); 
		query.append("             (BKG_40FT_QTY)      AS BKG_40FT_QTY     ," ).append("\n"); 
		query.append("             (BKG_40FT_HC_QTY)   AS BKG_40FT_HC_QTY  ," ).append("\n"); 
		query.append("             (BKG_45FT_HC_QTY)   AS BKG_45FT_HC_QTY  ," ).append("\n"); 
		query.append("             (BKG_53FT_QTY)      AS BKG_53FT_QTY     ," ).append("\n"); 
		query.append("             (BKG_RF_QTY)        AS BKG_RF_QTY       ," ).append("\n"); 
		query.append("             (BKG_TTL_WGT)       AS BKG_TTL_WGT      ," ).append("\n"); 
		query.append("             (BKG_20FT_DRY_QTY)  AS BKG_20FT_DRY_QTY  ," ).append("\n"); 
		query.append("             (BKG_40FT_DRY_QTY)  AS BKG_40FT_DRY_QTY  ," ).append("\n"); 
		query.append("             (BKG_RD_QTY)        AS BKG_RD_QTY        ," ).append("\n"); 
		query.append("             (CIF)               AS CIF              ," ).append("\n"); 
		query.append("             (FOB)               AS FOB              ," ).append("\n"); 
		query.append("             (OTH)               AS OTH              ," ).append("\n"); 
		query.append("             NVL((SELECT MAX('Y')" ).append("\n"); 
		query.append("                    FROM BKG_CUST_SLS_REP" ).append("\n"); 
		query.append("                   WHERE CUST_CNT_CD = A.CUST_CNT_CD" ).append("\n"); 
		query.append("                     AND CUST_SEQ    = A.CUST_SEQ" ).append("\n"); 
		query.append("                     AND DELT_FLG    = 'N'" ).append("\n"); 
		query.append("                     AND SREP_CD     = A.SREP_CD), 'N') AS EXIST_FLG," ).append("\n"); 
		query.append("             NVL((SELECT MAX('Y')" ).append("\n"); 
		query.append("                  FROM BKG_CUST_SLS_REP B," ).append("\n"); 
		query.append("                       SPC_SLS_REP_CUST SR" ).append("\n"); 
		query.append("                  WHERE B.DELT_FLG    = 'N'" ).append("\n"); 
		query.append("                    AND B.SREP_CD     = A.SREP_CD" ).append("\n"); 
		query.append("                    AND B.SREP_CD    <> SR.SREP_CD" ).append("\n"); 
		query.append("                    AND B.CUST_CNT_CD = SR.CUST_CNT_CD" ).append("\n"); 
		query.append("                    AND B.CUST_SEQ    = SR.CUST_SEQ" ).append("\n"); 
		query.append("                    AND SR.DELT_FLG   = 'N'" ).append("\n"); 
		query.append("                    AND NOT EXISTS (SELECT 'A'" ).append("\n"); 
		query.append("                                      FROM SPC_SLS_REP_CUST S" ).append("\n"); 
		query.append("                                     WHERE S.CUST_CNT_CD = B.CUST_CNT_CD" ).append("\n"); 
		query.append("                                       AND S.CUST_SEQ    = B.CUST_SEQ" ).append("\n"); 
		query.append("                                       AND S.SREP_CD     = B.SREP_CD" ).append("\n"); 
		query.append("                                       AND S.DELT_FLG    = 'N'" ).append("\n"); 
		query.append("                                       AND NVL(S.INDIV_CUST_USE_FLG, 'N') = 'Y')), 'N') AS NEW_FLG," ).append("\n"); 
		query.append("             SUM(TOT_CNT) OVER (PARTITION BY A.TRD_CD) AS TOT_CNT," ).append("\n"); 
		query.append("             A.FCAST_RMK," ).append("\n"); 
		query.append("             A.CTRL_ACCT," ).append("\n"); 
		query.append("             NVL(( SELECT /*+ INDEX_DESC (M XPKSPC_MDL_VER_MST) */'Y'" ).append("\n"); 
		query.append("                    FROM SPC_MDL_VER_MST" ).append("\n"); 
		query.append("                   WHERE TRD_CD = A.TRD_CD" ).append("\n"); 
		query.append("                     AND A.COST_YR||A.COST_WK BETWEEN VER_ST_YRWK AND VER_END_YRWK" ).append("\n"); 
		query.append("                     AND COST_YRWK||'-'||VER_SEQ <> '200001-1'" ).append("\n"); 
		query.append("                     AND CFM_FLG  = 'Y'" ).append("\n"); 
		query.append("                     AND R.HH_FLG = 'Y'" ).append("\n"); 
		query.append("                     AND ROWNUM   = 1), 'N') AS HH_FLG," ).append("\n"); 
		query.append("             (SELECT N2ND_PRNT_OFC_CD" ).append("\n"); 
		query.append("                FROM SPC_OFC_LVL" ).append("\n"); 
		query.append("               WHERE OFC_CD = A.RGN_OFC_CD" ).append("\n"); 
		query.append("                 AND A.COST_YR||A.COST_WK BETWEEN OFC_APLY_FM_YRWK AND OFC_APLY_TO_YRWK) AS SLS_RHQ_CD," ).append("\n"); 
		query.append("             ZERO_CUST," ).append("\n"); 
		query.append("             COUNT(A.CUST_CNT_CD||A.CUST_SEQ) OVER (PARTITION BY A.TRD_CD, A.SUB_TRD_CD, A.RLANE_CD, A.DIR_CD, A.IOC_TS_CD, A.RGN_OFC_CD, A.SUB_OFC_CD, A.OFC_CD, A.SREP_CD) AS SREP_CUST_CNT" ).append("\n"); 
		query.append("             , NVL(A.CTRL_D2_FLG, MAX(A.CTRL_D2_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_D2_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_D4_FLG, MAX(A.CTRL_D4_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_D4_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_RD_FLG, MAX(A.CTRL_RD_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_RD_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_ECC_FLG, MAX(A.CTRL_ECC_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_ECC_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_LOC_FLG, MAX(A.CTRL_LOC_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_LOC_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_USA_SVC_MOD_FLG, MAX(A.CTRL_USA_SVC_MOD_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_USA_SVC_MOD_FLG" ).append("\n"); 
		query.append("             , MAX(A.CTRL_USA_SVC_MOD_FLG) OVER (PARTITION BY A.TRD_CD) MAX_US_MOD_FLG" ).append("\n"); 
		query.append("             , NVL(A.CTRL_ACCT_FLG, MAX(A.CTRL_ACCT_FLG) OVER (PARTITION BY A.TRD_CD)) CTRL_ACCT_FLG             " ).append("\n"); 
		query.append("        FROM ALL_SUM_DATA A  ," ).append("\n"); 
		query.append("             VVD_POS      R  ," ).append("\n"); 
		query.append("             POL_SEQ      PLS," ).append("\n"); 
		query.append("             POL_POD_SEQ  PDS," ).append("\n"); 
		query.append("             MDM_SLS_REP  SR ," ).append("\n"); 
		query.append("             MDM_CUSTOMER MC ," ).append("\n"); 
		query.append("             MDM_CUSTOMER SC" ).append("\n"); 
		query.append("       WHERE R.TRD_CD      (+) = A.TRD_CD" ).append("\n"); 
		query.append("         AND R.SUB_TRD_CD  (+) = A.SUB_TRD_CD" ).append("\n"); 
		query.append("         AND R.RLANE_CD    (+) = A.RLANE_CD" ).append("\n"); 
		query.append("         AND R.DIR_CD      (+) = A.DIR_CD" ).append("\n"); 
		query.append("         AND R.VSL_CD      (+) = A.VSL_CD" ).append("\n"); 
		query.append("         AND R.SKD_VOY_NO  (+) = A.SKD_VOY_NO" ).append("\n"); 
		query.append("         AND R.SKD_DIR_CD  (+) = A.SKD_DIR_CD" ).append("\n"); 
		query.append("         AND PLS.TRD_CD    (+) = A.TRD_CD" ).append("\n"); 
		query.append("         AND PLS.SUB_TRD_CD(+) = A.SUB_TRD_CD" ).append("\n"); 
		query.append("         AND PLS.RLANE_CD  (+) = A.RLANE_CD" ).append("\n"); 
		query.append("         AND PLS.DIR_CD    (+) = A.DIR_CD" ).append("\n"); 
		query.append("         AND PLS.POL_CD    (+) = A.POL_CD" ).append("\n"); 
		query.append("         AND PDS.TRD_CD    (+) = A.TRD_CD" ).append("\n"); 
		query.append("         AND PDS.SUB_TRD_CD(+) = A.SUB_TRD_CD" ).append("\n"); 
		query.append("         AND PDS.RLANE_CD  (+) = A.RLANE_CD" ).append("\n"); 
		query.append("         AND PDS.DIR_CD    (+) = A.DIR_CD" ).append("\n"); 
		query.append("         AND PDS.POL_CD    (+) = A.POL_CD" ).append("\n"); 
		query.append("         AND PDS.POD_CD    (+) = A.POD_CD" ).append("\n"); 
		query.append("         AND SR.SREP_CD    (+) = A.SREP_CD" ).append("\n"); 
		query.append("         AND MC.CUST_CNT_CD(+) = A.CUST_CNT_CD" ).append("\n"); 
		query.append("         AND MC.CUST_SEQ   (+) = A.CUST_SEQ" ).append("\n"); 
		query.append("         AND SC.CUST_CNT_CD(+) = A.BKG_CUST_CNT_CD" ).append("\n"); 
		query.append("         AND SC.CUST_SEQ   (+) = A.BKG_CUST_SEQ" ).append("\n"); 
		query.append("         AND (A.LVL1 < 4 OR A.LVL2 = 0 OR A.CUST_TP_CD IS NOT NULL)" ).append("\n"); 
		query.append("         AND NOT (A.LVL1 = 4 AND A.LVL2 = 3 AND A.USA_BKG_MOD_CD IS NULL AND (A.SC_NO IS NOT NULL OR A.CUST_CNT_CD IS NOT NULL))" ).append("\n"); 
		query.append("         AND NOT (A.LVL1 = 2 AND A.LVL2>2)" ).append("\n"); 
		query.append("#if(${salesrep} == '')" ).append("\n"); 
		query.append("         AND NOT (A.SREP_CD IS NOT NULL AND CUST_TTL_CNT = 0)" ).append("\n"); 
		query.append("         AND NOT (A.LVL1>=4 AND A.CUST_CNT_CD IS NOT NULL AND NVL(A.ZERO_CUST,1) = 0)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("    ORDER BY" ).append("\n"); 
		query.append("         A.TRD_CD    ," ).append("\n"); 
		query.append("         A.SUB_TRD_CD," ).append("\n"); 
		query.append("         A.RLANE_CD  ," ).append("\n"); 
		query.append("         A.DIR_CD    ," ).append("\n"); 
		query.append("         NVL(A.IOC_TS_CD , '0')," ).append("\n"); 
		query.append("         NVL(A.RGN_OFC_CD, '0')," ).append("\n"); 
		query.append("         DECODE(A.SUB_OFC_CD, NULL, '0', A.RGN_OFC_CD, '1', A.SUB_OFC_CD)," ).append("\n"); 
		query.append("         LVL1     ," ).append("\n"); 
		query.append("         DECODE(LVL1, '3', ACCT_LVL)," ).append("\n"); 
		query.append("         DECODE(LVL1, '3', NVL(USA_BKG_MOD_CD2, '1'))      ," ).append("\n"); 
		query.append("         DECODE(LVL1, '3', DECODE(A.POL_CD, NULL, 0, NVL(PLS.POL_SEQ, 99)))," ).append("\n"); 
		query.append("         DECODE(LVL1, '3', LVL2)          ," ).append("\n"); 
		query.append("         DECODE(LVL1, '3', DECODE(A.POD_CD, NULL, 0, NVL(PDS.POD_SEQ, 99)))," ).append("\n"); 
		query.append("         A.SREP_CD," ).append("\n"); 
		query.append("         NVL(A.CUST_CTRL_CD, '0')," ).append("\n"); 
		query.append("         ACCT_LVL," ).append("\n"); 
		query.append("         NVL(USA_BKG_MOD_CD2, '1')," ).append("\n"); 
		query.append("         NVL(A.CUST_TP_CD  , '0')," ).append("\n"); 
		query.append("         DECODE(MC.CUST_LGL_ENG_NM, 'OTHERS', 'zzz', MC.CUST_LGL_ENG_NM)," ).append("\n"); 
		query.append("         NVL(A.CUST_CNT_CD, '0')," ).append("\n"); 
		query.append("         NVL(A.CUST_SEQ, 0)   ," ).append("\n"); 
		query.append("         NVL(SC.CUST_LGL_ENG_NM,'0')," ).append("\n"); 
		query.append("         NVL(A.BKG_CUST_CNT_CD, '0')," ).append("\n"); 
		query.append("         NVL(A.BKG_CUST_SEQ, 0)," ).append("\n"); 
		query.append("         A.DTL_SEQ ," ).append("\n"); 
		query.append("         GUIDE DESC," ).append("\n"); 
		query.append("         DECODE(A.POL_CD, NULL, 0, NVL(PLS.POL_SEQ, 99))," ).append("\n"); 
		query.append("         A.POL_CD," ).append("\n"); 
		query.append("         DECODE(A.POD_CD, NULL, 0, NVL(PDS.POD_SEQ, 99))," ).append("\n"); 
		query.append("         A.POD_CD," ).append("\n"); 
		query.append("         NVL(R.RNUM, 99)" ).append("\n"); 
		query.append(") A" ).append("\n"); 
		query.append("WHERE NOT (A.LVL1 = 2 AND A.LVL2=2 AND A.USA_BKG_MOD_CD2 IS NOT NULL AND A.POL_CD IS NULL AND MAX_US_MOD_FLG='N')" ).append("\n"); 
		query.append("#if(${salesrep} == '')" ).append("\n"); 
		query.append("AND NOT (LVL1=4 AND SREP_CUST_CNT=0)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("/*" ).append("\n"); 
		query.append("[CHM-201322502-01] SPC 프로젝트 - 성수기 선복운영 개선을 위한 T/F추진" ).append("\n"); 
		query.append("[CHM-201324211-01] SELBB 내 팀 소속의 경우, sub office 없이 조회시 2가지로 쪼개져서 조회되는 것 수정" ).append("\n"); 
		query.append("[CHM-201324211-01] Sub Trade 반영 전 individual로 fcst 넣은 내역이 Others에서 누락됨" ).append("\n"); 
		query.append("[CHM-201325983-01] BKG의 SC를 가져오지 않도록 수정.(BKG SC로 인해 동일화주에 SC 여러개인 경우로 인해 Guide Total이 잘못 조회됨)" ).append("\n"); 
		query.append("[CHM-201325983-01] MULTI SC 조회되도록, S.REP, ACCT 데이터 없는 것 보이지 않도록." ).append("\n"); 
		query.append("2013-08-05 비수기 sc 조회시 hh_flg 엮지 않도록" ).append("\n"); 
		query.append("2013-09-10 Contract Forecast S.Rep 의 이름에 , 가 포함되어 있어 에러남. |로 수정" ).append("\n"); 
		query.append("2013-09-17 [Trouble Shooting] 진마리아 SC Join 오류 수정" ).append("\n"); 
		query.append("2013.11.26 진마리아 [CHM-201326854] SAQ project로 인한 SPC 변경건_FNC 우선제거" ).append("\n"); 
		query.append("2013.12.05 김시몬 [CHM-201326854] SAQ project로 인한 SPC 변경건_테이블 변경" ).append("\n"); 
		query.append("2013.12.27 최윤성 [선반영] 연말 연초 걸쳐서 조회시 COST_YRWK 로직 부분 수정" ).append("\n"); 
		query.append("2014.01.17 최윤성 [긴급반영] SMP 의 Season-Version 가지고 오는 로직 부분 수정" ).append("\n"); 
		query.append("2014.02.04 [CHM-201428383-01] RFA 로직 추가" ).append("\n"); 
		query.append("2014.03.05 김시몬 [선처리] BKG RFA NULL 관련 보완" ).append("\n"); 
		query.append("2014.03.17 [CHM-20142960] SMP/Allocation control보완 요청 - SPC_GET_SMP_AMEND_FNC 적용" ).append("\n"); 
		query.append("2014.03.25 김시몬 [선처리] SQM 분기구하는 로직 관련 보완" ).append("\n"); 
		query.append("2014.05.22 [선반영] AES-SC관련 로직 추가" ).append("\n"); 
		query.append("2014.08.21 Data쪼개지는 오류 수정" ).append("\n"); 
		query.append("*/" ).append("\n"); 

	}
}
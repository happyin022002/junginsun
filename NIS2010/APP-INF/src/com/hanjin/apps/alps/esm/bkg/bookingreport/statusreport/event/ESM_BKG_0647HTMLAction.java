...
public class ESM_BKG_0647HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0647HTMLAction 객체를 생성
     */
    public ESM_BKG_0647HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0647Event event = new EsmBkg0647Event();

		if (command.isCommand(FormCommand.SEARCH)){
			event.setInfoVO((BlStsReportInVO) getVO(request, BlStsReportInVO.class));
		}else if (command.isCommand(FormCommand.SEARCH01)){
			event.setInfo2VO((ReportTemplateListVO) getVO(request, ReportTemplateListVO.class));
		}

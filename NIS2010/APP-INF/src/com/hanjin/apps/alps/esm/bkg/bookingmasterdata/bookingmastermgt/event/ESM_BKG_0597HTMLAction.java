...
public class ESM_BKG_0597HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0597HTMLAction 객체를 생성
	 */
	public ESM_BKG_0597HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0597Event event = new EsmBkg0597Event();
		
		if(command.isCommand(FormCommand.SEARCH01) || command.isCommand(FormCommand.SEARCH02) ) {
			event.setBdrAccessAuthorityInfoVO((BdrAccessAuthorityInfoVO) getVO(request, BdrAccessAuthorityInfoVO.class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setBdrAccessAuthorityInfoVOs((BdrAccessAuthorityInfoVO[]) getVOs(request, BdrAccessAuthorityInfoVO.class));
		}

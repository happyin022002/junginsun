...
public class NetworkDistributionDBDAOSearchMissingStatusRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SearchMissingStatus SELECT
	  * 2010.09.08 김민아 [CHM-201005891-01] SML Sales/ Slot Cht-out 로직 수정
	  *                            기존 : OTH 일때만 BSA_ZR_FLG = 'Y' 이면 카운트 안함
	  *                            수정 : SML, OTH 상관없이 BSA_ZR_FLG = 'Y' 이면 카운트 안함
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("          FROM MAS_MSS_STS" ).append("\n"); 
		...
		query.append("         AND PRC_NM IN('MAS_CREATE_NTCOST_PRC','MAS_CREATE_SPC_CHT_PRC')" ).append("\n"); 
		query.append("         AND BSA_ZR_FLG = 'N' /* OTH, SML 상관없이 BSA_ZR_FLG = 'Y' 이면 카운트 안함 */" ).append("\n"); 

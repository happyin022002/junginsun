...
public class UnmatchBLDBDAOCheckTotalBlAmountRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * BL에 Rating된 전체 금액과 심사시 Rating한 금액의 전체 합이 USD 5불 이내인지 확인한다.
	  * (Include Amount 제외, USD 환산 금액으로 비교)
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("*******************************************************************************************/" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("SELECT  SUM( (SELECT BR.CHG_AMT / XR.USD_LOCL_XCH_RT FROM XR WHERE XR.CURR_CD = BR.CURR_CD) )  CHG_USD_AMT" ).append("\n"); 
		...
		query.append("        SUM(( SELECT RA.CHG_AMT / XR.USD_LOCL_XCH_RT FROM XR WHERE XR.CURR_CD = RA.CURR_CD ))  CHG_USD_AMT" ).append("\n"); 

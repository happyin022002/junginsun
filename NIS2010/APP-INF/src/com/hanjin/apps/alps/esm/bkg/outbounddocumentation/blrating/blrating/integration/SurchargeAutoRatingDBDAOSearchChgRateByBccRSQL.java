...
*@FileName : SurchargeAutoRatingDBDAOSearchChgRateByBccRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SurchargeAutoRatingDBDAOSearchChgRateByBccRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Surcharge Table의 조건에 따라 BCC 조회
	  * 1) 조회조건 : BKG Route 정보를 기반으로 Surcharge의 Location 정보와 각각 일치 여부를 확인
	  * 2) FRT Term : Login OFC 와 Collect OFC가 동일 할 경우 C, 다를 경우 P
	  * 3) Surcharge Table 조회시 해당 금액이 여러 개가 존재 할 경우 가장 큰 금액을 적용한다.
	  * 
	  * [CHM-201325123] 
	  * 1. 조건 : Log in Office가 PKGSC일 경우
	  * 2. 변경 사항 : Payment Office 부여 Rule
	  * - PPD Office Charge 금액이 "0"이고 PPD-3rd Office 금액>0인 경우 : PPD 3rd Office로 부여
	  * - PPD-3rd Office가 복수인 경우 : OFT의 PPD-3rd Office로 부여
	  * - PPD office charge 금액>0인 경우 : PPD Office
	  * - CCT 금액만 있을 경우 : PPD Office
	  * [CHM-201536254] PHXSC Training : BCC Auto face 로직에 S/C Exception 반영 검토 요청
	  * [CHM-201539580] Doc Surcharge 자동 인터레이스 로직 변경 (Collection Office 관련)
	  *    > Log-in office가 Booking office의 Parent office(MDM 기준)일 경우에는 BKG Office를 Login Office로 치환하여 3rd office 처리
	  * </pre>
	  */
	public SurchargeAutoRatingDBDAOSearchChgRateByBccRSQL(){
		setQuery();
		...
		query.append("FileName : SurchargeAutoRatingDBDAOSearchChgRateByBccRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("     BKG_CHG_RT_HIS  BC," ).append("\n"); 
		...
		query.append("AND BR.CORR_NO = 'TMP0000001'" ).append("\n"); 
		...
		query.append("FROM    BK, PRI_SC_NOTE_CONV NC,  -- 일단 SC만 테스트," ).append("\n"); 
		...
		query.append("AND     NC.NOTE_CONV_CHG_CD = 'BCC'" ).append("\n"); 
		...
		query.append("                      ) ROW_RANK  " ).append("\n"); 
		query.append("FROM    BK, PRI_RFA_NOTE_CONV NC,  -- 일단 SC만 테스트," ).append("\n"); 
		query.append("        MDM_CHARGE                MC" ).append("\n"); 
		query.append("WHERE   NC.SVC_SCP_CD = BK.BKG_SVC_SCP_CD" ).append("\n"); 
		query.append("AND     NC.PROP_NO = BK.bk_prop_no" ).append("\n"); 
		query.append("AND     NC.AMDT_SEQ = BK.BK_AMDT_SEQ" ).append("\n"); 
		...
		query.append("       (SELECT CASE WHEN @[login_office] <> 'PKGSA' AND (@[login_office] = PPD_RCV_OFC_CD OR PPD_RCV_OFC_CD = CLT_OFC_CD) THEN @[login_office]" ).append("\n"); 
		query.append("                    WHEN @[login_office] <> 'PKGSA' AND @[login_office] = (SELECT PRNT_OFC_CD FROM MDM_ORGANIZATION WHERE OFC_CD = (SELECT BKG_OFC_CD FROM BKG_BOOKING WHERE BKG_NO = @[bkg_no]))" ).append("\n"); 
		query.append("                         THEN (SELECT BKG_OFC_CD FROM BKG_BOOKING WHERE BKG_NO = @[bkg_no]) " ).append("\n"); 
		query.append("                    WHEN @[login_office] <> 'PKGSA' THEN @[login_office]" ).append("\n"); 
		query.append("                    WHEN PPD_AMT = 0 AND PPD_TRD_AMT > 0 " ).append("\n"); 
		query.append("                         THEN (SELECT N3PTY_RCV_OFC_CD" ).append("\n"); 
		query.append("                               FROM (SELECT CHG_CD, N3PTY_RCV_OFC_CD" ).append("\n"); 
		query.append("#if(${ca_flg} == 'Y')" ).append("\n"); 
		...
		query.append("                                     ORDER BY DECODE(CHG_CD,'OFT',1,2))" ).append("\n"); 
		query.append("                               WHERE ROWNUM = 1)" ).append("\n"); 
		query.append("                    ELSE PPD_RCV_OFC_CD" ).append("\n"); 
		query.append("               END LOGIN_OFC                 " ).append("\n"); 
		query.append("        FROM ( SELECT SUM(CASE WHEN FRT_TERM_CD = 'P' AND N3PTY_RCV_OFC_CD IS NULL THEN CHG_AMT" ).append("\n"); 
		query.append("                            ELSE 0 " ).append("\n"); 
		query.append("                            END) PPD_AMT," ).append("\n"); 
		query.append("                      SUM(CASE WHEN  FRT_TERM_CD = 'P' AND N3PTY_RCV_OFC_CD IS NOT NULL THEN CHG_AMT" ).append("\n"); 
		query.append("                            ELSE 0 " ).append("\n"); 
		query.append("                            END) PPD_TRD_AMT" ).append("\n"); 
		query.append("#if(${ca_flg} == 'Y')" ).append("\n"); 
		query.append("                FROM BKG_CHG_RT_HIS" ).append("\n"); 
		query.append("                WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("                AND CORR_NO = 'TMP0000001'" ).append("\n"); 
		...
		query.append("                WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                AND FRT_INCL_XCLD_DIV_CD = 'N') C,    " ).append("\n"); 
		query.append("#if(${ca_flg} == 'Y')" ).append("\n"); 
		query.append("             BKG_RT_HIS R" ).append("\n"); 
		...
		query.append("                                      DESC, SR.SCG_AMT DESC) ROW_NUMBER  " ).append("\n"); 
		query.append("    ," ).append("\n"); 
		...
		query.append("                                            + DECODE(SR.POD_TP_CD, 'L', 600, 'G', 500, 'T', 400, 'R', 300, 'C', 200, 0)" ).append("\n"); 
		query.append("                                            + DECODE(SR.DEL_TP_CD, 'L', 600, 'G', 500, 'T', 400, 'R', 300, 'C', 200, 0)" ).append("\n"); 
		query.append("                                      DESC, SR.SCG_AMT DESC) ROW_RANK  " ).append("\n"); 
		...
		query.append("    AND   SP.CHG_CD  =  'BCC' /* BKG_SVC_SCP_CD 를 사용한다. */ " ).append("\n"); 
		...
		query.append("                     AND (" ).append("\n"); 
		query.append("                          MC.CHG_APLY_AREA_CD = 'C'" ).append("\n"); 
		query.append("                          OR  SUB_M.RCV_TERM_CD  IN ( DECODE(MC.NA_RD_TERM_FLG, 'Y', SUB_M.RCV_TERM_CD), " ).append("\n"); 
		query.append("                                                      DECODE(MC.CY_RD_TERM_FLG, 'Y', 'Y'), " ).append("\n"); 
		query.append("                                                      DECODE(MC.DOR_RD_TERM_FLG, 'Y', 'D'), " ).append("\n"); 
		query.append("                                                      DECODE(MC.CFS_RD_TERM_FLG, 'Y', 'S'), " ).append("\n"); 
		query.append("                                                      DECODE(MC.TKL_TML_FLG, 'Y', 'T') )" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append("                    )" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("    AND     (" ).append("\n"); 
		...
		query.append("                    AND (" ).append("\n"); 
		query.append("                        MC.CHG_APLY_AREA_CD = 'P'" ).append("\n"); 
		query.append("                        OR  SUB_M.DE_TERM_CD   IN ( DECODE(MC.NA_RD_TERM_FLG, 'Y', SUB_M.DE_TERM_CD), " ).append("\n"); 
		query.append("                                                    DECODE(MC.CY_RD_TERM_FLG, 'Y', 'Y'), " ).append("\n"); 
		query.append("                                                    DECODE(MC.DOR_RD_TERM_FLG, 'Y', 'D'), " ).append("\n"); 
		query.append("                                                    DECODE(MC.CFS_RD_TERM_FLG, 'Y', 'S'), " ).append("\n"); 
		query.append("                                                    DECODE(MC.TKL_TML_FLG, 'Y', 'T') )" ).append("\n"); 
		...
		query.append("       S3.LOGIN_OFFICE" ).append("\n"); 
		...
		query.append("                WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                AND CHG_CD IN ('MCF','SBL')" ).append("\n"); 
		query.append("                AND ROWNUM = 1" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("AND ROWNUM = 1" ).append("\n"); 

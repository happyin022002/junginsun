...
*@FileName : UI_PRI_2001_02HTMLAction.java
...
public class ESM_PRI_2001_02HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_2001_02HTMLAction 객체를 생성
	 */
	public ESM_PRI_2001_02HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri200102Event event = new EsmPri200102Event();
		
		if(command.isCommand(FormCommand.MULTI)) {		
			GrpCmdtGlineVO vo = new GrpCmdtGlineVO();
			vo.setPriRgGrpCmdtVOS((PriRgGrpCmdtVO[])getVOs(request, PriRgGrpCmdtVO.class, "sheet1_"));
			vo.setPriRgGrpCmdtDtlVOS((PriRgGrpCmdtDtlVO[])getVOs(request, PriRgGrpCmdtDtlVO.class, "sheet2_"));

			event.setGrpCmdtGlineVO(vo);			
		} else if (command.isCommand(FormCommand.SEARCH20)) {
			event.setPriRgGrpCmdtVO((PriRgGrpCmdtVO) getVO(request,
					PriRgGrpCmdtVO.class));
		} else {
			event.setPriRgGrpCmdtVO((PriRgGrpCmdtVO)getVO(request, PriRgGrpCmdtVO .class));
		}

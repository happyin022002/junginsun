...
*@FileName : ESM_MAS_0045HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.esm.mas.weeklypfmc.networkdistribution 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @author Kim Ki Dae
 * @see EsmMas0045Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0045HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0045HTMLAction 객체를 생성
	 */
	public ESM_MAS_0045HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0045Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0045Event event = new EsmMas0045Event();  		
		...
		else if (command.isCommand(FormCommand.COMMAND01)) { // 저장
			event.setSearchHJSSalesAmountListVOs((SearchHJSSalesAmountListVO[])getVOs(request, SearchHJSSalesAmountListVO.class));
			event.setNetworkDistributionCommonVO((NetworkDistributionCommonVO)getVO(request, NetworkDistributionCommonVO.class));
		}
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));

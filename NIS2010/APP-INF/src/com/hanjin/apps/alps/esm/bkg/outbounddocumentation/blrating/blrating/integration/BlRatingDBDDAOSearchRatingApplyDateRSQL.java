...
public class BlRatingDBDDAOSearchRatingApplyDateRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		CASE WHEN BKG_REV_APLY_DT_PKG.BKG_GET_APLY_DT_CHK_FNC(@[bkg_no], @[ca_flg]) = 'CRD' THEN SUBSTR(MAX(A.CGO_RCV_DT) OVER(PARTITION BY A.BKG_NO),1,10)" ).append("\n"); 
		query.append("		     ELSE SUBSTR(A.ETD,1,10)" ).append("\n"); 
		query.append("		END APPLY_DT," ).append("\n"); 
		query.append("		--CASE WHEN A.CONTI_CD = 'E' AND SUBSTR(A.DEL_CD,1,2) = 'CA' THEN SUBSTR(A.ETD,1,10)" ).append("\n"); 
		query.append("		--	 WHEN     SUBSTR(A.POR_CD,1,2) = 'US' OR SUBSTR(A.POR_CD,1,2) = 'MX' OR SUBSTR(A.POR_CD,1,2) = 'CA' " ).append("\n"); 
		query.append("	 	--	  	   OR SUBSTR(A.POL_CD,1,2) = 'US' OR SUBSTR(A.POL_CD,1,2) = 'MX' OR SUBSTR(A.POL_CD,1,2) = 'CA' " ).append("\n"); 
		query.append("		--		   OR SUBSTR(A.POD_CD,1,2) = 'US' OR SUBSTR(A.POD_CD,1,2) = 'MX' OR SUBSTR(A.POD_CD,1,2) = 'CA' " ).append("\n"); 
		query.append("		--		   OR SUBSTR(A.DEL_CD,1,2) = 'US' OR SUBSTR(A.DEL_CD,1,2) = 'MX' OR SUBSTR(A.DEL_CD,1,2) = 'CA' " ).append("\n"); 
		query.append("		--				THEN SUBSTR(MAX(A.CGO_RCV_DT) OVER(PARTITION BY A.BKG_NO),1,10)" ).append("\n"); 
		query.append("		--    ELSE SUBSTR(A.ETD,1,10)" ).append("\n"); 
		query.append("		--     END APPLY_DT," ).append("\n"); 
		...
		query.append("		  A.CGO_RCV_DT AS OC_DATE" ).append("\n"); 

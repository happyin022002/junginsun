...
public class CargoReleaseOrderDBDAOsearchCaCgoMkFileRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       " ).append("\n"); 
		query.append("       CASE WHEN EXISTS (SELECT ATTR_CTNT2 FROM BKG_CSTMS_CD_CONV_CTNT WHERE CNT_CD='CA' AND CSTMS_DIV_ID='US_CR_SNP_STUP' AND ATTR_CTNT1 = @[yd_cd] AND DELT_FLG = 'N')" ).append("\n"); 
		...
		query.append("       'MSGID : '             || @[edi_knd]                         || CHR(10)" ).append("\n"); 
		query.append("       FROM BKG_CSTMS_CD_CONV_CTNT" ).append("\n"); 
		query.append("       WHERE CNT_CD='CA'" ).append("\n"); 
		query.append("       AND CSTMS_DIV_ID='US_CR_SNP_STUP'" ).append("\n"); 
		query.append("       AND ATTR_CTNT1 = @[yd_cd]" ).append("\n"); 
		...
		query.append("       '322ID : '             || DECODE(CG.CSTMS_CLR_LST_DT,NULL,'',DECODE(@[new_cstms_clr_cd],'P','Y',@[new_cstms_clr_cd])) || CHR(10) ||" ).append("\n"); 
		query.append("       'MSGID : '             || @[edi_knd]                          || CHR(10) SNP_FILE_HEADER,  /* 추가 Inalnd 에 보낼때 사용 */" ).append("\n"); 
		query.append("       " ).append("\n"); 
		query.append("       'VIP_GRP_ID : '                                               || chr(10) ||" ).append("\n"); 
		query.append("	   'BL NBR : '            || @[bl_no]		                     || chr(10) ||" ).append("\n"); 
		query.append("	   'BKG NBR : '           || B.BKG_NO                            || chr(10) ||" ).append("\n"); 
		query.append("	   'TO VSL CODE : '       || VVD.VSL_CD                            || chr(10) ||" ).append("\n"); 
		query.append("	   'TO VOYAGE : '         || VVD.SKD_VOY_NO                        || chr(10) ||" ).append("\n"); 
		query.append("	   'TO DIR : '            || VVD.SKD_DIR_CD                        || chr(10) ||" ).append("\n"); 
		...
		query.append("       'POR AMSQUAL : '       || DECODE(LA.CNT_CD, 'CA', 'D', 'K')   || chr(10) ||" ).append("\n"); 
		...
		query.append("       'POL AMSQUAL : '       || DECODE(LB.CNT_CD, 'CA', 'D', 'K')   || chr(10) ||" ).append("\n"); 
		...
		query.append("       'POD AMSQUAL : '       || decode(LC.CNT_CD, 'CA', 'D', 'K')   || chr(10) ||" ).append("\n"); 
		...
		query.append("       'DEL AMSQUAL : '       || decode(LD.CNT_CD, 'CA', 'D', 'K')   || chr(10) ||" ).append("\n"); 

 ...
 * @see EsmAgt0059Event , ESM_AGT_0059EventResponse 참조
 * @since J2EE 1.4
 */
public class ESM_AGT_0059HTMLAction extends HTMLActionSupport {


	private static final long serialVersionUID = 1L;
	/**
	 * ESMAGT0059HTMLAction 객체를 생성
	 */
	public ESM_AGT_0059HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmAgt0059Event event = new EsmAgt0059Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setCmpnCommVO((CmpnCommVO)getVO(request, CmpnCommVO .class));
		}
		
		if(command.isCommand(FormCommand.MULTI)){
			event.setCmpnCommVOS((CmpnCommVO[])getVOs(request, CmpnCommVO .class, ""));
			event.setCmpnCommVO((CmpnCommVO)getVO(request, CmpnCommVO .class));
		}

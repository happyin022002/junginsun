...
*@FileName : RFARateProposalDBDAOPriRpScpRtRoutPntAmdBasicVOCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateProposalDBDAOPriRpScpRtRoutPntAmdBasicVOCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Master RFA에서 가져온 Rate Route Point 데이터를 Basic Rate에 넣는다.
	  * Basic이 가지고 있는 Route만 수행한다.
	  * </pre>
	  */
	public RFARateProposalDBDAOPriRpScpRtRoutPntAmdBasicVOCSQL(){
		setQuery();
		...
		query.append("FileName : RFARateProposalDBDAOPriRpScpRtRoutPntAmdBasicVOCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    B.FIC_ROUT_CMB_TP_CD  ," ).append("\n"); 
		query.append("    B.BSE_PORT_LOC_CD" ).append("\n"); 
		query.append("  FROM MASTER_RFA_INFO MST_INFO," ).append("\n"); 
		query.append("       PRI_RP_SCP_RT_ROUT_PNT M,-- Master" ).append("\n"); 
		query.append("       BASIC_RFA_INFO BSC_INFO," ).append("\n"); 
		query.append("       PRI_RP_SCP_RT_ROUT_PNT B -- Basic" ).append("\n"); 
		...
		query.append("   AND B.ORG_DEST_TP_CD = M.ORG_DEST_TP_CD" ).append("\n"); 
		query.append("   AND B.ROUT_PNT_SEQ = M.ROUT_PNT_SEQ" ).append("\n"); 
		query.append("   AND EXISTS (" ).append("\n"); 
		query.append("       SELECT 'OK'" ).append("\n"); 
		query.append("       FROM    " ).append("\n"); 
		query.append("           PRI_RP_SCP_RT" ).append("\n"); 
		query.append("       WHERE   " ).append("\n"); 
		query.append("           PROP_NO             = B.PROP_NO" ).append("\n"); 
		query.append("       AND AMDT_SEQ            = B.AMDT_SEQ" ).append("\n"); 
		query.append("       AND SVC_SCP_CD          = B.SVC_SCP_CD" ).append("\n"); 
		query.append("       AND CMDT_HDR_SEQ        = B.CMDT_HDR_SEQ" ).append("\n"); 
		query.append("       AND ROUT_SEQ            = B.ROUT_SEQ " ).append("\n"); 

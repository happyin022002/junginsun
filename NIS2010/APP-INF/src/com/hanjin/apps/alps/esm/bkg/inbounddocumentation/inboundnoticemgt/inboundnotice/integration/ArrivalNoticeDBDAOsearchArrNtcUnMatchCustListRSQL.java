...
public class ArrivalNoticeDBDAOsearchArrNtcUnMatchCustListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("/* Query 복잡도를 줄이기 위해 With 사용 Global Temporary 사용안함 - Comment Park Mangeon */" ).append("\n"); 
		query.append("WITH SUBQ AS (" ).append("\n"); 
		query.append("    SELECT /*+ USE_NL(BKGM BCST CVAL CNTC MLOC ECTT)*/" ).append("\n"); 
		query.append("           BKGM.BKG_NO" ).append("\n"); 
		query.append("         , '$dog' AS DOG" ).append("\n"); 
		...
		query.append("		 , CASE WHEN SUBSTR(BKGM.POD_CD,1,2)  = 'US' " ).append("\n"); 
		query.append("			    THEN ROW_NUMBER () OVER (PARTITION BY BCST.BKG_NO, BCST.BKG_CUST_TP_CD --BKG별, CONSIGNEE,NOTIFY별 추천" ).append("\n"); 
		query.append("                                           ORDER BY DECODE(CVAL.CUST_CNT_CD, SUBSTR(BKGM.DEL_CD,1,2), 0, 1) -- 10자리로 찾은 것 중 " ).append("\n"); 
		query.append("	                                      			/* 고객 주소 5자리 비교 - 특수문자,AVENUE|DRIVE|STREET|BOULEVARD 제거 */" ).append("\n"); 
		query.append("    				                                ,DECODE(SUBSTR(REGEXP_REPLACE(REGEXP_REPLACE(UPPER(TO_SINGLE_BYTE(CVAL.VAL_CUST_ADDR)),'AVENUE|DRIVE|STREET|BOULEVARD',''),'[^a-zA-Z]',''),1,5)" ).append("\n"); 
		query.append("                                             		       ,SUBSTR(REGEXP_REPLACE(REGEXP_REPLACE(UPPER(TO_SINGLE_BYTE(BCST.CUST_ADDR))    ,'AVENUE|DRIVE|STREET|BOULEVARD',''),'[^a-zA-Z]',''),1,5)" ).append("\n"); 
		query.append("                                             		       ,0,1)" ).append("\n"); 
		query.append("                                      				,DECODE(SUBSTR(REGEXP_REPLACE(REGEXP_REPLACE(UPPER(TO_SINGLE_BYTE(CVAL.VAL_CUST_ADDR)),'AVENUE|DRIVE|STREET|BOULEVARD',''),'[^0-9]',''),1,3)" ).append("\n"); 
		query.append("                                                           ,SUBSTR(REGEXP_REPLACE(REGEXP_REPLACE(UPPER(TO_SINGLE_BYTE(BCST.CUST_ADDR))    ,'AVENUE|DRIVE|STREET|BOULEVARD',''),'[^0-9]',''),1,3)" ).append("\n"); 
		query.append("                                                           ,0,1)" ).append("\n"); 
		query.append("													/* 추천 코드와 BKG CUSTOMER CONSIGNEE,NOTIFY 가 같으면 우선 함 */" ).append("\n"); 
		query.append("                                                    ,DECODE(CVAL.CUST_CNT_CD||CVAL.CUST_SEQ,BCST.CUST_CNT_CD||BCST.CUST_SEQ,0,1)" ).append("\n"); 
		query.append("												    ,CVAL.ROWID --모든 조건이 같다면 ROWID로 일관성있는 결과를 리턴한다." ).append("\n"); 
		query.append("                                        	      /* " ).append("\n"); 
		query.append("                                		            BKG_NO로 검색할 경우와 기타 VVD등으로 검색할 경우, 동일한 BKG_NO의 추천코드가 달라 질 수있다." ).append("\n"); 
		query.append("        						                                        내부적으로 BKG별로 재 소팅을 하게 되어, 여러 BKG가 있을 경우 단일 BKG로 조회 했을 경우와 추천 코드가 달라 질 수 있다." ).append("\n"); 
		query.append(" 	                                              */" ).append("\n"); 
		query.append(" " ).append("\n"); 
		query.append("                                          ) " ).append("\n"); 
		query.append("                ELSE ROW_NUMBER () OVER (PARTITION BY BCST.BKG_NO, BCST.BKG_CUST_TP_CD " ).append("\n"); 
		query.append("                                         ORDER BY   DECODE(CNTC.ROWID, NULL, 1, 0) -- 2015.08.04(기존 ROWID로 소팅되어 하위에서 더 나은 값이 있어도 우선 순위에서 밀려 바로 잡음) inbound contact person에  정보가 있는 것이 우선으로 처리된다." ).append("\n"); 
		query.append("                                                  , DECODE(CVAL.CUST_CNT_CD, SUBSTR(BKGM.DEL_CD,1,2), 1, BCST.CUST_CNT_CD, 2, 3) -- DEL EQ OFC가 BKG_IB_CUST_CNTC에 등록된 고객 중 국가 코드가 DEL > BCST 국가코드 같은 순" ).append("\n"); 
		query.append("                                                  , DECODE(ECTT.OFC_CD, NULL, 1, 0)  -- DEL EQ OFC가 BKG_IB_CUST_CNTC에 등록된 고객 " ).append("\n"); 
		query.append("                                                  , DECODE(MLOC.LOC_CD, NULL, 1, 0)  -- DEL_CD가 MDM에 존재(크게 의미 없는듯함)" ).append("\n"); 
		query.append("													/* 추천 코드와 BKG CUSTOMER CONSIGNEE,NOTIFY 가 같으면 우선 함 */" ).append("\n"); 
		query.append("                                                  , DECODE(CVAL.CUST_CNT_CD||CVAL.CUST_SEQ,BCST.CUST_CNT_CD||BCST.CUST_SEQ,0,1)" ).append("\n"); 
		query.append("												  , CNTC.ROWID --모든 조건이 같다면 ROWID로 일관성있는 결과를 리턴한다." ).append("\n"); 
		query.append("                                        	      /* " ).append("\n"); 
		query.append("                                		            BKG_NO로 검색할 경우와 기타 VVD등으로 검색할 경우, 동일한 BKG_NO의 추천코드가 달라 질 수있다." ).append("\n"); 
		query.append("        						                                        내부적으로 BKG별로 재 소팅을 하게 되어, 여러 BKG가 있을 경우 단일 BKG로 조회 했을 경우와 추천 코드가 달라 질 수 있다." ).append("\n"); 
		query.append(" 	                                              */" ).append("\n"); 
		query.append("                              ) " ).append("\n"); 
		query.append("			END AS ODR" ).append("\n"); 
		...
		query.append("         , BKGM.PORT_CLZ_DT" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		/* 2015.05.27  SC_NO,CTRT_CUST_CNT_CD,CTRT_CUST_SEQ 추가*/" ).append("\n"); 
		query.append("         , NVL(BKGM.SC_NO,BKGM.RFA_NO) AS SC_NO " ).append("\n"); 
		query.append("         , BKGM.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("         , BKGM.CTRT_CUST_SEQ" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                  OR BCST.VAL_CD = 'X' -- Auto-Cancel된 것은 Un-Match Customer List에 조회되어야 한다. 20100201" ).append("\n"); 
		query.append("                 )" ).append("\n"); 
		query.append("             AND TRIM(REPLACE(REPLACE(BCST.CUST_NM, CHR(10), ''), CHR(13), '')) IS NOT NULL -- 고객명 없으면 처리안함 (20090716)" ).append("\n"); 
		query.append("            )" ).append("\n"); 
		query.append("         LEFT OUTER JOIN BKG_CUST_CD_VAL CVAL" ).append("\n"); 
		query.append("         ON ( " ).append("\n"); 
		query.append("                --CVAL.VAL_CUST_NM = BCST.VAL_NM  -- NAME MATCH" ).append("\n"); 
		query.append("                -- 2015.08.31 US의 경우 이름 10자리 비교 주소 10자리 비교" ).append("\n"); 
		query.append("                CVAL.VAL_CUST_NM LIKE DECODE(SUBSTR(BKGM.DEL_CD,1,2), 'US',SUBSTR(BCST.VAL_NM,1,10),BCST.VAL_NM)||'%'" ).append("\n"); 
		query.append("             AND BCST.VAL_NM IS NOT NULL /* NULL 인것 제외 LIKE 검색 전환으로 전체가 나옴 */" ).append("\n"); 
		...
		query.append("         /* US의 경우 고객명 , 고객 주소만으로 우선순위를 결정함 - 성능을 고려해 CNTC NULL 로 만듬 OLD DECODE(SUBSTR(BKGM.POD_CD,1,2),'CA','TORSC' ,'PHXSA') */" ).append("\n"); 
		query.append("         ON (CNTC.OFC_CD = DECODE(SUBSTR(BKGM.DEL_CD,1,2), 'US','XXXXX',@[ofc_cd]) -- LOGIN OFFICE  " ).append("\n"); 
		...
		query.append("         /* US의 경우 고객명 , 고객 주소만으로 우선순위를 결정함 - 성능을 고려해 ECTT NULL */" ).append("\n"); 
		query.append("         ON (ECTT.OFC_CD = DECODE(SUBSTR(BKGM.DEL_CD,1,2), 'US','XXXXX', MLOC.EQ_CTRL_OFC_CD )  " ).append("\n"); 
		...
		query.append("       AND BVVD.POD_CD     IN (${pod_cd}) -- (OPTIONAL 3) " ).append("\n"); 
		...
		query.append("    SELECT /*+ USE_NL(MCST MADR VCST)*/" ).append("\n"); 
		query.append("           DENSE_RANK() OVER (ORDER BY SUBQ.VAL_NM" ).append("\n"); 
		...
		query.append("         , SUBQ.POL_CD" ).append("\n"); 
		query.append("         , SUBQ.POD_CD" ).append("\n"); 
		query.append("         , SUBQ.DEL_CD" ).append("\n"); 
		query.append("         , SUBQ.BKG_OFC_CD" ).append("\n"); 
		query.append("         , SUBQ.PORT_CLZ_DT" ).append("\n"); 
		query.append("         , NVL(( SELECT BDR_FLG  FROM BKG_BL_DOC DOC WHERE DOC.BKG_NO = SUBQ.BKG_NO), 'N') AS BDR_FLG" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		/* 2015.05.27  SC_NO,CTRT_CUST_CNT_CD,CTRT_CUST_SEQ 추가*/" ).append("\n"); 
		query.append("         , SC_NO " ).append("\n"); 
		query.append("         , SUBQ.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("         , SUBQ.CTRT_CUST_SEQ     " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("            SELECT DECODE(SUBSTR(O.LOC_CD,1,2),'US',DECODE(SUBSTR(RSLT.DEL_CD,1,2)||SUBSTR(RSLT.POD_CD,1,2),'USCA','TORSC' ,'PHXSA'),O.OFC_CD)" ).append("\n"); 
		query.append("            FROM MDM_ORGANIZATION O" ).append("\n"); 
		query.append("            WHERE O.OFC_CD = RSLT.BKG_OFC_CD" ).append("\n"); 
		query.append("            )" ).append("\n"); 
		query.append("          ,RSLT.BKG_OFC_CD)  AS BKG_OFC_CD" ).append("\n"); 
		query.append("	 , CASE WHEN     COR_CUST_CD IS NOT NULL AND LENGTH(COR_CUST_CD) > 2 " ).append("\n"); 
		query.append("                 AND COR_CUST_CD = BKG_CUST_CNT_CD||LPAD(DECODE(BKG_CUST_SEQ, 0, NULL,BKG_CUST_SEQ) , 6, '0')" ).append("\n"); 
		query.append("                 /* 유럽 지역 불편으로 롤백 요청 2014.10.21  AND TRUNC(PORT_CLZ_DT  + 30 ) +1 >= SYSDATE */" ).append("\n"); 
		query.append("            THEN 'O'" ).append("\n"); 
		query.append("       END AS VAL_CD  " ).append("\n"); 
		query.append("     , BDR_FLG" ).append("\n"); 
		query.append("     , TO_CHAR(PORT_CLZ_DT,'YYYY-MM-DD') AS PORT_CLZ_DT" ).append("\n"); 
		query.append("     /* 2014.10.10 PCT로 부터 한달이 지난 건을 Evaluation 하려고 하는 경우 */" ).append("\n"); 
		query.append("     , CASE WHEN TRUNC(PORT_CLZ_DT  + 30 ) +1 < SYSDATE  THEN 'N'" ).append("\n"); 
		query.append("       ELSE 'Y'" ).append("\n"); 
		query.append("       END AS VAL_AVAL_FLG    " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    /* 2015.05.27  SC_NO,CTRT_CUST_CNT_CD,CTRT_CUST_SEQ 추가*/" ).append("\n"); 
		query.append("      , RSLT.SC_NO " ).append("\n"); 
		query.append("      , DECODE(RSLT.SC_NO ,NULL,'',NVL(RSLT.CTRT_CUST_CNT_CD,'00')||TRIM(TO_CHAR(RSLT.CTRT_CUST_SEQ,'000000'))) CTRT_CUST_CD" ).append("\n"); 
		query.append("      , DECODE(RSLT.SC_NO ,NULL,'',( SELECT CTRT.CUST_LGL_ENG_NM" ).append("\n"); 
		query.append("                                       FROM MDM_CUSTOMER CTRT" ).append("\n"); 
		query.append("                                      WHERE CTRT.CUST_CNT_CD = RSLT.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("                                        AND CTRT.CUST_SEQ    = RSLT.CTRT_CUST_SEQ " ).append("\n"); 
		query.append("                                    )" ).append("\n"); 

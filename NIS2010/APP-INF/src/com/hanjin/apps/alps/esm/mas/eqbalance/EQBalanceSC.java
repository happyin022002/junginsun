...
=========================================================*/
package com.hanjin.apps.alps.esm.mas.eqbalance;
...
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.event.EsmMas0016Event;
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.event.EsmMas0018Event;
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.event.EsmMas0019Event;
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.event.EsmMas0136Event;
import com.hanjin.apps.alps.esm.mas.eqbalance.eqbalance.event.EsmMas0161Event;
...
public class EQBalanceSC extends ServiceCommandSupport {
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if (e.getEventName().equalsIgnoreCase("EsmMas0016Event")) {
		...
		if (e.getEventName().equalsIgnoreCase("EsmMas0018Event")) {
		...
		if (e.getEventName().equalsIgnoreCase("EsmMas0019Event")) {
			if (e.getFormCommand().isCommand(FormCommand.SEARCH)) {
		...
		if (e.getEventName().equalsIgnoreCase("EsmMas0136Event")) {
			if (e.getFormCommand().isCommand(FormCommand.SEARCH)) {
		...
		if (e.getEventName().equalsIgnoreCase("EsmMas0161Event")) {
			if (e.getFormCommand().isCommand(FormCommand.SEARCH)) {
	...
	private EventResponse searchEQBalance00162List(Event e) throws EventException {
		...
		EsmMas0016Event event = (EsmMas0016Event)e;
	...
	private EventResponse multiEQBalance00163(Event e) throws EventException {
		...
		try{
			...
			command.multiEQBalance00163(event.getMasCntrRepoShtgInfoVOS(),event.getSearchConditionVO(),account, excel_load_yn);
	...
	private EventResponse searchEQBalance0018List(Event e) throws EventException {
		// PDTO(Data Transfer Object including Parameters)
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmMas0018Event event = (EsmMas0018Event)e;
		EQBalanceBC command = new EQBalanceBCImpl();
	...
	private EventResponse multiEQBalance0018(Event e) throws EventException {
		...
		try{
			begin();
			command.multiEQBalance0018(event.getMasCntrRepoCrVOS(),account);
			eventResponse.setUserMessage(new ErrorHandler("XXXXXXXXX").getUserMessage());
			commit();
		} catch(EventException ex) {
			rollback();
	...
	private EventResponse searchEQBalance0019List (Event e) throws EventException {
		// PDTO(Data Transfer Object including Parameters)
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmMas0019Event event = (EsmMas0019Event)e;
		EQBalanceBC command = new EQBalanceBCImpl();
	...
	private EventResponse searchEQBalance0136List(Event e) throws EventException {
		// PDTO(Data Transfer Object including Parameters)
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmMas0136Event event = (EsmMas0136Event)e;
		EQBalanceBC command = new EQBalanceBCImpl();
	...
	private EventResponse multiEQBalance0136(Event e) throws EventException {
		...
		try{
			begin();
			command.multiEQBalance0136(event.getMasCntrRepoRoutEccVOS()
	...
	private EventResponse searchEQBalance0161List(Event e) throws EventException {
		// PDTO(Data Transfer Object including Parameters)
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmMas0161Event event = (EsmMas0161Event)e;
		EQBalanceBC command = new EQBalanceBCImpl();
	...
	private EventResponse multiEQBalance0161(Event e) throws EventException {
		...
		try{
			begin();
			command.multiEQBalance0161(event.getMasCntrRepoRoutEccVOS()
					                  ,event.getSearchConditionVO(),account);
			eventResponse.setUserMessage(new ErrorHandler("XXXXXXXXX").getUserMessage());
			commit();
		} catch(EventException ex) {
	...
	/**
	 * ESM_MAS_0016 : 공통코드 조회 이벤트 처리<br>
	 * 
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException
	...
	/**
	 * ESM_MAS_0016 : MAS_CNTR_REPO_SHTG_INFO 테이블의 키중복 check  합니다<br>
	 * 
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException
	...
	/**
	 * ESM_MAS_0019 : 공통코드 조회 이벤트 처리<br>
	 * 
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException

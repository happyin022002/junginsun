...
*@FileName : ESM_FMS_0079HTMLAction.java
 ...
 * @author JUNGYOONTAE
 * @see EsmFms0079Event 참조
 * @since J2EE 1.5 
 */

public class ESM_FMS_0079HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0079HTMLAction 객체를 생성
	 */
	public ESM_FMS_0079HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0079Event event = new EsmFms0079Event();
		...
		if(request.getContentType() != null && request.getContentType().toLowerCase().startsWith(SubSystemConfigFactory.get("COM.CONTENTS.TYPE.MULTIPART").trim())){
			try {
				...
				//FileUpload fileUpload = new FileUpload(request,SubSystemConfigFactory.get("COM.MODULE.ID"), SubSystemConfigFactory.get("COM.ENCODING.EUCKR"), event);
		...
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomSendEmailVO((CustomSendEmailVO)getVO(request, CustomSendEmailVO.class));
			event.setMailFlg(request.getParameter("mailFlg"));
			event.setCsrNo(request.getParameter("csrNo"));

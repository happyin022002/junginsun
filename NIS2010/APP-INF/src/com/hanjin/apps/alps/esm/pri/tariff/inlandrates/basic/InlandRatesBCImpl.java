...
import com.hanjin.apps.alps.esm.pri.tariff.inlandrates.integration.InlandRatesDBDAO;
...
public class InlandRatesBCImpl extends BasicCommandSupport implements InlandRatesBC {
	...
	/**
	 * InlandRatesBCImpl 객체 생성<br>
	 * InlandRatesDBDAO를 생성한다.<br>
	 */
	...
	public void manageInlandRates(PriTrfInlndListVO priTrfInlndListVO, SignOnUserAccount account) throws EventException{
		try {
			...
			if(paramVo.getExcelFlg().equals("Y")) {
				...
				// amend 된 데이터를 amend delete 처리한다.
			...
			for ( int i=0; i<priTrfInlndRtVO.length; i++ ) {
				...
				cdVO = dbDao.searchLocationCodeExist(vo);
             	if (cdVO == null) {
             		...
             		cdVO.setCd(vo.getCd());					// 코드값
             		cdVO.setNm("Loc. Code");				// 컬럼명(타이틀)
             		cdVO.setEtc1(String.valueOf(i));		// Row index
             		cdVO.setEtc2("inlnd_rt_bse_loc_cd");	// 컬럼명
             		cdVO.setEtc3(priTrfInlndRtVO[i].getTrfInlndRtSeq());	// Seq.
             		cdVO.setEtc4("PRI00315");				// 메세지 코드
             		cdVO.setEtc5("3");						// 컬럼 index
             		rslt.add(cdVO);
             	}
				//VIA
        		...
        		//One Way & Round Trip 이 모두 null 일 수 없다.
        		//엑셀업로드파일에 null 값이 존재하기때문에 해제
             	/*

...
*@FileName : SpacecontrolInquiryDBDAO021FcstPfmcAcctViewListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SpacecontrolInquiryDBDAO021FcstPfmcAcctViewListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 2014.11.05 [CHM-201432345] SMP Report 신규 생성(요건변경) -SC/RFA중 하나라도 있으면 실적 인지하도록처리 -TTLCM, CMPB, LOAD%(Guide대비)추가
	  * -Model에 존재하는 데이터만 가져오도록 변경 -Loading office hidden을 위해 SC/RFA grouping추가 -SUB Trade별 전체 실적 GUIDE추가
	  ...
	  * </pre>
	  */
	public SpacecontrolInquiryDBDAO021FcstPfmcAcctViewListRSQL(){
		setQuery();
		...
		query.append("FileName : SpacecontrolInquiryDBDAO021FcstPfmcAcctViewListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            SELECT DISTINCT  --20130424 DISTINCT 추가" ).append("\n"); 
		...
		query.append("               AND V.TRD_CD    = NVL(M.TRD_CD, V.TRD_CD)" ).append("\n"); 
		query.append("#if(${grp_acct} != '')" ).append("\n"); 
		query.append("               AND M.CUST_GRP_ID IN (${grp_acct})" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if(${acct} != '')" ).append("\n"); 
		query.append("               AND M.CUST_CNT_CD||TO_CHAR(M.CUST_SEQ, 'FM000000') IN (${acct})" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("             UNION ALL --전체" ).append("\n"); 
		...
		query.append("       --AND DECODE(M.TRD_CD, 'AES', NVL(M.RFA_NO, 'X'), NVL(M.SC_NO, 'X')) = DECODE(M.TRD_CD, 'AES', NVL(S.RFA_NO, 'X'), NVL(S.SC_NO, 'X'))" ).append("\n"); 
		query.append("       AND M.DELT_FLG    = 'N'" ).append("\n"); 
		...
		query.append("                             START WITH (V.RGN_OFC_CD IS NULL AND (O.PRNT_OFC_CD = V.AQ_CD OR O.PRNT_OFC_CD = V.RHQ_CD)) OR (O.OFC_CD = V.RGN_OFC_CD)))" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("               --AND DECODE(F.TRD_CD, 'AES', NVL(F.RFA_NO, 'X'), NVL(F.SC_NO, 'X')) = DECODE(F.TRD_CD, 'AES', NVL(S.RFA_NO, 'X'), NVL(S.SC_NO, NVL(F.SC_NO, 'X')))" ).append("\n"); 
		query.append("--               AND A.COST_YR         = S.COST_YR" ).append("\n"); 
		query.append("--               AND A.COST_WK         = S.COST_WK" ).append("\n"); 
		query.append("        )        " ).append("\n"); 
		...
		query.append("                         WHERE M.TRD_CD    IN ('AES', 'IAS') --2015.03.09 [CHM-201534504]SMP IAS 보완에 따른 FCST 추가 개발 요청" ).append("\n"); 
		...
		query.append("    --           NVL(C.CUST_CNT_CD, 'XX') AS MDL_CUST_CNT," ).append("\n"); 
		query.append("    --           NVL(C.CUST_SEQ, 999999 ) AS MDL_CUST_SEQ," ).append("\n"); 
		query.append("    --           NVL(C.CUST_CTRL_CD, 'C') AS MDL_CUST_CTRL_CD," ).append("\n"); 
		...
		query.append("					--SUM(NVL(CM.BKG_REV, 0) + NVL(CM.BKG_OFT_REV, 0) + NVL(CM.BKG_MISC_REV, 0) + NVL(CM.SCR_CHG_REV, 0) - NVL(CM.ESTM_CM_COST_AMT, 0))" ).append("\n"); 
		...
		query.append("               AND B.BKG_NO            = BC.BKG_NO  --20140304 추가" ).append("\n"); 
		query.append("               AND BC.BKG_CUST_TP_CD   = 'S'        --20140304 추가     " ).append("\n"); 
		query.append("               AND A.TRD_CD            IN ('AES', 'IAS') --2015.03.09 [CHM-201534504]SMP IAS 보완에 따른 FCST 추가 개발 요청" ).append("\n"); 
		...
		query.append("               --20141105 S/C와 RFA중 하나라도 맞으면 실적 인지하도록 " ).append("\n"); 
		...
		query.append("		   --20141105추가" ).append("\n"); 
		query.append("           SUM(MAS_QTY) MAS_QTY," ).append("\n"); 
		query.append("           SUM(MAS_CM) MAS_CM" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		...
		query.append("                             --AND DECODE(F.TRD_CD, 'AES', NVL(RFA_NO,'X'), NVL(SC_NO,'X')) = DECODE(F.TRD_CD, 'AES', NVL(F.RFA_NO,'X'),NVL(F.SC_NO,NVL(SC_NO,'X')))" ).append("\n"); 
		...
		query.append("           END       " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("--       AND NVL(B.SC_NO, NVL(B.RFA_NO,'X')) = NVL(SPC_GET_SMP_AMEND_FNC(B.TRD_CD, SUBSTR(B.SEASON, 1, 6), SUBSTR(B.SEASON, 8), A.SC_NO), NVL(SPC_GET_SMP_AMEND_FNC(B.TRD_CD, SUBSTR(B.SEASON, 1, 6), SUBSTR(B.SEASON, 8), A.RFA_NO),'X'))" ).append("\n"); 
		query.append("       --AND DECODE(B.TRD_CD, 'AES', NVL(B.RFA_NO,'X'), NVL(B.SC_NO,'X')) = DECODE(B.TRD_CD, 'AES', NVL(SPC_GET_SMP_AMEND_FNC(B.TRD_CD, SUBSTR(B.SEASON, 1, 6), SUBSTR(B.SEASON, 8), A.RFA_NO),'X'), NVL(SPC_GET_SMP_AMEND_FNC(B.TRD_CD, SUBSTR(B.SEASON, 1, 6), SUBSTR(B.SEASON, 8), A.SC_NO),NVL(B.SC_NO,'X')))" ).append("\n"); 
		...
		query.append("           AND MQR.SQM_VER_STS_CD  = 'R' -- 20131205 변경" ).append("\n"); 
		...
		query.append("                         --OR ( TRD_CD IS NOT NULL AND CUST_CLSS IS NOT NULL AND CUST_GRP_ID IS NULL AND MDL_CUST_CNT IS NULL AND MDL_CUST_SEQ IS NULL )" ).append("\n"); 
		...
		query.append("         OTH_CUST_NM ," ).append("\n"); 
		query.append("         SC_NO       ," ).append("\n"); 
		query.append("         DECODE(TRD_CD, 'AES',NVL(SC_NO,RFA_NO), 'IAS',NVL(SC_NO,RFA_NO),RFA_NO), --2015.03.09 [CHM-201534504]SMP IAS 보완에 따른 FCST 추가 개발 요청" ).append("\n"); 
		query.append("         --RFA_NO      ," ).append("\n"); 
		query.append("         CTRT_OFC_CD ," ).append("\n"); 
		query.append("         LOAD_OFC_CD ," ).append("\n"); 
		query.append("         SUB_TRD_CD  ," ).append("\n"); 
		...
		query.append("         --DECODE(CUST_CNT_CD, NULL, DECODE(LOAD_OFC_CD, NULL, 'Z', 'A'), CUST_CNT_CD), --(rfa레벨 그룹 추가에 따라)주석처리" ).append("\n"); 
		...
		query.append("         --DECODE(CUST_GRP_ID, 'XX999999', 'Z', DECODE(TRD_CD, 'AES', NVL(RFA_NO, 'Z'), NVL(SC_NO, 'Z')))," ).append("\n"); 
		query.append("         NVL(CTRT_OFC_CD, '0'), --ZZZ->'0'(rfa레벨 그룹 추가에 따라)" ).append("\n"); 
		query.append("         NVL(OFC_CD, '0'), --ZZZ->'0'(rfa레벨 그룹 추가에 따라)" ).append("\n"); 
		query.append("         SUB_TRD_CD," ).append("\n"); 
		query.append("         RLANE_CD" ).append("\n"); 
		query.append("         " ).append("\n"); 
		...
		query.append("                                 )" ).append("\n"); 
		query.append("            HAVING NOT (    ( NVL(MDL_CUST_CNT, 'XX') = NVL(CUST_CNT_CD, 'X') AND NVL(MDL_CUST_SEQ, 999999) = NVL(CUST_SEQ, 1) AND LOAD_OFC_CD IS NOT NULL)" ).append("\n"); 
		query.append("                         --OR ( TRD_CD IS NOT NULL AND CUST_CLSS IS NOT NULL AND CUST_GRP_ID IS NULL AND MDL_CUST_CNT IS NULL AND MDL_CUST_SEQ IS NULL ) --CUST_CLSS도 빼야는데" ).append("\n"); 
		query.append("                       )" ).append("\n"); 
		query.append("         ) T" ).append("\n"); 
		query.append("GROUP BY TRD_CD      ," ).append("\n"); 
		query.append("         LOAD_OFC_CD ," ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("/*" ).append("\n"); 
		query.append("2012.12.04 최윤성 [CHM-201221640-01]  FCST&PFMC by ACCT 신규 탭 추가" ).append("\n"); 
		query.append("2013.01.23 [CHM-201322502-01] SPC 프로젝트 - 성수기 선복운영 개선을 위한 T/F추진" ).append("\n"); 
		query.append("2013.02.22 김시몬[CHM-201323235-01]" ).append("\n"); 
		query.append("제목 : WAX 노선 Dest. 반영 건" ).append("\n"); 
		query.append("작업 내용 : WAX 노선은 Dest. 를 AU 로 집계되도록 보완, WAXIA 의 경우 Direction 변경(E=> W, W=> E)" ).append("\n"); 
		query.append("2013.04.28 김시몬[CHM-201324211-01] SPC_SLS_REP_CUST, SPC_MDL_CUST_CTRL에 SUB_TRD_CD추가에 따른 보완" ).append("\n"); 
		query.append("2013.06.25 진마리아 [선처리] CRM Customer Flag 정보 개편에 따른 작업" ).append("\n"); 
		query.append("2013.08.02 진마리아 [CHM-201325983-01] Yield Group 추가, timeout 오류 수정" ).append("\n"); 
		query.append("2013.08.23 진마리아 [Trouble shooting] CC,RC,LC별 grouping을 Yield Group별 grouping으로 변경" ).append("\n"); 

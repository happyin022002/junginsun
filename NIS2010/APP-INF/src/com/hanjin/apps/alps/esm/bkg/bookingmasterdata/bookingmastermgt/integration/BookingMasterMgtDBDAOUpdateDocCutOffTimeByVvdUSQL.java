...
*@FileName : BookingMasterMgtDBDAOUpdateDocCutOffTimeByVvdUSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BookingMasterMgtDBDAOUpdateDocCutOffTimeByVvdUSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Documentation Cut-Off Time Creation에 입력된 VVD정보에 따라서 Doc cut-off time을 재 setting한다.
	  * </pre>
	  */
	public BookingMasterMgtDBDAOUpdateDocCutOffTimeByVvdUSQL(){
		setQuery();
		...
		query.append("FileName : BookingMasterMgtDBDAOUpdateDocCutOffTimeByVvdUSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("ON (TM.BKG_NO = TGT.BKG_NO AND TM.CLZ_TP_CD = 'D')" ).append("\n"); 
		query.append("WHEN MATCHED THEN" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("UPDATE SET" ).append("\n"); 
		query.append(" TM.SYS_SET_DT = (SELECT BKG_GET_DCT_CUT_OFF_FNC(TGT.VVD_CD, TGT.POL_CD, TGT.POL_CLPT_IND_SEQ, TGT.POD_CD) FROM DUAL)" ).append("\n"); 
		query.append(",TM.MNL_SET_DT = TO_DATE(DECODE(TM.MNL_SET_DT,'','',TO_CHAR((SELECT BKG_GET_DCT_CUT_OFF_FNC(TGT.VVD_CD, TGT.POL_CD, TGT.POL_CLPT_IND_SEQ, TGT.POD_CD) FROM DUAL),'YYYYMMDDHH24MISS')),'YYYYMMDDHH24MISS')" ).append("\n"); 

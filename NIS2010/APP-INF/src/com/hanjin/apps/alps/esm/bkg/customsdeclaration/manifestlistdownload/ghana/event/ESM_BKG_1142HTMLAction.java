...
*@FileName : ESM_BKG_1142HTMLAction.java
...
public class ESM_BKG_1142HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1140HTMLAction 객체를 생성 
	 */
	public ESM_BKG_1142HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmBkg1142Event event = new EsmBkg1142Event();
    	ManifestListCondVO manifestListCondVO = new GhanaSearchManifestListVO();
		 
		if(command.isCommand(FormCommand.SEARCH)) {
			manifestListCondVO = (GhanaSearchManifestListVO)getVO(request, GhanaSearchManifestListVO.class);
			event.setGhanaSearchManifestListVO((GhanaSearchManifestListVO)manifestListCondVO);
			event.setPolCd(request.getParameter("pol_cd"));
			event.setPodCd(request.getParameter("pod_cd"));
		} else if(command.isCommand(FormCommand.MULTI01)) { 			 		
			event.setManifestTransmitVOs((GhanaManifestTransmitVO[])getVOs(request, GhanaManifestTransmitVO.class));
		} 

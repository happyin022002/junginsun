...
public class ESM_MAS_0134HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0134HTMLAction 객체를 생성
	 */
	public ESM_MAS_0134HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {		
		...
		EsmMas0134Event event = new EsmMas0134Event();		
		
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setRevExpChargeVO((RevExpChargeVO)getVO(request, RevExpChargeVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setRevExpChargeVOs((RevExpChargeVO[])getVOs(request, RevExpChargeVO.class, ""));
		}
		// 삭제

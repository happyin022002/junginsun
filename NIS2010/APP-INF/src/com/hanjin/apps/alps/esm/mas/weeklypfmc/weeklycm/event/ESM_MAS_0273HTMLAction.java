...
**@FileName : ESM_MAS_0273HTMLAction.java
 ...
 * @author Je Ryang Yoo
 * @see EsmMas0273Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0273HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0273HTMLAction 객체를 생성
	 */
	public ESM_MAS_0273HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0273Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmMas0273Event event = new EsmMas0273Event();  	
    			
		// 조회
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setChassisCostReportVO((ChassisCostReportVO)getVO(request, ChassisCostReportVO.class));			
		}
		// 저장
		else if(command.isCommand(FormCommand.MULTI)) {			
			event.setChassisCostReportVOs((ChassisCostReportVO[])getVOs(request, ChassisCostReportVO.class, ""));			
		}
		else {
			event.setChassisCostReportVO((ChassisCostReportVO)getVO(request, ChassisCostReportVO.class));

...
public class ArrivalNoticeDBDAOsearchNoticeHistoryListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	 , DECODE(NTC_VIA_CD, 'F','FAX'" ).append("\n"); 
		query.append("                        , 'M','MAIL'" ).append("\n"); 
		query.append("                        , 'E','EDI') NTC_VIA_CD" ).append("\n"); 
		...
		query.append("             /*+ INDEX_DESC (NHIS XAK3BKG_NTC_HIS) USE_NL(NHIS BKGM) USE_NL(BKGM VSKD) USE_NL(NHIS BKGM) USE_NL(NHIS CUSR) */" ).append("\n"); 
		...
		query.append("		   , NHIS.NTC_VIA_CD" ).append("\n"); 
		...
		query.append("           , TO_CHAR( NVL(NHIS.SND_DT,DECODE(NHIS.NTC_VIA_CD, 'F',FXSD.FAX_SND_LOCL_DT,'M',GLOBALDATE_PKG.TIME_CONV_FNC('KRSEL', EMSD.EML_DT,BKG_COM_USER_LOC_FNC(NHIS.SND_USR_ID)) )) , 'YYYY-MM-DD HH24:MI', 'NLS_DATE_LANGUAGE=ENGLISH') SND_DT  -- FINAL SENT" ).append("\n"); 
		...
		query.append("           , NHIS.BKG_CUST_TP_CD AS BKG_CUST_TP_CD" ).append("\n"); 
		...
		query.append("       WHERE NHIS.NTC_VIA_CD IN ('F', 'M', 'E')" ).append("\n"); 
		...
		query.append("      #if (${sch_tp} == 'V') " ).append("\n"); 
		query.append("         AND BVVD.SKD_VOY_NO = VSKD.SKD_VOY_NO  -- VVD기준일 경우 VVD와 BVVD가 연결되어야 한다.(20091230)" ).append("\n"); 
		...
		query.append("      #else" ).append("\n"); 
		query.append("         AND BVVD.SKD_VOY_NO = VSKD.SKD_VOY_NO(+)  -- VVD기준으로 볼 경우가 아닌 경우 Schedule이 없어도 데이터가 나오게 한다.(20091230)" ).append("\n"); 
		query.append("         AND BVVD.SKD_DIR_CD = VSKD.SKD_DIR_CD(+)" ).append("\n"); 
		query.append("         AND BVVD.POD_CD = VSKD.VPS_PORT_CD(+)" ).append("\n"); 
		query.append("         AND BVVD.POD_CLPT_IND_SEQ = VSKD.CLPT_IND_SEQ(+)" ).append("\n"); 
		query.append("      #end" ).append("\n"); 

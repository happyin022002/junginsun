...
public class BDRSettingDBDAOsearchBdrVvdListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("AND    NVL(TRNK_MNL_BDR_DT,TRNK_ESTM_BDR_DT) >  GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE-15,POL_CD) " ).append("\n"); 
		query.append("AND    NVL(TRNK_MNL_BDR_DT,TRNK_ESTM_BDR_DT) <=  GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE-1,POL_CD) " ).append("\n"); 
		query.append("AND    TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', SYSDATE, POL_CD), 'HH24') <= '03'" ).append("\n"); 
		...
		query.append("AND    NVL(FDR_MNL_BDR_DT,FDR_ESTM_BDR_DT) >  GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE-15,POL_CD) " ).append("\n"); 
		query.append("AND    NVL(FDR_MNL_BDR_DT,FDR_ESTM_BDR_DT) <=  GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE-1,POL_CD) " ).append("\n"); 
		query.append("AND    TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', SYSDATE, POL_CD), 'HH24') <= '03'" ).append("\n"); 

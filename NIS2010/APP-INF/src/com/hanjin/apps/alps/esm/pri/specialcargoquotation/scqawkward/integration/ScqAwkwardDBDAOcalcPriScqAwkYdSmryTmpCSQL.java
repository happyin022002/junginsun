...
*@FileName : ScqAwkwardDBDAOcalcPriScqAwkYdSmryTmpCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ScqAwkwardDBDAOcalcPriScqAwkYdSmryTmpCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 기 계산 처리되어 생성된 PRI_SCQ_AWK_YD_DTL_TMP 데이터를 기반하여 Summary Data 를 생성한다.
	  * </pre>
	  */
	public ScqAwkwardDBDAOcalcPriScqAwkYdSmryTmpCSQL(){
		setQuery();
		...
		query.append("FileName : ScqAwkwardDBDAOcalcPriScqAwkYdSmryTmpCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("INSERT INTO PRI_SCQ_AWK_YD_SMRY_TMP" ).append("\n"); 
		...
		query.append(")" ).append("\n"); 
		query.append("SELECT  @[scq_rqst_no], @[scq_ver_no_tmp], A.ROUT_SEQ, A.YD_COST_SEQ, A.IO_BND_CD, A.YD_CD, A.COST_TP_CD, SUM ( A.CNTR_QTY * A.USD_AMT )" ).append("\n"); 
		query.append("    ,   @[cre_usr_id], SYSDATE, @[cre_usr_id], SYSDATE" ).append("\n"); 
		query.append("FROM    PRI_SCQ_AWK_YD_DTL_TMP A" ).append("\n"); 
		query.append("WHERE   SCQ_RQST_NO = @[scq_rqst_no] " ).append("\n"); 
		query.append("AND     SCQ_VER_NO  = @[scq_ver_no_tmp] " ).append("\n"); 
		query.append("GROUP   BY A.ROUT_SEQ, A.YD_COST_SEQ, A.IO_BND_CD, A.YD_CD, A.COST_TP_CD" ).append("\n"); 

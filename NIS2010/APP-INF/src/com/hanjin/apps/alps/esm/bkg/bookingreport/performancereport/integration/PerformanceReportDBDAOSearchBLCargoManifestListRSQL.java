...
public class PerformanceReportDBDAOSearchBLCargoManifestListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("						   TRIM(TO_CHAR(NVL(D.ACT_WGT, 0),'9,999,990.999')) AS WGT1," ).append("\n"); 
		...
		query.append("                                                   , ( SELECT MAX('O'||VSL_PRE_PST_CD||VSL_SEQ||','||VSL_CD||SKD_VOY_NO||SKD_DIR_CD||','||POL_CD||','||SUBSTR(POL_YD_CD,6)||','||POD_CD ||','||SUBSTR(POD_YD_CD,6))" ).append("\n"); 
		query.append("                                                        FROM BKG_VVD" ).append("\n"); 
		query.append("                                                       WHERE BKG_NO        = A.BKG_NO " ).append("\n"); 

...
*@FileName : ESM_SPC_0047HTMLAction.java
...
public class ESM_SPC_0047HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SPC_0047HTMLAction 객체를 생성
	 */
	public ESM_SPC_0047HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSpc0047Event event = new EsmSpc0047Event();
		if(command.isCommand(FormCommand.MULTI)) {
			...
			event.setSpcAlocPolPodVOS((SpcAlocPolPodVO[])getVOs(request, SpcAlocPolPodVO .class,""));
		}
...
//					vos[i].setCtrlLvl( ctrl );
		...
		else if(command.isCommand(FormCommand.SEARCHLIST02) || command.isCommand(FormCommand.SEARCH)) {
			//event.setSearchSpaceAllocation0047DetailListVO((SearchSpaceAllocation0047DetailListVO)getVO(request, SearchSpaceAllocation0047DetailListVO .class));
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCHLIST01)) {
			//event.setSearchSpaceAllocation0047MasterListVO((SearchSpaceAllocation0047MasterListVO)getVO(request, SearchSpaceAllocation0047MasterListVO .class));
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}

...
*@FileName : RFARateProposalDBDAORsltCnoteNoteConvListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateProposalDBDAORsltCnoteNoteConvListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Cnote에 따른 Conversion 데이터 조회
	  * </pre>
	  */
	public RFARateProposalDBDAORsltCnoteNoteConvListRSQL(){
		setQuery();
		...
		query.append("FileName : RFARateProposalDBDAORsltCnoteNoteConvListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("          FROM PRI_RP_SCP_RT_CNOTE C,  PRI_RP_SCP_RT_CMDT_HDR H" ).append("\n"); 
		query.append("         WHERE C.PROP_NO = @[prop_no]" ).append("\n"); 
		query.append("           AND (C.AMDT_SEQ = @[amdt_seq] OR" ).append("\n"); 
		query.append("               (C.AMDT_SEQ = @[amdt_seq] - 1 AND" ).append("\n"); 
		query.append("               C.SRC_INFO_CD IN ('NW', 'PC', 'PM', 'GC', 'GM', 'AM')))" ).append("\n"); 
		query.append("           AND C.SVC_SCP_CD = @[svc_scp_cd]" ).append("\n"); 
		query.append("           AND C.PROP_NO = H.PROP_NO" ).append("\n"); 
		query.append("           AND C.AMDT_SEQ = H.AMDT_SEQ" ).append("\n"); 
		...
		query.append("           AND C.NOTE_CONV_MAPG_ID = A.NOTE_CONV_MAPG_ID)" ).append("\n"); 

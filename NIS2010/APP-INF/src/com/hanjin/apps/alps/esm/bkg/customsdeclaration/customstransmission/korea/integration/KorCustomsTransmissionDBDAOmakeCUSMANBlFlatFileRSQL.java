...
public class KorCustomsTransmissionDBDAOmakeCUSMANBlFlatFileRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("			TRIM(TO_CHAR(KT.CNTR_TTL_WGT,'999999999999999.999'))||'~'||	/*	Weight				*/" ).append("\n"); 
		...
		query.append("			BKG_SPCLCHAR_CONV_FNC(SUBSTR(S.CUST_NM, 36, 35),'Y')	||'~'||	/*	Shipper Name1	*/" ).append("\n"); 
		query.append("			BKG_SPCLCHAR_CONV_FNC(SUBSTR(S.CUST_NM, 71, 35),'Y')	||'~'||	/*	Shipper Name1	*/" ).append("\n"); 
		...
		query.append("			2017.10.18 SM상선 박준훈 과장 요청 사항 - 수하인/통지처/송하인 도시코드가 필수 값이 아니고, USFXQ 처럼 일부 도시코드명은 세관에 미등록된 코드로서" ).append("\n"); 
		query.append("            미등록된 코드는 전송시 에러를 발생시키므로 기존의 Shipper City 코드를 주석처리하고 아래처럼 공백으로 처리 " ).append("\n"); 
		query.append("*/" ).append("\n"); 
		query.append("			''||'~'||	/*	Shipper City */" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("			2017.10.18 SM상선 박준훈 과장 요청 사항 - 수하인/통지처/송하인 도시코드가 필수 값이 아니고, USFXQ 처럼 일부 도시코드명은 세관에 미등록된 코드로서" ).append("\n"); 
		query.append("            미등록된 코드는 전송시 에러를 발생시키므로 기존의 CNEE City 코드를 주석처리하고 아래처럼 공백으로 처리 " ).append("\n"); 
		query.append("*/" ).append("\n"); 
		query.append("			''||'~'||	/*	CNEE City */" ).append("\n"); 
		...
		query.append("			2017.10.18 SM상선 박준훈 과장 요청 사항 - 수하인/통지처/송하인 도시코드가 필수 값이 아니고, USFXQ 처럼 일부 도시코드명은 세관에 미등록된 코드로서" ).append("\n"); 
		query.append("            미등록된 코드는 전송시 에러를 발생시키므로 기존의 NTFY City 코드를 주석처리하고 아래처럼 공백으로 처리 " ).append("\n"); 
		query.append("*/" ).append("\n"); 
		query.append("			''||'~'||	/*	NTFY City */" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("FROM	BKG_CSTMS_KR_BL KT, BKG_CSTMS_KR_CUST C, BKG_CSTMS_KR_CUST N, BKG_CSTMS_KR_CUST S, MDM_LOCATION L1, MDM_LOCATION L2, MDM_LOCATION L3, BKG_BOOKING BKG, BKG_CSTMS_KR_CNTR CN" ).append("\n"); 

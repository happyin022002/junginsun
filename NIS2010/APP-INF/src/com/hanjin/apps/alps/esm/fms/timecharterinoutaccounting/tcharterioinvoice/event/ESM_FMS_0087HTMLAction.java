...
*@FileName : ESM_FMS_0087HTMLAction.java
...
public class ESM_FMS_0087HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0018HTMLAction 객체를 생성
	 */
	public ESM_FMS_0087HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0087Event event = new EsmFms0087Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setCondSearchOwnerInvoiceVO((CondSearchOwnerInvoiceVO)getVO(request, CondSearchOwnerInvoiceVO .class));
		} else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setCondSearchOwnerInvoiceAutoMatchVO((CondSearchOwnerInvoiceAutoMatchVO)getVO(request, CondSearchOwnerInvoiceAutoMatchVO .class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchType(request.getParameter("searchType"));
			event.setCustomOwnrAcctSlpVOS((CustomOwnrAcctSlpVO[])getVOs(request, CustomOwnrAcctSlpVO .class, ""));
			event.setCondSearchOwnerInvoiceVO((CondSearchOwnerInvoiceVO)getVO(request, CondSearchOwnerInvoiceVO .class));
			event.setCondSearchOwnerInvoiceAutoMatchVO((CondSearchOwnerInvoiceAutoMatchVO)getVO(request, CondSearchOwnerInvoiceAutoMatchVO .class));
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchOwnerInvoiceListVOS((SearchOwnerInvoiceListVO[])getVOs(request, SearchOwnerInvoiceListVO .class, ""));
		}

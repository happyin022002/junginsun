...
public class NetworkDistributionBackEndJob  extends BackEndCommandSupport{
	...
	public WeeklyCMCommonVO doStart() throws Exception {
		...
		try {	
			if(searchVo.getFInout().equalsIgnoreCase("ESM_MAS_0106")) {			//Allocation Results - Apply to P/L 
				reVo = applyToPL();
			} else if(searchVo.getFInout().equalsIgnoreCase("ESM_MAS_0108")) {	//Allocation Results(Commitment Base) - Apply to P/L 
				reVo = applyToPL();
			}
	...
	/**
	 * ESM_MAS_106,ESM_MAS_108: APPLY 이벤트 처리<br>
	 * 

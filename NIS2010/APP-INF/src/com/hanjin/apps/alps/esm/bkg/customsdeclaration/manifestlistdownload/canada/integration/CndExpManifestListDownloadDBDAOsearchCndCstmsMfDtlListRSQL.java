...
*@FileName : CndExpManifestListDownloadDBDAOsearchCndCstmsMfDtlListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CndExpManifestListDownloadDBDAOsearchCndCstmsMfDtlListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public CndExpManifestListDownloadDBDAOsearchCndCstmsMfDtlListRSQL(){
		setQuery();
		...
		query.append("FileName : CndExpManifestListDownloadDBDAOsearchCndCstmsMfDtlListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT  DISTINCT TB.*" ).append("\n"); 
		query.append("       ,DECODE(TB.SND_DT, NULL, 'N', 'Y') AS EDI_FLG" ).append("\n"); 
		query.append("       ,TB.SND_DT AS EDI_SND_DT" ).append("\n"); 
		query.append("       ,DECODE(EMP_FLG, 'N', decode(BKG_GET_CSTMS_ADV_ERR_CD_FNC(TB.BKG_NO, TB.BL_TYPE, 'CA'), 'H', '', 'F', '', BKG_GET_CSTMS_ADV_ERR_CD_FNC(TB.BKG_NO, TB.BL_TYPE, 'CA'))" ).append("\n"); 
		query.append("               , (SELECT DECODE (COUNT(1), 0, 'E', '')" ).append("\n"); 
		query.append("                   FROM BKG_CONTAINER" ).append("\n"); 
		query.append("                  WHERE BKG_NO = TB.BKG_NO)) AS ERR_CD" ).append("\n"); 
		...
		query.append("               ,BKG_CSTMS_AMER_BL ABL" ).append("\n"); 
		...
		query.append("AND  ABL.IO_BND_CD(+) = 'O'" ).append("\n"); 
		...
		query.append("   AND  TB.POL_CD 		= SKD.VPS_PORT_CD" ).append("\n"); 
		query.append("   " ).append("\n"); 

...
*@FileName : ESM_COA_0117HTMLAction.java
...
public class ESM_COA_0117HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0117HTMLAction 객체를 생성
	 */
	public ESM_COA_0117HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmCoa0117Event event = new EsmCoa0117Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCoaSltMgmtUtVOS((CoaSltMgmtUtVO[])getVOs(request, CoaSltMgmtUtVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setSearchSMUPfmcListVO((SearchSMUPfmcListVO)getVO(request, SearchSMUPfmcListVO .class));

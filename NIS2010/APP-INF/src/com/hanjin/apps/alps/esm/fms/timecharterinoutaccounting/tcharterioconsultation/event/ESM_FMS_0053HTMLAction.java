...
*@FileName : ESM_FMS_0053HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.nis2010.alps.fms.timecharterinoutaccounting 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESM_FMS_0053HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0053HTMLAction 객체를 생성
	 */
	public ESM_FMS_0053HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0053Event event = new EsmFms0053Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomInterfaceStatusVOS((CustomInterfaceStatusVO[])getVOs(request, CustomInterfaceStatusVO .class,"sheet1_"));
			event.setCondSearchInterfaceStatusVO((CondSearchInterfaceStatusVO)getVO(request, CondSearchInterfaceStatusVO .class));
			event.setStatusFlag(request.getParameter("condition"));
			event.setCsrNo(request.getParameter("csr_no"));
			event.setAproStep(request.getParameter("apro_step"));

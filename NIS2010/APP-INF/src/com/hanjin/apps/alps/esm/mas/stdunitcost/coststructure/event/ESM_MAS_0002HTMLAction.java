...
*@FileName : ESM_MAS_0002HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Kim Ki Dae
 * @see EsmMas0002Event 참조
 * @since J2EE 1.6
 */
public class ESM_MAS_0002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_MAS_0002HTMLAction 객체를 생성
	 */
	public ESM_MAS_0002HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmMas0002Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmMas0002Event event = new EsmMas0002Event();
		...
		if(command.isCommand(FormCommand.SEARCHLIST)) {
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class)); // 조건 DTD setting
			event.setMasStndAcctVOs((MasStndAcctVO[])getVOs(request, MasStndAcctVO.class));
		}
		else if (command.isCommand(FormCommand.MULTI02)) {
			event.setComboVO((EsmMas0002ComboVO)getVO(request, EsmMas0002ComboVO.class));
		}else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class)); // 조건 DTD setting
		}

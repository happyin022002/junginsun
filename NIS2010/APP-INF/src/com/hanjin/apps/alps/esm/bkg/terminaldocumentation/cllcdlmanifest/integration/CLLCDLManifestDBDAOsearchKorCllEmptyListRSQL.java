...
*@FileName : CLLCDLManifestDBDAOsearchKorCllEmptyListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CLLCDLManifestDBDAOsearchKorCllEmptyListRSQL implements ISQLTemplate{
	...
	public CLLCDLManifestDBDAOsearchKorCllEmptyListRSQL(){
		...
		query.append("FileName : CLLCDLManifestDBDAOsearchKorCllEmptyListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	CLL_RMK3," ).append("\n"); 
		query.append("	DECODE(POD_CD,'USSEA',DECODE(VVD_SLAN_CD, 'PNH', DECODE(BC_CD,'SE8','USSE8', DECODE(BLCK_STWG_CD,'ONE','USSE1','USSEA')), DECODE(BLCK_STWG_CD,'ONE','USSE1','USSEA')),'CAVAN',DECODE(BLCK_STWG_CD,'LOC','CAYVR','ONE','CAVA1','CAVAN'), 'CAPRR', DECODE(BC_CD, 'PR1', 'CAPR1', 'CAPRR'), POD_CD) POD_CD," ).append("\n"); 
		...
		query.append("		SUBSTR(MAX(A.VSL_CD),1,4) a12," ).append("\n"); 
		query.append("		SUBSTR(MAX(A.SKD_VOY_NO),-4) a13," ).append("\n"); 
		query.append("		SUBSTR(MAX(A.SKD_DIR_CD),1,1) a14," ).append("\n"); 
		...
		query.append("		SUBSTR(MAX(BKG_VVD.VSL_CD),1,4) a23," ).append("\n"); 
		query.append("		SUBSTR(MAX(BKG_VVD.SKD_VOY_NO),-4) a24," ).append("\n"); 
		query.append("		SUBSTR(MAX(BKG_VVD.SKD_DIR_CD),1,1) a25," ).append("\n"); 
		...
		query.append("		MAX(BKG_RF_CGO.CDO_TEMP) a33," ).append("\n"); 
		...
		query.append("		BKG_BOOKING.BKG_CGO_TP_CD IN ('R','P') AND" ).append("\n"); 
		...
		query.append("		NVL(BKG_VVD.POL_YD_CD,' ') LIKE @[in_pol_yd_cd]||'%' ) AND" ).append("\n"); 
		...
		query.append("	GROUP BY	BKG_CONTAINER.BKG_NO, " ).append("\n"); 
		...
		query.append("			BKG_VVD.POL_YD_CD) T2," ).append("\n"); 
		...
		query.append("order by BKG_POL_CD,POD_CD,BKG_NO" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '1' ) " ).append("\n"); 
		query.append("order by BKG_POL_CD,POD_CD,BKG_NO" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '2' ) " ).append("\n"); 
		query.append("order by BKG_POL_CD,POD_CD, A_POD_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '3' ) " ).append("\n"); 
		query.append("order by TS_VSL_CD||TS_SKD_VOY_NO||TS_SKD_DIR_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '4' ) " ).append("\n"); 
		query.append("order by DECODE(BLCK_STWG_CD,'LOC','11','ONE','12','TWO','13','THR','14','FOR','15', 'FIV', '15.5', 'SEV', '15.7', 'HOT','16','TRS','17', '18')" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '5' ) " ).append("\n"); 
		query.append("order by POD_CD, DECODE(BLCK_STWG_CD,'LOC','11','ONE','12','TWO','13','THR','14','FOR','15', 'FIV', '15.5', 'SEV', '15.7','HOT','16','TRS','17', '18'), CNTR_TPSZ_CD, CNTR_NO" ).append("\n"); 
		query.append("#end" ).append("\n"); 

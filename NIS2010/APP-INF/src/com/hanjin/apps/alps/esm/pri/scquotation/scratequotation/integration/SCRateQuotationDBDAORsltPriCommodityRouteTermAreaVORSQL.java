...
*@FileName : SCRateQuotationDBDAORsltPriCommodityRouteTermAreaVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCRateQuotationDBDAORsltPriCommodityRouteTermAreaVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public SCRateQuotationDBDAORsltPriCommodityRouteTermAreaVORSQL(){
		setQuery();
		...
		query.append("FileName : SCRateQuotationDBDAORsltPriCommodityRouteTermAreaVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT DISTINCT RCV_DE_TERM_CD AS CD" ).append("\n"); 
		query.append(",(SELECT MDM_TERM.INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL MDM_TERM WHERE INTG_CD_ID = 'CD02139' AND MDM_TERM.INTG_CD_VAL_CTNT = MN.RCV_DE_TERM_CD) AS NM" ).append("\n"); 
		query.append("FROM PRI_SQ_RT_ROUT_PNT MN" ).append("\n"); 
		...
		query.append("FROM PRI_SQ_RT_ROUT_PNT SUB" ).append("\n"); 
		query.append("WHERE 	    SUB.QTTN_NO = MN.QTTN_NO" ).append("\n"); 
		query.append("AND SUB.QTTN_VER_NO = MN.QTTN_VER_NO" ).append("\n"); 
		query.append("AND SUB.GEN_SPCL_RT_TP_CD = MN.GEN_SPCL_RT_TP_CD" ).append("\n"); 
		...
		query.append("AND EXISTS (" ).append("\n"); 
		query.append("SELECT 'FOUND'" ).append("\n"); 
		query.append("FROM PRI_SQ_RT_ROUT_VIA SUB" ).append("\n"); 
		query.append("WHERE 	    SUB.QTTN_NO = MN.QTTN_NO" ).append("\n"); 
		query.append("AND SUB.QTTN_VER_NO = MN.QTTN_VER_NO" ).append("\n"); 
		query.append("AND SUB.GEN_SPCL_RT_TP_CD = MN.GEN_SPCL_RT_TP_CD" ).append("\n"); 
		query.append("AND SUB.CMDT_HDR_SEQ = MN.CMDT_HDR_SEQ" ).append("\n"); 

...
*@FileName : ESM_BKG_0445HTMLAction.java
...
public class ESM_BKG_0445HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0445HTMLAction 객체를 생성
	 */
	public ESM_BKG_0445HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0445Event event = new EsmBkg0445Event();
		if(command.isCommand(FormCommand.MULTI)) {
			event.setDpcsSiSplitCandidateVOS((DpcsSiSplitCandidateVO[])getVOs(request, DpcsSiSplitCandidateVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setDpcsSiSplitCandidateVO((DpcsSiSplitCandidateVO)getVO(request, DpcsSiSplitCandidateVO .class));
			event.setDpcsSiSplitBkgCntrHisVO((DpcsSiSplitBkgCntrHisVO)getVO(request, DpcsSiSplitBkgCntrHisVO .class));
		}

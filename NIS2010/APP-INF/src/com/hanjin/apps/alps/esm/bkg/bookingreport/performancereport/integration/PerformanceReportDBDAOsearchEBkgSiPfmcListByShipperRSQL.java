...
*@FileName : PerformanceReportDBDAOsearchEBkgSiPfmcListByShipperRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class PerformanceReportDBDAOsearchEBkgSiPfmcListByShipperRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public PerformanceReportDBDAOsearchEBkgSiPfmcListByShipperRSQL(){
		setQuery();
		...
		query.append("FileName : PerformanceReportDBDAOsearchEBkgSiPfmcListByShipperRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("--3.Case1 => Kind of Report = Shipper Office, Kind of Report = Month" ).append("\n"); 
		query.append("--REPORT_TYPE:B, DURATION: M,W,D" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("--********************************************************************************************" ).append("\n"); 
		...
		query.append("    WHEN (SI_TTL - ( BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB )) > 0 THEN (SI_TTL - (BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB ))" ).append("\n"); 
		query.append("    ELSE 0" ).append("\n"); 
		query.append("  END BL_PENDING ," ).append("\n"); 
		query.append("  ROUND(((BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL+BL_WEB_SWB ) / DECODE((BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL +BL_WEB_SWB+ (" ).append("\n"); 
		query.append("                                          CASE" ).append("\n"); 
		query.append("                                            WHEN (SI_TTL - ( BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB )) > 0 THEN (SI_TTL - (BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB ))" ).append("\n"); 
		query.append("                                            ELSE 0" ).append("\n"); 
		query.append("                                          END)), 0, 1, (BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + (" ).append("\n"); 
		query.append("                                          CASE" ).append("\n"); 
		query.append("                                            WHEN (SI_TTL - ( BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB )) > 0 THEN (SI_TTL - (BL_NIS + BL_EDI + BL_WEB_OBL + BL_SWB_EMAIL + BL_WEB_SWB ))" ).append("\n"); 
		query.append("                                            ELSE 0" ).append("\n"); 
		query.append("                                          END) +BL_WEB_SWB)))*100, 1) BL_SVC" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT  " ).append("\n"); 
		query.append("#if (${duration_opt} == 'M' or ${duration_opt} == 'W' ) " ).append("\n"); 
		query.append("/*+ INDEX(MON XAK4MAS_MON_VVD) */ " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("		'CR' REPORT_TYPE" ).append("\n"); 
		query.append("--		,DECODE(BK.SC_NO,NULL, BK.RFA_NO,BK.SC_NO) sc_rfa_no" ).append("\n"); 
		query.append("        ,NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO)) sc_rfa_no" ).append("\n"); 
		query.append("        ,(SELECT CUST.CUST_LGL_ENG_NM  as CUST_NM" ).append("\n"); 
		query.append("          FROM PRI_SP_CTRT_PTY A, PRI_SP_HDR D, PRI_SP_MN MN, MDM_CUSTOMER CUST" ).append("\n"); 
		query.append("          WHERE A.CUST_CNT_CD = CUST.CUST_CNT_CD" ).append("\n"); 
		query.append("          AND A.CUST_SEQ    = CUST.CUST_SEQ" ).append("\n"); 
		query.append("          AND A.PROP_NO     = D.PROP_NO" ).append("\n"); 
		query.append("          AND D.SC_NO       = NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("          AND A.AMDT_SEQ    = (SELECT MAX(C.AMDT_SEQ) FROM PRI_SP_CTRT_PTY C  WHERE C.PROP_NO = A.PROP_NO  )" ).append("\n"); 
		query.append("          AND A.PRC_CTRT_PTY_TP_CD = 'C'" ).append("\n"); 
		query.append("          AND A.PROP_NO = MN.PROP_NO " ).append("\n"); 
		query.append("          AND A.AMDT_SEQ = MN.AMDT_SEQ" ).append("\n"); 
		query.append("          UNION" ).append("\n"); 
		query.append("          SELECT CUST.CUST_LGL_ENG_NM as CUST_NM" ).append("\n"); 
		query.append("          FROM MDM_CUSTOMER CUST, PRI_RP_MN MN, PRI_RP_HDR HDR" ).append("\n"); 
		...
		query.append("          AND MN.AMDT_SEQ = ( SELECT MAX(A.AMDT_SEQ) FROM PRI_RP_MN A WHERE MN.PROP_NO = A.PROP_NO )" ).append("\n"); 
		query.append("          AND HDR.RFA_NO =  NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("          UNION" ).append("\n"); 
		query.append("          SELECT CUST.CUST_LGL_ENG_NM AS CUST_NM" ).append("\n"); 
		query.append("          FROM PRI_TAA_HDR TD, PRI_TAA_MN TA, MDM_CUSTOMER CUST" ).append("\n"); 
		query.append("          WHERE TA.TAA_PROP_NO = TD.TAA_PROP_NO" ).append("\n"); 
		query.append("          AND   TD.TAA_NO =  NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("          AND   TA.AMDT_SEQ = (SELECT MAX(TM.AMDT_SEQ)FROM PRI_TAA_MN TM WHERE TM.TAA_PROP_NO = TA.TAA_PROP_NO)" ).append("\n"); 
		query.append("          AND   CUST.CUST_CNT_CD = TA.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("          AND   CUST.CUST_SEQ = TA.CTRT_CUST_SEQ ) AS CUST_NM" ).append("\n"); 
		query.append("         ,(SELECT MN.PROP_OFC_CD as REQ_OFC" ).append("\n"); 
		query.append("          FROM PRI_SP_CTRT_PTY A,  PRI_SP_HDR D,  PRI_SP_MN MN," ).append("\n"); 
		...
		query.append("          AND HDR.RFA_NO =  NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("          UNION" ).append("\n"); 
		query.append("          SELECT TA.RESPB_SLS_OFC_CD as REQ_OFC" ).append("\n"); 
		query.append("          FROM PRI_TAA_HDR TD, PRI_TAA_MN TA, MDM_CUSTOMER CUST" ).append("\n"); 
		query.append("          WHERE TA.TAA_PROP_NO = TD.TAA_PROP_NO" ).append("\n"); 
		query.append("          AND   TD.TAA_NO =  NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("          AND   TA.AMDT_SEQ = (SELECT MAX(TM.AMDT_SEQ)FROM PRI_TAA_MN TM WHERE TM.TAA_PROP_NO = TA.TAA_PROP_NO)" ).append("\n"); 
		query.append("          AND   CUST.CUST_CNT_CD = TA.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("          AND   CUST.CUST_SEQ = TA.CTRT_CUST_SEQ ) AS PROP_OFC_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        ,BK.OB_SLS_OFC_CD" ).append("\n"); 
		query.append("        ,BK.OB_SREP_CD" ).append("\n"); 
		query.append("		,MC.CUST_CNT_CD || MC.CUST_SEQ REGION_CD" ).append("\n"); 
		query.append("		,TRANSLATE(NVL(MC.CUST_LGL_ENG_NM,' '),CHR(13)||CHR(10),' ') GSO  " ).append("\n"); 
		...
		query.append("              AND ROWNUM = 1), null, DECODE(BII.N1ST_PRN_DT||BII.N2ND_PRN_DT, null, DECODE((" ).append("\n"); 
		query.append("                        SELECT BKG_NO" ).append("\n"); 
		query.append("                        FROM BKG_NTC_HIS" ).append("\n"); 
		query.append("                        WHERE BKG_NO = BK.BKG_NO" ).append("\n"); 
		...
		query.append("	  	,CASE WHEN SUM(DECODE(BK.XTER_BKG_RQST_CD,'EDI',1,0))+SUM(DECODE(nvl(BK.BL_TP_CD,'O') || BK.XTER_BKG_RQST_CD,'OWEB',1,0))" ).append("\n"); 
		query.append("				+ SUM(DECODE(nvl(BK.BL_TP_CD,'O') || BK.XTER_BKG_RQST_CD,'WWEB',1,0))+   SUM(DECODE(NVL(BK.BL_TP_CD,'O'),'S',1,0)) = 0 THEN '0.0' " ).append("\n"); 
		query.append(" 			 ELSE TRIM(TO_CHAR((SUM(DECODE(BK.XTER_BKG_RQST_CD,'EDI',1,0))+SUM(DECODE(nvl(BK.BL_TP_CD,'O') || BK.XTER_BKG_RQST_CD,'OWEB',1,0))" ).append("\n"); 
		query.append("				+ SUM(DECODE(nvl(BK.BL_TP_CD,'O') || BK.XTER_BKG_RQST_CD,'WWEB',1,0))+   SUM(DECODE(NVL(BK.BL_TP_CD,'O'),'S',1,0))" ).append("\n"); 
		query.append("				) /  (SUM(DECODE(NVL(BK.BL_TP_CD,'O'),'O',1,0)) +SUM(DECODE(NVL(BK.BL_TP_CD,'O'),'O',0,1))) *100,'99990.9')) END  BL_SVC" ).append("\n"); 
		...
		query.append("AND  BK.CTRT_OFC_CD = @[ctrt_ofc]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("AND BCS2.CUST_CNT_CD = MC.CUST_CNT_CD(+)" ).append("\n"); 
		query.append("AND BCS2.CUST_SEQ = MC.CUST_SEQ(+)" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("GROUP BY --NVL(BK.SC_NO,NVL(BK.RFA_NO,BK.TAA_NO))" ).append("\n"); 
		query.append("             BK.SC_NO,BK.RFA_NO,BK.TAA_NO" ).append("\n"); 
		query.append("           , BK.OB_SLS_OFC_CD" ).append("\n"); 
		query.append("           , BK.OB_SREP_CD" ).append("\n"); 
		query.append("           , MC.CUST_CNT_CD,MC.CUST_SEQ" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("ORDER BY MC.CUST_CNT_CD,MC.CUST_SEQ" ).append("\n"); 
		query.append("       , MC.CUST_LGL_ENG_NM" ).append("\n"); 
		query.append("       , BK.BKG_OFC_CD" ).append("\n"); 
		query.append("#if (${duration_opt} == 'W') " ).append("\n"); 
		query.append("       , MON.COST_WK" ).append("\n"); 

...
* 1.0 Creation
=========================================================*/
package com.hanjin.apps.alps.esm.acm.acmapproval;
...
import com.hanjin.apps.alps.esm.acm.acmapproval.agncommapproval.event.EsmAcm0009Event;
import com.hanjin.apps.alps.esm.acm.acmapproval.agncommapproval.event.EsmAcm0013Event;
...
import com.hanjin.apps.alps.esm.acm.acmapproval.ffcmpnapproval.event.EsmAcm0030Event;
...
import com.hanjin.apps.alps.esm.acm.acmapproval.spclcmpnapproval.basic.SPCLCmpnApprovalBC;
import com.hanjin.apps.alps.esm.acm.acmapproval.spclcmpnapproval.basic.SPCLCmpnApprovalBCImpl;
import com.hanjin.apps.alps.esm.acm.acmapproval.spclcmpnapproval.event.EsmAcm0031Event;
...
/**
 * ALPS-ACMApproval Business Logic ServiceCommand<br>
 * - ALPS-ACMApproval에 대한 비지니스 트랜잭션을 처리한다.
 *
 * @author KIM YOUNG-OH
...
public class ACMApprovalSC extends ServiceCommandSupport {
	 ...
	 * ALPS-ACMApproval system 업무에서 발생하는 모든 이벤트의 분기처리<br>
	 *
	 * @param e Event
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if (e.getEventName().equalsIgnoreCase("EsmAcm0009Event")) {
			...
			} else if (e.getFormCommand().isCommand(FormCommand.MULTI03)) { //@@ Approval 후 GW URL 호출
				eventResponse = manageAgentApplication(e);
			} else if (e.getFormCommand().isCommand(FormCommand.SEARCH04)) { //GW & ALPS 결재라인결정
				eventResponse = searchAGNCommApprovedYN(e);
			}
	...
	/**
	 * Agent Settlement Application gw open<br>
	 * @author 함대성
	 * @category ESM_ACM_0009
	 * @category manageAgentApplication 
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException
	 */
	private EventResponse manageAgentApplication(Event e) throws EventException {
		// 반환 객체 생성
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmAcm0009Event event = (EsmAcm0009Event)e;
		ComCsrApprovalBC comCommand = new ComCsrApprovalBCImpl();
		//PARAM
		String csrNo = event.getAgnCommApprovalMasterVO().getCsrNo();
		String invSubSysCd = "ACM";
		//그룹웨어 URL주소
		String gwUrl= "";

		//5. gwUrl 구하기 > 공통CSR 메소드 호출
		try{
			//5
			gwUrl = comCommand.sendGwApprovalRequestInfo(csrNo, invSubSysCd, account);
			if (gwUrl == null) gwUrl = "";
	...
	/**
	 * [ESM_ACM_0009]GW&Approval 분기로직<br>
	 * @author 함대성
	 * Agent Commission CSR Creation GW&Approval 분기로직 조회<br>
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException
	 */
	private EventResponse searchAGNCommApprovedYN(Event e) throws EventException {
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		EsmAcm0009Event event = (EsmAcm0009Event)e;
		AGNCommApprovalBC command = new AGNCommApprovalBCImpl();

		try{
			String approvedYN = command.approvedYN(event.getAgnCommApprovalMasterVO(), account);
			eventResponse.setETCData("approvedYN", approvedYN);
		} catch(EventException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
		} catch(Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);

...
public class USDomesticDBDAOAddEmptyRepoCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("MERGE INTO MAS_USA_DMST_UT_COST B1" ).append("\n"); 
		...
		query.append("                       ,MAS_LOC_FNC(A2.LOC_CD, 'ECC') ECC_CD" ).append("\n"); 
		query.append("                       ,A2.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                       ,A1.SIM_TRSP_UC_AMT" ).append("\n"); 
		query.append("                       ,SUM(A2.IB_QTY) IB_QTY" ).append("\n"); 
		query.append("                   FROM MAS_MTY_ECC_UT_COST A1, CIM_LOC_MTCH_BAK_SMRY A2" ).append("\n"); 
		...
		query.append("                                         AND TO_CHAR(LAST_DAY(TO_DATE(REPLACE(@[f_cre_end_dt], '-', ''), 'YYYYMM')), 'YYYYMMDD') --MAS MT 단가 생성 시 MB IF 쿼리 조건 이용" ).append("\n"); 
		...
		query.append("                    AND MAS_LOC_FNC(A2.LOC_CD, 'ECC') = A1.ECC_CD(+)" ).append("\n"); 
		query.append("                GROUP BY A2.CNTR_TPSZ_CD, MAS_LOC_FNC(A2.LOC_CD, 'ECC'), A1.SIM_TRSP_UC_AMT" ).append("\n"); 
		query.append("          )" ).append("\n"); 
		...
		query.append("      AND MAS_LOC_FNC(B1.ORG_RAIL_LOC_CD, 'ECC') = B2.ECC_CD" ).append("\n"); 

...
*@FileName : ESM_FMS_0039HTMLAction.java
...
public class ESM_FMS_0039HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0039HTMLAction 객체를 생성
	 */
	public ESM_FMS_0039HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0039Event event = new EsmFms0039Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomConsultationVO((CustomConsultationVO)getVO(request, CustomConsultationVO .class));
			event.setCustomCsulSlpVOS((CustomCsulSlpVO[])getVOs(request, CustomCsulSlpVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {//Retrieve
			event.setCondOwnerAccountExpenseVO((CondOwnerAccountExpenseVO)getVO(request, CondOwnerAccountExpenseVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {//Search Invoice By Owner Account Slip

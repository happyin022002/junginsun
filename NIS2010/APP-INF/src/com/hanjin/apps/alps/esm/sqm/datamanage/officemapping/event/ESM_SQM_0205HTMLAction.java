...
*@FileName       : ESM_SQM_0205HTMLAction.java
 ...
 * @author SQM USER
 * @see EsmSqm0205Event 참조
 * @since J2EE 1.6
 */

public class ESM_SQM_0205HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	
	/**
	 * ESM_SQM_0205HTMLAction 객체를 생성
	 */
	public ESM_SQM_0205HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmSqm0205Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSqm0205Event event = new EsmSqm0205Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setSearchAddSectorOfcRelSetVOS((SearchAddSectorOfcRelSetVO[])getVOs(request, SearchAddSectorOfcRelSetVO .class));
		}

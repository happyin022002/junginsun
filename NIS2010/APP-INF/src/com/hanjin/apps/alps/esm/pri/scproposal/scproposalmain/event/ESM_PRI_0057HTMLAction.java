...
*@FileName : ESM_PRI_0053HTMLAction.java
...
public class ESM_PRI_0057HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_0057HTMLAction 객체를 생성
	 */
	public ESM_PRI_0057HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri0057Event event = new EsmPri0057Event();

		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setPriSpHdrVO((PriSpHdrVO)getVO(request, PriSpHdrVO.class));
		}else if (command.isCommand(FormCommand.SEARCH02)) {
			event.setCstShHistVO((CstShHistVO)getVO(request, CstShHistVO.class));
		}else if (command.isCommand(FormCommand.SEARCH03)) {
			event.setPriSpMnVO((PriSpMnVO)getVO(request, PriSpMnVO.class));
		}		
		else if(command.isCommand(FormCommand.SEARCH04)) {
			event.setCstShHistVO((CstShHistVO)getVO(request, CstShHistVO .class));
		}else if(command.isCommand(FormCommand.SEARCH05)) {	//todo
			event.setPriSpHdrVO((PriSpHdrVO)getVO(request, PriSpHdrVO .class));
		}

...
public class CLLCDLManifestDBDAOsearchSpclCgoSumByPodRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", DECODE(SUM(O_LO_45),0,'',SUM(O_LO_45)) O_LO_45" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append(", DECODE(SUM(O_TS_45),0,'',SUM(O_TS_45)) O_TS_45" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append(", DECODE(SUM(NVL(O_LO_45,0)+NVL(O_TS_45,0)),0,'',SUM(NVL(O_LO_45,0)+NVL(O_TS_45,0))) O_LO_TS_45" ).append("\n"); 
		...
		query.append("		, SUM(DECODE(BV.POL_CD,BK.POL_CD, DECODE(BQ.CNTR_TPSZ_CD,'O7',BQ.OP_CNTR_QTY,0),0)) O_LO_45" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("		, SUM(DECODE(BV.POL_CD,BK.POL_CD, 0, DECODE(BQ.CNTR_TPSZ_CD,'O7',BQ.OP_CNTR_QTY,0))) O_TS_45" ).append("\n"); 
		query.append("	FROM BKG_BOOKING BK" ).append("\n"); 
		query.append("		, BKG_VVD BV" ).append("\n"); 
		query.append("		, VSK_VSL_PORT_SKD VPS" ).append("\n"); 
		...
		query.append("GROUP BY DG.POD_CD2,BQ.BLCK_STWG_CD, CLPT_SEQ" ).append("\n"); 

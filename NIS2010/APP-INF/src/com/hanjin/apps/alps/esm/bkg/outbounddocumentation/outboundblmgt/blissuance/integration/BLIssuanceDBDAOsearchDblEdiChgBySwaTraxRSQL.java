...
public class BLIssuanceDBDAOsearchDblEdiChgBySwaTraxRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            AND DECODE(B.N3PTY_RCV_OFC_CD, NULL, DECODE(B.FRT_TERM_CD, 'P', D.PPD_RCV_OFC_CD, D.CLT_OFC_CD) , B.N3PTY_RCV_OFC_CD) = 'SINSC'       " ).append("\n"); 
		...
		query.append("            AND DECODE(B.N3PTY_CUST_CNT_CD, NULL, DECODE(B.FRT_TERM_CD, 'P', D.PPD_PAYR_CUST_SEQ, D.CLT_PAYR_CUST_SEQ) , B.N3PTY_CUST_SEQ) IN (DECODE(@[edi_receive_id],'TRAX.HPENTAPAC',16932,260), 11029,17434)                                     " ).append("\n"); 
		...
		query.append("                                   AND AR_OFC_CD = 'SINSC'" ).append("\n"); 
		...
		query.append("            AND C.AR_OFC_CD = 'SINSC'" ).append("\n"); 
		query.append("            AND B.BKG_NO = D.BKG_NO" ).append("\n"); 
		...
		query.append("			END" ).append("\n"); 
		query.append("  AND G.VPS_PORT_CD = DECODE(A.IO_BND_CD , 'O', A.POL_CD, A.POD_CD)" ).append("\n"); 
		query.append("  AND G.VPS_PORT_CD = DECODE(A.IO_BND_CD , 'O', F.POL_CD, F.POD_CD)" ).append("\n"); 
		...
		query.append("                      WHEN A.IO_BND_CD = 'O' AND NVL(G.SKD_CNG_STS_CD, ' ') IN ('I', ' ', 'A') THEN 1" ).append("\n"); 
		query.append("                      WHEN A.IO_BND_CD = 'I' AND NVL(G.SKD_CNG_STS_CD, ' ') IN ('O', ' ', 'A') THEN 1" ).append("\n"); 

...
public class CommonDBDAOSearchBsaCommonRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Common 공동 메소드 쿼리 모음
	  * [CHM-201217556] 2012.04.27 김수정 - 첫번째 Port ETD Date 조회시 Skip 여부 확인하도록 조건 추가
	...
	public void setQuery(){
		...
		query.append("  	   AND NVL(SKD_CNG_STS_CD,'*') <> 'S'  -- [CHM-201217556] 첫번째 Port ETD Date 조회시 Skip 여부 확인하도록 조건 추가 @2012.04.27" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#elseif (${methodname} == 'searchVslCd')" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("/*VSL CD에 해당하는 결과건수를 리턴한다*/" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	SELECT COUNT(VSL_CD) AS VSL_CD_CNT " ).append("\n"); 

...
*@FileName : InVesselInfoVO.java
*@FileTitle : InVesselInfoVO
*Open Issues :
*Change history :
...
public class InVesselArrivalVO extends VesselArrivalVO   {

	private static final long serialVersionUID = 1L;
	
	private Collection<InVesselArrivalVO> models = new ArrayList<InVesselArrivalVO>();
	...
	public void fromRequest(HttpServletRequest request) {
		...
		setIdaVslImoNo(JSPUtil.getParameter(request, "ida_vsl_imo_no", ""));
		setIdaCstmsHusNo(JSPUtil.getParameter(request, "ida_cstms_hus_no", ""));
		setIdaTmlOprNo(JSPUtil.getParameter(request, "ida_tml_opr_no", ""));
		setIdaPortGnteNo(JSPUtil.getParameter(request, "ida_port_gnte_no", ""));
		setIdaPortGnteDt(JSPUtil.getParameter(request, "ida_port_gnte_dt", ""));
	}

	public InVesselArrivalVO[] fromRequestGrid(HttpServletRequest request) {
		return fromRequestGrid(request, "");
	}

	public InVesselArrivalVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		InVesselArrivalVO model = null;
		...
		try {
			...
			String[] arrDt2 = (JSPUtil.getParameter(request, prefix	+ "arr_dt2".trim(), length));
			...
			for (int i = 0; i < length; i++) {
				...
				if (idaPortGnteDt[i] != null)
					model.setIdaPortGnteDt(idaPortGnteNo[i]);
				models.add(model);
			}
		...
		return getInVesselArrivalVOs();
	}

	public InVesselArrivalVO[] getInVesselArrivalVOs(){
		InVesselArrivalVO[] vos = (InVesselArrivalVO[])models.toArray(new InVesselArrivalVO[models.size()]);
		return vos;
	}

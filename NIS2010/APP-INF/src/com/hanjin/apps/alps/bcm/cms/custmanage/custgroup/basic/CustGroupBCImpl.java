 ...
 * 
 * @author Lim Jaekwan
 * @see BCM_CMS_0306EventResponse,CustGroupBC 각 DAO 클래스 참조
 * @since J2EE 1.4
 */
public class CustGroupBCImpl   extends BasicCommandSupport implements CustGroupBC {
	...
	/**
	 * Customer Coderetrieve.(BCM_CMS_0306)<br>
	 * 
	 * @param String custCd
	...
	public void sendCustGrpToMdm(String custCd, String usrId, String creFlag) throws EventException {
		...
		try{
			CustomerGroupIfVO customerGroupIfVO = dbDao.searchCustPerfInterface(custCd);
			log.debug("Customer EAI Start : ");		
			if (customerGroupIfVO != null){
				eaiDao.sendCustGrpToMdm(customerGroupIfVO, usrId, creFlag);
			}
		}catch(DAOException ex) {
	 ...
	 * Customer Performance Group  (BCM_CMS_0306.do)<br>
	 * 
	...
	public void manageCustPerfCode(CustomerPerformanceVO customerPerformanceVO, SignOnUserAccount account) throws EventException {
		try {		
			if(customerPerformanceVO.getIbflag().equals("U")){
			    ...
			    sendCustGrpToMdm(customerPerformanceVO.getCustGrpId(), account.getUsr_id(), "U");
			}
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
...
/*	*//**
	 ...
	 * Group customer (BCM_CMS_0306.do)<br>
	 * 

...
public class BCM_CCD_2002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_2001HTMLAction 객체를 생성
	 */
	public BCM_CCD_2002HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd2002Event event = new BcmCcd2002Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setMdmDatProcVO((MdmDatProcVO)getVO(request, MdmDatProcVO .class,""));
		} else if(command.isCommand(FormCommand.SEARCH05)) {	// check Legacy Code(SAP ID)
			event.setMdmDatProcVO((MdmDatProcVO)getVO(request, MdmDatProcVO .class,""));
			event.setMdmDatProcVOS((MdmDatProcVO[])getVOs(request, MdmDatProcVO .class,"sheet1_"));
		} else if(command.isCommand(FormCommand.MULTI01)) {	// Approval
			event.setMdmDatProcVO((MdmDatProcVO)getVO(request, MdmDatProcVO .class,""));
			event.setMdmDatProcVOS((MdmDatProcVO[])getVOs(request, MdmDatProcVO .class,"sheet1_"));
		} else if(command.isCommand(FormCommand.MULTI02)) {	// Reject
			event.setMdmDatProcVO((MdmDatProcVO)getVO(request, MdmDatProcVO .class,""));
			event.setMdmDatProcVOS((MdmDatProcVO[])getVOs(request, MdmDatProcVO .class,"sheet1_"));
		} else if(command.isCommand(FormCommand.MULTI03)) { // ReOpen
			event.setMdmDatProcVO((MdmDatProcVO)getVO(request, MdmDatProcVO .class,""));
		} else if(command.isCommand(FormCommand.REMOVE)) {	// Delete
			event.setMdmDatProcVOS((MdmDatProcVO[])getVOs(request, MdmDatProcVO .class,"sheet2_"));
		}

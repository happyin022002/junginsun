...
*@LastVersion :
=========================================================*/
package com.hanjin.apps.alps.bcm.ccd.ccdcommon.ccdcommon.basic;
...
import com.hanjin.apps.alps.bcm.ccd.ccdcommon.ccdcommon.integration.CcdCommonDBDAO;
import com.hanjin.apps.alps.bcm.ccd.ccdcommon.ccdcommon.integration.CcdCommonEAIDAO;
import com.hanjin.apps.alps.bcm.ccd.ccdcommon.ccdcommon.vo.ComboListVO;
...
//        }
	...
	/**
	 * Vessel Container 정보를 EAI로 전송한다. <BR>
	 * @param String eaiIfTp
	 * @param String eaiIfMsg
	 * @param ContainerVesselIfVO cntrVslVo
	 */
	public void sendContainerVesselToMdm(String vslCd, String usrId, String cudFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			ContainerVesselMainIfVO cntrVslVo = dbDao.searchVesselContainerIf(vslCd);
			log.debug("Vessel Container EAI Start : ");		
			if (cntrVslVo != null){
				eaiDao.sendContainerVesselToMdm(cntrVslVo, usrId, cudFlag);
			}
	...
	/**
	 * DayLight Saving Time 정보를 EAI로 전송한다. <BR>
	 * @param String eaiIfTp
	 * @param String eaiIfMsg
	 * @param ContainerVesselIfVO cntrVslVo
	 */
	public void sendDyLgtSavTmToMdm(String dstId, String usrId, String cudFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			DaySavingTimeIfVO DyLgtSavTmVo = dbDao.searchDyLgtSavTmIf(dstId);
			log.debug("DayLight Saving Time EAI Start : ");		
			if (DyLgtSavTmVo != null){
				eaiDao.sendDyLgtSavTmToMdm(DyLgtSavTmVo, usrId, cudFlag);
			}
		}catch(DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler(ex).getMessage());
	...
	/**
	 * Leasing Company Yard 정보를 EAI로 전송한다. <BR>
	 * @param String eaiIfTp
	 * @param String eaiIfMsg
	 * @param LseComYardIfVO lseCoYdVo
	 */
	public void sendLseCoYdToMdm(String lseCoYdCd, String usrId, String cudFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			LseComYardIfVO lseCoYdVo = dbDao.searchLseCoYdIf(lseCoYdCd);
			log.debug("Leasing Company Yard EAI Start : ");		
			if (lseCoYdVo != null){
				eaiDao.sendLseCoYdToMdm(lseCoYdVo, usrId, cudFlag);
			}
		}catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
	...
	/**
	 * Organization 정보를 EAI로 전송한다. <BR>
	 * @param String eaiIfTp
	 * @param String eaiIfMsg
	 * @param OfficeMainIfVO ofcMainIfVO
	 */
	public void sendOfcToMdm(String ofcCd, String usrId, String cudFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			OfficeMainIfVO ofcMainIfVO = dbDao.searchOfcCodeIf(ofcCd);
			log.debug("Organization EAI Start : ");		
			if (ofcMainIfVO != null){
				eaiDao.sendOfcToMdm(ofcMainIfVO, usrId, cudFlag);
			}
		}catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
	 ...
	 * @exception EventException
	 */
	public String checkCtrCode(String vndrCd) throws EventException{
		DBRowSet rowSet = null;
        String check = "";
        
        try {
            rowSet=dbDao.checkCtrCode(vndrCd);
            if(rowSet!=null) {
            	while(rowSet.next()){
            		check = rowSet.getString(1);
            	}

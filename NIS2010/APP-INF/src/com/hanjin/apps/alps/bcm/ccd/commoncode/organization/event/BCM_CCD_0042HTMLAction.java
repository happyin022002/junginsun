...
*@FileName : BCM_CCD_0042HTMLAction.java
...
public class BCM_CCD_0042HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0042HTMLAction 객체를 생성
	 */
	public BCM_CCD_0042HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0042Event event = new BcmCcd0042Event();
		
		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setOfcGrpId(request.getParameter("ofc_grp_id"));
			event.setSybSysCd(request.getParameter("sub_sys_cd"));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setOfcAccGrpMapVOs((OfcAccGrpMapVO[])getVOs(request, OfcAccGrpMapVO.class,""));

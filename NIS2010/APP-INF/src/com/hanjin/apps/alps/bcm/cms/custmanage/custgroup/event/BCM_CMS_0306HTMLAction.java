...
*@FileName : BCM_CMS_0306HTMLAction.java
*@FileTitle : customer
*Open Issues :
*Change history :
...
public class BCM_CMS_0306HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CMS_0302HTMLAction 객체를 생성
	 */
	public BCM_CMS_0306HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCms0306Event event = new BcmCms0306Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setCustomerPerformanceVO((CustomerPerformanceVO)getVO(request, CustomerPerformanceVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomerPerformanceVO((CustomerPerformanceVO)getVO(request, CustomerPerformanceVO.class));
			event.setCustomerGroupCodeVOS((CustomerGroupCodeVO[])getVOs(request, CustomerGroupCodeVO .class,"t2bsheet2_"));

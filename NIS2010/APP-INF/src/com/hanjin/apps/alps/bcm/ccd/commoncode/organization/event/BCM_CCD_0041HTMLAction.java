...
*@FileName : BCM_CCD_0041HTMLAction.java
...
public class BCM_CCD_0041HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0041HTMLAction 객체를 생성
	 */
	public BCM_CCD_0041HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0041Event event = new BcmCcd0041Event();
		
		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setAccGrpId(request.getParameter("acc_grp_id"));
			event.setSybSysCd(request.getParameter("sub_sys_cd"));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setOfcAccGrpVOs((OfcAccGrpVO[])getVOs(request, OfcAccGrpVO.class,""));
		}

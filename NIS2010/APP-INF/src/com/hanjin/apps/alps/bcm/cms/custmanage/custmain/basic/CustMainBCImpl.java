...
import com.hanjin.apps.alps.bcm.cms.custmanage.custmain.vo.ComboListVO;
...
public class CustMainBCImpl extends BasicCommandSupport implements CustMainBC {
    ...
    public List<SearchCustomerVO> searchCustomerList(String custCd, String custNm, String ofcCd, int iPage, String include, String cust
        ...
        try {
			return  dbDao.searchCustomerList(custCd, custNm, ofcCd, iPage, include, cust
											, zipCd, custGrpId, locCd, steCd, srepCd
											, custLoclLangNm, custRgstNo
											, deltFlg);
	...
	public List<CreateMdmCustRepVO > searchCreditCustRepList(String custCntCd, String custSeq )throws EventException {
		try {
			return dbDao.searchCreditCustRepList(custCntCd, custSeq );
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
	...
	public String checkRgstNo (String custRgstNo, String rqstNo) throws EventException{
        ...
        try {
            rowSet=dbDao.checkRgstNo(custRgstNo, rqstNo);
            if(rowSet!=null) {
            	while(rowSet.next()){
            		check = rowSet.getString(1);
            	}
	...
	/**
	 * Customer (MDM_CUST_ADDR) 테이블의 MAX ADDR_SEQ 를 가져온다.(BCM_CMS_0302)<br>
	 ...
	 * Customer Address (BCM_CMS_0302.do)<br>
	 * 
...
//	                int ld_addr_seq = 0;
					if(custAddrVOs[i].getIbflag().equals("U")){
	                    ...
	                    custAddrVOs[i].setCudFlg("C");
	                    dbDao.addCustAddrCode(custAddrVOs[i]);
			        }else if(custAddrVOs[i].getIbflag().equals("D")){
			        	//custAddrVOs[i].setCreUsrId(account.getUsr_id());
						custAddrVOs[i].setUpdUsrId(account.getUsr_id());
			        	custAddrVOs[i].setDeltFlg("Y");
			        	custAddrVOs[i].setCudFlg("D");
			        	dbDao.removeCustAddrCode(custAddrVOs[i]);
			        }
					
					sendCustAddrToMdm(custAddrVOs[i].getCustCntCd(), custAddrVOs[i].getCustSeq(), custAddrVOs[i].getAddrSeq(), account.getUsr_id(), custAddrVOs[i].getCudFlg());
				}
			}
			return custAddrVOs;
	...
	public void manageCustRep(CreateMdmCustRepVO[] createMdmCustRepVOs, SignOnUserAccount account) throws EventException {
		...
		try {	
			if(createMdmCustRepVOs!=null){
				for(int i=0; i < createMdmCustRepVOs.length; i++) {
					if(createMdmCustRepVOs[i].getIbflag().equals("U")){
						createMdmCustRepVOs[i].setUpdUsrId(account.getUsr_id());
						dbDao.modifyMdmCustRep(createMdmCustRepVOs[i]);
			        }else if(createMdmCustRepVOs[i].getIbflag().equals("I")){
			        	createMdmCustRepVOs[i].setCreUsrId(account.getUsr_id());
			        	createMdmCustRepVOs[i].setUpdUsrId(account.getUsr_id());
	                    dbDao.addMdmCustRep(createMdmCustRepVOs[i]);
			        }
				}
			}
			return ;
	...
	public boolean mergeBkgCustCdVal(CustomerVO customerVo)throws EventException{
		...
		try {
			isSuccessful = dbDao.mergeBkgCustCdVal(customerVo);
        }catch (DAOException de) {
	...
	/**
	 * Modify/save/delete event process<br>
	 * Customer Contact Point Code (BCM_CMS_0302.do)<br>
	 * 
	...
	public void sendCreditCustToMdm(String custCntCd, String custSeq, String usrId, String creFlag) throws EventException {
		...
		try{
			CreditCustomerIfVO creditCustomerIfVO = dbDao.searchCrCustInterface(custCntCd, custSeq);
			log.debug("Customer EAI Start : ");		
			if (creditCustomerIfVO != null){
				eaiDao.sendCreditCustToMdm(creditCustomerIfVO, usrId, creFlag);
			}
		}catch(DAOException ex) {
            log.error("err " + ex.toString(), ex);
	...
	public void sendCustomerToMdm(String custCntCd, String custSeq, String usrId, String creFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			CustomerIfVO customerIfVO = dbDao.searchIfCustCode(custCntCd, custSeq);
			log.debug("Customer EAI Start : ");		
			if (customerIfVO != null){
				eaiDao.sendCustomerToMdm(customerIfVO, usrId, creFlag);
			}
		}catch(DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler(ex).getMessage());
	 ...
	 * @param String creFlag
	 */
	public void sendCustAddrToMdm(String custCntCd, String custSeq, String addrSeq, String usrId, String creFlag) throws EventException {
		// error에 대한 추가 처리이므호 exception을 throw하지 않음
		try{
			CustomerAddressIfVO customerAddressIfVO = dbDao.searchIfCustAddr(custCntCd, custSeq, addrSeq);
			log.debug("Customer EAI Start : ");		
			if (customerAddressIfVO != null){
				eaiDao.sendCustAddrToMdm(customerAddressIfVO, usrId, creFlag);
			}
		}catch(DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler(ex).getMessage());
	...
	/**
	 * Modify/save/delete event process<br>
	 * Customer Code Creation (BCM_CMS_0302.do)<br>
	 * 
	...
	public void modifyCreditCustCode(String custCntCd, String custSeq) throws EventException {
		try {
			dbDao.modifyCreditCustCode(custCntCd, custSeq);	
	...
	public List<BlHistVO> searchCreditCustHist(String custCd) throws EventException {
		...
		try {
			return dbDao.searchCreditCustHist(custCd);
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
	...
	/**
	 * Credit Customer Code retrieve.(BCM_CMS_0311)<br>
	 * 
	 * @param String custCntCd
	 * @param String custSeq
	...
	/**
	 * Modify/save/delete event process<br>
	 * Credit Customer Code Creation (BCM_CMS_0311.do)<br>
	 * 
	...
	public void manageCrCustCode(CreditCustomerVO creditCustomerVO,  SignOnUserAccount account) throws EventException {
		...
		try {		
			if(creditCustomerVO.getIbflag().equals("I")){
				creditCustomerVO.setCreUsrId(account.getUsr_id());
				creditCustomerVO.setUpdUsrId(account.getUsr_id());
				dbDao.addCrCustCode(creditCustomerVO);
			}else if(creditCustomerVO.getIbflag().equals("U")){
				creditCustomerVO.setUpdUsrId(account.getUsr_id());
				dbDao.modifyCrCustCode(creditCustomerVO);	
	        }
			
			// Create Refund Customer for ERP
			if (creditCustomerVO.getRfndPsdoVndrFlg().equals("Y") && creditCustomerVO.getRfndPsdoVndrSeq().length() == 0) {
				dbDao.modifyRefundCustCode(creditCustomerVO);
				sendCustomerToMdm(creditCustomerVO.getCustCntCd(), creditCustomerVO.getCustSeq(), account.getUsr_id(), "U");
			}

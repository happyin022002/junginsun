...
public class MNR_GCA_INTERFACE_HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * MNR_COM_HTMLAction 객체를 생성
	 */  
	public MNR_GCA_INTERFACE_HTMLAction() {}	
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(request.getContentType()!=null && request.getContentType().toLowerCase().startsWith(SubSystemConfigFactory.get("MNR.CONTENTS.TYPE.MULTIPART").trim())){
			try {
				...
				fileUpload = new FileUpload(request,"GCA", SubSystemConfigFactory.get("MNR.FILE.UPLOAD.ENCODING"), event, false);

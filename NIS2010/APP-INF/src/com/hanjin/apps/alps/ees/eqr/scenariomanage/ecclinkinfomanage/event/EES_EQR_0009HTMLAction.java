...
*@FileName : EES_EQR_0009HTMLAction.java
...
public class EES_EQR_0009HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_009HTMLAction 객체를 생성
	 */
	public EES_EQR_0009HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0009Event event = new EesEqr0009Event();
		
		// Retrieve 버튼 클릭시
		if(command.isCommand(FormCommand.SEARCHLIST)) {
			event.setEesEqr009ConditionVO((EesEqr0009ConditionVO)getVO(request, EesEqr0009ConditionVO .class));
		}
		
		// Save 버튼 클릭시
		else if (command.isCommand(FormCommand.MULTI)) {
			event.setEesEqr009ConditionVO((EesEqr0009ConditionVO)getVO(request, EesEqr0009ConditionVO .class));
			event.setEqrScnrEccLnkVOS(new EqrScnrEccLnkVO().fromRequestGrid(request, ""));
		}

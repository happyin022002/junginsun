...
*@FileName : OnhireApprovalDBDAOseachEqrReqListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class OnhireApprovalDBDAOseachEqrReqListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * EQR의 Request No 목록을 가져온다.
	  * </pre>
	  */
	public OnhireApprovalDBDAOseachEqrReqListRSQL(){
		setQuery();
		...
		params.put("titlelist",new String[]{arrTmp[0],arrTmp[1]});

		query.append("/*").append("\n"); 
		query.append("Path : com.hanjin.apps.alps.ees.lse.containerleasemgt.onhireapproval.integration").append("\n"); 
		query.append("FileName : OnhireApprovalDBDAOseachEqrReqListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("AND   A.LSE_RQST_NO NOT IN (SELECT LSE_RQST_NO FROM LSE_ONH_APRO WHERE DELT_FLG ='N' AND LSE_RQST_NO IS NOT NULL)" ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("@[titlelist] TITLELIST," ).append("\n"); 
		query.append("#foreach($key IN ${cntr_tpsz_cd})" ).append("\n"); 
		query.append("#if($velocityCount < $cntr_tpsz_cd.size())" ).append("\n"); 

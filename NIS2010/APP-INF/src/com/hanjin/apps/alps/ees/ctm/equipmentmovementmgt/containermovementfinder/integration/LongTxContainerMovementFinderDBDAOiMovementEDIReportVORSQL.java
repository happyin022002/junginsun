...
public class LongTxContainerMovementFinderDBDAOiMovementEDIReportVORSQL implements ISQLTemplate{
	...
	public LongTxContainerMovementFinderDBDAOiMovementEDIReportVORSQL(){
		...
		params.put("dom_flg",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("        @[gap] GAP, @[p_date1] P_DATE1, @[p_date2] P_DATE2, @[dom_flg] DOM_FLG, @[data_by] DATA_BY, SUM(NVL(L_TOT,0)) TOT, SUM(NVL(ML_TOT,0)) MTOT, SUM(NVL(EL_TOT,0)) ETOT, SUM(NVL(SL_TOT,0)) STOT," ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("      SELECT RCC_CD, LCC_CD, CNT_CD, LOC_CD, ORG_YD_CD, SUM(OFF_12) OFF_12, SUM(OFF_24) OFF_24, SUM(OFF_48) OFF_48, SUM(OVR_48) OVR_48, SUM(L_TOT) L_TOT" ).append("\n"); 
		query.append("      FROM (" ).append("\n"); 
		query.append("            SELECT /*+ USE_NL(CTM LOC EQ) */ EQ.RCC_CD, EQ.LCC_CD, LOC.CNT_CD, LOC.LOC_CD, ORG_YD_CD, OFF_12, OFF_24, OFF_48, OVR_48, (OFF_12 + OFF_24 + OFF_48 + OVR_48) L_TOT" ).append("\n"); 
		query.append("            FROM (" ).append("\n"); 
		query.append("                SELECT ORG_YD_CD, OFF_12, OFF_24, OFF_48, OVR_48" ).append("\n"); 
		query.append("                FROM" ).append("\n"); 
		query.append("                  (" ).append("\n"); 
		query.append("                    SELECT ORG_YD_CD, OFF_12, OFF_24, OFF_48, OVR_48" ).append("\n"); 
		query.append("                    FROM (" ).append("\n"); 
		query.append("                        SELECT   ORG_YD_CD, SUM(OFF_12) OFF_12, SUM(OFF_24) OFF_24, SUM(OFF_48) OFF_48, SUM(OVR_48) OVR_48" ).append("\n"); 
		query.append("                        FROM  (SELECT ORG_YD_CD," ).append("\n"); 
		query.append("                              CASE WHEN OFFSET_DT <= 0.5                     THEN 1 ELSE 0 END OFF_12," ).append("\n"); 
		query.append("                              CASE WHEN OFFSET_DT >  2                       THEN 1 ELSE 0 END OVR_48," ).append("\n"); 
		query.append("                              CASE WHEN OFFSET_DT >  0.5  AND OFFSET_DT <= 1 THEN 1 ELSE 0 END OFF_24," ).append("\n"); 
		query.append("                              CASE WHEN OFFSET_DT >  1    AND OFFSET_DT <= 2 THEN 1 ELSE 0 END OFF_48" ).append("\n"); 
		query.append("                           FROM (SELECT /*+ INDEX ( A XAK1CTM_MOVEMENT)*/ ORG_YD_CD, (CRE_LOCL_DT - CNMV_EVNT_DT) OFFSET_DT" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("                              FROM  (SELECT ORG_YD_CD," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='EDI' AND OFFSET_DT <= 0.5 THEN 1 ELSE 0 END EOFF_12," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='EDI' AND OFFSET_DT > 2      THEN 1 ELSE 0 END EOVR_48," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='EDI' AND OFFSET_DT > 0.5    AND OFFSET_DT <= 1 THEN 1 ELSE 0 END EOFF_24," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='EDI' AND OFFSET_DT > 1     AND OFFSET_DT <= 2 THEN 1 ELSE 0 END EOFF_48," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='SPP' AND OFFSET_DT <= 0.5 THEN 1 ELSE 0 END SOFF_12," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='SPP' AND OFFSET_DT > 2      THEN 1 ELSE 0 END SOVR_48," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='SPP' AND OFFSET_DT > 0.5    AND OFFSET_DT <= 1 THEN 1 ELSE 0 END SOFF_24," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='SPP' AND OFFSET_DT > 1     AND OFFSET_DT <= 2 THEN 1 ELSE 0 END SOFF_48," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='MAN' AND OFFSET_DT <= 0.5 THEN 1 ELSE 0 END MOFF_12," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='MAN' AND OFFSET_DT > 2      THEN 1 ELSE 0 END MOVR_48," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='MAN' AND OFFSET_DT > 0.5    AND OFFSET_DT <= 1 THEN 1 ELSE 0 END MOFF_24," ).append("\n"); 
		query.append("                                                 CASE WHEN MVMT_INP_TP_CD='MAN' AND OFFSET_DT > 1     AND OFFSET_DT <= 2 THEN 1 ELSE 0 END MOFF_48" ).append("\n"); 
		query.append("                                 FROM (SELECT /*+ INDEX ( A XAK1CTM_MOVEMENT)*/ ORG_YD_CD, (CRE_LOCL_DT - CNMV_EVNT_DT) OFFSET_DT, MVMT_INP_TP_CD" ).append("\n"); 
		query.append("                                         FROM CTM_MOVEMENT A" ).append("\n"); 
		query.append("                                        #if (${rcc_cd} != 'ALL')" ).append("\n"); 
		query.append("                                        WHERE SYS_AREA_GRP_ID = (SELECT SYS_AREA_GRP_ID FROM COM_SYS_AREA_GRP_ID WHERE CNT_CD = SUBSTR(@[rcc_cd],0,2) AND SUBSTR(SYS_AREA_GRP_ID,0,1) != 'D')" ).append("\n"); 
		query.append("                                        #else" ).append("\n"); 
		...
		query.append("  AND A.ORG_YD_CD = B.ORG_YD_CD(+)" ).append("\n"); 
		query.append("  #if (${rcc_cd} != 'ALL')" ).append("\n"); 

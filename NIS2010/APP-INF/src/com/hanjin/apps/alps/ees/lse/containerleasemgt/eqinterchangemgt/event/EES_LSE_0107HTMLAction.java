...
*@FileName : EES_LSE_0107HTMLAction.java
...
public class EES_LSE_0107HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_LSE_0107HTMLAction 객체를 생성
	 */
	public EES_LSE_0107HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesLse0107Event event = new EesLse0107Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setEqInterchangeReqVO((EqInterchangeReqVO)getVO(request, EqInterchangeReqVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setEqInterchangeReqVO((EqInterchangeReqVO)getVO(request, EqInterchangeReqVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setEqInterchangeReqVO((EqInterchangeReqVO)getVO(request, EqInterchangeReqVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setEqInterchangeReqVOS((EqInterchangeReqVO[])getVOs(request, EqInterchangeReqVO.class));
		}
		return  event;
	}

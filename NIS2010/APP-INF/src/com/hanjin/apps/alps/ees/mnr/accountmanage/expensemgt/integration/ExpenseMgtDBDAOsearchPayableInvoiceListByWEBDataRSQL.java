...
public class ExpenseMgtDBDAOsearchPayableInvoiceListByWEBDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       , ( SELECT VNDR_LGL_ENG_NM FROM MDM_VENDOR WHERE VNDR_SEQ = A.MNR_PRNR_SEQ ) AS PAY_VNDR_LGL_ENG_NM" ).append("\n"); 
		...
		query.append("           AND      DECODE(A.MNR_INV_STS_CD, 'HC', ( SELECT API.INV_STS_CD FROM  AP_PAY_INV API WHERE A.INV_RGST_NO = API.INV_RGST_NO AND API.INV_SUB_SYS_CD='MNR' AND ROWNUM = 1 ), A.MNR_INV_STS_CD) = MGC.MNR_CD_ID" ).append("\n"); 
		...
		query.append("WHERE    1 = 1" ).append("\n"); 
		query.append("AND      A.ORD_VNDR_SEQ = B.VNDR_SEQ(+)" ).append("\n"); 
		...
		query.append("AND      A.ISS_OFC_CD = O.OFC_CD" ).append("\n"); 

...
*@FileName : EesDuplicateCheckPODViewAdapter.java
*@FileTitle : EES_DUPLICATECHECKPOD 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_DUPLICATECHECKPOD 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesDuplicateCheckPODViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String row 								= "";
		String dubpliate_check    = "";
		String colname            = "";
		try{
			row    			= condiVO.getPodRow();	
			colname     	= condiVO.getPodColname();
			dubpliate_check = retVO.getDuplicateCheck();
			
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ colname +"\">"+ JSPUtil.getNull(dubpliate_check)+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

...
*@FileName : EES_MST_0017HTMLAction.java
...
public class EES_MST_0017HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_MST_0004HTMLAction 객체를 생성
	 */
	public EES_MST_0017HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesMst0017Event event = new EesMst0017Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMstEtcVO((MstEtcVO)getVO(request, MstEtcVO .class));
			
			String deyrmon = event.getMstEtcVO().getDeYrmon();
			int idx = deyrmon.indexOf("-");
			if (idx != 0 && idx != -1) {
				String tmpyrmon = deyrmon.substring(0, idx);
				tmpyrmon = tmpyrmon + deyrmon.substring(idx+1, deyrmon.length());
				event.getMstEtcVO().setDeYrmon(tmpyrmon);
			} else if (deyrmon.length()  ==  6){
				event.getMstEtcVO().setDeYrmon(deyrmon);
			}			
		}
		else if(command.isCommand(FormCommand.MULTI)){
			event.setFaTargetListVOs((FaTargetListVO[])getVOs(request, FaTargetListVO .class,""));			
		}

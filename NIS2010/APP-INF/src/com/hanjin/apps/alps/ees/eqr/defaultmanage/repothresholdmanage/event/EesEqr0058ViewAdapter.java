...
*@FileName : EesEqr0058ViewAdapter.java
*@FileTitle : 실행 계획 Feedback 기준 설정
*Open Issues :
*Change history :
* No.	Ver.		Modifier           		modifier date    explanation
* 1      	1.0      	Lee Byoung Hun	2009.07.16		1.0 최초 생성
*
...
/**
 * EES_EQR_0058 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesEqr0058ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(DBRowSet rs,String prefix) {
		...
		try{
			String[] changedColNms = getChangedColNms(realColNms, prefix);
			String firstColName = changedColNms[0];
			sb.append("<DATA COLORDER='" + JSPUtil.convertStringArrayToString(changedColNms, "|") + "' TOTAL='" + getRowSetCnt(rs) + "'>\n");
			
			int colCount = realColNms.length;
			int i =1;
			while (rs.next()) { 
				String timegap = JSPUtil.getNull(rs.getString("timegap"));
				sb.append("	<TR>");
				for (int j = 0 ; j < colCount ; j++) {
					if ("loc_grp_cd".equals(firstColName) && "Y".equals(timegap)) {
						sb.append("<TD BGCOLOR=\"YELLOW\">" + JSPUtil.getNull(rs.getString(i++)) + "</TD>");
					} else {

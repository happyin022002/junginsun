...
*@FileName : EesVvdCommonViewAdapter.java
*@FileTitle : EES_VVD_COMMON 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_VVD_COMMON 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesVvdCommonViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String[] vvd    						= null;
		try{
			row    = condiVO.getVvdRow();
			colname= condiVO.getVvdColname();
			vvd    = retVO.getVvdResult();
			
			if(!JSPUtil.getNull(vvd[0]).equals("")) rowCount=1;
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ colname +"\" DATA-TYPE=\"dtComboEdit\" COMBO-TEXT=\""+JSPUtil.getNull(vvd[0])+"\" COMBO-CODE=\""+JSPUtil.getNull(vvd[0])+"\" ></TD>");
			sb.append("	<TD COL=\""+ colname +"_cnt\" DATA-TYPE=\"dtData\" >"+rowCount+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

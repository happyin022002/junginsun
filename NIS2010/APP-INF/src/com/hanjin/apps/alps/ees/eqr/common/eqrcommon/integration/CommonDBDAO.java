...
* 2      1.124    yongchan shin    2008-01-17      CSR No : R200801154868 - LOC CODE 체크로직 보완
* 3      1.125    Jeonghwa Kwon  2008-02-25      CSR No :  N200801250015 - HJS Rule 선택시 Fm/To pair로 입력가능하게 함. 
* 4      1.126    yongchan shin    2008-03-26      CSR NO : N200803260007 - plan year week 안에 들어오는 ETD만 검색하는 방식으로 변경
* 5      1.127    Jeonghwa Kwon  2008-03-27      CSR No :  R2000803265677 - ENIS XA JDBC  Driver 구성에 따른, EQR WTC Application 수정 및 적용 
* 6      1.129    chae chang ho   2008-06-25      CSR No : N200806030017 - Match-Back Performance Inquiry
* 7      1.131    chae chang ho   2008-11-20      CSR No : N200811110008 - User 가 Vessel SKD 업데이트 할 수 있도록 시스템 보완.
* 8      1 132    yongchan shin    2009-01-07      CSR NO : N200901070038 - 200902주차 PLAN 없이 200903주차 PLAN을 바로 생성하므로 0117일까지만 -2주 작업을 허용
* 9      1.133    yongchan shin    2009-01-07      CSR NO : N200901080009 - 'SENCO' 를 위한 EQR09 Role 코드에 대한 EQR 설정 작업
* 10     1.134    yongchan shin    2009-01-20      CSR NO : R200901200003 - -1주 작업으로 원상복귀 
* 11     1.136    HaengJi, Lee       2009-02-10      신규프로젝트 No  - [ S2Q-09P-004 ], Bay Plan I/F 화면추가에 따른 메소드 추가 - searchBayPlan
* 12     1.137    HaengJi, Lee       2009-04-22      CSR No - N200904200110 : VVD Add 추가기능으로 인해 SEARCH10에 해당하는 로직 추가
* 13     2.0    chung eun ho       2009-07-29      new FrameWork 적용
* 14		2.1	Lee Byoung Hun	2010.05.11		CSR No : CHM-201003779 - getUserInfo 메소드 추가
=========================================================*/
...
public class CommonDBDAO extends DBDAOSupport {
	 ...
	 */
	 public CommonVO searchMaxInfo(String table, String condition) throws DAOException {
		...
		if(condition.length() > 3 && "SQL".equals(condition.substring(0, 3))){
			arrCondition = condition.split("\\|");
			condition = arrCondition[0];
			stPlnYrwk = arrCondition[1];
			endPlnYrwk = arrCondition[2];
		}
		//parameter		
		Map<String, Object> param = new HashMap<String, Object>();
		//velocity parameter
		...
		try{ 
			param.put("st_pln_yrwk", stPlnYrwk);
			param.put("end_pln_yrwk", endPlnYrwk);
			velParam.put("table_name", table);
			velParam.put("condition", condition);

			dbRowset = new SQLExecuter(strDs).executeQuery((ISQLTemplate)new CommonDBDAOSearchMaxInfoRSQL(), param, velParam);
			if (dbRowset.next()) {
	...
	*/
	public CommonVO searchLocYardInfo(String locyard_searchword) throws DAOException {
		...
		try {
			...
			dbRowset = new SQLExecuter(strDs).executeQuery((ISQLTemplate)new CommonDBDAOSearchLocYardInfoRSQL(), null, velParam);
...
//				 log.debug("\n inStr2 ---------------- "+inStr2);
				...
				velParam.put("gubun", (gubun == null ? "" : gubun) );
				velParam.put("check_gubun","Load"); 
				param.put("month", conditionVO.getGapmonth());
...
//				log.debug("new_Scnr_id =========>" + new_Scnr_id);
	...
	public CommonVO searchEtaDate(EesCommonConditionVO conditionVO) throws DAOException {
		...
		// 2015.02.25 CHM-201534210 EQR 소스 보안
		String etaDate = new String();  //return value
...
//		log.debug("-- railMode 		 : " + railMode);
		...
		try {
			...
			// modified by shin yongchan - 20100106
			// 남연호 과장 / 계은영씨 요청으로 ETA 못찾은 경우 보여주던 메세지를 표시하지 않기로 합의.
			/*
			if(etaDate==null || etaDate.equals("")) {
    			throw new DAOException(new ErrorHandler("EQR10026").getMessage());
			}
			*/

...
public class DisposalMgtDBDAOsearchDisposalInquiryListDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("        -- 2015/11/16, 신용찬 쿼리 수정" ).append("\n"); 
		query.append("        --,A.CURR_CD" ).append("\n"); 
		query.append("        ,NVL(IAA.CURR_CD, A.CURR_CD) CURR_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        ,A.VAT_AMT" ).append("\n"); 
		query.append("        ,A.WHLD_TAX_AMT" ).append("\n"); 
		query.append("		,NVL(IAA.INV_AMT, A.TTL_AMT) AS TTL_AMT" ).append("\n"); 
		query.append("        ,MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(A.ISS_DT, 'YYYYMM'), NVL(IAA.CURR_CD, A.CURR_CD), 'USD', NVL(IAA.INV_AMT, A.TTL_AMT)) AS USD_TTL_AMT" ).append("\n"); 
		...
		query.append("        ,TO_CHAR(A.INV_DUE_DT, 'yyyy-mm-dd') INV_DUE_DT" ).append("\n"); 
		query.append("        ,IAA.ERP_IF_GL_DT" ).append("\n"); 
		query.append("        ,TO_DATE(IAA.ERP_IF_GL_DT,'yyyymmdd')-TO_DATE(TO_CHAR(A.INV_DUE_DT, 'yyyymmdd') ,'yyyymmdd') AS OVER_DUE" ).append("\n"); 

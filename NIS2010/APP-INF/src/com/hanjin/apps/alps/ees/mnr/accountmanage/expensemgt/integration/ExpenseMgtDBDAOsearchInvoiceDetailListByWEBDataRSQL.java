...
*@FileName : ExpenseMgtDBDAOsearchInvoiceDetailListByWEBDataRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ExpenseMgtDBDAOsearchInvoiceDetailListByWEBDataRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ExpenseMgtDBDAOsearchInvoiceDetailListByWEBDataRSQL
	  * </pre>
	  */
	public ExpenseMgtDBDAOsearchInvoiceDetailListByWEBDataRSQL(){
		setQuery();
		...
		query.append("FileName : ExpenseMgtDBDAOsearchInvoiceDetailListByWEBDataRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        ,MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(SYSDATE, 'YYYYMM'), B.CURR_CD, @[target_curr_cd], A.COST_AMT) INV_AMT" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("        ,MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(SYSDATE, 'YYYYMM'), @[curr_cd], @[target_curr_cd], A.COST_AMT) INV_AMT" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("        ,(SELECT DP_PRCS_KNT FROM MDM_CURRENCY WHERE CURR_CD = @[target_curr_cd]) DP_PRCS_KNT " ).append("\n"); 
		query.append("        ,A.MNR_VRFY_TP_CD" ).append("\n"); 
		query.append("        ,A.ORD_DTL_RMK" ).append("\n"); 
		query.append("        ,A.INV_NO" ).append("\n"); 
		query.append("        ,A.FILE_SEQ" ).append("\n"); 
		query.append("		,NVL(W.ENV_CHG_TAX, 0) ENV_CHG_TAX -- 인도지역의 SBC Tax" ).append("\n"); 
		...
		query.append("FROM MNR_PAY_INV_TMP T, MNR_PAY_INV_WRK W, MNR_ORD_DTL A, MNR_ord_hdr B, MNR_GEN_CD C, MNR_GEN_CD E, MNR_RPR_RQST_HDR F" ).append("\n"); 
		query.append("WHERE  T.PAY_INV_SEQ = W.PAY_INV_SEQ" ).append("\n"); 
		query.append("AND T.MNR_ORD_OFC_CTY_CD = A.MNR_ORD_OFC_CTY_CD" ).append("\n"); 
		query.append("AND T.MNR_ORD_SEQ = A.MNR_ORD_SEQ" ).append("\n"); 
		query.append("AND T.ORD_DTL_SEQ = A.ORD_DTL_SEQ" ).append("\n"); 
		...
		query.append("AND	T.PAY_INV_SEQ IN (" ).append("\n"); 

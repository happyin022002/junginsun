...
public class ChassisMgsetInvoiceDBDAOSearchCHSInvoiceCreateDetailDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("#if (${chss_mgst_inv_knd_cd} == 'LS' || ${chss_mgst_inv_knd_cd} == 'ZP')  -- 2015.05.29 Chang Young Kim 운영서버와 내용이 틀림, 소스 동기화" ).append("\n"); 
		...
		query.append("        -- modified by shin yongchan, 2015-07-17, credit amount change to minus" ).append("\n"); 
		query.append("    	-- NVL(SUM(B.PAY_LSE_CHG_AMT),0) + NVL(SUM(B.PAY_CR_AMT),0) + NVL(SUM(B.PAY_TAX_AMT),0) AS INV_AMT" ).append("\n"); 
		query.append("        NVL(SUM(B.PAY_LSE_CHG_AMT),0) + NVL(SUM(B.PAY_CR_AMT),0) + NVL(SUM(B.PAY_TAX_AMT),0) AS INV_AMT  -- Credit Amount는 이미 마이너스 데이터이므로 연산 시 + 처리함" ).append("\n"); 

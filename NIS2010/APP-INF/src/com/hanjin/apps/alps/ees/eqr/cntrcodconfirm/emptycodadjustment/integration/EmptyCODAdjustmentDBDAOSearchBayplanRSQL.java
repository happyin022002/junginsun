...
*@FileName : EmptyCODAdjustmentDBDAOSearchBayplanRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class EmptyCODAdjustmentDBDAOSearchBayplanRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public EmptyCODAdjustmentDBDAOSearchBayplanRSQL(){
		setQuery();
		...
		query.append("FileName : EmptyCODAdjustmentDBDAOSearchBayplanRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("			--B.CLPT_IND_SEQ                              clptindseq," ).append("\n"); 
		query.append("			V.CLPT_IND_SEQ								clptindseq," ).append("\n"); 
		...
		query.append("			DECODE( TO_CHAR(V.VPS_ETB_DT,'YYYYMMDD'), NULL, DECODE(	S.SKD_CNG_STS_CD, 'S', 'SKIP', NULL ),TO_CHAR(V.VPS_ETB_DT,'YYYYMMDD') )	ETB," ).append("\n"); 
		query.append("			DECODE( V.CLPT_SEQ, NULL, S.CLPT_SEQ, V.CLPT_SEQ )  , " ).append("\n"); 
		...
		query.append("			SUM	( DECODE(B.CNTR_TPSZ_CD,'R9',1,0 ) )	R9		," ).append("\n"); 
		...
		query.append("			SUM	( DECODE(B.CNTR_TPSZ_CD,'A5',1,0 ) )	A5		," ).append("\n"); 
		query.append("			SUM	( DECODE(B.CNTR_TPSZ_CD,'O5',1,0 ) )	O5" ).append("\n"); 
		...
		query.append("					V.CLPT_SEQ       	/* 2010.05.10 */		" ).append("\n"); 
		...
		query.append("			AND     V.CLPT_IND_SEQ  =   (" ).append("\n"); 
		query.append("											SELECT  /*+ INDEX( VV XPKVSK_VSL_PORT_SKD ) */" ).append("\n"); 
		query.append("													VV.CLPT_IND_SEQ		" ).append("\n"); 
		query.append("											FROM	VSK_VSL_PORT_SKD   VV" ).append("\n"); 
		query.append("											WHERE	VV.VSL_CD		=	V.VSL_CD" ).append("\n"); 
		query.append("											AND		VV.SKD_VOY_NO	=	V.SKD_VOY_NO" ).append("\n"); 
		query.append("											AND		VV.SKD_DIR_CD	=	V.SKD_DIR_CD" ).append("\n"); 
		query.append("											AND     VV.VPS_PORT_CD  =   V.VPS_PORT_CD" ).append("\n"); 
		query.append("											AND     NVL(VV.SKD_CNG_STS_CD,' ')	<> 'S'    " ).append("\n"); 
		query.append("											AND     ROWNUM          =   1" ).append("\n"); 
		query.append("										)" ).append("\n"); 
		query.append("		)					V," ).append("\n"); 
		query.append("		VSK_VSL_PORT_SKD	S			/* 2010.05.10 */" ).append("\n"); 
		...
		query.append("--	AND     B.CLPT_IND_SEQ  =   V.CLPT_IND_SEQ(+)" ).append("\n"); 
		...
		query.append("	AND		B.SKD_VOY_NO	=	S.SKD_VOY_NO	(+)" ).append("\n"); 
		query.append("	AND		B.SKD_DIR_CD	=	S.SKD_DIR_CD	(+)" ).append("\n"); 
		query.append("	AND     B.POD_CD        =   S.VPS_PORT_CD	(+)" ).append("\n"); 
		query.append("	AND     '1'				=   S.CLPT_IND_SEQ	(+)" ).append("\n"); 
		query.append("	/* <--------------- 2010.05.10 */	" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("			/* 2010.05.10 ----------------> */		" ).append("\n"); 
		query.append("			DECODE( TO_CHAR(V.VPS_ETB_DT,'YYYYMMDD'), NULL, DECODE(	S.SKD_CNG_STS_CD, 'S', 'SKIP', NULL )," ).append("\n"); 

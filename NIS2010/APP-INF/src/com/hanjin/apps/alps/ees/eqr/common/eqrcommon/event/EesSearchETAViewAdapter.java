...
*@FileName : EesSearchETAViewAdapter.java
*@FileTitle : EES_SEARCH_ETA 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_SEARCH_ETA 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesSearchETAViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String eta 								= "";
		try{
			row    	= condiVO.getSearchetaRow();				
			colname	= condiVO.getSearchetaColname();
			eta		= retVO.getEtadate();
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ colname +"\" >"+JSPUtil.getNull(eta)+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

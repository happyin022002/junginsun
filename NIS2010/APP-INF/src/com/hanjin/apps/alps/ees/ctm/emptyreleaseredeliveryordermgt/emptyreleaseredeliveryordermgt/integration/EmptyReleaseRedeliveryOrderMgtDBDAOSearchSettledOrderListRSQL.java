...
public class EmptyReleaseRedeliveryOrderMgtDBDAOSearchSettledOrderListRSQL implements ISQLTemplate{
	...
	public EmptyReleaseRedeliveryOrderMgtDBDAOSearchSettledOrderListRSQL(){
		...
		params.put("row_cnt",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("first_pagerows",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("       BB.POR_CD AS POR_CD, " ).append("\n"); 
		query.append("       BB.DEL_CD AS DEL_CD,  " ).append("\n"); 
		query.append("       ORD.DOR_NOD_CD AS DOR_NOD_CD," ).append("\n"); 
		query.append("	   COUNT(*) OVER() TOTAL_CNT," ).append("\n"); 
		query.append("       ROW_NUMBER() OVER(ORDER BY STK.BKG_NO ASC) RNUM" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("     AND STK.CNTR_TPSZ_CD IN (${cntr_tpsz_cd})" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${bkg_chk} == 'Y' && ${tro_chk} == 'Y')   " ).append("\n"); 
		query.append("    AND STK.TRSP_SO_TP_CD = 'C'" ).append("\n"); 
		query.append("    AND ( ( ( ORD.TRSP_BND_CD = 'O' AND BB.POR_CD <> BB.POL_CD ) OR ( ORD.TRSP_BND_CD = 'I' AND BB.POD_CD <> BB.DEL_CD ) ) " ).append("\n"); 
		query.append("       OR ( ( ORD.TRSP_BND_CD = 'O' AND BB.POR_CD = BB.POL_CD  ) OR ( ORD.TRSP_BND_CD = 'I' AND BB.POD_CD = BB.DEL_CD  ) ) " ).append("\n"); 
		query.append("        )" ).append("\n"); 
		query.append("#elseif (${bkg_chk} == 'Y' && ${tro_chk} != 'Y')   " ).append("\n"); 
		query.append("   AND STK.TRSP_SO_TP_CD = 'C'" ).append("\n"); 
		query.append("   AND ( ( ORD.TRSP_BND_CD = 'O' AND BB.POR_CD <> BB.POL_CD ) OR ( ORD.TRSP_BND_CD = 'I' AND BB.POD_CD <> BB.DEL_CD ) ) " ).append("\n"); 
		query.append("#elseif (${bkg_chk} != 'Y' && ${tro_chk} == 'Y')       " ).append("\n"); 
		query.append("   AND STK.TRSP_SO_TP_CD = 'C'" ).append("\n"); 
		query.append("   AND ( ( ORD.TRSP_BND_CD = 'O' AND BB.POR_CD = BB.POL_CD ) OR ( ORD.TRSP_BND_CD = 'I' AND BB.POD_CD = BB.DEL_CD ) )  " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${mh_chk} == 'Y')      " ).append("\n"); 
		query.append("   AND STK.TRSP_SO_TP_CD = 'M'" ).append("\n"); 
		query.append("#end         " ).append("\n"); 
		query.append("#if (${mt_repo_chk} == 'Y')   " ).append("\n"); 
		query.append("   AND STK.TRSP_SO_TP_CD = 'R'" ).append("\n"); 
		query.append("#end " ).append("\n"); 
		query.append("#if (${st_chk} == 'Y')   " ).append("\n"); 
		query.append("   AND STK.TRSP_SO_TP_CD = 'S'" ).append("\n"); 
		query.append("#end   " ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("ORDER BY STK.BKG_NO ASC" ).append("\n"); 
		query.append("   ) " ).append("\n"); 
		query.append("WHERE 1 = 1" ).append("\n"); 
		query.append("#if (${page_no} == '1')" ).append("\n"); 
		query.append("AND RNUM BETWEEN 1 AND @[first_pagerows]" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("AND RNUM BETWEEN @[row_cnt] + 1 AND @[row_cnt] + @[pagerows]" ).append("\n"); 
		query.append("#end" ).append("\n"); 

...
import com.hanjin.apps.alps.ees.eqr.cntrfcstsimul.forecastreport.integration.ForecastReportDBDAO;
			...
			if(divFlag.equals("0")) { // sheet의 왼쪽 + 버튼 클릭
				...
				// RCC 선택되고 라디오 버튼 1 선택, Location By 입력값 있음
					...
					if(locTypeCode.equals("RE")) {       // RCC (by ECC)
						...
						presentFlag   = "S";
						conditionFlag = "L";
					}else if(locTypeCode.equals("RS")) { // RCC (by SCC)
						presentFlag   = "S";
						conditionFlag = "R";							
					}else {                              // ECC (by SCC)
						presentFlag   = "S";
...
//						conditionFlag = "E";							
//					}
//				// RCC 선택되고 라디오버튼 2 선택, Location 입력값 있음	
//				}else if(divFlag.equals("2") && locCdSecond != null) {
//					conditionValue = locCdSecond;  // 조회값
//					
//					if(locTpCdSecond.equals("L")) {        // LCC
...
//			eesEqr1048ConditionVO.setCurrFlag(currFlag);						
				...
				for ( int i=0; i<plannedRepoInVOs.length; i++ ) {					 
					...
					if ( plannedRepoInVOs[i].getIbflag().equals("I") || plannedRepoInVOs[i].getIbflag().equals("U")){
						...
						if(plannedRepoInVOs[i].getD2Ef().equals("U")){       // insert or update 
							...
							updateQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());   
						}else if(plannedRepoInVOs[i].getD2Ef().equals("D")){ // delete (row delete or return)
							eqrCtrlPlnRepoInQtyVO.setCntrTpszCd("D2");
							eqrCtrlPlnRepoInQtyVO.setCntrQty("");
							deleteQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());   
						}
						...
						if(plannedRepoInVOs[i].getO5Ef().equals("U")){       // insert or update 
							eqrCtrlPlnRepoInQtyVO.setCntrTpszCd("O5");
							eqrCtrlPlnRepoInQtyVO.setCntrQty(plannedRepoInVOs[i].getO5Qty());
							updateQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());   
						}else if(plannedRepoInVOs[i].getO5Ef().equals("D")){ // delete (row delete or return)
							eqrCtrlPlnRepoInQtyVO.setCntrTpszCd("O5");
							eqrCtrlPlnRepoInQtyVO.setCntrQty("");
							deleteQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());   	
						}						
						...
						if(plannedRepoInVOs[i].getA5Ef().equals("U")){       // insert or update 
							eqrCtrlPlnRepoInQtyVO.setCntrTpszCd("A5");
							eqrCtrlPlnRepoInQtyVO.setCntrQty(plannedRepoInVOs[i].getA5Qty());
							updateQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());      
						}else if(plannedRepoInVOs[i].getA5Ef().equals("D")){ // delete (row delete or return)
							eqrCtrlPlnRepoInQtyVO.setCntrTpszCd("A5");
							eqrCtrlPlnRepoInQtyVO.setCntrQty("");
							deleteQtyVoList.add((EqrCtrlPlnRepoInQtyVO) eqrCtrlPlnRepoInQtyVO.clone());   
						}
	...
	/**
	*  EQR의 Execution Plan에서 생성된 ECC별 MTY Load 계획 조회한다<br>
	* 
	...
	*/
    public List<ForecastReportListVO> searchMtyRepoOutDetailList(EesEqr1049ConditionVO eesEqr1049ConditionVO) throws EventException {
		try {
			String fcast_yrwk = eesEqr1049ConditionVO.getFcastYrwk();
			
			//String[] weekStEndDt = dbDao.searchWeekStEndDt(eesEqr1049ConditionVO);			
			String[] weekStEndDt = dbDao.searchWeekStEndDtPast(fcast_yrwk, "W");  // WEEK 만 해당
			
			eesEqr1049ConditionVO.setWkStDt(weekStEndDt[0]);
			eesEqr1049ConditionVO.setWkEndDt(weekStEndDt[1]);

			return dbDao.searchMtyRepoOutDeatilList(eesEqr1049ConditionVO);
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			//throw new EventException(new ErrorHandler(ex).getMessage());
			throw new EventException(new ErrorHandler("EQR10028", new String[]{"MTY Reposition Out Detail Retrieve"}).getMessage(),ex);
		}
	}		 
	...
	*/
	public void manageMtyRepoOutDetailList(ForecastReportListVO[] forecastReportListVOs, SignOnUserAccount account) throws EventException
	{
		try {
			...
			if(forecastReportListVOs != null) { 
				for ( int i=0; i<forecastReportListVOs.length; i++ ) {					
					if ( forecastReportListVOs[i].getIbflag().equals("I")){
						...
						if(forecastReportListVOs[i].getCreSeq().equals("1")) { // lvl =1 은 manual 입력을 의미함.(EQR_MTY_BAL_RPT_DCHG_MNL 수정)
							updateVoList.add(forecastReportListVOs[i]);
						}else if(forecastReportListVOs[i].getCreSeq().equals("0")) {  // lvl=0 은 EQR_MTY_COD_RMK 의 remark 만 수정
							forecastReportListVOs[i].setOfcCd(account.getOfc_cd());
							forecastReportListVOs[i].setCreUsrId(account.getUsr_id());
							updateCodRmkVoList.add(forecastReportListVOs[i]);
						}
			...
			if ( insertVoList.size() > 0 ) {
				dbDao.addMtyRepoOutDetailList(insertVoList);
			}
				
			if ( updateVoList.size() > 0 ) {
				dbDao.modifyMtyRepoOutDetailList(updateVoList);
			}
			...
			if ( deleteVoList.size() > 0 ) {
				dbDao.removeMtyRepoOutDetailList(deleteVoList);
			}
		} catch (DAOException de) {
			log.error("err " + de.toString(), de);
			throw new EventException(new ErrorHandler("EQR10028", new String[]{"Failed to save due to time/ auth restriction"}).getMessage(),de);
	...
	public List<ForecastReportListVO> searchMtyRepoInDetailPastList(EesEqr1081ConditionVO eesEqr1081ConditionVO) throws EventException {
		try {
			String fcast_yrwk = eesEqr1081ConditionVO.getFcastYrwk();
			String tp_cd      = eesEqr1081ConditionVO.getTpCd();
			
			String[] weekStEndDt = dbDao.searchWeekStEndDtPast(fcast_yrwk, tp_cd);
			eesEqr1081ConditionVO.setWkStDt(weekStEndDt[0]);
			eesEqr1081ConditionVO.setWkEndDt(weekStEndDt[1]);
			
			return dbDao.searchMtyRepoInDetailPastList(eesEqr1081ConditionVO);
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			//throw new EventException(new ErrorHandler(ex).getMessage());
			throw new EventException(new ErrorHandler("EQR10028", new String[]{"MTY Reposition In Detail Retrieve"}).getMessage(),ex);
	...
	public List<ForecastReportListVO> searchMtyRepoOutDetailPastList(EesEqr1082ConditionVO eesEqr1082ConditionVO) throws EventException {
		try {
			String fcast_yrwk = eesEqr1082ConditionVO.getFcastYrwk();
			String tp_cd      = eesEqr1082ConditionVO.getTpCd();
			
			String[] weekStEndDt = dbDao.searchWeekStEndDtPast(fcast_yrwk, tp_cd);
			eesEqr1082ConditionVO.setWkStDt(weekStEndDt[0]);
			eesEqr1082ConditionVO.setWkEndDt(weekStEndDt[1]);
			
			return dbDao.searchMtyRepoOutDetailPastList(eesEqr1082ConditionVO);
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			//throw new EventException(new ErrorHandler(ex).getMessage());
			throw new EventException(new ErrorHandler("EQR10028", new String[]{"MTY Reposition In Detail Retrieve"}).getMessage(),ex);
	 ...
	 /**
	 *  Reposition Ohter Detail 과거실적 조회<br>
	 * 
	...
	public List<ForecastReportListVO> searchMtyRepoOtherDetailPastList(EesEqr1083ConditionVO eesEqr1083ConditionVO) throws EventException {
		try {
			String fcast_yrwk = eesEqr1083ConditionVO.getFcastYrwk();
			String tp_cd      = eesEqr1083ConditionVO.getTpCd();
			
			String[] weekStEndDt = dbDao.searchWeekStEndDtPast(fcast_yrwk, tp_cd);
			eesEqr1083ConditionVO.setWkStDt(weekStEndDt[0]);
			eesEqr1083ConditionVO.setWkEndDt(weekStEndDt[1]);
			
			return dbDao.searchMtyRepoOtherDetailPastList(eesEqr1083ConditionVO);
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			//throw new EventException(new ErrorHandler(ex).getMessage());
			throw new EventException(new ErrorHandler("EQR10028", new String[]{"MTY Reposition In Detail Retrieve"}).getMessage(),ex);

...
public class ChassisMgsetOnOffhireDBDAOsearchMGSEquipmentDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       D.CHSS_TARE_WGT CHSS_TARE_WGT_KGS, -- CGM_EQ_SPEC(UNIT: KG)" ).append("\n"); 
		query.append("       A.CHSS_TARE_WGT, -- CGM_EQUIPMENT(UNIT: LBS)" ).append("\n"); 
		...
		query.append("	   NVL(A.DISP_FLG, 'N') AS DISPOSAL_CHK" ).append("\n"); 
		...
		query.append(",DECODE(D.DTCH_YD_CD , NULL,   DECODE(A.EQ_TPSZ_CD,'CLG','Y','N') ,'N') CNTR_CHK" ).append("\n"); 
		query.append(",DECODE(D.DTCH_YD_CD , NULL,   DECODE(A.EQ_TPSZ_CD,'UMG','Y','N') ,'N') CHS_CHK" ).append("\n"); 
		query.append(",DECODE(D.DTCH_YD_CD , NULL,   DECODE(A.EQ_TPSZ_CD,'CLG',D.CNTR_NO ,'') ,'') ATCH_CNTR" ).append("\n"); 
		query.append(",DECODE(D.DTCH_YD_CD , NULL,   DECODE(A.EQ_TPSZ_CD,'UMG',D.CHSS_NO ,'') ,'') ATCH_CHS" ).append("\n"); 
		...
		query.append("        CGM_EQ_SPEC D" ).append("\n"); 
		query.append(" WHERE A.AGMT_OFC_CTY_CD = B.AGMT_OFC_CTY_CD" ).append("\n"); 
		query.append("   AND A.EQ_NO = C.EQ_NO(+)" ).append("\n"); 
		query.append("   AND A.AGMT_SEQ = B.AGMT_SEQ" ).append("\n"); 
		query.append("   AND A.EQ_SPEC_NO = D.EQ_SPEC_NO(+)" ).append("\n"); 
		query.append("   AND A.EQ_KND_CD = D.EQ_KND_CD(+)" ).append("\n"); 

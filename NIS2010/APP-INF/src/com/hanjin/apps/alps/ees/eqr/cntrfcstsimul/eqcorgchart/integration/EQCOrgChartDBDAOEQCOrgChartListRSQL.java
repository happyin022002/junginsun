...
public class EQCOrgChartDBDAOEQCOrgChartListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , (" ).append("\n"); 
		query.append("                   SELECT DISTINCT SCC_CD LOC_CD, 'S' LOC_DPTH_CD FROM MDM_EQ_ORZ_CHT WHERE DELT_FLG = 'N' --AND RCC_CD IN ('TWTPE','CNHKG','CNSHA','JPTYO','KRSEL','SGSIN')" ).append("\n"); 
		query.append("         UNION ALL SELECT DISTINCT ECC_CD LOC_CD, 'E' LOC_DPTH_CD FROM MDM_EQ_ORZ_CHT WHERE DELT_FLG = 'N' --AND RCC_CD IN ('TWTPE','CNHKG','CNSHA','JPTYO','KRSEL','SGSIN')" ).append("\n"); 
		query.append("         UNION ALL SELECT DISTINCT LCC_CD LOC_CD, 'L' LOC_DPTH_CD FROM MDM_EQ_ORZ_CHT WHERE DELT_FLG = 'N' --AND RCC_CD IN ('TWTPE','CNHKG','CNSHA','JPTYO','KRSEL','SGSIN')" ).append("\n"); 
		query.append("         UNION ALL SELECT DISTINCT RCC_CD LOC_CD, 'R' LOC_DPTH_CD FROM MDM_EQ_ORZ_CHT WHERE DELT_FLG = 'N' --AND RCC_CD IN ('TWTPE','CNHKG','CNSHA','JPTYO','KRSEL','SGSIN')" ).append("\n"); 

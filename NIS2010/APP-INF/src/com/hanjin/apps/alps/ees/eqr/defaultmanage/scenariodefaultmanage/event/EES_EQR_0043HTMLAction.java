...
*@FileName : EES_EQR_0043HTMLAction.java
*@FileTitle : 컨테이너 Turn-Time 조회/수정
*Open Issues :
*Change history :
* No.   Ver.    Modifier            modifier date     explanation
* 1             Chang Ho Chae       2009-04-06        CSR NO : R200904030004 oddtpsz 조건값 추가 
...
public class EES_EQR_0043HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_0043HTMLAction 객체를 생성
	 */
	public EES_EQR_0043HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0043Event event = new EesEqr0043Event();
		
		EqrEccTurnTmVO eqrEccTurnTmVO = new EqrEccTurnTmVO();
		EqrEccTurnTmVO[] eqrEccTurnTmVOs = null;
		
		// 검색조건 VO Setting
		event.setEesEqr0043ConditionVO((EesEqr0043ConditionVO)getVO(request, EesEqr0043ConditionVO.class));
		
		if(command.isCommand(FormCommand.MULTI)) {
			// Cntr Tpsz 형태에 맞게 Vo에 Setting해주기 위해서 Table VO에 새로만든 함수 호출해서 Vo Settting
			String tpsztype = event.getEesEqr0043ConditionVO().getTpsztype();
			eqrEccTurnTmVOs = eqrEccTurnTmVO.fromRequestGridArrayList(request, tpsztype);
			event.setEqrEccTurnTmVOS(eqrEccTurnTmVOs);
		}

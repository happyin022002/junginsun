...
public class EES_LSE_0030HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		String strTpszCd    = request.getParameter("tpsz_cd");
	 	...
	 	String strReqNo     = request.getParameter("reqno");
	 	String strLocCod    = request.getParameter("loc_cod");
	 	String strEccCd     = request.getParameter("ecc_cd");
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setTpszCd(strTpszCd);
		 	...
		 	event.setReqno(strReqNo);
		 	event.setLocCod(strLocCod);
		 	event.setEccCd(strEccCd);
		} else {
			event.setAgmtSeq(strAgmtSeq);

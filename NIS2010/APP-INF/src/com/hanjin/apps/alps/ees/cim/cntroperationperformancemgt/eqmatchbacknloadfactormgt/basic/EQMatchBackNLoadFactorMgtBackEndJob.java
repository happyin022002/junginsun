 ...
 *  - Line 2120, Load Factor Weight 계산 시 분모가 0 일 경우 0 처리 해주는 logic 수정
=========================================================*/
	...
	public List doStart() throws Exception {
		...
		try {
			if (getJobType().equals("searchLoadFactorByTrade")) {
				...
				if ("TTL".equals(searchOptionByTradeLaneVvdVO.getCompany())) { 	// 회사
																				// 전체

					if (Integer.parseInt(fromDate) < 20090301) { // 20090301 이전
																 // 데이타에 대해서만
																 // SEN 대상 포함
						arrList.add("HJS");
						arrList.add("SEN");
				...
				else if ("HJS".equals(searchOptionByTradeLaneVvdVO.getCompany())) {
					if (Integer.parseInt(fromDate) < 20090301) {
						arrList.add("HJS");
						arrList.add("SEN");
					}
					else {
				...
				for (int x = 0; x < arrList.size(); x++) {
					...
					//		vo.setReleasedweight("0");
						...
						if ("".equals(vos.getDatasource())) {
							...
							if (rowQty.next()) { // RDR OR BKG가 있으면
								//RDR이 없으면서 HJS이면 BKG에서 실적을 가지고온다.
								if ("0".equals(rowQty.getString("weightTotal")) && "HJS".equals(vo.getCompany())) { 
						...
						else { // 최초 쿼리시 BAY 실적이 있으면 그냥 다시 담는다.
				...
				for (int k = 0; k < list3.size(); k++) {
					...
					// Company,Region,Tread 별 소계를 구한다.
					if ((zTCompany.equals(xTCompany) && zTRegion.equals(xTRegion) && zTTrade.equals(xTTrade)) || k == 0) {
						...
						// vvd 가 조건으로 들어오면 total 은 표현하지 않는다.
				...
				for (int x = 0; x < arrList.size(); x++) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						if (!"".equals(vos.getVvd())) {
							...
							// Releasedteu,Releasedweight 구하기
							if (!"".equals(vos.getReleasedteu())) {
								int index = vos.getReleasedteu().indexOf(",");
				...
				for (int i = 0; i < list1.size(); i++) {
					...
					// 실적 구하기
					if (!"".equals(vos.getDatasource())) { // BAY 이면...
						...
						if (rtnRow05.next()) {
							if ("0".equals(rtnRow05.getString("weightTotal")) && "HJS".equals(vos.getCom())) {
							...
							if ((vos.getIo()).equals("Out")) {
								...
								vos.setVal06("0");
								vos.setVal07("0");
								vos.setWeighttotal("0"); // weightTotal 추가
								vos.setDeadslot("0");
							}
							else { // if( (vos.getIo()).equals("Out") ){
					...
					/*
					 * 
					 * TEU Total : 산출로직 = Full TEU Sum { 20’+ 2 ( 40’+ H/C’+
					 * 45’) } + MTY TEU Sum { 20’+ 2 ( 40’+ H/C’+ 45’) }
					 * 
					 * 40ft(40’/ HC/ 45’)는 x2해서 TEU로 환산후, 아래 산식 시행 Full = If (
				...
				for (int i = 0; i < list1.size(); i++) {
					...
					if ((zTCompany.equals(xTCompany) && zTTrade.equals(xTTrade)) || i == 0) {
						...
						vo.setLfeq(vos.getLfeq());
						vo.setLfwgt(vos.getLfwgt());
						vo.setWeighttotal(vos.getWeighttotal()); // weightTotal 추가
						...
						if (xTIO.equals("Out")) {
							...
							outQtyG[10] = (Double.parseDouble(vos.getVal01()) - Double.parseDouble(vos.getFull20()))
			                  * 2 + Double.parseDouble(vos.getFull20()) + outQtyG[10];
							...
							innQtyG[10] = (Double.parseDouble(vos.getVal01()) - Double.parseDouble(vos.getFull20()))
						     * 2 + Double.parseDouble(vos.getFull20()) + innQtyG[10];
						...
						vo.setWeighttotal(outQtyS[18] + ""); // weightTotal 추가
						...
						vo.setWeighttotal(innQtyS[18] + ""); // weightTotal 추가
						...
						if ((vos.getIo()).equals("Out")) {
							...
							if (outQtyG[22] == 0.0) {//0.0일경우  나누기 에러 처리 2010.09.01 남궁진호
								outQtyG[25] = Double.parseDouble("0.0");
							}
							else {
								double lfWgt = (outQtyG[18] / (outQtyG[22] + outQtyG[20])) * 100;
				...
				vo.setWeighttotal(outQtyG[18] + ""); // weightTotal 추가
				...
				vo.setWeighttotal(innQtyG[18] + ""); // weightTotal 추가

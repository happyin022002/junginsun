...
*@FileName : RFAExceptionTariffMgtDBDAOSearchAfterBookingListByDARRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFAExceptionTariffMgtDBDAOSearchAfterBookingListByDARRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * DEM/DET Adjustment Request & Approval Status 조회(After Booking, DAR)용 쿼리
	  * </pre>
	  */
	public RFAExceptionTariffMgtDBDAOSearchAfterBookingListByDARRSQL(){
		setQuery();
		...
		query.append("FileName : RFAExceptionTariffMgtDBDAOSearchAfterBookingListByDARRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT  DISTINCT AFT_RQST.AFT_EXPT_DAR_NO AS DAR_NO" ).append("\n"); 
		...
		query.append("	,	(" ).append("\n"); 
		query.append("			SELECT	/*+ INDEX_DESC(DMT_AFT_BKG_ADJ_PROG XPKDMT_AFT_BKG_ADJ_PROG) */ DECODE(AFT_RQST.DMDT_EXPT_RQST_STS_CD, 'C', '', PROG_OFC_CD)" ).append("\n"); 
		query.append("			FROM	DMT_AFT_BKG_ADJ_PROG" ).append("\n"); 
		query.append("			WHERE	AFT_EXPT_DAR_NO = AFT_RQST.AFT_EXPT_DAR_NO" ).append("\n"); 
		...
		query.append("	,	(" ).append("\n"); 
		query.append("                SELECT CASE C.AFT_BKG_PATH_CD WHEN 'BBOPIC' THEN LVL.OFC_CD" ).append("\n"); 
		query.append("                                              WHEN 'BBGMGR' THEN LVL.OFC_CD" ).append("\n"); 
		query.append("                                              WHEN 'RHQPIC' THEN ( SELECT OFC_N3RD_LVL_CD FROM DMT_OFC_LVL_V WHERE OFC_N8TH_LVL_CD = LVL.OFC_CD )" ).append("\n"); 
		query.append("                                              WHEN 'RHQMGR' THEN ( SELECT OFC_N3RD_LVL_CD FROM DMT_OFC_LVL_V WHERE OFC_N8TH_LVL_CD = LVL.OFC_CD )" ).append("\n"); 
		query.append("                                              WHEN 'HDOPIC' THEN 'SELHO'" ).append("\n"); 
		query.append("                                              WHEN 'HDOMGR' THEN 'SELHO'" ).append("\n"); 
		query.append("                                              ELSE 'ERROR' END" ).append("\n"); 
		query.append("                FROM MAS_OFC_LVL LVL, DMT_CHG_CALC B, DMT_AFT_BKG_CNTR A, DMT_AFT_BKG_APRO_PATH C" ).append("\n"); 
		...
		query.append("                AND LVL.OFC_CD = B.OFC_CD" ).append("\n"); 
		query.append("                AND TO_CHAR(SYSDATE,'YYYYMM') BETWEEN LVL.OFC_APLY_FM_YRMON AND LVL.OFC_APLY_TO_YRMON" ).append("\n"); 
		query.append("                AND A.AFT_EXPT_DAR_NO 		= AFT_RQST.AFT_EXPT_DAR_NO" ).append("\n"); 
		query.append("                AND A.AFT_EXPT_DAR_NO 		= C.AFT_EXPT_DAR_NO" ).append("\n"); 
		query.append("                AND C.AFT_BKG_PATH_CPLS_FLG = 'Y'" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("WHERE	AFT_RQST.AFT_EXPT_DAR_NO 		= AFT_RQST_DTL.AFT_EXPT_DAR_NO" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if(${dar_no} != '')" ).append("\n"); 
		query.append("	AND AFT_RQST.AFT_EXPT_DAR_NO 		= @[dar_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if(${apvl_no} != '')" ).append("\n"); 
		query.append("	AND AFT_RQST.AFT_BKG_APRO_NO 		= @[apvl_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if(${bkg_no} != '')" ).append("\n"); 
		query.append("	AND AFT_RQST_DTL.BKG_NO 			= @[bkg_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if(${bl_no} != '')" ).append("\n"); 
		query.append("	AND AFT_RQST_DTL.BL_NO 				= @[bl_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("	AND BOOKING.SC_NO = @[sc_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if(${rfa_no} != '')" ).append("\n"); 
		query.append("	AND BOOKING.RFA_NO = @[rfa_no]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 

...
public class EmptyCODAdjustmentDBDAOEmptyCODVVDVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 5. RUVVO (블라디보스토) 하드코딩 추가, 신용찬, 2014-10-14
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("        -- 러시아 RUVVO(블라디보스토) 벙커링 추가, 2014-10-14, 신용찬" ).append("\n"); 
		query.append("		OR" ).append("\n"); 
		query.append("	    ( SUBSTR(TRD.TRD_CD,2,1) = 'P' AND DECODE(OSCNT.CONTI_CD, 'F', 'E', OSCNT.CONTI_CD) =  'E'                                AND (SUBSTR(DST.FM_PKEY,1,5)= 'RUNJK' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUSKA' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUVVO')) " ).append("\n"); 
		query.append("		OR		" ).append("\n"); 
		query.append("		( SUBSTR(TRD.TRD_CD,2,1) = 'E' AND DECODE(OSCNT.CONTI_CD, 'F', 'E', OSCNT.CONTI_CD) =  'E'                                AND (SUBSTR(DST.FM_PKEY,1,5)= 'RUNJK' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUSKA' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUVVO'))" ).append("\n"); 
		query.append("        OR" ).append("\n"); 
		query.append("        ( SUBSTR(TRD.TRD_CD,2,1) = 'M' AND TRD.TRD_CD != 'EMS' AND DECODE(OSCNT.CONTI_CD, 'F', 'E', OSCNT.CONTI_CD) =  'E'        AND (SUBSTR(DST.FM_PKEY,1,5)= 'RUNJK' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUSKA' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUVVO'))" ).append("\n"); 
		query.append("		OR " ).append("\n"); 
		query.append("        ( SUBSTR(TRD.TRD_CD,2,1) = 'M' AND TRD.TRD_CD = 'EMS' AND DECODE(OSCNT.CONTI_CD, 'F', 'E', OSCNT.CONTI_CD) IN ('M', 'E')  AND (SUBSTR(DST.FM_PKEY,1,5)= 'RUNJK' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUSKA' OR SUBSTR(DST.FM_PKEY,1,5)= 'RUVVO'))	" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("ORDER BY weekDivision,codcfmctscd,lane,firstetb" ).append("\n"); 

...
*@FileName : EES_LSE_0027HTMLAction.java
...
public class EES_LSE_0027HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_LSE_0027HTMLAction 객체를 생성
	 */
	public EES_LSE_0027HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesLse0027Event event = new EesLse0027Event();


		if(command.isCommand(FormCommand.SEARCH01)) {//조회 - 최대 요청번호
			event.setOfcCd(request.getParameter("ofc_cd"));
		} else if(command.isCommand(FormCommand.SEARCH02)) {//유효성 검증 - 컨테이너 정보
			event.setCntrNo(request.getParameter("cntr_no"));
		} else if(command.isCommand(FormCommand.SEARCH03)) {//조회 - 컨테이너 정보
			event.setCntrNo(request.getParameter("cntr_no"));
			...
			event.setMisUseReqContainerVOs((MisUseReqContainerVO[])getVOs(request, MisUseReqContainerVO.class));
		}

...
*@FileName : TimeClockStopMgtDBDAOSearchTimeClockChargeListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TimeClockStopMgtDBDAOSearchTimeClockChargeListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Time Clock 저장 후 계산 로직을 처리 할 대상 조회
	  * </pre>
	  */
	public TimeClockStopMgtDBDAOSearchTimeClockChargeListRSQL(){
		setQuery();
		...
		query.append("FileName : TimeClockStopMgtDBDAOSearchTimeClockChargeListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("FROM DMT_TM_CLK_STOP S" ).append("\n"); 
		query.append(",DMT_CHG_CALC C" ).append("\n"); 
		query.append(",DMT_CHG_BKG_CNTR B" ).append("\n"); 
		query.append(",COM_SYS_AREA_GRP_ID CS" ).append("\n"); 
		query.append("WHERE S.CLK_STOP_NO = @[clk_stop_no]" ).append("\n"); 
		query.append("AND C.DMDT_TRF_CD = S.DMDT_TRF_CD" ).append("\n"); 
		query.append("AND C.OFC_CD = S.CLK_STOP_OFC_CD" ).append("\n"); 
		query.append("AND (   (    C.FM_MVMT_DT <= TO_DATE(TO_CHAR(S.CLK_STOP_TO_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("+ .99999" ).append("\n"); 
		query.append("AND C.TO_MVMT_DT >= TO_DATE(TO_CHAR(S.CLK_STOP_TO_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("+ .0" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("OR (    C.FM_MVMT_DT <= TO_DATE(TO_CHAR(S.CLK_STOP_FM_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("+ .99999" ).append("\n"); 
		query.append("AND C.TO_MVMT_DT >= TO_DATE(TO_CHAR(S.CLK_STOP_FM_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("+ .0" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("OR (    C.FM_MVMT_DT >= TO_DATE(TO_CHAR(S.CLK_STOP_FM_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("+ .0" ).append("\n"); 
		...
		query.append("AND C.DMDT_CHG_LOC_DIV_CD NOT IN ('TSP', 'SZP')" ).append("\n"); 

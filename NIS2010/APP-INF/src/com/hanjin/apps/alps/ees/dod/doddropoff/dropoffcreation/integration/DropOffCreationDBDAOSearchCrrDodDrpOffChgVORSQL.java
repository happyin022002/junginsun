...
*@FileName : DropOffCreationDBDAOSearchCrrDodDrpOffChgVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class DropOffCreationDBDAOSearchCrrDodDrpOffChgVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * Invoice Creation & Correction화면에서 호출하여 AR INV 보내기전 RTN CY를 변경하기 위한 대상을 가져온다.
	  * </pre>
	  */
	public DropOffCreationDBDAOSearchCrrDodDrpOffChgVORSQL(){
		setQuery();
		...
		query.append("FileName : DropOffCreationDBDAOSearchCrrDodDrpOffChgVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       A.CNTR_RTN_YD_CD ORG_CNTR_RTN_YD_CD," ).append("\n"); 
		...
		query.append("       END ORG_CURR_CD," ).append("\n"); 
		...
		query.append("          FROM DOD_DRP_OFF_CHG_TRF F" ).append("\n"); 
		query.append("         WHERE F.DRP_OFF_CHG_TRF_SEQ = A.GEN_SEQ ) ORG_GEN_TRF_AMT," ).append("\n"); 
		query.append("       (SELECT F.DRP_OFF_CHG_TRF_AMT" ).append("\n"); 
		query.append("          FROM DOD_DRP_OFF_CHG_TRF F" ).append("\n"); 
		query.append("         WHERE F.DRP_OFF_CHG_TRF_SEQ = A.SPC_SEQ ) ORG_SPCL_TRF_AMT," ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("				   AND NVL(D.DEL_CD, 1) = " ).append("\n"); 
		query.append("                   		(SELECT NVL(MAX(D.DEL_CD), 1)" ).append("\n"); 
		query.append("							FROM DOD_DRP_OFF_CHG_TRF D" ).append("\n"); 
		query.append("                 			WHERE 1 = 1" ).append("\n"); 
		query.append("							AND DECODE(D.DEL_CD, NULL, '1', D.DEL_CD) = DECODE(D.DEL_CD, NULL, '1', B.POD_CD)" ).append("\n"); 
		query.append("                   			AND D.CNTR_RTN_LOC_CD = SUBSTR(A.CNTR_RTN_YD_CD, 1, 5)" ).append("\n"); 
		...
		query.append("                   			AND B.POL_ETD_DT BETWEEN TO_DATE(D.DRP_OFF_CHG_TRF_EFF_DT, 'YYYYMMDD') + .0 AND TO_DATE(D.DRP_OFF_CHG_TRF_EXP_DT, 'YYYYMMDD') + .99999" ).append("\n"); 
		query.append("						)" ).append("\n"); 
		query.append("				   AND NVL(D.CNTR_RTN_YD_SFX_CD, 1) = " ).append("\n"); 
		query.append("                   		(SELECT NVL(MAX(D.CNTR_RTN_YD_SFX_CD), 1)" ).append("\n"); 
		query.append("							FROM DOD_DRP_OFF_CHG_TRF D" ).append("\n"); 
		query.append("                 			WHERE 1 = 1" ).append("\n"); 
		query.append("							AND DECODE(D.DEL_CD, NULL, '1', D.DEL_CD) = DECODE(D.DEL_CD, NULL, '1', B.POD_CD)" ).append("\n"); 
		query.append("                   			AND D.CNTR_RTN_LOC_CD = SUBSTR(A.CNTR_RTN_YD_CD, 1, 5)" ).append("\n"); 

...
*@FileName : CntrForecastManageDBDAOSearchCntrGenerationAllDayRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CntrForecastManageDBDAOSearchCntrGenerationAllDayRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * eqr_ob_fcast + eqr_ib_bkg_fcast  테이블 데이터 조회(All/Day)
	  * 
	  * <Change History>
	  * 1	2009.08.12	Lee ByoungHun	최초작성
	  * </pre>
	  */
	public CntrForecastManageDBDAOSearchCntrGenerationAllDayRSQL(){
		setQuery();
		...
		query.append("FileName : CntrForecastManageDBDAOSearchCntrGenerationAllDayRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("TO_CHAR(TO_DATE (A.FCAST_DT, 'yyyymmdd'), 'DY', 'NLS_DATE_LANGUAGE=AMERICAN') ST_DY," ).append("\n"); 
		...
		query.append("DENSE_RANK() OVER (ORDER BY FCAST_YRWK, FM_ECC_CD, TO_ECC_CD ) AS RK" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT" ).append("\n"); 
		query.append("A.FCAST_DEL_YRWK FCAST_YRWK," ).append("\n"); 
		query.append("A.CO_CD," ).append("\n"); 
		query.append("A.FCAST_DEL_DT FCAST_DT," ).append("\n"); 
		query.append("(" ).append("\n"); 
		query.append("SELECT" ).append("\n"); 
		query.append("#if (${fmToAt} == '1')" ).append("\n"); 
		...
		query.append("SCNR_ID, A.FCAST_DEL_YRWK, A.CO_CD, A.FM_ECC_CD, A.TO_ECC_CD, A.CNTR_TPSZ_CD, A.FCAST_DEL_DT, A.FCAST_DT" ).append("\n"); 
		...
		query.append("MAX(A.CNTR_VOL_QTY)CNTR_VOL_QTY" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("EQR_IB_BKG_FCAST A," ).append("\n"); 
		query.append("#if (${fmToAt} == '1')" ).append("\n"); 
		query.append("(SELECT ECC_CD FROM EQR_ECC_MST WHERE 1 = 1" ).append("\n"); 
		query.append("#if ($arrFmEccCd.size() > 0)" ).append("\n"); 
		query.append("#if (${fmType} == 'R')" ).append("\n"); 
		...
		query.append("GROUP BY" ).append("\n"); 
		query.append("SCNR_ID, A.FCAST_YRWK, A.CO_CD, A.FM_ECC_CD, A.TO_ECC_CD, A.CNTR_TPSZ_CD, A.FCAST_DT" ).append("\n"); 
		query.append(")A" ).append("\n"); 
		query.append("GROUP BY" ).append("\n"); 
		query.append("SCNR_ID, A.FCAST_YRWK, A.CO_CD, A.FM_ECC_CD, A.TO_ECC_CD, A.FCAST_DT" ).append("\n"); 
		query.append("ORDER BY" ).append("\n"); 
		query.append("SCNR_ID, A.FCAST_YRWK, A.FM_ECC_CD, A.TO_ECC_CD, A.FCAST_DT" ).append("\n"); 

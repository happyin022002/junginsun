...
*@FileName : EES_EQR_124HTMLAction.java
*@FileTitle : S
*Open Issues :
*Change history :
...
public class EES_EQR_0124HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_124HTMLAction 객체를 생성
	 */
	public EES_EQR_0124HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0124Event event = null;
		
		EqrRepoCnstVO eqrRepoCnstVO = new EqrRepoCnstVO();
		EqrRepoCnstVO[] eqrRepoCnstVOs = null;

		// 검색조건 4개미만 - Event Setting..
    	String cnsttype	= JSPUtil.getParameter(request, "cnsttype".trim(), "");
		String tpsztype = JSPUtil.getParameter(request, "hidCntrType".trim() , "");
    	
    	event = new EesEqr0124Event(cnsttype, tpsztype);
    	
		if(command.isCommand(FormCommand.MULTI)) {
			// Cntr Tpsz 형태에 맞게 Vo에 Setting해주기 위해서 Table VO에 새로만든 함수 호출해서 Vo Settting
			eqrRepoCnstVOs = eqrRepoCnstVO.fromRequestGridArrayList(request, tpsztype);
			event.setEqrRepoCnstVOS(eqrRepoCnstVOs);
		}

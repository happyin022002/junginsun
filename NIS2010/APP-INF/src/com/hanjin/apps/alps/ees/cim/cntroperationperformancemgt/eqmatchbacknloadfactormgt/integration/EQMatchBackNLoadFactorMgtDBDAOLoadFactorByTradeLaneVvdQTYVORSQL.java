...
public class EQMatchBackNLoadFactorMgtDBDAOLoadFactorByTradeLaneVvdQTYVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 2011.11.08 신자영 [CHM-201114141-01] [CIM] L/F by trade & M/B by vessel 기능에 EM1 & EM2 Lane 추가
	  * 2012.01.12 신자영 [CHM-201215617-01] [CIM] LOAD-FACTOR / M/B by Vessel 기능 수정
	  * 2012.08.02 신자영 [CHM-201219416-01] ALPS / LF 기능 로직 수정
	  * 2012.08.20 신자영 [CHM-201219767-01] [CIM] Match-Back by Vessel BKG CNTR QTY 반올림해 정수로 산출 기능 보완
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("round(RD.R_FULL20QTY,0)	full20Qty ," ).append("\n"); 
		query.append("round(RD.R_FULL40QTY,0)	full40Qty," ).append("\n"); 
		query.append("round(RD.R_FULLHCQTY,0)	fullHcQty," ).append("\n"); 
		query.append("round(RD.R_FULL45QTY,0)	full45Qty," ).append("\n"); 
		query.append("round(RD.R_MTY20QTY,0)	mty20Qty," ).append("\n"); 
		query.append("round(RD.R_MTY40QTY,0)	mty40Qty," ).append("\n"); 
		query.append("round(RD.R_MTYHCQTY,0)	mtyHcQty," ).append("\n"); 
		query.append("round(RD.R_MTY45QTY,0)	mty45Qty," ).append("\n"); 
		...
		query.append("--			COUNT(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '5', BC.CNTR_TPSZ_CD))) + COUNT(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '9', BC.CNTR_TPSZ_CD)))fullHcQty_b	," ).append("\n"); 
		query.append("--			COUNT(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '7', BC.CNTR_TPSZ_CD))) full45Qty_b	," ).append("\n"); 
		query.append("round(NVL(SUM(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '2', BC.CNTR_VOL_QTY))),0),0) full20Qty_b ," ).append("\n"); 
		query.append("round(NVL(SUM(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '4', BC.CNTR_VOL_QTY))),0),0) full40Qty_b ," ).append("\n"); 
		query.append("round(NVL(SUM(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '5', BC.CNTR_VOL_QTY))),0),0) + round(NVL(SUM(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '9', BC.CNTR_VOL_QTY))),0),0) fullHcQty_b ," ).append("\n"); 
		query.append("round(NVL(SUM(DECODE(BK.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '7', BC.CNTR_VOL_QTY))),0),0) full45Qty_b ," ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("COUNT(DECODE(BK.BKG_CGO_TP_CD, 'P', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '5', BC.CNTR_TPSZ_CD))) + COUNT(DECODE(BK.BKG_CGO_TP_CD, 'P', DECODE(SUBSTR(BC.CNTR_TPSZ_CD,2,1), '9', BC.CNTR_TPSZ_CD))) mtyHcQty_b	," ).append("\n"); 
		...
		query.append("AND 	SL.CLPT_IND_SEQ 	= NVL(BV.POL_CLPT_IND_SEQ,1)" ).append("\n"); 
		query.append("AND 	NVL(SL.VPS_PORT_CD,   ' ') NOT IN ('PAPAC', 'EGSUZ')   --추가" ).append("\n"); 
		query.append("AND 	NVL(SL.TURN_PORT_IND_CD, ' ') NOT IN ('D', 'F', 'V')   --추가" ).append("\n"); 
		query.append("AND 	NVL(SL.SKD_CNG_STS_CD, ' ') <> 'S'                    --추가" ).append("\n"); 
		...
		query.append("AND     SD.CLPT_IND_SEQ 	= NVL(BV.POD_CLPT_IND_SEQ,1)" ).append("\n"); 
		query.append("AND 	NVL(SD.VPS_PORT_CD,   ' ') NOT IN ('PAPAC', 'EGSUZ')   --추가" ).append("\n"); 
		query.append("AND 	NVL(SD.SKD_CNG_STS_CD, ' ') <> 'S'                    --추가" ).append("\n"); 
		query.append("--	AND 	NVL(BK.BKG_STS_CD, ' ')  NOT IN ('X', 'S')" ).append("\n"); 
		query.append("AND 	NVL(BK.BKG_STS_CD, ' ')  = 'F'" ).append("\n"); 
		query.append("GROUP BY" ).append("\n"); 
		query.append("RD.R_DATASOURCE ," ).append("\n"); 
		query.append("round(RD.R_FULL20QTY,0)	," ).append("\n"); 
		query.append("round(RD.R_FULL40QTY,0)	," ).append("\n"); 
		query.append("round(RD.R_FULLHCQTY,0)	," ).append("\n"); 
		query.append("round(RD.R_FULL45QTY,0)	," ).append("\n"); 
		query.append("round(RD.R_MTY20QTY,0)	," ).append("\n"); 
		query.append("round(RD.R_MTY40QTY,0)	," ).append("\n"); 
		query.append("round(RD.R_MTYHCQTY,0)	," ).append("\n"); 
		query.append("round(RD.R_MTY45QTY,0)	," ).append("\n"); 

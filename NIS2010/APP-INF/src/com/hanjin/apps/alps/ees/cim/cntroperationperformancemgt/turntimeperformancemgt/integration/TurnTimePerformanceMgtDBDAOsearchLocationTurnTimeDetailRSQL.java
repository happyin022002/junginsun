...
*@FileName : TurnTimePerformanceMgtDBDAOsearchLocationTurnTimeDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TurnTimePerformanceMgtDBDAOsearchLocationTurnTimeDetailRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public TurnTimePerformanceMgtDBDAOsearchLocationTurnTimeDetailRSQL(){
		setQuery();
		...
		query.append("FileName : TurnTimePerformanceMgtDBDAOsearchLocationTurnTimeDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("   		SUM( DECODE( L.PORT_INLND_CD, 'Y', DECODE( T.ENR_TT_DYS, 0, T.TT_DYS,0), T.TT_DYS + T.ENR_TT_DYS ) ) T_TIME," ).append("\n"); 
		query.append("   		--SUM( DECODE( L.PORT_INLND_CD, 'Y', DECODE( T.ENR_TT_DYS, 0, T.CNTR_KNT, 0 ), T.CNTR_KNT ) )  T_CNT" ).append("\n"); 
		query.append("		/*--- 계산오류로 인해 원본 쿼리로 변경 ---*/" ).append("\n"); 
		query.append("   		SUM( DECODE( L.PORT_INLND_CD, 'Y', DECODE( T.ENR_TT_DYS, 0, 1, 0 ), 1 ) )  T_CNT" ).append("\n"); 
		...
		query.append("  		/* ---------------------------------------------------------------- 2009.09.22 By SBKIM */" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		FROM	#if (${inquiryLevel} == 'AR')   " ).append("\n"); 
		...
		query.append("GROUP BY" ).append("\n"); 
		query.append("ROLLUP(L.LOC_CD),ROLLUP(L.TPSZ), DECODE(Y.NO, '1', 'XX', '2', 'YY')" ).append("\n"); 

...
*@FileName : CntrMtyBkgCreateDBDAOSearchCntrMtyBkgSplitContainerInfoRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CntrMtyBkgCreateDBDAOSearchCntrMtyBkgSplitContainerInfoRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Container 개별정보를 조회
	  * - 2014.03.10 ROB BKG 은 FALSE 추가, 신용찬
	  * </pre>
	  */
	public CntrMtyBkgCreateDBDAOSearchCntrMtyBkgSplitContainerInfoRSQL(){
		setQuery();
		...
		params.put("excel_cntr_no",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : CntrMtyBkgCreateDBDAOSearchCntrMtyBkgSplitContainerInfoRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT MC.BKG_NO       VL_BKG_NO" ).append("\n"); 
		query.append("      ,MC.CNTR_NO      CNTR_NO" ).append("\n"); 
		query.append("      ,CM.CNTR_TPSZ_CD CNTR_TPSZ_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${flag} == 'M') -- MULTI SPLIT" ).append("\n"); 
		query.append("      ,DECODE(MC.MTY_ROB_FLG, 'Y', 'ROB BKG', DECODE(CM.MVMT_STS_CD, 'VL', 'OK', 'FALSE')) VRFY_STATUS  -- ROB BKG은 FALSE 추가" ).append("\n"); 
		query.append("#else  -- SINGLE SPLIT" ).append("\n"); 
		query.append("      ,DECODE(CM.MVMT_STS_CD, 'VL', 'OK', 'FALSE') VRFY_STATUS  " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("      ,CM.MVMT_STS_CD  MVMT_STS_CD " ).append("\n"); 
		query.append("      ,MC.POD_YD_CD    POD_YD_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("      ,MC.YD_CD        POL_YD_CD   -- HIDDEN        " ).append("\n"); 
		...
		query.append("               EC.POD_YD_CD," ).append("\n"); 
		...
		query.append("            ,EQR_CTRL_MTY_BKG_EXE EC" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("        AND   VV.CALL_YD_IND_SEQ = 1 -- VSL SKD 에서 CALL_YD_IND_SEQ = 1만 추출(2013.12.02, 신용찬, YD_CD 중복인 경우를 대비)        " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        AND   BB.VSL_CD     = EC.VSL_CD" ).append("\n"); 
		query.append("        AND   BB.SKD_VOY_NO = EC.SKD_VOY_NO" ).append("\n"); 
		query.append("        AND   BB.SKD_DIR_CD = EC.SKD_DIR_CD" ).append("\n"); 
		query.append("        AND   BB.BKG_NO     = EC.MTY_BKG_NO" ).append("\n"); 
		...
		query.append("#if(${flag} == 'S') -- SINGLE SPLIT 은 bkg no 적용" ).append("\n"); 
		...
		query.append("                           )    " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        AND  BC.CNTR_NO = @[excel_cntr_no] " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    ) MC" ).append("\n"); 
		query.append("WHERE CM.CNTR_NO = MC.CNTR_NO(+)" ).append("\n"); 
		query.append("AND   CM.CNTR_NO = @[excel_cntr_no]  " ).append("\n"); 
		...
		query.append("                                                      WHERE A.CNTR_NO = @[excel_cntr_no]  " ).append("\n"); 

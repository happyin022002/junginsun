...
public class PayableRentalCostDBDAOsearchPayableRentalInvoiceRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    WHEN ( B.INV_STS_CD  = 'P' OR   B.INV_STS_CD  = 'E') AND C.IF_FLG = 'E' AND C.IF_ERR_RSN IS NOT NULL THEN  'Sending Error'  " ).append("\n"); 
		query.append("    WHEN ( B.INV_STS_CD  = 'P' OR   B.INV_STS_CD  = 'E') AND C.IF_FLG = 'Y' AND C.IF_ERR_RSN IS NOT NULL THEN  'Sending…'  " ).append("\n"); 
		query.append("    WHEN ( B.INV_STS_CD  = 'P' OR   B.INV_STS_CD  = 'E') AND C.IF_FLG = 'Y' AND C.IF_ERR_RSN IS NULL THEN ( SELECT C.INTG_CD_VAL_DP_DESC FROM  COM_INTG_CD_DTL C WHERE C.INTG_CD_ID = 'CD02355' AND   C.INTG_CD_VAL_CTNT = B.INV_STS_CD ) " ).append("\n"); 
		query.append("    WHEN   B.INV_STS_CD <> 'P' AND  B.INV_STS_CD <> 'E'  THEN ( SELECT C.INTG_CD_VAL_DP_DESC FROM  COM_INTG_CD_DTL C WHERE C.INTG_CD_ID = 'CD02355' AND   C.INTG_CD_VAL_CTNT = B.INV_STS_CD ) " ).append("\n"); 
		...
		query.append("FROM LSE_PAY_RNTL_CHG A , AP_PAY_INV B , AP_INV_HDR C " ).append("\n"); 
		...
		query.append("A.APRO_USR_ID,C.IF_FLG,C.IF_ERR_RSN" ).append("\n"); 
		...
		query.append("  '20'||A.SKD_VOY_NO CHG_COST_YRMON," ).append("\n"); 
		...
		query.append("  '20'||A.SKD_VOY_NO," ).append("\n"); 

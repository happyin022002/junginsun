...
public class DisposalMgtDBDAOsearchDisposalSoldListDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , (SELECT MNR_CD_DP_DESC FROM MNR_GEN_CD WHERE PRNT_CD_ID = 'CD00078' AND MNR_CD_ID = B.DISP_UT_TP_CD ) AS DISP_UT_TP_NM" ).append("\n"); 
		...
		query.append("   AND A.APRO_DT BETWEEN GLOBALDATE_PKG.TIME_CONV_OFC_FNC(@[ofc_cd],TO_DATE(@[apro_dt_fr], 'YYYY-MM-DD'), MNR_COMMON_PKG.MNR_GET_HOOFC_FNC()) " ).append("\n"); 
		query.append("                     AND GLOBALDATE_PKG.TIME_CONV_OFC_FNC(@[ofc_cd],TO_DATE(@[apro_dt_to]||'23:59:59','YYYY-MM-DD HH24:MI:SS'), MNR_COMMON_PKG.MNR_GET_HOOFC_FNC())" ).append("\n"); 
		query.append("" ).append("\n"); 

...
*@FileName : EES_LSE_0109HTMLAction.java
...
public class EES_LSE_0109HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_LSE_0109HTMLAction 객체를 생성
	 */
	public EES_LSE_0109HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesLse0109Event event = new EesLse0109Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setEqInterchangeVO((EqInterchangeVO)getVO(request, EqInterchangeVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setEqInterchangeVO((EqInterchangeVO)getVO(request, EqInterchangeVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setEqInterchangeVOS((EqInterchangeVO[])getVOs(request, EqInterchangeVO.class));

...
*@FileName : EesCim10612ViewAdapter.java
*@FileTitle : Location M/B by COA BKG (Trend Tab 용)
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.5   
 */	
public class EesCim10612ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		List<String> sortedColList = new ArrayList<String>();
		for (int i = 0; i < 27; i++) {		
			String colname = "qty" + i;	
			sortedColList.add(colname);		
		}	
		
		for(int i=0;i<realCnt;i++){	
			Map<String, String> colValues = vos.get(i).getColumnValues();
			
			//배경컬러  total은 핑크 ,Total은 자색, Ratio 는 푸른색 
			String bgColor = "";		
			...
			String tpsz = colValues.get("tpsz");
			if(colValues.get("tpsz") == null || colValues.get("tpsz").equals("")){
				if(locCd.equals("G.Total")){
					tpsz = "";		 
				} else {
					tpsz = "Total";
					bgColor = " BGCOLOR=\"201,213,235\"";
				}		
			}				 
			sbufXML.append("<TD").append(bgColor).append("><![CDATA[").append(tpsz);sbufXML.append("]]></TD>");
			...
			for (int j = 0; j < sortedColList.size(); j++) {
				String originValue = colValues.get(sortedColList.get(j));
				if(originValue == null){
					originValue = "0";
				}
				...
				if(divCd.equalsIgnoreCase("M/B Ratio(%)") || divCd.equalsIgnoreCase("Balance")){
					if(Double.parseDouble(originValue) < 0){	
						if(locCd.equalsIgnoreCase("G.Total")){
							formatValue = " " + formatValue;	
						}

...
*@FileName : ChassisMgsetInvoiceDBDAOsearchCHSLeaseInvoiceVerifyDataRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ChassisMgsetInvoiceDBDAOsearchCHSLeaseInvoiceVerifyDataRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ChassisMgsetInvoiceDBDAOsearchCHSLeaseInvoiceVerifyDataRSQL(){
		setQuery();
		...
		query.append("FileName : ChassisMgsetInvoiceDBDAOsearchCHSLeaseInvoiceVerifyDataRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("A.EQ_NO = LAG(A.EQ_NO) OVER (PARTITION BY  A.EQ_NO, A.CHG_CD, A.INV_LSE_RT_AMT ORDER BY EQ_NO  )  THEN 'Equipment No. Duplicated!'" ).append("\n"); 
		query.append("ELSE A.VRFY_RSLT_DESC" ).append("\n"); 
		query.append("END AS VRFY_RSLT_DESC" ).append("\n"); 
		query.append(",CASE WHEN A.CHG_CD NOT IN ('CRD') AND  -- CRD 일때 EQ Dup 체크 제외. CRD 일때 EQ_NO 는 Seq하게 생성되므로 관계없음." ).append("\n"); 
		query.append("A.EQ_NO = LAG(A.EQ_NO) OVER (PARTITION BY  A.EQ_NO, A.CHG_CD, A.INV_LSE_RT_AMT ORDER BY EQ_NO  )  THEN 'N'" ).append("\n"); 
		...
		query.append("CASE" ).append("\n"); 
		query.append("WHEN D.EQ_NO IS NULL AND E.EQ_NO IS NULL AND F.EQ_NO IS NULL AND D2.EQ_NO IS NULL AND D3.EQ_NO IS NULL THEN 'Chassis No does not exist!'" ).append("\n"); 
		query.append("WHEN A.INV_EQ_NO IS NULL AND A.INV_CUST_EQ_NO IS NOT NULL AND E.EQ_NO IS NULL AND F.EQ_NO IS NULL THEN  'Lessor Chassis No does not exist!'" ).append("\n"); 
		query.append("END" ).append("\n"); 
		query.append("END" ).append("\n"); 
		...
		query.append("CASE" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQCZLT'" ).append("\n"); 
		...
		query.append("WHEN A.CHG_CD IN ('GTO','GTI') THEN" ).append("\n"); 
		query.append("CASE" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'FO' THEN 'EQCZSB'" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'SB' THEN 'EQCZSB'" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'SO' THEN 'EQCZSB'" ).append("\n"); 
		query.append("END" ).append("\n"); 
		query.append("ELSE" ).append("\n"); 
		query.append("CASE" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQXTXX'" ).append("\n"); 
		...
		query.append("WHEN A.CHG_CD = 'PDM' THEN" ).append("\n"); 
		query.append("CASE" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'ST' THEN '510831'" ).append("\n"); 
		...
		query.append("WHEN A.CHG_CD IN ('GTO','GTI') THEN" ).append("\n"); 
		query.append("CASE" ).append("\n"); 
		query.append("WHEN C.AGMT_LSTM_CD = 'FO' THEN '510831'" ).append("\n"); 

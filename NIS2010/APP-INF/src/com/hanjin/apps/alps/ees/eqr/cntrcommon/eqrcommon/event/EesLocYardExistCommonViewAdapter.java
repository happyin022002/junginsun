...
*@FileTitle : EES_LOCYARDEXIST_COMMON 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_LOCYARDEXIST_COMMON 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesLocYardExistCommonViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String locyard 						    = "";
		try{
			row    = condiVO.getLocyardexistRow();
			colname= condiVO.getLocyardexistColname();
			locyard= condiVO.getLocyardexistLocyard();
			result = retVO.getLocyardExistResult();
			if(result.equals("0")) {
				locyard = "";				
			}	
			sb.append("<DATA>\n");

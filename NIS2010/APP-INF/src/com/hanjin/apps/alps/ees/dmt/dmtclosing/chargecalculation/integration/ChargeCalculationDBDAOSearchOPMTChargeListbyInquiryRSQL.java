...
*@FileName : ChargeCalculationDBDAOSearchOPMTChargeListbyInquiryRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ChargeCalculationDBDAOSearchOPMTChargeListbyInquiryRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ChargeCalculationDBDAOSearchOPMTChargeListbyInquiryRSQL(){
		setQuery();
		...
		query.append("FileName : ChargeCalculationDBDAOSearchOPMTChargeListbyInquiryRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    (SELECT DECODE( DECODE(ACT_PAYR_CNT_CD,'00','',ACT_PAYR_CNT_CD) || TO_CHAR(ACT_PAYR_SEQ, 'FM000000'),'000000' , NULL, DECODE(ACT_PAYR_CNT_CD,'00','',ACT_PAYR_CNT_CD) || TO_CHAR(ACT_PAYR_SEQ, 'FM000000'))  FROM DMT_INV_MN WHERE DMDT_INV_NO = C.DMDT_INV_NO AND ROWNUM = 1)" ).append("\n"); 
		query.append("ELSE" ).append("\n"); 
		query.append("    DECODE( DECODE(C.ACT_CNT_CD,'00','',C.ACT_CNT_CD) || TO_CHAR(C.ACT_CUST_SEQ, 'FM000000'),'000000' , NULL, DECODE(C.ACT_CNT_CD,'00','',C.ACT_CNT_CD) || TO_CHAR(C.ACT_CUST_SEQ, 'FM000000'))" ).append("\n"); 
		query.append("END AS PAYER_CD" ).append("\n"); 
		query.append(",CASE WHEN  C.DMDT_CHG_STS_CD = 'I' THEN  -- 추가" ).append("\n"); 
		query.append("    CASE" ).append("\n"); 
		query.append("    WHEN C.DMDT_TRF_CD='DTIC'     AND INSTR(NVL(B.POD_CD,'  '),'US')=1 THEN" ).append("\n"); 
		query.append("    ( SELECT DECODE(ACT_PAYR_CNT_CD, '00', MV.VNDR_LGL_ENG_NM, MC.CUST_LGL_ENG_NM)" ).append("\n"); 
		query.append("    FROM MDM_VENDOR MV, DMT_INV_MN, MDM_CUSTOMER MC" ).append("\n"); 
		query.append("    WHERE DMDT_INV_NO = C.DMDT_INV_NO" ).append("\n"); 
		query.append("    AND   ( " ).append("\n"); 
		query.append("            ( ACT_PAYR_CNT_CD = '00' AND MV.VNDR_SEQ = ACT_PAYR_SEQ )" ).append("\n"); 
		query.append("          OR ( ACT_PAYR_CNT_CD <> '00' AND MC.CUST_CNT_CD = ACT_PAYR_CNT_CD AND MC.CUST_SEQ = ACT_PAYR_SEQ)" ).append("\n"); 
		query.append("          )" ).append("\n"); 
		query.append("    AND ROWNUM = 1" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		query.append("    WHEN C.DMDT_TRF_CD='DTOC'    AND INSTR(NVL(B.POL_CD,'  '),'US')=1 THEN" ).append("\n"); 
		query.append("    ( SELECT DECODE(ACT_PAYR_CNT_CD, '00', MV.VNDR_LGL_ENG_NM, MC.CUST_LGL_ENG_NM)" ).append("\n"); 
		query.append("    FROM MDM_VENDOR MV, DMT_INV_MN, MDM_CUSTOMER MC" ).append("\n"); 
		query.append("    WHERE DMDT_INV_NO = C.DMDT_INV_NO" ).append("\n"); 
		query.append("    AND   ( " ).append("\n"); 
		...
		query.append("    AND ROWNUM = 1" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		query.append("    WHEN C.DMDT_TRF_CD IN ('DMIF','CTIC', 'DMOF', 'CTOC') OR (C.DMDT_TRF_CD = 'DTIC' AND INSTR(NVL(B.POD_CD,'  '),'US')<>1) OR (C.DMDT_TRF_CD='DTOC'    AND INSTR(NVL(B.POL_CD,'  '),'US')<>1) THEN" ).append("\n"); 
		query.append("    ( SELECT MC.CUST_LGL_ENG_NM" ).append("\n"); 
		query.append("    FROM MDM_CUSTOMER MC, DMT_INV_MN" ).append("\n"); 
		query.append("    WHERE MC.CUST_CNT_CD = ACT_PAYR_CNT_CD" ).append("\n"); 
		query.append("    AND MC.CUST_SEQ = ACT_PAYR_SEQ" ).append("\n"); 
		query.append("    AND   DMDT_INV_NO = C.DMDT_INV_NO " ).append("\n"); 
		query.append("	AND   ROWNUM = 1)" ).append("\n"); 
		query.append("    END" ).append("\n"); 
		query.append("ELSE" ).append("\n"); 
		query.append("    CASE" ).append("\n"); 
		...
		query.append("	)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("ORDER BY B.CNTR_NO, C.FM_MVMT_DT" ).append("\n"); 

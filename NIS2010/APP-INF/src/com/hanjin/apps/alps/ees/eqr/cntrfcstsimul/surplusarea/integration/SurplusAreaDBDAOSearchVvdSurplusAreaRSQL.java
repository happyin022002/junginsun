...
public class SurplusAreaDBDAOSearchVvdSurplusAreaRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        -- HAMUR-->HAMRU, NYCNA-->NYCRA, SINWA-->SINRS" ).append("\n"); 
		query.append("        -- NVL(A.RHQ,DECODE(A.TRD_CD,'AES','HAMUR','TPS','NYCNA','IAS','SINWA','')) RHQ,  ," ).append("\n"); 
		query.append("		NVL(A.RHQ,DECODE(A.TRD_CD,'AES','HAMRU','TPS','NYCRA','IAS','SINRS','')) RHQ,  " ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("        FROM MDM_REV_LANE R, MDM_DTL_REV_LANE D, MDM_LOCATION L, MAS_MON_VVD C, MAS_WK_PRD P, VSK_VSL_PORT_SKD V, MDM_VSL_SVC_LANE M, MDM_LOCATION T" ).append("\n"); 
		query.append("        WHERE  1=1" ).append("\n"); 
		...
		query.append("                    D.RLANE_CD LIKE '$key'||'%'  -- LANE" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("        #end" ).append("\n"); 
		query.append("        )" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    #if(${subtrade} != '') -- S.Trade 콤보 조건 일 경우" ).append("\n"); 
		query.append("        AND (" ).append("\n"); 
		...
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    #if(${trade} != '')   -- Trade 콤보 조건 일 경우" ).append("\n"); 
		query.append("        AND (" ).append("\n"); 
		...
		query.append("            NVL(SUM(DECODE(D.CRR_CD,'SML',DECODE(D.BSA_OP_JB_CD,'007',NVL(D.CRR_BSA_CAPA,NVL(C.N2ND_FNL_HJS_BSA_CAPA,NVL(C.BSA_CAPA,0))),0))),0) FNL_HJS_BSA_CAPA_BSA," ).append("\n"); 
		query.append("            NVL(SUM(DECODE(D.CRR_CD,'SML',DECODE(D.BSA_OP_JB_CD,'009',NVL(D.CRR_BSA_CAPA,NVL(C.N2ND_FNL_HJS_BSA_CAPA,NVL(C.BSA_CAPA,0))),0))),0) FNL_HJS_BSA_CAPA_WT" ).append("\n"); 
		query.append("        FROM MAS_MON_VVD A, MAS_LANE_RGST B, BSA_VVD_MST C, BSA_VVD_OTR_CRR D, BSA_VVD_SWAP_INFO F " ).append("\n"); 
		...
		query.append("                     , C.VSL_CD, C.SKD_VOY_NO, C.SKD_DIR_CD, NVL(C.N2ND_FNL_HJS_BSA_CAPA,NVL(C.BSA_CAPA,0))" ).append("\n"); 

...
*@FileName : EES_DMT_7017HTMLAction.java
...
public class EES_DMT_7017HTMLAction extends HTMLActionSupport {
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EesDmt7017Event event = new EesDmt7017Event();
        
        if(command.isCommand(FormCommand.SEARCH)) {
        	event.setSearchChgDeltPathStupVO((SearchChgDeltPathStupVO)getVO(request, SearchChgDeltPathStupVO.class));
		} 
        else if(command.isCommand(FormCommand.MULTI)) {
        	event.setChgDeltPathStupVOs((ChgDeltPathStupVO[])getVOs(request, ChgDeltPathStupVO.class, ""));
		} 
        // [2015-10-20 김기태] Tab 구분 After BKG Approval Path 추가
        else if(command.isCommand(FormCommand.SEARCH01)) {
        	event.setAftBkgPathSetupVO((AftBkgPathSetupVO)getVO(request, AftBkgPathSetupVO.class));
        } 
        else if(command.isCommand(FormCommand.MULTI01)) {
        	event.setAftBkgPathSetupVOs((AftBkgPathSetupVO[])getVOs(request, AftBkgPathSetupVO.class, ""));
        } 

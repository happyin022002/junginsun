...
*@FileName : EES_EQR_0014HTMLAction.java
*@FileTitle : BSA 조회 / 수정
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.6
 */
public class EES_EQR_0014HTMLAction extends HTMLActionSupport {
	...
	/**
	 * EES_EQR_0014HTMLAction 객체를 생성
	 */
	public EES_EQR_0014HTMLAction() {
	}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EesEqr0014Event로 파싱하여 request에 셋팅<br>
	 * 
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0014Event event = new EesEqr0014Event();		
		event.setConditionVO((EesEqr0014ConditionVO)getVO(request, EesEqr0014ConditionVO.class));
		String newTitle = event.getConditionVO().getNewTitle();
		newTitle = Utils.replaceStringAll(newTitle,"|",",");
		
        if(f_cmd.isCommand(FormCommand.MULTI)) {
        	//  sheet layout 정보를 저장한다. 
        	event.setEqrScnrBsaVvdVOS((EqrScnrBsaVvdVO[])getVOs(request ,EqrScnrBsaVvdVO.class , "" ));
		} else if(f_cmd.isCommand(FormCommand.MULTI01)) {
        	// sheet layout 정보를 저장한다. 
       	  	event.setEqrScnrBsaPortVOS((EqrScnrBsaPortVO[])getVOs(request, EqrScnrBsaPortVO.class,""));
		}        

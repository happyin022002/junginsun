...
public class ChassisMgsetInvoiceDBDAOsearchMGSLeaseInvoiceVerifyDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT   /*+  USE_NL( A B D)   */" ).append("\n"); 
		...
		query.append("       , CASE WHEN A.CHG_CD NOT IN ('CRD') AND A.EQ_NO = LAG(A.EQ_NO) OVER (PARTITION BY  A.EQ_NO, A.CHG_CD, A.INV_LSE_RT_AMT ORDER BY EQ_NO  )  THEN 'Equipment No. Duplicated!'" ).append("\n"); 
		query.append("              ELSE A.VRFY_RSLT_DESC" ).append("\n"); 
		query.append("         END AS VRFY_RSLT_DESC" ).append("\n"); 
		query.append("       , CASE WHEN A.CHG_CD NOT IN ('CRD') AND A.EQ_NO = LAG(A.EQ_NO) OVER (PARTITION BY  A.EQ_NO, A.CHG_CD, A.INV_LSE_RT_AMT ORDER BY EQ_NO  )  THEN 'N'" ).append("\n"); 
		query.append("              ELSE A.VRFY_SCS_FLG" ).append("\n"); 
		query.append("         END AS VRFY_SCS_FLG" ).append("\n"); 
		query.append("       , A.INV_SEQ" ).append("\n"); 
		query.append("       , CASE WHEN B.EQ_NO  IS NOT NULL THEN CASE WHEN B.LSE_CHG_AMT = A.INV_LSE_CHG_AMT THEN 'C' ELSE 'D' END   --CHG_AMT 같으면 C , 다르면 D" ).append("\n"); 
		query.append("              ELSE 'I'" ).append("\n"); 
		query.append("         END LSE_CHG_AUD_STS_CD" ).append("\n"); 
		query.append("       , CASE WHEN B.EQ_NO IS NULL THEN NVL( ( CASE WHEN D.EQ_ASET_STS_CD = 'LSO' THEN 'P' WHEN D.EQ_ASET_STS_CD = 'LST' THEN 'Q' WHEN D.EQ_ASET_STS_CD = 'TLL' THEN 'R' ELSE 'I' END ) , 'O')" ).append("\n"); 
		query.append("              WHEN B.LSE_CHG_AMT <> REPLACE(REPLACE(A.INV_LSE_CHG_AMT,' ',''),',','') THEN CASE WHEN B.LSE_USE_DYS <> REPLACE(REPLACE(A.INV_LSE_USE_DYS,' ',''),',','') AND B.LSE_RT_AMT  <> REPLACE(REPLACE(A.INV_LSE_RT_AMT,' ',''),',','') THEN 'E'" ).append("\n"); 
		...
		query.append("           SELECT   EQ_KND_CD" ).append("\n"); 
		...
		query.append("           FROM     (" ).append("\n"); 
		query.append("                      SELECT   A.EQ_KND_CD" ).append("\n"); 
		query.append("                             , A.COST_YRMON" ).append("\n"); 
		query.append("                             , A.CHG_CRE_SEQ" ).append("\n"); 
		query.append("                             , A.INV_CUST_EQ_NO" ).append("\n"); 
		query.append("                             , A.INV_EQ_NO" ).append("\n"); 
		...
		query.append("                             , A.INV_LSE_CHG_AMT" ).append("\n"); 
		query.append("                             , A.INV_TAX_AMT" ).append("\n"); 
		query.append("                             , NVL( CASE WHEN  LENGTH( TO_CHAR(INV_EQ_ONH_DT  ,'YYYYMMDD' ) ) <>8  THEN 'Please check up the On-Hire Date!'" ).append("\n"); 
		...
		query.append("                                               CASE WHEN SUBSTR(INV_CHG_TP_NM,1,3) NOT IN ('X S', 'X V', 'X W', 'XSE', 'XVI', 'XWA', 'CRD','TAX') THEN CASE WHEN D.EQ_NO IS NULL AND E.EQ_NO IS NULL AND F.EQ_NO IS NULL AND D2.EQ_NO IS NULL AND D3.EQ_NO IS NULL THEN	'M.G.Set No does not exist!'" ).append("\n"); 
		...
		query.append("                             , DECODE(NVL( CASE WHEN LENGTH( TO_CHAR(INV_EQ_ONH_DT  ,'YYYYMMDD' ) ) <>8  THEN  'Please check up the On-Hire Date!'" ).append("\n"); 
		query.append("                                                WHEN LENGTH( TO_CHAR(INV_EQ_OFFH_DT ,'YYYYMMDD' ) ) <>8  THEN  'Please check up the Off-Hire Date!'" ).append("\n"); 
		query.append("                                                WHEN LENGTH( TO_CHAR(INV_BIL_ST_DT  ,'YYYYMMDD' ) ) <>8  THEN  'Please check up the Billing Start Date!'" ).append("\n"); 
		query.append("                                                WHEN LENGTH( TO_CHAR(INV_BIL_END_DT ,'YYYYMMDD' ) ) <>8  THEN  'Please check up the Billing End Date!'" ).append("\n"); 
		query.append("                                                WHEN SUBSTR(INV_CHG_TP_NM,1,3) IN ('HON','DON') AND LENGTH( TO_CHAR(INV_EQ_ONH_DT,'YYYYMMDD' ) ) <> 8 THEN  'Please check up the On-Hire Date!'" ).append("\n"); 
		query.append("                                                WHEN INV_CHG_TP_NM IN ('HOF','DOF') AND LENGTH( TO_CHAR(INV_EQ_OFFH_DT,'YYYYMMDD' ) ) <> 8 THEN  'Please check up the Off-Hire Date!'" ).append("\n"); 
		query.append("                                                WHEN G.AGMT_OFC_CTY_CD IS NULL THEN     'Please check up the Reference No!(AGMT NO Does Not Matching or Invalid Agreement No)'" ).append("\n"); 
		query.append("                                                ELSE CASE WHEN SUBSTR(INV_CHG_TP_NM,1,3) NOT IN ('X S', 'X V', 'X W', 'XSE', 'XVI', 'XWA', 'CRD','TAX') THEN CASE WHEN D.EQ_NO IS NULL AND E.EQ_NO IS NULL AND F.EQ_NO IS NULL  AND D2.EQ_NO IS NULL AND D3.EQ_NO IS NULL THEN	'M.G.Set No does not exist!'" ).append("\n"); 
		query.append("                                                                                                                                                                  WHEN A.INV_EQ_NO IS NULL AND A.INV_CUST_EQ_NO IS NOT NULL AND E.EQ_NO IS NULL AND F.EQ_NO IS NULL THEN  'Lessor M.G.Set No does not exist!'" ).append("\n"); 
		query.append("                                                                                                                                                             END" ).append("\n"); 
		query.append("                                                     END" ).append("\n"); 
		...
		query.append("                             , CASE WHEN A.CHG_CD = 'PDM' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LP' THEN 'EQCZLP'" ).append("\n"); 
		query.append("                                                                    ELSE 'EQCZLT'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD = 'WDP' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LP' THEN 'EQCZLP'" ).append("\n"); 
		query.append("                                                                    ELSE 'EQCZLT'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD = 'DOC' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                                                    ELSE 'EQCZLT'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('HON','DON') THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('HOF','DOF') THEN 'EQCZLT'" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('GTO','GTI') THEN CASE WHEN C.AGMT_LSTM_CD = 'FO' THEN 'EQCZSB'" ).append("\n"); 
		query.append("                                                                             WHEN C.AGMT_LSTM_CD = 'SB' THEN 'EQCZSB'" ).append("\n"); 
		query.append("                                                                             WHEN C.AGMT_LSTM_CD = 'SO' THEN 'EQCZSB'" ).append("\n"); 
		query.append("                                                                        END" ).append("\n"); 
		query.append("                                    ELSE CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN 'EQXTXX'" ).append("\n"); 
		query.append("                                              WHEN C.AGMT_LSTM_CD = 'LT' THEN 'EQXTXX'" ).append("\n"); 
		query.append("                                              ELSE 'EQXTXX'" ).append("\n"); 
		query.append("                                         END" ).append("\n"); 
		query.append("                               END AS COST_CD" ).append("\n"); 
		query.append("                             , CASE WHEN A.CHG_CD = 'PDM' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN '510831'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN '510831'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LP' THEN '510831'" ).append("\n"); 
		query.append("                                                                    ELSE '510831'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD = 'WDP' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN '510831'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN '510831'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LP' THEN '510831'" ).append("\n"); 
		query.append("                                                                    ELSE '510831'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD = 'DOC' THEN CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN '510831'" ).append("\n"); 
		query.append("                                                                    WHEN C.AGMT_LSTM_CD = 'LT' THEN '510831'" ).append("\n"); 
		query.append("                                                                    ELSE '510831'" ).append("\n"); 
		query.append("                                                               END" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('HON','DON') THEN '510831'" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('HOF','DOF') THEN '510831'" ).append("\n"); 
		query.append("                                    WHEN A.CHG_CD IN ('GTO','GTI') THEN CASE WHEN C.AGMT_LSTM_CD = 'FO' THEN '510831'" ).append("\n"); 
		query.append("                                                                             WHEN C.AGMT_LSTM_CD = 'SB' THEN '510831'" ).append("\n"); 
		query.append("                                                                             WHEN C.AGMT_LSTM_CD = 'SO' THEN '510831'" ).append("\n"); 
		query.append("                                                                        END" ).append("\n"); 
		query.append("                                    ELSE CASE WHEN C.AGMT_LSTM_CD = 'ST' THEN '510831'" ).append("\n"); 
		query.append("                                              WHEN C.AGMT_LSTM_CD = 'LT' THEN '510831'" ).append("\n"); 
		query.append("                                              ELSE '510831'" ).append("\n"); 
		query.append("                                         END" ).append("\n"); 
		...
		query.append("                     LEFT JOIN CGM_EQUIPMENT D ON (A.INV_EQ_NO = D.EQ_NO AND D.EQ_KND_CD = @[eq_knd_cd])      --EQ_NO JN" ).append("\n"); 

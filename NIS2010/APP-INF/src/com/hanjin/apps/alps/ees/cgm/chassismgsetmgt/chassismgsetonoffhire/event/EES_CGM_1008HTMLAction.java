...
*@FileName : EES_CGM_1008HTMLAction.java
...
public class EES_CGM_1008HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_CGM_1008HTMLAction 객체를 생성
	 */
	public EES_CGM_1008HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesCgm1008Event event = new EesCgm1008Event();
		if(command.isCommand(FormCommand.SEARCH)) {
			log.debug("####### SEARCH #######");
			event.setCHSOnHireINVO((CHSOnHireINVO)getVO(request, CHSOnHireINVO .class));
		}
		// VERIFY METHOD
		else if(command.isCommand(FormCommand.SEARCHLIST)) {
			log.debug("####### SEARCHLIST #######");
			event.setCHSOnHireMGTVOs((CHSOnHireMGTVO[])getVOs(request, CHSOnHireMGTVO.class, ""));
		}
		// DATA DUPLICATOR COMPARE METHOD
		else if(command.isCommand(FormCommand.SEARCH03)) {
			log.debug("####### SEARCH03 #######");
			event.setCHSOnHireINVO((CHSOnHireINVO)getVO(request, CHSOnHireINVO .class));
		}
		// OWN, LEASE COMPARE METHOD

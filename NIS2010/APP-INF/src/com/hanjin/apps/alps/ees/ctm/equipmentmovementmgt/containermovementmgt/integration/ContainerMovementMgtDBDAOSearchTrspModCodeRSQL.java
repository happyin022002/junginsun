...
public class ContainerMovementMgtDBDAOSearchTrspModCodeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(" WHERE A.PCTL_NO IN (SELECT PCTL_NO" ).append("\n"); 
		query.append("                     FROM SCE_COP_HDR" ).append("\n"); 
		query.append("                    WHERE BKG_NO = @[bkg_number]" ).append("\n"); 
		query.append("                      AND CNTR_NO = @[cntr_no])" ).append("\n"); 
		query.append("   AND A.ORG_NOD_CD = @[event_yard]" ).append("\n"); 
		query.append("   AND A.NOD_LNK_DIV_CD = 'L'" ).append("\n"); 
		query.append("   AND A.TRSP_MOD_CD = 'RD'" ).append("\n"); 
		query.append("   AND EXISTS (  SELECT 'Y' FROM BKG_BOOKING" ).append("\n"); 
		query.append("                 WHERE  BKG_NO = @[bkg_number]" ).append("\n"); 
		query.append("                 AND    SUBSTR(@[event_yard],1,5) = POD_CD" ).append("\n"); 

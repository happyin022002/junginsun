...
*@FileName : EES_CGM_1145HTMLAction.java
...
public class EES_CGM_1145HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_CGM_1145HTMLAction 객체를 생성
	 */
	public EES_CGM_1145HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesCgm1145Event event = new EesCgm1145Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setPoolUseddysINVO((PoolUseddysINVO)getVO(request, PoolUseddysINVO .class));
		}
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setPoolUseddysINVO((PoolUseddysINVO)getVO(request, PoolUseddysINVO .class));
			event.setPoolUseddysMGTVOs((PoolUseddysMGTVO[])getVOs(request, PoolUseddysMGTVO .class,""));

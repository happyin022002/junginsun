...
public class PayableRentalCostDBDAOPayableChargeAuditRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       , DECODE(A.CNTR_AUD_STS_CD, 'A', A.DSCR_COST_AMT, D.TTL_COST_AMT)                AS DSCR_COST_AMT" ).append("\n"); 
		query.append("       , A.AGMT_CTY_CD" ).append("\n"); 
		query.append("       , A.AGMT_SEQ" ).append("\n"); 
		query.append("       , A.DTL_SEQ" ).append("\n"); 
		query.append("       , B.CHG_COST_YRMON" ).append("\n"); 
		query.append("       , CASE WHEN A.CNTR_AUD_STS_CD = 'A' THEN A.CNTR_AUD_STS_CD" ).append("\n"); 
		query.append("              WHEN D.CNTR_NO IS NULL THEN 'H'" ).append("\n"); 
		query.append("              ELSE" ).append("\n"); 
		query.append("                   CASE WHEN     A.TTL_COST_AMT = D.TTL_COST_AMT" ).append("\n"); 
		query.append("                             AND A.PD_RT_AMT = D.PD_RT_AMT" ).append("\n"); 
		query.append("                             AND A.ONH_DT = ( CASE WHEN TO_CHAR(A.ONH_DT+A.CHG_FREE_DYS, 'YYYYMM') = B.CHG_COST_YRMON THEN D.ONH_DT ELSE A.ONH_DT END )" ).append("\n"); 
		...
		query.append("       , CASE WHEN A.CNTR_AUD_STS_CD = 'A' THEN A.PAY_CHG_STS_CD" ).append("\n"); 
		query.append("              WHEN D.CNTR_NO IS NULL THEN 'H'" ).append("\n"); 
		query.append("              ELSE" ).append("\n"); 
		query.append("                   CASE WHEN     A.TTL_COST_AMT = D.TTL_COST_AMT" ).append("\n"); 
		query.append("                             AND A.PD_RT_AMT = D.PD_RT_AMT" ).append("\n"); 
		...
		query.append("       , NULL AS CR_AMT" ).append("\n"); 
		...
		query.append("           WHERE    1 = 1" ).append("\n"); 
		query.append("           AND      CNTR_AUD_STS_CD = 'H'" ).append("\n"); 

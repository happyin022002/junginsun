...
public class EES_MST_0032HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_MST_0004HTMLAction 객체를 생성
	 */
	public EES_MST_0032HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesMst0032Event event = new EesMst0032Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setEqAsetAudVO((EqAsetAudVO)getVO(request, EqAsetAudVO .class));
		} else if (command.isCommand(FormCommand.SEARCH01)){
			event.setEqAsetAudVO((EqAsetAudVO)getVO(request, EqAsetAudVO .class));
		} else if (command.isCommand(FormCommand.MULTI)){
			event.setEqAsetAudVO((EqAsetAudVO)getVO(request, EqAsetAudVO .class));
			event.setAssetsAuditDetailVOs((AssetsAuditDetailVO[])getVOs(request, AssetsAuditDetailVO .class,""));
		}

...
public class PerformanceReportDBDAOsearchRepairPFMCByACCTListDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("           WHERE    1 = 1" ).append("\n"); 
		query.append("           AND      GD.MNR_CD_ID = MRD.COST_DTL_CD" ).append("\n"); 
		query.append("           AND      GD.PRNT_CD_ID IN" ).append("\n"); 
		query.append("                    (" ).append("\n"); 
		query.append("                      SELECT   A.MNR_CD_ID" ).append("\n"); 
		query.append("                      FROM     MNR_GEN_CD A" ).append("\n"); 
		query.append("                      WHERE    1 = 1" ).append("\n"); 
		query.append("                      AND      A.MNR_CD_ID = MRD.COST_CD" ).append("\n"); 
		...
		query.append("       , ROUND(SUM(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MPRH.CRE_DT, 'YYYYMM'), MPRH.CURR_CD, DECODE(@[usd_only], 'Y', 'USD', MPRH.CURR_CD), MPRH.MNR_WRK_AMT)) / SUM(DECODE(MRD.RPR_QTY,0,1,NVL(MRD.RPR_QTY,1))),2) AS AVG_AMT" ).append("\n"); 
		query.append("       , SUM(DECODE(MRD.COST_CD,'MRDRHD', 1, DECODE(MRD.RPR_QTY, 0, 1, NVL(MRD.RPR_QTY, 1)))) AS TOTAL_QTY" ).append("\n"); 
		query.append("       , MPRH.EQ_TPSZ_CD" ).append("\n"); 
		query.append("FROM     MNR_ORD_DTL MRD" ).append("\n"); 
		query.append("       , MNR_RPR_RQST_HDR MPRH" ).append("\n"); 
		query.append("WHERE    1 = 1" ).append("\n"); 
		...
		query.append("       , SUM(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MOH.CRE_DT, 'YYYYMM'), MOH.CURR_CD, DECODE(@[usd_only], 'Y', 'USD', MOH.CURR_CD), MRD.COST_AMT)) AS TOTAL_AMT" ).append("\n"); 
		query.append("       , ROUND(SUM(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MOH.CRE_DT, 'YYYYMM'), MOH.CURR_CD, DECODE(@[usd_only], 'Y', 'USD', MOH.CURR_CD), MRD.COST_AMT)) / SUM(DECODE(MRD.RPR_QTY,0,1,NVL(MRD.RPR_QTY,1))),2) AS AVG_AMT" ).append("\n"); 
		query.append("       , SUM(DECODE(MRD.COST_CD,'MRDRHD', 1, DECODE(MRD.RPR_QTY, 0, 1, NVL(MRD.RPR_QTY, 1)))) AS TOTAL_QTY" ).append("\n"); 
		query.append("       , MRD.EQ_TPSZ_CD" ).append("\n"); 
		query.append("FROM     MNR_ORD_HDR MOH" ).append("\n"); 
		query.append("       , MNR_ORD_DTL MRD" ).append("\n"); 
		query.append("WHERE    1 = 1" ).append("\n"); 
		...
		query.append("         ) AS CNT_NM" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , DECODE(MRD.ACCT_CD,'511591','Other',(SELECT DECODE(SIGN(INSTR(MNR_CD_DESC, 'Repair Labor')), 1, 'Repair Labor', MNR_CD_DESC) FROM MNR_GEN_CD GD WHERE GD.MNR_CD_ID = MRD.COST_CD AND GD.PRNT_CD_ID IN ('UG','ZG','GG') AND ROWNUM = 1)) AS COST_CD_NM" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		...
		query.append("       , SUM(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MIW.CRE_DT, 'YYYYMM'), MIW.CURR_CD, DECODE(@[usd_only], 'Y', 'USD', MIW.CURR_CD), NVL(MRD.INV_AMT,0))) AS TOTAL_AMT" ).append("\n"); 
		query.append("       , ROUND(SUM(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(TO_CHAR(MIW.CRE_DT, 'YYYYMM'), MIW.CURR_CD, DECODE(@[usd_only], 'Y', 'USD', MIW.CURR_CD), NVL(MRD.INV_AMT,0))) / SUM(DECODE(MRD.RPR_QTY,0,1,NVL(MRD.RPR_QTY,1))),2) AS AVG_AMT" ).append("\n"); 
		query.append("       , SUM(DECODE(MRD.COST_CD,'MRDRHD', 1, DECODE(MRD.RPR_QTY, 0, 1, NVL(MRD.RPR_QTY, 1)))) AS TOTAL_QTY" ).append("\n"); 
		query.append("       , MRD.EQ_TPSZ_CD" ).append("\n"); 
		query.append("FROM     MNR_ORD_HDR MOH" ).append("\n"); 
		query.append("       , MNR_ORD_DTL MRD" ).append("\n"); 
		query.append("       , MNR_PAY_INV_WRK MIW" ).append("\n"); 
		query.append("WHERE    1 = 1" ).append("\n"); 
		query.append("AND      MOH.MNR_ORD_OFC_CTY_CD  = MRD.MNR_ORD_OFC_CTY_CD" ).append("\n"); 
		query.append("AND      MOH.MNR_ORD_SEQ         = MRD.MNR_ORD_SEQ" ).append("\n"); 
		...
		query.append("	#if (${report_period_type} == 'EI')" ).append("\n"); 
		query.append("AND      MPRH.EQ_KND_CD IN" ).append("\n"); 
		query.append("         (" ).append("\n"); 
		...
		query.append("         )" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		query.append("AND      MOH.EQ_KND_CD IN" ).append("\n"); 
		query.append("         (" ).append("\n"); 
		query.append("		#foreach ($user_eqTypes IN ${eqTypes})" ).append("\n"); 
		query.append("			#if($velocityCount < $eqTypes.size())" ).append("\n"); 
		query.append("				'$user_eqTypes'," ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("#if (${report_period_type} == 'EI')" ).append("\n"); 
		query.append("GROUP BY MPRH.COST_OFC_CD" ).append("\n"); 
		query.append("       , MRD.ACCT_CD" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		query.append("       , MPRH.VNDR_SEQ" ).append("\n"); 
		query.append("       , MPRH.EQ_TPSZ_CD" ).append("\n"); 
		query.append("       , MPRH.CURR_CD" ).append("\n"); 
		query.append("ORDER BY MRD.ACCT_CD" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		query.append("       , MNR_COMMON_PKG.MNR_GET_RHQ_OFC_FNC(MPRH.COST_OFC_CD)" ).append("\n"); 
		query.append("       , MPRH.COST_OFC_CD" ).append("\n"); 
		query.append("#elseif (${report_period_type} == 'WI') " ).append("\n"); 
		query.append("GROUP BY MOH.EQ_KND_CD" ).append("\n"); 
		query.append("       , MOH.COST_OFC_CD" ).append("\n"); 
		query.append("       , MRD.ACCT_CD" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		...
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		query.append("       , MNR_COMMON_PKG.MNR_GET_RHQ_OFC_FNC(MOH.COST_OFC_CD)" ).append("\n"); 
		query.append("       , MOH.COST_OFC_CD" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("GROUP BY MOH.EQ_KND_CD" ).append("\n"); 
		query.append("       , MIW.ISS_OFC_CD" ).append("\n"); 
		query.append("       , MRD.ACCT_CD" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		query.append("       , MIW.MNR_PRNR_SEQ" ).append("\n"); 
		query.append("       , MRD.EQ_TPSZ_CD" ).append("\n"); 
		query.append("       , MIW.CURR_CD" ).append("\n"); 
		query.append("ORDER BY MRD.ACCT_CD" ).append("\n"); 
		query.append("       , MRD.COST_CD" ).append("\n"); 
		query.append("       , MRD.COST_DTL_CD" ).append("\n"); 
		query.append("       , MNR_COMMON_PKG.MNR_GET_RHQ_OFC_FNC(MIW.ISS_OFC_CD)" ).append("\n"); 
		query.append("       , MIW.ISS_OFC_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 

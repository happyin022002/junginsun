...
*@FileName : EesVvdInlandCommonViewAdapter.java
*@FileTitle : EES_VVDINLAND_COMMON 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_VVDINLAND_COMMON 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesVvdInlandCommonViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String[] result 					    = null;
		try{
			row    = condiVO.getVvdinlandRow();
			colname= condiVO.getVvdinlandColname();
			result = retVO.getVvdInlandResult();
			
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ colname +"\"  DATA-TYPE=\"dtCombo\" COMBO-TEXT='"+JSPUtil.getNull(result[0])+"' COMBO-CODE='"+JSPUtil.getNull(result[1])+"' ></TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

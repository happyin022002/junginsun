 ...
 *            조회 조건 추가 (Loc_Loc), searchCargoFlowMap 변경
 * 2010.10.01 남궁진호 [CHM-201006256-01] searchLoadFactorByTrade,searchMBByVessel
 *            Session User Id 변경, CreUsrId -> UsrId    
 * 2012.03.26 신자영 [CHM-201216788-01] M/B 기능 보완 요청
 * 2012.06.18 신자영 [CHM-201218336-01] INVENTORY / M/B 기능 수정 요청
...
import com.hanjin.apps.alps.ees.cim.cntroperationperformancemgt.eqmatchbacknloadfactormgt.integration.EQMatchBackNLoadFactorMgtDBDAO;
 ...
 * @see EES_CIM_1023EventResponse,EQMatchBackNLoadFactorMgtBC 각 DAO 클래스 참조
 * @since J2EE 1.6
 */
public class EQMatchBackNLoadFactorMgtBCImpl extends BasicCommandSupport implements EQMatchBackNLoadFactorMgtBC {
	...
	/**
	 * EQMatchBackNLoadFactorMgtBCImpl 객체 생성<br>
	 * EQMatchBackNLoadFactorMgtDBDAO를 생성한다.<br>
	 */
	...
	public List<QuantityByTypeSizeVO> searchTMLMBByLogisticWiseSummary(
		...
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				...
				// Match Back 만 VO에 다시 담는다.(SUMMARY Tab)
	...
	/**
	 * [Terminal MatchBack Detail]을 [조회] 합니다.<br>
	 * 
	 * @param mBSearchOptionInGereralVO
	 * @return List<QuantityByTypeSizeVO>
	 * @exception EventException
	...
	public List<QuantityByTypeSizeVO> searchLaneMBByLogisticWise(MBSearchOptionInGereralVO mBSearchOptionInGereralVO)
		...
		try {
			...
			// 조회된 결과에 S.Total,G.Total 이 담긴 편집된 리스트를 담는다.
			List<QuantityByTypeSizeVO> list2 = new ArrayList<QuantityByTypeSizeVO>(); 
			QuantityByTypeSizeVO vo = null;
...
//		log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseInSummary 시작");
		try {
...
//			log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseInSummary lastRNo [" + lastRNo+ "]");
			...
			list2.add(x, vo2);
			x++;
//			log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseInSummary 끝");
			return list2;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	public List<QuantityByTypeSizeVO> searchLocationMBByLogisticWiseInDetail(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		try {
			//log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseInDetail 시작");
			List<QuantityByTypeSizeVO> list = dbDao.searchLocationMBByLogisticWiseInDetail(mBSearchOptionInGereralVO);

			//log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseInDetail 끝");
			return list;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	/**
	 * [Logistic MatchBack Trend]을 [조회] 합니다.<br>
	 * 
	 * @param mBSearchOptionInGereralVO
	...
	public MatchBackByMonthlyWeeklyTrendSetVO searchTmnlMBByLogiticWiseByTrend(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		log.debug("####### EQMatchBackNLoadFactorMgtBC.searchTmnlMBByLogiticWiseByTrend 시작");
		try { 

			MatchBackByMonthlyWeeklyTrendSetVO list = dbDao.searchTmnlMBByLogiticWiseByTrend(mBSearchOptionInGereralVO);
			...
			for (int i = 0; i < list1.size(); i++) {
				...
				if (strLoccode.equals(xTempLocCdVal) || i == 0) {
					...
					vo.setYardCd(list1.get(i).getYardCd());
	...
	public MatchBackByMonthlyWeeklyTrendSetVO searchLocationMBByLogisticWiseByTrend(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByLogisticWiseByTrend 시작");
		try {
	...
	public List<QuantityByTypeSizeVO> searchMBByServiceMode(MBSearchOptionInGereralVO mBSearchOptionInGereralVO)
			throws EventException {
		/**
		 * String xTempLocCdVal = ""; for ( int i=0; i< list.size(); i++ ) {
		 * String strLocCd = list.get(i).getLocCd(); // 조회된 list 에서 row 하나를 리턴할
		 * list2 에 담기 if( xTempLocCdVal.equals(strLocCd) || i==0 ){ // Location이
		 * 달라지는 시점에 S.Total 추가를 위한 체크 if( strVvd.equals("LOC") ){
		 * if(strDivision.equals("M/B(%)")){ // LOC 의 S.Total G.Total 을 위한 배열
		 * 만들기 } }else if( strVvd.equals("IPI") ){
		 * if(strDivision.equals("M/B(%)")){ // IPI 의 S.Total G.Total 을 위한 배열
		 * 만들기 } }else if( strVvd.equals("MLB") ){
		 * if(strDivision.equals("M/B(%)")){ // MLB 의 S.Total G.Total 을 위한 배열
		 * 만들기 } } }else if( !xTempLocCdVal.equals(strLocCd) || i!=0 ){ //
		 * Location 이 달라져서 S.Total 추가 // S.Total 추가 // S.Total 계산용 배열 리셋 // 조회된
		 * list 에서 row 하나를 리턴할 list2 에 담기 if( strVvd.equals("LOC") ){
		 * if(strDivision.equals("M/B(%)")){ // LOC 의 S.Total G.Total 을 위한 배열
		 * 만들기 } }else if( strVvd.equals("IPI") ){
		 * if(strDivision.equals("M/B(%)")){ // IPI 의 S.Total G.Total 을 위한 배열
		 * 만들기 } }else if( strVvd.equals("MLB") ){
		 * if(strDivision.equals("M/B(%)")){ // MLB 의 S.Total G.Total 을 위한 배열
		 * 만들기 } } } xTempLocCdVal = strLocCd; } // 마지막 Location S.Total 추가 //
		 * G.Total 추가
		 */
		try {
			log.debug("####### EQMatchBackNLoadFactorMgtBC.searchMBByServiceMode 시작");
			// 조회된 결과를 담은 리스트
			List<QuantityByTypeSizeVO> list = dbDao.searchMBByServiceMode(mBSearchOptionInGereralVO); 
			// 조회된 결과에 S.Total,G.Total 이 담긴 편집된 리스트
			List<QuantityByTypeSizeVO> list2 = new ArrayList<QuantityByTypeSizeVO>(); 
			QuantityByTypeSizeVO vo = null;
...
//				log.debug("####### EQMatchBackNLoadFactorMgtBC.searchMBByServiceMode 1032 lastRNo [" + lastRNo + "]");
				...
				for (int i = 0; i < lastRNo; i++) {
					...
					else if (!xTempLocCdVal.equals(strLocCd) || i != 0) { // 소계
			...
			//log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchMBByServiceMode 끝");
			return list2;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	public List<QuantityByTypeSizeVO> searchLocationMBByBKGWiseInSummary(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseInSummary 1027 시작");
		try {
		...
		//	log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseInSummary lastRNo [" + lastRNo+ "]");
			...
			list2.add(x, vo2);
			x++;
		//	log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseInSummary 1027 끝");
			return list2;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	public List<QuantityByTypeSizeVO> searchLocationMBByBKGWiseInDetail(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		try {
		//	log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseInDetail 1027 시작");
			List<QuantityByTypeSizeVO> list = dbDao.searchLocationMBByBKGWiseInDetail(mBSearchOptionInGereralVO);
		...
		//	log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseInDetail 1027 끝");
			return list2;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	public MatchBackByMonthlyWeeklyTrendSetVO searchLocationMBByBKGWiseByTrend(
			MBSearchOptionInGereralVO mBSearchOptionInGereralVO) throws EventException {
		//log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseByTrend 1027 시작");
		try {
		...
		//	log.debug("####### EQMatchBackNLoadFactorMgtBC.searchLocationMBByBKGWiseByTrend 1027 끝");
			return list;
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011",
	...
	public List<QuantityByTypeSizeVO> searchCargoFlowMap(SearchOptionByFromToVO searchOptionByFromToVO)
			throws EventException {
		try {
			if(searchOptionByFromToVO.getDirectionWise().equals("L")){       			// Loc_Loc 조회
				...
				List<QuantityByTypeSizeVO> listSum = list4.getQuantitybytypesizevo2(); // 조회결과
				// S.Total
				List<QuantityByTypeSizeVO> list = list4.getQuantitybytypesizevo(); // 조회결과
				List<QuantityByTypeSizeVO> list2 = new ArrayList<QuantityByTypeSizeVO>(); // 리턴용
				...
				if (list.size() > 1) {
					...
					double[] volQtyT = new double[41]; // 임시 해당 Vol 저장
...
/*	
	public List<QuantityByTypeSizeVO> searchCargoFlowMap(SearchOptionByFromToVO searchOptionByFromToVO)
			throws EventException {
	//	log.debug("####### EQMatchBackNLoadFactorMgtBC.searchCargoFlowMap 1029 시작");
		try {
			if(searchOptionByFromToVO.getDirectionWise().equals("L")){       			// Loc_Loc 조회
				List<QuantityByTypeSizeVO> list = new ArrayList<QuantityByTypeSizeVO>(); 
...
//					x++;
			...
			//	log.debug("@@@@@@@ EQMatchBackNLoadFactorMgtBC.searchCargoFlowMap 1029 끝");
				return list2;
			}
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
...
*/
	...
	public List<QuantityByTypeSizeVO> searchCargoFlowMapTrend(SearchOptionByFromToVO searchOptionByFromToVO) throws EventException {
		try {									
			return dbDao.searchCargoFlowMapTrend(searchOptionByFromToVO);				
		} catch(DAOException ex) {				
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"Cargo Flow Map Retrieve"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"Cargo Flow Map Retrieve"}).getMessage(),ex);
		}				
	}
	...
	/**
	 * [MatchBack By Vessel Lane List]을 [SEARCH By Trade] 합니다.<br>
	 * 
	 * @param searchOptionByTradeLaneVvdVO
	 * @return String[]
	 * @exception EventException
	...
	/**
	 * [MatchBack By Vessel VVD List]을 [SEARCH By Trade,Lane] 합니다.<br>
	 * 
	 * @param searchOptionByTradeLaneVvdVO
	 * @return String[]
	 * @exception EventException
	...
	/*
	 * 
	 * TEU Total : 산출로직 = Full TEU Sum { 20’+ 2 ( 40’+ H/C’+ 45’) } + MTY TEU
	 * Sum { 20’+ 2 ( 40’+ H/C’+ 45’) }
	 * 
	 * 40ft(40’/ HC/ 45’)는 x2해서 TEU로 환산후, 아래 산식 시행 Full = If ( Full I/B vol >
	...
	public List<MBDResultCOABKGVO> searchLocationMBByCOABKGBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException {
		try {
			if(mBSearchOptionCOABKGVO.getTradeCd().equals("ALL")){
				return dbDao.searchLocationMBByCOABKGALLData(mBSearchOptionCOABKGVO);
			} else {		
				return dbDao.searchLocationMBByCOABKGData(mBSearchOptionCOABKGVO);
			}
							
		} catch(DAOException ex) {		
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchLocationMBByCOABKGBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchLocationMBByCOABKGBasic"}).getMessage(),ex);
		}		
	}	
	...
	public String searchMBTPeriodListBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException {
		List<MonthToWeekVO> monthToWeekVOS = null;		
		StringBuffer stringBuffer = new StringBuffer("");
		try {							
			monthToWeekVOS = dbDao.searchMBTPeriodListData(mBSearchOptionCOABKGVO);
				
			for (int i = 0; i < monthToWeekVOS.size(); i++) {
				stringBuffer.append(monthToWeekVOS.get(i).getDayUnit());
					
				if(i != (monthToWeekVOS.size() -1)){	
					stringBuffer.append(",");	
				}	
			}	
			...
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchMonthToWeekBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchMonthToWeekBasic"}).getMessage(),ex);
		}		
	}
	...
	public List<MBTResultCOABKGVO> searchLocationMBTByCOABKGBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException {
		try {									
			return dbDao.searchLocationMBTByCOABKGData(mBSearchOptionCOABKGVO);				
		} catch(DAOException ex) {				
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchLocationMBTByCOABKGBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchLocationMBTByCOABKGBasic"}).getMessage(),ex);
		}		
	}	
	...
	public List<ResultByLocationVO> searchLocationPOPBasic(SearchOptionByLocationVO searchOptionByLocationVO) throws EventException {
		...
		try {		
			int cnt = dbDao.checkLocationPOPData(searchOptionByLocationVO.getRqstUsrId(),"ALL");
				
			if(cnt > 0){	
				list = dbDao.searchLocationPOPData(searchOptionByLocationVO);
			} else {					
				dbDao.addLocationPOPData(searchOptionByLocationVO);
				list = dbDao.searchLocationFirstPOPData(searchOptionByLocationVO);
			}			
			
			return list;			
		} catch(DAOException ex) {						
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location POP] searchLocationPOPBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location POP] searchLocationPOPBasic"}).getMessage(),ex);
		}		
	}	
	...
	public void manageLocationPOPBasic(ResultByLocationVO[] resultByLocationVOS) throws EventException{
		try {     
			List<ResultByLocationVO> modifyVoList = new ArrayList<ResultByLocationVO>();
				
			for ( int i=0; i< resultByLocationVOS.length; i++ ) {      
				modifyVoList.add(resultByLocationVOS[i]);                 
			}				             
				
			if ( modifyVoList.size() > 0 ) {	
				dbDao.modifyLocationPOPData(modifyVoList);
			}     
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location POP] manageLocationPOPBasic"}).getMessage(),ex);
		} catch (Exception ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location POP] manageLocationPOPBasic"}).getMessage(),ex);
		}
	...
	public void clearLocationPOPBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException{
		try {				     
			dbDao.clearLocationPOPData(mBSearchOptionCOABKGVO);					
		} catch (DAOException ex) {	
			log.error("err " + ex.toString(), ex);	
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] clearLocationPOPBasic"}).getMessage(),ex);
		} catch (Exception ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] clearLocationPOPBasic"}).getMessage(),ex);
		}
	...
	public String checkLocationPOPBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException {
		try {											
			int count = dbDao.checkLocationPOPData(mBSearchOptionCOABKGVO.getUsrId(),"USE");
				
			return Integer.toString(count);
		} catch(DAOException ex) {			
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] checkLocationPOPBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] checkLocationPOPBasic"}).getMessage(),ex);
		}		
	}
	...
	public void manageFormatMBByCOABKGBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException{
		try {			
			//기타 옵션저장
			dbDao.clearFormatMBByCOABKGData(mBSearchOptionCOABKGVO);
			dbDao.addFormatMBByCOABKGData(mBSearchOptionCOABKGVO);	
			
			//FORMAT 로케이션 초기화 	
			String usrId = mBSearchOptionCOABKGVO.getUsrId();
			mBSearchOptionCOABKGVO.setUsrId(usrId + "SAVEFORMAT");
			dbDao.clearFormatLocationPOPData(mBSearchOptionCOABKGVO);
			
			//FORMAT 로케이션 정보 저장
			mBSearchOptionCOABKGVO.setUsrId(usrId);
			dbDao.copyLocationPOPData(mBSearchOptionCOABKGVO);				
		} catch (DAOException ex) {					
			log.error("err " + ex.toString(), ex);		
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] manageFormatMBByCOABKGBasic"}).getMessage(),ex);
		} catch (Exception ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] manageFormatMBByCOABKGBasic"}).getMessage(),ex);
		}
	...
	public SearchOptionByOTRVO searchFormatMBByCOABKGBasic(MBSearchOptionCOABKGVO mBSearchOptionCOABKGVO) throws EventException {
		...
		try {		
			//FORMAT Location이 저장되어 있는지 조회 
			int count = dbDao.checkLocationPOPData(mBSearchOptionCOABKGVO.getUsrId() + "SAVEFORMAT","ALL");
			
			if(count > 0){	
				//Location을 다시 카피 FORMAT => LOCATION		
				dbDao.clearFormatLocationPOPData(mBSearchOptionCOABKGVO);
				dbDao.copyFORMATLocationPOPData(mBSearchOptionCOABKGVO);
			}			
			
			//OTR 옵션 조회		
			searchOptionByOTRVO = dbDao.searchFormatMBByCOABKGData(mBSearchOptionCOABKGVO);
			return searchOptionByOTRVO;			
		} catch(DAOException ex) {							
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchFormatMBByCOABKGBasic"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"[Location M/B by COA BKG] searchFormatMBByCOABKGBasic"}).getMessage(),ex);
		}		
	}
	...
	public List<SearchCargoFlowMapDetailVO> searchCargoFlowMapDetail(SearchOptionByFromToVO searchOptionByFromToVO) throws EventException {
		try {									
			return dbDao.searchCargoFlowMapDetail(searchOptionByFromToVO);				
		} catch(DAOException ex) {				
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"Cargo Flow Map Detail)"}).getMessage(),ex);			
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("CIM00011", new String[]{"Cargo Flow Map Detail)"}).getMessage(),ex);
		}		
	}

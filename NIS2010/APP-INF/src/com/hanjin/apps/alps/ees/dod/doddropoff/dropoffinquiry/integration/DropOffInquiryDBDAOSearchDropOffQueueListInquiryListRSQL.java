...
*@FileName : DropOffInquiryDBDAOSearchDropOffQueueListInquiryListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class DropOffInquiryDBDAOSearchDropOffQueueListInquiryListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SearchDropOffQueueListInquiryList
	  * </pre>
	  */
	public DropOffInquiryDBDAOSearchDropOffQueueListInquiryListRSQL(){
		setQuery();
		...
		params.put("ofc_flg",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : DropOffInquiryDBDAOSearchDropOffQueueListInquiryListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	   WHEN @[ofc_flg] = 'R' AND @[ofc_cd] = 'HAMRU'" ).append("\n"); 
		query.append("	   THEN ROUND(SUM(NVL(MNR_COMMON_PKG.MNR_CAL_CURR_RATE_FNC(SUBSTR(REPLACE(X.TRO_IB_CFM_DT,'-',''),1,6),X.CURR_CD,'USD',CASE WHEN NVL(X.SPCL_TRF_AMT,0) <> 0 THEN X.SPCL_TRF_AMT ELSE NVL(X.GEN_TRF_AMT,0) END),0)) OVER (),2)" ).append("\n"); 
		query.append("	   ELSE ROUND(SUM(NVL(CASE WHEN NVL(X.SPCL_TRF_AMT,0) <> 0 THEN X.SPCL_TRF_AMT ELSE NVL(X.GEN_TRF_AMT,0) END,0)) OVER (),2)" ).append("\n"); 
		query.append("	   END STR_TTL_AMT," ).append("\n"); 
		query.append("	   CASE" ).append("\n"); 
		query.append("	   WHEN @[ofc_flg] = 'R' AND @[ofc_cd] = 'HAMRU'" ).append("\n"); 
		query.append("	   THEN 'USD'" ).append("\n"); 
		query.append("	   ELSE 'LCL' --X.CURR_CD --// CURR가 다른 경우가 있어서 LCL로만 표시하라는 말쌈" ).append("\n"); 
		query.append("	   END STR_TTL_CURR_CD," ).append("\n"); 
		...
		query.append("			   (SELECT RANK() OVER (PARTITION BY G.BKG_NO, G.CNTR_NO, G.DEL_CD ORDER BY G.UPD_DT DESC) RNK, G.*" ).append("\n"); 
		...
		query.append("                    START WITH OFC_CD = A.CFM_OFC_CD " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("       AND CASE WHEN  G.TRO_IB_CFM_OFC_CD(+) = 'GOASC' AND G.AR_IF_NO(+) IS NULL AND NVL(G.DC_RMK(+), 'N') <> 'Tariff Exemption' THEN" ).append("\n"); 
		query.append("             (SELECT '1' FROM DUAL WHERE EXISTS (SELECT 'OK'" ).append("\n"); 
		...
		query.append("                   AND A.CFM_OFC_CD = 'GOASC'" ).append("\n"); 
		query.append("                   AND G.AR_IF_NO IS NULL" ).append("\n"); 
		query.append("                   AND NVL(G.DC_RMK, 'N') <> 'Tariff Exemption' " ).append("\n"); 
		query.append("                   AND 'N' = G.TRO_IB_CXL_FLG" ).append("\n"); 
		query.append("                   AND 1 = G.RNK))" ).append("\n"); 
		query.append("          ELSE" ).append("\n"); 
		query.append("             (SELECT '1' FROM DUAL WHERE NOT EXISTS (SELECT 'OK'" ).append("\n"); 
		query.append("                  FROM DOD_DRP_OFF_CHG G" ).append("\n"); 
		query.append("                 WHERE A.BKG_NO = G.BKG_NO" ).append("\n"); 
		query.append("                   AND A.CNTR_NO = G.CNTR_NO" ).append("\n"); 
		query.append("                   AND B.DEL_CD = G.DEL_CD " ).append("\n"); 

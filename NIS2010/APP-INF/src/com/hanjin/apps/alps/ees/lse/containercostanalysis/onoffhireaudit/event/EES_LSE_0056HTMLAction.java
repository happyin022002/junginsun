...
*@FileName : EES_LSE_0056HTMLAction.java
...
public class EES_LSE_0056HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_LSE_0056HTMLAction 객체를 생성
	 */
	public EES_LSE_0056HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesLse0056Event event = new EesLse0056Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			String strRefNo = request.getParameter("ref_no");
			event.setRefNo(strRefNo);
			event.setOnOffHireAuditSearchVO((OnOffHireAuditSearchVO)getVO(request, OnOffHireAuditSearchVO.class));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setOnOffHireAuditSearchVOS((OnOffHireAuditSearchVO[])getVOs(request, OnOffHireAuditSearchVO.class,""));
		}

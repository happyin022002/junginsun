...
*@FileName : EqInterchangeDBDAOEqInterchangeContainerDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class EqInterchangeDBDAOEqInterchangeContainerDetailRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public EqInterchangeDBDAOEqInterchangeContainerDetailRSQL(){
		setQuery();
		...
		query.append("FileName : EqInterchangeDBDAOEqInterchangeContainerDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        , VNDR_SEQ, VNDR_ABBR_NM, CNTR_FULL_FLG" ).append("\n"); 
		query.append("        , RNTL_CHG_FREE_DYS" ).append("\n"); 
		query.append("        , TTL_USE_DYS" ).append("\n"); 
		query.append("        , PDM_AMT, LON_AMT, LOF_AMT, DOC_AMT" ).append("\n"); 
		query.append("        , NVL(PDM_AMT, 0) + LON_AMT + LOF_AMT + DOC_AMT AS TTL_AMT" ).append("\n"); 
		...
		query.append("               , TO_CHAR(E.CNTR_STS_EVNT_DT,'YYYYMMDD') 								AS CNTR_RTN_EVNT_DT" ).append("\n"); 
		...
		query.append("               , E.YD_CD 							AS RTN_YD_CD" ).append("\n"); 
		query.append("               , ROUND(NVL(E.CNTR_STS_EVNT_DT, SYSDATE) - B.CNTR_STS_EVNT_DT) AS TTL_USE_DYS" ).append("\n"); 
		query.append("               , NVL((SELECT (TRUNC(NVL(E.CNTR_STS_EVNT_DT,SYSDATE) - B.CNTR_STS_EVNT_DT  + NVL(B.RNTL_CHG_FREE_DYS , 0))) + 1 * RT.N1ST_CHG_AMT" ).append("\n"); 
		...
		query.append("                    (SELECT (TRUNC(NVL(E.CNTR_STS_EVNT_DT,SYSDATE) - B.CNTR_STS_EVNT_DT + NVL(B.RNTL_CHG_FREE_DYS , 0))) + 1 * AVG(RT.N1ST_CHG_AMT)" ).append("\n"); 
		query.append("                     FROM    LSE_AGMT_RT RT" ).append("\n"); 
		query.append("                     WHERE   RT.AGMT_CTY_CD 				= B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                     AND     RT.AGMT_SEQ 					= B.AGMT_SEQ" ).append("\n"); 
		query.append("                     AND     RT.CNTR_TPSZ_CD 				= A.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                     AND     RT.CNTR_RNTL_CHG_TP_CD 		= 'PDGV'" ).append("\n"); 
		query.append("                     AND     RT.AGMT_CHG_VAL 				= 1)) AS PDM_AMT," ).append("\n"); 
		query.append("                NVL(B.CNTR_LFT_CHG_AMT,0) AS LON_AMT," ).append("\n"); 
		query.append("                NVL(E.CNTR_LFT_CHG_AMT,0) AS LOF_AMT," ).append("\n"); 
		query.append("                NVL(E.CNTR_DRFF_CR_AMT,0) AS DOC_AMT        " ).append("\n"); 

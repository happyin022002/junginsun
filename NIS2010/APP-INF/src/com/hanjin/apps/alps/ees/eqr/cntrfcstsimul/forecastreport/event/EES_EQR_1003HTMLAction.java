...
*@FileName : EES_CIM_5010HTMLAction.java
...
public class EES_EQR_1003HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_CIM_5001HTMLAction 객체를 생성
	 */
	public EES_EQR_1003HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr1003Event event = new EesEqr1003Event();
		
		if(command.isCommand(FormCommand.SEARCH03)) { // 새로 만든거
			event.setForecastReportOptionVO((ForecastReportOptionVO)getVO(request, ForecastReportOptionVO .class));
		}
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setForecastReportOptionVO((ForecastReportOptionVO)getVO(request, ForecastReportOptionVO .class));
		}else if(command.isCommand(FormCommand.MULTI01)) { // 저장 등
			event.setForecastReportOptionVOS((ForecastReportOptionVO[])getVOs(request, ForecastReportOptionVO .class, ""));
		}else if(command.isCommand(FormCommand.SEARCH02)) { // loc 등 조회 / 유지
			event.setForecastReportOptionVO((ForecastReportOptionVO)getVO(request, ForecastReportOptionVO .class));
		} 

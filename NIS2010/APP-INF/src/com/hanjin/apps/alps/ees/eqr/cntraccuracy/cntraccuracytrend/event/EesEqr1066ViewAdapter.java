...
*@FileName 			: EesEqr1066ViewAdapter.java
 ...
 * @since 2013.06.10
 */
public class EesEqr1066ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		EesEqr1066ConditionVO conditionVO = (EesEqr1066ConditionVO) commonRsVO.getConditionVO();
		...
		try{
			while ( rs.next() ){
				...
				if(rs.getString("lcc_cd").equals("ZZZTTL")) { // G.TTL 에 대한 치환
					sbufXML.append("	<TD "+bgColor+" >TTL</TD>");
				}else {
					sbufXML.append("	<TD "+bgColor+" >" + getNull(rs.getString("lcc_cd"))      	+ "</TD>");
				}			
				
				sbufXML.append("	<TD "+bgColor+" >" + getNull(rs.getString("ecc_cd"))     	+ "</TD>");
				
				
				if(rs.getString("yd_cd").equals("XXXXXXX")) { // TTL 에 대한 치환
					sbufXML.append("	<TD "+bgColor+" >TTL</TD>");
				}else {
					sbufXML.append("	<TD "+bgColor+" >" + getNull(rs.getString("yd_cd"))      + "</TD>");
				}			
				
				sbufXML.append("	<TD "+bgColor+" ><![CDATA[" + getNull(rs.getString("week")) 	+ "]]></TD>");	
				
				if(rs.getString("slan_cd").equals("ZZZTTL")) { // TTL 에 대한 치환
					sbufXML.append("	<TD "+rowBgColor+" >TTL</TD>");
				}else {
					sbufXML.append("	<TD "+rowBgColor+" ><![CDATA[" + getNull(rs.getString("slan_cd"))    + "]]></TD>");
				}				
										
				if(rs.getString("vvd").equals("ZZZTTL")) { // TTL 에 대한 치환
					sbufXML.append("	<TD "+rowBgColor+" >TTL</TD>");
				}else {
				...
				if(sort_flag.equals("8") || sort_flag.equals("4") || sort_flag.equals("2") ){ // TTL 은 LF 를 빈칸으로 처리				

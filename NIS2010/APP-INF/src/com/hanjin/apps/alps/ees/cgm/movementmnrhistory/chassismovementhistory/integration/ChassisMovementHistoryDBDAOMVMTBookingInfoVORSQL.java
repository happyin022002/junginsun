...
public class ChassisMovementHistoryDBDAOMVMTBookingInfoVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("TO_CHAR(CTR.CNMV_EVNT_DT, 'YYYY-MM-DD HH24:MI:SS') CNMV_EVNT_DT," ).append("\n"); 
		...
		query.append("CTR.CNTR_RFUB_FLG CNTR_XCH_CD," ).append("\n"); 
		...
		query.append("FROM CTM_MOVEMENT CTR, MST_CONTAINER CNTR, MDM_VENDOR VEN, (SELECT CNTR_NO, CNMV_YR, CNMV_ID_NO, MAX(CLM_SEQ) SEQ FROM SCE_CLM WHERE CNTR_NO = @[p_cntrno] || @[check_digit] GROUP BY CNTR_NO, CNMV_YR, CNMV_ID_NO) CLM," ).append("\n"); 

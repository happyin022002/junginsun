...
public class EES_DMT_4009HTMLAction extends HTMLActionSupport {
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EesDmt4009Event event = new EesDmt4009Event();
        

        if(command.isCommand(FormCommand.SEARCH)) {
            event.setOtsInquiryParmVO((OtsInquiryParmVO)getVO(request, OtsInquiryParmVO .class));
        }else if(command.isCommand(FormCommand.SEARCH01)) {
        	event.setOtsInquiryParm2VO((OtsInquiryParm2VO)getVO(request, OtsInquiryParm2VO .class));
        }

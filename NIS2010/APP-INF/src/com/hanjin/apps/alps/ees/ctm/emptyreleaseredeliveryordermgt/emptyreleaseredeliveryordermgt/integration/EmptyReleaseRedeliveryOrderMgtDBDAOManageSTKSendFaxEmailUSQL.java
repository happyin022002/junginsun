...
public class EmptyReleaseRedeliveryOrderMgtDBDAOManageSTKSendFaxEmailUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("STK_EVNT_DT = TO_DATE (TO_CHAR (GLOBALDATE_PKG.TIME_CONV_FNC ('KRSEL', SYSDATE, GLOBALDATE_PKG.GET_LOCCD_FNC(@[user_ofc])), 'YYYY-MM-DD HH24:MI'), 'YYYY-MM-DD HH24:MI')," ).append("\n"); 
		query.append("CNTR_STK_JB_CD = 'MT'," ).append("\n"); 
		query.append("STK_JB_SEQ = DECODE (@[issue_type], 'P', 0, @[job_seq])," ).append("\n"); 
		query.append("UPD_LOCL_DT = GLOBALDATE_PKG.TIME_CONV_FNC ('KRSEL', SYSDATE, SUBSTR(@[empty_cy], 1, 5))," ).append("\n"); 

...
/**
 * ALPS CntrPlanGuidelineManageDBDAO <br>
 * - ALPS-CntrPlanGuide system Business Logic을 처리하기 위한 JDBC 작업수행.<br>
 * 
 * @author 	SHIN DONG IL
...
public class CntrPlanGuidelineManageDBDAO extends DBDAOSupport {
    ...
    */
	public CommonVO searchPolCdList(EesEqr1009ConditionVO conditionVO) throws DAOException {
		...
		String[] returnStr = new String[1];
//		String  str_pol_cd = "";
		StringBuffer str_pol_cd = new StringBuffer();
...
//				str_pol_cd = str_pol_cd +dbRowset.getString("POL_CD")+"|";
				str_pol_cd.append(dbRowset.getString("POL_CD")+"|");
			}					
			
			if(str_pol_cd.toString().equals("")){
				returnStr[0] ="";
			}else{
				returnStr[0]= str_pol_cd.toString().substring(0, str_pol_cd.toString().length()-1);
			}
			retVO.setResultStrArray(returnStr);
		} catch(SQLException se) {
			log.error(se.getMessage(),se);

...
*@FileName : EesWeekDatePeriodViewAdapter.java
*@FileTitle : EES_WEEKDATEPERIOD 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_WEEKDATEPERIOD 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesWeekDatePeriodViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String row 								= "";
		String fromdate_colname                 = "";
		String todate_colname                   = "";
		String fromdate 						= "";
		String todate 	    					= "";
		try{

			row    			= condiVO.getWeekperiodRow();				
			fromdate_colname= condiVO.getWeekperiodColname1();
			todate_colname	= condiVO.getWeekperiodColname2();
			fromdate		= retVO.getWeekPeriodFromdate();
			todate			= retVO.getWeekPeriodTodate();
			
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ fromdate_colname +"\" >"+JSPUtil.getNull(fromdate)+"</TD>");
			sb.append("	<TD COL=\""+ todate_colname +"\" >"+JSPUtil.getNull(todate)+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

...
*@FileName : InvoiceIssueCollectionMgtDBDAOSearchOTSCleanListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class InvoiceIssueCollectionMgtDBDAOSearchOTSCleanListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * InvoiceIssueCollectionMgtDBDAOSearchOTSCleanListRSQL
	  * </pre>
	  */
	public InvoiceIssueCollectionMgtDBDAOSearchOTSCleanListRSQL(){
		setQuery();
		...
		query.append("FileName : InvoiceIssueCollectionMgtDBDAOSearchOTSCleanListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("AND     PAYR.CUST_CNT_CD    = SUBSTR(PAYERC, 1, 2)" ).append("\n"); 
		query.append("AND     PAYR.CUST_SEQ       = SUBSTR(PAYERC, 3, 6)" ).append("\n"); 
		query.append("AND     PAYR.SYS_AREA_GRP_ID= CS.SYS_AREA_GRP_ID" ).append("\n"); 
		query.append("AND     CS.CO_IND_CD        = 'H'" ).append("\n"); 
		query.append("AND     CS.CNT_CD           = SUBSTR(PAYERC, 1, 2))) AS DMDT_PAYR_OTS_RMK" ).append("\n"); 
		query.append(", MAX(( SELECT" ).append("\n"); 
		query.append("REPLACE(" ).append("\n"); 
		query.append("NVL(PAYR.OTS_RMK        ,' ')" ).append("\n"); 
		query.append(",'@*',CHR(10)" ).append("\n"); 
		query.append(") AS OTS_RMK" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("DMT_PAYR_INFO PAYR" ).append("\n"); 
		query.append(",COM_SYS_AREA_GRP_ID    CS" ).append("\n"); 
		query.append("WHERE   1=1" ).append("\n"); 
		...
		query.append("AND     PAYR.SYS_AREA_GRP_ID= CS.SYS_AREA_GRP_ID" ).append("\n"); 
		query.append("AND     CS.CO_IND_CD        = 'H'" ).append("\n"); 
		query.append("AND     CS.CNT_CD           = SUBSTR(PAYERC, 1, 2))) AS OTS_RMK" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("FROM    (" ).append("\n"); 
		query.append("SELECT  DECODE( M.ACT_PAYR_CNT_CD , '00' , '' , M.ACT_PAYR_CNT_CD )||TO_CHAR( M.ACT_PAYR_SEQ , 'FM000000' )	PAYERC," ).append("\n"); 
		query.append("REPLACE( NVL( U.CUST_LGL_ENG_NM , V.VNDR_LGL_ENG_NM ) , '/' , '_' )									PAYERN,         /* PAYER NAME - CUSTOMER NAME OR VENDOR NAME */" ).append("\n"); 
		query.append("CUST.CUST_CNT_CD||TO_CHAR( CUST.CUST_SEQ , 'FM000000' ) AS SH_CD," ).append("\n"); 
		query.append("NVL(( SELECT CUST_LGL_ENG_NM FROM MDM_CUSTOMER WHERE CUST_CNT_CD = CUST.CUST_CNT_CD AND CUST_SEQ = CUST.CUST_SEQ ),'') AS SH_NM," ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("M.DMDT_TRF_CD AS TARFTP, M.DMDT_AR_IF_CD," ).append("\n"); 
		query.append("NVL((" ).append("\n"); 
		...
		query.append("AND    M.BKG_NO = CUST.BKG_NO(+)" ).append("\n"); 

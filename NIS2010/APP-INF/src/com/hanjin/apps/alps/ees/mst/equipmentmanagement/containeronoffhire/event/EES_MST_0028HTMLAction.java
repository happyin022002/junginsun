...
public class EES_MST_0028HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_MST_0028HTMLAction 객체를 생성
	 */
	public EES_MST_0028HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesMst0028Event event = new EesMst0028Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMstEtcVO((MstEtcVO)getVO(request, MstEtcVO .class));
		} else if (command.isCommand(FormCommand.MULTI)){
			event.setStatusHistoryVOs((StatusHistoryVO[])getVOs(request, StatusHistoryVO .class,""));
		} else if (command.isCommand(FormCommand.MULTI01)){
			event.setStatusHistoryVO((StatusHistoryVO)getVO(request, StatusHistoryVO .class,""));
		}

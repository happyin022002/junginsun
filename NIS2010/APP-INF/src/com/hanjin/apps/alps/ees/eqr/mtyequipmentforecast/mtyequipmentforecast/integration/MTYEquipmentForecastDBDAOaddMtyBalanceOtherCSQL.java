...
public class MTYEquipmentForecastDBDAOaddMtyBalanceOtherCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("INSERT INTO EQR_MTY_BAL_RPT_OTR(" ).append("\n"); 
		...
		query.append("    , @[fcast_yrwk]" ).append("\n"); 
		query.append("    , @[mty_bal_otr_tp_cd]" ).append("\n"); 
		query.append("    , (SELECT NVL(MAX(CRE_SEQ),0)+1 FROM EQR_MTY_BAL_RPT_OTR WHERE  CO_CD='H' AND LOC_CD=@[loc_cd] AND INP_YRWK=@[inp_yrwk] AND FCAST_YRWK=@[fcast_yrwk] AND MTY_BAL_OTR_TP_CD=@[mty_bal_otr_tp_cd] AND LOC_GRP_CD=@[loc_grp_cd] )" ).append("\n"); 
		query.append("    , @[fctr_ctnt]" ).append("\n"); 
		query.append("    , @[lstm_cd]" ).append("\n"); 
		query.append("    , @[yd_cd]" ).append("\n"); 

...
public class ReceivableRentalCostDBDAOReceivableRentalChargeDetailListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("FROM    PARAM P, TEMP_DROP02 T2," ).append("\n"); 
		...
		query.append("AND     Z.CNTR_TPSZ_CD = T2.CNTR_TPSZ_CD" ).append("\n"); 
		...
		query.append("COST_AMT, PKUP_CHG_FLG, PKUP_CHG_AMT, LON_CHG_FLG, LOF_CHG_FLG, LFT_CHG_AMT" ).append("\n"); 
		...
		query.append("B.LSE_RCV_CHG_TP_CD, B.CHG_RT_AMT, B.AGMT_CHG_VAL," ).append("\n"); 
		...
		query.append("CASE WHEN TO_CHAR(A.CNTR_STS_EVNT_DT,'YYYYMM') = SUBSTR(B.QTY_YRMON, 1, 6)" ).append("\n"); 
		...
		query.append("CASE WHEN TO_CHAR(A.CNTR_STS_EVNT_DT + NVL(A.RNTL_CHG_FREE_DYS,0) - 1,'YYYYMM') = SUBSTR(B.QTY_YRMON,1,6)" ).append("\n"); 
		...
		query.append("CASE WHEN TO_CHAR(A.CNTR_STS_EVNT_DT + B.CL_DPP_FRDAY,'YYYYMM') = SUBSTR(B.QTY_YRMON,1,6)" ).append("\n"); 
		...
		query.append("AND     A.CNTR_STS_SEQ = B.PARTN_SEQ" ).append("\n"); 
		...
		query.append("T1.CHG_RT_AMT AS CHG_RT_AMT," ).append("\n"); 

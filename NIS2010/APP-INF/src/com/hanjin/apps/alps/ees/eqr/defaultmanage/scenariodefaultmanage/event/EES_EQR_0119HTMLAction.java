...
*@FileName : EES_EQR_119HTMLAction.java
...
public class EES_EQR_0119HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_119HTMLAction 객체를 생성
	 */
	public EES_EQR_0119HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EqrShrtTermOffhCondVO eqrShrtTermOffhCondVO = new EqrShrtTermOffhCondVO();
    	EqrShrtTermOffhCondVO[] eqrShrtTermOffhCondVOs = null;
		
    	// 검색조건 4개미만 - Event Setting..
    	...
    	EesEqr0119Event event = new EesEqr0119Event(status, location, tpsztype);
   	
    	// Retrieve 버튼 클릭시
		if(command.isCommand(FormCommand.MULTI)) {
			// Cntr Tpsz 형태에 맞게 Vo에 Setting해주기 위해서 Table VO에 새로만든 함수 호출해서 Vo Settting
			eqrShrtTermOffhCondVOs = eqrShrtTermOffhCondVO.fromRequestGridArrayList(request, tpsztype);
			event.setEqrShrtTermOffhCondVOS(eqrShrtTermOffhCondVOs);
		}

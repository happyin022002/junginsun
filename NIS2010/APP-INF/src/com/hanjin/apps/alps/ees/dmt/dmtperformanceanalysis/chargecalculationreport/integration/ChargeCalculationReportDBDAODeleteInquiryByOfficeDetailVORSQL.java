...
public class ChargeCalculationReportDBDAODeleteInquiryByOfficeDetailVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    ,decode(C.DMDT_CHG_DELT_RSN_CD, 'OTH', ' ', CTNT.ATTR_CTNT4) as DELT_SPEC_RSN_DESC" ).append("\n"); 
		query.append("    ,ATTR_CTNT4 AS" ).append("\n"); 
		...
		query.append("     /* Soma 1.1.9 버전에서 쌍따옴표 처리가 안됨 JAVA에서  처리를 해줘야함*/" ).append("\n"); 
		...
		query.append("AND	    C.FM_MVMT_DT BETWEEN TO_DATE(@[fm_dt],'YYYYMMDD') AND TO_DATE(@[to_dt],'YYYYMMDD') + .99999" ).append("\n"); 
		...
		query.append("and  CTNT.ATTR_CTNT1(+)      = C.DMDT_CHG_DELT_RSN_CD" ).append("\n"); 
		query.append("and  CTNT.ATTR_CTNT3(+)      = C.DMDT_CHG_DELT_SPEC_RSN_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${grp_flg} == 'R')" ).append("\n"); 
		query.append("	AND (C.OFC_RHQ_CD,C.DMDT_CHG_DELT_RSN_CD,NVL(C.DMDT_CHG_DELT_SPEC_RSN_CD,' ')) " ).append("\n"); 
		...
		query.append("	AND (C.OFC_CD,C.DMDT_CHG_DELT_RSN_CD,NVL(C.DMDT_CHG_DELT_SPEC_RSN_CD,' ')) " ).append("\n"); 
		query.append("                                        IN ( SELECT BKG_GET_TOKEN_FNC(COLUMN_VALUE,1)" ).append("\n"); 
		query.append("                                                  , BKG_GET_TOKEN_FNC(COLUMN_VALUE,2)" ).append("\n"); 
		query.append("                                                  , NVL(BKG_GET_TOKEN_FNC(COLUMN_VALUE,3),' ')" ).append("\n"); 
		query.append("                                            FROM TABLE(BKG_SPLIT_CLOB_FNC(${ofc_cd_delt_rsn_cd_list},'@'))" ).append("\n"); 

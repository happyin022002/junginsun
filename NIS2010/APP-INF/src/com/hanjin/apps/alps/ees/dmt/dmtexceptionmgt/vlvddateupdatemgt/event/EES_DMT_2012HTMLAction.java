...
*@FileName : UI_DMT_2012HTMLAction.java
...
public class EES_DMT_2012HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * UI_DMT_2012HTMLAction 객체를 생성
	 */
	public EES_DMT_2012HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesDmt2012Event event = new EesDmt2012Event();

		if ( command.isCommand(FormCommand.SEARCH) ) {
		    
			event.setDmtVesselDateUpdateParmVO((DmtVesselDateUpdateParmVO)getVO(request, DmtVesselDateUpdateParmVO .class));
			
		} else if ( command.isCommand(FormCommand.MULTI) ) {
		    
            String xMvmt = request.getParameter( "mvmt" );
            String xPort = request.getParameter( "port" );
		    event.setAttribute( "xMvmt" , xMvmt );
            event.setAttribute( "xPort" , xPort );
		    event.setDmtVslDtUpdVOs((DmtVslDtUpdVO[])getVOs(request,DmtVslDtUpdVO.class,""));

...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.alps.ees.LSE.mnrcommon 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @see LseFileUploadEvent 참조
 * @since J2EE 1.4
 */

public class LSE_FILEUPLOAD_HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * LSE_COM_HTMLAction 객체를 생성
	 */  
	public LSE_FILEUPLOAD_HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		LseFileUploadEvent event = new LseFileUploadEvent();		
		
		//FILE UPLOAD하기
		try {
			log.debug("+++++++++++++++" + command.getCommand() + "+++++++++++++++");
			if ( command.isCommand(FormCommand.DEFAULT) ) {
				FileUpload fileUpload = new FileUpload(request, SubSystemConfigFactory.get("LSE.MODULE.ID"), SubSystemConfigFactory.get("LSE.ENCODING.EUCKR"), event);
				HttpServletRequest multipartRequest = fileUpload.getRequest();
				event.setAttribute("KEYS", (List<String>)multipartRequest.getAttribute(SubSystemConfigFactory.get("LSE.FILE.UPLOAD.KEYS")));
			}

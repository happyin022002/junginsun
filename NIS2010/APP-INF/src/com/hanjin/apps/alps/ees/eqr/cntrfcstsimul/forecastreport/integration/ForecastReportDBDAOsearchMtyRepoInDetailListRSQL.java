...
*@FileName : ForecastReportDBDAOsearchMtyRepoInDetailListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ForecastReportDBDAOsearchMtyRepoInDetailListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ForecastReportDBDAOsearchMtyRepoInDetailListRSQL(){
		setQuery();
		...
		query.append("FileName : ForecastReportDBDAOsearchMtyRepoInDetailListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("-- EES_EQR_1048" ).append("\n"); 
		query.append("-- 현재주에서는 하단에 출현" ).append("\n"); 
		query.append("-- 미래주에서는 혼자 출현" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("--Path : com.hanjin.apps.alps.ees.eqr.cntrfcstsimul.forecastreport.integration" ).append("\n"); 
		query.append("--FileName : ForecastReportDBDAOsearchMtyRepoInDetailListRSQL" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("WITH REPO_IN_RESULT AS (      " ).append("\n"); 
		query.append("    -- REPO IN DETAIL(EES_EQR_1048에서 수기입력)" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("       " ).append("\n"); 
		query.append("   -- PLUS OTHER 추가 (S) ------------------" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	UNION ALL    " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("          SELECT '0' CRE_SEQ -- 삭제불가능" ).append("\n"); 
		query.append("          ,A.LOC_CD YARD" ).append("\n"); 
		query.append("--        ,A.FCAST_YRWK WEEK" ).append("\n"); 
		query.append("          ,NULL LANE" ).append("\n"); 
		query.append("          ,NULL VVD" ).append("\n"); 
		query.append("          ,NULL ETB" ).append("\n"); 
		query.append("          ,NULL ETB_DAY" ).append("\n"); 
		query.append("          ,NULL POD_CD" ).append("\n"); 
		query.append("          ,NULL DIV" ).append("\n"); 
		query.append("          ,NULL RPT_SEQ" ).append("\n"); 
		...
		query.append("                    WHERE A.INP_YRWK          = ( SELECT /*+ INDEX_DESC(A XPKEQR_WK_PRD) */ PLN_YR||PLN_WK" ).append("\n"); 
		query.append("                                                    FROM EQR_WK_PRD A" ).append("\n"); 
		query.append("                                                   WHERE PLN_YR||PLN_WK < @[fcast_yrwk] -- MAIN 화면과 동일한 로직 사용" ).append("\n"); 
		query.append("                                                     AND ROWNUM = 1 )  -- +other는 -1WEEK가 곧 현재주차로 인식된다   -- '201328'" ).append("\n"); 
		query.append("                    AND   A.FCAST_YRWK  = @[fcast_yrwk]                                                -- '201329'                    " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("   -- PLUS OTHER 추가 (E) ------------------" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    UNION ALL    " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${curr_flag} =='T')  -- 현재주차" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    SELECT '0' CRE_SEQ -- 삭제불가능" ).append("\n"); 
		query.append("          ,B.YD_CD YARD" ).append("\n"); 
		...
		query.append("              ,NVL(SUM(DECODE(A.CNTR_TPSZ_CD, 'A5', A.CNTR_QTY)),0) A5_QTY" ).append("\n"); 
		...
		query.append("        WHERE A.ETD_DT BETWEEN TO_DATE(@[wk_st_dt], 'YYYYMMDD') " ).append("\n"); 
		query.append("                       AND     TO_DATE((SELECT TO_CHAR(NEXT_DAY(SYSDATE, 2)-7, 'YYYYMMDD') FROM DUAL), 'YYYYMMDD')+0.99999  -- LCC별 시작일부터 이번주 월요일까지                        " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#elseif(${curr_flag} =='F')  -- 미래주차" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("         SELECT '0' CRE_SEQ -- 삭제불가능" ).append("\n"); 
		...
		query.append("      ,@[curr_flag] CURR_FLAG -- HIDDEN     " ).append("\n"); 
		query.append("      ,DIV        -- HIDDEN     " ).append("\n"); 
		query.append("      ,RPT_SEQ    -- HIDDEN     " ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(" ).append("\n"); 

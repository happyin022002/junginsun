...
public class OverdayNStatusParmVO extends AbstractValueObject {
	 ...
	 * toYard 정보 필요로 인한 toYrdCd Attr 추가
	 * */
	private String toYrdCd = null;
	...
	public HashMap<String, String> getColumnValues(){
		...
		this.hashColumns.put("to_yrd_cd", getToYrdCd());
	...
	public HashMap<String, String> getFieldNames(){
		...
		this.hashFields.put("to_yrd_cd", "toYrdCd");
	...
	public String getToYrdCd() {
		return toYrdCd;
	}

	public void setToYrdCd(String toYrdCd) {
		// Yard 코드 뒤에 PT 글자가 붙기때문에 이를 제거한다.
		if( toYrdCd.length() > 5)
			this.toYrdCd = toYrdCd.substring(0, 5);
		else
			this.toYrdCd = toYrdCd;
	}
...
/**
	...
	public void fromRequest(HttpServletRequest request, String prefix) {
		...
		setEndHr(JSPUtil.getParameter(request, prefix + "end_hr", ""));
		setToYrdCd(JSPUtil.getParameter(request, prefix + "to_yrd_cd", ""));
	}
	...
	public OverdayNStatusParmVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		...
		try {
			...
			String[] toYrdCd = (JSPUtil.getParameter(request, prefix	+ "to_yrd_cd", length));
	...
	*/
	public void unDataFormat(){
		...
		this.toYrdCd = this.toYrdCd.replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
	}
}

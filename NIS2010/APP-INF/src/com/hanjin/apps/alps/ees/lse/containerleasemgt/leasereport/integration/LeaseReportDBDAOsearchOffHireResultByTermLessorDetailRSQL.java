...
*@FileName : LeaseReportDBDAOsearchOffHireResultByTermLessorDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class LeaseReportDBDAOsearchOffHireResultByTermLessorDetailRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 임차장비 임차실적을 상세 조히 Off-Hire Result by Lease Term/Lessor-Option
	  * 2010.09.13 남궁진호 [CHM-201005773-01] USED DAYS계산시 FREE DYAS 제외,RENTAL CHARGE계산로직 수정
	  * 2010.12.13 이준범 [CHM-201007442-01] LT일때 Per-Diem LCC로 변경
	  * </pre>
	  */
	public LeaseReportDBDAOsearchOffHireResultByTermLessorDetailRSQL(){
		setQuery();
		...
		query.append("FileName : LeaseReportDBDAOsearchOffHireResultByTermLessorDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       , CASE WHEN ( D.LSTM_CD = 'OW' OR D.LSTM_CD = 'LP' OR D.LSTM_CD = 'OL' ) THEN TO_CHAR(B.ONH_DT, 'YYYYMMDD') ELSE TO_CHAR(E.CNTR_STS_EVNT_DT, 'YYYYMMDD') END OHDATE " ).append("\n"); 
		query.append("       , CASE WHEN ( D.LSTM_CD = 'OW' OR D.LSTM_CD = 'LP' OR D.LSTM_CD = 'OL' ) THEN F.YD_CD ELSE E.YD_CD END OHLOC " ).append("\n"); 
		...
		query.append("            NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) -  E.CNTR_STS_EVNT_DT  + 1" ).append("\n"); 
		...
		query.append("                   (SELECT  ( NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) -  B.MFT_DT - NVL(E.RNTL_CHG_FREE_DYS , 0 ) + 1 ) * N1ST_CHG_AMT" ).append("\n"); 
		...
		query.append("                    AND     RT.LOC_CD             (+) = DECODE(D.LSTM_CD, 'LT', A.LCC_CD, 'KRSEL')" ).append("\n"); 
		...
		query.append("                   (SELECT  ( NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) - B.MFT_DT - NVL(E.RNTL_CHG_FREE_DYS , 0 ) + 1 ) * AVG(N1ST_CHG_AMT)" ).append("\n"); 
		query.append("                    FROM    LSE_AGMT_RT RT" ).append("\n"); 
		query.append("                    WHERE   RT.AGMT_CTY_CD        (+) = A.AGMT_CTY_CD" ).append("\n"); 
		query.append("                    AND     RT.AGMT_SEQ           (+) = A.AGMT_SEQ" ).append("\n"); 
		query.append("                    AND     RT.AGMT_CHG_VAL       (+) = 1" ).append("\n"); 
		...
		query.append("                   (SELECT  ( NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) - E.CNTR_STS_EVNT_DT - NVL(E.RNTL_CHG_FREE_DYS , 0 ) + 1 ) * N1ST_CHG_AMT" ).append("\n"); 
		query.append("                    FROM    LSE_AGMT_RT RT" ).append("\n"); 
		query.append("                    WHERE   RT.AGMT_CTY_CD        (+) = A.AGMT_CTY_CD" ).append("\n"); 
		query.append("                    AND     RT.AGMT_SEQ           (+) = A.AGMT_SEQ" ).append("\n"); 
		query.append("                    AND     RT.LOC_CD             (+) = DECODE(D.LSTM_CD, 'LT', A.LCC_CD, 'KRSEL')" ).append("\n"); 
		...
		query.append("                    AND     RT.CNTR_TPSZ_CD       (+) = B.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                    AND     RT.CNTR_RNTL_CHG_TP_CD(+) = 'PDGV')," ).append("\n"); 
		query.append("                   (SELECT  ( NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) - E.CNTR_STS_EVNT_DT - NVL(E.RNTL_CHG_FREE_DYS , 0 ) + 1 ) * AVG(N1ST_CHG_AMT)" ).append("\n"); 
		query.append("                    FROM    LSE_AGMT_RT RT" ).append("\n"); 
		query.append("                    WHERE   RT.AGMT_CTY_CD        (+) = A.AGMT_CTY_CD" ).append("\n"); 
		query.append("                    AND     RT.AGMT_SEQ           (+) = A.AGMT_SEQ" ).append("\n"); 
		query.append("                    AND     RT.AGMT_CHG_VAL       (+) = 1" ).append("\n"); 
		...
		query.append("          AND     RT.CNTR_TPSZ_CD        = B.CNTR_TPSZ_CD " ).append("\n"); 
		query.append("          AND     TO_CHAR(E.CNTR_STS_EVNT_DT, 'YYYYMMDD') BETWEEN @[period_stdt] AND @[period_eddt] ) AS DPP" ).append("\n"); 

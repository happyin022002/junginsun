...
public class InvoiceIssueCollectionMgtDBDAOSearchConfirmChargeByContainerRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	, ( SELECT TAA_NO FROM BKG_BOOKING WHERE BKG_NO = M.BKG_NO ) TAA_NO" ).append("\n"); 
		...
		query.append("		 , (" ).append("\n"); 
		query.append("                  CASE WHEN (B.DMDT_TRF_CD IN ('DTIC', 'CTIC') AND SUBSTR(A.DEL_CD,1,2) = 'US' AND A.BKG_DE_TERM_CD = 'Y')" ).append("\n"); 
		query.append("                    OR (B.DMDT_TRF_CD IN ('DTOC', 'CTOC') AND SUBSTR(A.POR_CD,1,2) = 'US' AND A.BKG_RCV_TERM_CD = 'Y')" ).append("\n"); 
		query.append("                    	  THEN ( SELECT	LPAD(VNDR_SEQ,6,'0') AS VNDR_CD" ).append("\n"); 
		query.append("                    	  			FROM	MDM_VENDOR" ).append("\n"); 
		query.append("                    	  			WHERE	VNDR_SEQ = B.VNDR_SEQ" ).append("\n"); 
		query.append("                    	  			AND	DELT_FLG <> 'Y' )" ).append("\n"); 
		query.append("                       ELSE " ).append("\n"); 
		query.append("                              CASE WHEN SUBSTR(B.DMDT_TRF_CD,3,1) = 'I' AND SUBSTR(A.DEL_CD,1,2) IN ('US', 'CA') THEN" ).append("\n"); 
		...
		query.append("                                   WHEN SUBSTR(B.DMDT_TRF_CD,3,1) = 'O' AND SUBSTR(A.POR_CD,1,2) IN ('US', 'CA') THEN " ).append("\n"); 
		query.append("                                                             ( SELECT CUST_CNT_CD||LPAD(CUST_SEQ,6,'0') " ).append("\n"); 
		query.append("                                                                 FROM BKG_CUSTOMER " ).append("\n"); 
		query.append("                                                                WHERE BKG_NO = A.BKG_NO " ).append("\n"); 
		...
		query.append("													WHERE BKG_NO = A.BKG_NO" ).append("\n"); 
		query.append("													AND IO_BND_CD = SUBSTR(B.DMDT_TRF_CD,3,1)" ).append("\n"); 
		query.append("													AND AR_IF_NO = (SELECT MAX(AR_IF_NO)" ).append("\n"); 
		query.append("																			FROM INV_AR_MN" ).append("\n"); 
		query.append("																			WHERE BKG_NO = A.BKG_NO" ).append("\n"); 
		query.append("																			AND   IO_BND_CD = SUBSTR(B.DMDT_TRF_CD,3,1)" ).append("\n"); 
		query.append("																	)" ).append("\n"); 
		query.append("												), NVL(DECODE(B.ACT_CNT_CD, '00', '', B.ACT_CNT_CD)|| DECODE(B.ACT_CUST_SEQ,NULL,'',0,'',LPAD(B.ACT_CUST_SEQ, 6, '0')), '')" ).append("\n"); 
		query.append("											)" ).append("\n"); 
		...
		query.append("                  CASE WHEN (B.DMDT_TRF_CD IN ('DTIC', 'CTIC') AND SUBSTR(A.DEL_CD,1,2) = 'US' AND A.BKG_DE_TERM_CD = 'Y')" ).append("\n"); 
		query.append("                    OR (B.DMDT_TRF_CD IN ('DTOC', 'CTOC') AND SUBSTR(A.POR_CD,1,2) = 'US' AND A.BKG_RCV_TERM_CD = 'Y') " ).append("\n"); 
		query.append("                    		THEN ( SELECT	VNDR_LGL_ENG_NM FROM MDM_VENDOR WHERE VNDR_SEQ = B.VNDR_SEQ AND DELT_FLG <> 'Y' AND ROWNUM = 1)" ).append("\n"); 
		query.append("                       ELSE " ).append("\n"); 
		query.append("                              CASE WHEN SUBSTR(B.DMDT_TRF_CD,3,1) = 'I' AND SUBSTR(A.DEL_CD,1,2) IN ('US', 'CA') THEN" ).append("\n"); 
		query.append("                                                         NVL(( SELECT CUST_LGL_ENG_NM" ).append("\n"); 
		query.append("                                                                 FROM BKG_CUSTOMER A, MDM_CUSTOMER B" ).append("\n"); 
		query.append("                                                                WHERE A.BKG_NO = A.BKG_NO " ).append("\n"); 
		...
		query.append("                                   WHEN SUBSTR(B.DMDT_TRF_CD,3,1) = 'O' AND SUBSTR(A.POR_CD,1,2) IN ('US', 'CA') THEN " ).append("\n"); 
		query.append("                                                             ( SELECT CUST_LGL_ENG_NM" ).append("\n"); 
		query.append("                                                                 FROM BKG_CUSTOMER C, MDM_CUSTOMER B" ).append("\n"); 
		...
		query.append("		, (SELECT SC_NO FROM BKG_BOOKING WHERE BKG_NO = A.BKG_NO )  AS SC_NO" ).append("\n"); 
		query.append("		, (SELECT RFA_NO FROM BKG_BOOKING WHERE BKG_NO = A.BKG_NO ) AS RFA_NO" ).append("\n"); 
		query.append("		, (SELECT TAA_NO FROM BKG_BOOKING WHERE BKG_NO = A.BKG_NO )	AS TAA_NO" ).append("\n"); 

...
public class ContainerOnOffhireDBDAOSearchLeaseOutTargetListOfLSODataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(") AS CNTR_DRFF_AMT" ).append("\n"); 
		query.append(", (" ).append("\n"); 
		...
		query.append("WHERE   LOC_CD = SUBSTR(@[sts_evnt_yd_cd], 1, 5))" ).append("\n"); 
		query.append("AND     CNTR_TPSZ_CD = B.CNTR_TPSZ_CD" ).append("\n"); 
		query.append(") AS CNTR_LFT_CHG_AMT" ).append("\n"); 
		query.append(", (" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("AND   A.OFFH_DUE_DT >= REPLACE(@[hire_date],'-','')" ).append("\n"); 
		query.append("AND   A.OFFH_DUE_DT >= TO_CHAR(GLOBALDATE_PKG.TIME_LOCAL_FNC(SUBSTR(B.CRNT_YD_CD,1,5)),'YYYYMMDD')" ).append("\n"); 
		...
		query.append("-- EN 일경우 Empty 상태에서 반납 가능토록 로직 변경. 2012-01-12" ).append("\n"); 

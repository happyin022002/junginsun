...
public class AgentCanalTransitFeeBCDBDAOsearchCanalTzInvAllowanceTEURSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("DECODE(A.SUZ_NET_TONG_WGT,NULL,(" ).append("\n"); 
		query.append("SELECT  B.SUZ_NET_TONG_WGT" ).append("\n"); 
		query.append("FROM    PSO_CNL_TZ_FEE B" ).append("\n"); 
		query.append("WHERE 	1=1" ).append("\n"); 
		query.append("AND     B.PSO_BZTP_CD = 5" ).append("\n"); 
		query.append("AND 	B.VSL_CD = regexp_substr(@[vvd], '[[:alpha:]]+', 1, 1)" ).append("\n"); 
		query.append("AND 	B.SKD_VOY_NO = regexp_substr(@[vvd], '[(0-9)]+', 1, 1)  --'0142'" ).append("\n"); 
		query.append("AND 	B.SKD_DIR_CD = regexp_substr(@[vvd], '[[:alpha:]]+', 1, 2)  --'E'" ).append("\n"); 
		query.append("AND     B.YD_CD = @[yd_cd]  --'EGSUZT1'" ).append("\n"); 
		query.append("AND     B.CALL_SEQ =  (" ).append("\n"); 
		...
		query.append("AND SKD_VOY_NO = regexp_substr(@[vvd], '[(0-9)]+', 1, 1)" ).append("\n"); 
		query.append("AND SKD_DIR_CD = regexp_substr(@[vvd], '[[:alpha:]]+', 1, 2)" ).append("\n"); 
		query.append("AND PSO_BZTP_CD = 5" ).append("\n"); 
		query.append("AND CNL_TZ_BZTP_CD= 'E'" ).append("\n"); 
		query.append("AND NTC_YRMON = REPLACE(@[rev_yrmon], '-', '')" ).append("\n"); 
		query.append("AND BUD_SCNR_NO = '999912' )" ).append("\n"); 
		query.append("AND 	B.VNDR_SEQ = @[vndr_seq]  --100870" ).append("\n"); 
		query.append("AND 	B.CNL_TZ_BZTP_CD = 'E'" ).append("\n"); 
		query.append("AND 	B.BUD_SCNR_NO 	 = '999912'),A.SUZ_NET_TONG_WGT) SUZ_NET_TONG_WGT,  --SCNT" ).append("\n"); 

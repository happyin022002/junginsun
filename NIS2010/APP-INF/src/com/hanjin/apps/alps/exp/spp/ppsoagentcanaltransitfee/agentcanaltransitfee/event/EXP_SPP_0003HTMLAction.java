...
*@FileName : EXP_SPP_0003HTMLAction.java
...
public class EXP_SPP_0003HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EXP_SPP_0003HTMLAction 객체를 생성
	 */
	public EXP_SPP_0003HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	ExpSpp0003Event event = new ExpSpp0003Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {	//windowsopen	
			...
			event.setPsoCnlTzAtchFileVOS((PsoCnlTzAtchFileVO[])getVOs(request, PsoCnlTzAtchFileVO .class, "hidden_sheets_file_"));
		}
		else if(command.isCommand(FormCommand.COMMAND01)) {//request button click
			event.setPsoCnlTzFeeVO((PsoCnlTzFeeVO)getVO(request, PsoCnlTzFeeVO .class));			
		}		
		else if(command.isCommand(FormCommand.SEARCH01)) {	//windowsopen	
			event.setCanalTzFeeEstDtlByVvdCondVO((CanalTzFeeEstDtlByVvdCondVO)getVO(request, CanalTzFeeEstDtlByVvdCondVO .class));		
		}

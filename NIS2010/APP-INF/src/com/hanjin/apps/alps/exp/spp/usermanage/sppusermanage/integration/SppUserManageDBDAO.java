...
public class SppUserManageDBDAO extends DBDAOSupport {
	...
	/**
	 ...
	 public String addMnrPartnerSeqData() throws DAOException{
		...
		/**
		 * Bid user Password 정보를 수정합니다.<br>
		 * 
		 * @param mnrPartnerVO   MnrPartnerVO
		 * @exception DAOException
		 */
		public void modifySppUserBidPwdInfo(MnrPartnerVO mnrPartnerVO) throws DAOException,Exception {
			...
			try {
				...
				SQLExecuter sqlExe = new SQLExecuter("");
				int result = sqlExe.executeUpdate((ISQLTemplate)new SppUserManageDBDAOMnrPartnerPwdUSQL(), param, param);
				if(result == Statement.EXECUTE_FAILED) {
					throw new DAOException("Fail to indert SQL");
				}
			} catch (SQLException se) {

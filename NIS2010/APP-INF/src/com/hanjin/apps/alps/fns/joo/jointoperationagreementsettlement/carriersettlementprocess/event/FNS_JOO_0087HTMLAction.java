...
public class FNS_JOO_0087HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setIntloadSumReportVO((IntloadSumReportVO)getVO(request, IntloadSumReportVO .class));
		}
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setIntloadSumReportVO((IntloadSumReportVO)getVO(request, IntloadSumReportVO .class));
		}
        if(command.isCommand(FormCommand.MULTI)) {
            event.setIntloadSumReportVOs((IntloadSumReportVO[])getVOs(request, IntloadSumReportVO .class, "sheet1_"));
        }
		return event;
	}

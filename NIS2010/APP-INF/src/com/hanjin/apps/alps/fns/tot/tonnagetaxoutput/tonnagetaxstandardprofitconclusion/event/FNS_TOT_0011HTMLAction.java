...
*@FileName : FNS_TOT_0011HTMLAction.java
*@FileTitle : TOT
*Open Issues :
*Change history :
...
public class FNS_TOT_0011HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_TOT_0001HTMLAction 객체를 생성
	 */
	public FNS_TOT_0011HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsTot0011Event event = new FnsTot0011Event();
        log.debug("::CALL::> FNS_TOT_0011HTMLAction - " + command.getCommand());

		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setStlCfmVO((StlCfmVO)getVO(request, StlCfmVO.class));
			
		}else if(command.isCommand(FormCommand.SEARCH)) {
			event.setVvdStlAmtVO((VvdStlAmtVO)getVO(request, VvdStlAmtVO.class));
		}else if(command.isCommand(FormCommand.SEARCHLIST02)) {
			event.setVvdStlAmtVO((VvdStlAmtVO)getVO(request, VvdStlAmtVO.class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setTotPortStlAmtVO((TotPortStlAmtVO)getVO(request, TotPortStlAmtVO.class));
			event.setStlCfmVO((StlCfmVO)getVO(request, StlCfmVO.class));
		}

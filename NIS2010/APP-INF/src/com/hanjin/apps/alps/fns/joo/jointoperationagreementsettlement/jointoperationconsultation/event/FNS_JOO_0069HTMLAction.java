...
*@FileName : FNS_JOO_0069HTMLAction.java
...
public class FNS_JOO_0069HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0069HTMLAction 객체를 생성
	 */
	public FNS_JOO_0069HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0069Event event = new FnsJoo0069Event(); 
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setSlipConditionVO((SlipConditionVO)getVO(request, SlipConditionVO .class));
		}
...
/*
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setSlp_ofc_cd_sel(request.getParameter("slp_ofc_cd_sel"));
			event.setGubun1(request.getParameter("gubun1"));
			event.setFr_dt(request.getParameter("fr_dt1"));
			event.setTo_dt(request.getParameter("to_dt1"));

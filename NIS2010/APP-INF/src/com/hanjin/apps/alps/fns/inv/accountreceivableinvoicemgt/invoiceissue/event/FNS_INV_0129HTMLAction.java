...
*@FileName : FNS_INV_0129HTMLAction.java
...
public class FNS_INV_0129HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0129HTMLAction 객체를 생성
	 */
	public FNS_INV_0129HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsInv0129Event event = new FnsInv0129Event();		
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setInvArEmlCustRgstVO((InvArEmlCustRgstVO)getVO(request, InvArEmlCustRgstVO.class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			String arOfcCd = request.getParameter("ofc");
            event.setArOfcCd(arOfcCd);
            event.setInvArEmlCustRgstVOs((InvArEmlCustRgstVO[])getVOs(request, InvArEmlCustRgstVO.class, "sheet1_"));

...
*@FileName : CarrierSettlementProcessDBDAOmcsStatusVVDRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CarrierSettlementProcessDBDAOmcsStatusVVDRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public CarrierSettlementProcessDBDAOmcsStatusVVDRSQL(){
		setQuery();
		...
		query.append("FileName : CarrierSettlementProcessDBDAOmcsStatusVVDRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT  ACCT_YRMON, REV_YRMON, JO_CRR_CD, TRD_CD, RLANE_CD, LOCL_CURR_CD, STL_CMB_SEQ, JO_REV, JO_EXP, JO_BALANCE, BENEFIT_LINE, FROM_DT, TO_DT, VVD, JO_SMRY_RMK, EFF_DT" ).append("\n"); 
		query.append("FROM    (" ).append("\n"); 
		query.append("SELECT  " ).append("\n"); 
		query.append("        CASE" ).append("\n"); 
		query.append("            WHEN GUBUN = '0011011' THEN 'SubSum:' || JO_CRR_CD" ).append("\n"); 
		...
		query.append("          CASE" ).append("\n"); 
		query.append("            WHEN GUBUN = '0011011' THEN 'Sub-Total'" ).append("\n"); 
		...
		query.append("FROM    (" ).append("\n"); 
		query.append("SELECT  ACCT_YRMON, REV_YRMON, JO_CRR_CD, TRD_CD, RLANE_CD, LOCL_CURR_CD, MAX(STL_CMB_SEQ) AS STL_CMB_SEQ, SUM(JO_REV) AS JO_REV" ).append("\n"); 
		...
		query.append("       , VVD, MAX(JO_SMRY_RMK) AS JO_SMRY_RMK, MAX(EFF_DT) AS EFF_DT" ).append("\n"); 
		query.append("       , GROUPING(ACCT_YRMON)|| GROUPING(JO_CRR_CD) || GROUPING(TRD_CD) || GROUPING(RLANE_CD) || GROUPING(LOCL_CURR_CD) || GROUPING(REV_YRMON)|| GROUPING(VVD) GUBUN" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(" ).append("\n"); 
		...
		query.append("FROM (" ).append("\n"); 
		query.append("         SELECT A.TRD_CD, A.RLANE_CD, A.ACCT_YRMON, V.REV_YRMON" ).append("\n"); 
		...
		query.append("             #end" ).append("\n"); 
		query.append("	 GROUP BY A.TRD_CD, A.RLANE_CD, A.ACCT_YRMON, A.JO_CRR_CD, K.JO_SMRY_RMK, A.LOCL_CURR_CD , V.REV_YRMON, A.VSL_CD||A.SKD_VOY_NO||A.SKD_DIR_CD||A.REV_DIR_CD, B.STL_CMB_SEQ ) Z )" ).append("\n"); 
		query.append("WHERE   1=1" ).append("\n"); 
		query.append("GROUP BY CUBE (ACCT_YRMON, JO_CRR_CD, LOCL_CURR_CD, TRD_CD, RLANE_CD, REV_YRMON, VVD)" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("WHERE   1=1" ).append("\n"); 
		query.append("AND     GUBUN IN ('0000000', '0011011', '1111011', '1111111')" ).append("\n"); 
		query.append("ORDER BY ACCT_YRMON2, JO_CRR_CD2, NVL2(TRD_CD, 0, 1), LOCL_CURR_CD, VVD" ).append("\n"); 
		query.append(")" ).append("\n"); 

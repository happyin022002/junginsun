...
*@FileName : FNS_INV_0052HTMLAction.java
...
public class FNS_INV_0052HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0052HTMLAction 객체를 생성
	 */
	public FNS_INV_0052HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsInv0052Event event = new FnsInv0052Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setOfcCd(request.getParameter("ar_ofc_cd"));
		}
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setOfcCd(request.getParameter("ar_ofc_cd"));
			event.setBlNo(request.getParameter("bl_src_no"));
			
			korGiroListVO = (KORGiroListVO)getVO(request, KORGiroListVO.class);
			
			//giro List
			InvArGiroVO[] invArGiroList = (InvArGiroVO[]) getVOs(request, InvArGiroVO.class , "sheet1_");
			if(invArGiroList != null) {
				List<InvArGiroVO> invArGiroVO = Arrays.asList(invArGiroList);
				korGiroListVO.setInvArGiroVO(invArGiroVO);
			}
			
			event.setKorGiroListVO(korGiroListVO);
		}

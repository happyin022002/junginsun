...
*@FileName : AccountReceivableEDISendDBDAOSearchSamsungARInvoiceListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountReceivableEDISendDBDAOSearchSamsungARInvoiceListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * EDI 전송가능한 대상 list를 조회한다.
	  * INV_AR_MN/ INV_AR_CHG , BKG_CHG_RT 테이블에서 조회조건으로 조회함.
	  * </pre>
	  */
	public AccountReceivableEDISendDBDAOSearchSamsungARInvoiceListRSQL(){
		setQuery();
		...
		query.append("FileName : AccountReceivableEDISendDBDAOSearchSamsungARInvoiceListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("  rtrim(XMLAGG(XMLELEMENT(C, CNTR_NO, '|') ORDER BY TB.BL_SRC_NO, C.CNTR_NO).EXTRACT( '//text()'), '|') CNTR_NO,  " ).append("\n"); 
		...
		query.append("  (" ).append("\n"); 
		query.append("    SELECT SUM(CASE WHEN CHG_CD ='OFT'" ).append("\n"); 
		query.append("          AND CURR_CD = 'USD'" ).append("\n"); 
		query.append("          AND RAT_UT_CD IN ('D2', '20') THEN RAT_AS_QTY ELSE 0 END)" ).append("\n"); 
		query.append("    FROM BKG_CHG_RT" ).append("\n"); 
		query.append("    WHERE BKG_NO = TB.BKG_NO)INV_EDI_D2_QTY," ).append("\n"); 
		query.append("  (" ).append("\n"); 
		query.append("    SELECT SUM(CASE WHEN CHG_CD ='OFT'" ).append("\n"); 
		query.append("          AND CURR_CD = 'USD'" ).append("\n"); 
		query.append("          AND RAT_UT_CD IN ('D4', '40') THEN RAT_AS_QTY ELSE 0 END)" ).append("\n"); 
		query.append("    FROM BKG_CHG_RT" ).append("\n"); 
		query.append("    WHERE BKG_NO = TB.BKG_NO) INV_EDI_D4_QTY," ).append("\n"); 
		query.append("  (" ).append("\n"); 
		query.append("    SELECT SUM(CASE WHEN CHG_CD ='OFT'" ).append("\n"); 
		query.append("          AND CURR_CD = 'USD'" ).append("\n"); 
		query.append("          AND RAT_UT_CD = 'D5' THEN RAT_AS_QTY ELSE 0 END)" ).append("\n"); 
		query.append("    FROM BKG_CHG_RT" ).append("\n"); 
		query.append("    WHERE BKG_NO = TB.BKG_NO) INV_EDI_D5_QTY," ).append("\n"); 
		query.append("  (" ).append("\n"); 
		query.append("    SELECT SUM(CASE WHEN CHG_CD ='OFT'" ).append("\n"); 
		query.append("          AND CURR_CD = 'USD'" ).append("\n"); 
		query.append("          AND RAT_UT_CD IN ('D7', '45') THEN RAT_AS_QTY ELSE 0 END)" ).append("\n"); 
		query.append("    FROM BKG_CHG_RT" ).append("\n"); 
		query.append("    WHERE BKG_NO = TB.BKG_NO) INV_EDI_D7_QTY," ).append("\n"); 
		query.append("  (" ).append("\n"); 
		query.append("    SELECT SUM(CASE WHEN CHG_CD ='OFT'" ).append("\n"); 
		query.append("          AND CURR_CD = 'USD'" ).append("\n"); 
		query.append("          AND RAT_UT_CD NOT IN ('D2', '20', 'D4', '40', 'D5', 'D7', '45') THEN RAT_AS_QTY ELSE 0 END)" ).append("\n"); 
		query.append("    FROM BKG_CHG_RT" ).append("\n"); 
		query.append("    WHERE BKG_NO = TB.BKG_NO) INV_EDI_ETC_QTY," ).append("\n"); 
		query.append("  TB.GRS_CNTR_WGT," ).append("\n"); 
		query.append("  TB.GRS_CBM_CAPA," ).append("\n"); 
		query.append("  MAX(TB.OFT_AMT) OFT_AMT," ).append("\n"); 
		query.append("  MAX(TB.CMS_AMT) CMS_AMT," ).append("\n"); 
		query.append("  MAX(TB.THC_AMT) THC_AMT," ).append("\n"); 
		query.append("  MAX(TB.DHF_AMT) DHF_AMT," ).append("\n"); 
		query.append("  MAX(TB.WHF_AMT) WHF_AMT," ).append("\n"); 
		query.append("  MAX(TB.OTR_AMT) OTR_AMT," ).append("\n"); 
		query.append("  --SUM(TB.CFR_AMT) CFR_AMT, -- 항목에서 삭제" ).append("\n"); 
		query.append("  --SUM(TB.BAF_AMT) BAF_AMT, -- 항목에서 삭제" ).append("\n"); 
		query.append("  MAX(TB.SLF_AMT) SLF_AMT,     -- 2011.05.23 추가" ).append("\n"); 
		query.append("  MAX(TB.OTH_AMT) OTH_AMT," ).append("\n"); 
		query.append("  (SELECT MAX(SR_INV_NO)" ).append("\n"); 
		query.append("   FROM INV_AR_EDI_GERP_BL" ).append("\n"); 
		query.append("   WHERE BL_SRC_NO = TB.BL_SRC_NO) SR_INV_NO_SEQ" ).append("\n"); 
		query.append("FROM(" ).append("\n"); 
		query.append("    SELECT /*+ INDEX(A XAK4INV_AR_MN)*/A.BL_SRC_NO," ).append("\n"); 
		...
		query.append("      AND (A.BL_SRC_NO, A.AR_IF_NO) IN ( SELECT BL_SRC_NO, AR_IF_NO" ).append("\n"); 
		...
		query.append("    GROUP BY A.BL_SRC_NO, A.BKG_NO, A.POR_CD, A.POL_CD, A.POD_CD, A.DEL_CD, A.CGO_WGT, A.CGO_MEAS_QTY, B.INV_XCH_RT) TB," ).append("\n"); 
		query.append("  BKG_CONTAINER C" ).append("\n"); 
		...
		query.append("ORDER BY TB.BL_SRC_NO" ).append("\n"); 

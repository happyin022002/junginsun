...
public class CarrierSettlementProcessDBDAOReverseSettlementCreationCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       --,NVL((SELECT /*+INDEX_DESC(X XPKJOO_SETTLEMENT)*/ X.STL_SEQ + 1 FROM JOO_SETTLEMENT X WHERE X.ACCT_YRMON = A.ACCT_YRMON AND X.STL_VVD_SEQ = A.STL_VVD_SEQ AND ROWNUM = 1),1) AS STL_SEQ" ).append("\n"); 
		...
		query.append("       ,'REVERSE('||STL_RMK||')' AS STL_RMK" ).append("\n"); 

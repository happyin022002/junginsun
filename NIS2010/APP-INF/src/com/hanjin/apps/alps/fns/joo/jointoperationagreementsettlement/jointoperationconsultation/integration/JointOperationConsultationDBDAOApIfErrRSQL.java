...
public class JointOperationConsultationDBDAOApIfErrRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("AND    A.SLP_TP_CD    = SUBSTR(C.CSR_NO,1,2)" ).append("\n"); 
		query.append("AND    A.SLP_FUNC_CD  = SUBSTR(C.CSR_NO,3,1)" ).append("\n"); 
		query.append("AND    A.SLP_OFC_CD   = DECODE(LENGTH(C.CSR_NO),20,SUBSTR(C.CSR_NO,4,6),SUBSTR(C.CSR_NO,4,5))" ).append("\n"); 
		query.append("AND    A.SLP_ISS_DT   = TO_CHAR(TO_DATE(DECODE(LENGTH(C.CSR_NO),20,SUBSTR(C.CSR_NO,10,6),SUBSTR(C.CSR_NO,9,6)),'RRMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("AND    A.SLP_SER_NO   = DECODE(LENGTH(C.CSR_NO),20,SUBSTR(C.CSR_NO,16),SUBSTR(C.CSR_NO,15))" ).append("\n"); 

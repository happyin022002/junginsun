...
public class FNS_JOO_0109HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0109HTMLAction 객체를 생성
	 */
	public FNS_JOO_0109HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0109Event event = new FnsJoo0109Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMvntEvntDtVO((MvntEvntDtVO)getVO(request, MvntEvntDtVO .class));			
		}else if(command.isCommand(FormCommand.SEARCH01) || command.isCommand(FormCommand.COMMAND01) || command.isCommand(FormCommand.COMMAND02)) {
			event.setMvntEvntDtVO((MvntEvntDtVO)getVO(request, MvntEvntDtVO .class));
        }else if(command.isCommand(FormCommand.MULTI)) {
        	MvntEvntDtVO[] mvntEvntDtVOs = (MvntEvntDtVO[])getVOs(request, MvntEvntDtVO.class,"sheet1_");        	
        	event.setMvntEvntDtVOs(mvntEvntDtVOs);        	
        }
		return  event;
	}

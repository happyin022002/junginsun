...
*@FileName : AccountReceivableEDISendDBDAOSearchEdiADIDASMainRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountReceivableEDISendDBDAOSearchEdiADIDASMainRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public AccountReceivableEDISendDBDAOSearchEdiADIDASMainRSQL(){
		setQuery();
		...
		query.append("FileName : AccountReceivableEDISendDBDAOSearchEdiADIDASMainRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("  C.INV_REF_NO AS  INV_REF_NUMBER," ).append("\n"); 
		query.append("  C.BL_INV_CFM_DT AS INV_REF_CREATION_DATE," ).append("\n"); 
		...
		query.append("  (SELECT  TO_CHAR(BL_OBRD_DT,'YYYYMMDD') FROM BKG_BL_DOC WHERE BKG_NO =C.BKG_NO) BL_ON_BOARD_DATE, --확인필요" ).append("\n"); 
		query.append("  (SELECT RCV_TERM_CD FROM BKG_BOOKING WHERE BKG_NO =C.BKG_NO) RD_TERM_PRECARRIAGE, " ).append("\n"); 
		query.append("  (SELECT DE_TERM_CD FROM BKG_BOOKING WHERE BKG_NO = C.BKG_NO) RD_TERM_ONCARRIAGE, " ).append("\n"); 
		query.append("  (SELECT CUST_CTY_NM FROM BKG_CUSTOMER WHERE BKG_NO = C.BKG_NO AND  BKG_CUST_TP_CD ='N') PT_CITY_NAME," ).append("\n"); 
		query.append("  (SELECT CUST_STE_CD FROM BKG_CUSTOMER WHERE BKG_NO = C.BKG_NO AND  BKG_CUST_TP_CD ='N') PT_ST_PROV_CD," ).append("\n"); 
		query.append("  (SELECT CUST_ZIP_ID FROM BKG_CUSTOMER WHERE BKG_NO = C.BKG_NO AND  BKG_CUST_TP_CD ='N') PT_ZIP_CD," ).append("\n"); 
		query.append("  (SELECT CSTMS_DECL_CNT_CD FROM BKG_CUSTOMER WHERE BKG_NO = C.BKG_NO AND  BKG_CUST_TP_CD ='N') PT_COUNTRY_CD," ).append("\n"); 
		query.append("  (SELECT VSL_RGST_CNT_CD FROM MDM_VSL_CNTR  WHERE VSL_CD = C.VSL_CD) VSL_FLAG," ).append("\n"); 
		query.append("  (SELECT INTER_RMK  FROM BKG_BOOKING WHERE BKG_NO = C.BKG_NO) BKG_REMARK," ).append("\n"); 
		query.append("  C.INV_RMK INV_REMARKS," ).append("\n"); 

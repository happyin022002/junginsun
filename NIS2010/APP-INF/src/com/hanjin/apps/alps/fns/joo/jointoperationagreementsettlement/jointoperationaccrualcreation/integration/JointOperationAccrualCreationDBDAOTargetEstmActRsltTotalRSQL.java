...
*@FileName : JointOperationAccrualCreationDBDAOTargetEstmActRsltTotalRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class JointOperationAccrualCreationDBDAOTargetEstmActRsltTotalRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public JointOperationAccrualCreationDBDAOTargetEstmActRsltTotalRSQL(){
		setQuery();
		...
		query.append("FileName : JointOperationAccrualCreationDBDAOTargetEstmActRsltTotalRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("				   AND EXE_YRMON IN ((SELECT REV_YRMON2 FROM TARGET_YRMON WHERE LVL=1),(SELECT REV_YRMON2 FROM TARGET_YRMON WHERE LVL=2))" ).append("\n"); 
		query.append("                   AND ESTM_BC_DIV_CD = 'C'" ).append("\n"); 
		query.append("                   AND ESTM_IOC_DIV_CD IN ('OO', 'IA', 'IE', 'IM')" ).append("\n"); 
		query.append("                   #if (${rlane_cd}!='')" ).append("\n"); 
		query.append("                   AND RLANE_CD = @[rlane_cd]" ).append("\n"); 
		...
		query.append(")" ).append("\n"); 
		query.append("SELECT SUM(ROUND(AA.ADJ_ESTM_AMT,2)) AS ESTM_AMT" ).append("\n"); 
		query.append("      ,SUM(ROUND(AA.ACT_AMT, 2)) AS ACT_AMT" ).append("\n"); 
		query.append("      ,SUM(ROUND(AA.ADJ_ACCL_AMT,2)) AS ACCL_AMT" ).append("\n"); 
		...
		query.append("      ,ROUND(A.BSA_SLT_PRC, 2) BSA_SLT_PRC" ).append("\n"); 
		...
		query.append("      CASE WHEN A.REV_YRMON = B.REV_YRMON AND A.JO_CRR_CD = B.JO_CRR_CD AND A.VVD = B.VSL_CD||B.SKD_VOY_NO||B.SKD_DIR_CD||B.REV_DIR_CD AND A.RLANE_CD = B.RLANE_CD AND A.JO_STL_JB_CD = B.JO_STL_JB_CD AND A.BSA_QTY = B.BSA_QTY AND A.BSA_SLT_PRC = B.BSA_SLT_PRC" ).append("\n"); 
		query.append("      THEN 'Y'" ).append("\n"); 
		query.append("      ELSE 'N'" ).append("\n"); 
		query.append("      END, DECODE(A.ADJ_ESTM_FLG, 'Y',     -- B.ADJ_ESTM_FLG 이 NULL 이니까 수행" ).append("\n"); 
		query.append("           CASE WHEN A.REV_YRMON = A.JER_REV_YRMON AND A.JO_CRR_CD = A.JER_JO_CRR_CD AND A.VVD = A.JER_VVD AND A.RLANE_CD = A.JER_RLANE_CD AND A.JO_STL_JB_CD = A.JER_JO_STL_JB_CD AND A.BSA_QTY = A.JER_BSA_QTY AND A.BSA_SLT_PRC = A.JER_BSA_SLT_PRC" ).append("\n"); 
		query.append("            THEN 'Y'" ).append("\n"); 
		query.append("            ELSE 'N'" ).append("\n"); 
		...
		query.append("        DECODE(B.ADJ_ESTM_FLG, 'Y',   CASE  WHEN  -50 <=  (B.ADJ_BSA_QTY * B.ADJ_BSA_SLT_PRC) - A.ACT_AMT AND  (B.ADJ_BSA_QTY * B.ADJ_BSA_SLT_PRC) - A.ACT_AMT <= 50 THEN 0  ELSE  (B.ADJ_BSA_QTY * B.ADJ_BSA_SLT_PRC) - A.ACT_AMT  END ," ).append("\n"); 
		query.append("            DECODE(A.ADJ_ESTM_FLG, 'Y',  CASE  WHEN  -50 <=  (A.ADJ_BSA_QTY * A.ADJ_BSA_SLT_PRC) - A.ACT_AMT AND (A.ADJ_BSA_QTY * A.ADJ_BSA_SLT_PRC) - A.ACT_AMT  <= 50 THEN 0 ELSE (A.ADJ_BSA_QTY * A.ADJ_BSA_SLT_PRC) - A.ACT_AMT END" ).append("\n"); 
		query.append("                , CASE  WHEN  -50 <=  (A.BSA_QTY * A.BSA_SLT_PRC) - A.ACT_AMT AND (A.BSA_QTY * A.BSA_SLT_PRC) - A.ACT_AMT <= 50 THEN 0 ELSE (A.BSA_QTY * A.BSA_SLT_PRC) - A.ACT_AMT END" ).append("\n"); 
		query.append("         )) END AS ADJ_ACCL_AMT" ).append("\n"); 
		query.append("         " ).append("\n"); 
		query.append("     ,DECODE(B.ADJ_ESTM_FLG, 'Y',   B.ADJ_RMK, " ).append("\n"); 
		query.append("          DECODE(A.ADJ_ESTM_FLG, 'Y',   A.ADJ_RMK, ''" ).append("\n"); 
		query.append("      )) AS ADJ_RMK" ).append("\n"); 
		query.append("        " ).append("\n"); 
		...
		query.append("AND NVL(AA.ADJ_ACCL_AMT,0) <> 0" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${adjust} != '')" ).append("\n"); 
		query.append("AND AA.ADJ_ESTM_FLG = @[adjust]" ).append("\n"); 
		query.append("#end" ).append("\n"); 

...
public class ARInvoiceInquiryDBDAOInvoiceIssueDateSumVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("  FROM (SELECT B.CURR_CD, A.INV_NO, ROUND(SUM(B.CHG_AMT)*B.INV_XCH_RT, 0) CURR_LOCL_AMT" ).append("\n"); 
		...
		query.append("		 GROUP BY A.INV_NO, B.CURR_CD, B.INV_XCH_RT)" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("SELECT NVL(SUM(CURR_LOCL_AMT),0) TTL_LOCL_AMT" ).append("\n"); 

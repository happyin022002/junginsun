...
*@FileName : FNS_JOO_0106HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class FNS_JOO_0106HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0106HTMLAction 객체를 생성
	 */
	public FNS_JOO_0106HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0106Event event = new FnsJoo0106Event();
		
		if(command.isCommand(FormCommand.MULTI) || command.isCommand(FormCommand.MULTI01)) {
			log.debug("FNS_JOO_0106HTMLAction acct_yrmon = "+JSPUtil.getParameter(request, "acct_yrmon"));
            event.setAcctYrmon( JSPUtil.getParameter(request, "acct_yrmon") );			
		...
		else if(command.isCommand(FormCommand.SEARCHLIST01) || command.isCommand(FormCommand.SEARCHLIST02)) {
			String curRow = request.getParameter("cur_row");
			if (curRow == null || "".equals(curRow)){
				return event;
			}

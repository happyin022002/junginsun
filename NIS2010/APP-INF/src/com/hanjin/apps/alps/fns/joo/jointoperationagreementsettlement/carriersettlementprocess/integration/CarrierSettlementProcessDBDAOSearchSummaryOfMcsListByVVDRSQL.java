...
public class CarrierSettlementProcessDBDAOSearchSummaryOfMcsListByVVDRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("#if (${summarydetail} == 'D')" ).append("\n"); 
		...
		query.append("    		DECODE(A.RE_DIVR_CD,'R',   A.STL_LOCL_AMT / DECODE" ).append("\n"); 
		...
		query.append("	   , DECODE(A.RE_DIVR_CD,'E',A.STL_LOCL_AMT) / DECODE" ).append("\n"); 
		query.append("      (A.LOCL_CURR_CD, 'USD', 1, " ).append("\n"); 
		query.append("         (SELECT NVL(EX1.USD_LOCL_XCH_RT,1) " ).append("\n"); 
		query.append("         FROM GL_MON_XCH_RT EX1" ).append("\n"); 
		query.append("         WHERE  EX1.CURR_CD                         = A.LOCL_CURR_CD" ).append("\n"); 
		...
		query.append("   	   NVL(DECODE(A.RE_DIVR_CD, 'R', A.BSA_QTY), '0') AS R_STL_LOCL_AMT_TEU," ).append("\n"); 
		query.append("	   NVL(DECODE(A.RE_DIVR_CD, 'E', A.BSA_QTY), '0') AS E_STL_LOCL_AMT_TEU," ).append("\n"); 
		query.append("	   NVL(DECODE(A.RE_DIVR_CD, 'R', A.BSA_SLT_PRC), '0') AS R_STL_LOCL_AMT_PRICE," ).append("\n"); 
		query.append("	   NVL(DECODE(A.RE_DIVR_CD, 'E', A.BSA_SLT_PRC), '0') AS E_STL_LOCL_AMT_PRICE," ).append("\n"); 
		query.append("        E.SLP_TP_CD||E.SLP_FUNC_CD||E.SLP_OFC_CD||TO_CHAR(TO_DATE(E.SLP_ISS_DT,'YYYYMMDD'),'RRMMDD')||E.SLP_SER_NO AS CSR_NO" ).append("\n"); 
		...
		query.append("        E.SLP_TP_CD||E.SLP_FUNC_CD||E.SLP_OFC_CD||TO_CHAR(TO_DATE(E.SLP_ISS_DT,'YYYYMMDD'),'RRMMDD')||E.SLP_SER_NO," ).append("\n"); 
		query.append("        A.LOCL_CURR_CD" ).append("\n"); 
		query.append("ORDER BY A.VSL_CD, " ).append("\n"); 
		query.append("        A.SKD_VOY_NO," ).append("\n"); 
		query.append("        A.SKD_DIR_CD," ).append("\n"); 

/*=========================================================
*Copyright(c) 2017 CyberLogitec
*@FileName : ARInvoiceInquiryDBDAOSearchARInvoiceChargeRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
*@LastModifyDate : 2017.08.01
*@LastModifier : 
*@LastVersion : 1.0
* 2017.08.01 
* 1.0 Creation
=========================================================*/
package com.hanjin.apps.alps.fns.inv.accountreceivableinvoicemgt.arinvoiceinquiry.integration;

import java.util.HashMap;
import org.apache.log4j.Logger;
import com.hanjin.framework.support.db.ISQLTemplate;

/**
 *
 * @author 
 * @see DAO 참조
 * @since J2EE 1.6
 */

public class ARInvoiceInquiryDBDAOSearchARInvoiceChargeRSQL implements ISQLTemplate{

	private StringBuffer query = new StringBuffer();
	
	Logger log =Logger.getLogger(this.getClass());
	
	/** Parameters definition in params/param elements */
	private HashMap<String,String[]> params = null;
	
	/**
	  * <pre>
	  * DESC Enter..
	  * </pre>
	  */
	public ARInvoiceInquiryDBDAOSearchARInvoiceChargeRSQL(){
		setQuery();
		params = new HashMap<String,String[]>();
		String tmp = null;
		String[] arrTmp = null;
		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("ar_if_no",new String[]{arrTmp[0],arrTmp[1]});

		query.append("/*").append("\n"); 
		query.append("Path : com.hanjin.apps.alps.fns.inv.accountreceivableinvoicemgt.arinvoiceinquiry.integration").append("\n"); 
		query.append("FileName : ARInvoiceInquiryDBDAOSearchARInvoiceChargeRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	
	public String getSQL(){
		return query.toString();
	}
	
	public HashMap<String,String[]> getParams() {
		return params;
	}

	/**
	 * Query 생성
	 */
	public void setQuery(){
		query.append("SELECT B.CHG_CD," ).append("\n"); 
		query.append("  B.MF_DIV_CD MNL_FLG," ).append("\n"); 
		query.append("  B.RAT_AS_CNTR_QTY," ).append("\n"); 
		query.append("  DECODE (C.DP_PRCS_KNT, '0', TRIM(TO_CHAR(ROUND(B.TRF_RT_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990')), '1', TRIM(TO_CHAR(ROUND(B.TRF_RT_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.9')), '2', TRIM(TO_CHAR(ROUND(B.TRF_RT_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.99')), '3', TRIM(TO_CHAR(ROUND(B.TRF_RT_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.999'))) TRF_RT_AMT," ).append("\n"); 
		query.append("  B.PER_TP_CD," ).append("\n"); 
		query.append("  B.CURR_CD," ).append("\n"); 
		query.append("  DECODE (C.DP_PRCS_KNT, '0', TRIM(TO_CHAR(ROUND(B.CHG_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990')), '1', TRIM(TO_CHAR(ROUND(B.CHG_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.9')), '2', TRIM(TO_CHAR(ROUND(B.CHG_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.99')), '3', TRIM(TO_CHAR(ROUND(B.CHG_AMT, C.DP_PRCS_KNT), '999,999,999,999,999,990.999'))) CHG_AMT," ).append("\n"); 
		query.append("  DECODE(B.TVA_FLG,'Y','Y','') TVA_FLG," ).append("\n"); 
		query.append("  B.CHG_RMK," ).append("\n"); 
		query.append("  --2017.08.01 인도 GST 세법 변경 관련 보완" ).append("\n"); 
		query.append("  TRIM(TO_CHAR(ROUND(B.IDA_CGST_AMT, 2), '999,999,999,999,999,990.99')) IDA_CGST_AMT," ).append("\n"); 
		query.append("  TRIM(TO_CHAR(ROUND(B.IDA_SGST_AMT, 2), '999,999,999,999,999,990.99')) IDA_SGST_AMT," ).append("\n"); 
		query.append("  TRIM(TO_CHAR(ROUND(B.IDA_UGST_AMT, 2), '999,999,999,999,999,990.99')) IDA_UGST_AMT," ).append("\n"); 
		query.append("  TRIM(TO_CHAR(ROUND(B.IDA_IGST_AMT, 2), '999,999,999,999,999,990.99')) IDA_IGST_AMT" ).append("\n"); 
		query.append("FROM INV_AR_MN A," ).append("\n"); 
		query.append("  INV_AR_CHG B," ).append("\n"); 
		query.append("  MDM_CURRENCY C" ).append("\n"); 
		query.append("WHERE A.AR_IF_NO = B.AR_IF_NO" ).append("\n"); 
		query.append("  AND C.CURR_CD = B.CURR_CD" ).append("\n"); 
		query.append("#if(${ar_if_no})" ).append("\n"); 
		query.append("  AND A.AR_IF_NO = @[ar_if_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("  AND NVL(A.INV_DELT_DIV_CD, 'N') <> 'Y'" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("ORDER BY B.AR_IF_SER_NO, B.CHG_SEQ" ).append("\n"); 

	}
}
...
=========================================================*/
package com.hanjin.apps.alps.fns.inv.invcommon;

import java.util.List;

import com.hanjin.apps.alps.fns.inv.invcommon.invcommon.basic.INVCommonUtil;
import com.hanjin.apps.alps.fns.inv.invcommon.invcommon.event.InvCommonEvent;
...
/**
 * ALPS-INVCommon Business Logic ServiceCommand - ALPS-INVCommon 대한 비지니스 트랜잭션을 처리한다.
 * 
 * @author saeil kim
 * @see INVCommonDBDAO
 * @since J2EE 1.4
...
public class INVCommonSC extends ServiceCommandSupport {
	 ...
	 * ALPS-INVCommon system 업무에서 발생하는 모든 이벤트의 분기처리<br>
	 * 
	 * @param Event e
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if (e.getEventName().equalsIgnoreCase("InvCommonEvent")) {
			if (e.getFormCommand().isCommand(FormCommand.SEARCH01)) {
				...
				eventResponse = searchOnBoardDateCustomerByCustRgstNo(e);
	...
	private EventResponse searchCommonCode(Event e) throws EventException {
		...
		try {
			List<CodeInputVO> list = command.searchCommonCode(event.getCodeInputVO());
			
			StringBuffer codes = new StringBuffer();
			if (list != null && list.size() > 0) {
				for (int i = 0 ; i < list.size() ; i++) {
					codes.append(list.get(i).getCode()).append("=").append(list.get(i).getCodeDesc());
	 ...
	 * @author Kwon Min
	 */
	private EventResponse searchOnBoardDateCustomerByCustRgstNo(Event e) throws EventException {
		InvCommonEvent event = (InvCommonEvent) e;
		INVCommonUtil command = new INVCommonUtil();
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		...
		try {
			...
			ExrateDivisionVO exrateDivisionVO = command.searchOnBoardDateCustomerByCustRgstNo(cntryCd, custCd);

			// 조회된 유저 유무 체크
			if (exrateDivisionVO != null) {
				eventResponse.setETCData(exrateDivisionVO.getColumnValues());
			} else {
				eventResponse.setUserMessage(new ErrorHandler("INV00008").getUserMessage());
			}
		} catch (EventException ex) {
	...
	private EventResponse searchRhqList(Event e) throws EventException {
		...
		try {
			...
			List<String> list = command.searchRhqList();
	 ...
	 * @author 
	 */
	private EventResponse searchAROfficeCode(Event e) throws EventException {
		...
		try {
			...
			List<String> list = command.searchAROfficeCode(ofcCd);
			String arOfcCd = "";
			StringBuffer arOfcCdBuff = new StringBuffer();
			if (list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					arOfcCdBuff.append("|" + list.get(i));
					//arOfcCd = arOfcCd + "|" + list.get(i);
				}
				arOfcCd = arOfcCdBuff.toString();
	 ...
	 * @author 
	 */
	private EventResponse searchChargeName(Event e) throws EventException {
		...
		try {
			...
			String chgCd = event.getChgCd().trim();

			String chgNm = command.searchChargeName(chgCd);
			eventResponse.setETCData("chg_nm", chgNm);
	 ...
	 * @author 
	 */
	private EventResponse searchRhqByOfcCd(Event e) throws EventException {
		...
		try {
			...
			List<String> list = command.searchRhqByOfcCd(ofcCd);
			String arHdQtrOfcCd = "";
			StringBuffer arHdQtrOfcCdBuff = new StringBuffer();
			if (list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					arHdQtrOfcCdBuff.append("|" + list.get(i));
					//arHdQtrOfcCd = arHdQtrOfcCd + "|" + list.get(i);
				}
				arHdQtrOfcCd = arHdQtrOfcCdBuff.toString();

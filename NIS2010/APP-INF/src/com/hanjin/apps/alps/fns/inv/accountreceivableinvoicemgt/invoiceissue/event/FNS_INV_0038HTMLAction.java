...
*@FileName : FNS_INV_0038HTMLAction.java
...
public class FNS_INV_0038HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0038HTMLAction 객체를 생성
	 */
	public FNS_INV_0038HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsInv0038Event event = new FnsInv0038Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setOfcCd(request.getParameter("ofc_cd"));
			event.setCurCd(request.getParameter("curCd"));
			event.setCHNIssuedInvoiceListVOS((CHNIssuedInvoiceListVO[])getVOs(request, CHNIssuedInvoiceListVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBlNo(request.getParameter("blNo"));
			event.setOfcCd(request.getParameter("ofc_cd"));
			event.setCurCd(request.getParameter("curCd"));
			//event.setCHNIssuedInvoiceListVO((CHNIssuedInvoiceListVO)getVO(request, CHNIssuedInvoiceListVO .class));

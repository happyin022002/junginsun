...
*@FileName : FNS_INV_0079HTMLAction.java
...
public class FNS_INV_0079HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0079HTMLAction 객체를 생성
	 */
	public FNS_INV_0079HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsInv0079Event event = new FnsInv0079Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setFromDt(request.getParameter("from_dt"));
			event.setToDt(request.getParameter("to_dt"));
			event.setBkgIfFlg(request.getParameter("bkg_if_flg"));
			...
			event.setToDt(request.getParameter("to_dt"));
			event.setBkgIfFlg(request.getParameter("bkg_if_flg"));
			event.setBatchJobID(request.getParameter("batchJobID"));

		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setARBkgInterfaceCreationVOS((ARBkgInterfaceCreationVO[])getVOs(request, ARBkgInterfaceCreationVO .class, ""));
		}

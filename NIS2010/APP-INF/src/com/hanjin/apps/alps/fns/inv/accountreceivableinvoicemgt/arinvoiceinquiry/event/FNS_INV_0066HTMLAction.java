...
*@FileName : FNS_INV_0066HTMLAction.java
...
public class FNS_INV_0066HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0066HTMLAction 객체를 생성
	 */
	public FNS_INV_0066HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsInv0066Event event = new FnsInv0066Event();
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			// rev_src_cd
			String arrRevSrcCd[] = request.getParameterValues("rev_src_cd");
			String revSrcCd = "";
			
			if(arrRevSrcCd == null) {
				arrRevSrcCd = new String[1];
				arrRevSrcCd[0] = request.getParameter("rev_src_cd");
			}
			
			if(arrRevSrcCd.length > 0) {
				for(int i=0; i<arrRevSrcCd.length; i++) {
					if(i==0) revSrcCd = revSrcCd + arrRevSrcCd[i];
					else revSrcCd = revSrcCd +","+ arrRevSrcCd[i];
				}
			}
			
			invoiceInquiryInPutVO = (ARInvoiceInquiryInPutVO)getVO(request, ARInvoiceInquiryInPutVO .class);
			invoiceInquiryInPutVO.setRevSrcCd(revSrcCd);
			
			event.setInvoiceInquiryInPutVO(invoiceInquiryInPutVO);
		}

...
public class JointOperationMasterDataMgtDBDAOTargetVVDBoundCreRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * JOO_STL_BSS_PORT, VSK_VSL_PORT_SKD에서 대상항차를 가져온다.
	  * (Bound)
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("                       END P_RN, '' AS STL_RMK, 'N' AS PENDING_FLG" ).append("\n"); 
		query.append("                FROM  (" ).append("\n"); 
		...
		query.append("      		      	         CASE WHEN (J.PORT_COND = 'F' AND J.OPTP_COND = 'A') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETA_DT)" ).append("\n"); 
		query.append("      		      	              WHEN (J.PORT_COND = 'F' AND J.OPTP_COND = 'B') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETB_DT)" ).append("\n"); 
		query.append("      		      	              WHEN (J.PORT_COND = 'F' AND J.OPTP_COND = 'D') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETD_DT)" ).append("\n"); 
		query.append("      		      	              WHEN (J.PORT_COND = 'L' AND J.OPTP_COND = 'A') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETA_DT DESC)" ).append("\n"); 
		query.append("      		      	              WHEN (J.PORT_COND = 'L' AND J.OPTP_COND = 'B') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETB_DT DESC)" ).append("\n"); 
		query.append("      		      	              WHEN (J.PORT_COND = 'L' AND J.OPTP_COND = 'D') THEN ROW_NUMBER() OVER (PARTITION BY V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, J.JO_STL_ITM_CD ORDER BY V.VPS_ETD_DT DESC)" ).append("\n"); 
		...
		query.append("      		                 AND    A.N1ST_STL_PAIR_PORT_CD IS NOT NULL" ).append("\n"); 

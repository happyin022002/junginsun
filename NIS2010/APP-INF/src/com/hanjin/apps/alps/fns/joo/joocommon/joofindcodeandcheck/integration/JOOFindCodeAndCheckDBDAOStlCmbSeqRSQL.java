...
public class JOOFindCodeAndCheckDBDAOStlCmbSeqRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                  CASE WHEN TO_CHAR(CODE) || NAME = TO_CHAR(CODE) THEN 'Y'" ).append("\n"); 
		query.append("                       ELSE 'N'" ).append("\n"); 
		query.append("                       END" ).append("\n"); 
		query.append("              END" ).append("\n"); 
		query.append("  FROM AP_INV_HDR AP WHERE AP.CSR_NO = A.NAME AND AP.RQST_APRO_STEP_FLG = 'Y') AS APPROVAL" ).append("\n"); 
		query.append("*/" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(" ).append("\n"); 
		query.append("SELECT" ).append("\n"); 
		query.append("       A.STL_CMB_SEQ AS CODE," ).append("\n"); 
		query.append("       MAX(A.SLP_TP_CD||A.SLP_FUNC_CD||A.SLP_OFC_CD||TO_CHAR(TO_DATE(A.SLP_ISS_DT,'YYYYMMDD'),'RRMMDD')||A.SLP_SER_NO) AS NAME" ).append("\n"); 

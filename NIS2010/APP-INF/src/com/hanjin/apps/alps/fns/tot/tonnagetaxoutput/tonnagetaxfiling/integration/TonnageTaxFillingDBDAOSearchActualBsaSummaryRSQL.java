...
*@FileName : TonnageTaxFillingDBDAOSearchActualBsaSummaryRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TonnageTaxFillingDBDAOSearchActualBsaSummaryRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 운항중인 모든 선박(피더선박 제외)들의 초과 BSA 변동별 내역을 조회한다.
	  * </pre>
	  */
	public TonnageTaxFillingDBDAOSearchActualBsaSummaryRSQL(){
		setQuery();
		...
		query.append("FileName : TonnageTaxFillingDBDAOSearchActualBsaSummaryRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("  CASE WHEN INSTR(TO_CHAR(A.USG_RT), '.') = 1 THEN '0'||TO_CHAR(A.USG_RT) ELSE TO_CHAR(A.USG_RT) END" ).append("\n"); 
		...
		query.append("FROM TOT_ACT_BSA_SMRY A," ).append("\n"); 

...
*@FileName : FNS_JOO_0111HTMLAction.java
...
public class FNS_JOO_0111HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0111HTMLAction 객체를 생성
	 */
	public FNS_JOO_0111HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0111Event event = new FnsJoo0111Event();
		...
		if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01)  
																				   ...
																				   || command.isCommand(FormCommand.COMMAND21)
				) {
			event.setTdrLoadVO((TdrLoadVO)getVO(request, TdrLoadVO .class));
        }else if(command.isCommand(FormCommand.MULTI)) {
			event.setTdrLoadVOs((TdrLoadVO[])getVOs(request, TdrLoadVO .class));
			String rlaneCd = request.getParameter("rlane_cd");
			event.setRlaneCd(rlaneCd);
        }else if(command.isCommand(FormCommand.MULTI01)) {
			event.setJoLoadingVOs((JoLoadingVO[])getVOs(request, JoLoadingVO .class));
			String rlaneCd = request.getParameter("rlane_cd");
			event.setRlaneCd(rlaneCd);						
		}
		return  event;

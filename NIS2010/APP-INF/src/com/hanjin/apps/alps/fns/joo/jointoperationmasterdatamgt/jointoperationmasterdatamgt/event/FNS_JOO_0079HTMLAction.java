...
public class FNS_JOO_0079HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0079HTMLAction 객체를 생성
	 */
	public FNS_JOO_0079HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0079Event event = new FnsJoo0079Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setAuthorityOfficeVOs((AuthorityOfficeVO[])getVOs(request, AuthorityOfficeVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCHLIST01)) {
			event.setAuthorityOfficeVO((AuthorityOfficeVO)getVO(request, AuthorityOfficeVO .class));

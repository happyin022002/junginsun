...
*@FileName : JointOperationLetterDBDAOSearchJooLetterRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class JointOperationLetterDBDAOSearchJooLetterRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public JointOperationLetterDBDAOSearchJooLetterRSQL(){
		setQuery();
		...
		query.append("FileName : JointOperationLetterDBDAOSearchJooLetterRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("NVL((SELECT 'Y' FROM JOO_LTR_ATCH_FILE S3 WHERE S3.JO_LTR_SEQ = A.JO_LTR_SEQ AND ROWNUM=1), 'N') ATTACH_YN ," ).append("\n"); 
		query.append("A.JO_LTR_SEQ                    ," ).append("\n"); 
		query.append("A.JO_LTR_TP_CD                  ," ).append("\n"); 
		query.append("A.TRD_CD                        ," ).append("\n"); 
		query.append("A.OFC_CD                        ," ).append("\n"); 
		query.append("TO_CHAR(A.LTR_ISS_DT,'YYYY-MM-DD') LTR_ISS_DT ," ).append("\n"); 
		...
		query.append("A.JO_CNTC_FAX_NO_CTNT               ," ).append("\n"); 
		...
		query.append("(SELECT S.JO_TMPLT_NO FROM  JOO_LTR_TMPLT S WHERE S.JO_LTR_TMPLT_SEQ= A.JO_LTR_TMPLT_SEQ )  jo_tmplt_no" ).append("\n"); 
		query.append("FROM JOO_LETTER A WHERE A.JO_LTR_SEQ= @[jo_ltr_seq]" ).append("\n"); 

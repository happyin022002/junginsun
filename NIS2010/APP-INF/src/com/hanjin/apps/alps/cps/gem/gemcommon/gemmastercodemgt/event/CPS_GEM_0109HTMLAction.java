...
*@FileName : CPS_GEM_0109HTMLAction.java
...
public class CPS_GEM_0109HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	
	/**
	 * CPS_GEM_0109HTMLAction 객체를 생성
	 */
	public CPS_GEM_0109HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		CpsGem0109Event event = new CpsGem0109Event();
		
		if(command.isCommand(FormCommand.SEARCH)
		|| command.isCommand(FormCommand.SEARCHLIST01)) {
			// Ofc_cd Setting
			GemOfcHisVO gemOfcHisVO = (GemOfcHisVO)getVO(request, GemOfcHisVO .class);
			String ofcCd = request.getParameter("ofc_cd")==null?"":request.getParameter("ofc_cd");
			
			gemOfcHisVO.setOfcCd(ofcCd);
			
			event.setGemOfcHisVO(gemOfcHisVO);
		}
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setGemOfcHisVOs((GemOfcHisVO[])getVOs(request, GemOfcHisVO .class,""));
		}

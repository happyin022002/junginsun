...
=========================================================*/
package com.hanjin.apps.alps.cps.gem.gemcommon.gemmastercodemgt.basic;
...
public class GEMMasterCodeMgtBCImpl extends BasicCommandSupport implements GEMMasterCodeMgtBC
{
	 ...
	 * @throws DAOException
	 */
	public List<SubsidiaryAccountMatrixInfoVO> searchSubsidiaryAccountMatrixInfo(OfficeMgtVO officeMgtVO) throws EventException {
		try {			
			List<SubsidiaryAccountMatrixInfoVO> returnList = new ArrayList<SubsidiaryAccountMatrixInfoVO>();			
			returnList = dbDao.searchSubsidiaryAccountMatrixInfo(officeMgtVO);	        	        
			return returnList;
		} catch (DAOException de) {
			log.error("err " + de.toString(), de);
			throw new EventException(new ErrorHandler("COM10001",new String[]{}).getMessage());
     ...
     * @throws DAOException
     */
    public String searchSubsidiaryAccountCheck(String ofcCd, String subsAcctCd) throws EventException {
    	try {
    		return dbDao.searchSubsidiaryAccountCheck(ofcCd, subsAcctCd);    		
    	} catch (DAOException de) {
			log.error("err " + de.toString(), de);
			throw new EventException(new ErrorHandler("COM10001",new String[]{}).getMessage());
		} catch (Exception ex) {
     ...
     * 
     * @param SubsidiaryAccountMatrixInfoVO[] subsidiaryAccountMatrixInfoVOs
     * @param String userId
     * @throws EventException
     */ 
	public void manageSubsidiaryAccountMatrixInfo(SubsidiaryAccountMatrixInfoVO[] subsidiaryAccountMatrixInfoVOs ,String userId) throws EventException {
			
		try {
			List<SubsidiaryAccountMatrixInfoVO> addVoList 	 = new ArrayList<SubsidiaryAccountMatrixInfoVO>();
			List<SubsidiaryAccountMatrixInfoVO> modifyVoList = new ArrayList<SubsidiaryAccountMatrixInfoVO>();
			List<SubsidiaryAccountMatrixInfoVO> removeVoList = new ArrayList<SubsidiaryAccountMatrixInfoVO>();
			
			for(int i=0; i<subsidiaryAccountMatrixInfoVOs.length; i++) {				
				//사용자 설정 
				subsidiaryAccountMatrixInfoVOs[i].setCreUsrId(userId);
				subsidiaryAccountMatrixInfoVOs[i].setUpdUsrId(userId);
				//플래그 취득 
				String ibFlag = subsidiaryAccountMatrixInfoVOs[i].getIbflag();
				
				if("I".equals(ibFlag)) {
					addVoList.add(subsidiaryAccountMatrixInfoVOs[i]);
				} else if("U".equals(ibFlag)) {
					modifyVoList.add(subsidiaryAccountMatrixInfoVOs[i]);					
				} else if ("D".equals(ibFlag)) {
					removeVoList.add(subsidiaryAccountMatrixInfoVOs[i]);
				}
			}
			
			//데이타 입력
			if(addVoList.size() > 0) {				
				dbDao.addSubsidiaryAccountMatrixInfo(addVoList);
			}
			
			//데이타 수정
			if(modifyVoList.size() > 0) {
				dbDao.modifySubsidiaryAccountMatrixInfo(modifyVoList);				
			}
			
			//데이타  삭제
			if(removeVoList.size() > 0) {
				dbDao.removeSubsidiaryAccountMatrixInfo(removeVoList);				
			}

...
=========================================================*/
package com.hanjin.apps.alps.cps.cni.codemgt.codemgt.basic;
...
public class CodeMgtBCImpl extends BasicCommandSupport implements CodeMgtBC {
    ...
    */
	 ...
	 * @param CniUsrRoleMtchVO cniUsrRoleMtchVO
	 * @return List<CniUsrRoleMtchVO>
	 * @throws EventException
	 */
	public List<CniUsrRoleMtchVO> searchCNIUserRoleInfoList(CniUsrRoleMtchVO cniUsrRoleMtchVO) throws EventException {
		try {
			return dbDao.searchCNIUserRoleInfoList(cniUsrRoleMtchVO);
		} catch (DAOException ex) {
			log.error(ex.getMessage());
			throw new EventException(new ErrorHandler("CNI999999").getUserMessage());
		} catch (Exception ex) {
			log.error(ex.getMessage());
			throw new EventException(new ErrorHandler("CNI999999").getUserMessage());
		}
	 ...
	 * 
	 * @param CniUsrRoleMtchVO[] cniUsrRoleMtchVOs
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
	public void manageCNIUserRoleInfoList(CniUsrRoleMtchVO[] cniUsrRoleMtchVOs, SignOnUserAccount account) throws EventException{
		try {
			List<CniUsrRoleMtchVO> 	insertVoList 	= new ArrayList<CniUsrRoleMtchVO>();
			List<CniUsrRoleMtchVO> 	deleteVoList 	= new ArrayList<CniUsrRoleMtchVO>();
			
			if (cniUsrRoleMtchVOs != null) {
				for (int i = 0 ; i < cniUsrRoleMtchVOs.length ; i++) {
					if (cniUsrRoleMtchVOs[i].getIbflag().equals("I")){
						cniUsrRoleMtchVOs[i].setCreUsrId(account.getUsr_id());
						
						insertVoList.add(cniUsrRoleMtchVOs[i]);
					} 
					else if (cniUsrRoleMtchVOs[i].getIbflag().equals("D")){
						deleteVoList.add(cniUsrRoleMtchVOs[i]);
					}
				}
				
				if ( insertVoList.size() > 0 ) {
					dbDao.addCNIUserRoleInfoList(insertVoList);
				}
	
				if ( deleteVoList.size() > 0 ) {
					dbDao.removeCNIUserRoleInfoList(deleteVoList);
				}
			}
		} catch (DAOException ex) {
			log.error(ex.getMessage());
    ...
    /**
	 * Role Code Description 조회<br>
	 * @author 강 환
	 * @category CPS_CNI_0096
	 * @category searchRoleDesc
	 * @param String roleCd
     * @return String
    * @throws EventException
    */
	public String searchRoleDesc(String roleCd) throws EventException{
	
		try {
			return dbDao.searchRoleDesc(roleCd);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CNI99999",new String[]{}).getMessage(), ex);
		} catch (Exception ex) {

 ...
 * ---------------------------------------------------------
 * History
 * 2011.05.26 표준희 [CHM-201111007-01]
 * 1.제목 : Cargo Claim Report 기능
 * 2.처리내역
 * 2.1 Report 상(Preview & Print & GW Approval) 에 B/L 데이터를
 *     Contract of Carriage에 등록된 모든 B/L 데이터를 보여주도록 보완 
 * 2011.06.01 
 ...
 *       확인 결과 Local, Dev(9400, 9300)에서는 에러가 없으나 Live만 에러 발생.  이에, 재 형상관리가 필요함     
=========================================================*/ 

package com.hanjin.apps.alps.cps.cni.containercargoclaimreport;
...
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.basic.ContainerCargoClaimReportBC;
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.basic.ContainerCargoClaimReportBCImpl;
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.event.CpsCni0006Event;
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.event.CpsCni0018Event;
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.event.CpsCni0020Event;
import com.hanjin.apps.alps.cps.cni.containercargoclaimreport.containercargoclaimreport.event.CpsCni0021Event;
...
public class ContainerCargoClaimReportSC extends ServiceCommandSupport {
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if ("CpsCni0006Event".equalsIgnoreCase(eventName)) {
			...
			if (cmd.isCommand(FormCommand.SEARCH)) {
				return searchCargoClaimReport(e);
			// [Print]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				return printCargoClaimReport(e);
			// [Open]	
			} else if (cmd.isCommand(FormCommand.DEFAULT)){
				return printCargoClaimReportOpen(e);	
		    }
		// ---------------------------------------------------------------------------
		// [CPS_CNI_0018] Status 
		// ---------------------------------------------------------------------------		
			...
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				return searchStausList(e);
			// [Open] [Print]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				return printStaus(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				return searchStatusOpen(e);
			}
		// ---------------------------------------------------------------------------
		...
		// ---------------------------------------------------------------------------		
				...
				// [Open]
			 	if (cmd.isCommand(FormCommand.DEFAULT)) {
					//return eventResponse; //searchStatusOpen(e);2011-11-18 [소스품질 조치사항]객체에 null이 배정된 이후 객체에 대한 참조를 하지 말아야 한다.
					return null;
				}
			}
		// ===========================================================================
		...
		// ---------------------------------------------------------------------------		
			...
			// [Open]
			if (cmd.isCommand(FormCommand.DEFAULT)) {
				return searchSettlementAnalysisOpen(e);
			}
			// [Retrieve , Open]
			else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				return searchSettlementAnalysisList(e);
			
			}
			// [PRINT]
			else if (cmd.isCommand(FormCommand.PRINT)) {
				return printSettlementAnalysisList(e);
			} 
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				return searchOccurrenceAnalysisList(e);
			// [Open] [Print]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				return printOccurrenceAnalysisList(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				return searchOccurrenceAnalysisOpen(e);
			}
		}
		//return eventResponse; //searchStatusOpen(e);2011-11-18 [소스품질 조치사항]객체에 null이 배정된 이후 객체에 대한 참조를 하지 말아야 한다.
		return null;
	...
	private EventResponse printStaus(Event e) throws EventException {
	    ...
	    for (int i = 0; i < list.size(); i++) {	
	    	...
	    	pw.print(vo.getN3rdLablPtyCd() +SP);
	...
	private EventResponse printCargoClaimReport(Event e) throws EventException {
	    ...
	    PrintWriter pw = new PrintWriter(sw);
	    // 서브페이지별로 EOR 갯수 생성함
	    // EOR 앞에 반드시 println 처리해야 함
	    // EOR도 println 처리해야함
	    
	    if (status.equals("B")){ //Before Litigation
		    //------------------- 주 쿼리 생성 -------------------//
		    ...
		    pw.print(cargoClaimReportVO.getN3rdLablPtyCd()+SP);
	    ...
	    }else if (status.equals("A")){ // After Litigation
		    //------------------- 주 쿼리 생성 -------------------//
		...
		try {
				...
				if(reportBy.equals("0")) {	
					...
					for (int i = 0; i < list.size(); i++) {			
						...
						pw.print(vo.getAreaPctAmt6() +SP);
						pw.print(vo.getAreaCase7() +SP);			
						pw.print(vo.getAreaPctCase7() +SP); 
						pw.print(vo.getAreaAmt7() +SP); 
						pw.println(vo.getAreaPctAmt7() +SP);
					}
					...
					for (int i = 0; i < list.size(); i++) {			
						...
						pw.print(vo.getHofcPctAmt6() +SP);
						pw.print(vo.getHofcCase7() +SP);			
						pw.print(vo.getHofcPctCase7() +SP); 
						pw.print(vo.getHofcAmt7() +SP); 
						pw.println(vo.getHofcPctAmt7() +SP);
					}

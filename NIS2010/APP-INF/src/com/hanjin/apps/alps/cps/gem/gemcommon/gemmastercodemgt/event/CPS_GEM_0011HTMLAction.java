...
*@FileName : CPS_GEM_0011HTMLAction.java
...
public class CPS_GEM_0011HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	
	/**
	 * CPS_GEM_0011HTMLAction 객체를 생성
	 */
	public CPS_GEM_0011HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		CpsGem0011Event event = new CpsGem0011Event();
		...
		if(command.isCommand(FormCommand.SEARCH)||
		   ...
		   command.isCommand(FormCommand.SEARCHLIST03)) {
			...
			// sch_app_div_gbn
			String schAppDivGbn[] = request.getParameterValues("sch_app_div_gbn");
			
			if(schAppDivGbn == null) {
				schAppDivGbn = new String[1];
				schAppDivGbn[0] = request.getParameter("sch_app_div_gbn");
			}
			
			String strAppDivGbn = "";			
			if(schAppDivGbn.length > 0) {
				for(int i=0; i<schAppDivGbn.length; i++) {
					if(i==0) strAppDivGbn = strAppDivGbn + schAppDivGbn[i];
					else strAppDivGbn = strAppDivGbn +","+ schAppDivGbn[i];
				}
			}
			dto.setSchAppDivGbn(strAppDivGbn);

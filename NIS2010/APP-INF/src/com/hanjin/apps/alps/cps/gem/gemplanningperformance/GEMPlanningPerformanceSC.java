 ...
 * 2011.11.14 이준범[CHM-201114511-01] 
 * 제목 : [GEM] Processing Status 조회시 KRW 기준 데이터 조회 오류 보완
 * 내용 : GemUtil.getLclToKrwAmt Method의 환율 Parameter를 LoclKrwXchRt -> usdKrwXchRt로 변경
 *       기존 Slip Error 신규 화면 라이브 미적용으로 인한 소스 오류 ( Slip Error 관련 소스 주석 처리 ) 
 * 2012.01.05 이준범[선조치] 
 * 제목 : 년도 변경시 승인 제약 사항 제거 
 * 내용 : 2012년 사업 계획 승인전에 년도가 변경되어, 기존 과거 예산 승인 오류 방지를 위한 제약 로직 수정
 * 2012.01.09 이준범[선조치] 
 * 제목 : 년도 변경시 승인 제약 사항 제거 ( 원복 ) 
 * 내용 : 2012년 사업 계획 승인전에 년도가 변경되어, 기존 과거 예산 승인 오류 방지를 위한 제약 로직 원복
 * 
 * 2012.03.16 김영오
 * 제목 : RD프린트 기능 동작 시 2줄 이상인 데이터 처리 시 오류 수정
 * 내용 : desc항목중 "enter"데이터에대한 처리 추가
...
=========================================================*/
package com.hanjin.apps.alps.cps.gem.gemplanningperformance;
...
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem000101Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem000102Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem000103Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0002Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0003Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0004Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem001401Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem001402Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0015Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0016Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0018Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0019Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0023Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0032Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0101Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0104Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0105Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0106Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0108Event;
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGem0112Event;
...
import com.hanjin.apps.alps.cps.gem.gemplanningperformance.gemplanningperformance.event.CpsGemFns0510001Event;
...
public class GEMPlanningPerformanceSC extends ServiceCommandSupport {
	...
	public EventResponse perform(Event e) throws EventException {
		...
		// ---------------------------------------------------------------------------    
		else if ("CpsGem0112Event".equalsIgnoreCase(eventName)) {
			
			// [Open]
			if (cmd.isCommand(FormCommand.INIT)) {
				eventResponse = searchSlpErrorInformation(e);			
			} 
		...
		// ---------------------------------------------------------------------------
		// [CPS_GEM_0018] Subsidiaries Slip Upload
		// ---------------------------------------------------------------------------
	    else if (e.getEventName().equalsIgnoreCase("CpsGem0032Event")) {
				// [FNS003_0001]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageSubsidiarySlipInfo(e);
			}else if (cmd.isCommand(FormCommand.SEARCHLIST)) {
				eventResponse = searchSubsidiarySlipInfo(e);
			}
		}		
...
//							.getFmLoclKrwXchRt());
					BigDecimal fmUsdKrwXchRt = new BigDecimal(vo
							.getFmUsdKrwXchRt());
					BigDecimal fmUsdLoclXchRt = new BigDecimal(vo
							.getFmUsdLoclXchRt());
...
//								.getToLoclKrwXchRt());
						BigDecimal toUsdKrwXchRt = new BigDecimal(vo
								.getToUsdKrwXchRt());
						BigDecimal toUsdLoclXchRt = new BigDecimal(vo
								.getToUsdLoclXchRt());
...
//								.getToLoclKrwXchRt());
	...
	private EventResponse report0023S1(Event e) throws EventException {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				//2012-03-16
				//desc항목중 "enter"데이터에대한 처리 추가
				String expnItmdesc = vo.getGenExpnItmDesc();
				if (expnItmdesc != null) {
					expnItmdesc = expnItmdesc.replaceAll("\r\n", "");
				} else {
					expnItmdesc = "";
				}				
				pw.print(expnItmdesc + SP);
	...
	private EventResponse report0024S1(Event e) throws EventException {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String fmGenExpnItmdesc = vo.getFmGenExpnItmDesc();
				if (fmGenExpnItmdesc != null) {
					fmGenExpnItmdesc = fmGenExpnItmdesc.replaceAll("\r\n", "");
				} else {
					fmGenExpnItmdesc = "";
				}				
				pw.print(fmGenExpnItmdesc + SP);
				
				String toGenExpnItmdesc = vo.getToGenExpnItmDesc();
				if (toGenExpnItmdesc != null) {
					toGenExpnItmdesc = toGenExpnItmdesc.replaceAll("\r\n", "");
				} else {
					toGenExpnItmdesc = "";
				}				
				pw.print(toGenExpnItmdesc + SP);
	...
	private EventResponse report0025S1(Event e) throws EventException {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String expnItmdesc = vo.getFmGenExpnItmDesc();
				if (expnItmdesc != null) {
					expnItmdesc = expnItmdesc.replaceAll("\r\n", "");
				} else {
					expnItmdesc = "";
	...
	private EventResponse searchSubsidiaryActualResults(Event e)
		...
		try {
			...
			list = command.searchSubsidiaryActualResults(event.getActRsltSubsPerfVO());
	...
	private EventResponse searchSlpErrorInformation(Event e) throws EventException {
		...
		try {
			...
			GEMPlanningPerformanceBC command = new GEMPlanningPerformanceBCImpl();
			// 리스트 취득
			List<RsltSlpErrorInformationVO> list = new ArrayList<RsltSlpErrorInformationVO>();
			// 이벤트 생성
			
			list = command.searchSlpErrorInformation();
			eventResponse.setRsVoList(list);
	...
	/**
     * 조회 기간에 대하여 Closing 반영된 HJS 집행단위별 집행실적 분석 Report<br>
     * 
     * @author P.C.J
	...
	// ---------------------------------------------------------------------------
	// [CPS_GEM_0032] Subsidiaries Slip UPload
	// ---------------------------------------------------------------------------
    
	/**
     * CPS_GEM_0032 Slip Upload 데이터를 저장한다.
     * 
     * @author lee jun bum
	 ...
	 * @exception EventException
	 */
	public EventResponse manageSubsidiarySlipInfo(Event e) throws EventException {
		
		//반환 객체
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		
		
		CpsGem0032Event event = (CpsGem0032Event) e;
		
		GEMPlanningPerformanceBC command = new GEMPlanningPerformanceBCImpl();
		
		GemSlpIfVO[] gemSlpIfVOs =  event.getGemSlpIfVOs();
		
		//insert 대상
		List<GemSlpIfVO> insertVOs  = new ArrayList<GemSlpIfVO>();
		//update 대상
		List<GemSlpIfVO> updateVOs  = new ArrayList<GemSlpIfVO>();
		//delete 대상
		List<GemSlpIfVO> deleteVOs  = new ArrayList<GemSlpIfVO>();
		//실적 대상 List
		List<GemSlpIfVO> subsSlpVOs = new ArrayList<GemSlpIfVO>();
		
		// 0. List Add
		for (int i = 0; i < gemSlpIfVOs.length; i++) {
			String ibFlag   = gemSlpIfVOs[i].getIbflag();
			String slpTjNo  = gemSlpIfVOs[i].getSlpTjNo();
			String slpSeqNo = gemSlpIfVOs[i].getSlpSeqNo();
			String ofcCd    = gemSlpIfVOs[i].getOfcCd();
			String glEffDt  = gemSlpIfVOs[i].getGlEffDt();
			String creUsrId = account.getUsr_id(); 
			String updUsrId = account.getUsr_id();
			
			gemSlpIfVOs[i].setSlpIfFlg("N");
			gemSlpIfVOs[i].setCreUsrId(creUsrId);
			gemSlpIfVOs[i].setUpdUsrId(updUsrId);	
			
			if("U".equals(ibFlag)) {
				updateVOs.add(gemSlpIfVOs[i]);
				subsSlpVOs.add(gemSlpIfVOs[i]);
			}else if("I".equals(ibFlag)) {
				//전표 번호 신규 채번 ( 중복 방지 )
				slpTjNo = slpTjNo + ofcCd + glEffDt ;
				gemSlpIfVOs[i].setSlpTjNo(slpTjNo);
				
				if(!command.searchSlpIfCheck(slpTjNo, slpSeqNo)) {
					insertVOs.add(gemSlpIfVOs[i]);
					subsSlpVOs.add(gemSlpIfVOs[i]);
				}else{
					// 전표가 중복되면 중단함
					eventResponse.setETCData("slpTjNo", slpTjNo);
					eventResponse.setETCData("slpSeqNo", slpSeqNo);
					return eventResponse;
				}
			}else if("D".equals(ibFlag)){
				deleteVOs.add(gemSlpIfVOs[i]);
			}
		}
		// GEM_SLP_IF Table에 Insert	
		if(insertVOs.size() > 0){
			command.createSubsidiarySlip(insertVOs);
		}
		// GEM_SLP_IF Table에 update	
		if(updateVOs.size() > 0){
			command.modifySubsidiarySlipIf(updateVOs);
		}
		// GEM_SLP_IF Table에 delete
		if(deleteVOs.size() > 0){
			command.removeSubsidiarySlipIf(deleteVOs);
		}
		
		// insCnt > 0 면 GEM_SLP_PERF Table에 Insert를 수행 오류가 나면 RollBack함.
		if( subsSlpVOs.size() > 0) {

			int saveCnt = 0;
			int failCnt = 0;
			
			for (int i = 0; i < subsSlpVOs.size(); i++) {				

				boolean saveboolean = false;
				GemSlpIfVO gemSlpIfVO = (GemSlpIfVO)subsSlpVOs.get(i);
				
				try {
					begin();
				// GEM_SLP_IF 테이블에 Insert 성공이면
					saveboolean = command.manageSubsidiarySlipInfo(gemSlpIfVO);
			...
			eventResponse.setETCData("saveCnt", Integer.toString(saveCnt));
			eventResponse.setETCData("failCnt", Integer.toString(failCnt));
		}		
		return eventResponse;
	}
	 ...
	 * @exception EventException
	 */
	private EventResponse searchSubsidiarySlipInfo(Event e) throws EventException {
		...
		try {
			...
			String slpIfFlg = event.getSlpIfFlg();
			
			list = command.searchSubsidiarySlipInfo(event.getActRsltSubsPerfVO(), slpIfFlg);

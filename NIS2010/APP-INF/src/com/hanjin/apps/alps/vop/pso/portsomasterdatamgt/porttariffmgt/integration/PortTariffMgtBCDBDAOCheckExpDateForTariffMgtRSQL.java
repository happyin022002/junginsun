...
public class PortTariffMgtBCDBDAOCheckExpDateForTariffMgtRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT DECODE(COUNT(*), 0, 'O', 'X') FLAG -- 불가(X)인 건이 있는지 확인" ).append("\n"); 
		query.append("FROM   (" ).append("\n"); 
		query.append("        SELECT DECODE(X.YD_CHG_NO, @[yd_chg_no]," ).append("\n"); 
		query.append("                      CASE WHEN TO_DATE(@[exp_dt], 'YYYY-MM-DD') <  X.EFF_DT THEN 'X'    -- 입력된 EXP_DT가 EFF_DT 보다 작은 경우 불가(X), 즉 TO_DT < FM_DT" ).append("\n"); 
		query.append("                           WHEN TO_DATE(@[exp_dt], 'YYYY-MM-DD') > NVL(LAG(X.EFF_DT) OVER(ORDER BY X.EFF_DT DESC), TO_DATE('9999-12-31','YYYY-MM-DD')) THEN 'X' -- 입력된 EXP_DT가 다음 버전 EFF_DT보다 클 경우 불가(X)" ).append("\n"); 

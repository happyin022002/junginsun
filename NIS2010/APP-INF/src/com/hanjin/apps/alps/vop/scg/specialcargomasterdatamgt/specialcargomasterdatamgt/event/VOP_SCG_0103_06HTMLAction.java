...
public class VOP_SCG_0103_06HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VMS_SCG_0031HTMLAction 객체를 생성
	 */
	public VOP_SCG_0103_06HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	VopScg010306Event event = new VopScg010306Event();
		
    	if(command.isCommand(FormCommand.MULTI)) {
			event.setScgPckGasReguVOS((ScgPckGasReguVO[])getVOs(request, ScgPckGasReguVO .class,""));
			event.setScgPckRefVOS((ScgPckRefVO[])getVOs(request, ScgPckRefVO .class,"scgPckRef_"));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setScgPckGasReguVO((ScgPckGasReguVO)getVO(request, ScgPckGasReguVO .class));

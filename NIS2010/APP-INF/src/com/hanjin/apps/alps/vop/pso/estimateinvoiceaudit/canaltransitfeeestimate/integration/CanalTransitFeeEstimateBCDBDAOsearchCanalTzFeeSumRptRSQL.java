...
public class CanalTransitFeeEstimateBCDBDAOsearchCanalTzFeeSumRptRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("--                            ) PAY_TO," ).append("\n"); 
		query.append("CNL_AGN_VNDR_SEQ AS PAY_TO," ).append("\n"); 
		query.append("                            (" ).append("\n"); 
		query.append("                                SELECT  VSL_SLAN_CD" ).append("\n"); 
		query.append("                                FROM    MDM_VSL_SVC_LANE T1" ).append("\n"); 
		query.append("                                WHERE   VSL_SLAN_CD  = T2.VSL_SLAN_CD" ).append("\n"); 
		query.append("                            ) LANE" ).append("\n"); 
		query.append("                            ,T1.VSL_CD, T1.SKD_VOY_NO, T1.SKD_DIR_CD,T3.YD_CD," ).append("\n"); 
		query.append("                            (" ).append("\n"); 
		...
		query.append("                            ,(" ).append("\n"); 
		query.append("                                NVL((	SELECT DECODE(INV_STS_CD,'D','P',CNL_TZ_PROC_STS_CD)" ).append("\n"); 
		...
		query.append("										   AND A.CNL_TZ_BZTP_CD = 'E'  ), 'X')" ).append("\n"); 
		...
		query.append("                                              CALL_SEQ" ).append("\n"); 
		query.append("                    FROM    ( -- CHM-201431550 : Canal Transit List내 Pay to 조회 로직 변경 " ).append("\n"); 
		query.append("                                SELECT PSO_BZTP_CD,VSL_CD,SKD_VOY_NO,SKD_DIR_CD,CNL_AGN_VNDR_SEQ,YD_CD" ).append("\n"); 
		query.append("                                FROM (" ).append("\n"); 
		query.append("                                        SELECT PSO_BZTP_CD,VSL_CD,SKD_VOY_NO,SKD_DIR_CD,CNL_AGN_VNDR_SEQ,YD_CD" ).append("\n"); 
		query.append("                                        ,CNT,CASE WHEN CNT > 1 AND CNL_AGN_VNDR_SEQ IS NULL THEN 'N' ELSE 'Y' END DIS" ).append("\n"); 
		query.append("                                        FROM (" ).append("\n"); 
		query.append("                                                SELECT  PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD" ).append("\n"); 
		query.append("                                                       ,CNL_AGN_VNDR_SEQ,YD_CD" ).append("\n"); 
		query.append("                                                       ,COUNT(1) OVER (PARTITION BY PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD,CNL_AGN_VNDR_SEQ) CNT" ).append("\n"); 
		query.append("                                                       ,MIN(SORT) SORT" ).append("\n"); 
		query.append("                                                  FROM    (" ).append("\n"); 
		query.append("                                                        SELECT  5 PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD, CNL_AGN_VNDR_SEQ, 'T' SORT," ).append("\n"); 
		query.append("                                                                 ( SELECT DECODE(VNDR_CNT_CD,'EG','EGSUZT1','PAPACT1')  " ).append("\n"); 
		query.append("                                                                     FROM MDM_VENDOR " ).append("\n"); 
		query.append("                                                                    WHERE VNDR_SEQ = M.CNL_AGN_VNDR_SEQ ) AS YD_CD" ).append("\n"); 
		query.append("                                                        FROM    PSO_TGT_VVD T, VSK_CNL_VNDR M" ).append("\n"); 
		query.append("                                                        WHERE   PSO_BZTP_CD = '5'" ).append("\n"); 
		query.append("                                                        AND     EXPN_YRMON = replace(@[revyrmon],'-', '')--'200906'                   " ).append("\n"); 
		query.append("                                                        AND     T.VSL_SLAN_CD = M.VSL_SLAN_CD                 " ).append("\n"); 
		query.append("                                                        UNION ALL" ).append("\n"); 
		query.append("                                                        SELECT 5 PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD,vndr_seq AS CNL_AGN_VNDR_SEQ, 'C',YD_CD" ).append("\n"); 
		query.append("                                                         FROM    PSO_CNL_TZ_FEE" ).append("\n"); 
		query.append("                                                        WHERE   PSO_BZTP_CD = '5'" ).append("\n"); 
		query.append("                                                        AND     NTC_YRMON = replace(@[revyrmon],'-', '')--'200906'" ).append("\n"); 
		query.append("                                                    )        " ).append("\n"); 
		query.append("                                                GROUP BY PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD,CNL_AGN_VNDR_SEQ,YD_CD" ).append("\n"); 
		query.append("                                        )" ).append("\n"); 
		query.append("                                        WHERE 1=1" ).append("\n"); 
		query.append("                                        AND 'Y' = (CASE WHEN CNT > 1 AND SORT = 'T' THEN 'N' ELSE 'Y' END)--PSO_TGT_VVD 와 PSO_CNL_TZ_FEE 다른 Vendor 를 가진경우 PSO_TGT_VVD 조회 결과는 무시." ).append("\n"); 
		query.append("                                )" ).append("\n"); 
		query.append("                                GROUP BY PSO_BZTP_CD, VSL_CD,SKD_VOY_NO,SKD_DIR_CD,CNL_AGN_VNDR_SEQ,YD_CD" ).append("\n"); 
		...
		query.append("                          OR (  NVL(T3.SKD_CNG_STS_CD,' ') =  'S'" ).append("\n"); 
		query.append("                                 AND  EXISTS ( SELECT * FROM PSO_CHARGE A, PSO_CHG_DTL B" ).append("\n"); 
		query.append("                                                WHERE A.ISS_CTY_CD  = B.ISS_CTY_CD" ).append("\n"); 
		query.append("                                                  AND A.SO_SEQ      = B.SO_SEQ" ).append("\n"); 
		...
		query.append("                                                  AND A.YD_CD       = T3.YD_CD )) )" ).append("\n"); 
		query.append("                    )" ).append("\n"); 
		query.append("            GROUP BY YD_CD, PAY_TO, ROLLUP (VSL_CD||SKD_VOY_NO||SKD_DIR_CD)" ).append("\n"); 
		query.append("            ) T11" ).append("\n"); 
		query.append(")        " ).append("\n"); 
		query.append("where 1 = 1" ).append("\n"); 
		query.append("  AND YD_CD IS NOT NULL" ).append("\n"); 

...
public class OnTimeResultAnalysisDBDAOSearchRsltSkipStsListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT  DECODE(T21.GRP, 'ZZZZZZZ', 'TOTAL', T21.GRP) AS LANE, DECODE(T21.NO, 1, 'CNT', 2, 'HRS') AS CNT_HRS" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 01, CNT_INFO)) AS SBW" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 02, CNT_INFO)) AS SMT" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 03, CNT_INFO)) AS SVD" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 04, CNT_INFO)) AS SDA" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 05, CNT_INFO)) AS WAD" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 06, CNT_INFO)) AS WPG" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 07, CNT_INFO)) AS WPC" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 08, CNT_INFO)) AS WPV" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 09, CNT_INFO)) AS WMT" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 10, CNT_INFO)) AS WCA" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 11, CNT_INFO)) AS WIO" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 12, CNT_INFO)) AS WNH" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 13, CNT_INFO)) AS OTH" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 14, CNT_INFO)) AS BTT" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 15, CNT_INFO)) AS BLS" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 16, CNT_INFO)) AS BDA" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 17, CNT_INFO)) AS BNS" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 18, CNT_INFO)) AS BCW" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 19, CNT_INFO)) AS BCM " ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 20, CNT_INFO)) AS BRB" ).append("\n"); 
		query.append("        ,MAX(DECODE(INTG_CD_VAL_DP_SEQ, 21, CNT_INFO)) AS BSP" ).append("\n"); 
		query.append("        ,MAX(DECODE(T21.NO, 1, NVL(TTL_SKIP_CNT, 0) ||CHR(10)|| '100' || '%'  , 2, TTL_SKIP_TM ||CHR(10)||'100' || '%' )) AS TTL        " ).append("\n"); 
		...
		query.append("            SELECT  NO, INTG_CD_VAL_DP_SEQ, INTG_CD_VAL_CTNT, GRP, RSN_CD, SKIP_CNT, SKIP_TM, TTL_SKIP_CNT, TTL_SKIP_TM" ).append("\n"); 
		query.append("					, DECODE(NO, 1, TO_CHAR(SKIP_CNT, '999,999') ||CHR(10)|| TO_CHAR(ROUND((SKIP_CNT / TTL_SKIP_CNT) * 100, 1), '990.0') || '%'  , 2, TO_CHAR(SKIP_TM, '999,999') ||CHR(10)||TO_CHAR(ROUND(DECODE(TTL_SKIP_TM,0,0,(SKIP_TM /TTL_SKIP_TM)) * 100, 1), '990.0')||'%' ) AS CNT_INFO" ).append("\n"); 
		query.append("            FROM    (" ).append("\n"); 
		...
		query.append("								                            T1.PORT_SKP_RSN_OFFR_RMK AS GRP," ).append("\n"); 
		query.append("								                        #end" ).append("\n"); 
		...
		query.append("						                        #end" ).append("\n"); 
		query.append("						                        #if (${grp_flg} == 'R')" ).append("\n"); 
		query.append("						                        GROUP BY T1.PORT_SKP_RSN_OFFR_RMK, PORT_SKP_RSN_CD" ).append("\n"); 
		query.append("						                        #end" ).append("\n"); 
		query.append("                                            )" ).append("\n"); 
		...
		query.append("        (" ).append("\n"); 
		query.append("            SELECT  NO, GRP, CALL_CNT" ).append("\n"); 
		query.append("            FROM    (" ).append("\n"); 
		...
		query.append("                        #end" ).append("\n"); 
		query.append("                        #if (${grp_flg} == 'R')" ).append("\n"); 
		query.append("                        GROUP BY ROLLUP(T1.PORT_SKP_RSN_OFFR_RMK)" ).append("\n"); 
		query.append("                        #end" ).append("\n"); 

...
public class OwnDangerousCargoApprovalDBDAOSearchOwnDGListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        A.NET_EXPLO_WGT,           -- 2012.11.13 Add" ).append("\n"); 
		query.append("        A.IMDG_SEGR_GRP_NO" ).append("\n"); 
		query.append("FROM	" ).append("\n"); 
		query.append("	(	SELECT 	" ).append("\n"); 
		query.append("			#if (${scg_flg} == 'DG1' || ${scg_flg} == 'DG2') " ).append("\n"); 
		query.append("				/*+ ORDERED USE_NL(B C E F A v vvs) use_hash(d) */" ).append("\n"); 
		...
		query.append("				DENSE_RANK() OVER (PARTITION BY A.BKG_NO ORDER BY F.DG_CNTR_SEQ) AS DG_CNTR_SEQ," ).append("\n"); 
		...
		query.append("                DECODE( SUBSTR(F.IMDG_CLSS_CD, 1, 1), '1', F.NET_EXPLO_WGT, 0) AS NET_EXPLO_WGT,   -- 2012.11.13 Add (class 1만 입력하도록 되어 있으므로, class 1 이외의 class는 0 으로 표시되면 됨)" ).append("\n"); 
		query.append("                F.HZD_CTNT AS IMDG_SEGR_GRP_NO" ).append("\n"); 
		query.append("		FROM	SCG_APRO_RQST B, " ).append("\n"); 
		query.append("                SCG_VVD_APRO_RQST C, " ).append("\n"); 
		query.append("                MDM_VSL_CNTR D," ).append("\n"); 
		query.append("			#if (${scg_flg} == 'SCG_DG')" ).append("\n"); 
		...
		query.append("		--AND DECODE(C.VSL_PRE_PST_CD,'U',V.VPS_ETA_DT,B.RQST_DT)	<= (SELECT GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE,BKG_COM_USER_LOC_FNC([usr_id])) FROM DUAL) + TO_NUMBER([from_eta_dt])" ).append("\n"); 
		query.append("    --#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("												   " ).append("\n"); 
		query.append("		) A, " ).append("\n"); 

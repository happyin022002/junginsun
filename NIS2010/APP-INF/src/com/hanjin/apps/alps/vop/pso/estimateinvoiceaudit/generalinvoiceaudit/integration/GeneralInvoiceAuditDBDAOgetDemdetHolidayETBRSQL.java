...
public class GeneralInvoiceAuditDBDAOgetDemdetHolidayETBRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                    DECODE(TO_CHAR(VPS_ETB_DT,'D'),5,'Y',6,'Y','N' )" ).append("\n"); 
		query.append("                WHEN WKND_TP_CD = 'FS' THEN" ).append("\n"); 
		query.append("                    DECODE(TO_CHAR(VPS_ETB_DT,'D'),6,'Y',7,'Y','N' )" ).append("\n"); 
		query.append("				WHEN SUBSTR(@[yd_cd], 1, 2) = 'KR' THEN" ).append("\n"); 
		query.append("				    DECODE(TO_CHAR(VPS_ETB_DT,'D'),7,'N',1,'Y','N')  " ).append("\n"); 
		query.append("                ELSE" ).append("\n"); 
		query.append("					" ).append("\n"); 
		query.append("                    DECODE(TO_CHAR(VPS_ETB_DT,'D'),7,'Y',1,'Y','N')  " ).append("\n"); 

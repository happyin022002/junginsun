...
public class OwnDangerousCargoApprovalDBDAOSearchScgRequestApvlTimeOutputVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("-- RSO 구분 없이 조회되도록 수정 2017.07.10" ).append("\n"); 
		query.append("SELECT --C.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("       C.TERM" ).append("\n"); 
		query.append("	 , MAX(D.AVG_DG) AS AVG_DG" ).append("\n"); 
		query.append("	 , MAX(D.AVG_RF) AS AVG_RF" ).append("\n"); 
		query.append("	 , MAX(D.AVG_AK) AS AVG_AK" ).append("\n"); 
		query.append("	 , MAX(D.AVG_BB) AS AVG_BB" ).append("\n"); 
		query.append("	 , MAX(D.TOT_AVG) AS TOT_AVG" ).append("\n"); 
		...
		query.append("					 , S1.AUTH_FLG_CDT" ).append("\n"); 
		...
		query.append("							/*" ).append("\n"); 
		query.append("                             , DECODE(NVL(MAX(R2.SPCL_CGO_AUTH_CD),'R'),'Y',NULL,'P',-1,'N',1,0) AUTH_FLG --AUTH_FLG=NULL --> Y, AUTH_FLG<0 --> P, AUTH_FLG>0 --> N, AUTH_FLG=0 --> R " ).append("\n"); 
		query.append("                             , NVL(MAX(R2.SPCL_CGO_AUTH_CD),'R') AS AUTH_FLG_CDT" ).append("\n"); 
		...
		query.append("							*/" ).append("\n"); 
		query.append("                             , DECODE(NVL(R2.SPCL_CGO_AUTH_CD,'R'),'Y',NULL,'P',-1,'N',1,0) AUTH_FLG --AUTH_FLG=NULL --> Y, AUTH_FLG<0 --> P, AUTH_FLG>0 --> N, AUTH_FLG=0 --> R " ).append("\n"); 
		query.append("                             , NVL(R2.SPCL_CGO_AUTH_CD,'R') AS AUTH_FLG_CDT" ).append("\n"); 
		query.append("                             , R3.RQST_GDT" ).append("\n"); 
		query.append("                             , DECODE(NVL(R2.SPCL_CGO_AUTH_CD,'R'),'P',4,'N',3,'R',2,1)||TO_CHAR(NVL(R2.AUTH_GDT,R3.RQST_GDT),'YYYYMMDDHH24MISS') AUTH_GDT " ).append("\n"); 
		...
		query.append("                           #end" ).append("\n"); 
		query.append("                           #if (${crr_cd} != '' && ${crr_cd} != 'NOTSML')" ).append("\n"); 
		query.append("                           AND VO.CRR_CD          = @[crr_cd] 		    --(5) VSL OPR" ).append("\n"); 
		query.append("                           #end" ).append("\n"); 
		query.append("                           #if (${crr_cd} == 'NOTSML')" ).append("\n"); 
		query.append("                           AND VO.CRR_CD          != 'SML' 		    --(5) VSL OPR" ).append("\n"); 
		query.append("                           #end" ).append("\n"); 
		query.append("                           #if (${auth_usr_id} != '')" ).append("\n"); 
		query.append("                           AND R2.AUTH_USR_ID     LIKE '%'||@[auth_usr_id]||'%'	--" ).append("\n"); 
		query.append("                           #end" ).append("\n"); 
		...
		query.append("				 AND S1.AUTH_FIX = 1 " ).append("\n"); 
		query.append("				 AND (S1.AUTH_FLG_CDT = 'Y' OR S1.AUTH_FLG_CDT = 'N' OR (S1.AUTH_FLG_CDT = 'R' AND S1.LST_RQST_DAT_FLG = 'Y')" ).append("\n"); 
		query.append("					#if (${option_pending} == 'Y') " ).append("\n"); 
		query.append("                    --OR AUTH_FLG < 0 --(2) Option" ).append("\n"); 
		query.append("						OR AUTH_FLG_CDT = 'P') --(2) Option" ).append("\n"); 
		query.append("					#else" ).append("\n"); 
		query.append("						)" ).append("\n"); 
		query.append("					#end" ).append("\n"); 
		...
		query.append("                         WHERE 1=1" ).append("\n"); 
		query.append("						 --AND NVL(TS.DELT_FLG,'N') = 'N'" ).append("\n"); 
		query.append("                         #if (${rgn_shp_opr_cd} != '')" ).append("\n"); 
		...
		query.append("         WHERE 1=1" ).append("\n"); 
		query.append("		   --AND B.RGN_SHP_OPR_CD = A.RGN_SHP_OPR_CD(+)" ).append("\n"); 
		query.append("           AND B.YYYY           = A.C_YYYY(+)" ).append("\n"); 
		query.append("           AND B.TERM           = A.C_TERM(+)" ).append("\n"); 
		query.append("           " ).append("\n"); 
		query.append("      ) C" ).append("\n"); 
		query.append("      , " ).append("\n"); 
		query.append("      (SELECT  --A.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                   (B.YYYY||DECODE(B.D_TERM, '', B.D_TERM, ' '||B.D_TERM||DECODE(@[term],'M','',@[term]))) AS TERM" ).append("\n"); 
		query.append("                 , SUM(A.AVG_DG) AS AVG_DG" ).append("\n"); 
		query.append("                 , SUM(A.AVG_RF) AS AVG_RF" ).append("\n"); 
		query.append("                 , SUM(A.AVG_AK) AS AVG_AK" ).append("\n"); 
		query.append("                 , SUM(A.AVG_BB) AS AVG_BB" ).append("\n"); 
		query.append("				 , ROUND(SUM(A.TRET_GAP) / SUM(A.CNT), 1) AS TOT_AVG" ).append("\n"); 
		query.append("        FROM    (SELECT --RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                       C_YYYY" ).append("\n"); 
		query.append("                     , C_TERM" ).append("\n"); 
		query.append("                     , DECODE(SPCL_CGO_CATE_CD, 'DG',NVL(AVG_GAP,0)) AS AVG_DG" ).append("\n"); 
		query.append("                     , DECODE(SPCL_CGO_CATE_CD, 'RF',NVL(AVG_GAP,0)) AS AVG_RF" ).append("\n"); 
		query.append("                     , DECODE(SPCL_CGO_CATE_CD, 'AK',NVL(AVG_GAP,0)) AS AVG_AK" ).append("\n"); 
		query.append("                     , DECODE(SPCL_CGO_CATE_CD, 'BB',NVL(AVG_GAP,0)) AS AVG_BB" ).append("\n"); 
		query.append("					 , CNT" ).append("\n"); 
		query.append("					 , TRET_GAP" ).append("\n"); 
		query.append("                FROM    (SELECT  S1.SPCL_CGO_CATE_CD" ).append("\n"); 
		query.append("                                --, S1.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                                , S1.C_YYYY" ).append("\n"); 
		query.append("                                , S1.C_TERM" ).append("\n"); 
		query.append("                                , COUNT(S1.SPCL_CGO_CATE_CD) AS CNT" ).append("\n"); 
		query.append("                                , SUM(TO_NUMBER(NVL(DECODE(S1.AUTH_FLG,'R','',CEIL((S1.AUTH_GDT-S1.RQST_GDT)*24)),0))) TRET_GAP" ).append("\n"); 
		query.append("								, ROUND(( (SUM(TO_NUMBER(NVL(DECODE(S1.AUTH_FLG,'R','',CEIL((S1.AUTH_GDT-S1.RQST_GDT)*24)),0)))) / (COUNT(S1.SPCL_CGO_CATE_CD)) ),1) AS AVG_GAP" ).append("\n"); 
		query.append("                        FROM (SELECT R3.SPCL_CGO_CATE_CD" ).append("\n"); 
		query.append("                                    , R3.LST_RQST_DAT_FLG" ).append("\n"); 
		query.append("                                    " ).append("\n"); 
		query.append("                                    --, RS.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                                    , TO_CHAR(R3.RQST_GDT, 'YYYY') C_YYYY" ).append("\n"); 
		query.append("                                    , DECODE(@[term], 'H', CEIL(EXTRACT(MONTH FROM R3.RQST_GDT)/6)" ).append("\n"); 
		query.append("                                                     , 'Q', CEIL(EXTRACT(MONTH FROM R3.RQST_GDT)/3)" ).append("\n"); 
		query.append("                                                     , 'M', EXTRACT(MONTH FROM R3.RQST_GDT)" ).append("\n"); 
		query.append("                                                     , 1" ).append("\n"); 
		query.append("                                      ) C_TERM" ).append("\n"); 
		query.append("                                    " ).append("\n"); 
		query.append("                                    , R3.RQST_GDT" ).append("\n"); 
		query.append("                                    , R2.AUTH_GDT" ).append("\n"); 
		...
		query.append("					    )" ).append("\n"); 
		query.append("    					#end" ).append("\n"); 
		query.append("                        GROUP BY S1.SPCL_CGO_CATE_CD" ).append("\n"); 
		query.append("                                --, S1.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                                , S1.C_YYYY" ).append("\n"); 
		query.append("                                , S1.C_TERM" ).append("\n"); 
		query.append("                        " ).append("\n"); 
		...
		query.append("               AND B.YYYY           = A.C_YYYY(+)" ).append("\n"); 
		query.append("               AND B.TERM           = A.C_TERM(+)" ).append("\n"); 
		query.append("        GROUP BY    --A.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("                      B.YYYY" ).append("\n"); 
		query.append("                    , B.D_TERM" ).append("\n"); 
		query.append("        ) D" ).append("\n"); 
		query.append("WHERE 1=1" ).append("\n"); 
		query.append("--AND C.RGN_SHP_OPR_CD = D.RGN_SHP_OPR_CD" ).append("\n"); 
		query.append("AND C.TERM = D.TERM" ).append("\n"); 
		query.append("--GROUP BY C.RGN_SHP_OPR_CD, C.TERM" ).append("\n"); 
		query.append("--ORDER BY C.RGN_SHP_OPR_CD, C.TERM" ).append("\n"); 
		query.append("GROUP BY C.TERM" ).append("\n"); 
		query.append("ORDER BY C.TERM" ).append("\n"); 

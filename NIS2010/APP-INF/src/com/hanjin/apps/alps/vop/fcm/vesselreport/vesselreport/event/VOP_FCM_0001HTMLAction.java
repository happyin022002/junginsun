...
*@FileName : VOP_FCM_0001HTMLAction.java
...
public class VOP_FCM_0001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_FCM_0001HTMLAction 객체를 생성
	 */
	public VOP_FCM_0001HTMLAction() {}
	...
	VopFcm0001Event event = new VopFcm0001Event();
	
	if (command.isCommand(FormCommand.SEARCH11)){
		...
		event.setVslDepRptErrVOs((VslDepRptErrVO[])getVOs(request, VslDepRptErrVO.class));
	}else if (command.isCommand(FormCommand.REMOVE02)){
		event.setVslDepRptOverlapVOs((VslDepRptOverlapVO[])getVOs(request, VslDepRptOverlapVO.class));
	}else{

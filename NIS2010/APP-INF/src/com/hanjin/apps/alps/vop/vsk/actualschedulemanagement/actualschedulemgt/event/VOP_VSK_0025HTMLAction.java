...
*@FileName : VOP_VSK_0025HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.hanjin.apps.nis2010.vop.vsk.actualschedulemanagement 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class VOP_VSK_0025HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_VSK_0025HTMLAction 객체를 생성
	 */
	public VOP_VSK_0025HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopVsk0025Event event = new VopVsk0025Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setYardVO((YardVO)getVO(request, YardVO .class));
			event.setActSkdMgtVO((ActSkdMgtVO)getVO(request, ActSkdMgtVO .class));
		}else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setVvdVO((VvdVO)getVO(request, VvdVO .class));
		}else if (command.isCommand(FormCommand.SEARCH10)){
			event.setVvdVO((VvdVO)getVO(request, VvdVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
//			event.setActSkdMgtVOs((ActSkdMgtVO[])getVOs(request, ActSkdMgtVO .class,"_sheet1"));
			event.setActSkdMgtVO((ActSkdMgtVO)getVO(request, ActSkdMgtVO .class));

...
public class ActualScheduleMgtDBDAOModifyVskActPortSkdUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("SET    PORT_SKD_STS_CD          = CASE WHEN @[act_arr_dt] IS NOT NULL AND @[act_brth_dt] IS NULL AND @[act_dep_dt] IS NULL THEN 'A'" ).append("\n"); 
		query.append("									   WHEN @[act_arr_dt] IS NOT NULL AND @[act_brth_dt] IS NOT NULL AND @[act_dep_dt] IS NULL THEN 'B'" ).append("\n"); 
		query.append("									   WHEN @[act_arr_dt] IS NOT NULL AND @[act_brth_dt] IS NOT NULL AND @[act_dep_dt] IS NOT NULL THEN 'D'" ).append("\n"); 
		query.append("							      END" ).append("\n"); 
		query.append("       , LST_ETA_DT             = TO_DATE(@[lst_eta_dt] , 'YYYYMMDDHH24MI')" ).append("\n"); 
		...
		query.append("                                  ELSE ACT_ATA_INP_USR_ID" ).append("\n"); 
		query.append("                                  END" ).append("\n"); 
		query.append("       , LST_ETB_DT             = TO_DATE(@[lst_etb_dt]  , 'YYYYMMDDHH24MI')" ).append("\n"); 
		...
		query.append("                                  ELSE ACT_ATB_INP_USR_ID" ).append("\n"); 
		query.append("                                  END" ).append("\n"); 
		query.append("       , LST_ETD_DT             = TO_DATE(@[lst_etd_dt],'YYYYMMDDHH24MI')" ).append("\n"); 
		...
		query.append("       , ACT_ARR_RMK            = @[act_arr_rmk]" ).append("\n"); 
		query.append("       , ACT_BRTH_RMK           = @[act_brth_rmk]" ).append("\n"); 
		query.append("       , ACT_DEP_RMK            = @[act_dep_rmk]" ).append("\n"); 

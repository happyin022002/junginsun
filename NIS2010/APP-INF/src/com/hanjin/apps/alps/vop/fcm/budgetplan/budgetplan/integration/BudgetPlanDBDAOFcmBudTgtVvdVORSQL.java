...
*@FileName : BudgetPlanDBDAOFcmBudTgtVvdVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BudgetPlanDBDAOFcmBudTgtVvdVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public BudgetPlanDBDAOFcmBudTgtVvdVORSQL(){
		setQuery();
		...
		query.append("FileName : BudgetPlanDBDAOFcmBudTgtVvdVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("NVL(T1.PF_SKD_TP_CD, (SELECT PF_SKD_TP_CD FROM VSK_BUD_VSL_SKD" ).append("\n"); 
		query.append(" WHERE VSL_SLAN_CD=SUBSTR(RLANE_CD, 1, 3)" ).append("\n"); 
		query.append(" AND VSL_CD=T1.VSL_CD" ).append("\n"); 
		query.append(" AND SKD_VOY_NO=T1.SKD_VOY_NO" ).append("\n"); 
		query.append(" AND SKD_DIR_CD=T1.SKD_DIR_CD)) PF_SKD_TP_CD," ).append("\n"); 
		...
		query.append("T2.FOIL_CSM_COST_AMT," ).append("\n"); 
		...
		query.append("T2.DOIL_CSM_COST_AMT," ).append("\n"); 
		...
		query.append("    AND TO_DATE(T1.BUD_YRMON, 'YYYY-MM') BETWEEN TO_DATE(REPLACE(@[fm_dt], '-', ''), 'YYYY-MM')     -- 화면조건 : FM_DT" ).append("\n"); 
		query.append("                                       AND TO_DATE(REPLACE(@[to_dt], '-', ''), 'YYYY-MM')+0.99999 -- 화면조건 : TO_DT" ).append("\n"); 
		query.append("#end                   " ).append("\n"); 

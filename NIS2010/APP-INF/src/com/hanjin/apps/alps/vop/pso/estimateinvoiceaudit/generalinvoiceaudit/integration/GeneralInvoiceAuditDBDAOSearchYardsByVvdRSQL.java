...
public class GeneralInvoiceAuditDBDAOSearchYardsByVvdRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("               ROW_NUMBER() OVER (PARTITION BY YD_CD" ).append("\n"); 
		query.append("                ORDER BY CLPT_IND_SEQ) NUM" ).append("\n"); 
		query.append("#if (${src_pso_bztp_cd} == '' || ${src_pso_bztp_cd} == '2') " ).append("\n"); 
		query.append("        FROM   VSK_VSL_PORT_SKD" ).append("\n"); 
		query.append("#elseif (${src_pso_bztp_cd} == '1') " ).append("\n"); 

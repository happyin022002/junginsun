...
=========================================================*/
package com.hanjin.apps.alps.vop.fcm.fcmcommonutil.fcmgeneralutil;
...
public class FCMGeneralUtil {
	...
	private static Logger log = Logger.getLogger("com.hanjin.apps.alps.vop.fcm.fcmcommonutil.fcmgeneralutil");
...
//		String tempCd = "CD00717";
	 ...
	 * "." 와 "-" 는 허용한다.<br>
	 * null 이나 blank 일 경우에는 true를 반환한다.<br>
	 *
	 * @param String str
	 * @return Boolean
	 */
	public static boolean isNumberChk( String str ) {
		...
		if (isNull(str)) {
			...
			for ( int i=0; i<str.length() ; i++ ) {
				...
				if ((ch >= 48 && ch <= 57) || (str.length() != 1 && dotCnt < 2 && ch == 46) || (str.length() != 1 && i == 0 && ch == 45)) {
					returnValue = true;
				} else {
					returnValue = false;
					break;
	...
	/**
	 * String 을 double 로 치환한다.<br>
	 * null 또는 blank 또는 숫자형이 아닐 경우에는 0을 반환한다.<br>
	 *
	 * @param String str
	 * @return double
	 */
	public static double convertDoubleByString( String str ) {
	...
	/**
	 * String을 delim 구분자로 나누어 List로 치환하여 반환한다.<br>
	 * 
	 * @param str
	 * @param delim
	 * @return List<String>
	 */
	public static List<String> convertListByString(String str, String delim) {
	...
	/**
	 * String List를 delim 구분자로 하여 String 으로 치환하여 반환한다.<br>
	 * 
	 * @param strList
	 * @param delim
	 * @return String
	 */
	public static String convertStringByList(List<String> strList, String delim) {
		StringBuffer resultStrBuf = new StringBuffer();
		
		for (int i = 0; i < strList.size(); i++) {
			resultStrBuf.append(strList.get(i)).append(delim);
		}
	...
	/**
	 * String이 0 인지 체크 한다.<br>
	 * 0 이 아닌 정상적인 숫자형 String 일 경우에는 true를 반환하고, 그 이외의 경우에는 false를 반환한다.<br>
	 * 
	 * @param str
	 * @return boolean
	 */
	public static boolean checkZeroStr(String str) {
		boolean returnVal = true;
		
		if (isNotNull(str) && isNumberChk(str)) {
			BigDecimal strVal = new BigDecimal(str);
			if (strVal.compareTo(new BigDecimal("0")) == 0) {
				returnVal = false;
			} else {
				returnVal = true;
			}
	...
	/**
	 * 숫자형 String 의 소수점 뒷자리수를 scale 크기에 맞추어 반환한다.<br>
	 * "0" 일 경우에는 scale 변환없이 그대로 "0"을 반환한다.<br>
	 * @param str
	 * @param scale
	 * @return String
	 */
	public static String convertScale(String str, int scale) {
		...
		if (isNotNull(str) && isNumberChk(str)) {
			if ("0".equals(str)) {
				...
				BigDecimal strVal = new BigDecimal(str).setScale(scale, BigDecimal.ROUND_HALF_UP);
				returnVal = strVal.toString();
			}
		} else {
			returnVal = "";
		}

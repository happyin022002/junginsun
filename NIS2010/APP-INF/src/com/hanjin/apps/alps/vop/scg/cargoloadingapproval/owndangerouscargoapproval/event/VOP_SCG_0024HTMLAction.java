...
public class VOP_SCG_0024HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_SCG_0024HTMLAction 객체를 생성
	 */
	public VOP_SCG_0024HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopScg0024Event event = new VopScg0024Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setScgNonDcgoRequestVOS((ScgNonDcgoRequestVO[])getVOs(request, ScgNonDcgoRequestVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setScgNonDcgoRequestVO((ScgNonDcgoRequestVO)getVO(request, ScgNonDcgoRequestVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH02)){
			event.setMdmVslSvcLaneVO((MdmVslSvcLaneVO)getVO(request, MdmVslSvcLaneVO .class));
		}
		else if(command.isCommand(FormCommand.MODIFY) || command.isCommand(FormCommand.SEARCH01)) {
			event.setScgNonDcgoRequestVO((ScgNonDcgoRequestVO)getVO(request, ScgNonDcgoRequestVO .class));
		}

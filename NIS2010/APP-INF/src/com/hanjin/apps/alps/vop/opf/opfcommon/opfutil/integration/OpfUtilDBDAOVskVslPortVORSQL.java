...
public class OpfUtilDBDAOVskVslPortVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("A.CLPT_SEQ, A.YD_CD, A.TURN_PORT_IND_CD" ).append("\n"); 
		query.append("FROM	(" ).append("\n"); 
		query.append("SELECT	A.VPS_PORT_CD, A.SKD_VOY_NO, A.CLPT_SEQ, A.SKD_CNG_STS_CD" ).append("\n"); 
		query.append(", A.YD_CD, A.TURN_PORT_IND_CD" ).append("\n"); 
		query.append("FROM	VSK_VSL_PORT_SKD A" ).append("\n"); 
		query.append("WHERE	A.VSL_CD		=	@[vsl_cd]" ).append("\n"); 
		query.append("AND     A.SKD_VOY_NO    =   @[skd_voy_no]" ).append("\n"); 
		query.append("AND     A.SKD_DIR_CD    =   @[skd_dir_cd]" ).append("\n"); 
		query.append("#if (${port_cd} == 'OrderBySeq')" ).append("\n"); 
		query.append("AND    A.TURN_PORT_IND_CD NOT IN ('D','V','F')" ).append("\n"); 
		query.append("#elseif (${port_cd} != '')" ).append("\n"); 

...
public class OnTimeResultAnalysisDBDAOSearchRsltOnTimeRtoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("--        VSKD_RSLT_XCLD_CD = 'A'는 Arrival 표시되지 않음란 의미로,실적 생성시 ATA를 입력" ).append("\n"); 
		query.append("--        입력하지 못하는 상태임으로 실적(COUNT)에서는 누락시킨다는 의미이다." ).append("\n"); 
		query.append("-- 2. D : Departure On-time Ratio 산출 선택시" ).append("\n"); 
		query.append("--        VSKD_RSLT_XCLD_CD = 'D'는 Departure 표시되지 않음란 의미로,실적 생성시 ATD를 입력" ).append("\n"); 
		query.append("--        입력하지 못하는 상태임으로 실적(COUNT)에서는 누락시킨다는 의미이다." ).append("\n"); 
		...
		query.append("               AND     T1.UPD_USR_ID     = @[upd_usr_id]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${crr_cd} != '')" ).append("\n"); 
		query.append("               AND     NVL(T2.ACT_CRR_CD, T3.CRR_CD)         = @[crr_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("               SELECT   T2.VSL_SLAN_CD AS GRP_ID," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                        SUM	(	CASE	WHEN T1.SKD_CNG_STS_CD	= 'S' 	OR T1.VSKD_RSLT_XCLD_CD = 'S' 	THEN 0" ).append("\n"); 
		query.append("										WHEN @[ratio_opt]		= 'D'									THEN DECODE(ACT_DEP_DT,NULL,0,1)" ).append("\n"); 
		query.append("										ELSE 1" ).append("\n"); 
		query.append("								END" ).append("\n"); 
		query.append("							) TTL_CALL" ).append("\n"); 
		query.append("					," ).append("\n"); 
		query.append("                        SUM	(	CASE	WHEN T1.SKD_CNG_STS_CD 	= 'S' 	OR T1.VSKD_RSLT_XCLD_CD = 'S' 	THEN 0" ).append("\n"); 
		query.append("								 		WHEN @[ratio_opt]		= 'D'									THEN DECODE(ACT_DEP_DT,NULL,0,1)" ).append("\n"); 
		query.append("										ELSE" ).append("\n"); 
		query.append("												DECODE(SIGN(@[ontime_opt] - DECODE	(@[delay_opt] || @[ratio_opt]" ).append("\n"); 
		query.append("																								, 'EA'" ).append("\n"); 
		query.append("																								, T1.XCLD_BRTH_DLAY_HRS" ).append("\n"); 
		query.append("																								, 'ED'" ).append("\n"); 
		query.append("																								, T1.XCLD_DEP_DLAY_HRS" ).append("\n"); 
		query.append("																								, 'IA'" ).append("\n"); 
		...
		query.append("															), -1, 0, 1)" ).append("\n"); 
		query.append("							 	END " ).append("\n"); 
		query.append("							) ONTIME_CALL" ).append("\n"); 
		query.append("               FROM     VSK_VSL_SKD_RSLT     T1," ).append("\n"); 
		query.append("                        VSK_VSL_SKD          T2," ).append("\n"); 
		query.append("                        MDM_VSL_CNTR         T3" ).append("\n"); 
		query.append("               WHERE    1 = 1" ).append("\n"); 

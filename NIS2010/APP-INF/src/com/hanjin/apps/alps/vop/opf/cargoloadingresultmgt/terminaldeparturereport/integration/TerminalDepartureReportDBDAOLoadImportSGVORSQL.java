...
public class TerminalDepartureReportDBDAOLoadImportSGVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       SUM(DECODE(DCGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,CNTR_WGT,0),0))      DG_20_WGT," ).append("\n"); 
		query.append("       SUM(DECODE(DCGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,1),0))             DG_40_QTY," ).append("\n"); 
		query.append("       SUM(DECODE(DCGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,CNTR_WGT),0))      DG_40_WGT," ).append("\n"); 
		query.append("       SUM(DECODE(RC_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,1,0),0))               RF_20_QTY," ).append("\n"); 
		query.append("       SUM(DECODE(RC_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,CNTR_WGT,0),0))        RF_20_WGT," ).append("\n"); 
		query.append("       SUM(DECODE(RC_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,1),0))               RF_40_QTY," ).append("\n"); 
		query.append("       SUM(DECODE(RC_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,CNTR_WGT),0))        RF_40_WGT," ).append("\n"); 
		query.append("       SUM(DECODE(AWK_CGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,1,0),0))          AK_20_QTY," ).append("\n"); 
		query.append("       SUM(DECODE(AWK_CGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,CNTR_WGT,0),0))   AK_20_WGT," ).append("\n"); 
		query.append("       SUM(DECODE(AWK_CGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,1),0))          AK_40_QTY," ).append("\n"); 
		query.append("       SUM(DECODE(AWK_CGO_FLG,'Y',DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),2,0,CNTR_WGT),0))   AK_40_WGT" ).append("\n"); 

...
public class VOP_PSO_0019HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {//VOP_PSO_0019 windows open시 
			...
			event.setAuditDataValidOwnerAccountVOs((AuditDataValidOwnerAccountVO[])getVOs(request, AuditDataValidOwnerAccountVO.class,"sheet3_"));//OA 비용 
...
//					vos[i].setCallSeq(vo.getCallSeq());
				...
				//	log.debug("vos[i].getLastInv()++)"+vos[i].getLastInv());
					if(!vos[i].getLastInv().equals("")&&!vos[i].getLastInv().equals("0")){
						vos[i].setDiffRmk("Before Invoice:"+vos[i].getLastInv()+" USD");
					}

...
public class VesselScheduleMgtDBDAOVvdBkgStsVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		,	CASE 	WHEN ACT_SKD_KNT > 0 THEN 'ACT SKD'" ).append("\n"); 
		query.append("					ELSE BKG_STATUS" ).append("\n"); 
		query.append("			END		BKG_STATUS" ).append("\n"); 
		query.append("		,	VIR_BKG_STATUS" ).append("\n"); 
		query.append("		,	TO_CHAR(VPS_ETA_DT, 'YYYYMMDD') START_ETA" ).append("\n"); 
		query.append("		,	CASE 	WHEN ROUND(VPS_ETA_DT - SYSDATE) < 3 AND BKG_STATUS = 'Booking' THEN 'NO'	/* VPS_ETA_DT가 시스템시간 3일 이내이고, booking이 걸려있는 VVD면 삭제불가 */" ).append("\n"); 
		query.append("					WHEN RUSE_PROHI_FLG = 'Y' 										THEN 'NO'	/* 재무VVD에 등록된 VVD면 삭제불가 */" ).append("\n"); 
		...
		query.append("	    					AND 		ROWNUM = 1" ).append("\n"); 
		query.append("							), NULL, T1.SKD_STS_CD, '1', 'BKG', '2', 'BKGNOD'" ).append("\n"); 
		query.append("					) VIR_BKG_STATUS" ).append("\n"); 

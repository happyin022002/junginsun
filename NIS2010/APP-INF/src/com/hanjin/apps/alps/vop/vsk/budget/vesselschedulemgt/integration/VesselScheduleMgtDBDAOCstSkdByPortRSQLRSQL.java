...
*@FileName : VesselScheduleMgtDBDAOCstSkdByPortRSQLRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class VesselScheduleMgtDBDAOCstSkdByPortRSQLRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  *                             기존 VSK_VSL_SKD, VSK_VSL_PORT_SKD 테이블을 아래 BUD가 추가된 테이블로 변경함.
	  * </pre>
	  */
	public VesselScheduleMgtDBDAOCstSkdByPortRSQLRSQL(){
		setQuery();
		...
		query.append("FileName : VesselScheduleMgtDBDAOCstSkdByPortRSQLRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("					, NVL(C1.CRR_CD, T3.CRR_CD) AS CARRIER_CD--CRR_CD를 가져오기 위해 변경 2014.07.14 이용준" ).append("\n"); 
		...
		query.append("                                		'SINWASHAAS', 'SINWA', SUBSTR(LTRIM(MAX(TO_CHAR(LEVEL, '000')||OFC_CD)), 4)" ).append("\n"); 
		query.append("                                		)" ).append("\n"); 
		query.append("                    		-- 1. 2건에 RHQ Code가 발생할 경우, Level이 낮은 것을 기준으로 하고," ).append("\n"); 
		query.append("                    		-- 2, 'SINWASHAAS'일 경우에는 'SINWA'로 표시하고, 일반적으로는 Level에 Max를 표시한다." ).append("\n"); 

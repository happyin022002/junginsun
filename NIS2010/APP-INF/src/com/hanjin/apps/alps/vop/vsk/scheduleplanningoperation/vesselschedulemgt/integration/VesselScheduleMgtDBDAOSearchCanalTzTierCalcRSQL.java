...
public class VesselScheduleMgtDBDAOSearchCanalTzTierCalcRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		, BOX AS SCG_CAR_BOX" ).append("\n"); 
		...
		query.append("									--// A2,D2,D3,F2,O2,P2,R2,S2,T2,A4,D4,D5,F4,F5,O5,O4,P4,R9,R5,S4,T4,D7,D9,DX //--  " ).append("\n"); 
		query.append("									, SUM	(	CASE	WHEN T1.SZTP				IN ('DX')			THEN 2.25  " ).append("\n"); 
		query.append("														WHEN SUBSTR(T1.SZTP,2,1)	IN ('5','7','9')	THEN 2.25  " ).append("\n"); 
		query.append("														WHEN SUBSTR(T1.SZTP,2,1)	IN ('4')			THEN 2 " ).append("\n"); 
		query.append("														ELSE 1  " ).append("\n"); 
		query.append("												END  " ).append("\n"); 
		query.append("											) AS TEU " ).append("\n"); 
		query.append("									, SUM	(	CASE	WHEN T1.SZTP				IN ('DX')			THEN 1  " ).append("\n"); 
		query.append("														WHEN SUBSTR(T1.SZTP,2,1)	IN ('5','7','9','4')THEN 1  														" ).append("\n"); 
		query.append("														ELSE 1  " ).append("\n"); 
		query.append("												END  " ).append("\n"); 
		...
		query.append("				GROUP BY VSL_CD, VOY_NO, DIR_CD, PORT_CD, CALL_IND, TIER_NO	,BOX" ).append("\n"); 
		query.append("				)" ).append("\n"); 

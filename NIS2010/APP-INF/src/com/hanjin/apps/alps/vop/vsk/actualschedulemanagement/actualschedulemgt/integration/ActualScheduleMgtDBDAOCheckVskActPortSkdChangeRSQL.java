...
public class ActualScheduleMgtDBDAOCheckVskActPortSkdChangeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("/*" ).append("\n"); 
		query.append("* GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',...) 에서 'KRPUS' 임창빈 수석이 하드코딩하기로 함." ).append("\n"); 
		query.append("*/" ).append("\n"); 
		...
		query.append(", TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', NVL(T22.ACT_ATA_INP_DT, SYSDATE), NVL(T22.VPS_PORT_CD, @[vps_port_cd])), 'YYYYMMDDHH24MI') AS ATA_LOC_TIME" ).append("\n"); 
		query.append(", TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', NVL(T22.ACT_ATB_INP_DT, SYSDATE), NVL(T22.VPS_PORT_CD, @[vps_port_cd])), 'YYYYMMDDHH24MI') AS ATB_LOC_TIME" ).append("\n"); 
		query.append(", TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', NVL(T22.ACT_ATD_INP_DT, SYSDATE), NVL(T22.VPS_PORT_CD, @[vps_port_cd])), 'YYYYMMDDHH24MI') AS ATD_LOC_TIME" ).append("\n"); 

 ...
 * 2013.02.27 CHM-201323017 Date Compare Method(compareSkdDate) 로직수정 - parameter 빈값체크
 * 2015.08.10 이병훈 [CHM-201536635] Split11-2015년 소스 보안 품질 3단계 중 4차 작업 진행 요청
=========================================================*/
package com.hanjin.apps.alps.vop.vsk.vskcommonutil.vskgeneralutil;
...
public class VSKGeneralUtil {
	...
	private static Logger log = Logger.getLogger("com.hanjin.apps.scheduleplanningoperation.vesselschedulemgt");
	...
	public static int compareSkdDate(String date1, String format1, String date2, String format2) throws EventException {
		...
		try{
			...
			if("".equals(date1) || "".equals(date2)){
				return 0;
			}else if(d1 != null && d1.before(d2)){
				return -1;
			}else if(d1 != null && d1.after(d2)){
				return 1;
			}else{
				return 0;
			}
...
//		String tempCd = "CD00717";
	 ...
	 * @return String
	 */
	public static String convertNulltoEmptyString(String sInputString){
		sInputString	= sInputString == null ? "" : sInputString;
	 ...
	 * @return boolean
	 */	
	public static boolean isEffectiveDate(String sInputDate){
		...
		try{
			...
			if(sInputDate == null)	return false;
			iInputDateLen	= sInputDate.length();
			
			if(iInputDateLen == 14){
				sDateFormat	= "yyyyMMddHHmmss";
			}else if(iInputDateLen == 12){
			...
			sInputDate		= replaceDateTypeToString(sInputDate);

...
public class VesselScheduleMgtDBDAOCstSkdByPortRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * 
	  * * History 
	  * 2012.07.10 이혜민  CHM-201218616-01 Port SKD Inquiry 에 PFS 대비 지연 시간 컬럼 추가
	...
	public void setQuery(){
		...
		query.append("--					,   S.AMP_TP_CD        AS SPEC_AMP" ).append("\n"); 
		query.append("--					,   DT.AMP_TP_CD        AS COND_AMP" ).append("\n"); 
		...
		query.append("				    ,   VSK_VSL_ADD_SPEC        S" ).append("\n"); 
		query.append("#if (${usr_def_grp_nm} != '')" ).append("\n"); 
		query.append("					,   (  SELECT  DECODE(D.VSL_SLAN_CD, ' ', NULL, D.VSL_SLAN_CD) AS VSL_SLAN_CD" ).append("\n"); 
		query.append("                                 , DECODE(D.DIR_CD, ' ', NULL, D.DIR_CD)    AS DIR_CD" ).append("\n"); 
		query.append("                                 , DECODE(D.PORT_CD, ' ', NULL, D.PORT_CD)  AS PORT_CD" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${usr_def_grp_nm} != '')" ).append("\n"); 
		query.append("			AND     ((T2.SLAN_CD,T2.SKD_DIR_CD, T2.VPS_PORT_CD, DECODE(DT.AMP_TP_CD, 'N', 'X', S.AMP_TP_CD))  IN (" ).append("\n"); 
		query.append("                                                       SELECT  D.VSL_SLAN_CD" ).append("\n"); 
		query.append("                                                             , D.DIR_CD" ).append("\n"); 
		query.append("                                                             , D.PORT_CD" ).append("\n"); 
		query.append(" 															 , DECODE(D.AMP_TP_CD, 'N', 'X', D.AMP_TP_CD)" ).append("\n"); 
		query.append("                                                       FROM    VSK_USR_DEF_COND_HDR  H" ).append("\n"); 
		query.append("                                                             , VSK_USR_DEF_COND_DTL  D" ).append("\n"); 
		query.append("                                                       WHERE   H.USR_ID              = D.USR_ID" ).append("\n"); 
		query.append("                                                       AND     H.USR_DEF_GRP_NM      = D.USR_DEF_GRP_NM" ).append("\n"); 
		...
		query.append("                                                       AND     H.USE_PGM_NM          = @[use_pgm_nm]   			/* Binding */" ).append("\n"); 
		query.append("                                                       AND     H.USR_DEF_GRP_NM      = @[usr_def_grp_nm]        /* Binding */" ).append("\n"); 
		query.append("                                                       AND     D.VSL_SLAN_CD         <> ' '" ).append("\n"); 
		query.append("					)			        " ).append("\n"); 
		query.append("					OR(T2.SLAN_CD,T2.SKD_DIR_CD, DECODE(DT.AMP_TP_CD, 'N', 'X', S.AMP_TP_CD))  IN (" ).append("\n"); 
		query.append("                                                       SELECT  D.VSL_SLAN_CD" ).append("\n"); 
		query.append("                                                             , D.DIR_CD" ).append("\n"); 
		query.append(" 															 , DECODE(D.AMP_TP_CD, 'N', 'X', D.AMP_TP_CD)" ).append("\n"); 
		query.append("                                                       FROM    VSK_USR_DEF_COND_HDR  H" ).append("\n"); 
		...
		query.append("                                                       AND     H.USR_DEF_GRP_NM      = @[usr_def_grp_nm]        /* Binding */" ).append("\n"); 
		query.append("                                                       AND     D.VSL_SLAN_CD         <> ' '" ).append("\n"); 
		query.append("                                                       AND     D.PORT_CD             = ' '" ).append("\n"); 
		query.append("					)" ).append("\n"); 
		query.append("					OR(T2.VPS_PORT_CD, DECODE(DT.AMP_TP_CD, 'N', 'X', S.AMP_TP_CD))  IN (" ).append("\n"); 
		query.append("                                                       SELECT  D.PORT_CD" ).append("\n"); 
		query.append("                                                             , DECODE(D.AMP_TP_CD, 'N', 'X', D.AMP_TP_CD)" ).append("\n"); 
		query.append("                                                       FROM    VSK_USR_DEF_COND_HDR  H" ).append("\n"); 
		query.append("                                                             , VSK_USR_DEF_COND_DTL  D" ).append("\n"); 
		query.append("                                                       WHERE   H.USR_ID              = D.USR_ID" ).append("\n"); 

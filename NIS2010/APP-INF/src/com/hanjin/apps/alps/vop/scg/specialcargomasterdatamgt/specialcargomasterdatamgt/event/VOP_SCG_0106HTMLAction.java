...
public class VOP_SCG_0106HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_AOM_0106HTMLAction 객체를 생성
	 */
	public VOP_SCG_0106HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopScg0106Event event = new VopScg0106Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setScgPckReguPkgOrgPrxVOS((ScgPckReguPkgOrgPrxVO[])getVOs(request, ScgPckReguPkgOrgPrxVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setScgPckReguPkgOrgPrxVO((ScgPckReguPkgOrgPrxVO)getVO(request, ScgPckReguPkgOrgPrxVO .class));

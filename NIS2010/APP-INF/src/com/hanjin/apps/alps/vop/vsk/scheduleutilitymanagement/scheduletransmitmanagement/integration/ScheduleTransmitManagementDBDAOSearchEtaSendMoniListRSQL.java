...
*@FileName : ScheduleTransmitManagementDBDAOSearchEtaSendMoniListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ScheduleTransmitManagementDBDAOSearchEtaSendMoniListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ETA Sending(Auto FAX/TLX) 내역을 조회합니다.
	  * </pre>
	  */
	public ScheduleTransmitManagementDBDAOSearchEtaSendMoniListRSQL(){
		setQuery();
		...
		query.append("FileName : ScheduleTransmitManagementDBDAOSearchEtaSendMoniListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    " ).append("\n"); 
		query.append("     , ROUND(T.STW_DIF_HRS*24,2)     AS STW_DIF_HRS" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("         --Adding:2014-01-02--" ).append("\n"); 
		query.append("     , TO_CHAR(T.PLT_IN_DT          ,'YYYY-MM-DD HH24:MI') AS POB_DT" ).append("\n"); 
		query.append("     , TO_CHAR(T.BFR_BRTH_ANK_DRP_DT,'YYYY-MM-DD HH24:MI') AS DRP_ANK_DT" ).append("\n"); 
		query.append("     , T.DIF_CST_ACT_ARR_DT" ).append("\n"); 
		query.append("     , T.DIF_CST_ACT_BRTH_DT" ).append("\n"); 
		query.append("     , T.DIF_CST_NTC_ARR_DT" ).append("\n"); 
		query.append("     , T.DIF_CST_NTC_BRTH_DT" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("				 --Adding:2014-01-02--" ).append("\n"); 
		query.append("            ,    RT.PLT_IN_DT                                   " ).append("\n"); 
		query.append("            ,    RT.BFR_BRTH_ANK_DRP_DT     " ).append("\n"); 
		query.append("            ,    ROUND(NVL(H.VPS_ETA_DT - AK.ACT_ARR_DT ,0)*24,1) AS DIF_CST_ACT_ARR_DT" ).append("\n"); 
		query.append("            ,    ROUND(NVL(H.VPS_ETB_DT - AK.ACT_BRTH_DT,0)*24,1) AS DIF_CST_ACT_BRTH_DT" ).append("\n"); 
		query.append("            ,    ROUND(NVL(H.VPS_ETA_DT - H.NTC_ETA_DT  ,0)*24,1) AS DIF_CST_NTC_ARR_DT" ).append("\n"); 
		query.append("            ,    ROUND(NVL(H.VPS_ETB_DT - H.NTC_ETB_DT  ,0)*24,1) AS DIF_CST_NTC_BRTH_DT" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("             ,   FCM_DEP_RPT                                     RT" ).append("\n"); 
		query.append("    WHERE        1 = 1       " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("--    AND          PS.VSL_CD                                       = H.VSL_CD" ).append("\n"); 
		...
		query.append("--    AND          PS.CLPT_IND_SEQ                                 = H.CLPT_IND_SEQ" ).append("\n"); 
		query.append("                             " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("    AND          H.VSL_CD                                        = RT.VSL_CD           (+)" ).append("\n"); 
		query.append("    AND          H.SKD_VOY_NO                                    = RT.SKD_VOY_NO       (+)" ).append("\n"); 
		query.append("    AND          H.SKD_DIR_CD                                    = RT.SKD_DIR_CD       (+)" ).append("\n"); 
		query.append("    AND          H.VPS_PORT_CD                                   = RT.DEP_PORT_CD      (+)" ).append("\n"); 
		query.append("    AND          H.CLPT_IND_SEQ                                  = RT.CLPT_IND_SEQ     (+)" ).append("\n"); 
		...
		query.append("    #end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if (${trsm_purp_cd} == 'STW')" ).append("\n"); 
		query.append("    	#if (${dif_over_arr_dt} != '')" ).append("\n"); 
		query.append("    	AND          ROUND(NVL(AK.ACT_BRTH_DT - H.TRSM_LOCL_DT,0)*24,1) <= TO_NUMBER(@[dif_over_arr_dt])" ).append("\n"); 
		query.append("    	#end" ).append("\n"); 
		query.append("	#else" ).append("\n"); 
		query.append("    	#if (${dif_over_arr_dt} != '')" ).append("\n"); 

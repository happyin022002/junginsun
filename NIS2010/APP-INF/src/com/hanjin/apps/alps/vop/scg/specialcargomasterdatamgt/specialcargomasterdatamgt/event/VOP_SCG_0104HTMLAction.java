...
public class VOP_SCG_0104HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_AOM_0106HTMLAction 객체를 생성
	 */
	public VOP_SCG_0104HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopScg0104Event event = new VopScg0104Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setScgPckReguPkgCdVOS((ScgPckReguPkgCdVO[])getVOs(request, ScgPckReguPkgCdVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setScgPckReguPkgCdVO((ScgPckReguPkgCdVO)getVO(request, ScgPckReguPkgCdVO .class));

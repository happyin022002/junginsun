...
*@FileName : VOP_VSK_9515HTMLAction.java
 ...
 * @author Jeong Un
 * @see VOP_VSK_9515Event 참조
 * @since J2EE 1.6
 */

public class VOP_VSK_9515HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_VSK_9515HTMLAction 객체를 생성
	 */
	public VOP_VSK_9515HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 VOP_VSK_9515Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopVsk9515Event event = new VopVsk9515Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setYardGroupVO((YardGroupVO)getVO(request, YardGroupVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setYardGroupVO((YardGroupVO)getVO(request, YardGroupVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setYardGroupVOs((YardGroupVO[])getVOs(request, YardGroupVO .class));
		}

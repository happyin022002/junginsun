...
*@FileName : COM_CSR_0001HTMLAction.java
...
public class COM_CSR_0001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * COM_CSR_0001HTMLAction 객체를 생성
	 */
	public COM_CSR_0001HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		ComCsr0001Event event = new ComCsr0001Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setApPayInvListVO((ApPayInvListVO)getVO(request, ApPayInvListVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setCheckAsaVO((CheckAsaVO)getVO(request, CheckAsaVO .class));
		}

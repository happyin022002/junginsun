...
public class ConsultationSlipRequestMgtEAIDAO extends EAIDAOSupport {
	...
	public FNS0080003Document[] transferAtOnceCSR024ToEAIByWS(DBRowSet rowSet, String csrNo, ComAproRqstRoutVO comAproRqstRoutVO) throws DAOException, EventException {
		...
		try {
			...
			if(rowSet != null) {
				...
				String agmtCfmCd = JSPUtil.getNull(new ApprovalUtil().searchAgmtCfmCd(csrNo));
				...
				for( int i=0; i<docReq.length; i++){
					...
					while (cnt <= 100 && rowSet.next()) {
			            ...
			            setInvoiceDatas(rowSet, invoiceReq, comAproRqstRoutVO, agmtCfmCd);
			            cnt++;
					}
				}
	 ...
	 * @exception Exception
	 */
	private void setInvoiceDatas(DBRowSet rowSet, APInvoiceRequest invoiceReq, ComAproRqstRoutVO comAproRqstRoutVO, String agmtCfmCd) throws Exception {
	      /**
		...
		try {
			if (rowSet != null) {
				...
				log.error(">>>>>>> ConsultationSlipRequestMgtEAIDAO setInvoiceDatas HINTERFACEDATE : "+ rowSet.getString("HDR_IF_DT")+"<<<<<<<<");

 ...
 * @since J2EE 1.4
 */
public class COM_ENS_0S4HTMLAction extends HTMLActionSupport {

	/**
	 * COM_ENS_0S1HTMLAction 객체를 생성
	 */
	public COM_ENS_0S4HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		String cnt_cd = JSPUtil.getParameter(request, "cnt_cd_month".trim(), "");
		String year = JSPUtil.getParameter(request, "year".trim(), "");
    	
    	int iPage           = JSPUtil.getParameterAsInt(request, "iPage", 1);  
    	
    	FormCommand f_cmd = FormCommand.fromRequest(request);
    	
    	ComEns0S4Event event = new ComEns0S4Event();
    	
		if (f_cmd.isCommand(FormCommand.SEARCHLIST)) {
	        event = new ComEns0S4Event(conti_cd, cnt_cd, year, iPage);
		}

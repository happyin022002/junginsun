 ...
 * @since J2EE 1.4
 */
public class COM_ENS_0S3HTMLAction extends HTMLActionSupport {

	/**
	 * COM_ENS_0S1HTMLAction 객체를 생성
	 */
	public COM_ENS_0S3HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		String cnt_cd = JSPUtil.getParameter(request, "cnt_cd_loc".trim(), "");
		String ste_cd = JSPUtil.getParameter(request, "ste_cd_loc".trim(), "");
		String loc_cd = JSPUtil.getParameter(request, "loc_cd_loc".trim(), "");
    	String frDate = JSPUtil.getParameter(request, "frDate_loc".trim(), "");
    	String toDate = JSPUtil.getParameter(request, "toDate_loc".trim(), "");
    	...
    	ComEns0S3Event event = new ComEns0S3Event();
    	
		if (f_cmd.isCommand(FormCommand.SEARCHLIST)) {
	        event = new ComEns0S3Event(cnt_cd, ste_cd, loc_cd, frDate, toDate, iPage);
		} 
		else if (f_cmd.isCommand(FormCommand.MULTI)) { // Save 버튼 클릭 시
			event.setSte_cd(ste_cd);
			event.setLoc_cd(loc_cd);
			...
			event.putValue("loc_cd", 		  	request.getParameterValues("loc_cd"));

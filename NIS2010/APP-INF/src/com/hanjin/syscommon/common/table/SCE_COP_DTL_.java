...
*@FileTitle : Internal User가 검색조건 또는 Container No 입력 후 관련 정보를 조회하여 Exception 대상 선택을 통해 Exception 하나 또는 여러 건을 일괄 등록
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.4
 */public final class SCE_COP_DTL_ implements java.io.Serializable {
    ...
    public String           getVvd_no          (){    return vvd_no              ;    }
    ...
    public void setVvd_no          ( String           vvd_no           ){    this.vvd_no           = vvd_no              ;    }

    public static SCE_COP_DTL_ fromRequest(HttpServletRequest request) {
        SCE_COP_DTL_ model = new SCE_COP_DTL_();
        try {
    ...
    public static Collection fromRequestGrid(HttpServletRequest request, String prefix) {
        SCE_COP_DTL_ model = null;
        Collection models = new ArrayList();
        int length = request.getParameterValues("dtcheck").length;
        ...
        try {
            ...
            String[] vvd_no           =  (JSPUtil.getParameter(request, prefix + "vvd_no                  ".trim(), length));
            
            for (int i = 0; i < length; i++) {
                model = new SCE_COP_DTL_();
//                System.out.println("ibflag  : " + ibflag[i]);
                model.setIbflag                    ( ibflag              [i]);
                model.setPage_rows                 ( page_rows           [i]);
                model.setCop_no                    ( cop_no              [i]);
                model.setCop_grp_seq               ( cop_grp_seq         [i]);

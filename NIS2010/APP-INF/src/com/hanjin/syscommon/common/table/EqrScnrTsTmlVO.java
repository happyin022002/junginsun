...
*@FileName : EqrScnrTsTmlVO.java
*@FileTitle : EqrScnrTsTmlVO
*Open Issues :
*Change history :
...
public class EqrScnrTsTmlVO extends AbstractValueObject {

	private static final long serialVersionUID = 1L;
	
	private Collection<EqrScnrTsTmlVO> models = new ArrayList<EqrScnrTsTmlVO>();
	...
	public EqrScnrTsTmlVO() {}

	public EqrScnrTsTmlVO(String ibflag, String pagerows, String scnrId, String eccCd, String fmSlanCd, String toSlanCd, String laneDirCd, String fmYdCd, String toYdCd, String ts20ftUcAmt, String ts40ftUcAmt, String ts45ftUcAmt, String deltFlg, String creUsrId, String creDt, String updUsrId, String updDt) {
		this.updDt = updDt;
		this.scnrId = scnrId;
	 ...
	 * @param request
	 * @return EqrScnrTsTmlVO[]
	 */
	public EqrScnrTsTmlVO[] fromRequestGrid(HttpServletRequest request) {
	 ...
	 * @return EqrScnrTsTmlVO[]
	 */
	public EqrScnrTsTmlVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		EqrScnrTsTmlVO model = null;
		...
		return getEqrScnrTsTmlVOs();
	}
	
	/**
	 * Grid Sheet1번에 해당하는 화면에서 사용됩니다.
	 */
	public EqrScnrTsTmlVO[] fromRequestGrid1(HttpServletRequest request, String prefix) {
		EqrScnrTsTmlVO model = null;
		int length = request.getParameterValues("ibflag1").length;
		
		try {
			String[] ibflag1        =  (JSPUtil.getParameter(request, prefix + "ibflag1", length));
			String[] scnr_id1       =  (JSPUtil.getParameter(request, prefix + "scnr_id1", length));
			String[] ecc_cd1        =  (JSPUtil.getParameter(request, prefix + "ecc_cd1", length));
			String[] fm_slan_cd     =  (JSPUtil.getParameter(request, prefix + "fm_slan_cd", length));
			String[] to_slan_cd     =  (JSPUtil.getParameter(request, prefix + "to_slan_cd", length));
			String[] lane_dir_cd    =  (JSPUtil.getParameter(request, prefix + "lane_dir_cd", length));
			String[] fm_yd_cd       =  (JSPUtil.getParameter(request, prefix + "fm_yd_cd", length));
			String[] to_yd_cd       =  (JSPUtil.getParameter(request, prefix + "to_yd_cd", length));
			String[] ts_20ft_uc_amt =  (JSPUtil.getParameter(request, prefix + "ts_20ft_uc_amt", length));
			String[] ts_40ft_uc_amt =  (JSPUtil.getParameter(request, prefix + "ts_40ft_uc_amt", length));
			String[] ts_45ft_uc_amt =  (JSPUtil.getParameter(request, prefix + "ts_45ft_uc_amt", length));
			for (int i = 0; i < length; i++) {
				model = new EqrScnrTsTmlVO();
				model.setIbflag        	  ( ibflag1        	[i]);
				model.setScnrId       	  ( scnr_id1       	[i]);
				model.setEccCd       		  ( ecc_cd1        	[i]);
				model.setFmSlanCd    		  ( fm_slan_cd    	[i]);
				model.setToSlanCd    		  ( to_slan_cd    	[i]);
				model.setLaneDirCd   		  ( lane_dir_cd   	[i]);
				model.setFmYdCd      		  ( fm_yd_cd      	[i]);
				model.setToYdCd      		  ( to_yd_cd      	[i]);
				model.setTs20ftUcAmt		  ( ts_20ft_uc_amt	[i]);
				model.setTs40ftUcAmt		  ( ts_40ft_uc_amt	[i]);
				model.setTs45ftUcAmt		  ( ts_45ft_uc_amt	[i]);
				models.add(model);
			}
		} catch (Exception ex) {
			return null;
	...
	public EqrScnrTsTmlVO[] getEqrScnrTsTmlVOs(){
		EqrScnrTsTmlVO[] vos = (EqrScnrTsTmlVO[])models.toArray(new EqrScnrTsTmlVO[models.size()]);
		return vos;
	}

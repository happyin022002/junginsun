...
public class BkgBlMkDescVO extends AbstractValueObject {
	...
	/* Column Info */
	private String autoCluzDpCd = null;
	/* Page Number */
	private String pagerows = null;
	
	/* Column Info */
	private String crLfTabOffMkDesc = null;
	/* Column Info */
	private String crLfTabOffCmdtDesc = null;
	...
	public BkgBlMkDescVO(String ibflag, String pagerows, String bkgNo, String bkgNoSplit, String mkSeq, String mkDesc, String cmdtDesc, String creUsrId, String creDt, String updUsrId, String updDt, String autoCluzDpCd) {
		...
		this.bkgNo = bkgNo;
		this.autoCluzDpCd = autoCluzDpCd;
	...
	public HashMap<String, String> getColumnValues(){
		...
		this.hashColumns.put("auto_cluz_dp_cd", getAutoCluzDpCd());
	...
	public HashMap<String, String> getFieldNames(){
		...
		this.hashFields.put("auto_cluz_dp_cd", "autoCluzDpCd");
	...
	public String getAutoCluzDpCd() {
		return this.autoCluzDpCd;
	}
	public String getPagerows() {
		return this.pagerows;
	}
	
	public String getCrLfTabOffMkDesc() {
		this.crLfTabOffMkDesc = this.mkDesc;

		this.crLfTabOffMkDesc = getOffCrLfTab(this.crLfTabOffMkDesc);
		return this.crLfTabOffMkDesc;
	}
	
	public String getCrLfTabOffCmdtDesc() {
		this.crLfTabOffCmdtDesc = this.cmdtDesc;

		this.crLfTabOffCmdtDesc = getOffCrLfTab(this.crLfTabOffCmdtDesc);
		return this.crLfTabOffCmdtDesc;
	}
	
	
	private String getOffCrLfTab(String strVal) {
		StringBuffer sbb = new StringBuffer();
		String regexTab = "\t";
		Pattern tabP = Pattern.compile(regexTab);
		Matcher mTab = tabP.matcher(strVal);
		StringBuffer tabSb = new StringBuffer();
		boolean tabResult = mTab.find();
		// 패턴과 일치하는 문자열을 ‘ ’으로 교체해가며
		// 새로운 문자열을 만든다.
		while(tabResult) {
			mTab.appendReplacement(tabSb, " ");
			tabResult = mTab.find();
		}
		mTab.appendTail(tabSb);
		 
		String regex = "\r\n|\r|\n";             
		Pattern p = Pattern.compile(regex);
		Matcher mm = p.matcher(tabSb);
		boolean resultbb = mm.find();
		// 패턴과 일치하는 문자열을 "\r\n"으로 교체해가며
		// 새로운 문자열을 만든다.
		while(resultbb) {
			mm.appendReplacement(sbb, "\r\n");
			resultbb = mm.find();
		}
		mm.appendTail(sbb); 
	...
	public void setAutoCluzDpCd(String autoCluzDpCd) {
		this.autoCluzDpCd = autoCluzDpCd;
		//this.bkgNo=true;
	}
	...
	public void fromRequest(HttpServletRequest request) {
		...
		setAutoCluzDpCd(JSPUtil.getParameter(request, "auto_cluz_dp_cd", ""));
	...
	public BkgBlMkDescVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		...
		try {
			...
			String[] autoCluzDpCd = (JSPUtil.getParameter(request, prefix	+ "auto_cluz_dp_cd".trim(), length));
			...
			for (int i = 0; i < length; i++) {
				...
				if (autoCluzDpCd[i] != null)
					model.setAutoCluzDpCd(autoCluzDpCd[i]);
				if (pagerows[i] != null)
	...
	*/
	public void onDataFormat(){
		...
		this.autoCluzDpCd = this.autoCluzDpCd .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");

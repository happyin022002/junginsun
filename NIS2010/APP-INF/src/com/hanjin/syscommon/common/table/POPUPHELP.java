 ...
 * 2. 처리개요 : <p> - 모든 업무에서 공통으로 사용하는 PDTO(Data Transfer Object including Parameters) 
 * <p> - 관련 Event에서 작성, 서버실행요청시 PDTO의 역할을 수행하는 Value Object <p> 
 * 3. 주의사항 : <p>
 * @author  : 박남준
 * @version  : v1.0.0
 ...
 */
public final class POPUPHELP implements java.io.Serializable {
    ...
    /**
	 * @return  Returns the codeGubun.
	 * @uml.property  name="codeGubun"
	 */
    public String    getCodeGubun(){    return codeGubun    ;    }
    /**
	 * @return  Returns the selGubun.
	 * @uml.property  name="selGubun"
	 */
    ...
    /**
	 * @param codeGubun  The codeGubun to set.
	 * @uml.property  name="codeGubun"
	 */
    public void setCodeGubun( String    codeGubun ){    this.codeGubun = codeGubun    ;    }
    /**
    ...
    public static POPUPHELP fromRequest(HttpServletRequest request) {
        POPUPHELP model = new POPUPHELP();
        try {
            model.setCodeGubun    (JSPUtil.getParameter(request, "codeGubun        ".trim(), ""));
            model.setSelGubun     (JSPUtil.getParameter(request, "selGubun         ".trim(), ""));
    ...
    public static Collection fromRequest(HttpServletRequest request, int length) {
        POPUPHELP model = null;
        Collection models = new ArrayList();
        try {
            String[] codeGubun =  (JSPUtil.getParameter(request, "codeGubun        ".trim(), length));
            String[] selGubun  =  (JSPUtil.getParameter(request, "selGubun         ".trim(), length));
            ...
            for (int i = 0; i < length; i++) {
                model = new POPUPHELP();
                model.setCodeGubun          (codeGubun    [i]);
                model.setSelGubun           (selGubun     [i]);

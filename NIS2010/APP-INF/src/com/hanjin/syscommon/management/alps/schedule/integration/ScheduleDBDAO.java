 ...
 * 1.0 최초 생성
=========================================================*/
package com.hanjin.syscommon.management.alps.schedule.integration;
...
import com.hanjin.syscommon.management.alps.schedule.event.ScheduleLogEvent;
...
public class ScheduleDBDAO extends DBDAOSupport {
	...
	public String getDirectExecuteJobID(String jobName) throws DAOException {
		...
		try {
			if(jobName.startsWith("T_")){
				rs = new SQLExecuter("Autosys_T").executeQuery((ISQLTemplate) new ScheduleDAODirectRunNumRSQL(), param, null);
			}else{
	...
	public String getReserveExecuteJobID(String jobName) throws DAOException {
		...
		try {
			while ( true ) {
				if(jobName.startsWith("T_")){
					rs = new SQLExecuter("Autosys_T").executeQuery((ISQLTemplate) new ScheduleDAOReserveEOIDRSQL(), param, null);
				}else{
	...
	public DBRowSet searchscheduleLogList(ScheduleLogEvent event) throws DAOException, Exception {
		...
		try {
			...
			if("T".equals(event.getMachine())){
				dbRowset = new SQLExecuter("Autosys_T").executeQuery((ISQLTemplate) new ScheduleLogDAOComScheduleLogRSQL(), param, velParam);
			}else{
	...
	public int getJobStatus(String jobName) throws DAOException {
		...
		try {
			if(jobName.startsWith("T_")){
				rs = new SQLExecuter("Autosys_T").executeQuery((ISQLTemplate) new ScheduleDAOJobStatusRSQL(), param, null);
			}else{
	...
	public HashMap<String,String> getJobRunsInfo(String id, String jobName) throws DAOException {
		...
		try {
			if(jobName.startsWith("T_")){
				dbRowset = new SQLExecuter("Autosys_T").executeQuery((ISQLTemplate) new ScheduleDAOJobRunInfoRSQL(), param, null);
			}else{

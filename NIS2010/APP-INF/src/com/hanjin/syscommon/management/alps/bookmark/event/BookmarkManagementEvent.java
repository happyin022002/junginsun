...
* 1.0 최초 생성 
=========================================================*/
package com.hanjin.syscommon.management.alps.bookmark.event;
...
public class BookmarkManagementEvent extends EventSupport {
	...
	public ComBookmarkVO[] getComBookmarkS(){
		...
		if(this.combookmarks != null){
			rtnVOs = Arrays.copyOf(this.combookmarks, this.combookmarks.length);
		}
		return rtnVOs;
	}
	
	public void setComBookmarkS(ComBookmarkVO[] vos) {
		if(vos != null){
		ComBookmarkVO[] tempVOs = Arrays.copyOf(vos, vos.length);
		this.combookmarks = tempVOs;
		}
	}

	public void setCombookmark(ComBookmarkVO combookmark) {
		this.combookmark = combookmark;
	}

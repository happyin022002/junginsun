...
public class UserDAOComUserVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   DECODE((SELECT COUNT(USR_ROLE_CD) FROM COM_USR_ROLE_MTCH WHERE COM_USR_ROLE_MTCH.USR_ID = upper(@[usr_id])  AND ROWNUM = 1),0,'X','O') AS JB_ENG_NM,	" ).append("\n"); 
		...
		query.append("   TO_CHAR(a.lst_lgin_dt,'YYYYMMDD') lst_lgin_dt," ).append("\n"); 
		query.append("   (SELECT USE_FLG FROM COM_APP_BASE_MENU_VIEW WHERE USR_ID = A.USR_ID) AS VIEW_FLG              " ).append("\n"); 
		...
		query.append(" AND e.SREP_STS_CD(+) = 'N'" ).append("\n"); 
		query.append("#if(${auth_cd} == 'S' && ${RHQ} != 'SELHO')" ).append("\n"); 
		...
		query.append(" AND (a.usr_locl_nm LIKE '%'||@[usr_nm]||'%' OR a.usr_nm LIKE '%'||@[usr_nm]||'%' )" ).append("\n"); 
		query.append("#end" ).append("\n"); 

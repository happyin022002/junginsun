...
*@FileName : ADM_SYS_0019HTMLAction.java
 ...
 * @author KIM, Sang-Soo
 * @see AdmSys0019Event 참조
 * @since J2EE 1.6
 */
public class ADM_SYS_0019HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	* ADM_SYS_0019HTMLAction 객체를 생성
	*/
	public ADM_SYS_0019HTMLAction() {}

	/**
	* HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	* HttpRequst의 정보를 AdmSys0019Event로 파싱하여 request에 셋팅<br>
	...
	*/
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		AdmSys0019Event event = new AdmSys0019Event();

		if (command.isCommand(FormCommand.SEARCH)) {
			event.setRequestVO((RequestVO)getVO(request, RequestVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {    // Approval / Reject 
			event.setAdjustmentVO((AdjustmentVO)getVO(request, AdjustmentVO.class));
			event.setRequestVOs((RequestVO[])getVOs(request, RequestVO.class, ""));
		}
		return  event;
	}

...
import com.hanjin.syscommon.common.table.COM_USR_ROLE;
...
/**
 * HTTP Parser<br> 
 * - com.hanjin.syscommon.management.bms 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class COM_SYS_014HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        ComUsrRoleVO[] com_usr_role = (ComUsrRoleVO[])getVOs(request,ComUsrRoleVO.class);
        //COM_USR_ROLE com_usr_role = COM_USR_ROLE.fromRequest(request);
        ComSys014Event event = new ComSys014Event();

		if(command.isCommand(FormCommand.MULTI)) {
			ComUsrRoleVO[] com_usr_roles = (ComUsrRoleVO[])getVOs(request,ComUsrRoleVO.class,"");
			//com_usr_roles = COM_USR_ROLE.fromRequestGrid(request,"");
			log.error("========command is : "+command.getCommand()+" and counts is : "+com_usr_roles.length);
			event.setComUsrRoleVOS(com_usr_roles);
		}
		
		event.setComUsrRoleVOS(com_usr_role);

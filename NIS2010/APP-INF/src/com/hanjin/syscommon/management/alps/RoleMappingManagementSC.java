...
import com.hanjin.syscommon.management.alps.role.basic.RoleMappingBC;
import com.hanjin.syscommon.management.alps.role.basic.RoleMappingBCImpl;
...
public class RoleMappingManagementSC extends ServiceCommandSupport {
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if (e.getEventName().equalsIgnoreCase("ComSys008Event")) {
			if (e.getFormCommand().isCommand(FormCommand.MULTI)) {
				...
				eventResponse = searchRoleList(e);
			} 
		}
	...
	private EventResponse searchUserRoleList(Event e) throws EventException {
		...
		try {
			...
			String comRollMenuFlg = command.getComRollMenuFlg(event.getUsrId());
			eventResponse.setETCData("view_flg", comRollMenuFlg);
	 ...
	 * @exception EventException
	 */
	private EventResponse searchRoleList(Event e) throws EventException {
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		ComSys008Event event = (ComSys008Event)e;
		RoleMappingBC command = new RoleMappingBCImpl();

		try {
			List<ComUsrRoleVO> list = command.searchRoleList(event.getComUsrRoleVO());

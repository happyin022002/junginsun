...
import com.hanjin.syscommon.management.alps.bookmark.basic.BookmarkManagementBC;
import com.hanjin.syscommon.management.alps.bookmark.basic.BookmarkManagementBCImpl;
import com.hanjin.syscommon.management.alps.bookmark.event.BookmarkManagementEvent;
		...
		*/
		...
		if (e.getEventName().equalsIgnoreCase("BookmarkManagementEvent")) {
			if (e.getFormCommand().isCommand(FormCommand.MULTI)) {
				...
				eventResponse = searchPgmNoAuthority(e);
			}
...
//			sender.send(comEmlSndInfoVO);
	 ...
	 */
	private EventResponse searchPgmNoAuthority(Event e) throws EventException {
		BookmarkManagementEvent event = (BookmarkManagementEvent)e; 
		GeneralEventResponse eventResponse = new GeneralEventResponse();
		try {
			String usrId = account.getUsr_id();
			BookmarkManagementBC command = new BookmarkManagementBCImpl();
			String flg = command.searchPgmNoAuthority(usrId, event.getComBookmark().getPgmNo());
			eventResponse.setETCData(flg, flg);
		} catch (EventException de) {
			throw new EventException(new ErrorHandler(de).getMessage());
		}
		return eventResponse;

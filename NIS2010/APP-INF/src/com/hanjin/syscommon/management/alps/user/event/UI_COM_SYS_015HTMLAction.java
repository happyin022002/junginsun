...
public class UI_COM_SYS_015HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        */ 
        ComUserVO[] com_user = (ComUserVO[])getVOs(request, ComUserVO.class);
        UiComSys015Event event = new UiComSys015Event();
       
		if(command.isCommand(FormCommand.MULTI)) {
			ComUsrOfcCngVO[] com_users = (ComUsrOfcCngVO[])getVOs(request, ComUsrOfcCngVO.class);
			//GNI_USR_FILE[] gni_usr_files = (GNI_USR_FILE[])getVOs(request, GNI_USR_FILE.class);
			Collection gni_usr_files = GNI_USR_FILE.fromRequestGrid(request,"");
			event.setComUserVO(com_users);
			event.setGNI_USR_FILES(gni_usr_files);
			//event.setGniUsrFileVOs(gni_usr_files);
		}
		event.setComUserVOs(com_user);

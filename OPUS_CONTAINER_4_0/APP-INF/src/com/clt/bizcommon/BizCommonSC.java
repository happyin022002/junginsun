...
//            eventResponse.setRsVoList(command.searchCommodityList(event.getCmdtCd(),event.getRepCmdtCd(),event.getRepImdgLvlCd(),event.getCmdtNm(),event.getIPage()));
    ...
    private EventResponse searchCustomerList(Event e) throws EventException {
        ...
        try {
            ...
            eventResponse.setRsVoList(command.searchCustomerList(event.getCustCd(),event.getCustNm(),event.getOfcCd(),event.getIPage(),event.getInclude(),event.getCust(), event.getMdmYN(), event.getDeltFlg(), event.getLocCd(), event.getCreditFlg()));
        } catch(EventException ex) {
			throw ex;
		} catch(Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);

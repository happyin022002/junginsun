...
*@FileName : RailSoManageDBDAOSearchRailSoCorrectionTargetListBkgUpdRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RailSoManageDBDAOSearchRailSoCorrectionTargetListBkgUpdRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SO 발행 된 건들 중 BKG UPDATE에 대한 조회 SQL
	  * </pre>
	  */
	public RailSoManageDBDAOSearchRailSoCorrectionTargetListBkgUpdRSQL(){
		setQuery();
		...
		query.append("FileName : RailSoManageDBDAOSearchRailSoCorrectionTargetListBkgUpdRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	MAX(TMP.POP_IMG) POP_IMG," ).append("\n"); 
		query.append("	MAX(TMP.DCGO_SEQ) DCGO_SEQ," ).append("\n"); 
		query.append("	MAX(TMP.DECL_NM) DECL_NM," ).append("\n"); 
		query.append("	MAX(TMP.VLD_CHK) VLD_CHK," ).append("\n"); 
		query.append("	MAX(TMP.DCGO_FLG) DCGO_FLG,		" ).append("\n"); 
		query.append("	MAX(TMP.VLD_FLG) VLD_FLG," ).append("\n"); 
		query.append("	MAX(TMP.DG_CNTR_SEQ) DG_CNTR_SEQ," ).append("\n"); 
		query.append("	MAX(TMP.BKG_DCGO_FLG) BKG_DCGO_FLG," ).append("\n"); 
		query.append("	DECODE(MAX(TMP.CND_CSTMS_CLR_FLG), 'Y', '1', '0') CND_CSTMS_CLR_FLG," ).append("\n"); 
		...
		query.append("            FROM TRS_TRSP_EDI_RAIL_ORD_RSND C1" ).append("\n"); 
		...
		query.append("			    CASE WHEN A.CGO_TP_CD = 'F' THEN '1' ELSE '0' END POP_IMG," ).append("\n"); 
		query.append("			    DG.DCGO_SEQ," ).append("\n"); 
		query.append("			    DG.DECL_NM," ).append("\n"); 
		query.append("			    DG.VLD_CHK," ).append("\n"); 
		query.append("			    D.DCGO_FLG," ).append("\n"); 
		query.append("				DG.DG_CNTR_SEQ,	" ).append("\n"); 
		query.append("				CASE WHEN (SUBSTR(A.FM_NOD_CD, 1, 2) = 'CA' OR SUBSTR(A.TO_NOD_CD, 1, 2) = 'CA') AND DG.VLD_CHK = 'N' AND D.DCGO_FLG = 'Y' THEN  'N'" ).append("\n"); 
		query.append("					 ELSE 'Y'" ).append("\n"); 
		query.append("				END VLD_FLG," ).append("\n"); 
		query.append("				B.DCGO_FLG BKG_DCGO_FLG," ).append("\n"); 
		query.append("				A.CND_CSTMS_CLR_FLG," ).append("\n"); 
		...
		query.append("					 AND DG.DCGO_SEQ = DE.DCGO_SEQ(+)" ).append("\n"); 
		query.append("					 AND DG.DG_CNTR_SEQ = DE.DG_CNTR_SEQ(+)) DG" ).append("\n"); 
		...
		query.append("			AND   B.BKG_NO = SKD.BKG_NO" ).append("\n"); 
		...
		query.append("			AND   NVL(DG.CNTR_NO(+), 'X') = NVL2(DG.CNTR_NO(+), A.EQ_NO, 'X')" ).append("\n"); 
		query.append("			AND   A.EQ_TPSZ_CD = DG.CNTR_TPSZ_CD(+)" ).append("\n"); 
		query.append("			AND   DG.DG_DECL_SEQ(+) = DECODE(A.TRSP_BND_CD, 'O', 1, 'I', 2)" ).append("\n"); 
		query.append("" ).append("\n"); 

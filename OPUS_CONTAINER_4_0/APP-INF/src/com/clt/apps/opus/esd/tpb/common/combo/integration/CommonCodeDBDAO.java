...
* 2009-11-19 Sun, CHOI      2.1 Renewal Migration
* -------------------------------------------------------
* History
...
//		    String tmpQuery = null;
			...
			if ( addOptionInfo!=null ){
				ArrayList addOptArr = new ArrayList(); 
...
//		    	
//				for (int idx=0; idx<addOptionInfo.length; idx++){
//					queryStr.append( "----------                                      ".trim() + " \n" );
//					queryStr.append( "UNION ALL                                       ".trim() + " \n" );
//					queryStr.append( "SELECT TO_NUMBER('");
//					queryStr.append( addOptionInfo[idx][0] );
//					queryStr.append( "') SEQ, NULL INTG_CD_ID, '");
//					queryStr.append( addOptionInfo[idx][1] );
//					queryStr.append( "' CODE, '");
//					queryStr.append( addOptionInfo[idx][2] );
//					queryStr.append( "' NAME, " );
//					queryStr.append( "DECODE('" );
//					queryStr.append( addOptionInfo[idx][1] );
//					queryStr.append( "', '" );
//					queryStr.append( sDefaultSelectCode );
//					queryStr.append( "','Y','N') SELECTEDYN                 		  ".trim() + " \n" ); 
//					queryStr.append( "FROM DUAL                                       ".trim() + " \n" ); 
//				}
//				tmpQuery = queryStr.toString();
//				velParam.put("tmpQuery", tmpQuery);
//			}
//		    
//		    velParam.put("sortKey", sortKey);
...
//		 String[] officeLevelInfo = null;
		 ...
		 try{
			 if(event != null){				 
				 param.put("s_if_ofc_cd", if_ofc_cd);
//				 param.put("s_usr_ofc_cd", if_ofc_cd);
...
//				 log.debug("velParam==========>"+velParam);
	...
	public DBRowSet checkRegOffice(CommonCodeEvent event) throws DAOException {
		 ...
		 try{
			 if(event != null){
				 ...
				 velParam.put("s_ofc_cd_reg", s_ofc_cd_reg);
				 velParam.put("s_n3pty_ofc_tp_cd", s_n3pty_ofc_tp_cd);				 
			 }
			 dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new CommonCodeDBDAOSearchCheckRegOfficeRSQL(), param, velParam);
		 }catch(SQLException se){
			 log.error(se.getMessage(),se);
			 throw new DAOException(new ErrorHandler(se).getMessage());
		 }catch(Exception ex){
	 ...
	 * @throws DAOException
	 */
	public DBRowSet checkRegOffice2(CommonCodeEvent event) throws DAOException {
		 DBRowSet dbRowset = null;
		 
		 HashMap params = event.getEventParams();
		 String s_ofc_cd_reg = JSPUtil.getNull((String)params.get("s_ofc_cd_reg"));
		 String s_n3pty_ofc_tp_cd_t = JSPUtil.getNull((String)params.get("s_n3pty_ofc_tp_cd_t"));
		 ...
		 try{
			 if(event != null){
				 param.put("s_ofc_cd_reg", s_ofc_cd_reg);
				 param.put("s_n3pty_ofc_tp_cd", s_n3pty_ofc_tp_cd_t);
				 
				 velParam.put("s_ofc_cd_reg", s_ofc_cd_reg);
				 velParam.put("s_n3pty_ofc_tp_cd", s_n3pty_ofc_tp_cd_t);
			 }
			 dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new CommonCodeDBDAOSearchCheckRegOffice2RSQL(), param, velParam);
		 }catch(SQLException se){
			 log.error(se.getMessage(),se);
			 throw new DAOException(new ErrorHandler(se).getMessage());
		 }catch(Exception ex){
	 ...
	 * @throws DAOException
	 */
	public DBRowSet checkRegOffice3(CommonCodeEvent event) throws DAOException {
		 DBRowSet dbRowset = null;
		 ...
		 try{
			 ...
			 dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new CommonCodeDBDAOSearchCheckRegOffice3RSQL(), param, velParam);
		 }catch(SQLException se){
			 log.error(se.getMessage(),se);
			 throw new DAOException(new ErrorHandler(se).getMessage());
		 }catch(Exception ex){

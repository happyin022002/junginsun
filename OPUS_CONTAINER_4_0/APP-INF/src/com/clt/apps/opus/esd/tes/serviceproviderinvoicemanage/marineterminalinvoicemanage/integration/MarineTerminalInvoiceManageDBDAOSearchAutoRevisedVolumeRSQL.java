...
public class MarineTerminalInvoiceManageDBDAOSearchAutoRevisedVolumeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(" AND    DECODE(BB_CGO_FLG,'Y','BB',DECODE(LOCL_TS_IND_CD,'T',DECODE(CNTR_STY_CD,'F','TS',DECODE(TS_MTY_FLG,'Y','TS','TM')),DECODE(CNTR_STY_CD,'F','FL','MT'))) = SUBSTR(@[lgs_cost_cd],5,2)" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${lgs_cost_cd} == 'TMNDTS' && ${cntr_sty_cd} == 'M')" ).append("\n"); 
		query.append(" AND    TS_MTY_FLG = 'Y'" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append(" AND    TS_MTY_FLG IS NULL" ).append("\n"); 
		...
		query.append(" AND    DECODE(@[tml_trns_mod_cd],'','S','S','S','O','O',NVL(TML_TRNS_MOD_CD,'S')) = NVL(@[tml_trns_mod_cd],'S')" ).append("\n"); 
		query.append(" AND    NVL(RC_FLG,'X')         = NVL(@[rc_flg],'X')" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${lgs_cost_cd} == 'TPNDFL' || ${lgs_cost_cd} == 'TPNDMT' || ${lgs_cost_cd} == 'TPNDTS' || ${lgs_cost_cd} == 'TPNDTM' || ${lgs_cost_cd} == 'SVLDFL' || ${lgs_cost_cd} == 'SVLDMT' || ${lgs_cost_cd} == 'SVLDTS' || ${lgs_cost_cd} == 'SVLDTM')" ).append("\n"); 
		query.append(" #if (${sub_trd_cd} == '')" ).append("\n"); 
		query.append("  AND    SUB_TRD_CD IS NULL" ).append("\n"); 
		query.append(" #elseif (${sub_trd_cd} == 'S')" ).append("\n"); 
		query.append("  AND    DECODE(@[sub_trd_cd],'S','S',NVL(SUB_TRD_CD,'S')) = NVL(@[sub_trd_cd],'S')" ).append("\n"); 
		query.append(" #else" ).append("\n"); 
		query.append("  AND    NVL(SUB_TRD_CD,'S')     = NVL(@[sub_trd_cd],'S')" ).append("\n"); 
		query.append(" #end" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append(" #if (${sub_trd_cd} == 'S' || ${sub_trd_cd} == '')" ).append("\n"); 
		query.append("  AND    DECODE(@[sub_trd_cd],'','S','S','S',NVL(SUB_TRD_CD,'S')) = NVL(@[sub_trd_cd],'S')" ).append("\n"); 
		query.append(" #else" ).append("\n"); 
		query.append("  AND    NVL(SUB_TRD_CD,'S')     = NVL(@[sub_trd_cd],'S')" ).append("\n"); 
		query.append(" #end" ).append("\n"); 
		query.append("#end" ).append("\n"); 

...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.trs.emptyreposomanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_TRS_0012HTMLAction extends HTMLActionSupport {
	...
	private static final long serialVersionUID = -7857819421892370514L;
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if (command.isCommand(FormCommand.SEARCH)) {
			...
			String rad_wo_issued = JSPUtil.getParameter(request, "rad_wo_issued", "");
			...
			String search_fm_node = JSPUtil.getParameter(request, "search_fm_node", "");
			String search_to_node = JSPUtil.getParameter(request, "search_to_node", "");

			event.setRadWoIssued(rad_wo_issued);
			...
			event.setSingleTransportationSOManageVOs(singleTransportationSOManageVO.fromRequestGrid(request));
		...
		} else if (command.isCommand(FormCommand.MODIFY) || command.isCommand(FormCommand.MODIFY02)) { // S/O Correction
			...
			event.setCbstatus(JSPUtil.getParameter(request, "cbstatus", ""));
			event.setCtrlOfcCd(JSPUtil.getParameter(request, "ctrl_ofc_cd", ""));
			event.setSingleTransportationVOs((SingleTransportationVO[]) getVOs(request, SingleTransportationVO.class));
		}

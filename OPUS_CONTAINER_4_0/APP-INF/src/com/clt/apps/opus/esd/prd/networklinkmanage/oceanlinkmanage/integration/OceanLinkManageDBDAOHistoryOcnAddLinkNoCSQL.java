...
*@FileName : OceanLinkManageDBDAOHistoryOcnAddLinkNoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class OceanLinkManageDBDAOHistoryOcnAddLinkNoCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public OceanLinkManageDBDAOHistoryOcnAddLinkNoCSQL(){
		setQuery();
		...
		query.append("FileName : OceanLinkManageDBDAOHistoryOcnAddLinkNoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    AND N1ST_LANE_CD = decode( @[linkNo] , '1', @[lane] , '','X', N1ST_LANE_CD)  " ).append("\n"); 
		query.append("    AND NVL(N1ST_LANE_FDR_FLG,'Z')  = decode( @[linkNo] , '1', 'N',   '','X',NVL(N1ST_LANE_FDR_FLG,'Z'))  " ).append("\n"); 
		...
		query.append("    AND NVL(N2ND_LANE_CD      ,'Z') = decode( @[linkNo] , '2', @[lane] , '','X',NVL(N2ND_LANE_CD      ,'Z'))  " ).append("\n"); 
		query.append("    AND NVL(N2ND_LANE_FDR_FLG ,'Z') = decode( @[linkNo] , '2', 'N',   '','X',NVL(N2ND_LANE_FDR_FLG ,'Z'))  " ).append("\n"); 
		...
		query.append("    AND NVL(N3RD_LANE_CD      ,'Z') = decode( @[linkNo] , '3', @[lane] , '','X',NVL(N3RD_LANE_CD      ,'Z'))  " ).append("\n"); 
		query.append("    AND NVL(N3RD_LANE_FDR_FLG ,'Z') = decode( @[linkNo] , '3', 'N',   '','X',NVL(N3RD_LANE_FDR_FLG ,'Z') )  " ).append("\n"); 
		...
		query.append("    AND NVL(N4TH_LANE_CD      ,'Z') = decode( @[linkNo] , '4', @[lane] , '','X',NVL(N4TH_LANE_CD      ,'Z'))  " ).append("\n"); 
		query.append("    AND NVL(N4TH_LANE_FDR_FLG ,'Z') = decode( @[linkNo] , '4', 'N',   '','X',NVL(N4TH_LANE_FDR_FLG ,'Z'))" ).append("\n"); 

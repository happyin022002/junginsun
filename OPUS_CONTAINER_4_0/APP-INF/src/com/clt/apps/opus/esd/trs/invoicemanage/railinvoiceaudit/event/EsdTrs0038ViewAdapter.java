...
public class EsdTrs0038ViewAdapter extends TrsDefaultViewAdapter {
	...
	protected String makeDataTag(DBRowSet rs, String prefix, Event event) {
		...
		try {
			if (formcommand.isCommand(FormCommand.MULTI) || formcommand.isCommand(FormCommand.ADD) || formcommand.isCommand(FormCommand.REMOVE) || formcommand.isCommand(FormCommand.MODIFY)) { // 저장XML인 경우
				...
				while (rs.next()) {
					...
					for (int j = 1; j < colCount + 1; j++) {
						if ("fm_nod_cd2".equals(changedColNms[j - 1]) || "to_nod_cd2".equals(changedColNms[j - 1])) {
							sb.append("<TD COMBO-TEXT='").append("|").append(getNull(rs.getObject(j))).toString();
							sb.append("' COMBO-CODE='").append("|").append(getNull(rs.getObject(j))).toString();
							sb.append("'><![CDATA[");

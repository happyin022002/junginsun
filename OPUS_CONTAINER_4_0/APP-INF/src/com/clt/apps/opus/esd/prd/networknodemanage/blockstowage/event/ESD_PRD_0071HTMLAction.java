 ...
 *@FileName : ESD_PRD_070HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESD_PRD_0071HTMLAction extends HTMLActionSupport{
	private static final long serialVersionUID = 1L;

	/**
	 * ESD_PRD_070HTMLAction 객체를 생성
	 */
	public ESD_PRD_0071HTMLAction(){
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0071Event event = new EsdPrd0071Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		FormCommand command = FormCommand.fromRequest(request);

		if(command.isCommand(FormCommand.SEARCH)){
			event.setGroupCreationVO((GroupCreationVO) getVO(request, GroupCreationVO.class));
		}else if(command.isCommand(FormCommand.SEARCH01)){
			event.setGroupCreationVO((GroupCreationVO) getVO(request, GroupCreationVO.class));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setGroupCreationVOs((GroupCreationVO[]) getVOs(request, GroupCreationVO.class));
		}

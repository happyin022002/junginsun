...
*@FileName : ProductCatalogCreateDBDAOCreateSceSoReplaneOcnInlndChangeCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ProductCatalogCreateDBDAOCreateSceSoReplaneOcnInlndChangeCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * CreateSceSoReplaneOcnInlndChange
	  * </pre>
	  */
	public ProductCatalogCreateDBDAOCreateSceSoReplaneOcnInlndChangeCSQL(){
		setQuery();
		...
		params.put("f_pu",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("manualFlg",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("skipActVal",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("new_pol",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("new_pod",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("m_rtn",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("obInclshtlso_flg",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("ibInclshtlso_flg",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("ioBndCd",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : ProductCatalogCreateDBDAOCreateSceSoReplaneOcnInlndChangeCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("		 -- Ocean Shuttle S/O Trans Mode 우선 적용하도록 수정 #Mod 2010.07.14 by jj" ).append("\n"); 
		...
		query.append("    (CASE WHEN NODE_NEW_KIND='N' AND F_NO2 IN (1,41) AND (SELECT NOD_TP_CD FROM PRD_NODE WHERE NOD_CD = NODE_NEW1) = 'Z' THEN ( SELECT CGO_HNDL_TM_HRS FROM MDM_ZONE WHERE ZN_CD = NODE_NEW1 )" ).append("\n"); 
		...
		query.append("    A_TIME," ).append("\n"); 
		query.append("    D_TIME," ).append("\n"); 
		query.append("    ( CASE 	WHEN NODE_NEW_KIND='N' THEN ( SELECT TO_CHAR(N1ST_VNDR_SEQ)  FROM MDM_YARD WHERE YD_CD = NODE_NEW1 )" ).append("\n"); 
		query.append("			WHEN NODE_NEW_KIND='L' AND F_NO2 IN (13,18,23,28) THEN 	" ).append("\n"); 
		query.append("			(" ).append("\n"); 
		...
		query.append("      ROW_NUMBER() over (PARTITION BY GP1, OI_BND_CD, NODE_NEW1, NODE_NEW2 ORDER BY F_NO2,L_N0,O1) O2," ).append("\n"); 
		...
		query.append("            WHEN F_N0 IN (18,23,28) AND (L_N0=1) 	THEN TO_DATE(DECODE(F_N0,13,POLT1,18,POLT2,23,POLT3,28,POLT4,NULL),'YYYYMMDDHH24MISS') -6/24 --CCT적용" ).append("\n"); 
		...
		query.append("            WHEN F_N0 IN (17,22,27) AND (L_N0=2) 	THEN TO_DATE(DECODE(F_N0,17,PODT1,22,PODT2,27,PODT3,NULL),'YYYYMMDDHH24MISS') + 36/24" ).append("\n"); 
		...
		query.append("                    WHEN 13 THEN DECODE(@[rcv_t],'T',OB_ORG,DECODE(@[rcv_t],'F',OB_ORG,NVL(POL0,POL1)))  " ).append("\n"); 
		query.append("                    WHEN 14 THEN DECODE(POD1,POD_NODE,'',DECODE(POD1,POL2,DECODE(TS1_2,NULL,'',POD1),POD1)) --DECODE(POD1,POD_NODE,'',DECODE(POD1,POL2,'',POD1))" ).append("\n"); 
		...
		query.append("                          WHEN LENGTH(NVL(@[m_rtn],'X')) = 7 THEN @[m_rtn]" ).append("\n"); 
		...
		query.append("                    RANK() OVER (ORDER BY  ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ, OB_ORG,OB_DEST,OB_SEQ, IB_ORG,IB_DEST,IB_SEQ,POL1,POD1,POL2, POD2,POL3,POD3,POL4,POD4) GP1" ).append("\n"); 
		query.append("                    FROM (" ).append("\n"); 
		query.append("                        SELECT" ).append("\n"); 
		query.append("                        OB_ORG,OB_DEST,OB_SEQ,ORG_ROUT,FULL_RTN_YD_CD," ).append("\n"); 
		query.append("                        RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,OB_DEST ORDER BY FULL_RTN_PRIO_SEQ,OB_BKG_FLG,OB_MODE_PRIO_SEQ,OB_TMP_FLG,OB_PRIO_SEQ,OB_ROUT_SEQ) OB_RK," ).append("\n"); 
		...
		query.append("                        RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,IB_ORG ORDER BY IB_MODE_PRIO_SEQ,IB_BKG_FLG,IB_TMP_FLG,IB_PRIO_SEQ,IB_ROUT_SEQ ) IB_RK," ).append("\n"); 
		...
		query.append("							(CASE WHEN @[manualFlg] = 'Y' THEN 'Y'" ).append("\n"); 
		query.append("                                  WHEN NVL((SELECT 'Y' " ).append("\n"); 
		query.append("											FROM BKG_TRO" ).append("\n"); 
		query.append("											WHERE BKG_NO = (SELECT BKG_NO FROM SCE_COP_HDR WHERE COP_NO =@[cop_no])" ).append("\n"); 
		...
		query.append("                                  WHEN O.ROUT_ORG_NOD_CD IS NULL AND @[por] = @[pol] AND @[rcv_t] = 'S' AND POL1_S ='Y' THEN 'Y'" ).append("\n"); 
		query.append("                                  WHEN O.ROUT_ORG_NOD_CD IS NULL AND @[por] = @[pol] AND @[rcv_t] NOT IN ('D', 'S') THEN 'Y'" ).append("\n"); 
		...
		query.append("                                         AND O.ROUT_ORG_NOD_CD  LIKE DECODE(@[rcv_t],'F','X','T','X','D',NVL(@[por_n],@[por]),@[por])||'%'" ).append("\n"); 
		...
		query.append("                                   WHEN I.ROUT_DEST_NOD_CD IS NULL AND NVL(@[pod],SUBSTR( T.POD_NODE ,1,5)) = @[del] AND @[del_t] = 'S' AND POD_NODE_S ='Y' THEN 'Y'" ).append("\n"); 
		query.append("                                   WHEN I.ROUT_DEST_NOD_CD IS NULL AND NVL(@[pod],SUBSTR( T.POD_NODE ,1,5)) = @[del] AND @[del_t] NOT IN ('D', 'S') THEN 'Y'" ).append("\n"); 
		...
		query.append("                                         AND I.ROUT_DEST_NOD_CD  LIKE DECODE(@[del_t],'F','X','T','X','D',NVL(@[del_n],@[del]),@[del])||'%'" ).append("\n"); 
		query.append("                                         THEN 'Y'" ).append("\n"); 
		query.append("                                  ELSE 'X'" ).append("\n"); 
		...
		query.append("                                N1ST_LANE_CD,N2ND_LANE_CD,N3RD_LANE_CD,N4TH_LANE_CD,N2ND_POL_CD, N3RD_POL_CD,N4TH_POL_CD," ).append("\n"); 
		query.append("                                N1ST_TZTM_HRS,N2ND_TZTM_HRS,N3RD_TZTM_HRS,N4TH_TZTM_HRS,CCT," ).append("\n"); 
		query.append("                                DECODE(@[new_pol],NULL,DECODE(N1ST_SVC_TP,'O',DECODE(DECODE(SIGN(INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1)),1,SUBSTR(@[ocn_str],INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1),7)),NULL,POL1," ).append("\n"); 
		query.append("                                                                                     DECODE(SIGN(INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1)),1,SUBSTR(@[ocn_str],INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1),7)))" ).append("\n"); 
		query.append("                                                                             ,POL1 )" ).append("\n"); 
		query.append("                                                      ,@[new_pol]) POL1," ).append("\n"); 
		...
		query.append("                                       				                   DECODE(SIGN(INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1)),1,SUBSTR(@[ocn_str],INSTR(@[ocn_str],SUBSTR(POL1,1,5),1,1),7)))" ).append("\n"); 
		query.append("                                       				           ,POL1) ) POL1_S,                           " ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                                POLT4,PODT4,VVD4,CRR4,POL_SEQ4,POD_SEQ4," ).append("\n"); 
		query.append("                                DECODE(@[new_pod],NULL,DECODE(DECODE(SIGN(INSTR(@[ocn_str],SUBSTR(POD_NODE,1,5),-1)),1,SUBSTR(@[ocn_str],INSTR(@[ocn_str],SUBSTR(POD_NODE,1,5),-1),7),POD_NODE),NULL,POD_NODE," ).append("\n"); 
		query.append("                                                            DECODE(SIGN(INSTR(@[ocn_str],SUBSTR(POD_NODE,1,5),-1)),1,SUBSTR(@[ocn_str],INSTR(@[ocn_str],SUBSTR(POD_NODE,1,5),-1),7),POD_NODE)),@[new_pod]) POD_NODE," ).append("\n"); 
		query.append("                                (SELECT YD_FCTY_TP_CFS_FLG" ).append("\n"); 
		query.append("                                 FROM MDM_YARD" ).append("\n"); 
		...
		query.append("                                    (CASE WHEN @[skipActVal] = 'Y' AND @[vvd1] IS NULL THEN NULL" ).append("\n"); 
		query.append("                                          ELSE TRIM(DECODE(INSTR(SKD_STR,'VVD1'),0,NULL,SUBSTR(SKD_STR,INSTR(SKD_STR,'VVD1')+4,9)))" ).append("\n"); 
		query.append("                                    END) VVD1," ).append("\n"); 
		...
		query.append("                                    (CASE WHEN @[skipActVal] = 'Y' AND @[vvd2] IS NULL THEN NULL" ).append("\n"); 
		query.append("                                          ELSE TRIM(DECODE(INSTR(SKD_STR,'VVD2'),0,NULL,SUBSTR(SKD_STR,INSTR(SKD_STR,'VVD2')+4,9)))" ).append("\n"); 
		query.append("									END) VVD2," ).append("\n"); 
		...
		query.append("                                    (CASE WHEN @[skipActVal] = 'Y' AND @[vvd3] IS NULL THEN NULL" ).append("\n"); 
		query.append("                                          ELSE TRIM(DECODE(INSTR(SKD_STR,'VVD3'),0,NULL,SUBSTR(SKD_STR,INSTR(SKD_STR,'VVD3')+4,9))) " ).append("\n"); 
		query.append("									END) VVD3," ).append("\n"); 
		...
		query.append("                                    (CASE WHEN @[skipActVal] = 'Y' AND @[vvd4] IS NULL THEN NULL" ).append("\n"); 
		query.append("                                          ELSE TRIM(DECODE(INSTR(SKD_STR,'VVD4'),0,NULL,SUBSTR(SKD_STR,INSTR(SKD_STR,'VVD4')+4,9))) " ).append("\n"); 
		query.append("									END) VVD4," ).append("\n"); 
		...
		query.append("                                        N1ST_TZTM_HRS,N2ND_TZTM_HRS,N3RD_TZTM_HRS,N4TH_TZTM_HRS," ).append("\n"); 
		query.append("                                        " ).append("\n"); 
		query.append("                                        (CASE WHEN @[skipActVal] = 'Y' THEN" ).append("\n"); 
		query.append("               									PRD_GET_OCN_SKD2_FNC(@[skd_date],@[skd_type],ORG_LOC_CD,DEST_LOC_CD," ).append("\n"); 
		query.append("                   							 		N1ST_POL_CD,@[n1st_pol_dc_seq], N1ST_POD_CD, @[n1st_pod_dc_seq], N1ST_LANE_CD,N1ST_SKD_DIR_CD,N1.VSL_SVC_TP_CD,@[vvd1]," ).append("\n"); 
		query.append("                    						 		N2ND_POL_CD,@[n2nd_pol_dc_seq], N2ND_POD_CD, @[n2nd_pod_dc_seq], N2ND_LANE_CD,N2ND_SKD_DIR_CD,N2.VSL_SVC_TP_CD,@[vvd2]," ).append("\n"); 
		query.append("                                             		N3RD_POL_CD,@[n3rd_pol_dc_seq], N3RD_POD_CD, @[n3rd_pod_dc_seq], N3RD_LANE_CD,N3RD_SKD_DIR_CD,N3.VSL_SVC_TP_CD,@[vvd3]," ).append("\n"); 
		query.append("                                             		N4TH_POL_CD,@[n4th_pol_dc_seq], DEST_LOC_CD, @[n4th_pod_dc_seq], N4TH_LANE_CD,N4TH_SKD_DIR_CD,N4.VSL_SVC_TP_CD,@[vvd4]," ).append("\n"); 
		query.append("                                        		NVL(@[cgo_tp],'AL'),'N') " ).append("\n"); 
		query.append("                                              ELSE " ).append("\n"); 
		...
		query.append("                                             		N4TH_POL_CD,@[n4th_pol_dc_seq], DEST_LOC_CD, @[n4th_pod_dc_seq], N4TH_LANE_CD,N4TH_SKD_DIR_CD,N4.VSL_SVC_TP_CD,@[vvd4]," ).append("\n"); 
		query.append("                                        		NVL(@[cgo_tp],'AL'),'N') " ).append("\n"); 
		query.append("										END) SKD_STR " ).append("\n"); 
		query.append("                                        FROM PRD_OCN_ROUT A,MDM_VSL_SVC_LANE N1,MDM_VSL_SVC_LANE N2,MDM_VSL_SVC_LANE N3,MDM_VSL_SVC_LANE N4" ).append("\n"); 
		query.append("                                        WHERE A.ORG_LOC_CD =@[org_loc_cd]" ).append("\n"); 
		query.append("                                        AND A.DEST_LOC_CD = @[dest_loc_cd]" ).append("\n"); 
		query.append("                                        AND A.ROUT_SEQ = DECODE(@[skipActVal],'Y',A.ROUT_SEQ,@[ocn_seq])" ).append("\n"); 
		...
		query.append("                                          WHEN @[rcv_t] = 'D' THEN NVL(@[por_n], OB_REP_ZN_CD)" ).append("\n"); 
		query.append("                                          WHEN @[manualFlg] = 'Y' AND @[ioBndCd] = 'O' THEN @[por] || '%'" ).append("\n"); 
		...
		query.append("                            AND NVL(O.INLND_ROUT_INCL_STTL_FLG(+),'N') = NVL(@[obInclshtlso_flg],NVL(O.INLND_ROUT_INCL_STTL_FLG(+),'N'))" ).append("\n"); 
		...
		query.append("                            AND NVL(I.INLND_ROUT_INCL_STTL_FLG(+),'N') = NVL(@[ibInclshtlso_flg],NVL(I.INLND_ROUT_INCL_STTL_FLG(+),'N'))" ).append("\n"); 
		query.append("                            AND I.ROUT_DEST_NOD_CD(+) " ).append("\n"); 
		query.append("                                LIKE CASE WHEN @[del_t] IN ('F', 'T') THEN 'X'" ).append("\n"); 
		query.append("                                          WHEN @[del_t] = 'D' THEN NVL(@[del_n], IB_REP_ZN_CD)" ).append("\n"); 
		query.append("                                          WHEN @[manualFlg] = 'Y' AND @[ioBndCd] = 'I' THEN @[del] || '%'" ).append("\n"); 
		...
		query.append("                            AND NVL(O.FULL_RTN_YD_CD(+),'X') = NVL(DECODE(@[ioBndCd],'O',NVL(@[f_rt],NVL(O.FULL_RTN_YD_CD(+),'X')),NVL(O.FULL_RTN_YD_CD(+),'X')),'X')    --FULL CY??OUTBOUND??곕르??TRO ?怨몄벥 FULL RETURN CY" ).append("\n"); 
		query.append("                            AND NVL(I.FULL_PKUP_YD_CD(+),'X') = NVL(DECODE(@[ioBndCd],'I',NVL(@[f_pu],NVL(I.FULL_PKUP_YD_CD(+),'X')),NVL(I.FULL_PKUP_YD_CD(+),'X')),'X')" ).append("\n"); 
		query.append("                            AND NVL(O.TRSP_MOD_CD(+),'X') = NVL(DECODE(@[ob_trsp_mode],'AL',NVL(O.TRSP_MOD_CD(+),'X'),@[ob_trsp_mode]),NVL(O.TRSP_MOD_CD(+),'X'))" ).append("\n"); 
		query.append("                            AND NVL(I.TRSP_MOD_CD(+),'X') = NVL(DECODE(@[ib_trsp_mode],'AL',NVL(I.TRSP_MOD_CD(+),'X'),@[ib_trsp_mode]),NVL(I.TRSP_MOD_CD(+),'X'))" ).append("\n"); 
		...
		query.append("                        AND NVL(ROUT.ORG_ROUT,'X')  LIKE DECODE(@[manualFlg], 'Y', DECODE(@[ioBndCd], 'I', NULL, REGEXP_REPLACE(@[ob_str],'-...-','-')), REGEXP_REPLACE(@[ob_str],'-...-','-'))||'%'" ).append("\n"); 
		query.append("                        AND NVL(ROUT.DEST_ROUT,'X') LIKE DECODE(@[manualFlg], 'Y', DECODE(@[ioBndCd], 'O', NULL, REGEXP_REPLACE(@[ib_str],'-...-','-')), REGEXP_REPLACE(@[ib_str],'-...-','-'))||'%' " ).append("\n"); 
		query.append("                    )" ).append("\n"); 
		query.append("                WHERE   OB_RK = DECODE(@[manualFlg],'Y', DECODE(@[ioBndCd],'O',OB_RK,1),1) " ).append("\n"); 
		query.append("                    AND IB_RK = DECODE(@[manualFlg],'Y', DECODE(@[ioBndCd],'I',IB_RK,1),1) " ).append("\n"); 

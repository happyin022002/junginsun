CREATE OR REPLACE PROCEDURE TRS_AGMT_APLY_TO_COA_PRC(pi_bkg_no        IN COA_COM_PARA.BKG_NO%TYPE,
...
--             , 'F' CGO_TP_CD
             ...
             , NVL((SELECT TO_NUMBER(TRIM(REGEXP_SUBSTR(TRS_GET_COM_SO_RAIL_WGT_FNC('C', null, null, null, A.BKG_NO, A.CNTR_NO, A.CNTR_TPSZ_CD, 'LBS', A.COP_NO), '[^|]+', 1, 1)))
                          FROM SCE_COP_HDR A
...
--               END SPCL_CGO_CNTR_TP_CD
             , CASE WHEN X.DG_SPCL_FLG = 'Y' AND X.SPCL_AWK_CGO_FLG = 'Y' THEN 'AD'
                    WHEN X.DG_SPCL_FLG = 'Y' AND X.RF_SPCL_FLG = 'Y' THEN 'RD'
                    WHEN X.DG_SPCL_FLG = 'Y' AND X.SPCL_AWK_CGO_FLG != 'Y' AND X.RF_SPCL_FLG != 'Y' THEN 'DG'
                    WHEN X.DG_SPCL_FLG != 'Y' AND X.SPCL_AWK_CGO_FLG = 'Y' THEN 'AK'
                    WHEN X.DG_SPCL_FLG != 'Y' AND X.RF_SPCL_FLG = 'Y' THEN 'RF'
                    WHEN X.BB_SPCL_FLG = 'Y'  THEN 'BB'
                    ELSE 'GP'

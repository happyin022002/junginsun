...
public class CopDetailReceiveDBDAOCheckUSBndForCSMRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("               NVL(MAX(CASE WHEN B.POL_CD = C.VPS_PORT_CD THEN CLPT_SEQ ELSE NULL END) , 99) AS POL_CLPT_SEQ" ).append("\n"); 
		query.append("          from ctm_movement a," ).append("\n"); 
		query.append("               bkg_booking b," ).append("\n"); 
		query.append("               bkg_vvd v," ).append("\n"); 
		query.append("               vsk_vsl_port_skd c" ).append("\n"); 
		query.append("         where (cntr_no, cnmv_yr, cnmv_id_no) in " ).append("\n"); 
		...
		query.append("         group by cntr_no, cnmv_yr, cnmv_id_no, mvmt_sts_cd, v.vsl_cd, v.skd_voy_no, v.skd_dir_cd " ).append("\n"); 
		...
		query.append("             and (   (c.clpt_seq <= pod_clpt_seq and c.vps_port_cd like 'US%')" ).append("\n"); 
		query.append("                  or (c.clpt_seq <= del_clpt_seq and c.vps_port_cd like 'US%') " ).append("\n"); 
		query.append("                 ) " ).append("\n"); 
		query.append("             AND C.CLPT_SEQ >= POL_CLPT_SEQ" ).append("\n"); 
		query.append("       )" ).append("\n"); 

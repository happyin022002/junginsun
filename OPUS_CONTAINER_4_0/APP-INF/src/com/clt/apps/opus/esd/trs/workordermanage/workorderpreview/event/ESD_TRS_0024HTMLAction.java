...
public class ESD_TRS_0024HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = -5531237456990842748L;
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if (command.isCommand(FormCommand.SEARCH01) || command.isCommand(FormCommand.SEARCH02)) {
			...
			wrkOrdPrvVO.setDraftFlg(JSPUtil.getParameter(request, "draft_flg", ""));
			...
			event.setTrsTrspWrkOrdVO((TrsTrspWrkOrdVO) getVO(request, TrsTrspWrkOrdVO.class));
			event.setTrsTrspWrkOrdPrvTmpVO((TrsTrspWrkOrdPrvTmpVO) getVO(request, TrsTrspWrkOrdPrvTmpVO.class));
			event.setRdserverIp(JSPUtil.getParameter(request, "rdserver_ip"));
			getValuesFromRequest(event, request);
		} else if (command.isCommand(FormCommand.SEARCH06) || command.isCommand(FormCommand.SEARCH07)) {
			event.setWorkOrderPreviewVO((WorkOrderPreviewVO) getVO(request, WorkOrderPreviewVO.class));
		} else if (command.isCommand(FormCommand.SEARCH08)) {
			event.setpWoNo(JSPUtil.getParameter(request, "p_wo_no", ""));
			event.setPwoPrvGrpSeq(JSPUtil.getParameter(request, "p_wo_prv_grp_seq", ""));
			event.setWorkOrderPreviewVO((WorkOrderPreviewVO) getVO(request, WorkOrderPreviewVO.class));
		} else if (command.isCommand(FormCommand.MULTI02) || command.isCommand(FormCommand.MULTI03)) {
			event.setTrsTrspSvcOrdVO((TrsTrspSvcOrdVO) getVO(request, TrsTrspSvcOrdVO.class));
			event.setTrsTrspWrkOrdVO((TrsTrspWrkOrdVO) getVO(request, TrsTrspWrkOrdVO.class));
			event.setTrsTrspWrkOrdPrvTmpVO((TrsTrspWrkOrdPrvTmpVO) getVO(request, TrsTrspWrkOrdPrvTmpVO.class));
			event.setRdserverIp(JSPUtil.getParameter(request, "rdserver_ip"));
//			getValuesFromRequest(event, request);
			event.setWorkOrderPreviewVOs((WorkOrderPreviewVO[]) getVOs(request, WorkOrderPreviewVO.class, "confirm_all_"));
		}
		return event;
	}

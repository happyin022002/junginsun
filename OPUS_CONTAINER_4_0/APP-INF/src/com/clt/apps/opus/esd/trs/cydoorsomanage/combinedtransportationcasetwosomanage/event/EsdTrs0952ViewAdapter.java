 ...
 *@FileName       : EsdTrs0952ViewAdapter.java
 *@FileTitle      : EsdTrs0952ViewAdapter IBSheet Generation Class
 *Open Issues :
 *Change history :
 *@LastModifyDate : 
 *@LastModifier : 
 *@LastVersion : 1.0
=========================================================*/
package com.clt.apps.opus.esd.trs.cydoorsomanage.combinedtransportationcasetwosomanage.event;
...
public class EsdTrs0952ViewAdapter extends TrsDefaultViewAdapter {
				...
				while (rs.next()) {
					...
					for (int j = 1; j <= colCount; j++) {
						...
						if (colName.equals("FM_NOD_CD") || colName.equals("VIA_NOD_CD") || colName.equals("TO_NOD_CD") || colName.equals("DOR_NOD_CD")) {
							...
							sb.append("<TD");
							sb.append(" COMBO-TEXT='").append("|").append(getNull(rs.getObject(i)));
							sb.append("' COMBO-CODE='").append("|").append(getNull(rs.getObject(i)));
							sb.append("'>");
							sb.append(getNull(rs.getObject(i++)));
							sb.append("</TD>");
						} else if (colName.equals("FM_NOD_CD2")) {
							i++;
							sb.append((new StringBuilder()).append("<TD>").append(getNull(rs.getObject("FM_NOD_CD"))).append("</TD>").toString());
						} else if (colName.equals("VIA_NOD_CD2")) {
							i++;
							sb.append((new StringBuilder()).append("<TD>").append(getNull(rs.getObject("VIA_NOD_CD"))).append("</TD>").toString());
						} else if (colName.equals("TO_NOD_CD2")) {
							i++;
							sb.append((new StringBuilder()).append("<TD>").append(getNull(rs.getObject("TO_NOD_CD"))).append("</TD>").toString());
						} else if (colName.equals("DOR_NOD_CD2")) {
							i++;
							sb.append((new StringBuilder()).append("<TD>").append(getNull(rs.getObject("DOR_NOD_CD"))).append("</TD>").toString());
							...
							i++;
							sb.append((new StringBuilder()).append("<TD>").append(JSPUtil.getNull(rs.getObject("DOR_NOD_YARD"))).append("</TD>").toString());
						...
						} else if (colName.equals("CRE_OFC_CD")) {
							i++;

...
public class AccrualBatchExecuteResultDBDAOSearchAccrualBatchResultRevenueMonthListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("SELECT  	B.EXE_YRMON EXE_YRMON" ).append("\n"); 
		query.append(",   SUBSTR(B.REV_YRMON,0,4)||'-'||SUBSTR(B.REV_YRMON,5,2)	REV_YRMON" ).append("\n"); 
		...
		query.append("AND		D.MN_COST_TP_CD 		IN ( 'MT','TR','TM')" ).append("\n"); 
		query.append("#elseif ( ${f_cost_type_f} == '1' && ${f_cost_type_m}!= '1' )" ).append("\n"); 
		query.append("AND 	D.MN_COST_TP_CD			<> 'MT'" ).append("\n"); 
		query.append("#elseif ( ${f_cost_type_f} != '1' && ${f_cost_type_m} == '1')" ).append("\n"); 
		query.append("AND 	D.MN_COST_TP_CD			= 'MT'" ).append("\n"); 
		query.append("#elseif (${f_cost_type_f} !='1' && ${f_cost_type_m} != '1' )" ).append("\n"); 
		query.append("AND		D.MN_COST_TP_CD 		NOT IN ( 'MT','TR','TM')" ).append("\n"); 

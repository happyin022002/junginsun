...
public class DOFChgColPermanageDBDAOSearchDofChgColPerListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT '' SEQ, TMP.CNTR_RTN_YD_CD, TMP.D2_QTY, TMP.D4_QTY, TMP.D5_QTY, (TMP.D2_QTY+TMP.D4_QTY+TMP.D5_QTY) CNTR_QTY," ).append("\n"); 
		query.append("TRF.TRF_D2_QTY, TRF.TRF_D4_QTY, TRF.TRF_D5_QTY," ).append("\n"); 
		query.append("(TMP.D2_QTY*TRF.TRF_D2_QTY) + (TMP.D4_QTY*TRF.TRF_D4_QTY) + (TMP.D5_QTY*TRF.TRF_D5_QTY) TRF_AMT," ).append("\n"); 
		query.append("TRO_AMT, DOD_AMT" ).append("\n"); 
		query.append("FROM  (" ).append("\n"); 
		query.append("SELECT SUBSTR(CNTR_RTN_YD_CD,1,5) CNTR_RTN_YD_CD," ).append("\n"); 
		query.append("SUM(D2_QTY) D2_QTY, SUM(D4_QTY) D4_QTY, SUM(D5_QTY) D5_QTY," ).append("\n"); 
		query.append("SUM(TRO_AMT) TRO_AMT, SUM(DOD_AMT) DOD_AMT" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT SUBSTR(CNTR_RTN_YD_CD,1,5) CNTR_RTN_YD_CD," ).append("\n"); 
		...
		query.append("TRNS_REV_AMT TRO_AMT, DIF_AMT DOD_AMT" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT A.CRE_OFC_CD, A.BKG_NO, B.BL_NO BL_NO," ).append("\n"); 
		query.append("E.CUST_CNT_CD, E.CUST_SEQ, C.CNTR_NO," ).append("\n"); 
		query.append("C.CNMV_ID_NO, C.CNMV_YR, C.CNTR_TPSZ_CD, A.CNTR_RTN_YD_CD," ).append("\n"); 
		query.append("B.POR_CD, B.POL_CD, B.POD_CD, B.DEL_CD, A.CRE_DT, D.MVMT_STS_CD, D.ORG_YD_CD," ).append("\n"); 
		query.append("A.CURR_CD, (NVL(A.TRNS_REV_AMT,0)+NVL(A.NMF_TRNS_REV_AMT,0)) TRNS_REV_AMT, AR_IF.AR_INV_CHG_TP_CD," ).append("\n"); 
		query.append("SUM(AR_IF.AR_CHG_AMT) DIF_AMT" ).append("\n"); 
		query.append("FROM   BKG_EUR_TRO A, BKG_BOOKING B, BKG_CUSTOMER E, BKG_CONTAINER C, CTM_MOVEMENT D," ).append("\n"); 
		query.append("(SELECT" ).append("\n"); 
		query.append("LOCL_CURR_CD AR_CURR_CD," ).append("\n"); 
		...
		query.append("AND    A.HLG_TP_CD = 'M'" ).append("\n"); 
		query.append("AND    A.IO_BND_CD = 'I'" ).append("\n"); 
		query.append("AND    A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("AND    B.BKG_NO = E.BKG_NO" ).append("\n"); 
		query.append("AND    E.BKG_CUST_TP_CD = ( SELECT MIN(BKG_CUST_TP_CD)" ).append("\n"); 
		...
		query.append("GROUP BY A.CRE_OFC_CD, A.BKG_NO, B.BL_NO," ).append("\n"); 
		query.append("E.CUST_CNT_CD, E.CUST_SEQ, C.CNTR_NO," ).append("\n"); 
		query.append("C.CNMV_ID_NO, C.CNMV_YR, C.CNTR_TPSZ_CD, A.CNTR_RTN_YD_CD," ).append("\n"); 
		query.append("B.POR_CD, B.POL_CD, B.POD_CD, B.DEL_CD, A.CRE_DT, D.MVMT_STS_CD, D.ORG_YD_CD," ).append("\n"); 
		query.append("A.CURR_CD,A.TRNS_REV_AMT,A.NMF_TRNS_REV_AMT, AR_IF.AR_INV_CHG_TP_CD, AR_IF.AR_CURR_CD)" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("GROUP BY SUBSTR(CNTR_RTN_YD_CD,1,5)" ).append("\n"); 
		query.append(") TMP," ).append("\n"); 
		query.append("( SELECT FM_LOC_CD, SUM(TRF_D2_QTY) TRF_D2_QTY, SUM(TRF_D4_QTY) TRF_D4_QTY," ).append("\n"); 
		...
		query.append("(SELECT FM_LOC_CD, CNT_CD, CUST_SEQ, MAX(EFF_DT) EFF_DT" ).append("\n"); 

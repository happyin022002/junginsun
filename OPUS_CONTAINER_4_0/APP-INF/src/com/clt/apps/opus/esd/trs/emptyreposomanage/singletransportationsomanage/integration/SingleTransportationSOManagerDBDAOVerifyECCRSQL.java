...
public class SingleTransportationSOManagerDBDAOVerifyECCRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                 WHERE L.LSE_CO_YD_CD = @[fm_nod_cd]) C" ).append("\n"); 
		query.append("         WHERE A.DELT_FLG = 'N'" ).append("\n"); 
		query.append("           AND A.SCC_CD = B.SCC_CD" ).append("\n"); 
		query.append("           AND B.LOC_CD = C.LOC_CD) AA" ).append("\n"); 
		...
		query.append("                      ,LOC_CD" ).append("\n"); 
		query.append("                  FROM MDM_YARD Y" ).append("\n"); 
		query.append("                 WHERE Y.YD_CD = @[to_nod_cd]" ).append("\n"); 
		query.append("                UNION ALL" ).append("\n"); 
		query.append("                SELECT LSE_CO_YD_CD" ).append("\n"); 
		query.append("                      ,SUBSTR(LSE_CO_YD_CD, 1, 5)" ).append("\n"); 
		query.append("                  FROM MDM_LSE_CO_YD L" ).append("\n"); 
		query.append("                 WHERE L.LSE_CO_YD_CD = @[to_nod_cd]) C" ).append("\n"); 
		query.append("         WHERE A.DELT_FLG = 'N'" ).append("\n"); 
		query.append("           AND A.SCC_CD = B.SCC_CD" ).append("\n"); 
		query.append("           AND B.LOC_CD = C.LOC_CD) BB" ).append("\n"); 

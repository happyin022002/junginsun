...
*@FileName : EsdTrs0908ViewAdapter.java
...
public class EsdTrs0908ViewAdapter extends TrsDefaultViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix, Event event) {
		...
		String editableStr 		= " EDIT=\"FALSE\"";
		String bgColor 			= " BGCOLOR=\"255,204,153\"";
		...
		for(int i=0;i<realCnt;i++){
			...
			/* apply so가 존재하는 경우 editable 불가 상태로 세팅한다. */
			applySoSeq = 	getNull(colValues.get("apply_so_seq"));
			sbufXML.append("	<TR");
			if (applySoSeq.equals("") || applySoSeq == null){
				sbufXML.append(">");
			}else{
				sbufXML.append(bgColor+editableStr+">");
			}

 ...
 *@FileName : ESD_PRD_0041HTMLAction.java
 ...
 *
 * @author jungsunyoung
 * @see EsdPrd0041Event , ESD_PRD_041EventResponse 참조
 * @since J2EE 1.4
 */
public class ESD_PRD_0041HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = 1L;

	/**
	 * ESD_PRD_0041HTMLAction 객체를 생성
	 */
	public ESD_PRD_0041HTMLAction(){
	}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ESD_PRD_041Event로 파싱하여 request에 셋팅<br>
	 * 
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0041Event event = new EsdPrd0041Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		event.setSearchCallingTmlMtxExptVO((SearchCallingTmlMtxExptVO) getVO(request, SearchCallingTmlMtxExptVO.class));

		if(command.isCommand(FormCommand.MULTI)){
			event.setSearchCallingTmlMtxExptVOs((SearchCallingTmlMtxExptVO[]) getVOs(request, SearchCallingTmlMtxExptVO.class, ""));
		}

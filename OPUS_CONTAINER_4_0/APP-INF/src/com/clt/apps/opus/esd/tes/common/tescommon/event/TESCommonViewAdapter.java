 ...
 * @since J2EE 1.6
 */
public class TESCommonViewAdapter extends ViewAdapter{
	
    /**
     * 
     */
    public TESCommonViewAdapter()
    {
...
protected String makeDataTag(List vos, String prefix, Event event,EventResponse eventResponse)
    {
        ...
        sbufXML.append( (new StringBuilder("<ETC KEY='")).toString() ).append((String)etc_data.get("ETC_KEY_NAME")).append("'><![CDATA[");

		if ( formcommand.isCommand(FormCommand.SEARCH03) ||
			formcommand.isCommand(FormCommand.SEARCH09) || 
			formcommand.isCommand(FormCommand.SEARCH17) ) {
		...
		} else if (formcommand.isCommand(FormCommand.SEARCH19)) {
	        for(int i = 0; i < realCnt; i++) {
	        	Map		colValues	= ((AbstractValueObject)vos.get(i)).getColumnValues();
	        	int		colCnt		= realColNms.length;
    ...
    protected String makeDataTag(DBRowSet rs, String prefix, Event event, EventResponse eventResponse)
    {
        ...
        try
        {
            ...
            sb.append((new StringBuilder("<ETC KEY='")).append((String)etc_data.get("ETC_KEY_NAME")).append("'><![CDATA[").toString());
			...
			} else if (formcommand.isCommand(FormCommand.SEARCH19) || formcommand.isCommand(FormCommand.SEARCH22)) {
                while( rs.next() ) {
                	sb.append( cnt>0?"|":"" );
                    for(int j = 0; j < colCount; j++) {
                    	sb.append( (j>0?"--":"") );

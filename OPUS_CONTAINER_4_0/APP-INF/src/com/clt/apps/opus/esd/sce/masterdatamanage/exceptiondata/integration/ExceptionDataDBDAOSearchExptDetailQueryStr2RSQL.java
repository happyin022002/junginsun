...
*@FileName : ExceptionDataDBDAOSearchExptDetailQueryStr2RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ExceptionDataDBDAOSearchExptDetailQueryStr2RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ExceptionDataDBDAOSearchExptDetailQueryStr2RSQL(){
		setQuery();
		...
		query.append("FileName : ExceptionDataDBDAOSearchExptDetailQueryStr2RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT SUBSTR(MIN(CUR_TOACT_CD),1,8) F_TO_EXPT_CD, SUBSTR(MIN(CUR_TOACT_CD),1,3)||'00000' F_COP_EXPT_TP_DTL_CD" ).append("\n"); 
		query.append(",(CASE WHEN COUNT(SUBSTR(CUR_TOACT_CD,1,8)) >= 2 AND SUBSTR(MIN(CUR_TOACT_CD),9,1) = 'N' AND SUBSTR(MAX(CUR_TOACT_CD),9,1) = 'Y'" ).append("\n"); 
		query.append("THEN 'Y' ELSE SUBSTR(MIN(CUR_TOACT_CD),9,1) END) F_VALIDATION, MAX(F_FM_EXPT_CD) F_FM_EXPT_CD" ).append("\n"); 
		query.append("FROM   (SELECT (CASE WHEN TA.CHK_TOACT = 'Y' THEN MIN(TA.CHK_TOACT_NO)||TA.CHK_TOACT ELSE MAX(TA.CHK_TOACT_NO)||TA.CHK_TOACT END) CUR_TOACT_CD" ).append("\n"); 
		query.append(",MAX(F_FM_EXPT_CD) F_FM_EXPT_CD" ).append("\n"); 
		query.append("FROM   (SELECT --INPUT To Activity Code" ).append("\n"); 
		query.append("(CASE WHEN SUBSTR(T.EXPT_CD,1,4) = SUBSTR(F.F_FM_EXPT_CD,1,3)||'2'" ).append("\n"); 
		query.append("THEN (CASE WHEN SUBSTR(T.EXPT_CD,5,2) = SUBSTR(F.F_FM_EXPT_CD,5,2)" ).append("\n"); 
		query.append("THEN (CASE WHEN T.EXPT_CD_NM = @[f_cop_expt_tp_dtl_nm] THEN 'Y' ELSE 'N' END) 		--'FTWMLO'" ).append("\n"); 
		query.append("ELSE 'N' END )" ).append("\n"); 
		query.append("ELSE  'N'  END ) CHK_TOACT" ).append("\n"); 
		query.append(",(CASE WHEN SUBSTR(T.EXPT_CD,1,4) = SUBSTR(F.F_FM_EXPT_CD,1,3)||'2'" ).append("\n"); 
		query.append("THEN (CASE WHEN SUBSTR(T.EXPT_CD,5,2) = SUBSTR(F.F_FM_EXPT_CD,5,2)" ).append("\n"); 
		query.append("THEN (CASE WHEN T.EXPT_CD_NM = @[f_cop_expt_tp_dtl_nm] THEN MAX(T.EXPT_CD) ELSE MAX(TO_NUMBER(T.EXPT_CD)+1)||'' END) 		--'FTWMLO'" ).append("\n"); 
		query.append("ELSE SUBSTR(F.F_FM_EXPT_CD,1,3)||'2'||SUBSTR(F.F_FM_EXPT_CD,5,2)||'01' END )" ).append("\n"); 
		query.append("ELSE  SUBSTR(F.F_FM_EXPT_CD,1,3)||'2'||SUBSTR(F.F_FM_EXPT_CD,5,2)||'01'" ).append("\n"); 
		query.append("END )  CHK_TOACT_NO" ).append("\n"); 
		query.append(", F.F_FM_EXPT_CD" ).append("\n"); 
		query.append("FROM   SCE_EXPT_CD T," ).append("\n"); 
		query.append("(" ).append("\n"); 
		...
		query.append("WHERE  T.ACT_FLG = 'Y'" ).append("\n"); 
		query.append("AND    SUBSTR(T.EXPT_CD,1,1)  = SUBSTR(F.F_FM_EXPT_CD,1,1)" ).append("\n"); 
		query.append("GROUP BY T.EXPT_CD, F.F_FM_EXPT_CD, T.EXPT_CD_NM) TA" ).append("\n"); 
		query.append("GROUP BY TA.CHK_TOACT )" ).append("\n"); 

...
public class CARIssueTransferSlipManageDBDAOModifyApInvDTRBLineNoUpdate03RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT ATTR_CTNT1, DTRB_COA_ACCT_CD, DTRB_COA_VVD_CD, ATTR_CTNT7, ROWNUM LINE_NO" ).append("\n"); 
		query.append(" FROM (" ).append("\n"); 
		query.append("	SELECT DISTINCT ATTR_CTNT1, DTRB_COA_ACCT_CD, DTRB_COA_VVD_CD, ATTR_CTNT7" ).append("\n"); 
		query.append("	FROM   AP_INV_DTRB" ).append("\n"); 
		query.append("	WHERE  CSR_NO = @[csr_no]" ).append("\n"); 
		query.append("	#if (${cnt_cd} != 'JP')" ).append("\n"); 

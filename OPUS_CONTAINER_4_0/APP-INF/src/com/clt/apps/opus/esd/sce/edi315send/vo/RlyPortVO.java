/*=========================================================
*Copyright(c) 2010 CyberLogitec
*@FileName : RlyPortVO.java
*@FileTitle : RlyPortVO
*Open Issues :
*Change history :
*@LastModifyDate : 2010.04.05
*@LastModifier : 이윤정
*@LastVersion : 1.0
* 2010.04.05 이윤정 
* 1.0 Creation
=========================================================*/

package com.clt.apps.opus.esd.sce.edi315send.vo;

import java.util.ArrayList;
import java.util.Collection;
import java.util.HashMap;

import javax.servlet.http.HttpServletRequest;

import org.apache.commons.lang.builder.ToStringBuilder;
import org.apache.commons.lang.builder.ToStringStyle;

import com.clt.framework.component.common.AbstractValueObject;
import com.clt.framework.component.util.JSPUtil;

/**
 * Table Value Ojbect<br>
 * 관련 Event 에서 생성, 서버실행요청시 Data 전달역할을 수행하는 Value Object
 *
 * @author 이윤정
 * @since J2EE 1.6
 * @see AbstractValueObject
 */

public class RlyPortVO extends AbstractValueObject {

	private static final long serialVersionUID = 1L;
	
	private Collection<RlyPortVO> models = new ArrayList<RlyPortVO>();
	
	/* Column Info */
	private String rlyAmsport = null;
	/* Column Info */
	private String rlyCode = null;
	/* VO Data Value( C:Creation, U:Update, D:Delete ) */
	private String ibflag = null;
	/* Column Info */
	private String rlyPort = null;
	/* Column Info */
	private String rlyName = null;
	/* Column Info */
	private String rlyAmsqual = null;
	/* Page Number */
	private String pagerows = null;

	/*	테이블 컬럼의 값을 저장하는 Hashtable */
	private HashMap<String, String> hashColumns = new HashMap<String, String>();

	/*	테이블 컬럼에 대응되는 멤버변수를 저장하는 Hashtable */
	private HashMap<String, String> hashFields = new HashMap<String, String>();
	
	public RlyPortVO() {}

	public RlyPortVO(String ibflag, String pagerows, String rlyName, String rlyPort, String rlyCode, String rlyAmsqual, String rlyAmsport) {
		this.rlyAmsport = rlyAmsport;
		this.rlyCode = rlyCode;
		this.ibflag = ibflag;
		this.rlyPort = rlyPort;
		this.rlyName = rlyName;
		this.rlyAmsqual = rlyAmsqual;
		this.pagerows = pagerows;
	}
	
	/**
	 * 테이블 컬럼에 저장할 값을 Hashtable<"column_name", "value"> 로 반환
	 * @return HashMap
	 */
	public HashMap<String, String> getColumnValues(){
		this.hashColumns.put("rly_amsport", getRlyAmsport());
		this.hashColumns.put("rly_code", getRlyCode());
		this.hashColumns.put("ibflag", getIbflag());
		this.hashColumns.put("rly_port", getRlyPort());
		this.hashColumns.put("rly_name", getRlyName());
		this.hashColumns.put("rly_amsqual", getRlyAmsqual());
		this.hashColumns.put("pagerows", getPagerows());
		return this.hashColumns;
	}
	
	/**
	 * 컬럼명에 대응되는 멤버변수명을 저장하여 Hashtable<"column_name", "variable"> 로 반환   
	 * @return
	 */
	public HashMap<String, String> getFieldNames(){
		this.hashFields.put("rly_amsport", "rlyAmsport");
		this.hashFields.put("rly_code", "rlyCode");
		this.hashFields.put("ibflag", "ibflag");
		this.hashFields.put("rly_port", "rlyPort");
		this.hashFields.put("rly_name", "rlyName");
		this.hashFields.put("rly_amsqual", "rlyAmsqual");
		this.hashFields.put("pagerows", "pagerows");
		return this.hashFields;
	}
	
	/**
	 * Column Info
	 * @return rlyAmsport
	 */
	public String getRlyAmsport() {
		return this.rlyAmsport;
	}
	
	/**
	 * Column Info
	 * @return rlyCode
	 */
	public String getRlyCode() {
		return this.rlyCode;
	}
	
	/**
	 * VO Data Value( C:Creation, U:Update, D:Delete )
	 * @return ibflag
	 */
	public String getIbflag() {
		return this.ibflag;
	}
	
	/**
	 * Column Info
	 * @return rlyPort
	 */
	public String getRlyPort() {
		return this.rlyPort;
	}
	
	/**
	 * Column Info
	 * @return rlyName
	 */
	public String getRlyName() {
		return this.rlyName;
	}
	
	/**
	 * Column Info
	 * @return rlyAmsqual
	 */
	public String getRlyAmsqual() {
		return this.rlyAmsqual;
	}
	
	/**
	 * Page Number
	 * @return pagerows
	 */
	public String getPagerows() {
		return this.pagerows;
	}
	

	/**
	 * Column Info
	 * @param rlyAmsport
	 */
	public void setRlyAmsport(String rlyAmsport) {
		this.rlyAmsport = rlyAmsport;
	}
	
	/**
	 * Column Info
	 * @param rlyCode
	 */
	public void setRlyCode(String rlyCode) {
		this.rlyCode = rlyCode;
	}
	
	/**
	 * VO Data Value( C:Creation, U:Update, D:Delete )
	 * @param ibflag
	 */
	public void setIbflag(String ibflag) {
		this.ibflag = ibflag;
	}
	
	/**
	 * Column Info
	 * @param rlyPort
	 */
	public void setRlyPort(String rlyPort) {
		this.rlyPort = rlyPort;
	}
	
	/**
	 * Column Info
	 * @param rlyName
	 */
	public void setRlyName(String rlyName) {
		this.rlyName = rlyName;
	}
	
	/**
	 * Column Info
	 * @param rlyAmsqual
	 */
	public void setRlyAmsqual(String rlyAmsqual) {
		this.rlyAmsqual = rlyAmsqual;
	}
	
	/**
	 * Page Number
	 * @param pagerows
	 */
	public void setPagerows(String pagerows) {
		this.pagerows = pagerows;
	}
	
/**
	 * Request 의 데이터를 추출하여 VO 의 멤버변수에 설정.
	 * @param request
	 */
	public void fromRequest(HttpServletRequest request) {
		fromRequest(request,"");
	}

	/**
	 * Request 의 데이터를 추출하여 VO 의 멤버변수에 설정.
	 * @param request
	 */
	public void fromRequest(HttpServletRequest request, String prefix) {
		setRlyAmsport(JSPUtil.getParameter(request, prefix + "rly_amsport", ""));
		setRlyCode(JSPUtil.getParameter(request, prefix + "rly_code", ""));
		setIbflag(JSPUtil.getParameter(request, prefix + "ibflag", ""));
		setRlyPort(JSPUtil.getParameter(request, prefix + "rly_port", ""));
		setRlyName(JSPUtil.getParameter(request, prefix + "rly_name", ""));
		setRlyAmsqual(JSPUtil.getParameter(request, prefix + "rly_amsqual", ""));
		setPagerows(JSPUtil.getParameter(request, prefix + "pagerows", ""));
	}

	/**
	 * Request 의 데이터를 VO 배열로 변환하여 반환.
	 * @param request
	 * @return RlyPortVO[]
	 */
	public RlyPortVO[] fromRequestGrid(HttpServletRequest request) {
		return fromRequestGrid(request, "");
	}

	/**
	 * Request 넘어온 여러 건 DATA를 VO Class 에 담는다. 
	 * @param request
	 * @param prefix
	 * @return RlyPortVO[]
	 */
	public RlyPortVO[] fromRequestGrid(HttpServletRequest request, String prefix) {
		RlyPortVO model = null;
		
		String[] tmp = request.getParameterValues(prefix + "ibflag");
  		if(tmp == null)
   			return null;

  		int length = request.getParameterValues(prefix + "ibflag").length;
  
		try {
			String[] rlyAmsport = (JSPUtil.getParameter(request, prefix	+ "rly_amsport", length));
			String[] rlyCode = (JSPUtil.getParameter(request, prefix	+ "rly_code", length));
			String[] ibflag = (JSPUtil.getParameter(request, prefix	+ "ibflag", length));
			String[] rlyPort = (JSPUtil.getParameter(request, prefix	+ "rly_port", length));
			String[] rlyName = (JSPUtil.getParameter(request, prefix	+ "rly_name", length));
			String[] rlyAmsqual = (JSPUtil.getParameter(request, prefix	+ "rly_amsqual", length));
			String[] pagerows = (JSPUtil.getParameter(request, prefix	+ "pagerows", length));
			
			for (int i = 0; i < length; i++) {
				model = new RlyPortVO();
				if (rlyAmsport[i] != null)
					model.setRlyAmsport(rlyAmsport[i]);
				if (rlyCode[i] != null)
					model.setRlyCode(rlyCode[i]);
				if (ibflag[i] != null)
					model.setIbflag(ibflag[i]);
				if (rlyPort[i] != null)
					model.setRlyPort(rlyPort[i]);
				if (rlyName[i] != null)
					model.setRlyName(rlyName[i]);
				if (rlyAmsqual[i] != null)
					model.setRlyAmsqual(rlyAmsqual[i]);
				if (pagerows[i] != null)
					model.setPagerows(pagerows[i]);
				models.add(model);
			}

		} catch (Exception e) {
			return null;
		}
		return getRlyPortVOs();
	}

	/**
	 * VO 배열을 반환
	 * @return RlyPortVO[]
	 */
	public RlyPortVO[] getRlyPortVOs(){
		RlyPortVO[] vos = (RlyPortVO[])models.toArray(new RlyPortVO[models.size()]);
		return vos;
	}
	
	/**
	 * VO Class의 내용을 String으로 변환
	 */
	public String toString() {
	   return ToStringBuilder.reflectionToString(this, ToStringStyle.MULTI_LINE_STYLE );
    }

	/**
	* 포맷팅된 문자열에서 특수문자 제거("-","/",",",":")
	*/
	public void unDataFormat(){
		this.rlyAmsport = this.rlyAmsport .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.rlyCode = this.rlyCode .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.ibflag = this.ibflag .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.rlyPort = this.rlyPort .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.rlyName = this.rlyName .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.rlyAmsqual = this.rlyAmsqual .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
		this.pagerows = this.pagerows .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
	}
}

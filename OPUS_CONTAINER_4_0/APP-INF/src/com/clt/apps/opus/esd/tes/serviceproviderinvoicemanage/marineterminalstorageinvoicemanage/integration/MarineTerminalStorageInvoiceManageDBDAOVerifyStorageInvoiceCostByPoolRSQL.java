...
public class MarineTerminalStorageInvoiceManageDBDAOVerifyStorageInvoiceCostByPoolRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("NVL(CALC.LGS_COST_CD,'SRNDMT') LGS_COST_CD, NVL(CALC.LGS_COST_CD,'SRNDMT') LGS_COST_CD2," ).append("\n"); 
		...
		query.append("NVL(CALC.UOM,AGMT.UOM) VOL_TR_UT_CD," ).append("\n"); 
		query.append("NVL(CALC.AGMT_UT_RT,AGMT.AGMT_UT_RT) CTRT_RT," ).append("\n"); 
		...
		query.append("NVL(CALC.STACKING_VOL,0) STK_VOL_QTY, NVL(CALC.STACKING_VOL,0) - NVL(ORG.INV_VOL_QTY,0) DIFF_VOL_QTY," ).append("\n"); 
		...
		query.append("FROM ( SELECT 'SRNDMT' LGS_COST_CD," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${mode} == 'M')" ).append("\n"); 
		query.append("SUBSTR(STAY_SMRY_DT,0,6) M_Date," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("SUBSTR(STAY_SMRY_DT,0,8) M_Date," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'D7',CNTR_KNT_QTY,0))*2.25+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'D8',CNTR_KNT_QTY,0))*2.4+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'D9',CNTR_KNT_QTY,0))*2.4+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'DW',CNTR_KNT_QTY,0))*2.65+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'DX',CNTR_KNT_QTY,0))*2.25+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'D7',0,'D8',0,'D9',0,'DW',0,'DX',0,DECODE(SUBSTR(M.CNTR_TPSZ_CD,2,1),2,M.CNTR_KNT_QTY,0)))+" ).append("\n"); 
		query.append("SUM(DECODE(M.CNTR_TPSZ_CD,'D7',0,'D8',0,'D9',0,'DW',0,'DX',0,DECODE(SUBSTR(M.CNTR_TPSZ_CD,2,1),2,0,M.CNTR_KNT_QTY*2))) Stacking_Vol," ).append("\n"); 
		...
		query.append("FROM   TES_EQ_LAND_STAY M, TES_TML_AGMT_HDR H, TES_TML_AGMT_DTL D, TES_FILE_IMP_TMP P" ).append("\n"); 
		query.append("WHERE  M.STAY_SMRY_DT  >= DECODE(@[mode],'D',P.WRK_DT,P.WRK_DT||'01')" ).append("\n"); 
		query.append("AND    M.STAY_SMRY_DT  <= DECODE(@[mode],'D',P.WRK_DT,TO_CHAR(LAST_DAY(TO_DATE(P.WRK_DT,'YYYYMM')),'YYYYMMDD'))" ).append("\n"); 
		...
		query.append("GROUP BY SUBSTR(STAY_SMRY_DT,0,6), P.INV_VOL_QTY," ).append("\n"); 
		...
		query.append("GROUP BY STAY_SMRY_DT, P.INV_VOL_QTY," ).append("\n"); 
		query.append("H.TML_AGMT_OFC_CTY_CD, H.TML_AGMT_SEQ, H.TML_AGMT_VER_NO," ).append("\n"); 
		query.append("D.FP_TEU_QTY, D.TML_AGMT_VOL_UT_CD, D.AGMT_UT_RT, D.CURR_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 

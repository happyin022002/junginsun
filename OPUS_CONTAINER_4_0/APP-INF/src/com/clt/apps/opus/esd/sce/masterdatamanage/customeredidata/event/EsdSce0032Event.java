...
public class EsdSce0032Event extends EventSupport{
	...
	public void setSchCusts(SearchCustomerDataVO[] schCusts){
		if(schCusts != null){
			SearchCustomerDataVO[] tmpVOs = Arrays.copyOf(schCusts, schCusts.length);
			this.schCusts = tmpVOs;
		}
	}
	...
	public SearchCustomerDataVO[] getSchCusts(){
		SearchCustomerDataVO[] rtnVOs = null;
		if (this.schCusts != null) {
			rtnVOs = Arrays.copyOf(schCusts, schCusts.length);
		}
		return rtnVOs;
	}
	...
	public void setSchEdits(SearchEdiStatusDataVO[] schEdits){
		if(schEdits != null){
			SearchEdiStatusDataVO[] tmpVOs = Arrays.copyOf(schEdits, schEdits.length);
			this.schEdits = tmpVOs;
		}
	}
	...
	public SearchEdiStatusDataVO[] getSchEdits(){
		SearchEdiStatusDataVO[] rtnVOs = null;
		if (this.schEdits != null) {
			rtnVOs = Arrays.copyOf(schEdits, schEdits.length);
		}
		return rtnVOs;
	}	

...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.trs.agreementmanage.agreementmanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_TRS_0220HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH03) 
			...
			event.setSearchAgmtHdrVOs(searchAgmtHdrVO.fromRequestGrid(request));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setTrsAgmtAplyVndrVOs(trsAgmtAplyVndrVO.fromRequestGrid(request));
		}

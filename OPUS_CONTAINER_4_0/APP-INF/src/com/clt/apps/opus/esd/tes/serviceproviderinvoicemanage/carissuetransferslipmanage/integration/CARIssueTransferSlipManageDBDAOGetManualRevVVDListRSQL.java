...
public class CARIssueTransferSlipManageDBDAOGetManualRevVVDListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		  GROUP BY D.LGS_COST_CD, D.TML_SO_OFC_CTY_CD, D.TML_SO_SEQ, D.TML_SO_DTL_SEQ, H.ISS_DT, H.TML_INV_TP_CD, D.CALC_TP_CD, D.REV_YRMON, H.FM_PRD_DT" ).append("\n"); 
		...
		query.append("--		                 DECODE(NVL(A.TML_CRR_CD,'HJS'),'HJS'," ).append("\n"); 
		query.append("--		                   DECODE(R.SLAN_DIR_CD||R.RLANE_DIR_CD,NULL," ).append("\n"); 
		...
		query.append("				 WHEN NVL(A.TML_CRR_CD,'NYK') = 'NYK'" ).append("\n"); 
		query.append("				 THEN" ).append("\n"); 
		query.append("					CASE" ).append("\n"); 
		...
		query.append("						WHERE M.VSL_CD = DECODE(NVL(A.TML_CRR_CD,'NYK'),'NYK',A.VSL_CD,'CNTC')" ).append("\n"); 
		query.append("						AND M.SKD_VOY_NO = DECODE(NVL(A.TML_CRR_CD,'NYK'),'NYK',A.SKD_VOY_NO,TO_CHAR(A.ATB_DT,'YYMM'))" ).append("\n"); 
		...
		query.append("					FROM   TES_TML_SO_HDR H, TES_TML_SO_DTL D, TES_TML_SO_COST C, AR_MST_REV_VVD V, MDM_LOCATION S" ).append("\n"); 
		query.append("					WHERE  H.INV_NO             = @[inv_no]" ).append("\n"); 
		query.append("					AND    H.VNDR_SEQ           = @[vndr_seq]" ).append("\n"); 
		query.append("					AND    H.TML_SO_OFC_CTY_CD  = D.TML_SO_OFC_CTY_CD(+)" ).append("\n"); 
		query.append("					AND    H.TML_SO_SEQ         = D.TML_SO_SEQ(+)" ).append("\n"); 

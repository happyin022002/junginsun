...
public class MarineTerminalInvoiceManageDBDAOSearchCostCodeListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("WITH V_MDM_YARD AS (" ).append("\n"); 
		query.append("        SELECT MY.YD_CD" ).append("\n"); 
		query.append("             , MY.YD_FCTY_TP_CFS_FLG AS YD_FCTY_TP_CFS_FLG" ).append("\n"); 
		query.append("             , MY.YD_FCTY_TP_RAIL_RMP_FLG AS YD_FCTY_TP_RAIL_RMP_FLG" ).append("\n"); 
		query.append("             , MY.YD_OSHP_CD AS YD_OSHP_CD" ).append("\n"); 
		query.append("          FROM MDM_YARD MY" ).append("\n"); 
		query.append("         WHERE 1=1" ).append("\n"); 
		query.append("           AND MY.YD_CD = @[yd_cd] " ).append("\n"); 
		query.append("    ) " ).append("\n"); 
		query.append("SELECT A.COST_CODE" ).append("\n"); 
		...
		query.append("     , CASE WHEN A.COST_CODE IN ('CGCFLS','CGCFMW','CGCFOH','CGCFOL','CGCFSF','CGCFSR','CGCFTL') AND YD_FCTY_TP_CFS_FLG <> 'Y' THEN 'CFS'" ).append("\n"); 
		query.append("            WHEN A.COST_CODE IN ('TMNDRF','TMNDRM') AND YD_FCTY_TP_RAIL_RMP_FLG <> 'Y' THEN 'RAIL'" ).append("\n"); 
		query.append("            WHEN A.COST_CODE IN ('ETXXDC','ETMREQ') AND YD_OSHP_CD <> 'O' THEN 'EXT'" ).append("\n"); 
		query.append("            ELSE NULL" ).append("\n"); 
		query.append("       END AS CHK_YD_FLG" ).append("\n"); 
		...
		query.append("	AND    TO_CHAR(H.EFF_FM_DT,'YYYYMMDD') <= DECODE(@[min_wrk_dt],'',REPLACE(@[iss_dt], '-'), REPLACE(@[min_wrk_dt],'-'))" ).append("\n"); 
		query.append("	AND    TO_CHAR(H.EFF_TO_DT,'YYYYMMDD') >= DECODE(@[max_wrk_dt],'',REPLACE(@[iss_dt], '-'), REPLACE(@[max_wrk_dt],'-'))" ).append("\n"); 
		...
		query.append("#elseif (${cost_cd_inv_tp_cd} == 'OS' || ${cost_cd_inv_tp_cd} == 'ST') " ).append("\n"); 
		...
		query.append("#elseif (${cost_cd_inv_tp_cd} == 'ON')" ).append("\n"); 
		query.append("									AND    TO_CHAR(M.EFF_FM_DT,'YYYYMMDD') <= DECODE(@[min_wrk_dt],'',REPLACE(@[iss_dt], '-'), REPLACE(@[min_wrk_dt],'-'))" ).append("\n"); 
		query.append("									AND    TO_CHAR(M.EFF_TO_DT,'YYYYMMDD') >= DECODE(@[max_wrk_dt],'',REPLACE(@[iss_dt], '-'), REPLACE(@[max_wrk_dt],'-'))" ).append("\n"); 
		query.append("#elseif (${cost_cd_inv_tp_cd} == 'OT') " ).append("\n"); 
		...
		query.append("WHERE 1=1" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (!(${cost_cd_inv_tp_cd} == 'OS' || ${cost_cd_inv_tp_cd} == 'ST'))" ).append("\n"); 
		query.append("  AND C.COST_CALC_MZD_CD = 'M' " ).append("\n"); 
		query.append("  AND C.STO_INV_FLG = 'N' " ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("  AND DECODE(@[cost_cd_inv_tp_cd],'MT',MRN_TML_FLG,'ON',ODCK_RAIL_CHG_FLG,'OT',FDCK_CY_TML_FLG,'OS',FDCK_CY_STO_FLG,'ST',STO_INV_FLG,'SE',STO_EQ_INV_FLG,'OE',OFF_DCK_STO_EQ_FLG)  = 'Y'" ).append("\n"); 
		...
		query.append("       ) A" ).append("\n"); 
		query.append("     , V_MDM_YARD Y" ).append("\n"); 
		query.append(" WHERE 1=1 " ).append("\n"); 

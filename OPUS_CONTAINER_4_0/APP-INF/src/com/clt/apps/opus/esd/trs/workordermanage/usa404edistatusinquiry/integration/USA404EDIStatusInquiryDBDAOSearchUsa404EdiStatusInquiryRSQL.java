...
public class USA404EDIStatusInquiryDBDAOSearchUsa404EdiStatusInquiryRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    MAX(DECODE(M.CGO_TP_CD, 'M', M.INTER_RMK, 'F', DECODE(M.INTER_RMK_CHK, '', '', 'Y'))) INTER_RMK," ).append("\n"); 
		...
		query.append("   (SELECT REPLACE(REPLACE(CUST_NM, CHR (13),' '), CHR (10), ' ') FROM BKG_CUSTOMER CUST WHERE CUST.BKG_NO = M.BKG_NO AND CUST.BKG_CUST_TP_CD = 'S') AS SHPR_CUST_NM," ).append("\n"); 
		query.append("   (SELECT REPLACE(REPLACE(CUST_NM, CHR (13),' '), CHR (10), ' ') FROM BKG_CUSTOMER CUST WHERE CUST.BKG_NO = M.BKG_NO AND CUST.BKG_CUST_TP_CD = 'C') AS CNEE_CUST_NM," ).append("\n"); 
		...
		query.append("    CASE WHEN MAX(M.BKG_CGO_TP_CD) = 'F' THEN '1' ELSE '0' END POP_IMG," ).append("\n"); 
		...
		query.append("                    MAX(DECODE(NVL(C.BIL_ISS_STS_CD, A.TRSP_SO_STS_CD) , 'I', 'EDI Sent' , 'C', 'S/O Created' , 'R', 'S/O Corrected', 'S/O Created')) AS TRSP_SO_STS_CD," ).append("\n"); 
		query.append("                    MAX(A.TRSP_RAIL_BIL_TP_CD) AS TRSP_RAIL_BIL_TP_CD,                    " ).append("\n"); 
		...
		query.append("                AND   A.CRE_OFC_CD = @[ctrlOfcCd]" ).append("\n"); 
		...
		query.append("            X.TRSP_FRST_FLG," ).append("\n"); 
		query.append("            X.WO_RJCT_RSN,            " ).append("\n"); 
		query.append("           ( SELECT AGMT.AGMT_REF_NO AS REF_NO FROM  TRS_AGMT_HDR AGMT  WHERE X.TRSP_AGMT_OFC_CTY_CD = AGMT.TRSP_AGMT_OFC_CTY_CD  AND   X.TRSP_AGMT_SEQ = AGMT.TRSP_AGMT_SEQ ) REF_NO," ).append("\n"); 
		query.append("            X.BKG_CNTR_ATTACHED," ).append("\n"); 
		query.append("            X.IBD_CSTMS_CLR_LOC_CD," ).append("\n"); 
		query.append("            X.POL_NOD_CD," ).append("\n"); 
		query.append("            X.POD_NOD_CD," ).append("\n"); 
		...
		query.append("            X.AUTO_XPT_SYS_CD," ).append("\n"); 
		query.append("            X.AUTO_XPT_SYS_NO" ).append("\n"); 
		query.append("            ,( SELECT REPLACE(MDM_CO.CMDT_NM, CHR(13)||CHR(10), ' ') AS CMDT_NAME FROM MDM_COMMODITY MDM_CO, BKG_BOOKING B WHERE X.BKG_NO = B.BKG_NO AND   B.CMDT_CD = MDM_CO.CMDT_CD ) CMDT_NAME" ).append("\n"); 
		query.append("            ,(" ).append("\n"); 
		query.append("                SELECT" ).append("\n"); 
		query.append("                    DECODE( BK.DCGO_FLG , 'Y', 'DG /') || " ).append("\n"); 
		query.append("                    DECODE( BK.RC_FLG , 'Y', 'RF /') || " ).append("\n"); 

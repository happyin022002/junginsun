...
public interface BkgCopManageBC {
	...
	/**
	 * 미주 Outbound, 구주 In / outbound 의 carrier's haulage 의 경우 booking 에서 confirm 된 TRO 정보를 받아 SCE_TRO_MAPG, SCE_COP_HDR 에 해당 정보를 관리한다.
	 * 
	 * @param bkg_no
	 * @param tro_seq
	 * @param tro_sub_seq
	...
	/**
	 * TRO 가 unconfirm 되었을 경우 SCE_COP_HDR 의 flag 변경, SCE_TRO_MAPG 정보 삭제를 수행한다. Booking 에서 term 이 변경될 때 Door S/O 가 발행되지 않은 TRO 에 대해 자동으로 unconfirm 을 진행하며 이때 booking 단위 일괄 정리를 위해 호출한다.
	 * 
	 * @param bkg_no
	 * @param io_bnd_cd
	 * @throws EventException
	...
	/**
	 * 각종 BOOKING REPLAN 작업이 있을 때 호출 되고 PRD 에서 생성한 PRD_BKG_COP_MAP TABLE 의 OPERATION 명세 대로 REPLAN 을 수행한다. (QTY 증감, CNTR TPSZ 변경, TERM 변경, ROUTE 재 생성 등 각종 BOOKING 변경 사항)
	 * 
	 * @param bkg_no
	 * @param mapg_seq
	 * @throws EventException
	...
	/**
	 * booking combine 시 호출되어 org_bkg_no 에 할당된 COP 를 combined_bkg_no 로 소속 변경 org_bkg_no 의 COP 가 combined_bkg_no 에 존재하는 COP 와 동일한 container 를 가진 경우 등 partial 관계를 적용하여 booking no 의 변경 / cancel 등의 작업을 수행하며 Master cop no 의 정제 및 기 발행된 S/O 의 booking no 수정을 수행한다.
	 * 
	 * @param org_bkg_no
	...
	/**
	 * booking split 화면에서 호출되며 PRD 에서 생성한 PRD_BKG_COP_MAP 의 OPERATION 명세에 따라 split 작업을 수행한다. VVD 가 변경될 경우는 본 method 호출 후 updateBkg method 를 booking 에서 재 호출한다.
	 * 
	 * @param org_bkg_no
	 * @param new_bkg_no
	 * @param mapg_seq
	...
	/**
	 * Container 를 org_bkg_no 의 booking 에서 (복수개의) dest_bkg_no 인 booking 으로 이동시킨다. 이때 booking 의 partial 여부에 따라 master cop no 의 정제가 이뤄진다. (S/O 의 booking no 변경도 병행)
	 * 
	 * @param org_bkg_no
	 * @param dest_bkg_no
	 * @param cntr_no
	...
	/**
	 * container confirm 시 혹시 누락된 attach / detach 현황이 있는지 확인하여 재 작업하고 qty 정제 작업도 수행한다.
	 * 
	 * @param bkg_no
	 * @throws EventException
	 */
	...
	/**
	 * booking split, create, copy 등 booking no 가 신규 생성 / 변경되는 case 에서 booking 에서 호출하여 SCE_COP_HDR 의 rail_rcv_coff_fm_dt, rail_rcv_coff_to_dt 를 변경한다.
	 * 
	 * @param bkg_no

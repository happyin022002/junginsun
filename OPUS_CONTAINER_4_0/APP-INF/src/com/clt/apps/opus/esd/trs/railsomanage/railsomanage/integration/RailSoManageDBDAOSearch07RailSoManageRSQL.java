...
public class RailSoManageDBDAOSearch07RailSoManageRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	SELECT " ).append("\n"); 
		query.append("		EQ_NO, " ).append("\n"); 
		query.append("		SUBSTR(  TO_CHAR(LOCL_CRE_DT, 'YYYYMMDD')|| 'COMPANY - CNTR_NO: ' " ).append("\n"); 
		...
		query.append("			WHEN SO.TRSP_BND_CD IS NULL " ).append("\n"); 
		query.append("			THEN DECODE(SIGN(TRUNC(GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC(@[sofficeCd]) - LOCL_CRE_DT )-3), -1, 'Y', 0, 'Y', 'N') " ).append("\n"); 

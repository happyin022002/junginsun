...
public class GuaranteeManageDBDAOSearchUSGuaranteeListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		, CASE	WHEN NVL(DELT_FLG, 'N') <> 'Y' " ).append("\n"); 
		query.append("				THEN TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT OFC_CD FROM TABLE (COM_OfficeCodeMgr_PKG.COM_getOfficeCodeList_FNC('000001', 'TES'))), GH.CRE_DT, GH.OFC_CD), 'YYYY-MM-DD')" ).append("\n"); 
		query.append("				ELSE TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT OFC_CD FROM TABLE (COM_OfficeCodeMgr_PKG.COM_getOfficeCodeList_FNC('000001', 'TES'))), GH.UPD_DT, GH.OFC_CD), 'YYYY-MM-DD')" ).append("\n"); 
		...
		query.append("	AND	GH.YD_CD = @[yd_cd]" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	" ).append("\n"); 
		query.append("	AND	GH.CRE_DT BETWEEN TO_DATE(@[fm_cre_dt], 'YYYY-MM-DD') + (540/24/60 - GLOBALDATE_PKG.GET_GMTHRS_FNC(@[ofc_cd])/24/60 ) " ).append("\n"); 
		query.append("                  AND TO_DATE(@[to_cre_dt], 'YYYY-MM-DD') + (540/24/60 - GLOBALDATE_PKG.GET_GMTHRS_FNC(@[ofc_cd])/24/60) + 0.99999" ).append("\n"); 

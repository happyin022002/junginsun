...
public class OceanRouteManageDBDAOSearchOceanRouteSingleCreationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , @[s_pol] ORG_LOC_CD" ).append("\n"); 
		query.append("     , @[s_pod] DEST_LOC_CD" ).append("\n"); 
		...
		query.append("     , TT1 + TT2 + TT3 + TT4 TOT_TT" ).append("\n"); 
		...
		query.append("                   , DECODE(LINK_POS, MAX(DECODE(LINK_POS, 5, 0, LINK_POS)) OVER (), @[s_pod], POD_CD) POD_CD" ).append("\n"); 
		query.append("                   , MAX(DECODE(LINK_POS, 5, 0, LINK_POS)) OVER () LINK_COUNT" ).append("\n"); 
		query.append("              FROM (" ).append("\n"); 
		query.append("                  SELECT CPY_NO LINK_POS" ).append("\n"); 
		query.append("                       , DECODE(CPY_NO, 1, @[s_pol], 2, @[s_ts1_port], 3, @[s_ts2_port], 4, @[s_ts3_port], 5, @[s_pod] ) POL_CD" ).append("\n"); 
		query.append("                       , DECODE(CPY_NO, 1, @[s_ts1_lane], 2, @[s_ts2_lane], 3, @[s_ts3_lane], 4, @[s_ts4_lane] ) SLAN_CD" ).append("\n"); 
		query.append("                       , DECODE(CPY_NO, 1, @[s_ts1_port], 2, @[s_ts2_port], 3, @[s_ts3_port] ) POD_CD" ).append("\n"); 
		...
		query.append("         , MAX(LINK_COUNT) LINK_COUNT" ).append("\n"); 
		query.append("         , CASE WHEN @[s_pod] IS NULL THEN 'N'" ).append("\n"); 
		...
		query.append("                                   AND A.SKD_DIR_CD   = TG.N1ST_SKD_DIR_CD" ).append("\n"); 
		...
		query.append("              AND NOT EXISTS ( SELECT 'X' FROM PRD_FDR_LNK TFS WHERE TFS.LNK_ORG_LOC_CD  = @[s_pol] AND TFS.LNK_DEST_LOC_CD = @[s_pod] )" ).append("\n"); 
		query.append("            )" ).append("\n"); 

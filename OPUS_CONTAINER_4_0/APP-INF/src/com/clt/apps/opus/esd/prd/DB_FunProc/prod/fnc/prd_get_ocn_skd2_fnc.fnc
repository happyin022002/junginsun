CREATE OR REPLACE FUNCTION OPUSADM.PRD_GET_OCN_SKD2_FNC
...
/* ========================================================
   ...
   7. Revision History : 2010.05.06 최초 생성
                         2010.10.19 FDR VVD를 BKG MAIN상 T.VVD에 입력한 경우 적용하도록(T_LAN_ORD) 수정
                         2011.03.14 FDR에 대해 VVD SKD조회 후 RHQ LINK 조회하도록 수정. 단 FDR DIRECT는 RHQ LINK조회 안함
                         2011.04.04 RHQ LINK로 첫 FDR읽었을 경우 VVD1이 없는 것을 이용해서, 첫 RHQ 재처리가 되도록 변경
======================================================== */

(
    ----------------------------------------PARAMETERS----------------------------------------
    V_D_PARA        IN VARCHAR2,   -- Loading Due Time ('20061115123000') / VVD (COME040E)
         ...
         FROM 
           ...
           SELECT  V.CNN_BUF_HRS
            ...
            WHERE  V.DCHG_CNT_CD = SUBSTR(V_POD1,1,2)
              ...
              AND  V.DCHG_SLAN_CD = DECODE(V.DCHG_SLAN_CD,'ALL','ALL',V_LANE1 )
              ...
              AND  NVL(V.LOD_SLAN_CD,'ALL')  = DECODE(NVL(V.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE2 )
                ...
                = (
                  SELECT MIN ( DECODE(V2.DCHG_TML_CD,'ALL',10000,0) + DECODE(V2.DCHG_SLAN_CD,'ALL',1000,0) + DECODE(NVL(V2.LOD_CNT_CD,'ALL'),'ALL',100,0) +
                   ...
                   WHERE 	V2.DCHG_CNT_CD 							=  V.DCHG_CNT_CD
                     ...
                     AND  	V2.DCHG_SLAN_CD 						=  DECODE(V2.DCHG_SLAN_CD,'ALL','ALL',V_LANE1 )
                     ...
                     AND  	NVL(V2.LOD_SLAN_CD,'ALL')  				=  DECODE(NVL(V2.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE2 )
                   )
             UNION ALL
             SELECT 12 FROM DUAL 
            ) 
         ...
         FROM 
           ...
           SELECT  V.CNN_BUF_HRS
            ...
            WHERE  V.DCHG_CNT_CD = SUBSTR(V_POD2,1,2)
              ...
              AND  V.DCHG_SLAN_CD = DECODE(V.DCHG_SLAN_CD,'ALL','ALL',V_LANE2 )
              ...
              AND  NVL(V.LOD_SLAN_CD,'ALL')  = DECODE(NVL(V.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE3 )
              AND  DECODE(V.DCHG_TML_CD,'ALL',10000,0) + DECODE(V.DCHG_SLAN_CD,'ALL',1000,0) + DECODE(NVL(V.LOD_CNT_CD,'ALL'),'ALL',100,0) +
                   DECODE(NVL(V.LOD_TML_CD,'ALL'),'ALL',10,0) + DECODE(NVL(V.LOD_SLAN_CD,'ALL'),'ALL',1,0) 
                = (
                  SELECT MIN ( DECODE(V2.DCHG_TML_CD,'ALL',10000,0) + DECODE(V2.DCHG_SLAN_CD,'ALL',1000,0) + DECODE(NVL(V2.LOD_CNT_CD,'ALL'),'ALL',100,0) +
                   ...
                   WHERE 	V2.DCHG_CNT_CD 							=  V.DCHG_CNT_CD
                     ...
                     AND  	V2.DCHG_SLAN_CD 						=  DECODE(V2.DCHG_SLAN_CD,'ALL','ALL',V_LANE2 )
                     ...
                     AND  	NVL(V2.LOD_SLAN_CD,'ALL')  				=  DECODE(NVL(V2.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE3 )
                   )
             UNION ALL
             SELECT 12 FROM DUAL 
            ) 
         ...
         FROM 
           ...
           SELECT  V.CNN_BUF_HRS
            ...
            WHERE  V.DCHG_CNT_CD = SUBSTR(V_POD3,1,2)
              ...
              AND  V.DCHG_SLAN_CD = DECODE(V.DCHG_SLAN_CD,'ALL','ALL',V_LANE3 )
              ...
              AND  NVL(V.LOD_SLAN_CD,'ALL')  = DECODE(NVL(V.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE4 )
              AND  DECODE(V.DCHG_TML_CD,'ALL',10000,0) + DECODE(V.DCHG_SLAN_CD,'ALL',1000,0) + DECODE(NVL(V.LOD_CNT_CD,'ALL'),'ALL',100,0) +
                   DECODE(NVL(V.LOD_TML_CD,'ALL'),'ALL',10,0) + DECODE(NVL(V.LOD_SLAN_CD,'ALL'),'ALL',1,0) 
                = (
                  SELECT MIN ( DECODE(V2.DCHG_TML_CD,'ALL',10000,0) + DECODE(V2.DCHG_SLAN_CD,'ALL',1000,0) + DECODE(NVL(V2.LOD_CNT_CD,'ALL'),'ALL',100,0) +
                   ...
                   WHERE 	V2.DCHG_CNT_CD 							=  V.DCHG_CNT_CD
                     ...
                     AND  	V2.DCHG_SLAN_CD 						=  DECODE(V2.DCHG_SLAN_CD,'ALL','ALL',V_LANE3 )
                     ...
                     AND  	NVL(V2.LOD_SLAN_CD,'ALL')  				=  DECODE(NVL(V2.LOD_SLAN_CD,'ALL'),'ALL','ALL',V_LANE4 )
                   )
             UNION ALL
             SELECT 12 FROM DUAL 
            ) 

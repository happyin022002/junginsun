...
public class CARIssueTransferSlipManageDBDAOCreateApInvDTRB_sumUpdateDiffUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	SET    INV_AMT = DECODE(@[curr_cd],'KRW',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0))," ).append("\n"); 
		query.append("                                       'JPY',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'TWD',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'IDR',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'CLP',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'DJF',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'VND',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'VUV',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'XAF',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 
		query.append("                                       'XPF',ROUND(NVL(INV_AMT,0)+NVL(@[gap],0),0)," ).append("\n"); 

 ...
 * @since J2EE 1.4
 */
public class ESD_PRD_0003HTMLAction extends HTMLActionSupport{
	private static final long serialVersionUID = 1L;

	/**
	 * ESD_PRD_003HTMLAction 객체를 생성
	 */
	public ESD_PRD_0003HTMLAction(){
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0003Event event = new EsdPrd0003Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		if(command.isCommand(FormCommand.SEARCH)){

			event.setSearchLeaseListRSQLVO((SearchLeaseListVO) getVO(request, SearchLeaseListVO.class));
		}

...
public class ProductCatalogcreateDBDAOSelectEmptyPuYdRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    (" ).append("\n"); 
		query.append("        SELECT MT,ORD,PCTL_NO, ARR_ST_DT, RANK() OVER(PARTITION BY MT ORDER BY ORD) RK" ).append("\n"); 
		query.append("        FROM " ).append("\n"); 
		query.append("        (                     " ).append("\n"); 
		...
		query.append("                         ,(SELECT POR_CD, POL_CD, VSL_SLAN_CD, RF_SPCL_FLG, DG_SPCL_FLG, m.pctl_no" ).append("\n"); 
		...
		query.append("                              AND SUBSTR(D.ORG_NOD_CD, 1, 5) = M.POL_CD) T" ).append("\n"); 
		...
		query.append("                     AND SPCL_CGO_CD IN (DECODE(T.DG_SPCL_FLG, 'Y', 'DG', 'AL'), DECODE(T.RF_SPCL_FLG, 'Y', 'RF', 'AL'), DECODE(T.RF_SPCL_FLG || T.DG_SPCL_FLG, 'NN', 'DR'), 'AL')" ).append("\n"); 
		query.append("                     AND MTY_PKUP_RTN_YD_CD not in (select MTY_PKUP_YD_CD from prd_prod_ctl_mst m WHERE M.PCTL_NO = @[pctl_no])" ).append("\n"); 
		query.append("                     AND (P.CNTR_TP_CD || P.CNTR_SZ_CD) IN (SELECT Q.CNTR_TPSZ_CD || Q.PCTL_QTY FROM PRD_PROD_CTL_QTY Q WHERE PCTL_NO = @[pctl_no] UNION ALL SELECT 'ALAL' FROM DUAL)" ).append("\n"); 
		query.append("    		UNION ALL" ).append("\n"); 
		query.append("            SELECT YD_CD MT, 4 ORD, '' pctl_no , '' ARR_ST_DT  " ).append("\n"); 
		query.append("				FROM mdm_yard " ).append("\n"); 
		query.append("			WHERE EXISTS (SELECT 1 FROM mdm_location WHERE LOC_CD=(SELECT POR_CD FROM PRD_PROD_CTL_MST WHERE PCTL_NO =@[pctl_no])  AND SCC_CD = SUBSTR(yd_cd,1,5))  " ).append("\n"); 
		...
		query.append("        AND   FCAST_DT = TO_DATE( (SELECT ARR_ST_DT FROM MT where pctl_no is not null and  rownum=1) ,'YYYYMMDD')" ).append("\n"); 
		query.append("        AND   CNTR_TPSZ_CD ='D2'" ).append("\n"); 
		query.append("        AND   CNTR_AVAL_FCAST_TP_CD = 'EA'" ).append("\n"); 
		query.append("        AND   CO_CD ='O' " ).append("\n"); 
		query.append("        UNION ALL" ).append("\n"); 
		query.append("        SELECT  " ).append("\n"); 
		...
		query.append(") t, mt m, prd_node n" ).append("\n"); 
		query.append("where m.mt = t.yd_cd(+)" ).append("\n"); 
		query.append("and   m.mt = n.nod_cd" ).append("\n"); 
		query.append("and   NVL(n.delt_flg, 'N') <> 'Y'" ).append("\n"); 
		query.append("union all" ).append("\n"); 
		query.append("select" ).append("\n"); 

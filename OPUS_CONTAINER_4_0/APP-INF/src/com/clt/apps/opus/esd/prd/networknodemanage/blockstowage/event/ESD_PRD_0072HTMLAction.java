 ...
 * @since J2EE 1.4
 */
public class ESD_PRD_0072HTMLAction extends HTMLActionSupport{
	private static final long serialVersionUID = 1L;

	/**
	 * ESD_PRD_070HTMLAction 객체를 생성
	 */
	public ESD_PRD_0072HTMLAction(){
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0072Event event = new EsdPrd0072Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));
		FormCommand command = FormCommand.fromRequest(request);

		if(command.isCommand(FormCommand.SEARCH)){
			event.setCodeInquiryVO((CodeInquiryVO) getVO(request, CodeInquiryVO.class));
		}

...
public class RailInvoiceInquiryCorrectionDBDAOSearchRailInvoiceInquiryCorrectionListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       SUM(C.FUEL_SCG_AMT) FUEL_SCG_AMT ," ).append("\n"); 
		query.append("       SUM(C.BZC_AMT+C.FUEL_SCG_AMT+C.OVR_WGT_SCG_AMT+C.HZD_MTRL_SCG_AMT+C.ETC_ADD_AMT) WO_TOT_AMT ," ).append("\n"); 
		...
		query.append("       E.GL_DT GL_DT ," ).append("\n"); 
		...
		query.append("			AND E.GL_DT      BETWEEN TO_DATE(@[fmdate],'YYYYMMDD') AND TO_DATE(@[todate],'YYYYMMDD') + 0.999999" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	AND A.TRSP_INV_AUD_STS_CD 			= DECODE(@[statusCd], 'ALL', A.TRSP_INV_AUD_STS_CD, @[statusCd]) " ).append("\n"); 
		query.append("	AND NVL(A.INV_HLD_FLG, 'N')         = DECODE(@[holdCd], 'ALL', NVL(A.INV_HLD_FLG, 'N'), @[holdCd])" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	AND A.CRE_OFC_CD = NVL(@[invCreOfc], A.CRE_OFC_CD)" ).append("\n"); 
		query.append("	AND A.CRE_USR_ID = NVL(@[ivcCreUsrId], A.CRE_USR_ID)" ).append("\n"); 
		query.append("" ).append("\n"); 

 ...
 *@FileName : ESD_PRD_0010HTMLAction.java
 ...
 *
 * @author jungsunyoung
 * @see EsdPrd0010Event , ESD_PRD_010EventResponse 참조
 * @since J2EE 1.4
 */
public class ESD_PRD_0010HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = 1L;

	/**
	 * ESD_PRD_0010HTMLAction 객체를 생성
	 */
	public ESD_PRD_0010HTMLAction(){
	}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ESD_PRD_010Event로 파싱하여 request에 셋팅<br>
	 * 
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException{
		...
		EsdPrd0010Event event = new EsdPrd0010Event();
		event.setAttribute("pgmNo", request.getServletPath().substring(1,13));

		event.setSearchEmbargoVO((SearchEmbargoVO) getVO(request, SearchEmbargoVO.class));
		if(command.isCommand(FormCommand.MULTI)){
			event.setSearchEmbargoVOs((SearchEmbargoVO[]) getVOs(request, SearchEmbargoVO.class, ""));
		}

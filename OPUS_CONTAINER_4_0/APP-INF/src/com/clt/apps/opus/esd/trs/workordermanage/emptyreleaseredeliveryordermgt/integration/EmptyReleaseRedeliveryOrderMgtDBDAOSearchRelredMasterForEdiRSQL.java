...
*@FileName : EmptyReleaseRedeliveryOrderMgtDBDAOSearchRelredMasterForEdiRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class EmptyReleaseRedeliveryOrderMgtDBDAOSearchRelredMasterForEdiRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * EmptyReleaseRedeliveryOrderMgtDBDAOSearchRelredMasterForEdi
	  * - 2015.07.30 CHAN WOO PARK : Chile case 추가 
	  * </pre>
	  */
	public EmptyReleaseRedeliveryOrderMgtDBDAOSearchRelredMasterForEdiRSQL(){
		setQuery();
		...
		query.append("FileName : EmptyReleaseRedeliveryOrderMgtDBDAOSearchRelredMasterForEdiRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      ,TO_CHAR(ORD.N1ST_NOD_PLN_DT, 'YYYY-MM-DD') AS PD_DATE_DISP" ).append("\n"); 
		query.append("      ,TO_CHAR(ORD.N1ST_NOD_PLN_DT, 'YYYY-MM-DD') AS PD_DATE" ).append("\n"); 
		...
		query.append("      ,DECODE(@[wo_iss_sts_cd], 'I', 'N', 'R', 'U', 'N', 'C') AS STATUS" ).append("\n"); 
		...
		query.append("	  ,'' AS STK_ISS_CD" ).append("\n"); 
		...
		query.append("--   AND (ORD.CGO_TP_CD = 'M' OR (ORD.CGO_TP_CD = 'F' AND ORD.TRSP_COST_DTL_MOD_CD IN ('DR', 'CY', 'LS', 'TS')))" ).append("\n"); 
		query.append("   AND ORD.TRSP_SO_TP_CD IN('Y', 'M')" ).append("\n"); 
		query.append("   AND ORD.CONTI_CD = 'E'" ).append("\n"); 
		query.append("   AND T.WO_CXL_FLG = (CASE  WHEN T.WO_ISS_STS_CD IN ('I', 'R') THEN 'N'   WHEN T.WO_ISS_STS_CD IN ('C', 'N') THEN 'Y' 	ELSE 'N'  END)" ).append("\n"); 
		...
		query.append("      ,ORD.BL_NO AS BL_NO" ).append("\n"); 
		query.append("      ,BK.VSL_CD || BK.SKD_VOY_NO || BK.SKD_DIR_CD AS VVD" ).append("\n"); 
		query.append("	  ,ORD.REF_ID ||'-'|| ORD.REF_SEQ  AS EPP_REF" ).append("\n"); 
		query.append("      ,ORD.TRSP_WO_OFC_CTY_CD || ORD.TRSP_WO_SEQ AS WO_NO" ).append("\n"); 
		query.append("      ,REPLACE(REPLACE(REPLACE(REPLACE(RTRIM(TRO.SPCL_INSTR_RMK), CHR(10), ''), CHR(13), ''), CHR(34), ''), CHR(9), ' ') AS SPCL_INST" ).append("\n"); 
		query.append("      ,REPLACE(REPLACE(REPLACE(REPLACE(SUBSTR(RTRIM(S.CUST_NM), 1, 50), CHR(10), ''), CHR(13), ''), CHR(34), ''), CHR(9), ' ') AS SHPR" ).append("\n"); 
		query.append("      ,REPLACE(REPLACE(REPLACE(REPLACE(SUBSTR(RTRIM(C.CUST_NM), 1, 50), CHR(10), ''), CHR(13), ''), CHR(34), ''), CHR(9), ' ') AS CNEE" ).append("\n"); 

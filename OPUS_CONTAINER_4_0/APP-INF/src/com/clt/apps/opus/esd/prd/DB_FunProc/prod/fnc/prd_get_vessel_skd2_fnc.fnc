CREATE OR REPLACE FUNCTION OPUSADM.PRD_GET_VESSEL_SKD2_FNC
/* ========================================================
   1. Object Name      : PRD_GET_VESSEL_SKD2_FNC
   2. Version          : 1.0
   3. Create Date      : 2010.05.06
   4. Sub System       : Product Catalog
   5. Author           : 조용인
   6. Description      : 해상 운송 구간중 Mother Vessel에 대한 Detail Route 정보 생성(ACTIVATION CHECK 삭제)
...
BEGIN
            ...
            (
	            ...
	            (
	                ...
	                ( 
                        ...
                        AND O.SLAN_CD    = DECODE(NVL(V_SLANE_CD, 'FDR'), 'FDR', O.SLAN_CD ,   NVL(V_SLANE_CD, O.SLAN_CD)) --ADD
                        AND O.SKD_DIR_CD = DECODE(NVL(V_SLANE_CD, 'FDR'), 'FDR', O.SKD_DIR_CD , NVL(V_DIR_CD, O.SKD_DIR_CD)) --ADD      
            ...
            (
	            ...
	            (
                    ...
                    SELECT  
	                    ...
	                    AND O.VPS_ETB_DT BETWEEN V_DATE + 0.5 AND V_DATE + 60
                        AND O.TURN_PORT_IND_CD IN ('Y', 'N')  --ADD
                        AND NVL(O.VT_ADD_CALL_FLG,'N') <> 'Y' --ADD  
	                    AND NVL(O.SKD_CNG_STS_CD,' ') <> 'S' 
	                    AND NVL(D.SKD_CNG_STS_CD,' ') <> 'S'                  
    ...
    IF (VV_ETD IS NOT NULL) AND (VV_ETB IS NOT NULL) THEN
        ...
        IF V_ORD = '1' THEN
            --VV_CCT := 'CCT'||TO_CHAR(NVL(PRD_GET_CCT_FNC(VV_ORG_YD_CD, V_SLANE_CD,VV_DIR_CD,V_CGO_TP, (VV_ORG_ETB +1.5), VV_ETD ),(VV_ORG_ETB+1.5) - 10/24),'yyyymmddhh24miss') ;
            --VV_NCCT := 'CCNT'||TO_CHAR(NVL(PRD_GET_CCT_FNC(VV_NORG_YD_CD, V_SLANE_CD,VV_DIR_CD,V_CGO_TP, (VV_NORG_ETB +1.5), VV_NETD ),(VV_NORG_ETB +1.5) - 10/24),'yyyymmddhh24miss') ;
            
            VV_CCT  :=   'CCC' ||TO_CHAR(NVL(PRD_COMMON_PKG.PRD_GET_CCT_BY_BKG_INFO_FNC(VV_VSL_CD, VV_VOY_NO, VV_DIR_CD, SUBSTR(VV_ORG_YD_CD, 1, 5),  VV_O_IND_SEQ, VV_ORG_YD_CD,  V_CGO_TP),TRUNC(VV_ORG_ETB+1.5 ) - 10/24),'yyyymmddhh24miss');
            VV_NCCT :=   'CCNC'||TO_CHAR(NVL(PRD_COMMON_PKG.PRD_GET_CCT_BY_BKG_INFO_FNC(VV_VSL_CD, VV_VOY_NO, VV_DIR_CD, SUBSTR(VV_NORG_YD_CD, 1, 5), VV_O_IND_SEQ, VV_NORG_YD_CD, V_CGO_TP),TRUNC(VV_NORG_ETB +1.5 ) - 10/24),'yyyymmddhh24miss');

...
public class EsdTrs0014ViewAdapter extends TrsDefaultViewAdapter {
	...
	protected String makeDataTag(DBRowSet rs, String prefix, Event event) {
		...
		try {
			if (formcommand.isCommand(FormCommand.MULTI) || formcommand.isCommand(FormCommand.MODIFY) || formcommand.isCommand(FormCommand.REMOVE) || formcommand.isCommand(FormCommand.REMOVE01)
				...
				if (formcommand.isCommand(FormCommand.SEARCH01) || formcommand.isCommand(FormCommand.SEARCH02)) {
					...
					while (rs.next()) {
						...
						for (int j = 0; j < rs.getMetaData().getColumnCount(); j++) {
							...
							columnValue = JSPUtil.getNull(rs.getString(xmlString));

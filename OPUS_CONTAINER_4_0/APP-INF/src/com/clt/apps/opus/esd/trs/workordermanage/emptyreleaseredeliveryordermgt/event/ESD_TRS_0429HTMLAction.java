...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.trs.emptyreleaseredeliveryordermgt 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @since J2EE 1.6
 */
public class ESD_TRS_0429HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdTrs0429Event event = new EsdTrs0429Event();
		if (command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH02) || command.isCommand(FormCommand.MULTI03)) {
			event.setCimCStockVO((CimCStockVO) getVO(request, CimCStockVO.class));
		} else {
			if (command.isCommand(FormCommand.PRINT) || command.isCommand(FormCommand.MULTI02)) {
				event.setRDFaxMailEAIVO((RDFaxMailEAIVO) getVO(request, RDFaxMailEAIVO.class));
			}

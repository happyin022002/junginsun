...
*@FileName : CopDetailReceiveDBDAOAddSceActRcvIfSetRcvDtCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CopDetailReceiveDBDAOAddSceActRcvIfSetRcvDtCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SCE_ACT_RCV_IF 에 데이터를 insert
	  * AddSceActRcvIf query 는 기존에 많은 영역에서 쓰는 관계로 영향도를 줄이기 위해 동일 목적 method 를 missing op mapping
	  * 시에 사용하기 위해 본 query 로 분리
	  * </pre>
	  */
	public CopDetailReceiveDBDAOAddSceActRcvIfSetRcvDtCSQL(){
		setQuery();
		...
		query.append("FileName : CopDetailReceiveDBDAOAddSceActRcvIfSetRcvDtCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(",@[vndr_seq]" ).append("\n"); 
		query.append(",@[bnd_vskd_seq_cd]" ).append("\n"); 
		query.append(",GLOBALDATE_PKG.TIME_CONV_FNC('KRSEL', TO_DATE(@[act_dat_rcv_dt], 'YYYY/MM/DD HH24:MI:SS'), substr(@[nod_cd],1,5))" ).append("\n"); 
		query.append(",@[vsl_cd]" ).append("\n"); 
		query.append(",@[skd_voy_no]" ).append("\n"); 
		query.append(",@[skd_dir_cd]" ).append("\n"); 

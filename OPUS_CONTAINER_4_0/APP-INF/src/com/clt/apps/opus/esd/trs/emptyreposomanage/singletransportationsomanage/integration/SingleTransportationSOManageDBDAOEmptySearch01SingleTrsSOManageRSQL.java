...
public class SingleTransportationSOManageDBDAOEmptySearch01SingleTrsSOManageRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    A.FM_DT AS  N1ST_NOD_PLN_DT," ).append("\n"); 
		query.append("    A.TO_DT AS LST_NOD_PLN_DT" ).append("\n"); 
		query.append("FROM EQR_REPO_EXE_SO_IF A, " ).append("\n"); 
		query.append("	MST_CONTAINER B , " ).append("\n"); 
		query.append("	TRS_TRSP_SVC_ORD SO, " ).append("\n"); 
		query.append("	MDM_LOCATION LOC," ).append("\n"); 
		query.append("	(SELECT CNTR_NO, OFFH_YD_CD  FROM LSE_AVAL_OFFH WHERE OFFH_DUE_DT > TO_CHAR(LAST_DAY(ADD_MONTHS(SYSDATE, -1)), 'YYYYMMDD') AND OFFH_STS_CD IN ('R','C') ) OFF" ).append("\n"); 

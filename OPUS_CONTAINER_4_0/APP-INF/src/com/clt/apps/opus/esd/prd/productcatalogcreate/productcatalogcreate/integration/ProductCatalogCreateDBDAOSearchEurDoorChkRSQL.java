...
public class ProductCatalogCreateDBDAOSearchEurDoorChkRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("MAX(CASE WHEN (SELECT CONTI_CD FROM MDM_LOCATION WHERE LOC_CD = SUBSTR(B.POR_NOD_CD,1,5)) ='E'" ).append("\n"); 
		...
		query.append("         WHEN (SELECT CONTI_CD FROM MDM_LOCATION WHERE LOC_CD = SUBSTR(B.DEL_NOD_CD,1,5)) ='E'" ).append("\n"); 
		query.append("           AND (NVL((SELECT DOR_NOD_CD FROM TRS_TRSP_SVC_ORD " ).append("\n"); 
		query.append("                     WHERE COP_NO =H.COP_NO " ).append("\n"); 

...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.tpb.common.fileupload 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
//		FormCommand f_cmd = FormCommand.fromRequest(request);
		TPBFileUploadEvent event = new TPBFileUploadEvent() ; 
//		TpbTtlFileMgmtVO tpbTtlFileMgmtVO = null;
		...
		if(request.getContentType()!=null && request.getContentType().toLowerCase().startsWith(SubSystemConfigFactory.get("COM.CONTENTS.TYPE.MULTIPART").trim())){
			...
			try {
				FileUpload fileUpload = new FileUpload(request,SubSystemConfigFactory.get("TPB.MODULE.ID"), SubSystemConfigFactory.get("COM.ENCODING.EUCKR"), event);
...
//			event.setCefFileCnt(request.getParameter("cef_file_cnt"));
		...
		event.setTpbTtlFileMgmtVOs((TpbTtlFileMgmtVO[])getVOs(request, TpbTtlFileMgmtVO .class));

...
*@FileName : CARIssueTransferSlipManageDBDAOSearchApInvDTRBInRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CARIssueTransferSlipManageDBDAOSearchApInvDTRBInRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public CARIssueTransferSlipManageDBDAOSearchApInvDTRBInRSQL(){
		setQuery();
		...
		query.append("FileName : CARIssueTransferSlipManageDBDAOSearchApInvDTRBInRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	 SELECT LINE_SEQ, LINE_NO, LINE_TP_LU_CD, ROUND((CSR_AMT/RATE)*FTU_USE_CTNT2) CSR_AMT," ).append("\n"); 
		query.append("			INV_DESC, INV_TAX_CD, DTRB_COA_CO_CD, DTRB_COA_RGN_CD, DTRB_COA_CTR_CD, DTRB_COA_ACCT_CD," ).append("\n"); 
		query.append("			DTRB_COA_VVD_CD, DTRB_COA_INTER_CO_CD, DTRB_COA_FTU_N1ST_CD, DTRB_COA_FTU_N2ND_CD," ).append("\n"); 
		query.append("			ATTR_CATE_NM, ATTR_CTNT1, ATTR_CTNT2, ATTR_CTNT3, ATTR_CTNT4, ATTR_CTNT5," ).append("\n"); 
		query.append("			ATTR_CTNT6, ATTR_CTNT7, ATTR_CTNT8, ATTR_CTNT9, ATTR_CTNT10, ATTR_CTNT11," ).append("\n"); 
		...
		query.append("				          AND SKD_DIR_CD = D.SKD_DIR_CD),H.TO_PRD_DT),D.WRK_DT) ATTR_CTNT11," ).append("\n"); 
		...
		query.append("					 D.TML_CRR_CD,D.VOL_TR_UT_CD,H.CURR_CD,H.TML_SO_OFC_CTY_CD,H.TML_SO_SEQ,D.VSL_CD,D.SKD_VOY_NO,D.SKD_DIR_CD,H.TO_PRD_DT,D.WRK_DT ) M," ).append("\n"); 
		query.append("		   ( SELECT D.LGS_COST_CD, D.CNTR_TPSZ_CD, SUM(NVL(D.INV_AMT,0)) CSR_AMT, D.TML_CRR_CD    --수정(20070723)" ).append("\n"); 
		...
		query.append("		   ( SELECT D.LGS_COST_CD, D.CNTR_TPSZ_CD, COUNT(L.CNTR_NO) RATE, D.TML_CRR_CD           --수정(20070723)" ).append("\n"); 
		query.append("			 FROM   TES_TML_SO_HDR H, TES_TML_SO_DTL D, TES_TML_SO_CNTR_LIST L" ).append("\n"); 
		query.append("			 WHERE  H.INV_NO              = @[inv_no]" ).append("\n"); 
		query.append("			 AND    H.VNDR_SEQ            = @[vndr_seq]" ).append("\n"); 
		query.append("			 AND    H.TML_INV_STS_CD      = 'C'" ).append("\n"); 
		...
		query.append("			 AND   M.CNTR_TPSZ_CD  = R.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("             AND   NVL(M.SO_CRR_CD,'N')  = NVL(R.TML_CRR_CD,'N') )                   --수정(20070723)" ).append("\n"); 
		query.append("	 UNION ALL" ).append("\n"); 
		query.append("	 SELECT --DISTINCT" ).append("\n"); 
		query.append("			@[line_seq]                                                       LINE_SEQ," ).append("\n"); 
		query.append("			''                                                      LINE_NO," ).append("\n"); 
		query.append("			'ITEM'                                                  LINE_TP_LU_CD," ).append("\n"); 
		query.append("			NVL(ROUND(SUM(D.INV_AMT)),0)                            CSR_AMT," ).append("\n"); 
		query.append("			( SELECT ACCT_ENG_NM" ).append("\n"); 
		query.append("			  FROM   MDM_ACCOUNT" ).append("\n"); 
		query.append("			  WHERE  ACCT_CD = D.ACCT_CD )                          INV_DESC," ).append("\n"); 
		query.append("			''                                                       INV_TAX_CD," ).append("\n"); 
		...
		query.append("			D.CALC_TP_CD                                            ATTR_CTNT15,    --(20080111)" ).append("\n"); 
		...
		query.append("	 AND    NVL(D.INV_AMT,0)      <> 0        --추가(20070523)" ).append("\n"); 
		query.append("	 GROUP BY D.TML_SO_DTL_SEQ, '', 'ITEM', D.ACCT_CD, '', '01', H.COST_OFC_CD," ).append("\n"); 
		...
		query.append("--		        GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC(), " ).append("\n"); 
		query.append("			GLOBALDATE_PKG.TIME_LOCAL_OFC_FNC(@[ofc_cd]), @[cre_usr_id], D.CALC_TP_CD, H.TML_SO_OFC_CTY_CD,H.TML_SO_SEQ,D.VSL_CD,D.SKD_VOY_NO,D.SKD_DIR_CD,H.TO_PRD_DT,D.WRK_DT  --추가(20080111)" ).append("\n"); 
		query.append("	 )" ).append("\n"); 

...
public class USA404EDIStatusInquiryDBDAOSearch03USA404EDIResendListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,ROUND(DECODE(A.TRSP_BND_CD, 'O', DECODE(A.WGT_MEAS_UT_CD, 'KGS', NVL(A.CNTR_WGT, 0), TRS_COMMON_PKG.GET_CONV_WGT_TO_KGS_FNC('LBS', NVL(A.CNTR_WGT, 0))), DECODE(D.WGT_UT_CD, 'KGS', NVL(D.CNTR_WGT, 0), TRS_COMMON_PKG.GET_CONV_WGT_TO_KGS_FNC('LBS', NVL(D.CNTR_WGT, 0))))) NET_WGT" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("       " ).append("\n"); 
		query.append("      ,DECODE(A.CGO_TP_CD, 'F', 'NYK INTERNATIONAL', 'NYK EMPTY') CNEE_N" ).append("\n"); 
		...
		query.append("         WHEN K.YD_LOC_CTY_NM IS NULL THEN ''" ).append("\n"); 
		query.append("         ELSE" ).append("\n"); 
		query.append("          NVL((SELECT NT.EDI_NTFY_NM FROM TRS_TRSP_NTFY_SET NT WHERE UPPER(NT.NTFY_YD_CTY_NM) = UPPER(K.YD_LOC_CTY_NM) AND NT.NTFY_BND_CD = DECODE(A.BKG_CGO_TP_CD, 'F', A.TRSP_BND_CD, 'E'))," ).append("\n"); 
		...
		query.append("      ,CASE" ).append("\n"); 
		query.append("         WHEN K.YD_LOC_CTY_NM IS NULL THEN ''" ).append("\n"); 
		query.append("         ELSE NVL((SELECT NT.EDI_NTFY_INFO_CTNT FROM TRS_TRSP_NTFY_SET NT WHERE UPPER(NT.NTFY_YD_CTY_NM) = UPPER(K.YD_LOC_CTY_NM) AND NT.NTFY_BND_CD = DECODE(A.BKG_CGO_TP_CD, 'F', A.TRSP_BND_CD, 'E'))," ).append("\n"); 
		...
		query.append("         WHEN K.YD_LOC_CTY_NM IS NULL THEN ''" ).append("\n"); 
		query.append("         ELSE" ).append("\n"); 
		query.append("          NVL((SELECT NT.EDI_NTFY_NM FROM TRS_TRSP_NTFY_SET NT WHERE UPPER(NT.NTFY_YD_CTY_NM) = UPPER(K.YD_LOC_CTY_NM) AND NT.NTFY_BND_CD = DECODE(A.BKG_CGO_TP_CD, 'F', A.TRSP_BND_CD, 'M'))," ).append("\n"); 
		query.append("              (SELECT NT.EDI_NTFY_NM FROM TRS_TRSP_NTFY_SET NT WHERE UPPER(NT.NTFY_YD_CTY_NM) = UPPER(K.YD_LOC_CTY_NM) AND NT.NTFY_BND_CD = 'A'))" ).append("\n"); 
		...
		query.append("            ,MAX(DECODE(BKG_CUST_TP_CD, 'C', CUST_NM)) CNEE_CUST_NM" ).append("\n"); 
		query.append("            ,MAX(DECODE(BKG_CUST_TP_CD, 'C', CUST_ADDR)) CNEE_CUST_ADDR" ).append("\n"); 
		query.append("            ,MAX(DECODE(BKG_CUST_TP_CD, 'C', CUST_CTY_NM)) CNEE_CUST_CTY_NM" ).append("\n"); 
		query.append("            ,MAX(DECODE(BKG_CUST_TP_CD, 'C', CUST_STE_CD)) CNEE_CUST_STE_CD" ).append("\n"); 
		query.append("            ,MAX(DECODE(BKG_CUST_TP_CD, 'C', CUST_ZIP_ID)) CNEE_CUST_ZIP_ID" ).append("\n"); 
		query.append("                 " ).append("\n"); 

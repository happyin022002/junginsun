...
public class MarineTerminalStorageInvoiceManageDBDAOCalStorageInvoiceCostRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("            ROUND(DECODE(SIGN(CALC_PDYS_CALC_AMT), 1, NEW_CALC_AMT - CALC_PDYS_CALC_AMT, NEW_CALC_AMT)*INV_XCH_RT,2) CALC_AMT,  --Paid Day에 대한 Rate를 구한 값(CALC_PDYS_CALC_AMT)이 0이나 (-)금액이 아닌 경우에만 Over Day+Paid Day에서 빼준다. " ).append("\n"); 
		query.append("            ROUND(DECODE(SIGN(CALC_PDYS_INV_AMT ), 1, NEW_INV_AMT  - CALC_PDYS_INV_AMT , NEW_INV_AMT )*INV_XCH_RT,2) INV_AMT,   --Paid 된 값을 뺀 나머지에 대한 Tier Rate를 구하기 위해 수정했음" ).append("\n"); 
		...
		query.append("          INV_XCH_RT," ).append("\n"); 
		query.append("          DECODE(L.COST_CODE,'SRNDTS',DECODE(L.CNTR_STY_CD,'F','512213','512222'),C.ACCT_CD) AS ACCT_CD," ).append("\n"); 
		...
		query.append("                     CNTR_STY_CD," ).append("\n"); 
		query.append("                     DECODE(PDYS,0,STAY_DAY,STAY_DAY2) STAY_DAY," ).append("\n"); 
		...
		query.append("              DECODE(@[sto_dys_ind_cd], 'IO', 0, DECODE(SIGN(PDYS), 1, PDYS, 0, 0, -1, 0)) PDYS," ).append("\n"); 
		query.append("              --DECODE(DECODE(SIGN(PDYS), 1, PDYS, 0, 0, -1, 0), 0, GIO_EDAY, INV_DT_EDAY) EDAY," ).append("\n"); 
		query.append("              DECODE(@[sto_dys_ind_cd], 'IO', GIO_EDAY , INV_DT_EDAY) EDAY,                                      " ).append("\n"); 
		query.append("              UOM," ).append("\n"); 
		query.append("              RATE, " ).append("\n"); 
		query.append("              FMDYS," ).append("\n"); 
		query.append("              TODYS," ).append("\n"); 
		...
		query.append("              TML_AGMT_VER_NO" ).append("\n"); 
		query.append("            FROM (" ).append("\n"); 
		query.append("                SELECT FF.PDYS-(DECODE(FF.SAT_FLG, 'Y', NVL(FF.PDYS_SAT,0), 0)+DECODE(FF.SUN_FLG, 'Y', NVL(FF.PDYS_SUN,0), 0)+DECODE(FF.HOL_FLG, 'Y', NVL(FF.PDYS_HOL,0), 0)) PDYS,    --PDYS 기간내의 토일휴 를 뺀다." ).append("\n"); 
		...
		query.append("                       FF.CNTR_STY_CD," ).append("\n"); 
		...
		query.append("                  F.CNTR_STY_CD," ).append("\n"); 
		...
		query.append("                  TES_WE_CNT_FNC ('SUN',     '', F.GIO_I-1,    F.GIO_O+1) GIO_SUN,  -- GI/GO 에 따른 excl day계산" ).append("\n"); 
		query.append("                  TES_WE_CNT_FNC ('SAT',     '', F.GIO_I-1,    F.GIO_O+1) GIO_SAT,  -- GI/GO 에 따른 excl day계산" ).append("\n"); 
		query.append("                  TES_HOL_CNT_FNC(@[yd_cd],  F.GIO_I,    F.GIO_O) GIO_HOL,  -- GI/GO 에 따른 excl day계산" ).append("\n"); 
		query.append("                  " ).append("\n"); 
		query.append("                  TES_WE_CNT_FNC ('SUN',     '', DECODE(SIGN((F.GIO_I-1)-(F.INV_DT_I-1)),-1,F.INV_DT_I-1,F.GIO_I-1), DECODE(SIGN((F.GIO_O+1)-(F.INV_DT_O+1)),-1,F.GIO_O+1,F.INV_DT_O+1)) INV_DT_SUN, -- inv period 에 따른 excl day계산" ).append("\n"); 
		query.append("                  TES_WE_CNT_FNC ('SAT',     '', DECODE(SIGN((F.GIO_I-1)-(F.INV_DT_I-1)),-1,F.INV_DT_I-1,F.GIO_I-1), DECODE(SIGN((F.GIO_O+1)-(F.INV_DT_O+1)),-1,F.GIO_O+1,F.INV_DT_O+1)) INV_DT_SAT,           " ).append("\n"); 
		query.append("                  TES_HOL_CNT_FNC(@[yd_cd],  DECODE(SIGN(F.GIO_I-F.INV_DT_I),-1,F.INV_DT_I,F.GIO_I), DECODE(SIGN(F.GIO_O-F.INV_DT_O),-1,F.GIO_O,F.INV_DT_O)) INV_DT_HOL," ).append("\n"); 
		query.append("                  " ).append("\n"); 
		query.append("                  CASE WHEN TES_WE_CNT_FNC ('SUN',     '', F.INV_DT_I-1, F.GIO_I+1   ) < 0" ).append("\n"); 
		query.append("                       THEN TES_WE_CNT_FNC ('SUN',     '', F.GIO_I-1,    F.INV_DT_I+1)" ).append("\n"); 
		query.append("                       ELSE TES_WE_CNT_FNC ('SUN',     '', F.INV_DT_I-1, F.GIO_I+1   )" ).append("\n"); 
		query.append("                  END PDYS_SUN,  -- PDYS 휴일계산" ).append("\n"); 
		query.append("                  CASE WHEN TES_WE_CNT_FNC ('SAT',     '', F.INV_DT_I-1, F.GIO_I+1   ) < 0" ).append("\n"); 
		query.append("                       THEN TES_WE_CNT_FNC ('SAT',     '', F.GIO_I-1,    F.INV_DT_I+1)" ).append("\n"); 
		query.append("                       ELSE TES_WE_CNT_FNC ('SAT',     '', F.INV_DT_I-1, F.GIO_I+1   )" ).append("\n"); 
		query.append("                  END PDYS_SAT,  -- PDYS 휴일계산      " ).append("\n"); 
		query.append("                  CASE WHEN TES_HOL_CNT_FNC(@[yd_cd],  F.INV_DT_I, F.GIO_I   ) <= 0" ).append("\n"); 
		query.append("                       THEN TES_HOL_CNT_FNC(@[yd_cd],  F.GIO_I,    F.INV_DT_I)" ).append("\n"); 
		query.append("                       ELSE TES_HOL_CNT_FNC(@[yd_cd],  F.INV_DT_I, F.GIO_I   )" ).append("\n"); 
		query.append("                  END PDYS_HOL,  -- PDYS 휴일계산" ).append("\n"); 
		query.append("                   " ).append("\n"); 
		...
		query.append("                      I.CNTR_STY_CD," ).append("\n"); 
		query.append("                      DECODE(@[sto_dys_ind_cd], 'IO', TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'), DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), DECODE(SIGN(replace(@[to_prd_dt],'-')-TO_CHAR(I.GO,'YYYYMMDD')),-1,TO_DATE(replace(@[to_prd_dt],'-'),'YYYYMMDD'),TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD')))) - TO_DATE(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDD'), 1, 8), 'YYYYMMDD') + DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1)) STAY_DAY," ).append("\n"); 
		query.append("                      DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), I.GO)  GIO_O," ).append("\n"); 
		...
		query.append("                      DECODE(@[sto_dys_ind_cd], 'IO', I.GO, DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), DECODE(SIGN(replace(@[to_prd_dt],'-')-TO_CHAR(I.GO,'YYYYMMDD')),-1,TO_DATE(replace(@[to_prd_dt],'-'),'YYYYMMDD'), I.GO))) INV_DT_O," ).append("\n"); 
		...
		query.append("                      DECODE(I.GO, NULL, TO_DATE(replace(@[to_prd_dt], '-'), 'YYYYMMDD'), DECODE(@[sto_dys_ind_cd],'IO',TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD'),DECODE(SIGN(replace(@[to_prd_dt],'-')-TO_CHAR(I.GO,'YYYYMMDD')),-1,TO_DATE(replace(@[to_prd_dt],'-'),'YYYYMMDD'),TO_DATE(SUBSTR(TO_CHAR(I.GO, 'YYYYMMDD'), 1, 8), 'YYYYMMDD')))) - TO_DATE(SUBSTR(TO_CHAR(TO_DATE(replace(@[fm_prd_dt], '-'), 'YYYYMMDD'), 'YYYYMMDD'), 1, 8), 'YYYYMMDD') + DECODE(D.CMNC_HRMNT, '2400', 1, '0000', 0, DECODE(SIGN(TO_NUMBER(D.CMNC_HRMNT) - TO_NUMBER(SUBSTR(TO_CHAR(I.GI, 'YYYYMMDDHH24MI'), 9, 4))), -1, 0, 0, 0, 1)) STAY_DAY2," ).append("\n"); 
		...
		query.append("                      AND DC.CNTR_TPSZ_CD = DECODE(I.CNTR_STY_CD, 'F', DECODE(SUBSTR(I.TPSZ, 1, 1), 'R', DECODE(I.RC_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'T', DECODE(I.RC_FLG,'Y','R'||SUBSTR(I.TPSZ,2,1),I.TPSZ), 'O', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'F', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'P', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'A', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'S', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), I.TPSZ), I.TPSZ)" ).append("\n"); 
		...
		query.append("                      AND NVL(D.TML_CNTR_STY_CD,'N') = DECODE(NVL(D.TML_CNTR_STY_CD,'N'),'N','N','S','S',I.CNTR_STY_CD)" ).append("\n"); 
		...
		query.append("                      AND NVL(RD.TML_CNTR_STY_CD,'N') = DECODE(NVL(RD.TML_CNTR_STY_CD,'N'),'N','N','S','S',I.CNTR_STY_CD)" ).append("\n"); 
		...
		query.append("                      AND DECODE(RD.TML_AGMT_VOL_UT_CD, 'C', RC.CNTR_TPSZ_CD, 'N') = DECODE(RD.TML_AGMT_VOL_UT_CD, 'C', DECODE(I.CNTR_STY_CD, 'F', DECODE(SUBSTR(I.TPSZ, 1, 1), 'R', DECODE(I.RC_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'T',DECODE(I.RC_FLG,'Y','R'||SUBSTR(I.TPSZ,2,1),I.TPSZ), 'O', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'F', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'P', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'A', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), 'S', DECODE(I.AWK_CGO_FLG, 'Y', I.TPSZ, 'D'||SUBSTR(I.TPSZ, 2, 1)), I.TPSZ) , I.TPSZ) , 'N')" ).append("\n"); 
		...
		query.append("    GROUP BY IBFLAG, CALC_TP_CD, CALC_COST_GRP_CD, LGS_COST_CD, CNTR_NO, CNTR_TPSZ_CD, IO_BND_CD, DCGO_IND_CD, CNTR_STY_CD, STAY_DYS, FREE_DYS, PAID_DAY, EXD_DAY, (new_TO_DAY - FM_DAY)+1, (calc_PDYS - FM_DAY)+1, VOL_TR_UT_CD, " ).append("\n"); 

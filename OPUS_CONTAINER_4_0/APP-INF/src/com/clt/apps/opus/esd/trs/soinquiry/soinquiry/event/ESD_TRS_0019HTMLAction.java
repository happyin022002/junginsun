...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.trs.soinquiry 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_TRS_0019HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		event.setSearchFmNode(JSPUtil.getParameter(request,"search_fm_node",""));
		event.setSearchToNode(JSPUtil.getParameter(request,"search_to_node",""));
		event.setPortIo(JSPUtil.getParameter(request,"port_io",""));
		event.setPortCd(JSPUtil.getParameter(request,"port_cd",""));
		event.setCntrSltNo(JSPUtil.getParameter(request,"cntr_slt_no",""));

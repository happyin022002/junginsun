...
public class ProductCatalogCreateDBDAOCreateProductCatalogInCludeReplaneCSQL implements ISQLTemplate{
	...
	public ProductCatalogCreateDBDAOCreateProductCatalogInCludeReplaneCSQL(){
		...
		params.put("ib_prio_seq",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("c_tpsz",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("ob_prio_seq",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("more_cnt",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("    REP_CMDT_CD, PRC_CGO_TP_CD" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		query.append("    VALUES " ).append("\n"); 
		query.append("    (PCTL_NO,MTPU_CY,SUBSTR(POR,1,5) , POR, SUBSTR(POL1,1,5) ,N2ND_POL_CD, N3RD_POL_CD,N4TH_POL_CD," ).append("\n"); 
		query.append("    SUBSTR(POD_NODE,1,5),SUBSTR(DEL,1,5),DEL,MTRTN_CY,OUT_CONTENT,IN_CONTENT,FULL_RTN_YD_CD,POL1,POD_NODE,FULL_PKUP_YD_CD," ).append("\n"); 
		...
		query.append("    @[spcl_awk_cgo_flg], @[bb_spcl_flg], @[rd_spcl_flg], @[hngr_spcl_flg], @[soc_flg],@[eq_subst_flg]," ).append("\n"); 
		query.append("    @[bkg_wgt], @[bkg_wgt_ut_cd], @[sls_ofc_cd], @[rfa_ofc_cd], @[prm_cust_flg]," ).append("\n"); 
		query.append("    @[rep_cmdt_cd], @[c_tpsz]" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		...
		query.append("      WHEN F_NO2 = 13 THEN TO_DATE(POL_T,'YYYYMMDDHH24MISS')" ).append("\n"); 
		...
		query.append("      WHEN NODE_NEW_KIND = 'L' AND F_NO2 =0 AND @[rcv_t] = 'D' THEN LEAD(N1ST_VNDR_SEQ, 1) OVER ( PARTITION BY GP1,NODE_NEW_KIND ORDER BY F_NO2,L_N0)" ).append("\n"); 
		query.append("      WHEN NODE_NEW_KIND = 'L' AND F_NO2 =41 AND @[del_t] = 'D' THEN LAG(N1ST_VNDR_SEQ, 1) OVER ( PARTITION BY GP1,NODE_NEW_KIND ORDER BY F_NO2,L_N0)" ).append("\n"); 
		...
		query.append("        WHEN NODE_NEW_KIND='L' AND F_NO2 IN (13,18,23,28) THEN ( SELECT DECODE(VSL_SVC_TP_CD, 'O', 'WD', 'VD') FROM MDM_VSL_SVC_LANE WHERE VSL_SLAN_CD = DECODE(F_NO2,13,N1ST_LANE_CD,18,N2ND_LANE_CD,23,N3RD_LANE_CD,28,N4TH_LANE_CD))" ).append("\n"); 
		query.append("       WHEN NODE_NEW_KIND='L' AND LENGTH(NODE_NEW2) > 7 THEN SUBSTR(NODE_NEW2,10,2)" ).append("\n"); 
		query.append("       ELSE" ).append("\n"); 
		query.append("        NVL((" ).append("\n"); 
		query.append("            SELECT NVL(SUBSTR(REGEXP_SUBSTR(@[ocn_str],M.ROUT_ORG_NOD_CD||'[-%].{2}[-%].{1}[-%].{3}[-%]'||M.ROUT_DEST_NOD_CD,1,O2), 9, 2), D.TRSP_MOD_CD)" ).append("\n"); 
		...
		query.append("    (CASE" ).append("\n"); 
		query.append("        WHEN NODE_NEW_KIND='N' AND F_NO2 IN (1,41) AND (SELECT NOD_TP_CD FROM PRD_NODE WHERE NOD_CD = NODE_NEW1) = 'Z' THEN ( SELECT CGO_HNDL_TM_HRS FROM MDM_ZONE WHERE ZN_CD = NODE_NEW1 )" ).append("\n"); 
		...
		query.append("    D_TIME," ).append("\n"); 
		query.append("    ( CASE" ).append("\n"); 
		query.append("        WHEN NODE_NEW_KIND='N' THEN ( SELECT TO_CHAR(N1ST_VNDR_SEQ) FROM MDM_YARD WHERE YD_CD = NODE_NEW1 )" ).append("\n"); 
		...
		query.append("                               M2.PRIO_SEQ" ).append("\n"); 
		query.append("                               FROM PRD_INLND_ROUT_MST M2, PRD_INLND_ROUT_DTL D2" ).append("\n"); 
		query.append("                               WHERE M2.ROUT_ORG_NOD_CD = NODE_NEW1" ).append("\n"); 
		query.append("                               AND M2.ROUT_DEST_NOD_CD = NODE_NEW2" ).append("\n"); 
		query.append("                               AND M2.ROUT_ORG_NOD_CD = D2.ROUT_ORG_NOD_CD" ).append("\n"); 
		query.append("                               AND M2.ROUT_DEST_NOD_CD = D2.ROUT_DEST_NOD_CD" ).append("\n"); 
		query.append("                               AND M2.ROUT_SEQ = D2.ROUT_SEQ" ).append("\n"); 
		...
		query.append("                               AND D2.ROUT_DTL_SEQ = 1" ).append("\n"); 
		query.append("                               AND NOT EXISTS ( SELECT 'X' FROM PRD_INLND_ROUT_DTL" ).append("\n"); 
		query.append("                                                WHERE ROUT_ORG_NOD_CD = D2.ROUT_ORG_NOD_CD" ).append("\n"); 
		query.append("                                                	AND ROUT_DEST_NOD_CD = D2.ROUT_DEST_NOD_CD" ).append("\n"); 
		query.append("                                                	AND ROUT_SEQ = D2.ROUT_SEQ" ).append("\n"); 
		query.append("                                                	AND ROUT_DTL_SEQ = 2 )" ).append("\n"); 
		query.append("                               AND ROWNUM = 1" ).append("\n"); 
		...
		query.append("    ( CASE WHEN NODE_NEW_KIND='N' THEN ( SELECT N2ND_VNDR_SEQ FROM MDM_YARD WHERE YD_CD = NODE_NEW1 ) END ) N2ND_VNDR_SEQ," ).append("\n"); 
		query.append("    ( CASE WHEN NODE_NEW_KIND='N' THEN ( SELECT N3RD_VNDR_SEQ FROM MDM_YARD WHERE YD_CD = NODE_NEW1 ) END ) N3RD_VNDR_SEQ," ).append("\n"); 
		query.append("    (CASE" ).append("\n"); 
		...
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN SUBSTR(DECODE(F_NO2,13,VVD1,18,VVD2,23,VVD3,28,VVD4,NULL),1,4) END ) VSL_CD," ).append("\n"); 
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN SUBSTR(DECODE(F_NO2,13,VVD1,18,VVD2,23,VVD3,28,VVD4,NULL),5,4) END ) VOY_NO," ).append("\n"); 
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN SUBSTR(DECODE(F_NO2,13,VVD1,18,VVD2,23,VVD3,28,VVD4,NULL),9,1) END ) DIR_CD," ).append("\n"); 
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN DECODE(F_NO2,13,CRR1,18,CRR2,23,CRR3,28,CRR4,NULL) END ) CRR_CD," ).append("\n"); 
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN DECODE(F_NO2,13,POL_SEQ1,18,POL_SEQ2,23,POL_SEQ3,28,POL_SEQ4,NULL) END ) ORG_PORT_SEQ," ).append("\n"); 
		query.append("    ( CASE WHEN F_NO2 IN (13,18,23,28) AND (NODE_NEW_KIND='L') THEN DECODE(F_NO2,13,POD_SEQ1,18,POD_SEQ2,23,POD_SEQ3,28,POD_SEQ4,NULL) END ) DST_PORT_SEQ,   " ).append("\n"); 
		...
		query.append("        FROM (" ).append("\n"); 
		query.append("            SELECT GP1,  F_N0 AS F_NO2, L_N0, O1," ).append("\n"); 
		...
		query.append("            ( CASE" ).append("\n"); 
		query.append("                WHEN F_N0 IN (13,18,23,28) AND (L_N0=2) THEN TO_DATE(DECODE(F_N0,13,POLT1,18,POLT2,23,POLT3,28,POLT4,NULL),'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("                WHEN F_N0 IN (18,23,28) AND (L_N0=1) THEN  TO_DATE(DECODE(F_N0,13,POLT1,18,POLT2,23,POLT3,28,POLT4,NULL),'YYYYMMDDHH24MISS') -6/24" ).append("\n"); 
		query.append("                WHEN F_N0 IN (14,19,24) AND (L_N0=2) THEN  TO_DATE(DECODE(F_N0,14,PODT1,19,PODT2,24,PODT3,NULL),'YYYYMMDDHH24MISS') + 6/24" ).append("\n"); 
		query.append("                WHEN F_N0 IN (14,19,24) AND (L_N0=1) THEN  TO_DATE(DECODE(F_N0,14,PODT1,19,PODT2,24,PODT3,NULL),'YYYYMMDDHH24MISS')                 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (15,20,25) AND (L_N0=1) THEN  TO_DATE(DECODE(F_N0,15,PODT1,20,PODT2,25,PODT3,NULL),'YYYYMMDDHH24MISS') + 12/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (15,20,25) AND (L_N0=2) THEN  TO_DATE(DECODE(F_N0,15,PODT1,20,PODT2,25,PODT3,NULL),'YYYYMMDDHH24MISS') + 18/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (16,21,26) AND (L_N0=1) THEN  TO_DATE(DECODE(F_N0,16,PODT1,21,PODT2,26,PODT3,NULL),'YYYYMMDDHH24MISS') + 24/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (16,21,26) AND (L_N0=2) THEN  TO_DATE(DECODE(F_N0,16,PODT1,21,PODT2,26,PODT3,NULL),'YYYYMMDDHH24MISS') + 30/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (17,22,27) AND (L_N0=1) THEN  TO_DATE(DECODE(F_N0,17,PODT1,22,PODT2,27,PODT3,NULL),'YYYYMMDDHH24MISS') + 36/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (17,22,27) AND (L_N0=2) THEN  TO_DATE(DECODE(F_N0,17,PODT1,22,PODT2,27,PODT3,NULL),'YYYYMMDDHH24MISS') + 42/24                     " ).append("\n"); 
		query.append("            END ) A_TIME," ).append("\n"); 
		query.append("            ( CASE" ).append("\n"); 
		query.append("                WHEN F_N0 IN (13,18,23,28) AND (L_N0=2) THEN 	TO_DATE(DECODE(F_N0,13,PODT1,18,PODT2,23,PODT3,28,PODT4,NULL),'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("                WHEN F_N0 IN (18,23,28) AND (L_N0=1) THEN	TO_DATE(DECODE(F_N0,13,POLT1,18,POLT2,23,POLT3,28,POLT4,NULL),'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("                WHEN F_N0 IN (14,19,24) AND (L_N0=2) THEN	TO_DATE(DECODE(F_N0,14,PODT1,19,PODT2,24,PODT3,NULL),'YYYYMMDDHH24MISS') + 12/24" ).append("\n"); 
		query.append("                WHEN F_N0 IN (14,19,24) AND (L_N0=1)  THEN	TO_DATE(DECODE(F_N0,14,PODT1,19,PODT2,24,PODT3,NULL),'YYYYMMDDHH24MISS') + 6/24                " ).append("\n"); 
		query.append("                WHEN F_N0 IN (15,20,25) AND (L_N0=1) THEN	TO_DATE(DECODE(F_N0,15,PODT1,20,PODT2,25,PODT3,NULL),'YYYYMMDDHH24MISS') + 18/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (15,20,25) AND (L_N0=2) THEN	TO_DATE(DECODE(F_N0,15,PODT1,20,PODT2,25,PODT3,NULL),'YYYYMMDDHH24MISS') + 24/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (16,21,26) AND (L_N0=1) THEN	TO_DATE(DECODE(F_N0,16,PODT1,21,PODT2,26,PODT3,NULL),'YYYYMMDDHH24MISS') + 30/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (16,21,26) AND (L_N0=2) THEN	TO_DATE(DECODE(F_N0,16,PODT1,21,PODT2,26,PODT3,NULL),'YYYYMMDDHH24MISS') + 36/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (17,22,27) AND (L_N0=1) THEN	TO_DATE(DECODE(F_N0,17,PODT1,22,PODT2,27,PODT3,NULL),'YYYYMMDDHH24MISS') + 42/24 " ).append("\n"); 
		query.append("                WHEN F_N0 IN (17,22,27) AND (L_N0=2) THEN	TO_DATE(DECODE(F_N0,17,PODT1,22,PODT2,27,PODT3,NULL),'YYYYMMDDHH24MISS') + 48/24 " ).append("\n"); 
		...
		query.append("                        WHEN 0 THEN (CASE WHEN @[rcv_t] ='S' AND OB_SEQ <> 0 THEN OB_ORG ELSE MTPU_CY END)" ).append("\n"); 
		query.append("                        WHEN 1 THEN DECODE(@[rcv_t],'T','',DECODE(@[rcv_t],'F','',DECODE(SUBSTR(OB_ORG,1,5),SUBSTR(POL1,1,5),DECODE(OB_SEQ,0,DECODE(@[rcv_t],'D',OB_ORG,'S',OB_ORG,''),OB_ORG),OB_ORG)))" ).append("\n"); 
		...
		query.append("                        WHEN 13 THEN DECODE(@[rcv_t],'T',OB_ORG,DECODE(@[rcv_t],'F',OB_ORG,NVL(POL0,POL1)))  WHEN 14 THEN DECODE(POD1,POD_NODE,'',DECODE(POD1,POL2,DECODE(TS1_2,NULL,'',POD1),POD1))" ).append("\n"); 
		query.append("                        WHEN 15 THEN TS1_2    WHEN 16 THEN TS1_3        WHEN 17 THEN TS1_4" ).append("\n"); 
		query.append("                        WHEN 18 THEN POL2     WHEN 19 THEN DECODE(POD2,POD_NODE,'',DECODE(POD2,POL3,DECODE(TS2_2,NULL,'',POD2),POD2))" ).append("\n"); 
		query.append("                        WHEN 20 THEN TS2_2    WHEN 21 THEN TS2_3        WHEN 22 THEN TS2_4    " ).append("\n"); 
		query.append("                        WHEN 23 THEN POL3     WHEN 24 THEN DECODE(POD3,POD_NODE,'',DECODE(POD3,POL4,DECODE(TS3_2,NULL,'',POD3),POD3))" ).append("\n"); 
		...
		query.append("                        WHEN 41 THEN DECODE(@[del_t],'T','',DECODE(@[del_t],'F','',DECODE(SUBSTR(POD_NODE,1,5),SUBSTR(IB_DEST,1,5),DECODE(IB_SEQ,0,DECODE(@[del_t],'D',DEL,'S',IB_DEST,''),DEL),DEL)))" ).append("\n"); 
		query.append("                        WHEN 42 THEN (CASE WHEN @[del_t] ='S' AND IB_SEQ <> 0 THEN IB_DEST ELSE MTRTN_CY END) ELSE 'N/A'" ).append("\n"); 
		query.append("                    END" ).append("\n"); 
		...
		query.append("						MTPU_CY, MTRTN_CY, " ).append("\n"); 
		...
		query.append("        			    RANK() OVER (ORDER BY  ORG_LOC_CD,DEST_LOC_CD,OCN_ROUT_PRIO_CD,OCN_SEQ, OB_ORG,OB_DEST,OB_SEQ, IB_ORG,IB_DEST,IB_SEQ,POL1,POD1,POL2, POD2,POL3,POD3,POL4,POD4) GP1                                           " ).append("\n"); 
		query.append("                        FROM (" ).append("\n"); 
		query.append("                            SELECT " ).append("\n"); 
		query.append("                            OB_ORG,OB_DEST,OB_SEQ,ORG_ROUT,FULL_RTN_YD_CD," ).append("\n"); 
		query.append("							 (CASE 	WHEN @[rcv_t] = 'S' THEN NULL" ).append("\n"); 
		...
		query.append("										  WHERE POR_DEL_CD = SUBSTR(OB_ORG,1,5)" ).append("\n"); 
		query.append("										  AND POL_POD_CD = SUBSTR(POL1,1,5)" ).append("\n"); 
		...
		query.append("									          FROM PRD_PKUP_RTN_YD X" ).append("\n"); 
		query.append("									         WHERE X.POR_DEL_CD = SUBSTR(OB_ORG,1,5)" ).append("\n"); 
		query.append("									           AND X.POL_POD_CD = SUBSTR(POL1,1,5)" ).append("\n"); 
		query.append("									           AND X.IO_BND_CD IN ('O','B')" ).append("\n"); 
		query.append("											   AND X.VSL_SLAN_CD IN (N1ST_LANE_CD,'ALL')" ).append("\n"); 
		query.append("											   AND X.SPCL_CGO_CD IN (  DECODE(NVL(@[dg_spcl_flg],'N'),'Y','DG','AL'), DECODE(NVL(@[rf_spcl_flg],'N'),'Y','RF','AL'), DECODE(NVL(@[rf_spcl_flg],'N')||NVL(@[dg_spcl_flg],'N'),'NN','DR'), 'AL')	 		" ).append("\n"); 
		query.append("									           AND NVL(X.DELT_FLG,'N') <> 'Y'										" ).append("\n"); 
		query.append("											   AND NVL(X.CNTR_TP_CD,'AL') || NVL(X.CNTR_SZ_CD, 'AL') IN ('ALAL'" ).append("\n"); 
		query.append("											   #if($arr_tpsz.size() > 0)  	" ).append("\n"); 
		query.append("											   #foreach($code IN ${arr_tpsz})" ).append("\n"); 
		query.append("												,'$code'												" ).append("\n"); 
		query.append("											   #end " ).append("\n"); 
		query.append("											   #end " ).append("\n"); 
		query.append("											   )										" ).append("\n"); 
		query.append("									          AND DECODE(X.VSL_SLAN_CD, 'ALL', 20, 0) + DECODE(X.SPCL_CGO_CD, 'AL', 10, 0) + DECODE(NVL(X.CNTR_TP_CD, 'AL') || NVL(X.CNTR_SZ_CD, 'AL'), 'ALAL', 1, 0) =" ).append("\n"); 
		query.append("									               (SELECT MIN(DECODE(Y.VSL_SLAN_CD, 'ALL', 20, 0) + DECODE(Y.SPCL_CGO_CD, 'AL', 10, 0) + DECODE(NVL(Y.CNTR_TP_CD, 'AL') || NVL(Y.CNTR_SZ_CD, 'AL'), 'ALAL', 1, 0))" ).append("\n"); 
		query.append("									                  FROM PRD_PKUP_RTN_YD Y" ).append("\n"); 
		query.append("									                 WHERE Y.POR_DEL_CD = X.POR_DEL_CD" ).append("\n"); 
		query.append("									                   AND Y.POL_POD_CD = X.POL_POD_CD" ).append("\n"); 
		query.append("									                   AND Y.IO_BND_CD = X.IO_BND_CD" ).append("\n"); 
		query.append("													   AND SPCL_CGO_CD IN (  DECODE(NVL(@[dg_spcl_flg],'N'),'Y','DG','AL'), DECODE(NVL(@[rf_spcl_flg],'N'),'Y','RF','AL'), DECODE(NVL(@[rf_spcl_flg],'N')||NVL(@[dg_spcl_flg],'N'),'NN','DR'), 'AL')" ).append("\n"); 
		query.append("									                   AND VSL_SLAN_CD IN (N1ST_LANE_CD,'ALL')" ).append("\n"); 
		query.append("													   AND NVL(DELT_FLG,'N') <> 'Y'" ).append("\n"); 
		query.append("										   			   AND NVL(Y.CNTR_TP_CD,'AL') || NVL(Y.CNTR_SZ_CD, 'AL') IN ('ALAL'" ).append("\n"); 
		query.append("													   #if($arr_tpsz.size() > 0)  	" ).append("\n"); 
		query.append("											           #foreach($code IN ${arr_tpsz})" ).append("\n"); 
		query.append("												         ,'$code'												" ).append("\n"); 
		query.append("											           #end " ).append("\n"); 
		...
		query.append("									ELSE (SELECT L.MTY_PKUP_YD_CD FROM MDM_LOCATION L WHERE L.LOC_CD = SUBSTR(OB_ORG,1,5))" ).append("\n"); 
		query.append("							 END) MTPU_CY,                         " ).append("\n"); 
		query.append("							 (CASE 	WHEN @[del_t] = 'S' THEN NULL							 		" ).append("\n"); 
		query.append("									WHEN (SELECT MTY_PKUP_RTN_YD_CD FROM PRD_PKUP_RTN_YD" ).append("\n"); 
		query.append("										  WHERE POR_DEL_CD = SUBSTR(IB_DEST,1,5)" ).append("\n"); 
		query.append("										  AND POL_POD_CD = SUBSTR(NVL(POD4, NVL(POD3, NVL(POD2, POD1))) ,1,5)" ).append("\n"); 
		...
		query.append("									  THEN (SELECT MTY_PKUP_RTN_YD_CD" ).append("\n"); 
		query.append("									          FROM PRD_PKUP_RTN_YD X" ).append("\n"); 
		query.append("									         WHERE X.POR_DEL_CD = SUBSTR(IB_DEST,1,5)" ).append("\n"); 
		query.append("									           AND X.POL_POD_CD = SUBSTR(NVL(POD4, NVL(POD3, NVL(POD2, POD1))) ,1,5)" ).append("\n"); 
		query.append("									           AND X.IO_BND_CD = 'I'" ).append("\n"); 
		query.append("											   AND X.VSL_SLAN_CD IN (NVL(N4TH_LANE_CD, NVL(N3RD_LANE_CD, NVL(N2ND_LANE_CD, N1ST_LANE_CD))),'ALL')" ).append("\n"); 
		query.append("											   AND X.SPCL_CGO_CD IN (  DECODE(NVL(@[dg_spcl_flg],'N'),'Y','DG','AL'), DECODE(NVL(@[rf_spcl_flg],'N'),'Y','RF','AL'), DECODE(NVL(@[rf_spcl_flg],'N')||NVL(@[dg_spcl_flg],'N'),'NN','DR'), 'AL')" ).append("\n"); 
		query.append("									           AND NVL(X.DELT_FLG,'N') <> 'Y'										" ).append("\n"); 
		query.append("											   AND NVL(X.CNTR_TP_CD,'AL') || NVL(X.CNTR_SZ_CD, 'AL') IN ('ALAL'" ).append("\n"); 
		query.append("											   #if($arr_tpsz.size() > 0)  	" ).append("\n"); 
		...
		query.append("									                   AND Y.POL_POD_CD = X.POL_POD_CD" ).append("\n"); 
		query.append("									                   AND Y.IO_BND_CD = X.IO_BND_CD" ).append("\n"); 
		query.append("									                   AND Y.SPCL_CGO_CD IN (  DECODE(NVL(@[dg_spcl_flg],'N'),'Y','DG','AL'), DECODE(NVL(@[rf_spcl_flg],'N'),'Y','RF','AL'), DECODE(NVL(@[rf_spcl_flg],'N')||NVL(@[dg_spcl_flg],'N'),'NN','DR'), 'AL')" ).append("\n"); 
		query.append("									                   AND Y.VSL_SLAN_CD IN (NVL(N4TH_LANE_CD, NVL(N3RD_LANE_CD, NVL(N2ND_LANE_CD, N1ST_LANE_CD))),'ALL')" ).append("\n"); 
		query.append("													   AND NVL(DELT_FLG,'N') <> 'Y'" ).append("\n"); 
		query.append("							   						   AND NVL(X.CNTR_TP_CD,'AL') || NVL(X.CNTR_SZ_CD, 'AL') IN ('ALAL'" ).append("\n"); 
		query.append("											           #if($arr_tpsz.size() > 0)  	" ).append("\n"); 
		query.append("											           #foreach($code IN ${arr_tpsz})" ).append("\n"); 
		...
		query.append("									ELSE (SELECT NVL(L.EQ_RTN_YD_CD, L.MTY_PKUP_YD_CD)  FROM MDM_LOCATION L WHERE L.LOC_CD =SUBSTR(IB_DEST,1,5)) " ).append("\n"); 
		query.append("							 END) MTRTN_CY," ).append("\n"); 
		query.append("                            RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,OB_DEST ORDER BY DECODE(OB_ORG, @[por_n], 1, DECODE(SUBSTR(OB_ORG, 1, 5), @[por], 2, 3)), FULL_RTN_PRIO_SEQ,OB_BKG_FLG,OB_MODE_PRIO_SEQ,OB_TMP_FLG,OB_PRIO_SEQ,OB_ROUT_SEQ) OB_RK," ).append("\n"); 
		...
		query.append("                            DECODE(TS1_2,NULL,NULL,POL2,NULL,TS1_2) TS1_2," ).append("\n"); 
		query.append("                            DECODE(TS1_3,NULL,NULL,POL2,NULL,TS1_3) TS1_3," ).append("\n"); 
		query.append("                            DECODE(TS1_4,NULL,NULL,POL2,NULL,TS1_4) TS1_4," ).append("\n"); 
		query.append("                            POLT1,PODT1,VVD1,CRR1,POL_SEQ1,POD_SEQ1," ).append("\n"); 
		query.append("                            POL2,POD2," ).append("\n"); 
		query.append("                            DECODE(TS2_2,NULL,NULL,POL3,NULL,TS2_2) TS2_2," ).append("\n"); 
		query.append("                            DECODE(TS2_3,NULL,NULL,POL3,NULL,TS2_3) TS2_3," ).append("\n"); 
		query.append("                            DECODE(TS2_4,NULL,NULL,POL3,NULL,TS2_4) TS2_4," ).append("\n"); 
		query.append("                            POLT2,PODT2,VVD2,CRR2,POL_SEQ2,POD_SEQ2," ).append("\n"); 
		query.append("                            POL3,POD3," ).append("\n"); 
		query.append("                            DECODE(TS3_2,NULL,NULL,POL4,NULL,TS3_2) TS3_2," ).append("\n"); 
		query.append("                            DECODE(TS3_3,NULL,NULL,POL4,NULL,TS3_3) TS3_3," ).append("\n"); 
		query.append("                            DECODE(TS3_4,NULL,NULL,POL4,NULL,TS3_4) TS3_4," ).append("\n"); 
		query.append("                            POLT3,PODT3,VVD3,CRR3,POL_SEQ3,POD_SEQ3," ).append("\n"); 
		query.append("                            POL4,POD4,POLT4,PODT4,VVD4,CRR4,POL_SEQ4,POD_SEQ4,     " ).append("\n"); 
		query.append("                            POD_NODE," ).append("\n"); 
		query.append("                            CHECK_ROUT," ).append("\n"); 
		query.append("                            IB_ORG,IB_DEST,IB_PRIO_SEQ,IB_SEQ,DEST_ROUT,FULL_PKUP_YD_CD," ).append("\n"); 
		query.append("                            RANK() OVER (PARTITION BY ORG_LOC_CD,DEST_LOC_CD,OCN_SEQ,IB_ORG ORDER BY DECODE(IB_DEST, @[del_n], 1, DECODE(SUBSTR(IB_DEST, 1, 5), @[del], 2, 3)), IB_MODE_PRIO_SEQ,IB_BKG_FLG,IB_TMP_FLG,IB_PRIO_SEQ,IB_ROUT_SEQ ) IB_RK,   " ).append("\n"); 
		...
		query.append("                            FROM " ).append("\n"); 
		query.append("                            (" ).append("\n"); 
		query.append("                                SELECT ROWNUM,NEW_POR, NEW_DEL," ).append("\n"); 
		...
		query.append("								NVL(O.PRIO_SEQ,0) OB_PRIO_SEQ," ).append("\n"); 
		...
		query.append("    					        DECODE(NVL(O.INLND_ROUT_BKG_FLG,'Y'), 'Y',1,2) OB_BKG_FLG," ).append("\n"); 
		query.append("    					        DECODE(NVL(I.INLND_ROUT_BKG_FLG,'Y'), 'Y',1,2) IB_BKG_FLG," ).append("\n"); 
		...
		query.append("                                DECODE(N1ST_SVC_TP,'O',DECODE(T.TS1_1,NULL,T.POD1,T.TS1_1),T.POD1) POD1," ).append("\n"); 
		query.append("                                DECODE(N1ST_SVC_TP,'O',DECODE(T.TS1_3,NULL,NULL,T.TS1_2),DECODE(T.TS1_1,T.POD1,DECODE(T.TS1_3,NULL,NULL,T.TS1_2),T.TS1_1)) TS1_2," ).append("\n"); 
		query.append("                                DECODE(N1ST_SVC_TP,'O',DECODE(T.TS1_4,NULL,NULL,T.TS1_3),DECODE(T.TS1_1,T.POD1,DECODE(T.TS1_4,NULL,NULL,T.TS1_3),DECODE(T.TS1_3,NULL,NULL,T.TS1_2))) TS1_3," ).append("\n"); 
		query.append("                                DECODE(N1ST_SVC_TP,'O',DECODE(T.TS1_5,NULL,NULL,T.TS1_4),DECODE(T.TS1_1,T.POD1,DECODE(T.TS1_5,NULL,NULL,T.TS1_4),DECODE(T.TS1_4,NULL,NULL,T.TS1_3))) TS1_4," ).append("\n"); 
		query.append("                                T.POLT1,T.PODT1,T.VVD1,T.CRR1,T.POL_SEQ1,T.POD_SEQ1, " ).append("\n"); 
		query.append("                                DECODE(T.VVD2, NULL, DECODE(T.POL2,NULL,NULL, DECODE(N2ND_SVC_TP,'O',NVL(SUBSTR(TRIM(T.TS1_1||T.TS1_2||T.TS1_3||T.TS1_4||T.TS1_5), -7),T.POL2),T.POL2)), T.POL2) POL2," ).append("\n"); 
		query.append("                                DECODE(N2ND_SVC_TP,'O',DECODE(T.TS2_1,NULL,T.POD2,T.TS2_1),T.POD2) POD2," ).append("\n"); 
		query.append("                                DECODE(N2ND_SVC_TP,'O',DECODE(T.TS2_3,NULL,NULL,T.TS2_2),DECODE(T.TS2_1,T.POD2,DECODE(T.TS2_3,NULL,NULL,T.TS2_2),T.TS2_1)) TS2_2," ).append("\n"); 
		query.append("                                DECODE(N2ND_SVC_TP,'O',DECODE(T.TS2_4,NULL,NULL,T.TS2_3),DECODE(T.TS2_1,T.POD2,DECODE(T.TS2_4,NULL,NULL,T.TS2_3),DECODE(T.TS2_3,NULL,NULL,T.TS2_2))) TS2_3," ).append("\n"); 
		query.append("                                DECODE(N2ND_SVC_TP,'O',DECODE(T.TS2_5,NULL,NULL,T.TS2_4),DECODE(T.TS2_1,T.POD2,DECODE(T.TS2_5,NULL,NULL,T.TS2_4),DECODE(T.TS2_4,NULL,NULL,T.TS2_3))) TS2_4," ).append("\n"); 
		query.append("                                T.POLT2,T.PODT2,T.VVD2,T.CRR2,T.POL_SEQ2,T.POD_SEQ2," ).append("\n"); 
		query.append("                                DECODE(T.VVD3, NULL, DECODE(T.POL3,NULL,NULL, DECODE(N3RD_SVC_TP,'O',NVL(SUBSTR(TRIM(T.TS2_1||T.TS2_2||T.TS2_3||T.TS2_4||T.TS2_5), -7),T.POL3),T.POL3)), T.POL3) POL3, " ).append("\n"); 
		query.append("                                DECODE(N3RD_SVC_TP,'O',DECODE(T.TS3_1,NULL,T.POD3,T.TS3_1),T.POD3) POD3," ).append("\n"); 
		query.append("                                DECODE(N3RD_SVC_TP,'O',DECODE(T.TS3_3,NULL,NULL,T.TS3_2),DECODE(T.TS3_1,T.POD3,DECODE(T.TS3_3,NULL,NULL,T.TS3_2),T.TS3_1)) TS3_2," ).append("\n"); 
		query.append("                                DECODE(N3RD_SVC_TP,'O',DECODE(T.TS3_4,NULL,NULL,T.TS3_3),DECODE(T.TS3_1,T.POD3,DECODE(T.TS3_4,NULL,NULL,T.TS3_3),DECODE(T.TS3_3,NULL,NULL,T.TS3_2))) TS3_3," ).append("\n"); 
		query.append("                                DECODE(N3RD_SVC_TP,'O',DECODE(T.TS3_5,NULL,NULL,T.TS3_4),DECODE(T.TS3_1,T.POD3,DECODE(T.TS3_5,NULL,NULL,T.TS3_4),DECODE(T.TS3_4,NULL,NULL,T.TS3_3))) TS3_4," ).append("\n"); 
		query.append("                                T.POLT3,T.PODT3,T.VVD3,T.CRR3,T.POL_SEQ3,T.POD_SEQ3,                            " ).append("\n"); 
		query.append("                                DECODE(T.VVD4, NULL, DECODE(T.POL4,NULL,NULL,DECODE(N4TH_SVC_TP,'O',NVL(SUBSTR(TRIM(T.TS3_1||T.TS3_2||T.TS3_3||T.TS3_4||T.TS3_5), -7),T.POL4),T.POL4)),T.POL4) POL4," ).append("\n"); 
		...
		query.append("                                       ELSE 'Y'" ).append("\n"); 
		query.append("                                END) DEST_FLG_CHK, --CHO" ).append("\n"); 
		query.append("                                (CASE WHEN O.ROUT_ORG_NOD_CD IS NULL  AND NEW_POR = SUBSTR(T.POL1,1,5) AND @[rcv_t] = 'S' AND POL1_S ='Y' THEN 'Y'" ).append("\n"); 
		query.append("                                      WHEN O.ROUT_ORG_NOD_CD IS NULL  AND NEW_POR = SUBSTR(T.POL1,1,5) AND @[rcv_t] NOT IN ('D', 'S') THEN 'Y'" ).append("\n"); 
		...
		query.append("                                                                                       DECODE(@[rcv_t],'D',(SELECT REP_ZN_CD FROM MDM_LOCATION WHERE LOC_CD = NEW_POR),NEW_POR||'%')" ).append("\n"); 
		...
		query.append("                                 (CASE WHEN I.ROUT_DEST_NOD_CD IS NULL AND SUBSTR( T.POD_NODE ,1,5) = NEW_DEL AND @[del_t] = 'S' AND POD_NODE_S ='Y' THEN 'Y' " ).append("\n"); 
		query.append("                                       WHEN I.ROUT_DEST_NOD_CD IS NULL AND SUBSTR( T.POD_NODE ,1,5) = NEW_DEL AND @[del_t] NOT IN ('D', 'S') THEN 'Y'" ).append("\n"); 
		...
		query.append("                                             AND DECODE(@[del_t],'S','Y','X') = NVL((SELECT DECODE(@[del_t],'S',YD_FCTY_TP_CFS_FLG,'X')  FROM MDM_YARD WHERE YD_CD=I.ROUT_DEST_NOD_CD),'X')" ).append("\n"); 
		query.append("                                             AND I.ROUT_DEST_NOD_CD  LIKE DECODE(@[del_t],'F','','T',''," ).append("\n"); 
		query.append("                                                                                  DECODE(@[del_n],''," ).append("\n"); 
		query.append("                                                                                       DECODE(@[del_t],'D',(SELECT REP_ZN_CD FROM MDM_LOCATION WHERE LOC_CD = NEW_DEL),NEW_DEL||'%')" ).append("\n"); 
		...
		query.append("                                    ,(SELECT REP_ZN_CD FROM MDM_LOCATION WHERE LOC_CD = NEW_POR) OB_REP_ZN_CD" ).append("\n"); 
		query.append("                                    ,(SELECT REP_ZN_CD FROM MDM_LOCATION WHERE LOC_CD = NEW_DEL) IB_REP_ZN_CD" ).append("\n"); 
		query.append("                                    FROM (" ).append("\n"); 
		query.append("                                        SELECT /*+ NO_MERGE(B)  */ " ).append("\n"); 
		query.append("                                        ROWNUM,NEW_POR,NEW_DEL, ORG_LOC_CD,DEST_LOC_CD,ROUT_SEQ,LNK_KNT,OCN_ROUT_PRIO_CD," ).append("\n"); 
		query.append("                                        N1ST_LANE_CD,N2ND_LANE_CD,N3RD_LANE_CD,N4TH_LANE_CD,N2ND_POL_CD, N3RD_POL_CD,N4TH_POL_CD," ).append("\n"); 
		query.append("                                        N1ST_SVC_TP,N2ND_SVC_TP, N3RD_SVC_TP,N4TH_SVC_TP," ).append("\n"); 
		query.append("                                        N1ST_TZTM_HRS,N2ND_TZTM_HRS,N3RD_TZTM_HRS,N4TH_TZTM_HRS, " ).append("\n"); 
		query.append("                                        --TRIM(DECODE(INSTR(SKD_STR,'CCC'),0,NULL,SUBSTR(SKD_STR,INSTR(SKD_STR,'CCC')+3,14))) CCT,  " ).append("\n"); 
		query.append("										TRIM(SUBSTR(SKD_STR,25, 14)) CCT,                             " ).append("\n"); 
		...
		query.append("                                        FROM ( " ).append("\n"); 
		query.append("                                            SELECT                                             " ).append("\n"); 
		query.append("                                            POR.POR_POL NEW_POR," ).append("\n"); 
		query.append("                                            DEL.POD_DEL NEW_DEL,  " ).append("\n"); 
		query.append("                                            ORG_LOC_CD,DEST_LOC_CD,ROUT_SEQ,LNK_KNT,OCN_ROUT_PRIO_CD," ).append("\n"); 
		query.append("                                            N1ST_LANE_CD,N2ND_LANE_CD,N3RD_LANE_CD,N4TH_LANE_CD,N2ND_POL_CD, N3RD_POL_CD,N4TH_POL_CD," ).append("\n"); 
		...
		query.append("                                                @[n1st_pod_dc_seq], N1ST_LANE_CD, DECODE(N1ST_LANE_CD, 'FDR', NULL, N1ST_SKD_DIR_CD), N1.VSL_SVC_TP_CD, @[vvd1]," ).append("\n"); 
		...
		query.append("                                                @[n2nd_pod_dc_seq], N2ND_LANE_CD, DECODE(N2ND_LANE_CD, 'FDR', NULL, N2ND_SKD_DIR_CD), N2.VSL_SVC_TP_CD, @[vvd2], " ).append("\n"); 
		...
		query.append("                                                @[n3rd_pod_dc_seq], N3RD_LANE_CD, DECODE(N3RD_LANE_CD, 'FDR', NULL, N3RD_SKD_DIR_CD), N3.VSL_SVC_TP_CD, @[vvd3]," ).append("\n"); 
		...
		query.append("                                                @[n4th_pod_dc_seq], N4TH_LANE_CD, DECODE(N4TH_LANE_CD, 'FDR', NULL, N4TH_SKD_DIR_CD), N4.VSL_SVC_TP_CD, @[vvd4],  @[cgo_tp_cd], 'N') SKD_STR											   " ).append("\n"); 
		...
		query.append("                                                   WHERE LOC_CD = (CASE WHEN @[por_n] IS NULL AND @[m_pu] IS NULL THEN @[por] ELSE 'X' END)" ).append("\n"); 
		query.append("                                                 ) POR," ).append("\n"); 
		query.append("                                                 (SELECT @[del] POD_DEL FROM DUAL " ).append("\n"); 
		query.append("                                                   UNION " ).append("\n"); 
		query.append("                                                  SELECT EQLZ_PORT_CD " ).append("\n"); 
		query.append("                                                    FROM BKG_EQLZ_PORT " ).append("\n"); 
		query.append("                                                   WHERE LOC_CD = (CASE WHEN @[del_n] IS NULL THEN @[del] ELSE 'X' END)" ).append("\n"); 
		query.append("                                                 ) DEL" ).append("\n"); 
		...
		query.append("    					    				AND A.DEST_LOC_CD = NVL(@[pod],A.DEST_LOC_CD)" ).append("\n"); 
		query.append("    					    				AND A.ROUT_SEQ = NVL(@[ocn_seq],A.ROUT_SEQ)" ).append("\n"); 
		query.append("                                            AND 'OK' = (CASE WHEN @[more_cnt] IS NULL THEN 'OK'" ).append("\n"); 
		query.append("    					    				                 WHEN @[more_cnt] = 0 AND ( ( A.OCN_ROUT_PRIO_CD = 1 AND A.LNK_KNT <= 3 )" ).append("\n"); 
		query.append("    					    				                                          OR (  A.OCN_ROUT_PRIO_CD = 2 AND A.LNK_KNT <= 2 ) ) THEN 'OK'" ).append("\n"); 
		query.append("    					    				                 WHEN @[more_cnt] = 1 AND NOT ( ( A.OCN_ROUT_PRIO_CD = 1 AND A.LNK_KNT <= 3 )" ).append("\n"); 
		query.append("    					    				                                              OR (  A.OCN_ROUT_PRIO_CD = 2 AND A.LNK_KNT <= 2 ) ) " ).append("\n"); 
		query.append("    					    				                                      AND A.OCN_ROUT_PRIO_CD <= 3 THEN 'OK'" ).append("\n"); 
		query.append("    					    				                 WHEN @[more_cnt] = 2 AND A.OCN_ROUT_PRIO_CD > 3 THEN 'OK'" ).append("\n"); 
		query.append("    					    				             END)" ).append("\n"); 
		...
		query.append("                                            ( SELECT 'X' FROM PRD_MBGO_MGMT TT WHERE SUBSTR (A.ORG_LOC_CD, 1, 2) = TT.FM_CNT_CD AND A.TS_IND_CD = 'D' AND SUBSTR (A.DEST_LOC_CD, 1, 2) = TT.TO_CNT_CD ) " ).append("\n"); 
		...
		query.append("                                AND  O.ROUT_ORG_NOD_CD(+) --CHO" ).append("\n"); 
		...
		query.append("                                              WHEN @[rcv_t] = 'S' AND NVL(T.POL1_S, 'N') <> 'Y' THEN NEW_POR || '%'" ).append("\n"); 
		query.append("                                              WHEN NEW_POR = SUBSTR(T.POL1, 1,5)  THEN DECODE(REPLACE(@[ob_str], '%', ''), NULL, 'X', NEW_POR || '%')" ).append("\n"); 
		query.append("                                              ELSE NEW_POR || '%'" ).append("\n"); 
		query.append("                                         END" ).append("\n"); 
		query.append("                                AND O.ROUT_DEST_NOD_CD(+)= DECODE(@[rcv_t], 'F', 'X', 'T', 'X', T.POL1)" ).append("\n"); 
		query.append("								AND NVL2(@[pol_n], T.POL1, 'X') = NVL(@[pol_n], 'X')" ).append("\n"); 
		query.append("								AND NVL2(@[pod_n], T.POD_NODE, 'X') = NVL(@[pod_n], 'X')" ).append("\n"); 
		query.append("								--If AL or PR of Transmode code is slected, return all type of Transmode " ).append("\n"); 
		query.append("								AND NVL(O.TRSP_MOD_CD(+),'X') = DECODE ( NVL(@[ob_trsp_mode],'AL'), 'AL', NVL(O.TRSP_MOD_CD(+),'X') , 'PR', NVL(O.TRSP_MOD_CD(+),'X') , @[ob_trsp_mode])" ).append("\n"); 
		query.append("	                            --If PR is selected and Sequence# is input, return the record of the Sequence# " ).append("\n"); 
		query.append("								AND NVL(O.PRIO_SEQ(+),0) = DECODE (@[ob_trsp_mode],'PR', TO_NUMBER(NVL(@[ob_prio_seq],NVL(O.PRIO_SEQ(+),0))), NVL(O.PRIO_SEQ(+),0))" ).append("\n"); 
		...
		query.append("                                              WHEN @[del_t] = 'S' AND NVL(T.POD_NODE_S, 'N') <> 'Y' THEN NEW_DEL || '%'" ).append("\n"); 
		query.append("                                              WHEN SUBSTR(T.POD_NODE, 1,5) = NEW_DEL THEN DECODE(REPLACE(@[ib_str], '%', ''), NULL, 'X', NEW_DEL || '%')" ).append("\n"); 
		query.append("                                              ELSE NEW_DEL || '%'" ).append("\n"); 
		query.append("                                         END" ).append("\n"); 
		query.append("    							--If AL or PR of Transmode code is slected, return all type of Transmode  " ).append("\n"); 
		query.append("								AND NVL(I.TRSP_MOD_CD(+),'X') = DECODE ( NVL(@[ib_trsp_mode],'AL'), 'AL', NVL(I.TRSP_MOD_CD(+),'X') , 'PR', NVL(I.TRSP_MOD_CD(+),'X') , @[ib_trsp_mode])" ).append("\n"); 
		query.append("                                --If PR is selected and Sequence# is input, return the record of the Sequence# " ).append("\n"); 
		query.append("								AND NVL(I.PRIO_SEQ(+),0) = DECODE (@[ib_trsp_mode],'PR', TO_NUMBER(NVL(@[ib_prio_seq],NVL(I.PRIO_SEQ(+),0))), NVL(I.PRIO_SEQ(+),0))" ).append("\n"); 
		...
		query.append("                    AND DECODE(@[rcv_t], 'S', 'S', MTPU_CY) IS NOT NULL AND DECODE(@[del_t], 'S', 'S', MTRTN_CY) IS NOT NULL" ).append("\n"); 

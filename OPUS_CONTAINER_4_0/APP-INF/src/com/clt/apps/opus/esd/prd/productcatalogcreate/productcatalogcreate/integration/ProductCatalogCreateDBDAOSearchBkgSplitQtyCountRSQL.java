...
public class ProductCatalogCreateDBDAOSearchBkgSplitQtyCountRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                         , DECODE(BKGM.FLEX_HGT_FLG, 'Y', GREATEST(MAX(CMAP.CNTR_TPSZ_CD), NVL(MAX(RPRL.PROV_CNTR_TPSZ_CD), ' ')), CMAP.CNTR_TPSZ_CD)  GRP_CNTR_TPSZ_CD" ).append("\n"); 
		...
		query.append("                      AND RPRL.PROV_CNTR_TPSZ_CD(+) = CMAP.CNTR_TPSZ_CD" ).append("\n"); 
		...
		query.append("                         , CEIL(BQTY.OP_CNTR_QTY) OP_CNTR_QTY -- CONTAINER각각에 대한 VOL_QTY가 들어오지 않기 때문에 어쩔수 없이 CEIL함 (기타 오류 가능성 있음) Park Mangeon 20101019" ).append("\n"); 
		...
		query.append("--                          WHERE BKG_GET_TOKEN_FNC(COLUMN_VALUE, 3, '@') = 'BKG'" ).append("\n"); 

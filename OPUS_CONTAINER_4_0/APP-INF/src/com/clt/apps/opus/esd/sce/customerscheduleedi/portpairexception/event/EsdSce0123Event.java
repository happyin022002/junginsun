...
public class EsdSce0123Event extends EventSupport {
	...
	public AdjustmentVO[] getAdjustmentVOs() {
		...
		if (this.adjustmentVOs != null) {
			rtnVOs = Arrays.copyOf(adjustmentVOs, adjustmentVOs.length);
		}
		return rtnVOs;
	}
	...
	public void setAdjustmentVOs(AdjustmentVO[] adjustmentVOs) {
		if(adjustmentVOs != null){
			AdjustmentVO[] tmpVOs = Arrays.copyOf(adjustmentVOs, adjustmentVOs.length);
			this.adjustmentVOs = tmpVOs;
		}
	}
	...
	public RouteForBLVO[] getRouteForBLVOs() {
		RouteForBLVO[] rtnVOs = null;
		if (this.routeForBLVOs != null) {
			rtnVOs = Arrays.copyOf(routeForBLVOs, routeForBLVOs.length);
		}
		return rtnVOs;
	}
	...
	public void setRouteForBLVOs(RouteForBLVO[] routeForBLVOs) {
		if(routeForBLVOs != null){
			RouteForBLVO[] tmpVOs = Arrays.copyOf(routeForBLVOs, routeForBLVOs.length);
			this.routeForBLVOs = tmpVOs;
		}
	}

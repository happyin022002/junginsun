...
public class AdjustmentManageDBDAOSearchResponsibleOfficeChangeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                 (SELECT OFC_CD FROM (SELECT N3PTY_OFC_TP_CD, OFC_CD, N3PTY_CTRL_OFC_CD, RHQ_CD FROM TPB_HNDL_OFC WHERE OFC_CD = @[s_user_ofc_cd] AND DELT_FLG='N') WHERE N3PTY_OFC_TP_CD='R'),'Y'," ).append("\n"); 
		...
		query.append("											SELECT DISTINCT RHQ_CD, N3PTY_CTRL_OFC_CD, OFC_CD FROM TPB_HNDL_OFC" ).append("\n"); 
		query.append("								   #end" ).append("\n"); 
		query.append("										   ) WHERE 1=1" ).append("\n"); 
		...
		query.append("										#end" ).append("\n"); 
		query.append("									  ) )" ).append("\n"); 
		query.append("    #if (${user_ofc_cd} == ${head_ofc_cd})" ).append("\n"); 
		query.append("       AND A.STL_FWRD_OFC_CD = (SELECT OFC_CD FROM MDM_OFC_GRP_MAPG WHERE SUB_SYS_CD='TPB' AND OFC_GRP_ID='000004' AND ROWNUM <= 1)" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("" ).append("\n"); 

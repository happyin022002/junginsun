...
*@FileName : ESD_LEA_0010HTMLAction.java
...
public class ESD_LEA_0010HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESD_LEA_0010HTMLAction 객체를 생성
	 */
	public ESD_LEA_0010HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdLea0010Event event 	= new EsdLea0010Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setSearchCostCodeListVO((SearchCostCodeListVO)getVO(request, SearchCostCodeListVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)){
			event.setSearchCostTypeCodeComboListVO((SearchCostTypeCodeComboListVO)getVO(request, SearchCostTypeCodeComboListVO .class));
		}else if(command.isCommand(FormCommand.SEARCH02)){
			event.setSearchSubCostTypeCodeComboListVO((SearchSubCostTypeCodeComboListVO)getVO(request, SearchSubCostTypeCodeComboListVO .class));
		//IB MULTI COMBO 조회
		}else if(command.isCommand(FormCommand.SEARCH20)){
			event.setCostKind			(request.getParameter("cost_kind")			);
			event.setMainCostTypeCode	(request.getParameter("sel_mn_cost_tp_cd")	);
		}
		return  event;
	}

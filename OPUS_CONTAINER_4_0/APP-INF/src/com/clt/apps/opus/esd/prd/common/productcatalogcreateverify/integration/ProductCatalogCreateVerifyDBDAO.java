...
public class ProductCatalogCreateVerifyDBDAO extends DBDAOSupport {
	private static final long serialVersionUID = -6288887183502933775L;
	 ...
	 * @throws DAOException
	 */
	public DBRowSet getPcVerify(EsdPrd0080Event event, PrdCreateParamVO prdCreateParamVO, PrdPcCreateVO prdPcCreateVO, PrdPatternVO prdPatternVO) throws DAOException {
		...
		try {
			...
			for (int i = 0; i < prdQuantityVO.length; i++) {
				if (prdSubQuantityVO != null && prdSubQuantityVO.length > 0) {
					arrTpsz.add(prdSubQuantityVO[i].getSTpsz() + prdSubQuantityVO[i].getSQty());
				} else {
					arrTpsz.add(prdQuantityVO[i].getCTpsz() + prdQuantityVO[i].getCQty());
				}
			}

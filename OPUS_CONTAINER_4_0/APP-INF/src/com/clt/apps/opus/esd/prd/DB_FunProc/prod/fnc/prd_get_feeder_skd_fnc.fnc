CREATE OR REPLACE FUNCTION OPUSADM.PRD_GET_FEEDER_SKD_FNC
/* ========================================================
   ...
   7. Revision History : 2009.08.24 최초 생성
                       : 2009.11.06 EXCEPTION 추가
                       : 2010.03.09 calling port seq 적용
                       : 2010.07.12 Double Call관련 Yard 비교를 위한 Data  Return
                       : 2010.08.19 Yard 값이 Null인 경우 공백 7문자 생기도록 처리
                       : 2011.02.07 PRD_PORT_TML_MTX.CRR_CD는 항상 FDR만 사용하도록 변경
...
--            WHERE DECODE(V_REVERSE,'Y',A_TIME,D_TIME) <= P_TIME 
            WHERE DECODE(V_REVERSE,'Y',A_TIME,D_TIME) <= P_TIME - 1/2 --2015-05-20 MR. JO
        )
        LOOP
            VV_ETD  := TO_CHAR(V_AA.D_TIME,'yyyymmddhh24miss') ;
            VV_ETB  := TO_CHAR(V_AA.A_TIME,'yyyymmddhh24miss') ;
    ...
    IF (VV_ETD IS NOT NULL) AND (VV_ETB IS NOT NULL) THEN
        ...
        ELSE
            (
                ...
                FROM (
                    ...
                    AND ORG.SKD_DIR_CD = NVL(V_DIR_CD, ORG.SKD_DIR_CD)
                    AND ORG.CRR_CD = 'FDR'
                    UNION ALL
                    SELECT ORG.TML_CD
        ...
        (
            ...
            FROM (
                ...
                AND DST.SKD_DIR_CD = NVL(V_DIR_CD, DST.SKD_DIR_CD)
                AND DST.CRR_CD = 'FDR'
                UNION ALL
                SELECT DST.TML_CD
        ...
        IF V_ORD = '1' THEN
            VV_CCT  := 'CCC' ||TO_CHAR(NVL(PRD_GET_CCT_FNC(VV_ORG_YD_CD1, V_SLANE_CD,V_DIR_CD,NVL(V_CGO_TP,'AL'),'','', TO_DATE(VV_ETD,'yyyymmddhh24miss'), TO_DATE(VV_ETD,'yyyymmddhh24miss') ),TRUNC(TO_DATE(VV_ETD,'yyyymmddhh24miss')) -7/24),'yyyymmddhh24miss') ;
            VV_CCNT := 'CCNC'||TO_CHAR(NVL(PRD_GET_CCT_FNC(VV_ORG_YD_CD2, V_SLANE_CD,V_DIR_CD,NVL(V_CGO_TP,'AL'),'','', TO_DATE(VV_ETD,'yyyymmddhh24miss'), TO_DATE(VV_ETD,'yyyymmddhh24miss') ),TRUNC(TO_DATE(VV_ETD,'yyyymmddhh24miss')) -7/24),'yyyymmddhh24miss') ;

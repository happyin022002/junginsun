...
public class ESD_TRS_0977HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsdTrs0977Event event = new EsdTrs0977Event();
		if (command.isCommand(FormCommand.MULTI)) {
			event.setSearchTrsSvcOrdBkgChmHisVOs((SearchTrsSvcOrdBkgChmHisVO[]) getVOs(request, SearchTrsSvcOrdBkgChmHisVO.class));
		} else if (command.isCommand(FormCommand.SEARCH)) {
			event.setWoIssueListVOs((WoIssueListVO[]) getVOs(request, WoIssueListVO.class));
		}
		return event;
	}

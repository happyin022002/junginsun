...
public class IrregularManageDBDAOSearchIrregularListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		, CASE	WHEN NVL(IH.DELT_FLG, 'N') <> 'Y' " ).append("\n"); 
		query.append("				THEN TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT OFC_CD FROM TABLE (COM_OfficeCodeMgr_PKG.COM_getOfficeCodeList_FNC('000001', 'TES'))), IH.CRE_DT, IH.OFC_CD), 'YYYY-MM-DD')" ).append("\n"); 
		query.append("				ELSE TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT OFC_CD FROM TABLE (COM_OfficeCodeMgr_PKG.COM_getOfficeCodeList_FNC('000001', 'TES'))), IH.UPD_DT, IH.OFC_CD), 'YYYY-MM-DD')" ).append("\n"); 

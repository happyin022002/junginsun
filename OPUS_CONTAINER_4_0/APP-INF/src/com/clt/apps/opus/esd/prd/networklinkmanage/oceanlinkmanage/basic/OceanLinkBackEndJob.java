...
import com.clt.apps.opus.esd.prd.networklinkmanage.oceanlinkmanage.integration.OceanLinkBackEndDBDAO;
...
/**
 * OPUS-Product Catalog Back End Job Business Logic Basic Command implementation<br>
 * - OPUS-Product Catalog BackEnd작업을 처리하는 Business Class
 * 
 * @author Park Mangeon
 * @see ESD_PRD_0012
 * @date 2009.06.05
...
public class OceanLinkBackEndJob extends BackEndCommandSupport {
	...
	public void setModifyOceanLink(SaveOceanLinkVO[] saveOceanLinkVOs, String ofcCd, String usrId) {
		if (saveOceanLinkVOs != null) {
			SaveOceanLinkVO[] tmp = Arrays.copyOf(saveOceanLinkVOs, saveOceanLinkVOs.length);
			this.saveOceanLinkVOs = tmp;
		}
		this.ofcCd = ofcCd;
		this.usrId = usrId;
	}
			...
			if (vos != null) {
				...
				for (int i = 0; i < vos.length; i++) {
					...
					// }
						...
						DBRowSet tmpDRs = dbDao.searchDirecOcnRout(vo.getPolprot(), vo.getPodprot(), vo.getLanecd(), vo.getDircd());
						// boolean isOcnRout = false;
						while (tmpDRs.next()) {
							isOcnRout = true;
							// history 생성 JSY
					...
					if (!isOcnRout) {
						...
						DBRowSet tmpDRs2 = dbDao.searchDirecOcnRout(vo.getPolprot(), vo.getPodprot(), vo.getLanecd(), vo.getDircd()); // 새로 추가된 ocn rout를 찾는다.
						if (tmpDRs2.next()) {
							// history 생성 JSY
					...
					} else if (vo.getIbflag().equals("U")) { // sStatus가 U , manual 일때

...
public class ProductCatalogCreateDBDAOCreateBkgSplitMapAdjCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("(" ).append("\n"); 
		query.append("    SELECT DECODE(MAPG.MTCH_FLG, 'Y', CMAP.ROWID, NULL) AS ROW_ID" ).append("\n"); 
		...
		query.append("                      , ROW_NUMBER() OVER (PARTITION BY MTCH_CD ORDER BY COP_NO ) MTCH_SEQ" ).append("\n"); 
		query.append("                 FROM (" ).append("\n"); 
		query.append("                     SELECT /*+ INDEX (M2 XAK1PRD_BKG_COP_MAP )*/  PCTL_NO" ).append("\n"); 
		query.append("                          , BKG_NO" ).append("\n"); 
		query.append("                          , COP_NO" ).append("\n"); 
		query.append("                          , COP_MAPG_SEQ" ).append("\n"); 
		query.append("                          -- 가급적 요청된 SIZE TYPE을 먼저 연결한 후, 호환 가능한 것으로 연결한다." ).append("\n"); 
		query.append("                          , CASE WHEN QTY1 > 0 AND TPSZ_CD1 = CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                                     AND ROW_NUMBER() OVER (PARTITION BY CNTR_TPSZ_CD ORDER BY COP_NO) <= QTY1 THEN TPSZ_CD1" ).append("\n"); 
		query.append("                                 WHEN QTY2 > 0 AND TPSZ_CD2 = CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                                     AND ROW_NUMBER() OVER (PARTITION BY CNTR_TPSZ_CD ORDER BY COP_NO) <= QTY2 THEN TPSZ_CD2" ).append("\n"); 
		query.append("                                 WHEN QTY1 > 0 AND TPSZ_CD2 = CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                                     AND (ROW_NUMBER() OVER (PARTITION BY CNTR_TPSZ_CD ORDER BY COP_NO) - NVL(QTY2,0)) <= QTY1 THEN TPSZ_CD1" ).append("\n"); 
		query.append("                                 WHEN QTY2 > 0 AND TPSZ_CD1 = CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                                     AND (ROW_NUMBER() OVER (PARTITION BY CNTR_TPSZ_CD ORDER BY COP_NO) - NVL(QTY1,0)) <= QTY2 THEN TPSZ_CD2" ).append("\n"); 
		...
		query.append("                                SELECT BKG_GET_TOKEN_FNC(COLUMN_VALUE, 1, '@') TPSZ_CD" ).append("\n"); 
		query.append("                                    , TO_NUMBER(BKG_GET_TOKEN_FNC(COLUMN_VALUE, 2, '@')) ADJ_CNTR_QTY" ).append("\n"); 
		query.append("                                    , NVL((SELECT PROV_CNTR_TPSZ_CD " ).append("\n"); 
		query.append("                                             FROM SCE_COP_CNTR_REPO_RULE RPRL " ).append("\n"); 
		query.append("                                            WHERE RPRL.CNTR_TPSZ_CD = BKG_GET_TOKEN_FNC(COLUMN_VALUE, 1, '@')" ).append("\n"); 
		query.append("                                              AND 'Y' = NVL(NVL(@[flex_hgt_flg], (SELECT FLEX_HGT_FLG FROM BKG_BOOKING WHERE BKG_NO = @[parent_bkg_no])), 'N'))" ).append("\n"); 
		query.append("                                         , BKG_GET_TOKEN_FNC(COLUMN_VALUE, 1, '@')) PROV_TPSZ_CD" ).append("\n"); 
		query.append("                                 FROM (TABLE(SELECT BKG_SPLIT_FNC(@[adj_tpsz_qty], ',') CNTR_TPSZ_QTY_TBL_STR FROM DUAL ))" ).append("\n"); 
		query.append("                                 WHERE TO_NUMBER(BKG_GET_TOKEN_FNC(COLUMN_VALUE, 2, '@')) > 0" ).append("\n"); 
		query.append("                               )" ).append("\n"); 
		query.append("                         GROUP BY GREATEST(TPSZ_CD, NVL(PROV_TPSZ_CD, ' '))" ).append("\n"); 
		query.append("                            ) AQTY" ).append("\n"); 
		query.append("                     WHERE BKG_NO IN ( SELECT COLUMN_VALUE AS BKG_NO  FROM TABLE(SELECT BKG_SPLIT_FNC(@[bkg_no_list], ',') BKG_NO_TBL_STR FROM DUAL ))" ).append("\n"); 
		...
		query.append("          	         AND (CNTR_NO IS NULL OR CNTR_NO = 'COMU0000000')" ).append("\n"); 
		query.append("                     AND (COP_PATT_ORD_NO = 1 OR  COP_PATT_ORD_NO IS NULL )" ).append("\n"); 
		query.append("                     )" ).append("\n"); 
		...
		query.append("      AND 0 = (SELECT COUNT(1) -- 한 세션중에 bkg_no가 2회 이상 호출되는 경우, 데이터가 다중 생성되는 것을 방지하기 위한 로직" ).append("\n"); 
		...
		query.append("      , 'COMU0000000', B.CNTR_TPSZ_CD, B.COP_OP_TP_CD, B.BKG_OP_RMK, B.OB_ITCHG_CTNT" ).append("\n"); 

...
public class PrdCreateManageDBDAOCreateActivityGroupIncludePatternCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                CASE 	WHEN cnt > 2 and RNK <>1 AND COST_ACT_GRP_CD ='NTST' AND RNK +1 = (LEAD(RNK) OVER(PARTITION BY COST_ACT_GRP_CD, SUBSTR(ORG_NOD_CD,1,5) ORDER BY PCTL_SEQ )) THEN 'NTSY'" ).append("\n"); 
		query.append("                    	ELSE COST_ACT_GRP_CD" ).append("\n"); 
		...
		query.append("					(CASE WHEN S.N1ST_NOD_TP_CD = 'Z' OR S.N2ND_NOD_TP_CD ='Z' OR S.N3RD_NOD_TP_CD = 'Z' OR S.N4TH_NOD_TP_CD = 'Z' THEN 'Z' ELSE 'C' END ) PCTL_COST_MOD_CD," ).append("\n"); 
		...
		query.append("									--DECODE(M.PCTL_IO_BND_CD,'I',DECODE(SUBSTR(MST.POD_CD,1,2), 'US','Y'), 'O',DECODE(SUBSTR(MST.POL_CD,1,2),'US','Y')) US_CHK" ).append("\n"); 

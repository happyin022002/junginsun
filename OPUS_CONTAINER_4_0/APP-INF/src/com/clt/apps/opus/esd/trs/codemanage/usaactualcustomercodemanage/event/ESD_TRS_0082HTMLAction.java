...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esd.trs.codemanage.usaactualcustomercodemanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESD_TRS_0082HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.MULTI) || command.isCommand(FormCommand.MULTI01) || command.isCommand(FormCommand.MULTI02)) {
			...
			event.setTrsTrspUsaActCustVOs(trsTrspUsaActCustVO.fromRequestGrid(request));
			event.setTrsTrspUsaActCustDtlVOS(trsTrspUsaActCustDtlVO.fromRequestGrid(request));
		}

...
public class AgreementImportDBDAOVerifyAgmtSurchargeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,@[trsp_tmp_seq] TRSP_TMP_SEQ" ).append("\n"); 
		...
		query.append("                END" ).append("\n"); 
		query.append("                -- 2015.01.22    Hyungwook Choi" ).append("\n"); 
		...
		query.append("                      -- 2015.03.13    Hyungwook Choi" ).append("\n"); 
		query.append("                      AND SCG_COND_CD                    = DECODE(T.TRSP_SCG_CD, 'SCFAAL', 'SCFCAL', T.TRSP_SCG_CD)" ).append("\n"); 
		query.append("--                      AND NVL(FM_LOC_COND_CD, 'N/A')     = DECODE(T.FM_NOD_CD, '0000000', 'N/A', 'NN')" ).append("\n"); 
		query.append("--                      AND NVL(VIA_LOC_COND_CD, 'N/A')    = DECODE(T.VIA_NOD_CD, '0000000', 'N/A', 'NN')" ).append("\n"); 
		query.append("--                      AND NVL(TO_LOC_COND_CD, 'N/A')     = DECODE(T.TO_NOD_CD, '0000000', 'N/A', 'NN')" ).append("\n"); 
		query.append("--                    AND NVL(OVWT_UT_COND_CD, 'N/A')     = DECODE(T.WGT_MEAS_UT_CD, 'XXX', 'N/A', 'NN')" ).append("\n"); 
		query.append("                      AND NVL(OVWT_UT_COND_CD, 'N/A')    = CASE WHEN T.TO_WGT > 0 THEN 'NN' ELSE 'N/A' END" ).append("\n"); 
		query.append("                      AND NVL(OVWT_STND_COND_CD, 'N/A')  = DECODE(T.TO_WGT, 0, 'N/A', 'NN')" ).append("\n"); 
		query.append("--                      AND TRSP_AGMT_CGO_TP_CD            = T.CGO_TP_CD" ).append("\n"); 
		query.append("                      AND NVL(TRSP_AGMT_CGO_TP_CD, '0')  = DECODE(NVL(T.CGO_TP_CD, '0'), 'M', '0', T.CGO_TP_CD)" ).append("\n"); 
		query.append("                      AND ROWNUM                         = 1" ).append("\n"); 
		...
		query.append("                    AND U.AGMT_TRSP_TP_CD               = T.AGMT_TRSP_TP_CD" ).append("\n"); 
		query.append("                    AND NVL(U.TRSP_BND_CD, 'X')         = NVL(T.TRSP_BND_CD, 'X')" ).append("\n"); 
		...
		query.append("                   AND NVL(C.TRSP_BND_CD, 'X')         = NVL(T.TRSP_BND_CD, 'X')" ).append("\n"); 
		...
		query.append("                   AND D.TRSP_SCG_CD                   = T.TRSP_SCG_CD -- 2015.01.26    Hyungwook Choi" ).append("\n"); 

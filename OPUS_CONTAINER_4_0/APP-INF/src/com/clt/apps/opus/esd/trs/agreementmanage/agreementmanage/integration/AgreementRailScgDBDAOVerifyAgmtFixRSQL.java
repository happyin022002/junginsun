...
*@FileName : AgreementRailScgDBDAOVerifyAgmtFixRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AgreementRailScgDBDAOVerifyAgmtFixRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Fix Type AgreementFileImport 의 Verify 수행
	  * </pre>
	  */
	public AgreementRailScgDBDAOVerifyAgmtFixRSQL(){
		setQuery();
		...
		query.append("FileName : AgreementRailScgDBDAOVerifyAgmtFixRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("		CASE WHEN T.AGMT_ROUT_ALL_FLG = 'N' AND T.FM_NOD_CD IS NULL AND T.TO_NOD_CD IS NULL" ).append("\n"); 
		...
		query.append("		END" ).append("\n"); 
		query.append("		|| -- Over Weight Check" ).append("\n"); 
		query.append("		CASE WHEN T.TRSP_RAIL_SCG_CD = 'OWS' AND T.LBS_OVR_WGT IS NULL" ).append("\n"); 
		query.append("		THEN ',OVER WEIGHT ERR'" ).append("\n"); 
		query.append("--		WHEN T.TRSP_RAIL_SCG_CD = 'OWS' " ).append("\n"); 
		query.append("--			AND (   (T.FX_SCG_20FT_RT IS NOT NULL AND T.FX_SCG_40FT_RT IS NOT NULL)" ).append("\n"); 
		query.append("--				 OR (T.FX_SCG_20FT_RT IS NOT NULL AND T.FX_SCG_45FT_RT IS NOT NULL)" ).append("\n"); 
		query.append("--				 OR (T.FX_SCG_40FT_RT IS NOT NULL AND T.FX_SCG_45FT_RT IS NOT NULL)" ).append("\n"); 
		query.append("--				 OR (T.FX_SCG_20FT_RT IS NOT NULL AND T.FX_SCG_40FT_RT IS NOT NULL AND T.FX_SCG_45FT_RT IS NOT NULL)" ).append("\n"); 
		query.append("--            	)" ).append("\n"); 
		query.append("--		THEN ',OVER WEIGHT ERR'" ).append("\n"); 
		query.append("		END		" ).append("\n"); 
		query.append("		||-- Effective date 체크" ).append("\n"); 
		query.append("		CASE WHEN  T.EFF_TO_DT IS NULL OR T.EFF_FM_DT IS NULL OR T.EFF_TO_DT - T.EFF_FM_DT < 0" ).append("\n"); 
		query.append("		THEN ',EFFECTIVE DATE ERR'" ).append("\n"); 
		query.append("		END" ).append("\n"); 
		query.append("		||-- Currency code 체크" ).append("\n"); 
		query.append("		CASE WHEN T.CURR_CD IS NULL" ).append("\n"); 
		query.append("		THEN ',CURRENCY ERR'" ).append("\n"); 
		query.append("		END " ).append("\n"); 
		query.append("		||-- Amount 체크" ).append("\n"); 
		query.append("		CASE " ).append("\n"); 
		query.append("--        WHEN T.TRSP_RAIL_SCG_CD = 'OWS' AND T.AGMT_EQ_SZ_NO = 'AL' AND T.FX_SCG_ALL_RT IS NULL" ).append("\n"); 
		query.append("--		THEN ',AMOUNT ERR'" ).append("\n"); 
		query.append("--		WHEN T.TRSP_RAIL_SCG_CD = 'OWS' AND T.AGMT_EQ_SZ_NO = '20' AND T.FX_SCG_20FT_RT IS NULL" ).append("\n"); 
		query.append("--		THEN ',AMOUNT ERR'" ).append("\n"); 
		query.append("--		WHEN T.TRSP_RAIL_SCG_CD = 'OWS' AND T.AGMT_EQ_SZ_NO = '40' AND T.FX_SCG_40FT_RT IS NULL" ).append("\n"); 
		query.append("--		THEN ',AMOUNT ERR'" ).append("\n"); 
		query.append("--		WHEN T.TRSP_RAIL_SCG_CD = 'OWS' AND T.AGMT_EQ_SZ_NO = '45' AND T.FX_SCG_45FT_RT IS NULL" ).append("\n"); 
		query.append("--		THEN ',AMOUNT ERR'" ).append("\n"); 
		query.append("        WHEN NVL(T.FX_SCG_ALL_RT, 0) > 0 AND (NVL(T.FX_SCG_20FT_RT, 0) + NVL(T.FX_SCG_40FT_RT, 0) + NVL(T.FX_SCG_45FT_RT, 0)) > 0" ).append("\n"); 
		query.append("        THEN ',AMOUNT ERR'" ).append("\n"); 
		query.append("		END " ).append("\n"); 
		query.append("		||-- All 항목 체크" ).append("\n"); 
		query.append("		CASE WHEN T.AGMT_ROUT_ALL_FLG = 'Y'" ).append("\n"); 
		query.append("				AND (T.FM_NOD_CD IS NOT NULL AND T.FM_NOD_CD <>'00')" ).append("\n"); 
		query.append("				AND (T.TO_NOD_CD IS NOT NULL AND T.TO_NOD_CD <> '00')" ).append("\n"); 
		query.append("		THEN ',ALL CHECK'" ).append("\n"); 
		query.append("--		WHEN (T.FX_SCG_ALL_RT IS NOT NULL AND T.FX_SCG_ALL_RT <> 0)" ).append("\n"); 
		query.append("--			AND (   (T.FX_SCG_20FT_RT IS NOT NULL AND T.FX_SCG_20FT_RT <> 0)" ).append("\n"); 
		query.append("--			     OR (T.FX_SCG_40FT_RT IS NOT NULL AND T.FX_SCG_40FT_RT <> 0)" ).append("\n"); 
		query.append("--			     OR (T.FX_SCG_45FT_RT IS NOT NULL AND T.FX_SCG_45FT_RT <> 0))" ).append("\n"); 
		query.append("--		THEN ',ALL CHECK'" ).append("\n"); 
		query.append("		END" ).append("\n"); 
		...
		query.append("        AND   NVL(U.USR_DEF_RMK, 'X') = NVL(T.USR_DEF_RMK, 'X')" ).append("\n"); 

...
public class BkgCopManageBCImpl extends BasicCommandSupport implements BkgCopManageBC {
	...
	/**
	 * 구주 TRO / I, 구주 TRO / O, 미주 TRO / O 가 confirm 될 때 호출됨 구주 TRO 는 TRO SEQ 별로, 미주 TRO 는 TRO_SUB_SEQ 별로 mapping 될 COP 를 지정한다. 지정된 COP 를 넘겨 받은 pctl_no 로 replan 하여 Door S/O 생성 대상을 관리할 수 있게 한다. 또한 Door arrival date (estimate) 값을 받아 해당 skd 을 COP Detail 에 적용한다.
	 * 
	 * @param bkg_no
	 * @param tro_seq
	 * @param tro_sub_seq
	...
	public void confirmTro(String bkg_no, String tro_seq, String tro_sub_seq, String io_bnd_cd, String pctl_no, String area_conti_cd) throws EventException {
		try {
			...
			if (validateTROSequenceVOs != null && validateTROSequenceVOs.size() != 0) {
				...
				if (searchCopVOs == null || searchCopVOs.size() == 0) {
					...
					if (pctl_no != null && !pctl_no.equals("")) {
						...
						if (isRplnNeeded) {
							...
							// String railChk = dbDao.searchCopRailChk(pctl_no);
				...
				/**
				 * TRO confirm 시의 miscellaneous 업무 <outbound> 1. header 의 cop 의 PCTL_NO 를 신규 PC no 로 수정 2. outbound 의 exception 을 O, R 을 X 로 변경하고 ntfd_flg = 'N' 3. SCE_PLN_SO_LIST 재생성시에는 SO 의 정보를 보존하여 주어야 함 4. replan 에서의 actual mapping 을 수행하여야 함 5. OD 의 Frustrated 된 S/O status 를 'P' 로 변경함 6. pol 과
				 * pol 의 departure node 가 다를 경우 pol 로 pol departure 의 node 를 변경 (?) : 필요한가? 7. MOTZAD 의 estimated date 를 TRO 의 door date 와 비교하여 날짜가 지연됬을 경우 re-scheduling / 줄었을 경우 기존 시간차를 반영하여 수정 8. pc no 가 없거나 기존과 같을 경우 에 TRO 에 MTY Pick up yard,등의 정보를 cop 에 update : 할 필요 없음 --> PC 에서 해당 case 에 관리
				 * 구주는 BKG_EUR_TRO, 기타지역은 BKG_TRO_DTL 참조해야 할듯 COP 생성 및 관리는 replan 으로 넘긴다.
				 */
					...
					if (sceCopDtlVOList.size() > 0) {
						...
						if (estm_dt != null && !estm_dt.equals("") && estm_dt.length() == "YYYYMMDDHHMISS".length()) {
							...
							if (dtlEstm != null && troEstm != null) {
								...
								log.error("While TRO Rescheduling, estm_Dt size error occurred!!!");
							}
						}
					}
				} else {
				...
				/**
				 * 
				 * 날짜 정보 mapping(ASIS) 구주 O/B 구주 I/B 미/아주 O/B 미/아주 I/B EMPTY RETURN YARD CODE TRM_PICKUP_CY TRM_RETURN_CY TRD_PICKUP_LOC||TRD_PICKUP_CY TRD_RETURN_LOC||TRD_RETURN_CY DOOR NODE CODE TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD CONTAINER RETURN DATE
				 * TRM_RETURN_DT TRM_PICKUP_DT N/A N/A DOOR ARRIVAL DATE TRD_DOOR_DT TRD_DOOR_DT TRD_DT TRD_DT
				 * 
				 * 
				 * 날짜 정보 mapping(TOBE) 구주 O/B 구주 I/B 미/아주 O/B 미/아주 I/B EMPTY RETURN YARD CODE TRM_PICKUP_CY TRM_RETURN_CY PKUP_LOC_CD||PKUPLOC_CD RTN_LOC_CD||RTN_YD_CD DOOR NODE CODE TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD TRD_DOOR_LOC||ZN_CD CONTAINER RETURN DATE TRM_RETURN_DT
				 * TRM_PICKUP_DT N/A N/A DOOR ARRIVAL DATE TRD_DOOR_DT TRD_DOOR_DT DOR_ARR_DT DOR_ARR_DT
				 */
			}
	 ...
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#createBkg(java.lang.String, java.lang.String)
	 */
	public void createBkg(String bkg_no, String pctl_no) throws EventException {
		try {
			...
			prdCreateManageBC.updateActivityGroupIncludePattern(null, pctl_no, null, "SYS");
			...
			if (rtnList != null && rtnList.size() != 0) {
				for (int i = 0; i < rtnList.size(); i++) {
					...
					// ==> 20150128-1 기존 주석 처리 부분 해제 START
					// ==> 20150128-2BKG DATA 생성 시점과 틀려 DATA 생성 안되어 주석 처리함
					/*
					 * List<SceCopHdrVO> railrcvList = dbDao.searchRailRcvCoffDt(bkg_no); if (railrcvList.size() > 0) { SceCopHdrVO tmpVO = railrcvList.get(0); rowVO.setRailRcvCoffFmDt(tmpVO.getRailRcvCoffFmDt()); rowVO.setRailRcvCoffToDt(tmpVO.getRailRcvCoffToDt()); }
					 */
					// ==> 20150128 기존 주석 처리 부분 해제 END
	 ...
	 * @param io_bnd_cd
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#unconfirmTro(java.lang.String, java.lang.String)
	 */
			...
			if (validateTROSequenceVOs != null && validateTROSequenceVOs.size() != 0) {
				...
				while (litor.hasNext()) {
					...
					if (odsoList != null && odsoList.size() > 0) {
						...
						} else if (validateTROSequenceVO.getIoBndCd().equals("O")) {
	 ...
	 * @param io_bnd_cd
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#unconfirmTro(java.lang.String, java.lang.String, java.lang.String, java.lang.String)
	 */
	 ...
	 * @param partialFlag
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#detachCntr(java.lang.String, java.lang.String, java.lang.String)
	 */
	...
	private SceCopHdrVO detachCntrMain(String bkg_no, String cntr_no, String partialFlag) throws DAOException, Exception {
		...
		if (toBeDetached.getCopNo().equals(toBeDetached.getMstCopNo())) {// partial 이며 Master 인 COP 의 container 가 detach 될 경우
			/**
			 * master flag 를 옮긴다. trnk_vsl_cd, trnk_skd_voy_no, trnk_skd_dir_cd, cntr_no, pol_cd, bkg_cgo_tp_cd 가 동일한 COP 를 찾아낸다. 없을 경우에는 booking 에서 해당 case 를 다시 찾아보고 찾을 수 없을 경우에는 flag 가 잘못 넘어온것으로 판단하고 FCL 로직으로 간다.
			 * 
			 * S/O 는 detach 전 booking 에서 validation 하므로 신경 쓰지 않는다. TRO / OD-SO 는 Booking 관련이므로 그냥 둔다.
			 * 
			 * CA (Correction advice) 의 경우도 마찬가지로 S/O validation 은 있으모로 신경 안써도 된다. (CA 는 단지 update 불가를 화면단위에서 푸는 것으로 세부 로직은 동일하다)
			 */
								...
								for (int i = 0; i < oldSoList.size(); i++) {
									...
									for (int j = 0; j < newSoList.size(); j++) {
										...
										if (oldSo.getTrspSoStsCd() != null && !oldSo.getTrspSoStsCd().equals(SCEConstants.SO_PLANNED)) {
											...
											if (n1st_nod_cd.equals(new_n1st_nod_cd) && n2nd_nod_cd.equals(new_n2nd_nod_cd) && n3rd_nod_cd.equals(new_n3rd_nod_cd) && n4th_nod_cd.equals(new_n4th_nod_cd) && cost_act_grp_cd.equals(new_cost_act_grp_cd) && trsp_mod_cd.equals(new_trsp_mod_cd)) {
		...
		if (!dbDao.checkCopToBeTroMapgAlive(toBeDetached.getCopNo())) { // sce_tro_mapg에 데이타가 있으면 cancel하지 않음
	 ...
	 * @param bkg_no
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#cancelBkg(java.lang.String)
	 */
	 ...
	 * @param isPartial
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#attachCntr(java.lang.String, java.lang.String, java.lang.String)
	 */
	public void attachCntr(String bkg_no, String cntr_no, String isPartial) throws EventException {
		/**
		 * 1. attach 되려는 container 가 다른 cop 에 attach 되어 있고 finish 되지 않았으며 Inbouond 까지 진행 되었고 vvd 가 다를 경우는 finish 시키고 exception resolve, MT 전송 여부 확인 후 edi 전송 등을 시도
		 * 
		 * 2. attach 되려는 container 가 동일 vvd 로 다른 cop 에 쓰이고 있다면 partial 이므로 master cop no 를 가져온다.
		 * 
		 * 3. prov_cntr_no prov_cntr_no 가 선 할당된 cop 에 먼저 붙인다. prov_cntr_no 가 할당된 cop 가 master 가 되어야 하는 법인듯..
		 * 
	...
	/**
	 * Mixed term 이다. attach 될 container 의 receive term 이 door 이다 : attach 대상을 tro confirm 된 것을 찾는다. 없으면 임의의 cop. yard 이다 : attach 대상을 tro confirm 되지 않은 것을 찾는다. 모두 tro confirm 되어 있다면 새로 COP 생성!
	 * 
	 * @param bkg_no
	 * @param cntr_no
	 * @param isPartial
	...
	private void attachCntrMain(String bkg_no, String cntr_no, String isPartial) throws DAOException, Exception {
		// log.error("********* ATTACH-CNTR-MAIN Called! bkg_no = " + bkg_no + " / cntr_no = " + cntr_no + " isPartial = " + isPartial);
		...
		if (bkgContainerVOList.size() == 0) {
			...
			tmpCntrTpszCd = StringUtils.isEmpty(bkgContainerVO.getCntrTpszCd()) ? "" : bkgContainerVO.getCntrTpszCd();
			// tmpCreDt = StringUtils.isEmpty(bkgContainerVO.getCreDt()) ? "" : bkgContainerVO.getCreDt();
			tmpRcvTermCd = StringUtils.isEmpty(bkgContainerVO.getRcvTermCd()) ? "" : bkgContainerVO.getRcvTermCd();
		}

		// container tpsz 가 조회가 안된 case 의 보완 부분 : mst_container 를 재 조회
		if (bkgContainerVO != null && StringUtils.isEmpty(tmpCntrTpszCd)) {
		...
		final String org_cntrTpszCd = tmpCntrTpszCd;

		Set<String> cntrTpszCdSet = new HashSet<String>();

		cntrTpszCdSet.add(tmpCntrTpszCd);

		// String bkg_evnt_dt = tmpCreDt;
		...
		if (bkgBookingVO.getFlexHgtFlg().equals("Y")) {
			Set<String> repoTpsz = dbDao.searchRepoRule(tmpCntrTpszCd);
			if (repoTpsz != null && repoTpsz.size() > 0) {
				cntrTpszCdSet.addAll(repoTpsz);
			}
		}

		if (null != bkgContainerVO) {
			if (bkgBookingVO.getRcvTermCd().equals("M") && (bkgContainerVO.getRcvTermCd() != null && !bkgContainerVO.getRcvTermCd().equals("D")) || bkgBookingVO.getDeTermCd().equals("M") && (bkgContainerVO.getDeTermCd() != null && !bkgContainerVO.getDeTermCd().equals("D"))) {
				isMixed = true;
			}
		}
		...
		List<SceCopHdrVO> sceCopHdrVOList = dbDao.searchProvCntr(bkg_no, cntr_no);
		...
		if (sceCopHdrVOList.size() >= 1) {
			...
			attachCopHdr.setCntrTpszCd(tmpCntrTpszCd);
			...
			if (bkgBookingVO.getRcvTermCd().equals("M") && StringUtils.isNotEmpty(tmpRcvTermCd) && tmpRcvTermCd.equals("D")) {
				for (int i = 0; i < attachCopHdrVOs.size(); i++) {
					SceCopHdrVO rowTmp = attachCopHdrVOs.get(i);
					// tro confirm 되고 od-so 여부는 중요하지 않다.. 나던 안나던....
					if (rowTmp.getCntrTpszCd().equals(org_cntrTpszCd) && (rowTmp.getObTroFlg().equals("Y"))) {
						attachCopHdr = rowTmp;
						break;
					}
				}
			} else if (bkgBookingVO.getRcvTermCd().equals("M") && StringUtils.isNotEmpty(tmpRcvTermCd) && !tmpRcvTermCd.equals("D")) {
				// tro confirm 되지 않은 COP 를 찾고
				for (int i = 0; i < attachCopHdrVOs.size(); i++) {
					SceCopHdrVO rowTmp = attachCopHdrVOs.get(i);
					// tro confirm 되고 od-so 여부는 중요하지 않다.. 나던 안나던....
					if (rowTmp.getCntrTpszCd().equals(org_cntrTpszCd) && (rowTmp.getObTroFlg() == null || rowTmp.getObTroFlg().equals("N"))) {
						attachCopHdr = rowTmp;
						break;
					}
				}
		...
		// // log.error("dtlVO.getCopNo() is null ! > Bkg_no = " +bkg_no + " / getCntrNo	 = " + attachCopHdr.getCntrNo());
		// // } else {
		// // log.error("dtlVO cop_no exists = " + dtlVO.getCopNo());
		// // }
		//
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#combineBkg(java.lang.String[], java.lang.String, java.util.List)
	 */
	public void combineBkg(String[] org_bkg_no, String combined_bkg_no, List<CombineTroNewSeqVO> combineTroNewSeqVOs) throws EventException {
		try {
			...
			Set<String> dupBkgCntr = new HashSet<String>();
			...
			for (int i = 0; i < slaveBkgCopList.size(); i++) {
				...
				if (sceCopHdrVO.getCntrNo() != null && !sceCopHdrVO.getCntrNo().equals(SCEConstants.PSEUDO_CNTR_NO)) { // pseudo container 가 아닐때만 actual mapping 시도
					List<SceCopHdrVO> prtlList = null;
					if(!dupBkgCntr.contains(sceCopHdrVO.getBkgNo() +"|"+sceCopHdrVO.getCntrNo())) {
						dupBkgCntr.add(sceCopHdrVO.getBkgNo() + "|" + sceCopHdrVO.getCntrNo());
						prtlList = dbDao.searchPartialCops(sceCopHdrVO.getBkgNo(), sceCopHdrVO.getCntrNo(), "C");
					}
					if (prtlList == null || prtlList.isEmpty()) {
			...
			/*
			 * Combine시에는 Combine target bkg에 source bkg에 속한 Cntr가 존재하지 않을수도 있기때문에 Source bkg와 비교하여 속하지 않은 CNTR의 경우 Source cop를 Actual Event copy 대상으로 추가한다. 단 Actual Event copy시 bkg no는 Combine target bkg로 변경한다.
			 */
			...
			for (int i = 0; i < copyActRcvIfList.size(); i++) {
				...
				if (oldBkgList.size() > 0) {
					...
					/*
					 * copy 하지 못했다는 의미는 partial bkg중 하나에 대해 actual 정보가 존재한다는 의미다 이경우 new bkg에 대해 actual 정보가 존재하는 경우가 아니면 1회성으로만 mapping 하도록 한다 replan 과 개별 event mapping 시에는 partial bkg 모두에 대해서 mapping 되지만 이전 발생 event에 대해서는 자동 mapping이 되지 않기 때문이다.
					 */
					if (insertCnt == 0) {
	 ...
	 * @param new_bkg_no
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#splitBkg(java.lang.String, java.lang.String[])
	 */
	...
	/**
	 * 2010.08.18 : split 시 COP 의 BKG_NO 가 변경될 경우 이전 / 이후 bkg_no 로 SCE_TRO_MAPG 의 BKG_NO 를 변경하여 준다. 또한 기 발생된 OD/S 포함 모든 bkg 의 BKG_NO / BL_NO 역시 변경시킨다.
	 * 
	 * @param org_bkg_no
	 * @param new_bkg_no
	 * @param mapg_seq
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#splitBkg(java.lang.String, java.lang.String[], java.lang.String[])
	 */
		...
		try {
			...
			for (int i = 0; i < prdBkgCopMapVOs.size(); i++) {
				...
				if (cop_op_tp_cd.equals(SCEConstants.CREATION_FRM_MAP)) {
					...
					prdCreateManageBC.updateActivityGroupIncludePattern(prdBkgCopMapVO, prdBkgCopMapVO.getPctlNo(), null, "SYS");

					List<SceCopHdrVO> tobeCrtCopHdrList = dbDao.searchCopHdrToBeCreated(prdBkgCopMapVO.getBkgNo(), prdBkgCopMapVO.getCntrNo(), prdBkgCopMapVO.getCntrTpszCd(), prdBkgCopMapVO.getPctlNo(), prdBkgCopMapVO.getCopNo());
	...
	private void interfaceLeaCntrBkgHis(String[] org_bkg_no, String[] new_bkg_no) throws DAOException {
		log.info("NOT USE LEA_BKG_CNTR_HIS!!");
		/*
		 * 2015.10.16 NOT USE INTERFACE
		for (int i = 0; i < org_bkg_no.length; i++) {
...
//	}
	 ...
	 * @param cntr_no
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#moveCntr(java.lang.String, java.lang.String[], java.lang.String)
	 */
	public void moveCntr(String org_bkg_no, String[] dest_bkg_no, String cntr_no) throws EventException {
		...
		try {
			...
			String tmpDestBkgNo = "";
			if (null != dest_bkg_no && dest_bkg_no.length > 0) {
				tmpDestBkgNo = transArrayIntoString(dest_bkg_no);
			}

			tmp.setBkgEvntRmk("Container Move : Origin = " + org_bkg_no + " / Dest = " + tmpDestBkgNo + " / cntr_no = " + cntr_no + " / partial = " + prtl_flg);
			// tmp.setPctlNo(pctl_no);
			SceBkgOpHisVO sceBkgOpHisVO = addBkgOpHis(tmp);
			...
			addBkgOpPara(sceBkgOpHisVO, new Integer(rowNum++).toString(), tmpDestBkgNo); // 2015.04.15 Critical Modify
			addBkgOpPara(sceBkgOpHisVO, new Integer(rowNum++).toString(), cntr_no);
			...
			SceCopHdrVO oldCop = null;
			String tmpCopNo = "";
			if (orgCopList != null && orgCopList.size() > 0) {
				oldCop = orgCopList.get(0);
				tmpCopNo = StringUtils.isBlank(oldCop.getCopNo()) ? "" : oldCop.getCopNo();
			} else {

			}
			List<ScePlnSoListVO> orgPlnList = dbDao.searchPlnSoList(tmpCopNo);
			...
			if (null != dest_bkg_no && dest_bkg_no.length > 0) {
				for (int i = 0; i < dest_bkg_no.length; i++) {
			...
			if (changedMst == null) {
				...
				if (null != dest_bkg_no && dest_bkg_no.length > 0) {
					...
					if (bfCopList != null && bfCopList.size() > 0) {
						...
						if(toBePlnSoList != null && toBePlnSoList.size() > 0) {
							...
							for (int i = 0; i < toBePlnSoList.size(); i++) {
								...
								for (int j = 0; j < orgPlnList.size(); j++) {
									...
									if (crntRowVO != null && crntRowVO.getCostActGrpSeq().equals(toBeRowVO.getCostActGrpSeq())) {
										// so 가 발생한 cop 의 cost_act_grp_seq 에 대해서 so status 를 변경
										if (crntRowVO.getTrspSoStsCd() != null && !crntRowVO.getTrspSoStsCd().equals("") && !crntRowVO.getTrspSoStsCd().equals(SCEConstants.SO_PLANNED)) {
							...
							/**
							 * Origin SCE_PLN_SO_LIST의 TRSP_SO_STS_CD 상태를 'P-> Plan' 상태로 변경.
							 */
							for (int j = 0; j < orgPlnList.size(); j++) {
								ScePlnSoListVO crntRowVO = orgPlnList.get(j);
								if (crntRowVO != null && crntRowVO.getTrspSoStsCd() != null && !crntRowVO.getTrspSoStsCd().equals(SCEConstants.SO_PLANNED)) {
									crntRowVO.setTrspSoStsCd(SCEConstants.SO_PLANNED);
									toBeUpdatedVO.add(crntRowVO);
								}
							}		
						}
	 ...
	 * @return SearchCopVO
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#searchCopNoFrmPrdByTro(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String)
	 */
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#updateBkg(com.clt.syscommon.common.table.PrdBkgCopMapVO)
	 */
	 ...
	 * @param mapg_seq
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#updateBkg(java.lang.String, java.lang.String)
	 */
	public void updateBkg(String bkg_no, String mapg_seq) throws EventException {
		...
		List<String> pctlNoArray = new ArrayList<String>();
		...
		try {
			...
			for (int i = 0; i < prdBkgCopMapVOList.size(); i++) {
				...
				if (!cop_op_tp_cd.equals(SCEConstants.CREATION_FRM_MAP)) {
					if (!dbDao.checkCopToBeTroMapgAlive(cop_no)) { // sce_tro_mapg에 데이타가 있으면 cancel하지 않음
				...
				if (cop_op_tp_cd.equals(SCEConstants.PRD_CHANGE_FRM_MAP) || cop_op_tp_cd.equals(SCEConstants.PSEUDO_VVD_UPDATE_FRM_MAP)) {
					...
					// 2015.04.17 소스품질 Modify isPctlNoContains pctlNoArray null 이면 false 로 리턴 하므로 바로 null 체크로 수정함.
					if (!isPctlNoContains(pctlNoArray, pctl_no)) {
						dbDao.callPrdSetActDwTimePrc(pctl_no, "I", "N", "SYS");
						prdCreateManageBC.updateActivityGroupIncludePattern(prdBkgCopMapVO, pctl_no, new String[] { mapg_seq }, "SYS");
					...
					// cancel 시킬 cop 가 tro 를 가지고 있다면 tro 이동을 시도한다. 만약 tro 이동이 불가하면 cancel 을 skip 한다.
			...
			if (backEndFlg) {
				...
				List<List<SceActRcvIfVO>> actualListArray = new ArrayList<List<SceActRcvIfVO>>();
	 ...
	 * @param bkg_no
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#confirmCntr(java.lang.String)
	 */
	public void confirmCntr(String bkg_no) throws EventException {
		...
		try {
			...
			// 1. cop 정보 (COMU0000000) 를 가져와 해당 cop 가 bkg_quantity - bkg_container 의 범위 내에 존재하는지 check
			...
			/**
			 * . TRO 의 정리 작업 진행 (1) bkg_eur_tro_dtl, bkg_tro 와 상호 비교 작업 (2) ob_tro_flg, ib_tro_flg 정리
			 */
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#searchPresetBfRpln(java.lang.String)
	 */
	 ...
	 * @throws EventException
	 */
	// private void addBkgOpPara(SceBkgOpHisVO sceBkgOpHisVO, String param_seq, String[] paramAry) throws EventException {
	 ...
	 * @throws EventException
	 */
	private void addBkgOpPara(SceBkgOpHisVO sceBkgOpHisVO, String param_seq, String param) throws EventException {
	 ...
	 * @return int
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#modifyRailRcvCoffDt(java.lang.String, java.lang.String, java.lang.String, java.lang.String)
	 */
	 ...
	 * @return int
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#initializeSceActRcvIf(java.lang.String, java.lang.String)
	 */
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#searchActTmlIfDtl(java.lang.String, java.lang.String)
	 */
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#searchCntrDiff(java.lang.String, java.lang.String)
	 */
	 ...
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#copyCop(java.lang.String, java.lang.String, java.lang.String)
	 */
	public List<CopHdrToBeCopiedVO> copyCop(String bkg_no, String cop_sts_cd, String cop_upd_rmk) throws EventException {
		/**
		 * 신규 생성될 COP_NO 와 기존 COP_NO 를 가지는 COPY 대상 HEADER 정보를 조회 조회된 정보로 ORG_COP_NO 의 정보를 조회해 신규 COP DETAIL 과 PLN SO LIST 를 생성한다
		 */
	...
	/**
	 * 정리할 tro 는 O/B 것만이 대상이된다. ib tro 가 존재시는 cntr 와 match 되기 때문에 qty 정리 대상이나 cop cancel, container detach 대상이 될 수 없기 때문이다. <br>
	 * 
	 * container detach 또는 booking replan 의 Cancel event 시 호출되어 (단, cop 에 tro 가 mapping 되어 있어야 함) <br>
	 * container detach 시에는 flex height 를 감안한 tpsz 호환 가능한 container 가 존재하는 COP 를 찾고 (TRO 가 미 mapping) <br>
	 * booking replan 시의 cancel event 도 역시 flex height 를 감안한 tpsz 호환 가능한 container 가 존재하는 cop 를 찾고 (tro 가 미 mapping) <br>
	 * from -> to cop 관계를 생성한 후 아래의 작업을 진행한다.<br>
	 * 
	 * 1. from cop 의 tro flag 를 'N', to cop 의 tro flag 를 'Y' 로 수정<br>
	 * 2. sce_tro_mapg 의 from cop 를 to cop 로 변경 <br>
	 * 3. od-so 의 cop_no 를 from cop 에서 to cop 로 변경 <br>
	 * 4. to cop 가 so 의 from ~ via -door -to (or from door via to) 경로를 가지고 있지 않을 경우 cop 를 replan <br>
	 * 5. replan 된 cop 를 대상으로 from cop 의 so status 를 변경함 <br>
	 * 6. 이동 성공 여부를 return (성공시 container detach 시에는 해당 tpsz 가 booking qty 에 미 존재시 cancel, booking replan 의 cancel 시에는 단순 cancel 처리 한다)<br>
	 * 
	 * 대표적인 case > bkg qty 는 D4 이어 tro 를 모두 d4 로 냈으나 container 가 추가로 붙고 tro 가 존재하는 cop 에 mapping 된 container 를 detach<br>
	 * 
	 * 
	 * @param String cop_no
	 * @return boolean
	 ...
	 * @param String bkg_no
	 * @throws EventException
	 * @see com.clt.apps.opus.esd.sce.bkgcopmanage.basic.BkgCopManageBC#reviveCopsByBkgRqst(java.lang.String)
	 */
	 ...
	 */
	/*
	 * private boolean isPctlNoContains(String[] pctlNos, String curPctlNo){ boolean returnFlag = false; if(pctlNos != null){ for(String pctlNo : pctlNos){ if(curPctlNo != null && pctlNo.equals(curPctlNo)){ returnFlag = true; break; } } } return returnFlag; }
	 */
	 ...
	 */
	/*
	 * private String[] addPctlNos(String[] pctlNos, String curPctlNo){ String [] tempArray2 = null; int i=0; if(pctlNos == null){ tempArray2 = new String[1]; tempArray2[0] = curPctlNo; }else{ tempArray2 = new String [pctlNos.length+1]; for(String pctlNo : pctlNos){ tempArray2[i++] = pctlNo; }
	 * tempArray2[i] = curPctlNo; } return tempArray2; }
	 */
	 ...
	 * @return
	 */
	private boolean isPctlNoContains(List<String> pctlNos, String curPctlNo) {
		boolean returnFlag = false;
		if (null != pctlNos && pctlNos.size() > 0) {
	 ...
	 * @return
	 */
	private List<String> addPctlNos(List<String> pctlNos, String curPctlNo) {
		// List<String> tempArray2 = new ArrayList<String>();
		int iExistCnt = 0;
		if (pctlNos.size() > 0) {
			for (String pctlNo : pctlNos) {
				if (pctlNo.equals(curPctlNo)) {
					iExistCnt++;
				}
			}
			// 0 Count 이면 미 등록된 PCTL NO 이므로 Add.
			if (iExistCnt < 1) {
				pctlNos.add(curPctlNo);
			}
		} else {
			pctlNos.add(curPctlNo);
		}
	...
	/**
	 * TRS- JOEDI Schedule Appt 시 SCE의 APPT_DT Update 처리해줌.
	 * 
	 * @param bkg_no
	 * @param cop_no
	 * @param io_bnd_cd
	 ...
	 * @throws EventException
	 */
	public void updateDorArrDtByTrs(String bkg_no, String cop_no, String io_bnd_cd, String pctl_no, String area_conti_cd, String iDorArrDt) throws EventException {
		try {
			List<SceCopHdrVO> toBeUpdCopHdrList = new ArrayList<SceCopHdrVO>();
			List<SceCopHdrVO> sceCopHdrVOs = dbDao.searchSceCopHdr(cop_no);
			if (sceCopHdrVOs.size() == 0) {
			...
			if (io_bnd_cd.equals("O")) {
				sceCopHdrVO.setObTroFlg("Y");
				sceCopHdrVO.setCfmObDorArrDt(iDorArrDt);
			} else {
				sceCopHdrVO.setIbTroFlg("Y");
				sceCopHdrVO.setCfmApntDt(iDorArrDt);
			}

			if (pctl_no != null && !pctl_no.equals("")) {
				replanManageBC.replanCop(sceCopHdrVO, "TRS OEDI");
			}
			toBeUpdCopHdrList.add(sceCopHdrVO);
			String estm_dt = "";
			String post_estm_dt = "";
			if (io_bnd_cd.equals("O")) {
				...
				if (sceCopDtlVOList.size() > 0) {
					...
					if (estm_dt != null && !estm_dt.equals("") && estm_dt.length() == 14) {
						Calendar dtlEstm = setStrIntoCalendar(estm_dt);
						Calendar troEstm = setStrIntoCalendar(iDorArrDt);

						if (dtlEstm != null && troEstm != null) {
							if (troEstm.compareTo(dtlEstm) > 0) {
								SceCopSkdHisVO copSkdHisVO = new SceCopSkdHisVO();
								copSkdHisVO.setCopNo(sceCopDtlVO.getCopNo());
								copSkdHisVO.setFmCopDtlSeq(sceCopDtlVO.getCopDtlSeq());
								copSkdHisVO.setAftEstmDt(iDorArrDt);
								copSkdHisVO.setCreUsrId("TRO");
								copSkdHisVO.setRcvEvntProcFlg("OE");
								copSkdHisVO.setSkdRcvDt(null);
								copSkdHisVO.setSkdVoyNo(null);
								...
								dbDao.modifyOBMtEstmDt(sceCopDtlVO.getCopNo(), estm_dt, iDorArrDt);
							}
						} else {
							log.error("While TRO Rescheduling, estm_Dt size error occurred!!!");
						}
					...
					if (estm_dt != null && !estm_dt.equals("") && estm_dt.length() == 14) {
						...
						if (dtlEstm != null && troEstm != null) {
							if (troEstm.compareTo(dtlEstm) > 0) {
								...
								dbDao.modifyIBMtEstmDt(sceCopHdrVO.getCopNo(), estm_dt, iDorArrDt, post_estm_dt);
							}
						} else {
							log.error("While TRO Rescheduling, estm_Dt size error occurred!!!");
						}

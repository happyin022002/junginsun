...
public class ProductCatalogTroDBDAOTroAutoChangeCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        (" ).append("\n"); 
		query.append("            SELECT" ).append("\n"); 
		query.append("             GP1,F_NO2,L_N0,O1,NODE_NEW1,(SELECT NOD_TP_CD FROM PRD_NODE WHERE NOD_CD = NODE_NEW1) NODE1_TP," ).append("\n"); 
		...
		query.append("              NODE2_TP,NODE_NEW_KIND,OI_BND_CD," ).append("\n"); 
		query.append("            (" ).append("\n"); 
		query.append("            CASE" ).append("\n"); 
		...
		query.append("                    	DECODE(@[srf_cntr] ,'Y',NVL(RF_MIN_DWLL_HRS,3), NVL(DRY_MIN_DWLL_HRS,6))," ).append("\n"); 
		query.append("                    	DECODE(@[srf_cntr] ,'Y',NVL(RF_AVG_DWLL_HRS,3), NVL(DRY_AVG_DWLL_HRS,6)) ))))" ).append("\n"); 
		...
		query.append("                                     DECODE(@[smt_rtn] ,'',MAX(DECODE(@[sd_term],'D',Z.REP_YD_CD,  NVL( L.EQ_RTN_YD_CD,L.MTY_PKUP_YD_CD) )),@[smt_rtn] ))) MTRTN_CY" ).append("\n"); 
		query.append("                                FROM" ).append("\n"); 
		query.append("                                (" ).append("\n"); 
		...
		query.append("                                            WHERE M1.ROUT_ORG_NOD_CD LIKE DECODE(@[io_bnd_cd],'O',SUBSTR(@[inlnd_rout_org],1,5)||'%',@[inlnd_rout_org])" ).append("\n"); 
		query.append("                                            AND M1.ROUT_DEST_NOD_CD LIKE DECODE(@[io_bnd_cd],'I',SUBSTR(@[inlnd_rout_dest],1,5)||'%',@[inlnd_rout_dest])" ).append("\n"); 

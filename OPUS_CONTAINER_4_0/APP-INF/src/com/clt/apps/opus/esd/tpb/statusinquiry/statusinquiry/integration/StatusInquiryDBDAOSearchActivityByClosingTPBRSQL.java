...
*@FileName : StatusInquiryDBDAOSearchActivityByClosingTPBRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class StatusInquiryDBDAOSearchActivityByClosingTPBRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 해당점소의 조회기간내에 Close된 TPB를 조회한다.
	  * 
	  * [2010.12.08 CHM-201007598-01 진마리아 Display 화면 Column 내역 변경]
	  * </pre>
	  */
	public StatusInquiryDBDAOSearchActivityByClosingTPBRSQL(){
		setQuery();
		...
		query.append("FileName : StatusInquiryDBDAOSearchActivityByClosingTPBRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("  ( SELECT RHQ_CD FROM TPB_HNDL_OFC WHERE OFC_CD = B.OFC_CD AND DELT_FLG='N' AND N3PTY_OFC_TP_CD = 'T') RHQ," ).append("\n"); 
		...
		query.append("            SELECT O.AR_CURR_CD" ).append("\n"); 
		...
		query.append("----Office --- TPB한 office (주의-responsible office가 아님)" ).append("\n"); 
		...
		query.append("				#end" ).append("\n"); 
		query.append("					)" ).append("\n"); 
		query.append("--2.Closing Date <Default: 1달, 최장 1년>" ).append("\n"); 
		query.append("  AND ( C.OTS_STS_CRE_DT >= TPB_GET_SRCH_DATE_FNC(@[s_sdate], 'YYYY-MM-DD', @[s_user_ofc_cd])" ).append("\n"); 
		query.append("      AND C.OTS_STS_CRE_DT < TPB_GET_SRCH_DATE_FNC(@[s_edate], 'YYYY-MM-DD', @[s_user_ofc_cd])+1)" ).append("\n"); 
		query.append("  AND ( ( C.OTS_STS_CD = 'D'  AND A.N3PTY_DELT_TP_CD IN ('S','C') )" ).append("\n"); 
		query.append("      OR ( C.OTS_STS_CD IN ('E','L','A')  AND A.N3PTY_DELT_TP_CD = 'N' ) )" ).append("\n"); 

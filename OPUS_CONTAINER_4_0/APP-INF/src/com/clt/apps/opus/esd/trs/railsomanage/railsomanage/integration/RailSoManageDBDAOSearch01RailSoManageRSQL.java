...
public class RailSoManageDBDAOSearch01RailSoManageRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		SELECT X.TRSP_BND_CD AS TRSP_BND_CD" ).append("\n"); 
		...
		query.append("			  ,CASE WHEN X.IBD_CSTMS_CLR_LOC_CD != SUBSTR(X.FM_NOD_CD, 1, 5) AND X.IBD_CSTMS_CLR_LOC_CD != SUBSTR(X.TO_NOD_CD, 1, 5) THEN '0' END AS EXPT" ).append("\n"); 
		...
		query.append("			  ,X.BKG_CNTR_QTY AS PCK_QTY" ).append("\n"); 
		...
		query.append("              ,(SELECT LOC_NM FROM MDM_LOCATION  WHERE LOC_CD = SUBSTR(X.POL_NOD_CD, 1, 5) AND NVL(DELT_FLG, 'N') <> 'Y') POL_NOD_CD_NM" ).append("\n"); 
		...
		query.append("			  ,(SELECT NVL(BAY.VSL_BAY_NO || BAY.VSL_ROW_NO || BAY.VSL_TR_NO, '')" ).append("\n"); 
		...
		query.append("				   AND VVD.BKG_NO = X.BKG_NO" ).append("\n"); 
		query.append("				   AND BAY.VSL_CD = SUBSTR(X.VVD_CD, 1, 4)" ).append("\n"); 
		query.append("				   AND BAY.SKD_VOY_NO = SUBSTR(X.VVD_CD, 5, 4)" ).append("\n"); 
		query.append("				   AND BAY.SKD_DIR_CD = SUBSTR(X.VVD_CD, 9, 1)" ).append("\n"); 
		query.append("				   AND BAY.VPS_PORT_CD = SUBSTR(X.POD_NOD_CD, 1, 5)" ).append("\n"); 
		query.append("				   AND BAY.CLPT_IND_SEQ = VVD.POD_CLPT_IND_SEQ" ).append("\n"); 
		query.append("				   AND BAY.CNTR_REF_NO = X.EQ_NO" ).append("\n"); 
		query.append("				   AND BAY.LODG_DCHG_IND_CD = 'D') STWG" ).append("\n"); 
		query.append("			  ,CASE WHEN EZ_TPSZ_FT = '20' AND CNTR_WGT1 >= DECODE(X.FIRST_VNDR, 'CN', 47901, 43000) THEN 'Y'" ).append("\n"); 
		...
		query.append("			  ,CASE WHEN X.STCC_CGO_USE_FLG = 'Y' THEN 'Y' ELSE (SELECT STCC_RSTR_FLG FROM TRS_STCC_CD_RSTR WHERE STCC_CD = REGEXP_SUBSTR(X.STCC_STR, '+[^|]+', 1, 1)) END STCC_RSTR_FLG" ).append("\n"); 
		...
		query.append("					AND  (   NVL(X.SHIP_CUST_NM, 'X') = 'X'   	OR NVL(X.SHIP_ADDR, 'X') = 'X'      OR NVL(X.SHIP_CTY_NM, 'X') = 'X'  " ).append("\n"); 
		query.append("					      OR NVL(X.SHIP_STE_CD, 'X') = 'X'    	OR NVL(X.SHIP_CNT_CD, 'X') = 'X'    OR NVL(X.SHIP_CUST_ZIP_ID, 'X') = 'X'" ).append("\n"); 
		query.append("					     )" ).append("\n"); 
		...
		query.append("			  ,CASE WHEN (SUBSTR(X.FM_NOD_CD, 1, 2) = 'CA' 	OR SUBSTR(X.TO_NOD_CD,1,2) = 'CA') " ).append("\n"); 
		query.append("					AND  (   NVL(X.CON_CUST_NM, 'X') = 'X' 	OR NVL(X.CON_ADDR, 'X') = 'X' 		OR NVL(X.CON_CTY_NM, 'X') = 'X'  " ).append("\n"); 
		query.append("						  OR NVL(X.CON_STE_CD, 'X') = 'X'  	OR NVL(X.CON_CNT_CD, 'X') = 'X'  	OR NVL(X.CON_CUST_ZIP_ID, 'X') = 'X'" ).append("\n"); 
		query.append("					     ) " ).append("\n"); 
		query.append("					AND X.DCGO_FLG = 'Y' " ).append("\n"); 
		query.append("					   THEN 'N'" ).append("\n"); 
		query.append("					ELSE 'Y'     " ).append("\n"); 
		query.append("				END CON_VLD_FLG" ).append("\n"); 
		query.append("		      ,DECODE(SIGN(INSTR(X.RAIL_VNDR_SEQ, '103266')), 1, 'Y', 'N')  CND_CSTMS_CLR_FLG" ).append("\n"); 
		...
		query.append("					  ,TRIM(REGEXP_SUBSTR(VNDR_STR, '[^|]+', 1, 2)) RAIL_VNDR_SEQ" ).append("\n"); 
		...
		query.append("						 WHERE RMK.BKG_NO IN(K.BKG_NO, 'DUM000000000')" ).append("\n"); 
		query.append("						   AND NVL(RMK.EQ_NO, 'X') = NVL2(RMK.EQ_NO, K.EQ_NO, 'X')" ).append("\n"); 
		...
		query.append("							  ,TRS_GET_COM_SO_RAIL_WGT_FNC('B', null, null, null, TMP.BKG_NO, TMP.EQ_NO, TMP.EQ_TPSZ_CD, 'LBS', TMP.COP_NO, DECODE(TMP.TRSP_BND_CD, 'O', 'N', 'Y')) CNTR_WGT_STR" ).append("\n"); 
		...
		query.append("							  ,(SELECT NVL(CSTMS_IBD.CSTMS_CLR_TP_CD, ' ') || '$' || NVL(CSTMS_IBD.IBD_TRSP_NO, ' ')" ).append("\n"); 
		query.append("								  FROM BKG_CSTMS_ADV_IBD CSTMS_IBD" ).append("\n"); 
		query.append("								 WHERE CSTMS_IBD.BL_NO = TMP.BL_NO" ).append("\n"); 
		query.append("								   AND CSTMS_IBD.CNT_CD = 'US') CSTMS_IBD" ).append("\n"); 
		...
		query.append("							  ,CASE WHEN (BKG_CNTR.DCGO_FLG = 'Y' OR BKG_CNTR.RC_FLG = 'Y') THEN 'Y' ELSE 'N' END STCC_CGO_USE_FLG" ).append("\n"); 
		...
		query.append("                        #end" ).append("\n"); 
		query.append("						   AND TMP.FM_NOD_CD = YD.YD_CD(+)" ).append("\n"); 

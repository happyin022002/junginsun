...
public class ESD_SCE_0056HTMLAction extends HTMLActionSupport {
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
    	log.debug("Event 0056 생성!!");
        EsdSce0056Event event = new EsdSce0056Event();   
//        HashMap map = null;
        FormCommand f_cmd = FormCommand.fromRequest(request);
        // SEARCH01 -> search service provider's name
        if (f_cmd.isCommand(FormCommand.SEARCHLIST) ||f_cmd.isCommand(FormCommand.SEARCH03) ||f_cmd.isCommand(FormCommand.SEARCH01) ) {
        	...
        	log.debug("===vendr_seq===" + condition.getVndrSeq());
        ...
        }else if (f_cmd.isCommand(FormCommand.MULTI)) { // 저장할 때  //ESD_SCE_0057
        	event.setPgmNo(JSPUtil.getParameter(request, "pgm_no", ""));
        	event.setRptInfoCtnt(JSPUtil.getParameter(request, "save_list", ""));
        }else  if (f_cmd.isCommand(FormCommand.MODIFY)){
        	event.setSearchUSIORList((SearchUSIORListVO) getVO(request, SearchUSIORListVO.class));
        	log.debug("MULTI01을 호출을 한다."); 
        	...
        	if(length >0 ){
        		...
        		for(int i=0; i <length;i++){
        			...
        			log.debug("vvd[" + i + "]: " + vvd[i]); 
        			log.debug("bkg_no[" + i + "]: " + bkg_no[i]); 
        			log.debug("bl_no[" + i + "]: " + bl_no[i]); 
        			log.debug("cntr_no[" + i + "]: " + cntr_no[i]); 
        			log.debug("cop_no[" + i + "]: " + cop_no[i]); 
        			log.debug("cop_no[" + i + "]: " + vendor_code); 
        			log.debug("vsl_cd[" + i + "]: " + vvd[i].substring(0, 4)); 
        			log.debug("skd_voy_no[" + i + "]: " + vvd[i].substring(4, 8)); 
        			log.debug("skd_dir_cd[" + i + "]: " + vvd[i].substring(8, 9)); 
        ...
        }else if (f_cmd.isCommand(FormCommand.SEARCH05)) {  //ESD_SCE_0057
        	event.setPgmNo(JSPUtil.getParameter(request, "pgm_no", ""));
        }

...
public class StatusReportDBDAOSearchOutBdMovementByTPSZSumRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT CNTR_TPSZ_CD_MV " ).append("\n"); 
		query.append("       ,SUM(DECODE(BKG_RANK,1,NVL(OP_CNTR_QTY,0),0)) BKG_QTY" ).append("\n"); 
		...
		query.append("       --,COUNT(CNMV_STS_CD) TTL" ).append("\n"); 
		query.append("       --,SUM(DECODE(BKG_RANK,1,OP_CNTR_QTY,0)) - COUNT(CNMV_STS_CD) DIFF_QTY" ).append("\n"); 
		query.append("       ,SUM(DECODE(BKG_RANK,1,NVL(OP_CNTR_QTY,0),0)) - SUM(NVL(CNTR_VOL_QTY,0)) DIFF_QTY" ).append("\n"); 
		query.append("       " ).append("\n"); 
		...
		query.append("					                (SELECT MST.CNMV_STS_CD FROM MST_CONTAINER MST WHERE MST.CNTR_NO = B.CNTR_NO)))) IN ('OP', 'OC', 'EN', 'TN', 'CY','VL','MT') THEN" ).append("\n"); 
		...
		query.append("GROUP  BY  CNTR_TPSZ_CD_MV" ).append("\n"); 
		query.append("ORDER BY CNTR_TPSZ_CD_MV" ).append("\n"); 

...
public class ACMSimulationDBDAOManageAGNCommMassSimListCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("(BAT_RCV_DT, BAT_RCV_SEQ, BKG_NO, COMM_TP_CD, BAT_FLG, BAT_ITM_NM, BAT_END_DT, SLAN_CD, RLANE_CD, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, REV_DIR_CD, ORG_LOC_CD, DEST_LOC_CD, BDR_FLG, CALL_FM_SRC_ID, BAT_DESC, CRE_USR_ID, CRE_DT, UPD_USR_ID, UPD_DT)" ).append("\n"); 
		...
		query.append(" 'S' || (SELECT TO_CHAR(SYSDATE, 'YYMMDD') || TO_CHAR( NVL( MAX(SUBSTR(BAT_ITM_NM, 8,4)) + 1, '1') , 'FM0000')" ).append("\n"); 
		query.append("                                            FROM ACM_CALC_BAT WHERE COMM_TP_CD = 'S' AND SUBSTR(BAT_ITM_NM, 2,6) = TO_CHAR(SYSDATE, 'YYMMDD'))," ).append("\n"); 

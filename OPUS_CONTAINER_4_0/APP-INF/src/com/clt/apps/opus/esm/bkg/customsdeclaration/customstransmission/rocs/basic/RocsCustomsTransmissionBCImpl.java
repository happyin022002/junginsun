...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.rocs.integration.RocsCustomsTransmissionDBDAO;
 ...
 * @since J2EE 1.4
 */
public class RocsCustomsTransmissionBCImpl extends BasicCommandSupport implements RocsCustomsTransmissionBC {
	...
	public ManifestTransmitDetailVO transmitManifestList(ManifestTransmitVO[] manifestTransmitVOs, SignOnUserAccount account) throws EventException {
		...
		List<RocsSearchAwkCgoInfoVO> rocsSearchAwkCgoInfoVOs = null;
...
//						flatFile.append("$$$MSGSTART:").append(ConstantMgr.getCompanyCode()).append("              NLRTMPCS            IFTMCS    ").append(msgDt);
						flatFile.append(command.searchCstmsEdiHeaderNew("NL_IFTMCS"));
						if (rocsSearchBlInfoVOs != null && rocsSearchBlInfoVOs.size() > 0) {
							...
							for (int i = 0; i < rocsSearchBlInfoVOs.size(); i++) {
								...
								flatFile.append("\n");
								if(rocsSearchBlInfoVO.getBlNo().length()==13){
									flatFile.append(
											rocsSearchBlInfoVO.getVvd().toString() + "-"
													+ rocsSearchBlInfoVO.getBlNo().substring(0, 12)).append("\n"); // Toyota B/L
								}else if(rocsSearchBlInfoVO.getBlNo().length()==11){
									flatFile.append(
											rocsSearchBlInfoVO.getVvd().toString() + "-"
													+ rocsSearchBlInfoVO.getBlNo().substring(0, 10)).append("\n"); // Toyota B/L
								}else{
									flatFile.append(
											rocsSearchBlInfoVO.getVvd().toString() + "-"
													+ rocsSearchBlInfoVO.getBlNo()).append("\n"); // Toyota B/L
								}
								flatFile.append("VVD:").append(rocsSearchBlInfoVO.getVvd()).append("\n");
								flatFile.append("CON_VVD:").append(comBc.searchConVvd(rocsSearchBlInfoVO.getVvd(), rocsSearchBlInfoVO.getPod(), "I")).append("\n");
								//flatFile.append("STATUS:").append(rocsSearchBlInfoVO.getFlag()).append("\n"); 기존 로직은 삭제하지 않고 주석처리
								flatFile.append("STATUS:").append("9").append("\n");
								...
								if(rocsSearchBlInfoVO.getBlNo().length()==13){
									...
									flatFile.append("BL_NO:").append(rocsSearchBlInfoVO.getBlNo().substring(0, 10)).append("\n"); // Toyota B/L
								}else{
									flatFile.append("BL_NO:").append(rocsSearchBlInfoVO.getBlNo()).append("\n"); // Toyota B/L
								}
								flatFile.append("MRN:").append(rocsSearchBlInfoVO.getMrnNo()).append("\n");								
								flatFile.append("TO_ORDER_IND:").append(rocsSearchBlInfoVO.getCustToOrdFlg()).append("\n");
								...
								flatFile.append("VSL_FULLNAME:").append(rocsSearchBlInfoVO.getVslFullname()).append("\n");
								flatFile.append("VSL_CNT_CD:").append(rocsSearchBlInfoVO.getVslCntCd()).append("\n");
								flatFile.append("POD:").append(rocsSearchBlInfoVO.getPod()).append("\n");
								flatFile.append("POD_FULLNAME:").append(rocsSearchBlInfoVO.getPodFullname()).append("\n");
								flatFile.append("BERTH_CD:").append(rocsSearchBlInfoVO.getBerthCd()).append("\n");
								flatFile.append("BERTH_DESC:").append(rocsSearchBlInfoVO.getBerthDesc()).append("\n");
								...
								flatFile.append("POL_FULLNAME:").append(rocsSearchBlInfoVO.getPolFullname()).append("\n");
								...
								flatFile.append("PRE_FULLNAME:").append(rocsSearchBlInfoVO.getPreFullname()).append("\n");
								flatFile.append("POST:").append(rocsSearchBlInfoVO.getPost()).append("\n");
								flatFile.append("POST_FULLNAME:").append(rocsSearchBlInfoVO.getPostFullname()).append("\n");
								...
								flatFile.append("SENDER:").append(rocsSearchBlInfoVO.getSender()).append("\n");
						...
						if (rocsSearchCmInfoVOs != null) {
							for (int i = 0; i < rocsSearchCmInfoVOs.size(); i++) {
								...
								flatFile.append("CM_PKG_DESC:").append(rocsSearchCmInfoVO.getCmPkgDesc()).append("\n");
								...
								flatFile.append("MARKNO:").append(rocsSearchCmInfoVO.getMarkno()).append("\n");
								flatFile.append("}CM_MARK").append("\n");
								flatFile.append("}CM_INFO").append("\n");
						...
						if (rocsSearchDgCntrInfoVOs != null) {
							for (int i = 0; i < rocsSearchDgCntrInfoVOs.size(); i++) {
								...
								flatFile.append("DG_PKG_DESC:").append(rocsSearchDgCntrInfoVO.getDgPkgDesc()).append(
										"\n");
								flatFile.append("PKGGRP:").append(rocsSearchDgCntrInfoVO.getPkggrp()).append("\n");
								flatFile.append("}DANGER_CGO").append("\n");
							}
						}
						...
						if (rocsSearchCntrInfoVOs != null) {
							for (int i = 0; i < rocsSearchCntrInfoVOs.size(); i++) {
								...
								flatFile.append("CNTR_SC:").append("2").append("\n");
								flatFile.append("CNTR_FM:").append(rocsSearchCntrInfoVO.getCntrFm()).append("\n");
								flatFile.append("CNTRTRW:").append(rocsSearchCntrInfoVO.getCntrtrw()).append("\n");
								flatFile.append("SEAL_NO:").append(rocsSearchCntrInfoVO.getSealNo()).append("\n");
								
								rocsSearchAwkCgoInfoVOs = dbDao.searchAwkCgoInfo(rocsSearchCntrInfoVO);
								
								if (rocsSearchAwkCgoInfoVOs != null) {
									for (int k = 0; k < rocsSearchAwkCgoInfoVOs.size(); k++) {
										RocsSearchAwkCgoInfoVO rocsSearchAwkCgoInfoVO = rocsSearchAwkCgoInfoVOs.get(k);
										
										flatFile.append("{AWKWARD_CGO").append("\n");
										flatFile.append("AK_LGT:").append(rocsSearchAwkCgoInfoVO.getAkLgt()).append("\n");
										flatFile.append("AK_HGT:").append(rocsSearchAwkCgoInfoVO.getAkHgt()).append("\n");
										flatFile.append("AK_WDT:").append(rocsSearchAwkCgoInfoVO.getAkWdt()).append("\n");
										flatFile.append("}AWKWARD_CGO").append("\n");
									}
								}	
						...
						sendFlatFileVO.setQueueNm(SubSystemConfigFactory.get("BKG.OPUSBKG_UBIZCOM_ROCS.IBMMQ.QUEUE"));
	...
	public RocsManifestTransmitVO receiveOpusbkgUdevcom(String flatFile, SignOnUserAccount account) throws EventException {
		...
		try {
			...
			for (int j = 0; j < tmpArray.size(); j++) {
				...
				if (data1.length() > 11 && data1.substring(0, 12).equalsIgnoreCase("RTMCUSAPERAK")) { // Toyota B/L data1이 뭔지 확인해야 한다.
					rocsManifestTransmitVO.setMsgDt(data1.substring(12).trim()); // Toyota B/L data1이 뭔지 확인해야 한다.
				}

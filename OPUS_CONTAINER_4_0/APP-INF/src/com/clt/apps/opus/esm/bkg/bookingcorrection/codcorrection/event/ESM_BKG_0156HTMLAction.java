...
//			event.setRgnCd			(JSPUtil.getParameter(request, "rgn_cd"));
			...
			event.setCodVO(codVO);			
		}else if(command.isCommand(FormCommand.COMMAND14) || command.isCommand(FormCommand.COMMAND16)){
			event.setBkgCODMgtConditionVO((BkgCODMgtConditionVO)getVO(request, BkgCODMgtConditionVO.class));
		}else if(command.isCommand(FormCommand.COMMAND15)){
			event.setCodMailSendVO((CodMailSendVO)getVO(request, CodMailSendVO.class));
		}
		return  event;
	}

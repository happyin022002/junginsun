...
public class BookingHistoryMgtDBDAOSearchHistBkgReferenceRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("          FROM OLD_REF LEFT OUTER JOIN " ).append("\n"); 
		query.append("#if (${ca_flg} == 'Y')" ).append("\n"); 
		query.append("              BKG_REF_HIS NOW_REF" ).append("\n"); 
		query.append("           ON NOW_REF.BKG_NO  = OLD_REF.BKG_NO " ).append("\n"); 
		...
		query.append("		   AND NOW_REF.BKG_REF_TP_CD NOT IN ('CMRN','CUCR')" ).append("\n"); 
		...
		query.append("		UNION" ).append("\n"); 
		query.append("        SELECT (SELECT INTG_CD_VAL_DP_DESC FROM COM_INTG_CD_DTL WHERE INTG_CD_ID = 'CD01658' AND NOW_REF.BKG_REF_TP_CD = INTG_CD_VAL_CTNT) HIS_CATE_NM" ).append("\n"); 
		query.append("                , OLD_REF.REF_NO||'/'||OLD_REF.CNTR_NO||'/'||SUBSTR(OLD_REF.BKG_REF_TP_CD,2,3)||'/'||OLD_REF.CUST_REF_NO_CTNT PRE_CTNT " ).append("\n"); 
		query.append("				, NOW_REF.REF_NO||'/'||NOW_REF.CNTR_NO||'/'||SUBSTR(NOW_REF.BKG_REF_TP_CD,2,3)||'/'||NOW_REF.CUST_REF_NO_CTNT CRNT_CTNT " ).append("\n"); 
		query.append("          FROM OLD_REF LEFT OUTER JOIN " ).append("\n"); 
		query.append("#if (${ca_flg} == 'Y')" ).append("\n"); 
		query.append("               BKG_REF_HIS NOW_REF" ).append("\n"); 
		query.append("           ON  NOW_REF.BKG_NO  = OLD_REF.BKG_NO " ).append("\n"); 
		...
		query.append("	       AND NOW_REF.CORR_NO = 'TMP0000001' " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("		   AND NOW_REF.BKG_REF_TP_CD IN ('CMRN','CUCR')" ).append("\n"); 
		query.append("    )    " ).append("\n"); 

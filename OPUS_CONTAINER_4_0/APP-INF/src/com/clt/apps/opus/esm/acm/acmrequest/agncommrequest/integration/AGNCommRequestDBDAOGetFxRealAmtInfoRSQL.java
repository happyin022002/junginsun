...
public class AGNCommRequestDBDAOGetFxRealAmtInfoRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * COMM_FX_AMT 를 구한다.
	  * 
	  * 2014.11.12 박다은 Running Reefer Container 인지 아닌 지 구분하는 로직 추가 및 Contract Office 계산 추가
	  * 2014.12.15 박다은 Merchant Haulage, Release Office 계산 로직 추가
	  * 2014.12.26 박다은 Merchant Haulage, Release Office 계산과 CHF 추가 계산 가능하도록 로직 보완
	  * 2016.06.03 박다은 EQ Sub qty 처리 보완
	  * 2016.07.28 박다은 Reefer container check 시 BKG_QTY_DTL.RC_FLG 체크
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("            ,( SELECT CASE WHEN COUNT(1) > 0 THEN COUNT(DISTINCT(R.AGN_AGMT_ROUT_SEQ)) * 2 ELSE 0 END " ).append("\n"); 
		query.append("               FROM ACM_AGN_AGMT_DTL_ROUT R" ).append("\n"); 
		query.append("              WHERE 1=1" ).append("\n"); 
		query.append("                AND D.AGN_CD      = R.AGN_CD" ).append("\n"); 
		query.append("                AND D.AGN_AGMT_NO = R.AGN_AGMT_NO" ).append("\n"); 

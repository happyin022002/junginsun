...
*@FileName : ESM_COA_4001HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
import com.clt.apps.opus.esm.coa.stdunitcost.sltint.event.EsmCoa4001Event;
 ...
 * @author Kim Ki Dae
 * @see EsmCoa4001Event 참조
 * @since J2EE 1.6
 */
public class ESM_COA_4001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * TESTHTMLAction 객체를 생성
	 */
	public ESM_COA_4001HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmCoa4001Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmCoa4001Event event = new EsmCoa4001Event();  	
    	CommonCoaRsVO commonCoaRsVO = new CommonCoaRsVO();    	
    	
		if(command.isCommand(FormCommand.SEARCH01)) {// Retrieve
			...
			event.setSlotInternalPricingVOS((SlotInternalPricingVO[])getVOs(request, SlotInternalPricingVO .class,""));
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO .class));
		}	
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			//event.setSlotIPCommonVO((SlotIPCommonVO)getVO(request, SlotIPCommonVO.class));
		}	

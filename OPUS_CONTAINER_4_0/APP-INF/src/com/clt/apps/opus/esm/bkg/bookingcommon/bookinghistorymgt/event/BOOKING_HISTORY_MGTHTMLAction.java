...
/**
 * HTTP Parser<br>
 * - bookingcommon 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class BOOKING_HISTORY_MGTHTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BOOKING_HISTORY_MGTHTMLAction 객체를 생성
	 */
	public BOOKING_HISTORY_MGTHTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BookingHistoryMgtEvent event = new BookingHistoryMgtEvent();
        log.debug("CALL:BOOKING_HISTORY_MGTHTMLAction - " + command.getCommand());
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setMndVOS((MndVO[])getVOs(request, MndVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setMndVO((MndVO)getVO(request, MndVO .class));
		}

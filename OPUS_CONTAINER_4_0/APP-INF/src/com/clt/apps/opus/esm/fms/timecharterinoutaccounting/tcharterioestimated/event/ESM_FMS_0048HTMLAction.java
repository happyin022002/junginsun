...
*@FileName : ESM_FMS_0048HTMLAction.java
...
public class ESM_FMS_0048HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0048HTMLAction 객체를 생성
	 */
	public ESM_FMS_0048HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0048Event event = new EsmFms0048Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomEstmIfVOS((CustomEstmIfVO[])getVOs(request, CustomEstmIfVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionEstmIfVO((ConditionEstmIfVO)getVO(request, ConditionEstmIfVO .class,""));
			//event.setEstRevMm(request.getParameter("fr_duration"));
		}

...
public class BLDocumentationBLDBDAOMndRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",      NVL(B.TTL_PCK_DESC, (SELECT BKG_JOIN_FNC(CURSOR(	SELECT A1.OP_CNTR_QTY || 'X' || SUBSTR(A2.CNTR_TPSZ_DESC, 1, 4)" ).append("\n"); 
		...
		query.append("							FROM DUAL)) TTL_PCK_DESC" ).append("\n"); 
		...
		query.append(",	   'N' AS IMG_FLG  --BKG_IMG_STO 테이블" ).append("\n"); 
		...
		query.append(",      (SELECT    BKG_JOIN_FNC(CURSOR(SELECT CASE WHEN 1<=A1.OP_CNTR_QTY THEN ''||A1.OP_CNTR_QTY ELSE '0'||A1.OP_CNTR_QTY END" ).append("\n"); 
		...
		query.append(",      (SELECT 1 FROM BKG_XPT_IMP_LIC_HIS WHERE BKG_NO=A.BKG_NO AND CORR_NO='TMP0000001' AND rownum=1) XPT_IMP_SEQ" ).append("\n"); 
		...
		query.append(",      (SELECT CASE WHEN NOT_EQUAL_DATA > 0 THEN 'Y'" ).append("\n"); 
		query.append("                    WHEN EQUAL_DATA+NOT_EQUAL_DATA > 0 THEN 'T'" ).append("\n"); 
		query.append("               ELSE 'N'" ).append("\n"); 
		query.append("               END" ).append("\n"); 
		query.append("        FROM" ).append("\n"); 
		query.append("            (" ).append("\n"); 
		query.append("             SELECT SUM(CASE WHEN T1.CNTR_NO IS NOT NULL AND T1.CNTR_NO = T2.CNTR_NO THEN 1 ELSE 0 END) AS EQUAL_DATA" ).append("\n"); 
		query.append("                    ,SUM(CASE WHEN T1.CNTR_NO IS NULL OR T1.CNTR_NO <> T2.CNTR_NO THEN 1 ELSE 0 END) AS NOT_EQUAL_DATA" ).append("\n"); 
		query.append("             FROM BKG_REF_HIS T1, BKG_CNTR_HIS T2 " ).append("\n"); 
		...
		query.append("             AND T1.CORR_NO = T2.CORR_NO(+)" ).append("\n"); 
		query.append("         	 AND T1.CNTR_NO = T2.CNTR_NO(+)" ).append("\n"); 
		query.append("      	     AND T1.BKG_REF_TP_CD IN ('CMRN','CUCR')" ).append("\n"); 
		query.append("    	    )" ).append("\n"); 
		query.append("       ) AS PO_MRN_UCR_CNTR_FLG" ).append("\n"); 
		query.append(",      B.MEAS_QTY_PRN_FLG" ).append("\n"); 
		query.append(",	   B.CNTR_WGT_PRN_FLG" ).append("\n"); 
		query.append(",	   B.CNTR_MEAS_PRN_FLG" ).append("\n"); 
		query.append(",	   B.MK_DESC_PRN_FLG" ).append("\n"); 
		query.append(",	   B.MF_DESC_PRN_FLG" ).append("\n"); 
		query.append(",      B.FRT_PAY_OFC_PRN_FLG" ).append("\n"); 
		...
		query.append(",      (SELECT 1 FROM BKG_XPT_IMP_LIC WHERE BKG_NO=A.BKG_NO AND rownum=1) XPT_IMP_SEQ" ).append("\n"); 
		query.append(",      (SELECT count(MDT_ITM_SEQ)" ).append("\n"); 
		query.append("FROM (SELECT A1.BKG_NO" ).append("\n"); 
		query.append("           ,DECODE(A2.BKG_CUST_TP_CD,'S','SH' ,'C','CN' ,'N','NT','') AS MDT_CUST_TP_CD" ).append("\n"); 
		query.append("           ,NVL(A2.CUST_CNT_CD,'0')       AS CUST_CNT_CD" ).append("\n"); 
		...
		query.append("             SELECT SUM(CASE WHEN T1.CNTR_NO IS NOT NULL AND T1.CNTR_NO = T2.CNTR_NO THEN 1 ELSE 0 END) AS EQUAL_DATA" ).append("\n"); 
		query.append("                    ,SUM(CASE WHEN T1.CNTR_NO IS NULL OR T1.CNTR_NO <> T2.CNTR_NO THEN 1 ELSE 0 END) AS NOT_EQUAL_DATA" ).append("\n"); 
		query.append("             FROM BKG_REFERENCE T1, BKG_CONTAINER T2 " ).append("\n"); 
		query.append("             WHERE T1.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("             AND T1.BKG_NO = T2.BKG_NO(+)" ).append("\n"); 
		query.append("         	 AND T1.CNTR_NO = T2.CNTR_NO(+)" ).append("\n"); 
		query.append("      	     AND T1.BKG_REF_TP_CD IN ('CMRN','CUCR')" ).append("\n"); 

...
*@FileName : ESM_BKG_1522HTMLAction.java
*@FileTitle : ESM_BKG_1522HTMLAction
*Open Issues :
*Change history :
*@LastModifyDate :
*@LastModifier :
...
/**
 * ESM_BKG_1522 의 HTML Action Class
 *
 * @author
 * @see HTMLActionSupport
 * @since J2EE 1.6
 */
public class ESM_BKG_1522HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	public ESM_BKG_1522HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1522Event event = new EsmBkg1522Event();
		FormCommand command = FormCommand.fromRequest(request);

		if (command.isCommand(FormCommand.INIT) || command.isCommand(FormCommand.SEARCH)) {
			event.setMalaysiaImpStsVO((MalaysiaImpStsVO)getVO(request, MalaysiaImpStsVO.class));

		} else if(command.isCommand(FormCommand.MULTI)) {    // Start Back End Job (SAVE)
			event.setMalaysiaImpStsVOs((MalaysiaImpStsVO[])getVOs(request, MalaysiaImpStsVO.class, "sheet1_"));

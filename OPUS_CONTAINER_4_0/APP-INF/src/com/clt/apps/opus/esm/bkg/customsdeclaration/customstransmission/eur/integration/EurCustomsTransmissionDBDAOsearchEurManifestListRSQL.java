...
public class EurCustomsTransmissionDBDAOsearchEurManifestListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       ,NVL2(SENT_TIME,'Sent'||CHR(10)||'('||SENT_TIME||')','Initial') AS TRAN_STATUS" ).append("\n"); 
		query.append("       ,NVL2(SENT_TIME,DECODE(SUBSTR(RECEIVE_TIME,1,1), null ,'Not Received','A','Accepted'||CHR(10)||'('||SUBSTR(RECEIVE_TIME,2,20)||')','R','Rejected'||CHR(10)||'('||SUBSTR(RECEIVE_TIME,2,20)||')'),DECODE(SUBSTR(RECEIVE_TIME,1,1), null ,'','A','Accepted'||CHR(10)||'('||SUBSTR(RECEIVE_TIME,2,20)||')','R','Rejected'||CHR(10)||'('||SUBSTR(RECEIVE_TIME,2,20)||')')) AS RECEIVE_STATUS" ).append("\n"); 
		...
		query.append("	       ,(SELECT DECODE(ORG_TRNS_MOD_CD,'B','Y','F','Y','U','Y','E','Y','N')" ).append("\n"); 
		query.append("             FROM BKG_BOOKING" ).append("\n"); 
		query.append("             WHERE 1=1" ).append("\n"); 
		query.append("             AND BKG_NO = T2.BKG_NO) TRNS_MOD_CD" ).append("\n"); 
		query.append("		  #else" ).append("\n"); 
		query.append("	       ,(SELECT DECODE(DEST_TRNS_MOD_CD,'B','Y','F','Y','U','Y','E','Y','N')" ).append("\n"); 
		query.append("             FROM BKG_BOOKING" ).append("\n"); 
		query.append("             WHERE 1=1" ).append("\n"); 
		query.append("             AND BKG_NO = T2.BKG_NO) TRNS_MOD_CD" ).append("\n"); 
		...
		query.append("               ,(SELECT ACK_RCV_STS_CD||TO_CHAR(ACK_DT,'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("                 FROM BKG_CSTMS_EUR_DG_RCV" ).append("\n"); 
		query.append("                 WHERE BL_NO = T1.BL_NO " ).append("\n"); 
		query.append("                 AND SUBSTR(MSG_RCV_NO,-14)>T1.SND" ).append("\n"); 
		query.append("                 AND EUR_EDI_MSG_TP_ID = 'CTA'" ).append("\n"); 
		query.append("                 AND MSG_RCV_NO = T1.MAX_MSG_RCV_NO" ).append("\n"); 
		query.append("                 AND  ROWNUM=1) AS RECEIVE_TIME" ).append("\n"); 
		query.append("        FROM (" ).append("\n"); 
		query.append("            SELECT " ).append("\n"); 
		query.append("                 BB.BL_NO" ).append("\n"); 
		query.append("                 ,DECODE(BB.BKG_CGO_TP_CD, 'P', 'EMPTY', 'FULL') AS CARGO_TYPE" ).append("\n"); 
		...
		query.append("--                                       WHERE C.BKG_NO  = BB.BKG_NO" ).append("\n"); 
		query.append("--                                         AND C.BKG_NO  = BR.BKG_NO(+)" ).append("\n"); 
		query.append("--                                         AND C.CNTR_NO = BR.CNTR_NO(+)" ).append("\n"); 
		query.append("--                                         AND BR.BKG_REF_TP_CD(+) = 'CMRN'" ).append("\n"); 
		query.append("--                                       ORDER BY C.CNTR_NO) ) , CHR(10)) AS EXPORT_MRN" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("               , BKG_JOIN_FNC((CURSOR(SELECT CUST_REF_NO_CTNT" ).append("\n"); 
		query.append("                                        FROM BKG_REFERENCE BR" ).append("\n"); 
		query.append("                                       WHERE 1=1" ).append("\n"); 
		query.append("                                         AND BB.BKG_NO  = BR.BKG_NO(+)" ).append("\n"); 
		query.append("                                         AND BR.BKG_REF_TP_CD(+) = 'CMRN'" ).append("\n"); 
		query.append("                                       ORDER BY BR.CNTR_NO) ) , CHR(10)) AS EXPORT_MRN" ).append("\n"); 
		query.append("                " ).append("\n"); 
		...
		query.append("                , (SELECT MAX(MSG_RCV_NO)" ).append("\n"); 
		query.append("                   FROM BKG_CSTMS_EUR_DG_RCV" ).append("\n"); 
		query.append("                   WHERE BL_NO = BB.BL_NO) AS MAX_MSG_RCV_NO" ).append("\n"); 
		...
		query.append("#else" ).append("\n"); 
		query.append("                , POD.SLAN_CD" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("              ,(SELECT TO_CHAR(SND_DT,'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("                FROM(SELECT *" ).append("\n"); 
		query.append("                     FROM BKG_CSTMS_EUR_DG_EDI_RSPN RS, BKG_CSTMS_EUR_DG_SND SND" ).append("\n"); 
		query.append("                     WHERE RS.EUR_EDI_MSG_TP_ID='CTA' " ).append("\n"); 
		...
		query.append("                     #end" ).append("\n"); 
		query.append("                     AND RS.MSG_SND_NO = SND.MSG_SND_NO" ).append("\n"); 
		query.append("                     ORDER BY SND.MSG_SND_NO DESC)  " ).append("\n"); 
		query.append("                 WHERE BL_NO=BB.BL_NO" ).append("\n"); 
		query.append("                 AND ROWNUM=1 ) AS SENT_TIME" ).append("\n"); 
		query.append("               ,(SELECT MAX(SUBSTR(MSG_SND_NO,-14))" ).append("\n"); 
		query.append("                 FROM BKG_CSTMS_EUR_DG_EDI_RSPN" ).append("\n"); 
		query.append("                 WHERE BL_NO =BB.BL_NO) AS SND" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("            AND POL.VSL_CD        =  B.VSL_CD" ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("            AND POL.VSL_CD        =  BV.VSL_CD" ).append("\n"); 
		query.append("            AND POL.SKD_VOY_NO    =  BV.SKD_VOY_NO" ).append("\n"); 
		query.append("            AND POL.SKD_DIR_CD    =  BV.SKD_DIR_CD " ).append("\n"); 
		query.append("            AND NVL(POL.SKD_CNG_STS_CD, ' ')    <> 'S'" ).append("\n"); 
		query.append("            AND POL.VPS_PORT_CD   = BV.POL_CD" ).append("\n"); 
		query.append("            AND POL.CLPT_IND_SEQ  = BV.POL_CLPT_IND_SEQ" ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("            " ).append("\n"); 
		query.append("            AND POD.VSL_CD        =  BV.VSL_CD" ).append("\n"); 
		query.append("            AND POD.SKD_VOY_NO    =  BV.SKD_VOY_NO" ).append("\n"); 
		query.append("            AND POD.SKD_DIR_CD    =  BV.SKD_DIR_CD " ).append("\n"); 
		query.append("            AND NVL(POD.SKD_CNG_STS_CD, ' ')    <> 'S'" ).append("\n"); 
		query.append("            AND POD.VPS_PORT_CD   = BV.POD_CD" ).append("\n"); 
		query.append("            AND POD.CLPT_IND_SEQ  = BV.POD_CLPT_IND_SEQ" ).append("\n"); 
		query.append("    " ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                               AND CLPT_SEQ BETWEEN POL.CLPT_SEQ AND POD.CLPT_SEQ" ).append("\n"); 
		query.append("                           )" ).append("\n"); 
		query.append("            #end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	#if (${cargo_type} == 'M')" ).append("\n"); 
		query.append("            AND BB.BKG_CGO_TP_CD =  'P'" ).append("\n"); 
		query.append("	#elseif (${cargo_type} == 'F')" ).append("\n"); 
		query.append("            AND BB.BKG_CGO_TP_CD <> 'P'" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("WHERE 1=1" ).append("\n"); 
		query.append(" #if (${trns_mode} != '')" ).append("\n"); 
		query.append("  AND T3.TRNS_MOD_CD = @[trns_mode]" ).append("\n"); 
		query.append(" #end" ).append("\n"); 

...
*@FileName       : EsmSaq0127ViewAdapter10.java
...
public class EsmSaq0127ViewAdapter10 extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		SearchMonthlyQuotaInquiry0127Tab10VO listVo = null; // 차후 루프문을 통해 결과리스트의 정보를 한 로우씩 입력할 결과VO를 세팅 (0126공유)
...
//		sbufXML.append("<DATA COLORDER='" + JSPUtil.convertStringArrayToString(changedColNms, "|") + "' COLSEPARATOR='" + DELIMITER + "' TOTAL='" + totCnt +"'>\n");
		...
		for(int j=0;j<realCnt;j++){ 
			listVo = (SearchMonthlyQuotaInquiry0127Tab10VO) list.get(j); //list 에서 한 줄 씩 빼내서 결과vo에 입력
			...
			if( "1".equals(listVo.getGrpAqCd())){
				...
				dir_cd = " ";
//				isExpand = "true";
			}else if( "1".equals(listVo.getGrpSaqTgtGrpCd())){
				colorIdx = 1;
				tree_lvl = 1;
...
//				item_nm = item_nm + "*";
//			}
			if(aq_cd == null || aq_cd.equals("000000")){
				aq_cd = " ";
			}

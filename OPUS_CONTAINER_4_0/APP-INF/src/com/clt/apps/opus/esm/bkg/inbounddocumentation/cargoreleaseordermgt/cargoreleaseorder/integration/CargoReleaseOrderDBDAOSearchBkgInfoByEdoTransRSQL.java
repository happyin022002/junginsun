...
public class CargoReleaseOrderDBDAOSearchBkgInfoByEdoTransRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , SUBSTR(REPLACE(NVL(BC1.CUST_NM,' ')  ,CHR(10),' '),1,  35) AS CNEE_NM" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC1.CUST_ADDR,' '),CHR(10),' '),1,  35) AS CNEE_ADDR1" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC1.CUST_ADDR,' '),CHR(10),' '),36, 35) AS CNEE_ADDR2" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC1.CUST_ADDR,' '),CHR(10),' '),70, 35) AS CNEE_ADDR3" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC2.CUST_NM,' ')  ,CHR(10),' '),1,  35) AS NTFY_NM" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC2.CUST_ADDR,' '),CHR(10),' '),1,  35) AS NTFY_ADDR1" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC2.CUST_ADDR,' '),CHR(10),' '),36, 35) AS NTFY_ADDR2" ).append("\n"); 
		query.append("     , SUBSTR(REPLACE(NVL(BC2.CUST_ADDR,' '),CHR(10),' '),70, 35) AS NTFY_ADDR3" ).append("\n"); 
		query.append("     , SUBSTR(NVL(REPLACE(DECODE(BK.CUST_TO_ORD_FLG, 'Y', BC2.CUST_NM, BC1.CUST_NM), CHR(10),' '), ' ') , 1, 35) AS ASPH_NM           -- 실화주명" ).append("\n"); 

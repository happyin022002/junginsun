...
*@FileName : ESM_BKG_0444HTMLAction.java
...
public class ESM_BKG_0444HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0440HTMLAction 객체를 생성 
	 */
	public ESM_BKG_0444HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0444Event event = new EsmBkg0444Event();
		 
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setRocsVesselArrivalCondVO((RocsVesselArrivalCondVO)getVO(request, RocsVesselArrivalCondVO.class));
		}  else if(command.isCommand(FormCommand.SEARCH01)) { 
			event.setRocsVesselArrivalCondVO((RocsVesselArrivalCondVO)getVO(request, RocsVesselArrivalCondVO.class));
		}  else if(command.isCommand(FormCommand.MULTI)) { 
			event.setBlVO((RocsBlVO)getVO(request, RocsBlVO.class));
		}  

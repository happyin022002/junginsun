 ...
 *@FileName : ESM_BKG_0028HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0028HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = 1L;

	/**
	 * ESM_BKG_0028HTMLAction 객체를 생성
	 */
	public ESM_BKG_0028HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0028Event event = new EsmBkg0028Event();
		...
		if (command.isCommand(FormCommand.SEARCH)) {    // Retrieve
			...
			if (CountryCode.CA.equals(event.getCntCd())) {
				...
				UsaCstmsManifestAmendmentCondVO usaCondVO = (UsaCstmsManifestAmendmentCondVO) getVO(request, UsaCstmsManifestAmendmentCondVO.class);
				usaCondVO.setStartNo(iStartNo + "");
				usaCondVO.setEndNo(iEndNo + "");
				event.setCstmsManifestAmendmentCondVO(usaCondVO);
			}
		} else if (command.isCommand(FormCommand.COMMAND01) ||    // Transmit - Start AI
				   command.isCommand(FormCommand.COMMAND02)) {    // Transmit -Immediate Delete & AI
			if (CountryCode.CA.equals(event.getCntCd())) {
				event.setCstmsManifestAmendmentVO((CndCstmsManifestAmendmentVO) getVO(request, CndCstmsManifestAmendmentVO.class));
			} else if (CountryCode.US.equals(event.getCntCd())) {
				event.setCstmsManifestAmendmentVO((UsaCstmsManifestAmendmentVO) getVO(request, UsaCstmsManifestAmendmentVO.class));
			}

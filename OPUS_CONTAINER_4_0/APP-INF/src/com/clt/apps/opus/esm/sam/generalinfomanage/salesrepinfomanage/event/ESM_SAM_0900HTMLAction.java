...
*@FileName : ESM_SAM_0900HTMLAction.java
...
public class ESM_SAM_0900HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SAM_0900HTMLAction 객체를 생성
	 */
	public ESM_SAM_0900HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSam0900Event event = new EsmSam0900Event();
			if(command.isCommand(FormCommand.SEARCH)) {
			SearchCustomerVO vo = (SearchCustomerVO)getVO(request, SearchCustomerVO .class);
			event.setSearchCustomerVO(vo);
			}else if (command.isCommand(FormCommand.MULTI)){
			// Primary 등록/수정
			event.setCustomsCustomerVO((CustomsCustomerVO)getVO(request, CustomsCustomerVO .class));

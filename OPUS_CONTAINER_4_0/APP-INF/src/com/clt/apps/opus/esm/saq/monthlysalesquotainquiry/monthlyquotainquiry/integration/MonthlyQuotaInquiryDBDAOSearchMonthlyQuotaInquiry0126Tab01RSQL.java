...
*@FileName : MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0126Tab01RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0126Tab01RSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 분기별별 확정된 내용을 Trade, Bound,Sub Trade 별로 Grouping 된 형태로 조회 한다. (주요 table saq_mon_cfm_qta, saq_mon_cfm_tgt_vvd)
	  * </pre>
	  */
	public MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0126Tab01RSQL(){
		setQuery();
		...
		query.append("FileName : MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0126Tab01RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            SMR.TRD_CD LIKE INP.TRD_CD||'%' AND " ).append("\n"); 
		...
		query.append("            )               " ).append("\n"); 
		query.append("     GROUP BY VVD.TRD_CD, VVD.CONV_DIR_CD, VVD.SUB_TRD_CD, VVD.BSE_YR||VVD.BSE_MON) " ).append("\n"); 
		query.append("SELECT " ).append("\n"); 
		...
		query.append("    DECODE(TEXT,'G.REV','G.REV*','CM(A)','CM*','CM(P)','CM*','CMPB(A)','CMPB','CMPB(P)','CMPB',TEXT) AS ITEM," ).append("\n"); 
		...
		query.append("                           AND   TRD_CD = SMR.TRD_CD) AS BSA_LVL_2," ).append("\n"); 
		query.append("                          (SELECT SUM(BSA) " ).append("\n"); 
		query.append("                           FROM  VVD " ).append("\n"); 
		query.append("                           WHERE YRMON = SMR.YRMON" ).append("\n"); 
		query.append("                           AND   TRD_CD = SMR.TRD_CD" ).append("\n"); 
		query.append("                           AND   CONV_DIR_CD = SMR.CONV_DIR_CD) AS BSA_LVL_3," ).append("\n"); 
		query.append("                          (SELECT SUM(BSA) " ).append("\n"); 
		query.append("                           FROM  VVD " ).append("\n"); 
		query.append("                           WHERE YRMON = SMR.YRMON" ).append("\n"); 
		query.append("                           AND   TRD_CD = SMR.TRD_CD" ).append("\n"); 
		query.append("                           AND   CONV_DIR_CD = SMR.CONV_DIR_CD" ).append("\n"); 
		query.append("                           AND   SUB_TRD_CD = SMR.SUB_TRD_CD) AS BSA_LVL_4, " ).append("\n"); 
		...
		query.append("                           FROM  VVD " ).append("\n"); 
		query.append("                           WHERE YRMON = SMR.YRMON" ).append("\n"); 
		query.append("                           AND   TRD_CD = SMR.TRD_CD) AS VOYAGE_LVL_2," ).append("\n"); 
		query.append("                          (SELECT SUM(VOYAGE) " ).append("\n"); 
		query.append("                           FROM  VVD " ).append("\n"); 
		query.append("                           WHERE YRMON = SMR.YRMON" ).append("\n"); 
		query.append("                           AND   TRD_CD = SMR.TRD_CD" ).append("\n"); 
		query.append("                           AND   CONV_DIR_CD = SMR.CONV_DIR_CD) AS VOYAGE_LVL_3," ).append("\n"); 
		query.append("                          (SELECT SUM(VOYAGE) " ).append("\n"); 
		query.append("                           FROM  VVD " ).append("\n"); 
		query.append("                           WHERE YRMON = SMR.YRMON" ).append("\n"); 
		query.append("                           AND   TRD_CD = SMR.TRD_CD" ).append("\n"); 
		query.append("                           AND   CONV_DIR_CD = SMR.CONV_DIR_CD" ).append("\n"); 
		query.append("                           AND   SUB_TRD_CD = SMR.SUB_TRD_CD) AS VOYAGE_LVL_4, " ).append("\n"); 
		...
		query.append("                    FROM  (" ).append("\n"); 
		query.append("                            SELECT" ).append("\n"); 
		query.append("                                  GROUPING(TRD_CD)||GROUPING(CONV_DIR_CD)||GROUPING(SUB_TRD_CD)AS G_FLAG," ).append("\n"); 
		query.append("                                  MIN(SMR.TRD_CD||SMR.CONV_DIR_CD||SMR.SUB_TRD_CD) AS KEY,                  " ).append("\n"); 
		query.append("                                  TRD_CD," ).append("\n"); 
		...
		query.append("                                         										 " ).append("\n"); 
		query.append("                                         )     										" ).append("\n"); 
		query.append("                                    GROUP BY SMR.TRD_CD, SMR.DIR_CD, SMR.CONV_DIR_CD, VVD.SUB_TRD_CD, SMR.BSE_YR||SMR.BSE_MON" ).append("\n"); 
		query.append("                                  ) SMR " ).append("\n"); 
		query.append("                            GROUP BY GROUPING SETS((YRMON), " ).append("\n"); 
		query.append("                                                    (SMR.TRD_CD, YRMON), " ).append("\n"); 
		query.append("                                                    (SMR.TRD_CD, SMR.CONV_DIR_CD, YRMON), " ).append("\n"); 
		query.append("                                                    (SMR.TRD_CD, SMR.CONV_DIR_CD, SMR.SUB_TRD_CD, YRMON))    " ).append("\n"); 
		query.append("                          ) SMR    " ).append("\n"); 
		query.append("                  ) SMR                                                                                " ).append("\n"); 
		query.append("          ) SMR " ).append("\n"); 
		query.append("          JOIN " ).append("\n"); 
		...
		query.append("      GROUP BY SMR.SLEVEL, SMR.TRD_CD, SMR.DIR_CD, SMR.SUB_TRD_CD, ITM.ROW_SEQ, ITM.TEXT, ITM.CODE           " ).append("\n"); 
		query.append("      ) SMR " ).append("\n"); 

...
import com.clt.apps.opus.esm.acm.acmagreement.ffcmpnagreement.integration.FFCmpnAgreementDBDAOAddFFAgreementCSQL;
...
public class SPCLCmpnAgreementDBDAO extends DBDAOSupport {
	...
	public List<SCompAgreementVO> searchSCompAgreement(SCompAgreementVO scompAgreementVO) throws DAOException {
		 ...
		 */
		 @SuppressWarnings("unchecked")
		public List<SCompAgreementVO> searchSCompAgreementDup(SCompAgreementVO scompAgreementVO) throws DAOException {
			DBRowSet dbRowset = null;
			List<SCompAgreementVO> list = null;
			//query parameter
			Map<String, Object> param = new HashMap<String, Object>();
			...
			try{
				...
				dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new SPCLCmpnAgreementDBDAOSearchSCompAgreementDupRSQL(), param, velParam);
				list = (List)RowSetUtil.rowSetToVOs(dbRowset, SCompAgreementVO.class);
			} catch(SQLException se) {
				log.error(se.getMessage(), se);
				throw new DAOException(new ErrorHandler(se).getMessage());
	 ...
	 *
	 * @param SCompAgreementVO insertVoList
	 * @throws DAOException
	 */
	public void addSCompAgreement(SCompAgreementVO insertVoList) throws DAOException {
		int insCnt = 0;
		// query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		// velocity parameter
		...
		try {
			...
			insCnt = sqlExe.executeUpdate((ISQLTemplate) new SPCLCmpnAgreementDBDAOAddSPCLCmpnAgreementCSQL(),param, velParam);

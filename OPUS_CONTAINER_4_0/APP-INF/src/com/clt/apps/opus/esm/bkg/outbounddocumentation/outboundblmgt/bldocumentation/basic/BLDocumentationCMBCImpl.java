...
package com.clt.apps.opus.esm.bkg.outbounddocumentation.outboundblmgt.bldocumentation.basic;
...
public class BLDocumentationCMBCImpl extends BasicCommandSupport implements BLDocumentationCMBC {
    ...
    public void manageContainer(List<ContainerVO> updateVoList, String caFlg) throws EventException {
    	manageContainer(updateVoList, caFlg, null);
    }
     ...
     */
    public void manageContainer(List<ContainerVO> updateVoList, String caFlg, String uiNum) throws EventException {
    	try {
            int len = updateVoList == null ? 0 : updateVoList.size();
            for (int i = 0; i < len; i++) {
                ContainerVO cntrVo = updateVoList.get(i);
                if(uiNum != null) cntrVo.setUiNum(uiNum);
                log.debug("***** Manage Container : " + cntrVo.getIbflag() + " - " + cntrVo.getCntrNo() + " - " + uiNum);
    ...
    public List<Map<String,Object>> validateContainer(CntrEtcInfoVO cntrEtcInfoVO, ContainerVO[] containerVOs, String fnlCfmFlg) throws EventException {
        ...
        try {
            ...
            for(int i = 0; i < len; i++) {
                ...
                }else if ("B4".equals(containerVOs[i].getCntrTpszCd())){
                	log.debug("***** Break Bulk *********");
                }else{
...
//            }
            ...
            log.debug("[占쎄쑴�뒄 占쎌눘�뒄 占싼됵옙] = " +isSameLoc+ " [BKG=" +currAreaGrpId+ ", CNTR=" +cntrMstInfoVO.getSysAreaGrpId()+ "]");
            ...
            log.debug("[VVD 筌ｋ똾寃� 占싼됵옙] = " +vvdCheckFlag);
...
//		            log.debug("@@@@@@@@ vvd cycle:"+cycleBkgInfoVO.getVvd() + " @@@@@@@@ vvd etc:"+cntrEtcInfoVO.getTVvd());	
    ...
    /**
     * Check special characters value in CM -- ESM_BKG_0079_07
     * @param CmVO cmVO
     * @return CmVO
     * @exception EventException
     */
    public CmVO checkSpecialValue(CmVO cmVO) throws EventException {
        CmVO reCmVO = new CmVO();

        CmBkgInfoVO vo1 = cmVO.getCmBkgInfoVO();
        List<CmCntrInfoVO> vo2 = cmVO.getCmCntrInfoVOs();
        List<BkgCntrSealNoVO> vo3 = cmVO.getCntrSealNoVOs();
        List<BkgCntrMfDescVO> vo4 = cmVO.getBkgCntrMfDescVOs();
        List<BkgDgCgoVO> vo5 = cmVO.getBkgDgCgoVOs();   
        
        try {

            int len = (vo4 == null) ? 0 : vo4.size();
            for(int i = 0; i < len; i++) {
        		BkgCntrMfDescVO tempVO = vo4.get(i);
            	if(!"D".equals(vo4.get(i).getIbflag())) {
            		if(tempVO == null)            			continue;
            		
	                // 1. checkSpecialValue - CntrMfGdsDesc
	                log.debug(" * before CntrMfGdsDesc : " + tempVO.getCntrMfGdsDesc());
	                if(vo4.get(i).getCntrMfGdsDesc() != null && vo4.get(i).getCntrMfGdsDesc().length() > 0) {
	                	tempVO.setCntrMfGdsDesc(checkSpecialValue(vo4.get(i).getCntrMfGdsDesc()));
	            	}
	                // 2. checkSpecialValue - CntrMfMkDesc
//	                log.debug(" * before CntrMfMkDesc : " + tempVO.getCntrMfMkDesc());
	                if(vo4.get(i).getCntrMfMkDesc() != null && vo4.get(i).getCntrMfMkDesc().length() > 0) {
	                	tempVO.setCntrMfMkDesc(checkSpecialValue(vo4.get(i).getCntrMfMkDesc()));
	            	}
            	}
//                log.debug(" * after CntrMfGdsDesc : " + tempVO.getCntrMfGdsDesc());
//                log.debug(" * after CntrMfMkDesc : " + tempVO.getCntrMfMkDesc());
            	vo4.set(i, tempVO);
            }
            reCmVO.setCmBkgInfoVO(vo1);
            reCmVO.setCmCntrInfoVOs(vo2);
            reCmVO.setCntrSealNoVOs(vo3);
            reCmVO.setCntrMfDescVOs(vo4);
            reCmVO.setBkgDgCgoVOs(vo5);
...
//            }
     ...
     * @exception   EventException
     */
    public void modifyBlActWgt(BkgBlNoVO bkgBlNoVO, String actWgt, String wgtUtCd, SignOnUserAccount account, String vslUpdate)
            throws EventException {
        try {
            dbDao.modifyBlActWgt(bkgBlNoVO, actWgt, wgtUtCd, account, vslUpdate);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
...
//                }
            	...
            	if(item.getMstBkgCntrOpUpdate()) {
                	...
                	// BKG Vol = 1 占쎈떯�� UCR or MRN 占쏙옙占쎄퉭以� 占쎌꼷堉� 占쎈뜆�몵筌롳옙Container No Update
                	if("OP".equals(item.getCusCtmMovementVO().getMvmtStsCd()) || "OC".equals(item.getCusCtmMovementVO().getMvmtStsCd())){
                		GeneralBookingReceiptBC receiptCmd = new GeneralBookingReceiptBCImpl();
                        BkgRefDtlVO bkgRefDtlVO = new BkgRefDtlVO();
                        bkgRefDtlVO.setBkgNo(item.getCusCtmMovementVO().getBkgNo());
                        bkgRefDtlVO.setCntrNo(item.getCusCtmMovementVO().getCntrNo());
                        receiptCmd.modifyCntrNoUcrByMvmt(bkgRefDtlVO, "SYSTEM");
                	}
                	// Cntr Mvmt OC History - end
...
//                            				// 4. remove Reference
			            					...
			            					historyLineVO.setCrntCtnt("Detach Container : "+cntrMoveOpInfoVOs.get(i).getCntrNo() + "(Transferred to BKG#" + item.getCusCtmMovementVO().getBkgNo() + ")");
...
//                        	}
                        ...
                        // OP & OC In the event that seal number of OC event differs from OP, overwrite OP's seal number in booking. The latest seal number (which is OC's) should be the valid seal number. 
                    	if("OP".equals(item.getCusCtmMovementVO().getMvmtStsCd()) || "OC".equals(item.getCusCtmMovementVO().getMvmtStsCd())){
                    		if(null != item.getCusCtmMovementVO().getMvmtInpTpCd() && !"SPP".equals(item.getCusCtmMovementVO().getMvmtInpTpCd())){
                    			if(null != item.getCusCtmMovementVO().getMvmtStsCd() && !"".equals(item.getCusCtmMovementVO().getCntrSealNo())) {
                    				//CntrSealNo != null
                    				if(null != item.getCusCtmMovementVO().getCntrSealNo() && !"".equals(item.getCusCtmMovementVO().getCntrSealNo())){
                    					if("N".equals(dbDao.searchDupSealNo(item.getCusCtmMovementVO()))){
//                    						dbDao.removeMvmtCntrSealNo(item.getCusCtmMovementVO().getBkgNo(),item.getCusCtmMovementVO().getCntrNo(), "N");
                    						BkgCntrSealNoVO bkgCntrSealNoVO = new BkgCntrSealNoVO();
            		                    	bkgCntrSealNoVO.setBkgNo(item.getCusCtmMovementVO().getBkgNo());
            		                    	bkgCntrSealNoVO.setCntrNo(item.getCusCtmMovementVO().getCntrNo());
            		                    	bkgCntrSealNoVO.setCntrSealNo(item.getCusCtmMovementVO().getCntrSealNo());
            		                    	bkgCntrSealNoVO.setSealInpTpCd("CTM");
            		                    	bkgCntrSealNoVO.setPrnFlg("1");
            		                    	bkgCntrSealNoVO.setCreUsrId(item.getCusCtmMovementVO().getCreUsrId());
            		                    	dbDao.insertCntrSealNo(bkgCntrSealNoVO, "N");	
                        					}
...
//                }
   ...
   public void manageCmByHbl(HblVO hblVO, SignOnUserAccount account, String caFlg) throws EventException{
	...
	/**
     * multi split占쎌뮇肉� sourceBkg占쎈Ŋ苑� bkg_cntr, bkg_cntr_seal_no�몴占쏙옙�럩堉깍옙占퐐argetBkg占쏙옙insert占쎌뮆�뼄.<br>
     * 
     * @param BkgBlNoVO sourceBkg
     * @param BkgBlNoVO[] targetBkg
	...
	public void copyCntrCmByBkgMulti(BkgBlNoVO sourceBkg, BkgBlNoVO[] targetBkg,
		...
		try {
            //new bkg占쎌눖以� cntr占쏙옙癰귣벊沅�
            for(int i = 0; i < targetBkg.length; i++) {
                if(!sourceBkg.getBkgNo().equals(targetBkg[i].getBkgNo())){
                    for(int icnt = 0; icnt < selectCntrVO.size(); icnt++) {
                        // sourceBkg占쏙옙bkg_container�몴占퐐argetBkg占쎌눖以� 癰귣벊沅쀯옙�뮆�뼄
                    	log.debug("sourceBkg:"+sourceBkg.getBkgNo()+",targetBkg:"+targetBkg[i].getBkgNo());
                        if (selectCntrVO.get(icnt).getBkg_no().equals(targetBkg[i].getBkgNo())
                            && selectCntrVO.get(icnt).getSplitNo().length()>1){
                            dbDao.copyBkgCntrByBkg(targetBkg[i], sourceBkg, (SelectCntrVO) selectCntrVO.get(icnt), account);
                            // sourceBkg占쎈Ŋ苑� bkg_cntr_mf�몴占쏙옙�럩堉깍옙占퐐argetBkg占쏙옙insert占쎌뮆�뼄
                            dbDao.copyCmByBkg(targetBkg[i], sourceBkg, "S", (SelectCntrVO) selectCntrVO.get(icnt), account);
                            // sourceBkg占쏙옙bkg_cntr_seal_no�몴占퐐argetBkg占쎌눖以� 癰귣벊沅쀯옙�뮆�뼄
                            dbDao.copyBkgCntrSealByBKG(targetBkg[i], sourceBkg, (SelectCntrVO) selectCntrVO.get(icnt),account);
                        }
                    }
                }
            ...
            //占쎈Ŧ�궚BkgNo占쏙옙占쏙옙鍮� Cntr筌ｋ똾寃� 占쎈똻�젫占쏙옙筌ｌ꼶�봺
            for(int i = 0; i < targetBkg.length; i++) {
                if(sourceBkg.getBkgNo().equals(targetBkg[i].getBkgNo())){
                for(int icnt = 0; icnt < selectCntrVO.size(); icnt++) {
                    if (selectCntrVO.get(icnt).getBkg_no().equals(targetBkg[i].getBkgNo())
                      && selectCntrVO.get(icnt).getSplitNo().length()<1){
                        // 占쎄쑬�뼎獄쏆룇占� cntr No嚥∽옙bkg_cntr_seal_no 占쎈Ŋ苑� 占쏙옙�젫占쎌뮆�뼄
                        dbDao.removeCntrSealNoAfterSplit(sourceBkg, (SelectCntrVO) selectCntrVO.get(icnt));
                        // BKG CONTAINER MANIFEST DESCRIPTION 占쎈베�궖�몴占쏙옙占쎌젫占쎌뮆�뼄.
                        dbDao.removeCmByCntr(sourceBkg.getBkgNo(), selectCntrVO.get(icnt).getCntr_no(), "N");
                        // BKG CONTAINER 占쎈슣�뵠�뇡遺용퓠占쏙옙Delete Flag�몴占�'Y'嚥∽옙癰귨옙瑗�
                        dbDao.removeContainer(sourceBkg.getBkgNo(), selectCntrVO.get(icnt).getCntr_no(), "N");
                    }
                  }
               }
...
//			}                    
     ...
     * @exception EventException
     */
    public void modifyShpFlg(String bkgNo, SignOnUserAccount account) throws EventException {
        try {
            dbDao.modifyShpFlg(bkgNo, account);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
     ...
     * @throws EventException
     */
	public void manageCmBkgCntrShp(SignOnUserAccount account, String bkgNo, BkgCntrShpVO[] bkgCntrShpVOs) throws EventException {
		try {
			dbDao.deleteCmBkgCntrShp(bkgNo);
			if(bkgCntrShpVOs != null){
				for (int i = 0; i < bkgCntrShpVOs.length; i++) {
					dbDao.manageCmBkgCntrShp(account, bkgNo, bkgCntrShpVOs[i]);
				}
			}
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
	   ...
	   /**
     * container information retrieve -- UI_BKG-007904, 
     * BKG CONTAINER, BKG CONTAINER SEAL NUMBER, BKG BOOKING, BKG DOCUMENT PROCESS SCHEDULE, BKG RATE etc.
     * @param String bkgNo
     * @param int cnt
     * @return CntrInfoOutVO
     * @exception EventException 
     */
    public CntrInfoOutVO searchBbContainer(String bkgNo, int cnt) throws EventException {

        CntrInfoOutVO cntrInfoOutVO = new CntrInfoOutVO();
        try {
            	cntrInfoOutVO.setCntrs(dbDao.searchBbContainer(bkgNo, cnt));
     ...
     * @exception EventException
     */
	public void modifyBkgCntrVgmWgt(BkgVgmWgtVO bkgVgmWgtVO, SignOnUserAccount account) throws EventException{
        try {
            dbDao.modifyBkgCntrVgmWgt(bkgVgmWgtVO, account);
	    } catch (DAOException ex) {
		   throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
       	} catch (Exception ex) {
    	   throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
    ...
    /**
     *  change rf_cgo_flg value according to Bkg_no of Container(ESM_BKG_0498)
     * @param   String bkgNo 
     * @param   String spclTp 
     * @param   String cntrNo
     * @param   String caFlg
     * @exception   EventException
     */
    public void modifyCmFlgBySpcl(String bkgNo, String spclTp, String cntrNo, String caFlg) throws EventException {
        try {
            dbDao.modifyCmFlgBySpcl(bkgNo, spclTp, cntrNo, caFlg);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
     ...
     * @exception   EventException
     */
    public void modifyCmFlgBySpcl2(String bkgNo, String spclTp, String cntrNo, String caFlg) throws EventException {
        try {
            dbDao.modifyCmFlgBySpcl2(bkgNo, spclTp, cntrNo, caFlg);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
    ...
    /**
     * Master Container Damage Search -- ESM_BKG_0079_04
     * @param ContainerVO[] containerVOs
     * @return String
     * @exception EventException 
     */
    public String searchDamageMstContainer(ContainerVO[] containerVOs) throws EventException {
    	try {
            int len = containerVOs == null ? 0 : containerVOs.length;
            for(int i = 0; i < len; i++) {
                String ibFlag = containerVOs[i].getIbflag();
                log.debug(">" + containerVOs[i].getCntrNo() + " : ibFlag=" + ibFlag);
    ...
    /**
     * CoBkg.js�쓽 chekcSpecialValue(value) �듅�닔臾몄옄 �삁�쇅泥섎━ 湲곕뒫 援ы쁽
     * @param inputText
     * @return
     * @throws EventException
     */
    public String checkSpecialValue(String inputText) throws EventException {
    	String re_value = null;
    	
    	try{
    		String srcCharValue = inputText.toUpperCase();
    		String SPECIAL_CHR = "[^(媛�-�옡�꽦-�뀕�뀖-�뀭a-zA-Z0-9\u4e00-\u9fff\u3400-\u4dff\uf900-\ufaff `~!@#$%^&*()\\[\\]\\{\\}\\_+-=:;/|<>?\\\"\\\')\\s]";
    		re_value = srcCharValue.replaceAll(SPECIAL_CHR, "");  //word
    	}catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        }
    	return re_value;
    ...
    /**
     *  change BL_MV_TP_NM of BKG_BL_DOC by RD Term of BKG_BOOKING
     * @param   String bkgNo 
     ...
     * @exception   EventException
     */
    public void modifyMoveTypeByBkg(String bkgNo, String rcvTermCd, String deTermCd, String usrId) throws EventException {
        try {
            dbDao.modifyMoveTypeByBkg(bkgNo, rcvTermCd, deTermCd, usrId);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);

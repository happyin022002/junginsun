...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esm.bkg.outbounddocumentation.outboundblmgt 화면을 통해 서버로 전송되는 HTML DOM 객체의
...
public class ESM_BKG_0170HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;

    /**
     * ESM_BKG_0170HTMLAction 객체를 생성
     */
    public ESM_BKG_0170HTMLAction() {}
    ...
    public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EsmBkg0170Event event = new EsmBkg0170Event();
        log.debug("::CALL:: ESM_BKG_0170HTMLAction : " + command.getCommand());

        event.setBkgNo(JSPUtil.getParameter(request, "bkg_no"));
        event.setCntrNo(JSPUtil.getParameter(request, "cntr_no"));
        event.setCntrVol(JSPUtil.getParameter(request, "cntr_vol"));
        log.debug("* bkg_no --> " + event.getBkgNo());
        log.debug("* cntr_no --> " + event.getCntrNo());
        log.debug("* cntr_vol --> " + event.getCntrVol());
        if(command.isCommand(FormCommand.MULTI01) || command.isCommand(FormCommand.MULTI02)) {
            event.setCntrCopyVOs((CntrCopyVO[]) getVOs(request, CntrCopyVO.class, ""));
            log.debug("* cntrCopyVOs --> " + event.getCntrCopyVOs());
        }

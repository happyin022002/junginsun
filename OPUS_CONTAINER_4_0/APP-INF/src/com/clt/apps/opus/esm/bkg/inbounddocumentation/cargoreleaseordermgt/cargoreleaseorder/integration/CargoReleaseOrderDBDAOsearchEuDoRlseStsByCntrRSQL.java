...
public class CargoReleaseOrderDBDAOsearchEuDoRlseStsByCntrRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , SUBQ.CNTR_HLD_FLG                         AS CNTR_HLD_FLG" ).append("\n"); 
		query.append("     , DECODE(SUBQ.CNTR_HLD_FLG,'Y','HOLD','')   AS CNTR_HLD_NM" ).append("\n"); 
		...
		query.append("     , TO_CHAR(SUBQ.CNTR_HLD_DT,'YYYY-MM-DD HH24:MI', 'NLS_DATE_LANGUAGE=ENGLISH') AS CNTR_HLD_DT" ).append("\n"); 
		query.append("     , SUBQ.CNTR_HLD_USR_ID                      AS CNTR_HLD_USR_ID" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("              , DECODE(HCNTR.CNTR_NO,'','N','Y') AS CNTR_HLD_FLG" ).append("\n"); 
		query.append("              , HCNTR.CRE_DT                     AS CNTR_HLD_DT" ).append("\n"); 
		query.append("              , HCNTR.CRE_USR_ID                 AS CNTR_HLD_USR_ID" ).append("\n"); 
		query.append("          FROM BKG_CONTAINER BCNT" ).append("\n"); 
		query.append("             , BKG_DO_CNTR   DCNT" ).append("\n"); 
		query.append("             , BKG_IB_CGO_RLSE_HLD_CNTR HCNTR" ).append("\n"); 
		...
		query.append("           AND BCNT.BKG_NO   = HCNTR.BKG_NO(+)" ).append("\n"); 
		query.append("           AND BCNT.CNTR_NO   = HCNTR.CNTR_NO(+)" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append(" ORDER BY CNTR_HLD_FLG DESC, RLSE_STS_CD " ).append("\n"); 

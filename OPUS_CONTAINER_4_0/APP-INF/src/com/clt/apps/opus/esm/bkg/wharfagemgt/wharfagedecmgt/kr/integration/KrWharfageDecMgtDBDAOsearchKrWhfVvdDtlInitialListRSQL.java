...
public class KrWharfageDecMgtDBDAOsearchKrWhfVvdDtlInitialListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   AND DECODE(@[whf_bnd_cd],'IN',SUBSTR(A.WHF_BND_CD,1,1),A.WHF_BND_CD) = DECODE(@[whf_bnd_cd],'IN','I',@[whf_bnd_cd])" ).append("\n"); 
		...
		query.append(" GROUP BY DECODE(NVL(TRIM(AA.WFG_EXPT_CD), 'Z'), 'Z', 'BN', 'B', 'BN', 'B' || AA.WFG_EXPT_CD) ) A, " ).append("\n"); 
		query.append("( SELECT 'dd20' AS SORT_VALUE ,'N' AS SORT_KEY FROM DUAL UNION ALL" ).append("\n"); 

...
public class ArrivalNoticeDBDAOsearchArrNtcSendListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("         , DECODE(INQR.CHG_DP_FLG, 'Y', '1', '0') AS CHG_DP_FLG" ).append("\n"); 
		...
		query.append("         , DECODE(@[ts_flg], 'Y', 'Y', DECODE(TRIM(DVVD.SKD_VOY_NO), NULL, 'N', 'Y')) AS RD_PRT_FLG -- RD를 출력하기 위한 기본요건을 갖추었는지 점검하는 Flag " ).append("\n"); 
		...
		query.append("                    OR MDM.CNT_CD ='US' THEN  DECODE( MDM.SCONTI_CD,'AF','AF'" ).append("\n"); 
		...
		query.append("             , MAX(NVL(ADTL.EML_SND_DT, GLOBALDATE_PKG.TIME_CONV_FNC(COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC,EMSD.EML_DT,GLOBALDATE_PKG.GET_LOCCD_FNC(@[ofc_cd]) ) )) EML_SND_DT" ).append("\n"); 
		...
		query.append("             , CASE WHEN MAX(NVL(TO_CHAR( NVL(ADTL.EML_SND_DT,GLOBALDATE_PKG.TIME_CONV_FNC(COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC,EMSD.EML_DT,BKGM.LOC_CD)), 'YYYYMMDDHH24MISS'),'00000000000000') || ADTL.EML_SND_USR_ID) >  MAX(NVL(TO_CHAR( NVL(ADTL.FAX_SND_DT,FXSD.FAX_SND_LOCL_DT), 'YYYYMMDDHH24MISS'),'00000000000000') || ADTL.FAX_SND_USR_ID)" ).append("\n"); 
		query.append("                       THEN SUBSTR(MAX(NVL(TO_CHAR(NVL(ADTL.EML_SND_DT,GLOBALDATE_PKG.TIME_CONV_FNC(COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC,EMSD.EML_DT,(SELECT B.LOC_CD" ).append("\n"); 
		...
		query.append("                   AND BVVD.VSL_CD = VSKD.VSL_CD(+)  -- Duration이 아닌경우에는 데이터를 추출하기 위하여 해당과 같이 처리한다." ).append("\n"); 
		query.append("                   AND BVVD.SKD_VOY_NO = VSKD.SKD_VOY_NO(+)" ).append("\n"); 
		query.append("                   AND BVVD.SKD_DIR_CD = VSKD.SKD_DIR_CD(+)" ).append("\n"); 
		query.append("                   AND BVVD.POD_CD = VSKD.VPS_PORT_CD(+)" ).append("\n"); 
		query.append("                   AND BVVD.POD_CLPT_IND_SEQ = VSKD.CLPT_IND_SEQ(+)" ).append("\n"); 
		...
		query.append("                   AND NVL(BCST.VAL_CD, '*') <> 'S'" ).append("\n"); 
		query.append("                   AND (" ).append("\n"); 

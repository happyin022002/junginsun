...
*@FileName : ESM_BKG_0493HTMLAction.java
...
public class ESM_BKG_0493HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0490HTMLAction 객체를 생성
	 */
	public ESM_BKG_0493HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0493Event event = new EsmBkg0493Event();

		VesselArrivalCondVO vesselArrivalCondVO = new SriLankaVesselArrivalCondVO();

		if(command.isCommand(FormCommand.SEARCH)) {
			vesselArrivalCondVO = (SriLankaVesselArrivalCondVO)getVO(request, SriLankaVesselArrivalCondVO.class);
			event.setPgNO(request.getParameter("pgNo"));
			event.setSriLankaVesselArrivalCondVO((SriLankaVesselArrivalCondVO)vesselArrivalCondVO);
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setSriLankaVesselArrivalVO((SriLankaVesselArrivalVO) getVO(request, SriLankaVesselArrivalVO.class));
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSriLankaVesselArrivalTransmitVO((SriLankaVesselArrivalTransmitVO)getVO(request, SriLankaVesselArrivalTransmitVO.class));
		}

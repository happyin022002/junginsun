...
public class ArrivalNoticeDBDAOsearchArrNtcInfoListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                         , NVL((SELECT RTYD.MCNTR_RTN_YD_CD" ).append("\n"); 
		...
		query.append("                       , BKGM.MTY_RTN_YD_CD                AS MTY_RTN_YD_CD" ).append("\n"); 
		query.append("                       , BKG_GET_ILND_TZTM_FNC(BKGM.BL_NO) AS ADD_DT" ).append("\n"); 
		query.append("                       , (SELECT  SUBSTR(" ).append("\n"); 
		query.append("                                  MAX ( CASE WHEN COPD.ACT_CD = 'FITRDO' OR COPD.ACT_CD = 'FITYDO' THEN '1'" ).append("\n"); 
		...
		query.append("                              AND COPD.ACT_CD IN ('FITRDO','FITYDO','FUVMAD')  -- FITRDO우선 처리 BP 19526 수정 1. FITRDO 2. FITYDO 3. FUVMAD" ).append("\n"); 
		...
		query.append("                          ) > 0   --- NVL(VAL_CD, '*') <> 'S' 의 처리 -- Consignee, Notify둘다 Skip이면 출력안함, 아닌경우 출력함" ).append("\n"); 

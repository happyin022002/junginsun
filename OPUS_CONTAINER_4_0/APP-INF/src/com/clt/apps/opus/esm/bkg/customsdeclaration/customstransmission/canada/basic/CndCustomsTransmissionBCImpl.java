...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.BayPlanCntrDetailCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.BayPlanCntrListConCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.BayPlanPolDetailVO;
...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsManifestAmendmentCondVO;
...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsRcvHisListCondVO;
...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsSndHisListCondVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsSndLogDtlCondVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsSndLogDtlVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsTrsmRsltListCondVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.CndCstmsTrsmRsltVO;
...
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.SendDetailLogCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.SendLogDetailCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.StiCondListCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.StiDetailCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.VesselEtaCondCndVO;
import com.clt.apps.opus.esm.bkg.customsdeclaration.customstransmission.canada.vo.VesselEtaInfoCndVO;
 ...
 * - OPUS-CustomsDeclaration handling business logic<br>
 *
 * @author c9014874
 * @see
 * @since J2EE 1.4
 */
public class CndCustomsTransmissionBCImpl extends BasicCommandSupport implements CndCustomsTransmissionBC {
		...
		try
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (!"".equals(vo.getBlNo()))
				{
					 ...
					 * 44	Rejection
					 * 48	Suspended */
					if ("06".equals(vo.getCstmsAckProcRsltCd()) || "01".equals(vo.getCstmsAckProcRsltCd()))
					{
						iProcessed++;
					}
...
//							&& !"A".equals(vo.getCstmsAckRcvRsltCd()))
					if ("37".equals(vo.getCstmsAckProcRsltCd()) || "44".equals(vo.getCstmsAckProcRsltCd())
							|| "21".equals(vo.getCstmsAckProcRsltCd()) || "48".equals(vo.getCstmsAckProcRsltCd())
							|| (!"A".equals(vo.getCstmsAckRcvRsltCd()) && !"E".equals(vo.getCstmsAckRcvRsltCd()) && !"01".equals(vo.getCstmsAckProcRsltCd())) )
					{
						iError++;
					}
				}
	...
	public String transmitManifest(ManifestTransmitVO manifestTransmitVO, SignOnUserAccount account) throws EventException {
		...
		try {
			if ("ESM_BKG_0002".equals(manifestTransmitVO.getPgmNo())) {
				 ...
				 * BackEndJob 방식으로 호출
				 *******************************************************/
				return backEndJobManager.execute(cndBackEndJob, account.getUsr_id(), "Canada Manifest Transmit(MI)");
...
//				flatFile.append(utilBC.searchCstmsEdiHeader("CK92756", "RCCECECPP", "311A6A")).append("\n");
				flatFile.append(utilBC.searchCstmsEdiHeaderNew("CA_311A6A")).append("\n");
			} else if ("S10".equals(blInfo.getCstmsMfTpCd())) {
//				flatFile.append(utilBC.searchCstmsEdiHeader("CK92756", "RCCECECPP", "311S10")).append("\n");
...
//			bkgCstmsAdvEdiBlRspnVO.setCrrBatNo("HJSC" + blInfo.getVvd() + blInfo.getBlpod());
			...
			if (flatFile.toString().startsWith("$$$MSGSTART:")) {
				...
				log.error("\n\n==================== [Incorrect Flatfile] ====================" +
							"\nCndCustomsTransmissionBCImpl > transAmendManifest" +
							"\n--------------------------------------------------------------\n" +
							flatFile.toString() +
							"\n==============================================================\n");
			}
	...
	public String makeCndVesselArrivalFlatFile(CndVesselArrivalTransmitVO condVO, CndVesselArrivalTransmitVO vslVO,
		...
		// 헤더
		BookingUtil utilBC = new BookingUtil();
//		flatFile.append(utilBC.searchCstmsEdiHeader("CK92756", "RCCECECPP", "311A6")).append("\n");
		...
		String strConVvd = comBc.searchConVvd(condVO.getVslCd() + condVO.getSkdVoyNo() + condVO.getSkdDirCd(), polVO.getPodCd(), "I");
		if (!"".equals(strConVvd) && strConVvd.length() > 4)
			strConVvd = strConVvd.substring(4);
		flatFile.append("CON_VOYAGE:").append(strConVvd).append("\n");
	...
	public String makeCndActualVesselArrivalFlatFile(CndVesselArrivalTransmitVO actVslVO) throws EventException {
		...
		// 헤더
		BookingUtil utilBC = new BookingUtil();
		flatFile.append(utilBC.searchCstmsEdiHeader(ConstantMgr.getCompanyCode(), "CANCUS_ACICA", "353")).append("\n");
		// 메시지 시작
		flatFile.append("{ARR_INFO").append("\n");
		flatFile.append("E_I_IND:I").append("\n");
		flatFile.append("CBSA_CARRIER_CD:9165").append("\n");
...
//				bkgCstmsAdvRcvLogDtlVO.setRcvSeq(rcvSeq);
				...
				if (bkgCstmsAdvRcvLogDtlVO.getMsgDesc().startsWith("EQUIPMENT:"))
				{
					cndCstmsBlByKeyVO.setCntrNo(bkgCstmsAdvRcvLogDtlVO.getMsgDesc().substring(10));
				}
			}
			// 담당자에 수신결과에 따른 메일전송
			if (!"".equals(strEmlType))
			{
				this.sendEml(strEmlType, bkgCstmsAdvRcvLogVO, cndCstmsBlByKeyVO);
			}
		}
		else
		{
	 ...
	 * @throws Exception
	 */
	private void setTemplateArg(TemplateMail template, BkgCstmsAdvRcvLogVO bkgCstmsAdvRcvLogVO, CndCstmsBlByKeyVO cndCstmsBlByKeyVO, boolean vvd, boolean blNo,
			boolean shipper, boolean consignee, boolean ofcNm, boolean pol,
			boolean pod, boolean container, boolean reason) throws Exception
	{
...
//		if (ofcNm) template.setArg("ofcNm", cndCstmsBlByKeyVO.getOfcEngNm());
	 ...
	 * @throws EventException
	 */
	public List<ManifestTransmitVO> searchLastForeignPort(StowageManifestCondVO stowageManifestCondVO)
			throws EventException {
		StiCondListCndVO condVO = (StiCondListCndVO) stowageManifestCondVO;
		List<StiDetailCndVO> list = null;
		List<ManifestTransmitVO> rtnList = new ArrayList<ManifestTransmitVO>();
		...
		try
		{
			...
			if (manifestTransmitVOs != null)
			{
				...
				for (int i = 0; i < bayDetailVos.length; i++)
				{
					...
					sendLogDetailVO.setSeq(sEdiHeader.substring(62,77));

...
* 1.0 Creation
=========================================================*/
package com.clt.apps.opus.esm.acm.acmsimulation.acmsimulation.basic;
...
/**
 * OPUS-ACMSimulation Business Logic Command Interface<br>
...
public class ACMSimulationBCImpl extends BasicCommandSupport implements ACMSimulationBC {
	...
	public void simulateAGNCommSimulation(String bkgNo, String usrId, String simNo) throws EventException {
		...
		ChgAmtInfoVO chgAmtInfoVO = new ChgAmtInfoVO();
...
//						}
						...
						dbDao.addAcmSimCommDtlFx(agmtVO);
						if(agmtVO.getMhFlg()=="Y"){
							dbDao.addAcmSimCommDtlFxMh(agmtVO);
						}
...
//			dbDao.modifyAcmSimCommZeroSum2(zeroSumList);
	...
	public void manageAGNCommMassSimList(AGNCommMassSimVO agnCommMassSimVO, SignOnUserAccount account) throws EventException {
		try {
			...
			//ACM_SIM_INFO에 insert 
			dbDao.addMassSimInfo(agnCommMassSimVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
	 ...
	 * @exception EventException
	 */
	public String getMassSimulationNo() throws EventException {
		try {
			return dbDao.getMassSimulationNo();
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);

...
/**
 *  RFANoteConversionProposalDBDAO <br>
...
public class RFANoteConversionProposalDBDAO extends DBDAOSupport {
	...
	public void addProposalNoteConversionAmend(List<PriRpMnVO> insModels) throws DAOException,Exception {
		try {
			...
			int insCnt[] = null;
			if(insModels.size() > 0){
				insCnt = sqlExe.executeBatch((ISQLTemplate)new RFANoteConversionProposalDBDAOPriRfaNoteConvAmdVOCSQL(), insModels,null);
				for(int i = 0; i < insCnt.length; i++){
					if(insCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to insert No"+ i + " SQL");
				}
    ...
    public void addRfaNoteConv(List<PriRfaNoteConvListVO> insModels) throws DAOException {
        try {
            ...
            int insCnt[] = null;
            if (insModels.size() > 0) {
                insCnt = sqlExe.executeBatch((ISQLTemplate) new RFANoteConversionProposalDBDAOPriRfaNoteConvVOCSQL(), insModels,
                        null);
                for (int i = 0; i < insCnt.length; i++) {
                    if (insCnt[i] == Statement.EXECUTE_FAILED)
                        throw new DAOException("Fail to insert No" + i + " SQL");

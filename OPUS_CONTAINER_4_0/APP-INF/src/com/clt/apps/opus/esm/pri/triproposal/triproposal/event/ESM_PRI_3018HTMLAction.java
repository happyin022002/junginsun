 ...
 *@FileName : ESM_PRI_3018HTMLAction.java
 *@FileTitle : ESM_PRI_3018HTMLAction
 *Open Issues :
 *Change history :
...
public class ESM_PRI_3018HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_PRI_3018HTMLAction 객체를 생성
	 */
	public ESM_PRI_3018HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri3018Event event = new EsmPri3018Event();

		if (command.isCommand(FormCommand.MODIFY13)) {
			String sPubDt = request.getParameter("pub_dt");
			PriTriRtVO[] vos = (PriTriRtVO[]) getVOs(request, PriTriRtVO.class, "sheet1_");
			if(vos != null) {
				for (int i = 0; i < vos.length; i++) {
					vos[i].setPubDt(sPubDt);
				}
				event.setPriTriRtVOs(vos);
			}
		}

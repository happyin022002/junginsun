			...
			if (rowSet != null) {
				...
				while (rowSet[1].next()) {
					...
					if (stnd_cost_cd.equals("LOADFACT")) {
						format_flg = " DATA-FORMAT='dfFloatOrg' POINT-COUNT='2'";
				    } else {
						format_flg = "";
				    }
				    ...
				    for(int idx=1; idx<=headCnt; idx++) {
				    	strBuilder.append("\n<TD ").append(bold).append(format_flg).append(">").append(JSPUtil.getNull(rowSet[1].getString("dir_e_amt" + idx))).append("</TD>");
				    	strBuilder.append("\n<TD ").append(bold).append(format_flg).append(">").append(JSPUtil.getNull(rowSet[1].getString("dir_w_amt" + idx))).append("</TD>");

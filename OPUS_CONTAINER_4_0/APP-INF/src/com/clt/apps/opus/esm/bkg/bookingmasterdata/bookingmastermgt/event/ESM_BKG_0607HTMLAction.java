...
*@FileName : ESM_BKG_0607HTMLAction.java
...
public class ESM_BKG_0607HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0607HTMLAction 객체를 생성
	 */
	public ESM_BKG_0607HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0607Event event = new EsmBkg0607Event();
	
		if(command.isCommand(FormCommand.MULTI)) {
			
			event.setBkgHamoTrfVOS((BkgHamoTrfVO[])getVOs(request, BkgHamoTrfVO .class,"sheet1_"));
			String hamoTpCd = JSPUtil.getParameter(request, "hamo_tp_cd");
			event.setHamoTpCd(hamoTpCd);
			
			}else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgHamoTrfVO((BkgHamoTrfVO)getVO(request, BkgHamoTrfVO .class));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
				event.setBkgHamoTrfVO((BkgHamoTrfVO)getVO(request, SearchBDRPolVO .class));

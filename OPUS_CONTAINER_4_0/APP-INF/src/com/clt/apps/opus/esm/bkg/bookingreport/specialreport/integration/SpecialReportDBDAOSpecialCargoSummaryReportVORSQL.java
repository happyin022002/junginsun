...
public class SpecialReportDBDAOSpecialCargoSummaryReportVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("               ,DENSE_RANK() OVER( PARTITION BY BK.BKG_NO, SP.SPCL_CGO_TYPE ORDER BY SP.SPCL_CNTR_SEQ) SPCL_CNTR_SEQ" ).append("\n"); 
		query.append("               ,DECODE(SP.SPCL_CGO_TYPE, 'DG', DENSE_RANK() OVER( PARTITION BY BK.BKG_NO, SP.SPCL_CGO_TYPE, SP.SPCL_CNTR_SEQ ORDER BY SP.SPCL_CGO_SEQ, SP.DCGO_SEQ)" ).append("\n"); 
		query.append("                                          	 , DENSE_RANK() OVER( PARTITION BY BK.BKG_NO, SP.SPCL_CGO_TYPE ORDER BY SP.SPCL_CNTR_SEQ)) SPCL_CGO_SEQ" ).append("\n"); 
		...
		query.append("               ,BKG_GET_DG_APLY_NO_FNC(BK.BKG_NO) AS DG_APPR_REF--2015.03.04 DG APLY NO RETRIEVE" ).append("\n"); 
		query.append("               ,SP.MRN_POLUT_FLG" ).append("\n"); 
		query.append("               ,SP.CDO_TEMP" ).append("\n"); 
		query.append("               ,SP.VENT" ).append("\n"); 
		query.append("			   ,SP.CNTR_VENT_TP_CD" ).append("\n"); 
		...
		query.append("              ,(SELECT BKG_VVD.BKG_NO,  BKG_VVD.POL_CD,BKG_VVD.POL_YD_CD, BKG_VVD.POD_CD" ).append("\n"); 
		...
		query.append("                                                   WHERE  BKG_VVD.BKG_NO = BV.BKG_NO" ).append("\n"); 

...
*@FileName : ESM_BKG_3002HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.6
 */
public class ESM_BKG_3002HTMLAction  extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_3002HTMLAction 객체를 생성
	 */
	public ESM_BKG_3002HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBkg3002Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg3002Event event = new EsmBkg3002Event();
		
		if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01)) {
			event.setBkgCtrlBlGrpVO((BkgCtrlBlGrpVO)getVO(request, BkgCtrlBlGrpVO.class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setBkgCtrlBlGrpVOs((BkgCtrlBlGrpVO[])getVOs(request, BkgCtrlBlGrpVO.class, "sheet1_"));
			event.setBkgCtrlBlGrpCustVOs((BkgCtrlBlGrpCustVO[])getVOs(request, BkgCtrlBlGrpCustVO.class, "sheet2_"));
		}else if(command.isCommand(FormCommand.REMOVE)) {
			event.setBkgCtrlBlGrpVOs((BkgCtrlBlGrpVO[])getVOs(request, BkgCtrlBlGrpVO.class, "sheet1_"));
		}else if(command.isCommand(FormCommand.REMOVE01)) {
			event.setBkgCtrlBlGrpCustVOs((BkgCtrlBlGrpCustVO[])getVOs(request, BkgCtrlBlGrpCustVO.class, "sheet1_"));
		}
		
//		else if(command.isCommand(FormCommand.SEARCH02)) {
//			// BKG_CTRL_BL_GRP_SEQ.nextval 호출
//		}
		return  event;
	}

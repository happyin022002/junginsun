...
*@FileName : MonthlyQuotaAdjustmentTradeRHQDBDAOSearchMonthlyQuotaAdjustmentTradeRhqModify0148ListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MonthlyQuotaAdjustmentTradeRHQDBDAOSearchMonthlyQuotaAdjustmentTradeRhqModify0148ListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public MonthlyQuotaAdjustmentTradeRHQDBDAOSearchMonthlyQuotaAdjustmentTradeRhqModify0148ListRSQL(){
		setQuery();
		...
		query.append("FileName : MonthlyQuotaAdjustmentTradeRHQDBDAOSearchMonthlyQuotaAdjustmentTradeRhqModify0148ListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("SELECT SUB_TRD_CD, LANE_GRP, RHQ_CD," ).append("\n"); 
		...
		query.append("	MIN(CTRT_RHQ_CD) CTRT_RHQ_CD," ).append("\n"); 
		...
		query.append("    DECODE(TRD.BSE_YR||TRD.BSE_MON, TRD.BSE_YR||@[month1], 'cur'," ).append("\n"); 
		query.append("	TO_CHAR(ADD_MONTHS(TO_DATE(TRD.BSE_YR||@[month2],'yyyymm'),1),'yyyymm'), 'next1'," ).append("\n"); 
		query.append("	TO_CHAR(ADD_MONTHS(TO_DATE(TRD.BSE_YR||@[month3],'yyyymm'),2),'yyyymm'), 'next2') CODE," ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("    RANK() OVER (PARTITION BY TRD.BSE_MON, SUBSTR(VVD.LANE_GRP, 0,7), TRD.CTRT_RHQ_CD ORDER BY SUBSTR(VVD.LANE_GRP, 9))  AS GRP_SEQ," ).append("\n"); 
		query.append("    TRD.SUB_TRD_CD, --hidden" ).append("\n"); 
		query.append("    TRD.RLANE_CD AS RLANE_CD,  --hidden" ).append("\n"); 
		query.append("    TRD.SPRT_GRP_CD AS SPRT_GRP_CD,  --hidden" ).append("\n"); 
		query.append("    TRD.BSA_GRP_CD AS BSA_GRP_CD,  --hidden" ).append("\n"); 
		query.append("    TRD.CTRT_RHQ_CD AS CTRT_RHQ_CD,  --hidden" ).append("\n"); 
		query.append("    TRD.BSE_MON AS BSE_MON, --hidden" ).append("\n"); 
		...
		query.append("    TRD.LOD_QTY / @[unit_flag] AS TOT_LOD ," ).append("\n"); 
		query.append("    TRD.GRS_RPB_REV * @[unit_flag] AS TOT_RPB," ).append("\n"); 
		...
		query.append("WHERE  TRD.MQTA_STEP_CD = INP.MQTA_STEP_CD" ).append("\n"); 
		query.append("AND    TRD.BSE_YR = INP.BSE_YR" ).append("\n"); 
		query.append("AND    TRD.BSE_QTR_CD = INP.BSE_QTR_CD" ).append("\n"); 
		query.append("AND    TRD.TRD_CD = INP.TRD_CD" ).append("\n"); 
		query.append("AND    TRD.DIR_CD = INP.DIR_CD" ).append("\n"); 
		query.append("AND    TRD.MQTA_VER_NO = INP.MQTA_VER_NO" ).append("\n"); 
		query.append("AND    TRD.BSE_MON = VVD.BSE_MON" ).append("\n"); 
		...
		query.append(")" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("GROUP BY SUB_TRD_CD, LANE_GRP, RHQ_CD" ).append("\n"); 
		query.append("ORDER BY SUB_TRD_CD, LANE_GRP, RHQ_CD" ).append("\n"); 

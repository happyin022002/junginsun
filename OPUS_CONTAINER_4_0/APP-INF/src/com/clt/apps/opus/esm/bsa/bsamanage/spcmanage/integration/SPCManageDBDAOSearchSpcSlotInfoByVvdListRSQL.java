...
public class SPCManageDBDAOSearchSpcSlotInfoByVvdListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      NVL(SUM(DECODE(D.CRR_CD, COM_ConstantMgr_PKG.COM_getCompanyCode_FNC, D.CRR_BSA_CAPA)), 0) FNL_CO_BSA_CAPA, " ).append("\n"); 
		...
		query.append("      C.N2ND_FNL_CO_BSA_CAPA, " ).append("\n"); 
		query.append("#foreach( ${keys} in ${keyList})        " ).append("\n"); 
		query.append("	    NVL(SUM(DECODE(D.CRR_CD, '${keys}', D.SPC_CTRL_SLT_CAPA)), 0) S${keys}, " ).append("\n"); 
		query.append("#end " ).append("\n"); 
		query.append("      MAX(DECODE(C.SPC_OTR_SWAP_FLG, 'Y', DECODE(F.BSA_OP_JB_CD, @[rdoopjob], DECODE(F.CRR_CD, COM_ConstantMgr_PKG.COM_getCompanyCode_FNC, '', 'Yes')))) SPC_OTR_SWAP_FLG," ).append("\n"); 
		query.append("#foreach( ${keys} in ${keyList})  " ).append("\n"); 

...
public class CLLCDLManifestDBDAOsearchCllLoadSumByPodRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      NVL(ROUND(SUM(WGT_MT), 0), 1) WGT_MT," ).append("\n"); 
		query.append("	  NVL(ROUND(SUM(VGM_MT), 0), 0) VGM_MT" ).append("\n"); 
		query.append("    FROM (" ).append("\n"); 
		...
		query.append("          SUM(DECODE(NVL(CLL.WGT_UT_CD, 0), 'LBS', ROUND (NVL(CLL.BL_WGT, 0)*0.4536, 3), NVL(CLL.BL_WGT, 0))) WGT_MT," ).append("\n"); 
		query.append("		  SUM(NVL(CLL.VGM_WGT,0)) VGM_MT" ).append("\n"); 
		...
		query.append("        GROUP BY CLL.POD_CD, CLL.KR_CLL_TS_CD, CLL.MTY_BKG_CD, CLL.CNTR_TPSZ_CD " ).append("\n"); 
		query.append("    )" ).append("\n"); 

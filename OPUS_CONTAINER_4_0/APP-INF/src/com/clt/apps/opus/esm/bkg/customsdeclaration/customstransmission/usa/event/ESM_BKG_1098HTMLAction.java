...
public class ESM_BKG_1098HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0253HTMLAction 객체를 생성
	 */
	public ESM_BKG_1098HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1098Event event = new EsmBkg1098Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setBaplieAlarmSetupVOs((BaplieAlarmSetupVO[])getVOs(request, BaplieAlarmSetupVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBaplieAlarmSetupVO((BaplieAlarmSetupVO)getVO(request, BaplieAlarmSetupVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setStrLocCd(request.getParameter("strLocCd"));
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setStrSlanCd(request.getParameter("strSlanCd"));
		}
		else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setStrUsrId(request.getParameter("strUsrId"));
		}

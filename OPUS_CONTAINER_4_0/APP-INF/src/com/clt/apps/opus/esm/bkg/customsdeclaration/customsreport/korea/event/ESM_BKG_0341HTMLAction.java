...
*@FileName : ESM_BKG_0341HTMLAction.java
*@FileTitle : ESM_BKG_0341HTMLAction
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0341HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 2792293631096541043L;

	public ESM_BKG_0341HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		// EVENT 생성
		EsmBkg0341Event event = new EsmBkg0341Event();

		// COMMAND 구분에 따른 처리
		if (command.isCommand(FormCommand.SEARCH)) {
			// 조회의 경우 처리
			KorDischCYCondVO korDischCYCondVO = (KorDischCYCondVO)getVO(request, KorDischCYCondVO.class);
			event.setKorDischCYCondVO(korDischCYCondVO);
		} else  if (command.isCommand(FormCommand.SEARCH03)) {
			KorImpPrintCondVO[] korImpPrintCondVOs = (KorImpPrintCondVO[])getVOs(request, KorImpPrintCondVO.class);
			event.setKorImpPrintCondVOs(korImpPrintCondVOs);
		}

...
*@FileName : MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0127Tab10RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0127Tab10RSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 분기별별 확정된 내용을 Week, Sub Trade,Lane, VVD Group, VVD, RHQ, Area Director,C.Office 별로 Grouping 된 형태로 조회 한다. (주요 table saq_mon_cfm_qta, saq_mon_cfm_tgt_vvd)
	  * </pre>
	  */
	public MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0127Tab10RSQL(){
		setQuery();
		...
		query.append("FileName : MonthlyQuotaInquiryDBDAOSearchMonthlyQuotaInquiry0127Tab10RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	 	SELECT @[rhqCd] OFC_CD, @[year] BSE_YR, @[quarter] BSE_QTR_CD , @[version] VER, @[targetGrp] SAQ_TGT_GRP_CD 											" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("@[month] ,'MM'),0),'MM') MON_0  " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	 		, TO_CHAR(ADD_MONTHS(TO_DATE( " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${month} < 10) " ).append("\n"); 
		query.append("'0' + " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("@[month] ,'MM'),1),'MM') MON_1 " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	 		, TO_CHAR(ADD_MONTHS(TO_DATE( " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if (${month} < 10) " ).append("\n"); 
		query.append("'0' + " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("@[month] ,'MM'),2),'MM') MON_2 														" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	 	FROM DUAL 																					" ).append("\n"); 
		query.append("	 )  , 																							" ).append("\n"); 
		query.append("	 VVD_VIEW AS ( 																						" ).append("\n"); 
		query.append("	 	SELECT VVD.SKD_VOY_NO,VVD.VSL_CD,VVD.SKD_DIR_CD,QTA.RHQ_CD, NVL(QTA.AQ_CD,'000000') AQ_CD, GRP_TRD.SAQ_TGT_GRP_CD ,VVD.BSE_MON 							" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("#if (${isRhq} == 'true')" ).append("\n"); 
		query.append("    AND ( QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.MQTA_RLSE_VER_NO ) = ( SELECT  OFC_CD , BSE_YR, BSE_QTR_CD,VER  FROM PARAMS ) " ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("    AND ( NVL(QTA.AQ_CD, '  '),QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.MQTA_RLSE_VER_NO ) = ( SELECT  OFC_CD, BSE_YR, BSE_QTR_CD,VER  FROM PARAMS )       " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(" 																	" ).append("\n"); 
		query.append("	 ), 																							" ).append("\n"); 
		...
		query.append("	 		    AND INTG_CD_VAL_CTNT NOT IN ('00') 																" ).append("\n"); 
		query.append("	 ) 																							" ).append("\n"); 
		query.append("	  																							" ).append("\n"); 
		...
		query.append("	 		 '08', SUM(DECODE(BSE_MON,P.MON_0,VAL.RA_CM,0))/1000,	   					" ).append("\n"); 
		query.append("	 		 '09', SUM(DECODE(BSE_MON,P.MON_0,DECODE(NVL(VAL.LOD,0),0,0,VAL.CM/(VAL.LOD)),0)),     --수정" ).append("\n"); 
		query.append("	 		 '10', SUM(DECODE(BSE_MON,P.MON_0,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_CM/(VAL.LOD)),0)),  --수정" ).append("\n"); 
		query.append("	 		 '12', SUM(DECODE(BSE_MON,P.MON_0,VAL.RA_OP,0))/1000,                                  		 									" ).append("\n"); 
		query.append("	 		 '14', SUM(DECODE(BSE_MON,P.MON_0,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_OP/(VAL.LOD)),0))), 0) AS CAL_VAL_01  --수정" ).append("\n"); 
		query.append("	 	, NVL(DECODE(ITM.CODE, 																				" ).append("\n"); 
		...
		query.append("	 		 '08', SUM(DECODE(BSE_MON,P.MON_1,VAL.RA_CM,0))/1000,	   					" ).append("\n"); 
		query.append("	 		 '09', SUM(DECODE(BSE_MON,P.MON_1,DECODE(NVL(VAL.LOD,0),0,0,VAL.CM/(VAL.LOD)),0)),     --수정" ).append("\n"); 
		query.append("	 		 '10', SUM(DECODE(BSE_MON,P.MON_1,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_CM/(VAL.LOD)),0)),  --수정" ).append("\n"); 
		query.append("	 		 '12', SUM(DECODE(BSE_MON,P.MON_1,VAL.RA_OP,0))/1000,                                  		 									" ).append("\n"); 
		query.append("	 		 '14', SUM(DECODE(BSE_MON,P.MON_1,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_OP/(VAL.LOD)),0))), 0) AS CAL_VAL_02  --수정" ).append("\n"); 
		query.append("	 	, NVL(DECODE(ITM.CODE, 																				" ).append("\n"); 
		...
		query.append("	 		 '08', SUM(DECODE(BSE_MON,P.MON_2,VAL.RA_CM,0))/1000,	  		  			" ).append("\n"); 
		query.append("	 		 '09', SUM(DECODE(BSE_MON,P.MON_2,DECODE(NVL(VAL.LOD,0),0,0,VAL.CM/(VAL.LOD)),0)),     --수정" ).append("\n"); 
		query.append("	 		 '10', SUM(DECODE(BSE_MON,P.MON_2,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_CM/(VAL.LOD)),0)),  --수정" ).append("\n"); 
		query.append("	 		 '12', SUM(DECODE(BSE_MON,P.MON_2,VAL.RA_OP,0))/1000,                                  		 									" ).append("\n"); 
		query.append("	 		 '14', SUM(DECODE(BSE_MON,P.MON_2,DECODE(NVL(VAL.LOD,0),0,0,VAL.RA_OP/(VAL.LOD)),0))), 0) AS CAL_VAL_03  --수정" ).append("\n"); 
		query.append("	 	, NVL(DECODE(ITM.CODE, 																				" ).append("\n"); 
		...
		query.append("	 		 '08', SUM(VAL.RA_CM)/1000," ).append("\n"); 
		query.append("	 		 '09', DECODE(NVL(SUM(VAL.LOD),0),0,0,SUM(VAL.CM)/SUM(VAL.LOD)),    --수정" ).append("\n"); 
		query.append("	 		 '10', DECODE(NVL(SUM(VAL.LOD),0),0,0,SUM(VAL.RA_CM)/SUM(VAL.LOD)), --수정" ).append("\n"); 
		query.append("	 		 '12', SUM(VAL.RA_OP)/1000,                                  		 									" ).append("\n"); 
		query.append("	 		 '14', DECODE(NVL(SUM(VAL.LOD),0),0,0,SUM(VAL.RA_OP)/SUM(VAL.LOD)),0),0) AS CAL_TOT --수정" ).append("\n"); 
		...
		query.append("	 	FROM ( 																						" ).append("\n"); 
		query.append("	 		SELECT QTA.MQTA_RLSE_VER_NO,QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD BSE_QTR_CD,QTA.BSE_MON,NVL(QTA.AQ_CD,'000000') AQ_CD,GRP_TRD.SAQ_TGT_GRP_CD,QTA.TRD_CD,QTA.CONV_DIR_CD 	" ).append("\n"); 
		query.append("	 			,SUM(QTA.LOD_QTY) AS LOD 																	" ).append("\n"); 
		...
		query.append("	 			 (QTA.MQTA_RLSE_VER_NO,QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.BSE_MON) 											" ).append("\n"); 
		query.append("	 			,(QTA.MQTA_RLSE_VER_NO,QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.BSE_MON,QTA.AQ_CD) 										" ).append("\n"); 
		query.append("	 			,(QTA.MQTA_RLSE_VER_NO,QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.BSE_MON,QTA.AQ_CD,GRP_TRD.SAQ_TGT_GRP_CD) 							" ).append("\n"); 
		query.append("	 			,(QTA.MQTA_RLSE_VER_NO,QTA.RHQ_CD,QTA.BSE_YR,QTA.BSE_QTR_CD,QTA.BSE_MON,QTA.AQ_CD,GRP_TRD.SAQ_TGT_GRP_CD,QTA.TRD_CD,QTA.CONV_DIR_CD) 				" ).append("\n"); 
		query.append("	 		) 																					" ).append("\n"); 

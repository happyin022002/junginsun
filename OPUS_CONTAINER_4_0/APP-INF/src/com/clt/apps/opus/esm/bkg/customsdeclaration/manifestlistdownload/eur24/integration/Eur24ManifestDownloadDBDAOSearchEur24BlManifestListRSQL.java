...
*@FileName : Eur24ManifestDownloadDBDAOSearchEur24BlManifestListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class Eur24ManifestDownloadDBDAOSearchEur24BlManifestListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public Eur24ManifestDownloadDBDAOSearchEur24BlManifestListRSQL(){
		setQuery();
		...
		params.put("p_first_of_multi_pofe_cd",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : Eur24ManifestDownloadDBDAOSearchEur24BlManifestListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("     , COUNT(DISTINCT DECODE(RESULT2,'A',BL_NO,'AL',BL_NO,NULL)) OVER()   AS A_CNT" ).append("\n"); 
		...
		query.append("     , COUNT(DISTINCT DECODE(RESULT2,'CA',BL_NO,NULL)) OVER()  AS CA_CNT" ).append("\n"); 
		query.append("     , COUNT(DISTINCT DECODE(RESULT2,'CC',BL_NO,NULL)) OVER()  AS CC_CNT" ).append("\n"); 
		query.append("     , COUNT(DISTINCT DECODE(RESULT2,'D',BL_NO,NULL)) OVER()  AS D_CNT" ).append("\n"); 
		query.append("     , COUNT(DISTINCT BL_NO) OVER() TTL_BL" ).append("\n"); 
		query.append("     , COUNT(DISTINCT CNTR_CNTR_NO) OVER() TTL_CNTR" ).append("\n"); 
		query.append("/* ENS ETA 가 전송날짜 보다 30일 이상 지난 경우 ENS전송 방지 */" ).append("\n"); 
		query.append(",DECODE( ROWNUM, 1," ).append("\n"); 
		query.append("        (SELECT CASE WHEN NVL(TO_DATE(ZZ.CSTMS_ESTM_ARR_DT,'YYYY-MM-DD HH24:MI'), TO_DATE(ZZ.INIT_ETA_DT,'YYYY-MM-DD HH24:MI')) " ).append("\n"); 
		query.append("                            < GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS',SYSDATE-30,ZZ.EU_1ST_PORT)" ).append("\n"); 
		...
		query.append("   AND CSTMS_DIV_ID = 'ERR_MSG'" ).append("\n"); 
		query.append("   AND DELT_FLG = 'N') AS ETA_ERR_MSG -- ETA 30일이 지나도 전송이 가능하게 하는 것 Customs Common Code 페이지에서 설정가능" ).append("\n"); 
		query.append(",(SELECT ATTR_CTNT2" ).append("\n"); 
		query.append("  FROM BKG_CSTMS_CD_CONV_CTNT" ).append("\n"); 
		query.append("  WHERE CNT_CD = 'EU'" ).append("\n"); 
		query.append("  AND CSTMS_DIV_ID = 'ERR_MSG'" ).append("\n"); 
		query.append("  AND DELT_FLG = 'N')	AS NL_ERR_MSG -- 네덜란드 세관에서 응답이 없어도 전송이 가능하게 하는 것 Customs Common Code 페이지에서 설정가능" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append(" -- Data Delete 관련 " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append(",DECODE( ROWNUM, 1," ).append("\n"); 
		query.append("        ( SELECT CASE WHEN NVL(MAX(ACT.ACT_ARR_DT),TO_DATE(ZZ.VPS_ETA_DT,'YYYY-MM-DD HH24:MI')) <= GLOBALDATE_PKG.TIME_CONV_FNC('KRPUS', SYSDATE, ZZ.EU_1ST_PORT) THEN 'Y'" ).append("\n"); 
		query.append("                      ELSE 'N'" ).append("\n"); 
		...
		query.append("             AND CLPT_IND_SEQ= NVL(@[eu_1st_port_clpt_seq],'1')" ).append("\n"); 
		query.append("             AND ROWNUM = 1)" ).append("\n"); 
		...
		query.append("                                                AND A.MSG_SND_NO    = B.MSG_RCV_NO" ).append("\n"); 
		query.append("                                                AND (ACK_KND_ID != 'S' OR ACK_RCV_STS_CD != 'A') )" ).append("\n"); 
		query.append("                       AND ROWNUM =1),'N') = 'Y'" ).append("\n"); 
		query.append("              OR NVL(( SELECT 'Y'" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append(", NVL(( SELECT 'Y'" ).append("\n"); 
		query.append("         FROM BKG_CSTMS_EUR_BL ENS_BL" ).append("\n"); 
		query.append("        WHERE 1=1" ).append("\n"); 
		query.append("          AND ENS_BL.VSL_CD        = ZZ.VSL_CD" ).append("\n"); 
		query.append("          AND ENS_BL.SKD_VOY_NO    = ZZ.SKD_VOY_NO" ).append("\n"); 
		query.append("          AND ENS_BL.SKD_DIR_CD    = ZZ.SKD_DIR_CD" ).append("\n"); 
		query.append("          AND ENS_BL.CSTMS_PORT_CD = SUBSTR(@[p_first_of_multi_pofe_cd],1,5)" ).append("\n"); 
		query.append("          AND ROWNUM =1 ), 'N') AS DOWN_YN_FIRST_OF_MULTI_POFE" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("  FROM ( SELECT DECODE( INSTR(SH_NM||SH_AD||SH_CT||SH_CN||SH_ZIP||SH_STR||SH_EORI" ).append("\n"); 
		...
		query.append("                        ||BL_PK||BL_WT||CNTR_PK||CNTR_WT||CM_PK||CM_WT||CM_DS||CM_HTS||CNTR_SEAL,'E'),0,'N','Y') AS ERR_YN" ).append("\n"); 
		...
		query.append("           FROM ( SELECT CASE WHEN BL_PK1 = 0 OR BL_PK1 != CNTR_PK_SUM THEN DECODE(BB_CGO_FLG,'Y','N','E') ELSE 'Y' END AS BL_PK" ).append("\n"); 
		...
		query.append("                    FROM ( SELECT SUM(CNTR_PK1) OVER(PARTITION BY BL_NO) AS  CNTR_PK_SUM" ).append("\n"); 
		...
		query.append("                                , CASE WHEN CNTR_PK_LMT_FLG = 'Y' THEN 'E'" ).append("\n"); 
		query.append("                                       ELSE DECODE(CM_PK1,0,DECODE(BB_CGO_FLG,'Y','N','E'),'Y') " ).append("\n"); 
		query.append("                                  END AS CM_PK" ).append("\n"); 
		query.append("                                , DECODE(CM_WT1,0,DECODE(BB_CGO_FLG,'Y','N','E'),'Y') AS CM_WT" ).append("\n"); 
		query.append("                                , XX.*" ).append("\n"); 
		query.append("                             FROM ( SELECT DECODE( ( SELECT BKG_NO FROM BKG_VVD WHERE BKG_NO = X.BKG_NO AND SLAN_CD ='RFS' AND ROWNUM = 1)," ).append("\n"); 
		...
		query.append("                                           AND CNTR_NO = X.CNTR_CNTR_NO" ).append("\n"); 
		query.append("                                           AND PCK_QTY > 99999) AS CNTR_PK_LMT_FLG" ).append("\n"); 
		...
		query.append("                                         , CASE WHEN (DECODE(NVL(OPUSADM.BKG_GET_TOKEN_FNC(CM_DATA,6),0),0,'N','Y') ='N' " ).append("\n"); 
		query.append("                                                      OR NVL(OPUSADM.BKG_GET_TOKEN_FNC(CM_DATA,6),0) != NVL(OPUSADM.BKG_GET_TOKEN_FNC(CM_DATA,7),0) )" ).append("\n"); 
		...
		query.append("                                                ELSE DECODE(NVL(OPUSADM.BKG_GET_TOKEN_FNC(CM_DATA,6),0),0,'N','Y')" ).append("\n"); 
		...
		query.append("                                                WHEN ACK = 'H'                                  THEN 'Hold'       ||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		query.append("                                                WHEN ACK = 'L'                                  THEN 'Release'    ||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		query.append("                                                WHEN ACK = 'CA'                                 THEN 'Add Info.'  ||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		query.append("                                                WHEN ACK = 'AL'                                 THEN 'Accepted with'||CHR(10)||'loading permission'  ||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		query.append("                                                WHEN ACK = 'R' AND RESULT_CD ='398'             THEN 'Cancellation'    ||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		query.append("                                                WHEN ACK = 'D'                                  THEN 'Relase rejection'||DECODE(ACK_KND_ID,'S','(S)')" ).append("\n"); 
		...
		query.append("                                                WHEN ACK = 'H'                                  THEN 'H'" ).append("\n"); 
		query.append("                                                WHEN ACK = 'L'                                  THEN 'L'" ).append("\n"); 
		query.append("                                                WHEN ACK = 'CA'                                 THEN 'CA'" ).append("\n"); 
		query.append("                                                WHEN ACK = 'AL'                                 THEN 'AL'" ).append("\n"); 
		query.append("                                                WHEN ACK = 'R' AND RESULT_CD ='398'             THEN 'CC'" ).append("\n"); 
		...
		query.append("                                            ACK ,ACK_KND_ID ,ACK_RCV_DT ,ACK_RCV_GMT_DT ,BKG_NO ,BL_MS ,BL_NO ,BL_PK1 ,BL_WT1 ,BPOD ,BPOL ,CM_DATA ,HTS_CNT_CD " ).append("\n"); 
		query.append("                                            ,CNEE_AD1 ,CNEE_CN1 ,CNEE_CT1 ,CNEE_EORI1 ,CNEE_NM1 ,CNEE_STR1 ,CNEE_ZIP1 ,CNTR_CNTR_NO ,CNTR_MS ,CNTR_PK1 ,CNTR_SEAL ,CNTR_WT1 " ).append("\n"); 
		query.append("                                            ,CT ,CUST_TO_ORD_FLG ,DEL ,DOWNLOAD_YN ,EDI_MRN ,KTS_SEND_DT,ENS_SEND_DT ,ENS_SEND_GMT_DT ,EU_1ST_PORT ,EU_1ST_PORT_YD_CD ,EU_1ST_PORT_CLPT_SEQ ,LT ,MSG_SND_NO" ).append("\n"); 
		query.append("                                            ,NTFY_AD1 ,NTFY_CN1 ,NTFY_CT1 ,NTFY_EORI1 ,NTFY_NM1 ,NTFY_STR1 ,NTFY_ZIP1 ,POD ,POD_CLPT_IND_SEQ ,POL ,POL_CLPT_IND_SEQ ,RCV ,RESULT_CD" ).append("\n"); 
		query.append("                                            ,SH_AD ,SH_CN ,SH_CT ,SH_EORI ,SH_NM ,SH_STR ,SH_ZIP ,SKD_DIR_CD ,SKD_VOY_NO ,SND_USR_OFC_CD" ).append("\n"); 
		query.append("                                            ,INIT_ETA_DT ,VPS_ETA_DT ,VPS_ETD_DT ,VSL_CD ,VVD ,RCV_MSG" ).append("\n"); 
		query.append("                                            ,EDI_RCV_DT ,EDI_RCV_SEQ ,BB_CGO_FLG ,TRSM_BLCK_FLG ,CSTMS_ESTM_ARR_DT" ).append("\n"); 
		query.append("                                            " ).append("\n"); 
		query.append("                                            FROM ( SELECT BKG_GET_TOKEN_FNC(RCV,3) AS ACK" ).append("\n"); 
		query.append("                                                        , BKG_GET_TOKEN_FNC(RCV,5) AS RESULT_CD" ).append("\n"); 
		query.append("                                                        ,TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT SLS_OFC_CD" ).append("\n"); 
		query.append("                                                       							 				   FROM MDM_LOCATION" ).append("\n"); 
		query.append("                                                                                                   WHERE LOC_CD =  COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC()),TO_DATE(BKG_GET_TOKEN_FNC(RCV,4),'YYYY-MM-DD HH24:MI'), SND_USR_OFC_CD ), 'YYYY-MM-DD HH24:MI')  AS ACK_RCV_DT -- 한국 시간을 Manifest를 전송 한 국가의 시간으로 전환" ).append("\n"); 
		query.append("            											,TO_CHAR(GLOBALDATE_PKG.TIME_CONV_OFC_FNC((SELECT SLS_OFC_CD" ).append("\n"); 
		query.append("                                                          										   FROM MDM_LOCATION" ).append("\n"); 
		query.append("                                                        										   WHERE LOC_CD = COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC()),TO_DATE(BKG_GET_TOKEN_FNC(RCV,4),'YYYY-MM-DD HH24:MI'), 'GMT' ),           'YYYY-MM-DD HH24:MI')  AS ACK_RCV_GMT_DT -- 한국 시간을 GMT 시간으로 전환" ).append("\n"); 
		...
		query.append("                                                     FROM ( SELECT (SELECT EDI_RCV_DT" ).append("\n"); 
		query.append("                                                                           ||','||EDI_RCV_SEQ" ).append("\n"); 
		query.append("                                                                           ||','||ACK_RCV_STS_CD" ).append("\n"); 
		query.append("                                                                           ||','||TO_CHAR(CRE_DT, 'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("                                                                           ||','||EUR_CSTMS_ACK_CD" ).append("\n"); 
		...
		query.append("                                                                                                                                   AND (VSL_CD != KKK.VSL_CD " ).append("\n"); 
		query.append("                                                                                                                                        OR SKD_VOY_NO != KKK.SKD_VOY_NO " ).append("\n"); 
		query.append("                                                                                                                                        OR SKD_DIR_CD != KKK.SKD_DIR_CD) )" ).append("\n"); 
		...
		query.append("                                                                 , BKG_GET_TOKEN_FNC(KKK.SND,8) AS CSTMS_ESTM_ARR_DT" ).append("\n"); 
		query.append("                                                                 , KKK.*" ).append("\n"); 
		query.append("                                                              FROM ( SELECT ( SELECT EBL.MSG_SND_NO" ).append("\n"); 
		query.append("                                                                                     ||','||TO_CHAR(SND.SND_DT, 'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("                                                                                     ||','||TO_CHAR(SND.SND_GDT,'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		query.append("                                                                                     ||','||SND.SND_USR_OFC_CD" ).append("\n"); 
		query.append("                                                                                     ||','||EBL.MVMT_REF_NO" ).append("\n"); 
		query.append("                                                                                     ||','||EBL.BL_NO" ).append("\n"); 
		query.append("                                                                                     ||','||TO_CHAR(SND.CRE_DT,'YYYYMMDDHH24')" ).append("\n"); 
		query.append("                                                                                     ||','||TO_CHAR(EBL.CSTMS_ESTM_ARR_DT,'YYYY-MM-DD HH24:MI')" ).append("\n"); 
		...
		query.append("                                                                       FROM ( SELECT VVD.VSL_CD || VVD.SKD_VOY_NO || VVD.SKD_DIR_CD AS VVD" ).append("\n"); 
		...
		query.append("                                                                                   , VVD.POL_CLPT_IND_SEQ   AS POL_CLPT_IND_SEQ" ).append("\n"); 
		query.append("                                                                                   , VVD.POD_CD             AS POD" ).append("\n"); 
		query.append("                                                                                   , VVD.POD_CLPT_IND_SEQ   AS POD_CLPT_IND_SEQ" ).append("\n"); 
		query.append("                                                                                        " ).append("\n"); 
		...
		query.append("                                                                                   , CASE WHEN CNEE.EORI_NO IS NOT NULL " ).append("\n"); 
		query.append("                                                                                          THEN CASE WHEN REGEXP_INSTR(CNEE.EORI_NO,'^[A-Z]{2}([a-zA-Z0-9]{1,15}$)') > 0 " ).append("\n"); 
		query.append("                                                                                                     AND REGEXP_INSTR(UPPER(CNEE.EORI_NO),'TEST|NONE') < 1" ).append("\n"); 
		query.append("                                                                                                    THEN 'Y' " ).append("\n"); 
		query.append("                                                                                                    ELSE 'VE' " ).append("\n"); 
		query.append("                                                                                                END" ).append("\n"); 
		...
		query.append("                                                                                   , CASE WHEN NTFY.EORI_NO IS NOT NULL " ).append("\n"); 
		query.append("                                                                                          THEN CASE WHEN REGEXP_INSTR(NTFY.EORI_NO,'^[A-Z]{2}([a-zA-Z0-9]{1,15}$)') > 0 " ).append("\n"); 
		query.append("                                                                                                     AND REGEXP_INSTR(UPPER(NTFY.EORI_NO),'TEST|NONE') < 1" ).append("\n"); 
		query.append("                                                                                                    THEN 'Y' " ).append("\n"); 
		query.append("                                                                                                    ELSE 'VE' " ).append("\n"); 
		query.append("                                                                                                END" ).append("\n"); 
		...
		query.append("                                                                                              COUNT(CNTR_MF_SEQ)            /* CARGO SEQ */" ).append("\n"); 
		...
		query.append("                                                                                          AND DELT_FLG = 'N')    AS HTS_CNT_CD" ).append("\n"); 
		query.append("                                                                                   , TO_CHAR(SKD1.VPS_ETD_DT, 'YYYY-MM-DD HH24:MI') AS VPS_ETD_DT" ).append("\n"); 
		query.append("                                                                                   , SUBSTR(@[p_pod_yard_cd],1,5) AS EU_1ST_PORT" ).append("\n"); 
		query.append("                                                                                   , @[p_pod_yard_cd] AS EU_1ST_PORT_YD_CD" ).append("\n"); 
		query.append("                                                                                   , @[eu_1st_port_clpt_seq] AS EU_1ST_PORT_CLPT_SEQ" ).append("\n"); 
		query.append("                                                                                   , TO_CHAR(SKD2.VPS_ETA_DT, 'YYYY-MM-DD HH24:MI') AS VPS_ETA_DT" ).append("\n"); 
		query.append("                                                                                   , TO_CHAR(SKD2.INIT_ETA_DT, 'YYYY-MM-DD HH24:MI') AS INIT_ETA_DT" ).append("\n"); 
		...
		query.append("                                                                                   , VSK_VSL_PORT_SKD SKD1 -- ENS 인 경우 POL / FI 인 경우 pre EU port" ).append("\n"); 
		query.append("                                                                                   , VSK_VSL_PORT_SKD SKD2 -- ENS 인 경우 POFE / FI 인 경우 POD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                                                                                   #if(${p_type} == 'ENS')" ).append("\n"); 
		query.append("                                                                                   , VSK_VSL_PORT_SKD SKD3 -- ENS 인 경우 POD" ).append("\n"); 
		query.append("                                                                                   #end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("                                                                                    /* DO NOT CHECK*/   " ).append("\n"); 
		query.append("                                                                                 #end" ).append("\n"); 
		query.append("                                                                                 " ).append("\n"); 
		...
		query.append("										     기본적으로 Original가 선택되어있어 Ack. Status가 All인지만 체크한다. */" ).append("\n"); 
		query.append("                                           " ).append("\n"); 
		...
		query.append("                                                 OR ACK  = 'AL'" ).append("\n"); 
		query.append("                                           #elseif (${p_ack_status} == 'R')" ).append("\n"); 
		query.append("                                                AND ACK  != 'A' AND ACK  != 'AL' AND ACK IS NOT NULL" ).append("\n"); 
		...
		query.append("                                            ,SH_AD ,SH_CN ,SH_CT ,SH_EORI ,SH_NM ,SH_STR ,SH_ZIP ,SKD_DIR_CD ,SKD_VOY_NO ,SND_USR_OFC_CD" ).append("\n"); 
		query.append("                                            ,INIT_ETA_DT ,VPS_ETA_DT ,VPS_ETD_DT ,VSL_CD ,VVD ,RCV_MSG" ).append("\n"); 
		query.append("                                            ,EDI_RCV_DT ,EDI_RCV_SEQ ,BB_CGO_FLG ,NVL(TRSM_BLCK_FLG, 'Y') ,CSTMS_ESTM_ARR_DT" ).append("\n"); 
		query.append("                                            " ).append("\n"); 
		query.append("                                            FROM ( SELECT M.*" ).append("\n"); 
		...
		query.append("                                                                 , MVMT_REF_NO AS EDI_MRN" ).append("\n"); 
		query.append("                                                                 , NVL(KTS_SEND_DT1,KTS_SEND_DT2) AS KTS_SEND_DT" ).append("\n"); 
		query.append("                                                              FROM ( SELECT 'Y'     AS DOWNLOAD_YN" ).append("\n"); 
		...
		query.append("                                                                          , BL.POL_CLPT_IND_SEQ AS POL_CLPT_IND_SEQ" ).append("\n"); 
		query.append("                                                                          , BL.POD_CD           AS POD" ).append("\n"); 
		query.append("                                                                          , BL.POD_CLPT_IND_SEQ AS POD_CLPT_IND_SEQ" ).append("\n"); 
		query.append("                                                                          " ).append("\n"); 
		...
		query.append("                                                                                 ELSE 'N'" ).append("\n"); 
		query.append("                                                                             END SH_STR" ).append("\n"); 
		query.append("                                                                          , CASE WHEN SHPR.EORI_NO IS NOT NULL " ).append("\n"); 
		query.append("                                                                                 THEN CASE WHEN REGEXP_INSTR(SHPR.EORI_NO,'^[A-Z]{2}([a-zA-Z0-9]{1,15}$)') > 0 " ).append("\n"); 
		query.append("                                                                                            AND REGEXP_INSTR(UPPER(SHPR.EORI_NO),'TEST|NONE') < 1" ).append("\n"); 
		query.append("                                                                                           THEN 'Y' " ).append("\n"); 
		...
		query.append("                                                                                                                           AND CSTMS_PORT_CD  = BL.CSTMS_PORT_CD" ).append("\n"); 
		query.append("                                                                                                                           AND (VSL_CD != BL.VSL_CD " ).append("\n"); 
		query.append("                                                                                                                                OR SKD_VOY_NO != BL.SKD_VOY_NO " ).append("\n"); 
		query.append("                                                                                                                                OR SKD_DIR_CD != BL.SKD_DIR_CD)" ).append("\n"); 
		query.append("                                                                                                                       )" ).append("\n"); 
		query.append("                                                                                              )) AS MVMT_REF_NO" ).append("\n"); 
		...
		query.append("                                                                          , BL.CSTMS_CLPT_IND_SEQ  AS EU_1ST_PORT_CLPT_SEQ" ).append("\n"); 
		query.append("                                                                          " ).append("\n"); 
		...
		query.append("                                                                              WHERE 1=1" ).append("\n"); 
		query.append("                                                                                AND BKV.BKG_NO     = BKG.BKG_NO" ).append("\n"); 
		query.append("                                                                                AND BKV.VSL_CD     = BL.VSL_CD" ).append("\n"); 
		query.append("                                                                                AND BKV.SKD_VOY_NO = BL.SKD_VOY_NO" ).append("\n"); 
		query.append("                                                                                AND BKV.SKD_DIR_CD = BL.SKD_DIR_CD" ).append("\n"); 
		query.append("                                                                                AND BKV.POL_CD     = BL.POL_CD" ).append("\n"); 
		query.append("                                                                                AND BKV.POD_CD     = BL.POD_CD" ).append("\n"); 
		query.append("                                                                            ) AS TRSM_BLCK_FLG" ).append("\n"); 
		query.append("                                                                           " ).append("\n"); 
		...
		query.append("                                                                        " ).append("\n"); 
		query.append("                                                                        #if (${p_pol_cd} != '')" ).append("\n"); 
		query.append("                                                                            AND BL.POL_CD = @[p_pol_cd]" ).append("\n"); 
		query.append("                                                                        #end" ).append("\n"); 
		query.append("                                                                        " ).append("\n"); 
		query.append("                                                                        #if (${p_pol_yard_cd} != '')" ).append("\n"); 
		query.append("                                                                            AND SUBSTR(BL.POL_YD_CD,6) = @[p_pol_yard_cd]" ).append("\n"); 
		query.append("                                                                        #end" ).append("\n"); 
		query.append("                                                                        " ).append("\n"); 
		query.append("                                                                        #if (${p_pod_cd} != '')" ).append("\n"); 
		query.append("                                                                            AND BL.CSTMS_PORT_CD = SUBSTR(@[p_pod_yard_cd],1,5)" ).append("\n"); 
		query.append("                                                                        #end" ).append("\n"); 
		query.append("                                                                        " ).append("\n"); 
		query.append("                                                                        #if (${p_pod_yard_cd} != '')" ).append("\n"); 
		query.append("                                                                            /* DO NOT CHECK*/" ).append("\n"); 
		...
		query.append("                            WHERE PORT_CD = @[p_pod_cd]" ).append("\n"); 
		query.append("                              AND TML_CD IN ('ALL', @[p_pod_yard_cd])" ).append("\n"); 
		query.append("                              AND ROWNUM =1" ).append("\n"); 

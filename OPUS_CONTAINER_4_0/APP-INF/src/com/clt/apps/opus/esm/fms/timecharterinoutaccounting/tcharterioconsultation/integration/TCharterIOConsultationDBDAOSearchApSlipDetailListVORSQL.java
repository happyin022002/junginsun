...
public class TCharterIOConsultationDBDAOSearchApSlipDetailListVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        CASE WHEN B.VSL_CD IS NULL THEN (SELECT MAX(VSL_SLAN_CD) FROM VSK_VSL_SKD VSK WHERE VSK.VSL_CD =SUBSTR(A.FLET_CTRT_NO , 1,4)  AND VSK.N1ST_PORT_BRTH_DT <= A.CRE_DT)" ).append("\n"); 
		query.append("             ELSE (SELECT MAX(VSL_SLAN_CD) FROM VSK_VSL_SKD VSK WHERE VSK.VSL_CD =B.VSL_CD AND VSK.SKD_VOY_NO=B.SKD_VOY_NO AND VSK.SKD_DIR_CD = B.SKD_DIR_CD) " ).append("\n"); 
		query.append("        END AS VSL_SLAN_CD" ).append("\n"); 

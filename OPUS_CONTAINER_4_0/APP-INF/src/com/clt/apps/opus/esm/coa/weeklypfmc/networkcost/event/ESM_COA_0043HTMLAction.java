...
*@FileName : ESM_COA_0043HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.6
 */
public class ESM_COA_0043HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0043HTMLAction 객체를 생성
	 */
	public ESM_COA_0043HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmCoa0043Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmCoa0043Event event = new EsmCoa0043Event();  	
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkCostCommonVO((NetworkCostCommonVO)getVO(request, NetworkCostCommonVO.class));
			event.setCoaOwnVslDlyHirVOs((CoaOwnVslDlyHirVO[])getVOs(request, CoaOwnVslDlyHirVO.class));
			event.getNetworkCostCommonVO().requestToHashMap(request);
		}
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkCostCommonVO((NetworkCostCommonVO)getVO(request, NetworkCostCommonVO.class));

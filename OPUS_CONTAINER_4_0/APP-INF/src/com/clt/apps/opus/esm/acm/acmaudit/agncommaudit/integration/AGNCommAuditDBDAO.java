...
public class AGNCommAuditDBDAO extends DBDAOSupport {
	 ...
	 * @throws DAOException
	 */
	public void rejectAGNCommAudit(List<AGNCommAuditVO> agnCommAuditVOs) throws DAOException {
		int updCnt[] = null;
		try {
			SQLExecuter sqlExe = new SQLExecuter("");
			if (agnCommAuditVOs.size() > 0) {
				updCnt = sqlExe.executeBatch((ISQLTemplate)new AGNCommAuditDBDAORejectAGNCommAuditUSQL(), agnCommAuditVOs, null);
				for (int i = 0; i < updCnt.length; i++) {
					if (updCnt[i]== Statement.EXECUTE_FAILED)
						throw new DAOException("Fail to update No"+ i + " SQL");
				}

...
public class CLLCDLManifestDBDAOsearchKorCllTsListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    MAX(T2.a31) TS_FLG," ).append("\n"); 
		...
		query.append("	MAX(T2.ESIG_CO_NM) ESIG_CO_NM," ).append("\n"); 
		query.append("	MAX(T2.VGM_DOC_TP_CD) VGM_DOC_TP_CD" ).append("\n"); 
		...
		query.append("        DECODE(NVL(A.POD_YD_CD,''),BKG_VVD.POL_YD_CD,'TT','TS') A31," ).append("\n"); 
		...
		query.append("		MAX(BKG_BOOKING.STWG_CD) a54," ).append("\n"); 
		...
		query.append("		MAX((DECODE(SUBSTR(BKG_RF_CGO.CDO_TEMP,1,5) ,NULL,BKG_DG_CGO.IMDG_UN_NO,TRIM(SUBSTR(TO_CHAR(BKG_RF_CGO.CDO_TEMP,'90.90'),1,5)||'C')))) a99," ).append("\n"); 
		query.append("		MAX((DECODE(DECODE(BKG_RF_CGO.CNTR_VENT_TP_CD,'P',BKG_RF_CGO.VENT_RTO||'%', 'C', BKG_RF_CGO.CBM_PER_HR_QTY||' CMH',null),null,BKG_DG_CGO.IMDG_CLSS_CD,DECODE(BKG_RF_CGO.CNTR_VENT_TP_CD,'P',BKG_RF_CGO.VENT_RTO||'%', 'C', BKG_RF_CGO.CBM_PER_HR_QTY||' CMH','')))) a100," ).append("\n"); 
		query.append("		MAX(BKG_DG_CGO.IMDG_SUBS_RSK_LBL_CD1) a58," ).append("\n"); 
		query.append("		MAX(BKG_RF_CGO.CDO_TEMP) a59," ).append("\n"); 
		query.append("		MAX(BKG_BOOKING.BLCK_STWG_CD) BLCK_STWG_CD," ).append("\n"); 
		query.append("		MAX(BKG_CONTAINER.VGM_WGT) VGM_WGT," ).append("\n"); 
		query.append("		MAX(BKG_CONTAINER.VGM_WGT_UT_CD) VGM_WGT_UT_CD," ).append("\n"); 
		query.append("		MAX(DECODE(BKG_CONTAINER.XTER_SNDR_ID,'WEB',(SELECT ESIG_CO_NM " ).append("\n"); 
		query.append("                                            		   FROM BKG_XTER_VGM XTER_VGM" ).append("\n"); 
		query.append("		                                              WHERE XTER_VGM.BKG_NO = BKG_CONTAINER.XTER_VGM_DOC_ID" ).append("\n"); 
		query.append("                                                        AND XTER_VGM.CNTR_NO = BKG_CONTAINER.CNTR_NO " ).append("\n"); 
		query.append("                                                        AND XTER_VGM.VGM_SEQ = BKG_CONTAINER.XTER_VGM_RQST_SEQ" ).append("\n"); 
		query.append("                                                        AND XTER_VGM.USR_ID = BKG_CONTAINER.XTER_VGM_USR_ID" ).append("\n"); 
		...
		query.append("                                                      WHERE XTER_SNDR_ID = BKG_CONTAINER.XTER_SNDR_ID " ).append("\n"); 
		query.append("                                                        AND XTER_VGM_DOC_ID = BKG_CONTAINER.XTER_VGM_DOC_ID " ).append("\n"); 
		query.append("                                                        AND XTER_VGM_RQST_SEQ = BKG_CONTAINER.XTER_VGM_RQST_SEQ " ).append("\n"); 
		query.append("														AND CNTR_NO = BKG_CONTAINER.CNTR_NO" ).append("\n"); 
		query.append("                                                        AND VGM_CUST_CNTC_TP_CD = 'RP'" ).append("\n"); 
		query.append("														AND ROWNUM = 1)" ).append("\n"); 
		query.append("            )) ESIG_CO_NM," ).append("\n"); 
		query.append("        MAX(NVL(DECODE(BKG_CONTAINER.XTER_SNDR_ID,'WEB','SM1'" ).append("\n"); 
		...
		query.append("			BKG_BOOKING.BKG_NO IN (${bkg_no_list}) AND" ).append("\n"); 
		query.append("			BKG_CONTAINER.VGM_WGT > 0 AND" ).append("\n"); 
		query.append("		#end " ).append("\n"); 
		...
		query.append("		( A.POD_CD(+) = 'KRPUS') AND" ).append("\n"); 
		...
		query.append("			BKG_VVD.POL_YD_CD, A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.POD_YD_CD) T2," ).append("\n"); 
		...
		query.append("order by AA, DECODE(BLCK_STWG_CD,'LOC','11','ONE','12','TWO','13','THR','14','FOR','15','FIV','15.5','HOT','16','TRS','17', '18')" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${in_sort_type} == '5' ) " ).append("\n"); 
		query.append("order by AA, POD_CD, DECODE(BLCK_STWG_CD,'LOC','11','ONE','12','TWO','13','THR','14','FOR','15','FIV','15.5','HOT','16','TRS','17', '18'), CNTR_TPSZ_CD, CNTR_NO" ).append("\n"); 
		query.append("#end" ).append("\n"); 

 ...
 *@FileName : UI_PRI_0001HTMLAction.java
...
public class ESM_PRI_0001_02HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * UI_PRI_0001HTMLAction 객체를 생성
	 */
	public ESM_PRI_0001_02HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri000102Event event = new EsmPri000102Event();

		if (command.isCommand(FormCommand.MULTI01)) {
			GrpLocGlineVO vo = new GrpLocGlineVO();
			vo.setPriSgGrpLocVOS((PriSgGrpLocVO[]) getVOs(request, PriSgGrpLocVO.class, "sheet1_"));
			vo.setPriSgGrpLocDtlVOS((PriSgGrpLocDtlVO[]) getVOs(request, PriSgGrpLocDtlVO.class, "sheet2_"));
			event.setGrpLocGlineVO(vo);
		} else if (command.isCommand(FormCommand.SEARCH01)) {
			event.setPriSgGrpLocVO((PriSgGrpLocVO) getVO(request, PriSgGrpLocVO.class));
		} else if (command.isCommand(FormCommand.SEARCH02)) {
			event.setPriSgGrpLocDtlVO((PriSgGrpLocDtlVO) getVO(request, PriSgGrpLocDtlVO.class));

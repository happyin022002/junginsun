...
public class ESM_COA_4014HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0157HTMLAction 객체를 생성
	 */
	public ESM_COA_4014HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmCoa4014Event event = new EsmCoa4014Event();
				
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO .class));
			event.setAvgAgencyCommissionVOS((AvgAgencyCommissionVO[])getVOs(request, AvgAgencyCommissionVO.class,"t1sheet1_"));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO .class));
			event.setAvgAgencyCommissionVOS((AvgAgencyCommissionVO[])getVOs(request, AvgAgencyCommissionVO.class,"t2sheet1_"));
		}

...
import com.clt.apps.opus.esm.bkg.customsdeclaration.manifestlistdownload.usa.vo.UsaCntrManifestCondVO;
 ...
 * @since J2EE 1.4
 */
public class UsaManifestListDownloadBCImpl extends BasicCommandSupport implements UsaManifestListDownloadBC {
...
//			usaBlCondVO.setBlNo(usaBlCondVO.getBlNos());
	...
	public List<ManifestListDetailVO> searchManifestList(ManifestListCondVO manifestListCondVO, SignOnUserAccount account) throws EventException {
		try {
			...
			condVO.setUsrId(account.getUpd_usr_id());
			String sheetId = condVO.getSheetId();
			if("sheet0".equals(sheetId)){ // 상단 그리드 조회
				List<UsaDownloadSummaryVO> list = dbDao.searchDownloadSummaryList(condVO, account);
	...
	public String manageManifest(ManifestListDetailVO[] manifestListDetailVO, SignOnUserAccount account) throws EventException {
		String sResult = "";
		try {
			if ("ESM_BKG_0028".equals(manifestListDetailVO[0].getPgmNo())) {
			...
			} else if ("ESM_BKG_0028_DN".equals(manifestListDetailVO[0].getPgmNo()) || "ESM_BKG_0568".equals(manifestListDetailVO[0].getPgmNo())) {
				...
				sResult = usaBackEndJob.doStart();

			} else {
				// ESM_BKG_0210 - BackEndJob
				UsaManifestListDownloadBackEndJob usaBackEndJob = new UsaManifestListDownloadBackEndJob();
				usaBackEndJob.setManifestListDetailVO(manifestListDetailVO, account, dbDao);
				BackEndJobManager backEndJobManager = new BackEndJobManager();
				sResult = backEndJobManager.execute(usaBackEndJob, account.getUsr_id(), "USA Data Download (D/L)");
			}
	...
	public String deleteManifest(ManifestListDetailVO[] manifestListDetailVO) throws EventException {
		...
		try{
			for(int i=0 ; i < manifestListDetailVO.length ; i++) {
				UsaManifestListDetailVO usaManifestListDetailVO = (UsaManifestListDetailVO) manifestListDetailVO[i];
				usaManifestListDetailVO.setCntCd("US");
				dbDao.removeBKGAdvManifestCntrUS(usaManifestListDetailVO);
				dbDao.removeBKGAdvManifestCustUS(usaManifestListDetailVO);
				dbDao.removeBKGAdvManifestSealNoUS(usaManifestListDetailVO);
				dbDao.removeBKGAdvManifestCntrMfUS(usaManifestListDetailVO);
				dbDao.removeBKGAdvManifestIbdUS(usaManifestListDetailVO);
		}
	...
	public String deleteManifestBl(ManifestListDetailVO[] manifestListDetailVO) throws EventException {
		...
		try{
			for(int i=0 ; i < manifestListDetailVO.length ; i++) {
				UsaManifestListDetailVO usaManifestListDetailVO = (UsaManifestListDetailVO) manifestListDetailVO[i];
				usaManifestListDetailVO.setCntCd("US");
				dbDao.removeBKGAdvManifestBlUS(usaManifestListDetailVO);
			}
		}
		 catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
...
//						}
			...
			for(int i = 0; i < inBondNumberVO.length; i++){
				...
				if("".equals(nextSeqNo)){
					...
					**********************************************************************/
					...
					BkgReferenceNoGenerationVO genVo = command2.manageBkgReferenceNumberGeneration("UIT", inBondNumberVO[i].getPod(), inBondNumberVO[i].getUserId());
	...
	public void updateCntrArrExpByBlCntr(String blNo, String cntrNo, String transGubun, String arrDt, String arrTime) throws EventException {
		...
		try {
			dbDao.updateCntrArrExpByBlCntr(blNo, cntrNo, transGubun, arrDt, arrTime);
		} catch (DAOException de) {
			log.error("err " + de.toString(), de);
			throw new EventException(new ErrorHandler("BKG06087", new String[] {}).getMessage(), de);
		} catch (Exception e) {

...
*@FileName : RFARateGuidelineDBDAORsltRtListVerticalExcelVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateGuidelineDBDAORsltRtListVerticalExcelVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public RFARateGuidelineDBDAORsltRtListVerticalExcelVORSQL(){
		setQuery();
		...
		query.append("FileName : RFARateGuidelineDBDAORsltRtListVerticalExcelVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                       LPAD(ROW_NUMBER() OVER(PARTITION BY T.SVC_SCP_CD,T.GLINE_SEQ,T.CMDT_HDR_SEQ" ).append("\n"); 
		query.append("                                              ORDER BY DECODE(T.PRC_CMDT_TP_CD, 'G', 1, 'R', 2, 'C', 3, 99)" ).append("\n"); 
		query.append("                                ,T.PRC_CMDT_DEF_CD)" ).append("\n"); 
		...
		query.append("                                      ,5" ).append("\n"); 
		query.append("                                      ,'0') AS ROUT_ROWKEY" ).append("\n"); 
		query.append("                             FROM PRI_RG_RT" ).append("\n"); 
		query.append("                            WHERE 1=1" ).append("\n"); 
		query.append("                              AND SVC_SCP_CD = @[svc_scp_cd]" ).append("\n"); 
		query.append("                              AND GLINE_SEQ = @[gline_seq]" ).append("\n"); 
		query.append("                          ) S" ).append("\n"); 
		...
		query.append("                                  LPAD(ROW_NUMBER() OVER(PARTITION BY T.SVC_SCP_CD,T.GLINE_SEQ,T.CMDT_HDR_SEQ,T.ROUT_SEQ " ).append("\n"); 
		query.append("                                                         ORDER BY DECODE(T.ROUT_PNT_LOC_TP_CD, 'G', 1, 'L', 2, 99)" ).append("\n"); 
		query.append("                                           ,T.ROUT_PNT_LOC_DEF_CD)" ).append("\n"); 
		query.append("                                      ,5" ).append("\n"); 
		query.append("                                      ,'0') AS ROUT_ROWKEY" ).append("\n"); 
		...
		query.append("                                 ,LPAD(T.CMDT_HDR_SEQ, 5, '0') || '|' || LPAD(T.ROUT_SEQ, 5, '0') || '|' ||" ).append("\n"); 
		query.append("                                  LPAD(ROW_NUMBER() OVER(PARTITION BY T.SVC_SCP_CD,T.GLINE_SEQ,T.CMDT_HDR_SEQ,T.ROUT_SEQ " ).append("\n"); 
		query.append("                                                         ORDER BY DECODE(T.ROUT_VIA_PORT_TP_CD, 'G', 1, 'L', 2, 99)" ).append("\n"); 
		query.append("                                           ,T.ROUT_VIA_PORT_DEF_CD)" ).append("\n"); 
		query.append("                                      ,5" ).append("\n"); 
		query.append("                                      ,'0') AS ROUT_ROWKEY" ).append("\n"); 

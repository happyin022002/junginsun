...
public class GeneralBookingSearchDBDAOsearchTmnl301CntrAwkInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	|| 'CNTN:'					|| RPAD(CNTR.CNTR_NO, 11, ' ')		|| CHR(10)" ).append("\n"); 
		...
		query.append("                                                  WHERE SPC_TP = 'AWK'" ).append("\n"); 
		query.append("                                                  AND SPC.TB_ROW_ID = AK.ROWID" ).append("\n"); 
		...
		query.append("    || 'NWGT_QTY:'              || CASE WHEN NVL(AK.NET_WGT,0) = 0 THEN ROUND((SELECT BLD.ACT_WGT/NVL((SELECT SUM(BQ.OP_CNTR_QTY) FROM BKG_QUANTITY BQ WHERE BQ.BKG_NO = BLD.BKG_NO),1) FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO),3)" ).append("\n"); 
		query.append("                                        ELSE AK.NET_WGT END 					|| CHR(10)" ).append("\n"); 
		query.append("	|| 'NWGT_TP:'               || CASE WHEN AK.WGT_UT_CD IS NULL THEN (SELECT BLD.WGT_UT_CD FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO)" ).append("\n"); 
		query.append("                                        ELSE AK.WGT_UT_CD END 				    || CHR(10)" ).append("\n"); 
		...
		query.append("    || 'AK_UNIT:'				|| 'M'											|| CHR(10)" ).append("\n"); 
		...
		query.append("                 AND (BK.AWK_CGO_FLG = 'Y' AND BHCC.HRD_CDG_ID = 'VNDR301_INSTRUCT' AND BHCC.ATTR_CTNT1 = AK.CNTR_TPSZ_CD)" ).append("\n"); 

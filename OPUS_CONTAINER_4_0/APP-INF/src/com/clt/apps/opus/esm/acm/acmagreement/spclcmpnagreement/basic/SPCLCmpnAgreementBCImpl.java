...
import com.clt.apps.opus.esm.acm.acmagreement.spclcmpnagreement.integration.SPCLCmpnAgreementDBDAO;
...
public class SPCLCmpnAgreementBCImpl extends BasicCommandSupport implements SPCLCmpnAgreementBC {
	...
	public List<SCompAgreementVO> searchSCompAgreementDup(SCompAgreementVO[] scompAgreementVOs) throws EventException {
		List<SCompAgreementVO> dupVOs = new ArrayList<SCompAgreementVO>();

		try {
			for (int i=0; i<scompAgreementVOs.length; i++) {			
				if (scompAgreementVOs[i].getIbflag().equals("I")) {
					List<SCompAgreementVO> dupList = dbDao.searchSCompAgreementDup(scompAgreementVOs[i]);
					if (dupList.size() > 0) {
						dupVOs.add(scompAgreementVOs[i]);
					}
				}
			}
		} catch (DAOException ex) {
...
//			List<SCompAgreementVO> insertVoList = new ArrayList<SCompAgreementVO>();
			...
			for (int i=0; i<scompAgreementVOs.length; i++) {
				...
				if (scompAgreementVOs[i].getIbflag().equals("I")) {
					dbDao.addSCompAgreement(scompAgreementVOs[i]);

...
*@FileName : AGTAuditDBDAOSearchApPayInvRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AGTAuditDBDAOSearchApPayInvRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * SearchApPayInv 정보를 가져옴
	  * </pre>
	  */
	public AGTAuditDBDAOSearchApPayInvRSQL(){
		setQuery();
		...
		query.append("FileName : AGTAuditDBDAOSearchApPayInvRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("AA.AGN_CD                           AS INV_OFC_CD," ).append("\n"); 
		query.append("AA.AP_OFC_CD                        AS COST_OFC_CD," ).append("\n"); 
		query.append("AA.VNDR_SEQ                         AS VNDR_SEQ," ).append("\n"); 
		query.append("@[inv_dt]                           AS INV_EFF_DT," ).append("\n"); 
		query.append("to_char(sysdate, 'YYYYMMDD')        AS INV_RCV_DT," ).append("\n"); 
		query.append("AA.CURR_CD                          AS INV_CURR_CD," ).append("\n"); 
		query.append("AA.CSR_AMT                          AS INV_TTL_AMT," ).append("\n"); 
		query.append("(AA.CSR_AMT - AA.VAT_AMT)           AS INV_NET_AMT," ).append("\n"); 
		query.append("AA.VAT_AMT                          AS INV_VAT_AMT," ).append("\n"); 
		query.append("AA.INV_DESC                         AS INV_RMK," ).append("\n"); 
		query.append("AA.CSR_NO                           AS CSR_NO," ).append("\n"); 
		query.append("@[cre_usr_id]                       AS CRE_USR_ID," ).append("\n"); 
		query.append("SYSDATE                             AS CRE_DT," ).append("\n"); 
		query.append("@[cre_usr_id]                       AS UPD_USR_ID," ).append("\n"); 
		query.append("SYSDATE                             AS UPD_DT," ).append("\n"); 
		query.append("AA.GL_DT		                    AS GL_DT," ).append("\n"); 
		query.append("to_char(sysdate, 'YYYYMMDD')        AS INV_ISS_DT," ).append("\n"); 
		query.append("to_char(sysdate, 'YYYYMMDD')        AS PAY_DUE_DT," ).append("\n"); 
		query.append("@[comm_apro_no]					    AS INV_NO," ).append("\n"); 
		query.append("(select GEN_PAY_TERM_CD  from mdm_vendor where VNDR_SEQ = aa.vndr_seq) as VNDR_TERM_NM" ).append("\n"); 
		...
		query.append("|| B.LOCAL_DT" ).append("\n"); 
		query.append("|| SUBSTR (TO_CHAR (SYSDATE, 'HH24MISS'), 1, 5)" ).append("\n"); 
		...
		query.append("THEN 0" ).append("\n"); 
		query.append("WHEN A.CURR_CD = 'JPY'" ).append("\n"); 
		query.append("THEN ROUND (SUM(A.ACT_IF_LOCL_COMM_AMT*NVL (@[inv_tax_rt],0)/100),0)" ).append("\n"); 
		query.append("WHEN A.CURR_CD = 'TWD'" ).append("\n"); 
		query.append("THEN ROUND (SUM(A.ACT_IF_LOCL_COMM_AMT*NVL (@[inv_tax_rt],0)/100),0)" ).append("\n"); 
		query.append("ELSE ROUND (SUM(A.ACT_IF_LOCL_COMM_AMT*NVL (@[inv_tax_rt],0)/100),2)" ).append("\n"); 
		query.append("END ) AS VAT_AMT" ).append("\n"); 

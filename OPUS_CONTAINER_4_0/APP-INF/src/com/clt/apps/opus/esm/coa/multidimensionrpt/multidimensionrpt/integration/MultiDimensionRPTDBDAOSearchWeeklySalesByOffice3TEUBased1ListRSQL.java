...
*@FileName : MultiDimensionRPTDBDAOSearchWeeklySalesByOffice3TEUBased1ListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MultiDimensionRPTDBDAOSearchWeeklySalesByOffice3TEUBased1ListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public MultiDimensionRPTDBDAOSearchWeeklySalesByOffice3TEUBased1ListRSQL(){
		setQuery();
		...
		params.put("f_locl_lang",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : MultiDimensionRPTDBDAOSearchWeeklySalesByOffice3TEUBased1ListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	 DECODE(GROUPING(RPT_ITM_DESC), 1, '1', '2') AS LV " ).append("\n"); 
		...
		query.append("		,SUM(DIR_E_AMT$velocityCount) AS DIR_E_AMT$velocityCount" ).append("\n"); 
		query.append("		,SUM(DIR_W_AMT$velocityCount) AS DIR_W_AMT$velocityCount" ).append("\n"); 
		...
		query.append("		" ).append("\n"); 
		query.append("		#foreach($key in ${allcols}) " ).append("\n"); 
		query.append("			,SUM(CASE WHEN (C2.TRD_CD = '$key' AND C2.DIR_CD IN ('E','S')) THEN NVL (C1.AMT, 0) ELSE 0 END) AS DIR_E_AMT$velocityCount" ).append("\n"); 
		query.append("			,SUM(CASE WHEN (C2.TRD_CD = '$key' AND C2.DIR_CD IN ('W','N')) THEN NVL (C1.AMT, 0) ELSE 0 END) AS DIR_W_AMT$velocityCount" ).append("\n"); 
		query.append("			,SUM(CASE WHEN  C2.TRD_CD = '$key' THEN NVL (C1.AMT, 0) ELSE 0 END) AS TRD_AMT$velocityCount" ).append("\n"); 
		query.append("		#end " ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		,SUM(NVL(C1.AMT, 0)) AS TTL_AMT " ).append("\n"); 
		query.append("	FROM ( " ).append("\n"); 
		...
		query.append("		    #end       " ).append("\n"); 
		query.append("		              " ).append("\n"); 
		query.append("			AND B3.RPT_VW_CD = 'P'" ).append("\n"); 
		...
		query.append("			#end" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		 GROUP BY B1.TRD_CD, B1.DIR_CD, B2.STND_COST_CD " ).append("\n"); 
		...
		query.append("			   ,9,(INCOME_TOTAL - DEM_DET) - (CM_COST + SPC_CHTR)    /* CMCTOTAL (CM(Net Revenue) Total) */" ).append("\n"); 
		query.append("			   ,10, DECODE (LOAD_TEU ,0, 0 ,((INCOME_TOTAL - DEM_DET) - (CM_COST + SPC_CHTR)) / LOAD_TEU )  /* CMB00000 (CM(Net Revenue)/TEU) */ " ).append("\n"); 
		...
		query.append("			   ,18, 0   /* BCMTOTAL (Branch CM) */" ).append("\n"); 
		...
		query.append("					           WHEN A3.STND_COST_CD IN ('43201011') THEN " ).append("\n"); 
		query.append("					           		DECODE (@[f_pro_vw]" ).append("\n"); 
		query.append("	                                       ,'P', DECODE(@[f_pro_lvl]||'|'||@[f_chtbiz],'O|CHT', A2.CO_AMT,'O|BIZ', A2.CO_SLS_AMT,A2.ESTM_PL_AMT) " ).append("\n"); 
		...
		query.append("					#end	" ).append("\n"); 
		query.append("		                                       " ).append("\n"); 
		query.append("				GROUP BY a1.trd_cd, a1.dir_cd " ).append("\n"); 
		query.append("					) " ).append("\n"); 
		...
		query.append("					,A1.DIR_CD " ).append("\n"); 
		query.append("					,DECODE (@[f_locl_lang], '1', A2.SGRP_LOCL_DESC, A2.SGRP_COST_CD_DESC) SGRP_COST_CD_DESC   /*조건:지역언어 표시 여부 //f_locl_lang */" ).append("\n"); 
		query.append("					,DECODE (@[f_locl_lang], '1', A2.LOCL_RPT_ITM_DESC, A2.RPT_ITM_DESC) RPT_ITM_DESC " ).append("\n"); 
		...
		query.append("					,COA_PFIT_LSS_RPT_ITM A2 " ).append("\n"); 
		query.append("				WHERE A2.RPT_VW_CD       = @[f_pro_vw] " ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("				#if(${f_pro_vw} =='P' && ${f_pro_lvl} =='O')" ).append("\n"); 
		query.append("					AND (a2.stnd_cost_tp_cd IN ('S','C','O',@[str_display])  /* f_pro_obj P->O,A,B */" ).append("\n"); 
		query.append("						OR a2.stnd_cost_cd IN ('OPCTOTAL','OPB00000','BOPTOTAL','BOPB0000')) " ).append("\n"); 
		query.append("				#else" ).append("\n"); 
		query.append("					#if( ${f_pro_lvl} =='C')" ).append("\n"); 
		query.append("						AND A2.STND_COST_TP_CD IN ('S','C') " ).append("\n"); 
		query.append("					#elseif (${f_pro_lvl} =='O')" ).append("\n"); 
		query.append("						AND A2.STND_COST_TP_CD IN ('S','C','O') " ).append("\n"); 
		query.append("					#end" ).append("\n"); 
		query.append("				#end	" ).append("\n"); 
		query.append("" ).append("\n"); 

/*=========================================================
*Copyright(c) 2009 CyberLogitec
*@FileName : EBookingReceiptDBDAOcopyBkgXterCntrSealNoCSQL.java
*@FileTitle :
*Open Issues :
*Change history :
*@LastModifyDate : 2009.06.04
*@LastModifier : 전용진
*@LastVersion : 1.0
* 2009.06.04 전용진
* 1.0 Creation
=========================================================*/
package com.clt.apps.opus.esm.bkg.bookingconduct.ebookingconduct.ebookingreceipt.integration;

import java.util.HashMap;
import org.apache.log4j.Logger;

import com.clt.framework.support.db.ISQLTemplate;

/**
 *
 * @author Jun Yong Jin
 * @see DAO 참조
 * @since J2EE 1.6
 */

public class EBookingReceiptDBDAOcopyBkgXterCntrSealNoCSQL implements ISQLTemplate{

	private StringBuffer query = new StringBuffer();

	Logger log =Logger.getLogger(this.getClass());

	/** Parameters definition in params/param elements */
	private HashMap<String,String[]> params = null;

	/**
	  * <pre>
	  * copyBkgXterCntrSealNo
	  * </pre>
	  */
	public EBookingReceiptDBDAOcopyBkgXterCntrSealNoCSQL(){
		setQuery();

		params = new HashMap<String,String[]>();
		String tmp = null;
		String[] arrTmp = null;
		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("rqst_no",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("rqst_seq",new String[]{arrTmp[0],arrTmp[1]});

		tmp = java.sql.Types.VARCHAR + ",N";
		arrTmp = tmp.split(",");
		if(arrTmp.length !=2){
			throw new IllegalArgumentException();
		}
		params.put("sender_id",new String[]{arrTmp[0],arrTmp[1]});

	}

	public String getSQL(){
		return query.toString();
	}

	public HashMap<String,String[]> getParams() {
		return params;
	}

	/**
	 * Query 생성
	 */
	public void setQuery(){
		query.append("INSERT INTO bkg_xter_cntr_seal_no" ).append("\n");
		query.append("(xter_sndr_id,  xter_rqst_no,  xter_rqst_seq" ).append("\n");
		query.append(", CNTR_NO, cntr_seal_seq" ).append("\n");
		query.append(", cntr_seal_no" ).append("\n");
		query.append(", seal_pty_nm" ).append("\n");
		query.append(", prn_flg)" ).append("\n");
		query.append("SELECT" ).append("\n");
		query.append("xter_sndr_id,  xter_rqst_no,  xter_rqst_seq+1" ).append("\n");
		query.append(", CNTR_NO, cntr_seal_seq" ).append("\n");
		query.append(", cntr_seal_no" ).append("\n");
		query.append(", seal_pty_nm" ).append("\n");
		query.append(", prn_flg" ).append("\n");
		query.append("FROM	bkg_xter_cntr_seal_no" ).append("\n");
		query.append("WHERE	xter_rqst_no    = @[rqst_no]" ).append("\n");
		query.append("AND	xter_rqst_seq  = @[rqst_seq]" ).append("\n");
		query.append("AND     xter_sndr_id    = @[sender_id]" ).append("\n");

		query.append("/*").append("\n");
		query.append("Path : com.clt.apps.opus.esm.bkg.bookingconduct.ebookingconduct.ebookingreceipt.integration").append("\n");
		query.append("FileName : EBookingReceiptDBDAOcopyBkgXterCntrSealNoCSQL").append("\n");
		query.append("*/").append("\n");
	}
}
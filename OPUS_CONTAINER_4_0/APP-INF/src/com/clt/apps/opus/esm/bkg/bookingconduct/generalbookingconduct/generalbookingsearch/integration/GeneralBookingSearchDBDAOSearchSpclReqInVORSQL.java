...
public class GeneralBookingSearchDBDAOSearchSpclReqInVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("select 'R' apro_cd, 0 dcgo_seq, nvl(rc_seq, 0) rc_seq, 0 awk_cgo_seq, 0 bb_cgo_seq, 0 stwg_cgo_seq" ).append("\n"); 
		query.append("  from bkg_rf_cgo" ).append("\n"); 
		query.append(" where bkg_no = @[bkg_no]" ).append("\n"); 
		query.append("   and SPCL_CGO_APRO_CD in ('R', 'Y', 'P')" ).append("\n"); 
		query.append("union all" ).append("\n"); 
		query.append("select 'R' apro_cd, 0  dcgo_seq, 0 rc_seq, nvl(awk_cgo_seq, 0) awk_cgo_seq, 0 bb_cgo_seq, 0 stwg_cgo_seq" ).append("\n"); 
		query.append("  from bkg_awk_cgo" ).append("\n"); 
		query.append(" where bkg_no = @[bkg_no]" ).append("\n"); 
		query.append("   and SPCL_CGO_APRO_CD in ('R', 'Y', 'P')" ).append("\n"); 
		query.append("union all" ).append("\n"); 
		query.append("select 'R' apro_cd, 0 dcgo_seq, 0 rc_seq, 0 awk_cgo_seq, nvl(bb_cgo_seq, 0) bb_cgo_seq, 0 stwg_cgo_seq" ).append("\n"); 
		query.append("  from bkg_bb_cgo" ).append("\n"); 
		query.append(" where bkg_no = @[bkg_no]" ).append("\n"); 
		query.append("   and SPCL_CGO_APRO_CD in ('R', 'Y', 'P')" ).append("\n"); 
		query.append("union all" ).append("\n"); 
		query.append("select 'R' apro_cd, 0 dcgo_seq, 0 rc_seq, 0 awk_cgo_seq, 0 bb_cgo_seq, nvl(stwg_seq, 0) stwg_cgo_seq" ).append("\n"); 

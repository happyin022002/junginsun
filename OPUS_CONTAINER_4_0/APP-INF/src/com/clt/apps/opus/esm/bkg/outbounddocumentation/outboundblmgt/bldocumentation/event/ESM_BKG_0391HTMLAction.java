...
*@FileName : ESM_BKG_0391HTMLAction.java
...
public class ESM_BKG_0391HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0391HTMLAction 객체를 생성
	 */
	public ESM_BKG_0391HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBkg0391Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0391Event event = new EsmBkg0391Event();
        log.debug("::CALL::> ESM_BKG_0391HTMLAction - " + command.getCommand());

        if(command.isCommand(FormCommand.SEARCH)) {
            
            String bkgNo      = JSPUtil.getParameter(request, "bkg_no");
            BkgCntrShpVO bkgCntrShpVO = new BkgCntrShpVO();
            bkgCntrShpVO.setBkgNo(bkgNo);
            event.setBkgCntrShpVO(bkgCntrShpVO);
                        
		}else if(command.isCommand(FormCommand.MULTI)) {
            String bkgNo      = JSPUtil.getParameter(request, "bkg_no");
			BkgCntrShpVO[] bkgCntrShpVOs = (BkgCntrShpVO[]) getVOs(request, BkgCntrShpVO.class);
			event.setBkgCntrShpVOs(bkgCntrShpVOs);

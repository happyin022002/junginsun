...
*@FileName : ESM_BKG_0236HTMLAction.java
...
public class ESM_BKG_0236HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0236HTMLAction 객체를 생성
	 */
	public ESM_BKG_0236HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmBkg0236Event event = new EsmBkg0236Event();


		if(command.isCommand(FormCommand.DEFAULT)) {
			event.setLogHdrVO((BkgLodFctrDlLogHdrVO)getVO(request, BkgLodFctrDlLogHdrVO .class));
		} else if(command.isCommand(FormCommand.SEARCH)) {
			event.setLdfDt(request.getParameter("ldf_dt"));
			event.setOfcCd(request.getParameter("ofc_cd"));
			event.setLogHdrVO((BkgLodFctrDlLogHdrVO)getVO(request, BkgLodFctrDlLogHdrVO .class));
		} else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setLdfDt(request.getParameter("ldf_dl_dt"));
			event.setOfcCd(request.getParameter("bkg_ofc_cd"));
			event.setLogHdrVO((BkgLodFctrDlLogHdrVO)getVO(request, BkgLodFctrDlLogHdrVO .class));
		} else if(command.isCommand(FormCommand.MULTI01)) {
			event.setLdfDt(request.getParameter("ldf_dl_dt"));
			event.setOfcCd(request.getParameter("bkg_ofc_cd"));
			event.setFromDt(request.getParameter("bkg_fm_dt"));
			event.setToDt(request.getParameter("bkg_to_dt"));
			event.setFileNm(request.getParameter("file_dl_nm"));
			event.setLogHdrVO((BkgLodFctrDlLogHdrVO)getVO(request, BkgLodFctrDlLogHdrVO .class));
		}

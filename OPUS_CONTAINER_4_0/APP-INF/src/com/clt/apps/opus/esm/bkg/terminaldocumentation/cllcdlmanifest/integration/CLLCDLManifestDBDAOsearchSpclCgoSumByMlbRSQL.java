...
public class CLLCDLManifestDBDAOsearchSpclCgoSumByMlbRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    SELECT BLCK_STWG_CD GUBUN_CD," ).append("\n"); 
		...
		query.append("    FROM (" ).append("\n"); 
		query.append("        SELECT DECODE(BLCK_STWG_CD, NULL, POD_CD, BLCK_STWG_CD) BLCK_STWG_CD," ).append("\n"); 
		...
		query.append("        GROUP BY BLCK_STWG_CD, POD_CD, KR_CLL_TS_CD, MTY_BKG_CD, CLL.CNTR_TPSZ_CD, DG.CNTR_NO, CLL_RMK2, KR_TML_PRCT_ID " ).append("\n"); 
		query.append("    )" ).append("\n"); 

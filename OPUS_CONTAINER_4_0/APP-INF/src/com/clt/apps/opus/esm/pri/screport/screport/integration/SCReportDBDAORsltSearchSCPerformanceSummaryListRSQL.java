...
*@FileName : SCReportDBDAORsltSearchSCPerformanceSummaryListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SCReportDBDAORsltSearchSCPerformanceSummaryListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * s/c performance summary - non by scope
	  * </pre>
	  */
	public SCReportDBDAORsltSearchSCPerformanceSummaryListRSQL(){
		setQuery();
		...
		query.append("FileName : SCReportDBDAORsltSearchSCPerformanceSummaryListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                        ( SELECT A.OFC_CD FROM MDM_ORGANIZATION A WHERE A.OFC_TP_CD = 'HQ' START WITH A.OFC_CD = SM.RESPB_SLS_OFC_CD CONNECT BY PRIOR A.PRNT_OFC_CD = A.OFC_CD ) RHQ_CD ," ).append("\n"); 
		...
		query.append("                        DECODE(MQ.CNTR_LOD_UT_CD, 'T', MQ.FNL_MQC_QTY / 2, MQ.FNL_MQC_QTY)    FNL_MQC_QTY         ," ).append("\n"); 
		...
		query.append("                AND     ( SELECT A.OFC_CD FROM MDM_ORGANIZATION A WHERE A.OFC_TP_CD = 'HQ' START WITH A.OFC_CD = SM.RESPB_SLS_OFC_CD CONNECT BY PRIOR A.PRNT_OFC_CD = A.OFC_CD ) = @[rhq]  -- RHQ" ).append("\n"); 
		query.append("                #end" ).append("\n"); 
		...
		query.append("                AND     SM.EFF_DT <= TO_DATE(@[exp_dt], 'YYYY-MM-DD')      -- S/C Effective Date (To)" ).append("\n"); 
		query.append("                #end" ).append("\n"); 
		query.append("                #if (${eff_dt} != '')" ).append("\n"); 
		query.append("                AND     SM.EXP_DT >= TO_DATE(@[eff_dt], 'YYYY-MM-DD')      -- S/C Effective Date (From)" ).append("\n"); 
		query.append("                #end" ).append("\n"); 

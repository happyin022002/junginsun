...
*@FileName : ESM_PRI_0004_06HTMLAction.java
...
public class ESM_PRI_0004_06HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_0004_06HTMLAction 객체를 생성
	 */
	public ESM_PRI_0004_06HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri000406Event event = new EsmPri000406Event();
		
		if(command.isCommand(FormCommand.SEARCH01)) {
			event.setPriSpScpGohChgVO((PriSpScpGohChgVO)getVO(request, PriSpScpGohChgVO .class));
		} else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setPriSpScpGohChgVO((PriSpScpGohChgVO)getVO(request, PriSpScpGohChgVO .class));
		} else if(command.isCommand(FormCommand.MULTI01)) { //save
			event.setPriSpScpGohChgVOS((PriSpScpGohChgVO[])getVOs(request, PriSpScpGohChgVO .class));
		} else if(command.isCommand(FormCommand.MULTI02)) { //guideline copy
			event.setCstPriSpScpGohChgVO((CstPriSpScpGohChgVO)getVO(request, CstPriSpScpGohChgVO .class));

...
public class EsmAcm0014Event extends EventSupport {
	...
	public OTRCommRequestVO[] getOTRCommRequestVOs() {
		OTRCommRequestVO[] rtnVOs = null;
		if (this.otrCommRequestVOs != null) {
			rtnVOs = Arrays.copyOf(otrCommRequestVOs, otrCommRequestVOs.length);
		}
		return rtnVOs;
	}

	public void setOTRCommRequestVOs(OTRCommRequestVO[] otrCommRequestVOs) {
		if(otrCommRequestVOs != null){
			OTRCommRequestVO[] tmpVOs = Arrays.copyOf(otrCommRequestVOs, otrCommRequestVOs.length);
			this.otrCommRequestVOs  = tmpVOs;
		}
	}

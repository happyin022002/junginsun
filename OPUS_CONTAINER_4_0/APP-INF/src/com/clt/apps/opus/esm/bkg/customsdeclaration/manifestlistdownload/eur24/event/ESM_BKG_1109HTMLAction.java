...
*@FileName : ESM_BKG_1109HTMLAction.java
...
public class ESM_BKG_1109HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1109HTMLAction 객체를 생성 
	 */
	public ESM_BKG_1109HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1109Event event = new EsmBkg1109Event();
		CustomsSetupVO vo = new CustomsSetupVO(); 
		if(command.isCommand(FormCommand.SEARCH)
			||command.isCommand(FormCommand.SEARCH01)
			||command.isCommand(FormCommand.SEARCH02)) {
			vo = (CustomsSetupVO)getVO(request, CustomsSetupVO.class);
			event.setCustomsSetupVO(vo);
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setCustomsSetupVOs((CustomsSetupVO[])getVOs(request, CustomsSetupVO.class));
		}      

...
*@FileName : ESM_BKG_0152HTMLAction.java
...
* 1.0 Creation
=========================================================*/
package com.clt.apps.opus.esm.bkg.customsdeclaration.customsreport.bangladesh.event;
...
public class ESM_BKG_1038HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		// EVENT 생성
		EsmBkg1038Event event = new EsmBkg1038Event();

		// COMMAND 구분에 따른 처리
		if(command.isCommand(FormCommand.SEARCH))
    	{
			// 조회의 경우 처리
			LicenseInfoCondVO licenseInfoCondVO = (LicenseInfoCondVO)getVO(request, LicenseInfoCondVO.class);
			event.setLicenseInfoCondVO(licenseInfoCondVO);
    	}
		else if (command.isCommand(FormCommand.SEARCH01)) {
			BangladeshCustCondVO custCondVO = (BangladeshCustCondVO)getVO(request, BangladeshCustCondVO.class);
			event.setCustCondVO(custCondVO);
		}
		else if(command.isCommand(FormCommand.MULTI)) {
            event.setBkgCstmsBdFrtFwrdLicDetailVOs((BkgCstmsBdFrtFwrdLicDetailVO[])getVOs(request, BkgCstmsBdFrtFwrdLicDetailVO.class));
        }

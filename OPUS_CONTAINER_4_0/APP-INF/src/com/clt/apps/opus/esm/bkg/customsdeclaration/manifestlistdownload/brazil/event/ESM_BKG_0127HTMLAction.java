...
public class ESM_BKG_0127HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {
		...
		} else if(command.isCommand(FormCommand.MODIFY)||command.isCommand(FormCommand.MULTI05)) { // 브라질 세관정보 등록
			...
			BrManifestTransmitVO[] vos = (BrManifestTransmitVO[]) getVOs(request, BrManifestTransmitVO.class);
			if (vos != null)
			{
				for (int i = 0; i < vos.length; i++)
				{
					vos[i].setIbflag(request.getParameter("io_type"));
				}
				event.setBrManifestTransmitVOs(vos);
			}

...
public class BookingUtilDBDAOSearchSoStatusRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("           AND NVL(TRSP_FRST_FLG, 'X') <> 'Y'" ).append("\n"); 
		query.append("        UNION" ).append("\n"); 
		query.append("		SELECT MAX(RSLT_CNTR)" ).append("\n"); 
		query.append("		FROM (" ).append("\n"); 
		query.append("				SELECT 'Y' RSLT_CNTR" ).append("\n"); 
		query.append("				  FROM TRS_TRSP_SVC_ORD TRS, SCE_COP_HDR SCE" ).append("\n"); 
		query.append("				 WHERE SCE.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("				   AND SCE.BKG_NO = TRS.BKG_NO" ).append("\n"); 
		query.append("#if (${cntr_no} != '')" ).append("\n"); 
		query.append("				   AND SCE.CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("				   AND TRS.COP_NO = SCE.COP_NO" ).append("\n"); 
		query.append("				   AND TRS.DELT_FLG = 'N'" ).append("\n"); 
		query.append("				   AND NVL(TRS.TRSP_FRST_FLG, 'X') <> 'Y'" ).append("\n"); 
		query.append("				   AND TRS.COST_ACT_GRP_CD LIKE 'OD%'" ).append("\n"); 
		query.append("				   AND TRS.TRSP_SO_TP_CD = 'Y'" ).append("\n"); 
		query.append("				   AND TRS.EQ_NO IS NULL" ).append("\n"); 
		query.append("				   AND SCE.COP_STS_CD <> 'X'" ).append("\n"); 
		query.append("				UNION" ).append("\n"); 
		...
		query.append("#if (${cntr_no} != '')" ).append("\n"); 
		query.append("						   AND (@[cntr_no] IS NULL OR @[cntr_no] = '')" ).append("\n"); 
		query.append("#end" ).append("\n"); 

...
public class FFCmpnAuditDBDAO extends DBDAOSupport {
	...
	/**
	 * [ESM_ACM_0027] Save<br>
	 * FF Compensation Audit 화면저장 전 bkg_ff_cnt_cd, bkg_ff_seq 조회<br>
	 *
	 * @param String bkg_ff_cnt_cd
	 * @param String bkg_ff_seq
	 * @return String
	 * @throws DAOException
	 */
	public String searchFrtFwrdSeqCheck(String bkg_ff_cnt_cd, String bkg_ff_seq) throws DAOException {
		DBRowSet dbRowset = null;
		String bkgFfCntCd = "";
		String bkgFfSeq = "";
		...
		try{
			if (bkg_ff_cnt_cd != null) {
				param.put("bkg_ff_cnt_cd", bkg_ff_cnt_cd);
				velParam.put("bkg_ff_cnt_cd", bkg_ff_cnt_cd);
			}
			if (bkg_ff_seq != null) {
				param.put("bkg_ff_seq", bkg_ff_seq);
				velParam.put("bkg_ff_seq", bkg_ff_seq);
			}
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new FFCmpnAuditDBDAOSearchFrtFwrdSeqCheckRSQL(), param, velParam);
			if (dbRowset.next()) {
				bkgFfCntCd = dbRowset.getString("CUSTOMER");
				bkgFfSeq = dbRowset.getString("FF SQUENCE");
			}
		} catch(SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage());

...
* 1.0 Creation
=========================================================*/
package com.clt.apps.opus.esm.pri.common;
...
/**
 * PRIUtil Common Utility - 공통 로직을 처리한다.
 *
 * @author Minseok Song
 * @see 
 * @since J2EE 1.6
...
public class PRIUtil {
	...
	static Logger log = Logger.getLogger("PRIUtil");
	...
	public static ArrayList<String> getJobIdAndStatus(String programNo, String params) {
		...
		*/
			if (programNo != null &&  programNo.trim().length() != 0){
				...
				String jobId = vo.getCd();
				String status = vo.getNm();
				list.add(jobId);
				list.add(status);
			}
		}catch(Exception e){
	...
	/**
	 * ctnt 문자열에서 특정 문자열이 있는지 검사 한다.
	 * 이때 특정 문자열은 있되 다른 특정 문자열이 바로 앞에 붙어 나오는 경우 이를 제외 시킨다. 
	 *  
	 * 
	 * @param String ctnt 전체 문자열
	 * @param String specText 찾고자 하는 특정 문자열
	 * @param String excludeText 제외시키고자 하는 문자열
	 * @return boolean true : 해당 문자열 있음, false : 해당문자열 없음
	 */	
...
//							}
	...
	/**
     * VO Class's specific Method Value(only first index).
     * 
     * @param List<?> listData (VO Class)
     * @param String colName VO Class's MethodName (ex: getXXXX)
     * @return String colName's value (only one)
     */
	public static String getStringValuePriParaCdDtl(List<?> listData, String colName) {
		String strReturnValue = "";
		try{
	        
            if (listData.size() > 0) {
                Class<?> tmpClass = listData.get(0).getClass();
                Method tmpMethod = tmpClass.getMethod(colName, new Class[0]);
                String strValue = (String)tmpMethod.invoke(listData.get(0), new Object[0]);

...
*@FileName : ESM_FMS_0046HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esm.fms.timecharterinoutaccounting 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESM_FMS_0046HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0046HTMLAction 객체를 생성
	 */
	public ESM_FMS_0046HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0046Event event = new EsmFms0046Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchEstimatedRevenueListVOS((SearchEstimatedRevenueListVO[])getVOs(request, SearchEstimatedRevenueListVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionEstmIfVO((ConditionEstmIfVO)getVO(request, ConditionEstmIfVO .class,""));
			//event.setEstRevMmFrom(request.getParameter("fr_duration"));
			//event.setEstRevMmTo(request.getParameter("to_duration"));

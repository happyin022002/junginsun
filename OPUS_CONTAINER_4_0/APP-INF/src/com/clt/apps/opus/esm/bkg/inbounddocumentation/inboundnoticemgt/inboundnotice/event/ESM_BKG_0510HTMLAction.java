...
*@FileName : ESM_BKG_0510HTMLAction.java
...
public class ESM_BKG_0510HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0510HTMLAction 객체를 생성
	 */
	public ESM_BKG_0510HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0510Event event = new EsmBkg0510Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setHldNtcSearchVO((HldNtcSearchVO)getVO(request, HldNtcSearchVO.class));
		} else if(command.isCommand(FormCommand.MULTI01) || 
				command.isCommand(FormCommand.MULTI02)) {
			event.setPreHldNtcSendListVOs((PreHldNtcSendListVO[])getVOs(request, PreHldNtcSendListVO.class, "sheet1_"));
			...
			event.setHldNtcTpCd(JSPUtil.getParameter(request, "hld_ntc_tp_cd"));
		// 테스트용!!!
		} else if(command.isCommand(FormCommand.COMMAND40)) {
			event.setCstmsHldVO((CstmsHldVO)getVO(request, CstmsHldVO.class));
		}
		return  event;
	}

...
public class KoreaCustomsReportDBDAOsearchOFMCustInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT MAX(DECODE(BKG_CUST_TP_CD,'S',replace(replace(NVL(CUST_NM,' '),'\"','\"\"'),CHR(10),' '),' ')) CUST_C_NM," ).append("\n");
		query.append("MAX(DECODE(BKG_CUST_TP_CD,'C',replace(replace(NVL(CUST_NM,' '),'\"','\"\"'),CHR(10),' '),' ')) CUST_N_NM" ).append("\n");

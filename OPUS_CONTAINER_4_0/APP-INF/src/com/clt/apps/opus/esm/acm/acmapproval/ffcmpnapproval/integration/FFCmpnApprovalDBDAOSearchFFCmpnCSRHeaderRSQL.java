...
public class FFCmpnApprovalDBDAOSearchFFCmpnCSRHeaderRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        ELSE NVL(SUM(ROUND(A.IF_AMT,2)),0) " ).append("\n"); 
		query.append("        END AS CSR_AMT," ).append("\n"); 
		query.append("        CASE " ).append("\n"); 
		query.append("        WHEN NVL(SUM(ROUND(A.IF_AMT,2)),0) >= 0 " ).append("\n"); 
		...
		query.append("SELECT " ).append("\n"); 
		query.append("--DECODE(B.CONTI_CD,'M','CMS CHECK(O/EXP)','E','CMS WIRE','WIRE') AS PAY_MZD_LU_CD, " ).append("\n"); 
		query.append("        CASE A.SO_IF_CD" ).append("\n"); 
		query.append("        WHEN 'O'" ).append("\n"); 
		query.append("        THEN @[ap_ofc_cd]||'_ZERO PAYMENT'" ).append("\n"); 
		query.append("        ELSE @[ap_ofc_cd]||'_O/EXP'" ).append("\n"); 

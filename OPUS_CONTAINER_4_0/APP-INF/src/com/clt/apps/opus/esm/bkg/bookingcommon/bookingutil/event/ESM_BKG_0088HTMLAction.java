...
*@FileName : ESM_BKG_0088HTMLAction.java
...
public class ESM_BKG_0088HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0088HTMLAction 객체를 생성
	 */
	public ESM_BKG_0088HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0088Event event = new EsmBkg0088Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMdmYardVO((MdmYardVO)getVO(request, MdmYardVO .class));
			event.getMdmYardVO().setYdCd(JSPUtil.getParameter(request, "yd_cd", ""));
			event.getMdmYardVO().setLocCd(JSPUtil.getParameter(request, "loc_cd", ""));
			event.getMdmYardVO().setN1stVndrCntCd(JSPUtil.getParameter(request, "n1st_vndr_cnt_cd", ""));

...
public class GeneralBookingSearchDBDAOsearchTmnl301CntrRfInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                                                  WHERE SPC_TP = 'RF'" ).append("\n"); 
		query.append("                                                  AND SPC.TB_ROW_ID = RF.ROWID" ).append("\n"); 
		...
		query.append("    || 'NWGT_QTY:'              || CASE WHEN NVL(RF.NET_WGT,0) = 0 THEN ROUND((SELECT BLD.ACT_WGT/NVL((SELECT SUM(BQ.OP_CNTR_QTY) FROM BKG_QUANTITY BQ WHERE BQ.BKG_NO = BLD.BKG_NO),1) FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO),3)" ).append("\n"); 
		query.append("                                        ELSE RF.NET_WGT END 					|| CHR(10)" ).append("\n"); 
		query.append("	|| 'NWGT_TP:'               || CASE WHEN RF.WGT_UT_CD IS NULL THEN (SELECT BLD.WGT_UT_CD FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO)" ).append("\n"); 
		query.append("                                        ELSE RF.WGT_UT_CD END 					|| CHR(10)" ).append("\n"); 
		...
		query.append("                 AND (BK.AWK_CGO_FLG = 'Y' AND BHCC.HRD_CDG_ID = 'VNDR301_INSTRUCT' AND BHCC.ATTR_CTNT1 = RF.CNTR_TPSZ_CD)" ).append("\n"); 
		...
		query.append("   AND RF.CNTR_TPSZ_CD is not null" ).append("\n"); 

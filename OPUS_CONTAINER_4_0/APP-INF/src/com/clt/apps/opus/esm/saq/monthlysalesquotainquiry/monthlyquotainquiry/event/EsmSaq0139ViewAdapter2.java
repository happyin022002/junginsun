...
*@FileName       : EsmSaq0139ViewAdapter2.java
...
public class EsmSaq0139ViewAdapter2 extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		SearchMonthlyQuotaInquiry0139Tab02VO listVo = null; // 차후 루프문을 통해 결과리스트의 정보를 한 로우씩 입력할 결과VO를 세팅 
...
//		sbufXML.append("<DATA COLORDER='" + JSPUtil.convertStringArrayToString(changedColNms, "|") + "' COLSEPARATOR='" + DELIMITER + "' TOTAL='" + totCnt +"'>\n");
		...
		for(int j=0;j<realCnt;j++){ 
			listVo = (SearchMonthlyQuotaInquiry0139Tab02VO) list.get(j); //list 에서 한 줄 씩 빼내서 결과vo에 입력
...
*/
					//		<TR LEVEL="<%=rowSet.getString("sLevel")%>" expand="<%=isExpand%>">
//					sbufXML.append("<TR LEVEL= \"" + listVo.getSlevel() + "\" expand= \"" + isExpand + "\">");
					...
					//			<TD <%=cellFormat%> bgColor="<%=bgColor[level-1]%>"><![CDATA[<%=strVal_00%>]]></TD>
					sbufXML.append("<TD " + cellFormat + " bgColor=\"" + bgColor[level-1] + "\"><![CDATA[" + strVal_00 + "]]></TD>");
					//			<TD <%=cellFormat%> bgColor="<%=bgColor[level-1]%>"><![CDATA[<%=strVal_01%>]]></TD>
					sbufXML.append("<TD " + cellFormat + " bgColor=\"" + bgColor[level-1] + "\"><![CDATA[" + strVal_01 + "]]></TD>");
					//			<TD <%=cellFormat%> bgColor="<%=bgColor[level-1]%>"><![CDATA[<%=strVal_02%>]]></TD>
					sbufXML.append("<TD " + cellFormat + " bgColor=\"" + bgColor[level-1] + "\"><![CDATA[" + strVal_02 + "]]></TD>");
					//			<TD <%=cellFormat%> bgColor="<%=bgColor[level-1]%>"><![CDATA[<%=strVal_03%>]]></TD>

...
/**
 * HTTP Parser<br>
 * - com.clt.apps.alps.esm.bkg.inbounddocumentation.inboundnoticemgt 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESM_BKG_1167HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setDoRefVOs(doRefVOs);
			//event.setEdoSearchVO(edoSearchVO);
		}else if(command.isCommand(FormCommand.MULTI04)) {//Multi EDI Sending
			
			
			BkgCgoRlseVO[] bkgCgoRlseVOs = (BkgCgoRlseVO[])getVOs(request,BkgCgoRlseVO.class,"master_");

...
*@FileName : ESM_AGT_033HTMLAction.java
 ...
 * @see EsmAgt0033Event , ESM_AGT_033EventResponse 참조
 * @since J2EE 1.4
 */
public class ESM_AGT_0033HTMLAction extends HTMLActionSupport {


	private static final long serialVersionUID = 1L;
	/**
	 * ESMAGT0033HTMLAction 객체를 생성
	 */
	public ESM_AGT_0033HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmAgt0033Event event = new EsmAgt0033Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setFACCommVO((FACCommVO)getVO(request, FACCommVO .class));
		}
		
		if(command.isCommand(FormCommand.REPLY)){
			event.setFACCommVO((FACCommVO)getVO(request, FACCommVO .class));
			event.setFACCommVOS((FACCommVO[])getVOs(request, FACCommVO .class));
		}

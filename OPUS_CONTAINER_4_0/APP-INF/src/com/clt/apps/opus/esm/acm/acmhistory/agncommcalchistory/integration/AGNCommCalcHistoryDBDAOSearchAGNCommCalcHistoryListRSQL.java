...
public class AGNCommCalcHistoryDBDAOSearchAGNCommCalcHistoryListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       DECODE(A.AC_TP_CD, 'G', 'General', 'K', 'BRKG', 'H', 'CHF', 'S', 'T/S', 'C', 'Cross', 'N', 'CSF','R', 'RCSF', A.AC_TP_CD) AS AC_TP_CD," ).append("\n"); 
		query.append("       DECODE( A.CALC_NO , B.CALC_NO , B.RNK , '' ) AS CALC_SEQ," ).append("\n"); 
		query.append("       A.AC_STS_CD," ).append("\n"); 
		query.append("       SUM(DECODE(SIGN(INSTR('GKHSCNR', A.AC_TP_CD)), 1, A.IF_AMT, 0)) AS IF_AMT," ).append("\n"); 
		query.append("       SUM(DECODE(SIGN(INSTR('GKHSCNR', A.AC_TP_CD)), 1, A.CRNT_AMT, 0)) AS CRNT_AMT," ).append("\n"); 

...
*@FileName : AGNCommRequestDBDAOAddAcmAgnCommDtlFxMhCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AGNCommRequestDBDAOAddAcmAgnCommDtlFxMhCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Merchant Haulage Commission 정보를 ACM_AGN_COMM_DTL 에 insert 한다
	  * </pre>
	  */
	public AGNCommRequestDBDAOAddAcmAgnCommDtlFxMhCSQL(){
		setQuery();
		...
		query.append("FileName : AGNCommRequestDBDAOAddAcmAgnCommDtlFxMhCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(", SUM(NVL(REAL_FX_AMT,0)) " ).append("\n"); 
		query.append("- ( SELECT NVL(SUM(IF_DTRB_AMT),0) " ).append("\n"); 
		...
		query.append("       AND DTL.AC_SEQ > 1000" ).append("\n"); 
		query.append("       AND DTL.AC_SEQ <= 1000+ TO_NUMBER(@[max_ac_seq]) -1 " ).append("\n"); 
		query.append("       AND DTL.CNTR_TPSZ_CD = INFO.CNTR_TPSZ_CD) AS IF_DTRB_AMT" ).append("\n"); 
		query.append(", CURR_CD" ).append("\n"); 
		query.append("--, COMM_FX_AMT" ).append("\n"); 
		query.append("--, SUM(NVL(REAL_FX_AMT,0)) AS PAY_IF_DTRB_AMT" ).append("\n"); 
		query.append(", ROUND( @[pay_xch_rt] * SUM(NVL(REAL_FX_AMT,0)), 2) " ).append("\n"); 
		query.append("- ( SELECT NVL(SUM(PAY_IF_DTRB_AMT),0) " ).append("\n"); 
		query.append("      FROM ACM_AGN_COMM_DTL DTL" ).append("\n"); 
		query.append("     WHERE DTL.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("       AND DTL.AGN_CD = @[agn_cd]" ).append("\n"); 
		query.append("       AND DTL.IO_BND_CD = @[io_bnd_cd]" ).append("\n"); 
		...
		query.append("       AND DTL.AC_SEQ > 1000" ).append("\n"); 
		query.append("       AND DTL.AC_SEQ <= 1000+ TO_NUMBER(@[max_ac_seq]) -1 " ).append("\n"); 
		query.append("       AND DTL.CNTR_TPSZ_CD = INFO.CNTR_TPSZ_CD) AS PAY_IF_DTRB_AMT" ).append("\n"); 
		...
		query.append("                                    WHEN D.OFC_SET_TP_CD = 'F' AND D.OFC_CVRG_CD = 'kDEL' THEN NVL(@[del_finc],@[del_ar])" ).append("\n"); 

...
public class BlRatingDBDAOSearchRateQtyListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		,NVL((SELECT 'Y' FROM BKG_QTY_DTL_HIS Q" ).append("\n"); 
		query.append("		       WHERE Q.BKG_NO = A1.BKG_NO" ).append("\n"); 
		query.append("		         AND DECODE(Q.AWK_CGO_FLG,'Y',1,0)+DECODE(Q.RC_FLG,'Y',1,0)+DECODE(Q.DCGO_FLG,'Y',1,0) >1" ).append("\n"); 
		query.append("		         AND ROWNUM=1" ).append("\n"); 
		query.append("		   ),'N') AS MULTI_CGO" ).append("\n"); 
		...
		query.append(" GROUP BY A1.BKG_NO, A1.CNTR_TPSZ_CD,A1.EQ_SUBST_CNTR_TPSZ_CD,A1.EQ_SUBST_CGO_QTY" ).append("\n"); 
		query.append("      ,CASE " ).append("\n"); 
		query.append("		WHEN LV = 1 AND DTL.BB_CGO_FLG = 'Y' THEN 'BB'" ).append("\n"); 
		query.append("		WHEN LV = 2 AND DTL.AWK_CGO_FLG ='Y' THEN 'AK'" ).append("\n"); 
		query.append("		WHEN LV = 3 AND DTL.RC_FLG ='Y' THEN 'RF'" ).append("\n"); 
		...
		query.append("		,A1.EQ_SUBST_CNTR_TPSZ_CD EQ_SUB" ).append("\n"); 
		query.append("		,A1.EQ_SUBST_CGO_QTY EQ_SUB_QTY" ).append("\n"); 
		query.append("		,NVL((SELECT 'Y' FROM BKG_QTY_DTL Q" ).append("\n"); 
		query.append("		       WHERE Q.BKG_NO = A1.BKG_NO" ).append("\n"); 
		query.append("		         AND DECODE(Q.AWK_CGO_FLG,'Y',1,0)+DECODE(Q.RC_FLG,'Y',1,0)+DECODE(Q.DCGO_FLG,'Y',1,0) >1" ).append("\n"); 
		query.append("		         AND ROWNUM=1" ).append("\n"); 
		query.append("		   ),'N') AS MULTI_CGO" ).append("\n"); 

...
import com.clt.apps.opus.esm.bkg.bookingconduct.specialcargobookingconduct.specialcargorider.integration.SpecialCargoRiderDBDAO;
...
public class SpecialCargoRiderBCImpl extends BasicCommandSupport implements SpecialCargoRiderBC {
	...
	public void manageSpclRider(EsmBkg022908Event event, SignOnUserAccount account) throws EventException {
		...
		BkgImgStoVO[] BkgImgStoVOs = event.getBkgImgStoVO();
		...
		try {
			...
			for (int i = 0; i < BkgImgStoVOs.length; i++) {
				BkgImgStoVOs[i].setBkgNo(bkgBlNoVO.getBkgNo());
				BkgImgStoVOs[i].setCreUsrId(account.getUsr_id());
				BkgImgStoVOs[i].setUpdUsrId(account.getUsr_id());
				BkgImgStoVOs[i].setRgstUsrId(account.getUsr_id());
				BkgImgStoVOs[i].setRgstOfcCd(account.getOfc_cd());
				if (BkgImgStoVOs[i].getIbflag().equals("U")) {
					log.debug("[START:: SpecialCargoRiderBCImpl]updateVoList=====" + BkgImgStoVOs[i].getFileSavId());
					deleteVoList.add(BkgImgStoVOs[i]);
					BkgImgStoVOs[i].setIbflag("I");
				}
				if (BkgImgStoVOs[i].getIbflag().equals("D")) {
					log.debug("[START:: SpecialCargoRiderBCImpl]deleteVoList=====" + BkgImgStoVOs[i].getFileSavId());
					deleteVoList.add(BkgImgStoVOs[i]);

				} else if (BkgImgStoVOs[i].getIbflag().equals("I")) {
					log.debug("[START:: SpecialCargoRiderBCImpl]insertVoList=====" + BkgImgStoVOs[i].getFileSavId());
					insertVoList.add(BkgImgStoVOs[i]);
				}
			}
			...
			if (insertVoList.size() > 0) {
				...
				updateVoList.add(BkgImgStoVOs[0]);

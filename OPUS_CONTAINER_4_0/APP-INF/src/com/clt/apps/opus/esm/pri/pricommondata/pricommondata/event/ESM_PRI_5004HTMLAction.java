...
public class ESM_PRI_5004HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_5004HTMLAction 객체를 생성
	 */
	public ESM_PRI_5004HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri5004Event event = new EsmPri5004Event();
		
		if(command.isCommand(FormCommand.SEARCH)) { 
			event.setPriSvcScpPptVO((PriSvcScpPptVO)getVO(request, PriSvcScpPptVO .class));
		}
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setPriSvcScpPptVO((PriSvcScpPptVO)getVO(request, PriSvcScpPptVO .class));
			event.setPriSvcScpPptVOs((PriSvcScpPptVO[])getVOs(request, PriSvcScpPptVO.class,""));
		}
		return  event;
	}

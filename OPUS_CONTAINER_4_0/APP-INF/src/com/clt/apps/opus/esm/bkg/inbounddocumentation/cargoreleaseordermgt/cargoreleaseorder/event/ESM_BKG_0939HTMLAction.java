...
*@FileName : esm_bkg_0939HTMLAction.java
...
public class ESM_BKG_0939HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * esm_bkg_0939HTMLAction 객체를 생성
	 */
	public ESM_BKG_0939HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmBkg0939Event event = new EsmBkg0939Event();
    	
		if(command.isCommand(FormCommand.SEARCH)) {
			
			IdaDoRlseSearchVO searchVo = (IdaDoRlseSearchVO)getVO(request, IdaDoRlseSearchVO .class);
			if (searchVo.getEvntDtFm() !=null) {
				searchVo.setEvntDtFm(searchVo.getEvntDtFm().replaceAll("-", ""));
			}
			if (searchVo.getEvntDtTo() !=null) {
				searchVo.setEvntDtTo(searchVo.getEvntDtTo().replaceAll("-", ""));
			}
			if (searchVo.getEvntDtYm() !=null) {
				searchVo.setEvntDtYm(searchVo.getEvntDtYm().replaceAll("-", ""));
			}
			event.setIdaDoRlseSearchVO(searchVo);
		}

...
*@FileName       : ESM_CSQ_0036HTMLAction.java
 ...
 * @author CSQ USER
 * @see EsmCsq0036Event 참조
 * @since J2EE 1.6
 */

public class ESM_CSQ_0036HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_CSQ_0036HTMLAction 객체를 생성
	 */
	public ESM_CSQ_0036HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmCsq0036Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmCsq0036Event event = new EsmCsq0036Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setConditionVO((ConditionVO)getVO(request, ConditionVO .class));
			event.setCsqQtaStepVerVOS((CsqQtaStepVerVO[])getVOs(request, CsqQtaStepVerVO .class));

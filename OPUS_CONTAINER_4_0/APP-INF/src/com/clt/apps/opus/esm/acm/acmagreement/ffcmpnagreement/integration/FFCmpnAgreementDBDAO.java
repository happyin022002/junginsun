...
import com.clt.apps.opus.esm.acm.acmagreement.facommagreement.integration.FACommAgreementDBDAOAddFACAgreementListCSQL;
...
public class FFCmpnAgreementDBDAO extends DBDAOSupport {
	...
	public List<FFAgreementVO> searchFFAgreement(FFAgreementVO ffagreementVO) throws DAOException {
		 ...
		 */
		 @SuppressWarnings("unchecked")
		public List<FFAgreementVO> searchFFAgreementDup(FFAgreementVO ffagreementVO) throws DAOException {
			DBRowSet dbRowset = null;
			List<FFAgreementVO> list = null;
			//query parameter
			Map<String, Object> param = new HashMap<String, Object>();
			...
			try{
				...
				dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new FFCmpnAgreementDBDAOSearchFFAgreementDupRSQL(), param, velParam);
				list = (List)RowSetUtil.rowSetToVOs(dbRowset, FFAgreementVO.class);
			} catch(SQLException se) {
				log.error(se.getMessage(), se);
				throw new DAOException(new ErrorHandler(se).getMessage());
...
/**			
			...
			if (ffagreementVOs.size() > 0) {			
				insCnt = sqlExe.executeBatch((ISQLTemplate)new FFCmpnAgreementDBDAOAddFFAgreementCSQL(), ffagreementVO, null);

...
/**
 * OPUS GeneralBookingSplitCombineDBDAO <br>
...
public class GeneralBookingSplitCombineDBDAO extends DBDAOSupport {
	 ...
	 * @throws DAOException
	 */
	public void bkgXterRqstMstUpdate(String bkgNo, String tagetBkgNo, String blSplitNo) throws DAOException{
		//query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		try{
			param.put("bkg_no", bkgNo);
			param.put("taget_bkg_no", tagetBkgNo);
			param.put("bl_split_no", blSplitNo);
			new SQLExecuter("").executeUpdate((ISQLTemplate)new GeneralBookingSplitCombineDBDAOXterRqutMstUpdateUSQL(), param, null);
		}catch(SQLException se){
			log.error(se.getMessage(),se);
			throw new DAOException(new ErrorHandler(se).getMessage(), se);
		}catch(Exception ex){

...
public class PerformanceReportDBDAOCaSummaryReportOutVO2RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      , ( SELECT REPLACE( CUST_NM,CHR(10),' ') FROM BKG_CUSTOMER WHERE BKG_NO = BKG.BKG_NO AND BKG_CUST_TP_CD = 'S' AND ROWNUM=1)         AS CUST_NM " ).append("\n"); 
		query.append("      ,REPLACE(COR.BKG_CORR_RMK,CHR(10),' ') AS DIFF_RMK" ).append("\n"); 
		...
		query.append("   AND OFC_V.GSO = @[gso_ofc_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 

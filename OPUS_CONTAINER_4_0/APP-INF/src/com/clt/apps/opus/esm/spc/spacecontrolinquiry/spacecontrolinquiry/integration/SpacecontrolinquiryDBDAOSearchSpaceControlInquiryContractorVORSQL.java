...
public class SpacecontrolinquiryDBDAOSearchSpaceControlInquiryContractorVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("           D.CTRT_CUST_CNT_CD   AS CUST_CNT_CD ," ).append("\n"); 
		query.append("           D.CTRT_CUST_SEQ      AS  CUST_SEQ," ).append("\n"); 
		...
		query.append("                           (D.CTRT_CUST_CNT_CD, D.CTRT_CUST_SEQ, DECODE(M.OFC_DIV, 'RHQ', D.SLS_RHQ_CD, D.SLS_RGN_OFC_CD), D.POL_YD_CD)," ).append("\n"); 
		query.append("                           (D.CTRT_CUST_CNT_CD, D.CTRT_CUST_SEQ, DECODE(M.OFC_DIV, 'RHQ', D.SLS_RHQ_CD, D.SLS_RGN_OFC_CD), D.POD_YD_CD)" ).append("\n"); 
		query.append("                         )" ).append("\n"); 
		query.append(")" ).append("\n"); 
		...
		query.append("                     DECODE(SUBSTR(AGMT_CD,1,1),'S',NULL,SUBSTR(AGMT_CD,2,2))   AS CUST_CNT_CD," ).append("\n"); 
		query.append("                     DECODE(SUBSTR(AGMT_CD,1,1),'S',NULL,SUBSTR(AGMT_CD,4))     AS CUST_SEQ   ," ).append("\n"); 
		...
		query.append("         Z.PORT_CD," ).append("\n"); 
		query.append("         Z.OFC_CD ," ).append("\n"); 
		query.append("         NVL(Z.FCT_TEU, 0) + NVL(Z.FCT_HC, 0) * SPC_GET_HC_RT_BSA_FNC(@[trade] ,@[lane] ,@[bound] ,SUBSTR(@[vvd],1,4), SUBSTR(@[vvd],5,4),'D5') + NVL(Z.FCT_45, 0) * SPC_GET_HC_RT_BSA_FNC(@[trade] ,@[lane] ,@[bound] ,SUBSTR(@[vvd],1,4), SUBSTR(@[vvd],5,4),'D7') + NVL(Z.FCT_53, 0) * 2 AS FC_TTL_TEU," ).append("\n"); 
		query.append("         Z.FCT_TEU," ).append("\n"); 
		query.append("         Z.FCT_HC ," ).append("\n"); 
		query.append("         Z.FCT_45 ," ).append("\n"); 
		query.append("         Z.FCT_53 ," ).append("\n"); 

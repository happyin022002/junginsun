...
import com.clt.apps.opus.esm.bkg.customsdeclaration.manifestlistdownload.ancs.integration.AncsManifestListDownloadDBDAO;
 ...
 * @since J2EE 1.4
 */
public class AncsManifestListDownloadBCImpl extends BasicCommandSupport implements AncsManifestListDownloadBC {
	...
	public List<CstmsVvdVO> searchCstmsVvdList(CstmsVvdListCondVO condVo, SignOnUserAccount account) throws EventException {
		...
		try {
			return dbDao.searchAncsCstmsVvdList( (AncsCstmsVvdListCondVO)condVo ,account);
	...
	public void manageManifest(ManifestModificationVO manifestModificationVO, SignOnUserAccount account) throws EventException {
		try { 
			...
			if ( !isSsr.equals("") ){
				...
				// BKG_CSTMS_ANR_BL_LOG의 VVD_SEQ가 비어있는 경우에 대비해서 만든 로직 시간이 지난뒤에 삭제할 예정 
				dbDao.modifyBlLogAncsCstmsArtNo(ancsMftMoVO);
//				dbDao.MODI
	...
	public String deleteCstmsAncsManifest(CstmsVvdListCondVO condVo) throws EventException {
		...
		try {
			 dbDao.deleteCstmsAnrNtfy((AncsCstmsVvdListCondVO)condVo); 
			 dbDao.deleteCstmsAnrBl((AncsCstmsVvdListCondVO)condVo);
			 dbDao.deleteCstmsAnrCmdt((AncsCstmsVvdListCondVO)condVo);
			 dbDao.deleteCstmsAnrCntr((AncsCstmsVvdListCondVO)condVo);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("BKG06086", new String[] {}).getMessage(), ex);
			...
			if( ancsCstmsBlContainerVOs.length > 0 ){
				...
				if( "Y".equals( ancsCstmsBlContainerVOs[0].getRcv() ) ){
					...
					if( ancsCstmsBlVO != null ) {
						...
						bkgCstmsAnrBlVO.setActWgt(ancsCstmsBlVO.getActWgt());
						bkgCstmsAnrBlVO.setActWgtUtCd(ancsCstmsBlVO.getActWgtUtCd());
						bkgCstmsAnrBlVO.setPckQty(ancsCstmsBlVO.getPckQty());
						bkgCstmsAnrBlVO.setPckTpCd(ancsCstmsBlVO.getPckTpCd());

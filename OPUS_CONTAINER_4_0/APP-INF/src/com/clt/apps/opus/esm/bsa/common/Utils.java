...
package com.clt.apps.opus.esm.bsa.common;
...
public class Utils{
	...
	*/
	public static String fillSpace(String str, int spaceNum, String spaceString, String flag){
		String val = str.trim();
//		String fillString = "";
		StringBuffer fillString = new StringBuffer();				//SJH.20150508.소스품질
	
		if(val.length() > 0) {
			for(int i = 0 ; i < (spaceNum - val.length()) ; i++ ) {
//				fillString = fillString + spaceString.toString();
				fillString.append(spaceString.toString());			//SJH.20150508.소스품질
			}
		}
		
		if(flag.equals("left")) {
			str = fillString.toString() + val;						//SJH.20150508.소스품질
		}else if(flag.equals("right")) {
			str = val + fillString.toString();						//SJH.20150508.소스품질
		}

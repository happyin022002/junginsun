...
public class ESM_BKG_0272HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException 
	{
		...
		{ 
			...
			event.setBkgEuPinNoVOs((BkgEuPinNoVO[]) getVOs(request, BkgEuPinNoVO.class));
	    }
		...
		{ 				 
			...
			event.setBkgEuPinNoVOs((BkgEuPinNoVO[]) getVOs(request, BkgEuPinNoVO.class,"sheet1_"));

		}
		else if(command.isCommand(FormCommand.MULTI03))  // Save (BKG_EU_PIN_NO 테이블 저장)
		{ 				 
			log.debug("MULTI03 호출");
			
			event.setBkgEuPinNoVOs((BkgEuPinNoVO[]) getVOs(request, BkgEuPinNoVO.class));

		}		
		else if(command.isCommand(FormCommand.COMMAND03))  // EMAIL 전송할 데이터를 검색
		{ 				 

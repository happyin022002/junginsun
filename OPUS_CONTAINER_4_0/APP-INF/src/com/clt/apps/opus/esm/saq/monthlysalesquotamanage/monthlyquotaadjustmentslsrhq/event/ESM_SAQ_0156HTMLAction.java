...
*@FileName : ESM_SAQ_0156HTMLAction.java
...
public class ESM_SAQ_0156HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_SAQ_0156HTMLAction 객체를 생성
	 */
	public ESM_SAQ_0156HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmSaq0156Event event = new EsmSaq0156Event();
		SaqMonQtaLodTgtVO saqMonQtaLodTgtVO = new SaqMonQtaLodTgtVO();
		SaqMonQtaLodTgtVO[] saqMonQtaLodTgtVOs = null; 
		
		if(command.isCommand(FormCommand.SEARCHLIST) || command.isCommand(FormCommand.SEARCH)) {
			...
			event.setQuotaConditionVO((QuotaConditionVO)getVO(request, QuotaConditionVO .class));
			saqMonQtaLodTgtVOs = saqMonQtaLodTgtVO.fromRequestGrid(request, event.getQuotaConditionVO());
			event.setSaqMonQtaLodTgtVOs(saqMonQtaLodTgtVOs);
			...
			event.setQuotaConditionVO((QuotaConditionVO)getVO(request, QuotaConditionVO .class));
		}else if(command.isCommand(FormCommand.MODIFY10)) {
			event.setQuotaConditionVO((QuotaConditionVO)getVO(request, QuotaConditionVO .class));
		}else if(command.isCommand(FormCommand.MODIFY11)) {
			event.setQuotaConditionVO((QuotaConditionVO)getVO(request, QuotaConditionVO .class));
		}else if(command.isCommand(FormCommand.MODIFY14)) {
			event.setQuotaConditionVO((QuotaConditionVO)getVO(request, QuotaConditionVO .class));
		}

...
public class CLLCDLManifestDBDAOsearchBkgQtyByCntrtsRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	NVL((SELECT	SUM(DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),'2',1,2)/DECODE(SUBSTR(@[cntr_tpsz_cd],2,1),'2',1,2))--전체 대비 내 비중을 위한 분모값" ).append("\n"); 
		query.append("	FROM	BKG_CONTAINER" ).append("\n"); 
		query.append("	WHERE	BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("	AND	CNTR_TPSZ_CD != 'Q2'" ).append("\n"); 
		query.append("	AND	CNTR_TPSZ_CD != 'Q4'),0) IN_QTY" ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("	NVL((SELECT	SUM(DECODE(SUBSTR(CNTR_TPSZ_CD,2,1),'2',1,2)*NVL(OP_CNTR_QTY,0)/DECODE(SUBSTR(@[cntr_tpsz_cd],2,1),'2',1,2))--전체 대비 내 비중을 위한 분모값" ).append("\n"); 
		query.append("	FROM	BKG_QUANTITY" ).append("\n"); 
		query.append("	WHERE	BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("	AND	CNTR_TPSZ_CD != 'Q2'" ).append("\n"); 
		query.append("	AND	CNTR_TPSZ_CD != 'Q4'),0) IN_QTY" ).append("\n"); 

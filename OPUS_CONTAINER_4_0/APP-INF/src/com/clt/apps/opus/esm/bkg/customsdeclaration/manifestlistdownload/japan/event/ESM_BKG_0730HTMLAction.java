 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0730HTMLAction extends HTMLActionSupport {
	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0730HTMLAction 객체를 생성
	 */
	public ESM_BKG_0730HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0730Event event = new EsmBkg0730Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setJapanManifestListCondVO((JapanManifestListCondVO)getVO(request, JapanManifestListCondVO.class));

		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setJapanManifestModificationVOs((JapanManifestModificationVO[])getVOs(request, JapanManifestModificationVO.class, ""));

		} else if(command.isCommand(FormCommand.MULTI01)) {
			JapanManifestTransmitContainerVO japanManifestTransmitContainerVO = new JapanManifestTransmitContainerVO();
			japanManifestTransmitContainerVO.setJapanManifestTransmitCondVO((JapanManifestTransmitCondVO)getVO(request, JapanManifestTransmitCondVO.class));
			japanManifestTransmitContainerVO.setJapanManifestTransmitVOs((JapanManifestTransmitVO[])getVOs(request, JapanManifestTransmitVO.class));
			event.setJapanManifestTransmitContainerVO(japanManifestTransmitContainerVO);

...
*@FileName : BangladeshManifestListDownloadDBDAOsearchManifestObBlListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BangladeshManifestListDownloadDBDAOsearchManifestObBlListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 방글라데시 Manifest Outbound B/L List
	  * searchManifestObBlList
	  * </pre>
	  */
	public BangladeshManifestListDownloadDBDAOsearchManifestObBlListRSQL(){
		setQuery();
		...
		query.append("FileName : BangladeshManifestListDownloadDBDAOsearchManifestObBlListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT DECODE(LENGTH(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO)),'1'," ).append("\n"); 
		query.append("       DECODE(BK.POL_CD,'BDCGP','RSL-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC','RSLICD-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00')))," ).append("\n"); 
		query.append("                                                                                                              '2'," ).append("\n"); 
		query.append("       DECODE(BK.POL_CD,'BDCGP','RSL-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC','RSLICD-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'000')))," ).append("\n"); 
		query.append("                                                                                                               '3'," ).append("\n"); 
		query.append("       DECODE(BK.POL_CD,'BDCGP','RSL-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'0000'))," ).append("\n"); 
		query.append("                        'BDDAC','RSLICD-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'0000')))," ).append("\n"); 
		query.append("                                                                                                               '4'," ).append("\n"); 
		query.append("       DECODE(BK.POL_CD,'BDCGP','RSL-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC','RSLICD-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00')))," ).append("\n"); 
		query.append("       DECODE(BK.POL_CD,'BDCGP','RSL-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC','RSLICD-'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.POL_CD ORDER BY BK.BL_NO),'00')))) SL_NO," ).append("\n"); 
		query.append("       '' TR_NO," ).append("\n"); 
		query.append("       '' TR_DATE ," ).append("\n"); 
		query.append("       '' SBILL_NO, " ).append("\n"); 
		query.append("       '' SB_DATE, " ).append("\n"); 
		query.append("       '' EXP_NO, " ).append("\n"); 
		query.append("       '' EXP_DATE, " ).append("\n"); 
		query.append("       LOC.LOC_NM POD_NM," ).append("\n"); 
		query.append("       'AS PER INV & PKG LIST' MARK_NO, " ).append("\n"); 
		query.append("       '' PACK_NO, " ).append("\n"); 
		query.append("       '' PACK_NAT, " ).append("\n"); 
		query.append("       '' GOODS_DESC, " ).append("\n"); 
		query.append("       TRIM(TO_CHAR(NVL(CNTR.CNTR_WGT,0),'999999999.99'))||' '||CNTR.WGT_UT_CD GROSS_WGT," ).append("\n"); 
		...
		query.append("       DECODE(SUBSTR(CNTR.CNTR_TPSZ_CD,2,1),'2','8.6','4','8.6','5','9.6','4','8.6','9','8.6',' ') HEIGHT," ).append("\n"); 
		...
		query.append("       CASE WHEN NVL(BK.RCV_TERM_CD, 'Y') = 'S' THEN 'LCL' --Local" ).append("\n"); 
		query.append("            WHEN CNTR.CNTR_PRT_FLG = 'Y' THEN 'PRT'" ).append("\n"); 
		query.append("            ELSE 'FCL'" ).append("\n"); 
		query.append("       END AS STATUS," ).append("\n"); 
		query.append("       DG.IMDG_CLSS_CD AS IMCO," ).append("\n"); 
		query.append("       DG.IMDG_UN_NO AS UN," ).append("\n"); 
		...
		query.append("       REPLACE(SUBSTR(REPLACE(REPLACE(TRANSLATE(NVL(S.CUST_NM, ' '), CHR(10), ' '), '', ' '), CHR(9), ' '), 1, 150)||SUBSTR(REPLACE(REPLACE(TRANSLATE(NVL(S.CUST_ADDR, ' '), CHR(10), ' '), '', ' '), CHR(9), ''), 1, 100),' ',' ') SHIPPER_NM," ).append("\n"); 
		query.append("       REPLACE(SUBSTR(REPLACE(REPLACE(TRANSLATE(NVL(C.CUST_NM, ' '), CHR(10), ' '), '', ' '), CHR(9), ' '), 1, 150)||SUBSTR(REPLACE(REPLACE(TRANSLATE(NVL(C.CUST_ADDR, ' '), CHR(10), ' '), '', ' '), CHR(9), ''), 1, 100),' ',' ') CNEE_NM,       " ).append("\n"); 
		...
		query.append("       ,DECODE(HBL.BKG_NO,NULL,'1','0') HBL_IND" ).append("\n"); 
		query.append("       ,NVL(DECODE(BK.DEL_CD, 'BDDAC', '102DICD', ' '), ' ')||CHR(10) AS CNTR_LOC_CD" ).append("\n"); 
		query.append("       ,DECODE(DECODE(HBL.BKG_NO,NULL,'1','0'),'1','MSB','HSB') AS HBL_CODE" ).append("\n"); 
		query.append("FROM  BKG_BOOKING BK, BKG_CONTAINER CNTR, BKG_BL_DOC DES," ).append("\n"); 
		query.append("      BKG_CUSTOMER S, BKG_CUSTOMER C, BKG_CUSTOMER N, BKG_VVD VVD, " ).append("\n"); 
		query.append("      BKG_BL_MK_DESC BL_MARK, MDM_VSL_CNTR VSL, MDM_PCK_TP PK, MDM_LOCATION LOC" ).append("\n"); 
		query.append("      ,BKG_HBL HBL, BKG_DG_CGO DG" ).append("\n"); 
		query.append("      ," ).append("\n"); 
		query.append("      (" ).append("\n"); 
		...
		query.append("            AND     VVD.POL_CD      = @[pol_code]" ).append("\n"); 
		...
		query.append("            	AND     VVD.POD_CD      			like @[pod_code]||'%'" ).append("\n"); 
		query.append("			#end" ).append("\n"); 
		...
		query.append("AND  BK.BKG_NO                              = HBL.BKG_NO(+)" ).append("\n"); 
		...
		query.append("AND  BK.POD_CD 								= LOC.LOC_CD(+) " ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("ORDER BY SL_NO, ORG_BL_NO, CNTR_NO" ).append("\n"); 

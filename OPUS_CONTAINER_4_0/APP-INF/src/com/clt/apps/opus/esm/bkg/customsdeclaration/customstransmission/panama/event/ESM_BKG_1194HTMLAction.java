...
*@FileName : ESM_BKG_1194HTMLAction.java
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_1194HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1194HTMLAction 객체를 생성 
	 */
	public ESM_BKG_1194HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if (command.isCommand(FormCommand.SEARCH)) {
			// Retrieve
			EsmBkg1194Event event = new EsmBkg1194Event();
			event.setBkgCstmsPnmRcvLogVO((BkgCstmsPnmRcvLogVO) getVO(request, BkgCstmsPnmRcvLogVO.class));
			request.setAttribute("Event", event);
			return event;
		} else if (command.isCommand(FormCommand.COMMAND01)) {
			// RCV TEST
			OpusbkgPanamaReceiveEvent event = new OpusbkgPanamaReceiveEvent();
			event.setFlatFile(request.getParameter("rcv_msg"));
			request.setAttribute("Event", event);
			return event;
		}

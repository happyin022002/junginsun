...
public class GeneralBookingSearchDBDAOsearchTmnl301BlMain1RSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT '{BKG_INFO'  || CHR(10)" ).append("\n"); 
		query.append("                || 'BKGNBR:'		|| BB.BKG_NO                   								|| CHR(10)" ).append("\n"); 
		query.append("				|| 'BKG_DT:'		|| TO_CHAR(BB.BKG_CRE_DT, 'RRRRMMDDHH24MISS')				|| CHR(10)" ).append("\n"); 
		query.append("                || 'BKG_DT_GMT:'    || TO_CHAR(BB.BKG_CRE_GDT, 'RRRRMMDDHH24MISS')				|| CHR(10)" ).append("\n"); 
		...
		query.append("				|| 'BL_NO:'			|| NVL(BB.BL_NO,' ')                                                            || CHR(10)" ).append("\n"); 
		...
		query.append("                || 'HAULAGE_IND:'   || CASE WHEN POL.CONTI_CD <> 'E' THEN DECODE(BB.RCV_TERM_CD,'D','C','M')" ).append("\n"); 
		query.append("                                            ELSE DECODE(BB.RCV_TERM_CD,'D','C','Y','M') END || CHR(10)" ).append("\n"); 
		query.append("                || 'LINE_CD:'       || (SELECT MVSL.MODI_VSL_SLAN_CD FROM MDM_VSL_SVC_LANE MVSL, VSK_VSL_SKD VSK WHERE MVSL.VSL_SLAN_CD = VSK.VSL_SLAN_CD AND VSK.VSL_CD = BB.VSL_CD AND VSK.SKD_VOY_NO = BB.SKD_VOY_NO AND VSK.SKD_DIR_CD = BB.SKD_DIR_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'TOVSL:'			|| VVD1.VSL_CD								|| CHR(10)" ).append("\n"); 
		query.append("				|| 'LOYD:'			|| MVC1.LLOYD_NO							|| CHR(10)" ).append("\n"); 
		query.append("				|| 'VSLNAME:'		|| BKG_SPCLCHAR_CONV_FNC(MVC1.VSL_ENG_NM,'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'VSL_CALL_SIGN:'	|| MVC1.CALL_SGN_NO							|| CHR(10)" ).append("\n"); 
		query.append("				|| 'VVD_REF_NO:'	|| case when bb.pol_cd = 'MYPKG' then (select BCMV.PSA_VOY_DIR_CD " ).append("\n"); 
		query.append("																		     from BKG_CSTMS_MY_VVD BCMV" ).append("\n"); 
		query.append("																			where BCMV.vsl_cd     = VVPS1.vsl_cd" ).append("\n"); 
		query.append("																			  and BCMV.skd_voy_no = VVPS1.skd_voy_no" ).append("\n"); 
		query.append("																			  and BCMV.skd_dir_cd = VVPS1.skd_dir_cd" ).append("\n"); 
		query.append("                                                                              AND ROWNUM = 1" ).append("\n"); 
		query.append("																			  )" ).append("\n"); 
		query.append("											else VVPS1.SHP_CALL_NO end 						|| CHR(10)" ).append("\n"); 
		query.append("				|| 'TOVOY:'			|| VVD1.SKD_VOY_NO										|| CHR(10)" ).append("\n"); 
		query.append("				|| 'TODIR:'			|| VVD1.SKD_DIR_CD										|| CHR(10)" ).append("\n"); 
		query.append("                || 'TOCONSORT_VOY:' || BKG_SPCLCHAR_CONV_FNC(VSK_COMMON_PKG.GET_CSSM_VOY_NO_FNC(VVD1.VSL_CD,VVD1.SKD_VOY_NO,VVD1.SKD_DIR_CD,VVD1.POL_CD,'O'),'T') || CHR(10)" ).append("\n"); 
		...
		query.append("                || 'OLDCONSORT_VOY:'|| DECODE(@[brac], 'B', NVL(VSK_COMMON_PKG.GET_CSSM_VOY_NO_FNC(@[vsl_cd],@[skd_voy_no],@[skd_dir_cd],VVD1.POL_CD,'O'), ' '), 'R', NVL(VSK_COMMON_PKG.GET_CSSM_VOY_NO_FNC(@[vsl_cd],@[skd_voy_no],@[skd_dir_cd],VVD1.POL_CD,'O'), ' '), ' ')				|| CHR(10)" ).append("\n"); 
		query.append("				|| 'OLDPOL:'		|| DECODE(@[brac], 'B', NVL(@[pol_nod_cd], '     '), 'R', NVL(@[pol_nod_cd], '     '), '     ')			|| CHR(10)" ).append("\n"); 
		...
		query.append("                || 'OLDCONSORT_VOY: '	|| CHR(10)" ).append("\n"); 
		query.append("                || 'OLDPOL: '			|| CHR(10)" ).append("\n"); 
		...
		query.append("                || 'TCONSORT_VOY:'  || (SELECT VSK_COMMON_PKG.GET_CSSM_VOY_NO_FNC(BB.VSL_CD,BB.SKD_VOY_NO,BB.SKD_DIR_CD,VVDT.POL_CD,'O') " ).append("\n"); 
		query.append("                                       FROM BKG_VVD VVDT WHERE VVDT.BKG_NO = BB.BKG_NO AND VVDT.VSL_PRE_PST_CD = 'T' AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'REF_VVD:'       || BB.MY_FWRD_VSL_DESC	                || CHR(10)" ).append("\n"); 
		query.append("                || 'POR_TMNL_NAME:'	|| (SELECT BKG_SPCLCHAR_CONV_FNC(YD_NM,'T') FROM MDM_YARD MY WHERE	MY.YD_CD = BB.POR_NOD_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'POR_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(POR.LOC_NM,'T')					|| CHR(10)" ).append("\n"); 
		...
		query.append("                || 'POL_TMNL_NAME:' || (SELECT BKG_SPCLCHAR_CONV_FNC(YD_NM,'T') FROM MDM_YARD MY WHERE	MY.YD_CD = BB.POL_NOD_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'POL_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(POL.LOC_NM,'T')					|| CHR(10)" ).append("\n"); 
		...
		query.append("								'COXX',TO_CHAR(ADD_MONTHS(BB.BKG_CRE_DT,1),'YYYYMMDDHH24MI')," ).append("\n"); 
		query.append("								TO_CHAR(ADD_MONTHS(VVPS1.VPS_ETD_DT,1),'YYYYMMDDHH24MI'))		|| CHR(10)" ).append("\n"); 
		query.append("				|| 'POL_CCT:'		|| NVL(TO_CHAR(CLZ.MNL_SET_DT, 'yyyymmddhh24mi'),NVL(TO_CHAR(CLZ.SYS_SET_DT, 'yyyymmddhh24mi'),'')) 				|| CHR(10)" ).append("\n"); 
		query.append("                || 'POL_CCT_GMT:'	|| NVL(TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC(BB.POL_CD,NVL(CLZ.MNL_SET_DT, CLZ.SYS_SET_DT),'GMT'), 'yyyymmddhh24mi'),'') || CHR(10)" ).append("\n"); 
		query.append("                || 'POL_CCT_EDT:'	|| TO_CHAR( NVL(CLZ.MNL_SET_DT,CLZ.SYS_SET_DT) - NVL(TO_NUMBER((SELECT NVL(TM.DST_MNTS,0)/(24*60) FROM MDM_DYLGT_SAV_TM TM WHERE TM.DST_CNT_CD = SUBSTR(BB.POL_CD,1,2) " ).append("\n"); 
		query.append("                                 AND TO_CHAR(NVL(CLZ.MNL_SET_DT,CLZ.SYS_SET_DT),'YYYYMMDD')  BETWEEN TM.ST_DST_DT AND TM.END_DST_DT AND ROWNUM = 1 )),0), 'yyyymmddhh24mi')  || CHR(10)" ).append("\n"); 
		query.append("                || 'POD_TMNL_NAME:'	|| (SELECT BKG_SPCLCHAR_CONV_FNC(YD_NM,'T') FROM MDM_YARD MY WHERE	MY.YD_CD = BB.POD_NOD_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'POD_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(POD.LOC_NM,'T')					|| CHR(10)" ).append("\n"); 
		...
		query.append("                || 'PLD_TMNL_NAME:'	|| (SELECT BKG_SPCLCHAR_CONV_FNC(YD_NM,'T') FROM MDM_YARD MY WHERE	MY.YD_CD = BB.DEL_NOD_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'PLD_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(DEL.LOC_NM,'T')					|| CHR(10)" ).append("\n"); 
		...
		query.append("                || 'RLY_TMNL_NAME:'	|| (SELECT BKG_SPCLCHAR_CONV_FNC(YD_NM,'T') FROM MDM_YARD MY WHERE	MY.YD_CD = VVD2.POL_YD_CD AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'RLY_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(POST.LOC_NM,'T')				    || CHR(10)" ).append("\n"); 
		...
		query.append("				|| 'HUB_NAME:'		|| BKG_SPCLCHAR_CONV_FNC(NVL(HUBNM.LOC_NM,' '),'T')			|| CHR(10)" ).append("\n"); 
		...
		query.append("				|| 'REMARK:'		|| REPLACE(BB.VNDR_RMK, CHR(10), ' ') || CHR(10)" ).append("\n"); 
		query.append("				|| 'CMD:'			|| NVL(MCMDT.cmdt_cd, MRCMDT.REP_CMDT_CD)					|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CMDD:'			|| BKG_SPCLCHAR_CONV_FNC(NVL(MCMDT.CMDT_NM, MRCMDT.REP_CMDT_NM),'T')|| CHR(10)" ).append("\n"); 
		query.append("				|| 'EQREL:'			|| BB.MTY_PKUP_YD_CD                      					|| CHR(10)" ).append("\n"); 
		query.append("                || 'EQREL_AMSQUAL:'	|| (SELECT DECODE(LENGTH(ML.LOC_AMS_PORT_CD), 4, 'D', 5, 'K', ' ') FROM MDM_LOCATION ML WHERE ML.LOC_CD = SUBSTR(BB.MTY_PKUP_YD_CD,1,5))	|| CHR(10)" ).append("\n"); 
		query.append("                || 'EQREL_AMSPORT:'	|| (SELECT ML.LOC_AMS_PORT_CD FROM MDM_LOCATION ML WHERE ML.LOC_CD = SUBSTR(BB.MTY_PKUP_YD_CD,1,5))	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'OLDEQREL:'		|| @[edi_old_rel]                                           || CHR(10)" ).append("\n"); 
		query.append("                || 'OLDEQREL_AMSQUAL:'|| (SELECT DECODE(LENGTH(ML.LOC_AMS_PORT_CD), 4, 'D', 5, 'K', ' ') FROM MDM_LOCATION ML WHERE ML.LOC_CD = SUBSTR(@[edi_old_rel],1,5))	|| CHR(10)" ).append("\n"); 
		query.append("                || 'OLDEQREL_AMSPORT:'|| (SELECT ML.LOC_AMS_PORT_CD FROM MDM_LOCATION ML WHERE ML.LOC_CD = SUBSTR(@[edi_old_rel],1,5))	|| CHR(10)" ).append("\n"); 
		query.append("                || 'SCNO:'          || BB.SC_NO     											|| CHR(10)" ).append("\n"); 
		query.append("				|| 'SHN1:'			|| BKG_SPCLCHAR_CONV_FNC(REPLACE(SH.cust_nm, CHR(10), ' '), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'FFN1:'			|| BKG_SPCLCHAR_CONV_FNC(REPLACE(FW.cust_nm, CHR(10), ' '), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CNE1:'			|| BKG_SPCLCHAR_CONV_FNC(REPLACE(CN.cust_nm, CHR(10), ' '), 'T')	|| CHR(10)" ).append("\n"); 
		...
		query.append("				|| 'SHPR1:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(SH.CUST_NM, 1, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'SHPR2:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(SH.CUST_NM, 2, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'SHPR3:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(SH.CUST_ADDR, 1, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("				|| 'SHPR4:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(SH.CUST_ADDR, 2, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("				|| 'SHPR5:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(SH.CUST_ADDR, 3, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("                || 'SHPR6:'			|| (SELECT CNTC_PSON_NM FROM MDM_CUST_ADDR MCA WHERE MCA.CUST_CNT_CD =SH.CUST_CNT_CD AND MCA.CUST_SEQ = SH.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ADDR_TP_CD = '1' AND ROWNUM = 1)|| CHR(10)" ).append("\n"); 
		query.append("                || 'SHPR7:'			|| (SELECT PHN_NO FROM MDM_CUST_CNTC_PNT MCC WHERE MCC.CUST_CNT_CD =SH.CUST_CNT_CD AND MCC.CUST_SEQ = SH.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'CNEE1:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(CN.CUST_NM, 1, ''),  'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CNEE2:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(CN.CUST_NM, 2, ''),  'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CNEE3:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(CN.CUST_ADDR, 1, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CNEE4:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(CN.CUST_ADDR, 2, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("				|| 'CNEE5:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(CN.CUST_ADDR, 3, ''), 'T')|| CHR(10)" ).append("\n"); 
		query.append("                || 'CNEE6:'			|| (SELECT CNTC_PSON_NM FROM MDM_CUST_ADDR MCA WHERE MCA.CUST_CNT_CD =CN.CUST_CNT_CD AND MCA.CUST_SEQ = CN.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ADDR_TP_CD = '1' AND ROWNUM = 1)|| CHR(10)" ).append("\n"); 
		query.append("                || 'CNEE7:'			|| (SELECT PHN_NO FROM MDM_CUST_CNTC_PNT MCC WHERE MCC.CUST_CNT_CD =CN.CUST_CNT_CD AND MCC.CUST_SEQ = CN.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'NTFY1:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(FW.CUST_NM, 1, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'NTFY2:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(FW.CUST_NM, 2, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'NTFY3:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(FW.CUST_NM, 3, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'NTFY4:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(FW.CUST_NM, 4, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("				|| 'NTFY5:'			|| BKG_SPCLCHAR_CONV_FNC(BKG_TOKEN_NL_FNC(FW.CUST_NM, 5, ''), 'T')	|| CHR(10)" ).append("\n"); 
		query.append("                || 'NTFY6:'			|| (SELECT CNTC_PSON_NM FROM MDM_CUST_ADDR MCA WHERE MCA.CUST_CNT_CD =FW.CUST_CNT_CD AND MCA.CUST_SEQ = FW.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ADDR_TP_CD = '1' AND ROWNUM = 1)|| CHR(10)" ).append("\n"); 
		query.append("                || 'NTFY7:'			|| (SELECT PHN_NO FROM MDM_CUST_CNTC_PNT MCC WHERE MCC.CUST_CNT_CD =FW.CUST_CNT_CD AND MCC.CUST_SEQ = FW.CUST_SEQ AND PRMRY_CHK_FLG = 'Y' AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("				|| 'FWD_REF_CD:'	|| CASE WHEN SUBSTR(BB.POR_CD,1,2) IN ('FR', 'GB') THEN (select cust_ref_no_ctnt from bkg_reference refNo where bB.bkg_no = refNo.bkg_no and bkg_ref_tp_cd = 'XMRN' AND ROWNUM = 1)" ).append("\n"); 
		query.append("                                            ELSE BB.MY_FWRD_CD	END										|| CHR(10)" ).append("\n"); 
		query.append("				|| 'FWD_REF_DESC:'	|| REPLACE(BMF.MY_FWRD_NM, CHR(10), '')							|| CHR(10) BL_MAIN1" ).append("\n"); 
		...
		query.append("#if(${vsl_cd} != '')" ).append("\n"); 
		query.append("			AND		OLD_T_VVD.VSL_CD    (+)= DECODE(@[vsl_cd],'COXX','COMC','COYY','COMC','COZZ','COMC',@[vsl_cd])" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("			AND		VVD2.POD_CLPT_IND_SEQ = VVPS2.CLPT_IND_SEQ(+) " ).append("\n"); 

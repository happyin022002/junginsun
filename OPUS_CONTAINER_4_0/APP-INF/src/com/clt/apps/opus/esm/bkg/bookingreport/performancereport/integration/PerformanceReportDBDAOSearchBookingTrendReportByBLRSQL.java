...
public class PerformanceReportDBDAOSearchBookingTrendReportByBLRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,BC.CUST_CNT_CD ||  BC.CUST_SEQ AS CUST_CD" ).append("\n"); 
		...
		query.append("AND    BC.CUST_CNT_CD || BC.CUST_SEQ = @[cust_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${sel_cust_cd} != '')" ).append("\n"); 
		query.append("AND    BC.CUST_CNT_CD || BC.CUST_SEQ = @[sel_cust_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("      ,REPLACE(X.CUST_NM,CHR(10),' ') AS CUST_NM" ).append("\n"); 
		...
		query.append("              ,(SELECT SUM(((NVL(BKG_OFT_REV,0)+NVL(BKG_MISC_REV,0)+NVL(SCR_CHG_REV,0)+NVL(BKG_REV,0)) - NVL(ESTM_CM_COST_AMT,0)) * DECODE(SUBSTR(CNTR_TPSZ_CD,-1),'2',${unit_op},1)) FROM coa_bkg_REV_DTL WHERE T.bkg_NO =BKG_NO) CM" ).append("\n"); 
		query.append("              ,(SELECT SUM(((NVL(BKG_OFT_REV,0)+NVL(BKG_MISC_REV,0)+NVL(SCR_CHG_REV,0)+NVL(BKG_REV,0)) - NVL(ESTM_CM_COST_AMT,0)) * DECODE(SUBSTR(CNTR_TPSZ_CD,-1),'2',${unit_op},1)) FROM coa_bkg_REV_DTL WHERE T.bkg_NO =BKG_NO) CMPB" ).append("\n"); 

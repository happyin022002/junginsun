...
/**
 * OPUS CndCustomsReportEAIDAO <br>
	...
	public List<BkgNtcHisVO> sendAvcNoteFax(List<CndCstmsReportVO> cndCstmsReportVOs, SignOnUserAccount account)
		...
		try
		{
			...
			for (int i = 0; i < cndCstmsReportVOs.size(); i++)
			{
				...
				for (int j = 0; j < faxList.size(); j++)
				{
					if (cndCstmsReportVO.getFaxNo1().equals(faxList.get(j).getFaxNo1()))
					{
						...
						showPuFlgList.add(cndCstmsReportVO.getShowPuFlg());
						break;
					}
				}
				if (!chkFax)
				{
					if (i > 0 && blList.size() > 0)
					{
						...
						List<String> pulist = faxList.get(faxList.size() - 1).getShowPuFlgList();
						for (int blCnt = 0; blCnt < showPuFlgList.size(); blCnt++)
						{
							pulist.add(showPuFlgList.get(blCnt));
						}
						faxList.get(faxList.size() - 1).setShowPuFlgList(pulist);
			...
			if (blList.size() > 0)
			{
				...
				List<String> pulist = faxList.get(faxList.size() - 1).getShowPuFlgList();
				for (int i = 0; i < showPuFlgList.size(); i++)
				{
					pulist.add(showPuFlgList.get(i));
				}
				faxList.get(faxList.size() - 1).setShowPuFlgList(pulist);
			}
			...
			for (int i = 0; i < faxList.size(); i++)
			{
				...
				for (int j = 0; j < cndCstmsReportVO.getBlNoList().size(); j++)
				{
							...
							&& cndCstmsReportVO.getBlNoList().get(j).equals(cndCstmsReportVO.getBlNoList().get(j - 1)))
					{
						...
						cndCstmsReportVO.getShowPuFlgList().remove(j);
						j--;
					}
				}
			}
			...
			for (int i = 0; i < faxList.size(); i++)
			{
				...
				StringBuffer tmpBlNo = new StringBuffer();
				...
				for (int j = 0; j < cndCstmsReportVO.getBlNoList().size(); j++)
				{
					...
					tmpBlNo.append("'");
					tmpBlNo.append(cndCstmsReportVO.getBlNoList().get(j) + cndCstmsReportVO.getShowPuFlgList().get(j));
					tmpBlNo.append("'");
					if (cndCstmsReportVO.getBlNoList().size() > 0 && j < cndCstmsReportVO.getBlNoList().size() - 1) {
						tmpBlNo.append(",");
					}
				...
				String offCd = "";
				if (StringUtils.isEmpty(cndCstmsReportVO.getFaxOfcCd())) {
					log.debug("****************** 세션 offCd 셋팅 ***************");
					offCd = account.getOfc_cd();
				} else {
					offCd = cndCstmsReportVO.getFaxOfcCd();
				}

				log.debug("****************** OffCd : " + offCd);

				FaxMetaInfo faxMetaInfo = new FaxMetaInfo("BKG", // 모듈명(ex.BKG)
						"ESM_BKG_0026.mrd", // MRD 파일 명 (ex.WO_NORMAL.mrd)
						"N", // 배치 유무(Y/N)
						"Arrival Notice(BL#: " + cndCstmsReportVO.getBlNo() + ")", // 제목
						"/rv form_blNo[( " + sbBlNo.toString() + ")] form_ofcCd[" + account.getOfc_cd()	+ "]  form_tmpBlNo[" + tmpBlNo.toString() + "] ",
						// RD Parameter (ex. [419][1][selho])
						cndCstmsReportVO.getCustNm()+ ";" + cndCstmsReportVO.getFaxNo1(),
				...
				for (int j = 0; j < cndCstmsReportVO.getBlNoList().size(); j++)
				{
					...
					bkgNtcHisVO.setBkgCustTpCd(cndCstmsReportVO.getBkgCustTpCd());
		...
		try
		{
				...
				for (int i = 0; i < cndCstmsReportVOs.size(); i++)
				{
					...
					if (!chkMailAddr)
					{
						if (i > 0 && blList.size() > 0)
						{
							...
							List<String> pulist = mailList.get(mailList.size() - 1).getShowPuFlgList();
							for (int blCnt = 0; blCnt < showPuFlgList.size(); blCnt++)
							{
								pulist.add(showPuFlgList.get(blCnt));
							}
							mailList.get(mailList.size() - 1).setShowPuFlgList(pulist);
						}
				...
				for (int i = 0; i < mailList.size(); i++)
				{
					...
					if (iAttachMaxCnt > 0 && cndCstmsReportVO.getBlNoList().size() > iAttachMaxCnt)
					{
						...
						for (int j = 0; j < cndCstmsReportVO.getBlNoList().size(); j++)
						{
							...
							list3.add(cndCstmsReportVO.getShowPuFlgList().get(j));
							if (list2.size() == iAttachMaxCnt)
							{
								temp.setBlNoList(list2);
								temp.setShowPuFlgList(list3);
								mailList.add(temp);
						...
						if (list2.size() > 0)
						{
							...
							cndCstmsReportVO.setShowPuFlgList(list3);
							mailList.add(cndCstmsReportVO);
						}
					}
				}
			...
			else
			{
				...
				for (int i = 0; i < cndCstmsReportVOs.size(); i++)
				{
					...
					cndCstmsReportVO.setShowPuFlgList(showPuFlgList);
					mailList.add(cndCstmsReportVO);
				}
			}
			...
			for (int i = 0; i < mailList.size(); i++)
			{
				...
				mail.setSubject(sbBlNm.toString() + "Advice Notes from "+ConstantMgr.getCompanyName());
				// 받는사람
				mail.setRecipient(cndCstmsReportVO.getNtcEml1());
				// 숨은참조
				String bccRcvrEml = new com.clt.apps.opus.esm.bkg.bookingcommon.bookingutil.basic.BookingUtil().searchBccEmailAddrRSQL("AV");
				if (bccRcvrEml != null && !"".equals(bccRcvrEml)) mail.setBccRcvrEml(bccRcvrEml);
				// 내용
				...
				sbCtnt.append("\n\n");
				sbCtnt.append("Thanks for using "+ConstantMgr.getCompanyName()+" company. [9165]").append("\n");
				...
				sbCtnt.append(ConstantMgr.getCompanyName()).append("\n\n");
				...
				comRptDsgnXptInfoVO.setRdParaCtnt("/rv form_blNo[(" + sbBlNo.toString() + ")] form_ofcCd["
						+ account.getOfc_cd() + "]  form_tmpBlNo[" + tmpBlNo.toString() + "] ");

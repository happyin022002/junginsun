...
*@FileName : ESM_BKG_0540HTMLAction.java
...
public class ESM_BKG_0540HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0540HTMLAction 객체를 생성
	 */
	public ESM_BKG_0540HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0540Event event = new EsmBkg0540Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setInbondManifestListCondVO((InbondManifestListCondVO)getVO(request, ClearanceTypeCondVO.class));
			...
			event.setStrCmdtCd(request.getParameter("strCmdtCd"));

		} else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setStrLocCd(request.getParameter("strLocCd"));

		} else if(command.isCommand(FormCommand.SEARCH04)) {
			ClearanceTypeCondVO condVo = new ClearanceTypeCondVO();
			condVo.setCustCd(request.getParameter("strCustCntCd")+request.getParameter("strCustSeq"));
			condVo.setPodCd(request.getParameter("strPod"));
			condVo.setDelCd(request.getParameter("strDel"));
			event.setInbondManifestListCondVO((InbondManifestListCondVO)condVo);

		} else if(command.isCommand(FormCommand.SEARCH05)) {
			event.setStrScNo(request.getParameter("strScNo"));

		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setInbondManifestDetailVOs((InbondManifestDetailVO[])getVOs(request, ClearanceTypeDetailVO.class));
		}

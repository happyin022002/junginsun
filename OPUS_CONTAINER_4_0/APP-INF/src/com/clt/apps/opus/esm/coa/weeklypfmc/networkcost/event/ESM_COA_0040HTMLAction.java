...
*@FileName : ESM_COA_0040HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esm.coa.weeklypfmc.networkcost 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @since J2EE 1.6
 */
public class ESM_COA_0040HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * TESTHTMLAction 객체를 생성
	 */
	public ESM_COA_0040HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmCoa0040Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmCoa0040Event event = new EsmCoa0040Event();  
...
//			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		...
		else if(command.isCommand(FormCommand.MULTI03)) { // COPY : SJH.20150206.ADD
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setSearchPortTariffListVOs((SearchPortTariffListVO[])getVOs(request, SearchPortTariffListVO.class));
		}		
		else if(command.isCommand(FormCommand.SEARCHLIST02) || command.isCommand(FormCommand.SEARCHLIST03)) {
			event.setCoaPortTrfVO((CoaPortTrfVO)getVO(request, CoaPortTrfVO.class));
		}
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setSearchPortTariffDetailListVOs((SearchPortTariffDetailListVO[])getVOs(request, SearchPortTariffDetailListVO.class));
		}
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
		}

...
public class IndonesiaCustomsTransmissionDBDAO extends DBDAOSupport {
	...
	public IndonesiaManifestDetail2VO searchManifestDetail2List(IdManifestListCondVO idManifestListCondVO) throws DAOException {
		...
		try {
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate) new IndonesiaCustomsTransmissionDBDAOSearchManifestDetail2ListRSQL(), param, velParam);
	...
	/**
	 * Indonesia 세관에 Manifest 신고할 내용을 download 받을 File로 생성하기 위해<br>
	 * CMDT_HS_CD를 조회한다. -- UI_BKG-0310, UI_BKG-0311<br>
	 *
	 * @param String bkgNo
	 * @return List<IndonesiaManifestCmdtHsCdVO>
	 * @throws DAOException
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<IndonesiaManifestCmdtHsCdVO> searchManifestCmdtHsCdList(String bkgNo) throws DAOException {
		Map<String, Object> param = new HashMap<String, Object>();
		try {
			param.put("bkg_no", bkgNo);
			DBRowSet dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate) new IndonesiaCustomsTransmissionDBDAOSearchManifestCmdtHsCdListRSQL(), param, null);
			return (List)RowSetUtil.rowSetToVOs(dbRowset, IndonesiaManifestCmdtHsCdVO.class);
		} catch(SQLException se) {
			throw new DAOException(new ErrorHandler(se).getMessage(), se);
		} catch(Exception ex) {
			throw new DAOException(new ErrorHandler(ex).getMessage(), ex);

...
*@FileName       : EsmSpc0024ViewAdapter5.java
 ...
 * @since J2EE 1.5
 */
public class EsmSpc0024ViewAdapter5 extends ViewAdapter{
		...
		if (rowSets != null && rowSets.getRowCount()>0) {//3.1
			...
			try{
				while(rowSets.next()){
					...
					rgn    = rowSets.getString("rgn_ofc_cd");
					...
					if((!searchOffice || (lvl != 3)) && (!searchVVD || (lvl >= 3)) && (!searchWeek || lvl != 2) && (!searchLane || lvl != 1)){
...
//						sbufXML.append("<tr BGCOLOR=\"" + color[lvl-1] + "\" LEVEL=\"" + lvl + "\" EXPAND=\"").append(lvl<3?"TRUE":"FALSE").append("\">\n");
						...
						sbufXML.append("<td>" + (lvl==4?1:0) + "</td>\n");
						sbufXML.append("</tr>\n");	
					}
				}
			}catch(Exception e){

...
public class ESM_BKG_1141HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if( command.isCommand(FormCommand.SEARCH) ){
			event.setMalaysiaManifestListCondVO((MalaysiaManifestListCondVO) getVO(request, MalaysiaManifestListCondVO.class));

		} else if( command.isCommand(FormCommand.MULTI) ){
			MalaysiaManifestTransmitVO malaysiaManifestTransmitVO = new MalaysiaManifestTransmitVO();
			malaysiaManifestTransmitVO.setMalaysiaManifestListCondVO((MalaysiaManifestListCondVO) getVO(request,MalaysiaManifestListCondVO.class));
			malaysiaManifestTransmitVO.setMalaysiaManifestListBlInfoVOs((MalaysiaManifestListBlInfoVO[]) getVOs(request,MalaysiaManifestListBlInfoVO.class, "sheet1_"));
			malaysiaManifestTransmitVO.setMalaysiaManifestListCntrInfoVOs((MalaysiaManifestListCntrInfoVO[]) getVOs(request,MalaysiaManifestListCntrInfoVO.class, "sheet2_"));

			event.setMalaysiaManifestTransmitVO(malaysiaManifestTransmitVO);

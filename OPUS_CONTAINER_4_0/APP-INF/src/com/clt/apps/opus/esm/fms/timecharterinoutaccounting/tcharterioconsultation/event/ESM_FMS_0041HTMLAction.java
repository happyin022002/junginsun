...
*@FileName : ESM_FMS_0041HTMLAction.java
...
public class ESM_FMS_0041HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0041HTMLAction 객체를 생성
	 */
	public ESM_FMS_0041HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FormCommand command = FormCommand.fromRequest(request);
 		EsmFms0041Event event = new EsmFms0041Event();

 		if(command.isCommand(FormCommand.SEARCH)) {
 			event.setCondSearchSlipApprovalVO((CondSearchSlipApprovalVO)getVO(request, CondSearchSlipApprovalVO.class));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setSearchSlipApprovalListVO((SearchSlipApprovalListVO)getVO(request, SearchSlipApprovalListVO.class));
		}

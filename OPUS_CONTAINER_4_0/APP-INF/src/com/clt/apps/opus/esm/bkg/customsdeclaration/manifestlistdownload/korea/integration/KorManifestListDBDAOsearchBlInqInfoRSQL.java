...
public class KorManifestListDBDAOsearchBlInqInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", TRUNC(DECODE(NVL(WGT_UT_CD, '   '), 'LBS', ROUND(NVL(CNTR_TTL_WGT, 0)*0.4536, 3), NVL(CNTR_TTL_WGT, 0)), 2) CNTR_TTL_WGT" ).append("\n");
		...
		query.append(", DECODE(TO_NUMBER(NVL(CSTMS_OFC_CTY_CD, '0')), 0, DECODE(TS_POD_CD, 'KRINC', 20, 'KRPUS', 30, 'KRKAN', 62, 'KRPTK', 16, 0), TO_NUMBER(NVL(CSTMS_OFC_CTY_CD, '0'))) TAX_CODE1" ).append("\n");
		query.append(", DECODE(TO_NUMBER(NVL(KR_CSTMS_DEPT_CD, '0')), 0, DECODE(TS_POD_CD, 'KRINC', 10, 'KRPUS', DECODE(@[trans_tp], 'I', 27, 'T', 27, 10), 'KRKAN', 10, 'KRPTK', 10, 0), TO_NUMBER(NVL(KR_CSTMS_DEPT_CD, '0'))) TAX_CODE2" ).append("\n");

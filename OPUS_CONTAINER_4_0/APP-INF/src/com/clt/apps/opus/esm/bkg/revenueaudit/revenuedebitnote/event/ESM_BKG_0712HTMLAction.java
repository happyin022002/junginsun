...
public class ESM_BKG_0712HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0712HTMLAction 객체를 생성
	 */
	public ESM_BKG_0712HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0712Event event = new EsmBkg0712Event();
        log.debug("::CALL::> ESM_BKG_0712HTMLAction - " + command.getCommand());
		...
		if(command.isCommand(FormCommand.MULTI01)) {
			...
			event.getRevDrNoteVO().setSearchGubun("4");
			event.getRevDrNoteVO().setBkgRevDrNoteVO((CstmBkgRevDrNoteVO)getVO(request, CstmBkgRevDrNoteVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH06)) {
			event.getRevDrNoteVO().setSearchGubun("6");
			event.getRevDrNoteVO().setBkgRevDrNoteVO((CstmBkgRevDrNoteVO)getVO(request, CstmBkgRevDrNoteVO .class));
			log.debug("*********************************************************");
			log.debug("searchGubun action : " + event.getRevDrNoteVO().getSearchGubun());
			log.debug("*********************************************************");
		}

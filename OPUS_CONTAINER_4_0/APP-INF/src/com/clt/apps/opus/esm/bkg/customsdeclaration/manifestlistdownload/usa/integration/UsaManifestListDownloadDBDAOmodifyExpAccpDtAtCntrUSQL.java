...
public class UsaManifestListDownloadDBDAOmodifyExpAccpDtAtCntrUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#if (${ibflag} == 'ARR')" ).append("\n"); 
		query.append("    SET CGO_ARR_ACPT_DT = TO_DATE(@[icr_date],'RRMMDDHH24MI')" ).append("\n"); 
		query.append("  WHERE CNT_CD = 'US'" ).append("\n"); 
		query.append("    AND BL_NO = @[bl_no]" ).append("\n"); 
		query.append("#if (${icr_code} == '13') " ).append("\n"); 
		query.append("    AND CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#else " ).append("\n"); 
		query.append("    SET XPT_ACPT_DT = TO_DATE(@[acp_date], 'RRMMDDHH24MI')" ).append("\n"); 
		query.append("  WHERE CNT_CD = 'US'" ).append("\n"); 
		query.append("    AND BL_NO = @[bl_no]" ).append("\n"); 

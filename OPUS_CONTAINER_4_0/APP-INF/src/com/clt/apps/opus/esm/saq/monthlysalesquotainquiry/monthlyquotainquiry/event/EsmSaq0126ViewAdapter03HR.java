...
*@FileName       : EsmSaq0126ViewAdapter03HR.java
*@FileTitle      : Monthly Quota Inquiry(H/O)
*Open Issues     :
*Change history  :
*@LastModifyDate : 
*@LastModifier   : 
...
public class EsmSaq0126ViewAdapter03HR extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		SearchMonthlyQuotaInquiry0126Tab03HRVO listVo = null; // 차후 루프문을 통해 결과리스트의 정보를 한 로우씩 입력할 결과VO를 세팅 
...
//		sbufXML.append("<DATA COLORDER='" + JSPUtil.convertStringArrayToString(changedColNms, "|") + "' COLSEPARATOR='" + DELIMITER + "' TOTAL='" + totCnt +"'>\n");
		...
		for(int j=0;j<realCnt;j++){ 
			...
			listVo = (SearchMonthlyQuotaInquiry0126Tab03HRVO) list.get(j); //list 에서 한 줄 씩 빼내서 결과vo에 입력
...
//			isEndRow = (Integer.parseInt(listVo.getRowSeq()) == 6
...
/*			sbufXML.append("<TD " + dir_image + " bgColor= \"" + bgColor[level-1] + "\"><![CDATA[" + dir_cd + "]]></TD>");
			...
			if(level>4){
			    sbufXML.append("<TD bgColor= \"" + bgColor[level-2] + "\"><![CDATA[" + aq_cd + "]]></TD>");
			}else{

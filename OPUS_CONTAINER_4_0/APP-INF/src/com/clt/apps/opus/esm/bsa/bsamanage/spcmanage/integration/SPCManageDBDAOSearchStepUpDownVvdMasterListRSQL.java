...
public class SPCManageDBDAOSearchStepUpDownVvdMasterListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("  SUBSTR(A.COST_YRMON, 1, 4) || A.COST_WK AS COST_YRWK," ).append("\n"); 
		...
		query.append("#if (${rdocode} == '007')" ).append("\n"); 
		query.append("      C.PORT_BSA_CAPA ,																--20150821.MOD" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("   (SELECT CRR_BSA_CAPA " ).append("\n"); 
		query.append("      FROM BSA_VVD_OTR_CRR E " ).append("\n"); 
		query.append("     WHERE C.TRD_CD = E.TRD_CD " ).append("\n"); 
		query.append("       AND C.RLANE_CD = E.RLANE_CD AND C.VSL_CD = E.VSL_CD " ).append("\n"); 
		query.append("       AND C.SKD_VOY_NO = E.SKD_VOY_NO AND C.SKD_DIR_CD = E.SKD_DIR_CD " ).append("\n"); 
		query.append("       AND E.BSA_OP_JB_CD = '009' AND  C.CRR_CD = E.CRR_CD) AS PORT_BSA_CAPA," ).append("\n"); 
		query.append("#end " ).append("\n"); 
		...
		query.append("#if (${chkprd} == 'M')" ).append("\n"); 
		query.append("  SUBSTR(A.COST_YRMON, 1, 4) || A.COST_WK ," ).append("\n"); 
		query.append("#elseif (${chkprd} == 'W')" ).append("\n"); 

...
*@FileName : PRISimulationDBDAORsltRFAContractInfoListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class PRISimulationDBDAORsltRFAContractInfoListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * GET RFA CONTRACT INFO
	  * </pre>
	  */
	public PRISimulationDBDAORsltRFAContractInfoListRSQL(){
		setQuery();
		...
		query.append("FileName : PRISimulationDBDAORsltRFAContractInfoListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        'RFA'                           AS CNTR_TP" ).append("\n"); 
		query.append("      , RH.RFA_NO                        AS CNTR_NO" ).append("\n"); 
		query.append("      , MU.CUST_LGL_ENG_NM                  AS CUST_NM" ).append("\n"); 
		...
		query.append("      , RS.PROP_SCP_SREP_CD            AS SREP_CD " ).append("\n"); 
		...
		query.append("       FROM   PRI_RP_SCP_RT_ROUT_PNT  OP, COM_INTG_CD_DTL CD" ).append("\n"); 
		...
		query.append("       FROM   PRI_RP_SCP_RT_ROUT_PNT  DP, COM_INTG_CD_DTL CD" ).append("\n"); 
		...
		query.append("            MDM_CUSTOMER            MU  ," ).append("\n"); 
		...
		query.append("    AND RH.RFA_NO LIKE @[scontract_no]||'%'" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    #if (${sprop_scp_srep_cd} != '')" ).append("\n"); 
		query.append("    AND RS.PROP_SCP_SREP_CD = @[sprop_scp_srep_cd]" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    #if (${sprc_ctrt_cust_tp_cd} != '')" ).append("\n"); 
		query.append("    AND RM.PRC_CTRT_CUST_TP_CD = @[sprc_ctrt_cust_tp_cd]" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    #if (${sprop_scp_ofc_cd} != '')" ).append("\n"); 
		query.append("    AND RS.PROP_SCP_OFC_CD   = @[sprop_scp_ofc_cd]" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    #if (${srout_pnt_loc_def_cd_ori} != '')" ).append("\n"); 
		query.append("    AND     EXISTS" ).append("\n"); 
		query.append("           (SELECT 'X'" ).append("\n"); 
		query.append("            FROM   PRI_RP_SCP_RT_ROUT_PNT  OP" ).append("\n"); 
		query.append("            WHERE  1=1" ).append("\n"); 
		...
		query.append("    AND     EXISTS" ).append("\n"); 
		query.append("           (SELECT 'X'" ).append("\n"); 
		query.append("            FROM   PRI_RP_SCP_RT_ROUT_PNT  DP" ).append("\n"); 
		query.append("            WHERE  1=1" ).append("\n"); 
		...
		query.append("            )" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    AND     TO_DATE ( @[seff_dt],'YYYY-MM-DD' ) BETWEEN RS.EFF_DT AND RS.EXP_DT " ).append("\n"); 
		query.append("    AND     RM.AMDT_SEQ = (SELECT MAX ( M.AMDT_SEQ ) " ).append("\n"); 
		query.append("                             FROM PRI_RP_MN M " ).append("\n"); 
		query.append("                            WHERE M.PROP_NO = RH.PROP_NO " ).append("\n"); 
		query.append("                              AND TO_DATE ( @[seff_dt],'YYYY-MM-DD' ) BETWEEN M.EFF_DT AND M.EXP_DT AND M.PROP_STS_CD = 'A')  " ).append("\n"); 
		query.append("    AND     RS.SVC_SCP_CD = @[ssvc_scp_cd]" ).append("\n"); 
		query.append("    #if (${sctrt_cust} != '' && ${sctrt_cust_seq} != '')" ).append("\n"); 
		query.append("    AND      RM.CTRT_CUST_CNT_CD          = @[sctrt_cust]" ).append("\n"); 
		query.append("    AND      RM.CTRT_CUST_SEQ             = @[sctrt_cust_seq]" ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("    AND      MU.CUST_CNT_CD               = RM.CTRT_CUST_CNT_CD" ).append("\n"); 
		query.append("    AND      MU.CUST_SEQ                  = RM.CTRT_CUST_SEQ" ).append("\n"); 
		query.append("    AND      MU.CNTR_DIV_FLG              = 'Y'" ).append("\n"); 
		query.append(" ORDER BY RFA_NO, CUST_NM, ACT_CUST_NM, PROP_SCP_SREP_CD, CMDT_NM " ).append("\n"); 

...
*@FileName : ESM_FMS_0043HTMLAction.java
...
public class ESM_FMS_0043HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0043HTMLAction 객체를 생성
	 */
	public ESM_FMS_0043HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FormCommand command = FormCommand.fromRequest(request);
 		EsmFms0043Event event = new EsmFms0043Event();

 		if(command.isCommand(FormCommand.SEARCH)) {
 			event.setCondSearchSlipApprovalVO((CondSearchSlipApprovalVO)getVO(request, CondSearchSlipApprovalVO.class));
			
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setCsrNo(request.getParameter("slp_no"));
			event.setCsrDesc(request.getParameter("csr_desc"));

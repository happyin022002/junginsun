...
public class StatusReportDBDAOSearchTsLoadingReportByLocationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                    VVD.VSL_CD||VVD.SKD_VOY_NO||VVD.SKD_DIR_CD        AS OUT_VVD," ).append("\n"); 
		query.append("                    CNTR.CNTR_TPSZ_CD," ).append("\n"); 
		query.append("                    CNTR_VOL_QTY," ).append("\n"); 
		query.append("                    BK.BL_NO," ).append("\n"); 
		query.append("                    REPLACE(CUST.CUST_NM,CHR(10),' ') AS SHIPPER_NM," ).append("\n"); 
		query.append("                    IN_VVD.SLAN_CD     AS IN_LANE," ).append("\n"); 
		query.append("                    VVD.SLAN_CD    AS OUT_LANE," ).append("\n"); 
		query.append("                    IN_VVD.POL_YD_CD   AS IN_TMNL," ).append("\n"); 
		query.append("                    VVD.POD_YD_CD  AS OUT_TMNL," ).append("\n"); 
		...
		query.append("                    DECODE(SUBSTR(VVD.POD_CD,1,2),'JP','JPN',DECODE(OUT_POD.CONTI_CD,'M','USA','E','EUR',DECODE(SUBSTR(OUT_POL.SCONTI_CD,1,1),SUBSTR(OUT_POD.SCONTI_CD,1,1),'IPC','PRC')))  OUT_ZONE" ).append("\n"); 
		query.append("                FROM   (SELECT  " ).append("\n"); 
		...
		query.append("                AND VSP.CLPT_IND_SEQ  = VVD.POL_CLPT_IND_SEQ" ).append("\n"); 
		...
		query.append("                AND  VVD.POL_CD = OUT_POL.LOC_CD(+)" ).append("\n"); 
		query.append("                AND  VVD.POD_CD = OUT_POD.LOC_CD(+)" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("  " ).append("\n"); 
		query.append("                /* bl_type_ob */" ).append("\n"); 
		query.append("				#if (${bl_type_ob} != '' && ${bl_type_ts} == '') " ).append("\n"); 
		query.append("				AND    BK.POL_CD = VVD.POL_CD" ).append("\n"); 
		query.append("				#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                /* bl_type_ts */" ).append("\n"); 
		query.append("				#if (${bl_type_ts} != '' && ${bl_type_ob} == '') " ).append("\n"); 
		query.append("				AND    BK.POL_CD <> VVD.POL_CD" ).append("\n"); 
		query.append("				#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		...
		query.append("				AND    VVD.SLAN_CD = @[out_lane]" ).append("\n"); 
		query.append("				#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                /* OUT TMNL */" ).append("\n"); 
		query.append("				#if (${out_tmnl} != '') " ).append("\n"); 
		query.append("				AND    VVD.POD_YD_CD = @[out_tmnl]" ).append("\n"); 
		query.append("				#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("" ).append("\n"); 

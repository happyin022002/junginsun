...
/**
 * BSA에 대한 DB 처리를 담당<br> - BSA Business Logic을 처리하기 위한 JDBC 작업수행.<br>
 * 
 * @author eunju park
 * @see CommonBCImpl 참조
 * @since J2EE 1.4
...
public class CommonDBDAO extends DBDAOSupport {
	...
	/**
	 * BSA Creation/Update 화면의 항목을 가져온다.<br>
	 * 사용프로그램 : BSA시스템 사용 테이블 : BSA_OP_JB
	 * 
	 * @param String bsaOpCd
	 * @return DBRowSet
	 * @throws DAOException
	 */
	public DBRowSet searchUI(String bsaOpCd) throws DAOException {
		DBRowSet dbRowset = new DBRowSet(); // 데이터 전송을 위해 DB ResultSet을 구현한 객체
		...
		try{
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new CommonDBDAOSearchBsaOpJbCodeRSQL(), param, velParam);

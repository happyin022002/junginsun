...
public class ESM_BKG_0253HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0253HTMLAction 객체를 생성
	 */
	public ESM_BKG_0253HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0253Event event = new EsmBkg0253Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setBkgEqlzPortVOS((BkgEqlzPortVO[])getVOs(request, BkgEqlzPortVO .class,""));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgEqlzPortVO((BkgEqlzPortVO)getVO(request, BkgEqlzPortVO .class));
		}

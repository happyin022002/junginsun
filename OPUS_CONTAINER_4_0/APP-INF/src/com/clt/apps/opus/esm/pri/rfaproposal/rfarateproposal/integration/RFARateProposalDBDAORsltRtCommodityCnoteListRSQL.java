...
*@FileName : RFARateProposalDBDAORsltRtCommodityCnoteListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RFARateProposalDBDAORsltRtCommodityCnoteListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * retrieve the cnote info according to cmdt_hdr_seq
	  * </pre>
	  */
	public RFARateProposalDBDAORsltRtCommodityCnoteListRSQL(){
		setQuery();
		...
		query.append("FileName : RFARateProposalDBDAORsltRtCommodityCnoteListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("              ,SRC_INFO_CD" ).append("\n"); 
		query.append("              ,N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,LEAD(N1ST_CMNC_AMDT_SEQ) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, CMDT_HDR_SEQ, CMDT_NOTE_SEQ ORDER BY AMDT_SEQ) AS NEXT_N1ST_CMNC_AMDT_SEQ" ).append("\n"); 
		query.append("              ,LAG(NOTE_CONV_MAPG_ID) OVER(PARTITION BY PROP_NO, SVC_SCP_CD, CMDT_HDR_SEQ, CMDT_NOTE_SEQ ORDER BY AMDT_SEQ) AS PREV_NOTE_CONV_MAPG_ID" ).append("\n"); 
		query.append("              ,ACPT_USR_ID" ).append("\n"); 
		query.append("              ,ACPT_OFC_CD" ).append("\n"); 

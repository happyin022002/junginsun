...
*@FileName : ESM_BKG_0172HTMLAction.java
 ...
 * @since J2EE 1.6
 */
public class ESM_BKG_0172HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0172HTMLAction 객체를 생성
	 */
	public ESM_BKG_0172HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0172Event event = new EsmBkg0172Event();
		if(command.isCommand(FormCommand.DEFAULT)||command.isCommand(FormCommand.SEARCH)) {
			...
			event.setVgmEdiSupVO((VgmEdiSupVO) getVO(request, VgmEdiSupVO.class));
			...
			event.setSlctFlg(JSPUtil.getParameter(request, "slct_flg"));
			event.setRefCode(JSPUtil.getParameter(request, "ref_code"));
			event.setEdiReceiveId(JSPUtil.getParameter(request, "edi_receive_id"));
			event.setNtcKndCd(JSPUtil.getParameter(request, "ntc_knd_cd"));
			event.setEdiSndId(JSPUtil.getParameter(request, "edi_snd_id"));
			event.setCntrNos(JSPUtil.getParameter(request, "cntr_nos"));
			event.setVgmEdiSupVOS((VgmEdiSupVO[]) getVOs(request, VgmEdiSupVO.class,""));
		}
		return  event;
	}

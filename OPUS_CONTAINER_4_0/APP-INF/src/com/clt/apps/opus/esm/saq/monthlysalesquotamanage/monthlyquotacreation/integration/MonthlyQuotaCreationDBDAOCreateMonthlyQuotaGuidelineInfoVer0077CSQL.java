...
*@FileName : MonthlyQuotaCreationDBDAOCreateMonthlyQuotaGuidelineInfoVer0077CSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MonthlyQuotaCreationDBDAOCreateMonthlyQuotaGuidelineInfoVer0077CSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * guideline data 생성 1. saq_mon_qta_gline_ver insert
	  * </pre>
	  */
	public MonthlyQuotaCreationDBDAOCreateMonthlyQuotaGuidelineInfoVer0077CSQL(){
		setQuery();
		...
		query.append("FileName : MonthlyQuotaCreationDBDAOCreateMonthlyQuotaGuidelineInfoVer0077CSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("--1. SAQ_MON_QTA_GLINE_VER INSERT" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("INSERT INTO SAQ_MON_QTA_GLINE_VER (                                           " ).append("\n"); 
		...
		query.append("  C.OFC_CD||SUBSTR(D.BSE_YR,3,2)                                              " ).append("\n"); 
		query.append("      ||D.BSE_QTR_CD" ).append("\n"); 
		query.append("      ||LPAD(NVL((SELECT SUBSTR(MAX(GLINE_VER_NO),                            " ).append("\n"); 
		query.append("                            LENGTH(MAX(GLINE_VER_NO))-1, 2)+1                 " ).append("\n"); 
		...
		query.append("                  AND   CRE_OFC_CD = C.OFC_CD),1),2,'0') AS GLINE_VER_NO,     " ).append("\n"); 
		query.append("  SUBSTR(D.BSE_YR,3,2)                                                        " ).append("\n"); 
		query.append("      ||D.BSE_QTR_CD" ).append("\n"); 
		query.append("      ||LPAD(NVL((SELECT SUBSTR(MAX(GLINE_VER_NO),                            " ).append("\n"); 
		query.append("                            LENGTH(MAX(GLINE_VER_NO))-1, 2)+1                 " ).append("\n"); 
		query.append("                  FROM  SAQ_MON_QTA_GLINE_VER                                 " ).append("\n"); 
		...
		query.append("                  AND   BSE_QTR_CD = D.BSE_QTR_CD                                   " ).append("\n"); 
		query.append("                  AND   SAQ_TGT_GRP_CD = C.SAQ_TGT_GRP_CD                     " ).append("\n"); 
		query.append("                  AND   CRE_OFC_CD = C.OFC_CD),1),2,'0') AS QTA_MST_VER_NO,    " ).append("\n"); 
		query.append("  '00' AS SAQ_STS_CD,                                                         " ).append("\n"); 
		query.append("  A.MQTA_MDL_VER_NO ,                                                         " ).append("\n"); 

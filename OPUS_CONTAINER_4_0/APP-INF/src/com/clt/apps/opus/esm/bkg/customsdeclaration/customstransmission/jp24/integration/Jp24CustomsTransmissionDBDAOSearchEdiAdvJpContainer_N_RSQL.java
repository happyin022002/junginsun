...
*@FileName : Jp24CustomsTransmissionDBDAOSearchEdiAdvJpContainer_N_RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class Jp24CustomsTransmissionDBDAOSearchEdiAdvJpContainer_N_RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public Jp24CustomsTransmissionDBDAOSearchEdiAdvJpContainer_N_RSQL(){
		setQuery();
		...
		query.append("FileName : Jp24CustomsTransmissionDBDAOSearchEdiAdvJpContainer_N_RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT --95.Container Number (12)" ).append("\n"); 
		query.append("       RPAD(NVL(CNTR.CNTR_NO, ' '), 12, ' ') AS DATA00," ).append("\n"); 
		query.append("       --96.Seal Number (15) x6" ).append("\n"); 
		query.append("       RPAD(NVL(UPPER(CNTR.CNTR_SEAL_NO), ' '), 15, ' ') AS DATA01," ).append("\n"); 
		query.append("       --96.Seal Number (15)" ).append("\n"); 
		...
		query.append("       --97.Empty/Full Container Identification (3)" ).append("\n"); 
		query.append("       RPAD(DECODE(NVL(CNTR.FULL_MTY_CD, ' '), 'M', '4', '5'), 3, ' ') AS DATA07," ).append("\n"); 
		query.append("       --98.Container Size Code (2)" ).append("\n"); 
		...
		query.append("       --99.Container Type Code (2)" ).append("\n"); 
		query.append("       DECODE(SUBSTR(CNTR.CNTR_TPSZ_CD, 1, 1), 'D', 'GP', 'R', 'RT', 'O', 'UT', 'S', 'UT', 'F', 'PF', 'A', 'PF', 'P', 'PF', 'T', 'TN', 'GP') AS DATA09," ).append("\n"); 
		query.append("       --100 Service Type on Delivery Code (2)" ).append("\n"); 
		query.append("       DECODE(NVL(CNTR.DE_TERM_CD, ' '), 'Y', '51', 'S', '52', 'D', '53', '  ') AS DATA10," ).append("\n"); 
		query.append("       --101 Container Ownership Code (3)" ).append("\n"); 
		query.append("       RPAD(NVL(CNTR.JP_CNTR_OWNR_CD, '2'), 3, ' ') AS DATA11," ).append("\n"); 
		query.append("       --102 Vanning Type Code (3)" ).append("\n"); 
		query.append("       DECODE(NVL(CNTR.RCV_TERM_CD, ' '), 'S', '1  ', '4  ') AS DATA12," ).append("\n"); 
		query.append("       --103 Customs Ｃonvention on Containers (CCC) Application Identifier (1)" ).append("\n"); 
		query.append("       '1' AS DATA13," ).append("\n"); 
		query.append("       --104 Automatic Search for Discharged Container Exclusion Identifier (1)" ).append("\n"); 

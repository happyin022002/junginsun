...
public class ESM_BKG_0134HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * esm_bkg_0134HTMLAction 객체를 생성
	 */
	public ESM_BKG_0134HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        EsmBkg0134Event event = new EsmBkg0134Event();
        ...
        log.debug("===================================");
        log.debug("    ESM_BKG_0134HTMLAction Start   ") ;
        log.debug("===================================");

        if(command.isCommand(FormCommand.SEARCH)) {
            rcvToDt = request.getParameter("rcv_to_dt").replaceAll("-", "");
            rcvFmDt = request.getParameter("rcv_fm_dt").replaceAll("-", "");
            blNo = request.getParameter("bl_no");
            schTp = request.getParameter("frm_sch_tp");
            
            if ("A".equals(schTp)) {
	            event.setRcvToDt(rcvToDt);
	            event.setRcvFmDt(rcvFmDt);
            	event.setBlNo("");
            } else {
	            event.setRcvToDt("");
	            event.setRcvFmDt("");
            ...
            log.debug("=======================================");
            log.debug("    rcvToDt : "+ rcvToDt                );
            log.debug("    rcvFmDt : "+ rcvFmDt                );
            log.debug("    blNo    : "+ blNo                   );
            log.debug("=======================================");
        }

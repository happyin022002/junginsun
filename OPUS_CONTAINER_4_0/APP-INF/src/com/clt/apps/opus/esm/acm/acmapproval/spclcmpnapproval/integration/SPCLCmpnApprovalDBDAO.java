...
public class SPCLCmpnApprovalDBDAO extends DBDAOSupport {
	...
	public List<SPCLCmpnApprovalPrintMasterVO> searchSPCLCmpnApprovalPrintMaster(SPCLCmpnApprovalPrintMasterVO spCLCmpnApprovalPrintMasterVO) throws DAOException {
		...
		try{
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new SPCLCmpnApprovalDBDAOSearchSPCLCmpnApprovalPrintMasterListRSQL(), param, velParam);
	...
	public List<SPCLCmpnApprovalPrintDetailVO> searchSPCLCmpnApprovalPrintDetail(SPCLCmpnApprovalPrintDetailVO spCLCmpnApprovalPrintDetailVO) throws DAOException {
		...
		try{
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new SPCLCmpnApprovalDBDAOSearchSPCLCmpnApprovalPrintDetailListRSQL(), param, velParam);
...
//			}
	...
	public List<SPCLCmpnCSRINFtoAPVO> searchSPCLINFtoAP(SPCLCmpnCSRHeaderVO spCLCmpnCSRHeaderVO) throws DAOException {
		...
		/**
		 * [ESM_ACM_0031]
		 * Special Compensation CSR Creation Ex.Rate Input 대상 booking 을 조회한다.<br>
		 *
		 * @param SPCLCmpnApprovalMasterVO spCLCmpnApprovalMasterVO
		 * @return
		 * @exception DAOException
		 */
		@SuppressWarnings("unchecked")
		public List<SPCLCmpnApprovalMasterVO> searchRateInputTarget(SPCLCmpnApprovalMasterVO spCLCmpnApprovalMasterVO) throws DAOException {
			DBRowSet dbRowset = null;
			List<SPCLCmpnApprovalMasterVO> list = null;
			//query parameter
			Map<String, Object> param = new HashMap<String, Object>();
			...
			try{
				...
				dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new SPCLCmpnApprovalDBDAOSearchSPCLCmpnApprovalRateInputTargetRSQL(), param, velParam);
				list = (List)RowSetUtil.rowSetToVOs(dbRowset, SPCLCmpnApprovalMasterVO.class);
			} catch(SQLException se) {
				log.error(se.getMessage(), se);
				throw new DAOException(new ErrorHandler(se).getMessage());
		 ...
		 *
		 * @param List<SPCLCmpnApprovalMasterVO> spCLCmpnApprovalMasterVOs
		 * @throws DAOException
		 */
		public void manageSPCLCmpnApproval(List<SPCLCmpnApprovalMasterVO> spCLCmpnApprovalMasterVOs) throws DAOException {
			try {
				SQLExecuter sqlExe = new SQLExecuter("");
				int updCnt[] = null;
				if (spCLCmpnApprovalMasterVOs.size() > 0) {
					updCnt = sqlExe.executeBatch((ISQLTemplate)new SPCLCmpnApprovalDBDAOModifySPCLCmpnApprovalListUSQL(), spCLCmpnApprovalMasterVOs, null);
					for (int i=0; i<updCnt.length; i++) {
						if (updCnt[i]== Statement.EXECUTE_FAILED)
							throw new DAOException("Fail to insert No"+ i + " SQL");
					}
		 ...
		 * @throws DAOException
		 */
		public void manageSPCLCmpnApprovalDtl(List<SPCLCmpnApprovalMasterVO> spCLCmpnApprovalMasterVOs) throws DAOException {
			try {
				SQLExecuter sqlExe = new SQLExecuter("");
				int insCnt[] = null;
				if (spCLCmpnApprovalMasterVOs.size() > 0) {
					insCnt = sqlExe.executeBatch((ISQLTemplate)new SPCLCmpnApprovalDBDAOModifySPCLCmpnApprovalDtlListUSQL(), spCLCmpnApprovalMasterVOs, null);
					for (int i=0; i<insCnt.length; i++) {
						if (insCnt[i]== Statement.EXECUTE_FAILED)
							throw new DAOException("Fail to insert No"+ i + " SQL");
					}

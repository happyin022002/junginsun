...
*@FileName : ESM_BKG_0204HTMLAction.java
...
public class ESM_BKG_0204HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_0204HTMLAction 객체를 생성
	 */
	public ESM_BKG_0204HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0204Event event = new EsmBkg0204Event();
        log.debug("::CALL::> ESM_BKG_0204_HTMLAction - " + command.getCommand());

        if(command.isCommand(FormCommand.DEFAULT)) {
            // search Init
            String unNo      = JSPUtil.getParameter(request, "imdg_un_no");
            String imdgClass       = JSPUtil.getParameter(request, "imdg_clss_cd");
            String prpShpNm       = JSPUtil.getParameter(request, "prp_shp_nm");            

            event.setUnNo(unNo);
            event.setImdgClass(imdgClass);
            event.setPrpShpNm(prpShpNm);            

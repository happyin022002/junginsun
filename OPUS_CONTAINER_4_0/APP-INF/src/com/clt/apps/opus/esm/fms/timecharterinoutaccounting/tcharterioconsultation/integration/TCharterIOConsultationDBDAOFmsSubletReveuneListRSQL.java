...
public class TCharterIOConsultationDBDAOFmsSubletReveuneListRSQL implements ISQLTemplate{
	...
	public TCharterIOConsultationDBDAOFmsSubletReveuneListRSQL(){
		...
		params.put("debit_acct_cd",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("   AND A.FLET_CTRT_NO IN (SELECT @[flet_ctrt_no]" ).append("\n"); 
		query.append("                            FROM DUAL" ).append("\n"); 
		query.append("                           UNION  " ).append("\n"); 
		query.append("                          SELECT Y.FLET_CTRT_NO" ).append("\n"); 
		query.append("                            FROM FMS_ID_VSL Z" ).append("\n"); 
		query.append("                               , FMS_CONTRACT Y" ).append("\n"); 
		query.append("                           WHERE 1=1" ).append("\n"); 
		query.append("                             AND Z.FLET_CTRT_NO = @[flet_ctrt_no]" ).append("\n"); 
		query.append("                             AND Z.USE_FLG = 'Y'" ).append("\n"); 
		query.append("                             AND Z.VSL_CD = Y.VSL_CD" ).append("\n"); 
		query.append("							) " ).append("\n"); 
		...
		query.append("   AND B.ACCT_CD = @[debit_acct_cd]" ).append("\n"); 

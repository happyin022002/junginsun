...
public class CsScreenDBDAOsearchBlCustInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,NTC.PKUP_YD_CD                    AS PKUP_YD_CD --Ceres 터미널 경우 CAHAL01일때 RD폼 변경위해" ).append("\n"); 
		query.append("FROM BKG_BOOKING BKGM" ).append("\n"); 
		query.append("    ,BKG_CUSTOMER BCST" ).append("\n"); 
		query.append("    , (SELECT PKUP_YD_CD, BKG_NO" ).append("\n"); 
		query.append("         FROM BKG_PKUP_NTC" ).append("\n"); 
		query.append("        WHERE BKG_NO = @[bkg_no] " ).append("\n"); 
		query.append("          AND NTC_SEQ = (SELECT MAX(NTC_SEQ)" ).append("\n"); 
		query.append("                           FROM BKG_PKUP_NTC" ).append("\n"); 
		query.append("                          WHERE BKG_NO = @[bkg_no])) NTC " ).append("\n"); 
		...
		query.append(" AND BKGM.BKG_NO = NTC.BKG_NO (+)" ).append("\n"); 

...
*@FileName : ESM_COA_0110HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
 ...
 * @author Kim Ki Dae
 * @see EsmCoa0110Event 참조
 * @since J2EE 1.6
 */
public class ESM_COA_0110HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_COA_0110HTMLAction 객체를 생성
	 */
	public ESM_COA_0110HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmCoa0110Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EsmCoa0110Event event = new EsmCoa0110Event();  		
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)) {
			...
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkCostCommonVO((NetworkCostCommonVO)getVO(request, NetworkCostCommonVO.class));
			event.setCoaNtwkCostCreVOs((CoaNtwkCostCreVO[])getVOs(request, CoaNtwkCostCreVO.class));
		}				
		else if(command.isCommand(FormCommand.MULTI02)) {
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkCostCommonVO((NetworkCostCommonVO)getVO(request, NetworkCostCommonVO.class));
			event.setCoaSltChtrInfoVOs((CoaSltChtrInfoVO[])getVOs(request, CoaSltChtrInfoVO.class));
		}				
		else{
			event.setSearchConditionVO((SearchConditionVO)getVO(request, SearchConditionVO.class));
			event.setNetworkCostCommonVO((NetworkCostCommonVO)getVO(request, NetworkCostCommonVO.class));

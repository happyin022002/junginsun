...
*@FileName : ESM_BKG_1068HTMLAction.java
...
public class ESM_BKG_1084HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1084HTMLAction 객체를 생성
	 */
	public ESM_BKG_1084HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1084Event event = new EsmBkg1084Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchTpbInfoVOs((SearchTpbInfoVO[])getVOs(request,  SearchTpbInfoVO.class, "sheet1_"));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgNo(JSPUtil.getParameter(request, "bkg_no"));
			event.setNtcSeq(JSPUtil.getParameter(request, "ntc_seq"));
		}

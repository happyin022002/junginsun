...
public class ESM_PRI_4034HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_4034HTMLAction 객체를 생성
	 */
	public ESM_PRI_4034HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri4034Event event = new EsmPri4034Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
			
			event.setRoutLocCdVO((RoutLocCdVO)getVO(request, RoutLocCdVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			
			event.setRoutLocCdVOs((RoutLocCdVO[])getVOs(request, RoutLocCdVO .class));
		}

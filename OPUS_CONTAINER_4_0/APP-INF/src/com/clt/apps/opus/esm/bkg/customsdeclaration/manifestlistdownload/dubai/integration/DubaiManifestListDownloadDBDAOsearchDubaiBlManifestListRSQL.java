...
public class DubaiManifestListDownloadDBDAOsearchDubaiBlManifestListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("          ,COM_ConstantMgr_PKG.COM_getCompanyCode_FNC() DU_LINE_ID" ).append("\n");
		query.append("          ,BKG_GET_BKG_CTMS_CD_FNC('DU','MANI_VOY_AGN_ID',3590,1) DU_VOY_AGN_ID" ).append("\n");
		...
		query.append("                         ,CASE WHEN NVL(INSTR(C.CUST_NM, CHR(10), 1), 0) = 0 THEN SUBSTR(C.CUST_NM, 1, 48)" ).append("\n");
		query.append("                               ELSE SUBSTR(SUBSTR(C.CUST_NM, 1, INSTR(C.CUST_NM, CHR(10), 1, 1)-1), 1, 48)" ).append("\n");
		query.append("                               END CUST_NM" ).append("\n");
		query.append("                         ,CASE WHEN NVL(INSTR(C.CUST_NM, CHR(10), 1), 0) = 0 THEN SUBSTR(C.CUST_ADDR, 1, 240)" ).append("\n");
		query.append("                               ELSE SUBSTR(SUBSTR(C.CUST_NM, INSTR(C.CUST_NM, CHR(10), 1, 1)+2) || CHR(13)||C.CUST_ADDR, 1, 240) " ).append("\n");
		...
		query.append("#elseif(${cgo_type} == 'P')" ).append("\n");

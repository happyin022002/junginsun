...
public class TCharterIOInquiryDBDAOSearchCapitalBudgetSumListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("WITH V_SUMDATA AS (" ).append("\n"); 
		query.append("SELECT TI_C11" ).append("\n"); 
		query.append("     , SUM(TI_V11) TI_V11" ).append("\n"); 
		query.append("     , TI_C12" ).append("\n"); 
		query.append("     , SUM(TI_V12) TI_V12" ).append("\n"); 
		query.append("     , TO_C21" ).append("\n"); 
		query.append("     , SUM(TO_V21) TO_V21" ).append("\n"); 
		query.append("     , TO_C22" ).append("\n"); 
		query.append("     , SUM(TO_V22) TO_V22" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("SELECT DECODE(IO, 'TI', CUR01 ) AS TI_C11" ).append("\n"); 
		...
		query.append("  FROM (SELECT AGMT" ).append("\n"); 
		...
		query.append("                     , CUR02" ).append("\n"); 
		query.append("                     , CALC_POINT" ).append("\n"); 
		query.append("                     , AGMT ||'-'|| PAY_TP ||'-'|| RK ||'-'|| SUBSTR(MAX(CALC_POINT||TO_CHAR(DT, 'YYYY-MM-DD')) OVER (PARTITION BY AGMT, PAY_TP, RK ORDER BY DT ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 2, 10) AS KEY_VALUE" ).append("\n"); 
		query.append("                  FROM (SELECT T01.*" ).append("\n"); 
		query.append("                             , NVL(LEAD(DT) OVER (PARTITION BY AGMT, T02.CTRT_PAY_TERM_CD ORDER BY AGMT, DT) " ).append("\n"); 
		query.append("                             , CASE WHEN TO_DT < TO_DATE(TO_CHAR(DT, 'YYYYMMDD')||'235959', 'YYYYMMDDHH24MISS') THEN TO_DT" ).append("\n"); 
		query.append("                                    ELSE TO_DATE(TO_CHAR(DT, 'YYYYMMDD')||'235959', 'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("                               END ) AS DT2" ).append("\n"); 
		query.append("                             , T02.EFF_DT" ).append("\n"); 
		query.append("                             , T02.EXP_DT" ).append("\n"); 
		query.append("                             , T02.CTRT_PAY_TERM_CD AS PAY_TP" ).append("\n"); 
		query.append("                             , DENSE_RANK() OVER (PARTITION BY AGMT ORDER BY AGMT, EFF_DT) AS RK" ).append("\n"); 
		query.append("                             , CASE WHEN (T02.CTRT_PAY_TERM_CD = 'A') AND (TO_CHAR(T01.DT, 'DD') IN ('01', '16')) THEN 'Y'" ).append("\n"); 
		query.append("                                    WHEN (T02.CTRT_PAY_TERM_CD = 'B') THEN DECODE(MOD(ROW_NUMBER() OVER (PARTITION BY AGMT, T02.CTRT_PAY_TERM_CD ORDER BY AGMT, DT) + 14, 15), 0, 'Y', 'N')" ).append("\n"); 
		query.append("                                    WHEN (T02.CTRT_PAY_TERM_CD = 'C') AND TO_CHAR(T01.DT, 'DD') = TO_CHAR(T01.FM_DT, 'DD') THEN 'Y'" ).append("\n"); 
		query.append("                                    ELSE 'N'" ).append("\n"); 
		query.append("                               END AS CALC_POINT" ).append("\n"); 
		query.append("                          FROM (SELECT FLET_CTRT_NO AS AGMT" ).append("\n"); 
		query.append("                                     , VSL_NM" ).append("\n"); 
		...
		query.append("                                     , AMT02" ).append("\n"); 
		query.append("                                     , CUR02" ).append("\n"); 
		query.append("                                  FROM (SELECT T01.FLET_CTRT_NO" ).append("\n"); 
		...
		query.append("						                      AND C.VSL_CD IN (" ).append("\n"); 
		...
		query.append("                                                   AND Z.VSL_CD = @[vsl_cd]" ).append("\n"); 
		...
		query.append("    )" ).append("\n"); 
		query.append("GROUP BY TI_C11, TI_C12, TO_C21, TO_C22" ).append("\n"); 
		query.append(")      " ).append("\n"); 
		query.append("SELECT A.TI_C11 AS TI_CURR_CD" ).append("\n"); 
		query.append("     , A.TI_V11 AS TI_INV_AMT" ).append("\n"); 
		query.append("     , B.TI_C12 AS TI_CURR_CD2" ).append("\n"); 
		query.append("     , B.TI_V12 AS TI_INV_AMT2" ).append("\n"); 
		query.append("     , C.TO_C21 AS TO_CURR_CD" ).append("\n"); 
		query.append("     , C.TO_V21 AS TO_INV_AMT" ).append("\n"); 
		query.append("     , D.TO_C22 AS TO_CURR_CD2" ).append("\n"); 
		query.append("     , D.TO_V22 AS TO_INV_AMT2" ).append("\n"); 
		query.append("  FROM (SELECT TI_C11 FROM (" ).append("\n"); 
		query.append("        SELECT DISTINCT TI_C11 FROM V_SUMDATA WHERE TI_C11 IS NOT NULL UNION" ).append("\n"); 
		query.append("        SELECT DISTINCT TI_C12 FROM V_SUMDATA WHERE TI_C12 IS NOT NULL UNION" ).append("\n"); 
		query.append("        SELECT DISTINCT TO_C21 FROM V_SUMDATA WHERE TO_C21 IS NOT NULL UNION" ).append("\n"); 
		query.append("        SELECT DISTINCT TO_C22 FROM V_SUMDATA WHERE TO_C22 IS NOT NULL " ).append("\n"); 
		query.append("        ) ORDER BY 1) CUR" ).append("\n"); 
		query.append("     , (SELECT TI_C11 AS NKEY" ).append("\n"); 
		query.append("             , TI_C11" ).append("\n"); 
		query.append("             , SUM(TI_V11) AS TI_V11" ).append("\n"); 
		query.append("          FROM V_SUMDATA" ).append("\n"); 
		query.append("         WHERE TI_C11 IS NOT NULL" ).append("\n"); 
		query.append("         GROUP BY TI_C11 ) A" ).append("\n"); 
		query.append("     , (SELECT TI_C12 AS NKEY" ).append("\n"); 
		query.append("             , TI_C12" ).append("\n"); 
		query.append("             , SUM(TI_V12) AS TI_V12" ).append("\n"); 
		query.append("          FROM V_SUMDATA" ).append("\n"); 
		query.append("         WHERE TI_C12 IS NOT NULL" ).append("\n"); 
		query.append("         GROUP BY TI_C12 ) B" ).append("\n"); 
		query.append("     , (SELECT TO_C21 AS NKEY" ).append("\n"); 
		query.append("             , TO_C21" ).append("\n"); 
		query.append("             , SUM(TO_V21) AS TO_V21" ).append("\n"); 
		query.append("          FROM V_SUMDATA" ).append("\n"); 
		query.append("         WHERE TO_C21 IS NOT NULL" ).append("\n"); 
		query.append("         GROUP BY TO_C21 ) C" ).append("\n"); 
		query.append("     , (SELECT TO_C22 AS NKEY" ).append("\n"); 
		query.append("             , TO_C22" ).append("\n"); 
		query.append("             , SUM(TO_V22) AS TO_V22" ).append("\n"); 
		query.append("          FROM V_SUMDATA" ).append("\n"); 
		query.append("         WHERE TO_C22 IS NOT NULL" ).append("\n"); 
		query.append("         GROUP BY TO_C22 ) D" ).append("\n"); 
		query.append(" WHERE CUR.TI_C11 = A.NKEY (+)" ).append("\n"); 
		query.append("   AND CUR.TI_C11 = B.NKEY (+)" ).append("\n"); 
		query.append("   AND CUR.TI_C11 = C.NKEY (+)" ).append("\n"); 
		query.append("   AND CUR.TI_C11 = D.NKEY (+)" ).append("\n"); 

...
public class GeneralBookingReceiptBCImpl extends BasicCommandSupport implements GeneralBookingReceiptBC {
	 ...
	 */
	 public void modifyChgRateBkgBooking(RateMainInfoVO rateMainInfoVO, String caflag)throws EventException{  
		    ...
		    try {
		    	...
		    	//for updating CNPT with contract
		    	PriRpHdrVO priRpHdrVO = new PriRpHdrVO();
		    	priRpHdrVO.setScNo(rateMainInfoVO.getScNo1());
		    	priRpHdrVO.setRfaNo(rateMainInfoVO.getRfaNo());
		    	priRpHdrVO.setTaaNo(rateMainInfoVO.getTaaNo());

		    	PriSpCtrtPtyVO ctrtPtyVO = searchBkgCustCntInfo(priRpHdrVO);
		    	if(ctrtPtyVO!=null){
		    		rateMainInfoVO.setBkgCtrlPtyCustCntCd(ctrtPtyVO.getCustCntCd());
		    		rateMainInfoVO.setBkgCtrlPtyCustSeq(ctrtPtyVO.getCustSeq());
		    	}
	 ...
	 * @exception EventException
	 */
	public void modifyBlTypeHistory(String bkgNo ,String blTpCd) throws EventException{   
	    try {	    	
	    	 dbDao.modifyBlTypeHistory(bkgNo,blTpCd);	    	 
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("BKG00391", new String[] {}).getMessage(), ex);
		} catch (Exception ex) {
	...
	public PoOtherNoVO searchPoOtherNo(PoOtherNoVO rPoOtherNoVO) throws EventException {
		...
		String caFlg = "";
		try {
	        bkgBlNoIN.setBkgNo(poOtherNoBkgVO.getBkgNo());
	        bkgBlNoIN.setCaUsrId(rPoOtherNoVO.getAccount().getUsr_id());
			
			BkgBlNoVO bkgBlNoVO = util.searchBkgBlNoVO(bkgBlNoIN);
			if(bkgBlNoVO != null){
				caFlg = bkgBlNoVO.getCaFlg();
			}
			poOtherNoBkgVO.setCaFlg(caFlg);
			...
			if(rPoOtherNoVO.isFirst()){
				...
				if(rPoOtherCntrVOs.size()>0){
					...
					if(rPoOtherCntrVOs.size()>dbDao.searchCntrCount(poOtherNoBkgVO)){ //Container No is duplicated in BKG_REFERENCE.
						rPoOtherNoVO.setCntrNoDuplicated("Y");
					}
				}
	...
	public PoOtherNoVO searchCmForPo(PoOtherNoVO rPoOtherNoVO) throws EventException {
		...
		log.debug("============================>[[ GeneralBookingReceiptBCImpl  searchCmForPo END ]]<============================");
			...
			if( poOtherCntrVOs != null ){
				...
				for(int i = 0; i < poOtherCntrVOs.length; i++) {
					if(poOtherCntrVOs[i].getRefSeq().equals("")) {
						...
						if(poOtherCntrVOs[i].getIbflag().equals("D")){
			...
			if( poOtherCmVO != null ){
				...
				for(int i = 0; i < poOtherCmCnt; i++) {
					if(poOtherCmVO[i].getRefSeq().equals("")) {
						log.debug("======>>> PO & Other No占쎈퓠 CM占쎌젟癰귣�占쏙옙 Copy占쎈릭疫뀐옙 占쎌맄占쎈퉸   ADD:: "+bkgNo+bkgNoSplit);
	 ...
	 * @exception EventException
	 */
	public void manageMRNNoUCRNo(PoOtherNoVO poOtherNoVO, String caFlg) throws EventException{
		...
		try {
			...
			if( poOtherMrnUcrVO != null ){
				...
				for(int i = 0; i < poOtherMrnUcrCnt; i++) {
					poOtherMrnUcrVO[i].setCaFlg(caFlg);
				...
				//CUST_REF_NO_CTNT 揶쏅�れ뵠 NULL 占쎌뵥 占쎈쑓占쎌뵠占쎄숲占쎈뮉 占쎄텣占쎌젫占쎈립占쎈뼄.(�겫�뜇釉섓옙�뒄占쎈립 占쎈쑓占쎌뵠占쎄숲)
				bkgRefVO.setRefSeq("");
				bkgRefVO.setCaFlg(caFlg);
...
//					befEtaDt = etaDt;
	...
	/**
	 * search system cct information<br>
	 * @param bkgNo
	 * @param clzTpCd
	 * @return
	 * @throws EventException
	 */
	public String serchSystemCargoClosingTime(String bkgNo, String clzTpCd) throws EventException {
		try {
			//cargo closing system time
			return dbDao.ssearchSystemCargoClosingTime(bkgNo, clzTpCd);
		} catch (DAOException de) {
			log.error("err " + de.toString(), de);
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
		} catch (Exception ex) {
...
//						bookingCreationVO.setBkgBookingInfoVO(bkgBookingInfoVO);
	...
	/**
	 * assess the suitability of input values�좎럥肉℡뜝�숈삕. in case of creation / modifications Booking(ESM_BKG_0079_01)<br>
	...
	public BookingCreationVO validateBookingSave(BookingCreationVO bookingCreationVO, SignOnUserAccount account) throws EventException {
		...
		try {
			...
			if(!bkgBookingInfoVO.getBkgPolCd().isEmpty() && !bkgBookingInfoVO.getBkgTrunkVvd().isEmpty()){
				BookingMasterMgtBC command = new BookingMasterMgtBCImpl();
				VskVslPortSkdConditionVO vo = new VskVslPortSkdConditionVO();
				vo.setVpsPortPol(bkgBookingInfoVO.getBkgPolCd());
				vo.setVslCd(bkgBookingInfoVO.getBkgTrunkVvd());
				List<VskVslPortSkdConditionVO> list = command.searchEtbEtdEta(vo);
				if(list.size() < 1) {
					...
					if(!"Y".equals(bkgBlNoVO.getBdrFlg())){
						if(!bkgBookingInfoVO.getPorCdOld().equals(porCd)
							...
							if("Y".equals(util.searchSoStatus("", "", bkgBlNoVO, "O"))){
								if(!"E0000".equals(util.searchPrdReplanCode(bkgBookingInfoVO))){
...
//					}
				...
				if("Y".equals(bkgBookingInfoVO.getDcgoFlg()) || "Y".equals(bkgBookingInfoVO.getRcFlg()) 
					|| "Y".equals(bkgBookingInfoVO.getAwkCgoFlg()) || "Y".equals(bkgBookingInfoVO.getBbCgoFlg()) || "Y".equals(bkgBookingInfoVO.getStwgFlg())){
	...
	public void createBooking(BookingCreationVO bookingCreationVO, SignOnUserAccount account) throws EventException {
		try {
			...
			if(bkgBookingInfoVO != null){
				...
				if((bkgBookingInfoVO.getBkgPtyCntCd() == null || bkgBookingInfoVO.getBkgPtyCntCd().equals(""))
					|| (bkgBookingInfoVO.getBkgPtyCustSeq() == null || bkgBookingInfoVO.getBkgPtyCustSeq().equals(""))
						){
					bkgBookingInfoVO.setBkgPtyCntCd(blCustomerInfoVO.getFCustCntCd());
					bkgBookingInfoVO.setBkgPtyCustSeq(blCustomerInfoVO.getFCustSeq());
				}
	...
	public BookingSaveValidationVO modifyBooking(BookingCreationVO bookingCreationVO, SignOnUserAccount account) throws EventException {
		try {
			...
			if(bkgBookingInfoVO != null){				
				...
				String vvdLstCreDt = "";
				if(bkgBookingInfoVO.getCaFlg().equals("Y")){
					vvdLstCreDt = dbDao.searchLastVVDHisCreTime(bkgNo);
				}
...
//				}
				...
				if("Y".equals(saveValidationVO.getRouteModifyFlag()) || "Y".equals(saveValidationVO.getQtyModifyFlag()) || "Y".equals(bkgBookingInfoVO.getModifyCargoFlg())){
					if(bkgBlNoVO.getPctlNo() != null && bkgBlNoVO.getPctlNo().length() > 0 && !"FAIL".equals(bkgBlNoVO.getPctlNo().substring(0, 4)) && !"Over T/S".equals(bkgBlNoVO.getPctlNo())){
						dbDao.modifyBkgRouteFromPrd(bkgBlNoVO, "U");
						dbDao.modifybkgMtPickUpYardDate(bkgBlNoVO, userId);
					}
				}
				...
				if("Y".equals(saveValidationVO.getCustomerModifyFlag())){
					...
					//by kimtaekyun 2016.03.30. BKG_INV_TAX_IF ��옣
					if(shFlag || fwFlag || cnFlag){
						...
						dbDao.addBkgInvTaxIF(bkgChgRateVO, account);
					}
				}
				...
				saveValidationVO.setSpclCgoApproval("N");
					...
					if("Y".equals(saveValidationVO.getRouteModifyFlag())){
						...
						if(vslSkdVOs != null){
							if(oldVslSkdVOs.size() != vslSkdVOs.length){
								saveValidationVO.setSpclCgoApproval("Y");
							}else{
				...
				if(!oldBkgInfoForPctlNo.getDelCd().equals(bkgBookingInfoVO.getBkgDelCd()) 
						|| !oldBkgInfoForPctlNo.getPodCd().equals(bkgBookingInfoVO.getBkgPodCd())
						|| !oldBkgInfoForPctlNo.getSlanCd().equals(bkgBookingInfoVO.getSlanCd())
						){
					String bsCode = dbDao.searchBlockStowage(bkgBookingInfoVO);
					bkgBookingInfoVO.setBlckStwgCd(bsCode);	
//					if(bsCode != null && !bsCode.equals("")){
						dbDao.modifyBookingBlckStwgCd(bkgBookingInfoVO);
//					}
				}
				...
				if(bkgQuantityVOs != null){
					...
					for(int i = 0 ; i < bkgQuantityVOs.length ; i++){
						...
						int modifyCnt = dbDao.modifyBkgQuantity(bkgQuantityVO, bkgBlNoVO);
						if( modifyCnt == 0){
							dbDao.addBkgQuantity(bkgQuantityVO, bkgBlNoVO);
						}
					}
				...
				if(bkgQtyDtlVOs != null){
					if(bkgQuantityVOs == null) dbDao.removeBkgQtyDtl(bkgBlNoVO, null);
				...
				if("Y".equals(saveValidationVO.getRouteModifyFlag())){
					...
					if(befFirstVvd!=null && newFirstVvd!=null && befFirstVvd.length() ==9 && newFirstVvd.length() == 9){
						if(!befFirstVvd.equals(newFirstVvd)){
							if(!CheckUtilities.isNull(vvdLstCreDt)){
								dbDao.removeCARollOvr(bkgBlNoVO.getBkgNo(), vvdLstCreDt);
							}
				...
				// Container VL check
				String vlFlg ="";
	            vlFlg = dbDao.searchVlFlg(bkgNo);
				if("Y".equals(vlFlg)){
					throw new EventException(new ErrorHandler("BKG00317", new String[]{}).getMessage());
				}
...
//			}
			...
			if(isContinue){
				...
				for(int i = 0 ; i < mvmtStsCd.length ; i++){
					if("VL".equals(mvmtStsCd[i])){
						if(validation && (newStsCd != null && !newStsCd.equals("F"))){
			...
			if(isContinue){
				...
				if(!"Y".equals(spclFlag)){//spcl�좎럩逾��좎럥�ⓨ뜝�뚮쾳
					spclFlag = "N";
				} else {
...
//				log.debug("\nold pending flag : " + dbDao.searchUserHold(bkgBlNoVO));
				...
				dbDao.modifyBookingStatus(pendingFlag, spclFlag, newStsCd, bkgBlNoVO, account, oldBkgStsCd);
				
				
				//BKG_DOC_PROC_SKD�좎럩踰�'BKGSTG' �좎럡留믣뜝�꾨Ф �좎럥�볟뜝�뚮턄�좎럡���좎럡臾멨뜝�꾩뎽
				utilBC.manageBkgStateFirm(newStsCd, bkgBlNoVO.getBkgNo(), account);
				return true;
			} else {
				return false;
			}
...
//			}
			...
			// �좎럥�멨뜝�덈샬 Booking�좎럩逾�DRY Cargo �좎럩逾η춯�묒삕 �좎럥�▼뜝�덈퉺嶺뚯쉻�숋옙紐닷뜝占�嶺뚳퐢�얍칰類㏃삕�됰냲�숋옙堉�
    		...
    		for(int i=0;i<targetBkg.length;i++){	
    			...
    			bkgBookingVO.setBlNo(targetBkg[i].getBlNo());
    			...
    			if (!sourceBkg.getBkgNo().equals(splitBkgVO.getSplitBlInfoVO().get(i).getBkgNo())){
	    			...
	    			for(int idx = 0; idx < bkgVvdVOList.size() ; idx++){
						...
						bkgVvdVO.setSplitFlg(splitFlg);
						bkgVvdVO.setSourceBkgNo(sourceBkg.getBkgNo());
...
//			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
	...
	public void copyBooking(BookingCopyVO bookingCopyVO, BkgBlNoVO sourceBkg, BkgBlNoVO targetBkg, SignOnUserAccount account) throws EventException {
		try {
			...
			if(bkgBookingVO.getIrrBlNoFlg() != null && bkgBookingVO.getIrrBlNoFlg().equals("Y")){
				BkgBlNoVO toyotaBlNoVO = new BookingUtil().manageToyotaBlNumberGeneration("TYB", account.getOfc_cd(), account.getUsr_id());
				bkgBookingVO.setBlNo(toyotaBlNoVO.getBlNo());
			}
	...
	public void modifyBkgVvdForVslSkdCng(List<VslSkdCngUpdateVO> vslSkdCngUpdateVOs, SignOnUserAccount account)throws EventException{
		try {
			...
			for(int icnt=0;icnt<vslSkdCngUpdateVOs.size();icnt++){
				...
				/* #15362 DEL yard changed without history */
				List<BkgBlNoVO> bkgBlNoVOs = dbDao.searchBkgRouteForVslSkdCng(vslSkdCngUpdateVO);
				for (int i = 0; i < bkgBlNoVOs.size(); i++) {
					HistoryTableVO historyTableVO = historyBC.searchOldBkgForHistory("VOP_VSK_0015", bkgBlNoVOs.get(i));
					vslSkdCngUpdateVO.setBkgNo(bkgBlNoVOs.get(i).getBkgNo());
					dbDao.modifyBkgRouteForVslSkdCng(vslSkdCngUpdateVO, account);
					historyBC.manageBookingHistory("VOP_VSK_0015", historyTableVO, account);
				}
				/* #15362 DEL yard changed without history */
...
//
    ...
    /**
     * SVC_SCP_CD占쎈ご�좑옙 �브퀗�э옙�녑뜝�덈┰�좎럥堉�
	...
	/**
	 * BKG_Quantity �좎럩�잏솻洹ｏ옙�좎룞���브퀗�э옙�녑뜝�덈┰�좎럥堉�
	 * @param bkgBlNoVO
	 * @return List<BkgQuantityVO>
	 * @throws EventException
	 */
	...
	/**
	 * Allocation Status占쎈ご�좑옙 manage�좎럥由썲뜝�덈펲.<br>
	 * 
	 * @param BkgBlNoVO bkgblNoVO
	 * @param SignOnUserAccount account
	...
	public AllocStsChgVO manageAlocStatus(BkgBlNoVO bkgBlNoVO, SignOnUserAccount account, String docTpCd) throws EventException{
		...
		try {
			...
			// PKGSC User �띠룊��SI �좎럥由��좎럥瑜닷뜝�덊뱺�좎럥裕�Standby BKG Process �좎럩�끻뜝�뚮뮔 �좎럥���좎럥留�
//			if(docTpCd != null && ("S".equals(docTpCd))||"U".equals(docTpCd)||"B".equals(docTpCd)){ // 0229_01 �좎럥�졾뜝�꾪맋 �좎럩源덌옙鍮듿뜝占�
			if(docTpCd != null && !"".equals(docTpCd)){ // 0229_01 �좎럥�졾뜝�꾪맋 �좎럩源덌옙鍮듿뜝占�
			...
			if( allocStsVO != null && allocStsVO.getAlocStsCd()!=null && "S".equals(allocStsVO.getAlocStsCd()) && "N".equals(alocPopFlg)
				...
				// popUp �좎럥�㎩뜝�뚮첎嶺뚮∥�삭땻占�""->S, F->S �좎럩逾��좎럥瑜�Stanby Notice �꾩룇裕뉛옙苑�
...
//				// S->F �좎럩逾��좎럥瑜�Booking Reciept Notice �꾩룇裕뉛옙苑�
//				// rcvrEml �좎럥���띠룊���좎럩肉녑뜝�뚮さ嶺뚮〕��SC �좎럥�졾뜝�꾪맋 Booking Reciept Notice �꾩룇裕뉛옙苑�
//				if(rcvrEml != null && rcvrEml.length() > 0){
//					allocStsChgVO.setRcvrEml(rcvrEml);
//				}
				firmMsgFlg = "Y";// Reciep Notice �좎룞�쇿뜝�덈뼁 嶺뚮∥�꾬옙六사춯�묒삕 �곌랜�삭굢�鍮녑뜝占�
				allocStsChgVO.setNtcKndCd("BK");// SC �좎럥�졾뜝�꾪맋 Customer301, Vendor301
			}		
	...
	/**
	 * EXPORT REFERENCES占쎈ご�좑옙 �브퀗�э옙�녑뜝�덈┰�좎럥堉�
	...
	/**
	 * EXPORT REFERENCES占쎈ご�좑옙 �브퀗�э옙�녑뜝�덈┰�좎럥堉�(BY CNTR)
	 * @param ExportReferencesVO exportReferencesVO
	 * @return List<ExportReferencesVO>
	 * @exception EventException
	 */
	...
	/**
	 * roll over �좎럩�잏솻洹ｏ옙�좎룞��update<br>
	 * 
	 * @author 		
...
//					  log.error(bkgRollOvrVOs[i]);
	...
	/**
	 * eBooking S/I�좎럥�졾뜝�꾪맋 BKG�좎럩紐드슖�쎌삕 �좎럥�쒎슖�る츊獄�씛�숋쭕占�Customer Name & Address�좎럩踰�Overlfow �좎럥�곤옙寃ュ뜝�밸ご�좑옙 �브퀗�э옙�녑뜝�덈┰�좎럥堉�
	 *
	...
	/**
	 * Booking cutomer tab�좎럥�졾뜝�꾪맋 cleared�좎럥由�customer �좎럩�잏솻洹ｏ옙�좎룞���좎럥�쒎뜝�덉몥�좎럩逾졾뜝�덈콦 �좎럥由썲뜝�덈펲.
	 *
	 * @param XterCustOvflwFlgStatusVO xterCustOvflwFlgStatusVO
	 ...
	 * @throws EventException
	 */
	public PriSpCtrtPtyVO searchBkgCustCntInfo(PriRpHdrVO priRpHdrVO) throws EventException{
		try {
			return dbDao.searchBkgCustCntInfo(priRpHdrVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	...
	/**
   	 * Check Legacy system VVD
   	 * @param String vvd
   	 * @throws EventException
   	 */
   	public void checkLegacySystemVVD (String vvd)  throws EventException {
		
		try {
			BkgHrdCdgCtntListCondVO bkgHrdCdgCtntListCondVO = new BkgHrdCdgCtntListCondVO();
			BookingUtil util = new BookingUtil();
			
			bkgHrdCdgCtntListCondVO.setHrdCdgId("OSCAR_VVD_CHK");
			bkgHrdCdgCtntListCondVO.setAttrCtnt4(vvd.substring(0,4));
			bkgHrdCdgCtntListCondVO.setAttrCtnt5(vvd.substring(4,8));
			bkgHrdCdgCtntListCondVO.setAttrCtnt6(vvd.substring(8,9));
			
			List<BkgHrdCdgCtntVO> list = util.searchHardCoding(bkgHrdCdgCtntListCondVO);
			if(list.size() > 0){
				throw new EventException((String)new ErrorHandler("BKG00628", new String[]{vvd+" is OSCAR VVD. OSCAR VVD handling is not allowed."}).getMessage());
			}
		} catch (EventException ex) {
			throw ex;								
		} catch (Exception de) {
   	...
   	/**
	 * Return Booking Trunk VVD
	 * @param bkgNo
	 * @return String
	 * @throws EventException
	 */
	public String searchTrunkVvdByBkg(String bkgNo) throws EventException{
		try {
			BkgBlNoVO bkgBlNoVO = new BkgBlNoVO();
			bkgBlNoVO.setBkgNo(bkgNo);
			return dbDao.searchTrunkVvdByBkg(bkgBlNoVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	...
	/**
   	 * remove roll over information after CA issue when cancel CA
   	 * @param String bkgNo
   	 * @throws EventException
   	 */
   	public void removeCARollOvr (String bkgNo)  throws EventException {
		
		try {
			String vvdLstCreDt = dbDao.searchLastVVDHisCreTime(bkgNo);
			dbDao.removeCARollOvr(bkgNo, vvdLstCreDt);
			dbDao.modifyCARollOvr(bkgNo, vvdLstCreDt);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	...
	/**
	 * Check Port Cut-off date<br>
	 * @param  BkgClzTmVO[] bkgClzTmVOs
	 * @return String
	 * @exception EventException
	 */
	public String searchPortCutOffChange(BkgClzTmVO[] bkgClzTmVOs) throws EventException {
		String rtnValue = "";
		try {
			
			for ( int i=0; i<bkgClzTmVOs.length; i++ ) {
				if ("T".equals(bkgClzTmVOs[i].getClzTpCd())){
					rtnValue = dbDao.searchPortCutOffChange(bkgClzTmVOs[i]);
					break;
				}
			}
	...
	/**
	 * Keep cut-off dates before update
	 * @param String bkgNo
	 * @return List<SearchCutOffDateVO>
	 * @throws EventException
	 */
	public List<SearchCutOffDateVO> searchCutOffDate(String bkgNo) throws EventException{
		try {
			return dbDao.searchCutOffDate(bkgNo);
		} catch (DAOException de) {
			throw new EventException(new ErrorHandler("BKG40121", new String[] {de.getMessage().replaceAll("<\\|\\|>", ":")}).getMessage(), de);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("BKG40121", new String[] {ex.toString() + "\n" + ex.getMessage() + "\n" + getClass().getSimpleName() + "." + Thread.currentThread().getStackTrace()[1].getMethodName() + "\n"  + new Date().toString()}).getMessage());
	 ...
	 * @throws EventException
	 */
	public String searchCutOffDateChange(SearchCutOffDateVO cutOffDateVO) throws EventException{
		try {
			return dbDao.searchCutOffDateChange(cutOffDateVO);
		} catch (DAOException de) {
			throw new EventException(new ErrorHandler("BKG40121", new String[] {de.getMessage().replaceAll("<\\|\\|>", ":")}).getMessage(), de);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("BKG40121", new String[] {ex.toString() + "\n" + ex.getMessage() + "\n" + getClass().getSimpleName() + "." + Thread.currentThread().getStackTrace()[1].getMethodName() + "\n"  + new Date().toString()}).getMessage());
	...
	/**
	 * Bkg���대떦�섎뒗 BKG_REFERENCE 以묒뿉 UCR ��cntr_no瑜��낅뜲�댄듃��
	 * @param BkgRefDtlVO bkgRefDtlVO
	 * @param SignOnUserAccount account
	 * @return String
	 * @throws EventException
	 */
	public String modifyCntrNoUCR(BkgRefDtlVO bkgRefDtlVO, SignOnUserAccount account) throws EventException{
		String chkFlg = "";
		String qtyMapFlg = "";
		String reValue = "N";
		
		try {
			qtyMapFlg = dbDao.searchQtyForUcrMap(bkgRefDtlVO.getBkgNo());
			chkFlg = dbDao.checkUcrNo(bkgRefDtlVO.getBkgNo());
			
			if("Y".equals(chkFlg)&& "Y".equals(qtyMapFlg)){
				dbDao.modifyCntrNoUCR(bkgRefDtlVO, account);
				reValue="Y";
			}
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
	...
	/**
	 * CTM�먯꽌 Container 媛�attach �섎뒗 寃쎌슦 Vol=1 ��寃쎌슦���대떦�섎뒗 BKG_REFERENCE 以묒뿉 UCR ��cntr_no瑜��낅뜲�댄듃��
	 * @param BkgRefDtlVO bkgRefDtlVO
	 * @param String usrId
	 * @throws EventException
	 */
	public void modifyCntrNoUcrByMvmt(BkgRefDtlVO bkgRefDtlVO, String usrId) throws EventException{
		String chkFlg = "";
		String qtyMapFlg = "";
		...
		try {
				...
				if("Y".equals(chkFlg)&& "Y".equals(qtyMapFlg)){
					dbDao.modifyCntrNoUcrByMvmt(bkgRefDtlVO, usrId);
				}
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
	 ...
	 * @throws EventException
	 */
	public String toyotaBlNoCheck(BkgBookingInfoVO bkgBookingInfoVO, BlCustomerInfoVO custVo) throws EventException {
		try {
			return dbDao.toyotaBlNoCheck(bkgBookingInfoVO, custVo);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	 ...
	 * @throws EventException 
	 */
	public void addBkgDocProcSkd(BkgDocProcSkdVO bkgDocProcSkdVO) throws EventException{
		try {
	...
	/**
	 * BKG_INV_TAX_IF���곗씠���앹꽦<br>
	 * 
	 * @param BkgChgRateVO bkgChgRateVO
	 * @param SignOnUserAccount account
	 * @exception EventException
	 */
    public void addBkgInvTaxIF(BkgChgRateVO bkgChgRateVO, SignOnUserAccount account) throws EventException{
    	try{
    		dbDao.addBkgInvTaxIF(bkgChgRateVO, account);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
	...
	/**
	 * Create data in BKG_ROLL_OVR<br>
	 * @param 		BkgBlNoVO bkgBlNoVO
	 * @param 		SignOnUserAccount account
	 * @exception 	EventException
	 */
	public void addBkgRollOvr(BkgBlNoVO bkgBlNoVO, SignOnUserAccount account) throws EventException {
		try {
			dbDao.modifyBkgRollOvr(bkgBlNoVO, account); //Update NEW_ columns of BKG_ROLL_OVER
			dbDao.addBkgRollOvr(bkgBlNoVO, account);	//Insert a new row to BKG_ROLL_OVR for next roll over operation
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	...
	/**
	 * update BkgVgmWgtVO <br>
	 * @param 		BkgVgmWgtVO BkgVgmWgtVO
	 * @param 		SignOnUserAccount account
	 * @exception 	EventException
	 */
	public void modifyBkgVgmWgt(BkgVgmWgtVO bkgVgmWgtVO, SignOnUserAccount account) throws EventException {
		try {
			dbDao.modifyBkgVgmWgt(bkgVgmWgtVO, account); 
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	 ...
	 * @exception 	EventException
	 */
	public void modifyCheckOutBkg(RepoBkgForUpdateVO repoBkgForUpdateVO, SignOnUserAccount account) throws EventException {
		try {
				
			dbDao.modifyCheckOutBooking(repoBkgForUpdateVO, account);		
	 ...
	 * @exception 	EventException
	 */
	public void modifyCheckInBkg(RepoBkgForUpdateVO repoBkgForUpdateVO, SignOnUserAccount account) throws EventException {
		try {
				
			dbDao.modifyCheckInBooking(repoBkgForUpdateVO, account);
			dbDao.modifyMTBookingNoToMovement(repoBkgForUpdateVO, account);
			dbDao.modifyBkgContainerbyMovement(repoBkgForUpdateVO, account);
			//2016-06-02 Park Young Jin 추가
			dbDao.modifyMTBookingCtmbyMovement(repoBkgForUpdateVO, account);
	 ...
	 * @throws EventException
	 */
	public void bkgCustremarkSave(BkgBookingInfoVO bkgBookingInfoVO) throws EventException {
		try {
			 dbDao.bkgCustremarkSave(bkgBookingInfoVO);		
		} catch (DAOException de) {
			throw new EventException(new ErrorHandler("COM12240",new String[]{}).getMessage(),de);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("COM12240",new String[]{}).getMessage(),ex);
   	...
   	/**
	 * Return block stowage code
	 * @param bkgNo
	 * @return String
	 * @throws EventException
	 */
	public String searchBlockStowage(String bkgNo) throws EventException{
		try {
			BkgBookingInfoVO bkgBookingInfoVO = new BkgBookingInfoVO();
			bkgBookingInfoVO.setBkgNo(bkgNo);
			return dbDao.searchBlockStowage(bkgBookingInfoVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), de);
	 ...
	 * @throws EventException
	 */
	public void modifyBookingBlckStwgCd(String bkgNo, String blckStwgCd) throws EventException{
		try {
			BkgBookingInfoVO bkgBookingInfoVO = new BkgBookingInfoVO();
			bkgBookingInfoVO.setBkgNo(bkgNo);
			bkgBookingInfoVO.setBlckStwgCd(blckStwgCd);
			dbDao.modifyBookingBlckStwgCd(bkgBookingInfoVO);
		} catch (DAOException ex) {
			throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
		} catch (Exception de) {
	 ...
	 * @throws EventException
	 */
	public void bkgPctlNoSync(String bkgNo, String pctlNo, String caFlg) throws EventException {
		try {
			 dbDao.bkgPctlNoSync(bkgNo, pctlNo, caFlg);		
		} catch (DAOException de) {
			throw new EventException(new ErrorHandler("COM12240",new String[]{}).getMessage(),de);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler("COM12240",new String[]{}).getMessage(),ex);

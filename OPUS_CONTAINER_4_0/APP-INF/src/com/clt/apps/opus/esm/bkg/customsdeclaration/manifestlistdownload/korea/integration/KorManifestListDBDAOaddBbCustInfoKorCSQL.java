...
*@FileName : KorManifestListDBDAOaddBbCustInfoKorCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class KorManifestListDBDAOaddBbCustInfoKorCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public KorManifestListDBDAOaddBbCustInfoKorCSQL(){
		setQuery();
		...
		query.append("FileName : KorManifestListDBDAOaddBbCustInfoKorCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("INSERT INTO BKG_CSTMS_KR_CUST " ).append("\n"); 
		...
		query.append("'', '', COM_ConstantMgr_PKG.COM_getCompanyName_FNC(), " ).append("\n"); 
		query.append("                   (SELECT " ).append("\n"); 
		query.append("                            CASE " ).append("\n"); 
		query.append("                            WHEN @[in_bound] = 'I' OR @[in_bound] = 'T' THEN" ).append("\n"); 
		query.append("									DECODE(@[bcs_tp], 'S', DECODE(INSTR(A2.LOC_NM, B2.CNT_NM), 0, A2.LOC_NM || ', ' || B2.CNT_NM, A2.LOC_NM )," ).append("\n"); 
		query.append("                                    DECODE(INSTR(A1.LOC_NM, B1.CNT_NM), 0, A1.LOC_NM || ', ' || B1.CNT_NM, A1.LOC_NM ))" ).append("\n"); 
		query.append("                            ELSE    " ).append("\n"); 
		query.append("									DECODE(@[bcs_tp], 'S', DECODE(INSTR(A1.LOC_NM, B1.CNT_NM), 0, A1.LOC_NM || ', ' || B1.CNT_NM, A1.LOC_NM )," ).append("\n"); 
		query.append("									DECODE(INSTR(A2.LOC_NM, B2.CNT_NM), 0, A2.LOC_NM || ', ' || B2.CNT_NM, A2.LOC_NM ))" ).append("\n"); 
		query.append("                            END                                                                                                        " ).append("\n"); 
		query.append("                    FROM BKG_CSTMS_KR_BL BB,MDM_LOCATION A1, MDM_COUNTRY B1, MDM_LOCATION A2, MDM_COUNTRY B2" ).append("\n"); 
		...
		query.append("                    )," ).append("\n"); 
		query.append("'', @[kt_port], @[username], @[username])" ).append("\n"); 

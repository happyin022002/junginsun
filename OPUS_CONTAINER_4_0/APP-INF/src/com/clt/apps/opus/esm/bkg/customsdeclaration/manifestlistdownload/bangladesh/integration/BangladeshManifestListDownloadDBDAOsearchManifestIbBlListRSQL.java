...
public class BangladeshManifestListDownloadDBDAOsearchManifestIbBlListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       DECODE(BK.DEL_CD,'BDCGP',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||'ICD'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00')))," ).append("\n"); 
		query.append("                                                                                                              '2'," ).append("\n"); 
		query.append("       DECODE(BK.DEL_CD,'BDCGP',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||'ICD'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'000')))," ).append("\n"); 
		query.append("                                                                                                              '3'," ).append("\n"); 
		query.append("       DECODE(BK.DEL_CD,'BDCGP',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'0000'))," ).append("\n"); 
		...
		query.append("                        'BDDAC',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||'ICD'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00')))," ).append("\n"); 
		query.append("       DECODE(BK.DEL_CD,'BDCGP',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00'))," ).append("\n"); 
		query.append("                        'BDDAC',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||'ICD'||TRIM(TO_CHAR(DENSE_RANK() OVER (PARTITION BY  BK.DEL_CD ORDER BY BK.BL_NO),'00')))) LINE_NO," ).append("\n"); 
		query.append("       COM_ConstantMgr_PKG.COM_getScacCode_FNC()||BK.BL_NO BL_NO," ).append("\n"); 
		...
		query.append("       TRIM(REPLACE(REPLACE(SUBSTR(REPLACE(REPLACE(REPLACE(NVL(BL_MARK.MK_DESC,''),CHR(10),' '),'',' '),CHR(9),''),1,700),CHR(34),''),' ',' ')) MARKS," ).append("\n"); 
		query.append("       TRIM(REPLACE(REPLACE(SUBSTR(REPLACE(REPLACE(REPLACE(NVL(DES.CSTMS_DESC,''),CHR(10),' '),'',' '),CHR(9),''),1,200),CHR(34),''),' ',' ')) GOODS_DESC," ).append("\n"); 
		query.append("       '' GOODS_DATE," ).append("\n"); 
		query.append("       LIC_C.CUST_LIC_NO CONS_LICE, " ).append("\n"); 
		query.append("       REPLACE(SUBSTR(REPLACE(REPLACE(REPLACE(NVL(C.CUST_NM,' '),CHR(10),' '),'',' '),CHR(9),' '),1,150)||SUBSTR(REPLACE(REPLACE(REPLACE(NVL(C.CUST_ADDR,' '),CHR(10),' '),'',' '),CHR(9),' '),1,100),' ',' ') CONS_NM," ).append("\n"); 
		query.append("       LIC_N.CUST_LIC_NO NOTY_LICE," ).append("\n"); 
		query.append("       REPLACE(SUBSTR(REPLACE(REPLACE(REPLACE(NVL(N.CUST_NM,' '),CHR(10),' '),'',' '),CHR(9),' '),1,150)||SUBSTR(REPLACE(REPLACE(REPLACE(NVL(N.CUST_ADDR,' '),CHR(10),' '),'',' '),CHR(9),' '),1,100),' ',' ') NOTY_NM," ).append("\n"); 
		...
		query.append("      BKG_CSTMS_BD_FRT_FWRD_LIC LIC_C," ).append("\n"); 
		query.append("      BKG_CSTMS_BD_FRT_FWRD_LIC LIC_N " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("WHERE 1=1 " ).append("\n"); 
		...
		query.append("AND  NVL(DG.CNTR_CGO_SEQ,1)  = 1" ).append("\n"); 
		...
		query.append("AND  VVD.SKD_DIR_CD   = SUBSTR(@[vvd],9,1)" ).append("\n"); 
		query.append("AND  VVD.POD_CD 	  = @[pod_code]" ).append("\n"); 
		query.append("AND  C.CUST_CNT_CD    = LIC_C.CNT_CD(+)" ).append("\n"); 
		query.append("AND  C.CUST_SEQ       = LIC_C.CUST_SEQ(+)" ).append("\n"); 
		query.append("AND  N.CUST_CNT_CD    = LIC_N.CNT_CD(+)" ).append("\n"); 
		query.append("AND  N.CUST_SEQ       = LIC_N.CUST_SEQ(+)" ).append("\n"); 
		query.append("#if (${pod_yd} != '') " ).append("\n"); 
		query.append("	AND SUBSTR(VVD.POD_YD_CD,6,2) like @[pod_yd]||'%'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${pol_code} != '') " ).append("\n"); 

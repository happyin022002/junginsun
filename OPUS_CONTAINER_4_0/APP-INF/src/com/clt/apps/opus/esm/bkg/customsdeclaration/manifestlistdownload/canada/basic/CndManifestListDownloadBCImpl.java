 ...
 * @since J2EE 1.4
 */
public class CndManifestListDownloadBCImpl extends BasicCommandSupport implements CndManifestListDownloadBC {
	...
	public List<ManifestListDetailVO> searchManifestList(ManifestListCondVO manifestListCondVO, SignOnUserAccount account) throws EventException {
		try
		{
			...
			{
				return dbDao.searchCndCstmsMfList(condVO, account);
			}
			else if ("sheet2".equals(condVO.getSheetId()))
			{
				...
				// CRN check
				if (dbDao.checkIfCCNAval(condVO.getVslCd(), condVO.getSkdVoyNo(), condVO.getSkdDirCd(), condVO.getPodCd()))
				{
	...
	public String manageManifest(ManifestListDetailVO[] manifestListDetailVOs, SignOnUserAccount account) throws EventException {
		...
		try {
			...
			if ("ESM_BKG_0028".equals(manifestListDetailVOs[0].getPgmNo()) || "ESM_BKG_0568".equals(manifestListDetailVOs[0].getPgmNo())) {
				/*******************************************************
			...
			} else if ("ESM_BKG_0210".equals(manifestListDetailVOs[0].getPgmNo())) {
	...
	public String deleteManifest(ManifestListDetailVO[] manifestListDetailVO) throws EventException {
		...
		try{
			for(int i=0 ; i < manifestListDetailVO.length ; i++) {
				CndManifestModificationVO cndManifestModificationVO = (CndManifestModificationVO) manifestListDetailVO[i];
...
//						}
	...
	public void removeBkgCstmsCndVslCrnNo(CndCstmsVslCrnNoVO cndCstmsVslCrnNoVO) throws EventException{
			...
			try {
				// 작업 구분 분할
				if (cndCstmsVslCrnNoVO!=null) dbDao.removeBkgCstmsCndVslCrnNo(cndCstmsVslCrnNoVO);

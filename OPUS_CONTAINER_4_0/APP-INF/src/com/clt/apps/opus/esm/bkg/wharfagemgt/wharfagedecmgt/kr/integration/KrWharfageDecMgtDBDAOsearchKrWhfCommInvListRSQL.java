...
public class KrWharfageDecMgtDBDAOsearchKrWhfCommInvListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",'2' AS BKRCISNOT" ).append("\n"); 
		...
		query.append("WHERE A.WHF_NTC_DT BETWEEN @[whf_ntc_dt1] AND @[whf_ntc_dt2]" ).append("\n"); 
		...
		query.append("AND A.PORT_NM     LIKE DECODE(NVL(@[port_nm],'A'), 'A',A.PORT_NM,'신항',@[port_nm]||'%',@[port_nm])" ).append("\n"); 
		query.append("AND A.PORT_CD     = 'KRPUS'" ).append("\n"); 
		...
		query.append("          WHERE  D.VSL_CD      = BL.VSL_CD" ).append("\n"); 

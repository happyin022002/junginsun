...
public class BlRatingDBDAOSearchMstCvrdInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("AND A2.BL_NO = CASE WHEN LENGTH(@[bl_no])>=12 THEN SUBSTR(@[bl_no],0,12)" ).append("\n"); 
		query.append("                    ELSE SUBSTR(@[bl_no],0,10)" ).append("\n"); 
		query.append("               END" ).append("\n"); 
		query.append("AND A1.BKG_NO = A2.BKG_NO" ).append("\n"); 
		query.append(") T7" ).append("\n"); 
		query.append("WHERE" ).append("\n"); 
		query.append("T1.BL_NO = CASE WHEN LENGTH(@[bl_no])>=12 THEN SUBSTR(@[bl_no],0,12)" ).append("\n"); 
		query.append("                ELSE SUBSTR(@[bl_no],0,10)" ).append("\n"); 
		query.append("           END" ).append("\n"); 
		...
		query.append("AND T4.BL_NO =  CASE WHEN LENGTH(@[c_bl_no])>=12 THEN SUBSTR(@[c_bl_no],0,12)" ).append("\n"); 
		query.append("                     ELSE SUBSTR(@[c_bl_no],0,10)" ).append("\n"); 
		query.append("                END	" ).append("\n"); 

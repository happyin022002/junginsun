 ...
 * @since J2EE 1.5
 */
public class EsmSaq0052ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		List<SearchMonthlyQuotaRelease0052List01VO> volist = null;
		int totCnt = vos.size();
		int realCnt =0;

		ReturnVO returnVO = (ReturnVO)vos.get(0);		
		if (returnVO !=null) {
		    volist = (List<SearchMonthlyQuotaRelease0052List01VO>)returnVO.getList(0);
		...
		if(formcommand.isCommand(FormCommand.SEARCHLIST)  || formcommand.isCommand(FormCommand.SEARCHLIST02)) { 
			...
			for(int i=0;i<realCnt;i++){
				...
				sbufXML.append("<TD><![CDATA["+ colValues.get("mqta_rlse_ver_no")+"]]></TD>");
				sbufXML.append("<TD><![CDATA["+ colValues.get("status") +"]]></TD>");
				sbufXML.append("<TD><![CDATA["+ colValues.get("rlse_gdt") +"]]></TD>");
				sbufXML.append("</TR>\n");
			}

 ...
 * @since J2EE 1.6
 */
public class ESM_BSA_0023HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * TESTHTMLAction 객체를 생성
	 */
	public ESM_BSA_0023HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		log.debug("################# ESM_BSA_0023HTMLAction.perform() ############################[START]");
    	FormCommand command = FormCommand.fromRequest(request);
    	EsmBsa0023Event event = new EsmBsa0023Event();  		
    	
		if(command.isCommand(FormCommand.SEARCHLIST)) {
			event.setBsaOpCd(JSPUtil.getParameter(request, "bsa_op_cd".trim(), ""));
		}	
		else if(command.isCommand(FormCommand.MODIFY)) {
			event.setBsaAddCarrierSaveVOs((BsaAddCarrierSaveVO[])getVOs(request, BsaAddCarrierSaveVO.class, "D_"));
		}else if(command.isCommand(FormCommand.MULTI01)) {
			event.setSearchBsaConditionVO((SearchBsaConditionVO)getVO(request, SearchBsaConditionVO .class));
		}	
		
		log.debug("################# ESM_BSA_0023HTMLAction.perform() ############################[END]");

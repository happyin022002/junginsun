...
public class KrWharfageDecMgtDBDAOsearchKrWhfCgoClassRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#if (${whf_bnd_cd} == 'II' || ${whf_bnd_cd} == 'IN' || ${whf_bnd_cd} == 'IT')" ).append("\n"); 
		...
		query.append("                     AND A.WHF_BND_CD = DECODE(@[whf_bnd_cd], 'IN', 'II', @[whf_bnd_cd])) ) E, ( SELECT A.BL_NO, MIN(B.CNTR_FULL_FLG) AS CNTR_FULL_FLG" ).append("\n"); 
		query.append("     FROM ( SELECT A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.WHF_BND_CD, A.BL_NO, A.BKG_NO," ).append("\n"); 
		query.append("          A.WGT_QTY, A.MEAS_QTY," ).append("\n"); 
		query.append("          MAX(CHG_RT_SEQ) AS CHG_RT_SEQ" ).append("\n"); 
		query.append("     FROM BKG_KR_WHF_BL A, BKG_KR_WHF_RT B" ).append("\n"); 
		...
		query.append("               T.WHF_CNTR_45FT_QTY * NVL(V.HC_PRC, 0) * (1 - NVL(V.HC_AMT_RT, 0)),0) AS WHF_RT_CNTR_WHF_AMT," ).append("\n"); 
		...
		query.append("                  SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) = '2' AND B.RCV_TERM_CD NOT IN ('I', 'T') THEN B.CNTR_VOL_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		query.append("                      END) AS WHF_CNTR_20FT_QTY," ).append("\n"); 
		query.append("                  SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) IN ('4', '5') AND B.RCV_TERM_CD NOT IN ('I', 'T') THEN B.CNTR_VOL_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		query.append("                      END) AS WHF_CNTR_40FT_QTY," ).append("\n"); 
		query.append("                  SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) NOT IN ('2', '4', '5') AND B.RCV_TERM_CD NOT IN ('I', 'T') THEN B.CNTR_VOL_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		...
		query.append("                  SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) IN ('4', '5') AND B.RCV_TERM_CD IN ('I', 'T') THEN B.CNTR_VOL_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		query.append("                      END) AS WHF_BLK_40FT_QTY," ).append("\n"); 
		query.append("                  SUM(CASE WHEN SUBSTR(B.CNTR_TPSZ_CD, 2, 1) NOT IN ('2', '4', '5') AND B.RCV_TERM_CD IN ('I', 'T') THEN B.CNTR_VOL_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		...
		query.append("      AND B.BL_NO(+) = A.BL_NO" ).append("\n"); 
		query.append("    GROUP BY A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.WHF_BND_CD, A.BL_NO, A.BKG_NO," ).append("\n"); 
		query.append("             A.WGT_QTY, A.MEAS_QTY ) A, BKG_CONTAINER B" ).append("\n"); 
		...
		query.append("                  SUM(CASE WHEN SUBSTR(E.CNTR_TPSZ_CD, 2, 1) IN ('4', '5') AND SUBSTR(E.CNTR_TPSZ_CD, 1, 1) <> 'Q' THEN E.OP_CNTR_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		query.append("                      END) AS WHF_BKG_40FT_QTY," ).append("\n"); 
		query.append("                  SUM(CASE WHEN SUBSTR(E.CNTR_TPSZ_CD, 2, 1) NOT IN ('2', '4', '5') AND SUBSTR(E.CNTR_TPSZ_CD, 1, 1) <> 'Q' THEN E.OP_CNTR_QTY" ).append("\n"); 
		query.append("                           ELSE 0" ).append("\n"); 
		...
		query.append("      AND F.BL_NO(+) = C.BL_NO ) T, BKG_KR_WHF_VOL U, BKG_KR_WHF_PORT_RT V, BKG_KR_WHF_PORT_RT W" ).append("\n"); 
		query.append("         WHERE U.VSL_CD(+) = T.VSL_CD" ).append("\n"); 
		query.append("           AND U.SKD_VOY_NO(+) = T.SKD_VOY_NO" ).append("\n"); 
		query.append("           AND U.SKD_DIR_CD(+) = T.SKD_DIR_CD" ).append("\n"); 
		query.append("           AND U.PORT_CD(+) = @[port_cd]" ).append("\n"); 
		query.append("           AND U.WHF_BND_CD(+) = T.WHF_BND_CD" ).append("\n"); 
		query.append("           AND V.CNTR_BLK_DIV_CD(+) = 'C'" ).append("\n"); 
		...
		query.append("           AND U.WHF_BND_CD(+) = T.WHF_BND_CD" ).append("\n"); 
		query.append("           AND W.DC_RTO_NO(+) = U.WHF_VOL_DC_CD) X," ).append("\n"); 
		...
		query.append("          SUM(CASE WHEN SUBSTR(C.CNTR_TPSZ_CD, 2, 1) IN ('4', '5') THEN C.CNTR_VOL_QTY ELSE 0 END) AS BKG_CNTR_40FT_QTY," ).append("\n"); 
		query.append("          SUM(CASE WHEN SUBSTR(C.CNTR_TPSZ_CD, 2, 1) NOT IN ('2', '4', '5') THEN C.CNTR_VOL_QTY ELSE 0 END) AS BKG_CNTR_45FT_QTY" ).append("\n"); 
		...
		query.append("      AND A.SKD_DIR_CD = SUBSTR(@[vvd],9,1)" ).append("\n"); 
		query.append("      AND A.PORT_CD = @[port_cd]" ).append("\n"); 
		query.append("      AND A.WHF_BND_CD = DECODE(@[whf_bnd_cd], 'IN', 'II', @[whf_bnd_cd]) )  A1" ).append("\n"); 

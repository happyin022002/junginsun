...
public class BookingHistoryMgtDBDAOSearchHistBkgContainerRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        SELECT DECODE(@[ui_id],'ESM_BKG_0079_07','[CNTR] PKG/Weight/Measure','PKG/Weight/Measure') HIS_CATE_NM" ).append("\n"); 
		...
		query.append("                , OLD.CNTR_NO||'('||OLD.CNTR_TPSZ_CD||')/'||trim(to_char(OLD.CNTR_VOL_QTY, '999,999,990.000'))||'/P:'||OLD.CNTR_PRT_FLG PRE_CTNT" ).append("\n"); 
		query.append("                , NOW.CNTR_NO||'('||NOW.CNTR_TPSZ_CD||')/'||trim(to_char(NOW.CNTR_VOL_QTY, '999,999,990.000'))||'/P:'||NOW.CNTR_PRT_FLG CRNT_CTNT" ).append("\n"); 
		query.append("          FROM OLD" ).append("\n"); 
		query.append("#if (${ca_flg} == 'Y')" ).append("\n"); 
		query.append("             , BKG_CNTR_HIS NOW" ).append("\n"); 
		query.append("         WHERE NOW.BKG_NO (+) = OLD.BKG_NO" ).append("\n"); 
		...
		query.append("                   ||'/A:'||OLD.AWK_CGO_FLG ||'/S:'|| OLD.SOC_FLG PRE_CTNT" ).append("\n"); 

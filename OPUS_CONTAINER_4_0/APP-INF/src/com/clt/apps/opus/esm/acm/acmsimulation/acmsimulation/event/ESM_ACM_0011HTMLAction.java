...
*@FileName : ESM_ACM_0011HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.esm.acm.acmsimulation 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
 ...
 * @author KIM, Sang-Soo
 * @see EsmAcm0011Event 참조
 * @since J2EE 1.6
 */

public class ESM_ACM_0011HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_ACM_0011HTMLAction 객체를 생성
	 */
	public ESM_ACM_0011HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmAcm0011Event event = new EsmAcm0011Event();

		if (command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH02) || command.isCommand(FormCommand.SEARCH11)) {
			event.setSimAgnRateMasterVO((SimAgnRateMasterVO)getVO(request, SimAgnRateMasterVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setSimAgnRateMasterVOs((SimAgnRateMasterVO[])getVOs(request, SimAgnRateMasterVO.class, ""));
		} else if (command.isCommand(FormCommand.MULTI01)) {
			event.setSimAgnRateDetailVOs((SimAgnRateDetailVO[])getVOs(request, SimAgnRateDetailVO.class, ""));
		}

...
*@FileName : ESM_BSA_0026HTMLAction.java
 ...
 * @author Kim Ki Dae
 * @see EsmBsa0026Event 참조
 * @since J2EE 1.6
 */
public class ESM_BSA_0026HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * TESTHTMLAction 객체를 생성
	 */
	public ESM_BSA_0026HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBsa0023Event로 파싱하여 request에 셋팅<br>
	 * @param request HttpServletRequest HttpRequest
	 * @return Event Event interface를 구현한 객체
	 * @exception HTMLActionException
	 */
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		log.debug("################# ESM_BSA_0026HTMLAction.perform() ############################[START]");
    	FormCommand command = FormCommand.fromRequest(request);
    	EsmBsa0026Event event = new EsmBsa0026Event();  		
		...
		if(command.isCommand(FormCommand.SEARCHLIST01)
			...
			event.setBsaJntOpCrrCapaVOs((BsaJntOpCrrCapaVO[])getVOs(request, BsaJntOpCrrCapaVO.class, "M_"));
		}	
		else if(command.isCommand(FormCommand.MULTI02)) {
			event.setBsaJntOpPortDwnVOs((BsaJntOpPortDwnVO[])getVOs(request, BsaJntOpPortDwnVO.class, "D_"));
		}	
		else if(command.isCommand(FormCommand.MULTI03)) {
			event.setBsaSltChtrCrrCapaVOs((BsaSltChtrCrrCapaVO[])getVOs(request, BsaSltChtrCrrCapaVO.class, "M_"));
		}	
		else if(command.isCommand(FormCommand.MULTI04)) {
			event.setBsaSltChtrPortDwnVOs((BsaSltChtrPortDwnVO[])getVOs(request, BsaSltChtrPortDwnVO.class, "D_"));
		}		
		...
		log.debug("################# ESM_BSA_0026HTMLAction.perform() ############################[END]");

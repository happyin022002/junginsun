 ...
 *@FileName : ESM_PRI_2022HTMLAction.java
...
public class ESM_PRI_2022HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_PRI_0091HTMLAction 객체를 생성
	 */
	public ESM_PRI_2022HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri2022Event event = new EsmPri2022Event();

		if (command.isCommand(FormCommand.MODIFY01)) {
			//Accept
			event.setPriRpScpRtCnoteVOS((PriRpScpRtCnoteVO[]) getVOs(request, PriRpScpRtCnoteVO.class, ""));
		...
		/* Retrieve */
			...
			//init(Combo, search data)
			event.setPriRfaNoteConvCommVO((PriRfaNoteConvCommVO)getVO(request, PriRfaNoteConvCommVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {
			//retrieve Cnote, Conversion
			event.setPriRfaNoteConvCommVO((PriRfaNoteConvCommVO)getVO(request, PriRfaNoteConvCommVO .class, "sheet4_"));
		}
			...
			event.setPriRpScpRtCnoteVOS((PriRpScpRtCnoteVO[]) getVOs(request, PriRpScpRtCnoteVO.class, "sheet2_"));
			event.setPriRfaNoteConvListVOs((PriRfaNoteConvListVO[]) getVOs(request, PriRfaNoteConvListVO.class, "sheet3_"));
		}
		else {
			// NO USE
			event.setPriRfaNoteConvVO((PriRfaNoteConvVO)getVO(request, PriRfaNoteConvVO .class));

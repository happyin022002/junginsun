...
public class KorManifestListDBDAOsearchMrnInfoKorRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   AND NVL(PORT_TML_CD, ' ') = DECODE(@[pod_tml], '', NVL(@[pol_tml], ' '), @[pod_tml])       " ).append("\n"); 
		query.append("  " ).append("\n"); 
		...
		query.append("    select m.mrn_no" ).append("\n"); 
		query.append("    from BKG_CSTMS_KR_MF_REF_NO m" ).append("\n"); 
		query.append("    where M.mrn_no = kv.mrn_no" ).append("\n"); 
		query.append("   AND M.vsl_cd     =   SUBSTR(@[vvd], 1, 4)" ).append("\n"); 
		query.append("   AND M.skd_voy_no =   SUBSTR(@[vvd], 5, 4)" ).append("\n"); 
		query.append("   AND M.skd_dir_cd =   SUBSTR(@[vvd], 9, 1)   " ).append("\n"); 
		query.append(")" ).append("\n"); 

...
public class KorManifestListDBDAOSearchManifestInfoKorRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                LENGTH( DECODE(@[in_bound],'O', NVL(@[in_pol_tmnl],'aa'), NVL(@[in_pod_tmnl],'aa')) ) = 7 " ).append("\n"); 
		query.append("                AND " ).append("\n"); 
		query.append("                DECODE(@[in_bound],'O', EVNT_YD_CD, PORT_TML_CD) = DECODE(@[in_bound],'O', @[in_pol_tmnl], @[in_pod_tmnl])" ).append("\n"); 
		query.append("            )				" ).append("\n"); 
		query.append("			 OR" ).append("\n"); 
		query.append("			(" ).append("\n"); 
		query.append("				LENGTH( DECODE(@[in_bound],'O', NVL(@[in_pol_tmnl],'aa'), NVL(@[in_pod_tmnl],'aa')) ) <> 7 " ).append("\n"); 
		query.append("                and " ).append("\n"); 
		query.append("                LENGTH( NVL(DECODE(@[in_bound],'O', EVNT_YD_CD, PORT_TML_CD), 'aa') ) <> 7" ).append("\n"); 
		query.append("            )" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("AND KR_CSTMS_BND_CD = @[sel_type]" ).append("\n"); 

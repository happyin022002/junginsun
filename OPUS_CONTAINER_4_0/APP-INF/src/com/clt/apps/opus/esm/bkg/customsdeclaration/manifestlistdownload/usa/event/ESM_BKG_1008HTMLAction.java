...
public class ESM_BKG_1008HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0926HTMLAction 객체를 생성
     */
    public ESM_BKG_1008HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1008Event event = new EsmBkg1008Event();

		if (command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01))
		{
			// Auth Setup 정보 조회
			event.setCondVO((UserAuthListCondVO) getVO(request, UserAuthListCondVO.class));
		}
		else if (command.isCommand(FormCommand.MULTI))
		{
			// Auth Setup 정보 수정
			event.setAuthSetupListVOS ((UserAuthListModiVO[]) getVOs(request, UserAuthListModiVO.class, "sheet1_"));

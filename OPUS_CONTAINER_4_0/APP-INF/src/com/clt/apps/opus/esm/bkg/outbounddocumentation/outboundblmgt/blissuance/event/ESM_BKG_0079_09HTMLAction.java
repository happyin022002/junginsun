...
public class ESM_BKG_0079_09HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.MULTI)) {
			...
			log.debug("size===>"+event.getBlIssInfoVOs().length);;
			...
			// 저장 변수 
			event.setBlIssInfoVOs((BlIssInfoVO[])getVOs(request, BlIssInfoVO.class,"t11sheet1_"));
			event.setTotalpage(JSPUtil.getParameter(request, "totalpage"));
			event.setTotalunrated(JSPUtil.getParameter(request, "totalunrated"));

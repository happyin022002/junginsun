...
*@FileName : ESM_BKG_1705HTMLAction.java
 ...
 * @see EsmBkg1705Event 참조
 * @since J2EE 1.6
 */

public class ESM_BKG_1705HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_1705HTMLAction 객체를 생성
     */
    public ESM_BKG_1705HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBkg1705Event 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1705Event event = new EsmBkg1705Event(); 		
		if (command.isCommand(FormCommand.INIT)||command.isCommand(FormCommand.SEARCH02)){
			event.setrInfoVO((ReportTemplateListVO) getVO(request, ReportTemplateListVO.class));
			event.setOutBdMovementStsListInVO((OutBdMovementStsListInVO) getVO(request, OutBdMovementStsListInVO.class));
		} else {
			event.setInfoVO((CmBkgRptVO) getVO(request, CmBkgRptVO.class));
		}	

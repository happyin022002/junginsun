...
public class EsmPri000101Event extends EventSupport {
	...
	public PriSgSlsRefVO[] getPriSgSlsRefVOS() {
		PriSgSlsRefVO[] tmpVOs = null;
		if (this.prisgslsrefvos != null) {
			tmpVOs = new PriSgSlsRefVO[prisgslsrefvos.length];
			System.arraycopy(prisgslsrefvos, 0, tmpVOs, 0, tmpVOs.length);
		}
		return tmpVOs;
	}

	public void setPriSgSlsRefVOS(PriSgSlsRefVO[] prisgslsrefvos) {
		if (prisgslsrefvos != null) {
			PriSgSlsRefVO[] tmpVOs = new PriSgSlsRefVO[prisgslsrefvos.length];
			System.arraycopy(prisgslsrefvos, 0, tmpVOs, 0, tmpVOs.length);
			this.prisgslsrefvos = tmpVOs;
		}
	}

...
public class CLLCDLManifestDBDAOsearchPreAdviceManifestListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	'NYK' LINE_CD,   " ).append("\n"); 
		query.append("    NVL( (SELECT REPLACE(S.CUST_NM,CHR(10),'  ') AS CUST_NM     FROM BKG_CUSTOMER S WHERE S.BKG_NO=B.BKG_NO AND S.BKG_CUST_TP_CD='S' AND ROWNUM=1 ), ' ' ) AS CUST_NM," ).append("\n"); 
		...
		query.append("    (SELECT CNTR_SEAL_NO FROM BKG_CNTR_SEAL_NO S WHERE B.BKG_NO=S.BKG_NO AND S.CNTR_NO=C.CNTR_NO) CNTR_SEAL_NO," ).append("\n"); 

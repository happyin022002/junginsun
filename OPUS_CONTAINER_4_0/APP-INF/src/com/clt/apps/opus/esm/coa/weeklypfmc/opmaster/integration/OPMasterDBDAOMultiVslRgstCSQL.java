...
public class OPMasterDBDAOMultiVslRgstCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     @[vsl_cd]" ).append("\n"); 
		query.append("   , (SELECT NVL(MAX(VSL_SEQ), 0)+1 FROM COA_VSL_RGST WHERE VSL_CD = @[vsl_cd])" ).append("\n"); 
		query.append("   , NVL(TO_DATE(@[vsl_aply_fm_dt],'YYYYMMDD'),SYSDATE)									--SJH.20150205.MOD : insert만 허용" ).append("\n"); 
		query.append("   , NVL(TO_DATE(@[vsl_aply_to_dt],'YYYYMMDD'),TO_DATE('99991230','YYYYMMDD'))			--SJH.20150205.MOD : insert만 허용" ).append("\n"); 
		query.append("   , @[vsl_tp_cd]" ).append("\n"); 
		query.append("   , @[vsl_oshp_cd]" ).append("\n"); 
		query.append("   , @[vop_cd]" ).append("\n"); 
		query.append("   , @[vsl_rgst_cnt_cd]" ).append("\n"); 

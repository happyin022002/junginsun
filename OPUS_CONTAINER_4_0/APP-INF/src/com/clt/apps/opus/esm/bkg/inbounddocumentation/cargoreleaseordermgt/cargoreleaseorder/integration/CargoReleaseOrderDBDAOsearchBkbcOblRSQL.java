...
*@FileName : CargoReleaseOrderDBDAOsearchBkbcOblRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CargoReleaseOrderDBDAOsearchBkbcOblRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public CargoReleaseOrderDBDAOsearchBkbcOblRSQL(){
		setQuery();
		...
		query.append("FileName : CargoReleaseOrderDBDAOsearchBkbcOblRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("	(SELECT SUBSTR(POD_CD,1,2) FROM BKG_BOOKING WHERE BL_NO = @[bl_no]) CA_CHK" ).append("\n"); 
		...
		query.append("                      NVL(C.FRT_CLT_FLG,'N')||(CASE WHEN A.BL_TP_CD='W' AND D.OBL_RLSE_FLG = 'Y' THEN 'Y'" ).append("\n"); 
		query.append("               								        WHEN D.OBL_SRND_FLG='Y' THEN 'Y' " ).append("\n"); 
		query.append("               									     ELSE 'N' END)||NVL(C.CSTMS_CLR_CD,'N'),'N','Y') FOC_YN," ).append("\n"); 
		query.append("               COUNT(C.BL_NO)," ).append("\n"); 
		query.append("               CASE WHEN A.BL_TP_CD='W' AND D.OBL_RLSE_FLG = 'Y' THEN 'Y'" ).append("\n"); 
		query.append("                    WHEN D.OBL_SRND_FLG='Y' THEN 'Y' " ).append("\n"); 
		...
		query.append("                        NVL(C.FRT_CLT_FLG,'N')||(CASE WHEN A.BL_TP_CD='W' AND D.OBL_RLSE_FLG = 'Y' THEN 'Y'" ).append("\n"); 
		query.append("               									      WHEN D.OBL_SRND_FLG='Y' THEN 'Y' " ).append("\n"); 
		query.append("               								     ELSE 'N' END)||NVL(C.CSTMS_CLR_CD,'N'),'N','Y')   ," ).append("\n"); 
		query.append("                 CASE WHEN A.BL_TP_CD='W' AND D.OBL_RLSE_FLG = 'Y' THEN 'Y'" ).append("\n"); 
		query.append("                      WHEN D.OBL_SRND_FLG='Y' THEN 'Y' " ).append("\n"); 
		query.append("                      ELSE 'N' END" ).append("\n"); 

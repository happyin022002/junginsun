...
*@FileName : ESM_BKG_1024HTMLAction.java
...
public class ESM_BKG_1024HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1024HTMLAction 객체를 생성
	 */
	public ESM_BKG_1024HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1024Event event = new EsmBkg1024Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setBkgBlNoVO((BkgBlNoVO)getVO(request, BkgBlNoVO.class));
		}else if(command.isCommand(FormCommand.MULTI)){
			event.setPartialBkgInfoVOs((PartialBkgInfoVO[])getVOs(request, PartialBkgInfoVO.class,"sheet2_"));	
			event.setVslSkdVOs((VslSkdVO[])getVOs(request, VslSkdVO.class,"sheetM_"));
		}

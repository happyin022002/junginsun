...
public class NetworkCostDBDAOMultiPortTariffUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    COA.PORT_USD_AMT = NVL(PSO.INV_USD_AMT,0)" ).append("\n"); 
		query.append("    COA.CNL_USD_AMT  = (CASE WHEN SUBSTR(PSO.YD_CD,1,5) IN ('EGSCA','PAPCA') THEN NVL(PSO.INV_USD_AMT,0)	--SJH.20150116.MOD : EGSUZ->EGSCA, PAPAC->PAPCA" ).append("\n"); 
		...
		query.append("        (CASE WHEN SUBSTR(PSO.YD_CD,1,5) IN ('EGSCA','PAPCA') THEN NVL(PSO.INV_USD_AMT,0)					--SJH.20150116.MOD : EGSUZ->EGSCA, PAPAC->PAPCA" ).append("\n"); 
		query.append("              ELSE 0" ).append("\n"); 
		query.append("        END)," ).append("\n"); 

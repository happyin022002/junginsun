...
public class EBookingReceiptDBDAOsearchXterRqstMstForRejectEdiRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    , DECODE(@[sender_id], 'INTT', substr(estm_wgt_ut_cd, 1, 1), DECODE(estm_wgt_ut_cd, 'KG', 'KGS', 'KGM', 'KGS',estm_wgt_ut_cd))	wunit   " ).append("\n"); 
		...
		query.append("    --, replace(rjct_rsn_rmk, CHR(10), ' ')	remark          " ).append("\n"); 
		query.append("	, REPLACE(REPLACE(REPLACE(rjct_rsn_rmk, CHR(10), ' '), '*', '-'), ':', '-') remark  " ).append("\n"); 
		...
		query.append("    , sh.CNT_CD||sh.CUST_seq	         sh_cd1          " ).append("\n"); 
		query.append("    , cn.CNT_CD||cn.CUST_seq	         ff_cd1          " ).append("\n"); 
		query.append("    , ff.CNT_CD||ff.CUST_seq	         cn_cd1          " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(sh.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	shpr1           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(sh.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	shpr2           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(sh.cust_addr,' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	shpr3           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(sh.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	shpr4           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(sh.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 71, 35)	shpr5           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(cn.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	cnee1           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(cn.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	cnee2           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(cn.cust_addr,' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	cnee3           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(cn.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	cnee4           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(cn.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 71, 35)	cnee5           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(ff.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	ntfy1           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(ff.cust_nm,  ' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	ntfy2           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(ff.cust_addr,' '), CHR(10), ' '), CHR(13), ' '),  1, 35)	ntfy3           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(ff.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 36, 35)	ntfy4           " ).append("\n"); 
		query.append("    , SUBSTR(REPLACE(REPLACE(NVL(ff.cust_addr,' '), CHR(10), ' '), CHR(13), ' '), 71, 35)	ntfy5           " ).append("\n"); 
		...
		query.append("    , CASE WHEN MST.XTER_SNDR_ID = 'PEGASUS' AND MST.XTER_RQST_VIA_CD = 'EDI' THEN MST.ACT_CUST_REF_NO ELSE mst.xter_rqst_no	END cust_ref_no     " ).append("\n"); 
		query.append("    , SKD_VOY_NO||SKD_DIR_CD	         ref_voyage      " ).append("\n"); 
		query.append("    , NULL		                         so_no           " ).append("\n"); 
		query.append("    , NULL		                         blkstwg         " ).append("\n"); 
		query.append("    , TO_CHAR(mty_pkup_dt, 'RRRRMMDDHH24MI') AS eqpickdt         " ).append("\n"); 
		...
		query.append("    , REPLACE(REPLACE(REPLACE(@[reject_rmk], CHR(10),' '), '*', '-'), ':', '-') rej_desc     " ).append("\n"); 
		...
		query.append("    , mst.upld_usr_id		             BKG_STF" ).append("\n"); 
		query.append("    , usr.usr_nm		                 BKG_STF_NAME" ).append("\n"); 
		query.append("    , (SELECT MO.OFC_PHN_NO FROM MDM_ORGANIZATION MO WHERE MO.OFC_CD = usr.ofc_cd AND ROWNUM = 1) BKG_STF_TEL" ).append("\n"); 
		query.append("    , (SELECT MO.OFC_FAX_NO FROM MDM_ORGANIZATION MO WHERE MO.OFC_CD = usr.ofc_cd AND ROWNUM = 1) BKG_STF_FAX" ).append("\n"); 
		query.append("    , (SELECT MO.OFC_ENG_NM FROM MDM_ORGANIZATION MO WHERE MO.OFC_CD = usr.ofc_cd AND ROWNUM = 1) BKG_CPY_NAME" ).append("\n"); 
		...
		query.append("    , mst.ob_trsp_mod_cd		         ob_trsp_mod_cd" ).append("\n"); 
		query.append("    , mst.ib_trsp_mod_cd		         ib_trsp_mod_cd" ).append("\n"); 
		...
		query.append("   AND mst.upld_usr_id = usr.usr_id(+)" ).append("\n"); 

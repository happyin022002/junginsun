...
public class BookingHistoryMgtDBDAOaddBkgNtcHisCSQL implements ISQLTemplate{
	...
	public BookingHistoryMgtDBDAOaddBkgNtcHisCSQL(){
		...
		params.put("his_ui_nm",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("#if (${his_ui_nm} != '') " ).append("\n"); 
		query.append(",	HIS_UI_NM" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(", 	PRNR_SUB_LNK_CD" ).append("\n"); 
		query.append("#if (${cntr_slt_no_ctnt} == 'Y') " ).append("\n"); 
		...
		query.append("          WHEN @[ntc_knd_cd] IN ('BT','BK','BL','FC','CN','EX','SN','HO','WB','PS','TM','AV','NN','CW','ES','ED','EI') THEN " ).append("\n"); 
		query.append("			DECODE(@[cre_usr_id],'ESM_BKG_B026',SYSDATE,GLOBALDATE_PKG.TIME_CONV_FNC(COM_CONSTANTMGR_PKG.COM_GETBASELOCATIONCODE_FNC(), SYSDATE, BKG_COM_USER_LOC_FNC(@[cre_usr_id])))" ).append("\n"); 
		query.append("	      WHEN @[ntc_knd_cd] IN ('VC','VP','VT','VM','3C','3P','3M') THEN GLOBALDATE_PKG.TIME_CONV_FNC(COM_CONSTANTMGR_PKG.COM_GETBASELOCATIONCODE_FNC(), sysdate, 'GMT')" ).append("\n"); 
		query.append("          WHEN @[ntc_knd_cd] IN ('TO','TW','TN','TC') THEN GLOBALDATE_PKG.TIME_CONV_FNC(COM_CONSTANTMGR_PKG.COM_GETBASELOCATIONCODE_FNC(), SYSDATE, BKG_COM_USER_LOC_FNC(@[cre_usr_id]))" ).append("\n"); 
		query.append("          WHEN @[ntc_knd_cd] IN ('TE','TU') THEN GLOBALDATE_PKG.TIME_CONV_FNC(COM_CONSTANTMGR_PKG.COM_GETBASELOCATIONCODE_FNC(), SYSDATE, BKG_COM_USER_LOC_FNC(@[snd_usr_id]))" ).append("\n"); 
		...
		query.append(",	@[prnr_sub_lnk_cd]" ).append("\n"); 
		query.append("#if (${cntr_slt_no_ctnt} == 'Y') " ).append("\n"); 
		query.append(",	(SELECT BKG_JOIN_FULL_CLOB_FNC(CURSOR(SELECT BKG_GET_SLOT_NO_FNC(COP_NO)||'-'||CNTR_NO FROM SCE_COP_HDR WHERE BKG_NO = @[bkg_no])) FROM DUAL)" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append(")" ).append("\n"); 

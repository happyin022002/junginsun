...
public class PanamaManifestListDownloadDBDAOsearchVesselListRSQL implements ISQLTemplate{
	...
	public PanamaManifestListDownloadDBDAOsearchVesselListRSQL(){
		...
		params.put("rcv_sts",new String[]{arrTmp[0],arrTmp[1]});
	...
	public void setQuery(){
		...
		query.append("   ,TO_CHAR(F.RCV_DT, 'YYYY-MM-DD HH24:MI') AS RCV_DT" ).append("\n"); 
		query.append("   ,F.RCV_LOG_SEQ" ).append("\n"); 
		query.append("   ,DECODE(F.CSTMS_ACK_CD, 'A', 'Accept', 'R', 'Reject') AS CSTMS_ACK_CD" ).append("\n"); 
		query.append("   ,ROW_NUMBER() OVER(PARTITION BY 	A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD ORDER BY F.RCV_DT DESC, F.RCV_LOG_SEQ DESC) AS RNUM" ).append("\n"); 
		query.append("FROM " ).append("\n"); 
		...
		query.append("   ,BKG_CSTMS_PNM_SND_LOG E" ).append("\n"); 
		query.append("   ,BKG_CSTMS_PNM_RCV_LOG F" ).append("\n"); 
		query.append("WHERE A.VPS_PORT_CD        = (SELECT ATTR_CTNT1" ).append("\n"); 
		query.append("                                FROM BKG_CSTMS_CD_CONV_CTNT" ).append("\n"); 
		query.append("                               WHERE CNT_CD = 'PA'" ).append("\n"); 
		query.append("                                 AND CSTMS_DIV_ID = 'MANI_PORT_CD'" ).append("\n"); 
		...
		query.append("AND D.EDI_SND_DT        =  E.SND_DT(+)" ).append("\n"); 
		query.append("AND D.VST_NO            =  E.VST_NO(+)" ).append("\n"); 
		query.append("AND E.CRR_BAT_NO        =  F.CRR_BAT_NO(+)" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${rcv_sts} != '')" ).append("\n"); 
		query.append("AND F.CSTMS_ACK_CD = @[rcv_sts]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("GROUP BY A.VSL_CD||A.SKD_VOY_NO||A.SKD_DIR_CD" ).append("\n"); 
		query.append(",A.VSL_CD,A.SKD_VOY_NO,A.SKD_DIR_CD, A.SLAN_CD, A.VPS_ETA_DT, D.SHP_ID_NO, D.VST_NO, D.MVMT_SEQ, D.PNM_VSL_OPR_CD, D.PNM_ORG_CD, D.PNM_DEST_CD, D.VSL_CD, D.UPD_USR_ID, D.UPD_DT, D.CRE_USR_ID, D.CRE_DT,	D.EDI_SND_DT,D.EDI_SND_SEQ,D.EDI_SND_USR_ID" ).append("\n"); 
		query.append(",F.RCV_DT" ).append("\n"); 
		query.append(",F.RCV_LOG_SEQ" ).append("\n"); 
		query.append(",F.CSTMS_ACK_CD" ).append("\n"); 
		query.append(")" ).append("\n"); 

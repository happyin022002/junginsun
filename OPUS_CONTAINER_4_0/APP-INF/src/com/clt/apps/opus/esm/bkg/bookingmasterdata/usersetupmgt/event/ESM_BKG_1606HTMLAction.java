...
public class ESM_BKG_1606HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1606HTMLAction 객체를 생성
	 */
	public ESM_BKG_1606HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1606Event event = new EsmBkg1606Event();
		log.debug("***** ESM_BKG_1606HTMLAction : " + command.getCommand());
		
		if(command.isCommand(FormCommand.SEARCH)) {
		    event.setOrgCntCd(JSPUtil.getParameter(request, "org_cnt_cd", ""));
		} else if(command.isCommand(FormCommand.MULTI)) {
            event.setBlCluzStupVOs((BlCluzStupVO[])getVOs(request, BlCluzStupVO.class, ""));
		}

...
public class ESM_BKG_0152HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		// EVENT 생성
		EsmBkg0152Event event = new EsmBkg0152Event();

		// COMMAND 구분에 따른 처리
		if(command.isCommand(FormCommand.SEARCH))
    	{
			// 조회의 경우 처리
			BkgCstmsChnDeModCondVO bkgCstmsChnDeModCondVO = (BkgCstmsChnDeModCondVO)getVO(request, BkgCstmsChnDeModCondVO.class);
			event.setBkgCstmsChnDeModCondVO(bkgCstmsChnDeModCondVO);
    	}
		else if(command.isCommand(FormCommand.MULTI)) {
            event.setBkgCstmsChnDeModDetailVOs((BkgCstmsChnDeModDetailVO[])getVOs(request, BkgCstmsChnDeModDetailVO.class));
        }
		else if (command.isCommand(FormCommand.SEARCH01)) {
			SearchLocationVO searchLocationVO = (SearchLocationVO)getVO(request, SearchLocationVO.class);
			event.setSearchLocationVO(searchLocationVO);
		}

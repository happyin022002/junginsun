...
public class BLDocumentationBLDBDAOcopyBkgBlDocByBkgCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    , BL_MV_TP_NM" ).append("\n"); 
		query.append("    , FNL_DEST_NM" ).append("\n"); 
		query.append("    -- If trunk VVD is not changed, keep the original vessel name" ).append("\n"); 
		query.append("    , CASE WHEN (SELECT VSL_CD||SKD_VOY_NO||SKD_DIR_CD FROM BKG_BOOKING WHERE BKG_NO = @[bkg_no])" ).append("\n"); 
		query.append("                = (SELECT VSL_CD||SKD_VOY_NO||SKD_DIR_CD FROM BKG_BOOKING WHERE BKG_NO = @[targetBkg]) THEN" ).append("\n"); 
		query.append("                VSL_NM" ).append("\n"); 
		...
		query.append("             	    AND VVD.BKG_NO = @[targetBkg]" ).append("\n"); 
		...
		query.append("    , CASE WHEN (SELECT VSL_CD||SKD_VOY_NO||SKD_DIR_CD FROM BKG_VVD WHERE BKG_NO = @[bkg_no] AND VSL_PRE_PST_CD = 'S' AND VSL_SEQ = '1')" ).append("\n"); 
		query.append("                = (SELECT VSL_CD||SKD_VOY_NO||SKD_DIR_CD FROM BKG_VVD WHERE BKG_NO = @[targetBkg] AND VSL_PRE_PST_CD = 'S' AND VSL_SEQ = '1') THEN" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("                PRE_VSL_NM" ).append("\n"); 
		query.append("           ELSE (SELECT MDM.VSL_ENG_NM||' '||VSK.OB_CSSM_VOY_NO " ).append("\n"); 
		query.append("           		   FROM MDM_VSL_CNTR MDM, VSK_VSL_PORT_SKD VSK, BKG_VVD VVD" ).append("\n"); 

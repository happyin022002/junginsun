...
*@FileName : BLIssuanceDBDAOsearchDblEdiMain2RSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BLIssuanceDBDAOsearchDblEdiMain2RSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public BLIssuanceDBDAOsearchDblEdiMain2RSQL(){
		setQuery();
		...
		query.append("FileName : BLIssuanceDBDAOsearchDblEdiMain2RSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                      BKG_TOKEN_NL_FNC(FW.CUST_NM, 5, '*:')" ).append("\n"); 
		query.append("                   END ||CHR(10)||" ).append("\n"); 
		query.append("			 'FFWD_CITY_NM:' || FW.CUST_CTY_NM || CHR(10) ||" ).append("\n"); 
		query.append("			 'FFWD_STAT_CD:' || FW.CUST_STE_CD || CHR(10) ||" ).append("\n"); 
		query.append("			 'FFWD_ZIP_CD:' || FW.CUST_ZIP_ID || CHR(10) ||" ).append("\n"); 
		query.append("			 'FFWD_CNT_CD:' || FW.CUST_CNT_CD || CHR(10) ||" ).append("\n"); 
		...
		query.append("                      BKG_TOKEN_NL_FNC(EX.CUST_NM, 5, '*:')" ).append("\n"); 
		query.append("                   END ||CHR(10)||" ).append("\n"); 
		query.append("			 'EXPO_CITY_NM:' || EX.CUST_CTY_NM || CHR(10) ||" ).append("\n"); 
		query.append("			 'EXPO_STAT_CD:' || EX.CUST_STE_CD || CHR(10) ||" ).append("\n"); 
		query.append("			 'EXPO_ZIP_CD:' || EX.CUST_ZIP_ID || CHR(10) ||" ).append("\n"); 
		query.append("			 'EXPO_CNT_CD:' || EX.CUST_CNT_CD || CHR(10) ||" ).append("\n"); 
		query.append("			 'EX_IND:' || CHR(10)||" ).append("\n"); 
		query.append("			 'BLCOPY:' || ISS.BL_CPY_KNT || CHR(10)||" ).append("\n"); 
		query.append("			 'BL_F_COPY:' || ISS.BL_CPY_KNT || CHR(10)||" ).append("\n"); 
		query.append("			 'BL_C_COPY:' || '' || CHR(10)||" ).append("\n"); 
		query.append("			 'BL_P_COPY:' || '' || CHR(10)||" ).append("\n"); 
		query.append("			 'BL_UF_COPY:' || '' || CHR(10)||" ).append("\n"); 
		query.append("			 'BLORG_TP:' || (SELECT CASE WHEN FRT_TERM_CD = 'YY' THEN 'F'" ).append("\n"); 
		query.append("							             WHEN FRT_TERM_CD = 'YN' THEN 'P'" ).append("\n"); 
		query.append("							             WHEN FRT_TERM_CD = 'NY' THEN 'C'" ).append("\n"); 
		query.append("						            ELSE 'U' END" ).append("\n"); 
		query.append("				 			 FROM " ).append("\n"); 
		query.append("								(             " ).append("\n"); 
		query.append("								SELECT " ).append("\n"); 
		query.append("									MAX(DECODE(FRT_TERM_CD,'P','Y', 0))|| MAX(DECODE(FRT_TERM_CD,'C','Y', 0))  FRT_TERM_CD" ).append("\n"); 
		...
		query.append("			 'BLNWGT:' || BL.ACT_WGT						|| CHR(10)||" ).append("\n"); 
		query.append("			 'BL_NWGT_UNIT:' || BL.WGT_UT_CD						|| CHR(10)||" ).append("\n"); 
		...
		query.append("			 'BL_CGO_TP_IND:' || CASE WHEN INSTR(BK.RCV_TERM_CD||BK.DE_TERM_CD,'S') > 0 THEN 'L' ELSE 'F' END || CHR(10) ||" ).append("\n"); 
		query.append("			 'CARGO_RCPT_DT:' || (SELECT TO_CHAR(MAX(CTM.CNMV_EVNT_DT),'RRRRMMDDHH24MI') FROM BKG_CONTAINER CNTR, CTM_MOVEMENT CTM" ).append("\n"); 
		query.append("								  WHERE 1=1" ).append("\n"); 
		...
		query.append("			 'CMDT_CD:' || BK.CMDT_CD	|| CHR(10)||" ).append("\n"); 
		query.append("			 'REMARK:' || CASE " ).append("\n"); 
		query.append("                      WHEN SUBSTR(@[edi_receive_id], 1, 6) = 'SDIXML' OR SUBSTR(@[edi_receive_id], 1, 6) = 'PHIXML' THEN" ).append("\n"); 
		query.append("                        TO_CLOB(BKG_TOKEN_NL_FNC(BK.XTER_RMK, 0, '*:&<>\"\\/'''))" ).append("\n"); 
		query.append("                      WHEN SUBSTR(@[edi_receive_id], 1, 6) = 'LEHXML' OR SUBSTR(@[edi_receive_id], 1, 6) = 'PKEXML' OR " ).append("\n"); 
		query.append("                           SUBSTR(@[edi_receive_id], 1, 6) = 'PKCXML' OR SUBSTR(@[edi_receive_id], 1, 6) = 'LGEXML' OR " ).append("\n"); 
		query.append("                           SUBSTR(@[edi_receive_id], 1, 8) = 'CNLGEXML' OR SUBSTR(@[edi_receive_id], 1, 5) = 'EIXML' OR " ).append("\n"); 
		query.append("                           SUBSTR(@[edi_receive_id], 1, 6) = 'EIXMLC' THEN" ).append("\n"); 
		...
		query.append("			 'MST_BKG:' || (SELECT DECODE(BKG_CRE_TP_CD,'S', FM_BKG_NO,BKG_NO) FROM BKG_BOOKING WHERE BKG_NO = BK.BKG_NO) || CHR(10)||" ).append("\n"); 
		query.append("			 'FMC_NO:' || REFNO.FMC_NO || CHR(10) ||" ).append("\n"); 
		...
		query.append("			 'FINAL_ATA:' || @[final_eta] || CHR(10)||" ).append("\n"); 
		query.append("			 'POR_ETD:' || (SELECT TO_CHAR(MAX(ESTM_DT), 'RRRRMMDDHH24MI', 'NLS_DATE_LANGUAGE=ENGLISH') AS DEL_EST_ARRIVAL_DATE" ).append("\n"); 
		...
		query.append("   							AND DTL.NOD_CD LIKE BK.POR_CD||'%'" ).append("\n"); 
		query.append("   							AND HDR.COP_STS_CD IN ('C','T','F')" ).append("\n"); 
		query.append("  							AND DTL.ACT_CD ='MOTYDO'" ).append("\n"); 
		query.append("   							AND BK.BKG_NO =  @[bkg_no] ) || CHR(10)||" ).append("\n"); 
		query.append("			 'POR_ATD:' || (SELECT TO_CHAR(MAX(ACT_DT), 'RRRRMMDDHH24MI', 'NLS_DATE_LANGUAGE=ENGLISH') AS DEL_EST_ARRIVAL_DATE" ).append("\n"); 
		query.append("  							FROM SCE_COP_HDR HDR" ).append("\n"); 
		query.append("							     ,SCE_COP_DTL DTL" ).append("\n"); 
		query.append("							     ,BKG_BOOKING BK" ).append("\n"); 
		query.append("							WHERE BK.BKG_NO = HDR.BKG_NO " ).append("\n"); 
		...
		query.append("  							AND DTL.ACT_CD ='MOTYDO'" ).append("\n"); 
		query.append("   							AND BK.BKG_NO =  @[bkg_no] ) || CHR(10)||" ).append("\n"); 
		query.append("			 'CCT:' || (SELECT TO_CHAR(SYS_SET_DT,'RRRRMMDDHH24MI') FROM BKG_CLZ_TM" ).append("\n"); 
		...
		query.append("			 'APPLICATION_DT:' || TO_CHAR(RT.RT_APLY_DT, 'RRRRMMDD')		|| CHR(10)||" ).append("\n"); 
		...
		query.append("			 'BLINFO_ORGIN_CD:' || BK.POL_CD || CHR(10)||" ).append("\n"); 
		...
		query.append("			                                        , DECODE(BK.BL_NO_TP, '0', ''" ).append("\n"); 
		...
		query.append("			                                          , 'S', 'SHIPPED ON BOARD'" ).append("\n"); 
		...
		query.append("			 'BLINFO_ISSUELOC:' || CASE WHEN LOC.LOC_NM||CNT.CNT_CD IS NULL THEN ''" ).append("\n"); 
		query.append("									    ELSE LOC.LOC_NM	||' , '||CNT.CNT_CD" ).append("\n"); 
		query.append("                                   END ||CHR(10)||" ).append("\n"); 
		...
		query.append("			 'TRAN_TP:' || DECODE(BK.RCV_TERM_CD, 'Y','CY','S','CFS','D','DOOR','')|| CHR(10)||" ).append("\n"); 

...
*@FileName : ESM_BKG_1511HTMLAction.java
*@FileTitle : ESM_BKG_1511HTMLAction
*Open Issues :
*Change history :
*@LastModifyDate :
*@LastModifier :
...
/**
 * HTTP Parser<br>
 * - ESM_BKG_1511 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class ESM_BKG_1511HTMLAction extends HTMLActionSupport {
	...
	/**
	 * ESM_BKG_1511HTMLAction 객체를 생성
	 */
	public ESM_BKG_1511HTMLAction() {}

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 EsmBkg1511Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if (command.isCommand(FormCommand.SEARCH)) {
			event.setCstmsJpWhVO((CstmsJpWhVO)getVO(request, CstmsJpWhVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setCstmsJpWhVOs((CstmsJpWhVO[])getVOs(request, CstmsJpWhVO.class, ""));
		}

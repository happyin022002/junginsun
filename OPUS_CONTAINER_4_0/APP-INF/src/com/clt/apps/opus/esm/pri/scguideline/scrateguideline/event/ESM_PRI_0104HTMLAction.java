 ...
 *@FileName : ESM_PRI_0104HTMLAction.java
...
public class ESM_PRI_0104HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_PRI_0104HTMLAction 객체를 생성
	 */
	public ESM_PRI_0104HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri0104Event event = new EsmPri0104Event();
		log.debug("::CALL::> ESM_PRI_0104HTMLAction - " + command.getCommand());

		if (command.isCommand(FormCommand.MULTI)) {
			event.setPriSgRtCmdtRnoteVOS((PriSgRtCmdtRnoteVO[]) getVOs(request, PriSgRtCmdtRnoteVO.class, ""));
		} else if (command.isCommand(FormCommand.SEARCH)) {
			event.setPriSgRtCmdtRnoteVO((PriSgRtCmdtRnoteVO) getVO(request, PriSgRtCmdtRnoteVO.class));
		} else {
			event.setPriSgRtCmdtRnoteVO((PriSgRtCmdtRnoteVO) getVO(request, PriSgRtCmdtRnoteVO.class));
		}

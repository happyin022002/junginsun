...
public class ESM_BKG_3005HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0926HTMLAction 객체를 생성
     */
    public ESM_BKG_3005HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg3005Event event = new EsmBkg3005Event();

		if (command.isCommand(FormCommand.SEARCH))
		{
			event.setCountryCode(request.getParameter("country_code"));
			event.setESignatureLastNm(request.getParameter("esignature_last_name"));
			
		} else if (command.isCommand(FormCommand.REMOVE)) {
			event.setBlEsigSeq(request.getParameter("bl_esig_seq"));
			event.setCountryCode(request.getParameter("country_code"));
			event.setESignatureLastNm(request.getParameter("esignature_last_name"));
		}

...
public class BLDocumentationBLDBDAOBkgRefTpCdRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("SELECT COLUMN_VALUE AS BKG_REF_TP_ML_CD FROM TABLE (BKG_SPLIT_FNC(('POB,POC,POM'),','))" ).append("\n"); 
		query.append("WHERE 1=1" ).append("\n"); 
		query.append("AND EXISTS (SELECT 'POYN' AS BKG_REF_TP_CD" ).append("\n"); 
		query.append("FROM BKG_REFERENCE" ).append("\n"); 
		query.append("WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("AND BKG_REF_TP_CD IN ('BKPO','LCNO','HINV','LCDT','HPDP','CTPO','INCO')" ).append("\n"); 

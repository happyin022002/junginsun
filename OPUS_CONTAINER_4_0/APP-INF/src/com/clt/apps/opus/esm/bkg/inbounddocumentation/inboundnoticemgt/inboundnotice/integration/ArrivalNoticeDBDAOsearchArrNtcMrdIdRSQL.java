...
public class ArrivalNoticeDBDAOsearchArrNtcMrdIdRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	                OR MDM.CNT_CD ='CA' THEN  DECODE( MDM.SCONTI_CD,'AF','AF' " ).append("\n"); 
		...
		query.append("	--     , NVL(ANTC.AN_FOM_CD,'GE') AS AN_FOM_CD  -- Fault" ).append("\n"); 
		query.append("	     , LOCL_LANG_FLG" ).append("\n"); 
		query.append("	     , ECLZ_BL_CPY_FLG" ).append("\n"); 
		query.append("	     , NVL(NTWD.ARR_PRV_FOM_CD, 'GE') AS ARR_PRV_FOM_CD  -- User Office Form" ).append("\n"); 
		query.append("	     , ROW_NUMBER() OVER ( PARTITION BY NTWD.AN_TP_CD  " ).append("\n"); 
		query.append("	                           ORDER BY DECODE(NVL(ANTC.CHN_AGN_CD, '*'), NTWD.CHN_AGN_CD, 0, 1)  -- ANTC쪽 값이 Null일 수 있음" ).append("\n"); 
		...
		query.append("	  AND CTNT.ATTR_CTNT3 = NTWD.ARR_PRV_FOM_CD               -- User Office Form을 선택해야함" ).append("\n"); 

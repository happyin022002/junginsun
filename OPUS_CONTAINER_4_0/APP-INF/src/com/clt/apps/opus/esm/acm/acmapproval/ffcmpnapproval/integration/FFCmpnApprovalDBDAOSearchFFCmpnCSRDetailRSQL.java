...
public class FFCmpnApprovalDBDAOSearchFFCmpnCSRDetailRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        		 IN (SELECT DECODE(SUBSTR(REV_VVD_CD,0,2),'FD','CFDR'||SUBSTR(REV_VVD_CD,3,4)||'EE',REV_VVD_CD) " ).append("\n"); 
		query.append("        	   	       FROM ACM_AGN_BKG_INFO " ).append("\n"); 
		query.append("        	      	  WHERE BKG_NO = A.BKG_NO )) AS ATT14," ).append("\n"); 
		...
		query.append("               'ITEM' AS LOOKUP, " ).append("\n"); 
		query.append("--               ROUND(NVL(B.IF_DTRB_AMT,A.IF_AMT),2) AS INV_AMT, " ).append("\n"); 
		query.append("               SUM (-ROUND(NVL(B.IF_DTRB_AMT,A.IF_AMT),2)) AS INV_AMT, " ).append("\n"); 
		...
		query.append("    			  WHERE ASA_NO = @[asa_no]) AS ATT11,  @[ap_ofc_cd] AS ATT12, '' AS ATT13," ).append("\n"); 
		query.append("               'COM' AS ATT14," ).append("\n"); 
		query.append("               '' AS ATT15, " ).append("\n"); 
		...
		query.append("               '' AS TMNL_CD, " ).append("\n"); 
		query.append("               A.AP_OFC_CD AS AGNT, " ).append("\n"); 
		query.append("               '' AS SUB_FLG, " ).append("\n"); 
		...
		query.append("           AND A.FF_CMPN_SEQ     = B.FF_CMPN_SEQ(+) " ).append("\n"); 
		query.append("           AND A.BKG_NO       = C.BKG_NO(+)        " ).append("\n"); 
		query.append("          GROUP BY A.CSR_NO, A.VNDR_SEQ, A.AP_OFC_CD, A.FF_OCCR_INFO_CD, A.FF_SLAN_CD       " ).append("\n"); 

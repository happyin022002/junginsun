...
public class PerformanceReportDBDAOSaelsPerformanceReportByShipGrpRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("              ,NVL(MCUST.CUST_GRP_ID,CUS.CUST_CNT_CD||CUS.CUST_SEQ) AS CUST_GRP_ID" ).append("\n"); 
		query.append("		      ,DECODE(MCUST.CUST_GRP_ID,null,(SELECT REPLACE(CUST_LGL_ENG_NM,CHR(10), ' ') FROM MDM_CUSTOMER WHERE CUST_CNT_CD = CUS.CUST_CNT_CD AND CUST_SEQ = CUS.CUST_SEQ), (SELECT REPLACE(CUST_GRP_NM,CHR(10), ' ') FROM MDM_CUST_PERF_GRP WHERE CUST_GRP_ID = MCUST.CUST_GRP_ID)) CUST_NM" ).append("\n"); 

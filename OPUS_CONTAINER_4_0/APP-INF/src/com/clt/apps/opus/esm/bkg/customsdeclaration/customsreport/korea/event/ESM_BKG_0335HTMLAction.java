...
*@FileName : ESM_BKG_0335HTMLAction.java
*@FileTitle : ESM_BKG_0335HTMLAction
*Open Issues :
*Change history :
 ...
 * @since J2EE 1.4
 */
public class ESM_BKG_0335HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 2792293631096541043L;

	public ESM_BKG_0335HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		// EVENT 생성
		EsmBkg0335Event event = new EsmBkg0335Event();

		// COMMAND 구분에 따른 처리
		if (command.isCommand(FormCommand.SEARCH)) {
			// 조회의 경우 처리
			KorEntryTpCondVO korEntryTpCondVO = (KorEntryTpCondVO)getVO(request, KorEntryTpCondVO.class);
			event.setKorEntryTpCondVO(korEntryTpCondVO);
		} else  if ( command.isCommand(FormCommand.MULTI))	{
			// 입력/수정/삭제의 경우 처리
			event.setKorEntryTpCondVOs((KorEntryTpCondVO[])getVOs(request, KorEntryTpCondVO.class));
		}

...
public class ArrivalNoticeDBDAOsearchArrNtcCustListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       DECODE(BKGM.CHG_DP_FLG, 'Y', '1', '0')   				AS CHG_DP_FLG" ).append("\n"); 
		...
		query.append("             , DECODE(BCST.VAL_CD, 'X', NULL, BCST.VAL_CD) AS VAL_CD -- X(Auto-Cancel)은 Evaluation하지 않은 것으로 처리" ).append("\n"); 
		...
		query.append("              , DECODE(BCST.AN_SND_FLG, 'Y', DECODE( BCST.MTCH_FLG,'Y','A', BCST.VAL_CD)  -- X(Auto-Cancel)은 AN_SND_FLG가 N이므로 변경 불필요" ).append("\n"); 
		...
		query.append("                   )" ).append("\n"); 
		query.append("              )" ).append("\n"); 
		query.append("          AND NVL(BCST.VAL_CD, '*') <> 'S'  -- SKIP은 NOTICE를 발송하지 않는다." ).append("\n"); 

...
*@FileName : AGNCommApprovalDBDAOGetAcmCsrCondInfoRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AGNCommApprovalDBDAOGetAcmCsrCondInfoRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * CSR Info에 대한 조건 정보 조회
	  * 
	  * 2015.12.28 [#9182] Change Biz Common Approval Office to Invoice Office
	  * </pre>
	  */
	public AGNCommApprovalDBDAOGetAcmCsrCondInfoRSQL(){
		setQuery();
		...
		query.append("FileName : AGNCommApprovalDBDAOGetAcmCsrCondInfoRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                   || SUBSTR(B.LOCAL_DT, 1, 4)" ).append("\n"); 
		query.append("                   || NVL (MAX (TO_NUMBER (SUBSTR(X.CSR_NO,LENGTH (X.CSR_NO)-3)))+1,1001)" ).append("\n"); 
		query.append("                 FROM AP_CSR_NO X" ).append("\n"); 
		query.append("                WHERE X.CSR_NO LIKE '08'||CASE WHEN A.CSR_AMT < 0 THEN 'C' ELSE 'S' END||B.AR_OFC_CD||SUBSTR(B.LOCAL_DT, 1, 4)||'%'" ).append("\n"); 
		...
		query.append("                  AND EFF_YRMON    >= SUBSTR(REPLACE (@[inv_dt], '-', ''), 1, 6)" ).append("\n"); 
		query.append("         )" ).append("\n"); 
		query.append("	  ELSE '01' " ).append("\n"); 
		query.append("       END AS GL_DT," ).append("\n"); 
		query.append("           F.ERR_VVD_BKG_NO," ).append("\n"); 
		query.append("         F.ERR_VVD," ).append("\n"); 
		query.append("           SUBSTR (G.ERR_VVD_LVL, 1, 12)     AS ERR_VVD_LVL_BKG_NO," ).append("\n"); 
		query.append("           SUBSTR (G.ERR_VVD_LVL, 13)        AS ERR_VVD_LVL_VVD_CD," ).append("\n"); 
		query.append("           SUBSTR (G.ERR_VVD_LVL_FLG, 1, 12) AS ERR_VVD_LVL_FLG_BKG_NO," ).append("\n"); 
		query.append("           SUBSTR (G.ERR_VVD_LVL_FLG, 13)    AS ERR_VVD_LVL_FLG_VVD_CD," ).append("\n"); 
		...
		query.append("           B.APRL_OFC_CD          		 	 AS H_APRL_OFC_CD," ).append("\n"); 
		query.append("           (SELECT OFC_ENG_NM FROM MDM_ORGANIZATION M WHERE M.OFC_CD = B.APRL_OFC_CD) AS H_APRL_OFC_NM," ).append("\n"); 
		query.append("           @[upd_usr_id]                     AS UPD_USR_ID," ).append("\n"); 
		query.append("           @[cre_usr_id]                     AS CRE_USR_ID" ).append("\n"); 
		query.append("      FROM" ).append("\n"); 
		query.append("         ( ---- GET CSR AMOUNT" ).append("\n"); 
		query.append("               SELECT SUM(CSR_AMT) AS CSR_AMT" ).append("\n"); 
		query.append("                 FROM(  " ).append("\n"); 
		query.append("                   SELECT" ).append("\n"); 
		query.append("                          MAX (" ).append("\n"); 
		query.append("                     CASE" ).append("\n"); 
		query.append("                     WHEN B.OFF_SET_FLG = 'Y'" ).append("\n"); 
		query.append("                     THEN 0" ).append("\n"); 
		query.append("                     ELSE ROUND (SUM (A.PAY_IF_AMT+(A.PAY_IF_AMT*NVL (@[inv_tax_rt],0)/100)) ,M.DP_PRCS_KNT)" ).append("\n"); 
		...
		query.append("                      WHERE OFC_CD = @[ar_ofc_cd] ) B," ).append("\n"); 
		...
		query.append("                 GROUP BY B.OFF_SET_FLG," ).append("\n"); 
		...
		query.append("                      B.AP_CTR_CD," ).append("\n"); 
		query.append("                      B.AR_HD_QTR_OFC_CD," ).append("\n"); 
		query.append("                      A.OFC_CD AS APRL_OFC_CD" ).append("\n"); 
		...
		query.append("                WHERE A.AC_STS_CD = 'AS'" ).append("\n"); 
		query.append("#if(${aud_no} != '')" ).append("\n"); 
		query.append("                  AND A.AUD_NO" ).append("\n"); 
		query.append("                   IN" ).append("\n"); 
		query.append("                    ( ${aud_no}" ).append("\n"); 
		...
		query.append("         (" ).append("\n"); 
		query.append("              SELECT MAX(ERR_VVD_BKG_NO) AS ERR_VVD_BKG_NO," ).append("\n"); 
		query.append("                     MAX(ERR_VVD)        AS ERR_VVD " ).append("\n"); 
		query.append("                FROM(" ).append("\n"); 
		query.append("                  SELECT" ).append("\n"); 
		query.append("                          MAX (X.BKG_NO) AS ERR_VVD_BKG_NO," ).append("\n"); 
		query.append("                     MAX (X.REV_VVD_CD) AS ERR_VVD" ).append("\n"); 
		query.append("                     FROM ACM_AGN_BKG_INFO X," ).append("\n"); 
		query.append("                          ACM_AGN_COMM Y" ).append("\n"); 
		query.append("                    WHERE X.BKG_NO = Y.BKG_NO" ).append("\n"); 
		query.append("#if(${aud_no} != '')" ).append("\n"); 
		query.append("                    AND Y.AUD_NO" ).append("\n"); 
		query.append("                             IN" ).append("\n"); 
		query.append("                              ( ${aud_no}" ).append("\n"); 
		query.append("                               )" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                     MAX (X.REV_VVD_CD) AS ERR_VVD" ).append("\n"); 
		query.append("                     FROM ACM_AGN_BKG_INFO X," ).append("\n"); 
		query.append("                          ACM_AGN_OTR_COMM Y" ).append("\n"); 
		query.append("                    WHERE X.BKG_NO = Y.OTR_COMM_NO" ).append("\n"); 
		query.append("#if(${aud_no} != '')" ).append("\n"); 
		query.append("                      AND Y.AUD_NO" ).append("\n"); 
		query.append("                               IN" ).append("\n"); 
		query.append("                                ( ${aud_no}" ).append("\n"); 
		...
		query.append("                                            ACM_AGN_BKG_INFO  Y," ).append("\n"); 
		query.append("                                            ACM_AGN_COMM      Z" ).append("\n"); 
		query.append("                                      WHERE Y.BKG_NO = Z.BKG_NO" ).append("\n"); 
		query.append("#if(${aud_no} != '')" ).append("\n"); 
		query.append("                                        AND Z.AUD_NO" ).append("\n"); 
		query.append("                                         IN" ).append("\n"); 
		query.append("                                          ( ${aud_no}" ).append("\n"); 
		query.append("                                          )" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                                       FROM AR_ROUT_RNK       X," ).append("\n"); 
		query.append("                                            ACM_AGN_BKG_INFO  Y," ).append("\n"); 
		query.append("                                            ACM_AGN_OTR_COMM  Z" ).append("\n"); 
		query.append("                                      WHERE Y.BKG_NO = Z.OTR_COMM_NO" ).append("\n"); 
		query.append("#if(${aud_no} != '')" ).append("\n"); 
		query.append("                                        AND Z.AUD_NO" ).append("\n"); 
		query.append("                                         IN" ).append("\n"); 
		query.append("                                          ( ${aud_no}" ).append("\n"); 

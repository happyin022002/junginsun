...
public class WeeklyCMDBDAOSearchVVDChkWithARListVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * coa_mon_vvd 테이블에서 Month VVD 정보를 조회   
	  * </pre>
	  */
	...
	public void setQuery(){
		query.append("SELECT   DECODE(A.TRD_CD    ,NULL, 'A', DECODE(B.TRD_CD, NULL, 'C' ,'B'   ) ) RSLT_CD /* C:COA ONLY, B:BOTH */	" ).append("\n"); 
		query.append("        ,DECODE(A.TRD_CD    ,NULL, 'IF Only', DECODE(B.TRD_CD, NULL, 'COA Only', 'BOTH') ) RSLT" ).append("\n"); 
		...
		query.append("        RIGHT OUTER JOIN  /* IF ONLY 찾기 위해(COA_MON_VVD_IF 에만 있는 항차)*/" ).append("\n"); 
		...
		query.append("  UNION	 /* COA ONLY를 위해 */													" ).append("\n"); 
		query.append("  SELECT  DECODE(A.TRD_CD    , NULL, 'A'       ,  					" ).append("\n"); 
		query.append("          DECODE(B.TRD_CD    , NULL, 'C'       , 'B'   ) ) RSLT_CD 	" ).append("\n"); 
		query.append("         ,DECODE(A.TRD_CD    , NULL, 'IF Only' ,  					" ).append("\n"); 
		query.append("          DECODE(B.TRD_CD    , NULL, 'COA Only', 'BOTH') ) RSLT 	    " ).append("\n"); 
		query.append("         ,NVL(A.TRD_CD       , B.TRD_CD                  ) TRD_CD 	" ).append("\n"); 
		query.append("         ,NVL(A.RLANE_CD     , B.RLANE_CD                ) RLANE_CD  " ).append("\n"); 
		query.append("         ,NVL(A.IOC_CD       , B.IOC_CD                  ) IOC_CD    " ).append("\n"); 
		query.append("         ,NVL(A.VSL_CD       , B.VSL_CD                  ) VSL_CD 	" ).append("\n"); 

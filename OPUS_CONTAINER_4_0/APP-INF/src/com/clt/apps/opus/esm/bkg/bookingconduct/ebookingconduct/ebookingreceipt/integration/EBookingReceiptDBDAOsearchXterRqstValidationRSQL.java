...
public class EBookingReceiptDBDAOsearchXterRqstValidationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT nvl(MAX(DECODE(SNACCS_MSG_TP_CD,'SAT049',DECODE(BKG_UPLD_STS_CD,'N','Y','P','Y','N'))),'N') as xter_cre_flag" ).append("\n"); 
		query.append(", nvl(MAX(DECODE(SNACCS_MSG_TP_CD,'SAT053',DECODE(BKG_UPLD_STS_CD,'N','Y','P','Y','N'))),'N') as xter_sr_flag" ).append("\n"); 
		query.append("FROM BKG_XTER_RQST_MST" ).append("\n"); 
		query.append("WHERE XTER_SNDR_ID = @[xter_sndr_id]" ).append("\n"); 
		query.append("AND SNACCS_MSG_TP_CD IN ('SAT049','SAT053' )" ).append("\n"); 

...
public class BLDocumentationBLDBDAOAddXptLicNoByXterCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       DECODE(@[caed_tp_cd], 'EX', decode(trim(@[b13a_xpt_ctnt]), '', '', substr(trim(@[b13a_xpt_ctnt]), 1, 10)), '')," ).append("\n"); 
		query.append("       DECODE(@[caed_tp_cd], 'EX', decode(trim(@[b13a_xpt_ctnt]), '', '', substr(trim(@[b13a_xpt_ctnt]), 11, 20)), '')," ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${ca_flg} == 'Y')" ).append("\n"); 
		query.append("       ( SELECT DECODE(SUBSTR(POR_CD,1,2),'MX','O',DECODE(SUBSTR(POL_CD,1,2),'MX','O','I')) FROM BKG_BKG_HIS" ).append("\n"); 
		query.append("          WHERE BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("            AND CORR_NO = 'TMP0000001' )," ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("       ( SELECT DECODE(SUBSTR(POR_CD,1,2),'MX','O',DECODE(SUBSTR(POL_CD,1,2),'MX','O','I')) FROM BKG_BOOKING" ).append("\n"); 
		query.append("          WHERE BKG_NO = @[bkg_no] )," ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("       NVL((SELECT MAX(XPT_IMP_SEQ) FROM BKG_XPT_IMP_LIC WHERE BKG_NO = @[bkg_no] AND IO_BND_CD = 'O'), 0) + 1," ).append("\n"); 
		...
		query.append("       DECODE(@[aes_tp_cd], 'EX', @[aes_expt_ctnt], '')," ).append("\n"); 
		query.append("	   0," ).append("\n"); 
		query.append("	   'N'," ).append("\n"); 
		query.append("       0," ).append("\n"); 
		query.append("       0," ).append("\n"); 

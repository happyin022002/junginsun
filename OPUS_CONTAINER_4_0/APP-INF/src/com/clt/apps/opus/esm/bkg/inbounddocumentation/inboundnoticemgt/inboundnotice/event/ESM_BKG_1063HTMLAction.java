...
*@FileName : ESM_BKG_1063HTMLAction.java
...
public class ESM_BKG_1063HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_BKG_1063HTMLAction 객체를 생성
	 */
	public ESM_BKG_1063HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg1063Event event = new EsmBkg1063Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setPkupNoMnlUpldVO((PkupNoMnlUpldVO)getVO(request, PkupNoMnlUpldVO .class));
		}
		else if(command.isCommand(FormCommand.MULTI)) {
			event.setPkupNoMnlUpldVOs((PkupNoMnlUpldVO[])getVOs(request, PkupNoMnlUpldVO .class, "sheet1_"));
		}
		else if(command.isCommand(FormCommand.SEARCH04)) {
			event.setJobId(JSPUtil.getParameter(request, "job_id"));
		}
		else if(command.isCommand(FormCommand.SEARCH05)) {
			event.setBatchCd(JSPUtil.getParameter(request, "batch_cd"));
		}

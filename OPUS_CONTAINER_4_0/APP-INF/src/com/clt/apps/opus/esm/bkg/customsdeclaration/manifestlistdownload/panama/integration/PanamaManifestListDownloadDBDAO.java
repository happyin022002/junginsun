	...
	public List<VesselVO> searchVesselList(
		...
		try {
			if (panamaVesselCondVO != null) {
				Map<String, String> mapVO = panamaVesselCondVO.getColumnValues();

				param.putAll(mapVO);
				velParam.putAll(mapVO);
			}
			dbRowset = new SQLExecuter("DEFAULT").executeQuery((ISQLTemplate)
					new PanamaManifestListDownloadDBDAOsearchVesselListRSQL(), param, velParam);
			list = (List) RowSetUtil.rowSetToVOs(dbRowset, PanamaVesselVO.class);
		} catch (SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage());
		} catch (Exception ex) {

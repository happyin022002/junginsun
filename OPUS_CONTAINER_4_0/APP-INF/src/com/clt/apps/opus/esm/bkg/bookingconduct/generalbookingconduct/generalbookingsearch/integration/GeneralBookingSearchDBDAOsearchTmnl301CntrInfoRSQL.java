...
public class GeneralBookingSearchDBDAOsearchTmnl301CntrInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                                                  WHERE SPC_TP = 'GC'" ).append("\n"); 
		query.append("                                                  AND SPC.TB_ROW_ID = SCHC.ROWID" ).append("\n"); 
		query.append("                                                  AND ROWNUM = 1" ).append("\n"); 
		query.append("                                   ), 11, ' ')		|| CHR(10)" ).append("\n"); 
		query.append("	|| 'CNTT:'					|| NVL(CNTR.CNTR_TPSZ_CD,SCHC.CNTR_TPSZ_CD)		|| CHR(10)" ).append("\n"); 
		...
		query.append("    || 'NWGT_QTY:'              || CASE WHEN NVL(Cntr.CNTR_WGT,0) = 0 THEN ROUND((SELECT BLD.ACT_WGT/NVL((SELECT SUM(BQ.OP_CNTR_QTY) FROM BKG_QUANTITY BQ WHERE BQ.BKG_NO = BLD.BKG_NO),1) FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO),3)" ).append("\n"); 
		query.append("                                        ELSE Cntr.CNTR_WGT END 					|| CHR(10)" ).append("\n"); 
		query.append("	|| 'NWGT_TP:'               || CASE WHEN Cntr.WGT_UT_CD IS NULL THEN (SELECT BLD.WGT_UT_CD FROM BKG_BL_DOC BLD WHERE BLD.BKG_NO = BK.BKG_NO)" ).append("\n"); 
		query.append("                                        ELSE Cntr.WGT_UT_CD END 					|| CHR(10)" ).append("\n"); 
		...
		query.append("    || CASE WHEN (SELECT COUNT(*) FROM BKG_CNTR_MF_DESC BCM WHERE BCM.BKG_NO = CNTR.BKG_NO AND BCM.CNTR_NO = CNTR.CNTR_NO) = 0 THEN '{CNTR_CARGO'|| CHR(10) || 'CARGO_DESC:'||CHR(10) ||'}CNTR_CARGO'||CHR(10)" ).append("\n"); 
		query.append("            ELSE" ).append("\n"); 
		...
		query.append("                 AND (BK.AWK_CGO_FLG = 'Y' AND BHCC.HRD_CDG_ID = 'VNDR301_INSTRUCT' AND BHCC.ATTR_CTNT1 = SCHC.CNTR_TPSZ_CD)" ).append("\n"); 
		...
		query.append(" WHERE BK.BKG_NO   	    = SCHC.BKG_NO" ).append("\n"); 
		...
		query.append("   AND NOT EXISTS (SELECT 'X' FROM BKG_CONTAINER BC1 WHERE BC1.BKG_NO = SCHC.BKG_NO AND BC1.CNTR_NO = SCHC.CNTR_NO  " ).append("\n"); 
		query.append("                                               AND (BC1.DCGO_FLG    = 'Y'" ).append("\n"); 
		query.append("                                               OR BC1.RC_FLG      = 'Y'" ).append("\n"); 
		query.append("                                               OR BC1.AWK_CGO_FLG = 'Y'))" ).append("\n"); 
		...
		query.append("   AND ROWNUM <= (SELECT TTL" ).append("\n"); 
		query.append("       -(SELECT COUNT(*) FROM BKG_RF_CGO RF WHERE RF.BKG_NO = BQ.BKG_NO AND RF.cntr_tpsz_cd is not null)" ).append("\n"); 
		query.append("       -(SELECT COUNT(DISTINCT DG.DG_CNTR_SEQ) FROM BKG_DG_CGO DG WHERE DG.BKG_NO = BQ.BKG_NO AND DG.cntr_tpsz_cd is not null)" ).append("\n"); 
		query.append("       -(SELECT COUNT(*) FROM BKG_AWK_CGO AWK WHERE AWK.BKG_NO = BQ.BKG_NO AND AWK.CNTR_TPSZ_CD NOT LIKE 'Q%')" ).append("\n"); 
		query.append("        FROM (" ).append("\n"); 
		query.append("            SELECT BQ.BKG_NO" ).append("\n"); 
		query.append("                   ,SUM(BQ.OP_CNTR_QTY) TTL" ).append("\n"); 
		query.append("            FROM BKG_QUANTITY BQ" ).append("\n"); 
		query.append("            WHERE BQ.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("            AND BQ.CNTR_TPSZ_CD NOT LIKE 'Q%'" ).append("\n"); 
		query.append("            GROUP BY BQ.BKG_NO) BQ)" ).append("\n"); 
		query.append("UNION ALL" ).append("\n"); 
		query.append("SELECT '{CNTR_INFO'																|| CHR(10)" ).append("\n"); 
		query.append("	|| 'CNTN:'					|| RPAD(NVL(CNTR.CNTR_NO, ' '), 11, ' ')		|| CHR(10)" ).append("\n"); 
		query.append("    || 'CSN:'                   || RPAD((SELECT SLOT_NO||SLOT_NO_SEQ" ).append("\n"); 
		...
		query.append("                                                      FROM (" ).append("\n"); 
		query.append("                                                          SELECT 'GC' SPC_TP, SCH.ROWID TB_ROW_ID, SUBSTR(SCH.BKG_NO,-5) SLOT_NO" ).append("\n"); 
		query.append("                                                          FROM SCE_COP_HIS SCH" ).append("\n"); 
		query.append("                                                          WHERE SCH.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("                                                          AND   SCH.ACCL_SEQ = (SELECT MAX(HIS.ACCL_SEQ) FROM SCE_COP_HIS HIS WHERE HIS.COP_NO = SCH.COP_NO AND HIS.COP_STS_CD <> 'X')" ).append("\n"); 
		...
		query.append("	|| 'CNTT:'					|| NVL(CNTR.CNTR_TPSZ_CD,SCHC.CNTR_TPSZ_CD)		|| CHR(10)" ).append("\n"); 
		query.append("	|| 'MRN:'					|| (SELECT BR.CUST_REF_NO_CTNT FROM BKG_REFERENCE BR WHERE BR.BKG_REF_TP_CD = 'CMRN' AND BR.BKG_NO = BK.BKG_NO AND BR.CNTR_NO = CNTR.CNTR_NO AND ROWNUM = 1)	|| CHR(10)" ).append("\n"); 
		query.append("    || 'UCR:'					|| (SELECT BR.CUST_REF_NO_CTNT FROM BKG_REFERENCE BR WHERE BR.BKG_REF_TP_CD = 'CUCR' AND BR.BKG_NO = BK.BKG_NO AND BR.CNTR_NO = CNTR.CNTR_NO AND ROWNUM = 1)	|| CHR(10)" ).append("\n"); 
		query.append("    || 'HS_CD:'					|| (SELECT BCMD.CMDT_HS_CD FROM BKG_CNTR_MF_DESC BCMD WHERE BCMD.BKG_NO = CNTR.BKG_NO AND BCMD.CNTR_NO = CNTR.CNTR_NO AND ROWNUM = 1) || CHR(10)" ).append("\n"); 
		query.append("	|| 'RIND:'                  || decode(CNTR.RC_FLG,'Y','1','0')				|| CHR(10)" ).append("\n"); 
		query.append("	|| 'DIND:'                  || decode(CNTR.DCGO_FLG,'Y','1','0')			|| CHR(10)" ).append("\n"); 
		query.append("	|| 'AIND:'                  || decode(CNTR.AWK_CGO_FLG,'Y','1','0')			|| CHR(10)" ).append("\n"); 

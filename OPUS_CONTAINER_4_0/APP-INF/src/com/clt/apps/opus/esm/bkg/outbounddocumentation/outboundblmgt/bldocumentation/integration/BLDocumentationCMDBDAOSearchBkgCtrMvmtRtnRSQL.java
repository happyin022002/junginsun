...
*@FileName : BLDocumentationCMDBDAOSearchBkgCtrMvmtRtnRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class BLDocumentationCMDBDAOSearchBkgCtrMvmtRtnRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * BLDocumentationCMDBDAOSearchBkgCtrMvmtRtnR
	  * </pre>
	  */
	public BLDocumentationCMDBDAOSearchBkgCtrMvmtRtnRSQL(){
		setQuery();
		...
		query.append("FileName : BLDocumentationCMDBDAOSearchBkgCtrMvmtRtnRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    SYS_AREA_GRP_CD," ).append("\n"); 
		query.append("    CNTR_NO" ).append("\n"); 
		query.append("FROM (" ).append("\n"); 
		query.append("SELECT /*+ ORDERED USE_NL(BC B B1) */" ).append("\n"); 
		query.append("       B1.BKG_NO , 'BKG95065' ERR_MSG_CD, '' CNT_CD, '' SYS_AREA_GRP_CD, BC.CNTR_NO" ).append("\n"); 
		...
		query.append("AND BC.CRE_DT >= (SYSDATE - 90) " ).append("\n"); 
		query.append("AND B1.BKG_CGO_TP_CD ='F'" ).append("\n"); 
		query.append("AND (B1.BKG_STS_CD = 'W' OR B1.BKG_STS_CD = 'F')" ).append("\n"); 
		query.append("AND (BC.RC_FLG = 'Y' OR BC.DCGO_FLG = 'Y' OR BC.AWK_CGO_FLG = 'Y')" ).append("\n"); 
		...
		query.append("SELECT /*+ ORDERED USE_NL(BC B B1) */" ).append("\n"); 
		query.append("       B1.BKG_NO , 'BKG00966' ERR_MSG_CD, '' CNT_CD, '' SYS_AREA_GRP_CD, BC.CNTR_NO" ).append("\n"); 
		query.append("FROM BKG_CONTAINER BC, BKG_BOOKING B, BKG_BOOKING B1" ).append("\n"); 
		query.append("WHERE 1=1 " ).append("\n"); 
		query.append("AND B.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("AND B.BKG_NO <> B1.BKG_NO" ).append("\n"); 
		...
		query.append("                   AND BC.CRE_DT >= (SYSDATE - 90) /* 오래된 bkg_no과 연결된 cntr_no는 해당 없음 */" ).append("\n"); 
		query.append("				)                 " ).append("\n"); 
		query.append("AND (CASE WHEN (SELECT HRD.ATTR_CTNT5  " ).append("\n"); 
		...
		query.append("(SELECT /*+ ORDERED USE_NL(BK B1 VVD1 VVD2 SKD1 SKD2 CNTR) */" ).append("\n"); 
		...
		query.append("                        AND B1.BKG_NO <> BK.BKG_NO" ).append("\n"); 
		query.append("                        AND BK.VSL_CD = B1.VSL_CD" ).append("\n"); 
		query.append("                        AND BK.SKD_VOY_NO = B1.SKD_VOY_NO" ).append("\n"); 
		query.append("                        AND BK.SKD_DIR_CD = B1.SKD_DIR_CD" ).append("\n"); 
		...
		query.append("                              ,ROW_NUMBER() OVER(PARTITION BY CNTR_NO ORDER BY CNMV_YR DESC, CNMV_ID_NO DESC, CNMV_SEQ DESC) RN" ).append("\n"); 
		query.append("                          FROM CTM_MOVEMENT" ).append("\n"); 
		query.append("                         WHERE CNTR_NO = @[cntr_no]) CTM" ).append("\n"); 
		query.append("                 WHERE @[mvmt_sts_cd] = 'OC'" ).append("\n"); 
		query.append("                   AND CTM.MVMT_STS_CD = 'OP'" ).append("\n"); 
		query.append("                   AND CTM.RN = 2)" ).append("\n"); 
		query.append("UNION" ).append("\n"); 
		query.append("SELECT /*+ ORDERED USE_NL(HRD A B) */" ).append("\n"); 
		query.append("       B.BKG_NO , 'BKG00966' ERR_MSG_CD, '' CNT_CD, '' SYS_AREA_GRP_CD, A.CNTR_NO" ).append("\n"); 
		...
		query.append("                  AND ROWNUM =1 " ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("AND NOT EXISTS (SELECT 'Y' --Why????" ).append("\n"); 
		query.append("                FROM BKG_BOOKING BK" ).append("\n"); 
		query.append("                WHERE 1=1" ).append("\n"); 
		query.append("                  AND BK.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("                  AND BK.VSL_CD = B.VSL_CD" ).append("\n"); 
		...
		query.append("UNION" ).append("\n"); 
		query.append("SELECT /*+ ORDERED USE_NL(HRD A B) */" ).append("\n"); 
		query.append("       B.BKG_NO , 'BKG95063' ERR_MSG_CD, '' CNT_CD, '' SYS_AREA_GRP_CD, A.CNTR_NO" ).append("\n"); 
		query.append("FROM   BKG_HRD_CDG_CTNT HRD, CTM_BKG_CNTR A, CTM_BOOKING B" ).append("\n"); 
		query.append("WHERE  A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("AND    A.CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("AND    A.CRE_DT >= (SYSDATE - 90) /* 오래된 bkg_no과 연결된 cntr_no는 해당 없음 */" ).append("\n"); 
		...
		query.append("       A.BKG_NO BKG_NO , 'BKG08046' ERR_MSG_CD, '' CNT_CD, '' SYS_AREA_GRP_CD, A.CNTR_NO" ).append("\n"); 

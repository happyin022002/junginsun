...
public class TCharterIOEstimatedDBDAOCustomEstmIfVOCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		   SUBSTR(@[vvd_cd], 9, 1), SUBSTR(@[vvd_cd], 10, 1)," ).append("\n"); 
		query.append("	       @[days],                   @[hire_amt],              @[est_amt],                   @[act_amt]," ).append("\n"); 
		query.append("	       @[acc_amt],                ESTM_VVD_TP_CD,           ESTM_IOC_DIV_CD,              DECODE(ESTM_VVD_HDR_ID, 0, NULL, ESTM_VVD_HDR_ID), ESTM_BC_DIV_CD," ).append("\n"); 
		query.append("		   @[cre_usr_id],			  SYSDATE,					@[cre_usr_id],			  	  SYSDATE	" ).append("\n"); 
		...
		query.append("			   AND  REV_DIR_CD = SUBSTR(@[vvd_cd], 10, 1)" ).append("\n"); 

...
public class TCharteIOContractDAOSearchContractByPrepaymentRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        CASE WHEN HIRE_MAX_EXP_DT  <= TO_DATE(REPLACE(EXP_DT,'-','')||REPLACE(TO_TIME,':',''),'YYYYMMDDHH24MI') THEN " ).append("\n"); 
		query.append("				  TO_CHAR(HIRE_MAX_EXP_DT,'HH24:MI') " ).append("\n"); 
		query.append("			 ELSE " ).append("\n"); 
		query.append("				  TO_TIME " ).append("\n"); 
		query.append("		END TO_TIME, " ).append("\n"); 
		query.append("		CASE WHEN SIGN(HIRE_MAX_EXP_DT  - TO_DATE(REPLACE(EXP_DT,'-',''),'YYYYMMDDHH24MI')) > -1 THEN " ).append("\n"); 
		query.append("                  CASE WHEN (HIRE_MAX_EXP_DT <= TO_DATE(REPLACE(EXP_DT,'-','')||REPLACE(TO_TIME,':',''),'YYYYMMDDHH24MI')) " ).append("\n"); 
		query.append("                            THEN DECODE(EXP_DT,NULL,NULL,TO_CHAR(ROUND(TO_DATE(REPLACE(EXP_DT,'-','') || REPLACE(TO_CHAR(HIRE_MAX_EXP_DT,'HH24:MI'),':',''),'YYYYMMDDHH24MI') - TO_DATE(REPLACE(EFF_DT,'-','') || REPLACE(FROM_TIME,':',''),'YYYYMMDDHH24MI'),4),'FM999,999.0000'))" ).append("\n"); 
		query.append("                       ELSE DECODE(EXP_DT,NULL,NULL,TO_CHAR(ROUND(TO_DATE(REPLACE(EXP_DT,'-','') || REPLACE(TO_TIME,':',''),'YYYYMMDDHH24MI') - TO_DATE(REPLACE(EFF_DT,'-','') || REPLACE(FROM_TIME,':',''),'YYYYMMDDHH24MI'),4),'FM999,999.0000'))" ).append("\n"); 

...
public class ESM_BKG_3011HTMLAction extends HTMLActionSupport {

    private static final long serialVersionUID = 1L;
    /**
     * ESM_BKG_0926HTMLAction 객체를 생성
     */
    public ESM_BKG_3011HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg3011Event event = new EsmBkg3011Event();

		if (command.isCommand(FormCommand.SEARCH))
		{
			event.setCountryCode(request.getParameter("country_code"));
			event.setBlIssOfcNm(request.getParameter("place_of_issue_name"));
			
		} else if (command.isCommand(FormCommand.REMOVE)) {
			event.setBlEsigAsgnSeq(request.getParameter("bl_esig_asgn_seq"));
			event.setCountryCode(request.getParameter("country_code"));
			event.setBlIssOfcNm(request.getParameter("place_of_issue_name"));
		}

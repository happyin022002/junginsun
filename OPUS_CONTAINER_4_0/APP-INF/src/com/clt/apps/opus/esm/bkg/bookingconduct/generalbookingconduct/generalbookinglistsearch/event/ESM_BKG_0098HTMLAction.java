...
public class ESM_BKG_0098HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.SEARCH)) {
			...
			event.setCctMnl(JSPUtil.getParameter(request, "sheet1_cct_mnl", length));
			event.setDocCct(JSPUtil.getParameter(request, "sheet1_doc_cct", length));
			event.setCargoCctMnl(JSPUtil.getParameter(request, "sheet1_cargo_cct_mnl", length));
			...
			event.setRemark(JSPUtil.getParameter(request, "sheet1_remark", length));
			event.setRmkChangeFlg(JSPUtil.getParameter(request, "sheet1_rmk_change_flg", length));
			event.setCct(JSPUtil.getParameter(request, "sheet1_cct", length));
			event.setCctMnl(JSPUtil.getParameter(request, "sheet1_cct_mnl", length));
			event.setDocCct(JSPUtil.getParameter(request, "sheet1_doc_cct", length));
			event.setCargoCctMnl(JSPUtil.getParameter(request, "sheet1_cargo_cct_mnl", length));
			event.setMrdNm(JSPUtil.getParameter(request, "mrd"));
			event.setBtnTp(JSPUtil.getParameter(request, "btnTp"));
			...
			if(command.isCommand(FormCommand.MULTI04))
				event.setCustBody(JSPUtil.getParameter(request, "custBody"));

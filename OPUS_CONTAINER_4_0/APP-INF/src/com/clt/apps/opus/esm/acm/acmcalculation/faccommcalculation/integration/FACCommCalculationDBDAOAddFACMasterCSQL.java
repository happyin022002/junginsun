...
public class FACCommCalculationDBDAOAddFACMasterCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", CASE WHEN @[ppd_ofc_cd_chg_yn] = 'Y' THEN NVL(@[chg_ppd_ofc_cd],'HAMBB') ELSE NVL(@[ppd_ofc_cd],'HAMBB') END AS SLS_OFC_CD" ).append("\n"); 
		...
		query.append("	AND FAC_SEQ = @[fac_seq] - 1 ),0) AS PPD_AMT" ).append("\n"); 
		query.append(", @[crnt_amt]               CRNT_AMT" ).append("\n"); 
		query.append(", ( @[crnt_amt] -" ).append("\n"); 
		query.append("NVL((SELECT ROUND (NVL (CRNT_AMT, 0), 2)" ).append("\n"); 
		query.append("  FROM ACM_FAC_COMM " ).append("\n"); 
		query.append(" WHERE BKG_NO = @[bkg_no]  " ).append("\n"); 
		query.append("	AND FAC_SEQ = @[fac_seq] - 1 ),0)" ).append("\n"); 
		...
		query.append(", CASE WHEN @[ppd_ofc_cd_chg_yn] = 'Y' THEN @[chg_ar_ofc_cd] ELSE @[bkg_ar_ofc_cd] END AS AR_OFC_CD" ).append("\n"); 
		query.append(", CASE WHEN @[ppd_ofc_cd_chg_yn] = 'Y' THEN @[chg_ap_ofc_cd] ELSE @[ap_ofc_cd] END AS AP_OFC_CD" ).append("\n"); 
		...
		query.append("  FROM ACM_FAC_COMM " ).append("\n"); 
		query.append(" WHERE BKG_NO = @[bkg_no]  " ).append("\n"); 
		query.append("	AND FAC_SEQ = @[fac_seq] - 1 ),0) AS PAY_PPD_AMT -- " ).append("\n"); 
		...
		query.append(", GLOBALDATE_PKG.TIME_CONV_OFC_FNC('SINHO',TO_DATE (@[apro_dt], 'YYYYMMDDHH24MISS'),'GMT') APRO_GDT --" ).append("\n"); 
		query.append(", @[if_usr_id]                  IF_USR_ID --" ).append("\n"); 
		query.append(", TO_DATE (@[if_dt], 'YYYYMMDDHH24MISS') IF_DT --" ).append("\n"); 
		query.append(", GLOBALDATE_PKG.TIME_CONV_OFC_FNC('SINHO',TO_DATE (@[if_dt], 'YYYYMMDDHH24MISS'),'GMT') IF_GDT --" ).append("\n"); 

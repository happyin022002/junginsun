...
public class SpecialCargoReceiptDBDAOcopyBbCgoBySplitCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",(" ).append("\n"); 
		query.append("	SELECT NVL(MAX(BB_CGO_SEQ), 0)" ).append("\n"); 
		query.append("	FROM   BKG_BB_CGO" ).append("\n"); 
		query.append("	WHERE  BKG_NO = @[targetBkg]) + ROW_NUMBER() OVER (PARTITION BY BKG_NO ORDER BY BB_CGO_SEQ " ).append("\n"); 
		query.append(") AS  BB_CGO_SEQ " ).append("\n"); 
		query.append(",RCV_TERM_CD " ).append("\n"); 
		query.append(",DE_TERM_CD " ).append("\n"); 
		query.append(",PCK_QTY " ).append("\n"); 
		query.append(",PCK_TP_CD " ).append("\n"); 

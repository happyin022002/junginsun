...
*@FileName : EurCustomsTransmissionDBDAOsearchDangerCgoRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class EurCustomsTransmissionDBDAOsearchDangerCgoRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public EurCustomsTransmissionDBDAOsearchDangerCgoRSQL(){
		setQuery();
		...
		query.append("FileName : EurCustomsTransmissionDBDAOsearchDangerCgoRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        ,NVL(A.IMDG_CLSS_CD, ' ') CLASS1" ).append("\n"); 
		query.append("        ,NVL(A.PRP_SHP_NM, '') DG_DESC" ).append("\n"); 
		query.append("        ,NVL(A.EMER_CNTC_PSON_NM, '') CONTACT_NM" ).append("\n"); 
		query.append("        ,NVL(A.EMER_CNTC_PHN_NO_CTNT, '') PHONE" ).append("\n"); 
		query.append("        ,'' PAGE" ).append("\n"); 
		query.append("        ,NVL(A.FLSH_PNT_CDO_TEMP,'') FLSH_TEMP" ).append("\n"); 
		query.append("        ,DECODE(A.FLSH_PNT_CDO_TEMP, NULL, '',  'C') FLSH_UNIT" ).append("\n"); 
		query.append("        ,BKG_TOKEN_NL_FNC(NVL(A.DIFF_RMK,' '),0,'') DG_REMARK" ).append("\n"); 
		query.append("        ,NVL(B.IMDG_EMER_NO, '') EMSNO" ).append("\n"); 
		query.append("        ,NVL(A.PSA_NO, '') PSACLS" ).append("\n"); 
		query.append("        ,NVL(A.IMDG_PCK_GRP_CD, '') PKGGRP" ).append("\n"); 
		...
		query.append("        ,NVL(A.OUT_IMDG_PCK_QTY1, 0) D_PKG" ).append("\n"); 
		query.append("        ,NVL(A.OUT_IMDG_PCK_CD1, '') D_PKGUNIT" ).append("\n"); 
		query.append("        ,NVL(NET_WGT, '') NWGT" ).append("\n"); 
		query.append("        ,'KGS' NWGT_UNIT" ).append("\n"); 
		query.append("        ,NVL(GRS_WGT, '') GWGT" ).append("\n"); 
		query.append("        ,NVL(WGT_UT_CD, '') GWGT_UNIT" ).append("\n"); 
		query.append("        ,NVL(MEAS_QTY, '') MEA" ).append("\n"); 

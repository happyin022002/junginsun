...
public class JapanManifestListDownloadDBDAOsearchVslEtaDmfRemarkRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT A.VSL_CD||A.SKD_VOY_NO||A.SKD_DIR_CD AS IN_VVD_CD," ).append("\n"); 
		query.append("       A.POD_CD AS IN_POD_CD," ).append("\n"); 
		query.append("       TO_CHAR(A.ETA_DT, 'YYYY-MM-DD') AS ETA_DT1," ).append("\n"); 
		query.append("       TO_CHAR(A.ETA_DT, 'HH24:MI:SS') AS ETA_DT2," ).append("\n"); 
		query.append("       A.ARR_YD_ID ASARR_YD_CD," ).append("\n"); 
		query.append("       NVL(B.LODG_WGT , 0) AS LODG_WGT," ).append("\n"); 
		query.append("       B.CSTMS_MF_ID AS CSTMS_MF_CD," ).append("\n"); 
		query.append("       B.MF_RMK," ).append("\n"); 
		query.append("       NVL(A.IB_CSSM_VOY_NO, '') AS IB_CSSM_VOY_NO," ).append("\n"); 
		...
		query.append("           AND SKD.SKD_VOY_TP_CD IN ('CA', 'CJ', 'CV')" ).append("\n"); 
		query.append("           AND ROWNUM = 1) AS IN_JOINT_FLG" ).append("\n"); 
		query.append("" ).append("\n"); 

...
*@FileName : ESM_BKG_0967HTMLAction.java
*@FileTitle :
*Open Issues :
*Change history :
 ...
 * @see EsmBkg0967Event 참조
 * @since J2EE 1.6
 */
public class ESM_BKG_0967HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * ESM_FMS_0967HTMLAction 객체를 생성
	 */
	public ESM_BKG_0967HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmBkg0967Event event = new EsmBkg0967Event();
		...
		if (command.isCommand(FormCommand.SEARCH01)
			...
			event.setDgCargoCondVO((DgCargoCondVO) getVO(request, DgCargoCondVO.class));
		} else if(command.isCommand(FormCommand.SEARCH05) // Forward Name 조회
					|| command.isCommand(FormCommand.SEARCH06) // UN NO desc 조회
				)  {
			event.setDgListCondVO((DgListCondVO) getVO(request, DgListCondVO.class));
		} else if (command.isCommand(FormCommand.MODIFY01) // Save(the Container)
					|| command.isCommand(FormCommand.MODIFY02) // Save(the Whole BKG)
				) {
			event.setDgInqModiVO((DgInqModiVO) getVO(request, DgInqModiVO.class));
		}

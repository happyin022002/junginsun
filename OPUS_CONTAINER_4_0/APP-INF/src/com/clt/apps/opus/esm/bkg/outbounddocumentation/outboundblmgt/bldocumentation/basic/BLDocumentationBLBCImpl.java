...
public class BLDocumentationBLBCImpl extends BasicCommandSupport implements BLDocumentationBLBC {
	...
	*/
     ...
     * @exception EventException
     */
	public void modifyBlIssInfoHistoryForBlDoc(BlIssInfoVO blIssInfoVO) throws EventException{
        try {
            dbDao.modifyBlIssInfoHistoryForBlDoc(blIssInfoVO);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
...
//            log.debug("bkgRefTpCd>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+bkgRefTpCd);
    ...
    public void manageMnd(MndVO mndVO, SignOnUserAccount account, String caFlg) throws EventException {
        ...
        bkgBlDocVO.setMeasQtyPrnFlg(mndVO.getMeasQtyPrnFlg());
        bkgBlDocVO.setCntrWgtPrnFlg(mndVO.getCntrWgtPrnFlg());
        bkgBlDocVO.setCntrMeasPrnFlg(mndVO.getCntrMeasPrnFlg());
        bkgBlDocVO.setMfDescPrnFlg(mndVO.getMfDescPrnFlg());
        bkgBlDocVO.setMkDescPrnFlg(mndVO.getMkDescPrnFlg());
        bkgBlDocVO.setFrtPayOfcPrnFlg(mndVO.getFrtPayOfcPrnFlg());
...
//            throw ex;
     ...
     * @exception EventException
     */
    public void modifyBlObrdDtHistory(GrpBlDtListVO grpBlDtListVO) throws EventException {
        try {
            dbDao.modifyBlObrdDtHistory(grpBlDtListVO);
        } catch(DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch(Exception ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
...
//            BookingUtil utilBC = new BookingUtil();
            if(mtyBookingCreateVO != null){
                ...
                // 020 result + new line + "       EMPTY CONTAINER (NYK Line)"append -> bkg_bl_mk_desc.cmdt_desc put in
                cmdtDesc = cmdtDesc + "\n" + "       EMPTY CONTAINER (NYK Line)";
...
//                    String svrId = utilBC.searchSvrByOfc(account.getOfc_cd());
                    /* MT�쓽 寃쎌슦 Server ID 泥댄겕 �븯吏� �븡�룄濡� 湲곕뒫 蹂�寃�
    ...
    public void modifyBkgBlDocByCust(BkgBlNoVO bkgBlNoVO, String orgCntNm, SignOnUserAccount account, String oldActCustCd, String newActCustCd) throws EventException {
    	...
    	try {       
            ...
            if(!oldActCustCd.equals(newActCustCd)){
            	...
            	dbDao.modifyMndByBooking(blMkDescVO);
            }
        } catch (DAOException ex) {
            throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);
        } catch (Exception ex) {
...
//                      if(dbDao.searchMtySocCntrCheck(cntrNo)){
                	...
                	if("I".equals(repoCntrVOs[i].getIbflag())){
	                    ...
	                    */
                        // 080. searchMtyCntrSts()result-> aciac_div_cd(active/inactive)<> 'A':ACT -> Message[BKG00952] view-> stop
                        if(!"A".equals(mtyCntrStsVO.getAciacDivCd())){
                            throw new EventException((String)new ErrorHandler("BKG00952",new String[]{cntrNo,mtyCntrStsVO.getCnmvStsCd()}).getMessage());
                        }             		
                	}
...
//                    }
            ...
            // 40 result + new line + "       EMPTY CONTAINER (NYK Line)"append -> bkg_bl_mk_desc.cmdt_desc put in
            cmdtDesc = cmdtDesc + "\n" + "       EMPTY CONTAINER (NYK Line)";
	 ...
	 * @throws EventException
	 */
    public List<BkgCntrShpVO> searchBkgCntrShp(String bkgNo) throws EventException{
		try {
            return dbDao.searchBkgCntrShp(bkgNo);
	    } catch(DAOException ex) {
	    	throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);       
	    } catch(Exception ex) {
	    	throw new EventException(new ErrorHandler("COM12240").getMessage(), ex);       
	 ...
	 * 
     * @param BkgCntrShpVO[] bkgCntrShpVOs
     * @param String bkgNo
     * @param SignOnUserAccount account
     * @exception EventException
	 */
    public void modifyMultiShpCntrPrn(BkgCntrShpVO[] bkgCntrShpVOs, String bkgNo, SignOnUserAccount account) throws EventException{
        try {        	
            if(bkgCntrShpVOs != null && bkgCntrShpVOs.length>0){
	            for(int i=0;i<bkgCntrShpVOs.length;i++){
	            	if(bkgCntrShpVOs[i].getIbflag().equals("I")){
		            	dbDao.addMultiShpCntrPrn(bkgCntrShpVOs[i], bkgNo, account);
	            	}else if(bkgCntrShpVOs[i].getIbflag().equals("U")){
		            	dbDao.modifyMultiShpCntrPrn(bkgCntrShpVOs[i], bkgNo, account);
	            	}else if(bkgCntrShpVOs[i].getIbflag().equals("D")){
		            	dbDao.removeMultiShpCntrPrn(bkgCntrShpVOs[i], bkgNo);
	            	}
	            }
            }
        } catch(DAOException ex) {

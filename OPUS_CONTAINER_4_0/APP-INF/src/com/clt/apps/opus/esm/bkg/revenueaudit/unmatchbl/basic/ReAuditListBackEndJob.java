...
*@FileTitle : ReAuditListBackEndJob
*Open Issues :
*Change history :
...
public class ReAuditListBackEndJob extends BackEndCommandSupport {
	...
	public String doStart() throws Exception {
		String bkgStsCd, bkgNo, ctrtTpCd, corrNo, svcScpCd, scRtTp;
		...
		try {
			...
			bkgStsCd = ""; ctrtTpCd = ""; corrNo = ""; svcScpCd = ""; scRtTp = ""; 
...
//		            		unmatchList = command.checkRfaUnmatchBLbyBooking(bkgNo, "N", "C"); // C : 데이터 생성시 디테일 가져옴.
	            			...
	            			listF  = command.checkRfaSurchargeDiscrepancy(bkgNo, "N");    // F
...
//	            				}else{
	            					scOftList = scOftCmd.searchScOftAutoratingList(bkgNo, "N", "", rtAplyDt, svcScpCd, "");
//	            				}
	            				// Self Audit SC 리스트 - 전체
...
//		            			command.modifyORCOTHChargeRate(bkgNo, "N"); // 특정 SC에 대해 OTH=ORC 호환하여 심사
	    	            		unmatchList = command.checkScUnmatchBLIncludeOFTbyBooking(bkgNo, "N", "C"); // C : 데이터 생성시 디테일 가져옴.
	            			}

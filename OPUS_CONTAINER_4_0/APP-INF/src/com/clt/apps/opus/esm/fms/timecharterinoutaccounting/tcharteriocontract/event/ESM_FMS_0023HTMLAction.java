...
*@FileName : ESM_FMS_0023HTMLAction.java
...
public class ESM_FMS_0023HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_FMS_0023HTMLAction 객체를 생성
	 */
	public ESM_FMS_0023HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmFms0023Event event = new EsmFms0023Event();

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setSearchContracNoListByVesselVO((SearchContracNoListByVesselVO)getVO(request, SearchContracNoListByVesselVO.class));
		} else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setTypeFlag(request.getParameter("typeFlag"));
		}
		
		event.setVslCd(request.getParameter("vsl_cd"));
		event.setFletCtrtTpCd(request.getParameter("flet_ctrt_tp_cd"));
		event.setCtrtFlag(request.getParameter("ctrtFlag"));

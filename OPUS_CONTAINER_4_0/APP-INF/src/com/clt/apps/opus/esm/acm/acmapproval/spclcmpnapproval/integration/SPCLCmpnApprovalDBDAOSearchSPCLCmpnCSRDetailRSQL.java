...
*@FileName : SPCLCmpnApprovalDBDAOSearchSPCLCmpnCSRDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SPCLCmpnApprovalDBDAOSearchSPCLCmpnCSRDetailRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * DAOSearchSPCLCmpnCSRDetailRSQL
	  * </pre>
	  */
	public SPCLCmpnApprovalDBDAOSearchSPCLCmpnCSRDetailRSQL(){
		setQuery();
		...
		query.append("FileName : SPCLCmpnApprovalDBDAOSearchSPCLCmpnCSRDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       ,DENSE_RANK() OVER(ORDER BY X.ATT1, X.COMPANY, X.REGION, X.CENTER, X.ACCT, X.VVD, X.ATT7) AS LINE_NO" ).append("\n"); 
		...
		query.append("               ROUND(NVL(B.PAY_IF_DTRB_AMT,A.IF_AMT),2) AS INV_AMT, " ).append("\n"); 
		...
		query.append("               C.BL_NO||SUBSTR(TO_CHAR(A.SPCL_CMPN_SEQ ,'FM000000'),4,6) AS ATT1, " ).append("\n"); 
		query.append("               SUBSTR(@[gl_dt], 0, 4)||'/'||SUBSTR(@[gl_dt], 5, 2)||'/'||SUBSTR(@[gl_dt], 7, 2)||' 00:00:00' AS ATT2," ).append("\n"); 
		query.append("               A.SPCL_OCCR_INFO_CD AS ATT3, " ).append("\n"); 
		query.append("               '' AS ATT4, '' AS ATT5, " ).append("\n"); 
		...
		query.append("               '' AS ATT8, '' AS ATT9, " ).append("\n"); 
		query.append("               '' AS ATT10, @[gl_dt] AS ATT11, @[ap_ofc_cd] AS ATT12, '' AS ATT13, " ).append("\n"); 
		...
		query.append("               DECODE(A.SPCL_SLAN_CD||SUBSTR(A.SPCL_VSL_CD,0,2),'RBCFD','CFDR'||SUBSTR(A.SPCL_VSL_CD,3,2)||SUBSTR(A.SPCL_SKD_VOY_NO,0,2)||'EE',  " ).append("\n"); 
		query.append("                      A.SPCL_VSL_CD||A.SPCL_SKD_VOY_NO||A.SPCL_SKD_DIR_CD||NVL(A.SPCL_REV_DIR_CD,A.SPCL_SKD_DIR_CD )) AS REV_VVD, " ).append("\n"); 
		...
		query.append("          FROM ACM_SPCL_CMPN A, ACM_SPCL_CMPN_DTL B, ACM_AGN_BKG_INFO C " ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("        /* [CHM-201114163] 남미 Compensation 계산시 VAT 계정 분리 */" ).append("\n"); 
		query.append("        UNION ALL " ).append("\n"); 
		query.append("        SELECT A.VNDR_SEQ VNDR, " ).append("\n"); 
		query.append("               'ITEM' AS LOOKUP, " ).append("\n"); 
		query.append("        --     ROUND(NVL(B.ACT_USD_COMM_AMT,A.ACT_IF_COMM_AMT),2) AS INV_AMT, " ).append("\n"); 
		query.append("               ROUND((SUM(A.PAY_IF_AMT) * NVL(A.INV_TAX_RT, 0) / 100), 2) AS INV_AMT, " ).append("\n"); 
		query.append("               'V.A.T' AS INV_DESC, " ).append("\n"); 
		query.append("               '' AS TAX_CD, " ).append("\n"); 
		query.append("               '01' AS COMPANY, " ).append("\n"); 
		query.append("               NVL((SELECT FINC_RGN_CD FROM MDM_ORGANIZATION WHERE OFC_CD = A.AP_OFC_CD),'00') AS REGION, " ).append("\n"); 
		query.append("               (SELECT AP_CTR_CD FROM MDM_ORGANIZATION WHERE OFC_CD = A.AP_OFC_CD) AS CENTER, '111821' AS ACCT, " ).append("\n"); 
		query.append("              '0000000000' AS VVD, " ).append("\n"); 
		query.append("               (SELECT NVL(LTRIM(SUBS_CO_CD),'00') FROM MDM_VENDOR WHERE VNDR_SEQ = A.VNDR_SEQ) AS INTR_CMPY, " ).append("\n"); 
		query.append("               '000000' AS FUTURE1, " ).append("\n"); 
		query.append("               '000000' AS FUTURE2, " ).append("\n"); 
		query.append("               '111821' AS ATT_CTLG, " ).append("\n"); 
		query.append("               --'000000000000000' AS ATT1, " ).append("\n"); 
		query.append("               'VAT'||TO_CHAR(SYSDATE,'YYMMDDHH24MISS') AS ATT1, " ).append("\n"); 
		query.append("               SUBSTR(@[gl_dt], 0, 4)||'/'||SUBSTR(@[gl_dt], 5, 2)||'/'||SUBSTR(@[gl_dt], 7, 2)||' 00:00:00' AS ATT2, " ).append("\n"); 
		query.append("               A.SPCL_OCCR_INFO_CD AS ATT3, " ).append("\n"); 
		query.append("               '' AS ATT4, '' AS ATT5, '' AS ATT6, '' AS ATT7, '' AS ATT8, '' AS ATT9, " ).append("\n"); 
		query.append("               '' AS ATT10, @[gl_dt] AS ATT11, @[ap_ofc_cd] AS ATT12, '' AS ATT13, 'COM' AS ATT14, '' AS ATT15, " ).append("\n"); 
		...
		query.append("               '' AS AGNT, " ).append("\n"); 
		query.append("               '' AS SUB_FLG, " ).append("\n"); 
		query.append("               '000000000000' AS BL_NO, " ).append("\n"); 
		query.append("               A.CSR_NO AS CSR_NO " ).append("\n"); 
		query.append("          FROM ACM_SPCL_CMPN A, ACM_AGN_BKG_INFO C " ).append("\n"); 
		query.append("         WHERE A.CUST_CNT_CD||TO_CHAR(A.CUST_SEQ,'FM000000') = @[cust_cnt_seq]" ).append("\n"); 
		query.append("/* 날짜 조회 조건 */       " ).append("\n"); 
		query.append("#if(${date_div} == 'C')      " ).append("\n"); 
		query.append("                        AND A.IF_DT IS NULL" ).append("\n"); 
		...
		query.append("                 ) " ).append("\n"); 
		query.append("           AND a.bkg_no       = c.bkg_no(+)        " ).append("\n"); 
		query.append("           AND NVL(a.inv_tax_rt,0) > 0 " ).append("\n"); 
		query.append("         GROUP BY A.CSR_NO, A.vndr_seq, A.INV_TAX_RT,A.AP_OFC_CD,A.SPCL_OCCR_INFO_CD, A.SPCL_SLAN_CD " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("        /* 상계정산 대리점 */" ).append("\n"); 
		query.append("        UNION ALL " ).append("\n"); 
		query.append("        SELECT A.VNDR_SEQ VNDR, " ).append("\n"); 
		query.append("               'ITEM' AS LOOKUP, " ).append("\n"); 
		query.append("        --     ROUND(NVL(B.ACT_USD_COMM_AMT,A.ACT_IF_COMM_AMT),2) AS INV_AMT, " ).append("\n"); 
		query.append("               SUM (-ROUND(A.PAY_IF_AMT,2)) AS INV_AMT, " ).append("\n"); 
		query.append("               " ).append("\n"); 
		query.append("                ( SELECT SUBSTR (ACCT_ENG_NM, 1, 39)" ).append("\n"); 
		...
		query.append("               '01' AS COMPANY, " ).append("\n"); 
		query.append("               NVL((SELECT FINC_RGN_CD FROM MDM_ORGANIZATION WHERE OFC_CD = A.AP_OFC_CD),'00') AS REGION, " ).append("\n"); 
		query.append("               (SELECT AP_CTR_CD FROM MDM_ORGANIZATION WHERE OFC_CD = A.AP_OFC_CD) AS CENTER, " ).append("\n"); 
		...
		query.append("               --'000000000000000' AS ATT1, " ).append("\n"); 
		query.append("               'ASA'||TO_CHAR(SYSDATE,'YYMMDDHH24MISS') AS ATT1, " ).append("\n"); 
		query.append("               SUBSTR(@[gl_dt], 0, 4)||'/'||SUBSTR(@[gl_dt], 5, 2)||'/'||SUBSTR(@[gl_dt], 7, 2)||' 00:00:00' AS ATT2, " ).append("\n"); 
		query.append("               A.SPCL_OCCR_INFO_CD AS ATT3, " ).append("\n"); 
		query.append("               '' AS ATT4, '' AS ATT5, '' AS ATT6, '' AS ATT7, '' AS ATT8, '' AS ATT9, " ).append("\n"); 
		...
		query.append("               @[ap_ofc_cd] AS ATT12, '' AS ATT13, 'COM' AS ATT14, '' AS ATT15, " ).append("\n"); 
		query.append("               '' AS BKG_NO, " ).append("\n"); 
		query.append("               '' AS TPSZ, " ).append("\n"); 
		query.append("               '' AS REV_VVD, " ).append("\n"); 
		...
		query.append("               0 AS QTY, " ).append("\n"); 
		query.append("               '' AS TMNL_CD, " ).append("\n"); 
		query.append("               A.SPCL_OFC_CD AS AGNT, " ).append("\n"); 
		query.append("               '' AS SUB_FLG, " ).append("\n"); 
		query.append("               '000000000000' AS BL_NO, " ).append("\n"); 
		query.append("               A.CSR_NO AS CSR_NO " ).append("\n"); 
		query.append("          FROM ACM_SPCL_CMPN A, ACM_AGN_BKG_INFO C " ).append("\n"); 
		...
		query.append("               WHERE OFC_CD = A.SPCL_OFC_CD" ).append("\n"); 
		query.append("             )         " ).append("\n"); 
		query.append("         " ).append("\n"); 
		query.append("/* 날짜 조회 조건 */       " ).append("\n"); 
		query.append("#if(${date_div} == 'C')      " ).append("\n"); 
		...
		query.append("                 ) " ).append("\n"); 
		query.append("           AND A.BKG_NO       = C.BKG_NO(+)" ).append("\n"); 
		query.append("         GROUP BY A.CSR_NO, A.VNDR_SEQ, A.AP_OFC_CD, A.SPCL_OFC_CD, A.SPCL_OCCR_INFO_CD, A.SPCL_SLAN_CD " ).append("\n"); 

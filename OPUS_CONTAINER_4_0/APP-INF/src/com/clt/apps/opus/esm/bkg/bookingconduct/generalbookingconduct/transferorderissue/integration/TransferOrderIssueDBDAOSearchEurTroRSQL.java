...
public class TransferOrderIssueDBDAOSearchEurTroRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		, CASE WHEN TRO.CNTR_RTN_DT IS NULL AND @[io_bnd_cd] = 'O' THEN TO_CHAR((SELECT NVL(MAX(MNL_SET_DT), MAX(SYS_SET_DT)) RTN_DT" ).append("\n"); 
		...
		query.append("		" ).append("\n"); 
		query.append("		 ELSE TO_CHAR(TRO.CNTR_RTN_DT, 'YYYY-MM-DD') " ).append("\n"); 
		query.append("		 END AS CNTR_RTN_DT" ).append("\n"); 
		query.append("		 ,CASE WHEN TRO.CNTR_RTN_DT IS NULL AND @[io_bnd_cd] = 'O' THEN TO_CHAR((SELECT NVL(MAX(MNL_SET_DT), MAX(SYS_SET_DT)) RTN_DT" ).append("\n"); 
		query.append("                                                                         FROM BKG_CLZ_TM CLZ, BKG_BOOKING BK " ).append("\n"); 
		query.append("                                                                        WHERE BK.BKG_NO     = CLZ.BKG_NO" ).append("\n"); 
		query.append("  		                                                                  AND BK.BKG_NO = @[bkg_no]" ).append("\n"); 
		query.append("                                                                          AND CLZ.CLZ_TP_CD = 'R'" ).append("\n"); 
		query.append("		                                                                GROUP BY BK.BKG_NO" ).append("\n"); 
		query.append("		                                                              ),'HH24:MI')" ).append("\n"); 
		query.append("		" ).append("\n"); 
		query.append("		 ELSE TO_CHAR(TRO.CNTR_RTN_DT, 'HH24:MI') " ).append("\n"); 
		query.append("		 END AS CNTR_RTN_DT_HHMI" ).append("\n"); 
		...
		query.append("--                            AND so_plan.COST_ACT_GRP_CD LIKE DECODE(tro.io_bnd_cd, 'O', 'OD', 'ID')||'%')" ).append("\n"); 
		query.append("--                           , 'F', '(FR)', '')) SO_NO" ).append("\n"); 
		query.append("				DECODE(SO.TRSP_FRST_FLG, 'Y', '(FR)', '')) SO_NO" ).append("\n"); 

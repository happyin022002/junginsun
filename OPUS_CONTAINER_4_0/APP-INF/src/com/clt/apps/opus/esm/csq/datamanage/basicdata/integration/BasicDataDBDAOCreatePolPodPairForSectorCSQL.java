...
public class BasicDataDBDAOCreatePolPodPairForSectorCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,NVL((SELECT DISTINCT T.CSQ_ACT_FLG" ).append("\n"); 
		query.append("      		FROM CSQ_SCTR_PAIR_MGMT T" ).append("\n"); 
		query.append("     		WHERE T.BSE_YR||T.BSE_QTR_CD = (SELECT /*+ INDEX_DESC(CSQ_SCTR_PAIR_MGMT XPKCSQ_SCTR_PAIR_MGMT) */" ).append("\n"); 
		...
		query.append("       		AND T.POD_CD   = B1.POD_PORT ), 'N') AS CSQ_ACT_FLG" ).append("\n"); 
		query.append("      ,NVL((SELECT DISTINCT T.CSQ_MN_SCTR_FLG" ).append("\n"); 
		query.append("      		FROM CSQ_SCTR_PAIR_MGMT T" ).append("\n"); 
		query.append("     		WHERE T.BSE_YR||T.BSE_QTR_CD = (SELECT /*+ INDEX_DESC(CSQ_SCTR_PAIR_MGMT XPKCSQ_SCTR_PAIR_MGMT) */" ).append("\n"); 
		query.append("                                             BSE_YR || BSE_QTR_CD" ).append("\n"); 
		query.append("                                        	FROM CSQ_SCTR_PAIR_MGMT" ).append("\n"); 
		...
		query.append("       		AND T.DIR_CD   = B1.SKD_DIR_CD" ).append("\n"); 
		query.append("       		AND T.POL_CD   = B1.POL_PORT" ).append("\n"); 
		query.append("       		AND T.POD_CD   = B1.POD_PORT ), '') AS CSQ_MN_SCTR_FLG" ).append("\n"); 
		...
		query.append("      ,SYSDATE" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		query.append("        SELECT @[f_trd_cd] AS TRD_CD" ).append("\n"); 

...
public class GeneralBookingReceiptDBDAOaddBkgBookingForCopySplitCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("         when length(@[bkg_no]) =  12 and 'Y' = (select 'Y' from bkg_chn_bkg_no_gen where bkg_no like substr(@[bkg_no], 1, 10)||'%' and rownum = 1) " ).append("\n"); 
		query.append("			then 'Y'" ).append("\n"); 
		...
		query.append("         when length(@[bkg_no]) =  12 and 'Y' = (select 'Y' from bkg_chn_bkg_no_gen where bkg_no like substr(@[bkg_no], 1, 10)||'%' and rownum = 1) " ).append("\n"); 
		query.append("			then (select chn_agn_cd from bkg_chn_bkg_no_gen where bkg_no like substr(@[bkg_no], 1, 10)||'%' and agn_flg = 'Y' and rownum = 1) " ).append("\n"); 
		...
		query.append(",	@[split_rsn_cd]" ).append("\n"); 
		query.append(",	@[split_rto]" ).append("\n"); 
		query.append(",	DECODE(@[bkg_cre_tp_cd], 'S', GLOBALDATE_PKG.TIME_CONV_FNC(COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC(),sysdate,BKG_COM_USER_LOC_FNC(@[cre_usr_id])), NULL)" ).append("\n"); 
		query.append(",   DECODE(@[bkg_cre_tp_cd], 'S', GLOBALDATE_PKG.TIME_CONV_FNC(COM_ConstantMgr_PKG.COM_getBaseLocationCode_FNC(), sysdate, 'GMT'), NULL)" ).append("\n"); 
		query.append(",	@[sc_no]" ).append("\n"); 
		query.append(",	@[rfa_no]" ).append("\n"); 
		query.append(",	@[taa_no]" ).append("\n"); 
		query.append(",	@[agmt_act_cnt_cd]" ).append("\n"); 

...
public class JapanTerminalTransmissionDBDAOsearchJapanRFCgoInfoRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT NVL(CDO_TEMP, '') AS CDO_TEMP," ).append("\n"); 
		query.append("       NVL2(CDO_TEMP, 'CEL', '') AS CLNG_TP_CD," ).append("\n"); 
		query.append("       NVL(HUMID_NO, '') AS HUMID_NO," ).append("\n"); 
		query.append("       NVL(VENT_RTO, CBM_PER_HR_QTY) AS VENT_RTO," ).append("\n"); 
		query.append("       NVL2(VENT_RTO, 'PCT', 'CMH') AS VENT_UNIT" ).append("\n"); 
		query.append("" ).append("\n"); 

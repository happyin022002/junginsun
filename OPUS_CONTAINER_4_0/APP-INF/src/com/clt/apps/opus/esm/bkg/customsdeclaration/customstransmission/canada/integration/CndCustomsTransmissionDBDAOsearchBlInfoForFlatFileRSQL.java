...
public class CndCustomsTransmissionDBDAOsearchBlInfoForFlatFileRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      ,'9165'    AS BLREPNO" ).append("\n"); 
		query.append("      ,DECODE(BL.CSTMS_MF_TP_CD, 'S10', BL.MF_NO, BL.BL_NO) AS BLNBR" ).append("\n"); 
		query.append("      ,DECODE(BL.CSTMS_MF_TP_CD, 'S10', '9165' || BL.BL_NO, '') AS BLREFNO" ).append("\n"); 
		...
		query.append("           AND YD_CD = (CASE WHEN SUBSTR(BL.POD_CD, 1, 2) = 'CA' THEN BL.POD_NOD_CD" ).append("\n"); 
		query.append("                             ELSE (SELECT SUBSTR(MIN(TO_CHAR(V2.VPS_ETA_DT, 'YYYYMMDDHH24MISS')||V2.YD_CD), 15)" ).append("\n"); 
		...
		query.append("                                      AND V1.VPS_PORT_CD = BL.POL_CD" ).append("\n"); 
		...
		query.append("                        END)) AS TERMINAL" ).append("\n"); 
		...
		query.append("            ELSE (SELECT SUBSTR( MAX( NVL( POD_YD_NO, '  ') ||CSTMS_CD) , 3)" ).append("\n"); 
		...
		query.append("                 )" ).append("\n"); 
		query.append("             END CUSTOFCD" ).append("\n"); 
		...
		query.append("  AND BL.CSTMS_PORT_CD= CVSL.PORT_CD(+)" ).append("\n"); 

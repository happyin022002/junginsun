...
public class NetworkDistributionDBDAOSearchFixCostDistListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("    , DECODE(RNUM,1,ORG_CO_SLS_AMT,'')  AS ORG_CO_SLS_AMT                       --SJH.20141028.ADD" ).append("\n"); 
		query.append("    , DECODE(RNUM,1,SLT_INTER_PRC_AMT,'') AS SLT_INTER_PRC_AMT                  --SJH.20141028.ADD" ).append("\n"); 
		query.append("    , DECODE(RNUM,1,ORG_CO_SLS_AMT+NVL(SLT_INTER_PRC_AMT,0)) M_CO_SLS_AMT       --SJH.20141028.MOD, 20141106.MOD" ).append("\n"); 
		query.append("    --, M_TS_UC_AMT                       AS M_TS_UC_AMT" ).append("\n"); 
		query.append("    --, DECODE(RNUM,1,M_CO_SLS_AMT,'')    AS M_CO_SLS_AMT" ).append("\n"); 
		...
		query.append("    --,(SLT_INTER_PRC_AMT * TS_RTO)+D_FX_COST_DTRB_AMT  D_FX_COST_DTRB_AMT      --SJH.20141028.MOD" ).append("\n"); 
		query.append("    , D_FX_COST_DTRB_AMT                										--SJH.20141030.MOD" ).append("\n"); 
		...
		query.append("		   ,(SELECT SUM(T.CO_SLS_AMT)" ).append("\n"); 
		query.append("       		FROM COA_VVD_HIR T" ).append("\n"); 
		...
		query.append("		 	AND t.stnd_cost_cd not in('43102011', '54600000', '54600002')		--SJH.20141028.MOD, SJH.20141104.MOD : 54600001->54600002" ).append("\n"); 
		query.append("           	#if (${f_selcost} != '')" ).append("\n"); 
		query.append("              AND T.STND_COST_CD = @[f_selcost]" ).append("\n"); 
		query.append("           	#end" ).append("\n"); 
		query.append("        	)	 AS ORG_CO_SLS_AMT" ).append("\n"); 
		query.append("		   ,(SELECT SUM(T.NTWK_HIR_COST_AMT)                      " ).append("\n"); 
		query.append("            FROM COA_VVD_HIR T" ).append("\n"); 
		query.append("            WHERE A.TRD_CD     = T.TRD_CD" ).append("\n"); 
		query.append("            AND A.RLANE_CD   = T.RLANE_CD" ).append("\n"); 
		query.append("            AND A.IOC_CD     = T.IOC_CD" ).append("\n"); 
		...
		query.append("            AND A.SKD_VOY_NO = T.SKD_VOY_NO" ).append("\n"); 
		query.append("            AND A.DIR_CD     = T.DIR_CD            " ).append("\n"); 
		query.append("            AND T.STND_COST_CD = '54600002'     --SJH.20141104.MOD : 54600001->54600002" ).append("\n"); 
		query.append("           	#if (${f_selcost} != '')" ).append("\n"); 
		query.append("              AND T.STND_COST_CD = @[f_selcost]" ).append("\n"); 
		query.append("           	#end                                 " ).append("\n"); 
		query.append("            )    AS SLT_INTER_PRC_AMT                                           --SJH.20141028.ADD" ).append("\n"); 
		query.append("			-------------------------------------------------                                                                                                         " ).append("\n"); 
		...
		query.append("           --,MAX(C.TS_RTO)              AS TS_RTO								--SJH.20141028.ADD, 20141030" ).append("\n"); 

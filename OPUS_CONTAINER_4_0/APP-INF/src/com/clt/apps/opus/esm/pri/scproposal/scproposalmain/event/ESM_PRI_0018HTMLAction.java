...
*@FileName : ESM_PRI_0018HTMLAction.java
...
public class ESM_PRI_0018HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * ESM_PRI_0018HTMLAction 객체를 생성
	 */
	public ESM_PRI_0018HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EsmPri0018Event event = new EsmPri0018Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {
            event.setSpScpGlineCopyVO((SpScpGlineCopyVO)getVO(request, SpScpGlineCopyVO.class));
        } else if(command.isCommand(FormCommand.MULTI)) {
			event.setSpScpGlineCopyVOS((SpScpGlineCopyVO[])getVOs(request, SpScpGlineCopyVO.class,""));
		} else {
		    event.setSpScpGlineCopyVO((SpScpGlineCopyVO)getVO(request, SpScpGlineCopyVO.class));
		}

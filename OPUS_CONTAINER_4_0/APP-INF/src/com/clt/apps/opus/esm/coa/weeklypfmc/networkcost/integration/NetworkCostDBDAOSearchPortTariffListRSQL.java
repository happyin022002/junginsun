...
public class NetworkCostDBDAOSearchPortTariffListRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      , COUNT(*) TTL_CNT" ).append("\n"); 
		query.append("      , SUM(PSO_CNT) PSO_CNT" ).append("\n"); 
		query.append("      , SUM(COA_CNT) COA_CNT" ).append("\n"); 
		query.append("   FROM (" ).append("\n"); 
		query.append("#if (${f_yrtype} == 'W')      --20150806.MOD : MAX->SUM     " ).append("\n"); 
		query.append("         SELECT ( CASE WHEN SUM(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.SLS_YRMON, A.COST_WK, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) <> 0" ).append("\n"); 
		query.append("                       THEN 'C' " ).append("\n"); 
		query.append("                       --VVD에 전체에 대해서 하나라도 COA DATA가 있는 경우" ).append("\n"); 
		query.append("                       WHEN SUM(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.SLS_YRMON, A.COST_WK, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD)= 0" ).append("\n"); 
		query.append("                            AND SUM(NVL( A.INV_USD_AMT, 0)) OVER (PARTITION BY A.SLS_YRMON, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) = 0" ).append("\n"); 
		...
		query.append("         SELECT ( CASE WHEN SUM(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.COST_YRMON, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) <> 0" ).append("\n"); 
		query.append("                       THEN 'C' " ).append("\n"); 
		query.append("                       --VVD에 전체에 대해서 하나라도 COA DATA가 있는 경우" ).append("\n"); 
		query.append("                       WHEN SUM(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.COST_YRMON, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD)= 0" ).append("\n"); 
		query.append("                            AND SUM(NVL( A.INV_USD_AMT, 0)) OVER (PARTITION BY A.COST_YRMON, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) = 0" ).append("\n"); 
		query.append("                       THEN 'N'" ).append("\n"); 
		query.append("                       --해당 VVD에 대한 데이터가 COA, PSO 둘다 모두 한 건도 없는 경우" ).append("\n"); 
		query.append("                       ELSE DECODE(LAST_VALUE (A.SRC_PSO_BZTP_CD IGNORE NULLS) OVER (PARTITION BY A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD, A.WRK_DT, A.WRK_SEQ ), 1, 'T', 2, 'E')" ).append("\n"); 
		query.append("                       --COA는 없고 PSO쪽만 있는 경우 WRK_DT나 WRK SEQ로 별로 가져와야 함" ).append("\n"); 
		...
		query.append("              , CASE WHEN A.INV_USD_AMT <> 0 THEN 1 ELSE 0 END PSO_CNT" ).append("\n"); 
		query.append("              , CASE WHEN A.PORT_USD_AMT <> 0 THEN 1 ELSE 0 END COA_CNT" ).append("\n"); 
		query.append("              , SUM( ( NVL(A.INV_USD_AMT, 0)) ) PSO_TTL_EXP " ).append("\n"); 
		query.append("#if (${f_yrtype} == 'W')" ).append("\n"); 
		query.append("              , ( CASE WHEN MIN(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.SLS_YRMON, A.COST_WK, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) = 0" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("              , ( CASE WHEN MIN(NVL(A.PORT_USD_AMT, 0)) OVER (PARTITION BY A.COST_YRMON, A.SLAN_CD, A.VSL_CD, A.SKD_VOY_NO, A.DIR_CD) = 0" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("                       -- VVD별로 COA 쪽에 YARD 하나라도 누락인 것을 찾아야함." ).append("\n"); 
		query.append("                       THEN 0 " ).append("\n"); 
		query.append("                       ELSE SUM(NVL(A.PORT_USD_AMT, 0)) END) TTL_EXP" ).append("\n"); 
		...
		query.append("                   FROM (SELECT A1.COST_YRMON" ).append("\n"); 
		query.append("                              , A1.SLS_YRMON" ).append("\n"); 
		query.append("                              , MAX(A1.COST_WK) COST_WK" ).append("\n"); 
		query.append("                              , A1.SLAN_CD" ).append("\n"); 
		query.append("                              , A1.VSL_CD" ).append("\n"); 
		query.append("                              , A1.SKD_VOY_NO" ).append("\n"); 
		query.append("                              , A1.DIR_CD" ).append("\n"); 
		query.append("                              , MIN(A1.N1ST_LODG_PORT_ETD_DT) N1ST_LODG_PORT_ETD_DT" ).append("\n"); 
		...
		query.append("							AND NVL(A3.TRNK_IPT_FLG,'N')    = 'N'" ).append("\n"); 
		query.append("        #if (${f_yrtype} == 'W')" ).append("\n"); 
		...
		query.append("                              , A1.VSL_CD" ).append("\n"); 
		query.append("                              , A1.SKD_VOY_NO" ).append("\n"); 
		query.append("                              , A1.DIR_CD) A1" ).append("\n"); 
		...
		query.append("                    AND A2.VSL_CLSS_CAPA   = @[f_selrlane]			--SJH.20141128.MOD : RLANE_CD -> VSL_CLSS_CAPA" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("            " ).append("\n"); 
		...
		query.append("       GROUP BY ( CASE WHEN NVL(A.PORT_USD_AMT, 0)<> 0 THEN 'C'" ).append("\n"); 
		query.append("                       WHEN NVL(A.PORT_USD_AMT, 0)= 0 AND NVL(A.INV_USD_AMT, 0) = 0 THEN 'N'" ).append("\n"); 

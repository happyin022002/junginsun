...
public class UsCgoRlseBackEndJob extends BackEndCommandSupport {
	...
	public String doStart() throws Exception {
		USCargoReleaseOrder usCargoReleaseOrder =  new USCargoReleaseOrder();

		for (BkgCgoRlseVO bkgCgoRlseVO : this.bkgCgoRlseVOs) {

			int modRowsByMaster = dbDao.modifyUsCgoRlseEdi(bkgCgoRlseVO, this.account);
			if (modRowsByMaster > 0) dbDao.addUsCgoRlseHisByMultiEdi(bkgCgoRlseVO, this.account);    // SUCCESS

//			if( !"X".equals(dbDao.checkCstmsEvnt(bkgCgoRlseVO.getBlNo()))){
				usCargoReleaseOrder.bkbcUsCgoEdi(bkgCgoRlseVO.getBlNo(), this.multiSnd, this.account);
//			} else {

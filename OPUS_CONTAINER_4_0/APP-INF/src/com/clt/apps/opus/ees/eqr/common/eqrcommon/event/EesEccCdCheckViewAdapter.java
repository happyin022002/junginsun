...
*@FileName : EesEccCdCheckViewAdapter.java
*@FileTitle : EES_ECCCDCHECK 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_ECCCDCHECK 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesEccCdCheckViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String col = "";
		try{
			resultEcc = retVO.getResultECC();
			row   = condiVO.getRow();
			col   = condiVO.getCol();
			
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("	<TD COL=\""+ col +"\">"+ JSPUtil.getNull(resultEcc)+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

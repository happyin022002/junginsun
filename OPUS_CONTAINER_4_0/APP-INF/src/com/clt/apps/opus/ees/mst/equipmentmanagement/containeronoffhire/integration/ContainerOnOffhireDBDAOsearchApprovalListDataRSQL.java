...
public class ContainerOnOffhireDBDAOsearchApprovalListDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       AA.ONH_QTY," ).append("\n"); 
		query.append("       AA.PICK_QTY," ).append("\n"); 
		query.append("	   AA.PKUP_FM_DT||'~'||AA.PKUP_DUE_DT AS PKUP_DUE_DT,       " ).append("\n"); 
		query.append("       AA.MIN_ONH_DYS," ).append("\n"); 
		query.append("       AA.PKUP_CHG_AMT," ).append("\n"); 
		query.append("       AA.PKUP_CHG_CR_AMT," ).append("\n"); 
		query.append("       NVL(MST_COMMON_PKG.MST_LSE_AGMT_RT_GET_FNC(@[agmt_cty_cd], @[agmt_seq], 'LON', AA.CNTR_TPSZ_CD, @[sts_evnt_yd_cd]), 0) AS LIFT_ON_CHG," ).append("\n"); 
		...
		query.append("                                 WHERE A.CNTR_ONH_AUTH_NO = AA.CNTR_ONH_AUTH_NO " ).append("\n"); 
		query.append("                                 AND A.CNTR_TPSZ_CD       = AA.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                                 AND A.NEW_VAN_TP_CD = 'L')) LON_CHG," ).append("\n"); 
		query.append("	  (SELECT A.CNTR_SPEC_NO" ).append("\n"); 
		query.append("	   FROM LSE_AGMT_RT A, MST_CNTR_SPEC B" ).append("\n"); 
		...
		query.append("       AND   ROWNUM                 = 1) CNTR_SPEC_NO" ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(       " ).append("\n"); 
		query.append("	SELECT B.CNTR_ONH_AUTH_NO,B.CNTR_TPSZ_CD,B.NEW_VAN_TP_CD, TO_CHAR(B.ONH_QTY, '9,999,999,999,999,999') ONH_QTY, " ).append("\n"); 
		query.append("	       (SELECT TO_CHAR(COUNT(DISTINCT D.CNTR_NO), '9,999,999,999,999,999') 		" ).append("\n"); 
		query.append("	        FROM MST_CONTAINER C, MST_CNTR_STS_HIS D					" ).append("\n"); 
		query.append("	        WHERE 1=1" ).append("\n"); 
		...
		query.append("	        AND   A.CNTR_ONH_AUTH_NO IN (C.CNTR_AUTH_NO, C.CNTR_OFFH_AUTH_NO)" ).append("\n"); 
		...
		query.append("     		TO_CHAR(A.PKUP_FM_DT,'YYYY-MM-DD') PKUP_FM_DT,		" ).append("\n"); 
		...
		query.append("	 			FROM MDM_LOCATION B , MDM_EQ_ORZ_CHT O" ).append("\n"); 
		query.append("	 			WHERE B.LOC_CD = SUBSTR(@[sts_evnt_yd_cd],1,5)" ).append("\n"); 
		query.append("                AND   B.SCC_CD = O.SCC_CD" ).append("\n"); 
		query.append("	 		    )					" ).append("\n"); 
		query.append("	AND  TO_DATE(REPLACE(@[hire_date], '-', ''), 'yyyymmdd') BETWEEN A.PKUP_FM_DT AND A.PKUP_DUE_DT --TRUNC(GLOBALDATE_PKG.TIME_LOCAL_FNC(A.ONH_LOC_CD))" ).append("\n"); 
		...
		query.append("WHERE AA.ONH_QTY > AA.PICK_QTY" ).append("\n"); 

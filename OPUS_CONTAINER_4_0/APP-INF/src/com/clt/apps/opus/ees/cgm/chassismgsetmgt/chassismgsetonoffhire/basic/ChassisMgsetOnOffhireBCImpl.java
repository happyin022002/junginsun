...
import com.clt.apps.opus.ees.cgm.chassismgsetmgt.chassismgsetonoffhire.integration.ChassisMgsetOnOffhireEAIDAO;
 ...
 *
 * @author 
 * @see EES_CGM_1002EventResponse,ChassisMgsetOnOffhireBC each DAO class reference
 * @since J2EE 1.4
 */
...
public class ChassisMgsetOnOffhireBCImpl extends BasicCommandSupport implements ChassisMgsetOnOffhireBC {
	...
	/**
	 * ChassisMgsetOnOffhireBCImpl objects creation<br>
	 * ChassisMgsetOnOffhireDBDAO creation.<br>
	 */
	...
	/**
	 * chassis Eq Type Size modification. Manage [EES_CGM_1001]<br>
	 *
	...
	public void manageCHSEqTypeSizeBasic(CHSEqTpSzINVO[] cHSEqTpSzINVOs, SignOnUserAccount account) throws EventException{
		try {
            ...
            log.error("err " + ex.toString(), ex);
			if(ex.getMessage().indexOf("ORA-02292") > -1) {
				throw new EventException(new ErrorHandler("CGM10070",new String[]{""}).getMessage());
			}
            throw new EventException(new ErrorHandler("CGM20031").getMessage(),ex);
		}catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
			...
			for ( int i=0; i<mGSStatusInfoMGTVOs.length; i++ ) {
				...
				if(mGSStatusInfoMGTVOs[i].getNo().equals("1"))	{
					...
					if(eqAsetStsCd.equals("LSI")){
						...
						mGSStatusInfoMGTVOs[i].setOnhOfcCd(mgsStatusinfo2.getStsEvntOfcCd());
...
//					tmp.setVerify("");
	...
	public void manageCHSOnhireBasic(CHSOnHireMGTVO[] cHSOnHireINVOs, SignOnUserAccount account) throws EventException{
		try {
			...
			for ( int i=0; i<cHSOnHireINVOs.length; i++ ) {
				...
				log.debug("####### history table insert ####### : addCHSOnhireHistoryData" +cHSOnHireINVOs[i].getAgmtVerNo() );
	...
	public void manageMGSOnhireBasic(MGSOnHireMGTVO[] mGSOnHireINVOs, SignOnUserAccount account) throws EventException{
		try {
			...
			for ( int i=0; i<mGSOnHireINVOs.length; i++ ) {
				...
				log.debug("####### history table insert ####### : getOwnLse===" +mGSOnHireINVOs[i].getOwnLse() );
	...
	/**
	 *  ERP FA I/F event list. Retrieve [EES_CGM_1146]<br>
	 *
	...
	/**
	 * ERP FA I/F event list. FNS026-0001 send. [IF_CGM_002]<br>
    ...
    public void sendToFABasic(ErpFaInterfaceINVO[] erpFaInterfaceINVOs, SignOnUserAccount account) throws EventException {
		...
		try {
			if(erpFaInterfaceINVOs != null)
			{
				...
				erpFaInterfaceINVO.setAttribute4(fastscd);	// CGM_EQUIPMENT:FA_IF_SEQ
				...
				for ( int i=0; i<erpFaInterfaceINVOs.length; i++ )
				{
					if (erpFaInterfaceINVOs[i].getDelChk().equals("1")){
						 ...
						 */
						//update data
						erpFaInterfaceINVOs[i].setAttribute1(erpFaInterfaceINVOs[i].getCurrCd());		// Currency/ CGM_EQUIPMENT:CURR_CD
						erpFaInterfaceINVOs[i].setAttribute2(erpFaInterfaceINVOs[i].getActAmt());		// CGM_EQUIPMENT:AQZ_AMT
	...
	public void manageCHSTermChangeBasic(CHSTermStatusINVO[] cHSTermStatusINVOs,
		...
		try{
			...
			/*---------------------------------------------
				 CGM_EQ_STS_HIS, CGM_EQUIPMENT record and edit
			 ----------------------------------------------*/
	...
	/**
	 *  Eq Damage, disposal Flagging management.. SAVE  [MNR call]<br>
			...
			if ( iFMnrFlagVOs.size() > 0 ) {
				...
				for(int i=0;i<iFMnrFlagVOs.size();i++){
					...
					if(iFMnrVo.getFlagType().equals("DMG") || iFMnrVo.getFlagType().equals("DSP") ){
						...
						if(!"G".equals(iFMnrVo.getEqKindCd())){
							bc.createMNRStatusBasic(iFMnrVo);
						}
					}
				}
			}
	...
	public List<String> searchComBackEndJobStatusBasic(String key) throws EventException {
		...
		try {
			...
			rowSet.next();
			Thread.sleep(1000);
			result.add((String) rowSet.getObject("jb_sts_flg"));
			result.add((String) rowSet.getObject("jb_usr_err_msg"));
			return result;
	...
	public void modifyMGSCgmEquipmentBasic(CHSMasterMGTVO cHSMasterMGTVO) throws EventException{
		try {
				dbDao.modifyMGSCgmEquipmentData(cHSMasterMGTVO);	
		}catch (DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("CGM20031").getMessage(),ex);
		}catch (Exception ex) {

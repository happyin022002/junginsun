...
import com.clt.apps.opus.ees.eqr.simulationmanage.codsimulate.integration.CodSimulateDBDAO;
...
/**
 * -SimulationManage Business Logic Command Interface<br>
 * - -SimulationManage에 대한 비지니스 로직에 대한 인터페이스<br>
 *
...
public class CodSimulateBCImpl extends BasicCommandSupport implements CodSimulateBC {
	 ...
	 * @exception EventException
	 */
	public CommonRsVO searchRepoPlanPreInfo(EesEqr0012ConditionVO eesEqr0012ConditionVO ,SignOnUserAccount account) throws EventException {
		try {
			return dbDao.searchRepoPlanPreInfo(eesEqr0012ConditionVO);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {
	 ...
	 * @exception EventException
	 */
	public CommonRsVO searchCntrRepoInOutPlanVvdInfo(EesEqr0012ConditionVO eesEqr0012ConditionVO,SignOnUserAccount account) throws EventException {
		try {
			return dbDao.searchCntrRepoInOutPlanVvdInfo(eesEqr0012ConditionVO,account);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {
	...
	public CommonRsVO searchCODVskLaneData(EesEqr0012ConditionVO eesEqr0012ConditionVO) throws EventException {
		try {
			return dbDao.searchCODVskLaneData(eesEqr0012ConditionVO); 
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {
	...
	public void modifyRepoPlanInfo(EesEqr0012ConditionVO conditionVO ,EesEqr0012MultiVO[] mutiVO,SignOnUserAccount account) throws EventException{
		...
		try {
			for(int i = 0 ; i < mutiVO.length ; i++){
				...
				if(mutiVO[i].getIbflag().equals("D")){
					
					dbDao.deleteCodTempQty(mutiVO[i]);
					dbDao.deleteCodTemp(mutiVO[i]);
					
				} else if (mutiVO[i].getIbflag().equals("I") || mutiVO[i].getIbflag().equals("U")){
					if (vo.getToEccCdFlg().equals("Y")) {
						...
						if(mutiVO[i].getIbflag().equals("U")) {
							dbDao.deleteCodTempQty(mutiVO[i]);
							dbDao.deleteCodTemp(mutiVO[i]);
						}
				...
				//* Clean Tmp Data
				dbDao.deleteCodTempClean(mutiVO[i]);
				dbDao.deleteCodTempQtyClean(mutiVO[i]);
	...
	/**
	 * [EES_EQR_0012 : Change Repo_rmk Nm 수정]<br>
	 * 
	 * @param conditionVO			EesEqr0012ConditionVO
	 * @param account				SignOnUserAccount
	 * @exception EventException
	...
	public void modifyRepoPlnIdNm(EesEqr0012ConditionVO conditionVO, SignOnUserAccount account) throws EventException{
		...
		try {
			...
			param.put("repo_pln_rmk", conditionVO.getRepormk());
			param.put("upd_usr_id", account.getUsr_id());
			dbDao.updateRepoPlnIdNm(param, velParam);
	...
	public String createManualRepoPlanId( EesEqr0012ConditionVO conditionVO ,SignOnUserAccount account) throws EventException{
		...
		try {
			...
			cnt  = dbDao.duplicateCreateRepoPlanCheck(param, velParam);
			if(cnt < 1){
				param.put("repo_rmk", conditionVO.getRepormk());
				velParam.put("repo_rmk", conditionVO.getRepormk());
			  //  dbDao.createManualScnrId(param, velParam);
				dbDao.createManualRepoPlanId(param, velParam);
				dupYN = "N";
			}
	...
	public String deleteRepoPlanId( EesEqr0012ConditionVO conditionVO ,SignOnUserAccount account) throws EventException{
		...
		try {
			...
			cnt  = dbDao.duplicateCreateRepoPlanCheck(param, velParam);
			plancnt  = dbDao.duplicateCreateRepoPlanViewCheck(param, velParam);
			if(cnt == 1 && plancnt == 0){
				dbDao.deleteEqrPlnCodQty(param, velParam);
				dbDao.deleteEqrPlnCodTmp(param, velParam);
				dbDao.deleteRepoPlanId(param, velParam);
				delYN = "Y";
			}
	...
	public String createManualNewRepoPlanId( EesEqr0012ConditionVO conditionVO ,SignOnUserAccount account) throws EventException{
		String repo_pln_Id = conditionVO.getYyyyww()+"_"+conditionVO.getExectype()+"_"+conditionVO.getFmContiCd()+"_"+conditionVO.getToContiCd()+"_"+conditionVO.getSeq();
		String scnr_id  = Constants.SCNR_WORD+conditionVO.getYyyyww()+Constants.SCNR_WEEK+"00"+conditionVO.getSeq();
	...
	public CommonRsVO searchCODVskLaneLocData(EesEqr0012ConditionVO eesEqr0012ConditionVO) throws EventException {
		try {
			return dbDao.searchCODVskLaneLocData(eesEqr0012ConditionVO);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {

...
public class LeaseReportDBDAOsearchOffHireResultvsDOLDetailRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       , CASE WHEN ( B.LSTM_CD = 'OW' OR B.LSTM_CD = 'LP' OR B.LSTM_CD = 'OL' ) THEN TO_CHAR(D.CNTR_STS_EVNT_DT, 'YYYYMMDD') ELSE TO_CHAR(D.CNTR_STS_EVNT_DT, 'YYYYMMDD') END OHDATE " ).append("\n"); 
		query.append("       , CASE WHEN ( B.LSTM_CD = 'OW' OR B.LSTM_CD = 'LP' OR B.LSTM_CD = 'OL' ) THEN D.YD_CD ELSE D.YD_CD END OHLOC " ).append("\n"); 
		...
		query.append("            TRUNC(NVL(A.CNTR_STS_EVNT_DT, TO_DATE(TO_CHAR(SYSDATE,'RRRR/MM/DD'),'RRRR/MM/DD')) - D.CNTR_STS_EVNT_DT)  + 1 " ).append("\n"); 

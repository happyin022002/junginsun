...
public class ContainerMovementFinderDBDAOSearchMovementListByBkgInfoVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       MC.CMDT_NM AS REP_CMDT_NM," ).append("\n"); 
		query.append("      (SELECT V.REF_ID" ).append("\n"); 
		query.append("        FROM EQR_PLAN_V V" ).append("\n"); 
		query.append("        WHERE V.TP_CD IN ('T.VVD EXE','TRW EXE','ONH EXE','OFH EXE','LCC')" ).append("\n"); 
		query.append("        AND V.MTY_BKG_NO = @[bkg_no]) AS MTY_PLN_NO" ).append("\n"); 

...
public class PayableRentalCostDBDAOPayableChargeCreationAgreementRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      CASE WHEN AA.CHG_SEQ IS NULL THEN 'N'                                                /* No Charge Creation */" ).append("\n"); 
		...
		query.append("                      WHEN AA.INV_NO  IS NULL THEN 'H'                                     /* Only 자사 Charge Creation */" ).append("\n"); 
		...
		query.append("	, AA.LSE_PAY_TP_CD" ).append("\n"); 
		...
		query.append("     , AA.TTL_COST_AMT AS COM_TTL_CHG_AMT" ).append("\n"); 
		...
		query.append("     , ( SELECT SUM(DTL.DSCR_COST_AMT + DTL.CR_AMT)" ).append("\n"); 
		query.append("         FROM LSE_PAY_RNTL_CHG_DTL DTL" ).append("\n"); 
		query.append("        WHERE DTL.LSE_PAY_CHG_TP_CD = 'PDM'" ).append("\n"); 
		query.append("           AND DTL.CHG_SEQ = AA.CHG_SEQ" ).append("\n"); 
		query.append("           AND DTL.CNTR_AUD_STS_CD = 'A') AS PDM_AMT " ).append("\n"); 
		...
		query.append("			, A.LSE_PAY_TP_CD" ).append("\n"); 
		...
		query.append("                      , NVL((SELECT SUB.AGMT_LST_VER_SEQ" ).append("\n"); 
		query.append("                               FROM LSE_AGREEMENT_V SUB" ).append("\n"); 
		query.append("                              WHERE SUB.AGMT_CTY_CD  = B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                AND SUB.AGMT_SEQ         = B.AGMT_SEQ" ).append("\n"); 
		query.append("                                AND SUB.AGMT_LST_VER_SEQ = MST_COMMON_PKG.MST_AGMT_LST_VER_GET_FNC(B.AGMT_CTY_CD, B.AGMT_SEQ, TRUNC(LAST_DAY(TO_DATE(COST_YRMON, 'YYYYMM'))))" ).append("\n"); 
		query.append("                                AND ROWNUM               = 1" ).append("\n"); 
		query.append("                        ), B.AGMT_LST_VER_SEQ) AS AGMT_LST_VER_SEQ" ).append("\n"); 
		query.append("                      , B.LSTM_CD" ).append("\n"); 
		query.append("					, B.LSE_PAY_TP_CD" ).append("\n"); 
		query.append("                      , B.VNDR_SEQ" ).append("\n"); 
		query.append("                      , NVL((SELECT SUB.REF_NO" ).append("\n"); 
		query.append("                               FROM LSE_AGREEMENT_V SUB" ).append("\n"); 
		query.append("                              WHERE SUB.AGMT_CTY_CD  = B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                AND SUB.AGMT_SEQ         = B.AGMT_SEQ" ).append("\n"); 
		query.append("                                AND SUB.AGMT_LST_VER_SEQ = MST_COMMON_PKG.MST_AGMT_LST_VER_GET_FNC(B.AGMT_CTY_CD, B.AGMT_SEQ, TRUNC(LAST_DAY(TO_DATE(COST_YRMON, 'YYYYMM'))))" ).append("\n"); 
		query.append("                                AND ROWNUM               = 1" ).append("\n"); 
		query.append("                        ), B.REF_NO) AS REF_NO" ).append("\n"); 
		query.append("                      , NVL((SELECT SUB.LSE_CTRT_NO" ).append("\n"); 
		query.append("                               FROM LSE_AGREEMENT_V SUB" ).append("\n"); 
		query.append("                              WHERE SUB.AGMT_CTY_CD  = B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                AND SUB.AGMT_SEQ         = B.AGMT_SEQ" ).append("\n"); 
		query.append("                                AND SUB.AGMT_LST_VER_SEQ = MST_COMMON_PKG.MST_AGMT_LST_VER_GET_FNC(B.AGMT_CTY_CD, B.AGMT_SEQ, TRUNC(LAST_DAY(TO_DATE(COST_YRMON, 'YYYYMM'))))" ).append("\n"); 
		query.append("                                AND ROWNUM               = 1" ).append("\n"); 
		query.append("                        ), B.LSE_CTRT_NO) AS LSE_CTRT_NO" ).append("\n"); 
		query.append("                      , NVL((SELECT SUB.LST_EFF_DT" ).append("\n"); 
		query.append("                               FROM LSE_AGREEMENT_V SUB" ).append("\n"); 
		query.append("                              WHERE SUB.AGMT_CTY_CD  = B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                AND SUB.AGMT_SEQ         = B.AGMT_SEQ" ).append("\n"); 
		query.append("                                AND SUB.AGMT_LST_VER_SEQ = MST_COMMON_PKG.MST_AGMT_LST_VER_GET_FNC(B.AGMT_CTY_CD, B.AGMT_SEQ, TRUNC(LAST_DAY(TO_DATE(COST_YRMON, 'YYYYMM'))))" ).append("\n"); 
		...
		query.append("                      , NVL((SELECT SUB.LST_EXP_DT" ).append("\n"); 
		query.append("                               FROM LSE_AGREEMENT_V SUB" ).append("\n"); 
		query.append("                              WHERE SUB.AGMT_CTY_CD  = B.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                AND SUB.AGMT_SEQ         = B.AGMT_SEQ" ).append("\n"); 
		query.append("                                AND SUB.AGMT_LST_VER_SEQ = MST_COMMON_PKG.MST_AGMT_LST_VER_GET_FNC(B.AGMT_CTY_CD, B.AGMT_SEQ, TRUNC(LAST_DAY(TO_DATE(COST_YRMON, 'YYYYMM'))))" ).append("\n"); 
		...
		query.append("     AND B.LSE_PAY_TP_CD = @[lse_pay_tp_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                                 WHERE B.LSE_PAY_TP_CD = 'AD'" ).append("\n"); 
		query.append("                                 UNION ALL" ).append("\n"); 
		query.append("                                 SELECT 'Y'" ).append("\n"); 
		query.append("                                 FROM LSE_PAY_RNTL_CHG SUB" ).append("\n"); 
		query.append("                                 WHERE B.AGMT_CTY_CD = SUB.AGMT_CTY_CD" ).append("\n"); 
		query.append("                                   AND B.AGMT_SEQ    = SUB.AGMT_SEQ" ).append("\n"); 
		query.append("                                   AND P.COST_YRMON  = SUB.CHG_COST_YRMON" ).append("\n"); 
		query.append("                                   AND ROWNUM        = 1)" ).append("\n"); 

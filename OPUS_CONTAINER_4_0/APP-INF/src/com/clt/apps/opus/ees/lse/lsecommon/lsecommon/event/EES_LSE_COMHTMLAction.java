	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if ( command.isCommand(FormCommand.SEARCH03)
		  ...
		  || command.isCommand(FormCommand.SEARCH15) ) {
			...
			event.setIntgCdId(request.getParameter("intg_cd_id")); 
		} else if ( command.isCommand(FormCommand.SEARCH21) ) {
			event.setLocCd(request.getParameter("loc_cd"));
			event.setEqLocTpCd(request.getParameter("eq_loc_tp_cd"));
			event.setLocTp(request.getParameter("loc_tp"));
			event.setSheetIdx(request.getParameter("sheet_idx"));			
		} else if ( command.isCommand(FormCommand.SEARCH22) ) {
			event.setSearchInvoiceNoVO((SearchInvoiceNoVO)getVO(request, SearchInvoiceNoVO.class));
		} else if ( command.isCommand(FormCommand.SEARCH23) ) {
			event.setSchDate(request.getParameter("sch_date"));
			event.setAgmtSeq(request.getParameter("sch_agmt_no")); 
		}

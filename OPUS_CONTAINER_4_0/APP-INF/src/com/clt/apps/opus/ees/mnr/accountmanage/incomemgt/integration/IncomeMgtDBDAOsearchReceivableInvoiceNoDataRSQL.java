...
public class IncomeMgtDBDAOsearchReceivableInvoiceNoDataRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("SUBSTR(@[user_ofc_cd], 1, 3)||TO_CHAR(sysdate, 'yyyymm')||LTRIM(TO_CHAR(NVL(SUBSTR(MAX(MRIW.INV_NO), 10), '0')+1, '000')) INV_NO" ).append("\n"); 
		query.append("FROM MNR_RCV_INV_WRK MRIW" ).append("\n"); 
		query.append("WHERE MRIW.INV_NO LIKE SUBSTR(@[user_ofc_cd], 1, 3)||TO_CHAR(sysdate, 'yyyymm')||'%'" ).append("\n"); 

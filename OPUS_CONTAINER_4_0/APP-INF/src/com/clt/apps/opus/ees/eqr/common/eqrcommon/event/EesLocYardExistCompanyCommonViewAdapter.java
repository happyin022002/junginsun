...
*@FileName : EesLocYardExistCommonViewAdapter.java
*@FileTitle : EES_LOCYARDEXISTCOMPANY_COMMON 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_LOCYARDEXISTCOMPANY_COMMON 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesLocYardExistCompanyCommonViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String locyard 						    = "";
		try{
			row    = condiVO.getLocyardexistcompanyRow();
			colname= condiVO.getLocyardexistcompanyColname();
			locyard= condiVO.getLocyardexistcompanyLocyard();
			result = retVO.getLocyardExistCompanyResult();		
			...
			sb.append("	<TD COL=\""+ colname +"\" DATA-TYPE=\"dtData\" >"+JSPUtil.getNull(locyard)+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

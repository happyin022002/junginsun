...
*@FileName : SurplusAreaDBDAOSearchSurplusAreaRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SurplusAreaDBDAOSearchSurplusAreaRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Surplus Area 데이터를 조회 함
	  * EES_EQR_1070
	  * </pre>
	  */
	public SurplusAreaDBDAOSearchSurplusAreaRSQL(){
		setQuery();
		...
		query.append("FileName : SurplusAreaDBDAOSearchSurplusAreaRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("        FROM EQR_CTRL_INVT_SIM_DTL D, EQR_CTRL_INVT_SIM_DTL_QTY Q, MDM_LOCATION A, MDM_EQ_ORZ_CHT B" ).append("\n"); 
		...
		query.append("                --, SUM(NVL(DECODE(A.CODE,'IV',A.QTY,0),0)-(NVL(DECODE(A.CODE,'IS',A.QTY,0),0)+NVL(DECODE(A.CODE,'ID',A.QTY,0),0))) QTY" ).append("\n"); 
		query.append("                , SUM(NVL(DECODE(A.CODE,'IV',A.QTY,0),0)-(NVL(DECODE(A.CODE,'IS',A.QTY,0),0))) QTY" ).append("\n"); 
		...
		query.append("                , A.CNTR_TPSZ_CD" ).append("\n"); 
		query.append("                , A.FCAST_YRWK" ).append("\n"); 
		query.append("                --, (SUM(NVL(DECODE(A.CODE,'IV',A.QTY,0),0)-(NVL(DECODE(A.CODE,'IS',A.QTY,0),0)+NVL(DECODE(A.CODE,'ID',A.QTY,0),0)))) " ).append("\n"); 
		query.append("                , (SUM(NVL(DECODE(A.CODE,'IV',A.QTY,0),0)-(NVL(DECODE(A.CODE,'IS',A.QTY,0),0))))" ).append("\n"); 
		...
		query.append("                FROM MDM_EQ_ORZ_CHT O, MDM_LOCATION L, CIM_LOC_MTCH_BAK_SMRY T, CIM_TP_SZ_DP_SEQ_V S--, EQR_WK_PRD W" ).append("\n"); 
		query.append("                WHERE 1=1" ).append("\n"); 
		...
		query.append("                    AND T.MVMT_CO_CD IN ('N')" ).append("\n"); 
		...
		query.append("                    AND S.CNTR_TPSZ_CD IN ('D2','D4','D5','D7','R2','R5','R9','O2','O4','S2','S4','F2','F4','F5','A2','A4')" ).append("\n"); 
		query.append("                #if (${loc_type_code} == '2') --RCC (by Country)" ).append("\n"); 

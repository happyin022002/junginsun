...
public class CntrRepoExecutionPlanEstablishDBDAOSearchOfficeCodeRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	DISTINCT NVL(DECODE(TRSP_MOD_CD, 'R', DECODE(SUBSTR(FM_YD_CD, 1, 2), 'US', 'CHIBB'), X.EQ_CTRL_OFC_CD), C.OFC_CD) OFFICE_CODE" ).append("\n"); 
		...
		query.append("        (                                                                   	                " ).append("\n"); 
		query.append("        SELECT " ).append("\n"); 
		query.append("            B.CTRL_OFC_CD EQ_CTRL_OFC_CD                                " ).append("\n"); 
		...
		query.append("--          AND      A.CO_CD       = 'O' -- 2016.03.17 CO_CD는 'H'만 존재. NYK는 해당조건 무의미." ).append("\n"); 

...
*@FileName : EES_EQR_1070HTMLAction.java
...
public class EES_EQR_1070HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_CIM_5001HTMLAction 객체를 생성
	 */
	public EES_EQR_1070HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr1070Event event = new EesEqr1070Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {         // 조회
			event.setSurplusAreaConditionVO((SurplusAreaConditionVO)getVO(request, SurplusAreaConditionVO .class));
		}else if(command.isCommand(FormCommand.MULTI01)) {  // 저장 
			event.setSurplusAreaVOS((SurplusAreaVO[])getVOs(request, SurplusAreaVO .class, ""));
		}else if(command.isCommand(FormCommand.SEARCH02)) { // LOC_CD 조회

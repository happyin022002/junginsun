...
*@FileName : EES_EQR_0048HTMLAction.java
...
public class EES_EQR_0048HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_0048HTMLAction 객체를 생성
	 */
	public EES_EQR_0048HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0048Event event = new EesEqr0048Event();
		
		String tpszType = JSPUtil.getParameter(request, "cntrTpszCd_5","");
		
		// Retrieve 버튼 클릭시
		if(command.isCommand(FormCommand.SEARCHLIST)) {
			event.setEesEqr0048ConditionVO((EesEqr0048ConditionVO)getVO(request, EesEqr0048ConditionVO .class));
		}
		
		// Save 버튼 클릭시
		else if (command.isCommand(FormCommand.MULTI)) {
			event.setEesEqr0048ConditionVO((EesEqr0048ConditionVO)getVO(request, EesEqr0048ConditionVO .class));
			event.setEesEqr0048MultiVOS(new EesEqr0048MultiVO().fromRequestGridArrayList(request, tpszType));
		}

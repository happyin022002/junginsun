...
public class ChargeCalculationDBDAOSearchChargeListByStatusRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	--,TO_CHAR(IM.CRE_DT, 'yyyymmdd') ISS_DT" ).append("\n"); 
		query.append("	--,IM.INV_CURR_CD" ).append("\n"); 
		query.append("	--,IM.INV_CHG_AMT" ).append("\n"); 
		...
		query.append("   ,DECODE( DECODE(BK.CTRT_CUST_CNT_CD,'00','',BK.CTRT_CUST_CNT_CD) || TO_CHAR(BK.CTRT_CUST_SEQ, 'FM000000')" ).append("\n"); 
		query.append("			,'000000' , NULL, DECODE(BK.CTRT_CUST_CNT_CD,'00','',BK.CTRT_CUST_CNT_CD) || TO_CHAR(BK.CTRT_CUST_SEQ, 'FM000000')) AS CTRT_CUST_CD" ).append("\n"); 
		query.append("   ,(SELECT SREP_NM FROM MDM_SLS_REP WHERE  SREP_CD =  BK.CTRT_SREP_CD) AS CTRT_SREP_CD" ).append("\n"); 
		query.append("   ,BK.CTRT_OFC_CD" ).append("\n"); 
		query.append("   ,(SELECT SREP_NM FROM MDM_SLS_REP WHERE  SREP_CD =  BK.OB_SREP_CD) AS OB_SREP_CD" ).append("\n"); 
		query.append("   ,BK.OB_SLS_OFC_CD" ).append("\n"); 
		query.append("   ,(" ).append("\n"); 
		query.append("        SELECT SREP_NM" ).append("\n"); 
		query.append("        FROM SAM_CUST_SLS_REP_INFO  SCS, MDM_SLS_REP MS" ).append("\n"); 
		query.append("        WHERE SCS.DELT_FLG='N' AND SREP_PRMRY_FLG = 'Y'" ).append("\n"); 
		query.append("        AND SCS.CUST_CNT_CD = BC.CUST_CNT_CD" ).append("\n"); 
		query.append("        AND SCS.CUST_SEQ = BC.CUST_SEQ" ).append("\n"); 
		query.append("        AND SCS.SREP_CD = MS.SREP_CD" ).append("\n"); 
		query.append("	) AS IB_SREP_CD" ).append("\n"); 
		query.append("   ,(" ).append("\n"); 
		query.append("		SELECT  OFC_CD" ).append("\n"); 
		query.append("	    FROM    MDM_SLS_REP MS" ).append("\n"); 
		query.append("    	WHERE   MS.SREP_CD = (" ).append("\n"); 
		query.append("        				SELECT SREP_CD" ).append("\n"); 
		query.append("				        FROM SAM_CUST_SLS_REP_INFO" ).append("\n"); 
		query.append("				        WHERE DELT_FLG='N' AND SREP_PRMRY_FLG = 'Y'" ).append("\n"); 
		query.append("				        AND CUST_CNT_CD = BC.CUST_CNT_CD" ).append("\n"); 
		query.append("				        AND CUST_SEQ = BC.CUST_SEQ" ).append("\n"); 
		query.append("			        )" ).append("\n"); 

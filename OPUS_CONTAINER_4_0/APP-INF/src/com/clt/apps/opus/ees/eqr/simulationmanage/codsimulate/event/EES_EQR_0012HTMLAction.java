...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.ees.eqr.simulationmanage 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class EES_EQR_0012HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		conditonVO.setTpsztypeall(JSPUtil.getParameter(request, "tpszCurrent".trim(), ""));
		 ...
		 if(command.isCommand(FormCommand.MULTI02) || command.isCommand(FormCommand.MULTI05)) {	
				conditonVO.setRepormk(JSPUtil.getParameter(request, "repo_rmk".trim(), ""));
				
				conditonVO.setExectype(JSPUtil.getParameter(request, "exectype".trim(), ""));
				conditonVO.setFmContiCd(JSPUtil.getParameter(request, "fm_conti_cd".trim(), ""));
				conditonVO.setToContiCd(JSPUtil.getParameter(request, "to_conti_cd".trim(), ""));
				
				event.setEesEqr0012ConditionVO(conditonVO);
		 }
		 
		 if(command.isCommand(FormCommand.MULTI03)) {	
				conditonVO.setRepormk(JSPUtil.getParameter(request, "repo_rmk".trim(), ""));
				event.setEesEqr0012ConditionVO(conditonVO);
		 }
		 
		 if(command.isCommand(FormCommand.MULTI04)) {	
			 conditonVO.setRepoPlnId(JSPUtil.getParameter(request, "repo_id".trim(), ""));
		 }
		 ...
		 if(command.isCommand(FormCommand.SEARCHLIST06))
		 {
			 ...
			 conditonVO.setViewSc(JSPUtil.getParameter(request, "view_sc".trim(), ""));
...
//			 event.setAtToPlnYrWk_2(JSPUtil.getParameter(request, "atToPlnYrWk_2".trim(), ""));
             ...
             conditonVO.setFmEtdDate(JSPUtil.getParameter(request, "fm_etd_date".trim(),""));	 
					 
					log.debug("++++++++++++++++htmlAction" + JSPUtil.getParameter(request, "vsl_loc_cd".trim(),""));
				 
			 }
		 
		 if(command.isCommand(FormCommand.SEARCHLIST07) || command.isCommand(FormCommand.SEARCHLIST08))
		 {
			 //conditonVO.setRepoPlnId(JSPUtil.getParameter(request, "repo_id".trim(), ""));//삭제하면안됨.
			 conditonVO.setVslCd(JSPUtil.getParameter(request , "vsl_cd".trim(),""));
			 conditonVO.setSkdVoyNo(JSPUtil.getParameter(request, "skd_voy_no".trim(), ""));
...
//			 event.setAtToPlnYrWk_2(JSPUtil.getParameter(request, "atToPlnYrWk_2".trim(), ""));
		 ...
		 if(command.isCommand(FormCommand.SEARCHLIST10))
		 {
			 conditonVO.setRepoPlnId(JSPUtil.getParameter(request, "repo_id".trim(), ""));//삭제하면안됨.
		 }
			...
			// html input 값을 넘겨준다. 
			// SEARCH, SEARCHLIST, SEARCHLIST01, SEARCHLIST02 할당값을 넘겨줌 

...
*@FileName : EES_EQR_1007HTMLAction.java
...
* 1.0 최초 생성
=========================================================*/
package com.clt.apps.opus.ees.eqr.cntrfcstsimul.eqcorgchart.event;
 ...
 * - Parsing 한 정보를 Event로 변환, request에 담아 BIZCOMMONSC로 실행<br>
 * - BIZCOMMONSC에서 View(JSP)로 실행결과를 전송하는 EventResponse를 request에 셋팅<br>
 *
 * @author Hyung Choon_Roh
 * @see EesEqr1007Event , EES_EQR_1007EventResponse 참조
 * @since J2EE 1.4
 */
@SuppressWarnings("serial")
public class EES_EQR_1007HTMLAction extends HTMLActionSupport {

	/**
	 * EES_EQR_1007HTMLAction 객체를 생성
	 */
	public EES_EQR_1007HTMLAction() {
	}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	EesEqr1007Event event = new EesEqr1007Event(depth, chkDepth);
    	
    	if(f_cmd.isCommand(FormCommand.SEARCH)) {
    		event.setEQCOrgChartListVO((EQCOrgChartListVO)getVO(request, EQCOrgChartListVO .class));
    	}else if(f_cmd.isCommand(FormCommand.MULTI)) {
    		EQCOrgChartListVO[] vos = (EQCOrgChartListVO[])getVOs(request, EQCOrgChartListVO .class,"");
    		event.setEQCOrgChartListVOs(vos);
    	}

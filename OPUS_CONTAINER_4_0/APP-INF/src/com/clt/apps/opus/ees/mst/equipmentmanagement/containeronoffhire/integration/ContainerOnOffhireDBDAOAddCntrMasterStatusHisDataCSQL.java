...
public class ContainerOnOffhireDBDAOAddCntrMasterStatusHisDataCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("#if (${st_cd} == '0' || ${st_cd} == '2' || ${st_cd} == '4' || ${st_cd} == '6' || ${st_cd} == '11')" ).append("\n"); 
		...
		query.append(", CNTR_OFFH_AUTH_NO" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("#elseif (${st_cd} == '0' || ${st_cd} == '11') " ).append("\n"); 
		query.append(", SUBSTR(@[agmt_no], 1, 3)" ).append("\n"); 
		query.append(", TO_NUMBER(SUBSTR(@[agmt_no], 4, 6))" ).append("\n"); 
		query.append("#elseif (${st_cd} == '2')" ).append("\n"); 
		query.append(", (SELECT /*+ INDEX_DESC(H XAK1MST_CNTR_STS_HIS) */ AGMT_CTY_CD FROM MST_CNTR_STS_HIS H WHERE CNTR_NO =  @[cntr_no] AND CNTR_STS_CD = 'SBO' AND ROWNUM = 1)" ).append("\n"); 
		...
		query.append(", (SELECT /*+ INDEX_DESC(H XAK1MST_CNTR_STS_HIS) */ AGMT_CTY_CD FROM MST_CNTR_STS_HIS H WHERE CNTR_NO =  @[cntr_no] AND CNTR_STS_CD = 'MUO' AND ROWNUM = 1)" ).append("\n"); 
		...
		query.append(", (SELECT /*+ INDEX_DESC(H XAK1MST_CNTR_STS_HIS) */ AGMT_CTY_CD FROM MST_CNTR_STS_HIS H WHERE CNTR_NO =  @[cntr_no] AND CNTR_STS_CD = 'SRO' AND ROWNUM = 1)" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append(", DECODE(@[st_cd],'0', 'LSO', '1', 'SBO', '2', 'SBI', '3', 'MUO', '4', 'MUI','5', 'SRO','6', 'SRI' ,'11','LSO')" ).append("\n"); 
		query.append(", @[ofc_cd]                                                      " ).append("\n"); 
		...
		query.append("				#if (${st_cd} == '0' || ${st_cd} == '11')" ).append("\n"); 
		query.append("					 AND   B.CNTR_STS_CD IN ('LSI','DII')" ).append("\n"); 
		query.append("				#else" ).append("\n"); 
		query.append("					AND   B.CNTR_STS_CD = DECODE(@[st_cd],'0', 'LSI', '2', 'SBO', '4', 'MUO','6', 'SRO' ,'11' ,'LSI')" ).append("\n"); 
		query.append("				#end" ).append("\n"); 

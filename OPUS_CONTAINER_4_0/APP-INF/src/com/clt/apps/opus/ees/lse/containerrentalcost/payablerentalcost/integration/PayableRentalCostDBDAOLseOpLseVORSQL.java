...
public class PayableRentalCostDBDAOLseOpLseVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , D.LGS_COST_FULL_NM AS COST_NM" ).append("\n"); 
		...
		query.append("AND    LPAD(A.AGMT_SEQ, 6, '0') = LPAD(@[agmt_seq], 6, '0') " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("AND    D.LSTM_CD = NVL( (SELECT  LRC.LSTM_CD" ).append("\n"); 
		query.append("                         FROM LSE_RNTL_COST_ACCT_ORD LRC" ).append("\n"); 
		query.append("                            , LSE_AGREEMENT LA" ).append("\n"); 
		query.append("                         WHERE LRC.LSE_RCV_CHG_TP_CD = D.LSE_RCV_CHG_TP_CD " ).append("\n"); 
		query.append("                         AND    LRC.LSTM_CD          =  LA.LSTM_CD" ).append("\n"); 
		query.append("                         AND    LA.AGMT_CTY_CD       = C.AGMT_CTY_CD" ).append("\n"); 
		query.append("                         AND    LA.AGMT_SEQ          = C.AGMT_SEQ" ).append("\n"); 
		query.append("                         AND    ROWNUM               = 1)" ).append("\n"); 
		query.append("                      , (SELECT  LRC.LSTM_CD" ).append("\n"); 
		query.append("                         FROM  LSE_RNTL_COST_ACCT_ORD LRC" ).append("\n"); 
		query.append("                         WHERE LRC.LSE_RCV_CHG_TP_CD = D.LSE_RCV_CHG_TP_CD " ).append("\n"); 

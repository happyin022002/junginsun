...
public class ContainerOnOffhireDBDAOModifyCntrMasterByUpdateDataUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("          AND DECODE((SELECT /*+ INDEX_DESC(SH XAK1MST_CNTR_STS_HIS) */" ).append("\n"); 
		query.append("                              DECODE(SUBSTR(SH.CNTR_STS_CD, -1), 'O', 'Y', 'N') CHK_OUT " ).append("\n"); 
		query.append("                         FROM MST_CNTR_STS_HIS SH" ).append("\n"); 
		query.append("                        WHERE SH.CNTR_NO = @[cntr_no]" ).append("\n"); 
		query.append("                          AND SUBSTR(SH.CNTR_STS_CD, 1, 2) IN ( 'SB', 'MU', 'SR')" ).append("\n"); 
		query.append("                          AND ROWNUM = 1), 'Y', B.CNTR_STS_CD, 'ALL')" ).append("\n"); 
		query.append("              IN ('ALL', 'SBO', 'MUO', 'SRO')" ).append("\n"); 

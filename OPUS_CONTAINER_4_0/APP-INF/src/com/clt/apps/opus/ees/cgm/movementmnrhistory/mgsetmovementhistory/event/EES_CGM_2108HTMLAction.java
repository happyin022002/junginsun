...
public class EES_CGM_2108HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_CGM_1108HTMLAction 객체를 생성
	 */
	public EES_CGM_2108HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesCgm2108Event event = new EesCgm2108Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setMGSMvmtINVO((MGSMvmtINVO)getVO(request, MGSMvmtINVO .class));
		} else if(command.isCommand(FormCommand.SEARCHLIST)) {
			event.setMgsMvmtBareMGTVOs((MGSMvmtBareMGTVO[])getVOs(request, MGSMvmtBareMGTVO .class,""));

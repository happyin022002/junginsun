...
public class EesEqr1013ViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> vos, String prefix) {
		...
		String main_page =	"";
		if(conditionVO != null) {
			main_page = JSPUtil.getNull(conditionVO.getMainPage());
		}
...
//				log.debug("\n ---->>  level: "+getNull(rs.getString("lvl"))+"\n");
				...
				// 2. SHAAS, SHAAOG 는 RCC / CNSHA, HKHKG, TWTPE, KRSEL, JPTYO 산하의 모든 PORT 수정 가능
				// 3. RCC : HKHKG 는 LCC 단위로 구분
			    //   --> CNSZP : CNSZP LCC 하단의 LOCATION 포함되는 OFFICE 만 수정 가능
			    //   --> CNXMN : CNXMN LCC 하단의 LOCATION 포함되는 OFFICE 만 수정 가능
			    //   --> HKHKG : HKHKG LCC 하단의 LOCATION 포함되는 OFFICE 만 수정 가능
				...
				if(main_page.equals("true")) { // 1013 오픈을 의미 (Inquiry 아님)
					...
					if(ofc_cd.equals("SINWA")) { // CHM-201428796, SELCTY --> SELCOE 로 변경, 
						colEdit = "\"TRUE\""; // 1. SELCOE 는 모든 PORT 수정 가능 
					}else if( (ofc_cd.equals("SHAAS") || 
							  ofc_cd.equals("SHAAOG")
							 ) 
							 &&
							 (pol_rcc_cd.equals("CNSHA") || 
							  pol_rcc_cd.equals("HKHKG") || 
							  pol_rcc_cd.equals("TWTPE") || 
							  pol_rcc_cd.equals("KRSEL") || 
							  pol_rcc_cd.equals("JPTYO")
							 )		 
						   )  {
						colEdit = "\"TRUE\""; // 2. SHAAS 혹은 SHAAOG 는 RCC / CNSHA, HKHKG, TWTPE, KRSEL, JPTYO 산하의 모든 PORT 수정 가능
					}else if(
							 (ofc_lcc_cd.equals("CNSZP") && pol_lcc_cd.equals("CNSZP")) ||
							 (ofc_lcc_cd.equals("CNXMN") && pol_lcc_cd.equals("CNXMN")) ||
							 (ofc_lcc_cd.equals("HKHKG") && pol_lcc_cd.equals("HKHKG")) 
						   ) {
						colEdit = "\"TRUE\"";   // 위의 3번 설명 참조
					} else if(pol_rcc_cd.equals(ofc_rcc_cd) && !pol_rcc_cd.equals("HKHKG")) {
						colEdit = "\"TRUE\"";  // 위의 모든 경우를 제외한 모든 office user 포함(RCC/HKHKG 는 3번 로직 보완)
					} else   {

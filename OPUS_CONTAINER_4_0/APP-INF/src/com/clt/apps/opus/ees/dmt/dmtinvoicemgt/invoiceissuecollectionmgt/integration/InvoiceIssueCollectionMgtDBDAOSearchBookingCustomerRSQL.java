...
public class InvoiceIssueCollectionMgtDBDAOSearchBookingCustomerRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       , MIN(DECODE(BKG_CUST_TP_CD, 'F', CUST_CNT_CD||LPAD(CUST_SEQ,6,'0'))) AS CUST_CD_F" ).append("\n"); 
		query.append("       , MIN(DECODE(BKG_CUST_TP_CD, 'F', CUST_NM)) AS CUST_NM_F" ).append("\n"); 
		...
		query.append("       , MIN(DECODE(BKG_CUST_TP_CD, 'A', CUST_CNT_CD||LPAD(CUST_SEQ,6,'0'))) AS CUST_CD_A" ).append("\n"); 
		query.append("       , MIN(DECODE(BKG_CUST_TP_CD, 'A', CUST_NM)) AS CUST_NM_A" ).append("\n"); 

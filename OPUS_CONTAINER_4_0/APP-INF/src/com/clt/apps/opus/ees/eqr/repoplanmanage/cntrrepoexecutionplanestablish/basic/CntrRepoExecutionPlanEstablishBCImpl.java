...
public class CntrRepoExecutionPlanEstablishBCImpl extends BasicCommandSupport implements CntrRepoExecutionPlanEstablishBC {
	...
	public CommonRsVO searchTrunkVesselAndFeederCntrRepoPlan(EesEqr0059ConditionVO eesEqr0059ConditionVO, SignOnUserAccount account) throws EventException {
		try {
			return dbDao.searchTrunkVesselAndFeederCntrRepoPlan(eesEqr0059ConditionVO, account);
...
//									else if (mtyBkgVO.getSplitFlg().equals("N") ){
	...
	public CommonRsVO searchTruckAndRailAndBargeCntrRepoPlan(EesEqr0059ConditionVO eesEqr0059ConditionVO , SignOnUserAccount account) throws EventException {
		...
		try {
			...
			commonRsVO = dbDao.searchTruckAndRailAndBargeCntrRepoPlan(eesEqr0059ConditionVO , account);
		...
		try {
			...
			if(vos != null && vos.length > 0){
				for(int i = 0 ; i < vos.length ; i++){
	    			...
	    			if(!"".equals(vo.getVvd()) && "".equals(dataYN) &&("I".equals(vo.getIbflag()) || "U".equals(vo.getIbflag()))) {
		    			
						param.put("vvd", vo.getVvd());
						param.put("fm_yd_cd", vo.getFmYdCd());
						param.put("to_yd_cd", vo.getToYdCd());
						param.put("ref_id", vo.getRefId());
						plancnt  = dbDao.duplicateCheckVvdLocationCheck(param, velParam);
						if(!"".equals(plancnt) && "".equals(dataYN)) {
							dataYN = plancnt;
						}
	    			}
				}
	...
	public void modifyTruckAndRailAndBargeCntrRepoPlan(EesEqr0059ConditionVO conditionVO ,EesEqr0080MultiVO[] vos , SignOnUserAccount account) throws EventException {
	    ...
	    try {
	    	...
	    	if(vos != null && vos.length > 0){
				...
				String[] tpszArrDefault= conditionVO.getTpszDefault().split(",");
	                	...
	                	if(vo.getDiv().equals("Plan")) {  // Plan 
	                        ...
	                        // checking PLAN INSERT target has LINK data
	                			...
	                			eqrInlndTrspPlnVO.setPastRepoPlnFlg("N");
    				       		...
    				       		int k = 0;
	                			for(int m=0; m<tpszArrDefault.length; m++) { // 17 TPSZ in using to PLAN
	                				// Type Size 여부 비교 하여 값이 있다면 화면에서 가져온값으로 셋팅
	                				// 값이 없다면 Default값 0으로 설정
	                				EqrInlndTrspPlnQtyVO eqrInlndTrspPlnQtyVO = new EqrInlndTrspPlnQtyVO();
	    				       		ObjectCloner.build(vo , eqrInlndTrspPlnQtyVO );
	    				       		
	                				if(conditionVO.getTpsztype().indexOf(tpszArrDefault[m]) > -1) { 
	                					volList = vo.getVolList();
			                			vol    = Integer.parseInt((String)volList.get(k)); //4
			                			
		                				tableCost = dbDao.searchUnitCost("P", vo.getFmYdCd(), vo.getToYdCd(), vo.getTrspModCd(), tpszArrDefault[m]).getResultStrArray();
		    				       		...
		    				       		eqrInlndTrspPlnQtyVO.setCntrTpszCd(tpszArrDefault[m]);
    ...
    public CommonRsVO searchOnHireAndOffHireCntrRepoPlan(EesEqr0059ConditionVO eesEqr0059ConditionVO , SignOnUserAccount account) throws EventException {
	    try{
	    	return dbDao.searchOnHireAndOffHireCntrRepoPlan(eesEqr0059ConditionVO,account);
	    } catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {
	...
	public void modifyOnHireAndOffHireCntrRepoPlan(EesEqr0059ConditionVO eesEqr0059ConditionVO ,EesEqr0081MultiVO[] vos , SignOnUserAccount account) throws EventException {
		...
		String[] tpszArrDefault= eesEqr0059ConditionVO.getTpszDefault().split(",");
		String[] tpszArr= tpszType.split(",");
		String user_id	= account.getUsr_id();
				...
				if(vo.getIbflag().equals("I")) { // in case of PLAN
	            	if(vo.getDiv().equals("Plan")) {
	            		if(dbDao.onOffHireInsertCheck(vo.getRepoPlnId(), vo.getPlnYrwk(), vo.getTrspModCd(), vo.getFmYdCd(), vo.getLeaseTerm())) {
	            			...
	            			int j = 0;
                			for(int m=0; m<tpszArrDefault.length; m++) { // 17 TPSZ in using to PLAN
                				if(eesEqr0059ConditionVO.getTpsztype().indexOf(tpszArrDefault[m]) > -1) { 
                					volList = vo.getVolList();
    	            				vol    = Integer.parseInt((String)volList.get(j));
            				       ...
            				       //	eqrOnfHirPlnQtyVO.setCntrQty("0");
            				       	eqrOnfHirPlnQtyVO.setCntrTpszCd(tpszArrDefault[m]);
...
//				}
	...
	public CommonRsVO searchShuttleCntrRepoPlan(EesEqr0059ConditionVO eesEqr0059ConditionVO , SignOnUserAccount account) throws EventException {
		try{
			return dbDao.searchShuttleCntrRepoPlan(eesEqr0059ConditionVO,account);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage());
		} catch (Exception ex) {
	...
	*/	
	public List<SearchInlandRouteVO> searchInlandRouteListBasic(SearchInlandRouteVO searchInlandRouteVO) throws EventException {
		try {
			...
			return dbDao.searchInlandRouteListData(searchInlandRouteVO);
			...
			if(vos != null && vos.length > 0){
				for(int i = 0 ; i < vos.length ; i++){
	    			...
	    			if(!"".equals(vo.getVvd()) && "".equals(dataYN) &&("I".equals(vo.getIbflag()) || "U".equals(vo.getIbflag()))) {
						...
						param.put("to_yd_cd", vo.getToYdCd());
						param.put("ref_id", vo.getRefId());
						plancnt  = dbDao.duplicateCheckVvdLocationNewCheck(param, velParam);
						if(!"".equals(plancnt) && "".equals(dataYN)) {
							dataYN = plancnt;
						}
	    			}
	 ...
	 * @exception EventException
	 */
	public void modifyCntrRepoExePlan(EesEqr0059ConditionVO conditionVO ,EesEqr0080MultiVO[] vos , SignOnUserAccount account) throws EventException {
	    	...
	    	if(vos != null && vos.length > 0){
	    		...
	    		for(int i = 0 ; i < vos.length ; i++){
	            	...
	            	if(vo.getIbflag().equals("I")) {	
            			...
            			String plnSeq = commondbDao.getNextPlnSeq("EQR_REPO_EXE_PLN", vo.getRepoPlnId(), vo.getPlnYrwk()).getResultString(); // creating pln_seq 
            			            			
            			String oldRepoPlnId = Constants.REPO_WORD+conditionVO.getYyyyww()+vo.getTrspModCd()+"00"+conditionVO.getSeq();
            			
                		ref_id_new = dbDao.makeRefIDCntrRepoExePlan("EQR_REPO_EXE_PLN", vo.getCoCd(), vo.getRepoPlnId(), vo.getFmYdCd(), vo.getTrspModCd(), oldRepoPlnId);
                		...
                		eqrInlndTrspExePlnVO.setRepoPlnId(vo.getRepoPlnId());
                		eqrInlndTrspExePlnVO.setPlnYrwk(vo.getPlnYrwk());
                		eqrInlndTrspExePlnVO.setTrspModCd(vo.getTrspModCd());
                		eqrInlndTrspExePlnVO.setFmYdCd(vo.getFmYdCd());
                		eqrInlndTrspExePlnVO.setFmEtdDt(vo.getFmEtdDt());
                		eqrInlndTrspExePlnVO.setToYdCd(vo.getToYdCd());
                		eqrInlndTrspExePlnVO.setToEtaDt(vo.getToEtaDt());
                		eqrInlndTrspExePlnVO.setPastRepoPlnFlg("N");
                		eqrInlndTrspExePlnVO.setExectype(conditionVO.getExectype());
                		
                		eqrInlndTrspExePlnVO.setDpSeq(Integer.toString(i + 1));	 	
                		
                		dbDao.insertCntrRepoExePlan(eqrInlndTrspExePlnVO,user_id);  // inserting EQR_INLND_TRSP_EXE_PLN
                		...
                		for(int m=0; m<tpszArr.length; m++) {
	                		...
	                		dbDao.insertCntrRepoExePlanQty(eqrInlndTrspExePlnQtyVO,user_id);  // inserting EQR_INLND_TRSP_EXE_PLN_QTY 	 
					    ...
					    for(int m=0; m<tpszArr.length; m++) {				    	
							...
							//if(flag.equals("T")) {
		                		...
		                		eqrInlndTrspExePlnQtyVO.setToEccUcAmt(tocost+"");
	    				       	...
	    				       	dbDao.mergeCntrRepoExePlanQty(eqrInlndTrspExePlnQtyVO, user_id, soFlag);
							//}
					    }
                		...
                		dbDao.updateCntrRepoExePlan(eqrInlndTrspExePlnVO,user_id, soFlag);  // EQR_INLND_TRSP_EXE_PLN 수정
            			...
            			EqrInlndTrspExePlnQtyVO eqrInlndTrspExePlnQtyVO = new EqrInlndTrspExePlnQtyVO();
				       	ObjectCloner.build(vo , eqrInlndTrspExePlnQtyVO );
            			dbDao.deleteCntrRepoExePlanQty(eqrInlndTrspExePlnQtyVO);

            			// removing EQR_ECC_INTER_EXE_PLN
            			EqrInlndTrspExePlnVO eqrInlndTrspExePlnVO = new EqrInlndTrspExePlnVO();
                		ObjectCloner.build(vo , eqrInlndTrspExePlnVO );
            			dbDao.deleteCntrRepoExePlan(eqrInlndTrspExePlnVO);
	...
	public CommonRsVO searchRepoExePlanList(EesEqr0059ConditionVO eesEqr0059ConditionVO , SignOnUserAccount account) throws EventException {
		...
		CommonRsVO commonRsVO = null;
		try {			
			commonRsVO = dbDao.searchRepoExePlanList(eesEqr0059ConditionVO , account);			
			return commonRsVO;
		} catch (DAOException de) {
			log.error("err "+de.toString(),de);
			throw new EventException(de.getMessage());
        			...
        			if(vo.getSoIssFlg().equals("1")) {
    	            	...
    	            	for(int m=0; m<tpszArr.length; m++) {
    	        	        ...
    	        	        if( vol > 0) {
    	        	        		...
    	        	        		office_code = dbDao.searchOfficeCode("EQR_REPO_EXE_PLN","EQR_REPO_EXE_PLN_QTY",target_key_list , vo.getRepoPlnId(), vo.getPlnYrwk(), vo.getRefId(),vo.getPlnSeq(), tpszArr[m]).getResultString();
    	        	        	//}
    	        	        }else {
    	        	        	office_code = null;
    	        	        }
    	        	        for(int j=0; j<vol; j++) {   
    	        				...
    	        				dbDao.createRepoExePlanSoData(eqrRepoExeSoIfVO,user_id);
	    		...
	    		for(int i = 0 ; i < vos.length ; i++ ){
	            	...
	            	if(vo.getSoIssFlg().equals("1") && vo.getSoCancelFlag().equals("T")) {    
	            		...
	            		if(exeFlag.equals("Y")) {
	        	        	...
	        	        	EqrInlndTrspExePlnVO eqrInlndTrspExePlnVO = new EqrInlndTrspExePlnVO();
	        	        	ObjectCloner.build(vo , eqrInlndTrspExePlnVO );
	        	        	dbDao.updateRepoExePlanExecute(eqrInlndTrspExePlnVO, user_id);
	 ...
	 * @exception EventException
	 */
	public String deleteRepoExePlanId( EesEqr0059ConditionVO conditionVO ,SignOnUserAccount account) throws EventException{
		String repo_pln_Id = conditionVO.getYyyyww()+"_"+conditionVO.getExectype()+"_"+conditionVO.getFmContiCd()+"_"+
				conditionVO.getToContiCd()+"_"+conditionVO.getSeq();
		...
		try {
			...
			cnt  = dbDao.duplicateCreateRepoExePlanCheck(param, velParam);
			plancnt  = dbDao.duplicateCreateRepoExePlanViewCheck(param, velParam);
			if(cnt == 1 && plancnt == 0){
				dbDao.deleteRepoExePlanQty(param, velParam);
				dbDao.deleteRepoExePlanId(param, velParam);
				delYN = "Y";
			}
	...
	public List<EqrCommonVO> searchRepoExePlanTypeSizeList(EesEqr0059ConditionVO conditionVO ,SignOnUserAccount account) throws EventException {
		try {
			return dbDao.searchRepoExePlanTypeSizeList(conditionVO,account); 
		} catch (DAOException ex) {

...
public class EQAvailabilityFinderDBDAO extends DBDAOSupport {
	...
	public List<AvailListVO> searchAvailList(AvailOptionVO availOptionVO) throws DAOException {
		...
		try{
			if(availOptionVO != null){
				...
				if ( !JSPUtil.getNull(availOptionVO.getRstUsgLbl()).equals("") ) {
					arrLabelValueList = JSPUtil.convertStringToArrayList(JSPUtil.replace(availOptionVO.getRstUsgLbl(),",","|"));
	 ...
	 */
	 @SuppressWarnings("unchecked")
	public List<AvailListVO> searchAvailTpSzList(AvailOptionVO availOptionVO) throws DAOException {
		DBRowSet dbRowset = null;
		List<AvailListVO> list = null;
		//query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		...
		try{
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate)new EQAvailabilityFinderDBDAOsearchAvailTpSzListRSQL(), param, velParam);
			list = (List)RowSetUtil.rowSetToVOs(dbRowset, AvailListVO.class);
		}catch(SQLException se){
			log.error(se.getMessage(),se);
			throw new DAOException(new ErrorHandler(se).getMessage());

...
*@FileName : MgsetMovementHistoryDBDAOAddMGSMovementDataCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class MgsetMovementHistoryDBDAOAddMGSMovementDataCSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public MgsetMovementHistoryDBDAOAddMGSMovementDataCSQL(){
		setQuery();
		...
		query.append("FileName : MgsetMovementHistoryDBDAOAddMGSMovementDataCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(" @[mgst_no]         " ).append("\n"); 
		query.append(",TO_DATE(@[cnmv_evnt_dt] ,'YYYY-MM-DD HH24:MI:SS') " ).append("\n"); 
		query.append(",@[mvmt_sts_cd]         " ).append("\n"); 
		query.append(",(SELECT DECODE(MAX(SYS_SEQ)+1,NULL,1,MAX(SYS_SEQ)+1) FROM CGM_MGST_MVMT_HIS WHERE MGST_NO= @[mgst_no] AND MVMT_DT = TO_DATE(@[cnmv_evnt_dt]   ,'YYYY-MM-DD HH24:MI:SS') )  " ).append("\n"); 
		...
		query.append(",SUBSTR(@[org_yd_cd],1,5)   " ).append("\n"); 
		query.append(", (SELECT SCC_CD FROM MDM_LOCATION AA WHERE LOC_CD = SUBSTR(@[org_yd_cd],1,5) )  " ).append("\n"); 
		...
		query.append("     )   " ).append("\n"); 
		query.append(",@[dest_yd_cd]      " ).append("\n"); 
		query.append(",(SELECT IO_BND_CD FROM MDM_MVMT_STS WHERE MVMT_STS_CD = @[mvmt_sts_cd]  AND ROWNUM =1 )    " ).append("\n"); 
		query.append(",@[vndr_seq]  " ).append("\n"); 
		query.append(",@[mvmt_rsn_cd]  " ).append("\n"); 
		query.append(",@[chss_no]     " ).append("\n"); 
		...
		query.append(",@[cnmv_yr]" ).append("\n"); 
		query.append(",@[cnmv_id_no]" ).append("\n"); 
		query.append(",DECODE(@[cntr_no],NULL,NULL,NVL((SELECT CASE WHEN  A.ACIAC_DIV_CD = 'A' OR  A.CNTR_STS_CD='LST' THEN 'H' ELSE 'O' END" ).append("\n"); 
		query.append("                                   FROM  MST_CONTAINER A" ).append("\n"); 
		query.append("                              WHERE CNTR_NO = @[cntr_no]),'O')) " ).append("\n"); 
		query.append(",@[bkg_no] " ).append("\n"); 
		query.append(",@[cre_usr_id]      " ).append("\n"); 

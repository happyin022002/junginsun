...
public class DMTCalculationDBDAOSearchSCExceptionByGenerationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("							                      AND CUST.CUST_SEQ = @[cust_cd]" ).append("\n"); 
		...
		query.append("       #if (${io_bnd_cd} == 'I') " ).append("\n"); 
		query.append("                                            ,CASE WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_CONTI_CD = NVL(@[por_conti_cd]  , ' ')  OR D.SC_EXPT_FM_CONTI_CD = ' ') ) THEN 1" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_CNT_CD   = NVL(@[por_cnt_cd]    , ' ')  OR D.SC_EXPT_FM_CNT_CD = ' ' ) ) THEN 2" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_RGN_CD   = NVL(@[por_rgn_cd]    , ' ')  OR D.SC_EXPT_FM_RGN_CD = ' ' ) ) THEN 3" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_STE_CD   = NVL(@[por_ste_cd]    , ' ')  OR D.SC_EXPT_FM_STE_CD = ' ' ) ) THEN 4" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_LOC_CD   = NVL(@[por_loc_cd]    , ' ')  OR D.SC_EXPT_FM_LOC_CD = ' ' ) ) THEN 5" ).append("\n"); 
		query.append("                                             END" ).append("\n"); 
		query.append("       #elseif (${io_bnd_cd} == 'O')" ).append("\n"); 
		query.append("                                            ,CASE WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_CONTI_CD = NVL(@[del_conti_cd]  , ' ')  OR D.SC_EXPT_FM_CONTI_CD = ' ') ) THEN 1" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_CNT_CD   = NVL(@[del_cnt_cd]    , ' ')  OR D.SC_EXPT_FM_CNT_CD = ' ' ) ) THEN 2" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_RGN_CD   = NVL(@[del_rgn_cd]    , ' ')  OR D.SC_EXPT_FM_RGN_CD = ' ' ) ) THEN 3" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_STE_CD   = NVL(@[del_ste_cd]    , ' ')  OR D.SC_EXPT_FM_STE_CD = ' ' ) ) THEN 4" ).append("\n"); 
		query.append("                                                  WHEN ( D.FM_TO_PAIR_FLG    =  'Y' AND ( D.SC_EXPT_FM_LOC_CD   = NVL(@[del_loc_cd]    , ' ')  OR D.SC_EXPT_FM_LOC_CD = ' ' ) ) THEN 5" ).append("\n"); 
		query.append("                                             END" ).append("\n"); 
		query.append("       #end" ).append("\n"); 

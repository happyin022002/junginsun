 ...
 * @see TurnTimePerformanceMgtBC DAO class reference
 * @since J2EE 1.4
 */
...
public class TurnTimePerformanceMgtBCImpl extends BasicCommandSupport implements TurnTimePerformanceMgtBC {
	...
	/**
	 * creating TurnTimePerformanceMgtBCImpl Object
	 * creating TurnTimePerformanceMgtDBDAO
	 */
	...
	/**
	 * creating PortTurnTimeListByPort Summary Tab
	 * 
	 * @param tTSearchOptionIngereralVo TTSearchOptionInGereralVO
	 * @return List<TurnTimeByTypeSizeVO>
	 * @exception EventException
	...
	public List<TurnTimeByTypeSizeVO> searchPortTurnTimeListByPortSummary(
		...
		try {
			...
			int x = 0;
			if (x == 0) { //showing container type by screen's TP/SZ choice
			...
			for (int i = 1; i < list1.size(); i++) {
				...
				// in case of Turn Time row and not total row
				...
				if ("".equals(strPol)) { // Total
					...
					if (strDivision.equals(sTime)) {
						for (int m = 0; m < cntr.length; m++) {
							...
							if (cntr[m] > 0) {
								ttime[m] = days[m] / cntr[m]; // calculating Turn Time 
							}
							else {
								ttime[m] = 0.0;
							}
	...
	/**
	 * retrieving TurnTimeByMovementDetail
	 * 
	 * @param tTSearchOptionIngereralVo TTSearchOptionInGereralVO
	 * @return List<TurnTimeByTypeSizeVO>
	 * @exception EventException

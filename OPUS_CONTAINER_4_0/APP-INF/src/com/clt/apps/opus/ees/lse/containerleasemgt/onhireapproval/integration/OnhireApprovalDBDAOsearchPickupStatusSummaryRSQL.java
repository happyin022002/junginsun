...
public class OnhireApprovalDBDAOsearchPickupStatusSummaryRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("FROM    (SELECT   NVL(V1.BB, V2.BB) AS BB," ).append("\n"); 
		...
		query.append("                  CASE V3.DIV_SEQ" ).append("\n"); 
		...
		query.append("                           , (SELECT LA.LSTM_CD FROM LSE_ONH_APRO LA WHERE LA.CNTR_ONH_AUTH_NO = A.CNTR_AUTH_NO AND ROWNUM = 1) GG" ).append("\n"); 
		...
		query.append("                                    WHERE    S.CNTR_ONH_AUTH_NO  = A.CNTR_OFFH_AUTH_NO" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("         GROUP BY A.CNTR_AUTH_NO,A.AGMT_CTY_CD,A.AGMT_SEQ,B.CNTR_TPSZ_CD,B.LSTM_CD, DECODE(A.CNTR_OLD_VAN_FLG , 'Y', 'O' , 'N', 'N')" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if(${lstm_tp_cd} == 'L')" ).append("\n"); 
		query.append("(" ).append("\n"); 
		query.append("SELECT   A.CNTR_OFFH_AUTH_NO   BB" ).append("\n"); 
		query.append("                           , A.AGMT_CTY_CD       CC" ).append("\n"); 
		query.append("                           , A.AGMT_SEQ          DD" ).append("\n"); 
		query.append("                           , B.CNTR_TPSZ_CD      EE " ).append("\n"); 
		query.append("                           , COUNT(A.CNTR_NO)    FF" ).append("\n"); 
		query.append("                           , (SELECT LA.LSTM_CD FROM LSE_ONH_APRO LA WHERE LA.CNTR_ONH_AUTH_NO = A.CNTR_OFFH_AUTH_NO AND ROWNUM = 1) GG" ).append("\n"); 
		query.append("                           , DECODE(A.CNTR_OLD_VAN_FLG , 'Y', 'O' , 'N', 'N') HH" ).append("\n"); 
		query.append("                  FROM     MST_CNTR_STS_HIS A, " ).append("\n"); 
		query.append("                           MST_CONTAINER    B" ).append("\n"); 
		query.append("                  WHERE    A.CNTR_NO           = B.CNTR_NO" ).append("\n"); 
		...
		query.append("                  AND      A.CNTR_OLD_VAN_FLG  = 'Y'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("         GROUP BY A.CNTR_OFFH_AUTH_NO,A.AGMT_CTY_CD,A.AGMT_SEQ,B.CNTR_TPSZ_CD,B.LSTM_CD, DECODE(A.CNTR_OLD_VAN_FLG , 'Y', 'O' , 'N', 'N')" ).append("\n"); 
		query.append(")" ).append("\n"); 
		query.append("#end                " ).append("\n"); 
		query.append(" V1 INNER JOIN" ).append("\n"); 
		...
		query.append("#if (${period_stdt} != '' && ${period_eddt} != '')" ).append("\n"); 
		query.append("	#if(${new_van_tp_cd} == 'N')" ).append("\n"); 
		query.append("                  AND B.CNTR_ONH_AUTH_NO IN ( SELECT DISTINCT SUB.CNTR_AUTH_NO" ).append("\n"); 
		query.append("                                                FROM MST_CONTAINER SUB" ).append("\n"); 
		query.append("                                               WHERE SUB.ONH_DT BETWEEN TO_DATE(@[period_stdt], 'YYYYMMDD') AND TO_DATE(@[period_eddt], 'YYYYMMDD')+ 0.99999" ).append("\n"); 
		query.append("										      UNION ALL" ).append("\n"); 
		query.append("                                              SELECT DISTINCT SUB.CNTR_OFFH_AUTH_NO" ).append("\n"); 
		query.append("                                                FROM MST_CONTAINER SUB" ).append("\n"); 
		query.append("                                               WHERE SUB.ONH_DT BETWEEN TO_DATE(@[period_stdt], 'YYYYMMDD') AND TO_DATE(@[period_eddt], 'YYYYMMDD')+ 0.99999       " ).append("\n"); 
		query.append("                                             ) " ).append("\n"); 
		query.append("   #else" ).append("\n"); 
		...
		query.append("                                               WHERE SUB.CNTR_STS_EVNT_DT BETWEEN TO_DATE(@[period_stdt], 'YYYYMMDD') AND TO_DATE(@[period_eddt], 'YYYYMMDD')+ 0.99999" ).append("\n"); 
		query.append("										      UNION ALL" ).append("\n"); 
		query.append("                                              SELECT DISTINCT SUB.CNTR_OFFH_AUTH_NO" ).append("\n"); 
		query.append("                                                FROM MST_CNTR_STS_HIS SUB" ).append("\n"); 
		query.append("                                               WHERE SUB.CNTR_STS_EVNT_DT BETWEEN TO_DATE(@[period_stdt], 'YYYYMMDD') AND TO_DATE(@[period_eddt], 'YYYYMMDD')+ 0.99999       " ).append("\n"); 
		...
		query.append("        )" ).append("\n"); 
		query.append("GROUP BY GROUPING SETS ((BB, CC, DD, DIV), (DIV))" ).append("\n"); 
		query.append("ORDER BY BB, CC, DD, DIV" ).append("\n"); 

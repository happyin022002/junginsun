...
public class MTYEquipmentForecastDBDAOsearchOpmgFcstInputRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("( " ).append("\n"); 
		query.append("    SELECT    W0, W1, W2, W3, W4, W5, W6, W7" ).append("\n"); 
		...
		query.append("        --UNION ALL SELECT 10 DP_SEQ, 'Balance(FCST)'  TITLE, ''  MTY_BAL_TP_CD  FROM DUAL" ).append("\n"); 
		query.append("        --UNION ALL SELECT 11 DP_SEQ, 'Balance(PFMC)'  TITLE, ''  MTY_BAL_TP_CD  FROM DUAL" ).append("\n"); 
		...
		query.append("        AND   A.BSE_DT         BETWEEN B.W1 AND B.W7  -- -3WEEK~+3WEEK" ).append("\n"); 
		...
		query.append("        AND   A.FCAST_YRWK  BETWEEN B.W1 AND B.W7 -- 화면최소주차                      " ).append("\n"); 
		...
		query.append("                GROUP BY A.FCAST_YRWK   " ).append("\n"); 
		query.append("                " ).append("\n"); 
		query.append("                -- -3W부터 +3W REPO IN SNAPSHOT" ).append("\n"); 
		query.append("                UNION ALL" ).append("\n"); 
		query.append("                SELECT 2 DP_SEQ" ).append("\n"); 
		query.append("                      ,A.WEEK WEEK                  " ).append("\n"); 
		query.append("                      ,SUM(NVL(A.D2_QTY,0)) D2_QTY" ).append("\n"); 
		...
		query.append("                    AND   A.ETB_DT <= (SELECT TO_DATE(WK_END_DT, 'YYYYMMDD') -B.ADD_DYS +0.99999  FROM EQR_WK_PRD WHERE PLN_YR||PLN_WK = D.W7) -- 7번째 week" ).append("\n"); 
		query.append("                    AND   A.YD_CD = C.YD_CD    " ).append("\n"); 
		query.append("                ) A" ).append("\n"); 
		query.append("                " ).append("\n"); 
		...
		query.append("                                                AND   WK_END_DT >= TO_CHAR(SYSDATE, 'YYYYMMDD')" ).append("\n"); 
		query.append("                                  )  -- SYSDATE 현재주차                       " ).append("\n"); 
		query.append("            AND   A.FCAST_YRWK BETWEEN B.W1 AND B.W7 -- 과거주차             " ).append("\n"); 

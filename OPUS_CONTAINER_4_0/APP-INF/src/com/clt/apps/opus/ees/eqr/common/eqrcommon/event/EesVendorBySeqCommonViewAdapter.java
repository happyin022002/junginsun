...
*@FileName : EesVendorBySeqCommonViewAdapter.java
*@FileTitle : EES_VENDORBYSEQ_COMMON 화면 IBSheet Generation Class
*Open Issues :
*Change history :
...
/**
 * EES_VENDORBYSEQ_COMMON 화면 IBSheet XML 생성<br>
 ...
 * @since J2EE 1.6
 */
public class EesVendorBySeqCommonViewAdapter extends ViewAdapter {
	...
	protected String makeDataTag(List<AbstractValueObject> OBVOS, String prefix) {
		...
		String[] vendor 						= null;
		try{
			row    = condiVO.getSeqRow();
			vendor = retVO.getSeqResult();
			sb.append("<DATA>\n");
			sb.append("<TR ROW=\""+row+"\" >");
			sb.append("<TD COL=\"vndr_abbr_nm_cd\" DATA-TYPE=\"dtData\">"+JSPUtil.getNull(vendor[0])+"</TD>");
			sb.append("<TD COL=\"vndr_cnt_cd\"     DATA-TYPE=\"dtData\">"+JSPUtil.getNull(vendor[2])+"</TD>");
			sb.append("<TD COL=\"vndr_abbr_nm\"    DATA-TYPE=\"dtData\">"+JSPUtil.getNull(vendor[0])+"</TD>");
			sb.append("<TD COL=\"vndr_seq\"        DATA-TYPE=\"dtData\">"+JSPUtil.getNull(vendor[1])+"</TD>");
			sb.append("</TR>  ");
			sb.append("</DATA>\n");
		}catch(Exception ex){

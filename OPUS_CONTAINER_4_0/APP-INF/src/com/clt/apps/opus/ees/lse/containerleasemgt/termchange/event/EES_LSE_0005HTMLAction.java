...
*@FileName : EES_LSE_0005HTMLAction.java
...
public class EES_LSE_0005HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_LSE_0005HTMLAction 객체를 생성
	 */
	public EES_LSE_0005HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesLse0005Event event = new EesLse0005Event();
		
		
		if(command.isCommand(FormCommand.SEARCH01)) {//유효성 검증
			event.setSearchParamVO((SearchParamVO)getVO(request, SearchParamVO .class));
		} else if(command.isCommand(FormCommand.SEARCH)) {//조회
			event.setSearchParamVO((SearchParamVO)getVO(request, SearchParamVO .class));
		} else if( command.isCommand(FormCommand.SEARCH02)) {
			event.setSearchParamVOs((SearchParamVO[])getVOs(request, SearchParamVO .class,""));		
			event.setSearchParamVO((SearchParamVO)getVO(request, SearchParamVO .class));
		} else if(command.isCommand(FormCommand.MULTI)) {//멀티	
			event.setTermChangeCreationVOs((TermChangeCreationVO[])getVOs(request, TermChangeCreationVO.class));

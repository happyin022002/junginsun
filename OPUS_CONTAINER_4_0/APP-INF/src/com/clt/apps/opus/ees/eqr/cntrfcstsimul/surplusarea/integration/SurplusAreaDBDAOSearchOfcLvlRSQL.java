...
public class SurplusAreaDBDAOSearchOfcLvlRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("--**/" ).append("\n"); 
		query.append("SELECT CASE WHEN @[ofc_cd] IN ('SELCOE')" ).append("\n"); 
		...
		query.append("                    AND     AR_HD_QTR_OFC_CD NOT IN ('SHAAS') --SHAAS는 제외" ).append("\n"); 
		...
		query.append("					AND     ROWNUM = 1" ).append("\n"); 
		query.append("                 )" ).append("\n"); 
		query.append("                  = 'SHAAS'" ).append("\n"); 
		query.append("                THEN 4 -- 조회못하는 office (SHAAS 하부의 모든 OFFICE)                " ).append("\n"); 
		query.append("            WHEN @[ofc_cd] IN ('ATLSC','PHXSC')" ).append("\n"); 

...
*@FileName : EES_MST_0016HTMLAction.java
...
public class EES_MST_0016HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_MST_0016HTMLAction 객체를 생성
	 */
	public EES_MST_0016HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesMst0016Event event = new EesMst0016Event();
		
		if(command.isCommand(FormCommand.MULTI01)) {
			event.setCntrLotVO((CntrLotVO)getVO(request, CntrLotVO.class));		// 생성
		}
		else if(command.isCommand(FormCommand.MULTI02)) {
			event.setCntrLotVO((CntrLotVO)getVO(request, CntrLotVO.class));		// 수정	
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setCntrLotVO((CntrLotVO)getVO(request, CntrLotVO.class));
		}
		else if(command.isCommand(FormCommand.SEARCH09)) {                    // AGMT_NO와 SPEC NO의 tpsz Compare
			event.setCntrLotVO((CntrLotVO)getVO(request, CntrLotVO.class));

...
public class LeaseReportDBDAOTotalContainerInventorySummaryListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 선사의 관리하는 자가 및 임차장비 현황목록을 조회합니다.
	  * </pre>
	  */
	...
	public void setQuery(){
		...
		query.append("		AGMT_NO, AGMT_CTY_CD, AGMT_SEQ,  REF_NO, " ).append("\n"); 
		...
		query.append("	   (SELECT  A.LSTM_CD, A.AGMT_NO, A.AGMT_CTY_CD, A.AGMT_SEQ, A.LSE_PAY_TP_CD, A.REF_NO, " ).append("\n"); 
		...
		query.append("           	   (SELECT  A.LSTM_CD, A.AGMT_CTY_CD||LPAD(A.AGMT_SEQ, 6, '0') AS AGMT_NO, A.LSE_PAY_TP_CD," ).append("\n"); 
		query.append("			            A.AGMT_CTY_CD, A.AGMT_SEQ, A.REF_NO, A.VNDR_SEQ," ).append("\n"); 
		...
		query.append("								C.LSTM_CD, A.AGMT_CTY_CD, A.AGMT_SEQ, C.LSE_PAY_TP_CD, C.REF_NO, " ).append("\n"); 
		...
		query.append("						AND     C.LSE_PAY_TP_CD = @[lse_pay_tp_cd]" ).append("\n"); 
		query.append("#end						" ).append("\n"); 
		...
		query.append("                    			)" ).append("\n"); 
		query.append("#end " ).append("\n"); 
		query.append("			            GROUP BY C.LSTM_CD, A.AGMT_CTY_CD, A.AGMT_SEQ, C.LSE_PAY_TP_CD, C.REF_NO, A.CNTR_TPSZ_CD, C.VNDR_SEQ) A," ).append("\n"); 
		...
		query.append("            			)" ).append("\n"); 
		query.append("#end       " ).append("\n"); 
		query.append("			    GROUP BY ROLLUP(A.LSTM_CD, A.AGMT_CTY_CD, A.AGMT_SEQ, A.LSE_PAY_TP_CD, A.REF_NO, A.VNDR_SEQ)             " ).append("\n"); 

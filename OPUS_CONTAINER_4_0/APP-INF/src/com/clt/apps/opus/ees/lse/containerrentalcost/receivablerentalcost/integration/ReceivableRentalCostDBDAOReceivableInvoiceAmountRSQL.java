...
public class ReceivableRentalCostDBDAOReceivableInvoiceAmountRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("				P.FM_CURR_CD, P.TO_CURR_CD, MAX(P.FM_CHG_AMT) FM_CHG_AMT, MAX(P.INV_ISU_DT) INV_ISU_DT,  " ).append("\n"); 
		query.append("                NVL(ROUND(MAX(DECODE(A.CURR_CD, P.FM_CURR_CD, A.USD_LOCL_XCH_RT)),2),0) AS FM_CURR_RT," ).append("\n"); 
		query.append("                NVL(ROUND(MAX(DECODE(A.CURR_CD, P.TO_CURR_CD, A.USD_LOCL_XCH_RT)),2),0) AS TO_CURR_RT" ).append("\n"); 
		query.append("        FROM   (SELECT  @[fm_curr_cd]     		  AS FM_CURR_CD," ).append("\n"); 
		query.append("						NVL(@[to_curr_cd]," ).append("\n"); 
		query.append("							(SELECT 'USD' FROM MDM_ORGANIZATION WHERE OFC_CD = @[ofc_cd])" ).append("\n"); 
		...
		query.append("        GROUP BY A.ACCT_XCH_RT_YRMON ,P.FM_CURR_CD, P.TO_CURR_CD" ).append("\n"); 

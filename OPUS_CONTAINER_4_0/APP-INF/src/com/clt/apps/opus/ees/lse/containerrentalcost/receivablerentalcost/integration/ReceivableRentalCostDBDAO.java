...
public class ReceivableRentalCostDBDAO extends DBDAOSupport {
	...
	public List<ReportSearchReceivableVO> searchReceivableRentalReportByChargeTypeData(ReportSearchReceivableVO reportSearchReceivableVO) throws DAOException {
		...
		try{
			if(reportSearchReceivableVO != null){
	            ...
	            param.put("search_tp"     , reportSearchReceivableVO.getSearchTp());
	            ...
	            velParam.put("search_tp"     , reportSearchReceivableVO.getSearchTp());
			}
	 ...
	 * @throws Exception
	 */
	public String searchReceivableRentalChargeSeqbyManualData(ReceivableChargeVO receivableChargeVO) throws DAOException {
		String rcvRntlSeq = "0";
		DBRowSet dbRowset = null;
		//query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		...
		try {
			...
			dbRowset = new SQLExecuter().executeQuery(new ReceivableRentalCostDBDAOsearchReceivableRentalChargeSeqbyManualDataRSQL(), param, velParam);
			if(dbRowset.next()) {
				rcvRntlSeq = dbRowset.getString("RCV_RNTL_SEQ");
	    	}			
		} catch(SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage());

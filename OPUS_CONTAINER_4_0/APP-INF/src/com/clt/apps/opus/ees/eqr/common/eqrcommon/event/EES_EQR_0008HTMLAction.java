...
*@FileName : EES_EQR_0008HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.ees.eqr.eqradjust 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class EES_EQR_0008HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EES_EQR_0008HTMLAction 객체를 생성
	 */
	public EES_EQR_0008HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		EesEqr0008Event event = new EesEqr0008Event();
		...
		if (command.isCommand(FormCommand.SEARCHLIST)) {
			event.setPlnYr(JSPUtil.getParameter(request, "searchYear".trim(),""));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setEqrWkPrdVOs((EqrWkPrdVO[])getVOs(request, EqrWkPrdVO .class,""));
//			event.setEqrWkPrdVOs((EqrWkPrdVO[])getVOs(request, EqrWkPrdVO.class,"sheet1_"));
			log.info(event.getEqrWkPrdVOs().length);
		}

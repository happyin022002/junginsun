...
//				}
	...
	public String[][] checkPREVLVD(CtmCntrMovInfoVO[] ctmCntrMovInfoVOs, SignOnUserAccount account, String oscaBkgFlg) {
		...
		for (int i = 0; i < ctmCntrMovInfoVOs.length; i++) {
			...
			try {
				bkgCntr = dbDao.getPreChkValue(vo.getCntrNo(), vo.getCheckDigit(), oscaBkgFlg);
				if (bkgCntr == null || bkgCntr.size() == 0) {
...
/*		String bkg = subStr(checkBookingVO.getBkgNo(), 0, 4);
					...
					if (subStr(orgYd, 0, 2).equals("US")) {
						if (delCd.equals(orgYd)) {
							...
							try {
								String vcnt = "";
								if(podCd != null){
									vcnt = dbDao.getCountOrgDesYd(delCd, podCd );
								}
								if (!vcnt.equals("0"))
									mvmtCd = "TN";
								else
	...
	private String[] autoCre(CusCtmMovementVO cusCtmMovementVo, CusCtmMovementVO prevCtmMovementVO, MstContainerInfoVO mstContainerInfoVO, BkgBookingInfoVO bkgBookingInfoVO, CheckBookingVO checkBookingVO, CtmCntrMovInfoVO ctmCntrMovInfoVO, String[] autoParam) throws EventException {
		...
		String evntDt    = cusCtmMovementVO.getCnmvEvntDt();
		String mtyPlnNo	 = cusCtmMovementVO.getMtyPlnNo();	// reference no logic 추가 by 2015/06/18 황미연
		...
		try {
			...
			if (mtyPlnNo == null) {
				mtyPlnNo = "";
				cusCtmMovementVO.setMtyPlnNo(mtyPlnNo);
			}
					...
					if ("C".equals(act.getTar())) {
						...
						if (cVO.getTrnkVslCd() != null && !cVO.getTrnkVslCd().equals("") &&
							...
							if (!cVO.getBkgNo().equals(prevCtmMovementVO.getBkgNo()) ) {
								...
								else if(prevCtmMovementVO.getMvmtStsCd().equals("MT") && (ctmCntrMovInfoVO != null && ctmCntrMovInfoVO.getMvmtStsCd() != null) && (ctmCntrMovInfoVO.getMvmtStsCd().equals("OP")) && (autoSts.equals("EN") || autoSts.equals("TN") || autoSts.equals("MT") ) ){
...
//						}
						...
						if ("MT".equals(autoSts)) {
							...
							if(prevCtmMovementVO.getObCntrFlg().equals("N") && !prevCtmMovementVO.getMvmtStsCd().equals("MT")){
						...
						// reference no logic 추가 start by 2015/06/18 황미연
						if (("MT".equals(prevCtmMovementVO.getMvmtStsCd()) || "TN".equals(prevCtmMovementVO.getMvmtStsCd()) || "EN".equals(prevCtmMovementVO.getMvmtStsCd()) || "CM".equals(prevCtmMovementVO.getMvmtStsCd())) && "N".equals(prevCtmMovementVO.getFcntrFlg())) {
							 if (autoSts.equals("MT") || autoSts.equals("EN") || autoSts.equals("TN")) {
	...
	public CrossItemVO manageStsOperation(CusCtmMovementVO[] cusCtmMovementVOs, String user_Id, String user_Nm, String ofc_Cd, int endCount, boolean nBkgNoFlg) throws EventException {
		...
		for (int ord = 0; ord < endCount; ord++) {
			...
			log.info("REFERENCENO IS :" + mtyPlnNo + ":");			// reference no 추가 by 2015/06/15 황미연
			String idNo  = "";
			String idSeq = "";
			...
			if ("MT".equals(mvmtStsCd)) {
				if (!"Y".equals(mstCntrInfo.getCoCreFlg())) {
					...
					if (pstCntrInfo.getMvmtStsCd().equals("ID") || pstCntrInfo.getMvmtStsCd().equals("IC") || pstCntrInfo.getMvmtStsCd().equals("TS")) {
						...
						//reference no logic 추가 start by 2015/06/18 황미연
						if (pstCntrInfo != null && "N".equals(pstCntrInfo.getFcntrFlg()) && ("MT".equals(pstCntrInfo.getMvmtStsCd()) || "EN".equals(pstCntrInfo.getMvmtStsCd()) || "TN".equals(pstCntrInfo.getMvmtStsCd()) || "CM".equals(pstCntrInfo.getMvmtStsCd()))) {
							crrCntrInfo.setMtyPlnNo(crrCntrInfo.getMtyPlnNo());
						} else {
							crrCntrInfo.setMtyPlnNo("");
						}
						// reference no logic 추가 end
			...
			if ("MT".equals(mvmtStsCd)) {
				...
				if ("Y".equals(mstCntrInfo.getCoCreFlg())) {
					...
					crrCntrInfo.setMtyPlnNo("");	// reference no logic 추가 by 2015/06/18 황미연
	 ...
	 * @exception EventException
	 */
	public void manageAutoCreSts(AutoCreStsListVO[] autoCreStsListVOs,SignOnUserAccount account) throws EventException {
		try {

...
=========================================================*/
package com.clt.apps.opus.cps.cni.containercargoclaim;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0001Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0002Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0003Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0008Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0009Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0010Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0033Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0036Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0037Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.claimmain.event.CpsCni0043Event;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.basic.IncidentSurveyBC;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.basic.IncidentSurveyBCImpl;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.event.CpsCni0012Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.event.CpsCni0013Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.event.CpsCni0030Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.event.CpsCni0031Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.incidentsurvey.event.CpsCni0032Event;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.indemnityclaim.basic.IndemnityClaimBC;
import com.clt.apps.opus.cps.cni.containercargoclaim.indemnityclaim.basic.IndemnityClaimBCImpl;
import com.clt.apps.opus.cps.cni.containercargoclaim.indemnityclaim.event.CpsCni0015Event;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.insurancerecovery.basic.InsuranceRecoveryBC;
import com.clt.apps.opus.cps.cni.containercargoclaim.insurancerecovery.basic.InsuranceRecoveryBCImpl;
import com.clt.apps.opus.cps.cni.containercargoclaim.insurancerecovery.event.CpsCni0016Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.insurancerecovery.event.CpsCni0017Event;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.prevention.basic.PreventionBC;
import com.clt.apps.opus.cps.cni.containercargoclaim.prevention.basic.PreventionBCImpl;
import com.clt.apps.opus.cps.cni.containercargoclaim.prevention.event.CpsCni0022Event;
import com.clt.apps.opus.cps.cni.containercargoclaim.prevention.event.CpsCni0023Event;
...
import com.clt.apps.opus.cps.cni.containercargoclaim.settlementclaim.basic.SettlementClaimBC;
import com.clt.apps.opus.cps.cni.containercargoclaim.settlementclaim.basic.SettlementClaimBCImpl;
import com.clt.apps.opus.cps.cni.containercargoclaim.settlementclaim.event.CpsCni0014Event;
	...
	public EventResponse perform(Event e) throws EventException {
		...
		if ("CpsCni0001Event".equalsIgnoreCase(eventName)) {							
			...
			if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchClientDefaultSetupOpen(e);
			// [Save]
			} else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse =  manageAreaOfc(e);
			} 
		}
		...
		// ---------------------------------------------------------------------------
			...
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchInsuranceRecoveryByVvdList(e);
			// [Save]
			} else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageInsuranceRecoveryByVvd(e);
		    // [Print]				
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printInsuranceRecoveryByVvdList(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchCreationOpen(e);
			}
		}
		...
		// ---------------------------------------------------------------------------	
			...
			// [Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchInsuranceRecoveryByCaseInfo(e);
			// [Save]
			} else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageInsuranceRecoveryByCase(e);
			// [Recovery Cancel]
			} else if (cmd.isCommand(FormCommand.MULTI01)) {
				eventResponse = manageInsuranceRecoveryByCaseCancel(e);
			// [Case Close]
			} else if (cmd.isCommand(FormCommand.MULTI02)) {
				eventResponse = manageInsuranceRecoveryByCaseClose(e);
			// [Recovery Open]
			} else if (cmd.isCommand(FormCommand.MULTI03)) {
				eventResponse = manageInsuranceRecoveryByCaseReopen(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchCreationOpen(e);
			}  
		...
		// ---------------------------------------------------------------------------
			...
			// [Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchImpendingTBClaim(e);
			// [default]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchImpendingTBClaimOpen(e);
			} 
		...
		// ---------------------------------------------------------------------------	
			...
			// [Retrieve]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchPreventionList(e);
			// [default]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchPreventionOpen(e);
		...
		// ---------------------------------------------------------------------------	
			...
			// [Retrieve]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchPreventionInfo(e);
			// [Retrieve View]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST02)) {
				eventResponse = searchPreventionInfoView(e);				
			// [default]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchPreventionRegisterOpen(e);
			// [modify]
			} else if (cmd.isCommand(FormCommand.MODIFY)) {
				eventResponse = modifyPrevention(e);
			// [add]
			} else if (cmd.isCommand(FormCommand.ADD)) {
				eventResponse = createPrevention(e);				
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printPrevention(e);
			// [remove]
			} else if (cmd.isCommand(FormCommand.REMOVE)) {
				eventResponse = removePrevention(e);					
			} 
		...
		// ---------------------------------------------------------------------------		
		    ...
		    if (cmd.isCommand(FormCommand.MULTI01)) {
			   eventResponse = modifyClaimReopen(e);
		    } else if (cmd.isCommand(FormCommand.SEARCH)) {
				eventResponse = searchClaimMain(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchClaimMainOpen(e);
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve , Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchFindList(e);
			// [PRINT]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printFind(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchFindOpen(e);
			}
		}
		// ---------------------------------------------------------------------------
		// [CPS_CNI_0012] Survey
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageSurvey(e);
			// [Retrieve , Open]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchSurveyInfo(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchSurveyOpen(e);
			}
		}
		...
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageSalvage(e);
			// [Retrieve , Open]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchSalvageInfo(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchCreationOpen(e);
			}
		...
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageIncident(e);
			// [Retrieve , Open]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchIncidentCreationInfo(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchIncidentOpen(e);
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printIncident(e);
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve , Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchIncidentInquiryList(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchIncidentInquiryOpen(e);
			// [PRINT]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printIncidentInquiry(e);
			} 
		}	
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve , Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchIncidentClaimInquiryList(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchIncidentClaimInquiryOpen(e);
			// [PRINT]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printIncidentClaimInquiry(e);
			}  
		}
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve , Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchPaymentInfo(e);
			}
			// [Save]
			else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = managePayment(e);
			}
		}	
		...
		// ---------------------------------------------------------------------------		
			...
			// [Retrieve , Open]
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchTransferList(e);
			// [Save]
			}else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageTransfer(e);	
			// [Row Search]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST02)) {
				eventResponse = searchTrnsOfcCd(e);
			// [Row Search]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST03)) {
				eventResponse = searchTrnsUsrId(e);
			// [PRINT]
			} else if (cmd.isCommand(FormCommand.PRINT)) {
				eventResponse = printTransfer(e);
			// [Open]
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchTransferOpen(e);
			}
		}
		...
		// ---------------------------------------------------------------------------		
			...
			if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				...
				eventResponse = searchHandlingCostOpen(e);
			}	
		} 	
		...
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageLiableParty(e);
			} else if (cmd.isCommand(FormCommand.MULTI01)) {
				eventResponse = manageLiablePartyCancel(e);	
			// [Retrieve , Open]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchLiablePartyInfo(e);
			} else if (cmd.isCommand(FormCommand.SEARCHLIST02)) {
				eventResponse = searchLiablePartyBookingNoInfo(e);	
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchCreationOpen(e);
			}
		}
		...
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchContractCarriageOpen(e);
			} else if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageContractCarriage(e);
			} else if (cmd.isCommand(FormCommand.MULTI01)) {
				eventResponse = manageContractCarriageBL(e);	
			// [Retrieve , Open]
			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchContractCarriageInfo(e);
			} else if (cmd.isCommand(FormCommand.SEARCHLIST02)) {
				eventResponse = searchClaimContainerDetailList(e);
			} else if (cmd.isCommand(FormCommand.SEARCHLIST03)) {
				eventResponse = searchContractCarriageBookingNoInfo(e);
			} else if (cmd.isCommand(FormCommand.SEARCHLIST04)) {
				eventResponse = searchContractCarriageBLGet(e);		
		...
		// ---------------------------------------------------------------------------		
			...
			// [Save]
			if (cmd.isCommand(FormCommand.MULTI)) {
				eventResponse = manageSettlement(e);

			} else if (cmd.isCommand(FormCommand.SEARCHLIST01)) {
				eventResponse = searchSettlementInfo(e);
			} else if (cmd.isCommand(FormCommand.MULTI01)) {
				eventResponse = manageCancel(e);	
			} else if (cmd.isCommand(FormCommand.MULTI02)) {
				eventResponse = modifyApplication(e);
			} else if (cmd.isCommand(FormCommand.DEFAULT)) {
				eventResponse = searchSettlementOpen(e);
			}
	...
	private EventResponse searchPreventionRegisterOpen(Event e) throws EventException {
		...
		try{
			...
			if(cniAreaOfcVO != null){
				...
				eventResponse.setCustomData("clm_area_cd", "");
			}
	...
	private EventResponse searchClaimMainOpen(Event e) throws EventException {
		...
		try {
			...
			if ("CpsCni0003Event".equalsIgnoreCase(eventName)) {
				...
				List<MiscellaneousVO> miscCodeList8 = command.searchMiscellaneousList("05");
...
//				etcData.put("errMsg",(String) new ErrorHandler("CNI00020",new String[]{"Survey Data"}).getUserMessage());
	...
	private EventResponse manageLiableParty(Event e) throws EventException {
		...
		try {			
			...
			/////////////////////////////////////////////////
			if( (miscCd.equals("F")|| miscCd.equals("L")|| miscCd.equals("P")) && (!lablPtyFmalClmDt.equals(""))  
	...
	private EventResponse manageContractCarriage(Event e) throws EventException {
		...
		try {
			...
			String manageStr = command.manageContractCarriage(cniCgoClmCtrtVO, cniCgoClmBlDtlVOs, cniCgoClmCntrDtlVOs, account);
	...
	private EventResponse manageContractCarriageBL(Event e) throws EventException {
		...
		try {			
			...
			String manageStr = command.manageContractCarriageBL(cgoClmNo, cniCgoClmBlDtlVOs, account);
	...
	private EventResponse manageSettlement(Event e) throws EventException {
		...
		try {			
			...
			if( miscCd.equals("N")||miscCd.equals("F")||miscCd.equals("L")||miscCd.equals("R")||miscCd.equals("J") ) {
				...
				if( cgoClmStlTpCd.equals("CM")||cgoClmStlTpCd.equals("PD") ) {
					...
					if(!clmStlAuthCd.equals("Y")) {
						...
						cniCgoClmVO.setClmStlApplUsrId(account.getUsr_id());
						cniCgoClmVO.setClmStlApplOfcCd(account.getOfc_cd());
						command1.modifyAppliedStatus(cniCgoClmVO);	
						// ---------------------------------------------
						// 상태변경 처리후  Status History에 이력정보 설정
						// ---------------------------------------------				
						HandlerHistoryVO handlerHistoryVO = new HandlerHistoryVO();
	 ...
	 * @exception EventException
	 */
	private EventResponse modifyApplication(Event e) throws EventException {
		...
		try {			
			...
			ApplicationStatusVO applicationStatusVO = event.getApplicationStatusVO();
			
			String usrId = account.getUsr_id();
			applicationStatusVO.setUpdUsrId(usrId);
			applicationStatusVO.setClmStlAuthOfcCd(account.getOfc_cd());
			command.modifyApplication(applicationStatusVO);
			
			CniCgoClmVO cniCgoClmVO = new CniCgoClmVO();
			
			String cgoClmNo = applicationStatusVO.getCgoClmNo();
			String cgoStlAuthCd = applicationStatusVO.getClmStlAuthCd();
			String cgoClmStsCd = "";
			
			if (cgoStlAuthCd.equals("Y")) {
				cgoClmStsCd = "V";
			} else if (cgoStlAuthCd.equals("N")){
				cgoClmStsCd = "J";
			}
	 ...
	 * @exception EventException
	 */
	private EventResponse searchContractCarriageOpen(Event e) throws EventException {
		...
		try {
			...
			//report_by
			List<MiscellaneousVO> miscCodeList41 = command.searchMiscellaneousList("41");
			eventResponse.setRsVoList(miscCodeList41);
			ClaimMainBC command1 = new ClaimMainBCImpl();
			// Office Cd 조회		
			String ofcCd = account.getOfc_cd();
	...
	/**
	 * CNI Creation Open<br>
	 * @category searchCreationOpen
	 * @param Event e
	 * @return EventResponse
	 * @exception EventException
	 */
	private EventResponse searchCreationOpen(Event e) throws EventException {

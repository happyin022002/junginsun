...
import com.clt.apps.opus.cps.gem.gemcommon.gemcommon.integration.GEMCommonDBDAO;
...
import com.clt.apps.opus.cps.gem.gemplanningperformance.gemplanningperformance.integration.GEMPlanningPerformanceEAIDAO;
...
public class GEMPlanningPerformanceBCImpl extends BasicCommandSupport implements GEMPlanningPerformanceBC
{
	...
	public String createSlipClosing2(String inClzYrmon) throws EventException {
		...
		try {
			...
			List<GemSlpIfVO> list = dbDao.searchSlpIf("", "", inClzYrmon);
			if(list.size() == 0) {
				returnStr = "2";
			} else {
				for(int i=0; i<list.size(); i++) {
									...
									if(rsltSlpIfVO != null) {
										...
										if(!"".equals(rsltOfcCd)) {
											...
											if(rsltSlpIfVO2 != null) {
												...
												if(!"".equals(rsltExpnCd)) {
													// 10. 품의서의 표기해줄 해당년월까지의 누적실적을 조회한다 ( 조직 , 비용별 )
													decimal2 = dbDao.searchPerfSlp(glEffDt, rsltOfcCd, rsltExpnCd);
												}
												...
												gemSlpPerfVO.setSlpSeqNo(slpSeqNo);
												gemSlpPerfVO.setRsltYrmon(glEffDt);
												gemSlpPerfVO.setOfcCd(subOfcCd);
												gemSlpPerfVO.setSubOfcCd(subOfcCd);
												gemSlpPerfVO.setGenExpnCd(rsltExpnCd);
												gemSlpPerfVO.setSubGenExpnCd(subExpnCd);
												gemSlpPerfVO.setAcctCd(acctCd);
												...
												gemRsltSmryVO.setGenExpnTrnsAmt("0");
												gemRsltSmryVO.setSlpPerfAmt(decimal3.toString());
												gemRsltSmryVO.setGenExpnOvrRtoRsn("Transferred Data");
												gemRsltSmryVO.setCreUsrId("ERP");
												gemRsltSmryVO.setUpdUsrId("ERP");

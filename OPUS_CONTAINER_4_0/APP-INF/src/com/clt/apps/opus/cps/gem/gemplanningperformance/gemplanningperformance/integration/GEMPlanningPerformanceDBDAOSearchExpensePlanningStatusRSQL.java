...
public class GEMPlanningPerformanceDBDAOSearchExpensePlanningStatusRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                  AND B.OFC_CD IN ( SELECT L_4 FROM GEM_OFC_LEVEL_V WHERE L_3 =@[user_ofc_cd] )" ).append("\n"); 
		query.append("                  -- YYYN BU주관팀" ).append("\n"); 
		query.append("                  #elseif (${auth_flg} == 'YYYN')" ).append("\n"); 
		query.append("                  AND ( B.OFC_CD IN ( SELECT L_4 FROM GEM_OFC_LEVEL_V WHERE L_3 = @[user_ofc_cd] ) OR E.TIC_CD = @[user_ofc_cd] )" ).append("\n"); 
		query.append("                  #elseif (${auth_flg} == 'YNYN')" ).append("\n"); 
		query.append("                  -- YNYN 비용주관팀" ).append("\n"); 
		query.append("                  AND (B.OFC_CD = @[user_ofc_cd] OR E.TIC_CD = @[user_ofc_cd] )" ).append("\n"); 
		query.append("				  AND  B.CRNT_GEN_EXPN_APRO_STEP_CD IN ('RQ','HQ','TC'  ,DECODE(B.CRNT_GEN_EXPN_APSTS_CD,'RJ','CO'))" ).append("\n"); 
		query.append("                  #elseif (${auth_flg} == 'YNYY')" ).append("\n"); 

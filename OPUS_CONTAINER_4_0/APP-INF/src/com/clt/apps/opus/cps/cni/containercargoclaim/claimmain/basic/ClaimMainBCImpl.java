...
//						throw new EventException(new ErrorHandler("CNI00020",new String[]{"Survey Data"}).getMessage()); 
   ...
   public String searchClaimStatus(String cgoClmNo) throws EventException {
	...
	public String manageContractCarriage(CniCgoClmCtrtVO cniCgoClmCtrtVO, CniCgoClmBlDtlVO[] cniCgoClmBlDtlVOs, 
											CniCgoClmCntrDtlVO[] cniCgoClmCntrDtlVOs, SignOnUserAccount account) throws EventException {
		try {
			String manageStr="";
	...
	public void modifyApplication(ApplicationStatusVO applicationStatusVO) throws EventException {
		try {		
			dbDao.modifyApplication(applicationStatusVO);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CNI99999",new String[]{}).getMessage(), ex);
		} catch (Exception ex) {
	...
	public void modifyAppliedStatus(CniCgoClmVO cniCgoClmVO) throws EventException {
		try {		
			dbDao.modifyAppliedStatus(cniCgoClmVO);
		} catch(DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler("CNI99999",new String[]{}).getMessage(), ex);
		} catch (Exception ex) {

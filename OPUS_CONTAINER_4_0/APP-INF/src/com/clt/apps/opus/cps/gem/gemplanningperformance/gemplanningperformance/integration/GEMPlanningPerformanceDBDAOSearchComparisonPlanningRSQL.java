...
public class GEMPlanningPerformanceDBDAOSearchComparisonPlanningRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("		   AND B.OFC_CD IN ( SELECT L_4 FROM GEM_OFC_LEVEL_V WHERE L_3 IN (@[login_office]))" ).append("\n"); 
		query.append("#elseif(${auth_flg} == 'YNYN')" ).append("\n"); 
		query.append("		   AND (B.OFC_CD = @[login_office] OR G.TIC_CD = @[login_office])" ).append("\n"); 
		query.append("#elseif(${auth_flg} == 'YYYN')" ).append("\n"); 
		query.append("		   AND (B.OFC_CD IN ( SELECT L_4 FROM GEM_OFC_LEVEL_V WHERE L_3 = @[login_office] ) OR G.TIC_CD = @[login_office] )" ).append("\n"); 
		query.append("#end" ).append("\n"); 

...
public class GEMPlanningPerformanceDBDAOSearchDetailByYearlyExpenseRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   AND (A.OFC_CD = @[auth_ofc_cd] OR C.TIC_CD = @[auth_ofc_cd])" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("#if(${auth_flg} == 'YYYN')" ).append("\n"); 
		query.append("   AND ( A.OFC_CD IN ( SELECT L_4 FROM GEM_OFC_LEVEL_V WHERE L_3 = @[auth_ofc_cd]) OR C.TIC_CD = @[auth_ofc_cd])" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("" ).append("\n"); 

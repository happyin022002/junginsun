...
public class BookingARCreationDBDAOSearchVVDSaDtRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("        (SELECT DECODE(@[io_bnd_cd], 'O', NVL(MAX(BV.POL_CLPT_IND_SEQ), MAX(CLPT_IND_SEQ)), NVL(MIN(BV.POD_CLPT_IND_SEQ), MIN(CLPT_IND_SEQ))) CLPT_IND_SEQ" ).append("\n"); 
		query.append("        FROM VSK_VSL_PORT_SKD SKD, INV_BKG_IF_MN MN, BKG_VVD BV" ).append("\n"); 
		...
		query.append("		AND BV.BKG_NO = MN.BKG_NO" ).append("\n"); 
		...
		query.append("        AND DECODE(@[io_bnd_cd],'O',BV.POL_CD,BV.POD_CD) = SKD.VPS_PORT_CD) C" ).append("\n"); 

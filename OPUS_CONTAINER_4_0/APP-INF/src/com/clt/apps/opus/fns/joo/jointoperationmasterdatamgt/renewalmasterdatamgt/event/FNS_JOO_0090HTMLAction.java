...
public class FNS_JOO_0090HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.MULTI)) {
			event.setAuthorityCarrierVOS((AuthorityCarrierVO[])getVOs(request, AuthorityCarrierVO .class,""));
		}else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setAuthorityCarrierVO((AuthorityCarrierVO)getVO(request, AuthorityCarrierVO .class));

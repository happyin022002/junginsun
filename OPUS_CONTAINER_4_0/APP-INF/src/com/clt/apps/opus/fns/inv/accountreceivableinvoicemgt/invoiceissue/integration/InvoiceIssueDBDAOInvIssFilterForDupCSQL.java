...
public class InvoiceIssueDBDAOInvIssFilterForDupCSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",INV_ISS_TP_CD,AR_IF_NO,CHG_CD,CURR_CD,AR_OFC_CD,CRE_USR_ID,UPD_USR_ID ) " ).append("\n"); 
		query.append("SELECT T2.WRK_NO" ).append("\n"); 
		...
		query.append("   AND T.BL_INV_CFM_DT BETWEEN REPLACE(@[from_dt],'-','') AND REPLACE(@[to_dt],'-','')" ).append("\n"); 
		query.append("      #else" ).append("\n"); 
		query.append("   AND T.UPD_DT >= TO_DATE(REPLACE(@[from_dt],'-',''), 'YYYYMMDD') AND B.UPD_DT < TO_DATE(REPLACE(@[to_dt],'-',''), 'YYYYMMDD') + 1" ).append("\n"); 
		query.append("   AND T.UPD_USR_ID = @[user_id]" ).append("\n"); 
		query.append("      #end  " ).append("\n"); 
		query.append(" #end                                                           " ).append("\n"); 

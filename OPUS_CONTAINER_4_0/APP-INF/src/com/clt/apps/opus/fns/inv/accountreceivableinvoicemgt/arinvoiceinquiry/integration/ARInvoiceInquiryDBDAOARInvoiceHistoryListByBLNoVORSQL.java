...
public class ARInvoiceInquiryDBDAOARInvoiceHistoryListByBLNoVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("  SAR_GET_FMT_MASK_FNC(CURR_CD,SUM(CHG_AMT)) CHG_AMT," ).append("\n"); 
		query.append("  DP_PRCS_KNT," ).append("\n"); 
		query.append("  INV_XCH_RT," ).append("\n"); 
		query.append("  SAR_GET_FMT_MASK_FNC(LOCL_CURR_CD,SUM(SAR_GET_CUR_AMT_FNC(LOCL_CURR_CD,CHG_AMT*INV_XCH_RT))) LOCAL_TOTAL," ).append("\n"); 
		...
		query.append("      -- DECODE(A.INV_CLR_FLG, 'Y', 'SYS CLEAR', NVL(A.INV_NO, CASE WHEN (F.DMDT_AR_INV_ISS_FLG = 'Y' OR F.N3PTY_BIL_AR_INV_FLG = 'Y' OR F.TML_INV_ISS_FLG = 'Y') THEN A.INV_NO ELSE A.INV_SRC_NO END)) INV_NO," ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("    )" ).append("\n"); 
		query.append("GROUP BY AR_IF_NO_INV_SPLIT_CD, AR_IF_NO, INV_SPLIT_CD, VVD, CUST_CD, REV_TP_CD, REV_SRC_CD, REV_TYPE, CURR_ORDER, CRE_DT, BL_INV_IF_DT, BL_INV_CFM_DT, INV_NO, CURR_CD, DP_PRCS_KNT, INV_XCH_RT, DP_PRCS_KNT_LOCAL, INV_CLR_FLG, LOCL_CURR_CD" ).append("\n"); 
		query.append("ORDER BY DECODE(REV_TP_CD,'M',1,0), AR_IF_NO, DECODE(CURR_CD, 'USD', 1, LOCL_CURR_CD, 2, 3), CURR_CD, CRE_DT" ).append("\n"); 

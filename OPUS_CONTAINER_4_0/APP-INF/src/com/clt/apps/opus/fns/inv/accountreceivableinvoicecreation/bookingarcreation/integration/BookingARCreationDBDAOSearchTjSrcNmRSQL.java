...
public class BookingARCreationDBDAOSearchTjSrcNmRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT DECODE((SELECT TAX_FLG FROM MDM_CHARGE WHERE CHG_CD = @[chg_cd]), 'Y', DECODE((SELECT TAX_CNT_CD FROM MDM_CHARGE WHERE CHG_CD = @[chg_cd]), B.CNT_CD, 'VAT', '', 'VAT', DECODE(@[rev_tp_src_cd], 'MTP', 'NONRE', 'MTH', 'OTHER', 'MRD', '3RD', DECODE(SUBSTR(@[rev_tp_src_cd], 1, 1), 'M', 'MRIAR', 'SALAR'))), " ).append("\n"); 
		query.append("                               DECODE(@[rev_tp_src_cd], 'MTP', 'NONRE', 'MTH', 'OTHER', 'MRD', '3RD', DECODE(SUBSTR(@[rev_tp_src_cd], 1, 1), 'M', 'MRIAR', 'SALAR'))) TJ_SRC_NM" ).append("\n"); 

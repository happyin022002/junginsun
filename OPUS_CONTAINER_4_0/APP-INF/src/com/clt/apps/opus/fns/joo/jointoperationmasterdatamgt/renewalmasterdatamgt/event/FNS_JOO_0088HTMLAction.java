...
public class FNS_JOO_0088HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.MULTI)) {
            ...
            event.setMstConditionVO((MstConditionVO)getVO(request, MstConditionVO.class));
        }else if(command.isCommand(FormCommand.SEARCH03)) { // Carrier Key In Check.
            event.setMstConditionVO((MstConditionVO)getVO(request, MstConditionVO.class));
        }else if(command.isCommand(FormCommand.SEARCH04)) { // Rev Lane Combo Item Search.
            event.setMstConditionVO((MstConditionVO)getVO(request, MstConditionVO.class));
        }else if(command.isCommand(FormCommand.SEARCH05)) { // Vendor Search.
            event.setMstConditionVO((MstConditionVO)getVO(request, MstConditionVO.class));
        }else if(command.isCommand(FormCommand.SEARCH06)) { // Customer Search.
            event.setMstConditionVO((MstConditionVO)getVO(request, MstConditionVO.class));
        }

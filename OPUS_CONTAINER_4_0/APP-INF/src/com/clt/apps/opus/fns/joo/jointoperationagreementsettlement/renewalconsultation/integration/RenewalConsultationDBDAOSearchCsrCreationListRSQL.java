...
*@FileName : RenewalConsultationDBDAOSearchCsrCreationListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class RenewalConsultationDBDAOSearchCsrCreationListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * CSR Creation List 조회.
	  * </pre>
	  */
	public RenewalConsultationDBDAOSearchCsrCreationListRSQL(){
		setQuery();
		...
		query.append("FileName : RenewalConsultationDBDAOSearchCsrCreationListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("     , INV.TOT_AMOUNT" ).append("\n"); 
		query.append("     , INV.ACCT_YRMON" ).append("\n"); 
		query.append("     , TO_CHAR(GLOBALDATE_PKG.TIME_CONV_FNC(GLOBALDATE_PKG.GET_LOCCD_FNC(@[ofc_cd]), TO_DATE(INV.EFF_DT, 'YYYYMMDD'), GLOBALDATE_PKG.GET_LOCCD_FNC(@[ofc_cd])),'YYYYMMDD') AS EFF_DT" ).append("\n"); 
		query.append("     , NULL AS RQST_DT --INV.RQST_DT" ).append("\n"); 
		query.append("     , INV.ISS_DT" ).append("\n"); 
		query.append("     , INV.AR_AP_DIV_CD" ).append("\n"); 
		query.append("     , 'F0' AS EVID_TP_CD" ).append("\n"); 
		query.append("     , INV.INV_NO AS CSR_OFFST_NO" ).append("\n"); 
		query.append("     , ORG.USR_ID AS ISSUER_ID" ).append("\n"); 
		...
		query.append("                 ( SELECT NVL(M.SUBS_CO_CD,'00')" ).append("\n"); 
		...
		query.append("       END AS SLP_ISS_INTER_CO_CD" ).append("\n"); 
		query.append("     , CASE WHEN INV.RE_DIVR_CD = 'R' THEN" ).append("\n"); 
		...
		query.append("             , INV.ACCT_YRMON" ).append("\n"); 
		query.append("             , INV.AR_AP_DIV_CD" ).append("\n"); 
		query.append("             , TO_CHAR(GLOBALDATE_PKG.TIME_LOCAL_FNC(GLOBALDATE_PKG.GET_LOCCD_FNC(@[ofc_cd])),'YYYYMMDD') AS RQST_DT" ).append("\n"); 
		query.append("             , TO_CHAR(GLOBALDATE_PKG.TIME_LOCAL_FNC(GLOBALDATE_PKG.GET_LOCCD_FNC(@[ofc_cd])),'YYYYMMDD') AS ISS_DT" ).append("\n"); 
		query.append("             , NVL((SELECT  TO_CHAR(TO_DATE(MAX(DECODE(CLZ_STS_CD,'O',EFF_YRMON))||'01','YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("                          FROM AP_PERIOD P" ).append("\n"); 
		query.append("                         WHERE CLZ_STS_CD = 'O'" ).append("\n"); 
		query.append("                           AND SYS_DIV_CD = DECODE(P.AR_AP_DIV_CD, 'R', '18', '19') --JOO 19 AP전표, '18' AR전표" ).append("\n"); 
		query.append("                           AND EFF_YRMON <= TO_CHAR(SYSDATE,'YYYYMM') --201409' --sysdate's month" ).append("\n"); 
		query.append("                           AND AR_AP_DIV_CD = INV.AR_AP_DIV_CD --DECODE(INV.RE_DIVR_CD, 'R', 'R', 'P') -- P : AP,  'R' AR인 경우" ).append("\n"); 
		...
		query.append("                    (SELECT TO_CHAR(TO_DATE(MAX(DECODE(CLZ_STS_CD,'O',EFF_YRMON))||'01','YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		query.append("                          FROM AP_PERIOD P" ).append("\n"); 
		query.append("                         WHERE CLZ_STS_CD = 'O'" ).append("\n"); 
		query.append("                           AND SYS_DIV_CD = DECODE(P.AR_AP_DIV_CD, 'R', '18', '19') --JOO 19 AP전표, '18' AR전표" ).append("\n"); 
		query.append("                           AND EFF_YRMON <= TO_CHAR(SYSDATE,'YYYYMM') --201409' --sysdate's month" ).append("\n"); 
		query.append("                           AND AR_AP_DIV_CD = INV.AR_AP_DIV_CD --DECODE(INV.RE_DIVR_CD, 'R', 'R', 'P')  --AP,  'R' AR인 경우" ).append("\n"); 
		...
		query.append("                                           AND Y.OFC_CD = @[ofc_cd]  ))) AS EFF_DT" ).append("\n"); 
		query.append("          FROM (" ).append("\n"); 
		query.append("                SELECT INV.INV_NO" ).append("\n"); 
		query.append("                     , INV.ACCTG_CRR_CD" ).append("\n"); 
		query.append("                     , INV.RE_DIVR_CD" ).append("\n"); 

...
public class ARInvoiceExRateMgtDBDAOSearchExRateVOUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("	inv_xch_rt = decode((select xch_rt_rvs_flg from inv_ar_stup_ofc where ar_ofc_cd=@[ar_ofc_cd]),'Y',round(1/to_number(@[inv_xch_rt]),6),round(@[inv_xch_rt],6))  " ).append("\n"); 
		query.append(",	ivs_xch_rt = decode((select xch_rt_rvs_flg from inv_ar_stup_ofc where ar_ofc_cd=@[ar_ofc_cd]),'Y',round(@[inv_xch_rt],6),round(1/to_number(@[inv_xch_rt]),6))   " ).append("\n"); 

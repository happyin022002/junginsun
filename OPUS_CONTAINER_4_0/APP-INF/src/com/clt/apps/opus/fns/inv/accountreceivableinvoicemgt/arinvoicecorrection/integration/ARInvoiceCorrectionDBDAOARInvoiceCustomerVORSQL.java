...
public class ARInvoiceCorrectionDBDAOARInvoiceCustomerVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("   AND MN.ACT_CUST_SEQ = TO_NUMBER(@[act_cust_seq])" ).append("\n"); 
		query.append("   AND MN.BL_INV_IF_DT = replace(@[bl_inv_if_dt],'-','')" ).append("\n"); 
		...
		query.append("                          						  AND BL_INV_IF_DT = replace(@[bl_inv_if_dt],'-','')" ).append("\n"); 
		query.append("#if (${io_bnd_cd} != '')" ).append("\n"); 

...
public class JointOperationAccrualCreationDBDAOJooEstmActRsltUSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("      BSA_QTY       = TO_NUMBER(@[bsa_qty])" ).append("\n"); 
		query.append("     ,BSA_SLT_PRC   = TO_NUMBER(@[bsa_slt_prc])" ).append("\n"); 
		query.append("     ,ESTM_AMT      = TO_NUMBER(@[estm_amt])" ).append("\n"); 
		query.append("     ,ACT_AMT       = TO_NUMBER(@[act_amt])" ).append("\n"); 
		query.append("     ,ACCL_AMT      = TO_NUMBER(@[accl_amt])" ).append("\n"); 
		query.append("     ,ACCL_AMT_CORR_FLG = @[accl_amt_corr_flg]" ).append("\n"); 
		query.append("     ,ST_DT         = TO_DATE(@[st_dt]  ,'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("     ,END_DT        = TO_DATE(@[end_dt] ,'YYYYMMDDHH24MISS')" ).append("\n"); 
		query.append("     ,SAIL_DYS      = TO_NUMBER(@[sail_dys])" ).append("\n"); 
		query.append("     ,ESTM_YRMON    = REPLACE(@[estm_yrmon],'-','')" ).append("\n"); 
		query.append("     ,ESTM_DYS      = TO_NUMBER(@[estm_dys])" ).append("\n"); 
		query.append("     ,ORG_ESTM_AMT  = TO_NUMBER(@[org_estm_amt])" ).append("\n"); 
		query.append("     ,ORG_ACT_AMT   = TO_NUMBER(@[org_act_amt])" ).append("\n"); 

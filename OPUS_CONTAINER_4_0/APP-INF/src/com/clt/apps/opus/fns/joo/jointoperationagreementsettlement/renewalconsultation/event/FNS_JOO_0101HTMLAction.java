...
*@FileName : FNS_JOO_0101HTMLAction.java
...
public class FNS_JOO_0101HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0101HTMLAction 객체를 생성
	 */
	public FNS_JOO_0101HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0101Event event = new FnsJoo0101Event();
		event.setAttribute("job_key", request.getParameter("job_key")); //Backendjob Key.
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSettlementTargetVOS((SettlementTargetVO[])getVOs(request, SettlementTargetVO.class,""));
		}else if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH01) || command.isCommand(FormCommand.SEARCH02) || command.isCommand(FormCommand.COMMAND01) ) {//BackEndJob
            event.setSettlementTargetVO((SettlementTargetVO)getVO(request, SettlementTargetVO.class));

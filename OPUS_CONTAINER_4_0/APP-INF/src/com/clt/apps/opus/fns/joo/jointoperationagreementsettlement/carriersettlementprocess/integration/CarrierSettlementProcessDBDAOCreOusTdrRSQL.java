...
public class CarrierSettlementProcessDBDAOCreOusTdrRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("             , T.FNL_OWN_BSA_QTY" ).append("\n"); 
		...
		query.append("                    , NVL(F.FNL_OWN_BSA_QTY,0) AS FNL_OWN_BSA_QTY, NVL(F.FNL_BSA_WGT,0) FNL_BSA_WGT" ).append("\n"); 
		...
		query.append("                               FROM (" ).append("\n"); 
		query.append("                                    SELECT V.VSL_CD, V.SKD_VOY_NO, V.SKD_DIR_CD, V.VPS_PORT_CD, V.CLPT_IND_SEQ, V.VPS_ETD_DT, V.TURN_PORT_IND_CD, V.VT_ADD_CALL_FLG" ).append("\n"); 
		...
		query.append("                             NVL(SUM(A.BSA_SLOT),0) + NVL(SUM(A.SWAP_SLOT),0) + NVL(SUM(A.RELEASE_SLOT),0) AS FNL_OWN_BSA_QTY," ).append("\n"); 
		query.append("                             NVL(SUM(A.BSA_WGT),0)  + NVL(SUM(A.SWAP_WGT),0) + NVL(SUM(A.RELEASE_WGT),0)  AS FNL_BSA_WGT" ).append("\n"); 

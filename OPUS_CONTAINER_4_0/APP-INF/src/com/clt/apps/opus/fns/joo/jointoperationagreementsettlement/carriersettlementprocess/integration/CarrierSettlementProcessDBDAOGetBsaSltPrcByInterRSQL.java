...
*@FileName : CarrierSettlementProcessDBDAOGetBsaSltPrcByInterRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class CarrierSettlementProcessDBDAOGetBsaSltPrcByInterRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Inter OUS RDR의 경우 VVD, FROM/To Port 변경시 Slot Price 정보를 조회한다.
	  * </pre>
	  */
	public CarrierSettlementProcessDBDAOGetBsaSltPrcByInterRSQL(){
		setQuery();
		...
		query.append("FileName : CarrierSettlementProcessDBDAOGetBsaSltPrcByInterRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT CASE WHEN SM.BSA_SLT_PRC1 > 0 THEN SM.BSA_SLT_PRC1" ).append("\n"); 
		query.append("            ELSE SM.BSA_SLT_PRC2" ).append("\n"); 
		query.append("       END AS BSA_SLT_PRC" ).append("\n"); 
		query.append("  FROM (SELECT NVL((SELECT MAX(CD.UC_AMT) AS BSA_SLT_PRC" ).append("\n"); 
		...
		query.append("                           AND CD.CRR_CD = @[jo_crr_cd] /*Revenue 일때만 적용한다.*/" ).append("\n"); 
		...
		query.append("                           AND CM.TO_PORT_CD = @[to_port_cd]), 0) AS BSA_SLT_PRC1" ).append("\n"); 

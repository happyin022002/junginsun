...
*@FileName : FNS_JOO_0002HTMLAction.java
...
public class FNS_JOO_0002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0002HTMLAction 객체를 생성
	 */
	public FNS_JOO_0002HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0002Event event = new FnsJoo0002Event();
		
		event.setCarrierVO((CarrierVO)getVO(request, CarrierVO .class));

		if(command.isCommand(FormCommand.MULTI)) {
			CarFinanMtrxVO[] revCarFinanMtrxVOs = (CarFinanMtrxVO[])getVOs(request, CarFinanMtrxVO.class,"sheet1_");
			CarFinanMtrxVO[] expCarFinanMtrxVOs = (CarFinanMtrxVO[])getVOs(request, CarFinanMtrxVO.class,"sheet2_");
			CarrierVO[] carrierVOs = (CarrierVO[])getVOs(request, CarrierVO.class, "sheet3_");

			event.setCarFinanMtrxGrpVO(revCarFinanMtrxVOs, "R");
			event.setCarFinanMtrxGrpVO(expCarFinanMtrxVOs, "E");
			if (carrierVOs != null)
				event.setCarFinanMtrxGrpVO(carrierVOs[0]);
			else
				event.setCarFinanMtrxGrpVO(null);
		}

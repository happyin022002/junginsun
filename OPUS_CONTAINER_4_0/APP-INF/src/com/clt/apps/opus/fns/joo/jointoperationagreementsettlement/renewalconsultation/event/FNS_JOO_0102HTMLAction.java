...
public class FNS_JOO_0102HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_JOO_0079HTMLAction 객체를 생성
	 */
	public FNS_JOO_0102HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		FnsJoo0102Event event = new FnsJoo0102Event();
		
		event.setConsultationConditionVO((ConsultationConditionVO)getVO(request, ConsultationConditionVO.class));
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setInvoiceVOS((InvoiceVO[])getVOs(request, InvoiceVO.class,"sheet1_"));
			event.setInvoiceDetailVOS((InvoiceDetailVO[])getVOs(request, InvoiceDetailVO.class,"sheet2_"));
		}

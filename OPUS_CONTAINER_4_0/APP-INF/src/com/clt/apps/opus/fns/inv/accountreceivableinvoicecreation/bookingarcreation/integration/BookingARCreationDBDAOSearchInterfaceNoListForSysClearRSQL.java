...
public class BookingARCreationDBDAOSearchInterfaceNoListForSysClearRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       (SELECT  AR_IF_NO, AR_OFC_CD, BL_SRC_NO, ACT_CUST_CNT_CD, ACT_CUST_SEQ, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, DECODE(REV_TP_CD,'M','M','B','B','C','B') REV_TP_CD, NVL(BFR_INV_CURR_CD, 'N') BFR_INV_CURR_CD          " ).append("\n"); 
		...
		query.append("       (SELECT  AR_OFC_CD, BL_SRC_NO, ACT_CUST_CNT_CD, ACT_CUST_SEQ, VSL_CD, SKD_VOY_NO, SKD_DIR_CD, REV_TP_CD, BFR_INV_CURR_CD" ).append("\n"); 
		query.append("        FROM   (SELECT  /*+ ORDERED USE_NL(A B) INDEX(B XAK3INV_AR_CHG) */" ).append("\n"); 
		query.append("                        A.AR_OFC_CD, A.BL_SRC_NO, A.ACT_CUST_CNT_CD, A.ACT_CUST_SEQ, A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.REV_TP_CD, A.BFR_INV_CURR_CD, B.CURR_CD," ).append("\n"); 
		...
		query.append("                GROUP BY A.AR_OFC_CD, A.BL_SRC_NO, A.ACT_CUST_CNT_CD, A.ACT_CUST_SEQ, A.VSL_CD, A.SKD_VOY_NO, A.SKD_DIR_CD, A.REV_TP_CD, A.BFR_INV_CURR_CD, B.CURR_CD, B.INV_XCH_RT" ).append("\n"); 
		query.append("				)" ).append("\n"); 
		...
		query.append("AND	 	A.BFR_INV_CURR_CD = B.BFR_INV_CURR_CD" ).append("\n"); 

...
*@FileName : VOP_SCG_0073HTMLAction.java
...
public class VOP_SCG_0073HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_SCG_0073HTMLAction 객체를 생성
	 */
	public VOP_SCG_0073HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopScg0073Event event = new VopScg0073Event();
		

		if(command.isCommand(FormCommand.SEARCHLIST01)) {
		    event.setImdgsclprovino(   JSPUtil.getParameter(request, "imdgsclprovino")  );
		}
        if(command.isCommand(FormCommand.SEARCH01)) {
            event.setImdgunno(       JSPUtil.getParameter(request, "imdg_un_no")  );
            event.setImdgunnoseq(   JSPUtil.getParameter(request, "imdg_un_no_seq")  );            
        }		
        if(command.isCommand(FormCommand.MULTI)) {
            event.setImdgsclprovino(   JSPUtil.getParameter(request, "imdgsclprovino")  );            
            event.setScgImdgSpclProvisVOs( (ScgImdgSpclProvisVO[])getVOs(request, ScgImdgSpclProvisVO .class));
        }

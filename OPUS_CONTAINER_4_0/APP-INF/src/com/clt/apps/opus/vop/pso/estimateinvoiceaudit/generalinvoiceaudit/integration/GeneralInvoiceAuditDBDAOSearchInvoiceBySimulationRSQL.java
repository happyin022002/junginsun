...
public class GeneralInvoiceAuditDBDAOSearchInvoiceBySimulationRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("     , MAX(C.VNDR_LGL_ENG_NM)   AS VNDR_NM" ).append("\n"); 
		query.append("     , NVL(SUM(B.CALC_AMT), 0)  AS CALC_AMT --Tariff Cost" ).append("\n"); 
		query.append("     , NVL(SUM(B.ADJ_AMT), 0)   AS ADJ_AMT --Adjustment Amount" ).append("\n"); 
		query.append("     , NVL(SUM(B.LOCL_AMT), 0)  AS LOCL_AMT --Invoice Amount" ).append("\n"); 
		query.append("     , NVL(SUM(B.USD_AMT), 0)   AS USD_AMT --TL USD Amount" ).append("\n"); 
		query.append("     , MAX(B.XPR_DESC)          AS XPR_DESC" ).append("\n"); 
		query.append("     , MAX(B.FOML_DESC)         AS FOML_DESC" ).append("\n"); 
		query.append("     , MAX(B.DIFF_RMK)          AS RMK" ).append("\n"); 
		...
		query.append("     , D.LGS_COST_CD            AS COST_CD" ).append("\n"); 
		query.append("     , D.LGS_COST_FULL_NM" ).append("\n"); 
		...
		query.append("   AND (B.VSL_CD, B.SKD_VOY_NO, B.SKD_DIR_CD, A.YD_CD, B.CLPT_IND_SEQ) IN (" ).append("\n"); 
		...
		query.append("                                     , @[skd_voy_no]" ).append("\n"); 
		query.append("                                     , @[skd_dir_cd]" ).append("\n"); 
		query.append("                                     , @[port_cd] || @[yard_cd]" ).append("\n"); 
		query.append("                                     , @[clpt_ind_seq]" ).append("\n"); 

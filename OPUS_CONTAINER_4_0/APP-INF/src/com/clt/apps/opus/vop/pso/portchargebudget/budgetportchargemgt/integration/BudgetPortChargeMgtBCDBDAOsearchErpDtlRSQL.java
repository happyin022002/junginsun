...
public class BudgetPortChargeMgtBCDBDAOsearchErpDtlRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT ACT_DT AS ACT_DT" ).append("\n"); 
		...
		query.append("#if (${acct_cds} !='')" ).append("\n"); 
		query.append("           AND G.ACCT_CD IN (" ).append("\n"); 
		query.append("               #foreach($key IN ${acct_cds}) #if($velocityCount < $acct_cds.size()) '$key', #else '$key' #end #end" ).append("\n"); 
		query.append("                )" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("           #end" ).append("\n"); 
		query.append("           #if( ${match_flag} == 'match003' )" ).append("\n"); 
		query.append("           AND G.ESTM_AMT = 0 /*-- Estimate Cost is 0*/" ).append("\n"); 
		query.append("           #end" ).append("\n"); 
		query.append("           #if( ${match_flag} == 'match004' )" ).append("\n"); 
		query.append("           AND G.ACCL_AMT > 0 /*-- Accrual Cost > 0*/" ).append("\n"); 
		query.append("           #end" ).append("\n"); 
		query.append("           #if( ${match_flag} == 'match005' )" ).append("\n"); 
		query.append("           AND G.ACCL_AMT < 0 /*-- Accrual Cost < 0*/" ).append("\n"); 
		query.append("           #end" ).append("\n"); 
		...
		query.append("       )" ).append("\n"); 
		query.append(" WHERE 1=1" ).append("\n"); 
		query.append("#if (${conti_cds} !='')" ).append("\n"); 
		query.append("   AND CONTI_CD IN (" ).append("\n"); 
		query.append("        #foreach($key IN ${conti_cds}) #if($velocityCount < $conti_cds.size()) '$key', #else '$key' #end #end" ).append("\n"); 
		query.append("       )" ).append("\n"); 
		query.append("#end" ).append("\n"); 

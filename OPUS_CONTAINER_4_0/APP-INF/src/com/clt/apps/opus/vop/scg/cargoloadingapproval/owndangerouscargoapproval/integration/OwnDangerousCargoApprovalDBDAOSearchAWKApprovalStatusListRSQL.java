...
*@FileName : OwnDangerousCargoApprovalDBDAOSearchAWKApprovalStatusListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class OwnDangerousCargoApprovalDBDAOSearchAWKApprovalStatusListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Awkvard Cargo의 Application 신청 및 승인에 대한 Status를 보여주는 화면
	  * </pre>
	  */
	public OwnDangerousCargoApprovalDBDAOSearchAWKApprovalStatusListRSQL(){
		setQuery();
		...
		query.append("FileName : OwnDangerousCargoApprovalDBDAOSearchAWKApprovalStatusListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    ROW_NUMBER() OVER (PARTITION BY NVL(C.BKG_NO, A.BKG_NO), NVL(C.SPCL_CGO_APRO_RQST_SEQ, A.SPCL_CGO_APRO_RQST_SEQ), NVL(C.AWK_CGO_SEQ, A.AWK_CGO_SEQ), NVL(C.VSL_PRE_PST_CD, A.VSL_PRE_PST_CD), NVL(C.VSL_SEQ, A.VSL_SEQ)" ).append("\n"); 
		...
		query.append("    	A.BKG_NO," ).append("\n"); 
		query.append("		A.BKG_STS_CD," ).append("\n"); 
		query.append("        D.OVR_VOID_SLT_QTY," ).append("\n"); 
		...
		query.append("		D.AWK_CGO_SEQ," ).append("\n"); 
		...
		query.append("	    D.TTL_DIM_LEN," ).append("\n"); 
		query.append("    	D.TTL_DIM_WDT," ).append("\n"); 
		query.append("	    D.TTL_DIM_HGT," ).append("\n"); 
		...
		query.append("	    D.XTD_OVR_QTY," ).append("\n"); 
		query.append("    	D.GRS_WGT," ).append("\n"); 
		query.append("	    D.NET_WGT," ).append("\n"); 
		query.append("		D.SPCL_CGO_APRO_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	FROM BKG_BOOKING A, SCG_APRO_RQST B, SCG_VVD_APRO_RQST C, SCG_AWK_CGO D" ).append("\n"); 
		query.append("	WHERE A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_CATE_CD = 'AK'" ).append("\n"); 
		...
		query.append("		D.SPCL_CGO_APRO_CD" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	FROM BKG_BOOKING A, SCG_APRO_RQST B, SCG_VVD_APRO_RQST C, BKG_AWK_CGO D" ).append("\n"); 
		query.append("	WHERE A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_CATE_CD = 'AK'" ).append("\n"); 
		query.append("	AND B.BKG_NO = C.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_APRO_RQST_SEQ = C.SPCL_CGO_APRO_RQST_SEQ" ).append("\n"); 
		...
		query.append("    --::2016-04-19::--		" ).append("\n"); 
		query.append("	AND NVL(D.SPCL_CGO_APRO_CD, '*') <> 'D'" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	AND   A.BKG_NO = @[booking_no]" ).append("\n"); 
		query.append("	#if (${vsl_cd} != '' && ${vsl_cd} != 'null')" ).append("\n"); 
		query.append("	AND   C.VSL_CD = @[vsl_cd]" ).append("\n"); 

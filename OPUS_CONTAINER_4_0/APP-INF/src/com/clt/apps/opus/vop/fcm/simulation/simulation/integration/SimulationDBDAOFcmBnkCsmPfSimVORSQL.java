...
*@FileName : SimulationDBDAOFcmBnkCsmPfSimVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SimulationDBDAOFcmBnkCsmPfSimVORSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Bunker Consumption Simulation 화면 P/F SKED Simulation 조회
	  * 2014.09.25 Lee Byoung Hoon [CHM-201431761] PF SKED simulation 기능과 연동으로 Bunker 소모량 산출 기능
	  * </pre>
	  */
	public SimulationDBDAOFcmBnkCsmPfSimVORSQL(){
		setQuery();
		...
		query.append("FileName : SimulationDBDAOFcmBnkCsmPfSimVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    (SELECT VSL_SLAN_NM FROM MDM_VSL_SVC_LANE WHERE VSL_SLAN_CD = T.VSL_SLAN_CD) AS VSL_SLAN_NM ," ).append("\n"); 
		query.append("    (SELECT VSL_SVC_TP_CD FROM MDM_VSL_SVC_LANE WHERE VSL_SLAN_CD = T.VSL_SLAN_CD) AS VSL_SVC_TP_CD ," ).append("\n"); 
		query.append("    (SELECT FDR_DIV_CD FROM MDM_VSL_SVC_LANE WHERE VSL_SLAN_CD = T.VSL_SLAN_CD) AS FDR_DIV_CD ," ).append("\n"); 
		query.append("    SUBSTR(T.YD_CD,6,7) AS TEMP_YD_CD ," ).append("\n"); 
		query.append("    T.ZD ," ).append("\n"); 
		query.append("    T.MAX_SPD	AS TOT_MAX_SPD ," ).append("\n"); 
		query.append("    T.AVG_SPD AS TOT_AVG_SPD ," ).append("\n"); 
		query.append("    ROUND(T.AVG_SEA_BUF_SPD, 1) AS AVG_SEA_BUF_SPD ," ).append("\n"); 
		...
		query.append("    '11' AS PF_SPD_RAT ," ).append("\n"); 
		query.append("    '22' AS BUF_SPD_RAT ," ).append("\n"); 
		query.append("    '33' AS MIN_MAX_SPD ," ).append("\n"); 
		query.append("    T.CHECK_WK_TM ," ).append("\n"); 
		query.append("    T.CRANE_WK_TM ," ).append("\n"); 
		query.append("    NVL((" ).append("\n"); 
		query.append("        SELECT 'Y'" ).append("\n"); 
		query.append("        FROM VSK_VSL_SKD S" ).append("\n"); 
		query.append("        WHERE S.VSL_SLAN_CD  = T.VSL_SLAN_CD" ).append("\n"); 
		query.append("        AND  S.PF_SKD_TP_CD  = T.PF_SVC_TP_CD" ).append("\n"); 
		...
		query.append("	      , T2.N1ST_VSL_CLSS_CD		AS N1ST_VSL_CLSS_CD" ).append("\n"); 
		query.append("	      , T2.N1ST_VSL_CLSS_KNT	AS N1ST_VSL_CLSS_KNT" ).append("\n"); 
		query.append("	      , T2.N2ND_VSL_CLSS_CD		AS N2ND_VSL_CLSS_CD" ).append("\n"); 
		query.append("	      , T2.N2ND_VSL_CLSS_KNT	AS N2ND_VSL_CLSS_KNT" ).append("\n"); 
		query.append("	      , T2.N3RD_VSL_CLSS_CD		AS N3RD_VSL_CLSS_CD" ).append("\n"); 
		query.append("	      , T2.N3RD_VSL_CLSS_KNT	AS N3RD_VSL_CLSS_KNT" ).append("\n"); 
		...
		query.append("	    FROM VSK_SIM_INFO T1," ).append("\n"); 
		query.append("	      VSK_SIM_TML_INFO T2 ," ).append("\n"); 
		query.append("	      (" ).append("\n"); 

...
*@FileName : OwnDangerousCargoApprovalDBDAOPreRestrictionPortVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class OwnDangerousCargoApprovalDBDAOPreRestrictionPortVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public OwnDangerousCargoApprovalDBDAOPreRestrictionPortVORSQL(){
		setQuery();
		...
		query.append("FileName : OwnDangerousCargoApprovalDBDAOPreRestrictionPortVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("		--::2015-09-01:by TOP:Adding Checking Logic for Subsdiary Risk(s)::--" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		UNION ALL" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		SELECT  	XX.SPCL_CNTR_SEQ" ).append("\n"); 
		...
		query.append("    	  AND VR.VSL_OPR_TP_CD = COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append(")," ).append("\n"); 
		...
		query.append("--     , DECODE(TT.NUM,1,DECODE(TT.IMDG_CMPTN_AUTH_DESC,'Prohibition',TT.IMDG_CMPTN_AUTH_DESC,'Permit', IMDG_CMPTN_AUTH_DESC, 'Declare',IMDG_CMPTN_AUTH_DESC, IMDG_CMPTN_AUTH_DESC),TT.IMDG_CMPTN_AUTH_DESC) AS IMDG_CMPTN_AUTH_DESC" ).append("\n"); 
		...
		query.append("     , DECODE(TT.NUM,1,SPCL_CNTR_SEQ,TT.SPCL_CNTR_SEQ)               AS SPCL_CNTR_SEQ " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.SPCL_CGO_SEQ,TT.SPCL_CGO_SEQ)                 AS SPCL_CGO_SEQ " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.IMDG_UN_NO,TT.IMDG_UN_NO)                     AS IMDG_UN_NO " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.IMDG_UN_NO_SEQ,TT.IMDG_UN_NO_SEQ)             AS IMDG_UN_NO_SEQ " ).append("\n"); 
		...
		query.append("--    		 , ROW_NUMBER() OVER(PARTITION BY ST.PORT_TYPE, ST.PORT_CD, ST.SPCL_CNTR_SEQ, ST.SPCL_CGO_SEQ ORDER BY ST.ORDER_NUM1, ST.ORDER_NUM2, DECODE(NVL(ST.IMDG_CMPTN_AUTH_DESC,''),'Prohibition',0,'',1,2)) AS NUM" ).append("\n"); 
		query.append("			 , ROW_NUMBER() OVER(PARTITION BY ST.PORT_TYPE,ST.PORT_CD, ST.SPCL_CNTR_SEQ, ST.SPCL_CGO_SEQ ORDER BY ST.ORDER_NUM1, ST.ORDER_NUM2, DECODE(NVL(ST.IMDG_CMPTN_AUTH_DESC,''),'Prohibition',0,'Declare',1,'No Need',1,'Permit',1,'',2)) AS NUM" ).append("\n"); 
		...
		query.append("                                         CASE WHEN ((R.R_TYPE='U' AND R.C_TYPE='Y') and (R.IMDG_UN_NO=C.IMDG_UN_NO AND R.IMDG_UN_NO_SEQ=C.IMDG_UN_NO_SEQ)) THEN" ).append("\n"); 
		...
		query.append("								CASE WHEN ((R.R_TYPE='C' AND R.C_TYPE='Y')) and (R.IMDG_CLSS_CD=C.IMDG_CLSS_CD) THEN  " ).append("\n"); 
		query.append("                                          CASE WHEN RT.PORT_TYPE='L' THEN" ).append("\n"); 
		...
		query.append("							)" ).append("\n"); 
		query.append("					   ) IMDG_CMPTN_AUTH_DESC" ).append("\n"); 
		query.append("    	        	 , DECODE(RT.PORT_TYPE,'S','',DECODE(RT.PORT_TYPE,'T',COM_ConstantMgr_PKG.COM_getCompanyCode_FNC()||' allow UN No. '||R.IMDG_UN_NO||' only for the direct calling from POL to POD')) TXT_DESC" ).append("\n"); 
		...
		query.append("     , DECODE(TT.NUM,1,TT.SPCL_CNTR_SEQ, TT.SPCL_CNTR_SEQ)      " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.SPCL_CGO_SEQ,TT.SPCL_CGO_SEQ)          " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.IMDG_UN_NO,TT.IMDG_UN_NO)               " ).append("\n"); 
		query.append("     , DECODE(TT.NUM,1,TT.IMDG_UN_NO_SEQ,TT.IMDG_UN_NO_SEQ)         " ).append("\n"); 
		query.append("     , TT.ORDER_NUM1" ).append("\n"); 
		query.append("     , TT.ORDER_NUM2" ).append("\n"); 
		query.append("     , TT.VVD_CD" ).append("\n"); 

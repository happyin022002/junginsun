...
public class GeneralInvoiceAuditDBDAOgetDemdetHolidayETARSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("                   AND D.HOL_YR         = TO_CHAR(V.VPS_ETA_DT, 'YYYY')" ).append("\n"); 
		...
		query.append("                SELECT CASE WHEN WKND_TP_CD = 'TF' THEN DECODE(TO_CHAR(V.VPS_ETA_DT,'D'),5, 1, 6, 1, 0 )" ).append("\n"); 
		query.append("                            WHEN WKND_TP_CD = 'FS' THEN DECODE(TO_CHAR(V.VPS_ETA_DT,'D'),6, 1, 7, 1, 0 )" ).append("\n"); 
		query.append("                            WHEN SUBSTR(V.VPS_PORT_CD, 1, 2) = 'KR' THEN DECODE(TO_CHAR(V.VPS_ETA_DT,'D'), 7, 0, 1, 1, 0)" ).append("\n"); 
		query.append("                            ELSE DECODE(TO_CHAR(V.VPS_ETA_DT,'D'), 7, 1, 1, 1, 0)" ).append("\n"); 

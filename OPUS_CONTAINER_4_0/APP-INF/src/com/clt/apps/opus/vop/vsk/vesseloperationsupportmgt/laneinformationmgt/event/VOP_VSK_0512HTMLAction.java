...
public class VOP_VSK_0512HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_VSK_0010HTMLAction 객체를 생성
	 */
	public VOP_VSK_0512HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	VopVsk0512Event event = new VopVsk0512Event();
		if(command.isCommand(FormCommand.MULTI)) {
			VskLanePicVO[][] list = new VskLanePicVO[3][];  
			
			int modifyCnt = 0;
			for(int cnt = 0; cnt < list.length; cnt++){
				log.debug(request.getParameterValues("t2sheet" + (cnt + 1) + "_ibflag"));				
			...
			event.setLaneInfoConditionVO((LaneInfoConditionVO)getVO(request, LaneInfoConditionVO.class));
		}

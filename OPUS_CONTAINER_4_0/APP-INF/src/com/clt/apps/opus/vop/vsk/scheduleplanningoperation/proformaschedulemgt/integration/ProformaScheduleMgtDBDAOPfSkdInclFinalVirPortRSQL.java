...
*@FileName : ProformaScheduleMgtDBDAOPfSkdInclFinalVirPortRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ProformaScheduleMgtDBDAOPfSkdInclFinalVirPortRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * PfSkdInclFinalVirPort
	  * </pre>
	  */
	public ProformaScheduleMgtDBDAOPfSkdInclFinalVirPortRSQL(){
		setQuery();
		...
		query.append("FileName : ProformaScheduleMgtDBDAOPfSkdInclFinalVirPortRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT	#if (${pf_svc_tp_cd} != '') " ).append("\n"); 
		query.append("        ROW_NUMBER() OVER ( ORDER BY DECODE(PF_SVC_TP_CD, " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		#foreach($key IN ${pf_svc_tp_cd}) " ).append("\n"); 
		query.append("			#if($velocityCount < $pf_svc_tp_cd.size())" ).append("\n"); 
		query.append("				'$key', $velocityCount ," ).append("\n"); 
		query.append("			#else" ).append("\n"); 
		query.append("				'$key', $velocityCount )" ).append("\n"); 
		query.append("			#end" ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("			, PORT_ROTN_SEQ ASC ) AS ROW_SEQ" ).append("\n"); 
		query.append("        #else" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	        ROW_NUMBER() OVER (ORDER BY PF_SVC_TP_CD, PORT_ROTN_SEQ ASC) AS ROW_SEQ" ).append("\n"); 
		query.append("		#end" ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("		, 	TT.VSL_SLAN_CD" ).append("\n"); 
		query.append("		, 	TT.PF_SVC_TP_CD" ).append("\n"); 
		query.append(" 		, 	TT.SLAN_STND_FLG" ).append("\n"); 
		query.append("		, 	TT.MRG_FLG" ).append("\n"); 
		query.append("		, 	TT.TOT_BRTH_ITVAL_DYS" ).append("\n"); 
		query.append("		,	TT.TOT_VSL_CNT" ).append("\n"); 
		...
		query.append("        ,'' MRG_FLG" ).append("\n"); 
		query.append("		,(SELECT SUM(BRTH_ITVAL_DYS) FROM VSK_PF_SKD" ).append("\n"); 
		query.append("            WHERE VSL_SLAN_CD = T.VSL_SLAN_CD" ).append("\n"); 
		query.append("            AND NVL('','N') = NVL('','N')" ).append("\n"); 
		query.append("          #if (${pf_svc_tp_cd} != '') " ).append("\n"); 
		query.append("            AND T.PF_SVC_TP_CD IN  (" ).append("\n"); 
		query.append("		  #foreach($key IN ${pf_svc_tp_cd}) " ).append("\n"); 
		query.append("			#if($velocityCount < $pf_svc_tp_cd.size())" ).append("\n"); 
		...
		query.append("			)" ).append("\n"); 
		query.append("          #else" ).append("\n"); 
		query.append("	    	AND PF_SVC_TP_CD = T.PF_SVC_TP_CD " ).append("\n"); 
		query.append("          #end     " ).append("\n"); 
		query.append("            ) AS TOT_BRTH_ITVAL_DYS" ).append("\n"); 
		query.append("        ,(SELECT SUM(N1ST_VSL_CLSS_KNT+N2ND_VSL_CLSS_KNT+N3RD_VSL_CLSS_KNT) FROM VSK_PF_SKD" ).append("\n"); 
		query.append("            WHERE VSL_SLAN_CD = T.VSL_SLAN_CD" ).append("\n"); 
		query.append("            AND NVL('','N') = NVL('','N')" ).append("\n"); 
		query.append("            ) AS TOT_VSL_CNT" ).append("\n"); 
		...
		query.append("		(" ).append("\n"); 
		query.append("			SELECT LOC_CD, MAX( DECODE(SEQ, 01, TM || ' - ' || TM2))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 02), 1, CHR(10)|| MAX( DECODE(SEQ, 02, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 03), 1, CHR(10)|| MAX( DECODE(SEQ, 03, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 04), 1, CHR(10)|| MAX( DECODE(SEQ, 04, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 05), 1, CHR(10)|| MAX( DECODE(SEQ, 05, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 06), 1, CHR(10)|| MAX( DECODE(SEQ, 06, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 07), 1, CHR(10)|| MAX( DECODE(SEQ, 07, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 08), 1, CHR(10)|| MAX( DECODE(SEQ, 08, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 09), 1, CHR(10)|| MAX( DECODE(SEQ, 09, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 10), 1, CHR(10)|| MAX( DECODE(SEQ, 10, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 11), 1, CHR(10)|| MAX( DECODE(SEQ, 11, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 12), 1, CHR(10)|| MAX( DECODE(SEQ, 12, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 13), 1, CHR(10)|| MAX( DECODE(SEQ, 13, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 14), 1, CHR(10)|| MAX( DECODE(SEQ, 14, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 15), 1, CHR(10)|| MAX( DECODE(SEQ, 15, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 16), 1, CHR(10)|| MAX( DECODE(SEQ, 16, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 17), 1, CHR(10)|| MAX( DECODE(SEQ, 17, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 18), 1, CHR(10)|| MAX( DECODE(SEQ, 18, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 19), 1, CHR(10)|| MAX( DECODE(SEQ, 19, TM || ' - ' || TM2)))" ).append("\n"); 
		query.append("			|| DECODE(SIGN(CNT + 1 - 20), 1, CHR(10)|| MAX( DECODE(SEQ, 20, TM || ' - ' || TM2))) AS CRANE_WK_TM" ).append("\n"); 
		...
		query.append("                    , SUBSTR(GNG_WRK_END_HRMNT, 1, 2) || ':' || SUBSTR(GNG_WRK_END_HRMNT, 3, 2) AS TM2" ).append("\n"); 
		...
		query.append("    " ).append("\n"); 
		query.append("    #if (${pf_svc_tp_cd} != '') " ).append("\n"); 
		query.append("        AND T1.PF_SVC_TP_CD IN  (" ).append("\n"); 
		query.append("		#foreach($key IN ${pf_svc_tp_cd}) " ).append("\n"); 
		query.append("			#if($velocityCount < $pf_svc_tp_cd.size())" ).append("\n"); 
		query.append("				'$key'," ).append("\n"); 
		query.append("			#else" ).append("\n"); 

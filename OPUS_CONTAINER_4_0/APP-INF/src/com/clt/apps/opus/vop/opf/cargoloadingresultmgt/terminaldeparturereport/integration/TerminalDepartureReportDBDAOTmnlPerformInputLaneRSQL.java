...
public class TerminalDepartureReportDBDAOTmnlPerformInputLaneRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("SELECT -- RHQ,  -- 삭제" ).append("\n"); 
		...
		query.append("                                THEN  SUM(NVL(C.FL,0))+SUM(NVL(C.MT,0))+SUM(NVL(C.MVS,0))" ).append("\n"); 
		query.append("                                ELSE (SUM(NVL(C.FL,0))+SUM(NVL(C.MT,0))+SUM(NVL(C.MVS,0))) / (SUM(NVL((C.DEP - C.BRTH),0)) * 24)" ).append("\n"); 
		...
		query.append("-- OPUS에서 TDR_HEADER테이블에서  UPDATE_SYS 필드 없음" ).append("\n"); 
		...
		query.append("                                FROM   TDR_SUMMARY S, MDM_VSL_CNTR M, TDR_HEADER H,                                " ).append("\n"); 
		...
		query.append("                                                          CASE WHEN ML.CONTI_CD  IN ('E','F') AND ML.LOC_CD NOT IN('EGAIS','ZADUR')                 THEN 'HAMUR'" ).append("\n"); 
		query.append("                                                               WHEN ML.CONTI_CD  = 'M'                                                              THEN 'NYCNA'" ).append("\n"); 
		query.append("                                                               WHEN ML.CONTI_CD  = 'A' AND ML.SCONTI_CD = 'AF'                                      THEN 'SHAAS'" ).append("\n"); 
		query.append("                                                               WHEN (ML.CONTI_CD = 'A' AND ML.SCONTI_CD <> 'AF') OR ML.LOC_CD IN ('EGAIS','ZADUR')  THEN 'SINWA'" ).append("\n"); 
		query.append("                                                               ELSE ''" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${carr_cd} != '' && ${carr_cd} != 'O')" ).append("\n"); 
		query.append("                                AND    M.CRR_CD   = @[carr_cd]" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${carr_cd} == 'O')" ).append("\n"); 
		query.append("                                AND    M.CRR_CD   <>'NYK'     " ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("                     ROUND(AVG(NVL(T.GROSS_TML,0)),2)                              WORK_PRD_GROSS," ).append("\n"); 
		query.append("                     ROUND(AVG(NVL(T.NET_TML,0)),2)                                WORK_PRD_NET," ).append("\n"); 
		query.append("                     ROUND(AVG(NVL(T.GROSS_GC,0)),2)                               GANG_PRD_GROSS," ).append("\n"); 
		query.append("                     ROUND(AVG(NVL(T.NET_GC,0)),2)                                 GANG_PRD_NET" ).append("\n"); 
		query.append("              FROM   ( SELECT A.VVD," ).append("\n"); 
		query.append("                              A.CAL," ).append("\n"); 
		query.append("                              A.RHQ," ).append("\n"); 
		query.append("                              A.LANE," ).append("\n"); 

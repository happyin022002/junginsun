...
*@FileName : OwnDangerousCargoApprovalDBDAOSearchRFApprovalStatusListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class OwnDangerousCargoApprovalDBDAOSearchRFApprovalStatusListRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 해당 Reefer의 Application 신청 및 승인에 대한 Status를 보여주는 화면
	  * </pre>
	  */
	public OwnDangerousCargoApprovalDBDAOSearchRFApprovalStatusListRSQL(){
		setQuery();
		...
		query.append("FileName : OwnDangerousCargoApprovalDBDAOSearchRFApprovalStatusListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("    'RF' AS SCG_FLG," ).append("\n"); 
		...
		query.append("	--::2016-04-19::--" ).append("\n"); 
		query.append("    ROW_NUMBER() OVER (PARTITION BY NVL(C.BKG_NO, A.BKG_NO), NVL(C.SPCL_CGO_APRO_RQST_SEQ, A.SPCL_CGO_APRO_RQST_SEQ), NVL(C.RC_SEQ, A.RC_SEQ), NVL(C.VSL_PRE_PST_CD, A.VSL_PRE_PST_CD), NVL(C.VSL_SEQ, A.VSL_SEQ)" ).append("\n"); 
		...
		query.append("    	D.VLTG_NO," ).append("\n"); 
		query.append("	    D.CDO_TEMP," ).append("\n"); 
		query.append("	    D.FDO_TEMP," ).append("\n"); 
		query.append("    	D.VENT_RTO," ).append("\n"); 
		query.append("    	D.GRS_WGT," ).append("\n"); 
		query.append("		D.SPCL_CGO_APRO_CD        " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	FROM BKG_BOOKING A, SCG_APRO_RQST B, SCG_VVD_APRO_RQST C, SCG_RF_CGO D" ).append("\n"); 
		query.append("	WHERE A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_CATE_CD = 'RF'" ).append("\n"); 
		...
		query.append("		D.SPCL_CGO_APRO_CD        " ).append("\n"); 
		query.append("" ).append("\n"); 
		query.append("	FROM BKG_BOOKING A, SCG_APRO_RQST B, SCG_VVD_APRO_RQST C, BKG_RF_CGO D" ).append("\n"); 
		query.append("	WHERE A.BKG_NO = B.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_CATE_CD = 'RF'" ).append("\n"); 
		query.append("	AND B.BKG_NO = C.BKG_NO" ).append("\n"); 
		query.append("	AND B.SPCL_CGO_APRO_RQST_SEQ = C.SPCL_CGO_APRO_RQST_SEQ" ).append("\n"); 

...
*@FileName : GeneralInvoiceAuditDBDAOgetNonexistentRevLaneDirRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class GeneralInvoiceAuditDBDAOgetNonexistentRevLaneDirRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Expense에 존재하지 않고 AR에 존재하는 Pendulum 의 Rev lane 조회
	  * </pre>
	  */
	public GeneralInvoiceAuditDBDAOgetNonexistentRevLaneDirRSQL(){
		setQuery();
		...
		query.append("FileName : GeneralInvoiceAuditDBDAOgetNonexistentRevLaneDirRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("         , @[io_bnd]        AS IO_BND" ).append("\n"); 
		...
		query.append("SELECT A.RLANE_CD ||'|'|| A.RLANE_DIR_CD ||'|'|| A.REV_YRMON AS REV_DATA" ).append("\n"); 
		query.append("  FROM (" ).append("\n"); 
		...
		query.append("             , CASE WHEN CHK.NOT_USED_PEND_YN = 'Y' AND A.OB_RTO_CNT = A.TOT_CNT THEN DECODE(RNUM , 1, 100, 0) ELSE A.OB_RTO END AS OB_RTO" ).append("\n"); 
		query.append("             , CASE WHEN CHK.NOT_USED_PEND_YN = 'Y' AND A.OB_RTO_CNT = A.TOT_CNT THEN DECODE(RNUM , 2, 100, 0) ELSE A.IB_RTO END AS IB_RTO" ).append("\n"); 
		...
		query.append("        ) A" ).append("\n"); 
		query.append(" WHERE 1=1  " ).append("\n"); 
		query.append("   AND A.NOT_USED_PEND_YN   = 'Y' /*expense에 등록되지 않고 AR에 2건의 Rev Lane 이 존재 한 Case.*/" ).append("\n"); 
		query.append("   AND A.TURN               = 'N' /*Turning port가 아닌 Case일때만 처리.*/" ).append("\n"); 
		query.append("   #if (${io_bnd} != '' && ${io_bnd} == 'OUT') " ).append("\n"); 
		query.append("   AND A.OB_RTO             >= 100 /*OUT Case.*/" ).append("\n"); 
		query.append("   #elseif (${io_bnd} != '' && ${io_bnd} == 'IN') " ).append("\n"); 
		query.append("   AND A.IB_RTO             >= 100 /*IN Case.*/" ).append("\n"); 
		query.append("   #end   " ).append("\n"); 
		query.append("   AND ROWNUM               = 1 /*In or out 무조건 1건만 리턴하도록 해야 함.*/" ).append("\n"); 

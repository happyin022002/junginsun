...
public class VesselScheduleMgtDBDAOCalCanalBunkerSavingRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("										, T2.VPS_PORT_CD	AS FM_PORT" ).append("\n"); 
		...
		query.append("										, NVL(T4.SPL_FOIL_WGT + T4.SPL_LOW_SULP_FOIL_WGT	, 0	)	AS SUPPLY_BUNKER_QTY	/* 가장 최근에 수급한 BUNKER 량 표시 */" ).append("\n"); 
		query.append("										, T4.VPS_PORT_CD									AS SUPPLY_BUNKER_PORT" ).append("\n"); 
		query.append("										, (" ).append("\n"); 
		...
		query.append("											WHERE	PORT_CD				= T4.VPS_PORT_CD" ).append("\n"); 
		query.append("											AND		FOIL_DOIL_DIV_CD	= 'F'	/* BUNKER만 */" ).append("\n"); 
		query.append("											AND		TO_DATE(TO_CHAR(T4.CRN_WRK_CMNC_DT, 'YYYYMMDD'), 'YYYYMMDD') BETWEEN EVNT_DT - 2 AND  EVNT_DT + 2" ).append("\n"); 
		query.append("											) AS ACTUAL_BUNKER_PRICE" ).append("\n"); 
		query.append("										, ((T2.DEP_FOIL_WGT + T2.DEP_LOW_SULP_FOIL_WGT)  	/* LAST PORT 출항 당시 BUNKER 양 				*/" ).append("\n"); 
		query.append("										- ( SELECT	SUM(FOIL_CSM_WGT)						/* LAST PORT 출항 이후 현재까지 BUNKER 사용량	*/" ).append("\n"); 
		...
		query.append("											AND		S.NXT_PORT_CD	= 'EGSCA'	/* 수에즈 도착 전까지 사용한 BUNKER를 산출한다. */" ).append("\n"); 
		...
		query.append("										, T3.NVGT_DIST       											AS DIST_O" ).append("\n"); 
		query.append("										, T3.CRNT_ACT_SPD    											AS SPD_O" ).append("\n"); 
		query.append("										, ROUND(T3.ENG_ML_DIST / (T3.NVGT_DIST / T3.CRNT_ACT_SPD),1)	AS SPD_P" ).append("\n"); 
		query.append("										, ROUND(T3.FOIL_CSM_WGT)    									AS BUNK_CONS" ).append("\n"); 
		query.append("										, (" ).append("\n"); 
		...
		query.append("										, VSK_DEP_RPT			T2" ).append("\n"); 
		query.append("										, VSK_NOON_RPT			T3" ).append("\n"); 
		query.append("										, VSK_DEP_RPT			T4" ).append("\n"); 
		...
		query.append("																							AND		S.YD_CD				= 'EGSCA10'		/* 수에즈 						*/" ).append("\n"); 
		...
		query.append("											AND     T1.SKD_VOY_NO	= S.SKD_VOY_NO" ).append("\n"); 
		query.append("											AND     T1.SKD_DIR_CD	= S.SKD_DIR_CD" ).append("\n"); 
		query.append("											AND		S.NXT_PORT_CD	= 'EGSCA'" ).append("\n"); 
		query.append("											AND		ROWNUM    		= 1" ).append("\n"); 
		query.append("										)" ).append("\n"); 
		...
		query.append("											AND     S.SPL_FOIL_WGT		> 0" ).append("\n"); 
		query.append("											AND		ROWNUM    			= 1" ).append("\n"); 
		query.append("										)" ).append("\n"); 
		...
		query.append("AND		T42.YD_CD		        (+)	= 'EGSCA10'		/* 수에즈 							*/" ).append("\n"); 

...
public class VOP_PSO_0006HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		if(command.isCommand(FormCommand.MULTI)) {
			...
			event.setServiceLaneListVOS((ServiceLaneListVO[])getVOs(request, ServiceLaneListVO.class,"sheet2_"));
			...
			String vsl_slan_cd = (String) request.getParameter("vsl_slan_cd");
			//parameter로 넘어온 vndrSeq를 event객체에 set한다.
			if(vsl_slan_cd !=null) event.setVsl_slan_cd(vsl_slan_cd);
			event.setServiceLaneListVO((ServiceLaneListVO)getVO(request, ServiceLaneListVO.class));
			request.setAttribute("Event", event);

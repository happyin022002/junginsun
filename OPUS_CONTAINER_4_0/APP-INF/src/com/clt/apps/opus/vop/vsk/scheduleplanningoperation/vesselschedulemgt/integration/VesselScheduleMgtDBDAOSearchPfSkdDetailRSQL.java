...
public class VesselScheduleMgtDBDAOSearchPfSkdDetailRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("" ).append("\n"); 
		query.append("AND 		'*'         			= CASE WHEN (	SELECT 		COUNT(1)" ).append("\n"); 
		query.append("                             						FROM   		VSK_PF_SKD_DTL  		PD" ).append("\n"); 
		query.append("                                  							,	VSK_PF_SKD_DIR  		DR" ).append("\n"); 
		query.append("                             						WHERE  		1 = 1" ).append("\n"); 
		query.append("                             						AND    		PD.VSL_SLAN_CD  		= DR.VSL_SLAN_CD" ).append("\n"); 
		query.append("                             						AND    		PD.PF_SVC_TP_CD 		= DR.PF_SVC_TP_CD" ).append("\n"); 
		query.append("                             						AND    		PD.SKD_DIR_CD   		= DR.VSL_SLAN_DIR_CD" ).append("\n"); 
		query.append("                             						AND    		PD.VSL_SLAN_CD  		= T2.VSL_SLAN_CD" ).append("\n"); 
		query.append("                             						AND    		PD.PF_SVC_TP_CD 		= T2.PF_SVC_TP_CD" ).append("\n"); 
		query.append("                             						AND    		PD.TURN_PORT_IND_CD 	<> 'F'" ).append("\n"); 
		query.append("                             						AND    		DR.VSL_SLAN_DIR_SEQ 	= '2'" ).append("\n"); 
		query.append("                             					) = 0  " ).append("\n"); 
		query.append("													AND T2.TURN_PORT_IND_CD = 'F' THEN 'X' ELSE '*' END" ).append("\n"); 
		query.append("" ).append("\n"); 

...
public class LaneExpenseRatioMgtDBDAO extends DBDAOSupport {
	 ...
	 * @throws DAOException
	 */
	public String searchRevenueDirection(ServiceLaneListVO serviceLaneListVO) throws DAOException {
		DBRowSet dbRowset = null;
		// query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		// velocity parameter
		...
		String sRet = "";
		try {
			if (serviceLaneListVO != null) {
				Map<String, String> mapVO = serviceLaneListVO.getColumnValues();

				param.putAll(mapVO);
				velParam.putAll(mapVO);
			}
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate) new LaneExpenseRatioMgtDAOsearchRevenueDirectionRSQL(), param, velParam);
			if (dbRowset.next()) {
				sRet = dbRowset.getString(1);
			}
		} catch (SQLException se) {
			log.error(se.getMessage(), se);
			throw new DAOException(new ErrorHandler(se).getMessage());
	 ...
	 * @throws DAOException
	 */
	public String searchRevenueDirectionByRevLane(ServiceLaneListVO serviceLaneListVO) throws DAOException {
		DBRowSet dbRowset = null;
		// query parameter
		Map<String, Object> param = new HashMap<String, Object>();
		// velocity parameter
		...
		try {
			...
			dbRowset = new SQLExecuter("").executeQuery((ISQLTemplate) new LaneExpenseRatioMgtDAOsearchRevenueDirectionByRevLaneRSQL(), param, velParam);
			if (dbRowset.next()) {
				sRet = dbRowset.getString(1);
			}
		} catch (SQLException se) {

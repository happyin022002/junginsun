...
public class GeneralInvoiceAuditDBDAOgetArrPobRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT ROUND(( VPS_ETB_DT - NVL(VPS_ETA_DT, PILOT_ARR) )*24,1)" ).append("\n"); 
		query.append("  FROM (SELECT A.VPS_ETB_DT" ).append("\n"); 
		query.append("             , A.VPS_ETA_DT" ).append("\n"); 
		query.append("             , T.PILOT_ARR" ).append("\n"); 
		...
		query.append("       )" ).append("\n"); 
		query.append(" WHERE VPS_ETB_DT >= NVL(VPS_ETA_DT, PILOT_ARR) " ).append("\n"); 

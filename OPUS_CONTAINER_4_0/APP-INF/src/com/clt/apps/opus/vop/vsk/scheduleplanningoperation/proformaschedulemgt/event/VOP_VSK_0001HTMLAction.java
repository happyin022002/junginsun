...
*@FileName : VOP_VSK_0001HTMLAction.java
...
import com.clt.apps.opus.vop.vsk.scheduleplanningoperation.vesselschedulemgt.event.SchedulePlanningOperationEvent;
...
public class VOP_VSK_0001HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_VSK_0001HTMLAction 객체를 생성
	 */
	public VOP_VSK_0001HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopVsk0001Event event = new VopVsk0001Event();
		...
		if(command.isCommand(FormCommand.MULTI)) {			
			...
			String minMaxSpd = request.getParameter("min_max_spd");
			String newSimData = request.getParameter("new_sim_data");
			PfSkdGRPVO grpVO = new PfSkdGRPVO();
			...
			VskPfSkdDtlVO vskPfSkdDtlVO = vskPfSkdDtlVOs.get(0);
			vskPfSkdDtlVO.setMinMaxSpd(minMaxSpd);
			vskPfSkdDtlVO.setNewSimData(newSimData);
			vskPfSkdDtlVOs.set(0, vskPfSkdDtlVO);
			
			List<VskPfSkdDtlVO> vskPfSkdDtlVOS = new ArrayList<VskPfSkdDtlVO>();
			for(int i=0; i<vskPfSkdDtlVOs.size(); i++){
				if(!"D".equals(vskPfSkdDtlVOs.get(i).getIbflag())){
					VskPfSkdDtlVO paramVO = vskPfSkdDtlVOs.get(i);
					vskPfSkdDtlVOS.add(paramVO);
				}
			}
			...
			event.setPfSkdRequestVO((PfSkdRequestVO)getVO(request, PfSkdRequestVO .class));
			...
			event.setVskPfSkdVO((VskPfSkdVO)getVO(request, VskPfSkdVO .class));
			...
			VskPfSkdDtlVO vskPfSkdDtlVO = vskPfSkdDtlVOs.get(0);
			vskPfSkdDtlVO.setMinMaxSpd(minMaxSpd);
			vskPfSkdDtlVO.setCurrPos(currPos);
			vskPfSkdDtlVOs.set(0, vskPfSkdDtlVO);
			...
			VskPfSkdDtlVO[] dtlVOs = (VskPfSkdDtlVO[])getVOs(request, VskPfSkdDtlVO .class,"sheet2_");
			List<VskPfSkdDtlVO> vskPfSkdDtlVOs = Arrays.asList(dtlVOs);
			List<VskPfSkdDtlVO> paramVOs = new ArrayList<VskPfSkdDtlVO>();
			
			for(int i=0; i<vskPfSkdDtlVOs.size(); i++){
				if(!"D".equals(vskPfSkdDtlVOs.get(i).getIbflag())){
					paramVOs.add(vskPfSkdDtlVOs.get(i));
				}
			}
			
			grpVO.setVskPfSkdDtlVOs(paramVOs);

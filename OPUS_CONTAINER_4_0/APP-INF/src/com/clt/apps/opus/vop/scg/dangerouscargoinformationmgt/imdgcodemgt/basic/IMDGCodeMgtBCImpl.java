...
/**
 * OPUS-DangerousCargoInformationMgt Business Logic Basic Command implementation<br>
 * - Handling business transactions of OPUS-DangerousCargoInformationMgt<br>
 *
 * @author
 * @see VOP_SCG-0001EventResponse,IMDGCodeMgtBC
 * @since J2EE 1.4
 */
public class IMDGCodeMgtBCImpl extends BasicCommandSupport implements IMDGCodeMgtBC {

	// Database Access Object
	private transient IMDGCodeMgtDBDAO dbDao = null;

	/**
	 * IMDGCodeMgtBCImpl object creation<br>
	 * IMDGCodeMgtDBDAO creation<br>
	 */
	...
	public List<UNNumberListOptionVO> searchUNNumberList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
			return dbDao.searchUNNumberList(unNumberListOptionVO);
	...
	public List<ScgImdgUnNoSegrListVO> searchSegrList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
            ...
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation"}).getMessage(), ex);
		}
	...
	public String[] searchSubsRskLblList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
			String[] returnList = dbDao.searchSubsRskLblList(unNumberListOptionVO);
			return returnList;

        } catch (DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Subsidiary risk(s)"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Subsidiary risk(s)"}).getMessage(), ex);
		}
	...
	public String[] searchSpclProviList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
			String[] returnList = dbDao.searchSpclProviList(unNumberListOptionVO);
			return returnList;
	...
	/**
	 * Segregation Group Detail retrieve <br>
	 * 
	 * @param unNumberListOptionVO
	 * @return String[]
	 * @exception EventException
	...
	public String[] searchSegrGrpDtlList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
			String[] returnList = dbDao.searchSegrGrpDtlList(unNumberListOptionVO);
			return returnList;

        } catch (DAOException ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation Group Detail"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation Group Detail"}).getMessage(), ex);
		}
	...
	public String[] searchSegrGrpList(UNNumberListOptionVO unNumberListOptionVO) throws EventException {
		try {
            ...
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation Group"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Segregation Group"}).getMessage(), ex);
		}
	...
	public List<ScgImdgExptQtyVO> searchExceptedQtyList() throws EventException {
		try {
            ...
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Excepted Q'ty"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"Excepted Q'ty"}).getMessage(), ex);
		}
	...
	/**
	 * Segregation Group List retrieve <br>
	 * 
	...
	/**
	 * AutoCreation Segregation retrieve <br>
	 * 
	...
	public List<ScgImdgUnNoSegrListVO> searchAutoCreation(AutoCreationVO autoCreationVO) throws EventException {
		try {
            ...
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"AutoCreation"}).getMessage(), ex);
        } catch (Exception ex) {
            log.error("err " + ex.toString(), ex);
            throw new EventException(new ErrorHandler("COM12203", new String[]{"AutoCreation"}).getMessage(), ex);
		}
	...
	public void manageUNNumber(UNNumberGrpVO unNumberGrpVO, SignOnUserAccount account) throws EventException{
		try {
			...
			for ( int i=0; i < 4; i++ ) {
				...
				scgImdgSubsRskLblVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgSubsRskLblVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());
				scgImdgSubsRskLblVOs.setCreUsrId(account.getUsr_id());
				if (i==0 && unNumberListOptionVO[0].getImdgSubsRskLblCd1().trim() != null && !unNumberListOptionVO[0].getImdgSubsRskLblCd1().equals("")){
					scgImdgSubsRskLblVOs.setImdgSubsRskLblCd(unNumberListOptionVO[0].getImdgSubsRskLblCd1());
					arrScgImdgSubsRskLblVO.add(scgImdgSubsRskLblVOs);
				}else if (i==1 && unNumberListOptionVO[0].getImdgSubsRskLblCd2().trim() != null && !unNumberListOptionVO[0].getImdgSubsRskLblCd2().equals("")){
					scgImdgSubsRskLblVOs.setImdgSubsRskLblCd(unNumberListOptionVO[0].getImdgSubsRskLblCd2());
					arrScgImdgSubsRskLblVO.add(scgImdgSubsRskLblVOs);
				}else if (i==2 && unNumberListOptionVO[0].getImdgSubsRskLblCd3().trim() != null && !unNumberListOptionVO[0].getImdgSubsRskLblCd3().equals("")){
					scgImdgSubsRskLblVOs.setImdgSubsRskLblCd(unNumberListOptionVO[0].getImdgSubsRskLblCd3());
					arrScgImdgSubsRskLblVO.add(scgImdgSubsRskLblVOs);
				}else if (i==3 && unNumberListOptionVO[0].getImdgSubsRskLblCd4().trim() != null && !unNumberListOptionVO[0].getImdgSubsRskLblCd4().equals("")){
					scgImdgSubsRskLblVOs.setImdgSubsRskLblCd(unNumberListOptionVO[0].getImdgSubsRskLblCd4());
					arrScgImdgSubsRskLblVO.add(scgImdgSubsRskLblVOs);
				}
			}
			...
			for ( int i=0; i < 8; i++ ) {
				...
				scgImdgUnNoSpclProviVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoSpclProviVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());					
				scgImdgUnNoSpclProviVOs.setCreUsrId(account.getUsr_id());
				
				if (i==0 && unNumberListOptionVO[0].getImdgSpclProviNo1().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo1().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo1());
					scgImdgUnNoSpclProviVOs.setDpSeq("1");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==1 && unNumberListOptionVO[0].getImdgSpclProviNo2().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo2().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo2());
					scgImdgUnNoSpclProviVOs.setDpSeq("2");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==2 && unNumberListOptionVO[0].getImdgSpclProviNo3().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo3().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo3());
					scgImdgUnNoSpclProviVOs.setDpSeq("3");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==3 && unNumberListOptionVO[0].getImdgSpclProviNo4().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo4().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo4());
					scgImdgUnNoSpclProviVOs.setDpSeq("4");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==4 && unNumberListOptionVO[0].getImdgSpclProviNo5().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo5().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo5());
					scgImdgUnNoSpclProviVOs.setDpSeq("5");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==5 && unNumberListOptionVO[0].getImdgSpclProviNo6().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo6().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo6());
					scgImdgUnNoSpclProviVOs.setDpSeq("6");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==6 && unNumberListOptionVO[0].getImdgSpclProviNo7().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo7().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo7());
					scgImdgUnNoSpclProviVOs.setDpSeq("7");
					arrScgImdgUnNoSpclProviVO.add(scgImdgUnNoSpclProviVOs);
				}else if (i==7 && unNumberListOptionVO[0].getImdgSpclProviNo8().trim() != null && !unNumberListOptionVO[0].getImdgSpclProviNo8().equals("")){
					scgImdgUnNoSpclProviVOs.setImdgSpclProviNo(unNumberListOptionVO[0].getImdgSpclProviNo8());
			...
			for ( int i=0; i < 20; i++ ) {
				...
				scgImdgUnNoSegrListVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoSegrListVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());
				if (i==0){
					scgImdgUnNoSegrListVOs.setImdgClssCd("1.1");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd11());
					...
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd12());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==2){
					scgImdgUnNoSegrListVOs.setImdgClssCd("1.3");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd13());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==3){
					scgImdgUnNoSegrListVOs.setImdgClssCd("1.4");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd14());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==4){
					scgImdgUnNoSegrListVOs.setImdgClssCd("1.5");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd15());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==5){
					scgImdgUnNoSegrListVOs.setImdgClssCd("1.6");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd16());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==6){
					scgImdgUnNoSegrListVOs.setImdgClssCd("2.1");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd21());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==7){
					scgImdgUnNoSegrListVOs.setImdgClssCd("2.2");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd22());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==8){
					scgImdgUnNoSegrListVOs.setImdgClssCd("2.3");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd23());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==9){
					scgImdgUnNoSegrListVOs.setImdgClssCd("3");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd3());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==10){
					scgImdgUnNoSegrListVOs.setImdgClssCd("4.1");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd41());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==11){
					scgImdgUnNoSegrListVOs.setImdgClssCd("4.2");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd42());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==12){
					scgImdgUnNoSegrListVOs.setImdgClssCd("4.3");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd43());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==13){
					scgImdgUnNoSegrListVOs.setImdgClssCd("5.1");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd51());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==14){
					scgImdgUnNoSegrListVOs.setImdgClssCd("5.2");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd52());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==15){
					scgImdgUnNoSegrListVOs.setImdgClssCd("6.1");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd61());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==16){
					scgImdgUnNoSegrListVOs.setImdgClssCd("6.2");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd62());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==17){
					scgImdgUnNoSegrListVOs.setImdgClssCd("7");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd7());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==18){
					scgImdgUnNoSegrListVOs.setImdgClssCd("8");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd8());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}else if (i==19){
					scgImdgUnNoSegrListVOs.setImdgClssCd("9");
					scgImdgUnNoSegrListVOs.setImdgSegrCd(scgImdgUnNoSegrListVO[0].getClssCd9());
					scgImdgUnNoSegrListVOs.setCreUsrId(account.getUsr_id());
					arrScgImdgUnNoSegrListVO.add(scgImdgUnNoSegrListVOs);
				}
			}
			...
			String strAwayFmImdgClssCd 		= unNumberListOptionVO[0].getAwayFmImdgClssCd().toString().trim();
			String strSprtFmImdgClssCd 		= unNumberListOptionVO[0].getSprtFmImdgClssCd().toString().trim();
			String strSprtHldFmImdgClssCd 	= unNumberListOptionVO[0].getSprtHldFmImdgClssCd().toString().trim();
			String strSprtLonFmImdgClssCd 	= unNumberListOptionVO[0].getSprtLonFmImdgClssCd().toString().trim();
			String[] arrAwayFmImdgClssCd 	= strAwayFmImdgClssCd.split("\\/");
			String[] arrSprtFmImdgClssCd 	= strSprtFmImdgClssCd.split("\\/");
			String[] arrSprtHldFmImdgClssCd = strSprtHldFmImdgClssCd.split("\\/");
			String[] arrSprtLonFmImdgClssCd = strSprtLonFmImdgClssCd.split("\\/");
			for (int i=0; i < arrAwayFmImdgClssCd.length; i++) {
				...
				scgImdgUnNoClssCdVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoClssCdVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());					
			...
			for (int i=0; i < arrSprtHldFmImdgClssCd.length; i++) {
				if (arrSprtHldFmImdgClssCd[i] != null && !arrSprtHldFmImdgClssCd[i].equals("")) {
				ScgImdgUnNoClssCdVO scgImdgUnNoClssCdVOs = new ScgImdgUnNoClssCdVO();
				scgImdgUnNoClssCdVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoClssCdVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());					
				scgImdgUnNoClssCdVOs.setCreUsrId(account.getUsr_id());
				scgImdgUnNoClssCdVOs.setImdgClssCd(arrSprtHldFmImdgClssCd[i]);
			...
			for (int i=0; i < arrSprtLonFmImdgClssCd.length; i++) {
				if (arrSprtLonFmImdgClssCd[i] != null && !arrSprtLonFmImdgClssCd[i].equals("")) {
				ScgImdgUnNoClssCdVO scgImdgUnNoClssCdVOs = new ScgImdgUnNoClssCdVO();
				scgImdgUnNoClssCdVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoClssCdVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());					
				scgImdgUnNoClssCdVOs.setCreUsrId(account.getUsr_id());
				scgImdgUnNoClssCdVOs.setImdgClssCd(arrSprtLonFmImdgClssCd[i]);
			...
			String strAwayDpSeq = unNumberListOptionVO[0].getAwayDpSeq().toString().trim();
			String strSprtDpSeq = unNumberListOptionVO[0].getSprtDpSeq().toString().trim();
			...
			for (int i=0; i < arrAwayDpSeq.length; i++) {
				...
				scgImdgUnNoSegrGrpVOs.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
				scgImdgUnNoSegrGrpVOs.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());					
			...
			scgImdgUnNoOrgRactVO.setImdgUnNo(unNumberListOptionVO[0].getImdgUnNo());
			scgImdgUnNoOrgRactVO.setImdgUnNoSeq(unNumberListOptionVO[0].getImdgUnNoSeq());
			scgImdgUnNoOrgRactVO.setImdgOrgRactTpCd(unNumberListOptionVO[0].getImdgOrgRactTpCd());
			scgImdgUnNoOrgRactVO.setImdgTecNm(unNumberListOptionVO[0].getImdgTecNm());
			scgImdgUnNoOrgRactVO.setImdgConcRtCtnt(unNumberListOptionVO[0].getImdgConcRtCtnt());
			scgImdgUnNoOrgRactVO.setImdgPckMzdCd(unNumberListOptionVO[0].getImdgPckMzdCd());
			scgImdgUnNoOrgRactVO.setImdgCtrlTemp(unNumberListOptionVO[0].getImdgCtrlTemp());
			scgImdgUnNoOrgRactVO.setImdgEmerTemp(unNumberListOptionVO[0].getImdgEmerTemp());
	...
	public void manageSegregation(SegregationTableGrpVO segregationTableGrpVO, SignOnUserAccount account) throws EventException{
		try {
			...
			scgImdgClssSegrListVOs  = segregationTableGrpVO.getScgImdgClssSegrListVOs();
			scgImdgCompGrpSegrListVOs = segregationTableGrpVO.getScgImdgCompGrpSegrListVOs();
				...
				for ( int i=0; i<scgImdgClssSegrListVOs.length; i++ ) {
					if ( scgImdgClssSegrListVOs[i].getIbflag().equals("I")){
						for ( int j=0; j < 20; j++ ) {
							...
							scgImdgClssSegrVO.setRowImdgClssCd(scgImdgClssSegrListVOs[i].getRowImdgClssCd());
							if (j==0){
								scgImdgClssSegrVO.setColImdgClssCd("1.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd11());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==1){
								scgImdgClssSegrVO.setColImdgClssCd("1.2");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd12());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==2){
								scgImdgClssSegrVO.setColImdgClssCd("1.3");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd13());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==3){
								scgImdgClssSegrVO.setColImdgClssCd("1.4");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd14());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==4){
								scgImdgClssSegrVO.setColImdgClssCd("1.5");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd15());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==5){
								scgImdgClssSegrVO.setColImdgClssCd("1.6");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd16());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==6){
								scgImdgClssSegrVO.setColImdgClssCd("2.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd21());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==7){
								scgImdgClssSegrVO.setColImdgClssCd("2.2");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd22());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==8){
								scgImdgClssSegrVO.setColImdgClssCd("2.3");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd23());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==9){
								scgImdgClssSegrVO.setColImdgClssCd("3");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd3());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==10){
								scgImdgClssSegrVO.setColImdgClssCd("4.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd41());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==11){
								scgImdgClssSegrVO.setColImdgClssCd("4.2");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd42());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==12){
								scgImdgClssSegrVO.setColImdgClssCd("4.3");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd43());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==13){
								scgImdgClssSegrVO.setColImdgClssCd("5.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd51());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==14){
								scgImdgClssSegrVO.setColImdgClssCd("5.2");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd52());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==15){
								scgImdgClssSegrVO.setColImdgClssCd("6.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd61());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==16){
								scgImdgClssSegrVO.setColImdgClssCd("6.2");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd62());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==17){
								scgImdgClssSegrVO.setColImdgClssCd("7");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd7());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==18){
								scgImdgClssSegrVO.setColImdgClssCd("8");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd8());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}else if (j==19){
								scgImdgClssSegrVO.setColImdgClssCd("9");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd9());
								scgImdgClssSegrVO.setCreUsrId(account.getUsr_id());
								insertVoList.add(scgImdgClssSegrVO);
							}
						}					
					} else if ( scgImdgClssSegrListVOs[i].getIbflag().equals("U")){
						for ( int j=0; j < 20; j++ ) {
							ScgImdgClssSegrVO scgImdgClssSegrVO = new ScgImdgClssSegrVO();
							scgImdgClssSegrVO.setRowImdgClssCd(scgImdgClssSegrListVOs[i].getRowImdgClssCd());
							if (j==0){
								scgImdgClssSegrVO.setColImdgClssCd("1.1");
								scgImdgClssSegrVO.setImdgSegrCd(scgImdgClssSegrListVOs[i].getClssCd11());
								scgImdgClssSegrVO.setUpdUsrId(account.getUsr_id());
					...
					} else if ( scgImdgClssSegrListVOs[i].getIbflag().equals("D")){
						for ( int j=0; j < 20; j++ ) {
							ScgImdgClssSegrVO scgImdgClssSegrVO = new ScgImdgClssSegrVO();
							scgImdgClssSegrVO.setRowImdgClssCd(scgImdgClssSegrListVOs[i].getRowImdgClssCd());
							if (j==0){
				...
				for ( int i=0; i<scgImdgCompGrpSegrListVOs.length; i++ ) {
					if ( scgImdgCompGrpSegrListVOs[i].getIbflag().equals("I")){
						for ( int j=0; j < 13; j++ ) {
							...
							scgImdgCompGrpSegrVO.setRowImdgCompGrpCd(scgImdgCompGrpSegrListVOs[i].getRowImdgCompGrpCd());
							if (j==0){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("A");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdA().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdA().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdA().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdA());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==1){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("B");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdB().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdB().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdB().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdB());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==2){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("C");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdC().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdC().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdC().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdC());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==3){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("D");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdD().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdD().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdD().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdD());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==4){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("E");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdE().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdE().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdE().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdE());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==5){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("F");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdF().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdF().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdF().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdF());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==6){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("G");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdG().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdG().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdG().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdG());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==7){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("H");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdH().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdH().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdH().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdH());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==8){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("J");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdJ().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdJ().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdJ().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdJ());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==9){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("K");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdK().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdK().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdK().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdK());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==10){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("L");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdL().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdL().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdL().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdL());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==11){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("N");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdN().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdN().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdN().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdN());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}else if (j==12){
								scgImdgCompGrpSegrVO.setColImdgCompGrpCd("S");
								if (scgImdgCompGrpSegrListVOs[i].getSegrCdS().length() > 1) {
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdS().substring(0,1));
									scgImdgCompGrpSegrVO.setImdgSegrNtcNo(scgImdgCompGrpSegrListVOs[i].getSegrCdS().substring(1,2));
								}else{
									scgImdgCompGrpSegrVO.setImdgSegrCd(scgImdgCompGrpSegrListVOs[i].getSegrCdS());								
								}
								scgImdgCompGrpSegrVO.setCreUsrId(account.getUsr_id());
								insertVoCompList.add(scgImdgCompGrpSegrVO);
							}
						}					
					} else if ( scgImdgCompGrpSegrListVOs[i].getIbflag().equals("U")){
						for ( int j=0; j < 13; j++ ) {
							ScgImdgCompGrpSegrVO scgImdgCompGrpSegrVO = new ScgImdgCompGrpSegrVO();
							scgImdgCompGrpSegrVO.setRowImdgCompGrpCd(scgImdgCompGrpSegrListVOs[i].getRowImdgCompGrpCd());
							if (j==0){
								...
								scgImdgCompGrpSegrVO.setUpdUsrId(account.getUsr_id());
					...
					} else if ( scgImdgCompGrpSegrListVOs[i].getIbflag().equals("D")){
						for ( int j=0; j < 13; j++ ) {
							ScgImdgCompGrpSegrVO scgImdgCompGrpSegrVO = new ScgImdgCompGrpSegrVO();
							scgImdgCompGrpSegrVO.setRowImdgCompGrpCd(scgImdgCompGrpSegrListVOs[i].getRowImdgCompGrpCd());
							if (j==0){
    ...
    public void manageSpecialProvisionSegregationList(
            ScgImdgSpclProvisVO[] scgImdgSpclProvisVOs, String imdgSpclProviNo,  String usrId) throws EventException {
        try {
            List<ScgImdgSpclProvisVO> updateVoList = new ArrayList<ScgImdgSpclProvisVO>();

            for ( int i=0; i<scgImdgSpclProvisVOs.length; i++ ) {

                if ( scgImdgSpclProvisVOs[i].getIbflag().equals("I") || scgImdgSpclProvisVOs[i].getIbflag().equals("U")){
    ...
    /**
	 * Segregation Simulation in a CNTR Segregation Validation retrieve <br>
	 * 
	...
	public List<SegregationSimulationOutputVO> checkSegregation(SegregationSimulationInputVO[] segregationSimulationInputVOs) throws EventException {
		try {
			List<SegregationSimulationOutputVO> rsltVOs = dbDao.checkSegregation(segregationSimulationInputVOs);
			...
			for(int rsltCt=0; rsltCt<rsltVOs.size(); rsltCt++) {
				for(int rmCt=rsltCt+1; rmCt<rsltVOs.size(); rmCt++) {
					if(rsltVOs.get(rsltCt).getImdgUnNo1().equals(rsltVOs.get(rmCt).getImdgUnNo2())
					   && rsltVOs.get(rsltCt).getImdgUnNoSeq1().equals(rsltVOs.get(rmCt).getImdgUnNoSeq2())
					   && rsltVOs.get(rsltCt).getImdgUnNo2().equals(rsltVOs.get(rmCt).getImdgUnNo1())
					   && rsltVOs.get(rsltCt).getImdgUnNoSeq2().equals(rsltVOs.get(rmCt).getImdgUnNoSeq1())
					) 
					{
						rmList[rmCt] = "Y";
					}

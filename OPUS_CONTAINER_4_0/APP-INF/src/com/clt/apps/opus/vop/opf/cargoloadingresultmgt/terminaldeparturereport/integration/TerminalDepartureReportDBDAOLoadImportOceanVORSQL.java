...
public class TerminalDepartureReportDBDAOLoadImportOceanVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append("       SUM(DECODE(V.POL_CD, B.POL_CD, DECODE(B.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(C.CNTR_TPSZ_CD, 2, 1), 5, 1, 0), 0), 0)) 	FULL_IN_4H,  " ).append("\n"); 
		...
		query.append("       SUM(DECODE(V.POL_CD, B.POL_CD, 0, DECODE(B.BKG_CGO_TP_CD, 'F', DECODE(SUBSTR(CCNTR_TPSZ_CD, 2, 1), 5, 1, 0), 0)))  	FULL_TS_4H,   " ).append("\n"); 
		...
		query.append("       SUM(DECODE(V.POL_CD, B.POL_CD, DECODE(B.BKG_CGO_TP_CD, 'F', 0, DECODE(SUBSTR(CCNTR_TPSZ_CD, 2, 1), 5, 1, 0)), 0))  	ET_IN_4H,     " ).append("\n"); 
		...
		query.append("       SUM(DECODE(V.POL_CD, B.POL_CD, 0, DECODE(B.BKG_CGO_TP_CD, 'F', 0, DECODE(SUBSTR(C.CNTR_TPSZ_CD, 2, 1), 5, 1, 0)))) 	ET_TS_4H,    " ).append("\n"); 
		...
		query.append("       SUM(DECODE(SUBSTR(C.CNTR_TPSZ_CD, 2, 1), '5', C.CNTR_WGT, 0))                                                		WT_4H,       " ).append("\n"); 

...
public class RegionDepartureReportDBDAOSearchImpVslMvmtRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(",      TO_CHAR(A.VPS_ETA_DT, 'YYYYMMDDHH24MI') AS  ARR_TIME" ).append("\n"); 
		query.append(",      TO_CHAR(A.VPS_ETB_DT, 'YYYYMMDDHH24MI') AS  BERTH_TIME" ).append("\n"); 
		query.append(",      TO_CHAR(U.ACT_DEP_DT, 'YYYYMMDDHH24MI') AS  UNBERTH_TIME" ).append("\n"); 
		query.append(",      TO_CHAR(A.VPS_ETD_DT, 'YYYYMMDDHH24MI') AS  DEP_TIME" ).append("\n"); 
		...
		query.append("                        FROM   VSK_VSL_PORT_SKD C, MDM_LOCATION D," ).append("\n"); 
		query.append("                               ( SELECT COUNT(*) CNT" ).append("\n"); 
		query.append("                                 FROM   ( SELECT B.CONTI_CD" ).append("\n"); 
		...
		query.append("                                          GROUP BY B.CONTI_CD ) ) L" ).append("\n"); 
		query.append("                        WHERE C.VSL_CD            = @[vsl_cd]" ).append("\n"); 
		query.append("                        AND   ( C.SKD_CNG_STS_CD IS NULL OR C.SKD_CNG_STS_CD <> 'S')" ).append("\n"); 
		query.append("                        AND   D.CONTI_CD         <> DECODE (L.CNT, 1, @[region] ||@[region], @[region])" ).append("\n"); 
		...
		query.append("                        AND   C.VPS_PORT_CD      =  D.LOC_CD" ).append("\n"); 

...
public class PortInformationMgtDBDAOMdmRhqComboRSQL implements ISQLTemplate{
	...
	public void setQuery(){
		query.append("SELECT    (SELECT MAX(ML.LOC_NM) FROM MDM_LOCATION ML WHERE ML.LOC_CD = @[loc_cd]) AS NAME" ).append("\n"); 
		...
		query.append("--                --THEN (SELECT OFC_CD FROM TABLE(COM_OfficeCodeMgr_PKG.COM_getOfficeCodeList_FNC('000004', 'VSK'))) -- SOUTHWEST ASIA & MIDDLE EAST REGIONAL HEADQUARTERS" ).append("\n"); 

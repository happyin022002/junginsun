...
*@FileName : TerminalDepartureReportDBDAOTerminalDepartureReportMonthVORSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class TerminalDepartureReportDBDAOTerminalDepartureReportMonthVORSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * dummy
	  * 2010.08.16 이준범 
	  *  - ZADUR는 HAMUR에서 SINWA으로 소속 변경됨
	  * </pre>
	  */
	public TerminalDepartureReportDBDAOTerminalDepartureReportMonthVORSQL(){
		setQuery();
		...
		query.append("FileName : TerminalDepartureReportDBDAOTerminalDepartureReportMonthVORSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("              DECODE(C2,1,'',2,'','') LANE," ).append("\n"); 
		query.append("              DECODE(C2,1,'',2,'','') VVD," ).append("\n"); 
		query.append("              DECODE(C2,1,'',2,'','') ATA," ).append("\n"); 
		query.append("              DECODE(C2,1,'',2,'','') ATD," ).append("\n"); 
		query.append("              DECODE(C2,1,'',2,'',MONTH) MONTH," ).append("\n"); 
		...
		query.append("         FROM   " ).append("\n"); 
		query.append("         ( " ).append("\n"); 
		query.append("           SELECT /*+ ORDERED */ V.VPS_PORT_CD               PORT," ).append("\n"); 
		query.append("                  V.YD_CD                                    YARD," ).append("\n"); 
		query.append("                  TO_CHAR(A.ACT_DEP_DT,'YYYYMM')             MONTH," ).append("\n"); 
		query.append("                  GROUPING(V.VPS_PORT_CD) V, GROUPING(V.YD_CD) Y, GROUPING(TO_CHAR(A.ACT_DEP_DT,'YYYYMM')) Y," ).append("\n"); 
		query.append("                  CASE" ).append("\n"); 
		query.append("                  WHEN GROUPING(V.VPS_PORT_CD)=0 AND GROUPING(V.YD_CD)=0 AND GROUPING(TO_CHAR(A.ACT_DEP_DT,'YYYYMM'))=0 THEN 1" ).append("\n"); 
		query.append("                  ELSE 0" ).append("\n"); 
		query.append("                  END C1," ).append("\n"); 
		query.append("                  CASE" ).append("\n"); 
		query.append("                  WHEN GROUPING(V.VPS_PORT_CD)=0 AND GROUPING(V.YD_CD)=0 AND GROUPING(TO_CHAR(A.ACT_DEP_DT,'YYYYMM'))=1 THEN 1" ).append("\n"); 
		query.append("                  ELSE 0" ).append("\n"); 
		query.append("                  END C3," ).append("\n"); 
		query.append("                  CASE" ).append("\n"); 
		query.append("                  WHEN GROUPING(V.VPS_PORT_CD)=1 AND GROUPING(V.YD_CD)=1 AND GROUPING(TO_CHAR(A.ACT_DEP_DT,'YYYYMM'))=1 THEN 1" ).append("\n"); 
		query.append("                  ELSE 0" ).append("\n"); 
		...
		query.append("#if (${loc_cd} != '' && ${yd_cd} != 'All')" ).append("\n"); 
		query.append("AND    V.YD_CD        LIKE @[loc_cd]||@[yd_cd]||'%'                                                --:yd_cd" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("AND    R.POR_RHQ      = DECODE(@[rhq], 'ALL', R.POR_RHQ, @[rhq])                                   --:rhq_cd" ).append("\n"); 
		query.append("#if (${slan_cd} != 'ALL')" ).append("\n"); 
		query.append("AND    V.SLAN_CD      LIKE @[slan_cd]||'%'                                                         --:Lane_cd" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${target_lanes} != 'ALL')" ).append("\n"); 
		query.append("AND    L.TML_PROD_RPT_FLG LIKE @[target_lanes]||'%'                                                --Target Lanes Only 'Y', ALL NULL" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${target_ports} != 'ALL')" ).append("\n"); 
		query.append("AND    R.TP           LIKE @[target_ports]||'%'                                                    --Target Ports Only 'B', ALLNULL" ).append("\n"); 
		query.append("AND    R.VOP_PORT_FLG = 'Y'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${tml_prod_rpt_rsn_cd} != '')" ).append("\n"); 
		query.append("AND    DECODE(D.TML_PROD_RPT_RSN_CD,NULL,'Y','N') = DECODE(@[tml_prod_rpt_rsn_cd],'N','Y','N')       --Exclude fm TPR Check 'Y', Check 'N'" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("AND    DECODE(D.TML_PROD_RPT_RSN_CD,NULL,'Y','N') = DECODE('N','N','Y','N')       --Exclude fm TPR Check 'Y', Check   'N'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		...
		query.append("" ).append("\n"); 
		query.append("                  GROUP BY CUBE( V.VPS_PORT_CD, V.YD_CD, TO_CHAR(A.ACT_DEP_DT,'YYYYMM') )" ).append("\n"); 
		query.append("       )" ).append("\n"); 
		...
		query.append("#end" ).append("\n"); 
		query.append("#if (${target_lanes} != 'ALL')" ).append("\n"); 
		query.append("AND    L.TML_PROD_RPT_FLG LIKE @[target_lanes]||'%'                                                --Target Lanes Only   NULL" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${target_ports} != 'ALL')" ).append("\n"); 
		query.append("AND    R.TP           LIKE @[target_ports]||'%'                                                    --Target Ports Only  NULL" ).append("\n"); 
		query.append("AND    R.VOP_PORT_FLG = 'Y'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("#if (${tml_prod_rpt_rsn_cd} != '')" ).append("\n"); 
		query.append("AND    DECODE(D.TML_PROD_RPT_RSN_CD,NULL,'Y','N') = DECODE(@[tml_prod_rpt_rsn_cd],'N','Y','N')       --Exclude fm TPR  'N'" ).append("\n"); 
		query.append("#else" ).append("\n"); 
		query.append("AND    DECODE(D.TML_PROD_RPT_RSN_CD,NULL,'Y','N') = DECODE('N','N','Y','N')       --Exclude fm TPR , Check  'N'" ).append("\n"); 
		query.append("#end" ).append("\n"); 
		query.append("         AND    DECODE(@[carr_cd], NULL, 'N', DECODE(" ).append("\n"); 
		query.append("                               CASE" ).append("\n"); 
		query.append("                               WHEN TO_DATE(C.EFF_DT,'YYYYMMDD') <= TO_DATE(TO_CHAR(A.ACT_DEP_DT,'YYYYMMDD'),'YYYYMMDD')" ).append("\n"); 
		...
		query.append("       /* Q2 */   " ).append("\n"); 
		query.append("     )" ).append("\n"); 
		query.append("     ORDER BY DECODE(C2,1,'YYYYY',2,'ZZZZZ',PORT), DECODE(C2,3,YARD||'Z',YARD), MONTH" ).append("\n"); 
		query.append(")" ).append("\n"); 

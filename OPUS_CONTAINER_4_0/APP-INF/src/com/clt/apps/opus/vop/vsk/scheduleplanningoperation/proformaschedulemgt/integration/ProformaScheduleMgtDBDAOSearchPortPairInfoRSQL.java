...
*@FileName : ProformaScheduleMgtDBDAOSearchPortPairInfoRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class ProformaScheduleMgtDBDAOSearchPortPairInfoRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public ProformaScheduleMgtDBDAOSearchPortPairInfoRSQL(){
		setQuery();
		...
		params.put("pod_loc_cd",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : ProformaScheduleMgtDBDAOSearchPortPairInfoRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("SELECT  NVL(PORT_DIST,0) AS LNK_DIST,NVL(FM_PORT_GMT,0) AS FM_ZD,NVL(TO_PORT_GMT,0) AS TO_ZD," ).append("\n"); 
		query.append("        NVL(MNVR_IO_TIME,0) AS MNVR_IN_HRS, NVL(MNVR_IO_TIME,0) AS MNVR_OUT_HRS, " ).append("\n"); 
		query.append("		NVL(PORT_BUF_TIME,0) AS PORT_BUF_HRS,NVL(LNK_SPD,0) AS LNK_SPD" ).append("\n"); 
		...
		query.append("                        AND     TO_LOC_CD   = @[pod_loc_cd]" ).append("\n"); 
		...
		query.append("					(  SELECT  GMT_HRS / 60" ).append("\n"); 
		query.append("					   FROM    MDM_LOCATION" ).append("\n"); 
		query.append("					   WHERE   LOC_CD      = @[pod_loc_cd]" ).append("\n"); 
		...
		query.append("					,(" ).append("\n"); 
		query.append("                        SELECT AVG(VSL_SVC_SPD)" ).append("\n"); 
		query.append("						FROM  ( SELECT CNTR_VSL_CLSS_CAPA, VSL_SVC_SPD" ).append("\n"); 
		query.append("						        FROM MDM_VSL_CNTR" ).append("\n"); 
		query.append("						        WHERE VSL_SVC_SPD > 0" ).append("\n"); 
		query.append("							   )" ).append("\n"); 
		query.append("                        WHERE CNTR_VSL_CLSS_CAPA = @[n1st_vsl_clss_cd]" ).append("\n"); 
		query.append("						#if (${n2nd_vsl_clss_cd} != '') " ).append("\n"); 
		query.append("                        OR CNTR_VSL_CLSS_CAPA =  @[n2nd_vsl_clss_cd]" ).append("\n"); 
		query.append("						#end" ).append("\n"); 
		query.append("						#if (${n3rd_vsl_clss_cd} != '') " ).append("\n"); 
		query.append("                        OR CNTR_VSL_CLSS_CAPA =  @[n3rd_vsl_clss_cd]" ).append("\n"); 
		query.append("						#end" ).append("\n"); 
		...
		query.append("								AND		MVS				IS NOT NULL			--::2015-04-17::--" ).append("\n"); 

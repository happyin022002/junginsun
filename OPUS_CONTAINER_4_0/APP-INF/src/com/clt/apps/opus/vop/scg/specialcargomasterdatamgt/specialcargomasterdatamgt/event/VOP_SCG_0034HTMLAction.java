...
*@FileName : VMS_SCG_0034HTMLAction.java
...
public class VOP_SCG_0034HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VMS_SCG_0034HTMLAction 객체를 생성
	 */
	public VOP_SCG_0034HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopScg0034Event event = new VopScg0034Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setScgCntcPntVOS((ScgCntcPntVO[])getVOs(request, ScgCntcPntVO .class,""));
			event.setScgCntcPntVO((ScgCntcPntVO)getVO(request, ScgCntcPntVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH)) {
			event.setScgCntcPntVO((ScgCntcPntVO)getVO(request, ScgCntcPntVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH01)) {
			event.setScgRgnShpOprCdVO((ScgRgnShpOprCdVO)getVO(request, ScgRgnShpOprCdVO .class));
		}

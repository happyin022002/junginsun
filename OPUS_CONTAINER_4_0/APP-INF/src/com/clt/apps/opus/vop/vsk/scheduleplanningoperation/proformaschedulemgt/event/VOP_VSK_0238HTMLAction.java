...
*@FileName : VOP_VSK_0238HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opuscntr.vop.vsk.scheduleplanningoperation 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class VOP_VSK_0238HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * VOP_VSK_0238HTMLAction 객체를 생성
	 */
	public VOP_VSK_0238HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		VopVsk0238Event event = new VopVsk0238Event();
			
		if(command.isCommand(FormCommand.SEARCH)) {					
			//event.setPfSkdBerthWdoVO((PfSkdBerthWdoVO)getVO(request, PfSkdBerthWdoVO .class));
			...
			for(int i=0; i<vskPfSkdDtlVOs.size(); i++){
				vskPfSkdDtlVOs.get(i).setYdCd(vskPfSkdDtlVOs.get(i).getPortCd()+vskPfSkdDtlVOs.get(i).getYdCd());
			}
			
			event.setVskPfSkdDtlVOs(vskPfSkdDtlVOs);
		}

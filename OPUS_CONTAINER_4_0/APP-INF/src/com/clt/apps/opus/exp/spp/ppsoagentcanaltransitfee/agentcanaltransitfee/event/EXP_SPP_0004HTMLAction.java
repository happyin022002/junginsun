...
*@FileName : EXP_SPP_0004HTMLAction.java
...
/**
 * HTTP Parser<br>
 * - com.clt.apps.opus.exp.spp.ppsoagentcanaltransitfee 화면을 통해 서버로 전송되는 HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
...
public class EXP_SPP_0004HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * EXP_SPP_0004HTMLAction 객체를 생성
	 */
	public EXP_SPP_0004HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	ExpSpp0004Event event = new ExpSpp0004Event();
		
		if(command.isCommand(FormCommand.SEARCH)) {  //MSA Balance search
			event.setMsaBalVO((MsaBalVO)getVO(request, MsaBalVO .class));
		}
		else if(command.isCommand(FormCommand.MULTI)) {//save button click
			event.setPsoMsaVO((PsoMsaVO)getVO(request, PsoMsaVO .class));	
			event.setMsaBalVOS((MsaBalVO[])getVOs(request, MsaBalVO .class, "t1sheet1_"));
		}	
		else if(command.isCommand(FormCommand.COMMAND01)) {//request button click
			event.setPsoMsaVO((PsoMsaVO)getVO(request, PsoMsaVO .class));			
		}
		else if(command.isCommand(FormCommand.SEARCH02)) {  //MSA Remittance search
			event.setMsaRemVO((MsaRemVO)getVO(request, MsaRemVO .class));
		}
		else if(command.isCommand(FormCommand.SEARCH03)) {  //MSA Disbursement search
			event.setMsaDisbVO((MsaDisbVO)getVO(request, MsaDisbVO .class));
		}		

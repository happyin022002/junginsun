...
public class BCM_CCD_0039HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0039HTMLAction 객체를 생성
	 */
	public BCM_CCD_0039HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0039Event event = new BcmCcd0039Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setCreditCustomerVO((CreditCustomerVO)getVO(request, CreditCustomerVO .class));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setCreditCustomerVOS((CreditCustomerVO[])getVOs(request, CreditCustomerVO.class,""));

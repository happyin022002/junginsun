...
*@FileName : BCM_CCD_0012HTMLAction.java
...
public class BCM_CCD_0012HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0012HTMLAction 객체를 생성
	 */
	public BCM_CCD_0012HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0012Event event = new BcmCcd0012Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setCustPackageTypeVOS((CustPackageTypeVO[])getVOs(request, CustPackageTypeVO.class, "sheet1_"));
			
		} else if(command.isCommand(FormCommand.SEARCH)) {
			event.setCustPackageTypeVO((CustPackageTypeVO)getVO(request, CustPackageTypeVO.class));
			event.setPckCd(request.getParameter("pck_cd"));
			event.setCstmsCntCd(request.getParameter("cstms_cnt_cd"));

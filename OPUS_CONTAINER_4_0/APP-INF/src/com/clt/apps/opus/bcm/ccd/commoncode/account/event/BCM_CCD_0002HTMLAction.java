...
*@FileName : BCM_CCD_0002HTMLAction.java
*@FileTitle : Charge
*Open Issues :
*Change history :
...
public class BCM_CCD_0002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0002HTMLAction 객체를 생성
	 */
	public BCM_CCD_0002HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0002Event event = new BcmCcd0002Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setChargeVOS((ChargeVO[])getVOs(request, ChargeVO.class, "sheet1_"));
		} else if(command.isCommand(FormCommand.SEARCH)) {
			event.setChargeVO((ChargeVO)getVO(request, ChargeVO.class));
			event.setChgCd(request.getParameter("chg_cd"));
		} else if(command.isCommand(FormCommand.SEARCH02)) {
			event.setAcctCd(request.getParameter("co_chg_acct_cd"));
		} else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setTaxCntCd(request.getParameter("tax_cnt_cd"));

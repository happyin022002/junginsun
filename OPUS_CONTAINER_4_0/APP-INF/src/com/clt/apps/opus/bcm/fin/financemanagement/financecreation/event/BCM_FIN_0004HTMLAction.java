...
*@FileName : BCM_FIN_0004HTMLAction.java
 ...
 * @see BcmFin0004Event 참조
 * @since J2EE 1.6
 */
public class BCM_FIN_0004HTMLAction  extends HTMLActionSupport{
	private static final long serialVersionUID = 1L;
	/**
	 * BCM_FIN_0004HTMLAction 객체를 생성
	 */
	public BCM_FIN_0004HTMLAction(){}
	/**
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	BcmFin0004Event event = new BcmFin0004Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchArRouteRankListVOs((SearchArRouteRankListVO[])getVOs(request, SearchArRouteRankListVO.class,""));
			event.setSlanCd(request.getParameter("s_slan_cd"));
			event.setRlaneCd(request.getParameter("s_rlane_cd"));
			event.setDeltFlg(request.getParameter("s_delt_flg"));

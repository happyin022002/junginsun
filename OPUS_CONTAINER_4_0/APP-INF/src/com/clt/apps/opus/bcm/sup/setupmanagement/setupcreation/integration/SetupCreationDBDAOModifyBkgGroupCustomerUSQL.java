...
*@FileName : SetupCreationDBDAOModifyBkgGroupCustomerUSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class SetupCreationDBDAOModifyBkgGroupCustomerUSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 2011.02.22 정윤태 BKG EDI GROUP CUSTOMER 정보를 변경한다
	  * </pre>
	  */
	public SetupCreationDBDAOModifyBkgGroupCustomerUSQL(){
		setQuery();
		...
		query.append("FileName : SetupCreationDBDAOModifyBkgGroupCustomerUSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		query.append("UPDATE BKG_EDI_GRP_CUST" ).append("\n"); 
		query.append("   SET BKG_CFM_FLG = @[bkg_cfm_flg]," ).append("\n"); 
		query.append("       BKG_CFM_AUTO_FLG = @[bkg_cfm_auto_flg]," ).append("\n"); 
		query.append("       BL_DRFT_AUTO_FLG = @[bl_drft_auto_flg]," ).append("\n"); 
		query.append("       BL_DRFT_FLG = @[bl_drft_flg]," ).append("\n"); 
		query.append("       CGO_TRAK_FLG = @[cgo_trc_svc_flg]," ).append("\n"); 
		query.append("       AN_FLG = @[an_flg]," ).append("\n"); 
		query.append("       ESVC_BL_TP_CD = @[esvc_bl_tp_cd]," ).append("\n"); 
		query.append("       BKG_CTRT_TP_CD = @[bkg_ctrt_div_cd]," ).append("\n"); 
		query.append("       DELT_FLG = @[ib_svc_flg]," ).append("\n"); 
		query.append("       UPD_USR_ID = @[upd_usr_id]," ).append("\n"); 
		query.append("       UPD_DT = SYSDATE," ).append("\n"); 
		query.append("       VT_CUST_OFC_CD = @[vt_cust_ofc_cd]" ).append("\n"); 
		query.append(" WHERE ESVC_GRP_CD = @[edi_grp_cd]" ).append("\n"); 
		query.append("   AND CO_CD = SUBSTR(@[co_div_cd],1,1)" ).append("\n"); 

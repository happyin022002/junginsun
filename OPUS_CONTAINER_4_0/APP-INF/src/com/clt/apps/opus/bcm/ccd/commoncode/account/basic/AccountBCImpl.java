...
*@LastVersion :
=========================================================*/
package com.clt.apps.opus.bcm.ccd.commoncode.account.basic;
 
import java.util.ArrayList;
import java.util.List;

import com.clt.apps.opus.bcm.ccd.commoncode.account.integration.AccountDBDAO;
...
public class AccountBCImpl extends BasicCommandSupport implements AccountBC {
	 ...
	 * @exception EventException
	 */
	public List<ChargeVO> searchChargeRqst(String rqstNo) throws EventException {
		try {
			return dbDao.searchChargeRqst(rqstNo);
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
	 ...
	 * @exception EventException
	 */
	public void manageChargeCode(ChargeVO[] chgVOs, String usrId) throws EventException{
		try {
		...
		//date adding
		if(addVoList.size() > 0) {
			dbDao.addChargeCode(addVoList);
		}
		
		//date changing
		if(modifyVoList.size() > 0) {
			dbDao.modifyChargeCode(modifyVoList);
		}
		} catch (DAOException ex) {
			log.error("err " + ex.toString(), ex);
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);

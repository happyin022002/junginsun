...
*@FileName : BCM_CCD_0010HTMLAction.java
...
public class BCM_CCD_0010HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0010HTMLAction 객체를 생성
	 */
	public BCM_CCD_0010HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0010Event event = new BcmCcd0010Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setRepCommodityVOS((RepCommodityVO[])getVOs(request, RepCommodityVO.class, "sheet1_"));
			
		} else if(command.isCommand(FormCommand.SEARCH)) {
			event.setRepCommodityVO((RepCommodityVO)getVO(request, RepCommodityVO.class));
			event.setRepCmdtCd(request.getParameter("rep_cmdt_cd"));
		}

...
*@FileName : BCM_CCD_0003HTMLAction.java
...
public class BCM_CCD_0003HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0003HTMLAction 객체를 생성
	 */
	public BCM_CCD_0003HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0003Event event = new BcmCcd0003Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setRepChargeVOS((RepChargeVO[])getVOs(request, RepChargeVO.class, "sheet1_"));
			
		} else if(command.isCommand(FormCommand.SEARCH)) {
			event.setRepChargeVO((RepChargeVO)getVO(request, RepChargeVO.class));
			event.setRepChgCd(request.getParameter("rep_chg_cd"));

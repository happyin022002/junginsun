...
*@FileName : BCM_CCD_0020HTMLAction.java
*@FileTitle : yard
*Open Issues :
*Change history :
...
public class BCM_CCD_0020HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0020HTMLAction 객체를 생성
	 */
	public BCM_CCD_0020HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0020Event event = new BcmCcd0020Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {
			event.setYdCd(request.getParameter("yd_cd"));
		}else if(command.isCommand(FormCommand.MULTI)) {
			event.setYdVO((YardVO)getVO(request, YardVO .class));
		}if(command.isCommand(FormCommand.SEARCH15)) {
...
//		}
			...
			event.setN1stVndrSeq(request.getParameter("n1st_vndr_seq"));
		}else if(command.isCommand(FormCommand.SEARCH03)) {
			event.setN2ndVndrSeq(request.getParameter("n2nd_vndr_seq"));
		}else if(command.isCommand(FormCommand.SEARCH04)) {
			event.setN3rdVndrSeq(request.getParameter("n3rd_vndr_seq"));
		}else if(command.isCommand(FormCommand.SEARCH05)) {
			event.setOfcCd(request.getParameter("ofc_cd"));
		}else if(command.isCommand(FormCommand.SEARCH06)) {
			event.setDmdtOfcCd(request.getParameter("dmdt_ofc_cd"));

...
*@FileName : BCM_CCD_0027HTMLAction.java
...
public class BCM_CCD_0027HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * BCM_CCD_0027HTMLAction 객체를 생성
	 */
	public BCM_CCD_0027HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		BcmCcd0027Event event = new BcmCcd0027Event();
		...
		if(command.isCommand(FormCommand.SEARCH02)) {
			...
			RLaneGroupVO rlanGroupVO = new RLaneGroupVO();
			
			rlanGroupVO.setRLaneDtlVOS((RLaneDtlVO[])getVOs(request, RLaneDtlVO .class, "sheet1_"));
			rlanGroupVO.setRLaneVO((RLaneVO)getVO(request, RLaneVO .class));
			RLaneDtlVO[] rLaneDtlVOs = rlanGroupVO.getRLaneDtlVOs();
			if(rLaneDtlVOs != null){
				for(int i=0; i < rLaneDtlVOs.length; i++){
					rLaneDtlVOs[i].setUserId(rlanGroupVO.getRLaneVO().getUserId());
					rLaneDtlVOs[i].setRlaneCd(rlanGroupVO.getRLaneVO().getRlaneCd());
				}
				rlanGroupVO.setRLaneDtlVOS(rLaneDtlVOs);
			}
			event.setRLaneGroupVO(rlanGroupVO);

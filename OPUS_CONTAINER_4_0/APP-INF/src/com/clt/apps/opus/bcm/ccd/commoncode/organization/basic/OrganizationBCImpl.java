...
import com.clt.apps.opus.bcm.ccd.commoncode.organization.integration.OrganizationDBDAO;
...
import com.clt.apps.opus.bcm.ccd.commoncode.organization.vo.OfficeIfVO;
...
public class OrganizationBCImpl extends BasicCommandSupport implements OrganizationBC {
	...
	public void manageOfcCode(OfficeVO ofcVO,SignOnUserAccount account) throws EventException {
			try {
				if(ofcVO != null){
					// check Invoice Prefix duplication
//					if(dbDao.isInvPrefixUnique(ofcVO) > 0) {
//						throw new EventException(new ErrorHandler("COM12115", new String[] {"Invoice Prefix"}).getMessage());
//						//throw new EventException(new ErrorHandler("CCD00004", new String[] {"Invoice Prefix is duplicated. Office Code do not have same Invoice Prefix"}).getMessage());
//					}
						if ( ofcVO.getIbflag().equals("I")){
							ofcVO.setCreUsrId(account.getUsr_id());
							ofcVO.setUpdUsrId(account.getUsr_id());
							ofcVO.setVndrCntCd(dbDao.searchVnderCntCd(ofcVO.getVndrCd()));
						...
						manageOfcIf(ofcVO, account.getUsr_id());
				}
		}catch (DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(), ex);
		} catch (Exception ex) {

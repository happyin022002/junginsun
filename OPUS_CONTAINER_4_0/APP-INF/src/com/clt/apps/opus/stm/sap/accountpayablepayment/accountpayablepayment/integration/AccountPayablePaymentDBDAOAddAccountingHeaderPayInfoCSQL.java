...
*@FileName : AccountPayablePaymentDBDAOAddAccountingHeaderPayInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayablePaymentDBDAOAddAccountingHeaderPayInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddAccountingHeaderPayInfo
	  * </pre>
	  */
	public AccountPayablePaymentDBDAOAddAccountingHeaderPayInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayablePaymentDBDAOAddAccountingHeaderPayInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , TO_CHAR((SELECT  SPD.ACCTG_DT " ).append("\n"); 
		query.append("                       FROM SAP_PAY_DTL SPD " ).append("\n"); 
		query.append("                       WHERE SPD.PAY_SEQ = SPH.PAY_SEQ " ).append("\n"); 
		query.append("                       AND   NVL(SPD.ACCTG_PST_FLG, 'N') <> 'Y'" ).append("\n"); 
		query.append("                       AND   SPD.PRNT_RVS_INV_PAY_SEQ IS NULL " ).append("\n"); 
		query.append("                       AND   ROWNUM = 1), 'YYYY/MM') AS ACCTG_PRD_NM" ).append("\n"); 
		query.append("            , (SELECT  SPD.ACCTG_DT " ).append("\n"); 
		query.append("               FROM SAP_PAY_DTL SPD " ).append("\n"); 
		query.append("               WHERE SPD.PAY_SEQ = SPH.PAY_SEQ " ).append("\n"); 
		query.append("               AND NVL(SPD.ACCTG_PST_FLG, 'N') <> 'Y'" ).append("\n"); 
		query.append("               AND     SPD.PRNT_RVS_INV_PAY_SEQ IS NULL " ).append("\n"); 
		query.append("               AND ROWNUM = 1)         AS ACCTG_DT" ).append("\n"); 

...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailPrepayRoundInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailPrepayRoundInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 선급금 정산 처리시 Line의 Activity Date의 의한 환율과 Apply 선급금 Header G/L date간의 동일 환율 적용시 전체 금액 대 남은 금액을 직접 계산시 Round 발생 내역에 대해서 추가 처리
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailPrepayRoundInfoCSQL(){
		setQuery();
		...
		params.put("prepay_round",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailPrepayRoundInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("               , DECODE(SIGN(REMAIN_LIA.REMAIN_LIA_AMT), -1, '40', '50') AS POST_KEY" ).append("\n"); 
		query.append("               , NULL                                AS TAX_ON_SALES" ).append("\n"); 
		query.append("               , ABS(REMAIN_LIA.REMAIN_LIA_AMT)      AS  LOCAL_AMOUNT" ).append("\n"); 
		...
		query.append("               , '7001000000'                        AS GL_ACCOUNT_NO" ).append("\n"); 
		...
		query.append("               , 'PREPAY(ROUND)'                     AS IF_ITEM_CATEGROY" ).append("\n"); 
		...
		query.append("               , (SELECT  SIH2.INV_SEQ, " ).append("\n"); 
		query.append("                          @[prepay_round] - NVL(SUM(ROUND((SID2.DTRB_AMT - SID2.GLO_ATTR_CTNT1) * NVL(SID2.DTRB_XCH_RT, SIH2.INV_XCH_RT), " ).append("\n"); 
		query.append("                                           (SELECT  DP_PRCS_KNT FROM MDM_CURRENCY MC WHERE MC.CURR_CD = @[functional_currency] AND DELT_FLG = 'N' AND ROWNUM=1))),0) AS REMAIN_LIA_AMT" ).append("\n"); 
		query.append("                  FROM    SAP_INV_DTL SID2, SAP_INV_HDR SIH2 " ).append("\n"); 
		query.append("                  WHERE   SIH2.INV_SEQ = SID2.INV_SEQ AND SID2.LINE_TP_LU_CD <> 'PREPAY' " ).append("\n"); 
		query.append("                  AND     SIH2.INV_SEQ = @[inv_seq]" ).append("\n"); 
		query.append("                  GROUP   BY SIH2.INV_SEQ) REMAIN_LIA" ).append("\n"); 
		...
		query.append("         AND     SID.ROWID = (SELECT  MIN(SID3.ROWID) FROM SAP_INV_DTL SID3 WHERE SID3.INV_SEQ = SIH.INV_SEQ AND SID3.LINE_TP_LU_CD <> 'PREPAY')" ).append("\n"); 
		query.append("         AND     SIH.INV_SEQ = REMAIN_LIA.INV_SEQ" ).append("\n"); 
		query.append("         AND     REMAIN_LIA.REMAIN_LIA_AMT <> 0" ).append("\n"); 

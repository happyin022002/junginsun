...
*@FileName : AccountPayablePaymentDBDAOAddAccountingDetailInvHeaderInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayablePaymentDBDAOAddAccountingDetailInvHeaderInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddAccountingDetailInvHeaderInfo
	  * </pre>
	  */
	public AccountPayablePaymentDBDAOAddAccountingDetailInvHeaderInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayablePaymentDBDAOAddAccountingDetailInvHeaderInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , NVL((SELECT MAX(SAED.ACCTG_EVNT_LINE_NO) FROM SAP_ACCTG_EVNT_DTL SAED WHERE SAED.ACCTG_HDR_SEQ = @[accounting_header_id]),0) + 1 AS ACCTG_EVNT_LINE_NO" ).append("\n"); 
		query.append("            , 'LIABILITY'                      AS ACCTG_LINE_TP_CD" ).append("\n"); 
		...
		query.append("            , DECODE(SIGN(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT)), -1, NVL(SIH.INV_CXL_AMT, SIH.INV_AMT) * (-1), NULL)                  AS INP_DR_AMT" ).append("\n"); 
		query.append("            , DECODE(SIGN(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT)), -1, NULL, NVL(SIH.INV_CXL_AMT, SIH.INV_AMT))                         AS INP_CR_AMT" ).append("\n"); 
		query.append("            , DECODE(SIGN(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT)), -1, ROUND(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT) * (-1) * NVL(SIH.INV_XCH_RT, 1), " ).append("\n"); 
		query.append("                                                                  (SELECT DP_PRCS_KNT FROM MDM_CURRENCY MC WHERE MC.CURR_CD = @[functional_currency] AND DELT_FLG = 'N' AND ROWNUM=1)), NULL) AS ACCT_DR_AMT" ).append("\n"); 
		query.append("            , DECODE(SIGN(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT)), -1, NULL, ROUND(NVL(SIH.INV_CXL_AMT, SIH.INV_AMT) * NVL(SIH.INV_XCH_RT, 1), " ).append("\n"); 
		query.append("                                                                  (SELECT DP_PRCS_KNT FROM MDM_CURRENCY MC WHERE MC.CURR_CD = @[functional_currency] AND DELT_FLG = 'N' AND ROWNUM=1))) AS ACCT_CR_AMT" ).append("\n"); 
		...
		query.append("            , 'LIABILITY'                      AS ATTR_CTNT10" ).append("\n"); 
		...
		query.append("            , SIH.LIAB_CD_CMB_SEQ              AS ACCT_CD_CMB_SEQ" ).append("\n"); 

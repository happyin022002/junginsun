...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPUpstreamStandardJPHeaderPartialInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPUpstreamStandardJPHeaderPartialInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * Upstream module interfaced standard slip에 대한 Partially apply되는 경우 Partial 부분만 Liability로 처리하여 SAKURA로 interface
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPUpstreamStandardJPHeaderPartialInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPUpstreamStandardJPHeaderPartialInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                      , CASE WHEN SUM(DTRB_AMT) < 0 THEN '31'" ).append("\n"); 
		query.append("                            WHEN SUM(DTRB_AMT) >= 0 THEN '21'" ).append("\n"); 
		query.append("                            ELSE '21' END AS POST_KEY" ).append("\n"); 
		...
		query.append("                      , 'LIABILITY_UPSTREAM' AS IF_ITEM_CATEGROY" ).append("\n"); 
		query.append("                FROM    (" ).append("\n"); 
		...
		query.append("                               , ROUND(NVL(SID.GLO_ATTR_CTNT1, 0) * NVL(SID.DTRB_XCH_RT, SIH.INV_XCH_RT), " ).append("\n"); 
		query.append("                                        (SELECT  DP_PRCS_KNT FROM MDM_CURRENCY MC WHERE MC.CURR_CD = @[functional_currency] AND DELT_FLG = 'N' AND ROWNUM=1)) AS DTRB_FUNC_AMT " ).append("\n"); 
		query.append("                               , NVL(SID.GLO_ATTR_CTNT1, 0)          AS DTRB_AMT" ).append("\n"); 
		...
		query.append("                               , '6038001000'                        AS ALTERNATIVE_ACCOUNT_NO -- 2015.01.19 하드코딩  2014.10.23 변경  --- VTM의 G/L Account 처리 MA.MODI_ACCT_CD  " ).append("\n"); 
		query.append("                               , NVL((SELECT MT.MODI_COST_CTR_CD FROM MDM_TRADE MT WHERE MT.TRD_CD = SID.ATTR_CTNT7 AND ROWNUM = 1)," ).append("\n"); 
		query.append("                                     NVL((SELECT DECODE(MVSL.VSL_SLAN_CD, 'COM', NULL, 'CNT', NULL, MVSL.MODI_COST_CTR_CD) FROM MDM_VSL_SVC_LANE MVSL WHERE MVSL.VSL_SLAN_CD = SID.ATTR_CTNT14), " ).append("\n"); 
		query.append("                                            NVL(DECODE((SELECT  SSCC.TGT_CD FROM SCO_STMT_CD_CONV SSCC WHERE SSCC.CONV_TP_CD = 'GL ACCT OF PROFIT CENTER'" ).append("\n"); 
		...
		query.append("                               , '01'                                AS BIZ_PARTNER_REF_KEY2 " ).append("\n"); 
		query.append("                               , NULL                                AS LINE_ITEM_REF_KEY   -- 2015.01.26 변경 " ).append("\n"); 
		...
		query.append("                         AND     NVL(SID.GLO_ATTR_CTNT1, 0) <> 0" ).append("\n"); 

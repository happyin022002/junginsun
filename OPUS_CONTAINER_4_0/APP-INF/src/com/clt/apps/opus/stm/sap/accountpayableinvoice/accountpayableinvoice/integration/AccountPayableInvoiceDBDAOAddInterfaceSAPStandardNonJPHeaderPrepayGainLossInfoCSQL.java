...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPHeaderPrepayGainLossInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPHeaderPrepayGainLossInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * 선급금 정산 처리 후 남은 Liability에 대해서 Apply된 Row의 환율과 Matching되는 Line의 환율이 다를 경우 Gain & Loss 발생하는 부분에 대해서 추가 처리
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPHeaderPrepayGainLossInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPHeaderPrepayGainLossInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("               , ABS(AMT.ITEM_AMT + AMT.REMAIN_AMT - AMT.REMAIN_LIABILITY - AMT.APPLY_AMT) AS  LOCAL_AMOUNT" ).append("\n"); 
		...
		query.append("               , 'LIA_PREPAY(GAIN/LOSS)'             AS IF_ITEM_CATEGROY" ).append("\n"); 
		...
		query.append("                         , SUM(A.REMAIN_LIABILITY) AS REMAIN_LIABILITY" ).append("\n"); 
		query.append("                         , DECODE(SIGN(SUM(A.ITEM_AMT) + SUM(A.REMAIN_AMT) + SUM(A.REMAIN_LIABILITY) - SUM(A.APPLY_AMT)), -1, '40', '50') AS POST_KEY" ).append("\n"); 
		query.append("                  FROM     (" ).append("\n"); 
		query.append("                           SELECT  SIH.INV_SEQ" ).append("\n"); 
		query.append("                                 , DECODE(SID.LINE_TP_LU_CD, 'PREPAY', 0, NVL(SID.DTRB_FUNC_AMT, SID.DTRB_AMT)) AS ITEM_AMT" ).append("\n"); 
		...
		query.append("                                 , DECODE(SID.LINE_TP_LU_CD, 'PREPAY', 0, (SELECT ROUND((SID2.DTRB_AMT - NVL(SID2.GLO_ATTR_CTNT1, 0)) * NVL(SID2.DTRB_XCH_RT, 1)," ).append("\n"); 
		...
		query.append("                                                                           FROM   SAP_INV_DTL SID2 " ).append("\n"); 
		query.append("                                                                           WHERE  SID.INV_DTRB_SEQ = SID2.INV_DTRB_SEQ AND SID2.LINE_TP_LU_CD <> 'PREPAY')) AS REMAIN_LIABILITY" ).append("\n"); 
		query.append("                           FROM    SAP_INV_HDR SIH" ).append("\n"); 
		query.append("                                 , SAP_INV_DTL SID" ).append("\n"); 
		...
		query.append("         AND     AMT.ITEM_AMT + AMT.REMAIN_AMT - AMT.REMAIN_LIABILITY - AMT.APPLY_AMT <> 0" ).append("\n"); 

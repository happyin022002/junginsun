...
*@FileName : AccountPayablePaymentDBDAOAddAccountingDetailInvLineGainLossInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayablePaymentDBDAOAddAccountingDetailInvLineGainLossInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AP Invoice accounting처리시 Ex. Rate을 Line단위에서 처리하는 경우 환차손익이 발생할 수 있는데 발생된 환차손익 처리
	  * </pre>
	  */
	public AccountPayablePaymentDBDAOAddAccountingDetailInvLineGainLossInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayablePaymentDBDAOAddAccountingDetailInvLineGainLossInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("                  , (SELECT  MAX(SID2.DTRB_LINE_NO) FROM SAP_INV_DTL SID2 WHERE SID2.INV_SEQ = @[inv_seq] AND SID2.LINE_TP_LU_CD <> 'PREPAY' AND NVL(SID2.ACCTG_PST_FLG, 'N') <> 'Y'" ).append("\n"); 
		query.append("                     AND     NVL(SID2.RVS_FLG, 'N') <> 'Y' AND SID2.PRNT_RVS_DTRB_SEQ IS NULL AND SID2.DTRB_AMT <> 0) + 1 AS ACCTG_EVNT_LINE_NO" ).append("\n"); 

...
*@FileName : AccountPayableInvoiceDBDAOAddApplyedPrepaymentLineInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddApplyedPrepaymentLineInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * addApplyedPrepaymentLineInfo
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddApplyedPrepaymentLineInfoCSQL(){
		setQuery();
		...
		params.put("apply_gl_date",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("prepayment_inv_seq",new String[]{arrTmp[0],arrTmp[1]});
		...
		params.put("prepayment_line_no",new String[]{arrTmp[0],arrTmp[1]});
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddApplyedPrepaymentLineInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append(")" ).append("\n"); 
		query.append("SELECT TO_DATE(REPLACE(@[apply_gl_date],'-',''), 'YYYYMMDD')" ).append("\n"); 
		...
		query.append("     , TO_CHAR(TO_DATE( REPLACE(@[apply_gl_date],'-',''), 'YYYYMMDD') , 'YYYYMM') AS EFF_YRMON" ).append("\n"); 
		query.append("     , @[apply_amount] * (-1) AS DTRB_AMT" ).append("\n"); 
		query.append("     , DECODE((SELECT SIH.INV_XCH_RT FROM SAP_INV_HDR SIH WHERE SIH.INV_SEQ = @[prepayment_inv_seq]), NULL, ( @[apply_amount] * (-1) )," ).append("\n"); 
		query.append("               ROUND(@[apply_amount]  * (SELECT SIH.INV_XCH_RT FROM SAP_INV_HDR SIH WHERE SIH.INV_SEQ = @[prepayment_inv_seq]), " ).append("\n"); 
		query.append("                            (SELECT NVL(MC.DP_PRCS_KNT, 0) FROM MDM_CURRENCY MC WHERE MC.CURR_CD = @[functional_currency])) * (-1)) AS DTRB_FUNC_AMT" ).append("\n"); 
		...
		query.append("     , SID.RVS_FLG" ).append("\n"); 
		query.append("     , SID.DTRB_XCH_DT" ).append("\n"); 
		query.append("     , SID.DTRB_XCH_RT" ).append("\n"); 
		query.append("     , SID.DTRB_XCH_RT_TP_CD" ).append("\n"); 
		query.append("     , NVL((SELECT SID1.ATTR_CTNT1 FROM SAP_INV_DTL SID1 WHERE SID1.INV_SEQ = @[standard_inv_seq] AND SID1.LINE_TP_LU_CD NOT IN ('PREPAY', 'TAX') AND SID1.ATTR_CTNT1 IS NOT NULL AND ROWNUM = 1), SID.ATTR_CTNT1) AS ATTR_CTNT1" ).append("\n"); 
		...
		query.append("     , NULL INV_RND_AMT" ).append("\n"); 
		query.append("     , @[prepayment_inv_seq]" ).append("\n"); 
		query.append("     , @[prepayment_line_no]" ).append("\n"); 
		query.append("FROM   SAP_INV_DTL SID" ).append("\n"); 
		query.append("WHERE  SID.INV_SEQ = @[prepayment_inv_seq]" ).append("\n"); 
		query.append("AND    SID.DTRB_LINE_NO = @[prepayment_line_no]" ).append("\n"); 

...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddInterfaceSAPStandardNonJPDetailInfo
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardNonJPDetailInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD = 'TAX' AND SID.DTRB_AMT < 0 THEN '11'           ----- 2014/10/06 PREPAY 하드코딩 부분 모두 제외 처리" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND MA.ACCT_CD IN ('954116', '954115', '954117') AND SID.DTRB_AMT < 0 THEN '11'  --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND MA.ACCT_CD IN ('954116', '954115', '954117') AND SID.DTRB_AMT >= 0 THEN '01' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') AND SID.DTRB_AMT < 0 THEN '50' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') AND SID.DTRB_AMT >= 0 THEN '40' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		...
		query.append("                   ELSE DECODE(SIH.INV_CURR_CD, @[functional_currency], NULL, ABS(NVL(SID.DTRB_FUNC_AMT, 0))) END AS LOCAL_AMOUNT  -- 2014.10.27 금액 수정" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD <> 'PREPAY' AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') THEN SUBSTR(SIH.INV_NO, 1, 16)" ).append("\n"); 
		...
		query.append("                                      AND     SSCC.SRC_CD = MA.MODI_ACCT_CD AND SSCC.USE_FLG = 'Y' AND SSCC.DELT_FLG = 'N' AND ROWNUM = 1)), --2015.02.11 수정 " ).append("\n"); 
		query.append("                                'A106') ) " ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' THEN NULL ELSE NULL END  --2015.02.09 --2015.01.28 Profit Center 수정" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD <> '211691' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') THEN MA.MODI_ACCT_CD   --- 2015.10.01 REV(Local) 계정 추가  " ).append("\n"); 
		...
		query.append("                   WHEN SIH.AP_INV_SRC_CD <> 'AR' AND MA.ACCT_CD IN ('954113', '954116', '954115', '954117', '954114') THEN '8705300000' --Overseas Income Tax(P123)" ).append("\n"); 
		query.append("                   ELSE NULL END -- 2015.01.19 수정  2014.10.23 수정 --- VTM의 G/L Account 처리" ).append("\n"); 
		query.append("                                                  AS GL_ACCOUNT_NO" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR  MA.ACCT_CD = '211691' OR MA.ACCT_CD IN ('954116', '954115', '954117') THEN  --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                        MO.MODI_AGN_CD -- 2015.01.08 변경 --- Office SAP-ID  (Code Conversion)" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'PREPAY' THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0')  --2015.03.30 수정" ).append("\n"); 
		...
		query.append("                                       AND     SSCC.SRC_CD = MA.MODI_ACCT_CD AND SSCC.USE_FLG = 'Y' AND SSCC.DELT_FLG = 'N' AND ROWNUM = 1)), --2015.02.11 수정 " ).append("\n"); 
		query.append("                                'A106') ) " ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD LIKE '954%' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') THEN 'A106' ELSE NULL END --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                                                  AS PROFIT_CENTER " ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR  MA.ACCT_CD = '211691' OR MA.ACCT_CD IN ('954116', '954115', '954117') THEN '5360000000' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN '5400269050'" ).append("\n"); 
		query.append("                   --WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' THEN MA.MODI_ACCT_CD  -- 2014.10.23 수정 " ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR  MA.ACCT_CD = '211691' OR MA.ACCT_CD IN ('954116', '954115', '954117') THEN 'A106'  --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'PREPAY' THEN NVL((SELECT MVSL.MODI_COST_CTR_CD FROM MDM_VSL_SVC_LANE MVSL WHERE MVSL.VSL_SLAN_CD = SID.ATTR_CTNT14), " ).append("\n"); 
		query.append("                                                                NVL(DECODE((SELECT  SSCC.TGT_CD FROM SCO_STMT_CD_CONV SSCC WHERE SSCC.CONV_TP_CD = 'GL ACCT OF PROFIT CENTER'" ).append("\n"); 
		query.append("                                                                            AND     SSCC.SRC_CD = MA.MODI_ACCT_CD AND SSCC.USE_FLG = 'Y' AND SSCC.DELT_FLG = 'N' AND ROWNUM = 1), 'ZH', MO.MODI_COST_CTR_CD," ).append("\n"); 
		query.append("                                                                           (SELECT  SSCC.TGT_CD FROM SCO_STMT_CD_CONV SSCC WHERE SSCC.CONV_TP_CD = 'GL ACCT OF PROFIT CENTER'" ).append("\n"); 
		query.append("                                                                            AND     SSCC.SRC_CD = MA.MODI_ACCT_CD AND SSCC.USE_FLG = 'Y' AND SSCC.DELT_FLG = 'N' AND ROWNUM = 1)), --2015.02.11 수정 " ).append("\n"); 
		query.append("                                                                     'A106') ) --2015.02.09 수정 --2015.01.28 수정" ).append("\n"); 
		query.append("                   ELSE NULL END                  AS BIZ_PARTNER_REF_KEY1 -- Profit Center (Code Conversion)" ).append("\n"); 
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN 'GT' ELSE NULL END AS BIZ_PARTNER_REF_KEY2 -- 2015.01.19 추가 " ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD <> '211691' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0') --- 2015.10.01 REV(Local) 계정 추가 " ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN MO.MODI_AGN_CD -- 2015.01.19 변경 " ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR  MA.ACCT_CD = '211691' OR MA.ACCT_CD IN ('954116', '954115', '954117') THEN MO.MODI_AGN_CD --- 2015.10.01 REV(Local) 계정 추가 -- 2015.01.08 변경 --- Office SAP-ID  (Code Conversion)" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'PREPAY' THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0')  --2015.03.30 수정 " ).append("\n"); 
		...
		query.append("                   WHEN SIH.AP_INV_SRC_CD = 'AR' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' THEN " ).append("\n"); 
		query.append("                        SUBSTR(REPLACE(REPLACE(SID.ATTR_CTNT11, '/', ''), '-', ''), 1, 8) ELSE NULL END --2015.02.05 --2014.10.29 --2014.10.23 변경 " ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD LIKE '954%' AND MA.ACCT_CD IN ('954116', '954115', '954117') THEN 'ITEM(CLR)_REV(LCL)' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD LIKE '954%' AND MA.ACCT_CD NOT IN ('954116', '954115', '954117') THEN 'ITEM(CLEAR)' --- 2015.10.01 REV(Local) 계정 추가" ).append("\n"); 

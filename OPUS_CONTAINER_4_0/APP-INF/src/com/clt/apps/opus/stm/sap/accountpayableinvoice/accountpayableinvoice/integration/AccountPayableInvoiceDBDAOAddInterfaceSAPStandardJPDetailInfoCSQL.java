...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardJPDetailInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardJPDetailInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddInterfaceSAPStandardJPDetailInfo
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardJPDetailInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardJPDetailInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , DECODE(SID.LINE_TP_LU_CD, 'TAX', 'X', NULL) AS RECORD_ID" ).append("\n"); 
		...
		query.append("            , CASE WHEN (SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR MA.ACCT_CD = '211691') AND SID.DTRB_AMT < 0 THEN '50'   ----- 2014/10/06 PREPAY 하드코딩 부분 모두 제외 처리" ).append("\n"); 
		query.append("                   WHEN (SID.LINE_TP_LU_CD IN ('TAX', 'WITHHOLDING TAX') OR MA.ACCT_CD = '211691') AND SID.DTRB_AMT >= 0 THEN '40'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND SID.DTRB_AMT < 0 THEN '50'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM <> 'REFUND' AND SID.DTRB_AMT >= 0 THEN '40'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' AND SID.DTRB_AMT < 0 THEN '11'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' AND SID.DTRB_AMT >= 0 THEN '01'" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND SID.DTRB_VAT_CD IS NOT NULL AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691'" ).append("\n"); 
		query.append("                        AND NVL((SELECT  SLD.ATTR_CTNT2 FROM SCO_LU_HDR SLH, SCO_LU_DTL SLD" ).append("\n"); 
		query.append("                                 WHERE   SLH.LU_TP_CD = SLD.LU_TP_CD AND SLH.LU_TP_CD = 'AP TAX CODE' AND NVL(SLD.ENBL_FLG, 'Y') = 'Y'" ).append("\n"); 
		query.append("                                 AND     NVL(SLD.LU_ST_DT, SYSDATE) >= SYSDATE AND SLH.LU_APPL_CD = 'SAP'AND SLD.LU_CD = SID.DTRB_VAT_CD AND ROWNUM = 1), 'N') = 'Y' THEN 'E3'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND SID.DTRB_VAT_CD IS NULL AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' THEN 'F0' --2015.02.04 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND SID.DTRB_VAT_CD IS NOT NULL AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691'" ).append("\n"); 
		query.append("                        AND NVL((SELECT  SLD.ATTR_CTNT2 FROM SCO_LU_HDR SLH, SCO_LU_DTL SLD" ).append("\n"); 
		query.append("                                 WHERE   SLH.LU_TP_CD = SLD.LU_TP_CD AND SLH.LU_TP_CD = 'AP TAX CODE' AND NVL(SLD.ENBL_FLG, 'Y') = 'Y'" ).append("\n"); 
		query.append("                                 AND     NVL(SLD.LU_ST_DT, SYSDATE) >= SYSDATE AND SLH.LU_APPL_CD = 'SAP'AND SLD.LU_CD = SID.DTRB_VAT_CD AND ROWNUM = 1), 'N') = 'N' THEN 'F0' --2015.02.04 추가" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'TAX' AND SID.DTRB_AMT <> 0 THEN 'E3' " ).append("\n"); 
		query.append("                   WHEN (SID.LINE_TP_LU_CD IN ( 'WITHHOLDING TAX' ) OR MA.ACCT_CD = '211691') AND SID.DTRB_AMT <> 0 THEN 'F0' ELSE NULL END   --- Tax Code (Business Logic)" ).append("\n"); 
		query.append("                                                  AS TAX_ON_SALES" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD = 'TAX' THEN NULL" ).append("\n"); 
		query.append("                   WHEN SIH.AP_INV_SRC_CD = 'AR' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' THEN DECODE(SIH.INV_CURR_CD, @[functional_currency], NULL, ABS(NVL(SID.DTRB_FUNC_AMT, 0)))" ).append("\n"); 
		query.append("              ELSE DECODE(SIH.INV_CURR_CD, @[functional_currency], NULL, ABS(NVL(SID.DTRB_FUNC_AMT, 0))) END  -- 2014.10.27 금액수정" ).append("\n"); 
		query.append("                                                  AS LOCAL_AMOUNT" ).append("\n"); 
		query.append("            , CASE WHEN SIH.AP_INV_SRC_CD = 'AR' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' THEN TO_CHAR(ABS(NVL(SID.DTRB_AMT, 0))) " ).append("\n"); 
		query.append("                   ELSE DECODE(SID.LINE_TP_LU_CD, 'TAX', NULL, ABS(NVL(SID.DTRB_AMT, 0))) END AS DOCUMENT_AMOUNT" ).append("\n"); 
		query.append("            , DECODE(SID.LINE_TP_LU_CD, 'TAX', DECODE(SIH.INV_CURR_CD, @[functional_currency], NULL, ABS(NVL(SID.DTRB_FUNC_AMT, 0))) --2014.10.27 금액수정" ).append("\n"); 
		query.append("                                      , NULL)     AS LOCAL_TAX_AMOUNT" ).append("\n"); 
		query.append("            , DECODE(SID.LINE_TP_LU_CD, 'TAX', ABS(NVL(SID.DTRB_AMT, 0)), NULL) AS DOCUMENT_TAX_AMOUNT" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT', 'WITHHOLDING TAX') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') THEN SUBSTR(SIH.INV_NO, 1, 16)" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'TAX' THEN NULL ELSE SUBSTR(SIH.INV_NO, 1, 16) END AS ASSIGNMENT_NUMBER" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' --2015.01.19 수정" ).append("\n"); 
		...
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN SIH.OFC_CD || ':' || CU.USR_NM ELSE NULL END --2014.10.17 USER 정보 추가" ).append("\n"); 
		...
		query.append("                   ELSE NULL END                  AS PLANNING_DATE  --2015.01.19 수정" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' THEN " ).append("\n"); 
		...
		query.append("                                    'A106') ) ) --2015.02.09 수정 --2015.01.28 수정 -- Profit Center(Code Conversion)" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' THEN NULL ELSE NULL END  --2015.01.26 수정" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD <> '211691' THEN MA.MODI_ACCT_CD " ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM = 'ARAPOFFSET' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN '5400269051' -- 2015.01.19 수정  2014.10.23 수정 --- VTM의 G/L Account 처리" ).append("\n"); 
		...
		query.append("                   WHEN (SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691') THEN '6480204000' ELSE NULL END " ).append("\n"); 
		...
		query.append("                                                    THEN NVL((SELECT SR.RCT_DPS_DT FROM SAR_RECEIPT SR WHERE SR.RCT_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1),  " ).append("\n"); 
		query.append("                                                              (SELECT SAH.ADJ_DT FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1))" ).append("\n"); 
		query.append("                   ELSE NULL END                  AS CALCULATE_DUE_DATE  -- 2015.01.19 추가" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR  MA.ACCT_CD = '211691' THEN 'Z002'  ELSE NULL END" ).append("\n"); 
		query.append("                                                  AS PROFIT_CENTER " ).append("\n"); 
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN '5400269050'" ).append("\n"); 
		query.append("                   --WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' THEN MA.MODI_ACCT_CD -- 2014.10.23 변경" ).append("\n"); 
		query.append("                   ELSE NULL END --- VTM의 G/L Account 처리" ).append("\n"); 
		query.append("                                                  AS ALTERNATIVE_ACCOUNT_NO" ).append("\n"); 
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y'" ).append("\n"); 
		query.append("                                                    THEN 'A106'  --2015.04.20 수정" ).append("\n"); 
		query.append("                   ELSE NULL END                  AS BIZ_PARTNER_REF_KEY1 -- Profit Center (Code Conversion)" ).append("\n"); 
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN '01' ELSE NULL END AS BIZ_PARTNER_REF_KEY2 -- 2015.01.19 추가" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD <> '211691' THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0') --2015.03.30 수정" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691' THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0') --2015.03.30 수정" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y'" ).append("\n"); 
		query.append("                        AND NVL((SELECT SR.RCT_CUST_CNT_CD FROM SAR_RECEIPT SR WHERE SR.RCT_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1)," ).append("\n"); 
		query.append("                                (SELECT SAH.BIL_TO_CUST_CNT_CD FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1)) = 'TB' " ).append("\n"); 
		query.append("                                 THEN NVL((SELECT 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0') " ).append("\n"); 
		...
		query.append("                   ELSE NULL END                  AS CONTRACT_NUMBER --- Office SAP-ID  (Code Conversion)" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD = 'TAX' THEN NULL ELSE 'Z' END AS CONTRACT_TYPE  -- WHEN SIH.ATTR_CATE_NM = 'ARAPOFFSET' THEN NULL" ).append("\n"); 
		query.append("            , NULL                                AS PAYMENT_REASON" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691' THEN 'C130' ELSE NULL END AS CLASSIFICATION --2014.10.17 Withholding인 경우 상수값 추가" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' THEN " ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' AND SUBSTR(SLCC.SGM_CTNT6, 1, 4) NOT IN ('CFDR', '0000', 'CNTC', 'COMC') THEN " ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691' THEN 'ITEM(PL)'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD LIKE '954%' THEN 'ITEM(CLEAR)'" ).append("\n"); 
		query.append("                   WHEN MA.ACCT_CD = '211691' THEN 'WITHHOLDING TAX(ITEM)'" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' THEN 'ITEM(REFUND)'" ).append("\n"); 
		...
		query.append("            , MDM_ORGANIZATION MO --2015.01.19 추가" ).append("\n"); 
		...
		query.append("      AND     SIH.OFC_CD = MO.OFC_CD --2015.01.19 추가" ).append("\n"); 
		...
		query.append("      AND     SID.LINE_TP_LU_CD <> 'PREPAY'   -- 2014/10/06 선급금 정산 Line 제외" ).append("\n"); 

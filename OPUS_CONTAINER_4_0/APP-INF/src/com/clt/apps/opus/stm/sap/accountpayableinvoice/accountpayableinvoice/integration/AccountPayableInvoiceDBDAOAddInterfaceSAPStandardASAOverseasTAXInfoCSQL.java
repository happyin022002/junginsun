...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASAOverseasTAXInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASAOverseasTAXInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * ASA 전표 중 TAX인 경우에는 해당 Item Line과 별도로 분리하여 해당 Tax Line 생성 처리
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASAOverseasTAXInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASAOverseasTAXInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , CASE WHEN NVL(AMT.DIFF_AMT, 0) <> 0 THEN " ).append("\n"); 
		query.append("                   (SELECT  TO_CHAR((TO_DATE(SAM.ASA_PRD_TO_DT, 'YYYYMMDD') + NVL(MO1.ASA_CR_TERM_DYS, 0)), 'YYYYMMDD') " ).append("\n"); 
		query.append("                    FROM    SAR_ASA_MST SAM, MDM_ORGANIZATION MO1 WHERE SAM.OFC_CD = MO1.OFC_CD AND SAM.ASA_NO = SIH.ATTR_CTNT2 AND ROWNUM = 1)" ).append("\n"); 
		...
		query.append("            , DECODE(SUBSTR(SLCC.SGM_CTNT4, 1, 1), '2', '35', '31') AS PAYMENT_REFERENCE" ).append("\n"); 
		...
		query.append("            , 'REV_TAX(OVERSEAS)'                 AS IF_ITEM_CATEGROY" ).append("\n"); 

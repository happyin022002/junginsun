...
public class STM_SCO_0420HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0013HTMLAction 객체를 생성
	 */
	public STM_SCO_0420HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	StmSco0420Event event = new StmSco0420Event();

    	if(command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.SEARCH02) || command.isCommand(FormCommand.SEARCH03)) {			
			event.setAccrualVerificationVO((AccrualVerificationVO)getVO(request, AccrualVerificationVO .class));
		} else if(command.isCommand(FormCommand.COMMAND01)){
			event.setAccrualVerificationVO((AccrualVerificationVO)getVO(request, AccrualVerificationVO.class)); //code 조회.
		}
    	event.setAttribute("key", request.getParameter("key")); //BackEndJob 으로 돌린 후 결과 및 상태 조회

...
*@FileName : AccountReceivableOutstandingDBDAOsearchPaymentRequestLetterSumRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountReceivableOutstandingDBDAOsearchPaymentRequestLetterSumRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * searchPaymentRequestLetterSum
	  * </pre>
	  */
	public AccountReceivableOutstandingDBDAOsearchPaymentRequestLetterSumRSQL(){
		setQuery();
		...
		query.append("FileName : AccountReceivableOutstandingDBDAOsearchPaymentRequestLetterSumRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       RES.INV_CURR_CD AS SUM_CURR_CD" ).append("\n"); 
		query.append("     , SAR_GET_FMT_MASK_FNC(RES.INV_CURR_CD,SUM(RES.IN_BAL)) AS SUM_IN_BAL" ).append("\n"); 
		query.append("     , SAR_GET_FMT_MASK_FNC(RES.INV_CURR_CD,SUM(RES.OVER_BAL)) AS SUM_OVER_BAL" ).append("\n"); 
		query.append("     , SAR_GET_FMT_MASK_FNC(RES.INV_CURR_CD,SUM(RES.BAL_AMT)) AS SUM_TOT_LCL" ).append("\n"); 
		query.append("     , COUNT(RES.BAL_AMT) AS SUM_COUNT" ).append("\n"); 
		query.append("     ,(SELECT NVL(MC.DP_PRCS_KNT, '0') FROM MDM_CURRENCY MC WHERE MC.CURR_CD = RES.INV_CURR_CD) AS DP_PRCS_KNT " ).append("\n"); 
		query.append("FROM" ).append("\n"); 
		query.append("(     " ).append("\n"); 
		query.append("     SELECT INV_CURR_CD" ).append("\n"); 
		query.append("        , DECODE(SIGN(OVER_DUE),1,0,BAL_AMT) IN_BAL" ).append("\n"); 
		query.append("        , DECODE(SIGN(OVER_DUE),1,BAL_AMT,0) OVER_BAL" ).append("\n"); 
		query.append("        , BAL_AMT" ).append("\n"); 
		query.append("     FROM" ).append("\n"); 
		query.append("     (" ).append("\n"); 
		query.append("        SELECT " ).append("\n"); 
		query.append("            MAX(NVL(SOH.INV_CURR_CD, SOH.OFC_CURR_CD)) INV_CURR_CD                " ).append("\n"); 
		query.append("            , SUM(SAR_GET_CUR_AMT_FNC(NVL(SOH.INV_CURR_CD, SOH.OFC_CURR_CD), SOD.BAL_AMT * DECODE(SOH.INV_CURR_CD, 'USD', SOD.USD_XCH_RT, SOD.LOCL_XCH_RT))) BAL_AMT" ).append("\n"); 
		query.append("            , TRUNC(SYSDATE) - TRUNC(TO_DATE(MAX(SOH.DUE_DT),'YYYYMMDD')) OVER_DUE" ).append("\n"); 
		...
		query.append("        #end   " ).append("\n"); 
		query.append("    #end" ).append("\n"); 
		query.append("        GROUP BY SOH.RHQ_CD, SOH.CLT_OFC_CD ,SOH.BL_NO ,SOH.INV_NO, SOH.INV_CURR_CD, SOH.BIL_TO_CUST_CNT_CD, SOH.BIL_TO_CUST_SEQ, SOH.OTS_TP_CD, SOH.OTS_OFC_CD" ).append("\n"); 
		query.append("#if ( ${ots_opy} != 'OTS')        " ).append("\n"); 
		query.append("		UNION ALL" ).append("\n"); 
		query.append("        SELECT " ).append("\n"); 
		query.append("            SR.RCT_CURR_CD INV_CURR_CD   " ).append("\n"); 
		query.append("            , TO_NUMBER(SAR_GET_CUR_AMT_FNC(SR.RCT_CURR_CD, (SR.BAL_RCT_AMT * -1))) BAL_AMT" ).append("\n"); 
		...
		query.append("GROUP BY INV_CURR_CD" ).append("\n"); 

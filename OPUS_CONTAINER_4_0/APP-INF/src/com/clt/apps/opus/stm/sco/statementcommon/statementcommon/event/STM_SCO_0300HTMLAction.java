...
public class STM_SCO_0300HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0013HTMLAction 객체를 생성
	 */
	public STM_SCO_0300HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	StmSco0300Event event = new StmSco0300Event();
		

    	if(command.isCommand(FormCommand.SEARCH)) {			
			
			String luTpCd   = JSPUtil.getParameter(request, "lu_tp_cd" , "");
			String enblFlg  = JSPUtil.getParameter(request, "enbl_flg" , "");
			
			event.setLuTpCd(luTpCd);
			event.setEnblFlg(enblFlg);
			
		} else if(command.isCommand(FormCommand.MULTI)) {
			event.setLuTpCd(JSPUtil.getParameter(request, "lu_tp_cd" , ""));
			event.setScoStmtCdConvVOs((ScoStmtCdConvVO[])getVOs(request, ScoStmtCdConvVO.class, "sheet1_"));
		}

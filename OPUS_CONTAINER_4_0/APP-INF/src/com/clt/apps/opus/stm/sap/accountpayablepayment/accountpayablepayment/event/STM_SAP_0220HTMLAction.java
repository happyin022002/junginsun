...
*@FileName : STM_SAP_0220HTMLAction.java
*@FileTitle : 
*Open Issues :
*Change history :
*@LastModifyDate : 
...
public class STM_SAP_0220HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0013HTMLAction 객체를 생성
	 */
	public STM_SAP_0220HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	StmSap0220Event event = new StmSap0220Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {								
			event.setAccountingCondVO((AccountingCondVO)getVO(request, AccountingCondVO.class));
		} else if(command.isCommand(FormCommand.MULTI01)) {			
			event.setFunctionalCurrency(request.getParameter("functional_currency"));	
			event.setCapturePeriod(request.getParameter("capture_period"));	
			...
			event.setFunctionalCurrency(request.getParameter("functional_currency"));	
			event.setCapturePeriod(request.getParameter("capture_period"));	
			event.setaPAccountingListVOs((APAccountingListVO[])getVOs(request, APAccountingListVO.class, ""));

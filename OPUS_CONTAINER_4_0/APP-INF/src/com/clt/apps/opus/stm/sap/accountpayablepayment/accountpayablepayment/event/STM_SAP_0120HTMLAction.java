...
public class STM_SAP_0120HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;
	/**
	 * FNS_INV_0013HTMLAction 객체를 생성
	 */
	public STM_SAP_0120HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	StmSap0120Event event = new StmSap0120Event();
		

		if(command.isCommand(FormCommand.SEARCH)) {			
			BankBalanceByOfficeCondVO vo = (BankBalanceByOfficeCondVO)getVO(request, BankBalanceByOfficeCondVO.class);
			
			event.setBankBalanceByOfficeCondVO(vo);
			
		} else if(command.isCommand(FormCommand.MULTI)) { //recalculate
			event.setBankBalanceAdjustmentListVOs((BankBalanceAdjustmentListVO[])getVOs(request, BankBalanceAdjustmentListVO.class, "sheet1_"));
		}

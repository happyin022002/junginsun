...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASALineInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASALineInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddInterfaceSAPStandardASALineInfo
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASALineInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPStandardASALineInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT', 'WITHHOLDING TAX') AND SID.DTRB_AMT < 0 THEN '11'" ).append("\n"); 
		query.append("                   WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT', 'WITHHOLDING TAX') AND SID.DTRB_AMT >= 0 THEN '01'" ).append("\n"); 
		...
		query.append("            , SIH.ATTR_CTNT2                      AS ASSIGNMENT_NUMBER" ).append("\n"); 
		...
		query.append("            , (SELECT  TO_CHAR((TO_DATE(SAM.ASA_PRD_TO_DT, 'YYYYMMDD') + NVL(MO1.ASA_CR_TERM_DYS, 0)), 'YYYYMMDD') " ).append("\n"); 
		query.append("               FROM    SAR_ASA_MST SAM, MDM_ORGANIZATION MO1 WHERE SAM.OFC_CD = MO1.OFC_CD AND SAM.ASA_NO = SIH.ATTR_CTNT2 AND ROWNUM = 1) AS CALCULATE_DUE_DATE" ).append("\n"); 
		...
		query.append("            , 'A106'                              AS BIZ_PARTNER_REF_KEY1 -- Profit Center (Code Conversion)" ).append("\n"); 
		...
		query.append("            , '42'                                AS PAYMENT_REFERENCE" ).append("\n"); 
		...
		query.append("            , 'REC'                               AS IF_ITEM_CATEGROY" ).append("\n"); 

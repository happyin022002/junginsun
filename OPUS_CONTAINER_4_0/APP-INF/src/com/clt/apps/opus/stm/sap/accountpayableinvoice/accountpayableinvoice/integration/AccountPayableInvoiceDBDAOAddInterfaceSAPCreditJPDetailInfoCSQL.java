...
*@FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPCreditJPDetailInfoCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddInterfaceSAPCreditJPDetailInfoCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AddInterfaceSAPCreditJPDetailInfo
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddInterfaceSAPCreditJPDetailInfoCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddInterfaceSAPCreditJPDetailInfoCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("            , 'U7'                                AS DOCUMENT_TYPE" ).append("\n"); 
		...
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM <> 'REFUND' AND SID.DTRB_AMT < 0 THEN '50'" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM <> 'REFUND' AND SID.DTRB_AMT >= 0 THEN '40'" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND SAH.ADJ_TP_CD = 'EX' AND ROWNUM = 1), 'N') = 'Y' AND SID.DTRB_AMT < 0 THEN '50'" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND SAH.ADJ_TP_CD = 'EX' AND ROWNUM = 1), 'N') = 'Y' AND SID.DTRB_AMT >= 0 THEN '40'" ).append("\n"); 
		...
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' AND SID.DTRB_AMT < 0 THEN '50'" ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND (SAM.LEGR_XCH_DIFF_INCM_ACCT_CD = SLCC.SGM_CTNT4 OR SAM.LEGR_XCH_DIFF_LSS_ACCT_CD = SLCC.SGM_CTNT4) AND ROWNUM = 1),'N') = 'Y' AND SID.DTRB_AMT >= 0 THEN '40'" ).append("\n"); 
		...
		query.append("                   WHEN (SID.LINE_TP_LU_CD IN ( 'WITHHOLDING TAX' ) OR MA.ACCT_CD = '211691') AND SID.DTRB_AMT <> 0 THEN 'F0' ELSE NULL END   --- Tax Code (Business Logic) --2015.02.04 수정" ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691'" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'MISCELLANEOUS' THEN NULL ELSE NULL END --2015.01.26 수정" ).append("\n"); 
		...
		query.append("                   WHEN (SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691') THEN '6480204000' ELSE NULL END -- 2014.10.23 문자상수값 변경" ).append("\n"); 
		...
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' " ).append("\n"); 
		query.append("                        AND NVL((SELECT  'Y' FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND SAH.ADJ_TP_CD = 'EX' AND ROWNUM = 1), 'N') = 'N' THEN 'A106'  --2015.04.20 수정 " ).append("\n"); 
		query.append("                   ELSE NULL END                  AS BIZ_PARTNER_REF_KEY1 -- Profit Center (Code Conversion)" ).append("\n"); 
		query.append("            , CASE WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y' " ).append("\n"); 
		query.append("                        AND NVL((SELECT  'Y' FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND SAH.ADJ_TP_CD = 'EX' AND ROWNUM = 1), 'N') = 'N' THEN '01' ELSE NULL END AS BIZ_PARTNER_REF_KEY2 -- GT " ).append("\n"); 
		...
		query.append("            , NULL                                AS PAYMENT_REFERENCE -- 2014.10.17 NULL 처리  DECODE(SID.LINE_TP_LU_CD, 'WITHHOLDING TAX', '35', NULL)" ).append("\n"); 
		...
		query.append("                   WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691' THEN 'G5'|| ML.CNT_CD || LPAD(TO_CHAR(MV.VNDR_SEQ),6, '0') --2015.03.30 수정 -- 2014.10.23 Withholding Tax 부분 존재 여부 체크, 존재해야 함.." ).append("\n"); 
		query.append("                   WHEN SIH.ATTR_CATE_NM = 'REFUND' AND NVL((SELECT  'Y' FROM SAR_ACCT_MTX SAM WHERE SAM.PAY_ACCT_CD IS NOT NULL AND SAM.AR_ACCT_CD = SLCC.SGM_CTNT4 AND ROWNUM = 1),'N') = 'Y'" ).append("\n"); 
		query.append("                        AND NVL((SELECT  'Y' FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND SAH.ADJ_TP_CD = 'EX' AND ROWNUM = 1), 'N') = 'N'" ).append("\n"); 
		query.append("                        AND NVL((SELECT SR.RCT_CUST_CNT_CD FROM SAR_RECEIPT SR WHERE SR.RCT_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1)," ).append("\n"); 
		query.append("                                (SELECT SAH.BIL_TO_CUST_CNT_CD FROM SAR_ADJ_HDR SAH WHERE SAH.ADJ_NO = SIH.ATTR_CTNT4 AND ROWNUM = 1)) = 'TB' " ).append("\n"); 
		...
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD = 'WITHHOLDING TAX' OR MA.ACCT_CD = '211691' THEN 'C130' ELSE NULL END AS CLASSIFICATION  --2014.10.17 추가 Withholding인 경우 문자상수값" ).append("\n"); 
		query.append("            , CASE WHEN SID.LINE_TP_LU_CD IN ('ITEM', 'MISCELLANEOUS', 'FREIGHT') AND SIH.ATTR_CATE_NM NOT IN ('REFUND', 'ARAPOFFSET') AND MA.ACCT_CD NOT LIKE '954%' AND MA.ACCT_CD <> '211691'THEN " ).append("\n"); 

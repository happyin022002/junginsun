...
*@FileName : AccountReceivableAdjustDBDAOsearchOtsAdjustHdrListRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountReceivableAdjustDBDAOsearchOtsAdjustHdrListRSQL implements ISQLTemplate{
	...
	/**
	  ...
	  * </pre>
	  */
	public AccountReceivableAdjustDBDAOsearchOtsAdjustHdrListRSQL(){
		setQuery();
		...
		query.append("FileName : AccountReceivableAdjustDBDAOsearchOtsAdjustHdrListRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("       , A.BIL_TO_CUST_CNT_CD||'-'||LPAD(A.BIL_TO_CUST_SEQ, 6, '0') AS BIL_TO_CUST_CD " ).append("\n"); 
		query.append("       , A.VSL_CD||A.SKD_VOY_NO||A.DIR_CD AS LOCL_VVD_CD" ).append("\n"); 
		query.append("	   , A.RHQ_CD" ).append("\n"); 
		query.append("       , A.BIL_TO_CUST_CNT_CD" ).append("\n"); 
		query.append("       , A.BIL_TO_CUST_SEQ" ).append("\n"); 
		query.append("	   , A.RHQ_CD||B.INV_OFC_CD||A.BL_NO||A.INV_NO AS OTS_HDR_KEY" ).append("\n"); 
		query.append("	   , A.SHP_TO_CUST_CNT_CD||'-'||LPAD(A.SHP_TO_CUST_SEQ, 6, '0') AS SHP_BIL_TO_CUST_CD" ).append("\n"); 
		query.append("	   , A.TRNK_VVD_CD" ).append("\n"); 
		query.append("       , A.SAIL_ARR_DT" ).append("\n"); 
		query.append("       , DECODE(A.BKG_IO_BND_CD, 'O', 'O/B', 'I/B') AS IO_BND_CD" ).append("\n"); 
		query.append("       , A.DUE_DT" ).append("\n"); 
		query.append("       , A.CUST_SREP_CD AS SREP_CD" ).append("\n"); 
		query.append("       , A.OTS_RMK" ).append("\n"); 
		query.append("	   , A.XCH_RT_TP_CD" ).append("\n"); 
		query.append("       , A.XCH_RT_DT" ).append("\n"); 
		query.append("       , A.CR_MK_FLG AS CR_FLG" ).append("\n"); 
		query.append("       , A.OTS_SRC_CD AS AR_FINC_SRC_CD" ).append("\n"); 

...
*@FileName : AccountPayableInvoiceDBDAOAddUnsettledAccountPaymentSummaryCSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class AccountPayableInvoiceDBDAOAddUnsettledAccountPaymentSummaryCSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * AccountPayableInvoiceDBDAOAddUnsettledAccountPaymentSummaryCSQL
	  * </pre>
	  */
	public AccountPayableInvoiceDBDAOAddUnsettledAccountPaymentSummaryCSQL(){
		setQuery();
		...
		query.append("FileName : AccountPayableInvoiceDBDAOAddUnsettledAccountPaymentSummaryCSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("      , TO_CHAR(SPD.ACCTG_DT, 'YYYYMM')   AS UNSTL_YRMON" ).append("\n"); 
		query.append("      , SPD.INV_SEQ                       AS INV_SEQ" ).append("\n"); 
		query.append("      , SIH.INV_NO                        AS INV_NO" ).append("\n"); 
		query.append("      , TO_CHAR(SPD.ACCTG_DT, 'YYYYMMDD') AS GL_DT" ).append("\n"); 
		query.append("      , 'AP'                              AS UNSTL_SRC_CD" ).append("\n"); 
		query.append("      , 'SAP_PSY_DTL'                     AS UNSTL_CATE_NM" ).append("\n"); 
		...
		query.append("      , SPH.CURR_CD                       AS UNSTL_CURR_CD" ).append("\n"); 
		query.append("      , SPH.PAY_XCH_RT                    AS CONV_XCH_RT" ).append("\n"); 
		query.append("      , SPD.PAY_AMT                       AS INP_DR_AMT" ).append("\n"); 
		query.append("      , 0                                 AS INP_CR_AMT" ).append("\n"); 
		query.append("      , SPD.PAY_FUNC_AMT                  AS ACCT_DR_AMT" ).append("\n"); 
		query.append("      , 0                                 AS ACCT_CR_AMT" ).append("\n"); 
		query.append("      , SPD.PAY_AMT * -1                  AS UNSTL_AMT" ).append("\n"); 
		query.append("      , TO_CHAR(SPH.PAY_DT, 'YYYYMMDD')   AS UNSTL_TJ_DT" ).append("\n"); 
		...
		query.append("AND     SPH.PAY_SEQ = SPD.PAY_SEQ" ).append("\n"); 
		...
		query.append("AND     SPD.PAY_AMT <> 0" ).append("\n"); 
		query.append("AND     SPD.ACCTG_DT >= TO_DATE(REPLACE(@[unstl_yrmon],'-',''), 'YYYYMM')" ).append("\n"); 
		query.append("AND     SPD.ACCTG_DT <  ADD_MONTHS(TO_DATE(REPLACE(@[unstl_yrmon],'-',''), 'YYYYMM'), 1)" ).append("\n"); 

...
import com.clt.syscommon.common.table.GNI_USR_FILE;
...
public class UI_COM_SYS_015HTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
        ...
        */ 
        COM_USER2 com_user = COM_USER2.fromRequest(request);
        Collection gni_usr_files = null;
        ComUsrOfcCngVO[] com_users = null;
		if(command.isCommand(FormCommand.MULTI)) {
			com_users = (ComUsrOfcCngVO[])getVOs(request, ComUsrOfcCngVO.class);
			gni_usr_files = GNI_USR_FILE.fromRequestGrid(request,"");
		}
		
		UiComSys015Event event = new UiComSys015Event();
		
        if(com_users != null){
        	if(gni_usr_files != null){
        		event = new UiComSys015Event(
	                com_user,  // table value object
	                com_users,  // table value object
	                gni_usr_files,  // table value object
	                usr_id  ,  // form tag name
	                usr_nm  ,  // form tag name
	                auth_type  ); // form tag name
        	}
        }

        ...
        switch(sAction) {
        	...
        	case IBSEARCH:      //Retrieving
        		if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml=sheetObj.GetSearchData("BCM_FIN_0001GS.do", FormQueryString(formObj) + "&" + ComGetPrefixParam(""));
 	...
 	function changeCloseStatus(flag){							// 2014.8.28 ISA-1 오동현 수석님 Closing Status : All Close, All Open 286~298
 		...
 		if(flag == "O"){
 			for (var ir=1; ir<=sheetObj.RowCount(); ir++){
 	     		sheetObj.SetCellValue(ir, "clz_sts_cd","O");
 	     	}
 		}else{
 			for (var ir=1; ir<=sheetObj.RowCount(); ir++){
 	     		sheetObj.SetCellValue(ir, "clz_sts_cd","C");
 	     	}
 		} 		
 	}
    /**

...
function initSheet(sheetObj, sheetNo) {
	...
	case 1: //t1sheet1 init
        with(sheetObj){       
			  var HeadTitle1="|Account Number|Supplier Code|Currency|Supplier Name|";
			  var headCount=ComCountHeadTitle(HeadTitle1);  (headCount, 0, 0, true);
			  SetConfig( { SearchMode:2, MergeSheet:5, Page:20, DataRowMerge:1 } );
			  var info    = { Sort:0, ColMove:1, HeaderCheck:0, ColResize:1 };
			  var headers = [ { Text:HeadTitle1, Align:"Center"} ];
			  InitHeaders(headers, info);
			  var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
							{Type:"Text",      Hidden:0,  Width:200,  Align:"Center",  ColMerge:1,   SaveName:"bank_acct_no",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch (sAction) {		
		case IBSEARCH: //retrieve
			...
			var sXml=sheetObj.GetSearchData("STM_SAP_0032GS.do", FormQueryString(formObj));
...
function sheet1_OnClick(sheetObj, Row, Col){
	window.returnValue=sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'bank_acct_no');
	window.returnValue=sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'vndr_seq');
	window.returnValue=sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'bank_acct_seq');
}

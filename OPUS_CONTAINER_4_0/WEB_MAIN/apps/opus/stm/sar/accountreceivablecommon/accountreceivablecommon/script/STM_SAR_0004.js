 ...
 */
function loadPage(paramOtsSrcXcld) {
		...
		case 1: // t1sheet1 init
			with (sheetObj) {
				...
				var cols = [ { Type : "Status",		Hidden : 1, Width : 30, 	Align : "Center",	ColMerge : 1, 	SaveName : "ibflag" }, 
				             { Type : "CheckBox",	Hidden : 0, Width : 30, 	Align : "Left",		ColMerge : 1, 	SaveName : "checkbox" }, 
	...
	switch (sAction) {
		case IBSEARCH: // retrieve
			...
			setTimeout( function () {
				...
				var sXml = sheetObj.GetSearchData("STM_SAR_0004GS.do", FormQueryString(formObj));
				sheetObj.LoadSearchData(sXml, { Sync : 1 });
				ComOpenWait(false); 
				  
				// parameter 체크박스 설정
				for ( var i = 0; i < otsSrcXcld.length; i++) {
					for ( var row = 1; row <= sheetObj.RowCount(); row++) {
						var paramOfcCd = otsSrcXcld[i];

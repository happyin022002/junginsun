...
<%@ page import="com.clt.apps.opus.stm.sar.accountreceivableagent.accountreceivableagent.event.StmSar5901Event"%> 
<%@ page import="org.apache.log4j.Logger" %>
<%
    StmSar5901Event event = null;				//PDTO(Data Transfer Object including Parameters)
	...
	try {
	   ...
	   */
		event = (StmSar5901Event)request.getAttribute("Event");
		...
		*/
		bl_no 		= JSPUtil.getNull(request.getParameter("bl_no"));
		ar_ofc_cd 	= JSPUtil.getNull(request.getParameter("ar_ofc_cd"));
		gl_yrmon_to = JSPUtil.getNull(request.getParameter("gl_yrmon_to"));
		option1 	= JSPUtil.getNull(request.getParameter("option1"));
		
		due_dt_fm   = JSPUtil.getNull(request.getParameter("due_dt_fm"));	
		due_dt_to   = JSPUtil.getNull(request.getParameter("due_dt_to"));	
		delt_flg    = JSPUtil.getNull(request.getParameter("delt_flg")); 
		asa_curr_cd    = JSPUtil.getNull(request.getParameter("asa_curr_cd")); 
...
<input type="hidden" name="gl_yrmon_to" value="<%=gl_yrmon_to%>" id="gl_yrmon_to" />
<input type="hidden" name="option1" value="<%=option1%>" id="option1" />
<input type="hidden" name="due_dt_fm" value="<%=due_dt_fm%>" id="due_dt_fm" />
<input type="hidden" name="due_dt_to" value="<%=due_dt_to%>" id="due_dt_to" />
<input type="hidden" name="asa_curr_cd" value="<%=asa_curr_cd%>" id="asa_curr_cd" />
		<!-- page_title_area(S) -->
		<div class="page_title_area clear">
		   <!-- page_title(S) -->
			<h2 class="page_title"><span>BL NO Pop up</span></h2>
			<!-- page_title(E) -->
			<!-- opus_design_btn(S) -->
	...
	<div class= "wrap_search">
		<div class= "opus_design_inquiry wFit">
			<table>
					...
					<tr>
						<th>BL NO</th> 
            			<td><input name="bl_no" type="text" style="width:90px;" dataformat="engup" class="input" value="<%=bl_no%>" id="bl_no" /> </td>
					</tr>	
				</tbody>
			</table>
		</div>

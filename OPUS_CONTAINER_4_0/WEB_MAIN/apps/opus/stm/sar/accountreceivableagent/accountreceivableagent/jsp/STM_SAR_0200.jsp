...
<%@ page import="com.clt.apps.opus.stm.sar.accountreceivableagent.accountreceivableagent.event.StmSar0200Event"%> 
<%@ page import="org.apache.log4j.Logger" %>
<%@ page import="com.clt.framework.component.util.StringUtil" %>

<%
    StmSar0200Event event = null;				//PDTO(Data Transfer Object including Parameters)
	...
	try {
	   ...
	   */
		...
		event = (StmSar0200Event)request.getAttribute("Event");
		serverException = (Exception)request.getAttribute(CommonWebKeys.EXCEPTION_OBJECT);

		asa_no = StringUtil.xssFilter(request.getParameter("asa_no"));
		asa_no = asa_no==null?"":asa_no;

		asa_ofc_cd = StringUtil.xssFilter(request.getParameter("ofc_cd"));  //.substring(0,5);   // TEMBA  //
		asa_ofc_cd = asa_ofc_cd==null?"":asa_ofc_cd;

		flag_cd = StringUtil.xssFilter(request.getParameter("flag_cd"));  //.substring(0,5);   // TEMBA  //
		flag_cd = flag_cd==null?"":flag_cd;
		//System.out.println( "-----------------------------"+flag_cd);	
		/* 
...
<input type="hidden" name="flag_cd" value="<%=flag_cd%>" id="flag_cd" />
<!-- page_title_area(S) -->
<div class="layer_popup_title">
	<div class="page_title_area clear">
	   <!-- page_title(S) -->
		<!-- 타이틀 내용 동적생성 (별도 코딩 불필요) -->
		<h2 class="page_title"><span>ASA No</span></h2>
		<!-- page_title(E) -->
		<!-- opus_design_btn(S) -->
		...
		<div class="opus_design_inquiry wFit">
			<table>
				...
				<tbody>
					<tr>
						<th>ASA No</th>
                        <td><input type="text" style="width:100px;" class="input" name="asa_no" value="<%=asa_no%>" id="asa_no" /><!-- 
                             --><input type="text" style="width:100px;display:none;" class="input" name="asa_ofc_cd" value="<%=asa_ofc_cd%>" id="asa_ofc_cd"/></td>
					</tr>
				</tbody>
			</table>
		</div>

...
/*------------------다음 코드는 JSDoc을 잘 만들기 위해서 추가된 코드임 ------------------*/
    ...
    /**
     * @extends
     * @class ESM_ACM_0012 : ESM_ACM_0012 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
     */
/* 개발자 작업 */
    // 공통전역변수
...
//        try {
            ...
            switch (srcName) {
                ...
                case "btn2_bkg_expt":
    			    ...
    			    frmObj.action="ESM_ACM_0012DL.do?" + FormQueryString(frmObj);
    			    frmObj.submit();
    			    ComOpenWait(false);
                    break;
                case "btn_chk_sts":
                	ComOpenWindowCenter("ESM_ACM_0032_POP.do?pop_mode=Y", "Batch Management", 1010, 580, false);
                    break;
                case "btn_start_clc":
...
//        }
    ...
    function loadPage() {
        ...
        memo_sheet3_OnLoadFinish(sheetObjects[2]);
        memo_sheet4_OnLoadFinish(sheetObjects[3]);
        memo_sheet5_OnLoadFinish(sheetObjects[4]);
        ACMMemoSheet_Open(sheetObjects[0]);    // CoAcm.js에 정의
        ACMMemoSheet_Open(sheetObjects[1]);    // CoAcm.js에 정의
        ACMMemoSheet_Open(sheetObjects[2]);    // CoAcm.js에 정의
        ACMMemoSheet_Open(sheetObjects[3]);    // CoAcm.js에 정의
        ACMMemoSheet_Open(sheetObjects[4]);    // CoAcm.js에 정의
        ACMMemoSheet_Close(sheetObjects[0], frmObj.vvd_cd);    // CoAcm.js에 정의
        ACMMemoSheet_Close(sheetObjects[1], frmObj.agmt_no1);    // CoAcm.js에 정의
        ACMMemoSheet_Close(sheetObjects[2], frmObj.agmt_no2);    // CoAcm.js에 정의
        ACMMemoSheet_Close(sheetObjects[3], frmObj.ofc_cd);    // CoAcm.js에 정의
        ACMMemoSheet_Close(sheetObjects[4], frmObj.loc_cd);    // CoAcm.js에 정의
    }
    /**
     * 시트 초기설정값, 헤더 정의
    ...
    function doActionIBSheet(shtObj, frmObj, sAction, CondParam, PageNo) {
        switch (sAction) {
            case IBSEARCH:    //조회
				...
				if (document.form.ofc_div.value == "F") {
					if (document.form.port_div.value == "") {
						ComShowMessage(ComGetMsg("ACM00002", "Office", "", ""));
						return;
					}
				}				
                ComOpenWait(true);
                frmObj.f_cmd.value=SEARCH;
                var xmlStr=shtObj.GetSearchData("ESM_ACM_0012GS.do", FormQueryString(frmObj));
            ...
            case IBSAVE:      //저장
                ...
                shtObj.GetSaveData("ESM_ACM_0012GS.do", FormQueryString(frmObj));
                ComOpenWait(false);
                break;
        }
    }
     ...
     * (ESM_ACM_0012일때만 coAcm.js의 ACMMemoSheet_Close이벤트에서 호출됨)
     */
    function frmObj_OnChange() {
        ...
        with (document.form) {
            switch (elementName) {
				...
				//BKG office, agent 선택시 route부분이 활성화
	            case "ofc_div":
					if (document.form.ofc_div.value == "B" || document.form.ofc_div.value == "A") {
						...
						//document.form.port_div.className = "input";
						//document.form.port_div.removeAttribute('required');
					} else {
						document.form.port_div.disabled=false;
						//document.form.port_div.className = "input1";
						//document.form.port_div.setAttribute('required','');
					}
	                break;
            }
        }

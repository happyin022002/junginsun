    ...
    function initSheet(shtObj, shtNo) {
        ...
        with(shtObj){
          ...
          var cols = [ {Type:"DelCheck",  Hidden:0, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"chk" },
                 ...
                 {Type:"PopupEdit", Hidden:0, Width:120,  Align:"Center",  ColMerge:0,   SaveName:"vndr_seq",         KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:6 },
          ...
          SetColProperty("ff_cnt_seq", {AcceptKeys:"E|N", InputCaseSensitive:1});
          SetColProperty("vndr_seq", {AcceptKeys:"N", InputCaseSensitive:1});
          }
    ...
    function doActionIBSheet(shtObj, frmObj, sAction, CondParam, PageNo) {
        switch (sAction) {
            case IBSEARCH:       // 조회
                ...
                shtObj.DoSearch("ESM_ACM_0019GS.do", FormQueryString(frmObj), {Sync:2} );
                ComOpenWait(false);
                break;
            case IBSAVE:         // 저장
    ...
    function sheet1_OnPopupClick(shtObj, Row, Col) {
        with (shtObj) {
            switch (ColSaveName(Col)) {
                case "ff_cnt_seq":
                    var param="?cust_cd=" + document.form.ff_cnt_cd.value;
                    // ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll)
                    ComOpenPopup("COM_ENS_041.do"+param, 775, 475, "setPopupData", "1,0,1,1,1,1,1", true, false, Row, Col, 0);
                    break;
                case "vndr_seq":
                    // ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll)
        ...
        with (shtObj) {
            switch (ColSaveName(Col)) {
                case "ff_cnt_seq":
                    ...
                    // Duplication check
                    var xmlStr0=shtObj.GetSearchData("ESM_ACM_0019GS.do", "f_cmd=" + SEARCH01 + "&ff_cnt_seq=" + Value);

        ...
        try {
            ...
            switch (srcName) {
                case "btn_retrive":     // Retrieve
                	if(frmObj.fac_ofc_cd.value ==""){
                		ComShowCodeMessage("COM130201","Office")
                	}else{
                ...
                case "btn_ofc_popup":
                    ...
                    ComOpenPopup("COM_ENS_071.do?ofc_pts_cd=" + frmObj.fac_ofc_cd.value, 700, 515, "setPopupData_facOfcCd", "1,0,1,1,1,1,1", true);
                    break;
                case "btn_add":           // Row Add
                    if (!ComChkValid(frmObj)) return;
                    var newRowIdx=shtObj.DataInsert();
                    // 조회 조건의 Office를 fac_ofc_cd에 setting
                    shtObj.SetCellValue(newRowIdx, "fac_ofc_cd",frmObj.fac_ofc_cd.value,0);
                    break;
            } // end switch
        } catch(e) {
            if (e == "[object Error]") {
    ...
    function initSheet(shtObj, shtNo) {
        ...
        with(shtObj){
         ...
         var cols = [ {Type:"DelCheck",  Hidden:0, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"chk" },
                      ...
                      {Type:"Text",      Hidden:1, Width:80,   Align:"Center",  ColMerge:0,   SaveName:"fac_ofc_cd" },
    ...
    function doActionIBSheet(shtObj, frmObj, sAction, CondParam, PageNo) {
        switch (sAction) {
            case IBSEARCH:       // 조회
                ...
                shtObj.DoSearch("ESM_ACM_0021GS.do", FormQueryString(frmObj), {Sync:2} );
                ComOpenWait(false);
                break;
            case IBSAVE:         // 저장
                if (!ComChkValid(frmObj)) return;

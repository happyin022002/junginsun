...
/**
 * @extends 
 * @class ESM_CSQ_0014 : ESM_CSQ_0014 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
	/* 개발자 작업	*/
	//공통전역변수
	...
	function initSheet(sheetObj,sheetNo) {
		...
		switch(sheetNo) {
			case 1:		// sheet1 init
			    with(sheetObj){
		        var HeadTitle="STS|SEL|Trade|New Lane";
		        ...
		        var cols = [ {Type:"Status",    Hidden:1, Width:80,   Align:"Center",  ColMerge:1,   SaveName:"ibflag",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
		             {Type:"CheckBox",  Hidden:0, Width:50,   Align:"Center",  ColMerge:1,   SaveName:"add_flg",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1, TrueValue:"Y", FalseValue:"N" },
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch(sAction) {
			case IBSEARCH:          // 화면 조회
				...
				var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0014GS.do",searchParams);
	    		sheetObj.LoadSearchData(rtnXml,{Sync:1} );
				var checkRow=sheetObj.FindCheckedRow("add_flg");
				var arrRow=checkRow.split("|");
				for (i=0; i<arrRow.length; i++) {
					sheetObj.SetCellEditable(arrRow[i], "add_flg",0);
				}
				break;
			case "Apply":          // Apply 버튼 클릭
				...
				var sXml=sheetObj.GetSaveData("ESM_CSQ_0014GS.do", searchParams + "&" + saveStr);
				ComOpenWait(false);

...
/**
 * @extends
 * @class ESM_CSQ_0218 : ESM_CSQ_0218 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
/* 개발자 작업	*/
//공통전역변수
...
function initSheet(sheetObj,sheetNo) {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
		    with(sheetObj){
	        	var HeadTitle1="SEQ|Year|Quarter|Office View|Trade|Sub Trade|R.Lane|Lane Bound|Month|Supply|VVD Count|RHQ|Office|POL|POD|Main/Sector|Load|G.RPB|G.REV|CM Cost(PA)|CM Cost(RA)|CMCB(PA)|CMCB(RA)|CM(PA)|CM(RA)|CMPB(PA)|CMPB(RA)";
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml=sheetObj.GetSearchData("ESM_CSQ_0218GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          //화면 조회시
			...
			setTimeout(function(){
				...
				var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0218GS.do", searchParams);

...
/**
 * @extends
 * @class ESM_CSQ_0215 : ESM_CSQ_0215 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
/* 개발자 작업	*/
//공통전역변수
...
function loadPage(){
	...
	setSubTradeCombo();
	if(formObj.p_sub_trd_cd.value != "All")	ComSetObjValue(f_sub_trd_cd, formObj.p_sub_trd_cd.value);
	if(formObj.p_rlane_cd.value != "All")   ComSetObjValue(f_rlane_cd, formObj.p_rlane_cd.value);
...
function initSheet(sheetObj,sheetNo) {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
		    with(sheetObj){
				var HeadTitle1="STS|Trade|Sub Trade|R.Lane|Bound|P/F SKD Group|P/F SKD Ver.|Route|SEL";
				...
				var cols = [ {Type:"Status",    Hidden:0, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
				             ...
				             {Type:"Text",      Hidden:0,  Width:90,   Align:"Center",  ColMerge:1,   SaveName:"pf_grp_cd",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
				             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"pf_svc_tp_cd",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBSEARCH:          // 화면 조회 시
			...
			ComOpenWait(true);
			var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0215GS.do",searchParams);
		...
		case "AddFreezing":		// Add Freezing
            ...
            var sXml=sheetObj.GetSaveData("ESM_CSQ_0215GS.do", searchParams + "&" +sParam);
			ComOpenWait(false);
			...
			if(State != "S"){
				...
				ComShowCodeMessage('CSQ00001','Data'); //{?msg1} saved successfully.
...
function validateForm(sheetObj, formObj, sAction){
	...
	case IBSEARCH:  //Retrieve
		if(ComGetObjValue(f_rlane_cd) == "") {
			ComShowCodeMessage("CSQ00013","R/Lane"); //{?msg1} is mandatory item.
			f_rlane_cd.Focus();
			return false;
		}
    	break;
	case "AddFreezing":  // Add-Creation
		if(sheetObj.CheckedRows("sel_flg") < 1){
			ComShowCodeMessage("CSQ00046"); ////Please check at least one row.
			return false;
		}
    	break;
	}

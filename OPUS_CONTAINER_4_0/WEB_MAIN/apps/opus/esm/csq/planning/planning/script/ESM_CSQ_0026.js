...
/**
 * @extends 
 * @class ESM_CSQ_0026 : ESM_CSQ_0026 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
	/* 개발자 작업	*/
	//공통전역변수
	...
	*/
	function f_bse_tp_cd_OnChange() {
		...
		if (bse_tp_cd == "Y") {
			$("f_bse_qtr_cd").prop('disabled', true);
			sheetObjects[0].SetColHidden("bse_qtr_cd",1);
		} else {
			$("f_bse_qtr_cd").prop('disabled', false);
		...
		switch(sheetNo) {
			case 1:		//sheet1 init
			    with(sheetObj){
			        ...
			        InitColumns(cols);
			        SetEditable(1);
			        SetRangeBackColor(1, 9, 1, 29, "#555555");
			        SetSheetHeight(300);
	            }
			break;
		}
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch(sAction) {
			case IBSEARCH:          //조회
				...
				sheetObj.DoSearch("ESM_CSQ_0026GS.do",sParam );

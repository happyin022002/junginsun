...
/**
 * @extends
 * @class ESM_CSQ_0224 : ESM_CSQ_0224 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
/* 개발자 작업	*/
//공통전역변수
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
		    with(sheetObj){
	        	...
	        	SetSheetHeight(360);
	        	SetEditable(1);
	        	SetRangeBackColor(0, 17,0, 19,"#555555");
	        	SetRangeBackColor(0, 24,0, 27,"#555555");
	      	}
			break;
	}
}
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml=sheetObj.GetSearchData("ESM_CSQ_0224GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시
			...
			var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0224GS.do", searchParams);
		...
		case "RawDataDownExcel":		// Raw Data 엑셀 다운로드
           ...
           //Year, Quarter, Office view user 선택. office level은 항상 office. week, vvd, pol-pod pair check.
			var param="f_cmd="         + COMMAND01
			          ...
			          + "&f_chk_vvd=V"
					  + "&f_chk_pair=Y";
			document.location.href="ESM_CSQ_0224DL.do?" + param;
			ComOpenWait(false);
			break;
  }
}
			...
			if(formObj.chk_week.checked){ //week 체크시
				if(formObj.f_fm_wk.value != "" && formObj.f_to_wk.value != ""){
		 			if(formObj.f_fm_wk.value < qtaWkArr[qta][0] || formObj.f_to_wk.value > qtaWkArr[qta][1]){
		 					ComShowCodeMessage('CSQ00025',qta); //Duration doesn't include {?msg1}
		 					return false;
		 			}
		 		}else if((formObj.f_fm_wk.value == "" && formObj.f_to_wk.value != "") || (formObj.f_fm_wk.value != "" && formObj.f_to_wk.value == "")){

...
=========================================================*/	
	...
	/**
	 * @extends
	 * @class ESM_CSQ_0057 : ESM_CSQ_0057 �앹꽦���꾪븳 �붾㈃�먯꽌 �ъ슜�섎뒗 �낅Т �ㅽ겕由쏀듃瑜��뺤쓽�쒕떎.
	 */
	/* 媛쒕컻���묒뾽	*/
	//怨듯넻�꾩뿭蹂�닔
		...
		switch(sheetNo) {
			case 1:		//sheet1 init
		        ...
		        var cols = [ {Type:"Seq",       Hidden:0, Width:35,   Align:"Center",  ColMerge:1,   SaveName:"seq",           KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
		             ...
		             {Type:"Float",     Hidden:0,  Width:250,  Align:"Right",   ColMerge:1,   SaveName:"grpb_decimal",  KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:13,   UpdateEdit:0,   InsertEdit:0 },
		      ...
		      InitColumns(cols);
		      SetEditable(1);
		      SetRangeBackColor(0, 13, 0, 17,"#555555");
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		...
		switch(sAction) {
			case IBCLEAR:          // �붾㈃ �묒냽 ��
				...
				var sXml=sheetObj.GetSearchData("ESM_CSQ_0057GS.do", FormQueryString(formObj));
				...
				if (arrXml.length > 1)
					ComSetYear(arrXml[1]);
				if (arrXml.length > 2)
					ComXml2ComboItem(arrXml[2], f_ofc_lvl, "code", "name");
				if (arrXml.length > 3)
			...
			case IBSEARCH:          // �붾㈃ 議고쉶 ��
				...
				sheetObj.DoSearch("ESM_CSQ_0057GS.do", FormQueryString(formObj), {Sync:1});
				break;
			...
			case "RawDataDownExcel":		// Raw Data �묒� �ㅼ슫濡쒕뱶
				...
				var param="f_cmd="         + COMMAND01
				          ...
				          + "&f_chk_week=W"
				          + "&f_chk_vvd=V";
				document.location.href="ESM_CSQ_0057DL.do?" + param;
				ComOpenWait(false);
				break;
	    }
	}
...
//			 f_rhq_cd.style.display="inline";
//			 f_rgn_ofc_cd.style.display="inline";
		 }
	}
	 /**

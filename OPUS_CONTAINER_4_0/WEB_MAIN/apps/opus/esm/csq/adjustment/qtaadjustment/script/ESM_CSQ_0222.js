...
function initSheet(sheetObj,sheetNo) {
	...
	switch(sheetNo) {
		case 1:		//sheet1 init
		    with(sheetObj){
	        	var HeadTitle1="STS|Sub Trade|R.Lane|Lane Bound|RHQ|Office|POL|POD|Select";
	        	...
	        	var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             ...
	        	             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"rhq_cd",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"rgn_ofc_cd",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             ...
	        	             {Type:"CheckBox",  Hidden:0, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"sel_flg",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 } ];
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml=sheetObj.GetSearchData("ESM_CSQ_0222GS.do", FormQueryString(formObj));
			var arrXml=sXml.split("|$$|");
			if (arrXml.length > 0)
				ComXml2ComboItem(arrXml[0], f_dir_cd, "code", "name");
			if (arrXml.length > 1)
            	ComXml2ComboItem(arrXml[1], f_rhq_cd, "code", "name");
			ComOpenWait(false);
			break;
		case IBSEARCH:          // 화면 조회 시
			...
			ComOpenWait(true);
			var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0222GS.do",searchParams);
		...
		case MULTI:		// Creation시에
            ...
            var sXml=sheetObj.GetSaveData("ESM_CSQ_0222GS.do", searchParams + "&" +sParam);
//			sheetObj.DoSave("ESM_CSQ_0222GS.do", searchParams, "sel_flg", false);
			ComOpenWait(false);

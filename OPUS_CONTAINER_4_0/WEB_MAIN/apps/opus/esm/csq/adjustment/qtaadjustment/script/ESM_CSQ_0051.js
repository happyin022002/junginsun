...
/**
 * @extends 
 * @class ESM_CSQ_0051 : ESM_CSQ_0051 생성을 위한 화면에서 사용하는 업무 스크립트를 정의한다.
 */
/* 개발자 작업	*/
//공통전역변수
	...
	try {
		...
		switch(srcName) {
			...
			case "btn_RowGrpAdd":
				...
				break;	
			case "btn_RowAdd":
				doActionIBSheet(sheetObject, formObj, "RhqRowAdd");
				break;		
		...
		case 1:		//sheet1 init
		    with(sheetObj){
	        	...
	        	var cols = [ {Type:"Status",    Hidden:0,  Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             ...
	        	             {Type:"Text",      Hidden:0,  Width:65,   Align:"Center",  ColMerge:1,   SaveName:"conv_dir_cd",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             ...
	        	             {Type:"Text",      Hidden:1,  Width:75,   Align:"Center",  ColMerge:1,   SaveName:"org_fm_yrwk",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             {Type:"Text",      Hidden:0,  Width:55,   Align:"Center",  ColMerge:1,   SaveName:"aply_fm_yrwk",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             ...
	        	             {Type:"Text",      Hidden:1,  Width:75,   Align:"Center",  ColMerge:1,   SaveName:"org_to_yrwk",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
	        	             {Type:"Text",      Hidden:0,  Width:55,   Align:"Center",  ColMerge:1,   SaveName:"aply_to_yrwk",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
function doActionIBSheet(sheetObj, formObj, sAction) {
	...
	switch(sAction) {
		case IBCLEAR:          // 화면 접속 시
			...
			var sXml=sheetObj.GetSearchData("ESM_CSQ_0051GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:          // 화면 조회 시
			...
			ComOpenWait(true);
			var rtnXml=sheetObj.GetSearchData("ESM_CSQ_0051GS.do",searchParams);
		...
		case IBSAVE:          // 화면 저장시
			...
			//ratio가 100%인지 check
			if (!checkRatioAdj(sheetObj)) return;
			...
			var sXml = sheetObj.GetSaveData("ESM_CSQ_0051GS.do", searchParams + "&" + saveStr);
		...
		case MULTI01:		// Creation시에
			...
			var sXml=sheetObj.GetSaveData("ESM_CSQ_0051GS.do", searchParams);
			var State=ComGetEtcData(sXml, ComWebKey.Trans_Result_Key);
			ComOpenWait(false);
			if (State == "S") {
		...
		case MULTI02:		// RHQ Group Row Add 클릭시 
			...
			break;
		case "FigureInquiry":  // Figure Inquiry 클릭시
			//sheet에 변경사항이 있는지 확인
			if (sheetObj.IsDataModified()== true) {
				ComShowCodeMessage("CSQ00017");
		        return false;
			...
			ComSetSearchParams("f_cmd", "");
			 ComOpenWindow("ESM_CSQ_0052.do?" + searchParams + "&div_period=" + document.getElementById("div_period").innerHTML,  window,  "dialogHeight:560px;dialogWidth:1050px;center:yes;resizable:no;scroll:no;status:no;unadorned:yes;" , true);
			break;
		case "RhqRowAdd":  // Rhq Row Add 클릭시
			//선택한 Row 복사
			...
			sheetObj.InitCellProperty(row, "rhq_cd",{ Type:"Combo"} );
			...
			var f_dir_cd		=sheetObj.GetCellValue(selRow1, "dir_cd");
			var param="&f_bse_yr="     + f_bse_yr_val  
		             ...
		             +"&f_dir_cd="     +f_dir_cd
		            ;
			var sXml=sheetObj.GetSearchData("CommonGS.do", "f_cmd="+ SEARCH02 + "&code_name=rhqByPortion&code_param=" + param + "&all_flag=");
			ComCsqSetIBCombo(sheetObj, sXml, "rhq_cd", true, 0, row);
			break;	
  }
}
 /**
 	...
 	switch(sAction) {
 		case IBSAVE:  // 화면 저장시에
 			for(var i=sheetObj.HeaderRows();i<sheetObj.RowCount()+sheetObj.HeaderRows();i++){
 				if(sheetObj.GetCellValue(i,"rhq_cd") == ""){
 					ComShowCodeMessage('CSQ00024','RHQ');//Please Input RHQ.
 					return false;
 				}

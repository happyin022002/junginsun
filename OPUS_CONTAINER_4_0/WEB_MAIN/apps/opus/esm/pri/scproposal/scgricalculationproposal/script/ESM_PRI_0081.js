...
//        		sheetObj.CheckAll(colName,0);
        		sheetObj.SetCellValue(Row, colName, 1);
        	}
        }
	}
	/**
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "prc_cmdt_def_cd") {
			if (sheetObj.GetCellEditable(Row, "prc_cmdt_def_cd")) {
				...
				sUrl += "&grp_cd=" + PRI_SP_SCP + "&commodity_cmd=CG&prc_cmdt_tp_cd=" + sheetObj.GetCellValue(Row, "prc_cmdt_tp_cd") + "&multi_yn=Y";
				sUrl += "&func="+"sheet1_popupReturnVal";
				
				//2014.12.10
				//3th popup 
				//ComOpenPopup을 사용하면 일반 popup창이 두개가 뜨면서 window name 이 겹쳐 이전 팝업창이 사라지게됨
				ComOpenWindowCenter(sUrl, "ESM_PRI_4027", 600, 320, false);
			}
		}
	}
	...
	function sheet1_popupReturnVal(rtnVal) {
    	...
    	if (rtnVal != null && rtnVal.length > 0) {
			for (var i=0; i < rtnVal.length; i++) {
				if (i == 0) {
					sheetObj.SetCellValue(G_TMP_OBJECT.row, "prc_cmdt_def_cd", rtnVal[i].cd, 0);
					sheetObj.SetCellValue(G_TMP_OBJECT.row, "prc_cmdt_def_nm", rtnVal[i].nm, 0);
					if (rtnVal[i].cd.length == 5) {
						sheetObj.SetCellValue(G_TMP_OBJECT.row, "prc_cmdt_tp_cd", "G", 0);
					} else if (rtnVal[i].cd.length == 6) {
						sheetObj.SetCellValue(G_TMP_OBJECT.row, "prc_cmdt_tp_cd", "C", 0);
					}
				} else {
					...
					sheetObj.SetCellValue(idx, "prc_cmdt_def_cd",rtnVal[i].cd,0);
					sheetObj.SetCellValue(idx, "prc_cmdt_def_nm",rtnVal[i].nm,0);
					if (rtnVal[i].cd.length == 5) {
						sheetObj.SetCellValue(idx, "prc_cmdt_tp_cd","G",0);
					} else if (rtnVal[i].cd.length == 6) {
	...
	function sheet2_OnPopupClick(sheetObj, Row, Col) {
		...
		if (colName == "cust_seq") {
			if (sheetObj.GetCellEditable(Row, "cust_seq")) {
	            ...
	            sUrl += "&func="+"cust_seq_returnVal";
				
				//2014.12.10
				//3th popup 
				//ComOpenPopup을 사용하면 일반 popup창이 두개가 뜨면서 window name 이 겹쳐 이전 팝업창이 사라지게됨
				ComOpenWindowCenter(sUrl, "ESM_PRI_4014_POP", 640, 465, false);
			}
		}
	}
	...
	function cust_seq_returnVal(rtnVal) {
		if (rtnVal != null){
			G_TMP_OBJECT.sheet.SetCellValue(G_TMP_OBJECT.row, "cust_cnt_cd", rtnVal.custCntCd, 0);
			G_TMP_OBJECT.sheet.SetCellValue(G_TMP_OBJECT.row, "cust_seq", ComLpad(rtnVal.custSeq, 6, "0"), 0);

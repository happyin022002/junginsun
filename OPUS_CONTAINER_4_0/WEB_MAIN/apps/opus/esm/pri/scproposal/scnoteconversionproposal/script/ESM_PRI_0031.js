...
/**
 * @extends 
 * @class ESM_PRI_0031 : Business Script for ESM_PRI_0031
 */
     	...
     	switch(sheetID) {
     	    case "sheet1":
     	    	with(sheetObj){
		     	   ...
		     	   HeadTitle +=  "|1|2|3|4|5|6|7";
		     	   var headCount=ComCountHeadTitle(HeadTitle);
		     	   SetConfig( { SearchMode:2, MergeSheet:0, Page:20, DataRowMerge:1 } );
		     	   var info    = { Sort:1, ColMove:1, HeaderCheck:1, ColResize:1 };
		     	   var headers = [ { Text:HeadTitle, Align:"Center"} ];
		     	   InitHeaders(headers, info);
		     	   var cols = [ {Type:"Text",       Hidden:0, Width:60,   Align:"Center",  ColMerge:0,   SaveName:"cmdt_hdr_seq" },
		     	             ...
		     	             {Type:"Text",      Hidden:1, Width:90,   Align:"Left",    ColMerge:0,   SaveName:"nd_cnt" },
		     	             {Type:"Text",      Hidden:1, Width:90,   Align:"Left",    ColMerge:0,   SaveName:"na_cnt" },
		     	             {Type:"Text",      Hidden:1, Width:90,   Align:"Left",    ColMerge:0,   SaveName:"n1st_cmnc_amdt_seq" } ];
		     	   InitColumns(cols);
		     	   SetEditable(0);
		     	   SetWaitImageVisible(0);
		     	   SetCountPosition(0);
		     	   SetSheetHeight(107, 1);
     	      	}
     		break;
         	...
         	case "sheet2":
         	    with(sheetObj){
		              ...
		              var cols = [ {Type:"Seq",       Hidden:0, Width:60,   Align:"Center",  ColMerge:0,   SaveName:"seq" },
		                     ...
		                     {Type:"Text",      Hidden:0,  Width:200,  Align:"Left",    ColMerge:0,   SaveName:"note_ctnt",           KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,  EditLen:4000 },
		                     {Type:"Text",      Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"svc_scp_cd" },
		                     {Type:"Text",      Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"prop_no" },
		                     {Type:"Text",      Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"amdt_seq" },
		                     {Type:"Text",      Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"gen_spcl_rt_tp_cd" },
         	...
         	case "sheet3":
         	    with(sheetObj){
		              ...
		              var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
		                     ...
		                     {Type:"Combo",     Hidden:0, Width:79,   Align:"Center",  ColMerge:1,   SaveName:"conv_prc_rcv_term_cd",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
		                     {Type:"Combo",     Hidden:0, Width:79,   Align:"Center",  ColMerge:1,   SaveName:"conv_prc_de_term_cd",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
   ...
   */
 		...
 		try {
	 		...
	 		switch (sAction) {
				...
				case IBSEARCHAPPEND: // Retrieving
					ComOpenWait(true);
					if(openSheet2YN) {
						// While processing, Set the waiting image show option to false.
						formObj.f_cmd.value=SEARCH02;
						sheetObj.DoSearch("ESM_PRI_0031GS.do", FormQueryString(formObj) );
					} else {
						var sArr=opener.getSheetXml(0);
						var sXml=sArr[1];
						sheetObj.LoadSearchData(sXml,{Sync:1} );
	 			...
	 			case IBSEARCH_ASYNC01: // Retrieving
					...
					var sNm=sheetObj.GetComboInfo(0,"chg_rule_def_cd","Text");
		 			...
		 			var sXml=sheetObj.GetSearchData("ESM_PRI_0031GS.do", FormQueryString(formObj));
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified()) {
							...
							var sParam=FormQueryString(formObj);
			  				...
			  				var sXml=sheetObj.GetSaveData("ESM_PRI_0031GS.do", sParam);
			  				sheetObj.LoadSaveData(sXml);
		  				} else {
		  					formObj.f_cmd.value=MODIFY;
		  					formObj.note_conv_mapg_id.value=sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"note_conv_mapg_id");
			  				ComOpenWait(true);
			  				var sXml=sheetObj.GetSaveData("ESM_PRI_0031GS.do", FormQueryString(formObj));
			  				sheetObj.LoadSaveData(sXml);
		  				}
						...
						if(sheetObj.RowCount() > 0){
							obj.note_conv_flg="1";
							obj.master_seq=sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "cmdt_hdr_seq");
							obj.detail_seq=sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "cmdt_note_seq");
				...
				case IBINSERT: // Row Add
					...
					for(var i = 0; i < rowcnt; i++) {
						...
						sheetObj.SetCellValue(idx, "exp_dt",sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"exp_dt") == -1 ? "" : sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"exp_dt"),0);
						...
						sheetObj.SetCellValue(idx, "note_conv_tp_cd","C",0);//Proposal Rate Bullet Note
						sheetObj.SetCellValue(idx, "note_conv_seq",parseInt(ComPriGetMax(sheetObj, "note_conv_seq")) + 1,0);
						if(sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"note_clss_cd") == "G") {
						...
						} else if(sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"note_clss_cd") == "S") {
							sheetObj.SetCellValue(idx, "chg_rule_tp_cd","C",0);//CHARGE
				      		...
				      		if(arrRow != null && arrRow.length > 0) {  
				      			for(var i=0; i<arrRow.length; i++) {
									...
									sheetObj.SetCellValue(arrRow[i], "amdt_seq",formObj.amdt_seq.value,0);
					      			sheetObj.SetCellValue(arrRow[i], "note_conv_tp_cd","C",0);//Proposal Rate Bullet Note
				      			}
				      		}
				      	} else {
	 ...
	 */  
				...
				case "chg_rule_def_cd":		
					if (Value != null && Value != "" && Value.length == 3) {
						...
						if (sCode.indexOf(Value) < 0) {
							...
							sXml=sheetObj.GetSearchData("PRICommonGS.do", FormQueryString(formObj) + "&etc1=" + Value);
	 ...
	 function checkDuration(sheetObj) {
			...
			var effDt=ComGetDateAdd(sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "eff_dt") == -1 ? "" : sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "eff_dt"), "D", 0, "");
			var expDt=ComGetDateAdd(sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "exp_dt") == -1 ? "" : sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(), "exp_dt"), "D", 0, "");
	 		//Don't validate in case of legacy data
	 		if(returnCreationTypeCode()) {
	 			return true;
	 		}
	  ...
	  */
		    ...
		    switch(colname)
		    {
		    	...
		    	case "bkg_cmdt_def_cd":
		    		var sUrl="/opuscntr/ESM_PRI_4027.do?"
		   	    		...
		   	    		ComOpenPopup(sUrl, 600, 320, "Callback_findBkgCmdtDefCd", "1,0", true);
		  			break;
		    	case "bkg_por_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findBkgPorDefCd", "1,0", true);
					break;
		    	case "bkg_pol_def_cd":
		  			var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findBkgPolDefCd", "1,0", true);
					break;
		    	case "bkg_pod_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findBkgPodDefCd", "1,0", true);
					break;
		    	case "bkg_del_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findBkgDelDefCd", "1,0", true);
					break;
		    	case "conv_cmdt_def_cd":
		    		var sUrl="/opuscntr/ESM_PRI_4027.do?"
		   	    		sUrl += "commodity_cmd=CG";
		   	    		...
		   	    		sUrl += "&amdt_seq="+formObj.amdt_seq.value;
		   	    		sUrl += "&svc_scp_cd="+formObj.svc_scp_cd.value;
		   	    		ComOpenPopup(sUrl, 600, 320, "Callback_findConvCmdtDefCd", "1,0", true);
		  			break;
		    	case "conv_org_loc_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findConvOrgLocDefCd", "1,0", true);
					break;
		    	case "conv_org_via_loc_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findConvOrgViaLocDefCd", "1,0", true);
					break;
		    	case "conv_dest_via_loc_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findConvDestViaLocDefCd", "1,0", true);
					break;
		    	case "conv_dest_loc_def_cd":	
		    		var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
		  				sUrl += "&group_cmd=" + PRI_SP_SCP;
		  				sUrl += "&location_cmd=LGTCR";
		  				ComOpenPopup(sUrl, 700, 300, "Callback_findConvDestLocDefCd", "1,0", true);
					break;
		    	case "bkg_ts_port_def_cd":	
					var sUrl="/opuscntr/ESM_PRI_4026.do?" + FormQueryString(document.form);
						sUrl += "&location_cmd=L";
						ComOpenPopup(sUrl, 700, 300, "Callback_findBkgTsPortDefCd", "1,0", true);
					break;	
		    	case "bkg_slan_cd":	
		    	...
		    	case "bkg_vvd_cd":	
					var sUrl="/opuscntr/ESM_PRI_4013.do?" + FormQueryString(document.form);
						ComOpenPopup(sUrl, 415, 360, "Callback_findBkgVvdCd", "1,0", true);
					break;	
		    	case "bkg_act_cust_def_cd":
		    		var cust_cnt_cd="";
	   	    		var cust_seq="";
	  ...
	  function Callback_findBkgPorDefCd(rtnVal) {
		  ...
		  if (rtnVal != null){
				...
				var sLastVal = sheetObj.GetCellValue(sheetObj.GetSelectRow(), "bkg_por_tp_cd");
				if(sLastVal != null && sLastVal != "") {
	...
	*/ 	
	  ...
	  */	
	  ...
	  /**
	    * Setting the color of master sheet row. <br>
	    * <br><b>Example :</b>
	    * <pre>
	    *	restylingPagePostTr(sheetObj);
	    * </pre>
	    ...
	    * @version 2009.07.02
	    */
	    function restylingPagePostTr(sheetObj) {
	    	var formObj=document.form;
	   		var sAmdtSeq=formObj.amdt_seq.value;
			var sLgcyIfFlg=formObj.lgcy_if_flg.value;
	    	if (sAmdtSeq == "0") {
       		...
       		for(var i=sheetObj.HeaderRows(); sheetObj.RowCount() > 0 && i <= sheetObj.LastRow(); i++) {
       			var ndCnt=sheetObj.GetCellValue(i, "nd_cnt");
       			var naCnt=sheetObj.GetCellValue(i, "na_cnt");
       			if (ndCnt == 0) {
       				sheetObj.SetCellFont("FontStrike", i, 1, i, sheetObj.LastCol(),true);
    			} else {
    				sheetObj.SetCellFont("FontStrike", i, 1, i, sheetObj.LastCol(), false);
    			}
    			if (naCnt == 0) {
     ...
     * It controls button when Commodity Note has no data. <br>
     * <br><b>Example :</b>
     * <pre>
     * buttonControl()
     * </pre>
     ...
     */
  	function buttonControl(){
 		var formObj=document.form;
 		var amdtSeq=formObj.amdt_seq.value;
 		var row_cnt = sheetObjects[1].RowCount();
 		var propStsCd=formObj.prop_sts_cd.value;
 		var convCfmFlg=formObj.conv_cfm_flg.value;
 		var ofcAuthYn=formObj.ofc_auth_yn.value;
 		try{		
			...
			if(convCfmFlg == "Y" && propStsCd != "F" && row_cnt > 0  && ofcAuthYn == "Y"

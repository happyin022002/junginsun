...
//ESM_PRI_0090
  ...
  /**
	...
	* The array has defined at the top of this page  
	*/
         	...
         	case "sheet3":
         	    with(sheetObj){
                    ...
                    SetColProperty("bkg_prc_cgo_tp_cd", {ComboText:bkgPrcCgoTpCdComboText, ComboCode:bkgPrcCgoTpCdComboValue} );
	              	...
	              	SetColProperty(0 ,"bkg_vvd_cd" , {AcceptKeys:"E", InputCaseSensitive:1});
	              	SetColProperty(0 ,"bkg_act_cust_def_cd" , {AcceptKeys:"E", InputCaseSensitive:1});
	              	
	              	SetRangeBackColor(1,36,1,45,"#555555");
	              	SetSheetHeight(207);
         		}
              	break;
     	}
				...
				case IBSEARCHAPPEND: // Retrieving
	 			...
	 			case IBSEARCH_ASYNC01: // Retrieving
					...
					var sNm=sheetObj.GetComboInfo(0,"chg_rule_def_cd","Text");
		 			...
		 			var sXml=sheetObj.GetSearchData("ESM_PRI_0100GS.do", FormQueryString(formObj));
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified()) {
							...
							var sParam=FormQueryString(formObj);
			  				...
			  				var sXml=sheetObj.GetSaveData("ESM_PRI_0100GS.do", sParam);
			  				sheetObj.LoadSaveData(sXml);
		  				} else {
			  				ComOpenWait(true);
		  					formObj.f_cmd.value=MODIFY;
		  					formObj.note_conv_mapg_id.value=sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"note_conv_mapg_id");
		  					var sXml=sheetObj.GetSaveData("ESM_PRI_0100GS.do", FormQueryString(formObj));
		  					sheetObj.LoadSaveData(sXml);
		  				}
	  ...
	  function sheet3_OnPopupClick(sheetObj, Row, Col, Value) {
	   	...
	   	switch(colname)
	   	{
		    	...
		    	case "bkg_slan_cd":	
					...
					ComOpenPopup(sUrl, 700, 310, "bkg_slan_cd_returnVal", "1,0", true);
	  ...
	  /**
	    ...
	    */
	    function restylingPagePostTr(sheetObj) {
	    	...
	    	var cmdtCurRow=sheetObj.GetSelectRow();
       		for(var i=sheetObj.HeaderRows(); sheetObj.RowCount() > 0 && i <= sheetObj.LastRow(); i++) {
       			var ndCnt=sheetObj.GetCellValue(i, "nd_cnt");
       			var naCnt=sheetObj.GetCellValue(i, "na_cnt");
       			cmdtCurRow=sheetObj.GetCellValue(i, "cmdt_hdr_seq");
       			if (cmdtCurRow < 0) {
    				continue;
    			}
       			if (ndCnt == 0) {
       				sheetObj.SetCellFont("FontStrike", cmdtCurRow, 1, cmdtCurRow, sheetObj.LastCol(),1);
    			} else {
    				sheetObj.SetCellFont("FontStrike", cmdtCurRow, 1, cmdtCurRow, sheetObj.LastCol(),0);
    			}
    			if (naCnt == 0) {
    				sheetObj.SetRowFontColor(cmdtCurRow,"#000000");
    			} else {
    				if(sLgcyIfFlg != "Y") {
    					sheetObj.SetRowFontColor(cmdtCurRow,"#FF0000");
    				}
    			}
       		}
	    }
	     ...
	     */
	  	function buttonControl(){
	 		...
	 		try{		
				...
				if(convCfmFlg == "Y" && propStsCd != "I"  && row_cnt > 0  && ofcAuthYn == "Y"

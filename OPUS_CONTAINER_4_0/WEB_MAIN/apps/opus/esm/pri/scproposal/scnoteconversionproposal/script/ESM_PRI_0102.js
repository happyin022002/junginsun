         	...
         	case "sheet3":
         	    with(sheetObj){
                    ...
                    SetColProperty("pay_term_cd", {ComboText:payTermCdComboText, ComboCode:payTermCdComboValue} );
              		...
              		SetColProperty(0 ,"conv_cmdt_def_cd" , {AcceptKeys:"E|N"});
	...
	function doRowChange(sheetM, sheetD, OldRow, NewRow, OldCol, NewCol, appendRow) {
		...
		if (!isFiredNested && (!openSheet2YN || OldRow != NewRow)) {  
			if(sheetM.GetCellValue(NewRow, "note_clss_cd") =="D") {
				...
				//Item 에서 SURCHARGE 를 selection할 경우 SURCHARGE 항목 활성화
   ...
   */
				...
				case IBSEARCHAPPEND: // Retrieving
	 			...
	 			case IBSEARCH_ASYNC01: // Retrieving
					...
					var sNm=sheetObj.GetComboInfo(0,"chg_rule_def_cd","Text");
		 			...
		 			var sXml=sheetObj.GetSearchData("ESM_PRI_0102GS.do", FormQueryString(formObj));
					...
					//Getting duration exp_dt of selected seq
		  			...
		  			if((ComShowCodeConfirm("PRI00001")) ) {
		  				...
		  				if(sheetObj.IsDataModified()) {
							...
							var sParam=FormQueryString(formObj);
			  				...
			  				var sXml=sheetObj.GetSaveData("ESM_PRI_0102GS.do", sParam);
			  				sheetObj.LoadSaveData(sXml);
		  				} else {
							ComOpenWait(true);
		  					formObj.f_cmd.value=MODIFY;
		  					formObj.note_conv_mapg_id.value=sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"note_conv_mapg_id");
		  					var sXml=sheetObj.GetSaveData("ESM_PRI_0102GS.do", FormQueryString(formObj));
		  					sheetObj.LoadSaveData(sXml);
		  				}
	 			...
	 			case IBINSERT: // Row Add
					var idx=sheetObj.DataInsert();
	 				...
	 				sheetObj.SetCellValue(idx, "eff_dt",sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"eff_dt")==-1 ? "" : sheetObjects[1].GetCellValue(sheetObjects[1].GetSelectRow(),"eff_dt"),0);
 	...
 	function sheet3_OnChange(sheetObj, Row, Col, Value) {
 		...
 		switch(colName)
     	{
 			...
 			case "eff_dt":	
		 		...
		 		//Don't valid in case of legacy data
			...
			case "bkg_prc_cgo_tp_cd": 	
				...
				if(chgRuleDefCd == "APP"){
					if(Value != "DG") {
						ComShowCodeMessage("PRI00333", sheetObj.SetCellText(Row, Col));
	...
	function sheet1_OnSearchEnd(sheetObj, errMsg){
 	...
 	function sheet3_OnSearchEnd(sheetObj, errMsg){ 		
  	 ...
  	 * setStateColor(sheetObj, Row);
  	 * </pre>
  	 * @param {ibsheet} sheetObj Mandatory
       	...
       	switch(colname)
       	{
   	    	...
   	    	case "bkg_cmdt_def_cd":
   	    		var sUrl="/opuscntr/ESM_PRI_4027.do?"
   	  			...
   	  			ComOpenPopup(sUrl, 600, 300, "bkg_cmdt_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "bkg_por_def_cd":	
 	  			...
 	  			ComOpenPopup(sUrl, 700, 325, "bkg_por_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "bkg_pol_def_cd":
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "bkg_pol_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "bkg_pod_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "bkg_pod_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "bkg_del_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "bkg_del_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "conv_cmdt_def_cd":
   	    		var sUrl="/opuscntr/ESM_PRI_4027.do?"
   	   	    		...
   	   	    		sUrl += "&amdt_seq="+formObj.amdt_seq.value;
   	   	    		sUrl += "&svc_scp_cd="+formObj.svc_scp_cd.value;
	  			ComOpenPopup(sUrl, 700, 325, "conv_cmdt_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "conv_org_loc_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "conv_org_loc_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "conv_org_via_loc_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "conv_org_via_loc_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "conv_dest_via_loc_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "conv_dest_via_loc_def_cd_returnVal", "1,0", true);
   	    	...
   	    	case "conv_dest_loc_def_cd":	
 	  			...
 	  			sUrl += "&group_cmd=" + PRI_SP_SCP;
 	  			sUrl += "&location_cmd=LGTCR";
 	  			ComOpenPopup(sUrl, 700, 325, "conv_dest_loc_def_cd_returnVal", "1,0", true);
 	  			
   				break;
   	    	case "bkg_ts_port_def_cd":	
   				var sUrl="/opuscntr/ESM_PRI_4026.do?";
   				var sParam="&location_cmd=L";
   				ComOpenPopup(sUrl+sParam, 700, 325, "bkg_ts_port_def_cd_returnVal", "1,0", true);
    ...
    function bkg_cmdt_def_cd_returnVal(rtnVal) {
    	  if (rtnVal != null){
    		  ...
    		  _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_cmdt_tp_cd",rtnVal.tp,0);
    	  }
    }
      
    function bkg_por_def_cd_returnVal(rtnVal) {
    	if (rtnVal != null){
    		 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_por_def_cd",rtnVal.cd,0);
    		 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_por_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_por_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_por_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_por_def_cd", 0);
			}
		}
    }
	...
	function bkg_pol_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_pol_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_pol_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_pol_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_pol_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_pol_def_cd", 0);
			}
		}  
	}
	...
	function bkg_pod_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue(Row, "bkg_pod_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue(Row, "bkg_pod_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_pod_cnt_cd",rtnVal.cnt_cd,0);
  				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_pod_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_pod_def_cd", 0);
			}
		}
	}
	...
	function bkg_del_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_del_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_del_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_del_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_del_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"bkg_del_def_cd", 0);
			}
		}
	}
	...
	function conv_cmdt_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, _tmp_sheetObject.col,rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_cmdt_tp_cd",rtnVal.tp,0);
		}
	}
	
	function conv_org_loc_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_loc_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_loc_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_loc_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_org_loc_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_org_loc_def_cd", 0);
			}
		} 	  	
	}
	...
	function conv_org_via_loc_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_via_loc_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_via_loc_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_org_via_loc_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_org_via_loc_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_org_via_loc_def_cd", 0);
			}
		} 
	}
	...
	function conv_dest_via_loc_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_via_loc_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_via_loc_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_via_loc_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_dest_via_loc_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_dest_via_loc_def_cd", 0);
			}
		}
	}
	...
	function conv_dest_loc_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_loc_def_cd",rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_loc_tp_cd",rtnVal.tp,0);
			// Pink as background color in case of State
			if(rtnVal.tp == "T"){
				 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "conv_dest_loc_cnt_cd",rtnVal.cnt_cd,0);
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_dest_loc_def_cd", pinkColor);
			} else {
				 _tmp_sheetObject.sheet.SetCellBackColor( _tmp_sheetObject.row,"conv_dest_loc_def_cd", 0);
			}
		}
	}
	...
	function bkg_ts_port_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, _tmp_sheetObject.col, rtnVal.cd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_ts_port_tp_cd",rtnVal.tp,0);
		}
	}
	
	function bkg_slan_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, _tmp_sheetObject.col, rtnVal.toString(),0);
		}
	}
	...
	function bkg_vvd_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, _tmp_sheetObject.col, rtnVal.toString(), 0);
			 _tmp_sheetObject.sheet.SelectCell( _tmp_sheetObject.row, _tmp_sheetObject.col);
		}
	}
	
	function bkg_act_cust_def_cd_returnVal(rtnVal) {
		if (rtnVal != null){
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, _tmp_sheetObject.col, rtnVal.custCntCd + ComLpad(rtnVal.custSeq, 6, "0"),0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_act_cust_cnt_cd",rtnVal.custCntCd,0);
			 _tmp_sheetObject.sheet.SetCellValue( _tmp_sheetObject.row, "bkg_act_cust_seq",rtnVal.custSeq,0);
		}
	}
      /**
 		...
 		if(    chgRuleDefCd != "APP" && chgRuleDefCd != "RAS" && chgRuleDefCd != "TYP" 
 			...
 			if(sheetObj.GetCellValue(Row, "eff_dt") == "") {
 				...
 				return false;
 	 			...
 	 			// In case that application is fixed amount or adjust amound when inputting SURCHARGE CODE
 			...
 			if(sheetObj.GetCellValue(Row, "eff_dt") == "") {
 				...
 				//Allowing TYP|RAR|RAC|RAP|DOR with 0
 				ComShowCodeMessage("PRI00316","Amount");
 				sheetObj.SelectCell(Row, "frt_rt_amt");
 				return false;
 			} else if(sheetObj.GetCellValue(Row, "bkg_rat_ut_cd") == "") {
    ...
    */
 	function buttonControl(){
		...
		try{	
			 ...
			 * 2. In case of not initial status of main
			 * 3. Log in office : 
			 * 4. In case selected note content row's amdt_seq is same with current amdt_seq
			 * 5. in case of no Amend Delete 
			 */		

 ...
 ***************************************************************************************/
    ...
    function loadPage() {
        ...
        var custType = document.form.prc_ctrt_cust_tp_cd.value;
    	if(custType == "N"){
    ...
    function setCellEditable(sheetObj, rowIdx, ieditable){
		...
		for(var i = 0; i <= eColIdx; i++){
			var saveNm = sheetObj.ColSaveName(0, i);
			
			if(saveNm == "ibflag" || saveNm == "type"){
				...
				if(saveNm == "cust_seq"){
					var custType = formObj.prc_ctrt_cust_tp_cd.value;
			    	if(custType == "N"){
			    		sheetObj.SetCellEditable(rowIdx, saveNm, 1);
			    	} else {
			    		sheetObj.SetCellEditable(rowIdx, saveNm, 0);
        ...
        try {
	        ...
	        case IBSEARCH_ASYNC01: // Check
	            ...
	            try {
	                 ...
	                 var sXml=sheetObjects[1].GetSearchData("ESM_PRI_2060GS.do", sParam);
	        ...
	        case IBSAVE: // Save
	            ...
	            try {
	                 ...
	                 var sXml=sheetObj.GetSearchData("ESM_PRI_2060GS.do", sParam);
	                 var backendJobKey=ComGetEtcData(sXml, "BackEndJobKey");
	                 if (backendJobKey.length > 0) {
    ...
    function sheet1_OnLoadExcel(sheetObj, result, code, msg) {
		...
		var iErrCnt = 0;
		for(var i = sIdx; i <= eIdx; i++){
			var typeVal = sheetObj.GetCellValue(i, "type");
			var val = sheetObj.GetCellValue(i, "prefix_nm");
			if(typeVal == "D" || typeVal == ""){
				setCellEditable(sheetObj, i, 0);

			} else {
				setCellEditable(sheetObj, i, 1);
			}
	...
	function getBackEndJobStatus() {
	    ...
	    var sXml=sheet1.GetSearchData("ESM_PRI_2060GS.do", FormQueryString(form));
	...
	function getBackEndJobStatusForCheck(sheetObj, sKey) {
		var form=document.form;	
	    var sXml=sheetObj.GetSearchData("ESM_PRI_2060GS.do?f_cmd=" + COMMAND03 + "&backendjob_key=" + sKey);
	    var jobState=ComGetEtcData(sXml, "jb_sts_flg");
	    if (jobState == "3") {
	    	clearInterval(timer);
        ...
        case IBSEARCH_ASYNC01: // Check
        	...
        	for(var i = fRowIdx; i <= lRowIdx; i++){
        		...
        		if(sType == "U" || sType == "C"){
        			...
        			//(4) Check Rate
        			var rate20 = sheetObj.GetCellValue(i, "rate_20");
        			if (rate20 != "") {
                        if (rate20 != "" && parseFloat(rate20) >= 999999999.99) {
                            add2Tooltip(i, "rate_20", ComGetMsg("PRI00336", "Rate(20)", "999999999.99"));
                            errCnt++;
                        }
                    }
        			var rate40 = sheetObj.GetCellValue(i, "rate_40");
        			if (rate40 != "") {
                        if (rate40 != "" && parseFloat(rate40) >= 999999999.99) {
                            add2Tooltip(i, "rate_40", ComGetMsg("PRI00336", "Rate(40)", "999999999.99"));
                            errCnt++;
                        }
                    }
        			var rate40hc = sheetObj.GetCellValue(i, "rate_40hc");
        			if (rate40hc != "") {
                        if (rate40hc != "" && parseFloat(rate40hc) >= 999999999.99) {
                            add2Tooltip(i, "rate_40hc", ComGetMsg("PRI00336", "Rate(40HC)", "999999999.99"));
                            errCnt++;
                        }
                    }
        			var rate45 = sheetObj.GetCellValue(i, "rate_45");
        			if (rate45 != "") {
                        if (rate45 != "" && parseFloat(rate45) >= 999999999.99) {
                            add2Tooltip(i, "rate_45", ComGetMsg("PRI00336", "Rate(45)", "999999999.99"));
                            errCnt++;
                        }
                    }

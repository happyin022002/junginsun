...
document.onclick=processButtonClick;
	...
	function processButtonClick() {
		...
		try {
			...
			switch(srcName) {
				case "btn_RowAdd":
					var newRowIdx = sheetObj1.DataInsert(-1);
					sheetObj1.SetCellEditable(newRowIdx, "n1st_pod_cd", 1);
				break;
			...
			case "sheet1":
				with(sheetObj) {
					var HeadTitle = "|Seq.|Sel.|SPC|Save|Container No.|TP/SZ|Booking No.|SEAL No.|POL|Last POD|Weight|TP|RF|DG|AK|BB|RD|FM" +
									"|Next VVD|Next VVD|Next VVD|PSA Name|Voyage|Next POD|Final Destination|COP|IOP|OOP|Batch No.|L/Ins.|Send Date" +
									"|del_cntr_yn|n3rd_pod_cd|psa_cre_tp_cd";
					...
					var cols = [{Type:"Status",    Hidden:1,  Width:30,   Align:"Center",  SaveName:"ibflag" },
								...
								{Type:"Text",      Hidden:0,  Width:70,   Align:"Center",  SaveName:"n1st_pod_cd",        Edit:0,         EditLen:5,         AcceptKeys:"E|N",    InputCaseSensitive:1 },
								{Type:"Text",      Hidden:0,  Width:110,  Align:"Center",  SaveName:"n2nd_pod_cd",        Edit:1,         EditLen:5,         AcceptKeys:"E|N",    InputCaseSensitive:1 },
	...
	function doActionIBSheet(sheetObj, formObj, sAction) {
		switch(sAction) {
			case IBCLEAR:      // IBSheet내 Combo initialization
				var sXml = sheetObj.GetSearchData("ESM_BKG_0420GS.do", "f_cmd=" + INIT);
			...
			case IBSEARCH:
				...
				var xmlStr = sheetObj.GetSearchData("ESM_BKG_0420GS.do", FormQueryString(formObj));
			...
			case IBSAVE:
				...
				var xmlStr = sheetObj.GetSaveData("ESM_BKG_0420GS.do", FormQueryString(formObj) + "&" + saveString);
			...
			case COMMAND01:    // TRANSMIT
				...
				var xmlStr = sheetObj.GetSaveData("ESM_BKG_0420GS.do", FormQueryString(formObj));
				backEndJobKey = ComGetEtcData(xmlStr, "backEndJob_Key")    // 전역변수 setting
				if (backEndJobKey != null && backEndJobKey != undefined && backEndJobKey != "" &&
					backEndJobKey.toUpperCase() != "NULL" && backEndJobKey.toUpperCase() != "UNDEFINDE") {
					sheetObj.SetWaitTimeOut(20000);
	...
	function getBackEndJobStatus() {
		...
		var xmlStr = sheetObj2.GetSearchData("ESM_BKG_0420GS.do", "f_cmd=" + SEARCH02 + "&backEndJob_Key=" + backEndJobKey);
		...
		if (jbStsFlg == "3") {
			...
			if (formObj.f_cmd.value == MULTI) {
				xmlStr = "";
				xmlStr = sheetObj2.GetSaveData("ESM_BKG_0420GS.do", "f_cmd=" + MULTI03 + "&backEndJob_Key=" + backEndJobKey);
				clearInterval(timer);
				backEndJobKey = "";
				ComOpenWait(false);
				...
				xmlStr = "";
				xmlStr = sheetObj2.GetSaveData("ESM_BKG_0420GS.do", "f_cmd=" + COMMAND03 + "&backEndJob_Key=" + backEndJobKey);
				clearInterval(timer);
				backEndJobKey = "";
				ComOpenWait(false);
				// 저장 후 재조회 (sheet2_OnSaveEnd에서 재조회를 하게 되면 backEndJob의 ComOpenWait이 정상작동하지 않음으로, 여기에 위치)
			...
			if (SearchRows() > 0) {
				...
				for (var i=HeaderRows(); i<=LastRow(); i++) {
					if (GetCellValue(i, "sav") == "+") {
						...
						//if (GetCellValue(i, "psa_cre_tp_cd") == "A") SetCellEditable(i, "n1st_pod_cd", 1);
						SetRowStatus(i, "");
					}
				}
	...
	function openSpecialInfo() {
		...
		var params = "?pgmNo=ESM_BKG_1012&cntr_no=" + (sheetObjects[0].GetCellValue(sheetObjects[0].GetSelectRow(), "cntr_no") == -1 ? "" : sheetObjects[0].GetCellValue(sheetObjects[0].GetSelectRow(), "cntr_no"))
		...
		ComOpenPopup("ESM_BKG_1012.do?" + params, 715, 450, "", "1,0", true);
	}

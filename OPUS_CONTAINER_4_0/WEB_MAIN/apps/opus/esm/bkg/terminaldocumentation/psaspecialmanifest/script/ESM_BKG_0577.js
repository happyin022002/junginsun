...
var comboCnt=0;
	...
	function initSheet(sheetObj, sheetNo) {
		...
		case "sheet1": // 상단 master 정보
			with (sheetObj) {
				...
				var HeadTitle = "|d_type|vvd_cd|port_cd|eta_d|eta_t|etd_d|etd_t|yd_cd|loc_nm|auto_snd_tp_cd|vsl_cd|vsl_cnt_cd|lloyd_no|call_sgn_no|anr_role_div_cd|ssr_no|ack_rslt_id|local_db_yn";
	...
	function doActionIBSheet(sheetObj, formObj, sAction, row, col) {
		...
		switch (sAction) {
			case SEARCH02: // Retrieve버튼 클릭시 조회
				...
				var xmlStr = sheetObj.GetSearchData("ESM_BKG_0577GS.do", FormQueryString(formObj));
				if (!ComBkgErrMessage(sheetObj, xmlStr)) return;
				var arrXml = xmlStr.split("|$$|");

				if (ComGetEtcData(arrXml[0], "vsl_nm") == "") {
				...
				formObj.frm_vsl_eng_nm.value = ComGetEtcData(arrXml[0], "vsl_nm");
			...
			case MULTI02: // Flat File 생성 및 EDI전송(CANCEL 전송)
				...
				var xmlStr = sheetObj.GetSaveData("ESM_BKG_0577GS.do", FormQueryString(formObj) + "&" + saveString);
	...
	function getBackEndJobStatus() {
		...
		var xmlStr = sheetObj2.GetSearchData("ESM_BKG_0577GS.do", "f_cmd=" + COMMAND02 + "&backEndJob_Key=" + backEndJobKey);
		var jbStsFlg = ComGetEtcData(xmlStr, "jb_sts_flg");

		if (jbStsFlg == "3") {
			xmlStr = "";
			xmlStr = sheetObj2.GetSaveData("ESM_BKG_0577GS.do", "f_cmd=" + COMMAND03 + "&backEndJob_Key=" + backEndJobKey);
			clearInterval(timer);
			backEndJobKey = "";
			ComOpenWait(false);

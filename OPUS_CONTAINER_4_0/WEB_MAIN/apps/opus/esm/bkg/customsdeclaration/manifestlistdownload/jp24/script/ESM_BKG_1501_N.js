		...
		try {
			...
			switch (ComGetEvent("name")) {
				...
				case "btn_new":
					...
					ComBtnDisable("btn_bll_separate");
					ComBtnDisable("btn_bll_combine");
				break;
				...
				case "btn_bl_inquiry":
					...
					ComOpenPopup("ESM_BKG_1502_N_POP.do?pgmNo=ESM_BKG_1502_N" + param, 1100, 640, "", "1,0", true);
				break;
				...
				case "btn_del_transmit":
					...
					// ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll);
					ComOpenPopup("ESM_BKG_1501_01.do?pgmNo=ESM_BKG_1501_01&opnrPgmDiv=1501", 500, 225, "", "1,0", true);
				break;
				...
				case "btn_bll_separate":
					if (!chkOneBlSelect(shtObj2, "chk", "bl_no")) return;  // One B/L Selected Check
					var blNo = shtObj2.GetCellValue(shtObj2.FindCheckedRow("chk").split("|")[0], "bl_no");
					// ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll);
					ComOpenPopup("ESM_BKG_1526_POP.do?pgmNo=ESM_BKG_1526&bl_no=" + blNo, 975, 500, "", "1,0", true);
				break;

				case "btn_bll_combine":
					if (!chkOneBlSelect(shtObj2, "chk", "bl_no")) return;  // One B/L Selected Check
					var blNo = shtObj2.GetCellValue(shtObj2.FindCheckedRow("chk").split("|")[0], "bl_no");
					// ComOpenPopup(sUrl, iWidth, iHeight, sFunc, sDisplay, bModal, b2ndSheet, iRow, iCol, iSheetIdx, sWinName, sScroll);
					ComOpenPopup("ESM_BKG_1527_POP.do?pgmNo=ESM_BKG_1527&bl_no=" + blNo, 975, 500, "", "1,0", true);
				break;
		...
		with (shtObj) {
			switch (shtObj.id) {
				case "sheet1":
						...
						SetSheetHeight(ComGetSheetHeight(shtObj, 6));
				break;
				...
				case "sheet2":
					...
					SetSheetHeight(ComGetSheetHeight(shtObj, 13));
				break;
			}
		}
	}
		...
		if (jbStsFlg == "3") {
			...
			if (frmObj.f_cmd.value == SEARCH01) {
				...
				if (frmObj.search_div.value == "BL") {
					...
					ComBtnEnable("btn_bll_separate");
//					ComBtnEnable("btn_bll_combine");
				}
				return;

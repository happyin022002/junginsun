				...
				case "sheet1":    // 트랜잭션을 위해 사용되는 Dummy Sheet
					with(shtObj){
						...
						var cols = [ {Type:"Status",    Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"ibflag",      Wrap:1 },
									 {Type:"Text",     Hidden:0,  Width:170,  Align:"Center",    ColMerge:0,   SaveName:"err_code",  Wrap:1 } ];
						InitColumns(cols);
						SetEditable(0);
	...
	function doActionIBSheet(shtObj, frmObj, sAction) {
		switch(sAction) {
			case IBSEARCH:    //조회
				...
				var param = "f_cmd=" + SEARCH04 + "&bl_no="+ frmObj.bl_no.value +
							"&vvd_pod_cd=" + frmObj.vvd_pod_cd.value +
							"&cntr_no=" + frmObj.cntr_no.value +
							"&vvd=" + frmObj.vvd.value;
				//shtObj.DoSearch("ESM_BKG_1505GS.do", FormQueryString(frmObj) );
				shtObj.DoSearch("ESM_BKG_1518GS.do?"+param, "" );
				break;
		}
	}
/* 개발자 작업  끝 */

...
function processButtonClick() {
	...
	case "btn_prevDoc_pop":
		...
		ComOpenPopup("/opuscntr/ESM_BKG_1146.do?"+param, 600, 450, "", "1,0,1,1,1,1", true, true);
		break;		
	}// end switch
}
/**
 ...
 * @return
 */
function vvd_OnChange(comboObj, OldIdx, OldTxt, OldCod, NewIdx, NewTxt, NewCod) {
	
    var formObj=document.form;
    if (NewTxt != null) {
        formObj.cstms_yd_cd.value = comboObj.GetText(NewCod, 1);
    } else {
		...
		case "t1sheet1":
			with(sheetObj){
				...
				var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
				 ...
				 {Type:"Seq",       Hidden:0, Width:50,   Align:"Center",  ColMerge:1,   SaveName:"Seq" },
				 {Type:"Text",      Hidden:1, Width:70,   Align:"Center",  ColMerge:1,   SaveName:"vvd",            KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
				 {Type:"Text",      Hidden:1, Width:80,   Align:"Center",  ColMerge:1,   SaveName:"cstms_port_cd",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
				 ...
				 {Type:"Text",      Hidden:0, Width:150,  Align:"Center",  ColMerge:1,   SaveName:"seal_no",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:20 },
				 {Type:"Text",      Hidden:0, Width:130,  Align:"Center",  ColMerge:1,   SaveName:"seal_no2",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:20 },
				 ...
				 {Type:"Combo",     Hidden:0, Width:120,  Align:"Center",  ColMerge:1,   SaveName:"full_mty_cd",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
				 ...
				 {Type:"Text",      Hidden:0, Width:50,   Align:"Center",  ColMerge:1,   SaveName:"pck_tp_cd",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:2 },
				 ...
				 {Type:"Text",      Hidden:0, Width:50,   Align:"Center",  ColMerge:1,   SaveName:"meas_ut_cd",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:3 } ];
		...
		case "sheet2":
			with(sheetObj){
				...
				var cols = [ {Type:"Status",    Hidden:0, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
				 ...
				 {Type:"Text",      Hidden:0,  Width:50,   Align:"Center",  ColMerge:0,   SaveName:"seal_pty_nm",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
				 {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:1,   SaveName:"vvd",             KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
...
function doActionIBSheet(sheetObj, formObj, sAction, Row, Col) {
	...
	case IBSEARCH: // Retrieve
		...
		var sXml=sheetObj.GetSearchData("ESM_BKG_1124GS.do",FormQueryString(formObj));
	...
	case SEARCH01: // 조회
		...
		var sXml=sheetObj.GetSearchData("ESM_BKG_1124GS.do", params);
...
//			comboObjects[0].MultiSeparator = "|";
			...
			ComXml2ComboItem(sXml, comboObjects[0], "vvd",
			...
			comboObjects[0].SetSelectText(vvd[0]);
			return;
		}
		break;
	...
	case IBRESET: // New
		...
		if (start_bl_no_flag) { //초기 값을 전달 받았을 경우
			...
			// vvd.RemoveAll();
			// vvd.SetSelectIndex(0);
		}
		formObj.reset();
		formObj.cstms_yd_cd.value=cstms_yd_cd_tmp;
		document.form.wgt_ut_cd.SetSelectCode("KGS");
		document.form.meas_ut_cd.SetSelectCode("CBM");
	...
	case IBSAVE: // Save
		...
		// "t3_"));
		var sXml=sheetObj.GetSaveData("ESM_BKG_1124GS.do", params);
		var State=ComGetEtcData(sXml, ComWebKey.Trans_Result_Key);
		if (State == "S") {
//			sheetObj.RenderSheet(0);
		...
		if (sheetID == 't1sheet1') {
			...
			for ( var i=1; i <= maxLength; i++) {
				if (sheetObjects[3].GetCellValue(i, "cntr_no").trim() == sheetObjects[2].GetCellValue(selectedRow, "cntr_no").trim()) {
					sheetObjects[3].SetRowHidden(i,1);// 선택된 행 숨기기
					sheetObjects[3].SetRowStatus(i,"D");// 선택된 행 삭제 flag를 셋팅
				}
			}
			...
			for ( var i=1; i <= maxLength; i++) {
				if (sheetObjects[5].GetCellValue(i, "cntr_no").trim() == sheetObjects[2].GetCellValue(selectedRow, "cntr_no").trim()) {
					sheetObjects[5].SetRowHidden(i,1);// 선택된 행 숨기기
					sheetObjects[5].SetRowStatus(i,"D");// 선택된 행 삭제 flag를 셋팅
				}
			}
		}
		break;
...
function doActionValidationResult(sheetObj, sKey) {
	var sXml=sheetObj.GetSearchData("ESM_BKG_1106GS.do?f_cmd=" + SEARCH03
			+ "&key=" + sKey);
...
function t1sheet1_OnChange(sheetObj, row, col, val) {
	...
	if(col_save_name=="seal_no2"||col_save_name=="pck_qty"||col_save_name=="meas_qty"){
	...
	if (sheetObj.ColSaveName(col) == "cntr_no"
		...
		for ( var i=1; i <= sheetObj.LastRow(); i++) {
			if (i == row)
				continue;
			if (sheetObj.GetCellValue(i, "cntr_no") == sheetObj.GetCellValue(row,
					"cntr_no")) {
				// "Container No. [{?msg1}] is duplicated. Check container
				// number.
				ComShowCodeMessage("BKG00965", sheetObj.GetCellValue(row,
						"cntr_no"));
				sheetObj.SetCellValue(row, "cntr_no","",0);
				sheetObj.SelectCell(row, "cntr_no");
				return;
	...
	if (col_save_name == "seal_no" || col_save_name == "seal_no2") {
		...
		if (len <= 1) {
			if (val != '') {
				...
				fmObject.SetCellValue(newRow, "seal_no",val.toUpperCase(),0);
			} else {
				if (col_save_name == "seal_no") {
					fmObject.SetCellValue(arrRow[0], "seal_no",val.toUpperCase(),0);
				} else {
					fmObject.SetCellValue(arrRow[0], "seal_no",val.toUpperCase(),0);
				}
			}
			...
			if (col_save_name == "seal_no") {
				...
				fmObject.SetCellValue(arrRow[1], "seal_no",val.toUpperCase(),0);
			}
		}
...
function GetAddRowIndex(sheetObj) {
	...
	sheetObj.SetCellValue(row, "vvd",document.form.vvd.value,0);
...
//		var win=window.open("","ESM_BKG_0706","width="
//								+ width
//								+ ",height="
//								+ height
//								+ ",left="
...
//		frm2.submit();
        ...
        "&gds_desc=" + sheetObj.GetCellValue(row, "cntr_mf_gds_desc");
           ComOpenPopup(url,"600","600","callbackMFDesc","1,0",true);		
		break;
	} // end switch
}
...
function showAndHideSheet(sheetObj, colName, colValue) {
	...
	for (rn=1; rn < rcnt; rn++) {
		...
		if (sheetObj.GetRowStatus(rn) != 'D'
			&& sheetObj.GetCellValue(rn, colName) == colValue) {
...
function findText(sheetObj, colName, colValue) {
	...
	for (ix=1; ix < rcnt; ix++) {
		if (sheetObj.GetRowStatus(ix) != 'D'
			&& sheetObj.GetCellValue(ix, colName) == colValue) {
			idxs.push('' + ix);
		}
	}
	return idxs;

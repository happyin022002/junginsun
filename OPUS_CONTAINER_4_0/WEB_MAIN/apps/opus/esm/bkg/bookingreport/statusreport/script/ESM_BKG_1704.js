 ...
 *@FileName   : ESM_BKG_1704.js
 *@FileTitle  : Container Discrepancy Report
 *@author     : CLT
 *@version    : 1.0
 *@since      : 2015/01/19
=========================================================*/
...
/*********************** KEY EVENT END ********************/
    ...
    function doActionIBSheet(sheetObj,formObj,sAction,Row,Col,PageNo,subGubun) {
		...
		case IBCLEAR:	
			...
			var sXml=sheetObj.GetSearchData("ESM_BKG_1704GS.do", FormQueryString(formObj));
		...
		case IBSEARCH:
				...
				formObj.target="_top";
	            formObj.action="ESM_BKG_1704DL.do?"+FormQueryString(formObj);
	            formObj.submit();
    		...
    		case IBSEARCH:
    			if (ComIsNull(formObj.vvd_cd)) {
    				ComShowCodeMessage('BKG00626','VVD');
 					...
 					return false;
    			}else if ((formObj.io_bnd_cd.value=="O" && ComIsNull(formObj.pol_cd) ) 
   					   || (formObj.io_bnd_cd.value=="I" && ComIsNull(formObj.pod_cd) ) ) {
 	    			if(formObj.rdo1_1.checked) {
 	    				ComShowCodeMessage('BKG00627','If you select Outbound You must enter POL.');
 	    				formObj.pol_cd.focus();
 	    			} else {
 	    				ComShowCodeMessage('BKG00627','If you select Inbound You must enter POD.');
...
/*############################# combo onchage end ########################*/			
	...
	function initAll(formObject){
		...
		ComBkgXml2ComboItem(arrMultiCombo[4], deli_mode, 	"val", "name");

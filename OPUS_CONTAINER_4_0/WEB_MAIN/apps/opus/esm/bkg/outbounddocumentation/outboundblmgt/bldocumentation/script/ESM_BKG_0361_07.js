 			...
 			case 1:      //sheet1 init
 			    with(sheetObj){
		 		      ...
		 		      var cols = [ {Type:"Status",    Hidden:0, Width:20,   Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
		 		             ...
		 		             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"mx_shpr_tax_id",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
		 		             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"mx_cnee_tax_id",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
		 		             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"mx_ntfy_tax_id",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 } ];
 			...
 			case IBSEARCH:      //retrieve
 				...
 				if(validateForm(sheetObj, formObj, sAction)) {
 					formObj.f_cmd.value=SEARCH;
  					sheetObj.DoSearch("ESM_BKG_0361_07GS.do", FormQueryString(formObj) );
 				}
 		 		ComOpenWait(false);
 			break;
 			case IBSAVE:        
				if(validateForm(sheetObj,formObj,sAction)) {
					var tmpSaveStr = sheetObj.GetSaveString();
					
					if(formObj.name=="form"){
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "exp_"); 					
						sheetObj.SetCellValue(1,"cnt_cd",ComGetObjValue(formObj.exp_cnt_cd));	//MX, CO, EC, PE
						ComSetObjValue(formObj.go_cnt_cd, ComGetObjValue(formObj.exp_cnt_cd));
 					}else if(formObj.name=="form2"){ 						
 						IBS_CopyFormToRow(formObj,sheetObj, 1, "imp_"); 					
						sheetObj.SetCellValue(1,"cnt_cd","MX");
						ComSetObjValue(formObj.go_cnt_cd, ComGetObjValue(formObj.imp_cnt_cd));
 					}
					if(sheetObj.GetCellValue(1,0)=='I'&&sheetObj.GetCellValue(1,"mx_shpr_tax_id")==''
						&&sheetObj.GetCellValue(1,"mx_cnee_tax_id")==''&&sheetObj.GetCellValue(1,"mx_ntfy_tax_id")==''){
						sheetObj.RowDelete(1, 0);
 						//sheetObj.SetCellValue(1,0,'D');
 					}
					if(sheetObj.GetCellValue(1,0)=='U'&&sheetObj.GetCellValue(1,"mx_shpr_tax_id")==''
						&&sheetObj.GetCellValue(1,"mx_cnee_tax_id")==''&&sheetObj.GetCellValue(1,"mx_ntfy_tax_id")==''){
 						sheetObj.SetCellValue(1,0,'D');
 					}
...
// 						if (!sheetObj.IsDataModified() && SaveStr == "")	return;
 						if ((sheetObj.IsDataModified() && tmpSaveStr == "")||(!sheetObj.IsDataModified() && SaveStr == ""))	return;
 						
						if(confirm(ComGetMsg("BKG00962"))){
					 		ComOpenWait(true);
	 						var rtnData = sheetObj.GetSaveData("ESM_BKG_0361_07GS.do", FormQueryString(formObj)+"&"+SaveStr ,"");

...
*@FileName   ESM_BKG_1167.jsp
*@FileTitle  : CA Cargo Release 
*@author     : CLT
*@version    : 1.0
*@since      : 2014/05/06
=========================================================*/
	...
	// t6sheet1, t6sheet2, t6sheet3, t6sheet4, t6sheet5, t6sheet6, t6sheet7
    ...
    switch(srcName) {
        ...
        case "btn_Ivc": //Multi TDC 315 sending
        	...
        	//DEL 이 US인 경우 US cargo release 사용 alert
        	if(!fncDestChk()){ 
          	   break;   
             }   
    ...
    switch(sAction) {
        case IBSEARCH:      //retrieve
            ...
            if (sheetObj.id=="master"){
            	...
            	sheetObj.DoSearch("ESM_BKG_1167GS.do",FormQueryString(formObj) + "&" + ComGetPrefixParam("master_") , {Sync:2} );
            }
        ...
        case IBSEARCH_ASYNC01:
            ...
            //parameter changed[check again]CLT             
            var sXml=sheetObj.GetSaveData("ESM_BKG_1167GS.do", sParam);
            //parameter changed[check again]CLT 			
		...
		case MULTI04:
			...
			var xmlStr=sheetObjects["master"].GetSaveData("ESM_BKG_1167GS.do", params);
...
//			var sXml=sheetObj.GetSaveData("ESM_BKG_1167GS.do", params);
//	    	
//	    	sheetObj.LoadSaveData(sXml);
//			ComOpenWait(false);
//
...
function getBackEndJobStatus() {
	...
	var xmlStr = sheetObj.GetSearchData("ESM_BKG_1167GS.do", "f_cmd=" + COMMAND02 + "&backEndJob_Key=" + backEndJobKey);
	var jbStsFlg = ComGetEtcData(xmlStr, "jb_sts_flg");

	if (jbStsFlg == "3") {
		ComOpenWait(false);
		sheetObj.LoadSaveData(sheetObj.GetSaveData("ESM_BKG_1167GS.do", "f_cmd=" + COMMAND03 + "&backEndJob_Key=" + backEndJobKey), {Sync:1});
		clearInterval(timer);
		backEndJobKey = "";
...
function fncSearchSheet2(){
    ...
    //parameter changed[check again]CLT     
    sheetObj.DoSearch("ESM_BKG_1167GS.do",FormQueryString(formObj) + "&" + ComGetPrefixParam("container_") );
    return;
}
/**
...
function container_OnSearchEnd(sheetObj, ErrMsg){
    ...
    for(var i=0;i<formObj.info_frt_clt_flg.options.length;i++){
    	if(sheetObjects["master"].GetCellValue(Row, prefix + "frt_clt_flg")== null ||
    			sheetObjects["master"].GetCellValue(Row, prefix + "frt_clt_flg") == ""){
...
function fncGetErpDem(sheetObj,formObj){
    ...
    //parameter changed[check again]CLT     
    var sXml=sheetObj.GetSearchData("ESM_BKG_1167GS.do", param);
...
function fncGetBLStatus(){
    ...
    //parameter changed[check again]CLT    
    var sXml=sheetObj.GetSearchData("ESM_BKG_1167GS.do",FormQueryString(formObj) + "&" + ComGetPrefixParam("sheet_bl_status_"));
...
function fncBtnSave(){
   ...
   // selRow=sheetObjMaster.GetSelectRow();
    ...
    if(del_cd.indexOf("US") == 0 ){
    	ComShowCodeMessage("BKG08326","US","US");//O/BL receipt update is not allowed for canada destination cargo. please use CA cargo release to update it.
    	return;
    }    
    ...
    if(sheetObjCgoRlse.RowCount() > 0){
        ...
        sheetObjCgoRlse.DoSave("ESM_BKG_1167GS.do"
        ...
        sheetObjOblByCgo.DoSave("ESM_BKG_1167GS.do"
            ,FormQueryString(formObj) + "&" + sParam + "&" + ComGetPrefixParam(aryPrefix)
            ,-1
            ,false);
    }
...
function fncBtnHistory(){
    ...
    param += "1=1";
    param += "&bl_no="+sheetObjMaster.GetCellValue(selRow,"master_" + "bl_no")+"&cnt_cd=CA";
...
function fncTestObl(){
    ...
    sheetObj.DoAllSave("ESM_BKG_1167GS.do",FormQueryString(formObj),false);
}
/*****************************************************************
*
*****************************************************************/
...
function fncDestChk(){
    ...
    for(i= 1; i < rCnt;i++){
      ...
      if(sheetObjects["master"].GetCellValue(i, "chk") == 1){
    	  ...
    	  if(delCd == "US"){
			chkCnt++;	
			usBls += sheetObjects["master"].GetCellValue(i, "master_" + "bl_no")+" ";
    ...
    if(chkCnt >= 1) 
    {	ComShowCodeMessage("BKG08326", "US","US", usBls); //O/BL receipt update is not allowed for {?msg1} destination cargo.\nplease use {?msg2} cargo release to update it. ({?msg3}).
      return false;}
    else{
      return true;
    }

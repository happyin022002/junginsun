    		...
    		switch(sAction) {
    			case IBSAVE:        //저장//    				
    		        ...
    		        //sheetObj.SetEtcData("BatchStatus","");
     		        //sheetObj.DoSearch("ESM_COA_0184GS.do", coaFormQueryString(formObj) );
    		        //var BatchStatus=sheetObj.GetEtcData("BatchStatus");
    		        
                    var sXml=sheetObj.GetSearchData("ESM_COA_0184GS.do", coaFormQueryString(formObj) );    
    				...
    				switch(BatchStatus){
    					case "R": // 작업 submit	
    						...
    						break;
    					case "P"://해당 작업이 진행 중 
    						ComShowCodeMessage("COA00003", "Average RPB Creation");
    						ComOpenWait(false);  
    						break;
    					default: break;							
    				}  
       ...
       /**
        ...
        function monitoringBatchJob(){
        	...
        	formObj.f_cmd.value=SEARCH02;
         	var sXml=sheetObj.GetSearchData("ESM_COA_0184GS.do", FormQueryString(formObj));
        	var BatchStatus=ComGetEtcData(sXml, "BatchStatus");
        	if( BatchStatus == "P" ){

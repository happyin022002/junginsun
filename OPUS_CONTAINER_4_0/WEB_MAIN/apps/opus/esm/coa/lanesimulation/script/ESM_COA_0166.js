    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            case IBSEARCH:      //조회
                ...
                sheetObj.DoSearch("ESM_COA_0166GS.do", coaFormQueryString(formObj));
                ComOpenWait(false);
                break;
...
//                    if(freq_no > (parseInt(sheetObj2.RowCount)-1)){
                    ...
                    if(sheetObj.CellValue(arrIndex[j],"vop_cd") == "vop_cd") sheetObj2.CellValue2(l, "vop_flg") = "1";
			  ...
			  for(j=i+1;j<arrStr.length;j++) {
			     if(arrStr[i].length > 0 && arrStr[j].length >0) {
			       if(arrStr[i] == arrStr[j]) {
						   ComShowMessage(ComGetMsg("COA10008","same Vessels"));

	...
	function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            ...
            case IBSEARCH:      //조회
                ...
                sheetObj.DoSearch4Post("ESM_COA_0053GS.do", coaFormQueryString(formObj));
                ComOpenWait(false);
                break;
...
//            formObj.sRow.value = row;
            ...
            // [COA10015] : Proforma SKD를 먼저 확인하세요.
            ComShowMessage(ComGetMsg("COA10015","Proforma SKD"));
            sheetObj1.RemoveAll();
        }
    }
	...
	function doActionIBSheet1(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {
            case IBSEARCH:      //조회
                ...
                var sXml = sheetObj2.GetSearchXml("ESM_COA_0053GS2.do?f_sect_no="+sect_no, coaFormQueryString(formObj));
            ...
            case IBSAVE:        //저장
                ...
                if(sheetObj1.SelectRow < 1){
                    // [COA10005] = Lane Summary 에 조회된 데이터가 존재하지 않습니다. Lane Summary 을(를) 먼저 조회하세요.
                    ComShowMessage(ComGetMsg("COA10005","Lane Summary","Lane Summary"));
                    return false;
                }
                ...
                var savexml = sheetObj2.GetSaveXml("ESM_COA_0053GS.do?f_sect_no="+sect_no, ComGetAryJoin(params,"&")+"&"+coaFormQueryString(formObj));
	...
	function doActionIBSheet3(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0053GS3.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
			...
			case IBSAVE:        //저장
				...
				sheetObj.DoAllSave("ESM_COA_0053GS3.do", coaFormQueryString(formObj));
				ComOpenWait(false);
	...
	function doActionIBSheet4(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0053GS4.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
...
//				sheetObj.DoSearch4Post("ESM_COA_0053GS4.do", coaFormQueryString(formObj));
				sheetObj.DoAllSave("ESM_COA_0053GS4.do", coaFormQueryString(formObj));
				ComOpenWait(false);
                var Result = sheetObj.EtcData("result");
                if (Result=="OK"){
                      ComShowMessage(ComGetMsg("COA10006"));
    ...
    function callVslConsum(){
        ...
        if(sheetObj.SelectRow <2){
            ComShowMessage(ComGetMsg("COA10002","Vessel Class"));
        } else {
            ...
            ComOpenWindowCenter("/opuscntr/ESM_COA_0151.do"+param,"PopupEsmCoa0151",400,500,false);
        }
    }
...
// Step3 : Tab4 : Hire/TC Rev/Layup(Daily) --------------------------------------------
	...
	function doActionIBSheet5(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0053GS5.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
			...
			case IBSAVE:        //저장
				...
				sheetObj.DoAllSave("ESM_COA_0053GS5.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				doActionIBSheet5(sheetObj,formObj,IBSEARCH);
				break;
	...
	function callTCO() {
	    ...
	    ComOpenWindowCenter("/opuscntr/ESM_COA_0167.do","PopupEsmCoa01671",400,350,false);
//	    noRtnPopup("ESM_COA_167.do"+param, "width=400, height=500,menubar=0,status=0,scrollbars=0,resizable=0");
	}
	...
	function doActionIBSheet6(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0053GS6.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
	...
	function doActionIBSheet7(sheetObj,formObj,sAction) {
		...
		switch(sAction) {
			case IBSEARCH:      //조회
				...
				sheetObj.DoSearch4Post("ESM_COA_0053GS7.do", coaFormQueryString(formObj));
				ComOpenWait(false);
				break;
    	 ...
    	 var formObj = document.form;
      	...
      	if (ComGetObjValue(obj) != "") {
         	var simNo = ComGetObjValue(formObj.f_sim);
 	 		...
 	 		var sXml = sheetObj.GetSearchXml("ESM_COA_0053GS3.do", coaFormQueryString(formObj));
 	 		var arrXml = sXml.split("|$$|");
		...
		with(formObj){
			...
			if(tabObjects[0].SelectedIndex==3 && sAction == COMMAND01){
				if(ComGetObjValue(formObj.f_fm_yyyymm) == ""){
				    ComShowMessage(ComGetMsg("COA10002","Source Period"));
			    ...
			    if(parseInt(ComGetObjValue(f_to_yyyymm).substring(5)) - parseInt(ComGetObjValue(f_fm_yyyymm).substring(5)) > 2){
			        //msgs['COA10003'] = msg1 + '는(은) ' + msg2 + '만 처리할수 있습니다.';
    			    ComShowMessage(ComGetMsg("COA10003","Creation", "3 month"));
    			    ComSetFocus(f_to_yyyymm);
    			    return false;
			    }
			}
            ...
            if(tabObjects[0].SelectedIndex==2 && sAction == IBCREATE){
                ...
                if(values == true){
                	ComShowMessage(ComGetMsg("COA10014",""));
                    return false;
                }
            }

	...
	with (sheetObj) {
		...
		case "sheet1":
           ...
           //전체Edit 허용 여부 [선택, Default false]
			...
			var HeadTitle1 = "Select|Code|Name|Name|gen_expense_group_cd|TIC ";
			...
			InitDataProperty(0 ,cnt++ , dtCheckBox , 60 , daCenter, true , "delChk" , false , "" , dfNone , 0 , true , true , 400 , false , true , "" , true );
			InitDataProperty(0, cnt++ , dtData,			70,		daCenter,	true,		"gen_expn_cd",			false,		"",			dfNone,		0,			false,		false);
			InitDataProperty(0, cnt++ , dtHidden,			450,	daLeft,		true,		"eng_abbr_nm",			false,		"",			dfNone,		0,			false,		false);
			InitDataProperty(0, cnt++ , dtHidden,			450,	daLeft,		true,		"krn_abbr_nm",			false,		"",			dfNone,		0,			false,		false);
			InitDataProperty(0, cnt++ , dtHidden,			450,	daLeft,		true,		"gen_expn_group_cd",			false,		"",			dfNone,		0,			false,		false);						
			InitDataProperty(0, cnt++ , dtData,			50,		daCenter,	true,		"tic_cd",			false,		"",			dfNone,		0,			false,		false);
			
			var langDiv = frm.lang_div.value;
			if (langDiv == "KOR") {
				sheet1.ColHidden("eng_abbr_nm")  = true;
				sheet1.ColHidden("krn_abbr_nm")  = false;
			} else if(langDiv == "ENG") {
				sheet1.ColHidden("eng_abbr_nm")  = false;
				sheet1.ColHidden("krn_abbr_nm")  = true;				
			}
		...
		case "btn_Select":
			...
			if (arrRow != null) {
				...
				for ( var i = 0; i < arrRow.length ; i++) {
					...
					var gen_expn_cd = 
						sheet1.CellValue(row, "gen_expn_cd");
					var eng_abbr_nm = 
						sheet1.CellValue(row, "eng_abbr_nm");
					var krn_abbr_nm = 
						sheet1.CellValue(row, "krn_abbr_nm");
					var gen_expn_group_cd = 
						sheet1.CellValue(row, "gen_expn_group_cd");					
					var tic_cd = 
						sheet1.CellValue(row, "tic_cd");
					...
					expenseInfo.ofc_cd = frm.ofc_cd.value;
					expenseInfo.gen_expn_cd = gen_expn_cd;
					expenseInfo.eng_abbr_nm = eng_abbr_nm;
					expenseInfo.krn_abbr_nm = krn_abbr_nm;
					expenseInfo.gen_expn_group_cd = gen_expn_group_cd;
					expenseInfo.tic_cd = tic_cd;
					expenseInfoList[i] = expenseInfo;
					
				}
				
				opener.setExpenseCode(expenseInfoList);		
				
				self.close();
				
			} else {
				//msgs['COM12114'] = 'Please check {?msg1}';
				ComShowCodeMessage("COM12114" , "select");	
			}

			
			break;
		case "btn_RequestOfExpenseCode":
			var contents = "";
...
function doActionIBSheet(sAction) {
	...
	if (sAction == SEARCHLIST) {		
		...
		frm.gen_expn_group_cd.value = combo1.Code;
		var sXml = sheet1.GetSearchXml("CPS_GEM_0101GS.do", FormQueryString(frm));	

 ...
 *@FileName : cps_gem_0001_03t.js
 *@FileTitle : [CPS_GEM-0001_03] Expense Adjustment
 *Open Issues :
 *Change history :
	...
	with (sheetObj) {
		...
		case "sheet1":
           ...
           //전체Edit 허용 여부 [선택, Default false]
            ...
            var HeadHidden = "|to_gen_expn_itm_no|to_gen_expn_itm_desc|cre_usr_id|fm_ut_val|to_ut_val|fm_usd_locl_xch_rt|fm_locl_krw_xch_rt|fm_ut_val|fm_usd_krw_xch_rt|to_usd_locl_xch_rt|to_locl_krw_xch_rt|to_usd_krw_xch_rt|gen_expn_rqst_tp_cd|gen_expn_apro_auth_ofc_cd|crnt_gen_expn_apro_step_cd|gen_expn_rqst_seq|fm_eng_abbr_nm|to_eng_abbr_nm|fm_krn_abbr_nm|to_krn_abbr_nm|rqst_opin_rmk|req_upd_dt|itm_upd_dt";
 ...
 */                                                                                                                                                                                                                                                                                                  
 function setSheet1ToForm2( srow ) {   
		...
		if (arrXml.length > 1) {		
			...
			sheet4.CellValue2(row, "title") = "Assigned";
		 	sheet4.CellValue2(row ,"jan_amt") = item["jan_amt"];
		 	sheet4.CellValue2(row ,"feb_amt") = item["feb_amt"];
		 	sheet4.CellValue2(row ,"mar_amt") = item["mar_amt"];
		 	sheet4.CellValue2(row ,"apr_amt") = item["apr_amt"];
		 	sheet4.CellValue2(row ,"may_amt") = item["may_amt"];
		 	sheet4.CellValue2(row ,"jun_amt") = item["jun_amt"];
		 	sheet4.CellValue2(row ,"jul_amt") = item["jul_amt"];
		 	sheet4.CellValue2(row ,"aug_amt") = item["aug_amt"];
		 	sheet4.CellValue2(row ,"sep_amt") = item["sep_amt"];
		 	sheet4.CellValue2(row ,"oct_amt") = item["oct_amt"];
		 	sheet4.CellValue2(row ,"nov_amt") = item["nov_amt"];
		 	sheet4.CellValue2(row ,"dec_amt") = item["dec_amt"];	
		...
		if (arrXml.length > 2) {		
			...
			//Edit ===========================
			var janCol = sheet4.SaveNameCol("jan_amt");
			var decCol = sheet4.SaveNameCol("dec_amt");
			var editRow = 0;
			...
			for ( var i = 0; i < list.length; i++) {
				...
				if (gen_expn_apsts_cd == "RQ" || gen_expn_apsts_cd == "AD") {
					if (gen_expn_rqst_tp_cd == "EI" && init_clz_flg == "N") {
						for ( var j = janCol ; j <= decCol; j++) {		
							sheet4.CellEditable(row,j) = true;
						}
				...
				var step = aproStep["gen_expn_apro_step_cd"];
			 	...
			 	//저장ROW
			 	sheet4.CellValue2(row ,"gen_expn_apsts_cd") = gen_expn_apsts_cd;			 	
				
				sheet4.CellValue2(row ,"gen_expn_apro_step_cd") = step;
				
				sheet4.CellValue2(row, "title") = getStepOffice(step);				
			 	sheet4.CellValue2(row ,"jan_amt") = aproStep["jan_amt"];
			 	sheet4.CellValue2(row ,"feb_amt") = aproStep["feb_amt"];
			 	sheet4.CellValue2(row ,"mar_amt") = aproStep["mar_amt"];
			 	sheet4.CellValue2(row ,"apr_amt") = aproStep["apr_amt"];
			 	sheet4.CellValue2(row ,"may_amt") = aproStep["may_amt"];
			 	sheet4.CellValue2(row ,"jun_amt") = aproStep["jun_amt"];
			 	sheet4.CellValue2(row ,"jul_amt") = aproStep["jul_amt"];
			 	sheet4.CellValue2(row ,"aug_amt") = aproStep["aug_amt"];
			 	sheet4.CellValue2(row ,"sep_amt") = aproStep["sep_amt"];
			 	sheet4.CellValue2(row ,"oct_amt") = aproStep["oct_amt"];
			 	sheet4.CellValue2(row ,"nov_amt") = aproStep["nov_amt"];
			 	sheet4.CellValue2(row ,"dec_amt") = aproStep["dec_amt"];
			...
			if (gen_expn_rqst_tp_cd != "EI") {							
				...
				for ( var i = janCol ; i <= start; i++) {		
					sheet4.CellValue2(editRow,i) = 0;
				}
			} 
			
			frm2.fm_rq_amt.value = sheet4.CellText(2, "ttl");	
			frm2.fm_ad_amt.value = sheet4.CellText(editRow ,"ttl");
...
function setSheet1ToForm( srow ) {   
	...
	frm1.to_usd_amt.value  = 
		ComAddComma(getUsdAmt(frm1.fm_rq_amt.value,frm1.to_ut_val.value,frm1.to_usd_locl_xch_rt.value));
	*/	
	...
	frm1.to_ad_usd_amt.value  = frm1.fm_ad_usd_amt.value;
	*/
...
function processButtonClick() {
	...
	case "t3_btn2_New":
		...
		if (ComShowCodeConfirm("GEM00015")) {	
		    ...
		    if (frm6.popup_yn.value == "N") {
				...
				//윈도우 사이즈가 안늘어나서 탭이동후 원래탭으로 설정
				var tab1 = window.parent.document.getElementById("tab1");
	...
	case "t3_btn2_Down_Excel":
		if (sheet1.RowCount > 0 ) {
			var columnSkipList = "ibflag|to_gen_expn_itm_no|to_gen_expn_itm_desc|cre_usr_id|fm_ut_val|to_ut_val|fm_usd_locl_xch_rt|fm_locl_krw_xch_rt|fm_ut_val|fm_usd_krw_xch_rt|to_usd_locl_xch_rt|to_locl_krw_xch_rt|to_usd_krw_xch_rt|gen_expn_rqst_tp_cd|gen_expn_apro_auth_ofc_cd|crnt_gen_expn_apro_step_cd|gen_expn_rqst_seq|fm_eng_abbr_nm|to_eng_abbr_nm|fm_krn_abbr_nm|to_krn_abbr_nm|rqst_opin_rmk";
			sheet1.Down2Excel(1,false,false,true,"","",false,false,"Expense Request - Adjustment",false,columnSkipList);
		}
		break;	
	case "btn_Retrieve":
		doActionIBSheet(SEARCHLIST);
	...
	case "btn_Print":		
		...
		if (gen_expn_rqst_tp_cd == "EI") {
		    frm5.rfn.value = "/CPS_GEM_0025.do?lang_div="+langDiv+"&gen_expn_rqst_no="+gen_expn_rqst_no+"&gen_expn_rqst_seq="+gen_expn_rqst_seq+"&pln_yrmon="+frm.pln_yrmon.value+"&f_cmd="+SEARCHLIST03;	    
		    frm5.mrd.value = "apps/opus/cps/gem/gemplanningperformance/gemplanningperformance/report/CPS_GEM_0025.mrd";
		    ...
		    frm5.rfn.value = "/CPS_GEM_0027.do?lang_div="+langDiv+"&gen_expn_rqst_no="+gen_expn_rqst_no+"&gen_expn_rqst_seq="+gen_expn_rqst_seq+"&pln_yrmon="+frm.pln_yrmon.value+"&f_cmd="+SEARCHLIST04;	    
		    frm5.mrd.value = "apps/opus/cps/gem/gemplanningperformance/gemplanningperformance/report/CPS_GEM_0027.mrd";
...
function onChangeFmMonthTo2() {
	...
	if (frm2.ei_fm_assign_rule[1].checked ) {
		...
		//var row = sheet4.RowCount;
		var row = getEditableRow(sheet4);
		//초기화 
		for ( var i = janCol ; i <= decCol; i++) {		
			sheet4.CellValue2(row,i) = 0;
		}
		...
		if (total < locl_amt ) {
			var diffAmt = locl_amt - total;
			sheet4.CellValue2(row, start) = (avg + diffAmt);
		} else {
			var diffAmt =  total - locl_amt;
			sheet4.CellValue2(row, start) = (avg - diffAmt);
		}
...
function frm1_deactivate() {
	...
	case "fm_ad_amt":		
		...
		if (Math.abs(removeComma(frm1.fm_ad_amt.value)) > sheet2.CellValue(1,"ttl")) {
...
/**
* init/additional sheet2 편집처리후 이벤트
* @param {long} row 해당 셀의 Row Index
* @param {long} col 해당 셀의 Column Index
* @param {string} col 해당 셀의 value
*/
...
function doActionIBSheet(sAction) {
	...
	if (sAction == SEARCHLIST) {
		...
		sheet1.DoSearch4Fx("CPS_GEM_0001_03GS.do", FormQueryString(frm));	
    ...
    //[open]	 	
		...
		var sXml = sheet5.GetSearchXml("CPS_GEM_0001_03GS.do", FormQueryString(frm));		
		var arrXml = sXml.split("|$$|");
		var authFlg  = "";
		...
		if (arrXml.length > 1) {
			...
			if (list.length > 0) {
				...
				if ( authFlg == "YNNN" || authFlg == "YYNN" ) {
					...
					//LV2
					selLevelChange('GEM_COMMONGS.do','SEARCHLIST01','sheet5','sls_ofc_div_cd','1','document.form.ofc_lvl');			
					ComSetObjValue(frm.ofc_lvl2,level3);					
					//LV3
					selLevelChange('GEM_COMMONGS.do','SEARCHLIST02','sheet5','sls_ofc_div_cd','2','document.form.ofc_lvl');			
		...
		// 2009-10-14 추가 변경 (sheet1_onloadfinish 에서 옮김)
	    if (frm6.popup_yn.value == "N") {
		    //iframe 사이즈 자동조절
		    resizeIframe("t3frame");
		    top.document.body.scrollTop = 0;
	...
	//[Save]	
		...
		if ("ET" == gen_expn_rqst_tp_cd) {
			...
			if ( fm_ttl != fm_ad_amt) {
				...
				ComShowCodeMessage("GEM01040" , ComAddComma(fm_ad_amt));		
				return false;
			}
			...
			if (to_ttl != to_ad_amt) {
				//RQST Amount {?msg1} 금액과 Request TTL금액과 일치 하지 않습니다.
				ComShowCodeMessage("GEM01040" , ComAddComma(to_ad_amt));		
				return false;
			}
			...
			queryString += "&" + sheet2Query + "&" + sheet3Query;
			param += "&" + queryString;
			
			var sXml = sheet2.GetSaveXml("CPS_GEM_0001_03GS.do", param);
			sheet2.LoadSaveXml(sXml);
			
			/*
			frm.f_cmd.value = SEARCHLIST;			
			var sXml = sheet1.GetSearchXml("CPS_GEM_0001_03GS.do", FormQueryString(frm));		
			sheet1.LoadSearchXml(sXml);			
			sheet1.SelectRow = frm.rownum.value;
			sheet1.SelectCell(sheet1.SelectRow, sheet1.SelectCol, false);
			*/
			
		} else {		
			//RQST_OPIN_RMK			
			var rqstOpinRmk = frm2.fm_rqst_opin_rmk.value;			
			sheet1.CellValue2(srow , "rqst_opin_rmk") = rqstOpinRmk;			
			var queryString = getSheetRowQueryString(sheet1,srow);
			queryString = ComSetPrifix(queryString, "sheet1_");
			
			var editRow = getEditableRow(sheet4);
			
			var fm_ad_amt = frm2.fm_ad_amt.value;			
			var fm_ttl  = sheet4.CellText(editRow , "ttl");			
			if ( fm_ttl != fm_ad_amt) {
				//RQST Amount {?msg1} 금액과 Request TTL금액과 일치 하지 않습니다.
				ComShowCodeMessage("GEM01040" , ComAddComma(fm_ad_amt));		
				return false;
			...
			param = "f_cmd=" + MULTI01;
			var sheet4Query = getSheetRowQueryString(sheet4,editRow);
			sheet4Query = ComSetPrifix(sheet4Query, "sheet4_");			
			queryString += "&" + sheet4Query;
			param += "&" + queryString;
			
			var sXml = sheet4.GetSaveXml("CPS_GEM_0001_03GS.do", param);
			sheet4.LoadSaveXml(sXml);
		}
	...
	// Page Onload 시 Lvl 1 콤보 취득  
		...
		var sXml = sheet5.GetSearchXml("GEM_COMMONGS.do", FormQueryString(frm));

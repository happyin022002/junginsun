...
*/
function loadPage() {
	/*Create temp to crete column sheet.*/
//	var formObj = document.form;
...
function initSheet(sheetObj,sheetNo) {
	...
	case "sheet1":
		with (sheetObj) {
		    ...
		    SetConfig( { SearchMode:2, FrozenCol:7, MergeSheet:2, Page:20, DataRowMerge:0 } );
		    ...
		    cols.push({Type:"Text",      Hidden:1, Width:0,    Align:"Right",   ColMerge:0,   SaveName:"agmt_cty_cd",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1 });
		    cols.push({Type:"Text",      Hidden:1, Width:0,    Align:"Right",   ColMerge:0,   SaveName:"agmt_seq",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1 });
	...
	case "sheet2":
		with (sheetObj) {
		    ...
		    var HeadTitle="|Seq.|CNTR No.|TP/SZ|AGMT No.|Ref No.|Term|MVMT|Auth No.|On-hire\nDate|On-hire\nYard|F/Days|Off-hire\nDate|Off-hire\nYard|Used\nDays|Min\nO/H Days|Term\nChange|Direct\nInterchange In|Immediate\nExit|Rental\nCharge|HON|PUC|PCR|HOF|DOC|DCR|On-hire\nDrayage|Off-hire\nDrayage|M & R\nCost|DPP|G.TTL";
		    var headCount=ComCountHeadTitle(HeadTitle);
		    ...
		    var cols = [ {Type:"Status",    Hidden:1, Width:0,    Align:"Center",  ColMerge:1,   SaveName:"Status" },
		              ...
		              {Type:"Text",      Hidden:0,  Width:150,  Align:"Center",  ColMerge:0,   SaveName:"cntr_auth_no",      KeyField:0,   CalcLogic:"",   Format:"" },
		              ...
		              {Type:"CheckBox",  Hidden:0, Width:65,   Align:"Center",  ColMerge:0,   SaveName:"term_change",       KeyField:0,   CalcLogic:"",   Format:"",TrueValue:"Y", FalseValue:"N" },
		              {Type:"CheckBox",  Hidden:0, Width:105,  Align:"Center",  ColMerge:0,   SaveName:"dii",               KeyField:0,   CalcLogic:"",   Format:"" ,TrueValue:"Y", FalseValue:"N"},
		              {Type:"CheckBox",  Hidden:0, Width:80,   Align:"Center",  ColMerge:0,   SaveName:"immediately",       KeyField:0,   CalcLogic:"",   Format:"" ,TrueValue:"Y", FalseValue:"N"},
...
function doActionIBSheet(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	if(validateForm(sheetObj,formObj,sAction))
		if (sheetObj.id == "sheet1"){
			...
			ComOpenWait(true);
			setTimeout( function () {
                var sXml=sheetObj.GetSearchData("EES_LSE_0036GS.do", FormQueryString(formObj));
    ...
    //					gTTL = sheetObjects[0].GetSumValue(arrTpSz2[i]).replace(",", "");
                        
                        if( comboObjects[1].GetItemCheck(0) == true ){
                            if( gTTL == "" || Number(gTTL) <= 0 ){
	...
	break;
	case IBCREATE: //creation tpsz commbo
	formObj.f_cmd.value=SEARCH02;
	...
	break;
	case IBSEARCH_ASYNC01:      //retrieving for detail
	if(validateForm(sheetObj,formObj,sAction)) {
	...
	break;
	case IBSEARCH_ASYNC02:      //retrieving for Lessor
	if(validateForm(sheetObj,formObj,sAction)) {
		if(sheetObj.id == "sheet1") {
	...
	case IBSEARCH_ASYNC03:
		...
		break;
	case IBSEARCH_ASYNC04:	//retrieving for Lessor No
	if ( validateForm(sheetObj, formObj, sAction) ) {
	...
	break;
	case IBSEARCH_ASYNC07:	// retireving for Country
	if(validateForm(sheetObj,formObj,sAction)) {
		if ( sheetObj.id == "sheet1") {
	...
	case IBSEARCHAPPEND:
		if(sheetObj.id == "sheet2") {
			formObj.f_cmd.value=SEARCH01;
			sheetObj.SetWaitImageVisible(1);
			sheetObj.DoSearch("EES_LSE_0036GS.do", CondParam+"&"+ "iPage="+ PageNo,{Append:true} );
		}
		break;
...
/**
* calling event after Key-Down combo
*/
...
/**
* handling event in case of Check-Click combo
* @return
*/
...
/**
* handling event in case of Blur combo
*/
...
function openPopup(type, Row, Col) {
	...
	if ( type == "1" ) {
		...
		case "R" :	//RCC
		ComOpenPopupWithTarget('/opuscntr/COM_ENS_051.do', 800, 480,"rcc_cd:loc_cd", "1,0,1,1,1,1,1", true);
		break;
		case "L" :	//LCC
...
function doActionIBSheet1(sheetObj, formObj, sAction, CondParam, PageNo) {
	...
	case IBSEARCHAPPEND:
        ...
        setTimeout( function () {	        	
			...
			sheetObjects[1].DoSearch("EES_LSE_0036GS.do", CondParam+"&"+ "iPage="+ appendPageNo,{Append:true} );  	   
			} , 100);

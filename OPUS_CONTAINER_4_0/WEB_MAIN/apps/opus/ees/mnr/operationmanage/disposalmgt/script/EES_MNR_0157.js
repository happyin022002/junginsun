 ...
 ***************************************************************************************/
    /**
     * @extends
     * @class EES_MNR_0157 : business script for EES_MNR_0157.
     */
				...
				switch(srcName) {
					...
					case "btn_Inquiry":
						if(MnrNullToBlank(formObject.rqst_ofc_cd.value) == ''){
							...
							ComOpenPopup('/opuscntr/EES_MNR_0158.do?cost_ofc_cd=' + formObject.rqst_ofc_cd.value, 900, 440, 'setPopData_DSPC', '0,1,1,1,1,1,1,1', false, false, 1, 1, 3);
						}
					break;
					break;
					case "btn_FileAdd":
    ...
    function loadPage() {
		...
		//2016/05/10 added by Kato (#12951 Allow deletion of Confirmed Disposal Candidate)-> removed
            ...
            case "sheet2":
                with (sheetObj) {
	                ...
	                var cols = [ {Type:"Status",    Hidden:1, Width:50,   Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
	                    ...
	                    {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"rqst_ofc_cd",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
	                    {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"rqst_usr_id",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
			...
			case "t1_sheet1":
                with (sheetObj) {
	                ...
	                var cols = [ {Type:"Status",    Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
							...
							{Type:"Seq",       Hidden:0, Width:30,   Align:"Center",  ColMerge:0,   SaveName:"Seq",               KeyField:0,   CalcLogic:"",   Format:"" },
							{Type:"Text",      Hidden:0,  Width:100,  Align:"Left",    ColMerge:0,   SaveName:"eq_no",             KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1, EditLen:14, AcceptKeys:"E", InputCaseSensitive:1  },
        ...
        switch(sAction) {
			case IBSEARCH:      //retrieving
				if(validateForm(sheetObj,formObj,sAction)){
				    ...
				    var sXml=sheetObj.GetSaveData("EES_MNR_0157GS.do", sParam);
				    sheetObjects[1].LoadSearchData(sXml,{Sync:0} );
				    
				    //2016/05/10 added by Kato (#12951 Allow deletion of Confirmed Disposal Candidate)
				    var stsCd = formObj.status_cd.value;
				    if(stsCd == 'R') {
				  ...
				  //initializing sheet
					...
					ComSetObjValue(formObj.rqst_ofc_cd,"");
					 //
			...
			case IBSAVE:        //REJECT
              if(validateForm(sheetObj,formObj,sAction)){
				    ...
				    var sXml=sheetObjects[0].GetSaveData("EES_MNR_0157GS.do", sParam);
			...
			case IBCLEAR:      // initializing
				...
				for(var i=3; i < comboList.length;i++){
					if(comboList[i] != null){
						...
						if(comboSaveNames[i - 3] == "eq_knd_cd"){
							sheetObjects[1].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "curr_cd"){
							sheetObjects[1].InitDataCombo (0, comboSaveNames[i - 3], sheetComboCode, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "disp_sts_cd"){
							sheetObjects[1].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "mnr_prnr_knd_cd"){
							sheetObjects[2].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "disp_rsn_cd"){
							sheetObjects[3].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "mkr_nm"){
							sheetObjects[3].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						} else if(comboSaveNames[i - 3] == "disp_vrfy_tp_cd"){
							sheetObjects[3].InitDataCombo (0, comboSaveNames[i - 3], sheetComboText, sheetComboCode ,sheetComboDefault[i - 3]);
						}
					}
				}
...
//					}
	...
	function sheet2_OnDblClick(sheetObj,Row,Col) {
		...
		with(sheetObj){
			...
			formObj.rqst_ofc_cd.value=GetCellValue(Row,"rqst_ofc_cd");
			formObj.rqst_usr_id.value=GetCellValue(Row,"rqst_usr_id");
			...
			if(disp_sts_cd.GetSelectCode()=="HA") {
				curr_cd.SetEnable(0);
			} else {
				curr_cd.SetEnable(1);
			}
...
//			}
	...
	/**
	 * EES_MNR_158 Location handling Pop-up Return Value <br>
	 * @param {arry} Return value array of returnedValues Pop-up
	 * @param Row IBSheet Row index
	 * @param Col IBSheet Col index
	 * @param Sheet Array index

 ...
 ***************************************************************************************/
	...
	function initSheet(sheetObj,sheetNo) {
	    ...
	    switch(sheetNo) {
	    	case 1: // sheet1 init
	    	    with(sheetObj){
	    		var HeadTitle="||||Reference No.|OWN Agreement No.|Lessor Code|Lessor Name";
	    		...
	    		var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
				    		...
				    		{Type:"Text",      Hidden:0, Width:195,  Align:"Center",  ColMerge:0,   SaveName:"inv_ref_no",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:25 },
				    		{Type:"PopupEdit", Hidden:0, Width:195,  Align:"Center",  ColMerge:0,   SaveName:"agmt_no",          KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:9, AcceptKeys:"E|N" },
				    		
				    		{Type:"Text",      Hidden:0, Width:195,  Align:"Center",  ColMerge:0,   SaveName:"vndr_seq",         KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
//	    		SetSheetHeight(570);
	    		resizeSheet();
	          //기능변경[확인요망]HANJIN: 	 				InitDataValid(0, "inv_ref_no", vtEngUpOther, "1234567890-;:.,~!@#$%^&*()_+{}|[]");
	          }
...
//	 				if(arrChkRows.length == 1 && arrDelRows.length == 1){  //원본 , delRows,chkRows 의 값이 없을 경우도 조건에 포함시킴. <2014.09.24 김성욱>
	 				if(delRows == '' && arrChkRows.length == 1 && chkRows == '' && arrDelRows.length == 1){
						...
						sheetObj.DoSave("EES_CGM_2086GS.do", FormQueryString(formObj));
						ComOpenWait(false);
	 				}
	 			}
	...
	function sheet1_OnPopupClick(sheetObj, Row, Col)
	{
		switch(sheetObj.ColSaveName(Col)){
			case "agmt_no":
				ComOpenPopup('EES_CGM_2022.do' , 800, 435, 'setPopupAgreementNo','1,0,1,1,1,1,1,1,1', true, false, Row, Col, 0);
				break;
	 	}
	}
	...
	function setPopupAgreementNo(aryPopupData, row, col, sheetIdx){
		...
		sheetObject.SetCellValue(row, "agmt_no",aryPopupData[0][2],0);
	...
	function sheet1_OnChange(sheetObj, Row, Col){
	 	...
	 	if (Col == agmtNoCol && Row !=0) {
	 		...
	 		if (cellValue != ''){
	 			...
	 			if(cellValue.length != 9 || !ComIsNumber(cellValue.substring(3))){
	 				...
	 				formObj.eq_knd_cd.value=EQ_KND_CD_MGSET;
 			        var sXml=sheetObj.GetSearchData("EES_CGM_2086GS.do", FormQueryString(formObj), '', true);
		...
		var Row2=sheetObj.FindText(Col, sValue, -1);

	 		if(Row2 > 0){

				Row2=sheetObj.FindText(Col, sValue, Row2+1);

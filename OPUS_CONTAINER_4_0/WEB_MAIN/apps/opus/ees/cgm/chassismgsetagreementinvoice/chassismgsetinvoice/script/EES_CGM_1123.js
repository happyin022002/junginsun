...
/**
 * @extends 
 * @class ees_cgm_1123 : ees_cgm_1123 business script for
 */
    ...
    function processButtonClick(){
        ...
        var comboObjpool=combo_pool.GetSelectText();
    ...
    //	axon_event.addListenerForm  ('beforedeactivate', 'obj_deactivate', form);
    	...
    	comboObjects[comboCnt++]= combo_pool;
...
//    	formObj.cost_yrmon.value=cost_yrmon;
    	...
    	for(var i=1; i <= sheetObjects[0].RowCount(); i++){
			sheetObjects[0].SetCellValue(i,"pool_bsrt_amt", "");
        	...
        	case "sheet1":
        	    with(sheetObj){
		              ...
		              var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
		                     ...
		                     {Type:"Text",      Hidden:0,  Width:200,  Align:"Left",    ColMerge:1,   SaveName:"intg_cd_val_dp_desc",         KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
		                     {Type:"Float",     Hidden:0,  Width:100,  Align:"Right",   ColMerge:1,   SaveName:"pool_bsrt_amt",               KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:1,   InsertEdit:1,   EditLen:7 },
		                     {Type:"Int",       Hidden:0,  Width:150,  Align:"Right",   ColMerge:1,   SaveName:"cost_bil_dys",                KeyField:0,   CalcLogic:"",   Format:"NullInteger", PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:7 },
    ...
    function doActionIBSheet(sheetObj,formObj,sAction) {
        ...
        switch(sAction) {	
    		case IBSEARCH:      //retrieve
    			...
    			formObj.chss_mgst_inv_knd_cd.value="CP";
			    sheetObj.SetWaitImageVisible(0);
			 	ComOpenWait(true);
			 	var sXml=sheetObj.GetSearchData("EES_CGM_1123GS.do" , FormQueryString(formObj), '', true);
    			sXml_1 = sXml;
    		...
    		case IBSAVE:        //saving , Confirm
    			...
    			formObj.chss_pool_cd.value=combo_pool.GetSelectText();
    			var sParam=sheetObj.GetSaveString(true);
	         	sParam=sParam + "&";
	         	sParam=sParam + FormQueryString(formObj);
	         	sheetObj.SetWaitImageVisible(0);
			 	ComOpenWait(true);
			 	var sXml=sheetObj.GetSaveData("EES_CGM_1123GS.do", sParam);
    		...
    		case IBDELETE:      // deleting
    			...
    			sheetObj.SetWaitImageVisible(0);
			 	ComOpenWait(true);
			 	var sXml=sheetObj.GetSaveData("EES_CGM_1123GS.do", FormQueryString(formObj));
			 	sheetObj.LoadSaveData(sXml);
    			ComOpenWait(false);
    			break;
	    	...
	    	case IBSEARCH_ASYNC02:	// agreemnt no Combo retrieve
	    		formObj.f_cmd.value=SEARCH01;
	    		formObj.agmt_lstm_cd.value="CP";
	    		var sXml=sheetObj.GetSearchData("CgmCodeMgtGS.do", FormQueryString(formObj), '', true);
				var sStr=ComGetEtcData(sXml,"comboList");    			
				var arrStr=sStr.split("@");
				MakeComboObject2(combo_agmt, arrStr, 4);
	    		break;
	    		
	    	case IBSEARCH_ASYNC03:	// pool list Combo retrieve
	    		formObj.f_cmd.value=SEARCH02;
	    		var sXml=sheetObj.GetSearchData("CgmCodeMgtGS.do", FormQueryString(formObj), '', true);
            ...
            case IBRESET:
    	 		...
    	 		for ( var i=0; i < vArrayListData.length; i++) {
    		  	...
    		  	MakeComboObject4(combo_pool, arrStr1, arrStr2);
    	 		idx++;       
    	 		break;
        }
    }
       ...
       switch(sAction) {
        	...
        	case IBSAVE:
		        if(formObj.cost_yrmon.value == ''){
	        		...
	        		return false;
	        	} else if(combo_pool.GetSelectText()== ''){
    ...
    function sheet1_OnChange(sheetObj, Row, Col){
    	if(sheetObj.ColSaveName(Col) == "pool_bsrt_amt"  || sheetObj.ColSaveName(Col) == "cost_bil_dys"){
    		var colBsrtAmt=sheetObj.GetCellValue(Row, "pool_bsrt_amt");
    		var costBilDys=sheetObj.GetCellValue(Row, "cost_bil_dys");
    		if(colBsrtAmt=='') colBsrtAmt=0;
	    	if(costBilDys=='') costBilDys=0;
	    	if(colBsrtAmt < 0){
	    		sheetObj.SetCellValue(Row, "pool_bsrt_amt", Math.abs(sheetObj.GetCellValue(Row, "pool_bsrt_amt")));
	    	}
	    	if(costBilDys < 0){
	    		sheetObj.SetCellValue(Row, "cost_bil_dys", Math.abs(sheetObj.GetCellValue(Row, "cost_bil_dys")));
	    	}
	    	if(sheetObj.GetCellValue(Row,"dtl_pool_cost_itm_cd") == 'CG'){
	    		sheetObj.SetCellValue(Row, "cost_amt", -1 * Math.abs(colBsrtAmt * costBilDys));
	    	} else {	
	    		sheetObj.SetCellValue(Row, "cost_amt", Math.abs(colBsrtAmt * costBilDys));
	    	}
    	}
...
//	}
     ...
     * @version
     */ 
    function combo_inv_OnChange(comboObj, oldIndex, oldText, oldCode, newIndex, newText, newCode) {//CHECK OLD CODE: OnChange(comboObj, newIndex, text){
    	var formObj=document.form;
...
//    	}
     ...
     * @version
     */ 
    function combo_agmt_OnChange(comboObj, oldIndex, oldText, oldCode, newIndex, newText, newCode) {//CHECK OLD CODE: OnChange(comboObj, newIndex, text){
    	var formObj=document.form;
    	var comboObjAgmt=combo_agmt;
    	var comboObjPool=combo_pool;
    	...
    	comboObjPool.SetSelectCode(newCode,false);
    }
    /** 
     * Pool MultiCombo change event handling  <br>
     * @param  
     * @return 
     * @author 
     * @version
     */ 
    function combo_pool_OnChange(comboObj, oldIndex, oldText, oldCode, newIndex, newText, newCode) {//CHECK OLD CODE: OnChange(comboObj, newIndex, text){
    	var formObj=document.form;
    	var comboObjAgmt= combo_agmt;
    	var comboObjPool= combo_pool;
    	formObj.agmt_ofc_cty_cd.value="";
    	...
    	comboObjAgmt.SetSelectCode(newIndex);
    }
    /** 
     * MultiCombo reset  <br>
     * @param  
...
//        	combo_inv.SetSelectText(text, false);
	...
	function sheet1_OnSearchEnd(sheetObj, errMsg){
		...
		if(onLoad == "N"){
			...
			document.form.chss_pool_cd.value=ComCgmNullToBlank(sheetObj.GetEtcData("chss_pool_cd"));
			// text value

             ...
             case 1:      //sheet1 init
            	 with(sheetObj){
	            	 ...
	            	 var cols = [ {Type:"Status",    Hidden:1, Width:40,   Align:"Center",  ColMerge:0,   SaveName:"ibflag" },
	            	              ...
	            	              {Type:"Text",      Hidden:0, Width:150,  Align:"Center",  ColMerge:0,   SaveName:"mgst_no",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:0,   EditLen:11, AcceptKeys:"E|N", InputCaseSensitive:1},
			...
			for(i=1;i<=sheetObj.LastRow();i++) {
				if(sheetObj.GetCellValue(i, "mnl_inp_flg")!= 'Y'){
					...
					|| (sheetObj.GetCellValue(i, "mvmt_sts_cd") ==  'BO')){
						...
						sheetObj.SetCellEditable(i,"mgst_no",0);
     ...
     function doActionIBSheet(sheetObj,formObj,sAction) {
         ...
         switch(sAction) {
				case IBSEARCH:      //retrieve
					...
					ComOpenWait(true);
	 			 	sheetObj.DoSearch("EES_CGM_1108GS.do", queryString+"&"+params);
	 			 	break;
				case IBSAVE:        //saving
					if(formObj.str_gubun[0].checked == true){
	                    ...
	                    {
							if(sheetObj.GetCellValue(i, "del_chk") == "1" && sheetObj.GetCellValue(i, "at_dt_status")!= 'OK' && sheetObj.GetRowStatus(i) !='D'){
							 sheetObj.SetCellValue(i, "del_chk","0");
							sheetObj.SetRowStatus(i,"R");
							} else if (sheetObj.GetCellValue(i, "del_chk") == "0" && sheetObj.GetRowStatus(i) !='D') {
							 sheetObj.SetRowStatus(i,"R");
...
//
   ...
   case "gate_io_cd" :
    ...
    case "mgst_no" :
    	formObj.f_cmd.value=SEARCH;
	   	...
	   	{
	   		var sXml=sheetObj.GetSearchData("CGM_CHS_MASTERGS.do?eq_no="+sheetObj.GetCellValue(Row, "mgst_no"), FormQueryString(formObj));
		    // data count
		    var dataCount=ComGetTotalRows(sXml);
		    // data existing
		    if(dataCount > 0){
		    	sheetObj.SetCellValue(Row, "del_chk","1");
		   		...
		   		sheetObj.SetCellValue(Row, "mgst_no","");
		   		sheetObj.SelectCell(Row, Col, true);
		   	}
	   	}
	   	sheetObj.SetCellValue(Row, "verify","",0);
   ...
   function sheet1_edit(ROW,status)
   {
	   ...
	   sheetObjects[0].SetCellEditable(ROW, "mgst_no",status);

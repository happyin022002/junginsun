         ...
         switch(sheetID) {
             case "sheet1":
                 with (sheetObj) {
	                 ...
	                 var HeadTitle="|Seq.|M.G Set No.|TP/SZ|Term|Yard|Status|MVMT|Movement Date|Staying Days|Container No.|Chassis No.|B/L No.|Booking No.|Trucker|Trucker Description|Shipper|Shipper Description|Consignee|Consignee Description|UPDATE USER ID|";
...
//  			 	 sheetObj.Redraw = false;
  			 	 ...
  			 	 sheetObj.SetWaitImageVisible(0);
 			 	 ComOpenWait(true);
 			 	 var sXml=sheetObj.GetSearchData("EES_CGM_2104GS.do", appendCondParam);
...
//	  			 	 if(!validateForm(sheetObj,formObj,sAction)) return;
		 	     sheetObj.SetWaitImageVisible(0);
	 	         ComOpenWait(true);
	 	         sheetObj.DoSearch("EES_CGM_2104GS.do",  CondParam+"&"+"&i_page=" + appendPageNo,{Append:true} );
                 for(i=1; i<sheetObj.RowCount()+1; i++){
                	 if(sheetObj.GetCellValue(i, "aciac_div_cd") == "In-active"){
          				sheetObj.SetRowFontColor(i,"#FF0000");
          			}

...
*@FileName   : EES_CGM_2109.js
*@FileTitle  : Mgset Movement Update by Container
*@author     : CLT
*@version    : 1.0
*@since      : 2016/11/22
=========================================================*/
		...
		try {
	     ...
	     switch(srcName) {
			 ...
			 case "btn_new":
	        	...
	        	formObject.p_date1.value = to_dt.substring(0, 4) + '-'+ to_dt.substring(4, 6) + '-' + to_dt.substring(6, 8);
	        	formObject.p_date2.value = fr_dt.substring(0, 4) + '-'+ fr_dt.substring(4, 6) + '-' + fr_dt.substring(6, 8);
			 break;
			 case "btn_save":
...
//     	    	}
     	    	var fr_dt = new String(document.form.calend1.value).substring(0, 8);
     			var to_dt = new String(document.form.calend2.value).substring(0, 8);
     			formObj.p_date1.value = to_dt.substring(0, 4) + '-'+ to_dt.substring(4, 6) + '-' + to_dt.substring(6, 8);
     			formObj.p_date2.value = fr_dt.substring(0, 4) + '-'+ fr_dt.substring(4, 6) + '-' + fr_dt.substring(6, 8);
                ...
                {Type:"Text",      Hidden:0,  Width:90,   Align:"Center",  ColMerge:0,   SaveName:"rep_cmdt_nm",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:13 } ];
                 ...
                 with (sheetObj) {
                	 ...
                	 var cols = [ 
    	              ...
    	              {Type:"Text",       Hidden:0, Width:80,   Align:"Center",  ColMerge:0,   SaveName:"chss_no",               KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:10 },
    	              {Type:"Text",       Hidden:0, Width:80,   Align:"Center",  ColMerge:0,   SaveName:"mgst_no",               KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10 },
         ...
         case IBSEARCH:
				if(validateForm(sheetObj,formObj,sAction)){
				    ...
				    sheetObjects[1].SetWaitImageVisible(0);
	 			 	ComOpenWait(true);
	 				var sXml=sheetObj.GetSearchData("EES_CGM_2109GS.do" , FormQueryString(formObj));
			...
			case IBSAVE:
			   if(validateForm(sheetObj,formObj,sAction)){
				   ...
				   if(sheetObj.DoSave("EES_CGM_2109GS.do",queryString + "&" + ComGetPrefixParam(""))){
				   }
			   }
			   break;
         }
   	...
   	function sheet2_OnChange(sheetObj, Row, Col){
    	 ...
    	 switch (sheetObj.ColSaveName(Col)) {
          ...
          case "mgst_no" :
        	    ...
        	    formObj.mgst_no.value=sheetObj.GetCellValue(Row, "mgst_no");
        	    formObj.chss_no.value=sheetObj.GetCellValue(Row, "chss_no");
	  		    ...
	  		    if(sheetObj.GetCellValue(Row, "mgst_no")!="")
	  		   	{
  				    ...
  				    if(dataCount > 0){
  				    	if(DomXml2String(sXml,"aciac_div_cd")=="I"){
  				    		...
  				    		if(DomXml2String(sXml,"eq_tpsz_cd") == "UMG"){
  				    			if(sheetObj.GetCellValue(Row, "chss_no") == ""){

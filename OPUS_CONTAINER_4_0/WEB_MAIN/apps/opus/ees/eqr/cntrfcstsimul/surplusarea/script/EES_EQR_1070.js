    ...
    function initSheet(sheetObj,sheetNo) {
        ...
        switch(shtID) {
            case "sheet1":      //sheet1 init
             ...
             var cols = [ {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:1,   SaveName:"loc_cd",            KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                 {Type:"Text",      Hidden:0,  Width:170,  Align:"Left",    ColMerge:1,   SaveName:"name",              KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                 {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"code",              KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                 {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"grp_cd",            KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                 {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:0,   SaveName:"sort",              KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 } ];
                      for(i=0; i < rptTtlArr.length; i++){
             var calcLogic="0*1";
             if (tpsztype.GetSelectText()!= "") {
        ...
        switch (sAction) {
			case IBSEARCH: // 조회
				if (validateForm(sheetObj, formObj, sAction)) {
					...
					formObj.f_cmd.value=SEARCH;
 					var mainXml=sheetObj.GetSearchData("EES_EQR_1070GS.do", FormQueryString(formObj));
			...
			case SEARCH02: // Location 유효성 조회
		        ...
		        if(code_type == "1"){ // ALL
                 ...
                 var sXml=sheetObj.GetSearchData("EES_EQR_1070GS.do", sParam);
   ...
   function search_end(sheetObj, startRow, endRow) {
    ...
    function calcAllFormula(startRow, endRow){
		...
		if (rptTtl != "") {
			...
			for (var i=startRow; i < endRow; i++) {
				if (sheetObj.GetCellValue(i, "code") == "F1" && sheetObj.GetCellValue(i, "loc_cd") != "TTL") {
		...
		with(sheetObj){
			...
			for(var k=0; k<weekArr.length; k++){
				for(var t=0; t<tpszArr.length; t++){
					...
					SetCellValue(f1_row,tpszArr[t]+"_"+weekArr[k],( GetCellValue(fbb_row,tpszArr[t]+"_"+weekArr[k])*1 - GetCellValue(f2_row,tpszArr[t]+"_"+weekArr[k])*1 ).toFixed(0));
			...
			for(var k=0; k<weekArr.length; k++){
                for(var t=0; t<tpszArr.length; t++){
					if (tpszArr[t] != "ttl") {
                        ...
                        SetCellValue(f2_row,tpszArr[t]+"_"+weekArr[k],( f2Sum ).toFixed(0),0);
					}else{

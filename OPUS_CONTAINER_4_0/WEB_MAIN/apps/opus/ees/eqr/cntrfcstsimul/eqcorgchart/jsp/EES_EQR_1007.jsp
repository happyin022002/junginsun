...
<%@ page import="com.clt.apps.opus.ees.eqr.cntrfcstsimul.eqcorgchart.event.EesEqr1007Event"%>
<%
    EesEqr1007Event  event = null;				//PDTO(Data Transfer Object including Parameters)
	Exception serverException   = null;			//서버에서 발생한 에러
	String strErrMsg = "";								 //에러메세지
	int rowCount	 = 0;								  //DB ResultSet 리스트의 건수
	event = (EesEqr1007Event)request.getAttribute("Event");
	String chkDepth = event.getChkDepth();
	try {
		/*
		꼭 의 정보를 받을 필요는 없습니다. 화면에서 의 이름이나 
		권한같은 정보를 이용할 필요가 있을 경우에만 사용하면 됩니다.
		덧붙여 USER 정보에 대해서 한마디로 정리하면 user 의 정보를 이용할수 있는 곳은 jsp 와 command 입니다.
		jsp에서는 의 정보를 가지고 권한에 따른 버튼 처리등을 할수가 있는 것이고 (enable/disable)
		command에서는 역시 의 정보로 예를 들어 update 권한등이 있는지를 확인할 수가 있는 것입니다.
	   ...
	   */ 
	   ...
	   //userAuth=account.getAuth(); 
		...
		이때 가 작성한 자료는 event 에 서버로부터 전송된 자료는 eventResponse에 담기게 됩니다.
...
<input	type="hidden" name="chkDepth" value="<%= chkDepth %>">
 ...
 <div class="layer_popup_title">
	...
	<div class="page_title_area clear">
		...
		<div class="opus_design_btn">
			<button type="button" class="btn_normal" name="btn_Save" name="btn_Save">Save</button><!-- 
			--><button type="button" class="btn_normal" name="btn_Close" name="btn_Close">Close</button>
		</div>
		<!-- opus_design_btn(E) -->	
	</div>
	<!-- page_title_area(E) -->

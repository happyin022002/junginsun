 ...
 ***************************************************************************************/
    /**
     * @extends 
     * @class ESD_TPB_0135 : business script for ESD_TPB_0135
     */
    /* Global Variables */
  ...
  var sheetCnt=0;
     ...
     */
  	function loadPage() {
		...
		formObj.s_ctrl_ofc_cd.value=user_ctrl_ofc_cd;
  		...
  		setSelectBox(s_if_rhq_cd,document.form.s_rhq_cd_for_rhq.value);
  		//setSelectBox(document.form.s_if_rhq_cd, "ALL");
  		if_rhq_cd_OnChange();
  		getCtrlOfcList();
  		if( real_office_level == 'C') 
  			setSelectBox(document.form.s_if_ctrl_cd,document.form.s_user_ofc_cd.value);
  	...
  	function initSheet(sheetObj,sheetNo) {
  		...
  		switch(sheetNo) {
  			case 1:	  //IBSheet1 init
  			    with(sheetObj){
  		        	var cnt=0;
  		        	var HeadTitle="STS|S/N|TPB No.|RHQ|Con. Office|Source|Invoice No.|Invoice Date|Source No.|BKG No.|B/L No.|VVD|EQ No|Type|3rd Party Code|3rd Party Name|TPB Status|Overdue|Currency|Original|Invoice|Collect|Adjust|Balance|I/F User|I/F Office|Adjust Type|R.O.C From|R.O.C To|Recovery Activity|EAC Type|CSR No.";
  		        	...
  		        	var cols = [ {Type:"Status",    Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"" },
  		             ...
  		             {Type:"Text",      Hidden:0,  Width:90,   Align:"Center",  ColMerge:1,   SaveName:"cfm_ofc_cd",             KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1 },
     ...
     */
  	function sheet1_OnSearchEnd(sheetObj,errMsg){
  		...
  		sheetObj.SetCellValue(sheetObj.LastRow(), "n3pty_no_dp_seq", "TOTAL");
  	...
  	/**
	 * Inputting in RHQ office in combobox
	 */
	function getRHQOfcList() {
		...
		var comboObj=eval("document.form.s_if_rhq_cd");
	...
	/**
	 * Inputting in Control office in combobox
	 */
	function getCtrlOfcList() {
		...
		var comboObj= document.form.s_if_ctrl_cd;
		if(comboObj != undefined) {
			ComClearCombo(comboObj);
		}
		ComAddComboItem(comboObj, "ALL", "" );
	...
	/**
	 * Inputting in TPB office in combobox
	 */
	function getTpbOfcList() {
		...
		var comboObj= document.form.s_if_ofc_cd;
		if(comboObj != undefined) {
			ComClearCombo(comboObj);
		}
		ComAddComboItem(comboObj, "ALL", "" );

...
/**
 * @extends 
 * @class ESD_TPB_0104 : business script for ESD_TPB_0104
 */
    /* Global Variables */
	...
	function loadPage(n3pty_no_strs) {
		...
		if(document.form.s_office_level.value == "H"){
			...
			if_rhq_cd_OnChange(); //Setting RHQ ComboBox
			if_ctrl_cd_OnChange(); //Setting Control Office ComboBox
		}
	...
	function initSheet(sheetObj,sheetNo) {
		...
		switch(sheetNo) {
			case 1 :      //sheet1 init
			    with(sheetObj){
			      var HeadTitle="Cancel|STS|approval|Request|Approve|Reject|Res.Office|TPB No.|Req.Date|Req.Office|Req.ID|App.Date|App.Office|App.ID|Rej.Date|Rej.Office|Rej.ID|Exp.Type|Invoice No.||W/O Status|BKG No.|B/L No.|EQ No.|RHQ|Office|3rd Party|Currency|Confirmed Amount|Confirmed Date|Invoice|ERP I/F|Approval|Balance|Request|Activity|W/O Date|Review Step";
			      ...
			      var cols = [ {Type:"DelCheck",  Hidden:0, Width:70,   Align:"Center",  ColMerge:1,   SaveName:"",                       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
			             ...
			             {Type:"CheckBox",  Hidden:0, Width:53,   Align:"Center",  ColMerge:1,   SaveName:"chk_req",                KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:false },
			             {Type:"CheckBox",  Hidden:0, Width:53,   Align:"Center",  ColMerge:1,   SaveName:"chk_app",                KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:false },
			             {Type:"CheckBox",  Hidden:0, Width:53,   Align:"Center",  ColMerge:1,   SaveName:"chk_rej",                KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:false },
			             {Type:"PopupEdit", Hidden:0, Width:80,   Align:"Center",  ColMerge:1,   SaveName:"stl_to_clt_cng_ofc_cd",  KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:0,   EditLen:8 },
			             ...
			             {Type:"Text",      Hidden:1, Width:100,  Align:"Right",   ColMerge:1,   SaveName:"stl_clt_ofc_cng_amt",    KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
			      ...
			      SetColHidden("stl_to_clt_cng_ofc_cd",1);
			      SetColHidden("review_step",1);
				...
				if( curr_action == "REQUEST" ){
					...
					for( var i=1 ; i<sheetObj.RowCount()+1 ; i++ ){
						...
						if(sheetObj.GetRowStatus(i) != 'R'){
							if(sheetObj.GetCellValue(i,"chk_req")==0){
								if(bugs!=""){bugs += ",";}
								bugs += i;
							}
						}
...
//    		sheetObjects[0].DoSave("ESD_TPB_0114GS.do", tpbFrmQryStr(document.form));
//		}
//  	}
	...
	function sheet1_OnClick(sheetObj, Row,Col){ 
		...
		if( colNm == "chk_app"){
			sheetObj.SetCellValue(Row,"chk_app",1);
			sheetObj.SetCellValue(Row,"chk_rej",0);
		} else if( colNm == "chk_rej" ){
			sheetObj.SetCellValue(Row,"chk_app",0);
			sheetObj.SetCellValue(Row,"chk_rej",1);
		}
	}
	/**
	 * in case of sheet1 cell value changing 
	 */
	function sheet1_OnChange(sheetObj, Row, Col, Value){
		...
		if(colNm == 'stl_to_clt_cng_ofc_cd'){
            ...
            if ( stl_to_clt_cng_ofc_cd.length == 0 ){
				...
				document.all.s_vndr_cust_div_cd.value="S"; // input type hidden
				document.all.s_trd_party_val.value=stl_to_clt_cng_ofc_cd; // input type hidden 
				// ComShowMessage(" .. " +Row);
		...
		if ( sheetObj.ColSaveName(Col) == "chk_req" ) {  // chk_req on checking or unchecking
			if ( Value == 0 ) { // on unckecking 
				...
				if ( val == "C" || val == "D" ) { //  L, K 
					...
					sheetObj.SetCellValue(Row, "stl_clt_ofc_cng_amt",sheetObj.GetCellValue(Row, "bal_amt")==""?"0":sheetObj.GetCellValue(Row, "bal_amt"),0);
				}
			}
	...
	function sheet1_OnSearchEnd(sheetObj,errMsg){
		...
		if( s_date_kind_flag == "R" ){
			...
			for( var i=1 ; i<sheetObj.RowCount()+1 ; i++ ){
				//Changing IBSheet column of s_date_kind_flag에
				if( s_date_flag_r == "AL" ){
			...
			if(s_date_flag_i=="OL"){
				...
				for( var i=1 ; i<sheetObj.RowCount()+1 ; i++ ){
					if( sheetObj.GetCellValue(i,"ots_sts_cd_val") == "Requested" &&
							sheetObj.GetCellValue(i,"if_ofc_cd") == document.form.s_user_ofc_cd.value ){
	...
	function sheet1_OnSaveEnd(sheetObj,errMsg){
		// SAVE후 Request기능 활성화.Activating Request after saving
		if( curr_action == "REQUEST" &&

...
/**
 * @extends 
 * @class ESD_TPB_0122 : Defining logic script for ESD_TPB_0122 screen
 */
/* Global Variables */
...
function loadPage() {
    ...
    if (document.form.s_office_level.value == "H") {
        ...
        ComAddComboItem(s_if_rhq_cd, document.form.s_rhq_cd_for_rhq.value, document.form.s_rhq_cd_for_rhq.value);
    ...
    $('#s_n3pty_src_sub_sys_cd').on('change', tpb_searchBillingCaseByExpenseType);
}
		...
		case 1:	  //IBSheet1 init
			with(sheetObj){			
				...
				var cols = [ {Type:"Text",      Hidden:1, Width:30,   Align:"Left",    ColMerge:1,   SaveName:"",                      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1 },
							...
							{Type:"Text",      Hidden:0,  Width:65,   Align:"Center",  ColMerge:1,   SaveName:"if_rmk_flag",           KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1 },
							{Type:"Float",     Hidden:0,  Width:80,   Align:"Right",   ColMerge:1,   SaveName:"cfm_amt",               KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:1,   InsertEdit:1 },
							...
							{Type:"Text",      Hidden:0,  Width:140,  Align:"Center",  ColMerge:1,   SaveName:"csr_no",                KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
							{Type:"Text",      Hidden:0,  Width:70,   Align:"Center",  ColMerge:1,   SaveName:"gl_dt",                 KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
				...
				InitColumns(cols);
				SetEditable(1);
				SetRowHeight(0,SetRowHeight(1,0));
        ...
        switch (srcName) {
            ...
            case "btn_inv":
                ...
                if (str != '') {
                    ...
                    formObject.action = "ESD_TPB_0124.do?pgmNo=ESD_TPB_0124&parentPgmNo=ESD_TPB_M001";
                    formObject.submit();
                }
                break;
...
function doActionIBSheet(sheetObj, formObj, sAction) {
    ...
    switch (sAction) {
        case IBSEARCH: //Retrieve
            ...
            sheetObj.DoSearch("ESD_TPB_0122GS.do", tpbFrmQryStr(formObj));
            break;
        ...
        case IBSAVE: //Save
            ...
            var grpCnt = 0;
            //Impossible to save in case of not checked Confirmed or not checked Review. Removing UPD
            var check_cnt = 0;
            var chkFlg = true;
            cancel = "N";
            //Remark require in case of Confirmed is 'N' or checked Review
            ...
            // Checking selected count of checkbox - select can over 2 and under 50
            ...
            if(cancel == "Y"){
            	if(ComShowCodeConfirm("TPB90110")){
            		...
            		var sXml = sheetObj.GetSaveData("ESD_TPB_0122GS.do", tpbFrmQryStr(formObj)+"&"+sParam);
...
function sheet1_OnSearchEnd(sheetObj, errMsg) {
    ...
    for (var i = 2; i <= sheetObj.RowCount() + 1; i++) {
        ...
        if (sheetObj.GetCellValue(i, "n3pty_bil_tp_cd") == "JO") {
            ...
            // sheetObj.CellEditable(i, "cfm_flg_n") = false;
        }
        //Setting cfm_amt same if_amt
        sheetObj.SetCellValue(i, "cfm_amt", sheetObj.GetCellValue(i, "if_amt"));
        //A can be confirmed only  I/F target by office of myself
        if (document.form.s_ofc_cd_for_rhq.value != sheetObj.GetCellValue(i, "ofc_cd") &&
...
function sheet1_OnSaveEnd(sheetObj, errMsg) {
    ...
    for (var i = 2; i <= sheetObj.LastRow(); i++) {
        ...
        sheetObj.SetCellEditable(i, "n3pty_non_cfm_rsn_cd", 0);
        sheetObj.SetCellEditable(i, "cfm_amt", 0);
...
function sheet1_OnPopupClick(sheetObj, Row, Col) {
    //get3rdParty_sheet( sheetObj.CellValue(Row,"vndr_cust_div_cd"), Row,Col,sheetObj ); // 3rd party open popup
    ...
    if (parseFloat(sheetObj.GetCellValue(Row, 'cfm_amt')) > 0 &&
        sheetObj.GetCellValue(Row, 'if_curr_cd') != "" &&
        ComTrim(sheetObj.GetCellValue(Row, 'vndr_cust_div_cd')) != '' &&
        sheetObj.GetCellValue(Row, 'trd_party_val') != '') {
...
function sheet1_OnClick(sheetObj, Row, Col, Value) {
    ...
    if (colNm == 'n3pty_src_no' && sheetObj.GetCellFontUnderline(Row, colNm)) {
        ...
        if (src_cd == 'TRS') {
            ...
            if (tes_gubun != "") {
                if (tes_gubun == "TM") {
                	param = "?pgmNo=ESD_TES_0017&parentPgmNo=ESD_TPB_M001&inv_no=" + encodeURIComponent(sheetObj.GetCellValue(Row, "n3pty_src_no")) + "&vndr_seq=" + sheetObj.GetCellValue(Row, "src_vndr_seq");
                    ...
                    param = "?pgmNo=ESD_TES_0068&parentPgmNo=ESD_TPB_M001&inv_no=" + sheetObj.GetCellValue(Row, "n3pty_src_no") + "&vndr_seq=" + sheetObj.GetCellValue(Row, "src_vndr_seq");
                    ...
                    param = "?pgmNo=ESD_TES_0018&parentPgmNo=ESD_TPB_M001&inv_no=" + sheetObj.GetCellValue(Row, "n3pty_src_no") + "&vndr_seq=" + sheetObj.GetCellValue(Row, "src_vndr_seq");
                    ...
                    param = "?pgmNo=ESD_TES_0019&parentPgmNo=ESD_TPB_M001&inv_no=" + sheetObj.GetCellValue(Row, "n3pty_src_no") + "&vndr_seq=" + sheetObj.GetCellValue(Row, "src_vndr_seq");
...
function sheet1_OnChange(sheetObj, Row, Col, Value) {
    ...
    switch (Col) {
        ...
        case 6:
            sheetObj.SetCellValue(Row, 'cfm_d', 0, 0);
            break;

        case 7:
            sheetObj.SetCellValue(Row, 'cfm_r', 0, 0);
            break;
        ...
        case 8:
            ...
            sheetObj.SetCellValue(Row, 'cfm_i', 0, 0);
            sheetObj.SetCellValue(Row, 'cfm_n', 0, 0);
            sheetObj.SetCellValue(Row, 'cfm_g', 0, 0);
            break;
    }
    ...
    if (sheetObj.GetCellValue(Row, 'cfm_g') == 1) { // when "G" checked
        ...
        if (grp_trd_party_val == "_NULL") {
            ...
            if (sheetObj.GetCellValue(Row, 'trd_party_val') != grp_trd_party_val) {
                sheetObj.SetCellValue(Row, 'cfm_g', 0, 0); // cancle to "G" checking
                errCheck++;
        ...
        if (grp_csr_no == "_NULL") {
            grp_csr_no = sheetObj.GetCellValue(Row, 'csr_no');
        } else {
            if (sheetObj.GetCellValue(Row, 'csr_no') != grp_csr_no) {
                sheetObj.SetCellValue(Row, 'cfm_g', 0, 0);
                errCheck++;
                errMsg += "\n" + errCheck + ". CSR No";
            }
        ...
        if (grp_gl_dt == "_NULL") {
            grp_gl_dt = sheetObj.GetCellValue(Row, 'gl_dt');
        } else {
            if (sheetObj.GetCellValue(Row, 'gl_dt') != grp_gl_dt) {
                sheetObj.SetCellValue(Row, 'cfm_g', 0, 0);
                errCheck++;
                errMsg += "\n" + errCheck + ". G/L Date";
            }
        ...
        if (grp_vvd_cd == "_NULL") {
            grp_vvd_cd = sheetObj.GetCellValue(Row, 'vvd_cd');
        } else {
            if (sheetObj.GetCellValue(Row, 'vvd_cd') != grp_vvd_cd) {
                sheetObj.SetCellValue(Row, 'cfm_g', 0, 0);
                errCheck++;
                errMsg += "\n" + errCheck + ". Actual VVD";
            }
    ...
    if (colNm == 'cfm_amt') {
        ...
        if (sheetObj.GetCellValue(Row, "n3pty_bil_tp_cd") == "JO") {
            // if(parseFloat(sheetObj.CellValue(Row,"if_amt")) > parseFloat(sheetObj.CellValue(Row,"cfm_amt"))){
            if (parseFloat(sheetObj.GetCellValue(Row, "cfm_amt")) <= 0) {
    ...
    if (lvl == "R") {
        ...
        sheetObj.SetCellEditable(Row, "cfm_i", 0);
        sheetObj.SetCellEditable(Row, "cfm_g", 0);
        sheetObj.SetCellEditable(Row, "cfm_c", 0);
    }
    ...
    if (sheetObj.GetCellValue(Row, "cfm_i") == "0" &&
        ...
        sheetObj.GetCellValue(Row, "cfm_c") == "0") {
        sheetObj.SetRowStatus(Row, "");
    }
    
    //Initializing 'trd_party_val' by detect a changed of sheet1.vndr_cust_div_cd'
    if (colNm == "vndr_cust_div_cd") {
        sheetObj.SetCellValue(Row, "trd_party_val", "", 0);
    }
    
    //'Reason for Non-TPB' must Initialize exclude from in case of Non-Confirm
    if (sheetObj.GetCellValue(Row, 'cfm_g') == 1 || sheetObj.GetCellValue(Row, 'cfm_g') == 1) {
        sheetObj.SetCellValue(Row, "n3pty_non_cfm_rsn_cd", "", 0);
    }
}

 ...
 ***************************************************************************************/
    /**
     * @extends 
     * @class ESD_TPB_0128 : business script for ESD_TPB_0128
     */
  			...
  			case 1:	  //IBSheet1 init
  			    with(sheetObj){
	        		...
	        		var cols = [ {Type:"Status",    Hidden:1, Width:100,  Align:"Center",  ColMerge:0,   SaveName:"sts",               KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
  		             ...
  		             {Type:"Text",      Hidden:0,  Width:110,  Align:"Right",   ColMerge:1,   SaveName:"if_amt_usd",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
  		             ...
  		             {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:1,   SaveName:"overdue_days",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
  		             {Type:"Text",      Hidden:0,  Width:130,  Align:"Left",    ColMerge:1,   SaveName:"cfm_rmk",           KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 } ];
  		            if ( isReadOnly!='Y' ){
...
//  						ComShowCodeMessage('TPB90010','Cancel Close ','','');
  				...
  				case "bttn_print":
  					...
  					break;
  				case "btn_retrieve":
  					doActionIBSheet(sheet2,formObject,SEARCH);
  	...
  	function doActionIBSheet(sheetObj,formObj,sAction) {
  		...
  		switch(sAction) {
  			case SEARCH:	  //Retrieve
  				...
  				sheetObj.DoSearch("ESD_TPB_0128GS.do", tpbFrmQryStr(formObj) );  				
  				break;
  			...
  			case COMMAND02:		// Close Process By General User
                  ...
                  //===== show messge according to the Source (TES/TRS/MNR)
...
//  		}
     ...
     */
  	...
  	function sheet2_OnSearchEnd(sheetObj, errMsg){
  		...
  		if(sheetObj.GetCellValue(1,"n3pty_no")!= -1)
				formObj.s_n3pty_no.value=sheetObj.GetCellValue(1,"n3pty_no");
			if(sheetObj.GetCellValue(1,"n3pty_inv_no")!= -1)
		formObj.s_n3pty_inv_no.value=sheetObj.GetCellValue(1,"n3pty_inv_no");
			if(sheetObj.GetCellValue(1,"ofc_cd")!= -1)
				formObj.s_ofc_cd.value=sheetObj.GetCellValue(1,"ofc_cd");
			if(sheetObj.GetCellValue(1,"ots_sts_nm")!= -1)
				formObj.s_ots_sts_nm.value=sheetObj.GetCellValue(1,"ots_sts_nm");
			if(sheetObj.GetCellValue(1,"overdue")!= -1)
				formObj.s_overdue.value=sheetObj.GetCellValue(1,"overdue");
		formObj.s_vndr_cust_div_cd.value=sheetObj.GetCellValue(1,"vndr_cust_div_cd");
			if(sheetObj.GetCellValue(1,"trd_party_val")!= -1)
		formObj.s_trd_party_val.value=sheetObj.GetCellValue(1,"trd_party_val");
			if(sheetObj.GetCellValue(1,"trd_party_nm")!= -1)
				formObj.s_trd_party_nm.value=sheetObj.GetCellValue(1,"trd_party_nm");
			if(sheetObj.GetCellValue(1,"csr_no")!= -1)
				formObj.s_csr_no.value=sheetObj.GetCellValue(1,"csr_no");
			if(sheetObj.GetCellValue(1,"roc_in")!= -1)
				formObj.s_roc_in.value=sheetObj.GetCellValue(1,"roc_in");
			if(sheetObj.GetCellValue(1,"roc_out")!= -1)
				formObj.s_roc_out.value=sheetObj.GetCellValue(1,"roc_out");
...
//  		}
  	...
  	function tpb_getEmailInfo(){
  		//comPopupWith2ndSheet('/opuscntr/COM_ENS_091.do', 770, 580, 'getStaff_formail', '1,0,1,1,1,1,1', false);
  		var classId="COM_ENS_092";
  		var param='?classId='+classId;
  		comPopup('/opuscntr/COM_ENS_092.do' + param, 850, 550, 'getStaff_formail', 'none', true);
  	}
  	...
  	/**
  	 * Pop-up for message info. registering in case of process closing
  	 */

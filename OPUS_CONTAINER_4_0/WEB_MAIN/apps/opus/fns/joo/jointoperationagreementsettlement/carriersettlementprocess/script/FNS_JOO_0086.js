		...
		case 1:
			with (sheetObj) {
			    ...
			    var cols = [ 
			              ...
			              {Type:"Int",      Hidden:0,  	Width:70,  Align:"Center", 	ColMerge:0,   SaveName:prefix+"dft_teu_cnt"			,KeyField:0,   CalcLogic:"",   Format:"NullInteger" 	,	PointCount:0,   UpdateEdit:1,   InsertEdit:1,	EditLen:5, DefaultValue:"1" },
		...
		case 2:
			with (sheetObj) {
			    ...
			    var cols = [ 
			              ...
			              {Type:"Text",     Hidden:0,  	Width:70,  Align:"Center", 	ColMerge:0,   SaveName:prefix+"src_tpsz"		,KeyField:0,   CalcLogic:"",   Format:"" ,	PointCount:0,   UpdateEdit:0,   InsertEdit:1,	EditLen:3 },
		...
		case 3:
			with (sheetObj) {
			    ...
			    var cols = [ 
			              ...
			              {Type:"Int",      Hidden:0,  	Width:50,   Align:"Center", 	ColMerge:0,   SaveName:prefix+"teu_cnt"			,KeyField:0,   CalcLogic:"",   Format:"NullInteger" ,	PointCount:0,   UpdateEdit:1,   InsertEdit:1,	EditLen:10 }];
...
function setMakeTeuTpsz(){
	...
	//TEU Sheet 상태를 전부 D로 변경 한다.
	for (var j=teuSheetObj.HeaderRows(); j <= teuSheetObj.LastRow(); j++){
		teuSheetObj.SetCellValue(j, arrPrefix[2]+"del_chk"	, 1		, 0);
		teuSheetObj.SetCellValue(j, arrPrefix[2]+"ibflag"	, "U"	, 0);
	}
	
	//조회 된 데이타를 tpszsheet 의 TP/SZ 사이즈로 재로딩 한다.
	for(var i = tpszSheetObj.HeaderRows() ; i <= tpszSheetObj.LastRow(); i++){
		var tmpDftTgtNormalTpsz	= tpszSheetObj.GetCellValue(i, arrPrefix[4]+"tgt_normal_tpsz");
		
		if(tpszSheetObj.GetRowStatus(i) != "D"){
			//TEU Loop를 돌면서 tmpTgtNormalTpsz 존재하는
			for (var j=teuSheetObj.LastRow(); j >= teuSheetObj.HeaderRows(); j--){
				...
				if(tmpTgtNormalTpsz == tmpDftTgtNormalTpsz){
					teuSheetObj.SetCellValue(j, arrPrefix[2]+"del_chk"	, 0	, 0);	
					teuSheetObj.SetCellValue(j, arrPrefix[2]+"ibflag"	, "", 0);
				}else{
					//TP/SZ R,U,I 가 TEU 에 존재 하지 않으면
				}
			}
		}
	}

	var sCheckedRow=teuSheetObj.FindCheckedRow(arrPrefix[2]+"del_chk");
	if(sCheckedRow != ""){
		JooRowHideDelete(teuSheetObj, arrPrefix[2]+"del_chk");
	}
	...
	var tmpSlanCd = "";
	var idx = 0;
	for(var i = teuSheetObj.HeaderRows() ; i <= teuSheetObj.LastRow(); i++){
		var tmpChkSlanCd = teuSheetObj.GetCellValue(i, arrPrefix[2]+"slan_cd");
		if(tmpSlanCd != tmpChkSlanCd && teuSheetObj.GetRowStatus(i) != "D"){
			arrSlanCd[idx] = tmpChkSlanCd;
			tmpSlanCd = tmpChkSlanCd;
			idx++;
		}
	}
	//Lane 별로 Insert 한다.(I 상태인 TP/SZ)
	for(var i = tpszSheetObj.HeaderRows() ; i <= tpszSheetObj.LastRow(); i++){
		var tmpDftTgtNormalTpsz	= tpszSheetObj.GetCellValue(i, arrPrefix[4]+"tgt_normal_tpsz");
		
		if(tpszSheetObj.GetRowStatus(i) == "I"){
			//TEU Sheet Row Add.
			for (var idx=0; idx < arrSlanCd.length; idx++){
				//저장전 Tab 이동시 기존에 들어가 있는지 체크
				var isDupFlag = getCheckDuplication(teuSheetObj, tmpDftTgtNormalTpsz, arrSlanCd[idx]);
				if(isDupFlag) continue;
				
				var iRow = teuSheetObj.DataInsert(-1);
				teuSheetObj.SetCellValue(iRow, arrPrefix[2]+"slan_cd"			, arrSlanCd[idx]		, 0);
				teuSheetObj.SetCellValue(iRow, arrPrefix[2]+"line_seq"			, 1						, 0);
				teuSheetObj.SetCellValue(iRow, arrPrefix[2]+"tgt_normal_tpsz"	, tmpDftTgtNormalTpsz	, 0);
				teuSheetObj.SetCellValue(iRow, arrPrefix[2]+"teu_cnt"			, getDefaultTpszTeu(tmpDftTgtNormalTpsz)					, 0);
			}
		}else{
			//D 가 아닌 경우에는 기존에 데이타가 있는지 체크하고 없으면 넣어 준다.
			if(tpszSheetObj.GetRowStatus(i) != "D"){
				//TEU Sheet Row Add.
				for (var idx=0; idx < arrSlanCd.length; idx++){
					var iChkCnt = 0;
					for(var j = teuSheetObj.HeaderRows() ; j <= teuSheetObj.LastRow(); j++){
						...
						if(arrSlanCd[idx] == tmpChkSlanCd && teuSheetObj.GetRowStatus(j) != "D" && tmpDftTgtNormalTpsz == tmpTgtNormalTpsz){
							iChkCnt++;
							continue;
						}
					}

...
//		SetGetSelectionMode(smSelectionList);
		...
		var HeadTitle1="|Sel.|VVD|Cust. Code|Cust. Name|Invoice No.|LPO No.|E-mail Address|Fax No.|INV Ref. No.|I/F No.|B/L No.|Port Cd.|Remark(s)|inv_rmk2|Attach more from my PC|Attach more from my PC|Attach more from my PC|Attach more from my PC|File Key|Inv Seq.|CMB FLG|f_save_id|iss_grp_tp_cd|rd_name";
		...
		setTimeout( function () {
			...
			if (formObj.state.value == "S") {
				...
				for (var idx=0; idx < arrRow.length; idx++){
					...
					var iss_grp_tp_cd=sheetObj.GetCellValue(arrRow[idx], "iss_grp_tp_cd");
					var logo="ORIGINAL";
					report = rdOpen(inv_no, 15, formObj.user_nm.value, formObj.iss_ofc_cd.value, logo, vvd, port_cd, "N", "Y", cmb_flg, iss_grp_tp_cd);
			...
			if (formObj.state.value == "S") {
				...
				for (var idx=0; idx < arrRow.length ; idx++){
					...
					if (sheetObj.GetCellValue(arrRow[idx], "cust_eml") == "") {
						report = rdOpen(inv_no, 15, formObj.user_nm.value, formObj.iss_ofc_cd.value, "ORIGINAL", vvd, port_cd, "N", "Y", cmb_flg, iss_grp_tp_cd); 
						appendReport.push(report);
						for(var j=0; j<v_copy_cnt; j++) {  	
							report = rdOpen(inv_no, 15, formObj.user_nm.value, formObj.iss_ofc_cd.value, "COPY", vvd, port_cd, "N", "Y", cmb_flg, iss_grp_tp_cd);
							appendReport.push(report);
						}

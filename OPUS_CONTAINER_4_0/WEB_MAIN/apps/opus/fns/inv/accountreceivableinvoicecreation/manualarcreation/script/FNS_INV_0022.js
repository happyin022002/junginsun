...
//					sheetObjects[3] = sheetObjt4;
			...
			case IBSEARCH_ASYNC12: // auto BL No retrieve
				...
				if (tmpBlSrcNo == '') {
					...
					if (blMaxSeq != undefined) {
						// when RHQ is 'SELHO' OFC setting (ex : OFC = SELPIO → BL NO = PIO090828001) 
			...
			if (glEffDt != '') {
				if(formObj.sail_arr_dt.value == '') { //2016.06.13 Add
					...
					for(var j=1; j <= rowCnt; j++){
						if (sheetObj.GetCellValue(j, "acct_cd") == "411911" || sheetObj.GetCellValue(j, "acct_cd") == "411915" || sheetObj.GetCellValue(j, "acct_cd") == "710111") {
							acctCnt++;
						}
					}
					if (acctCnt > 0) {
	...
	function calTVAAmount(sheetObj, formObj) {
		...
		var taxChgAmt=0;
		for(var i=sheetObj.RowCount(); i>0; i--){
			if(sheetObj.GetCellValue(i, "ibflag") != "" && sheetObj.GetCellValue(i, "acct_cd") == "212111" && sheetObj.GetCellValue(i, "tax_yn") == "T") {
				taxInfoCnt ++;
			}
		...
		if (tvaFlgCnt > 0 && taxInfoCnt == 0) {
			...
			sheetObj.SetCellValue(sheetObj.GetSelectRow(), "acct_cd","212111");
	...
	function dtlGridSet(gridType) {
		...
		if (gridType == 'amount') {
			...
			for(var i=sheetObj.RowCount(); i>0; i--){
				if(sheetObj.GetCellValue(i, "ibflag") != "" && sheetObj.GetCellValue(i, "acct_cd")=="212111" && sheetObj.GetCellValue(i, "tax_yn")== "Y") {
					sheetObj.SetCellValue(i, "chg_amt",formObj.tax_amount.value,0);
				}
				else if(sheetObj.GetCellValue(i, "ibflag") != "" && sheetObj.GetCellValue(i, "acct_cd")=="212111" && sheetObj.GetCellValue(i, "tax_yn")== "T") {
...
//		sheetObj4.RemoveAll();
		...
		for(i=sheetObj1.RowCount(); i>0; i--){
			...
			if(sheetObj1.GetCellValue(i, "ibflag") != "" && sheetObj1.GetCellValue(i, "acct_cd") != "212111" && sheetObj1.GetCellValue(i, "mnl_flg") == "Y") {
	...
	function sheet1_OnChange(sheetObj,Row,Col,Value){
    	...
    	if (Col == 1) {
    		...
    		if(formObj.sail_arr_dt.value == '' && lclVvd != '' && (lclVvd.substring(0,4) == "CNTC" || lclVvd.substring(0,4) == "COMC")) { //2016.06.13 Add
				...
				var chkAcct=false;
				for(var i=sheetObj.RowCount(); i>0; i--){
					if(sheetObj.GetCellValue(i, "ibflag") != "" && (sheetObj.GetCellValue(i, "acct_cd")=="411911" || sheetObj.GetCellValue(i, "acct_cd")=="411915" || sheetObj.GetCellValue(i, "acct_cd")=="710111")) {
						chkAcct=true;
					}
				}
				if (chkAcct) {
			...
			var invVatChgRt=formObj.inv_vat_chg_rt.value;
			//2015.05.14 Remove KR,HQ hardcoding by IY Cho
			if (invVatChgCd != '' && invVatChgRt > 0 && sheetObj.GetCellValue(Row, "acct_cd") == "212111") {
				sheetObj.SetCellValue(Row, "tva_flg",0);
				sheetObj.SetCellEditable(Row, "tva_flg",0);
			}
			else if (invVatChgCd != '' && invVatChgRt > 0 && sheetObj.GetCellValue(Row, "acct_cd") != "212111") {
	...
	function chkAccountCondition(sheetObj, Row, val) {
		...
		if (val != '') {
			for(var i=sheetObj.RowCount(); i>0; i--){
				if(sheetObj.GetCellValue(i, "ibflag") != "" && (sheetObj.GetCellValue(i, "acct_cd")=="411911" || sheetObj.GetCellValue(i, "acct_cd")=="411915" || sheetObj.GetCellValue(i, "acct_cd")=="957112" || sheetObj.GetCellValue(i, "acct_cd")=="954111" || sheetObj.GetCellValue(i, "acct_cd")=="422011" ||sheetObj.GetCellValue(i, "acct_cd")=="710111")) {
					if (curs.indexOf(sheetObj.GetCellValue(i, "acct_cd")) == -1) {
						curs=curs + sheetObj.GetCellValue(i, "acct_cd") +"|";
					}
				}
				else {
					if(sheetObj.GetCellValue(i, "ibflag") != "" && sheetObj.GetCellValue(i, "acct_cd")!="212111") {
						if (curs.indexOf(sheetObj.GetCellValue(i, "acct_cd")) == -1) {
		...
		if (curs_item.length == 2) {
			if (otherCurs.length >= 2) {
				...
				if (sheetObj.GetCellValue(Row, "acct_cd") != '212111') {
    ...
    /**
	 * whent CR detail Grid의 Account changed Account Nmae, Remark change <br>
	...
	function gridCopyDeleteOK(sheetObj, Row, formObj) {
		var rtnValue=true;
		if (sheetObj.GetCellValue(Row, "acct_cd") == "212111" && sheetObj.GetCellValue(Row, "mnl_flg") == "N") {

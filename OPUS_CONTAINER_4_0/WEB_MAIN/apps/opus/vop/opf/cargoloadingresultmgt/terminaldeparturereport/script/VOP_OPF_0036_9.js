...
var mBtnDis="N";
    ...
    function processButtonClick(){
    	...
    	try {
			...
			switch(srcName) {
				...
				case "btn_t9Delete":
						var sheetObj=t9sheet1;
						var prefixDel="t9sheet1_del_chk";
    ...
    function loadPage(btnDis) {
		...
		initControl();
		parent.frameButtonSheet(parent.document.t9frame, parent.readonlStatus());
		parent.topSync();
		parent.iframeResize(true);
    }
	function initControl(){
		axon_event.addListener('blur', 't9sheet1_onblur', 't9sheet1', '');	
	}
	/**
	...
	function initSheet(sheetObj,sheetNo) {
		...
		with (sheetObj) {
            var HeadTitle1="";
            if(mBtnDis == "Y"){
            	HeadTitle1="|Sel.|Container No.|Operator|Type/Size|POL|POD|Position|Weight|Reason";
            }else{
            	HeadTitle1="|Container No.|Operator|Type/Size|POL|POD|Position|Weight|Reason";
            }
            ...
            cols.push({Type:"Text",      Hidden:0, Width:170,  Align:"Center",  ColMerge:1,   SaveName:prefix+"cntr_no",  KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:13 });
            ...
            cols.push({Type:"Float",     Hidden:0, Width:100,  Align:"Right",   ColMerge:1,   SaveName:prefix+"weight",   KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:1,   InsertEdit:1,   EditLen:8, ApproximateType:2 });
            cols.push({Type:"Text",      Hidden:0, Width:100,  Align:"Left",    ColMerge:1,   SaveName:prefix+"remark",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:15 });
	...
	function podComboInit(){
		var prefix="t9sheet1_";
		parent.document.t9frame.t9sheet1.SetColProperty(prefix+"pod", {ComboText:parent.mPodName, ComboCode:parent.mPodCode} );
		parent.document.t9frame.t9sheet1.SetColProperty(prefix+"pol", {ComboText:parent.mPodName, ComboCode:parent.mPodCode} );
	}
	function t9sheet1_OnPopupClick(sheetObj, Row, Col){
		if(sheetObj.ColSaveName(Col) == "t9sheet1_opr_cd"){
			parent.getCallBackOprCd(sheetObj, "t9sheet1_", "opr_cd", Row, Col);
			parent.mCheckValue=false;
		}
	}
	function t9sheet1_OnSelectCell(sheetObj,OldRow, OldCol, NewRow, NewCol) {
		if(mBtnDis != "Y"){
			return;
		}
		if(sheetObj.ColSaveName(OldCol) == "t9sheet1_opr_cd" && sheetObj.GetCellValue(OldRow, "t9sheet1_opr_cd") != "" && parent.mCheckValue){
			parent.checkOprCd(sheetObj, OldRow, OldCol);
		}
	}
	function t9sheet1_OnKeyDown(sheetObj, Row, Col, KeyCode, Shift){
		var prefix="t9sheet1_";
		if(sheetObj.ColSaveName(Col) == prefix + "opr_cd"){
			if(KeyCode >= 65 && KeyCode<= 90){
				parent.mCheckValue=true;
				parent.mPopUpEditSheet=document.t9sheet1;

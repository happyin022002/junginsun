...
*@FileName   : CoScg.js
*@FileTitle  : SCG Common javascript
*@author     : CLT
*@version    : 1.0
*@since      : 2014/04/08
=========================================================*/
	...
	msgs['SCG50029']='This booking doesn\'t comply with Own policy according to Pre Checking Report.\nDo you want to proceed approval?';
	...
	msgs['SCG50034']='Please use after Retrieve.';
	msgs['SCG50035']='There is no selected marking, Please check again.\n';
	msgs['SCG50036']='Please Check the POL ETD`s end date';
	msgs['SCG50037']="DG EDI is sent successfully.";
	msgs['SCG50038']="E-mail is sent successfully.";
	msgs['SCG50039']="Mail address does not exist. \nPlease register receiver email address first.\n{?msg1}";	
	msgs['SCG50040']='The Sequence of UN No. for CFR doesn\'t exist. \nDo you want to input a new Seq for CFR?';
	msgs['SCG50041']='In order to input additional class information, you must save first.';
	msgs['SCG50042']='data is duplicated.';
	msgs['SCG50043']='Data was saved successfully.';
	msgs['SCG50044']='Please enter {?msg1} Limit\'s data';
	msgs['SCG50045']='Please select Entry Type.';
	msgs['SCG50046']='Please enter {?msg1} data';
	msgs['SCG50047']='You can not add a new row Because already so a new row';
	msgs['SCG50048']='In view of the data processing time, inquiry period will be limited to four {?msg1}.';
	msgs['SCG01001']='{?msg1} is mandatory item';
	msgs['SCG50049']="Do you want to request{?msg1}?";
	msgs['SCG50050']="Booking Number [{?msg1}] Already Requested.\nWould You Request Again ?"
	msgs['SCG50051']='Vessel Code [{?msg1}] is invalid.';
	msgs['SCG50052']="Requested successfully."
	msgs['SCG50053']="Request Canceled Successfully."
	msgs['SCG50054']="Please input the detail for IMDG class 1."
	
	msgs['SCG50055']="Cannot be deleted because it has been approved or rejected."
	msgs['SCG50056']="At least one container must be remained."
		
	msgs['SCG50057']="{?msg2} is not belonging to {?msg1}."			
    msgs['SCG50058']="Booking Reference Number cannot exceed 12 digit."
    msgs['SCG50059']="Please register {?msg1} for RSO first."	
    	
    msgs['SCG50060']="Sending DG EDI is failed.";    	
	
	msgs['SCG50061']= "This Booking may have been already requested before.\nDo you still want to delete?";
	msgs['SCG50062']='Some Bookings will be deleted physically.\n It can not be restored.';
	...
	function ComScgGetRowValue(xmlStr, cRow, savename)  {
		...
		try {
          ...
          var xmlRoot = xmlDoc.documentElement;
			...
			//var arrData=dataChildNodes[cRow-1].firstChild.nodeValue.split(sep); //dataChildNodes[cRow-1].firstChild returns text => firstChild is undefined.
			// Luc Duong [06-19-2014] (S)
			var firstChild = dataChildNodes[cRow - 1].firstChild;
			var arrData = firstChild == undefined ? dataChildNodes[cRow].firstChild.nodeValue.split(sep) : firstChild;
			// Luc Duong [06-19-2014] (E)
	...
	function ComScgGetTotalValue(sXml){
		return ComGetSelectSingleNode(sXml, "TOTAL");
  	} 
    /**
    ...
    function ComScgSendMail(sheetObj, formObj, mailObj, returnYn, sDomain, returnProc) {
    	var mailParams=new Array("com_rdSubSysCd","com_from","com_recipient","com_carbonCopy","com_blindCarbonCopy","com_subject","com_content","com_fileKey","com_template","com_argument");
 	    try {
 	    	...
 	    	sParam[sParam.length]="slan_cd="+mailObj.slan_cd;
 	    	...
 	    	if(returnYn) {
 	    		...
 	    		if(sDomain != "") {
 	    			sProc += "try {";
 	    			sProc += "   var sheetObj;"; 
 	    			sProc += "   var send = false;";
 	    			sProc += "   for(var sCt=0; sCt<sheetObjects.length; sCt++) {";
 	    			sProc += "      if('"+sheetObj.id+"'==sheetObjects[sCt].id) sheetObj=sheetObjects[sCt];";
 	    			sProc += "   }";
 	    			sProc += "   if(key != 'null') {";
	    			sProc += "     var sXml=sheetObj.GetSaveData('"+sDomain+"', 'f_cmd="+MODIFY+"&"+sParam.join("&")+"&eml_snd_no='+key+'&'+FormQueryString(document.form));";
	    			//sProc += "   var sXml=sheetObj.GetSaveData('"+sDomain+"', 'f_cmd="+MODIFY+"&vsl_pre_pst_cd="+mailObj.vsl_pre_pst_cd+"&vsl_seq="+mailObj.vsl_seq+"&eml_snd_no='+key+'&'+FormQueryString(document.form));";
// 	    			sProc += "   }";
 	    	...
 	    	var sXml=sheetObj.GetSearchData("VOP_SCG_0019GS.do", sParam.join("&")+"&"+FormQueryString(formObj));
	    	...
	    	if(mailParamObjs == null) {
	    		...
	    		paramDiv.setAttribute('name', 'mail_param');
	    		paramDiv.setAttribute('id', 'mail_param');
	    		paramDiv.setAttribute('stytle', 'display:');
		    	...
		    	for(var idx=0; idx<=mailParams.length; idx++) {   
		    		...
		    		paramObj.setAttribute('name', mailParams[idx]);
		    		paramObj.setAttribute('id', mailParams[idx]);
		    		paramObj.setAttribute('type', 'hidden');
		    		paramObj.setAttribute('value', '');
		    		paramDiv.appendChild(paramObj); 
		    	}
		    	//paramObj=document.createElement("<input type='hidden' id='com_mailKeyFlag' value=''>");
		    	paramObj = document.createElement("INPUT");
	    		paramObj.setAttribute('name', 'com_mailKeyFlag');
	    		paramObj.setAttribute('id', 'com_mailKeyFlag');
	    		paramObj.setAttribute('type', 'hidden');
	    		paramObj.setAttribute('value', '');
	    	...
	    	ComSendMail(); 			
 			...
 			//document.form.action = "/opuscntr/Mail.do?f_cmd=2";
...
//	    	ComSetObjValue(formObj.com_mailKeyFlag,"");
    ...
    function ComScgCallAfterMailing(sheetObj, sDomain, sParam) {
    	try {
     		sheetObj.GetSaveData(sDomain, sParam);
    	} catch(err) { 
    		ComFuncErrMsg(err.message); 
    	}
    }
     ...
     */
      ...
      */
     function ComScgGetExcelDown(sheetObj){
     	var resultStr = "";
      ...
      */
     function ComScgGetExcelDownCols(sheetObj){
		var colAry = new Array();
		colAry = makeHiddenSkipCol(sheetObj).split("|");

 ...
 ***************************************************************************************/
   /**
    ...
    /**
     * @extends 
     * @class vop_scg_1017 : business script for vop_scg_1017 
     */
     	...
     	try {
     		...
     		switch(srcName) {
     			...
     			case "btn_DownExcel":
                    ...
                    paramObj.title="Application Request"+ " § " +"Approval Status - Break Bulk";
     	...
     	switch(sheetObj.id) {
	 		case "sheet1":      //sheet1 init
	 		    with(sheetObj){
	 				...
	 				HeadTitle1 += "DT(GMT)|By|ST|DT(GMT)|By|Reject\nCode|Remark(s)|APVL\nRef. No.|";
	 				HeadTitle1 += "Commodity|Length\n(cm)|Width\n(cm)|Height\n(cm)|Gross\nWeight(kg)|Void\n(FEU)|bkg status||||||";
	 				var headCount=ComCountHeadTitle(HeadTitle);
	 				SetConfig( { SearchMode:2, MergeSheet:9, Page:20, DataRowMerge:0, FrozenCol:8 } );
	 				...
	 				var cols = [ {Type:"Text",      Hidden:0,  Width:40,   Align:"Center",  ColMerge:0,   SaveName:"vsl_pre_pst_nm",          KeyField:0,   CalcLogic:"",   Format:"",            Wrap:1 },
	 			 	             ...
	 			 	             {Type:"Text",      Hidden:0,  Width:40,   Align:"Center",  ColMerge:0,   SaveName:"bb_cgo_seq",              KeyField:0,   CalcLogic:"",   Format:"",            Wrap:1 },
	 			 	             ...
	 			 	             {Type:"Float",     Hidden:0,  Width:50,   Align:"Right",   ColMerge:0,   SaveName:"dim_len",                 KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:0,   UpdateEdit:0,   Wrap:1 },
	 			 	             {Type:"Float",     Hidden:0,  Width:50,   Align:"Right",   ColMerge:0,   SaveName:"dim_wdt",                 KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:0,   UpdateEdit:0,   Wrap:1 },
	 			 	             {Type:"Float",     Hidden:0,  Width:50,   Align:"Right",   ColMerge:0,   SaveName:"dim_hgt",                 KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:0,   UpdateEdit:0,   Wrap:1 },
	 			 	             {Type:"Float",     Hidden:0,  Width:70,   Align:"Right",   ColMerge:0,   SaveName:"grs_wgt",                 KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:0,   Wrap:1 },
	 	            ...
	 	            SetRangeBackColor(1,11,1,18,"#555555");
    	 ...
    	 with(sheetObj)
    	 {
    		 ...
    		 //for (var i = 2; i <= LastRow(); i ++)
    		 {
    			 ...
    			 if (HeaderRows() != nextSeq && GetCellValue(i, "spcl_cgo_rqst_seq") == maxSeq) {
    				 ...
    				 arrAftData[k][0]=GetCellValue(i, "cmdt_nm");
    				 arrAftData[k][1]=GetCellValue(i, "dim_len");
    				 arrAftData[k][2]=GetCellValue(i, "dim_wdt");
    				 arrAftData[k][3]=GetCellValue(i, "dim_hgt");
    				 arrAftData[k][4]=GetCellValue(i, "grs_wgt");
    				 arrAftData[k][5]=GetCellValue(i, "net_wgt");
    				 arrAftData[k][6]=GetCellValue(i, "bb_cgo_seq");
    				 arrAftData[k][7]=i;
    				 k++;
    			 }
    			 if (GetCellValue(findSeq-1, "bb_cgo_seq") == "") {
    				 findSeq--;
    			 }
    			 if (HeaderRows() != nextSeq && GetCellValue(findSeq-1, "spcl_cgo_rqst_seq") == GetCellValue(i, "spcl_cgo_rqst_seq")) {
    				 arrBefData[j]=new Array();
    				 arrBefData[j][0]=GetCellValue(i, "cmdt_nm");
    				 arrBefData[j][1]=GetCellValue(i, "dim_len");
    				 arrBefData[j][2]=GetCellValue(i, "dim_wdt");
    				 arrBefData[j][3]=GetCellValue(i, "dim_hgt");
    				 arrBefData[j][4]=GetCellValue(i, "grs_wgt");
    				 arrBefData[j][5]=GetCellValue(i, "net_wgt");
    				 arrBefData[j][6]=GetCellValue(i, "bb_cgo_seq");
    				 j++;
    			 }
    		 }
    		 for (var x=0; x <= j-1; x++)
    		 {
        		 for (var z=0; z <= k-1; z++)
        		 {
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][0] != arrAftData[z][0]) SetCellFontColor(arrAftData[z][7], "cmdt_nm","#FF0000");
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][1] != arrAftData[z][1]) SetCellFontColor(arrAftData[z][7], "dim_len","#FF0000");
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][2] != arrAftData[z][2]) SetCellFontColor(arrAftData[z][7], "dim_wdt","#FF0000");
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][3] != arrAftData[z][3]) SetCellFontColor(arrAftData[z][7], "dim_hgt","#FF0000");
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][4] != arrAftData[z][4]) SetCellFontColor(arrAftData[z][7], "grs_wgt","#FF0000");
        			 if (arrBefData[x][6] == arrAftData[z][6] && arrBefData[x][5] != arrAftData[z][5]) SetCellFontColor(arrAftData[z][7], "net_wgt","#FF0000");
        		 }
    		 }
    	 }     
     }

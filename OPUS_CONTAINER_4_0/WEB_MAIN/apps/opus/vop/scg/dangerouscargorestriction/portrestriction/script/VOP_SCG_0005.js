 ...
 ***************************************************************************************/
   /**
    ...
    /**
     * @extends 
     * @class VOP_SCG_0005 : business script for VOP_SCG_0005 
     */
...
//                	ComOpenPopup('/opuscntr/VOP_VSK_0043.do?port_cd=' + port_cd, 517, 530, "setPortCd", "0,0", true);
     ...
     function initSheet(sheetObj,sheetNo) {
         ...
         switch(sheetNo) {
             case 1:      // sheet1 init
                 ...
                 SetVisible(1);
                 SetCountPosition(0);
                 SetSheetHeight(ComGetSheetHeight(sheetObj, 3), 1);
               }
               break;
             case 2:      // sheet2 init
            	 with (sheetObj) {
            	 ...
            	 cRow++;
            	 SetCountPosition(0);
            	 SetSheetHeight(ComGetSheetHeight(sheetObj, 5), 1);
               }
               break;
             case 3:      // sheet3 init
             with (sheetObj) {
             ...
             with (sheetObj) {
            	 ...
            	 cRow++;     
            	 SetCountPosition(0);
            	 SetSheetHeight(ComGetSheetHeight(sheetObj, 8), 1);
               }
               break;    
         }
     }
   // handling sheet process
     ...
     function doActionIBSheet(sheetObj, formObj, sAction) {
         ...
         if(sAction != IBDELETE) { //삭제 이외의 동작일 경우, msgUseUnno 초기화
        	 msgUseUnno = "";
         }
         switch(sAction) {
...
//                              document.all.btn_saveAs.className='btn1';
                              ...
                              /*  in case data doesn't exist, */
                              if( !ComShowConfirm(sMsg) ){//CLEAR process
                                     doActionIBSheet(sheetObjects[0], formObj,IBINSERT);   
                                     return;                              
                              }else{//New Insert Ok
                                     /******************* grid initialize setting ********************/
                                     for(i=0;i<sheetObjects.length;i++){
                                         setInitValue(sheetObjects[i], true);    
                                     }  
                                     setIuputMode();
                                     return;
                                     /******************* change insert mode END ********************/                                     
                              }  
                          }
                  }else{//Unno retrieve
...
//                              document.all.btn_saveAs.className='btn1';
             ...
             case IBSAVE:        //save
                 ...
                 setCmptnAuthCd();
                 var aryPrefix=new Array("sheet1_","sheet2_","sheet3_","sheet4_","sheet5_" );
                 var sParam=ComGetSaveString(sheetObjects,true, true);//3rd param save all.
                 if (sParam == "") return;
                  sParam +="&f_cmd="+formObj.f_cmd.value+"&port_cd="+formObj.port_cd.value+"&imdg_clss_cd="+imdg_clss_cd.GetSelectText();
                  ...
                  sParam +="&rstr_rmk="+encodeURIComponent(formObj.rstr_rmk.value);
                  ...
                  sParam +="&pass_imdg_cmptn_auth_cd="+formObj.pass_imdg_cmptn_auth_cd.value;
                   var sXml=sheetObjects[0].GetSaveData("VOP_SCG_0005GS.do", sParam+"&" + ComGetPrefixParam( aryPrefix ) );
             ...
             case IBDELETE:        //delete
            	 if(msgUseUnno != "") ComShowMessage(msgUseUnno);
                 if(!validateForm(sheetObj,formObj,sAction)){
                     return;
                 }
...
//                 $(formObj.optClass[0]).click();
            ...
            case COMMAND04:      //SAVE  AS function.
                 ...
                 ComOpenPopup('/opuscntr/VOP_SCG_1005.do?'+param, 750, 360, "setSaveAs", "0,1,1,1,1,1,1,1", true,false);            
                 break;
     ...
     function setInitValue(sheetObj, dataInsYn){
         with (sheetObj)
         {
             ...
             document.form.rstr_rmk.value="";
             document.form.imdg_port_rstr_seq.value="";               
             document.form.upd_usr_id.value="";
             document.form.upd_dt.value="";

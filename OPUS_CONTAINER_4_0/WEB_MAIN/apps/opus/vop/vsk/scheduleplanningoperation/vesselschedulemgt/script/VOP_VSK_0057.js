...
*@FileName 	 : VOP_VSK_0057.js
*@FileTitle  : VSL SKD Creation by VVD (CCA)
*@author     : CLT
*@version    : 1.0
*@since      : 2014/05/29
=========================================================**/
 ...
 ***************************************************************************************/
	...
	var comboCnt=0;
	var portDataFlgs=new Array();		//Port change status
...
//		glbDisableColor="#EFEFEF";
		glbDisableColor="#F4F4F4";
		glbFixColor="#FFFFFF";
		//::jsk::sheetObjects[0].DataInsert(-1);
			...
			case 1:      // sheet1 init
			    with(sheetObj){				        
				      ...
				      var cols = [
									...
									{Type:"Text",      Hidden:1, 	Width:50,   Align:"Center",  ColMerge:1,   SaveName:prefix+"clpt_seq",               KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 			},
									{Type:"Text",      Hidden:0, 	Width:80,   Align:"Center",  ColMerge:1,   SaveName:prefix+"vps_port_cd",            KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:5  },
									{Type:"Combo",     Hidden:0, 	Width:80,   Align:"Center",  ColMerge:1,   SaveName:prefix+"tml_cd",                 KeyField:0,   CalcLogic:"",   Format:"",       	 PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:2 	},
									{Type:"Text",      Hidden:0, 	Width:140,  Align:"Center",  ColMerge:1,   SaveName:prefix+"vps_eta_dt",             KeyField:1,   CalcLogic:"",   Format:"",       	 PointCount:0,   UpdateEdit:1,   InsertEdit:1 			},
									{Type:"Text",      Hidden:0, 	Width:140,  Align:"Center",  ColMerge:1,   SaveName:prefix+"vps_etb_dt",             KeyField:1,   CalcLogic:"",   Format:"",       	 PointCount:0,   UpdateEdit:1,   InsertEdit:1 			},
									{Type:"Text",      Hidden:0, 	Width:140,  Align:"Center",  ColMerge:1,   SaveName:prefix+"vps_etd_dt",             KeyField:1,   CalcLogic:"",   Format:"",       	 PointCount:0,   UpdateEdit:1,   InsertEdit:1 			},
									{Type:"Combo",     Hidden:0, 	Width:90,   Align:"Center",  ColMerge:1,   SaveName:prefix+"turn_port_flg",          KeyField:1,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1 	},
					               ...
					               //:2016-04-13:by TOP://
									...
									{Type:"Text",      Hidden:0, 	Width:90,   Align:"Center",  ColMerge:0,   SaveName:prefix+"turn_skd_voy_no",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:4 	},
									{Type:"Combo",     Hidden:0, 	Width:80,   Align:"Center",  ColMerge:0,   SaveName:prefix+"turn_skd_dir_cd",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:1 	},
				      ...
				      SetColProperty(prefix+"vps_port_cd", {AcceptKeys:"E|N" , InputCaseSensitive:1});
				      SetCountPosition("0");
				      //conversion of function[check again]CLT 					InitDataValid(0, prefix+"turn_skd_voy_no", vtNumericOnly);
				      //conversion of function[check again]CLT 					InitDataValid(0, prefix+"vps_port_cd", vtEngUpOnly);
		...
		switch(sAction) {
			...
			case SEARCH01:		//Terminal(Yard) List
				if(validateForm(sheetObj,formObj,sAction)){
					...
					var sXml=sheetObj.GetSearchData("VOP_VSK_0057GS.do", sParam);
					return sXml;
				}
				break;
			...
			case IBSAVE:        //Save
				if(validateForm(sheetObj,formObj,sAction)){
					setPortRotnSeq(sheetObj, prefix); // PortRotnSeq 조정
					...
					ComOpenWait(true); 					
					var sXml=sheetObj.GetSaveData("VOP_VSK_0057GS.do", sParam);
					ComOpenWait(false);				
...
//				var rowIdx = getLastRowAfterVirtual(sheetObj);
			...
			case COMMAND03:      			// VVD Pop-up
            	...
            	if(vslCd == ""){
            		...
            		ComOpenPopup(sUrl, 480, 470, "getVslCdData", "0,0", true);
            	}else{
            		sUrl="/opuscntr/VOP_VSK_0230.do?ctrl_cd=NORL&vsl_cd="+vslCd;
            		ComOpenPopup(sUrl, 480, 470, "getVvdData", "0,0", true);
            	}
				break;
...
//	        			var dataNode=rootNode.selectSingleNode("//ERROR/MESSAGE");
	...
	/**
	 * 첫번째 row의 PortRotnSeq가 설정되어있지 않은 경우, 첫번째 row가 vsk의 첫번째 row일 경우 1을 설정
	 */
	function setPortRotnSeq(sheetObj, prefix) {
		var sParam=ComGetSaveString(sheetObjects, false);
		var portRotnSeq = "";
		var row = -1;
		for(var i = sheetObj.HeaderRows();i<=sheetObj.LastRow();i++) {
			if(sheetObj.GetCellValue(i, prefix+"ibflag") != "D") {
				portRotnSeq = sheetObj.GetCellValue(i, prefix+"port_rotn_seq");
				row = i;
				break;
			}
		}
		if(portRotnSeq == "" && row >= 0) {
			var formObj = document.form;
			...
			if(result == 'T') {
				sheetObj.SetCellValue(row, prefix+"port_rotn_seq", "1",0);
			}
		}
	}
    ...
    function validateForm(sheetObj,formObj,sAction){
    	...
    	switch(sAction) {
			...
			case IBSAVE:      //save
                ...
                if (formObj.skd_rmk.value.length > 1500){
                    ComShowCodeMessage( "VSK01019", "[Remark(s)]" );
                    formObj.skd_rmk.focus();
                    return false ;
                }
//                if (formObj.pf_svc_tp_cd ==null){
//                	ComShowCodeMessage("VSK00021" , "[The Proforma of Inputted Feeder schedule]")
//                	return false;
//                }
//           
...
//						    		if Arrival / Departure Report exist, can not delete
						    		...
						    		if(sheetObj.GetCellValue(i, prefix+"skd_cng_sts_cd") == "S"){
						    			...
						    			// turn_skd_voy_no,turn_skd_dir_cd >> disabled
						    			sheetObj.SetCellBackColor(i, prefix+"turn_skd_voy_no",glbDisableColor);
						    			sheetObj.SetCellBackColor(i, prefix+"turn_skd_dir_cd",glbDisableColor);
						    		}
						    		ydIdx++;
...
/*	            	
	            case "skd_rmk":	  
	            	if(ComChkLen(formObj.skd_rmk.value, 2000) == "0"){
	            		ComShowCodeMessage('COM131901','');
	            		formObj.skd_rmk.value=formObj.skd_rmk.value.substring(0,2000);
	            	}
*/	            	
	        } // end switch
		}catch(e) {
    	...
    	if(rtnObjs){
			...
			if(rtnDatas){
				if(rtnDatas.length > 0){
					formObj.vsl_slan_cd.value=rtnDatas[0][1];
				}
			}
    	}
	}
...
//					sheetObj.CellValue2(i, prefix+"ibflag") = "I";
					...
					//sheetObj.SetCellValue(i, prefix+"turn_port_flg" , "N", 0);
				}
			}
		}
		formObj.skd_rmk.value="";
...
//    	sheetObj.CellValue2(rowIdx, prefix+"init_etd_dt") = sheetObj.CellValue(rowIdx-1, prefix+"init_etd_dt");
    	//sheetObj.SetCellValue(rowIdx, prefix+"turn_port_flg" , "N", 0);
    }
	/**
     ...
     * @return
     */
    //function getLastRow()AfterVirtual(sheetObj){

        ...
        if (!ComIsBtnEnable(srcName)) return;  
         switch(srcName) {
            case "btn_ok":
            	...
            	// opener.document.form.port_cd.value = sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd');
            	ComPopUpReturnValue(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd'));
            	//ComClosePopup(); 
				//comPopupOK();
                break;
            case "btn_close":
            ...
            case "btn_popup":
                ...
                //@@ComOpenPopup("/opuscntr/COM_ENS_0M1.do", 790, 500, "countryCodeHelp", "1,0,1,1,1,1,1,1", true);
//                ComOpenPopup('/opuscntr/COM_ENS_0M1.do?'+param, 790, 500, "countryCodeHelp", "1,0,1,1,1,1,1,1", true);
                var sUrl="/opuscntr/COM_ENS_0M1.do?"
				ComOpenPopup(sUrl, 800, 550, "countryCodeHelp", "1,0,1,1,1,1,1,1", false);
                //ComOpenPopup(sUrl, 800, 550, "countryCodeHelp", "0,1,1,1,1,1,1,1", false);
 ...
 function doActionIBSheet(sheetObj,formObj,sAction) {
     switch(sAction) {
        case IBSEARCH:      //Retrieve
            ...
            if(validateForm(sheetObj,formObj,sAction))
                if ( sheetObj.id == "sheet1"){
                    ...
                    var rXml=sheetObj.GetSearchData("VOP_VSK_0043GS.do", sParam);
        ...
        case SEARCH01:  // Country Name Retrieve
            ...
            var sParam=FormQueryString(formObj);
            var sXml=sheetObj.GetSearchData("VOP_VSK_0043GS.do", sParam);
...
//	 alert(sheetObj.GetSelectRow());
//	 alert(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 2));
//	 alert(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'sheet1_loc_cd'));
    //window.returnValue=sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd');
    /*@@ComPopUpReturnValue(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd'));
    ComClosePopup();*/	
	 //ComPopUpReturnValue(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 2)); // loc_cd
 	 //var opener = window.dialogArguments;
 	 //if (!opener) opener = parent;
 	 ComPopUpReturnValue(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd'));
	 //ComClosePopup(); 
...
////	 comPopupOK();
//    //window.returnValue=sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'loc_cd');
////	 ComPopUpReturnValue(sheetObj.GetCellValue(sheetObj.GetSelectRow(), 'sheet1_loc_cd'));
//}//end sheet1_OnDblClick

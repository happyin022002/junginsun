...
<%
	...
	for ( Enumeration e = request.getParameterNames(); e.hasMoreElements(); ) {
		...
		if ( parameterNames.equals("pgmNo") ) pgmNo = StringUtil.xssFilter(request.getParameter(parameterNames));
		else if ( parameterNames.equals("pgmUrl") ) pgmUrl = StringUtil.xssFilter(request.getParameter(parameterNames));
		else if ( parameterNames.equals("parentPgmNo") ) parentPgmNo = StringUtil.xssFilter(request.getParameter(parameterNames));
		else redirectParameter += "&" + parameterNames + "=" + StringUtil.xssFilter(request.getParameter(parameterNames));
	}
...
//마우스 버튼 click 시 Bookmark div가 사라지게 한다.
 ...
 function changeBookmark(){
		...
		if(document.getElementById("imgBookmark").src.indexOf('img/new_sub/btn_plus.gif') >= 0){
			cmd = 1; // ADD
			var tmp =window.showModalDialog("/opuscntr/CREATEBOOKMARK.do?f_cmd=" + SEARCH01 + "&title=" + title, window, "scroll:no;status:no;help:no;dialogWidth:500px;dialogHeight:255px");
//			if ( tmp = '' ) return;
			...
			document.getElementById("bookMark").src = "/opuscntr/CREATEREMOVEBOOKMARK.do?ibflag=I&f_cmd="+cmd+"&pgm_no="+pgmNo+"&prnt_pgm_no=" + tmpArr[0] + "&dp_nm="+encodeURI(tmpArr[1]);
		}else{
			cmd = 5; // REMOVE
			document.getElementById("bookMark").src = "/opuscntr/CREATEREMOVEBOOKMARK.do?ibflag=D&f_cmd="+cmd+"&pgm_no="+pgmNo;
		}
...
//		alert('selLen='+selLen);
		...
		if(isAdded==true){
			img.src="/opuscntr/img/new_sub/btn_minus.gif";
			img.alt="Delete";
		}else{ 
			img.src="/opuscntr/img/new_sub/btn_plus.gif";
			img.alt="Add";
		}			
...
function callDisappearMenu2Flash(){
	...
	/**
  	...
  	*/
 	function dctionary(){
		ComOpenPopup('/opuscntr/Dctionary.do', 405, 230, 'Dctionary', "0,0", true);
	}
</script>
</head>
						...
						<tr>
                         ...
                         <script type="text/javascript">
	                         //window["opusMenu"] = new Object();
								var so = new SWFObject("/opuscntr/img/new_sub/OpusMenu.swf", "opusMenu", "990", "34", "8", "#ffffff");
...
<iframe id="ifmenu01" style="position: absolute; width: 780px; height: 272px; left: 215px; top: 26px; z-index: 3; visibility: hidden;" frameBorder="0" scrolling="no"></iframe>
...
<table width="780" style="border: #525267 1px solid;background-color: F4F4F4;border-top:1px solid #FFFFFF;" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<td valign="top" style="padding:3 0 3 0;">
			...
			<tr><td style="padding:0 0 11 0"><img src="img/new_sub/sub_menu_dot.gif" width="8" height="20" alt="" border="0"></td>
				<td class="full"><a href="javascript:menu_reolad('ESD_SPE_M001')">Service Provider Evaluation &amp;<br>Differentiation</a></td>
			</tr>
			</table>
		</td>
		<td bgcolor="D2D2D2"></td>
...
<iframe id="_iFrameWait_" src="/opuscntr/img/waiting.gif" frameborder="0" marginHeight="0" marginWidth="0" width="249" height="76" style="position:absolute;visibility:hidden;z-index:999;display:none;" ></iframe>
</body>
</html>

	    ...
	    if (bToken) {
			...
			if ("".equals(frm)) {
				...
				if (oFile.exists()) {
				      ...
				      if(L > 4000000){			 // 특정 사이즈 초과시에 alert창 호출 (3MB)
				      	 oFile.delete();		 // 파일 삭제
		   		      	 out.println("<script>parent.ComOpenWait(false);</script>");
		   		      	 out.println("<script>var targetWnd = null; if(opener!=null) {targetWnd = opener;} else {targetWnd = parent;} targetWnd.Grids[0].LoadExcelError({Code:-4,Msg:'Excel size cannot exceed 4MB'}); </script>");

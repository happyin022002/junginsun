...
//    	if( obj.value == -1) return "";
     ...
     *          false-허용된 문자 이외의 문자가 포함된 경우
     */
    function ComIsEmailAddr(obj, delimiter) {
    	var varDelimi = ";";
        ...
        try {
            ...
            var res = sVal.split(varDelimi);
            for(var i=0; i<res.length; i++) {
            	if(res[i].search(format) == -1) {
            		return false;
            	}
            }
    ...
    function ComTrunc(obj, pos)    {
        try {
            ...
            //첫번째 인자가 문자열 또는 HTML태그(Object)인 경우 처리
            var num = getArgValue(obj);
            var pointCount = ComPointCountLength(num);
            
            if (pos==undefined || pos==null ) pos = 2;

            var posNum = Math.pow(10, pos);
            num = num * posNum;
            //2009-11-10 ComTrunc("1.14",2) ==> 1.15가 되는 오류 수정
            if(pos == pointCount){

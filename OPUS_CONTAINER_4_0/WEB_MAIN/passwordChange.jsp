...
<%
    ...
    try {
    	...
    	if( StringUtil.xssFilter(request.getParameter("user_id")) !=null )
		user_id = StringUtil.xssFilter(request.getParameter("user_id"));
        ...
        function validateForm(){        
          ...
          **/
          ...
          /*   
             ...
             */ 
            var returnVal = false;  
       	if(str.match(/^((?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])|(?=.*[a-z])(?=.*[A-Z])(?=.*[!@\#$%&/=?_.,:;\\~\^\(\)\^\*\+\{\}\|\'\"\<\>\]\[\--])|(?=.*[a-z])(?=.*[0-9])(?=.*[!@\#$%&/=?_.,:;\\~\^\(\)\^\*\+\{\}\|\'\"\<\>\]\[\--])|(?=.*[A-Z])(?=.*[0-9])(?=.*[!@\#$%&/=?_.,:;\\~\^\(\)\^\*\+\{\}\|\'\"\<\>\]\[\--])).{8,}$/)

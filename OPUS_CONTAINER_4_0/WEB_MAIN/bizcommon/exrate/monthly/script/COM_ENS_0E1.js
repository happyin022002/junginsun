...
/* 업무별전역변수는 아래 부분에 추가 선언하여 사용한다. */
   ...
   //업무 자바스크립트 OnFocus 이벤트 처리
     function obj_activate() {
     	obj = event.srcElement;
 	  	//마스크 구분자 없애기
     	if(obj.name == 'frYearMon' || obj.name == 'toYearMon'){
     		if(document.form.frYearMon.value == ''){
     			document.form.frYearMon.value = getCurrDate('-');
     		}else if(document.form.toYearMon.value == ''){
     			document.form.toYearMon.value = getCurrDate('-');
     		}
     	}
        ...
        switch(sheetNo) {
            case 1:      //IBSheet1 init
                with (sheetObj) {
                    ...
                    var HeadTitle = "||Seq|MTH/Year|CUR|USD:LCL|LCL:KRW|USD:KRW" ;
                    ...
                    InitDataProperty(0, cnt++ , dtData,      70,    daCenter,  false,    "acct_xch_rt_yrmon",        false,          "",       dfNone,     0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,      70,    daCenter,  false,    "curr_cd",        false,          "",       dfNone,     0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,      70,    daLeft,  false,      "usd_locl_xch_rt",    false,          "",       dfNone,     0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,      70,    daCenter,  false,    "locl_krw_xch_rt",        false,          "",       dfNone,     0,     false,       true);
                    InitDataProperty(0, cnt++ , dtData,      70,    daCenter,  false,    "usd_krw_xch_rt",        false,          "",       dfNone,     0,     false,       true);
                }
                break;
        }
    }
    ...
    function doActionIBSheet(sheetObj,formObj,sAction, a, PageNo) {
        ...
        switch(sAction) {
          ...
          case IBSEARCH:        //조회
                ...
                sheetObj.DoSearch4Post("COM_ENS_0E1GS.do", selectVal);               
               
           break;
           case IBSEARCHAPPEND:  // 페이징 조회
                formObj.f_cmd.value = SEARCHLIST;  
                sheetObj.DoSearch4Post("COM_ENS_0E1GS.do", selectVal, "iPage=" + PageNo, true);
           break;
   ...
   /**
    ...
    function checkYyyyMonth(obj) {
        if(!onlyNumber(obj)) {
        	ComShowMessage("You must input only numeric value");
        }
    }
    
    /**
     * 현재일자를 구함
     * str : YYYY-MM형태의 일자를 리턴
     */

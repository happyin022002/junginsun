...
//            }
    ...
    function sheetDept_OnClick(sheetObj, row, col, value) {
      var formObject = document.form;
      formObject.ofc_cd_deptsrch.value = sheetObj.CellValue(sheetObj.SelectRow, 'ofc_cd');      
    ...
    function expandUserDept(sheetObj, usr_ofc_cd) {
    	...
    	do {
			sheetObj.RowExpanded(selRow) = true;
    ...
    function getUpperDept(sheetObj, selRow, rowLevel) {
    	...
    	for(var i=selRow-1; i>0; i--) {
    		if(sheetObj.RowLevel(i) == rowLevel - 1) {
    			upperDept = sheetObj.cellValue(i, "ofc_cd");
    			break;
    		}
    	}
    ...
    function addUser() {
        ...
        if ( selrow > 0 )
        {
            var usr_id = sheetObject1.CellValue(selrow, "usr_id");
            var usr_nm = sheetObject1.CellValue(selrow, "usr_nm");
            var ofc_cd = sheetObject1.CellValue(selrow, "ofc_cd");
            var title = sheetObject1.CellValue(selrow, "title");
    ...
    function delUser() {
        ...
        // Approval 완료 체크
        if(sheetObject2.CellValue(selrow, "apspo_cd") == "Approved"){
